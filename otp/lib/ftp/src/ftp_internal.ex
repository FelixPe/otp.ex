defmodule :m_ftp_internal do
  ...
  def user(pid, user, pass, account) do
    case {is_name_sane(user), is_name_sane(pass), is_name_sane(account)} do
      {true, true, true} ->
    ...
    case key_search(:host, opts, :undefined) do
      ...
      host ->
        ...
        case setup_ctrl_connection(host, port, timeout, state2) do
          ...
          when is_list(tLSOpts) and
                 tLSSecMethod == :ftps ->
            handle_ctrl_result(
              {:tls_upgrade, tLSSecMethod},
              r_state(state3,
                tls_options: tLSOpts,
  ...
  def handle_call({_, {:user, user, password}}, from, r_state(csock: cSock) = state)
    ...
    handle_user(user, password, ~c"", r_state(state, client: from))
  end
  ...
  def handle_call({_, :pwd}, from, r_state(chunk: false) = state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"PWD", []))
  ...
  def handle_call({_, {:cd, dir}}, from, r_state(chunk: false) = state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"CWD ~s", [dir]))
    state = activate_ctrl_connection(state0)
  ...
  def handle_call({_, {:rename, currFile, newFile}}, from, r_state(chunk: false) = state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"RNFR ~s", [currFile]))
    state = activate_ctrl_connection(state0)
  ...
  def handle_call({_, {:type, type}}, from, r_state(chunk: false) = state0) do
    case type do
      :ascii ->
        _ = send_ctrl_message(state0, mk_cmd(~c"TYPE A", []))
        state = activate_ctrl_connection(state0)
        {:noreply, r_state(state, caller: :type, type: :ascii, client: from)}

      :binary ->
        _ = send_ctrl_message(state0, mk_cmd(~c"TYPE I", []))
        state = activate_ctrl_connection(state0)
  ...
  def handle_call({_, {:recv_chunk_start, remoteFile}}, from, r_state(chunk: false) = state) do
    setup_data_connection(
      r_state(state,
        caller: {:start_chunk_transfer, ~c"RETR", remoteFile},
        client: from
      )
    )
  end
  ...
  def handle_call({_, :recv_chunk}, _, r_state(chunk: false) = state) do
    {:reply, {:error, ~c"ftp:recv_chunk_start/2 not called"}, state}
  end
  ...
  def handle_call(
    ...
    case bin do
      <<>> ->
        {:reply, :ok, r_state(state0, caller: :undefined, chunk: false, client: :undefined)}

      data ->
        {:reply, data, r_state(state0, caller: :undefined, chunk: false, client: :undefined)}
    end
  end
  ...
  def handle_call(
        ...
        r_state(
          chunk: true,
          caller: r_recv_chunk_closing(pos_compl_received: true) = r
        ) = state0
      ) do
  ...
  def handle_call(
    ...
    setup_data_connection(
      r_state(state,
        caller: {:transfer_file, {~c"STOR", localFile, remoteFile}},
        client: from
      )
    )
  end
  ...
  def handle_call({_, {:append, localFile, remoteFile}}, from, r_state(chunk: false) = state) do
    setup_data_connection(
      r_state(state,
        caller: {:transfer_file, {~c"APPE", localFile, remoteFile}},
        client: from
      )
    )
  end
  ...
  def handle_call({_, {:send_bin, bin, remoteFile}}, from, r_state(chunk: false) = state) do
    setup_data_connection(
      r_state(state,
        caller: {:transfer_data, {~c"STOR", bin, remoteFile}},
        client: from
      )
    )
  end
  ...
  def handle_call({_, {:append_bin, bin, remoteFile}}, from, r_state(chunk: false) = state) do
    setup_data_connection(
      r_state(state,
        caller: {:transfer_data, {~c"APPE", bin, remoteFile}},
        client: from
      )
    )
  end
  ...
  def handle_call({_, {:send_chunk_start, remoteFile}}, from, r_state(chunk: false) = state) do
    setup_data_connection(
      r_state(state,
        caller: {:start_chunk_transfer, ~c"STOR", remoteFile},
        client: from
      )
    )
  end
  ...
  def handle_call({_, {:append_chunk_start, remoteFile}}, from, r_state(chunk: false) = state) do
    setup_data_connection(
      r_state(state,
        caller: {:start_chunk_transfer, ~c"APPE", remoteFile},
        client: from
      )
    )
  end
  ...
  def handle_cast({pid, :close}, r_state(owner: pid) = state) do
    _ = send_ctrl_message(state, mk_cmd(~c"QUIT", []))
  ...
  def handle_cast({pid, :close}, state) do
    report =
      :io_lib.format(~c"A none owner process ~p tried to close an ftp connection: ~n", [pid])
  ...
  def handle_info(
    ...
    {:noreply, r_state(state, client: :undefined, caller: :undefined, data: <<>>)}
  end
  ...
  def handle_info({err, socket, reason}, _)
    ...
    report = :io_lib.format(~c"~p on socket: ~p  for reason: ~p~n", [err, socket, reason])
  ...
  def handle_info(
    ...
    report = :io_lib.format(~c"Progress reporting stopped for reason ~p~n", [reason])
    :error_logger.info_report(report)
  ...
  def handle_info(info, state) do
    report =
      :io_lib.format(~c"ftp : ~p : Unexpected message: ~p~nState: ~p~n", [self(), info, state])
  ...
  def terminate(reason, state) do
    report = :io_lib.format(~c"Ftp connection closed due to: ~p~n", [reason])
    :error_logger.error_report(report)
  ...
  def start_link(opts, genServerOptions) do
    case :lists.keysearch(:client, 1, opts) do
      {:value, _} ->
        :gen_server.start_link(:ftp_internal, opts, genServerOptions)

      false ->
        opts2 = [{:client, self()} | opts]
        :gen_server.start_link(:ftp_internal, opts2, genServerOptions)
    end
  end

  defp handle_user(user, password, acc, state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"USER ~s", [user]))
    state = activate_ctrl_connection(state0)
    {:noreply, r_state(state, caller: {:handle_user, password, acc})}
  end

  defp handle_user_passwd(password, acc, state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"PASS ~s", [password]))
    state = activate_ctrl_connection(state0)
    {:noreply, r_state(state, caller: {:handle_user_passwd, acc})}
  end

  defp handle_user_account(acc, state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"ACCT ~s", [acc]))
    state = activate_ctrl_connection(state0)
  ...
  defp handle_ctrl_result(
         {:pos_compl, _},
         r_state(csock: {:tcp, _Socket}, tls_options: tLSOptions, timeout: timeout, caller: :open) =
           state0
       )
       when is_list(tLSOptions) do
    _ = send_ctrl_message(state0, mk_cmd(~c"AUTH TLS", []))
    state = activate_ctrl_connection(state0)
  ...
  defp handle_ctrl_result(
    ...
    case :ssl.connect(socket, tLSOptions, timeout) do
      {:ok, tLSSocket} when s == :ftps ->
        ...
        state = activate_ctrl_connection(state1)
        {:noreply, r_state(state, tls_upgrading_data_connection: :pending), timeout}
  ...
  defp handle_ctrl_result(
         {:pos_compl, _},
         r_state(tls_upgrading_data_connection: :pending) = state0
       ) do
    _ = send_ctrl_message(state0, mk_cmd(~c"PBSZ 0", []))
    state = activate_ctrl_connection(state0)
  ...
  defp handle_ctrl_result(
    ...
    _ = send_ctrl_message(state0, mk_cmd(~c"PROT P", []))
    state = activate_ctrl_connection(state0)
  ...
  defp handle_ctrl_result(
         {:pos_compl, _},
         r_state(
           caller: {:handle_dir_result, ~c"" = _CurrentDir, data},
           client: from
         ) = state
       ) do
    :gen_server.reply(from, {:ok, data})
  ...
  defp handle_ctrl_result(
         {:pos_compl, _},
         r_state(
           caller: {:handle_dir_result, _Dir, data},
           client: from
         ) = state
       ) do
    :gen_server.reply(from, {:ok, data})
  ...
  defp handle_ctrl_result(
    ...
    _ = send_ctrl_message(state0, mk_cmd(~c"RNTO ~s", [newFile]))
    state = activate_ctrl_connection(state0)
  ...
  defp handle_ctrl_result(
         {:pos_prel, _},
         r_state(caller: :start_chunk_transfer) = state0
       ) do
    case accept_data_connection(state0) do
      {:ok, state1} ->
  ...
  defp handle_ctrl_result(
         {:pos_compl, _},
         r_state(caller: :undefined, chunk: true) = state0
       ) do
    {:noreply, r_state(state0, caller: r_recv_chunk_closing(pos_compl_received: true))}
  end
  ...
  defp handle_caller(r_state(caller: {:dir, dir, len}) = state0) do
    ...
    _ =
      case dir do
        ~c"" ->
          send_ctrl_message(state0, mk_cmd(cmd, ~c""))

        _ ->
          send_ctrl_message(state0, mk_cmd(cmd ++ ~c" ~s", [dir]))
      end
  ...
  defp handle_caller(r_state(caller: {:recv_bin, remoteFile}) = state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"RETR ~s", [remoteFile]))
    state = activate_ctrl_connection(state0)
  ...
  defp setup_data_connection(
    ...
    case (try do
      ...
      {:ok, {{_, _, _, _, _, _, _, _} = iP0, _}} ->
        ...
        cmd = mk_cmd(~c"EPRT |2|~s|~p|", [ipAddress, port])
      ...
      {:ok, {{_, _, _, _} = iP0, _}} ->
        ...
        _ =
          case ftpExt do
            ...
            true ->
              ipAddress = :inet_parse.ntoa(iP)
              cmd = mk_cmd(~c"EPRT |1|~s|~p|", [ipAddress, port])
  ...
  defp setup_data_connection(r_state(mode: :passive, ipfamily: :inet6, caller: caller) = state0) do
    _ = send_ctrl_message(state0, mk_cmd(~c"EPSV", []))
    state = activate_ctrl_connection(state0)
  ...
  defp setup_data_connection(
    ...
    _ = send_ctrl_message(state0, mk_cmd(~c"PASV", []))
    state = activate_ctrl_connection(state0)
    {:noreply, r_state(state, caller: {:setup_data_connection, caller})}
  end
  ...
  defp accept_data_connection_tls_options(
         r_state(csock: {:ssl, socket}, tls_options: tO0, tls_ctrl_session_reuse: true)
       ) do
    tO = :lists.keydelete(:reuse_sessions, 1, tO0)

    {:ok, [{:session_id, sSLSessionId}, {:session_data, sSLSessionData}]} =
  ...
  defp send_data_message(_S = r_state(dsock: socket), message) do
    ...
    case send_message(socket, message) do
      ...
      {:error, reason} ->
        report =
          :io_lib.format(~c"send/2 for socket ~p failed with reason ~p~n", [socket, reason])
  ...
  defp send_file(
    ...
    {:noreply,
     r_state(state, tls_upgrading_data_connection: {true, cTRL, :ftp_internal, :send_file, fd})}
  end
  ...
  defp send_bin(
    ...
    r_state(state, tls_upgrading_data_connection: {true, cTRL, :ftp_internal, :send_bin, bin})
  end
  ...
  defp start_chunk(r_state(tls_upgrading_data_connection: {true, cTRL, _}) = state) do
    r_state(state,
      tls_upgrading_data_connection: {true, cTRL, :ftp_internal, :start_chunk, :undefined}
    )
  end
  ...
  defp open_options(options) do
    ...
    validateTLSCtrlSessionReuse = fn
      reuse
