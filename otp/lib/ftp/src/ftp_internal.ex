defmodule :m_ftp_internal do
  use Bitwise
  import Kernel, except: [send: 2]
  @behaviour :gen_server
  require Record
  Record.defrecord(:r_state, :state, csock: :undefined,
                                 dsock: :undefined, tls_options: :undefined,
                                 verbose: false, ldir: :undefined,
                                 type: :ftp_server_default, chunk: false,
                                 mode: :passive, timeout: 60 * 1000, data: <<>>,
                                 ctrl_data: {<<>>, [], :start},
                                 latest_ctrl_response: '', owner: :undefined,
                                 client: :undefined, caller: :undefined,
                                 ipfamily: :undefined, sockopts_ctrl: [],
                                 sockopts_data_passive: [],
                                 sockopts_data_active: [], progress: :ignore,
                                 dtimeout: :infinity,
                                 tls_ctrl_session_reuse: false,
                                 tls_upgrading_data_connection: false,
                                 ftp_extension: false)
  Record.defrecord(:r_recv_chunk_closing, :recv_chunk_closing, dconn_closed: false,
                                              pos_compl_received: false,
                                              client_called_us: false)
  def start_service(options) do
    try do
      {:ok, startOptions} = start_options(options)
      case (:ftp_sup.start_child([[[{:client, self()} |
                                        startOptions],
                                       []]])) do
        {:ok, pid} ->
  ...
  def user(pid, user, pass, account) do
    case ({is_name_sane(user), is_name_sane(pass),
             is_name_sane(account)}) do
      {true, true, true} ->
  ...
  def append(pid, localFileName, remotFileName) do
    case (is_name_sane(remotFileName)) do
      true ->
        call(pid, {:append, localFileName, remotFileName},
               :atom)
      _ ->
        {:error, :efnamena}
    end
  ...
  def init(options) do
    ...
    {:value, {:client, client}} = :lists.keysearch(:client,
                                                     1, options)
    ...
    case (key_search(:debug, options, :disable)) do
      :trace ->
        :dbg.tracer()
        :dbg.p(:all, [:call])
        {:ok, _} = :dbg.tpl(:ftp_internal,
                              [{:_, [], [{:return_trace}]}])
        {:ok, _} = :dbg.tpl(:ftp_response,
                              [{:_, [], [{:return_trace}]}])
        {:ok, _} = :dbg.tpl(:ftp_progress,
                              [{:_, [], [{:return_trace}]}])
        :ok
      :debug ->
        :dbg.tracer()
        :dbg.p(:all, [:call])
        {:ok, _} = :dbg.tp(:ftp_internal,
                             [{:_, [], [{:return_trace}]}])
        {:ok, _} = :dbg.tp(:ftp_response,
                             [{:_, [], [{:return_trace}]}])
        {:ok, _} = :dbg.tp(:ftp_progress,
                             [{:_, [], [{:return_trace}]}])
        :ok
      _ ->
        :ok
    ...
    state = r_state(owner: client, verbose: verbose,
                ipfamily: ipFamily, ldir: dir)
    priority = key_search(:priority, options, :low)
    :erlang.process_flag(:priority, priority)
    {:ok, state}
  end

  def handle_call({_, :info}, _,
           r_state(verbose: verbose, mode: mode, timeout: timeout,
               ipfamily: ipFamily, csock: socket,
               progress: progress) = state) do
    {:ok, {_, localPort}} = sockname(socket)
    {:ok, {address, port}} = peername(socket)
    options = [{:verbose, verbose}, {:ipfamily, ipFamily},
                                        {:mode, mode}, {:peer, address},
                                                           {:peer_port, port},
                                                               {:local_port,
                                                                  localPort},
                                                                   {:timeout,
  ...
  def handle_call({_, :latest_ctrl_response}, _,
           r_state(latest_ctrl_response: resp) = state) do
    {:reply, {:ok, resp}, state}
  end
  ...
  def handle_call({_, {:open, :ip_comm, options}}, from, state) do
    ...
    case (key_search(:host, opts, :undefined)) do
      ...
      host ->
        ...
        {:ok,
           {ctrlOpts, dataPassOpts,
              dataActOpts}} = socket_options(options)
        state2 = r_state(state, client: from,  mode: mode, 
                            progress: progress(progress),  ipfamily: ipFamily, 
        ...
        case (setup_ctrl_connection(host, port, timeout,
                                      state2)) do
          {:ok, state3, waitTimeout} when (is_list(tLSOpts) and
                                             tLSSecMethod == :ftps)
                                          ->
            handle_ctrl_result({:tls_upgrade, tLSSecMethod},
                                 r_state(state3, tls_options: tLSOpts, 
  ...
  def handle_call({_, {:user, user, password}}, from,
           r_state(csock: cSock) = state)
      when cSock !== :undefined do
    handle_user(user, password, '', r_state(state, client: from))
  end

  def handle_call({_, {:user, user, password, acc}}, from,
           r_state(csock: cSock) = state)
      when cSock !== :undefined do
  ...
  def handle_call({_, {:cd, dir}}, from,
           r_state(chunk: false) = state0) do
    _ = send_ctrl_message(state0, mk_cmd('CWD ~s', [dir]))
    state = activate_ctrl_connection(state0)
    {:noreply, r_state(state, client: from,  caller: :cd)}
  end

  def handle_call({_, {:lcd, dir}}, _From,
           r_state(ldir: lDir0) = state) do
  ...
  def handle_call({_, {:dir, len, dir}}, {_Pid, _} = from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:dir, dir,
                                              len}, 
                                     client: from))
  end

  def handle_call({_, {:rename, currFile, newFile}}, from,
           r_state(chunk: false) = state0) do
    _ = send_ctrl_message(state0, mk_cmd('RNFR ~s', [currFile]))
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, caller: {:rename, newFile},  client: from)}
  end

  def handle_call({_, {:delete, file}}, {_Pid, _} = from,
           r_state(chunk: false) = state0) do
  ...
  def handle_call({_, {:mkdir, dir}}, from,
           r_state(chunk: false) = state0) do
  ...
  def handle_call({_, {:rmdir, dir}}, from,
           r_state(chunk: false) = state0) do
  ...
  def handle_call({_, {:type, type}}, from,
           r_state(chunk: false) = state0) do
    case (type) do
      :ascii ->
        ...
        state = activate_ctrl_connection(state0)
        {:noreply,
           r_state(state, caller: :type,  type: :ascii,  client: from)}
      :binary ->
        _ = send_ctrl_message(state0, mk_cmd('TYPE I', []))
        state = activate_ctrl_connection(state0)
        {:noreply,
           r_state(state, caller: :type,  type: :binary,  client: from)}
      _ ->
        {:reply, {:error, :etype}, state0}
    end
  end

  def handle_call({_, {:recv, remoteFile, localFile}}, from,
           r_state(chunk: false, ldir: localDir) = state) do
    ...
    case (file_open(newLocalFile, :write)) do
      {:ok, fd} ->
        setup_data_connection(r_state(state, client: from, 
  ...
  def handle_call({_, {:recv_bin, remoteFile}}, from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:recv_bin,
                                              remoteFile}, 
                                     client: from))
  end

  def handle_call({_, {:recv_chunk_start, remoteFile}}, from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:start_chunk_transfer,
                                              'RETR', remoteFile}, 
                                     client: from))
  end
  ...
  def handle_call({_, :recv_chunk}, _From,
           r_state(chunk: true, data: bin,
               caller: r_recv_chunk_closing(dconn_closed: true, pos_compl_received: true,
                           client_called_us: true)) = state0) do
    case (bin) do
      <<>> ->
        {:reply, :ok,
           r_state(state0, caller: :undefined,  chunk: false, 
                       client: :undefined)}
      data ->
        {:reply, data,
           r_state(state0, caller: :undefined,  chunk: false, 
  ...
  def handle_call({_, :recv_chunk}, _From,
           r_state(chunk: true,
               caller: r_recv_chunk_closing(dconn_closed: true,
                           pos_compl_received: true)) = state0) do
    :"n/a"
    state = activate_ctrl_connection(state0)
    {:reply, :ok,
       r_state(state, caller: :undefined,  chunk: false, 
                  client: :undefined)}
  end

  def handle_call({_, :recv_chunk}, from,
           r_state(chunk: true,
               caller: r_recv_chunk_closing(pos_compl_received: true) = r) = state0) do
    state = activate_data_connection(state0)
    {:noreply,
       r_state(state, client: from, 
                  caller: r_recv_chunk_closing(r, client_called_us: true))}
  end

  def handle_call({_, :recv_chunk}, from,
           r_state(chunk: true, caller: r_recv_chunk_closing() = r) = state) do
    :"n/a"
    {:noreply,
       r_state(state, client: from, 
                  caller: r_recv_chunk_closing(r, client_called_us: true))}
  ...
  def handle_call({_, :recv_chunk}, from,
           r_state(chunk: true) = state0) do
    state = activate_data_connection(state0)
    {:noreply, r_state(state, client: from,  caller: :recv_chunk)}
  end

  def handle_call({_, {:send, localFile, remoteFile}}, from,
           r_state(chunk: false, ldir: localDir) = state) do
    progress_report({:local_file,
                       :filename.absname(localFile, localDir)},
                      state)
    setup_data_connection(r_state(state, caller: {:transfer_file,
                                              {'STOR', localFile, remoteFile}}, 
                                     client: from))
  end

  def handle_call({_, {:append, localFile, remoteFile}}, from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:transfer_file,
  ...
  def handle_call({_, {:send_bin, bin, remoteFile}}, from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:transfer_data,
                                              {'STOR', bin, remoteFile}}, 
                                     client: from))
  end

  def handle_call({_, {:append_bin, bin, remoteFile}}, from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:transfer_data,
                                              {'APPE', bin, remoteFile}}, 
                                     client: from))
  ...
  def handle_call({_, {:send_chunk_start, remoteFile}}, from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:start_chunk_transfer,
                                              'STOR', remoteFile}, 
                                     client: from))
  ...
  def handle_call({_, {:append_chunk_start, remoteFile}}, from,
           r_state(chunk: false) = state) do
    setup_data_connection(r_state(state, caller: {:start_chunk_transfer,
                                              'APPE', remoteFile}, 
                                     client: from))
  ...
  def handle_call({_, {:transfer_chunk, bin}}, _,
           r_state(chunk: true) = state) do
    send_data_message(state, bin)
    {:reply, :ok, state}
  end

  def handle_call({_, {:transfer_chunk, _}}, _,
           r_state(chunk: false) = state) do
    {:reply, {:error, :echunk}, state}
  end

  def handle_call({_, :chunk_end}, from,
           r_state(chunk: true) = state0) do
    close_data_connection(state0)
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, client: from,  dsock: :undefined, 
  ...
  def handle_call({_, {:quote, cmd}}, from,
           r_state(chunk: false) = state0) do
  ...
  def handle_cast({pid, :close}, r_state(owner: pid) = state) do
    ...
    {:stop, :normal,
       r_state(state, csock: :undefined,  dsock: :undefined)}
  end
  ...
  def handle_info({trpt, socket, data},
           r_state(dsock: {trpt, socket},
  ...
  def handle_info({trpt, socket, data},
           r_state(dsock: {trpt, socket}, client: from,
               caller: :recv_chunk) = state)
      when trpt == :tcp or trpt == :ssl do
    :"n/a"
    :gen_server.reply(from, {:ok, data})
    {:noreply,
       r_state(state, client: :undefined,  caller: :undefined, 
                  data: <<>>)}
  end
  ...
  def handle_info({trpt, socket, data},
    ...
    state = activate_data_connection(state0)
    {:noreply,
       r_state(state, data: <<r_state(state, :data) :: binary,
                          data :: binary>>)}
  end

  def handle_info({cls, socket},
           r_state(dsock: {trpt, socket},
               caller: {:recv_file, fd}) = state0)
  ...
  def handle_info({cls, socket},
           r_state(dsock: {trpt, socket}, client: client,
  ...
  def handle_info({cls, socket},
           r_state(dsock: {trpt, socket},
               caller: r_recv_chunk_closing(client_called_us: true,
                           pos_compl_received: true) = r) = state)
      when {cls, trpt} == {:tcp_closed, :tcp} or
             {cls, trpt} == {:ssl_closed, :ssl} do
    send(self(), {cls, socket})
    {:noreply, r_state(state, caller: r_recv_chunk_closing(r, dconn_closed: true))}
  end

  def handle_info({cls, socket},
           r_state(dsock: {trpt, socket}, caller: :recv_bin,
               data: data) = state0)
      when {cls, trpt} == {:tcp_closed, :tcp} or
             {cls, trpt} == {:ssl_closed, :ssl} do
    :"n/a"
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, dsock: :undefined,  data: <<>>, 
                  caller: {:recv_bin, data})}
  end

  def handle_info({cls, socket},
           r_state(dsock: {trpt, socket}, data: data,
               caller: {:handle_dir_result, dir}) = state0)
      when {cls, trpt} == {:tcp_closed, :tcp} or
             {cls, trpt} == {:ssl_closed, :ssl} do
    :"n/a"
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, dsock: :undefined, 
                  caller: {:handle_dir_result, dir, data},  data: <<>>)}
  end

  def handle_info({err, socket, reason},
    ...
    close_data_connection(state)
    {:noreply,
       r_state(state, dsock: :undefined,  client: :undefined, 
                  data: <<>>,  caller: :undefined,  chunk: false)}
  end

  def handle_info({transport, socket, data},
           r_state(csock: {transport, socket}, verbose: verbose,
               caller: caller, client: from,
               ctrl_data: {binCtrlData, accLines,
                             lineStatus}) = state0) do
    :"n/a"
    case (:ftp_response.parse_lines(<<binCtrlData :: binary,
                                        data :: binary>>,
                                      accLines, lineStatus)) do
      {:ok, lines, nextMsgData} ->
        ...
        case (caller) do
          :quote ->
            ...
            {:noreply,
               r_state(state0, client: :undefined,  caller: :undefined, 
                           latest_ctrl_response: lines, 
                           ctrl_data: {nextMsgData, [], :start})}
          _ ->
            :"n/a"
            handle_ctrl_result(ctrlResult,
                                 r_state(state0, latest_ctrl_response: lines, 
  ...
  def handle_info({:DOWN, _Ref, _Type, _Process, :normal},
           state) do
    {:stop, :normal, r_state(state, client: :undefined)}
  end

  def handle_info({:DOWN, _Ref, _Type, _Process, :shutdown},
           state) do
    {:stop, :normal, r_state(state, client: :undefined)}
  end

  def handle_info({:DOWN, _Ref, _Type, _Process, :timeout},
           state) do
    {:stop, :normal, r_state(state, client: :undefined)}
  end
  ...
  def handle_info({:DOWN, _Ref, _Type, process, reason}, state) do
    {:stop, {:stopped, {:EXIT, process, reason}},
       r_state(state, client: :undefined)}
  end

  def handle_info({:EXIT, pid, reason},
           r_state(progress: pid) = state) do
  ...
  def handle_info(info, state) do
    report = :io_lib.format('ftp : ~p : Unexpected message: ~p~nState: ~p~n', [self(), info, state])
    :error_logger.info_report(report)
    {:noreply, state}
  end
  ...
  def code_change(_Vsn, state1, :upgrade_from_pre_5_12) do
    {:state, cSock, dSock, verbose, lDir, type, chunk, mode,
       timeout, data, ctrlData, owner, client, caller,
       iPv6Disable, progress} = state1
    ipFamily = (cond do
                  iPv6Disable === true ->
                    :inet
                  true ->
    ...
    state2 = r_state(csock: cSock, dsock: dSock, verbose: verbose,
                 ldir: lDir, type: type, chunk: chunk, mode: mode,
                 timeout: timeout, data: data, ctrl_data: ctrlData,
                 owner: owner, client: client, caller: caller,
                 ipfamily: ipFamily, progress: progress)
    {:ok, state2}
  end

  def code_change(_Vsn, state1, :downgrade_to_pre_5_12) do
    r_state(csock: cSock, dsock: dSock, verbose: verbose,
        ldir: lDir, type: type, chunk: chunk, mode: mode,
        timeout: timeout, data: data, ctrl_data: ctrlData,
        owner: owner, client: client, caller: caller,
        ipfamily: ipFamily, progress: progress) = state1
    iPv6Disable = (cond do
                     ipFamily === :inet ->
                       true
                     true ->
    ...
    state2 = {:state, cSock, dSock, verbose, lDir, type,
                chunk, mode, timeout, data, ctrlData, owner, client,
                caller, iPv6Disable, progress}
    {:ok, state2}
  end
  ...
  def start_link(opts, genServerOptions) do
    case (:lists.keysearch(:client, 1, opts)) do
      {:value, _} ->
        :gen_server.start_link(:ftp_internal, opts,
                                 genServerOptions)
      false ->
        opts2 = [{:client, self()} | opts]
        :gen_server.start_link(:ftp_internal, opts2,
                                 genServerOptions)
    end
  end
  ...
  defp handle_user(user, password, acc, state0) do
    ...
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, caller: {:handle_user, password, acc})}
  end
  ...
  defp handle_ctrl_result({:pos_compl, _},
            r_state(csock: {:tcp, _Socket}, tls_options: tLSOptions,
                timeout: timeout, caller: :open) = state0)
  ...
  defp handle_ctrl_result({:tls_upgrade, s},
            r_state(csock: {:tcp, socket}, tls_options: tLSOptions,
                timeout: timeout, caller: :open, client: from) = state0)
      when is_list(tLSOptions) do
    :"n/a"
    (try do
      :ssl.start()
    ...
    case (:ssl.connect(socket, tLSOptions, timeout)) do
      {:ok, tLSSocket} when s == :ftps ->
        ...
        state = activate_ctrl_connection(state1)
        {:noreply,
           r_state(state, tls_upgrading_data_connection: :pending),
           timeout}
      {:ok, tLSSocket} ->
        state1 = r_state(state0, csock: {:ssl, tLSSocket})
        handle_ctrl_result({:pos_compl, s},
                             r_state(state1, tls_upgrading_data_connection: :pending))
      {:error, _} = error ->
  ...
  defp handle_ctrl_result({:pos_compl, _},
            r_state(tls_upgrading_data_connection: :pending) = state0) do
    _ = send_ctrl_message(state0, mk_cmd('PBSZ 0', []))
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, tls_upgrading_data_connection: {true, :pbsz})}
  end

  defp handle_ctrl_result({:pos_compl, _},
            r_state(tls_upgrading_data_connection: {true,
                                                :pbsz}) = state0) do
    _ = send_ctrl_message(state0, mk_cmd('PROT P', []))
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, tls_upgrading_data_connection: {true, :prot})}
  end

  defp handle_ctrl_result({:pos_compl, _},
            r_state(tls_upgrading_data_connection: {true, :prot},
                client: from) = state) do
  ...
  defp handle_ctrl_result({:pos_compl, _},
            r_state(caller: :open, client: from) = state) do
    :gen_server.reply(from, {:ok, self()})
    {:noreply,
       r_state(state, client: :undefined,  caller: :undefined)}
  end
  ...
  defp handle_ctrl_result({:pos_compl, _Lines},
            r_state(mode: :active,
                caller: {:setup_data_connection,
                           {lSock, caller}}) = state) do
    handle_caller(r_state(state, caller: caller, 
                             dsock: {:lsock, lSock}))
  end

  defp handle_ctrl_result({status, _Lines},
            r_state(mode: :active,
                caller: {:setup_data_connection,
                           {lSock, _}}) = state) do
  ...
  defp handle_ctrl_result({:pos_compl, lines},
            r_state(mode: :passive, ipfamily: :inet6, client: from,
                caller: {:setup_data_connection, caller}, csock: cSock,
    ...
    case (connect(iP, :erlang.list_to_integer(portStr),
                    sockOpts, timeout, state)) do
      {:ok, _, socket} ->
        handle_caller(r_state(state, caller: caller, 
                                 dsock: {:tcp, socket}))
      {:error, _Reason} = error ->
  ...
  defp handle_ctrl_result({:pos_compl, lines},
            r_state(mode: :passive, ipfamily: :inet, client: from,
                caller: {:setup_data_connection, caller},
                timeout: timeout, sockopts_data_passive: sockOpts,
                ftp_extension: false) = state)
      when is_list(lines) do
    {_, [?( | rest]} = :lists.splitwith(fn ?( ->
                                             false
                                           _ ->
                                             true
                                        end,
                                          lines)
    {newPortAddr, _} = :lists.splitwith(fn ?) ->
                                             false
                                           _ ->
                                             true
                                        end,
                                          rest)
    [a1, a2, a3, a4, p1, p2] = :lists.map(fn x ->
  ...
  defp handle_ctrl_result(_,
            r_state(mode: :passive,
                caller: {:setup_data_connection, caller}) = state) do
    setup_data_connection(r_state(state, mode: :active, 
                                     caller: caller))
  end

  defp handle_ctrl_result({:pos_interm, _},
            r_state(caller: {:handle_user, passWord, acc}) = state) do
    handle_user_passwd(passWord, acc, state)
  end

  defp handle_ctrl_result({status, _},
            r_state(caller: {:handle_user, _, _}) = state) do
    ctrl_result_response(status, state, {:error, :euser})
  end

  defp handle_ctrl_result({:pos_interm_acct, _},
  ...
  defp handle_ctrl_result({status, _},
            r_state(caller: {:handle_user_passwd, _}) = state) do
    ctrl_result_response(status, state, {:error, :euser})
  end

  defp handle_ctrl_result({:pos_compl, lines},
            r_state(caller: :pwd, client: from) = state) do
  ...
  defp handle_ctrl_result({:pos_prel, _},
            r_state(caller: {:dir, dir}) = state0) do
  ...
  defp handle_ctrl_result({:pos_compl, _},
            r_state(caller: {:handle_dir_result, '' = _CurrentDir, data},
                client: from) = state) do
    :gen_server.reply(from, {:ok, data})
    {:noreply,
       r_state(state, client: :undefined,  caller: :undefined)}
  ...
  defp handle_ctrl_result({:pos_compl, _},
            r_state(caller: {:handle_dir_result, _Dir, data},
                client: from) = state) do
    :gen_server.reply(from, {:ok, data})
    {:noreply,
       r_state(state, client: :undefined,  caller: :undefined)}
  ...
  defp handle_ctrl_result({:pos_compl, _} = operation,
            r_state(caller: {:handle_dir_result, dir},
                data: data) = state) do
    handle_ctrl_result(operation,
                         r_state(state, caller: {:handle_dir_result, dir, data}))
  end

  defp handle_ctrl_result(s = {_Status, _},
            r_state(caller: {:handle_dir_result, _, _}) = state) do
  ...
  defp handle_ctrl_result(status = {:epath, _},
            r_state(caller: {:dir, _}) = state) do
    ctrl_result_response(status, state, {:error, :epath})
  end

  defp handle_ctrl_result({:pos_interm, _},
            r_state(caller: {:rename, newFile}) = state0) do
  ...
  defp handle_ctrl_result({status, _},
            r_state(caller: :rename_second_phase) = state) do
    ctrl_result_response(status, state, {:error, status})
  end

  defp handle_ctrl_result({:pos_prel, _},
            r_state(caller: :recv_bin) = state0) do
    case (accept_data_connection(state0)) do
      {:ok, state1} ->
        state = activate_data_connection(state1)
        {:noreply, state}
  ...
  defp handle_ctrl_result({:pos_compl, _},
            r_state(caller: {:recv_bin, data}, client: from) = state) do
    :gen_server.reply(from, {:ok, data})
    close_data_connection(state)
    {:noreply,
       r_state(state, client: :undefined,  caller: :undefined)}
  ...
  defp handle_ctrl_result({status, _}, r_state(caller: :recv_bin) = state) do
    ...
    ctrl_result_response(status,
                           r_state(state, dsock: :undefined), {:error, :epath})
  end

  defp handle_ctrl_result({status, _},
            r_state(caller: {:recv_bin, _}) = state) do
    close_data_connection(state)
    ctrl_result_response(status,
                           r_state(state, dsock: :undefined), {:error, :epath})
  end
  ...
  defp handle_ctrl_result({:pos_prel, _},
            r_state(caller: :start_chunk_transfer) = state0) do
    case (accept_data_connection(state0)) do
      {:ok, state1} ->
  ...
  defp handle_ctrl_result({:pos_compl, _},
            r_state(client: from,
                caller: r_recv_chunk_closing(dconn_closed: true, client_called_us: true,
                            pos_compl_received: false)) = state0)
  ...
  defp handle_ctrl_result({:pos_compl, _}, r_state(caller: r_recv_chunk_closing() = r) = state0) do
    :"n/a"
    {:noreply,
       r_state(state0, caller: r_recv_chunk_closing(r, pos_compl_received: true))}
  end

  defp handle_ctrl_result({:pos_compl, _},
            r_state(caller: :undefined, chunk: true) = state0) do
    {:noreply,
       r_state(state0, caller: r_recv_chunk_closing(pos_compl_received: true))}
  end

  defp handle_ctrl_result({:pos_prel, _},
            r_state(caller: {:recv_file, _}) = state0) do
    case (accept_data_connection(state0)) do
      {:ok, state1} ->
        state = activate_data_connection(state1)
        {:noreply, state}
  ...
  defp handle_ctrl_result({status, _},
            r_state(caller: {:recv_file, fd}) = state) do
    file_close(fd)
    close_data_connection(state)
    ctrl_result_response(status,
                           r_state(state, dsock: :undefined), {:error, :epath})
  ...
  defp handle_ctrl_result({:pos_prel, _},
            r_state(caller: {:transfer_file, fd}) = state0) do
    case (accept_data_connection(state0)) do
      {:ok, state1} ->
        send_file(state1, fd)
      {:error, _Reason} = error ->
  ...
  defp handle_ctrl_result({:pos_prel, _},
            r_state(caller: {:transfer_data, bin}) = state0) do
    case (accept_data_connection(state0)) do
      {:ok, state} ->
        send_bin(state, bin)
      {:error, _Reason} = error ->
  ...
  defp handle_ctrl_result(ctrlMsg, r_state(caller: :undefined) = state) do
    :logger.log(:info,
                  %{protocol: :ftp, unexpected_msg: ctrlMsg})
    {:noreply, state}
  end
  ...
  defp ctrl_result_response(:error, state0, {:error, _Reason} = error) do
  ...
  defp ctrl_result_response(status, r_state(client: from) = state, _)
      when status === :etnospc or status === :epnospc or status === :efnamena or status === :econn do
  ...
  defp handle_caller(r_state(caller: {:start_chunk_transfer, cmd,
                       remoteFile}) = state0) do
    _ = send_ctrl_message(state0,
                            mk_cmd('~s ~s', [cmd, remoteFile]))
    state = activate_ctrl_connection(state0)
    {:noreply, r_state(state, caller: :start_chunk_transfer)}
  end

  defp handle_caller(r_state(caller: {:recv_file, remoteFile,
                       fd}) = state0) do
    _ = send_ctrl_message(state0, mk_cmd('RETR ~s', [remoteFile]))
    state = activate_ctrl_connection(state0)
  ...
  defp handle_caller(r_state(caller: {:transfer_file,
                       {cmd, localFile, remoteFile}},
              ldir: localDir, client: from) = state0)
      when is_binary(localFile) or is_list(localFile) or is_atom(localFile) do
    case (file_open(:filename.absname(localFile, localDir),
                      :read)) do
      {:ok, fd} ->
        _ = send_ctrl_message(state0,
                                mk_cmd('~s ~s', [cmd, remoteFile]))
  ...
  defp handle_caller(r_state(caller: {:transfer_data,
                       {cmd, bin, remoteFile}}) = state0) do
    _ = send_ctrl_message(state0,
                            mk_cmd('~s ~s', [cmd, remoteFile]))
    state = activate_ctrl_connection(state0)
  ...
  defp setup_ctrl_connection(host, port, timeout,
            r_state(sockopts_ctrl: sockOpts) = state0) do
    msTime = :erlang.monotonic_time()
    case (connect(host, port, sockOpts, timeout, state0)) do
      {:ok, ipFam, cSock} ->
        state1 = r_state(state0, csock: {:tcp, cSock}, 
                             ipfamily: ipFam)
        state = activate_ctrl_connection(state1)
  ...
  defp setup_data_connection(r_state(mode: :active, caller: caller, csock: cSock,
              sockopts_data_active: sockOpts,
              ftp_extension: ftpExt) = state0) do
    case ((try do
            sockname(cSock)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
      ...
      {:ok, {{_, _, _, _, _, _, _, _} = iP0, _}} ->
        ...
        state = activate_ctrl_connection(state0)
        {:noreply,
           r_state(state, caller: {:setup_data_connection,
                               {lSock, caller}})}
      {:ok, {{_, _, _, _} = iP0, _}} ->
        iP = :proplists.get_value(:ip, sockOpts, iP0)
        {:ok, lSock} = :gen_tcp.listen(0,
  ...
  defp setup_data_connection(r_state(mode: :passive, ipfamily: :inet6,
              caller: caller) = state0) do
    _ = send_ctrl_message(state0, mk_cmd('EPSV', []))
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, caller: {:setup_data_connection, caller})}
  end

  defp setup_data_connection(r_state(mode: :passive, ipfamily: :inet,
              caller: caller, ftp_extension: false) = state0) do
  ...
  defp setup_data_connection(r_state(mode: :passive, ipfamily: :inet,
              caller: caller, ftp_extension: true) = state0) do
    _ = send_ctrl_message(state0, mk_cmd('EPSV', []))
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, caller: {:setup_data_connection, caller})}
  ...
  defp connect(host, port, sockOpts, timeout,
            r_state(ipfamily: :inet = ipFam)) do
    connect2(host, port, ipFam, sockOpts, timeout)
  end

  defp connect(host, port, sockOpts, timeout,
            r_state(ipfamily: :inet6 = ipFam)) do
    connect2(host, port, ipFam, sockOpts, timeout)
  end

  defp connect(host, port, sockOpts, timeout,
            r_state(ipfamily: :inet6fb4)) do
  ...
  defp accept_data_connection_tls_options(r_state(csock: {:ssl, socket}, tls_options: tO0,
              tls_ctrl_session_reuse: true)) do
    tO = :lists.keydelete(:reuse_sessions, 1, tO0)
    {:ok,
       [{:session_id, sSLSessionId}, {:session_data,
                                        sSLSessionData}]} = :ssl.connection_information(socket,
                                                                                          [:session_id,
                                                                                               :session_data])
    :lists.keystore(:reuse_session, 1, tO,
                      {:reuse_session, {sSLSessionId, sSLSessionData}})
  end

  defp accept_data_connection_tls_options(r_state(tls_options: tO)) do
    tO
  end

  defp accept_data_connection(r_state(mode: :active, dtimeout: dTimeout,
              tls_options: tLSOptions0,
              dsock: {:lsock, lSock}) = state0) do
  ...
  defp accept_data_connection(r_state(mode: :passive, dtimeout: dTimeout,
              dsock: {:tcp, socket},
              tls_options: tLSOptions0) = state)
      when is_list(tLSOptions0) do
  ...
  defp send_ctrl_message(_S = r_state(csock: socket, verbose: verbose),
            message) do
  ...
  defp activate_ctrl_connection(r_state(csock: cSock,
              ctrl_data: {<<>>, _, _}) = state) do
  ...
  defp activate_ctrl_connection(r_state(csock: cSock) = state0) do
    _ = activate_connection(cSock)
    {:noreply, state} = handle_info({socket_type(cSock),
                                       unwrap_socket(cSock), <<>>},
                                      state0)
    state
  end
  ...
  defp send_file(r_state(tls_upgrading_data_connection: {true, cTRL,
                                              _}) = state,
            fd) do
    {:noreply,
       r_state(state, tls_upgrading_data_connection: {true, cTRL,
                                                  :ftp_internal, :send_file,
                                                  fd})}
  end

  defp send_file(r_state(client: client) = state0, fd) do
    case (file_read(fd)) do
      ...
      {:ok, _, _} ->
        ...
        state = activate_ctrl_connection(state0)
        {:noreply,
           r_state(state, caller: :transfer_file_second_phase, 
                      dsock: :undefined)}
      {:error, reason} ->
  ...
  defp send_bin(r_state(tls_upgrading_data_connection: {true, cTRL,
                                              _}) = state,
            bin) do
    r_state(state, tls_upgrading_data_connection: {true, cTRL,
                                               :ftp_internal, :send_bin, bin})
  end
  ...
  defp send_bin(state0, bin) do
    ...
    state = activate_ctrl_connection(state0)
    {:noreply,
       r_state(state, caller: :transfer_data_second_phase, 
                  dsock: :undefined)}
  end
  ...
  defp progress_report(:stop, r_state(progress: progressPid))
      when is_pid(progressPid) do
    :ftp_progress.stop(progressPid)
  end

  defp progress_report({:binary, data}, r_state(progress: progressPid))
      when (is_binary(data) and is_pid(progressPid)) do
    :ftp_progress.report(progressPid,
                           {:transfer_size, byte_size(data)})
  end

  defp progress_report(report, r_state(progress: progressPid))
      when is_pid(progressPid) do
  ...
  defp start_chunk(r_state(tls_upgrading_data_connection: {true, cTRL,
                                              _}) = state) do
    r_state(state, tls_upgrading_data_connection: {true, cTRL,
                                               :ftp_internal, :start_chunk,
                                               :undefined})
  end

  defp start_chunk(r_state(client: from) = state) do
    :gen_server.reply(from, :ok)
    r_state(state, chunk: true,  client: :undefined, 
               caller: :undefined)
  end
  ...
  defp start_options(options) do
    case (:lists.keysearch(:flags, 1, options)) do
      {:value, {:flags, flags}} ->
        ...
        {:ok,
           [{:verbose, verbose}, {:debug, debugLevel}, {:priority,
                                                          :low}]}
      false ->
        validateVerbose = fn true ->
                               true
                             false ->
                               true
                             _ ->
        ...
        validOptions = [{:verbose, validateVerbose, false,
                           false},
  ...
  defp open_options(options) do
    ...
    validateHost = fn host when is_list(host) ->
                        true
                      host when tuple_size(host) === 4 or
                                  tuple_size(host) === 8
                                ->
                        true
                      _ ->
    ...
    validatePort = fn port
                          when is_integer(port) and port >= 0 ->
                        true
                      _ ->
                        false
                   end
    validateIpFamily = fn :inet ->
                            true
                          :inet6 ->
                            true
                          :inet6fb4 ->
    ...
    validateTLS = fn tLS when is_list(tLS) ->
                       true
                     :undefined ->
                       true
                     _ ->
    ...
    validateTLSCtrlSessionReuse = fn reuse
                                         when is_boolean(reuse) ->
                                       true
                                     _ ->
                                       false
                                  end
    validateTimeout = fn timeout
                             when is_integer(timeout) and timeout >= 0 ->
                           true
                         _ ->
                           false
                      end
    validateDTimeout = fn dTimeout
    ...
    validOptions = [{:mode, validateMode, false, :passive},
                        {:host, validateHost, true, :ehost}, {:port,
                                                                validatePort,
                                                                false, 0},
                                                                 {:ipfamily,
                                                                    validateIpFamily,
  ...
  defp socket_options(options) do
    ...
    dataActOpts = :proplists.get_value(:sock_data_act,
                                         options, ctrlOpts)
    dataPassOpts = :proplists.get_value(:sock_data_pass,
                                          options, ctrlOpts)
  ...
  defp validate_options([], validOptions, acc) do
    case (for {key, _, true, reason} <- validOptions do
      ...
      [] ->
        defaults = (for {key, _, _, default} <- validOptions do
                      {key, default}
                    end)
  ...
  defp millisec_passed(t0) do
    div(:erlang.convert_time_unit(:erlang.monotonic_time() - t0,
                                    :native, :micro_seconds), 1000)
  end
