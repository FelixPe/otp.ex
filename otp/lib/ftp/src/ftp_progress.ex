defmodule :m_ftp_progress do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_progress, :progress, file: :undefined,
                                    cb_module: :undefined,
  ...
  defp progress({cBModule, cBFunction, initProgressTerm})
    ...
    r_progress(cb_module: cBModule, cb_function: cBFunction,
  ...
  defp report_progress({:local_file, file}, progress) do
    ...
    report_progress({:file_size, r_file_info(fileInfo, :size)},
                      r_progress(progress, file: file))
  end

  defp report_progress({:remote_file, file}, progress) do
    report_progress({:file_size, :unknown},
                      r_progress(progress, file: file))
  end

  defp report_progress(size,
            r_progress(file: file, cb_module: cBModule,
                cb_function: cBFunction, current_progress_term: term,
                init_progress_term: initTerm) = progress) do
    newProgressTerm = apply(cBModule, cBFunction,
                              [term, file, size])
    case (size) do
      {:transfer_size, 0} ->
        r_progress(progress, current_progress_term: initTerm, 
                      file: :undefined)
      _ ->
