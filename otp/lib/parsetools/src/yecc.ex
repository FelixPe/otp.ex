defmodule :m_yecc do
  ...
  def format_error(:bad_declaration) do
    :io_lib.fwrite(~c"unknown or bad declaration, ignored", [])
  end

  def format_error({:bad_expect, symName}) do
    :io_lib.fwrite(~c"argument ~ts of Expect is not an integer", [format_symbol(symName)])
  end

  def format_error({:bad_rootsymbol, symName}) do
    :io_lib.fwrite(~c"rootsymbol ~ts is not a nonterminal", [format_symbol(symName)])
  end

  def format_error({:bad_states, symName}) do
    :io_lib.fwrite(~c"argument ~ts of States is not an integer", [format_symbol(symName)])
  end
  ...
  def format_error({:conflicts, sR, rR}) do
    :io_lib.fwrite(~c"conflicts: ~w shift/reduce, ~w reduce/reduce", [sR, rR])
  end

  def format_error({:duplicate_declaration, tag}) do
    :io_lib.fwrite(~c"duplicate declaration of ~s", [:erlang.atom_to_list(tag)])
  end

  def format_error({:duplicate_nonterminal, nonterminal}) do
    :io_lib.fwrite(~c"duplicate non-terminals ~ts", [format_symbol(nonterminal)])
  end

  def format_error({:duplicate_precedence, op}) do
    :io_lib.fwrite(~c"duplicate precedence operator ~ts", [format_symbol(op)])
  end

  def format_error({:duplicate_terminal, terminal}) do
    :io_lib.fwrite(~c"duplicate terminal ~ts", [format_symbol(terminal)])
  end

  def format_error({:endsymbol_is_nonterminal, symbol}) do
    :io_lib.fwrite(~c"endsymbol ~ts is a nonterminal", [format_symbol(symbol)])
  end

  def format_error({:endsymbol_is_terminal, symbol}) do
    :io_lib.fwrite(~c"endsymbol ~ts is a terminal", [format_symbol(symbol)])
  end
  ...
  def format_error(:illegal_empty) do
    :io_lib.fwrite(~c"illegal use of empty symbol", [])
  end

  def format_error({:internal_error, error}) do
    :io_lib.fwrite(~c"internal yecc error: ~w", [error])
  end

  def format_error({:missing_syntax_rule, nonterminal}) do
    :io_lib.fwrite(~c"no syntax rule for non-terminal symbol ~ts", [format_symbol(nonterminal)])
  end

  def format_error({:n_states, exp, n}) do
    :io_lib.fwrite(~c"expected ~w states, but got ~p states", [exp, n])
  end

  def format_error(:no_grammar_rules) do
    :io_lib.fwrite(~c"grammar rules are missing", [])
  end

  def format_error(:nonterminals_missing) do
    :io_lib.fwrite(~c"Nonterminals is missing", [])
  end

  def format_error({:precedence_op_is_endsymbol, symName}) do
    :io_lib.fwrite(~c"precedence operator ~ts is endsymbol", [format_symbol(symName)])
  end

  def format_error({:precedence_op_is_unknown, symName}) do
    :io_lib.fwrite(~c"unknown precedence operator ~ts", [format_symbol(symName)])
  end

  def format_error({:reserved, n}) do
    :io_lib.fwrite(~c"the use of ~w should be avoided", [n])
  end

  def format_error({:symbol_terminal_and_nonterminal, symName}) do
    :io_lib.fwrite(~c"symbol ~ts is both a terminal and nonterminal", [format_symbol(symName)])
  end

  def format_error(:rootsymbol_missing) do
    :io_lib.fwrite(~c"Rootsymbol is missing", [])
  end

  def format_error(:terminals_missing) do
    :io_lib.fwrite(~c"Terminals is missing", [])
  end

  def format_error({:undefined_nonterminal, symbol}) do
    :io_lib.fwrite(~c"undefined nonterminal: ~ts", [format_symbol(symbol)])
  end

  def format_error({:undefined_pseudo_variable, atom}) do
    :io_lib.fwrite(~c"undefined pseudo variable ~w", [atom])
  end

  def format_error({:undefined_symbol, symName}) do
    :io_lib.fwrite(~c"undefined rhs symbol ~ts", [format_symbol(symName)])
  end

  def format_error({:unused_nonterminal, nonterminal}) do
    :io_lib.fwrite(~c"non-terminal symbol ~ts not used", [format_symbol(nonterminal)])
  end

  def format_error({:unused_terminal, terminal}) do
    :io_lib.fwrite(~c"terminal symbol ~ts not used", [format_symbol(terminal)])
  end

  def format_error({:bad_symbol, string}) do
    :io_lib.fwrite(~c"bad symbol ~ts", [string])
  end

  def format_error(:cannot_parse) do
    :io_lib.fwrite(~c"cannot parse; possibly encoding mismatch", [])
  end
  ...
  defp check_options(
         [
           {:error_location, eLoc} = optionValue
           | options
         ],
         allOptions,
         l
  ...
  defp check_options([{option, boolean} = optionValue | options], allOptions, l)
  ...
  defp start(infilex, options) do
    infile = assure_extension(infilex, ~c".yrl")
    ...
    outfilex =
      case outfilex0 do
        [] ->
          :filename.rootname(infilex, ~c".yrl")
    ...
    includefile =
      case includefilex do
        ...
        _ ->
          assure_extension(includefilex, ~c".hrl")
      end

    includefileVersion = includefile_version(includefile)
    outfile = assure_extension(outfilex, ~c".erl")
  ...
  defp os_process_size() do
    case :os.type() do
      {:unix, :sunos} ->
        size = :os.cmd(~c"ps -o vsz -p " ++ :os.getpid() ++ ~c" | tail -1")
  ...
  defp parse_grammar(st) do
    startLocation =
      case :lists.keyfind(:error_location, 1, r_yecc(st, :options)) do
  ...
  defp parse_grammar({r_symbol(name: :Header), ss}, inport, nextLocation, st0) do
    st1 =
      r_yecc(st0,
        header:
  ...
  defp parse_grammar(
         {r_symbol(name: :Erlang), [r_symbol(name: :code)]},
         _Inport,
         nextLocation,
         st
       ) do
  ...
  defp parse_grammar({:rule, rule, tokens, allTokens}, st0) do
    ...
    {tokens1, st} = subst_pseudo_vars(allTokens, nmbrOfDaughters, st0)
  ...
  defp check_rules(st0) do
    ...
    case r_yecc(st, :rules_list) do
      ...
      _ ->
        rule = r_rule(location: :none, symbols: [{}, r_yecc(st, :rootsymbol)], tokens: [])
        rules1 = [rule | rules0]
        ...
        foreach(
          fn
            ...
            {file, {location, mod, w}} ->
              :io.fwrite(
                "~ts:~s: ~s~ts\n",
                [file, pos(location), prefix, mod.format_error(w)]
              )
          end,
      ...
      [?$ | rest] ->
        try do
          ...
          n when n > 0 and n <= nmbrOfDaughters ->
            pseudoVarName = append(~c"___", rest)
            newAnno = :erl_anno.set_text(pseudoVarName, anno)
            {{:var, newAnno, :erlang.list_to_atom(pseudoVarName)}, st0}
    ...
    s2 =
      case reduce do
        {[hR | tR], ruleNmbr, ruleLocation} ->
          :io_lib.fwrite(
            "   Reduce to ~ts from ~ts (rule ~w ~s)\n      vs.\n",
            [format_symbol(hR), format_symbols(tR), ruleNmbr, rule_pos(ruleLocation)]
          )
      end
    ...
    s3 =
      case confl do
        {:reduce, [hR2 | tR2], ruleNmbr2, ruleLocation2} ->
          :io_lib.fwrite(
            "   reduce to ~ts from ~ts (rule ~w ~s).",
            [format_symbol(hR2), format_symbols(tR2), ruleNmbr2, rule_pos(ruleLocation2)]
          )
  ...
  defp rule_pos(line) when is_integer(line) do
    :io_lib.format(~c"at line ~w", [line])
  end

  defp rule_pos({line, column})
       when is_integer(line) and
              is_integer(column) do
    :io_lib.format(~c"at location ~w:~w", [line, column])
  end
  ...
  defp output_prelude(outport, inport, st0)
    ...
    st30 = nl(st25)
    includeFile = :filename.join([:code.lib_dir(:parsetools), ~c"include", ~c"yeccpre.hrl"])
  ...
  defp output_goto1(st0, [{from, to} | tail], f, stateInfo, isFirst) do
    ...
    cond do
      rO ->
        fromS = :io_lib.fwrite(~c"~w=_S", [from])
        toS = ~c"_S"

      true ->
        fromS = :io_lib.fwrite(~c"~w", [from])
        toS = :io_lib.fwrite(~c"~w", [to])
    end
  ...
  defp find_user_code2([
    ...
    [{action, [~c"Cat"], ruleNmbr, nmbrOfDaughters}]
  end
  ...
  defp output_state_actions1(st0, state, [{_, r_reduce() = action}], isFirst, _End, sI) do
    st = output_reduce(st0, state, ~c"Cat", action, isFirst, sI)
    fwrite(st, ".\n\n", [])
  end
      ...
      cond do
        ...
        true ->
          ...
          tmp =
            :lists.join(
              ~c",",
              :lists.duplicate(nmbrOfDaughters - 1, ~c"_")
            )
    ...
    cond do
      nmbrOfDaughters === 0 ->
        ...
        cond do
          ...
          true ->
            nextS = :io_lib.fwrite(~c"~w", [nextState])
        end
  ...
  defp function_name(st, name, suf) do
    :erlang.list_to_atom(concat([name, :_, safe_atom_chars(st, suf)]))
  end
  ...
  defp rule(rulePointer, st) do
    r_rule(n: n, location: location, symbols: symbols) =
      :maps.get(
        rulePointer,
  ...
  defp format_symbols1([h | t]) do
    [~c" ", format_symbol(h) | format_symbols1(t)]
  end
  ...
  defp line_of_location({line, column})
       when is_integer(line) and
              is_integer(column) do
    line
  end
