defmodule :m_yeccparser do
  ...
  def parse_and_scan({m, f, a}) do
    arity = length(a)
    yeccpars0([], {{Function.capture(m, f, arity), a}, :no_location}, 0, [], [])
  end
    ...
    try do
      ...
      :error, error ->
        try do
          ...
          desc ->
            :erlang.raise(:error, {:yecc_bug, ~c"1.4", desc}, __STACKTRACE__)
        end
  ...
  defp yeccpars1([], {{f, a}, _Location}, state, states, vstack) do
    case apply(f, a) do
  ...
  defp yeccpars1([], {:no_func, :no_location}, state, states, vstack) do
    line = 999_999
    yeccpars2(state, :"$end", states, vstack, yecc_end(line), [], {:no_func, line})
  end

  defp yeccpars1([], {:no_func, endLocation}, state, states, vstack) do
  ...
  defp yeccpars1(state1, state, states, vstack, token0, [], {{_F, _A}, _Location} = tzr) do
    yeccpars1([], tzr, state, [state1 | states], [token0 | vstack])
  end

  defp yeccpars1(state1, state, states, vstack, token0, [], {:no_func, :no_location}) do
  ...
  defp yeccpars1(state1, state, states, vstack, token0, [], {:no_func, location}) do
    yeccpars2(
      state,
      :"$end",
      [state1 | states],
  ...
  defp yeccerror(token) do
    ...
    {:error, {location, :yeccparser, [~c"syntax error before: ", text]}}
  end
  ...
  defp yeccpars2(other, _, _, _, _, _, _) do
    :erlang.error({:yecc_bug, ~c"1.4", {:missing_state_in_action_table, other}})
  end
  ...
  defp yeccpars2_11_(__Stack0) do
    [
      {:erlang_code,
       [{:atom, :erl_anno.set_text(~c"'$undefined'", :erl_anno.new(0)), :"$undefined"}]}
      | __Stack0
    ]
  end
