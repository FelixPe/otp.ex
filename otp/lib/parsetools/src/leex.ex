defmodule :m_leex do
  use Bitwise
  import :lists, only: [flatmap: 2, foldl: 3, foldr: 3,
                          foreach: 2, keysort: 2, map: 2, member: 2, reverse: 1,
                          sort: 1]
  ...
  Record.defrecord(:r_leex, :leex, xfile: [], efile: [],
                                ifile: [], gfile: [], module: :undefined,
                                opts: [], encoding: :none, errors: [],
                                warnings: [])
  Record.defrecord(:r_nfa_state, :nfa_state, no: :undefined,
                                     edges: [], accept: :noaccept)
  Record.defrecord(:r_dfa_state, :dfa_state, no: :undefined,
                                     nfa: [], trans: [], accept: :noaccept)
  def compile(input0, output0,
    ...
    deterministic = :proplists.get_bool(:deterministic,
                                          specific)
    opts = [{:scannerfile, output}, {:includefile,
                                       includefile},
  ...
  defp check_options([{:error_location, loc} = o | options],
            allOptions, l)
  ...
  defp all_options() do
    [:dfa_graph, :includefile, :report_errors,
  ...
  defp werror(st) do
    r_leex(st, :warnings) !== [] and member(:warnings_as_errors,
                                         r_leex(st, :opts))
  end
  ...
  defp report_warnings(st) do
    ...
    reportWerror = werror and member(:report_errors,
                                       r_leex(st, :opts))
    shouldReport = member(:report_warnings,
                            r_leex(st, :opts)) or reportWerror
    when_bool(fn () ->
  ...
  defp add_warning(line, w, st) do
    r_leex(st, warnings: [{r_leex(st, :xfile), {line, :leex, w}} |
                         r_leex(st, :warnings)])
  end
  ...
  defp filenames(file, opts, st0) do
    ...
    {:includefile, ifile0} = :lists.keyfind(:includefile, 1,
                                              opts)
    ifile = inc_file_name(ifile0)
    {:scannerfile, ofile} = :lists.keyfind(:scannerfile, 1,
                                             opts)
    cond do
      ofile === [] ->
        r_leex(st1, efile: :filename.join(dir, efile), 
                 ifile: ifile,  gfile: :filename.join(dir, gfile))
      true ->
        d = :filename.dirname(ofile)
        r_leex(st1, efile: ofile,  ifile: ifile, 
    ...
    case (:file.open(r_leex(st0, :xfile), [:read])) do
      {:ok, xfile} ->
        ...
        try do
          ...
          {:ok, line3, rEAs, actions, st4} = parse_rules(xfile,
                                                           line2, macs, st3)
  ...
  defp parse_defs(ifile, {:ok, chars, l} = line, ms, st) do
    mS = '^[ \t]*([A-Z_][A-Za-z0-9_]*)[ \t]*=[ \t]*([^ \t\r\n]*)[ \t\r\n]*$'
    case (:re.run(chars, mS,
                    [{:capture, :all_but_first, :list}, :unicode])) do
      {:match, [name, def__]} ->
        parse_defs(ifile, nextline(ifile, l, st),
                     [{name, def__} | ms], st)
      _ ->
        {:ok, line, ms, st}
    end
  ...
  defp parse_rules(ifile, {:ok, 'Rules.' ++ rest, l}, ms, st) do
    st1 = warn_ignored_chars(l, rest, st)
    parse_rules(ifile, nextline(ifile, l, st), ms, [], [],
                  0, st1)
  end
    ...
    case (nextLine) do
      ...
      {:ok, chars, l0} ->
        case (collect_rule(ifile, chars, l0)) do
          {:ok, re, atoks, l1} ->
            {:ok, rEA, a, st1} = parse_rule(re, l0, atoks, ms, n,
                                              st)
  ...
  defp collect_action(ifile, chars, l0, cont0) do
    case (:erl_scan.tokens(cont0, chars, l0)) do
      ...
      {:done, {:error, e, _}, _} ->
        {:error, e, l0}
      {:more, cont1} ->
        collect_action(ifile, :io.get_line(ifile, :leex),
                         l0 + 1, cont1)
    end
  end
  ...
  defp parse_rule(s, line, atoks, ms, n, st) do
    case (parse_rule_regexp(s, ms, st)) do
      {:ok, r} ->
        ...
        {:ok, {r, n},
           {n, atoks, tokenChars, tokenLen, tokenLine, tokenCol,
              tokenLoc},
           st}
      {:error, e} ->
        add_error({line, :leex, e}, st)
  ...
  defp re_seq1([c | _] = cs0, sn0, st) when (c !== ?| and
                                          c !== ?)) do
  ...
  defp re_char(?\\, [o1, o2, o3 | s]) when (o1 >= ?0 and
  ...
  defp re_char(?\\, [?x, h1, h2 | s])
      when (h1 >= ?0 and h1 <= ?9 or h1 >= ?A and h1 <= ?F or h1 >= ?a and h1 <= ?f and
    ...
    case (re_char(c1, cs0)) do
      {cf, [?-, c2 | cs1]} when c2 !== ?] ->
        case (re_char(c2, cs1)) do
          ...
          {_, cs2} ->
            parse_error({:char_class,
                           string_between([c1 | cs0], cs2)})
        end
      {c, cs1} ->
  ...
  defp string_between(cs1, cs2) do
    :string.slice(cs1, 0,
                    :string.length(cs1) - :string.length(cs2))
  end
  ...
  defp build_nfa({:pclosure, rE}, n0, f, nFA0) do
    {nFA1, n1, e1} = build_nfa(rE, n0 + 1, n0, nFA0)
    e = n1
    {[r_nfa_state(no: f, edges: [{:epsilon, n0}]), r_nfa_state(no: e1,
                                             edges: [{:epsilon, n0}, {:epsilon,
                                                                        e}]) |
                                             nFA1],
       n1 + 1, e}
  end
  ...
  defp build_nfa({:comp_class, cc}, n, f, nFA) do
    {[r_nfa_state(no: f, edges: [{comp_class(cc), n}]) | nFA], n + 1,
       n}
  end
  ...
  defp build_nfa_alt([rE | rEs], n0, f, nFA0) do
    ...
    {[r_nfa_state(no: f, edges: [{:epsilon, n0}, {:epsilon, n1}]),
          r_nfa_state(no: e1, edges: [{:epsilon, e}]), r_nfa_state(no: e2,
                                                 edges: [{:epsilon, e}]) |
                                                 nFA2],
       n2 + 1, e}
  end
  ...
  defp pack_crs([{c1, c2}, {c3, c4} | crs]) when (c2 >= c3 and
                                              c2 < c4) do
    pack_crs([{c1, c4} | crs])
  end
  ...
  defp build_dfa([u | us0], n0, ms, nFA) do
    {ts, us1, n1} = build_dfa(r_dfa_state(u, :nfa), us0, n0, [],
                                [u | ms], nFA)
  ...
  defp disjoint_crs([{c1, c2}, {c3, c4} | crs]) when c1 === c3 do
    [{c1, c2} | disjoint_crs(add_element({c2 + 1, c4},
                                           crs))]
  end

  defp disjoint_crs([{c1, c2}, {c3, c4} | crs]) when (c1 < c3 and
                                              c2 >= c3 and c2 < c4) do
    [{c1, c3 - 1} | disjoint_crs(union([{c3, c2}, {c2 + 1,
                                                     c4}],
                                         crs))]
  end
  ...
  defp disjoint_crs([{c1, c2}, {c3, c4} | crs]) when (c1 < c3 and
                                              c2 === c4) do
    [{c1, c3 - 1} | disjoint_crs(add_element({c3, c4},
                                               crs))]
  end

  defp disjoint_crs([{c1, c2}, {c3, c4} | crs]) when (c1 < c3 and
                                              c2 > c4) do
    [{c1, c3 - 1} | disjoint_crs(union([{c3, c4}, {c4 + 1,
                                                     c2}],
                                         crs))]
  end
      ...
      s when s !== [] ->
        case (dfa_state_exist(s, us, ms)) do
          ...
          :no ->
            ...
            build_dfa(crs, set, [u | us], n + 1, store(cr, n, ts),
                        ms, nFA)
        end
      [] ->
  ...
  defp move(sts, cr, nFA) do
    for n <- sts,
          ...
          crs !== :epsilon, in_crs(cr, crs) do
      st
    end
  end

  defp in_crs({c1, c2}, [{c3, c4} | _Crs]) when (c1 >= c3 and
                                               c2 <= c4) do
    true
  end
  ...
  defp min_dfa([d | dFA0], rs0, mDFA) do
    {dFA1, rs1} = min_delete(dFA0, r_dfa_state(d, :trans),
                               r_dfa_state(d, :accept), r_dfa_state(d, :no), rs0, [])
  ...
  defp min_delete([r_dfa_state(no: n, trans: t, accept: a) | dFA], t, a,
            newN, rs, mDFA) do
  ...
  defp pack_dfa([d | dFA], newN, rs, pDFA) do
    pack_dfa(dFA, newN + 1, [{r_dfa_state(d, :no), newN} | rs],
               [r_dfa_state(d, no: newN) | pDFA])
  end
  ...
  defp out_file(st0, dFA, dF, actions, code) do
    ...
    case (open_inc_file(st0)) do
          ...
          case (:file.open(r_leex(st0, :efile), [:write])) do
            {:ok, ofile} ->
              ...
              try do
                ...
                deterministic = :proplists.get_bool(:deterministic,
                                                      r_leex(st0, :opts))
                output_file_directive(ofile, r_leex(st0, :ifile),
                                        deterministic, 0)
      ...
      {{:error, error}, ifile} ->
        add_error(ifile, {:none, :leex, {:file_error, error}},
                    st0)
    end
  end
  ...
  defp out_file(ifile, ofile, st, dFA, dF, actions, code, l) do
    ...
    case (:io.get_line(ifile, :leex)) do
      :eof ->
        output_file_directive(ofile, r_leex(st, :ifile),
                                deterministic, l)
      {:error, _} ->
      ...
      line ->
        case (:string.slice(line, 0, 5)) do
          ...
          '##act' ->
            out_actions(ofile, r_leex(st, :xfile), deterministic,
                          actions)
          _ ->
            :io.put_chars(ofile, line)
        end
        out_file(ifile, ofile, st, dFA, dF, actions, code,
                   l + 1)
    end
  end
  ...
  defp out_erlang_code(file, st, code, l) do
    ...
    deterministic = :proplists.get_bool(:deterministic,
                                          r_leex(st, :opts))
    output_file_directive(file, r_leex(st, :xfile),
                            deterministic, codeL)
    ...
    :io.nl(file)
    output_file_directive(file, r_leex(st, :ifile),
                            deterministic, l)
  end
  ...
  defp out_dfa(file, st, dFA, code, dF, l) do
    ...
    deterministic = :proplists.get_bool(:deterministic,
                                          r_leex(st, :opts))
    output_file_directive(file, r_leex(st, :efile),
                            deterministic, l + (nCodeLines - 1) + 3)
  ...
  defp out_accept_body(file, next, line, col, action) do
    out_body(file, next, line, col, :io_lib.write(action),
               'Tlen')
  end
  ...
  defp prep_out_actions(as) do
    map(fn {a, :empty_action} ->
             {a, :empty_action}
           {a, code, tokenChars, tokenLen, tokenLine, tokenCol,
              tokenLoc} ->
             vs = [{tokenChars, 'TokenChars'}, {tokenLen, 'TokenLen'},
                                        {:erlang.or(tokenLine, tokenLoc), 'TokenLine'},
                                            {:erlang.or(tokenCol, tokenLoc), 'TokenCol'},
                                                {tokenChars, 'YYtcs'},
                                                    {:erlang.or(tokenLen,
                                                                  tokenChars),
                                                       'TokenLen'}]
             ...
             name = :erlang.list_to_atom(:lists.concat([:yyaction_,
                                                            a]))
             [chars, len, line, col, _, _] = vars
             args = (for v <- [chars, len, line, col], v !== '_' do
                       v
                     end)
  ...
  defp out_action(file,
            {a, _Code, vars, name, _Args, argsChars, _TokenLoc}) do
  ...
  defp out_action_code(_File, _XrlFile, _Deterministic,
            {_A, :empty_action}) do
    :ok
  end

  defp out_action_code(file, xrlFile, deterministic,
            {_A, code, _Vars, name, args, argsChars, tokenLoc}) do
    :io.fwrite(file, '\n-compile({inline,~w/~w}).\n', [name, length(args)])
    l = :erl_scan.line(hd(code))
    output_file_directive(file, xrlFile, deterministic,
                            l - 2)
    :io.fwrite(file, '~s(~s) ->~n', [name, argsChars])
    cond do
      tokenLoc ->
  ...
  defp out_dfa_edges(file, dFA) do
    foreach(fn r_dfa_state(no: s, trans: trans) ->
                 pt = pack_trans(trans)
                 tdict = foldl(fn {cr, t}, d ->
