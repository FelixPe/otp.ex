defmodule :m_cpu_sup do
  use Bitwise
  require Record
  Record.defrecord(:r_cpu_util, :cpu_util, cpu: :undefined,
                                    busy: [], non_busy: [])
  Record.defrecord(:r_state, :state, server: :undefined,
                                 os_type: :undefined)
  Record.defrecord(:r_internal, :internal, port: :not_used,
                                    util: [], os_type: :undefined)
  def start() do
  ...
  def util(args) when is_list(args) do
    case (:lists.foldl(fn :detailed, {_, pC} ->
                            {true, pC}
                          :per_cpu, {d, _} ->
                            {d, true}
                          _, _ ->
  ...
  def handle_call({'u', d, pC}, {client, _Tag},
           r_state(os_type: {:unix, flavor}) = state)
      when flavor == :sunos or flavor == :linux or
             flavor == :freebsd or flavor == :openbsd or
             flavor == :darwin do
    case (measurement_server_call(r_state(state, :server),
                                    {'u', d, pC, client})) do
      {:error, reason} ->
        {:reply, {:error, reason},
  ...
  def handle_call(request, _From, state) when request == 'n' or
  ...
  def handle_info({:EXIT, _Port, reason}, state) do
    {:stop, {:server_died, reason},
       r_state(state, server: :not_used)}
  end
  ...
  defp get_uint32_measurement(request,
            r_internal(port: p, os_type: {:unix, :linux})) do
    case (:file.open('/proc/loadavg', [:read, :raw])) do
      {:ok, f} ->
        {:ok, d} = :file.read_line(f)
        :ok = :file.close(f)
        {:ok, [load1, load5, load15, _PRun, pTotal],
           _} = :io_lib.fread('~f ~f ~f ~d/~d', d)
        case (request) do
          '1' ->
            sunify(load1)
          '5' ->
  ...
  defp keysearchdelete(k, n, [t | ts]) when :erlang.element(n,
                                                 t) == k do
    {{:value, t}, ts}
  end
  ...
  defp cpu_util_diff([r_cpu_util(cpu: cpu, busy: newBusy,
               non_busy: newNonBusy) |
               newCpuUtils],
    ...
    {nonBusy, gotNonBusy} = state_list_diff(newNonBusy,
                                              oldNonBusy)
    busy = (case (gotBusy or gotNonBusy) do
              true ->
                preBusy
              false ->
                :lists.map(fn {:user, 0} ->
  ...
  defp cpu_util_diff([r_cpu_util(cpu: nC) | _] = new, [r_cpu_util(cpu: oC) | _] = old,
            acc)
      when nC < oC do
  ...
  defp cpu_util_rel(newCpuUtils, oldCpuUtils, detailed, perCpu) do
    cpu_util_rel(cpu_util_diff(newCpuUtils, oldCpuUtils),
                   detailed, perCpu)
  end

  defp cpu_util_rel(cUDiff, false, false) do
    {b, t} = :lists.foldl(fn r_cpu_util(busy: busyList,
  ...
  defp cpu_util_rel_pcpu([r_cpu_util(cpu: c, busy: busyList,
               non_busy: nonBusyList) |
               rest],
            acc) do
    busy = state_list_sum(busyList)
    nonBusy = state_list_sum(nonBusyList)
    tot = busy + nonBusy
    cpu_util_rel_pcpu(rest,
                        [{c, busy / tot * 100, nonBusy / tot * 100, []} | acc])
  end

  defp cpu_util_rel_det([],
            r_cpu_util(cpu: cpuAcc, busy: busyAcc, non_busy: nonBusyAcc)) do
    total = state_list_sum(busyAcc) + state_list_sum(nonBusyAcc)
    {cpuAcc, mk_rel_states(busyAcc, total),
       mk_rel_states(nonBusyAcc, total), []}
  end

  defp cpu_util_rel_det([r_cpu_util(cpu: cpu, busy: busy, non_busy: nonBusy) |
               rest],
            r_cpu_util(cpu: cpuAcc, busy: busyAcc, non_busy: nonBusyAcc)) do
    cpu_util_rel_det(rest,
                       r_cpu_util(cpu: [cpu | cpuAcc],
  ...
  defp state_list_diff([{state, valueNew} | restNew],
            [{state, valueOld} | restOld]) do
    valDiff = val_diff(state, valueNew, valueOld)
    {restStateDiff, foundDiff} = state_list_diff(restNew,
                                                   restOld)
    {[{state, valDiff} | restStateDiff],
       foundDiff or valDiff != 0}
  end
  ...
  defp state_list_add([{state, valueA} | restA],
            [{state, valueB} | restB]) do
    [{state, valueA + valueB} | state_list_add(restA,
                                                 restB)]
  end
  ...
  defp val_diff(state, 4294967295, 0) do
  ...
  defp measurement_server_init() do
    ...
    oS = :os.type()
    server = (case (oS) do
                {:unix, flavor} when flavor == :sunos or
                  ...
                  pid
                {:unix, flavor} when flavor == :irix64 or
                                       flavor == :irix
                                     ->
                  :not_used
                _ ->
  ...
  defp measurement_server_loop(state) do
    receive do
      {:DOWN, monitor, :process, _, _} ->
        measurement_server_loop(r_internal(state, util: :lists.keydelete(monitor,
                                                                  2,
                                                                  r_internal(state, :util))))
      {pid, {'u', d, pC, client}} ->
        {monitor, oldCpuUtil,
           utils2} = (case (keysearchdelete(client, 1,
                                              r_internal(state, :util))) do
        ...
        try do
          ...
          newCpuUtil ->
            ...
            measurement_server_loop(r_internal(state, util: [{client,
                                                       monitor, newCpuUtil} |
                                                        utils2]))
        end
      {pid, request} ->
  ...
  defp port_receive_util(port, timeout) do
    receive do
      {^port,
         {:data,
            [nP3, nP2, nP1, nP0, nE3, nE2, nE1, nE0 | cpuData]}} ->
        port_receive_cpu_util(nP3 <<< 24 ||| (nP2 <<< 16) ||| (nP1 <<< 8) ||| nP0,
  ...
  defp port_receive_cpu_util(nP, nE, cpuData, cpuList) do
    {cpuUtil, rest} = port_receive_cpu_util_entries(nE, r_cpu_util(),
                                                      cpuData)
    port_receive_cpu_util(nP - 1, nE, rest,
                            [cpuUtil | cpuList])
  end
  ...
  defp port_receive_cpu_util_entries(nE, cU,
    ...
    case (tagId) do
      ...
      1 ->
        ...
        port_receive_cpu_util_entries(nE - 1, newCU, cpuData)
      2 ->
        newCU = r_cpu_util(cU, busy: [{:nice_user, value} |
                                 r_cpu_util(cU, :busy)])
        port_receive_cpu_util_entries(nE - 1, newCU, cpuData)
      3 ->
        ...
        port_receive_cpu_util_entries(nE - 1, newCU, cpuData)
      4 ->
        newCU = r_cpu_util(cU, non_busy: [{:wait, value} |
                                     r_cpu_util(cU, :non_busy)])
        port_receive_cpu_util_entries(nE - 1, newCU, cpuData)
      5 ->
        newCU = r_cpu_util(cU, non_busy: [{:idle, value} |
                                     r_cpu_util(cU, :non_busy)])
        port_receive_cpu_util_entries(nE - 1, newCU, cpuData)
      6 ->
      ...
      7 ->
        ...
        port_receive_cpu_util_entries(nE - 1, newCU, cpuData)
      8 ->
        newCU = r_cpu_util(cU, non_busy: [{:steal, value} |
                                     r_cpu_util(cU, :non_busy)])
        port_receive_cpu_util_entries(nE - 1, newCU, cpuData)
      unhandled ->
