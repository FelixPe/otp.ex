defmodule :m_cpu_sup do
  ...
  def stop() do
    :gen_server.call(:cpu_sup, ~c"q", :infinity)
  end

  def nprocs() do
    :os_mon.call(:cpu_sup, ~c"n", :infinity)
  end

  def avg1() do
    :os_mon.call(:cpu_sup, ~c"1", :infinity)
  end

  def avg5() do
    :os_mon.call(:cpu_sup, ~c"5", :infinity)
  end

  def avg15() do
    :os_mon.call(:cpu_sup, ~c"f", :infinity)
  end
  ...
  def util(args) when is_list(args) do
    case :lists.foldl(
      ...
      {detailed, perCpu} ->
        :os_mon.call(:cpu_sup, {~c"u", detailed, perCpu}, :infinity)
    end
  end
  ...
  def dummy_reply(~c"n") do
    0
  end

  def dummy_reply(~c"1") do
    0
  end

  def dummy_reply(~c"5") do
    0
  end

  def dummy_reply(~c"f") do
    0
  end

  def dummy_reply({~c"u", _, _}) do
    {:all, 0, 0, []}
  end

  def ping() do
    :gen_server.call(:cpu_sup, ~c"p")
  end
  ...
  def handle_call(~c"q", _From, state) do
    {:stop, :normal, :ok, state}
  end

  def handle_call({~c"u", d, pC}, {client, _Tag}, r_state(os_type: {:unix, flavor}) = state)
  ...
  def handle_call({~c"u", detailed, perCpu}, _From, state) do
    string = ~c"OS_MON (cpu_sup), util/1 unavailable for this OS~n"
    :error_logger.warning_msg(string)
    {:reply, dummy_reply({~c"u", detailed, perCpu}), state}
  end

  def handle_call(request, _From, state)
      when request == ~c"n" or
             request == ~c"1" or request == ~c"5" or
             request == ~c"f" or request == ~c"p" do
    case measurement_server_call(
           r_state(state, :server),
           request
         ) do
  ...
  defp get_uint32_measurement(
    ...
    case :file.open(~c"/proc/loadavg", [:read, :raw]) do
      {:ok, f} ->
        {:ok, d} = :file.read_line(f)
        :ok = :file.close(f)
        {:ok, [load1, load5, load15, _PRun, pTotal], _} = :io_lib.fread(~c"~f ~f ~f ~d/~d", d)
  ...
  defp get_uint32_measurement(request, r_internal(os_type: {:unix, sys}))
    ...
    {:ok, [l1, l5, l15], _} = :io_lib.fread(~c"~f, ~f, ~f", avg)
    ...
    case request do
      ...
      ~c"n" ->
        {:ok, procList} = :file.list_dir(~c"/proc/pinfo")
        length(procList)
    end
  end
  ...
  defp get_util_measurement(~c"u", r_internal(port: p)) do
    case port_server_call(p, ~c"u") do
      {:error, error} ->
        {:error, error}
  ...
  defp measurement_server_loop(state) do
    receive do
      ...
      {pid, {~c"u", d, pC, client}} ->
        ...
        try do
          get_util_measurement(~c"u", state)
        catch
          error ->
  ...
  defp port_server_loop(port, timeout) do
    receive do
      ...
      {pid, ~c"n"} ->
        :erlang.port_command(port, ~c"n")
        result = port_receive_uint32(port, timeout)
        send(pid, {self(), {:data, result}})
        port_server_loop(port, timeout)

      {pid, ~c"1"} ->
        :erlang.port_command(port, ~c"1")
        result = port_receive_uint32(port, timeout)
        send(pid, {self(), {:data, result}})
        port_server_loop(port, timeout)

      {pid, ~c"5"} ->
        :erlang.port_command(port, ~c"5")
        result = port_receive_uint32(port, timeout)
        send(pid, {self(), {:data, result}})
        port_server_loop(port, timeout)

      {pid, ~c"f"} ->
        :erlang.port_command(port, ~c"f")
        result = port_receive_uint32(port, timeout)
        send(pid, {self(), {:data, result}})
        port_server_loop(port, timeout)

      {pid, ~c"u"} ->
        :erlang.port_command(port, ~c"u")
        result = port_receive_util(port, timeout)
        send(pid, {self(), {:data, result}})
        port_server_loop(port, timeout)

      {pid, ~c"p"} ->
        :erlang.port_command(port, ~c"p")
        result = port_receive_uint32(port, timeout)
        send(pid, {self(), {:data, result}})
        port_server_loop(port, timeout)

      {pid, ~c"q"} ->
        send(port, {self(), {:command, ~c"q"}})
  ...
  defp start_portprogram() do
    port = :os_mon.open_port(~c"cpu_sup", [:stream])
    :erlang.port_command(port, ~c"p")
