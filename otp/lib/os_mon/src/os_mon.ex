defmodule :m_os_mon do
        ...
        case :lists.keysearch(:os_mon, 1, :application.which_applications()) do
          {:value, _AppInfo} ->
            case startp(service) do
              ...
              false ->
                string = ~c"OS_MON (~p) called by ~p, unavailable~n"
                :error_logger.warning_msg(string, [service, self()])
                service.dummy_reply(request)
            end

          false ->
            string = ~c"OS_MON (~p) called by ~p, not started~n"
            :error_logger.warning_msg(string, [service, self()])
            service.dummy_reply(request)
        end
    end
    ...
    case :application.get_env(:os_mon, param) do
      {:ok, value} ->
        case service.param_type(param, value) do
          ...
          false ->
            string =
              ~c"OS_MON (~p), ignoring bad configuration parameter (~p=~p)~nUsing default value instead~n"
  ...
  def open_port(name, opts) do
    ...
    releasedPath = :filename.join([privDir, ~c"bin", name])

    case :filelib.is_regular(releasedPath) do
      true ->
        :erlang.open_port(
          {:spawn, ~c"\"" ++ releasedPath ++ ~c"\""},
          opts
        )

      false ->
        archPath =
          :filename.join([privDir, ~c"bin", :erlang.system_info(:system_architecture), name])

        :erlang.open_port({:spawn, ~c"\"" ++ archPath ++ ~c"\""}, opts)
    end
  end
