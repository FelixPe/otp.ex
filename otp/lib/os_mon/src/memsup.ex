defmodule :m_memsup do
  ...
  def handle_info(:reg_collection_timeout, state) do
    ...
    flush(:collected_sys)
    str = ~c"OS_MON (memsup) timeout, no data collected~n"
  ...
  def format_status(
    ...
    [
      {:data, [{~c"Timeout", timeout}]},
      {:items, {~c"Memory Usage", [{~c"Allocated", allocated}, {~c"Total", total}]}},
      {:items, {~c"Worst Memory User", worstMemFormat}}
    ]
  end

  defp get_os_wordsize({:unix, :sunos}) do
    string = clean_string(:os.cmd(~c"isainfo -b"))
  ...
  defp get_os_wordsize_with_uname() do
    string = clean_string(:os.cmd(~c"uname -m"))
  ...
  defp get_memory_usage({:unix, oSname})
       when oSname == :freebsd or
              oSname == :dragonfly do
    pageSize = freebsd_sysctl(~c"vm.stats.vm.v_page_size")
    pageCount = freebsd_sysctl(~c"vm.stats.vm.v_page_count")
    freeCount = freebsd_sysctl(~c"vm.stats.vm.v_free_count")
  ...
  defp get_memory_usage({:win32, _OSname}) do
    ...
    {:ok, [_MemLoad, totPhys, availPhys, _TotPage, _AvailPage, _TotV, _AvailV], _RestStr} =
      :io_lib.fread(~c"~d~d~d~d~d~d~d", result)

    {totPhys - availPhys, totPhys}
  end

  defp freebsd_sysctl(def__) do
    :erlang.list_to_integer(:os.cmd(~c"/sbin/sysctl -n " ++ def__) -- ~c"\n")
  end
  ...
  defp start_portprogram() do
    :os_mon.open_port(~c"memsup", [{:packet, 1}])
  end
  ...
  defp get_ext_memory_usage(port, accum, memsup) do
    receive do
      ...
      {^port, {:data, [tag]}} ->
        get_ext_memory_usage(tag2atag(port, tag), port, accum, memsup)
  ...
  defp get_ext_memory_usage_cancelled(port) do
    receive do
      ...
      {^port, {:data, [tag]}} ->
        get_ext_memory_usage_cancelled(
          tag2atag(port, tag),
          port
        )
