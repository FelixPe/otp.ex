defmodule :m_erl_bif_types do
  use Bitwise
  import :erl_types, only: [number_max: 2, number_min: 2,
                              t_any: 0, t_arity: 0, t_atom: 0, t_atom: 1,
                              t_atom_vals: 2, t_atoms: 1, t_binary: 0,
                              t_bitstr: 0, t_boolean: 0, t_cons: 0, t_cons: 2,
                              t_cons_hd: 1, t_cons_tl: 1, t_fixnum: 0,
                              t_float: 0, t_from_range: 2, t_from_term: 1,
                              t_fun: 0, t_fun: 2, t_fun_args: 2, t_fun_range: 2,
                              t_has_opaque_subtype: 2, t_identifier: 0,
                              t_inf: 3, t_integer: 0, t_integer: 1,
                              t_integers: 1, t_is_any: 1, t_is_atom: 2,
                              t_is_binary: 2, t_is_bitstr: 2, t_is_boolean: 2,
                              t_is_cons: 2, t_is_float: 2, t_is_fun: 2,
                              t_is_impossible: 1, t_is_integer: 2, t_is_map: 2,
                              t_is_maybe_improper_list: 2, t_is_nil: 1,
                              t_is_nil: 2, t_is_none: 1, t_is_number: 2,
                              t_is_pid: 2, t_is_port: 2, t_is_reference: 2,
                              t_is_subtype: 2, t_is_tuple: 2, t_list: 0,
                              t_list: 1, t_list_elements: 2,
                              t_list_termination: 2, t_map: 0, t_map: 3,
                              t_map_def_key: 2, t_map_def_val: 2,
                              t_map_entries: 2, t_map_get: 3, t_map_is_key: 3,
                              t_map_pairwise_merge: 4, t_map_put: 3,
                              t_map_remove: 3, t_map_update: 3,
                              t_maybe_improper_list: 0, t_module: 0, t_nil: 0,
                              t_node: 0, t_non_neg_fixnum: 0,
                              t_non_neg_integer: 0, t_none: 0,
                              t_nonempty_list: 0, t_nonempty_list: 1,
                              t_number: 0, t_number_vals: 2, t_pid: 0,
                              t_port: 0, t_pos_fixnum: 0, t_pos_integer: 0,
                              t_reference: 0, t_string: 0, t_subtract: 2,
                              t_sup: 1, t_sup: 2, t_tuple: 0, t_tuple: 1,
                              t_tuple_args: 2, t_tuple_size: 2,
                              t_tuple_subtypes: 2]
  def type(m, f, a) do
  ...
  def type(:erlang, :error, 3, _, _) do
    t_none()
  end
  ...
  def type(:erlang, :"==", 2, xs = [x1, x2], opaques) do
    case (t_is_atom(x1, opaques) and t_is_atom(x2,
                                                 opaques)) do
      true ->
        type(:erlang, :"=:=", 2, xs, opaques)
      false ->
        case (t_is_integer(x1, opaques) and t_is_integer(x2,
                                                           opaques)) do
          true ->
            type(:erlang, :"=:=", 2, xs, opaques)
          false ->
  ...
  def type(:erlang, :"=:=", 2, xs = [lhs, rhs], opaques) do
    ans = (case (t_is_none(t_inf(lhs, rhs, opaques))) do
             true ->
               t_atom(false)
             false ->
               case (t_is_atom(lhs, opaques) and t_is_atom(rhs,
                                                             opaques)) do
                 true ->
                   case ({t_atom_vals(lhs, opaques),
                            t_atom_vals(rhs, opaques)}) do
                     {:unknown, _} ->
                       t_boolean()
                     {_, :unknown} ->
                       t_boolean()
                 ...
                 false ->
                   case (t_is_integer(lhs, opaques) and t_is_integer(rhs,
                                                                       opaques)) do
                     false ->
                       t_boolean()
                     true ->
                       case ({t_number_vals(lhs, opaques),
                                t_number_vals(rhs, opaques)}) do
                 ...
                 true ->
                   case ({t_atom_vals(lhs, opaques),
                     ...
                     {[val], [val]} ->
                       t_atom(false)
                     {lhsVals, rhsVals} ->
                       t_sup(for x <- lhsVals, y <- rhsVals do
                               t_from_term(x !== y)
                             end)
                   end
                 false ->
  ...
  def type(:erlang, :">", 2, xs = [lhs, rhs], opaques) do
    ans = (case (t_is_integer(lhs,
                                opaques) and t_is_integer(rhs, opaques)) do
             true ->
               lhsMax = number_max(lhs, opaques)
               lhsMin = number_min(lhs, opaques)
               rhsMax = number_max(rhs, opaques)
             ...
             fn [x1, x2] ->
                  case (arith(:"+", x1, x2, opaques)) do
                    {:ok, t} ->
                      t
                    :error ->
                      case (t_is_float(x1, opaques) or t_is_float(x2,
                                                                    opaques)) do
                        true ->
                          t_float()
                        false ->
  ...
  def type(:erlang, :binary_part, 2, xs, opaques) do
    strict(:erlang, :binary_part, 2, xs,
             fn _ ->
                  t_binary()
             end,
             opaques)
  ...
  def type(:erlang, :binary_part, 3, xs, opaques) do
    strict(:erlang, :binary_part, 3, xs,
             fn _ ->
                  t_binary()
             end,
             opaques)
                    ...
                    [_] ->
                      ...
                      case (t_number_vals(x1, opaques)) do
                        :unknown ->
                          t_sup(as)
                        ns when is_list(ns) ->
                          fun = fn n, x when (is_integer(n) and 1 <= n and
                                                n <= sz)
                                             ->
  ...
  def type(:erlang, :build_stacktrace, 0, _, _Opaques) do
    t_list(t_tuple([t_module(), t_atom(), t_sup([t_arity(),
                                                     t_list()]),
                                              t_list(t_sup([t_tuple([t_atom(:error_info),
                                                                         t_map()]),
                                                                t_tuple([t_atom(:file),
                                                                             t_string()]),
                                                                    t_tuple([t_atom(:line),
    ...
    fun = fn [x, y, z] ->
               ...
               case (t_is_tuple(x, opaques)) do
                 false when length(arity) === 1 ->
                   [realArity] = arity
                   case (t_is_none(t_inf(t_tuple(realArity), x,
                                           opaques))) do
                     true ->
                       case (t_has_opaque_subtype(x, opaques)) do
                         true ->
  ...
  def type(:erlang, :max, 2, xs, opaques) do
  ...
  def type(:erlang, :min, 2, xs, opaques) do
  ...
  def type(:erlang, :raise, 3, xs, opaques) do
                    ...
                    [_] ->
                      ...
                      case (t_number_vals(x1, opaques)) do
                        ...
                        [n] when (is_integer(n) and n > sz) ->
                          t_none()
                        ns ->
                          fun = fn n, xL when (is_integer(n) and 1 <= n and
                                                 n <= sz)
                                              ->
                                     x = :lists.nth(n, xL)
                                     y = t_sup(x, x3)
                  ...
                  case (t_is_atom(type, opaques)) do
                    true ->
                      case (t_atom_vals(type, opaques)) do
                        [:allocated_areas] ->
                          t_list(t_sup([t_tuple([t_atom(),
                                                     t_non_neg_integer()]),
                                            t_tuple([t_atom(),
                                                         t_non_neg_integer(),
                                                             t_non_neg_integer()])]))
                        [:allocator] ->
                          t_tuple([t_sup([t_atom(:undefined), t_atom(:glibc)]),
                                       t_list(t_integer()), t_list(t_atom()),
                        ...
                        [:dist] ->
                          t_binary()
                        [:dist_ctrl] ->
                          t_list(t_tuple([t_atom(), t_sup([t_pid(),
                                                               t_port()])]))
                        [:endian] ->
                          t_endian()
                        [:fullsweep_after] ->
                          t_tuple([t_atom(:fullsweep_after),
                                       t_non_neg_integer()])
                        [:garbage_collection] ->
                          t_list()
                        [:heap_sizes] ->
                        ...
                        [:multi_scheduling_blockers] ->
                          t_list(t_pid())
                        [:os_type] ->
                          t_tuple([t_sup([t_atom(:unix), t_atom(:win32)]),
                                       t_atom()])
                        [:os_version] ->
                          t_sup(t_tuple([t_non_neg_fixnum(), t_non_neg_fixnum(),
                                                                 t_non_neg_fixnum()]),
                                  t_string())
                        [:otp_release] ->
                          t_string()
             ...
             fn [x] ->
                  case (t_tuple_subtypes(x, opaques)) do
                    :unknown ->
                      t_list()
                    subTypes ->
                      args = :lists.append(for sT <- subTypes do
                  ...
                  case (t_is_nil(x, opaques)) do
                    ...
                    false ->
                      ...
                      case (check_fun_application(f, [x1], opaques)) do
                        :ok ->
                          case (t_atom_vals(t_fun_range(f, opaques),
                                              opaques)) do
                            [true] ->
                  ...
                  case (t_is_nil(l, opaques)) do
                    ...
                    false ->
                      ...
                      case (check_fun_application(f, [t], opaques)) do
                        :ok ->
                          rangeVals = t_atom_vals(t_fun_range(f, opaques),
                                                    opaques)
             ...
             fn [f, list] ->
                  case (t_is_nil(list, opaques)) do
                    true ->
                      t_nil()
                    false ->
                      case (check_fun_application(f,
                                                    [t_list_elements(list,
                                                                       opaques)],
                                                    opaques)) do
                        :ok ->
                        ...
                        false ->
                          case (t_tuple_subtypes(tuple, opaques)) do
                            :unknown ->
                              ret
                            list ->
                              case (key_comparisons_fail(x, y, list,
                                                           opaques)) do
                                true ->
                                  t_atom(false)
                                false ->
             ...
             fn [f, _I, l] ->
                  case (t_is_nil(l, opaques)) do
                    true ->
                      l
                    false ->
                      t_list(t_sup(t_fun_range(f, opaques),
                        ...
                        true ->
                          case (check_fun_application(f, [el, acc], opaques)) do
                            :ok ->
                              ...
                              t_sup(for sT <- t_tuple_subtypes(r, opaques) do
                                      fun.(sT)
                                    end)
                            :error ->
                              t_none()
  ...
  def type(:lists, :zip3, 3, xs, opaques) do
    strict(:lists, :zip3, 3, xs,
             fn [as, bs, cs] ->
                  case (t_is_nil(as, opaques) or t_is_nil(bs,
                                                            opaques) or t_is_nil(cs,
                                                                                   opaques)) do
                    true ->
                      t_nil()
                    false ->
  ...
  def type(:maps, :from_keys, 2, xs, opaques) do
             ...
             fn [list, value] ->
                  case (t_is_nil(list, opaques)) do
                    true ->
                      t_from_term(%{})
                    false ->
                      t_map([], t_list_elements(list, opaques), value)
                  end
             end,
             opaques)
  end
  ...
  def type(:maps, :merge, 2, xs, opaques) do
    strict(:maps, :merge, 2, xs,
             fn [mapA, mapB] ->
                  ...
                  t_map(t_map_pairwise_merge(fn k, _, _, :mandatory, v ->
                                                  {k, :mandatory, v}
  ...
  def type(:maps, :size, 1, xs, opaques) do
    strict(:maps, :size, 1, xs,
             fn [map] ->
                  mand = (for (e = {_, :mandatory,
                                      _}) <- t_map_entries(map, opaques) do
                            e
                          end)
                  ...
                  case (t_is_none(t_map_def_key(map, opaques))) do
                    ...
                    true ->
                      opt = (for (e = {_, :optional, _}) <- t_map_entries(map,
                                                                            opaques) do
                               e
                             end)
  ...
  def type(m, f, a, xs, _O) when (is_atom(m) and
  ...
  defp any_is_none_or_unit(ts) do
    :lists.any(&:erl_types.t_is_impossible/1, ts)
  end
  ...
  defp infinity_band(type1, type2) when (is_integer(type1) and
                                is_integer(type2)) do
    type1 &&& type2
  end
  ...
  defp infinity_bor(type1, type2) when (is_integer(type1) and
                                is_integer(type2)) do
    type1 ||| type2
  end
  ...
  defp infinity_div(:pos_inf, number) when (is_integer(number) and
                                    number > 0) do
    :pos_inf
  end
  ...
  defp infinity_div(:neg_inf, number) when (is_integer(number) and
                                    number > 0) do
    :neg_inf
  end
  ...
  defp infinity_div(number, :pos_inf) when (is_integer(number) and
                                    number >= 0) do
    :pos_inf
  end
  ...
  defp infinity_div(number, :neg_inf) when (is_integer(number) and
                                    number >= 0) do
    :neg_inf
  end
  ...
  defp infinity_div(number1, number2) when (is_integer(number1) and
  ...
  defp infinity_bsl(number, :pos_inf) when (is_integer(number) and
                                    number > 0) do
    :pos_inf
  end
  ...
  defp infinity_bsl(number, :neg_inf) when (is_integer(number) and
                                    number >= 0) do
    0
  end
  ...
  defp infinity_bsl(number1, number2) when (is_integer(number1) and
                                    is_integer(number2)) do
    bits = 128
    cond do
      number2 > bits * 2 ->
  ...
  def infinity_add(number1, number2) when (is_integer(number1) and
                                   is_integer(number2)) do
    try do
      number1 + number2
    catch
  ...
  defp infinity_mult(number1, number2) when (is_integer(number1) and
                                    is_integer(number2)) do
    try do
      number1 * number2
    catch
  ...
  defp arith_bnot(x1, opaques) do
    case (t_is_integer(x1, opaques)) do
      ...
      true ->
        ...
        {:ok,
           t_from_range(infinity_add(infinity_inv(max1), - 1),
  ...
  defp arith_abs(x1, opaques) do
    case (t_is_integer(x1, opaques)) do
      ...
      true ->
        min1 = number_min(x1, opaques)
        max1 = number_max(x1, opaques)
        {newMin, newMax} = (case (infinity_geq(min1, 0)) do
                              true ->
                                {min1, max1}
                              false ->
  ...
  defp arith_mult(min1, max1, min2, max2) do
    tmp_list = [infinity_mult(min1, min2),
                    infinity_mult(min1, max2), infinity_mult(max1, min2),
  ...
  defp arith_rem(min1, max1, min2, max2) do
    ...
    max_range2 = infinity_max([infinity_abs(min2),
                                   infinity_abs(max2)])
    new_min = (cond do
                 min1_geq_zero ->
                   0
  ...
  defp arith_band_range_set({min, max}, [int | intList]) do
    safeAnd = :lists.foldl(fn intFromSet, safeAndAcc ->
                                intFromSet ||| safeAndAcc
                           end,
                             int, intList)
    {infinity_band(min, safeAnd),
       infinity_band(max, safeAnd)}
  end
  ...
  defp arith_band(x1, x2, opaques) do
    ...
    case ({l1 === :unknown, l2 === :unknown}) do
      {true, false} ->
        arith_band_range_set(arith_band_ranges(min1, max1, min2,
                                                 max2),
                               l2)
      {false, true} ->
        arith_band_range_set(arith_band_ranges(min1, max1, min2,
  ...
  defp arith_bor(x1, x2, opaques) do
    ...
    case ({l1 === :unknown, l2 === :unknown}) do
      {true, false} ->
        arith_bor_range_set(arith_bor_ranges(min1, max1, min2,
                                               max2),
                              l2)
      {false, true} ->
        arith_bor_range_set(arith_bor_ranges(min1, max1, min2,
  ...
  defp arith_band_ranges(min1, max1, min2, max2) do
    width = infinity_min([width({min1, max1}), width({min2,
                                                        max2})])
    min = (case (infinity_geq(min1, 0) or infinity_geq(min2,
                                                         0)) do
             true ->
               0
             false ->
    ...
    max = (case (infinity_geq(max1, 0) or infinity_geq(max2,
                                                         0)) do
             true ->
  ...
  defp arith_bor_ranges(min1, max1, min2, max2) do
    width = infinity_max([width({min1, max1}), width({min2,
                                                        max2})])
    min = (case (infinity_geq(min1,
                                0) and infinity_geq(min2, 0)) do
             true ->
               0
             false ->
               infinity_bsl(- 1, width)
           end)
    max = (case (infinity_geq(max1,
                                0) and infinity_geq(max2, 0)) do
             true ->
               infinity_add(infinity_bsl(1, width), - 1)
             false ->
               - 1
      ...
      true ->
        ...
        case (l1 === :unknown or l2 === :unknown) do
          true ->
            ...
            min2 = number_min(x2, opaques)
            max2 = number_max(x2, opaques)
            {newMin, newMax} = (case (op) do
                                  :"+" ->
                                    {infinity_add(min1, min2),
                                       infinity_add(max1, max2)}
                                  :- ->
  ...
  defp compare(op, lhs, rhs, opaques) do
    case (t_is_none(t_inf(lhs, rhs, opaques))) do
      false ->
        t_boolean()
      true ->
        case (opaque_args(:erlang, op, 2, [lhs, rhs],
                            opaques) === []) do
          true ->
            case (op) do
              :"<" ->
  ...
  defp type_ranks(type, i, min, max, [typeClass | rest],
            opaques) do
    {newMin, newMax} = (case (t_is_none(t_inf(type,
                                                typeClass, opaques))) do
                          true ->
                            {min, max}
                          false ->
                            case (min) do
  ...
  defp type_order() do
    [t_number(), t_atom(), t_reference(), t_fun(), t_port(),
                                                       t_pid(), t_tuple(),
                                                                    t_map(),
                                                                        t_list(),
                                                                            t_bitstr()]
  ...
  def arg_types(:erlang, :apply, 2) do
    [t_sup(t_tuple([t_module(), t_atom()]), t_fun()),
         t_list()]
  end
  ...
  def arg_types(:erlang, :binary_part, 2) do
    [t_binary(), t_tuple([t_non_neg_integer(),
                              t_integer()])]
  end
  ...
  def arg_types(:erlang, :halt, 1) do
    [t_sup([t_non_neg_fixnum(), t_atom(:abort),
                                    t_string()])]
  end
  ...
  def arg_types(:erlang, :error, 3) do
    [t_any(), t_list(), t_list()]
  end
  ...
  def arg_types(:erlang, :make_tuple, 3) do
    [t_non_neg_fixnum(), t_any(),
                             t_list(t_tuple([t_pos_integer(), t_any()]))]
  end

  def arg_types(:erlang, :min, 2) do
    [t_any(), t_any()]
  end
  ...
  def arg_types(:erlang, :raise, 3) do
    class = t_sup([t_atom(:error), t_atom(:exit),
                                       t_atom(:throw)])
    infoList = t_list()
    arityOrArgs = t_any()
    mFA = t_tuple([t_module(), t_atom(), arityOrArgs])
    mFAS = t_tuple([t_module(), t_atom(), arityOrArgs,
                                              infoList])
  ...
  def arg_types(:erlang, :system_info, 1) do
    [t_sup([t_atom(), t_tuple([t_atom(), t_any()]),
                          t_tuple([t_atom(), t_atom(), t_any()]),
                              t_tuple([t_atom(:allocator_sizes), t_reference(),
                                                                     t_any()])])]
  end
  ...
  def arg_types(:lists, :keymap, 3) do
    [t_fun([t_any()], t_any()), t_pos_fixnum(),
                                    t_list(t_tuple())]
  end
  ...
  def arg_types(:lists, :mapfoldl, 3) do
    [t_fun([t_any(), t_any()], t_tuple([t_any(), t_any()])),
         t_any(), t_list()]
  end
  ...
  def arg_types(:maps, :from_keys, 2) do
    [t_list(), t_any()]
  end
  ...
  def arg_types(m, f, a) when (is_atom(m) and is_atom(f) and
                 ...
                 true ->
                   case (t_tuple_subtypes(x, opaques)) do
                     ...
                     list when length(list) >= 1 ->
                       t_is_atom(y, opaques) and opaque_recargs(list, y,
                                                                  opaques)
                   end
                 false ->
  ...
  defp kind_of_check(:erlang, f, a) do
    case (:erl_internal.guard_bif(f,
                                    a) or :erl_internal.bool_op(f, a)) do
      true ->
        :subtype
      false ->
    ...
    case (t_is_fun(fun, opaques)) do
      true ->
        case (t_fun_args(fun, opaques)) do
          :unknown ->
            case (t_is_impossible(t_fun_range(fun, opaques))) do
              true ->
                :error
              false ->
                :ok
          ...
          funDom when length(funDom) === length(args) ->
            case (any_is_none_or_unit(inf_lists(funDom, args,
                                                  opaques))) do
              true ->
                :error
              false ->
  ...
  defp t_internal_cpu_topology() do
    t_sup(t_list(t_tuple([t_atom(:cpu), t_non_neg_fixnum(),
                                            t_non_neg_fixnum(),
                                                t_non_neg_fixnum(),
                                                    t_non_neg_fixnum(),
                                                        t_non_neg_fixnum(),
  ...
  defp t_scheduler_bind_type_results() do
    t_sup([t_atom(:no_node_processor_spread),
               t_atom(:no_node_thread_spread), t_atom(:no_spread),
  ...
  defp t_system_multi_scheduling() do
    t_sup([t_atom(:blocked), t_atom(:disabled),
                                 t_atom(:enabled)])
  end
