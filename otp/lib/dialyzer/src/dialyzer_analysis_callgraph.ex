defmodule :m_dialyzer_analysis_callgraph do
  ...
  defp analysis_start(parent, analysis, legalWarnings) do
    ...
    modTypeDeps =
      :dict.from_list(
        :maps.to_list(
          :dialyzer_typegraph.module_type_deps(
            r_analysis(analysis, :use_contracts),
            cServer,
            modules
          )
        )
    ...
    modDeps =
      :dialyzer_callgraph.merge_module_deps(
        modCallDeps,
        modTypeDeps
      )
  ...
  defp create_analysis_state({plt, analysis, parent}, legalWarnings, cServer) do
    r_analysis_state(
      codeserver: cServer,
    ...
    (fn ->
       ...
       remMods =
         for f <- files do
           case startFrom do
             :byte_code ->
               :erlang.list_to_atom(:filename.basename(f, ~c".beam"))

             :src_code ->
               :erlang.list_to_atom(:filename.basename(f, ~c".erl"))
           end
         end
       ...
       tmpServer4 =
         (
           :dialyzer_timing.start_stamp(timingServer, ~c"remote")
  ...
  defp compile_and_store(
    ...
    send_log(parent, ~c"Reading files and computing callgraph... ")
    ...
    {{failed, modules}, nextLabel} =
      (
        :dialyzer_timing.start_stamp(timing, ~c"compile")
    ...
    case failed === [] do
        ...
        modDict =
          :lists.foldl(
            fn f, dict ->
              ...
              mod = :filename.basename(modFile, ~c".beam")
      ...
      false ->
        msg =
          :io_lib.format(
            ~c"Could not scan the following file(s):~n~ts",
            [
    ...
    msg1 = :io_lib.format(~c"done in ~.2f secs\nRemoving edges... ", [(t2 - t1) / 1000])
    send_log(parent, msg1)

    {modCallDeps, ^callgraph} =
      (
        :dialyzer_timing.start_stamp(timing, ~c"clean")
    ...
    msg2 = :io_lib.format(~c"done in ~.2f secs\n", [(t3 - t2) / 1000])
  ...
  defp cleanup_callgraph(
    ...
    badCalls2 =
      for call = {_From3, to} <- nonLocalCalls,
    ...
    cond do
      ...
      true ->
        extCallsWithFileAndLocation =
          for {from, to} <- realExtCalls do
            {to,
             find_call_file_and_location(
               from,
               to,
               codeServer
             )}
  ...
  defp check_for_duplicate_modules(modDict) do
    ...
    case :dict.size(duplicates) === 0 do
      ...
      false ->
        ...
        msg = :io_lib.format(~c"Duplicate modules: ~p", [mods])
        exit({:error, msg})
    end
  end

  defp default_includes(dir) do
    l1 = [~c"..", ~c"../incl", ~c"../inc", ~c"../include"]
                    ...
                    true ->
                      case {:cerl.concrete(cA1), :cerl.concrete(cA2), :cerl.concrete(cA3)} do
                        ^toMFA ->
                          ...
                          [
                            {get_file(codeServer, module, ann), get_location(subTree), fromMFA}
                            | acc
                          ]
  ...
  defp dump_callgraph(_CallGraph, _State, r_analysis(callgraph_file: ~c"")) do
    :ok
  end

  defp dump_callgraph(callGraph, state, r_analysis(callgraph_file: file) = analysis) do
    extension = :filename.extension(file)
    start_Msg = :io_lib.format(~c"Dumping the callgraph... ", [])
    send_log(r_analysis_state(state, :parent), start_Msg)
    {t1, _} = :erlang.statistics(:wall_clock)
    dump_callgraph(callGraph, state, analysis, extension)
    {t2, _} = :erlang.statistics(:wall_clock)
    finish_Msg = :io_lib.format(~c"done in ~2f secs\n", [(t2 - t1) / 1000])
    send_log(r_analysis_state(state, :parent), finish_Msg)
    :ok
  end

  defp dump_callgraph(callGraph, _State, r_analysis(callgraph_file: file), ~c".dot") do
    :dialyzer_callgraph.to_dot(callGraph, file)
  end

  defp dump_callgraph(callGraph, _State, r_analysis(callgraph_file: file), ~c".ps") do
    args = ~c"-Gratio=compress -Gsize=\"100,100\""
  ...
  defp dump_callgraph(callGraph, state, r_analysis(callgraph_file: file), _Ext) do
    case :file.open(file, [:write]) do
      {:ok, fd} ->
        :io.format(fd, ~c"~p", [callGraph])
        :ok = :file.close(fd)

      {:error, reason} ->
        msg = :io_lib.format(~c"Could not open output file ~tp, Reason: ~p\n", [file, reason])
        send_log(r_analysis_state(state, :parent), msg)
    end
  end

  defp dump_mod_deps(_ModDeps, _State, r_analysis(mod_deps_file: ~c"")) do
    :ok
  end

  defp dump_mod_deps(modDeps, state, r_analysis(mod_deps_file: file) = analysis) do
    extension = :filename.extension(file)
    start_Msg = :io_lib.format(~c"Dumping the full module dependencies graph... ", [])
    send_log(r_analysis_state(state, :parent), start_Msg)
    {t1, _} = :erlang.statistics(:wall_clock)
  ...
  defp dump_mod_deps(modDeps, _State, r_analysis(mod_deps_file: file), ~c".dot") do
    :dialyzer_callgraph.mod_deps_to_dot(modDeps, file)
  end

  defp dump_mod_deps(modDeps, _State, r_analysis(mod_deps_file: file), ~c".ps") do
    args = ~c"-Gratio=compress -Gsize=\"100,100\""
  ...
  defp dump_mod_deps(_ModDeps, state, r_analysis(mod_deps_file: file), ext) do
    msg =
      :io_lib.format(
        ~c"Could not write full modules dependencies file ~tp, Reason: Unrecognised file extension '~ts'. Only .dot and .ps are supported\n",
        [file, ext]
      )
