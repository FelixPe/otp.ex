defmodule :m_dialyzer_incremental do
  ...
  Record.defrecord(:r_incremental_state, :incremental_state,
  ...
  Record.defrecord(:r_incrementality_metrics, :incrementality_metrics,
    total_modules: :undefined,
  ...
  def start_report_modules_analyzed(r_options(analysis_type: :incremental) = options) do
    {{ret, warn}, _Changed, analyzed} = start_report_modules_changed_and_analyzed(options)
    {{ret, warn}, analyzed}
  end
  ...
  def start_report_modules_changed_and_analyzed(r_options(analysis_type: :incremental) = options) do
    ...
    case :dialyzer_iplt.check_incremental_plt(initPlt, opts1, files) do
      {:ok, r_iplt_info(files: md5, warning_map: :none), moduleToPathLookup} ->
        ...
        enrich_with_modules_changed(
          do_analysis(:maps.values(moduleToPathLookup), opts1, :dialyzer_plt.new(), pltInfo),
          []
        )

      {:ok, r_iplt_info(warning_map: warningMap, files: md5), moduleToPathLookup} ->
        report_stored_warnings_no_changes(opts1, md5)
        write_module_to_path_lookup(opts1, moduleToPathLookup)

        enrich_with_modules_changed(
          return_existing_errors(
            opts1,
            warningMap
          ),
          []
      ...
      {:differ, md5, _DiffMd5, _ModDeps, :none, moduleToPathLookup} ->
        ...
        enrich_with_modules_changed(
          do_analysis(allFiles, opts1, :dialyzer_plt.new(), pltInfo),
          :undefined
        )
      ...
      {:differ, md5, diffMd5, modDeps, warningMap, moduleToPathLookup} ->
        ...
        changedOrRemovedMods =
          for {_, changedOrRemovedMod} <- diffMd5 do
            changedOrRemovedMod
          end
        ...
        case analFiles === [] do
          true ->
            ...
            :dialyzer_iplt.to_file(
              r_options(opts1, :output_plt),
              plt,
              modDepsInRemainingPlt,
              pltInfo
            )
          ...
          false ->
            ...
            enrich_with_modules_changed(
              do_analysis(analFiles, opts1, plt, pltInfo),
              changedOrRemovedMods
            )
        end
      ...
      {:error, :not_valid} ->
        msg =
          :io_lib.format(
            ~c"The file: ~ts is not a valid PLT file\n~s",
  ...
  defp default_plt_error_msg() do
    ~c"Remove the broken PLT file or point to the correct location.\n"
  end
  ...
  defp init_opts_for_incremental(opts) do
    initPlt =
      case r_options(opts, :init_plts) do
        ...
        plts ->
          msg =
            :io_lib.format(~c"Incremental mode does not support multiple PLT files (~ts)\n", [
              format_plts(plts)
            ])
  ...
  defp assert_metrics_file_valid(r_options(metrics_file: metricsFile)) do
    case check_if_writable(metricsFile) do
      ...
      false ->
        msg = :io_lib.format(~c"    The metrics file ~ts is not writable", [metricsFile])
        cl_error(msg)
    end
  end
  ...
  defp write_metrics_file(r_options(metrics_file: metricsFile), format, args) do
    case :file.write_file(
      ...
      {:error, reason} ->
        msg = :io_lib.format(~c"Could not write metrics file ~ts: ~w\n", [metricsFile, reason])
        throw({:dialyzer_error, msg})
    end
  end

  defp write_metrics_file(
         opts,
         r_incrementality_metrics(
    ...
    reasonDescription =
      case reason do
        :no_stored_warnings_in_plt ->
          ~c"no_stored_warnings_in_plt"

        :plt_built_with_different_version ->
          ~c"plt_built_with_different_version"
        ...
        {:incremental_changes, numChangedModules} ->
          :io_lib.format(~c"incremental_changes\nchanged_or_removed_modules: ~B", [
            numChangedModules
          ])
      end

    write_metrics_file(opts, ~c"total_modules: ~B\nanalysed_modules: ~B\nreason: ~s\n", [
      numTotalModules,
      numAnalysedModules,
      reasonDescription
    ])
  ...
  defp report_new_plt_file(r_options(report_mode: reportMode) = opts, initPlt, md5) do
    ...
    case reportMode do
      ...
      :verbose ->
        :io.format(
          ~c"PLT does not yet exist at ~s, so an analysis must be run for ~w modules to populate it\n",
  ...
  defp report_different_plt_version(r_options(report_mode: reportMode) = opts, md5) do
    ...
    case reportMode do
      ...
      :verbose ->
        :io.format(
          ~c"PLT is for a different Dialyzer version, so an analysis must be run for ~w modules to rebuild it\n",
          [numTotalModules]
        )
    end
  ...
  defp report_stored_warnings_no_changes(r_options(report_mode: reportMode) = opts, md5) do
    ...
    case reportMode do
      ...
      :verbose ->
        :io.format(
          ~c"PLT has fully cached the request, so no additional analysis is needed\n",
          []
        )
    end
  ...
  defp report_stored_warnings_only_safe_removals(
         r_options(report_mode: reportMode) = opts,
         md5,
         removed
       ) do
    numTotalModules = length(md5)
    ...
    case reportMode do
      ...
      :verbose ->
        :io.format(
          ~c"PLT has fully cached the request because nothing depended on the file removed, so no additional analysis is needed\n",
          []
        )
    end
  ...
  defp report_no_stored_warnings(r_options(report_mode: reportMode) = opts, md5) do
    ...
    case reportMode do
      ...
      :verbose ->
        :io.format(
          ~c"PLT does not contain cached warnings, so an analysis must be run for ~w modules to rebuild it\n",
          [length(md5)]
        )
    end
  ...
  defp report_incremental_analysis_needed(r_options(report_mode: reportMode), diffMd5) do
    case reportMode do
      ...
      :normal ->
        :io.format(~c"There have been changes to analyze\n", [])
  ...
  defp report_degree_of_incrementality(
         r_options(report_mode: reportMode) = opts,
         md5,
         changedOrRemovedFiles,
         filesThatNeedAnalysis
       ) do
    numTotalModules = length(md5)
    ...
    reportFun = fn ->
      :io.format(
        ~c"    Of the ~B files being tracked, ~B have been changed or removed, resulting in ~B requiring analysis because they depend on those changes\n",
    ...
    case reportMode do
      ...
      :verbose ->
        ...
        :io.format(
          ~c"    Modules which will be analysed: ~p\n",
          [
  ...
  defp report_change_in_legal_warnings(r_options(report_mode: reportMode) = opts, md5) do
    ...
    reportFun = fn ->
      :io.format(
        ~c"PLT was built for a different set of enabled warnings, so an analysis must be run for ~w modules to rebuild it\n",
        [numTotalModules]
      )
    end
  ...
  defp report_analysis_start(_) do
    :io.format(~c"Proceeding with incremental analysis...")
  end
  ...
  defp report_elapsed_time(t1, t2, r_options(report_mode: reportMode)) do
    case reportMode do
      ...
      _ ->
        ...
        :io.format(~c" done in ~wm~.2fs\n", [mins, secs])
    end
  end

  defp report_md5_diff(list) do
    :io.format(~c"    The PLT information is not up to date:\n", [])
  ...
  defp assert_writable(pltFile) do
    case check_if_writable(pltFile) do
      ...
      false ->
        msg = :io_lib.format(~c"    The PLT file ~ts is not writable", [pltFile])
        cl_error(msg)
    end
  end
  ...
  defp path_to_mod(file) do
    :erlang.list_to_atom(:filename.basename(file, ~c".beam"))
  end
  ...
  defp failed_anal_msg(reason, logCache) do
    msg = ~c"Analysis failed with error:\n" ++ :lists.flatten(reason) ++ ~c"\n"
    ...
    case logCache === [] do
      ...
      false ->
        msg ++ ~c"Last messages in the log cache:\n  " ++ format_log_cache(logCache)
    end
  end

  defp format_log_cache(logCache) do
    str = :lists.append(:lists.reverse(logCache))
    :lists.join(~c"\n  ", :string.lexemes(str, ~c"\n"))
  end

  defp store_warnings(
         r_incremental_state(stored_warnings: storedWarnings) = st,
         warnings
       ) do
  ...
  defp return_value(
    ...
    pLTWarningList =
      for mod <- :maps.keys(pLTWarningMap),
  ...
  defp handle_return_and_print(state, allWarningsMap, outputPltInUse) do
    ...
    case r_incremental_state(state, :erlang_mode) do
      ...
      true ->
        {retValue,
         set_warning_id(
           process_warnings(warningsToReport),
  ...
  defp unknown_warnings_by_module(
         r_incremental_state(
           legal_warnings: legalWarnings,
  ...
  defp unknown_functions(calls) do
    for {mFA, warningInfo = {_, _, {mod, _, _}}} <- calls do
      {mod, {:warn_unknown, warningInfo, {:unknown_function, mFA}}}
    end
  end
  ...
  defp set_warning_id(warnings, eOpt) do
    :lists.map(
      fn {tag, {file, location, _MorMFA}, msg} ->
    ...
    case prWarnings do
      ...
      [_ | _] ->
        ...
        s =
          case format do
            :formatted ->
              opts = [{:filename_opt, fOpt}, {:indent_opt, iOpt}, {:error_location, eOpt}]
