defmodule :m_dialyzer_behaviours do
  ...
  def get_behaviours(attrs) do
    ...
    behaviours =
      :lists.append(
        for {behs, _} <- behaviourListsAndLocation do
          behs
        end
      )

    behLocations =
      for {l1, l} <- behaviourListsAndLocation,
          b <- l1 do
        {b, l}
      end
  ...
  defp check_all_callbacks(
         ...
         behaviour,
         [cb | rest],
         r_state(plt: plt, codeserver: codeserver) = state,
         acc0
       ) do
    {{^behaviour, function, arity}, {{_BehFile, _BehLocation}, callback, xtra}} = cb
    ...
    acc1 =
      case :dialyzer_plt.lookup(plt, cbMFA) do
        ...
        {:value, retArgTypes} ->
          case :dialyzer_codeserver.is_exported(
                 cbMFA,
                 codeserver
               ) do
            true ->
  ...
  defp check_callback(
    ...
    acc1 =
      case not :erl_types.t_is_none(returnType) and
             :erl_types.t_is_none(
               :erl_types.t_inf(
                 returnType,
                 cbReturnType
    ...
    case :dialyzer_codeserver.lookup_mfa_contract(
      ...
      {:ok, {{file, location}, contract, _Xtra}} ->
        ...
        acc3 =
          case not :erl_types.t_is_none(specReturnType) and
                 :erl_types.t_is_none(
                   :erl_types.t_inf(
                     specReturnType,
                     cbReturnType
  ...
  defp add_tag_warning_info(module, {tag, [b | _R]} = warn, state)
    ...
    {^b, location} = :lists.keyfind(b, 1, r_state(state, :behlines))
    ...
    {category, {r_state(state, :filename), location, module}, warn}
  end
