defmodule :m_dialyzer_cl_parse do
  ...
  defp parse_define(arg) do
    case :re.split(arg, ~c"=", [{:return, :list}, :unicode]) do
      [def__, val] ->
        {:ok, tokens, _} = :erl_scan.string(val ++ ~c".")
    ...
    %{
      arguments: [
        ...
        %{
          ...
          nargs: :list,
          help:
            "Search the specified directories recursively for subdirectories containing .erl or .beam files in them, depending on the type of analysis."
        },
        %{
          name: :files,
          long: ~c"-input_list_file",
          type: {:custom, &parse_input_list/1},
          action: :extend,
          help:
            "Specify the name of a file that contains the names of the files to be analyzed (one file name per line)."
        },
        %{
          name: :files_rec,
          long: ~c"-apps",
          type: {:custom, &parse_app/1},
          nargs: :list,
          action: :extend,
          help:
        ...
        %{
          ...
          action: {:store, :quiet},
          default: :normal,
          help: "Make Dialyzer a bit more quiet."
        },
        %{
          name: :report_mode,
          long: ~c"-verbose",
          type: :boolean,
          action: {:store, :verbose},
        ...
        %{
          name: :analysis_type,
          long: ~c"-remove_from_plt",
          type: :boolean,
          action: {:store, :plt_remove},
          help:
        ...
        %{
          name: :mod_deps_file,
          long: ~c"-dump_full_dependencies_graph",
          help:
        ...
        %{
          name: :error_location,
          long: ~c"-error_location",
          type: {:atom, [:column, :line]},
          help:
  ...
  defp postprocess_side_effects(argMap)
       when :erlang.is_map_key(
              :version,
              argMap
            ) do
    :io.format(~c"Dialyzer version " ++ :EFE_TODO_VSN_MACRO ++ ~c"\n")
    :erlang.halt(0)
  end
  ...
  defp postprocess_side_effects(argMap) when :erlang.is_map_key(:pa, argMap) do
    for path <- :erlang.map_get(:pa, argMap) do
      :code.add_patha(path) !== true and cl_error(~c"Bad directory for -pa: " ++ path)
    end
  ...
  defp postprocess_side_effects(argMap) do
    ...
    case :maps.get(:analysis_type, argMap1, :undefined) do
      :plt_info ->
        {:plt_info, Map.put(argMap1, :analysis_type, :plt_check)}
