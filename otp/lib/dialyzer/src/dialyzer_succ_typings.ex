defmodule :m_dialyzer_succ_typings do
  ...
  def analyze_callgraph(callgraph, plt, codeserver, timingServer, solvers) do
    _ = get_success_typings(callgraph, plt, codeserver, timingServer, solvers)
    plt
  end

  def get_warnings(callgraph, plt, docPlt, codeserver, timingServer, solvers) do
    initState = get_success_typings(callgraph, plt, codeserver, timingServer, solvers)
    ...
    modWarns =
      (
        :dialyzer_timing.start_stamp(timingServer, ~c"warning")
  ...
  def find_succ_types_for_scc(sCC0, {codeserver, callgraph, plt, solvers}) do
    ...
    debug_pp_functions(~c"SCC", filteredFunTypes, decoratedFunTypes, callgraph)
  ...
  def refine_one_module(m, {codeServer, callgraph, plt, _Solvers}) do
    ...
    debug_pp_functions(~c"Refine", newFunTypes, decoratedFunTypes, callgraph)
  ...
  def add_to_result(labels, result, {_Codeserver, callgraph, _Plt, _Solver}) do
    for label <- labels do
      lookup_name(label, callgraph)
    end ++ result
  end

  defp get_success_typings(callgraph, plt, codeserver, timingServer, solvers) do
  ...
  defp find_succ_typings(labelledSCCs, state) do
    ...
    updated =
      (
        :dialyzer_timing.start_stamp(timing, ~c"typesig")
        _T = :dialyzer_coordinator.parallel_job(:typesig, labelledSCCs, init, timing)
  ...
  defp refine_succ_typings(modules, state) do
    ...
    updated =
      (
        :dialyzer_timing.start_stamp(timing, ~c"refine")
  ...
  defp updated_types_1([{key, type1} | types1], [{key, type2} | types2], acc) do
    case is_failed_or_not_called_fun(type2) do
      true ->
