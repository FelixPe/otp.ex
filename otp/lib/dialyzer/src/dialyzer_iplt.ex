defmodule :m_dialyzer_iplt do
  use Bitwise
  require Record
  Record.defrecord(:r_incremental_data, :incremental_data, mod_deps: :undefined,
                                            warning_map: :none,
                                            legal_warnings: :none)
  ...
  Record.defrecord(:r_contract, :contract, contracts: [], args: [],
                                    forms: [])
  Record.defrecord(:r_ifile_plt, :ifile_plt, version: '',
                                     module_md5_list: [],
  ...
  def get_default_iplt_filename() do
    case (:os.getenv('DIALYZER_IPLT')) do
      false ->
      ...
      {:ok, rec} ->
        case (check_version(rec)) do
          :error ->
            ...
            {:error, msg}
          :ok ->
            r_ifile_plt(info: compressedInfo, contracts: compressedContracts,
                callbacks: compressedCallbacks, types: compressedTypes,
            ...
            [true, true, true] = (for {eTS, data} <- [{eTSInfo,
                                                         :maps.to_list(fileInfo)},
                                                          {eTSTypes, fileTypes},
                                                              {eTSContracts,
                                                                 :maps.to_list(fileContracts)}] do
                                    :ets.insert(eTS, data)
                                  end)
            true = :ets.insert(eTSCallbacks, callbacksByModule)
            true = :ets.insert(eTSExpTypes,
            ...
            case (returnInfo) do
              ...
              true ->
                ...
                pltInfo = r_iplt_info(files: r_ifile_plt(rec, :module_md5_list),
  ...
  defp check_version(r_ifile_plt(version: :EFE_TODO_VSN_MACRO,
              implementation_md5: implMd5)) do
    case (compute_new_md5(implMd5, [], [],
                            implementation_module_paths())) do
      :ok ->
        :ok
      {:differ, _, _} ->
        :error
  ...
  defp to_file1(fileName, plt, modDeps, pltInfo) do
    to_file_custom_vsn(fileName, plt, modDeps, pltInfo,
                         :none, :none)
  end
  ...
  def merge_warnings(newWarnings, unknownWarnings, oldWarningMap) do
    :maps.merge(convert_to_warning_map(newWarnings,
                                         unknownWarnings),
                  oldWarningMap)
  end

  defp convert_to_warning_map(warningList, unknownWarnings) do
    temp = :lists.foldl(fn {_, {_, _, morMFA}, _} = warn,
                             acc ->
                             update = fn old ->
                                           [warn | old]
                                      end
                             :maps.update_with(get_module(morMFA), update,
                                                 [warn], acc)
                        end,
                          %{}, warningList)
    :lists.foldl(fn {m, warn}, acc ->
                      update = fn old ->
                                    [warn | old]
                               end
                      :maps.update_with(m, update, [warn], acc)
                 end,
                   temp, unknownWarnings)
  end
  ...
  defp find_files_to_remove_and_add(md5, pltModules) do
    oldPltFiles = :gb_sets.from_list(for {name,
                                            _Md5Bin} <- md5 do
                                       name
                                     end)
    newPltFiles = :gb_sets.from_list(pltModules)
    {:gb_sets.to_list(:gb_sets.subtract(oldPltFiles,
                                          newPltFiles)),
       :gb_sets.to_list(:gb_sets.subtract(newPltFiles,
                                            oldPltFiles))}
  end

  defp check_version_and_compute_md5(rec, removeFiles, addFiles,
            moduleToPathLookup) do
    md5 = r_ifile_plt(rec, :module_md5_list)
    case (check_version(rec)) do
      :ok ->
        case (compute_new_md5(md5, removeFiles, addFiles,
                                moduleToPathLookup)) do
          :ok ->
            :ok
          {:differ, newMd5, diffMd5} ->
            incrementalData = get_incremental_data(rec)
            {:differ, newMd5, diffMd5,
               r_incremental_data(incrementalData, :mod_deps),
  ...
  defp compute_new_md5(md5, removeFiles0, addFiles0,
            moduleToPathLookup) do
  ...
  defp compute_new_md5_1(entries, initDiffs, moduleToPathLookup) do
    modules = (for {module, _Md5} <- entries do
                 module
               end)
    existingHashes = (for {_Module, md5} <- entries do
                        md5
                      end)
    files = (for module <- modules do
               :maps.get(module, moduleToPathLookup)
             end)
    newHashes = :dialyzer_utils.p_map(&compute_md5_from_file/1,
                                        files)
    diffs = :lists.zipwith3(fn module, beforeHash,
                                 afterHash ->
                                 case (beforeHash) do
                                   ^afterHash ->
                                     :none
                              ...
                              modules, existingHashes, newHashes)
    diffs1 = initDiffs ++ :lists.filter(fn {:differ, _} ->
                                             true
                                           :none ->
                                             false
                                        end,
  ...
  defp compute_md5_from_files(moduleToPathLookup) do
    {modules,
       files} = :lists.unzip(:maps.to_list(moduleToPathLookup))
    hashes = :dialyzer_utils.p_map(&compute_md5_from_file/1,
                                     files)
  ...
  defp compute_md5_from_file(file) do
    case (:beam_lib.chunks(file, [:debug_info])) do
      {:ok,
        ...
        case (backend.debug_info(:erlang_v1, moduleName, data,
                                   [])) do
          {:ok, code} ->
            stabilisedCode = :lists.filtermap(fn form ->
  ...
  defp make_stable(_,
            {:attribute, anno, :file, {srcFilePath, line}}) do
    {true,
       {:attribute, anno, :file,
          {:filename.basename(srcFilePath), line}}}
  end
  ...
  defp init_diff_list(removeFiles, addFiles) do
    removeSet0 = :sets.from_list(for f <- removeFiles do
                                   beam_file_to_module(f)
                                 end)
    addSet0 = :sets.from_list(for f <- addFiles do
                                beam_file_to_module(f)
                              end)
  ...
  defp init_md5_list(md5, removeFiles, addFiles) do
    mods = (for f <- removeFiles do
              {:remove, beam_file_to_module(f)}
            end) ++ (for f <- addFiles do
  ...
  defp init_md5_list_1([{mod, _Md5} | md5Left],
            [{:remove, mod} | diffLeft], acc) do
    init_md5_list_1(md5Left, diffLeft, acc)
  end

  defp init_md5_list_1([{mod, _Md5} = entry | md5Left],
            [{:add, mod} | diffLeft], acc) do
    init_md5_list_1(md5Left, diffLeft, [entry | acc])
  end

  defp init_md5_list_1([{mod1, _Md5} = entry | md5Left] = md5List,
            [{tag, mod2} | diffLeft] = diffList, acc) do
    case (mod1 < mod2) do
      true ->
  ...
  def pp_mod(mod) when is_atom(mod) do
    ...
    case (:dialyzer_plt.lookup_module(plt, mod)) do
      {:value, list} ->
        :lists.foreach(fn {{_, f, _}, ret, args} ->
