defmodule :m_dialyzer do
  ...
  defp cl_check_init(r_options(analysis_type: analType) = opts) do
    case (analType) do
      ...
      :incremental ->
        {:ok, 0}
      other when other === :succ_typings or
                   other === :plt_check
                 ->
        f = fn () ->
  ...
  defp get_plt_info([pLT | pLTs]) do
    string = (case (:dialyzer_plt.plt_kind(pLT)) do
                :cplt ->
  ...
  defp cl(opts) do
    f = fn () ->
             {ret, _Warnings} = (case (r_options(opts, :analysis_type)) do
                                   :incremental ->
  ...
  def run(opts) do
    {warnings,
       _ModulesAnalyzed} = run_report_modules_analyzed(opts)
    warnings
  end

  def run_report_modules_analyzed(opts) do
    {warnings, _ModulesChanged,
       modulesAnalyzed} = run_report_modules_changed_and_analyzed(opts)
  ...
  def run_report_modules_changed_and_analyzed(opts) do
    try do
      :dialyzer_options.build([{:report_mode, :quiet},
                                   {:erlang_mode, true} | opts])
    catch
      {:dialyzer_error, errorMsg} ->
        :erlang.error({:dialyzer_error,
                         :lists.flatten(errorMsg)})
    else
      {:error, msg} ->
        throw({:dialyzer_error, msg})
      optsRecord ->
        :ok = check_init(optsRecord)
        analysisResult = (case (r_options(optsRecord, :analysis_type)) do
                            :incremental ->
  ...
  def format_warning({_Tag, {file, location}, msg}, opts)
    ...
    f = (case (:proplists.get_value(:filename_opt, opts,
                                      :basename)) do
           :fullpath ->
             file
           :basename ->
  ...
  defp message_to_string({:apply,
             [args, argNs, failReason, sigArgs, sigRet, contract]},
            i, _E) do
    :io_lib.format('Fun application with arguments ~ts ',
                     [a(args, i)]) ++ call_or_apply_to_string(argNs,
                                                                failReason,
                                                                sigArgs, sigRet,
                                                                contract, i)
  end

  defp message_to_string({:app_call,
             [m, f, args, culprit, expectedType, foundType]},
            i, _E) do
    :io_lib.format('The call ~s:~ts~ts requires that ~ts is of type ~ts not ~ts\n',
                     [m, f, a(args, i), c(culprit, i), t(expectedType, i),
                                                           t(foundType, i)])
  end

  defp message_to_string({:bin_construction, [culprit, size, seg, type]},
            i, _E) do
  ...
  defp message_to_string({:call,
             [m, f, args, argNs, failReason, sigArgs, sigRet,
                                                          contract]},
            i, _E) do
    :io_lib.format('The call ~w:~tw~ts ',
                     [m, f, a(args, i)]) ++ call_or_apply_to_string(argNs,
                                                                      failReason,
                                                                      sigArgs,
                                                                      sigRet,
                                                                      contract,
  ...
  defp message_to_string({:fun_app_args, [argNs, args, type]}, i, _E) do
    ...
    :io_lib.format('Fun application with arguments ~ts will fail since the function has type ~ts, which differs in the ~s argument\n',
  ...
  defp message_to_string({:neg_guard_fail, [arg1, infix, arg2]}, i,
            _E) do
  ...
  defp message_to_string({:record_constr, [recConstr, fieldDiffs]}, i,
            _E) do
    :io_lib.format('Record construction ~ts violates the declared type of field ~ts\n',
  ...
  defp message_to_string({:record_match, [pat, type]}, i, _E) do
    :io_lib.format('Matching of ~ts tagged with a record name violates the declared type of ~ts\n', [ps(pat, i), t(type, i)])
  end
  ...
  defp message_to_string({:contract_diff, [m, f, _A, contract, sig]}, i,
            _E) do
    :io_lib.format('Type specification ~ts is not equal to the success typing: ~ts\n',
                     [con(m, f, contract, i), con(m, f, sig, i)])
  end

  defp message_to_string({:contract_subtype, [m, f, _A, contract, sig]},
            i, _E) do
    :io_lib.format('Type specification ~ts is a subtype of the success typing: ~ts\n',
                     [con(m, f, contract, i), con(m, f, sig, i)])
  end

  defp message_to_string({:contract_supertype,
             [m, f, _A, contract, sig]},
            i, _E) do
  ...
  defp message_to_string({:contract_range,
             [contract, m, f, argStrings, location, cRet]},
            i, e) do
    :io_lib.format('The contract ~ts cannot be right because the inferred return for ~tw~ts on position ~s is ~ts\n',
                     [con(m, f, contract, i), f, a(argStrings, i),
                                                     pos(location, e), t(cRet,
                                                                           i)])
  end

  defp message_to_string({:invalid_contract,
             [m, f, a, :none, contract, sig]},
            i, _E) do
  ...
  defp message_to_string({:invalid_contract,
             [m, f, a, invalidContractDetails, contract, sig]},
            i, _E) do
    :io_lib.format('Invalid type specification for function ~w:~tw/~w.\n The success typing is ~ts\n But the spec is ~ts\n~ts',
                     [m, f, a, con(m, f, sig, i), con(m, f, contract, i),
  ...
  defp message_to_string({:contract_with_opaque,
             [m, f, a, opaqueType, sigType]},
            i, _E) do
    :io_lib.format('The specification for ~w:~tw/~w has an opaque subtype ~ts which is violated by the success typing ~ts\n',
                     [m, f, a, t(opaqueType, i), sig(sigType, i)])
  end

  defp message_to_string({:extra_range,
  ...
  defp message_to_string({:missing_range,
             [m, f, a, extraRanges, contrRange]},
            i, _E) do
    :io_lib.format('The success typing for ~w:~tw/~w implies that the function might also return ~ts but the specification return is ~ts\n',
                     [m, f, a, t(extraRanges, i), t(contrRange, i)])
  end

  defp message_to_string({:overlapping_contract, [m, f, a]}, _I, _E) do
    :io_lib.format('Overloaded contract for ~w:~tw/~w has overlapping domains; such contracts cannot establish a dependency between the overloaded input and output types\n', [m, f, a])
  end
  ...
  defp message_to_string({:call_with_opaque,
             [m, f, args, argNs, expArgs]},
            i, _E) do
  ...
  defp message_to_string({:call_without_opaque,
             [m, f, args, expectedTriples]},
            i, _E) do
    :io_lib.format('The call ~w:~tw~ts does not have ~ts\n',
                     [m, f, a(args, i),
                                form_expected_without_opaque(expectedTriples,
                                                               i)])
  end
  ...
  defp message_to_string({:opaque_guard, [arg1, infix, arg2, argNs]}, i,
            _E) do
  ...
  defp message_to_string({:opaque_match, [pat, opaqueType, opaqueTerm]},
            i, _E) do
    term = (cond do
  ...
  defp message_to_string({:opaque_neq, [type, _Op, opaqueType]}, i,
            _E) do
    :io_lib.format('Attempt to test for inequality between a term of type ~ts and a term of opaque type ~ts\n', [t(type, i), t(opaqueType, i)])
  end

  defp message_to_string({:opaque_type_test, [fun, args, arg, argType]},
            i, _E) do
  ...
  defp message_to_string({:opaque_call,
  ...
  defp message_to_string({:callback_type_mismatch, [b, f, a, sT, cT]}, i,
            _E) do
    :io_lib.format('The inferred return type of ~tw/~w ~ts has nothing in common with ~ts, which is the expected return type for the callback of the ~w behaviour\n',
                     [f, a, t('(' ++ sT ++ ')', i), t(cT, i), b])
  end

  defp message_to_string({:callback_arg_type_mismatch,
             [b, f, a, n, sT, cT]},
            i, _E) do
    :io_lib.format('The inferred type for the ~s argument of ~tw/~w (~ts) has nothing in common with ~ts, which is expected type for this argument in the callback of the ~w behaviour\n',
                     [ordinal(n), f, a, t(sT, i), t(cT, i), b])
  end

  defp message_to_string({:callback_spec_type_mismatch,
             [b, f, a, sT, cT]},
            i, _E) do
    :io_lib.format('The return type ~ts in the specification of ~tw/~w has nothing in common with ~ts, which is the expected return type for the callback of the ~w behaviour\n', [t(sT, i), f, a, t(cT, i), b])
  end

  defp message_to_string({:callback_spec_arg_type_mismatch,
             [b, f, a, n, sT, cT]},
            i, _E) do
    :io_lib.format('The specified type for the ~ts argument of ~tw/~w (~ts) has nothing in common with ~ts, which is expected type for this argument in the callback of the ~w behaviour\n',
                     [ordinal(n), f, a, t(sT, i), t(cT, i), b])
  end
  ...
  defp format_invalid_contract_details({invalidArgIdxs, isRangeInvalid}) do
    ...
    argDesc = (case (invalidArgIdxs) do
                 [] ->
                   ''
                 [_] ->
                   :io_lib.format('They do not overlap in the ~ts argument', [argOrd])
                 [_ | _] ->
                   :io_lib.format('They do not overlap in the ~ts arguments', [argOrd])
               end)
    rangeDesc = (case (isRangeInvalid) do
                   true ->
                     'return types do not overlap'
                   false ->
                     ''
                 end)
    case ({argDesc, rangeDesc}) do
      {'', ''} ->
        ''
      {'', [_ | _]} ->
        :io_lib.format(' The ~ts\n', [rangeDesc])
      {[_ | _], ''} ->
        :io_lib.format(' ~ts\n', [argDesc])
      {[_ | _], [_ | _]} ->
        :io_lib.format(' ~ts, and the ~ts\n', [argDesc, rangeDesc])
    end
  end

  defp call_or_apply_to_string(argNs, failReason, sigArgs, sigRet,
            {isOverloaded, contract}, i) do
    positionString = form_position_string(argNs)
    case (failReason) do
      :only_sig ->
        case (argNs === []) do
          ...
          false ->
            :io_lib.format('will never return since it differs in the ~s argument from the success typing arguments: ~ts\n', [positionString, t(sigArgs, i)])
        end
      :only_contract ->
      ...
      :both ->
        :io_lib.format('will never return since the success typing is ~ts -> ~ts and the contract is ~ts\n',
  ...
  defp form_positions(argNs) do
    (case (argNs) do
       ...
       [_, _ | _] ->
         'opaque terms as '
     end) ++ form_position_string(argNs) ++ (case (argNs) do
                                               [_] ->
                                                 ' argument'
                                               [_, _ | _] ->
                                                 ' arguments'
  ...
  defp form_position_string(argNs) do
    case (argNs) do
      ...
      [_, _ | _] ->
        ...
        ', ' ++ head = :lists.flatten(for n <- :lists.reverse(prevs) do
  ...
  defp ordinal(n) when (is_integer(n) and
  ...
  defp sig(src, true) do
    try do
      ...
      {:ok,
         {:attribute, _, :spec,
            {_MFA, types}}} = :erl_parse.parse_form(tokens)
  ...
  defp ts(src) do
    ...
    try do
      ...
      commaInd = [?, | ind]
      indentation(10 - 1) ++ [c1 | :lists.join(commaInd,
                                                 for type <- types do
                                                   pp_type(type)
                                                 end)] ++ [c2]
    catch
  ...
  defp pp_type(type) do
    form = {:attribute, :erl_anno.new(0), :type,
              {:t, type, []}}
  ...
  defp pp_spec(spec) do
    form = {:attribute, :erl_anno.new(0), :spec,
              {{:a, :b, 0}, spec}}
    sig = :erl_pp.form(form,
                         [{:quote_singleton_atom_types, true}])
