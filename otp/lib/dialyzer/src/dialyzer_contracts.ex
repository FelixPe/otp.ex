defmodule :m_dialyzer_contracts do
  ...
  defp check_extraneous([], _SuccType, _Opaques) do
    :ok
  end
  ...
  defp check_extraneous_1(contract, succType, opaques) do
  ...
  defp remove_use({:var, anno, v}, v) do
    {:var, anno, :_}
  end
  ...
  defp t_from_forms_without_remote([{_FType, _Constrs}], _MFA, _File, _RecDict) do
    :unsupported
  end

  defp t_from_forms_without_remote(_Forms, _MFA, _File, _RecDict) do
    :unsupported
  end
  ...
  defp is_subtype(t1, t2, opaques) do
    inf = :erl_types.t_inf(t1, t2, opaques)
    :erl_types.t_is_equal(t1, inf)
  end

  def constraint_form_to_remote_modules([]) do
    []
  end
