defmodule :m_dialyzer_options do
  ...
  def build(opts) do
    ...
    try do
      ...
      errLoc = :proplists.get_value(:error_location, env, :column)
  ...
  defp update_path_from_config() do
    ...
    case is_list(pAs) do
      ...
      false ->
        bad_option(~c"Bad list of paths in config", {:add_pathsa, pAs})
    end
    ...
    case is_list(pZs) do
      ...
      false ->
        bad_option(~c"Bad list of paths in config", {:add_pathsz, pZs})
    end
    ...
    for pA <- pAs do
      case :code.add_patha(pA) do
        ...
        {:error, _} ->
          bad_option(~c"Failed to add path from config", {:add_patha, pA})
      end
    end
    ...
    for pZ <- pZs do
      case :code.add_pathz(pZ) do
        ...
        {:error, _} ->
          bad_option(~c"Failed to add path from config", {:add_pathz, pZ})
      end
    end
  ...
  defp postprocess_opts(opts = r_options()) do
    opts1 =
      case {r_options(opts, :init_plts), r_options(opts, :analysis_type)} do
        {[], :incremental} ->
  ...
  defp check_metrics_file_validity(
         r_options(
  ...
  defp check_metrics_file_validity(
    ...
    bad_option(
      ~c"A metrics filename may only be given when in incremental mode",
      {:metrics_file, fileName}
    )
  end

  defp check_module_lookup_file_validity(
         r_options(
           analysis_type: :incremental,
           module_lookup_file: :none
         )
  ...
  defp check_module_lookup_file_validity(
    ...
    bad_option(
      ~c"A module lookup filename may only be given when in incremental mode",
      {:module_lookup_file, fileName}
    )
  end
  ...
  defp check_file_existence(
         r_options(
           files: files,
           files_rec: filesRec,
           warning_files: warningFiles,
           warning_files_rec: warningFilesRec
         )
       ) do
    ...
    case is_plt_mode(mode) do
      true ->
        case from === :byte_code do
          ...
          false ->
            msg = ~c"Byte code compiled with debug_info is needed to build the PLT"
      ...
      false ->
        case outPLT === :none do
          ...
          false ->
            msg = :io_lib.format(~c"Output PLT cannot be specified in analysis mode ~w", [mode])
            throw({:dialyzer_error, :lists.flatten(msg)})
        end
    end
  end

  defp check_init_plt_kind(
         r_options(
           analysis_type: :incremental,
           init_plts: initPlts
         )
    ...
    runCheck = fn fileName ->
      case :dialyzer_plt.plt_kind(fileName) do
        ...
        :cplt ->
          bad_option(
            ~c"Given file is a classic PLT file, but in incremental mode, an incremental PLT file is expected",
            {:init_plt_file, fileName}
          )

        :bad_file ->
          bad_option(~c"Given file is not a PLT file", {:init_plt_file, fileName})
      end
    end
  ...
  defp check_init_plt_kind(
    ...
    runCheck = fn fileName ->
      case :dialyzer_plt.plt_kind(fileName) do
        ...
        :iplt ->
          bad_option(
            ~c"Given file is an incremental PLT file, but outside of incremental mode, a classic PLT file is expected",
            {:init_plt_file, fileName}
          )
  ...
  defp manage_default_incremental_apps(
         opts =
           r_options(
             analysis_type: :incremental,
             files: [],
  ...
  defp set_default_apps([configElem | moreConfig], opts) do
    case configElem do
      ...
      _ when :erlang.element(1, configElem) === :incremental ->
        bad_option(
          ~c"Given Erlang terms in 'incremental' section could not be understood as Dialyzer config",
          configElem
        )
  ...
  def get_default_config_filename() do
    case :os.getenv(~c"DIALYZER_CONFIG") do
      false ->
        cacheDir = :filename.basedir(:user_config, ~c"erlang")
        :filename.join(cacheDir, ~c"dialyzer.config")
  ...
  defp bad_option(string, term) do
    msg = :io_lib.format(~c"~ts: ~tP", [string, term, 25])
  ...
  defp build_options([{optionName, value} = term | rest], options) do
    case optionName do
      ...
      :analysis_type ->
        newOptions =
          case value do
            ...
            :dataflow ->
              bad_option(~c"Analysis type is no longer supported", term)

            :old_style ->
              bad_option(~c"Analysis type is no longer supported", term)

            other ->
              bad_option(~c"Unknown analysis type", other)
          end
      ...
      :old_style ->
        bad_option(~c"Analysis type is no longer supported", :old_style)
      ...
      _ ->
        bad_option(~c"Unknown dialyzer command line option", term)
    end
  end
  ...
  defp get_app_dirs(apps) do
    bad_option(~c"Use a list of otp applications", apps)
  end
  ...
  defp assert_filenames_form(term, [_ | _]) do
    bad_option(~c"Malformed or non-existing filename", term)
  end
  ...
  defp assert_filenames_exist([fileName | left]) do
    case :filelib.is_file(fileName) or :filelib.is_dir(fileName) do
      ...
      false ->
        bad_option(~c"No such file, directory or application", fileName)
    end
  ...
  defp assert_filename(fileName) do
    bad_option(~c"Malformed or non-existing filename", fileName)
  end
  ...
  defp assert_defines(term, [_ | _]) do
    bad_option(~c"Malformed define", term)
  end
  ...
  defp assert_output_format(term) do
    bad_option(~c"Illegal value for output_format", term)
  end
  ...
  defp assert_filename_opt(term) do
    bad_option(~c"Illegal value for filename_opt", term)
  end
  ...
  defp assert_plt_op(
    ...
    case is_plt_mode(oldVal) and is_plt_mode(newVal) do
      true ->
        bad_option(~c"Options cannot be combined", [oldVal, newVal])
  ...
  defp assert_error_location(term) do
    bad_option(~c"Illegal value for error_location", term)
  end
  ...
  defp assert_solvers([term | _]) do
    bad_option(~c"Illegal value for solver", term)
  end
  ...
  def build_warnings([opt | opts], warnings) do
    newWarnings =
      case opt do
        ...
        :overspecs ->
          s = :ordsets.from_list([:warn_contract_subtype, :warn_contract_missing_return])
          :ordsets.union(s, warnings)

        :underspecs ->
          s = :ordsets.from_list([:warn_contract_supertype, :warn_contract_extra_return])
          :ordsets.union(s, warnings)
        ...
        otherAtom ->
          bad_option(~c"Unknown dialyzer warning option", otherAtom)
      end
