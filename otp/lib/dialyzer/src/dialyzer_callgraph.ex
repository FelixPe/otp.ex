defmodule :m_dialyzer_callgraph do
  ...
  Record.defrecord(:r_contract, :contract, contracts: [], args: [],
                                    forms: [])
  Record.defrecord(:r_callgraph, :callgraph, digraph: :digraph.new(),
                                     active_digraph: :undefined,
                                     esc: :undefined, letrec_map: :undefined,
                                     name_map: :undefined,
                                     rev_name_map: :undefined,
                                     rec_var_map: :undefined,
                                     self_rec: :undefined, calls: :undefined)
  def new() do
    [eTSEsc, eTSNameMap, eTSRevNameMap, eTSRecVarMap,
                                            eTSLetrecMap, eTSSelfRec,
                                                              eTSCalls] = (for n <- [:callgraph_esc,
                                                                                         :callgraph_name_map,
    ...
    r_callgraph(esc: eTSEsc, letrec_map: eTSLetrecMap,
        name_map: eTSNameMap, rev_name_map: eTSRevNameMap,
        rec_var_map: eTSRecVarMap, self_rec: eTSSelfRec,
        calls: eTSCalls)
  end
  ...
  def modules(r_callgraph(digraph: dG)) do
    :ordsets.from_list(for {m, _F,
                              _A} <- digraph_vertices(dG) do
                         m
                       end)
  end

  defp module_postorder(r_callgraph(digraph: dG)) do
    edges = :lists.foldl(&edge_fold/2,
                           :sets.new([{:version, 2}]), digraph_edges(dG))
    modules = :ordsets.from_list(for {m, _F,
                                        _A} <- digraph_vertices(dG) do
                                   m
                                 end)
  ...
  def merge_module_deps(left, right) do
    :dict.merge(fn _Mod, l, r ->
                     :gb_sets.to_list(:gb_sets.union(:gb_sets.from_list(l),
                                                       :gb_sets.from_list(r)))
                end,
                  left, right)
  end
  ...
  def reset_from_funs(funs,
           r_callgraph(digraph: dG, active_digraph: aDG) = cG) do
    active_digraph_delete(aDG)
    subGraph = digraph_reaching_subgraph(funs, dG)
    {newActiveDG,
       labelledPostorder} = condensation(subGraph)
    digraph_delete(subGraph)
    {labelledPostorder, r_callgraph(cG, active_digraph: newActiveDG)}
  end

  def module_postorder_from_funs(funs,
           r_callgraph(digraph: dG, active_digraph: aDG) = cG) do
    active_digraph_delete(aDG)
    subGraph = digraph_reaching_subgraph(funs, dG)
    {pO,
       active} = module_postorder(r_callgraph(cG, digraph: subGraph))
    digraph_delete(subGraph)
  ...
  def scan_core_tree(tree,
           r_callgraph(calls: eTSCalls, esc: eTSEsc,
               letrec_map: eTSLetrecMap, name_map: eTSNameMap,
               rec_var_map: eTSRecVarMap, rev_name_map: eTSRevNameMap,
               self_rec: eTSSelfRec)) do
    build_maps(tree, eTSRecVarMap, eTSNameMap,
                 eTSRevNameMap, eTSLetrecMap)
    {deps0, escapingFuns, calls,
       letrecs} = :dialyzer_dep.analyze(tree)
    true = :ets.insert(eTSCalls, :dict.to_list(calls))
    true = :ets.insert(eTSLetrecMap, :dict.to_list(letrecs))
    true = :ets.insert(eTSEsc,
    ...
    newNamedEdges2 = (for ({from, to} = e) <- namedEdges2,
                            from !== :top, to !== :top do
                        e
                      end)
    newNamedEdges1 = (for ({from, to} = e) <- namedEdges1,
                            from !== :top, to !== :top do
                        e
                      end)
  ...
  defp build_maps(tree, eTSRecVarMap, eTSNameMap, eTSRevNameMap,
            eTSLetrecMap) do
                                 ...
                                 {:erlang, :make_fun, 3} ->
                                   ...
                                   case (:cerl.is_c_atom(cA1) and :cerl.is_c_atom(cA2) and :cerl.is_c_int(cA3)) do
                                     true ->
                                       ...
                                       aA = :cerl.int_val(cA3)
                                       case (:erl_bif_types.is_known(mM, fF,
                                                                       aA)) do
                                         true ->
                                           acc
                                         false ->
  ...
  def mod_deps_to_dot(modDeps, file) do
    depEdges = :lists.flatten(for {mod,
                                     modulesDependingOnIt} <- :dict.to_list(modDeps) do
  ...
  def to_dot(r_callgraph(digraph: dG, esc: esc) = cG, file) do
    ...
    :dialyzer_dot.translate_list(vertices, file, 'CG',
                                   escaping)
  end
  ...
  defp condensation(g) do
    outETS = :ets.new(:callgraph_label_deps_out,
    ...
    labelledPostorder = (for i <- sCCInts do
