defmodule :m_dialyzer_cl do
  ...
  def start_report_modules_analyzed(opts) do
    {{ret, warns}, _ModulesChanged, modulesAnalyzed} =
      start_report_modules_changed_and_analyzed(opts)
    ...
    case r_options(opts, :output_plt) === :none do
      true ->
        case r_options(opts, :init_plts) do
          ...
          plts ->
            msg =
              :io_lib.format(~c"Could not build multiple PLT files: ~ts\n", [format_plts(plts)])
    ...
    case r_options(opts, :output_plt) === :none do
      true ->
        case r_options(opts, :init_plts) do
          ...
          plts ->
            msg =
              :io_lib.format(~c"Could not add to multiple PLT files: ~ts\n", [format_plts(plts)])
  ...
  defp check_plt_aux([plt | plts], opts) do
    case check_plt_aux([plt], opts) do
      {{0, []}, modulesChanged, modulesAnalyzed} ->
        ...
        {{ret, warns}, :ordsets.union(modulesChanged, moreModulesChanged),
      ...
      {{2, warns}, modulesChanged, modulesAnalyzed} ->
        ...
        {{2, warns ++ moreWarns}, :ordsets.union(modulesChanged, moreModulesChanged),
         :ordsets.union(modulesAnalyzed, moreModulesAnalyzed)}
    end
  end
    ...
    case r_options(opts, :output_plt) === :none do
      true ->
        case r_options(opts, :init_plts) do
          ...
          plts ->
            msg =
              :io_lib.format(~c"Could not remove from multiple PLT files: ~ts\n", [
                format_plts(plts)
              ])
  ...
  defp plt_common(r_options(init_plts: [initPlt]) = opts, removeFiles, addFiles) do
    case check_plt(opts, removeFiles, addFiles) do
      ...
      {:differ, md5, diffMd5, modDeps} ->
        ...
        case analFiles === [] do
          true ->
            :dialyzer_cplt.to_file(
              r_options(opts, :output_plt),
              plt,
              modDeps1,
      ...
      {:error, :no_such_file} ->
        msg =
          :io_lib.format(
            ~c"Could not find the PLT: ~ts~n~s",
      ...
      {:error, :not_valid} ->
        msg =
          :io_lib.format(
            ~c"The file: ~ts is not a valid PLT file~n~s",
            [initPlt, default_plt_error_msg()]
          )
      ...
      {:error, :read_error} ->
        msg =
          :io_lib.format(
            ~c"Could not read the PLT: ~ts~n~s",
            [initPlt, default_plt_error_msg()]
          )
      ...
      {:error, {:no_file_to_remove, f}} ->
        msg = :io_lib.format(~c"Could not remove the file ~ts from the PLT: ~ts~n", [f, initPlt])
        cl_error(msg)
    end
  end
  ...
  defp check_plt(r_options(init_plts: [plt]) = opts, removeFiles, addFiles) do
    case :dialyzer_cplt.check_plt(plt, removeFiles, addFiles) do
  ...
  defp report_check(
    ...
    case reportMode do
      ...
      _ ->
        :io.format(~c"  Checking whether the PLT ~ts is up-to-date...", [initPlt])
    end
  end
  ...
  defp report_old_version(
    ...
    case reportMode do
      ...
      _ ->
        :io.put_chars(~c" no\n")
        :io.format(~c"    (the PLT ~ts was built with an old version of Dialyzer)\n", [initPlt])
    end
  end
        ...
        case type do
          :plt_add ->
            ...
            case initPlt === outputPlt do
              true ->
                :io.format(~c"Adding information to ~ts...", [outputPlt])

              false ->
                :io.format(~c"Adding information from ~ts to ~ts...", [initPlt, outputPlt])
            end

          :plt_build ->
            :io.format(~c"Creating PLT ~ts ...", [outputPlt])

          :plt_check ->
            :io.format(~c"Rebuilding the information in ~ts...", [outputPlt])
          ...
          :plt_remove ->
            ...
            case initPlt === outputPlt do
              true ->
                :io.format(~c"Removing information from ~ts...", [outputPlt])

              false ->
                :io.format(~c"Removing information from ~ts to ~ts...", [initPlt, outputPlt])
            end

          :succ_typings ->
            :io.format(~c"Proceeding with analysis...")
        end
    end
  end
  ...
  defp unknown_warnings(state) do
    for {_M, warning} <- unknown_warnings_by_module(state) do
      warning
    end
  end
  ...
  defp unknown_functions(
         r_cl_state(
           external_calls: calls,
           code_server: codeServer
         )
       ) do
    for {mFA, warningInfo = {_, _, {mod, _, _} = warnMFA}} <- calls,
        not :dialyzer_codeserver.is_member_meta_info(
          warnMFA,
          codeServer
        ) do
  ...
  defp print_ext_calls(
    ...
    case calls === [] do
      ...
      false ->
        ...
        case format do
          :formatted ->
            :io.put_chars(output, ~c"Unknown functions:\n")
            do_print_ext_calls(output, calls1, ~c"  ")
  ...
  defp print_ext_types(
    ...
    case types === [] do
      ...
      false ->
        ...
        case format do
          :formatted ->
            :io.put_chars(output, ~c"Unknown types:\n")
            do_print_ext_types(output, types1, ~c"  ")
  ...
  defp do_print_ext_types(output, [{{m, f, a}, {file, location, _}} | t], before) do
    :io.format(output, ~c"~s~tp:~tp/~p (~ts)\n", [before, m, f, a, file_pos(file, location)])
  ...
  defp file_pos(file, 0) do
    :io_lib.format(~c"~ts", [file])
  end

  defp file_pos(file, pos) do
    :io_lib.format(~c"~ts:~s", [file, pos(pos)])
  end
  ...
  defp limit_unknown(unknowns) do
    l =
      for {mFA, {file, line, fromMFA}} <- unknowns do
        {{mFA, file}, {fromMFA, line}}
      end

    for {{mFA, file}, [{fromMFA, line} | _]} <- :dialyzer_utils.family(l) do
