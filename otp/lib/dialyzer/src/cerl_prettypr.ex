defmodule :m_cerl_prettypr do
    ...
    case strip_line(as0) do
      ...
      as ->
        case r_ctxt(ctxt, :noann) do
          false ->
            es = seq(as, floating(text(~c",")), ctxt, &lay_concrete/2)

            follow(
              beside(floating(text(~c"(")), doc),
      ...
      line ->
        cond do
          line > r_ctxt(ctxt, :line) ->
            ...
            txt = :io_lib.format(~c"% Line ~w", [line])
    ...
    case var_name(node) do
      ...
      {n, a} when is_atom(n) and is_integer(a) ->
        beside(
          ...
          beside(text(~c"/"), lay_noann(c_int(a), ctxt))
        )
    end
  end
  ...
  defp lay_tuple(node, ctxt) do
    beside(
      floating(text(~c"{")),
      beside(
        par(seq(tuple_es(node), floating(text(~c",")), ctxt, &lay/2)),
        floating(text(~c"}"))
      )
    )
  end
  ...
  defp lay_map(node, ctxt) do
    ...
    after__ =
      case is_c_map_empty(arg) do
        ...
        false ->
          beside(
            floating(text(~c" | ")),
            beside(lay(arg, ctxt), floating(text(~c"}~")))
          )
      end
    ...
    beside(
      floating(text(~c"~{")),
      beside(
        par(seq(map_es(node), floating(text(~c",")), ctxt, &lay/2)),
        after__
      )
    )
  end
  ...
  defp lay_let(node, ctxt) do
    ...
    d1 =
      par(
        [
          follow(text(~c"let"), beside(v, floating(text(~c" ="))), r_ctxt(ctxt, :sub_indent)),
    ...
    case is_c_let(b) do
      true ->
        sep([beside(d1, floating(text(~c" in"))), d2])

      false ->
        sep([d1, beside(text(~c"in "), d2)])
    end
  end

  defp lay_seq(node, ctxt) do
    d1 = beside(text(~c"do "), lay(seq_arg(node), ctxt))
    b = seq_body(node)
    d2 = lay(b, ctxt)
  ...
  defp lay_apply(node, ctxt) do
    as = seq(apply_args(node), floating(text(~c",")), ctxt, &lay/2)

    beside(
      follow(text(~c"apply"), lay(apply_op(node), ctxt)),
      beside(text(~c"("), beside(par(as), floating(text(~c")"))))
    )
  end

  defp lay_call(node, ctxt) do
    as = seq(call_args(node), floating(text(~c",")), ctxt, &lay/2)
  ...
  defp lay_primop(node, ctxt) do
    as = seq(primop_args(node), floating(text(~c",")), ctxt, &lay/2)

    beside(
      follow(text(~c"primop"), lay(primop_name(node), ctxt), r_ctxt(ctxt, :sub_indent)),
      beside(text(~c"("), beside(par(as), floating(text(~c")"))))
    )
  end
  ...
  defp lay_case(node, ctxt) do
    ...
    sep([
      par(
        [
          follow(text(~c"case"), lay(case_arg(node), ctxt), r_ctxt(ctxt, :sub_indent)),
          text(~c"of")
        ],
        r_ctxt(ctxt, :sub_indent)
      ),
    ...
    h =
      par(
        [
          p,
          follow(
            follow(text(~c"when"), g, r_ctxt(ctxt, :sub_indent)),
            floating(text(~c"->"))
          )
        ],
        r_ctxt(ctxt, :sub_indent)
  ...
  defp lay_alias(node, ctxt) do
    follow(
      beside(lay(alias_var(node), ctxt), text(~c" =")),
  ...
  defp lay_fun(node, ctxt) do
    vs = seq(fun_vars(node), floating(text(~c",")), ctxt, &lay/2)

    par(
      [
        follow(
          text(~c"fun"),
          beside(text(~c"("), beside(par(vs), floating(text(~c") ->")))),
          r_ctxt(ctxt, :sub_indent)
        ),
  ...
  defp lay_try(node, ctxt) do
    ...
    sep([
      follow(text(~c"try"), lay(try_arg(node), ctxt), r_ctxt(ctxt, :body_indent)),
      follow(
        beside(beside(text(~c"of "), vs), floating(text(~c" ->"))),
        lay(try_body(node), ctxt),
        r_ctxt(ctxt, :body_indent)
      ),
      follow(
        beside(beside(text(~c"catch "), evs), floating(text(~c" ->"))),
  ...
  defp lay_catch(node, ctxt) do
    follow(text(~c"catch"), lay(catch_body(node), ctxt), r_ctxt(ctxt, :sub_indent))
  end
  ...
  defp lay_letrec(node, ctxt) do
    ...
    sep([
      ...
      beside(text(~c"in "), lay(letrec_body(node), ctxt))
    ])
  end

  defp lay_module(node, ctxt) do
    xs = seq(module_exports(node), floating(text(~c",")), ctxt, &lay_noann/2)
    as = seq(module_attrs(node), floating(text(~c",")), ctxt, &lay_attrdef/2)
    ...
    sep([
      follow(
        ...
        follow(
          ...
          beside(beside(text(~c"["), par(xs)), floating(text(~c"]")))
        ),
        r_ctxt(ctxt, :sub_indent)
      ),
      nest(
  ...
  defp lay_binary(node, ctxt) do
    beside(
      floating(text(~c"\#{")),
      beside(
        sep(seq(binary_segments(node), floating(text(~c",")), ctxt, &lay_bitstr/2)),
        floating(text(~c"}#"))
      )
    )
  end
  ...
  defp lay_bitstr(node, ctxt) do
    head =
      beside(
        floating(text(~c"#<")),
        beside(lay(bitstr_val(node), ctxt), floating(text(~c">")))
      )
    ...
    beside(
      ...
      beside(
        floating(text(~c"(")),
        beside(
          sep(seq(as, floating(text(~c",")), ctxt, &lay/2)),
          floating(text(~c")"))
        )
      )
    )
  ...
  defp lay_value_list(es, ctxt) do
    beside(
      floating(text(~c"<")),
      beside(
        par(seq(es, floating(text(~c",")), ctxt, &lay/2)),
        floating(text(~c">"))
      )
    )
  end
  ...
  defp lay_attrdef({k, v}, ctxt) do
    follow(
      beside(lay_noann(k, ctxt), floating(text(~c" ="))),
      lay_noann(v, ctxt),
      r_ctxt(ctxt, :body_indent)
    )
  end
  ...
  defp lay_fdef({n, f}, ctxt) do
    par(
      [
        beside(lay(n, ctxt), floating(text(~c" ="))),
        lay(
          f,
          ctxt
        )
    ...
    case is_c_cons(t) do
      ...
      _ ->
        case is_c_nil(t) do
          ...
          _ ->
            [h, beside(floating(text(~c"| ")), lay(t, ctxt))]
        end
    end
  end
