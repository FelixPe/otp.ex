defmodule :m_cerl_prettypr do
  ...
  import :cerl, only: [abstract: 1, alias_pat: 1,
                         alias_var: 1, apply_args: 1, apply_op: 1, atom_lit: 1,
                         binary_segments: 1, bitstr_flags: 1, bitstr_size: 1,
                         bitstr_type: 1, bitstr_unit: 1, bitstr_val: 1,
                         c_atom: 1, c_binary: 1, c_bitstr: 5, c_int: 1,
                         call_args: 1, call_module: 1, call_name: 1,
                         case_arg: 1, case_clauses: 1, catch_body: 1,
                         clause_body: 1, clause_guard: 1, clause_pats: 1,
                         concrete: 1, cons_hd: 1, cons_tl: 1, float_lit: 1,
                         fun_body: 1, fun_vars: 1, get_ann: 1, int_lit: 1,
                         is_c_cons: 1, is_c_let: 1, is_c_map_empty: 1,
                         is_c_nil: 1, is_c_seq: 1, is_print_string: 1,
                         let_arg: 1, let_body: 1, let_vars: 1, letrec_body: 1,
                         letrec_defs: 1, map_arg: 1, map_es: 1, map_pair_key: 1,
                         ...
                         try_evars: 1, try_handler: 1, try_vars: 1, tuple_es: 1,
                         type: 1, values_es: 1, var_name: 1]
  import :prettypr, only: [above: 2, beside: 2, empty: 0,
                             floating: 1, follow: 2, follow: 3, nest: 2, par: 1,
                             par: 2, sep: 1, text: 1]
  require Record
  Record.defrecord(:r_ctxt, :ctxt, line: 0, body_indent: 4,
                                sub_indent: 2, hook: :none, noann: false,
                                paper: 76, ribbon: 45, user: :undefined)
  def format(node) do
    format(node, [])
  end
  ...
  defp lay_literal(node, ctxt) do
    case (concrete(node)) do
      ...
      v when is_bitstring(v) ->
        ...
        sz = fn i when is_integer(i) ->
                  8
                b when is_bitstring(b) ->
                  bit_size(b)
             end
        lay_binary(c_binary(for b <- :erlang.bitstring_to_list(v) do
                              c_bitstr(abstract(val.(b)), abstract(sz.(b)),
                                         abstract(1), abstract(:integer),
  ...
  defp lay_var(node, ctxt) do
    case (var_name(node)) do
      ...
      {n, a} when (is_atom(n) and is_integer(a)) ->
        beside(lay_noann(c_atom(:erlang.atom_to_list(n)), ctxt),
  ...
  defp lay_tuple(node, ctxt) do
    beside(floating(text('{')),
             beside(par(seq(tuple_es(node), floating(text(',')), ctxt,
                              &lay/2)),
                      floating(text('}'))))
  end
  ...
  defp lay_map(node, ctxt) do
    arg = map_arg(node)
    after__ = (case (is_c_map_empty(arg)) do
                 true ->
                   floating(text('}~'))
                 false ->
                   beside(floating(text(' | ')),
    ...
    beside(floating(text('~{')),
             beside(par(seq(map_es(node), floating(text(',')), ctxt,
                              &lay/2)),
                      after__))
  end
  ...
  defp lay_map_pair(node, ctxt) do
    k = map_pair_key(node)
    v = map_pair_val(node)
    opTxt = (case (concrete(map_pair_op(node))) do
               :assoc ->
                 '=>'
               :exact ->
                 ':='
  ...
  defp lay_let(node, ctxt) do
    ...
    d1 = par([follow(text('let'), beside(v, floating(text(' ='))),
                       r_ctxt(ctxt, :sub_indent)),
  ...
  defp lay_primop(node, ctxt) do
    as = seq(primop_args(node), floating(text(',')), ctxt,
               &lay/2)
    beside(follow(text('primop'), lay(primop_name(node), ctxt),
                    r_ctxt(ctxt, :sub_indent)),
             beside(text('('), beside(par(as), floating(text(')')))))
  end
  ...
  defp lay_case(node, ctxt) do
    ...
    sep([par([follow(text('case'), lay(case_arg(node), ctxt),
                       r_ctxt(ctxt, :sub_indent)),
                  text('of')],
               r_ctxt(ctxt, :sub_indent)),
             nest(r_ctxt(ctxt, :sub_indent), vertical(cs)), text('end')])
  end
  ...
  defp lay_alias(node, ctxt) do
    follow(beside(lay(alias_var(node), ctxt), text(' =')),
             lay(alias_pat(node), ctxt), r_ctxt(ctxt, :body_indent))
  end

  defp lay_fun(node, ctxt) do
    vs = seq(fun_vars(node), floating(text(',')), ctxt,
               &lay/2)
    par([follow(text('fun'),
  ...
  defp lay_receive(node, ctxt) do
    ...
    sep([text('receive'), nest(r_ctxt(ctxt, :sub_indent), vertical(cs)),
                      sep([follow(text('after'),
                                    beside(lay(receive_timeout(node), ctxt),
                                             floating(text(' ->'))),
                                    r_ctxt(ctxt, :sub_indent)),
                               nest(r_ctxt(ctxt, :sub_indent),
  ...
  defp lay_try(node, ctxt) do
    ...
    sep([follow(text('try'), lay(try_arg(node), ctxt),
                  r_ctxt(ctxt, :body_indent)),
             follow(beside(beside(text('of '), vs), floating(text(' ->'))),
                      lay(try_body(node), ctxt), r_ctxt(ctxt, :body_indent)),
                 follow(beside(beside(text('catch '), evs), floating(text(' ->'))),
                          lay(try_handler(node), ctxt), r_ctxt(ctxt, :body_indent))])
  end

  defp lay_catch(node, ctxt) do
    follow(text('catch'), lay(catch_body(node), ctxt),
             r_ctxt(ctxt, :sub_indent))
  end

  defp lay_letrec(node, ctxt) do
    es = seq(letrec_defs(node), :none, ctxt, &lay_fdef/2)
    sep([text('letrec'), nest(r_ctxt(ctxt, :sub_indent), vertical(es)),
             ...
             nest(r_ctxt(ctxt, :sub_indent),
                    follow(text('attributes'),
                             beside(beside(text('['), par(as)),
                                      floating(text(']'))),
                             r_ctxt(ctxt, :sub_indent))),
                 nest(r_ctxt(ctxt, :sub_indent), vertical(es)), text('end')])
  end
  ...
  defp lay_bitstr(node, ctxt) do
    ...
    as = [bitstr_size(node), bitstr_unit(node),
                                 bitstr_type(node), bitstr_flags(node)]
    beside(head,
             beside(floating(text('(')),
                      beside(sep(seq(as, floating(text(',')), ctxt, &lay/2)),
                               floating(text(')')))))
  end
  ...
  defp lay_value_list(es, ctxt) do
    beside(floating(text('<')),
             beside(par(seq(es, floating(text(',')), ctxt, &lay/2)),
                      floating(text('>'))))
  end
  ...
  defp lay_attrdef({k, v}, ctxt) do
    follow(beside(lay_noann(k, ctxt), floating(text(' ='))),
             lay_noann(v, ctxt), r_ctxt(ctxt, :body_indent))
  end

  defp lay_fdef({n, f}, ctxt) do
    par([beside(lay(n, ctxt), floating(text(' ='))), lay(f,
                                                        ctxt)],
          r_ctxt(ctxt, :body_indent))
  end
  ...
  defp lay_list_elements(node, ctxt) do
    ...
    case (is_c_cons(t)) do
      true when a === [] ->
        [beside(h, floating(text(','))) | lay_list_elements(t,
                                                            ctxt)]
      _ ->
        case (is_c_nil(t)) do
          true when a === [] ->
