defmodule :m_dialyzer_dataflow do
  use Bitwise
  import :erl_types, only: [any_none: 1, t_any: 0,
                              t_atom: 0, t_atom: 1, t_atom_vals: 1,
                              t_atom_vals: 2, t_binary: 0, t_bitstr: 0,
                              t_bitstr: 2, t_bitstr_concat: 1,
                              t_bitstr_match: 2, t_boolean: 0, t_cons: 0,
                              t_cons: 2, t_cons_hd: 2, t_cons_tl: 2,
                              t_contains_opaque: 2, t_find_opaque_mismatch: 3,
                              t_float: 0, t_from_range: 2, t_from_term: 1,
                              t_fun: 0, t_fun: 2, t_fun_args: 1, t_fun_args: 2,
                              t_fun_range: 1, t_fun_range: 2, t_inf: 2,
                              t_inf: 3, t_inf_lists: 2, t_inf_lists: 3,
                              t_integer: 0, t_integers: 1, t_is_any: 1,
                              t_is_any_atom: 3, t_is_atom: 1, t_is_atom: 2,
                              t_is_boolean: 2, t_is_equal: 2,
                              t_is_impossible: 1, t_is_integer: 2, t_is_list: 1,
                              t_is_nil: 2, t_is_none: 1, t_is_number: 2,
                              t_is_pid: 2, t_is_port: 2, t_is_reference: 2,
                              t_is_singleton: 2, t_is_subtype: 2, t_is_unit: 1,
                              t_limit: 2, t_list: 0, t_list_elements: 2,
                              t_map: 0, t_map: 1, t_maybe_improper_list: 0,
                              t_module: 0, t_non_neg_integer: 0, t_none: 0,
                              t_number: 0, t_number_vals: 2, t_pid: 0,
                              t_port: 0, t_product: 1, t_reference: 0,
                              t_subtract: 2, t_sup: 1, t_sup: 2, t_to_string: 2,
                              t_to_tlist: 1, t_tuple: 0, t_tuple: 1,
                              t_tuple_args: 1, t_tuple_args: 2,
                              t_tuple_subtypes: 2, t_unit: 0, t_unopaque: 2]
  require Record
  ...
  Record.defrecord(:r_state, :state, callgraph: :undefined,
                                 codeserver: :undefined, envs: :undefined,
                                 fun_tab: :undefined, fun_homes: :undefined,
                                 reachable_funs: :undefined, plt: :undefined,
                                 opaques: :undefined, records: :dict.new(),
                                 tree_map: :undefined, warning_mode: false,
                                 warnings: [], work: :undefined,
                                 module: :undefined, curr_fun: :undefined)
  Record.defrecord(:r_map, :map, map: :maps.new(),
                               subst: :maps.new(), modified: [],
                               modified_stack: [], ref: :undefined)
  def get_warnings(tree, plt, callgraph, codeserver, records) do
    state1 = analyze_module(tree, plt, callgraph,
                              codeserver, records, true)
  ...
  def get_fun_types(tree, plt, callgraph, codeserver, records) do
    state = analyze_module(tree, plt, callgraph, codeserver,
                             records, false)
    state__all_fun_types(state)
  end

  defp analyze_module(tree, plt, callgraph, codeserver, records,
            getWarnings) do
    debug_pp(tree, false)
    module = :cerl.atom_val(:cerl.module_name(tree))
    topFun = :cerl.ann_c_fun([{:label, :top}], [], tree)
    state = state__new(callgraph, codeserver, topFun, plt,
                         module, records)
  ...
  defp analyze_loop(state) do
    case (state__get_work(state)) do
      ...
      {fun, newState0} ->
        newState1 = state__set_curr_fun(get_label(fun),
                                          newState0)
        {argTypes, isCalled} = state__get_args_and_status(fun,
                                                            newState1)
          ...
          false ->
            case (state__fun_env(fun, newState1)) do
              ...
              map ->
                ...
                {newState2, _Map2, bodyType} = traverse(body, map1,
                                                          newState1)
                :ok
                newState3 = state__update_fun_entry(fun, argTypes,
                                                      bodyType, newState2)
                :ok
  ...
  defp traverse(tree, map, state) do
    ...
    case (:cerl.type(tree)) do
      ...
      :binary ->
        ...
        {state1, map1, segTypes} = traverse_list(segs, map,
                                                   state)
      ...
      :catch ->
        {state1, _Map1, _} = traverse(:cerl.catch_body(tree),
                                        map, state)
        {state1, map, t_any()}
      :cons ->
      ...
      :letrec ->
        ...
        foldFun = fn {var, fun}, {accState, accMap} ->
                       {newAccState, newAccMap0, funType} = traverse(fun,
                                                                       accMap,
                                                                       accState)
                       newAccMap = enter_type(var, funType, newAccMap0)
                       {newAccState, newAccMap}
                  end
        {state1, map1} = :lists.foldl(foldFun, {state, map},
                                        defs)
      ...
      :seq ->
        ...
        {state1, map1, argType} = (sMA = traverse(arg, map,
                                                    state))
        case (t_is_impossible(argType)) do
          true ->
            sMA
          false ->
            state2 = maybe_warn_unmatched_return(arg, argType,
                                                   state1)
      ...
      :values ->
        ...
        {state1, map1, esType} = traverse_list(elements, map,
                                                 state)
  ...
  defp maybe_warn_unmatched_return(arg, argType, state) do
    case (state__warning_mode(state)) do
      false ->
        state
      true ->
        case (t_is_any(argType) or t_is_simple(argType,
                                                 state) or is_call_to_send(arg) or is_lc_simple_list(arg,
                                                                                                       argType,
                                                                                                       state)) do
          true ->
            state
          false ->
            state__add_warning(state, :warn_umatched_return, arg,
                                 {:unmatched_return,
  ...
  defp handle_apply(tree, map, state) do
    ...
    {state0, map1, argTypes} = traverse_list(args, map,
                                               state)
    ...
    case (any_none(argTypes)) do
      true ->
        {state1, map2, t_none()}
      false ->
        funList = (case (state__lookup_call_site(tree,
                                                   state)) do
                     :error ->
                       [:external]
                     {:ok, list} ->
        ...
        funInfoList = (for fun <- funList do
                         {:local, state__fun_info(fun, state)}
                       end)
        case (handle_apply_or_call(funInfoList, args, argTypes,
                                     map2, tree, state1)) do
          {:had_external, state2} ->
            arity = length(args)
                ...
                case (any_none(newArgs)) do
                  true ->
                    enumNewArgs = :lists.zip(:lists.seq(1, length(newArgs)),
                                               newArgs)
                    argNs = (for {arg, type} <- enumNewArgs,
                                   t_is_none(type) do
                               arg
                             end)
                    msg = {:fun_app_args,
                             [argNs, format_args(args, argTypes, state),
                  ...
                  false ->
                    ...
                    range = (case (t_is_unit(range0)) do
                               true ->
                                 t_none()
                               false ->
                                 range0
  ...
  defp handle_apply_or_call(funInfoList, args, argTypes, map, tree,
            state) do
    none = t_none()
    handle_apply_or_call(funInfoList, args, argTypes, map,
                           tree, state,
                           for _ <- argTypes do
                             none
                           end,
  ...
  defp handle_apply_or_call([{:local, :external} | left], args, argTypes,
            map, tree, state, _AccArgTypes, _AccRet, _HadExternal,
            warns) do
    {howMany, _} = warns
    newHowMany = (case (howMany) do
                    :none ->
                      :one
                    _ ->
                      :many
    ...
    handle_apply_or_call(left, args, argTypes, map, tree,
                           state, argTypes, t_any(), true, newWarns)
  end

  defp handle_apply_or_call([{typeOfApply, {fun, sig, contr, localRet}} |
               left],
            args, argTypes, map, tree, r_state(opaques: opaques) = state,
            accArgTypes, accRet, hadExternal, warns) do
    any = t_any()
    anyArgs = (for _ <- args do
                 any
               end)
    genSig = {anyArgs,
                fn _ ->
    ...
    {cArgs, cRange} = (case (contr) do
                         {:value, r_contract(args: as) = c} ->
                           {as,
                              fn funArgs ->
                                   :dialyzer_contracts.get_contract_return(c,
                                                                             funArgs)
                              end}
                         :none ->
                           genSig
                       end)
    {bifArgs, bifRange} = (case (typeOfApply) do
                             :remote ->
                               {m, f, a} = fun
    ...
    :ok
    :ok
    state2 = (case (failedConj and not
                                   (isFailBif or isFailSig)) do
                true ->
                  case (t_is_none(retWithoutLocal) and not
                                                       t_is_none(retWithoutContr) and not
                                                                                      any_none(newArgTypes)) do
                    true ->
                      {:value, c1} = contr
                      ...
                      msg = {:contract_range,
                               [contract, m1, f1, a1, argStrings, cRet]}
                      state__add_warning(state, :warn_contract_range, tree,
                                           msg)
                    false ->
                      failedSig = any_none(newArgsSig)
                      failedContract = any_none([cRange.(newArgsContract) |
                                                     newArgsContract])
                      failedBif = any_none([bifRange.(newArgsBif) |
                                                newArgsBif])
                      infSig = t_inf(t_fun(sigArgs, sigRange),
                                       t_fun(bifArgs, bifRange.(bifArgs)))
                      failReason = apply_fail_reason(failedSig, failedBif,
                                                       failedContract)
                      msg = get_apply_fail_msg(fun, args, argTypes,
                                                 newArgTypes, infSig, contr,
                                                 cArgs, state, failReason,
                                                 opaques)
                      warnType = (case (msg) do
                                    {:call, _} ->
                                      :warn_failing_call
                                    {:apply, _} ->
                                      :warn_failing_call
                      ...
                      frc = {:erlang, :is_record,
                               3} === state__lookup_name(fun, state)
                      state__add_warning(state, warnType, locTree, msg, frc)
                  end
                false ->
                  state
              end)
    state3 = (case (typeOfApply) do
                :local ->
                  case (state__is_escaping(fun, state2)) do
                    true ->
                      state2
                    false ->
                      forwardArgs = (for x <- argTypes do
                                       t_limit(x, 3)
                                     end)
    ...
    newAccArgTypes = (case (failedConj) do
                        true ->
                          accArgTypes
                        false ->
    ...
    totalRet = (case (t_is_none(localRet) and t_is_unit(retWithoutLocal)) do
                  true ->
                    retWithoutLocal
                  false ->
    ...
    :ok
    {newWarnings,
       state4} = state__remove_added_warnings(state, state3)
    {howMany, oldWarnings} = warns
    newWarns = (case (howMany) do
                  :none ->
                    {:one, newWarnings}
                  _ ->
                    case (oldWarnings === []) do
    ...
    handle_apply_or_call(left, args, argTypes, map, tree,
                           state4, newAccArgTypes, newAccRet, hadExternal,
                           newWarns)
  end

  defp handle_apply_or_call([], args, _ArgTypes, map, _Tree, state,
            accArgTypes, accRet, hadExternal, {_, warnings}) do
  ...
  defp get_apply_fail_msg(fun, args, argTypes, newArgTypes, sig, contract,
            contrArgs, state, failReason, opaques) do
    argStrings = format_args(args, argTypes, state)
    contractInfo = (case (contract) do
                      {:value, r_contract() = c} ->
    ...
    enumArgTypes = :lists.zip(:lists.seq(1,
                                           length(newArgTypes)),
                                newArgTypes)
    argNs = (for {arg, type} <- enumArgTypes,
                   t_is_none(type) do
               arg
             end)
    case (state__lookup_name(fun, state)) do
      {m, f, a} ->
        case (is_opaque_type_test_problem(fun, args,
                                            newArgTypes, state)) do
          {:yes, arg, argType} ->
            {:opaque_type_test,
               [:erlang.atom_to_list(f), argStrings, format_arg(arg),
                                                         format_type(argType,
                                                                       state)]}
          :no ->
            sigArgs = t_fun_args(sig)
            badOpaque = opaque_problems([sigArgs, contrArgs],
                                          argTypes, opaques, argNs)
            case (:lists.keyfind(:decl, 1, badOpaque)) do
              {:decl, badArgs} ->
                expectedTriples = (case (failReason) do
                                     :only_sig ->
                                       expected_arg_triples(badArgs, sigArgs,
                                                              state)
                                     _ ->
                                       expected_arg_triples(badArgs, contrArgs,
                                                              state)
                                   end)
                {:call_without_opaque,
                   [m, f, argStrings, expectedTriples]}
              false ->
                case (:lists.keyfind(:use, 1, badOpaque)) do
                  {:use, badArgs} ->
                    expectedArgs = (case (failReason) do
                                      :only_sig ->
                                        sigArgs
                                      _ ->
                                        contrArgs
                    ...
                    {:call_with_opaque,
                       [m, f, argStrings, badArgs, expectedArgs]}
                  false ->
                    case (:erl_bif_types.opaque_args(m, f, a, argTypes,
                                                       opaques)) do
                      [] ->
                        {:call,
                           [m, f, argStrings, argNs, failReason,
                                                         format_sig_args(sig,
                                                                           state),
                                                             format_type(t_fun_range(sig),
                                                                           state),
                                                                 contractInfo]}
                      ns ->
      ...
      label when is_integer(label) ->
        {:apply,
           [argStrings, argNs, failReason, format_sig_args(sig,
                                                             state),
                                               format_type(t_fun_range(sig),
                                                             state),
                                                   contractInfo]}
  ...
  defp opaque_problems(contractOrSigList, argTypes, opaques, argNs) do
    argElementList = find_unknown(contractOrSigList,
                                    argTypes, opaques, argNs)
    f = fn 1 ->
             :decl
           2 ->
    ...
    for elementI <- :lists.usort(for {_,
                                        eI} <- argElementList do
                                   eI
                                 end) do
      {f.(elementI),
         :lists.usort(for {argN, eI} <- argElementList,
                            eI === elementI do
                        argN
                      end)}
    end
  ...
  defp find_unknown(contractOrSigList, argTypes, opaques,
            noneArgNs) do
    argNs = :lists.seq(1, length(argTypes))
    for contractOrSig <- contractOrSigList,
          {e1, e2, argN} <- :lists.zip3(contractOrSig, argTypes,
                                          argNs),
          :lists.member(argN, noneArgNs),
          elementI <- :erl_types.t_find_unknown_opaque(e1, e2,
                                                         opaques) do
      {argN, elementI}
    end
  end
  ...
  defp type_test_opaque_arg([arg | args], [argType | argTypes], opaques) do
    case (:erl_types.t_has_opaque_subtype(argType,
                                            opaques)) do
      true ->
        {:yes, arg, argType}
      false ->
  ...
  defp add_bif_warnings({:erlang, op, 2}, [t1, t2] = ts, tree, state)
    ...
    case (t_is_none(inf) and not any_none(ts) and not
                                                  is_int_float_eq_comp(t1, op,
                                                                         t2,
                                                                         opaques)) do
      true ->
        case (:erl_types.t_find_unknown_opaque(t1, t2,
                                                 opaques)) do
          [] ->
          ...
          ns ->
            ...
            state__add_warning(state, :warn_opaque, tree,
                                 {:opaque_eq, args})
        end
      false ->
        state
  ...
  defp add_bif_warnings({:erlang, op, 2}, [t1, t2] = ts, tree, state)
      when op === :"=/=" or op === :"/=" do
    opaques = r_state(state, :opaques)
    case (not any_none(ts) and not
                               is_int_float_eq_comp(t1, op, t2, opaques)) do
      true ->
        case (:erl_types.t_find_unknown_opaque(t1, t2,
                                                 opaques)) do
          [] ->
  ...
  defp is_int_float_eq_comp(t1, op, t2, opaques) do
    (op === :"==" or op === :"/=") and (:erl_types.t_is_float(t1,
                                                          opaques) and t_is_integer(t2,
                                                                                      opaques) or t_is_integer(t1,
                                                                                                                 opaques) and :erl_types.t_is_float(t2,
                                                                                                                                                      opaques))
  end
  ...
  defp handle_bitstr(tree, map, state) do
    ...
    case (:cerl.bitstr_bitsize(tree)) do
      bitSz when bitSz === :all or bitSz === :utf ->
        valType = (case (bitSz) do
                     :all ->
                       true = bitstrType === :binary
                       t_inf(valType0, t_bitstr())
                     :utf ->
                       true = :lists.member(bitstrType,
                                              [:utf8, :utf16, :utf32])
        ...
        case (t_is_none(valType)) do
          true ->
            msg = {:bin_construction,
                     ['value', format_cerl(val), format_cerl(tree),
                                               format_type(valType0, state2)]}
            state3 = state__add_warning(state2,
                                          :warn_bin_construction, val, msg)
            {state3, map3, t_none()}
          false ->
      ...
      bitSz when is_integer(bitSz) or bitSz === :any ->
        ...
        case (any_none([sizeType, valType])) do
          true ->
            {msg, offending} = (case (t_is_none(sizeType)) do
                                  true ->
                                    {{:bin_construction,
                                        ['size', format_cerl(size),
                                                format_cerl(tree),
                                                    format_type(sizeType0,
                                                                  state2)]},
                                       size}
                                  false ->
                                    {{:bin_construction,
                                        ['value', format_cerl(val), format_cerl(tree),
                                                                  format_type(valType0,
                                                                                state2)]},
                                       val}
                                end)
            state3 = state__add_warning(state2,
                                          :warn_bin_construction, offending,
                                          msg)
            {state3, map2, t_none()}
          false ->
            ...
            {state3, type} = (case (t_contains_opaque(sizeType,
                                                        opaques)) do
                                true ->
                                  msg = {:opaque_size,
                                           [format_type(sizeType, state2),
                                                format_cerl(size)]}
                                  {state__add_warning(state2, :warn_opaque,
                                                        size, msg),
                                     t_none()}
                                false ->
                                  case (numberVals) do
  ...
  defp handle_call(tree, map, state) do
    ...
    mFAList = [m, f | args]
    {state1, map1,
       [mType0, fType0 | as]} = traverse_list(mFAList, map,
                                                state)
    opaques = r_state(state, :opaques)
        ...
        state2 = (cond do
                    mOpaque ->
                      ...
                      case (t_is_none(t_inf(t_module(), mType0))) do
                        true ->
                          msg = {:app_call,
                                   [mS, format_cerl(f), format_args(args, as,
                                                                      state1),
                                                            mS,
                                                                format_type(t_module(),
                                                                              state1),
                                                                    format_type(mType0,
                                                                                  state1)]}
                    ...
                    fOpaque ->
                      ...
                      case (t_is_none(t_inf(t_atom(), fType0))) do
                        true ->
                          msg = {:app_call,
                                   [format_cerl(m), fS, format_args(args, as,
                                                                      state1),
                                                            fS,
                                                                format_type(t_atom(),
                                                                              state1),
        ...
        case (t_is_atom(mType)) do
          true ->
            case ({t_atom_vals(mType), t_atom_vals(fType)}) do
              {[mAtom], [fAtom]} ->
                funInfo = [{:remote,
                              state__fun_info({mAtom, fAtom, length(args)},
                                                state1)}]
                handle_apply_or_call(funInfo, args, as, map2, tree,
                                       state1)
              {_MAtoms, _FAtoms} ->
                {state1, map2, t_any()}
            end
  ...
  defp handle_case(tree, map, state) do
    ...
    case (t_is_impossible(argType)) do
      ...
      false ->
        ...
        {mapList, state3, type, warns} = handle_clauses(clauses,
                                                          arg, argType, map2,
                                                          state1)
        doForce = not
                  is_compiler_generated(:cerl.get_ann(tree)) or t_is_none(type)
        state4 = :lists.foldl(fn {t, r, m, f}, s ->
  ...
  defp handle_cons(tree, map, state) do
    ...
    state3 = (case (t_is_none(t_inf(tlType, t_list(),
                                      r_state(state2, :opaques)))) do
                true ->
                  msg = {:improper_list_constr,
                           [format_type(tlType, state2)]}
                  state__add_warning(state2, :warn_non_proper_list, tree,
                                       msg)
                false ->
                  state2
              end)
  ...
  defp handle_let(tree, map, state) do
    ...
    {map0, state0} = (case (:cerl.is_c_var(arg)) do
                        true ->
                          [var] = vars
    ...
    {state1, map1, argTypes} = (sMA = traverse(arg, map0,
                                                 state0))
    case (t_is_impossible(argTypes)) do
      true ->
        sMA
      false ->
        map2 = enter_type_lists(vars, t_to_tlist(argTypes),
                                  map1)
  ...
  defp handle_primop(tree, map, state) do
    case (:cerl.atom_val(:cerl.primop_name(tree))) do
      ...
      :build_stacktrace ->
        {state, map,
           :erl_bif_types.type(:erlang, :build_stacktrace, 0)}
      :dialyzer_unknown ->
        {state, map, t_any()}
      :recv_peek_message ->
      ...
      :recv_wait_timeout ->
        ...
        case (t_is_atom(timeoutType,
                          opaques) and t_atom_vals(timeoutType,
                                                     opaques) === [:infinity]) do
          true ->
  ...
  defp handle_try(tree, map, state) do
    ...
    cond do
      ...
      true ->
        ...
        {succState, succMap,
           succType} = (case (bind_pat_vars(vars, typeList, map2,
                                              state1)) do
                          {:error, _, _, _, _} ->
                            {state1, map__new(), t_none()}
                          {succMap1, varTypes} ->
                            succMap2 = (case (bind_pat_vars_reverse([arg],
                                                                      [t_product(varTypes)],
                                                                      succMap1,
                                                                      state1)) do
                                          {:error, _, _, _, _} ->
        ...
        {state2, excMap2, handlerType} = traverse(handler,
                                                    excMap1, succState)
  ...
  defp handle_map(tree, map, state) do
    ...
    case (t_is_impossible(argType1)) do
      ...
      false ->
        {state2, map2, typePairs,
           exactKeys} = traverse_map_pairs(pairs, map1, state1,
                                             t_none(), [], [])
        insertPair = fn {kV, :assoc, _}, acc ->
                          :erl_types.t_map_put(kV, acc)
                        {kV, :exact, kVTree}, acc ->
                          case (t_is_none(t = :erl_types.t_map_update(kV,
                                                                        acc))) do
                            true ->
        ...
        try do
          ...
          {:none, mapType, {k, _}, kVTree} ->
            msg2 = {:map_update,
                      [format_type(mapType, state2), format_type(k, state2)]}
            {state__add_warning(state2, :warn_map_construction,
                                  kVTree, msg2),
               map2, t_none()}
        else
          resT ->
            bindT = t_map(for k <- exactKeys do
                            {k, t_any()}
                          end)
            case (bind_pat_vars_reverse([arg], [bindT], map2,
                                          state2)) do
              {:error, _, _, _, _} ->
                {state2, map2, resT}
              {map3, _} ->
  ...
  defp traverse_map_pairs([pair | pairs], map, state, shadowKeys, pairAcc,
            keyAcc) do
    key = :cerl.map_pair_key(pair)
    val = :cerl.map_pair_val(pair)
    op = :cerl.map_pair_op(pair)
    {state1, map1, [k, v]} = traverse_list([key, val], map,
                                             state)
    keyAcc1 = (case (:cerl.is_literal(op) and :cerl.concrete(op) === :exact and t_is_singleton(k,
                                                                                                 r_state(state, :opaques)) and t_is_none(t_inf(shadowKeys,
                                                                                                                                           k))) do
                 true ->
                   [k | keyAcc]
                 false ->
          ...
          [tag | left] ->
            case (:cerl.is_c_atom(tag) and is_literal_record(tree)) do
              true ->
                ...
                case (state__lookup_record(tagVal, length(left),
                                             state1)) do
                  :error ->
                      ...
                      false ->
                        case (bind_pat_vars(elements, t_tuple_args(recType),
                          ...
                          {:error, :opaque, errorPat, errorType, opaqueType} ->
                            ...
                            msg = {:opaque_match,
                                     ['record field' ++ name ++ ' declared to be of type ' ++ format_type(errorType,
                                                                      state1),
                                          opaqueStr, opaqueStr]}
                            locTree = hd(errorPat)
  ...
  defp handle_clauses([c | cs], arg, argType, origArgType, mapIn,
            state, caseTypes, acc, warnAcc0) do
    {state1, clauseMap, bodyType, newArgType,
       warnAcc} = do_clause(c, arg, argType, origArgType,
                              mapIn, state, warnAcc0)
    case (t_is_none(bodyType)) do
      true ->
        handle_clauses(cs, arg, newArgType, origArgType, mapIn,
                         state1, caseTypes, acc, warnAcc)
      false ->
        handle_clauses(cs, arg, newArgType, origArgType, mapIn,
                         state1, [bodyType | caseTypes], [clauseMap | acc],
                         warnAcc)
    end
  end

  defp handle_clauses([], _Arg, _ArgType, _OrigArgType, _MapIn, state,
            caseTypes, acc, warnAcc) do
    {:lists.reverse(acc), state, t_sup(caseTypes), warnAcc}
  end

  defp do_clause(c, arg, argType, origArgType, map, state,
            warns) do
    pats = :cerl.clause_pats(c)
    map0 = mark_as_fresh(pats, map)
    map1 = bind_subst(arg, pats, map0)
    bindRes = (case (t_is_none(argType)) do
                 true ->
    ...
    case (bindRes) do
      {:error, _ErrorType, _NewPats, _Type, _OpaqueTerm} ->
        :ok
        newWarns = (case (state__warning_mode(state)) do
                      false ->
                        warns
                      true ->
                        warn = clause_error(state, map1, bindRes, c, pats,
                                              argType)
                        [warn | warns]
                    end)
        {state, map, t_none(), argType, newWarns}
      {map2, patTypes} ->
        map3 = (case (bind_pat_vars_reverse([arg],
                                              [t_product(patTypes)], map2,
                                              state)) do
                  {:error, _, _, _, _} ->
                    map2
                  {newMap, _} ->
        ...
        genType = :dialyzer_typesig.get_safe_underapprox(pats,
                                                           guard)
        newArgType = t_subtract(t_product(t_to_tlist(argType)),
                                  genType)
        case (bind_guard(guard, map3, state)) do
          {:error, reason} ->
            :ok
            warn = clause_guard_error(state, reason, c, pats,
                                        argType)
            {state, map, t_none(), newArgType, [warn | warns]}
          map4 ->
            body = :cerl.clause_body(c)
            {retState, retMap, bodyType} = traverse(body, map4,
                                                      state)
  ...
  defp clause_error(state, map,
            {:error, :maybe_covered, origArgType, _, _}, c, pats,
            _) do
    origArgTypes = get_arg_list(origArgType, pats)
    msg = (case (bind_pat_vars(pats, origArgTypes, map,
                                 state)) do
             {_, _} ->
             ...
             {:error, errorType, _, _, opaqueTerm} ->
               failed_msg(state, errorType, pats, origArgType, pats,
                            origArgType, opaqueTerm)
           end)
    force = false
  ...
  defp clause_error(state, _Map, bindRes, c, pats, argType) do
    ...
    {:error, errorType, newPats, newType,
       opaqueTerm} = bindRes
    msg = failed_msg(state, errorType, pats, argType,
                       newPats, newType, opaqueTerm)
    clause_error_warning(msg, force, c)
  end

  defp failed_msg(state, errorType, pats, type, newPats, newType,
            opaqueTerm) do
    case (errorType) do
      :bind ->
        {:pattern_match,
           [format_patterns(pats), format_type(type, state)]}
      :record ->
        {:record_match,
           [format_patterns(newPats), format_type(newType, state)]}
      :opaque ->
        {:opaque_match,
           [format_patterns(newPats), format_type(newType, state),
    ...
    case (is_compiler_generated(:cerl.get_ann(c))) do
      ...
      true ->
        case (:cerl.clause_pats(c)) do
          [pat] ->
            :cerl.is_c_cons(pat) and :cerl.is_c_var(:cerl.cons_hd(pat)) and :cerl.is_c_var(:cerl.cons_tl(pat)) and does_guard_succeed(c)
          [pat0, pat1] ->
            :cerl.is_c_cons(pat0) and :cerl.is_c_var(:cerl.cons_hd(pat0)) and :cerl.is_c_var(:cerl.cons_tl(pat0)) and :cerl.is_c_var(pat1) and does_guard_succeed(c)
          _ ->
            false
        end
    end
      ...
      :none ->
        msg = (case (pats) do
                 ...
                 [_ | _] ->
                   patString = format_patterns(pats)
                   {:guard_fail_pat,
                      [patString, format_type(argType, state)]}
               end)
  ...
  defp bind_subst_list([arg | argLeft], [pat | patLeft], map) do
    newMap = (case ({:cerl.type(arg), :cerl.type(pat)}) do
                {:var, :var} ->
                  enter_subst(arg, pat, map)
                {:var, :alias} ->
                ...
                {:literal, :literal} ->
                  map
                {t, t} ->
                  bind_subst_list(:lists.flatten(:cerl.subtrees(arg)),
                                    :lists.flatten(:cerl.subtrees(pat)), map)
                _ ->
                  map
              end)
  ...
  defp do_bind_pat_vars([pat | pats], [type | types], map, state, rev,
            acc) do
    :ok
    opaques = r_state(state, :opaques)
    {newMap, typeOut} = (case (:cerl.type(pat)) do
                           :alias ->
                             aliasPat = :dialyzer_utils.refold_pattern(:cerl.alias_pat(pat))
                             var = :cerl.alias_var(pat)
                             map1 = enter_subst(var, aliasPat, map)
                             {map2, [patType]} = do_bind_pat_vars([aliasPat],
                                                                    [type],
                                                                    map1, state,
                                                                    rev, [])
                           ...
                           :binary ->
                             case (rev) do
                               true ->
                                 {map, t_bitstr()}
                               false ->
                                 binType = bind_checked_inf(pat, t_bitstr(),
                                                              type, opaques)
                                 segs = :cerl.binary_segments(pat)
                                 {map1, segTypes} = bind_bin_segs(segs, binType,
                                                                    map, state)
                           ...
                           :cons ->
                             ...
                             {map1,
                                [hdType,
                                     tlType]} = do_bind_pat_vars([:cerl.cons_hd(pat),
                                                                      :cerl.cons_tl(pat)],
                                                                   [t_cons_hd(cons,
                                                                                opaques),
                                                                        t_cons_tl(cons,
                           ...
                           :literal ->
                             ...
                             case (:cerl.is_literal(pat0)) do
                               true ->
                                 literalType = bind_checked_inf(pat,
                                                                  literal_type(pat),
                                                                  type, opaques)
                                 {map, literalType}
                               false ->
                                 {map1, [patType]} = do_bind_pat_vars([pat0],
                                                                        [type],
                                                                        map,
                                                                        state,
                                                                        rev, [])
                           ...
                           :values ->
                             ...
                             {map1, esTypes} = do_bind_pat_vars(es,
                                                                  t_to_tlist(type),
                                                                  map, state,
                                                                  rev, [])
                             {map1, t_product(esTypes)}
                           :var ->
                             varType1 = (case (state__lookup_type_for_letrec(pat,
                                                                               state)) do
                                           :error ->
                                             lookup_type(pat, map)
                                           {:ok, recType} ->
    ...
    do_bind_pat_vars(pats, types, newMap, state, rev,
                       [typeOut | acc])
  end
    ...
    case (rev) do
      ...
      false ->
        foldFun = fn pair, {mapAcc, listAcc} ->
                       ...
                       key = :cerl.map_pair_key(pair)
                       keyType = (case (:cerl.type(key)) do
                                    :var ->
                                      case (state__lookup_type_for_letrec(key,
                                                                            state)) do
                                        :error ->
                       ...
                       {mapAcc1,
                          [valType]} = do_bind_pat_vars([:cerl.map_pair_val(pair)],
                                                          [bind], mapAcc, state,
                                                          rev, [])
        ...
        {map1, pairs} = :lists.foldl(foldFun, {map, []},
                                       :cerl.map_es(pat))
  ...
  defp bind_tuple(pat, type, map, state, opaques, rev) do
    ...
    {isTypedRecord, prototype} = (case (es) do
                                    [] ->
                                      {false, t_tuple([])}
                                    [tag | tags] ->
                                      case (:cerl.is_c_atom(tag) and is_literal_record(pat)) do
                                        true ->
                                          any = t_any()
                                          [_Head | anyTail] = (for _ <- es do
                                                                 any
                                                               end)
                                          tagAtomVal = :cerl.atom_val(tag)
                                          untypedRecord = t_tuple([t_atom(tagAtomVal) |
                                                                       anyTail])
                                          case (state__lookup_record(tagAtomVal,
                                                                       length(tags),
                                                                       state)) do
                                            :error ->
                                              {false, untypedRecord}
    ...
    results = (for subTuple <- subTuples do
                 bind_pat_vars(es, t_tuple_args(subTuple, opaques), mapJ,
                                 state, rev)
               end)
      ...
      false ->
        case (for {m, _} <- results, m !== :error do
          ...
          maps ->
            ...
            tupleType = t_sup(for {m, esTypes} <- results,
                                    m !== :error do
                                t_tuple(esTypes)
                              end)
            {map1, tupleType}
    ...
    case (bitstr_bitsize_type(size)) do
      {:literal, :all} ->
        ...
        {map1, [type]} = do_bind_pat_vars([val], [t], map,
                                            state, false, [])
        type1 = remove_local_opaque_types(type,
                                            r_state(state, :opaques))
        bind_bin_segs(segs, t_bitstr(0, 0), [type1 | acc], map1,
                        state)
      sizeType when segType === :utf8 or segType === :utf16 or
                      segType === :utf32
                    ->
      ...
      _ ->
        {map1, [sizeType]} = do_bind_pat_vars([size],
                                                [t_non_neg_integer()], map,
                                                state, false, [])
        opaques = r_state(state, :opaques)
        numberVals = t_number_vals(sizeType, opaques)
        ...
        {map2, [_]} = do_bind_pat_vars([val], [valConstr], map1,
                                         state, false, [])
        ...
        case (t_is_none(newBinType)) do
          true ->
            bind_error([seg], binType, t_none(), :bind)
          false ->
            bind_bin_segs(segs, newBinType, [type | acc], map2,
                            state)
        end
    end
  end
  ...
  defp bind_checked_inf(pat, expectedType, type, opaques) do
    ...
    case (t_is_impossible(inf)) do
      true ->
        case (t_find_opaque_mismatch(expectedType, type,
                                       opaques)) do
          {:ok, t1, t2} ->
  ...
  defp bind_guard(guard, map, env, eval, state) do
    ...
    case (:cerl.type(guard)) do
      ...
      :case ->
        ...
        bind_guard_case_clauses(arg, clauses, map, env, eval,
                                  state)
      :cons ->
        hd = :cerl.cons_hd(guard)
        tl = :cerl.cons_tl(guard)
        {map1, hdType} = bind_guard(hd, map, env, :dont_know,
                                      state)
        {map2, tlType} = bind_guard(tl, map1, env, :dont_know,
                                      state)
      ...
      :try ->
        ...
        {{handlerMap, handlerType}, handlerE} = (try do
                                                   {bind_guard(:cerl.try_handler(guard),
                                                                 map2, env,
                                                                 eval, state),
                                                      :none}
                                                 catch
        ...
        case (t_is_none(guard_eval_inf(eval, handlerType))) do
          true ->
            bind_guard(:cerl.try_body(guard), map, bodyEnv, eval,
                         state)
          false ->
            {{bodyMap, bodyType}, bodyE} = (try do
                                              {bind_guard(:cerl.try_body(guard),
                                                            map1, bodyEnv, eval,
                                                            state),
                                                 :none}
                                            catch
            ...
            case (t_is_none(sup = t_sup(bodyType, handlerType))) do
              true ->
                fatality = (case ({bodyE, handlerE}) do
                              {{:fatal_fail, _}, _} ->
                                :fatal_fail
                              {_, {:fatal_fail, _}} ->
                                :fatal_fail
      ...
      :tuple ->
        ...
        {map1, es} = bind_guard_list(es0, map, env, :dont_know,
                                       state)
        {map1, t_tuple(es)}
      :map ->
        case (eval) do
      ...
      :let ->
        ...
        bind_guard(:cerl.let_body(guard), map, newEnv, eval,
                     state)
      :values ->
        ...
        type = t_product(for {_, t} <- list do
                           t
                         end)
        {map, type}
      :var ->
  ...
  defp handle_guard_call(guard, map, env, eval, state) do
    ...
    case (mFA) do
      ...
      {:erlang, f, 3} when f === :internal_is_record or
                             f === :is_record
                           ->
      ...
      {:erlang, comp, 2} when comp === :"<" or comp === :"=<" or
                                comp === :">" or comp === :">="
                              ->
      ...
      {:erlang, f, a} ->
        ...
        case (t_is_any(typeTestType)) do
          ...
          false ->
            handle_guard_type_test(guard, typeTestType, map, env,
                                     eval, state)
        end
    end
  end
  ...
  defp handle_guard_gen_fun({m, f, a}, guard, map, env, eval, state) do
    ...
    {map1, as} = bind_guard_list(args, map, env, :dont_know,
                                   state)
    opaques = r_state(state, :opaques)
    ...
    case (t_is_none(bifRet)) do
      ...
      false ->
        ...
        map2 = enter_type_lists(args,
                                  t_inf_lists(bifArgs, as, opaques), map1)
  ...
  defp handle_guard_type_test(guard, typeTestType, map, env, eval, state) do
    ...
    {map1, argType} = bind_guard(arg, map, env, :dont_know,
                                   state)
    case (bind_type_test(eval, typeTestType, argType,
                           state)) do
      :error ->
        :ok
  ...
  defp handle_guard_comp(guard, comp, map, env, eval, state) do
    args = :cerl.call_args(guard)
    [arg1, arg2] = args
    {map1, argTypes} = bind_guard_list(args, map, env,
                                         :dont_know, state)
    opaques = r_state(state, :opaques)
    ...
    case ({type(arg1), type(arg2)}) do
      {{:literal, lit1}, {:literal, lit2}} ->
        case (apply(:erlang, comp,
                      [:cerl.concrete(lit1), :cerl.concrete(lit2)])) do
          true when eval === :pos ->
            {map, t_atom(true)}
      ...
      {{:literal, lit1}, :var} when (isInt1 and isInt2 and
                                       eval === :pos)
                                    ->
        case (bind_comp_literal_var(lit1, arg2, type2, comp,
                                      map1, opaques)) do
          :error ->
            signal_guard_fail(eval, guard, argTypes, state)
          {:ok, newMap} ->
      ...
      {:var, {:literal, lit2}} when (isInt1 and isInt2 and
                                       eval === :pos)
                                    ->
        case (bind_comp_literal_var(lit2, arg1, type1,
                                      invert_comp(comp), map1, opaques)) do
          :error ->
            signal_guard_fail(eval, guard, argTypes, state)
          {:ok, newMap} ->
            {newMap, t_atom(true)}
      ...
      {_, _} ->
        handle_guard_gen_fun({:erlang, comp, 2}, guard, map,
                               env, eval, state)
    end
  end
  ...
  defp bind_comp_literal_var(lit, var, varType, compOp, map, opaques) do
    ...
    newVarType = (case (t_number_vals(varType, opaques)) do
                    :unknown ->
                      range = (case (compOp) do
                                 :"=<" ->
                    ...
                    numberVals ->
                      newNumberVals = (for x <- numberVals,
                                             apply(:erlang, compOp,
                                                     [litVal, x]) do
                                         x
                                       end)
  ...
  defp handle_guard_is_function(guard, map, env, eval, state) do
    args = :cerl.call_args(guard)
    {map1, argTypes0} = bind_guard_list(args, map, env,
                                          :dont_know, state)
    ...
    case (t_is_none(arityType)) do
      ...
      false ->
        funTypeConstr = (case (t_number_vals(arityType,
                                               r_state(state, :opaques))) do
                           :unknown ->
                             t_fun()
                           vals ->
                             t_sup(for x <- vals do
                                     t_fun(:lists.duplicate(x, t_any()),
                                             t_any())
                                   end)
                         end)
        ...
        case (t_is_none(funType)) do
          ...
          false ->
            case (eval) do
              :pos ->
                {enter_type_lists(args, [funType, arityType], map1),
                   t_atom(true)}
              :neg ->
                {map1, t_atom(false)}
              :dont_know ->
  ...
  defp handle_guard_is_record(guard, map, env, eval, state) do
    ...
    {map1, recType} = bind_guard(rec, map, env, :dont_know,
                                   state)
    arityMin1 = arity - 1
    opaques = r_state(state, :opaques)
    tuple = t_tuple([t_atom(tag) |
                         :lists.duplicate(arityMin1, t_any())])
    case (t_is_none(t_inf(tuple, recType, opaques))) do
      true ->
        case (:erl_types.t_has_opaque_subtype(recType,
                                                opaques)) do
          true ->
      ...
      false ->
        tupleType = (case (state__lookup_record(tag, arityMin1,
                                                  state)) do
                       :error ->
                         tuple
    ...
    case ({type(arg1), type(arg2)}) do
      ...
      {{:literal, lit1}, _} when eval === :pos ->
        case (:cerl.concrete(lit1)) do
          atom when is_atom(atom) ->
            bind_eqeq_guard_lit_other(guard, lit1, arg2, map, env,
                                        state)
          [] ->
            bind_eqeq_guard_lit_other(guard, lit1, arg2, map, env,
                                        state)
      ...
      {_, {:literal, lit2}} when eval === :pos ->
        case (:cerl.concrete(lit2)) do
          atom when is_atom(atom) ->
            bind_eqeq_guard_lit_other(guard, lit2, arg1, map, env,
                                        state)
          [] ->
            bind_eqeq_guard_lit_other(guard, lit2, arg1, map, env,
                                        state)
  ...
  defp bind_eq_guard(guard, arg1, arg2, map, env, eval, state) do
    {map1, type1} = bind_guard(arg1, map, env, :dont_know,
                                 state)
    {map2, type2} = bind_guard(arg2, map1, env, :dont_know,
                                 state)
    opaques = r_state(state, :opaques)
    case (t_is_nil(type1, opaques) or t_is_nil(type2,
                                                 opaques) or t_is_atom(type1,
                                                                         opaques) or t_is_atom(type2,
                                                                                                 opaques)) do
      true ->
        bind_eqeq_guard(guard, arg1, arg2, map, env, eval,
                          state)
      false ->
        opArgs = :erl_types.t_find_unknown_opaque(type1, type2,
                                                    opaques)
        case (opArgs === []) do
          true ->
  ...
  defp bind_eqeq_guard_lit_other(guard, arg1, arg2, map, env, state) do
    ...
    case (:cerl.concrete(arg1)) do
      true ->
        {_, type} = (mT = bind_guard(arg2, map, env, :pos,
                                       state))
        case (t_is_any_atom(true, type, opaques)) do
          ...
          false ->
            ...
            signal_guard_fail(eval, guard, [type0, t_atom(true)],
                                state)
        end
      false ->
        ...
        case (t_is_any_atom(false, type, opaques)) do
          ...
          false ->
            {_, type0} = bind_guard(arg2, map, env, eval, state)
            signal_guard_fail(eval, guard, [type0, t_atom(false)],
                                state)
        end
      term ->
  ...
  defp handle_guard_and(guard, map, env, eval, state) do
    ...
    case (eval) do
      ...
      :neg ->
        ...
        case (t_is_any_atom(false, type1,
                              opaques) or t_is_any_atom(false, type2,
                                                          opaques)) do
          true ->
      ...
      :dont_know ->
        mapJ = join_maps_begin(map)
        {map1, type1} = bind_guard(arg1, mapJ, env, :dont_know,
                                     state)
        {map2, type2} = bind_guard(arg2, mapJ, env, :dont_know,
                                     state)
        ...
        case (t_is_none(bool1) or t_is_none(bool2)) do
          ...
          false ->
            ...
            newType = (case ({t_atom_vals(bool1, opaques),
                                t_atom_vals(bool2, opaques)}) do
                         {[true], [true]} ->
                           t_atom(true)
                         {[false], _} ->
                           t_atom(false)
  ...
  defp handle_guard_or(guard, map, env, eval, state) do
    ...
    case (eval) do
      :pos ->
        ...
        case (t_is_any_atom(true, bool1,
                              opaques) and t_is_boolean(bool2,
                                                          opaques) or t_is_any_atom(true,
                                                                                      bool2,
                                                                                      opaques) and t_is_boolean(bool1,
                                                                                                                  opaques)) do
          true ->
  ...
  defp handle_guard_not(guard, map, env, eval, state) do
    ...
    case (eval) do
      ...
      :dont_know ->
        {map1, type} = bind_guard(arg, map, env, :dont_know,
                                    state)
  ...
  defp handle_guard_map(guard, map, env, state) do
    ...
    {map1, argType0} = bind_guard(arg, map, env, :dont_know,
                                    state)
    ...
    case (t_is_impossible(argType1)) do
      true ->
        {map1, t_none()}
      false ->
        {map2, typePairs} = bind_guard_map_pairs(pairs, map1,
                                                   env, state, [])
        {map2,
           :lists.foldl(fn {kV, :assoc}, acc ->
                             :erl_types.t_map_put(kV, acc)
                           {kV, :exact}, acc ->
  ...
  defp bind_guard_map_pairs([pair | pairs], map, env, state, pairAcc) do
    ...
    val = :cerl.map_pair_val(pair)
    op = :cerl.map_pair_op(pair)
    {map1, [k, v]} = bind_guard_list([key, val], map, env,
                                       :dont_know, state)
  ...
  defp signal_guard_fail(eval, guard, argTypes, state) do
    signal_guard_failure(eval, guard, argTypes, :fail,
                           state)
  end

  defp signal_guard_fatal_fail(eval, guard, argTypes, state) do
    signal_guard_failure(eval, guard, argTypes, :fatal_fail,
                           state)
  end
  ...
  defp signal_guard_failure(eval, guard, argTypes, tag, state) do
    ...
    {m, ^f,
       a} = (mFA = {:cerl.atom_val(:cerl.call_module(guard)),
                      f, length(args)})
    opaques = r_state(state, :opaques)
    {kind, xInfo} = (case (:erl_bif_types.opaque_args(m, f,
                                                        a, argTypes,
                                                        opaques)) do
                       [] ->
                         {case (eval) do
    ...
    fArgs = (case (is_infix_op(mFA)) do
               true ->
                 [argType1, argType2] = argTypes
                 [arg1, arg2] = args
                 [format_args_1([arg1], [argType1], state),
                      :erlang.atom_to_list(f), format_args_1([arg2],
                                                               [argType2],
                                                               state)] ++ xInfo
               false ->
  ...
  defp bind_guard_case_clauses(arg, clauses, map0, env, eval, state) do
    ...
    {genMap, genArgType} = bind_guard(arg, map, env,
                                        :dont_know, state)
    bind_guard_case_clauses(genArgType, genMap, arg,
                              clauses1, map, env, eval, t_none(), [], [], state)
  end
  ...
  defp filter_fail_clauses([clause | left]) do
    case (:cerl.clause_pats(clause) === []) do
      true ->
        ...
        case (:cerl.is_literal(body) and :cerl.concrete(body) === :fail or :cerl.is_c_primop(body) and :cerl.atom_val(:cerl.primop_name(body)) === :match_fail) do
          true ->
  ...
  defp bind_guard_case_clauses(genArgType, genMap, argExpr, [clause | left],
            map, env, eval, accType, accMaps, throws, state) do
    pats = :cerl.clause_pats(clause)
    {newMap0, argType} = (case (pats) do
                            [pat] ->
                                ...
                                true ->
                                  try do
                                    case (:cerl.concrete(pat)) do
                                      true ->
                                        bind_guard(argExpr, map, env, :pos,
                                                     state)
                                      false ->
                                        bind_guard(argExpr, map, env, :neg,
                                                     state)
                                      _ ->
                                        {genMap, genArgType}
                                    end
                 ...
                 false ->
                   case (t_is_none(argType)) do
                     true ->
                       :none
                     false ->
                       argTypes = (case (t_is_any(argType)) do
                                     true ->
                                       any = t_any()
                                       for _ <- pats do
                                         any
    ...
    genPatType = :dialyzer_typesig.get_safe_underapprox(pats,
                                                          guard)
    newGenArgType = t_subtract(genArgType, genPatType)
    case (newMap1 === :none or t_is_none(genArgType)) do
      true ->
        bind_guard_case_clauses(newGenArgType, genMap, argExpr,
                                  left, map, env, eval, accType, accMaps,
                                  throws, state)
      false ->
        {newAccType, newAccMaps, newThrows} = (try do
                                                 {newMap2,
                                                    guardType} = bind_guard(guard,
                                                                              newMap1,
                                                                              env,
                                                                              :pos,
                                                                              state)
                                                 case (t_is_none(t_inf(t_atom(true),
                                                                         guardType))) do
                                                   true ->
                                                     throw({:fail, :none})
                                                   false ->
                                                 ...
                                                 {newMap3,
                                                    cType} = bind_guard(:cerl.clause_body(clause),
                                                                          newMap2,
                                                                          env,
                                                                          eval,
                                                                          state)
                                                 ...
                                                 {t_sup(accType, cType),
                                                    [newMap3 | accMaps], throws}
                                               catch
                                                 {:fail, reason} ->
                                                   throws1 = (case (reason) do
                                                                :none ->
                                                                  throws
                                                                _ ->
                                                                  throws ++ [reason]
        ...
        bind_guard_case_clauses(newGenArgType, genMap, argExpr,
                                  left, map, env, eval, newAccType, newAccMaps,
                                  newThrows, state)
    end
  end

  defp bind_guard_case_clauses(_GenArgType, _GenMap, _ArgExpr, [], map, _Env,
            _Eval, accType, accMaps, throws, _State) do
  ...
  defp join_maps_begin(r_map(modified: m, modified_stack: s,
              ref: ref) = map) do
    r_map(map, ref: make_ref(),  modified: [], 
             modified_stack: [{m, ref} | s])
  end
  ...
  defp join_maps_end(maps, mapOut) do
    ...
    true = :lists.all(fn m ->
                        ...
                        maps)
    keys0 = :lists.usort(:lists.append(for m <- maps do
                                         r_map(m, :modified)
                                       end))
    ...
    r_map(out, ref: r1,  modified: r_map(out, :modified) ++ m1, 
             modified_stack: s)
  end
  ...
  defp join_maps_one_key([map | left], key, accType) do
    case (t_is_any(accType)) do
      true ->
        accType
      false ->
        join_maps_one_key(left, key,
                            t_sup(lookup_type(key, map), accType))
    end
  end
  ...
  defp store_map(key, val,
            r_map(map: map, ref: :undefined) = mapRec) do
    r_map(mapRec, map: :maps.put(key, val, map))
  end

  defp store_map(key, val,
            r_map(map: map, modified: mod) = mapRec) do
    r_map(mapRec, map: Map.put(map, key, val), 
                modified: [key | mod])
  end
  ...
  defp store_subst(key, val, r_map(subst: s, modified: mod) = map) do
    r_map(map, subst: Map.put(s, key, val), 
             modified: [key | mod])
  end
  ...
  defp lookup_fun_sig(fun, callgraph, plt) do
    mFAorLabel = (case (:dialyzer_callgraph.lookup_name(fun,
                                                          callgraph)) do
                    :error ->
                      fun
                    {:ok, mFA} ->
  ...
  defp mark_as_fresh([tree | left], map) do
    ...
    {subTrees2, map1} = (case (:cerl.type(tree)) do
                           :bitstr ->
  ...
  defp t_is_simple(argType, state) do
    opaques = r_state(state, :opaques)
    t_is_atom(argType, opaques) or t_is_number(argType,
                                                 opaques) or t_is_port(argType,
                                                                         opaques) or t_is_pid(argType,
                                                                                                opaques) or t_is_reference(argType,
                                                                                                                             opaques) or t_is_nil(argType,
                                                                                                                                                    opaques)
  end
  ...
  defp is_call_to_send(tree) do
    case (:cerl.is_c_call(tree)) do
      ...
      true ->
        ...
        :cerl.is_c_atom(mod) and :cerl.is_c_atom(name) and is_send(:cerl.atom_val(name)) and :cerl.atom_val(mod) === :erlang and arity === 2
    end
  end
  ...
  defp is_lc_simple_list(tree, treeType, state) do
    ...
    :lists.member(:list_comprehension,
                    ann) and t_is_list(treeType) and t_is_simple(t_list_elements(treeType,
                                                                                   opaques),
                                                                   state)
  end

  defp state__new(callgraph, codeserver, tree, plt, module,
            records) do
    opaques = :erl_types.t_opaque_from_records(records)
    {treeMap, funHomes} = build_tree_map(tree, callgraph)
    funs = :dict.fetch_keys(treeMap)
    funTab = init_fun_tab(funs, :dict.new(), treeMap,
                            callgraph, plt)
    exportedFunctions = (for fun <- funs -- [:top],
                               :dialyzer_callgraph.is_escaping(fun, callgraph),
                               :dialyzer_callgraph.lookup_name(fun,
                                                                 callgraph) !== :error do
                           fun
                         end)
    work = init_work(exportedFunctions)
    env = :lists.foldl(fn fun, env ->
                            :dict.store(fun, map__new(), env)
                       end,
                         :dict.new(), funs)
    r_state(callgraph: callgraph, codeserver: codeserver,
        envs: env, fun_tab: funTab, fun_homes: funHomes,
        opaques: opaques, plt: plt, records: records,
        warning_mode: false, warnings: [], work: work,
        tree_map: treeMap, module: module,
  ...
  defp state__set_warning_mode(r_state(tree_map: treeMap, fun_tab: funTab,
              callgraph: callgraph,
              reachable_funs: reachableFuns) = state) do
    :ok
    funs = :dict.fetch_keys(treeMap)
    work = (for fun <- funs -- [:top],
                  :dialyzer_callgraph.lookup_name(fun,
                                                    callgraph) !== :error or :sets.is_element(fun,
                                                                                                reachableFuns) do
              fun
            end)
    r_state(state, work: init_work(work),  fun_tab: funTab, 
               warning_mode: true)
  end
  ...
  defp state__add_warning(r_state(warnings: warnings,
              warning_mode: true) = state,
            tag, tree, msg, force) do
    ann = :cerl.get_ann(tree)
    case (force) do
      true ->
        warningInfo = {get_file(ann, state), get_location(tree),
                         r_state(state, :curr_fun)}
    ...
    case (newWarnings === oldWarnings) do
      ...
      false ->
        {newWarnings -- oldWarnings,
           r_state(newState, warnings: oldWarnings)}
    end
  end
  ...
  defp state__get_warnings(r_state(tree_map: treeMap, fun_tab: funTab,
              callgraph: callgraph, plt: plt,
              reachable_funs: reachableFuns) = state) do
    foldFun = fn {:top, _}, accState ->
                   accState
                 {funLbl, fun}, accState ->
                   accState1 = state__set_curr_fun(funLbl, accState)
                   {notCalled, ret} = (case (:dict.fetch(get_label(fun),
                                                           funTab)) do
                   ...
                   case (notCalled) do
                     true ->
                       case (:dialyzer_callgraph.lookup_name(funLbl,
                                                               callgraph)) do
                         :error ->
                           accState1
                         {:ok, {_M, f, a}} ->
                           msg = {:unused_fun, [f, a]}
                           state__add_warning(accState1, :warn_not_called, fun,
                                                msg)
                       end
                     false ->
                       {name,
                          contract} = (case (:dialyzer_callgraph.lookup_name(funLbl,
                                                                               callgraph)) do
                                         :error ->
                                           {[], :none}
                                         {:ok, {_M, f, a} = mFA} ->
                                           {[f, a],
                                              :dialyzer_plt.lookup_contract(plt,
                                                                              mFA)}
                                       end)
                       case (t_is_none(ret)) do
                         true ->
                           warn = (case (contract) do
                                     :none ->
                           ...
                           case (warn and (:dialyzer_callgraph.lookup_name(funLbl,
                                                                             callgraph) !== :error or :sets.is_element(funLbl,
                                                                                                                         reachableFuns))) do
                             true ->
                               case (classify_returns(fun)) do
                                 :no_match ->
                                   msg = {:no_return, [:no_match | name]}
                                   state__add_warning(accState1,
                                                        :warn_return_no_exit,
                                                        fun, msg)
                                 :only_explicit ->
    ...
    r_state(warnings: warn) = :lists.foldl(foldFun, state,
                                       :dict.to_list(treeMap))
    warn
  end
  ...
  defp state__lookup_type_for_letrec(var, r_state(callgraph: callgraph) = state) do
    label = get_label(var)
    case (:dialyzer_callgraph.lookup_letrec(label,
                                              callgraph)) do
      :error ->
        :error
      {:ok, funLabel} ->
  ...
  defp state__lookup_name(fun, r_state(callgraph: callgraph)) do
    case (:dialyzer_callgraph.lookup_name(fun,
                                            callgraph)) do
      {:ok, mFA} ->
        mFA
      :error ->
  ...
  defp state__lookup_record(tag, arity, r_state(records: records)) do
    case (:erl_types.lookup_record(tag, arity, records)) do
      {:ok, fields} ->
        recType = t_tuple([t_atom(tag) | for {_FieldName,
                                                _Abstr, fieldType} <- fields do
                                           fieldType
                                         end])
        fieldNames = (for {fieldName, _Abstr,
                             _FieldType} <- fields do
                        fieldName
                      end)
        {:ok, recType, fieldNames}
      :error ->
  ...
  defp state__add_reachable(funLbl,
            r_state(reachable_funs: reachableFuns) = state) do
    newReachableFuns = :sets.add_element(funLbl,
                                           reachableFuns)
  ...
  defp build_tree_map(tree, callgraph) do
    ...
    dict0 = :dict.new()
    {dict, homes, _} = :cerl_trees.fold(fun,
                                          {dict0, dict0, []}, tree)
    {dict, homes}
  end
  ...
  defp init_fun_tab([fun | left], dict, treeMap, callgraph, plt) do
    ...
    funEntry = (case (:dialyzer_callgraph.is_escaping(fun,
                                                        callgraph)) do
                  true ->
                  ...
                  false ->
                    {:not_handled,
                       {:lists.duplicate(arity, t_none()), t_unit()}}
                end)
  ...
  defp state__clean_not_called(r_state(fun_tab: funTab) = state) do
    newFunTab = :dict.map(fn :top, entry ->
                               entry
  ...
  defp state__all_fun_types(state) do
    ...
    list = (for {fun, {args, ret}} <- :dict.to_list(tab1) do
              {fun, t_fun(args, ret)}
            end)
  ...
  defp state__update_fun_entry(tree, argTypes, out0,
            r_state(fun_tab: funTab, callgraph: cG, plt: plt) = state) do
    fun = get_label(tree)
    out1 = (cond do
              fun === :top ->
                out0
  ...
  defp state__add_work_from_fun(tree,
            r_state(callgraph: callgraph, tree_map: treeMap) = state) do
    case (get_label(tree)) do
      :top ->
        state
      label when is_integer(label) ->
        case (:dialyzer_callgraph.in_neighbours(label,
                                                  callgraph)) do
          :none ->
            state
          mFAList ->
            labelList = (for mFA <- mFAList do
                           :dialyzer_callgraph.lookup_label(mFA, callgraph)
                         end)
            filteredList = (for {:ok, l} <- labelList,
                                  :dict.is_key(l, treeMap) do
                              l
                            end)
            :ok
            :lists.foldl(fn l, accState ->
  ...
  defp state__fun_info({_, _, _} = mFA, r_state(plt: pLT)) do
    {mFA, :dialyzer_plt.lookup(pLT, mFA),
       :dialyzer_plt.lookup_contract(pLT, mFA), t_any()}
  end

  defp state__fun_info(fun,
            r_state(callgraph: cG, fun_tab: funTab, plt: pLT)) do
    {sig,
       contract} = (case (:dialyzer_callgraph.lookup_name(fun,
                                                            cG)) do
                      :error ->
    ...
    localRet = (case (:dict.fetch(fun, funTab)) do
  ...
  defp forward_args(fun, argTypes,
            r_state(work: work, fun_tab: funTab) = state) do
    {newArgTypes, oldOut, fixpoint} = (case (:dict.find(fun,
                                                          funTab)) do
                                         {:ok,
                                            {:not_handled,
                                               {_OldArgTypesAreNone,
                                         ...
                                         {:ok, {oldArgTypes0, oldOut0}} ->
                                           newArgTypes0 = (for {x,
                                                                  y} <- :lists.zip(argTypes,
                                                                                     oldArgTypes0) do
                                                             t_sup(x, y)
                                                           end)
                                           {newArgTypes0, oldOut0,
                                              t_is_equal(t_product(newArgTypes0),
    ...
    case (fixpoint) do
      ...
      false ->
        newWork = add_work(fun, work)
        :ok
        newFunTab = :dict.store(fun, {newArgTypes, oldOut},
                                  funTab)
  ...
  defp format_args_1([arg | args], [type | types], state) do
    format_arg_1(arg, type,
                   state) ++ ',' ++ format_args_1(args, types, state)
  end
  ...
  defp format_field_diffs(recConstruction, r_state(records: r)) do
    :erl_types.record_field_diffs_to_string(recConstruction,
                                              r)
  end
  ...
  defp format_sig_args(type, r_state(opaques: opaques) = state) do
    ...
    case (sigArgs) do
      [] ->
        '()'
      [sArg | sArgs] ->
        :lists.flatten('(' ++ format_type(sArg,
                                          state) ++ (for t <- sArgs do
                                                       ',' ++ format_type(t,
                                                                          state)
                                                     end) ++ ')')
    end
  ...
  defp format_cerl(tree) do
    :cerl_prettypr.format(:cerl.set_ann(tree, []),
                            [{:hook, :dialyzer_utils.pp_hook()}, {:noann, true},
                                                                     {:paper,
                                                                        100000},
                                                                         {:ribbon,
                                                                            100000}])
  ...
  defp parent_allows_this(funLbl,
            r_state(callgraph: callgraph, plt: plt) = state) do
      ...
      true ->
        case (state__lookup_name(funLbl, state)) do
          {_M, _F, _A} ->
            false
          _ ->
            case (:dialyzer_callgraph.in_neighbours(funLbl,
                                                      callgraph)) do
              [parent] ->
    ...
    case (:cerl.type(tree)) do
      ...
      :call ->
        ...
        case ({is_literal(m0), is_literal(f0)}) do
          {{:yes, litM}, {:yes, litF}} ->
            ...
            case (:erl_bif_types.is_known(m, f,
                                            a) and t_is_none(:erl_bif_types.type(m,
                                                                                   f,
                                                                                   a))) do
              true ->
                {true, false}
      ...
      :case ->
        case (:cerl.case_clauses(tree)) do
          [] ->
            case (:lists.member(:receive_timeout,
                                  :cerl.get_ann(tree))) do
              true ->
                {false, true}
              false ->
                {false, false}
      ...
      :receive ->
        ...
        case (:cerl.is_literal(timeout) and :cerl.concrete(timeout) === :infinity) do
          ...
          false ->
            find_terminals_list([:cerl.receive_action(tree) |
                                     clauses])
        end
      :seq ->
        find_terminals(:cerl.seq_body(tree))
      :try ->
        find_terminals_list([:cerl.try_handler(tree),
                                 :cerl.try_body(tree)])
      :tuple ->
        {false, true}
      :values ->
  ...
  defp find_terminals_list([tree | left], explicit1, normal1) do
    ...
    case ({:erlang.or(explicit1, explicit2),
             :erlang.or(normal1, normal2)}) do
      {true, true} = ans ->
        ans
