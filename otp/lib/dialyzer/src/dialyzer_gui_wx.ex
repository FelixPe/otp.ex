defmodule :m_dialyzer_gui_wx do
  ...
  defp create_window(
    ...
    frame = :wxFrame.new(wx, -1, ~c"Dialyzer " ++ :EFE_TODO_VSN_MACRO ++ ~c" @ " ++ host)
    ...
    :wxMenuBar.append(menuBar, fileMenu, ~c"File")
    :wxMenuBar.append(menuBar, warningsMenu, ~c"Warnings")
    :wxMenuBar.append(menuBar, pltMenu, ~c"Plt")
    :wxMenuBar.append(menuBar, optionsMenu, ~c"Options")
    :wxMenuBar.append(menuBar, helpMenu, ~c"Help")
    :wxFrame.setMenuBar(frame, menuBar)
    :ok = :wxFrame.connect(frame, :command_menu_selected)
    lab1 = :wxStaticText.new(frame, 502, ~c"Directories or modules to analyze")
    optionsLabel = :wxStaticText.new(frame, 503, ~c"Analysis Options")
    logLabel = :wxStaticText.new(frame, 504, ~c"Log")
    fileLabel = :wxStaticText.new(frame, 505, ~c"File: ")
    dirLabel = :wxStaticText.new(frame, 505, ~c"Dir: ")
    warningsLabel = :wxStaticText.new(frame, 506, ~c"Warnings")

    chosenBox =
      :wxListBox.new(frame, 510, [{:size, {250, 200}}, {:style, 128 ||| 1_073_741_824 ||| 0}])
    ...
    filePicker =
      :wxFilePickerCtrl.new(frame, 512, [
        {:path, defaultPath},
        {:message, ~c"Choose File to Analyse"},
        {:style, 8192 ||| 2}
      ])
    ...
    dirPicker =
      :wxDirPickerCtrl.new(frame, 513, [
        {:path, defaultPath},
        {:message, ~c"Choose Directory to Analyze"},
        {:style, 8 ||| 2}
      ])

    warningsBox = :wxListBox.new(frame, 521, [{:size, {700, 200}}, {:style, 1_073_741_824 ||| 0}])
    deleteButton = :wxButton.new(frame, 514, [{:label, ~c"Delete"}])
    deleteAllButton = :wxButton.new(frame, 515, [{:label, ~c"Delete All"}])

    fileType =
      :wxRadioBox.new(frame, 524, ~c" File Type: ", {1, 1}, {150, 90}, [
        [~c"BeamFiles"],
        [~c"SourceFiles"]
      ])

    clearLogButton = :wxButton.new(frame, 516, [{:label, ~c"Clear Log"}])
    addButton = :wxButton.new(frame, 517, [{:label, ~c"Add"}])
    addDirButton = :wxButton.new(frame, 532, [{:label, ~c"Add Dir"}])
    addRecButton = :wxButton.new(frame, 518, [{:label, ~c"Add Recursively"}])
    explainWarnButton = :wxButton.new(frame, 523, [{:label, ~c"Explain Warning"}])
    clearWarningsButton = :wxButton.new(frame, 519, [{:label, ~c"Clear Warnings"}])
    runButton = :wxButton.new(frame, 520, [{:label, ~c"Run"}])
    stopButton = :wxButton.new(frame, 522, [{:label, ~c"Stop"}])
    ...
    _ = :wxSizer.add(buttons, :wxStaticText.new(frame, 507, ~c""), [{:flag, 8192}])
    ...
    warnings = [
      ...
      {:warn_callgraph, 208},
      {:warn_contract_types, 209},
      {:warn_contract_syntax, 210}
    ]
    ...
    initPlt =
      case initPltFiles do
        ...
        _ ->
          ...
          :dialyzer_cplt.merge_plts_or_report_conflicts(
            initPltFiles,
            plts
          )
      end
  ...
  defp createFileMenu() do
    ...
    _ =
      :wxMenu.append(
        fileMenu,
        :wxMenuItem.new([{:id, 100}, {:text, ~c"Save &Warnings"}])
      )

    _ =
      :wxMenu.append(
        fileMenu,
        :wxMenuItem.new([{:id, 101}, {:text, ~c"Save &Log"}])
      )

    _ =
      :wxMenu.append(
        fileMenu,
        :wxMenuItem.new([{:id, 102}, {:text, ~c"E&xit\tAlt-X"}])
      )
  ...
  defp createWarningsMenu() do
    ...
    addCheckedItem(warningsMenu, 200, ~c"Match failures")
    addCheckedItem(warningsMenu, 201, ~c"Failing function calls")
    addCheckedItem(warningsMenu, 202, ~c"Bad fun applications")
    addCheckedItem(warningsMenu, 203, ~c"Opacity violations")
    addCheckedItem(warningsMenu, 204, ~c"Improper list constructions")
    addCheckedItem(warningsMenu, 205, ~c"Unused functions")
    _ = :wxMenu.appendCheckItem(warningsMenu, 206, ~c"Error handling functions")
    addCheckedItem(warningsMenu, 207, ~c"Functions of no return")
    addCheckedItem(warningsMenu, 208, ~c"Call to unexported function")
    addCheckedItem(warningsMenu, 209, ~c"Wrong contracts")
    addCheckedItem(warningsMenu, 210, ~c"Wrong contract syntax")
    warningsMenu
  end
  ...
  defp createPltMenu() do
    pltMenu = :wxMenu.new()
    _ = :wxMenu.appendCheckItem(pltMenu, 300, ~c"Init with empty PLT")

    _ =
      :wxMenu.append(
        pltMenu,
        :wxMenuItem.new([{:id, 301}, {:text, ~c"Show contents"}])
      )

    _ =
      :wxMenu.append(
        pltMenu,
        :wxMenuItem.new([{:id, 302}, {:text, ~c"Search contents"}])
      )
  ...
  defp createOptionsMenu() do
    ...
    _ =
      :wxMenu.append(
        optsMenu,
        :wxMenuItem.new([{:id, 400}, {:text, ~c"Manage Macro Definitions"}])
      )

    _ =
      :wxMenu.append(
        optsMenu,
        :wxMenuItem.new([{:id, 401}, {:text, ~c"Manage Include Directories"}])
      )
  ...
  defp createHelpMenu() do
    ...
    _ =
      :wxMenu.append(
        helpMenu,
        :wxMenuItem.new([{:id, 500}, {:text, ~c"Manual"}])
      )

    _ =
      :wxMenu.append(
        helpMenu,
        :wxMenuItem.new([{:id, 501}, {:text, ~c"Warning Options"}])
      )

    _ =
      :wxMenu.append(
        helpMenu,
        :wxMenuItem.new([{:id, 499}, {:text, ~c"About"}])
      )
  ...
  defp gui_loop(
    ...
    receive do
      r_wx(event: r_wxClose()) ->
        :ok = :wxFrame.setStatusText(frame, ~c"Closing...", [])
      ...
      r_wx(id: 302, obj: ^frame, event: r_wxCommand(type: :command_menu_selected)) ->
        case :dialyzer_plt.get_specs(docPlt) do
          ~c"" ->
            error_sms(state, ~c"No analysis has been made yet!\n")
      ...
      r_wx(id: 500, obj: ^frame, event: r_wxCommand(type: :command_menu_selected)) ->
        handle_help(state, ~c"Dialyzer Manual", ~c"manual.txt")
        gui_loop(state)

      r_wx(id: 501, obj: ^frame, event: r_wxCommand(type: :command_menu_selected)) ->
        handle_help(state, ~c"Dialyzer Warnings", ~c"warnings.txt")
        gui_loop(state)

      r_wx(id: 499, obj: ^frame, event: r_wxCommand(type: :command_menu_selected)) ->
        message =
          ~c"\t       This is DIALYZER version " ++
            :EFE_TODO_VSN_MACRO ++
            ~c" \n" ++
            ~c"DIALYZER is a DIscrepancy AnaLYZer for ERlang programs.\n\n" ++
            ~c"     Copyright (C) Tobias Lindahl <tobiasl@it.uu.se>\n" ++
            ~c"                   Kostis Sagonas <kostis@it.uu.se>\n\n"

        output_sms(state, ~c"About Dialyzer", message, :info)
        gui_loop(state)
      ...
      {^backendPid, :ext_calls, extCalls} ->
        extCalls1 =
          :lists.usort(
            for {mFA, _FileLocation} <- extCalls do
              mFA
            end
          )

        map = fn {m, f, a} ->
          :io_lib.format(~c"\t~tp:~tp/~p", [m, f, a])
        end
        ...
        msg =
          :io_lib.format(
            ~c"The following functions are called but type information about them is not available.\nThe analysis might get more precise by including the modules containing these functions:\n\n~ts\n",
            [extCallString]
          )

        free_editor(state, ~c"Analysis Done", msg)
        gui_loop(state)

      {^backendPid, :ext_types, extTypes} ->
        extTypes1 =
          :lists.usort(
            for {mFA, _FileLocation} <- extTypes do
              mFA
            end
          )
      ...
      {^backendPid, :done, _NewPlt, newDocPlt} ->
        message(state, ~c"Analysis done")
        config_gui_stop(state)
        :dialyzer_plt.delete(r_gui_state(state, :doc_plt))
        gui_loop(r_gui_state(state, doc_plt: newDocPlt))

      {:EXIT, ^backendPid, {:error, reason}} ->
        free_editor(state, ~c"Dialyzer Error", reason)
        config_gui_stop(state)
        gui_loop(state)

      {:EXIT, ^backendPid, reason} when reason !== :normal ->
        free_editor(state, ~c"Dialyzer Error", :io_lib.format(~c"~tp", [reason]))
        config_gui_stop(state)
        gui_loop(state)
    end
  end
  ...
  defp maybe_quit(r_gui_state(frame: frame) = state) do
    case dialog(state, ~c"Do you really want to quit?", ~c"Dialyzer Message") do
      true ->
        :wxWindow.destroy(frame)
        true
  ...
  defp search_doc_plt(r_gui_state(gui: wx) = state) do
    dialog = :wxFrame.new(wx, 535, ~c"Search the PLT", [{:size, {400, 100}}, {:style, 32768}])
    size = {:size, {120, 30}}
    modLabel = :wxStaticText.new(dialog, 536, ~c"Module")
    modText = :wxTextCtrl.new(dialog, 539, [size])
    funLabel = :wxStaticText.new(dialog, 537, ~c"Function")
    funText = :wxTextCtrl.new(dialog, 540, [size])
    arLabel = :wxStaticText.new(dialog, 538, ~c"Arity")
    arText = :wxTextCtrl.new(dialog, 541, [size])
    searchButton = :wxButton.new(dialog, 542, [{:label, ~c"Search"}])
    :wxButton.connect(searchButton, :command_button_clicked)
    cancel = :wxButton.new(dialog, 543, [{:label, ~c"Cancel"}])
        ...
        cond do
          ...
          true ->
            case :dialyzer_plt.get_specs(docPlt, m, f, a) do
              :none ->
                error_sms(state, ~c"No such function")
                search_plt_loop(state, win, modText, funText, arText, search, cancel)

              nonEmptyString ->
                :wxWindow.destroy(win)
                free_editor(state, ~c"Content of PLT", nonEmptyString)
            end
        end
    end
  end
  ...
  defp message(state, message) do
    output_sms(state, ~c"Dialyzer Message", message, :info)
  end

  defp error_sms(state, message) do
    output_sms(state, ~c"Dialyzer Error", message, :error)
  end
  ...
  defp free_editor(r_gui_state(gui: wx, frame: frame), title, contents0) do
    ...
    tokens = :string.lexemes(contents, ~c"\n")
    ...
    ok = :wxButton.new(win, 531, [{:label, ~c"OK"}])
  ...
  defp start_analysis(state) do
    ...
    case get_anal_files(
      ...
      :error ->
        msg = ~c"You must choose one or more files or dirs\nbefore starting the analysis!"
        error_sms(state, msg)
        config_gui_stop(state)
        state
  ...
  defp get_anal_files(r_gui_state(files_to_analyze: files), startFrom) do
    filteredMods =
      case startFrom do
        :src_code ->
          filter_mods(files, ~c".erl")

        :byte_code ->
          filter_mods(files, ~c".beam")
      end
  ...
  defp save_file(
    ...
    case :wxTextCtrl.getValue(box) do
      ~c"" ->
        error_sms(state, ~c"There is nothing to save...\n")
        ...
        case :wxFileDialog.showModal(fileDialog) do
          5100 ->
            ...
            case :wxTextCtrl.saveFile(box, [{:file, path}]) do
              ...
              false ->
                error_sms(state, ~c"Could not write to file:\n" ++ path)
            end
  ...
  defp include_dialog(r_gui_state(gui: wx, frame: frame, options: options)) do
    size = {:size, {300, 480}}
    dialog = :wxFrame.new(wx, 544, ~c"Include Directories", [size])
    dirLabel = :wxStaticText.new(dialog, 545, ~c"Directory: ")
    defaultPath = :code.root_dir()

    dirPicker =
      :wxDirPickerCtrl.new(dialog, 546, [
        {:path, defaultPath},
        {:message, ~c"Choose Directory to Include"},
        {:style, 8 ||| 2}
      ])

    box =
      :wxListBox.new(dialog, 547, [{:size, {200, 300}}, {:style, 128 ||| 1_073_741_824 ||| 0}])

    addButton = :wxButton.new(dialog, 548, [{:label, ~c"Add"}])
    deleteButton = :wxButton.new(dialog, 549, [{:label, ~c"Delete"}])
    deleteAllButton = :wxButton.new(dialog, 550, [{:label, ~c"Delete All"}])
    ok = :wxButton.new(dialog, 551, [{:label, ~c"OK"}])
    cancel = :wxButton.new(dialog, 552, [{:label, ~c"Cancel"}])
    :wxButton.connect(addButton, :command_button_clicked)
    :wxButton.connect(deleteButton, :command_button_clicked)
  ...
  defp macro_dialog(r_gui_state(gui: wx, frame: frame, options: options)) do
    size = {:size, {300, 480}}
    size1 = {:size, {120, 30}}
    dialog = :wxFrame.new(wx, 553, ~c"Macro Definitions", [size])
    macroLabel = :wxStaticText.new(dialog, 554, ~c"Macro")
    termLabel = :wxStaticText.new(dialog, 556, ~c"Term")
    macroText = :wxTextCtrl.new(dialog, 555, [size1])
    termText = :wxTextCtrl.new(dialog, 557, [size1])

    box =
      :wxListBox.new(dialog, 558, [{:size, {250, 300}}, {:style, 128 ||| 1_073_741_824 ||| 0}])

    addButton = :wxButton.new(dialog, 559, [{:label, ~c"Add"}])
    deleteButton = :wxButton.new(dialog, 560, [{:label, ~c"Delete"}])
    deleteAllButton = :wxButton.new(dialog, 561, [{:label, ~c"Delete All"}])
    ok = :wxButton.new(dialog, 562, [{:label, ~c"OK"}])
    cancel = :wxButton.new(dialog, 563, [{:label, ~c"Cancel"}])
    :wxButton.connect(addButton, :command_button_clicked)
    :wxButton.connect(deleteButton, :command_button_clicked)
          ...
          case :wxListBox.getSelections(box) do
            ...
            {_, list} ->
              fun = fn x ->
                ...
                [macroName | _] = :re.split(val, ~c" ", [{:return, :list}, :unicode])
  ...
  defp handle_help(state, title, txt) do
    fileName = :filename.join([:code.lib_dir(:dialyzer), ~c"doc", txt])

    case :file.open(fileName, [:read]) do
      {:error, reason} ->
        error_sms(state, :io_lib.format(~c"Could not find doc/~ts file!\n\n ~tp", [txt, reason]))
      ...
      {:ok, _Handle} ->
        case :file.read_file(fileName) do
          {:error, reason} ->
            error_sms(
              state,
              :io_lib.format(~c"Could not read doc/~ts file!\n\n ~tp", [txt, reason])
            )
    ...
    case explanation do
      :none ->
        output_sms(
          state,
          ~c"Dialyzer Message",
          ~c"There is not any explanation for this error!\n",
          :info
        )
      ...
      expl ->
        ...
        size = {:size, {700, 300}}
        win = :wxFrame.new(wx, 564, ~c"Dialyzer Explanation", [{:size, {740, 350}}])

        editor =
          :wxTextCtrl.new(win, 565, [size, {:style, 32 ||| 16 ||| -2_147_483_648 ||| 8192}])

        :wxTextCtrl.appendText(editor, explString)
        :wxFrame.connect(win, :close_window)
        explButton = :wxButton.new(win, 566, [{:label, ~c"Further Explain"}])
        :wxButton.connect(explButton, :command_button_clicked)
        ok = :wxButton.new(win, 567, [{:label, ~c"OK"}])
        :wxButton.connect(ok, :command_button_clicked)
        layout = :wxBoxSizer.new(8)
        buttons = :wxBoxSizer.new(4)
    ...
    receive do
      {^explPid, :none, _} ->
        output_sms(
          state,
          ~c"Dialyzer Message",
          ~c"There is not any other explanation for this error!\n",
          :info
        )
