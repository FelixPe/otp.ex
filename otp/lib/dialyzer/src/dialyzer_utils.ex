defmodule :m_dialyzer_utils do
    ...
    case :beam_lib.chunks(file, [:debug_info]) do
      {:ok, {module, [{:debug_info, {:debug_info_v1, backend, metadata}}]}} ->
        case backend.debug_info(:core_v1, module, metadata, opts ++ src_compiler_opts()) do
          ...
          {:error, _} ->
            {:error, ~c"  Could not get Core Erlang code for: " ++ file ++ ~c"\n"}
        end

      _ ->
        {:error,
         ~c"  Could not get Core Erlang code for: " ++
           file ++ ~c"\n" ++ ~c"  Recompile with +debug_info or analyze starting from source code"}
    end
  end
  ...
  def get_record_and_type_info(core) do
    ...
    get_record_and_type_info(tuples, module, :maps.new(), ~c"nofile")
  end

  defp get_record_and_type_info(
         [{:record, location, [{name, fields0}]} | left],
         module,
         recDict,
         file
       ) do
  ...
  defp get_record_and_type_info(
         [
           {:type, location, [{{:record, name}, fields0, []}]}
           | left
         ],
         module,
         recDict,
  ...
  defp get_record_and_type_info(
         [{attr, location, [{name, typeForm}]} | left],
         module,
         recDict,
         file
       )
  ...
  defp get_record_and_type_info(
         [
           {attr, location, [{name, typeForm, args}]}
           | left
         ],
         module,
         recDict,
  ...
  defp add_new_type(typeOrOpaque, name, typeForm, argForms, module, fN, recDict) do
    ...
    case :erl_types.type_is_defined(typeOrOpaque, name, arity, recDict) do
      true ->
        msg = flat_format(~c"Type ~ts/~w already defined\n", [name, arity])
        throw({:error, msg})
      ...
      false ->
        try do
          ...
          _, _ ->
            throw(
              {:error,
               flat_format(~c"Type declaration for ~tw does not have variables as parameters", [
                 name
               ])}
            )
        else
  ...
  defp get_record_fields([{:record_field, _Location, name} | left], recDict, acc) do
  ...
  defp get_record_fields(
         [
           {:record_field, _Location, name, _Init}
           | left
         ],
         recDict,
         acc
  ...
  defp get_spec_info(
    ...
    try do
      ...
      {:error, error} ->
        {:error, flat_format(~c"  Error while parsing contract in line ~w: ~ts\n", [ln, error])}
    else
      :error ->
      ...
      {:ok, {{otherFile, l}, _D}} ->
        ...
        msg =
          flat_format(
            ~c"  Contract/callback for function ~w:~tw/~w already defined in ~ts:~w\n",
  ...
  defp get_core_location([{l, c} | _As])
  ...
  defp massage_type(
         {:type, loc, :fun, [{:type, argsLoc, :product, argTypes}, ret0]},
         defs
       ) do
  ...
  defp get_func_suppressions(m, core, functions) do
    ...
    fun = fn
      {{[:nowarn_function], _Loc, _File}, _FA} ->
        :ok
  ...
  defp get_options1([{args, loc, file} | left], warnings) do
    ...
    try do
      ...
      {:dialyzer_options_error, msg} ->
        msg1 = flat_format(~c"  ~ts:~s: ~ts", [file, pos(loc), msg])
        throw({:error, msg1})
    else
      newWarnings ->
  ...
  defp collect_attribute(core, tag) do
    collect_attribute(:cerl.module_attrs(core), tag, ~c"nofile")
  end
  ...
  defp check_fa_list1([{args, loc, file} | left], tag, funcs) do
    ...
    case :lists.dropwhile(
      ...
      [{_, bad} | _] ->
        msg1 = flat_format(~c"  Bad function ~tw at location ~ts:~s", [bad, file, pos(loc)])
        throw({:error, msg1})
    end
    ...
    case :lists.dropwhile(
      ...
      [{_, {f, a}} | _] ->
        msg2 =
          flat_format(~c"  Unknown function ~tw/~w at location ~ts:~s", [f, a, file, pos(loc)])
  ...
  defp pp_size(size, ctxt, cont) do
    case :cerl.is_c_atom(size) do
      ...
      false ->
        :prettypr.beside(:prettypr.text(~c":"), cont.(size, ctxt))
    end
  end
  ...
  defp pp_map(node, ctxt, cont) do
    ...
    before =
      case :cerl.is_c_map_empty(arg) do
        true ->
          :prettypr.floating(:prettypr.text(~c"\#{"))
    ...
    :prettypr.beside(
      before,
      :prettypr.beside(
        :prettypr.par(seq(mapEs, :prettypr.floating(:prettypr.text(~c",")), ctxt, cont)),
        :prettypr.floating(:prettypr.text(~c"}"))
      )
    )
  end
    ...
    case :cerl.type(tree) do
      :cons ->
        is_foldable(
          :cerl.cons_hd(tree),
          inMatch
        ) and is_foldable(:cerl.cons_tl(tree), inMatch)
