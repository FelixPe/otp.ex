defmodule :m_typer_core do
  ...
  def run(opts) do
    ...
    trustedFiles = filter_fd(r_args(args, :trusted), [], &is_erl_file/1, analysis)
    analysis2 = extract(analysis, trustedFiles)
    all_Files = get_all_files(args, analysis2)
    msg(:debug, ~c"All_Files: ~tp", [all_Files], analysis2)
    analysis3 = r_analysis(analysis2, files: all_Files)
    analysis4 = collect_info(analysis3)
    msg(:debug, ~c"Final: ~p", [r_analysis(analysis4, :fms)], analysis3)
  ...
  defp extract(
    ...
    msg(:debug, ~c"Extracting trusted typer info...", [], analysis)
      ...
      case :dialyzer_utils.get_core_from_src(
        ...
        {:ok, core} ->
          case :dialyzer_utils.get_record_and_type_info(core) do
            {:ok, recDict} ->
              mod = :erlang.list_to_atom(:filename.basename(file, ~c".erl"))
  ...
  defp get_type_info(
    ...
    msg(:debug, ~c"Analyizing callgraph...", [], analysis)
    ...
    try do
      ...
      :error, what ->
        fatal_error(
          :io_lib.format(~c"Analysis failed with message: ~tp", [{what, __STACKTRACE__}]),
          analysis
        )

      {:dialyzer_succ_typing_error, msg} ->
        fatal_error(:io_lib.format(~c"Analysis failed with message: ~ts", [msg]), analysis)
    end
  end
  ...
  defp remove_external(callGraph, pLT, analysis) do
    ...
    case get_external(ext, pLT) do
      ...
      externals ->
        msg(:warn, ~c" Unknown functions: ~tp", [:lists.usort(externals)], analysis)
        ...
        case extTypes do
          ...
          _ ->
            msg(:warn, ~c" Unknown types: ~tp", [extTypes], analysis)
        end
    end
  ...
  defp write_and_collect_inc_info(analysis) do
    fun = fn {file, module}, inc ->
      ...
      collect_imported_functions(incFuns, r_info(info, :types), inc, analysis)
    end
  ...
  defp write_inc_files(inc, analysis) do
    fun = fn file ->
      ...
      info =
        r_info(
          types: :maps.from_list(val1),
          records: :maps.new(),
          functions: :lists.keysort(1, functions)
        )

      msg(:debug, ~c"Types ~tp", [r_info(info, :types)], analysis)
      msg(:debug, ~c"Functions ~tp", [r_info(info, :functions)], analysis)
      msg(:debug, ~c"Records ~tp", [r_info(info, :records)], analysis)
      write_typed_file(file, info, analysis)
    end
    ...
    case :lists.member(file, fs) do
      ...
      false ->
        case :filename.extension(file) do
          ~c".yrl" ->
            rootname = :filename.rootname(file, ~c".yrl")
            obj = rootname ++ ~c".erl"
    ...
    case incMap do
      %{^file => val} ->
        case :lists.keyfind(fA, 1, val) do
          false ->
            r_inc(inc, map: Map.put(incMap, file, val ++ [{fA, {line, type}}]))
  ...
  defp inc_warning({f, a}, file, analysis) do
    msg(
      :warn,
      ~c"      ***Warning: Skip function ~tp/~p in file ~tp because of inconsistent type",
      [f, a, file],
      analysis
    )
  end
  ...
  defp normalize_obj(tmpInc) do
    ...
    r_inc(tmpInc, map: :maps.fold(fun, :maps.new(), r_inc(tmpInc, :map)))
  end
  ...
  defp get_type({{m, f, a} = mFA, range, arg}, codeServer, records, analysis) do
      ...
      {:ok, {_FileLine, contract, _Xtra}} ->
        ...
        case :dialyzer_contracts.check_contract(
          ...
          {:error, {:invalid_contract, _}} ->
            ...
            msg =
              :io_lib.format(
                ~c"Error in contract of function ~w:~tw/~w\n\t The contract is: " ++
                  cString ++ ~c"\n" ++ ~c"\t but the inferred signature is: ~ts",
                [m, f, a, sigString]
              )
          ...
          {:error, errorStr} when is_list(errorStr) ->
            msg =
              :io_lib.format(~c"Error in contract of function ~w:~tw/~w: ~ts", [m, f, a, errorStr])
  ...
  defp get_functions(file, analysis) do
    case r_analysis(analysis, :mode) do
      :show ->
        ...
        incFuncs = :maps.get(file, r_analysis(analysis, :inc_func), :none)
      ...
      :annotate_inc_files ->
        normalize_inc_funcs(:maps.get(file, r_analysis(analysis, :inc_func), :none))
    end
  end
  ...
  defp write_typed_file(file, info, analysis) do
    msg(:info, ~c"      Processing file: ~tp", [file], analysis)
    ...
    case r_analysis(analysis, :mode) do
      ...
      _ ->
        typerAnnDir = :filename.join(dir, ~c"typer_ann")
        tmpNewFilename = :lists.concat([rootName, ~c".ann", ext])
        ...
        case :file.make_dir(typerAnnDir) do
          {:error, reason} ->
            case reason do
              ...
              :enospc ->
                msg = :io_lib.format(~c"Not enough space in ~tp", [dir])
                fatal_error(msg, analysis)

              :eacces ->
                msg = :io_lib.format(~c"No write permission in ~tp", [dir])
                fatal_error(msg, analysis)

              _ ->
                msg = :io_lib.format(~c"Unhandled error ~ts when writing ~tp", [reason, dir])
                fatal_error(msg, analysis)
            end
  ...
  defp delete_file(file, analysis) do
    case :file.delete(file) do
      ...
      {:error, _} ->
        msg = :io_lib.format(~c"Error in deleting file ~ts", [file])
        fatal_error(msg, analysis)
    end
  end
  ...
  defp write_typed_file(file, info, newFileName, analysis) do
    ...
    msg(:info, ~c"             Saved as: ~tp", [newFileName], analysis)
  end

  defp write_typed_file(chars, file, r_info(functions: []), _LNo, _Acc, _Analysis) do
    :ok = :file.write_file(file, :unicode.characters_to_binary(chars), [:append])
  end

  defp write_typed_file([ch | chs] = chars, file, info, lineNo, acc, analysis) do
  ...
  defp raw_write(f, a, info, file, content, r_analysis(mode: mode) = analysis) do
    typeInfo = get_type_string(f, a, info, :file, analysis)

    contentList =
      case {typeInfo, mode} do
        {~c"", :annotate_in_place} ->
          :lists.reverse(content) ++ typeInfo

        _ ->
          :lists.reverse(content) ++ typeInfo ++ ~c"\n"
      end
    ...
    case r_info(info, :edoc) do
      false ->
        case {mode, type} do
          ...
          _ ->
            prefix = :lists.concat([~c"-spec ", :erl_types.atom_to_string(f)])
            :lists.concat([prefix, typeStr, ~c"."])
        end

      true ->
        prefix = :lists.concat([~c"%% @spec ", f])
        :lists.concat([prefix, typeStr, ~c"."])
    end
  end
  ...
  defp get_type_info(func, types, analysis) do
    case types do
      ...
      _ ->
        msg = :io_lib.format(~c"No type info for function: ~tp\n", [func])
        fatal_error(msg, analysis)
    end
  end
  ...
  defp get_all_files(r_args(files: fs, files_r: ds), analysis) do
    case filter_fd(fs, ds, &test_erl_file_exclude_ann/1, analysis) do
      [] ->
        fatal_error(~c"no file(s) to analyze", analysis)
  ...
  defp test_erl_file_exclude_ann(file) do
    case is_erl_file(file) do
      true ->
        case :re.run(file, ~c"[.]ann[.]erl$", [:unicode]) do
          {:match, _} ->
            false
  ...
  defp check_dir(dir, recursive, acc, fun, analysis) do
    case :file.list_dir(dir) do
      ...
      {:error, :eacces} ->
        fatal_error(~c"no access permission to dir \"" ++ dir ++ ~c"\"", analysis)

      {:error, :enoent} ->
        fatal_error(~c"cannot access " ++ dir ++ ~c": No such file or directory", analysis)

      {:error, _Reason} ->
        fatal_error(~c"error involving a use of file:list_dir/1", analysis)
    end
  end
    ...
    newPlt =
      try do
        ...
        {:dialyzer_error, _Reason} ->
          fatal_error(
            ~c"Dialyzer's PLT is missing or is not up-to-date; please (re)create it",
            analysis
          )
      else
        dialyzerPlt ->
  ...
  defp analyze_core_tree(core, records, specInfo, cbInfo, expTypes, analysis, file) do
    ...
    exportedFuncMap = :maps.put(file, exFuncs, r_analysis(analysis, :ex_func))
    sortedFunctions = :lists.keysort(1, r_tmpAcc(acc, :funcAcc))
    funcMap = :maps.put(file, sortedFunctions, r_analysis(analysis, :func))
    incFuncMap = :maps.put(file, r_tmpAcc(acc, :incFuncAcc), r_analysis(analysis, :inc_func))
    fMs = r_analysis(analysis, :fms) ++ [{file, module}]
    recordMap = :maps.put(file, records, r_analysis(analysis, :record))
  ...
  defp get_dialyzer_plt(r_analysis(plt: pltFile0) = analysis) do
          ...
          case :filelib.is_regular(:dialyzer_cplt.get_default_cplt_filename()) do
            ...
            false ->
              case :filelib.is_regular(:dialyzer_iplt.get_default_iplt_filename()) do
                ...
                false ->
                  fatal_error(
                    ~c"No PLT file given, and no existing PLT was found at default locations " ++
                      :dialyzer_cplt.get_default_cplt_filename() ++
                      ~c" and " ++ :dialyzer_iplt.get_default_iplt_filename(),
                    analysis
                  )
              end
          end
    ...
    case :dialyzer_plt.plt_kind(pltFile) do
      ...
      :bad_file ->
        fatal_error(~c"Invalid PLT file at path " ++ pltFile, analysis)

      :no_file ->
        fatal_error(~c"No PLT file found at path " ++ pltFile, analysis)
    end
  end
  ...
  defp default_io() do
    %{
      debug: &swallow_output/2,
      info: &format/2,
      warn: &format_on_stderr/2,
      abort: &format_and_halt/2
    }
  end

  defp fatal_error(slogan, analysis) do
    msg(:abort, ~c"typer: ~ts", [slogan], analysis)
  end
  ...
  defp compile_error(reason, analysis) do
    ...
    msg = ~c"Analysis failed with error report:\n" ++ joinedString
    fatal_error(msg, analysis)
  end
  ...
  defp format_on_stderr(format, data) do
    :io.format(:standard_error, format ++ ~c"\n", data)
  end
