defmodule :m_typer do
  ...
  defp cl([~c"--version" | _]) do
    version_message()
  end

  defp cl([~c"--edoc" | opts]) do
    {:edoc, opts}
  end

  defp cl([~c"--show" | opts]) do
    {{:mode, :show}, opts}
  end

  defp cl([~c"--show_exported" | opts]) do
    {{:mode, :show_exported}, opts}
  end

  defp cl([~c"--show-exported" | opts]) do
    {{:mode, :show_exported}, opts}
  end

  defp cl([~c"--show_success_typings" | opts]) do
    {:show_succ, opts}
  end

  defp cl([~c"--show-success-typings" | opts]) do
    {:show_succ, opts}
  end

  defp cl([~c"--annotate" | opts]) do
    {{:mode, :annotate}, opts}
  end

  defp cl([~c"--annotate-inc-files" | opts]) do
    {{:mode, :annotate_inc_files}, opts}
  end

  defp cl([~c"--annotate-in-place" | opts]) do
    {{:mode, :annotate_in_place}, opts}
  end

  defp cl([~c"--no_spec" | opts]) do
    {:no_spec, opts}
  end

  defp cl([~c"--plt", plt | opts]) do
    {{:plt, plt}, opts}
  end

  defp cl([~c"-D" | _Opts]) do
    fatal_error(~c"no variable name specified after -D")
  end

  defp cl([~c"-D" ++ def__ | opts]) do
    defPair = process_def_list(:re.split(def__, ~c"=", [{:return, :list}, :unicode]))
    {{:def, defPair}, opts}
  end

  defp cl([~c"-I", dir | opts]) do
    {{:inc, dir}, opts}
  end

  defp cl([~c"-I" | _Opts]) do
    fatal_error(~c"no include directory specified after -I")
  end
  ...
  defp cl([~c"-T" | opts]) do
    ...
    case files do
      [] ->
        fatal_error(~c"no file or directory specified after -T")
  ...
  defp cl([~c"-pa", dir | opts]) do
    {{:pa, dir}, opts}
  end

  defp cl([~c"-pz", dir | opts]) do
    {{:pz, dir}, opts}
  end

  defp cl([~c"-" ++ h | _]) do
    fatal_error(~c"unknown option -" ++ h)
  end
  ...
  defp collect_args_1([~c"-" ++ _ | _] = l, acc) do
    {:lists.reverse(acc), l}
  end
  ...
  defp process_def_list(l) do
    case l do
      [name, value] ->
        {:ok, tokens, _} = :erl_scan.string(value ++ ~c".")
  ...
  defp fatal_error(slogan) do
    msg(:io_lib.format(~c"typer: ~ts\n", [slogan]))
    :erlang.halt(1)
  end

  defp mode_error(oldMode, newMode) do
    msg =
      :io_lib.format(~c"Mode was previously set to '~s'; cannot set it to '~s' now", [
        oldMode,
        newMode
      ])
  ...
  defp msg(msg) do
    :io.format(:standard_error, ~c"~ts", [msg])
  end

  defp version_message() do
    :io.format(~c"TypEr version " ++ :EFE_TODO_VSN_MACRO ++ ~c"\n")
    :erlang.halt(0)
  end
