defmodule :m_dialyzer_cplt do
  ...
  Record.defrecord(:r_contract, :contract, contracts: [], args: [],
                                    forms: [])
  Record.defrecord(:r_file_plt, :file_plt, version: '',
                                    file_md5_list: [], info: :dict.new(),
                                    contracts: :dict.new(),
                                    callbacks: :dict.new(), types: :dict.new(),
      ...
      {:ok, rec} ->
        case (check_version(rec)) do
          :error ->
            ...
            {:error, msg}
          :ok ->
            r_file_plt(info: fileInfo, contracts: fileContracts,
                callbacks: fileCallbacks, types: fileTypes,
                exported_types: fileExpTypes) = rec
            types = (for {mod, types} <- :dict.to_list(fileTypes) do
            ...
            [true, true, true] = (for {eTS, data} <- [{eTSInfo,
                                                         :dict.to_list(fileInfo)},
                                                          {eTSTypes, types},
                                                              {eTSContracts,
                                                                 :dict.to_list(fileContracts)}] do
                                    :ets.insert(eTS, data)
                                  end)
            ...
            case (returnInfo) do
              false ->
                {:ok, plt}
              true ->
                pltInfo = r_plt_info(files: r_file_plt(rec, :file_md5_list),
  ...
  defp check_version(r_file_plt(version: :EFE_TODO_VSN_MACRO,
  ...
  defp merge_disj_plts(list) do
    ...
    r_plt(info: table_disj_merge(infoList),
  ...
  def merge_plts_or_report_conflicts(pltFiles, plts) do
    try do
      ...
      {:dialyzer_error, :not_disjoint_plts} ->
        incFiles = :lists.append(for f <- pltFiles do
                                   (
  ...
  defp to_file1(fileName,
            r_plt(info: eTSInfo, types: eTSTypes,
                contracts: eTSContracts, callbacks: eTSCallbacks,
                exported_types: eTSExpTypes),
            modDeps, r_plt_info(files: mD5, mod_deps: oldModDeps)) do
    newModDeps = :dict.merge(fn _Key, oldVal, newVal ->
                                  :ordsets.union(oldVal, newVal)
                             end,
                               oldModDeps, modDeps)
    implMd5 = compute_implementation_md5()
    callbacksList = (for {_M,
                            cbs} <- :dialyzer_utils.ets_tab2list(eTSCallbacks),
                           cb <- cbs do
                       cb
                     end)
    ...
    expTypes = :sets.from_list(for {e} <- :dialyzer_utils.ets_tab2list(eTSExpTypes) do
                                 e
                               end)
    fileTypes = :dict.from_list(for {mod,
                                       mTypes} <- types do
                                  {mod, :dict.from_list(:maps.to_list(mTypes))}
                                end)
    record = r_file_plt(version: :EFE_TODO_VSN_MACRO,
                 file_md5_list: mD5, info: info, contracts: contracts,
                 callbacks: callbacks, types: fileTypes,
                 exported_types: expTypes, mod_deps: newModDeps,
  ...
  defp compute_new_md5_1([{file, md5} = entry | entries], newList,
            diff) do
    case (compute_md5_from_file(file)) do
      ...
      newMd5 ->
        ...
        compute_new_md5_1(entries, [{file, newMd5} | newList],
                            [{:differ, modName} | diff])
    end
  end
  ...
  defp compute_md5_from_file(file) do
    case (:beam_lib.all_chunks(file)) do
      {:ok, _, chunks} ->
        filtered = (for {iD, chunk} <- chunks, iD !== 'CInf',
                          iD !== 'Docs' do
                      [iD, chunk]
                    end)
  ...
  defp init_md5_list_1([{file, _Md5} | md5Left],
            [{:remove, file} | diffLeft], acc) do
    init_md5_list_1(md5Left, diffLeft, acc)
  end

  defp init_md5_list_1([{file, _Md5} = entry | md5Left],
            [{:add, file} | diffLeft], acc) do
    init_md5_list_1(md5Left, diffLeft, [entry | acc])
  end

  defp init_md5_list_1([{file1, _Md5} = entry | md5Left] = md5List,
            [{tag, file2} | diffLeft] = diffList, acc) do
    case (file1 < file2) do
      true ->
      ...
      false ->
        ...
        case (tag) do
          :add ->
            init_md5_list_1(md5List, diffLeft,
                              [{file2, <<>>} | acc])
          :remove ->
  ...
  defp init_md5_list_1([], diffList, acc) do
    addFiles = (for {:add, f} <- diffList do
                  {f, <<>>}
                end)
