defmodule :m_eunit_data do
  ...
  def iter_init(tests, parentID, options) do
    r_iter(tests: tests, parent: :lists.reverse(parentID), options: options)
  end
  ...
  defp parse({:generator, m, f}, options)
    ...
    parse(
      {:generator, :eunit_test.mf_wrapper(m, f), {m, f, 0}},
      options
    )
  end
  ...
  defp parse({:inparallel, n, t}, options)
    ...
    group(r_group(tests: t, options: options, order: {:inparallel, n}))
  end

  defp parse({:timeout, n, t}, options)
       when is_number(n) and n >= 0 do
    group(r_group(tests: t, options: options, timeout: round(n * 1000)))
  end
  ...
  defp parse({:spawn, n, t}, options) when is_atom(n) do
    group(r_group(tests: t, options: options, spawn: {:remote, n}))
  end

  defp parse({:setup, s, i}, options)
  ...
  defp parse({:module, m}, options) when is_atom(m) do
    {:data, {~c"module '" ++ :erlang.atom_to_list(m) ++ ~c"'", get_module_tests(m, options)}}
  end

  defp parse({:application, a}, options) when is_atom(a) do
    try do
      parse({:file, :erlang.atom_to_list(a) ++ ~c".app"}, options)
    catch
      {:file_read_error, {:enoent, _, _}} ->
        case :code.lib_dir(a) do
          dir when is_list(dir) ->
            binDir = :filename.join(dir, ~c"ebin")
    ...
    case :proplists.get_value(:modules, info) do
      ms when is_list(ms) ->
        case (for m <- ms, not is_atom(m) do
          ...
          [] ->
            {:data, {~c"application '" ++ :erlang.atom_to_list(a) ++ ~c"'", ms}}
  ...
  defp parse({:file, f} = t, _Options) when is_list(f) do
    case :eunit_lib.is_string(f) do
      true ->
        {:data, {~c"file \"" ++ f ++ ~c"\"", get_file_tests(f)}}
  ...
  defp parse({:dir, d} = t, _Options) when is_list(d) do
    case :eunit_lib.is_string(d) do
      true ->
        {:data, {~c"directory \"" ++ d ++ ~c"\"", get_directory_module_tests(d)}}
  ...
  defp parse({s, t1} = t, options) when is_list(s) do
    case :eunit_lib.is_string(s) do
      true ->
        group(r_group(tests: t1, options: options, desc: :unicode.characters_to_binary(s)))
  ...
  defp extract_module_tests(module, exports, options) do
    testFuns = extract_testfuns(exports, module, ~c"_test", ~c"_test_")
    moduleName = :erlang.atom_to_list(module)
    exact = :proplists.get_bool(:exact_execution, options)

    case {:lists.suffix(~c"_tests", moduleName), exact} do
      {false, false} ->
        moduleNameWithSuffix = moduleName ++ ~c"_tests"
        ...
        try do
          ...
          moreTestFuns ->
            testFuns ++ [{~c"module '" ++ moduleNameWithSuffix ++ ~c"'", moreTestFuns}]
        end
  ...
  defp objfile_module(file) do
    try do
      ...
      _, _ ->
        throw({:file_read_error, {:undefined, ~c"extracting module name failed", file}})
    end
  end
  ...
  defp get_directory_module_tests(d) do
    ...
    f = fn {m, _}, s ->
      ...
      case :lists.suffix(~c"_tests", name) do
        false ->
          name1 = name ++ ~c"_tests"
  ...
  defp data_test_() do
    ...
    [
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 746},
                     {:expression, ~c"eunit : test ( { test , ? MODULE , trivial_test } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 747},
                     {:expression, ~c"eunit : test ( { generator , fun ( ) -> Tests end } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 748},
                     {:expression, ~c"eunit : test ( { generator , fun generator / 0 } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     ...
                     {:line, 749},
                     {:expression,
                      ~c"eunit : test ( { generator , ? MODULE , generator_exported_ } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 750},
                     {:expression, ~c"eunit : test ( { inorder , Tests } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 751},
                     {:expression, ~c"eunit : test ( { inparallel , Tests } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 752},
                     {:expression, ~c"eunit : test ( { timeout , 10 , Tests } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 753},
                     {:expression, ~c"eunit : test ( { spawn , Tests } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     ...
                     {:line, 754},
                     {:expression,
                      ~c"eunit : test ( { setup , Setup , Cleanup , fun ( P ) -> ? _test ( ok = ping ( P ) ) end } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 757},
                     {:expression, ~c"eunit : test ( { module , eunit_lib } )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 758},
                     {:expression, ~c"eunit : test ( eunit_lib )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
      ...
      {759,
       fn ->
         (fn ->
            case :eunit.test(~c"examples/tests.txt") do
              :ok ->
                :ok
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_data},
                     {:line, 759},
                     {:expression, ~c"eunit : test ( \"examples/tests.txt\" )"},
                     {:pattern, ~c"ok"},
                     {:value, x__V}
                   ]}
                )
