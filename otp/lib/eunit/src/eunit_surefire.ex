defmodule :m_eunit_surefire do
  ...
  Record.defrecord(:r_state, :state, verbose: false, indent: 0, xmldir: ~c".", testsuites: [])
  ...
  def init(options) do
    xMLDir = :proplists.get_value(:dir, options, ~c".")
  ...
  defp write_report(r_testsuite(name: name) = testSuite, xmlDir) do
    filename =
      :filename.join(
        xmlDir,
        :lists.flatten([~c"TEST-", escape_suitename(name)], ~c".xml")
      )
  ...
  defp write_header(fileDescriptor) do
    :io.format(fileDescriptor, ~c"~ts~ts", ["<?xml version=\"1.0\" encoding=\"UTF-8\" ?>", "\n"])
  end
  ...
  defp write_start_tag(
    ...
    :io.format(fileDescriptor, ~c"~ts", [startTag])
  end
  ...
  defp write_end_tag(fileDescriptor) do
    :io.format(fileDescriptor, ~c"~ts~ts", ["</testsuite>", "\n"])
  end
  ...
  defp write_testcase(
    ...
    :io.format(fileDescriptor, ~c"~ts~ts", [startTag, contentAndEndTag])
  end
  ...
  defp escape_suitename(~c"module '" ++ string) do
