defmodule :m_eunit_proc do
  ...
  Record.defrecord(:r_procstate, :procstate, ref: :undefined,
                                     id: :undefined, super: :undefined,
                                     insulator: :undefined, parent: :undefined,
                                     order: :undefined, options: :undefined)
  def start(tests, order, super, reference, options)
      when (is_pid(super) and is_reference(reference)) do
    st = r_procstate(ref: reference, id: [], super: super,
  ...
  defp insulator_wait(child, parent, buf, st) do
    receive do
      ...
      {:child, ^child, id, {:end, status, time}} ->
        data = [{:time, time}, {:output,
                                  :lists.reverse(hd(buf))}]
      ...
      {:io_request, from, replyAs, req} when is_pid(from) ->
        ...
        insulator_wait(child, parent, buf, st)
      {:timeout, ^child, id} ->
        timeout = (case (:erlang.process_info(child,
                                                :current_stacktrace)) do
                     :undefined ->
                       :timeout
  ...
  defp message_insulator(data, st) do
    send(r_procstate(st, :insulator), {:child, self(), r_procstate(st, :id),
                               data})
  end

  defp set_timeout(time, st) do
    :erlang.send_after(time, r_procstate(st, :insulator),
  ...
  defp scale_timeout(:infinity, _St) do
    :infinity
  end

  defp scale_timeout(time, st) do
    case (:proplists.get_value(:scale_timeouts,
                                 r_procstate(st, :options))) do
      :undefined ->
        time
      n when is_integer(n) ->
        n * time
  ...
  defp with_timeout(time, f, st) when (is_integer(time) and
                               time > 4294967295) do
    with_timeout(4294967295, f, st)
  end
  ...
  defp tests_inparallel(i, n, st, k, k0, children) when (k <= 0 and
                                             k0 > 0) do
  ...
  defp handle_test(t, st) do
    data = [{:desc, r_test(t, :desc)}, {:source,
                                     r_test(t, :location)},
                                      {:line, r_test(t, :line)}]
    ...
    {status, time} = with_timeout(r_test(t, :timeout), 5000,
                                    fn () ->
                                         run_test(t)
                                    end,
                                    st)
  ...
  defp run_group(t, st) do
    ...
    data = [{:desc, r_group(t, :desc)}, {:spawn, r_group(t, :spawn)},
  ...
  defp io_request({:get_until, _Enc, _Prompt, _M, _F, _As},
            buf) do
    {:eof, buf}
  end
