defmodule :m_eunit_proc do
  ...
  def start(tests, order, super, reference, options)
    ...
    st = r_procstate(ref: reference, id: [], super: super, order: order, options: options)
  ...
  defp child_test_() do
    [
      {~c"test processes do not trap exit signals",
       {401,
        fn ->
          (fn ->
                 ...
                 :erlang.error(
                   {:assertMatch,
                    [
                      {:module, :eunit_proc},
                      {:line, 401},
                      {:expression, ~c"process_flag ( trap_exit , false )"},
                      {:pattern, ~c"false"},
                      {:value, x__V}
                    ]}
                 )
  ...
  defp tests(t, st) do
    i = :eunit_data.iter_init(t, r_procstate(st, :id), r_procstate(st, :options))
                ...
                :erlang.error(
                  {:assertMatch,
                   [
                     {:module, :eunit_proc},
                     {:line, 705},
                     {:expression, ~c"io : getopts ( )"},
                     {:pattern, ~c"{ error , enotsup }"},
                     {:value, x__V}
                   ]}
                )
            end
