defmodule :m_eunit_striptests do
  ...
  def parse_transform(forms, options) do
    ...
    exportSuffix = :proplists.get_value(:eunit_export_suffix,
                                          options, '_exported_')
    exports = :lists.foldl(fn {:attribute, _, :export, es},
                                s ->
  ...
  defp form({:function, _L, name, 0, _Cs} = form, acc,
            exports, testSuffix, generatorSuffix, exportSuffix) do
    n = :erlang.atom_to_list(name)
    case (not
          :sets.is_element({name, 0},
                             exports) and (:lists.suffix(testSuffix,
                                                           n) or :lists.suffix(generatorSuffix,
                                                                                 n) or :lists.suffix(exportSuffix,
                                                                                                       n))) do
      true ->
        acc
      false ->
  ...
  defp form({:function, _L, :eunit_wrapper_, 1, _Cs}, acc,
            _, _, _, _) do
    acc
  end
