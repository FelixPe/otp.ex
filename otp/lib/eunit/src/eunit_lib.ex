defmodule :m_eunit_lib do
    ...
    case (is_stacktrace(trace)) do
      true ->
        :io_lib.format('~ts**~w:~ts',
                         [format_stacktrace(trace), class, format_term(term,
                                                                         depth)])
      false ->
  ...
  defp is_stacktrace([{m, f, a, l} | fs]) when (is_atom(m) and
                                       is_atom(f) and is_integer(a) and
                                       is_list(l)) do
    is_stacktrace(fs)
  end

  defp is_stacktrace([{m, f, as, l} | fs]) when (is_atom(m) and
                                        is_atom(f) and is_list(as) and
                                        is_list(l)) do
    is_stacktrace(fs)
  end

  defp is_stacktrace([{m, f, a} | fs]) when (is_atom(m) and
                                    is_atom(f) and is_integer(a)) do
    is_stacktrace(fs)
  end

  defp is_stacktrace([{m, f, as} | fs]) when (is_atom(m) and
  ...
  defp is_op(:erlang, f, a) do
    :erl_internal.arith_op(f, a) or :erl_internal.bool_op(f,
                                                            a) or :erl_internal.comp_op(f,
                                                                                          a) or :erl_internal.list_op(f,
                                                                                                                        a) or :erl_internal.send_op(f,
                                                                                                                                                      a)
  end
  ...
  def format_error({:generator_failed, {{m, f, a}, exception}},
           depth) do
  ...
  def format_error({{:bad_instantiator, {{m, f, a}, term}},
            _DummyException},
           depth) do
    error_msg(:io_lib.format('result from instantiator ~w:~tw/~w is not a test', [m, f, a]), '~tP',
                [term, depth])
  end
  ...
  defp format_exception_test_() do
    [{205,
        fn () ->
             (
               (fn () ->
                     case (:lists.reverse(:lists.flatten(format_exception(try do
                                                                            :erlang.error(:dummy)
                                                                          catch
                                                                            c, r
                                                                            ->
  ...
  def is_string([c | cs]) when (is_integer(c) and c >= 0 and
                           c <= 1114111) do
    is_string(cs)
  end
  ...
  defp split_node_2(as, cs) do
    {:erlang.list_to_atom(:lists.reverse(as)),
                       ...
                       fn () ->
                        ...
                        {395,
                           fn () ->
                            ...
                            {396,
                               fn () ->
                                    [1, 2, 3, 2, 1] = uniq([1, 2, 2, 3, 2, 2,
                                                                               1])
                               end},
                                {397,
                                   fn () ->
                                        [1, 2, 3] = uniq([1, 1, 1, 2, 2, 2, 3,
                                                                                3,
                                                                                    3])
                                   end},
                                    {398,
  ...
  def command(cmd, dir, env) do
    ...
    opt = cD ++ setEnv ++ [:stream, :exit_status,
  ...
  def consult_file(file) do
    case (:file.path_consult(['.'] ++ :code.get_path(),
                               file)) do
      {:ok, data, _Path} ->
        data
      {:error, reason} ->
                             ...
                             case (trie_store([1],
                               ...
                               x__V ->
                                 :erlang.error({:assert,
                                                  [{:module, :eunit_lib},
                                                       {:line, 571},
                                                           {:expression, 'trie_store ( [ 1 ] , trie_new ( ) ) =:= gb_trees : insert ( 1 , [ ] , gb_trees : empty ( ) )'},
                                                               {:expected,
                                                                  true},
                                                                   case (not
                                                                         x__T) do
                                 ...
                                 case (trie_store([1, 2],
                                   ...
                                   x__V ->
                                     :erlang.error({:assert,
                                                      [{:module, :eunit_lib},
                                                           {:line, 576},
                                                               {:expression, 'trie_store ( [ 1 , 2 ] , trie_new ( ) ) =:= gb_trees : insert ( 1 , gb_trees : insert ( 2 , [ ] , gb_trees : empty ( ) ) , gb_trees : empty ( ) )'},
                                                                   {:expected,
                                                                      true},
                                                                       case (not
                                                                             x__T) do
