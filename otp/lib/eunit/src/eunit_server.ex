defmodule :m_eunit_server do
  ...
  Record.defrecord(:r_job, :job, super: :undefined,
  ...
  Record.defrecord(:r_state, :state, name: :undefined,
                                 stopped: :undefined, jobs: :undefined,
                                 queue: :undefined, auto_test: :undefined,
                                 modules: :undefined, paths: :undefined,
                                 regexps: :undefined)
  defp server_init(name) do
    server(r_state(name: name, stopped: false, jobs: :dict.new(),
               queue: :queue.new(), auto_test: :queue.new(),
               modules: :sets.new(), paths: :sets.new(),
               regexps: :sets.new()))
  end
  ...
  defp server_command(from, {:start, job}, st) do
    reference = make_ref()
    st1 = (case (:proplists.get_bool(:enqueue,
                                       r_job(job, :options))) do
             true ->
  ...
  defp enqueue(job, from, reference, st) do
    case (:dict.size(r_state(st, :jobs))) do
      0 ->
        start_job(job, from, reference, st)
      _ ->
        r_state(st, queue: :queue.in({job, from, reference},
                                 r_state(st, :queue)))
    end
  end
  ...
  defp start_job(job, from, reference, st) do
    ...
    order = :proplists.get_value(:order, r_job(job, :options),
                                   :inorder)
    :eunit_proc.start(r_job(job, :test), order, r_job(job, :super),
                        reference, r_job(job, :options))
  ...
  defp handle_done(reference, st) do
    case (:dict.find(reference, r_state(st, :jobs))) do
      {:ok, from} ->
        ...
        dequeue(r_state(st, jobs: :dict.erase(reference,
                                          r_state(st, :jobs))))
      :error ->
        st
    end
  ...
  defp is_watched(m, st) when is_atom(m) do
    :sets.is_element(m,
                       r_state(st, :modules)) or is_watched(:code.which(m), st)
  end

  defp is_watched(path, st) do
    :sets.is_element(:filename.dirname(path),
                       r_state(st, :paths)) or match_any(:sets.to_list(r_state(st, :regexps)),
                                                     path)
  end
  ...
  defp new_auto_test(server, m, st) do
    ...
    r_state(st, auto_test: :queue.in({server, m},
