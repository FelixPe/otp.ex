defmodule :m_docgen_xml_to_chunk do
  ...
  def main([_Application, fromBeam, _Escript, toChunk]) do
    ...
    {:ok,
       {_Module,
          [{:exports, exports}]}} = :beam_lib.chunks(fromBeam,
                                                       [:exports])
    anno = :erl_anno.set_file(name, :erl_anno.new(0))
    emptyDocs = add_hidden_docs(exports,
                                  r_docs_v1(anno: anno, module_doc: :hidden, docs: []))
    :ok = :file.write_file(toChunk,
                             :erlang.term_to_binary(emptyDocs, [:compressed]))
    :ok
  end

  def main([application, fromXML, fromBeam, escript,
                                              toChunk]) do
    ...
    case (docs(application, fromXML, fromBeam)) do
      ...
      {:docs_v1, _, _, _, _, %{source: s}, []}
          when (s !== '../xml/gen_fsm.xml' and s !== '../xml/shell_default.xml' and s !== '../xml/user.xml' and s !== '../xml/wxClipboardTextEvent.xml' and
                  s !== '../xml/wxDisplayChangedEvent.xml' and s !== '../xml/wxGBSizerItem.xml' and s !== '../xml/wxGraphicsBrush.xml' and s !== '../xml/wxGraphicsFont.xml' and
                  s !== '../xml/wxGraphicsPen.xml' and s !== '../xml/wxInitDialogEvent.xml' and s !== '../xml/wxMaximizeEvent.xml' and s !== '../xml/wxMouseCaptureLostEvent.xml' and
                  s !== '../xml/wxPaintEvent.xml' and s !== '../xml/wxPreviewCanvas.xml' and s !== '../xml/wxSysColourChangedEvent.xml' and s !== '../xml/wxTaskBarIconEvent.xml' and
                  s !== '../xml/wxWindowCreateEvent.xml' and s !== '../xml/wxWindowDestroyEvent.xml' and s !== '../xml/wxDataObject.xml')
               ->
  ...
  Record.defrecord(:r_state, :state, tags: [], cno: [],
                                 namespaces: [], dom: [])
  ...
  defp build_dom(:endDocument,
            r_state(dom: [{tag, attributes, content} | d]) = state) do
    case (d) do
      [:startDocument] ->
        r_state(state, dom: [{tag, attributes,
                          :lists.reverse(content)}])
      [decl, :startDocument] ->
        r_state(state, dom: [decl, {tag, attributes,
                                :lists.reverse(content)}])
      _ ->
        state
    end
  ...
  defp build_dom({:startElement, _Uri, localName, _QName,
             attributes},
            r_state(tags: t, dom: d) = state) do
    a = parse_attributes(localName, attributes)
    cName = :erlang.list_to_atom(localName)
    r_state(state, tags: [cName | t], 
               dom: [{cName, :lists.reverse(a), []} | d])
  end

  defp build_dom({:endElement, _Uri, localName, _QName},
            r_state(tags: [_ | t],
                dom: [{cName, cAttributes, cContent}, {pName,
                                                         pAttributes,
                                                         pContent} = _Parent |
                                                          d]) = state) do
    case (:erlang.list_to_atom(localName)) do
      ^cName ->
        sectionDepth = length(for e <- t, e === :section do
                                e
                              end)
        mappedCName = (case (cName) do
                         :title ->
                           :lists.nth(sectionDepth + 1,
                                        [:h1, :h2, :h3, :h4, :h5, :h6])
  ...
  defp build_dom({:characters, string},
            r_state(dom: [{name, attributes, content} | d]) = state) do
    htmlEnts = [{'&nbsp;', [160]}, {'&times;', [215]}, {'&plusmn;', [177]}, {'&ouml;', 'Ã¶'},
                                                        {'&auml;', 'Ã¤'}, {'&aring;', 'Ã¥'}]
    noHtmlEnt = :lists.foldl(fn {pat, sub}, str ->
                                  :re.replace(str, pat, sub,
                                                [:global, :unicode])
                             end,
                               string, htmlEnts)
    case (:re.run(noHtmlEnt, '&[a-z]*;',
                    [{:capture, :first, :binary}, :unicode])) do
      :nomatch ->
        :ok
      {:match, ["&lt;"]} ->
        :ok
    ...
    newContent = [:unicode.characters_to_binary(noHtmlEnt,
                                                  :utf8) |
                      content]
  ...
  defp build_dom({:ignorableWhitespace, string},
            r_state(dom: [{name, _, _} = _E | _]) = state) do
    case (:lists.member(name,
                          [:p, :pre, :input, :code, :quote, :warning, :note,
                                                                          :change,
                                                                              :dont,
                                                                                  :do,
                                                                                      :c,
  ...
  defp parse_attributes(elName,
            [{_Uri, _Prefix, localName, attrValue} | as], n, acc) do
    parse_attributes(elName, as, n + 1,
                       [{:erlang.list_to_atom(localName), attrValue} | acc])
  end

  defp docs(application, oTPXml, fromBEAM) do
    case (:xmerl_sax_parser.file(oTPXml,
                                   [:skip_external_dtd, {:event_fun, &event/3},
                                                            {:event_state,
                                                               initial_state()}])) do
      {:ok, tree, _} ->
        {:ok, {module, chunks}} = :beam_lib.chunks(fromBEAM,
        ...
        case (get_dom(tree)) do
          [{:erlref, _, _}] = dom ->
            ...
            chunk = add_hidden_docs(:proplists.get_value(:exports,
                                                           chunks),
                                      to_chunk(newDom, oTPXml, module,
                                                 :proplists.get_value(:abstract_code,
                                                                        chunks)))
            verify_chunk(module,
                           :proplists.get_value(:exports, chunks), chunk)
            chunk
          _Else ->
            {:error, :not_erlref}
        end
  ...
  defp add_hidden_docs(exports, r_docs_v1(anno: anno, docs: docs) = chunk) do
    hiddenFuncs = (for {f, a} <- exports,
                         f !== :module_info,
                         :lists.keysearch({:function, f, a}, 1,
                                            docs) == false do
                     {{:function, f, a}, anno,
                        [:erlang.iolist_to_binary(:io_lib.format('~p/~p', [f, a]))],
                        :hidden, %{}}
                   end)
  ...
  defp verify_chunk(m, exports, r_docs_v1(docs: docs) = doc) do
    exported = (for {{:function, f, a}, _, _, _,
                       _} <- docs do
                  (
                    fA = {f, a}
    ...
    :lists.foreach(fn {_M, _F, _A, true} ->
                        :ok
                   end,
                     exported)
    try do
  ...
  defp transform([{:erlref, _Attr, content} | t], acc) do
    module = (for (mod = {:module, _, _}) <- content do
                mod
              end)
    newContent = content -- module
    [{:module, sinceAttr, [mname]}] = module
    since = (case (:proplists.get_value(:since,
                                          sinceAttr)) do
               :undefined ->
                 []
               [] ->
    ...
    transform([{:module, [{:name, mname} | since],
                  newContent} |
                   t],
                acc)
  end
  ...
  defp transform([{:code, attr, content} | t], acc) do
    transform(t,
                [{:pre, [],
                    [{:code, a2b(attr), transform(content, [])}]} |
                     acc])
  end
  ...
  defp transform([{:funcs, _Attr, content} | t], acc) do
    fnAttr = (case (:lists.keyfind(:fsdescription, 1,
                                     content)) do
                false ->
                  []
  ...
  defp transform([{:datatypes, _Attr, content} | t], acc) do
    ...
    dtAttr = (case (:lists.keyfind(:datatype_title, 1,
                                     content)) do
                false ->
                  []
  ...
  defp transform([{:datatype_title, _Attr, _Content} | t],
            acc) do
    transform(t, acc)
  end
  ...
  defp transform([{what, [], content} | t], acc)
    ...
    whatP = {:div, [{:class, :erlang.atom_to_binary(what)}],
               transform(content, [])}
  ...
  defp transform([{:term, attr, []} | t], acc) do
    transform([:erlang.list_to_binary(:proplists.get_value(:id,
                                                             attr)) |
                   t],
                acc)
  end
  ...
  defp transform_taglist(attr, content) do
    items = :lists.map(fn {:tag, _A, _C} = tag ->
                            transform_tag(tag)
                          {:item, a, c} ->
                            {:dd, a, c}
                       end,
  ...
  defp func2func({:func, attr, contents}) do
    contentsNoName = (for nC <- contents,
                            :erlang.element(1, nC) != :name do
                        nC
                      end)
    editLink = (case (:proplists.get_value(:ghlink,
                                             attr)) do
                  :undefined ->
                    %{}
                  ghLink ->
    ...
    verifyNameList = fn nameList, test ->
                          for {:name, t, c} <- nameList do
                            (
                              :ok = test.(c)
                              {:proplists.get_value(:name, t),
                                 :proplists.get_value(:arity, t)}
                            )
                          end
                     end
    nameList = (for ({:name, _, _} = name) <- contents do
                  name
                end)
    sinceMD = (case (for {:name, sinceAttr, _} <- nameList,
                 ...
                 [] ->
                   editLink
                 sinces ->
                   Map.put(editLink, :since,
                                       :unicode.characters_to_binary(:lists.join(',',
                                                                                   :lists.usort(sinces))))
               end)
    functions = (case (nameList) do
                   [{:name, _, []} | _] ->
                     tagsToFA = fn tags ->
                                     {:proplists.get_value(:name, tags),
                                        :proplists.get_value(:arity, tags)}
                                end
                     _ = verifyNameList.(nameList,
                                           fn [] ->
                                                :ok
                                           end)
                     fAs = (for {:name, fAttr, []} <- nameList do
                              tagsToFA.(fAttr)
                            end)
                     sortedFAs = :lists.reverse(:lists.usort(fAs))
                     fAClauses = :lists.usort(for {:name, fAttr,
                                                     []} <- nameList do
                                                {tagsToFA.(fAttr),
                                                   :proplists.get_value(:clause_i,
                                                                          fAttr)}
                                              end)
                     makeFunc = fn {f, a}, mD, doc ->
                                     specs = (for {{cF, cA}, c} <- fAClauses,
                                                    f === cF, a === cA do
                                                (
                                                  {:function,
                                                     name} = func_to_atom(cF)
                                                  {name,
                                                     :erlang.list_to_integer(cA),
                                                     c}
                                                )
                                              end)
                                     {:function,
                                        [{:name, f}, {:arity,
                                                        :erlang.list_to_integer(a)},
                                                         {:signature,
                                                            [:erlang.iolist_to_binary([f,
                                                                                           '/',
                                                                                               a])]},
                                                             {:meta,
                                                                Map.put(mD, :signature,
                                                                              specs)}],
                                        doc}
                                end
                     base = makeFunc.(hd(sortedFAs), sinceMD, contentsNoName)
                     {baseF, baseA} = hd(sortedFAs)
                     mD = Map.put(sinceMD, :equiv,
                                             {:function,
                                                :erlang.list_to_atom(baseF),
                                                :erlang.list_to_integer(baseA)})
                     equiv = :lists.map(fn fA ->
                                             makeFunc.(fA, mD, [])
                                        end,
                                          tl(sortedFAs))
                     :lists.reverse([base | equiv])
                   ^nameList ->
                     fAs = :lists.foldl(fn {:name, _, nameString}, acc ->
                                             fAs = func_to_tuple(nameString)
                                             :lists.foldl(fn fA, fAAcc ->
                                                               slogan = :maps.get(fA,
                                                                                    fAAcc,
                                                                                    [])
                                                               Map.put(fAAcc, fA,
                     ...
                     base = {:function,
                               [{:name, baseF}, {:arity, baseA}, {:signature,
                                                                    baseSig},
                                                                     {:meta,
                                                                        sinceMD}],
                               contentsNoName}
                     {equivKind, equivF} = func_to_atom(baseF)
                     equiv = (for {{f, a}, signature} <- tl(sortedFAs) do
                                {:function,
                                   [{:name, f}, {:arity, a}, {:signature,
                                                                signature},
                                                                 {:meta,
                                                                    Map.put(sinceMD, :equiv,
                                                                                       {equivKind,
                                                                                          equivF,
                                                                                          baseA})}],
                                   []}
  ...
  defp parse_args([open | t], arity, stack) when open === ?[ or
                                           open === ?{ or open === ?( do
  ...
  defp transform_datatype(dom, _Acc) do
    contentsNoName = transform(for nC <- dom,
                                     :erlang.element(1, nC) != :name do
                                 nC
                               end,
                                 [])
  ...
  defp transform_see({see, [{:marker, marker}], content}) do
    absMarker = (case (:string.split(marker, '#')) do
                   [appFile] ->
                     marker_defaults(appFile)
                   [appFile, anchor] ->
                     [marker_defaults(appFile), '#', anchor]
                 end)
    {:a,
       [{:href, :erlang.iolist_to_binary(absMarker)}, {:rel,
                                                         <<"https://erlang.org/doc/link/",
                                                             :erlang.atom_to_binary(see)
                                                             ::
                                                             binary>>}],
       content}
  end

  defp marker_defaults('') do
    [:erlang.get(:application), ':', :erlang.get(:module)]
  end

  defp marker_defaults(appFile) do
    case (:string.split(appFile, ':')) do
      [file] ->
        [:erlang.get(:application), ':', file]
      [app, file] ->
        [app, ':', file]
    end
  end
  ...
  defp to_chunk(dom, source, module, aST) do
    ...
    moduleDocs = :lists.flatmap(fn {tag, _, content}
                                  ...
                                  mcontent)
    typeMeta = add_types(aST,
                           :maps.from_list([{:source, source} | mAttr]))
    typeMap = :maps.get(:types, typeMeta, [])
    anno = :erl_anno.set_file(:erlang.atom_to_list(module) ++ '.erl',
                                :erl_anno.new(0))
    typeEntries = :lists.flatmap(fn {:datatypes, dTsAttr,
                                       types} ->
                                      titleMD = (case (:proplists.get_value(:title,
                                                                              dTsAttr)) do
                                                   :undefined ->
                                                     %{}
                                                   title ->
                                      ...
                                      :lists.map(fn {:datatype, attr, descr} ->
                                                      {:function,
                                                         typeName} = func_to_atom(:proplists.get_value(:name,
                                                                                                         attr))
                                                      typeArity = (case (:proplists.get_value(:n_vars,
                                                                                                attr)) do
                                                                     :undefined ->
                                                                       find_type_arity(typeName,
                                                                                         typeMap)
                                                                     arity ->
                                                      ...
                                                      {typeSignature,
                                                         metaSig} = (case (:proplists.get_value(:signature,
                                                                                                  attr)) do
                                                                       :undefined ->
                                                                         placeholderSig = :erlang.iolist_to_binary(:io_lib.format('-type ~p(~s) :: term().',
                                                                                                                                    [typeName,
                                                                                                                                         typeArgs]))
                                                                         {[placeholderSig],
                                                                            case (:maps.get({typeName,
                                                                                               typeArity},
                                                                                              typeMap,
                                                                                              :undefined)) do
                                                                              :undefined ->
                                                      ...
                                                      metaDepr = (case (apply(:otp_internal,
                                                                                :obsolete_type,
                                                                                [module,
                                                                                     typeName,
                                                                                         typeArity])) do
                                                                    {:deprecated,
                                                                       text} ->
                                                                      Map.put(metaSig, :deprecated,
                                                                                         :unicode.characters_to_binary(:erl_lint.format_error({:deprecated_type,
                                                                                                                                                 {module,
                                                                                                                                                    typeName,
                                                                                                                                                    typeArity},
                                                                                                                                                 text})))
                                   ...
                                   mcontent)
    funcEntrys = :lists.flatmap(fn {:functions, fsAttr,
                                      functions} ->
                                     titleMD = (case (:proplists.get_value(:title,
                                                                             fsAttr)) do
                                                  :undefined ->
                                     ...
                                     :lists.map(fn {:function, attr, fdoc} ->
                                                     {type,
                                                        name} = func_to_atom(:proplists.get_value(:name,
                                                                                                    attr))
                                                     arity = :proplists.get_value(:arity,
                                                                                    attr)
                                                     signature = :proplists.get_value(:signature,
                                                                                        attr)
                                                     fMeta = :maps.merge(:proplists.get_value(:meta,
                                                                                                attr),
                                                                           titleMD)
                                                     metaWSpec = add_spec(aST,
                                                                            fMeta)
                                                     metaDepr = (case (apply(:otp_internal,
                                                                               :obsolete,
                                                                               [module,
                                                                   ...
                                                                   {:deprecated,
                                                                      text} ->
                                                                     Map.put(metaWSpec, :deprecated,
                                                                                          :unicode.characters_to_binary(:erl_lint.format_error({:deprecated,
                                                                                                                                                  {module,
                                                                                                                                                     name,
                                                                                                                                                     arity},
                                                                                                                                                  text})))
                                  ...
                                  mcontent)
    docs_v1(moduleDocs, anno, typeMeta,
              funcEntrys ++ typeEntries)
  end
  ...
  defp docs_v1_entry(kind, anno, name, arity, signature, metadata,
            docContents) do
    annoWLine = (case (metadata) do
                   %{signature: [sig | _]} ->
    ...
    doc = (case (docContents) do
             [] ->
               %{}
             ^docContents ->
               %{"en" => :shell_docs.normalize(docContents)}
           end)
    {{kind, name, arity}, annoWLine,
       :lists.flatten(signature), doc, metadata}
  end
  ...
  defp add_spec({:raw_abstract_v1, aST},
            meta = %{signature: specs}) do
    %{meta
      |
      signature:
      add_spec_clauses(aST, merge_clauses(specs, %{}))}
  end
  ...
  defp add_types({:raw_abstract_v1, aST}, meta) do
    Map.put(meta, :types,
                    :maps.from_list(for (t = {:attribute, _, tO,
                                                {name, _, args}}) <- aST,
  ...
  defp filter_clauses({:attribute, ln, :spec, {fA, clauses}},
            clauseIds) do
    {_, filteredClauses} = :lists.foldl(fn {tO, _, _,
                                              _} = c,
                                             {cnt, acc}
                                               when tO === :type or tO === :opaque
                                                    ->
                                             case (:lists.member(:erlang.integer_to_list(cnt),
                                                                   clauseIds)) do
                                               true ->
                                                 {cnt + 1, [c | acc]}
                                               false ->
                                          ...
                                          {1, []}, clauses)
    {:attribute, ln, :spec,
       {fA, :lists.reverse(filteredClauses)}}
  end

  defp merge_clauses([{f, a, clause} | t], acc) do
    merge_clauses(t,
                    Map.put(acc, {f, a},
                                   [clause | :maps.get({f, a}, acc, [])]))
  end
  ...
  defp find_spec(aST, func, arity) do
    specs = :lists.filter(fn {:attribute, _, :spec,
                                {{f, a}, _}} ->
                               f === func and a === arity
