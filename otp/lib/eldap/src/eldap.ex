defmodule :m_eldap do
    ...
    send(
      ...
      {:error,
       {{:forbidden_tcp_option, opt},
        ~c"This option affects the eldap functionality and can't be set by user"}}
    )
  ...
  defp try_connect([host | hosts], data) do
    ...
    try do
      ...
      _, err ->
        log2(data, ~c"Connect: ~p failed ~p~n", [host, err])
  ...
  defp exec_start_tls(data, controls) do
    req = r_ExtendedRequest(requestName: ~c"1.3.6.1.4.1.1466.20037")
  ...
  defp do_simple_bind(data, :anon, :anon, controls) do
    do_the_simple_bind(data, ~c"", ~c"", controls)
  end
    ...
    case r_LDAPMessage(msg, :protocolOp) do
      {:searchResDone, r} ->
        case r_LDAPResult(r, :resultCode) do
          :success ->
            log2(data, ~c"search reply = searchResDone ~n", [])
            {:ok, acc, ref, r_LDAPMessage(msg, :controls), data}

          :sizeLimitExceeded ->
            log2(data, ~c"[TRUNCATED] search reply = searchResDone ~n", [])
            {:ok, acc, ref, r_LDAPMessage(msg, :controls), data}
  ...
  defp do_add_0(data, entry, attrs, controls) do
    ...
    s = r_eldap(data, :fd)
    id = bump_id(data)
    log2(data, ~c"add request = ~p~n", [req])
    resp = request(s, data, id, {:addRequest, req, controls})
    log2(data, ~c"add reply = ~p~n", [resp])
  ...
  defp do_delete_0(data, entry, controls) do
    s = r_eldap(data, :fd)
    id = bump_id(data)
    log2(data, ~c"del request = ~p~n", [entry])
    resp = request(s, data, id, {:delRequest, entry, controls})
    log2(data, ~c"del reply = ~p~n", [resp])
  ...
  defp do_modify_0(data, obj, mod, controls) do
    ...
    s = r_eldap(data, :fd)
    id = bump_id(data)
    log2(data, ~c"modify request = ~p~n", [req])
    resp = request(s, data, id, {:modifyRequest, req, controls})
    log2(data, ~c"modify reply = ~p~n", [resp])
  ...
  defp do_passwd_modify_0(data, dn, newPasswd, oldPasswd, controls) do
    ...
    log2(data, ~c"modify password request = ~p~n", [req])
    {:ok, bytes} = :ELDAPv3.encode(:PasswdModifyRequestValue, req)
    extReq = r_ExtendedRequest(requestName: ~c"1.3.6.1.4.1.4203.1.11.1", requestValue: bytes)
    id = bump_id(data)
    log2(data, ~c"extended request = ~p~n", [extReq])
    reply = request(r_eldap(data, :fd), data, id, {:extendedReq, extReq, controls})
    log2(data, ~c"modify password reply = ~p~n", [reply])
  ...
  defp do_modify_dn_0(data, entry, newRDN, delOldRDN, newSup, controls) do
    ...
    s = r_eldap(data, :fd)
    id = bump_id(data)
    log2(data, ~c"modify DN request = ~p~n", [req])
    resp = request(s, data, id, {:modDNRequest, req, controls})
    log2(data, ~c"modify DN reply = ~p~n", [resp])
    check_reply(r_eldap(data, id: id), resp, :modDNResponse)
  end

  defp do_unbind(data) do
    req = ~c""
    log2(data, ~c"unbind request = ~p (has no reply)~n", [req])
  ...
  defp v_filter(_Filter) do
    throw({:error, concat([~c"unknown filter: ", _Filter])})
  end
  ...
  defp v_scope(_Scope) do
    throw({:error, concat([~c"unknown scope: ", _Scope])})
  end
  ...
  defp v_bool(_Bool) do
    throw({:error, concat([~c"not Boolean: ", _Bool])})
  end
  ...
  defp v_size_limit(_I) do
    throw({:error, concat([~c"size_limit not positive integer: ", _I])})
  end
  ...
  defp v_timeout(_I) do
    throw({:error, concat([~c"timeout not positive integer: ", _I])})
  end
  ...
  defp v_attributes(attrs) do
    f = fn
      ...
      a ->
        throw({:error, concat([~c"attribute not String: ", a])})
    end
  ...
  def parse_ldap_url(~c"ldap://" ++ rest1 = str) do
  ...
  defp parse_attributes([?? | tail]) do
    case split_string(tail, ??) do
      {[], attributes} ->
        {[], {:attributes, :string.lexemes(attributes, ~c",")}}

      {attributes, rest} ->
        {rest, {:attributes, :string.lexemes(attributes, ~c",")}}
    end
  end
  ...
  def paged_result_control(pageSize) when is_integer(pageSize) do
    paged_result_control(pageSize, ~c"")
  end
  ...
  def paged_result_control(pageSize, cookie) when is_integer(pageSize) do
    ...
    {:control, ~c"1.2.840.113556.1.4.319", true, controlValue}
  end
  ...
  defp find_paged_result_cookie([c | controls]) do
    case c do
      r_Control(controlType: ~c"1.2.840.113556.1.4.319", controlValue: controlValue) ->
        {:ok, r_RealSearchControlValue(cookie: cookie)} =
          :ELDAPv3.decode(
            :RealSearchControlValue,
            controlValue
          )
