defmodule :m_eldap do
  ...
  @vc :"$Id$ "
  require Record
  Record.defrecord(:r_eldap_search, :eldap_search, base: [],
                                        filter: [], size_limit: 0,
                                        scope: :wholeSubtree,
                                        deref: :derefAlways, attributes: [],
                                        types_only: false, timeout: 0)
  Record.defrecord(:r_eldap_search_result, :eldap_search_result, entries: [],
                                               referrals: [], controls: [])
  Record.defrecord(:r_eldap_entry, :eldap_entry, object_name: '',
                                       attributes: [])
  Record.defrecord(:r_eldap, :eldap, version: 3,
                                 host: :undefined, port: 389, fd: :undefined,
                                 prev_fd: :undefined, binddn: '',
                                 passwd: :undefined, id: 0, log: :undefined,
                                 timeout: :infinity, anon_auth: false,
                                 ldaps: false, using_tls: false, tls_opts: [],
                                 tcp_opts: [])
  def open(hosts) do
    open(hosts, [])
  end

  def open(hosts, opts) when (is_list(hosts) and
                              is_list(opts)) do
    self = self()
    pid = spawn_link(fn () ->
  ...
  def modify_password(handle, dn, newPasswd, oldPasswd, controls)
      when (is_pid(handle) and is_list(dn) and
              is_list(newPasswd) and is_list(oldPasswd)) do
    send(handle,
           {:passwd_modify, optional(dn), optional(newPasswd),
              optional(oldPasswd), controls})
    recv(handle)
  end

  def getopts(handle, optNames) when (is_pid(handle) and
                                   is_list(optNames)) do
  ...
  def controlling_process(handle, pid) when (is_pid(handle) and
                              is_pid(pid)) do
    :erlang.link(pid)
  ...
  def info(handle) when is_pid(handle) do
    send(handle, :info)
    recv(handle)
  end
  ...
  defp add_attrs(attrs) do
    f = fn {type, vals} when (is_list(type) and
                                is_list(vals))
                             ->
  ...
  def delete(handle, entry) when (is_pid(handle) and
                                is_list(entry)) do
    delete(handle, entry, :asn1_NOVALUE)
  end

  def delete(handle, entry, controls) when (is_pid(handle) and
                                          is_list(entry)) do
    send(handle, {:delete, entry, controls})
    recv(handle)
  end

  def modify(handle, object, mods) when (is_pid(handle) and
  ...
  def mod_add(type, values) when (is_list(type) and
                               is_list(values)) do
    m(:add, type, values)
  end

  def mod_delete(type, values) when (is_list(type) and
                               is_list(values)) do
    m(:delete, type, values)
  end

  def mod_replace(type, values) when (is_list(type) and
                               is_list(values)) do
    m(:replace, type, values)
  end

  defp m(operation, type, values) do
    r_ModifyRequest_changes_SEQOF(operation: operation,
  ...
  defp modify_dn(handle, entry, newRDN, delOldRDN, newSup,
            controls)
      when (is_pid(handle) and is_list(entry) and
              is_list(newRDN) and is_atom(delOldRDN) and
              is_list(newSup)) do
  ...
  def search(handle, x) when (is_pid(handle) and
                            elem(x, 0) === :eldap_search) or
                           is_list(x) do
    search(handle, x, :asn1_NOVALUE)
  end

  def search(handle, a, controls) when (is_pid(handle) and
                                      elem(a, 0) === :eldap_search) do
    call_search(handle, a, controls)
  end

  def search(handle, l, controls) when (is_pid(handle) and
                                      is_list(l)) do
    case ((try do
            parse_search_args(l)
          catch
  ...
  defp parse_search_args([{:size_limit, sizeLimit} | t], a)
      when is_integer(sizeLimit) do
    parse_search_args(t, r_eldap_search(a, size_limit: sizeLimit))
  end
  ...
  def substrings(type, subStr) when (is_list(type) and
                               is_list(subStr)) do
    ss = v_substr(subStr)
  ...
  def extensibleMatch(matchValue, optArgs) do
    matchingRuleAssertion = mra(optArgs,
  ...
  defp parse_args([{:ssl, true} | t], cpid, data) do
    parse_args(t, cpid,
                 r_eldap(data, ldaps: true,  using_tls: true))
  end
  ...
  defp parse_args([{:sslopts, opts} | t], cpid, data)
    ...
    parse_args(t, cpid,
                 r_eldap(data, ldaps: true,  using_tls: true, 
                           tls_opts: opts ++ r_eldap(data, :tls_opts)))
  end
  ...
  defp parse_args([{:tcpopts, opts} | t], cpid, data)
      when is_list(opts) do
    parse_args(t, cpid,
                 r_eldap(data, tcp_opts: tcp_opts(opts, cpid,
                                              r_eldap(data, :tcp_opts))))
  end
  ...
  defp tcp_opts([opt | opts], cpid, acc) do
    ...
    case (:lists.member(key,
                          [:active, :binary, :deliver, :list, :mode,
                                                                  :packet])) do
      false ->
  ...
  defp tcp_opts_error(opt, cpid) do
    send(cpid, {:error, {{:forbidden_tcp_option, opt}, 'This option affects the eldap functionality and can\'t be set by user'}})
    :erlang.unlink(cpid)
  ...
  defp do_connect(host, data, opts)
    ...
    :gen_tcp.connect(host, r_eldap(data, :port),
                       opts ++ r_eldap(data, :tcp_opts), r_eldap(data, :timeout))
  end

  defp do_connect(host, data, opts)
      when r_eldap(data, :ldaps) == true do
    :ssl.connect(host, r_eldap(data, :port),
  ...
  def loop(cpid, data) do
    receive do
      ...
      {from, {:modify, obj, mod, controls}} ->
        ...
        :eldap.loop(cpid, newData)
      {from,
         {:modify_dn, obj, newRDN, delOldRDN, newSup,
            controls}} ->
        {res, newData} = do_modify_dn(data, obj, newRDN,
                                        delOldRDN, newSup, controls)
        send(from, res)
        :eldap.loop(cpid, newData)
      ...
      {from, {:start_tls, tlsOptions, timeout, controls}} ->
        ...
        :eldap.loop(cpid, newData)
      {from,
         {:passwd_modify, dn, newPasswd, oldPasswd, controls}} ->
        {res, newData} = do_passwd_modify(data, dn, newPasswd,
                                            oldPasswd, controls)
        send(from, res)
        :eldap.loop(cpid, newData)
      {_From, :close} ->
      ...
      {from, :info} ->
        socketType = (case (r_eldap(data, :ldaps)) do
                        true ->
                          :ssl
                        false ->
                          :tcp
                      end)
        res = %{socket: r_eldap(data, :fd), socket_type: socketType}
        send(from, res)
        :eldap.loop(cpid, data)
      {^cpid, :EXIT, reason} ->
        true
  ...
  defp do_start_tls(data = r_eldap(fd: fD), tlsOptions, timeout,
            controls) do
    case ((try do
      ...
      {:ok, newData} ->
        case (:ssl.connect(fD, tlsOptions, timeout)) do
          {:ok, sslSocket} ->
            {:ok,
               r_eldap(newData, prev_fd: fD,  fd: sslSocket, 
                            using_tls: true)}
          {:error, error} ->
            {{:error, error}, data}
        end
  ...
  defp exec_start_tls(data, controls) do
    ...
    reply = request(r_eldap(data, :fd), data, r_eldap(data, :id),
                      {:extendedReq, req, controls})
  ...
  defp do_simple_bind(data, dn, _Passwd, _) when (dn == '' and
                                        r_eldap(data, :anon_auth) == false) do
    {{:error, :anonymous_auth}, data}
  end

  defp do_simple_bind(data, _Dn, passwd, _) when (passwd == '' and
                                        r_eldap(data, :anon_auth) == false) do
    {{:error, :anonymous_auth}, data}
  end
  ...
  defp do_the_simple_bind(data, dn, passwd, controls) do
    case ((try do
            exec_simple_bind(r_eldap(data, binddn: dn,  passwd: passwd, 
                                       id: bump_id(data)),
                               controls)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp exec_simple_bind(data, controls) do
    req = r_BindRequest(version: r_eldap(data, :version),
  ...
  defp do_search(data, a, controls) do
    case ((try do
      ...
      {{:ok, val}, newData} ->
        {{:ok, val}, newData}
      {:ok, res, ref, resultControls, newData} ->
        {{:ok, polish(res, ref, resultControls)}, newData}
  ...
  defp polish(res, ref, controls) do
    r = polish_result(res)
    r_eldap_search_result(entries: r, referrals: ref, controls: controls)
  end
  ...
  defp do_search_0(data, a, controls) do
    req = r_SearchRequest(baseObject: r_eldap_search(a, :base),
              scope: v_scope(r_eldap_search(a, :scope)),
              derefAliases: v_deref(r_eldap_search(a, :deref)),
              sizeLimit: v_size_limit(r_eldap_search(a, :size_limit)),
    ...
    id = bump_id(data)
    collect_search_responses(r_eldap(data, id: id), req, id,
                               controls)
  end
  ...
  defp collect_search_responses(data, req, iD, controls) do
    ...
    send_request(s, data, iD,
                   {:searchRequest, req, controls})
  ...
  defp collect_search_responses(data, s, iD, {:ok, msg}, acc, ref)
    ...
    case (r_LDAPMessage(msg, :protocolOp)) do
      {:searchResDone, r} ->
        case (r_LDAPResult(r, :resultCode)) do
          :success ->
            ...
            {:ok, acc, ref, r_LDAPMessage(msg, :controls), data}
          :sizeLimitExceeded ->
      ...
      {:searchResEntry, r}
        ...
        resp = recv_response(s, data)
        log2(data, 'search reply = ~p~n', [resp])
        collect_search_responses(data, s, iD, resp, [r | acc],
                                   ref)
      {:searchResRef, r} ->
        resp = recv_response(s, data)
        log2(data, 'search reply = ~p~n', [resp])
        collect_search_responses(data, s, iD, resp, acc,
                                   [r | ref])
      else__ ->
        throw({:error, else__})
    end
  ...
  defp do_add_0(data, entry, attrs, controls) do
    ...
    resp = request(s, data, id,
                     {:addRequest, req, controls})
  ...
  defp do_passwd_modify_0(data, dn, newPasswd, oldPasswd, controls) do
    req = r_PasswdModifyRequestValue(userIdentity: dn, oldPasswd: oldPasswd,
              newPasswd: newPasswd)
    log2(data, 'modify password request = ~p~n', [req])
    {:ok,
       bytes} = :ELDAPv3.encode(:PasswdModifyRequestValue, req)
    extReq = r_ExtendedRequest(requestName: '1.3.6.1.4.1.4203.1.11.1', requestValue: bytes)
    id = bump_id(data)
    log2(data, 'extended request = ~p~n', [extReq])
    reply = request(r_eldap(data, :fd), data, id,
                      {:extendedReq, extReq, controls})
          ...
          :success ->
            case (r_ExtendedResponse(result, :responseValue)) do
              :asn1_NOVALUE ->
                {:ok, data}
              value ->
                case (:ELDAPv3.decode(:PasswdModifyResponseValue,
                                        value)) do
  ...
  defp do_modify_dn(data, entry, newRDN, delOldRDN, newSup,
            controls) do
    case ((try do
  ...
  defp do_modify_dn_0(data, entry, newRDN, delOldRDN, newSup,
            controls) do
    req = r_ModifyDNRequest(entry: entry, newrdn: newRDN,
              deleteoldrdn: delOldRDN, newSuperior: newSup)
    s = r_eldap(data, :fd)
    id = bump_id(data)
    log2(data, 'modify DN request = ~p~n', [req])
    resp = request(s, data, id,
                     {:modDNRequest, req, controls})
  ...
  defp do_unbind(data) do
    ...
    _ = (case (r_eldap(data, :using_tls)) do
           true ->
             send_request(r_eldap(data, :fd), data, r_eldap(data, :id),
                            {:unbindRequest, req})
    ...
    {:no_reply,
       r_eldap(data, binddn: r_eldap(r_eldap(), :binddn), 
                 passwd: r_eldap(r_eldap(), :passwd),  fd: r_eldap(r_eldap(), :fd), 
                 using_tls: false)}
  end
  ...
  defp send_request(s, data, id, {t, p}) do
    send_the_LDAPMessage(s, data,
                           r_LDAPMessage(messageID: id, protocolOp: {t, p}))
  end
  ...
  defp do_recv(s, r_eldap(using_tls: false, timeout: timeout),
            len) do
  ...
  defp v_substr([{key, str} | t]) when (is_list(str) and
                                    key == :initial) or
  ...
  defp v_size_limit(i) when (is_integer(i) and i >= 0) do
    i
  end

  defp v_size_limit(_I) do
  ...
  defp parse_attribute_type([h | t]) when (h >= ?a and h <= ?z) or
                          (h >= ?A and h <= ?Z) do
  ...
  defp parse_attribute_value([?#, x, y | t]) when (x >= ?0 and x <= ?9) or
                                 (x >= ?a and x <= ?f) or
  ...
  defp parse_hexstring([x, y | t], acc) when (x >= ?0 and x <= ?9) or
                                  (x >= ?a and x <= ?f) or
                                  (x >= ?A and x <= ?F and y >= ?0 and
                                     y <= ?9) or
                                  (y >= ?a and y <= ?f) or
  ...
  defp parse_quotation([?\\, x | t], acc) when x == ?, or x == ?= or
                                    x == ?+ or x == ?< or x == ?> or x == ?# or
                                    x == ?; do
    parse_quotation(t, [x, ?\\ | acc])
  end
  ...
  defp parse_quotation([?\\, x, y | t], acc) when (x >= ?0 and
                                        x <= ?9) or
                                       (x >= ?a and x <= ?f) or
                                       (x >= ?A and x <= ?F and y >= ?0 and
                                          y <= ?9) or
  ...
  defp parse_string([h | t], acc) when (h !== ?, and h !== ?= and
                                h !== ?+ and h !== ?< and h !== ?> and
                                h !== ?# and h !== ?; and h !== ?\\ and
                                h !== ?") do
  ...
  defp parse_string([?\\, x | t], acc) when x == ?, or x == ?= or
                                    x == ?+ or x == ?< or x == ?> or x == ?# or
                                    x == ?; do
  ...
  defp parse_string([?\\, x, y | t], acc) when (x >= ?0 and
                                        x <= ?9) or
                                       (x >= ?a and x <= ?f) or
                                       (x >= ?A and x <= ?F and y >= ?0 and
                                          y <= ?9) or
  ...
  defp parse_keychars([h | t], acc) when (h >= ?a and h <= ?z) or
                               (h >= ?A and h <= ?Z) do
  ...
  defp parse_hostport(str) do
    ...
    case (split_string(hostPort, ?:)) do
      ...
      {shost, [?: | sport]} ->
        {rest,
           {parse_host(rest, shost), parse_port(rest, sport)}}
    end
  end
  ...
  def paged_result_control(pageSize) when is_integer(pageSize) do
    paged_result_control(pageSize, '')
  end

  def paged_result_control(pageSize, cookie) when is_integer(pageSize) do
    rSCV = r_RealSearchControlValue(size: pageSize, cookie: cookie)
    {:ok,
       controlValue} = :ELDAPv3.encode(:RealSearchControlValue,
                                         rSCV)
    {:control, '1.2.840.113556.1.4.319', true, controlValue}
  end

  def paged_result_cookie(r_eldap_search_result(controls: controls)) do
    find_paged_result_cookie(controls)
  end

  defp find_paged_result_cookie([]) do
    {:error, :no_cookie}
  end

  defp find_paged_result_cookie([c | controls]) do
    case (c) do
      r_Control(controlType: '1.2.840.113556.1.4.319', controlValue: controlValue) ->
        {:ok,
           r_RealSearchControlValue(cookie: cookie)} = :ELDAPv3.decode(:RealSearchControlValue,
                                                  controlValue)
        {:ok, cookie}
      _ ->
        find_paged_result_cookie(controls)
