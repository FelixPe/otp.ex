defmodule :m_ct_master do
  ...
  defp run_all([], allLogDirs, _, allEvHs, _AllIncludes, nodeOpts, logDirs, initOptions, specs) do
    ...
    log(:tty, ~c"Master Logdir", ~c"~ts", [masterLogDir])
  ...
  def init_master(parent, nodeOptsList, evHandlers, masterLogDir, logDirs, initOptions, specs) do
    case :erlang.whereis(:ct_master) do
      ...
      _Pid ->
        :io.format(~c"~nWarning: ct_master already running!~n")
        exit(:aborted)
    end
    ...
    log(:all, ~c"Master Logger process started", ~c"~w", [mLPid])
    ...
    case specs do
      ...
      _ ->
        ...
        :ct_master_logs.log(~c"Test Specification file(s)", ~c"~ts", [:lists.flatten(specsStr)])
    end
    ...
    add = fn {h, args} ->
      log(:all, ~c"Adding Event Handler", ~c"~w", [h])
  ...
  defp init_master1(parent, nodeOptsList, initOptions, logDirs) do
    ...
    case inaccessible do
      ...
      _ ->
        :io.format(~c"~nThe following nodes are inaccessible: ~p~n~n", [inaccessible])
        :io.format(~c"Proceed(p), Rescan(r) or Abort(a)? ")

        case :io.get_line(:"[p/r/a]>") do
          ~c"p\n" ->
            log(:html, ~c"Inaccessible Nodes", ~c"Proceeding without: ~p", [inaccessible])
  ...
  defp init_master2(parent, nodeOptsList, logDirs) do
    ...
    log(:all, ~c"Cookie", ~c"~tw", [cookie])

    log(:all, ~c"Starting Tests", ~c"Tests starting on: ~p", [
      for {n, _} <- nodeOptsList do
        n
      end
    ])
    ...
    spawnAndMon = fn {node, opts} ->
      ...
      log(:all, ~c"Test Info", ~c"Starting test(s) on ~w...", [node])
  ...
  defp master_loop(r_state(node_ctrl_pids: [], logdirs: logDirs, results: finished)) do
    ...
    log(:all, ~c"TEST RESULTS", ~c"~ts", [str])
    log(:all, ~c"Info", ~c"Updating log files", [])
    ...
    receive do
      {:EXIT, pid, reason} ->
        case get_node(pid, nodeCtrlPids) do
          {node, nodeCtrlPids1} ->
            ...
            case reason do
              :normal ->
                log(:all, ~c"Test Info", ~c"Test(s) on node ~w finished.", [node])
              ...
              bad ->
                ...
                log(:all, ~c"Test Info", ~c"Test on node ~w failed! Reason: ~tp", [node, error])
          ...
          :undefined ->
            log(:all, ~c"Test Info", ~c"Warning! Process ~w has terminated. Reason: ~tp", [
              pid,
              reason
            ])
      ...
      {:nodedown, node} ->
        case get_pid(node, nodeCtrlPids) do
          {_Pid, nodeCtrlPids1} ->
            :erlang.monitor_node(node, false)
            log(:all, ~c"Test Info", ~c"No connection to testnode ~w!", [node])
            {locks1, blocked1} = update_queue(:exit, node, locks, blocked)
      ...
      {:call, :abort, from} ->
        :lists.foreach(
          fn {pid, node} ->
            log(:all, ~c"Test Info", ~c"Aborting tests on ~w", [node])
  ...
  defp update_queue(:take, node, from, lock = {op, resource}, locks, blocked) do
    case :lists.keysearch(lock, 1, locks) do
      {:value, {_Lock, owner}} ->
        log(:html, ~c"Lock Info", ~c"Node ~w blocked on ~w by ~w. Resource: ~tp", [
          node,
          op,
          owner,
          resource
  ...
  defp update_queue(:release, node, _From, lock = {op, resource}, locks, blocked) do
    ...
    case :lists.keysearch(lock, 1, blocked) do
      {:value, e = {^lock, someNode, waitingPid}} ->
        ...
        log(:html, ~c"Lock Info", ~c"Node ~w proceeds with ~w. Resource: ~tp", [
          someNode,
          op,
          resource
        ])
  ...
  defp refresh_logs([], refreshed) do
    str =
      :lists.map(
        fn {d, result} ->
          :io_lib.format(~c"Refreshing logs in ~tp... ~tp", [d, result])
        end,
        refreshed
      )

    log(:all, ~c"Info", ~c"~ts", [str])
  end
  ...
  def init_node_ctrl(masterPid, cookie, opts) do
    ...
    :io.format(~c"~n********** node_ctrl process ~w started on ~w **********~n", [self(), node()])
    ...
    case :net_adm.ping(masterNode) do
      ...
      :pang ->
        :io.format(~c"Warning! Connection to master node ~w is lost. Can't report result!~n~n", [
          masterNode
        ])
    end
  end
    ...
    :lists.foreach(
      fn {nodeName, options} ->
        ...
        case {hasNodeStart, isAlive} do
          {false, false} ->
            :io.format(~c"WARNING: Node ~w is not alive but has no node_start option~n", [
              nodeName
            ])

          {false, true} ->
            :io.format(~c"Node ~w is alive~n", [nodeName])
          ...
          {true, false} ->
            ...
            case callback.start(host, node, nodeStart2) do
              {:ok, ^nodeName} ->
                :io.format(~c"Node ~w started successfully with callback ~w~n", [
                  nodeName,
                  callback
                ])

              {:error, reason, _NodeName} ->
                :io.format(~c"Failed to start node ~w with callback ~w! Reason: ~tp~n", [
                  nodeName,
                  callback,
                  reason
                ])
          ...
          {true, true} ->
            :io.format(~c"WARNING: Node ~w is alive but has node_start option~n", [nodeName])
        end
      end,
      initOptions
    )
    ...
    :lists.foreach(
      fn {nodeName, options} ->
        ...
        case {hasEval, isAlive} do
          ...
          {true, false} ->
            :io.format(~c"WARNING: Node ~w is not alive but has eval option~n", [nodeName])
  ...
  defp evaluate(node, [{m, f, a} | mFAs]) do
    case :rpc.call(node, m, f, a) do
      {:badrpc, reason} ->
        :io.format(~c"WARNING: Failed to call ~w:~tw/~w on node ~w due to ~tp~n", [
          m,
          f,
          length(a),
          node,
      ...
      result ->
        :io.format(~c"Called ~w:~tw/~w on node ~w, result: ~tp~n", [m, f, length(a), node, result])
    end
