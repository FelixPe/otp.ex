defmodule :m_ct_master do
  ...
  Record.defrecord(:r_state, :state, node_ctrl_pids: [],
                                 logdirs: [], results: [], locks: [],
                                 blocked: [])
  ...
  def run([tS | testSpecs], allowUserTerms, inclNodes,
           exclNodes)
    ...
    case ((try do
      ...
      {:error, reason} ->
        [{:error, reason} | run(testSpecs, allowUserTerms,
                                  inclNodes, exclNodes)]
      tests ->
        runResult = :lists.map(fn {specs,
                                     tSRec = r_testspec(logdir: allLogDirs,
                                                 config: stdCfgFiles,
                                    ...
                                    runSkipPerNode2 = exclude_nodes(exclNodes,
                                                                      runSkipPerNode)
                                    tSList = (cond do
                                                is_integer(hd(tS)) ->
                                                  [tS]
                                    ...
                                    {specs,
                                       run_all(runSkipPerNode2, allLogDirs,
                                                 allCfgFiles, allEvHs,
                                                 allIncludes, [], [],
                                                 allInitOpts, tSList)}
                               end,
                                 tests)
        runResult ++ run(testSpecs, allowUserTerms, inclNodes,
                           exclNodes)
    end
  end
  ...
  def run_on_node([tS | testSpecs], allowUserTerms, node)
    ...
    case ((try do
      ...
      {:error, reason} ->
        [{:error, reason} | run_on_node(testSpecs,
                                          allowUserTerms, node)]
      tests ->
        runResult = :lists.map(fn {specs,
                                     tSRec = r_testspec(logdir: allLogDirs,
                                    ...
                                    allCfgFiles = {stdCfgFiles, userCfgFiles}
                                    {run,
                                       skip} = :ct_testspec.prepare_tests(tSRec,
                                                                            node)
                                    tSList = (cond do
                                                is_integer(hd(tS)) ->
                                                  [tS]
                                    ...
                                    {specs,
                                       run_all([{node, run, skip}], allLogDirs,
                                                 allCfgFiles, allEvHs,
                                                 allIncludes, [], [],
                                                 allInitOpts, tSList)}
                               end,
                                 tests)
        runResult ++ run_on_node(testSpecs, allowUserTerms,
                                   node)
    end
  end
  ...
  defp run_all([{node, run, skip} | rest], allLogDirs,
            {allStdCfgFiles, allUserCfgFiles} = allCfgFiles,
            allEvHs, allIncludes, nodeOpts, logDirs, initOptions,
            specs) do
    logDir = :lists.foldl(fn {n, dir}, _Found when n == node
                                                   ->
                               dir
                             {_N, _Dir}, found ->
                               found
                            ...
                            '.', allLogDirs)
    stdCfgFiles = :lists.foldr(fn {n, f}, fs when n == node
                                                  ->
                                    [f | fs]
                                  {_N, _F}, fs ->
                                    fs
                                 ...
                                 [], allStdCfgFiles)
    userCfgFiles = :lists.foldr(fn {n, f}, fs when n == node
                                                   ->
                                     [{:userconfig, f} | fs]
                                   {_N, _F}, fs ->
                                     fs
                                  ...
                                  [], allUserCfgFiles)
    includes = :lists.foldr(fn {n, i}, acc when n === node
                                                ->
                                 [i | acc]
                               {_, _}, acc ->
                                 acc
                              ...
                              [], allIncludes)
    evHs = :lists.foldr(fn {n, h, a}, hs when n == node ->
                             [{h, a} | hs]
                           {_N, _H, _A}, hs ->
                             hs
                           {h, a}, hs ->
                          ...
                          [], allEvHs)
    nO = {node,
            [{:prepared_tests, {run, skip}, specs}, {:logdir,
                                                       logDir},
                                                        {:include, includes},
                                                            {:config,
                                                               stdCfgFiles},
                                                                {:event_handler,
                                                                   evHs}] ++ userCfgFiles}
    run_all(rest, allLogDirs, allCfgFiles, allEvHs,
              allIncludes, [nO | nodeOpts], [logDir | logDirs],
              initOptions, specs)
  end

  defp run_all([], allLogDirs, _, allEvHs, _AllIncludes,
            nodeOpts, logDirs, initOptions, specs) do
    handlers = (for {master, h, a} <- allEvHs,
                      master == :master do
                  {h, a}
                end)
    masterLogDir = (case (:lists.keysearch(:master, 1,
                                             allLogDirs)) do
                      {:value, {_, dir}} ->
                        dir
                      false ->
    ...
    start_master(:lists.reverse(nodeOpts), handlers,
  ...
  def basic_html(bool) do
    :application.set_env(:common_test_master, :basic_html,
                           bool)
    :ok
  end

  def esc_chars(bool) do
    :application.set_env(:common_test_master, :esc_chars,
                           bool)
    :ok
  end
  ...
  defp start_master(nodeOptsList, evHandlers, masterLogDir, logDirs,
            initOptions, specs) do
    master = spawn_link(:ct_master, :init_master,
                          [self(), nodeOptsList, evHandlers, masterLogDir,
                                                                 logDirs,
                                                                     initOptions,
                                                                         specs])
    receive do
  ...
  def init_master(parent, nodeOptsList, evHandlers, masterLogDir,
           logDirs, initOptions, specs) do
    ...
    add = fn {h, args} ->
               ...
               case (:gen_event.add_handler(:ct_master_event, h,
                                              args)) do
                 :ok ->
                   :ok
                 {:EXIT, why} ->
  ...
  defp init_master1(parent, nodeOptsList, initOptions, logDirs) do
    {inaccessible, nodeOptsList1,
       initOptions1} = init_nodes(nodeOptsList, initOptions)
    case (inaccessible) do
      ...
      _ ->
        ...
        case (:io.get_line(:"[p/r/a]>")) do
          ...
          'r\n' ->
            init_master1(parent, nodeOptsList, initOptions1,
                           logDirs)
          _ ->
  ...
  defp init_master2(parent, nodeOptsList, logDirs) do
    ...
    spawnAndMon = fn {node, opts} ->
                       ...
                       {:erlang.spawn_link(node, :ct_master, :init_node_ctrl,
                                             [self(), cookie, opts]),
                          node}
                  end
    nodeCtrlPids = :lists.map(spawnAndMon, nodeOptsList)
    result = master_loop(r_state(node_ctrl_pids: nodeCtrlPids,
                             logdirs: logDirs))
    send(parent, {self(), result})
  end

  defp master_loop(r_state(node_ctrl_pids: [], logdirs: logDirs,
              results: finished)) do
    str = :lists.map(fn {node, result} ->
                          :io_lib.format('~-40.40.*ts~tp\n',
                                           [?_, :erlang.atom_to_list(node),
                                                    result])
                     end,
  ...
  defp master_loop(state = r_state(node_ctrl_pids: nodeCtrlPids,
                      results: results, locks: locks, blocked: blocked)) do
    receive do
      {:EXIT, pid, reason} ->
          ...
          {node, nodeCtrlPids1} ->
            ...
            case (reason) do
              ...
              bad ->
                ...
                {locks1, blocked1} = update_queue(:exit, node, locks,
                                                    blocked)
                master_loop(r_state(state, node_ctrl_pids: nodeCtrlPids1, 
                                       results: [{node, error} | results], 
                                       locks: locks1,  blocked: blocked1))
            end
          :undefined ->
            log(:all, 'Test Info', 'Warning! Process ~w has terminated. Reason: ~tp', [pid, reason])
            master_loop(state)
        end
      {:nodedown, node} ->
      ...
      {pid, {:result, result}} ->
        ...
        master_loop(r_state(state, results: [{node, result} |
                                           results]))
      {:call, :progress, from} ->
        reply(master_progress(nodeCtrlPids, results), from)
        master_loop(state)
      {:call, :abort, from} ->
        :lists.foreach(fn {pid, node} ->
      ...
      {:call, r_event(name: name, node: node, data: data), from} ->
        ...
        {locks1, blocked1} = update_queue(op, node, from, lock,
                                            locks, blocked)
        cond do
          op == :release ->
            reply(:ok, from)
  ...
  defp update_queue(:take, node, from, lock = {op, resource}, locks,
            blocked) do
    case (:lists.keysearch(lock, 1, locks)) do
      {:value, {_Lock, owner}} ->
        log(:html, 'Lock Info', 'Node ~w blocked on ~w by ~w. Resource: ~tp', [node, op, owner, resource])
  ...
  defp update_queue(:release, node, _From, lock = {op, resource},
            locks, blocked) do
    ...
    case (:lists.keysearch(lock, 1, blocked)) do
      {:value, e = {^lock, someNode, waitingPid}} ->
        ...
        log(:html, 'Lock Info', 'Node ~w proceeds with ~w. Resource: ~tp', [someNode, op, resource])
        reply(:ok, waitingPid)
        {locks1, blocked1}
      false ->
        {locks1, blocked}
  ...
  defp update_queue(:exit, node, locks, blocked) do
    nodeLocks = :lists.foldl(fn {l, n}, ls when n == node ->
                                  [l | ls]
                                _, ls ->
                                  ls
                             end,
  ...
  defp release_locks(node, [lock | ls], locks, blocked) do
    {locks1, blocked1} = update_queue(:release, node,
                                        :undefined, lock, locks, blocked)
  ...
  defp ping_nodes([nO = {node, _Opts} | nOs], inaccessible,
            nodeOpts) do
  ...
  defp master_progress(nodeCtrlPids, results) do
    results ++ :lists.map(fn {_Pid, node} ->
                               {node, :ongoing}
                          end,
                            nodeCtrlPids)
  end
  ...
  def init_node_ctrl(masterPid, cookie, opts) do
    ...
    case (:net_adm.ping(masterNode)) do
      ...
      :pang ->
        :io.format('Warning! Connection to master node ~w is lost. Can\'t report result!~n~n', [masterNode])
    end
  end
  ...
  def status(masterPid,
           event = r_event(name: :finished_write_file)) do
    call(masterPid, event)
  end
  ...
  defp init_nodes(nodeOptions, initOptions) do
    ...
    initOptions1 = filter_accessible(initOptions,
                                       inaccessible)
  ...
  defp start_nodes(initOptions) do
    :lists.foreach(fn {nodeName, options} ->
                        [nodeS,
                             hostS] = :string.lexemes(:erlang.atom_to_list(nodeName),
                                                        '@')
                        ...
                        case ({hasNodeStart, isAlive}) do
                          ...
                          {true, false} ->
                            {:node_start,
                               nodeStart} = :lists.keyfind(:node_start, 1,
                                                             options)
                            {:value, {:callback_module, callback},
                               nodeStart2} = :lists.keytake(:callback_module, 1,
                                                              nodeStart)
                            case (callback.start(host, node, nodeStart2)) do
                              ...
                              {:error, reason, _NodeName} ->
                                :io.format('Failed to start node ~w with callback ~w! Reason: ~tp~n', [nodeName, callback, reason])
                            end
                          {true, true} ->
  ...
  defp evaluate(node, [{m, f, a} | mFAs]) do
    case (:rpc.call(node, m, f, a)) do
      {:badrpc, reason} ->
        :io.format('WARNING: Failed to call ~w:~tw/~w on node ~w due to ~tp~n', [m, f, length(a), node, reason])
      result ->
