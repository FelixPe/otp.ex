defmodule :m_test_server_ctrl do
  ...
  def add_spec(spec) do
    name = :filename.rootname(spec, ~c".spec")
  ...
  defp parse_cmd_line([:SPEC, spec | cmds], specList, names, param, cov, tCCB) do
    case :file.consult(spec) do
      {:ok, termList} ->
        name = :filename.rootname(spec)
        parse_cmd_line(cmds, termList ++ specList, [name | names], param, cov, tCCB)

      {:error, reason} ->
        :io.format(~c"Can't open ~tw: ~tp\n", [spec, :file.format_error(reason)])
        parse_cmd_line(cmds, specList, names, param, cov, tCCB)
    end
  end

  defp parse_cmd_line([:NAME, name | cmds], specList, names, param, cov, tCCB) do
    parse_cmd_line(
      cmds,
      specList,
  ...
  defp parse_cmd_line([:SKIPMOD, mod | cmds], specList, names, param, cov, tCCB) do
    parse_cmd_line(
      cmds,
      [{:skip, {mod, ~c"by command line"}} | specList],
      names,
      param,
      cov,
      tCCB
  ...
  defp parse_cmd_line([:SKIPCASE, mod, case__ | cmds], specList, names, param, cov, tCCB) do
    parse_cmd_line(
      cmds,
      [{:skip, {mod, case__, ~c"by command line"}} | specList],
      names,
      param,
      cov,
      tCCB
  ...
  defp parse_cmd_line([:DIR, dir | cmds], specList, names, param, cov, tCCB) do
    name = :filename.basename(dir)
    parse_cmd_line(cmds, [{:topcase, {:dir, name}} | specList], [name | names], param, cov, tCCB)
  end

  defp parse_cmd_line([:MODULE, mod | cmds], specList, names, param, cov, tCCB) do
    parse_cmd_line(
      cmds,
  ...
  defp parse_cmd_line([:CASE, mod, case__ | cmds], specList, names, param, cov, tCCB) do
    parse_cmd_line(
      cmds,
  ...
  defp parse_cmd_line([:COVER, app, cF, analyse | cmds], specList, names, param, _Cov, tCCB) do
    parse_cmd_line(cmds, specList, names, param, {{app, cF}, analyse}, tCCB)
  end

  defp parse_cmd_line([:TESTCASE_CALLBACK, mod, func | cmds], specList, names, param, cov, _) do
    parse_cmd_line(cmds, specList, names, param, cov, {mod, func})
  end

  defp parse_cmd_line([obj | _Cmds], _SpecList, _Names, _Param, __Cov, _TCCB) do
    :io.format(~c"~w: Bad argument: ~tw\n", [:test_server_ctrl, obj])
    :io.format(~c" Use the `ts' module to start tests.\n", [])
    :io.format(~c" (If you ARE using `ts', there is a bug in `ts'.)\n", [])
    :erlang.halt(1)
  end

  defp parse_cmd_line([], specList, names, param, cov, tCCB) do
    nameList = :lists.reverse(names, [~c"suite"])
  ...
  defp cast_to_list(x) do
    :lists.flatten(:io_lib.format(~c"~tw", [x]))
  end
  ...
  def init([]) do
    case :os.getenv(~c"TEST_SERVER_CALL_TRACE") do
      false ->
        :ok
    ...
    case modFunc do
      {mod, func} ->
        ...
        case :erlang.function_exported(mod, func, 4) do
          ...
          false ->
            :io.format(:user, ~c"WARNING! Callback function ~w:~tw/4 undefined.~n~n", [mod, func])
        end
  ...
  def handle_call({:find_release, release}, from, state = r_state(old_releases: oldReleases)) do
    case :maps.find(release, oldReleases) do
      :error ->
        r =
          case :test_server_node.find_release(release) do
            :none ->
              :not_available
        ...
        handle_call(
          ...
          from,
          r_state(state, old_releases: Map.put(oldReleases, release, r))
        )
    ...
    case :lists.keysearch(pid, 2, r_state(state, :jobs)) do
      ...
      {:value, {name, _}} ->
        ...
        case reason do
          ...
          :killed ->
            :io.format(~c"Suite ~ts was killed\n", [name])

          _Other ->
            :io.format(~c"Suite ~ts was killed with reason ~tp\n", [name, reason])
        end
  ...
  defp init_tester(
    ...
    case :os.getenv(~c"TEST_SERVER_FRAMEWORK") do
      fW when fW === false or fW === ~c"undefined" ->
      ...
      fW ->
        ...
        case :os.getenv(~c"TEST_SERVER_FRAMEWORK_NAME") do
          fWName when fWName === false or fWName === ~c"undefined" ->
    ...
    case result do
      ...
      {:EXIT, _Pid, reason} ->
        print(1, ~c"EXIT, reason ~tp", [reason])
    ...
    elapsedTimeSeconds = timeMy / 1_000_000
    ...
    {skippedN, skipStr} =
      case :erlang.get(:test_server_skipped) do
        ...
        {uSkipped, aSkipped} ->
          ...
          {skipped, :io_lib.format(~c", ~w Skipped", [skipped])}
      end
  ...
  defp do_spec_terms([other | terms], topCases, skipList, config) do
    :io.format(~c"** WARNING: Spec file contains unknown directive ~tp\n", [other])
  ...
  def do_test_cases(topCases, skipCases, config, timetrapData)
    ...
    case collect_all_cases(topCases, skipCases) do
      {:error, why} ->
        print(1, ~c"Error starting: ~tp", [why])
        exit(:test_suites_done)
      ...
      testSpec0 ->
        ...
        print(1, ~c"Starting test~ts", [
          print_if_known(n, {~c", ~w test cases", [n]}, {~c" (with repeated test cases)", []})
        ])
        ...
        testName =
          cond do
            is_list(test) ->
              :lists.flatten(:io_lib.format(~c"~ts", [test]))

            true ->
              :lists.flatten(:io_lib.format(~c"~tp", [test]))
          end

        testDescr = ~c"Test " ++ testName ++ ~c" results"
        ...
        {header, footer} =
          case :test_server_sup.framework_call(
            ...
            empty when empty == ~c"" or :erlang.element(2, empty) == ~c"" ->
              :erlang.put(:basic_html, true)

              {[html_header(testDescr), ~c"<h2>Results for test ", testName, ~c"</h2>\n"],
               ~c"\n</body>\n</html>\n"}

            {:basic_html, html0, html1} ->
              :erlang.put(:basic_html, true)
              {html0 ++ [~c"<h1>Results for <i>", testName, ~c"</i></h1>\n"], html1}
        ...
        print(:html, ~c"~ts", [header])
        print(:html, xhtml(~c"<p>", ~c"<h4>"))
        print_timestamp(:html, ~c"Test started at ")
        print(:html, xhtml(~c"</p>", ~c"</h4>"))
        print(:html, xhtml(~c"\n<p><b>Host info:</b><br>\n", ~c"\n<p><b>Host info:</b><br />\n"))
        ...
        print(
          :html,
          xhtml(
            ~c"<br>Used Erlang v~ts in <code>~ts</code></p>\n",
            ~c"<br />Used Erlang v~ts in \"~ts\"</p>\n"
          ),
        ...
        coverLog =
          case :erlang.get(:test_server_cover_log_dir) do
            ...
            absLogDir ->
              absLog = :filename.join(absLogDir, ~c"cover.html")
        ...
        print(
          ...
          [~c"suite.log", coverLog, ~c"unexpected_io.log.html"]
        )
        ...
        print(
          ...
          ~c"<p>~ts</p>\n" ++
            xhtml(
              ~c"<table bgcolor=\"white\" border=\"3\" cellpadding=\"5\">\n" ++ ~c"<thead>\n",
              ~c"<table id=\"" ++ ~c"SortableTable" ++ ~c"\">\n" ++ ~c"<thead>\n"
            ) ++
            ~c"<tr><th>Num</th><th>Module</th><th>Group</th>" ++
            ~c"<th>Case</th><th>Log</th><th>Time</th><th>Result</th>" ++
            ~c"<th>Comment</th></tr>\n</thead>\n<tbody>\n",
          [
            print_if_known(
              n,
              {~c"<i>Executing <b>~w</b> test cases...</i>" ++
                 xhtml(~c"\n<br>\n", ~c"\n<br />\n"), [n]},
              {~c"", []}
            )
          ]
        ...
        print(:major, ~c"=cases         ~w", [:erlang.get(:test_server_cases)])
        print(:major, ~c"=user          ~ts", [r_target_info(tI, :username)])
        print(:major, ~c"=host          ~ts", [r_target_info(tI, :host)])

        case controller_call(:get_hosts) do
          [] ->
            print(:major, ~c"=hosts         ~ts", [r_target_info(tI, :host)])
          ...
          hosts ->
            ...
            print(:major, ~c"=hosts         ~ts", [str])
        end

        print(:major, ~c"=emulator_vsn  ~ts", [r_target_info(tI, :version)])
        print(:major, ~c"=emulator      ~ts", [r_target_info(tI, :emulator)])
        print(:major, ~c"=otp_release   ~ts", [r_target_info(tI, :otp_release)])
        print(:major, ~c"=started       ~s", [:lists.flatten(timestamp_get(~c""))])
  ...
  defp start_log_file() do
    ...
    testDir = timestamp_filename_get(:filename.join(dir, ~c"run."))
    ...
    :ok = write_file(:filename.join(dir, ~c"last_name"), testDir1 ++ ~c"\n", filenameMode)
    :ok = write_file(~c"last_name", testDir1 ++ ~c"\n", filenameMode)
    :erlang.put(:test_server_log_dir_base, testDir1)
    majorName = :filename.join(testDir1, ~c"suite.log")
    htmlName = majorName ++ ~c".html"
    unexpectedName = :filename.join(testDir1, ~c"unexpected_io.log.html")
    ...
    {unexpHeader, unexpFooter} =
      case :test_server_sup.framework_call(
             :get_html_wrapper,
             [~c"Unexpected I/O log", false, testDir, :undefined],
             ~c""
           ) do
        uEmpty
        when uEmpty == ~c"" or
               :erlang.element(2, uEmpty) == ~c"" ->
          {html_header(~c"Unexpected I/O log"), ~c"\n</body>\n</html>\n"}
    ...
    redirectLink = :filename.join(topDir, ~c"suite.log.latest" ++ ~c".html")
    make_html_link(redirectLink, htmlName, :redirect)

    make_html_link(
      :filename.absname(~c"last_test" ++ ~c".html"),
      htmlName,
      :filename.basename(dir)
    )

    linkName = :filename.join(dir, ~c"last_link")
    make_html_link(linkName ++ ~c".html", htmlName, :filename.basename(dir))
    privDir = :filename.join(testDir1, ~c"log_private")
    :ok = :file.make_dir(privDir)
    :erlang.put(:test_server_priv_dir, privDir ++ ~c"/")
    print_timestamp(:major, ~c"Suite started at ")
  ...
  defp make_html_link(linkName, target, explanation) do
    ...
    href =
      case :lists.prefix(pwdL, targetL) do
        ...
        false ->
          ~c"file:" ++ uri_encode(target)
      end

    h =
      cond do
        explanation == :redirect ->
          meta = [~c"<meta http-equiv=\"refresh\" content=\"0; url=", href, ~c"\" />\n"]
          [html_header(~c"redirect", meta), ~c"</html>\n"]
  ...
  defp start_minor_log_file1(mod, func, logDir, absName, mFA) do
    ...
    testDescr = :io_lib.format(~c"Test ~w:~tw result", [mod, func])
    ...
    :io.put_chars(fd, ~c"<a name=\"top\"></a>")
    :io.put_chars(fd, ~c"<pre>\n")
    srcListing = downcase(:erlang.atom_to_list(mod)) ++ ~c".src.html"
        ...
        case {:filelib.is_file(
          ...
          {true, false} ->
            print(
              ...
              [
                ~c"$tc_html",
                info ++ ~c"<a href=\"~ts#~ts\">~w:~tw/~w</a> (click for source code)\n"
              ],
              [
                uri_encode(srcListing),
                uri_encode(:erlang.atom_to_list(func) ++ ~c"-1", :utf8),
                mod,
                func,
                arity
              ]
          ...
          _ ->
            print(lev, [~c"$tc_html", info ++ ~c"~w:~tw/~w\n"], [mod, func, arity])
        end
    end
  ...
  defp minor_log_file_name(mod, func) do
    minor_log_file_name(mod, func, ~c"")
  end
  ...
  defp html_convert_modules([mod | mods]) do
    case :code.which(mod) do
      path when is_list(path) ->
        srcFile = :filename.rootname(path) ++ ~c".erl"
        ...
        case foundSrcFile do
          ...
          {srcFile1, srcFileInfo} ->
            ...
            destFile = :filename.join(destDir, downcase(name) ++ ~c".src.html")
  ...
  defp html_possibly_convert(src, srcInfo, dest) do
    case :file.read_file_info(dest) do
      ...
      _ ->
        ...
        case :test_server_sup.framework_call(
               :get_html_wrapper,
               [~c"Module " ++ src, false, outDir, :undefined, encoding(src)],
               ~c""
             ) do
          empty when empty == ~c"" or :erlang.element(2, empty) == ~c"" ->
  ...
  defp copy_html_files(inDir, outDir) do
    files = :filelib.wildcard(:filename.join(inDir, ~c"*" ++ ~c".src.html"))
  ...
  defp copy_html_file(src, destDir) do
    ...
    case :file.read_file(src) do
      ...
      {:error, _Reason} ->
        :io.format(~c"File ~ts: read failed\n", [src])
    end
  end
  ...
  defp run_test_cases(testSpec, config, timetrapData) do
    ...
    {allSkippedN, userSkipN, autoSkipN, skipStr} =
      case :erlang.get(:test_server_skipped) do
        ...
        {uS, aS} ->
          {uS + aS, uS, aS, :io_lib.format(~c", ~w skipped", [uS + aS])}
      end

    okN = :erlang.get(:test_server_ok)
    failedN = :erlang.get(:test_server_failed)

    print(1, ~c"TEST COMPLETE, ~w ok, ~w failed~ts of ~w test cases\n", [
      okN,
      failedN,
      skipStr,
    ...
    print(:major, ~c"=finished      ~s", [:lists.flatten(timestamp_get(~c""))])
    print(:major, ~c"=failed        ~w", [failedN])
    print(:major, ~c"=successful    ~w", [okN])
    print(:major, ~c"=user_skipped  ~w", [userSkipN])
    print(:major, ~c"=auto_skipped  ~w", [autoSkipN])
    exit(:test_suites_done)
  end
  ...
  defp run_test_cases_loop(
      ...
      cond do
        startConf ->
          case get_shuffle(props) do
            ...
            {_, seed} ->
              useSeed =
                case :os.getenv(~c"TS_RANDOM_SEED") do
                  undef
                  when undef == false or
                         undef == ~c"undefined" ->
      ...
      cond do
        ...
        not startConf ->
          ...
          reportStop = fn ->
            print(
              :minor,
              ~c"~n*** Stopping repeat operation ~w",
              [repVal]
            )

            print(
              1,
              ~c"Stopping repeat operation ~w",
              [repVal]
            )
          end
    ...
    reportAbortRepeat = fn
      what when repeating ->
        print(:minor, ~c"~n*** Aborting repeat operation (configuration case ~w)", [what])
        print(1, ~c"Aborting repeat operation (configuration case ~w)", [what])
    ...
    case confCaseResult do
      ...
      when func == :init_per_suite and
        ...
        case :lists.filter(
          ...
          bad ->
            print(:minor, ~c"~n*** ~tw returned bad elements in Config: ~tp.~n", [func, bad])
      ...
      {_, {:framework_error, {fwMod, fwFunc}, reason}, _} ->
        print(:minor, ~c"~n*** ~w failed in ~tw. Reason: ~tp~n", [fwMod, fwFunc, reason])
        print(1, ~c"~w failed in ~tw. Reason: ~tp~n", [fwMod, fwFunc, reason])
        ...
        {cases2, config1, status3} =
          cond do
            startConf ->
              ...
              print(:minor, ~c"~n*** ~tw failed.~n    Skipping all cases.", [func])
        ...
        {cases2, config1, status3} =
          cond do
            startConf ->
              ...
              print(:minor, ~c"~n*** ~tw auto skipped.~n    Skipping all cases.", [func])
      ...
      when startConf and (skip == :skip or skip == :skipped) ->
        ...
        print(:minor, ~c"~n*** ~tw skipped.~n    Skipping all cases.", [func])
        set_io_buffering(iOHandler)
        stop_minor_log_file()
      ...
      {_, _Other, _} when func == :init_per_suite ->
        print(:minor, ~c"~n*** init_per_suite failed to return a Config list.~n", [])
        reason = {:failed, {mod, :init_per_suite, :bad_return}}
        cases2 = skip_cases_upto(ref, cases, reason, :conf, currMode, :auto_skip_case)
        set_io_buffering(iOHandler)
        stop_minor_log_file()
        ...
        case check_prop(:sequence, mode) do
          ...
          ref ->
            cond do
              ...
              true ->
                print(:minor, ~c"~n*** ~tw failed.~n    Skipping all other cases in sequence.", [
                  func
                ])
  ...
  defp get_data_dir(mod, suite) do
    ...
    case :code.which(useMod) do
      :non_existing ->
        print(12, ~c"The module ~w is not loaded", [mod])
        []
  ...
  defp do_get_data_dir(mod, file) do
    :filename.dirname(file) ++ ~c"/" ++ :erlang.atom_to_list(mod) ++ ~c"_data/"
  end
  ...
  defp print_conf_time(confTime) do
    print(:major, ~c"=group_time    ~.3fs", [confTime])
    print(:minor, ~c"~n=== Total execution time of group: ~.3fs~n", [confTime])
  end
  ...
  defp print_props(props) do
    print(:major, ~c"=group_props   ~tp", [props])
    print(:minor, ~c"Group properties: ~tp~n", [props])
  end
  ...
  defp skip_case1(type, caseNum, mod, func, comment, mode) do
    ...
    print(:major, ~c"~n=case          ~w:~tw", [mod, func])
    ...
    groupName =
      case get_name(mode) do
        ...
        grName ->
          ...
          print(:major, ~c"=group_props   ~tp", [[{:name, grName1}]])
          grName1
      end
    ...
    cond do
      type == :auto ->
        print(:major, ~c"=result        auto_skipped: ~ts", [comment1])

      type == :user ->
        print(:major, ~c"=result        skipped: ~ts", [comment1])
    end

    cond do
      caseNum == 0 ->
        print(2, ~c"*** Skipping ~tw ***", [{mod, func}])

      true ->
        print(2, ~c"*** Skipping test case #~w ~tw ***", [caseNum, {mod, func}])
    end

    tR = xhtml(~c"<tr valign=\"top\">", ~c"<tr class=\"" ++ odd_or_even() ++ ~c"\">")
    ...
    print(
      ...
      tR ++
        ...
        col0 ++
        ~c"0.000s" ++
        col1 ++ ~c"</td><td><font color=\"~ts\">SKIPPED</font></td><td>~ts</td></tr>\n",
  ...
  defp wait_and_resend(ref, [{_, currPid, caseNum, mod, func} | ps] = cases, ok, skip, fail) do
    receive do
      ...
      {:EXIT, ^currPid, reason} when reason != :normal ->
        ...
        print(1, ~c"Error! Process for test case #~w (~w:~tw) died! Reason: ~tp", [
          caseNum,
          mod,
          func,
          reason
  ...
  defp run_test_case1(ref, num, mod, func, args, runInit, timetrapData, mode, main) do
    ...
    print(:major, ~c"=case          ~w:~tw", [mod, func])
    minorName = start_minor_log_file(mod, func, self() != main)
    minorBase = :filename.basename(minorName)
    print(:major, ~c"=logfile       ~ts", [:filename.basename(minorName)])
    ...
    updatedArgs =
      case :erlang.get(:test_server_create_priv_dir) do
        ...
        privDirMode ->
          ...
          ext =
            cond do
              num == 0 ->
                ...
                :lists.flatten(:io_lib.format(~c".cfg.~w", [int]))

              true ->
                :lists.flatten(:io_lib.format(~c".~w", [num]))
            end

          privDir = :filename.join(runDir, ~c"log_private") ++ ext
          ...
          update_config(
            hd(args),
            [{:priv_dir, privDir ++ ~c"/"}, {:tc_logfile, minorName}]
          )
      end
    ...
    print(:minor, ~c"~ts", [
      escape_chars(:io_lib.format(~c"Config value:\n\n    ~tp\n", [args2Print]))
    ])

    print(:minor, ~c"Current directory is ~tp\n", [cwd])
    ...
    print(
      ...
      tR ++
        ...
        ~c"~ts" ++
        col1 ++
        ~c"</td><td><a href=\"~ts\">~tw</a></td><td><a href=\"~ts#top\">&lt;</a> <a href=\"~ts#end\">&gt;</a></td>",
    ...
    print(:minor, ~c"<a name=\"end\"></a>", [], :internal_raw)
    print(:minor, ~c"\n", [], :internal_raw)
    print_timestamp(:minor, ~c"Ended at ")
    print(:major, ~c"=ended         ~s", [timestamp_get(~c"")])
          ...
          true ->
            cond do
              procsBefore < procsAfter ->
                print(:minor, ~c"WARNING: ~w more processes in system after test case", [
                  procsAfter - procsBefore
                ])

              procsBefore > procsAfter ->
                print(:minor, ~c"WARNING: ~w less processes in system after test case", [
                  procsBefore - procsAfter
                ])
          ...
          true ->
            case (try do
              ...
              {:EXIT, _} = exit ->
                print(
                  :minor,
                  ~c"WARNING: There might be slavenodes left in the system. I tried to kill them, but I failed: ~tp\n",
                  [exit]
                )
              ...
              list ->
                print(
                  :minor,
                  ~c"WARNING: ~w slave nodes in system after test" ++
                    ~c"case. Tried to killed them.~n" ++ ~c"         Names:~tp",
                  [length(list), list]
                )
            end
  ...
  defp progress(:skip, caseNum, mod, func, grName, loc, reason, time, comment, {st0, st1}) do
    {reason1, {color, ret, reportTag}} =
      if_auto_skip(
        reason,
        fn ->
          {~c"#FFA64D", :auto_skip, :auto_skipped}
        end,
        fn ->
          {~c"#FF8000", :skip, :skipped}
        end
      )

    print(:major, ~c"=result        ~w: ~tp", [reportTag, reason1])

    print(1, ~c"*** SKIPPED ~ts ***", [
    ...
    reasonStr2 =
      cond do
        reasonLength > 80 ->
          :string.slice(reasonStr1, 0, 77) ++ ~c"..."
    ...
    comment1 =
      case comment do
        ...
        _ ->
          xhtml(~c"<br>(", ~c"<br />(") ++ to_string(comment) ++ ~c")"
      end
    ...
    print(
      ...
      ~c"<td>" ++
        st0 ++
        ~c"~ts" ++
        st1 ++ ~c"</td><td><font color=\"~ts\">SKIPPED</font></td><td>~ts~ts</td></tr>\n",
      [timeStr, color, reasonStr2, comment1]
    )

    formatLoc = :test_server_sup.format_loc(loc)
    print(:minor, ~c"=== Location: ~ts", [formatLoc])
    print(:minor, ~c"=== Reason: ~ts", [reasonStr1])
    ret
  end
  ...
  defp progress(
    ...
    print(:major, ~c"=result        failed: timeout, ~tp", [loc])
    ...
    errorReason = :io_lib.format(~c"{timetrap_timeout,~ts}", [formatLastLoc])

    comment =
      case comment0 do
        ~c"" ->
          ~c"<font color=\"red\">" ++ errorReason ++ ~c"</font>"

        _ ->
          ~c"<font color=\"red\">" ++
            errorReason ++ xhtml(~c"</font><br>", ~c"</font><br />") ++ to_string(comment0)
      end
    ...
    print(
      ...
      ~c"<td>" ++
        st0 ++
        ~c"~.3fs" ++ st1 ++ ~c"</td><td><font color=\"red\">FAILED</font></td><td>~ts</td></tr>\n",
      [t / 1000, comment]
    )

    formatLoc = :test_server_sup.format_loc(loc)
    print(:minor, ~c"=== Location: ~ts", [formatLoc])
    print(:minor, ~c"=== Reason: timetrap timeout", [])
    :failed
  end
  ...
  defp progress(
    ...
    print(:major, ~c"=result        failed: testcase_aborted, ~tp", [loc])
    ...
    formatLastLoc = :test_server_sup.format_loc(get_last_loc(loc))
    errorReason = :io_lib.format(~c"{testcase_aborted,~ts}", [formatLastLoc])
    ...
    print(
      ...
      ~c"<td>" ++
        st0 ++
        ~c"died" ++ st1 ++ ~c"</td><td><font color=\"red\">FAILED</font></td><td>~ts</td></tr>\n",
      [comment]
    )
    ...
    print(:minor, ~c"~ts", [
      escape_chars(:io_lib.format(~c"=== Reason: {testcase_aborted,~tp}", [reason]))
    ])
  ...
  defp progress(:failed, caseNum, mod, func, grName, :unknown, reason, time, comment0, {st0, st1}) do
    print(:major, ~c"=result        failed: ~tp, ~w", [reason, :unknown_location])
    ...
    errorReason2 =
      cond do
        errorReasonLength > 63 ->
          :string.slice(errorReason1, 0, 60) ++ ~c"..."
    ...
    comment =
      case comment0 do
        ~c"" ->
          ~c"<font color=\"red\">" ++ errorReason2 ++ ~c"</font>"

        _ ->
          ~c"<font color=\"red\">" ++
            errorReason2 ++ xhtml(~c"</font><br>", ~c"</font><br />") ++ to_string(comment0)
      end
    ...
    print(
      ...
      ~c"<td>" ++
        st0 ++
        ~c"~ts" ++ st1 ++ ~c"</td><td><font color=\"red\">FAILED</font></td><td>~ts</td></tr>\n",
      [timeStr, comment]
    )

    print(:minor, ~c"=== Location: ~w", [:unknown])
  ...
  defp progress(:failed, caseNum, mod, func, grName, loc, reason, time, comment0, {st0, st1}) do
    ...
    print(:major, ~c"=result        failed: ~tp, ~tp", [reason, locMaj])
    ...
    comment =
      case comment0 do
        ...
        _ ->
          xhtml(~c"<br>", ~c"<br />") ++ to_string(comment0)
      end
    ...
    print(
      ...
      ~c"<td>" ++
        ...
        ~c"~ts" ++
        st1 ++
        ~c"</td><td><font color=\"red\">FAILED</font></td><td><font color=\"red\">~ts</font>~ts</td></tr>\n",
  ...
  defp progress(:ok, _CaseNum, mod, func, grName, _Loc, retVal, time, comment0, {st0, st1}) do
    print(:minor, ~c"successfully completed test case", [])
    ...
    comment =
      case retVal do
        {:comment, retComment} ->
          ...
          print(:major, ~c"=result        ok: ~ts", [string])
          ~c"<td>" ++ htmlCmt ++ ~c"</td>"

        _ ->
          print(:major, ~c"=result        ok", [])
          ...
          case comment0 do
            ...
            _ ->
              ~c"<td>" ++ to_string(comment0) ++ ~c"</td>"
          end
      end

    print(:major, ~c"=elapsed       ~p", [time])

    print(
      :html,
      ~c"<td>" ++
        st0 ++ ~c"~ts" ++ st1 ++ ~c"</td><td><font color=\"green\">Ok</font></td>~ts</tr>\n",
      [timeStr, comment]
    )

    print(:minor, ~c"~ts", [escape_chars(:io_lib.format(~c"=== Returned value: ~tp", [retVal]))])
    :ok
  end
  ...
  def escape_chars(term)
    ...
    esc_chars_in_list(:io_lib.format(~c"~tp", [term]))
  end

  def escape_chars(list = [term | _])
      when not is_list(term) and
             not is_integer(term) do
    esc_chars_in_list(:io_lib.format(~c"~tp", [list]))
  end
  ...
  defp get_info_str(mod, func, 0, _Cases) do
    :io_lib.format(~c"~tw", [{mod, func}])
  end

  defp get_info_str(_Mod, _Func, caseNum, :unknown) do
    ~c"test case " ++ :erlang.integer_to_list(caseNum)
  end

  defp get_info_str(_Mod, _Func, caseNum, cases) do
    ~c"test case " ++
      :erlang.integer_to_list(caseNum) ++ ~c" of " ++ :erlang.integer_to_list(cases)
  end
  ...
  def to_string(term) when is_list(term) do
    case (try do
      ...
      {:EXIT, _} ->
        :lists.flatten(:io_lib.format(~c"~tp", [term]))
  ...
  defp reason_to_string({:failed, {_, failFunc, :bad_return}}) do
    :erlang.atom_to_list(failFunc) ++ ~c" bad return value"
  end

  defp reason_to_string({:failed, {_, failFunc, {:timetrap_timeout, _}}}) do
    :erlang.atom_to_list(failFunc) ++ ~c" timed out"
  end
  ...
  defp reason_to_string({:failed, {_, failFunc, _}}) do
    :erlang.atom_to_list(failFunc) ++ ~c" failed"
  end
  ...
  defp get_font_style(normalCase, mode) do
    ...
    {{~c"<font color=" ++ col ++ ~c">", ~c"</font>"},
     {~c"<font color=" ++ col ++ ~c">" ++ st0, st1 ++ ~c"</font>"}}
  end

  defp get_font_style1(:parallel) do
    {~c"\"darkslategray\"", ~c"<i>", ~c"</i>"}
  end
  ...
  defp do_format_exception(reason = {error, stack}) do
    ...
    pF = fn term, i ->
      :io_lib.format(
        ~c"~." ++ :erlang.integer_to_list(i) ++ ~c"tp",
        [term]
      )
    end
    ...
    case (try do
      ...
      formatted ->
        formatted1 =
          :re.replace(formatted, ~c"exception error: ", ~c"", [{:return, :list}, :unicode])

        {~c"~ts", :lists.flatten(formatted1)}
    end
  end
  ...
  def print(detail, [~c"$tc_html", format], args, printer) do
    msg = :io_lib.format(format, args)
    print_or_buffer(detail, [~c"$tc_html", msg], printer)
  end
  ...
  defp print_who(host, user) do
    ...
    print(:html, ~c"Run~ts on ~ts", [userStr, host])
  end
  ...
  def format(detail, format, args) do
    str =
      case (try do
        ...
        {:EXIT, _} ->
          :io_lib.format(~c"illegal format; ~tp with args ~tp.\n", [format, args])
  ...
  defp timestamp_filename_get(leader) do
    timestamp_get_internal(leader, ~c"~ts~w-~2.2.0w-~2.2.0w_~2.2.0w.~2.2.0w.~2.2.0w")
  end

  defp timestamp_get(leader) do
    timestamp_get_internal(leader, ~c"~ts~w-~2.2.0w-~2.2.0w ~2.2.0w:~2.2.0w:~2.2.0w")
  end
  ...
  defp collect_cases({:dir, subDir}, st, mode) do
    collect_files(subDir, ~c"*_SUITE", st, mode)
  end

  defp collect_cases({:dir, subDir, pattern}, st, mode) do
    collect_files(subDir, pattern ++ ~c"*", st, mode)
  end
  ...
  defp collect_files(dir, pattern, st, mode) do
    ...
    wc = :filename.join([dir1, pattern ++ ~c"{.erl," ++ :code.objfile_extension() ++ ~c"}"])
    ...
    case (try do
      ...
      {:EXIT, reason} ->
        :io.format(~c"Could not collect files: ~tp~n", [reason])
  ...
  defp check_deny_req({req, val}, denyList) do
    case :lists.keysearch(req, 1, denyList) do
      {:value, {_Req, denyVal}} when val >= denyVal ->
        {:denied, :io_lib.format(~c"Requirement ~tp=~tp", [req, val])}
  ...
  defp check_deny_req(req, denyList) do
    case :lists.member(req, denyList) do
      true ->
        {:denied, :io_lib.format(~c"Requirement ~tp", [req])}
  ...
  def start_node(name, type, options) do
    ...
    format(:minor, ~c"Attempt to start ~w node ~tp with options ~tp", [type, name, options])
    ...
    case controller_call(
      ...
      {{:ok, nodename}, host, cmd, info, warning} ->
        format(:minor, ~c"Successfully started node ~w on ~tp with command: ~ts", [
          nodename,
          host,
          cmd
        ])

        format(:major, ~c"=node_start    ~w", [nodename])
      ...
      {:fail, {ret, host, cmd}} ->
        format(:minor, ~c"Failed to start node ~tp on ~tp with command: ~ts~nReason: ~tp", [
          name,
          host,
          cmd,
          ret
      ...
      {ret, :undefined, :undefined} ->
        format(:minor, ~c"Failed to start node ~tp: ~tp", [name, ret])
        ret
  ...
  def i() do
    hformat(~c"Pid", ~c"Initial Call", ~c"Current Function", ~c"Reducts", ~c"Msgs")
    line = :lists.duplicate(27, ~c"-")
  ...
  defp ts_pid(x, y, z)
    ...
    :erlang.list_to_pid(
      ~c"<" ++
        ...
        ~c"." ++ :erlang.integer_to_list(y) ++ ~c"." ++ :erlang.integer_to_list(z) ++ ~c">"
    )
  end
    ...
    case pinfo(pid) do
      ...
      info ->
        ...
        pformat(
          ...
          :io_lib.format(~c"~tw", [call]),
          :io_lib.format(~c"~tw", [curr]),
          reds,
          lM
        )
  ...
  defp display_info([], r, m) do
    line = :lists.duplicate(27, ~c"-")
    hformat(line, line, line, line, line)
    pformat(~c"Total", ~c"", ~c"", r, m)
  end

  defp hformat(a1, a2, a3, a4, a5) do
    :io.format(~c"~-10s ~-27s ~-27s ~8s ~4s~n", [a1, a2, a3, a4, a5])
  end

  defp pformat(a1, a2, a3, a4, a5) do
    :io.format(~c"~-10s ~-27s ~-27s ~8w ~4w~n", [a1, a2, a3, a4, a5])
  end
    ...
    case :file.consult(coverFile) do
      {:ok, list} ->
        case check_cover_file(list, [], [], []) do
          ...
          :error ->
            :io.fwrite(~c"Faulty format of CoverFile ~tp\n", [coverFile])
            {[], [], []}
        end

      {:error, reason} ->
        :io.fwrite(~c"Can't read CoverFile ~ts\nReason: ~tp\n", [coverFile, reason])
        {[], [], []}
    end
  end
  ...
  def cover_analyse(coverInfo, testDir) do
    ...
    :io.fwrite(coverLog, ~c"<h1>Coverage for application '~w'</h1>\n", [app])

    :io.fwrite(coverLog, ~c"<p><a href=\"~ts\">Coverdata collected over all tests</a></p>", [
      ~c"cross_cover.html"
    ])

    :io.fwrite(coverLog, ~c"<p>CoverFile: <code>~tp</code>\n", [coverFile])
    :ok = write_cross_cover_info(testDir, cross)

    case length(:cover.imported_modules()) do
      imps when imps > 0 ->
        :io.fwrite(coverLog, ~c"<p>Analysis includes data from ~w imported module(s).\n", [imps])

      _ ->
        :ok
    end

    :io.fwrite(coverLog, ~c"<p>Excluded module(s): <code>~tp</code>\n", [excluded])
    ...
    :ok =
      write_binary_file(
        :filename.join(testDir, ~c"cover.log"),
    ...
    case :lists.filter(
      ...
      bad ->
        :io.fwrite(coverLog, ~c"<p>Analysis failed for ~w module(s): <code>~w</code>\n", [
          length(bad),
    ...
    :ok =
      write_binary_file(
        :filename.join(testDir, ~c"total_cover.log"),
        ...
        :details ->
          fn dir, m ->
            outFile =
              :filename.join(
                dir,
                :erlang.atom_to_list(m) ++ ~c".CROSS_COVER.html"
              )
  ...
  defp write_cross_cover_info(dir, cross) do
    write_binary_file(
      :filename.join(dir, ~c"cross_cover.info"),
  ...
  defp write_cross_cover_logs([{tag, coverage} | t], tagDirMods) do
    case :lists.keyfind(tag, 1, tagDirMods) do
      {_, dir, mods} when mods !== [] ->
        :ok =
          write_binary_file(
            :filename.join(dir, ~c"cross_cover.log"),
            :erlang.term_to_binary(coverage)
          )

        coverLogName = :filename.join(dir, ~c"cross_cover.html")
        {:ok, coverLog} = open_html_file(coverLogName)
        write_coverlog_header(coverLog)
        :io.fwrite(coverLog, ~c"<h1>Coverage results for '~w' from all tests</h1>\n", [tag])
        write_cover_result_table(coverLog, coverage)
        :io.fwrite(~c"Written file ~tp\n", [coverLogName])
  ...
  defp get_all_cross_info([{_Tag, dir} | rest], acc) do
    case :file.read_file(:filename.join(dir, ~c"cross_cover.info")) do
      {:ok, bin} ->
  ...
  defp get_coverdata_files(tagDirMods) do
    :lists.flatmap(
      fn {_, latestDir, _} ->
        :filelib.wildcard(:filename.join(latestDir, ~c"all.coverdata"))
      end,
      tagDirMods
    )
  end
  ...
  defp write_coverlog_header(coverLog) do
    case (try do
            :io.put_chars(coverLog, html_header(~c"Coverage results"))
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
            e -> e
      ...
      {:EXIT, reason} ->
        ...
        :io.format(coverLog, ~c"<html><body>\n", [])
  ...
  defp format_analyse(m, cov, notCov, :undefined) do
    :io_lib.fwrite(
      ~c"<tr><td>~w</td><td align=right>~w %</td><td align=right>~w</td><td align=right>~w</td></tr>\n",
  ...
  defp format_analyse(m, cov, notCov, {:file, file}) do
    :io_lib.fwrite(
      ~c"<tr><td><a href=\"~ts\">~w</a></td><td align=right>~w %</td><td align=right>~w</td><td align=right>~w</td></tr>\n",
      [
  ...
  defp format_analyse(m, cov, notCov, {:lines, lines}) do
    coverOutName = :erlang.atom_to_list(m) ++ ~c".COVER.html"
  ...
  defp write_not_covered(coverOut, m, lines) do
    :io.put_chars(
      coverOut,
      html_header(~c"Coverage results for " ++ :erlang.atom_to_list(m))
    )

    :io.fwrite(
      coverOut,
      ~c"The following lines in module ~w are not covered:\n<table border=3 cellpadding=5>\n<th>Line Number</th>\n",
      [m]
    )
    ...
    :lists.foreach(
      fn
        {{_M, line}, {0, 1}} ->
          :io.fwrite(coverOut, ~c"<tr><td>~w</td></tr>\n", [line])
  ...
  defp write_default_coverlog(testDir) do
    ...
    write_coverlog_header(coverLog)
    :io.put_chars(coverLog, ~c"Cover tool is not used\n</body></html>\n")
  ...
  defp write_default_cross_coverlog(testDir) do
    ...
    :io.put_chars(crossCoverLog, [
      ~c"No cross cover modules exist for this application,",
      xhtml(~c"<br>", ~c"<br />"),
      ~c"or cross cover analysis is not completed.\n</body></html>\n"
    ])
  ...
  defp write_cover_result_table(coverLog, coverage) do
    ...
    :io.fwrite(
      coverLog,
      ~c"<tr><th align=left>Total</th><th align=right>~w %</th><th align=right>~w</th><th align=right>~w</th></tr>\n</table>\n</body>\n</html>\n",
  ...
  defp report_stop_repeat_testcase(reason, repVal) do
    print(:minor, ~c"~n*** Stopping test case repeat operation: ~w", [reason])
    print(1, ~c"Stopping test case repeat operation: ~w", [repVal])
  end

  defp report_repeat_testcase(m, :forever) do
    print(:minor, ~c"~n=== Repeated test case: ~w of infinity", [m])
  end

  defp report_repeat_testcase(m, n) do
    print(:minor, ~c"~n=== Repeated test case: ~w of ~w", [m, n])
  end
end
