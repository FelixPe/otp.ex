defmodule :m_test_server_ctrl do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_state, :state, jobs: [],
                                 levels: {1, 19, 10}, reject_io_reqs: false,
                                 multiply_timetraps: 1, scale_timetraps: true,
                                 create_priv_dir: :auto_per_run, finish: false,
                                 target_info: :undefined, cover: false,
                                 wait_for_node: [],
  ...
  def add_case(mod, case__) when (is_atom(mod) and
                              is_atom(case__)) do
    add_job(:erlang.atom_to_list(mod), {mod, case__})
  end

  def add_case(name, mod, case__) when (is_atom(mod) and
                                    is_atom(case__)) do
    add_job(name, {mod, case__})
  end

  def add_cases(mod, cases) when (is_atom(mod) and
                             is_list(cases)) do
    add_job(:erlang.atom_to_list(mod), {mod, cases})
  end

  def add_cases(name, mod, cases) when (is_atom(mod) and
                                   is_list(cases)) do
  ...
  def add_case_with_skip(mod, case__, skip) when (is_atom(mod) and
                                    is_atom(case__)) do
    add_job(:erlang.atom_to_list(mod), {mod, case__}, skip)
  end

  def add_case_with_skip(name, mod, case__, skip) when (is_atom(mod) and
                                          is_atom(case__)) do
    add_job(name, {mod, case__}, skip)
  end

  def add_cases_with_skip(mod, cases, skip) when (is_atom(mod) and
                                   is_list(cases)) do
    add_job(:erlang.atom_to_list(mod), {mod, cases}, skip)
  end

  def add_cases_with_skip(name, mod, cases, skip) when (is_atom(mod) and
                                         is_list(cases)) do
  ...
  defp parse_cmd_line([:SPEC, spec | cmds], specList, names, param,
            cov, tCCB) do
    case (:file.consult(spec)) do
      {:ok, termList} ->
        name = :filename.rootname(spec)
        parse_cmd_line(cmds, termList ++ specList,
                         [name | names], param, cov, tCCB)
      {:error, reason} ->
  ...
  defp parse_cmd_line([:NAME, name | cmds], specList, names, param,
            cov, tCCB) do
    parse_cmd_line(cmds, specList,
                     [{:name, :erlang.atom_to_list(name)} | names], param,
                     cov, tCCB)
  end

  defp parse_cmd_line([:SKIPMOD, mod | cmds], specList, names, param,
            cov, tCCB) do
    parse_cmd_line(cmds, [{:skip, {mod, 'by command line'}} | specList],
                     names, param, cov, tCCB)
  end

  defp parse_cmd_line([:SKIPCASE, mod, case__ | cmds], specList,
            names, param, cov, tCCB) do
    parse_cmd_line(cmds,
                     [{:skip, {mod, case__, 'by command line'}} | specList], names, param,
                     cov, tCCB)
  end

  defp parse_cmd_line([:DIR, dir | cmds], specList, names, param, cov,
            tCCB) do
    name = :filename.basename(dir)
    parse_cmd_line(cmds,
                     [{:topcase, {:dir, name}} | specList], [name | names],
                     param, cov, tCCB)
  end

  defp parse_cmd_line([:MODULE, mod | cmds], specList, names, param,
            cov, tCCB) do
    parse_cmd_line(cmds,
                     [{:topcase, {mod, :all}} | specList],
                     [:erlang.atom_to_list(mod) | names], param, cov, tCCB)
  end

  defp parse_cmd_line([:CASE, mod, case__ | cmds], specList, names,
            param, cov, tCCB) do
    parse_cmd_line(cmds,
  ...
  defp parse_cmd_line([:COVER, app, cF, analyse | cmds], specList,
            names, param, _Cov, tCCB) do
    parse_cmd_line(cmds, specList, names, param,
                     {{app, cF}, analyse}, tCCB)
  end

  defp parse_cmd_line([:TESTCASE_CALLBACK, mod, func | cmds],
            specList, names, param, cov, _) do
    parse_cmd_line(cmds, specList, names, param, cov,
                     {mod, func})
  end

  defp parse_cmd_line([obj | _Cmds], _SpecList, _Names, _Param, __Cov,
            _TCCB) do
  ...
  defp parse_cmd_line([], specList, names, param, cov, tCCB) do
    nameList = :lists.reverse(names, ['suite'])
    name = (case (:lists.keysearch(:name, 1, nameList)) do
              {:value, {:name, n}} ->
                n
              false ->
                hd(nameList)
  ...
  def start() do
    case (:gen_server.start({:local, :test_server_ctrl},
                              :test_server_ctrl, [], [])) do
  ...
  defp start_link() do
    case (:gen_server.start_link({:local,
                                    :test_server_ctrl},
                                   :test_server_ctrl, [], [])) do
      {:error, {:already_started, pid}} ->
        {:ok, pid}
  ...
  def run_test(commandLine) do
    ...
    {specList, name, param, cov,
       tCCB} = parse_cmd_line(commandLine)
  ...
  def node_started(node) do
    :gen_server.cast(:test_server_ctrl,
                       {:node_started, node})
  end
  ...
  def cover(app, coverFile, analyse) do
    ...
    coverInfo = r_cover(app: app, file: coverFile, excl: excl,
                    incl: incl, cross: cross, level: analyse)
  ...
  def cover_compile(app, file, excl, incl, cross, analyse, stop) do
    cover_compile(r_cover(app: app, file: file, excl: excl,
                      incl: incl, cross: cross, level: analyse, stop: stop))
  end
  ...
  def init([]) do
    ...
    case (:init.get_argument(:test_server_format_exception)) do
      {:ok, [[tSFE]]} ->
        :application.set_env(:test_server, :format_exception,
                               :erlang.list_to_atom(tSFE))
      _ ->
        :ok
    end
    ...
    tI = r_target_info(tI0, host: targetHost,  naming: naming(), 
                  master: targetHost)
    _ = :ets.new(:slave_tab,
  ...
  def handle_call({:add_job, dir, name, topCase, skip}, _From,
           state) do
    logDir = dir ++ '.logs'
    extraTools = (case (r_state(state, :cover)) do
                    false ->
                      []
                    coverInfo ->
                      [{:cover, coverInfo}]
                  end)
    extraTools1 = (case (r_state(state, :random_seed)) do
                     :undefined ->
                       extraTools
                     seed ->
    ...
    case (:lists.keysearch(name, 1, r_state(state, :jobs))) do
      false ->
        case (topCase) do
          {:spec, specName} ->
            pid = spawn_tester(:test_server_ctrl, :do_spec,
                                 [specName, {r_state(state, :multiply_timetraps),
                                               r_state(state, :scale_timetraps)}],
                                 logDir, name, r_state(state, :levels),
                                 r_state(state, :reject_io_reqs),
                                 r_state(state, :create_priv_dir),
                                 r_state(state, :testcase_callback), extraTools1)
            newJobs = [{name, pid} | r_state(state, :jobs)]
            {:reply, :ok, r_state(state, jobs: newJobs)}
          {:command_line, specList} ->
            pid = spawn_tester(:test_server_ctrl, :do_spec_list,
                                 [specList, {r_state(state, :multiply_timetraps),
                                               r_state(state, :scale_timetraps)}],
                                 logDir, name, r_state(state, :levels),
                                 r_state(state, :reject_io_reqs),
                                 r_state(state, :create_priv_dir),
          ...
          ^topCase ->
            case (r_state(state, :get_totals)) do
              ...
              _ ->
                cfg = make_config([])
                pid = spawn_tester(:test_server_ctrl, :do_test_cases,
                                     [topCase, skip, cfg,
                                                         {r_state(state, :multiply_timetraps),
                                                            r_state(state, :scale_timetraps)}],
                                     logDir, name, r_state(state, :levels),
                                     r_state(state, :reject_io_reqs),
                                     r_state(state, :create_priv_dir),
  ...
  def handle_call({:abort_current_testcase, reason}, _From,
           state) do
  ...
  def handle_call({:idle_notify, fun}, {cli, _Ref}, state) do
    ...
    {:reply, {:ok, self()},
       r_state(state, idle_notify: [{cli, fun} | subscribed])}
  end

  def handle_call({:start_get_totals, fun}, {cli, _Ref}, state) do
    {:reply, {:ok, self()},
       r_state(state, get_totals: {cli, fun})}
  end
  ...
  def handle_call({:set_levels, show, major, minor}, _From,
           state) do
  ...
  def handle_call({:start_node, name, type, options}, from,
           state) do
    :test_server_node.start_node(name, type, options, from,
                                   r_state(state, :target_info))
    {:noreply, state}
  end

  def handle_call({:wait_for_node, node}, from, state) do
    newWaitList = (case (:ets.lookup(:slave_tab, node)) do
                     [] ->
  ...
  def handle_call({:is_release_available, release}, _From,
           state) do
    r = :test_server_node.is_release_available(release)
    {:reply, r, state}
  end

  def handle_call({:find_release, release}, from,
           state = r_state(old_releases: oldReleases)) do
    case (:maps.find(release, oldReleases)) do
      :error ->
        r = (case (:test_server_node.find_release(release)) do
               :none ->
                 :not_available
               pathToRelease ->
                 pathToRelease
        ...
        handle_call({:find_release, release}, from,
                      r_state(state, old_releases: Map.put(oldReleases, release,
                                                                    r)))
      {:ok, r} ->
        {:reply, r, state}
    end
  ...
  def handle_cast({:node_started, node}, state) do
    newWaitList = (case (:lists.keysearch(node, 1,
                                            r_state(state, :wait_for_node))) do
                     {:value, {^node, from}} ->
                       :gen_server.reply(from, :ok)
  ...
  defp spawn_tester(mod, func, args, dir, name, levels,
            rejectIoReqs, createPrivDir, tCCallback, extraTools) do
    spawn_link(fn () ->
                    init_tester(mod, func, args, dir, name, levels,
                                  rejectIoReqs, createPrivDir, tCCallback,
                                  extraTools)
               end)
  end

  defp init_tester(mod, func, args, dir, name,
            {_, _, minLev} = levels, rejectIoReqs, createPrivDir,
            tCCallback, extraTools) do
    :erlang.process_flag(:trap_exit, true)
    _ = :test_server_io.start_link()
    :erlang.put(:app, :common_test)
    :erlang.put(:test_server_name, name)
    :erlang.put(:test_server_dir, dir)
    :erlang.put(:test_server_total_time, 0.0)
    ...
    :erlang.put(:test_server_random_seed,
    ...
    case (:os.getenv('TEST_SERVER_FRAMEWORK')) do
      ...
      fW ->
        :erlang.put(:test_server_framework_name,
    ...
    fWLogDir = (case (:test_server_sup.framework_call(:get_log_dir,
                                                        [], [])) do
                  {:ok, fwDir} ->
                    fwDir
                  _ ->
    ...
    :test_server_io.set_gl_props([{:levels, levels},
                                      {:auto_nl,
                                         not :lists.member(:no_nl, logOpts)},
                                          {:reject_io_reqs, rejectIoReqs}])
    :erlang.group_leader(:test_server_io.get_gl(true),
                           self())
    ...
    successStr = (case (:erlang.get(:test_server_failed)) do
                    0 ->
                      'Ok'
                    _ ->
                      'FAILED'
    ...
    {skippedN,
       skipStr} = (case (:erlang.get(:test_server_skipped)) do
                     {0, 0} ->
                       {0, ''}
                     {uSkipped, aSkipped} ->
    ...
    totalTestTime = :erlang.get(:test_server_total_time)
    print(:html, '\n</tbody>\n<tfoot>\n<tr><td></td><td><b>TOTAL</b></td><td></td><td></td><td></td><td>~.3fs<br></td><td><b>~ts</b></td><td>~w Ok, ~w Failed~ts of ~w<br>Elapsed Time: ~.3fs</td></tr>\n</tfoot>\n',
            [totalTestTime, successStr, okN, failedN, skipStr,
                                                          okN + failedN + skippedN,
                                                              elapsedTimeSeconds])
    :test_server_io.stop([:major, :html, :unexpected_io])
    {unexpectedIoName,
       unexpectedIoFooter} = :erlang.get(:test_server_unexpected_footer)
    {:ok, unexpectedIoFd} = open_html_file(unexpectedIoName,
                                             [:append])
  ...
  defp ts_tc(m, f, a) do
    ...
    elapsed = :erlang.convert_time_unit(after__ - before,
  ...
  def do_spec_list(termList0, timetrapSpec) do
    nodes = []
    termList = (case (:lists.keysearch(:hosts, 1,
                                         termList0)) do
                  {:value, {:hosts, hosts0}} ->
                    hosts = :lists.map(fn h ->
                                            cast_to_list(h)
                                       end,
                                         hosts0)
    ...
    {topCases, skipList, config} = do_spec_terms(termList,
                                                   [], [], defaultConfig)
  ...
  defp do_spec_terms([{:topcase, topCase} | terms], topCases,
            skipList, config) do
    do_spec_terms(terms, [topCase | topCases], skipList,
                    config)
  end

  defp do_spec_terms([{:skip, skip} | terms], topCases, skipList,
            config) do
    do_spec_terms(terms, topCases, [skip | skipList],
                    config)
  end

  defp do_spec_terms([{:nodes, nodes} | terms], topCases, skipList,
            config) do
    do_spec_terms(terms, topCases, skipList,
                    update_config(config, {:nodes, nodes}))
  end

  defp do_spec_terms([{:diskless, how} | terms], topCases, skipList,
            config) do
    do_spec_terms(terms, topCases, skipList,
                    update_config(config, {:diskless, how}))
  end

  defp do_spec_terms([{:config, moreConfig} | terms], topCases,
            skipList, config) do
    do_spec_terms(terms, topCases, skipList,
                    config ++ moreConfig)
  end

  defp do_spec_terms([{:default_timeout, tmo} | terms], topCases,
            skipList, config) do
    do_spec_terms(terms, topCases, skipList,
                    update_config(config, {:default_timeout, tmo}))
  end

  defp do_spec_terms([{:require_nodenames, numNames} | terms],
            topCases, skipList, config) do
  ...
  defp generate_nodenames(num) do
    hosts = (case (controller_call(:get_hosts)) do
               [] ->
  ...
  defp generate_nodenames2(n, hosts, acc) do
    ...
    name = :erlang.list_to_atom(temp_nodename('nod',
                                                []) ++ '@' ++ host)
  ...
  defp remove_conf([{:conf, _Ref, props, _MF} | cases], noConf,
            repeats) do
  ...
  defp remove_conf([{:make, _Ref, _MF} | cases], noConf,
            repeats) do
    remove_conf(cases, noConf, repeats)
  end

  defp remove_conf([{:skip_case, {{_M, :all}, _Cmt}, _Mode} |
               cases],
            noConf, repeats) do
    remove_conf(cases, noConf, repeats)
  end

  defp remove_conf([{:skip_case, {type, _Ref, _MF, _Cmt}} | cases],
            noConf, repeats)
      when type == :conf or type == :make do
    remove_conf(cases, noConf, repeats)
  end

  defp remove_conf([{:skip_case, {type, _Ref, _MF, _Cmt}, _Mode} |
               cases],
            noConf, repeats)
      when type == :conf or type == :make do
    remove_conf(cases, noConf, repeats)
  ...
  defp remove_conf([c = {mod, :error_in_suite, _} | cases], noConf,
            repeats) do
  ...
  defp remove_conf([c = {:repeat, _, _} | cases], noConf,
            _Repeats) do
  ...
  defp get_suites([{:skip_case, {{mod, _F}, _Cmt}, _Mode} |
               tests],
            mods)
      when is_atom(mod) do
  ...
  def do_test_cases(topCases, skipCases, config, multiplyTimetrap)
    ...
    do_test_cases(topCases, skipCases, config,
                    {multiplyTimetrap, true})
  end
  ...
  def do_test_cases(topCases, skipCases, config, timetrapData)
    ...
    case (collect_all_cases(topCases, skipCases)) do
      {:error, why} ->
        ...
        exit(:test_suites_done)
      testSpec0 ->
        n = (case (remove_conf(testSpec0)) do
               {:repeats, _} ->
                 :unknown
               tS ->
                 length(tS)
        ...
        testSpec = add_init_and_end_per_suite(testSpec0,
                                                :undefined, :undefined, fwMod)
        tI = get_target_info()
        print(1, 'Starting test~ts', [print_if_known(n, {', ~w test cases', [n]}, {' (with repeated test cases)', []})])
        ...
        :test_server_sup.framework_call(:report,
                                          [:tests_start, {test, n}])
        {header,
           footer} = (case (:test_server_sup.framework_call(:get_html_wrapper,
                                                              [testDescr, true,
                                                                              testDir,
                                                                                  {[],
                                                                                     [2,
        ...
        print(:html, xhtml('</p>', '</h4>'))
        print(:html, xhtml('\n<p><b>Host info:</b><br>\n', '\n<p><b>Host info:</b><br />\n'))
        print_who(:test_server_sup.hoststr(),
        ...
        cond do
          ...
          true ->
            case (:test_server_sup.framework_call(:target_info,
                                                    [])) do
              targetInfo when (is_list(targetInfo) and
                                 length(targetInfo) > 0)
                              ->
                print(:html, xhtml('\n<p><b>Target info:</b><br>\n', '\n<p><b>Target info:</b><br />\n'))
                print(:html, '~ts</p>\n', [targetInfo])
        ...
        coverLog = (case (:erlang.get(:test_server_cover_log_dir)) do
                      :undefined ->
                        'cover.html'
                      absLogDir ->
        ...
        case (controller_call(:get_hosts)) do
          ...
          hosts ->
            str = :lists.flatten(:lists.map(fn x ->
                                                 [x, ' ']
                                            end,
                                              hosts))
  ...
  def do_test_cases(topCase, skipCases, config, timetrapSpec) do
    do_test_cases([topCase], skipCases, config,
                    timetrapSpec)
  end
  ...
  defp start_log_file() do
    ...
    testDir1 = (case (:file.make_dir(testDir)) do
                  :ok ->
                    testDir
                  {:error, :eexist} ->
                    :timer.sleep(1000)
                    testDirX = timestamp_filename_get(:filename.join(dir,
                                                                       'run.'))
    ...
    {unexpHeader,
       unexpFooter} = (case (:test_server_sup.framework_call(:get_html_wrapper,
                                                               ['Unexpected I/O log', false,
                                                                       testDir,
                                                                           :undefined],
                                                               '')) do
                         uEmpty when uEmpty == '' or
    ...
    :io.put_chars(unexpected, [unexpHeader, xhtml('<br>\n<h2>Unexpected I/O</h2>', '<br />\n<h3>Unexpected I/O</h3>'), '\n<pre>\n'])
    :erlang.put(:test_server_unexpected_footer,
    ...
    make_html_link(:filename.absname('last_test' ++ '.html'), htmlName,
                     :filename.basename(dir))
    linkName = :filename.join(dir, 'last_link')
    make_html_link(linkName ++ '.html', htmlName,
                     :filename.basename(dir))
    ...
    logInfo = [{:topdir, dir}, {:rundir,
                                  :lists.flatten(testDir1)}]
    :test_server_sup.framework_call(:report,
                                      [:loginfo, logInfo])
    {:ok, testDir1}
  end
  ...
  defp log_file_error(error, dir) do
    exit({:cannot_create_log_dir,
            {error, :lists.flatten(dir)}})
  end
  ...
  defp make_html_link(linkName, target, explanation) do
    ...
    href = (case (:lists.prefix(pwdL, targetL)) do
              true ->
                uri_encode(:filename.join(:lists.nthtail(length(pwdL),
                                                           targetL)))
              false ->
    ...
    h = (cond do
           ...
           true ->
             [html_header(explanation), '<h1>Last test</h1>\n<a href="', href, '">', explanation, '</a>\n</body>\n</html>\n']
         end)
  ...
  defp start_minor_log_file(mod, func, parallelTC) do
    ...
    case (parallelTC or :erlang.element(1,
                                          :file.read_file_info(absName)) == :ok) do
      false ->
  ...
  defp start_minor_log_file1(mod, func, logDir, absName, mFA) do
    ...
    :test_server_gl.set_minor_fd(:erlang.group_leader(), fd,
                                   mFA)
    ...
    case (get_fw_mod(:test_server_ctrl)) do
      ...
      _ ->
        ...
        case ({:filelib.is_file(:filename.join(logDir,
                                                 srcListing)),
                 :lists.member(:no_src,
                                 :erlang.get(:test_server_logopts))}) do
          {true, false} ->
            print(lev, ['$tc_html', info ++ '<a href="~ts#~ts">~w:~tw/~w</a> (click for source code)\n'],
  ...
  defp minor_log_file_name(mod, func, tag) do
    name = downcase(:lists.flatten(:io_lib.format('~w.~tw~s~s',
                                                    [mod, func, tag, '.html'])))
  ...
  defp downcase([uc | rest], result) when (?A <= uc and
                                       uc <= ?Z) do
  ...
  defp html_convert_modules(testSpec, _Config, fwMod) do
    ...
    copy_html_files(:erlang.get(:test_server_dir),
  ...
  defp html_isolate_modules([{:skip_case, {_Case, _Cmt}, _Mode} | cases],
            set, fwMod) do
    html_isolate_modules(cases, set, fwMod)
  end

  defp html_isolate_modules([{:conf, _Ref, props, {fwMod, _Func}} | cases],
            set, fwMod) do
    set1 = (case (:proplists.get_value(:suite, props)) do
              :undefined ->
                set
              mod ->
  ...
  defp html_isolate_modules([{:conf, _Ref, _Props, {mod, _Func}} | cases],
            set, fwMod) do
    html_isolate_modules(cases, :sets.add_element(mod, set),
                           fwMod)
  end

  defp html_isolate_modules([{:skip_case,
              {:conf, _Ref, {fwMod, _Func}, _Cmt}, mode} |
               cases],
            set, fwMod) do
    set1 = (case (:proplists.get_value(:suite,
                                         get_props(mode))) do
              :undefined ->
                set
              mod ->
  ...
  defp html_isolate_modules([{:skip_case, {:conf, _Ref, {mod, _Func}, _Cmt},
              _Props} |
               cases],
            set, fwMod) do
    html_isolate_modules(cases, :sets.add_element(mod, set),
  ...
  defp html_convert_modules([mod | mods]) do
    case (:code.which(mod)) do
      path when is_list(path) ->
        ...
        foundSrcFile = (case (:file.read_file_info(srcFile)) do
                          {:ok, sInfo} ->
                            {srcFile, sInfo}
                          {:error, _} ->
        ...
        case (foundSrcFile) do
          ...
          {srcFile1, srcFileInfo} ->
            ...
            _ = html_possibly_convert(srcFile1, srcFileInfo,
                                        destFile)
            html_convert_modules(mods)
        end
      _Other ->
  ...
  defp html_possibly_convert(src, srcInfo, dest) do
    case (:file.read_file_info(dest)) do
      {:ok, destInfo}
        ...
        :ok
      _ ->
        inclPath = (case (:application.get_env(:test_server,
                                                 :include)) do
                      {:ok, incls} ->
                        incls
                      _ ->
  ...
  defp add_init_and_end_per_suite([{:make, _, _} = case__ | cases], lastMod,
            lastRef, fwMod) do
    [case__ | add_init_and_end_per_suite(cases, lastMod,
                                           lastRef, fwMod)]
  end

  defp add_init_and_end_per_suite([{:skip_case, {{mod, :all}, _}, _} = case__ |
               cases],
            lastMod, lastRef, fwMod)
      when mod !== lastMod do
    {preCases, nextMod,
       nextRef} = do_add_end_per_suite_and_skip(lastMod,
                                                  lastRef, mod, fwMod)
    preCases ++ [case__ | add_init_and_end_per_suite(cases,
                                                       nextMod, nextRef, fwMod)]
  end

  defp add_init_and_end_per_suite([{:skip_case, {{mod, _}, _Cmt},
              _Mode} = case__ |
               cases],
            lastMod, lastRef, fwMod)
      when mod !== lastMod do
    {preCases, nextMod,
       nextRef} = do_add_init_and_end_per_suite(lastMod,
                                                  lastRef, mod, fwMod)
    preCases ++ [case__ | add_init_and_end_per_suite(cases,
                                                       nextMod, nextRef, fwMod)]
  end

  defp add_init_and_end_per_suite([{:skip_case, {:conf, _, {mod, _}, _},
              _} = case__ |
               cases],
            lastMod, lastRef, fwMod)
      when mod !== lastMod do
  ...
  defp add_init_and_end_per_suite([{:skip_case,
              {:conf, _, {mod, _}, _}} = case__ |
               cases],
            lastMod, lastRef, fwMod)
      when mod !== lastMod do
    {preCases, nextMod,
  ...
  defp add_init_and_end_per_suite([{:conf, ref, props, {fwMod, func}} = case__ |
               cases],
            lastMod, lastRef, fwMod) do
      ...
      suite when (suite !== :undefined and suite !== lastMod)
        ...
        {preCases, nextMod,
           nextRef} = do_add_init_and_end_per_suite(lastMod,
                                                      lastRef, suite, fwMod)
        case1 = {:conf, ref,
                   [{:suite, nextMod} | :proplists.delete(:suite, props)],
                   {fwMod, func}}
        preCases ++ [case1 | add_init_and_end_per_suite(cases,
                                                          nextMod, nextRef,
                                                          fwMod)]
      _ ->
        [case__ | add_init_and_end_per_suite(cases, lastMod,
  ...
  defp add_init_and_end_per_suite([{:conf, _, _, {mod, _}} = case__ | cases],
            lastMod, lastRef, fwMod)
  ...
  defp add_init_and_end_per_suite([skipCase | cases], lastMod, lastRef, fwMod)
    ...
    [skipCase | add_init_and_end_per_suite(cases, lastMod,
                                             lastRef, fwMod)]
  end

  defp add_init_and_end_per_suite([{:conf, _, _, _} = case__ | cases], lastMod,
            lastRef, fwMod) do
    [case__ | add_init_and_end_per_suite(cases, lastMod,
                                           lastRef, fwMod)]
  end

  defp add_init_and_end_per_suite([{:repeat, {mod, _}, _} = case__ | cases],
            lastMod, lastRef, fwMod)
      when (mod !== lastMod and mod !== fwMod) do
    {preCases, nextMod,
       nextRef} = do_add_init_and_end_per_suite(lastMod,
  ...
  defp add_init_and_end_per_suite([{:repeat, _, _} = case__ | cases], lastMod,
            lastRef, fwMod) do
    [case__ | add_init_and_end_per_suite(cases, lastMod,
                                           lastRef, fwMod)]
  end

  defp add_init_and_end_per_suite([{mod, _} = case__ | cases], lastMod, lastRef,
            fwMod)
      when (mod !== lastMod and mod !== fwMod) do
    {preCases, nextMod,
       nextRef} = do_add_init_and_end_per_suite(lastMod,
  ...
  defp add_init_and_end_per_suite([{mod, _, _} = case__ | cases], lastMod,
            lastRef, fwMod)
      when (mod !== lastMod and mod !== fwMod) do
    {preCases, nextMod,
       nextRef} = do_add_init_and_end_per_suite(lastMod,
  ...
  defp add_init_and_end_per_suite([], lastMod, lastRef, fwMod) do
    case ({:erlang.function_exported(lastMod,
                                       :end_per_suite, 1),
             :erlang.function_exported(lastMod, :init_per_suite,
                                         1)}) do
      {false, false} ->
        case (:erlang.function_exported(fwMod, :end_per_suite,
                                          1)) do
          true ->
  ...
  defp do_add_init_and_end_per_suite(lastMod, lastRef, mod, fwMod) do
    ...
    {init, nextMod,
       nextRef} = (case ({:erlang.function_exported(mod,
                                                      :init_per_suite, 1),
                            :erlang.function_exported(mod, :end_per_suite,
                                                        1)}) do
                     {false, false} ->
                       case (:erlang.function_exported(fwMod, :init_per_suite,
                                                         1)) do
                         true ->
                           ref = make_ref()
                           {[{:conf, ref, [{:suite, mod}],
                                {fwMod, :init_per_suite}}],
                              mod, ref}
                         false ->
                           {[], mod, :undefined}
                       end
        ...
        case ({:erlang.function_exported(lastMod,
          ...
          {false, false} ->
            case (:erlang.function_exported(fwMod, :end_per_suite,
                                              1)) do
              true ->
                {[{:conf, lastRef, [], {fwMod, :end_per_suite}}], mod,
                   :skipped_suite}
              false ->
                {[{:conf, lastRef, [], {lastMod, :end_per_suite}}], mod,
                   :skipped_suite}
            end
          _ ->
            {[{:conf, lastRef, [], {lastMod, :end_per_suite}}], mod,
  ...
  defp run_test_cases_loop([{skipTag,
              caseData = {type, _Ref, _Case, _Comment}} |
               cases],
            config, timetrapData, mode, status)
      when (skipTag == :auto_skip_case or skipTag == :skip_case) and (type == :conf or type == :make) do
    run_test_cases_loop([{skipTag, caseData, mode} | cases],
                          config, timetrapData, mode, status)
  end

  defp run_test_cases_loop([{skipTag, {type, ref, case__, comment},
              skipMode} |
               cases],
            config, timetrapData, mode, status)
      when (skipTag == :auto_skip_case or skipTag == :skip_case) and (type == :conf or type == :make) do
    ...
    case ({curr_ref(mode), check_props(:parallel, mode)}) do
      {^ref, ^ref} ->
        case (check_props(:parallel, parentMode)) do
          false ->
            ...
            set_io_buffering(:undefined)
            {mod, func} = skip_case(autoOrUser, ref, 0, case__,
                                      comment, false, skipMode)
            confData = {mod, {func, get_name(skipMode)}, comment}
            :test_server_sup.framework_call(:report,
                                              [reportTag, confData])
            run_test_cases_loop(cases, config, timetrapData,
                                  parentMode, delete_status(ref, status))
          _ ->
            _ = wait_for_cases(ref)
            {mod, func} = skip_case(autoOrUser, ref, 0, case__,
                                      comment, true, skipMode)
            confData = {mod, {func, get_name(skipMode)}, comment}
            :test_server_sup.framework_call(:report,
                                              [reportTag, confData])
      ...
      {^ref, false} ->
        ...
        cases1 = (case (get_tc_results(status)) do
      ...
      {^ref, _} ->
        ...
        run_test_cases_loop(cases, config, timetrapData,
                              tl(mode), delete_status(ref, status))
      {_, false} ->
        {mod, func} = skip_case(autoOrUser, ref, 0, case__,
                                  comment, false, skipMode)
        confData = {mod, {func, get_name(skipMode)}, comment}
  ...
  defp run_test_cases_loop([{:auto_skip_case, {case__, comment},
              skipMode} |
               cases],
            config, timetrapData, mode, status) do
    {mod, func} = skip_case(:auto, :undefined,
                              :erlang.get(:test_server_case_num) + 1, case__,
                              comment, is_io_buffered(), skipMode)
    :test_server_sup.framework_call(:report,
                                      [:tc_auto_skip, {mod,
                                                         {func,
                                                            get_name(skipMode)},
  ...
  defp run_test_cases_loop([{:skip_case, {{mod, :all} = case__, comment},
              skipMode} |
               cases],
            config, timetrapData, mode, status) do
    _ = skip_case(:user, :undefined, 0, case__, comment,
                    false, skipMode)
    :test_server_sup.framework_call(:report,
                                      [:tc_user_skip, {mod,
                                                         {:all,
  ...
  defp run_test_cases_loop([{:skip_case, {case__, comment}, skipMode} |
               cases],
            config, timetrapData, mode, status) do
    {mod, func} = skip_case(:user, :undefined,
                              :erlang.get(:test_server_case_num) + 1, case__,
                              comment, is_io_buffered(), skipMode)
    :test_server_sup.framework_call(:report,
                                      [:tc_user_skip, {mod,
  ...
  defp run_test_cases_loop([{:conf, ref, props, {mod, func}} |
               _Cases] = cs0,
            config, timetrapData, mode0, status) do
    currIOHandler = :erlang.get(:test_server_common_io_handler)
    {startConf, mode, iOHandler, confTime,
       status1} = (case ({curr_ref(mode0),
                            check_props(:parallel, mode0)}) do
                     {^ref, ^ref} ->
                       case (check_props(:parallel, tl(mode0))) do
                         false ->
                           ...
                           elapsed = :timer.now_diff(after__, before) / 1000000
                         ...
                         _ ->
                           ...
                           elapsed = :timer.now_diff(:os.timestamp(),
                                                       conf_start(ref,
                                                                    mode0)) / 1000000
                           case (currIOHandler) do
                             {^ref, _} ->
                     ...
                     {_, false} ->
                       case (:lists.member(:parallel, props)) do
                         true ->
                           :erlang.put(:test_server_parallel_start_time,
                                         :os.timestamp())
                           :erlang.put(:test_server_queued_io, [])
                         false ->
                           :ok
                       end
                       {true, [conf(ref, props) | mode0], :undefined, 0,
                          status}
                     {_, _Ref0} ->
                       queue_test_case_io(ref, self(), 0, mod, func)
                       iOHandler1 = (cond do
                       ...
                       {true, [conf(ref, props) | mode0], iOHandler1, 0,
                          status}
                   end)
    {[_Conf | cases1] = cs1, shuffle} = (cond do
                                           startConf ->
                                             case (get_shuffle(props)) do
                                               ...
                                               {_, :repeated} ->
                                                 ...
                                                 {shuffle_cases(ref, cs0,
                                                                  currSeed),
                                                    {:shuffle, currSeed}}
                                               {_, seed} ->
                                                 useSeed = (case (:os.getenv('TS_RANDOM_SEED')) do
                                                              undef
                                                                  when undef == false or
                                                                         undef == 'undefined'
                                                                       ->
                                                              ...
                                                              numStr ->
                                                                :erlang.list_to_tuple(for nS <- :string.lexemes(numStr,
                                                                                                                  [?\s,
                                                                                                                       ?:,
                                                                                                                           ?,]) do
    ...
    {repeating, status2, cases, reportRepeatStop} = (cond do
                                                       startConf ->
                                                         case (get_repeat(props)) do
                                                           :undefined ->
                                                             {false,
                                                       ...
                                                       not startConf ->
                                                         ...
                                                         case (repVal) do
                                                           ...
                                                           {:repeat_until_all_ok,
                                                              _} ->
                                                             {restCs,
                                                                fun} = (case (get_tc_results(status1)) do
                                                                          {_, _,
                                                                             []} ->
                                                                            {cases1,
                                                                               reportStop}
    ...
    reportAbortRepeat = fn what when repeating ->
    ...
    case (:erlang.get(:test_server_create_priv_dir)) do
      :auto_per_run ->
        tSDirs = [{:priv_dir,
                     :erlang.get(:test_server_priv_dir)},
    ...
    actualCfg = (cond do
                   ...
                   true ->
                     groupPath = :lists.flatmap(fn {_Ref, [], _T} ->
                                                     []
                                                   {_Ref, grProps, _T} ->
                                                     [grProps]
                                                end,
                     ...
                     update_config(hd(config),
                                     tSDirs ++ [{:tc_group_path, groupPath} |
                                                    cfgProps])
                 end)
    currMode = curr_mode(ref, mode0, mode)
    confCaseResult = run_test_case(ref, 0, mod, func,
                                     [actualCfg], :skip_init, timetrapData,
                                     currMode)
    case (confCaseResult) do
      {_, newCfg, _} when (func == :init_per_suite and
                             is_list(newCfg))
                          ->
        case (:lists.filter(fn {_, _} ->
                                 false
                               _ ->
                                 true
                            end,
          ...
          bad ->
            ...
            cases2 = skip_cases_upto(ref, cases, reason, :conf,
                                       currMode, :auto_skip_case)
            set_io_buffering(iOHandler)
            stop_minor_log_file()
      ...
      {_, {skip, reason}, _}
        ...
        set_io_buffering(iOHandler)
        stop_minor_log_file()
        run_test_cases_loop(skip_cases_upto(ref, cases, reason,
                                              :conf, currMode, :skip_case),
      ...
      {_, _Other, _} when startConf ->
        ...
        reportRepeatStop.()
        stop_minor_log_file()
        run_test_cases_loop(cases, [hd(config) | config],
                              timetrapData, mode, status2)
      {_, _EndConfRetVal, opts} ->
        grName = get_name(mode0, func)
        {cases2,
           status3} = (case (:lists.keysearch(:return_group_result,
                                                1, opts)) do
                         {:value, {_, :failed}} ->
                           case ({curr_ref(mode),
                                    check_prop(:sequence, mode)}) do
                             {parentRef, parentRef} ->
                               reason = {:group_result, grName, :failed}
                               {skip_cases_upto(parentRef, cases, reason, :tc,
                                                  mode, :auto_skip_case),
                         ...
                         {:value, {_, groupResult}} ->
                           {cases,
                              update_status(groupResult, :group_result, grName,
                                              delete_status(ref, status2))}
                         false ->
                           {cases,
                              update_status(:ok, :group_result, grName,
                                              delete_status(ref, status2))}
                       end)
        print_conf_time(confTime)
        reportRepeatStop.()
  ...
  defp run_test_cases_loop([{:make, ref, {mod, func, args}} | cases0],
            config, timetrapData, mode, status) do
    case (run_test_case(ref, 0, mod, func, args, :skip_init,
                          timetrapData)) do
      {_, why = {:EXIT, _}, _} ->
        print(:minor, '~n*** ~tw failed.~n    Skipping all cases.', [func])
        reason = {:failed, {mod, func, why}}
        cases = skip_cases_upto(ref, cases0, reason, :conf,
                                  mode, :auto_skip_case)
        stop_minor_log_file()
        run_test_cases_loop(cases, config, timetrapData, mode,
                              status)
      {_, _Whatever, _} ->
        stop_minor_log_file()
        run_test_cases_loop(cases0, config, timetrapData, mode,
                              status)
    end
  end

  defp run_test_cases_loop([{:conf, _Ref, _Props, _X} = conf | _Cases0],
            config, _TimetrapData, _Mode, _Status) do
    :erlang.error(:badarg, [conf, config])
  end

  defp run_test_cases_loop([{:repeat, case__, {repeatType, n}} | cases0],
            config, timeTrapData, mode, status) do
    ref = make_ref()
    ...
    cond do
      parallel and repeatStop ->
        :erlang.error({:illegal_combination,
                         {:parallel, repeatType}})
      sequence and repeatStop ->
        :erlang.error({:illegal_combination,
                         {:sequence, repeatType}})
      true ->
        mode1 = [{ref, [{:repeat, {repeatType, 1, n}}],
                    :os.timestamp()} |
                     mode]
        run_test_cases_loop([case__ | cases0], config,
                              timeTrapData, mode1, status)
    end
  end

  defp run_test_cases_loop([{mod, case__} | cases], config, timetrapData,
            mode, status) do
    actualCfg = (case (:erlang.get(:test_server_create_priv_dir)) do
                   :auto_per_run ->
                     update_config(hd(config),
                                     [{:priv_dir,
                                         :erlang.get(:test_server_priv_dir)},
    ...
    run_test_cases_loop([{mod, case__, [actualCfg]} |
                             cases],
                          config, timetrapData, mode, status)
  end

  defp run_test_cases_loop([{mod, func, args} = case__ | cases], config,
            timetrapData, mode0, status) do
    {num,
       runInit} = (case (fwMod = get_fw_mod(:test_server_ctrl)) do
                     ^mod when func == :error_in_suite ->
                       {- 1, :skip_init}
                     _ ->
                       {:erlang.put(:test_server_case_num,
    ...
    case (check_prop(:parallel,
                       mode) === false and is_io_buffered()) do
      true ->
        queue_test_case_io(:undefined, self(), num + 1, mod,
                             func)
      false ->
        :ok
    end
    case (run_test_case(:undefined, num + 1, mod, func,
                          args, runInit, timetrapData, mode)) do
      {_, {:framework_error, {^fwMod, fwFunc}, reason}, _} ->
        print(:minor, '~n*** ~w failed in ~tw. Reason: ~tp~n', [fwMod, fwFunc, reason])
        print(1, '~w failed in ~tw. Reason: ~tp~n', [fwMod, fwFunc, reason])
        stop_minor_log_file()
      ...
      {time, retVal, _} ->
        ...
        {result, failed, status1} = (case (retTag) do
                                       skip when skip == :skip or
                                                   skip == :skipped
                                                 ->
                                         {:skipped, false,
                                            update_status(:skipped, mod, func,
                                                            status)}
                                       fail when fail == :EXIT or
                                                   fail == :failed
                                                 ->
                                         {:failed, true,
                                            update_status(:failed, mod, func,
                                                            status)}
                                       ...
                                       _ ->
                                         {:ok, false,
                                            update_status(:ok, mod, func,
                                                            status)}
                                     end)
        case (check_prop(:sequence, mode)) do
          false ->
            {cases1, mode1} = check_repeat_testcase(case__, result,
                                                      cases, mode0)
            stop_minor_log_file()
            run_test_cases_loop(cases1, config, timetrapData, mode1,
                                  status1)
          ref ->
            cond do
              not failed ->
                ...
                run_test_cases_loop(cases1, config, timetrapData, mode1,
                                      status1)
              true ->
                print(:minor, '~n*** ~tw failed.~n    Skipping all other cases in sequence.', [func])
                {cases1, mode1} = check_repeat_testcase(case__, result,
                                                          cases, mode0)
                reason = {:failed, {mod, func}}
                cases2 = skip_cases_upto(ref, cases1, reason, :tc, mode,
                                           :auto_skip_case)
                stop_minor_log_file()
                run_test_cases_loop(cases2, config, timetrapData, mode1,
                                      status1)
            end
        end
      pid ->
        queue_test_case_io(:undefined, pid, num + 1, mod, func)
        {cases1, mode1} = check_repeat_testcase(case__, :ok,
                                                  cases, mode0)
        run_test_cases_loop(cases1, config, timetrapData, mode1,
                              status)
    end
  ...
  defp update_status(:ok, mod, func,
            [{ref, {{ok, skip, fail}, cs}} | status]) do
    [{ref, {{ok ++ [{mod, func}], skip, fail}, cs}} |
         status]
  end

  defp update_status(:skipped, mod, func,
            [{ref, {{ok, skip, fail}, cs}} | status]) do
    [{ref, {{ok, skip ++ [{mod, func}], fail}, cs}} |
         status]
  end

  defp update_status(:failed, mod, func,
            [{ref, {{ok, skip, fail}, cs}} | status]) do
  ...
  defp update_status(ref, {ok, skip, fail},
            [{ref, {{ok0, skip0, fail0}, cs}} | status]) do
    [{ref,
        {{ok0 ++ ok, skip0 ++ skip, fail0 ++ fail}, cs}} |
         status]
  end
  ...
  defp get_data_dir(mod, suite) do
    ...
    case (:code.which(useMod)) do
      ...
      :cover_compiled ->
        ...
        {:file, file} = :rpc.call(mainCoverNode, :cover,
                                    :is_compiled, [useMod])
  ...
  defp get_repeat(props) do
    get_prop([:repeat, :repeat_until_all_ok,
  ...
  defp shuffle_cases(ref, [{:conf, ref, _, _} = start | cases],
            seed0) do
    ...
    shuffledCases = random_order(n,
                                   :rand.uniform_s(n, seed), casesToShuffle, [])
  ...
  defp cases_to_shuffle(ref, [{:skip_case, {_, ref, _, _}, _} | _] = cs,
            n, ix) do
    {n - 1, ix, cs}
  end
  ...
  defp cases_to_shuffle(ref, [{:conf, ref1, _, _} = c | cs], n, ix) do
    ...
    cases_to_shuffle(ref, rest, n + 1,
                       [{n, [c | cs1]} | ix])
  end

  defp cases_to_shuffle(ref,
            [{:skip_case, {_, ref1, _, _}, _} = c | cs], n, ix) do
    {cs1, rest} = get_subcases(ref1, cs, [])
    cases_to_shuffle(ref, rest, n + 1,
                       [{n, [c | cs1]} | ix])
  end
  ...
  defp get_subcases(subRef, [{:conf, subRef, _, _} = c | cs],
            subCs) do
    {:lists.reverse([c | subCs]), cs}
  end

  defp get_subcases(subRef,
            [{:skip_case, {_, subRef, _, _}, _} = c | cs], subCs) do
    {:lists.reverse([c | subCs]), cs}
  end
  ...
  defp random_order(1, {_Pos, seed}, [{_Ix, caseOrGroup}],
            shuffled) do
  ...
  defp random_order(n, {pos, newSeed}, ixCases, shuffled) do
    {first,
       [{_Ix, caseOrGroup} | rest]} = :lists.split(pos - 1,
                                                     ixCases)
    random_order(n - 1, :rand.uniform_s(n - 1, newSeed),
                   first ++ rest, shuffled ++ caseOrGroup)
  end

  defp skip_case(type, ref, caseNum, case__, comment, sendSync,
            mode) do
    mF = ({mod, func} = (case (case__) do
                           {m, f, _A} ->
                             {m, f}
                           {m, f} ->
                             {m, f}
  ...
  defp skip_case1(type, caseNum, mod, func, comment, mode) do
    ...
    groupName = (case (get_name(mode)) do
                   :undefined ->
                     ''
                   grName ->
    ...
    print(:html,
            tR ++ '<td>' ++ col0 ++ '~ts' ++ col1 ++ '</td><td>' ++ col0 ++ '~w' ++ col1 ++ '</td><td>' ++ col0 ++ '~ts' ++ col1 ++ '</td><td>' ++ col0 ++ '~tw' ++ col1 ++ '</td><td>' ++ col0 ++ '< >' ++ col1 ++ '</td><td>' ++ col0 ++ '0.000s' ++ col1 ++ '</td><td><font color="~ts">SKIPPED</font></td><td>~ts</td></tr>\n',
            [num2str(caseNum), fw_name(mod), groupName, func,
                                                            resultCol,
                                                                comment1])
    cond do
      caseNum > 0 ->
  ...
  defp skip_cases_upto(ref, cases, reason, origin, mode, skipType) do
    {_, modified, rest} = modify_cases_upto(ref,
                                              {:skip, reason, origin, mode,
                                                 skipType},
                                              cases)
    modified ++ rest
  end
  ...
  defp copy_cases(origRef, newRef, cases) do
    {original, altered, rest} = modify_cases_upto(origRef,
                                                    {:copy, newRef}, cases)
    {altered, original ++ altered ++ rest}
  end

  defp modify_cases_upto(ref, modOp, cases) do
    {original, altered, rest} = modify_cases_upto(ref,
                                                    modOp, cases, [], [])
    {:lists.reverse(original), :lists.reverse(altered),
       rest}
  end

  defp modify_cases_upto(ref, {:copy, newRef} = op,
            [{:conf, ref, props, mF} = c | t], orig, alt) do
    modify_cases_upto(ref, op, t, [c | orig],
                        [{:conf, newRef, update_repeat(props), mF} | alt])
  end

  defp modify_cases_upto(ref, modOp, cases, orig, alt) do
    case (:lists.any(fn {_, r, _, _} when r == ref ->
                          true
                        {_, r, _} when r == ref ->
                          true
  ...
  defp modify_cases_upto1(ref, {:skip, reason, :conf, mode, :skip_case},
            [{:conf, ref, _Props, mF} | t], orig, alt) do
    {orig,
       [{:skip_case, {:conf, ref, mF, reason}, mode} | alt], t}
  end

  defp modify_cases_upto1(ref,
  ...
  defp modify_cases_upto1(ref, {:copy, newRef},
            [{:conf, ref, props, mF} = c | t], orig, alt) do
    {[c | orig],
       [{:conf, newRef, update_repeat(props), mF} | alt], t}
  end

  defp modify_cases_upto1(ref, {:skip, _, :tc, _, _},
            [{:conf, ref, _Props, _MF} | _] = cs, orig, alt) do
    {orig, alt, cs}
  end

  defp modify_cases_upto1(ref, {:skip, reason, _, mode, skipType},
            [{:make, ref, mF} | t], orig, alt) do
    {orig,
       [{skipType, {:make, ref, mF, reason}, mode} | alt], t}
  end

  defp modify_cases_upto1(ref, {:copy, newRef},
            [{:make, ref, mF} = m | t], orig, alt) do
    {[m | orig], [{:make, newRef, mF} | alt], t}
  end

  defp modify_cases_upto1(ref, {:skip, reason, _, mode, skipType},
            [{:skip_case, {type, ref, mF, _Cmt}, _} | t], orig,
            alt) do
    {orig,
       [{skipType, {type, ref, mF, reason}, mode} | alt], t}
  end

  defp modify_cases_upto1(ref, {:skip, reason, _, mode, skipType},
            [{:skip_case, {type, ref, mF, _Cmt}} | t], orig, alt) do
    {orig,
       [{skipType, {type, ref, mF, reason}, mode} | alt], t}
  end
  ...
  defp modify_cases_upto1(ref, {:copy, newRef},
            ...
            orig, alt) do
    {[c | orig],
       [{:skip_case, {type, newRef, mF, cmt}, mode} | alt], t}
  end

  defp modify_cases_upto1(ref, {:copy, newRef},
            [{:skip_case, {type, ref, mF, cmt}} = c | t], orig,
            alt) do
    {[c | orig],
       [{:skip_case, {type, newRef, mF, cmt}} | alt], t}
  end

  defp modify_cases_upto1(ref, modOp,
            [{:skip_case, {_F, _Cmt}, _Mode} = mF | t], orig,
            alt) do
    modify_cases_upto1(ref, modOp, t, [mF | orig],
                         [mF | alt])
  end

  defp modify_cases_upto1(ref, {:skip, reason, _, mode, :skip_case} = op,
            [{_M, _F} = mF | t], orig, alt) do
    modify_cases_upto1(ref, op, t, orig,
                         [{:skip_case, {mF, reason}, mode} | alt])
  end
  ...
  defp modify_cases_upto1(ref,
            ...
            [{_M, _F} = mF | t], orig, alt) do
    modify_cases_upto1(ref, op, t, orig,
                         [{:auto_skip_case, {mF, reason}, mode} | alt])
  end

  defp modify_cases_upto1(ref, copyOp, [{_M, _F} = mF | t], orig, alt) do
    modify_cases_upto1(ref, copyOp, t, [mF | orig],
                         [mF | alt])
  end

  defp modify_cases_upto1(ref, {:skip, reason, fType, mode, skipType},
            [{:conf, otherRef, props, _MF} | t], orig, alt) do
    case (hd(mode)) do
      {^otherRef, _, _} ->
        modify_cases_upto1(ref,
                             {:skip, reason, fType, tl(mode), skipType}, t,
                             orig, alt)
      _ ->
        mode1 = [conf(otherRef, props) | mode]
        modify_cases_upto1(ref,
                             {:skip, reason, fType, mode1, skipType}, t, orig,
                             alt)
    end
  end

  defp modify_cases_upto1(ref, {:skip, reason, _, mode, skipType} = op,
            [{:repeat, {_M, _F} = mF, _Repeat} | t], orig, alt) do
    modify_cases_upto1(ref, op, t, orig,
                         [{skipType, {mF, reason}, mode} | alt])
  end

  defp modify_cases_upto1(ref, {:skip, _, _, _, _} = op,
            [{skipType, _, _} | t], orig, alt)
  ...
  defp modify_cases_upto1(ref, {:skip, reason, _, mode, skipType} = op,
            [other | t], orig, alt) do
    modify_cases_upto1(ref, op, t, orig,
                         [{skipType, {other, reason}, mode} | alt])
  end

  defp modify_cases_upto1(ref, copyOp, [c | t], orig, alt) do
    modify_cases_upto1(ref, copyOp, t, [c | orig],
                         [c | alt])
  end
  ...
  defp wait_for_cases(ref) do
    case (:erlang.get(:test_server_queued_io)) do
      [] ->
        {[], [], []}
      cases ->
        [_Start | tCs] = :lists.dropwhile(fn {r, _, _, _, _}
                                                 when r == ref ->
                                               false
                                             _ ->
                                               true
  ...
  defp wait_and_resend(ref, [{otherRef, _, 0, _, _} | ps], ok, skip,
            fail)
  ...
  defp wait_and_resend(ref,
            [{_, currPid, caseNum, mod, func} | ps] = cases, ok,
            skip, fail) do
    receive do
      {:finished, _Ref, ^currPid, ^caseNum, ^mod, ^func,
         result, _RetVal} = msg ->
        send(self(), msg)
        mF = {mod, func}
        {ok1, skip1, fail1} = (case (result) do
                                 :ok ->
                                   {[mF | ok], skip, fail}
                                 :skipped ->
                                   {ok, [mF | skip], fail}
      ...
      {:EXIT, ^currPid, reason} when reason != :normal ->
        {:value,
           {_, _, ^caseNum, ^mod,
              ^func}} = :lists.keysearch(currPid, 2, cases)
        print(1, 'Error! Process for test case #~w (~w:~tw) died! Reason: ~tp', [caseNum, mod, func, reason])
        exit({:unexpected_termination, {caseNum, mod, func},
                {currPid, reason}})
    end
  end

  defp wait_and_resend(_, [], ok, skip, fail) do
    {:lists.reverse(ok), :lists.reverse(skip),
       :lists.reverse(fail)}
  end
  ...
  defp handle_test_case_io_and_status() do
    case (:erlang.get(:test_server_queued_io)) do
      ...
      cases ->
        ...
        main = self()
        :lists.foreach(fn {_, pid, _, _, _} when pid != main ->
                            receive do
                              {:EXIT, ^pid, :normal} ->
                                :ok
                            after 1000 ->
  ...
  defp handle_io_and_exit_loop([],
    ...
    receive do
      {:started, _, ^currPid, ^caseNum, ^mod, ^func} ->
        {ok1, skip1, fail1} = (case (handle_io_and_exits(self(),
                                                           currPid, caseNum,
                                                           mod, func, cases)) do
                                 {:ok, mF} ->
                                   {[mF | ok], skip, fail}
  ...
  defp handle_io_and_exit_loop(refs,
            [{ref, currPid, caseNum, mod, func} | ps] = cases, ok,
            skip, fail) do
    receive do
      {:started, _, ^currPid, ^caseNum, ^mod, ^func} ->
        _ = handle_io_and_exits(self(), currPid, caseNum, mod,
                                  func, cases)
        refs1 = (case (refs) do
                   [^ref | rs] ->
                     rs
  ...
  defp handle_io_and_exits(main, currPid, caseNum, mod, func, cases) do
    receive do
      {:abort_current_testcase = tag, _Reason, from} ->
        ...
        handle_io_and_exits(main, currPid, caseNum, mod, func,
                              cases)
      {:finished, _, ^main, ^caseNum, ^mod, ^func, result,
         _RetVal} ->
        :test_server_io.print_buffered(currPid)
        {result, {mod, func}}
      {:finished, _, ^currPid, ^caseNum, ^mod, ^func, result,
         retVal} ->
        :test_server_io.print_buffered(currPid)
        case (result) do
          :ok ->
            {returnTime, _, _} = retVal
            :erlang.put(:test_server_total_time,
                          :erlang.get(:test_server_total_time) + returnTime)
            :erlang.put(:test_server_ok,
                          :erlang.get(:test_server_ok) + 1)
            returnTime
          :failed ->
            returnTime = (case (retVal) do
            ...
            :erlang.put(:test_server_total_time,
                          :erlang.get(:test_server_total_time) + returnTime)
            :erlang.put(:test_server_failed,
                          :erlang.get(:test_server_failed) + 1)
            returnTime
          :skipped ->
            skipCounters = update_skip_counters(retVal,
      ...
      {:EXIT, tCPid, reason} when reason != :normal ->
        :test_server_io.print_buffered(currPid)
        {:value, {_, _, num, m, f}} = :lists.keysearch(tCPid, 2,
                                                         cases)
        print(1, 'Error! Process for test case #~w (~w:~tw) died! Reason: ~tp', [num, m, f, reason])
        exit({:unexpected_termination, {num, m, f},
                {tCPid, reason}})
    end
  end

  defp run_test_case(ref, num, mod, func, args, runInit,
            timetrapData) do
    :ok = :file.set_cwd(:filename.dirname(:erlang.get(:test_server_dir)))
  ...
  defp run_test_case(ref, num, mod, func, args, :skip_init,
            timetrapData, mode) do
  ...
  defp run_test_case1(ref, num, mod, func, args, runInit,
            timetrapData, mode, main) do
    :erlang.group_leader(:test_server_io.get_gl(main == self()),
                           self())
    ...
    updatedArgs = (case (:erlang.get(:test_server_create_priv_dir)) do
                     :auto_per_run ->
                     ...
                     privDirMode ->
                       ...
                       update_config(hd(args),
                                       [{:priv_dir, privDir ++ '/'}, {:tc_logfile,
                                                                      minorName}])
                   end)
    grName = get_name(mode)
    :test_server_sup.framework_call(:report,
    ...
    print(:major, '=started       ~s', [:lists.flatten(timestamp_get(''))])
    {{col0, col1},
       style} = get_font_style(runInit == :run_init, mode)
    tR = xhtml('<tr valign="top">', '<tr class="' ++ odd_or_even() ++ '">')
    encMinorBase = uri_encode(minorBase)
    print(:html,
            tR ++ '<td>' ++ col0 ++ '~ts' ++ col1 ++ '</td><td>' ++ col0 ++ '~w' ++ col1 ++ '</td><td>' ++ col0 ++ '~ts' ++ col1 ++ '</td><td><a href="~ts">~tw</a></td><td><a href="~ts#top">&lt;</a> <a href="~ts#end">&gt;</a></td>',
            [num2str(num), fw_name(mod), grNameStr, encMinorBase,
                                                        func, encMinorBase,
                                                                  encMinorBase])
    do_unless_parallel(main, &:erlang.yield/0)
    {result, detectedFail, procsBefore,
       procsAfter} = run_test_case_apply(num, mod, func,
                                           [updatedArgs], grName, runInit,
                                           timetrapData)
    {time, retVal, loc, opts, comment} = (case (result) do
                                            normal = {_Time, _RetVal, _Loc,
                                                        _Opts, _Comment} ->
                                              normal
    ...
    do_unless_parallel(main,
                         fn () ->
                              :file.set_cwd(:filename.dirname(tSDir))
                         end)
    status = (case ({time, retVal}) do
                ...
                {:died, reason = {:auto_skip, _Why}} ->
                  progress(:skip, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {:died, {skip, reason}} when skip == :skip or
                                               skip == :skipped
                                             ->
                  progress(:skip, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {:died, reason} when reason !== :ok ->
                  progress(:failed, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {_, {:EXIT, {skip, reason}}} when skip == :skip or
                                                    skip == :skipped or
                                                    skip == :auto_skip
                                                  ->
                  progress(:skip, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {_, {:EXIT, _Pid, {skip, reason}}} when skip == :skip or
                                                          skip == :skipped
                                                        ->
                  progress(:skip, num, mod, func, grName, loc, reason,
                             time, comment, style)
                ...
                {_, {:EXIT, reason}} ->
                  progress(:failed, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {_, {fail, reason}} when fail === :fail or
                                           fail === :failed
                                         ->
                  progress(:failed, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {_, reason = {:auto_skip, _Why}} ->
                  progress(:skip, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {_, {skip, reason}} when skip == :skip or
                                           skip == :skipped
                                         ->
                  progress(:skip, num, mod, func, grName, loc, reason,
                             time, comment, style)
                {^time, ^retVal} ->
                  case (detectedFail) do
                    [] ->
                      progress(:ok, num, mod, func, grName, loc, retVal, time,
                                 comment, style)
                    reason ->
                      progress(:failed, num, mod, func, grName, loc, reason,
                                 time, comment, style)
    ...
    case ({runInit, status}) do
      ...
      {_, :failed} ->
        ...
        :erlang.put(:test_server_total_time,
                      :erlang.get(:test_server_total_time) + diedTime)
        :erlang.put(:test_server_failed,
                      :erlang.get(:test_server_failed) + 1)
      {_, :skip} ->
        {uS, aS} = :erlang.get(:test_server_skipped)
    ...
    case (self()) do
      ^main ->
        case (:test_server_sup.framework_call(:warn,
                                                [:processes], true)) do
          true ->
            cond do
              procsBefore < procsAfter ->
                print(:minor, 'WARNING: ~w more processes in system after test case', [procsAfter - procsBefore])
              procsBefore > procsAfter ->
                print(:minor, 'WARNING: ~w less processes in system after test case', [procsBefore - procsAfter])
              true ->
                :ok
            end
          false ->
        ...
        case (:test_server_sup.framework_call(:warn, [:nodes],
                                                true)) do
          true ->
            case ((try do
                    controller_call(:kill_slavenodes)
              ...
              {:EXIT, _} = exit ->
                print(:minor, 'WARNING: There might be slavenodes left in the system. I tried to kill them, but I failed: ~tp\n', [exit])
              [] ->
                :ok
              list ->
                print(:minor, 'WARNING: ~w slave nodes in system after test' ++ 'case. Tried to killed them.~n' ++ '         Names:~tp', [length(list), list])
            end
          false ->
            :ok
        end
    ...
    case (is_io_buffered()) do
      ...
      true ->
        :test_server_io.end_transaction()
        send(main, {:finished, ref, self(), num, mod, func,
                      cond do
                        status == :skip ->
                          :skipped
                        status == :auto_skip ->
  ...
  defp progress(:skip, caseNum, mod, func, grName, loc, reason,
            time, comment, {st0, st1}) do
    {reason1,
       {color, ret, reportTag}} = if_auto_skip(reason,
                                                 fn () ->
                                                      {'#FFA64D', :auto_skip,
                                                         :auto_skipped}
                                                 end,
    ...
    print(1, '*** SKIPPED ~ts ***',
            [get_info_str(mod, func, caseNum,
                            :erlang.get(:test_server_cases))])
    :test_server_sup.framework_call(:report,
                                      [:tc_done, {mod, {func, grName},
                                                    {reportTag, reason1}}])
    timeStr = :io_lib.format(cond do
                               is_float(time) ->
                                 '~.3fs'
                               true ->
                                 '~w'
    ...
    reasonStr1 = :lists.flatten(for s <- :string.lexemes(reasonStr,
                                                           [?\n]) do
    ...
    comment1 = (case (comment) do
                  '' ->
                    ''
                  _ ->
                    xhtml('<br>(', '<br />(') ++ to_string(comment) ++ ')'
                end)
    print(:html, '<td>' ++ st0 ++ '~ts' ++ st1 ++ '</td><td><font color="~ts">SKIPPED</font></td><td>~ts~ts</td></tr>\n',
  ...
  defp progress(:failed, caseNum, mod, func, grName, loc,
            :timetrap_timeout, t, comment0, {st0, st1}) do
    ...
    comment = (case (comment0) do
                 '' ->
                   '<font color="red">' ++ errorReason ++ '</font>'
                 _ ->
                   '<font color="red">' ++ errorReason ++ xhtml('</font><br>', '</font><br />') ++ to_string(comment0)
               end)
    print(:html, '<td>' ++ st0 ++ '~.3fs' ++ st1 ++ '</td><td><font color="red">FAILED</font></td><td>~ts</td></tr>\n',
            [t / 1000, comment])
    formatLoc = :test_server_sup.format_loc(loc)
    print(:minor, '=== Location: ~ts', [formatLoc])
  ...
  defp progress(:failed, caseNum, mod, func, grName, loc,
            {:testcase_aborted, reason}, _T, comment0,
            {st0, st1}) do
    ...
    print(:html, '<td>' ++ st0 ++ 'died' ++ st1 ++ '</td><td><font color="red">FAILED</font></td><td>~ts</td></tr>\n', [comment])
    formatLoc = :test_server_sup.format_loc(loc)
    print(:minor, '=== Location: ~ts', [formatLoc])
    print(:minor, '~ts',
  ...
  defp progress(:failed, caseNum, mod, func, grName, :unknown,
            reason, time, comment0, {st0, st1}) do
    ...
    timeStr = :io_lib.format(cond do
                               ...
                               [time])
    errorReason = escape_chars(:lists.flatten(:io_lib.format('~tp',
                                                               [reason])))
    errorReason1 = :lists.flatten(for s <- :string.lexemes(errorReason,
                                                             [?\n]) do
                                    :string.trim(s, :leading, ' ')
                                  end)
    errorReasonLength = :string.length(errorReason1)
    errorReason2 = (cond do
    ...
    comment = (case (comment0) do
                 ...
                 _ ->
                   '<font color="red">' ++ errorReason2 ++ xhtml('</font><br>', '</font><br />') ++ to_string(comment0)
               end)
    print(:html, '<td>' ++ st0 ++ '~ts' ++ st1 ++ '</td><td><font color="red">FAILED</font></td><td>~ts</td></tr>\n',
            [timeStr, comment])
    print(:minor, '=== Location: ~w', [:unknown])
    {fStr, formattedReason} = format_exception(reason)
    print(:minor, '~ts',
            [escape_chars(:io_lib.format('=== Reason: ' ++ fStr,
                                           [formattedReason]))])
    :failed
  end
  ...
  defp progress(:failed, caseNum, mod, func, grName, loc,
    ...
    print(:html, '<td>' ++ st0 ++ '~ts' ++ st1 ++ '</td><td><font color="red">FAILED</font></td><td><font color="red">~ts</font>~ts</td></tr>\n',
            [timeStr, formatLastLoc, comment])
    formatLoc = :test_server_sup.format_loc(locMin)
    print(:minor, '=== Location: ~ts', [formatLoc])
    {fStr, formattedReason} = format_exception(reason)
    print(:minor, '~ts',
            ['=== Reason: ' ++ escape_chars(:io_lib.format(fStr,
                                                [formattedReason]))])
    :failed
  end

  defp progress(:ok, _CaseNum, mod, func, grName, _Loc, retVal,
            time, comment0, {st0, st1}) do
    ...
    print(:html, '<td>' ++ st0 ++ '~ts' ++ st1 ++ '</td><td><font color="green">Ok</font></td>~ts</tr>\n',
            [timeStr, comment])
    print(:minor, '~ts',
  ...
  def escape_chars(term) when (not is_list(term) and
                       not is_binary(term)) do
    esc_chars_in_list(:io_lib.format('~tp', [term]))
  end

  def escape_chars(list = [term | _]) when (not is_list(term) and
  ...
  defp if_auto_skip(reason = {:failed, {_, :init_per_testcase, _}},
            true__, _False) do
    {reason, true__.()}
  end

  defp if_auto_skip({:skip,
             reason = {:failed, {_, :init_per_testcase, _}}},
            true__, _False) do
    {reason, true__.()}
  end
  ...
  defp reason_to_string({:failed,
             {_, failFunc, {:timetrap_timeout, _}}}) do
    :erlang.atom_to_list(failFunc) ++ ' timed out'
  end

  defp reason_to_string(fWInitFail = {:failed,
                          {_CB, :init_tc, _Reason}}) do
  ...
  defp get_font_style(normalCase, mode) do
    ...
    {{'<font color=' ++ col ++ '>', '</font>'}, {'<font color=' ++ col ++ '>' ++ st0, st1 ++ '</font>'}}
  end
  ...
  defp format_exception(reason = {_Error, stack}) when is_list(stack) do
    case (get_fw_mod(:undefined)) do
      :undefined ->
        case (:application.get_env(:test_server,
                                     :format_exception)) do
          {:ok, false} ->
            {'~tp', reason}
          _ ->
  ...
  defp do_format_exception(reason = {error, stack}) do
    ...
    pF = fn term, i ->
              :io_lib.format('~.' ++ :erlang.integer_to_list(i) ++ 'tp',
                               [term])
         end
    case ((try do
            :erl_error.format_exception(1, :error, error, stack,
                                          stackFun, pF, :utf8)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
      ...
      {:EXIT, _R} ->
        {'~tp', reason}
      formatted ->
        formatted1 = :re.replace(formatted, 'exception error: ', '',
                                   [{:return, :list}, :unicode])
        {'~ts', :lists.flatten(formatted1)}
    end
  end

  defp run_test_case_apply(caseNum, mod, func, args, name, runInit,
            timetrapData) do
    :test_server.run_test_case_apply({caseNum, mod, func,
                                        args, name, runInit, timetrapData})
  end
  ...
  defp print_or_buffer(detail, msg, printer) do
    :test_server_gl.print(:erlang.group_leader(), detail,
                            msg, printer)
  end
  ...
  Record.defrecord(:r_cc, :cc, mod: :undefined,
                              skip: :undefined)
  defp collect_all_cases(top, skip) when is_list(skip) do
    result = (case (collect_cases(top,
                                    r_cc(mod: [], skip: skip), [])) do
                {:ok, cases, _St} ->
                  cases
                other ->
                  other
  ...
  defp collect_cases({:conf, initF, caseList, finMF}, st, mode)
    ...
    collect_cases({:conf, [], {r_cc(st, :mod), initF},
                     caseList, finMF},
                    st, mode)
  end
  ...
  defp collect_cases({:conf, initMF, caseList, finF}, st, mode)
    ...
    collect_cases({:conf, [], initMF, caseList,
                     {r_cc(st, :mod), finF}},
                    st, mode)
  end
  ...
  defp collect_cases({:conf, props, initF, caseList, finMF}, st,
            mode)
      when is_atom(initF) do
    case (init_props(props)) do
      {:error, _} ->
        {:ok, [], st}
      props1 ->
        collect_cases({:conf, props1, {r_cc(st, :mod), initF},
                         caseList, finMF},
                        st, mode)
    end
  end

  defp collect_cases({:conf, props, initMF, caseList, finF}, st,
            mode)
      when is_atom(finF) do
    case (init_props(props)) do
      {:error, _} ->
        {:ok, [], st}
      props1 ->
        collect_cases({:conf, props1, initMF, caseList,
                         {r_cc(st, :mod), finF}},
                        st, mode)
    end
  end

  defp collect_cases({:conf, props, initMF, caseList, finMF} = conf,
            st, mode) do
    case (init_props(props)) do
      {:error, _} ->
        {:ok, [], st}
      props1 ->
        ref = make_ref()
        skips = r_cc(st, :skip)
        props2 = [{:suite, r_cc(st, :mod)} | :lists.delete(:suite,
                                                          props1)]
        ...
        case (in_skip_list({r_cc(st, :mod), conf}, skips)) do
          ...
          {true, toSkip, _} when is_list(toSkip) ->
            case (collect_cases(caseList,
                                  r_cc(st, skip: toSkip ++ skips), mode1)) do
              {:ok, [], _St} = empty ->
                empty
              {:ok, flatCases, st1} ->
                {:ok,
                   [{:conf, ref, props1, initMF} | flatCases ++ [{:conf,
                                                                    ref,
                                                                    keep_name(props1),
                                                                    finMF}]],
                   r_cc(st1, skip: skips)}
  ...
  defp collect_cases({:make, initMFA, caseList, finMFA}, st0,
            mode) do
    case (collect_cases(caseList, st0, mode)) do
      ...
      {:ok, flatCases, st} ->
        ref = make_ref()
        {:ok,
           [{:make, ref, initMFA} | flatCases ++ [{:make, ref,
                                                     finMFA}]],
           st}
      {:error, _Reason} = error ->
        error
  ...
  defp collect_cases({:repeat, {module, case__}, repeat}, st,
            mode) do
    case ((try do
  ...
  defp collect_cases(case__, st, mode) when (is_atom(case__) and
  ...
  defp collect_case([case__ | cases], st, acc, mode) do
    {:ok, flatCases, newSt} = collect_case({r_cc(st, :mod),
                                              case__},
                                             st, mode)
  ...
  defp collect_subcases(mod, case__, mFA, st, suite, mode) do
    case (suite) do
      ...
      [] ->
        {:ok, [mFA], st}
      {:req, reqList} ->
        collect_case_deny(mod, case__, mFA, reqList, [], st,
                            mode)
      {:req, reqList, subCases} ->
        collect_case_deny(mod, case__, mFA, reqList, subCases,
                            st, mode)
  ...
  defp collect_case_subcases(mod, case__, subCases, st0, mode) do
    oldMod = r_cc(st0, :mod)
    case (collect_cases(subCases, r_cc(st0, mod: mod),
                          mode)) do
      {:ok, flatCases, st} ->
  ...
  defp collect_files(dir, pattern, st, mode) do
    ...
    wc = :filename.join([dir1,
                             pattern ++ '{.erl,' ++ :code.objfile_extension() ++ '}'])
    case ((try do
            :filelib.wildcard(wc)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
      ...
      files ->
        mods = :lists.foldl(fn file, acc ->
                              ...
                              [], files)
        tests = (for mod <- :lists.sort(mods) do
                   {mod, :all}
                 end)
  ...
  defp collect_case_deny(mod, case__, mFA, reqList, subCases, st,
            mode) do
  ...
  defp in_skip_list({mod,
             {:conf, props, initMF, _CaseList, _FinMF}},
            skipList) do
      ...
      _ ->
        case (:proplists.get_value(:name, props)) do
          :undefined ->
            false
          name ->
            toSkip = :lists.flatmap(fn {m,
                                          {:conf, sProps, _, sCaseList, _}, cmt}
                                           when m == mod ->
                                         case (:proplists.get_value(:name,
                                                                      sProps)) do
                                           :all ->
                                             [{m, :all, cmt}]
                                           ^name ->
  ...
  defp in_skip_list({mod, func},
            [{mod, func, comment} | _SkipList]) do
    {true, comment}
  end
  ...
  def start_node(name, type, options) do
    ...
    case (controller_call({:start_node, name, type,
                             options},
                            t)) do
      {{:ok, nodename}, host, cmd, info, warning} ->
        format(:minor, 'Successfully started node ~w on ~tp with command: ~ts', [nodename, host, cmd])
        ...
        {:ok, nodename}
      {:fail, {ret, host, cmd}} ->
        format(:minor, 'Failed to start node ~tp on ~tp with command: ~ts~nReason: ~tp', [name, host, cmd, ret])
        {:fail, ret}
  ...
  defp ts_pid(x, y, z) when (is_integer(x) and
    ...
    case (pinfo(pid)) do
      ...
      info ->
        ...
        pformat(:io_lib.format('~w', [pid]),
                  :io_lib.format('~tw', [call]), :io_lib.format('~tw', [curr]),
                  reds, lM)
  ...
  defp check_cover_file([{:exclude, exclude} | rest], _, include,
            cross) do
    case (:lists.all(fn m ->
                          is_atom(m)
                     end,
  ...
  defp check_cover_file([{:include, include} | rest], exclude, _,
            cross) do
    case (:lists.all(fn m ->
                          is_atom(m)
                     end,
  ...
  defp check_cover_file([{:cross, cross} | rest], exclude, include,
            _) do
  ...
  def cover_analyse(coverInfo, testDir) do
    ...
    {:ok, coverLog} = open_html_file(:filename.join(testDir,
                                                      'cover.html'))
    write_coverlog_header(coverLog)
    r_cover(app: app, file: coverFile, excl: excluded,
        cross: cross) = coverInfo
    :io.fwrite(coverLog, '<h1>Coverage for application \'~w\'</h1>\n', [app])
    :io.fwrite(coverLog, '<p><a href="~ts">Coverdata collected over all tests</a></p>', ['cross_cover.html'])
    ...
    coverage = :test_server.cover_analyse(testDir,
                                            coverInfo)
    :ok = write_binary_file(:filename.join(testDir, 'cover.log'),
                              :erlang.term_to_binary(coverage))
    case (:lists.filter(fn {_M, {_, _, _}} ->
                             false
                           _ ->
                             true
                        end,
      ...
      bad ->
        :io.fwrite(coverLog, '<p>Analysis failed for ~w module(s): <code>~w</code>\n',
                     [length(bad), for {badM, {_, _Why}} <- bad do
                                     badM
                                   end])
    end
    totPercent = write_cover_result_table(coverLog,
                                            coverage)
    :ok = write_binary_file(:filename.join(testDir, 'total_cover.log'),
  ...
  def cross_cover_analyse(analyse, tagDirs0) do
    ...
    :lists.foreach(fn cDF ->
                        :cover.import(cDF)
                   end,
                     coverdataFiles)
    :io.fwrite('Cover analysing...\n', [])
    detailsFun = (case (analyse) do
                    :details ->
                      fn dir, m ->
  ...
  defp write_cross_cover_info(dir, cross) do
    write_binary_file(:filename.join(dir, 'cross_cover.info'),
  ...
  defp write_cross_cover_logs([{tag, coverage} | t], tagDirMods) do
    case (:lists.keyfind(tag, 1, tagDirMods)) do
      {_, dir, mods} when mods !== [] ->
        :ok = write_binary_file(:filename.join(dir, 'cross_cover.log'),
                                  :erlang.term_to_binary(coverage))
  ...
  defp do_add_cross_modules([{tag, mods1} | tagMods], tagDirMods) do
    newTagDirMods = (case (:lists.keytake(tag, 1,
                                            tagDirMods)) do
  ...
  defp get_coverdata_files(tagDirMods) do
    :lists.flatmap(fn {_, latestDir, _} ->
                        :filelib.wildcard(:filename.join(latestDir, 'all.coverdata'))
                   end,
                     tagDirMods)
  end

  defp analyse_tests([{tag, lastTest, modules} | t], detailsFun,
            acc) do
  ...
  defp format_analyse(m, cov, notCov, :undefined) do
    :io_lib.fwrite('<tr><td>~w</td><td align=right>~w %</td><td align=right>~w</td><td align=right>~w</td></tr>\n', [m, pc(cov, notCov), cov, notCov])
  end

  defp format_analyse(m, cov, notCov, {:file, file}) do
    :io_lib.fwrite('<tr><td><a href="~ts">~w</a></td><td align=right>~w %</td><td align=right>~w</td><td align=right>~w</td></tr>\n',
                     [uri_encode(:filename.basename(file)), m, pc(cov,
                                                                    notCov),
                                                                   cov, notCov])
  end
  ...
  defp format_analyse(m, cov, notCov, {:lines, lines}) do
    ...
    :io_lib.fwrite('<tr><td><a href="~ts">~w</a></td><td align=right>~w %</td><td align=right>~w</td><td align=right>~w</td></tr>\n',
                     [uri_encode(coverOutName), m, pc(cov, notCov), cov,
                                                                        notCov])
  end
  ...
  defp write_not_covered(coverOut, m, lines) do
    ...
    :io.fwrite(coverOut, 'The following lines in module ~w are not covered:\n<table border=3 cellpadding=5>\n<th>Line Number</th>\n', [m])
    :lists.foreach(fn {{_M, line}, {0, 1}} ->
  ...
  defp write_default_cross_coverlog(testDir) do
    {:ok,
       crossCoverLog} = open_html_file(:filename.join(testDir,
                                                        'cross_cover.html'))
  ...
  defp write_cover_result_table(coverLog, coverage) do
    :io.fwrite(coverLog, '<p><table border=3 cellpadding=5>\n<tr><th>Module</th><th>Covered (%)</th><th>Covered (Lines)</th><th>Not covered (Lines)</th>\n', [])
    {totCov, totNotCov} = :lists.foldl(fn {m,
                                             {cov, notCov, details}},
                                            {accCov, accNotCov} ->
                                            str = format_analyse(m, cov, notCov,
                                                                   details)
    ...
    :io.fwrite(coverLog, '<tr><th align=left>Total</th><th align=right>~w %</th><th align=right>~w</th><th align=right>~w</th></tr>\n</table>\n</body>\n</html>\n', [totPercent, totCov, totNotCov])
    :ok = :file.close(coverLog)
    totPercent
  end

  defp html_header(title) do
    ['<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">\n<!-- autogenerated by \'', :erlang.atom_to_list(:test_server_ctrl), '\'. -->\n<html>\n<head>\n<title>', title,
  ...
  defp open_utf8_file(file) do
    case (:file.open(file,
                       allOpts = [:write, {:encoding, :utf8}])) do
      {:error, reason} ->
  ...
  def uri_encode(file, encoding) do
    ...
    :filename.join(for c <- components do
    ...
    case (char > 127 and encoding == :latin1 or reserved) do
      true ->
        [?% | :erlang.integer_to_list(char,
                                        16)] ++ uri_encode_comp(chars, encoding)
      false ->
  ...
  defp check_repeat_testcase(case__, result, cases,
            [{ref, [{:repeat, repeatData0}], startTime} | mode0]) do
    case (do_update_repeat_data(result, repeatData0)) do
      false ->
        {cases, mode0}
      repeatData ->
        {[case__ | cases],
           [{ref, [{:repeat, repeatData}], startTime} | mode0]}
    end
  end
