defmodule :m_unix_telnet do
  use Bitwise
  import :ct_telnet, only: [end_gen_log: 0, log: 4,
                              start_gen_log: 1]
  def get_prompt_regexp() do
    'login: |Password: |\\$ |> '
  end

  def connect(connName, ip, port, timeout, keepAlive,
           tCPNoDelay, extra) do
    case (extra) do
      {username, password} ->
        connect1(connName, ip, port, timeout, keepAlive,
                   tCPNoDelay, username, password)
      keyOrName ->
  ...
  defp connect1(name, ip, port, timeout, keepAlive, tCPNoDelay,
            username, password) do
    start_gen_log('unix_telnet connect')
    result = (case (:ct_telnet_client.open(ip, port,
                                             timeout, keepAlive, tCPNoDelay,
                                             name)) do
                {:ok, pid} ->
                  case (:ct_telnet.silent_teln_expect(name, pid, [],
                                                        [:prompt], 'login: |Password: |\\$ |> ', [])) do
                    {:ok, {:prompt, 'login: '}, _} ->
                      ...
                      case (:ct_telnet.silent_teln_expect(name, pid, [],
                        ...
                        {:ok, {:prompt, 'Password: '}, _} ->
                          ...
                          case (:ct_telnet.silent_teln_expect(name, pid, [],
                                                                ...
                                                                [])) do
                            {:ok, {:prompt, prompt}, _} when (prompt !== 'login: ' and
                                                                prompt !== 'Password: ')
                                                             ->
                              {:ok, pid}
                            error ->
                    ...
                    {:ok,
                       [{:prompt, _OtherPrompt1}, {:prompt, _OtherPrompt2}],
                       _} ->
                      {:ok, pid}
                    error ->
