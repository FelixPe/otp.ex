defmodule :m_ct_make do
  ...
  def files(fs0, options) do
    ...
    {makeOpts, compileOpts} = sort_options(options, [], [])
    case (get_opts_from_emakefile(fs, :Emakefile,
                                    compileOpts)) do
      files when is_list(files) ->
        do_make_files(files, makeOpts)
      :error ->
  ...
  defp do_make_files(fs, opts) do
    process(fs, :lists.member(:noexec, opts),
              load_opt(opts), [])
  end

  defp sort_options([h | t], make, comp) do
    case (:lists.member(h,
                          [:noexec, :load, :netload, :noload])) do
      true ->
  ...
  defp read_emakefile(emakefile, opts) do
    case (:file.consult(emakefile)) do
      ...
      {:error, :enoent} ->
        mods = (for f <- :filelib.wildcard('*.erl') do
                  :filename.rootname(f)
                end)
        [{mods, opts}]
      {:error, other} ->
  ...
  defp transform([{mod, modOpts} | emake], opts, files,
            already) do
    case (expand(mod, already)) do
      ...
      mods ->
        transform(emake, opts,
                    [{mods, modOpts ++ opts} | files], mods ++ already)
    end
  end
  ...
  defp expand(mods, already) when (is_list(mods) and
                                 not is_integer(hd(mods))) do
    :lists.concat(for mod <- mods do
                    expand(mod, already)
                  end)
  end
  ...
  defp get_opts_from_emakefile2([{makefileMods, o} | rest], mods, opts,
            result) do
    case (members(mods, makefileMods, [], mods)) do
      {[], _} ->
        get_opts_from_emakefile2(rest, mods, opts, result)
      {i, restOfMods} ->
        get_opts_from_emakefile2(rest, restOfMods, opts,
                                   [{i, o} | result])
    end
  end
  ...
  defp process([{[h | t], opts} | rest], noExec, load,
            result) do
    case (recompilep(coerce_2_list(h), noExec, load,
                       opts)) do
      :error ->
  ...
  defp recompilep(file, noExec, load, opts) do
    ...
    objFile = (case (:lists.keysearch(:outdir, 1, opts)) do
                 {:value, {:outdir, outDir}} ->
  ...
  defp recompilep1(file, noExec, load, opts, objFile) do
    {:ok, erl} = :file.read_file_info(:lists.append(file,
                                                      '.erl'))
  ...
  defp recompilep1(r_file_info(mtime: te), r_file_info(mtime: to), file, noExec, load,
            opts)
      when te > to do
    recompile(file, noExec, load, opts)
  end
  ...
  defp recompile2(objMTime, file, noExec, load, opts) do
    ...
    case (check_includes(:lists.append(file, '.erl'),
                           includePath, objMTime)) do
      true ->
        recompile(file, noExec, load, opts)
      false ->
        :up_to_date
  ...
  defp do_recompile(file, false, load, opts) do
    ...
    case (:compile.file(file,
                          [:report_errors, :report_warnings | opts])) do
