defmodule :m_ct_telnet do
  ...
  def open(keyOrName, connType, targetMod, extra) do
    case :ct.get_config({keyOrName, connType}) do
      :undefined ->
        log(:undefined, :open, ~c"Failed: ~tp", [{:not_available, keyOrName}])
      ...
      addr ->
        ...
        log(:undefined, :open, ~c"Connecting to ~tp(~tp)", [keyOrName, addr1])
  ...
  def close(connection) do
    case get_handle(connection) do
      {:ok, pid} ->
        log(:undefined, :close, ~c"Connection closed, handle: ~w", [pid])
  ...
  defp set_telnet_defaults([unknown | ss], s) do
    force_log(s, :error, ~c"Bad element in telnet_settings: ~tp", [unknown])
    set_telnet_defaults(ss, s)
  end
  ...
  def handle_msg({:cmd, cmd, opts}, state) do
    ...
    log(state, :cmd, ~c"Cmd: ~tp", [cmd])
    debug_cont_gen_log(~c"Throwing Buffer:", [])
    ...
    {return, newBuffer, prompt} =
      case teln_cmd(r_state(state, :teln_pid), cmd, r_state(state, :prx), newline, tO) do
        {:ok, data, _PromptType, rest} ->
          log(state, :recv, ~c"Return: ~tp", [{:ok, data}])
          {{:ok, data}, rest, true}
        ...
        error ->
          ...
          log(state, :recv, ~c"Return: ~tp", [error])
          {retry, [], false}
      end
  ...
  def handle_msg({:send, cmd, opts}, state) do
    ...
    log(state, :send, ~c"Sending: ~tp", [cmd])
    debug_cont_gen_log(~c"Throwing Buffer:", [])
    debug_log_lines(r_state(state, :buffer))
    ...
    {return, newBuffer, prompt} =
      case teln_expect(
        ...
        {:ok, data, haltReason, rest} ->
          force_log(state, :expect, ~c"HaltReason: ~tp", [haltReason])
          p = check_if_prompt_was_reached(data, haltReason)
          {{:ok, data, haltReason}, rest, p}

        {:error, reason, rest} ->
          force_log(state, :expect, ~c"Expect failed\n~tp", [{:error, reason}])
  ...
  defp reconnect(
    ...
    case connResult do
      ...
      _Error ->
        log(state, :reconnect, ~c"Reconnect failed!", ~c"Retries left: ~w", [n])
  ...
  def terminate(telnPid, state) do
    ...
    log(state, :close, ~c"Telnet connection for ~w closed.", [telnPid])
    result
  end
  ...
  defp heading(action, :undefined) do
    :io_lib.format(~c"~w ~w", [:ct_telnet, action])
  end

  defp heading(action, name) do
    :io_lib.format(~c"~w ~w for ~tp", [:ct_telnet, action, name])
  end
      ...
      notFinished ->
        ...
        case :timer.tc(:ct_gen_conn, :do_within_time, [fun, breakAfter]) do
          {_, {:error, reason}} ->
            cond do
              rest1 != [] ->
                log(name_or_pid(name, pid), ~c"       ~ts", [rest1])
      ...
      {:prompt, uptoPrompt, promptType, rest} ->
        case pattern do
          ...
          when prompt == :prompt or
            ...
            log(name_or_pid(name, pid), ~c"PROMPT: ~ts", [promptType])
  ...
  defp match_line(name, pid, line, [:prompt | _Patterns], foundPrompt, _Term, _EO, retTag) do
    log(name_or_pid(name, pid), ~c"       ~ts", [line])
    log(name_or_pid(name, pid), ~c"PROMPT: ~ts", [foundPrompt])
  ...
  defp match_line(name, pid, line, [{tag, pattern} | patterns], foundPrompt, term, eO, retTag) do
    case :re.run(line, pattern, [{:capture, :all, :list}]) do
      ...
      {:match, match} ->
        log(name_or_pid(name, pid), ~c"MATCH: ~ts", [line])
        {retTag, {tag, match}}
    end
  end
  ...
  defp log_lines(name, pid, string) do
    case log_lines_not_last(name, pid, string) do
      ...
      lastLine ->
        log(name_or_pid(name, pid), ~c"       ~ts", [lastLine])
    end
  end
  ...
  defp log_lines_not_last(name, pid, string) do
    case add_tabs(string, [], []) do
      ...
      {string1, lastLine} ->
        log(name_or_pid(name, pid), ~c"~ts", [string1])
        lastLine
    end
  end
  ...
  defp match_prompt(str, prx, acc) do
    case :re.run(str, prx, [:unicode]) do
      ...
      {:match, [{start, len}]} ->
        <<uptoPrompt::size(start)-binary, prompt::size(len)-binary, rest::binary>> = str

        case validate_prompt(start, uptoPrompt, prompt) do
          :ok ->
            {:prompt, :unicode.characters_to_list([:lists.reverse(acc), uptoPrompt, prompt]),
             :unicode.characters_to_list(prompt), :unicode.characters_to_list(rest)}

          :recurse ->
            <<skip::size(start + len)-binary, cont::binary>> = str
  ...
  defp validate_prompt(size, prePrompt, prompt) do
    case prePrompt do
      ...
      <<_::size(size - 5)-binary, _L::size(8), "ast ", _::binary>>
      when prompt === "login: " ->
        :recurse
