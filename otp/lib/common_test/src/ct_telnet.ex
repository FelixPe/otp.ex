defmodule :m_ct_telnet do
  ...
  Record.defrecord(:r_state, :state, host: :undefined,
                                 port: :undefined, teln_pid: :undefined,
                                 prx: :undefined, buffer: [], prompt: false,
                                 name: :undefined, type: :undefined,
                                 target_mod: :undefined, keep_alive: :undefined,
                                 poll_limit: 0, poll_interval: 1000,
                                 extra: :undefined, conn_to: 10000,
                                 com_to: 10000, reconns: 3, reconn_int: 5000,
                                 tcp_nodelay: false)
  def open(name) do
    open(name, :telnet)
  end
  ...
  def open(keyOrName, connType, targetMod, extra) do
    case (:ct.get_config({keyOrName, connType})) do
      ...
      addr ->
        ...
        keepAlive = (case (:ct.get_config({keyOrName,
                                             :keep_alive})) do
                       :undefined ->
                         case (:ct.get_config({:telnet_settings,
                                                 :keep_alive})) do
                           :undefined ->
                             true
                           bool ->
        ...
        reconnect = (case (:ct.get_config({:telnet_settings,
                                             :reconnection_attempts})) do
                       0 ->
                         false
                       _ ->
                         true
        ...
        :ct_gen_conn.start(full_addr(addr1, connType),
                             {targetMod, keepAlive, extra}, :ct_telnet,
                             [{:name, keyOrName}, {:reconnect, reconnect},
                                                      {:old, true}])
    end
  end
  ...
  defp check_send_opts([{:newline, string} | opts])
    ...
    case (:lists.all(fn i when (is_integer(i) and i >= 0 and
                                  i <= 127)
                               ->
                          true
                        _ ->
    ...
    case (get_handle(connection)) do
      {:ok, pid} ->
        case (call(pid, {:expect, patterns, opts})) do
          {:error, reason} when :erlang.element(1,
                                                  reason) == :bad_pattern
                                ->
  ...
  def init(name, {ip, port, type},
           {targetMod, keepAlive, extra}) do
    s0 = (case (:ct.get_config(:telnet_settings)) do
            :undefined ->
              r_state()
            settings ->
    ...
    try do
      case (:erlang.function_exported(targetMod, :connect,
                                        7)) do
        true ->
          targetMod.connect(name, ip, port, r_state(s0, :conn_to),
                              keepAlive, r_state(s0, :tcp_nodelay), extra)
        false ->
          targetMod.connect(name, ip, port, r_state(s0, :conn_to),
                              keepAlive, extra)
      end
      ...
      {:ok, telnPid} ->
        ...
        s1 = r_state(s0, host: ip,  port: port,  teln_pid: telnPid, 
                     name: name,  type: type(type),  target_mod: targetMod, 
                     keep_alive: keepAlive,  extra: extra, 
                     prx: targetMod.get_prompt_regexp())
        log(s1, :open, 'Opened telnet connection\nIP: ~p\nPort: ~p\nCommand timeout: ~p\nReconnection attempts: ~p\nReconnection interval: ~p\nConnection timeout: ~p\nKeep alive: ~w\nPoll limit: ~w\nPoll interval: ~w\nTCP nodelay: ~w',
              [ip, port, r_state(s1, :com_to), r_state(s1, :reconns),
                                             r_state(s1, :reconn_int),
                                                 r_state(s1, :conn_to), keepAlive,
  ...
  def handle_msg({:cmd, cmd, opts}, state) do
    ...
    _ = (case ({r_state(state, :type), r_state(state, :prompt)}) do
           {:ts, _} ->
             silent_teln_expect(r_state(state, :name), r_state(state, :teln_pid),
                                  r_state(state, :buffer), :prompt, r_state(state, :prx),
                                  [{:idle_timeout, 2000}])
           {:ip, false} ->
             silent_teln_expect(r_state(state, :name), r_state(state, :teln_pid),
                                  r_state(state, :buffer), :prompt, r_state(state, :prx),
    ...
    tO = (case (:proplists.get_value(:timeout, opts,
                                       :default)) do
            :default ->
    ...
    {return, newBuffer,
       prompt} = (case (teln_cmd(r_state(state, :teln_pid), cmd,
                                   r_state(state, :prx), newline, tO)) do
                    {:ok, data, _PromptType, rest} ->
                      log(state, :recv, 'Return: ~tp', [{:ok, data}])
                      {{:ok, data}, rest, true}
                    error ->
                      retry = {:retry,
                                 {error, {r_state(state, :name), r_state(state, :type)},
                                    r_state(state, :teln_pid), {:cmd, cmd, opts}}}
  ...
  def handle_msg({:send, cmd, opts}, state) do
    ...
    newline = :proplists.get_value(:newline, opts, true)
    :ct_telnet_client.send_data(r_state(state, :teln_pid), cmd,
                                  newline)
    end_gen_log()
  ...
  def handle_msg(:get_data, state) do
    ...
    {:ok, data, buffer} = teln_get_all_data(state,
                                              r_state(state, :buffer), [], [],
                                              r_state(state, :poll_limit))
    log(state, :recv, 'Return: ~tp', [{:ok, data}])
    end_gen_log()
  ...
  def handle_msg({:expect, pattern, opts}, state) do
    ...
    {return, newBuffer,
       prompt} = (case (teln_expect(r_state(state, :name),
                                      r_state(state, :teln_pid), r_state(state, :buffer),
                                      pattern, r_state(state, :prx), opts)) do
                    {:ok, data, rest} ->
    ...
    end_gen_log()
    return1 = (case (return) do
                 {:error, _} ->
                   {:retry,
                      {return, {r_state(state, :name), r_state(state, :type)},
                         r_state(state, :teln_pid), {:expect, pattern, opts}}}
                 _ ->
                   return
               end)
  ...
  defp reconnect(ip, port, n,
            state = r_state(name: name, target_mod: targetMod,
                        keep_alive: keepAlive, extra: extra, conn_to: connTo,
                        reconn_int: reconnInt, tcp_nodelay: noDelay)) do
    connResult = (case (:erlang.function_exported(targetMod,
                                                    :connect, 7)) do
                    true ->
                      targetMod.connect(name, ip, port, connTo, keepAlive,
                                          noDelay, extra)
                    false ->
                      targetMod.connect(name, ip, port, connTo, keepAlive,
                                          extra)
  ...
  defp get_handle({name, type}) when type == :telnet or
  ...
  defp log(r_state(name: name, teln_pid: telnPid, host: host,
              port: port),
            action, string, args, forcePrint) do
    name1 = (cond do
               name == :undefined ->
    ...
    cond do
      action == :general_io ->
        case (:ct_util.get_testdata({:cth_conn_log,
                                       :ct_telnet})) do
          hookMode when (hookMode != :undefined and
                           hookMode != :silent and silent != true)
                        ->
            :error_logger.info_report(r_conn_log(header: false,
                                          client: self(), conn_pid: telnPid,
                                          address: {host, port}, name: name1,
                                          action: action, module: :ct_telnet),
                                        {string, args})
          _ ->
            :ok
        end
  ...
  defp teln_get_all_data(state = r_state(teln_pid: pid, prx: prx), data, acc,
            lastLine, polls) do
    case (check_for_prompt(prx, lastLine ++ data)) do
      {:prompt, lines, _PromptType, rest} ->
        teln_get_all_data(state, rest, [lines | acc], [],
                            r_state(state, :poll_limit))
      {:noprompt, lines, lastLine1} ->
        case (:ct_telnet_client.get_data(pid)) do
          {:ok, []} when (lastLine1 != [] and polls > 0) ->
            ...
            teln_get_all_data(state, [], [lines | acc], lastLine1,
                                newPolls)
          {:ok, []} ->
            {:ok, :lists.reverse(:lists.append([lines | acc])),
               lastLine1}
          {:ok, data1} ->
            teln_get_all_data(state, data1, [lines | acc],
                                lastLine1, r_state(state, :poll_limit))
        end
    end
  end

  Record.defrecord(:r_eo, :eo, teln_pid: :undefined,
                              prx: :undefined, idle_timeout: :undefined,
                              total_timeout: :undefined, haltpatterns: [],
                              seq: false, repeat: false, found_prompt: false,
                              prompt_check: true)
  def silent_teln_expect(name, pid, data, pattern, prx, opts) do
    old = :erlang.put(:silent, true)
    result = teln_expect(name, pid, data, pattern, prx,
                           opts)
    :erlang.put(:silent, old)
    result
  end
  ...
  defp teln_expect(name, pid, data, pattern0, prx, opts) do
    haltPatterns0 = (case (get_ignore_prompt(opts)) do
                       true ->
    ...
    case (convert_pattern(haltPatterns0, false)) do
      {:ok, haltPatterns} ->
        {waitForPrompt, pattern1,
           opts1} = wait_for_prompt(pattern0, opts)
        seq = get_seq(opts1)
        case (convert_pattern(pattern1, seq)) do
          {:ok, pattern2} ->
            {idleTimeout, totalTimeout} = get_timeouts(opts1)
            promptCheck = get_prompt_check(opts1)
            eO = r_eo(teln_pid: pid, prx: prx,
                     idle_timeout: idleTimeout, total_timeout: totalTimeout,
                     seq: seq, haltpatterns: haltPatterns,
                     prompt_check: promptCheck)
            case (get_repeat(opts1)) do
              false ->
                case (teln_expect1(name, pid, data, pattern2, [],
                                     eO)) do
  ...
  defp convert_pattern(pattern0, seq)
      when pattern0 == [] or is_list(pattern0) and not
                                                   is_integer(hd(pattern0)) do
    pattern = (case (seq) do
                 true ->
                   pattern0
                 false ->
  ...
  defp wait_for_prompt2(prompt, pattern, opts) do
    pattern1 = (case (:lists.reverse(pattern)) do
                  [:prompt | _] ->
                    pattern
                  [{:prompt, _} | _] ->
                    pattern
    ...
    opts1 = (case (:lists.member(:sequence, opts)) do
               true ->
                 opts
               false ->
                 [:sequence | opts]
  ...
  defp repeat_expect(_Name, _Pid, rest, _Pattern, acc,
            r_eo(repeat: 0)) do
  ...
  defp repeat_expect(name, pid, data, pattern, acc, eO) do
    case (teln_expect1(name, pid, data, pattern, [], eO)) do
      {:ok, matched, rest} ->
        ...
        repeat_expect(name, pid, rest, pattern, [matched | acc],
                        eO1)
      {:halt, why, rest} ->
  ...
  defp teln_expect1(name, pid, data, pattern, acc,
            eO = r_eo(idle_timeout: idleTO, total_timeout: totalTO)) do
    eOMod = (cond do
               totalTO != :infinity ->
    ...
    expectFun = (case (r_eo(eOMod, :seq)) do
                   true ->
                     fn () ->
    ...
    case (expectFun.()) do
      ...
      notFinished ->
        ...
        {patOrPats1, acc1, rest1} = (case (notFinished) do
                                       {:nomatch, rest0} ->
                                         {pattern, [], rest0}
                                       {:continue, pats0, acc0, rest0} ->
                                         {pats0, acc0, rest0}
                                     end)
        case (:timer.tc(:ct_gen_conn, :do_within_time,
                          [fun, breakAfter])) do
          {_, {:error, reason}} ->
            cond do
              rest1 != [] ->
          ...
          {_, {:ok, data1}} when totalTO == :infinity ->
            teln_expect1(name, pid, rest1 ++ data1, patOrPats1,
                           acc1, eOMod)
          {elapsed, {:ok, data1}} ->
  ...
  defp seq_expect(name, pid, data, patterns, acc, eO)
      when r_eo(eO, :prompt_check) == false do
    seq_expect1(name, pid, data, patterns, acc, [],
                  r_eo(eO, found_prompt: false))
  end
  ...
  defp seq_expect1(name, pid, data, [:prompt | patterns], acc,
            rest, eO) do
    case (r_eo(eO, :found_prompt)) do
      ...
      promptType ->
        ...
        log(name_or_pid(name, pid), 'PROMPT: ~ts', [promptType])
        seq_expect(name, pid, rest, patterns,
                     [{:prompt, promptType} | acc], eO)
    end
  end

  defp seq_expect1(name, pid, data,
            [{:prompt, promptType} | patterns], acc, rest, eO) do
    case (r_eo(eO, :found_prompt)) do
      false ->
        lastLine = log_lines_not_last(name, pid, data)
        {:continue, [{:prompt, promptType} | patterns], acc,
           lastLine}
      ^promptType ->
        log_lines(name, pid, data)
        log(name_or_pid(name, pid), 'PROMPT: ~ts', [promptType])
      ...
      _OtherPromptType ->
        log_lines(name, pid, data)
        seq_expect(name, pid, rest,
                     [{:prompt, promptType} | patterns], acc, eO)
    end
  end

  defp seq_expect1(name, pid, data, [pattern | patterns], acc,
            rest, eO) do
    case (match_lines(name, pid, data, [pattern], eO)) do
      {:match, match, matchRest} ->
        seq_expect1(name, pid, matchRest, patterns,
                      [match | acc], rest, eO)
      {:nomatch, :prompt} ->
        seq_expect(name, pid, rest, [pattern | patterns], acc,
                     eO)
  ...
  defp match_lines(name, pid, data, patterns, eO) do
    ...
    case (one_line(data, [])) do
      {:noline, rest} when foundPrompt !== false ->
        case (match_line(name, pid, rest, patterns, foundPrompt,
                           false, eO)) do
          :nomatch ->
            {:nomatch, :prompt}
          {tag, match} ->
      ...
      {:noline, rest} when r_eo(eO, :prompt_check) == false ->
        case (match_line(name, pid, rest, patterns, false,
                           false, eO)) do
          :nomatch ->
            {:nomatch, rest}
          {tag, match} ->
      ...
      {:noline, rest} ->
        {:nomatch, rest}
      {line, rest} ->
        case (match_line(name, pid, line, patterns, false, true,
                           eO)) do
          :nomatch ->
  ...
  defp match_line(name, pid, line, patterns, foundPrompt,
            terminated, eO) do
    match_line(name, pid, line, patterns, foundPrompt,
                 terminated, eO, :match)
  end

  defp match_line(name, pid, line, [:prompt | patterns], false,
            term, eO, retTag) do
    match_line(name, pid, line, patterns, false, term, eO,
                 retTag)
  end

  defp match_line(name, pid, line, [:prompt | _Patterns],
            foundPrompt, _Term, _EO, retTag) do
    log(name_or_pid(name, pid), '       ~ts', [line])
    log(name_or_pid(name, pid), 'PROMPT: ~ts', [foundPrompt])
    {retTag, {:prompt, foundPrompt}}
  end

  defp match_line(name, pid, line,
            [{:prompt, promptType} | _Patterns], foundPrompt, _Term,
            _EO, retTag)
  ...
  defp match_line(name, pid, line,
            [{:prompt, promptType} | patterns], foundPrompt, term,
            eO, retTag)
      when promptType !== foundPrompt do
    match_line(name, pid, line, patterns, foundPrompt, term,
                 eO, retTag)
  end

  defp match_line(name, pid, line, [{tag, pattern} | patterns],
            foundPrompt, term, eO, retTag) do
    case (:re.run(line, pattern,
                    [{:capture, :all, :list}])) do
      :nomatch ->
        match_line(name, pid, line, patterns, foundPrompt, term,
                     eO, retTag)
  ...
  defp match_line(name, pid, line, [pattern | patterns],
            foundPrompt, term, eO, retTag) do
    case (:re.run(line, pattern,
                    [{:capture, :all, :list}])) do
      :nomatch ->
  ...
  defp match_line(name, pid, line, [], foundPrompt, term, eO,
            :match) do
    match_line(name, pid, line, r_eo(eO, :haltpatterns),
                 foundPrompt, term, eO, :halt)
  end

  defp match_line(name, pid, line, [], _FoundPrompt, true, _EO,
            :halt) do
    log(name_or_pid(name, pid), '       ~ts', [line])
    :nomatch
  end

  defp match_line(_Name, _Pid, _Line, [], _FoundPrompt, false,
            _EO, :halt) do
    :nomatch
  end
  ...
  defp teln_receive_until_prompt(pid, prx, acc, lastLine) do
    ...
    case (check_for_prompt(prx, lastLine ++ data)) do
      ...
      {:noprompt, lines, lastLine1} ->
        teln_receive_until_prompt(pid, prx, [lines | acc],
                                    lastLine1)
    end
  end
  ...
  defp match_prompt(str, prx, acc) do
    case (:re.run(str, prx, [:unicode])) do
      :nomatch ->
        :noprompt
      {:match, [{start, len}]} ->
        <<uptoPrompt :: size(start) - binary,
            prompt :: size(len) - binary, rest :: binary>> = str
        case (validate_prompt(start, uptoPrompt, prompt)) do
          :ok ->
            {:prompt,
  ...
  defp validate_prompt(size, prePrompt, prompt) do
    case (prePrompt) do
      <<_ :: size(size - 8) - binary, "PROMPT=\"", _ :: binary>> ->
        :recurse
      <<_ :: size(size - 5) - binary, _L :: size(8), "ast ",
          _ :: binary>>
          when prompt === "login: " ->
        :recurse
      _ ->
