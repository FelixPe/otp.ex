defmodule :m_ct_cover do
      ...
      _ ->
        ...
        case (:cover.start(nodes1)) do
          result = {:ok, startedNodes} ->
            :ct_logs.log('COVER INFO', 'Successfully added nodes to cover test: ~w', [startedNodes])
            result
          error ->
    ...
    case (:filelib.is_file(file)) do
      true ->
        case (:file.consult(file)) do
          {:ok, terms} ->
            import = (case (:lists.keysearch(:import, 1, terms)) do
                        {:value, {_, imps = [s | _]}} when is_list(s) ->
                          impsFN = :lists.map(fn f ->
            ...
            export = (case (:lists.keysearch(:export, 1, terms)) do
            ...
            nodes = (case (:lists.keysearch(:nodes, 1, terms)) do
                       {:value, {_, ns}} ->
                         ns
                       _ ->
                         []
  ...
  defp get_app_info(app = r_cover(app: :none),
            [{:incl_dirs, dirs} | terms], dir) do
    get_app_info(app, [{:incl_dirs, :none, dirs} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:incl_dirs, name, dirs} | terms], dir) do
    case (get_files(dirs, dir, '.beam', false, [])) do
      e = {:error, _} ->
        e
      mods1 ->
        mods = r_cover(app, :incl_mods)
        get_app_info(r_cover(app, incl_mods: mods ++ mods1), terms,
                       dir)
    end
  end

  defp get_app_info(app = r_cover(app: :none),
            [{:incl_dirs_r, dirs} | terms], dir) do
    get_app_info(app, [{:incl_dirs_r, :none, dirs} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:incl_dirs_r, name, dirs} | terms], dir) do
  ...
  defp get_app_info(app = r_cover(app: :none),
            [{:incl_mods, mods1} | terms], dir) do
    get_app_info(app, [{:incl_mods, :none, mods1} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:incl_mods, name, mods1} | terms], dir) do
    mods = r_cover(app, :incl_mods)
    get_app_info(r_cover(app, incl_mods: mods ++ mods1), terms,
                   dir)
  end
  ...
  defp get_app_info(app = r_cover(app: :none),
            [{:excl_dirs, dirs} | terms], dir) do
    get_app_info(app, [{:excl_dirs, :none, dirs} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:excl_dirs, name, dirs} | terms], dir) do
    case (get_files(dirs, dir, '.beam', false, [])) do
      e = {:error, _} ->
        e
      mods1 ->
        mods = r_cover(app, :excl_mods)
        get_app_info(r_cover(app, excl_mods: mods ++ mods1), terms,
                       dir)
    end
  end

  defp get_app_info(app = r_cover(app: :none),
            [{:excl_dirs_r, dirs} | terms], dir) do
    get_app_info(app, [{:excl_dirs_r, :none, dirs} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:excl_dirs_r, name, dirs} | terms], dir) do
    case (get_files(dirs, dir, '.beam', true, [])) do
      e = {:error, _} ->
        e
      mods1 ->
  ...
  defp get_app_info(app = r_cover(app: :none),
            [{:excl_mods, mods1} | terms], dir) do
    get_app_info(app, [{:excl_mods, :none, mods1} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:excl_mods, name, mods1} | terms], dir) do
    mods = r_cover(app, :excl_mods)
    get_app_info(r_cover(app, excl_mods: mods ++ mods1), terms,
                   dir)
  end

  defp get_app_info(app = r_cover(app: :none), [{:cross, cross} | terms],
            dir) do
    get_app_info(app, [{:cross, :none, cross} | terms], dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:cross, name, cross1} | terms], dir) do
    cross = r_cover(app, :cross)
    get_app_info(r_cover(app, cross: cross ++ cross1), terms, dir)
  end

  defp get_app_info(app = r_cover(app: :none),
            [{:src_dirs, dirs} | terms], dir) do
    get_app_info(app, [{:src_dirs, :none, dirs} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:src_dirs, name, dirs} | terms], dir) do
  ...
  defp get_app_info(app = r_cover(app: :none),
            [{:src_dirs_r, dirs} | terms], dir) do
    get_app_info(app, [{:src_dirs_r, :none, dirs} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:src_dirs_r, name, dirs} | terms], dir) do
  ...
  defp get_app_info(app = r_cover(app: :none),
            [{:src_files, src1} | terms], dir) do
    get_app_info(app, [{:src_files, :none, src1} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:src_files, name, src1} | terms], dir) do
    src = r_cover(app, :src)
    get_app_info(r_cover(app, src: src ++ src1), terms, dir)
  end

  defp get_app_info(app = r_cover(app: :none),
            [{:local_only, bool} | terms], dir) do
    get_app_info(app, [{:local_only, :none, bool} | terms],
                   dir)
  end

  defp get_app_info(app = r_cover(app: name),
            [{:local_only, name, bool} | terms], dir) do
    ...
    case (:file.list_dir(dirAbs)) do
      {:ok, entries} ->
        ...
        cond do
          ...
          true ->
            files1 = get_files(subDirs, rootDir, ext, recurse,
                                 files ++ matches)
  ...
  defp remove_excludes_and_dups(coverData = r_cover(excl_mods: excl,
                          incl_mods: incl)) do
    incl1 = (for mod <- incl,
                   :lists.member(mod, excl) == false do
               mod
             end)
    incl2 = :lists.sort(:lists.foldl(fn m, l ->
  ...
  defp files2mods(info = r_cover(excl_mods: exclFs, incl_mods: inclFs,
                     cross: cross)) do
    r_cover(info, excl_mods: files2mods1(exclFs), 
              incl_mods: files2mods1(inclFs), 
              cross: for {tag, fs} <- cross do
                       {tag, files2mods1(fs)}
                     end)
  end
