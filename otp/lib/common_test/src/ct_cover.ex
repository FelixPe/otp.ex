defmodule :m_ct_cover do
  ...
  def add_nodes(nodes) do
    case :erlang.whereis(:cover_server) do
      ...
      _ ->
        ...
        :ct_logs.log(~c"COVER INFO", ~c"Adding nodes to cover test: ~w", [nodes1])

        case :cover.start(nodes1) do
          result = {:ok, startedNodes} ->
            :ct_logs.log(~c"COVER INFO", ~c"Successfully added nodes to cover test: ~w", [
              startedNodes
            ])
          ...
          error ->
            :ct_logs.log(~c"COVER INFO", ~c"Failed to add nodes to cover test: ~tp", [error])
            error
        end
    end
  end
  ...
  def remove_nodes(nodes) do
    case :erlang.whereis(:cover_server) do
      ...
      _ ->
        ...
        :ct_logs.log(~c"COVER INFO", ~c"Removing nodes from cover test: ~w", [toRemove])

        case :cover.stop(toRemove) do
          :ok ->
            :ct_logs.log(~c"COVER INFO", ~c"Successfully removed nodes from cover test.", [])
            :ok

          error ->
            :ct_logs.log(~c"COVER INFO", ~c"Failed to remove nodes from cover test: ~tp", [error])
            error
        end
    end
  end
  ...
  defp get_app_info(app = r_cover(app: name), [{:incl_dirs, name, dirs} | terms], dir) do
    case get_files(dirs, dir, ~c".beam", false, []) do
      e = {:error, _} ->
        e
  ...
  defp get_app_info(app = r_cover(app: name), [{:incl_dirs_r, name, dirs} | terms], dir) do
    case get_files(dirs, dir, ~c".beam", true, []) do
      e = {:error, _} ->
        e
  ...
  defp get_app_info(app = r_cover(app: name), [{:src_dirs, name, dirs} | terms], dir) do
    case get_files(dirs, dir, ~c".erl", false, []) do
      e = {:error, _} ->
        e
  ...
  defp get_app_info(app = r_cover(app: name), [{:src_dirs_r, name, dirs} | terms], dir) do
    case get_files(dirs, dir, ~c".erl", true, []) do
      e = {:error, _} ->
        e
