defmodule :m_ct_telnet_client do
  ...
  defp init(parent, server, port, timeout, keepAlive, noDelay, connName) do
    ...
    case :gen_tcp.connect(server, port, [:list, {:packet, 0}, {:nodelay, noDelay}], timeout) do
      {:ok, sock} ->
        dbg(~c"~tp connected to: ~tp (port: ~w, keep_alive: ~w)\n", [
          connName,
          server,
          port,
          keepAlive
  ...
  defp loop(state, sock, acc) do
    receive do
      {:tcp_closed, _} ->
        dbg(~c"Connection closed\n", [])
        data = :lists.reverse(:lists.append(acc))
        dbg(~c"Printing queued messages: ~tp", [data])

        :ct_telnet.log(r_state(state, :conn_name), :general_io, ~c"~ts", [
    ...
    after
      wait(r_state(state, :keep_alive), 8000) ->
        ...
        case data do
          ...
          _ ->
            dbg(~c"idle timeout, printing ~tp\n", [data])
            len = length(data)
  ...
  defp send(data, sock, connName) do
    case data do
      [255 | _] = cmd ->
        cmd_dbg(~c"Sending", cmd)

        try do
          :io_lib.format(~c"[~w] ~w", [:ct_telnet_client, data])
        catch
          _, _ ->
            :ok
        else
      ...
      _ ->
        ...
        try do
          :io_lib.format(~c"[~w] ~ts", [:ct_telnet_client, data])
        catch
          _, _ ->
            :ok
        else
  ...
  defp respond_cmd([251, 3], _Sock) do
    dbg(~c"Server will suppress-go-ahead\n", [])
  end

  defp respond_cmd([252, 3], _Sock) do
    dbg(~c"Warning! Server won't suppress-go-ahead\n", [])
  end
  ...
  defp respond_cmd([cmd | opt], _Sock)
       when cmd >= 240 and
              cmd <= 255 do
    dbg(~c"Received cmd: ~w. Ignored!\n", [[cmd | opt]])
  end

  defp respond_cmd([cmd | opt], _Sock) do
    dbg(~c"WARNING: Received unknown cmd: ~w. Ignored!\n", [[cmd | opt]])
  end
