defmodule :m_ct_telnet_client do
  use Bitwise
  require Record
  Record.defrecord(:r_state, :state, conn_name: :undefined,
                                 get_data: :undefined, keep_alive: true,
                                 log_pos: 1)
  ...
  def open(server, port, timeout, keepAlive, noDelay,
           connName) do
    self = self()
    {pid, mRef} = spawn_monitor(fn () ->
  ...
  defp init(parent, server, port, timeout, keepAlive,
            noDelay, connName) do
    :ct_util.mark_process()
    case (:gen_tcp.connect(server, port,
                             [:list, {:packet, 0}, {:nodelay, noDelay}],
                             timeout)) do
      {:ok, sock} ->
        ...
        loop(r_state(conn_name: connName, get_data: 10,
    ...
    receive do
      {:tcp_closed, _} ->
        ...
        :ct_telnet.log(r_state(state, :conn_name), :general_io, '~ts',
                         [:lists.sublist(data, r_state(state, :log_pos),
                                           length(data))])
        receive do
          {:get_data, pid} ->
            send(pid, :closed)
      ...
      {:get_data_delayed, pid} ->
        ...
        {newAcc, pos} = (case (:erlang.is_process_alive(pid)) do
                           true when acc != [] ->
                             data = :lists.reverse(:lists.append(acc))
                             len = length(data)
                             dbg('get_data_delayed ~tp\n', [data])
                             :ct_telnet.log(r_state(state, :conn_name), :general_io,
                                              '~ts',
                                              [:lists.sublist(data,
                                                                r_state(state, :log_pos),
                                                                len)])
      ...
      {:close, pid} ->
        ...
        :gen_tcp.close(sock)
        send(pid, :closed)
    after wait(r_state(state, :keep_alive), 8000) ->
      dbg('idle timeout\n', [])
      data = :lists.reverse(:lists.append(acc))
      case (data) do
        [] ->
  ...
  defp respond_cmd([cmd | opt], _Sock) when (cmd >= 240 and
                                      cmd <= 255) do
  ...
  defp get_cmd([cmd | rest]) when (cmd >= 240 and
                                cmd <= 249) do
    {241, rest}
  end

  defp get_cmd([cmd, opt | rest]) when (cmd >= 251 and
                                     cmd <= 254) do
    {[cmd, opt], rest}
  end
