defmodule :m_test_server do
  ...
  def cover_compile(coverInfo = r_cover(app: :none, incl: include, cross: cross)) do
    ...
    case length(compileMods) do
      0 ->
        :io.fwrite(~c"WARNING: No modules to cover compile!\n\n", [])
        {:ok, _} = start_cover()
        {:ok, r_cover(coverInfo, mods: [])}

      n ->
        :io.fwrite(~c"Cover compiling ~w modules - this may take some time... ", [n])
  ...
  def cover_compile(coverInfo = r_cover(app: app, excl: :all, incl: include, cross: cross)) do
    ...
    case length(compileMods) do
      ...
      n ->
        :io.fwrite(~c"Cover compiling '~w' (~w files) - this may take some time... ", [app, n])
    ...
    case :code.lib_dir(app) do
      {:error, :bad_name} ->
        case include ++ crossMods do
          ...
          compileMods ->
            :io.fwrite(~c"Cover compiling '~w' (~w files) - this may take some time... ", [
              app,
              length(compileMods)
            ])
      ...
      libDir ->
        ebinDir = :filename.join([libDir, ~c"ebin"])
        wC = :filename.join(ebinDir, ~c"*.beam")
  ...
  def cover_analyse(
    ...
    :io.fwrite(:user, ~c"Cover analysing... ", [])

    {aTFOk, aTFFail} =
      case analyse do
        :details ->
          case :cover.export(:filename.join(dir, ~c"all.coverdata")) do
            :ok ->
              {:result, ok1, fail1} =
  ...
  def run_test_case_apply({caseNum, mod, func, args, name, runInit, timetrapData}) do
    mC =
      case {func, memory_checker()} do
        ...
        {_, :valgrind} ->
          valgrind_format(~c"Test case #~w ~w:~w/1", [caseNum, mod, func])

          :os.putenv(
            ~c"VALGRIND_LOGFILE_INFIX",
            :erlang.atom_to_list(mod) ++ ~c"." ++ :erlang.atom_to_list(func) ++ ~c"-"
          )
        ...
        {_, :asan} ->
          case asan_take_logpath() do
            ...
            {logPath, otherOpts} ->
              ...
              [exe, app | _] = :string.lexemes(logFile, ~c"-")
              ...
              newOpts =
                asan_make_opts([
                  ~c"log_path=" ++ newLogPath ++ ~c".subnode"
                  | otherOpts
                ])

              :os.putenv(~c"ASAN_OPTIONS", newOpts)
          end
  ...
  defp run_test_case_apply(mod, func, args, name, runInit, timetrapData) do
    print_timestamp(:minor, ~c"Started at ")
    print(:minor, ~c"", [], :internal_raw)
  ...
  defp run_test_case_msgloop(r_st(ref: ref, pid: pid, end_conf_pid: endConfPid0) = st0) do
    receive do
        ...
        st =
          ...
          after
            10000 ->
              ...
              error = :lists.flatten(:io_lib.format(~c"Aborted: ~tp", [reason]))
      ...
      {:EXIT, _FwCallPid, {:fw_notify_done, func, error}} ->
        cB = :os.getenv(~c"TEST_SERVER_FRAMEWORK")
        ...
        st =
          setup_termination(
            retVal,
            r_st(st0, comment: ~c"Framework error", config: :undefined)
          )
    ...
    endConfProc = fn ->
      ...
      endConfApply = fn ->
        ...
        try do
          ...
          _, error ->
            :timer.sleep(1)
            print_end_conf_result(mod, func, conf, ~c"crashed", error)
        else
          _ ->
            :ok
        end

        send(supervisor, {self(), :end_conf, endConf})
      end
      ...
      receive do
        {^pid, :end_conf, endConf} ->
          send(starter, {self(), {:call_end_conf, data, endConf, :ok}})

        {:EXIT, ^pid, reason} ->
          print_end_conf_result(mod, func, conf, ~c"failed", reason)
  ...
  defp print_end_conf_result(mod, func, conf, cause, error) do
    str2Print = fn
      ...
      when noHTML == :stdout or
             noHTML == :major ->
        :io_lib.format(~c"WARNING! ~w:end_per_testcase(~tw, ~tp) ~s!\n\tReason: ~tp\n", [
          mod,
          func,
          conf,
          cause,
  ...
  defp spawn_fw_call(mod, iPTC = {:init_per_testcase, func}, currConf, pid, why, loc, sendTo) do
    fwCall = fn ->
      ...
      send(
        :erlang.group_leader(),
        {:printout, 12, ~c"ERROR! ~w:init_per_testcase(~tw, ~tp) failed!\n\tReason: ~tp\n",
        ...
        case why do
          {:timetrap_timeout, tVal} ->
            send(
              :erlang.group_leader(),
              {:printout, 12,
               ~c"WARNING! ~w:end_per_testcase(~tw, ~tp) failed!\n\tReason: timetrap timeout after ~w ms!\n",
               [mod, func, endConf, tVal]}
            )

            w = ~c"<font color=\"red\">WARNING: end_per_testcase timed out!</font>"
            {tVal / 1000, w}

          _ ->
            send(
              :erlang.group_leader(),
              {:printout, 12, ~c"WARNING! ~w:end_per_testcase(~tw, ~tp) failed!\n\tReason: ~tp\n",
               [mod, func, endConf, why]}
            )

            w = ~c"<font color=\"red\">WARNING: end_per_testcase failed!</font>"
            {:died, w}
        end
      ...
      comment =
        case warnOrError do
          :warn ->
            send(
              :erlang.group_leader(),
              {:printout, 12, ~c"WARNING! ~w:~tw(~w,~tw,...) failed!\n    Reason: ~tp\n",
               [fwMod, fwFunc, a1, a2, fwError]}
            )

            :lists.flatten(
              :io_lib.format(
                ~c"<font color=\"red\">WARNING! ~w:~tw(~w,~tw,...) failed!</font>",
                [fwMod, fwFunc, a1, a2]
              )
            )
          ...
          :error ->
            send(
              :erlang.group_leader(),
              {:printout, 12, ~c"Error! ~w:~tw(~w,~tw,...) failed!\n    Reason: ~tp\n",
               [fwMod, fwFunc, a1, a2, fwError]}
            )

            :lists.flatten(
              :io_lib.format(
                ~c"<font color=\"red\">ERROR! ~w:~tw(~w,~tw,...) failed!</font>",
                [fwMod, fwFunc, a1, a2]
              )
            )
        end
  ...
  defp do_end_tc_call1(mod, func, res, return) do
    fwMod = :os.getenv(~c"TEST_SERVER_FRAMEWORK")
    ref = make_ref()

    cond do
      fwMod == ~c"ct_framework" or fwMod == ~c"undefined" or fwMod == false ->
  ...
  defp do_init_per_testcase(mod, args) do
    try do
      ...
      other ->
        ...
        line = get_loc()
        print_init_conf_result(line, ~c"thrown", other)
      ...
      _, reason0 ->
        ...
        set_loc(__STACKTRACE__)
        line = get_loc()
        print_init_conf_result(line, ~c"crashed", reason)
        ...
        case :lists.filter(
          ...
          bad ->
            send(
              :erlang.group_leader(),
              {:printout, 12,
               ~c"ERROR! init_per_testcase has returned bad elements in Config: ~tp\n", [bad]}
            )
      ...
      _Other ->
        send(
          :erlang.group_leader(),
          {:printout, 12, ~c"ERROR! init_per_testcase did not return a Config list.\n", []}
        )
  ...
  defp print_init_conf_result(line, cause, reason) do
    ...
    str2Print = fn
      ...
      when noHTML == :stdout or
             noHTML == :major ->
        :io_lib.format(~c"ERROR! init_per_testcase ~s!\n\tLocation: ~tp\n\tReason: ~tp\n", [
          cause,
          line,
          reason
        ])
  ...
  defp do_end_per_testcase(mod, endFunc, func, conf) do
    ...
    try do
      ...
      other ->
        comment0 =
          case read_comment() do
            ...
            cmt ->
              cmt ++ :test_server_ctrl.xhtml(~c"<br>", ~c"<br />")
          end

        set_loc(__STACKTRACE__)

        comment(
          :io_lib.format(~c"~ts<font color=\"red\">WARNING: ~w thrown!</font>\n", [
            comment0,
            endFunc
          ])
        )

        print_end_tc_warning(endFunc, other, ~c"thrown", get_loc())
      ...
      class, reason ->
        ...
        comment(
          :io_lib.format(~c"~ts<font color=\"red\">WARNING: ~w crashed!</font>\n", [
            comment0,
            endFunc
          ])
        )

        print_end_tc_warning(endFunc, reason, ~c"crashed", get_loc())
  ...
  def lookup_config(key, config) do
    case :lists.keysearch(key, 1, config) do
      ...
      _ ->
        :io.format(~c"Could not find element ~tp in Config.~n", [key])
        :undefined
    end
  end
  ...
  defp cast_to_list(x) do
    :lists.flatten(:io_lib.format(~c"~tp", [x]))
  end
  ...
  def break(cBM, testCase, comment) do
    ...
    {tCName, cntArg, pName} =
      cond do
        testCase == :"" ->
          {~c"", ~c"", :test_server_break_process}

        true ->
          str = :erlang.atom_to_list(testCase)
          {[32 | str], str, :erlang.list_to_atom(~c"test_server_break_process_" ++ str)}
      end
  ...
  def continue(testCase) when is_atom(testCase) do
    pName = :erlang.list_to_atom(~c"test_server_break_process_" ++ :erlang.atom_to_list(testCase))
      ...
      _ ->
        case :erlang.get(:test_server_default_timetrap) do
          ...
          garbage ->
            ...
            format(~c"=== WARNING: garbage in test_server_default_timetrap: ~tp~n", [garbage])
        end
        ...
        format(~c"=== test_server setting default timetrap of ~p seconds~n", [dTmo])
  ...
  defp cancel_default_timetrap(true) do
    case :erlang.get(:test_server_default_timetrap) do
      ...
      timeTrap when is_pid(timeTrap) ->
        ...
        :erlang.erase(:test_server_default_timetrap)
        format(~c"=== test_server canceled default timetrap since another timetrap was set~n")
        :ok
  ...
  defp time_ms({other, _N}, _, _) do
    format(
      ~c"=== ERROR: Invalid time specification: ~tp. Should be seconds, minutes, or hours.~n",
      [other]
    )
    ...
    after
      5000 ->
        ...
        cond do
          ...
          true ->
            format(~c"=== ERROR: User timetrap execution failed!", [])
            :ignore
        end
    end
  end
    ...
    :lists.foreach(
      fn n ->
        case firstname(n) do
          ^name ->
            format(
              ~c"=== WARNING: Trying to start node '~w' when node with same first name exists: ~w",
              [name, n]
            )
  ...
  def stop_node(slave) do
    ...
    case result do
      :ok ->
        ...
        receive do
          {:nodedown, ^slave} ->
            format(:minor, ~c"Stopped slave node: ~w", [slave])
            format(:major, ~c"=node_stop     ~w", [slave])
        ...
        after
          30000 ->
            format(~c"=== WARNING: Node ~w does not seem to terminate.", [slave])
      ...
      {:error, _Reason} ->
        format(
          ~c"=== WARNING: Attempt to stop a nonexisting slavenode (~w)~n===          Trying to kill it anyway!!!",
          [slave]
        )
  ...
  def find_release(release) do
    send(
      :erlang.group_leader(),
      {:sync_apply, self(), {:test_server_ctrl, :find_release, [release]}}
    )
  ...
  def peer_name(module, testCase) do
    :peer.random_name(:lists.concat([module, ~c"-", testCase]))
  end

  def start_peer(args, module, testCase) when is_list(args) do
    start_peer(
      %{args: args, name: peer_name(module, testCase)},
      module
    )
  end
  ...
  def start_peer(opts, module, testCase) do
    start_peer(
      Map.put(opts, :name, peer_name(module, testCase)),
      module
    )
  end
  ...
  def start_peer(opts, module, testCase, release, outDir) do
    case find_release(release) do
      ...
      erl ->
        env = :maps.get(:env, opts, []) ++ [{~c"ERL_AFLAGS", false}]

        newArgs = [
          ~c"-pa",
          peer_compile(erl, :code.which(:peer), outDir)
          | :maps.get(:args, opts, [])
        ]

        start_peer(
          Map.merge(opts, %{exec: erl, args: newArgs, env: env, start_cover: false}),
          module,
          testCase
        )
    end
  ...
  defp start_peer(%{name: name} = opts, module) do
    ...
    crashFile = :filename.join([crashDir, :lists.concat([~c"erl_crash_dump.", name])])
    args = :maps.get(:args, opts, [])

    cookieArg =
      case :lists.member(~c"-setcookie", args) do
        false ->
    ...
    fullArgs =
      cookieArg ++
        [~c"-pa", :filename.dirname(:code.which(module)), ~c"-env", ~c"ERL_CRASH_DUMP", crashFile] ++
        args

    case :maps.get(:start_cover, opts, true) and :test_server.is_cover() do
      true ->
        coverMain = :cover.get_main_node()
        shutdown = :erlang.binary_to_term(:erlang.term_to_binary({10000, coverMain}))

        case :peer.start_link(Map.merge(opts, %{args: fullArgs, shutdown: shutdown})) do
          {:ok, peer, node} ->
            {:ok, peer, node}
  ...
  defp peer_compile(erl, modPath, outDir) do
    ...
    erlc = :filename.join(:filename.dirname(erl), ~c"erlc")

    cmd(
      erlc,
      [~c"-o", outDir, :unicode.characters_to_binary(modSrc)]
    )
  ...
  defp cmd(exec, args) do
    env = [{~c"ERL_AFLAGS", false}, {~c"ERLC_USE_SERVER", false}]

    port =
      :erlang.open_port(
        {:spawn_executable, exec},
        [{:args, args}, {:env, env}, :stream, :binary, :exit_status, :stderr_to_stdout]
      )

    read_std(port, :lists.join(~c" ", [exec | args]), <<>>)
  end
  ...
  def run_on_shielded_node(fun, cArgs)
    ...
    name = ~c"shielded_node-" ++ :erlang.integer_to_list(nr)

    node =
      case start_node(name, :slave, [{:args, ~c"-hidden " ++ cArgs}]) do
        {:ok, n} ->
          n
  ...
  defp valgrind_format(format, args) do
    try do
      :erlang.system_info({:memory_checker, :print, :io_lib.format(format, args)})
    catch
      :error, e -> {:EXIT, {e, __STACKTRACE__}}
      :exit, e -> {:EXIT, e}
      e -> e
  ...
  defp asan_take_logpath() do
    case :os.getenv(~c"ASAN_OPTIONS") do
      false ->
        false

      s ->
        opts = :string.lexemes(s, ~c":")
        asan_take_logpath_loop(opts, [])
    end
  end

  defp asan_take_logpath_loop([~c"log_path=" ++ logPath | t], acc) do
    {logPath, t ++ acc}
  end
