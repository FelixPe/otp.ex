defmodule :m_test_server do
  ...
  def init_target_info() do
    ...
    r_target_info(os_family: :test_server_sup.get_os_family(),
        os_type: :os.type(),
        version: :erlang.system_info(:version),
        system_version: :erlang.system_info(:system_version),
        root_dir: :code.root_dir(), emulator: emu,
        otp_release: oTPRel,
        username: :test_server_sup.get_username(),
        cookie: :erlang.atom_to_list(:erlang.get_cookie()))
  end

  def init_memory_checker() do
    check_memory_leaks()
  end

  def cover_compile(coverInfo = r_cover(app: :none, incl: include,
                         cross: cross)) do
    crossMods = :lists.flatmap(fn {_, m} ->
                                    m
                               end,
                                 cross)
  ...
  def cover_compile(coverInfo = r_cover(app: app, excl: :all,
                         incl: include, cross: cross)) do
    crossMods = :lists.flatmap(fn {_, m} ->
                                    m
                               end,
                                 cross)
  ...
  def cover_compile(coverInfo = r_cover(app: app, excl: exclude,
                         incl: include, cross: cross)) do
    crossMods = :lists.flatmap(fn {_, m} ->
                                    m
                               end,
    ...
    case (:code.lib_dir(app)) do
      {:error, :bad_name} ->
        case (include ++ crossMods) do
          ...
          compileMods ->
            :io.fwrite('Cover compiling \'~w\' (~w files) - this may take some time... ', [app, length(compileMods)])
  ...
  def cover_analyse(dir,
           r_cover(level: analyse, mods: modules, stop: stop)) do
    :io.fwrite(:user, 'Cover analysing... ', [])
    {aTFOk, aTFFail} = (case (analyse) do
                          :details ->
                            case (:cover.export(:filename.join(dir, 'all.coverdata'))) do
                              :ok ->
                                {:result, ok1,
                                   fail1} = :cover.analyse_to_file(modules,
                                                                     [{:outdir,
                                                                         dir},
                                                                          :html])
                                {:lists.map(fn outFile ->
                                                 m = :erlang.list_to_atom(:filename.basename(:filename.rootname(outFile,
                                                                                                                  '.COVER.html')))
                                                 {m, {:file, outFile}}
                                            end,
                                              ok1),
                                   :lists.map(fn {reason, m} ->
                                                   {m, {:error, reason}}
                                              end,
                                                fail1)}
                              error ->
  ...
  def run_test_case_apply({caseNum, mod, func, args, name, runInit,
            timetrapData}) do
    mC = (case ({func, memory_checker()}) do
            {:init_per_suite, _} ->
              :none
            {:init_per_group, _} ->
              :none
            ...
            {_, :valgrind} ->
              ...
              :os.putenv('VALGRIND_LOGFILE_INFIX',
                           :erlang.atom_to_list(mod) ++ '.' ++ :erlang.atom_to_list(func) ++ '-')
              :valgrind
            {_, :asan} ->
              case (asan_take_logpath()) do
                false ->
                  false
                {logPath, otherOpts} ->
                  ...
                  _PrevLog = :erlang.system_info({:memory_checker, :log,
                                                    newLogPath})
    ...
    result = run_test_case_apply(mod, func, args, name,
  ...
  Record.defrecord(:r_st, :st, ref: :undefined,
                              pid: :undefined, mf: :undefined,
                              last_known_loc: :undefined, status: :undefined,
                              ret_val: :undefined, comment: :undefined,
                              timeout: :undefined, config: :undefined,
  ...
  defp run_test_case_apply(mod, func, args, name, runInit, timetrapData) do
    ...
    ref = make_ref()
    pid = spawn_link(run_test_case_eval_fun(mod, func, args,
                                              name, ref, runInit, timetrapData,
                                              logOpts, tCCallback))
    :erlang.put(:test_server_detected_fail, [])
    st = r_st(ref: ref, pid: pid, mf: {mod, func},
             last_known_loc: :unknown, status: :starting,
             ret_val: [], comment: '', timeout: :infinity,
             config: hd(args))
    :ct_util.mark_process()
    run_test_case_msgloop(st)
  end

  defp run_test_case_msgloop(r_st(ref: ref, pid: pid,
              end_conf_pid: endConfPid0) = st0) do
    receive do
      {:set_tc_state = tag, from, {status, config0}} ->
        config = (case (config0) do
                    :unknown ->
                      r_st(st0, :config)
                    _ ->
                      config0
      ...
      {:abort_current_testcase, reason, from} ->
        line = (case (:erlang.is_process_alive(pid)) do
                  true ->
                    get_loc(pid)
                  false ->
                    :unknown
        ...
        st = (receive do
              ...
              after 10000 ->
                ...
                error1 = :lists.flatten(for s <- :string.lexemes(error,
                                                                   [?\n]) do
      ...
      {:read_comment, from} ->
        ...
        run_test_case_msgloop(st0)
      {:make_priv_dir, from} ->
        config = (case (r_st(st0, :config)) do
                    :undefined ->
                      []
                    config0 ->
                      config0
                  end)
        result = (case (:proplists.get_value(:priv_dir,
                                               config)) do
                    :undefined ->
      ...
      {:EXIT, ^pid, reason} ->
        st = (case (reason) do
                ...
                {what,
                   [details, loc0 = {_M, _F, a, [{:file, _} | _]} | _]}
                    when is_integer(a) ->
                  loc = rewrite_loc_item(loc0)
                  handle_tc_exit({what, details},
        ...
        run_test_case_msgloop(st)
      {^endConfPid0,
         {:call_end_conf, data, endConf, _Result}} ->
        r_st(mf: {mod, func}, config: currConfFromState) = st0
        currConf = (case (endConf) do
                      [] ->
                        currConfFromState
                      _ ->
                        endConf
        ...
        case (currConf) do
          _ when is_list(currConf) ->
            ...
            spawn_fw_call(mod, func, currConf, tCPid, tCExitReason,
                            loc, self())
            st = r_st(st0, config: :undefined, 
                          end_conf_pid: :undefined)
            run_test_case_msgloop(st)
          _ ->
            run_test_case_msgloop(st0)
        end
      {_FwCallPid, :fw_notify_done,
         {t, value, loc, opts, addToComment}} ->
        retVal = {t, value, loc, opts}
        comment0 = r_st(st0, :comment)
        comment = (case (addToComment) do
                     :undefined ->
                       comment0
                     _ ->
                       cond do
                         comment0 === '' ->
                           addToComment
                         true ->
                           comment0 ++ :test_server_ctrl.xhtml('<br>',
                                                                 '<br />') ++ addToComment
                       end
                   end)
        st = setup_termination(retVal,
      ...
      {:failed, file, line} ->
        ...
        run_test_case_msgloop(st0)
      {:user_timetrap, ^pid, _TrapTime, startTime,
         e = {:user_timetrap_error, _}, _} ->
        ...
        run_test_case_msgloop(st0)
      {:user_timetrap, ^pid, trapTime, startTime, elapsedTime,
         scale} ->
        case (update_user_timetraps(pid, startTime)) do
          :proceed ->
            totalTime = (cond do
  ...
  defp handle_tc_exit(:killed, st) do
    ...
    spawn_fw_call(mod, func, config, pid, msg, :unknown,
                    self())
    st
  end

  defp handle_tc_exit({:testcase_aborted,
             {:user_timetrap_error, _} = msg, _},
            st) do
    r_st(config: config, mf: {mod, func}, pid: pid) = st
    spawn_fw_call(mod, func, config, pid, msg, :unknown,
                    self())
  ...
  defp handle_tc_exit(reason,
            r_st(status: {:framework, {fwMod, fwFunc, _} = fwMFA},
                config: config, mf: {mod, func}, pid: pid) = st) do
    r = (case (reason) do
    ...
    spawn_fw_call(mod, func, config, pid, {error, fwMFA},
                    :unknown, self())
    st
  end

  defp handle_tc_exit(reason,
            r_st(status: :tc, config: config0, mf: {mod, func},
                pid: pid) = st)
      when is_list(config0) do
    {r, loc1, f} = (case (reason) do
    ...
    endConfPid = call_end_conf(mod, func, pid, r, loc1,
                                 config, timeout)
    r_st(st, end_conf_pid: endConfPid)
  end

  defp handle_tc_exit(reason,
            r_st(config: config, mf: {mod, func0}, pid: pid,
                status: status) = st) do
    {r, loc1} = (case (reason) do
                   {:timetrap_timeout = e, tVal, loc0} ->
                     {{e, tVal}, loc0}
                   {:testcase_aborted = e, abortReason, loc0} ->
                     {{e, abortReason}, loc0}
  ...
  defp end_conf_timeout(_, r_st(config: config)) when is_list(config) do
    :proplists.get_value(:default_timeout, config,
                           60 * 1000)
  end
  ...
  defp call_end_conf(mod, func, tCPid, tCExitReason, loc, conf,
            tVal) do
    starter = self()
    data = {mod, func, tCPid, tCExitReason, loc}
    case (:erlang.function_exported(mod, :end_per_testcase,
                                      2)) do
      false ->
        spawn_link(fn () ->
      ...
      true ->
        do_call_end_conf(starter, mod, func, data, tCExitReason,
                           conf, tVal)
    end
  end

  defp do_call_end_conf(starter, mod, func, data, tCExitReason, conf,
            tVal) do
    endConfProc = fn () ->
                       ...
                       endConfApply = fn () ->
                                           _ = timetrap(tVal)
                                           endConf = (case (do_init_tc_call(mod,
                                                                              {:end_per_testcase,
                                                                                 func},
                                                                              [conf],
                                                                              {tCExitReason,
                                           ...
                                           try do
                                             apply(mod, :end_per_testcase,
                                                     [func, endConf])
                                           catch
                                             _, error ->
                                               :timer.sleep(1)
                                               print_end_conf_result(mod, func,
                                                                       conf, 'crashed',
                                                                       error)
                                           else
                                             _ ->
  ...
  defp print_end_conf_result(mod, func, conf, cause, error) do
    str2Print = fn noHTML when noHTML == :stdout or
                                 noHTML == :major
                               ->
  ...
  defp spawn_fw_call(mod, iPTC = {:init_per_testcase, func},
            currConf, pid, why, loc, sendTo) do
    fwCall = fn () ->
                  :ct_util.mark_process()
                  skip = {:skip,
                  ...
                  send(:erlang.group_leader(), {:printout, 12, 'ERROR! ~w:init_per_testcase(~tw, ~tp) failed!\n\tReason: ~tp\n',
                                                  [mod, func, currConf, why]})
                  send(sendTo, {self(), :fw_notify_done,
                                  {time, skip, loc, [], :undefined}})
             end
    spawn_link(fwCall)
  end

  defp spawn_fw_call(mod, ePTC = {:end_per_testcase, func}, endConf,
            pid, why, _Loc, sendTo) do
    fwCall = fn () ->
                  :ct_util.mark_process()
                  {retVal,
                     report} = (case (:proplists.get_value(:tc_status,
                                                             endConf)) do
                                  :undefined ->
                  ...
                  {time, warn} = (case (why) do
                                    {:timetrap_timeout, tVal} ->
                                      send(:erlang.group_leader(), {:printout,
                                                                      12, 'WARNING! ~w:end_per_testcase(~tw, ~tp) failed!\n\tReason: timetrap timeout after ~w ms!\n',
                                                                      [mod,
                                                                           func,
                                                                               endConf,
                                                                                   tVal]})
                  ...
                  {retVal1, failLoc} = (try do
                                          do_end_tc_call(mod, ePTC,
                                                           {pid, report,
                                                              [endConf]},
                                                           why)
                                        catch
                                          _, fwEndTCErr ->
                                            exit({:fw_notify_done, :end_tc,
                                                    fwEndTCErr})
                                        else
                                          ^why ->
                                            {retVal, failLoc0}
  ...
  defp spawn_fw_call(mod, func, conf, pid,
            {{:framework_error, fwError},
               {fwMod, fwFunc, [a1, a2 | _]} = fwMFA},
            _, sendTo) do
    fwCall = fn () ->
                  :ct_util.mark_process()
                  time = (case (fwError) do
                            {:timetrap, tVal} ->
                              tVal / 1000
                            _ ->
                              :died
                          end)
                  {ret, loc, warnOrError} = cleanup_after_fw_error(mod,
                                                                     func, conf,
                                                                     pid,
                                                                     fwError,
                                                                     fwMFA)
                  comment = (case (warnOrError) do
                               :warn ->
                                 send(:erlang.group_leader(), {:printout, 12, 'WARNING! ~w:~tw(~w,~tw,...) failed!\n    Reason: ~tp\n',
                                                                 [fwMod, fwFunc,
                                                                             a1,
                                                                                 a2,
                                                                                     fwError]})
                                 :lists.flatten(:io_lib.format('<font color="red">WARNING! ~w:~tw(~w,~tw,...) failed!</font>',
                                                                 [fwMod, fwFunc,
                                                                             a1,
                                                                                 a2]))
                               :error ->
                                 send(:erlang.group_leader(), {:printout, 12, 'Error! ~w:~tw(~w,~tw,...) failed!\n    Reason: ~tp\n',
                                                                 [fwMod, fwFunc,
                                                                             a1,
                                                                                 a2,
                                                                                     fwError]})
                                 :lists.flatten(:io_lib.format('<font color="red">ERROR! ~w:~tw(~w,~tw,...) failed!</font>',
                                                                 [fwMod, fwFunc,
                                                                             a1,
                                                                                 a2]))
                             end)
  ...
  defp spawn_fw_call(mod, func, currConf, pid, error, loc, sendTo) do
    :ct_util.mark_process()
    {func1, endTCFunc} = (case (func) do
                            cF when cF == :init_per_suite or
                                      cF == :end_per_suite or
    ...
    fwCall = fn () ->
                  ...
                  {time, retVal, loc1} = (try do
                                            do_end_tc_call(mod, endTCFunc,
                                                             {pid, error,
                                                                [conf]},
                                                             error)
                                          catch
                                            ...
                                            {:failed, reason} = newReturn ->
                                              fw_error_notify(mod, func1, conf,
                                                                reason)
                  ...
                  send(sendTo, {self(), :fw_notify_done,
                                  {time, retVal, loc1, [], :undefined}})
             end
    spawn_link(fwCall)
  end

  defp cleanup_after_fw_error(_Mod, _Func, conf, pid, fwError,
            {fwMod, fwFunc = :init_tc,
               [mod, {:init_per_testcase, func} = iPTC | _]}) do
  ...
  defp cleanup_after_fw_error(_Mod, _Func, conf, pid, fwError,
            {fwMod, fwFunc = :end_tc,
               [mod, {:init_per_testcase, func} | _]}) do
    skip = {:auto_skip, {:failed, {fwMod, fwFunc, fwError}}}
    try do
      (
        do_init_tc_call(mod, {:end_per_testcase_not_run, func},
  ...
  defp cleanup_after_fw_error(_Mod, _Func, conf, pid, fwError,
            {fwMod, fwFunc = :init_tc,
               [mod, {:end_per_testcase, func} | _]}) do
    {retVal, loc} = (case ({:proplists.get_value(:tc_status,
                                                   conf),
                              :proplists.get_value(:tc_fail_loc, conf,
                                                     :unknown)}) do
                       {:undefined, _} ->
  ...
  defp cleanup_after_fw_error(mod, func, conf, pid, fwError,
            {fwMod, fwFunc = :end_tc,
               [mod, {:end_per_testcase, func} | _]}) do
    {retVal, report,
       loc} = (case ({:proplists.get_value(:tc_status, conf),
                        :proplists.get_value(:tc_fail_loc, conf, :unknown)}) do
                 {:undefined, _} ->
                   {{:failed, {fwMod, fwFunc, fwError}},
                      {{fwMod, fwError}, fwError}, {fwMod, fwFunc}}
                 {e = {:failed, _Reason}, :unknown} ->
  ...
  defp cleanup_after_fw_error(mod, func, conf, pid, fwError,
            {fwMod, fwFunc = :end_tc, _})
      when func === :init_per_suite or
             func === :init_per_group do
    retVal = {:failed, {fwMod, fwFunc, fwError}}
    try do
      do_end_tc_call(mod, {:cleanup, func},
                       {pid, retVal, [conf]}, fwError)
    catch
      _, fwEndTCErr ->
        exit({:fw_notify_done, :end_tc, fwEndTCErr})
    ...
    :test_server_sup.framework_call(:report,
                                      [:framework_error, {mod, reportFunc,
                                                            retVal}])
    {retVal, {fwMod, fwFunc}, :error}
  end
  ...
  defp cleanup_after_fw_error(_Mod, _Func, _Conf, _Pid, fwError,
            {fwMod, fwFunc, _}) do
    :test_server_sup.framework_call(:report,
                                      [:framework_error, {{fwMod, fwFunc},
                                                            fwError}])
    {fwError, {fwMod, fwFunc}, :error}
  end

  defp start_job_proxy() do
    :erlang.group_leader(spawn(fn () ->
                                    job_proxy_msgloop()
                               end),
                           self())
    :ok
  ...
  defp io_reply_proxy(replyTo) do
    :ct_util.mark_process()
    receive do
      ioReply when (is_tuple(ioReply) and
  ...
  defp job_proxy_msgloop() do
    :ct_util.mark_process()
    receive do
      ioReq when (tuple_size(ioReq) >= 2 and
                    :erlang.element(1, ioReq) == :io_request)
                 ->
        replyProxy = spawn(fn () ->
                                io_reply_proxy(:erlang.element(2, ioReq))
                           end)
        send(:erlang.group_leader(), :erlang.setelement(2,
                                                          ioReq, replyProxy))
      {:sync_apply, from, mFA} ->
        send(:erlang.group_leader(), {:sync_apply_proxy, self(),
                                        from, mFA})
  ...
  defp run_test_case_eval_fun(mod, func, args, name, ref, runInit,
            timetrapData, logOpts, tCCallback) do
    fn () ->
         run_test_case_eval(mod, func, args, name, ref, runInit,
                              timetrapData, logOpts, tCCallback)
    end
  end

  defp run_test_case_eval(mod, func, args0, name, ref, runInit,
            timetrapData, logOpts, tCCallback) do
    :erlang.put(:test_server_multiply_timetraps,
                  timetrapData)
    :erlang.put(:test_server_logopts, logOpts)
    where = [{mod, func}]
    :erlang.put(:test_server_loc, where)
    fWInitFunc = (case (runInit) do
                    :run_init ->
                      {:init_per_testcase, func}
                    _ ->
                      func
                  end)
    fWInitResult0 = do_init_tc_call(mod, fWInitFunc, args0,
                                      {:ok, args0})
    set_tc_state(:running)
    {{time, value}, loc, opts} = (case (fWInitResult0) do
                                    {:ok, args} ->
                                      run_test_case_eval1(mod, func, args, name,
                                                            runInit, tCCallback)
                                    error = {:error, _Reason} ->
                                      newResult = do_end_tc_call(mod,
                                                                   fWInitFunc,
                                                                   {error,
                                                                      args0},
                                                                   {:auto_skip,
    ...
    case (runInit) do
      :run_init ->
        ...
        case (init_per_testcase(mod, func, args)) do
          skip = {skipType, reason} when skipType == :skip or
                                           skipType == :skipped
                                         ->
            line = get_loc()
          ...
          {:skip_and_save, reason, saveCfg} ->
            line = get_loc()
            conf = [{:tc_status, {:skipped, reason}}, {:save_config,
                                                         saveCfg} |
                                                          hd(args)]
            newRes = do_end_tc_call(mod, {:init_per_testcase, func},
                                      {{:skip, reason}, [conf]},
          ...
          failTC = {:fail, reason} ->
            ...
            newRes = do_end_tc_call(mod, {:init_per_testcase, func},
                                      {{:error, reason}, [endConf]}, failTC)
            {{0, newRes}, [{mod, func}], []}
          {:ok, newConf} ->
            iPTCEndRes = do_end_tc_call(mod,
                                          {:init_per_testcase, func},
                                          {:ok, [newConf]}, newConf)
            {{t, return}, loc, newConf1} = (cond do
                                              not is_list(iPTCEndRes) ->
                                                {{0, iPTCEndRes}, :undefined,
                                                   newConf}
                                              true ->
                                                newConfUC = user_callback(tCCallback,
                                                                            mod,
                                                                            func,
                                                                            :init,
                                                                            iPTCEndRes)
                                                set_tc_state(:tc, newConfUC)
                                                {ts_tc(mod, func, [newConfUC]),
                                                   get_loc(), newConfUC}
                                            end)
            {endConf, tSReturn, fWReturn} = (case (return) do
                                               {e, tCError} when e == :EXIT or
                                                                   e == :failed
                                                                 ->
                                                 fw_error_notify(mod, func,
                                                                   newConf1,
                                                                   tCError, loc)
                                                 {[{:tc_status,
                                                      {:failed, tCError}},
            ...
            endConf1 = user_callback(tCCallback, mod, func, :end,
                                       endConf)
            set_tc_state(:tc, endConf1)
            endConf2 = (case (do_init_tc_call(mod,
            ...
            {fWReturn1, tSReturn1,
               endConf3} = (case (end_per_testcase(mod, func,
                                                     endConf2)) do
            ...
            case (do_end_tc_call(mod, {:end_per_testcase, func},
                                   {fWReturn1, [endConf3]}, tSReturn1)) do
              {:failed, reason} = newReturn ->
      ...
      :skip_init ->
        ...
        args1 = user_callback(tCCallback, mod, func, :init,
                                args)
        ...
        {{t, return}, loc} = {ts_tc(mod, func, args2),
                                get_loc()}
        return1 = user_callback(tCCallback, mod, func, :end,
                                  return)
        {return2, opts} = process_return_val([return1], mod,
                                               func, args1, [{mod, func}],
                                               return1)
        {{t, return2}, loc, opts}
    end
  ...
  defp do_init_tc_call(mod, func, res, return) do
    :test_server_sup.framework_call(:init_tc,
                                      [mod, func, res], return)
  end

  defp do_end_tc_call(mod, iPTC = {:init_per_testcase, func}, res,
            return) do
    case (return) do
      {nOk, _} when nOk == :auto_skip or nOk == :fail or
                      nOk == :skip or nOk == :skipped
                    ->
        {_, args} = res
        {newConfig, iPTCEndRes} = (case (do_end_tc_call1(mod,
                                                           iPTC, res,
                                                           return)) do
        ...
        ePTCInitRes = (case (do_init_tc_call(mod,
                                               {:end_per_testcase_not_run,
                                                  func},
                                               newConfig, iPTCEndRes)) do
  ...
  defp process_return_val([return], m, f, a, loc, final)
    ...
    returnTags = [:skip, :skip_and_save, :save_config,
                                             :comment, :return_group_result]
    case (:lists.all(fn val when is_tuple(val) ->
                          :lists.any(fn t ->
  ...
  defp process_return_val1([failed = {e, tCError} | _], m, f, a = [args],
            loc, _, saveOpts)
      when e == :EXIT or e == :failed do
    fw_error_notify(m, f, a, tCError, loc)
    case (do_end_tc_call(m, f,
                           {{:error, tCError},
  ...
  defp process_return_val1([saveCfg = {:save_config, _} | opts], m, f,
            [args], loc, final, saveOpts) do
    process_return_val1(opts, m, f, [[saveCfg | args]], loc,
                          final, saveOpts)
  end

  defp process_return_val1([{:skip_and_save, why, saveCfg} | opts], m, f,
            [args], loc, _, saveOpts) do
    process_return_val1(opts, m, f,
                          [[{:save_config, saveCfg} | args]], loc, {:skip, why},
                          saveOpts)
  end

  defp process_return_val1([gR = {:return_group_result, _} | opts], m, f,
            a, loc, final, saveOpts) do
    process_return_val1(opts, m, f, a, loc, final,
                          [gR | saveOpts])
  end

  defp process_return_val1([retVal = {tag, _} | opts], m, f, a, loc, _,
            saveOpts)
      when tag == :skip or tag == :comment do
    process_return_val1(opts, m, f, a, loc, retVal,
                          saveOpts)
  end
  ...
  defp init_per_testcase(mod, func, args) do
    ...
    case (:erlang.function_exported(mod, :init_per_testcase,
                                      2)) do
      true ->
  ...
  defp do_init_per_testcase(mod, args) do
    try do
      ...
      {skip, reason} when skip === :skip or skip === :skipped
                          ->
        {:skip, reason}
      :exit, {skip, reason} when skip === :skip or
                                   skip === :skipped
                                 ->
        {:skip, reason}
      other ->
      ...
      newConf when is_list(newConf) ->
        case (:lists.filter(fn t when is_tuple(t) ->
                                 false
                               _ ->
                                 true
                            end,
          ...
          [] ->
            {:ok, newConf}
          bad ->
            send(:erlang.group_leader(), {:printout, 12, 'ERROR! init_per_testcase has returned bad elements in Config: ~tp\n', [bad]})
            {:skip,
               {:failed, {mod, :init_per_testcase, :bad_return}}}
        end
      {:fail, _Reason} = res ->
        res
      _Other ->
        send(:erlang.group_leader(), {:printout, 12, 'ERROR! init_per_testcase did not return a Config list.\n', []})
        {:skip,
           {:failed, {mod, :init_per_testcase, :bad_return}}}
    end
  end
  ...
  defp end_per_testcase(mod, func, conf) do
    case (:erlang.function_exported(mod, :end_per_testcase,
      ...
      false ->
        case (:erlang.function_exported(mod, :fin_per_testcase,
                                          2)) do
          true ->
  ...
  defp do_end_per_testcase(mod, endFunc, func, conf) do
    ...
    try do
      ...
      class, reason ->
        ...
        comment(:io_lib.format('~ts<font color="red">WARNING: ~w crashed!</font>\n', [comment0, endFunc]))
  ...
  def get_loc(pid) do
    [{:current_stacktrace, stk0}, {:dictionary,
                                     dict}] = :erlang.process_info(pid,
                                                                     [:current_stacktrace,
                                                                          :dictionary])
    :lists.foreach(fn {key, val} ->
                        :erlang.put(key, val)
                   end,
                     dict)
    stk = (for loc <- stk0 do
    ...
    case (:erlang.get(:test_server_loc)) do
      [{suite, case__}] ->
        case (for {s, c, _L} <- stk, s == suite, c == case__ do
          ...
          _ ->
            {preTC, postTC} = :lists.splitwith(fn {:test_server,
                                                     :ts_tc, _} ->
                                                    false
                                                  _ ->
                                                    true
  ...
  defp ts_tc(m, f, a) do
    ...
    result = (try do
                ...
                type, reason ->
                  ...
                  case (type) do
                    :throw ->
                      {:failed, {:thrown, {reason, __STACKTRACE__}}}
                    :error ->
                      {:EXIT, {reason, __STACKTRACE__}}
                    :exit ->
                      {:EXIT, reason}
  ...
  defp log(msg) do
    send(:erlang.group_leader(), {:structured_io, self(),
                                    msg})
    :ok
  end
  ...
  def adjusted_sleep(mSecs) do
    {multiplier,
       scaleFactor} = (case (:test_server_ctrl.get_timetrap_parameters()) do
                         {:undefined, :undefined} ->
                           {1, 1}
                         {:undefined, false} ->
                           {1, 1}
    ...
    receive do after trunc(mSecs * multiplier * scaleFactor)
                     ->
      :ok
    end
    :ok
  ...
  def break(cBM, testCase, comment) do
    timetrap_cancel()
    {tCName, cntArg, pName} = (cond do
                                 testCase == :"" ->
                                   {'', '', :test_server_break_process}
                                 true ->
                                   str = :erlang.atom_to_list(testCase)
                                   {[32 | str], str,
                                      :erlang.list_to_atom('test_server_break_process_' ++ str)}
                               end)
  ...
  def timetrap(timeout) do
    multAndScale = (case (:erlang.get(:test_server_multiply_timetraps)) do
                      :undefined ->
                        {fn t ->
                              t
                         end,
  ...
  defp timetrap(timeout0, timeToReport0, tCPid,
            multAndScale = {multiplier, scale}) do
    timeout = time_ms(timeout0, tCPid, multAndScale)
    timeout1 = multiplier.(timeout)
    timeToReport = (cond do
    ...
    handle = (case (timeout1) do
                :infinity ->
                  :infinity
                _ ->
                  spawn_link(:test_server_sup, :timetrap,
                               [timeout1, timeToReport, scale, tCPid])
              end)
    case (:erlang.get(:test_server_timetraps)) do
      :undefined ->
        :erlang.put(:test_server_timetraps,
                      [{handle, tCPid, {timeToReport, scale}}])
      list ->
        list1 = :lists.delete({:infinity, tCPid,
                                 {:infinity, false}},
                                list)
        :erlang.put(:test_server_timetraps,
  ...
  defp ensure_timetrap(config) do
    case (:erlang.get(:test_server_timetraps)) do
      ...
      _ ->
        ...
        dTmo = (case (:lists.keysearch(:default_timeout, 1,
                                         config)) do
        ...
        :erlang.put(:test_server_default_timetrap,
  ...
  defp time_ms({other, _N}, _, _) do
    format('=== ERROR: Invalid time specification: ~tp. Should be seconds, minutes, or hours.~n', [other])
  ...
  defp time_ms_check(mFA = {m, f, a}) when (is_atom(m) and
  ...
  defp time_ms_apply(func, tCPid, multAndScale) do
    ...
    userTTSup = spawn(fn () ->
                           user_timetrap_supervisor(func, whoAmI, tCPid, gL, t0,
                                                      multAndScale)
                      end)
    receive do
      {^userTTSup, :infinity} ->
  ...
  defp user_timetrap_supervisor(func, spawner, tCPid, gL, t0, multAndScale) do
    ...
    receive do
      {^userTT, result} ->
        ...
        elapsed = :erlang.convert_time_unit(t1 - t0, :native,
                                              :milli_seconds)
        try do
          time_ms_check(result)
        catch
          _, _ ->
            send(gL, {:user_timetrap, tCPid, 0, t0, elapsed,
                        multAndScale})
        else
          timeVal ->
            send(gL, {:user_timetrap, tCPid, timeVal, t0, elapsed,
                        multAndScale})
        end
      {:EXIT, ^userTT, error} when error != :normal ->
        :erlang.demonitor(monRef, [:flush])
        send(gL, {:user_timetrap, tCPid, 0, t0,
                    {:user_timetrap_error, error}, multAndScale})
      {:DOWN, ^monRef, _, _, _} ->
        :erlang.demonitor(monRef, [:flush])
    ...
    case (:erlang.get(:test_server_user_timetrap)) do
      ...
      userTTSups ->
        case (:proplists.get_value(tCPid, userTTSups)) do
          :undefined ->
            :erlang.put(:test_server_user_timetrap,
      ...
      timers ->
        case (:lists.keysearch(handle, 1, timers)) do
          ...
          false when sendToServer == true ->
            send(:erlang.group_leader(), {:timetrap_cancel_one,
                                            handle, self()})
          false ->
            :ok
        end
  ...
  defp timetrap_cancel_all(tCPid, sendToServer) do
    ...
    cond do
      sendToServer == true ->
        send(:erlang.group_leader(), {:timetrap_cancel_all,
                                        tCPid, self()})
        :ok
      true ->
        :ok
  ...
  defp get_timetrap_info(tCPid, sendToServer) do
    case (:erlang.get(:test_server_timetraps)) do
      :undefined ->
        :undefined
      timers ->
        case (for {handle, pid, info} <- timers, pid == tCPid,
                    handle != :infinity do
                info
              end) do
          [{tVal, true} | _] ->
  ...
  def start_node(name, type, options) do
    :lists.foreach(fn n ->
                        case (firstname(n)) do
                          ^name ->
                            format('=== WARNING: Trying to start node \'~w\' when node with same first name exists: ~w', [name, n])
                          _other ->
                            :ok
                        end
                   end,
                     :erlang.nodes())
    send(:erlang.group_leader(), {:sync_apply, self(),
  ...
  def stop_node(slave) do
    ...
    send(:erlang.group_leader(), {:sync_apply, self(),
                                    {:test_server_ctrl, :stop_node, [slave]}})
    result = (receive do
                {:sync_result, r} ->
                  r
              end)
    case (result) do
      ...
      {:error, _Reason} ->
        format('=== WARNING: Attempt to stop a nonexisting slavenode (~w)~n===          Trying to kill it anyway!!!', [slave])
  ...
  def peer_name(module, testCase) do
    :peer.random_name(:lists.concat([module, '-', testCase]))
  end

  def start_peer(args, module, testCase) when is_list(args) do
    start_peer(%{args: args,
                   name: peer_name(module, testCase)},
                 module)
  end

  def start_peer(%{name: _Name} = opts, module, _TestCase) do
    start_peer(opts, module)
  end

  def start_peer(opts, module, testCase) do
    start_peer(Map.put(opts, :name,
                               peer_name(module, testCase)),
                 module)
  end

  def start_peer(args, module, testCase, release, outDir)
      when is_list(args) do
    start_peer(%{args: args}, module, testCase, release,
                 outDir)
  end

  def start_peer(opts, module, testCase, release, outDir) do
    case (find_release(release)) do
      :not_available ->
        :not_available
      erl ->
        env = :maps.get(:env, opts, []) ++ [{'ERL_AFLAGS', false}]
        newArgs = ['-pa', peer_compile(erl, :code.which(:peer),
                                     outDir) |
                          :maps.get(:args, opts, [])]
        start_peer(Map.merge(opts, %{exec: erl, args: newArgs,
                                       env: env, start_cover: false}),
                     module, testCase)
    end
  end

  defp start_peer(%{name: name} = opts, module) do
    crashDir = :test_server_sup.crash_dump_dir()
    crashFile = :filename.join([crashDir, :lists.concat(['erl_crash_dump.',
                                                             name])])
    args = :maps.get(:args, opts, [])
    cookieArg = (case (:lists.member('-setcookie', args)) do
                   false ->
                     ['-setcookie', :erlang.atom_to_list(:erlang.get_cookie())]
                   true ->
                     []
                 end)
    fullArgs = cookieArg ++ ['-pa',
                                 :filename.dirname(:code.which(module)), '-env', 'ERL_CRASH_DUMP',
                                                                                crashFile] ++ args
    case (:maps.get(:start_cover, opts,
                      true) and :test_server.is_cover()) do
      true ->
        coverMain = :cover.get_main_node()
        shutdown = :erlang.binary_to_term(:erlang.term_to_binary({10000,
                                                                    coverMain}))
        case (:peer.start_link(Map.merge(opts, %{args: fullArgs,
      ...
      false ->
        :peer.start_link(Map.put(opts, :args, fullArgs))
    end
  end

  defp peer_compile(erl, :cover_compiled, outDir) do
    {:file, path} = :cover.is_compiled(:peer)
    peer_compile(erl, path, outDir)
  end

  defp peer_compile(erl, modPath, outDir) do
    {:ok, modSrc} = :filelib.find_source(modPath)
    erlc = :filename.join(:filename.dirname(erl), 'erlc')
    cmd(erlc,
          ['-o', outDir, :unicode.characters_to_binary(modSrc)])
    outDir
  end
  ...
  defp cmd(exec, args) do
    ...
    port = :erlang.open_port({:spawn_executable, exec},
                               [{:args, args}, {:env, env}, :stream, :binary,
                                                                         :exit_status,
                                                                             :stderr_to_stdout])
    read_std(port, :lists.join(' ', [exec | args]), <<>>)
  end

  defp read_std(port, exec, out) do
    receive do
      {^port, {:data, more}} ->
  ...
  def run_on_shielded_node(fun, cArgs) when (is_function(fun) and
                             is_list(cArgs)) do
    nr = :erlang.unique_integer([:positive])
    name = 'shielded_node-' ++ :erlang.integer_to_list(nr)
    node = (case (start_node(name, :slave,
                               [{:args, '-hidden ' ++ cArgs}])) do
              {:ok, n} ->
                n
              err ->
  ...
  defp is_shielded(name) do
    case ({cast_to_list(name),
             :erlang.atom_to_list(node())}) do
      {'shielded_node' ++ _, _} ->
        true
      {_, 'shielded_node' ++ _} ->
        true
  ...
  defp same_version(name) do
    ...
    otherVersion = :rpc.call(name, :erlang, :system_info,
                               [:version])
  ...
  def memory_checker() do
    case ((try do
            :erlang.system_info({:memory_checker, :running})
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
            e -> e
  ...
  defp check_memory_leaks(:valgrind) do
    (try do
      :erlang.system_info({:memory_checker, :check_leaks})
    catch
      :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp check_memory_leaks(_) do
    :ok
  end

  defp valgrind_format(format, args) do
    (try do
      :erlang.system_info({:memory_checker, :print,
                             :io_lib.format(format, args)})
    catch
      :error, e -> {:EXIT, {e, __STACKTRACE__}}
      :exit, e -> {:EXIT, e}
  ...
  defp asan_take_logpath() do
    case (:os.getenv('ASAN_OPTIONS')) do
      false ->
        false
      s ->
  ...
  defp asan_take_logpath_loop(['log_path=' ++ logPath | t], acc) do
    {logPath, t ++ acc}
  end

  defp asan_take_logpath_loop([opt | t], acc) do
    asan_take_logpath_loop(t, [opt | acc])
  end

  defp asan_take_logpath_loop([], _) do
    false
  end

  defp asan_make_opts([a | t]) do
    asan_make_opts(t, a)
  end

  defp asan_make_opts([], acc) do
    acc
  end

  defp asan_make_opts([a | t], acc) do
