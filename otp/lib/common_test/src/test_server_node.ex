defmodule :m_test_server_node do
  ...
  defp start_node_peer(slaveName, optList, from, tI) do
    ...
    nodeStarted =
      :lists.concat([~c" -s ", :test_server_node, ~c" node_started ", hostStr, ~c" ", waitPort])

    crashDir = :test_server_sup.crash_dump_dir()
    crashFile = :filename.join([crashDir, ~c"erl_crash_dump." ++ cast_to_list(slaveName)])
    crashArgs = :lists.concat([~c" -env ERL_CRASH_DUMP \"", crashFile, ~c"\" "])
    ...
    args =
      case :string.find(suppliedArgs, ~c"-setcookie") do
        :nomatch ->
          ~c"-setcookie " ++ r_target_info(tI, :cookie) ++ ~c" " ++ suppliedArgs
    ...
    opts =
      case {clearAFlags, start_node_get_option_value(:env, optList, [])} do
        {false, []} ->
          []
        ...
        {true, []} ->
          [{:env, [{~c"ERL_AFLAGS", false}]}]

        {true, env} ->
          [{:env, [{~c"ERL_AFLAGS", false} | env]}]
      end
    ...
    case start_node_get_option_value(:wait, optList, true) do
      ...
      false ->
        nodename = :erlang.list_to_atom(slaveName ++ ~c"@" ++ hostStr)
        i = ~c"=== Not waiting for node"
  ...
  defp start_node_slave(slaveName, optList, from, _TI) do
    ...
    crashFile = :filename.join([crashDir, ~c"erl_crash_dump." ++ cast_to_list(slaveName)])
    crashArgs = :lists.concat([~c" -env ERL_CRASH_DUMP \"", crashFile, ~c"\" "])
    args = :lists.concat([~c" ", suppliedArgs, crashArgs])
    prog0 = start_node_get_option_value(:erl, optList, :default)
  ...
  defp do_start_node_slave(host0, slaveName, args, prog, cleanup, clearAFlags) do
    host =
      case host0 do
        :local ->
  ...
  defp save_clear_aflags(true) do
    case :os.getenv(~c"ERL_AFLAGS") do
      false ->
        false

      erlAFlags ->
        :os.unsetenv(~c"ERL_AFLAGS")
        erlAFlags
    end
  end
  ...
  defp restore_aflags(erlAFlags) do
    true = :os.putenv(~c"ERL_AFLAGS", erlAFlags)
    :ok
  end
  ...
  defp handle_start_node_return(version, vsnStr, {:started, node, oVersion, oVsnStr}) do
    str =
      :io_lib.format(
        ~c"WARNING: Started node reports different system version than current node! Current node version: ~p, ~p Started node version: ~p, ~p",
  ...
  defp quote_progname(progname) do
    do_quote_progname(:string.lexemes(progname, ~c" "))
  end
  ...
  defp do_quote_progname([prog, arg | args]) do
    case :os.find_executable(prog) do
      false ->
        do_quote_progname([prog ++ ~c" " ++ arg | args])
  ...
  defp otp_release_root() do
    case :erlang.get(:test_server_release_root) do
      :undefined ->
        root = :os.getenv(~c"TEST_SERVER_RELEASE_ROOT", ~c"/usr/local/otp/releases")
  ...
  def find_release(:latest) do
    otp_release_path(~c"latest/bin/erl")
  end
    ...
    case find_release(:os.type(), rel) do
      :none ->
        case find_release_path(rel) do
          :none ->
            case :string.take(rel, ~c"_", true) do
              {^rel, []} ->
                :none
  ...
  defp find_release_path(rel) do
    paths = :string.lexemes(:os.getenv(~c"PATH"), ~c":")
    find_release_path(paths, rel)
  end

  defp find_release_path([path | t], rel) do
    case :os.find_executable(~c"erl", path) do
      false ->
        find_release_path(t, rel)

      erlExec ->
        quotedExec = ~c"\"" ++ erlExec ++ ~c"\""

        release =
          :os.cmd(
            quotedExec ++
              ~c" -noinput -eval 'io:format(\"~ts\", [erlang:system_info(otp_release)])' -s init stop"
          )
        ...
        case release === rel do
          true ->
            case :os.cmd(
                   quotedExec ++
                     ~c" -noinput -eval 'io:format(\"~p\",[filelib:is_file(filename:join([code:root_dir(),\"OTP_VERSION\"]))]).' -s init stop"
                 ) do
              ~c"true" ->
                find_release_path(t, rel)
  ...
  defp find_release({:unix, :sunos}, rel) do
    case :os.cmd(~c"uname -p") do
      ~c"sparc" ++ _ ->
        otp_release_path(~c"otp_beam_solaris8_" ++ rel ++ ~c"/bin/erl")
  ...
  defp find_rel_suse_1(rel, rootWc) do
    case :erlang.system_info(:wordsize) do
      4 ->
        find_rel_suse_2(rel, rootWc ++ ~c"_32")
  ...
  defp find_rel_suse_2(rel, rootWc) do
    ...
    pat = :filename.basename(rootWc ++ ~c"_" ++ rel) ++ ~c".*"
    ...
    case :file.list_dir(relDir) do
      {:ok, dirs} ->
        case :lists.filter(
          ...
          [r | _] ->
            [:filename.join([relDir, r, ~c"bin", ~c"erl"])]
        end
  ...
  defp suse_release() do
    case :file.open(~c"/etc/SuSE-release", [:read]) do
      {:ok, fd} ->
        try do
          suse_release(fd)
        after
  ...
  defp suse_release(fd) do
    case :io.get_line(fd, :"") do
      ...
      line when is_list(line) ->
        case :re.run(line, ~c"^VERSION\\s*=\\s*(\\d+) *", [{:capture, :all_but_first, :list}]) do
          :nomatch ->
            suse_release(fd)
  ...
  defp find_rel_ubuntu(rel, minUbuntuRel, maxUbuntuRel)
    ...
    root = otp_release_path(~c"ubuntu")
  ...
  defp find_rel_ubuntu_aux1(rel, rootWc) do
    case :erlang.system_info(:wordsize) do
      4 ->
        find_rel_ubuntu_aux2(rel, rootWc ++ ~c"_32")
  ...
  defp ubuntu_release() do
    case :file.open(~c"/etc/lsb-release", [:read]) do
      {:ok, fd} ->
        try do
  ...
  defp ubuntu_release(fd, distroId, rel) do
    case :io.get_line(fd, :"") do
      ...
      line when is_list(line) ->
        case :re.run(line, ~c"^DISTRIB_ID=(\\w+)$", [{:capture, :all_but_first, :list}]) do
          {:match, [newDistroId]} ->
            ubuntu_release(fd, newDistroId, rel)

          :nomatch ->
            case :re.run(line, ~c"^DISTRIB_RELEASE=(\\d+(?:\\.\\d+)*)$", [
