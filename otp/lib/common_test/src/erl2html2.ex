defmodule :m_erl2html2 do
        ...
        case :file.open(file, [:raw, {:read_ahead, 10000}]) do
          {:ok, sFd} ->
            case :file.open(dest, [:write, :raw]) do
              {:ok, dFd} ->
                :ok = :file.write(dFd, [header, ~c"<pre>\n"])
                _Lines = build_html(sFd, dFd, encoding(file), functions)
                :ok = :file.write(dFd, [~c"</pre>\n", footer(), ~c"</body>\n</html>\n"])
                :ok = :file.close(sFd)
                :ok = :file.close(dFd)
                :ok
  ...
  defp build_html(sFd, dFd, enc, {:ok, str}, lastL, funcsAndCs, _IsFuncDef, {f, lastL}) do
    ...
    :ok =
      :file.write(
        dFd,
        [~c"<a name=\"", to_raw_list(lastLineLink, enc), ~c"\"/>"]
      )
  ...
  defp build_html(
    ...
    fALink =
      :test_server_ctrl.uri_encode(
        f ++ ~c"-" ++ :erlang.integer_to_list(a),
        :utf8
      )

    :ok = :file.write(dFd, [~c"<a name=\"", to_raw_list(fALink, enc), ~c"\"/>"])
