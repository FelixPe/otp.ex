defmodule :m_ct_framework do
            ...
            case (:ct_util.read_suite_data({:seq, suite, func})) do
              ...
              seq when is_atom(seq) ->
                case (:ct_util.read_suite_data({:seq, suite, seq})) do
                  [^func | tCs] ->
                    :lists.foreach(fn tC ->
                                        :ct_util.save_suite_data({:seq, suite,
                                                                    tC},
                                                                   seq)
                                   end,
                                     tCs)
  ...
  defp init_tc1(:ct_framework, _, :error_in_suite, _, [config0])
    ...
    _ = ct_suite_init(:ct_framework, :error_in_suite, [],
                        config0)
  ...
  defp init_tc1(mod, suite, func, hookFunc, [config0])
      when is_list(config0) do
    config1 = (case (:ct_util.read_suite_data(:last_saved_config)) do
    ...
    groupPath = :proplists.get_value(:tc_group_path, config,
                                       [])
    allGroups = [:proplists.get_value(:tc_group_properties,
                                        config, []) |
                     groupPath]
      ...
      {suiteInfo, mergeResult} ->
        case (mergeResult) do
          {:error, reason} ->
            ...
            {:fail, reason}
          _ ->
            init_tc2(mod, suite, func, hookFunc1, suiteInfo,
                       mergeResult, config)
        end
    end
  end
  ...
  defp init_tc2(mod, suite, func, hookFunc, suiteInfo,
            mergeResult, config) do
    mergedInfo = timetrap_first(mergeResult, [], [])
    _ = (case (:lists.keysearch(:stylesheet, 1,
                                  mergeResult ++ config)) do
           {:value, {:stylesheet, sSFile}} ->
             :ct_logs.set_stylesheet(func,
    ...
    case (:ct_util.get_overridden_silenced_connections()) do
      :undefined ->
        case (:lists.keysearch(:silent_connections, 1,
                                 mergeResult ++ config)) do
    ...
    case ((try do
            configure(mergedInfo, mergedInfo, suiteInfo, funcSpec,
                        [], config)
          catch
      ...
      {:suite0_failed, reason} ->
        :ct_util.set_testdata({:curr_tc,
                                 {mod, {:suite0_failed, {:require, reason}}}})
        {:auto_skip, {:require_failed_in_suite0, reason}}
      {:error, reason} ->
  ...
  defp initialize(refreshLogs, mod, func, [config])
    ...
    initialize(refreshLogs, mod,
                 group_or_func(func, config))
  end
  ...
  defp initialize(refreshLogs, mod, funcSpec) do
    ...
    :ct_event.notify(r_event(name: :tc_start, node: node(),
                         data: {mod, funcSpec}))
  end
  ...
  defp add_defaults(mod, func, groupPath) do
    ...
    case ((try do
      ...
      {:EXIT, {:undef, _}} ->
        ...
        suiteInfoNoCTH = (for i <- suiteInfo,
      ...
      suiteInfo when is_list(suiteInfo) ->
        case (:lists.all(fn e when is_tuple(e) ->
                              true
                            _ ->
                              false
                         end,
          ...
          true ->
            ...
            suiteInfoNoCTH = (for i <- suiteInfo1,
                                    :erlang.element(1, i) !== :ct_hooks do
                                i
                              end)
            case (add_defaults1(mod, func, groupPath,
          ...
          false ->
            errStr = :io_lib.format('~n*** ERROR *** Invalid return value from ~w:suite/0: ~tp~n', [suite, suiteInfo])
            :io.format('~ts', [errStr])
            :io.format(:ct_default_gl, '~ts', [errStr])
  ...
  defp add_defaults1(mod, func, groupPath, suiteInfo) do
    suite = get_suite_name(mod, groupPath)
    groupPathInfo = :lists.map(fn groupProps ->
    ...
    case (:lists.keysearch(:error, 1, groupPathInfo)) do
      {:value, {:error, badGr0Val, grName}} ->
        gr0ErrStr = :io_lib.format('~n*** ERROR *** Invalid return value from ~w:group(~tw): ~tp~n', [mod, grName, badGr0Val])
      ...
      _ ->
        ...
        case (testCaseInfo) do
          {:error, badTC0Val} ->
            tC0ErrStr = :io_lib.format('~n*** ERROR *** Invalid return value from ~w:~tw/0: ~tp~n', [mod, func, badTC0Val])
            :io.format('~ts', [tC0ErrStr])
            :io.format(:ct_default_gl, '~ts', [tC0ErrStr])
            {:testcase0_failed, :bad_return_value}
          _ ->
            tCAndGroupInfo = [testCaseInfo |
                                  remove_info_in_prev(testCaseInfo,
                                                        groupPathInfo)]
            suiteReqs = (for sDDef <- suiteInfo,
                               :erlang.or(:require == :erlang.element(1, sDDef),
                                            :default_config == :erlang.element(1,
                                                                                 sDDef)) do
                           sDDef
                         end)
            case (check_for_clashes(testCaseInfo, groupPathInfo,
                                      suiteReqs)) do
              [] ->
                add_defaults2(mod, func, tCAndGroupInfo, suiteInfo,
                                suiteReqs)
              clashes ->
  ...
  defp get_suite_name(:ct_framework, [cfg | _]) when (is_list(cfg) and
                                            cfg != []) do
    get_suite_name(:ct_framework, cfg)
  end

  defp get_suite_name(:ct_framework, cfg) when (is_list(cfg) and
                                      cfg != []) do
  ...
  defp check_for_clashes(tCInfo, [currGrInfo | path], suiteInfo) do
    reqNames = fn info ->
                    for r <- info, tuple_size(r) == 3,
                          :require == :erlang.element(1, r) do
                      :erlang.element(2, r)
                    end
               end
    existingNames = :lists.flatten(for l <- [suiteInfo |
                                                 path] do
                                     reqNames.(l)
                                   end)
    currGrReqNs = reqNames.(currGrInfo)
    grClashes = (for name <- currGrReqNs,
                       true == :lists.member(name, existingNames) do
                   name
                 end)
    allReqNs = currGrReqNs ++ existingNames
    tCClashes = (for name <- reqNames.(tCInfo),
  ...
  defp remove_info_in_prev(terms, [info | rest]) do
    uniqueInInfo = (for u <- info,
                          :erlang.or(:erlang.or(:erlang.and(:timetrap == :erlang.element(1,
                                                                                           u),
                                                              not
                                                              :lists.keymember(:timetrap,
                                                                                 1,
                                                                                 terms)),
                                                  :erlang.and(:require == :erlang.element(1,
                                                                                            u),
                                                                not
                                                                :lists.member(u,
                                                                                terms))),
                                       :erlang.and(:default_config == :erlang.element(1,
                                                                                        u),
                                                     not
                                                     keysmember([:default_config,
                                                                     1,
                                                                         :erlang.element(2,
                                                                                           u),
                                                                             2],
  ...
  defp add_defaults2(_Mod, :init_per_suite, iPSInfo, suiteInfo,
            suiteReqs) do
    info = :lists.flatten([iPSInfo, suiteReqs])
    :lists.flatten([info, remove_info_in_prev(info,
                                                [suiteInfo])])
  end

  defp add_defaults2(_Mod, :init_per_group, iPGAndGroupInfo,
            suiteInfo, suiteReqs) do
    suiteInfo1 = remove_info_in_prev(:lists.flatten([iPGAndGroupInfo,
                                                         suiteReqs]),
                                       [suiteInfo])
  ...
  defp add_defaults2(_Mod, _Func, tCAndGroupInfo, suiteInfo,
            suiteReqs) do
  ...
  defp merge_with_suite_defaults(mod, suiteInfo) do
    case (:lists.keysearch(:suite_defaults, 1,
                             mod.module_info(:attributes))) do
      {:value, {:suite_defaults, defaults}} ->
        sDReqs = (for sDDef <- defaults,
                        :require == :erlang.element(1, sDDef),
                        false == :lists.keymember(:erlang.element(2, sDDef), 2,
                                                    suiteInfo) do
                    sDDef
                  end)
        suiteInfo ++ sDReqs ++ (for sDDef <- defaults,
                                      :require != :erlang.element(1, sDDef),
                                      false == :lists.keymember(:erlang.element(1,
                                                                                  sDDef),
                                                                  1,
                                                                  suiteInfo) do
                                  sDDef
  ...
  defp configure([{:require, required} | rest], info, suiteInfo,
            scope, postInitHook, config) do
    case (:ct.require(required)) do
      :ok ->
        configure(rest, info, suiteInfo, scope, postInitHook,
                    config)
      error = {:error, reason} ->
  ...
  defp configure([{:require, name, required} | rest], info,
            suiteInfo, scope, postInitHook, config) do
    case (:ct.require(name, required)) do
      :ok ->
        configure(rest, info, suiteInfo, scope, postInitHook,
                    config)
      error = {:error, reason} ->
        case (required_default(name, required, info, suiteInfo,
                                 scope)) do
          :ok ->
            configure(rest, info, suiteInfo, scope, postInitHook,
                        config)
  ...
  defp configure([{:timetrap, :off} | rest], info, suiteInfo,
            scope, postInitHook, config) do
    configure(rest, info, suiteInfo, scope, postInitHook,
                config)
  end

  defp configure([{:timetrap, time} | rest], info, suiteInfo,
            scope, postInitHook, config) do
    postInitHook1 = [{:watchdog,
                        fn () ->
                         ...
                         postInitHook]
    configure(rest, info, suiteInfo, scope, postInitHook1,
                config)
  end

  defp configure([{:ct_hooks, hook} | rest], info, suiteInfo,
            scope, postInitHook, config) do
    configure(rest, info, suiteInfo, scope, postInitHook,
                [{:ct_hooks, hook} | config])
  end

  defp configure([_ | rest], info, suiteInfo, scope,
            postInitHook, config) do
    configure(rest, info, suiteInfo, scope, postInitHook,
                config)
  end
  ...
  defp required_default(name, key, info, _,
            {:init_per_group, grName, _}) do
  ...
  defp try_set_default(name, key, info, where) do
    cfgElems = (case (:lists.keysearch(name, 1, info)) do
                  {:value, {^name, val}} ->
                    [val]
                  false ->
                    case ((try do
  ...
  def end_tc(:ct_framework, :error_in_suite, {result, [args]},
           return) do
    case (:proplists.get_value(:force_failed, args)) do
      :undefined ->
        :ok
      _ ->
        add_to_stats(:failed)
    end
    finalNotify = (case (:ct_hooks.end_tc(:ct_framework,
                                            :error_in_suite, args, result,
                                            return)) do
                     :"$ct_no_change" ->
                       result
                     hookResult ->
    ...
    event = r_event(name: :tc_done, node: node(),
                data: {:ct_framework, :error_in_suite,
                         tag(finalNotify)})
  ...
  defp end_tc(mod, iPTC = {:init_per_testcase, _Func}, _TCPid,
            result, args, return) do
    case (end_hook_func(iPTC, return, iPTC)) do
      :undefined ->
        :ok
      _ ->
        suite = get_suite_name(mod, args)
        case (:ct_hooks.end_tc(suite, iPTC, args, result,
                                 return)) do
          :"$ct_no_change" ->
            :ok
          hookResult ->
  ...
  defp end_tc(mod, func00, tCPid, result, args, return) do
    suite = get_suite_name(mod, args)
    {onlyCleanup, func0} = (case (func00) do
                              {:cleanup, f0} ->
                                {true, f0}
                              _ ->
                                {false, func00}
                            end)
    {func, funcSpec, hookFunc} = (case (func0) do
    ...
    :ct_util.delete_suite_data(:last_saved_config)
    {result1, finalNotify} = (case (hookFunc) do
                                :undefined ->
                                  {:ok, result}
                                _ when onlyCleanup ->
                                  {:ok, result}
                                _ ->
                                  case (:ct_hooks.end_tc(suite, hookFunc, args,
                                                           result, return)) do
                                    :"$ct_no_change" ->
                                      {:ok, result}
                                    hookResult ->
    ...
    finalResult = (case (:erlang.get(:"$test_server_framework_test")) do
                     _ when onlyCleanup ->
                       result1
                     :undefined ->
                       event = r_event(name: :tc_done, node: node(),
                                   data: {mod, funcSpec, tag(finalNotify)})
                       :ct_event.sync_notify(event)
                       result1
                     fun ->
                       event = r_event(name: :tc_done, node: node(),
                                   data: {mod, funcSpec,
                                            tag({:"$test_server_framework_test", finalNotify})})
                       :ct_event.sync_notify(event)
                       fun.(:end_tc, return)
                   end)
    case (funcSpec) do
        ...
        case (:lists.keysearch(:save_config, 1, args)) do
          {:value, {:save_config, saveConfig}} ->
            :ct_util.save_suite_data(:last_saved_config,
                                       {suite, {:group, groupName}}, saveConfig)
          false ->
            :ok
        end
      _ ->
    ...
    case (:ct_util.update_testdata(:curr_tc,
                                     clearCurrTC)) do
      {:error, _} = clearError ->
        exit(clearError)
      _ ->
  ...
  defp end_hook_func({:init_per_testcase, _},
            {:auto_skip, {:sequence_failed, _, _}}, _) do
    :undefined
  end

  defp end_hook_func({:init_per_testcase, _}, {:auto_skip, 'Repeated test stopped by force_stop option'}, _) do
    :undefined
  end

  defp end_hook_func({:init_per_testcase, _},
            {:fail, {:config_name_already_in_use, _}}, _) do
    :undefined
  end

  defp end_hook_func({:init_per_testcase, _},
            {:auto_skip, {infoFuncError, _}}, _)
      when infoFuncError == :testcase0_failed or
             infoFuncError == :require_failed do
    :undefined
  end

  defp end_hook_func(:init_per_group,
            {:auto_skip, {infoFuncError, _}}, _)
  ...
  defp end_hook_func(:init_per_suite,
            {:auto_skip, {:require_failed_in_suite0, _}}, _) do
    :undefined
  end
  ...
  defp tag({:skipped,
             reason = {:failed, {_, :init_per_testcase, _}}}) do
    {:auto_skipped, reason}
  end

  defp tag({sTag, reason}) when sTag == :skip or
                                 sTag == :skipped do
    case (reason) do
  ...
  defp tag(e = {eTag, _}) when eTag == :error or
  ...
  defp tag(userTimetrap = {:user_timetrap_error,
                            _Reason}) do
    userTimetrap
  end
  ...
  def error_notification(mod, func, _Args, {error, loc}) do
    errorSpec = (case (error) do
                   {what = {_E, _R}, trace} when is_list(trace) ->
                     what
                   what ->
                     what
                 end)
    errorStr = (case (errorSpec) do
                  {:badmatch, descr} ->
    ...
    case ({mod, error}) do
      ...
      {:undefined, _OtherError} ->
        :ok
      _ ->
        case (:ct_util.get_testdata({:comment,
                                       :erlang.group_leader()})) do
          :undefined ->
            :test_server.comment(errorHtml)
          comment ->
            commentHtml = '<font color="green">' ++ '(' ++ '</font>' ++ comment ++ '<font color="green">' ++ ')' ++ '</font>'
    ...
    printError = fn errorFormat, errorArgs ->
                      ...
                      :io.format(:ct_default_gl, '~ts~n',
                                   [:lists.concat([div, errorStr2, div])])
                      link = '\n\n<a href="#end">Full error description and stacktrace</a>'
                      errorHtml2 = :ct_logs.escape_chars(errorStr2)
                      :ct_logs.tc_log(:ct_error_notify, 99, 'CT Error Notification', '~ts',
                                        [errorHtml2 ++ link], [])
                 end
    case (loc) do
      ...
      [{lastMod, lastFunc, lastLine} | _] ->
        printError.('~w:~tw failed on line ~w~nReason: ~ts', [lastMod, lastFunc, lastLine, errorStr])
  ...
  defp mark_as_failed1(seq, mod, func, [tC | tCs]) do
    :ct_util.save_suite_data({:seq, mod, tC},
                               {:failed, seq, func})
    mark_as_failed1(seq, mod, func, tCs)
  end
  ...
  defp group_or_func(func, config) when func == :init_per_group or
  ...
  def get_all_cases(suite) do
    case (get_suite(suite, :all)) do
      ...
      tests ->
        ...
        :lists.reverse(:lists.foldl(fn tC, tCs ->
  ...
  defp get_all_cases1(suite,
            [{:conf, _Props, _Init, grTests, _End} | tests]) do
    get_all_cases1(suite, grTests) ++ get_all_cases1(suite,
                                                       tests)
  end
    ...
    case (safe_apply_all_0(mod)) do
      {:ok, allTCs} ->
        try do
          ...
          {:error, error} ->
            [{:ct_framework, :error_in_suite,
                [[{:error, error}, {:force_failed, true}]]}]
          _, error ->
            [{:ct_framework, :error_in_suite,
                [[{:error, {error, __STACKTRACE__}}, {:force_failed,
                                                        true}]]}]
        else
          {:error, _} = error ->
            [{:ct_framework, :error_in_suite, [[error]]}]
      ...
      {:error, :undef} ->
        ...
        [{:ct_framework, :error_in_suite, [[{:error, reason}]]}]
      {:error, {:bad_return, _Bad}} ->
        reason = :erlang.list_to_atom('Bad return value from ' ++ :erlang.atom_to_list(mod) ++ ':all/0')
        [{:ct_framework, :error_in_suite, [[{:error, reason}]]}]
      {:error, {:bad_hook_return, bad}} ->
  ...
  defp save_seqs(mod, [{:sequence, seq} | tCs], seqs, all) do
    case (:lists.keysearch(seq, 1, seqs)) do
      ...
      false ->
        reason = :erlang.list_to_atom(:erlang.atom_to_list(seq) ++ ' is missing in ' ++ :erlang.atom_to_list(mod))
        throw({:error, reason})
    end
  end
  ...
  defp check_private(seq, tCs, all) do
    ...
    cond do
      bad != [] ->
        ...
        throw({:error,
                 :erlang.list_to_atom(:lists.flatten(reason))})
      true ->
        :ok
    end
  end
  ...
  def init_per_group(groupName, config) do
    ...
    :ct_logs.log('TEST INFO', 'init_per_group/2 for ~tw missing in suite, using default.', [groupName])
    config
  end

  def end_per_group(groupName, _) do
    :ct.comment(:io_lib.format('end of ~tp', [groupName]))
    :ct_logs.log('TEST INFO', 'end_per_group/2 for ~tw missing in suite, using default.', [groupName])
    :ok
  end
  ...
  def report(what, data) do
    case (what) do
      :loginfo ->
        testName = :filename.basename(:proplists.get_value(:topdir,
                                                             data),
                                        '.logs')
      ...
      :tests_done ->
        :ok
      :severe_error ->
        :ct_event.sync_notify(r_event(name: what, node: node(),
                                  data: data))
      ...
      :tc_start ->
        ...
        :ct_event.sync_notify(r_event(name: :tc_logfile, node: node(),
                                  data: data1))
        :ok
      :tc_done ->
        ...
        reportingPid = self()
        :ct_logs.register_groupleader(reportingPid,
                                        :erlang.group_leader())
        case (result) do
          {:failed, reason} ->
            :ct_hooks.on_tc_fail(what, {suite, funcSpec, reason})
          {:skipped,
             {:failed, {_, :init_per_testcase, _}} = reason} ->
            :ct_hooks.on_tc_skip(:tc_auto_skip,
                                   {suite, funcSpec, reason})
          {:skipped, {:require_failed, _} = reason} ->
            :ct_hooks.on_tc_skip(:tc_auto_skip,
                                   {suite, funcSpec, reason})
          {:skipped, reason} ->
            :ct_hooks.on_tc_skip(:tc_user_skip,
                                   {suite, funcSpec, reason})
          {:auto_skipped, reason} ->
            :ct_hooks.on_tc_skip(:tc_auto_skip,
                                   {suite, funcSpec, reason})
      ...
      :tc_user_skip ->
        {func, data1} = (case (data) do
                           {suite, {f, :undefined}, comment} ->
                             {f, {suite, f, comment}}
                           d = {_, {f, _}, _} ->
                             {f, d}
                         end)
        :ct_event.sync_notify(r_event(name: :tc_user_skip,
                                  node: node(), data: data1))
      ...
      :tc_auto_skip ->
        ...
        :ct_event.sync_notify(r_event(name: :tc_auto_skip,
                                  node: node(), data: data1))
        :ct_hooks.on_tc_skip(what, data1)
        cond do
          (func != :end_per_suite and func != :end_per_group) ->
      ...
      :framework_error ->
        case (data) do
          {{m, f}, e} ->
            :ct_event.sync_notify(r_event(name: :tc_done, node: node(),
  ...
  defp add_to_stats(result) do
    update = fn {ok, failed,
                  ...
                  :ct_event.sync_notify(r_event(name: :test_stats, node: node(),
                                            data: stats))
                  stats
             end
  ...
  def warn(what) when what == :nodes or
                      what == :processes do
    false
  end
  ...
  defp get_html_wrapper(testName, printLabel, cwd, tableCols,
            encoding) do
  ...
  defp safe_apply_all_0(mod) do
    try do
      ...
      _, reason ->
        handle_callback_crash(reason, __STACKTRACE__, mod, :all,
                                {:error, :undef})
    else
  ...
  defp safe_apply_groups_0(mod, default) do
    try do
      ...
      _, reason ->
        handle_callback_crash(reason, __STACKTRACE__, mod,
                                :groups, default)
    else
  ...
  defp handle_callback_crash(:undef, [{mod, func, [], _} | _], mod, func,
            default) do
  ...
  defp expand_tests(mod, [{:testcase, case__, [prop]} | tests]) do
    [{:repeat, {mod, case__}, prop} | expand_tests(mod,
                                                     tests)]
  end
