defmodule :m_ct_framework do
  ...
  def init_tc(mod, func0, args) do
    ...
    case func !== :end_per_suite and func !== :end_per_group and :ct_util.get_testdata(:skip_rest) do
      true ->
        ...
        {:auto_skip, ~c"Repeated test stopped by force_stop option"}
  ...
  defp add_defaults(mod, func, groupPath) do
    ...
    case (try do
      ...
      {:EXIT, reason} ->
        errStr = :io_lib.format(~c"~n*** ERROR *** ~w:suite/0 failed: ~tp~n", [suite, reason])
        :io.format(~c"~ts", [errStr])
        :io.format(:ct_default_gl, ~c"~ts", [errStr])
      ...
      suiteInfo when is_list(suiteInfo) ->
        case :lists.all(
          ...
          false ->
            errStr =
              :io_lib.format(~c"~n*** ERROR *** Invalid return value from ~w:suite/0: ~tp~n", [
                suite,
                suiteInfo
              ])
  ...
  defp add_defaults1(mod, func, groupPath, suiteInfo) do
    ...
    case :lists.keysearch(:error, 1, groupPathInfo) do
      {:value, {:error, badGr0Val, grName}} ->
        gr0ErrStr =
          :io_lib.format(~c"~n*** ERROR *** Invalid return value from ~w:group(~tw): ~tp~n", [
            mod,
            grName,
            badGr0Val
          ])

        :io.format(~c"~ts", [gr0ErrStr])
        :io.format(:ct_default_gl, ~c"~ts", [gr0ErrStr])
      ...
      _ ->
        ...
        case testCaseInfo do
          {:error, badTC0Val} ->
            tC0ErrStr =
              :io_lib.format(~c"~n*** ERROR *** Invalid return value from ~w:~tw/0: ~tp~n", [
                mod,
                func,
                badTC0Val
              ])

            :io.format(~c"~ts", [tC0ErrStr])
            :io.format(:ct_default_gl, ~c"~ts", [tC0ErrStr])
  ...
  defp check_for_clashes(tCInfo, [currGrInfo | path], suiteInfo) do
    reqNames = fn info ->
      for r <- info, tuple_size(r) == 3, :require == :erlang.element(1, r) do
        :erlang.element(2, r)
      end
    end
  ...
  def error_notification(mod, func, _Args, {error, loc}) do
    ...
    errorStr =
      case errorSpec do
        {:badmatch, descr} ->
          descr1 = :io_lib.format(~c"~tP", [descr, 10])
          descrLength = :string.length(descr1)

          cond do
            descrLength > 50 ->
              descr2 = :string.slice(descr1, 0, 50)
              :io_lib.format(~c"{badmatch,~ts...}", [descr2])

            true ->
              :io_lib.format(~c"{badmatch,~ts}", [descr1])
          end

        {:test_case_failed, reason} ->
          case (try do
                  :io_lib.format(~c"{test_case_failed,~ts}", [reason])
                catch
                  :error, e -> {:EXIT, {e, __STACKTRACE__}}
                  :exit, e -> {:EXIT, e}
                  e -> e
            ...
            {:EXIT, _} ->
              :io_lib.format(~c"{test_case_failed,~tp}", [reason])
        ...
        other ->
          :io_lib.format(~c"~tP", [other, 5])
      end

    errorHtml = ~c"<font color=\"brown\">" ++ :ct_logs.escape_chars(errorStr) ++ ~c"</font>"
        ...
        case :ct_util.get_testdata({:comment, :erlang.group_leader()}) do
          ...
          comment ->
            commentHtml =
              ~c"<font color=\"green\">" ++
                ~c"(" ++
                ~c"</font>" ++ comment ++ ~c"<font color=\"green\">" ++ ~c")" ++ ~c"</font>"

            str = :io_lib.format(~c"~ts   ~ts", [errorHtml, commentHtml])
    ...
    printError = fn errorFormat, errorArgs ->
      ...
      :io.format(:ct_default_gl, ~c"~ts~n", [:lists.concat([div, errorStr2, div])])
    ...
    case loc do
      [{:ct_framework, :error_in_suite}] ->
        printError.(~c"Error in suite detected: ~ts", [errorStr])

      r when r == :unknown or r == :undefined ->
        printError.(~c"Error detected: ~ts", [errorStr])

      [{lastMod, lastFunc} | _] when errorStr == ~c"undef" ->
        printError.(~c"~w:~tw could not be executed~nReason: ~ts", [lastMod, lastFunc, errorStr])

      [{lastMod, lastFunc} | _] ->
        printError.(~c"~w:~tw failed~nReason: ~ts", [lastMod, lastFunc, errorStr])

      [{lastMod, lastFunc, lastLine} | _] ->
        printError.(~c"~w:~tw failed on line ~w~nReason: ~ts", [
          lastMod,
          lastFunc,
          lastLine,
          errorStr
  ...
  def get_suite(mod, :all) do
    case safe_apply_groups_0(mod, {:ok, []}) do
      ...
      {:error, {:bad_return, _Bad}} ->
        e = ~c"Bad return value from " ++ :erlang.atom_to_list(mod) ++ ~c":groups/0"
        [{:ct_framework, :error_in_suite, [[{:error, :erlang.list_to_atom(e)}]]}]

      {:error, {:bad_hook_return, bad}} ->
        e = ~c"Bad return value from post_groups/2 hook function"
      ...
      {:error, {:failed, exitReason}} ->
        case :ct_util.get_testdata({:error_in_suite, mod}) do
          :undefined ->
            errStr =
              :io_lib.format(~c"~n*** ERROR *** ~w:groups/0 failed: ~p~n", [mod, exitReason])
        ...
        reason = :erlang.list_to_atom(:erlang.atom_to_list(mod) ++ ~c":groups/0 failed")
    ...
    case safe_apply_all_0(mod) do
      {:ok, allTCs} ->
        try do
          ...
          {:error, error} ->
            [{:ct_framework, :error_in_suite, [[{:error, error}, {:force_failed, true}]]}]

          _, error ->
            [
              {:ct_framework, :error_in_suite,
               [[{:error, {error, __STACKTRACE__}}, {:force_failed, true}]]}
            ]
        else
          {:error, _} = error ->
            [{:ct_framework, :error_in_suite, [[error]]}]
      ...
      {:error, :undef} ->
        reason =
          case :code.which(mod) do
            :non_existing ->
              :erlang.list_to_atom(:erlang.atom_to_list(mod) ++ ~c" cannot be compiled or loaded")

            _ ->
              :erlang.list_to_atom(:erlang.atom_to_list(mod) ++ ~c":all/0 is missing")
          end
      ...
      {:error, {:bad_return, _Bad}} ->
        reason =
          :erlang.list_to_atom(
            ~c"Bad return value from " ++ :erlang.atom_to_list(mod) ++ ~c":all/0"
          )

        [{:ct_framework, :error_in_suite, [[{:error, reason}]]}]

      {:error, {:bad_hook_return, bad}} ->
        reason = :erlang.list_to_atom(~c"Bad return value from post_all/3 hook function")
        [{:ct_framework, :error_in_suite, [[{:error, {reason, bad}}]]}]

      {:error, {:failed, exitReason}} ->
        case :ct_util.get_testdata({:error_in_suite, mod}) do
          :undefined ->
            errStr = :io_lib.format(~c"~n*** ERROR *** ~w:all/0 failed: ~tp~n", [mod, exitReason])
            :io.format(:ct_default_gl, ~c"~ts", [errStr])
            :ct_util.set_testdata_async({{:error_in_suite, mod}, exitReason})
        ...
        reason = :erlang.list_to_atom(:erlang.atom_to_list(mod) ++ ~c":all/0 failed")
        [{:ct_framework, :error_in_suite, [[{:error, reason}]]}]
    ...
    case :lists.keymember(:sequence, 1, allTCs) do
      true ->
        case (try do
          ...
          {:EXIT, _} ->
            reason =
              :erlang.list_to_atom(:erlang.atom_to_list(mod) ++ ~c":sequences/0 is missing")
  ...
  defp save_seqs(mod, [{:sequence, seq} | tCs], seqs, all) do
    case :lists.keysearch(seq, 1, seqs) do
      ...
      false ->
        reason =
          :erlang.list_to_atom(
            :erlang.atom_to_list(seq) ++ ~c" is missing in " ++ :erlang.atom_to_list(mod)
          )
  ...
  defp check_private(seq, tCs, all) do
    ...
    cond do
      bad != [] ->
        reason =
          :io_lib.format(~c"regular test cases not allowed in sequence ~tp: ~tp", [seq, bad])
  ...
  defp check_multiple(mod, seq, tCs) do
    ...
    cond do
      bad != [] ->
        reason = :io_lib.format(~c"test cases found in multiple sequences: ~tp", [bad])
        throw({:error, :erlang.list_to_atom(:lists.flatten(reason))})
  ...
  def init_per_group(groupName, config) do
    :ct.comment(:io_lib.format(~c"start of ~tp", [groupName]))

    :ct_logs.log(~c"TEST INFO", ~c"init_per_group/2 for ~tw missing in suite, using default.", [
      groupName
    ])
  ...
  def end_per_group(groupName, _) do
    :ct.comment(:io_lib.format(~c"end of ~tp", [groupName]))

    :ct_logs.log(~c"TEST INFO", ~c"end_per_group/2 for ~tw missing in suite, using default.", [
      groupName
    ])
  ...
  def format_comment(comment) do
    ~c"<font color=\"green\">" ++ comment ++ ~c"</font>"
  end
