defmodule :m_cth_log_redirect do
  ...
  defp do_log(log, config) do
    :gen_server.call(:cth_log_redirect, {:log, log, config}, :infinity)
  end
  ...
  def handle_call(
    ...
    case logFunc do
      :tc_log ->
        :ct_logs.tc_log(category, 50, header, ~c"~ts", [string], [])

      :tc_log_async ->
        :ct_logs.tc_log_async(:sasl, 50, header, ~c"~ts", [string])
    end
  ...
  defp set_curr_func(currFunc, config) do
    :gen_server.call(:cth_log_redirect, {:set_curr_func, currFunc, config}, :infinity)
  end

  defp set_log_func(func) do
    :gen_server.call(:cth_log_redirect, {:set_logfunc, func}, :infinity)
  end

  def handle_remote_events(bool) do
    :gen_server.call(:cth_log_redirect, {:handle_remote_events, bool}, :infinity)
  end

  defp format_header(
         r_eh_state(curr_suite: :undefined, curr_group: :undefined, curr_func: :undefined)
       ) do
    :lists.flatten(:io_lib.format(~c"System report", []))
  end

  defp format_header(r_eh_state(curr_suite: suite, curr_group: :undefined, curr_func: :undefined)) do
    :lists.flatten(:io_lib.format(~c"System report during ~w", [suite]))
  end

  defp format_header(r_eh_state(curr_suite: suite, curr_group: :undefined, curr_func: tcOrConf)) do
    :lists.flatten(:io_lib.format(~c"System report during ~w:~tw/1", [suite, tcOrConf]))
  end
  ...
  defp format_header(r_eh_state(curr_suite: suite, curr_group: group, curr_func: conf))
    ...
    :lists.flatten(:io_lib.format(~c"System report during ~w:~w/2 for ~tw", [suite, conf, group]))
  end

  defp format_header(r_eh_state(curr_suite: suite, curr_group: group, parallel_tcs: true)) do
    :lists.flatten(:io_lib.format(~c"System report during ~tw in ~w", [group, suite]))
  end

  defp format_header(r_eh_state(curr_suite: suite, curr_group: group, curr_func: tC)) do
    :lists.flatten(:io_lib.format(~c"System report during ~w:~tw/1 in ~tw", [suite, tC, group]))
  end
end
