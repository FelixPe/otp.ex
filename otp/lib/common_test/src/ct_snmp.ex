defmodule :m_ct_snmp do
  ...
  def stop(config) do
    ...
    mgrDir = :filename.join(privDir, ~c"mgr")
    confDir = :filename.join(privDir, ~c"conf")
    dbDir = :filename.join(privDir, ~c"db")
  ...
  def set_info(config) do
    privDir = :test_server.lookup_config(:priv_dir, config)
    setLogFile = :filename.join(privDir, ~c"ct_snmp_set.log")
  ...
  defp setup_manager(true, mgrConfName, snmpConfName, config, iP) do
    ...
    engineID = :ct.get_config({mgrConfName, :engine_id}, ~c"mgrEngine")
    mgrDir = :filename.join(privDir, ~c"mgr")
    users = []
    agents = []
    usms = []
  ...
  defp log(privDir, agent, {_, _, varbinds}, newVarsAndVals) do
    ...
    file = :filename.join(privDir, ~c"ct_snmp_set.log")
    {:ok, fd} = :file.open(file, [:write, :append])
    :io.format(fd, ~c"~p.~n", [{agent, oldVarsAndVals, newVarsAndVals}])
    :ok = :file.close(fd)
    :ok
  end
  ...
  defp override_contexts(config, contexts) do
    ...
    file = :filename.join(dir, ~c"context.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_context_config(dir, ~c"", contexts)
  end
  ...
  defp override_sysinfo(config, sysInfo) do
    ...
    file = :filename.join(dir, ~c"standard.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_standard_config(dir, ~c"", sysInfo)
  end
  ...
  defp override_target_address(config, targetAddressConf) do
    ...
    file = :filename.join(dir, ~c"target_addr.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_target_addr_config(dir, ~c"", targetAddressConf)
  end
  ...
  defp override_target_params(config, targetParamsConf) do
    ...
    file = :filename.join(dir, ~c"target_params.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_target_params_config(dir, ~c"", targetParamsConf)
  end
  ...
  defp override_notify(config, notifyConf) do
    ...
    file = :filename.join(dir, ~c"notify.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_notify_config(dir, ~c"", notifyConf)
  end
  ...
  defp override_usm(config, usmConf) do
    ...
    file = :filename.join(dir, ~c"usm.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_usm_config(dir, ~c"", usmConf)
  end
  ...
  defp override_community(config, communityConf) do
    ...
    file = :filename.join(dir, ~c"community.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_community_config(dir, ~c"", communityConf)
  end
  ...
  defp override_vacm(config, vacmConf) do
    ...
    file = :filename.join(dir, ~c"vacm.conf")
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_vacm_config(dir, ~c"", vacmConf)
  end
