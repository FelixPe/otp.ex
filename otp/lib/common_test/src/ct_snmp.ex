defmodule :m_ct_snmp do
  use Bitwise
  require Record
  Record.defrecord(:r_varbind, :varbind, oid: :undefined,
                                   variabletype: :undefined, value: :undefined,
                                   org_index: :undefined)
  Record.defrecord(:r_ivarbind, :ivarbind, status: :noError,
                                    mibentry: :undefined, varbind: :undefined)
  Record.defrecord(:r_asn1_type, :asn1_type, bertype: :undefined,
                                     lo: :undefined, hi: :undefined,
                                     assocList: [], imported: false,
                                     aliasname: :undefined, implied: false,
                                     display_hint: :undefined)
  Record.defrecord(:r_table_info, :table_info, nbr_of_cols: :undefined,
                                      defvals: [], status_col: :undefined,
  ...
  Record.defrecord(:r_me, :me, oid: :undefined,
                              entrytype: :undefined, aliasname: :undefined,
                              asn1_type: :undefined, access: :undefined,
                              mfa: :undefined, imported: false, assocList: [],
                              description: :undefined, units: :undefined)
  Record.defrecord(:r_trap, :trap, trapname: :undefined,
                                enterpriseoid: :undefined,
                                specificcode: :undefined,
                                oidobjects: :undefined, description: :undefined)
  Record.defrecord(:r_notification, :notification, trapname: :undefined,
                                        oid: :undefined, oidobjects: :undefined,
                                        description: :undefined)
  Record.defrecord(:r_mib, :mib, misc: [],
                               mib_format_version: '3.3', name: '',
                               module_identity: :undefined, mes: [],
                               asn1_types: [], traps: [], variable_infos: [],
                               table_infos: [], imports: :undefined)
  Record.defrecord(:r_module_identity, :module_identity, last_updated: :undefined,
                                           organization: :undefined,
                                           contact_info: :undefined,
                                           description: :undefined,
                                           revisions: :undefined)
  Record.defrecord(:r_message, :message, version: :undefined,
                                   community: :undefined, data: :undefined)
  Record.defrecord(:r_v3_hdr, :v3_hdr, msgID: :undefined,
                                  msgMaxSize: :undefined, msgFlags: :undefined,
                                  msgSecurityModel: :undefined,
                                  msgSecurityParameters: :undefined,
                                  hdr_size: :undefined)
  Record.defrecord(:r_scopedPdu, :scopedPdu, contextEngineID: :undefined,
                                     contextName: :undefined, data: :undefined)
  Record.defrecord(:r_usmSecurityParameters, :usmSecurityParameters, msgAuthoritativeEngineID: :undefined,
  ...
  Record.defrecord(:r_pdu, :pdu, type: :undefined,
                               request_id: :undefined, error_status: :undefined,
                               error_index: :undefined, varbinds: :undefined)
  Record.defrecord(:r_trappdu, :trappdu, enterprise: :undefined,
                                   agent_addr: :undefined,
                                   generic_trap: :undefined,
                                   specific_trap: :undefined,
                                   time_stamp: :undefined, varbinds: :undefined)
  Record.defrecord(:r_snmp_variables, :snmp_variables, name: :undefined,
                                          value: :undefined)
  Record.defrecord(:r_snmp_incr_timer, :snmp_incr_timer, wait_for: :timer.seconds(5),
                                           factor: 2, incr: 0,
                                           max_retries: :infinity)
  Record.defrecord(:r_snmpa_notification_delivery_info, :snmpa_notification_delivery_info, tag: :undefined,
                                                            mod: :undefined,
                                                            extra: :undefined)
  ...
  def start(config, mgrAgentConfName, snmpAppConfName) do
    startManager = :ct.get_config({mgrAgentConfName,
                                     :start_manager},
                                    true)
    startAgent = :ct.get_config({mgrAgentConfName,
                                   :start_agent},
                                  false)
    sysName = :ct.get_config({mgrAgentConfName,
                                :agent_sysname},
                               'ct_test')
    {:ok, hostName} = :inet.gethostname()
    {:ok, addr} = :inet.getaddr(hostName, :inet)
    iP = :erlang.tuple_to_list(addr)
    agentManagerIP = :ct.get_config({mgrAgentConfName,
                                       :agent_manager_ip},
                                      iP)
    prepare_snmp_env()
    setup_agent(startAgent, mgrAgentConfName,
                  snmpAppConfName, config, sysName, agentManagerIP, iP)
    setup_manager(startManager, mgrAgentConfName,
                    snmpAppConfName, config, agentManagerIP)
  ...
  def get_values(agent, oids, mgrAgentConfName) do
    ...
    {:ok, snmpReply, _} = :snmpm.sync_get2(uid,
                                             target_name(agent), oids)
    snmpReply
  end

  def get_next_values(agent, oids, mgrAgentConfName) do
    [uid | _] = agent_conf(agent, mgrAgentConfName)
    {:ok, snmpReply, _} = :snmpm.sync_get_next2(uid,
                                                  target_name(agent), oids)
    snmpReply
  end
  ...
  def set_values(agent, varsAndVals, mgrAgentConfName, config) do
    privDir = :test_server.lookup_config(:priv_dir, config)
    [uid | _] = agent_conf(agent, mgrAgentConfName)
    oids = :lists.map(fn {oid, _, _} ->
                           oid
                      end,
                        varsAndVals)
    targetName = target_name(agent)
    {:ok, snmpGetReply, _} = :snmpm.sync_get2(uid,
                                                targetName, oids)
    {:ok, snmpSetReply, _} = :snmpm.sync_set2(uid,
                                                targetName, varsAndVals)
  ...
  def register_agents(mgrAgentConfName, managedAgents) do
    case (setup_managed_agents(mgrAgentConfName,
                                 managedAgents)) do
      :ok ->
        snmpVals = :ct.get_config(mgrAgentConfName)
        oldAgents = :ct.get_config({mgrAgentConfName,
                                      :managed_agents},
                                     [])
        newSnmpVals = :lists.keystore(:managed_agents, 1,
                                        snmpVals,
                                        {:managed_agents,
  ...
  def unregister_users(mgrAgentConfName) do
    users = (for {id,
                    _} <- :ct.get_config({mgrAgentConfName, :users}, []) do
               id
             end)
  ...
  def unregister_users(mgrAgentConfName, users) do
    ...
    allUsers = :ct.get_config({mgrAgentConfName, :users},
                                [])
    remainingUsers = :lists.filter(fn {id, _} ->
                                        not :lists.member(id, users)
                                   end,
                                     allUsers)
    newSnmpVals = :lists.keyreplace(:users, 1, snmpVals,
  ...
  def unregister_agents(mgrAgentConfName) do
    managedAgents = (for {agentName,
                            _} <- :ct.get_config({mgrAgentConfName,
                                                    :managed_agents},
                                                   []) do
                       agentName
                     end)
  ...
  def unregister_agents(mgrAgentConfName, managedAgents) do
    ...
    allAgents = :ct.get_config({mgrAgentConfName,
                                  :managed_agents},
                                 [])
    remainingAgents = :lists.filter(fn {name, _} ->
                                         not :lists.member(name, managedAgents)
                                    end,
                                      allAgents)
    newSnmpVals = :lists.keyreplace(:managed_agents, 1,
                                      snmpVals,
  ...
  def unregister_usm_users(mgrAgentConfName) do
    usmUsers = (for {id,
                       _} <- :ct.get_config({mgrAgentConfName, :usm_users},
                                              []) do
                  id
                end)
  ...
  def unregister_usm_users(mgrAgentConfName, usmUsers) do
    ...
    allUsmUsers = :ct.get_config({mgrAgentConfName,
                                    :usm_users},
                                   [])
    remainingUsmUsers = :lists.filter(fn {id, _} ->
  ...
  defp setup_manager(true, mgrConfName, snmpConfName, config, iP) do
    privDir = :test_server.lookup_config(:priv_dir, config)
    maxMsgSize = :ct.get_config({mgrConfName,
                                   :max_msg_size},
                                  484)
    ...
    usms = []
    :ok = make_dir(mgrDir)
    :snmp_config.write_manager_snmp_files(mgrDir, iP, port,
                                            maxMsgSize, engineID, users, agents,
                                            usms)
    snmpEnv = merge_snmp_conf([{:config,
                                  [{:dir, mgrDir}, {:db_dir, mgrDir},
                                                       {:verbosity, :trace}]},
                                   {:server, [{:verbosity, :trace}]}, {:net_if,
                                                                         [{:verbosity,
                                                                             :trace}]},
                                                                          {:versions,
                                                                             [:v1,
  ...
  defp setup_agent(true, agentConfName, snmpConfName, config,
            sysName, managerIP, agentIP) do
    :ok = start_application(:mnesia)
    privDir = :test_server.lookup_config(:priv_dir, config)
    vsns = :ct.get_config({agentConfName, :agent_vsns},
                            [:v2])
    trapUdp = :ct.get_config({agentConfName,
                                :agent_trap_udp},
                               5000)
    agentUdp = :ct.get_config({agentConfName, :agent_udp},
                                4000)
    notifType = :ct.get_config({agentConfName,
                                  :agent_notify_type},
                                 :trap)
    secType = :ct.get_config({agentConfName,
                                :agent_sec_type},
                               :none)
    passwd = :ct.get_config({agentConfName, :agent_passwd},
                              '')
    agentEngineID = :ct.get_config({agentConfName,
                                      :agent_engine_id},
                                     'agentEngine')
    agentMaxMsgSize = :ct.get_config({agentConfName,
                                        :agent_max_msg_size},
                                       484)
    confDir = :filename.join(privDir, 'conf')
    dbDir = :filename.join(privDir, 'db')
    :ok = make_dir(confDir)
    :ok = make_dir(dbDir)
    :snmp_config.write_agent_snmp_files(confDir, vsns,
                                          managerIP, trapUdp, agentIP, agentUdp,
                                          sysName, notifType, secType, passwd,
                                          agentEngineID, agentMaxMsgSize)
    override_default_configuration(config, agentConfName)
    snmpEnv = merge_snmp_conf([{:db_dir, dbDir}, {:config,
                                                    [{:dir, confDir},
                                                         {:verbosity, :trace}]},
                                                     {:agent_type, :master},
                                                         {:agent_verbosity,
  ...
  defp merge_snmp_conf([def__ = {key, defList = [p | _]} | defParams],
            userParams)
      when is_tuple(p) do
  ...
  defp manager_register(true, mgrAgentConfName) do
    agents = :ct.get_config({mgrAgentConfName,
                               :managed_agents},
                              [])
  ...
  defp setup_managed_agents(agentConfName, agents) do
    fun = fn {agentName,
                [uid, agentIp, agentUdpPort, agentConf0]} ->
               newAgentIp = (case (agentIp) do
               ...
               agentConf = (case (:lists.keymember(:engine_id, 1,
                                                     agentConf0)) do
                              true ->
                                agentConf0
                              false ->
                                defaultEngineID = :ct.get_config({agentConfName,
                                                                    :agent_engine_id},
                                                                   'agentEngine')
                                [{:engine_id, defaultEngineID} | agentConf0]
                            end)
               :snmpm.register_agent(uid, target_name(agentName),
                                       [{:address, newAgentIp}, {:port,
                                                                   agentUdpPort} |
                                                                    agentConf])
          end
    while_ok(fun, agents)
  ...
  defp log(privDir, agent, {_, _, varbinds},
            newVarsAndVals) do
    fun = fn r_varbind(oid: oid, variabletype: type,
                 value: value) ->
               {oid, type, value}
          end
    ...
    :io.format(fd, '~p.~n',
                 [{agent, oldVarsAndVals, newVarsAndVals}])
    :ok = :file.close(fd)
    :ok
  end
  ...
  defp del_dir(dir) do
    ...
    fullPathFiles = :lists.map(fn file ->
  ...
  defp override_default_configuration(config, mgrAgentConfName) do
    override_contexts(config,
                        :ct.get_config({mgrAgentConfName, :agent_contexts},
                                         :undefined))
    override_community(config,
                         :ct.get_config({mgrAgentConfName, :agent_community},
                                          :undefined))
    override_sysinfo(config,
                       :ct.get_config({mgrAgentConfName, :agent_sysinfo},
                                        :undefined))
    override_vacm(config,
                    :ct.get_config({mgrAgentConfName, :agent_vacm},
                                     :undefined))
    override_usm(config,
                   :ct.get_config({mgrAgentConfName, :agent_usm},
                                    :undefined))
    override_notify(config,
                      :ct.get_config({mgrAgentConfName, :agent_notify_def},
                                       :undefined))
  ...
  defp override_contexts(config, contexts) do
    dir = :filename.join(:test_server.lookup_config(:priv_dir,
                                                      config),
                           'conf')
    file = :filename.join(dir, 'context.conf')
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_context_config(dir, '',
                                                    contexts)
  end
  ...
  defp override_sysinfo(config, sysInfo) do
    ...
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_standard_config(dir, '',
                                                     sysInfo)
  end
  ...
  defp override_target_address(config, targetAddressConf) do
    ...
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_target_addr_config(dir,
                                                        '', targetAddressConf)
  end
  ...
  defp override_target_params(config, targetParamsConf) do
    ...
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_target_params_config(dir,
                                                          '', targetParamsConf)
  end
  ...
  defp override_notify(config, notifyConf) do
    ...
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_notify_config(dir, '',
                                                   notifyConf)
  end
  ...
  defp override_usm(config, usmConf) do
    ...
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_usm_config(dir, '',
                                                usmConf)
  end
  ...
  defp override_community(config, communityConf) do
    ...
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_community_config(dir, '',
                                                      communityConf)
  end
  ...
  defp override_vacm(config, vacmConf) do
    ...
    :ok = delete_file(file)
    :ok = :snmp_config.write_agent_vacm_config(dir, '',
                                                 vacmConf)
  end
