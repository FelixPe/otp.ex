defmodule :m_ct_slave do
  use Bitwise
  require Record
  Record.defrecord(:r_options, :options, username: :undefined,
                                   password: :undefined,
                                   ...
                                   erl_flags: :undefined, env: :undefined,
                                   ssh_port: :undefined, ssh_opts: :undefined,
  ...
  defp fetch_options(options) do
    ...
    bootTimeout = get_option_value(:boot_timeout, options,
                                     3)
    initTimeout = get_option_value(:init_timeout, options,
                                     1)
    startupTimeout = get_option_value(:startup_timeout,
                                        options, 1)
    startupFunctions = get_option_value(:startup_functions,
                                          options, [])
    monitor = get_option_value(:monitor_master, options,
                                 false)
    killIfFail = get_option_value(:kill_if_fail, options,
                                    true)
    ...
    stopTimeout = get_option_value(:stop_timeout, options,
                                     5)
    r_options(username: userName, password: password,
        ...
        erl_flags: erlFlags, env: envVars, ssh_port: sSHPort,
        ssh_opts: sSHOpts, stop_timeout: stopTimeout)
  end
  ...
  defp do_start(host, node, options) do
    eNode = enodename(host, node)
    functions = :lists.append([[{:ct_slave, :slave_started,
                                   [eNode, self()]}],
                                   r_options(options, :startup_functions), [{:ct_slave,
                                                                       :slave_ready,
                                                                       [eNode,
                                                                            self()]}]])
    functions2 = (cond do
    ...
    result = (case (wait_for_node_alive(eNode,
                                          bootTimeout)) do
                :pong ->
    ...
    _ = (case (result) do
           {:ok, ^eNode} ->
             :ok
           {:error, timeout, ^eNode}
               when (timeout == :init_timeout or timeout == :startup_timeout) and r_options(options, :kill_if_fail)
                    ->
             do_stop(eNode)
           _ ->
             :ok
  ...
  defp gethostname() do
    hostname = (case (:net_kernel.longnames()) do
                  true ->
                    :net_adm.localhost()
                  _ ->
  ...
  defp get_cmd(node, flags) do
    ...
    'erl -detached -noinput -setcookie ' ++ :erlang.atom_to_list(cookie) ++ long_or_short() ++ :erlang.atom_to_list(node) ++ ' ' ++ flags
  end
  ...
  defp spawn_remote_node(host, node, options) do
    r_options(username: username, password: password,
        erl_flags: erlFlags, env: env, ssh_port: maybeSSHPort,
        ssh_opts: sSHOpts) = options
    sSHPort = (case (maybeSSHPort) do
                 [] ->
                   22
                 a ->
                   a
               end)
    sSHOptions = (case ({username, password}) do
                    {[], []} ->
                      []
                    {_, []} ->
                      [{:user, username}]
    ...
    {:ok,
       sSHConnRef} = :ssh.connect(:erlang.atom_to_list(host),
                                    sSHPort, sSHOptions)
    {:ok,
       sSHChannelId} = :ssh_connection.session_channel(sSHConnRef,
                                                         :infinity)
    ssh_setenv(sSHConnRef, sSHChannelId, env)
    :ssh_connection.exec(sSHConnRef, sSHChannelId,
                           get_cmd(node, erlFlags), :infinity)
  end

  defp ssh_setenv(sSHConnRef, sSHChannelId, [{var, value} | vars])
      when (is_list(var) and is_list(value)) do
    :success = :ssh_connection.setenv(sSHConnRef,
                                        sSHChannelId, var, value, :infinity)
  ...
  defp do_stop(eNode, options) do
    {cover,
       mainCoverNode} = (case (:test_server.is_cover()) do
                           true ->
