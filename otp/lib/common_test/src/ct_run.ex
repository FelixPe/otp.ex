defmodule :m_ct_run do
    ...
    args =
      case :application.get_env(
        ...
        {:ok, envStartOpts} ->
          ...
          :io.format(:user, ~c"~n-------------------- START ARGS --------------------~n", [])
          :io.format(:user, ~c"--- Init args:~n~tp~n", [flagFilter.(init)])
          :io.format(:user, ~c"--- CT args:~n~tp~n", [flagFilter.(ctArgs)])
          envArgs = opts2args(envStartOpts)
          :io.format(:user, ~c"--- Env opts -> args:~n~tp~n   =>~n~tp~n", [envStartOpts, envArgs])
          merged = merge_arguments(ctArgs ++ envArgs)
          :io.format(:user, ~c"--- Merged args:~n~tp~n", [flagFilter.(merged)])
          :io.format(:user, ~c"----------------------------------------------------~n~n", [])
          merged
  ...
  defp script_start(args) do
    ...
    case :ct_repeat.loop_test(:script, args) do
      false ->
        ...
        cTVsn =
          case :filename.basename(:code.lib_dir(:common_test)) do
            cTBase when is_list(cTBase) ->
              case :string.lexemes(cTBase, ~c"-") do
                [~c"common_test", vsn] ->
                  ~c" v" ++ vsn
        ...
        :io.format(~c"~nCommon Test~s starting (cwd is ~ts)~n~n", [cTVsn, cwd])
        self = self()
        ...
        receive do
          {:EXIT, ^pid, reason} ->
            case reason do
              ...
              _ ->
                :io.format(
                  ~c"Test run crashed! This could be an internal error - please report!\n\n~tp\n\n\n",
                  [reason]
                )
    ...
    {autoCompile, includeDirs} =
      case :proplists.get_value(
        ...
        :undefined ->
          ...
          case :os.getenv(~c"CT_INCLUDE_PATH") do
            false ->
        ...
        case (try do
          ...
          _ ->
            case (try do
              ...
              _ ->
                :ok = :file.set_cwd(cwd)
                :io.format(~c"Logs in ~ts refreshed!~n~n", [logDir1])
                :timer.sleep(500)
                :ok
            end
        end
    ...
    case {:proplists.get_value(:dir, args), :proplists.get_value(:suite, args),
      ...
      {:undefined, :undefined, _} ->
        cond do
          ...
          true ->
            {:ok, dir} = :file.get_cwd()
            :io.format(~c"ct_run -dir ~ts~n~n", [dir])
    ...
    case :erlang.whereis(:ct_util_server) do
      :undefined ->
        ...
        case :file.open(
          ...
          {:error, reason} ->
            :io.format(
              ~c"CT failed to install configuration data. Please verify that the log directory exists and that write permission is set.\n\n",
              []
            )
  ...
  def variables_file_name(dir) do
    :filename.join(dir, ~c"variables-" ++ :erlang.atom_to_list(node()))
  end
  ...
  defp run_test1(startOpts) when is_list(startOpts) do
    case :proplists.get_value(:refresh_logs, startOpts) do
      :undefined ->
        ...
        {:ok, cwd} = :file.get_cwd()
        :io.format(~c"~nCommon Test starting (cwd is ~ts)~n~n", [cwd])
  ...
  defp run_testspec2(file)
    ...
    case :file.read_file_info(file) do
      {:ok, _} ->
        exit(~c"Bad argument, use ct:run_test([{spec," ++ file ++ ~c"}])")

      _ ->
        exit(
          ~c"Bad argument, list of tuples expected, use ct:run_test/1 for test specification files"
        )
    end
  end
        ...
        case (try do
          ...
          _ ->
            case (try do
              ...
              _ ->
                :ok = :file.set_cwd(cwd)
                :io.format(~c"Logs in ~ts refreshed!~n", [logDir])
                :ok
            end
        end
    end
  ...
  defp groups_and_cases(gOrGs, cs)
    ...
    cond do
      cs == :undefined or cs == [] or cs == :all or cs == [:all] or cs == [~c"all"] ->
        [{gOrGs, :all}]
  ...
  defp do_run(tests, skip, opts, args)
    ...
    r_opts(label: label, profile: profile, verbosity: vLvls, stylesheet: customStylesheet) = opts
    ...
    case :code.which(:test_server) do
      ...
      _ ->
        case :os.getenv(~c"TEST_SERVER_FRAMEWORK") do
          false ->
            :os.putenv(~c"TEST_SERVER_FRAMEWORK", ~c"ct_framework")
            :os.putenv(~c"TEST_SERVER_FRAMEWORK_NAME", ~c"common_test")

          ~c"ct_framework" ->
            :ok

          other ->
            :erlang.display(:erlang.list_to_atom(~c"Note: TEST_SERVER_FRAMEWORK = " ++ other))
        end

        verbosity = add_verbosity_defaults(vLvls)

        case :ct_util.start(r_opts(opts, :logdir), verbosity, customStylesheet) do
          {:error, :interactive_mode} ->
            :io.format(
              ~c"CT is started in interactive mode. To exit this mode, run ct:stop_interactive().\nTo enter the interactive mode again, run ct:start_interactive()\n\n",
              []
            )
      ...
      conns ->
        case :lists.member(:all, conns) do
          true ->
            ...
            :ct_logs.log(~c"Silent connections", ~c"~tp", [conns1])

          false ->
            :ct_util.override_silence_connections(conns)
            :ct_logs.log(~c"Silent connections", ~c"~tp", [conns])
        end
    end
  ...
  defp possibly_spawn(true, tests, skip, opts) do
    ...
    supervisor = fn ->
      ...
      receive do
        {:EXIT, ^testRunPid, {:ok, testResult}} ->
          :io.format(:user, ~c"~nCommon Test returned ~tp~n~n", [testResult])
    ...
    :io.format(:user, ~c"~nTest control handed over to process ~w~n~n", [supPid])
    supPid
  end

  defp auto_compile(testSuites) do
    :io.format(~c"~nCommon Test: Running make in test directories...~n")
    ...
    userInclude =
      case :application.get_env(
        ...
        {:ok, userInclDirs} when length(userInclDirs) > 0 ->
          :io.format(~c"Including the following directories:~n")

          for userInclDir <- userInclDirs do
            :io.format(~c"~tp~n", [userInclDir])
            {:i, userInclDir}
          end
      ...
      :lists.flatmap(
        fn {testDir, suite} = tS ->
          case run_make(:suites, testDir, suite, userInclude, [:nowarn_export_all]) do
            ...
            {:error, _} ->
              [{tS, [:filename.join(testDir, ~c"*_SUITE")]}]
      ...
      case locate_test_dir(dir, suite) do
          ...
          cond do
            ...
            true ->
              beam =
                :filename.join(
                  testDir,
                  :erlang.atom_to_list(suite) ++ ~c".beam"
                )
                ...
                false ->
                  case :code.is_loaded(suite) do
                    ...
                    false ->
                      ...
                      :io.format(:user, ~c"Suite ~w not found in directory ~ts~n", [
                        suite,
                        testDir
                      ])
        ...
        {:error, _Reason} ->
          case :code.is_loaded(suite) do
            ...
            false ->
              :io.format(:user, ~c"Directory ~ts is invalid~n", [dir])
  ...
  defp save_make_errors(errors) do
    ...
    :ct_logs.log(~c"MAKE RESULTS", ~c"Error compiling or locating the following suites: ~n~p", [
      suites
    ])
                ...
                case groupOrGroups do
                  [groupList] when is_list(groupList) ->
                    {grpNames, props} =
                      :lists.foldl(
                        fn
                          {grpName, _} = grSpec, {grpNames, props} ->
                            {:lists.append(
                               grpNames,
                               [grpName]
                             ),
                    ...
                    [:ct_groups.make_conf(testDir, suite, [grpNames], [{:override, props}], tCs)]
      ...
      true ->
        ...
        after
          15000 ->
            ...
            :io.format(~c"... timeout - continuing!!\n")
            true
        end
  ...
  defp do_run_test(tests, skip, opts0) do
    case check_and_add(tests, [], []) do
      {:ok, addedToPath} ->
        ...
        {:ok, cwd} = :file.get_cwd()
        :io.format(~c"~nCWD set to: ~tp~n", [cwd])

        cond do
          noOfCases == :unknown ->
            :io.format(~c"~nTEST INFO: ~w test(s), ~w suite(s)~n~n", [noOfTests, noOfSuites])
            :ct_logs.log(~c"TEST INFO", ~c"~w test(s), ~w suite(s)", [noOfTests, noOfSuites])

          true ->
            :io.format(~c"~nTEST INFO: ~w test(s), ~w case(s) in ~w suite(s)~n~n", [
              noOfTests,
              noOfCases,
              noOfSuites
            ])

            :ct_logs.log(~c"TEST INFO", ~c"~w test(s), ~w case(s) in ~w suite(s)", [
              noOfTests,
              noOfCases,
              noOfSuites
            ])
        ...
        :ct_logs.log(
          ~c"TEST INFO",
          ~c"Timetrap time multiplier = ~w~nTimetrap scaling enabled = ~w",
          [multTT, scaleTT]
        )
        ...
        case :ct_util.get_testdata(:severe_error) do
          ...
          severeError ->
            :ct_logs.log(~c"SEVERE ERROR", ~c"~tp\n", [severeError])
            exit(severeError)
        end
  ...
  defp start_cover(
    ...
    :ct_logs.log(
      ~c"COVER INFO",
      ~c"Using cover specification file: ~ts~nApp: ~w~nLocal only: ~w~nCross cover: ~w~nIncluding ~w modules~nExcluding ~w modules",
    ...
    :ct_logs.log(~c"COVER INFO", ~c"Compilation completed - test_server cover info: ~tp", [
      tsCoverInfo
    ])

    cond do
      covNodes != [] and covNodes != :undefined ->
        :ct_logs.log(~c"COVER INFO", ~c"Nodes included in cover session: ~tw", [covNodes])
    ...
    :lists.foreach(
      fn imp ->
        case :cover.import(imp) do
          ...
          {:error, reason} ->
            :ct_logs.log(~c"COVER INFO", ~c"Importing cover data from: ~ts fails! Reason: ~tp", [
              imp,
              reason
            ])
        end
  ...
  defp maybe_stop_cover(r_opts(coverspec: covData), tsCoverInfo, logDir) do
    ...
    case covExport do
      ...
      _ ->
        :ct_logs.log(~c"COVER INFO", ~c"Exporting cover data to ~tp", [covExport])
        :cover.export(covExport)
    end

    :ct_logs.log(~c"COVER INFO", ~c"Analysing cover data to ~tp", [logDir])
    :test_server_ctrl.cover_analyse(tsCoverInfo, logDir)
    :ct_logs.log(~c"COVER INFO", ~c"Analysis completed.", [])
    :ok
  end
  ...
  defp add_jobs([{testDir, suites, :all} | tests], skip, opts, cleanUp)
       when is_list(suites) do
    name = get_name(testDir) ++ ~c".suites"
  ...
  defp add_jobs([{testDir, suite, :all} | tests], skip, opts, cleanUp) do
    case maybe_interpret(suite, :all, opts) do
      :ok ->
        name = get_name(testDir) ++ ~c"." ++ :erlang.atom_to_list(suite)
  ...
  defp add_jobs([{testDir, suite, confs} | tests], skip, opts, cleanUp)
        ...
        [conf] ->
          case group.(conf) do
            grName when is_atom(grName) ->
              ~c"." ++ :erlang.atom_to_list(grName) ++ tCTestName.(testCases.(conf))

            _ ->
              ~c".groups" ++ tCTestName.(testCases.(conf))
          end
    ...
    testName = get_name(testDir) ++ ~c"." ++ :erlang.atom_to_list(suite) ++ grTestName
  ...
  defp add_jobs([{testDir, suite, cases} | tests], skip, opts, cleanUp)
    ...
    case maybe_interpret(suite, cases1, opts) do
      :ok ->
        name = get_name(testDir) ++ ~c"." ++ :erlang.atom_to_list(suite) ++ ~c".cases"
  ...
  defp add_jobs([{testDir, suite, case__} | tests], skip, opts, cleanUp)
    ...
    case maybe_interpret(suite, case__, opts) do
      :ok ->
        name =
          get_name(testDir) ++
            ~c"." ++ :erlang.atom_to_list(suite) ++ ~c"." ++ :erlang.atom_to_list(case__)
  ...
  defp skiplist(dir, [{dir, :all, cmt} | skip]) do
    ss = :filelib.wildcard(:filename.join(dir, ~c"*_SUITE.beam"))

    for s <- ss do
      {:erlang.list_to_atom(:filename.basename(s, ~c".beam")), cmt}
  ...
  defp run_make(targets, testDir0, mod, userInclude, cOpts) do
    case locate_test_dir(testDir0, mod) do
      {:ok, testDir} ->
        ...
        ctInclude = get_dir(:common_test, ~c"include")
        xmerlInclude = get_dir(:xmerl, ~c"include")
        ...
        case result do
          ...
          {:EXIT, reason} ->
            :io.format(~c"{error,{make_crashed,~tp}\n", [reason])
            {:error, {:make_crashed, testDir, reason}}

          {:error, modInfo} ->
            :io.format(~c"{error,make_failed}\n", [])
      ...
      {:error, _} ->
        :io.format(~c"{error,{invalid_directory,~tp}}\n", [testDir0])
        {:error, {:invalid_directory, testDir0}}
    end
  end
    ...
    _ =
      for case__ <- cases, is_atom(case__) do
        try do
          ...
          _, _Error ->
            :io.format(:user, ~c"Invalid breakpoint: ~w:~tw/1~n", [suite, case__])
        else
          _ ->
            :ok
        end
  ...
  defp log_ts_names(specs) do
    ...
    :ct_logs.log(~c"Test Specification file(s)", ~c"~ts", [:lists.flatten(list)])
  end
  ...
  defp handle_arg(:merge, {:event_handler_init, elems}, [
    ...
    [
      {:event_handler_init, prevElems ++ [~c"add" | elems]}
      | merged
    ]
  end

  defp handle_arg(:merge, {:userconfig, elems}, [{:userconfig, prevElems} | merged]) do
    [{:userconfig, prevElems ++ [~c"add" | elems]} | merged]
  end
  ...
  defp ct_hooks_args2opts([cTH, arg, prio, ~c"and" | rest], acc)
       when arg != ~c"and" do
    ct_hooks_args2opts(
      rest,
      [
  ...
  defp ct_hooks_args2opts([cTH, arg, ~c"and" | rest], acc) do
    ct_hooks_args2opts(
      rest,
      [
  ...
  defp ct_hooks_args2opts([cTH], acc) do
    ct_hooks_args2opts([cTH, ~c"and"], acc)
  end

  defp ct_hooks_args2opts([cTH, ~c"and" | rest], acc) do
    ct_hooks_args2opts(
      rest,
  ...
  defp ct_hooks_args2opts([cTH, args], acc) do
    ct_hooks_args2opts([cTH, args, ~c"and"], acc)
  end

  defp ct_hooks_args2opts([cTH, args, prio], acc) do
    ct_hooks_args2opts([cTH, args, prio, ~c"and"], acc)
  end
  ...
  defp parse_cth_args(string) do
    try do
      ...
      {:ok, toks, _} = :erl_scan.string(string ++ ~c".")
  ...
  defp event_handler_init_args2opts([eH, arg, ~c"and" | eHs]) do
    [
      {:erlang.list_to_atom(eH), :lists.flatten(:io_lib.format(~c"~ts", [arg]))}
      | event_handler_init_args2opts(eHs)
    ]
  end
  ...
  defp verbosity_args2opts(args) do
    case :proplists.get_value(:verbosity, args) do
      ...
      vArgs ->
        getVLvls = fn
          ~c"and", {:new, soFar} when is_list(soFar) ->
            {:new, soFar}

          ~c"and", {lvl, soFar} when is_list(soFar) ->
        ...
        _ =
          cond do
            dir != abs ->
              ...
              :io.format(:user, ~c"Converting ~tp to ~tp and re-inserting with add_pathz/1~n", [
                dir,
                abs
              ])
        ...
        _ =
          cond do
            dir != abs ->
              ...
              :io.format(:user, ~c"Converting ~tp to ~tp and re-inserting with add_patha/1~n", [
                dir,
                abs
              ])
  ...
  defp make_abs1([~c".." | dirs], [_Dir | path]) do
    make_abs1(dirs, path)
  end

  defp make_abs1([~c"." | dirs], path) do
    make_abs1(dirs, path)
  end
    ...
    :lists.flatmap(
      fn
          ...
          strs =
            :lists.map(
              fn
                {cBM, cfgStr = [x | _]}
                when is_integer(x) ->
                  [:erlang.atom_to_list(cBM), cfgStr, ~c"and"]
        ...
        {:group, gs} when is_list(gs) ->
          lOfGStrs =
            for g <- gs do
              :lists.flatten(:io_lib.format(~c"~tw", [g]))
            end
        ...
        {:event_handler, {eH, arg}} when is_atom(eH) ->
          argStr = :lists.flatten(:io_lib.format(~c"~tp", [arg]))
        ...
        {:event_handler, {eHs, arg}} when is_list(eHs) ->
          ...
          strs =
            :lists.flatmap(
              fn eH ->
                [:erlang.atom_to_list(eH), argStr, ~c"and"]
              end,
              eHs
            )
        ...
        {:verbosity, vLvls} when is_list(vLvls) ->
          vLvlArgs =
            :lists.flatmap(
              fn
                {:"$unspecified", lvl} ->
                  [:erlang.integer_to_list(lvl), ~c"and"]

                {cat, lvl} ->
                  [:erlang.atom_to_list(cat), :erlang.integer_to_list(lvl), ~c"and"]
        ...
        {:ct_hooks, cTHs} when is_list(cTHs) ->
          :io.format(:user, ~c"ct_hooks: ~tp", [cTHs])
            ...
            :lists.flatmap(
              fn
                ...
                cTH when is_atom(cTH) ->
                  [:erlang.atom_to_list(cTH), ~c"and"]
              end,
              cTHs
            )

          [_LastAnd | strsR] = :lists.reverse(strs)
          :io.format(:user, ~c"return: ~tp", [:lists.reverse(strsR)])
        ...
        case :file.consult(traceSpec) do
          {:ok, terms} ->
            case (try do
              ...
              {_, error} ->
                :io.format(~c"Warning! Tracing not started. Reason: ~tp~n~n", [error])
                false
            end

          {_, error} ->
            :io.format(~c"Warning! Tracing not started. Reason: ~ts~n~n", [
