defmodule :m_ct_run do
  ...
  Record.defrecord(:r_opts, :opts, label: :undefined,
                                profile: :undefined, shell: :undefined,
                                cover: :undefined, cover_stop: :undefined,
                                coverspec: :undefined, step: :undefined,
                                logdir: :undefined, logopts: [],
                                basic_html: :undefined, esc_chars: true,
                                verbosity: [], config: [], event_handlers: [],
                                ct_hooks: [], enable_builtin_hooks: :undefined,
                                include: [], auto_compile: :undefined,
                                abort_if_missing_suites: :undefined,
                                silent_connections: [], stylesheet: :undefined,
                                multiply_timetraps: :undefined,
                                scale_timetraps: :undefined,
                                create_priv_dir: :undefined, testspec_files: [],
                                current_testspec: :undefined, tests: :undefined,
                                starter: :undefined)
  def script_start() do
    :erlang.process_flag(:trap_exit, true)
    init = :init.get_arguments()
    ctArgs = :lists.takewhile(fn {:ct_erl_args, _} ->
                                   false
                                 _ ->
                                   true
                              end,
                                init)
    rel_to_abs(ctArgs)
    args = (case (:application.get_env(:common_test,
                                         :run_test_start_opts)) do
              {:ok, envStartOpts} ->
                flagFilter = fn flags ->
                                  :lists.filter(fn {:root, _} ->
                                                     false
                                                   {:progname, _} ->
                                                     false
                                                   {:home, _} ->
  ...
  defp script_start(args) do
    ...
    case (:ct_repeat.loop_test(:script, args)) do
      false ->
        {:ok, cwd} = :file.get_cwd()
        cTVsn = (case (:filename.basename(:code.lib_dir(:common_test))) do
  ...
  defp analyze_test_result({_Ok, failed, {_UserSkipped, autoSkipped}},
            args) do
    cond do
      failed > 0 ->
        1
    ...
    cond do
      ...
      true ->
        case (get_start_opt(:halt_with,
                              fn [haltMod, haltFunc] ->
                                   {:erlang.list_to_atom(haltMod),
                                      :erlang.list_to_atom(haltFunc)}
                              end,
                              args)) do
          :undefined ->
            :erlang.halt(exitStatus, [{:flush, false}])
          {m, f} ->
  ...
  def script_start1(parent, args) do
    ...
    label = get_start_opt(:label,
                            fn [lbl] ->
                                 lbl
                            end,
                            args)
    profile = get_start_opt(:profile,
                              fn [prof] ->
                                   prof
                              end,
                              args)
    ...
    {autoCompile,
       includeDirs} = (case (:proplists.get_value(:no_auto_compile,
                                                    args)) do
                         :undefined ->
                           :application.set_env(:common_test, :auto_compile,
                                                  true)
                           inclDirs = (case (:proplists.get_value(:include,
                                                                    args)) do
                           ...
                           case (:os.getenv('CT_INCLUDE_PATH')) do
                             false ->
                               :application.set_env(:common_test, :include,
                                                      inclDirs)
                               {:undefined, inclDirs}
                             ctInclPath ->
                               allInclDirs = :string.lexemes(ctInclPath,
                                                               [?:, ?\s,
                                                                        ?,]) ++ inclDirs
                               :application.set_env(:common_test, :include,
                                                      allInclDirs)
    ...
    stylesheet = get_start_opt(:stylesheet,
                                 ...
                                 args)
    basicHtml = (case (:proplists.get_value(:basic_html,
                                              args)) do
                   :undefined ->
    ...
    escChars = (case (:proplists.get_value(:no_esc_chars,
                                             args)) do
                  :undefined ->
    ...
    case (:proplists.get_value(:disable_log_cache, args)) do
      :undefined ->
        :application.set_env(:common_test, :disable_log_cache,
                               false)
      _ ->
        :application.set_env(:common_test, :disable_log_cache,
                               true)
    ...
    keepLogs = get_start_opt(:keep_logs,
                               &:ct_logs.parse_keep_logs/1, :all, args)
    :application.set_env(:common_test, :keep_logs, keepLogs)
    opts = r_opts(label: label, profile: profile, shell: shell,
               cover: cover, cover_stop: coverStop, logdir: logDir,
               logopts: logOpts, basic_html: basicHtml,
               esc_chars: escChars, verbosity: verbosity,
               event_handlers: evHandlers, ct_hooks: cTHooks,
               enable_builtin_hooks: enableBuiltinHooks,
               auto_compile: autoCompile,
               abort_if_missing_suites: abortIfMissing,
               include: includeDirs, silent_connections: silentConns,
               stylesheet: stylesheet, multiply_timetraps: multTT,
               scale_timetraps: scaleTT,
               create_priv_dir: createPrivDir, starter: :script)
    result = run_or_refresh(opts, args)
    send(parent, {self(), result})
  end

  defp run_or_refresh(opts = r_opts(logdir: logDir,
    ...
    case (:proplists.get_value(:refresh_logs, args)) do
      ...
      refresh ->
        ...
        :io.nl()
        case ((try do
                :ct_logs.make_all_runs_index(:refresh, customStylesheet)
              catch
  ...
  defp script_start2(opts = r_opts(shell: :undefined), args) do
    case (:proplists.get_value(:spec, args)) do
      specs when (specs !== [] and specs !== :undefined) ->
        specs1 = get_start_opt(:join_specs, [specs], specs,
                                 args)
        relaxed = get_start_opt(:allow_user_terms, true, false,
                                  args)
        try do
      ...
      _ ->
        ...
        case (check_and_install_configfiles(initConfig,
                                              theLogDir, opts)) do
          :ok ->
            script_start3(r_opts(opts, config: initConfig, 
                                    logdir: theLogDir),
                            args)
          error ->
            error
      ...
      false ->
        case (:lists.keysearch(:error, 1, result1)) do
          {:value, error} ->
            error
          false ->
            :lists.foldl(fn {ok, fail, {userSkip, autoSkip}},
                              {ok1, fail1, {userSkip1, autoSkip1}} ->
                              {ok1 + ok, fail1 + fail,
                                 {userSkip1 + userSkip, autoSkip1 + autoSkip}}
                         end,
                           {0, 0, {0, 0}}, result1)
        end
    end
  ...
  defp execute_one_spec(tS, opts, args) do
    ...
    case (check_and_install_configfiles(allConfig,
      ...
      :ok ->
        ...
        result = do_run(run, skip,
                          r_opts(opts, config: allConfig,  logdir: theLogDir, 
                                    current_testspec: tS),
                          args)
  ...
  defp combine_test_opts(tS, specs, opts) do
    ...
    profile = choose_val(r_opts(opts, :profile),
                           r_opts(tSOpts, :profile))
    logDir = choose_val(r_opts(opts, :logdir),
                          r_opts(tSOpts, :logdir))
    allLogOpts = merge_vals([r_opts(opts, :logopts),
                                 r_opts(tSOpts, :logopts)])
    allVerbosity = merge_keyvals([r_opts(opts, :verbosity),
                                      r_opts(tSOpts, :verbosity)])
    allSilentConns = merge_vals([r_opts(opts, :silent_connections),
                                     r_opts(tSOpts, :silent_connections)])
    cover = choose_val(r_opts(opts, :cover), r_opts(tSOpts, :cover))
    coverStop = choose_val(r_opts(opts, :cover_stop),
                             r_opts(tSOpts, :cover_stop))
    multTT = choose_val(r_opts(opts, :multiply_timetraps),
                          r_opts(tSOpts, :multiply_timetraps))
    scaleTT = choose_val(r_opts(opts, :scale_timetraps),
                           r_opts(tSOpts, :scale_timetraps))
    createPrivDir = choose_val(r_opts(opts, :create_priv_dir),
                                 r_opts(tSOpts, :create_priv_dir))
    allEvHs = merge_vals([r_opts(opts, :event_handlers),
                              r_opts(tSOpts, :event_handlers)])
    allCTHooks = merge_vals([r_opts(opts, :ct_hooks),
                                 r_opts(tSOpts, :ct_hooks)])
    enableBuiltinHooks = choose_val(r_opts(opts, :enable_builtin_hooks),
                                      r_opts(tSOpts, :enable_builtin_hooks))
    stylesheet = choose_val(r_opts(opts, :stylesheet),
                              r_opts(tSOpts, :stylesheet))
    allInclude = merge_vals([r_opts(opts, :include),
                                 r_opts(tSOpts, :include)])
    :application.set_env(:common_test, :include, allInclude)
    autoCompile = (case (choose_val(r_opts(opts, :auto_compile),
                                      r_opts(tSOpts, :auto_compile))) do
                     :undefined ->
                       true
                     aCBool ->
                       :application.set_env(:common_test, :auto_compile,
    ...
    abortIfMissing = choose_val(r_opts(opts, :abort_if_missing_suites),
                                  r_opts(tSOpts, :abort_if_missing_suites))
    basicHtml = (case (choose_val(r_opts(opts, :basic_html),
                                    r_opts(tSOpts, :basic_html))) do
                   :undefined ->
                     false
                   bHBool ->
    ...
    escChars = (case (choose_val(r_opts(opts, :esc_chars),
                                   r_opts(tSOpts, :esc_chars))) do
                  :undefined ->
                    true
                  eCBool ->
    ...
    r_opts(opts, label: label,  profile: profile, 
              testspec_files: specs,  cover: cover, 
              cover_stop: coverStop,  logdir: which(:logdir, logDir), 
              logopts: allLogOpts,  basic_html: basicHtml, 
              esc_chars: escChars,  verbosity: allVerbosity, 
              silent_connections: allSilentConns, 
              config: r_opts(tSOpts, :config),  event_handlers: allEvHs, 
              ct_hooks: allCTHooks, 
              enable_builtin_hooks: enableBuiltinHooks, 
              stylesheet: stylesheet,  auto_compile: autoCompile, 
              abort_if_missing_suites: abortIfMissing, 
              include: allInclude,  multiply_timetraps: multTT, 
              scale_timetraps: scaleTT, 
              create_priv_dir: createPrivDir)
  end

  defp check_and_install_configfiles(configs, logDir,
            r_opts(event_handlers: evHandlers, ct_hooks: cTHooks,
                enable_builtin_hooks: enableBuiltinHooks)) do
    case (:ct_config.check_config_files(configs)) do
      false ->
        install([{:config, configs}, {:event_handler,
                                        evHandlers},
                                         {:ct_hooks, cTHooks},
  ...
  defp script_start3(opts, args) do
    ...
    case ({:proplists.get_value(:dir, args),
             ...
             groups_and_cases(:proplists.get_value(:group, args),
                                :proplists.get_value(:testcase, args))}) do
      {_, _, error = {:error, _}} ->
        error
      {_, [], _} ->
      ...
      {[dir], suite, gsAndCs} when (is_list(dir) and
                                      is_list(suite))
                                   ->
        case (for s <- suite do
                suite_to_test(dir, s)
  ...
  defp script_start4(r_opts(label: label, profile: profile, shell: true,
              config: config, event_handlers: evHandlers,
              ct_hooks: cTHooks, logopts: logOpts,
              verbosity: verbosity,
              enable_builtin_hooks: enableBuiltinHooks,
              stylesheet: customStylesheet, logdir: logDir,
    ...
    case (install([{:config, config}, {:event_handler,
                                         evHandlers},
                                          {:ct_hooks, cTHooks},
                                              {:enable_builtin_hooks,
                                                 enableBuiltinHooks}])) do
      :ok ->
  ...
  def install(opts, logDir) do
    ...
    case (:erlang.whereis(:ct_util_server)) do
      :undefined ->
        ...
        case (:file.open(varFile,
                           [:write, {:encoding, :utf8}])) do
          {:ok, fd} ->
            _ = (for opt <- confOpts do
                   :io.format(fd, '~tp.\n', [opt])
                 end)
            :ok = :file.close(fd)
          {:error, reason} ->
            :io.format('CT failed to install configuration data. Please verify that the log directory exists and that write permission is set.\n\n', [])
            {:error, {varFile, reason}}
        end
      _ ->
        :io.format('It is not possible to install CT while running in interactive mode.\nTo exit this mode, run ct:stop_interactive().\nTo enter the interactive mode again, run ct:start_interactive()\n\n', [])
  ...
  defp run_test1(startOpts) when is_list(startOpts) do
    case (:proplists.get_value(:refresh_logs, startOpts)) do
      :undefined ->
        ...
        res = (case (:ct_repeat.loop_test(:func, startOpts)) do
                 false ->
                   case ((try do
                           run_test2(startOpts)
                         catch
      ...
      refreshDir ->
        keepLogs = get_start_opt(:keep_logs,
                                   &:ct_logs.parse_keep_logs/1, :all, startOpts)
        :application.set_env(:common_test, :keep_logs, keepLogs)
        customStylesheet = :proplists.get_value(:stylesheet,
                                                  startOpts)
  ...
  def run_test2(startOpts) do
    ...
    verbosity = get_start_opt(:verbosity,
                                fn vLvls when is_list(vLvls) ->
                                     :lists.map(fn vLvl = {_Cat, _Lvl} ->
                                                     vLvl
                                                   lvl ->
                                                     {:"$unspecified", lvl}
                                                end,
    ...
    evHandlers = (case (:proplists.get_value(:event_handler,
                                               startOpts)) do
                    :undefined ->
                      []
                    h when is_atom(h) ->
                    ...
                    h ->
                      ...
                      :lists.flatten(:lists.map(fn eH when is_atom(eH) ->
                                                     {eH, []}
    ...
    cTHooks = get_start_opt(:ct_hooks, :value, [],
                              startOpts)
    enableBuiltinHooks = get_start_opt(:enable_builtin_hooks,
    ...
    cover = get_start_opt(:cover,
                            ...
                            startOpts)
    coverStop = get_start_opt(:cover_stop, :value,
                                startOpts)
    multiplyTT = get_start_opt(:multiply_timetraps, :value,
                                 startOpts)
    scaleTT = get_start_opt(:scale_timetraps, :value,
                              startOpts)
    createPrivDir = get_start_opt(:create_priv_dir, :value,
                                    startOpts)
    {autoCompile,
       include} = (case (:proplists.get_value(:auto_compile,
                                                startOpts)) do
                     :undefined ->
    ...
    escChars = (case (:proplists.get_value(:esc_chars,
                                             startOpts)) do
                  :undefined ->
                    :application.set_env(:common_test, :esc_chars, true)
                    :undefined
    ...
    case (:proplists.get_value(:disable_log_cache,
                                 startOpts)) do
      :undefined ->
        :application.set_env(:common_test, :disable_log_cache,
                               false)
    ...
    opts = r_opts(label: label, profile: profile, cover: cover,
               cover_stop: coverStop, step: step, logdir: logDir,
               logopts: logOpts, basic_html: basicHtml,
               esc_chars: escChars, config: cfgFiles,
               verbosity: verbosity, event_handlers: evHandlers,
               ct_hooks: cTHooks,
               enable_builtin_hooks: enableBuiltinHooks,
               auto_compile: autoCompile,
               abort_if_missing_suites: abortIfMissing,
               include: include, silent_connections: silentConns,
               stylesheet: stylesheet, multiply_timetraps: multiplyTT,
               scale_timetraps: scaleTT,
               create_priv_dir: createPrivDir, starter: :ct)
    case (:proplists.get_value(:spec, startOpts)) do
      :undefined ->
        case (:lists.keysearch(:prepared_tests, 1,
                                 startOpts)) do
          {:value, {_, {run, skip}, specs}} ->
            run_prepared(run, skip, r_opts(opts, testspec_files: specs),
                           startOpts)
          false ->
            run_dir(opts, startOpts)
        end
      specs ->
        relaxed = get_start_opt(:allow_user_terms, :value,
                                  false, startOpts)
        run_spec_file(relaxed, r_opts(opts, testspec_files: specs),
                        startOpts)
    end
  end

  defp run_spec_file(relaxed, opts = r_opts(testspec_files: specs),
            startOpts) do
    specs1 = (case (specs) do
                [x | _] when is_integer(x) ->
                  [specs]
                _ ->
                  specs
              end)
    absSpecs = :lists.map(fn sF ->
                               :filename.absname(sF)
                          end,
                            specs1)
    absSpecs1 = get_start_opt(:join_specs, [absSpecs],
                                absSpecs, startOpts)
    try do
      ...
      {:error, cTReason} ->
        exit({:error,
                {:invalid_testspec, {cTReason, __STACKTRACE__}}})
      _, cTReason ->
        exit({:error,
                {:invalid_testspec, {cTReason, __STACKTRACE__}}})
    else
  ...
  defp run_all_specs([{specs, tS} | tSs], opts, startOpts,
            totResult) do
    combined = (r_opts(config: tSConfig) = combine_test_opts(tS,
                                                          specs, opts))
    ...
    try do
      ...
      _, reason ->
        run_all_specs(tSs, opts, startOpts,
                        [{:error, reason} | totResult])
    else
      result ->
        run_all_specs(tSs, opts, startOpts,
                        [result | totResult])
  ...
  defp run_one_spec(tS, combinedOpts, startOpts) do
    ...
    case (check_and_install_configfiles(config, logdir,
                                          combinedOpts)) do
      :ok ->
        {run, skip} = :ct_testspec.prepare_tests(tS, node())
        reformat_result((try do
  ...
  defp run_prepared(run, skip,
            opts = r_opts(logdir: logDir, config: cfgFiles),
            startOpts) do
  ...
  defp run_dir(opts = r_opts(logdir: logDir, config: cfgFiles,
                     event_handlers: evHandlers, ct_hooks: cTHook,
                     enable_builtin_hooks: enableBuiltinHooks),
            startOpts) do
    logDir1 = which(:logdir, logDir)
    opts1 = r_opts(opts, logdir: logDir1)
    absCfgFiles = :lists.map(fn {callback, fileList} ->
                                  case (:code.is_loaded(callback)) do
                                    {:file, _Path} ->
                                      :ok
                                    false ->
                                      case (:code.load_file(callback)) do
                                        ...
                                        {:error, _} ->
                                          exit({:error,
                                                  {:no_such_module, callback}})
                                      end
                                  end
                                  {callback,
                                     :lists.map(fn file ->
                                                     check_config_file(callback,
                                                                         file)
                                                end,
                                                  fileList)}
                             end,
                               cfgFiles)
    case (install([{:config, absCfgFiles}, {:event_handler,
                                              evHandlers},
                                               {:ct_hooks, cTHook},
                                                   {:enable_builtin_hooks,
                                                      enableBuiltinHooks}],
    ...
    case ({:proplists.get_value(:dir, startOpts),
             ...
             groups_and_cases(:proplists.get_value(:group,
                                                     startOpts),
                                :proplists.get_value(:testcase, startOpts))}) do
      {_, _, error = {:error, _}} ->
        error
      {_, [], _} ->
        {:error, :no_suite_specified}
      ...
      {dir, :undefined, []}
        ...
        reformat_result((try do
                          do_run(tests(:erlang.atom_to_list(dir)), [], opts1,
                                   startOpts)
                        catch
                          :error, e -> {:EXIT, {e, __STACKTRACE__}}
                          :exit, e -> {:EXIT, e}
      ...
      {:undefined, suites = [hd | _], []} when not
                                               is_integer(hd)
                                               ->
        suites1 = (for s <- suites do
                     suite_to_test(s)
                   end)
        reformat_result((try do
      ...
      {:undefined, [hd, _ | _], _GsAndCs} when not
                                               is_integer(hd)
                                               ->
      ...
      {[hd, _ | _], _Suites, []} when is_list(hd) or
                                        not is_integer(hd)
                                      ->
        exit({:error, :multiple_dirs_and_suites})
      {:undefined, :undefined, gsAndCs} when gsAndCs != [] ->
        exit({:error, :incorrect_start_options})
      {dir, suite, gsAndCs} when is_integer(hd(dir)) or
        ...
        cond do
          ...
          is_integer(hd(suite)) or
            ...
            case (gsAndCs) do
              ...
              _ ->
                reformat_result((try do
                                  do_run(tests(dir2, mod, gsAndCs), [], opts1,
                                           startOpts)
                                catch
                                  :error, e -> {:EXIT, {e, __STACKTRACE__}}
                                  :exit, e -> {:EXIT, e}
      ...
      {dir, suite, gsAndCs} ->
        exit({:error,
                {:incorrect_start_options, {dir, suite, gsAndCs}}})
    end
  end
  ...
  defp run_testspec2(file) when (is_list(file) and
                        is_integer(hd(file))) do
    case (:file.read_file_info(file)) do
      {:ok, _} ->
        exit('Bad argument, use ct:run_test([{spec,' ++ file ++ '}])')
      _ ->
        exit('Bad argument, list of tuples expected, use ct:run_test/1 for test specification files')
    end
  end
  ...
  defp run_testspec2(testSpec) do
    case ((try do
      ...
      tS ->
        ...
        allInclude = (case (:os.getenv('CT_INCLUDE_PATH')) do
                        false ->
                          r_opts(opts, :include)
                        ctInclPath ->
        ...
        case (check_and_install_configfiles(r_opts(opts, :config),
                                              logDir1, opts)) do
          :ok ->
            opts1 = r_opts(opts, testspec_files: [],  logdir: logDir1, 
                              include: allInclude)
            {run, skip} = :ct_testspec.prepare_tests(tS, node())
            reformat_result((try do
  ...
  defp get_data_for_node(r_testspec(label: labels, profile: profiles,
              logdir: logDirs, logopts: logOptsList, basic_html: bHs,
              esc_chars: escChs, stylesheet: sSs, verbosity: vLvls,
              silent_connections: silentConnsList, cover: coverFs,
              cover_stop: coverStops, config: cfgs,
              userconfig: usrCfgs, event_handler: evHs,
              ct_hooks: cTHooks,
              enable_builtin_hooks: enableBuiltinHooks,
              auto_compile: aCs, abort_if_missing_suites: aiMSs,
              include: incl, multiply_timetraps: mTs,
              scale_timetraps: sTs, create_priv_dir: pDs),
            node) do
    label = :proplists.get_value(node, labels)
    profile = :proplists.get_value(node, profiles)
    logDir = (case (:proplists.get_value(node, logDirs)) do
                :undefined ->
                  '.'
                dir ->
                  dir
              end)
    logOpts = (case (:proplists.get_value(node,
                                            logOptsList)) do
                 :undefined ->
                   []
                 lOs ->
    ...
    verbosity = (case (:proplists.get_value(node, vLvls)) do
                   :undefined ->
                     []
                   lvls ->
                     lvls
                 end)
    silentConns = (case (:proplists.get_value(node,
                                                silentConnsList)) do
                     :undefined ->
                       []
                     sCs ->
    ...
    configFiles = (for {n, f} <- cfgs, n == node do
                     {:ct_config_plain, f}
                   end) ++ (for {n, cBF} <- usrCfgs, n == node do
                              cBF
                            end)
    evHandlers = (for {n, h, a} <- evHs, n == node do
                    {h, a}
                  end)
    filtCTHooks = (for {n, hook} <- cTHooks, n == node do
                     hook
                   end)
    autoCompile = :proplists.get_value(node, aCs)
    abortIfMissing = :proplists.get_value(node, aiMSs)
    include = (for {n, i} <- incl, n == node do
                 i
               end)
    r_opts(label: label, profile: profile, logdir: logDir,
        logopts: logOpts, basic_html: basicHtml,
        esc_chars: escChars, stylesheet: stylesheet,
        verbosity: verbosity, silent_connections: silentConns,
        cover: cover, cover_stop: coverStop,
        config: configFiles, event_handlers: evHandlers,
        ct_hooks: filtCTHooks,
        enable_builtin_hooks: enableBuiltinHooks,
        auto_compile: autoCompile,
        abort_if_missing_suites: abortIfMissing,
        include: include, multiply_timetraps: mT,
        scale_timetraps: sT, create_priv_dir: createPrivDir)
  end

  def refresh_logs(logDir, customStylesheet) do
    {:ok, cwd} = :file.get_cwd()
  ...
  def run(testDir, suite) when (is_list(testDir) and
  ...
  defp suite_to_test(dir, suite) when is_list(suite) do
    case (:filename.dirname(suite)) do
      ...
      dirName ->
        ...
        {dirName,
           :erlang.list_to_atom(:filename.rootname(file))}
    end
  end
  ...
  defp groups_and_cases(gOrGs, cs)
      when is_atom(gOrGs) or is_list(gOrGs) and (is_atom(hd(gOrGs)) or is_list(hd(gOrGs)) and is_atom(hd(hd(gOrGs)))) do
    cond do
  ...
  defp tests(testDir, suites, []) when (is_list(testDir) and
                                       is_integer(hd(testDir))) do
    [{:ct_util.get_testdir(testDir, suites),
        ensure_atom(suites), :all}]
  end

  defp tests(testDir, suite, cases)
      when (is_list(testDir) and is_integer(hd(testDir))) do
    [{:ct_util.get_testdir(testDir, suite),
        ensure_atom(suite), cases}]
  end
  ...
  defp tests([{dir, suite}], cases) do
    [{:ct_util.get_testdir(dir, suite), ensure_atom(suite),
        cases}]
  end

  defp tests(testDir, suite) when (is_list(testDir) and
                                  is_integer(hd(testDir))) do
  ...
  defp tests(dirSuites) when (is_list(dirSuites) and
  ...
  defp tests(testDir) when (is_list(testDir) and
                           is_integer(hd(testDir))) do
    tests([testDir])
  end

  defp tests(testDirs) when (is_list(testDirs) and
  ...
  defp do_run(tests, misc, logDir, logOpts)
      when (is_list(misc) and is_list(logDir) and
              is_list(logOpts)) do
    opts = (case (:proplists.get_value(:step, misc)) do
              :undefined ->
                r_opts()
              stepOpts ->
  ...
  defp do_run(tests, skip, opts, args)
    ...
    r_opts(label: label, profile: profile, verbosity: vLvls,
        stylesheet: customStylesheet) = opts
    testLabel = (cond do
                   label == :undefined ->
                     :undefined
                   is_atom(label) ->
    ...
    :application.set_env(:common_test, :test_label,
                           testLabel)
    testProfile = (cond do
                     profile == :undefined ->
                       :undefined
    ...
    :application.set_env(:common_test, :profile,
                           testProfile)
    case (:code.which(:test_server)) do
      ...
      _ ->
        ...
        case (:ct_util.start(r_opts(opts, :logdir), verbosity,
                               customStylesheet)) do
          {:error, :interactive_mode} ->
            :io.format('CT is started in interactive mode. To exit this mode, run ct:stop_interactive().\nTo enter the interactive mode again, run ct:start_interactive()\n\n', [])
            {:error, :interactive_mode}
          _Pid ->
            :ct_util.set_testdata({:starter, r_opts(opts, :starter)})
            compile_and_run(tests, skip,
                              r_opts(opts, verbosity: verbosity), args)
        end
    end
  end

  defp compile_and_run(tests, skip, opts, args) do
    :ct_util.set_testdata({:stylesheet,
                             r_opts(opts, :stylesheet)})
    :ct_util.set_testdata({:logopts, r_opts(opts, :logopts)})
    :ct_util.set_testdata({:testspec,
                             r_opts(opts, :current_testspec)})
    ...
    {_TestSuites1, suiteMakeErrors,
       allMakeErrors} = (case (:application.get_env(:common_test,
                                                      :auto_compile)) do
                           {:ok, false} ->
                             {testSuites1,
    ...
    case (continue(allMakeErrors,
                     r_opts(opts, :abort_if_missing_suites))) do
      true ->
        ...
        try do
          ...
          {tests1, skip1} ->
            ...
            testResult = possibly_spawn(releaseSh == true, tests1,
                                          skip1, opts)
            case (testResult) do
              {ok, errors, skipped} ->
                noOfMakeErrors = :lists.foldl(fn {_, badMods}, x ->
                                                   x + length(badMods)
                                              end,
                                                0, suiteMakeErrors)
      ...
      false ->
        :io.nl()
        :ct_util.stop(:clean)
        badMods = :lists.foldr(fn {{_, _}, ms}, acc ->
                                    ms ++ :lists.foldl(fn m, acc1 ->
                                                            :lists.delete(m,
                                                                            acc1)
                                                       end,
                                                         acc, ms)
  ...
  defp possibly_spawn(false, tests, skip, opts) do
    ...
    case (testResult) do
      {eType, _} = error when eType == :user_error or
                                eType == :error
                              ->
        :ct_util.stop(:clean)
        exit(error)
  ...
  defp auto_compile(testSuites) do
    ...
    userInclude = (case (:application.get_env(:common_test,
                                                :include)) do
    ...
    suiteMakeErrors = :lists.flatmap(fn {testDir,
                                           suite} = tS ->
                                          case (run_make(:suites, testDir,
                                                           suite, userInclude,
                                                           [:nowarn_export_all])) do
                                            {:error, {:make_failed, bad}} ->
                                              [{tS, bad}]
                                            {:error, _} ->
                                              [{tS,
                                                  [:filename.join(testDir, '*_SUITE')]}]
                                            _ ->
                                              []
                                          end
                                     end,
                                       testSuites)
    {_, helpMakeErrors} = :lists.foldl(fn {dir, suite},
                                            {done, failed} ->
                                            case (:lists.member(dir, done)) do
                                              false ->
                                                failed1 = (case (run_make(:helpmods,
                                                                            dir,
                                                                            suite,
                                                                            userInclude,
                                                                            [])) do
                    ...
                    {:error, _Reason} ->
                      case (:code.is_loaded(suite)) do
                        ...
                        false ->
                          ...
                          name = :filename.join(dir,
                                                  :erlang.atom_to_list(suite))
                          {found, [{dS, [name]} | notFound]}
                      end
                  end
             end
    {actualFound, missing} = :lists.foldl(verify, {[], []},
                                            testSuites)
  ...
  defp save_make_errors(errors) do
    ...
    :ct_logs.log('MAKE RESULTS', 'Error compiling or locating the following suites: ~n~p', [suites])
    :ok = :file.write_file('missing_suites.info',
                             :erlang.term_to_binary(errors))
    errors
  end

  defp get_bad_suites([{{_TestDir, _Suite}, failed} | errors],
            badSuites) do
  ...
  defp final_tests1([{testDir, suites, _} | tests], final, skip,
            bad)
      when (is_list(suites) and is_atom(hd(suites))) do
    skip1 = (for {{tD, s}, _} <- bad, s1 <- suites, s == s1,
                   tD == testDir do
               {tD, s, :make_failed}
             end)
    final1 = (for s <- suites do
                {testDir, s, :all}
              end)
    final_tests1(tests, :lists.reverse(final1) ++ final,
                   skip ++ skip1, bad)
  end

  defp final_tests1([{testDir, :all, :all} | tests], final, skip,
            bad) do
    missingSuites = (case (:lists.keysearch({testDir, :all},
                                              1, bad)) do
                       {:value, {_, failed}} ->
                         for f <- failed do
    ...
    missing = (for s <- missingSuites do
  ...
  defp final_tests1([{testDir, suite, cases} | tests], final, skip,
            bad)
      when cases == [] or cases == :all do
    final_tests1([{testDir, [suite], :all} | tests], final,
                   skip, bad)
  end

  defp final_tests1([{testDir, suite, grsOrCs} | tests], final,
            skip, bad)
      when is_list(grsOrCs) do
    case (:lists.keymember({testDir, suite}, 1, bad)) do
      true ->
        skip1 = skip ++ [{testDir, suite, :all, :make_failed}]
        final_tests1(tests, [{testDir, suite, :all} | final],
                       skip1, bad)
      false ->
        grsOrCs1 = :lists.flatmap(fn {:all, :all} ->
                                     ...
                                     {grSpec = {groupName, _, _}, tCs} ->
                                       ...
                                       [:ct_groups.make_conf(testDir, suite,
                                                               ...
                                                               tCs)]
                                     {groupOrGroups, tCs} ->
                                       case (groupOrGroups) do
                                         [groupList] when is_list(groupList) ->
                                           {grpNames,
                                              props} = :lists.foldl(fn {grpName,
                                                                          _} = grSpec,
                                                                         {grpNames,
                                                                            props} ->
                                                                         {:lists.append(grpNames,
                                                                                          [grpName]),
                                                                            [grSpec |
                                                                                 props]}
                                                                       {grpName,
  ...
  defp final_skip([{testDir, suite, {:all, :all}, reason} |
               skips],
            final) do
    skipConf = :ct_groups.make_conf(testDir, suite, :all,
                                      [], :all)
    skip = {testDir, suite, skipConf, reason}
    final_skip(skips, [skip | final])
  end

  defp final_skip([{testDir, suite, {group, tCs}, reason} |
               skips],
            final) do
    conf = :ct_groups.make_conf(testDir, suite, group, [],
                                  tCs)
  ...
  defp set_group_leader_same_as_shell(oldGL) do
    gS2or3 = fn p ->
                  case (:erlang.process_info(p, :initial_call)) do
                    {:initial_call, {:group, :server, x}} when x == 2 or
                                                                 x == 3
                                                               ->
                      true
                    _ ->
    ...
    case (for p <- :erlang.processes(), gS2or3.(p),
  ...
  defp do_run_test(tests, skip, opts0) do
    case (check_and_add(tests, [], [])) do
      {:ok, addedToPath} ->
        ...
        inclPath = (case (:application.get_env(:common_test,
                                                 :include)) do
                      {:ok, incls} ->
                        incls
                      _ ->
        ...
        escChars = (case (:application.get_env(:common_test,
                                                 :esc_chars)) do
                      {:ok, eCBool} ->
                        eCBool
                      _ ->
        ...
        cond do
          ...
          true ->
            :io.format('~nTEST INFO: ~w test(s), ~w case(s) in ~w suite(s)~n~n', [noOfTests, noOfCases, noOfSuites])
            :ct_logs.log('TEST INFO', '~w test(s), ~w case(s) in ~w suite(s)', [noOfTests, noOfCases, noOfSuites])
        end
        case (:proplists.get_value(:default,
                                     r_opts(opts0, :verbosity))) do
        ...
        :ct_logs.log('TEST INFO', 'Timetrap time multiplier = ~w~nTimetrap scaling enabled = ~w', [multTT, scaleTT])
        :test_server_ctrl.multiply_timetraps(multTT)
        :test_server_ctrl.scale_timetraps(scaleTT)
        :test_server_ctrl.create_priv_dir(choose_val(r_opts(opts0, :create_priv_dir),
                                                       :auto_per_run))
        {:ok, logDir} = :ct_logs.get_log_dir(true)
        {tsCoverInfo, opts} = maybe_start_cover(opts0, logDir)
        :ct_event.notify(r_event(name: :start_info, node: node(),
        ...
        :lists.foreach(fn suite ->
                            maybe_cleanup_interpret(suite, r_opts(opts, :step))
                       end,
                         cleanUp)
        _ = (for dir <- addedToPath do
               :code.del_path(dir)
             end)
  ...
  defp maybe_start_cover(opts = r_opts(cover: cover, cover_stop: coverStop0),
            logDir) do
    cond do
      cover == :undefined ->
        {:undefined, opts}
      true ->
        case (:ct_cover.get_spec(cover)) do
          {:error, reason} ->
            exit({:error, reason})
          coverSpec ->
            coverStop = (case (coverStop0) do
                           :undefined ->
                             true
                           stop ->
                             stop
                         end)
            start_cover(r_opts(opts, coverspec: coverSpec, 
  ...
  defp start_cover(opts = r_opts(coverspec: covData,
                     cover_stop: covStop),
            logDir) do
    {covFile, covNodes, covImport, _CovExport,
       r_cover(app: covApp, local_only: localOnly, level: covLevel,
           excl_mods: covExcl, incl_mods: covIncl, cross: covCross,
           src: _CovSrc)} = covData
    case (localOnly) do
      true ->
        :cover.local_only()
    ...
    :ct_logs.log('COVER INFO', 'Using cover specification file: ~ts~nApp: ~w~nLocal only: ~w~nCross cover: ~w~nIncluding ~w modules~nExcluding ~w modules',
                   [covFile, covApp, localOnly, covCross, length(covIncl),
                                                              length(covExcl)])
    :test_server_ctrl.cover({:log, logDir})
    {:ok,
       tsCoverInfo} = :test_server_ctrl.cover_compile(covApp,
                                                        covFile, covExcl,
                                                        covIncl, covCross,
                                                        covLevel, covStop)
    :ct_logs.log('COVER INFO', 'Compilation completed - test_server cover info: ~tp', [tsCoverInfo])
    cond do
    ...
    :lists.foreach(fn imp ->
                        case (:cover.import(imp)) do
                          :ok ->
                            :ok
                          {:error, reason} ->
                            :ct_logs.log('COVER INFO', 'Importing cover data from: ~ts fails! Reason: ~tp', [imp, reason])
                        end
                   end,
                     covImport)
    {tsCoverInfo, opts}
  ...
  defp maybe_stop_cover(r_opts(coverspec: covData), tsCoverInfo, logDir) do
    {_CovFile, _CovNodes, _CovImport, covExport,
       _AppData} = covData
    case (covExport) do
      :undefined ->
        :ok
  ...
  defp count_test_cases1(jobs, n, suites, ref) do
    receive do
      {_, {:error, _Reason} = error} ->
        throw(error)
      {:no_of_cases, {ss, n1}} ->
        count_test_cases1(jobs - 1, add_known(n, n1),
                            [ss | suites], ref)
      {:DOWN, ^ref, _, _, info} ->
  ...
  defp add_jobs([{testDir, :all, _} | tests], skip, opts,
            cleanUp) do
    name = get_name(testDir)
    case ((try do
            :test_server_ctrl.add_dir_with_skip(name, testDir,
  ...
  defp add_jobs([{testDir, [suite], :all} | tests], skip, opts,
            cleanUp)
      when is_atom(suite) do
    add_jobs([{testDir, suite, :all} | tests], skip, opts,
               cleanUp)
  end

  defp add_jobs([{testDir, suites, :all} | tests], skip, opts,
            cleanUp)
      when is_list(suites) do
    name = get_name(testDir) ++ '.suites'
    case ((try do
            :test_server_ctrl.add_module_with_skip(name, suites,
                                                     skiplist(testDir, skip))
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
  ...
  defp add_jobs([{testDir, suite, :all} | tests], skip, opts,
    ...
    case (maybe_interpret(suite, :all, opts)) do
      :ok ->
        ...
        case ((try do
                :test_server_ctrl.add_module_with_skip(name, [suite],
                                                         skiplist(testDir,
                                                                    skip))
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp add_jobs([{testDir, suite, confs} | tests], skip, opts,
            cleanUp)
    ...
    case (maybe_interpret(suite, :init_per_group, opts)) do
      :ok ->
        case ((try do
                :test_server_ctrl.add_conf_with_skip(testName, suite,
                                                       confs,
                                                       skiplist(testDir, skip))
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp add_jobs([{testDir, suite, [case__]} | tests], skip,
            opts, cleanUp)
      when is_atom(case__) do
    add_jobs([{testDir, suite, case__} | tests], skip, opts,
               cleanUp)
  end

  defp add_jobs([{testDir, suite, cases} | tests], skip, opts,
            cleanUp)
      when is_list(cases) do
    cases1 = :lists.map(fn {groupName, _}
    ...
    case (maybe_interpret(suite, cases1, opts)) do
      :ok ->
        ...
        case ((try do
                :test_server_ctrl.add_cases_with_skip(name, suite,
                                                        cases1,
                                                        skiplist(testDir, skip))
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp add_jobs([{testDir, suite, case__} | tests], skip, opts,
            cleanUp)
      when is_atom(case__) do
    case (maybe_interpret(suite, case__, opts)) do
      :ok ->
        name = get_name(testDir) ++ '.' ++ :erlang.atom_to_list(suite) ++ '.' ++ :erlang.atom_to_list(case__)
        case ((try do
                :test_server_ctrl.add_case_with_skip(name, suite,
                                                       case__,
                                                       skiplist(testDir, skip))
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp get_name(dir) do
    testDir = (case (:filename.basename(dir)) do
                 'test' ->
  ...
  defp run_make(targets, testDir0, mod, userInclude, cOpts)
      when is_list(mod) do
    run_make(targets, testDir0, :erlang.list_to_atom(mod),
               userInclude, cOpts)
  end
  ...
  defp run_make(targets, testDir0, mod, userInclude, cOpts) do
    case (locate_test_dir(testDir0, mod)) do
      {:ok, testDir} ->
        :ct_event.sync_notify(r_event(name: :start_make, node: node(),
                                  data: testDir))
        {:ok, cwd} = :file.get_cwd()
        :ok = :file.set_cwd(testDir)
        ctInclude = get_dir(:common_test, 'include')
        xmerlInclude = get_dir(:xmerl, 'include')
        erlFlags = userInclude ++ [{:i, ctInclude}, {:i,
                                                       xmerlInclude},
                                                        :debug_info] ++ cOpts
        result = (cond do
                    mod == :all or targets == :helpmods ->
                      case ((try do
                        ...
                        {:EXIT, _} = failure ->
                          failure
                        makeInfo ->
                          fileTest = fn f, :suites ->
                                          is_suite(f)
                                        f, :helpmods ->
                                          not is_suite(f)
                                     end
                          files = :lists.flatmap(fn {f, :out_of_date} ->
                                                      case (fileTest.(f,
                                                                        targets)) do
                                                        true ->
                                                          [f]
        ...
        :ok = :file.set_cwd(cwd)
        :ct_event.notify(r_event(name: :finished_make, node: node(),
                             data: testDir))
        case (result) do
          {:up_to_date, _} ->
            :ok
          ...
          {:error, modInfo} ->
            ...
            bad = (for {m, r} <- modInfo, r == :error do
  ...
  defp maybe_interpret2(suite, cases, stepOpts) do
    ...
    _ = (for case__ <- cases, is_atom(case__) do
           (
             try do
               :i.ib(suite, case__, 1)
             catch
    ...
    winOp = (case (:lists.member(:keep_inactive,
                                   ensure_atom(stepOpts))) do
               true ->
                 :no_kill
               false ->
                 :kill
             end)
    :ct_util.set_testdata({:interpret,
                             {{suite, cases}, winOp, {:undefined, :undefined}}})
    :ok
  end
  ...
  defp merge_arguments([coverStop = {:cover_stop, _} | args],
            merged) do
    merge_arguments(args,
  ...
  defp handle_arg(:merge, {:event_handler_init, elems},
            [{:event_handler_init, prevElems} | merged]) do
    [{:event_handler_init, prevElems ++ ['add' | elems]} |
         merged]
  end

  defp handle_arg(:merge, {:userconfig, elems},
            [{:userconfig, prevElems} | merged]) do
    [{:userconfig, prevElems ++ ['add' | elems]} | merged]
  end

  defp handle_arg(:merge, {key, elems},
            [{key, prevElems} | merged]) do
  ...
  defp ct_hooks_args2opts(args) do
    :lists.foldl(fn {:ct_hooks, hooks}, acc ->
  ...
  defp event_handler_args2opts(args) do
    case (:proplists.get_value(:event_handler, args)) do
      ...
      eHs ->
        event_handler_args2opts(for eH <- eHs do
  ...
  defp event_handler_args2opts(default, args) do
    case (:proplists.get_value(:event_handler_init,
                                 args)) do
      :undefined ->
        default
      eHs ->
  ...
  defp verbosity_args2opts(args) do
    case (:proplists.get_value(:verbosity, args)) do
      :undefined ->
        []
      vArgs ->
        getVLvls = fn 'and', {:new, soFar} when is_list(soFar) ->
                        {:new, soFar}
                      ...
                      lvl, {cat, soFar} ->
                        {:new,
                           [{:erlang.list_to_atom(cat),
                               :erlang.list_to_integer(lvl)} |
                                soFar]}
                   end
  ...
  defp add_verbosity_defaults(vLvls) do
    case ({:proplists.get_value(:"$unspecified", vLvls),
             :proplists.get_value(:default, vLvls)}) do
      {:undefined, :undefined} ->
           ...
           (
             ...
             _ = (cond do
                    dir != abs ->
                      _ = :code.del_path(dir)
                      _ = :code.del_path(abs)
                      :io.format(:user, 'Converting ~tp to ~tp and re-inserting with add_pathz/1~n', [dir, abs])
                    true ->
                      _ = :code.del_path(dir)
                  end)
             :code.add_pathz(abs)
           ...
           (
             ...
             _ = (cond do
                    dir != abs ->
                      _ = :code.del_path(dir)
                      _ = :code.del_path(abs)
                      :io.format(:user, 'Converting ~tp to ~tp and re-inserting with add_patha/1~n', [dir, abs])
                    true ->
                      _ = :code.del_path(dir)
                  end)
             :code.add_patha(abs)
  ...
  defp opts2args(envStartOpts) do
    :lists.flatmap(fn {:exit_status, exitStatusOpt}
                          when is_atom(exitStatusOpt) ->
                        [{:exit_status, [:erlang.atom_to_list(exitStatusOpt)]}]
                      {:halt_with, {haltM, haltF}} ->
                        [{:halt_with,
                            [:erlang.atom_to_list(haltM),
                                 :erlang.atom_to_list(haltF)]}]
                      ...
                      {:userconfig, userCfg} when is_list(userCfg) ->
                        strs = :lists.map(fn {cBM, cfgStr = [x | _]}
                                                 when is_integer(x) ->
                      ...
                      {:event_handler, {eH, arg}} when is_atom(eH) ->
                        ...
                        [{:event_handler_init,
                            [:erlang.atom_to_list(eH), argStr]}]
                      {:event_handler, {eHs, arg}} when is_list(eHs) ->
                        argStr = :lists.flatten(:io_lib.format('~tp', [arg]))
                        strs = :lists.flatmap(fn eH ->
                                                   [:erlang.atom_to_list(eH),
                                                        argStr, 'and']
                                              end,
                                                eHs)
                      ...
                      {:verbosity, vLvls} when is_list(vLvls) ->
                        vLvlArgs = :lists.flatmap(fn {:"$unspecified", lvl} ->
                                                       [:erlang.integer_to_list(lvl),
                                                            'and']
                                                     {cat, lvl} ->
                                                       [:erlang.atom_to_list(cat),
                                                            :erlang.integer_to_list(lvl),
                                                                'and']
                                                     lvl ->
                                                       [:erlang.integer_to_list(lvl),
                      ...
                      {:ct_hooks, cTHs} when is_list(cTHs) ->
                        ...
                        strs = :lists.flatmap(fn {cTH, arg, prio} ->
  ...
  defp locate_test_dir(dir, suite) do
    testDir = (case (:ct_util.is_test_dir(dir)) do
                 true ->
                   dir
                 false ->
  ...
  defp is_suite(modOrFile) when is_list(modOrFile) do
    case (:lists.reverse(:filename.basename(modOrFile,
                                              '.erl'))) do
      [?E, ?T, ?I, ?U, ?S, ?_ | _] ->
        true
      _ ->
      ...
      suiteCases ->
        ...
        try do
          ...
          [] ->
            cases
          seqs ->
            tCs1 = :lists.flatten(for {_, tCs} <- seqs do
                                    tCs
                                  end)
            :lists.reverse(:lists.foldl(fn tC, acc ->
    ...
    case (:lists.keysearch(:ct_trace, 1, args)) do
      {:value, {:ct_trace, file}} ->
        ...
        case (:file.consult(traceSpec)) do
          ...
          {_, error} ->
            :io.format('Warning! Tracing not started. Reason: ~ts~n~n', [:file.format_error(error)])
            false
        end
      false ->
        false
  ...
  defp ensure_atom(string) when (is_list(string) and
