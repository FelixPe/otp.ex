defmodule :m_ct_repeat do
  ...
  def loop_test(if__, args) when is_list(args) do
    ...
    case get_loop_info(args) do
      ...
      e = {:error, _} ->
        :io.format(~c"Common Test error: ~tp\n\n", [e])
        :ok = :file.set_cwd(cwd)
        e
  ...
  defp get_stop_time(:until, time = [_, _, _, _, _, _]) do
    get_stop_time(:until, ~c"000000" ++ time)
  end
  ...
  defp stop_after(_CtrlPid, secs, forceStop) do
    ...
    case forceStop do
      skipRest when skipRest == :skip_rest or skipRest == [~c"skip_rest"] ->
  ...
  def log_loop_info(args) do
    case :lists.keysearch(:loop_info, 1, args) do
      ...
      {:value, {_, [{:repeat, c, n}]}} ->
        :ct_logs.log(~c"Test loop info", ~c"Test run ~w of ~w", [c, n])

      {:value, {_, [{:stop_time, secs0, stopTime, n}]}} ->
        logStr1 =
          case :lists.keysearch(:duration, 1, args) do
            {:value, {_, dur}} ->
              :io_lib.format(~c"Specified test duration: ~s (~w secs)\n", [delistify(dur), secs0])

            _ ->
              case :lists.keysearch(:until, 1, args) do
                {:value, {_, until}} ->
                  :io_lib.format(~c"Specified end time: ~s (duration ~w secs)\n", [
                    delistify(until),
                    secs0
                  ])
        ...
        logStr2 = :io_lib.format(~c"Test run #~w\n", [n])
        secs = remaining_time(stopTime)

        logStr3 =
          :io_lib.format(
            ~c"Test time remaining: ~w secs (~w%)\n",
        ...
        logStr4 =
          case :proplists.get_value(
            ...
            forceStop ->
              :io_lib.format(~c"force_stop is set to: ~w", [forceStop])
          end

        :ct_logs.log(~c"Test loop info", ~c"~ts", [logStr1 ++ logStr2 ++ logStr3 ++ logStr4])
    end
  end

  defp ts(secs) do
    :erlang.integer_to_list(secs) ++ ~c" secs"
  end
