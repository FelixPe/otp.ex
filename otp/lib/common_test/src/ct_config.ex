defmodule :m_ct_config do
  ...
  Record.defrecord(:r_ct_conf, :ct_conf, key: :undefined,
                                   value: :undefined, handler: :undefined,
                                   config: :undefined, name: :_UNDEF,
                                   default: false)
  def start(mode) do
  ...
  defp do_start(parent) do
    ...
    opts = (case (:ct_util.read_opts()) do
              {:ok, opts1} ->
                opts1
              error ->
  ...
  defp process_default_configs(opts) do
    :lists.flatmap(fn {:config, [_ | _] = fileOrFiles} ->
  ...
  defp process_user_configs(opts, acc) do
    case (:lists.keytake(:userconfig, 1, opts)) do
      false ->
        :lists.reverse(acc)
      {:value, {:userconfig, config = [{_, _} | _]},
         newOpts} ->
        acc1 = :lists.map(fn {_Callback, []} = cfg ->
                               cfg
  ...
  def read_config_files(opts) do
    ...
    configFiles = (case (:lists.keyfind(:config, 1,
                                          opts)) do
                     {:config, configLists} ->
                       :lists.foldr(fn {callback, files}, acc ->
  ...
  defp store_config(config, callback, file) when is_list(config) do
    for {key, val} <- config do
      :ets.insert(:ct_attributes,
                    r_ct_conf(key: key, value: val, handler: callback, config: file,
                        default: false))
    end
  end
  ...
  defp rewrite_config(config, callback, file) do
    oldRows = :ets.match_object(:ct_attributes,
                                  r_ct_conf(handler: callback, config: file, _: :_))
    :ets.match_delete(:ct_attributes,
                        r_ct_conf(handler: callback, config: file, _: :_))
    updater = fn {key, value} ->
                   case (keyfindall(key, r_ct_conf(:key), oldRows)) do
                     [] ->
                       :ets.insert(:ct_attributes,
                                     r_ct_conf(key: key, value: value,
                                         handler: callback, config: file))
                     rowsToUpdate ->
                       inserter = fn row ->
                                       :ets.insert(:ct_attributes,
  ...
  def get_config({deepKey, subKey}, default, opts)
    ...
    get_config(:erlang.append_element(deepKey, subKey),
                 default, opts)
  end
  ...
  def get_config(keyOrName, default, opts)
    ...
    case (lookup_config(:erlang.element(1, keyOrName))) do
      ...
      vals ->
        newVals = :lists.map(fn {val} ->
                                  get_config(tl(:erlang.tuple_to_list(keyOrName)),
                                               val, default, opts)
                             end,
                               vals)
  ...
  defp do_get_config([key | _], available, _Mapped) when not
                                              is_list(available) do
  ...
  defp lookup_key(key) do
    :ets.select(:ct_attributes,
                  [{r_ct_conf(key: key, value: :"$1", name: :_UNDEF, _: :_), [],
                      [{{:"$1"}}]}])
  end

  defp format_value([subVal | _] = subVals, keyOrName, opts) do
    case ({:lists.member(:all, opts),
             :lists.member(:element, opts)}) do
      {true, true} ->
        for val <- subVals do
          {keyOrName, val}
        end
  ...
  defp lookup_handler_for_name(name) do
    :ets.select(:ct_attributes,
                  [{r_ct_conf(handler: :"$1", config: :"$2", name: name, _: :_), [],
                      [{{:"$1", :"$2"}}]}])
  end

  defp lookup_handler_for_key(key) do
    :ets.select(:ct_attributes,
                  [{r_ct_conf(handler: :"$1", config: :"$2", key: key, _: :_), [],
                      [{{:"$1", :"$2"}}]}])
  end

  defp update_conf(name, newConfig) do
    old = :ets.select(:ct_attributes,
                        [{r_ct_conf(name: name, _: :_), [], [:"$_"]}])
    :lists.foreach(fn oldElem ->
  ...
  defp reload_conf(keyOrName) do
    case (lookup_handler_for_config(keyOrName)) do
      ...
      handlerList ->
        ...
        case (read_config_files(handlerList2,
                                  &rewrite_config/3)) do
          :ok ->
  ...
  def release_allocated() do
    allocated = :ets.select(:ct_attributes,
  ...
  defp associate_int(name, configs, 'true') do
    :lists.foreach(fn {k, _Config} ->
                        cs = :ets.match_object(:ct_attributes,
                                                 r_ct_conf(key: :erlang.element(1, k),
                                                     name: :_UNDEF, _: :_))
                        for c <- cs do
  ...
  def require(name, key) when (is_atom(name) and
  ...
  defp do_require(name, key) do
    case (get_key_from_name(name)) do
      {:error, _} ->
        allocate(name, key)
      {:ok, nameKey} when nameKey == key or
  ...
  def get_key_from_name(name) do
    case (:ets.select(:ct_attributes,
                        [{r_ct_conf(name: name, key: :"$1", _: :_), [], [:"$1"]}])) do
      [key | _] ->
        {:ok, key}
      _ ->
  ...
  def encrypt_config_file(srcFileName, encryptFileName,
           {:file, keyFile}) do
  ...
  def encrypt_config_file(srcFileName, encryptFileName, {:key, key}) do
    ...
    case (:file.read_file(srcFileName)) do
      {:ok, bin0} ->
        ...
        bin2 = (case (rem(byte_size(bin1), 8)) do
                  0 ->
                    bin1
                  n ->
                    :erlang.list_to_binary([bin1, random_bytes(8 - n)])
                end)
        encBin = :crypto.crypto_one_time(:des_ede3_cbc,
                                           cryptoKey, iVec, bin2, true)
  ...
  def decrypt_config_file(encryptFileName, targetFileName,
           {:file, keyFile}) do
    case (get_crypt_key_from_file(keyFile)) do
      {:error, _} = e ->
        e
  ...
  def decrypt_config_file(encryptFileName, targetFileName, {:key, key}) do
    ...
    case (:file.read_file(encryptFileName)) do
      {:ok, bin} ->
        decBin = :crypto.crypto_one_time(:des_ede3_cbc,
                                           cryptoKey, iVec, bin, false)
        case ((try do
  ...
  def get_crypt_key_from_file(file) do
    case (:file.read_file(file)) do
      {:ok, bin} ->
        case ((try do
                :string.lexemes(:erlang.binary_to_list(bin),
                                  [?\n, [?\r, ?\n]])
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                :exit, e -> {:EXIT, e}
  ...
  def get_crypt_key_from_file() do
    ...
    {result, fullName} = (case (:file.read_file(cwdFile)) do
                            {:ok, bin} ->
                              {bin, cwdFile}
                            _ ->
    ...
    case (fullName) do
      ...
      _ ->
        case ((try do
                :string.lexemes(:erlang.binary_to_list(result),
                                  [?\n, [?\r, ?\n]])
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                :exit, e -> {:EXIT, e}
  ...
  defp check_exports(callback) do
    ...
    case ({:lists.member({:check_parameter, 1}, fs),
             :lists.member({:read_config, 1}, fs)}) do
      {true, true} ->
        {:ok, callback}
      _ ->
  ...
  def check_config_files(configs) do
    configChecker = fn {callback, [f | _R] = files} ->
    ...
    :lists.keysearch(:error, 1,
                       :lists.flatten(:lists.map(configChecker, configs)))
  end
  ...
  defp prepare_user_configs([configString | userConfigs],
            [{lastMod, lastList} | acc], :cur) do
    prepare_user_configs(userConfigs,
                           [{lastMod, [configString | lastList]} | acc], :cur)
  end
  ...
  def prepare_config_list(args) do
    configFiles = (case (:lists.keysearch(:ct_config, 1,
                                            args)) do
    ...
    userConfigs = (case (:lists.keysearch(:userconfig, 1,
                                            args)) do
