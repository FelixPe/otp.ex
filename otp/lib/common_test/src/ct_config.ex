defmodule :m_ct_config do
  ...
  defp associate(name, {key, subKeys}, configs)
    ...
    associate_int(name, configs, ~c"true")
  end

  defp associate(name, _Key, configs) do
    associate_int(name, configs, :os.getenv(~c"COMMON_TEST_ALIAS_TOP"))
  end

  defp associate_int(name, configs, ~c"true") do
    :lists.foreach(
      fn {k, _Config} ->
        cs =
          :ets.match_object(
    ...
    case :file.read_file(srcFileName) do
      {:ok, bin0} ->
        ...
        case :file.write_file(encryptFileName, encBin) do
          :ok ->
            :io.format(~c"~ts --(encrypt)--> ~ts~n", [srcFileName, encryptFileName])
            :ok
            ...
            case targetFileName do
              ...
              _ ->
                case :file.write_file(targetFileName, srcBin) do
                  :ok ->
                    :io.format(~c"~ts --(decrypt)--> ~ts~n", [encryptFileName, targetFileName])
                    :ok
  ...
  def get_crypt_key_from_file() do
    cwdFile = :filename.join(~c".", ~c".ct_config.crypt")
      ...
      case :file.read_file(cwdFile) do
        ...
        _ ->
          case :init.get_argument(:home) do
            {:ok, [[home]]} ->
              homeFile = :filename.join(home, ~c".ct_config.crypt")
    ...
    case fullName do
      ...
      _ ->
        case (try do
          ...
          [key] ->
            :io.format(~c"~nCrypt key file: ~ts~n", [fullName])
            key
  ...
  defp prepare_user_configs([~c"and" | userConfigs], acc, _) do
