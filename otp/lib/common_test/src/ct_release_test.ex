defmodule :m_ct_release_test do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_ct_data, :ct_data, from: :undefined,
                                   to: :undefined)
  def init(config) do
    try do
      init_upgrade_test()
  ...
  def upgrade(apps, level, callback, config) do
    ...
    try do
      upgrade(apps, level, callback, createDir, installDir,
                config)
    catch
      {:fail, reason} ->
        :ct.fail(reason)
  ...
  def get_app_vsns(r_ct_data(from: fromApps, to: toApps), app) do
    case ({:lists.keyfind(app, 1, fromApps),
             :lists.keyfind(app, 1, toApps)}) do
  ...
  def get_appup(r_ct_data(from: fromApps, to: toApps), app) do
    case (:lists.keyfind(app, 1, toApps)) do
      {^app, toVsn, toDir} ->
        appup = :filename.join([toDir, 'ebin',
                                           :erlang.atom_to_list(app) ++ '.appup'])
        {:ok, [{^toVsn, ups, downs}]} = :file.consult(appup)
        {^app, fromVsn, _} = :lists.keyfind(app, 1, fromApps)
        case ({:systools_relup.appup_search_for_version(fromVsn,
                                                          ups),
                 :systools_relup.appup_search_for_version(fromVsn,
                                                            downs)}) do
          {{:ok, up}, {:ok, down}} ->
  ...
  defp init_upgrade_test(level) do
    ...
    oldRel = (case (:test_server.is_release_available(fromVsn)) do
                true ->
                  {:release, fromVsn}
                false ->
                  case (:ct.get_config({:otp_releases,
                                          :erlang.list_to_atom(fromVsn)})) do
                    :undefined ->
                      false
                    prog0 ->
    ...
    case (oldRel) do
      false ->
        :ct.log('Release ~tp is not available. Upgrade on \'~p\' level cannot be tested.', [fromVsn, level])
        :undefined
      _ ->
  ...
  defp fetch_all_apps(node) do
    ...
    appVsns = :lists.flatmap(fn f ->
                               ...
                               appFiles)
    ertsVsn = :rpc.call(node, :erlang, :system_info,
                          [:version])
    {[{:erts, ertsVsn} | appVsns], paths}
  end

  defp upgrade(apps, level, callback, createDir, installDir,
            config) do
    ...
    case (:proplists.get_value(:release_test, config)) do
      ...
      rTConfig ->
        case (:proplists.get_value(level, rTConfig)) do
          ...
          data ->
            {fromVsn, fromRel, fromAppsVsns} = target_system(apps,
                                                               createDir,
                                                               installDir, data)
            {toVsn, toRel, toAppsVsns} = upgrade_system(apps,
                                                          fromRel, createDir,
                                                          installDir, data)
  ...
  defp target_system(apps, createDir, installDir,
            {fromVsn, _, allAppsVsns, path}) do
    relName0 = 'otp-' ++ fromVsn
    appsVsns = (for {a, v, d} <- allAppsVsns,
    ...
    :ok = :file.change_mode(:filename.join(ertsBinDir, 'start_erl.src'),
                              493)
    subst_src_scripts(['erl', 'start', 'start_erl'], ertsBinDir, binDir,
                        [{'FINAL_ROOTDIR', installDir}, {'EMU', 'beam'}], [:preserve])
  ...
  defp start_script() do
    ['#!/bin/sh\nROOTDIR=%FINAL_ROOTDIR%\n\nif [ -z "$RELDIR" ]\nthen\n   RELDIR=$ROOTDIR/releases\nfi\n\nSTART_ERL_DATA=${1:-$RELDIR/start_erl.data}\n\n$ROOTDIR/bin/run_erl -daemon /tmp/ $ROOTDIR/log "exec $ROOTDIR/bin/start_erl $ROOTDIR $RELDIR $START_ERL_DATA -sname ', :erlang.atom_to_list(:"ct_release_test-upgrade"), ' -heart"\n']
  end

  defp upgrade_system(apps, fromRel, createDir, installDir,
            {_, toVsn, _, _}) do
    :ct.log('Generating release to upgrade to.')
    relName0 = 'otp-' ++ toVsn
    appsVsns = get_vsns(apps)
    {relName, _} = create_relfile(appsVsns, createDir,
                                    relName0, toVsn)
    fromPath = :filename.join([installDir, :lib, '*', :ebin])
    :ok = systools(:make_script, [relName])
    :ok = systools(:make_relup,
                     [relName, [fromRel], [fromRel], [{:path, [fromPath]},
                                                          {:outdir,
                                                             createDir}]])
  ...
  defp do_upgrade({cb, initState}, fromVsn, fromAppsVsns, toRel,
            toAppsVsns, installDir) do
    :ct.log('Upgrade test attempting to start node.~nIf test fails, logs can be found in:~n~ts', [:filename.join(installDir, :log)])
    ...
    state1 = do_callback(node, cb, :upgrade_init,
                           [ctData, initState])
    [{'OTP upgrade test', ^fromVsn, _, :permanent}] = :rpc.call(node,
                                                 :release_handler,
                                                 :which_releases, [])
    ...
    {:ok, toVsn} = :rpc.call(node, :release_handler,
                               :unpack_release, [toRelName])
    [{'OTP upgrade test', ^toVsn, _, :unpacked}, {'OTP upgrade test', ^fromVsn, _,
                                   :permanent}] = :rpc.call(node,
                                                              :release_handler,
                                                              :which_releases,
                                                              [])
    :ct.log('Installing new release')
    case (:rpc.call(node, :release_handler,
                      :install_release, [toVsn])) do
      {:ok, ^fromVsn, _} ->
        :ok
    ...
    [{'OTP upgrade test', ^toVsn, _, :current}, {'OTP upgrade test', ^fromVsn, _,
                                  :permanent}] = :rpc.call(node,
                                                             :release_handler,
                                                             :which_releases,
                                                             [])
    :ct.log('Permanenting new release')
    :ok = :rpc.call(node, :release_handler, :make_permanent,
                      [toVsn])
    [{'OTP upgrade test', ^toVsn, _, :permanent}, {'OTP upgrade test', ^fromVsn, _,
                                    :old}] = :rpc.call(node, :release_handler,
                                                         :which_releases, [])
    state2 = do_callback(node, cb, :upgrade_upgraded,
                           [ctData, state1])
    :ct.log('Re-installing old release')
    case (:rpc.call(node, :release_handler,
                      :install_release, [fromVsn])) do
      {:ok, ^fromVsn, _} ->
        :ok
      {:continue_after_restart, ^fromVsn, _} ->
        :ct.log('Waiting for node restart')
    ...
    :ok = :rpc.call(node, :release_handler, :make_permanent,
                      [fromVsn])
    [{'OTP upgrade test', ^toVsn, _, :old}, {'OTP upgrade test', ^fromVsn, _,
                              :permanent}] = :rpc.call(node, :release_handler,
                                                         :which_releases, [])
    _State3 = do_callback(node, cb, :upgrade_downgraded,
                            [ctData, state2])
  ...
  defp do_callback(node, mod, func, args) do
    ...
    case (r) do
      {:badrpc, error} ->
        throw({:fail,
                 {:test_upgrade_callback, mod, func, args, error}})
      newState ->
        newState
    end
  end
  ...
  defp create_relfile(appsVsns, createDir, relName0, relVsn) do
    upgradeAppsVsns = (for {a, v, _D} <- appsVsns do
                         {a, v, restart_type(a)}
                       end)
    coreAppVsns0 = get_vsns([:kernel, :stdlib, :sasl])
    coreAppVsns = (for {a, v, _D} <- coreAppVsns0,
                         false == :lists.keymember(a, 1, appsVsns) do
                     {a, v, restart_type(a)}
                   end)
    apps = (for {app, _, _} <- appsVsns do
              app
            end)
    startDepsVsns = get_start_deps(apps, coreAppVsns)
    startApps = (for {startApp, _, _} <- startDepsVsns do
                   startApp
                 end) ++ apps
    {runtimeDepsVsns, _} = get_runtime_deps(startApps,
                                              startApps, [], [])
    allAppsVsns0 = startDepsVsns ++ upgradeAppsVsns ++ runtimeDepsVsns
    testToolAppsVsns0 = get_vsns([:common_test])
    testToolAppsVsns = (for {a, v, _D} <- testToolAppsVsns0,
                              false == :lists.keymember(a, 1, allAppsVsns0) do
                          {a, v, :none}
                        end)
    allAppsVsns1 = allAppsVsns0 ++ testToolAppsVsns
    allAppsVsns = (for (aV = {a, _, _}) <- allAppsVsns1,
                         false == :lists.member(a, [:dialyzer]) do
                     aV
                   end)
    ertsVsn = :erlang.system_info(:version)
    relContent = {:release, {'OTP upgrade test', relVsn}, {:erts, ertsVsn},
                    allAppsVsns}
    relName = :filename.join(createDir, relName0)
    relFile = relName ++ '.rel'
    {:ok, fd} = :file.open(relFile,
                             [:write, {:encoding, :utf8}])
  ...
  defp get_start_deps([app | apps], acc) do
    ...
    {:ok, startDeps} = :application.get_key(app,
                                              :applications)
    startDepsVsns = (for startApp <- startDeps,
    ...
    depsStartDeps = get_start_deps(startDeps,
                                     acc ++ startDepsVsns)
  ...
  defp get_runtime_deps([app | apps], startApps, acc, visited) do
    case (:lists.member(app, visited)) do
      ...
      false ->
        ...
        {:ok,
           [{:application, ^app, attrs}]} = :file.consult(appFile)
        runtimeDeps = :lists.flatmap(fn str ->
                                          [runtimeAppStr,
                                               _] = :string.lexemes(str, '-')
                                          runtimeApp = :erlang.list_to_atom(runtimeAppStr)
                                          case ({:lists.keymember(runtimeApp, 1,
                                                                    acc),
                                                   :lists.member(runtimeApp,
                                                                   startApps)}) do
                                            {false, false}
                                       ...
                                       :proplists.get_value(:runtime_dependencies,
                                                              attrs, []))
        runtimeDepsVsns = (for runtimeApp <- runtimeDeps do
                             (
                               _ = :application.load(runtimeApp)
                               {:ok,
                                  runtimeVsn} = :application.get_key(runtimeApp,
                                                                       :vsn)
                               {runtimeApp, runtimeVsn, :none}
                             )
                           end)
        {depsRuntimeDeps,
           newVisited} = get_runtime_deps(runtimeDeps, startApps,
                                            acc ++ runtimeDepsVsns,
                                            [app | visited])
        get_runtime_deps(apps, startApps, depsRuntimeDeps,
                           newVisited)
    end
  end
  ...
  defp restart_type(app) when app == :kernel or app == :stdlib or
                      app == :sasl do
    :permanent
  end
  ...
  defp wait_node_up(expStatus, expVsn, expAppsVsns0) do
    ...
    expAppsVsns = (for {a, v, _D} <- expAppsVsns0 do
                     {a, v}
                   end)
    wait_node_up(node, expStatus, expVsn,
                   :lists.keysort(1, expAppsVsns), 60)
  end

  defp wait_node_up(node, expStatus, expVsn, expAppsVsns, 0) do
    :ct.fail({:node_not_started, :app_check_failed, expVsn,
                expAppsVsns,
    ...
    case ({:rpc.call(node, :release_handler,
      ...
      {[{_, ^expVsn, _, _}], apps} when is_list(apps) ->
        case (for {a, _, v} <- :lists.keysort(1, apps),
          ...
          _ ->
            :timer.sleep(2000)
            wait_node_up(node, expStatus, expVsn, expAppsVsns,
                           n - 1)
        end
      _ ->
        :timer.sleep(2000)
