defmodule :m_ct_release_test do
  ...
  def upgrade(apps, level, callback, config) do
    ...
    :ok = :filelib.ensure_dir(:filename.join(createDir, ~c"*"))
    :ok = :filelib.ensure_dir(:filename.join(installDir, ~c"*"))
    ...
    try do
      ...
      :ok ->
        tars = :filelib.wildcard(:filename.join(createDir, ~c"*.tar.gz"))
  ...
  def get_appup(r_ct_data(from: fromApps, to: toApps), app) do
    case :lists.keyfind(app, 1, toApps) do
      {^app, toVsn, toDir} ->
        appup = :filename.join([toDir, ~c"ebin", :erlang.atom_to_list(app) ++ ~c".appup"])
  ...
  defp init_upgrade_test() do
    ...
    case :release_handler.which_releases() do
      [{_, _, [], _}] ->
        throw({:skip, ~c"Need a real release running to create other releases"})
  ...
  defp init_upgrade_test(level) do
    ...
    case oldRel do
      false ->
        :ct.log(~c"Release ~tp is not available. Upgrade on '~p' level cannot be tested.", [
          fromVsn,
          level
        ])
  ...
  defp get_rels(:minor) do
    ...
    current = currentMajor ++ ~c"_patched"
    {currentMajor, current}
  end

  defp init_upgrade_test(fromVsn, toVsn, oldRel) do
    name = :erlang.list_to_atom(~c"ct_release_test-otp-" ++ fromVsn)
    :ct.log(~c"Starting node to fetch application versions to upgrade from")
  ...
  defp fetch_all_apps(node) do
    ...
    appFiles =
      :lists.flatmap(
        fn p ->
          :filelib.wildcard(:filename.join(p, ~c"*.app"))
        end,
        paths
      )
  ...
  defp upgrade(apps, level, callback, createDir, installDir, config) do
    :ct.log(~c"Test upgrade of the following applications: ~p", [apps])
    :ct.log(~c".rel files and start scripts are created in:~n~ts", [createDir])
    :ct.log(~c"The release is installed in:~n~ts", [installDir])

    case :proplists.get_value(:release_test, config) do
      :undefined ->
        throw({:fail, ~c"ct_release_test:init/1 not run"})

      rTConfig ->
        case :proplists.get_value(level, rTConfig) do
          :undefined ->
            throw({:skip, ~c"Old release not available"})
          ...
          data ->
            ...
            :ct.log(~c"Upgrade from: OTP-~ts, ~tp", [fromVsn, fromAppsVsns])
            :ct.log(~c"Upgrade to: OTP-~ts, ~tp", [toVsn, toAppsVsns])
  ...
  defp target_system(apps, createDir, installDir, {fromVsn, _, allAppsVsns, path}) do
    ...
    binDir = :filename.join([installDir, ~c"bin"])
    :ok = make_dir(binDir)
    :ok = make_dir(:filename.join(installDir, ~c"log"))
    ertsBinDir = :filename.join([installDir, ~c"erts-" ++ ertsVsn, ~c"bin"])
    :ok = delete_file(:filename.join([ertsBinDir, ~c"erl"]))
    :ok = delete_file(:filename.join([ertsBinDir, ~c"start"]))
    :ok = delete_file(:filename.join([ertsBinDir, ~c"start_erl"]))
    copy_file(relName ++ ~c".boot", :filename.join([binDir, ~c"start.boot"]))

    copy_file(:filename.join([ertsBinDir, ~c"epmd"]), :filename.join([binDir, ~c"epmd"]), [
      :preserve
    ])

    copy_file(:filename.join([ertsBinDir, ~c"run_erl"]), :filename.join([binDir, ~c"run_erl"]), [
      :preserve
    ])

    copy_file(:filename.join([ertsBinDir, ~c"to_erl"]), :filename.join([binDir, ~c"to_erl"]), [
      :preserve
    ])

    startErlData = :filename.join([installDir, ~c"releases", ~c"start_erl.data"])

    :ok =
      write_file(
        startErlData,
        :io_lib.fwrite(~c"~s ~s~n", [ertsVsn, fromVsn])
      )

    sysConfig = :filename.join([installDir, ~c"releases", fromVsn, ~c"sys.config"])
    :ok = write_file(sysConfig, ~c"[].")
    startSrc = :filename.join(ertsBinDir, ~c"start.src")
    :ok = write_file(startSrc, start_script())
    :ok = :file.change_mode(startSrc, 493)

    :ok =
      :file.change_mode(
        :filename.join(ertsBinDir, ~c"start_erl.src"),
        493
      )
    ...
    subst_src_scripts(
      ...
      ertsBinDir,
      binDir,
      [{~c"FINAL_ROOTDIR", installDir}, {~c"EMU", ~c"beam"}],
      [:preserve]
    )

    relFile = :filename.join([installDir, ~c"releases", :filename.basename(relName) ++ ~c".rel"])
  ...
  defp upgrade_system(apps, fromRel, createDir, installDir, {_, toVsn, _, _}) do
    :ct.log(~c"Generating release to upgrade to.")
    relName0 = ~c"otp-" ++ toVsn
    appsVsns = get_vsns(apps)
    {relName, _} = create_relfile(appsVsns, createDir, relName0, toVsn)
    fromPath = :filename.join([installDir, :lib, ~c"*", :ebin])
    ...
    sysConfig = :filename.join([createDir, ~c"sys.config"])
    :ok = write_file(sysConfig, ~c"[].")
  ...
  defp do_upgrade({cb, initState}, fromVsn, fromAppsVsns, toRel, toAppsVsns, installDir) do
    ...
    copy_file(
      toRel ++ ~c".tar.gz",
      :filename.join([installDir, :releases, toRelName ++ ~c".tar.gz"])
    )

    :ct.log(~c"Unpacking new release")
    {:ok, toVsn} = :rpc.call(node, :release_handler, :unpack_release, [toRelName])

    [
      {~c"OTP upgrade test", ^toVsn, _, :unpacked},
      {~c"OTP upgrade test", ^fromVsn, _, :permanent}
    ] =
      :rpc.call(
        node,
    ...
    :ct.log(~c"Installing new release")
    ...
    case :rpc.call(node, :release_handler, :install_release, [toVsn]) do
      ...
      {:continue_after_restart, ^fromVsn, _} ->
        :ct.log(~c"Waiting for node restart")
    end

    {:ok, _} = wait_node_up(:current, toVsn, toAppsVsns)

    [{~c"OTP upgrade test", ^toVsn, _, :current}, {~c"OTP upgrade test", ^fromVsn, _, :permanent}] =
      :rpc.call(
        node,
        :release_handler,
        :which_releases,
    ...
    :ct.log(~c"Permanenting new release")
    :ok = :rpc.call(node, :release_handler, :make_permanent, [toVsn])

    [{~c"OTP upgrade test", ^toVsn, _, :permanent}, {~c"OTP upgrade test", ^fromVsn, _, :old}] =
      :rpc.call(node, :release_handler, :which_releases, [])

    state2 = do_callback(node, cb, :upgrade_upgraded, [ctData, state1])
    :ct.log(~c"Re-installing old release")
    ...
    [{~c"OTP upgrade test", ^toVsn, _, :permanent}, {~c"OTP upgrade test", ^fromVsn, _, :current}] =
      :rpc.call(
        node,
        :release_handler,
        :which_releases,
    ...
    :ct.log(~c"Permanenting old release")
    :ok = :rpc.call(node, :release_handler, :make_permanent, [fromVsn])

    [{~c"OTP upgrade test", ^toVsn, _, :old}, {~c"OTP upgrade test", ^fromVsn, _, :permanent}] =
      :rpc.call(node, :release_handler, :which_releases, [])

    _State3 = do_callback(node, cb, :upgrade_downgraded, [ctData, state2])
    :ct.log(~c"Terminating node ~p", [node])
  ...
  defp do_callback(node, mod, func, args) do
    ...
    :ct.log(~c"Calling ~p:~tp/1", [mod, func])
    r = :rpc.call(node, mod, func, args)
    :ct.log(~c"~p:~tp/~w returned: ~tp", [mod, func, length(args), r])
  ...
  defp create_relfile(appsVsns, createDir, relName0, relVsn) do
    ...
    relContent = {:release, {~c"OTP upgrade test", relVsn}, {:erts, ertsVsn}, allAppsVsns}
    ...
    :io.format(fd, ~c"~tp.~n", [relContent])
    :ok = :file.close(fd)
    {relName, ertsVsn}
  end
  ...
  defp get_runtime_deps([app | apps], startApps, acc, visited) do
    case :lists.member(app, visited) do
      ...
      false ->
        appFile = :code.where_is_file(:erlang.atom_to_list(app) ++ ~c".app")
        {:ok, [{:application, ^app, attrs}]} = :file.consult(appFile)

        runtimeDeps =
          :lists.flatmap(
            fn str ->
              [runtimeAppStr, _] = :string.lexemes(str, ~c"-")
  ...
  defp subst(str, [{var, val} | vars]) do
    subst(
      :re.replace(str, ~c"%" ++ var ++ ~c"%", val, [{:return, :list}, :unicode]),
      vars
    )
  end
  ...
  defp node_name(sname) do
    ...
    :erlang.list_to_atom(:erlang.atom_to_list(sname) ++ ~c"@" ++ host)
  end
