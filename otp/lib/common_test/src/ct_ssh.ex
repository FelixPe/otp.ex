defmodule :m_ct_ssh do
  use Bitwise
  require Record
  Record.defrecord(:r_state, :state, ssh_ref: :undefined,
                                 conn_type: :undefined, target: :undefined)
  ...
  def connect(keyOrName, connType, extraOpts) do
    case (:ct.get_config(keyOrName)) do
      ...
      sSHData ->
        ...
        {connType1, addr, allOpts1} = (case (connType) do
                                         :host ->
                                           case (:proplists.get_value(:ssh,
                                                                        allOpts)) do
                                             :undefined ->
                                               case (:proplists.get_value(:sftp,
                                                                            allOpts)) do
                                                 :undefined ->
                                                   log(heading(:connect,
                                                                 keyOrName),
                                         ...
                                         _ ->
                                           case (:proplists.get_value(connType,
                                                                        allOpts)) do
                                             :undefined when connType == :ssh ->
                                               case (:proplists.get_value(:sftp,
                                                 ...
                                                 sFTPAddr ->
                                                   try_log(heading(:connect,
                                                                     keyOrName),
                                                             'Note: Opening ssh connection to sftp host.\n', [])
                                                   {:ssh, sFTPAddr,
                                                      [{:ssh, sFTPAddr} |
                                                           :proplists.delete(:sftp,
                                                                               allOpts)]}
                                               end
                                             :undefined when connType == :sftp
                                               ...
                                               case (:proplists.get_value(:ssh,
                                                 ...
                                                 sSHAddr ->
                                                   ...
                                                   {:sftp, sSHAddr,
                                                      [{:sftp, sSHAddr} |
                                                           :proplists.delete(:ssh,
                                                                               allOpts)]}
                                               end
                                             sSHorSFTPAddr ->
                                               {connType, sSHorSFTPAddr,
                                                  allOpts}
                                           end
                                       end)
  ...
  def send_and_receive(sSH, channelId, data, timeout)
      when is_integer(timeout) do
    send_and_receive(sSH, channelId, 0, data, :close,
                       timeout)
  end

  def send_and_receive(sSH, channelId, type, data)
      when is_integer(type) do
    send_and_receive(sSH, channelId, type, data, :close,
                       10000)
  end

  def send_and_receive(sSH, channelId, data, end__, timeout)
      when is_integer(timeout) do
    send_and_receive(sSH, channelId, 0, data, end__,
                       timeout)
  end
  ...
  def send_and_receive(sSH, channelId, type, data, end__)
      when is_function(end__) do
    send_and_receive(sSH, channelId, type, data, end__,
                       10000)
  end

  def send_and_receive(sSH, channelId, type, data, end__, timeout) do
    call(sSH,
           {:send_and_receive, channelId, type, data, end__,
              timeout})
  end
  ...
  def init(keyOrName, {connType, addr, port}, allOpts) do
    ...
    allOpts1 = (case (:proplists.get_value(:connect_timeout,
                                             allOpts)) do
                  :undefined ->
    ...
    options = :lists.foldl(fn {:ssh, _}, opts ->
                                opts
                              {:sftp, _}, opts ->
                                opts
                              {:port, _}, opts ->
        ...
        try_log(heading(:init, keyOrName), 'Opened ~w connection:\nHost: ~tp (~p)\nUser: ~tp\nPassword: ~p\n',
                  [connType, addr, port, user,
                                             :lists.duplicate(:string.length(password),
                                                                ?*)])
        {:ok, sSHRef,
           r_state(ssh_ref: sSHRef, conn_type: connType,
               target: keyOrName)}
    end
  end
  ...
  def handle_msg({:session_close, chn}, state) do
    r_state(ssh_ref: sSHRef, target: target) = state
    try_log(heading(:session_close, target), 'SSH Ref: ~p, Chn: ~p',
              [sSHRef, chn])
  ...
  def handle_msg({:exec, chn, command, tO}, state) do
    ...
    case (chn1) do
      ...
      _ ->
        ...
        case (:ssh_connection.exec(sSHRef, chn1, command,
                                     tO)) do
          :success ->
  ...
  def handle_msg({:send, chn, type, data, tO}, state) do
    ...
    result = :ssh_connection.send(sSHRef, chn, type, data,
                                    tO)
    {result, state}
  end

  def handle_msg({:send_and_receive, chn, type, data, end__, tO},
           state) do
    r_state(ssh_ref: sSHRef, target: target) = state
    try_log(heading(:send_and_receive, target), 'SSH Ref: ~p, Chn: ~p, Type: ~p, Timeout: ~p~nData: ~tp',
              [sSHRef, chn, type, tO, data])
    case (:ssh_connection.send(sSHRef, chn, type, data,
                                 tO)) do
      :ok ->
        result = do_recv_response(sSHRef, chn, [], end__, tO)
        {result, state}
  ...
  def handle_msg({:subsystem, chn, subsystem, tO}, state) do
    ...
    result = :ssh_connection.subsystem(sSHRef, chn,
                                         subsystem, tO)
    {result, state}
  end
  ...
  def handle_msg({:read_file, srv, file} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
  ...
  def handle_msg({:write_file, srv, file, iolist} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.write_file(ref(srv, sSHRef), file, iolist),
       s}
  end

  def handle_msg({:list_dir, srv, path} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:open, srv, file, mode} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:opendir, srv, path} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:close, srv, handle} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:read, srv, handle, len} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:pread, srv, handle, position, length} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.pread(ref(srv, sSHRef), handle, position,
                       length),
       s}
  end

  def handle_msg({:aread, srv, handle, len} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:apread, srv, handle, position, length} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.apread(ref(srv, sSHRef), handle, position,
                        length),
       s}
  end

  def handle_msg({:write, srv, handle, data} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:pwrite, srv, handle, position, data} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.pwrite(ref(srv, sSHRef), handle, position,
                        data),
       s}
  end

  def handle_msg({:awrite, srv, handle, data} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:apwrite, srv, handle, position, data} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.apwrite(ref(srv, sSHRef), handle, position,
                         data),
       s}
  end

  def handle_msg({:position, srv, handle, location} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.position(ref(srv, sSHRef), handle, location),
       s}
  end

  def handle_msg({:read_file_info, srv, name} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:get_file_info, srv, handle} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:read_link_info, srv, name} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:write_file_info, srv, name, info} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.write_file_info(ref(srv, sSHRef), name,
                                 info),
       s}
  end

  def handle_msg({:read_link, srv, name} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:make_symlink, srv, name, target} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.make_symlink(ref(srv, sSHRef), name, target),
       s}
  end

  def handle_msg({:rename, srv, oldName, newName} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
    {:ssh_sftp.rename(ref(srv, sSHRef), oldName, newName),
       s}
  end

  def handle_msg({:delete, srv, name} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:make_dir, srv, name} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def handle_msg({:del_dir, srv, name} = cmd,
           s = r_state(ssh_ref: sSHRef)) do
    try_log(heading(:sftp, r_state(s, :target)), 'SSH Ref: ~p, Server: ~p~nCmd: ~tp',
              [sSHRef, ref(srv, sSHRef), mod(cmd)])
  ...
  def terminate(sSHRef, state) do
    case (r_state(state, :conn_type)) do
      :ssh ->
        try_log(heading(:disconnect_ssh, r_state(state, :target)), 'SSH Ref: ~p',
                  [sSHRef], 5000)
        :ssh.close(sSHRef)
      :sftp ->
        try_log(heading(:disconnect_sftp, r_state(state, :target)), 'SFTP Ref: ~p',
                  [sSHRef], 5000)
  ...
  defp do_recv_response(sSH, chn, data, end__, timeout) do
    receive do
      ...
      {:ssh_cm, ^sSH, {:data, ^chn, _, newData}}
          when is_binary(newData) ->
        :ssh_connection.adjust_window(sSH, chn,
                                        byte_size(newData))
      ...
      {:ssh_cm, ^sSH, {:eof, ^chn}} ->
        ...
        {:ok, data}
      {:ssh_cm, ^sSH,
         {:exit_signal, ^chn, signal, err, _Lang}} ->
