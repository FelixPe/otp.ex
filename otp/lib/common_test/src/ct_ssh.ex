defmodule :m_ct_ssh do
  ...
  def connect(keyOrName, connType, extraOpts) do
    case :ct.get_config(keyOrName) do
      :undefined ->
        log(heading(:connect, keyOrName), ~c"Failed: ~tp\n", [{:not_available, keyOrName}])
      ...
      sSHData ->
                  ...
                  case :proplists.get_value(
                    ...
                    :undefined ->
                      log(
                        ...
                        ~c"No host information specified!\n",
                        []
                      )
                  ...
                  case :proplists.get_value(
                    ...
                    sFTPAddr ->
                      try_log(
                        ...
                        ~c"Note: Opening ssh connection to sftp host.\n",
                        []
                      )
                  ...
                  case :proplists.get_value(
                    ...
                    sSHAddr ->
                      try_log(
                        ...
                        ~c"Note: Opening sftp connection to ssh host.\n",
                        []
                      )
        ...
        case {addr, :proplists.get_value(:port, allOpts1)} do
          ...
          {_, :undefined} ->
            try_log(heading(:connect, keyOrName), ~c"Opening ~w connection to ~tp:22\n", [
              connType1,
              addr
            ])
          ...
          {_, port} ->
            try_log(heading(:connect, keyOrName), ~c"Opening ~w connection to ~tp:~w\n", [
              connType1,
              addr,
              port
            ])
  ...
  def disconnect(sSH) do
    case get_handle(sSH) do
      {:ok, pid} ->
        try_log(heading(:disconnect, sSH), ~c"Handle: ~p", [pid], 5000)
  ...
  def handle_msg(:sftp_connect, state) do
    ...
    try_log(heading(:sftp_connect, target), ~c"SSH Ref: ~p", [sSHRef])
    {:ssh_sftp.start_channel(sSHRef), state}
  end

  def handle_msg({:session_open, tO}, state) do
    r_state(ssh_ref: sSHRef, target: target) = state
    try_log(heading(:session_open, target), ~c"SSH Ref: ~p, Timeout: ~p", [sSHRef, tO])
    {:ssh_connection.session_channel(sSHRef, tO), state}
  end

  def handle_msg({:session_close, chn}, state) do
    r_state(ssh_ref: sSHRef, target: target) = state
    try_log(heading(:session_close, target), ~c"SSH Ref: ~p, Chn: ~p", [sSHRef, chn])
  ...
  def handle_msg({:exec, chn, command, tO}, state) do
    ...
    chn1 =
      cond do
        chn == :undefined ->
          try_log(heading(:exec, target), ~c"Opening channel for exec, SSH Ref: ~p", [sSHRef])
    ...
    case chn1 do
      {:error, _} = chnError ->
        log(heading(:exec, target), ~c"Opening channel failed: ~tp", [chnError])
        {chnError, state}

      _ ->
        try_log(heading(:exec, target), ~c"SSH Ref: ~p, Chn: ~p, Command: ~tp, Timeout: ~p", [
          sSHRef,
          chn1,
          command,
          tO
  ...
  def handle_msg({:receive_response, chn, end__, tO}, state) do
    ...
    try_log(heading(:receive_response, target), ~c"SSH Ref: ~p, Chn: ~p, Timeout: ~p", [
      sSHRef,
      chn,
      tO
    ])
  ...
  def handle_msg({:send, chn, type, data, tO}, state) do
    ...
    try_log(heading(:send, target), ~c"SSH Ref: ~p, Chn: ~p, Type: ~p, Timeout: ~p~nData: ~tp", [
      sSHRef,
      chn,
      type,
      tO,
  ...
  def handle_msg(
    ...
    try_log(
      ...
      ~c"SSH Ref: ~p, Chn: ~p, Type: ~p, Timeout: ~p~nData: ~tp",
  ...
  def handle_msg({:subsystem, chn, subsystem, tO}, state) do
    ...
    try_log(heading(:subsystem, target), ~c"SSH Ref: ~p, Chn: ~p, Subsys: ~tp, Timeout: ~p", [
      sSHRef,
      chn,
      subsystem,
      tO
  ...
  def handle_msg({:shell, chn, tO}, state) do
    r_state(ssh_ref: sSHRef, target: target) = state
    try_log(heading(:shell, target), ~c"SSH Ref: ~p, Chn: ~p, Timeout: ~p", [sSHRef, chn, tO])
  ...
  def handle_msg(
    ...
    try_log(heading(:sftp, r_state(s, :target)), ~c"SSH Ref: ~p, Server: ~p~nCmd: ~tp", [
      sSHRef,
      ref(srv, sSHRef),
      mod(cmd)
    ])
  ...
  def terminate(sSHRef, state) do
    case r_state(state, :conn_type) do
      :ssh ->
        try_log(
          heading(:disconnect_ssh, r_state(state, :target)),
          ~c"SSH Ref: ~p",
          [sSHRef],
          5000
        )
  ...
  defp do_recv_response(sSH, chn, data, end__, timeout) do
    receive do
      ...
      {:ssh_cm, ^sSH, {:eof, ^chn}} ->
        debug(~c"RECVD EOF~n~p ~p", [sSH, chn])
        {:ok, data}

      {:ssh_cm, ^sSH, {:exit_signal, ^chn, signal, err, _Lang}} ->
        debug(~c"RECVD exit_signal~n~p ~p~n~p ~p", [sSH, chn, signal, err])
        do_recv_response(sSH, chn, data, end__, timeout)

      {:ssh_cm, ^sSH, {:exit_status, ^chn, status}} ->
        debug(~c"RECVD exit_status~n~p ~p~n~p", [sSH, chn, status])
        do_recv_response(sSH, chn, data, end__, timeout)

      other ->
        debug(~c"UNEXPECTED MESSAGE~n~p ~p~n~tp", [sSH, chn, other])
        do_recv_response(sSH, chn, data, end__, timeout)
    after
      timeout ->
        case end__ do
  ...
  defp heading(function, ref) do
    :io_lib.format(~c"ct_ssh:~tw ~tp", [function, ref])
  end
