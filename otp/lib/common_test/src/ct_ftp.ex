defmodule :m_ct_ftp do
  ...
  def open(keyOrName) do
    case :ct_util.get_key_from_name(keyOrName) do
      {:ok, :node} ->
        open(keyOrName, ~c"erlang", ~c"x")

      _ ->
        case :ct.get_config(keyOrName) do
          :undefined ->
            log(heading(:open, keyOrName), ~c"Failed: ~tp\n", [{:not_available, keyOrName}])
  ...
  def handle_msg({:send, localFile, remoteFile}, state) do
    log(heading(:send, r_state(state, :target_name)), ~c"LocalFile: ~tp\nRemoteFile: ~tp\n", [
      localFile,
      remoteFile
    ])
  ...
  def handle_msg({:recv, remoteFile, localFile}, state) do
    log(heading(:recv, r_state(state, :target_name)), ~c"RemoteFile: ~tp\nLocalFile: ~tp\n", [
      remoteFile,
      localFile
    ])
  ...
  defp heading(function, name) do
    :io_lib.format(~c"ct_ftp:~tw ~tp", [function, name])
  end
