defmodule :m_ct_ftp do
  ...
  Record.defrecord(:r_state, :state, ftp_pid: :undefined,
  ...
  defp open(keyOrName, username, password) do
    ...
    case (:ct.get_config({keyOrName, :ftp})) do
      ...
      addr ->
        :ct_gen_conn.start(keyOrName, full_addr(addr),
                             {username, password}, :ct_ftp)
    end
  end
  ...
  def init(keyOrName, {iP, port}, {username, password}) do
    case (ftp_connect(iP, port, username, password)) do
      {:ok, ftpPid} ->
        log(heading(:init, keyOrName), 'Opened ftp connection:\nIP: ~tp\nUsername: ~tp\nPassword: ~p\n',
              [iP, username,
                       :lists.duplicate(:string.length(password), ?*)])
        {:ok, ftpPid,
           r_state(ftp_pid: ftpPid, target_name: keyOrName)}
      error ->
        error
    end
  end
  ...
  def handle_msg({:send, localFile, remoteFile}, state) do
    ...
    result = :ftp.send(r_state(state, :ftp_pid), localFile,
                         remoteFile)
    {result, state}
  end
  ...
  def handle_msg({:recv, remoteFile, localFile}, state) do
    ...
    result = :ftp.recv(r_state(state, :ftp_pid), remoteFile,
                         localFile)
    {result, state}
  end
  ...
  def terminate(ftpPid, state) do
    log(heading(:terminate, r_state(state, :target_name)), 'Closing FTP connection.\nHandle: ~p\n',
          [ftpPid])
    :ftp.close(ftpPid)
  end
