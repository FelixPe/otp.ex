defmodule :m_ct_master_event do
  ...
  def stop() do
    case flush() do
      {:error, reason} ->
        :ct_master_logs.log(
          ~c"Error",
          ~c"No response from CT Master Event.\nReason = ~tp\nTerminating now!\n",
          [reason]
        )
  ...
  def init(_) do
    ...
    :ct_master_logs.log(~c"CT Master Event Handler started", ~c"", [])
    {:ok, r_state()}
  end
  ...
  def handle_event(
    ...
    :ct_master_logs.log(~c"CT Master Event Handler", ~c"Got ~ts from ~w", [runDir, node])
    :ct_master_logs.nodedir(node, runDir)
    {:ok, state}
  end

  def handle_event(r_event(name: name, node: node, data: data), state) do
    print(~c"~n=== ~w ===~n", [:ct_master_event])
  ...
  def terminate(_Reason, _State) do
    :ct_master_logs.log(~c"CT Master Event Handler stopping", ~c"", [])
    :ok
  end
