defmodule :m_ct_master_event do
  ...
  def add_handler() do
    :gen_event.add_handler(:ct_master_event,
                             :ct_master_event, [])
  end

  def add_handler(args) do
    :gen_event.add_handler(:ct_master_event,
                             :ct_master_event, args)
  end

  def stop() do
    case (flush()) do
      {:error, reason} ->
        :ct_master_logs.log('Error', 'No response from CT Master Event.\nReason = ~tp\nTerminating now!\n', [reason])
        (try do
  ...
  defp flush() do
    try do
      :gen_event.call(:ct_master_event, :ct_master_event,
                        :flush, 1800000)
    catch
      _, reason ->
        {:error, reason}
  ...
  def handle_event(r_event(name: :start_logging, node: node,
             data: runDir),
           state) do
  ...
  def handle_call(:flush, state) do
    case (:erlang.process_info(self(),
                                 :message_queue_len)) do
      {:message_queue_len, 0} ->
        {:ok, :done, state}
      _ ->
        {:ok, :flushing, state}
