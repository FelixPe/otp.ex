defmodule :m_ct_hooks do
  ...
  defp call([{hook, :call_id, nextFun} | rest], config, meta, hooks) do
    try do
      ...
      error, reason ->
        :ct_logs.log(~c"Suite Hook", ~c"Failed to start a CTH: ~tp:~tp", [
          error,
          {reason, __STACKTRACE__}
        ])

        call([], {:fail, ~c"Failed to start CTH, see the CT Log for details"}, meta, hooks)
    end
  end
  ...
  defp catch_apply(m, f, a) do
    try do
      ...
      _, reason ->
        :ct_logs.log(~c"Suite Hook", ~c"Call to CTH failed: ~w:~tp", [
          :error,
          {reason, __STACKTRACE__}
        ])
        ...
        throw(
          {:error_in_cth_call,
           :lists.flatten(:io_lib.format(~c"~w:~tw/~w CTH call failed", [m, f, length(a)]))}
        )
    end
  end
