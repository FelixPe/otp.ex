defmodule :m_cth_surefire do
  ...
  def init(path, opts) do
    {:ok, pid} = :gen_server.start(:cth_surefire, [path, opts], [])
    pid
  end
  ...
  def handle_call({function, args}, _From, state)
    ...
    newState = apply(:cth_surefire, function, args ++ [state])
    {:reply, :ok, newState}
  end

  def handle_call({function, args}, _From, state) do
    {reply, newState} = apply(:cth_surefire, function, args ++ [state])
  ...
  def pre_init_per_suite(suite, config, r_state(test_cases: []) = state) do
    ...
    path =
      case r_state(state, :filepath) do
        ~c"junit_report.xml" ->
          ...
          :filename.join(rootDir, ~c"junit_report.xml")
    ...
    ast =
      case :beam_lib.chunks(
        ...
        {:ok, {^suite, [{:debug_info, {:debug_info_v1, :erl_abstract_code, {abstr, _Opts}}}]}} ->
          cond do
            abstr === :none ->
              :undefined
  ...
  defp get_file(suite) do
    case :beam_lib.chunks(:code.which(suite), [~c"CInf"]) do
      {:ok, {_, [{~c"CInf", bin}]}} ->
        source =
          :proplists.get_value(
            :source,
  ...
  def on_tc_fail(suite, _TC, res, state) do
    ...
    line =
      case get_line_from_result(suite, res) do
        :undefined ->
          r_testcase(tC, :line)
    ...
    newTC =
      r_testcase(tC,
        line: line,
        result: {:fail, :lists.flatten(:io_lib.format(~c"~tp", [res]))}
      )

    r_state(state, test_cases: [newTC | tl(tCs)])
  end

  defp get_line_from_result(
         suite,
         {_Error, [{__M, __F, __A, __I} | _] = stackTrace}
       ) do
    case :lists.filter(
           fn {mod, _Func, _Arity, _Info} ->
             mod === suite
           end,
           stackTrace
         ) do
  ...
  defp do_tc_skip(res, state) do
    tCs = r_state(state, :test_cases)
    tC = hd(tCs)
    newTC = r_testcase(tC, result: {:skipped, :lists.flatten(:io_lib.format(~c"~tp", [res]))})
    r_state(state, test_cases: [newTC | tl(tCs)])
  end

  defp init_tc(state, config) when is_list(config) == false do
    r_state(state, timer: :os.timestamp(), tc_log: ~c"")
  end
  ...
  defp end_tc(func, config, res, state = r_state(tc_log: ~c"")) do
    end_tc(
      func,
      config,
      res,
    ...
    r_state(state,
      test_cases: [
        r_testcase(
          ...
          name: name,
          time: timeTakes,
          file: r_state(state, :curr_suite_file),
          line:
            get_line_from_suite(
              r_state(state, :curr_suite_ast),
              name
            ),
          result: :passed
        )
  ...
  defp close_suite(
    ...
    suiteLog = :filename.join(r_state(state, :curr_log_dir), ~c"suite.log.html")
    ...
    suite =
      r_testsuite(
        ...
        time: :io_lib.format(~c"~f", [timeTaken]),
  ...
  def terminate(state = r_state(test_cases: [])) do
    ...
    :io.format(d, ~c"<?xml version=\"1.0\" encoding= \"UTF-8\" ?>", [])
    :io.format(d, ~c"~ts", [to_xml(state)])
  ...
  defp to_xml(
    ...
    [
      ~c"<testcase ",
      for _ <- [:EFE_DUMMY_GEN], group != ~c"" do
        [~c"group=\"", group, ~c"\" "]
      end,
      ~c"name=\"",
      n,
      ...
      for _ <- [:EFE_DUMMY_GEN],
          line != :undefined do
        [~c"line=\"", :erlang.integer_to_list(line), ~c"\" "]
      end,
      ~c"log=\"",
      l,
      ~c"\">",
      case r do
        ...
        {:skipped, reason} ->
          [
            ~c"<skipped type=\"skip\" message=\"Test ",
            n,
            ~c" in ",
            cL,
            ~c" skipped!\">",
  ...
  defp properties_to_xml(axis, props) do
    [
      ...
      for {name, value} <- axis do
        [~c"<property name=\"", name, ~c"\" axis=\"yes\" value=\"", value, ~c"\" />"]
      end,
      for {name, value} <- props do
        [~c"<property name=\"", name, ~c"\" value=\"", value, ~c"\" />"]
      end,
      ~c"</properties>"
    ]
  end
  ...
  defp now_to_string(now) do
    ...
    :io_lib.format(~c"~w-~2..0B-~2..0BT~2..0B:~2..0B:~2..0B", [yY, mM, dD, hH, mi, sS])
  end
  ...
  defp count_tcs([r_testcase(name: confCase) | tCs], ok, f, s)
       when confCase == ~c"init_per_suite" or confCase == ~c"end_per_suite" or
              confCase == ~c"init_per_group" or
              confCase == ~c"end_per_group" do
