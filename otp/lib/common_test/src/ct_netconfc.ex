defmodule :m_ct_netconfc do
  ...
  def handle_msg({:ssh_cm, cM, {:data, ch, _Type, data}}, state)
  ...
  defp opt({:receiver, dest}, r_options(receivers: t) = rec) do
    r_options(rec, receivers: [dest | t])
  end
  ...
  defp session_opt({:receiver, dest}, r_options(receivers: t) = rec) do
    r_options(rec, receivers: [dest | t])
  end
  ...
  defp chunk(bin, chunks, len) when byte_size(bin) < 4 do
    [bin, 3 = len | chunks]
  end
