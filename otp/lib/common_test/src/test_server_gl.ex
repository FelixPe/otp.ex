defmodule :m_test_server_gl do
  use Bitwise
  require Record
  Record.defrecord(:r_st, :st, tc_supervisor: :undefined,
                              tc: :undefined, minor: :undefined,
                              minor_monitor: :undefined,
                              tsio_monitor: :undefined, capture: :undefined,
                              reject_io: :undefined, permit_io: :undefined,
                              auto_nl: true, levels: :undefined,
                              escape_chars: true)
  def start_link(tSIO) do
    case (:gen_server.start_link(:test_server_gl, [tSIO],
                                   [])) do
      {:ok, pid} ->
        {:ok, pid}
      other ->
  ...
  def init([tSIO]) do
    ...
    escChars = (case (:application.get_env(:test_server,
                                             :esc_chars)) do
                  {:ok, eCBool} ->
                    eCBool
                  _ ->
    ...
    {:ok,
       r_st(tc_supervisor: :none, minor: :none,
           minor_monitor: :none, tsio_monitor: ref, capture: :none,
           reject_io: false, permit_io: :gb_sets.empty(),
           auto_nl: true, levels: {1, 19, 10},
  ...
  def handle_call(:get_tc_supervisor, _From,
           r_st(tc_supervisor: pid) = st) do
    {:reply, pid, st}
  end

  def handle_call({:set_minor_fd, fd, mFA, supervisor}, _From,
           st) do
    ref = :erlang.monitor(:process, fd)
    {:reply, :ok,
       r_st(st, tc: mFA,  minor: fd,  minor_monitor: ref, 
               tc_supervisor: supervisor)}
  end

  def handle_call(:unset_minor_fd, _From, st) do
    {:reply, :ok,
       r_st(st, minor: :none,  tc_supervisor: :none)}
  end
  ...
  def handle_info({:DOWN, ref, :process, _, reason} = d,
           r_st(minor_monitor: ref) = st) do
    case (reason) do
      :normal ->
        :ok
      _ ->
        data = :io_lib.format('=== WARNING === TC: ~tw\nGot down from minor Fd ~w: ~tw\n\n', [r_st(st, :tc), r_st(st, :minor), d])
        :test_server_io.print_unexpected(data)
    end
    {:noreply, r_st(st, minor: :none,  minor_monitor: :none)}
  end

  def handle_info({:DOWN, ref, :process, _, _},
           r_st(tsio_monitor: ref) = st) do
    {:stop, :normal, st}
  end
  ...
  def handle_info({:structured_io, clientPid, {detail, str}},
           st) do
  ...
  defp io_req({:put_chars, encoding, mod, func,
             [format, args]},
            _, _) do
    case (format) do
      ['$tc_html', format0] ->
  ...
  defp output(level, str, sender, from, st)
    ...
    output_to_file(level, dress_output(str, sender, st),
                     from, st)
  end

  defp output_to_file(:minor, data0, from,
            r_st(tc: {m, f, a}, minor: :none)) do
  ...
  defp output_to_file(:minor, data, from, r_st(tc: tC, minor: fd)) do
    try do
      ...
      type, reason ->
        data1 = [:io_lib.format('=== ERROR === TC: ~tw\nFailed to write to minor Fd: ~w\nType: ~w\nReason: ~tw\n', [tC, fd, type, reason]),
                     data, '\n']
