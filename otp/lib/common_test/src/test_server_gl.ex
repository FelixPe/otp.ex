defmodule :m_test_server_gl do
  ...
  def handle_info({:printout, detail, [~c"$tc_html", format], args}, st) do
    str = :io_lib.format(format, args)
    output(detail, [~c"$tc_html", str], :internal, :none, st)
    {:noreply, st}
  end
  ...
  defp output(level, strOrFun, sender, from, st)
    ...
    case selected_by_level(level, :stdout, st) do
      true when hd(strOrFun) == ~c"$tc_html" ->
