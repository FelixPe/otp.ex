defmodule :m_ct_groups do
  ...
  defp find(mod, _GrNames, _TCs, [badTerm | _Gs], known, _Defs, _FindAll) do
    where =
      cond do
        length(known) == 0 ->
          :erlang.atom_to_list(mod) ++ ~c":groups/0"

        true ->
          ~c"group " ++
            :erlang.atom_to_list(:lists.last(known)) ++
            ~c" in " ++ :erlang.atom_to_list(mod) ++ ~c":groups/0"
      end

    term = :io_lib.format(~c"~tp", [badTerm])
    e = ~c"Bad term " ++ :lists.flatten(term) ++ ~c" in " ++ where
  ...
  defp cyclic_test(mod, name, names) do
    case :lists.member(name, names) do
      true ->
        e =
          ~c"Cyclic reference to group " ++
            :erlang.atom_to_list(name) ++ ~c" in " ++ :erlang.atom_to_list(mod) ++ ~c":groups/0"
    ...
    {initConf, endConf, extraProps} =
      case {:erlang.function_exported(mod, :init_per_group, 2),
        ...
        {false, false} ->
          :ct_logs.log(
            ~c"TEST INFO",
            ~c"init_per_group/2 and end_per_group/2 missing for group ~tw in ~w, using default.",
            [name, mod]
          )
  ...
  def search_and_override([conf = {:conf, props, init, tests, end__}], oRSpec, mod) do
    ...
    case :lists.keysearch(name, 1, oRSpec) do
      ...
      {:value, {^name, oRProps}} ->
        [
          {:conf, insProps.(name, suite, oRProps), init, search_and_override(tests, oRSpec, mod),
           end__}
        ]
  ...
  defp invalid_ref_msg(name, mod) do
    e =
      ~c"Invalid reference to group " ++
        :erlang.atom_to_list(name) ++ ~c" in " ++ :erlang.atom_to_list(mod) ++ ~c":all/0"
  ...
  defp invalid_ref_msg(name0, name1, mod) do
    e =
      ~c"Invalid reference to group " ++
        ...
        ~c" from " ++
        :erlang.atom_to_list(name0) ++ ~c" in " ++ :erlang.atom_to_list(mod) ++ ~c":all/0"
