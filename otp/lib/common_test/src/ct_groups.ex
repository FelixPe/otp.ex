defmodule :m_ct_groups do
  ...
  def find_groups(mod, groups, tCs, groupDefs) when groups != [] do
    :lists.append(for grNames <- groups do
                    find_groups1(mod, [grNames], tCs, groupDefs)
                  end)
  end
  ...
  defp find_groups1(mod, grNames, tCs, groupDefs) do
    {grNames1, findAll} = (case (grNames) do
  ...
  defp find(mod, :all, :all, [{name, props, tests} | gs],
            known, defs, _)
      when (is_atom(name) and is_list(props) and
              is_list(tests)) do
    cyclic_test(mod, name, known)
    trim(make_conf(mod, name, props,
                     find(mod, :all, :all, tests, [name | known], defs,
                            true))) ++ find(mod, :all, :all, gs, known, defs,
                                              true)
  end

  defp find(mod, :all, tCs, [{name, props, tests} | gs],
            known, defs, _)
      when (is_atom(name) and is_list(props) and
              is_list(tests)) do
    cyclic_test(mod, name, known)
    ...
    trim(make_conf(mod, name, props,
                     find(mod, :all, tCs, tests1, [name | known], defs,
                            true))) ++ find(mod, :all, tCs, gs, known, defs,
                                              true)
  end

  defp find(mod, [name | grNames] = sPath, tCs,
            [{name, props, tests} | gs], known, defs, findAll)
      when (is_atom(name) and is_list(props) and
              is_list(tests)) do
    cyclic_test(mod, name, known)
    tests1 = modify_tc_list(tests, tCs, grNames)
    trim(make_conf(mod, name, props,
                     find(mod, grNames, tCs, tests1, [name | known], defs,
                            findAll))) ++ find(mod, sPath, tCs, gs, known, defs,
                                                 findAll)
  end

  defp find(mod, [], tCs, tests, _Known, _Defs, false) do
    cases = :lists.flatmap(fn tC when (is_atom(tC) and
                                         tCs == :all)
                                      ->
                                [{mod, tC}]
                              {:group, _} ->
                                []
                              {:testcase, tC, [prop]} when (is_atom(tC) and
                                                              tC == :all)
                                                           ->
                                [{:repeat, {mod, tC}, prop}]
                              {_, _} = tC when tCs == :all ->
  ...
  defp find(mod, grNames, tCs, [{name, props, tests} | gs],
            known, defs, findAll)
      when (is_atom(name) and is_list(props) and
              is_list(tests)) do
    cyclic_test(mod, name, known)
    ...
    trim(make_conf(mod, name, props,
                     find(mod, grNames, tCs, tests1, [name | known], defs,
                            findAll))) ++ find(mod, grNames, tCs, gs, known,
                                                 defs, findAll)
  end

  defp find(mod, grNames, tCs, [{:group, name1} | gs],
            known, defs, findAll)
      when is_atom(name1) do
    find(mod, grNames, tCs, [expand(mod, name1, defs) | gs],
           known, defs, findAll)
  end

  defp find(mod, grNames, tCs,
            [{:group, extMod, extGrp} | gs], known, defs, findAll)
      when (is_atom(extMod) and is_atom(extGrp)) do
    externalDefs = extMod.groups()
    externalTCs = find(extMod, extGrp, tCs,
                         [{:group, extGrp}], [], externalDefs, findAll)
    externalTCs ++ find(mod, grNames, tCs, gs, known, defs,
                          findAll)
  end

  defp find(mod, grNames, tCs, [{name1, tests} | gs], known,
            defs, findAll)
      when (is_atom(name1) and is_list(tests)) do
    find(mod, grNames, tCs, [{name1, [], tests} | gs],
           known, defs, findAll)
  end

  defp find(mod, grNames, tCs,
            [{externalTC, case__} = tC | gs], known, defs, findAll)
      when (is_atom(externalTC) and is_atom(case__)) do
    [tC | find(mod, grNames, tCs, gs, known, defs, findAll)]
  end

  defp find(mod, grNames, :all, [tC | gs], known, defs,
            findAll)
      when is_atom(tC) do
    [{mod, tC} | find(mod, grNames, :all, gs, known, defs,
                        findAll)]
  end

  defp find(mod, grNames, :all, [{m, tC} | gs], known, defs,
            findAll)
      when (is_atom(m) and m != :group and is_atom(tC)) do
    [{m, tC} | find(mod, grNames, :all, gs, known, defs,
                      findAll)]
  end

  defp find(mod, grNames, :all,
            [{:testcase, tC, [prop]} | gs], known, defs, findAll)
      when is_atom(tC) do
    [{:repeat, {mod, tC}, prop} | find(mod, grNames, :all,
                                         gs, known, defs, findAll)]
  end

  defp find(mod, grNames, tCs, [tC | gs], known, defs,
            findAll)
      when is_atom(tC) or tuple_size(tC) == 3 and :erlang.element(1,
                                                                    tC) == :testcase or tuple_size(tC) == 2 and :erlang.element(1,
                                                                                                                                  tC) != :group do
    case__ = (case (tC) do
                _ when is_atom(tC) ->
                  tuple = {mod, tC}
    ...
    cond do
      ...
      true ->
        [case__ | find(mod, grNames, tCs, gs, known, defs,
                         findAll)]
    end
  end

  defp find(mod, _GrNames, _TCs, [badTerm | _Gs], known,
            _Defs, _FindAll) do
    where = (cond do
               length(known) == 0 ->
                 :erlang.atom_to_list(mod) ++ ':groups/0'
               true ->
                 'group ' ++ :erlang.atom_to_list(:lists.last(known)) ++ ' in ' ++ :erlang.atom_to_list(mod) ++ ':groups/0'
             end)
  ...
  defp modify_tc_list(grSpecTs, _TSCs, _) do
    for test <- grSpecTs, not is_atom(test),
          :erlang.element(1, test) !== :testcase do
      test
    end
  end

  defp modify_tc_list1(grSpecTs, tSCs) do
    grSpecTs1 = :lists.flatmap(fn test = {:testcase, tC,
                                            _} ->
                                 ...
                                 grSpecTs)
    {tSCs2, grSpecTs3} = :lists.foldr(fn tC,
                                           {tSCs1, grSpecTs2} ->
                                           case (:lists.member(tC,
                                                                 grSpecTs1)) do
                                             true ->
                                               {[tC | tSCs1],
                                                  :lists.delete(tC, grSpecTs2)}
                                             false ->
                                               case (:lists.keysearch(tC, 2,
                                                                        grSpecTs)) do
                                                 {:value, test} ->
                                                   {[test | tSCs1],
                                                      :lists.keydelete(tC, 2,
                                                                         grSpecTs2)}
                                                 false ->
                                                   {tSCs1, grSpecTs2}
                                               end
  ...
  defp is_sub({:conf, props, _, _, _} = conf,
            [{:conf, _, _, tests, _} | confs]) do
    name = :proplists.get_value(:name, props)
    case (:lists.any(fn {:conf, props0, _, _, _} ->
  ...
  defp cyclic_test(mod, name, names) do
    case (:lists.member(name, names)) do
      true ->
        e = 'Cyclic reference to group ' ++ :erlang.atom_to_list(name) ++ ' in ' ++ :erlang.atom_to_list(mod) ++ ':groups/0'
        throw({:error, :erlang.list_to_atom(e)})
      false ->
        :ok
    end
  ...
  defp expand(mod, name, defs) do
    case (:lists.keysearch(name, 1, defs)) do
      {:value, def__} ->
        def__
      false ->
        e = 'Invalid group ' ++ :erlang.atom_to_list(name) ++ ' in ' ++ :erlang.atom_to_list(mod) ++ ':groups/0'
        throw({:error, :erlang.list_to_atom(e)})
    end
  end
  ...
  defp load_abs(dir, mod) do
    case (:code.is_loaded(mod)) do
      false ->
        :code.load_abs(:filename.join(dir,
  ...
  defp make_conf(mod, name, props, testSpec) do
    ...
    {initConf, endConf,
       extraProps} = (case ({:erlang.function_exported(mod,
                                                         :init_per_group, 2),
                               :erlang.function_exported(mod, :end_per_group,
                                                           2)}) do
                        {false, false} ->
                          :ct_logs.log('TEST INFO', 'init_per_group/2 and end_per_group/2 missing for group ~tw in ~w, using default.', [name, mod])
                          {{:ct_framework, :init_per_group},
                             {:ct_framework, :end_per_group}, [{:suite, mod}]}
                        _ ->
                          {{mod, :init_per_group}, {mod, :end_per_group}, []}
                      end)
    {:conf, [{:name, name} | props ++ extraProps], initConf,
       testSpec, endConf}
  end

  def expand_groups([h | t], confTests, mod) do
    [expand_groups(h, confTests, mod) | expand_groups(t,
                                                        confTests, mod)]
  end
  ...
  def expand_groups({:group, name}, confTests, mod) do
    expand_groups({:group, name, :default, []}, confTests,
                    mod)
  end
  ...
  def expand_groups({:group, name, oRProps}, confTests, mod)
    ...
    expand_groups({:group, name, oRProps, []}, confTests,
                    mod)
  end

  def expand_groups({:group, name, oRProps, subORSpec}, confTests,
           mod) do
    findConf = fn conf = {:conf, props, init, ts, end__} ->
                    case (:proplists.get_value(:name, props)) do
                      ^name when oRProps == :default ->
                        [conf]
                      ^name ->
                        props1 = (case (:proplists.get_value(:suite, props)) do
                                    :undefined ->
                                      oRProps
                                    suiteName ->
  ...
  def search_and_override([conf = {:conf, props, init, tests, end__}],
           oRSpec, mod) do
    insProps = fn grName, :undefined, ps ->
                    [{:name, grName} | ps]
                  grName, suite, ps ->
    ...
    case (:lists.keysearch(name, 1, oRSpec)) do
      {:value, {^name, :default}} ->
        [{:conf, props, init,
            search_and_override(tests, oRSpec, mod), end__}]
      {:value, {^name, oRProps}} ->
        [{:conf, insProps.(name, suite, oRProps), init,
            search_and_override(tests, oRSpec, mod), end__}]
      {:value, {^name, :default, []}} ->
        [{:conf, props, init,
            search_and_override(tests, oRSpec, mod), end__}]
  ...
  def search_and_override(tests, _, _) do
    tests
  end

  defp override_props([{:conf, props, init, tests, end__} | confs],
            subORSpec, name, mod) do
    {subs, subORSpec1} = override_sub_props(tests, [],
                                              subORSpec, mod)
  ...
  defp override_sub_props([t = {:conf, props, init, tests, end__} | ts],
            new, oRSpec, mod) do
    name = :proplists.get_value(:name, props)
    suite = :proplists.get_value(:suite, props)
    case (:lists.keysearch(name, 1, oRSpec)) do
      {:value, spec} ->
        props1 = (case (:erlang.element(2, spec)) do
                    :default ->
                      props
  ...
  defp invalid_ref_msg(name, mod) do
    e = 'Invalid reference to group ' ++ :erlang.atom_to_list(name) ++ ' in ' ++ :erlang.atom_to_list(mod) ++ ':all/0'
    :erlang.list_to_atom(e)
  end

  defp invalid_ref_msg(name0, name1, mod) do
    e = 'Invalid reference to group ' ++ :erlang.atom_to_list(name1) ++ ' from ' ++ :erlang.atom_to_list(name0) ++ ' in ' ++ :erlang.atom_to_list(mod) ++ ':all/0'
    :erlang.list_to_atom(e)
  end
