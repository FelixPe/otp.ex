defmodule :m_ct_master_logs do
  ...
  Record.defrecord(:r_state, :state, log_fd: :undefined,
                                 start_time: :undefined, logdir: :undefined,
                                 rundir: :undefined, nodedir_ix_fd: :undefined,
                                 nodes: :undefined, nodedirs: [])
  def start(logDir, nodes) do
    ...
    receive do
      ...
      {:DOWN, ^mRef, :process, _, reason} ->
        exit({:could_not_start_process, :ct_master_logs,
                reason})
    end
  end
  ...
  defp init(parent, logDir, nodes) do
      ...
      basicHtml ->
        ...
        case (copy_priv_files(privFilesSrc,
          ...
          {:error, src1, dest1, reason1} ->
            :io.format(:user, 'ERROR! ' ++ 'Priv file ~tp could not be copied to ~tp. ' ++ 'Reason: ~tp~n', [src1, dest1, reason1])
            exit({:priv_file_error, dest1})
          :ok ->
            case (copy_priv_files(privFilesSrc,
                                    privFilesDestRun)) do
              {:error, src2, dest2, reason2} ->
                :io.format(:user, 'ERROR! ' ++ 'Priv file ~tp could not be copied to ~tp. ' ++ 'Reason: ~tp~n', [src2, dest2, reason2])
    ...
    nodeStr = :lists.flatten(:lists.map(fn n ->
                                             :erlang.atom_to_list(n) ++ ' '
                                        end,
                                          nodes))
    :io.format(ctLogFd, int_header(),
                 [log_timestamp(:os.timestamp()), 'Test Nodes\n'])
    ...
    loop(r_state(log_fd: ctLogFd, start_time: time,
             logdir: logDir, rundir: runDirAbs,
             nodedir_ix_fd: nodeDirIxFd, nodes: nodes,
             nodedirs: :lists.map(fn n ->
                                       {n, ''}
                                  end,
                                    nodes)))
  end
  ...
  defp loop(state) do
    receive do
      ...
      :stop ->
        _ = make_all_runs_index(r_state(state, :logdir))
        :io.format(r_state(state, :log_fd),
  ...
  defp print_nodedir(node, runDir, fd) do
    ...
    :io.put_chars(fd,
                    ['<tr>\n<td align=center>', :erlang.atom_to_list(node), '</td>\n', '<td align=left><a href="',
                                                           :ct_logs.uri(index),
                                                               '">', index, '</a></td>\n', '</tr>\n'])
    :ok
  end
  ...
  defp nodedir_index_header(startTime) do
    [header('Log Files ' ++ format_time(startTime), {[], [1, 2], []}),
         '<center>\n', '<p><a href="', 'ct_master_log.html', '">Common Test Master Log</a></p>', xhtml(['<table border="3" cellpadding="5" bgcolor="', 'lightblue', '">\n'], ['<table id="', 'SortableTable', '">\n', '<thead>\n<tr>\n']), '<th><b>Node</b></th>\n', '<th><b>Log</b></th>\n',
                                                            xhtml('', '</tr>\n</thead>\n<tbody>\n')]
  end
  ...
  defp sort_all_runs(dirs) do
    keyList = :lists.map(fn dir ->
                              case (:lists.reverse(:string.lexemes(dir,
                                                                     [?.,
                                                                          ?_]))) do
                                [sS, mM, hH, date | _] ->
                                  {{date, hH, mM, sS}, dir}
                           ...
                           dirs)
    :lists.reverse(:lists.map(fn {_, dir} ->
                                   dir
                              end,
  ...
  defp runentry(dir) do
    {masterStr,
       nodesStr} = (case (read_details_file(dir)) do
                      {master, nodes} when is_list(nodes) ->
                        [_,
                             host] = :string.lexemes(:erlang.atom_to_list(master),
                                                       '@')
  ...
  defp all_runs_header() do
    [header('Master Test Runs', {[1], [2, 3], []}), '<center>\n', xhtml(['<table border="3" cellpadding="5" bgcolor="', 'lightblue', '">\n'],
                                              ['<table id="', 'SortableTable', '">\n', '<thead>\n<tr>\n']),
  ...
  defp timestamp(dir) do
    [s, min, h, d, m, y |
                          _] = :lists.reverse(:string.lexemes(dir, '.-_'))
    [s1, min1, h1, d1, m1, y1] = (for n <- [s, min, h, d, m,
                                                              y] do
                                                                                ...
                                                                                xhtml('',
                                                                                    ...
                                                                                    xhtml('',
                                                                                        ...
                                                                                        xhtml(fn () ->
                                                                                                ...
                                                                                                fn () ->
                                                                                                     :ct_logs.insert_javascript({:tablesorter,
                                                                                                                                   'SortableTable',
                                                                                                                                   tableCols})
                                                                                                end),
                                                                                            '</head>\n',
  ...
  defp basic_html() do
    case (:application.get_env(:common_test_master,
                                 :basic_html)) do
      {:ok, true} ->
        true
      _ ->
