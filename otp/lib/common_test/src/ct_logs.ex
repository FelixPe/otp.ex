defmodule :m_ct_logs do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_log_cache, :log_cache, version: :undefined,
                                     all_runs: [], tests: [])
  ...
  defp date2str({{yY, mM, dD}, {h, m, s}}) do
    :lists.flatten(:io_lib.format('~w-~2.2.0w-~2.2.0w_~2.2.0w.~2.2.0w.~2.2.0w', [yY, mM, dD, h, m, s]))
  end
  ...
  defp datestr_from_dirname([y1, y2, y3, y4, ?-, mo1, mo2, ?-, d1, d2, ?_,
                                                         h1, h2, ?., m1, m2, ?.,
                                                                                 s1,
                                                                                     s2 |
                                                                                         _]) do
    [y1, y2, y3, y4, ?-, mo1, mo2, ?-, d1, d2, ?_, h1, h2,
                                                           ?., m1, m2, ?., s1,
                                                                               s2]
  end
  ...
  def close(info, startDir, customStylesheet) do
    logCacheBin = (case (make_last_run_index()) do
                     {:error, reason} ->
    ...
    :ct_event.notify(r_event(name: :stop_logging, node: node(),
                         data: []))
        ...
        case (:ct_util.get_profile_data(:browser, startDir)) do
          ...
          browserData ->
            case ({:proplists.get_value(:prog, browserData),
              ...
              {prog, args, page} when (is_list(args) and
                                         is_list(page))
                                      ->
  ...
  defp cast(msg) do
    case (:erlang.whereis(:ct_logs)) do
      :undefined ->
        ...
        _ = (for {format, args} <- formatArgs do
  ...
  def init_tc(refreshLog) do
    call({:init_tc, self(), :erlang.group_leader(),
            refreshLog})
  ...
  def log(heading, format, args) do
    cast({:log, :sync, self(), :erlang.group_leader(),
            :ct_internal, 99,
            [{:hd, int_header(),
                [log_timestamp(:os.timestamp()), heading]},
                 {format, args}, {:ft, int_footer(), []}],
            true})
    :ok
  end
  ...
  def cont_log(format, args) do
    maybe_log_timestamp()
    cast({:log, :sync, self(), :erlang.group_leader(),
            :ct_internal, 99, [{format, args}], true})
    :ok
  end
  ...
  def end_log() do
    cast({:log, :sync, self(), :erlang.group_leader(),
            :ct_internal, 99, [{:ft, int_footer(), []}], false})
    :ok
  end
  ...
  def tc_log(category, importance, heading, format, args,
           opts) do
    data = (case (:lists.member(:no_css, opts)) do
              true ->
                [{format, args}]
              false ->
                heading1 = (case (:proplists.get_value(:heading,
                                                         opts)) do
                              :undefined ->
                                heading
                              str ->
                ...
                [{:hd, div_header(category, heading1), []}, {format,
                                                               args},
                                                                {:ft,
                                                                   div_footer(),
                                                                   []}]
  ...
  def tc_log_async(category, importance, heading, format, args) do
    cast({:log, :async, self(), :erlang.group_leader(),
            category, importance,
            [{:hd, div_header(category, heading), []}, {format,
                                                          args},
                                                           {:ft, div_footer(),
                                                              []}],
            true})
  ...
  def tc_print(category, importance, format, args, opts) do
    vLvl = (case (:ct_util.get_verbosity(category)) do
              :undefined ->
    ...
    cond do
      importance >= 100 - vLvl ->
        heading = (case (:proplists.get_value(:heading,
                                                opts)) do
                     :undefined ->
  ...
  def ct_log(category, format, args) do
    cast({:ct_log,
            [{:hd, div_header(category), []}, {format, args}, {:ft,
                                                                 div_footer(),
                                                                 []}],
            true})
    :ok
  ...
  defp div_header(class, heading) do
    '\n</pre>\n<div class="' ++ :erlang.atom_to_list(class) ++ '"><pre><b>*** ' ++ heading ++ ' ' ++ log_timestamp(:os.timestamp()) ++ ' ***</b>'
  end
  ...
  defp log_timestamp({mS, s, uS}) do
    ...
    {{year, month, day},
       {hour, min, sec}} = :calendar.now_to_local_time({mS, s,
                                                          uS})
  ...
  Record.defrecord(:r_logger_state, :logger_state, parent: :undefined,
                                        log_dir: :undefined,
  ...
  defp logger(parent, mode, verbosity, customStylesheet) do
    ...
    {time, dir} = (case (:filelib.is_dir(dir0)) do
                     true ->
                       :timer.sleep(1000)
    ...
    case (basic_html()) do
      ...
      basicHtml ->
        ...
        privFilesSrc = (for f <- privFiles do
                          :filename.join(:filename.join(cTPath, 'priv'), f)
                        end)
        privFilesDestTop = (for f <- privFiles do
                              :filename.join(cwd, f)
                            end)
        privFilesDestRun = (for f <- privFiles do
                              :filename.join(absDir, f)
                            end)
        case (copy_priv_files(privFilesSrc,
                                privFilesDestTop)) do
          {:error, src1, dest1, reason1} ->
            :io.format(:ct_default_gl, 'ERROR! ' ++ 'Priv file ~tp could not be copied to ~tp. ' ++ 'Reason: ~tp~n',
                         [src1, dest1, reason1])
    ...
    {miscIoHeader,
       miscIoFooter} = (case (get_ts_html_wrapper('Pre/post-test I/O log', dir,
                                                    false, dir, :undefined,
                                                    :utf8, customStylesheet)) do
                          {:basic_html, uH, uF} ->
                            {uH, uF}
                          {:xhtml, uH, uF} ->
                            {uH, uF}
    ...
    :io.put_chars(miscIoFd,
                    [miscIoHeader, '<a name="pretest"></a>\n', xhtml('<br>\n<h2>Pre-test Log</h2>', '<br />\n<h3>PRE-TEST LOG</h3>'), '\n<pre>\n'])
    miscIoDivider = '\n<a name="posttest"></a>\n' ++ xhtml('</pre>\n<br><h2>Post-test Log</h2>\n<pre>\n', '</pre>\n<br />\n<h3>POST-TEST LOG</h3>\n<pre>\n')
    :ct_util.set_testdata_async({:misc_io_log,
                                   {:filename.absname(miscIoName),
                                      miscIoDivider, miscIoFooter}})
    :ct_event.notify(r_event(name: :start_logging, node: node(),
                         data: absDir))
    ...
    ctLogFd = open_ctlog('misc_io.log.html', customStylesheet)
    :io.format(ctLogFd, int_header() ++ int_footer(),
                 [log_timestamp(:os.timestamp()), 'Common Test Logger started'])
    send(parent, {:started, self(),
                    {time, :filename.absname('')}})
    ...
    :io.nl(ctLogFd)
    tcEscChars = (case (:application.get_env(:common_test,
                                               :esc_chars)) do
                    {:ok, eCBool} ->
                      eCBool
                    _ ->
    ...
    logger_loop(r_logger_state(parent: parent, log_dir: absDir,
                    start_time: time, orig_GL: :erlang.group_leader(),
                    ct_log_fd: ctLogFd, tc_groupleaders: [],
                    async_print_jobs: [], tc_esc_chars: tcEscChars,
                    stylesheet: customStylesheet, log_index: 1))
  end
  ...
  defp logger_loop(state) do
    receive do
      {:log, syncOrAsync, pid, gL, category, importance,
         content, escChars} ->
        vLvl = (case (category) do
                  :ct_internal ->
                    100
                  _ ->
        ...
        cond do
          importance >= 100 - vLvl ->
            ...
            doEscChars = :erlang.and(r_logger_state(state, :tc_esc_chars),
                                       escChars)
            case (get_groupleader(pid, gL, state)) do
              {:tc_log, tCGL, tCGLs} ->
                case (:erlang.is_process_alive(tCGL)) do
                  true ->
                    state1 = print_to_log(syncOrAsync, pid, category, tCGL,
                                            content, doEscChars, state)
      ...
      {{:end_tc, tCPid}, from} ->
        ...
        return(from, :ok)
        logger_loop(r_logger_state(state, tc_groupleaders: rm_tc_gl(tCPid,
                                                         state)))
      ...
      {{:unregister_groupleader, pid}, from} ->
        return(from, :ok)
        logger_loop(r_logger_state(state, tc_groupleaders: rm_tc_gl(pid,
                                                         state)))
      ...
      {:ct_log, content, escChars} ->
        str = :lists.map(fn {_HdOrFt, str, args} ->
        ...
        logger_loop(state)
      {:DOWN, ref, _, _Pid, _} ->
        case (:lists.delete(ref,
                              r_logger_state(state, :async_print_jobs))) do
          [] ->
      ...
      :stop ->
        :io.format(r_logger_state(state, :ct_log_fd),
  ...
  defp create_io_fun(fromPid, ctLogFd, escChars, logIndex) do
    fn formatData, ioList ->
         {escapable, addAnchor, str,
            args} = (case (formatData) do
                       {:hd, s, a} ->
                         {false, true, s, a}
                       {_ft, s, a} ->
                         {false, false, s, a}
         ...
         try do
           ...
           ioStr when (escapable and escChars) ->
             [ioList, '\n', escape_chars(ioStr)]
           ioStr when ioList == [] ->
             ioStr ++ (for _ <- [:EFE_DUMMY_GEN], addAnchor do
                         anchor_link(logIndex)
                       end)
           ioStr ->
             [ioList, '\n', ioStr] ++ (for _ <- [:EFE_DUMMY_GEN],
                                          addAnchor do
                                      anchor_link(logIndex)
                                    end)
         end
  ...
  defp anchor_link(logIndex) do
    idLink = ['e-', :erlang.integer_to_list(logIndex)]
    ['<a id=', idLink, ' class="link-to-entry" ', 'href="#', idLink, '">&#x1f517;</a>']
  end
  ...
  defp print_to_log(:sync, fromPid, category, tCGL, content,
            escChars, r_logger_state(log_index: logIndex) = state) do
    ctLogFd = r_logger_state(state, :ct_log_fd)
    cond do
      fromPid != tCGL ->
      ...
      true ->
        unexpected_io(fromPid, category, 99, content, ctLogFd,
                        escChars)
    end
    r_logger_state(state, log_index: logIndex + 1)
  end

  defp print_to_log(:async, fromPid, category, tCGL, content,
            escChars, r_logger_state(log_index: logIndex) = state) do
    ctLogFd = r_logger_state(state, :ct_log_fd)
    printer = (cond do
                 fromPid != tCGL ->
                   ...
                   fn () ->
                        ...
                        case (:erlang.is_process_alive(tCGL)) do
                          true ->
                            try do
                              tc_io_format(tCGL, '~ts',
                                             [:lists.foldl(ioFun, [], content)])
                            catch
                              _, :terminated ->
                                unexpected_io(fromPid, category, 99, content,
                                                ctLogFd, escChars)
                              _, _ ->
                                :io.format(tCGL, '~ts',
                                             [:lists.foldl(ioFun, [], content)])
      ...
      {:tc, gL} ->
        ...
        case (:lists.keysearch({:tc, gL}, 2, tCGLs1)) do
          {:value, _} ->
            tCGLs1
          false ->
            :lists.filter(fn {_, {:io, gLPid}} when gL == gLPid ->
                               false
                             _ ->
                               true
                          end,
  ...
  defp open_ctlog(miscIoName, customStylesheet) do
    ...
    :io.format(fd, '~ts',
                 [header('Common Test Framework Log', '', {[], [1, 2], []}, customStylesheet)])
    case (:file.consult(:ct_run.variables_file_name('../'))) do
      ...
      {:error, reason} ->
        ...
        :io.format(fd, 'Can not read the file \'~ts\' Reason: ~tw\nNo configuration found for test!!\n', [variables, reason])
    end
    :io.format(fd, xhtml('<br><br><h2>Pre/post-test I/O Log</h2>\n', '<br /><br />\n<h4>PRE/POST TEST I/O LOG</h4>\n'), [])
    ...
    case (:file.read_file(path)) do
      {:ok, bin} ->
        ...
        case (:re.run(str, '<style>.*</style>',
                        [:dotall, :caseless, {:capture, :all, :list}])) do
          :nomatch ->
            case (:re.run(str, '</?style>',
                            [:caseless, {:capture, :all, :list}])) do
              :nomatch ->
  ...
  defp make_last_run_index(startTime, customStylesheet) do
    ...
    result = (case ((try do
                ...
                :ok ->
                  :ok
                err ->
                  :io.format('Unknown internal error while updating ~ts. Please report.\n(Err: ~p, ID: 1)', [absIndexName, err])
                  {:error, err}
              end)
    result
  end
  ...
  defp make_last_run_index1(startTime, indexName, customStylesheet) do
    logs1 = (case (:filelib.wildcard([?* | '.logs'])) do
               [log] ->
                 [log]
               logs ->
    ...
    missing = (case (:file.read_file('missing_suites.info')) do
                 {:ok, bin} ->
                   :erlang.binary_to_term(bin)
                 _ ->
                   []
               end)
    label = (case (:application.get_env(:common_test,
                                          :test_label)) do
               {:ok, lbl} ->
                 lbl
               _ ->
    ...
    case (force_write_file(indexName,
                             :unicode.characters_to_binary(index))) do
      :ok ->
        :ok
      {:error, reason} ->
  ...
  defp make_last_run_index([name | rest], result, totSucc, totFail,
            userSkip, autoSkip, totNotBuilt, missing) do
    case (get_run_dirs(name)) do
      ...
      logDirs ->
        ...
        {result1, totSucc1, totFail1, userSkip1, autoSkip1,
           totNotBuilt1} = make_last_run_index1(suiteName, logDirs,
                                                  result, totSucc, totFail,
                                                  userSkip, autoSkip,
                                                  totNotBuilt, missing)
        make_last_run_index(rest, result1, totSucc1, totFail1,
                              userSkip1, autoSkip1, totNotBuilt1, missing)
    end
  end

  defp make_last_run_index([], result, totSucc, totFail, userSkip,
            autoSkip, totNotBuilt, _) do
    {:ok,
       [result | total_row(totSucc, totFail, userSkip,
                             autoSkip, totNotBuilt, false)],
       {totSucc, totFail, userSkip, autoSkip, totNotBuilt}}
  end

  defp make_last_run_index1(suiteName, [logDir | logDirs], result, totSucc,
            totFail, userSkip, autoSkip, totNotBuilt, missing) do
    case (make_one_index_entry(suiteName, logDir, '-', false,
                                 missing, :undefined)) do
      {result1, succ, fail, uSkip, aSkip, notBuilt, _URIs1} ->
        autoSkip1 = (case ((try do
                             autoSkip + aSkip
                           catch
                             :error, e -> {:EXIT, {e, __STACKTRACE__}}
                             :exit, e -> {:EXIT, e}
        ...
        make_last_run_index1(suiteName, logDirs,
                               [result | result1], totSucc + succ,
  ...
  defp make_last_run_index1(_, [], result, totSucc, totFail, userSkip,
            autoSkip, totNotBuilt, _) do
  ...
  defp make_one_index_entry(suiteName, logDir, label, all, missing, uRIs) do
    case (count_cases(logDir)) do
      {succ, fail, userSkip, autoSkip} ->
        ...
        {newResult, uRIs1} = make_one_index_entry1(suiteName,
                                                     logDir, label, succ, fail,
                                                     userSkip, autoSkip,
                                                     notBuilt, all, :normal,
                                                     uRIs)
  ...
  defp make_one_index_entry1(suiteName, link, label, success, fail, userSkip,
            autoSkip, notBuilt, all, mode, uRIs) do
    ...
    uRIs1 = ({ctRunLogURI, logFileURI,
                crashDumpURI} = (case (uRIs) do
                                   :undefined ->
                                     {uri(:filename.join(ctRunDir, 'ctlog.html')),
                                        uri(logFile), uri(crashDumpName)}
                                   _ ->
                                     uRIs
                                 end))
    crashDumpLink = (case (mode) do
                       :temp ->
                         ''
                       :normal ->
    ...
    {lbl, timestamp, node, allInfo} = (case (all) do
                                         {true, oldRuns} ->
                                           [_Prefix, nodeOrDate |
                                                         _] = :string.lexemes(link,
                                                                                '.')
                                           node1 = (case (:string.find(nodeOrDate,
                                                                         [?@])) do
                                                      :nomatch ->
                                                        '-'
                                                      _ ->
                                           ...
                                           n = xhtml(['<td align=right><font size="-1">', node1, '</font></td>\n'],
                                           ...
                                           oldRunsLink = (case (oldRuns) do
                                                            [] ->
                                                              'none'
                                                            _ ->
                                                              '<a href="' ++ 'all_runs.html' ++ '">Old Runs</a>'
                                                          end)
                                           a = xhtml(['<td><font size="-1"><a href="', ctRunLogURI, '">CT Log</a></font></td>\n', '<td><font size="-1">',
                                                                             oldRunsLink,
                                                                                 '</font></td>\n'],
    ...
    notBuiltStr = (cond do
                     ...
                     true ->
                       ['<td align=right><a href="', ctRunLogURI, '">', :erlang.integer_to_list(notBuilt),
                                               '</a></td>\n']
                   end)
    failStr = (cond do
    ...
    {allSkip, userSkipStr, autoSkipStr} = (cond do
                                             autoSkip == :undefined ->
                                               {userSkip, '?', '?'}
                                             true ->
                                               aSStr = (cond do
                                               ...
                                               {userSkip + autoSkip,
                                                  :erlang.integer_to_list(userSkip),
                                                  aSStr}
                                           end)
    {[xhtml('<tr valign=top>\n', ['<tr class="', odd_or_even(), '">\n']), xhtml('<td><font size="-1"><a href="', '<td><a href="'),
                                           logFileURI, '">', suiteName, '</a>',
                                                                         crashDumpLink,
                                                                             xhtml('</font></td>\n',
                                                                                     '</td>\n'),
                                                                                 lbl,
  ...
  defp total_row(success, fail, userSkip, autoSkip, notBuilt,
            all) do
    {label, timestampCell, allInfo} = (case (all) do
                                         true ->
                                           {'<td>&nbsp;</td>\n', '<td>&nbsp;</td>\n', '<td>&nbsp;</td>\n<td>&nbsp;</td>\n<td>&nbsp;</td>\n'}
                                         false ->
                                           {'', '', ''}
    ...
    [xhtml('<tr valign=top>\n', ['</tbody>\n<tfoot>\n<tr class="', odd_or_even(), '">\n']), '<td><b>Total</b></td>\n', label,
                                             timestampCell, '<td align=right><b>',
                                                                :erlang.integer_to_list(success),
                                                                    '</b></td>\n', '<td align=right><b>',
                                                                           :erlang.integer_to_list(fail),
                                                                               '</b></td>\n',
  ...
  defp not_built(baseName, _LogDir, _All, missing) do
    failed = (case (:string.lexemes(baseName, '.')) do
                [t, o] when is_list(t) ->
  ...
  defp locate_info(path = {top, obj}, allOrSuite,
            [{{dir, suite}, failed} | errors]) do
    case (:lists.reverse(:filename.split(dir))) do
      ['test', ^obj, ^top | _] ->
        get_missing_suites(allOrSuite,
                             {suite, failed}) ++ locate_info(path, allOrSuite,
                                                               errors)
      [^obj, ^top | _] ->
        get_missing_suites(allOrSuite,
                             {suite, failed}) ++ locate_info(path, allOrSuite,
  ...
  defp index_header(label, startTime, customStylesheet) do
    head = (case (label) do
              ...
              _ ->
                header('Test Results for \'' ++ label ++ '\'', format_time(startTime),
                         {[], [1], [2, 3, 4, 5]}, customStylesheet)
            end)
    cover = (case (:filelib.is_regular(:filename.absname('cover.html'))) do
               true ->
                 xhtml(['<p><a href="', 'cover.html', '">Cover Log</a></p><br>\n'], ['<br /><div id="button_holder" class="btn">\n<a href="', 'cover.html', '">COVER LOG</a>\n</div><br /><br />'])
               false ->
                 xhtml('<br>\n', '<br /><br /><br />\n')
             end)
    [head, '<center>\n', xhtml(['<p><a href="', 'ctlog.html', '">Common Test Framework Log</a></p>'], ['<br /><div id="button_holder" class="btn">\n<a href="', 'ctlog.html', '">COMMON TEST FRAMEWORK LOG</a>\n</div><br>\n']), cover, xhtml(['<table border="3" cellpadding="5" bgcolor="',
                                                             '#F0F8FF', '">\n'],
                                                          ['<table id="', 'SortableTable', '">\n', '<thead>\n<tr>\n']),
                                                      '<th><b>Test Name</b></th>\n', xhtml(['<th><font color="', '#F0F8FF', '">_</font>Ok<font color="', '#F0F8FF', '">_</font></th>\n'],
                                                                 '<th>Ok</th>\n'),
                                                             '<th>Failed</th>\n', '<th>Skipped', xhtml('<br>', '<br />'),
                                                                       '(User/Auto)</th>\n<th>Missing',
                                                                           xhtml('<br>',
                                                                                   '<br />'),
                                                                               'Suites</th>\n',
  ...
  defp all_suites_index_header(indexDir, customStylesheet) do
    ...
    allRunsLink = xhtml(['<a href="', 'all_runs.html', '">', allRuns, '</a>\n'],
    ...
    [header('Test Results', '', {[3], [1, 2, 8, 9, 10], [4, 5, 6, 7]},
         ...
         '<center>\n', allRunsLink, xhtml('<br><br>\n', '<br /><br />\n'), xhtml(['<table border="3" cellpadding="5" bgcolor="', '#E4F0FE', '">\n'],
                                              ['<table id="', 'SortableTable', '">\n', '<thead>\n<tr>\n']),
                                          '<th>Test Name</th>\n', '<th>Label</th>\n', '<th>Test Run Started</th>\n', xhtml(['<th><font color="', '#E4F0FE', '">_</font>Ok<font color="', '#E4F0FE', '">_</font></th>\n'], '<th>Ok</th>\n'), '<th>Failed</th>\n',
  ...
  defp all_runs_header(customStylesheet) do
    ...
    ixLink = [xhtml(['<p><a href="', 'index.html', '">Test Index Page</a></p>'], ['<div id="button_holder" class="btn">\n<a href="', 'index.html', '">TEST INDEX PAGE</a>\n</div>']), xhtml('<br>\n', '<br /><br />\n')]
    [header(title, '', {[1], [2, 3, 5], [4, 6, 7, 8, 9, 10]},
              customStylesheet),
         '<center>\n', ixLink, xhtml(['<table border="3" cellpadding="5" bgcolor="', '#ADD8E6', '">\n'], ['<table id="', 'SortableTable', '">\n', '<thead>\n<tr>\n']), '<th><b>History</b></th>\n<th><b>Node</b></th>\n<th><b>Label</b></th>\n<th>Tests</th>\n<th><b>Test Names</b></th>\n<th>Total</th>\n', xhtml(['<th><font color="',
                                                                  '#ADD8E6', '">_</font>Ok<font color="', '#ADD8E6', '">_</font></th>\n'],
                                                               '<th>Ok</th>\n'),
                                                           '<th>Failed</th>\n<th>Skipped<br>(User/Auto)</th>\n<th>Missing<br>Suites</th>\n', xhtml('', '</tr>\n</thead>\n<tbody>\n')]
  end

  defp header(title, subTitle, tableCols, customStylesheet) do
    subTitleHTML = (cond do
                      subTitle !== '' ->
                        ['<center>\n', '<h3>' ++ subTitle ++ '</h3>\n', xhtml('</center>\n<br>\n', '</center>\n<br />\n')]
                      true ->
                        xhtml('<br>', '<br />')
  ...
  defp footer() do
    ['<center>\n', xhtml('<hr>\n', ''), xhtml('<p><font size="-1">\n', '<div class="copyright">'), 'Copyright &copy; ', year(), ' <a href="http://www.erlang.org">Open Telecom Platform</a>', xhtml('<br>\n', '<br />\n'),
  ...
  defp body_tag() do
    ...
    xhtml('<body background="' ++ tileFile ++ '" bgcolor="#FFFFFF" text="#000000" link="#0000FF" vlink="#800080" alink="#FF0000">\n', '<body>\n')
  end
  ...
  defp read_summary(name, keys) do
    case (:file.consult(name)) do
      ...
      {:ok, terms} ->
        {:ok,
           :lists.map(fn key ->
                           {:value, {_, value}} = :lists.keysearch(key, 1,
                                                                     terms)
                           value
                      end,
                        keys)}
  ...
  defp count_cases1('=failed' ++ rest,
            {success, _Fail, userSkip, autoSkip}) do
    {nextLine, count} = get_number(rest)
    count_cases1(nextLine,
                   {success, count, userSkip, autoSkip})
  end

  defp count_cases1('=successful' ++ rest,
            {_Success, fail, userSkip, autoSkip}) do
    {nextLine, count} = get_number(rest)
    count_cases1(nextLine,
                   {count, fail, userSkip, autoSkip})
  end

  defp count_cases1('=skipped' ++ rest,
            {success, fail, _UserSkip, _AutoSkip}) do
    {nextLine, count} = get_number(rest)
    count_cases1(nextLine,
                   {success, fail, count, :undefined})
  end

  defp count_cases1('=user_skipped' ++ rest,
            {success, fail, _UserSkip, autoSkip}) do
    {nextLine, count} = get_number(rest)
    count_cases1(nextLine, {success, fail, count, autoSkip})
  end

  defp count_cases1('=auto_skipped' ++ rest,
            {success, fail, userSkip, _AutoSkip}) do
    {nextLine, count} = get_number(rest)
    count_cases1(nextLine, {success, fail, userSkip, count})
  end

  defp count_cases1([], {su, f, uSk, _ASk}) when su == :undefined or
  ...
  defp get_number([digit | rest]) when (?0 <= digit and
                                  digit <= ?9) do
    get_number(rest, digit - ?0)
  end

  defp get_number([digit | rest], acc) when (?0 <= digit and
                                       digit <= ?9) do
  ...
  defp config_table_header() do
    [xhtml(['<h2>Configuration</h2>\n<table border="3" cellpadding="5" bgcolor="', '#ADD8E6', '">\n'], ['<h4>CONFIGURATION</h4>\n', '<table id="', 'SortableTable', '">\n', '<thead>\n']), '<tr><th>Key</th><th>Value</th></tr>\n', xhtml('', '</thead>\n<tbody>\n')]
  end

  defp config_table1([{key, value} | vars]) do
    [xhtml(['<tr><td>', :erlang.atom_to_list(key), '</td>\n', '<td><pre>',
                                                 :io_lib.format('~tp', [value]), '</pre></td></tr>\n'],
             ['<tr class="', odd_or_even(), '">\n', '<td>', :erlang.atom_to_list(key), '</td>\n',
                                                                     '<td>',
                                                                         :io_lib.format('~tp',
                                                                                          [value]),
                                                                             '</td>\n</tr>\n']) |
  ...
  def make_all_runs_index(when__, customStylesheet) do
    ...
    useCache = (cond do
                  when__ == :refresh ->
                    :save_only
                  true ->
                    case (:application.get_env(:common_test,
                                                 :disable_log_cache)) do
                      {:ok, true} ->
                        :disabled
                      _ ->
                        case (:erlang.get(:ct_log_cache)) do
                ...
                _WhyNot ->
                  ...
                  getLogResult = fn dir, {runData, logTxt} ->
                                      {tot, xHTML, ixLink} = runentry(dir,
                                                                        :undefined,
                                                                        :undefined)
                                      {[{dir, tot, ixLink} | runData],
                                         [xHTML | logTxt]}
                                 end
                  {allRunsData, index} = :lists.foldr(getLogResult,
                                                        {[], []}, dirsSorted)
                  cond do
                    useCache == :disabled ->
                      :ok
  ...
  defp make_all_runs_from_cache(absName, dirs, logCache, customStylesheet) do
    ...
    getLogResult = fn {dir, :no_test_data, ixLink},
                        {runData, logTxt} ->
                      ...
                      {dir, cachedTotals, ixLink}, {runData, logTxt} ->
                        ...
                        {[{dir, tot, ixLink} | runData], [xHTML | logTxt]}
                      dir, {runData, logTxt} ->
                        {tot, xHTML, ixLink} = runentry(dir, :undefined,
                                                          :undefined)
                        {[{dir, tot, ixLink} | runData], [xHTML | logTxt]}
                   end
    {allRunsData, index} = :lists.foldr(getLogResult,
  ...
  defp update_all_runs_in_cache(allRunsData) do
    case (:erlang.get(:ct_log_cache)) do
      :undefined ->
        logCache = r_log_cache(version: cache_vsn(),
                       all_runs: allRunsData)
        case ({self(), :erlang.whereis(:ct_logs)}) do
          {pid, pid} ->
            :erlang.put(:ct_log_cache,
  ...
  defp sort_all_runs(dirs) do
    :lists.sort(fn dir1, dir2 ->
                     [sS1, mM1, hH1, date1 |
                                         _] = :lists.reverse(:string.lexemes(dir1,
                                                                               [?.,
                                                                                    ?_]))
                     [sS2, mM2, hH2, date2 |
                                         _] = :lists.reverse(:string.lexemes(dir2,
                                                                               [?.,
                                                                                    ?_]))
  ...
  defp sort_ct_runs(dirs) do
    :lists.sort(fn dir1, dir2 ->
                     [sS1, mM1, dateHH1 |
                                    _] = :lists.reverse(:string.lexemes(:filename.dirname(dir1),
                                                                          [?.]))
                     [sS2, mM2, dateHH2 |
                                    _] = :lists.reverse(:string.lexemes(:filename.dirname(dir2),
                                                                          [?.]))
  ...
  defp maybe_delete_old_dirs(sorted) do
    {keep,
       delete} = (case (:application.get_env(:common_test,
                                               :keep_logs)) do
                    {:ok, maxN} when (is_integer(maxN) and
                                        length(sorted) > maxN)
                                     ->
  ...
  defp dir_diff_all_runs(dirs, logCache) do
    case (r_log_cache(logCache, :all_runs)) do
      [] ->
        dirs
      cached = [{cDir, _, _} | _] ->
        allRunsDirs = dir_diff_all_runs(dirs, cached,
                                          datestr_from_dirname(cDir), [])
        :lists.reverse(allRunsDirs)
    end
  end

  defp dir_diff_all_runs(logDirs = [dir | dirs],
            cached = [cElem | cElems], latestInCache,
            allRunsDirs) do
    ...
    cond do
      ...
      (dirDate == latestInCache and cElems != []) ->
        elemToAdd = (case (cElem) do
                       {_CDir, {_NodeStr, _Label, _Logs, {0, 0, 0, 0, 0}},
                          _IxLink} ->
                         dir
                       _ ->
                         cElem
        ...
        dir_diff_all_runs(dirs, cElems,
                            datestr_from_dirname(:erlang.element(1,
                                                                   hd(cElems))),
  ...
  defp interactive_link() do
    [dir |
         _] = :lists.reverse(:filelib.wildcard(logdir_prefix() ++ '*.*'))
                                                                                     ...
                                                                                     '">',
                                                                                         timestamp(dir),
                                                                                             '</a>',
                                                                                                 '</body>\n',
                                                                                                     '</html>\n']
    _ = :file.write_file('last_interactive.html',
  ...
  defp runentry(dir,
            totals = {node, label, logs,
                        {totSucc, totFail, userSkip, autoSkip, notBuilt}},
            index) do
    totFailStr = (cond do
    ...
    noOfTests = (case (length(logs)) do
                   0 ->
                     '-'
                   n ->
                     :erlang.integer_to_list(n)
                 end)
    rootNames = :lists.map(fn f ->
                                :filename.rootname(f, '.logs')
                           end,
                             logs)
    testNames = :lists.flatten(:lists.join(', ', rootNames))
    testNamesTrunc = (cond do
                        length(testNames) < 60 ->
                          testNames
                        true ->
                          trunc = :string.trim(:string.slice(testNames, 0,
                                                               60 - 3),
                                                 :trailing, ', ')
                          :lists.flatten(:io_lib.format('~ts...', [trunc]))
                      end)
    totMissingStr = (cond do
                       notBuilt > 0 ->
    ...
    c = ['<td align=right>', :erlang.integer_to_list(total), '</td>\n', '<td align=right>',
  ...
  defp write_totals_file(name, label, logs, totals) do
    ...
    _ = force_write_file(absName,
                           :erlang.term_to_binary({:erlang.atom_to_list(node()),
                                                     label, logs, totals}))
    notify_and_unlock_file(absName)
  end
  ...
  defp read_totals_file(name) do
    absName = :filename.absname(name)
    notify_and_lock_file(absName)
    result = (case (:file.read_file(absName)) do
                {:ok, bin} ->
                  case ((try do
                          :erlang.binary_to_term(bin)
                        catch
  ...
  defp timestamp(dir) do
    ...
    [s, min, h, d, m, y] = (for n <- :lists.sublist(tsR,
                                                      6) do
  ...
  def make_all_suites_index(when__, customStylesheet) when is_atom(when__) do
    ...
    result = (case (logCacheInfo) do
                {:ok, logCache} ->
                  make_all_suites_index_from_cache(when__, absIndexName,
                                                     logDirs, logCache,
                                                     customStylesheet)
                _WhyNot ->
                  sorted = sort_and_filter_logdirs(logDirs)
                  tempData = make_all_suites_index1(when__, absIndexName,
  ...
  def make_all_suites_index(newTestData = {_TestName, dirName},
           customStylesheet) do
    :erlang.put(:basic_html, basic_html())
    {absIndexName,
       logDirData} = :ct_util.get_testdata(:test_index)
    ctRunDirPos = length(:filename.split(absIndexName))
    ctRunDir = :filename.join(:lists.sublist(:filename.split(dirName),
                                               ctRunDirPos))
    label = (case (read_totals_file(:filename.join(ctRunDir,
                                                     'totals.info'))) do
               {_, '-', _, _} ->
                 '...'
               {_, lbl, _, _} ->
    ...
    notify_and_lock_file(absIndexName)
    result = (case ((try do
                      make_all_suites_ix_temp(absIndexName, newTestData,
                                                label, logDirData,
                                                customStylesheet)
                    catch
                      :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp make_all_suites_index_from_cache(when__, absIndexName, logDirs, logCache,
            customStylesheet) do
    ...
    tempData = (cond do
                  ...
                  true ->
                    ...
                    :ct_util.set_testdata_async({:test_index,
                                                   {absIndexName, data}})
                    data
                end)
    notify_and_unlock_file(absIndexName)
  ...
  defp sort_and_filter_logdirs([{testName, ixDirs} | tests], cachedTests,
            combined) do
  ...
  defp sort_and_filter_logdirs([], cachedTests, combined) do
    cached1 = :lists.foldl(fn {testName, _}, cached ->
                                :lists.keydelete(testName, 1, cached)
                           end,
                             cachedTests, combined)
  ...
  defp sort_and_filter_logdirs1([dir | dirs], groups) do
    ...
    case (:filelib.wildcard(:filename.join(dir, 'run.*'))) do
      runDirs = [_ | _] ->
        groups1 = sort_and_filter_logdirs2(testName, runDirs,
                                             groups)
  ...
  defp sort_and_filter_logdirs2(testName, [runDir | runDirs], groups) do
    groups1 = insert_test(testName,
                            {:filename.basename(runDir), runDir}, groups)
  ...
  defp sort_each_group([{test, ixDirs} | groups]) do
    sorted = :lists.reverse(for {_,
                                   dir} <- :lists.keysort(1, ixDirs) do
                              dir
                            end)
  ...
  defp dir_diff_tests(logDirs, r_log_cache(tests: cachedTests)) do
    allTestNames = (for {testName, _, _, _,
                           _} <- cachedTests do
                      testName
                    end)
  ...
  defp dir_diff_tests([logDir | logDirs], cachedTests, newAdded,
            deletedTests, validLast, invalidLast) do
    testName = :filename.rootname(:filename.basename(logDir))
    time = datestr_from_dirname(logDir)
    {new, deletedTests1, validLast1,
       invalidLast1} = (case (:lists.keysearch(testName, 1,
                                                 cachedTests)) do
                          {:value,
                             {_, _, _, {lastLogDir, _, _}, _PrevLogDirs}} ->
                          ...
                          _ ->
                            {[logDir | newAdded], deletedTests, validLast,
                               invalidLast}
                        end)
  ...
  defp dir_diff_tests([], _CachedTests, newAdded, deletedTests,
            validLast, invalidLast) do
    invalidLast1 = :lists.foldl(fn tDir, iL ->
  ...
  defp update_tests_in_cache(tempData, logCache = r_log_cache(tests: tests)) do
    cached1 = (cond do
                 tests == [] ->
                   []
                 true ->
                   :lists.foldl(fn {testName, _, _, _, _}, cached ->
                                     :lists.keydelete(testName, 1, cached)
                                end,
                                  tests, tempData)
               end)
  ...
  defp make_all_suites_index1(when__, absIndexName, allTestLogDirs,
            customStylesheet) do
    indexName = 'index.html'
    cond do
      when__ == :start ->
  ...
  defp make_all_suites_index2(indexName, allTestLogDirs, customStylesheet) do
    {:ok, index0, _Totals,
       tempData} = make_all_suites_index3(allTestLogDirs,
  ...
  defp make_all_suites_index3([ixEntry = {testName, label, missing,
                        {lastLogDir, summary, uRIs}, oldDirs} |
               rest],
            result, totSucc, totFail, userSkip, autoSkip,
            totNotBuilt, labels, tempData) do
    ...
    case (summary) do
      {succ, fail, uSkip, aSkip} ->
        ...
        {result1, _} = make_one_index_entry1(testName,
                                               lastLogDir, label, succ, fail,
                                               uSkip, aSkip, notBuilt, all,
                                               :temp, uRIs)
        autoSkip1 = (case ((try do
                             autoSkip + aSkip
                           catch
        ...
        make_all_suites_index3(rest, [result | result1],
                                 totSucc + succ, totFail + fail,
                                 userSkip + uSkip, autoSkip1,
  ...
  defp make_all_suites_index3([{testName, [lastLogDir | oldDirs]} | rest],
            result, totSucc, totFail, userSkip, autoSkip,
            totNotBuilt, labels, tempData) do
    [entryDir | _] = :filename.split(lastLogDir)
    missing = (case (:file.read_file(:filename.join(entryDir,
                                                      'missing_suites.info'))) do
                 {:ok, bin} ->
                   :erlang.binary_to_term(bin)
                 _ ->
                   []
               end)
    {label, labels1} = (case (:proplists.get_value(entryDir,
                                                     labels)) do
                          :undefined ->
                            case (read_totals_file(:filename.join(entryDir,
                                                                    'totals.info'))) do
                              {_, lbl, _, _} ->
    ...
    case (make_one_index_entry(testName, lastLogDir, label,
                                 {true, oldDirs}, missing, :undefined)) do
  ...
  defp make_all_suites_index3([_ | rest], result, totSucc, totFail, userSkip,
            autoSkip, totNotBuilt, labels, tempData) do
    make_all_suites_index3(rest, result, totSucc, totFail,
                             userSkip, autoSkip, totNotBuilt, labels, tempData)
  end

  defp make_all_suites_index3([], result, totSucc, totFail, userSkip,
            autoSkip, totNotBuilt, _, tempData) do
    {:ok,
       [result | total_row(totSucc, totFail, userSkip,
                             autoSkip, totNotBuilt, true)],
       {totSucc, totFail, userSkip, autoSkip, totNotBuilt},
       :lists.reverse(tempData)}
  end

  defp make_all_suites_ix_temp(absIndexName, newTestData, label,
            allTestLogDirs, customStylesheet) do
    allTestLogDirs1 = insert_new_test_data(newTestData,
                                             label, allTestLogDirs)
    ...
    index0 = make_all_suites_ix_temp1(allTestLogDirs1,
                                        ...
                                        0, 0, 0, 0, 0)
    index = [index0 | all_suites_index_footer()]
    case (force_write_file(absIndexName,
                             :unicode.characters_to_binary(index))) do
      :ok ->
        :ok
      {:error, reason} ->
  ...
  defp insert_new_test_data({newTestName, newTestDir}, newLabel,
            allTestLogDirs) do
    allTestLogDirs1 = (case (:lists.keysearch(newTestName,
                                                1, allTestLogDirs)) do
                         {:value, {_, _, _, {lastLogDir, _, _}, oldDirs}} ->
                           [{newTestName, newLabel, [],
                               {newTestDir, {0, 0, 0, 0}, :undefined},
                               [lastLogDir | oldDirs]} |
                                :lists.keydelete(newTestName, 1,
                                                   allTestLogDirs)]
                         false ->
                           [{newTestName, newLabel, [],
                               {newTestDir, {0, 0, 0, 0}, :undefined}, []} |
  ...
  defp make_all_suites_ix_temp1([{testName, label, missing, lastLogDirData,
              oldDirs} |
               rest],
            result, totSucc, totFail, userSkip, autoSkip,
            totNotBuilt) do
    case (make_one_ix_entry_temp(testName, lastLogDirData,
                                   label, {true, oldDirs}, missing)) do
      {result1, succ, fail, uSkip, aSkip, notBuilt, _URIs} ->
        ...
        make_all_suites_ix_temp1(rest, [result | result1],
                                   totSucc + succ, totFail + fail,
                                   userSkip + uSkip, autoSkip1,
  ...
  defp make_all_suites_ix_temp1([], result, totSucc, totFail, userSkip,
            autoSkip, totNotBuilt) do
    [result | total_row(totSucc, totFail, userSkip,
                          autoSkip, totNotBuilt, true)]
  end

  defp make_one_ix_entry_temp(testName, {logDir, summary, uRIs}, label, all,
            missing) do
    case (summary) do
      {succ, fail, userSkip, autoSkip} ->
        notBuilt = not_built(testName, logDir, all, missing)
        {newResult, uRIs1} = make_one_index_entry1(testName,
                                                     logDir, label, succ, fail,
                                                     userSkip, autoSkip,
    ...
    case (:application.get_key(:common_test, :vsn)) do
      ...
      _ ->
        ...
        case (:file.read_file(vSNfile)) do
          {:ok, bin} ->
            [_, vSN] = :string.lexemes(:erlang.binary_to_list(bin),
                                         [?=, ?\n, ?\s])
            vSN
          _ ->
            :undefined
  ...
  defp notify_and_lock_file(file) do
    case (:ct_event.is_alive()) do
      true ->
        :ct_event.sync_notify(r_event(name: :start_write_file,
                                  node: node(), data: file))
      false ->
        :ok
    end
  ...
  defp notify_and_unlock_file(file) do
    case (:ct_event.is_alive()) do
      true ->
        :ct_event.sync_notify(r_event(name: :finished_write_file,
                                  node: node(), data: file))
      false ->
        :ok
    end
  ...
  def xhtml(hTML, xHTML) when (is_function(hTML) and
  ...
  def locate_priv_file(fileName) do
    ...
    case (:filelib.is_file(privFileInCwd)) do
      true ->
        privFileInCwd
      false ->
        privResultFile = (case ({:erlang.whereis(:ct_logs),
                                   self()}) do
                            {self, self} ->
  ...
  def get_ts_html_wrapper(testName, printLabel, cwd, tableCols,
           encoding) do
    get_ts_html_wrapper(testName, :undefined, printLabel,
                          cwd, tableCols, encoding, :unknown)
  end

  defp get_ts_html_wrapper(testName, logdir, printLabel, cwd, tableCols,
            encoding, customStylesheet) do
    testName1 = (cond do
                   is_list(testName) ->
    ...
    labelStr = (cond do
                  ...
                  true ->
                    case ({basic,
                             :application.get_env(:common_test,
                                                    :test_label)}) do
    ...
    case (basic) do
      true ->
        ...
        copyright = ['<p><font size="-1">\n', 'Copyright &copy; ', year(), ' <a href="http://www.erlang.org">', 'Open Telecom Platform</a><br>\n', 'Updated: <!--date-->', current_time(), '<!--/date-->',
                                                                '<br>\n</font></p>\n']
        {:basic_html,
           ...
           ['<center>\n<br><hr><p>\n', '<a href="', uri(allRuns), '">Test run history\n</a>  |  ', '<a href="', uri(testIndex), '">Top level test index\n</a>  |  ', '<a href="',
                                                             uri(latestTest), '">Latest test result</a>\n</p>\n',
                                                                                  copyright,
                                                                                      '</center>\n</body>\n</html>\n']}
      _ ->
        copyright = ['<div class="copyright">', 'Copyright &copy; ', year(), ' <a href="http://www.erlang.org">', 'Open Telecom Platform</a><br />\n', 'Updated: <!--date-->', current_time(), '<!--/date-->',
                                                                '<br />\n</div>\n']
        cSSFile = xhtml(fn () ->
                             ''
                        end,
        ...
        tableSorterScript = xhtml(fn () ->
                                       ''
                                  end,
                                    fn () ->
                                         insert_javascript({:tablesorter, 'SortableTable',
                                                              tableCols})
                                    end)
        {:xhtml,
  ...
  def insert_javascript({:tablesorter, tableName,
            {dateCols, textCols, valCols}}) do
    headers = :lists.flatten(:lists.sort(:lists.flatmap(fn {sorter,
                                                              cols} ->
                                                             for col <- cols do
                                                               :lists.flatten(:io_lib.format('      ~w: { sorter: \'~s\' },\n',
                                                                                               [col - 1,
                                                                                                    sorter]))
                                                             end
                                                        end,
                                                          [{'CTDateSorter', dateCols}, {'CTTextSorter',
                                                                             textCols},
                                                                              {'CTValSorter',
                                                                                 valCols}])))
  ...
  defp unexpected_io(pid, :ct_internal, _Importance, content,
            ctLogFd, escChars) do
  ...
  defp unexpected_io(pid, _Category, _Importance, content, ctLogFd,
            escChars) do
    ioFun = create_io_fun(pid, ctLogFd, escChars)
    data = :io_lib.format('~ts',
                            [:lists.foldl(ioFun, [], content)])
