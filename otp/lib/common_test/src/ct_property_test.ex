defmodule :m_ct_property_test do
  ...
  def init_per_suite(config) do
    case init_tool(config) do
      ...
      config1 ->
        path = property_tests_path(~c"property_test", config1)

        case compile_tests(path, config1) do
          :error ->
            {:fail, ~c"Property test compilation failed in " ++ path}
  ...
  def init_tool(config) do
    ...
    case which_module_exists(toolsToCheck) do
      {:ok, toolModule} ->
        case :code.where_is_file(:lists.concat([toolModule, ~c".beam"])) do
          :non_existing ->
            :ct.log(~c"Found ~p, but ~tp~n is not found", [
              toolModule,
              :lists.concat([toolModule, ~c".beam"])
            ])

            {:skip, ~c"Strange Property testing tool installation"}

          toolPath ->
            :ct.log(~c"Found property tester ~p~nat ~tp", [toolModule, toolPath])
            [{:property_test_tool, toolModule} | config]
        end

      :not_found ->
        :ct.log(~c"No property tester found", [])
        {:skip, ~c"No property testing tool found"}
    end
  end
  ...
  def present_result(module, cmds, {h, sf, result}, config, options0) do
    defSpec =
      cond do
        is_tuple(cmds) ->
          [{~c"Distribution sequential/parallel", &sequential_parallel/1}]

        is_list(cmds) ->
          []
      end ++
        [
          {~c"Function calls", &cmnd_names/1},
          {~c"Length of command sequences", &print_frequency_ranges/0, &num_calls/1}
        ]
  ...
  def title(str, fun, printFun) do
    fn l ->
      printFun.(~c"~n~s~n~n~s~n", [str, fun.(l)])
    end
  end
  ...
  def print_frequency() do
    fn l ->
      for {v, _Num, pcnt} <-
        ...
        :io_lib.format(~c"~5.1f% ~p~n", [pcnt, v])
      end
    end
  end
  ...
  defp mk_ct_return(other, tool) do
    try do
      ...
      {:set, {:var, _}, {:call, m, f, args}} ->
        {:fail, :io_lib.format(~c"~p:~tp/~p returned bad result", [m, f, length(args)])}
    end
  end
  ...
  defp compile_tests(path, config) do
    ...
    case :file.set_cwd(path) do
      :ok ->
        case :file.list_dir(~c".") do
          {:ok, []} ->
            :ct.log(~c"No files found in ~tp", [path])
            :ok = :file.set_cwd(cwd)
            {:skip, ~c"No files found"}

          {:ok, fileNames} ->
            beamFiles =
              for f <- fileNames,
                  :filename.extension(f) == ~c".beam" do
                f
              end

            erlFiles =
              for f <- fileNames,
                  :filename.extension(f) == ~c".erl" do
                f
              end
            ...
            :ct.log(~c"Compiling in ~tp~n  Deleted:   ~p~n  ErlFiles:  ~tp~n  MacroDefs: ~p", [
              path,
              beamFiles,
              erlFiles,
              macroDefs
      ...
      {:error, error} ->
        :ct.pal(~c"file:set_cwd(~tp) returned ~p.~nCwd = ~tp", [path, {:error, error}, cwd])
        :error
    end
  end
  ...
  defp do_present_result(module, cmds, h, sf, result, _Config, options) do
    ...
    printFun.(~c"Module = ~p,~nCommands = ~p,~nHistory = ~p,~nFinalDynState = ~p,~nResult = ~p", [
      module,
      cmds,
      h,
      sf,
  ...
  def sequential_parallel(cs) do
    traverse_commands(
      ...
      fn l ->
        for l1 <- l do
          dup_module(l1, mkmod(~c"parallel", num(l1, l)))
        end
      end,
      cs
    )
  ...
  defp mkmod(pfxStr, n) do
    :erlang.list_to_atom(pfxStr ++ ~c"_" ++ :erlang.integer_to_list(n))
  end
  ...
  defp do_print_frequency_ranges(l0, options) do
    ...
    try do
      ...
      [
        :io_lib.format(~c"Range~*s: ~s~n", [2 * npos_range - 2, ~c"", ~c"Number in range"]),
        :io_lib.format(
          ~c"~*c:~*c~n",
