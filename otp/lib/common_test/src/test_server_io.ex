defmodule :m_test_server_io do
  use Bitwise
  require Record
  Record.defrecord(:r_st, :st, fds: :undefined, tags: [],
                              shared_gl: :undefined, gls: :undefined,
                              io_buffering: false, buffered: :undefined,
                              html_footer: :undefined, job_name: :undefined,
                              gl_props: :undefined, phase: :undefined,
                              offline_buffer: :undefined, stopping: :undefined,
                              pending_ops: :undefined)
  def start_link() do
    case (:erlang.whereis(:test_server_io)) do
      :undefined ->
        case (:gen_server.start_link({:local, :test_server_io},
                                       :test_server_io, [], [])) do
          {:ok, pid} ->
            {:ok, pid}
          other ->
  ...
  def init([]) do
    ...
    {:ok,
       r_st(fds: empty, shared_gl: shared, gls: :gb_sets.empty(),
           io_buffering: :gb_sets.empty(), buffered: empty,
           html_footer: '</body>\n</html>\n', job_name: '<name not set>', gl_props: [],
           phase: :starting, offline_buffer: [], pending_ops: [])}
  end
  ...
  def handle_call({:get_gl, false}, _From,
           r_st(gls: gls, gl_props: props) = st) do
    {:ok, pid} = :test_server_gl.start_link(self())
    :test_server_gl.set_props(pid, props)
    {:reply, pid, r_st(st, gls: :gb_sets.insert(pid, gls))}
  end

  def handle_call({:get_gl, true}, _From,
           r_st(shared_gl: shared) = st) do
    {:reply, shared, st}
  end

  def handle_call({:set_fd, tag, fd}, _From,
           r_st(fds: fds0, tags: tags0,
               offline_buffer: offlineBuff) = st) do
    fds = :gb_trees.enter(tag, fd, fds0)
    st1 = r_st(st, fds: fds, 
                  tags: [tag | :lists.delete(tag, tags0)])
    offlineBuff1 = (cond do
                      offlineBuff == [] ->
                        []
                      true ->
                        :lists.filtermap(fn {t, from, str} when t == tag ->
    ...
    {:reply, :ok,
       r_st(st1, phase: :started, 
                offline_buffer: :lists.reverse(offlineBuff1))}
  end

  def handle_call({:start_transaction, pid}, _From,
           r_st(io_buffering: buffer0, buffered: buf0) = st) do
    buf = (case (:gb_trees.is_defined(pid, buf0)) do
             false ->
    ...
    {:reply, :ok,
       r_st(st, io_buffering: buffer,  buffered: buf)}
  end
  ...
  def handle_call({:end_transaction, pid}, _From,
           r_st(io_buffering: buffer0, buffered: buffered0) = st0) do
  ...
  def handle_call({:print_buffered, pid}, _From,
           r_st(buffered: buffered0) = st0) do
    q0 = :gb_trees.get(pid, buffered0)
  ...
  def handle_call({:set_gl_props, props}, _From,
           r_st(shared_gl: shared) = st) do
    :test_server_gl.set_props(shared, props)
    {:reply, :ok, r_st(st, gl_props: props)}
  end

  def handle_call(:reset_state, from,
           r_st(phase: :stopping, pending_ops: ops) = st) do
    op = fn newSt ->
              {_, result, newSt1} = handle_call(:reset_state, from,
                                                  newSt)
              {result, newSt1}
         end
  ...
  def handle_call(:reset_state, _From,
           r_st(fds: fds, tags: tags, shared_gl: shared0, gls: gls,
               offline_buffer: offlineBuff)) do
    ...
    {:reply, :ok,
       r_st(fds: empty, shared_gl: shared, gls: :gb_sets.empty(),
           io_buffering: :gb_sets.empty(), buffered: empty,
           html_footer: '</body>\n</html>\n', job_name: '<name not set>', gl_props: [],
           phase: :starting, offline_buffer: offlineBuff,
           pending_ops: [])}
  end

  def handle_call({:stop, fdTags}, from,
           r_st(fds: fds0, tags: tags0, shared_gl: sGL,
               gls: gls0) = st0) do
    st = r_st(st0, gls: :gb_sets.insert(sGL, gls0), 
                  phase: :stopping,  stopping: from)
    gc(st)
    {fds1, tags1} = :lists.foldl(fn tag, {fds, tags} ->
                                      case (:gb_trees.lookup(tag, fds)) do
                                        :none ->
                                          {fds, tags}
                                        {:value, fd} ->
  ...
  def handle_info({:EXIT, pid, :normal},
           r_st(gls: gls0, stopping: from) = st) do
    gls = :gb_sets.delete_any(pid, gls0)
    case (:gb_sets.is_empty(gls) and from !== :undefined) do
      true ->
        :gen_server.reply(from, :ok)
        {:noreply,
           r_st(st, gls: gls,  phase: :stopping, 
                   stopping: :undefined)}
      false ->
  ...
  def handle_info(:stop_group_leaders, r_st(gls: gls) = st) do
    ...
    :timer.sleep(100)
    wait = (case (:lists.filter(fn glPid ->
                                     :erlang.is_process_alive(glPid)
                                end,
                                  glPids)) do
              [] ->
  ...
  def handle_info(:kill_group_leaders,
           r_st(gls: gls, stopping: from, pending_ops: ops) = st) do
    _ = (for gL <- :gb_sets.to_list(gls) do
           :erlang.exit(gL, :kill)
         end)
    cond do
      from != :undefined ->
    ...
    st1 = :lists.foldr(fn {replyTo, op}, newSt ->
                            {result, newSt1} = op.(newSt)
                            :gen_server.reply(replyTo, result)
                            newSt1
                       end,
                         r_st(st, phase: :idle,  pending_ops: []), ops)
    {:noreply, st1}
  end
  ...
  defp output(from, tag, str,
            r_st(io_buffering: buffered, buffered: buf0, phase: phase,
                offline_buffer: offlineBuff) = st) do
  ...
  defp gc(r_st(gls: gls0)) do
    inUse0 = (for p <- :erlang.processes() do
                (
