defmodule :m_cth_conn_log do
  ...
  defp merge_log_info([{mod, confOpts} | confList], hookList) do
    {opts, hookList1} = (case (:lists.keytake(mod, 1,
                                                hookList)) do
                           false ->
                             {confOpts, hookList}
  ...
  defp get_log_opts(mod, opts) do
    defaultLogType = (cond do
                        mod == :ct_telnet ->
                          :raw
                        true ->
                          :html
                      end)
    logType = :proplists.get_value(:log_type, opts,
                                     defaultLogType)
  ...
  def pre_init_per_testcase(_Suite, testCase, config, cthState) do
    logs = :lists.map(fn {connMod, {logType, hosts}} ->
                           :ct_util.set_testdata({{:cth_conn_log, connMod},
                                                    logType})
                           case (logType) do
                             ^logType when logType == :raw or logType == :pretty
                                           ->
                               dir = :test_server.lookup_config(:priv_dir,
                                                                  config)
                               ...
                               ls = (for host <- hosts do
                                       {host,
                                          :filename.join(dir,
                                                           tCStr ++ '-' ++ :erlang.atom_to_list(host) ++ '-' ++ connModStr ++ '.txt')}
                                     end) ++ [{:default, defLog}]
                               str = '<table borders=1><b>' ++ connModStr ++ ' logs:</b>\n' ++ (for {s,
                                                                     l} <- ls do
                                                                :io_lib.format('<tr><td>~tp</td><td><a href="~ts">~ts</a></td></tr>',
                                                                                 [s,
                                                                                      :ct_logs.uri(l),
                                                                                          :filename.basename(l)])
                                                              end) ++ '</table>'
    ...
    update = fn init when init == :undefined or init == []
                          ->
    ...
    :ct_util.update_testdata(:cth_conn_log, update,
                               [:create])
    {config, cthState}
  end
  ...
  def post_end_per_testcase(_Suite, testCase, _Config, return, cthState) do
    ...
    case (:ct_util.update_testdata(:cth_conn_log,
                                     update)) do
      :deleted ->
        _ = (for {connMod, _} <- cthState do
