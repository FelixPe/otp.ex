defmodule :m_test_server_sup do
  ...
  Record.defrecord(:r_cover, :cover, app: :undefined,
                                 ...
                                 cross: :undefined)
  Record.defrecord(:r_util_state, :util_state, starter: :undefined,
  ...
  defp kill_the_process(pid, timeout0, truncTO, reportTVal) do
    case (:erlang.is_process_alive(pid)) do
      true ->
        ...
        after 10000 ->
          (try do
            :error_logger.warning_msg('Testcase process ~w not responding to timetrap timeout:~n  ~tp.~nKilling testcase...~n', [pid, trap])
          catch
  ...
  defp is_app(application) do
    case (:file.consult(:filename.join([:code.lib_dir(application),
                                            'ebin',
                                                :erlang.atom_to_list(application) ++ '.app']))) do
      {:ok, [{:application, ^application, appFile}]} ->
        {:ok, appFile}
      _ ->
        :test_server.format(:minor, 'Application (.app) file not found, or it has very bad syntax.~n')
        {:error, :not_an_application}
    end
  end

  defp do_app_tests(appFile, appName, mode) do
    dictList = [{:missing_fields, []}, {:missing_mods, []},
                                           {:superfluous_mods_in_ebin, []},
                                               {:export_all_mods, []},
                                                   {:missing_apps, []}]
    fill_dictionary(dictList)
    check_fields([:description, :modules, :registered,
                                              :applications],
                   appFile)
    {:value, {:modules, mods}} = :lists.keysearch(:modules,
                                                    1, appFile)
    ...
    {:value,
       {:applications, apps}} = :lists.keysearch(:applications,
                                                   1, appFile)
    check_apps(apps)
    ...
    c = check_dict_tolerant(:superfluous_mods_in_ebin, 'Inconsistent app file, Modules not included in app file.',
                              mode)
  ...
  def appup_test(application) do
    case (is_app(application)) do
      {:ok, appFile} ->
        case (is_appup(application,
                         :proplists.get_value(:vsn, appFile))) do
          {:ok, up, down} ->
  ...
  defp is_appup(application, version) do
    ...
    appupPath = :filename.join([:code.lib_dir(application),
                                    'ebin', appupFile])
    case (:file.consult(appupPath)) do
      {:ok, [{^version, up, down}]} when (is_list(up) and
                                            is_list(down))
                                         ->
        {:ok, up, down}
      _ ->
        :test_server.format(:minor, 'Application upgrade (.appup) file not found, or it has very bad syntax.~n')
      ...
      [{".*", [{:restart_application, ^application}]}] ->
        case (down) do
          [{".*", [{:restart_application, ^application}]}] ->
            :ok
          _ ->
            :test_server.format(:minor, 'Library application needs restart_application downgrade instruction.~n')
            {:error, :library_downgrade_instruction_malformed}
        end
      _ ->
        :test_server.format(:minor, 'Library application needs restart_application upgrade instruction.~n')
        {:error, :library_upgrade_instruction_malformed}
    end
  end

  defp do_appup_tests(_, _Application, up, down, modules) do
    case (check_appup_clauses_plausible(up, :up,
                                          modules)) do
      :ok ->
        case (check_appup_clauses_plausible(down, :down,
                                              modules)) do
          :ok ->
  ...
  defp check_appup_clauses_plausible([{re, instrs} | rest], direction, modules)
    ...
    case (:re.compile(re, [:unicode])) do
      {:ok, _} ->
        case (check_appup_instructions(instrs, direction,
                                         modules)) do
          :ok ->
  ...
  defp check_appup_instructions(instrs, direction, modules) do
    case (check_instructions(direction, instrs, instrs, [],
                               [], modules)) do
      {_Good, []} ->
        :ok
      {_, bad} ->
  ...
  defp check_instructions(upDown, [instr | rest], all, good, bad,
            modules) do
    case ((try do
      ...
      :ok ->
        check_instructions(upDown, rest, all, [instr | good],
                             bad, modules)
      {:error, reason} ->
        newBad = [{instr, reason} | bad]
        check_instructions(upDown, rest, all, good, newBad,
                             modules)
    end
  end
  ...
  defp check_instruction(_, {:load_module, module, depMods}, _,
            modules) do
    check_module(module, modules)
    check_depend(depMods)
  end

  defp check_instruction(_, {:load_module, module, pre, post, depMods},
            _, modules) do
    check_module(module, modules)
    check_depend(depMods)
    check_purge(pre)
  ...
  defp check_instruction(_, {:update, module, :supervisor}, _,
            modules) do
    check_module(module, modules)
  end
  ...
  defp check_instruction(_, {:update, module, change, depMods}, _,
            modules) do
    check_module(module, modules)
    check_change(change)
    check_depend(depMods)
  ...
  defp check_instruction(_,
            {:update, module, change, pre, post, depMods}, _,
            modules) do
    check_module(module, modules)
    check_change(change)
    check_purge(pre)
  ...
  defp check_instruction(_,
            {:update, module, modType, timeout, change, pre, post,
               depMods},
            _, modules) do
    check_module(module, modules)
  ...
  defp check_instruction(_, {:add_application, application, type}, _,
            _) do
    check_application(application)
  ...
  defp check_instruction(_, instr, _, _) do
    throw({:error,
             {:low_level_or_invalid_instruction, instr}})
  end

  defp check_module(module, modules) do
    case ({is_atom(module),
             :lists.member(module, modules)}) do
      {true, true} ->
        :ok
      {true, false} ->
  ...
  def crash_dump_dir() do
    {:ok,
       dir} = :test_server_sup.framework_call(:get_log_dir, [],
                                                :file.get_cwd())
    dir
  end
  ...
  def tar_crash_dumps() do
    ...
    case (:filelib.wildcard(:filename.join(dir, 'erl_crash_dump*'))) do
      ...
      dumps ->
        ...
        {:ok, tar} = :erl_tar.open(tarFileName,
                                     [:write, :compressed])
        :lists.foreach(fn file ->
                            :ok = :erl_tar.add(tar, file,
      ...
      {:ok, bin} ->
        case (:file.write(fd, bin)) do
          :ok ->
            :ok
          {:error, error} ->
            :io.format(fd, 'Unable to write the crash dump to this file: ~tp~n', [:file.format_error(error)])
        end
      _Error ->
  ...
  def framework_call(callback, func, args, defaultReturn) do
    ...
    case (:erlang.function_exported(mod, func,
      ...
      true ->
        ...
        case (setTcState) do
          true ->
            :test_server.set_tc_state({:framework,
                                         {mod, func, args}})
          false ->
            :ok
        end
