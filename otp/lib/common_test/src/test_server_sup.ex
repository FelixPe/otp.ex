defmodule :m_test_server_sup do
          ...
          10000 ->
            try do
              :error_logger.warning_msg(
                ~c"Testcase process ~w not responding to timetrap timeout:~n  ~tp.~nKilling testcase...~n",
                [pid, trap]
              )
            catch
  ...
  def call_crash(time, crash, m, f, a) do
    ...
    answer =
      receive do
        ...
        {:EXIT, reason} ->
          :test_server.format(12, ~c"Wrong crash reason. Wanted ~tp, got ~tp.", [crash, reason])
  ...
  defp is_app(application) do
    case :file.consult(
           :filename.join([
             ...
             ~c"ebin",
             :erlang.atom_to_list(application) ++ ~c".app"
           ])
         ) do
      ...
      _ ->
        :test_server.format(
          :minor,
          ~c"Application (.app) file not found, or it has very bad syntax.~n"
        )
  ...
  defp do_app_tests(appFile, appName, mode) do
    ...
    check_apps(apps)
    a = check_dict(:missing_fields, ~c"Inconsistent app file, missing fields")
    b = check_dict(:missing_mods, ~c"Inconsistent app file, missing modules")

    c =
      check_dict_tolerant(
        :superfluous_mods_in_ebin,
        ~c"Inconsistent app file, Modules not included in app file.",
        mode
      )

    d = check_dict(:export_all_mods, ~c"Inconsistent app file, Modules have `export_all'.")
    e = check_dict(:missing_apps, ~c"Inconsistent app file, missing applications.")
  ...
  defp is_appup(application, version) do
    appupFile = :erlang.atom_to_list(application) ++ ~c".appup"
    appupPath = :filename.join([:code.lib_dir(application), ~c"ebin", appupFile])
    ...
    case :file.consult(appupPath) do
      ...
      _ ->
        :test_server.format(
          :minor,
          ~c"Application upgrade (.appup) file not found, or it has very bad syntax.~n"
        )
  ...
  defp do_appup_tests(:undefined, application, up, down, _Modules) do
    case up do
      [{".*", [{:restart_application, ^application}]}] ->
        case down do
          ...
          _ ->
            :test_server.format(
              :minor,
              ~c"Library application needs restart_application downgrade instruction.~n"
            )
      ...
      _ ->
        :test_server.format(
          :minor,
          ~c"Library application needs restart_application upgrade instruction.~n"
        )
    ...
    case check_appup_clauses_plausible(up, :up, modules) do
      :ok ->
        case check_appup_clauses_plausible(down, :down, modules) do
          :ok ->
            :test_server.format(:minor, ~c"OK~n")

          error ->
            :test_server.format(:minor, ~c"ERROR ~tp~n", [error])
            :ct.fail(error)
        end
  ...
  defp check_dict(dict, reason) do
    case :erlang.get(dict) do
      ...
      list ->
        :io.format(~c"** ~ts (~ts) ->~n~tp~n", [reason, dict, list])
        0
    end
  end
  ...
  defp get_ebin_modnames(appName) do
    wc = :filename.join([:code.lib_dir(appName), ~c"ebin", ~c"*" ++ :code.objfile_extension()])
  ...
  def cleanup_crash_dumps() do
    dir = crash_dump_dir()
    dumps = :filelib.wildcard(:filename.join(dir, ~c"erl_crash_dump*"))
    delete_files(dumps)
  end
  ...
  def tar_crash_dumps() do
    ...
    case :filelib.wildcard(:filename.join(dir, ~c"erl_crash_dump*")) do
      [] ->
        {:error, :no_crash_dumps}

      dumps ->
        tarFileName = :filename.join(dir, ~c"crash_dumps.tar.gz")
  ...
  def check_new_crash_dumps() do
    ...
    case length(dumps) do
      ...
      num ->
        :test_server_ctrl.format(:minor, ~c"Found ~w crash dumps:~n", [num])
  ...
  defp append_files_to_logfile([file | files]) do
    nodeName = from(?., file)
    :test_server_ctrl.format(:minor, ~c"Crash dump from node ~tp:~n", [nodeName])
    ...
    case :file.read_file(file) do
      {:ok, bin} ->
        case :file.write(fd, bin) do
          ...
          {:error, error} ->
            :io.format(fd, ~c"Unable to write the crash dump to this file: ~tp~n", [
              :file.format_error(error)
            ])
        end

      _Error ->
        :io.format(fd, ~c"Failed to read: ~ts\n", [file])
    end
  ...
  defp delete_files([file | files]) do
    :io.format(~c"Deleting file: ~ts~n", [file])

    case :file.delete(file) do
      {:error, _} ->
        case :file.rename(file, file ++ ~c".old") do
          {:error, error} ->
            :io.format(~c"Could neither delete nor rename file ~ts: ~ts.~n", [file, error])
  ...
  def framework_call(fW, _Func, _Args, defaultReturn)
      when fW === false or fW === ~c"undefined" do
    defaultReturn
  end
  ...
  def format_loc({mod, func}) when is_atom(func) do
    :io_lib.format(~c"{~w,~tw}", [mod, func])
  end

  def format_loc(loc) do
    :io_lib.format(~c"~tp", [loc])
  end

  defp format_loc1([{mod, func, line}]) do
    [~c"              ", format_loc1({mod, func, line}), ~c"]"]
  end
      ...
      {false, [?E, ?T, ?I, ?U, ?S, ?_ | _]} ->
        link =
          cond do
            ...
            line == :last_expr ->
              :erlang.list_to_atom(:erlang.atom_to_list(func) ++ ~c"-last_expr")
        ...
        :io_lib.format(
          ~c"{~w,~tw,<a href=\"~ts~ts#~ts\">~tw</a>}",
          [mod, func, :test_server_ctrl.uri_encode(downcase(modStr)), ~c".src.html", link, line]
        )

      _ ->
        :io_lib.format(~c"{~w,~tw,~tw}", [mod, func, line])
    end
  end
