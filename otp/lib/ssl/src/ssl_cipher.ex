defmodule :m_ssl_cipher do
  ...
  defp block_cipher(fun, blockSz, r_cipher_state(key: key, iv: iV) = cS0, mac, fragment, {3, 1}) do
             ...
             :undefined ->
               :ssl_logger.log(
                 ...
                 %{
                   ...
                   line: 265,
                   file: ~c"otp/lib/ssl/src/ssl_cipher.erl"
                 }
               )
  ...
  def anonymous_suites(version) when :erlang.element(1, version) == 3 do
    ...
    :lists.flatmap(
      &:tls_v1.exclusive_anonymous_suites/1,
      suitesToTest
    )
  end

  def anonymous_suites(version)
      when :erlang.element(
             1,
             version
           ) == 254 do
  ...
  def filter(derCert, ciphers0, version) do
    ...
    type =
      case :ssl_certificate.public_key_type(r_PublicKeyAlgorithm(pubKeyAlg, :algorithm)) do
        :rsa_pss_pss ->
          :rsa
  ...
  defp filter_suite(
    ...
    keyPairs = [
      {keyExchange, keyFilters},
      {cipher, cipherFilters},
      {hash, hashFilters},
      {prf, prfFilters}
    ]
  ...
  defp mac_hash(version, macAlg, macSecret, seqNo, type, length, fragment)
  ...
  def bulk_cipher_algorithm(cipher)
  ...
  defp prf_algorithm(:default_prf, version)
       when :erlang.element(
              1,
              version
            ) == 3 do
  ...
  defp generic_block_cipher_from_bin(version, t, iV, hashSize)
       when version == {3, 2} or
              version == {3, 3} do
    sz1 = byte_size(t) - 1
  ...
  defp filter_kex(ciphers, fn__) do
    filter_suites(
      ciphers,
      %{key_exchange_filters: [fn__], cipher_filters: [], mac_filters: [], prf_filters: []}
    )
  end
  ...
  def encrypt_ticket(
    ...
    plaintext =
      case certificateLength do
        0 ->
          <<plaintext1::binary, 0::size(16)-unsigned-big-integer>>

        _ ->
          <<plaintext1::binary, certificateLength::size(16)-unsigned-big-integer,
            certificate::binary>>
      end
    ...
    case decrypt_ticket_data(cipherFragment, shard, iV) do
      ...
      plaintext ->
        ...
        <<pSK::size(hashSize)-binary, ticketAgeAdd::size(64)-unsigned-big-integer,
          ...
          certificateLength::size(16)-unsigned-big-integer,
          certificate1::size(certificateLength)-binary, _::binary>> = t
