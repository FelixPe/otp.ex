defmodule :m_ssl_cipher do
  ...
  def security_parameters(<<0 :: size(8) - unsigned - big - integer,
             0 :: size(8) - unsigned - big - integer>> = cipherSuite,
           secParams) do
  ...
  def security_parameters(version, cipherSuite, secParams) do
    %{cipher: cipher, mac: hash,
        prf:
        prfHashAlg} = :ssl_cipher_format.suite_bin_to_map(cipherSuite)
    r_security_parameters(secParams, cipher_suite: cipherSuite, 
  ...
  def security_parameters_1_3(secParams, cipherSuite) do
    ...
    r_security_parameters(secParams, cipher_suite: cipherSuite, 
                   bulk_cipher_algorithm: bulk_cipher_algorithm(cipher), 
                   prf_algorithm: prfHashAlg,  cipher_type: 2)
  end
  ...
  def cipher(cipherEnum,
           cipherState = r_cipher_state(state: {:stream_init, :rc4, key, _IV}),
           mac, fragment, version) do
    state = :crypto.crypto_init(:rc4, key, true)
    cipher(cipherEnum, r_cipher_state(cipherState, state: state), mac,
             fragment, version)
  end

  def cipher(1, cipherState = r_cipher_state(state: state), mac, fragment,
           _Version) do
  ...
  def cipher(3, cipherState, mac, fragment, version) do
    block_cipher(fn key, iV, t ->
                      :crypto.crypto_one_time(:des_cbc, key, iV, t, true)
                 end,
                   block_size(:des_cbc), cipherState, mac, fragment,
                   version)
  end
  ...
  def cipher(4, cipherState, mac, fragment, version) do
    block_cipher(fn key, iV, t ->
                   ...
                   block_size(:des_ede3_cbc), cipherState, mac, fragment,
                   version)
  end

  def cipher(7, cipherState, mac, fragment, version) do
    block_cipher(fn key, iV, t when byte_size(key) === 16 ->
                      :crypto.crypto_one_time(:aes_128_cbc, key, iV, t, true)
                    key, iV, t when byte_size(key) === 32 ->
                      :crypto.crypto_one_time(:aes_256_cbc, key, iV, t, true)
                 end,
                   block_size(:aes_128_cbc), cipherState, mac, fragment,
                   version)
  end

  def aead_encrypt(type, key, nonce, fragment, additionalData,
           tagLen) do
    :crypto.crypto_one_time_aead(aead_type(type,
                                             byte_size(key)),
                                   key, nonce, fragment, additionalData, tagLen,
                                   true)
  end

  def aead_decrypt(type, key, nonce, cipherText, cipherTag,
           additionalData) do
    :crypto.crypto_one_time_aead(aead_type(type,
                                             byte_size(key)),
  ...
  defp block_cipher(fun, blockSz, r_cipher_state(key: key, iv: iV) = cS0, mac,
            fragment, {3, 1}) do
  ...
  defp block_cipher(fun, blockSz,
            r_cipher_state(key: key, iv: iV, state: iV_Cache0) = cS0, mac,
            fragment, version)
      when version > {3, 1} do
    iV_Size = byte_size(iV)
    <<nextIV :: size(iV_Size) - binary,
        iV_Cache :: binary>> = (case (iV_Cache0) do
                                  <<>> ->
  ...
  def decipher(cipherEnum, hashSz,
           cipherState = r_cipher_state(state: {:stream_init, :rc4, key, _IV}),
           fragment, version, paddingCheck) do
    state = :crypto.crypto_init(:rc4, key, false)
    decipher(cipherEnum, hashSz,
               r_cipher_state(cipherState, state: state), fragment, version,
               paddingCheck)
  end

  def decipher(1, hashSz, cipherState = r_cipher_state(state: state),
           fragment, _, _) do
    try do
      ...
      _, reason ->
                ...
                :undefined ->
                  :ssl_logger.log(:debug, :debug,
                                    ...
                                    %{mfa: {:ssl_cipher, :decipher, 6},
                                        line: 265, file: 'otp/lib/ssl/src/ssl_cipher.erl'})
                __LogLevel__ ->
        ...
        r_alert(level: 2, description: 20,
            where: %{mfa: {:ssl_cipher, :decipher, 6}, line: 266,
  ...
  def decipher(3, hashSz, cipherState, fragment, version,
           paddingCheck) do
    block_decipher(fn key, iV, t ->
                        :crypto.crypto_one_time(:des_cbc, key, iV, t, false)
                   end,
                     cipherState, hashSz, fragment, version, paddingCheck)
  end

  def decipher(4, hashSz, cipherState, fragment, version,
           paddingCheck) do
    block_decipher(fn key, iV, t ->
                        :crypto.crypto_one_time(:des_ede3_cbc, key, iV, t,
                                                  false)
                   end,
                     cipherState, hashSz, fragment, version, paddingCheck)
  end

  def decipher(7, hashSz, cipherState, fragment, version,
           paddingCheck) do
    block_decipher(fn key, iV, t when byte_size(key) === 16
                                      ->
  ...
  defp block_decipher(fun, r_cipher_state(key: key, iv: iV) = cipherState0, hashSz,
            fragment, version, paddingCheck) do
    try do
      text = fun.(key, iV, fragment)
      nextIV = next_iv(fragment, iV)
      gBC = generic_block_cipher_from_bin(version, text,
                                            nextIV, hashSz)
      ...
      _, reason ->
                ...
                :undefined ->
                  :ssl_logger.log(:debug, :debug,
                                    ...
                                    %{mfa: {:ssl_cipher, :block_decipher, 6},
                                        line: 311, file: 'otp/lib/ssl/src/ssl_cipher.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:debug, __LogLevel__,
                                    %{description: :decrypt_error,
                                        reason:
        ...
        r_alert(level: 2, description: 20,
            where: %{mfa: {:ssl_cipher, :block_decipher, 6},
                       line: 312, file: 'otp/lib/ssl/src/ssl_cipher.erl'},
            reason: :decryption_failed)
    end
  end
  ...
  def suites(version) when :erlang.element(1,
                                         version) == 254 do
    :dtls_v1.suites(version)
  end

  def all_suites({3, 4} = version) do
    suites(version) ++ tls_legacy_suites({3, 3})
  end

  def all_suites(version) when :erlang.element(1, version) == 3 do
  ...
  defp tls_legacy_suites(version) do
    tests = [&:tls_v1.psk_suites/1, &:tls_v1.srp_suites/1,
                                        &:tls_v1.rsa_suites/1,
                                            &:tls_v1.des_suites/1,
                                                &:tls_v1.rc4_suites/1]
    :lists.flatmap(fn fun ->
                        fun.(version)
                   end,
                     tests)
  end

  def anonymous_suites(version) when :erlang.element(1, version) == 3 do
    suitesToTest = anonymous_suite_to_test(version)
    :lists.flatmap(&:tls_v1.exclusive_anonymous_suites/1,
                     suitesToTest)
  end

  def anonymous_suites(version) when :erlang.element(1,
                                         version) == 254 do
    :dtls_v1.anonymous_suites(version)
  end

  defp anonymous_suite_to_test({3, 1}) do
    [{3, 1}]
  end

  defp anonymous_suite_to_test({3, 2}) do
    [{3, 2}, {3, 1}]
  end

  defp anonymous_suite_to_test({3, 3}) do
    [{3, 3}, {3, 2}, {3, 1}]
  end

  defp anonymous_suite_to_test({3, 4}) do
    [{3, 4}]
  end
  ...
  def filter(derCert, ciphers0, version) do
    ...
    pubKeyInfo = r_OTPTBSCertificate(r_OTPCertificate(otpCert, :tbsCertificate), :subjectPublicKeyInfo)
    pubKeyAlg = r_OTPSubjectPublicKeyInfo(pubKeyInfo, :algorithm)
    type = (case (:ssl_certificate.public_key_type(r_PublicKeyAlgorithm(pubKeyAlg, :algorithm))) do
              :rsa_pss_pss ->
                :rsa
              other ->
                other
            end)
    ciphers = filter_suites_pubkey(type, ciphers0, version,
                                     otpCert)
    sigAlgo = r_SignatureAlgorithm(sigAlg, :algorithm)
  ...
  def filter_suites(suites, filters) do
    fn__ = fn suite when :erlang.is_map_key(:key_exchange,
                                              suite)
                         ->
                suite
              suite ->
    ...
    :lists.filter(fn suite ->
                       filter_suite(fn__.(suite), filters)
                  end,
                    suites)
  end

  defp filter_suite(%{key_exchange: keyExchange, cipher: cipher,
              mac: hash, prf: prf},
            %{key_exchange_filters: keyFilters,
                cipher_filters: cipherFilters, mac_filters: hashFilters,
                prf_filters: prfFilters}) do
    keyPairs = [{keyExchange, keyFilters}, {cipher,
                                              cipherFilters},
                                               {hash, hashFilters}, {prf,
                                                                       prfFilters}]
    :lists.all(&all_filters/1, keyPairs)
  end
  ...
  defp all_filters({value, filters}) do
    :lists.all(fn filterFn ->
                    filterFn.(value)
               end,
                 filters)
  end
  ...
  defp is_acceptable_keyexchange(keyExchange, _Algos) when keyExchange == :psk or
                                      keyExchange == :null or
                                      keyExchange == :any do
    true
  end
  ...
  defp is_acceptable_keyexchange(:dhe_dss, algos) do
    :proplists.get_bool(:dh,
                          algos) and :proplists.get_bool(:dss, algos)
  end

  defp is_acceptable_keyexchange(:dhe_rsa, algos) do
    :proplists.get_bool(:dh,
                          algos) and :proplists.get_bool(:rsa, algos)
  end
  ...
  defp is_acceptable_keyexchange(keyExchange, algos)
    ...
    :proplists.get_bool(:ecdh,
                          algos) and :proplists.get_bool(:ecdsa, algos)
  end
  ...
  defp is_acceptable_keyexchange(keyExchange, algos) when keyExchange == :rsa or
  ...
  def calc_mac_hash(type, version, plainFragment,
           %{sequence_number: seqNo, mac_secret: macSecret,
               security_parameters: r_security_parameters(mac_algorithm: macAlgorithm)}) do
    calc_mac_hash(type, version, plainFragment,
                    macAlgorithm, macSecret, seqNo)
  end

  def calc_mac_hash(type, version, plainFragment, macAlgorithm,
           macSecret, seqNo) do
    length = :erlang.iolist_size(plainFragment)
    mac_hash(version, macAlgorithm, macSecret, seqNo, type,
  ...
  def is_supported_sign({hash, :rsa} = signAlgo, hashSigns) do
    :lists.member(signAlgo,
                    hashSigns) or :lists.member({hash, :rsa_pss_rsae},
                                                  hashSigns)
  end

  def is_supported_sign(signAlgo, hashSigns) do
  ...
  def signature_scheme(:eddsa_ed25519) do
    2055
  end

  def signature_scheme(:eddsa_ed448) do
    2056
  end
  ...
  def signature_scheme({:sha512, :rsa_pss_pss}) do
    2059
  end

  def signature_scheme({:sha384, :rsa_pss_pss}) do
    2058
  end

  def signature_scheme({:sha256, :rsa_pss_pss}) do
    2057
  end

  def signature_scheme({:sha512, :rsa_pss_rsae}) do
    2054
  end

  def signature_scheme({:sha384, :rsa_pss_rsae}) do
    2053
  end

  def signature_scheme({:sha256, :rsa_pss_rsae}) do
    2052
  end
  ...
  def signature_scheme({hash0, sign0}) do
    ...
    <<sigAlg
      ::
      size(16) - unsigned - big - integer>> = <<hash
                                                ::
                                                size(8) - unsigned - big -
                                                  integer,
                                                  sign
  ...
  def signature_scheme(signAlgo) when is_integer(signAlgo) do
    <<hash :: size(8) - unsigned - big - integer,
        sign
        ::
        size(8) - unsigned - big - integer>> = <<signAlgo
                                                 ::
  ...
  def signature_schemes_1_2(sigAlgs) do
    :lists.reverse(:lists.foldl(fn alg, acc
                                       when is_atom(alg) ->
                                     case (scheme_to_components(alg)) do
  ...
  def scheme_to_components(:eddsa_ed25519) do
    {:none, :eddsa, :ed25519}
  end

  def scheme_to_components(:eddsa_ed448) do
    {:none, :eddsa, :ed448}
  end
  ...
  defp format_sign(:rsa_pkcs1) do
    :rsa
  end

  defp format_sign(value) do
    value
  end

  defp mac_hash({_, _}, 0, _MacSecret, _SeqNo, _Type, _Length,
            _Fragment) do
    <<>>
  end

  defp mac_hash(version, macAlg, macSecret, seqNo, type, length,
            fragment)
      when (version <= {3, 3} and version !== {3, 0}) do
    :tls_v1.mac_hash(macAlg, macSecret, seqNo, type,
                       version, length, fragment)
  end

  def bulk_cipher_algorithm(:null) do
    0
  end

  def bulk_cipher_algorithm(:rc4_128) do
    1
  end

  def bulk_cipher_algorithm(:des_cbc) do
    3
  end

  def bulk_cipher_algorithm(:"3des_ede_cbc") do
    4
  end

  def bulk_cipher_algorithm(cipher) when cipher == :aes_128_cbc or
                        cipher == :aes_256_cbc do
    7
  end

  def bulk_cipher_algorithm(cipher) when cipher == :aes_128_gcm or
                        cipher == :aes_256_gcm do
    8
  end

  def bulk_cipher_algorithm(cipher) when cipher == :aes_128_ccm or
                        cipher == :aes_256_ccm do
    10
  end

  def bulk_cipher_algorithm(cipher) when cipher == :aes_128_ccm_8 or
                        cipher == :aes_256_ccm_8 do
    11
  end

  def bulk_cipher_algorithm(:chacha20_poly1305) do
    9
  end

  defp type(cipher) when cipher == :null or
                         cipher == :rc4_128 do
    0
  end

  defp type(cipher) when cipher == :des_cbc or
                         cipher == :"3des_ede_cbc" or cipher == :aes_128_cbc or
                         cipher == :aes_256_cbc do
    1
  end

  defp type(cipher) when cipher == :aes_128_gcm or
  ...
  defp iv_size(cipher) when cipher == :null or
                         cipher == :rc4_128 do
    0
  end

  defp iv_size(cipher) when cipher == :aes_128_gcm or
                         cipher == :aes_256_gcm or cipher == :aes_128_ccm or
                         cipher == :aes_256_ccm or cipher == :aes_128_ccm_8 or
                         cipher == :aes_256_ccm_8 do
    4
  ...
  defp block_size(cipher) when cipher == :des_cbc or
  ...
  defp prf_algorithm(:default_prf, {3, 3}) do
    4
  end

  defp prf_algorithm(:default_prf, version) when :erlang.element(1,
                                                        version) == 3 do
    4711
  end
  ...
  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 10},
             parameters: r_RSASSA_PSS_params(maskGenAlgorithm: r_MaskGenAlgorithm(algorithm: {1, 2, 840,
                                                             113549, 1, 1, 8},
                                                 parameters: hashAlgo)))) do
  ...
  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 11})) do
    :rsa_pkcs1_sha256
  end

  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 12})) do
    :rsa_pkcs1_sha384
  end

  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 13})) do
    :rsa_pkcs1_sha512
  end
  ...
  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 5})) do
    :rsa_pkcs1_sha1
  end
  ...
  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 1},
             parameters: 0)) do
    :rsa_pkcs1_sha1
  end

  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 1})) do
    :rsa_pss_rsae
  end

  def signature_algorithm_to_scheme(r_SignatureAlgorithm(algorithm: {1, 2, 840, 113549, 1, 1, 10})) do
    :rsa_pss_pss
  end

  defp generic_block_cipher_from_bin({3, 1}, t, iV, hashSize) do
    sz1 = byte_size(t) - 1
    <<_ :: size(sz1) - binary,
        padLength0 :: size(8) - unsigned - big - integer>> = t
    padLength = (cond do
                   padLength0 >= sz1 ->
                     0
                   true ->
    ...
    <<content :: size(compressedLength) - binary,
        mac :: size(hashSize) - binary,
        padding :: size(padLength) - binary,
        ^padLength0 :: size(8) - unsigned - big - integer>> = t
    r_generic_block_cipher(content: content, mac: mac, padding: padding,
        padding_length: padLength0, next_iv: iV)
  end

  defp generic_block_cipher_from_bin(version, t, iV, hashSize) when version == {3,
                                                       2} or
                                           version == {3, 3} do
    sz1 = byte_size(t) - 1
    <<_ :: size(sz1) - binary,
    ...
    <<nextIV :: size(iVLength) - binary,
        content :: size(compressedLength) - binary,
        mac :: size(hashSize) - binary,
        padding :: size(padLength) - binary,
        ^padLength :: size(8) - unsigned - big - integer>> = t
    r_generic_block_cipher(content: content, mac: mac, padding: padding,
        padding_length: padLength, next_iv: nextIV)
  end
  ...
  defp generic_stream_cipher_from_bin(t, hashSz) do
    ...
    <<content :: size(compressedLength) - binary,
        mac :: size(hashSz) - binary>> = t
    r_generic_stream_cipher(content: content, mac: mac)
  end

  defp is_correct_padding(r_generic_block_cipher(padding_length: len, padding: padding),
            {3, 0}, _) do
  ...
  defp is_correct_padding(r_generic_block_cipher(padding_length: len, padding: padding), _,
            _) do
  ...
  defp next_iv(bin, iV) do
    ...
    <<_ :: size(firstPart) - binary,
        nextIV :: size(iVSz) - binary>> = bin
    nextIV
  end

  defp filter_suites_pubkey(:rsa, ciphersSuites0, _Version, otpCert) do
    keyUses = key_uses(otpCert)
    notECDSAKeyed = (ciphersSuites0 -- ec_keyed_suites(ciphersSuites0)) -- dss_keyed_suites(ciphersSuites0)
    ciphersSuites = filter_keyuse_suites(:keyEncipherment,
                                           keyUses, notECDSAKeyed,
                                           rsa_suites_encipher(ciphersSuites0))
    filter_keyuse_suites(:digitalSignature, keyUses,
                           ciphersSuites, rsa_ecdhe_dhe_suites(ciphersSuites))
  end
  ...
  defp filter_suites_pubkey(:dsa, ciphers, _, otpCert) do
    ...
    filter_keyuse_suites(:digitalSignature, keyUses,
                           notECRSAKeyed, dss_dhe_suites(ciphers))
  end

  defp filter_suites_pubkey(:ecdsa, ciphers, _, otpCert) do
    uses = key_uses(otpCert)
    notRSADSAKeyed = (ciphers -- rsa_keyed_suites(ciphers)) -- dss_keyed_suites(ciphers)
    ciphersSuites = filter_keyuse_suites(:digitalSignature,
                                           uses, notRSADSAKeyed,
                                           ec_ecdhe_suites(ciphers))
    filter_keyuse_suites(:keyAgreement, uses, ciphersSuites,
                           ec_ecdh_suites(ciphers))
  end

  defp filter_suites_signature(_, ciphers, version) when version >= {3, 3} do
    ciphers
  end

  defp filter_suites_signature(:rsa, ciphers, version) do
    (ciphers -- ecdsa_signed_suites(ciphers,
                                      version)) -- dsa_signed_suites(ciphers)
  end

  defp filter_suites_signature(:dsa, ciphers, version) do
    (ciphers -- ecdsa_signed_suites(ciphers,
                                      version)) -- rsa_signed_suites(ciphers,
                                                                       version)
  end

  defp filter_suites_signature(:ecdsa, ciphers, version) do
    (ciphers -- rsa_signed_suites(ciphers,
                                    version)) -- dsa_signed_suites(ciphers)
  end

  defp rsa_signed({3, 3}) do
    fn :rsa ->
         true
       :dhe_rsa ->
         true
  ...
  defp ecdsa_signed(version) when version >= {3, 3} do
    fn :ecdhe_ecdsa ->
         true
       _ ->
         false
  ...
  defp rsa_keyed_suites(ciphers) do
    filter_kex(ciphers, &rsa_keyed/1)
  end
  ...
  defp dsa_signed_suites(ciphers) do
    filter_kex(ciphers,
                 fn :dhe_dss ->
                      true
                    _ ->
  ...
  defp filter_kex(ciphers, fn__) do
    filter_suites(ciphers,
                    %{key_exchange_filters: [fn__], cipher_filters: [],
                        mac_filters: [], prf_filters: []})
  end
  ...
  defp key_uses(otpCert) do
    ...
    case (:ssl_certificate.select_extension({2, 5, 29, 15},
                                              extensions)) do
      :undefined ->
        []
  ...
  defp filter_keyuse_suites(use, keyUse, cipherSuits, suites) do
    case (:ssl_certificate.is_valid_key_usage(keyUse,
                                                use)) do
      true ->
        cipherSuits
      false ->
  ...
  def generate_client_shares(groups) do
    keyShareEntry = fn group ->
                         r_key_share_entry(group: group,
                             key_exchange: generate_key_exchange(group))
                    end
  ...
  def encrypt_ticket(r_stateless_ticket(hash: hash, pre_shared_key: pSK,
             ticket_age_add: ticketAgeAdd, lifetime: lifetime,
             timestamp: timestamp, certificate: certificate),
           shard, iV) do
    plaintext1 = <<:ssl_cipher.hash_algorithm(hash)
                   ::
                   size(8),
                     pSK :: binary,
                     ticketAgeAdd :: size(64) - unsigned - big - integer,
                     lifetime :: size(32) - unsigned - big - integer,
                     timestamp :: size(32) - unsigned - big - integer>>
    certificateLength = (case (certificate) do
                           :undefined ->
                             0
                           _ ->
                             byte_size(certificate)
                         end)
    plaintext = (case (certificateLength) do
                   0 ->
                     <<plaintext1 :: binary,
  ...
  def decrypt_ticket(cipherFragment, shard, iV) do
    case (decrypt_ticket_data(cipherFragment, shard, iV)) do
      :error ->
        :error
      plaintext ->
        <<hKDF :: size(8) - unsigned - big - integer,
            t :: binary>> = plaintext
        hash = hash_algorithm(hKDF)
        hashSize = hash_size(hash)
        <<pSK :: size(hashSize) - binary,
            ticketAgeAdd :: size(64) - unsigned - big - integer,
            lifetime :: size(32) - unsigned - big - integer,
            timestamp :: size(32) - unsigned - big - integer,
            certificateLength
            ::
            size(16) - unsigned - big - integer,
            certificate1 :: size(certificateLength) - binary,
            _ :: binary>> = t
        certificate = (case (certificateLength) do
                         0 ->
                           :undefined
                         _ ->
                           certificate1
                       end)
        r_stateless_ticket(hash: hash, pre_shared_key: pSK,
            ticket_age_add: ticketAgeAdd, lifetime: lifetime,
            timestamp: timestamp, certificate: certificate)
    end
  end
  ...
  defp encrypt_ticket_data(plaintext, shard, iV) do
    aAD = additional_data("ticket",
                            :erlang.iolist_size(plaintext) + 16)
    {oTP, key} = make_otp_key(shard)
    {content,
       cipherTag} = :crypto.crypto_one_time_aead(:aes_256_gcm,
                                                   key, iV, plaintext, aAD, 16,
                                                   true)
  ...
  defp decrypt_ticket_data(cipherFragment, shard, iV) do
    ...
    case (cipherFragment) do
      <<encrypted :: size(len) - binary,
          cipherTag :: size(16) - binary,
          oTP :: size(size) - binary>> ->
        key = :crypto.exor(oTP, shard)
        :crypto.crypto_one_time_aead(:aes_256_gcm, key, iV,
                                       encrypted, aAD, cipherTag, false)
      _ ->
        :error
    end
  end
  ...
  def decrypt_data(aDTag, cipherFragment, shard, iV) do
    ...
    <<encrypted :: size(len) - binary,
        cipherTag :: size(16) - binary,
        oTP :: size(size) - binary>> = cipherFragment
    key = :crypto.exor(oTP, shard)
    :crypto.crypto_one_time_aead(:aes_256_gcm, key, iV,
                                   encrypted, aAD, cipherTag, false)
  end
