defmodule :m_ssl_alert do
  ...
  defp alert_prefix_format(protocolName, role, stateName) do
    {~c"~s ~p: In state ~p", [protocolName, role, stateName]}
  end
  ...
  defp own_alert_txt(
    ...
    ~c"at " ++
      ...
      ~c" ALERT: " ++ level_txt(level) ++ description_txt(description)
  end
  ...
  defp decode(
    ...
    r_alert(
      level: 2,
      description: 50,
      where: %{mfa: {:ssl_alert, :decode, 3}, line: 131, file: ~c"otp/lib/ssl/src/ssl_alert.erl"},
    ...
    alert =
      r_alert(
        ...
        where: %{
          ...
          line: 133,
          file: ~c"otp/lib/ssl/src/ssl_alert.erl"
        }
      )
  ...
  defp decode(<<_Level::size(8)-unsigned-big-integer, _::binary>>, _, _) do
    r_alert(
      level: 2,
      description: 47,
      where: %{mfa: {:ssl_alert, :decode, 3}, line: 139, file: ~c"otp/lib/ssl/src/ssl_alert.erl"},
  ...
  defp description_txt(113) do
    ~c"Bad Certificate Status Response"
  end

  defp description_txt(114) do
    ~c"Bad Certificate Hash Value"
  end

  defp description_txt(115) do
    ~c"Unknown Psk Identity"
  end
  ...
  defp description_txt(120) do
    ~c"No application protocol"
  end

  defp description_txt(enum) do
    :lists.flatten(:io_lib.format(~c"unsupported/unknown alert: ~p", [enum]))
  end
