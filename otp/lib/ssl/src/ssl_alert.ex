defmodule :m_ssl_alert do
  ...
  def reason_code(r_alert(description: description, role: role) = alert,
           role, protocolName, stateName) do
    {prefixFmt,
       prefixArgs} = alert_prefix_format(protocolName, role,
                                           stateName)
    {fmt, args} = own_alert_format_depth(alert)
    txt = :lists.flatten(:io_lib.format(prefixFmt ++ fmt,
                                          prefixArgs ++ args))
    {:tls_alert, {description_atom(description), txt}}
  end

  def reason_code(r_alert(description: description) = alert, role,
           protocolName, stateName) do
    {fmt, args} = alert_format(protocolName, role,
                                 stateName, alert)
  ...
  defp own_alert_txt(r_alert(level: level, description: description,
              where: %{line: line, file: mod}, role: role)) do
    'at ' ++ mod ++ ':' ++ :erlang.integer_to_list(line) ++ ' generated ' ++ :string.uppercase(:erlang.atom_to_list(role)) ++ ' ALERT: ' ++ level_txt(level) ++ description_txt(description)
  end
  ...
  defp alert_txt(r_alert(level: level, description: description,
              role: role)) do
    'received ' ++ :string.uppercase(:erlang.atom_to_list(role)) ++ ' ALERT: ' ++ level_txt(level) ++ description_txt(description)
  end

  defp decode(<<level :: size(8) - unsigned - big - integer,
              _ :: size(8) - unsigned - big - integer, _ :: binary>>,
            _, n)
      when (level == 1 and n > 10) do
    r_alert(level: 2, description: 50,
        where: %{mfa: {:ssl_alert, :decode, 3}, line: 131,
  ...
  defp decode(<<level :: size(8) - unsigned - big - integer,
            ...
            acc, n)
      when level == 1 do
    alert = r_alert(level: level, description: description,
                where: %{mfa: {:ssl_alert, :decode, 3}, line: 133,
                           file: 'otp/lib/ssl/src/ssl_alert.erl'})
  ...
  defp decode(<<level :: size(8) - unsigned - big - integer,
      ...
      when level == 2 do
    alert = r_alert(level: level, description: description,
                where: %{mfa: {:ssl_alert, :decode, 3}, line: 136,
                           file: 'otp/lib/ssl/src/ssl_alert.erl'})
    :lists.reverse([alert | acc])
  end

  defp decode(<<_Level :: size(8) - unsigned - big - integer,
              _ :: binary>>,
            _, _) do
    r_alert(level: 2, description: 47,
        where: %{mfa: {:ssl_alert, :decode, 3}, line: 139,
                   file: 'otp/lib/ssl/src/ssl_alert.erl'},
