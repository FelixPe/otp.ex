defmodule :m_dtls_handshake do
  ...
  def client_hello(host, port, connectionStates, sslOpts, id,
           renegotiation) do
    client_hello(host, port, <<>>, connectionStates,
                   sslOpts, id, renegotiation, :undefined)
  end

  def client_hello(_Host, _Port, cookie, connectionStates,
           %{versions: versions, ciphers: userSuites,
               fallback: fallback} = sslOpts,
           id, renegotiation, ocspNonce) do
    version = :dtls_record.highest_protocol_version(versions)
    pending = :ssl_record.pending_connection_state(connectionStates,
                                                     :read)
    ...
    extensions = :ssl_handshake.client_hello_extensions(tLSVersion,
                                                          ...
                                                          :undefined,
                                                          :undefined)
    r_client_hello(session_id: id, client_version: version,
        cipher_suites: :ssl_handshake.cipher_suites(cipherSuites,
                                                      renegotiation, fallback),
        compression_methods: :ssl_record.compressions(),
        random: r_security_parameters(secParams, :client_random), cookie: cookie,
        extensions: extensions)
  end

  def hello(r_server_hello(server_version: version, random: random,
             cipher_suite: cipherSuite,
             compression_method: compression, session_id: sessionId,
             extensions: helloExt),
           %{versions: supportedVersions} = sslOpt,
           connectionStates0, renegotiation, oldId) do
    isNew = :ssl_session.is_new(oldId, sessionId)
    case (:dtls_record.is_acceptable_version(version,
                                               supportedVersions)) do
      true ->
        handle_server_hello_extensions(version, sessionId,
                                         random, cipherSuite, compression,
                                         helloExt, sslOpt, connectionStates0,
                                         renegotiation, isNew)
      false ->
        throw(r_alert(level: 2, description: 70,
                  where: %{mfa: {:dtls_handshake, :hello, 5}, line: 113,
                             file: 'otp/lib/ssl/src/dtls_handshake.erl'}))
    end
  end

  def hello(r_client_hello(client_version: clientVersion) = hello,
           %{versions: versions} = sslOpts, info, renegotiation) do
    version = :ssl_handshake.select_version(:dtls_record,
                                              clientVersion, versions)
    handle_client_hello(version, hello, sslOpts, info,
                          renegotiation)
  end

  def cookie(key, address, port,
           r_client_hello(client_version: version, random: random,
               session_id: sessionId, cipher_suites: cipherSuites,
               compression_methods: compressionMethods)) do
    {major, minor} = version
    cookieData = [address_to_bin(address, port), <<major
                                                   ::
                                                   size(8) - unsigned - big -
                                                     integer,
                                                     minor
  ...
  def fragment_handshake([msgType, len, seq, _, len, bin], size) do
    bins = bin_fragments(bin, size - 26)
  ...
  def encode_handshake(handshake, version, seq) do
    ...
    [msgType, <<len
                ...
                size(24) - unsigned - big - integer>>,
                  <<seq :: size(16) - unsigned - big - integer>>, <<0
                                                                    ::
                                                                    size(24) -
                                                                      unsigned -
                                                                      big -
  ...
  defp handle_client_hello(version,
            r_client_hello(session_id: sugesstedId, cipher_suites: cipherSuites,
                compression_methods: compressions, random: random,
                extensions: helloExt),
            %{versions: versions,
                signature_algs: supportedHashSigns, eccs: supportedECCs,
                honor_ecc_order: eCCOrder} = sslOpts,
            {sessIdTracker, session0, connectionStates0,
               certKeyPairs, _},
            renegotiation) do
    case (:dtls_record.is_acceptable_version(version,
                                               versions)) do
      true ->
        curves = :maps.get(:elliptic_curves, helloExt,
                             :undefined)
        clientHashSigns = :maps.get(:signature_algs, helloExt,
                                      :undefined)
        tLSVersion = :dtls_v1.corresponding_tls_version(version)
        availableHashSigns = :ssl_handshake.available_signature_algs(clientHashSigns,
                                                                       supportedHashSigns,
                                                                       tLSVersion)
        eCCCurve = :ssl_handshake.select_curve(curves,
                                                 supportedECCs, eCCOrder)
        {type,
           r_session(cipher_suite: cipherSuite,
               own_certificates: [ownCert |
                                      _]) = session1} = :ssl_handshake.select_session(sugesstedId,
                                                                                        cipherSuites,
                                                                                        availableHashSigns,
                                                                                        compressions,
                                                                                        sessIdTracker,
            ...
            throw(r_alert(level: 2, description: 71,
                      where: %{mfa:
                               {:dtls_handshake, :handle_client_hello, 5},
                                 line: 202, file: 'otp/lib/ssl/src/dtls_handshake.erl'}))
          _ ->
            %{key_exchange:
              keyExAlg} = :ssl_cipher_format.suite_bin_to_map(cipherSuite)
            case (:ssl_handshake.select_hashsign({clientHashSigns,
                                                    :undefined},
                                                   ownCert, keyExAlg,
                                                   supportedHashSigns,
                                                   tLSVersion)) do
      ...
      false ->
        throw(r_alert(level: 2, description: 70,
                  where: %{mfa:
                           {:dtls_handshake, :handle_client_hello, 5},
                             line: 216, file: 'otp/lib/ssl/src/dtls_handshake.erl'}))
    end
  end

  defp handle_client_hello_extensions(version, type, random, cipherSuites, helloExt,
            sslOpts, session0, connectionStates0, renegotiation,
            hashSign) do
    {session, connectionStates, protocol,
       serverHelloExt} = :ssl_handshake.handle_client_hello_extensions(:dtls_record,
                                                                         random,
                                                                         cipherSuites,
                                                                         helloExt,
  ...
  defp handle_server_hello_extensions(version, sessionId, random, cipherSuite,
            compression, helloExt, sslOpt, connectionStates0,
            renegotiation, isNew) do
    {connectionStates, protoExt, protocol,
       ocspState} = :ssl_handshake.handle_server_hello_extensions(:dtls_record,
                                                                    random,
                                                                    cipherSuite,
                                                                    compression,
                                                                    helloExt,
                                                                    ...
                                                                    renegotiation,
                                                                    isNew)
    {version, sessionId, connectionStates, protoExt,
       protocol, ocspState}
  end

  defp enc_handshake(r_hello_verify_request(protocol_version: version, cookie: cookie),
            _Version) do
    ...
    {3,
       <<major :: size(8) - unsigned - big - integer,
           ...
           cookieLength :: size(8) - unsigned - big - integer,
           cookie :: size(cookieLength) - binary>>}
  end
  ...
  defp enc_handshake(r_client_hello(client_version: clientVersion, random: random,
              session_id: sessionID, cookie: cookie,
    ...
    {1,
       <<major :: size(8) - unsigned - big - integer,
           ...
           sIDLength :: size(8) - unsigned - big - integer,
           sessionID :: binary,
           cookieLength :: size(8) - unsigned - big - integer,
           cookie :: binary,
           csLength :: size(16) - unsigned - big - integer,
           binCipherSuites :: binary,
           cmLength :: size(8) - unsigned - big - integer,
           binCompMethods :: binary, extensionsBin :: binary>>}
  end
  ...
  defp enc_handshake(r_server_hello() = handshakeMsg, version) do
    {type,
       <<major :: size(8) - unsigned - big - integer,
           ...
           rest
           ::
           binary>>} = :ssl_handshake.encode_handshake(handshakeMsg,
                                                         version)
    {dTLSMajor,
       dTLSMinor} = :dtls_v1.corresponding_dtls_version({major,
                                                           minor})
    {type,
       <<dTLSMajor :: size(8) - unsigned - big - integer,
           dTLSMinor :: size(8) - unsigned - big - integer,
           rest :: binary>>}
  end
  ...
  defp handshake_bin(r_handshake_fragment(type: type, length: len, message_seq: seq,
              fragment_length: len, fragment_offset: 0,
  ...
  defp handshake_bin(type, length, seq, fragmentData) do
    <<type :: size(8) - unsigned - big - integer,
        ...
        length :: size(24) - unsigned - big - integer,
        fragmentData :: size(length) - binary>>
  end
  ...
  defp bin_fragments(bin, binSize, fragSize, offset, fragments) do
    case (binSize - offset - fragSize > 0) do
      true ->
        ...
        bin_fragments(bin, binSize, fragSize, offset + fragSize,
                        [{frag, offset} | fragments])
      false ->
  ...
  defp handshake_fragments(msgType, seq, len, [{bin, offset} | bins],
            acc) do
    fragLen = byte_size(bin)
    handshake_fragments(msgType, seq, len, bins,
                          [<<msgType :: size(8) - unsigned - big - integer,
                               len :: binary, seq :: binary,
                               offset :: size(24) - unsigned - big - integer,
                               fragLen :: size(24) - unsigned - big - integer,
                               bin :: binary>> |
                               acc])
  end

  defp address_to_bin({a, b, c, d}, port) do
    <<0 :: size(80), 65535 :: size(16), a, b, c, d,
        port :: size(16)>>
  end
  ...
  defp address_to_bin({a, b, c, d, e, f, g, h}, port) do
    <<a :: size(16), b :: size(16), c :: size(16),
        ...
        g :: size(16), h :: size(16), port :: size(16)>>
  end

  defp handle_fragments(version, fragmentData, buffers0, options,
            acc) do
    fragments = decode_handshake_fragments(fragmentData)
    do_handle_fragments(version, fragments, buffers0,
                          options, acc)
  end
  ...
  defp do_handle_fragments(version, [fragment | fragments], buffers0,
            %{log_level: logLevel} = options, acc) do
    case (reassemble(version, fragment, buffers0)) do
      {:more_data, buffers} when fragments == [] ->
        {:lists.reverse(acc), buffers}
      {:more_data, buffers} ->
        do_handle_fragments(version, fragments, buffers,
                              options, acc)
      {{handshake, _} = hsPacket, buffers} ->
        :ssl_logger.debug(logLevel, :inbound, :handshake,
                            handshake)
        do_handle_fragments(version, fragments, buffers,
                              options, [hsPacket | acc])
    end
  end

  defp decode_handshake(version,
            <<type :: size(8) - unsigned - big - integer,
                bin :: binary>>) do
  ...
  defp decode_handshake(version, 1,
            <<_ :: size(24) - unsigned - big - integer,
                ...
                cookieLength :: size(8) - unsigned - big - integer,
                cookie :: size(cookieLength) - binary,
                cs_length :: size(16) - unsigned - big - integer,
                cipherSuites :: size(cs_length) - binary,
                cm_length :: size(8) - unsigned - big - integer,
                comp_methods :: size(cm_length) - binary,
                extensions :: binary>>) do
    tLSVersion = :dtls_v1.corresponding_tls_version(version)
    legacyVersion = :dtls_v1.corresponding_tls_version({major,
                                                          minor})
    exts = :ssl_handshake.decode_vector(extensions)
    decodedExtensions = :ssl_handshake.decode_hello_extensions(exts,
                                                                 tLSVersion,
                                                                 legacyVersion,
                                                                 :client)
    r_client_hello(client_version: {major, minor}, random: random,
        cookie: cookie, session_id: session_ID,
        cipher_suites: :ssl_handshake.decode_suites(:"2_bytes",
                                                      cipherSuites),
        compression_methods: comp_methods,
        extensions: decodedExtensions)
  end

  defp decode_handshake(_Version, 3,
            <<_ :: size(24) - unsigned - big - integer,
                _ :: size(16) - unsigned - big - integer,
                _ :: size(24) - unsigned - big - integer,
                _ :: size(24) - unsigned - big - integer,
                ...
                minor :: size(8) - unsigned - big - integer,
                cookieLength :: size(8) - unsigned - big - integer,
                cookie :: size(cookieLength) - binary>>) do
    r_hello_verify_request(protocol_version: {major, minor}, cookie: cookie)
  end

  defp decode_handshake(version, tag,
            <<_ :: size(24) - unsigned - big - integer,
                _ :: size(16) - unsigned - big - integer,
                _ :: size(24) - unsigned - big - integer,
                _ :: size(24) - unsigned - big - integer,
  ...
  defp decode_handshake_fragments(<<type :: size(8) - unsigned - big - integer,
              length :: size(24) - unsigned - big - integer,
              messageSeq :: size(16) - unsigned - big - integer,
              fragmentOffset :: size(24) - unsigned - big - integer,
              fragmentLength :: size(24) - unsigned - big - integer,
              fragment :: size(fragmentLength) - binary,
              rest :: binary>>) do
    [r_handshake_fragment(type: type, length: length, message_seq: messageSeq,
         fragment_offset: fragmentOffset,
         fragment_length: fragmentLength, fragment: fragment) |
         decode_handshake_fragments(rest)]
  end

  defp reassemble(version, r_handshake_fragment(message_seq: seq) = fragment,
            r_protocol_buffers(dtls_handshake_next_seq: seq,
                dtls_handshake_next_fragments: fragments0,
                dtls_handshake_later_fragments: laterFragments0) = buffers0) do
    case (reassemble_fragments(fragment, fragments0)) do
      {:more_data, fragments} ->
        {:more_data,
           r_protocol_buffers(buffers0, dtls_handshake_next_fragments: fragments)}
      {:raw, rawHandshake} ->
        handshake = decode_handshake(version, rawHandshake)
        {nextFragments,
           laterFragments} = next_fragments(laterFragments0)
        {{handshake, rawHandshake},
           r_protocol_buffers(buffers0, dtls_handshake_next_seq: seq + 1, 
                         dtls_handshake_next_fragments: nextFragments, 
                         dtls_handshake_later_fragments: laterFragments)}
    end
  end

  defp reassemble(_, r_handshake_fragment(message_seq: fragSeq) = fragment,
            r_protocol_buffers(dtls_handshake_next_seq: seq,
                dtls_handshake_later_fragments: laterFragments) = buffers0)
      when fragSeq > seq do
    {:more_data,
       r_protocol_buffers(buffers0, dtls_handshake_later_fragments: [fragment |
                                                        laterFragments])}
  end
  ...
  defp reassemble_fragments(current, fragments0) do
    [frag1 | frags] = :lists.keysort(r_handshake_fragment(:fragment_offset),
                                       [current | fragments0])
    [fragment | _] = (fragments = merge_fragment(frag1,
                                                   frags))
  ...
  defp merge_fragments(r_handshake_fragment(fragment_offset: previousOffSet,
              fragment_length: previousLen,
              fragment: previousData) = previous,
            r_handshake_fragment(fragment_offset: previousOffSet,
                fragment_length: previousLen,
                fragment: previousData)) do
    previous
  end

  defp merge_fragments(r_handshake_fragment(fragment_offset: previousOffset,
              fragment_length: previousLen) = previous,
            r_handshake_fragment(fragment_offset: currentOffset,
                fragment_length: currentLen))
      when previousOffset <= currentOffset and currentOffset <= previousOffset + previousLen and currentOffset + currentLen <= previousOffset + previousLen do
    previous
  end
  ...
  defp merge_fragments(r_handshake_fragment(fragment_offset: previousOffset,
      ...
      when currentOffset <= previousOffset and currentOffset + currentLen >= previousOffset + previousLen do
    current
  end
  ...
  defp merge_fragments(r_handshake_fragment(fragment_offset: previousOffset,
              ...
              fragment: previousData) = previous,
            r_handshake_fragment(fragment_offset: currentOffset,
                fragment_length: currentLen, fragment: currentData))
      when currentOffset < previousOffset and currentOffset + currentLen < previousOffset + previousLen do
    newDataLen = previousOffset - currentOffset
    <<newData :: size(newDataLen) - binary,
        _ :: binary>> = currentData
    r_handshake_fragment(previous, fragment_length: previousLen + newDataLen, 
  ...
  defp merge_fragments(r_handshake_fragment(fragment_offset: previousOffset,
    ...
    <<_ :: size(dropLen) - binary,
        newData :: binary>> = currentData
    r_handshake_fragment(previous, fragment_length: previousLen + newDataLen, 
  ...
  defp merge_fragments(r_handshake_fragment(fragment_offset: previousOffset,
    ...
    r_handshake_fragment(previous, fragment_length: previousLen + currentLen, 
                  fragment: <<previousData :: binary,
                                currentData :: binary>>)
  end
  ...
  defp next_fragments(laterFragments) do
    case (:lists.keysort(r_handshake_fragment(:message_seq),
                           laterFragments)) do
      [] ->
        {[], []}
  ...
  defp split_frags([r_handshake_fragment(message_seq: seq) = frag | rest], seq,
            acc) do
