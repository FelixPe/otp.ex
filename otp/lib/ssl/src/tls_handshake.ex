defmodule :m_tls_handshake do
  ...
  def client_hello(_Host, _Port, connectionStates,
    ...
    legacyVersion = (case (:tls_record.is_higher(version,
                                                   {3, 2})) do
                       true ->
                         {3, 3}
                       false ->
    ...
    extensions = :ssl_handshake.client_hello_extensions(version,
                                                          ...
                                                          certDbHandle,
                                                          certDbRef)
    cipherSuites = :ssl_handshake.cipher_suites(availableCipherSuites,
                                                  renegotiation, fallback)
    r_client_hello(session_id: id, client_version: legacyVersion,
  ...
  def hello(r_server_hello(server_version: {major, minor},
             random: <<_ :: size(24) - binary,
                         down :: size(8) - binary>>),
           %{versions: [{m, n} | _]}, _, _, _)
      when (m > 3 or m === 3 and n >= 4) and major === 3 and minor === 3 and down === <<68,
                                                                                          79,
                                                                                          87,
                                                                                          78,
                                                                                          71,
                                                                                          82,
                                                                                          68,
                                                                                          1>> or (m > 3 or m === 3 and n >= 4) and major === 3 and minor < 3 and down === <<68,
                                                                                                                                                                              79,
                                                                                                                                                                              87,
                                                                                                                                                                              78,
                                                                                                                                                                              71,
    ...
    throw(r_alert(level: 2, description: 47,
              where: %{mfa: {:tls_handshake, :hello, 5}, line: 132,
  ...
  def hello(r_server_hello(server_version: {major, minor},
             random: <<_ :: size(24) - binary,
                         down :: size(8) - binary>>),
           %{versions: [{m, n} | _]}, _, _, _)
      when (m === 3 and n === 3) and major === 3 and minor < 3 and down === <<68,
                                                                                79,
                                                                                87,
                                                                                78,
                                                                                71,
                                                                                ...
                                                                                0>> do
    throw(r_alert(level: 2, description: 47,
              where: %{mfa: {:tls_handshake, :hello, 5}, line: 142,
                         file: 'otp/lib/ssl/src/tls_handshake.erl'}))
  end

  def hello(r_server_hello(server_version: legacyVersion, random: random,
    ...
    stapling = :maps.get(:ocsp_stapling, sslOpt, false)
    case (legacyVersion < {3, 3} or legacyVersion === {3,
                                                         3} and version < {3,
                                                                             3}) do
      true ->
        throw(r_alert(level: 2, description: 47,
                  where: %{mfa: {:tls_handshake, :hello, 5}, line: 174,
                             file: 'otp/lib/ssl/src/tls_handshake.erl'}))
      false ->
        case (:tls_record.is_acceptable_version(version,
          ...
          false ->
            throw(r_alert(level: 2, description: 47,
                      where: %{mfa: {:tls_handshake, :hello, 5}, line: 192,
                                 file: 'otp/lib/ssl/src/tls_handshake.erl'}))
        end
    end
  end
    ...
    case (:tls_record.is_acceptable_version(version,
      ...
      false ->
        throw(r_alert(level: 2, description: 70,
                  where: %{mfa: {:tls_handshake, :hello, 5}, line: 211,
                             file: 'otp/lib/ssl/src/tls_handshake.erl'}))
    end
  end

  def hello(r_client_hello(client_version: _ClientVersion,
             cipher_suites: cipherSuites,
    ...
    try do
      version = :ssl_handshake.select_supported_version(clientVersions,
                                                          versions)
      do_hello(version, versions, cipherSuites, hello,
      ...
      :error, reason ->
                ...
                :undefined ->
                  :ssl_logger.log(:info, :debug,
                                    ...
                                    %{mfa: {:tls_handshake, :hello, 4},
                                        line: 252, file: 'otp/lib/ssl/src/tls_handshake.erl'})
                __LogLevel__ ->
        ...
        throw(r_alert(level: 2, description: 40,
                  where: %{mfa: {:tls_handshake, :hello, 4}, line: 253,
                             file: 'otp/lib/ssl/src/tls_handshake.erl'},
                  reason: :malformed_handshake_data))
    end
  end

  def hello(r_client_hello(client_version: clientVersion,
             cipher_suites: cipherSuites) = hello,
           %{versions: versions} = sslOpts, info, renegotiation) do
    try do
      version = :ssl_handshake.select_version(:tls_record,
                                                clientVersion, versions)
      do_hello(version, versions, cipherSuites, hello,
                 sslOpts, info, renegotiation)
    catch
      :error, {:case_clause, {:asn1, asn1Reason}} ->
        throw(r_alert(level: 2, description: 80,
                  where: %{mfa: {:tls_handshake, :hello, 4}, line: 266,
                             file: 'otp/lib/ssl/src/tls_handshake.erl'},
                  reason: {:failed_to_decode_own_certificate,
                             asn1Reason}))
      :error, reason ->
        (fn () ->
              case (:erlang.get(:log_level)) do
                  ...
                  :ssl_logger.log(:info, :debug,
                                    %{description: :handshake_error,
                                        ...
                                        [{:reason, reason}, {:stacktrace,
                                                               __STACKTRACE__}]},
                                    %{mfa: {:tls_handshake, :hello, 4},
                                        line: 268, file: 'otp/lib/ssl/src/tls_handshake.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:info, __LogLevel__,
                                    %{description: :handshake_error,
                                        reason:
        ...
        throw(r_alert(level: 2, description: 40,
                  where: %{mfa: {:tls_handshake, :hello, 4}, line: 269,
                             file: 'otp/lib/ssl/src/tls_handshake.erl'},
                  reason: :malformed_handshake_data))
    end
  end
  ...
  def get_tls_handshakes(version, data, <<>>, options) do
    get_tls_handshakes_aux(version, data, options, [])
  end

  def get_tls_handshakes(version, data, buffer, options) do
    get_tls_handshakes_aux(version,
                             :erlang.list_to_binary([buffer, data]), options,
                             [])
  end

  def ocsp_nonce(sslOpts) do
    case (:maps.get(:ocsp_stapling, sslOpts, :disabled)) do
      %{ocsp_nonce: true} ->
        :public_key.der_encode(:Nonce,
  ...
  defp handle_client_hello(version,
            r_client_hello(session_id: sugesstedId, cipher_suites: cipherSuites,
                ...
                extensions: helloExt),
            %{versions: versions, eccs: supportedECCs,
    ...
    case (:tls_record.is_acceptable_version(version,
                                              versions)) do
      true ->
        supportedHashSigns = supported_hashsigns(:maps.get(:signature_algs,
                                                             sslOpts,
                                                             :undefined))
        curves = :maps.get(:elliptic_curves, helloExt,
                             :undefined)
        clientHashSigns = get_signature_ext(:signature_algs,
                                              helloExt, version)
        clientSignatureSchemes = get_signature_ext(:signature_algs_cert,
                                                     helloExt, version)
        ...
        case (cipherSuite) do
          :no_suite ->
            throw(r_alert(level: 2, description: 71,
                      where: %{mfa: {:tls_handshake, :handle_client_hello, 5},
                                 line: 354, file: 'otp/lib/ssl/src/tls_handshake.erl'},
      ...
      false ->
        throw(r_alert(level: 2, description: 70,
                  where: %{mfa: {:tls_handshake, :handle_client_hello, 5},
                             line: 372, file: 'otp/lib/ssl/src/tls_handshake.erl'}))
    end
  end

  defp supported_hashsigns(:undefined) do
    :undefined
  end

  defp supported_hashsigns(sigAlgs) do
  ...
  defp handle_client_hello_extensions(version, type, random, cipherSuites, helloExt,
    ...
    {session, connectionStates, protocol,
       serverHelloExt} = :ssl_handshake.handle_client_hello_extensions(:tls_record,
                                                                         random,
                                                                         cipherSuites,
                                                                         helloExt,
                                                                         version,
  ...
  defp handle_server_hello_extensions(version, sessionId, random, cipherSuite,
    ...
    {connectionStates, protoExt, protocol,
       ocspState} = :ssl_handshake.handle_server_hello_extensions(:tls_record,
                                                                    random,
                                                                    cipherSuite,
                                                                    compression,
                                                                    helloExt,
  ...
  defp do_hello(:undefined, _Versions, _CipherSuites, _Hello,
            _SslOpts, _Info, _Renegotiation) do
    throw(r_alert(level: 2, description: 70,
              where: %{mfa: {:tls_handshake, :do_hello, 7}, line: 401,
                         file: 'otp/lib/ssl/src/tls_handshake.erl'}))
  end

  defp do_hello(version, versions, cipherSuites, hello, sslOpts,
            info, renegotiation) do
    case (:ssl_cipher.is_fallback(cipherSuites)) do
      true ->
        ...
        case (:tls_record.is_higher(highest, version)) do
          true ->
            throw(r_alert(level: 2, description: 86,
                      where: %{mfa: {:tls_handshake, :do_hello, 7}, line: 408,
                                 file: 'otp/lib/ssl/src/tls_handshake.erl'}))
          false ->
  ...
  defp enc_handshake(r_hello_request(), version) when version < {3, 4} do
    {0, <<>>}
  end

  defp enc_handshake(r_client_hello(client_version: serverVersion, random: random,
    ...
    try do
      ...
      :error, reason ->
                  ...
                  :ssl_logger.log(:info, :debug,
                                    %{description: :handshake_error,
                                        ...
                                        [{:reason, reason}, {:stacktrace,
                                                               __STACKTRACE__}]},
                                    %{mfa:
                                      {:tls_handshake, :get_tls_handshakes_aux,
                                         4},
                                        line: 453, file: 'otp/lib/ssl/src/tls_handshake.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:info, __LogLevel__,
                                    %{description: :handshake_error,
                                        reason:
        ...
        throw(r_alert(level: 2, description: 50,
                  where: %{mfa:
                           {:tls_handshake, :get_tls_handshakes_aux, 4},
                             line: 454, file: 'otp/lib/ssl/src/tls_handshake.erl'},
                  reason: :handshake_decode_error))
    else
      handshake ->
        :ssl_logger.debug(logLevel, :inbound, :handshake,
                            handshake)
        get_tls_handshakes_aux(version, rest, opts,
                                 [{handshake, raw} | acc])
    end
  end

  defp get_tls_handshakes_aux(_Version, data, _, acc) do
    {:lists.reverse(acc), data}
  end

  def decode_handshake(version, 0, <<>>) when version < {3, 4} do
    r_hello_request()
  end

  def decode_handshake(version, 1,
           <<major :: size(8) - unsigned - big - integer,
               minor :: size(8) - unsigned - big - integer,
               random :: size(32) - binary,
  ...
  defp get_signature_ext(ext, helloExt, {3, 3}) do
    case (:maps.get(ext, helloExt, :undefined)) do
      :undefined ->
        :undefined
      r_signature_algorithms(signature_scheme_list: schemes) ->
        r_hash_sign_algos(hash_sign_algos: :ssl_cipher.signature_schemes_1_2(schemes))
      r_signature_algorithms_cert() = algos ->
        algos
      r_hash_sign_algos() = algos ->
        algos
    end
  end

  defp get_signature_ext(ext, helloExt, _) do
