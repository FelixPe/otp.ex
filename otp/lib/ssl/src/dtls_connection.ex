defmodule :m_dtls_connection do
  ...
  Record.defrecord(:r_state, :state, static_env: :undefined,
                                 ...
                                 log_level: :undefined)
  Record.defrecord(:r_protocol_buffers, :protocol_buffers, dtls_record_buffer: <<>>,
  ...
  Record.defrecord(:r_hello_verify_request, :hello_verify_request, protocol_version: :undefined,
                                                cookie: :undefined)
  Record.defrecord(:r_handshake_fragment, :handshake_fragment, type: :undefined,
                                              length: :undefined,
  ...
  def init([role, host, port, socket, options, user,
                                                 cbInfo]) do
    :erlang.process_flag(:trap_exit, true)
    state0 = initial_state(role, host, port, socket,
                             options, user, cbInfo)
    try do
      state = :ssl_gen_statem.init_ssl_config(r_state(state0, :ssl_options),
                                                role, state0)
      :gen_statem.enter_loop(:dtls_connection, [],
                               :initial_hello, state)
    catch
      error ->
  ...
  def renegotiate(r_state(static_env: r_static_env(role: :client)) = state0,
           actions) do
  ...
  def renegotiate(r_state(static_env: r_static_env(role: :server)) = state0,
           actions) do
    helloRequest = :ssl_handshake.hello_request()
    state1 = prepare_flight(state0)
    {state,
       moreActions} = :dtls_gen_connection.send_handshake(helloRequest,
                                                            state1)
    :dtls_gen_connection.next_event(:hello, :no_record,
                                      state, actions ++ moreActions)
  end
  ...
  def initial_hello({:call, from}, {:start, timeout},
           r_state(static_env: r_static_env(host: host, port: port, role: :client,
                             socket: {_, socket}, transport_cb: transport,
    ...
    :dtls_socket.setopts(transport, socket,
                           [{:active, packages}])
    certKeyPairs = :ssl_certificate.available_cert_key_pairs(certKeyAlts)
    session = :ssl_session.client_select_session({host,
                                                    port, sslOpts},
                                                   cache, cacheCb, session0,
                                                   certKeyPairs)
    hello = :dtls_handshake.client_hello(host, port,
                                           connectionStates0, sslOpts,
                                           r_session(session, :session_id),
                                           renegotiation)
    maxFragEnum = :maps.get(:max_frag_enum,
                              r_client_hello(hello, :extensions), :undefined)
    connectionStates1 = :ssl_record.set_max_fragment_length(maxFragEnum,
                                                              connectionStates0)
    version = r_client_hello(hello, :client_version)
    helloVersion = :dtls_record.hello_version(version,
                                                versions)
    state1 = prepare_flight(r_state(state0, connection_env: r_connection_env(cEnv, negotiated_version: version), 
                                        connection_states: connectionStates1))
    {state2,
       actions} = :dtls_gen_connection.send_handshake(hello,
    ...
    state = r_state(state2, connection_env: r_connection_env(cEnv, negotiated_version: version), 
                        session: session,  start_or_recv_from: from, 
  ...
  def initial_hello({:call, _} = type, event,
           r_state(static_env: r_static_env(role: :server),
               protocol_specific: pS0) = state) do
    pS = Map.merge(pS0, %{current_cookie_secret:
                          :dtls_v1.cookie_secret(),
                            previous_cookie_secret: <<>>})
    result = :ssl_gen_statem.initial_hello(type, event,
                                             r_state(state, protocol_specific: pS))
    :erlang.send_after(:dtls_v1.cookie_timeout(), self(),
                         :new_cookie_secret)
    result
  end
  ...
  def hello(:enter, _,
           r_state(static_env: r_static_env(role: :server)) = state) do
    {:keep_state, state}
  end

  def hello(:enter, _,
           r_state(static_env: r_static_env(role: :client)) = state0) do
  ...
  def hello(:internal,
           ...
           r_state(static_env: r_static_env(role: :server, transport_cb: transport,
                             socket: socket),
               handshake_env: hsEnv, connection_env: cEnv,
               protocol_specific: %{current_cookie_secret:
                                    secret}) = state0) do
    try do
      ...
      r_state() = state1 ->
        {:ok, {iP, port}} = :dtls_socket.peername(transport,
                                                    socket)
        cookie = :dtls_handshake.cookie(secret, iP, port, hello)
        verifyRequest = :dtls_handshake.hello_verify_request(cookie,
                                                               {254, 255})
        state2 = prepare_flight(r_state(state1, connection_env: r_connection_env(cEnv, negotiated_version: version)))
        {state,
           actions} = :dtls_gen_connection.send_handshake(verifyRequest,
                                                            state2)
        :dtls_gen_connection.next_event(:hello, :no_record,
  ...
  def hello(:internal, r_hello_verify_request(cookie: cookie),
           r_state(static_env: r_static_env(role: :client, host: host, port: port),
               handshake_env: r_handshake_env(renegotiation: {renegotiation, _},
                                  ocsp_stapling_state: ocspState0) = hsEnv,
               connection_env: cEnv, ssl_options: sslOpts,
               session: r_session(session_id: id),
               connection_states: connectionStates0,
               protocol_specific: pS) = state0) do
    ocspNonce = :tls_handshake.ocsp_nonce(sslOpts)
    hello = :dtls_handshake.client_hello(host, port, cookie,
                                           connectionStates0, sslOpts, id,
                                           renegotiation, ocspNonce)
    version = r_client_hello(hello, :client_version)
    state1 = prepare_flight(r_state(state0, handshake_env: r_handshake_env(hsEnv, tls_handshake_history: :ssl_handshake.init_handshake_history(), 
                                                                ocsp_stapling_state: Map.put(ocspState0, :ocsp_nonce,
                                                                                                           ocspNonce))))
    {state2,
       actions} = :dtls_gen_connection.send_handshake(hello,
                                                        state1)
    state = r_state(state2, connection_env: r_connection_env(cEnv, negotiated_version: version), 
                        protocol_specific: Map.put(pS, :current_cookie_secret,
                                                         cookie))
    :dtls_gen_connection.next_event(:hello, :no_record,
                                      state, actions)
  end

  def hello(:internal, r_client_hello(extensions: extensions) = hello,
           r_state(handshake_env: r_handshake_env(continue_status: :pause),
               start_or_recv_from: from) = state0) do
    try do
      :tls_dtls_connection.handle_sni_extension(state0, hello)
    catch
      r_alert() = alert ->
  ...
  def hello(:internal, r_client_hello(cookie: cookie) = hello,
           r_state(static_env: r_static_env(role: :server, transport_cb: transport,
                             socket: socket),
    ...
    {:ok, {iP, port}} = :dtls_socket.peername(transport,
                                                socket)
    case (:dtls_handshake.cookie(secret, iP, port,
                                   hello)) do
      ^cookie ->
        handle_client_hello(hello, state)
      _ ->
        case (:dtls_handshake.cookie(pSecret, iP, port,
                                       hello)) do
          ^cookie ->
            handle_client_hello(hello, state)
          _ ->
  ...
  def hello(:internal, r_server_hello(extensions: extensions),
           r_state(handshake_env: r_handshake_env(continue_status: :pause),
  ...
  def hello(:internal, r_server_hello() = hello,
           r_state(static_env: r_static_env(role: :client),
               handshake_env: r_handshake_env(renegotiation: {renegotiation, _},
                                  ocsp_stapling_state: ocspState0) = hsEnv,
               connection_states: connectionStates0,
               session: r_session(session_id: oldId),
               ssl_options: sslOptions) = state) do
    try do
      {version, newId, connectionStates, protoExt, protocol,
         ocspState} = :dtls_handshake.hello(hello, sslOptions,
                                              connectionStates0, renegotiation,
                                              oldId)
      :tls_dtls_connection.handle_session(hello, version,
                                            newId, connectionStates, protoExt,
                                            protocol,
                                            r_state(state, handshake_env: r_handshake_env(hsEnv, ocsp_stapling_state: :maps.merge(ocspState0,
                                                                                                                ocspState))))
    catch
      r_alert() = alert ->
  ...
  def hello(:internal, {:handshake, {r_hello_verify_request() = handshake, _}},
           state) do
    {:next_state, :hello, state,
       [{:next_event, :internal, handshake}]}
  end
  ...
  def hello(:internal, r_change_cipher_spec(type: <<1>>), state0) do
    {state1,
       actions0} = :dtls_gen_connection.send_handshake_flight(state0,
                                                                retransmit_epoch(:hello,
                                                                                   state0))
    {:next_state, :hello, state,
       actions} = :dtls_gen_connection.next_event(:hello,
                                                    :no_record, state1,
                                                    actions0)
  ...
  def abbreviated(:internal = type, r_change_cipher_spec(type: <<1>>) = event,
           r_state(connection_states: connectionStates0) = state) do
    connectionStates1 = :dtls_record.save_current_connection_state(connectionStates0,
                                                                     :read)
    connectionStates = :dtls_record.next_epoch(connectionStates1,
                                                 :read)
    gen_handshake(:abbreviated, type, event,
                    r_state(state, connection_states: connectionStates))
  end

  def abbreviated(:internal = type, r_finished() = event,
           r_state(connection_states: connectionStates,
               protocol_specific: pS) = state) do
    gen_handshake(:abbreviated, type, event,
                    prepare_flight(r_state(state, connection_states: connectionStates, 
                                              protocol_specific: Map.put(pS, :flight_state,
                                                                               :connection))))
  end
  ...
  def certify(:internal = type, r_server_hello_done() = event, state) do
    gen_handshake(:certify, type, event,
                    prepare_flight(state))
  end

  def certify(:internal, r_change_cipher_spec(type: <<1>>), state0) do
    {state1,
       actions0} = :dtls_gen_connection.send_handshake_flight(state0,
                                                                retransmit_epoch(:certify,
                                                                                   state0))
    {:next_state, :certify, state,
       actions} = :dtls_gen_connection.next_event(:certify,
                                                    :no_record, state1,
                                                    actions0)
    {:repeat_state, state, actions}
  end
  ...
  def cipher(:internal = type, r_change_cipher_spec(type: <<1>>) = event,
           r_state(connection_states: connectionStates0) = state) do
    connectionStates1 = :dtls_record.save_current_connection_state(connectionStates0,
                                                                     :read)
    connectionStates = :dtls_record.next_epoch(connectionStates1,
                                                 :read)
    gen_handshake(:cipher, type, event,
                    r_state(state, connection_states: connectionStates))
  end

  def cipher(:internal = type, r_finished() = event,
           r_state(connection_states: connectionStates,
               protocol_specific: pS) = state) do
    gen_handshake(:cipher, type, event,
                    prepare_flight(r_state(state, connection_states: connectionStates, 
  ...
  def connection(:enter, _,
           r_state(connection_states: cs0, static_env: env) = state0) do
    state = (case (env) do
  ...
  def connection(:internal, r_hello_request(),
           r_state(static_env: r_static_env(host: host, port: port,
                             data_tag: dataTag, session_cache: cache,
    ...
    {state2,
       actions} = :dtls_gen_connection.send_handshake(hello,
                                                        r_state(state1, connection_env: r_connection_env(cEnv, negotiated_version: helloVersion)))
    state = r_state(state2, protocol_specific: Map.put(pS, :flight_state,
                                                       :dtls_gen_connection.initial_flight_state(dataTag)), 
                        session: session)
    :dtls_gen_connection.next_event(:hello, :no_record,
                                      state, actions)
  end

  def connection(:internal, r_client_hello() = hello,
           r_state(static_env: r_static_env(role: :server),
               handshake_env: r_handshake_env(allow_renegotiate: true) = hsEnv) = state) do
  ...
  def connection(:internal, r_client_hello(),
           r_state(static_env: r_static_env(role: :server, protocol_cb: connection),
               handshake_env: r_handshake_env(allow_renegotiate: false)) = state0) do
    alert = r_alert(level: 1, description: 100,
                where: %{mfa: {:dtls_connection, :connection, 3},
                           line: 574, file: 'otp/lib/ssl/src/dtls_connection.erl'})
    state1 = :dtls_gen_connection.send_alert(alert, state0)
    {record,
       state} = :ssl_gen_statem.prepare_connection(state1,
                                                     connection)
    :dtls_gen_connection.next_event(:connection, record,
                                      state)
  end

  def connection(:internal, :new_connection,
           r_state(ssl_options: sSLOptions, handshake_env: hsEnv,
               static_env: r_static_env(socket: {listener, {client, _}}),
               connection_states: oldCs) = state) do
    case (:maps.get(:previous_cs, oldCs, :undefined)) do
      :undefined ->
        case (:dtls_packet_demux.new_connection(listener,
                                                  client)) do
          true ->
            {:keep_state, state}
          false ->
            beastMitigation = :maps.get(:beast_mitigation,
                                          sSLOptions, :disabled)
            connectionStates0 = :dtls_record.init_connection_states(:server,
                                                                      beastMitigation)
            connectionStates = Map.put(connectionStates0, :previous_cs,
                                                            oldCs)
            {:next_state, :hello,
               r_state(state, handshake_env: r_handshake_env(hsEnv, renegotiation: {false,
                                                                  :first}), 
  ...
  def connection({:call, from}, {:application_data, data},
           state) do
    try do
  ...
  def connection({:call, from}, {:downgrade, pid},
           r_state(connection_env: cEnv,
               static_env: r_static_env(transport_cb: transport,
                               socket: {_Server,
                                          socket} = dTLSSocket)) = state) do
    :dtls_socket.setopts(transport, socket,
                           [{:active, false}, {:packet, 0}, {:mode, :binary}])
    transport.controlling_process(socket, pid)
    {:stop_and_reply, {:shutdown, :normal},
       {:reply, from, {:ok, dTLSSocket}},
  ...
  def downgrade(type, event, state) do
    try do
      ...
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert, :downgrade,
                                           state)
    end
  end
  ...
  defp initial_state(role, host, port, socket,
    ...
    beastMitigation = :maps.get(:beast_mitigation,
                                  sSLOptions, :disabled)
    connectionStates = :dtls_record.init_connection_states(role,
                                                             beastMitigation)
    %{session_cb:
      sessionCacheCb} = :ssl_config.pre_1_3_session_opts(role)
    internalActiveN = :ssl_config.get_internal_active_n()
    monitor = :erlang.monitor(:process, user)
    initStatEnv = r_static_env(role: role, transport_cb: cbModule,
                      protocol_cb: :dtls_gen_connection, data_tag: dataTag,
                      close_tag: closeTag, error_tag: errorTag,
                      passive_tag: passiveTag, host: host, port: port,
                      socket: socket, session_cache_cb: sessionCacheCb,
                      trackers: trackers)
    r_state(static_env: initStatEnv,
        handshake_env: r_handshake_env(tls_handshake_history: :ssl_handshake.init_handshake_history(),
                           ...
                           allow_renegotiate: :maps.get(:client_renegotiation,
                                                          sSLOptions,
                                                          :undefined)),
  ...
  defp handle_client_hello(r_client_hello(client_version: clientVersion) = hello,
            state0) do
    try do
      r_state(connection_states: connectionStates0,
          static_env: r_static_env(trackers: trackers),
          handshake_env: r_handshake_env(kex_algorithm: keyExAlg,
                             renegotiation: {renegotiation, _},
                             negotiated_protocol: currentProtocol) = hsEnv,
          connection_env: r_connection_env(cert_key_alts: certKeyAlts) = cEnv,
          session: session0,
          ssl_options: sslOpts) = :tls_dtls_connection.handle_sni_extension(state0,
                                                                              hello)
      sessionTracker = :proplists.get_value(:session_id_tracker,
                                              trackers)
      {version, {type, session}, connectionStates, protocol0,
         serverHelloExt, hashSign} = :dtls_handshake.hello(hello,
                                                             sslOpts,
                                                             {sessionTracker,
                                                                session0,
                                                                connectionStates0,
                                                                ...
                                                                keyExAlg},
                                                             renegotiation)
      protocol = (case (protocol0) do
                    :undefined ->
                      currentProtocol
                    _ ->
                      protocol0
                  end)
      state = prepare_flight(r_state(state0, connection_states: connectionStates, 
  ...
  defp handle_state_timeout(:flight_retransmission_timeout, stateName,
            r_state(protocol_specific: %{flight_state:
                                   {:retransmit, currentTimeout}}) = state0) do
    {state1,
       actions0} = :dtls_gen_connection.send_handshake_flight(state0,
                                                                retransmit_epoch(stateName,
                                                                                   state0))
    {:next_state, ^stateName,
       r_state(protocol_specific: pS) = state2,
       actions} = :dtls_gen_connection.next_event(stateName,
                                                    :no_record, state1,
                                                    actions0)
    state = r_state(state2, protocol_specific: Map.put(pS, :flight_state,
                                                       {:retransmit,
                                                          new_timeout(currentTimeout)}))
    {:repeat_state, state, actions}
  end

  def alert_or_reset_connection(alert, stateName,
           r_state(connection_states: cs) = state) do
    case (:maps.get(:previous_cs, cs, :undefined)) do
      ...
      previousConn ->
        ...
        newState = r_state(state, connection_states: previousConn, 
                              handshake_env: hsEnv)
        {:next_state, :connection, newState}
    end
  end

  defp gen_handshake(_, {:call, _From}, {:application_data, _Data},
            _State) do
  ...
  defp gen_handshake(stateName, type, event, state) do
    try do
      ...
      :error, reason ->
                  ...
                  :ssl_logger.log(:info, :debug,
                                    %{description: :handshake_error,
                                        ...
                                        [{:error, reason}, {:stacktrace,
                                                              __STACKTRACE__}]},
                                    %{mfa:
                                      {:dtls_connection, :gen_handshake, 4},
                                        line: 770, file: 'otp/lib/ssl/src/dtls_connection.erl'})
                __LogLevel__ ->
        ...
        alert = r_alert(level: 2, description: 40,
                    where: %{mfa: {:dtls_connection, :gen_handshake, 4},
                               line: 771, file: 'otp/lib/ssl/src/dtls_connection.erl'},
  ...
  defp gen_info(event, :connection = stateName, state) do
    try do
      :dtls_gen_connection.handle_info(event, stateName,
                                         state)
    catch
      :error, reason ->
        (fn () ->
                ...
                :undefined ->
                  :ssl_logger.log(:info, :debug,
                                    ...
                                    %{mfa: {:dtls_connection, :gen_info, 3},
                                        line: 778, file: 'otp/lib/ssl/src/dtls_connection.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:info, __LogLevel__,
                                    %{description: :internal_error,
                                        reason:
        ...
        alert = r_alert(level: 2, description: 80,
                    where: %{mfa: {:dtls_connection, :gen_info, 3},
                               line: 779, file: 'otp/lib/ssl/src/dtls_connection.erl'},
  ...
  defp gen_info(event, stateName, state) do
    try do
      ...
      :error, reason ->
                  ...
                  :ssl_logger.log(:info, :debug,
                                    %{description: :handshake_error,
                                        ...
                                        [{:error, reason}, {:stacktrace,
                                                              __STACKTRACE__}]},
                                    %{mfa: {:dtls_connection, :gen_info, 3},
                                        line: 785, file: 'otp/lib/ssl/src/dtls_connection.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:info, __LogLevel__,
                                    %{description: :handshake_error,
                                        reason:
        ...
        alert = r_alert(level: 2, description: 40,
                    where: %{mfa: {:dtls_connection, :gen_info, 3},
                               line: 786, file: 'otp/lib/ssl/src/dtls_connection.erl'},
                    reason: :malformed_handshake_data)
        alert_or_reset_connection(alert, stateName, state)
    end
  end

  defp prepare_flight(r_state(flight_buffer: flight,
              connection_states: connectionStates0,
              protocol_buffers: r_protocol_buffers() = buffers) = state) do
    connectionStates = :dtls_record.save_current_connection_state(connectionStates0,
                                                                    :write)
    r_state(state, flight_buffer: next_flight(flight), 
               connection_states: connectionStates, 
               protocol_buffers: r_protocol_buffers(buffers, dtls_handshake_next_fragments: [], 
                                              dtls_handshake_later_fragments: []))
  end

  defp next_flight(flight) do
    Map.merge(flight, %{handshakes: [],
                          change_cipher_spec: :undefined,
                          handshakes_after_change_cipher_spec: []})
  end

  defp handle_flight_timer(r_state(static_env: r_static_env(data_tag: :udp),
              protocol_specific: %{flight_state:
                                   {:retransmit, timeout}}) = state) do
  ...
  defp handle_flight_timer(r_state(protocol_specific: %{flight_state:
                                 :reliable}) = state) do
    {state, []}
  end

  defp start_retransmision_timer(timeout, r_state(protocol_specific: pS) = state) do
    {r_state(state, protocol_specific: Map.put(pS, :flight_state,
                                               {:retransmit, timeout})),
  ...
  defp retransmit_epoch(_StateName,
            r_state(connection_states: connectionStates)) do
    %{epoch:
      epoch} = :ssl_record.current_connection_state(connectionStates,
                                                      :write)
    epoch
  end

  defp send_application_data(data, from, _StateName,
            r_state(static_env: r_static_env(socket: socket,
                ...
                ssl_options: %{renegotiate_at: renegotiateAt,
                                 log_level: logLevel}) = state0) do
    case (time_to_renegotiate(data, connectionStates0,
                                renegotiateAt)) do
      true ->
        renegotiate(r_state(state0, handshake_env: r_handshake_env(hsEnv, renegotiation: {true,
                                                                        :internal})),
                      [{:next_event, {:call, from},
                          {:application_data, data}}])
      false ->
        {msgs,
           connectionStates} = :dtls_record.encode_data(data,
                                                          version,
                                                          connectionStates0)
  ...
  defp send_msgs(transport, socket, [msg | msgs]) do
    case (:dtls_gen_connection.send(transport, socket,
                                      msg)) do
      :ok ->
  ...
  defp time_to_renegotiate(_Data,
