defmodule :m_ssl_gen_statem do
  ...
  Record.defrecord(:r_protocol_buffers, :protocol_buffers, tls_record_buffer: <<>>,
                                            ...
                                            tls_cipher_texts: [])
  def start_link(role, sender, host, port, socket,
           {sslOpts, _, _} = options, user, cbInfo) do
    receiverOpts = :maps.get(:receiver_spawn_opts, sslOpts,
                               [])
    opts = [:link | :proplists.delete(:link, receiverOpts)]
    pid = :proc_lib.spawn_opt(:ssl_gen_statem, :init,
                                [[role, sender, host, port, socket, options,
                                                                        user,
                                                                            cbInfo]],
                                opts)
    {:ok, pid}
  ...
  def start_link(role, host, port, socket,
           {sslOpts, _, _} = options, user, cbInfo) do
    receiverOpts = :maps.get(:receiver_spawn_opts, sslOpts,
                               [])
    opts = [:link | :proplists.delete(:link, receiverOpts)]
    pid = :proc_lib.spawn_opt(:ssl_gen_statem, :init,
                                [[role, host, port, socket, options, user,
                                                                         cbInfo]],
                                opts)
    {:ok, pid}
  end

  def init([role, _Sender, _Host, _Port, _Socket, {tLSOpts,
                                                   _, _},
                                                    _User,
                                                        _CbInfo] = initArgs) do
  ...
  def init([_Role, _Host, _Port, _Socket, _TLSOpts, _User,
                                                      _CbInfo] = initArgs) do
    :erlang.process_flag(:trap_exit, true)
    :dtls_connection.init(initArgs)
  end

  def init_ssl_config(opts, role,
           r_state(ssl_options: %{handshake: handshake},
               handshake_env: hsEnv) = state0) do
    continueStatus = (case (handshake) do
                        :hello ->
                          :pause
                        :full ->
                          handshake
  ...
  def ssl_config(opts, role,
           r_state(static_env: initStatEnv0, handshake_env: hsEnv,
               connection_env: cEnv) = state0) do
    {:ok,
       %{cert_db_ref: ref, cert_db_handle: certDbHandle,
           fileref_db_handle: fileRefHandle,
           session_cache: cacheHandle, crl_db_info: cRLDbHandle,
           cert_key_alts: certKeyAlts,
           dh_params: dHParams}} = :ssl_config.init(opts, role)
    timeStamp = :erlang.monotonic_time()
    session = r_state(state0, :session)
    r_state(state0, session: r_session(session, time_stamp: timeStamp), 
                static_env: r_static_env(initStatEnv0, file_ref_db: fileRefHandle, 
                                              cert_db_ref: ref, 
  ...
  def connect(connection, host, port, socket, options, user,
           cbInfo, timeout) do
    try do
      connection.start_fsm(:client, host, port, socket,
                             options, user, cbInfo, timeout)
    catch
      :exit, {:noproc, _} ->
  ...
  def handshake(connection, port, socket, opts, user, cbInfo,
           timeout) do
    try do
      connection.start_fsm(:server, 'localhost', port, socket, opts,
                             user, cbInfo, timeout)
    catch
      :exit, {:noproc, _} ->
        {:error, :ssl_not_started}
  ...
  def handshake(r_sslsocket(pid: [pid | _]) = socket, sslOptions,
           timeout) do
  ...
  def handshake_continue(r_sslsocket(pid: [pid | _]) = socket, sslOptions,
           timeout) do
    case (call(pid,
                 {:handshake_continue, sslOptions, timeout})) do
      :connected ->
        {:ok, socket}
      error ->
        error
  ...
  def socket_control(:dtls_gen_connection, socket, pids, transport,
           :udp_listener) do
    {:ok,
       :dtls_gen_connection.socket(pids, transport, socket,
                                     :undefined)}
  end

  def socket_control(:tls_gen_connection, socket, [pid | _] = pids,
           transport, trackers) do
    case (transport.controlling_process(socket, pid)) do
      :ok ->
        {:ok,
           :tls_gen_connection.socket(pids, transport, socket,
                                        trackers)}
      {:error, reason} ->
        {:error, reason}
    end
  ...
  def socket_control(:dtls_gen_connection, {peerAddrPort, socket},
           [pid | _] = pids, transport, trackers) do
    case (transport.controlling_process(socket, pid)) do
      :ok ->
        {:ok,
           :dtls_gen_connection.socket(pids, transport,
                                         {peerAddrPort, socket}, trackers)}
      {:error, reason} ->
        {:error, reason}
    end
  end

  def prepare_connection(r_state(handshake_env: r_handshake_env(renegotiation: renegotiate),
             start_or_recv_from: recvFrom) = state0,
           connection)
  ...
  def connection_information(pid, includeSecrityInfo) when is_pid(pid) do
    case (call(pid,
                 {:connection_information, includeSecrityInfo})) do
  ...
  def initial_hello({:call, from}, {:start, timeout},
           r_state(static_env: r_static_env(role: :client = role, host: host,
                             port: port, cert_db: certDbHandle,
                             cert_db_ref: certDbRef, protocol_cb: connection),
               handshake_env: r_handshake_env(renegotiation: {renegotiation, _},
                                  ocsp_stapling_state: ocspState0),
               connection_env: cEnv,
               ssl_options: %{versions: [helloVersion | _] = versions,
    ...
    {useTicket,
       state1} = :tls_client_connection_1_3.maybe_automatic_session_resumption(state0)
    ticketData = :tls_handshake_1_3.get_ticket_data(self(),
    ...
    hello0 = :tls_handshake.client_hello(host, port,
                                           ...
                                           renegotiation, keyShare, ticketData,
                                           ocspNonce, certDbHandle, certDbRef)
    hello1 = :tls_handshake_1_3.maybe_add_early_data_indication(hello0,
                                                                  earlyData,
                                                                  helloVersion)
    hello2 = :tls_handshake_1_3.maybe_add_binders(hello1,
                                                    ticketData, helloVersion)
    maxFragEnum = :maps.get(:max_frag_enum,
                              r_client_hello(hello1, :extensions), :undefined)
    connectionStates1 = :ssl_record.set_max_fragment_length(maxFragEnum,
                                                              connectionStates0)
    state2 = r_state(state1, connection_states: connectionStates1, 
    ...
    try do
      ...
      {r_state(handshake_env: hsEnv1) = state5,
         _} = connection.send_handshake_flight(state4)
      ocspStaplingKeyPresent = :maps.is_key(:ocsp_stapling,
                                              sslOpts)
      state = r_state(state5, connection_env: r_connection_env(cEnv, negotiated_version: requestedVersion), 
                          session: session, 
                          handshake_env: r_handshake_env(hsEnv1, ocsp_stapling_state: Map.merge(ocspState0, %{ocsp_nonce:
                                                                                                ocspNonce,
                                                                                                  ocsp_stapling:
                                                                                                  ocspStaplingKeyPresent})), 
                          start_or_recv_from: from,  key_share: keyShare)
      nextState = next_statem_state(versions, role)
      connection.next_event(nextState, :no_record, state,
                              [{{:timeout, :handshake}, timeout, :close}])
    catch
      {^ref, r_alert() = alert} ->
        handle_own_alert(alert, :init,
                           r_state(state0, start_or_recv_from: from))
    end
  end

  def initial_hello({:call, from}, {:start, timeout},
           r_state(static_env: r_static_env(role: role, protocol_cb: connection),
               ssl_options: %{versions: versions}) = state0) do
    nextState = next_statem_state(versions, role)
    connection.next_event(nextState, :no_record,
                            r_state(state0, start_or_recv_from: from),
                            [{{:timeout, :handshake}, timeout, :close}])
  end

  def initial_hello({:call, from}, {:start, {opts, emOpts}, timeout},
           r_state(static_env: r_static_env(role: role),
               ssl_options: origSSLOptions,
               socket_options: sockOpts) = state0) do
    try do
      sslOpts = :ssl.update_options(opts, role,
                                      origSSLOptions)
      state = ssl_config(sslOpts, role, state0)
      initial_hello({:call, from}, {:start, timeout},
                      r_state(state, ssl_options: sslOpts, 
                                 socket_options: new_emulated(emOpts,
                                                                sockOpts)))
    catch
      error ->
        {:stop_and_reply, {:shutdown, :normal},
           {:reply, from, {:error, error}}, state0}
    end
  end
  ...
  def config_error({:call, from}, {:start, _Timeout},
           r_state(protocol_specific: %{error: error}) = state) do
    {:stop_and_reply, {:shutdown, :normal},
  ...
  def connection({:call, recvFrom}, {:recv, n, timeout},
           r_state(static_env: r_static_env(protocol_cb: connection),
               socket_options: r_socket_options(active: false)) = state0) do
    passive_receive(r_state(state0, bytes_to_read: n, 
                                start_or_recv_from: recvFrom),
                      :connection, connection,
                      [{{:timeout, :recv}, timeout, :timeout}])
  end

  def connection({:call, from}, :peer_certificate,
           r_state(session: r_session(peer_certificate: cert)) = state) do
    hibernate_after(:connection, state,
                      [{:reply, from, {:ok, cert}}])
  end

  def connection({:call, from}, {:connection_information, true},
           state) do
  ...
  def connection({:call, from}, {:connection_information, false},
           state) do
  ...
  def connection({:call, from},
           ...
           r_state(connection_states: connectionStates,
               ...
               connection_env: r_connection_env(socket_tls_closed: peerClosedTLS) = cEnv) = state0) do
    action = (case (peerClosedTLS) do
                true ->
                  [{:next_event, :internal,
                      r_alert(level: 1, description: 0,
                          where: %{mfa: {:ssl_gen_statem, :connection, 3},
                                     line: 652, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'})}]
                false ->
                  [{:timeout, timeout, :downgrade}]
              end)
    case (:tls_sender.downgrade(sender, timeout)) do
      {:ok, write} ->
        state = connection.send_alert(r_alert(level: 1,
                                          description: 0,
                                          where: %{mfa:
                                                   {:ssl_gen_statem,
                                                      :connection, 3},
                                                     line: 663, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'}),
                                        r_state(state0, connection_states: Map.put(connectionStates, :current_write,
                                                                                                 write)))
        {:next_state, :downgrade,
           r_state(state, connection_env: r_connection_env(cEnv, downgrade: {newController,
                                                          from}), 
                      protocol_specific: Map.merge(pS, %{active_n_toggle:
                                                         true,
                                                           active_n: 1})),
           action}
      {:error, :timeout} ->
        {:stop_and_reply, {:shutdown, :downgrade_fail},
           [{:reply, from, {:error, :timeout}}]}
    end
  end

  def connection({:call, from}, :ktls_handover,
           r_state(static_env: r_static_env(transport_cb: transport,
                             socket: socket),
               connection_env: r_connection_env(user_application: {_Mon, pid},
                                   negotiated_version: tlsVersion),
               ssl_options: %{ktls: true}, socket_options: socketOpts,
               connection_states: %{current_write:
                                    %{security_parameters:
                                      r_security_parameters(cipher_suite: cipherSuite),
                                        cipher_state: writeState,
                                        sequence_number: writeSeq},
                                      current_read:
                                      %{cipher_state: readState,
                                          sequence_number: readSeq}})) do
    reply = (case (transport.controlling_process(socket,
                                                   pid)) do
               :ok ->
                 {:ok,
                    %{socket: socket, tls_version: tlsVersion,
                        cipher_suite: cipherSuite, socket_options: socketOpts,
                        write_state: writeState, write_seq: writeSeq,
                        read_state: readState, read_seq: readSeq}}
               {:error, reason} ->
                 {:error, reason}
             end)
    {:stop_and_reply, {:shutdown, :ktls},
       [{:reply, from, reply}]}
  end
  ...
  def connection(:cast, {:dist_handshake_complete, dHandle},
           r_state(ssl_options: %{erl_dist: true},
               static_env: r_static_env(protocol_cb: connection),
               connection_env: cEnv,
               socket_options: sockOpts) = state0) do
    :erlang.process_flag(:priority, :normal)
    state1 = r_state(state0, socket_options: r_socket_options(sockOpts, active: true), 
  ...
  def connection(:info, msg,
           r_state(static_env: r_static_env(protocol_cb: connection)) = state) do
    connection.handle_info(msg, :connection, state)
  end

  def connection(:internal, {:recv, recvFrom},
           r_state(start_or_recv_from: recvFrom,
               static_env: r_static_env(protocol_cb: connection)) = state) do
  ...
  def downgrade(:internal, r_alert(description: 0),
           r_state(static_env: r_static_env(transport_cb: transport,
                             socket: socket),
               connection_env: r_connection_env(downgrade: {pid, from}),
               protocol_buffers: r_protocol_buffers(tls_record_buffer: tlsRecordBuffer)) = state) do
    :tls_socket.setopts(transport, socket,
                          [{:active, false}, {:packet, 0}, {:mode, :binary}])
    transport.controlling_process(socket, pid)
    returnValue = (case (tlsRecordBuffer) do
                     {:undefined, {[bin] = _Front, _Size, []}} ->
                       {:ok, socket, bin}
                     _ ->
                       {:ok, socket}
                   end)
    {:stop_and_reply, {:shutdown, :downgrade},
       [{:reply, from, returnValue}], state}
  end

  def downgrade(:timeout, :downgrade,
           r_state(connection_env: r_connection_env(downgrade: {_, from})) = state) do
    {:stop_and_reply, {:shutdown, :normal},
       [{:reply, from, {:error, :timeout}}], state}
  end

  def downgrade(:info, {closeTag, socket},
           r_state(static_env: r_static_env(socket: socket, close_tag: closeTag),
               connection_env: r_connection_env(downgrade: {_, from})) = state) do
    {:stop_and_reply, {:shutdown, :normal},
       [{:reply, from, {:error, closeTag}}], state}
  end
  ...
  def handle_common_event(:internal, {:handshake, {handshake, raw}},
           stateName,
           r_state(handshake_env: r_handshake_env(tls_handshake_history: hist0) = hsEnv,
               connection_env: r_connection_env(negotiated_version: _Version)) = state0) do
    hist = :ssl_handshake.update_handshake_history(hist0,
                                                     raw)
  ...
  def handle_common_event(:internal, {:protocol_record, tLSorDTLSRecord},
           stateName,
           r_state(static_env: r_static_env(protocol_cb: connection)) = state) do
    connection.handle_protocol_record(tLSorDTLSRecord,
                                        stateName, state)
  end
  ...
  def handle_common_event({:timeout, :handshake}, :close, _StateName,
           r_state(start_or_recv_from: startFrom) = state) do
    {:stop_and_reply, {:shutdown, :user_timeout},
       {:reply, startFrom, {:error, :timeout}},
       r_state(state, start_or_recv_from: :undefined)}
  end

  def handle_common_event({:timeout, :recv}, :timeout, stateName,
           r_state(start_or_recv_from: recvFrom) = state) do
    {:next_state, stateName,
       r_state(state, start_or_recv_from: :undefined, 
                  bytes_to_read: :undefined),
  ...
  def handle_common_event(:internal, {:recv, recvFrom}, stateName,
           r_state(start_or_recv_from: recvFrom))
  ...
  def handle_common_event(type, msg, stateName, state) do
    alert = r_alert(level: 2, description: 10,
                where: %{mfa:
                         {:ssl_gen_statem, :handle_common_event, 4},
                           line: 803, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'},
  ...
  def handle_call({:close, _} = close, from, stateName,
           r_state(connection_env: cEnv) = state) do
  ...
  def handle_call({:shutdown, :read_write = how}, from, stateName,
           r_state(static_env: r_static_env(transport_cb: transport,
                             socket: socket),
               connection_env: cEnv) = state) do
    try do
      send_alert(r_alert(level: 1, description: 0,
                     where: %{mfa: {:ssl_gen_statem, :handle_call, 4},
                                line: 820, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'}),
                   stateName, state)
    catch
      return ->
        return
  ...
  def handle_call({:shutdown, how0}, from, stateName,
           r_state(static_env: r_static_env(transport_cb: transport,
  ...
  def handle_call({:recv, _N, _Timeout}, from, _,
  ...
  def handle_call({:recv, n, timeout}, recvFrom, stateName,
           state) do
    {:next_state, stateName,
       r_state(state, bytes_to_read: n, 
                  start_or_recv_from: recvFrom),
       [{:next_event, :internal, {:recv, recvFrom}},
            {{:timeout, :recv}, timeout, :timeout}]}
  end

  def handle_call({:new_user, user}, from, stateName,
           state = r_state(connection_env: r_connection_env(user_application: {oldMon,
                                                            _}) = cEnv)) do
    newMon = :erlang.monitor(:process, user)
    :erlang.demonitor(oldMon, [:flush])
    {:next_state, stateName,
       r_state(state, connection_env: r_connection_env(cEnv, user_application: {newMon,
                                                             user})),
       [{:reply, from, :ok}]}
  end

  def handle_call({:get_opts, optTags}, from, _,
           r_state(static_env: r_static_env(protocol_cb: connection, socket: socket,
                             transport_cb: transport),
               socket_options: sockOpts)) do
    optsReply = get_socket_opts(connection, transport,
                                  socket, optTags, sockOpts, [])
    {:keep_state_and_data, [{:reply, from, optsReply}]}
  end

  def handle_call({:set_opts, opts0}, from, stateName,
           r_state(static_env: r_static_env(protocol_cb: connection, socket: socket,
                             transport_cb: transport, trackers: trackers),
               connection_env: r_connection_env(user_application: {_Mon, pid}),
               socket_options: opts1) = state0) do
    {reply, opts} = set_socket_opts(connection, transport,
                                      socket, opts0, opts1, [])
    case ({:proplists.lookup(:active, opts0), opts}) do
      {{_, n}, r_socket_options(active: false)} when is_integer(n) ->
        send_user(pid,
                    format_passive(connection.pids(state0), transport,
                                     socket, trackers, connection))
      _ ->
        :ok
    end
    state = r_state(state0, socket_options: opts)
    handle_active_option(r_socket_options(opts, :active), stateName, from,
                           reply, state)
  end
  ...
  def handle_info({errorTag, socket, :econnaborted}, stateName,
           r_state(static_env: r_static_env(role: role, host: host, port: port,
                             socket: socket, transport_cb: transport,
                             error_tag: errorTag, trackers: trackers,
      ...
      when stateName !== :connection do
    maybe_invalidate_session(version, type, role, host,
                               port, session)
    pids = connection.pids(state)
    alert_user(pids, transport, trackers, socket, startFrom,
                 r_alert(level: 2, description: 0,
                     where: %{mfa: {:ssl_gen_statem, :handle_info, 3},
                                line: 916, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'}),
                 role, stateName, connection)
    {:stop, {:shutdown, :normal}, state}
  end

  def handle_info({errorTag, socket, reason}, stateName,
           r_state(static_env: r_static_env(role: role, socket: socket,
                             error_tag: errorTag)) = state) do
    (fn () ->
            ...
            :undefined ->
              :ssl_logger.log(:info, :debug,
                                %{description: 'Socket error',
                                    reason:
                                    [{:error_tag, errorTag}, {:description,
                                                                reason}]},
                                %{mfa: {:ssl_gen_statem, :handle_info, 3},
                                    line: 924, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'})
            __LogLevel__ ->
    ...
    alert = r_alert(level: 2, description: 0,
                where: %{mfa: {:ssl_gen_statem, :handle_info, 3},
                           line: 925, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'},
                reason: {:transport_error, reason})
    handle_normal_shutdown(r_alert(alert, role: role), stateName,
                             state)
    {:stop, {:shutdown, :normal}, state}
  end

  def handle_info({:DOWN, monitorRef, _, _, reason}, _,
           r_state(connection_env: r_connection_env(user_application: {monitorRef,
                                                    _Pid}),
               ssl_options: %{erl_dist: true})) do
    {:stop, {:shutdown, reason}}
  end

  def handle_info({:DOWN, monitorRef, _, _, _}, _,
           r_state(connection_env: r_connection_env(user_application: {monitorRef,
                                                    _Pid}))) do
    {:stop, {:shutdown, :normal}}
  end

  def handle_info({:EXIT, pid, _Reason}, stateName,
           r_state(connection_env: r_connection_env(user_application: {_MonitorRef,
                                                    pid})) = state) do
    {:next_state, stateName, state}
  end
  ...
  def handle_info({:EXIT, socket, :normal}, _StateName,
           r_state(static_env: r_static_env(socket: socket)) = state) do
    {:stop, {:shutdown, :transport_closed}, state}
  end

  def handle_info({:EXIT, socket, reason}, _StateName,
           r_state(static_env: r_static_env(socket: socket)) = state) do
    {:stop, {:shutdown, reason}, state}
  end

  def handle_info(:allow_renegotiate, stateName,
           r_state(handshake_env: hsEnv) = state) do
    {:next_state, stateName,
       r_state(state, handshake_env: r_handshake_env(hsEnv, allow_renegotiate: true))}
  end

  def handle_info(msg, stateName,
           r_state(static_env: r_static_env(socket: socket,
                             error_tag: errorTag)) = state) do
    (fn () ->
          case (:erlang.get(:log_level)) do
            :undefined ->
              :ssl_logger.log(:notice, :debug,
                                %{description: 'Unexpected INFO message',
                                    reason:
                                    [{:message, msg}, {:socket, socket},
                                                          {:error_tag,
                                                             errorTag}]},
                                %{mfa: {:ssl_gen_statem, :handle_info, 3},
                                    line: 953, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'})
            __LogLevel__ ->
              :ssl_logger.log(:notice, __LogLevel__,
                                %{description: 'Unexpected INFO message',
                                    reason:
                                    [{:message, msg}, {:socket, socket},
                                                          {:error_tag,
  ...
  def read_application_data(data,
           r_state(user_data_buffer: {front0, bufferSize0, rear0},
               connection_env: r_connection_env(erl_dist_handle: dHandle)) = state) do
    front = front0
    ...
    case (dHandle) do
      ...
      _ ->
        try do
          read_application_dist_data(dHandle, front, bufferSize,
                                       rear)
        catch
          :error, :notsup ->
  ...
  defp passive_receive(r_state(user_data_buffer: {front, bufferSize, rear},
              connection_env: r_connection_env(erl_dist_handle: :undefined)) = state0,
            stateName, connection, startTimerAction) do
    case (bufferSize) do
      0 ->
        connection.next_event(stateName, :no_record, state0,
                                startTimerAction)
      _ ->
        case (read_application_data(state0, front, bufferSize,
                                      rear)) do
  ...
  def hibernate_after(:connection = stateName,
           r_state(ssl_options: sslOpts) = state, actions) do
    hibernateAfter = :maps.get(:hibernate_after, sslOpts,
                                 :infinity)
    {:next_state, stateName, state,
  ...
  def send_alert(alert, :connection,
           r_state(static_env: r_static_env(protocol_cb: connection)) = state) do
    connection.send_alert_in_connection(alert, state)
  end

  def send_alert(alert, _,
           r_state(static_env: r_static_env(protocol_cb: connection)) = state) do
    connection.send_alert(alert, state)
  end

  def handle_own_alert(alert0, stateName,
           r_state(static_env: r_static_env(role: role, protocol_cb: connection),
               ssl_options: %{log_level: logLevel}) = state) do
    try do
  ...
  def handle_normal_shutdown(alert, stateName,
           r_state(static_env: r_static_env(role: role, socket: socket,
                             transport_cb: transport, protocol_cb: connection,
                             trackers: trackers),
               handshake_env: r_handshake_env(renegotiation: {false, :first}),
               start_or_recv_from: startFrom) = state) do
    pids = connection.pids(state)
    alert_user(pids, transport, trackers, socket, startFrom,
                 alert, role, stateName, connection)
  end
  ...
  def handle_normal_shutdown(alert, stateName,
           r_state(static_env: r_static_env(role: role, socket: socket,
               ...
               handshake_env: r_handshake_env(renegotiation: type),
               socket_options: opts,
               start_or_recv_from: recvFrom) = state) do
    pids = connection.pids(state)
    alert_user(pids, transport, trackers, socket, type,
                 opts, pid, recvFrom, alert, role, stateName, connection)
  end
  ...
  def handle_alert(r_alert(level: 1, description: 0) = alert,
           :downgrade = stateName, state) do
    {:next_state, stateName, state,
       [{:next_event, :internal, alert}]}
  end

  def handle_alert(r_alert(level: 1, description: 0) = alert0, stateName,
           r_state(static_env: r_static_env(role: role)) = state) do
    alert = r_alert(alert0, role: opposite_role(role))
    handle_normal_shutdown(alert, stateName, state)
    {:stop, {:shutdown, :peer_close}, state}
  end

  def handle_alert(r_alert(level: 1, description: 100) = alert, stateName,
    ...
    log_alert(logLevel, role, connection.protocol_name(),
                stateName, r_alert(alert, role: opposite_role(role)))
    ownAlert = r_alert(level: 2, description: 10,
                   where: %{mfa: {:ssl_gen_statem, :handle_alert, 3},
                              line: 1101, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'},
                   reason: :unexpected_renegotiate_alert_during_initial_handshake)
    handle_own_alert(ownAlert, stateName, state)
  end

  def handle_alert(r_alert() = alert, stateName,
           r_state(static_env: r_static_env(role: :server = role,
                             protocol_cb: connection),
               handshake_env: r_handshake_env(renegotiation: {false, :first}),
               ssl_options: %{log_level: logLevel}) = state)
    ...
    ownAlert = r_alert(level: 2, description: 10,
                   where: %{mfa: {:ssl_gen_statem, :handle_alert, 3},
                              line: 1112, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'},
                   reason: :unexpected_alert)
    handle_own_alert(ownAlert, stateName, state)
  end

  def handle_alert(r_alert(level: 1, description: 100) = alert0,
           stateName,
           r_state(static_env: r_static_env(role: role, protocol_cb: connection),
  ...
  def handle_alert(r_alert(level: 1, description: 100) = alert,
           :connection = stateName,
           r_state(static_env: r_static_env(role: role, protocol_cb: connection),
               handshake_env: r_handshake_env(renegotiation: {true, from}) = hsEnv,
               ssl_options: %{log_level: logLevel}) = state0) do
    log_alert(logLevel, role, connection.protocol_name(),
                stateName, r_alert(alert, role: opposite_role(role)))
  ...
  def handle_alert(r_alert(level: 1, description: 100) = alert, stateName,
    ...
    :gen_statem.reply(from,
                        {:error, :renegotiation_rejected})
    state = connection.reinit(r_state(state0, handshake_env: r_handshake_env(hsEnv, renegotiation: :undefined)))
    connection.next_event(:connection, :no_record, state)
  end

  def handle_alert(r_alert(level: 1, description: 90) = alert, stateName,
           r_state(static_env: r_static_env(role: role, protocol_cb: connection),
               ssl_options: %{log_level: logLevel}) = state)
      when stateName !== :connection do
    log_alert(logLevel, role, connection.protocol_name(),
  ...
  def handle_alert(r_alert(level: 1) = alert, stateName,
           r_state(static_env: r_static_env(role: role, protocol_cb: connection),
               connection_env: r_connection_env(negotiated_version: version),
               ssl_options: %{log_level: logLevel}) = state)
      when version < {3, 4} do
  ...
  defp handle_fatal_alert(alert0, stateName,
            r_state(static_env: r_static_env(role: role, socket: socket, host: host,
                              port: port, trackers: trackers,
                              transport_cb: transport, protocol_cb: connection),
                connection_env: r_connection_env(user_application: {_Mon, pid}),
                ssl_options: %{log_level: logLevel},
                start_or_recv_from: from, session: session,
                socket_options: opts) = state) do
    invalidate_session(role, host, port, session)
    alert = r_alert(alert0, role: opposite_role(role))
    log_alert(logLevel, role, connection.protocol_name(),
                stateName, alert)
    pids = connection.pids(state)
    alert_user(pids, transport, trackers, socket, stateName,
                 opts, pid, from, alert, role, stateName, connection)
    {:stop, {:shutdown, :normal}, state}
  end

  def handle_trusted_certs_db(r_state(ssl_options: %{cacerts: []} = opts)) when not
                                                     :erlang.is_map_key(:cacertfile,
                                                                          opts) do
    :ok
  end

  def handle_trusted_certs_db(r_state(static_env: r_static_env(cert_db_ref: ref,
                           cert_db: certDb),
             ssl_options: opts))
  ...
  def handle_trusted_certs_db(r_state(static_env: r_static_env(cert_db_ref: ref,
                           file_ref_db: refDb),
             ssl_options: %{cacertfile: file})) do
  ...
  def maybe_invalidate_session(version, type, role, host, port, session)
      when version < {3, 4} do
    maybe_invalidate_session(type, role, host, port,
                               session)
  end

  def maybe_invalidate_session({false, :first}, :server = role, host, port,
           session) do
    invalidate_session(role, host, port, session)
  end
  ...
  def terminate(_, _,
           r_state(connection_env: r_connection_env(socket_terminated: true))) do
    :ok
  end

  def terminate({:shutdown, :transport_closed} = reason,
           _StateName,
           r_state(static_env: r_static_env(protocol_cb: connection, socket: socket,
  ...
  def terminate({:shutdown, :own_alert}, _StateName,
           r_state(static_env: r_static_env(protocol_cb: connection, socket: socket,
                             transport_cb: transport)) = state) do
    handle_trusted_certs_db(state)
    case (:application.get_env(:ssl, :alert_timeout)) do
      {:ok, timeout} when is_integer(timeout) ->
        connection.close({:timeout, timeout}, socket, transport,
                           :undefined)
      _ ->
        connection.close({:timeout, 5000}, socket, transport,
                           :undefined)
    end
  end

  def terminate(reason, :connection,
           r_state(static_env: r_static_env(protocol_cb: connection,
  ...
  def terminate(reason, _StateName,
           r_state(static_env: r_static_env(transport_cb: transport,
                             protocol_cb: connection,
                             socket: socket)) = state) do
    handle_trusted_certs_db(state)
  ...
  def format_status(:terminate, [_, stateName, state]) do
    ...
    newOptions = Map.merge(sslOptions, %{certs_keys: '***',
                                           cacerts: '***', dh: '***', psk_identity: '***',
                                           srp_identity: '***'})
    [{:data,
        [{'State',
            {stateName,
               r_state(state, connection_states: '***',  protocol_buffers: '***', 
                          user_data_buffer: '***',  handshake_env: '***', 
                          connection_env: '***',  session: '***', 
                          ssl_options: newOptions,  flight_buffer: '***')}}]}]
  end
  ...
  defp check_hostname('') do
    r_alert(level: 2, description: 112,
        where: %{mfa: {:ssl_gen_statem, :check_hostname, 1},
                   line: 1332, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'},
        reason: :empty_sni)
  end
  ...
  defp check_hostname(hostname) do
    case (:lists.reverse(hostname)) do
      [?. | _] ->
        r_alert(level: 2, description: 112,
            where: %{mfa: {:ssl_gen_statem, :check_hostname, 1},
                       line: 1336, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'},
  ...
  defp handle_sni_hostname(hostname,
            r_state(static_env: initStatEnv0, handshake_env: hsEnv,
                connection_env: cEnv, ssl_options: opts) = state0) do
      ...
      newOptions ->
        {:ok,
           %{cert_db_ref: ref, cert_db_handle: certDbHandle,
               ...
               session_cache: cacheHandle, crl_db_info: cRLDbHandle,
               cert_key_alts: certKeyAlts,
               dh_params: dHParams}} = :ssl_config.init(newOptions,
                                                          :server)
        r_state(state0, static_env: r_static_env(initStatEnv0, file_ref_db: fileRefHandle, 
                                                cert_db_ref: ref, 
                                                cert_db: certDbHandle, 
                                                crl_db: cRLDbHandle, 
                                                session_cache: cacheHandle), 
                    ...
                    handshake_env: r_handshake_env(hsEnv, sni_hostname: hostname, 
                                              sni_guided_cert_selection: true, 
                                              diffie_hellman_params: dHParams))
    end
  end

  defp update_ssl_options_from_sni(%{sni_fun: sNIFun} = origSSLOptions,
            sNIHostname) do
    case (sNIFun.(sNIHostname)) do
      :undefined ->
        :undefined
      sSLOptions ->
        versionsOpt = :proplists.get_value(:versions,
                                             sSLOptions, [])
        fallBackOptions = filter_for_versions(versionsOpt,
                                                origSSLOptions)
        :ssl.update_options(sSLOptions, :server,
                              fallBackOptions)
    end
  end
  ...
  defp filter_for_versions([:"tlsv1.3"], origSSLOptions) do
    opts = [:client_renegotiation, :dh_file, :eccs,
                                                 :fallback, :secure_renegotiate,
                                                                :psk_identity,
                                                                    :reuse_session,
                                                                        :reuse_sessions,
                                                                            :srp_identity,
                                                                                :user_lookup_fun] ++ [:padding_check,
                                                                                                          :beast_mitigation]
  ...
  defp filter_for_versions([:"tlsv1.2"], origSSLOptions) do
    opts = [:anti_replay, :certificate_authorities, :cookie,
                                                        :early_data,
                                                            :key_update_at,
                                                                :middlebox_comp_mode,
                                                                    :session_tickets,
                                                                        :supported_groups,
                                                                            :use_ticket] ++ [:padding_check,
                                                                                                 :beast_mitigation]
    :maps.without(opts, origSSLOptions)
  end
                                                        ...
                                                        :early_data,
                                                            :key_update_at,
                                                                :middlebox_comp_mode,
                                                                    :session_tickets,
                                                                        :supported_groups,
                                                                            :use_ticket] ++ [:signature_algs,
                                                                                                 :signature_algs_cert] ++ [:padding_check,
                                                                                                                               :beast_mitigation]
    :maps.without(opts, origSSLOptions)
  end
  ...
  defp ack_connection(r_state(handshake_env: r_handshake_env(renegotiation: {true,
                                               initiater}) = hsEnv) = state)
  ...
  defp ack_connection(r_state(handshake_env: r_handshake_env(renegotiation: {false,
                                               :first}) = hsEnv,
              start_or_recv_from: startFrom) = state)
  ...
  defp handle_active_option(false, :connection = stateName, to, reply,
            state) do
    hibernate_after(stateName, state, [{:reply, to, reply}])
  end

  defp handle_active_option(_, :connection = stateName, to, reply,
            r_state(static_env: r_static_env(role: role),
                connection_env: r_connection_env(socket_tls_closed: true),
                user_data_buffer: {_, 0, _}) = state) do
    alert = r_alert(level: 2, description: 0,
                where: %{mfa:
                         {:ssl_gen_statem, :handle_active_option, 5},
                           line: 1444, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'},
                reason: :all_data_delivered)
    handle_normal_shutdown(r_alert(alert, role: role), stateName,
                             state)
    {:stop_and_reply, {:shutdown, :peer_close},
       [{:reply, to, reply}]}
  end

  defp handle_active_option(_, :connection = stateName0, to, reply,
            r_state(static_env: r_static_env(protocol_cb: connection),
                user_data_buffer: {_, 0, _}) = state0) do
    case (connection.next_event(stateName0, :no_record,
                                  state0)) do
      {:next_state, stateName, state} ->
        hibernate_after(stateName, state, [{:reply, to, reply}])
      {:next_state, stateName, state, actions} ->
  ...
  defp handle_active_option(_, stateName, to, reply,
            r_state(user_data_buffer: {_, 0, _}) = state) do
    {:next_state, stateName, state, [{:reply, to, reply}]}
  end

  defp handle_active_option(_, stateName0, to, reply,
            r_state(static_env: r_static_env(protocol_cb: connection)) = state0) do
    case (read_application_data(<<>>, state0)) do
      {:stop, _, _} = stop ->
        stop
      {record, state1} ->
        case (connection.next_event(stateName0, record,
                                      state1)) do
          {:next_state, stateName, state} ->
            hibernate_after(stateName, state, [{:reply, to, reply}])
          {:next_state, stateName, state, actions} ->
  ...
  defp read_application_data(r_state(socket_options: socketOpts,
              bytes_to_read: bytesToRead,
              start_or_recv_from: recvFrom) = state,
            front, bufferSize, rear) do
    read_application_data(state, front, bufferSize, rear,
                            socketOpts, recvFrom, bytesToRead)
  end

  defp read_application_data(state, [bin | front], bufferSize, rear,
            socketOpts, recvFrom, bytesToRead) do
    read_application_data_bin(state, front, bufferSize,
                                rear, socketOpts, recvFrom, bytesToRead, bin)
  end

  defp read_application_data(state, [] = front, bufferSize, [] = rear,
            socketOpts, recvFrom, bytesToRead) do
    0 = bufferSize
    {:no_record,
       r_state(state, socket_options: socketOpts, 
                  bytes_to_read: bytesToRead, 
                  start_or_recv_from: recvFrom, 
                  user_data_buffer: {front, bufferSize, rear})}
  end

  defp read_application_data(state, [], bufferSize, rear, socketOpts,
            recvFrom, bytesToRead) do
    [bin | front] = :lists.reverse(rear)
    read_application_data_bin(state, front, bufferSize, [],
                                socketOpts, recvFrom, bytesToRead, bin)
  end

  defp read_application_data_bin(state, front, bufferSize, rear, socketOpts,
            recvFrom, bytesToRead, <<>>) do
    read_application_data(state, front, bufferSize, rear,
                            socketOpts, recvFrom, bytesToRead)
  end

  defp read_application_data_bin(state, front0, bufferSize0, rear0, socketOpts0,
            recvFrom, bytesToRead, bin0) do
    case (get_data(socketOpts0, bytesToRead, bin0)) do
      {:ok, data, bin} ->
        bufferSize = bufferSize0 - (byte_size(bin0) - byte_size(bin))
        read_application_data_deliver(state, [bin | front0],
                                        bufferSize, rear0, socketOpts0,
                                        recvFrom, data)
      {:more, :undefined} ->
        cond do
          byte_size(bin0) < bufferSize0 ->
            bin = :erlang.iolist_to_binary([bin0, front0 |
                                                      :lists.reverse(rear0)])
            read_application_data_bin(state, [], bufferSize0, [],
                                        socketOpts0, recvFrom, bytesToRead, bin)
          true ->
            {:no_record,
               r_state(state, socket_options: socketOpts0, 
                          bytes_to_read: bytesToRead, 
                          start_or_recv_from: recvFrom, 
                          user_data_buffer: {[bin0 | front0], bufferSize0,
                                               rear0})}
        end
      {:more, size} when size <= bufferSize0 ->
        {data, front,
           rear} = iovec_from_front(size - byte_size(bin0), front0,
                                      rear0, [bin0])
        bin = :erlang.iolist_to_binary(data)
        read_application_data_bin(state, front, bufferSize0,
                                    rear, socketOpts0, recvFrom, bytesToRead,
                                    bin)
      {:more, _Size} ->
        {:no_record,
           r_state(state, socket_options: socketOpts0, 
        ...
        {:no_record,
           r_state(state, socket_options: socketOpts0, 
                      ...
                      user_data_buffer: {[bin0 | front0], bufferSize0,
                                           rear0})}
      {:error, _Reason} ->
        r_state(static_env: r_static_env(socket: socket, protocol_cb: connection,
                          transport_cb: transport, trackers: trackers),
            connection_env: r_connection_env(user_application: {_Mon,
                                                   pid})) = state
        buffer = :erlang.iolist_to_binary([bin0, front0 |
                                                     :lists.reverse(rear0)])
        deliver_packet_error(connection.pids(state), transport,
                               socket, socketOpts0, buffer, pid, recvFrom,
                               trackers, connection)
        {:stop, {:shutdown, :normal},
           r_state(state, socket_options: socketOpts0, 
                      bytes_to_read: bytesToRead, 
  ...
  defp read_application_data_deliver(state, front, bufferSize, rear, socketOpts0,
            recvFrom, data) do
    r_state(static_env: r_static_env(socket: socket, protocol_cb: connection,
                      transport_cb: transport, trackers: trackers),
        connection_env: r_connection_env(user_application: {_Mon,
                                               pid})) = state
    socketOpts = deliver_app_data(connection.pids(state),
                                    transport, socket, socketOpts0, data, pid,
                                    recvFrom, trackers, connection)
    cond do
      r_socket_options(socketOpts, :active) === false ->
        {:no_record,
           r_state(state, user_data_buffer: {front, bufferSize, rear}, 
                      start_or_recv_from: :undefined, 
                      bytes_to_read: :undefined,  socket_options: socketOpts)}
      true ->
        case (r_handshake_env(r_state(state, :handshake_env), :early_data_accepted)) do
          false ->
            read_application_data(state, front, bufferSize, rear,
                                    socketOpts, :undefined, :undefined)
          true ->
            read_application_data(state, front, bufferSize, rear,
                                    socketOpts, recvFrom, :undefined)
        end
    end
  end
  ...
  defp read_application_dist_data(dHandle, front0, bufferSize, rear0, bin0) do
    case (bin0) do
      <<sizeA :: size(32), dataA :: size(sizeA) - binary,
          sizeB :: size(32), dataB :: size(sizeB) - binary,
          sizeC :: size(32), dataC :: size(sizeC) - binary,
          sizeD :: size(32), dataD :: size(sizeD) - binary,
          rest :: binary>>
        ...
        read_application_dist_data(dHandle, front0,
      ...
      <<sizeA :: size(32), dataA :: size(sizeA) - binary,
        ...
        read_application_dist_data(dHandle, front0,
                                     ...
                                     rear0, rest)
      <<size :: size(32), data :: size(size) - binary,
          rest :: binary>> ->
        0 < size and :erlang.dist_ctrl_put_data(dHandle, data)
        read_application_dist_data(dHandle, front0,
                                     bufferSize - (4 + size), rear0, rest)
      <<size :: size(32), firstData :: binary>>
          when 4 + size <= bufferSize ->
        {data, front,
           rear} = iovec_from_front(size - byte_size(firstData),
                                      front0, rear0, [firstData])
        0 < size and :erlang.dist_ctrl_put_data(dHandle, data)
        read_application_dist_data(dHandle, front,
                                     bufferSize - (4 + size), rear)
      <<bin :: binary>> ->
        case (bin) do
          ...
          <<incompleteLengthField :: binary>> when 4 < bufferSize
                                                   ->
            {lengthField, front,
               rear} = (case (incompleteLengthField) do
                          <<>> ->
                            iovec_from_front(4, front0, rear0, [])
                          _ ->
                            iovec_from_front(4 - byte_size(incompleteLengthField),
                                               front0, rear0,
  ...
  defp iovec_from_front(size, [bin | front], rear, acc) do
    case (bin) do
      ...
      <<_ :: binary>> ->
        binSize = byte_size(bin)
        iovec_from_front(size - binSize, front, rear,
                           [bin | acc])
    end
  end
  ...
  defp get_data(r_socket_options(active: active, packet: raw), bytesToRead,
            bin)
  ...
  defp deliver_app_data(cPids, transport, socket,
            r_socket_options(active: active, packet: type) = sOpts, data, pid,
            from, trackers, connection) do
    send_or_reply(active, pid, from,
                    format_reply(cPids, transport, socket, sOpts, data,
                                   trackers, connection))
    sO = (case (data) do
            {p, _, _, _}
                when (p === :http_request or p === :http_response and
    ...
    case (active) do
      ...
      1 ->
        send_user(pid,
                    format_passive(cPids, transport, socket, trackers,
                                     connection))
        r_socket_options(sO, active: false)
      n when is_integer(n) ->
  ...
  defp format_reply(_, _, _,
            r_socket_options(active: false, mode: mode, packet: packet,
                header: header),
            data, _, _) do
    {:ok, do_format_reply(mode, packet, header, data)}
  end

  defp format_reply(cPids, transport, socket,
            r_socket_options(active: _, mode: mode, packet: packet,
                header: header),
            data, trackers, connection) do
    {:ssl,
       connection.socket(cPids, transport, socket, trackers),
       do_format_reply(mode, packet, header, data)}
  end

  defp deliver_packet_error(cPids, transport, socket,
            sO = r_socket_options(active: active), data, pid, from, trackers,
            connection) do
    send_or_reply(active, pid, from,
                    format_packet_error(cPids, transport, socket, sO, data,
                                          trackers, connection))
  end

  defp format_packet_error(_, _, _, r_socket_options(active: false, mode: mode), data, _,
            _) do
    {:error,
       {:invalid_packet, do_format_reply(mode, :raw, 0, data)}}
  end

  defp format_packet_error(cPids, transport, socket,
            r_socket_options(active: _, mode: mode), data, trackers, connection) do
    {:ssl_error,
       connection.socket(cPids, transport, socket, trackers),
       {:invalid_packet, do_format_reply(mode, :raw, 0, data)}}
  end
  ...
  defp do_format_reply(:list, packet, _, data) when packet == :http or
  ...
  defp format_passive(cPids, transport, socket, trackers,
            connection) do
    {:ssl_passive,
       connection.socket(cPids, transport, socket, trackers)}
  end
  ...
  defp header(n, binary) do
    <<byteN :: size(8) - unsigned - big - integer,
  ...
  defp alert_user(pids, transport, trackers, socket, _, opts, pid,
            from, alert, role, :connection = stateName,
            connection) do
    alert_user(pids, transport, trackers, socket,
                 r_socket_options(opts, :active), pid, from, alert, role, stateName,
                 connection)
  end

  defp alert_user(pids, transport, trackers, socket,
            {true, :internal}, opts, pid, from, alert, role,
  ...
  defp alert_user(pids, transport, trackers, socket, _, _, _,
            from, alert, role, stateName, connection) do
    alert_user(pids, transport, trackers, socket, from,
                 alert, role, stateName, connection)
  end

  defp alert_user(pids, transport, trackers, socket, from, alert,
            role, stateName, connection) do
    alert_user(pids, transport, trackers, socket, false,
                 :no_pid, from, alert, role, stateName, connection)
  end

  defp alert_user(_, _, _, _, false = active, pid, from, alert,
            role, stateName, connection)
      when from !== :undefined do
  ...
  defp alert_user(pids, transport, trackers, socket, active, pid,
            from, alert, role, stateName, connection) do
    case (:ssl_alert.reason_code(alert, role,
                                   connection.protocol_name(), stateName)) do
      :closed ->
        send_or_reply(active, pid, from,
                        {:ssl_closed,
                           connection.socket(pids, transport, socket,
                                               trackers)})
      reasonCode ->
        send_or_reply(active, pid, from,
                        {:ssl_error,
                           connection.socket(pids, transport, socket, trackers),
                           reasonCode})
    end
  end

  defp log_alert(level, role, protocolName, stateName,
            r_alert(role: role) = alert) do
    :ssl_logger.log(:notice, level,
                      %{protocol: protocolName, role: role,
                          statename: stateName, alert: alert, alerter: :own},
                      r_alert(alert, :where))
  end
  ...
  defp log_alert(level, role, protocolName, stateName, alert) do
    :ssl_logger.log(:notice, level,
                      %{protocol: protocolName, role: role,
                          statename: stateName, alert: alert, alerter: :peer},
                      r_alert(alert, :where))
  end

  defp terminate_alert(:normal) do
    r_alert(level: 1, description: 0,
        where: %{mfa: {:ssl_gen_statem, :terminate_alert, 1},
                   line: 1892, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'})
  end

  defp terminate_alert({reason, _}) when reason == :close or
                              reason == :shutdown do
    r_alert(level: 1, description: 0,
        where: %{mfa: {:ssl_gen_statem, :terminate_alert, 1},
                   line: 1895, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'})
  end

  defp terminate_alert(_) do
    r_alert(level: 2, description: 80,
        where: %{mfa: {:ssl_gen_statem, :terminate_alert, 1},
                   line: 1897, file: 'otp/lib/ssl/src/ssl_gen_statem.erl'})
  end
  ...
  defp connection_info(r_state(handshake_env: r_handshake_env(sni_hostname: sNIHostname,
                               resumption: resumption),
              session: r_session(session_id: sessionId,
                           cipher_suite: cipherSuite, srp_username: srpUsername,
              ...
              ssl_options: %{protocol: protocol} = opts)) do
    recordCB = record_cb(protocol)
    cipherSuiteDef = (%{key_exchange:
                        kexAlg} = :ssl_cipher_format.suite_bin_to_map(cipherSuite))
    isNamedCurveSuite = :lists.member(kexAlg,
                                        [:ecdh_ecdsa, :ecdhe_ecdsa, :ecdh_rsa,
                                                                        :ecdhe_rsa,
                                                                            :ecdh_anon])
    curveInfo = (case (eCCCurve) do
    ...
    mFLInfo = (case (:maps.get(:max_fragment_length,
                                 currentWrite, :undefined)) do
    ...
    [{:protocol, recordCB.protocol_version(version)},
         {:session_id, sessionId}, {:session_data,
                                           ...
                                           {:selected_cipher_suite,
                                              cipherSuiteDef},
                                               {:sni_hostname, sNIHostname},
                                                   {:srp_username,
                                                      srpUsername} |
                                                       curveInfo] ++ mFLInfo ++ ssl_options_list(opts)
  end

  defp security_info(r_state(connection_states: connectionStates,
              static_env: r_static_env(role: role), ssl_options: opts,
              protocol_specific: protocolSpecific)) do
    readState = :ssl_record.current_connection_state(connectionStates,
                                                       :read)
    %{security_parameters:
      r_security_parameters(client_random: clientRand, server_random: serverRand,
          master_secret: masterSecret,
          application_traffic_secret: appTrafSecretRead,
          client_early_data_secret: serverEarlyData)} = readState
    baseSecurityInfo = [{:client_random, clientRand},
                            {:server_random, serverRand}, {:master_secret,
                                                             masterSecret}]
    ...
    cond do
      ...
      true ->
        %{security_parameters:
          r_security_parameters(application_traffic_secret: appTrafSecretWrite0,
              client_early_data_secret: clientEarlyData)} = :ssl_record.current_connection_state(connectionStates,
                                                                                                   :write)
        sender = :maps.get(:sender, protocolSpecific,
                             :undefined)
  ...
  defp get_socket_opts(connection, transport, socket, [:mode | tags],
            sockOpts, acc) do
    get_socket_opts(connection, transport, socket, tags,
                      sockOpts, [{:mode, r_socket_options(sockOpts, :mode)} | acc])
  end

  defp get_socket_opts(connection, transport, socket, [:packet | tags],
            sockOpts, acc) do
    case (r_socket_options(sockOpts, :packet)) do
      {type, :headers} ->
        get_socket_opts(connection, transport, socket, tags,
                          sockOpts, [{:packet, type} | acc])
      type ->
        get_socket_opts(connection, transport, socket, tags,
                          sockOpts, [{:packet, type} | acc])
    end
  ...
  defp get_socket_opts(connection, transport, socket, [:header | tags],
            sockOpts, acc) do
    get_socket_opts(connection, transport, socket, tags,
                      sockOpts, [{:header, r_socket_options(sockOpts, :header)} | acc])
  end

  defp get_socket_opts(connection, transport, socket, [:active | tags],
            sockOpts, acc) do
    get_socket_opts(connection, transport, socket, tags,
                      sockOpts, [{:active, r_socket_options(sockOpts, :active)} | acc])
  end

  defp get_socket_opts(connection, transport, socket,
            [:packet_size | tags], sockOpts, acc) do
    get_socket_opts(connection, transport, socket, tags,
                      sockOpts,
                      [{:packet_size, r_socket_options(sockOpts, :packet_size)} | acc])
  end

  defp get_socket_opts(connection, transport, socket, [tag | tags],
            sockOpts, acc) do
  ...
  defp get_socket_opts(_, _, _, opts, _, _) do
    {:error,
       {:options, {:socket_options, opts, :function_clause}}}
  end
  ...
  defp set_socket_opts(connectionCb, transport, socket, [], sockOpts,
            other) do
    try do
      connectionCb.setopts(transport, socket, other)
    catch
      _, error ->
        {{:error, {:options, {:socket_options, other, error}}},
           sockOpts}
    else
      :ok ->
        {:ok, sockOpts}
      {:error, inetError} ->
        {{:error,
            {:options, {:socket_options, other, inetError}}},
           sockOpts}
    end
  end

  defp set_socket_opts(connectionCb, transport, socket,
            [{:mode, mode} | opts], sockOpts, other)
      when mode == :list or mode == :binary do
    set_socket_opts(connectionCb, transport, socket, opts,
                      r_socket_options(sockOpts, mode: mode), other)
  end
  ...
  defp set_socket_opts(connectionCb, transport, socket,
            [{:packet, packet} | opts], sockOpts, other)
      when packet == :raw or packet == 0 or packet == 1 or
             ...
             packet == :httph_bin do
    set_socket_opts(connectionCb, transport, socket, opts,
                      r_socket_options(sockOpts, packet: packet), other)
  end

  defp set_socket_opts(_, _, _, [{:packet, _} = opt | _], sockOpts,
            _) do
    {{:error, {:options, {:socket_options, opt}}}, sockOpts}
  end

  defp set_socket_opts(connectionCb, transport, socket,
            [{:header, header} | opts], sockOpts, other)
      when is_integer(header) do
    set_socket_opts(connectionCb, transport, socket, opts,
                      r_socket_options(sockOpts, header: header), other)
  end

  defp set_socket_opts(_, _, _, [{:header, _} = opt | _], sockOpts,
            _) do
    {{:error, {:options, {:socket_options, opt}}}, sockOpts}
  end

  defp set_socket_opts(connectionCb, transport, socket,
            [{:active, active} | opts], sockOpts, other)
      when active == :once or active == true or
             active == false do
    set_socket_opts(connectionCb, transport, socket, opts,
                      r_socket_options(sockOpts, active: active), other)
  end

  defp set_socket_opts(connectionCb, transport, socket,
            [{:active, active1} = opt | opts],
            sockOpts = r_socket_options(active: active0), other)
  ...
  defp set_socket_opts(_, _, _, [{:active, _} = opt | _], sockOpts,
            _) do
    {{:error, {:options, {:socket_options, opt}}}, sockOpts}
  end

  defp set_socket_opts(connectionCb, transport, socket,
            [{:packet_size, size} | opts], sockOpts, other)
      when is_integer(size) do
    set_socket_opts(connectionCb, transport, socket, opts,
                      r_socket_options(sockOpts, packet_size: size), other)
  end

  defp set_socket_opts(_, _, _, [{:packet_size, _} = opt | _],
            sockOpts, _) do
    {{:error, {:options, {:socket_options, opt}}}, sockOpts}
  end

  defp set_socket_opts(connectionCb, transport, socket, [opt | opts],
            sockOpts, other) do
    set_socket_opts(connectionCb, transport, socket, opts,
                      sockOpts, [opt | other])
  end
  ...
  defp maybe_add_keylog(info) do
    maybe_add_keylog(:lists.keyfind(:protocol, 1, info),
                       info)
  end
  ...
  defp maybe_add_keylog({_, :"tlsv1.3"}, info) do
    try do
      {:client_random,
         clientRandomBin} = :lists.keyfind(:client_random, 1,
                                             info)
      maybeUpdateTrafficSecret = fn {direction,
                                       {sender, trafficSecret0}} ->
                                      trafficSecret = (case (call(sender,
                                                                    :get_application_traffic_secret)) do
                                                         {:ok,
      ...
      {:client_traffic_secret_0,
         clientTrafficSecret0Bin} = maybeUpdateTrafficSecret.(:lists.keyfind(:client_traffic_secret_0,
                                                                               1,
                                                                               info))
      {:server_traffic_secret_0,
         serverTrafficSecret0Bin} = maybeUpdateTrafficSecret.(:lists.keyfind(:server_traffic_secret_0,
                                                                               1,
                                                                               info))
      {:client_handshake_traffic_secret,
         clientHSecretBin} = :lists.keyfind(:client_handshake_traffic_secret,
                                              1, info)
      {:server_handshake_traffic_secret,
         serverHSecretBin} = :lists.keyfind(:server_handshake_traffic_secret,
                                              1, info)
      {:selected_cipher_suite,
         %{prf: prf}} = :lists.keyfind(:selected_cipher_suite, 1,
                                         info)
      clientRandom = :binary.decode_unsigned(clientRandomBin)
      clientTrafficSecret0 = keylog_secret(clientTrafficSecret0Bin,
                                             prf)
      serverTrafficSecret0 = keylog_secret(serverTrafficSecret0Bin,
                                             prf)
      ...
      keylog0 = [:io_lib.format('CLIENT_HANDSHAKE_TRAFFIC_SECRET ~64.16.0B ',
                     ...
                     :io_lib.format('SERVER_HANDSHAKE_TRAFFIC_SECRET ~64.16.0B ', [clientRandom]) ++ serverHSecret,
                         :io_lib.format('CLIENT_TRAFFIC_SECRET_0 ~64.16.0B ',
                                          [clientRandom]) ++ clientTrafficSecret0,
                             :io_lib.format('SERVER_TRAFFIC_SECRET_0 ~64.16.0B ',
                                              [clientRandom]) ++ serverTrafficSecret0]
      keylog = (case (:lists.keyfind(:client_early_data_secret,
                                       1, info)) do
    ...
    try do
      {:client_random,
         clientRandomBin} = :lists.keyfind(:client_random, 1,
                                             info)
      {:master_secret,
         masterSecretBin} = :lists.keyfind(:master_secret, 1,
                                             info)
      clientRandom = :binary.decode_unsigned(clientRandomBin)
      masterSecret = :binary.decode_unsigned(masterSecretBin)
      keylog = [:io_lib.format('CLIENT_RANDOM ~64.16.0B ~96.16.0B',
  ...
  defp maybe_generate_client_shares(%{versions: [{3, 4} | _],
              supported_groups: r_supported_groups(supported_groups: [group | _])}) do
  ...
  def handle_trace(:api,
           {:call, {:ssl_gen_statem, :connect, [connection | _]}},
           stack0) do
    {:io_lib.format('Connection = ~w', [connection]), stack0}
  end

  def handle_trace(:rle,
           {:call, {:ssl_gen_statem, :init, args = [[role | _]]}},
           stack0) do
    {:io_lib.format('(*~w) Args = ~W', [role, args, 3]),
       [{:role, role} | stack0]}
  end

  def handle_trace(:hbn,
           {:call,
              {:ssl_gen_statem, :hibernate_after,
                 [_StateName = :connection, state, actions]}},
           stack) do
    r_state(ssl_options: %{hibernate_after:
                     hibernateAfter}) = state
    {:io_lib.format('* * * maybe hibernating in ~w ms * * * Actions = ~W ', [hibernateAfter, actions, 10]),
       stack}
  end

  def handle_trace(:hbn,
           {:return_from, {:ssl_gen_statem, :hibernate_after, 3},
  ...
  def handle_trace(:hbn,
           {:call,
              {:ssl_gen_statem, :handle_common_event,
                 [:timeout, :hibernate, :connection | _]}},
           stack) do
