defmodule :m_tls_gen_connection_1_3 do
  ...
  def initial_state(role, sender, host, port, socket,
  ...
  def wait_cert(:internal = type, r_change_cipher_spec() = msg,
  ...
  def wait_cv(:internal = type, r_change_cipher_spec() = msg,
           r_state(session: r_session(session_id: id)) = state)
      when id !== <<>> do
  ...
  def connection({:call, from},
           {:export_key_materials, labels, contexts, wantedLengths,
              last},
           r_state(connection_states: connectionStates,
               protocol_specific: pS) = state0) do
    %{security_parameters:
      r_security_parameters(prf_algorithm: pRFAlgorithm)} = :ssl_record.current_connection_state(connectionStates,
                                                                               :read)
    case (:maps.get(:exporter_master_secret, pS,
                      :undefined)) do
      :undefined ->
        {:next_state, :connection, state0,
           [{:reply, from,
               {:error, :exporter_master_secret_already_consumed}}]}
      exporterMasterSecret ->
        expSecrets = exporter_secrets(exporterMasterSecret,
                                        labels, pRFAlgorithm)
        state = (case (last) do
                   true ->
                     r_state(state0, protocol_specific: :maps.without([:exporter_master_secret],
                                                                  pS))
                   false ->
                     state0
                 end)
        exportKeyMaterials = export_key_materials(expSecrets,
                                                    contexts, wantedLengths,
                                                    pRFAlgorithm)
        {:next_state, :connection, state,
           [{:reply, from, exportKeyMaterials}]}
    end
  end
  ...
  def maybe_queue_change_cipher_spec(r_state(flight_buffer: flightBuffer0) = state0,
           :first) do
    {state,
       flightBuffer} = maybe_prepend_change_cipher_spec(state0,
                                                          flightBuffer0)
  ...
  def maybe_queue_change_cipher_spec(r_state(flight_buffer: flightBuffer0) = state0,
           :last) do
    {state,
       flightBuffer} = maybe_append_change_cipher_spec(state0,
                                                         flightBuffer0)
    r_state(state, flight_buffer: flightBuffer)
  end

  def handle_change_cipher_spec(type, msg, stateName,
  ...
  def maybe_prepend_change_cipher_spec(r_state(session: r_session(session_id: id),
             handshake_env: r_handshake_env(change_cipher_spec_sent: false) = hSEnv) = state,
           bin)
      when id !== <<>> do
  ...
  def maybe_append_change_cipher_spec(r_state(session: r_session(session_id: id),
             handshake_env: r_handshake_env(change_cipher_spec_sent: false) = hSEnv) = state,
           bin)
      when id !== <<>> do
    cCSBin = :tls_handshake_1_3.create_change_cipher_spec(state)
  ...
  defp handle_new_session_ticket(r_new_session_ticket(ticket_nonce: nonce) = newSessionTicket,
            r_state(connection_states: connectionStates,
                ssl_options: %{session_tickets:
                               sessionTickets} = sslOpts,
    ...
    sNI = :maps.get(:server_name_indication, sslOpts,
                      :undefined)
           ...
           r_alert(level: 2, description: 80,
               where: %{mfa:
                        {:tls_gen_connection_1_3, :handle_key_update, 2},
                          line: 371, file: 'otp/lib/ssl/src/tls_gen_connection_1_3.erl'},
               reason: reason)}
    end
  end
  ...
  defp exporter_secrets(exporterMasterSecret, labels, pRFAlgorithm) do
    deriveSecret = fn label ->
                        :tls_v1.derive_secret(exporterMasterSecret, label, <<>>,
                                                pRFAlgorithm)
                   end
    for label <- labels do
      deriveSecret.(label)
  ...
  defp export_key_materials(secrets, contexts, lengths, pRFAlgorithm) do
    try do
      export_key_materials(secrets, contexts, lengths,
                             pRFAlgorithm, [])
    catch
      _, _ ->
        {:error, :bad_input}
  ...
  defp export_key_materials([exporterSecret | exS], [context0 | contexts],
            [wantedLength | lengths], pRFAlgorithm, acc) do
    exporterKeyMaterial = exporter(exporterSecret, context0,
                                     wantedLength, pRFAlgorithm)
    export_key_materials(exS, contexts, lengths,
                           pRFAlgorithm, [exporterKeyMaterial | acc])
  end

  defp exporter(exporterSecret, context0, wantedLength,
            pRFAlgorithm) do
    context = (case (context0) do
                 :no_context ->
                   <<>>
                 _ ->
                   context0
               end)
    hashContext = :tls_v1.transcript_hash(context,
                                            pRFAlgorithm)
    :tls_v1.hkdf_expand_label(exporterSecret, "exporter",
                                hashContext, wantedLength, pRFAlgorithm)
  end
