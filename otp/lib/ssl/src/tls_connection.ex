defmodule :m_tls_connection do
  ...
  def init([role, sender, host, port, socket, options, user,
                                                         cbInfo]) do
    state0 = initial_state(role, sender, host, port, socket,
                             options, user, cbInfo)
    try do
      state1 = (r_state(static_env: r_static_env(session_cache: cache,
                                  session_cache_cb: cacheCb),
                    connection_env: r_connection_env(cert_key_alts: certKeyAlts),
                    ssl_options: sslOptions,
                    session: session0) = :ssl_gen_statem.init_ssl_config(r_state(state0, :ssl_options),
                                                                           role,
                                                                           state0))
      state = (case (role) do
                 :client ->
      ...
      :gen_statem.enter_loop(:tls_connection, [],
                               :initial_hello, state)
    catch
      error ->
  ...
  def hello(:internal, r_client_hello(extensions: extensions),
  ...
  def hello(:internal,
    ...
    try do
      r_state(ssl_options: sslOpts) = (state1 = :tls_dtls_connection.handle_sni_extension(state0,
                                                                                      hello))
      case (choose_tls_fsm(sslOpts, hello)) do
        ...
        :tls_1_0_to_1_2_fsm ->
          {serverHelloExt, type,
             state} = handle_client_hello(hello, state1)
      ...
      r_alert() = alert ->
        alertState = r_state(state0, connection_env: r_connection_env(cEnv, negotiated_version: clientVersion))
  ...
  def hello(type, event, state) do
    try do
      :tls_dtls_connection.gen_handshake(:hello, type, event,
                                           state)
    catch
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert, :hello, state)
  ...
  def user_hello(type, event, state) do
    try do
      :tls_dtls_connection.gen_handshake(:user_hello, type,
                                           event, state)
    catch
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert, :user_hello,
                                           state)
    end
  end
  ...
  def abbreviated(type, event, state) do
    try do
      :tls_dtls_connection.gen_handshake(:abbreviated, type,
                                           event, state)
    catch
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert, :abbreviated,
                                           state)
    end
  end
  ...
  def wait_ocsp_stapling(type, event, state) do
    try do
      :tls_dtls_connection.gen_handshake(:wait_ocsp_stapling,
                                           type, event, state)
    catch
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert,
  ...
  def certify(type, event, state) do
    try do
      :tls_dtls_connection.gen_handshake(:certify, type,
                                           event, state)
    catch
      r_alert() = alert ->
  ...
  def wait_cert_verify(type, event, state) do
    try do
      :tls_dtls_connection.gen_handshake(:wait_cert_verify,
                                           type, event, state)
    catch
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert,
  ...
  def cipher(type, event, state) do
    try do
      :tls_dtls_connection.gen_handshake(:cipher, type, event,
                                           state)
    catch
      r_alert() = alert ->
  ...
  def connection({:call, from}, {:user_renegotiate, writeState},
           r_state(connection_states: connectionStates) = state) do
    {:next_state, :connection,
       r_state(state, connection_states: Map.put(connectionStates, :current_write,
                                                               writeState)),
  ...
  def connection(:internal, r_hello_request(),
           r_state(static_env: r_static_env(role: :client, host: host, port: port,
               ...
               handshake_env: r_handshake_env(renegotiation: {renegotiation, :peer},
                                  ocsp_stapling_state: ocspState),
               connection_env: r_connection_env(cert_key_alts: certKeyAlts),
               session: session0, ssl_options: sslOpts,
               protocol_specific: %{sender: pid},
               connection_states: connectionStates) = state0) do
    try do
                ...
                :undefined ->
                  :ssl_logger.log(:info, :debug,
                                    ...
                                    %{mfa: {:tls_connection, :connection, 3},
                                        line: 398, file: 'otp/lib/ssl/src/tls_connection.erl'})
                __LogLevel__ ->
      ...
      {:ok, write} ->
        ...
        session = :ssl_session.client_select_session({host,
                                                       ...
                                                       certKeyPairs)
        hello = :tls_handshake.client_hello(host, port,
                                              connectionStates, sslOpts,
                                              r_session(session, :session_id),
                                              renegotiation, :undefined,
                                              :undefined,
                                              :maps.get(:ocsp_nonce, ocspState,
                                                          :undefined),
                                              certDbHandle, certDbRef)
        {state,
           actions} = :tls_gen_connection.send_handshake(hello,
  ...
  def connection(:internal, r_hello_request(),
    ...
    hello = :tls_handshake.client_hello(host, port,
                                          connectionStates, sslOpts, <<>>,
                                          renegotiation, :undefined, :undefined,
                                          :maps.get(:ocsp_nonce, ocspState,
                                                      :undefined),
                                          certDbHandle, certDbRef)
    {state,
  ...
  def connection(:internal, r_client_hello() = hello,
           r_state(static_env: r_static_env(role: :server),
               ...
               protocol_specific: %{sender: sender}) = state) do
    :erlang.send_after(12000, self(), :allow_renegotiate)
    {:ok, write} = :tls_sender.renegotiate(sender)
    :tls_gen_connection.next_event(:hello, :no_record,
                                     r_state(state, connection_states: Map.put(cS, :current_write,
                                                                               write), 
                                                handshake_env: r_handshake_env(hsEnv, renegotiation: {true,
                                                                                          :peer}, 
  ...
  def connection(:internal, r_client_hello(),
    ...
    alert = r_alert(level: 1, description: 100,
                where: %{mfa: {:tls_connection, :connection, 3},
                           line: 442, file: 'otp/lib/ssl/src/tls_connection.erl'})
  ...
  defp initial_state(role, sender, host, port, socket,
    ...
    r_state(static_env: initStatEnv,
        ...
        protocol_specific: %{sender: sender,
                               active_n:
                               :ssl_config.get_internal_active_n(:maps.get(:erl_dist,
                                                                             sSLOptions,
                                                                             false)),
  ...
  defp handle_client_hello(r_client_hello(client_version: clientVersion) = hello,
    ...
    serverHelloExt = (case (sNICertSelection) do
                        true ->
      ...
      _, reason ->
                ...
                :undefined ->
                  :ssl_logger.log(:info, :debug,
                                    ...
                                    %{mfa: {:tls_connection, :gen_info, 3},
                                        line: 576, file: 'otp/lib/ssl/src/tls_connection.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:info, __LogLevel__,
                                    %{description: :internal_error,
                                        reason:
        ...
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_connection,
                                                         :gen_info, 3},
                                                        line: 577, file: 'otp/lib/ssl/src/tls_connection.erl'},
      ...
      _, reason ->
                  ...
                  :ssl_logger.log(:info, :debug,
                                    %{description: :handshake_error,
                                        ...
                                        [{:error, reason}, {:stacktrace,
                                                              __STACKTRACE__}]},
                                    %{mfa: {:tls_connection, :gen_info, 3},
                                        line: 587, file: 'otp/lib/ssl/src/tls_connection.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:info, __LogLevel__,
                                    %{description: :handshake_error,
                                        reason:
        ...
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_connection,
                                                         :gen_info, 3},
                                                        line: 588, file: 'otp/lib/ssl/src/tls_connection.erl'},
                                             reason: :malformed_handshake_data),
                                           stateName, state)
    end
  end

  def choose_tls_fsm(%{versions: versions},
           r_client_hello(extensions: %{client_hello_versions:
                           r_client_hello_versions(versions: clientVersions)})) do
  ...
  def handle_trace(:csp,
           {:call,
              {:tls_connection, :wait_ocsp_stapling,
                 [type, event | _]}},
           stack) do
