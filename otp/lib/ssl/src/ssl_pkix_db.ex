defmodule :m_ssl_pkix_db do
        ...
        case :logger.allow(:notice, :ssl_pkix_db) do
          true ->
            :erlang.apply(:logger, :macro_log, [
              %{
                mfa: {:ssl_pkix_db, :decode_cert, 2},
                line: 347,
                file: ~c"otp/lib/ssl/src/ssl_pkix_db.erl"
              },
              :notice,
              ~c"SSL WARNING: Ignoring a CA cert as it could not be correctly decoded.~n"
            ])
  ...
  def add_crls([_, _, _, {_, mapping} | _], ~c"http://dummy/no_distribution_point", cRLs) do
    for cRL <- cRLs do
  ...
  def remove_crls([_, _, _, {_, mapping} | _], {~c"http://dummy/no_distribution_point", cRLs}) do
    for cRL <- cRLs do
      rm_crls(cRL, mapping)
    end
  end
