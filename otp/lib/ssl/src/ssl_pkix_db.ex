defmodule :m_ssl_pkix_db do
  ...
  def create(pEMCacheName) do
    [:ets.new(:ssl_otp_cacertificate_db,
                [:set, :public, {:read_concurrency, true}]),
         {:ets.new(:ssl_otp_ca_file_ref, [:set, :public]),
            :ets.new(:ssl_otp_ca_ref_file_mapping,
                       [:set, :protected])},
             pEMCacheName, {:ets.new(:ssl_otp_crl_cache,
                                       [:set, :protected]),
                              :ets.new(:ssl_otp_crl_issuer_mapping,
                                         [:bag, :protected])}]
  end
  ...
  def lookup_trusted_cert(_DbHandle, {:extracted, certs}, serialNumber,
           issuer) do
    try do
      for {:decoded,
             {{_Ref, certSerial, certIssuer}, cert}} <- certs,
            certSerial === serialNumber, certIssuer === issuer do
        throw(cert)
      end
      :undefined
    catch
  ...
  def refresh_trusted_certs(file, [certsDb, {_, fileMapDb} | _], pemCache) do
    case (lookup(file, fileMapDb)) do
      [ref] ->
        certs = :ssl_certificate.file_to_certificats(file,
                                                       pemCache)
  ...
  def refresh_trusted_certs([_, {_, fileMapDb} | _] = db, pemCache) do
    refresh = fn {file, _}, acc ->
  ...
  def extract_trusted_certs({:der_otp, derList}) do
    {:ok, {:extracted, certs_from_der(derList)}}
  end
  ...
  def extract_trusted_certs(file) do
    case (:file.read_file(file)) do
      {:ok, pemBin} ->
        ...
        derList = (for {:Certificate, cert,
                          :not_encrypted} <- content do
                     cert
                   end)
        {:ok, {:extracted, certs_from_der(derList)}}
      error ->
  ...
  def select_certentries_by_ref(ref, cache) do
    :ets.select(cache, [{{{ref, :_, :_}, :_}, [], [:"$_"]}])
  end

  def select_certs_by_ref(ref, cache) do
  ...
  defp add_certs_from_pem(pemEntries, ref, certsDb) do
    ...
    for {:Certificate, cert,
           :not_encrypted} <- pemEntries do
      add.(cert)
    end
    :ok
  end

  defp add_cert(cert, ref, certsDb) do
    try do
  ...
  defp decode_cert(ref, r_cert(otp: erlCert) = cert) do
  ...
  defp decode_cert(ref, der) do
    try do
        ...
        case (:logger.allow(:notice, :ssl_pkix_db)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:ssl_pkix_db, :decode_cert, 2}, line: 347,
                                 file: 'otp/lib/ssl/src/ssl_pkix_db.erl'},
                                 :notice, 'SSL WARNING: Ignoring a CA cert as it could not be correctly decoded.~n'])
          false ->
            :ok
        end
        :undefined
      ...
      erlCert ->
        decode_cert(ref, r_cert(der: der, otp: erlCert))
    end
  end
  ...
  defp update_certs(ref, certList, keyList, certsDb) do
    {insert, delete} = insert_delete_lists(ref, certList,
                                             certsDb, [], keyList)
    insert_cert_entries(insert, certsDb)
    remove_cert_entries(delete, certsDb)
  end

  defp insert_delete_lists(_, [], _, insert, delete) do
    {insert, delete}
  end

  defp insert_delete_lists(ref, [cert | rest], certsDb, insert, delete) do
    case (decode_cert(ref, cert)) do
      {:decoded, {key, value} = entry} ->
        case (lookup(key, certsDb)) do
          [^value] ->
            insert_delete_lists(ref, rest, certsDb, insert,
                                  :lists.keydelete(key, 1, delete))
          _ ->
            insert_delete_lists(ref, rest, certsDb,
                                  [entry | insert],
                                  :lists.keydelete(key, 1, delete))
        end
      :undefined ->
  ...
  defp insert_cert_entries(entryList, certsDb) do
    :ets.insert(certsDb, entryList)
  end

  defp remove_cert_entries(entryList, certsDb) do
    :lists.foreach(fn {key, value} ->
                        remove(key, value, certsDb)
                   end,
                     entryList)
  end
