defmodule :m_dtls_socket do
  ...
  def listen(port,
           r_config(inet_ssl: sockOpts, ssl: sslOpts, emulated: emOpts,
               inet_user: options) = config) do
    iP = :proplists.get_value(:ip, sockOpts,
                                default_ip(sockOpts))
    case (:dtls_listener_sup.lookup_listener(iP, port)) do
      ...
      {:ok, listener} ->
        ...
        :dtls_packet_demux.set_all_opts(listener,
                                          {options,
  ...
  def accept(:dtls,
           ...
           _Timeout) do
    case (:dtls_packet_demux.accept(listener, self())) do
      {:ok, pid, socket} ->
        {:ok,
           socket([pid], transport, {listener, socket},
                    connectionCb)}
      {:error, reason} ->
        {:error, reason}
    end
  ...
  def connect(address, port,
           ...
           timeout) do
    case (transport.open(0,
                           socketOpts ++ internal_inet_values())) do
      {:ok, socket} ->
  ...
  def close(r_sslsocket(pid: {:dtls,
                   r_config(dtls_handler: {pid, port0}, inet_ssl: sockOpts)})) do
    iP = :proplists.get_value(:ip, sockOpts,
                                default_ip(sockOpts))
    port = get_real_port(pid, port0)
    :dtls_listener_sup.register_listener({:undefined, pid},
                                           iP, port)
    :dtls_packet_demux.close(pid)
  end

  defp default_ip(sockOpts) do
    case (:proplists.get_value(:inet6, sockOpts, false)) do
      false ->
        {0, 0, 0, 0}
      true ->
        {0, 0, 0, 0, 0, 0, 0, 0}
  ...
  def socket(pids, :gen_udp = transport,
           peerAndSock = {{_Host, _Port}, _Socket},
           connectionCb) do
  ...
  def setopts(_,
           socket = r_sslsocket(pid: {:dtls,
                              r_config(dtls_handler: {listenPid, _})}),
           options) do
    splitOpts = ({_,
                    emOpts} = :tls_socket.split_options(options))
  ...
  defp check_active_n(emulatedOpts,
            socket = r_sslsocket(pid: {:dtls,
                               r_config(dtls_handler: {listenPid, _})})) do
    case (:proplists.lookup(:active, emulatedOpts)) do
      ...
      {_, n} when is_integer(n) ->
        {:ok, r_socket_options(active: active),
           _} = :dtls_packet_demux.get_all_opts(listenPid)
        case (active) do
  ...
  def getopts(:gen_udp, r_sslsocket(pid: {socket, r_config(emulated: emOpts)}),
           options) do
    {sockOptNames,
       emulatedOptNames} = :tls_socket.split_options(options)
    emulatedOpts = get_emulated_opts(emOpts,
                                       emulatedOptNames)
    socketOpts = :tls_socket.get_socket_opts(socket,
                                               sockOptNames, :inet)
    {:ok, emulatedOpts ++ socketOpts}
  end

  def getopts(_Transport,
           r_sslsocket(pid: {socket, r_config(emulated: emOpts)}), options) do
    {sockOptNames,
       emulatedOptNames} = :tls_socket.split_options(options)
    emulatedOpts = get_emulated_opts(emOpts,
                                       emulatedOptNames)
  ...
  def default_inet_values() do
    [{:active, true}, {:mode, :list}, {:packet, 0},
                                          {:packet_size, 0}]
  end
  ...
  defp emulated_socket_options(inetValues,
            r_socket_options(mode: mode, packet: packet, packet_size: packetSize,
                active: active)) do
  ...
  defp get_real_port(listener, port0)
    ...
    case (port0) do
      0 ->
        {:ok,
           {_, newPort}} = :dtls_packet_demux.sockname(listener)
        newPort
      _ ->
        port0
    end
  ...
  defp start_new_listener(iP, port0, owner,
            r_config(transport_info: {transportModule, _, _, _, _},
                inet_user: options) = config) do
  ...
  defp start_dtls_packet_demux(r_config(transport_info: {transportModule, _, _, _,
                               _} = transportInfo,
              emulated: emOpts0, ssl: sslOpts) = config,
            iP, port, socket, owner) do
    emOpts = emulated_socket_options(emOpts0, r_socket_options())
    case (:dtls_listener_sup.start_child([owner, port,
                                                     transportInfo, emOpts,
                                                                        sslOpts,
                                                                            socket])) do
      {:ok, multiplexer} ->
        :ok = transportModule.controlling_process(socket,
                                                    multiplexer)
        :dtls_listener_sup.register_listener({self(),
                                                multiplexer},
                                               iP, port)
        dTLSSocket = create_dtls_socket(config, multiplexer,
                                          port)
        {:ok, dTLSSocket}
      error ->
        error
  ...
  defp create_dtls_socket(r_config(emulated: emOpts) = config, listener, port) do
    socket = r_sslsocket(pid: {:dtls,
                       r_config(config, dtls_handler: {listener, port})})
    check_active_n(emOpts, socket)
    socket
  end
