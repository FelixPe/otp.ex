defmodule :m_tls_bloom_filter do
  ...
  def add_elem(%{k: k, m: m, current: bitField0} = bloomFilter,
           elem) do
    hash = hash(elem, k, m)
  ...
  def contains(%{k: k, m: m, current: bFCurrent, old: bFOld},
           elem) do
    hash = hash(elem, k, m)
    :lists.all(fn pos ->
                    bit_is_set(bFCurrent, pos)
               end,
                 hash) or :lists.all(fn pos ->
  ...
  defp set_bit(bitField, b) do
    <<front :: size(b), _ :: size(1),
        rest :: bitstring>> = bitField
  ...
  defp hash(elem, k, m, acc) do
    h = rem(:erlang.phash2({elem, 0},
                             m) + (k - 1) * :erlang.phash2({elem, 1}, m), m)
