defmodule :m_tls_server_session_ticket do
  ...
  Record.defrecord(:r_cipher_state, :cipher_state, iv: :undefined,
                                        ...
                                        tag_len: :undefined)
  Record.defrecord(:r_state, :state, stateless: :undefined,
                                 stateful: :undefined, nonce: :undefined,
                                 lifetime: :undefined,
                                 max_early_data_size: :undefined,
                                 listen_monitor: :undefined)
  def start_link(listener, mode1, lifetime, ticketStoreSize,
           maxEarlyDataSize, antiReplay, seed) do
    mode = (case (mode1) do
              :stateful_with_cert ->
                :stateful
              :stateless_with_cert ->
    ...
    :gen_server.start_link(:tls_server_session_ticket,
                             [listener, mode, lifetime, ticketStoreSize,
                                                            maxEarlyDataSize,
                                                                antiReplay,
                                                                    seed],
                             [])
  ...
  def handle_call({:new_session_ticket, prf, masterSecret,
            peerCert},
           _From,
           r_state(nonce: nonce, lifetime: lifeTime,
               max_early_data_size: maxEarlyDataSize,
               stateful: %{id_generator: idGen}) = state0) do
    id = stateful_psk_ticket_id(idGen)
    pSK = :tls_v1.pre_shared_key(masterSecret,
                                   ticket_nonce(nonce), prf)
    sessionTicket = new_session_ticket(id, nonce, lifeTime,
                                         maxEarlyDataSize)
    state = stateful_ticket_store(id, sessionTicket, prf,
                                    pSK, peerCert, state0)
  ...
  def handle_call({:new_session_ticket, prf, masterSecret,
            peerCert},
           _From, r_state(nonce: nonce, stateless: %{}) = state) do
    baseSessionTicket = new_session_ticket_base(state)
    sessionTicket = generate_stateless_ticket(baseSessionTicket,
                                                prf, masterSecret, peerCert,
                                                state)
    {:reply, sessionTicket, r_state(state, nonce: nonce + 1)}
  end

  def handle_call({:use_ticket, identifiers, prf, handshakeHist},
           _From, r_state(stateful: %{}) = state0) do
  ...
  def handle_call({:use_ticket, identifiers, prf, handshakeHist},
           _From, r_state(stateless: %{}) = state0) do
  ...
  def handle_info(:rotate_bloom_filters,
           r_state(stateless: %{bloom_filter: bloomFilter0,
                            warm_up_windows_remaining: warmUp0,
                            window: window} = stateless) = state) do
    bloomFilter = :tls_bloom_filter.rotate(bloomFilter0)
    :erlang.send_after(window * 1000, self(),
                         :rotate_bloom_filters)
    warmUp = max(warmUp0 - 1, 0)
    {:noreply,
       r_state(state, stateless: Map.merge(stateless, %{bloom_filter:
                                                  bloomFilter,
                                                    warm_up_windows_remaining:
                                                    warmUp}))}
  end

  def handle_info({:DOWN, monitor, _, _, _},
           r_state(listen_monitor: monitor) = state) do
    {:stop, :normal, state}
  end
  ...
  defp initial_state([:stateless, lifetime, _, maxEarlyDataSize,
                                        :undefined, seed]) do
    r_state(nonce: 0,
  ...
  defp initial_state([:stateless, lifetime, _, maxEarlyDataSize,
    ...
    r_state(nonce: 0,
        stateless: %{bloom_filter: :tls_bloom_filter.new(k, m),
                       ...
                       seed: stateless_seed(seed), window: window},
        lifetime: lifetime,
        max_early_data_size: maxEarlyDataSize)
  end

  defp initial_state([:stateful, lifetime, ticketStoreSize,
                                    maxEarlyDataSize | _]) do
    r_state(lifetime: lifetime,
        max_early_data_size: maxEarlyDataSize, nonce: 0,
        stateful: %{db: stateful_store(), max: ticketStoreSize,
                      ref_index: %{},
  ...
  defp ticket_age_add() do
    ...
    <<i
      ...
      size(32) - unsigned - big -
        integer>> = :crypto.strong_rand_bytes(4)
    case (i > maxAgeAdd) do
      true ->
        i - maxTicketAge
      false ->
  ...
  defp new_session_ticket_base(r_state(nonce: nonce, lifetime: lifetime,
              max_early_data_size: maxEarlyDataSize)) do
  ...
  defp new_session_ticket(id, nonce, lifetime, maxEarlyDataSize) do
    ...
    extensions = %{early_data:
                   r_early_data_indication_nst(indication: maxEarlyDataSize)}
    r_new_session_ticket(ticket: id, ticket_lifetime: lifetime,
  ...
  defp validate_binder(binder, handshakeHist, pSK, prf, alertDetail) do
    case (:tls_handshake_1_3.is_valid_binder(binder,
                                               handshakeHist, pSK, prf)) do
      true ->
        true
      false ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_server_session_ticket, :validate_binder, 5},
                          line: 236, file: 'otp/lib/ssl/src/tls_server_session_ticket.erl'},
               reason: alertDetail)}
    end
  end
  ...
  defp stateful_ticket_store(ref, newSessionTicket, hash, psk, peerCert,
            r_state(nonce: nonce,
                stateful: %{db: tree0, max: max,
                              ref_index: index0} = stateful) = state0) do
    id = {:erlang.monotonic_time(),
            :erlang.unique_integer([:monotonic])}
    statefulTicket = {newSessionTicket, hash, psk, peerCert}
    case (:gb_trees.size(tree0)) do
      ^max ->
        {_, {r_new_session_ticket(ticket: oldRef), _, _, _},
           tree1} = :gb_trees.take_smallest(tree0)
        tree = :gb_trees.insert(id, statefulTicket, tree1)
        index = :maps.without([oldRef], index0)
        r_state(state0, nonce: nonce + 1, 
                    stateful: Map.merge(stateful, %{db: tree,
                                                      ref_index:
                                                      Map.put(index, ref, id)}))
      _ ->
  ...
  defp stateful_use(r_offered_psks(identities: identities, binders: binders),
            prf, handshakeHist, state) do
    stateful_use(identities, binders, prf, handshakeHist, 0,
                   state)
  end
  ...
  defp stateful_use([r_psk_identity(identity: ref) | refs], [binder | binders],
            prf, handshakeHist, index,
            r_state(stateful: %{db: tree0,
                            ref_index: refIndex0} = stateful) = state) do
    try do
      :maps.get(ref, refIndex0)
    catch
      _, {:badkey, ^ref} ->
        stateful_use(refs, binders, prf, handshakeHist,
                       index + 1, state)
    else
      key ->
        case (stateful_usable_ticket(key, prf, binder,
                                       handshakeHist, tree0)) do
          true ->
            refIndex = :maps.without([ref], refIndex0)
            {{_, _, pSK, peerCert}, tree} = :gb_trees.take(key,
                                                             tree0)
            {{:ok, {index, pSK, peerCert}},
               r_state(state, stateful: Map.merge(stateful, %{db: tree,
                                                          ref_index:
                                                          refIndex}))}
          false ->
            stateful_use(refs, binders, prf, handshakeHist,
    ...
    case (:gb_trees.lookup(key, tree)) do
      ...
      {:value, {newSessionTicket, ^prf, pSK, _PeerCert}} ->
        case (stateful_living_ticket(key, newSessionTicket)) do
          true ->
            validate_binder(binder, handshakeHist, pSK, prf,
                              :stateful)
          _ ->
            false
        end
  ...
  defp stateful_living_ticket({timeStamp, _}, r_new_session_ticket(ticket_lifetime: lifeTime)) do
    ...
    lived = :erlang.convert_time_unit(now - timeStamp,
                                        :native, :seconds)
    lived < lifeTime
  end
  ...
  defp stateful_psk_ticket_id(key) do
    ...
    :crypto.crypto_one_time(:aes_128_ecb, key,
                              <<unique :: size(128)>>, true)
  end

  defp generate_stateless_ticket(r_new_session_ticket(ticket_nonce: nonce,
              ticket_age_add: ticketAgeAdd,
              ticket_lifetime: lifetime) = ticket,
            prf, masterSecret, peerCert,
            r_state(stateless: %{seed: {iV, shard}})) do
    pSK = :tls_v1.pre_shared_key(masterSecret, nonce, prf)
    timestamp = :erlang.system_time(:second)
    encrypted = :ssl_cipher.encrypt_ticket(r_stateless_ticket(hash: prf,
                                               pre_shared_key: pSK,
                                               ticket_age_add: ticketAgeAdd,
                                               lifetime: lifetime,
                                               timestamp: timestamp,
  ...
  defp stateless_use(r_offered_psks(identities: identities, binders: binders),
            prf, handshakeHist, state) do
    stateless_use(identities, binders, prf, handshakeHist,
                    0, state)
  end
  ...
  defp stateless_use([r_psk_identity(identity: encrypted,
               obfuscated_ticket_age: obfAge) |
               ids],
            [binder | binders], prf, handshakeHist, index,
            r_state(stateless: %{seed: {iV, shard},
                             window: window}) = state) do
    case (:ssl_cipher.decrypt_ticket(encrypted, shard,
                                       iV)) do
      r_stateless_ticket(hash: ^prf, pre_shared_key: pSK,
          certificate: peerCert) = ticket ->
        case (stateless_usable_ticket(ticket, obfAge, binder,
                                        handshakeHist, window)) do
          true ->
  ...
  defp stateless_usable_ticket(r_stateless_ticket(hash: prf, ticket_age_add: ticketAgeAdd,
              lifetime: lifetime, timestamp: timestamp,
              pre_shared_key: pSK),
            obfAge, binder, handshakeHist, window) do
    case (stateless_living_ticket(obfAge, ticketAgeAdd,
                                    lifetime, timestamp, window)) do
      true ->
        validate_binder(binder, handshakeHist, pSK, prf,
                          :stateless)
      false ->
  ...
  defp stateless_living_ticket(obfAge, ticketAgeAdd, lifetime, timestamp,
            window) do
    realAge = :erlang.system_time(:second) - timestamp
    reportedAge = obfAge - ticketAgeAdd
    deltaAge = abs(realAge - reportedAge / 1000)
    reportedAge <= lifetime * 1000 and realAge <= lifetime and in_window(deltaAge,
                                                                           window)
  end
  ...
  defp stateless_anti_replay(_Index, _PSK, _Binder, _PeerCert,
            r_state(stateless: %{warm_up_windows_remaining:
                           warmUpRemaining}) = state)
      when warmUpRemaining > 0 do
    {{:ok, :undefined}, state}
  end

  defp stateless_anti_replay(index, pSK, binder, peerCert,
            r_state(stateless: %{bloom_filter:
                           bloomFilter0} = stateless) = state) do
    case (:tls_bloom_filter.contains(bloomFilter0,
                                       binder)) do
      true ->
        {{:ok, :undefined}, state}
      false ->
        bloomFilter = :tls_bloom_filter.add_elem(bloomFilter0,
                                                   binder)
        {{:ok, {index, pSK, peerCert}},
           r_state(state, stateless: Map.put(stateless, :bloom_filter,
                                                    bloomFilter))}
    end
  end
  ...
  defp stateless_seed(seed) do
    <<iV :: size(16) - binary, shard :: size(32) - binary,
        _ :: binary>> = :crypto.hash(:sha512, seed)
    {iV, shard}
  end
  ...
  def handle_trace(:rle,
           {:call,
              {:tls_server_session_ticket, :init,
                 [[listenSocket, mode, lifetime, storeSize | _T]]}},
           stack) do
    {:io_lib.format('(*server) ([ListenSocket = ~w Mode = ~w Lifetime = ~w StoreSize = ~w, ...])',
                      [listenSocket, mode, lifetime, storeSize]),
       [{:role, :server} | stack]}
  end

  def handle_trace(:ssn,
           {:call,
              {:tls_server_session_ticket, :terminate,
                 [reason, _State]}},
           stack) do
  ...
  def handle_trace(:ssn,
           {:call,
              {:tls_server_session_ticket, :handle_call,
                 [callTuple, _From, _State]}},
           stack) do
    {:io_lib.format('(Call = ~w)', [:erlang.element(1, callTuple)]),
       stack}
  end
  ...
  def handle_trace(:ssn,
           {:call,
              {:tls_server_session_ticket, :handle_call,
                 [{call = :use_ticket,
                     {:offered_psks, [{:psk_identity, pskIdentity, _ObfAge}],
                        [binder]},
                     _Prf, _HandshakeHist},
                      _From, _State]}},
           stack) do
    {:io_lib.format('(Call = ~w PskIdentity = ~W Binder = ~W)', [call, pskIdentity, 5, binder, 5]),
       stack}
  end
  ...
  def handle_trace(:ssn,
           {:call,
              {:tls_server_session_ticket, :stateless_use,
                 [[{:psk_identity, encrypted, _ObfAge} | _], [binder |
                                                                  _],
                                                                 _Prf,
                                                                     _HandshakeHist,
                                                                         _Index,
  ...
  def handle_trace(:ssn,
           {:call,
              {:tls_server_session_ticket, :in_window,
                 [realAge, window]}},
           stack) do
    {:io_lib.format('(RealAge = ~w Window = ~w)', [realAge, window]), stack}
  end
           ...
           {:call,
              {:tls_server_session_ticket, :stateless_usable_ticket,
                 [{:stateless_ticket, _Prf, _PreSharedKey, _TicketAgeAdd,
                     _Lifetime, _Timestamp},
                      _ObfAge, binder, _HandshakeHist, window]}},
           stack) do
  ...
  def handle_trace(:ssn,
           {:call,
              {:tls_server_session_ticket, :stateless_anti_replay,
                 [_Index, pSK, _Binder, _State]}},
           stack) do
  ...
  def handle_trace(:ssn,
           {:return_from,
              {:tls_server_session_ticket, :stateless_use, 6},
  ...
  def handle_trace(:ssn,
           {:return_from,
              {:tls_server_session_ticket, :generate_stateless_ticket,
                 4},
              {:new_session_ticket, _LifeTime, _AgeAdd, _Nonce,
                 ticket, _Extensions}},
           stack) do
  ...
  def handle_trace(:ssn,
           {:return_from,
              {:tls_server_session_ticket, :initial_state, 1},
              {:state,
                 _Stateless = %{seed: {iV, shard}, window: window},
                 _Stateful = :undefined, _Nonce, _Lifetime,
                 _MaxEarlyDataSize, listenMonitor}},
           stack) do
  ...
  def handle_trace(:ssn,
           {:return_from,
              {:tls_server_session_ticket, :stateless_anti_replay, 4},
              {{:ok, {_Index, pSK}}, _State}},
           stack) do
