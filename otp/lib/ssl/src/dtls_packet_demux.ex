defmodule :m_dtls_packet_demux do
  ...
  def start_link(owner, port, transportInfo, emOpts, inetOptions, dTLSOptions) do
    :gen_server.start_link(
      :dtls_packet_demux,
      [owner, port, transportInfo, emOpts, inetOptions, dTLSOptions],
      []
    )
  end
  ...
  def init([owner, port0, transportInfo, emOpts, dTLSOptions, socket]) do
  ...
  def handle_call({:new_owner, owner}, _, state) do
    {:reply, :ok, r_state(state, close: false, first: true, owner: owner)}
  end

  def handle_call(
        {:new_connection, old, _Pid},
        _,
        r_state(
          accepters: accepters,
          dtls_msq_queues: msgQs0
        ) = state
      ) do
  ...
  def handle_cast(
        {:connection_setup, client},
        r_state(dtls_msq_queues: msgQueues) = state
      ) do
  ...
  def handle_info(
    ...
    report = :io_lib.format(~c"Ignore SSL UDP Listener: Socket error: ~p ~n", [error])
      ...
      true ->
        :erlang.apply(:logger, :macro_log, [
          %{
            ...
            line: 223,
            file: ~c"otp/lib/ssl/src/dtls_packet_demux.erl"
          },
          :notice,
          report
        ])
  ...
  def handle_info(
    ...
    report = :io_lib.format(~c"SSL Packet muliplexer shutdown: Socket error: ~p ~n", [error])
  ...
  def handle_info(
    ...
    state =
      case kv_lookup(client, msgQueues0) do
        :none ->
  ...
  defp dispatch({pid, queue0}, client, msg, r_state(dtls_msq_queues: msgQueues) = state) do
    case :queue.out(queue0) do
      {{:value, ^pid}, queue} when is_pid(pid) ->
        send(pid, msg)
        r_state(state, dtls_msq_queues: kv_update(client, {pid, queue}, msgQueues))
  ...
  defp handle_active_once(client, pid, r_state(dtls_msq_queues: msgQueues) = state0) do
    ...
    case :queue.out(queue0) do
      ...
      {{:value, msg}, queue} ->
        send(pid, msg)
        r_state(state0, dtls_msq_queues: kv_update(key, {pid, queue}, msgQueues))
