defmodule :m_dtls_packet_demux do
  ...
  Record.defrecord(:r_ticket_data, :ticket_data, key: :undefined,
                                       ...
                                       max_size: :undefined)
  Record.defrecord(:r_state, :state, active_n: :undefined,
                                 port: :undefined, listener: :undefined,
                                 transport: :undefined,
                                 dtls_options: :undefined,
                                 emulated_options: :undefined,
                                 dtls_msq_queues: kv_new(),
                                 dtls_processes: kv_new(),
                                 accepters: :queue.new(), owner: :undefined,
                                 first: :undefined, close: :undefined,
                                 session_id_tracker: :undefined)
  def start_link(owner, port, transportInfo, emOpts, inetOptions,
           dTLSOptions) do
    :gen_server.start_link(:dtls_packet_demux,
                             [owner, port, transportInfo, emOpts, inetOptions,
                                                                      dTLSOptions],
                             [])
  end

  def active_once(packetSocket, client, pid) do
    :gen_server.cast(packetSocket,
  ...
  def new_owner(packetSocket, owner) do
    call(packetSocket, {:new_owner, owner})
  end

  def new_connection(packetSocket, client) do
    call(packetSocket, {:new_connection, client, self()})
  end

  def connection_setup(packetSocket, client) do
    :gen_server.cast(packetSocket,
                       {:connection_setup, client})
  end
  ...
  def init([owner, port0, transportInfo, emOpts,
                                           dTLSOptions, socket]) do
    internalActiveN = get_internal_active_n()
    :erlang.monitor(:process, owner)
    {:ok, sessionIdHandle} = session_id_tracker(socket,
                                                  dTLSOptions)
    {:ok,
       r_state(active_n: internalActiveN, port: port0, first: true,
           transport: transportInfo, dtls_options: dTLSOptions,
           emulated_options: emOpts, listener: socket,
           owner: owner, close: false,
  ...
  def handle_call({:accept, accepter}, from,
           r_state(active_n: n, first: true, accepters: accepters,
               listener: socket) = state0) do
    next_datagram(socket, n)
    state = r_state(state0, first: false, 
                        accepters: :queue.in({accepter, from}, accepters))
    {:noreply, state}
  end

  def handle_call({:accept, accepter}, from,
           r_state(accepters: accepters) = state0) do
    state = r_state(state0, accepters: :queue.in({accepter, from},
                                             accepters))
    {:noreply, state}
  end
  ...
  def handle_call(:close, _, state0) do
    case (do_close(state0)) do
      {:stop, state} ->
  ...
  def handle_call({:new_owner, owner}, _, state) do
    {:reply, :ok,
       r_state(state, close: false,  first: true,  owner: owner)}
  end

  def handle_call({:new_connection, old, _Pid}, _,
           r_state(accepters: accepters,
               dtls_msq_queues: msgQs0) = state) do
    case (:queue.is_empty(accepters)) do
      false ->
        case (kv_lookup(old, msgQs0)) do
          {:value, oldQueue} ->
            msgQs1 = kv_delete(old, msgQs0)
            msgQs = kv_insert({:old, old}, oldQueue, msgQs1)
            {:reply, true, r_state(state, dtls_msq_queues: msgQs)}
          :none ->
            {:reply, true, state}
        end
      true ->
  ...
  def handle_call({:get_sock_opts, {socketOptNames, emOptNames}},
           _,
           r_state(listener: socket,
               emulated_options: emOpts) = state) do
    case (get_socket_opts(socket, socketOptNames)) do
      {:ok, opts} ->
        {:reply,
           {:ok,
              emulated_opts_list(emOpts, emOptNames, []) ++ opts},
           state}
      {:error, reason} ->
  ...
  def handle_call(:get_all_opts, _,
           r_state(dtls_options: dTLSOptions,
               emulated_options: emOpts) = state) do
    {:reply, {:ok, emOpts, dTLSOptions}, state}
  end

  def handle_call({:set_sock_opts, {socketOpts, newEmOpts}}, _,
           r_state(listener: socket,
               emulated_options: emOpts0) = state) do
  ...
  def handle_call({:set_all_opts,
            {socketOpts, newEmOpts, sslOpts}},
           _, r_state(listener: socket) = state) do
    set_socket_opts(socket, socketOpts)
    {:reply, :ok,
       r_state(state, emulated_options: newEmOpts, 
                  dtls_options: sslOpts)}
  end

  def handle_call({:getstat, options}, _,
           r_state(listener: socket,
               transport: {transportCb, _, _, _, _}) = state) do
    stats = :dtls_socket.getstat(transportCb, socket,
                                   options)
    {:reply, stats, state}
  end
  ...
  def handle_cast({:connection_setup, client},
           r_state(dtls_msq_queues: msgQueues) = state) do
    case (kv_lookup({:old, client}, msgQueues)) do
      :none ->
        {:noreply, state}
      {:value, {pid, _}} ->
        send(pid, {:socket_reused, client})
        {:noreply, state}
    end
  end

  def handle_info({transport, socket, iP, inPortNo, _} = msg,
           r_state(listener: socket,
               transport: {_, transport, _, _, _}) = state0) do
    state = handle_datagram({iP, inPortNo}, msg, state0)
    {:noreply, state}
  end

  def handle_info({passiveTag, socket},
           r_state(active_n: n, listener: socket,
               transport: {_, _, _, _, passiveTag}) = state) do
    next_datagram(socket, n)
    {:noreply, state}
  end

  def handle_info({:udp_error, socket, :econnreset = error},
           r_state(listener: socket,
               transport: {_, _, _, :udp_error, _}) = state) do
      ...
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:dtls_packet_demux, :handle_info, 2},
                             line: 223, file: 'otp/lib/ssl/src/dtls_packet_demux.erl'},
                             :notice, report])
      false ->
        :ok
    end
  ...
  def handle_info({errorTag, socket, error},
           r_state(listener: socket,
               transport: {_, _, _, errorTag, _}) = state) do
    report = :io_lib.format('SSL Packet muliplexer shutdown: Socket error: ~p ~n', [error])
    case (:logger.allow(:notice, :dtls_packet_demux)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:dtls_packet_demux, :handle_info, 2},
                             line: 227, file: 'otp/lib/ssl/src/dtls_packet_demux.erl'},
                             :notice, report])
      false ->
        :ok
    end
  ...
  def handle_info({:DOWN, _, :process, owner, _},
           r_state(owner: owner) = state0) do
    case (do_close(state0)) do
      {:stop, state} ->
        {:stop, :normal, state}
      {:wait, state} ->
  ...
  def handle_info({:DOWN, _, :process, pid, _},
           r_state(dtls_processes: processes0,
               dtls_msq_queues: msgQueues0,
               close: listenClosed) = state0) do
    client = kv_get(pid, processes0)
    processes = kv_delete(pid, processes0)
    state = (case (kv_lookup(client, msgQueues0)) do
               :none ->
                 msgQueues1 = kv_delete({:old, client}, msgQueues0)
                 r_state(state0, dtls_processes: processes, 
                             dtls_msq_queues: msgQueues1)
               {:value, {^pid, _}} ->
                 msgQueues1 = kv_delete(client, msgQueues0)
                 case (kv_lookup({:old, client}, msgQueues1)) do
                   :none ->
                     r_state(state0, dtls_processes: processes, 
                                 dtls_msq_queues: msgQueues1)
                   {:value, old} ->
  ...
  defp do_close(r_state(dtls_processes: processes,
              accepters: accepters) = state) do
    case (kv_empty(processes)) do
      true ->
        {:stop, r_state(state, close: true)}
      false ->
        :lists.foreach(fn {_, from} ->
                            :gen_server.reply(from, {:error, :closed})
                       end,
                         :queue.to_list(accepters))
        {:wait, r_state(state, close: true,  accepters: :queue.new())}
    end
  end

  defp handle_datagram(client, msg,
            r_state(dtls_msq_queues: msgQueues,
                accepters: acceptorsQueue0) = state) do
  ...
  defp dispatch({pid, queue0}, client, msg,
            r_state(dtls_msq_queues: msgQueues) = state) do
    case (:queue.out(queue0)) do
      {{:value, ^pid}, queue} when is_pid(pid) ->
        send(pid, msg)
        r_state(state, dtls_msq_queues: kv_update(client,
                                              {pid, queue}, msgQueues))
      {{:value, _UDP}, _Queue} ->
        r_state(state, dtls_msq_queues: kv_update(client,
                                              {pid, :queue.in(msg, queue0)},
                                              msgQueues))
      {:empty, queue} ->
        r_state(state, dtls_msq_queues: kv_update(client,
                                              {pid, :queue.in(msg, queue)},
                                              msgQueues))
    end
  end
  ...
  defp handle_active_once(client, pid,
            r_state(dtls_msq_queues: msgQueues) = state0) do
    {key, queue0} = (case (kv_lookup(client, msgQueues)) do
                       {:value, {^pid, q0}} ->
                         {client, q0}
                       _ ->
                         oldKey = {:old, client}
    ...
    case (:queue.out(queue0)) do
      ...
      {{:value, msg}, queue} ->
        send(pid, msg)
        r_state(state0, dtls_msq_queues: kv_update(key, {pid, queue},
                                               msgQueues))
      {:empty, ^queue0} ->
        r_state(state0, dtls_msq_queues: kv_update(key,
                                               {pid, :queue.in(pid, queue0)},
                                               msgQueues))
    end
  end

  defp setup_new_connection(user, from, client, msg,
            r_state(dtls_processes: processes, dtls_msq_queues: msgQueues,
                dtls_options: dTLSOpts, port: port, listener: socket,
                session_id_tracker: tracker,
                emulated_options: emOpts) = state) do
    connArgs = [:server, 'localhost', port, {self(),
                                     {client, socket}},
                                      {dTLSOpts, emOpts,
                                         [{:session_id_tracker, tracker}]},
    ...
    case (:dtls_connection_sup.start_child(connArgs)) do
      {:ok, pid} ->
        ...
        send(pid, msg)
        r_state(state, dtls_msq_queues: kv_insert(client,
                                              {pid, :queue.new()}, msgQueues), 
  ...
  defp do_set_emulated_opts([{:active, n0} | rest],
  ...
  defp emulated_opts_list(opts, [:mode | rest], acc) do
    emulated_opts_list(opts, rest,
                         [{:mode, r_socket_options(opts, :mode)} | acc])
  end

  defp emulated_opts_list(opts, [:active | rest], acc) do
    emulated_opts_list(opts, rest,
                         [{:active, r_socket_options(opts, :active)} | acc])
  end

  defp session_id_tracker(listener, _) do
