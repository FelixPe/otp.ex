defmodule :m_ssl_session do
  ...
  def is_new(
        r_session(session_id: sessionId, is_resumable: true),
        sessionId
      ) do
    false
  end
  ...
  def client_select_session(
        {_, _, %{versions: versions, protocol: protocol} = opts} = clientInfo,
        cache,
        cacheCb,
        newSession,
        certKeyPairs
  ...
  def server_select_session(_, sessIdTracker, <<>>, _SslOpts, _CertKeyPairs) do
    {:ssl_server_session_cache.new_session_id(sessIdTracker), :undefined}
  end

  def server_select_session(_, sessIdTracker, suggestedId, options, certKeyPairs) do
    case is_resumable(suggestedId, sessIdTracker, options, certKeyPairs) do
      {true, resumed} ->
        {suggestedId, resumed}
  ...
  defp do_client_select_session(clientInfo, cache, cacheCb, newSession, certKeyPairs) do
    case select_session(clientInfo, cache, cacheCb, certKeyPairs) do
      :no_session ->
        r_session(newSession, session_id: <<>>)
  ...
  defp select_session({_, _, %{reuse_sessions: reuse}}, _Cache, _CacheCb, _)
       when reuse !== true do
    :no_session
  end

  defp select_session({hostIP, port, sslOpts}, cache, cacheCb, certKeyPairs) do
  ...
  defp select_session(sessions, %{ciphers: ciphers}, certKeyPairs) do
    isNotResumable = fn session ->
      sessionOwnCert =
        case r_session(session, :own_certificates) do
          [ownCert | _] ->
            ownCert
  ...
  defp is_resumable(
    ...
    case :ssl_server_session_cache.reuse_session(
      ...
      r_session(
        ...
        case resumable(isResumable) and
               is_owncert(
                 sessionOwnCert,
                 ownCertKeyPairs
               ) and
  ...
  defp maybe_handle_middlebox({3, 4}, r_session(session_id: <<>>) = session, options) do
