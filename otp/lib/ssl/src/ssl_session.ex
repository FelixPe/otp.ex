defmodule :m_ssl_session do
  ...
  Record.defrecord(:r_generic_block_cipher, :generic_block_cipher, iv: :undefined,
                                                ...
                                                next_iv: :undefined)
  def legacy_session_id(opts) do
    case (:maps.get(:middlebox_comp_mode, opts, true)) do
      true ->
        legacy_session_id()
      false ->
        <<>>
  ...
  def is_new(r_session(session_id: <<>>), _) do
    true
  end

  def is_new(r_session(session_id: sessionId, is_resumable: true),
           sessionId) do
    false
  end
  ...
  def client_select_session({_, _,
            %{versions: versions,
                protocol: protocol} = opts} = clientInfo,
           cache, cacheCb, newSession, certKeyPairs) do
    recordCb = record_cb(protocol)
    lVersion = recordCb.lowest_protocol_version(versions)
    hVersion = recordCb.highest_protocol_version(versions)
    case (lVersion) do
      {3, 4} ->
        r_session(newSession, session_id: legacy_session_id(opts))
      _ ->
        session = do_client_select_session(clientInfo, cache,
                                             cacheCb, newSession, certKeyPairs)
        maybe_handle_middlebox(hVersion, session, opts)
    end
  end

  def server_select_session(_, sessIdTracker, <<>>, _SslOpts,
           _CertKeyPairs) do
    {:ssl_server_session_cache.new_session_id(sessIdTracker),
       :undefined}
  end

  def server_select_session(_, sessIdTracker, suggestedId, options,
           certKeyPairs) do
    case (is_resumable(suggestedId, sessIdTracker, options,
                         certKeyPairs)) do
      {true, resumed} ->
        {suggestedId, resumed}
      {false, :undefined} ->
  ...
  def valid_session(r_session(time_stamp: timeStamp),
           {:invalidate_before, before}) do
    timeStamp > before
  end
  ...
  defp do_client_select_session({_, _,
  ...
  defp do_client_select_session({host, port, %{reuse_session: sessionId}},
            cache, cacheCb, newSession, _)
      when is_binary(sessionId) do
    case (cacheCb.lookup(cache,
                           {{host, port}, sessionId})) do
      :undefined ->
        r_session(newSession, session_id: <<>>)
      r_session() = session ->
        session
  ...
  defp do_client_select_session(clientInfo, cache, cacheCb, newSession,
            certKeyPairs) do
    case (select_session(clientInfo, cache, cacheCb,
                           certKeyPairs)) do
      :no_session ->
        r_session(newSession, session_id: <<>>)
      session ->
  ...
  defp select_session({_, _, %{reuse_sessions: reuse}}, _Cache,
            _CacheCb, _)
      when reuse !== true do
    :no_session
  end

  defp select_session({hostIP, port, sslOpts}, cache, cacheCb,
            certKeyPairs) do
  ...
  defp select_session(sessions, %{ciphers: ciphers}, certKeyPairs) do
    isNotResumable = fn session ->
                          sessionOwnCert = (case (r_session(session, :own_certificates)) do
                                              [ownCert | _] ->
                                                ownCert
                                              other ->
                                                other
                          ...
                          not
                          (resumable(r_session(session, :is_resumable)) and :lists.member(r_session(session, :cipher_suite),
                                                                                    ciphers) and (is_owncert(sessionOwnCert,
                                                                                                               certKeyPairs) or sessionOwnCert == :undefined))
                     end
  ...
  defp is_resumable(suggestedSessionId, sessIdTracker,
            %{reuse_session: reuseFun} = options,
            ownCertKeyPairs) do
    case (:ssl_server_session_cache.reuse_session(sessIdTracker,
                                                    suggestedSessionId)) do
      r_session(cipher_suite: cipherSuite,
          own_certificates: [sessionOwnCert | _],
          compression_method: compression,
          is_resumable: isResumable,
          peer_certificate: peerCert) = session ->
        case (resumable(isResumable) and is_owncert(sessionOwnCert,
                                                      ownCertKeyPairs) and reusable_options(options,
                                                                                              session) and reuseFun.(suggestedSessionId,
                                                                                                                       peerCert,
                                                                                                                       compression,
                                                                                                                       cipherSuite)) do
          true ->
  ...
  defp is_owncert(_, []) do
    false
  end

  defp is_owncert(sessionOwnCert,
            [%{certs: [sessionOwnCert | _]} | _]) do
    true
  end

  defp is_owncert(sessionOwnCert, [_ | rest]) do
  ...
  defp reusable_options(%{fail_if_no_peer_cert: true,
              verify: :verify_peer},
            session) do
  ...
  defp legacy_session_id() do
    :crypto.strong_rand_bytes(32)
  end

  defp maybe_handle_middlebox({3, 4}, r_session(session_id: <<>>) = session,
            options) do
    case (:maps.get(:middlebox_comp_mode, options, true)) do
      true ->
        r_session(session, session_id: legacy_session_id())
      false ->
        session
    end
  end

  defp maybe_handle_middlebox(_, session, _) do
    session
  end
