defmodule :m_ssl_certificate do
  ...
  Record.defrecord(:r_generic_block_cipher, :generic_block_cipher, iv: :undefined,
                                                ...
                                                next_iv: :undefined)
  def trusted_cert_and_paths([peer], certDbHandle, certDbRef,
           partialChainHandler) do
    ...
    case (:public_key.pkix_is_self_signed(otpCert)) do
      ...
      false ->
        [handle_incomplete_chain(chain, partialChainHandler,
                                   {:unknown_ca, chain}, certDbHandle,
                                   certDbRef)]
    end
  end

  def trusted_cert_and_paths(chain0, certDbHandle, certDbRef,
           partialChainHandler) do
    chain = (for der <- chain0 do
               r_cert(der: der,
                   otp: :public_key.pkix_decode_cert(der, :otp))
             end)
    paths = paths(chain, certDbHandle)
    :lists.map(fn path ->
                    case (handle_partial_chain(path, partialChainHandler,
  ...
  def certificate_chain(derCert, certDbHandle, certsDbRef)
    ...
    {:ok, root, chain} = build_certificate_chain(cert,
  ...
  def certificate_chain(r_OTPCertificate() = otpCert, certDbHandle, certsDbRef) do
    derCert = :public_key.pkix_encode(:OTPCertificate,
                                        otpCert, :otp)
  ...
  def certificate_chain(derCert, certDbHandle, certsDbRef, candidates,
           type)
      when is_binary(derCert) do
    erlCert = :public_key.pkix_decode_cert(derCert, :otp)
    cert = r_cert(der: derCert, otp: erlCert)
  ...
  def certificate_chain(r_OTPCertificate() = otpCert, certDbHandle, certsDbRef,
           candidates, type) do
    derCert = :public_key.pkix_encode(:OTPCertificate,
                                        otpCert, :otp)
    cert = r_cert(der: derCert, otp: otpCert)
  ...
  def certificate_chain(r_cert() = cert, certDbHandle, certsDbRef, candidates,
           type) do
    {:ok, root, chain} = build_certificate_chain(cert,
                                                   certDbHandle, certsDbRef,
                                                   [cert], candidates)
  ...
  def validate(_,
           {:extension,
              r_Extension(extnID: {2, 5, 29, 37}, critical: critical,
                  extnValue: keyUse)},
           %{path_len: 1} = userState) do
    case (is_valid_extkey_usage(keyUse, critical,
                                  userState)) do
      true ->
        {:valid, userState}
      false ->
  ...
  def validate(issuer, {:bad_cert, :cert_expired},
           %{issuer: issuer}) do
  ...
  def validate(cert, :valid_peer,
           userState = %{role: :client, server_name: hostname,
                           customize_hostname_check: customize})
      when hostname !== :disable do
    case (verify_hostname(hostname, customize, cert,
                            userState)) do
      {:valid, ^userState} ->
  ...
  def public_key_type({1, 2, 840, 113549, 1, 1, 10}) do
    :rsa_pss_pss
  end

  def public_key_type({1, 2, 840, 113549, 1, 1, 1}) do
    :rsa
  end
  ...
  def foldl_db(isIssuerFun, certDbHandle, []) do
    :ssl_pkix_db.foldl(isIssuerFun, :issuer_not_found,
                         certDbHandle)
  end
  ...
  def find_cross_sign_root_paths([], _CertDbHandle, _CertDbRef,
           _InvalidatedList) do
    []
  end

  def find_cross_sign_root_paths([_ | rest] = path, certDbHandle, certDbRef,
           invalidatedList) do
    case (find_alternative_root(path, certDbHandle,
                                  certDbRef, invalidatedList)) do
      :unknown_ca ->
        find_cross_sign_root_paths(rest, certDbHandle,
                                     certDbRef, invalidatedList)
      root ->
        [{root, path}]
    end
  end
  ...
  def handle_cert_auths([cert], certAuths, certDbHandle, certDbRef) do
    case (certificate_chain(cert, certDbHandle, certDbRef,
                              [], :both)) do
      {:ok, {_, [^cert | _] = eChain}, {_, [_ | dCerts]}} ->
        case (cert_auth_member(cert_issuers(dCerts),
                                 certAuths)) do
          true ->
            {:ok, eChain}
          false ->
  ...
  def handle_cert_auths([_ | certs] = eChain, certAuths, _, _) do
    case (cert_auth_member(cert_issuers(certs),
                             certAuths)) do
      true ->
        {:ok, eChain}
      false ->
  ...
  defp cert_key_group_to_list([algo | rest], certKeyGroups, acc) do
    ...
    cert_key_group_to_list(rest, certKeyGroups,
                             certKeyPairs ++ acc)
  end
  ...
  defp build_certificate_chain(r_cert(otp: otpCert) = cert, certDbHandle,
            certsDbRef, chain, listDb) do
    issuerAndSelfSigned = (case (:public_key.pkix_is_self_signed(otpCert)) do
                             true ->
                               {:public_key.pkix_issuer_id(otpCert, :self),
                                  true}
                             false ->
                               {:public_key.pkix_issuer_id(otpCert, :other),
                                  false}
                           end)
    case (issuerAndSelfSigned) do
      {_, true = selfSigned} ->
        do_certificate_chain(certDbHandle, certsDbRef, chain,
                               :ignore, :ignore, selfSigned, listDb)
      {{:error, :issuer_not_found}, selfSigned} ->
        case (find_issuer(cert, certDbHandle, certsDbRef,
                            listDb, [])) do
          {:ok, {serialNr, issuer}} ->
            do_certificate_chain(certDbHandle, certsDbRef, chain,
  ...
  defp do_certificate_chain(certDbHandle, certsDbRef, chain, serialNr,
            issuer, _, listDb) do
    case (:ssl_manager.lookup_trusted_cert(certDbHandle,
                                             certsDbRef, serialNr, issuer)) do
      {:ok, cert} ->
        build_certificate_chain(cert, certDbHandle, certsDbRef,
                                  [cert | chain], listDb)
      _ ->
        {:ok, :undefined, :lists.reverse(chain)}
    end
  ...
  defp find_alternative_root([cert | _], certDbHandle, certDbRef,
            invalidatedList) do
    case (find_issuer(cert, certDbHandle, certDbRef, [],
                        invalidatedList)) do
  ...
  defp find_issuer(r_cert(der: derCert, otp: otpCert), certDbHandle,
            certsDbRef, listDb, invalidatedList) do
    isIssuerFun = fn {_Key, r_cert(otp: erlCertCandidate)},
                       acc ->
                       case (:public_key.pkix_is_issuer(otpCert,
                                                          erlCertCandidate)) do
                         true ->
                           case (verify_cert_signer(derCert,
                                                      r_OTPCertificate(erlCertCandidate, :tbsCertificate)) and not
                                                                                                :lists.member(erlCertCandidate,
                                                                                                                invalidatedList)) do
                             true ->
                               throw(:public_key.pkix_issuer_id(erlCertCandidate,
                                                                  :self))
                             false ->
                               acc
                           end
    ...
    result = (case (is_reference(certsDbRef)) do
                true when listDb == [] ->
                  certEntryList = :ssl_pkix_db.select_certentries_by_ref(certsDbRef,
                                                                           certDbHandle)
                  do_find_issuer(isIssuerFun, certDbHandle, certEntryList)
                false when listDb == [] ->
                  {:extracted, certsData} = certsDbRef
                  certEntryList = (for {:decoded, entry} <- certsData do
                                     entry
                                   end)
                  do_find_issuer(isIssuerFun, certDbHandle, certEntryList)
                _ ->
  ...
  defp public_key(r_OTPSubjectPublicKeyInfo(algorithm: r_PublicKeyAlgorithm(algorithm: {1, 2, 840, 10045, 2,
                                       1},
                           parameters: params),
              subjectPublicKey: point)) do
    {point, params}
  end

  defp public_key(r_OTPSubjectPublicKeyInfo(algorithm: r_PublicKeyAlgorithm(algorithm: {1, 3, 101, 112}),
              subjectPublicKey: point)) do
    {point, {:namedCurve, {1, 3, 101, 112}}}
  end

  defp public_key(r_OTPSubjectPublicKeyInfo(algorithm: r_PublicKeyAlgorithm(algorithm: {1, 3, 101, 113}),
              subjectPublicKey: point)) do
    {point, {:namedCurve, {1, 3, 101, 113}}}
  end

  defp public_key(r_OTPSubjectPublicKeyInfo(algorithm: r_PublicKeyAlgorithm(algorithm: {1, 2, 840, 113549, 1,
                                       1, 1}),
              subjectPublicKey: key)) do
    key
  end
  ...
  defp public_key(r_OTPSubjectPublicKeyInfo(algorithm: r_PublicKeyAlgorithm(algorithm: {1, 2, 840, 10040, 4,
                                       1},
  ...
  defp other_issuer(r_cert(otp: otpCert) = cert, certDbHandle,
            certDbRef) do
  ...
  defp verify_hostname(hostname, customize, cert, userState)
    ...
    case (:public_key.pkix_verify_hostname(cert,
                                             [{:ip, hostname}], customize)) do
      true ->
        {:valid, userState}
      false ->
  ...
  defp verify_hostname(hostname, customize, cert, userState) do
    hostId = (case (:inet.parse_strict_address(hostname)) do
                {:ok, iP} ->
                  {:ip, iP}
                _ ->
                  {:dns_id, hostname}
              end)
    case (:public_key.pkix_verify_hostname(cert, [hostId],
                                             customize)) do
      true ->
        {:valid, userState}
      false ->
  ...
  defp verify_cert_extensions(cert,
            %{ocsp_responder_certs: responderCerts,
                ocsp_state: oscpState, issuer: issuer} = userState,
            [r_certificate_status(response: ocspResponsDer) | exts], context) do
    %{ocsp_nonce: nonce} = oscpState
    case (:public_key.pkix_ocsp_validate(cert, issuer,
                                           ocspResponsDer, responderCerts,
                                           nonce)) do
      :valid ->
  ...
  defp verify_sign(cert,
            %{version: {3, 3}, signature_algs: signAlgs,
                signature_algs_cert: :undefined}) do
    is_supported_signature_algorithm_1_2(cert, signAlgs)
  end

  defp verify_sign(cert,
            %{version: {3, 3}, signature_algs_cert: signAlgs}) do
    is_supported_signature_algorithm_1_2(cert, signAlgs)
  end

  defp verify_sign(cert,
            %{version: {3, 4}, signature_algs: signAlgs,
                signature_algs_cert: :undefined}) do
    is_supported_signature_algorithm_1_3(cert, signAlgs)
  end

  defp verify_sign(cert,
            %{version: {3, 4}, signature_algs_cert: signAlgs}) do
    is_supported_signature_algorithm_1_3(cert, signAlgs)
  end

  defp is_supported_signature_algorithm_1_2(r_OTPCertificate(signatureAlgorithm: r_SignatureAlgorithm(algorithm: {1, 2, 840,
                                                10040, 4, 3})),
            signAlgs) do
  ...
  defp is_supported_signature_algorithm_1_2(r_OTPCertificate(signatureAlgorithm: signAlg), signAlgs) do
    scheme = :ssl_cipher.signature_algorithm_to_scheme(signAlg)
    {hash, sign,
       _} = :ssl_cipher.scheme_to_components(scheme)
    :ssl_cipher.is_supported_sign({hash,
                                     pre_1_3_sign(sign)},
                                    :ssl_cipher.signature_schemes_1_2(signAlgs))
  end

  defp is_supported_signature_algorithm_1_3(r_OTPCertificate(signatureAlgorithm: signAlg), signAlgs) do
    scheme = :ssl_cipher.signature_algorithm_to_scheme(signAlg)
  ...
  defp paths([r_cert(otp: c1) = cert1, r_cert(otp: c2) = cert2 | rest],
            chain, certDbHandle, path) do
    case (:public_key.pkix_is_issuer(c1, c2)) do
      true ->
        paths([cert2 | rest], chain, certDbHandle,
                [cert1 | path])
      false ->
  ...
  defp path_candidate(cert, chainCandidateCAs, certDbHandle) do
    {:ok,
       extractedCerts} = :ssl_pkix_db.extract_trusted_certs({:der_otp,
                                                               chainCandidateCAs})
    case (build_certificate_chain(cert, certDbHandle,
                                    extractedCerts, [cert], [])) do
      {:ok, :undefined, chain} ->
        :lists.reverse(chain)
      {:ok, root, chain} ->
  ...
  defp handle_partial_chain([r_cert(der: dERIssuerCert,
               otp: otpIssuerCert) = cert |
               rest] = path,
            partialChainHandler, certDbHandle, certDbRef) do
    case (:public_key.pkix_is_self_signed(otpIssuerCert)) do
      true ->
        {:ok,
           {serialNr,
              issuerId}} = :public_key.pkix_issuer_id(otpIssuerCert,
                                                        :self)
        case (:ssl_manager.lookup_trusted_cert(certDbHandle,
                                                 certDbRef, serialNr,
                                                 issuerId)) do
            ...
            case (:ssl_manager.lookup_trusted_cert(certDbHandle,
              ...
              {:ok, r_cert(otp: newOtp) = newCert} ->
                case (:public_key.pkix_is_self_signed(newOtp)) do
                  true ->
                    maybe_shorten_path([newCert | path],
                                         partialChainHandler, {newCert, path})
                  false ->
                    maybe_shorten_path([newCert | path],
                                         partialChainHandler,
  ...
  defp maybe_shorten_path(path, partialChainHandler, default) do
    derCerts = (for r_cert(der: der) <- path do
                  der
                end)
    try do
  ...
  defp handle_incomplete_chain([r_cert() = peer | _] = chain0, partialChainHandler,
            default, certDbHandle, certDbRef) do
    case (build_certificate_chain(peer, certDbHandle,
                                    certDbRef, [peer], [])) do
      {:ok, _, [^peer | _] = chainCandidate}
        ...
        case (:lists.prefix(chain0, chainCandidate)) do
          true ->
            {root,
               chain} = handle_partial_chain(:lists.reverse(chainCandidate),
                                               partialChainHandler,
  ...
  defp extraneous_chains(certs) do
    subjects = (for (r_cert(otp: oTP) = cert) <- certs do
                  {subject(oTP), cert}
                end)
  ...
  defp build_candidates(map, duplicates, combinations) do
    ...
    build_candidates(subjects, map, duplicates, 1,
                       combinations, [])
  end
  ...
  defp build_candidates([h | t], map, duplicates, combinations, max,
            acc0) do
    case (:maps.get(h, map)) do
      {certs, counter}
        ...
        case (acc0) do
          [] ->
            acc = (for cert <- certs do
                     [cert]
                   end)
            build_candidates(t, map, duplicates - 1,
                               combinations * counter, max, acc)
          _Else ->
            acc = (for cert <- certs, l <- acc0 do
                     [cert | l]
                   end)
            build_candidates(t, map, duplicates - 1,
                               combinations * counter, max, acc)
      ...
      {[cert | _Throw], _Counter} ->
        case (acc0) do
          [] ->
            acc = [[cert]]
            build_candidates(t, map, duplicates, combinations, max,
                               acc)
          _Else ->
            acc = (for l <- acc0 do
                     [cert | l]
  ...
  defp cert_auth_member(chainSubjects, certAuths) do
    commonAuthorities = :sets.intersection(:sets.from_list(chainSubjects),
  ...
  def handle_trace(:crt,
           {:call,
              {:ssl_certificate, :validate, [cert, statusOrExt | _]}},
           stack) do
  ...
  def handle_trace(:crt,
           {:call,
              {:ssl_certificate, :verify_cert_extensions,
                 [cert, %{ocsp_responder_certs: _ResponderCerts,
                            ocsp_state: ocspState, issuer: issuer} = _UserState,
                            [r_certificate_status(response: ocspResponsDer) | _Exts], _Context]}},
           stack) do
