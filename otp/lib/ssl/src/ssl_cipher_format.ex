defmodule :m_ssl_cipher_format do
  ...
  Record.defrecord(:r_ticket_data, :ticket_data, key: :undefined,
                                       ...
                                       max_size: :undefined)
  def suite_map_to_str(%{key_exchange: :null, cipher: :null, mac: :null,
             prf: :null}) do
    'TLS_EMPTY_RENEGOTIATION_INFO_SCSV'
  end

  def suite_map_to_str(%{key_exchange: :any, cipher: cipher, mac: :aead,
             prf: pRF}) do
    'TLS_' ++ :string.to_upper(:erlang.atom_to_list(cipher)) ++ '_' ++ :string.to_upper(:erlang.atom_to_list(pRF))
  end

  def suite_map_to_str(%{key_exchange: kex, cipher: cipher, mac: :aead,
             prf: pRF}) do
    'TLS_' ++ kex_str(kex) ++ '_WITH_' ++ :string.to_upper(:erlang.atom_to_list(cipher)) ++ prf_str('_',
                                                                                          pRF)
  end

  def suite_map_to_str(%{key_exchange: kex, cipher: cipher,
             mac: mac}) do
    'TLS_' ++ kex_str(kex) ++ '_WITH_' ++ :string.to_upper(:erlang.atom_to_list(cipher)) ++ '_' ++ :string.to_upper(:erlang.atom_to_list(mac))
  end

  def suite_str_to_map('TLS_EMPTY_RENEGOTIATION_INFO_SCSV') do
    %{key_exchange: :null, cipher: :null, mac: :null,
        prf: :null}
  end
  ...
  def suite_map_to_openssl_str(%{key_exchange: :rsa = kex, cipher: cipher,
             mac: :aead, prf: pRF})
      when pRF !== :default_prf do
    openssl_cipher_name(kex,
                          :string.to_upper(:erlang.atom_to_list(cipher))) ++ '-' ++ :string.to_upper(:erlang.atom_to_list(pRF))
  end

  def suite_map_to_openssl_str(%{key_exchange: kex, cipher: cipher, mac: mac})
      when kex == :rsa or kex == :srp_anon and cipher == 'des_cbc' or cipher == '3des_ede_cbc' do
    openssl_cipher_name(kex,
                          :string.to_upper(:erlang.atom_to_list(cipher))) ++ '-' ++ :string.to_upper(:erlang.atom_to_list(mac))
  end

  def suite_map_to_openssl_str(%{key_exchange: kex,
             cipher: :chacha20_poly1305 = cipher, mac: :aead,
             prf: :sha256}) do
    openssl_suite_start(kex_str(kex),
                          cipher) ++ openssl_cipher_name(kex,
                                                           :string.to_upper(:erlang.atom_to_list(cipher)))
  end

  def suite_map_to_openssl_str(%{key_exchange: kex, cipher: cipher, mac: :aead,
             prf: pRF}) do
    openssl_suite_start(kex_str(kex),
                          cipher) ++ openssl_cipher_name(kex,
                                                           :string.to_upper(:erlang.atom_to_list(cipher))) ++ prf_str('-',
                                                                                                                        pRF)
  end

  def suite_map_to_openssl_str(%{key_exchange: kex, cipher: cipher,
             mac: mac}) do
    openssl_suite_start(kex_str(kex),
                          cipher) ++ openssl_cipher_name(kex,
                                                           :string.to_upper(:erlang.atom_to_list(cipher))) ++ '-' ++ :string.to_upper(:erlang.atom_to_list(mac))
  ...
  def suite_openssl_str_to_map('ADH' ++ rest) do
    suite_openssl_str_to_map('DH-anon', rest)
  end

  def suite_openssl_str_to_map('AECDH' ++ rest) do
  ...
  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             0 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :null, cipher: :null, mac: :null,
        prf: :null}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             255 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :null, cipher: :null, mac: :null,
        prf: :null}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             4 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :rc4_128, mac: :md5,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             5 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             9 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :des_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             10 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             18 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :des_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             19 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             21 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :des_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             22 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             47 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :aes_128_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             50 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             51 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             53 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :aes_256_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             56 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             57 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             60 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :aes_128_cbc,
        mac: :sha256, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             61 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :aes_256_cbc,
        mac: :sha256, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             64 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :aes_128_cbc,
        mac: :sha256, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             103 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :aes_128_cbc,
        mac: :sha256, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             106 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :aes_256_cbc,
        mac: :sha256, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             107 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :aes_256_cbc,
        mac: :sha256, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             24 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :rc4_128, mac: :md5,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             26 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :des_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             27 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             52 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             58 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             108 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             109 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_256_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             138 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             139 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             140 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_128_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             141 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_256_cbc, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             142 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             143 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             144 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             145 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             146 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             147 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             148 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             149 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             44 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             45 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             46 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             168 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_128_gcm, mac: :aead,
        prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             169 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_256_gcm, mac: :aead,
        prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             170 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             171 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             172 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             173 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             174 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             175 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_256_cbc,
        mac: :sha384, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             178 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             179 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_256_cbc,
        mac: :sha384, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             182 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :aes_128_cbc,
        mac: :sha256, prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             183 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :aes_256_cbc,
        mac: :sha384, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             176 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :null, mac: :sha256,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             177 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :null, mac: :sha384,
        prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             180 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :null, mac: :sha256,
        prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             181 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :null, mac: :sha384,
        prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             184 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :null, mac: :sha256,
        prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             185 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa_psk, cipher: :null, mac: :sha384,
        prf: :sha384}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             51 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             52 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             53 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             54 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             55 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             58 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :null, mac: :sha256,
        prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             59 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :null, mac: :sha384,
        prf: :sha384}
  end

  def suite_bin_to_map(<<208 :: size(8) - unsigned - big - integer,
             1 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<208 :: size(8) - unsigned - big - integer,
             2 :: size(8) - unsigned - big - integer>>) do
  ...
  def suite_bin_to_map(<<208 :: size(8) - unsigned - big - integer,
             5 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :aes_128_ccm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<208 :: size(8) - unsigned - big - integer,
             3 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :aes_128_ccm_8,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             26 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_anon, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             27 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_rsa, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             28 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_dss, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             29 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_anon, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             30 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_rsa, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             31 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_dss, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             32 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_anon, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             33 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_rsa, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             34 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :srp_dss, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             1 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             2 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :rc4_128,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             3 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             4 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             5 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             6 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             7 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :rc4_128,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             8 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             9 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             10 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             172 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_ccm,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             173 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_ccm,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             174 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_ccm_8,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             175 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_ccm_8,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             11 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             12 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             13 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             14 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             15 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             16 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             17 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             18 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             19 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             20 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             21 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_anon, cipher: :null, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             22 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_anon, cipher: :rc4_128, mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             23 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_anon, cipher: :"3des_ede_cbc", mac: :sha,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             24 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_anon, cipher: :aes_128_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             25 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_anon, cipher: :aes_256_cbc,
        mac: :sha, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             35 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             36 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_cbc,
        mac: :sha384, prf: :sha384}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             37 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             38 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :aes_256_cbc,
        mac: :sha384, prf: :sha384}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             39 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             40 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :aes_256_cbc,
        mac: :sha384, prf: :sha384}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             41 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :aes_128_cbc,
        mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             42 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :aes_256_cbc,
        mac: :sha384, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             156 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :aes_128_gcm, mac: :aead,
        prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             157 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :rsa, cipher: :aes_256_gcm, mac: :aead,
        prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             158 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             159 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             160 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_rsa, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             161 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_rsa, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             162 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             163 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_dss, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             164 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_dss, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             165 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_dss, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             166 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<0 :: size(8) - unsigned - big - integer,
             167 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             43 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             45 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             46 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_ecdsa, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             47 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             48 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :aes_256_gcm,
        mac: :aead, prf: :sha384}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             49 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdh_rsa, cipher: :aes_128_gcm,
        mac: :aead, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             164 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_128_ccm, mac: :aead,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             165 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_256_ccm, mac: :aead,
        prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             166 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_128_ccm,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             167 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_256_ccm,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             168 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_128_ccm_8,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             169 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :psk, cipher: :aes_256_ccm_8,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             170 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_128_ccm_8,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192 :: size(8) - unsigned - big - integer,
             171 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_256_ccm_8,
        mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(%{key_exchange: :psk_dhe, cipher: :aes_256_ccm_8,
             mac: :aead, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        171 :: size(8) - unsigned - big - integer>>
  end

  def suite_bin_to_map(<<204 :: size(8) - unsigned - big - integer,
             168 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :ecdhe_rsa, cipher: :chacha20_poly1305,
        mac: :aead, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<204 :: size(8) - unsigned - big - integer,
    ...
    %{key_exchange: :ecdhe_ecdsa,
        cipher: :chacha20_poly1305, mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<204 :: size(8) - unsigned - big - integer,
             170 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :dhe_rsa, cipher: :chacha20_poly1305,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<19 :: size(8) - unsigned - big - integer,
             1 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :any, cipher: :aes_128_gcm, mac: :aead,
        prf: :sha256}
  end

  def suite_bin_to_map(<<19 :: size(8) - unsigned - big - integer,
             2 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :any, cipher: :aes_256_gcm, mac: :aead,
        prf: :sha384}
  end

  def suite_bin_to_map(<<19 :: size(8) - unsigned - big - integer,
             3 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :any, cipher: :chacha20_poly1305,
        mac: :aead, prf: :sha256}
  end

  def suite_bin_to_map(<<19 :: size(8) - unsigned - big - integer,
             4 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :any, cipher: :aes_128_ccm, mac: :aead,
        prf: :sha256}
  end

  def suite_bin_to_map(<<19 :: size(8) - unsigned - big - integer,
             5 :: size(8) - unsigned - big - integer>>) do
    %{key_exchange: :any, cipher: :aes_128_ccm_8,
        mac: :aead, prf: :sha256}
  end
  ...
  def suite_legacy(%{key_exchange: keyExchange, cipher: cipher,
             mac: hash, prf: prf}) do
    case (prf) do
      :default_prf ->
  ...
  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :rc4_128,
             mac: :md5}) do
  ...
  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :des_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
  ...
  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :des_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        18 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        19 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :des_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        21 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        22 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :rc4_128,
             mac: :md5}) do
    <<0 :: size(8) - unsigned - big - integer,
        24 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :des_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        26 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        27 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        47 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        50 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        51 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        52 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        53 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        56 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        57 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        58 :: size(8) - unsigned - big - integer>>
  end
  ...
  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :aes_128_cbc,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        64 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_128_cbc,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        103 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :aes_256_cbc,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        106 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_256_cbc,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        107 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :aes_128_cbc,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        108 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :aes_256_cbc,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        109 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :rc4_128,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        138 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :"3des_ede_cbc", mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        139 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        140 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        141 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :rc4_128,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        142 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        143 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        144 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        145 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :rc4_128,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        146 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        147 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        148 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        149 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :null,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        44 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :null,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        45 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :null,
             mac: :sha}) do
    <<0 :: size(8) - unsigned - big - integer,
        46 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        168 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        169 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        170 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        171 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        172 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
  ...
  def suite_map_to_bin(%{key_exchange: :psk, cipher: :null,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        176 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :null,
             mac: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        177 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :null,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        180 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :null,
             mac: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        181 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :null,
             mac: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        184 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa_psk, cipher: :null,
             mac: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        185 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :rc4_128,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        51 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        52 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        53 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        54 :: size(8) - unsigned - big - integer>>
  end
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :null,
             mac: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        58 :: size(8) - unsigned - big - integer>>
  end
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :null,
             mac: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
        59 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<208 :: size(8) - unsigned - big - integer,
        1 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<208 :: size(8) - unsigned - big - integer,
        2 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk,
             cipher: :aes_128_ccm_8, mac: :aead, prf: :sha256}) do
    <<208 :: size(8) - unsigned - big - integer,
        3 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_128_ccm,
             mac: :aead, prf: :sha256}) do
    <<208 :: size(8) - unsigned - big - integer,
        5 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_anon, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        26 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_rsa, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        27 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_dss, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        28 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_anon, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        29 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_rsa, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        30 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_dss, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        31 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_anon, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        32 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_rsa, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        33 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :srp_dss, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        34 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_ecdsa, cipher: :null,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        1 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_ecdsa, cipher: :rc4_128,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        2 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_ecdsa, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        3 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_ecdsa,
             cipher: :aes_128_cbc, mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        4 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_ecdsa,
             cipher: :aes_256_cbc, mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        5 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa, cipher: :null,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        6 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa, cipher: :rc4_128,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        7 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        8 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_128_cbc, mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        9 :: size(8) - unsigned - big - integer>>
  end
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_128_ccm, mac: :aead}) do
    <<192 :: size(8) - unsigned - big - integer,
        172 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_256_ccm, mac: :aead}) do
    <<192 :: size(8) - unsigned - big - integer,
        173 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_128_ccm_8, mac: :aead}) do
    <<192 :: size(8) - unsigned - big - integer,
        174 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_256_ccm_8, mac: :aead}) do
    <<192 :: size(8) - unsigned - big - integer,
        175 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :null,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        11 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :rc4_128,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        12 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        13 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        14 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        15 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :null,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        16 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :rc4_128,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        17 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        18 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        19 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        20 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_anon, cipher: :null,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        21 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_anon, cipher: :rc4_128,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        22 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_anon, cipher: :"3des_ede_cbc",
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        23 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_anon, cipher: :aes_128_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        24 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_anon, cipher: :aes_256_cbc,
             mac: :sha}) do
    <<192 :: size(8) - unsigned - big - integer,
        25 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_128_cbc, mac: :sha256, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        35 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_256_cbc, mac: :sha384, prf: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
  ...
  def suite_map_to_bin(%{key_exchange: :ecdh_ecdsa,
             cipher: :aes_128_cbc, mac: :sha256, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        37 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_ecdsa,
             cipher: :aes_256_cbc, mac: :sha384, prf: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
        38 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :aes_128_cbc,
             mac: :sha256, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        39 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :aes_256_cbc,
             mac: :sha384, prf: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
  ...
  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :aes_128_cbc,
             mac: :sha256, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        41 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :aes_256_cbc,
             mac: :sha384, prf: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
        42 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        156 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        157 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        158 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        159 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_rsa, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        160 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_rsa, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        161 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        162 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_dss, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        163 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_dss, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        164 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_dss, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
        165 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<0 :: size(8) - unsigned - big - integer,
        166 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dh_anon, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<0 :: size(8) - unsigned - big - integer,
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_128_gcm, mac: :aead, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        43 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa,
             cipher: :aes_256_gcm, mac: :aead, prf: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
        44 :: size(8) - unsigned - big - integer>>
  end
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        47 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
        48 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        49 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdh_rsa, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<192 :: size(8) - unsigned - big - integer,
        50 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_rsa,
             cipher: :chacha20_poly1305, mac: :aead,
             prf: :sha256}) do
  ...
  def suite_map_to_bin(%{key_exchange: :dhe_rsa,
             cipher: :chacha20_poly1305, mac: :aead,
             prf: :sha256}) do
    <<204 :: size(8) - unsigned - big - integer,
        170 :: size(8) - unsigned - big - integer>>
  ...
  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_128_ccm,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        164 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_256_ccm,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        165 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_128_ccm,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        166 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_256_ccm,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        167 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_128_ccm,
             mac: :aead, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        156 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_256_ccm,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        157 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_128_ccm,
             mac: :aead, prf: :sha256}) do
    <<192 :: size(8) - unsigned - big - integer,
        159 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_256_ccm,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        158 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_128_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        168 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_256_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        169 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_128_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        170 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_psk, cipher: :aes_256_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        171 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_128_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        161 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_256_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        160 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_128_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        162 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :dhe_rsa, cipher: :aes_256_ccm_8,
             mac: :aead, prf: :default_prf}) do
    <<192 :: size(8) - unsigned - big - integer,
        163 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :any, cipher: :aes_128_gcm,
             mac: :aead, prf: :sha256}) do
    <<19 :: size(8) - unsigned - big - integer,
        1 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :any, cipher: :aes_256_gcm,
             mac: :aead, prf: :sha384}) do
    <<19 :: size(8) - unsigned - big - integer,
        2 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :any, cipher: :chacha20_poly1305,
             mac: :aead, prf: :sha256}) do
    <<19 :: size(8) - unsigned - big - integer,
        3 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :any, cipher: :aes_128_ccm,
             mac: :aead, prf: :sha256}) do
    <<19 :: size(8) - unsigned - big - integer,
        4 :: size(8) - unsigned - big - integer>>
  end

  def suite_map_to_bin(%{key_exchange: :any, cipher: :aes_128_ccm_8,
             mac: :aead, prf: :sha256}) do
    <<19 :: size(8) - unsigned - big - integer,
        5 :: size(8) - unsigned - big - integer>>
  end
  ...
  defp tls_1_3_suite_str_to_map(cipherStr) do
    {cipher, mac, prf} = cipher_str_to_algs(:any, cipherStr,
                                              '')
    %{key_exchange: :any, mac: mac, cipher: cipher,
        prf: prf}
  end
  ...
  defp pre_tls_1_3_suite_str_to_map(kexStr, rest) do
    ...
    {cipher, mac, prf} = cipher_str_to_algs(kex, cipherStr,
                                              algStr)
    %{key_exchange: kex, mac: mac, cipher: cipher, prf: prf}
  end

  defp kex_str(:srp_dss) do
    'SRP_SHA_DSS'
  end

  defp kex_str(:srp_rsa) do
    'SRP_SHA_RSA'
  end

  defp kex_str(:srp_anon) do
    'SRP_SHA'
  end

  defp kex_str(:dh_anon) do
    'DH_anon'
  end

  defp kex_str(:ecdh_anon) do
    'ECDH_anon'
  end
  ...
  defp prf_str(_, :default_prf) do
    ''
  end

  defp prf_str(prefix, pRF) do
    prefix ++ :string.to_upper(:erlang.atom_to_list(pRF))
  end

  defp cipher_str_to_algs(:any, cipherStr0, '') do
    [cipherStr, algStr] = :string.split(cipherStr0, '_',
                                          :trailing)
  ...
  defp cipher_str_to_algs(_Kex, cipherStr, 'CCM' = end__) do
    cipher = algo_str_to_atom(cipherStr ++ '_' ++ end__)
    {cipher, :aead, :default_prf}
  end

  defp cipher_str_to_algs(_Kex, cipherStr, 'GCM' = end__) do
    cipher = algo_str_to_atom(cipherStr ++ '_' ++ end__)
    {cipher, :aead, :default_prf}
  end

  defp cipher_str_to_algs(_Kex, cipherStr, '8' = end__) do
    cipher = algo_str_to_atom(cipherStr ++ '_' ++ end__)
    {cipher, :aead, :default_prf}
  end

  defp cipher_str_to_algs(_Kex, 'CHACHA20_POLY1305' = cipherStr, '') do
    cipher = algo_str_to_atom(cipherStr)
    {cipher, :aead, :sha256}
  end
  ...
  defp openssl_is_aead_cipher(cipherStr) do
    case (:string.split(cipherStr, '-', :trailing)) do
      [_, rest] ->
        rest == 'GCM' or rest == 'CCM' or rest == 'CCM8'
      [_] ->
        false
    end
  end
  ...
  defp algo_str_to_atom('SRP_SHA_RSA') do
    :srp_rsa
  end

  defp algo_str_to_atom('SRP_SHA') do
    :srp_anon
  end

  defp algo_str_to_atom('SRP') do
    :srp_anon
  end
  ...
  defp openssl_cipher_name(kex, '3DES_EDE_CBC' ++ _) when kex == :ecdhe_psk or
                              kex == :srp_anon or kex == :psk or
                              kex == :dhe_psk do
    '3DES-EDE-CBC'
  end
  ...
  defp openssl_cipher_name(kex, 'AES_128_CBC' ++ _ = cipherStr) when kex == :rsa or
                                          kex == :dhe_rsa or kex == :dhe_dss or
                                          kex == :ecdh_rsa or
                                          kex == :ecdhe_rsa or
                                          kex == :ecdh_ecdsa or
                                          kex == :ecdhe_ecdsa or
  ...
  defp openssl_cipher_name(kex, 'AES_256_CBC' ++ _ = cipherStr) when kex == :rsa or
                                          kex == :dhe_rsa or kex == :dhe_dss or
                                          kex == :ecdh_rsa or
                                          kex == :ecdhe_rsa or
                                          kex == :ecdh_ecdsa or
  ...
  defp openssl_cipher_name(kex, 'AES_128_CBC' ++ _ = cipherStr)
      when kex == :srp_anon or kex == :srp_rsa do
  ...
  defp openssl_cipher_name(_, 'AES_128_GCM_8') do
    openssl_name_concat('AES_128_GCM') ++ '-GCM8'
  end

  defp openssl_cipher_name(_, 'AES_256_GCM_8') do
    openssl_name_concat('AES_256_GCM') ++ '-GCM8'
  end

  defp openssl_cipher_name(_, 'AES_128_CCM_8') do
    openssl_name_concat('AES_128_CCM') ++ '-CCM8'
  end

  defp openssl_cipher_name(_, 'AES_256_CCM_8') do
  ...
  defp openssl_cipher_name(_, 'AES_128_CCM' ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ '-CCM'
  end

  defp openssl_cipher_name(_, 'AES_256_CCM' ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ '-CCM'
  end
  ...
  defp openssl_suite_start(kex, cipher) do
    case (openssl_kex_name(kex, cipher)) do
      '' ->
        ''
      name ->
        name ++ '-'
  ...
  defp openssl_kex_name('RSA', _) do
    ''
  end

  defp openssl_kex_name('DH_anon', _) do
    'ADH'
  end

  defp openssl_kex_name('ECDH_anon', _) do
    'AECDH'
  end

  defp openssl_kex_name('SRP_SHA', _) do
    'SRP'
  end

  defp openssl_kex_name('SRP_SHA_RSA', _) do
    'SRP-RSA'
  end

  defp openssl_kex_name('SRP_SHA_DSS', _) do
    'SRP-DSS'
  end

  defp openssl_kex_name('DHE_RSA', cipher) when cipher == :des_cbc or
                            cipher == :"3des_ede_cbc" do
    'EDH-RSA'
  end

  defp openssl_kex_name(kex, _) do
    :lists.append(:string.replace(kex, '_', '-', :all))
  end

  defp kex_name_from_openssl(kex) do
    case (:lists.append(:string.replace(kex, '-', '_',
                                          :all))) do
      'EDH-RSA' ->
        'DHE_RSA'
      'SRP' ->
  ...
  defp cipher_name_from_openssl('AES128-CCM8') do
    'AES_128_CCM_8'
  end

  defp cipher_name_from_openssl('AES256-CCM8') do
    'AES_256_CCM_8'
  end

  defp cipher_name_from_openssl('AES128-' ++ suffix) do
    'AES_128_' ++ :lists.append(:string.replace(suffix, '-', '_', :all))
  end

  defp cipher_name_from_openssl('AES256-' ++ suffix) do
    'AES_256_' ++ :lists.append(:string.replace(suffix, '-', '_', :all))
  end

  defp cipher_name_from_openssl('AES128_' ++ suffix) do
    'AES_128_' ++ suffix
  end

  defp cipher_name_from_openssl('AES256_' ++ suffix) do
    'AES_256_' ++ suffix
  end
  ...
  defp cipher_name_from_openssl('3DES-EDE-CBC') do
    '3DES_EDE_CBC'
  end
  ...
  defp cipher_name_from_openssl('CHACHA20-POLY1305') do
    'CHACHA20_POLY1305'
  end
  ...
  defp suite_openssl_str_to_map(kex0, rest) do
    ...
    {cipher, mac, prf} = openssl_cipher_str_to_algs(kex,
                                                      part1, part2)
    %{key_exchange: kex, mac: mac, cipher: cipher, prf: prf}
  end

  defp openssl_cipher_str_to_algs(_, part1, 'CCM' = end__) do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part1 ++ '_' ++ end__))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, part1, 'GCM' = end__) do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part1 ++ '_' ++ end__))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, part2, 'CCM8') do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part2 ++ '-CCM-8'))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, part2, 'GCM8') do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part2 ++ '-GCM-8'))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, 'CHACHA20', 'POLY1305') do
    cipher = :chacha20_poly1305
    {cipher, :aead, :sha256}
  end

  defp openssl_cipher_str_to_algs(kex, part1, part2) do
    hash = algo_str_to_atom(part2)
    cipher = algo_str_to_atom(cipher_name_from_openssl(:string.strip(part1,
                                                                       :left,
                                                                       ?-)))
    case (openssl_is_aead_cipher(part1)) do
      true ->
        {cipher, :aead, hash}
      false ->
        {cipher, hash, default_prf(kex, hash)}
