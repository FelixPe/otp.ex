defmodule :m_ssl_cipher_format do
  ...
  def suite_map_to_str(%{key_exchange: :any, cipher: cipher, mac: :aead, prf: pRF}) do
    ~c"TLS_" ++
      ...
      ~c"_" ++ :string.to_upper(:erlang.atom_to_list(pRF))
  end
  ...
  def suite_map_to_str(%{key_exchange: kex, cipher: cipher, mac: mac}) do
    ~c"TLS_" ++
      ...
      ~c"_WITH_" ++
      :string.to_upper(:erlang.atom_to_list(cipher)) ++
      ~c"_" ++ :string.to_upper(:erlang.atom_to_list(mac))
  end

  def suite_str_to_map(~c"TLS_EMPTY_RENEGOTIATION_INFO_SCSV") do
    %{key_exchange: :null, cipher: :null, mac: :null, prf: :null}
  end

  def suite_str_to_map(suiteStr) do
    str0 = :string.prefix(suiteStr, ~c"TLS_")

    case :string.split(str0, ~c"_WITH_") do
      [rest] ->
  ...
  def suite_map_to_openssl_str(%{key_exchange: :rsa = kex, cipher: cipher, mac: :aead, prf: pRF})
      when pRF !== :default_prf do
    openssl_cipher_name(
      kex,
      :string.to_upper(:erlang.atom_to_list(cipher))
    ) ++ ~c"-" ++ :string.to_upper(:erlang.atom_to_list(pRF))
  end

  def suite_map_to_openssl_str(%{key_exchange: kex, cipher: cipher, mac: mac})
      when kex == :rsa or (kex == :srp_anon and cipher == ~c"des_cbc") or
             cipher == ~c"3des_ede_cbc" do
    openssl_cipher_name(
      kex,
      :string.to_upper(:erlang.atom_to_list(cipher))
    ) ++ ~c"-" ++ :string.to_upper(:erlang.atom_to_list(mac))
  end
  ...
  def suite_openssl_str_to_map(~c"TLS_" ++ _ = suiteStr) do
    suite_str_to_map(suiteStr)
  end

  def suite_openssl_str_to_map(~c"DES-CBC-SHA") do
    suite_str_to_map(~c"TLS_RSA_WITH_DES_CBC_SHA")
  end

  def suite_openssl_str_to_map(~c"DES-CBC3-SHA") do
    suite_str_to_map(~c"TLS_RSA_WITH_3DES_EDE_CBC_SHA")
  end

  def suite_openssl_str_to_map(~c"SRP-DSS-DES-CBC3-SHA") do
    suite_str_to_map(~c"TLS_SRP_SHA_DSS_WITH_3DES_EDE_CBC_SHA")
  end

  def suite_openssl_str_to_map(~c"ADH" ++ rest) do
    suite_openssl_str_to_map(~c"DH-anon", rest)
  end

  def suite_openssl_str_to_map(~c"AECDH" ++ rest) do
    suite_openssl_str_to_map(~c"ECDH-anon", rest)
  end

  def suite_openssl_str_to_map(~c"EDH-RSA" ++ rest) do
    suite_openssl_str_to_map(~c"DHE-RSA", rest)
  end

  def suite_openssl_str_to_map(~c"EDH-DSS-" ++ rest) do
    suite_openssl_str_to_map(~c"DHE-DSS", rest)
  end

  def suite_openssl_str_to_map(~c"DHE-RSA-" ++ rest) do
    suite_openssl_str_to_map(~c"DHE-RSA", rest)
  end

  def suite_openssl_str_to_map(~c"DHE-DSS-" ++ rest) do
    suite_openssl_str_to_map(~c"DHE-DSS", rest)
  end

  def suite_openssl_str_to_map(~c"DHE-PSK-" ++ rest) do
    suite_openssl_str_to_map(~c"DHE-PSK", rest)
  end

  def suite_openssl_str_to_map(~c"DES" ++ _ = rest) do
    suite_openssl_str_to_map(~c"RSA", rest)
  end

  def suite_openssl_str_to_map(~c"AES" ++ _ = rest) do
    suite_openssl_str_to_map(~c"RSA", rest)
  end

  def suite_openssl_str_to_map(~c"RC4" ++ _ = rest) do
    suite_openssl_str_to_map(~c"RSA", rest)
  end

  def suite_openssl_str_to_map(~c"ECDH-RSA-" ++ rest) do
    suite_openssl_str_to_map(~c"ECDH-RSA", rest)
  end

  def suite_openssl_str_to_map(~c"ECDH-ECDSA-" ++ rest) do
    suite_openssl_str_to_map(~c"ECDH-ECDSA", rest)
  end

  def suite_openssl_str_to_map(~c"ECDHE-RSA-" ++ rest) do
    suite_openssl_str_to_map(~c"ECDHE-RSA", rest)
  end

  def suite_openssl_str_to_map(~c"ECDHE-ECDSA-" ++ rest) do
    suite_openssl_str_to_map(~c"ECDHE-ECDSA", rest)
  end

  def suite_openssl_str_to_map(~c"RSA-PSK-" ++ rest) do
    suite_openssl_str_to_map(~c"RSA-PSK", rest)
  end

  def suite_openssl_str_to_map(~c"RSA-" ++ rest) do
    suite_openssl_str_to_map(~c"RSA", rest)
  end

  def suite_openssl_str_to_map(~c"ECDHE-PSK-" ++ rest) do
    suite_openssl_str_to_map(~c"ECDHE-PSK", rest)
  end

  def suite_openssl_str_to_map(~c"PSK-" ++ rest) do
    suite_openssl_str_to_map(~c"PSK", rest)
  end

  def suite_openssl_str_to_map(~c"SRP-RSA-" ++ rest) do
    suite_openssl_str_to_map(~c"SRP-RSA", rest)
  end

  def suite_openssl_str_to_map(~c"SRP-DSS-" ++ rest) do
    suite_openssl_str_to_map(~c"SRP-DSS", rest)
  end

  def suite_openssl_str_to_map(~c"SRP-" ++ rest) do
    suite_openssl_str_to_map(~c"SRP", rest)
  end
  ...
  def suite_bin_to_map(<<0::size(8)-unsigned-big-integer, 108::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_128_cbc, mac: :sha256, prf: :sha256}
  end

  def suite_bin_to_map(<<0::size(8)-unsigned-big-integer, 109::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :dh_anon, cipher: :aes_256_cbc, mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<0::size(8)-unsigned-big-integer, 174::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :psk, cipher: :aes_128_cbc, mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<0::size(8)-unsigned-big-integer, 178::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :dhe_psk, cipher: :aes_128_cbc, mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<0::size(8)-unsigned-big-integer, 180::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :dhe_psk, cipher: :null, mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<0::size(8)-unsigned-big-integer, 184::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :rsa_psk, cipher: :null, mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<192::size(8)-unsigned-big-integer, 55::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :aes_128_cbc, mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<192::size(8)-unsigned-big-integer, 58::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :ecdhe_psk, cipher: :null, mac: :sha256, prf: :sha256}
  end
  ...
  def suite_bin_to_map(<<192::size(8)-unsigned-big-integer, 172::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_ccm, mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192::size(8)-unsigned-big-integer, 173::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_ccm, mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192::size(8)-unsigned-big-integer, 174::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_ccm_8, mac: :aead, prf: :default_prf}
  end

  def suite_bin_to_map(<<192::size(8)-unsigned-big-integer, 175::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_ccm_8, mac: :aead, prf: :default_prf}
  end
  ...
  def suite_bin_to_map(<<0::size(8)-unsigned-big-integer, 164::size(8)-unsigned-big-integer>>) do
    %{key_exchange: :dh_dss, cipher: :aes_128_gcm, mac: :aead, prf: :sha256}
  end
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_128_gcm, mac: :aead, prf: :sha256}) do
    <<208::size(8)-unsigned-big-integer, 1::size(8)-unsigned-big-integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_256_gcm, mac: :aead, prf: :sha384}) do
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_psk, cipher: :aes_128_ccm, mac: :aead, prf: :sha256}) do
  ...
  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_ccm, mac: :aead}) do
    <<192::size(8)-unsigned-big-integer, 172::size(8)-unsigned-big-integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_ccm, mac: :aead}) do
    <<192::size(8)-unsigned-big-integer, 173::size(8)-unsigned-big-integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa, cipher: :aes_128_ccm_8, mac: :aead}) do
    <<192::size(8)-unsigned-big-integer, 174::size(8)-unsigned-big-integer>>
  end

  def suite_map_to_bin(%{key_exchange: :ecdhe_ecdsa, cipher: :aes_256_ccm_8, mac: :aead}) do
    <<192::size(8)-unsigned-big-integer, 175::size(8)-unsigned-big-integer>>
  end
  ...
  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_128_ccm, mac: :aead, prf: :default_prf}) do
    <<192::size(8)-unsigned-big-integer, 164::size(8)-unsigned-big-integer>>
  end

  def suite_map_to_bin(%{key_exchange: :psk, cipher: :aes_256_ccm, mac: :aead, prf: :default_prf}) do
  ...
  def suite_map_to_bin(%{key_exchange: :rsa, cipher: :aes_256_ccm, mac: :aead, prf: :default_prf}) do
  ...
  defp tls_1_3_suite_str_to_map(cipherStr) do
    {cipher, mac, prf} = cipher_str_to_algs(:any, cipherStr, ~c"")
  ...
  defp pre_tls_1_3_suite_str_to_map(kexStr, rest) do
    ...
    [cipherStr, algStr] = :string.split(rest, ~c"_", :trailing)
  ...
  defp cipher_str_to_algs(:any, cipherStr0, ~c"") do
    [cipherStr, algStr] = :string.split(cipherStr0, ~c"_", :trailing)
    hash = algo_str_to_atom(algStr)
    cipher = algo_str_to_atom(cipherStr)
    {cipher, :aead, hash}
  end

  defp cipher_str_to_algs(_Kex, cipherStr, ~c"CCM" = end__) do
    cipher = algo_str_to_atom(cipherStr ++ ~c"_" ++ end__)
    {cipher, :aead, :default_prf}
  end

  defp cipher_str_to_algs(_Kex, cipherStr, ~c"GCM" = end__) do
    cipher = algo_str_to_atom(cipherStr ++ ~c"_" ++ end__)
    {cipher, :aead, :default_prf}
  end

  defp cipher_str_to_algs(_Kex, cipherStr, ~c"8" = end__) do
    cipher = algo_str_to_atom(cipherStr ++ ~c"_" ++ end__)
    {cipher, :aead, :default_prf}
  end

  defp cipher_str_to_algs(_Kex, ~c"CHACHA20_POLY1305" = cipherStr, ~c"") do
    cipher = algo_str_to_atom(cipherStr)
    {cipher, :aead, :sha256}
  end
  ...
  defp is_aead_cipher(~c"CHACHA20_POLY1305") do
    true
  end

  defp is_aead_cipher(cipherStr) do
    [_, rest] = :string.split(cipherStr, ~c"_", :trailing)
    rest == ~c"GCM" or rest == ~c"CCM" or rest == ~c"8"
  end

  defp openssl_is_aead_cipher(~c"CHACHA20-POLY1305") do
    true
  end

  defp openssl_is_aead_cipher(cipherStr) do
    case :string.split(cipherStr, ~c"-", :trailing) do
      [_, rest] ->
        rest == ~c"GCM" or rest == ~c"CCM" or rest == ~c"CCM8"
  ...
  defp algo_str_to_atom(~c"SRP_SHA_DSS") do
    :srp_dss
  end

  defp algo_str_to_atom(~c"SRP_SHA_RSA") do
    :srp_rsa
  end

  defp algo_str_to_atom(~c"SRP_SHA") do
    :srp_anon
  end

  defp algo_str_to_atom(~c"SRP") do
    :srp_anon
  end
  ...
  defp openssl_cipher_name(kex, ~c"3DES_EDE_CBC" ++ _)
       when kex == :ecdhe_psk or
              kex == :srp_anon or kex == :psk or
              kex == :dhe_psk do
    ~c"3DES-EDE-CBC"
  end

  defp openssl_cipher_name(_, ~c"3DES_EDE_CBC" ++ _) do
    ~c"DES-CBC3"
  end

  defp openssl_cipher_name(kex, ~c"AES_128_CBC" ++ _ = cipherStr)
       when kex == :rsa or
  ...
  defp openssl_cipher_name(kex, ~c"AES_256_CBC" ++ _ = cipherStr)
       when kex == :rsa or
              kex == :dhe_rsa or kex == :dhe_dss or
              kex == :ecdh_rsa or
              kex == :ecdhe_rsa or
  ...
  defp openssl_cipher_name(kex, ~c"AES_128_CBC" ++ _ = cipherStr)
    ...
    :lists.append(:string.replace(cipherStr, ~c"_", ~c"-", :all))
  end
  ...
  defp openssl_cipher_name(_, ~c"AES_128_CBC" ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ ~c"-CBC"
  end

  defp openssl_cipher_name(_, ~c"AES_256_CBC" ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ ~c"-CBC"
  end

  defp openssl_cipher_name(_, ~c"AES_128_GCM_8") do
    openssl_name_concat(~c"AES_128_GCM") ++ ~c"-GCM8"
  end

  defp openssl_cipher_name(_, ~c"AES_256_GCM_8") do
    openssl_name_concat(~c"AES_256_GCM") ++ ~c"-GCM8"
  end

  defp openssl_cipher_name(_, ~c"AES_128_CCM_8") do
    openssl_name_concat(~c"AES_128_CCM") ++ ~c"-CCM8"
  end

  defp openssl_cipher_name(_, ~c"AES_256_CCM_8") do
    openssl_name_concat(~c"AES_256_CCM") ++ ~c"-CCM8"
  end

  defp openssl_cipher_name(_, ~c"AES_128_GCM" ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ ~c"-GCM"
  end

  defp openssl_cipher_name(_, ~c"AES_256_GCM" ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ ~c"-GCM"
  end

  defp openssl_cipher_name(_, ~c"AES_128_CCM" ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ ~c"-CCM"
  end

  defp openssl_cipher_name(_, ~c"AES_256_CCM" ++ _ = cipherStr) do
    openssl_name_concat(cipherStr) ++ ~c"-CCM"
  end

  defp openssl_cipher_name(_, ~c"RC4" ++ _) do
    ~c"RC4"
  end
  ...
  defp openssl_kex_name(~c"RSA", _) do
    ~c""
  end

  defp openssl_kex_name(~c"DH_anon", _) do
    ~c"ADH"
  end

  defp openssl_kex_name(~c"ECDH_anon", _) do
    ~c"AECDH"
  end

  defp openssl_kex_name(~c"SRP_SHA", _) do
    ~c"SRP"
  end

  defp openssl_kex_name(~c"SRP_SHA_RSA", _) do
    ~c"SRP-RSA"
  end

  defp openssl_kex_name(~c"SRP_SHA_DSS", _) do
    ~c"SRP-DSS"
  end

  defp openssl_kex_name(~c"DHE_RSA", cipher)
  ...
  defp openssl_kex_name(kex, _) do
    :lists.append(:string.replace(kex, ~c"_", ~c"-", :all))
  end

  defp kex_name_from_openssl(kex) do
    case :lists.append(:string.replace(kex, ~c"-", ~c"_", :all)) do
      ~c"EDH-RSA" ->
        ~c"DHE_RSA"
  ...
  defp cipher_name_from_openssl(~c"AES128") do
    ~c"AES_128_CBC"
  end

  defp cipher_name_from_openssl(~c"AES256") do
    ~c"AES_256_CBC"
  end

  defp cipher_name_from_openssl(~c"AES128-CCM8") do
    ~c"AES_128_CCM_8"
  end

  defp cipher_name_from_openssl(~c"AES256-CCM8") do
    ~c"AES_256_CCM_8"
  end

  defp cipher_name_from_openssl(~c"AES128-" ++ suffix) do
    ~c"AES_128_" ++ :lists.append(:string.replace(suffix, ~c"-", ~c"_", :all))
  end

  defp cipher_name_from_openssl(~c"AES256-" ++ suffix) do
    ~c"AES_256_" ++ :lists.append(:string.replace(suffix, ~c"-", ~c"_", :all))
  end

  defp cipher_name_from_openssl(~c"AES128_" ++ suffix) do
    ~c"AES_128_" ++ suffix
  end

  defp cipher_name_from_openssl(~c"AES256_" ++ suffix) do
    ~c"AES_256_" ++ suffix
  end

  defp cipher_name_from_openssl(~c"DES-CBC") do
    ~c"DES_CBC"
  end

  defp cipher_name_from_openssl(~c"DES-CBC3") do
    ~c"3DES_EDE_CBC"
  end

  defp cipher_name_from_openssl(~c"3DES-EDE-CBC") do
    ~c"3DES_EDE_CBC"
  end

  defp cipher_name_from_openssl(~c"RC4") do
    ~c"RC4_128"
  end

  defp cipher_name_from_openssl(~c"CHACHA20-POLY1305") do
    ~c"CHACHA20_POLY1305"
  end

  defp cipher_name_from_openssl(str) do
    :lists.append(:string.replace(str, ~c"-", ~c"_", :all))
  end

  defp openssl_name_concat(str0) do
    [str, _] = :string.split(str0, ~c"_", :trailing)
    [part1, part2] = :string.split(str, ~c"_", :trailing)
    part1 ++ part2
  end

  defp suite_openssl_str_to_map(kex0, rest) do
    kex = algo_str_to_atom(kex_name_from_openssl(kex0))
    [part1, part2] = :string.split(rest, ~c"-", :trailing)
    {cipher, mac, prf} = openssl_cipher_str_to_algs(kex, part1, part2)
    %{key_exchange: kex, mac: mac, cipher: cipher, prf: prf}
  end

  defp openssl_cipher_str_to_algs(_, part1, ~c"CCM" = end__) do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part1 ++ ~c"_" ++ end__))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, part1, ~c"GCM" = end__) do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part1 ++ ~c"_" ++ end__))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, part2, ~c"CCM8") do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part2 ++ ~c"-CCM-8"))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, part2, ~c"GCM8") do
    cipher = algo_str_to_atom(cipher_name_from_openssl(part2 ++ ~c"-GCM-8"))
    {cipher, :aead, :default_prf}
  end

  defp openssl_cipher_str_to_algs(_, ~c"CHACHA20", ~c"POLY1305") do
    cipher = :chacha20_poly1305
    {cipher, :aead, :sha256}
  end
