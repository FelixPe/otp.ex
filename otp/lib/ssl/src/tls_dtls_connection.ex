defmodule :m_tls_dtls_connection do
  ...
  def hello(:internal, {:common_client_hello, type, serverHelloExt}, state) do
  ...
  def hello(type, event, state) do
    :ssl_gen_statem.handle_common_event(type, event, :hello, state)
  end
  ...
  def user_hello({:call, from}, :cancel, _State) do
    ...
    throw(
      r_alert(
        ...
        description: 90,
        where: %{
          mfa: {:tls_dtls_connection, :user_hello, 3},
          line: 163,
          file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
        },
  ...
  def user_hello(
        ...
        r_state(static_env: r_static_env(role: role), handshake_env: hSEnv, ssl_options: options0) =
          state0
      ) do
    try do
  ...
  def abbreviated(
        ...
        r_finished(verify_data: data) = finished,
        r_state(
          static_env: r_static_env(role: :client, protocol_cb: connection),
  ...
  def abbreviated(type, event, state) do
    :ssl_gen_statem.handle_common_event(type, event, :abbreviated, state)
  end

  def wait_ocsp_stapling(
        :internal,
        r_certificate(),
        r_state(static_env: r_static_env(protocol_cb: _Connection)) = state
      ) do
    {:next_state, :wait_ocsp_stapling, state, [{:postpone, true}]}
  end
  ...
  def wait_ocsp_stapling(
        ...
        r_state(
          static_env: r_static_env(protocol_cb: _Connection),
          handshake_env: r_handshake_env(ocsp_stapling_state: ocspState) = hsEnv
        ) = state
      ) do
    {:next_state, :certify,
     r_state(state,
  ...
  def wait_ocsp_stapling(type, event, state) do
    :ssl_gen_statem.handle_common_event(type, event, :wait_ocsp_stapling, state)
  end
  ...
  def certify(
    ...
    throw(
      r_alert(
        ...
        description: 40,
        where: %{
          mfa: {:tls_dtls_connection, :certify, 3},
          line: 320,
          file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
        },
  ...
  def certify(
        ...
        r_state(
          static_env: r_static_env(protocol_cb: connection),
          handshake_env: r_handshake_env(ocsp_stapling_state: %{ocsp_expect: :staple})
        ) = state
      ) do
    ...
    case :ssl_handshake.certificate_verify(
      ...
      :valid ->
        connection.next_event(
          ...
          :no_record,
          r_state(state,
            client_certificate_status: :verified,
  ...
  def wait_cert_verify(type, event, state) do
    :ssl_gen_statem.handle_common_event(type, event, :wait_cert_verify, state)
  end
  ...
  def cipher(type, event, state) do
    :ssl_gen_statem.handle_common_event(type, event, :cipher, state)
  end
  ...
  def connection(
        ...
        :renegotiate,
        r_state(
          static_env: r_static_env(protocol_cb: :tls_gen_connection),
          handshake_env: hsEnv
        ) = state
      ) do
    :tls_connection.renegotiate(
  ...
  def connection(
        ...
        :renegotiate,
        r_state(
          static_env: r_static_env(protocol_cb: :dtls_gen_connection),
          handshake_env: hsEnv
        ) = state
      ) do
    :dtls_connection.renegotiate(
      r_state(state, handshake_env: r_handshake_env(hsEnv, renegotiation: {true, from})),
      []
    )
  end
  ...
  def downgrade(type, event, state) do
    :ssl_gen_statem.handle_common_event(type, event, :downgrade, state)
  end
           ...
           case :erlang.get(:log_level) do
             :undefined ->
               :ssl_logger.log(
                 ...
                 %{
                   mfa: {:tls_dtls_connection, :gen_handshake, 4},
                   line: 695,
                   file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
                 }
               )
  ...
  def handle_call(
        {:export_key_materials, [label], [context0], [wantedLength], _},
        from,
        _,
    ...
    seed =
      case context0 do
        ...
        _ ->
          ...
          <<clientRandom::binary, serverRandom::binary, size::size(16)-unsigned-big-integer,
            context0::binary>>
      end
             ...
             case :erlang.get(:log_level) do
               :undefined ->
                 :ssl_logger.log(
                   ...
                   %{
                     mfa: {:tls_dtls_connection, :handle_call, 4},
                     line: 727,
                     file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
                   }
                 )
  ...
  defp certify_client(
         r_state(client_certificate_status: :not_requested) = state,
         _
       ) do
    state
  end
  ...
  defp client_certify_and_key_exchange(state0, connection) do
    state1 =
      do_client_certify_and_key_exchange(
        state0,
        connection
      )
  ...
  defp certify_client_key_exchange(
         ...
         r_state(
           handshake_env: r_handshake_env(kex_keys: eCDHKey),
           client_certificate_status: cCStatus
         ) = state,
         connection
       ) do
  ...
  defp certify_client_key_exchange(
         ...
         r_state(
           handshake_env: r_handshake_env(srp_params: params, kex_keys: key),
           client_certificate_status: cCStatus
         ) = state0,
         connection
       ) do
  ...
  defp key_exchange(
         r_state(
           ...
           session: r_session(private_key: r_ECPrivateKey(parameters: eCCurve) = key) = session
         ) = state,
         _
       )
  ...
  defp rsa_key_exchange(_, _, _) do
    throw(
      r_alert(
        ...
        description: 40,
        where: %{
          mfa: {:tls_dtls_connection, :rsa_key_exchange, 3},
          line: 1302,
          file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
        },
  ...
  defp rsa_psk_key_exchange(_, _, _, _) do
    throw(
      r_alert(
        ...
        description: 40,
        where: %{
          mfa: {:tls_dtls_connection, :rsa_psk_key_exchange, 4},
          line: 1319,
          file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
        },
        reason: :pub_key_is_not_rsa
  ...
  defp request_client_cert(
         r_state(
           ...
           connection_env: r_connection_env(negotiated_version: version),
           ssl_options: %{verify: :verify_peer} = opts
         ) = state0,
         connection
       ) do
    ...
    msg = :ssl_handshake.certificate_request(certDbHandle, certDbRef, hashSigns, tLSVersion)
  ...
  defp generate_srp_server_keys(_SrpParams, 10) do
    throw(
      r_alert(
        ...
        description: 47,
        where: %{
          mfa: {:tls_dtls_connection, :generate_srp_server_keys, 2},
          line: 1494,
          file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
        }
      )
  ...
  defp generate_srp_client_keys(_Generator, _Prime, 10) do
    throw(
      r_alert(
        ...
        description: 47,
        where: %{
          mfa: {:tls_dtls_connection, :generate_srp_client_keys, 3},
          line: 1506,
          file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
        }
      )
      ...
      _ ->
        throw(
          r_alert(
            ...
            description: 47,
            where: %{
              mfa: {:tls_dtls_connection, :handle_srp_identity, 2},
              line: 1526,
              file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
            }
          )
  ...
  def handle_sni_extension(
    ...
    case :ssl_gen_statem.handle_sni_extension(
           possibleSNI,
           state0
         ) do
      {:ok, state} ->
  ...
  defp ocsp_info(
         ...
         %{ocsp_stapling: ocspStapling} = _SslOpts,
         peerCert
       ) do
  ...
  defp select_client_cert_key_pair(
    ...
    case :ssl_handshake.select_hashsign(certRequest, cert, supportedHashSigns, tLSVersion) do
      r_alert() ->
        select_client_cert_key_pair(
          session0,
          certRequest,
      ...
      :no_curve ->
        throw(
          r_alert(
            ...
            description: 71,
            where: %{
              mfa: {:tls_dtls_connection, :assert_curve, 1},
              line: 1736,
              file: ~c"otp/lib/ssl/src/tls_dtls_connection.erl"
            },
  ...
  def handle_trace(
        :csp,
        {:call, {:tls_dtls_connection, :wait_ocsp_stapling, [type, msg | _]}},
        stack
      ) do
    {:io_lib.format(~c"Type = ~w Msg = ~W", [type, msg, 10]), stack}
  end
end
