defmodule :m_tls_record do
  ...
  def init_connection_states(role, version, beastMitigation, maxEarlyDataSize) do
  ...
  def decode_cipher_text(
    ...
    r_ssl_tls(type: type, version: version, fragment: fragment) = cipherTextRecord
          ...
          false ->
            r_alert(
              ...
              where: %{
                ...
                line: 265,
                file: ~c"otp/lib/ssl/src/tls_record.erl"
              }
            )
        end
  ...
  def is_acceptable_version(version, versions) do
    :erlang.element(
      1,
      version
    ) == 3 and :lists.member(version, versions)
  end
  ...
  defp initial_connection_state(connectionEnd, beastMitigation, maxEarlyDataSize) do
    %{
  ...
  def build_tls_record(r_ssl_tls(type: type, version: version, fragment: fragment)) do
  ...
  defp validate_tls_records_type(versions, q, maxFragLen, sslOpts, acc, type, version, length)
  ...
  defp validate_tls_record_version(versions, q, maxFragLen, sslOpts, acc, type, version, length)
  ...
  defp validate_tls_record_version(version, q, maxFragLen, sslOpts, acc, type, version, length) do
