defmodule :m_tls_record do
  ...
  Record.defrecord(:r_cipher_state, :cipher_state, iv: :undefined,
                                        ...
                                        tag_len: :undefined)
  def init_connection_states(role, version, beastMitigation) do
    maxEarlyDataSize = :ssl_config.get_max_early_data_size()
    init_connection_states(role, version, beastMitigation,
                             maxEarlyDataSize)
  end

  def init_connection_states(role, version, beastMitigation,
           maxEarlyDataSize) do
    ...
    pending = :ssl_record.empty_connection_state(connectionEnd,
                                                   version, beastMitigation,
                                                   maxEarlyDataSize)
    %{current_read: current, pending_read: pending,
  ...
  def get_tls_records(data, versions, buffer, maxFragLen, sslOpts)
    ...
    parse_tls_records(versions,
                        {[data], byte_size(data), []}, maxFragLen, sslOpts,
                        :undefined)
  end

  def get_tls_records(data, versions, {hdr, {front, size, rear}},
           maxFragLen, sslOpts) do
    parse_tls_records(versions,
                        {front, size + byte_size(data), [data | rear]},
                        maxFragLen, sslOpts, hdr)
  end
  ...
  def encode_handshake(frag, version,
           %{current_write:
             %{beast_mitigation: beastMitigation,
                 max_fragment_length: maxFragmentLength,
                 security_parameters:
                 r_security_parameters(bulk_cipher_algorithm: bCA)}} = connectionStates) do
    maxLength = (cond do
    ...
    case (:erlang.iolist_size(frag)) do
      n when n > maxLength ->
        data = split_iovec(:erlang.iolist_to_iovec(frag),
                             version, bCA, beastMitigation, maxLength)
  ...
  def encode_alert_record(alert, {3, 4}, connectionStates) do
    :tls_record_1_3.encode_alert_record(alert,
                                          connectionStates)
  end

  def encode_alert_record(r_alert(level: level, description: description),
           version, connectionStates) do
    encode_plain_text(21, version,
                        <<level :: size(8) - unsigned - big - integer,
                            description :: size(8) - unsigned - big - integer>>,
                        connectionStates)
  end

  def encode_change_cipher_spec(version, connectionStates) do
    encode_plain_text(20, version,
  ...
  def encode_data(data, version,
    ...
    fragments = split_iovec(data, version, bCA,
                              beastMitigation, maxLength)
    encode_fragments(23, version, fragments,
                       connectionStates)
  end

  def decode_cipher_text({3, 4}, cipherTextRecord, connectionStates, _) do
    :tls_record_1_3.decode_cipher_text(cipherTextRecord,
                                         connectionStates)
  end

  def decode_cipher_text(_, cipherTextRecord,
           %{current_read:
             %{sequence_number: seq,
                 security_parameters:
                 r_security_parameters(cipher_type: 2,
                     bulk_cipher_algorithm: bulkCipherAlgo),
                 cipher_state: cipherS0}} = connectionStates0,
           _) do
    seqBin = <<seq :: size(64) - unsigned - big - integer>>
    r_ssl_tls(type: type, version: version,
        fragment: fragment) = cipherTextRecord
    {majVer, minVer} = version
    startAdditionalData = <<seqBin :: binary,
                              type :: size(8) - unsigned - big - integer,
                              majVer :: size(8) - unsigned - big - integer,
                              minVer :: size(8) - unsigned - big - integer>>
    cipherS = :ssl_record.nonce_seed(bulkCipherAlgo, seqBin,
                                       cipherS0)
    case (:ssl_record.decipher_aead(bulkCipherAlgo, cipherS,
                                      startAdditionalData, fragment,
                                      version)) do
      plainFragment when is_binary(plainFragment) ->
        %{current_read:
          %{security_parameters: secParams,
              compression_state:
              compressionS0} = readState0} = connectionStates0
        {plain,
           compressionS} = :ssl_record.uncompress(r_security_parameters(secParams, :compression_algorithm),
                                                    plainFragment,
                                                    compressionS0)
        connectionStates = Map.put(connectionStates0, :current_read,
                                                        Map.merge(readState0, %{cipher_state:
                                                                                cipherS,
                                                                                  sequence_number:
                                                                                  seq + 1,
                                                                                  compression_state:
  ...
  def decode_cipher_text(_,
           r_ssl_tls(version: version,
               fragment: cipherFragment) = cipherTextRecord,
           %{current_read: readState0} = connnectionStates0,
           paddingCheck) do
    case (:ssl_record.decipher(version, cipherFragment,
                                 readState0, paddingCheck)) do
      {plainFragment, mac, readState1} ->
        macHash = :ssl_cipher.calc_mac_hash(r_ssl_tls(cipherTextRecord, :type),
                                              version, plainFragment,
                                              readState1)
        case (:ssl_record.is_correct_mac(mac, macHash)) do
          true ->
            %{sequence_number: seq,
                ...
                security_parameters:
                r_security_parameters(compression_algorithm: compAlg)} = readState0
            {plain, compressionS1} = :ssl_record.uncompress(compAlg,
                                                              plainFragment,
                                                              compressionS0)
            connnectionStates = Map.put(connnectionStates0, :current_read,
                                                              Map.merge(readState1, %{sequence_number:
                                                                                      seq + 1,
                                                                                        compression_state:
                                                                                        compressionS1}))
            {r_ssl_tls(cipherTextRecord, fragment: plain),
               connnectionStates}
          false ->
            r_alert(level: 2, description: 20,
                where: %{mfa: {:tls_record, :decode_cipher_text, 4},
                           line: 265, file: 'otp/lib/ssl/src/tls_record.erl'})
        end
      r_alert() = alert ->
        alert
    end
  ...
  def protocol_version_name(:"tlsv1.3") do
    {3, 4}
  end

  def protocol_version_name(:"tlsv1.2") do
    {3, 3}
  end

  def protocol_version_name(:"tlsv1.1") do
    {3, 2}
  end

  def protocol_version_name(:tlsv1) do
    {3, 1}
  end

  def protocol_version_name(:sslv3) do
    {3, 0}
  end

  def protocol_version_name(:sslv2) do
    {2, 0}
  end
  ...
  def lowest_protocol_version(version1, version2) when version1 < version2 do
    version1
  end

  def lowest_protocol_version(_, version2) do
    version2
  end

  def lowest_protocol_version(versions) do
    check_protocol_version(versions,
                             &lowest_protocol_version/2)
  end

  def highest_protocol_version(versions) do
    check_protocol_version(versions,
                             &highest_protocol_version/2)
  end

  defp check_protocol_version([], fun) do
    check_protocol_version(supported_protocol_versions(),
                             fun)
  end

  defp check_protocol_version([ver | versions], fun) do
    :lists.foldl(fun, ver, versions)
  end

  def highest_protocol_version(version1, version2) when version1 > version2 do
    version1
  end

  def highest_protocol_version(_, version2) do
    version2
  end

  def is_higher(v1, v2) when v1 > v2 do
    true
  end
  ...
  def supported_protocol_versions() do
    ...
    case (:application.get_env(:ssl, :protocol_version)) do
      ...
      {:ok, vsns} when is_list(vsns) ->
        versions = :lists.filter(&is_acceptable_version/1,
                                   :lists.map(fun, vsns))
  ...
  defp sufficient_crypto_support(cryptoSupport, version)
    ...
    pKeys = :proplists.get_value(:public_keys,
                                   cryptoSupport)
    :proplists.get_bool(:sha,
                          hashes) and :proplists.get_bool(:md5,
                                                            hashes) and :proplists.get_bool(:aes_cbc,
                                                                                              :proplists.get_value(:ciphers,
                                                                                                                     cryptoSupport)) and (:proplists.get_bool(:ecdsa,
                                                                                                                                                                pKeys) or :proplists.get_bool(:rsa,
                                                                                                                                                                                                pKeys) or :proplists.get_bool(:dss,
                                                                                                                                                                                                                                pKeys)) and (:proplists.get_bool(:ecdh,
                                                                                                                                                                                                                                                                   pKeys) or :proplists.get_bool(:dh,
                                                                                                                                                                                                                                                                                                   pKeys))
  end
  ...
  defp sufficient_crypto_support(cryptoSupport, :"tlsv1.2") do
    pKeys = :proplists.get_value(:public_keys,
                                   cryptoSupport)
    :proplists.get_bool(:sha256,
                          :proplists.get_value(:hashs,
                                                 cryptoSupport)) and :proplists.get_bool(:aes_cbc,
                                                                                           :proplists.get_value(:ciphers,
                                                                                                                  cryptoSupport)) and (:proplists.get_bool(:ecdsa,
                                                                                                                                                             pKeys) or :proplists.get_bool(:rsa,
                                                                                                                                                                                             pKeys) or :proplists.get_bool(:dss,
  ...
  defp sufficient_crypto_support(cryptoSupport, :"tlsv1.3") do
    ...
    l = [{:ciphers, :aes_gcm}, {:ciphers,
                                  :chacha20_poly1305},
                                   {:hashs, :sha256}, {:hashs, :sha384},
                                                          {:rsa_opts,
                                                             :rsa_pkcs1_padding},
                                                              {:rsa_opts,
  ...
  defp sufficient_crypto_support(cryptoSupport, version) do
    sufficient_crypto_support(cryptoSupport,
                                protocol_version(version))
  end

  defp is_algorithm_supported(cryptoSupport, group, algorithm) do
    :proplists.get_bool(algorithm,
                          :proplists.get_value(group, cryptoSupport))
  end

  def is_acceptable_version(version) when :erlang.element(1, version) == 3 do
    true
  end
  ...
  def is_acceptable_version(version, versions) do
    :erlang.element(1,
                      version) == 3 and :lists.member(version, versions)
  end
  ...
  defp initial_connection_state(connectionEnd, beastMitigation,
            maxEarlyDataSize) do
    %{security_parameters:
      :ssl_record.initial_security_params(connectionEnd),
        sequence_number: 0, beast_mitigation: beastMitigation,
        compression_state: :undefined, cipher_state: :undefined,
        ...
        trial_decryption: false, early_data_expected: false}
  end

  def build_tls_record(r_ssl_tls(type: type, version: version,
  ...
  defp parse_tls_records(versions, q, maxFragLen, sslOpts, :undefined) do
    decode_tls_records(versions, q, maxFragLen, sslOpts, [],
                         :undefined, :undefined, :undefined)
  end

  defp parse_tls_records(versions, q, maxFragLen, sslOpts,
            r_ssl_tls(type: type, version: version, fragment: length)) do
    decode_tls_records(versions, q, maxFragLen, sslOpts, [],
                         type, version, length)
  end

  defp decode_tls_records(versions, {_, size, _} = q0, maxFragLen,
            sslOpts, acc, :undefined, _Version, _Length) do
    cond do
      5 <= size ->
        {<<type :: size(8) - unsigned - big - integer,
             majVer :: size(8) - unsigned - big - integer,
        ...
        validate_tls_records_type(versions, q, maxFragLen,
                                    sslOpts, acc, type, version, length)
      3 <= size ->
        {<<type :: size(8) - unsigned - big - integer,
             majVer :: size(8) - unsigned - big - integer,
             minVer :: size(8) - unsigned - big - integer>>,
           q} = binary_from_front(3, q0)
        version = {majVer, minVer}
        validate_tls_records_type(versions, q, maxFragLen,
                                    sslOpts, acc, type, version, :undefined)
      1 <= size ->
        {<<type :: size(8) - unsigned - big - integer>>,
           q} = binary_from_front(1, q0)
        validate_tls_records_type(versions, q, maxFragLen,
                                    sslOpts, acc, type, :undefined, :undefined)
      true ->
        validate_tls_records_type(versions, q0, maxFragLen,
                                    sslOpts, acc, :undefined, :undefined,
                                    :undefined)
    end
  end

  defp decode_tls_records(versions, {_, size, _} = q0, maxFragLen,
            sslOpts, acc, type, :undefined, _Length) do
    cond do
      4 <= size ->
        {<<majVer :: size(8) - unsigned - big - integer,
             minVer :: size(8) - unsigned - big - integer,
             length :: size(16) - unsigned - big - integer>>,
           q} = binary_from_front(4, q0)
        version = {majVer, minVer}
        validate_tls_record_version(versions, q, maxFragLen,
                                      sslOpts, acc, type, version, length)
      2 <= size ->
        {<<majVer :: size(8) - unsigned - big - integer,
             minVer :: size(8) - unsigned - big - integer>>,
           q} = binary_from_front(2, q0)
        version = {majVer, minVer}
        validate_tls_record_version(versions, q, maxFragLen,
                                      sslOpts, acc, type, version, :undefined)
      true ->
        validate_tls_record_version(versions, q0, maxFragLen,
                                      sslOpts, acc, type, :undefined,
                                      :undefined)
    end
  end

  defp decode_tls_records(versions, {_, size, _} = q0, maxFragLen,
            sslOpts, acc, type, version, :undefined) do
    cond do
      2 <= size ->
        {<<length :: size(16) - unsigned - big - integer>>,
           q} = binary_from_front(2, q0)
        validate_tls_record_length(versions, q, maxFragLen,
                                     sslOpts, acc, type, version, length)
      true ->
        validate_tls_record_length(versions, q0, maxFragLen,
                                     sslOpts, acc, type, version, :undefined)
    end
  end

  defp decode_tls_records(versions, q, maxFragLen, sslOpts, acc, type,
            version, length) do
    validate_tls_record_length(versions, q, maxFragLen,
                                 sslOpts, acc, type, version, length)
  end

  defp validate_tls_records_type(_Versions, q, _MaxFragLen, _SslOpts, acc,
            :undefined, _Version, _Length) do
    {:lists.reverse(acc), {:undefined, q}}
  end

  defp validate_tls_records_type(versions, q, maxFragLen, sslOpts, acc, type,
            version, length)
      when is_integer(type) and 20 <= type and type <= 23 do
    validate_tls_record_version(versions, q, maxFragLen,
                                  sslOpts, acc, type, version, length)
  end

  defp validate_tls_records_type(_Versions, _Q, _MaxFragLen, _SslOpts, _Acc,
            type, _Version, _Length) do
    r_alert(level: 2, description: 10,
        where: %{mfa:
                 {:tls_record, :validate_tls_records_type, 8},
                   line: 561, file: 'otp/lib/ssl/src/tls_record.erl'},
        reason: {:unsupported_record_type, type})
  end

  defp validate_tls_record_version(_Versions, q, _MaxFragLen, _SslOpts, acc, type,
            :undefined, _Length) do
    {:lists.reverse(acc),
       {r_ssl_tls(type: type, version: :undefined,
            fragment: :undefined),
          q}}
  end

  defp validate_tls_record_version(versions, q, maxFragLen, sslOpts, acc, type,
            version, length)
    ...
    case (is_acceptable_version(version, versions)) do
      ...
      false ->
        r_alert(level: 2, description: 20,
            where: %{mfa:
                     {:tls_record, :validate_tls_record_version, 8},
                       line: 572, file: 'otp/lib/ssl/src/tls_record.erl'},
            reason: {:unsupported_version, version})
    end
  end

  defp validate_tls_record_version({3, 4} = versions, q, maxFragLen, sslOpts, acc,
            type, {3, 3} = version, length) do
    validate_tls_record_length(versions, q, maxFragLen,
                                 sslOpts, acc, type, version, length)
  end

  defp validate_tls_record_version(version, q, maxFragLen, sslOpts, acc, type,
            version, length) do
    validate_tls_record_length(version, q, maxFragLen,
                                 sslOpts, acc, type, version, length)
  end

  defp validate_tls_record_version(_Versions, _Q, _MaxFragLen, _SslOpts, _Acc,
            _Type, version, _Length) do
    r_alert(level: 2, description: 20,
        where: %{mfa:
                 {:tls_record, :validate_tls_record_version, 8},
                   line: 580, file: 'otp/lib/ssl/src/tls_record.erl'},
        reason: {:unsupported_version, version})
  end

  defp validate_tls_record_length(_Versions, q, _MaxFragLen, _SslOpts, acc, type,
            version, :undefined) do
    {:lists.reverse(acc),
       {r_ssl_tls(type: type, version: version, fragment: :undefined),
          q}}
  end

  defp validate_tls_record_length(versions, {_, size0, _} = q0, maxFragLen,
            %{log_level: logLevel, downgrade: downgrade} = sslOpts,
            acc, type, version, length) do
    max = (cond do
      ...
      length <= max ->
        cond do
          length <= size0 ->
            ...
            record = r_ssl_tls(type: type, version: version,
                         fragment: fragment)
            ...
            case (downgrade) do
              ...
              _ ->
                decode_tls_records(versions, q, maxFragLen, sslOpts,
                                     [record | acc], :undefined, :undefined,
                                     :undefined)
            end
          true ->
            {:lists.reverse(acc),
               {r_ssl_tls(type: type, version: version, fragment: length), q0}}
        end
      true ->
        r_alert(level: 2, description: 22,
            where: %{mfa:
                     {:tls_record, :validate_tls_record_length, 8},
                       line: 614, file: 'otp/lib/ssl/src/tls_record.erl'})
    end
  end
  ...
  defp binary_from_front(splitSize, [], size, rear, acc) do
    case (rear) do
      ...
      [bin2, bin1] ->
        binary_from_front(splitSize, [bin1, bin2], size, [],
                            acc)
      [bin3, bin2, bin1] ->
        binary_from_front(splitSize, [bin1, bin2, bin3], size,
                            [], acc)
      [_, _, _ | _] ->
        binary_from_front(splitSize, :lists.reverse(rear), size,
                            [], acc)
    end
  end
  ...
  defp binary_from_front(splitSize, [bin | front], size, rear, []) do
    ...
    cond do
      ...
      binSize < splitSize ->
        binary_from_front(splitSize - binSize, front, size,
                            rear, [bin])
      true ->
  ...
  defp binary_from_front(splitSize, [bin | front], size, rear, acc) do
    ...
    cond do
      splitSize < binSize ->
        ...
        retBin = :erlang.iolist_to_binary(:lists.reverse(acc,
                                                           [last]))
        {retBin,
           {[rest | front], size - byte_size(retBin), rear}}
      binSize < splitSize ->
        binary_from_front(splitSize - binSize, front, size,
                            rear, [bin | acc])
      true ->
  ...
  def encode_plain_text(type, version, data, connectionStates0) do
    {[cipherText],
       connectionStates} = encode_fragments(type, version,
                                              [data], connectionStates0)
    {cipherText, connectionStates}
  end

  defp encode_fragments(type, version, data,
            %{current_write:
              %{compression_state: compS, cipher_state: cipherS,
                  sequence_number: seq}} = connectionStates) do
    encode_fragments(type, version, data, connectionStates,
                       compS, cipherS, seq, [])
  end

  defp encode_fragments(_Type, _Version, [],
            %{current_write: writeS} = cS, compS, cipherS, seq,
            cipherFragments) do
  ...
  defp encode_fragments(type, version, [text | data],
            %{current_write:
              %{security_parameters:
                r_security_parameters(cipher_type: 2, bulk_cipher_algorithm: bCAlg,
                    compression_algorithm: compAlg) = secPars}} = cS,
            compS0, cipherS0, seq, cipherFragments) do
    {compText, compS} = :ssl_record.compress(compAlg, text,
                                               compS0)
    seqBin = <<seq :: size(64) - unsigned - big - integer>>
    cipherS1 = :ssl_record.nonce_seed(bCAlg, seqBin,
                                        cipherS0)
    {majVer, minVer} = version
    versionBin = <<majVer
                   ::
    ...
    startAdditionalData = <<seqBin :: binary,
                              ...
                              versionBin :: binary>>
    {cipherFragment,
       cipherS} = :ssl_record.cipher_aead(version, compText,
                                            cipherS1, startAdditionalData,
                                            secPars)
    ...
    cipherHeader = <<type
                     ...
                     size(8) - unsigned - big - integer,
                       versionBin :: binary,
                       length :: size(16) - unsigned - big - integer>>
    encode_fragments(type, version, data, cS, compS,
                       cipherS, seq + 1,
  ...
  defp encode_fragments(type, version, [text | data],
            %{current_write:
              %{security_parameters:
                r_security_parameters(compression_algorithm: compAlg,
                    mac_algorithm: macAlgorithm) = secPars,
                  mac_secret: macSecret}} = cS,
            compS0, cipherS0, seq, cipherFragments) do
    {compText, compS} = :ssl_record.compress(compAlg, text,
                                               compS0)
    macHash = :ssl_cipher.calc_mac_hash(type, version,
                                          compText, macAlgorithm, macSecret,
                                          seq)
    {cipherFragment, cipherS} = :ssl_record.cipher(version,
                                                     compText, cipherS0,
                                                     macHash, secPars)
    length = byte_size(cipherFragment)
    {majVer, minVer} = version
  ...
  defp split_iovec(data, {3, 1}, bCA, :one_n_minus_one, maxLength)
      when bCA !== 1 do
    {part, restData} = split_iovec(data, 1, [])
    [part | split_iovec(restData, maxLength)]
  end

  defp split_iovec(data, {3, 1}, bCA, :zero_n, maxLength)
      when bCA !== 1 do
    {part, restData} = split_iovec(data, 0, [])
    [part | split_iovec(restData, maxLength)]
  end

  defp split_iovec(data, _Version, _BCA, _BeatMitigation,
            maxLength) do
