defmodule :m_ssl_record do
  ...
  def step_encryption_state_read(
        r_state(connection_states: %{pending_read: pendingRead} = connStates) = state
      ) do
    newRead = Map.put(pendingRead, :sequence_number, 0)
    r_state(state, connection_states: Map.put(connStates, :current_read, newRead))
  end

  def step_encryption_state_write(
        r_state(connection_states: %{pending_write: pendingWrite} = connStates) = state
      ) do
    newWrite = Map.put(pendingWrite, :sequence_number, 0)
    r_state(state, connection_states: Map.put(connStates, :current_write, newWrite))
  end
               ...
               :undefined ->
                 :ssl_logger.log(
                   ...
                   %{
                     ...
                     line: 451,
                     file: ~c"otp/lib/ssl/src/ssl_record.erl"
                   }
                 )
  ...
  def empty_connection_state(connectionEnd, version, beastMitigation, maxEarlyDataSize) do
    secParams =
