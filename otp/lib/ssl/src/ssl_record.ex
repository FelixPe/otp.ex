defmodule :m_ssl_record do
  ...
  def activate_pending_connection_state(%{current_read: current,
    ...
    emptyPending = connection.empty_connection_state(connectionEnd,
                                                       beastMitigation)
    newPending = Map.put(emptyPending, :secure_renegotiation,
                                         secureRenegotation)
    Map.merge(states, %{current_read: newCurrent,
                          pending_read: newPending})
  end

  def activate_pending_connection_state(%{current_write: current,
    ...
    newPending = Map.put(emptyPending, :secure_renegotiation,
                                         secureRenegotation)
    Map.merge(states, %{current_write: newCurrent,
                          pending_write: newPending})
  end

  def step_encryption_state(r_state(connection_states: %{pending_read: pendingRead,
                                  pending_write:
                                  pendingWrite} = connStates) = state) do
    newRead = Map.put(pendingRead, :sequence_number, 0)
    newWrite = Map.put(pendingWrite, :sequence_number, 0)
    r_state(state, connection_states: Map.merge(connStates, %{current_read:
                                                        newRead,
                                                          current_write:
                                                          newWrite}))
  end

  def step_encryption_state_read(r_state(connection_states: %{pending_read:
                                pendingRead} = connStates) = state) do
    newRead = Map.put(pendingRead, :sequence_number, 0)
    r_state(state, connection_states: Map.put(connStates, :current_read,
                                                      newRead))
  end

  def step_encryption_state_write(r_state(connection_states: %{pending_write:
                                pendingWrite} = connStates) = state) do
    newWrite = Map.put(pendingWrite, :sequence_number, 0)
    r_state(state, connection_states: Map.put(connStates, :current_write,
                                                      newWrite))
  end

  def set_security_params(readParams, writeParams,
           %{pending_read: read, pending_write: write} = states) do
    Map.merge(states, %{pending_read:
                        Map.put(read, :security_parameters, readParams),
                          pending_write:
                          Map.put(write, :security_parameters, writeParams)})
  end

  def set_mac_secret(clientWriteMacSecret, serverWriteMacSecret,
           :client, states) do
    set_mac_secret(serverWriteMacSecret,
                     clientWriteMacSecret, states)
  end

  def set_mac_secret(clientWriteMacSecret, serverWriteMacSecret,
           :server, states) do
    set_mac_secret(clientWriteMacSecret,
                     serverWriteMacSecret, states)
  end

  defp set_mac_secret(readMacSecret, writeMacSecret,
            states = %{pending_read: read, pending_write: write}) do
    Map.merge(states, %{pending_read:
                        Map.put(read, :mac_secret, readMacSecret),
                          pending_write:
                          Map.put(write, :mac_secret, writeMacSecret)})
  end

  def set_master_secret(masterSecret,
           states = %{pending_read:
                      read = %{security_parameters: readSecPar},
                        pending_write:
                        write = %{security_parameters: writeSecPar}}) do
    read1 = Map.put(read, :security_parameters,
                            r_security_parameters(readSecPar, master_secret: masterSecret))
    write1 = Map.put(write, :security_parameters,
                              r_security_parameters(writeSecPar, master_secret: masterSecret))
    Map.merge(states, %{pending_read: read1,
                          pending_write: write1})
  end

  def set_renegotiation_flag(flag,
           %{current_read: currentRead0,
               current_write: currentWrite0,
               pending_read: pendingRead0,
               pending_write: pendingWrite0} = connectionStates) do
    currentRead = Map.put(currentRead0, :secure_renegotiation,
                                          flag)
    currentWrite = Map.put(currentWrite0, :secure_renegotiation,
                                            flag)
    pendingRead = Map.put(pendingRead0, :secure_renegotiation,
                                          flag)
    pendingWrite = Map.put(pendingWrite0, :secure_renegotiation,
                                            flag)
    Map.merge(connectionStates, %{current_read: currentRead,
                                    current_write: currentWrite,
                                    pending_read: pendingRead,
                                    pending_write: pendingWrite})
  end

  def set_max_fragment_length(r_max_frag_enum(enum: maxFragEnum),
           %{current_read: currentRead0,
               current_write: currentWrite0,
               pending_read: pendingRead0,
               pending_write: pendingWrite0} = connectionStates)
      when maxFragEnum == 1 or maxFragEnum == 2 or maxFragEnum == 3 or maxFragEnum == 4 do
    maxFragmentLength = (cond do
                           maxFragEnum == 1 ->
                             512
                           maxFragEnum == 2 ->
                             1024
    ...
    currentRead = Map.put(currentRead0, :max_fragment_length,
                                          maxFragmentLength)
    currentWrite = Map.put(currentWrite0, :max_fragment_length,
                                            maxFragmentLength)
    pendingRead = Map.put(pendingRead0, :max_fragment_length,
                                          maxFragmentLength)
    pendingWrite = Map.put(pendingWrite0, :max_fragment_length,
                                            maxFragmentLength)
    Map.merge(connectionStates, %{current_read: currentRead,
                                    current_write: currentWrite,
                                    pending_read: pendingRead,
  ...
  def set_client_verify_data(:current_read, data,
           %{current_read: currentRead0,
               pending_write: pendingWrite0} = connectionStates) do
    currentRead = Map.put(currentRead0, :client_verify_data,
                                          data)
    pendingWrite = Map.put(pendingWrite0, :client_verify_data,
                                            data)
    Map.merge(connectionStates, %{current_read: currentRead,
                                    pending_write: pendingWrite})
  end

  def set_client_verify_data(:current_write, data,
           %{pending_read: pendingRead0,
               current_write: currentWrite0} = connectionStates) do
    pendingRead = Map.put(pendingRead0, :client_verify_data,
                                          data)
    currentWrite = Map.put(currentWrite0, :client_verify_data,
                                            data)
    Map.merge(connectionStates, %{pending_read: pendingRead,
                                    current_write: currentWrite})
  end

  def set_client_verify_data(:current_both, data,
           %{current_read: currentRead0,
               current_write: currentWrite0} = connectionStates) do
    currentRead = Map.put(currentRead0, :client_verify_data,
                                          data)
  ...
  def set_server_verify_data(:current_write, data,
           %{pending_read: pendingRead0,
               current_write: currentWrite0} = connectionStates) do
    pendingRead = Map.put(pendingRead0, :server_verify_data,
                                          data)
    currentWrite = Map.put(currentWrite0, :server_verify_data,
                                            data)
    Map.merge(connectionStates, %{pending_read: pendingRead,
                                    current_write: currentWrite})
  end

  def set_server_verify_data(:current_read, data,
           %{current_read: currentRead0,
               pending_write: pendingWrite0} = connectionStates) do
    currentRead = Map.put(currentRead0, :server_verify_data,
                                          data)
    pendingWrite = Map.put(pendingWrite0, :server_verify_data,
                                            data)
    Map.merge(connectionStates, %{current_read: currentRead,
                                    pending_write: pendingWrite})
  end

  def set_server_verify_data(:current_both, data,
           %{current_read: currentRead0,
               current_write: currentWrite0} = connectionStates) do
    currentRead = Map.put(currentRead0, :server_verify_data,
                                          data)
  ...
  def set_pending_cipher_state(%{pending_read: read,
             pending_write: write} = states,
           clientState, serverState, :server) do
    Map.merge(states, %{pending_read:
                        Map.put(read, :cipher_state, clientState),
                          pending_write:
                          Map.put(write, :cipher_state, serverState)})
  end

  def set_pending_cipher_state(%{pending_read: read,
             pending_write: write} = states,
           clientState, serverState, :client) do
    Map.merge(states, %{pending_read:
                        Map.put(read, :cipher_state, serverState),
                          pending_write:
                          Map.put(write, :cipher_state, clientState)})
  end
  ...
  def cipher(version, fragment,
           %{cipher_state: cipherS0,
               security_parameters:
               r_security_parameters(bulk_cipher_algorithm: bulkCipherAlgo)} = writeState0,
           macHash) do
    {cipherFragment,
       cipherS1} = :ssl_cipher.cipher(bulkCipherAlgo, cipherS0,
                                        macHash, fragment, version)
    {cipherFragment,
       Map.put(writeState0, :cipher_state, cipherS1)}
  end

  def cipher(version, fragment, cipherS0, macHash,
           r_security_parameters(bulk_cipher_algorithm: bulkCipherAlgo)) do
    :ssl_cipher.cipher(bulkCipherAlgo, cipherS0, macHash,
                         fragment, version)
  end

  def cipher_aead(_Version, fragment,
           %{cipher_state: cipherS0,
               security_parameters:
               r_security_parameters(bulk_cipher_algorithm: bulkCipherAlgo)} = writeState0,
           aAD) do
    {cipherFragment,
       cipherS1} = do_cipher_aead(bulkCipherAlgo, fragment,
                                    cipherS0, aAD)
    {cipherFragment,
       Map.put(writeState0, :cipher_state, cipherS1)}
  end

  def cipher_aead(_Version, fragment, cipherS, aAD,
           r_security_parameters(bulk_cipher_algorithm: bulkCipherAlgo)) do
    do_cipher_aead(bulkCipherAlgo, fragment, cipherS, aAD)
  end

  def decipher(version, cipherFragment,
           %{security_parameters:
             r_security_parameters(bulk_cipher_algorithm: bulkCipherAlgo,
                 hash_size: hashSz),
               cipher_state: cipherS0} = readState,
           paddingCheck) do
    case (:ssl_cipher.decipher(bulkCipherAlgo, hashSz,
                                 cipherS0, cipherFragment, version,
                                 paddingCheck)) do
  ...
  def decipher_aead(type, r_cipher_state(key: key) = cipherState, aAD0,
           cipherFragment, _) do
    try do
      nonce = decrypt_nonce(type, cipherState, cipherFragment)
      {aAD, cipherText,
         cipherTag} = aead_ciphertext_split(type, cipherState,
                                              cipherFragment, aAD0)
      case (:ssl_cipher.aead_decrypt(type, key, nonce,
                                       cipherText, cipherTag, aAD)) do
        ...
        reason ->
                  ...
                  :undefined ->
                    :ssl_logger.log(:debug, :debug,
                                      ...
                                      %{mfa: {:ssl_record, :decipher_aead, 5},
                                          line: 451, file: 'otp/lib/ssl/src/ssl_record.erl'})
                  __LogLevel__ ->
          ...
          r_alert(level: 2, description: 20,
              where: %{mfa: {:ssl_record, :decipher_aead, 5},
                         line: 452, file: 'otp/lib/ssl/src/ssl_record.erl'},
      ...
      _, reason2 ->
              ...
              case (:erlang.get(:log_level)) do
                :undefined ->
                  :ssl_logger.log(:debug, :debug,
                                    %{description: :decrypt_error,
                                        reason:
                                        [{:reason, reason2}, {:stacktrace,
                                                                __STACKTRACE__}]},
                                    %{mfa: {:ssl_record, :decipher_aead, 5},
                                        line: 456, file: 'otp/lib/ssl/src/ssl_record.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:debug, __LogLevel__,
                                    %{description: :decrypt_error,
                                        reason:
        ...
        r_alert(level: 2, description: 20,
            where: %{mfa: {:ssl_record, :decipher_aead, 5},
                       line: 457, file: 'otp/lib/ssl/src/ssl_record.erl'},
            reason: :decryption_failed)
    end
  end
  ...
  def empty_connection_state(connectionEnd, beastMitigation) do
    ...
    empty_connection_state(connectionEnd,
                             _Version = :undefined, beastMitigation,
                             maxEarlyDataSize)
  end

  def empty_connection_state(connectionEnd, version, beastMitigation,
           maxEarlyDataSize) do
    secParams = init_security_parameters(connectionEnd,
                                           version)
    %{security_parameters: secParams,
        ...
        trial_decryption: false, early_data_accepted: false}
  end

  defp init_security_parameters(1, version) do
    r_security_parameters(connection_end: 1,
        client_random: make_random(version))
  end

  defp init_security_parameters(0, version) do
    r_security_parameters(connection_end: 0,
        server_random: make_random(version))
  end

  defp make_random(version) when version >= {3, 4} do
    :ssl_cipher.random_bytes(32)
  end

  defp make_random(_Version) do
    secs_since_1970 = :calendar.datetime_to_gregorian_seconds(:calendar.universal_time()) - 62167219200
    random_28_bytes = :ssl_cipher.random_bytes(28)
    <<secs_since_1970
      ::
      size(32) - unsigned - big - integer,
        random_28_bytes :: binary>>
  end
  ...
  def initial_security_params(connectionEnd) do
    secParams = r_security_parameters(connection_end: connectionEnd,
                    compression_algorithm: 0)
    :ssl_cipher.security_parameters(<<0
                                      ::
  ...
  defp do_cipher_aead(9 = type, fragment,
            r_cipher_state(key: key, tag_len: tagLen) = cipherState, aAD0) do
    aAD = <<(
              aAD0
            )
            ::
            binary,
              (
                :erlang.iolist_size(fragment)
              )
              ::
              size(16) - unsigned - big - integer>>
    nonce = chacha_nonce(cipherState)
    {content, cipherTag} = :ssl_cipher.aead_encrypt(type,
                                                      key, nonce, fragment, aAD,
                                                      tagLen)
    {<<content :: binary, cipherTag :: binary>>,
       cipherState}
  end

  defp do_cipher_aead(type, fragment,
            r_cipher_state(key: key, tag_len: tagLen,
                nonce: explicitNonce) = cipherState,
            aAD0) do
    aAD = <<(
              aAD0
            )
  ...
  defp aead_ciphertext_split(9, r_cipher_state(tag_len: len), cipherTextFragment, aAD) do
    ...
    <<cipherText :: size(cipherLen) - bytes,
        cipherTag :: size(len) - bytes>> = cipherTextFragment
    {<<(
         aAD
       )
       ::
