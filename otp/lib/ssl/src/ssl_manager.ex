defmodule :m_ssl_manager do
  ...
  def name(:dist) do
    :erlang.list_to_atom(:erlang.atom_to_list(:ssl_manager) ++ ~c"_dist")
  end
  ...
  def insert_crls(~c"dummy/no_distribution_point" = path, cRLs, managerType) do
    :erlang.put(:ssl_manager, name(managerType))
  ...
  def delete_crls(~c"dummy/no_distribution_point" = path, managerType) do
    :erlang.put(:ssl_manager, name(managerType))
  ...
  def init([managerName, pemCacheName, opts]) do
    ...
    sessionLifeTime = :proplists.get_value(:session_lifetime, opts, defaultSessLifeTime)
  ...
  def handle_call({:refresh_trusted_db, _}, _, r_state(certificate_db: db) = state) do
    pemCache = :erlang.get(:ssl_pem_cache)
    :ssl_pkix_db.refresh_trusted_certs(db, pemCache)
    {:reply, :ok, state}
  end

  def handle_call({{:refresh_trusted_db, file}, _}, _, r_state(certificate_db: db) = state) do
    pemCache = :erlang.get(:ssl_pem_cache)
  ...
  defp invalidate_session(
         cache,
         cacheCb,
         key,
         _Session,
         r_state(client_session_order: order) = state
       ) do
  ...
  defp do_register_session(
         key,
         r_session(time_stamp: timeStamp) = session0,
         max,
         cache,
         cacheCb,
         options,
