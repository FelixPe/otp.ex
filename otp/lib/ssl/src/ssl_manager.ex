defmodule :m_ssl_manager do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_state, :state, session_cache_client: :undefined,
                                 session_cache_client_cb: :undefined,
                                 ...
                                 options: :undefined,
                                 client_session_order: :undefined)
  def name(:normal) do
    :ssl_manager
  end
  ...
  def start_link(opts) do
    ...
    :gen_server.start_link({:local, mangerName},
                             :ssl_manager, [mangerName, cacheName, opts], [])
  end
  ...
  def connection_init({:der, _} = trustedcerts, role, cRLCache) do
    {:ok,
       extracted} = :ssl_pkix_db.extract_trusted_certs(trustedcerts)
  ...
  def lookup_trusted_cert(dbHandle, ref, serialNumber, issuer) do
    :ssl_pkix_db.lookup_trusted_cert(dbHandle, ref,
                                       serialNumber, issuer)
  end

  def clean_cert_db(ref, file) do
    :erlang.send_after(500, :erlang.get(:ssl_manager),
                         {:clean_cert_db, ref, file})
    :ok
  end

  def refresh_trusted_db(managerType) do
    :erlang.put(:ssl_manager, name(managerType))
    call(:refresh_trusted_db)
  end

  def refresh_trusted_db(managerType, file) do
    :erlang.put(:ssl_manager, name(managerType))
  ...
  def init([managerName, pemCacheName, opts]) do
    ...
    %{session_cb: defaultCacheCb,
        ...
        max:
        clientSessMax} = :ssl_config.pre_1_3_session_opts(:client)
    cacheCb = :proplists.get_value(:session_cb, opts,
                                     defaultCacheCb)
    sessionLifeTime = :proplists.get_value(:session_lifetime,
                                             opts, defaultSessLifeTime)
    clientSessionCache = cacheCb.init([{:role, :client} |
                                           :proplists.get_value(:session_cb_init_args,
                                                                  opts,
                                                                  defaultCacheCbInitArgs)])
    certDb = :ssl_pkix_db.create(pemCacheName)
    timer = :erlang.send_after(sessionLifeTime * 1000 + 5000,
                                 self(), :validate_sessions)
    {:ok,
       r_state(certificate_db: certDb,
           session_cache_client: clientSessionCache,
           session_cache_client_cb: cacheCb,
           session_lifetime: sessionLifeTime,
           session_validation_timer: timer,
           session_cache_client_max: clientSessMax,
           session_client_invalidator: :undefined, options: opts,
           client_session_order: :gb_trees.empty())}
  end

  def handle_call({{:connection_init, <<>>, role,
             {cRLCb, userCRLDb}},
            _Pid},
           _From,
           r_state(certificate_db: [certDb, fileRefDb, pemCache |
                                                     _] = db) = state) do
    ref = make_ref()
    {:reply,
       {:ok,
          %{cert_db_ref: ref, cert_db_handle: certDb,
              fileref_db_handle: fileRefDb, pem_cache: pemCache,
              session_cache: session_cache(role, state),
              crl_db_info: {cRLCb, crl_db_info(db, userCRLDb)}}},
       state}
  end

  def handle_call({{:connection_init, trustedcerts, role,
             {cRLCb, userCRLDb}},
            pid},
           _From,
           r_state(certificate_db: [certDb, fileRefDb, pemCache |
  ...
  def handle_call({{:insert_crls, path, cRLs}, _}, _From,
           r_state(certificate_db: db) = state) do
    :ssl_pkix_db.add_crls(db, path, cRLs)
    {:reply, :ok, state}
  end

  def handle_call({{:delete_crls, cRLsOrPath}, _}, _From,
           r_state(certificate_db: db) = state) do
    :ssl_pkix_db.remove_crls(db, cRLsOrPath)
    {:reply, :ok, state}
  end

  def handle_call({{:register_session, host, port, session}, _}, _,
           state0) do
    state = client_register_session(host, port, session,
                                      state0)
    {:reply, :ok, state}
  end

  def handle_call({:refresh_trusted_db, _}, _,
           r_state(certificate_db: db) = state) do
    pemCache = :erlang.get(:ssl_pem_cache)
    :ssl_pkix_db.refresh_trusted_certs(db, pemCache)
    {:reply, :ok, state}
  end

  def handle_call({{:refresh_trusted_db, file}, _}, _,
           r_state(certificate_db: db) = state) do
    pemCache = :erlang.get(:ssl_pem_cache)
  ...
  def handle_cast({:register_session, host, port, session,
            :unique},
           state0) do
    state = client_register_unique_session(host, port,
                                             session, state0)
    {:noreply, state}
  end

  def handle_cast({:invalidate_session, host, port,
            r_session(session_id: iD) = session},
           r_state(session_cache_client: cache,
               session_cache_client_cb: cacheCb) = state) do
    invalidate_session(cache, cacheCb, {{host, port}, iD},
                         session, state)
  end

  def handle_cast({:insert_crls, path, cRLs},
           r_state(certificate_db: db) = state) do
    :ssl_pkix_db.add_crls(db, path, cRLs)
    {:noreply, state}
  end

  def handle_cast({:delete_crls, cRLsOrPath},
           r_state(certificate_db: db) = state) do
    :ssl_pkix_db.remove_crls(db, cRLsOrPath)
    {:noreply, state}
  end

  def handle_info(:validate_sessions,
           r_state(session_cache_client_cb: cacheCb,
               session_cache_client: clientCache,
               session_lifetime: lifeTime,
               session_client_invalidator: client) = state) do
    timer = :erlang.send_after(60000, self(),
                                 :validate_sessions)
    cPid = start_session_validator(clientCache, cacheCb,
                                     lifeTime, client)
    {:noreply,
       r_state(state, session_validation_timer: timer, 
                  session_client_invalidator: cPid)}
  end

  def handle_info({:clean_cert_db, ref, file},
           r_state(certificate_db: [certDb, {refDb, fileMapDb} |
                                          _]) = state) do
  ...
  def handle_info({:EXIT, pid, _},
           r_state(session_client_invalidator: pid) = state) do
    {:noreply,
       r_state(state, session_client_invalidator: :undefined)}
  end
  ...
  def terminate(_Reason,
           r_state(certificate_db: db,
               session_cache_client: clientSessionCache,
               session_cache_client_cb: cacheCb,
               session_validation_timer: timer)) do
  ...
  defp call(msg) do
    :gen_server.call(:erlang.get(:ssl_manager),
                       {msg, self()}, :infinity)
  end
  ...
  defp invalidate_session(cache, cacheCb, key, _Session,
            r_state(client_session_order: order) = state) do
    case (cacheCb.lookup(cache, key)) do
      :undefined ->
        {:noreply, state}
      r_session(internal_id: internalId) ->
        cacheCb.delete(cache, key)
        {:noreply,
           r_state(state, session_cache_client: cache, 
                      client_session_order: :gb_trees.delete(internalId,
                                                               order))}
    end
  end
  ...
  defp client_register_unique_session(host, port, session,
            r_state(session_cache_client: cache0,
                session_cache_client_cb: cacheCb,
                session_cache_client_max: max, options: options,
                client_session_order: order0) = state) do
    ...
    case (cacheCb.select_session(cache0, {host, port})) do
      :no_session ->
        {cache, order} = do_register_session({{host, port},
                                               ...
                                               newSession, max, cache0, cacheCb,
                                               options, order0)
        r_state(state, session_cache_client: cache, 
                   client_session_order: order)
      sessions ->
        register_unique_session(sessions, newSession,
                                  {host, port}, state)
    end
  end

  defp client_register_session(host, port, session,
            r_state(session_cache_client: cache0,
                session_cache_client_cb: cacheCb,
                session_cache_client_max: max, options: options,
                client_session_order: order0) = state) do
    timeStamp = :erlang.monotonic_time()
    newSession = r_session(session, time_stamp: timeStamp)
    sessionId = r_session(newSession, :session_id)
    {cache, order} = do_register_session({{host, port},
                                            sessionId},
                                           newSession, max, cache0, cacheCb,
                                           options, order0)
  ...
  defp do_register_session(key, r_session(time_stamp: timeStamp) = session0, max,
            cache, cacheCb, options, order0) do
    try do
      case (cacheCb.size(cache)) do
        ^max ->
          internalId = {timeStamp,
                          :erlang.unique_integer([:monotonic])}
          session = r_session(session0, internal_id: internalId)
      ...
      _, _ ->
        args = :proplists.get_value(:session_cb_init_args,
                                      options, [])
  ...
  defp register_unique_session(sessions, session, partialKey,
            r_state(session_cache_client_max: max,
                session_cache_client: cache0,
                session_cache_client_cb: cacheCb, options: options,
                client_session_order: order0) = state) do
  ...
  defp exists_equivalent(r_session(peer_certificate: peerCert,
              own_certificates: [ownCert | _],
              compression_method: compress, cipher_suite: cipherSuite,
              srp_username: sRP, ecc: eCC),
            [r_session(peer_certificate: peerCert,
                 own_certificates: [ownCert | _],
                 compression_method: compress, cipher_suite: cipherSuite,
                 srp_username: sRP, ecc: eCC) |
                 _]) do
