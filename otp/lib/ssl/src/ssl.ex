defmodule :m_ssl do
  ...
  def connect(socket, sslOptions0, timeout)
      when is_list(sslOptions0) and (is_integer(timeout) and timeout >= 0 or timeout == :infinity) do
    try do
      cbInfo = handle_option_cb_info(sslOptions0, :tls)
      transport = :erlang.element(1, cbInfo)
      {:ok, config} = handle_options(transport, socket,
                                       sslOptions0, :client, :undefined)
  ...
  def transport_accept(r_sslsocket(pid: {listenSocket,
                   r_config(connection_cb: connectionCb) = config}),
           timeout)
      when is_integer(timeout) and timeout >= 0 or timeout == :infinity do
  ...
  def handshake(r_sslsocket(fd: {_, _, _, trackers}) = socket, sslOpts,
           timeout)
      when is_integer(timeout) and timeout >= 0 or timeout == :infinity do
    try do
      tracker = :proplists.get_value(:option_tracker,
                                       trackers)
  ...
  def handshake(r_sslsocket(pid: [pid | _], fd: {_, _, _}) = socket,
           sslOpts, timeout)
      when is_integer(timeout) and timeout >= 0 or timeout == :infinity do
    try do
  ...
  def handshake(socket, sslOptions, timeout)
    ...
    try do
      ...
      {:ok,
         r_config(transport_info: ^cbInfo, ssl: sslOpts,
             emulated: emOpts)} = handle_options(transport, socket,
                                                   sslOptions, :server,
                                                   :undefined)
      :ok = :tls_socket.setopts(transport, socket,
                                  :tls_socket.internal_inet_values())
      {:ok, port} = :tls_socket.port(transport, socket)
      {:ok,
         sessionIdHandle} = :tls_socket.session_id_tracker(:ssl_unknown_listener,
                                                             sslOpts)
  ...
  def handshake_continue(socket, sSLOptions, timeout) do
    :ssl_gen_statem.handshake_continue(socket, sSLOptions,
                                         timeout)
  end
  ...
  def close(r_sslsocket(pid: {:dtls,
                   r_config(dtls_handler: {_, _})}) = dTLSListen) do
    :dtls_socket.close(dTLSListen)
  end

  def close(r_sslsocket(pid: {listenSocket,
                   r_config(transport_info: {transport, _, _, _, _})})) do
  ...
  def close(r_sslsocket(pid: [tLSPid | _]), {pid, timeout} = downGrade)
    ...
    case (:ssl_gen_statem.close(tLSPid,
                                  {:close, downGrade})) do
      :ok ->
        {:error, :closed}
      other ->
  ...
  def close(r_sslsocket(pid: {:dtls,
                   r_config(dtls_handler: {_, _})}) = dTLSListen,
           _) do
    :dtls_socket.close(dTLSListen)
  end

  def close(r_sslsocket(pid: {listenSocket,
                   r_config(transport_info: {transport, _, _, _, _})}),
           _) do
  ...
  def send(r_sslsocket(pid: [_, pid]), data) when is_pid(pid) do
    :tls_sender.send_data(pid,
                            :erlang.iolist_to_iovec(data))
  end

  def send(r_sslsocket(pid: {_, r_config(transport_info: {_, :udp, _, _})}),
           _) do
    {:error, :enotconn}
  end
  ...
  def send(r_sslsocket(pid: {listenSocket, r_config(transport_info: info)}),
           data) do
  ...
  def recv(r_sslsocket(pid: [pid | _]), length, timeout)
      when is_pid(pid) and (is_integer(length) and length >= 0) and (is_integer(timeout) and timeout >= 0 or timeout == :infinity) do
  ...
  def recv(r_sslsocket(pid: {listen, r_config(transport_info: info)}), _,
           _) do
    transport = :erlang.element(1, info)
  ...
  def controlling_process(r_sslsocket(pid: {listen,
                   r_config(transport_info: {transport, _, _, _, _})}),
           newOwner)
      when is_pid(newOwner) do
  ...
  def connection_information(r_sslsocket(pid: [pid | _])) when is_pid(pid) do
    case (:ssl_gen_statem.connection_information(pid,
                                                   false)) do
      {:ok, info} ->
        {:ok,
  ...
  def connection_information(r_sslsocket(pid: [pid | _]), items) when is_pid(pid) do
    case (:ssl_gen_statem.connection_information(pid,
                                                   include_security_info(items))) do
      {:ok, info} ->
        {:ok,
           for (item = {key, value}) <- info,
                 :lists.member(key, items), value !== :undefined do
             item
           end}
      error ->
        error
  ...
  def peername(r_sslsocket(pid: {listenSocket,
                   r_config(transport_info: {transport, _, _, _, _})})) do
  ...
  def cipher_suites(description, version) when version == :"tlsv1.3" or
                                      version == :"tlsv1.2" or version == :"tlsv1.1" or
                                      version == :tlsv1 do
    cipher_suites(description,
                    :tls_record.protocol_version_name(version))
  end

  def cipher_suites(description, version) when version == :"dtlsv1.2" or
                                      version == :dtlsv1 do
    cipher_suites(description,
  ...
  def cipher_suites(description, version, stringType)
      when version == :"tlsv1.3" or version == :"tlsv1.2" or version == :"tlsv1.1" or
             version == :tlsv1 do
    cipher_suites(description,
                    :tls_record.protocol_version_name(version), stringType)
  end

  def cipher_suites(description, version, stringType)
      when version == :"dtlsv1.2" or version == :dtlsv1 do
    cipher_suites(description,
                    :dtls_record.protocol_version_name(version), stringType)
  end
  ...
  def filter_cipher_suites(suites, filters0) do
    %{key_exchange_filters: kexF, cipher_filters: cipherF,
        mac_filters: macF,
        prf_filters:
        prfF} = :ssl_cipher.crypto_support_filters()
    filters = %{key_exchange_filters:
                add_filter(:proplists.get_value(:key_exchange,
                                                  filters0),
                             kexF),
                  cipher_filters:
                  add_filter(:proplists.get_value(:cipher, filters0),
                               cipherF),
                  mac_filters:
                  add_filter(:proplists.get_value(:mac, filters0), macF),
                  prf_filters:
                  add_filter(:proplists.get_value(:prf, filters0), prfF)}
  ...
  def signature_algs(:all, :"tlsv1.2") do
    :tls_v1.default_signature_algs([:tls_record.protocol_version_name(:"tlsv1.2")]) ++ :tls_v1.legacy_signature_algs_pre_13()
  end
  ...
  def signature_algs(description, version)
    ...
    :erlang.error({:signature_algs_not_supported_in_protocol_version,
                     version})
  end
  ...
  def eccs(version) when version == :"tlsv1.2" or version == :"tlsv1.1" or
                         version == :tlsv1 do
  ...
  def getopts(r_sslsocket(pid: {:dtls,
                   r_config(transport_info: {transport, _, _, _,
                                        _})}) = listenSocket,
           optionTags)
      when is_list(optionTags) do
    try do
      :dtls_socket.getopts(transport, listenSocket,
                             optionTags)
    catch
      _, error ->
        {:error,
           {:options, {:socket_options, optionTags, error}}}
    else
      {:ok, _} = result ->
        result
      {:error, inetError} ->
        {:error,
           {:options, {:socket_options, optionTags, inetError}}}
    end
  end

  def getopts(r_sslsocket(pid: {_,
                   r_config(transport_info: {transport, _, _, _,
                                        _})}) = listenSocket,
           optionTags)
      when is_list(optionTags) do
  ...
  def setopts(r_sslsocket(pid: [pid, sender]), options0)
    ...
    try do
      :proplists.expand([{:binary, [{:mode, :binary}]},
                             {:list, [{:mode, :list}]}],
                          options0)
    catch
      _, _ ->
      ...
      options ->
        case (:proplists.get_value(:packet, options,
                                     :undefined)) do
          :undefined ->
            :ssl_gen_statem.set_opts(pid, options)
          packetOpt ->
            case (:tls_sender.setopts(sender,
                                        [{:packet, packetOpt}])) do
              :ok ->
                :ssl_gen_statem.set_opts(pid, options)
              error ->
  ...
  def setopts(r_sslsocket(pid: {:dtls,
                   r_config(transport_info: {transport, _, _, _,
                                        _})}) = listenSocket,
           options)
    ...
    try do
      ...
      {:error, inetError} ->
        {:error,
           {:options, {:socket_options, options, inetError}}}
    end
  end

  def setopts(r_sslsocket(pid: {_,
                   r_config(transport_info: {transport, _, _, _,
                                        _})}) = listenSocket,
           options)
      when is_list(options) do
  ...
  def getstat(r_sslsocket(pid: {:dtls,
                   r_config(transport_info: info, dtls_handler: {listener, _})}),
           options)
      when is_list(options) do
    transport = :erlang.element(1, info)
  ...
  def getstat(r_sslsocket(pid: {listen, r_config(transport_info: info)}),
           options)
      when is_list(options) do
    transport = :erlang.element(1, info)
  ...
  def getstat(r_sslsocket(pid: [pid | _], fd: {transport, socket, _, _}),
           options)
      when (is_pid(pid) and is_list(options)) do
    :tls_socket.getstat(transport, socket, options)
  end

  def getstat(r_sslsocket(pid: [pid | _], fd: {transport, socket, _}),
           options)
      when (is_pid(pid) and is_list(options)) do
  ...
  def shutdown(r_sslsocket(pid: {listen, r_config(transport_info: info)}),
           how) do
    transport = :erlang.element(1, info)
  ...
  def renegotiate(r_sslsocket(pid: [pid, sender | _]) = socket)
    ...
    case (:ssl.connection_information(socket,
                                        [:protocol])) do
      {:ok, [{:protocol, :"tlsv1.3"}]} ->
        {:error, :notsup}
      _ ->
  ...
  def export_key_materials(r_sslsocket(pid: [pid | _]), labels, contexts,
           wantedLengths)
      when is_pid(pid) do
    :ssl_gen_statem.call(pid,
                           {:export_key_materials, labels, contexts,
                              wantedLengths, true})
  end
  ...
  def prf(r_sslsocket(pid: [pid | _]) = socket, :master_secret,
           label, [:client_random, :server_random], wantedLength)
      when is_pid(pid) do
    case (export_key_materials(socket, [label],
                                 [:no_context], [wantedLength], true)) do
      {:ok, [keyMaterial]} ->
        {:ok, keyMaterial}
      error ->
        error
  ...
  def prf(r_sslsocket(pid: [pid | _]) = socket, :master_secret,
           label, [:client_random, :server_random, context],
           wantedLength)
      when (is_pid(pid) and is_binary(context)) do
    case (export_key_materials(socket, [label], [context],
                                 [wantedLength], true)) do
      {:ok, [keyMaterial]} ->
        {:ok, keyMaterial}
      error ->
        error
  ...
  def prf(socket, secret, label, context, wantedLength) do
    {:ok,
       [{:selected_cipher_suite,
           %{prf: pRFAlg}}]} = connection_information(socket,
                                                        [:selected_cipher_suite])
    {:ok,
       :tls_v1.prf(pRFAlg, secret, label,
                     :erlang.iolist_to_binary(context), wantedLength)}
  end
  ...
  def tls_version(version) when :erlang.element(1,
                                         version) == 254 do
  ...
  defp supported_suites(:exclusive, version) when :erlang.element(1,
                                                      version) == 3 do
  ...
  def update_options(opts, role, inheritedSslOpts)
    ...
    process_options(userSslOpts, inheritedSslOpts,
                      %{role: role})
  end

  defp process_options(userSslOpts, sslOpts0, env) do
    userSslOptsMap = :proplists.to_map(:lists.reverse(userSslOpts))
    sslOpts1 = opt_protocol_versions(userSslOptsMap,
                                       sslOpts0, env)
    sslOpts2 = opt_verification(userSslOptsMap, sslOpts1,
                                  env)
    ...
    sslOpts7 = opt_signature_algs(userSslOptsMap, sslOpts6,
                                    env)
    ...
    sslOpts12 = opt_renegotiate(userSslOptsMap, sslOpts11,
                                  env)
    sslOpts13 = opt_reuse_sessions(userSslOptsMap,
                                     sslOpts12, env)
    sslOpts14 = opt_identity(userSslOptsMap, sslOpts13, env)
    sslOpts15 = opt_supported_groups(userSslOptsMap,
                                       sslOpts14, env)
    sslOpts16 = opt_crl(userSslOptsMap, sslOpts15, env)
    sslOpts17 = opt_handshake(userSslOptsMap, sslOpts16,
                                env)
  ...
  defp handle_options(transport, socket, opts0, role, host) do
    {userSslOptsList, sockOpts0} = split_options(opts0,
                                                   ssl_options())
    env = %{role: role, host: host}
    sslOpts = process_options(userSslOptsList, %{}, env)
    %{protocol: protocol} = sslOpts
    {sock, emulated} = emulated_options(transport, socket,
                                          protocol, sockOpts0)
    connetionCb = connection_cb(protocol)
    cbInfo = handle_option_cb_info(opts0, protocol)
    {:ok,
       r_config(ssl: sslOpts, emulated: emulated, inet_ssl: sock,
           inet_user: sock, transport_info: cbInfo,
           connection_cb: connetionCb)}
  end

  defp opt_protocol_versions(userOpts, opts, env) do
    {_, pRC} = get_opt_of(:protocol, [:tls, :dtls], :tls,
                            userOpts, opts)
    logLevels = [:none, :all, :emergency, :alert, :critical,
                                                      :error, :warning, :notice,
                                                                            :info,
                                                                                :debug]
    defaultLevel = (case (:logger.get_module_level(:ssl)) do
                      [] ->
                        :notice
                      [{:ssl, level}] ->
                        level
                    end)
    {_, lL} = get_opt_of(:log_level, logLevels,
                           defaultLevel, userOpts, opts)
    opts1 = set_opt_bool(:keep_secrets, false, userOpts,
                           opts)
    {distW, dist} = get_opt_bool(:erl_dist, false, userOpts,
                                   opts1)
    option_incompatible(pRC === :dtls and dist,
                          [{:protocol, pRC}, {:erl_dist, dist}])
    opts2 = set_opt_new(distW, :erl_dist, false, dist,
                          opts1)
    {ktlsW, ktls} = get_opt_bool(:ktls, false, userOpts,
                                   opts1)
    option_incompatible(pRC === :dtls and ktls,
                          [{:protocol, pRC}, {:ktls, ktls}])
    opts3 = set_opt_new(ktlsW, :ktls, false, ktls, opts2)
    opt_versions(userOpts,
                   Map.merge(opts3, %{protocol: pRC, log_level: lL}), env)
  end

  defp opt_versions(userOpts, %{protocol: protocol} = opts, _Env) do
    versions = (case (get_opt(:versions, :unbound, userOpts,
                                opts)) do
                  {:default, :unbound} ->
    ...
    {where, mCM} = get_opt_bool(:middlebox_comp_mode, true,
                                  userOpts, opts)
    assert_version_dep(where === :new, :middlebox_comp_mode,
                         versions, [:"tlsv1.3"])
    opts1 = set_opt_new(where, :middlebox_comp_mode, true,
                          mCM, opts)
    ...
    validate = fn version ->
                    try do
                      ...
                      true ->
                        :tls_record.protocol_version_name(version)
                      false ->
                        option_error(:insufficient_crypto_support,
  ...
  defp validate_versions(:dtls, vsns0) do
    validate = fn version ->
                    try do
                      :tls_record.sufficient_crypto_support(:dtls_v1.corresponding_tls_version(:dtls_record.protocol_version_name(version)))
                    catch
                      :error, :function_clause ->
                        option_error(version, {:versions, vsns0})
                    else
  ...
  defp opt_verification(userOpts, opts0, %{role: role} = env) do
    {verify, opts1} = (case (get_opt_of(:verify,
                                          [:verify_none, :verify_peer],
                                          default_verify(role), userOpts,
                                          opts0)) do
                         {:old, val} ->
                           {val, opts0}
                         {_, :verify_none} ->
                           {:verify_none,
                              Map.merge(opts0, %{verify: :verify_none,
                                                   verify_fun:
                                                   {none_verify_fun(), []}})}
                         {_, :verify_peer} ->
                           temp = Map.merge(opts0, %{verify: :verify_peer,
                                                       verify_fun: :undefined})
                           {:verify_peer,
                              :maps.remove(:fail_if_no_peer_cert, temp)}
                       end)
    opts2 = opt_cacerts(userOpts, opts1, env)
    {_, partialChain} = get_opt_fun(:partial_chain, 1,
                                      fn _ ->
                                           :unknown_ca
                                      end,
                                      userOpts, opts2)
    ...
    {_,
       failNoPeerCert} = get_opt_bool(:fail_if_no_peer_cert,
                                        defFailNoPeer, userOpts, opts2)
    assert_server_only(role, failNoPeerCert,
                         :fail_if_no_peer_cert)
    option_incompatible(failNoPeerCert and verify === :verify_none,
                          [{:verify, :verify_none}, {:fail_if_no_peer_cert,
                                                       true}])
    opts = set_opt_int(:depth, 0, 255, 10, userOpts, opts2)
    case (role) do
      :client ->
        opt_verify_fun(userOpts,
                         Map.put(opts, :partial_chain, partialChain), env)
      :server ->
        opt_verify_fun(userOpts,
                         Map.merge(opts, %{partial_chain: partialChain,
                                             fail_if_no_peer_cert:
                                             failNoPeerCert}),
                         env)
    end
  ...
  defp opt_verify_fun(userOpts, opts, _Env) do
    verifyFun = (case (get_opt(:verify_fun, :undefined,
                                 userOpts, opts)) do
                   {_, {f, _} = fA} when is_function(f, 3) or
                                           is_function(f, 4)
                                         ->
                     fA
  ...
  defp opt_certs(userOpts, %{log_level: logLevel} = opts0,
            env) do
  ...
  defp opt_old_certs(userOpts, certKeys,
            %{log_level: logLevel} = sSLOpts, _Env) do
  ...
  defp check_cert_key(userOpts, certKeys, logLevel) do
    certKeys0 = (case (get_opt(:cert, :undefined, userOpts,
                                 certKeys)) do
                   {where, cert} when is_binary(cert) ->
                     warn_override(where, userOpts, :cert, [:certfile],
                                     logLevel)
                   ...
                   {_, :undefined} ->
                     case (get_opt_file(:certfile, :unbound, userOpts,
                                          certKeys)) do
                       {:default, :unbound} ->
                         certKeys
                       {_, certFile} ->
    ...
    certKeys1 = (case (get_opt(:key, :undefined, userOpts,
                                 certKeys)) do
                   {_, :undefined} ->
                     case (get_opt_file(:keyfile, <<>>, userOpts,
                                          certKeys)) do
                       {:new, keyFile} ->
                   ...
                   {_, {kF, k0} = key} when (is_binary(k0) and
                                               kF === :rsa) or
                                              kF === :dsa or
                                              kF == :RSAPrivateKey or
                                              kF == :DSAPrivateKey or
    ...
    certKeys2 = (case (get_opt(:password, :unbound,
                                 userOpts, certKeys)) do
                   {:default, _} ->
                     certKeys1
                   {_, pwd} when is_binary(pwd) or is_list(pwd) ->
                     Map.put(certKeys1, :password,
                                          fn () ->
                                               pwd
                                          end)
  ...
  defp opt_cacerts(userOpts,
            %{verify: verify, log_level: logLevel,
                versions: versions} = opts,
            %{role: role}) do
    {_, caCerts} = get_opt_list(:cacerts, :undefined,
                                  userOpts, opts)
    caCertFile = (case (get_opt_file(:cacertfile, <<>>,
                                       userOpts, opts)) do
                    {where1, _FileName} when caCerts !== :undefined ->
                      warn_override(where1, userOpts, :cacerts, [:cacertfile],
                                      logLevel)
                      <<>>
                    {:new, fileName} ->
    ...
    option_incompatible(caCertFile === <<>> and caCerts === :undefined and verify === :verify_peer,
                          [{:verify, :verify_peer}, {:cacerts, :undefined}])
    {where2, cA} = get_opt_bool(:certificate_authorities,
                                  role === :server, userOpts, opts)
    assert_version_dep(where2 === :new,
                         :certificate_authorities, versions, [:"tlsv1.3"])
    opts1 = set_opt_new(:new, :cacertfile, <<>>, caCertFile,
                          opts)
    opts2 = set_opt_new(where2, :certificate_authorities,
                          role === :server, cA, opts1)
    Map.put(opts2, :cacerts, caCerts)
  end

  defp opt_tickets(userOpts, %{versions: versions} = opts,
            %{role: :client}) do
    {_, sessionTickets} = get_opt_of(:session_tickets,
                                       [:disabled, :manual, :auto], :disabled,
                                       userOpts, opts)
    assert_version_dep(sessionTickets !== :disabled,
                         :session_tickets, versions, [:"tlsv1.3"])
    {_, useTicket} = get_opt_list(:use_ticket, :undefined,
                                    userOpts, opts)
    option_error(useTicket === [], :use_ticket, useTicket)
    option_incompatible(useTicket !== :undefined and sessionTickets !== :manual,
                          [{:use_ticket, useTicket}, {:session_tickets,
                                                        sessionTickets}])
    {_, earlyData} = get_opt_bin(:early_data, :undefined,
                                   userOpts, opts)
    option_incompatible(is_binary(earlyData) and sessionTickets === :disabled,
                          [:early_data, {:session_tickets, :disabled}])
    option_incompatible(is_binary(earlyData) and sessionTickets === :manual and useTicket === :undefined,
                          [:early_data, {:session_tickets, :manual},
                                            {:use_ticket, :undefined}])
    assert_server_only(:anti_replay, userOpts)
    assert_server_only(:stateless_tickets_seed, userOpts)
    Map.merge(opts, %{session_tickets: sessionTickets,
                        use_ticket: useTicket, early_data: earlyData})
  end
  ...
  defp opt_tickets(userOpts, %{versions: versions} = opts,
    ...
    {_, sessionTickets} = get_opt_of(:session_tickets,
                                       [:disabled, :stateful, :stateless,
                                                                  :stateful_with_cert,
                                                                      :stateless_with_cert],
                                       :disabled, userOpts, opts)
    assert_version_dep(sessionTickets !== :disabled,
                         :session_tickets, versions, [:"tlsv1.3"])
    {_, earlyData} = get_opt_of(:early_data,
                                  [:enabled, :disabled], :disabled, userOpts,
                                  opts)
    option_incompatible(sessionTickets === :disabled and earlyData === :enabled,
                          [:early_data, {:session_tickets, :disabled}])
    stateless = :lists.member(sessionTickets,
                                [:stateless, :stateless_with_cert])
    antiReplay = (case (get_opt(:anti_replay, :undefined,
                                  userOpts, opts)) do
                    {_, :undefined} ->
                      :undefined
                    {_, aR} when not stateless ->
                      option_incompatible([{:anti_replay, aR},
                                               {:session_tickets,
                                                  sessionTickets}])
                    {_, :"10k"} ->
                      {10, 5, 72985}
    ...
    {_, sTS} = get_opt_bin(:stateless_tickets_seed,
                             :undefined, userOpts, opts)
    option_incompatible(sTS !== :undefined and not
                                               stateless,
                          [:stateless_tickets_seed, {:session_tickets,
                                                       sessionTickets}])
    assert_client_only(:use_ticket, userOpts)
    Map.merge(opts, %{session_tickets: sessionTickets,
                        early_data: earlyData, anti_replay: antiReplay,
                        stateless_tickets_seed: sTS})
  end

  defp opt_ocsp(userOpts, %{versions: _Versions} = opts,
            %{role: role}) do
    {stapling, sMap} = (case (get_opt(:ocsp_stapling, false,
                                        userOpts, opts)) do
    ...
    {_, nonce} = get_opt_bool(:ocsp_nonce, true, userOpts,
                                sMap)
    option_incompatible(stapling === false and nonce === false,
                          [{:ocsp_nonce, false}, {:ocsp_stapling, false}])
    {_, oRC} = get_opt_list(:ocsp_responder_certs, [],
                              userOpts, sMap)
    checkBinary = fn cert when is_binary(cert) ->
                       :ok
                     _Cert ->
    ...
    option_incompatible(stapling === false and oRC !== [],
                          [:ocsp_responder_certs, {:ocsp_stapling, false}])
    case (stapling) do
      true ->
        Map.put(opts, :ocsp_stapling,
                        %{ocsp_nonce: nonce, ocsp_responder_certs: oRC})
      false ->
        opts
    end
  end

  defp opt_sni(userOpts, %{versions: _Versions} = opts,
            %{role: :server}) do
    {_, sniHosts} = get_opt_list(:sni_hosts, [], userOpts,
                                   opts)
    check = fn {[_ | _], sO} when is_list(sO) ->
                 case (:proplists.get_value(:sni_hosts, sO,
                                              :undefined)) do
                   :undefined ->
                     :ok
                   recursive ->
    ...
    {where, sniFun0} = get_opt_fun(:sni_fun, 1, :undefined,
                                     userOpts, opts)
    option_incompatible(is_function(sniFun0) and sniHosts !== [] and where === :new,
                          [:sni_fun, :sni_hosts])
    assert_client_only(:server_name_indication, userOpts)
    sniFun = (case (sniFun0 === :undefined) do
                true ->
                  fn host ->
  ...
  defp opt_sni(userOpts, %{versions: _Versions} = opts,
            %{role: :client} = env) do
    sNI = (case (get_opt(:server_name_indication, :unbound,
                           userOpts, opts)) do
             {_, :unbound} ->
               server_name_indication_default(:maps.get(:host, env,
                                                          :undefined))
             {_, [_ | _] = sN} ->
               sN
             {_, :disable} ->
  ...
  defp opt_signature_algs(userOpts, %{versions: versions} = opts, _Env) do
    [tlsVersion | _] = (tlsVsns = (for v <- versions do
                                     tls_version(v)
                                   end))
    sA = (case (get_opt_list(:signature_algs, :undefined,
                               userOpts, opts)) do
            ...
            {:new, algs} ->
              ...
              option_error(sA0 === [], :no_supported_algorithms,
                             {:signature_algs, algs})
              sA0
            {_, algs} ->
              algs
          end)
    sAC = (case (get_opt_list(:signature_algs_cert,
                                :undefined, userOpts, opts)) do
             {:new, schemes} ->
               assert_version_dep(:signature_algs_cert, versions,
                                    [:"tlsv1.2", :"tlsv1.3"])
               sAC0 = handle_signature_algorithms_option(schemes,
                                                           tlsVersion)
               option_error(sAC0 === [],
    ...
    Map.merge(opts, %{signature_algs: sA,
                        signature_algs_cert: sAC})
  end

  defp opt_alpn(userOpts, %{versions: versions} = opts,
            %{role: :server}) do
    {_, aPP} = get_opt_list(:alpn_preferred_protocols,
                              :undefined, userOpts, opts)
    validate_protocols(is_list(aPP),
                         :alpn_preferred_protocols, aPP)
    {where, nPA} = get_opt_list(:next_protocols_advertised,
                                  :undefined, userOpts, opts)
    validate_protocols(is_list(nPA),
                         :next_protocols_advertised, nPA)
    assert_version_dep(is_list(nPA),
                         :next_protocols_advertised, versions, [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
    assert_client_only(:alpn_advertised_protocols, userOpts)
    assert_client_only(:client_preferred_next_protocols,
                         userOpts)
    opts1 = set_opt_new(where, :next_protocols_advertised,
                          :undefined, nPA, opts)
  ...
  defp opt_alpn(userOpts, %{versions: versions} = opts,
            %{role: :client}) do
    {_, aAP} = get_opt_list(:alpn_advertised_protocols,
                              :undefined, userOpts, opts)
    validate_protocols(is_list(aAP),
                         :alpn_advertised_protocols, aAP)
    {where,
       nPS} = (case (get_opt(:client_preferred_next_protocols,
                               :undefined, userOpts, opts)) do
                 {:new, cPNP} ->
    ...
    validate_protocols(is_list(nPS),
                         :client_preferred_next_protocols, nPS)
    assert_server_only(:alpn_preferred_protocols, userOpts)
    assert_server_only(:next_protocols_advertised, userOpts)
    opts1 = set_opt_new(where, :next_protocol_selector,
                          :undefined, nPS, opts)
  ...
  defp opt_mitigation(userOpts, %{versions: versions} = opts, _Env) do
    defBeast = (case (:lists.last(versions) > {3, 1}) do
                  true ->
                    :disabled
                  false ->
                    :one_n_minus_one
                end)
    {where1, bM} = get_opt_of(:beast_mitigation,
                                [:disabled, :one_n_minus_one, :zero_n],
                                defBeast, userOpts, opts)
    assert_version_dep(where1 === :new, :beast_mitigation,
                         versions, [:tlsv1])
    {where2, pC} = get_opt_bool(:padding_check, true,
                                  userOpts, opts)
    assert_version_dep(where2 === :new, :padding_check,
                         versions, [:tlsv1])
    opts1 = set_opt_new(:new, :beast_mitigation, :disabled,
                          bM, opts)
    set_opt_new(where2, :padding_check, true, pC, opts1)
  end

  defp opt_server(userOpts,
            %{versions: versions, log_level: logLevel} = opts,
            %{role: :server}) do
    {_, eCC} = get_opt_bool(:honor_ecc_order, false,
                              userOpts, opts)
    {_, cipher} = get_opt_bool(:honor_cipher_order, false,
                                 userOpts, opts)
    {where1, cookie} = get_opt_bool(:cookie, true, userOpts,
                                      opts)
    assert_version_dep(where1 === :new, :cookie, versions,
                         [:"tlsv1.3"])
    {where2, reNeg} = get_opt_bool(:client_renegotiation,
                                     true, userOpts, opts)
    assert_version_dep(where2 === :new,
                         :client_renegotiation, versions, [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
    opts1 = (case (get_opt(:dh, :undefined, userOpts,
                             opts)) do
               ...
               {:new, dH} ->
                 option_error(:dh, dH)
               {_, :undefined} ->
                 case (get_opt_file(:dhfile, :unbound, userOpts,
                                      opts)) do
                   {:default, :unbound} ->
                     opts
                   {_, dHFile} ->
    ...
    Map.merge(opts1, %{honor_ecc_order: eCC,
                         honor_cipher_order: cipher, cookie: cookie,
  ...
  defp opt_client(userOpts, %{versions: versions} = opts,
            %{role: :client}) do
    {where, fB} = get_opt_bool(:fallback, false, userOpts,
                                 opts)
    assert_version_dep(where === :new, :fallback, versions,
                         [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
    {_, cHC} = get_opt_list(:customize_hostname_check, [],
                              userOpts, opts)
    validMFL = [:undefined, 512, 1024, 2048, 4096]
    {_, mFL} = get_opt_of(:max_fragment_length, validMFL,
                            :undefined, userOpts, opts)
    Map.merge(opts, %{fallback: fB,
  ...
  defp opt_renegotiate(userOpts, %{versions: versions} = opts, _Env) do
    {where1, kUA} = get_opt_pos_int(:key_update_at,
                                      388736063997, userOpts, opts)
    assert_version_dep(where1 === :new, :key_update_at,
                         versions, [:"tlsv1.3"])
    {_, rA0} = get_opt_pos_int(:renegotiate_at, 268435456,
                                 userOpts, opts)
    rA = min(rA0, 268435456)
    {where3, sR} = get_opt_bool(:secure_renegotiate, true,
                                  userOpts, opts)
    assert_version_dep(where3 === :new, :secure_renegotiate,
                         versions, [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
    Map.merge(opts, %{secure_renegotiate: sR,
                        key_update_at: kUA, renegotiate_at: rA})
  end

  defp opt_reuse_sessions(userOpts, %{versions: versions} = opts,
            %{role: :client}) do
    {where1, rUSS} = get_opt_of(:reuse_sessions,
                                  [true, false, :save], true, userOpts, opts)
    {where2, rS} = (rST = get_opt(:reuse_session,
                                    :undefined, userOpts, opts))
    case (rST) do
    ...
    assert_version_dep(where1 === :new, :reuse_sessions,
                         versions, [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
    assert_version_dep(where2 === :new, :reuse_session,
                         versions, [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
    Map.merge(opts, %{reuse_sessions: rUSS,
                        reuse_session: rS})
  end

  defp opt_reuse_sessions(userOpts, %{versions: versions} = opts,
            %{role: :server}) do
    {where1, rUSS} = get_opt_bool(:reuse_sessions, true,
                                    userOpts, opts)
    defRS = fn _, _, _, _ ->
                 true
            end
    {where2, rS} = get_opt_fun(:reuse_session, 4, defRS,
                                 userOpts, opts)
    assert_version_dep(where1 === :new, :reuse_sessions,
                         versions, [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
    assert_version_dep(where2 === :new, :reuse_session,
  ...
  defp opt_identity(userOpts, %{versions: versions} = opts, _Env) do
    pSK = (case (get_opt_list(:psk_identity, :undefined,
                                userOpts, opts)) do
             {:new, pSK0} ->
               pSK1 = :unicode.characters_to_binary(pSK0)
               pSKSize = byte_size(pSK1)
               assert_version_dep(:psk_identity, versions,
                                    [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
               option_error(not (0 < pSKSize and pSKSize < 65536),
                              :psk_identity, {:psk_identity, pSK0})
               pSK1
             {_, pSK0} ->
               pSK0
           end)
    sRP = (case (get_opt(:srp_identity, :undefined,
                           userOpts, opts)) do
             {:new, {s1, s2}} when (is_list(s1) and is_list(s2)) ->
               user = :unicode.characters_to_binary(s1)
               userSize = byte_size(user)
               assert_version_dep(:srp_identity, versions,
                                    [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
               option_error(not (0 < userSize and userSize < 65536),
    ...
    uLF = (case (get_opt(:user_lookup_fun, :undefined,
                           userOpts, opts)) do
             {:new, {fun, _} = uLF0} when is_function(fun, 3) ->
               assert_version_dep(:user_lookup_fun, versions,
                                    [:tlsv1, :"tlsv1.1", :"tlsv1.2"])
               uLF0
             {:new, uLF0} ->
    ...
    Map.merge(opts, %{psk_identity: pSK, srp_identity: sRP,
                        user_lookup_fun: uLF})
  end

  defp opt_supported_groups(userOpts, %{versions: tlsVsns} = opts, _Env) do
    sG = (case (get_opt_list(:supported_groups, :undefined,
                               userOpts, opts)) do
            {:default, :undefined} ->
    ...
    cPHS = (case (get_opt_list(:ciphers, [], userOpts,
                                 opts)) do
              {:old, cPS0} ->
                cPS0
              {_, cPS0} ->
    ...
    eCCS = (try do
              ...
              _ ->
                case (get_opt_list(:eccs, :undefined, userOpts,
                                     opts)) do
                  {:old, eCCS0} ->
                    eCCS0
                  {:default, _} ->
    ...
    Map.merge(opts, %{ciphers: cPHS, eccs: eCCS,
                        supported_groups: sG})
  end

  defp opt_crl(userOpts, opts, _Env) do
    {_, check} = get_opt_of(:crl_check,
                              [:best_effort, :peer, true, false], false,
                              userOpts, opts)
    cache = (case (get_opt(:crl_cache,
                             {:ssl_crl_cache, {:internal, []}}, userOpts,
                             opts)) do
  ...
  defp opt_handshake(userOpts, opts, _Env) do
    {_, hS} = get_opt_of(:handshake, [:hello, :full], :full,
                           userOpts, opts)
    {_, mHSS} = get_opt_int(:max_handshake_size, 1, 8388607,
                              div(256 * 1024, 2), userOpts, opts)
    Map.merge(opts, %{handshake: hS,
                        max_handshake_size: mHSS})
  end

  defp opt_use_srtp(userOpts, %{protocol: protocol} = opts, _Env) do
    useSRTP = (case (get_opt_map(:use_srtp, :undefined,
                                   userOpts, opts)) do
                 {:old, useSRTP0} ->
                   useSRTP0
                 {:default, :undefined} ->
  ...
  defp validate_use_srtp(%{protection_profiles:
            [_ | _] = pPs} = useSRTP) do
    case (:maps.keys(useSRTP) -- [:protection_profiles,
                                      :mki]) do
      [] ->
        :ok
      extra ->
  ...
  defp opt_process(userOpts, opts0, _Env) do
    opts1 = set_opt_list(:receiver_spawn_opts, [], userOpts,
                           opts0)
    opts2 = set_opt_list(:sender_spawn_opts, [], userOpts,
                           opts1)
    set_opt_int(:hibernate_after, 0, :infinity, :infinity,
                  userOpts, opts2)
  end
  ...
  defp get_opt(opt, default, userOpts, opts) do
    case (:maps.get(opt, userOpts, :unbound)) do
      :unbound ->
        case (:maps.get(maybe_map_key_internal(opt), opts,
                          :unbound)) do
          :unbound ->
            {:default, default}
          value ->
  ...
  defp get_opt_int(opt, min, max, default, userOpts, opts) do
    case (get_opt(opt, default, userOpts, opts)) do
      {_, value} = res when (is_integer(value) and
                               min <= value and value <= max)
                            ->
        res
      {_, value} = res when (value === :infinity and
                               max === :infinity)
                            ->
        res
      {_, value} ->
  ...
  defp set_opt_int(opt, min, max, default, userOpts, opts) do
    case (:maps.get(opt, userOpts, default)) do
      ^default ->
        opts
      value when (is_integer(value) and min <= value and
                    value <= max)
                 ->
        Map.put(opts, opt, value)
  ...
  defp handle_cb_info({v1, v2, v3, v4}) do
    {v1, v2, v3, v4,
       :erlang.list_to_atom(:erlang.atom_to_list(v2) ++ '_passive')}
  end
  ...
  defp handle_option_cb_info(options, protocol) do
    cbInfo = :proplists.get_value(:cb_info, options,
                                    default_cb_info(protocol))
  ...
  defp split_options(opts0, allOptions) do
    ...
    allOpts = [:cb_info,
                   :client_preferred_next_protocols] ++ allOptions
  ...
  defp assert_version_dep(true, option, sSLVsns, allowedVsn) do
    case (is_dtls_configured(sSLVsns)) do
      true ->
        true
      false ->
        aPIVsns = :lists.map(&:tls_record.protocol_version/1,
                               sSLVsns)
  ...
  defp warn_override(:new, userOpts, newOpt, oldOpts, logLevel) do
    ...
    case (:lists.filter(check, oldOpts)) do
      ...
      ignored ->
        ...
        :ssl_logger.log(:notice, logLevel,
                          %{description: desc, reason: reas},
                          %{mfa: {:ssl, :warn_override, 5}, line: 2651,
                              file: 'otp/lib/ssl/src/ssl.erl'})
    end
  end
  ...
  defp validate_filename(fN, _Option) when (is_binary(fN) and
                               fN !== <<>>) do
    fN
  end
  ...
  defp emulated_options(transport, socket, protocol, opts) do
    ...
    {:ok, original} = :tls_socket.getopts(transport, socket,
                                            emulatedOptions)
    {inet, emulated0} = emulated_options(:undefined,
                                           :undefined, protocol, opts)
  ...
  defp binary_cipher_suites(versions, ciphers0) do
    ciphers = (for c <- :string.lexemes(ciphers0, ':') do
                 :ssl_cipher_format.suite_openssl_str_to_map(c)
               end)
    binary_cipher_suites(versions, ciphers)
  end
  ...
  defp all_suites([{3, 4}, version1 | _]) do
    all_suites([{3,
                   4}]) ++ :ssl_cipher.all_suites(version1) ++ :ssl_cipher.anonymous_suites(version1)
  end
  ...
  defp tuple_to_map({kex, cipher, mac}) do
    %{key_exchange: kex, cipher: cipher, mac: mac,
        prf: :default_prf}
  end

  defp tuple_to_map({kex, cipher, mac, prf}) do
    %{key_exchange: kex, cipher: cipher,
        mac: tuple_to_map_mac(cipher, mac), prf: prf}
  end
  ...
  defp handle_supported_groups_option(value) when is_list(value) do
    try do
      ...
      groups ->
        option_error(groups === [], :supported_groups,
                       :none_valid)
  ...
  defp do_format_error({:options, {:socket_options, option, error}}) do
    :lists.flatten(:io_lib.format('Invalid transport socket option ~p: ~s',
  ...
  defp do_format_error({:options,
             {:option_not_a_key_value_tuple = r, opts}}) do
    :lists.flatten(:io_lib.format('Invalid option ~w ~w', [opts, r]))
  end

  defp do_format_error({:options,
             {:no_supported_algorithms = r, opts}}) do
    :lists.flatten(:io_lib.format('Invalid option ~w ~w', [opts, r]))
  end

  defp do_format_error({:options,
             {:no_supported_signature_schemes = r, opts}}) do
    :lists.flatten(:io_lib.format('Invalid option ~w ~w', [opts, r]))
  end

  defp do_format_error({:options,
             {:insufficient_crypto_support = r, opts}}) do
    :lists.flatten(:io_lib.format('Invalid option ~w ~w', [opts, r]))
  end
  ...
  defp make_next_protocol_selector({precedence, prefProtcol} = v) do
    option_error(not is_list(prefProtcol),
  ...
  defp make_next_protocol_selector({precedence, allProtocols, defP} = v) do
    option_error(not is_list(allProtocols),
                   :client_preferred_next_protocols, v)
    option_error(not
                 (is_binary(defP) and byte_size(defP) < 256),
                   :client_preferred_next_protocols, v)
    validate_protocols(true,
                         :client_preferred_next_protocols, allProtocols)
    case (precedence) do
      :client ->
        fn advertised ->
             search = fn p ->
      ...
      value ->
        option_error(:client_preferred_next_protocols,
  ...
  defp connection_cb(opts) do
    connection_cb(:proplists.get_value(:protocol, opts,
                                         :tls))
  end
  ...
  defp handle_option_format([{:log_alert, bool} | rest], acc)
    ...
    case (:proplists.get_value(:log_level, acc ++ rest,
                                 :undefined)) do
      :undefined ->
        handle_option_format(rest,
  ...
  defp unambiguous_path(value) do
    ...
    uP = (case (:file.read_link(absName)) do
            {:ok, pathWithNoLink} ->
  ...
  defp format_ocsp_params(map) do
    ...
    :io_lib.format('Stapling = ~W Nonce = ~W Certs = ~W', [stapling, 5, nonce, 5, certs, 5])
  end
