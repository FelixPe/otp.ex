defmodule :m_ssl_crl_cache do
  ...
  def lookup(r_DistributionPoint(distributionPoint: {:fullName, names}),
           _Issuer, cRLDbInfo) do
  ...
  def select(genNames, cRLDbHandle) when is_list(genNames) do
    :lists.flatmap(fn {:directoryName, issuer} ->
  ...
  def insert(cRLSrc) do
    insert('http://dummy/no_distribution_point', cRLSrc)
  end

  def insert(distPointURI, {:file, file})
      when is_list(distPointURI) do
    case (:file.read_file(file)) do
      {:ok, pemBin} ->
        pemEntries = :public_key.pem_decode(pemBin)
        cRLs = (for {:CertificateList, cRL,
  ...
  def insert(distPointURI, {:der, cRLs}) do
    do_insert(distPointURI, cRLs)
  end
  ...
  def delete(uRI) do
    case (:uri_string.normalize(uRI, [:return_map])) do
      ...
      _ ->
        {:error,
           {:only_http_distribution_points_supported, uRI}}
    end
  end
  ...
  defp do_insert(uRI, cRLs) do
    case (:uri_string.normalize(uRI, [:return_map])) do
      %{scheme: 'http', path: path} ->
        :ssl_manager.insert_crls(:string.trim(path, :leading,
                                                '/'),
                                   cRLs)
      _ ->
        {:error,
  ...
  defp get_crls([{:uniformResourceIdentifier, 'http' ++ _ = uRL} |
               rest],
            cRLDbInfo) do
  ...
  defp http_get(uRL, rest, cRLDbInfo, timeout) do
    case (:httpc.request(:get, {uRL, [{'connection', 'close'}]},
                           [{:timeout, timeout}], [{:body_format, :binary}])) do
      {:ok, {_Status, _Headers, body}} ->
        case (body) do
          <<"-----BEGIN", _ :: binary>> ->
            pem = :public_key.pem_decode(body)
            :lists.filtermap(fn {:CertificateList, cRL,
                                   :not_encrypted} ->
                                  {true, cRL}
                                _ ->
                                  false
  ...
  defp cache_lookup(uRL, {{cache, _}, _}) do
    %{path: path} = :uri_string.normalize(uRL,
                                            [:return_map])
    case (:ssl_pkix_db.lookup(:string.trim(path, :leading,
                                             '/'),
                                cache)) do
      :undefined ->
        []
  ...
  defp handle_http(uRI, rest,
            {_, [{:http, timeout}]} = cRLDbInfo) do
