defmodule :m_ssl_crl_cache do
  ...
  def insert(cRLSrc) do
    insert(~c"http://dummy/no_distribution_point", cRLSrc)
  end
  ...
  def delete({:file, file}) do
    case :file.read_file(file) do
      {:ok, pemBin} ->
        ...
        :ssl_manager.delete_crls({~c"http://dummy/no_distribution_point", cRLs})
  ...
  def delete(uRI) do
    case :uri_string.normalize(uRI, [:return_map]) do
      %{scheme: ~c"http", path: path} ->
        :ssl_manager.delete_crls(:string.trim(path, :leading, ~c"/"))
  ...
  defp get_crls(
         [
           {:uniformResourceIdentifier, ~c"http" ++ _ = uRL}
           | rest
         ],
         cRLDbInfo
       ) do
  ...
  defp http_get(uRL, rest, cRLDbInfo, timeout) do
    case :httpc.request(:get, {uRL, [{~c"connection", ~c"close"}]}, [{:timeout, timeout}], [
           {:body_format, :binary}
         ]) do
