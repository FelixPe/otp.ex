defmodule :m_ssl_crl do
  ...
  Record.defrecord(:r_ticket_data, :ticket_data, key: :undefined,
                                       ...
                                       max_size: :undefined)
  def trusted_cert_and_path(cRL, {serialNumber, issuer}, certPath,
           {db, dbRef}) do
    case (:ssl_pkix_db.lookup_trusted_cert(db, dbRef,
                                             serialNumber, issuer)) do
      :undefined ->
        search_certpath(cRL, certPath, db, dbRef)
      {:ok, r_cert(otp: otpCert)} ->
        {:ok, root,
           chain} = :ssl_certificate.certificate_chain(otpCert, db,
                                                         dbRef)
  ...
  def trusted_cert_and_path(cRL, :issuer_not_found, certPath, {db, dbRef}) do
    case (search_certpath(cRL, certPath, db, dbRef)) do
      {:error, :unknown_ca} ->
        issuer = :public_key.pkix_normalize_name(:public_key.pkix_crl_issuer(cRL))
        isIssuerFun = fn {_Key, certCandidate}, acc ->
                           verify_crl_issuer(cRL, certCandidate, issuer, acc)
                         _, acc ->
                           acc
                      end
        case (search_db(isIssuerFun, db, dbRef)) do
          {:ok, otpCert} ->
            {:ok, root,
               chain} = :ssl_certificate.certificate_chain(otpCert, db,
                                                             dbRef)
  ...
  defp search_certpath(cRL, certPath, db, dbRef) do
    issuer = :public_key.pkix_normalize_name(:public_key.pkix_crl_issuer(cRL))
    isIssuerFun = fn certCandidate, acc ->
                       verify_crl_issuer(cRL, certCandidate, issuer, acc)
                  end
    case (find_issuer(isIssuerFun, :certpath, certPath)) do
      {:ok, otpCert} ->
        {:ok, root,
           chain} = :ssl_certificate.certificate_chain(otpCert, db,
                                                         dbRef)
  ...
  defp search_db(isIssuerFun, _, {:extracted, extractedCerts}) do
    find_issuer(isIssuerFun, :extracted, extractedCerts)
  end

  defp search_db(isIssuerFun, db, dbRef) do
  ...
  defp find_issuer(isIssuerFun, :extracted, certsData) do
    certs = (for {:decoded, entry} <- certsData do
               entry
             end)
    try do
      :lists.foldl(isIssuerFun, :issuer_not_found, certs)
  ...
  defp verify_crl_issuer(cRL, r_cert(otp: oTPCertCandidate), issuer,
            notIssuer) do
    tBSCert = r_OTPCertificate(oTPCertCandidate, :tbsCertificate)
    case (:public_key.pkix_normalize_name(r_OTPTBSCertificate(tBSCert, :subject))) do
      ^issuer ->
        case (:public_key.pkix_crl_verify(cRL,
                                            oTPCertCandidate)) do
          true ->
