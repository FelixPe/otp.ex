defmodule :m_ssl_logger do
          ...
          true ->
            :erlang.apply(:logger, :macro_log, [
              %{
                ...
                line: 65,
                file: ~c"otp/lib/ssl/src/ssl_logger.erl"
              },
              :debug,
  ...
  def format(%{alert: alert, alerter: :ignored} = report) do
    ...
    {~c"~s " ++ fmt, [~c"Ignored alert to mitigate DoS attacks", args]}
  end

  def format(%{description: desc, reason: reason}) do
    {~c"~12s ~p~n~12s ~p~n", [~c"Description:", desc, ~c"Reason:", reason]}
  end
  ...
  defp format_handshake(direction, binMsg) do
    ...
    :io_lib.format(~c"~s~n~s~n", [header, message])
  end
  ...
  defp parse_handshake(direction, r_hello_verify_request() = helloVerifyRequest) do
    header = :io_lib.format(~c"~s Handshake, HelloVerifyRequest", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_certificate() = certificate) do
    header = :io_lib.format(~c"~s Handshake, Certificate", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_certificate_status() = certificateStatus) do
    header = :io_lib.format(~c"~s Handshake, CertificateStatus", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_server_key_exchange() = serverKeyExchange) do
    header = :io_lib.format(~c"~s Handshake, ServerKeyExchange", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_certificate_request() = certificateRequest) do
    header = :io_lib.format(~c"~s Handshake, CertificateRequest", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_server_hello_done() = serverHelloDone) do
    header = :io_lib.format(~c"~s Handshake, ServerHelloDone", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_client_key_exchange() = clientKeyExchange) do
    header = :io_lib.format(~c"~s Handshake, ClientKeyExchange", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_certificate_verify() = certificateVerify) do
    header = :io_lib.format(~c"~s Handshake, CertificateVerify", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_finished() = finished) do
    header = :io_lib.format(~c"~s Handshake, Finished", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_hello_request() = helloRequest) do
    header = :io_lib.format(~c"~s Handshake, HelloRequest", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_encrypted_extensions() = encryptedExtensions) do
    header = :io_lib.format(~c"~s Handshake, EncryptedExtensions", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_new_session_ticket() = newSessionTicket) do
    header = :io_lib.format(~c"~s Post-Handshake, NewSessionTicket", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_key_update() = keyUpdate) do
    header = :io_lib.format(~c"~s Post-Handshake, KeyUpdate", [header_prefix(direction)])
  ...
  defp parse_handshake(direction, r_end_of_early_data() = endOfEarlyData) do
    header = :io_lib.format(~c"~s Handshake, EndOfEarlyData", [header_prefix(direction)])
    ...
    message =
      :io_lib.format(
        ...
        [
          :lists.zip(
            Keyword.keys(r_end_of_early_data(r_end_of_early_data())),
  ...
  defp version({m, n}) do
    :io_lib.format(~c"TLS/DTLS [0x0~B0~B]", [m, n])
  end
  ...
  defp tls_record_version([
    ...
    :io_lib.format(~c"TLS 1.2 Record Protocol, ~s", [msg_type(b)])
  end
  ...
  defp tls_record_version([
    ...
    :io_lib.format(~c"TLS 1.1 Record Protocol, ~s", [msg_type(b)])
  end
  ...
  defp tls_record_version([
    ...
    :io_lib.format(~c"TLS 1.0 Record Protocol, ~s", [msg_type(b)])
  end
  ...
  defp tls_record_version([
    ...
    :io_lib.format(~c"SSL 3.0 Record Protocol, ~s", [msg_type(b)])
  end
  ...
  defp tls_record_version([
    ...
    :io_lib.format(~c"DTLS 1.2 Record Protocol, ~s", [msg_type(b)])
  end
  ...
  defp tls_record_version([
    ...
    :io_lib.format(~c"DTLS 1.0 Record Protocol, ~s", [msg_type(b)])
  end
  ...
  defp tls_record_version([
    ...
    :io_lib.format(~c"TLS/DTLS [0x0~B0~B] Record Protocol, ~s", [m, n, msg_type(b)])
  end
  ...
  defp row_prefix(_, n) do
    ...
    :lists.reverse(:lists.flatten(s ++ ~c" - "))
  end
  ...
  defp format_uknown_cipher_suite(
    ...
    ~c"0x" ++ number_to_hex(x) ++ ~c"0x" ++ number_to_hex(y)
  end
end
