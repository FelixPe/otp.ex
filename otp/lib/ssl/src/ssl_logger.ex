defmodule :m_ssl_logger do
  ...
  def debug(logLevel, direction, protocol, message)
      when (direction === :inbound or direction === :outbound) and (protocol === :record or protocol === :handshake) do
    case (:logger.compare_levels(logLevel, :debug)) do
      :lt ->
        case (:logger.allow(:debug, :ssl_logger)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:ssl_logger, :debug, 4}, line: 65,
      ...
      :eq ->
        case (:logger.allow(:debug, :ssl_logger)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:ssl_logger, :debug, 4}, line: 70,
                                 file: 'otp/lib/ssl/src/ssl_logger.erl'},
                                 :debug, %{direction: direction,
                                             protocol: protocol,
                                             message: message},
  ...
  def format(%{alert: alert, alerter: :own} = report) do
    %{protocol: protocolName, role: role, alert: ^alert,
        statename: stateName} = report
    :ssl_alert.own_alert_format(protocolName, role,
                                  stateName, alert)
  end
  ...
  def format(%{alert: alert, alerter: :peer} = report) do
    %{protocol: protocolName, role: role, alert: ^alert,
        statename: stateName} = report
    :ssl_alert.alert_format(protocolName, role, stateName,
                              alert)
  end
  ...
  def format(%{alert: alert, alerter: :ignored} = report) do
    %{protocol: protocolName, role: role, alert: ^alert,
        statename: stateName} = report
    {fmt, args} = :ssl_alert.own_alert_format(protocolName,
                                                role, stateName, alert)
    {'~s ' ++ fmt, ['Ignored alert to mitigate DoS attacks', args]}
  end

  def format(%{description: desc, reason: reason}) do
    {'~12s ~p~n~12s ~p~n', ['Description:', desc, 'Reason:', reason]}
  end
  ...
  defp parse_handshake(direction,
            r_client_hello(client_version: version0,
                cipher_suites: cipherSuites0,
                extensions: extensions) = clientHello) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_client_hello(r_client_hello())),
                                             tl(:erlang.tuple_to_list(r_client_hello(clientHello, cipher_suites: cipherSuites))))])
    {header, message}
  end

  defp parse_handshake(direction,
            r_server_hello(server_version: version0, cipher_suite: cipherSuite0,
                extensions: extensions) = serverHello) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_server_hello(r_server_hello())),
  ...
  defp parse_handshake(direction, r_hello_verify_request() = helloVerifyRequest) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_hello_verify_request(r_hello_verify_request())),
  ...
  defp parse_handshake(direction, r_certificate() = certificate) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_certificate(r_certificate())),
  ...
  defp parse_handshake(direction, r_certificate_status() = certificateStatus) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_certificate_status(r_certificate_status())),
  ...
  defp parse_handshake(direction, r_server_key_exchange() = serverKeyExchange) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_server_key_exchange(r_server_key_exchange())),
  ...
  defp parse_handshake(direction, r_server_key_params() = serverKeyExchange) do
    header = :io_lib.format('~s Handshake, ServerKeyExchange', [header_prefix(direction)])
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_server_key_params(r_server_key_params())),
                                             tl(:erlang.tuple_to_list(serverKeyExchange)))])
    {header, message}
  end
  ...
  defp parse_handshake(direction, r_certificate_request() = certificateRequest) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_certificate_request(r_certificate_request())),
  ...
  defp parse_handshake(direction, r_server_hello_done() = serverHelloDone) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_server_hello_done(r_server_hello_done())),
  ...
  defp parse_handshake(direction, r_client_key_exchange() = clientKeyExchange) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_client_key_exchange(r_client_key_exchange())),
  ...
  defp parse_handshake(direction, r_certificate_verify() = certificateVerify) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_certificate_verify(r_certificate_verify())),
  ...
  defp parse_handshake(direction, r_finished() = finished) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_finished(r_finished())),
  ...
  defp parse_handshake(direction, r_hello_request() = helloRequest) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_hello_request(r_hello_request())),
  ...
  defp parse_handshake(direction, r_certificate_request_1_3() = certificateRequest) do
    header = :io_lib.format('~s Handshake, CertificateRequest', [header_prefix(direction)])
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_certificate_request_1_3(r_certificate_request_1_3())),
                                             tl(:erlang.tuple_to_list(certificateRequest)))])
    {header, message}
  end
  ...
  defp parse_handshake(direction, r_certificate_1_3() = certificate) do
    header = :io_lib.format('~s Handshake, Certificate', [header_prefix(direction)])
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_certificate_1_3(r_certificate_1_3())),
                                             tl(:erlang.tuple_to_list(certificate)))])
    {header, message}
  end
  ...
  defp parse_handshake(direction, r_certificate_verify_1_3() = certificateVerify) do
    header = :io_lib.format('~s Handshake, CertificateVerify', [header_prefix(direction)])
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_certificate_verify_1_3(r_certificate_verify_1_3())),
                                             tl(:erlang.tuple_to_list(certificateVerify)))])
    {header, message}
  end
  ...
  defp parse_handshake(direction, r_encrypted_extensions() = encryptedExtensions) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_encrypted_extensions(r_encrypted_extensions())),
  ...
  defp parse_handshake(direction, r_new_session_ticket() = newSessionTicket) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_new_session_ticket(r_new_session_ticket())),
  ...
  defp parse_handshake(direction, r_key_update() = keyUpdate) do
    ...
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_key_update(r_key_update())),
                                             tl(:erlang.tuple_to_list(keyUpdate)))])
    {header, message}
  end

  defp parse_handshake(direction, r_end_of_early_data() = endOfEarlyData) do
    header = :io_lib.format('~s Handshake, EndOfEarlyData', [header_prefix(direction)])
    message = :io_lib.format('~p',
                               [:lists.zip(Keyword.keys(r_end_of_early_data(r_end_of_early_data())),
                                             tl(:erlang.tuple_to_list(endOfEarlyData)))])
    {header, message}
  end
  ...
  defp get_client_version(version, extensions) do
    cHVersions = :maps.get(:client_hello_versions,
                             extensions, :undefined)
    case (cHVersions) do
  ...
  defp get_server_version(version, extensions) do
    sHVersion = :maps.get(:server_hello_selected_version,
                            extensions, :undefined)
    case (sHVersion) do
  ...
  defp format_tls_record(direction, binMsg) do
    ...
    header = :io_lib.format('~s (~B bytes) ~s~n',
                              [header_prefix_tls_record(direction), size,
  ...
  defp tls_record_version([<<b :: size(8) - unsigned - big - integer,
  ...
  defp tls_record_version([<<b :: size(8) - unsigned - big - integer,
               254 :: size(8) - unsigned - big - integer,
               253 :: size(8) - unsigned - big - integer,
               _ :: binary>> |
               _]) do
  ...
  defp convert_to_hex(p,
            [<<a :: size(4), b :: size(4), r :: binary>> | t], row,
            acc, c)
      when c === 0 do
    convert_to_hex(p, [<<r :: binary>> | t],
                     update_row(<<a :: size(4), b :: size(4)>>, row),
                     prepend_first_row(p, a, b, acc, c), c + 1)
  end
  ...
  defp convert_to_hex(p,
    ...
    convert_to_hex(p, [<<r :: binary>> | t],
                     ...
                     prepend_row(p, a, b, row, acc, c), c + 1)
  end
  ...
  defp convert_to_hex(p,
    ...
    convert_to_hex(p, [<<r :: binary>> | t],
                     update_row(<<a :: size(4), b :: size(4)>>, row),
                     prepend_eighths_hex(a, b, acc), c + 1)
  end
  ...
  defp convert_to_hex(p,
    ...
    convert_to_hex(p, [<<r :: binary>> | t],
                     update_row(<<a :: size(4), b :: size(4)>>, row),
                     prepend_hex(a, b, acc), c + 1)
  end

  defp convert_to_hex(p, [h | t], row, acc, c) when (is_integer(h) and
                                           c === 0) do
    convert_to_hex(p, t, update_row(h, row),
                     prepend_first_row(p, h, acc, c), c + 1)
  end

  defp convert_to_hex(p, [h | t], row, acc, c) when (is_integer(h) and
                                           rem(c, 16) === 0) do
    convert_to_hex(p, t, update_row(h, []),
                     prepend_row(p, h, row, acc, c), c + 1)
  end

  defp convert_to_hex(p, [h | t], row, acc, c) when (is_integer(h) and
                                           rem(c, 8) === 7) do
    convert_to_hex(p, t, update_row(h, row),
                     prepend_eighths_hex(h, acc), c + 1)
  end

  defp convert_to_hex(p, [h | t], row, acc, c) when is_integer(h) do
    convert_to_hex(p, t, update_row(h, row),
                     prepend_hex(h, acc), c + 1)
  end

  defp row_prefix(_, n) do
    s = :string.pad(:string.to_lower(:erlang.integer_to_list(n,
                                                               16)),
                      4, :leading, ?0)
  ...
  defp prepend_row(p, a, b, row, acc, c) do
    prepend_hex(a, b,
                  row_prefix(p, c) ++ :io_lib.nl() ++ end_row(p,
                                                                row) ++ acc)
  end
  ...
  defp number_to_hex(n) do
    case (:string.to_lower(:erlang.integer_to_list(n,
                                                     16))) do
      h when length(h) < 2 ->
        :lists.append(h, '0')
      h ->
  ...
  defp format_uknown_cipher_suite(<<x :: size(8) - unsigned - big - integer,
              y :: size(8) - unsigned - big - integer>>) do
