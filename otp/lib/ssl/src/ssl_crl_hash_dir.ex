defmodule :m_ssl_crl_hash_dir do
        ...
        {:logger,
         {:notice, report,
          %{
            ...
            line: 54,
            file: ~c"otp/lib/ssl/src/ssl_crl_hash_dir.erl"
          }}, dERs}

      {:error, error} ->
        {:logger,
         {:error,
          %{description: ~c"CRL retrieval", reason: [{:cannot_find_crl, error}, {:dir, dir}]},
          %{
            mfa: {:ssl_crl_hash_dir, :select, 2},
            line: 58,
            file: ~c"otp/lib/ssl/src/ssl_crl_hash_dir.erl"
  ...
  defp find_crls(issuer, dir) do
    case :filelib.is_dir(dir) do
      true ->
        ...
        find_crls(issuer, hash, dir, 0, [], %{description: ~c"CRL file traversal", reason: []})
  ...
  defp find_crls(issuer, hash, dir, n, acc, %{reason: reason} = report) do
    filename =
      :filename.join(
        dir,
        hash ++ ~c".r" ++ :erlang.integer_to_list(n)
      )
