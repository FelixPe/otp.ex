defmodule :m_ssl_crl_hash_dir do
  ...
  def lookup(r_DistributionPoint(cRLIssuer: cRLIssuer) = dP, certIssuer,
           cRLDbInfo) do
    case (cRLIssuer) do
      :asn1_NOVALUE ->
  ...
  def select([{:directoryName, issuer} | _],
           {_DbHandle, [{:dir, dir}]}) do
    case (find_crls(:public_key.pkix_normalize_name(issuer),
                      dir)) do
      {%{reason: []}, dERs} ->
        dERs
      {%{reason: [_ | _]} = report, dERs} ->
        {:logger,
           {:notice, report,
              %{mfa: {:ssl_crl_hash_dir, :select, 2}, line: 54,
                  file: 'otp/lib/ssl/src/ssl_crl_hash_dir.erl'}},
           dERs}
      {:error, error} ->
        {:logger,
           {:error,
              %{description: 'CRL retrieval',
                  reason: [{:cannot_find_crl, error}, {:dir, dir}]},
              %{mfa: {:ssl_crl_hash_dir, :select, 2}, line: 58,
                  file: 'otp/lib/ssl/src/ssl_crl_hash_dir.erl'}},
           []}
    end
  end
    ...
    case (:filelib.is_dir(dir)) do
      true ->
        ...
        find_crls(issuer, hash, dir, 0, [],
                    %{description: 'CRL file traversal', reason: []})
      false ->
        {:error, :not_a_directory}
    end
  end

  defp find_crls(issuer, hash, dir, n, acc,
            %{reason: reason} = report) do
  ...
  defp maybe_parse_pem(<<"-----BEGIN", _ :: binary>> = pEM) do
    [{:CertificateList, dER,
        :not_encrypted}] = :public_key.pem_decode(pEM)
    dER
  end
