defmodule :m_ssl_server_session_cache do
  ...
  Record.defrecord(:r_ticket_data, :ticket_data, key: :undefined,
                                       ...
                                       cipher_suite: :undefined,
                                       max_size: :undefined)
  Record.defrecord(:r_state, :state, store_cb: :undefined,
                                 lifetime: :undefined, db: :undefined,
                                 max: :undefined, session_order: :undefined,
                                 id_generator: :undefined, listener: :undefined)
  def start_link(:ssl_unknown_listener = listener, map) do
    :gen_server.start_link({:local, listener},
                             :ssl_server_session_cache, [listener, map], [])
  end

  def start_link(listener, map) do
    :gen_server.start_link(:ssl_server_session_cache,
                             [listener, map], [])
  end
  ...
  def init([listener, %{lifetime: lifetime, session_cb: cb,
                        session_cb_init_args: initArgs, max: max}]) do
    :erlang.process_flag(:trap_exit, true)
    monitor = monitor_listener(listener)
    dbRef = init(cb, [{:role, :server} | initArgs])
    state = r_state(store_cb: cb, lifetime: lifetime, db: dbRef,
                max: max, session_order: :gb_trees.empty(),
                id_generator: :crypto.strong_rand_bytes(16),
                listener: monitor)
    {:ok, state}
  end

  def handle_call(:new_session_id, _From,
           r_state(id_generator: idGen) = state) do
    sessionId = session_id(idGen)
    {:reply, sessionId, state}
  end

  def handle_call({:reuse_session, sessionId}, _From,
           r_state(store_cb: cb, db: store0, lifetime: lifetime,
               session_order: order0) = state0) do
    case (lookup(cb, store0, sessionId)) do
      :undefined ->
        {:reply, :not_reusable, state0}
      r_session(internal_id: inId) = session ->
        case (:ssl_session.valid_session(session, lifetime)) do
          true ->
            {:reply, session, state0}
          false ->
            {store, order} = invalidate_session(cb, store0, order0,
                                                  sessionId, inId)
            {:reply, :not_reusable,
               r_state(state0, db: store,  session_order: order)}
        end
    end
  end

  def handle_cast({:register_session,
            r_session(session_id: sessionId,
                time_stamp: timeStamp) = session0},
           r_state(store_cb: cb, db: store0, max: max,
               lifetime: lifetime, session_order: order0) = state0) do
    internalId = {timeStamp,
                    :erlang.unique_integer([:monotonic])}
    session = r_session(session0, internal_id: internalId)
    state = (case (size(cb, store0)) do
               ^max ->
               ...
               size when size > 0 ->
                 ...
                 case (:ssl_session.valid_session(oldestSession,
                                                    lifetime)) do
                   true ->
                     store = update(cb, store0, sessionId,
                                      r_session(session, time_stamp: timeStamp))
                     r_state(state0, db: store, 
                                 session_order: :gb_trees.insert(internalId,
                                                                   sessionId,
                                                                   order0))
                   false ->
                     store1 = delete(cb, store0, oldSessId)
                     store = update(cb, store1, sessionId,
                                      r_session(session, time_stamp: timeStamp))
                     r_state(state0, db: store, 
                                 session_order: :gb_trees.insert(internalId,
                                                                   sessionId,
  ...
  def handle_info({:DOWN, monitor, _, _, _},
           r_state(listener: monitor) = state) do
    {:stop, :normal, state}
  end
  ...
  defp session_id(key) do
    ...
    bin1 = :crypto.crypto_one_time(:aes_128_ecb, key,
                                     <<unique1 :: size(128)>>, true)
    bin2 = :crypto.crypto_one_time(:aes_128_ecb, key,
                                     <<unique2 :: size(128)>>, true)
    <<bin1 :: binary, bin2 :: binary>>
  end

  defp invalidate_session(cb, store0, order, sessionId, internalId) do
  ...
  defp update(:ssl_server_session_cache_db = cb, cache, key,
            session) do
  ...
  defp delete(:ssl_server_session_cache_db = cb, cache,
            key) do
    cb.delete(cache, key)
  end
  ...
  defp monitor_listener(:ssl_unknown_listener) do
    :undefined
  end

  defp monitor_listener(listen) do
    :inet.monitor(listen)
  end
