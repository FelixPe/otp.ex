defmodule :m_ssl_server_session_cache do
  ...
  Record.defrecord(:r_state, :state,
    ...
    db: :undefined,
    max: :undefined,
    session_order: :undefined,
    id_generator: :undefined,
    listener: :undefined
  )

  def start_link(:ssl_unknown_listener = listener, map) do
    :gen_server.start_link({:local, listener}, :ssl_server_session_cache, [listener, map], [])
  end

  def start_link(listener, map) do
    :gen_server.start_link(:ssl_server_session_cache, [listener, map], [])
  end
  ...
  def handle_call(
        ...
        _From,
        r_state(store_cb: cb, db: store0, lifetime: lifetime, session_order: order0) = state0
      ) do
      ...
      r_session(internal_id: inId) = session ->
        case :ssl_session.valid_session(session, lifetime) do
          ...
          false ->
            ...
            {:reply, :not_reusable, r_state(state0, db: store, session_order: order)}
        end
    end
  end
  ...
  def handle_cast(
        ...
        r_state(store_cb: cb, db: store0, max: max, lifetime: lifetime, session_order: order0) =
          state0
      ) do
      ...
      case size(cb, store0) do
        ...
        size when size > 0 ->
          ...
          case :ssl_session.valid_session(
            ...
            true ->
              store = update(cb, store0, sessionId, r_session(session, time_stamp: timeStamp))
            ...
            false ->
              store1 = delete(cb, store0, oldSessId)
              store = update(cb, store1, sessionId, r_session(session, time_stamp: timeStamp))
  ...
  def handle_info(
        {:DOWN, monitor, _, _, _},
        r_state(listener: monitor) = state
      ) do
    {:stop, :normal, state}
  end
