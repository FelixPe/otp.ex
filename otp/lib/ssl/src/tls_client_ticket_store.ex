defmodule :m_tls_client_ticket_store do
  ...
  Record.defrecord(:r_state, :state, db: :undefined,
                                 lifetime: :undefined, max: :undefined)
  Record.defrecord(:r_data, :data, pos: :undefined,
                                cipher_suite: :undefined, sni: :undefined,
                                psk: :undefined, timestamp: :undefined,
                                ticket: :undefined, lock: :undefined)
  def start_link(max, lifetime) do
    :gen_server.start_link({:local,
                              :tls_client_ticket_store},
                             :tls_client_ticket_store, [max, lifetime], [])
  end

  def find_ticket(pid, ciphers, hashAlgos, sNI, earlyDataSize) do
    :gen_server.call(:tls_client_ticket_store,
                       {:find_ticket, pid, ciphers, hashAlgos, sNI,
                          earlyDataSize},
                       :infinity)
  end
  ...
  def remove_tickets(keys) do
    :gen_server.cast(:tls_client_ticket_store,
                       {:remove_tickets, keys})
  end
  ...
  def unlock_tickets(pid, keys) do
    :gen_server.call(:tls_client_ticket_store,
                       {:unlock, pid, keys}, :infinity)
  end

  def update_ticket(key, pos) do
    :gen_server.call(:tls_client_ticket_store,
                       {:update_ticket, key, pos}, :infinity)
  end
  ...
  def handle_call({:find_ticket, pid, ciphers, hashAlgos, sNI,
            earlyDataSize},
           _From, state) do
    key = do_find_ticket(state, pid, ciphers, hashAlgos,
                           sNI, earlyDataSize)
    {:reply, key, state}
  end
  ...
  def handle_call({:store_ticket, ticket, cipherSuite, sNI, pSK},
           _From, state0) do
    state = store_ticket(state0, ticket, cipherSuite, sNI,
                           pSK)
    {:reply, :ok, state}
  end
  ...
  defp inital_state([max, lifetime]) do
    :erlang.send_after(lifetime * 1000, self(),
                         :remove_invalid_tickets)
    r_state(db: :gb_trees.empty(), lifetime: lifetime, max: max)
  end

  defp do_find_ticket(iter, pid, ciphers, hashAlgos, sNI,
            earlyDataSize) do
    do_find_ticket(iter, pid, ciphers, hashAlgos, sNI,
                     earlyDataSize, [])
  end
  ...
  defp do_find_ticket(r_state(db: db, lifetime: lifetime) = state, pid,
            ciphers, [hash | t], sNI, earlyDataSize, acc) do
    case (iterate_tickets(:gb_trees.iterator(db), pid,
                            ciphers, hash, sNI, lifetime, earlyDataSize)) do
      {:undefined, :undefined} ->
        do_find_ticket(state, pid, ciphers, t, sNI,
                         earlyDataSize, acc)
      {:undefined, key} ->
        do_find_ticket(state, pid, ciphers, t, sNI,
  ...
  defp iterate_tickets(iter0, pid, ciphers, hash, sNI, lifetime,
            earlyDataSize) do
    iterate_tickets(iter0, pid, ciphers, hash, sNI,
                      lifetime, earlyDataSize, [])
  end

  defp iterate_tickets(iter0, pid, ciphers, hash, sNI, lifetime,
            earlyDataSize, acc) do
    case (:gb_trees.next(iter0)) do
      {key,
         r_data(cipher_suite: {cipher, ^hash}, sni: ticketSNI,
             ticket: r_new_session_ticket(extensions: extensions), timestamp: timestamp,
             lock: lock),
         iter}
  ...
  defp get_tickets(r_state(db: db) = state, pid, [key | t], acc) do
    try do
      ...
      r_data(pos: pos, cipher_suite: cipherSuite, psk: pSK,
          timestamp: timestamp, ticket: newSessionTicket,
          lock: lock)
          when lock === :undefined or lock === pid ->
        ...
        maxEarlyData = :tls_handshake_1_3.get_max_early_data(extensions)
        ticketData = r_ticket_data(key: key, pos: pos, identity: identity,
                         psk: pSK, nonce: nonce, cipher_suite: cipherSuite,
  ...
  defp remove_invalid_tickets(r_state(db: db, lifetime: lifetime) = state0) do
    keys = collect_invalid_tickets(:gb_trees.iterator(db),
                                     lifetime)
  ...
  defp collect_invalid_tickets(iter0, lifetime, acc) do
    case (:gb_trees.next(iter0)) do
      {key, r_data(timestamp: timestamp, lock: :undefined),
         iter} ->
        age = :erlang.monotonic_time(:millisecond) - timestamp
        cond do
          age < lifetime * 1000 ->
  ...
  defp store_ticket(r_state(db: db0, max: max) = state, ticket,
            cipherSuite, sNI, pSK) do
    ...
    db = :gb_trees.insert(key,
                            r_data(cipher_suite: cipherSuite, sni: sNI, psk: pSK,
                                timestamp: timestamp, ticket: ticket),
                            db1)
    r_state(state, db: db)
  end
  ...
  defp set_lock(r_state(db: db0) = state, pid, [key | t], cmd) do
    try do
      ...
      value ->
        db = :gb_trees.update(key,
                                update_data_lock(value, pid, cmd), db0)
