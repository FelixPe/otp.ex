defmodule :m_ssl_dist_sup do
  ...
  def start_link() do
    case (:init.get_argument(:ssl_dist_optfile)) do
      {:ok, [file]} ->
        ...
        :supervisor.start_link({:local, :ssl_dist_sup},
                                 :ssl_dist_sup, [])
      {:ok, badArg} ->
  ...
  defp ssl_admin_child_spec() do
    %{id: :ssl_dist_admin_sup,
        ...
        modules: [:ssl_dist_admin_sup], type: :supervisor}
  end
  ...
  def consult(file) do
    case (:erl_prim_loader.get_file(file)) do
      {:ok, binary, _FullName} ->
        encoding = (case (:epp.read_encoding_from_binary(binary)) do
                      :none ->
                        :latin1
                      enc ->
                        enc
        ...
        case (:unicode.characters_to_list(binary, encoding)) do
          {:error, _String, rest} ->
            :erlang.error({:bad_ssl_dist_optfile,
                             {:encoding_error, rest}})
          {:incomplete, _String, rest} ->
            :erlang.error({:bad_ssl_dist_optfile,
                             {:encoding_incomplete, rest}})
  ...
  defp consult_expr(expr) do
    {:value, value, bs} = :erl_eval.expr(expr,
