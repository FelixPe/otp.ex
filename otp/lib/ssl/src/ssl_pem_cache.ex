defmodule :m_ssl_pem_cache do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_state, :state, pem_cache: :undefined,
                                 last_pem_check: :undefined, clear: :undefined)
  def name(:normal) do
    :ssl_pem_cache
  end
  ...
  def init([name]) do
    ...
    {:ok,
       r_state(pem_cache: pemCache,
           last_pem_check: :erlang.convert_time_unit(:os.system_time(),
                                                       :native, :second),
           clear: interval)}
  end

  def handle_call({:unconditionally_clear_pem_cache, _}, _,
           r_state(pem_cache: pemCache) = state) do
    :ssl_pkix_db.clear(pemCache)
    result = :ssl_manager.refresh_trusted_db(ssl_manager_type())
    {:reply, result, state}
  end

  def handle_cast({:cache_pem, file, content},
           r_state(pem_cache: db) = state) do
    :ssl_pkix_db.insert(file, content, db)
    {:noreply, state}
  end

  def handle_cast({:invalidate_pem, file},
           r_state(pem_cache: db) = state) do
    :ssl_pkix_db.remove(file, db)
    :ssl_manager.refresh_trusted_db(ssl_manager_type(),
                                      file)
    {:noreply, state}
  end

  def handle_info(:clear_pem_cache,
           r_state(pem_cache: pemCache, clear: interval,
               last_pem_check: checkPoint) = state) do
    newCheckPoint = :erlang.convert_time_unit(:os.system_time(),
                                                :native, :second)
  ...
  defp call(msg) do
    :gen_server.call(:erlang.get(:ssl_pem_cache),
                       {msg, self()}, :infinity)
  end
  ...
  def init_pem_cache_validator([cacheName, pemCache, checkPoint]) do
    ...
    :ssl_pkix_db.foldl(&pem_cache_validate/2, checkPoint,
                         pemCache)
  end
  ...
  defp ssl_manager_type() do
    case (:erlang.get(:ssl_pem_cache)) do
      :ssl_pem_cache ->
        :normal
      _ ->
        :dist
