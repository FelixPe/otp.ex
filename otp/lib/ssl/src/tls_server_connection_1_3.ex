defmodule :m_tls_server_connection_1_3 do
  ...
  def init([:server, sender, host, port, socket, options,
                                                   user, cbInfo]) do
    state0 = (r_state(protocol_specific: map) = :tls_gen_connection_1_3.initial_state(:server,
                                                                                  sender,
                                                                                  host,
                                                                                  port,
                                                                                  socket,
    ...
    try do
      ...
      :gen_statem.enter_loop(:tls_server_connection_1_3, [],
                               :initial_hello, state)
    catch
      error ->
    ...
    :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                         ...
                                         where: %{mfa:
                                                  {:tls_server_connection_1_3,
                                                     :user_hello, 3},
                                                    line: 168, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'},
      ...
      {:error, reason} ->
        ...
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_server_connection_1_3,
                                                         :user_hello, 3},
                                                        line: 192, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'},
                                             reason: reason),
                                           :user_hello, state0)
    else
      options = %{versions: versions} ->
        state = :ssl_gen_statem.ssl_config(options, :server,
                                             state0)
            ...
            :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                                 ...
                                                 where: %{mfa:
                                                          {:tls_server_connection_1_3,
                                                             :user_hello, 3},
                                                            line: 182,
                                                            file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'}),
                                               :user_hello, state)
          _Else ->
            {:next_state, :hello,
               r_state(state, start_or_recv_from: from, 
  ...
  def start(:internal = type, r_change_cipher_spec() = msg,
           r_state(handshake_env: r_handshake_env(tls_handshake_history: hist)) = state) do
    case (:ssl_handshake.init_handshake_history()) do
      ^hist ->
        :ssl_gen_statem.handle_common_event(type, msg, :start,
                                              state)
      _ ->
        :tls_gen_connection_1_3.handle_change_cipher_spec(type,
                                                            msg, :start, state)
    end
  end
      ...
      false ->
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_server_connection_1_3,
                                                         :start, 3},
                                                        line: 223, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'}),
                                           :start, state)
    end
  end
  ...
  def start(:internal, r_client_hello(), state0) do
    :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                         ...
                                         where: %{mfa:
                                                  {:tls_server_connection_1_3,
                                                     :start, 3},
                                                    line: 240, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'}),
                                       :start, state0)
  end
  ...
  def negotiated(:internal = type, r_change_cipher_spec() = msg, state) do
    :tls_gen_connection_1_3.handle_change_cipher_spec(type,
                                                        msg, :negotiated, state)
  end

  def negotiated(:internal, {:start_handshake, _} = message,
           state0) do
  ...
  def wait_cv(:internal, r_certificate_verify_1_3() = certificateVerify, state0) do
    ...
    try do
      state1 = maybe.(:tls_handshake_1_3.verify_signature_algorithm(state0,
                                                                      certificateVerify))
      {state,
         nextState} = maybe.(:tls_handshake_1_3.verify_certificate_verify(state1,
                                                                            certificateVerify))
      :tls_gen_connection.next_event(nextState, :no_record,
                                       state)
    catch
      {^ref, {r_alert() = alert, aState}} ->
        :ssl_gen_statem.handle_own_alert(alert, :wait_cv,
                                           aState)
    end
  end
  ...
  def wait_finished(:internal, r_finished(verify_data: verifyData), state0) do
    {ref, maybe} = :tls_gen_connection_1_3.do_maybe()
    try do
      maybe.(:tls_handshake_1_3.validate_finished(state0,
                                                    verifyData))
      ...
      {record,
         r_state(protocol_specific: pS) = state} = :ssl_gen_statem.prepare_connection(state5,
                                                                                  :tls_gen_connection)
      :tls_gen_connection.next_event(:connection, record,
                                       r_state(state, protocol_specific: Map.put(pS, :exporter_master_secret,
                                                                                 exporterSecret)),
                                       [{{:timeout, :handshake}, :cancel}])
    catch
      {^ref, r_alert() = alert} ->
        :ssl_gen_statem.handle_own_alert(alert, :wait_finished,
                                           state0)
    end
  end
  ...
  def wait_eoed(:internal, r_end_of_early_data(),
           r_state(handshake_env: hsEnv0) = state0) do
    try do
      state = :ssl_record.step_encryption_state_read(state0)
      hsEnv = r_handshake_env(hsEnv0, early_data_accepted: false)
      :tls_gen_connection.next_event(:wait_finished,
                                       :no_record,
      ...
      :error, reason ->
                  ...
                  :ssl_logger.log(:info, :debug,
                                    %{description: :internal_error,
                                        ...
                                        [{:error, reason}, {:stacktrace,
                                                              __STACKTRACE__}]},
                                    %{mfa:
                                      {:tls_server_connection_1_3, :wait_eoed,
                                         3},
                                        line: 352, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'})
                __LogLevel__ ->
        ...
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_server_connection_1_3,
                                                         :wait_eoed, 3},
                                                        line: 353, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'},
                                             reason: reason),
                                           :wait_eoed, state0)
    end
  end
  ...
  defp do_handle_client_hello(r_client_hello(cipher_suites: clientCiphers, random: random,
              session_id: sessionId, extensions: extensions) = hello,
            r_state(ssl_options: %{ciphers: serverCiphers,
                               signature_algs: serverSignAlgs,
                               supported_groups: serverGroups0,
                               alpn_preferred_protocols: aLPNPreferredProtocols,
                               honor_cipher_order: honorCipherOrder,
                               early_data:
                               earlyDataEnabled} = opts) = state0) do
    ...
    try do
      ...
      offeredPSKs = :maps.get(:pre_shared_key, extensions,
                                :undefined)
      clientALPN0 = :maps.get(:alpn, extensions, :undefined)
      clientALPN = :ssl_handshake.decode_alpn(clientALPN0)
      clientSignAlgs = :tls_handshake_1_3.get_signature_scheme_list(:maps.get(:signature_algs,
                                                                                extensions,
                                                                                :undefined))
      ...
      r_state(connection_states: connectionStates0,
          session: session0,
          connection_env: r_connection_env(cert_key_alts: certKeyAlts)) = (state1 = maybe.(:ssl_gen_statem.handle_sni_extension(sNI,
                                                                                                                   state0)))
      maybe.(validate_cookie(cookie, state1))
      aLPNProtocol = maybe.(handle_alpn(aLPNPreferredProtocols,
                                          clientALPN))
      cipher = maybe.(select_cipher_suite(honorCipherOrder,
                                            clientCiphers, serverCiphers))
      groups = maybe.(:tls_handshake_1_3.select_common_groups(serverGroups,
                                                                clientGroups))
      ...
      certKeyPairs = :ssl_certificate.available_cert_key_pairs(certKeyAlts,
                                                                 {3, 4})
      r_session(own_certificates: [cert |
                               _]) = (session = maybe.(select_server_cert_key_pair(session0,
                                                                                     certKeyPairs,
                                                                                     clientSignAlgs,
                                                                                     clientSignAlgsCert,
                                                                                     certAuths,
                                                                                     state0,
                                                                                     :undefined)))
      {publicKeyAlgo, _, _, rSAKeySize,
         curve} = :tls_handshake_1_3.get_certificate_params(cert)
      selectedSignAlg = maybe.(:tls_handshake_1_3.select_sign_algo(publicKeyAlgo,
                                                                     rSAKeySize,
                                                                     clientSignAlgs,
                                                                     serverSignAlgs,
                                                                     curve))
      {group, clientPubKey} = select_client_public_key(groups,
                                                         clientShares)
      keyShare = :ssl_cipher.generate_server_share(group)
      state2 = (case (:maps.get(:max_frag_enum, extensions,
                                  :undefined)) do
                  maxFragEnum when elem(maxFragEnum, 0) === :max_frag_enum
                    ...
                    r_state(state1, handshake_env: hsEnv1,  session: session, 
                                connection_states: connectionStates1)
                  _ ->
                    r_state(state1, session: session)
                end)
      state3 = (case (:maps.get(:keep_secrets, opts,
                                  false)) do
                  true ->
                    :tls_handshake_1_3.set_client_random(state2,
      ...
      state4 = :tls_handshake_1_3.update_start_state(state3,
                                                       %{cipher: cipher,
                                                           ...
                                                           alpn: aLPNProtocol,
                                                           random: random})
      case (maybe.(send_hello_retry_request(state4,
                                              clientPubKey, keyShare,
                                              sessionId))) do
        {_, :start} = nextStateTuple ->
          nextStateTuple
        {state5, :negotiated} ->
          state = handle_early_data(state5, earlyDataEnabled,
                                      earlyDataIndication)
          pSK = maybe.(:tls_handshake_1_3.handle_pre_shared_key(state,
                                                                  offeredPSKs,
                                                                  cipher))
  ...
  defp send_hello_flight({:start_handshake, pSK0},
            r_state(connection_states: connectionStates0,
                handshake_env: r_handshake_env(early_data_accepted: earlyDataAccepted),
                static_env: r_static_env(protocol_cb: connection),
                session: r_session(session_id: sessionId, ecc: selectedGroup,
                             dh_public_value: clientPublicKey),
                ssl_options: %{} = sslOpts,
                key_share: keyShare) = state0) do
    serverPrivateKey = select_server_private_key(keyShare)
    %{security_parameters:
      secParamsR} = :ssl_record.pending_connection_state(connectionStates0,
                                                           :read)
    r_security_parameters(prf_algorithm: hKDF) = secParamsR
    {ref, maybe} = :tls_gen_connection_1_3.do_maybe()
    try do
      serverHello = :tls_handshake_1_3.server_hello(:server_hello,
                                                      sessionId, keyShare, pSK0,
                                                      connectionStates0)
      state1 = connection.queue_handshake(serverHello, state0)
      state2 = :tls_gen_connection_1_3.maybe_queue_change_cipher_spec(state1,
                                                                        :last)
      pSK = :tls_handshake_1_3.get_pre_shared_key(pSK0, hKDF)
      state3 = :tls_handshake_1_3.calculate_handshake_secrets(clientPublicKey,
                                                                serverPrivateKey,
                                                                selectedGroup,
                                                                pSK, state2)
      state4 = (case (earlyDataAccepted) do
                  true ->
                    :ssl_record.step_encryption_state_write(state3)
                  false ->
                    update_current_read(:ssl_record.step_encryption_state(state3),
                                          true, false)
                end)
      encryptedExtensions = :tls_handshake_1_3.encrypted_extensions(state4)
      state5 = connection.queue_handshake(encryptedExtensions,
                                            state4)
      {state6,
         nextState} = maybe_send_certificate_request(state5,
                                                       sslOpts, pSK0)
      state7 = maybe.(maybe_send_certificate(state6, pSK0))
      state8 = maybe.(maybe_send_certificate_verify(state7,
                                                      pSK0))
      ...
      :error, :badarg ->
        r_alert(level: 47,
            ...
            where: %{mfa:
                     {:tls_server_connection_1_3, :send_hello_flight, 2},
                       line: 587, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'})
    end
  end
  ...
  defp validate_cookie(r_cookie(cookie: cookie0),
            r_state(ssl_options: %{cookie: true},
                handshake_env: r_handshake_env(tls_handshake_history: {[_CH2, _HRR,
                                                                    messageHash |
                                                                        _],
                                                           _},
                                   cookie_iv_shard: {iV, shard}))) do
      ...
      false ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_server_connection_1_3, :validate_cookie, 2},
                          line: 605, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'})}
    end
  end
  ...
  defp validate_cookie(_, _) do
    {:error,
       r_alert(level: 2, description: 47,
           where: %{mfa:
                    {:tls_server_connection_1_3, :validate_cookie, 2},
                      line: 608, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'})}
  end

  defp session_resumption({r_state(ssl_options: %{session_tickets:
                            :disabled}) = state,
             :negotiated},
            _) do
  ...
  defp session_resumption({r_state(ssl_options: %{session_tickets: tickets},
               handshake_env: r_handshake_env(early_data_accepted: false)) = state0,
             :negotiated},
            pSKInfo)
      when tickets !== :disabled do
  ...
  defp session_resumption({r_state(ssl_options: %{session_tickets: tickets},
               handshake_env: r_handshake_env(early_data_accepted: true)) = state0,
             :negotiated},
            pSKInfo)
      when tickets !== :disabled do
    state1 = :tls_gen_connection_1_3.handle_resumption(state0,
                                                         :ok)
    {index, pSK, peerCert} = pSKInfo
    state2 = :tls_handshake_1_3.calculate_client_early_traffic_secret(state1,
                                                                        pSK)
  ...
  defp maybe_send_session_ticket(state) do
    number = (case (:application.get_env(:ssl,
                                           :server_session_tickets_amount)) do
  ...
  defp maybe_send_session_ticket(r_state(ssl_options: %{session_tickets:
                           :disabled}) = state,
            _) do
    state
  end
  ...
  defp maybe_send_session_ticket(r_state(connection_states: connectionStates,
    ...
    tracker = :proplists.get_value(:session_tickets_tracker,
                                     trackers)
    %{security_parameters:
      secParamsR} = :ssl_record.current_connection_state(connectionStates,
                                                           :read)
    r_security_parameters(prf_algorithm: hKDF,
  ...
  defp new_session_ticket(tracker, hKDF, rMS,
            r_state(ssl_options: %{session_tickets: :stateful_with_cert},
                session: r_session(peer_certificate: peerCert))) do
    :tls_server_session_ticket.new(tracker, hKDF, rMS,
                                     peerCert)
  end

  defp new_session_ticket(tracker, hKDF, rMS,
            r_state(ssl_options: %{session_tickets: :stateless_with_cert},
                session: r_session(peer_certificate: peerCert))) do
    :tls_server_session_ticket.new(tracker, hKDF, rMS,
                                     peerCert)
  end
  ...
  defp select_server_cert_key_pair(_, [], _, _, _, _,
            {:fallback, r_session() = session}) do
    {:ok, session}
  end
       ...
       r_alert(level: 2, description: 40,
           where: %{mfa:
                    {:tls_server_connection_1_3,
                      ...
                      line: 689, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'},
           reason: :unable_to_supply_acceptable_cert)}
  end

  defp select_server_cert_key_pair(session,
            [%{private_key: key, certs: [cert | _] = certs} | rest],
            clientSignAlgs, clientSignAlgsCert, certAuths,
            r_state(static_env: r_static_env(cert_db: certDbHandle,
                              cert_db_ref: certDbRef) = state),
            default0) do
    {_, signAlgo, signHash, _,
       _} = :tls_handshake_1_3.get_certificate_params(cert)
    case (:tls_handshake_1_3.check_cert_sign_algo(signAlgo,
                                                    signHash, clientSignAlgs,
                                                    clientSignAlgsCert)) do
      :ok ->
        case (:ssl_certificate.handle_cert_auths(certs,
          ...
          {:ok, encodeChain} ->
            {:ok,
               r_session(session, own_certificates: encodeChain, 
                            private_key: key)}
          {:error, encodeChain, :not_in_auth_domain} ->
            default = r_session(session, own_certificates: encodeChain, 
                                   private_key: key)
            select_server_cert_key_pair(session, rest,
                                          clientSignAlgs, clientSignAlgsCert,
                                          certAuths, state,
                                          default_or_fallback(default0,
                                                                default))
        end
      _ ->
        case (signHash) do
          ...
          _ ->
            fallback = {:fallback,
                          r_session(session, own_certificates: certs, 
                                       private_key: key)}
            select_server_cert_key_pair(session, rest,
                                          clientSignAlgs, clientSignAlgsCert,
                                          certAuths, state,
  ...
  defp select_client_public_key([group | groups],
  ...
  defp send_hello_retry_request(r_state(connection_states: connectionStates0,
              static_env: r_static_env(protocol_cb: connection)) = state0,
            :no_suitable_key, keyShare, sessionId) do
    serverHello0 = :tls_handshake_1_3.server_hello(:hello_retry_request,
                                                     sessionId, keyShare,
                                                     :undefined,
                                                     connectionStates0)
    {state1,
       serverHello} = :tls_handshake_1_3.maybe_add_cookie_extension(state0,
                                                                      serverHello0)
    state2 = connection.queue_handshake(serverHello, state1)
    state3 = :tls_gen_connection_1_3.maybe_queue_change_cipher_spec(state2,
                                                                      :last)
  ...
  defp update_current_read(r_state(connection_states: cS) = state,
            trialDecryption, earlyDataExpected) do
    read0 = :ssl_record.current_connection_state(cS, :read)
    read = Map.merge(read0, %{trial_decryption:
                              trialDecryption,
                                early_data_accepted: earlyDataExpected})
    r_state(state, connection_states: Map.put(cS, :current_read,
                                              read))
  end
  ...
  defp maybe_send_certificate_request(r_state(handshake_env: r_handshake_env(early_data_accepted: true)) = state,
            _, pSK)
  ...
  defp maybe_send_certificate_request(r_state(static_env: r_static_env(protocol_cb: connection,
                            cert_db: certDbHandle,
                            cert_db_ref: certDbRef)) = state,
            %{verify: :verify_peer, signature_algs: signAlgs,
                signature_algs_cert: signAlgsCert} = opts,
            _) do
    addCertAuth = :maps.get(:certificate_authorities, opts,
                              true)
    certificateRequest = :tls_handshake_1_3.certificate_request(signAlgs,
                                                                  signAlgsCert,
                                                                  certDbHandle,
                                                                  certDbRef,
                                                                  addCertAuth)
    {connection.queue_handshake(certificateRequest, state),
       :wait_cert}
  end
  ...
  defp maybe_send_certificate(r_state(session: r_session(own_certificates: ownCerts),
              static_env: r_static_env(protocol_cb: connection,
                              cert_db: certDbHandle,
                              cert_db_ref: certDbRef)) = state,
            _) do
    case (:tls_handshake_1_3.certificate(ownCerts,
  ...
  defp maybe_send_certificate_verify(r_state(session: r_session(sign_alg: signatureScheme,
                         private_key: certPrivateKey),
              static_env: r_static_env(protocol_cb: connection)) = state,
            _) do
    case (:tls_handshake_1_3.certificate_verify(certPrivateKey,
                                                  signatureScheme, state,
                                                  :server)) do
      {:ok, certificateVerify} ->
        {:ok,
           connection.queue_handshake(certificateVerify, state)}
      error ->
        error
    end
  end
  ...
  defp validate_client_key_share([], _) do
    {:error,
       r_alert(level: 2, description: 47,
           where: %{mfa:
                    {:tls_server_connection_1_3, :validate_client_key_share,
                       2},
                      line: 876, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'})}
  end

  defp validate_client_key_share([group | clientGroups],
            [r_key_share_entry(group: group) | clientShares]) do
  ...
  defp select_cipher_suite(_, [], _) do
    {:error,
       r_alert(level: 2, description: 71,
           where: %{mfa:
                    {:tls_server_connection_1_3, :select_cipher_suite, 3},
                      line: 883, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'},
  ...
  defp select_cipher_suite(false, [cipher | clientCiphers],
            serverCiphers) do
    case (:lists.member(cipher,
                          :tls_v1.exclusive_suites({3,
                                                      4})) and :lists.member(cipher,
                                                                               serverCiphers)) do
      true ->
        {:ok, cipher}
      false ->
  ...
  defp handle_alpn([], _) do
    {:error,
       r_alert(level: 2, description: 120,
           where: %{mfa:
                    {:tls_server_connection_1_3, :handle_alpn, 2},
                      line: 908, file: 'otp/lib/ssl/src/tls_server_connection_1_3.erl'})}
  end
