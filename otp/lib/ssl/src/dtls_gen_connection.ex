defmodule :m_dtls_gen_connection do
  ...
  def start_fsm(
    ...
    try do
      ...
      {:ok, sslSocket} =
        :ssl_gen_statem.socket_control(:dtls_gen_connection, socket, [pid], cbModule, tracker)
  ...
  defp next_record(
         r_state(
           ...
           connection_states: %{current_read: %{epoch: currentEpoch}}
         ) = state
       )
  ...
  def send_handshake_flight(
        r_state(
          static_env: r_static_env(socket: socket, transport_cb: transport, data_tag: dataTag),
  ...
  def handle_protocol_record(
        r_ssl_tls(type: 22, epoch: epoch, fragment: data),
        stateName,
        r_state(
    ...
    try do
      case :dtls_handshake.get_dtls_handshake(version, data, buffers0, options) do
        ...
        {packets, buffers} ->
          ...
          events =
            case is_new_connection(epoch, packets, state) do
              true ->
          ...
          {:next_state, stateName,
           r_state(state,
             ...
             handshake_env:
               r_handshake_env(hsEnv, unprocessed_handshake_events: unprocessed_events(hSEvents))
           ), events}
      end
    catch
      r_alert() = alert ->
  ...
  def socket(pid, transport, socket, _Tracker) do
    :dtls_socket.socket(pid, transport, socket, :dtls_gen_connection)
  end
      ...
      false ->
        ...
        alert =
          r_alert(
            ...
            description: 0,
            where: %{
              mfa: {:dtls_gen_connection, :handle_info, 3},
              line: 577,
              file: ~c"otp/lib/ssl/src/dtls_gen_connection.erl"
            },
  ...
  def handle_info(
        {:socket_reused, client},
        stateName,
        r_state(static_env: r_static_env(socket: {_, {client, _}})) = state
      ) do
    alert =
      r_alert(
        level: 2,
    ...
    :ssl_gen_statem.handle_normal_shutdown(r_alert(alert, role: :server), stateName, state)
    {:stop, {:shutdown, :transport_closed}, state}
  end
  ...
  defp is_new_connection(
         0,
         [{r_client_hello(), _Raw} | _],
         r_state(connection_states: %{current_read: %{epoch: currentEpoch}})
       )
  ...
  defp handle_alerts_or_reset([alert | alerts], stateName, r_state(connection_states: cs) = state) do
