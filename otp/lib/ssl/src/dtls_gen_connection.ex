defmodule :m_dtls_gen_connection do
  use Bitwise
  require Record
  ...
  def start_fsm(role, host, port, socket, {_, _, tracker} = opts,
           user, {cbModule, _, _, _, _} = cbInfo, timeout) do
    try do
      {:ok, pid} = :dtls_connection_sup.start_child([role,
                                                         host, port, socket,
                                                                         opts,
                                                                             user,
                                                                                 cbInfo])
      {:ok,
         sslSocket} = :ssl_gen_statem.socket_control(:dtls_gen_connection,
                                                       socket, [pid], cbModule,
                                                       tracker)
  ...
  defp next_record(r_state(handshake_env: r_handshake_env(unprocessed_handshake_events: n) = hsEnv) = state)
      when n > 0 do
    {:no_record,
       r_state(state, handshake_env: r_handshake_env(hsEnv, unprocessed_handshake_events: n - 1))}
  end

  defp next_record(r_state(protocol_buffers: r_protocol_buffers(dtls_cipher_texts: [r_ssl_tls(epoch: epoch) = cT |
                                                        rest]) = buffers,
              connection_states: %{current_read:
                                   %{epoch: epoch}} = connectionStates,
              static_env: r_static_env(data_tag: dataTag)) = state) do
    currentRead = :dtls_record.get_connection_state_by_epoch(epoch,
                                                               connectionStates,
                                                               :read)
    case (is_reliable(dataTag)) do
      true ->
      ...
      false ->
        case (:dtls_record.replay_detect(cT, currentRead)) do
          false ->
            decode_cipher_text(state)
          true ->
            next_record(r_state(state, protocol_buffers: r_protocol_buffers(buffers, dtls_cipher_texts: rest)))
        end
    end
  end

  defp next_record(r_state(protocol_buffers: r_protocol_buffers(dtls_cipher_texts: [r_ssl_tls(epoch: epoch) |
                                                        rest]) = buffers,
              connection_states: %{current_read:
  ...
  defp next_record(r_state(static_env: r_static_env(role: :server,
                            socket: {listener, {client, _}})) = state) do
    :dtls_packet_demux.active_once(listener, client, self())
    {:no_record, state}
  end

  defp next_record(r_state(protocol_specific: %{active_n_toggle: true,
    ...
    case (:dtls_socket.setopts(transport, socket,
                                 [{:active, n}])) do
      :ok ->
        {:no_record,
  ...
  defp is_reliable(:udp) do
    false
  end

  defp is_reliable(:sctp) do
    true
  end
  ...
  def next_event(stateName, :no_record,
           r_state(connection_states: %{current_read:
                                  %{epoch: currentEpoch}}) = state0,
           actions) do
    case (next_record(state0)) do
      {:no_record, state} ->
        :ssl_gen_statem.hibernate_after(stateName, state,
                                          actions)
      {r_ssl_tls(epoch: ^currentEpoch, type: 22,
      ...
      {r_ssl_tls(epoch: epoch, type: 22, version: _Version),
        ...
        {state, moreActions} = send_handshake_flight(state1,
                                                       currentEpoch)
  ...
  def next_event(:connection = stateName, record,
           r_state(connection_states: %{current_read:
                                  %{epoch: currentEpoch}}) = state0,
           actions) do
    case (record) do
      r_ssl_tls(epoch: epoch, type: 22, version: version) = ^record
          when epoch === currentEpoch or epoch === 0 ->
      ...
      r_ssl_tls(epoch: epoch, type: 22, version: _Version)
          when epoch == currentEpoch - 1 ->
        {state, moreActions} = send_handshake_flight(state0,
                                                       currentEpoch)
        next_event(stateName, :no_record, state,
                     actions ++ moreActions)
      r_ssl_tls(epoch: epoch, type: 20, version: _Version)
  ...
  def next_event(stateName, record,
           r_state(connection_states: %{current_read:
                                  %{epoch: currentEpoch}}) = state0,
           actions) do
    case (record) do
  ...
  def initial_flight_state(:udp) do
    {:retransmit, 1000}
  end

  def initial_flight_state(_) do
    :reliable
  end

  def new_flight() do
    %{next_sequence: 0, handshakes: [],
        change_cipher_spec: :undefined,
        handshakes_after_change_cipher_spec: []}
  end

  def send_handshake_flight(r_state(static_env: r_static_env(socket: socket,
                           transport_cb: transport, data_tag: dataTag),
    ...
    %{current_write:
      %{max_fragment_length:
        maxFragmentLength}} = connectionStates0
    maxSize = mtu(maxFragmentLength, dataTag)
    {encoded,
       connectionStates} = encode_handshake_flight(:lists.reverse(flight),
                                                     version, maxSize, epoch,
                                                     connectionStates0)
  ...
  def send_handshake_flight(r_state(static_env: r_static_env(socket: socket,
    ...
    maxSize = mtu(maxFragmentLength, dataTag)
    {hsBefore,
       connectionStates1} = encode_handshake_flight(:lists.reverse(flight0),
                                                      version, maxSize, epoch,
                                                      connectionStates0)
    {encChangeCipher,
       connectionStates} = encode_change_cipher(changeCipher,
                                                  version, epoch,
                                                  connectionStates1)
    send_packets(transport, socket, maxSize,
                   hsBefore ++ encChangeCipher)
  ...
  def send_handshake_flight(r_state(static_env: r_static_env(socket: socket,
    ...
    {hsBefore,
       connectionStates1} = encode_handshake_flight(:lists.reverse(flight0),
                                                      ...
                                                      connectionStates0)
    {encChangeCipher,
       connectionStates2} = encode_change_cipher(changeCipher,
                                                   version, epoch - 1,
                                                   connectionStates1)
    {hsAfter,
       connectionStates} = encode_handshake_flight(:lists.reverse(flight1),
                                                     version, maxSize, epoch,
                                                     connectionStates2)
    send_packets(transport, socket, maxSize,
                   hsBefore ++ encChangeCipher ++ hsAfter)
    :ssl_logger.debug(logLevel, :outbound, :record,
                        [hsBefore])
    :ssl_logger.debug(logLevel, :outbound, :record,
                        [encChangeCipher])
  ...
  def send_handshake_flight(r_state(static_env: r_static_env(socket: socket,
    ...
    maxSize = mtu(maxFragmentLength, dataTag)
    {encChangeCipher,
       connectionStates1} = encode_change_cipher(changeCipher,
                                                   version, epoch - 1,
                                                   connectionStates0)
    {hsAfter,
       connectionStates} = encode_handshake_flight(:lists.reverse(flight1),
  ...
  defp mtu(maxFragmentLength, :udp) do
    min(maxFragmentLength, 1200)
  end

  defp mtu(maxFragmentLength, :sctp) do
    min(maxFragmentLength, 16384)
  end

  def handle_protocol_record(r_ssl_tls(type: 23, fragment: data), stateName0,
           state0) do
    case (:ssl_gen_statem.read_application_data(data,
      ...
      {record, state1} ->
        {:next_state, stateName, state,
           actions} = next_event(stateName0, record, state1)
        :ssl_gen_statem.hibernate_after(stateName, state,
                                          actions)
    end
  end

  def handle_protocol_record(r_ssl_tls(type: 22, epoch: epoch, fragment: data),
           stateName,
           r_state(protocol_buffers: buffers0,
               connection_env: r_connection_env(negotiated_version: version),
               ssl_options: options) = state) do
    try do
      case (:dtls_handshake.get_dtls_handshake(version, data,
                                                 buffers0, options)) do
        {[], buffers} ->
          next_event(stateName, :no_record,
                       r_state(state, protocol_buffers: buffers))
        {packets, buffers} ->
          hsEnv = r_state(state, :handshake_env)
          hSEvents = dtls_handshake_events(packets)
          events = (case (is_new_connection(epoch, packets,
                                              state)) do
                      true ->
                        [{:next_event, :internal, :new_connection} | hSEvents]
                      false ->
                        hSEvents
                    end)
          {:next_state, stateName,
             r_state(state, protocol_buffers: buffers, 
                        handshake_env: r_handshake_env(hsEnv, unprocessed_handshake_events: unprocessed_events(hSEvents))),
             events}
      end
    catch
      r_alert() = alert ->
  ...
  def send_handshake(handshake,
           r_state(connection_states: connectionStates) = state) do
    %{epoch:
  ...
  def queue_handshake(handshake0,
    ...
    handshake = :dtls_handshake.encode_handshake(handshake0,
                                                   version, seq)
    hist = update_handshake_history(handshake0, handshake,
                                      hist0)
    :ssl_logger.debug(logLevel, :outbound, :handshake,
                        handshake0)
    r_state(state, flight_buffer: Map.merge(flight0, %{handshakes:
                                                 [handshake | hsBuffer0],
                                                   next_sequence: seq + 1}), 
  ...
  def queue_handshake(handshake0,
           r_state(handshake_env: r_handshake_env(tls_handshake_history: hist0) = hsEnv,
               connection_env: r_connection_env(negotiated_version: version),
               flight_buffer: %{handshakes_after_change_cipher_spec:
                                buffer0,
                                  next_sequence: seq} = flight0,
               ssl_options: %{log_level: logLevel}) = state) do
    handshake = :dtls_handshake.encode_handshake(handshake0,
    ...
    :ssl_logger.debug(logLevel, :outbound, :handshake,
                        handshake0)
    r_state(state, flight_buffer: Map.merge(flight0, %{handshakes_after_change_cipher_spec:
                                                 [handshake | buffer0],
                                                   next_sequence: seq + 1}), 
               handshake_env: r_handshake_env(hsEnv, tls_handshake_history: hist))
  end

  def queue_change_cipher(changeCipher,
           r_state(flight_buffer: flight,
               connection_states: connectionStates0) = state) do
    connectionStates = :dtls_record.next_epoch(connectionStates0,
                                                 :write)
    r_state(state, flight_buffer: Map.put(flight, :change_cipher_spec,
                                              changeCipher), 
  ...
  def reinit_handshake_data(r_state(static_env: r_static_env(data_tag: dataTag),
             protocol_buffers: buffers, protocol_specific: pS,
    ...
    r_state(state, handshake_env: r_handshake_env(hsEnv, tls_handshake_history: :ssl_handshake.init_handshake_history(), 
               ...
               protocol_specific: Map.put(pS, :flight_state,
                                                initial_flight_state(dataTag)), 
               flight_buffer: new_flight(), 
               protocol_buffers: r_protocol_buffers(buffers, dtls_handshake_next_seq: 0, 
                                              dtls_handshake_next_fragments: [], 
                                              dtls_handshake_later_fragments: []))
  end
  ...
  defp encode_alert(r_alert() = alert, version, connectionStates) do
    :dtls_record.encode_alert_record(alert, version,
                                       connectionStates)
  end
  ...
  def send(transport, {listener, socket}, data)
  ...
  defp send_packets(_Transport, _Socket, _Max, []) do
    :ok
  end

  defp send_packets(transport, socket, max, packets) do
    {packet, rest} = pack_packets(packets, 0, max, [])
    case (send(transport, socket, packet)) do
      :ok ->
        send_packets(transport, socket, max, rest)
      err ->
        err
    end
  end

  defp pack_packets([p | rest] = packets, soFar, max, acc) do
  ...
  defp pack_packets([], _, _, acc) do
    {:lists.reverse(acc), []}
  end

  def socket(pid, transport, socket, _Tracker) do
    :dtls_socket.socket(pid, transport, socket,
                          :dtls_gen_connection)
  end
  ...
  def handle_info({protocol, _, _, _, data}, stateName,
  ...
  def handle_info({passiveTag, socket}, stateName,
           r_state(static_env: r_static_env(socket: {_, socket},
    ...
    case (active == false and cTs !== []) do
      false ->
        ...
        alert = r_alert(level: 2, description: 0,
                    where: %{mfa: {:dtls_gen_connection, :handle_info, 3},
                               line: 577, file: 'otp/lib/ssl/src/dtls_gen_connection.erl'},
  ...
  def handle_info(:new_cookie_secret, stateName,
           r_state(protocol_specific: %{current_cookie_secret:
                                  secret} = cookieInfo) = state) do
    :erlang.send_after(:dtls_v1.cookie_timeout(), self(),
                         :new_cookie_secret)
    {:next_state, stateName,
       r_state(state, protocol_specific: Map.merge(cookieInfo, %{current_cookie_secret:
                                                           :dtls_v1.cookie_secret(),
                                                             previous_cookie_secret:
                                                             secret}))}
  end

  def handle_info({:socket_reused, client}, stateName,
           r_state(static_env: r_static_env(socket: {_, {client, _}})) = state) do
    alert = r_alert(level: 2, description: 0,
                where: %{mfa: {:dtls_gen_connection, :handle_info, 3},
                           line: 596, file: 'otp/lib/ssl/src/dtls_gen_connection.erl'},
                reason: :transport_closed)
    :ssl_gen_statem.handle_normal_shutdown(r_alert(alert, role: :server),
                                             stateName, state)
    {:stop, {:shutdown, :transport_closed}, state}
  end
  ...
  defp is_new_connection(0, [{r_client_hello(), _Raw} | _],
            r_state(connection_states: %{current_read:
                                   %{epoch: currentEpoch}}))
      when currentEpoch > 0 do
    true
  end

  defp is_new_connection(_, _, _) do
    false
  end
  ...
  defp encode_handshake_flight(flight, version, maxFragmentSize, epoch,
            connectionStates) do
    encode = fn fragment, {acc, cs0} ->
                  {enc, cs} = :dtls_record.encode_handshake(fragment,
                                                              version, epoch,
                                                              cs0)
                  {[enc | acc], cs}
             end
    {rev, cs} = :lists.foldl(fn handshake, {acc, cs0} ->
                                  frags = :dtls_handshake.fragment_handshake(handshake,
                                                                               maxFragmentSize)
                                  :lists.foldl(encode, {acc, cs0}, frags)
                             end,
                               {[], connectionStates}, flight)
    {:lists.reverse(rev), cs}
  end

  defp encode_change_cipher(r_change_cipher_spec(), version, epoch, connectionStates) do
    :dtls_record.encode_change_cipher_spec(version, epoch,
                                             connectionStates)
  end
  ...
  defp update_handshake_history(_, handshake, hist) do
    :ssl_handshake.update_handshake_history(hist,
                                              :erlang.iolist_to_binary(handshake))
  end

  defp next_dtls_record(data, stateName,
            r_state(protocol_buffers: r_protocol_buffers(dtls_record_buffer: buf0,
                                    dtls_cipher_texts: cT0) = buffers,
                connection_env: r_connection_env(negotiated_version: version),
                static_env: r_static_env(data_tag: dataTag),
                ssl_options: sslOpts) = state0) do
    case (:dtls_record.get_dtls_records(data,
                                          {dataTag, stateName, version,
                                             for vsn <- [:"dtlsv1.2", :dtlsv1] do
                                               :dtls_record.protocol_version_name(vsn)
                                             end},
                                          buf0, sslOpts)) do
      {records, buf1} ->
        cT1 = cT0 ++ records
        next_record(r_state(state0, protocol_buffers: r_protocol_buffers(buffers, dtls_record_buffer: buf1, 
  ...
  defp decode_cipher_text(r_state(protocol_buffers: r_protocol_buffers(dtls_cipher_texts: [cT |
                                                        rest]) = buffers,
              connection_states: connStates0) = state) do
    case (:dtls_record.decode_cipher_text(cT,
                                            connStates0)) do
      {plain, connStates} ->
        {plain,
           r_state(state, protocol_buffers: r_protocol_buffers(buffers, dtls_cipher_texts: rest), 
  ...
  defp handle_alerts(alerts, {:next_state, stateName, state}) do
    handle_alerts_or_reset(alerts, stateName, state)
  end

  defp handle_alerts(alerts,
            {:next_state, stateName, state, _Actions}) do
    handle_alerts_or_reset(alerts, stateName, state)
  end

  defp handle_alerts_or_reset([alert | alerts], stateName,
  ...
  defp handle_own_alert(alert, stateName,
            r_state(static_env: r_static_env(data_tag: :udp, role: role),
                ssl_options: %{log_level: logLevel}) = state0) do
    case (ignore_alert(alert, state0)) do
      {true, state} ->
        ...
        {:next_state, stateName, state}
      {false, state} ->
        :dtls_connection.alert_or_reset_connection(alert,
                                                     stateName, state)
    end
  end

  defp handle_own_alert(alert, stateName, state) do
    :dtls_connection.alert_or_reset_connection(alert,
                                                 stateName, state)
  end

  defp ignore_alert(r_alert(level: 2),
            r_state(protocol_specific: %{ignored_alerts: n,
                                     max_ignored_alerts: n}) = state) do
    {false, state}
  end

  defp ignore_alert(r_alert(level: 2) = alert,
            r_state(protocol_specific: %{ignored_alerts:
                                   n} = pS) = state) do
    case (is_ignore_alert(alert)) do
      true ->
        {true,
           r_state(state, protocol_specific: Map.put(pS, :ignored_alerts,
                                                     n + 1))}
      false ->
        {false, state}
    end
  ...
  defp log_ignore_alert(level, stateName, r_alert(where: location) = alert,
            role) do
    :ssl_logger.log(:info, level,
                      %{alert: alert, alerter: :ignored, statename: stateName,
                          role: role, protocol: protocol_name()},
                      location)
  end

  defp dtls_version(:hello, version,
