defmodule :m_tls_gen_connection do
  ...
  def start_fsm(
    ...
    senderOptions =
      handle_sender_options(
        erlDist,
        senderSpawnOpts
      )

    starter =
      start_connection_tree(user, erlDist, senderOptions, role, [
        host,
        port,
        socket,
        opts,
  ...
  defp start_connection_tree(user, isErlDist, senderOpts, role, receiverOpts) do
    startConnectionTree = fn ->
      try do
        start_dyn_connection_sup(isErlDist)
      catch
        :exit, {:noproc, _} ->
          send(user, {self(), {:error, :ssl_not_started}})
        ...
        _, reason ->
             ...
             case :erlang.get(:log_level) do
               :undefined ->
                 :ssl_logger.log(
                   ...
                   %{
                     mfa: {:tls_gen_connection, :start_connection_tree, 5},
                     line: 128,
                     file: ~c"otp/lib/ssl/src/tls_gen_connection.erl"
                   }
                 )
          ...
          send(user, {self(), {:error, :internal_error}})
      else
        {:ok, dynSup} ->
          case :tls_dyn_connection_sup.start_child(
                 dynSup,
                 :sender,
                 senderOpts
               ) do
            {:ok, sender} ->
              case :tls_dyn_connection_sup.start_child(
                ...
                {:ok, receiver} ->
                  send(user, {self(), receiver, sender})
  ...
  defp socket_control(socket, sockReceiver, sockSender, cbModule, trackers, timeout) do
    case :ssl_gen_statem.socket_control(
           :tls_gen_connection,
           socket,
           [sockReceiver, sockSender],
           cbModule,
           trackers
         ) do
      {:ok, sslSocket} ->
  ...
  def initialize_tls_sender(
        r_state(
          ...
          ssl_options:
            %{renegotiate_at: renegotiateAt, key_update_at: keyUpdateAt, log_level: logLevel} =
              sSLOpts,
  ...
  def handle_info(
        {protocol, _, data},
        stateName,
        r_state(static_env: r_static_env(data_tag: protocol)) = state0
      ) do
  ...
  def handle_info(
    ...
    alert =
      r_alert(
        ...
        description: 0,
        where: %{
          mfa: {:tls_gen_connection, :handle_info, 3},
          line: 318,
          file: ~c"otp/lib/ssl/src/tls_gen_connection.erl"
        },
    ...
    :ssl_gen_statem.handle_normal_shutdown(r_alert(alert, role: role), stateName, state)
  ...
  def handle_info(
        {:ssl_tls, port, type, {major, minor}, data},
        stateName,
        r_state(
    ...
    handle_info(
      {protocol, port, <<type, major, minor, len::size(16), data::binary>>},
      stateName,
      state0
    )
  end
  ...
  def next_event(
    ...
    case next_record(stateName, state0) do
      ...
      r_alert() = alert ->
        :ssl_gen_statem.handle_normal_shutdown(r_alert(alert, role: role), stateName, state0)
  ...
  def handle_protocol_record(
        r_ssl_tls(type: 23, early_data: false),
        stateName,
  ...
  def handle_protocol_record(
    ...
    case :ssl_gen_statem.read_application_data(
           data,
           state0
         ) do
      {:stop, _, _} = stop ->
  ...
  def handle_protocol_record(
    ...
    try do
      ...
      state =
        r_state(state0,
          protocol_buffers: r_protocol_buffers(buffers, tls_handshake_buffer: newHSBuffer)
        )
            ...
            _ ->
              ...
              {:next_state, stateName,
               r_state(state,
                 handshake_env:
                   r_handshake_env(hsEnv,
                     unprocessed_handshake_events: unprocessed_events(events)
                   )
               ), events}
          end
      end
  ...
  def handle_protocol_record(r_ssl_tls(type: 21, fragment: encAlerts), stateName, state) do
    try do
           ...
           case :erlang.get(:log_level) do
             :undefined ->
               :ssl_logger.log(
                 ...
                 :debug,
                 %{
                   description: ~c"Send failed",
                   reason: [{:error, reason}, {:stacktrace, __STACKTRACE__}]
                 },
                 %{
                   mfa: {:tls_gen_connection, :send_sync_alert, 2},
                   line: 545,
                   file: ~c"otp/lib/ssl/src/tls_gen_connection.erl"
                 }
               )
  ...
  def close({:close, timeout}, socket, transport = :gen_tcp, _) do
  ...
  def close({:shutdown, :transport_closed}, socket, transport = :gen_tcp, connectionStates) do
    close({:close, 0}, socket, transport, connectionStates)
  end

  def close({:shutdown, :own_alert}, socket, transport = :gen_tcp, connectionStates) do
    close({:close, 5000}, socket, transport, connectionStates)
  end
  ...
  defp get_tls_handshakes(
         data,
         stateName,
         r_state(
           protocol_buffers: r_protocol_buffers(tls_handshake_buffer: hSBuffer),
           connection_env: r_connection_env(negotiated_version: version),
           static_env: r_static_env(role: role),
           ssl_options: options
         )
       ) do
    case handle_unnegotiated_version(version, options, data, hSBuffer, role, stateName) do
  ...
  defp tls_handshake_events(hSPackets, recordRest) do
    restEvent =
      {:next_event, :internal, {:protocol_record, r_ssl_tls(type: 22, fragment: recordRest)}}
  ...
  defp next_record(
    ...
    case :tls_record.decode_cipher_text(version, cT, connectionStates0, check) do
      {record = r_ssl_tls(type: 23, fragment: fragment), connectionStates} ->
        case cipherTexts do
          [] ->
            next_record_done(
              state,
  ...
  defp next_record(state, cipherTexts, connectionStates, _, [_ | _] = acc, isEarlyData) do
    next_record_done(
      state,
      cipherTexts,
      connectionStates,
  ...
  defp handle_unnegotiated_version(
         {3, 3},
         %{versions: [{3, 4} = version | _]} = options,
         data,
         buffer,
         :client,
         :hello
       ) do
    <<_::size(8), length::size(24)-unsigned-big-integer, _::binary>> = data
    <<firstPacket::size(length + 4)-binary, recordRest::binary>> = data

    {hSPacket, <<>> = newHsBuffer} =
      :tls_handshake.get_tls_handshakes(
        version,
        firstPacket,
        buffer,
        options
      ...
      n when n > length ->
        throw(
          r_alert(
            ...
            description: 40,
            where: %{
              mfa: {:tls_gen_connection, :assert_buffer_sanity, 2},
              line: 864,
              file: ~c"otp/lib/ssl/src/tls_gen_connection.erl"
            },
