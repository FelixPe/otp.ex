defmodule :m_tls_gen_connection do
  use Bitwise
  require Record
  ...
  def start_fsm(role, host, port, socket,
           {sSLOpts, _, trackers} = opts, user,
           {cbModule, _, _, _, _} = cbInfo, timeout) do
    ...
    senderOptions = handle_sender_options(erlDist,
                                            senderSpawnOpts)
    starter = start_connection_tree(user, erlDist,
                                      senderOptions, role,
                                      [host, port, socket, opts, user, cbInfo])
    receive do
      {^starter, sockReceiver, sockSender} ->
        socket_control(socket, sockReceiver, sockSender,
                         cbModule, trackers, timeout)
      {^starter, error} ->
        error
    end
  end

  defp handle_sender_options(erlDist, spawnOpts) do
    case (erlDist) do
      true ->
        [[{:spawn_opt,
             [{:priority, :max} | :proplists.delete(:priority,
                                                      spawnOpts)]}]]
      false ->
        [[{:spawn_opt, spawnOpts}]]
    end
  ...
  defp start_connection_tree(user, isErlDist, senderOpts, role,
            receiverOpts) do
    startConnectionTree = fn () ->
                               try do
                                 start_dyn_connection_sup(isErlDist)
                               catch
                                 :exit, {:noproc, _} ->
                                   send(user, {self(),
                                         ...
                                         case (:erlang.get(:log_level)) do
                                           :undefined ->
                                             :ssl_logger.log(:notice, :debug,
                                                               ...
                                                               %{mfa:
                                                                 {:tls_gen_connection,
                                                                    :start_connection_tree,
                                                                    5},
                                                                   line: 128,
                                                                   file: 'otp/lib/ssl/src/tls_gen_connection.erl'})
                                           __LogLevel__ ->
                                             :ssl_logger.log(:notice,
                                                               __LogLevel__,
                                                               %{description:
                                                                 :internal_error,
                                                                   reason:
                                 ...
                                 {:ok, dynSup} ->
                                   case (:tls_dyn_connection_sup.start_child(dynSup,
                                                                               :sender,
                                                                               senderOpts)) do
                                     {:ok, sender} ->
                                       case (:tls_dyn_connection_sup.start_child(dynSup,
                                                                                   ...
                                                                                   [role,
                                                                                        sender |
                                                                                            receiverOpts])) do
                                         {:ok, receiver} ->
                                           send(user, {self(), receiver,
                                                         sender})
                                         {:error, error} ->
                                           send(user, {self(), error})
                                           :erlang.exit(dynSup, :shutdown)
                                       end
                                     {:error, error} ->
                                       send(user, {self(), error})
                                       :erlang.exit(dynSup, :shutdown)
  ...
  defp start_dyn_connection_sup(true) do
    :tls_connection_sup.start_child_dist([])
  end

  defp start_dyn_connection_sup(false) do
    :tls_connection_sup.start_child([])
  end

  defp socket_control(socket, sockReceiver, sockSender, cbModule,
            trackers, timeout) do
    case (:ssl_gen_statem.socket_control(:tls_gen_connection,
                                           socket, [sockReceiver, sockSender],
                                           cbModule, trackers)) do
      {:ok, sslSocket} ->
  ...
  def initialize_tls_sender(r_state(static_env: r_static_env(role: role,
             ...
             connection_states: %{current_write:
                                  connectionWriteState},
             protocol_specific: %{sender: sender})) do
    hibernateAfter = :maps.get(:hibernate_after, sSLOpts,
                                 :infinity)
    init = %{current_write: connectionWriteState,
               role: role, socket: socket, socket_options: sockOpts,
               erl_dist: :maps.get(:erl_dist, sSLOpts, false),
               trackers: trackers, transport_cb: transport,
               negotiated_version: version,
               renegotiate_at: renegotiateAt,
               key_update_at: keyUpdateAt, log_level: logLevel,
  ...
  def renegotiate(r_state(static_env: r_static_env(role: :client),
  ...
  def renegotiate(r_state(static_env: r_static_env(role: :server, socket: socket,
    ...
    frag = :tls_handshake.encode_handshake(helloRequest,
                                             version)
    hs0 = :ssl_handshake.init_handshake_history()
    {binMsg,
       connectionStates} = :tls_record.encode_handshake(frag,
                                                          version,
                                                          connectionStates0)
  ...
  def queue_handshake(handshake,
    ...
    {binHandshake, connectionStates,
       hist} = encode_handshake(handshake, version,
                                  connectionStates0, hist0)
    :ssl_logger.debug(logLevel, :outbound, :handshake,
                        handshake)
  ...
  def send_handshake_flight(r_state(static_env: r_static_env(socket: socket,
                           transport_cb: transport),
             flight_buffer: flight) = state0) do
    :tls_socket.send(transport, socket, flight)
    {r_state(state0, flight_buffer: []), []}
  end

  def queue_change_cipher(msg,
           r_state(connection_env: r_connection_env(negotiated_version: version),
               flight_buffer: flight0,
               ssl_options: %{log_level: logLevel},
               connection_states: connectionStates0) = state0) do
    {binChangeCipher,
       connectionStates} = encode_change_cipher(msg, version,
                                                  connectionStates0)
    :ssl_logger.debug(logLevel, :outbound, :record,
                        binChangeCipher)
    r_state(state0, connection_states: connectionStates, 
  ...
  def reinit(r_state(protocol_specific: %{sender: sender},
             connection_env: r_connection_env(negotiated_version: version),
             connection_states: %{current_write: write}) = state) do
    :tls_sender.update_connection_state(sender, write,
                                          version)
    reinit_handshake_data(state)
  end

  def reinit_handshake_data(r_state(handshake_env: hsEnv) = state) do
    r_state(state, handshake_env: r_handshake_env(hsEnv, tls_handshake_history: :ssl_handshake.init_handshake_history(), 
                                       public_key_info: :undefined, 
                                       premaster_secret: :undefined))
  end
  ...
  def socket(pids, transport, socket, trackers) do
    :tls_socket.socket(pids, transport, socket,
                         :tls_connection, trackers)
  end
  ...
  def handle_info({protocol, _, data}, stateName,
           r_state(static_env: r_static_env(data_tag: protocol)) = state0) do
    case (next_tls_record(data, stateName, state0)) do
      {record, state} ->
        next_event(stateName, record, state)
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert, stateName,
                                           state0)
    end
  end

  def handle_info({passiveTag, socket}, stateName,
           r_state(static_env: r_static_env(socket: socket,
               ...
               protocol_specific: pS) = state0) do
    case (from !== :undefined and cTs == []) do
      true ->
        {record,
           state} = activate_socket(r_state(state0, protocol_specific: Map.put(pS, :active_n_toggle,
                                                                               true)))
        next_event(stateName, record, state)
      false ->
        next_event(stateName, :no_record,
                     r_state(state0, protocol_specific: Map.put(pS, :active_n_toggle,
                                                                true)))
    end
  end

  def handle_info({closeTag, socket}, stateName,
           r_state(static_env: r_static_env(role: role, host: host, port: port,
                             socket: socket, close_tag: closeTag),
               handshake_env: r_handshake_env(renegotiation: type),
               session: session) = state)
      when stateName !== :connection do
    :ssl_gen_statem.maybe_invalidate_session(type, role,
                                               host, port, session)
    alert = r_alert(level: 2, description: 0,
                where: %{mfa: {:tls_gen_connection, :handle_info, 3},
                           line: 318, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                reason: :transport_closed)
    :ssl_gen_statem.handle_normal_shutdown(r_alert(alert, role: role),
                                             stateName, state)
  ...
  def handle_info({closeTag, socket}, stateName,
           r_state(static_env: r_static_env(role: role, socket: socket,
               ...
               protocol_specific: pS) = state) do
    case (active == false and from == :undefined) do
      false ->
        alert = r_alert(level: 2, description: 0,
                    where: %{mfa: {:tls_gen_connection, :handle_info, 3},
                               line: 347, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                    reason: :transport_closed)
        :ssl_gen_statem.handle_normal_shutdown(r_alert(alert, role: role),
                                                 stateName, state)
        {:stop, {:shutdown, :transport_closed}, state}
      true ->
        {:next_state, stateName,
           r_state(state, protocol_specific: Map.put(pS, :active_n_toggle,
                                                     true)),
           []}
    end
  end

  def handle_info({:ssl_tls, port, type, {major, minor}, data},
           stateName,
           r_state(static_env: r_static_env(data_tag: protocol),
               ssl_options: %{ktls: true}) = state0) do
    len = byte_size(data)
    handle_info({protocol, port,
                   <<type, major, minor, len :: size(16),
                       data :: binary>>},
                  stateName, state0)
  end
  ...
  def next_event(stateName, :no_record,
           r_state(static_env: r_static_env(role: role)) = state0, actions) do
  ...
  def next_event(stateName, r_alert() = alert, state, actions) do
    {:next_state, stateName, state,
       [{:next_event, :internal, alert} | actions]}
  end

  def handle_protocol_record(r_ssl_tls(type: 23), stateName,
           r_state(static_env: r_static_env(role: :server),
               handshake_env: r_handshake_env(renegotiation: {false,
                                                  :first})) = state)
      when stateName == :initial_hello or
             stateName == :hello or stateName == :certify or
             stateName == :wait_cert_verify or
             stateName == :wait_ocsp_stapling or
             stateName == :abbreviated or stateName == :cipher do
    alert = r_alert(level: 2, description: 10,
                where: %{mfa:
                         {:tls_gen_connection, :handle_protocol_record, 3},
                           line: 407, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                reason: :application_data_before_initial_handshake)
    :ssl_gen_statem.handle_own_alert(alert, stateName,
                                       state)
  end

  def handle_protocol_record(r_ssl_tls(type: 23, early_data: false), stateName,
           r_state(static_env: r_static_env(role: :server)) = state)
      when stateName == :start or stateName == :recvd_ch or
             stateName == :negotiated or stateName == :wait_eoed do
    alert = r_alert(level: 2, description: 10,
                where: %{mfa:
                         {:tls_gen_connection, :handle_protocol_record, 3},
                           line: 415, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                reason: :none_early_application_data_before_handshake)
    :ssl_gen_statem.handle_own_alert(alert, stateName,
                                       state)
  end
  ...
  def handle_protocol_record(r_ssl_tls(type: 23), stateName,
           r_state(static_env: r_static_env(role: :server)) = state)
      when stateName == :wait_cert or stateName == :wait_cv or
             stateName == :wait_finished do
    alert = r_alert(level: 2, description: 10,
                where: %{mfa:
                         {:tls_gen_connection, :handle_protocol_record, 3},
                           line: 422, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                reason: :application_data_before_handshake_or_intervened_in_post_handshake_auth)
    :ssl_gen_statem.handle_own_alert(alert, stateName,
                                       state)
  end

  def handle_protocol_record(r_ssl_tls(type: 23, fragment: data), stateName,
           r_state(start_or_recv_from: from,
               socket_options: r_socket_options(active: false)) = state0)
      when from !== :undefined do
    case (:ssl_gen_statem.read_application_data(data,
                                                  state0)) do
      {:stop, _, _} = stop ->
        stop
      {record, r_state(start_or_recv_from: caller) = state} ->
        timerAction = (case (caller) do
                         :undefined ->
  ...
  def handle_protocol_record(r_ssl_tls(type: 22, fragment: data), stateName,
           r_state(ssl_options: options,
               protocol_buffers: buffers) = state0) do
    try do
      {hSPackets, newHSBuffer,
         recordRest} = get_tls_handshakes(data, stateName,
                                            state0)
      state = r_state(state0, protocol_buffers: r_protocol_buffers(buffers, tls_handshake_buffer: newHSBuffer))
      case (hSPackets) do
        [] ->
        ...
        _ ->
          ...
          case (stateName) do
            ...
            _ ->
              ...
              {:next_state, stateName,
                 r_state(state, handshake_env: r_handshake_env(hsEnv, unprocessed_handshake_events: unprocessed_events(events))),
                 events}
          end
      end
    catch
  ...
  def handle_protocol_record(r_ssl_tls(type: 20, fragment: data), stateName, state) do
    {:next_state, stateName, state,
       [{:next_event, :internal, r_change_cipher_spec(type: data)}]}
  end

  def handle_protocol_record(r_ssl_tls(type: 21, fragment: encAlerts), stateName,
           state) do
    try do
                  ...
                  :ssl_logger.log(:info, :debug,
                                    ...
                                    %{mfa:
                                      {:tls_gen_connection,
                                        ...
                                        line: 487, file: 'otp/lib/ssl/src/tls_gen_connection.erl'})
                __LogLevel__ ->
        ...
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_gen_connection,
                                                         ...
                                                         3},
                                                        line: 488, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
        ...
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_gen_connection,
                                                         :handle_protocol_record,
                                                         3},
                                                        line: 481, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                                             reason: :empty_alert),
                                           stateName, state)
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert, stateName,
  ...
  defp encode_alert(r_alert() = alert, version, connectionStates) do
    :tls_record.encode_alert_record(alert, version,
                                      connectionStates)
  end
  ...
  def send_alert(alert,
           r_state(static_env: r_static_env(socket: socket,
                             transport_cb: transport),
               connection_env: r_connection_env(negotiated_version: version0),
               ssl_options: %{log_level: logLevel, versions: versions},
               connection_states: connectionStates0) = stateData0) do
    version = available_version(version0, versions)
    {binMsg, connectionStates} = encode_alert(alert,
  ...
  defp available_version(:undefined, versions) do
    [version | _] = :lists.reverse(versions)
    version
  end

  defp available_version(negotiatedVersion, _) do
    negotiatedVersion
  end
  ...
  def send_alert_in_connection(alert,
           r_state(protocol_specific: %{sender: sender})) do
    :tls_sender.send_alert(sender, alert)
  end

  def send_sync_alert(alert,
           r_state(protocol_specific: %{sender: sender}) = state) do
    try do
                  ...
                  :ssl_logger.log(:info, :debug,
                                    %{description: 'Send failed',
                                        ...
                                        [{:error, reason}, {:stacktrace,
                                                              __STACKTRACE__}]},
                                    %{mfa:
                                      {:tls_gen_connection, :send_sync_alert,
                                         2},
                                        line: 545, file: 'otp/lib/ssl/src/tls_gen_connection.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:info, __LogLevel__,
                                    %{description: 'Send failed',
                                        reason:
  ...
  def close({:close, timeout}, socket, transport = :gen_tcp,
           _) do
  ...
  def close({:shutdown, :transport_closed}, socket,
           transport = :gen_tcp, connectionStates) do
    close({:close, 0}, socket, transport, connectionStates)
  end

  def close({:shutdown, :own_alert}, socket,
           transport = :gen_tcp, connectionStates) do
    close({:close, 5000}, socket, transport,
            connectionStates)
  end

  def close(_, socket, transport, _) do
  ...
  defp get_tls_handshakes(data, stateName,
            r_state(protocol_buffers: r_protocol_buffers(tls_handshake_buffer: hSBuffer),
                connection_env: r_connection_env(negotiated_version: version),
                static_env: r_static_env(role: role), ssl_options: options)) do
    case (handle_unnegotiated_version(version, options,
                                        data, hSBuffer, role, stateName)) do
      {hSPackets, newHSBuffer} ->
        noRecordRest = <<>>
  ...
  defp tls_handshake_events(hSPackets, <<>>) do
    :lists.map(fn hSPacket ->
                    {:next_event, :internal, {:handshake, hSPacket}}
               end,
                 hSPackets)
  end

  defp tls_handshake_events(hSPackets, recordRest) do
    restEvent = {:next_event, :internal,
                   {:protocol_record, r_ssl_tls(type: 22, fragment: recordRest)}}
  ...
  def encode_handshake(handshake, version, connectionStates0, hist0) do
    frag = :tls_handshake.encode_handshake(handshake,
                                             version)
  ...
  defp encode_change_cipher(r_change_cipher_spec(), version, connectionStates) do
    :tls_record.encode_change_cipher_spec(version,
                                            connectionStates)
  end

  defp next_tls_record(data, stateName,
            r_state(protocol_buffers: r_protocol_buffers(tls_record_buffer: buf0,
                                    tls_cipher_texts: cT0) = buffers,
                connection_env: r_connection_env(downgrade: downgrade),
                ssl_options: sslOpts0) = state0) do
    versions = (case (stateName) do
                  state when state === :hello or state === :start ->
                    allHelloVersions = [:sslv3, :"tlsv1.3", :"tlsv1.2", :"tlsv1.1", :tlsv1]
                    for vsn <- allHelloVersions do
                      :tls_record.protocol_version_name(vsn)
                    end
                  _ ->
    ...
    %{current_write:
      %{max_fragment_length:
        maxFragLen}} = r_state(state0, :connection_states)
    sslOpts = :maps.put(:downgrade, downgrade, sslOpts0)
    case (:tls_record.get_tls_records(data, versions, buf0,
                                        maxFragLen, sslOpts)) do
      {records, buf1} ->
        cT1 = cT0 ++ records
        next_record(stateName,
                      r_state(state0, protocol_buffers: r_protocol_buffers(buffers, tls_record_buffer: buf1, 
  ...
  defp next_record(_,
            r_state(protocol_buffers: r_protocol_buffers(tls_cipher_texts: [_ |
                                                         _] = cipherTexts),
                connection_states: connectionStates,
                ssl_options: sslOpts) = state) do
    check = :maps.get(:padding_check, sslOpts, true)
    next_record(state, cipherTexts, connectionStates, check)
  end

  defp next_record(:connection,
            r_state(protocol_buffers: r_protocol_buffers(tls_cipher_texts: []),
                protocol_specific: %{active_n_toggle: true}) = state) do
    flow_ctrl(state)
  end
  ...
  defp flow_ctrl(r_state(ssl_options: %{ktls: true}) = state) do
    {:no_record, state}
  end

  defp flow_ctrl(r_state(user_data_buffer: {_, size, _},
  ...
  defp flow_ctrl(r_state(socket_options: r_socket_options(active: false,
                                packet: packet)) = state)
  ...
  defp activate_socket(r_state(protocol_specific: %{active_n_toggle: true,
                                   active_n: n} = protocolSpec,
              static_env: r_static_env(socket: socket, close_tag: closeTag,
                              transport_cb: transport)) = state) do
    case (:tls_socket.setopts(transport, socket,
                                [{:active, n}])) do
      :ok ->
        {:no_record,
           r_state(state, protocol_specific: Map.put(protocolSpec, :active_n_toggle,
                                                               false))}
      _ ->
  ...
  defp next_record(r_state(connection_env: r_connection_env(negotiated_version: {3,
                                                     4} = version)) = state,
            [cT | cipherTexts], connectionStates0, check, acc,
            isEarlyData) do
    case (:tls_record.decode_cipher_text(version, cT,
                                           connectionStates0, check)) do
      {record = r_ssl_tls(type: 23, fragment: fragment),
         connectionStates} ->
        case (cipherTexts) do
          [] ->
            next_record_done(state, [], connectionStates,
                               r_ssl_tls(record, type: 23, 
                                           fragment: :erlang.iolist_to_binary(:lists.reverse(acc,
                                                                                               [fragment]))))
          [_ | _] ->
            next_record(state, cipherTexts, connectionStates, check,
                          [fragment | acc], r_ssl_tls(record, :early_data))
        end
      ...
      {_Record, _ConnectionStates_to_forget} ->
        next_record_done(state, [cT | cipherTexts],
                           connectionStates0,
                           r_ssl_tls(type: 23, early_data: isEarlyData,
                               fragment: :erlang.iolist_to_binary(:lists.reverse(acc))))
      r_alert() = alert ->
        alert
    end
  ...
  defp next_record(r_state(connection_env: r_connection_env(negotiated_version: version)) = state,
            [r_ssl_tls(type: 23) = cT | cipherTexts], connectionStates0,
            check, acc, notRelevant) do
    case (:tls_record.decode_cipher_text(version, cT,
                                           connectionStates0, check)) do
      {record = r_ssl_tls(type: 23, fragment: fragment),
         connectionStates} ->
  ...
  defp next_record(state, cipherTexts, connectionStates, _,
            [_ | _] = acc, isEarlyData) do
    next_record_done(state, cipherTexts, connectionStates,
                       r_ssl_tls(type: 23, early_data: isEarlyData,
                           fragment: :erlang.iolist_to_binary(:lists.reverse(acc))))
  end
  ...
  defp next_record(r_state(connection_env: r_connection_env(negotiated_version: version)) = state,
            [cT | cipherTexts], connectionStates0, check, [], _) do
    case (:tls_record.decode_cipher_text(version, cT,
                                           connectionStates0, check)) do
  ...
  defp accumulated_app_record([], _) do
    :no_record
  end

  defp accumulated_app_record([_ | _] = acc, isEarlyData) do
    r_ssl_tls(type: 23, early_data: isEarlyData,
        fragment: :erlang.iolist_to_binary(:lists.reverse(acc)))
  end

  defp next_record_done(r_state(protocol_buffers: buffers) = state,
            cipherTexts, connectionStates, record) do
    {record,
       r_state(state, protocol_buffers: r_protocol_buffers(buffers, tls_cipher_texts: cipherTexts), 
                  connection_states: connectionStates)}
  end

  defp handle_unnegotiated_version({3, 3},
            %{versions: [{3, 4} = version | _]} = options, data,
            buffer, :client, :hello) do
    <<_ :: size(8),
        length :: size(24) - unsigned - big - integer,
        _ :: binary>> = data
    <<firstPacket :: size(length + 4) - binary,
        recordRest :: binary>> = data
    {hSPacket,
       <<>> = newHsBuffer} = :tls_handshake.get_tls_handshakes(version,
                                                                 firstPacket,
                                                                 buffer,
                                                                 options)
  ...
  defp handle_unnegotiated_version({3, 3},
            %{versions: [{3, 4} = version | _]} = options, data,
            buffer, :client, :wait_sh) do
    :tls_handshake.get_tls_handshakes(version, data, buffer,
                                        options)
  end

  defp handle_unnegotiated_version(:undefined,
            %{versions: [version | _]} = options, data, buff, _,
            _) do
    :tls_handshake.get_tls_handshakes(version, data, buff,
                                        options)
  end

  defp handle_unnegotiated_version(version, options, data, buff, _, _) do
    :tls_handshake.get_tls_handshakes(version, data, buff,
                                        options)
  end

  defp assert_buffer_sanity(<<_Type :: size(8) - unsigned - big - integer,
              length :: size(24) - unsigned - big - integer,
              rest :: binary>>,
    ...
    case (byte_size(rest)) do
      ...
      n when n > length ->
        throw(r_alert(level: 2, description: 40,
                  where: %{mfa:
                           {:tls_gen_connection, :assert_buffer_sanity, 2},
                             line: 864, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                  reason: :too_big_handshake_data))
      _ ->
        throw(r_alert(level: 2, description: 40,
                  where: %{mfa:
                           {:tls_gen_connection, :assert_buffer_sanity, 2},
                             line: 867, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
    ...
    case (byte_size(bin)) do
      ...
      _ ->
        throw(r_alert(level: 2, description: 40,
                  where: %{mfa:
                           {:tls_gen_connection, :assert_buffer_sanity, 2},
                             line: 875, file: 'otp/lib/ssl/src/tls_gen_connection.erl'},
                  reason: :malformed_handshake_data))
    end
  end
  ...
  defp handle_alerts([r_alert(level: 1, description: 0) | _Alerts],
            {:next_state, :connection = stateName,
               r_state(connection_env: cEnv,
                   socket_options: r_socket_options(active: false),
                   start_or_recv_from: from) = state})
      when from == :undefined do
    {:next_state, stateName,
       r_state(state, connection_env: r_connection_env(cEnv, socket_tls_closed: true))}
  end

  defp handle_alerts([alert | alerts],
            {:next_state, stateName, state}) do
    handle_alerts(alerts,
                    :ssl_gen_statem.handle_alert(alert, stateName, state))
  end

  defp handle_alerts([alert | alerts],
            {:next_state, stateName, state, _Actions}) do
    handle_alerts(alerts,
                    :ssl_gen_statem.handle_alert(alert, stateName, state))
  end
