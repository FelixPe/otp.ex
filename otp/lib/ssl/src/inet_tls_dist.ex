defmodule :m_inet_tls_dist do
  ...
  defp f_address(family, address, node) do
    case :dist_util.split_node(node) do
      {:node, _, host} ->
        r_net_address(address: address, host: host, protocol: :tls, family: family)
  ...
  defp accept_loop({_, _, netKernelPid, _} = fLNC, continue, handshakePid, maxPending, pending) do
    receive do
      {^continue, ^handshakePid} when is_pid(handshakePid) ->
        accept_loop(fLNC, continue, :undefined, maxPending, Map.put(pending, handshakePid, true))
      ...
      when :erlang.is_map_key(
            ...
            true ->
              :erlang.apply(:logger, :macro_log, [
                %{
                  ...
                  line: 301,
                  file: ~c"otp/lib/ssl/src/inet_tls_dist.erl"
                },
                :error,
                ~c"TLS distribution handshake failed: ~p~n",
                [reason]
              ])
        ...
        accept_loop(fLNC, continue, handshakePid, maxPending, :maps.remove(pid, pending))
        ...
        case :logger.allow(:warning, :inet_tls_dist) do
          true ->
            :erlang.apply(:logger, :macro_log, [
              ...
              :warning,
              ~c"TLS distribution: unexpected message: ~p~n",
              [unexpected]
            ])
            ...
            {false, iP} ->
              case :logger.allow(:error, :inet_tls_dist) do
                true ->
                  :erlang.apply(:logger, :macro_log, [
                    %{
                      mfa: {:inet_tls_dist, :spawn_accept, 1},
                      line: 329,
                      file: ~c"otp/lib/ssl/src/inet_tls_dist.erl"
                    },
                    :error,
                    ~c"** Connection attempt from disallowed IP ~w ** ~n",
                    [iP]
                  ])
    ...
    case :ssl.handshake(
      ...
      {:ok, sslSocket} ->
        ...
        case kTLS do
                ...
                case :logger.allow(:error, :inet_tls_dist) do
                  true ->
                    :erlang.apply(:logger, :macro_log, [
                      ...
                      :error,
                      [{:slogan, :set_ktls_failed}, {:reason, ktlsReason}, {:pid, self()}]
                    ])
          ...
          false ->
            accept_one(family, sslSocket, netKernel, &:ssl.controlling_process/2)
        end
        ...
        case :logger.allow(:error, :inet_tls_dist) do
          true ->
            :erlang.apply(:logger, :macro_log, [
              ...
              :error,
              ~c"Cannot accept TLS distribution connection: ~s~n",
  ...
  defp accept_one(family, distSocket, netKernel, controllingProcessFun) do
    trace(send(netKernel, {:accept, self(), distSocket, family, :tls}))
            ...
            case :logger.allow(:error, :inet_tls_dist) do
              true ->
                :erlang.apply(:logger, :macro_log, [
                  ...
                  :error,
                  [{:slogan, :controlling_process_failed}, {:reason, reason}, {:pid, self()}]
                ])
  ...
  def accept_connection(acceptPid, distSocket, myNode, allowed, setupTime) do
    fam_accept_connection(:inet, acceptPid, distSocket, myNode, allowed, setupTime)
  end

  def fam_accept_connection(family, acceptPid, distSocket, myNode, allowed, setupTime) do
    kernel = self()

    monitor_pid(
      :erlang.spawn_opt(
        fn ->
          do_accept(family, acceptPid, distSocket, myNode, allowed, setupTime, kernel)
        end,
        :dist_util.net_ticker_spawn_options()
      )
    )
  ...
  defp do_accept(family, acceptPid, distSocket, myNode, allowed, setupTime, kernel) do
    ...
    receive do
        ...
        {hSData0, newAllowed} =
          case distSocket do
            sslSocket =
              ...
              :erlang.link(sender)
              {hs_data_ssl(family, sslSocket), allowed_nodes(sslSocket, allowed)}
      ...
      {^acceptPid, :exit} ->
        :dist_util.shutdown(:inet_tls_dist, 535, myNode, :connection_setup_failed)

      {:DOWN, ^mRef, _, _, _Reason} ->
        :dist_util.shutdown(:inet_tls_dist, 540, myNode, :connection_setup_failed)
    end
  end
    ...
    case :ssl.peercert(sslSocket) do
      {:ok, peerCertDER} ->
        case :ssl.peername(sslSocket) do
                ...
                case :logger.allow(:error, :inet_tls_dist) do
                  true ->
                    :erlang.apply(:logger, :macro_log, [
                      ...
                      :error,
                      ~c"** Connection attempt from disallowed node(s) ~p ** ~n",
                      [peerIP]
                    ])
          ...
          error1 ->
            :dist_util.shutdown(:inet_tls_dist, 567, :no_peer_ip, trace(error1))
        end
      ...
      error2 ->
        :dist_util.shutdown(:inet_tls_dist, 572, :no_peer_cert, trace(error2))
    end
  end
  ...
  def setup(node, type, myNode, longOrShortNames, setupTime) do
    fam_setup(:inet, node, type, myNode, longOrShortNames, setupTime)
  end

  def fam_setup(family, node, type, myNode, longOrShortNames, setupTime) do
    netKernel = self()

    monitor_pid(
      :erlang.spawn_opt(
        setup_fun(family, node, type, myNode, longOrShortNames, setupTime, netKernel),
        :dist_util.net_ticker_spawn_options()
      )
    )
  end

  defp setup_fun(family, node, type, myNode, longOrShortNames, setupTime, netKernel) do
    fn ->
      do_setup(family, node, type, myNode, longOrShortNames, setupTime, netKernel)
    end
  end
  ...
  defp ssl_options(:client, [~c"client_" ++ opt, value | t] = opts) do
    ssl_options(:client, t, opts, opt, value)
  end

  defp ssl_options(:server, [~c"server_" ++ opt, value | t] = opts) do
  ...
  defp termify(string) when is_list(string) do
    {:ok, tokens, _} = :erl_scan.string(string ++ ~c".")
  ...
  def set_ktls_ulp(
        %{socket: socket, setopt_fun: setoptFun, getopt_fun: getoptFun},
        oS
      ) do
  ...
  def ktls_opt_cipher(
        _OS,
        _TLS_version = {3, 4},
        _CipherSpec = <<19::size(8)-unsigned-big-integer, 2::size(8)-unsigned-big-integer>>,
        r_cipher_state(
          key: <<key::size(32)-bytes>>,
    ...
    value =
      <<tLS_1_3_VERSION::size(16)-native, tLS_CIPHER_AES_GCM_256::size(16)-native, iV::bytes,
        key::bytes, salt::bytes, cipherSeq::size(64)-native>>
  ...
  def ktls_opt_cipher(_OS, tLS_version, cipherSpec, _CipherState, _CipherSeq, _TxRx) do
    {:error, {:ktls_notsup, {:cipher, tLS_version, cipherSpec, _CipherState}}}
  end
