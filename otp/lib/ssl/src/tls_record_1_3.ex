defmodule :m_tls_record_1_3 do
  ...
  Record.defrecord(:r_cipher_state, :cipher_state, iv: :undefined,
                                        ...
                                        tag_len: :undefined)
  def encode_handshake(frag,
           %{current_write:
             %{max_fragment_length:
               maxFragmentLength}} = connectionStates) do
    maxLength = (cond do
    ...
    case (:erlang.iolist_size(frag)) do
      n when n > maxLength ->
        data = :tls_record.split_iovec(:erlang.iolist_to_iovec(frag),
                                         maxLength)
  ...
  def encode_plain_text(type, data0,
           %{current_write: write0} = connectionStates) do
    padLen = 0
    data = inner_plaintext(type, data0, padLen)
    cipherFragment = encode_plain_text(data, write0)
    {cipherText,
       write} = encode_tls_cipher_text(cipherFragment, write0)
    {cipherText,
       Map.put(connectionStates, :current_write, write)}
  end

  defp encode_iolist(type, data, connectionStates0) do
    {connectionStates, encodedMsg} = :lists.foldl(fn text,
                                                       {cS0, encoded} ->
                                                       {enc,
                                                          cS1} = encode_plain_text(type,
                                                                                     text,
                                                                                     cS0)
                                                       {cS1, [enc | encoded]}
                                                  end,
  ...
  def decode_cipher_text(r_ssl_tls(type: 23, version: {3, 3},
           ...
           %{current_read:
             %{sequence_number: seq,
                 ...
                 early_data_accepted:
                 earlyDataAccepted} = readState0} = connectionStates0) do
    case (decipher_aead(cipherFragment, bulkCipherAlgo, key,
                          seq, iV, tagLen)) do
      r_alert()
          when trialDecryption === true and earlyDataAccepted === false and pendingMaxEarlyDataSize0 > 0
               ->
        ignore_early_data(connectionStates0, readState0,
                            pendingMaxEarlyDataSize0, bulkCipherAlgo,
                            cipherFragment)
      r_alert() = alert ->
        alert
      plainFragment0
      ...
      plainFragment0 ->
        ...
        connectionStates = Map.put(connectionStates0, :current_read,
                                                        Map.put(readState0, :sequence_number,
                                                                              seq + 1))
        {decode_inner_plaintext(plainFragment),
           connectionStates}
    end
  end

  def decode_cipher_text(r_ssl_tls(type: 21, version: {3, 3},
             fragment: <<2, 47>>),
           connectionStates0) do
    {r_ssl_tls(type: 21, version: {3, 4}, fragment: <<2, 47>>),
       connectionStates0}
  end
  ...
  def decode_cipher_text(r_ssl_tls(type: 21, version: {3, 3},
             fragment: <<2, 90>>),
           connectionStates0) do
    {r_ssl_tls(type: 21, version: {3, 4}, fragment: <<2, 90>>),
       connectionStates0}
  end

  def decode_cipher_text(r_ssl_tls(type: 20, version: {3, 3}, fragment: <<1>>),
           connectionStates0) do
    {r_ssl_tls(type: 20, version: {3, 4}, fragment: <<1>>),
       connectionStates0}
  end

  def decode_cipher_text(r_ssl_tls(type: type, version: {3, 3},
             fragment: cipherFragment),
           %{current_read:
             %{security_parameters:
               r_security_parameters(cipher_suite: <<0
                                 ::
                                 size(8) - unsigned - big - integer,
                                   0
                                   ::
                                   size(8) - unsigned - big -
                                     integer>>)}} = connnectionStates0) do
    {r_ssl_tls(type: type, version: {3, 4},
         fragment: cipherFragment),
       connnectionStates0}
  end
  ...
  def decode_cipher_text(r_ssl_tls(type: type), _) do
    r_alert(level: 2, description: 20,
        where: %{mfa: {:tls_record_1_3, :decode_cipher_text, 2},
                   line: 213, file: 'otp/lib/ssl/src/tls_record_1_3.erl'},
        reason: {:record_type_mismatch, type})
  end

  defp ignore_early_data(connectionStates0, readState0,
            pendingMaxEarlyDataSize0, bulkCipherAlgo,
            cipherFragment) do
    pendingMaxEarlyDataSize = approximate_pending_early_data_size(pendingMaxEarlyDataSize0,
                                                                    ...
                                                                    cipherFragment)
    connectionStates = Map.put(connectionStates0, :current_read,
                                                    Map.put(readState0, :pending_early_data_size,
                                                                          pendingMaxEarlyDataSize))
    cond do
      pendingMaxEarlyDataSize < 0 ->
        r_alert(level: 2, description: 20,
            where: %{mfa: {:tls_record_1_3, :ignore_early_data, 5},
                       line: 230, file: 'otp/lib/ssl/src/tls_record_1_3.erl'},
  ...
  defp process_early_data(connectionStates0, readState0,
            pendingMaxEarlyDataSize0, seq, plainFragment) do
    record = decode_inner_plaintext(plainFragment)
    case ({r_ssl_tls(record, :type),
             remove_padding(r_ssl_tls(record, :fragment))}) do
      {22, <<5>>} ->
        connectionStates = Map.put(connectionStates0, :current_read,
                                                        Map.put(readState0, :sequence_number,
                                                                              seq + 1))
      ...
      {23, data} ->
        pendingMaxEarlyDataSize = pending_early_data_size(pendingMaxEarlyDataSize0,
                                                            data)
        cond do
          pendingMaxEarlyDataSize < 0 ->
            r_alert(level: 2, description: 10,
                where: %{mfa: {:tls_record_1_3, :process_early_data, 5},
                           line: 252, file: 'otp/lib/ssl/src/tls_record_1_3.erl'},
                reason: {:too_much_early_data,
                           {:max_early_data_threshold_exceeded,
                              pendingMaxEarlyDataSize}})
          true ->
            connectionStates = Map.put(connectionStates0, :current_read,
                                                            Map.merge(readState0, %{sequence_number:
                                                                                    seq + 1,
  ...
  defp encode_plain_text(r_inner_plaintext(content: data, type: type, zeros: zeros),
            %{cipher_state: r_cipher_state(key: key, iv: iV, tag_len: tagLen),
                sequence_number: seq,
                security_parameters:
                r_security_parameters(cipher_type: 2,
                    bulk_cipher_algorithm: bulkCipherAlgo)}) do
    plainText = [data, type, zeros]
    encoded = cipher_aead(plainText, bulkCipherAlgo, key,
                            seq, iV, tagLen)
    r_tls_cipher_text(opaque_type: 23, legacy_version: {3, 3},
        encoded_record: encoded)
  end

  defp encode_plain_text(r_inner_plaintext(content: data, type: type),
            %{security_parameters:
              r_security_parameters(cipher_suite: <<0
                                ::
                                size(8) - unsigned - big - integer,
                                  0
                                  ::
                                  size(8) - unsigned - big - integer>>)}) do
    r_tls_cipher_text(opaque_type: type, legacy_version: {3, 3},
        encoded_record: data)
  end
  ...
  defp nonce(seq, iV) do
    ...
    :crypto.exor(<<padding :: binary,
                     seq :: size(64) - unsigned - big - integer>>,
                   iV)
  end

  defp cipher_aead(fragment, bulkCipherAlgo, key, seq, iV,
            tagLen) do
    aAD = additional_data(:erlang.iolist_size(fragment) + tagLen)
    nonce = nonce(seq, iV)
    {content,
       cipherTag} = :ssl_cipher.aead_encrypt(bulkCipherAlgo,
                                               key, nonce, fragment, aAD,
                                               tagLen)
    <<content :: binary, cipherTag :: binary>>
  end

  defp encode_tls_cipher_text(r_tls_cipher_text(opaque_type: type, legacy_version: version,
              encoded_record: encoded),
            %{sequence_number: seq} = write) do
  ...
  defp decipher_aead(cipherFragment, bulkCipherAlgo, key, seq, iV,
            tagLen) do
    try do
      aAD = additional_data(:erlang.iolist_size(cipherFragment))
      nonce = nonce(seq, iV)
      {cipherText,
         cipherTag} = aead_ciphertext_split(cipherFragment,
                                              tagLen)
      case (:ssl_cipher.aead_decrypt(bulkCipherAlgo, key,
                                       nonce, cipherText, cipherTag, aAD)) do
        ...
        reason ->
                                      ...
                                      %{description: :decrypt_error,
                                          ...
                                          [{:reason, reason}, {:stacktrace,
                                                                 :erlang.process_info(self(),
                                                                                        :current_stacktrace)}]},
                                      %{mfa:
                                        {:tls_record_1_3, :decipher_aead, 6},
                                          line: 351, file: 'otp/lib/ssl/src/tls_record_1_3.erl'})
                  __LogLevel__ ->
          ...
          r_alert(level: 2, description: 20,
              where: %{mfa: {:tls_record_1_3, :decipher_aead, 6},
                         line: 352, file: 'otp/lib/ssl/src/tls_record_1_3.erl'},
              reason: :decryption_failed)
      end
    catch
      _, reason2 ->
                ...
                :undefined ->
                  :ssl_logger.log(:debug, :debug,
                                    ...
                                    %{mfa: {:tls_record_1_3, :decipher_aead, 6},
                                        line: 356, file: 'otp/lib/ssl/src/tls_record_1_3.erl'})
                __LogLevel__ ->
                  :ssl_logger.log(:debug, __LogLevel__,
                                    %{description: :decrypt_error,
                                        reason:
        ...
        r_alert(level: 2, description: 20,
            where: %{mfa: {:tls_record_1_3, :decipher_aead, 6},
                       line: 357, file: 'otp/lib/ssl/src/tls_record_1_3.erl'},
            reason: :decryption_failed)
    end
  end
  ...
  defp aead_ciphertext_split(cipherTextFragment, tagLen)
    ...
    <<cipherText :: size(cipherLen) - bytes,
        cipherTag :: size(tagLen) - bytes>> = cipherTextFragment
  ...
  defp aead_ciphertext_split(cipherTextFragment, tagLen)
    ...
    <<cipherText :: size(cipherLen) - bytes,
        ...
        ::
        size(tagLen) -
          bytes>> = :erlang.iolist_to_binary(cipherTextFragment)
    {cipherText, cipherTag}
  end
  ...
  defp decode_inner_plaintext(plainText) do
    case (:binary.last(plainText)) do
      ...
      _Else ->
        r_alert(level: 2, description: 10,
            where: %{mfa:
                     {:tls_record_1_3, :decode_inner_plaintext, 1},
                       line: 382, file: 'otp/lib/ssl/src/tls_record_1_3.erl'},
            reason: :empty_alert)
    end
  end
  ...
  defp approximate_pending_early_data_size(pendingMaxEarlyDataSize, bulkCipherAlgo,
            cipherFragment) do
    innerContTypeLen = 1
    pendingMaxEarlyDataSize - (byte_size(cipherFragment) - innerContTypeLen - bca_tag_len(bulkCipherAlgo))
  end
