defmodule :m_tls_client_connection_1_3 do
  ...
  def init([:client, sender, host, port, socket, options,
                                                   user, cbInfo]) do
    state0 = (r_state(protocol_specific: map) = :tls_gen_connection_1_3.initial_state(:client,
                                                                                  sender,
                                                                                  host,
                                                                                  port,
                                                                                  socket,
    ...
    try do
      ...
      :gen_statem.enter_loop(:tls_client_connection_1_3, [],
                               :initial_hello, state)
    catch
      error ->
    ...
    :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                         ...
                                         where: %{mfa:
                                                  {:tls_client_connection_1_3,
                                                     :user_hello, 3},
                                                    line: 177, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'},
  ...
  def user_hello({:call, from},
           ...
           r_state(handshake_env: r_handshake_env(continue_status: :pause) = hSEnv,
      ...
      {:error, reason} ->
        ...
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_client_connection_1_3,
                                                         :user_hello, 3},
                                                        line: 195, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'},
                                             reason: reason),
                                           :user_hello, state0)
    else
      options ->
        state = :ssl_gen_statem.ssl_config(options, :client,
                                             state0)
        {:next_state, :wait_sh,
  ...
  def start(:internal,
    ...
    case (:tls_record.is_acceptable_version(version,
      ...
      true ->
        handle_exlusive_1_3_hello_or_hello_retry_request(serverHello,
                                                           state)
      false ->
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             description: 70,
                                             where: %{mfa:
                                                      {:tls_client_connection_1_3,
                                                         :start, 3},
                                                        line: 224, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'}),
                                           :start, state)
    end
  end
      ...
      false ->
        :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                             ...
                                             where: %{mfa:
                                                      {:tls_client_connection_1_3,
                                                         :start, 3},
                                                        line: 243, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'}),
                                           :start, state)
    end
  end
  ...
  def start(:internal, r_server_hello(), state0) do
    :ssl_gen_statem.handle_own_alert(r_alert(level: 2,
                                         ...
                                         where: %{mfa:
                                                  {:tls_client_connection_1_3,
                                                     :start, 3},
                                                    line: 253, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'}),
                                       :start, state0)
  end
  ...
  def wait_sh(:internal, r_server_hello() = hello,
      ...
      when not
           :erlang.is_map_key(:middlebox_comp_mode, sSLOpts) do
  ...
  def hello_middlebox_assert(:internal = type, r_encrypted_extensions() = msg,
           r_state(ssl_options: %{log_level: level}) = state) do
    :ssl_logger.log(:warning, level,
                      %{description: 'Failed to assert middlebox server message', reason: [{:missing, r_change_cipher_spec()}]},
                      %{mfa:
                        {:tls_client_connection_1_3, :hello_middlebox_assert,
                           3},
                          line: 321, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'})
  ...
  def hello_retry_middlebox_assert(:internal = type, r_server_hello() = msg,
           r_state(ssl_options: %{log_level: level}) = state) do
    :ssl_logger.log(:warning, level,
                      %{description: 'Failed to assert middlebox server message', reason: [{:missing, r_change_cipher_spec()}]},
                      %{mfa:
                        {:tls_client_connection_1_3,
                          ...
                          line: 340, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'})
    :ssl_gen_statem.handle_common_event(type, msg,
  ...
  def wait_ee(:internal, r_encrypted_extensions(extensions: extensions), state0) do
    case (handle_encrypted_extensions(extensions,
                                        state0)) do
      r_alert() = alert ->
  ...
  def wait_cert_cr(:internal, r_certificate_request_1_3() = certificateRequest, state0) do
    case (handle_certificate_request(certificateRequest,
                                       state0)) do
      r_alert() = alert ->
        :ssl_gen_statem.handle_own_alert(alert, :wait_cert_cr,
                                           state0)
  ...
  def wait_cv(:internal, r_certificate_verify_1_3() = certificateVerify, state0) do
    ...
    try do
      {state,
         nextState} = maybe.(:tls_handshake_1_3.verify_certificate_verify(state0,
                                                                            certificateVerify))
      :tls_gen_connection.next_event(nextState, :no_record,
                                       state)
    catch
  ...
  def wait_finished(:internal, r_finished(verify_data: verifyData),
    ...
    try do
      ...
      {record,
         r_state(protocol_specific: pS) = state} = :ssl_gen_statem.prepare_connection(state9,
                                                                                  :tls_gen_connection)
  ...
  defp do_handle_exlusive_1_3_hello_or_hello_retry_request(r_server_hello(cipher_suite: selectedCipherSuite,
              random: random, session_id: sessionId,
              extensions: extensions),
            r_state(static_env: r_static_env(host: host, port: port,
                              cert_db: certDbHandle, cert_db_ref: certDbRef,
                              protocol_cb: connection, transport_cb: transport,
                              socket: socket),
    ...
    try do
      ...
      hello = :tls_handshake_1_3.maybe_add_binders(hello1,
                                                     hHistory0, ticketData,
                                                     negotiatedVersion)
      {binMsg0, connectionStates,
         hHistory} = connection.encode_handshake(hello,
                                                   negotiatedVersion,
                                                   connectionStates0, hHistory0)
      {r_state(handshake_env: hsEnv) = state3,
         binMsg} = :tls_gen_connection_1_3.maybe_prepend_change_cipher_spec(state2,
                                                                              binMsg0)
      :tls_socket.send(transport, socket, binMsg)
      :ssl_logger.debug(logLevel, :outbound, :handshake,
                          hello)
      :ssl_logger.debug(logLevel, :outbound, :record, binMsg)
      state = r_state(state3, connection_states: connectionStates, 
      ...
      case (:maps.get(:hello_retry, pS,
                        false) and :maps.get(:middlebox_comp_mode, sslOpts,
                                               true)) do
        true ->
  ...
  defp handle_server_hello(r_server_hello(cipher_suite: selectedCipherSuite,
              random: random, session_id: sessionId,
              extensions: extensions) = serverHello,
            r_state(key_share: clientKeyShare,
                ssl_options: %{ciphers: clientCiphers,
                                 supported_groups: clientGroups0,
                                 session_tickets: sessionTickets,
                                 use_ticket: useTicket}) = state0) do
    {ref, maybe} = :tls_gen_connection_1_3.do_maybe()
    try do
      clientGroups = maybe.(:tls_handshake_1_3.get_supported_groups(clientGroups0))
      serverKeyShare = server_share(:maps.get(:key_share,
                                                extensions))
      serverPreSharedKey = :maps.get(:pre_shared_key,
                                       extensions, :undefined)
      maybe.(:tls_handshake_1_3.maybe_hello_retry_request(serverHello,
                                                            state0))
      state1 = :tls_gen_connection_1_3.handle_resumption(state0,
                                                           serverPreSharedKey)
      maybe.(validate_cipher_suite(selectedCipherSuite,
                                     clientCiphers))
      maybe.(validate_server_key_share(clientGroups,
                                         serverKeyShare))
      r_key_share_entry(group: selectedGroup,
          key_exchange: serverPublicKey) = serverKeyShare
      clientPrivateKey = client_private_key(selectedGroup,
      ...
      pSK = maybe.(:tls_handshake_1_3.get_pre_shared_key(sessionTickets,
                                                           useTicket, hKDFAlgo,
                                                           serverPreSharedKey))
      state3 = :tls_handshake_1_3.calculate_handshake_secrets(serverPublicKey,
                                                                clientPrivateKey,
                                                                selectedGroup,
                                                                pSK, state2)
  ...
  defp handle_encrypted_extensions(extensions, state0) do
    ...
    try do
      ...
      state1 = maybe_check_early_data_indication(earlyDataIndication,
                                                   state0)
  ...
  defp handle_certificate(r_certificate_1_3() = certificate, state0) do
    {ref, maybe} = :tls_gen_connection_1_3.do_maybe()
    try do
      maybe.(:tls_handshake_1_3.process_certificate(certificate,
                                                      state0))
    catch
      {^ref, r_alert() = alert} ->
        {alert, state0}
  ...
  defp handle_certificate_request(r_certificate_request_1_3() = certificateRequest, state0) do
    {ref, maybe} = :tls_gen_connection_1_3.do_maybe()
    try do
      maybe.(:tls_handshake_1_3.process_certificate_request(certificateRequest,
                                                              state0))
    catch
      {^ref, r_alert() = alert} ->
        {alert, state0}
  ...
  def maybe_send_early_data(r_state(handshake_env: r_handshake_env(tls_handshake_history: {hist,
                                                      _}),
             protocol_specific: %{sender: _Sender},
             ssl_options: %{versions: [{3, 4} | _],
                              use_ticket: useTicket,
                              session_tickets: sessionTickets,
                              early_data: earlyData} = _SslOpts0) = state0)
    ...
    case (:tls_handshake_1_3.get_pre_shared_key_early_data(sessionTickets,
      ...
      {:ok, {pSK, cipher, hKDF, maxSize}}
        ...
        state2 = :tls_handshake_1_3.calculate_client_early_traffic_secret(hist,
                                                                            pSK,
                                                                            cipher,
                                                                            hKDF,
                                                                            state1)
        ...
        {:ok,
           :tls_handshake_1_3.encode_early_data(cipher, state3)}
      {:ok, {_, _, _, maxSize}} ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_client_connection_1_3, :maybe_send_early_data, 1},
                          line: 777, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'},
  ...
  defp maybe_send_end_of_early_data(r_state(handshake_env: r_handshake_env(early_data_accepted: true),
              protocol_specific: %{sender: _Sender},
              ssl_options: %{versions: [{3, 4} | _],
  ...
  def maybe_automatic_session_resumption(r_state(ssl_options: %{versions: [version | _],
                            ciphers: userSuites, early_data: earlyData,
                            session_tickets: sessionTickets,
                            server_name_indication: sNI} = sslOpts0) = state0)
      when version >= {3, 4} and sessionTickets === :auto do
    availableCipherSuites = :ssl_handshake.available_suites(userSuites,
                                                              version)
    hashAlgos = cipher_hash_algos(availableCipherSuites)
    ciphers = :tls_handshake_1_3.ciphers_for_early_data(availableCipherSuites)
    earlyDataSize = :tls_handshake_1_3.early_data_size(earlyData)
    keyPair = :tls_client_ticket_store.find_ticket(self(),
                                                     ciphers, hashAlgos, sNI,
                                                     earlyDataSize)
    useTicket = :tls_handshake_1_3.choose_ticket(keyPair,
                                                   earlyData)
    :tls_client_ticket_store.lock_tickets(self(),
                                            [useTicket])
    state = r_state(state0, ssl_options: Map.put(sslOpts0, :use_ticket,
                                                       [useTicket]))
    {[useTicket], state}
  end

  def maybe_automatic_session_resumption(r_state(ssl_options: %{use_ticket:
                          useTicket}) = state) do
    {useTicket, state}
  end
  ...
  defp maybe_check_early_data_indication(earlyDataIndication,
            r_state(handshake_env: hsEnv,
                ssl_options: %{versions: [{3, 4} | _],
                                 use_ticket: useTicket,
                                 early_data: earlyData}) = state)
      when useTicket !== [:undefined] and earlyData !== :undefined and earlyDataIndication !== :undefined do
    signal_user_early_data(state, :accepted)
    r_state(state, handshake_env: r_handshake_env(hsEnv, early_data_accepted: true))
  end

  defp maybe_check_early_data_indication(earlyDataIndication,
            r_state(protocol_specific: %{sender: _Sender},
                ssl_options: %{versions: [{3, 4} | _],
                                 use_ticket: useTicket,
                                 early_data: earlyData} = _SslOpts0) = state)
      when useTicket !== [:undefined] and earlyData !== :undefined and earlyDataIndication === :undefined do
  ...
  defp signal_user_early_data(r_state(connection_env: r_connection_env(user_application: {_,
                                                   user}),
              static_env: r_static_env(socket: socket, protocol_cb: connection,
                              transport_cb: transport,
                              trackers: trackers)) = state,
            result) do
    cPids = connection.pids(state)
    sslSocket = connection.socket(cPids, transport, socket,
                                    trackers)
    send(user, {:ssl, sslSocket, {:early_data, result}})
  end

  defp maybe_max_fragment_length(extensions, state) do
    serverMaxFragEnum = :maps.get(:max_frag_enum,
                                    extensions, :undefined)
    clientMaxFragEnum = :ssl_handshake.max_frag_enum(:maps.get(:max_fragment_length,
    ...
    cond do
      ...
      true ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_client_connection_1_3, :maybe_max_fragment_length,
                           2},
                          line: 916, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'})}
    end
  end
  ...
  defp cipher_hash_algos(ciphers) do
    fun = fn cipher ->
               %{prf:
                 hash} = :ssl_cipher_format.suite_bin_to_map(cipher)
               hash
          end
  ...
  defp maybe_queue_cert_cert_cv(r_state(connection_states: _ConnectionStates0,
              session: r_session(session_id: _SessionId,
                           own_certificates: ownCerts),
              ssl_options: %{} = _SslOpts, key_share: _KeyShare,
              handshake_env: r_handshake_env(tls_handshake_history: _HHistory0),
              static_env: r_static_env(protocol_cb: connection,
                              cert_db: certDbHandle, cert_db_ref: certDbRef,
                              socket: _Socket,
                              transport_cb: _Transport)) = state0) do
    {ref, maybe} = :tls_gen_connection_1_3.do_maybe()
    try do
      ...
      state = maybe.(maybe_queue_cert_verify(certificate,
                                               state1))
      {:ok, state}
    catch
      {^ref, r_alert() = alert} ->
  ...
  defp maybe_queue_cert_verify(_Certificate,
            r_state(connection_states: _ConnectionStates0,
    ...
    {ref, maybe} = :tls_gen_connection_1_3.do_maybe()
    try do
      certificateVerify = maybe.(:tls_handshake_1_3.certificate_verify(certPrivateKey,
                                                                         signatureScheme,
                                                                         state,
                                                                         :client))
      {:ok,
  ...
  defp validate_server_key_share([], _) do
    {:error,
       r_alert(level: 2, description: 47,
           where: %{mfa:
                    {:tls_client_connection_1_3, :validate_server_key_share,
                       1},
                      line: 992, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'})}
  end
  ...
  defp validate_selected_group(selectedGroup, [selectedGroup | _]) do
    {:error,
       r_alert(level: 2, description: 47,
           where: %{mfa:
                    {:tls_client_connection_1_3, :validate_selected_group,
                       2},
                      line: 1000, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'},
           reason: 'Selected group sent by the server shall not correspond to a group which was provided in the key_share extension')}
  end
        ...
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_client_connection_1_3, :validate_selected_group,
                           2},
                          line: 1008, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'},
               reason: 'Selected group sent by the server shall correspond to a group which was provided in the supported_groups extension')}
    end
  end
    ...
    case (:lists.member(cipher, clientCiphers)) do
      ...
      false ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_client_connection_1_3, :validate_cipher_suite, 2},
                          line: 1021, file: 'otp/lib/ssl/src/tls_client_connection_1_3.erl'})}
    end
  end
