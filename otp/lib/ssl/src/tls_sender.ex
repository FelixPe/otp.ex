defmodule :m_tls_sender do
  ...
  def connection(
    ...
    :ok =
      :ssl_gen_statem.dist_handshake_complete(
        pid,
        dHandle
      )
  ...
  def connection({:call, from}, :get_application_traffic_secret, state) do
    currentWrite =
      :maps.get(
        :current_write,
    ...
    hibernate_after(:connection, state, [{:reply, from, {:ok, applicationTrafficSecret}}])
  end
  ...
  def connection(:info, :dist_data, r_data(static: r_static(dist_handle: dHandle)) = stateData) do
    case dist_data(dHandle) do
      [] ->
  ...
  defp handle_common(
         ...
         r_data(static: r_static(erl_dist: true)) = stateData
       ) do
  ...
  defp handle_common(_StateName, :info, {:EXIT, _Sup, :shutdown}, stateData) do
  ...
  defp handle_common(
         ...
         :info,
         msg,
         r_data(static: r_static(log_level: level)) = stateData
       ) do
    :ssl_logger.log(
      :info,
      level,
      %{event: ~c"TLS sender received unexpected info", reason: [{:message, msg}]},
      %{
        mfa: {:tls_sender, :handle_common, 4},
        line: 475,
        file: ~c"otp/lib/ssl/src/tls_sender.erl"
      }
    )
  ...
  defp handle_common(stateName, type, msg, r_data(static: r_static(log_level: level)) = stateData) do
    :ssl_logger.log(
      :error,
      level,
      %{
        event: ~c"TLS sender received unexpected event",
  ...
  defp send_application_data(
    ...
    case time_to_rekey(version, data, connectionStates0, renegotiateAt, keyUpdateAt, bytesSent) do
      ...
      :renegotiate ->
        :tls_dtls_connection.internal_renegotiation(
          pid,
          connectionStates0
        )
  ...
  defp maybe_update_cipher_key(
    ...
    connectionStates =
      :tls_gen_connection_1_3.update_cipher_key(
        :current_write,
        connectionStates0
      )
  ...
  def handle_trace(
        ...
        {:call,
         {:tls_sender, :time_to_rekey,
          [_Version, data, map, _RenegotiateAt, keyUpdateAt, bytesSent]}},
        stack
      ) do
    ...
    {:io_lib.format(
       ~c"~w) (BytesSent:~w + DataSize:~w) > KeyUpdateAt:~w",
  ...
  def handle_trace(
        :kdt,
        {:call, {:tls_sender, :send_post_handshake_data, [{:key_update, :update_requested} | _]}},
        stack
      ) do
    {:io_lib.format(~c"KeyUpdate procedure 1/4 - update_requested sent", []), stack}
  end

  def handle_trace(
        :kdt,
        {:call,
         {:tls_sender, :send_post_handshake_data, [{:key_update, :update_not_requested} | _]}},
        stack
      ) do
    {:io_lib.format(~c"KeyUpdate procedure 3/4 - update_not_requested sent", []), stack}
  end

  def handle_trace(
        :hbn,
        {:call, {:tls_sender, :connection, [:timeout, :hibernate | _]}},
        stack
      ) do
  ...
  def handle_trace(
        :hbn,
        {:call, {:tls_sender, :hibernate_after, [_StateName = :connection, state, actions]}},
        stack
      ) do
    r_data(static: r_static(hibernate_after: hibernateAfter)) = state
  ...
  def handle_trace(
        :hbn,
        {:return_from, {:tls_sender, :hibernate_after, 3}, {cmd, arg, _State, actions}},
        stack
      ) do
  ...
  def handle_trace(
    ...
    {pid,
     %{
       ...
       erl_dist: isErlDist,
       trackers: trackers,
       negotiated_version: _Version
     }} = opts

    {:io_lib.format(
       ~c"(*~w) Type = ~w Pid = ~w Trackers = ~w Dist = ~w KeyUpdateAt = ~w",
