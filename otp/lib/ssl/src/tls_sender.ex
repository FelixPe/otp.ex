defmodule :m_tls_sender do
  ...
  Record.defrecord(:r_static, :static, connection_pid: :undefined,
                                  role: :undefined, socket: :undefined,
                                  socket_options: :undefined,
                                  erl_dist: :undefined, trackers: :undefined,
  ...
  Record.defrecord(:r_data, :data, static: :EFE_TODO_NESTED_RECORD,
                                connection_states: %{})
  def start_link() do
    :gen_statem.start_link(:tls_sender, [], [])
  end

  def start_link(spawnOpts) do
    :gen_statem.start_link(:tls_sender, [], spawnOpts)
  end
  ...
  def init({:call, from},
           {pid,
              %{current_write: writeState, role: role, socket: socket,
                  socket_options: sockOpts, erl_dist: isErlDist,
                  trackers: trackers, transport_cb: transport,
                  negotiated_version: version,
                  renegotiate_at: renegotiateAt,
                  key_update_at: keyUpdateAt, log_level: logLevel,
                  hibernate_after: hibernateAfter}},
           r_data(connection_states: connectionStates,
               static: static0) = stateData0) do
    stateData = r_data(stateData0, connection_states: Map.put(connectionStates, :current_write,
                                                                             writeState), 
                                static: r_static(static0, connection_pid: pid, 
                                                     role: role, 
                                                     socket: socket, 
  ...
  def init(:info = type, msg, stateData) do
  ...
  def connection({:call, from}, {:application_data, appData},
           r_data(static: r_static(socket_options: r_socket_options(packet: packet))) = stateData) do
    case (encode_packet(packet, appData)) do
      {:error, _} = error ->
        {:next_state, :connection, stateData,
           [{:reply, from, error}]}
      data ->
        send_application_data(data, from, :connection,
                                stateData)
    end
  end

  def connection({:call, from}, {:post_handshake_data, hSData},
           stateData) do
    send_post_handshake_data(hSData, from, :connection,
                               stateData)
  end

  def connection({:call, from}, {:ack_alert, r_alert() = alert},
           stateData0) do
  ...
  def connection({:call, from}, :renegotiate,
           r_data(connection_states: %{current_write:
                                  write}) = stateData) do
    {:next_state, :handshake, stateData,
       [{:reply, from, {:ok, write}}]}
  end

  def connection({:call, from}, :downgrade,
           r_data(connection_states: %{current_write:
                                  write}) = stateData) do
  ...
  def connection({:call, from}, :dist_get_tls_socket,
           r_data(static: r_static(transport_cb: transport, socket: socket,
                         connection_pid: pid,
                         trackers: trackers)) = stateData) do
    tLSSocket = :tls_gen_connection.socket([pid, self()],
                                             transport, socket, trackers)
    hibernate_after(:connection, stateData,
                      [{:reply, from, {:ok, tLSSocket}}])
  end

  def connection({:call, from},
           {:dist_handshake_complete, _Node, dHandle},
           r_data(static: r_static(connection_pid: pid) = static) = stateData) do
    false = :erlang.dist_ctrl_set_opt(dHandle, :get_size,
                                        true)
    :ok = :erlang.dist_ctrl_input_handler(dHandle, pid)
    :ok = :ssl_gen_statem.dist_handshake_complete(pid,
                                                    dHandle)
    :erlang.process_flag(:priority, :normal)
    case (dist_data(dHandle)) do
      [] ->
        hibernate_after(:connection,
                          r_data(stateData, static: r_static(static, dist_handle: dHandle)),
                          [{:reply, from, :ok}])
      data ->
        {:keep_state,
           r_data(stateData, static: r_static(static, dist_handle: dHandle)),
           [{:reply, from, :ok}, {:next_event, :internal,
                                    {:application_packets, {self(), :undefined},
                                       data}}]}
    end
  end

  def connection({:call, from}, :get_application_traffic_secret,
           state) do
    currentWrite = :maps.get(:current_write,
                               r_data(state, :connection_states))
    securityParams = :maps.get(:security_parameters,
                                 currentWrite)
    applicationTrafficSecret = r_security_parameters(securityParams, :application_traffic_secret)
    hibernate_after(:connection, state,
                      [{:reply, from, {:ok, applicationTrafficSecret}}])
  end

  def connection(:internal, {:application_packets, from, data},
           stateData) do
    send_application_data(data, from, :connection,
                            stateData)
  end

  def connection(:internal, {:post_handshake_data, from, hSData},
           stateData) do
    send_post_handshake_data(hSData, from, :connection,
                               stateData)
  end
  ...
  def connection(:cast, {:new_write, writesState, version},
           r_data(connection_states: connectionStates,
               static: static) = stateData) do
    hibernate_after(:connection,
                      r_data(stateData, connection_states: Map.put(connectionStates, :current_write,
                                                                                  writesState), 
                                     static: r_static(static, negotiated_version: version)),
                      [])
  end

  def connection(:info, :dist_data,
           r_data(static: r_static(dist_handle: dHandle)) = stateData) do
    case (dist_data(dHandle)) do
      [] ->
        hibernate_after(:connection, stateData, [])
      data ->
  ...
  def connection(:timeout, :hibernate, _StateData) do
  ...
  def handshake(:cast, {:new_write, writeState, version},
           r_data(connection_states: connectionStates,
               static: r_static(key_update_at: keyUpdateAt0) = static) = stateData) do
    keyUpdateAt = key_update_at(version, writeState,
                                  keyUpdateAt0)
  ...
  def death_row(:state_timeout, reason, _StateData) do
    {:stop, {:shutdown, reason}}
  end

  def death_row(:info = type, msg, stateData) do
    handle_common(:death_row, type, msg, stateData)
  end

  def death_row(_Type, _Msg, _StateData) do
    :keep_state_and_data
  end

  defp death_row_shutdown(reason, stateData) do
    {:next_state, :death_row, stateData,
  ...
  defp handle_set_opts(stateName, from, opts,
            r_data(static: r_static(socket_options: sockOpts) = static) = stateData) do
    hibernate_after(stateName,
                      r_data(stateData, static: r_static(static, socket_options: set_opts(sockOpts,
                                                                                opts))),
                      [{:reply, from, :ok}])
  end

  defp handle_common(stateName, {:call, from}, {:set_opts, opts},
            r_data(static: r_static(socket_options: sockOpts) = static) = stateData) do
    hibernate_after(stateName,
                      r_data(stateData, static: r_static(static, socket_options: set_opts(sockOpts,
                                                                                opts))),
  ...
  defp handle_common(_StateName, :info,
            {:EXIT, _Sup, :shutdown = reason},
            r_data(static: r_static(erl_dist: true)) = stateData) do
    death_row_shutdown(reason, stateData)
  end

  defp handle_common(_StateName, :info, {:EXIT, _Dist, reason},
            r_data(static: r_static(erl_dist: true)) = stateData) do
    {:stop, {:shutdown, reason}, stateData}
  end

  defp handle_common(_StateName, :info, {:EXIT, _Sup, :shutdown},
            stateData) do
    {:stop, :shutdown, stateData}
  end

  defp handle_common(stateName, :info, msg,
            r_data(static: r_static(log_level: level)) = stateData) do
    :ssl_logger.log(:info, level,
                      %{event: 'TLS sender received unexpected info', reason: [{:message, msg}]},
                      %{mfa: {:tls_sender, :handle_common, 4}, line: 475,
                          file: 'otp/lib/ssl/src/tls_sender.erl'})
    hibernate_after(stateName, stateData, [])
  end

  defp handle_common(stateName, type, msg,
            r_data(static: r_static(log_level: level)) = stateData) do
    :ssl_logger.log(:error, level,
                      %{event: 'TLS sender received unexpected event', reason: [{:type, type}, {:message, msg}]},
                      %{mfa: {:tls_sender, :handle_common, 4}, line: 480,
                          file: 'otp/lib/ssl/src/tls_sender.erl'})
    hibernate_after(stateName, stateData, [])
  end

  defp send_tls_alert(r_alert() = alert,
            r_data(static: r_static(negotiated_version: version, socket: socket,
                          transport_cb: transport, log_level: logLevel),
                connection_states: connectionStates0) = stateData0) do
    {binMsg,
       connectionStates} = :tls_record.encode_alert_record(alert,
                                                             version,
                                                             connectionStates0)
  ...
  defp send_application_data(data, from, stateName,
            r_data(static: r_static(connection_pid: pid, socket: socket,
                          dist_handle: distHandle, negotiated_version: version,
                          transport_cb: transport,
                          renegotiate_at: renegotiateAt,
                          key_update_at: keyUpdateAt, bytes_sent: bytesSent,
                          log_level: logLevel),
                connection_states: connectionStates0) = stateData0) do
    case (time_to_rekey(version, data, connectionStates0,
                          renegotiateAt, keyUpdateAt, bytesSent)) do
      :key_update ->
        keyUpdate = :tls_handshake_1_3.key_update(:update_requested)
        {:keep_state_and_data,
           [{:next_event, :internal,
               {:post_handshake_data, from, keyUpdate}},
                {:next_event, :internal,
                   {:application_packets, from, data}}]}
      :renegotiate ->
        :tls_dtls_connection.internal_renegotiation(pid,
                                                      connectionStates0)
      ...
      :chunk_and_key_update ->
        ...
        {:keep_state_and_data,
           [{:next_event, :internal,
               {:post_handshake_data, from, keyUpdate}},
                {:next_event, :internal,
                   {:application_packets, from, chunk}},
                    {:next_event, :internal,
                       {:application_packets, from, rest}}]}
      false ->
  ...
  defp send_post_handshake_data(handshake, from, stateName,
            r_data(static: r_static(socket: socket, dist_handle: distHandle,
                          negotiated_version: version, transport_cb: transport,
                          log_level: logLevel),
                connection_states: connectionStates0) = stateData0) do
    binHandshake = :tls_handshake.encode_handshake(handshake,
                                                     version)
    {encoded,
       connectionStates} = :tls_record.encode_handshake(binHandshake,
                                                          version,
                                                          connectionStates0)
    ...
    case (:tls_socket.send(transport, socket, encoded)) do
      :ok when distHandle !== :undefined ->
        ...
        stateData = maybe_update_cipher_key(stateData1,
                                              handshake)
  ...
  defp maybe_update_cipher_key(r_data(connection_states: connectionStates0,
              static: static0) = stateData,
            r_key_update()) do
    connectionStates = :tls_gen_connection_1_3.update_cipher_key(:current_write,
                                                                   connectionStates0)
    static = r_static(static0, bytes_sent: 0)
    r_data(stateData, connection_states: connectionStates, 
                   static: static)
  end
  ...
  defp key_update_at({3, 4},
            %{security_parameters: r_security_parameters(bulk_cipher_algorithm: 9)},
            _KeyUpdateAt) do
    :seq_num_wrap
  end

  defp key_update_at({3, 4}, _, keyUpdateAt) do
    keyUpdateAt
  end
  ...
  defp encode_packet(packet, data) do
    ...
    case (packet) do
      ...
      n when n === 1 or n === 2 or n === 4 ->
        {:error,
           {:badarg,
              {:packet_to_large, len, 1 <<< (packet <<< 3) - 1}}}
      _ ->
        data
    end
  end
  ...
  defp time_to_rekey(version, _Data,
            %{current_write:
  ...
  defp time_to_rekey(_, _Data,
            %{current_write: %{sequence_number: num}},
            renegotiateAt, _, _) do
    is_time_to_renegotiate(num, renegotiateAt)
  end

  defp chunk_data(data, size) do
    {chunk,
       rest} = :erlang.split_binary(:erlang.iolist_to_binary(data),
                                      size)
    {[chunk], [rest]}
  end
  ...
  defp hibernate_after(:connection = stateName,
            r_data(static: r_static(hibernate_after: hibernateAfter)) = state,
            actions) do
    {:next_state, stateName, state,
       [{:timeout, hibernateAfter, :hibernate} | actions]}
  end

  defp hibernate_after(stateName, state, actions) do
    {:next_state, stateName, state, actions}
  end

  def handle_trace(:kdt,
           {:call,
              {:tls_sender, :time_to_rekey,
                 [_Version, data, map, _RenegotiateAt, keyUpdateAt,
                                                           bytesSent]}},
           stack) do
    %{current_write: %{sequence_number: sn}} = map
    dataSize = :erlang.iolist_size(data)
    {:io_lib.format('~w) (BytesSent:~w + DataSize:~w) > KeyUpdateAt:~w',
                      [sn, bytesSent, dataSize, keyUpdateAt]),
       stack}
  end

  def handle_trace(:kdt,
           {:call,
              {:tls_sender, :send_post_handshake_data,
                 [{:key_update, :update_requested} | _]}},
           stack) do
  ...
  def handle_trace(:kdt,
           {:call,
              {:tls_sender, :send_post_handshake_data,
                 [{:key_update, :update_not_requested} | _]}},
           stack) do
  ...
  def handle_trace(:hbn,
           {:call,
              {:tls_sender, :connection, [:timeout, :hibernate | _]}},
           stack) do
  ...
  def handle_trace(:hbn,
           {:return_from, {:tls_sender, :hibernate_after, 3},
  ...
  def handle_trace(:rle,
           {:call, {:tls_sender, :init, [type, opts, _StateData]}},
           stack0) do
    {pid,
       %{role: role, socket: _Socket,
           key_update_at: keyUpdateAt, erl_dist: isErlDist,
           trackers: trackers,
