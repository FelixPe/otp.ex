defmodule :m_dtls_record do
  ...
  def next_epoch(%{pending_read: pending,
    ...
    %{states
      ...
      %{pending
        |
        epoch: epoch + 1, replay_window: init_replay_window()}}
  end

  def next_epoch(%{pending_write: pending,
  ...
  def get_connection_state_by_epoch(epoch,
           %{current_write: %{epoch: epoch} = current}, :write) do
    current
  end

  def get_connection_state_by_epoch(epoch, %{saved_write: %{epoch: epoch} = saved},
           :write) do
    saved
  end

  def get_connection_state_by_epoch(epoch,
           %{current_read: %{epoch: epoch} = current}, :read) do
    current
  end

  def get_connection_state_by_epoch(epoch, %{saved_read: %{epoch: epoch} = saved},
           :read) do
    saved
  end

  defp set_connection_state_by_epoch(writeState, epoch,
            %{current_write: %{epoch: epoch}} = states, :write) do
    %{states | current_write: writeState}
  end

  defp set_connection_state_by_epoch(writeState, epoch,
            %{saved_write: %{epoch: epoch}} = states, :write) do
    %{states | saved_write: writeState}
  end

  defp set_connection_state_by_epoch(readState, epoch,
            %{current_read: %{epoch: epoch}} = states, :read) do
    %{states | current_read: readState}
  end

  defp set_connection_state_by_epoch(readState, epoch,
            %{saved_read: %{epoch: epoch}} = states, :read) do
    %{states | saved_read: readState}
  end

  def init_connection_state_seq(version,
           %{current_read: %{epoch: 0, sequence_number: seq},
               current_write:
               %{epoch: 0} = write} = connnectionStates0)
      when :erlang.element(1, version) == 254 do
    Map.put(connnectionStates0, :current_write,
                                  Map.put(write, :sequence_number, seq))
  end
  ...
  def get_dtls_records(data, vinfo, buffer, %{log_level: logLevel}) do
  ...
  def encode_handshake(frag, version, epoch, connectionStates) do
    encode_plain_text(22, version, epoch, frag,
                        connectionStates)
  end
  ...
  def encode_alert_record(r_alert(level: level, description: description),
    ...
    %{epoch:
      epoch} = :ssl_record.current_connection_state(connectionStates,
                                                      :write)
    encode_plain_text(21, version, epoch,
  ...
  def encode_change_cipher_spec(version, epoch, connectionStates) do
    {enc, cs} = encode_plain_text(20, version, epoch,
  ...
  def encode_data(data, version, connectionStates) do
    %{epoch: epoch,
        max_fragment_length:
        maxFragmentLength} = :ssl_record.current_connection_state(connectionStates,
                                                                    :write)
    maxLength = (cond do
    ...
    case (:erlang.iolist_size(data)) do
      n when n > maxLength ->
        frags = :tls_record.split_iovec(:erlang.iolist_to_iovec(data),
                                          maxLength)
        {revCipherText,
           connectionStates1} = :lists.foldl(fn frag, {acc, cS0} ->
                                                  {cipherText,
                                                     cS1} = encode_plain_text(23,
                                                                                version,
                                                                                epoch,
                                                                                frag,
                                                                                cS0)
                                               ...
                                               {[], connectionStates}, frags)
        {:lists.reverse(revCipherText), connectionStates1}
      _ ->
        {enc, cs} = encode_plain_text(23, version, epoch, data,
                                        connectionStates)
        {[enc], cs}
    end
  end
  ...
  def encode_plain_text(type, version, epoch, data, connectionStates) do
    write0 = get_connection_state_by_epoch(epoch,
                                             connectionStates, :write)
    {cipherFragment, write1} = encode_plain_text(type,
                                                   version, data, write0)
    {cipherText, write} = encode_dtls_cipher_text(type,
                                                    version, cipherFragment,
                                                    write1)
    {cipherText,
       set_connection_state_by_epoch(write, epoch,
                                       connectionStates, :write)}
  end

  def decode_cipher_text(r_ssl_tls(epoch: epoch) = cipherText,
           connnectionStates0) do
    readState = get_connection_state_by_epoch(epoch,
                                                connnectionStates0, :read)
    decode_cipher_text(cipherText, readState,
                         connnectionStates0)
  end

  def protocol_version_name(:"dtlsv1.2") do
    {254, 253}
  end

  def protocol_version_name(:dtlsv1) do
    {254, 255}
  end
  ...
  defp supported_protocol_versions([]) do
    vsns = (case (sufficient_dtlsv1_2_crypto_support()) do
              true ->
                [:"dtlsv1.2"]
              false ->
                [:dtlsv1]
  ...
  defp initial_connection_state(connectionEnd, beastMitigation) do
    %{security_parameters:
      :ssl_record.initial_security_params(connectionEnd),
        epoch: :undefined, sequence_number: 0,
        replay_window: init_replay_window(),
  ...
  defp get_dtls_records_aux({dataTag, stateName, _, versions} = vinfo,
            <<type :: size(8) - unsigned - big - integer,
                majVer :: size(8) - unsigned - big - integer,
                minVer :: size(8) - unsigned - big - integer,
                epoch :: size(16) - unsigned - big - integer,
                sequenceNumber :: size(48) - unsigned - big - integer,
                length :: size(16) - unsigned - big - integer,
                data :: size(length) - binary,
                rest :: binary>> = rawDTLSRecord,
            acc0, logLevel)
      when (stateName == :hello or stateName == :certify and dataTag == :udp or stateName == :abbreviated and dataTag == :udp) and (type == 22 or type == 21) do
    :ssl_logger.debug(logLevel, :inbound, :record,
                        [rawDTLSRecord])
    version = {majVer, minVer}
    acc = [r_ssl_tls(type: type, version: version, epoch: epoch,
               sequence_number: sequenceNumber, fragment: data) |
               acc0]
    case (is_acceptable_version(version, versions)) do
      ...
      false ->
        r_alert(level: 2, description: 20,
            where: %{mfa: {:dtls_record, :get_dtls_records_aux, 4},
                       line: 442, file: 'otp/lib/ssl/src/dtls_record.erl'})
    end
  end

  defp get_dtls_records_aux({_, _, version, versions} = vinfo,
            <<type :: size(8) - unsigned - big - integer,
                majVer :: size(8) - unsigned - big - integer,
                minVer :: size(8) - unsigned - big - integer,
                epoch :: size(16) - unsigned - big - integer,
    ...
    cond do
      version1 === version ->
        get_dtls_records_aux(vinfo, rest, acc, logLevel)
      type == 22 ->
        case (is_acceptable_version(version1, versions)) do
          true ->
            get_dtls_records_aux(vinfo, rest, acc, logLevel)
          false ->
            r_alert(level: 2, description: 20,
                where: %{mfa: {:dtls_record, :get_dtls_records_aux, 4},
                           line: 465, file: 'otp/lib/ssl/src/dtls_record.erl'})
        end
      true ->
        r_alert(level: 2, description: 20,
            where: %{mfa: {:dtls_record, :get_dtls_records_aux, 4},
                       line: 468, file: 'otp/lib/ssl/src/dtls_record.erl'})
    end
  end

  defp get_dtls_records_aux(_,
            <<_ :: size(8) - unsigned - big - integer,
                _MajVer :: size(8) - unsigned - big - integer,
                _MinVer :: size(8) - unsigned - big - integer,
                _Epoch :: size(16) - unsigned - big - integer,
                _Seq :: size(48) - unsigned - big - integer,
                length :: size(16) - unsigned - big - integer,
                _ :: binary>>,
            _Acc, _)
    ...
    r_alert(level: 2, description: 22,
        where: %{mfa: {:dtls_record, :get_dtls_records_aux, 4},
                   line: 474, file: 'otp/lib/ssl/src/dtls_record.erl'})
  end

  defp get_dtls_records_aux(_, data, acc, _) do
    case (byte_size(data) <= 16384 + 2048 + 5) do
      true ->
        {:lists.reverse(acc), data}
      false ->
        r_alert(level: 2, description: 10,
            where: %{mfa: {:dtls_record, :get_dtls_records_aux, 4},
                       line: 481, file: 'otp/lib/ssl/src/dtls_record.erl'})
    end
  end

  def init_replay_window() do
    init_replay_window(58)
  end

  defp init_replay_window(size) do
    %{top: size - 1, bottom: 0, mask: 0}
  end

  def replay_detect(r_ssl_tls(sequence_number: sequenceNumber),
           %{replay_window: window}) do
    is_replay(sequenceNumber, window)
  end

  def is_replay(sequenceNumber, %{bottom: bottom})
      when sequenceNumber < bottom do
    true
  end

  def is_replay(sequenceNumber,
           %{top: top, bottom: bottom, mask: mask})
      when bottom <= sequenceNumber and sequenceNumber <= top do
    index = sequenceNumber - bottom
    (mask >>> index) &&& 1 === 1
  end

  def is_replay(_, _) do
    false
  end

  def update_replay_window(sequenceNumber,
           %{replay_window:
             %{top: top, bottom: bottom,
                 mask: mask0} = window0} = connectionStates) do
    noNewBits = sequenceNumber - top
    window = (case (noNewBits > 0) do
                true ->
                  newBottom = bottom + noNewBits
                  index = sequenceNumber - newBottom
                  mask = mask0 >>> noNewBits ||| (1 <<< index)
                  Map.merge(window0, %{top: top + noNewBits,
                                         bottom: newBottom, mask: mask})
                false ->
                  index = sequenceNumber - bottom
                  mask = mask0 ||| (1 <<< index)
                  Map.put(window0, :mask, mask)
              end)
    %{connectionStates | replay_window: window}
  end

  defp encode_dtls_cipher_text(type, version, fragment,
            %{epoch: epoch, sequence_number: seq} = writeState) do
    ...
    {[<<type :: size(8) - unsigned - big - integer,
          ...
          minVer :: size(8) - unsigned - big - integer,
          epoch :: size(16) - unsigned - big - integer,
          seq :: size(48) - unsigned - big - integer,
          length :: size(16) - unsigned - big - integer>>,
          fragment],
       Map.put(writeState, :sequence_number, seq + 1)}
  end

  defp encode_plain_text(type, version, data,
            %{compression_state: compS0, cipher_state: cipherS0,
                epoch: epoch, sequence_number: seq,
                security_parameters:
                r_security_parameters(cipher_type: 2, bulk_cipher_algorithm: bCAlg,
                    compression_algorithm: compAlg)} = writeState0) do
    {comp, compS1} = :ssl_record.compress(compAlg, data,
                                            compS0)
    aAD = start_additional_data(type, version, epoch, seq)
    cipherS = :ssl_record.nonce_seed(bCAlg,
                                       <<epoch
                                         ::
                                         size(16) - unsigned - big - integer,
                                           seq
                                           ::
                                           size(48) - unsigned - big -
                                             integer>>,
                                       cipherS0)
    writeState = Map.merge(writeState0, %{compression_state:
                                          compS1,
                                            cipher_state: cipherS})
    tLSVersion = :dtls_v1.corresponding_tls_version(version)
    :ssl_record.cipher_aead(tLSVersion, comp, writeState,
                              aAD)
  end

  defp encode_plain_text(type, version, fragment,
            %{compression_state: compS0, epoch: epoch,
                sequence_number: seq, cipher_state: cipherS0,
                security_parameters:
                r_security_parameters(compression_algorithm: compAlg,
                    bulk_cipher_algorithm: bulkCipherAlgo)} = writeState0) do
    {comp, compS1} = :ssl_record.compress(compAlg, fragment,
                                            compS0)
    writeState1 = Map.put(writeState0, :compression_state,
                                         compS1)
    mAC = calc_mac_hash(type, version, writeState1, epoch,
                          seq, comp)
    tLSVersion = :dtls_v1.corresponding_tls_version(version)
    {cipherFragment,
       cipherS1} = :ssl_cipher.cipher(bulkCipherAlgo, cipherS0,
                                        mAC, fragment, tLSVersion)
    {cipherFragment,
       Map.put(writeState0, :cipher_state, cipherS1)}
  end

  defp decode_cipher_text(r_ssl_tls(type: type, version: version, epoch: epoch,
              sequence_number: seq,
              fragment: cipherFragment) = cipherText,
            %{compression_state: compressionS0,
                cipher_state: cipherS0,
                security_parameters:
                r_security_parameters(cipher_type: 2, bulk_cipher_algorithm: bulkCipherAlgo,
                    compression_algorithm: compAlg)} = readState0,
            connnectionStates0) do
    aAD = start_additional_data(type, version, epoch, seq)
    cipherS = :ssl_record.nonce_seed(bulkCipherAlgo,
                                       <<epoch
                                         ::
                                         size(16) - unsigned - big - integer,
                                           seq
    ...
    case (:ssl_record.decipher_aead(bulkCipherAlgo, cipherS,
                                      aAD, cipherFragment, tLSVersion)) do
      plainFragment when is_binary(plainFragment) ->
        {plain, compressionS} = :ssl_record.uncompress(compAlg,
                                                         plainFragment,
                                                         compressionS0)
        readState1 = %{readState0
                       |
                       compression_state: compressionS, cipher_state: cipherS}
        readState = update_replay_window(seq, readState1)
        connnectionStates = set_connection_state_by_epoch(readState,
                                                            epoch,
                                                            connnectionStates0,
                                                            :read)
  ...
  defp decode_cipher_text(r_ssl_tls(type: type, version: version, epoch: epoch,
            ...
            connnectionStates0) do
    {plainFragment, mac,
       readState1} = :ssl_record.decipher(:dtls_v1.corresponding_tls_version(version),
                                            cipherFragment, readState0, true)
    macHash = calc_mac_hash(type, version, readState1,
                              epoch, seq, plainFragment)
    case (:ssl_record.is_correct_mac(mac, macHash)) do
      true ->
        {plain, compressionS1} = :ssl_record.uncompress(compAlg,
                                                          plainFragment,
                                                          compressionS0)
        readState2 = Map.put(readState1, :compression_state,
                                           compressionS1)
      ...
      false ->
        r_alert(level: 2, description: 20,
            where: %{mfa: {:dtls_record, :decode_cipher_text, 3},
                       line: 625, file: 'otp/lib/ssl/src/dtls_record.erl'})
    end
  end

  defp calc_mac_hash(type, version,
            %{mac_secret: macSecret,
                security_parameters: r_security_parameters(mac_algorithm: macAlg)},
            epoch, seqNo, fragment) do
    length = :erlang.iolist_size(fragment)
    mac_hash(version, macAlg, macSecret, epoch, seqNo, type,
               length, fragment)
  end

  defp mac_hash(version, macAlg, macSecret, epoch, seqNo, type,
            length, fragment) do
    {major, minor} = version
    value = [<<epoch :: size(16) - unsigned - big - integer,
                 seqNo :: size(48) - unsigned - big - integer,
                 type :: size(8) - unsigned - big - integer,
  ...
  defp start_additional_data(type, version, epoch, seqNo) do
    {majVer, minVer} = version
    <<epoch :: size(16) - unsigned - big - integer,
        seqNo :: size(48) - unsigned - big - integer,
        type :: size(8) - unsigned - big - integer,
