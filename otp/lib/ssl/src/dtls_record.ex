defmodule :m_dtls_record do
  ...
  def empty_connection_state(empty) do
    Map.merge(empty, %{epoch: :undefined, replay_window: init_replay_window()})
  end
  ...
  def next_epoch(
    ...
    %{states | pending_read: %{pending | epoch: epoch + 1, replay_window: init_replay_window()}}
  end
  ...
  def next_epoch(
    ...
    %{states | pending_write: %{pending | epoch: epoch + 1, replay_window: init_replay_window()}}
  end
  ...
  def encode_data(data, version, connectionStates) do
    ...
    case :erlang.iolist_size(data) do
      ...
      _ ->
        {enc, cs} = encode_plain_text(23, version, epoch, data, connectionStates)
        {[enc], cs}
    end
  end
      ...
      false ->
        r_alert(
          ...
          where: %{
            ...
            line: 442,
            file: ~c"otp/lib/ssl/src/dtls_record.erl"
          }
        )
    end
  end
  ...
  defp get_dtls_records_aux(
         ...
         <<_::size(8)-unsigned-big-integer, _MajVer::size(8)-unsigned-big-integer,
           _MinVer::size(8)-unsigned-big-integer, _Epoch::size(16)-unsigned-big-integer,
           _Seq::size(48)-unsigned-big-integer, length::size(16)-unsigned-big-integer,
           _::binary>>,
         _Acc,
         _
       )
  ...
  def update_replay_window(
        sequenceNumber,
        %{replay_window: %{top: top, bottom: bottom, mask: mask0} = window0} = connectionStates
      ) do
    ...
    window =
      case noNewBits > 0 do
        true ->
          ...
          Map.merge(window0, %{top: top + noNewBits, bottom: newBottom, mask: mask})
  ...
  defp mac_hash(version, macAlg, macSecret, epoch, seqNo, type, length, fragment) do
    {major, minor} = version
