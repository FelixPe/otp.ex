defmodule :m_ssl_trace do
  ...
  def start(ioFmtFun, traceOpts) when is_function(ioFmtFun,
                                                 2) or
    ...
    {:ok, pid} = :gen_server.start({:local, :ssl_trace},
                                     :ssl_trace,
  ...
  def init(args) do
    ...
    {:ok,
       r_state(write_fun: :proplists.get_value(:write_fun, args))}
  end
  ...
  def handle_call(:file_close, _From,
           r_state(io_device: iODevice) = state) do
  ...
  def handle_info({:DOWN, _MonitorRef, :process, pid, _Info},
           state) do
    :timer.apply_after(20000, :ssl_trace, :ets_delete,
                         [:ssl_trace, pid])
    {:noreply, state}
  end
  ...
  defp put_proc_stack(pid, stack) when (is_pid(pid) and
                              is_list(stack)) do
  ...
  defp start_tracer(writeFun, traceOpts) when is_function(writeFun,
                                                  2) do
  ...
  defp start_tracer(writeFun, traceOpts) when is_function(writeFun,
                                                  3) do
    acc0 = [{:budget,
               :proplists.get_value(:budget, traceOpts, 10000)}]
    acc1 = (case (:lists.member(:file, traceOpts)) do
              true ->
                traceFile = (case (:init.get_argument(:ssl_trace_file)) do
                               {:ok, [[path]]} ->
                                 path
                               _ ->
                                 'ssl_trace.txt'
  ...
  defp start_dbg_tracer(writeFun, initHandlerAcc0)
    ...
    handler = fn arg, acc0 ->
                   try_handle_trace(:gen_server.call(:ssl_trace, :get_on,
                                                       15000),
                                      arg, writeFun, acc0)
              end
    initHandlerAcc1 = (case (:proplists.get_value(:file,
                                                    initHandlerAcc0)) do
                         :undefined ->
                           initHandlerAcc0
                         file ->
                           {:ok, iODevice} = :gen_server.call(:ssl_trace,
                                                                {:file_open,
                                                                   file},
                                                                15000)
  ...
  defp try_handle_trace(profilesOn, arg, writeFun0, handlerAcc) do
    ...
    budget1 = :lists.foldl(fn profile, bAcc ->
                                case (bAcc > 1) do
                                  true ->
                                    try do
                                      module.handle_trace(profile, traceInfo,
                                                            processStack)
                                    catch
                                      _, _ ->
                                        bAcc
                             ...
                             budget0, profilesOn)
    budget2 = (case (budget1 == budget0 and budget0 > 0) do
                 true ->
                   writeFun.('~.100s ~W~n',
                               [:io_lib.format('~s ~p ~s ',
    ...
    [{:budget, budget2} | :proplists.delete(:budget,
                                              handlerAcc)]
  end
  ...
  defp get_all_trace_profiles() do
    unsorted = (for {profile, _TraceOn, _TraceOff,
                       _TracedFuns} <- trace_profiles() do
                  profile
                end)
    :lists.usort(unsorted)
  end

  defp switch(x, profile) when is_atom(profile) or
                             is_tuple(profile) do
    switch(x, [profile])
  end
  ...
  defp switch(x, profiles) when is_list(profiles) do
    ...
    case (unknown_types(profiles, get_all_trace_profiles(),
                          [])) do
      [] ->
        :gen_server.call(:ssl_trace, {:switch, x, profiles},
                           15000)
      l ->
        {:error, {:unknown, l}}
    end
  ...
  defp unknown_types([modProfile = {_Mod, profile} | tail],
            allProfiles, acc)
  ...
  defp common_prefix({:call, {m, f, args}}, role, profile) do
    [:io_lib.format('~s (~w) -> ~w:~w/~w ', [profile, role, m, f, length(args)])]
  end

  defp common_prefix({:return_from, {m, f, arity}, _Return}, role,
            profile) do
    [:io_lib.format('~s (~w) <- ~w:~w/~w returned ', [profile, role, m, f, arity])]
  end

  defp common_prefix({:exception_from, {m, f, arity}, reason}, role,
            profile) do
  ...
  defp enable_profile({module, profile}) when is_atom(module) or
                                    is_atom(profile) do
  ...
  defp disable_profile({module, profile}) when is_atom(module) or
                                    is_atom(profile) do
  ...
  def trace_profiles() do
    [{:api,
        ...
        [{:ssl,
            [{:listen, 2}, {:connect, 3}, {:handshake, 2}, {:close,
                                                              1}]},
             {:ssl_gen_statem,
                [{:initial_hello, 3}, {:connect, 8}, {:close, 2},
                                                         {:terminate_alert,
                                                            1}]},
                 {:tls_gen_connection,
         ...
         {:csp,
            ...
            [{:ssl_handshake,
                                                                         ...
                                                                         {:handle_client_hello_extensions,
                                                                            10},
                                                                             {:cert_status_check,
                                                                                5}]},
                 {:public_key,
                    [{:ocsp_extensions, 1}, {:pkix_ocsp_validate, 5},
                                                {:ocsp_responder_id, 1},
                                                    {:otp_cert, 1}]},
                     {:pubkey_ocsp,
                        [{:find_responder_cert, 2}, {:do_verify_ocsp_signature,
                                                       4},
                                                                   ...
                                                                   5},
                                                                    {:do_verify_ocsp_response,
                                                                       3},
                                                                        {:is_responder,
                                                                           2},
                                                                            {:find_single_response,
                                                                               3},
                                                                                {:ocsp_status,
                                                                                   1},
                                                                                    {:match_single_response,
                                                                                       4}]},
                         {:ssl, [{:opt_ocsp, 3}]}, {:ssl_certificate,
             ...
             {:crt,
                ...
                [{:public_key,
                    [{:pkix_path_validation, 3}, {:path_validation, 2},
                                                     {:pkix_decode_cert, 2}]},
                     {:ssl_certificate,
                        [{:validate, 3}, {:trusted_cert_and_paths, 4},
                                             {:certificate_chain, 3},
                                                 {:certificate_chain, 5},
                                                     {:issuer, 1}]},
                         {:ssl_cipher, [{:filter, 3}]}, {:ssl_gen_statem,
                                                           [{:initial_hello,
                                                               3}]},
                                                            {:ssl_handshake,
                                                               [{:path_validate,
                                                                           ...
                                                                           1},
                                                                            {:validate_binder,
                                                                               5},
                                                                                {:stateful_store,
                                                                                   0},
                                                                                    {:stateful_ticket_store,
                                                                                       6},
                                                                                        {:stateful_use,
                                                                                           4},
                                                                                            {:stateful_use,
                                                                                               6},
                                                                                                {:stateful_usable_ticket,
                                                                                                   5},
                                                                                                    {:stateful_living_ticket,
                                                                                                       2},
                                                                                                        {:stateful_psk_ticket_id,
                                                                                                           1},
                                                                                                            {:generate_stateless_ticket,
                                                                                                               5},
                                                                                                                {:stateless_use,
                                                                                                                   6},
                                                                                                                    {:stateless_usable_ticket,
                                                                                                                       5},
                                                                                                                        {:stateless_living_ticket,
                                                                                                                           5},
                                                                                                                            {:in_window,
                                                                                                                               2},
                                                                                                                                {:stateless_anti_replay,
                                                                                                                                   5}]},
                                 {:tls_handshake_1_3,
                                    [{:get_ticket_data, 3}]}]},
                             {:hbn,
