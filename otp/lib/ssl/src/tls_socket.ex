defmodule :m_tls_socket do
  ...
  Record.defrecord(:r_generic_block_cipher, :generic_block_cipher, iv: :undefined,
                                                ...
                                                next_iv: :undefined)
  Record.defrecord(:r_state, :state, emulated_opts: :undefined,
  ...
  def listen(transport, port,
           r_config(transport_info: {transport, _, _, _, _},
               inet_user: options, ssl: sslOpts,
               emulated: emOpts) = config) do
    case (transport.listen(port,
                             options ++ internal_inet_values())) do
      {:ok, listenSocket} ->
        {:ok, tracker} = inherit_tracker(listenSocket, emOpts,
                                           sslOpts)
        ...
        {:ok,
           sessionHandler} = session_tickets_tracker(listenSocket,
                                                       lifeTime,
                                                       ticketStoreSize,
                                                       maxEarlyDataSize,
                                                       sslOpts)
        {:ok,
           sessionIdHandle} = session_id_tracker(listenSocket,
                                                   sslOpts)
        trackers = [{:option_tracker, tracker},
                        {:session_tickets_tracker, sessionHandler},
                            {:session_id_tracker, sessionIdHandle}]
        socket = r_sslsocket(pid: {listenSocket,
                           r_config(config, trackers: trackers)})
  ...
  def accept(listenSocket,
           r_config(transport_info: {transport, _, _, _, _} = cbInfo,
               connection_cb: connectionCb, ssl: sslOpts,
               trackers: trackers),
           timeout) do
    case (transport.accept(listenSocket, timeout)) do
      {:ok, socket} ->
        ...
        start_tls_server_connection(sslOpts, connectionCb,
                                      transport, port, socket, emOpts, trackers,
                                      cbInfo)
      {:error, reason} ->
        {:error, reason}
    end
  ...
  def upgrade(socket,
           r_config(transport_info: {transport, _, _, _, _} = cbInfo,
               ssl: sslOptions, emulated: emOpts,
               connection_cb: connectionCb),
           timeout) do
    :ok = setopts(transport, socket,
                    :tls_socket.internal_inet_values())
    case (peername(transport, socket)) do
      {:ok, {address, port}} ->
        :ssl_gen_statem.connect(connectionCb, address, port,
                                  socket,
                                  {sslOptions,
                                     emulated_socket_options(emOpts, r_socket_options()),
                                     :undefined},
                                  self(), cbInfo, timeout)
      {:error, error} ->
        {:error, error}
  ...
  def connect(address, port,
           r_config(transport_info: cbInfo, inet_user: userOpts,
               ssl: sslOpts, emulated: emOpts, inet_ssl: socketOpts,
    ...
    try do
      ...
      {:ok, socket} ->
        :ssl_gen_statem.connect(connetionCb, address, port,
                                  socket,
                                  {sslOpts,
                                     emulated_socket_options(emOpts, r_socket_options()),
                                     :undefined},
  ...
  def socket(pids, transport, socket, connectionCb,
           trackers) do
    r_sslsocket(pid: pids,
  ...
  defp check_active_n(emulatedOpts,
            socket = r_sslsocket(pid: {_, r_config(trackers: trackers)})) do
    tracker = :proplists.get_value(:option_tracker,
                                     trackers)
  ...
  def getopts(:gen_tcp,
    ...
    tracker = :proplists.get_value(:option_tracker,
                                     trackers)
    {sockOptNames,
       emulatedOptNames} = split_options(options)
    emulatedOpts = get_emulated_opts(tracker,
                                       emulatedOptNames)
    socketOpts = get_socket_opts(listenSocket, sockOptNames,
                                   :inet)
    {:ok, emulatedOpts ++ socketOpts}
  end

  def getopts(transport,
           r_sslsocket(pid: {listenSocket, r_config(trackers: trackers)}),
           options) do
    tracker = :proplists.get_value(:option_tracker,
                                     trackers)
  ...
  def emulated_options(opts) do
    emulated_options(opts, internal_inet_values(),
                       default_inet_values())
  end

  def internal_inet_values() do
    [{:packet_size, 0}, {:packet, 0}, {:header, 0},
  ...
  def inherit_tracker(listenSocket, emOpts,
           %{erl_dist: true} = sslOpts) do
    :ssl_listen_tracker_sup.start_child_dist([listenSocket,
                                                  emOpts, sslOpts])
  end

  def inherit_tracker(listenSocket, emOpts, sslOpts) do
    :ssl_listen_tracker_sup.start_child([listenSocket,
                                             emOpts, sslOpts])
  end

  defp session_tickets_tracker(listenSocket, lifetime, ticketStoreSize,
            maxEarlyDataSize,
            %{erl_dist: true, session_tickets: mode,
                anti_replay: antiReplay,
                stateless_tickets_seed: seed}) do
    ...
    case (workers) do
      0 ->
        :tls_server_session_ticket_sup.start_child([listenSocket,
                                                        mode, lifetime,
                                                                  ticketStoreSize,
                                                                      maxEarlyDataSize,
                                                                          antiReplay,
  ...
  defp session_tickets_tracker(_, _, _, _, %{session_tickets: :disabled}) do
    {:ok, :disabled}
  end

  defp session_tickets_tracker(listenSocket, lifetime, ticketStoreSize,
            maxEarlyDataSize,
            %{session_tickets: mode, anti_replay: antiReplay,
                stateless_tickets_seed: seed}) do
    :tls_server_session_ticket_sup.start_child([listenSocket,
                                                    mode, lifetime,
                                                              ticketStoreSize,
  ...
  def session_id_tracker(_, %{versions: [{3, 4}]}) do
    {:ok, :not_relevant}
  end

  def session_id_tracker(_, %{erl_dist: true}) do
    :ssl_upgrade_server_session_cache_sup.start_child(:dist)
  end

  def session_id_tracker(:ssl_unknown_listener, _) do
    :ssl_upgrade_server_session_cache_sup.start_child(:normal)
  end

  def session_id_tracker(listenSocket, _) do
  ...
  def start_link(port, sockOpts, sslOpts) do
    :gen_server.start_link(:tls_socket,
                             [port, sockOpts, sslOpts], [])
  end

  def init([listen, opts, sslOpts]) do
    :erlang.process_flag(:trap_exit, true)
    monitor = :inet.monitor(listen)
    {:ok,
       r_state(emulated_opts: do_set_emulated_opts(opts, []),
           listen_monitor: monitor, ssl_opts: sslOpts)}
  end

  def handle_call({:set_emulated_opts, opts0}, _From,
           r_state(emulated_opts: opts1) = state) do
    opts = do_set_emulated_opts(opts0, opts1)
    {:reply, :ok, r_state(state, emulated_opts: opts)}
  end

  def handle_call(:get_emulated_opts, _From,
           r_state(emulated_opts: opts) = state) do
    {:reply, {:ok, opts}, state}
  end

  def handle_call(:get_all_opts, _From,
           r_state(emulated_opts: emOpts, ssl_opts: sslOpts) = state) do
  ...
  defp start_tls_server_connection(sslOpts, connectionCb, transport, port, socket,
            emOpts, trackers, cbInfo) do
    try do
      {:ok, dynSup} = :tls_connection_sup.start_child([])
      senderOpts = :maps.get(:sender_spawn_opts, sslOpts, [])
      {:ok,
         sender} = :tls_dyn_connection_sup.start_child(dynSup,
                                                         :sender,
                                                         [[{:spawn_opt,
                                                              senderOpts}]])
      connArgs = [:server, sender, 'localhost', port, socket, {sslOpts,
                                                       emulated_socket_options(emOpts,
                                                                                 r_socket_options()),
                                                       trackers},
                                                        self(), cbInfo]
      {:ok, pid} = :tls_dyn_connection_sup.start_child(dynSup,
                                                         :receiver, connArgs)
      :ssl_gen_statem.socket_control(connectionCb, socket,
                                       [pid, sender], transport, trackers)
    catch
  ...
  defp split_options([{name, _} = opt | opts], emu, socketOpts,
            emuOpts) do
  ...
  defp split_options([name | opts], emu, socketOptNames,
            emuOptNames) do
    case (:lists.member(name, emu)) do
      true ->
        split_options(opts, emu, socketOptNames,
                        [name | emuOptNames])
      false ->
        split_options(opts, emu, [name | socketOptNames],
                        emuOptNames)
    end
  end
  ...
  defp do_set_emulated_opts([{:active, n0} | rest], opts)
    ...
    n = update_active_n(n0,
                          :proplists.get_value(:active, opts, false))
    do_set_emulated_opts(rest,
  ...
  def emulated_socket_options(inetValues,
           r_socket_options(mode: mode, header: header, active: active,
               packet: packet, packet_size: size)) do
    r_socket_options(mode: :proplists.get_value(:mode, inetValues, mode),
  ...
  defp emulated_options([{:mode, value} = opt | opts], inet,
            emulated) do
    validate_inet_option(:mode, value)
    emulated_options(opts, inet,
                       [opt | :proplists.delete(:mode, emulated)])
  end

  defp emulated_options([{:header, value} = opt | opts], inet,
            emulated) do
    validate_inet_option(:header, value)
    emulated_options(opts, inet,
                       [opt | :proplists.delete(:header, emulated)])
  end

  defp emulated_options([{:active, value} = opt | opts], inet,
            emulated) do
    validate_inet_option(:active, value)
    emulated_options(opts, inet,
                       [opt | :proplists.delete(:active, emulated)])
  end

  defp emulated_options([{:packet, value} = opt | opts], inet,
            emulated) do
    validate_inet_option(:packet, value)
    emulated_options(opts, inet,
                       [opt | :proplists.delete(:packet, emulated)])
  end

  defp emulated_options([{:packet_size, value} = opt | opts], inet,
            emulated) do
    validate_inet_option(:packet_size, value)
    emulated_options(opts, inet,
                       [opt | :proplists.delete(:packet_size, emulated)])
  end
  ...
  defp validate_inet_option(:mode, value) when (value !== :list and
                                value !== :binary) do
    throw({:error, {:options, {:mode, value}}})
  end

  defp validate_inet_option(:packet, value) when not
                               (is_atom(value) or is_integer(value)) do
    throw({:error, {:options, {:packet, value}}})
  end

  defp validate_inet_option(:packet_size, value) when not
                                    is_integer(value) do
  ...
  defp validate_inet_option(:active, value) when (value >= - 32768 and
                                  value <= 32767) do
    :ok
  end

  defp validate_inet_option(:active, value) when (value !== true and
