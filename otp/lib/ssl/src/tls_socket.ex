defmodule :m_tls_socket do
  ...
  defp session_tickets_tracker(listenSocket, lifetime, ticketStoreSize, maxEarlyDataSize, %{
         erl_dist: true,
  ...
  def init([listen, opts, sslOpts]) do
    ...
    {:ok,
     r_state(
       emulated_opts: do_set_emulated_opts(opts, []),
  ...
  defp start_tls_server_connection(
         sslOpts,
         connectionCb,
         transport,
         port,
    ...
    try do
      ...
      {:ok, sender} =
        :tls_dyn_connection_sup.start_child(
          dynSup,
          :sender,
          [[{:spawn_opt, senderOpts}]]
        )
