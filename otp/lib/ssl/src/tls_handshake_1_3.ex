defmodule :m_tls_handshake_1_3 do
  ...
  Record.defrecord(:r_ssl_tls, :ssl_tls, type: :undefined,
                                   ...
                                   early_data: false)
  Record.defrecord(:r_inner_plaintext, :inner_plaintext, content: :undefined,
                                           type: :undefined, zeros: :undefined)
  Record.defrecord(:r_tls_cipher_text, :tls_cipher_text, opaque_type: 23,
                                           legacy_version: {3, 3},
                                           encoded_record: :undefined)
  def server_hello(msgType, sessionId, keyShare, pSK,
           connectionStates) do
    %{security_parameters:
    ...
    extensions = server_hello_extensions(msgType, keyShare,
                                           pSK)
    r_server_hello(server_version: {3, 3},
  ...
  defp server_hello_extensions(:hello_retry_request = msgType, keyShare, _) do
    ...
    :ssl_handshake.add_server_share(msgType, extensions,
                                      keyShare)
  end
  ...
  defp server_hello_extensions(msgType, keyShare, {selectedIdentity, _}) do
    ...
    :ssl_handshake.add_server_share(msgType,
                                      Map.put(extensions, :pre_shared_key,
                                                            preSharedKey),
                                      keyShare)
  end
  ...
  defp server_hello_random(:hello_retry_request, _) do
    <<207, 33, 173, 116, 229, 154, 97, 17, 190, 29, 140, 2,
        30, 101, 184, 145, 194, 162, 17, 22, 122, 187, 140, 94,
        7, 158, 9, 226, 200, 168, 51, 156>>
  end

  def maybe_add_cookie_extension(r_state(ssl_options: %{cookie: false}) = state,
           serverHello) do
    {state, serverHello}
  end

  def maybe_add_cookie_extension(r_state(connection_states: connectionStates,
             ssl_options: %{cookie: true},
             handshake_env: r_handshake_env(tls_handshake_history: {[cH1 | _],
                                                        _}) = hsEnv0) = state,
           r_server_hello(extensions: extensions0) = serverHello) do
    ...
    cookie = :ssl_cipher.encrypt_data("cookie", messageHash, shard,
                                        iV)
    hsEnv = r_handshake_env(hsEnv0, cookie_iv_shard: {iV, shard})
    extensions = Map.put(extensions0, :cookie,
                                        r_cookie(cookie: cookie))
    {r_state(state, handshake_env: hsEnv),
       r_server_hello(serverHello, extensions: extensions)}
  end
  ...
  def maybe_add_cookie_extension(cookie,
           r_client_hello(extensions: extensions0) = clientHello) do
  ...
  def encrypted_extensions(r_state(handshake_env: handshakeEnv)) do
    e0 = %{}
    e1 = (case (r_handshake_env(handshakeEnv, :alpn)) do
            :undefined ->
              e0
            aLPNProtocol ->
              :ssl_handshake.add_alpn(%{}, aLPNProtocol)
          end)
    e2 = (case (r_handshake_env(handshakeEnv, :max_frag_enum)) do
            :undefined ->
              e1
            maxFragEnum ->
              Map.put(e1, :max_frag_enum, maxFragEnum)
          end)
    e3 = (case (r_handshake_env(handshakeEnv, :sni_guided_cert_selection)) do
            false ->
              e2
            true ->
              Map.put(e2, :sni, r_sni(hostname: ''))
          end)
    e = (case (r_handshake_env(handshakeEnv, :early_data_accepted)) do
           false ->
             e3
           true ->
  ...
  def certificate_request(signAlgs0, signAlgsCert0, certDbHandle,
           certDbRef, certAuthBool) do
    ...
    extensions1 = add_signature_algorithms_cert(extensions0,
                                                  signAlgsCert)
    extensions = (cond do
                    certAuthBool === true ->
                      auths = :ssl_handshake.certificate_authorities(certDbHandle,
                                                                       certDbRef)
                      Map.put(extensions1, :certificate_authorities,
                                             r_certificate_authorities(authorities: auths))
                    true ->
                      extensions1
                  end)
    r_certificate_request_1_3(certificate_request_context: <<>>,
        extensions: extensions)
  end

  defp add_signature_algorithms(extensions, signAlgs) do
    Map.put(extensions, :signature_algorithms,
  ...
  defp add_signature_algorithms_cert(extensions, signAlgsCert) do
    Map.put(extensions, :signature_algorithms_cert,
  ...
  defp filter_tls13_algs(algo) do
    :lists.foldl(fn atom, acc when is_atom(atom) ->
                      [atom | acc]
                    {:sha512, :rsa}, acc ->
  ...
  def certificate(:undefined, _, _, _, :client) do
    {:ok,
       r_certificate_1_3(certificate_request_context: <<>>,
           certificate_list: [])}
  end

  def certificate([ownCert], certDbHandle, certDbRef, _CRContext,
           role) do
    case (:ssl_certificate.certificate_chain(ownCert,
      ...
      {:error, error} when role === :server ->
        {:error,
           r_alert(level: 2, description: 80,
               where: %{mfa: {:tls_handshake_1_3, :certificate, 5},
                          line: 300, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
  ...
  def maybe_hello_retry_request(r_server_hello(random: <<207, 33, 173, 116, 229, 154, 97, 17,
                       190, 29, 140, 2, 30, 101, 184, 145, 194, 162, 17, 22,
                       122, 187, 140, 94, 7, 158, 9, 226, 200, 168, 51,
                       156>>) = serverHello,
           r_state(protocol_specific: pS) = state0) do
    {:error,
       {r_state(state0, protocol_specific: Map.put(pS, :hello_retry,
                                                   true)),
          :start, serverHello}}
  end
  ...
  def finished(r_state(connection_states: connectionStates,
             handshake_env: r_handshake_env(tls_handshake_history: {messages,
                                                        _}))) do
    %{security_parameters: secParamsR,
        cipher_state:
        r_cipher_state(finished_key: finishedKey)} = :ssl_record.current_connection_state(connectionStates,
                                                                               :write)
    r_security_parameters(prf_algorithm: hKDFAlgo) = secParamsR
    verifyData = :tls_v1.finished_verify_data(finishedKey,
                                                hKDFAlgo, messages)
  ...
  def create_change_cipher_spec(r_state(ssl_options: %{log_level: logLevel})) do
    connectionStates = %{current_write:
                         %{compression_state: :undefined,
                             cipher_state: :undefined, sequence_number: 1,
                             security_parameters:
                             r_security_parameters(bulk_cipher_algorithm: 0,
                                 compression_algorithm: 0, mac_algorithm: 0),
                             mac_secret: :undefined}}
    {binChangeCipher,
       _} = :tls_record.encode_change_cipher_spec({3, 3},
                                                    connectionStates)
    :ssl_logger.debug(logLevel, :outbound, :record,
                        binChangeCipher)
    [binChangeCipher]
  end

  def process_certificate_request(r_certificate_request_1_3(extensions: extensions),
           r_state(ssl_options: %{signature_algs: clientSignAlgs},
               connection_env: r_connection_env(cert_key_alts: certKeyAlts,
                                   negotiated_version: version),
               static_env: r_static_env(cert_db: certDbHandle,
                               cert_db_ref: certDbRef),
               session: session0) = state) do
    serverSignAlgs = get_signature_scheme_list(:maps.get(:signature_algs,
                                                           extensions,
                                                           :undefined))
    serverSignAlgsCert = get_signature_scheme_list(:maps.get(:signature_algs_cert,
                                                               extensions,
                                                               :undefined))
    certAuths = get_certificate_authorities(:maps.get(:certificate_authorities,
                                                        extensions, :undefined))
    certKeyPairs = :ssl_certificate.available_cert_key_pairs(certKeyAlts,
                                                               version)
    session = select_client_cert_key_pair(session0,
                                            certKeyPairs, serverSignAlgs,
                                            serverSignAlgsCert, clientSignAlgs,
  ...
  def process_certificate(r_certificate_1_3(certificate_request_context: <<>>,
             certificate_list: []),
           r_state(ssl_options: %{fail_if_no_peer_cert:
                            false}) = state) do
  ...
  def process_certificate(r_certificate_1_3(certificate_request_context: <<>>,
    ...
    {:error,
       {r_alert(level: 2, description: 116,
            where: %{mfa:
                     {:tls_handshake_1_3, :process_certificate, 2},
                       line: 444, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
            reason: :certificate_required),
          state}}
  end

  def process_certificate(r_certificate_1_3(certificate_list: certEntries),
           r_state(ssl_options: sslOptions,
               static_env: r_static_env(role: role, host: host,
                               cert_db: certDbHandle, cert_db_ref: certDbRef,
                               crl_db: cRLDbHandle),
               handshake_env: r_handshake_env(ocsp_stapling_state: ocspState)) = state0) do
    case (validate_certificate_chain(certEntries,
                                       certDbHandle, certDbRef, sslOptions,
                                       cRLDbHandle, role, host, ocspState)) do
      r_alert() = alert ->
  ...
  def verify_certificate_verify(r_state(static_env: r_static_env(role: role),
             connection_states: connectionStates,
             handshake_env: r_handshake_env(public_key_info: publicKeyInfo,
                                tls_handshake_history: hHistory)) = state0,
           r_certificate_verify_1_3(algorithm: signatureScheme, signature: signature)) do
    %{security_parameters:
      secParamsR} = :ssl_record.pending_connection_state(connectionStates,
                                                           :write)
    r_security_parameters(prf_algorithm: hKDFAlgo) = secParamsR
    ...
    case (verify(tHash, contextString, hashAlgo, signAlg,
      ...
      {:ok, false} ->
        ...
        {:error,
           {r_alert(level: 2, description: 40,
                where: %{mfa:
                         {:tls_handshake_1_3, :verify_certificate_verify, 2},
                           line: 500, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
  ...
  def validate_finished(r_state(connection_states: connectionStates,
             handshake_env: r_handshake_env(tls_handshake_history: {messages0,
                                                        _})),
           verifyData) do
    %{security_parameters: secParamsR,
        cipher_state:
    ...
    controlData = :tls_v1.finished_verify_data(finishedKey,
                                                 hKDFAlgo, messages)
  ...
  defp compare_verify_data(_, _) do
    {:error,
       r_alert(level: 2, description: 51,
           where: %{mfa:
                    {:tls_handshake_1_3, :compare_verify_data, 2},
                      line: 531, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
           reason: :decrypt_error)}
  end

  def encode_handshake(r_certificate_request_1_3(certificate_request_context: context,
             extensions: exts)) do
  ...
  def encode_handshake(r_certificate_1_3(certificate_request_context: context,
             certificate_list: entries)) do
    encContext = encode_cert_req_context(context)
  ...
  def encode_handshake(r_new_session_ticket(ticket_lifetime: lifeTime, ticket_age_add: age,
             ticket_nonce: nonce, ticket: ticket,
             extensions: exts)) do
    ticketSize = byte_size(ticket)
    nonceSize = byte_size(nonce)
    binExts = encode_extensions(exts)
    {4,
       <<lifeTime :: size(32) - unsigned - big - integer,
           age :: size(32) - unsigned - big - integer,
           nonceSize :: size(8) - unsigned - big - integer,
           nonce :: binary,
           ticketSize :: size(16) - unsigned - big - integer,
           ticket :: binary, binExts :: binary>>}
  end
  ...
  def encode_early_data(cipher,
           r_state(flight_buffer: flight0,
               protocol_specific: %{sender: _Sender},
               ssl_options: %{versions: [version | _],
                                early_data: earlyData} = _SslOpts0) = state0) do
    r_state(connection_states: %{current_write:
                           %{security_parameters:
                             securityParameters0} = write0} = connectionStates0) = state0
    bulkCipherAlgo = :ssl_cipher.bulk_cipher_algorithm(cipher)
    securityParameters = r_security_parameters(securityParameters0, cipher_type: 2, 
                                                  bulk_cipher_algorithm: bulkCipherAlgo)
    write = Map.put(write0, :security_parameters,
                              securityParameters)
    connectionStates1 = Map.put(connectionStates0, :current_write,
                                                     write)
    {binEarlyData,
       connectionStates} = :tls_record.encode_data([earlyData],
                                                     version, connectionStates1)
    r_state(state0, connection_states: connectionStates, 
                flight_buffer: flight0 ++ [binEarlyData])
  end

  def decode_handshake(2,
      ...
      when random === <<207, 33, 173, 116, 229, 154, 97, 17,
                          190, 29, 140, 2, 30, 101, 184, 145, 194, 162, 17, 22,
                          122, 187, 140, 94, 7, 158, 9, 226, 200, 168, 51,
                          156>> do
    helloExtensions = :ssl_handshake.decode_hello_extensions(extensions,
                                                               {3, 4},
                                                               {major, minor},
                                                               :hello_retry_request)
  ...
  def decode_handshake(13,
           <<0 :: size(8) - unsigned - big - integer,
               size :: size(16) - unsigned - big - integer,
               encExts :: size(size) - binary>>) do
    exts = decode_extensions(encExts, :certificate_request)
    r_certificate_request_1_3(certificate_request_context: <<>>, extensions: exts)
  end

  def decode_handshake(13,
           <<cSize :: size(8) - unsigned - big - integer,
               context :: size(cSize) - binary,
               size :: size(16) - unsigned - big - integer,
               encExts :: size(size) - binary>>) do
    exts = decode_extensions(encExts, :certificate_request)
    r_certificate_request_1_3(certificate_request_context: context,
        extensions: exts)
  end

  def decode_handshake(11,
           <<0 :: size(8) - unsigned - big - integer,
               size :: size(24) - unsigned - big - integer,
               certs :: size(size) - binary>>) do
  ...
  def decode_handshake(11,
           <<cSize :: size(8) - unsigned - big - integer,
               ...
               certs :: size(size) - binary>>) do
    certList = decode_cert_entries(certs)
    r_certificate_1_3(certificate_request_context: context,
        certificate_list: certList)
  end

  def decode_handshake(15,
           <<encAlgo :: size(16) - unsigned - big - integer,
               size :: size(16) - unsigned - big - integer,
               signature :: size(size) - binary>>) do
    algorithm = :ssl_cipher.signature_scheme(encAlgo)
    r_certificate_verify_1_3(algorithm: algorithm, signature: signature)
  end

  def decode_handshake(8,
           <<size :: size(16) - unsigned - big - integer,
               encExts :: size(size) - binary>>) do
    r_encrypted_extensions(extensions: decode_extensions(encExts,
                                      :encrypted_extensions))
  end

  def decode_handshake(4,
           <<lifeTime :: size(32) - unsigned - big - integer,
               age :: size(32) - unsigned - big - integer,
               nonceSize :: size(8) - unsigned - big - integer,
               nonce :: size(nonceSize) - binary,
               ticketSize :: size(16) - unsigned - big - integer,
               ticket :: size(ticketSize) - binary,
               binExtSize :: size(16) - unsigned - big - integer,
               binExts :: size(binExtSize) - binary>>) do
    exts = decode_extensions(binExts, :encrypted_extensions)
    r_new_session_ticket(ticket_lifetime: lifeTime, ticket_age_add: age,
        ticket_nonce: nonce, ticket: ticket, extensions: exts)
  end
  ...
  def decode_handshake(24,
           <<update :: size(8) - unsigned - big - integer>>) do
    r_key_update(request_update: decode_key_update(update))
  end

  def decode_handshake(tag, handshakeMsg) do
    :ssl_handshake.decode_handshake({3, 4}, tag,
                                      handshakeMsg)
  end
  ...
  def is_valid_binder(binder, hHistory, pSK, hash) do
    case (hHistory) do
      [clientHello2, hRR, messageHash | _] ->
        ...
        binder == calculate_binder(finishedKey, hash,
  ...
  defp encode_cert_req_context(bin) do
    size = byte_size(bin)
    <<size :: size(8) - unsigned - big - integer,
        bin :: binary>>
  end
  ...
  defp encode_cert_entries([r_certificate_entry(data: data, extensions: exts) | rest],
            acc) do
    dSize = byte_size(data)
    binExts = encode_extensions(exts)
    encode_cert_entries(rest,
                          [<<dSize :: size(24) - unsigned - big - integer,
                               data :: binary, binExts :: binary>> |
                               acc])
  end
  ...
  defp decode_key_update(n) do
    throw(r_alert(level: 2, description: 47,
              where: %{mfa:
                       {:tls_handshake_1_3, :decode_key_update, 1},
                         line: 729, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
              reason: {:request_update, n}))
  end

  defp decode_cert_entries(entries) do
    for << <<dSize :: size(24) - unsigned - big - integer,
               data :: size(dSize) - binary,
               esize :: size(16) - unsigned - big - integer,
               binExts :: size(esize) - binary>> <- entries >> do
      r_certificate_entry(data: data,
          extensions: decode_extensions(binExts,
                                          :certificate_request))
    end
  end
  ...
  defp sign(tHash, context, hashAlgo, privateKey,
            signAlgo) do
    content = build_content(context, tHash)
    try do
      {:ok,
         :ssl_handshake.digitally_signed({3, 4}, content,
                                           hashAlgo, privateKey, signAlgo)}
    catch
      alert ->
        {:error, alert}
    end
  ...
  defp verify(tHash, context, hashAlgo, signAlgo, signature,
            publicKeyInfo) do
    content = build_content(context, tHash)
    try do
      :ssl_handshake.verify_signature({3, 4}, content,
                                        {hashAlgo, signAlgo}, signature,
                                        publicKeyInfo)
    catch
      :error, reason ->
        (fn () ->
                ...
                :undefined ->
                  :ssl_logger.log(:debug, :debug,
                                    ...
                                    %{mfa: {:tls_handshake_1_3, :verify, 6},
                                        line: 791, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'})
                __LogLevel__ ->
        ...
        {:error,
           r_alert(level: 2, description: 80,
               where: %{mfa: {:tls_handshake_1_3, :verify, 6},
                          line: 792, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
               reason: :badarg)}
    else
      result ->
        {:ok, result}
  ...
  defp validate_certificate_chain(certEntries, certDbHandle, certDbRef,
            sslOptions, cRLDbHandle, role, host, ocspState0) do
    {certs, certExt,
       ocspState} = split_cert_entries(certEntries, ocspState0)
    ocspResponderCerts = (case (:maps.get(:ocsp_stapling,
                                            sslOptions, :disabled)) do
                            %{ocsp_responder_certs: v} ->
                              v
                            :disabled ->
                              []
                          end)
    :ssl_handshake.certify(r_certificate(asn1_certificates: certs),
                             certDbHandle, certDbRef, sslOptions, cRLDbHandle,
                             role, host, {3, 4},
                             %{cert_ext: certExt, ocsp_state: ocspState,
                                 ocsp_responder_certs: ocspResponderCerts})
  end

  defp store_peer_cert(r_state(session: session,
              handshake_env: hsEnv) = state,
            peerCert, publicKeyInfo) do
  ...
  defp split_cert_entries([r_certificate_entry(data: derCert, extensions: extensions0) |
               certEntries],
            ocspState0, chain, ext) do
    id = :public_key.pkix_subject_id(derCert)
    extensions = (for {_,
                         extValue} <- :maps.to_list(extensions0) do
                    extValue
                  end)
    ocspState = (case (:maps.get(:status_request,
                                   extensions0, :undefined)) do
                   :undefined ->
                     ocspState0
                   _ ->
    ...
    split_cert_entries(certEntries, ocspState,
                         [derCert | chain], Map.put(ext, id, extensions))
  end

  def replace_ch1_with_message_hash(r_state(connection_states: connectionStates,
             handshake_env: r_handshake_env(tls_handshake_history: {[hRR, cH1 |
                                                                hHistory],
                                                        lM}) = hSEnv) = state0) do
    %{security_parameters:
      secParamsR} = :ssl_record.pending_connection_state(connectionStates,
    ...
    r_state(state0, handshake_env: r_handshake_env(hSEnv, tls_handshake_history: {[hRR,
                                                                   messageHash |
                                                                       hHistory],
                                                                lM}))
  end
  ...
  defp message_hash(clientHello1, hKDFAlgo) do
    [254, 0, 0, :ssl_cipher.hash_size(hKDFAlgo),
                    :crypto.hash(hKDFAlgo, clientHello1)]
  end

  def calculate_handshake_secrets(publicKey, privateKey, selectedGroup, pSK,
           r_state(connection_states: connectionStates,
               handshake_env: r_handshake_env(tls_handshake_history: hHistory)) = state0) do
    %{security_parameters:
      secParamsR} = :ssl_record.pending_connection_state(connectionStates,
                                                           :read)
    r_security_parameters(prf_algorithm: hKDFAlgo,
        cipher_suite: cipherSuite) = secParamsR
    earlySecret = :tls_v1.key_schedule(:early_secret,
                                         hKDFAlgo, {:psk, pSK})
    iKM = calculate_shared_secret(publicKey, privateKey,
                                    selectedGroup)
    handshakeSecret = :tls_v1.key_schedule(:handshake_secret,
                                             hKDFAlgo, iKM, earlySecret)
    {messages, _} = hHistory
    clientHSTrafficSecret = :tls_v1.client_handshake_traffic_secret(hKDFAlgo,
                                                                      handshakeSecret,
                                                                      :lists.reverse(messages))
    serverHSTrafficSecret = :tls_v1.server_handshake_traffic_secret(hKDFAlgo,
                                                                      handshakeSecret,
                                                                      :lists.reverse(messages))
    keyLength = :tls_v1.key_length(cipherSuite)
    {readKey,
       readIV} = :tls_v1.calculate_traffic_keys(hKDFAlgo,
                                                  keyLength,
                                                  clientHSTrafficSecret)
    {writeKey,
       writeIV} = :tls_v1.calculate_traffic_keys(hKDFAlgo,
                                                   keyLength,
                                                   serverHSTrafficSecret)
    readFinishedKey = :tls_v1.finished_key(clientHSTrafficSecret,
                                             hKDFAlgo)
    writeFinishedKey = :tls_v1.finished_key(serverHSTrafficSecret,
                                              hKDFAlgo)
    state1 = maybe_store_handshake_traffic_secret(state0,
                                                    clientHSTrafficSecret,
                                                    serverHSTrafficSecret)
    update_pending_connection_states(state1,
                                       handshakeSecret, :undefined, :undefined,
                                       :undefined, readKey, readIV,
                                       readFinishedKey, writeKey, writeIV,
                                       writeFinishedKey)
  end

  def calculate_client_early_traffic_secret(r_state(connection_states: connectionStates,
    ...
    %{cipher: cipher,
        prf:
        hKDF} = :ssl_cipher_format.suite_bin_to_map(cipherSuite)
    calculate_client_early_traffic_secret(hist, pSK, cipher,
                                            hKDF, state)
  end

  def calculate_client_early_traffic_secret(clientHello, pSK, cipher, hKDFAlgo,
           r_state(connection_states: connectionStates,
               ssl_options: opts,
               static_env: r_static_env(role: role)) = state0) do
    earlySecret = :tls_v1.key_schedule(:early_secret,
                                         hKDFAlgo, {:psk, pSK})
    clientEarlyTrafficSecret = :tls_v1.client_early_traffic_secret(hKDFAlgo,
                                                                     earlySecret,
                                                                     clientHello)
    keyLength = :ssl_cipher.key_material(cipher)
    {key, iV} = :tls_v1.calculate_traffic_keys(hKDFAlgo,
                                                 keyLength,
                                                 clientEarlyTrafficSecret)
    case (role) do
      :client ->
        pendingWrite0 = :ssl_record.pending_connection_state(connectionStates,
                                                               :write)
        pendingWrite1 = maybe_store_early_data_secret(opts,
                                                        clientEarlyTrafficSecret,
                                                        pendingWrite0)
        pendingWrite = update_connection_state(pendingWrite1,
                                                 :undefined, :undefined,
                                                 :undefined, key, iV,
                                                 :undefined)
        r_state(state0, connection_states: Map.put(connectionStates, :pending_write,
                                                                 pendingWrite))
      :server ->
        pendingRead0 = :ssl_record.pending_connection_state(connectionStates,
                                                              :read)
        pendingRead1 = maybe_store_early_data_secret(opts,
                                                       clientEarlyTrafficSecret,
                                                       pendingRead0)
        pendingRead = update_connection_state(pendingRead1,
                                                :undefined, :undefined,
                                                :undefined, key, iV, :undefined)
        r_state(state0, connection_states: Map.put(connectionStates, :pending_read,
                                                                 pendingRead))
    end
  end
  ...
  def get_pre_shared_key(:manual = sessionTickets, useTicket, hKDFAlgo,
           serverPSK) do
    ticketData = get_ticket_data(self(), sessionTickets,
                                   useTicket)
    case (choose_psk(ticketData, serverPSK)) do
      ...
      :illegal_parameter ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_handshake_1_3, :get_pre_shared_key, 4},
                          line: 1008, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'})}
      {_, pSK, _, _, _} ->
        {:ok, pSK}
    end
  end

  def get_pre_shared_key(:auto = sessionTickets, useTicket, hKDFAlgo,
           serverPSK) do
    ticketData = get_ticket_data(self(), sessionTickets,
                                   useTicket)
    case (choose_psk(ticketData, serverPSK)) do
      :undefined ->
        :tls_client_ticket_store.unlock_tickets(self(),
                                                  useTicket)
        {:ok,
           :binary.copy(<<0>>, :ssl_cipher.hash_size(hKDFAlgo))}
      :illegal_parameter ->
        ...
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_handshake_1_3, :get_pre_shared_key, 4},
                          line: 1020, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'})}
      {key, pSK, _, _, _} ->
  ...
  def get_pre_shared_key_early_data(sessionTickets, useTicket) do
    ticketData = get_ticket_data(self(), sessionTickets,
                                   useTicket)
    case (choose_psk(ticketData,
                       r_pre_shared_key_server_hello(selected_identity: 0))) do
      :undefined ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_handshake_1_3, :get_pre_shared_key_early_data, 2},
                          line: 1032, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'})}
      :illegal_parameter ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_handshake_1_3, :get_pre_shared_key_early_data, 2},
                          line: 1034, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'})}
  ...
  def get_supported_groups(:undefined = groups) do
    {:error,
       r_alert(level: 2, description: 47,
           where: %{mfa:
                    {:tls_handshake_1_3, :get_supported_groups, 1},
                      line: 1040, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
  ...
  defp choose_psk([r_ticket_data(key: key, pos: selectedIdentity, psk: pSK,
               cipher_suite: {cipher, hKDF}, max_size: maxSize) |
               _],
            r_pre_shared_key_server_hello(selected_identity: selectedIdentity)) do
  ...
  def calculate_traffic_secrets(r_state(static_env: r_static_env(role: role),
             connection_states: connectionStates,
             handshake_env: r_handshake_env(tls_handshake_history: hHistory)) = state0) do
    %{security_parameters:
      secParamsR} = :ssl_record.pending_connection_state(connectionStates,
                                                           :read)
    r_security_parameters(prf_algorithm: hKDFAlgo, cipher_suite: cipherSuite,
        master_secret: handshakeSecret) = secParamsR
    masterSecret = :tls_v1.key_schedule(:master_secret,
                                          hKDFAlgo, handshakeSecret)
    messages = get_handshake_context(role, hHistory)
    clientAppTrafficSecret0 = :tls_v1.client_application_traffic_secret_0(hKDFAlgo,
                                                                            masterSecret,
                                                                            :lists.reverse(messages))
    serverAppTrafficSecret0 = :tls_v1.server_application_traffic_secret_0(hKDFAlgo,
                                                                            masterSecret,
                                                                            :lists.reverse(messages))
    keyLength = :tls_v1.key_length(cipherSuite)
    {readKey,
    ...
    update_pending_connection_states(state0, masterSecret,
                                       :undefined, clientAppTrafficSecret0,
                                       serverAppTrafficSecret0, readKey, readIV,
                                       :undefined, writeKey, writeIV,
                                       :undefined)
  end
  ...
  def maybe_calculate_resumption_master_secret(r_state(ssl_options: %{session_tickets:
                          :disabled}) = state) do
    state
  end
  ...
  def maybe_calculate_resumption_master_secret(r_state(ssl_options: %{session_tickets:
                          sessionTickets},
             connection_states: connectionStates,
             handshake_env: r_handshake_env(tls_handshake_history: hHistory)) = state)
    ...
    rMS = :tls_v1.resumption_master_secret(hKDFAlgo,
                                             masterSecret,
                                             :lists.reverse(messages0))
    update_resumption_master_secret(state, rMS)
  end

  def calculate_exporter_master_secret(r_state(static_env: r_static_env(role: role),
             connection_states: connectionStates,
             handshake_env: r_handshake_env(tls_handshake_history: hHistory))) do
    %{security_parameters:
      secParamsR} = :ssl_record.pending_connection_state(connectionStates,
                                                           :read)
    r_security_parameters(prf_algorithm: hKDFAlgo,
    ...
    messages = get_handshake_context(role, hHistory)
    :tls_v1.exporter_master_secret(hKDFAlgo, masterSecret,
                                     :lists.reverse(messages))
  end

  def forget_master_secret(r_state(connection_states: %{pending_read: pendingRead,
                                  pending_write: pendingWrite,
                                  current_read: currentRead,
                                  current_write: currentWrite} = cS) = state) do
    r_state(state, connection_states: Map.merge(cS, %{pending_read:
                                                overwrite_master_secret(pendingRead),
                                                  pending_write:
                                                  overwrite_master_secret(pendingWrite),
                                                  current_read:
                                                  overwrite_master_secret(currentRead),
                                                  current_write:
                                                  overwrite_master_secret(currentWrite)}))
  end

  defp overwrite_master_secret(connectionState = %{security_parameters:
                              securityParameters0}) do
    securityParameters = r_security_parameters(securityParameters0, master_secret: {:master_secret,
                                                                  <<0>>})
    Map.put(connectionState, :security_parameters,
                               securityParameters)
  end

  def set_client_random(r_state(connection_states: %{pending_read: pendingRead,
                                  pending_write: pendingWrite,
                                  current_read: currentRead,
                                  current_write: currentWrite} = cS) = state,
           clientRandom) do
    r_state(state, connection_states: Map.merge(cS, %{pending_read:
                                                overwrite_client_random(pendingRead,
                                                                          clientRandom),
                                                  pending_write:
                                                  overwrite_client_random(pendingWrite,
                                                                            clientRandom),
                                                  current_read:
                                                  overwrite_client_random(currentRead,
                                                                            clientRandom),
                                                  current_write:
                                                  overwrite_client_random(currentWrite,
                                                                            clientRandom)}))
  end

  defp overwrite_client_random(connectionState = %{security_parameters:
                              securityParameters0},
            clientRandom) do
  ...
  defp maybe_store_handshake_traffic_secret(r_state(connection_states: %{pending_read:
                                 pendingRead} = cS,
              ssl_options: %{keep_secrets: true}) = state,
            clientHSTrafficSecret, serverHSTrafficSecret) do
    pendingRead1 = store_handshake_traffic_secret(pendingRead,
                                                    clientHSTrafficSecret,
                                                    serverHSTrafficSecret)
    r_state(state, connection_states: Map.put(cS, :pending_read,
                                              pendingRead1))
  end
  ...
  defp store_handshake_traffic_secret(connectionState, clientHSTrafficSecret,
            serverHSTrafficSecret) do
    Map.merge(connectionState, %{client_handshake_traffic_secret:
                                 clientHSTrafficSecret,
                                   server_handshake_traffic_secret:
                                   serverHSTrafficSecret})
  end

  defp update_pending_connection_states(r_state(static_env: r_static_env(role: :server),
              connection_states: cS = %{pending_read: pendingRead0,
                                          pending_write:
                                          pendingWrite0}) = state,
            handshakeSecret, resumptionMasterSecret,
            clientAppTrafficSecret, serverAppTrafficSecret, readKey,
            readIV, readFinishedKey, writeKey, writeIV,
            writeFinishedKey) do
    pendingRead = update_connection_state(pendingRead0,
                                            handshakeSecret,
                                            resumptionMasterSecret,
                                            clientAppTrafficSecret, readKey,
                                            readIV, readFinishedKey)
    pendingWrite = update_connection_state(pendingWrite0,
                                             handshakeSecret,
                                             resumptionMasterSecret,
                                             serverAppTrafficSecret, writeKey,
  ...
  defp update_pending_connection_states(r_state(static_env: r_static_env(role: :client),
              connection_states: cS = %{pending_read: pendingRead0,
                                          pending_write:
                                          pendingWrite0}) = state,
            handshakeSecret, resumptionMasterSecret,
  ...
  defp update_connection_state(connectionState = %{security_parameters:
                              securityParameters0},
            handshakeSecret, resumptionMasterSecret,
            applicationTrafficSecret, key, iV, finishedKey) do
    securityParameters = r_security_parameters(securityParameters0, master_secret: handshakeSecret, 
    ...
    Map.merge(connectionState, %{security_parameters:
                                 securityParameters,
                                   cipher_state:
                                   cipher_init(bulkCipherAlgo, key, iV,
                                                 finishedKey)})
  end
  ...
  def update_start_state(state, map) do
    ...
    peerPublicKey = :maps.get(:peer_public_key, map,
                                :undefined)
  ...
  defp update_start_state(r_state(connection_states: connectionStates0,
              handshake_env: r_handshake_env() = hsEnv, static_env: r_static_env(role: role),
              connection_env: cEnv, session: session) = state,
            cipher, keyShare, sessionId, group, selectedSignAlg,
            peerPublicKey, aLPNProtocol, random) do
    %{security_parameters:
      secParamsR0} = (pendingRead = :maps.get(:pending_read,
                                                connectionStates0))
    %{security_parameters:
      secParamsW0} = (pendingWrite = :maps.get(:pending_write,
                                                 connectionStates0))
    secParamsR1 = :ssl_cipher.security_parameters_1_3(secParamsR0,
                                                        cipher)
    secParamsW1 = :ssl_cipher.security_parameters_1_3(secParamsW0,
                                                        cipher)
    secParamsR = update_random(role, secParamsR1, random)
    secParamsW = update_random(role, secParamsW1, random)
    connectionStates = Map.merge(connectionStates0, %{pending_read:
                                                      Map.put(pendingRead, :security_parameters,
                                                                             secParamsR),
                                                        pending_write:
                                                        Map.put(pendingWrite, :security_parameters,
                                                                                secParamsW)})
    r_state(state, connection_states: connectionStates, 
               handshake_env: r_handshake_env(hsEnv, alpn: aLPNProtocol), 
               key_share: keyShare, 
               session: r_session(session, session_id: sessionId,  ecc: group, 
  ...
  defp update_resumption_master_secret(r_state(connection_states: connectionStates0) = state,
            resumptionMasterSecret) do
    %{security_parameters:
      secParamsR0} = (pendingRead = :maps.get(:pending_read,
                                                connectionStates0))
    %{security_parameters:
      secParamsW0} = (pendingWrite = :maps.get(:pending_write,
                                                 connectionStates0))
    secParamsR = r_security_parameters(secParamsR0, resumption_master_secret: resumptionMasterSecret)
    secParamsW = r_security_parameters(secParamsW0, resumption_master_secret: resumptionMasterSecret)
    connectionStates = Map.merge(connectionStates0, %{pending_read:
                                                      Map.put(pendingRead, :security_parameters,
                                                                             secParamsR),
                                                        pending_write:
  ...
  defp cipher_init(11, key, iV, finishedKey) do
    r_cipher_state(key: key, iv: iV, finished_key: finishedKey,
        tag_len: 8)
  end
  ...
  def verify_signature_algorithm(r_state(static_env: r_static_env(role: role),
             ssl_options: %{signature_algs: localSignAlgs}) = state0,
           r_certificate_verify_1_3(algorithm: peerSignAlg)) do
    case (:lists.member(peerSignAlg,
                          filter_tls13_algs(localSignAlgs))) do
      true ->
        {:ok,
           maybe_update_selected_sign_alg(state0, peerSignAlg,
                                            role)}
      false ->
        state1 = calculate_traffic_secrets(state0)
        state = :ssl_record.step_encryption_state(state1)
           ...
           {r_alert(level: 2, description: 40,
                where: %{mfa:
                         {:tls_handshake_1_3, :verify_signature_algorithm, 2},
                           line: 1400, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
                reason: 'CertificateVerify uses unsupported signature algorithm'),
              state}}
    end
  end

  defp maybe_update_selected_sign_alg(r_state(session: session) = state, signAlg,
            :client) do
  ...
  def select_common_groups(_, []) do
    {:error,
       r_alert(level: 2, description: 71,
           where: %{mfa:
                    {:tls_handshake_1_3, :select_common_groups, 2},
                      line: 1432, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
  ...
  def check_cert_sign_algo(signAlgo, signHash, clientSignAlgs,
           :undefined) do
    do_check_cert_sign_algo(signAlgo, signHash,
                              clientSignAlgs)
  end
  ...
  def select_sign_algo(:dsa, _RSAKeySize, _CertSignAlg, _OwnSignAlgs,
           _Curve) do
    {:error,
       r_alert(level: 2, description: 71,
           where: %{mfa:
                    {:tls_handshake_1_3, :select_sign_algo, 5},
                      line: 1464, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
  ...
  def select_sign_algo(_, _RSAKeySize, [], _, _) do
    {:error,
       r_alert(level: 2, description: 71,
           where: %{mfa:
                    {:tls_handshake_1_3, :select_sign_algo, 5},
                      line: 1466, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
  ...
  def select_sign_algo(_, _RSAKeySize, :undefined, _OwnSignAlgs, _) do
    {:error,
       r_alert(level: 2, description: 71,
           where: %{mfa:
                    {:tls_handshake_1_3, :select_sign_algo, 5},
                      line: 1468, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
           reason: :no_suitable_public_key)}
  end

  def select_sign_algo(publicKeyAlgo, rSAKeySize,
           [certSignAlg | certSignAlgs], ownSignAlgs, curve) do
    {_, s,
       _} = :ssl_cipher.scheme_to_components(certSignAlg)
    case ((publicKeyAlgo === :rsa and s === :rsa_pss_rsae or publicKeyAlgo === :rsa_pss_pss and s === :rsa_pss_pss or publicKeyAlgo === :ecdsa and s === :ecdsa or publicKeyAlgo === :eddsa and s === :eddsa) and :lists.member(certSignAlg,
                                                                                                                                                                                                                                  ownSignAlgs)) do
      true ->
        validate_key_compatibility(publicKeyAlgo, rSAKeySize,
                                     [certSignAlg | certSignAlgs], ownSignAlgs,
                                     curve)
      false ->
        select_sign_algo(publicKeyAlgo, rSAKeySize,
                           certSignAlgs, ownSignAlgs, curve)
    end
  end

  defp validate_key_compatibility(publicKeyAlgo, rSAKeySize,
            [certSignAlg | certSignAlgs], ownSignAlgs, curve)
      when publicKeyAlgo === :rsa or publicKeyAlgo === :rsa_pss_pss do
    {hash, sign,
       _} = :ssl_cipher.scheme_to_components(certSignAlg)
    case ((sign === :rsa_pss_rsae or sign === :rsa_pss_pss) and is_rsa_key_compatible(rSAKeySize,
                                                                                        hash)) do
      true ->
        {:ok, certSignAlg}
      false ->
  ...
  defp validate_key_compatibility(publicKeyAlgo, rSAKeySize,
    ...
    {_, sign,
       peerCurve} = :ssl_cipher.scheme_to_components(certSignAlg)
  ...
  defp do_check_cert_sign_algo(_, _, :undefined) do
    {:error,
       r_alert(level: 2, description: 71,
           where: %{mfa:
                    {:tls_handshake_1_3, :do_check_cert_sign_algo, 3},
                      line: 1535, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
           reason: :no_suitable_signature_algorithm)}
  end
  ...
  defp do_check_cert_sign_algo(_, _, []) do
    {:error,
       r_alert(level: 2, description: 71,
           where: %{mfa:
                    {:tls_handshake_1_3, :do_check_cert_sign_algo, 3},
                      line: 1537, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'},
           reason: :no_suitable_signature_algorithm)}
  end

  defp do_check_cert_sign_algo(signAlgo, signHash, [scheme | t]) do
    {hash, sign,
       _Curve} = :ssl_cipher.scheme_to_components(scheme)
    case (compare_sign_algos(signAlgo, signHash, sign,
                               hash)) do
      true ->
        :ok
      _Else ->
  ...
  def get_certificate_params(cert) do
    {signAlgo0, param, subjectPublicKeyAlgo0, rSAKeySize,
       curve} = :ssl_handshake.get_cert_params(cert)
  ...
  defp oids_to_atoms({1, 2, 840, 113549, 1, 1, 10},
  ...
  defp public_key_algo({1, 2, 840, 113549, 1, 1, 10}) do
    :rsa_pss_pss
  end

  defp public_key_algo({1, 2, 840, 113549, 1, 1, 1}) do
    :rsa
  end
  ...
  def get_signature_scheme_list(r_signature_algorithms_cert(signature_scheme_list: clientSignatureSchemes)) do
    clientSignatureSchemes
  end

  def get_signature_scheme_list(r_signature_algorithms(signature_scheme_list: clientSignatureSchemes)) do
    :lists.filter(fn e ->
  ...
  def handle_pre_shared_key(r_state(ssl_options: %{session_tickets: :disabled}), _,
           _) do
    {:ok, :undefined}
  end

  def handle_pre_shared_key(r_state(ssl_options: %{session_tickets: tickets},
             handshake_env: r_handshake_env(tls_handshake_history: {hHistory, _}),
             static_env: r_static_env(trackers: trackers)),
           r_pre_shared_key_client_hello(offered_psks: offeredPreSharedKeys), cipher)
      when tickets !== :disabled do
    tracker = :proplists.get_value(:session_tickets_tracker,
                                     trackers)
    %{prf:
      cipherHash} = :ssl_cipher_format.suite_bin_to_map(cipher)
    :tls_server_session_ticket.use(tracker,
                                     offeredPreSharedKeys, cipherHash, hHistory)
  end
  ...
  def maybe_add_binders(hello0, ticketData, {3, 4} = version) do
    helloBin0 = :tls_handshake.encode_handshake(hello0,
                                                  version)
  ...
  def maybe_add_binders(hello0, {[hRR, messageHash | _], _}, ticketData,
           {3, 4} = version) do
    helloBin0 = :tls_handshake.encode_handshake(hello0,
                                                  version)
    helloBin1 = :erlang.iolist_to_binary(helloBin0)
    truncated = truncate_client_hello(helloBin1)
    binders = create_binders([messageHash, hRR, truncated],
                               ticketData)
    update_binders(hello0, binders)
  end
  ...
  defp create_binders(context,
            [r_ticket_data(psk: pSK, cipher_suite: {_, hKDF}) | t], acc) do
  ...
  defp truncate_client_hello(helloBin0) do
    <<type :: size(8) - unsigned - big - integer,
        _Length :: size(24) - unsigned - big - integer,
        body :: binary>> = helloBin0
    cH0 = (r_client_hello(extensions: %{pre_shared_key:
                           pSK0} = extensions0) = :tls_handshake.decode_handshake({3,
                                                                                     4},
                                                                                    type,
                                                                                    body))
    ...
    truncatedSize = :erlang.iolist_size(:tls_handshake.encode_handshake(cH,
                                                                          {3,
                                                                             4}))
    refSize = :erlang.iolist_size(:tls_handshake.encode_handshake(cH0,
                                                                    {3, 4}))
    bindersSize = refSize - truncatedSize
    {truncated, _} = :erlang.split_binary(helloBin0,
                                            byte_size(helloBin0) - bindersSize - 2)
    truncated
  end

  def maybe_add_early_data_indication(r_client_hello(extensions: extensions0) = clientHello,
           earlyData, {3, 4})
  ...
  def supported_groups_from_extensions(extensions) do
    case (:maps.get(:elliptic_curves, extensions,
                      :undefined)) do
      ...
      r_elliptic_curves() ->
        {:error,
           r_alert(level: 2, description: 47,
               where: %{mfa:
                        {:tls_handshake_1_3, :supported_groups_from_extensions,
                           1},
                          line: 1741, file: 'otp/lib/ssl/src/tls_handshake_1_3.erl'})}
      :undefined ->
        {:ok, :undefined}
    end
  end
  ...
  defp calculate_finished_key(pSK, hKDFAlgo) do
    earlySecret = :tls_v1.key_schedule(:early_secret,
                                         hKDFAlgo, {:psk, pSK})
    pRK = :tls_v1.resumption_binder_key(hKDFAlgo,
                                          earlySecret)
    :tls_v1.finished_key(pRK, hKDFAlgo)
  end

  defp calculate_binder(binderKey, hKDF, truncated) do
    :tls_v1.finished_verify_data(binderKey, hKDF,
                                   [truncated])
  end

  defp update_binders(r_client_hello(extensions: %{pre_shared_key:
                          preSharedKey0} = extensions0) = hello,
            binders) do
    r_pre_shared_key_client_hello(offered_psks: r_offered_psks(identities: identities)) = preSharedKey0
    preSharedKey = r_pre_shared_key_client_hello(offered_psks: r_offered_psks(identities: identities,
                                       binders: binders))
    extensions = Map.put(extensions0, :pre_shared_key,
                                        preSharedKey)
  ...
  def ciphers_for_early_data(cipherSuites0) do
    :lists.filtermap(&ciphers_for_early_data0/1,
                       cipherSuites0)
  end

  defp ciphers_for_early_data0(cipherSuite) do
    case (:lists.member(cipherSuite,
                          :tls_v1.exclusive_suites({3, 4}))) do
      true ->
        {true,
           :maps.get(:cipher,
  ...
  defp process_ticket(%{cipher_suite: cipherSuite, sni: _SNI,
              psk: pSK, timestamp: timestamp,
              ticket: newSessionTicket},
            n) do
    r_new_session_ticket(ticket_lifetime: _LifeTime, ticket_age_add: ageAdd,
        ticket_nonce: nonce, ticket: ticket,
        extensions: extensions) = newSessionTicket
    ticketAge = :erlang.system_time(:millisecond) - timestamp
    obfuscatedTicketAge = obfuscate_ticket_age(ticketAge,
                                                 ageAdd)
    identity = r_psk_identity(identity: ticket,
                   obfuscated_ticket_age: obfuscatedTicketAge)
    maxEarlyData = get_max_early_data(extensions)
    r_ticket_data(key: :undefined, pos: n, identity: identity, psk: pSK,
        nonce: nonce, cipher_suite: cipherSuite,
  ...
  def get_max_early_data(extensions) do
    earlyDataIndication = :maps.get(:early_data, extensions,
                                      :undefined)
  ...
  def path_validation(trustedCert, path, serverName, role,
           ...
           %{verify_fun: verifyFun,
               ...
               crl_check: crlCheck, log_level: logLevel,
               signature_algs: signAlgos,
               signature_algs_cert: signAlgosCert} = opts,
           %{cert_ext: certExt,
               ocsp_responder_certs: ocspResponderCerts,
               ocsp_state: ocspState}) do
    validationFunAndState = :ssl_handshake.validation_fun_and_state(verifyFun,
                                                                      %{role:
                                                                        role,
                                                                          certdb:
                                                                          certDbHandle,
  ...
  defp select_client_cert_key_pair(session0,
            ...
            certDbHandle, certDbRef, certAuths, plausible0) do
    {publicKeyAlgo, signAlgo, signHash, maybeRSAKeySize,
       curve} = get_certificate_params(cert)
    case (select_sign_algo(publicKeyAlgo, maybeRSAKeySize,
                             serverSignAlgs, clientSignAlgs, curve)) do
      {:ok, selectedSignAlg} ->
        case (check_cert_sign_algo(signAlgo, signHash,
                                     serverSignAlgs, serverSignAlgsCert)) do
          :ok ->
            case (:ssl_certificate.handle_cert_auths(certs,
                                                       certAuths, certDbHandle,
                                                       certDbRef)) do
              {:ok, encodedChain} ->
                r_session(session0, sign_alg: selectedSignAlg, 
                              own_certificates: encodedChain,  private_key: key)
              {:error, encodedChain, :not_in_auth_domain} ->
                plausible = plausible_missing_chain(encodedChain,
                                                      plausible0,
                                                      selectedSignAlg, key,
                                                      session0)
                select_client_cert_key_pair(session0, rest,
                                              serverSignAlgs,
                                              serverSignAlgsCert,
                                              clientSignAlgs, certDbHandle,
                                              certDbRef, certAuths, plausible)
            end
          _ ->
            select_client_cert_key_pair(session0, rest,
                                          serverSignAlgs, serverSignAlgsCert,
                                          clientSignAlgs, certDbHandle,
                                          certDbRef, certAuths, plausible0)
        end
      {:error, _} ->
        select_client_cert_key_pair(session0, rest,
                                      serverSignAlgsCert, serverSignAlgsCert,
                                      clientSignAlgs, certDbHandle, certDbRef,
                                      certAuths, plausible0)
    end
  end

  defp plausible_missing_chain([_] = encodedChain, :undefined, signAlg, key,
            session0) do
    r_session(session0, sign_alg: signAlg, 
                  own_certificates: encodedChain,  private_key: key)
  end
