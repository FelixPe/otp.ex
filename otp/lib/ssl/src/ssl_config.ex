defmodule :m_ssl_config do
  ...
  def init(sslOpts, role) do
    ...
    %{pem_cache: pemCache} = (config = init_cacerts(sslOpts,
                                                      role))
    dHParams = init_diffie_hellman(pemCache, sslOpts, role)
    certKeyAlts = init_certs_keys(sslOpts, role, pemCache)
    {:ok,
       Map.merge(config, %{cert_key_alts: certKeyAlts,
                             dh_params: dHParams})}
  end

  defp init_certs_keys(%{certs_keys: certsKeys} = opts, role,
            pemCache) do
    pairs = :lists.map(fn certKey ->
  ...
  defp init_cert_key_pair(certKey, role, pemCache) do
    ...
    privateKey = init_private_key(:maps.get(:key, certKey,
                                              :undefined),
                                    certKey, pemCache)
  ...
  defp group_pairs([%{private_key:
             r_ECPrivateKey(parameters: {:namedCurve, {1, 3, 101, 112}})} = pair |
               rest],
            %{eddsa: eDDSA} = group) do
    group_pairs(rest,
                  Map.put(group, :eddsa, [pair | eDDSA]))
  end

  defp group_pairs([%{private_key:
             r_ECPrivateKey(parameters: {:namedCurve, {1, 3, 101, 113}})} = pair |
               rest],
            %{eddsa: eDDSA} = group) do
    group_pairs(rest,
                  Map.put(group, :eddsa, [pair | eDDSA]))
  ...
  defp group_pairs([%{private_key: r_ECPrivateKey()} = pair | rest],
            %{ecdsa: eCDSA} = group) do
    group_pairs(rest,
                  Map.put(group, :ecdsa, [pair | eCDSA]))
  end

  defp group_pairs([%{private_key: {r_RSAPrivateKey(), r_RSASSA_PSS_params()}} = pair | rest],
            %{rsa_pss_pss: rSAPSS} = group) do
    group_pairs(rest,
                  Map.put(group, :rsa_pss_pss, [pair | rSAPSS]))
  end

  defp group_pairs([%{private_key: r_RSAPrivateKey()} = pair | rest],
            %{rsa: rSA} = group) do
    group_pairs(rest, Map.put(group, :rsa, [pair | rSA]))
  end

  defp group_pairs([%{private_key: r_DSAPrivateKey()} = pair | rest],
            %{dsa: dSA} = group) do
    group_pairs(rest, Map.put(group, :dsa, [pair | dSA]))
  end

  defp group_pairs([%{private_key:
             %{algorithm: :dss, engine: _}} = pair |
               rest],
            group) do
  ...
  defp prioritize_groups(%{eddsa: eDDSA, ecdsa: eCDSA,
              rsa_pss_pss: rSAPSS, rsa: rSA,
              dsa: dSA} = certKeyGroups,
            opts) do
    eC = ecdsa_support(opts)
    Map.merge(certKeyGroups, %{eddsa: prio_eddsa(eDDSA),
                                 ecdsa: prio_ecdsa(eCDSA, eC),
                                 rsa_pss_pss: prio_rsa_pss(rSAPSS),
                                 rsa: prio_rsa(rSA), dsa: prio_dsa(dSA)})
  end

  defp prio_eddsa(eDDSA) do
    using_curve({:namedCurve, {1, 3, 101, 112}}, eDDSA,
                  []) ++ using_curve({:namedCurve, {1, 3, 101, 113}},
                                       eDDSA, [])
  end

  defp prio_ecdsa(eCDSA, curves) do
    enginePairs = (for (pair = %{private_key:
                                 %{engine: _}}) <- eCDSA do
                     pair
                   end)
    enginePairs ++ :lists.foldr(fn curve, accIn ->
                                     curveOid = :pubkey_cert_records.namedCurves(curve)
                                     pairs = using_curve({:namedCurve,
                                                            curveOid},
                                                           eCDSA -- enginePairs,
                                                           [])
                                     pairs ++ accIn
  ...
  defp prio_rsa_pss(rSAPSS) do
    order = fn %{privat_key: {r_RSAPrivateKey(modulus: n), params1}},
  ...
  defp prio_params_1(r_RSASSA_PSS_params(hashAlgorithm: r_HashAlgorithm(algorithm: oid1)),
            r_RSASSA_PSS_params(hashAlgorithm: r_HashAlgorithm(algorithm: oid2))) do
    :public_key.pkix_hash_type(oid1) > :public_key.pkix_hash_type(oid2)
  end

  defp prio_rsa(rSA) do
    order = fn %{key: r_RSAPrivateKey(modulus: n)}, %{key: r_RSAPrivateKey(modulus: m)}
                   when m > n ->
                 true
               %{private_key: %{engine: _}}, _ ->
                 true
  ...
  defp prio_dsa(dSA) do
    order = fn %{key: r_DSAPrivateKey(q: n)}, %{key: r_DSAPrivateKey(q: m)} when m > n
                                                     ->
                 true
               %{private_key: %{engine: _}}, _ ->
                 true
  ...
  def get_ticket_lifetime() do
    case (:application.get_env(:ssl,
                                 :server_session_ticket_lifetime)) do
      {:ok, seconds}
          when is_integer(seconds) and seconds <= 604800 ->
        seconds
      _ ->
        7200
    end
  ...
  def get_ticket_store_size() do
    case (:application.get_env(:ssl,
                                 :server_session_ticket_store_size)) do
  ...
  def get_max_early_data_size() do
    case (:application.get_env(:ssl,
                                 :server_session_ticket_max_early_data)) do
      {:ok, size} when is_integer(size) ->
        size
      _ ->
        16384
  ...
  defp init_cacerts(%{cacerts: caCerts, crl_cache: cRLCache} = opts,
            role) do
    cACertFile = :maps.get(:cacertfile, opts, <<>>)
    {:ok, config} = (try do
                       certs = (case (caCerts) do
                                  :undefined ->
                                    cACertFile
                                  _ ->
                                    {:der, caCerts}
                                end)
                       {:ok, _} = :ssl_manager.connection_init(certs, role,
                                                                 cRLCache)
                     catch
                       _, reason ->
  ...
  defp init_certificates(certKey, pemCache, role) do
    case (:maps.get(:cert, certKey, :undefined)) do
      :undefined ->
        init_certificate_file(:maps.get(:certfile, certKey,
                                          <<>>),
                                pemCache, role)
  ...
  defp init_private_key(%{algorithm: alg} = key, _, _PemCache)
    ...
    case (:maps.is_key(:engine,
                         key) and :maps.is_key(:key_id, key)) do
      true ->
        key
      false ->
  ...
  defp init_private_key({asn1Type, privateKey}, _, _) do
    private_key(:public_key.der_decode(asn1Type,
                                         privateKey))
  end
      ...
      keyFile ->
        ...
        try do
          {:ok, list} = :ssl_manager.cache_pem_file(keyFile,
                                                      dbHandle)
          [pemEntry] = (for (pemEntry = {pKey, _, _}) <- list,
                              pKey === :RSAPrivateKey or pKey === :DSAPrivateKey or pKey === :ECPrivateKey or pKey === :PrivateKeyInfo do
                          pemEntry
                        end)
  ...
  defp private_key(r_PrivateKeyInfo(privateKeyAlgorithm: r_PrivateKeyInfo_privateKeyAlgorithm(algorithm: {1, 2, 840,
                                                 113549, 1, 1, 1}),
              privateKey: key)) do
  ...
  defp private_key(r_PrivateKeyInfo(privateKeyAlgorithm: r_PrivateKeyInfo_privateKeyAlgorithm(algorithm: {1, 2, 840,
                                                 10040, 4, 1}),
              privateKey: key)) do
    :public_key.der_decode(:DSAPrivateKey,
                             :erlang.iolist_to_binary(key))
  end
  ...
  defp private_key(r_PrivateKeyInfo(privateKeyAlgorithm: r_PrivateKeyInfo_privateKeyAlgorithm(algorithm: {1, 2, 840,
              ...
              privateKey: key)) do
    eCKey = :public_key.der_decode(:ECPrivateKey,
                                     :erlang.iolist_to_binary(key))
    eCParameters = :public_key.der_decode(:EcpkParameters,
                                            parameters)
  ...
  defp file_error(file, throw) do
    case (throw) do
      {opt, {:badmatch, {:error, {:badmatch, error}}}} ->
        throw({:options,
                 {opt, :erlang.binary_to_list(file), error}})
      {opt, {:badmatch, error}} ->
        throw({:options,
                 {opt, :erlang.binary_to_list(file), error}})
      _ ->
    ...
    case (:maps.get(:dh, opts, :undefined)) do
      ...
      _ ->
        case (:maps.get(:dhfile, opts, :undefined)) do
          :undefined ->
            r_DHParameter(prime: :ssl_dh_groups.modp2048_prime(),
  ...
  defp session_cb_init_args(:client) do
    case (:application.get_env(:ssl,
                                 :client_session_cb_init_args)) do
      :undefined ->
        case (:application.get_env(:ssl,
  ...
  defp session_cb_init_args(:server) do
    case (:application.get_env(:ssl,
                                 :server_session_cb_init_args)) do
      :undefined ->
        case (:application.get_env(:ssl,
                                     :session_cb_init_args)) do
          {:ok, args} when is_list(args) ->
  ...
  defp session_cb_opts(:client = role) do
    case (:application.get_env(:ssl, :session_cb,
                                 :ssl_client_session_cache_db)) do
  ...
  defp session_cb_opts(:server = role) do
    case (:application.get_env(:ssl, :session_cb,
                                 :ssl_server_session_cache_db)) do
