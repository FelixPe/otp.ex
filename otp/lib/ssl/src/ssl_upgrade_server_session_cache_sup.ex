defmodule :m_ssl_upgrade_server_session_cache_sup do
  use Bitwise
  ...
  def start_link() do
    :supervisor.start_link({:local, sup_name(:normal)}, :ssl_upgrade_server_session_cache_sup, [])
  end

  def start_link_dist() do
    :supervisor.start_link({:local, sup_name(:dist)}, :ssl_upgrade_server_session_cache_sup, [])
  end
    ...
    case workers do
      0 ->
        case :supervisor.start_child(
               supName,
               [:ssl_unknown_listener, :ssl_config.pre_1_3_session_opts(:server)]
             ) do
  ...
  def init(_) do
    supFlags = %{strategy: :simple_one_for_one, intensity: 3, period: 3600}
  ...
  defp sup_name(:normal) do
    :ssl_upgrade_server_session_cache_sup
  end

  defp sup_name(:dist) do
    :erlang.list_to_atom(:erlang.atom_to_list(:ssl_upgrade_server_session_cache_sup) ++ ~c"_dist")
  end
end
