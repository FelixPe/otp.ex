defmodule :m_tls_v1 do
  ...
  def finished(role, {3, 3}, prfAlgo, masterSecret, handshake) do
  ...
  def setup_keys(
    ...
    keyBlock =
      prf(4711, masterSecret, ~c"key expansion", [serverRandom, clientRandom], wantedLength)
  ...
  def setup_keys(
    ...
    keyBlock =
      prf(prfAlgo, masterSecret, ~c"key expansion", [serverRandom, clientRandom], wantedLength)
  ...
  def mac_hash(method, mac_write_secret, seq_num, type, version, length, fragment) do
    {major, minor} = version
  ...
  def default_signature_algs([{3, 4}]) do
    default_signature_schemes({3, 4}) ++ legacy_signature_schemes({3, 4})
  end

  def default_signature_algs([{3, 4}, {3, 3} | _]) do
    default_signature_schemes({3, 4}) ++
      legacy_signature_schemes({3, 4}) ++ default_pre_1_3_signature_algs_only()
  end
  ...
  defp default_pre_1_3_signature_algs_only() do
    default = [{:sha512, :ecdsa}, {:sha384, :ecdsa}, {:sha256, :ecdsa}]
      ...
      scheme, acc when is_atom(scheme) ->
        ...
        case (:proplists.get_bool(
                ...
                is_pair(
                  ...
                  sign,
                  hashes
                )) or (sign == :eddsa and (curve == :ed448 or curve == :ed25519)) do
          true ->
            [scheme | acc]
  ...
  defp legacy_signature_schemes(version) do
    legacySchemes = [:rsa_pkcs1_sha512, :rsa_pkcs1_sha384, :rsa_pkcs1_sha256]
  ...
  defp supported_groups() do
    :crypto.supports(:curves) ++ [:ffdhe2048, :ffdhe3072, :ffdhe4096, :ffdhe6144, :ffdhe8192]
  end
  ...
  def handle_trace(
        :kdt,
        {:call, {:tls_v1, :update_traffic_secret, [_HKDF, applicationTrafficSecret0]}},
        stack
      ) do
    aTS0 =
      :string.sub_string(:binary.bin_to_list(:binary.encode_hex(applicationTrafficSecret0)), 1, 5) ++
        ~c"..."

    {:io_lib.format(~c"ApplicationTrafficSecret0 = \"~s\"", [aTS0]), stack}
  end

  def handle_trace(
        :kdt,
        {:return_from, {:tls_v1, :update_traffic_secret, 2}, return},
        stack
      ) do
    aTS = :string.sub_string(:binary.bin_to_list(:binary.encode_hex(return)), 1, 5) ++ ~c"..."
    {:io_lib.format(~c"ApplicationTrafficSecret = \"~s\"", [aTS]), stack}
  end
end
