defmodule :m_tls_connection_1_3 do
  ...
  def init([role, sender, host, port, socket, options, user, cbInfo]) do
    ...
    try do
      state = :ssl_gen_statem.ssl_config(r_state(state0, :ssl_options), role, state0)
      :tls_gen_connection.initialize_tls_sender(state)
      :gen_statem.enter_loop(:tls_connection_1_3, [], :initial_hello, state)
    catch
      error ->
        eState = r_state(state0, protocol_specific: Map.put(map, :error, error))
        :gen_statem.enter_loop(:tls_connection_1_3, [], :config_error, eState)
    end
  end
  ...
  def user_hello({:call, from}, :cancel, state) do
    ...
    :ssl_gen_statem.handle_own_alert(
      r_alert(
        ...
        description: 90,
        where: %{
          mfa: {:tls_connection_1_3, :user_hello, 3},
          line: 244,
          file: ~c"otp/lib/ssl/src/tls_connection_1_3.erl"
        },
  ...
  def user_hello(
        ...
        r_state(
          static_env: r_static_env(role: :client = role),
          handshake_env: hSEnv,
  ...
  def user_hello(
        ...
        {:handshake_continue, newOptions, timeout},
        r_state(
          static_env: r_static_env(role: :server = role),
    ...
    case :ssl_handshake.select_supported_version(
      ...
      _Else ->
        {:next_state, :hello,
         ...
         [{:change_callback_module, :tls_connection}, {{:timeout, :handshake}, timeout, :close}]}
    end
  end
      ...
      false ->
        :ssl_gen_statem.handle_own_alert(
          r_alert(
            ...
            description: 70,
            where: %{
              mfa: {:tls_connection_1_3, :start, 3},
              line: 301,
              file: ~c"otp/lib/ssl/src/tls_connection_1_3.erl"
            }
          ),
  ...
  def wait_finished(:internal = type, r_change_cipher_spec() = msg, state) do
    handle_change_cipher_spec(type, msg, :wait_finished, state)
  end
  ...
  def hello_retry_middlebox_assert(:internal, r_server_hello(), state) do
    :tls_gen_connection.next_event(:hello_retry_middlebox_assert, :no_record, state, [:postpone])
  end
  ...
  def wait_cert_cr(:internal = type, r_change_cipher_spec() = msg, state) do
    handle_change_cipher_spec(type, msg, :wait_cert_cr, state)
  end
  ...
  defp initial_state(
    ...
    %{erl_dist: isErlDist, versions: [version | _], client_renegotiation: clientRenegotiation} =
      sSLOptions
      ...
      {:error, reason} ->
        {:error, state1,
         r_alert(
           ...
           description: 80,
           where: %{
             mfa: {:tls_connection_1_3, :handle_key_update, 2},
             line: 714,
             file: ~c"otp/lib/ssl/src/tls_connection_1_3.erl"
           },
           reason: reason
  ...
  defp handle_change_cipher_spec(type, msg, stateName, r_state(protocol_specific: pS0) = state) do
