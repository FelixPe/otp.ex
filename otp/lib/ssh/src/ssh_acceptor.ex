defmodule :m_ssh_acceptor do
  ...
  def start_link(systemSup, address, options) do
    :proc_lib.start_link(:ssh_acceptor, :acceptor_init,
                           [self(), systemSup, address, options])
  end
  ...
  def listen(port, options) do
    {_, callback, _} = :ssh_options.get_value(:user_options,
                                                :transport, options,
                                                :ssh_acceptor, 49)
    sockOpts = [{:active, false}, {:reuseaddr, true} |
  ...
  def acceptor_init(parent, systemSup,
           r_address(address: address, port: port, profile: _Profile),
           opts) do
    acceptTimeout = :ssh_options.get_value(:internal_options,
                                             :timeout, opts,
                                             fn () ->
                                                  5000
                                             end,
                                             :ssh_acceptor, 76)
    case (:ssh_options.get_value(:internal_options,
                                   :lsocket, opts,
                                   fn () ->
                                        :undefined
                                   end,
        ...
        case (:inet.sockname(lSock)) do
          ...
          {:error, _Error} ->
            case (try_listen(port, opts, 4)) do
              {:ok, newLSock} ->
                :proc_lib.init_ack(parent, {:ok, self()})
                opts1 = :ssh_options.delete_key(:internal_options,
                                                  :lsocket, opts, :ssh_acceptor,
                                                  93)
                acceptor_loop(port, address, opts1, newLSock,
                                acceptTimeout, systemSup)
              {:error, error} ->
  ...
  def acceptor_loop(port, address, opts, listenSocket, acceptTimeout,
           systemSup) do
    try do
      case (accept(listenSocket, acceptTimeout, opts)) do
        {:ok, socket} ->
          peerName = :inet.peername(socket)
          maxSessions = :ssh_options.get_value(:user_options,
                                                 :max_sessions, opts,
                                                 :ssh_acceptor, 137)
          numSessions = number_of_connections(systemSup)
          parallelLogin = :ssh_options.get_value(:user_options,
                                                   :parallel_login, opts,
                                                   :ssh_acceptor, 139)
          case (handle_connection(address, port, peerName, opts,
                                    socket, maxSessions, numSessions,
                                    parallelLogin)) do
            {:error, error} ->
              (try do
                close(socket, opts)
      ...
      class, err ->
        handle_error({:error,
                        {:unhandled, class, err, __STACKTRACE__}},
                       address, port, :undefined)
    end
    :ssh_acceptor.acceptor_loop(port, address, opts,
                                  listenSocket, acceptTimeout, systemSup)
  end

  defp handle_connection(_Address, _Port, _Peer, _Options, _Socket,
            maxSessions, numSessions, _ParallelLogin)
      when numSessions >= maxSessions do
    {:error, {:max_sessions, maxSessions}}
  end

  defp handle_connection(_Address, _Port, {:error, error}, _Options,
            _Socket, _MaxSessions, _NumSessions, _ParallelLogin) do
    {:error, error}
  end

  defp handle_connection(address, port, _Peer, options, socket,
  ...
  defp handle_error(reason, toAddress, toPort,
            {:ok, {fromIP, fromPort}}) do
    handle_error(reason, toAddress, toPort, fromIP,
                   fromPort)
  end

  defp handle_error(reason, toAddress, toPort, _) do
    handle_error(reason, toAddress, toPort, :undefined,
                   :undefined)
  end

  defp handle_error(reason, toAddress, toPort, fromAddress,
            fromPort) do
    case (reason) do
      {:max_sessions, maxSessions} ->
        :error_logger.info_report(:lists.concat(['Ssh login attempt to ',
                                                     :ssh_lib.format_address_port(toAddress,
                                                                                    toPort),
                                                         ' from ',
                                                             :ssh_lib.format_address_port(fromAddress,
                                                                                            fromPort),
                                                                 ' denied due to option max_sessions limits to ', maxSessions,
                                                                        ' sessions.']))
      limit when limit == :enfile or limit == :emfile ->
        :error_logger.info_report([:erlang.atom_to_list(limit),
                                       ': out of accept sockets on ',
                                           :ssh_lib.format_address_port(toAddress,
                                                                          toPort),
                                               ' - retrying'])
        :timer.sleep(200)
      :closed ->
        :error_logger.info_report(['The ssh accept socket on ',
                                       :ssh_lib.format_address_port(toAddress,
                                                                      toPort),
      ...
      error when is_list(error) ->
        :ok
      error when (fromAddress !== :undefined and
                    fromPort !== :undefined)
                 ->
        :error_logger.info_report(['Accept failed on ',
                                       :ssh_lib.format_address_port(toAddress,
                                                                      toPort),
                                           ' for connect from ',
                                               :ssh_lib.format_address_port(fromAddress,
  ...
  def number_of_connections(sysSupPid) do
    :lists.foldl(fn {_Ref, _Pid, :supervisor,
                       [:ssh_subsystem_sup]},
                      n ->
                      n + 1
                    _, n ->
                   ...
                   0, :supervisor.which_children(sysSupPid))
  end
  ...
  def ssh_dbg_format(:tcp,
           {:return_from, {:ssh_acceptor, :listen, 2},
              {:ok, sock}},
           [{:port, port} | stack]) do
    {['TCP listener started\n', :io_lib.format('Port: ~p~nListeningSocket: ~p~n', [port, sock])], stack}
  end

  def ssh_dbg_format(:tcp,
           {:return_from, {:ssh_acceptor, :listen, 2}, result},
           [{:port, port} | stack]) do
    {['TCP listener start ERROR\n', :io_lib.format('Port: ~p~nReturn: ~p~n', [port, result])], stack}
  end

  def ssh_dbg_format(:tcp,
           {:call,
              {:ssh_acceptor, :accept,
                 [listenSocket, _AcceptTimeout, _Options]}},
           stack) do
    {:skip, [{:lsock, listenSocket} | stack]}
  end

  def ssh_dbg_format(:tcp,
           {:return_from, {:ssh_acceptor, :accept, 3},
              {:ok, sock}},
           [{:lsock, listenSocket} | stack]) do
    {['TCP accept\n', :io_lib.format('ListenSock: ~p~nNew Socket: ~p~n', [listenSocket, sock])], stack}
  end

  def ssh_dbg_format(:tcp,
           {:return_from, {:ssh_acceptor, :accept, 3},
              {:error, :timeout}},
           [{:lsock, _ListenSocket} | stack]) do
    {:skip, stack}
  end
  ...
  def ssh_dbg_format(:tcp,
           ...
           [{:lsock, listenSocket} | stack]) do
    {['TCP accept returned\n', :io_lib.format('ListenSock: ~p~nReturn: ~p~n', [listenSocket, return])], stack}
  end

  def ssh_dbg_format(:tcp,
           {:call, {:ssh_acceptor, :close, [socket, _Options]}}) do
    ['TCP close listen socket\n', :io_lib.format('Socket: ~p~n', [socket])]
  end

  def ssh_dbg_format(:tcp,
           {:return_from, {:ssh_acceptor, :close, 2}, _Return}) do
    :skip
  end
  ...
  def ssh_dbg_format(:connections,
           {:call,
              {:ssh_acceptor, :acceptor_init,
                 [_Parent, _SysSup, address, _Opts]}}) do
    [:io_lib.format('Starting LISTENER on ~s\n', [:ssh_lib.format_address(address)])]
  end

  def ssh_dbg_format(:connections,
           {:return_from, {:ssh_acceptor, :acceptor_init, 4},
              _Ret}) do
    :skip
  end
  ...
  def ssh_dbg_format(:connections,
           {:call,
              {:ssh_acceptor, :handle_connection,
                 [_Address, _Port, _Options, _Sock]}}) do
    :skip
  end

  def ssh_dbg_format(:connections,
           {:return_from, {:ssh_acceptor, :handle_connection, 4},
              {:error, error}}) do
