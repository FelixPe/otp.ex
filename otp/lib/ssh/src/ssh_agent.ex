defmodule :m_ssh_agent do
  ...
  def add_host_key(host, publicKey, options) do
    keyCbOpts = :proplists.get_value(:key_cb_private,
                                       options, [])
    sshFileCb = :proplists.get_value(:call_ssh_file,
                                       keyCbOpts, :ssh_file)
  ...
  def is_host_key(key, peerName, port, algorithm, opts) do
    ...
    sshFileCb = :proplists.get_value(:call_ssh_file,
                                       keyCbOpts, :ssh_file)
    sshFileCb.is_host_key(key, peerName, port, algorithm,
                            opts)
  end
  ...
  def user_key(algorithm, opts) do
    ...
    matchingKeys = :lists.filter(fn key ->
  ...
  def sign(pubKeyBlob, sigData, opts) do
    keyCbOpts = :proplists.get_value(:key_cb_private, opts,
                                       [])
    signFlags = 2 ||| 4
    signRequest = r_ssh_agent_sign_request(key_blob: pubKeyBlob, data: sigData,
                      flags: signFlags)
  ...
  defp has_key_type(r_ssh_agent_key(blob: keyBlob), type) do
    <<_KeyTypeLen :: size(32) - unsigned - big - integer,
        keyType :: size(_KeyTypeLen) - binary,
        _KeyBlobRest :: binary>> = keyBlob
    :erlang.binary_to_list(keyType) == type
  end

  def send(request, opts) do
    socketPath = :proplists.get_value(:socket_path, opts,
                                        :os.getenv('SSH_AUTH_SOCK'))
    timeout = :proplists.get_value(:timeout, opts, 1000)
    connectOpts = [:binary, {:packet, 0}, {:active, false}]
    {:ok, socket} = :gen_tcp.connect({:local, socketPath},
                                       0, connectOpts, timeout)
    ...
    {:ok,
       <<len
         ...
         size(32) - unsigned - big -
           integer>>} = :gen_tcp.recv(socket, 4, timeout)
  ...
  defp encode(r_ssh_agent_sign_request(key_blob: keyBlob, data: data,
              flags: flags)) do
  ...
  defp decode_keys(<<_KeyBlobLen
            ::
            size(32) - unsigned - big - integer,
              keyBlob :: size(_KeyBlobLen) - binary,
              _CommentLen :: size(32) - unsigned - big - integer,
              comment :: size(_CommentLen) - binary, rest :: binary>>,
            acc, n) do
    key = r_ssh_agent_key(blob: keyBlob, comment: comment)
    decode_keys(rest, [key | acc], n - 1)
  end

  defp decode_signature(<<_FormatLen
            ::
            size(32) - unsigned - big - integer,
              format :: size(_FormatLen) - binary,
              blob :: binary>>) do
    <<_SignatureBlobLen
      ::
      size(32) - unsigned - big - integer,
        signatureBlob
        ::
        size(_SignatureBlobLen) - binary>> = blob
  ...
  defp decode(<<12 :: size(8) - unsigned - big - integer,
              numKeys :: size(32) - unsigned - big - integer,
              keyData :: binary>>) do
    r_ssh_agent_identities_response(keys: decode_keys(keyData, [], numKeys))
  end

  defp decode(<<14 :: size(8) - unsigned - big - integer,
              _SignatureLen :: size(32) - unsigned - big - integer,
              signature :: size(_SignatureLen) - binary>>) do
