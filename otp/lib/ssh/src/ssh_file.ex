defmodule :m_ssh_file do
  ...
  def is_auth_key(key0, user, opts) do
    ...
    lookup_auth_keys(keyType, key, :filename.join(dir, ~c"authorized_keys"), opts) or
      lookup_auth_keys(keyType, key, :filename.join(dir, ~c"authorized_keys2"), opts)
  end
  ...
  def is_host_key(key0, hosts0, port, algorithm, opts) do
    ...
    file = :filename.join(dir, ~c"known_hosts")
  ...
  def add_host_key(hosts0, port, key, opts) do
    file = file_name(:user, ~c"known_hosts", opts)
  ...
  def decode(keyBin, type)
    ...
    case decode_ssh_file(:public, :any, keyBin, :ignore) do
      {:ok, keys} ->
  ...
  def decode(bin, :known_hosts) when is_binary(bin) do
    for l <- split_in_nonempty_lines(bin),
        [
          ...
          keyBin
          | comment
        ] <- [:binary.split(l, " ", [:global, :trim_all])] do
      attrs =
        for _ <- [:EFE_DUMMY_GEN], comment !== [] do
          {:comment,
          ...
          [
            {:hostnames,
             for hP <- :binary.split(hostPort, ",", [:global, :trim_all]) do
               :erlang.binary_to_list(hP)
             end}
          ]

      {:ssh_message.ssh2_pubkey_decode(:base64.decode(keyBin)), attrs}
    end
  end
  ...
  def decode(bin, :auth_keys) when is_binary(bin) do
    for l <- split_in_nonempty_lines(bin),
        [
          ...
          (case :binary.match(
                  l,
                  ["ssh-rsa", "rsa-sha2-", "ssh-dss", "ecdsa-sha2-nistp", "ssh-ed"]
                ) do
             :nomatch ->
               []
      ...
      attrs =
        for _ <- [:EFE_DUMMY_GEN], comment !== [] do
          ...
          for _ <- [:EFE_DUMMY_GEN], options !== [] do
            {:options, :lists.map(&:erlang.binary_to_list/1, options)}
          end
  ...
  def encode(keyAttrs, type)
      when type == :rfc4716_key or
             type == :openssh_key_v1 do
    {begin, end__, f} =
      case type do
        :rfc4716_key ->
          {~c"---- BEGIN SSH2 PUBLIC KEY ----\n", ~c"---- END SSH2 PUBLIC KEY ----\n",
  ...
  def encode(keyAttrs, type)
    ...
    :erlang.iolist_to_binary(
      for {key, attributes} <- keyAttrs do
        <<__0::size(32)-unsigned-big-integer, keyType::size(__0)-binary, _::binary>> =
          enc = :ssh_message.ssh2_pubkey_encode(key)
  ...
  def extract_public_key(
        r_ECPrivateKey(parameters: {:namedCurve, oID}, publicKey: pub0, privateKey: priv)
      )
  ...
  defp replace_localhost(~c"localhost") do
  ...
  defp ssh_dir(:system, opts) do
    :proplists.get_value(:system_dir, opts, ~c"/etc/ssh")
  end

  defp default_user_dir() do
    try do
      default_user_dir(:os.getenv(~c"HOME"))
    catch
      _, _ ->
  ...
  defp default_user_dir(home) when is_list(home) do
    userDir = :filename.join(home, ~c".ssh")
    :ok = :filelib.ensure_dir(:filename.join(userDir, ~c"dummy"))
    userDir
  end
  ...
  defp get_body(lines, expectedEndLine) do
    ...
    {:base64.mime_decode(:erlang.iolist_to_binary(keyPart)), restLines}
  end
  ...
  defp openssh_key_v1_decode(
    ...
    openssh_key_v1_decode(rest, i - 1, pwd, cipherName, kdfName, kdfOptions, n, [
      publicKey | pubKeyAcc
    ])
  end
  ...
  defp openssh_key_v1_decode_priv_keys(_Padding, 0, _N, privKeyAccRev, commentAccRev) do
  ...
  defp decrypt_openssh_key_v1(_Encrypted, kdfName, _KdfOpts, _CipherName, _Pwd) do
  ...
  defp openssh_key_v1_encode(keyPairs) do
    ...
    unEncrypted0 =
      <<checkInt::binary, checkInt::binary,
        openssh_key_v1_encode_priv_keys_cmnts(keyPairs)::binary>>

    unEncrypted = <<unEncrypted0::binary, pad(byte_size(unEncrypted0), blockSize)::binary>>
    ...
    <<"openssh-key-v1", 0, byte_size(cipherName)::size(32)-unsigned-big-integer,
      ...
      numKeys::size(32)-unsigned-big-integer, openssh_key_v1_encode_pub_keys(keyPairs)::binary,
      byte_size(encrypted)::size(32)-unsigned-big-integer, encrypted::binary>>
  end
  ...
  defp openssh_key_v1_encode_pub_keys([{priv = r_ECPrivateKey(), _Cmnt} | ks], acc) do
    ...
    openssh_key_v1_encode_pub_keys(
      ks,
      [
        <<byte_size(bk)::size(32)-unsigned-big-integer, bk::binary>>
        | acc
      ]
    )
  end
  ...
  defp openssh_key_v1_encode_priv_keys_cmnts([{k = r_ECPrivateKey(), c} | ks], acc) do
    ...
    openssh_key_v1_encode_priv_keys_cmnts(
      ks,
      [
        <<bk::binary, byte_size(c)::size(32)-unsigned-big-integer, c::binary>>
        | acc
      ]
    )
  end
  ...
  defp encrypt_openssh_key_v1(_UnEncrypted, kdfName, _KdfOptions, _CipherName, _Pwd) do
    :erlang.error({:decryption, {:not_supported, kdfName}})
  end
