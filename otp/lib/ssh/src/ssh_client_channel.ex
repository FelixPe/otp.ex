defmodule :m_ssh_client_channel do
  ...
  def handle_call({:get_print_info, :channel_cb}, _From, state) do
  ...
  defp do_the_close({:EXIT, _Pid, :shutdown = reason}, _, state) do
    {:stop, reason, state}
  end
  ...
  defp do_the_close(_Msg, _, state) do
    {:stop, :normal, state}
  end
