defmodule :m_ssh_transport do
  ...
  defp format_version({major, minor}, ~c"") do
    :lists.concat([~c"SSH-", major, ~c".", minor])
  end

  defp format_version({major, minor}, softwareVersion) do
    :lists.concat([~c"SSH-", major, ~c".", minor, ~c"-", softwareVersion])
  end

  defp software_version(options) do
    case :ssh_options.get_value(:user_options, :id_string, options, :ssh_transport, 285) do
      {:random, nlo, nup} ->
        random_id(nlo, nup)
  ...
  def handle_hello_version(version) do
    try do
      ...
      case :string.tokens(version, ~c"-") do
        [_, ~c"2.0" | _] ->
          {{2, 0}, strVersion}
  ...
  def adjust_algs_for_peer_version(~c"SSH-2.0-OpenSSH_6.2" ++ _, prefAlgs) do
  ...
  def handle_kexinit_msg(
    ...
    try do
      {:ok, algorithms} = select_algorithm(:client, own, counterPart, ssh, reNeg)
      ...
      class, error ->
        ...
        :ssh_connection_handler.disconnect(3, msg, :ssh_transport, 404)
    else
      algos ->
  ...
  def handle_kexinit_msg(
    ...
    try do
      ...
      class, error ->
        ...
        :ssh_connection_handler.disconnect(3, msg, :ssh_transport, 421)
    else
      algos ->
  ...
  defp kexinit_error(class, error, role, own, counterPart) do
    {fmt, args} =
      case {class, error} do
        {:error, {:badmatch, {false, alg}}} ->
          ...
          {~c"No common ~s algorithm,~n  we have:~n    ~s~n  peer have:~n    ~s~n",
           [
             txt,
             :lists.join(~c", ", :erlang.element(w, own)),
             :lists.join(
               ~c", ",
               :erlang.element(
                 c,
        ...
        _ ->
          {~c"Kexinit failed in ~p: ~p:~p", [role, class, error]}
      end
    ...
    try do
      ...
      _, _ ->
        :io_lib.format(~c"Kexinit failed in ~p: ~p:~p", [role, class, error])
    else
      r ->
        r
    end
  ...
  defp alg_info(~c"kex") do
    {~c"key exchange", r_ssh_msg_kexinit(:kex_algorithms), r_ssh_msg_kexinit(:kex_algorithms)}
  end

  defp alg_info(~c"hkey") do
    {~c"key", r_ssh_msg_kexinit(:server_host_key_algorithms),
     r_ssh_msg_kexinit(:server_host_key_algorithms)}
  end

  defp alg_info(~c"send_mac") do
    {~c"mac", r_ssh_msg_kexinit(:mac_algorithms_client_to_server),
     r_ssh_msg_kexinit(:mac_algorithms_server_to_client)}
  end

  defp alg_info(~c"recv_mac") do
    {~c"mac", r_ssh_msg_kexinit(:mac_algorithms_client_to_server),
     r_ssh_msg_kexinit(:mac_algorithms_server_to_client)}
  end

  defp alg_info(~c"encrypt") do
    {~c"cipher", r_ssh_msg_kexinit(:encryption_algorithms_client_to_server),
     r_ssh_msg_kexinit(:encryption_algorithms_server_to_client)}
  end

  defp alg_info(~c"decrypt") do
    {~c"cipher", r_ssh_msg_kexinit(:encryption_algorithms_client_to_server),
     r_ssh_msg_kexinit(:encryption_algorithms_server_to_client)}
  end

  defp alg_info(~c"compress") do
    {~c"compress", r_ssh_msg_kexinit(:compression_algorithms_client_to_server),
     r_ssh_msg_kexinit(:compression_algorithms_server_to_client)}
  end

  defp alg_info(~c"decompress") do
    {~c"compress", r_ssh_msg_kexinit(:compression_algorithms_client_to_server),
     r_ssh_msg_kexinit(:compression_algorithms_server_to_client)}
  end
  ...
  defp verify_kexinit_is_first_msg(r_alg(kex_strict_negotiated: true), _, :renegotiate) do
    true
  end

  defp verify_kexinit_is_first_msg(
         r_alg(kex_strict_negotiated: true),
         r_ssh(send_sequence: 1, recv_sequence: 1),
         :init
       ) do
    true
  end
  ...
  defp verify_kexinit_is_first_msg(
    ...
    :error_logger.warning_report(
      :lists.concat([~c"KEX strict violation (", sendSequence, ~c", ", recvSequence, ~c")."])
    )
  ...
  defp key_exchange_first_msg(kex, ssh0 = r_ssh(opts: opts))
    ...
    {min, nBits0, max} =
      :ssh_options.get_value(:user_options, :dh_gex_limits, opts, :ssh_transport, 528)
  ...
  def handle_kexdh_init(
    ...
    cond do
        ...
        case sign(h, signAlg, myPrivHostKey, ssh0) do
          ...
          {:error, :unsupported_sign_alg} ->
            :ssh_connection_handler.disconnect(
              3,
              :io_lib.format(~c"Unsupported algorithm ~p", [signAlg]),
              :ssh_transport,
              595
            )
        end
      ...
      true ->
        :ssh_connection_handler.disconnect(
          3,
          :io_lib.format(~c"Kexdh init failed, received 'e' out of bounds~n  E=~p~n  P=~p", [e, p]),
          :ssh_transport,
          601
        )
    end
  ...
  def handle_kexdh_reply(
    ...
    cond do
      1 <= f and f <= p - 1 ->
        ...
        case verify_host_key(ssh0, peerPubHostKey, h, h_SIG) do
          ...
          error ->
            :ssh_connection_handler.disconnect(
              3,
              :io_lib.format(~c"Kexdh init failed. Verify host key: ~p", [error]),
              :ssh_transport,
              623
            )
        end
      ...
      true ->
        :ssh_connection_handler.disconnect(
          3,
          :io_lib.format(~c"Kexdh init failed, received 'f' out of bounds~n  F=~p~n  P=~p", [f, p]),
          :ssh_transport,
          630
        )
    end
  ...
  def handle_kex_dh_gex_request(
    ...
    case :public_key.dh_gex_group(
      ...
      {:error, _} ->
        :ssh_connection_handler.disconnect(
          3,
          :io_lib.format(~c"No possible diffie-hellman-group-exchange group found", []),
          :ssh_transport,
          656
        )
    end
  ...
  def handle_kex_dh_gex_request(_, _) do
    :ssh_connection_handler.disconnect(
      3,
      ~c"Key exchange failed, bad values in ssh_msg_kex_dh_gex_request",
      :ssh_transport,
      694
    )
  end
  ...
  defp adjust_gex_min_max(min0, max0, opts) do
    {min1, max1} =
      :ssh_options.get_value(:user_options, :dh_gex_limits, opts, :ssh_transport, 697)
    ...
    cond do
      ...
      max2 < min2 ->
        :ssh_connection_handler.disconnect(
          2,
          ~c"No possible diffie-hellman-group-exchange group possible",
          :ssh_transport,
          705
        )
    end
  ...
  def handle_kex_dh_gex_init(
    ...
    cond do
      1 <= e and e <= p - 1 ->
        ...
        cond do
          ...
          true ->
            :ssh_connection_handler.disconnect(
              3,
              ~c"Kexdh init failed, received 'k' out of bounds",
              :ssh_transport,
              751
            )
        end
      ...
      true ->
        :ssh_connection_handler.disconnect(
          3,
          :io_lib.format(~c"Kexdh gex init failed, received 'e' out of bounds~n  E=~p~n  P=~p", [
            e,
            p
          ]),
          :ssh_transport,
  ...
  def handle_kex_dh_gex_reply(
    ...
    cond do
      1 <= f and f <= p - 1 ->
        ...
        cond do
          1 < k and k < p - 1 ->
            ...
            case verify_host_key(ssh0, peerPubHostKey, h, h_SIG) do
              ...
              error ->
                :ssh_connection_handler.disconnect(
                  3,
                  :io_lib.format(~c"Kexdh gex reply failed. Verify host key: ~p", [error]),
                  :ssh_transport,
                  783
                )
            end
          ...
          true ->
            :ssh_connection_handler.disconnect(
              3,
              ~c"Kexdh gex init failed, 'K' out of bounds",
              :ssh_transport,
              789
            )
        end
      ...
      true ->
        :ssh_connection_handler.disconnect(
          3,
          :io_lib.format(~c"Kexdh gex init failed, received 'f' out of bounds~n  F=~p~n  P=~p", [
            f,
            p
          ]),
          :ssh_transport,
    ...
    try do
      ...
      class, error ->
        :ssh_connection_handler.disconnect(
          3,
          :io_lib.format(
            ~c"ECDH compute key failed in server: ~p:~p~nKex: ~p, Curve: ~p~nPeerPublic: ~p",
  ...
  def handle_kex_ecdh_reply(
    ...
    try do
      ...
      class, error ->
        :ssh_connection_handler.disconnect(
          3,
          :io_lib.format(~c"Peer ECDH public key seem invalid: ~p:~p", [class, error]),
          :ssh_transport,
          870
        )
    else
      k ->
        ...
        case verify_host_key(ssh0, peerPubHostKey, h, h_SIG) do
          ...
          error ->
            :ssh_connection_handler.disconnect(
              3,
              :io_lib.format(~c"ECDH reply failed. Verify host key: ~p", [error]),
              :ssh_transport,
              863
            )
        end
  ...
  def handle_new_keys(r_ssh_msg_newkeys(), ssh0) do
    try do
      ...
      class, error ->
        :ssh_connection_handler.disconnect(
          2,
          :io_lib.format(~c"Install alg failed: ~p:~p", [class, error]),
          :ssh_transport,
          884
        )
    else
  ...
  defp kex_strict_alg(:client) do
    [~c"kex-strict-c-v00@openssh.com"]
  end

  defp kex_strict_alg(:server) do
    [~c"kex-strict-s-v00@openssh.com"]
  end

  defp kex_ext_info(role, opts) do
    case :ssh_options.get_value(:user_options, :recv_ext_info, opts, :ssh_transport, 894) do
  ...
  def ext_info_message(r_ssh(role: :client, send_ext_info: true, opts: opts) = ssh0) do
    case :proplists.get_value(
           :ext_info_client,
           :ssh_options.get_value(:user_options, :tstflg, opts, :ssh_transport, 906)
         ) do
      true ->
        msg =
          r_ssh_msg_ext_info(
            nr_extensions: 1,
            data: [{~c"test@erlang.org", ~c"Testing,PleaseIgnore"}]
          )
  ...
  def ext_info_message(r_ssh(role: :server, send_ext_info: true, opts: opts) = ssh0) do
    algsList =
      :lists.map(
        ...
        :ssh_options.get_value(:user_options, :pref_public_key_algs, opts, :ssh_transport, 921)
      )

    msg =
      r_ssh_msg_ext_info(
        nr_extensions: 1,
        data: [{~c"server-sig-algs", :string.join(algsList, ~c",")}]
      )
  ...
  def call_KeyCb(f, args, opts) do
    {keyCb, keyCbOpts} = :ssh_options.get_value(:user_options, :key_cb, opts, :ssh_transport, 952)
    userOpts = :ssh_options.get_value(:user_options, :key_cb_options, opts, :ssh_transport, 953)
  ...
  defp verify_host_key(r_ssh(algorithms: alg) = sSH, publicKey, digest, {algStr, signature}) do
    case :erlang.atom_to_list(r_alg(alg, :hkey)) do
      ^algStr ->
        case verify(digest, r_alg(alg, :hkey), signature, publicKey, sSH) do
          false ->
            {:error, :bad_signature}
  ...
  defp accepted_host(ssh, peerName, port, public, opts) do
    ...
    case :ssh_options.get_value(:user_options, :silently_accept_hosts, opts, :ssh_transport, 977) do
      false ->
        :yes == yes_no(ssh, ~c"New host " ++ peerName ++ portStr ++ ~c" accept")
      ...
      {false, alg} ->
        ...
        prompt =
          :io_lib.format(
            ~c"The authenticity of the host can't be established.~n~s host key fingerprint is ~s.~nNew host ~p~p accept",
            [fmt_hostkey(hostKeyAlg), :ssh.hostkey_fingerprint(alg, public), peerName, portStr]
          )
      ...
      {digestAlg, f} when is_function(f, 3) ->
        case (try do
                f.(peerName, port, :ssh.hostkey_fingerprint(digestAlg, public))
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                :exit, e -> {:EXIT, e}
                e -> e
  ...
  defp known_host_key(
    ...
    case isHostKey do
      ...
      false ->
        doAdd =
          :ssh_options.get_value(:user_options, :save_accepted_host, opts, :ssh_transport, 1052)
  ...
  defp select_algorithm(
    ...
    kexStrictNegotiated =
      case reNeg do
        :init ->
          ...
          case result do
            true ->
              :logger.debug(:lists.concat([role, ~c" will use strict KEX ordering"]))
    ...
    sendExtInfo =
      :ssh_options.get_value(:user_options, :send_ext_info, opts, :ssh_transport, 1135) and
        case role do
          :server ->
            :lists.member(
              ~c"ext-info-c",
    ...
    recvExtInfo =
      :ssh_options.get_value(:user_options, :recv_ext_info, opts, :ssh_transport, 1146)
  ...
  defp select_all(cL, sL) do
    error =
      :lists.concat([
        ~c"Received too many algorithms (",
        length(cL),
        ~c"+",
        length(sL),
        ~c" >= ",
    ...
    :ssh_connection_handler.disconnect(2, error, :ssh_transport, 1321)
  end
  ...
  defp get_length(:common, :rfc4253, encryptedBuffer, r_ssh(decrypt_block_size: blockSize) = ssh0) do
    case byte_size(encryptedBuffer) >=
           :erlang.max(
             8,
             blockSize
           ) do
  ...
  defp get_length(:aead, _, encryptedBuffer, ssh) do
    case {byte_size(encryptedBuffer) >= 4, r_ssh(ssh, :decrypt)} do
  ...
  def sign(sigData, hashAlg, key = r_ECPrivateKey(parameters: {:namedCurve, curve}))
      when curve == {1, 3, 101, 112} or curve == {1, 3, 101, 113} do
  ...
  defp do_verify(
         ...
         sig,
         r_RSAPublicKey() = key,
         r_ssh(role: :server, c_version: ~c"SSH-2.0-OpenSSH_7." ++ _)
       )
      ...
      :common ->
        case r_ssh(sSH, :role) do
          :client ->
            ...
            key = hash(sSH, ~c"E", keySize)
            {:ok, r_ssh(sSH, send_mac_key: key)}

          :server ->
            keySize = 8 * mac_key_bytes(r_ssh(sSH, :send_mac))
            key = hash(sSH, ~c"F", keySize)
            {:ok, r_ssh(sSH, send_mac_key: key)}
        end
  ...
  defp recv_mac_init(sSH) do
    case pkt_type(r_ssh(sSH, :recv_mac)) do
      :common ->
        case r_ssh(sSH, :role) do
          :client ->
            key = hash(sSH, ~c"F", 8 * mac_key_bytes(r_ssh(sSH, :recv_mac)))
            {:ok, r_ssh(sSH, recv_mac_key: key)}

          :server ->
            key = hash(sSH, ~c"E", 8 * mac_key_bytes(r_ssh(sSH, :recv_mac)))
            {:ok, r_ssh(sSH, recv_mac_key: key)}
        end
  ...
  defp kex_plaintext(sSH, key, args) do
    ...
    <<byte_size(
      ...
      cond do
        ...
        r_ssh(sSH, :s_version) == :undefined ->
          <<>>
      end::binary, byte_size(r_ssh(sSH, :c_keyinit))::size(32)-unsigned-big-integer,
      r_ssh(sSH, :c_keyinit)::binary,
      byte_size(r_ssh(sSH, :s_keyinit))::size(32)-unsigned-big-integer,
      r_ssh(sSH, :s_keyinit)::binary, byte_size(encodedKey)::size(32)-unsigned-big-integer,
  ...
  defp kex_alg_dependent({q_c, q_s, k})
       when is_binary(q_c) and
              is_binary(q_s) do
    <<byte_size(q_c)::size(32)-unsigned-big-integer, q_c::binary,
      byte_size(q_s)::size(32)-unsigned-big-integer, q_s::binary, :ssh_bits.mpint(k)::binary>>
  end
  ...
  def valid_key_sha_alg(:private, r_ECPrivateKey(parameters: {:namedCurve, oID}), alg) do
    valid_key_sha_alg_ec(oID, alg)
  end
  ...
  def ssh_dbg_format(
        :alg,
        {:call, {:ssh_transport, :select_algorithm, [_, _, _, _, _]}}
      ) do
    :skip
  end
  ...
  def ssh_dbg_format(
        :alg,
        {:return_from, {:ssh_transport, :select_algorithm, 5}, {:ok, alg}}
      ) do
    [~c"Negotiated algorithms:\n", wr_record(alg)]
  end
