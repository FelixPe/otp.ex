defmodule :m_ssh_system_sup do
        ...
        :supervisor.start_child(sup(role),
                                  %{id: {:ssh_system_sup, address0},
                                      ...
                                      {:ssh_system_sup, :start_link,
                                         [role, address0, options]},
                                      restart: :temporary, type: :supervisor})
    end
  end

  def stop_system(role, sysSup) when is_pid(sysSup) do
    case (:lists.keyfind(sysSup, 2,
                           :supervisor.which_children(sup(role)))) do
  ...
  def stop_system(role, id) do
    :supervisor.terminate_child(sup(role),
                                  {:ssh_system_sup, id})
  end
            ...
            try do
              receive do
                {:new_connection_ref, ^id, connPid} ->
                  :ssh_connection_handler.takeover(connPid, role, socket,
                                                     options)
              after 10000 ->
  ...
  def start_link(role, address, options) do
    :supervisor.start_link(:ssh_system_sup,
                             [role, address, options])
  end

  def addresses(role,
           r_address(address: address, port: port, profile: profile)) do
    for {{:ssh_system_sup, a}, sysSup, :supervisor,
           _} <- :supervisor.which_children(sup(role)),
  ...
  def init([role, address, options]) do
    supFlags = %{strategy: :one_for_one,
                   auto_shutdown: :all_significant, intensity: 0,
                   period: 3600}
    childSpecs = (case ({role,
                           is_socket_server(options)}) do
                    {:server, false} ->
                      [acceptor_sup_child_spec(_SysSup = self(), address,
                                                 options)]
                    _ ->
                      []
                  end)
      ...
      {:ok,
         %{start:
           {:ssh_acceptor_sup, :start_link,
              [_, _, options]}}} = :supervisor.get_childspec(sup,
                                                               {:ssh_acceptor_sup,
                                                                  address})
      {:ok, options}
    catch
  ...
  defp is_socket_server(options) do
    :undefined !== :ssh_options.get_value(:internal_options,
                                            :connected_socket, options,
                                            fn () ->
                                                 :undefined
                                            end,
                                            :ssh_system_sup, 251)
  ...
  defp restart_acceptor(sysPid, address, options) do
    case (lookup(:ssh_acceptor_sup, sysPid)) do
      {_, _, :supervisor, _} ->
        {:error, :eaddrinuse}
      false ->
        childSpec = acceptor_sup_child_spec(sysPid, address,
                                              options)
