defmodule :m_ssh_dbg do
  ...
  def shrink_bin(b) when is_binary(b) and byte_size(b) > 256 do
    {:"*** SHRUNK BIN", byte_size(b), :erlang.element(1, :erlang.split_binary(b, 64)), :...,
     :erlang.element(
       2,
  ...
  defp try_all_types_in_all_modules(typesOn, arg, writeFun, acc0) do
    ...
    case acc do
      ^acc0 ->
        writeFun.(~c"~n~s ~p DEBUG~n~p~n", [:lists.flatten(tS), pID, iNFO], acc0)
  ...
  def wr_record([_Name | values], fields, blackL) do
    ...
    for {tag, value} <- :lists.zip(fields, values),
      ...
      :io_lib.format(~c"  ~*p: ~p~n", [w, tag, value])
    end
  end
  ...
  def fmt_cbuf_items() do
    :lists.flatten(
      :io_lib.format(
        ~c"Circular trace buffer. Latest item first.~n~s~n",
        [
          case :erlang.get(:circ_buf) do
            {max, _} ->
              l = cbuf_list()
    ...
    :io_lib.format(
      ...
      [
        ...
        for _ <- [:EFE_DUMMY_GEN], n > 1 do
          :io_lib.format(~c" (Repeated ~p times)", [n])
        end,
        fmt_value(value)
      ]
    )
  ...
  defp fmt_ts(tS = {_, _, us}) do
    ...
    :io_lib.format(~c"~w-~.2.0w-~.2.0w ~.2.0w:~.2.0w:~.2.0w.~.6.0w UTC", [yY, mM, dD, h, m, s, us])
  end

  defp fmt_value(r_circ_buf_entry(module: m, line: l, function: {f, a}, pid: pid, value: v)) do
    :io_lib.format(~c"~p:~p  ~p/~p ~p~n~s", [m, l, f, a, pid, fmt_value(v)])
  end
