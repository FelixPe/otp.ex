defmodule :m_ssh_fsm_userauth_server do
  ...
  def handle_event(:internal,
    ...
    case ({serviceName, r_ssh(ssh0, :service), method}) do
      {'ssh-connection', 'ssh-connection', 'none'} ->
        case (:ssh_auth.handle_userauth_request(msg,
      ...
      {'ssh-connection', 'ssh-connection', ^method} ->
        case (:lists.member(method,
                              r_ssh(ssh0, :userauth_methods))) do
          true ->
            case (:ssh_auth.handle_userauth_request(msg,
                                                      r_ssh(ssh0, :session_id),
                                                      ssh0)) do
              ...
              {:not_authorized, {user, reason}, {reply, ssh}}
                ...
                d = :ssh_connection_handler.send_msg(reply,
                                                       r_data(d0, ssh_params: ssh))
                {:next_state, {:userauth_keyboard_interactive, :server},
                   d}
              {:not_authorized, {user, reason}, {reply, ssh}} ->
                retry_fun(user, reason, d0)
                d = :ssh_connection_handler.send_msg(reply,
  ...
  def handle_event(:internal, r_ssh_msg_userauth_info_response() = msg,
           {:userauth_keyboard_interactive, :server}, d0) do
    case (:ssh_auth.handle_userauth_info_response(msg,
      ...
      {:authorized_but_one_more, _User, {reply, ssh}} ->
        d = :ssh_connection_handler.send_msg(reply,
                                               r_data(d0, ssh_params: ssh))
        {:next_state,
           {:userauth_keyboard_interactive_extra, :server}, d}
    end
  end

  def handle_event(:internal, r_ssh_msg_userauth_info_response() = msg,
           {:userauth_keyboard_interactive_extra, :server}, d0) do
    {:authorized, user,
       {reply,
          ssh1}} = :ssh_auth.handle_userauth_info_response({:extra,
                                                              msg},
  ...
  defp set_max_initial_idle_timeout(r_data(ssh_params: r_ssh(opts: opts))) do
    {{:timeout, :max_initial_idle_time},
       :ssh_options.get_value(:user_options,
                                :max_initial_idle_time, opts,
                                :ssh_fsm_userauth_server, 183),
       :none}
  end

  defp connected_fun(user, method,
            r_data(ssh_params: r_ssh(peer: {_, peer})) = d) do
    (try do
      (:ssh_options.get_value(:user_options, :connectfun,
                                r_ssh(r_data(d, :ssh_params), :opts),
                                :ssh_fsm_userauth_server, 186)).(user, peer,
                                                                   method)
    catch
  ...
  defp retry_fun(user, reason,
            r_data(ssh_params: r_ssh(opts: opts, peer: {_, peer}))) do
    {tag, info} = (case (reason) do
                     {:error, error} ->
                       {:failfun, error}
                     _ ->
                       {:infofun, reason}
                   end)
    fun = :ssh_options.get_value(:user_options, tag, opts,
                                   :ssh_fsm_userauth_server, 201)
    try do
