defmodule :m_ssh_shell do
  ...
  @behaviour :ssh_dbg
  Record.defrecord(:r_state, :state, io: :undefined,
                                 channel: :undefined, cm: :undefined)
  def init([connectionManager, channelId] = args) do
    ...
    case (:ssh_connection.shell(connectionManager,
                                  channelId)) do
      :ok ->
        {:group_leader, gIO} = :erlang.process_info(self(),
                                                      :group_leader)
        ioPid = spawn_link(:ssh_shell, :input_loop,
                             [gIO, self()])
        {:ok,
           r_state(io: ioPid, channel: channelId, cm: connectionManager)}
      error ->
        {:stop, error}
    end
  end

  def handle_ssh_msg({:ssh_cm, _, {:data, _ChannelId, 0, data}},
           state) do
    :io.format('~ts', [data])
    {:ok, state}
  end

  def handle_ssh_msg({:ssh_cm, _, {:data, _ChannelId, 1, data}},
           state) do
    :io.format('~ts', [data])
    {:ok, state}
  end
  ...
  def handle_msg({:ssh_channel_up, channelId, connectionManager},
           r_state(channel: channelId, cm: connectionManager) = state) do
    {:ok, state}
  end

  def handle_msg({:input, ioPid, :eof},
           r_state(io: ioPid, channel: channelId,
               cm: connectionManager) = state) do
    :ssh_connection.send_eof(connectionManager, channelId)
    {:ok, state}
  end

  def handle_msg({:input, ioPid, line0},
           r_state(io: ioPid, channel: channelId,
               cm: connectionManager) = state) do
    line = (case (encoding(line0)) do
              :utf8 ->
                line0
              :unicode ->
  ...
  defp encoding(bin) do
    case (:unicode.characters_to_binary(bin, :utf8,
                                          :utf8)) do
      ^bin ->
        :utf8
    ...
    [:io_lib.format('SHELL conn ~p chan ~p, req ~p',
         ...
         case (request) do
           ...
           {:data, channelId, type, data} ->
             fmt_kv([{:channel_id, channelId}, {:type,
                                                  case (type) do
                                                    0 ->
                                                      '0 (normal data)'
                                                    1 ->
  ...
  def ssh_dbg_format(:shell,
           {:call, {:ssh_shell, :handle_ssh_msg, _}}) do
    :skip
  end

  def ssh_dbg_format(:shell,
           {:return_from, {:ssh_shell, :handle_ssh_msg, 2},
              _Result}) do
    :skip
  end

  def ssh_dbg_format(:terminate,
           {:call, {:ssh_shell, :terminate, [reason, state]}}) do
    ['Shell Terminating:\n', :io_lib.format('Reason: ~p,~nState:~n~s', [reason, wr_record(state)])]
  end

  def ssh_dbg_format(:terminate,
           {:return_from, {:ssh_shell, :terminate, 2}, _Ret}) do
    :skip
  end
