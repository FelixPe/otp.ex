defmodule :m_ssh_sftp do
  ...
  Record.defrecord(:r_state, :state, xf: :undefined,
                                 rep_buf: <<>>, req_id: :undefined,
                                 req_list: [], inf: :undefined,
                                 opts: :undefined)
  Record.defrecord(:r_fileinf, :fileinf, handle: :undefined,
                                   offset: :undefined, size: :undefined,
                                   mode: :undefined)
  Record.defrecord(:r_bufinf, :bufinf, mode: :undefined,
                                  crypto_state: :undefined,
                                  crypto_fun: :undefined, size: 0,
                                  chunksize: :undefined, enc_text_buf: <<>>,
                                  plain_text_buf: <<>>)
  def start_channel(dest) do
    start_channel(dest, [])
  end

  def start_channel(cm, userOptions0) when is_pid(cm) do
    userOptions = legacy_timeout(userOptions0)
    timeout = :proplists.get_value(:timeout, userOptions,
                                     :infinity)
    {_SshOpts, chanOpts,
       sftpOpts} = handle_options(userOptions)
    windowSize = :proplists.get_value(:window_size,
                                        chanOpts, 20 * 65536)
    packetSize = :proplists.get_value(:packet_size,
                                        chanOpts, 65536)
    case (:ssh_connection.session_channel(cm, windowSize,
                                            packetSize, timeout)) do
      {:ok, channelId} ->
        case (:ssh_connection_handler.start_channel(cm,
                                                      :ssh_sftp, channelId,
  ...
  def start_channel(dest, userOptions0) do
    userOptions = legacy_timeout(userOptions0)
    {sshOpts, chanOpts,
       sftpOpts} = handle_options(userOptions)
    case (:ssh.is_host(dest, sshOpts)) do
      true ->
  ...
  def start_channel(host, port, userOptions0) do
    userOptions = legacy_timeout(userOptions0)
    timeout = :proplists.get_value(:connect_timeout,
                                     userOptions, :infinity)
    {sshOpts, _ChanOpts,
       _SftpOpts} = handle_options(userOptions)
  ...
  def open_tar(pid, file, mode, fileOpTimeout) do
    case ({:lists.member(:write, mode),
             :lists.member(:read, mode), mode -- [:write, :read]}) do
      {true, false, []} ->
        {:ok, handle} = open(pid, file, [:write], fileOpTimeout)
        :erl_tar.init(pid, :write,
                        fn :write, {_, data} ->
                             write_to_remote_tar(pid, handle, to_bin(data),
                                                   fileOpTimeout)
                           :position, {_, pos} ->
      ...
      {true, false,
         [{:crypto,
             {cryptoInitFun, cryptoEncryptFun, cryptoEndFun}}]} ->
        {:ok, sftpHandle} = open(pid, file, [:write],
                                   fileOpTimeout)
        bI = r_bufinf(mode: :write, crypto_fun: cryptoEncryptFun)
        {:ok, bufHandle} = open_buf(pid, cryptoInitFun, bI,
                                      fileOpTimeout)
        :erl_tar.init(pid, :write,
                        fn :write, {_, data} ->
                             write_buf(pid, sftpHandle, bufHandle, to_bin(data),
                                         fileOpTimeout)
                           :position, {_, pos} ->
                             position_buf(pid, sftpHandle, bufHandle, pos,
                                            fileOpTimeout)
                           :close, _ ->
                             {:ok,
                                r_bufinf(plain_text_buf: plainBuf0,
                                    enc_text_buf: encBuf0,
                                    crypto_state: cState0)} = call(pid,
                                                                     {:get_bufinf,
                                                                        bufHandle},
                                                                     fileOpTimeout)
                             {:ok, encTextTail} = cryptoEndFun.(plainBuf0,
                                                                  cState0)
                             encTextBuf = <<encBuf0 :: binary,
                                              encTextTail :: binary>>
                             case (write(pid, sftpHandle, encTextBuf,
                                           fileOpTimeout)) do
                               :ok ->
                                 call(pid, {:erase_bufinf, bufHandle},
                                        fileOpTimeout)
      ...
      {false, true, []} ->
        {:ok, handle} = open(pid, file, [:read, :binary],
                               fileOpTimeout)
      ...
      {false, true,
         [{:crypto, {cryptoInitFun, cryptoDecryptFun}}]} ->
        {:ok, sftpHandle} = open(pid, file, [:read, :binary],
                                   fileOpTimeout)
        ...
        :erl_tar.init(pid, :read,
                        fn :read2, {_, len} ->
                             read_buf(pid, sftpHandle, bufHandle, len,
                                        fileOpTimeout)
                           :position, {_, pos} ->
                             position_buf(pid, sftpHandle, bufHandle, pos,
                                            fileOpTimeout)
  ...
  def pread(pid, handle, offset, len, fileOpTimeout) do
    call(pid, {:pread, false, handle, offset, len},
           fileOpTimeout)
  end
  ...
  def apread(pid, handle, offset, len) do
    call(pid, {:pread, true, handle, offset, len},
           :infinity)
  end
  ...
  def pwrite(pid, handle, offset, data, fileOpTimeout) do
    call(pid, {:pwrite, false, handle, offset, data},
           fileOpTimeout)
  end
  ...
  def apwrite(pid, handle, offset, data) do
    call(pid, {:pwrite, true, handle, offset, data},
           :infinity)
  end
  ...
  def get_file_info(pid, handle, fileOpTimeout) do
    call(pid, {:get_file_info, false, handle},
           fileOpTimeout)
  end
  ...
  def write_file_info(pid, name, info, fileOpTimeout) do
    call(pid, {:write_file_info, false, name, info},
           fileOpTimeout)
  end
  ...
  def read_link(pid, linkName, fileOpTimeout) do
    case (call(pid, {:read_link, false, linkName},
                 fileOpTimeout)) do
      {:ok, [{name, _Attrs}]} ->
        {:ok, name}
      errMsg ->
  ...
  def make_symlink(pid, name, target, fileOpTimeout) do
    call(pid, {:make_symlink, false, name, target},
           fileOpTimeout)
  end
  ...
  def rename(pid, fromFile, toFile, fileOpTimeout) do
    call(pid, {:rename, false, fromFile, toFile},
           fileOpTimeout)
  end
  ...
  def list_dir(pid, name, fileOpTimeout) do
    case (opendir(pid, name, fileOpTimeout)) do
      {:ok, handle} ->
        ...
        case (res) do
          {:ok, list} ->
            nList = :lists.foldl(fn {nm, _Info}, acc ->
                                      [nm | acc]
                                 end,
                                   [], list)
            {:ok, nList}
  ...
  def read_file(pid, name, fileOpTimeout) do
    case (open(pid, name, [:read, :binary],
                 fileOpTimeout)) do
      {:ok, handle} ->
  ...
  defp read_file_loop(pid, handle, packetSz, fileOpTimeout, acc) do
    case (read(pid, handle, packetSz, fileOpTimeout)) do
      {:ok, data} ->
        read_file_loop(pid, handle, packetSz, fileOpTimeout,
                         [data | acc])
      :eof ->
  ...
  def write_file(pid, name, bin, fileOpTimeout) do
    case (open(pid, name, [:write, :binary],
                 fileOpTimeout)) do
      {:ok, handle} ->
        {:ok, {_Window, packet}} = send_window(pid,
                                                 fileOpTimeout)
        res = write_file_loop(pid, handle, 0, bin,
                                byte_size(bin), packet, fileOpTimeout)
        close(pid, handle, fileOpTimeout)
        res
      error ->
        error
  ...
  defp write_file_loop(_Pid, _Handle, _Pos, _Bin, 0, _PacketSz,
            _FileOpTimeout) do
    :ok
  end

  defp write_file_loop(pid, handle, pos, bin, remain, packetSz,
            fileOpTimeout) do
    cond do
      remain >= packetSz ->
        <<_ :: size(pos) - binary,
            data :: size(packetSz) - binary, _ :: binary>> = bin
        case (write(pid, handle, data, fileOpTimeout)) do
          :ok ->
            write_file_loop(pid, handle, pos + packetSz, bin,
                              remain - packetSz, packetSz, fileOpTimeout)
          error ->
            error
        end
      true ->
  ...
  def init([cm, channelId, options]) do
    ...
    case (:ssh_connection.subsystem(cm, channelId, 'sftp',
                                      timeout)) do
      :success ->
        xf = r_ssh_xfer(cm: cm, channel: channelId)
        {:ok,
           r_state(xf: xf, req_id: 0, rep_buf: <<>>, inf: new_inf(),
               opts: options)}
      :failure ->
  ...
  def handle_call({{:timeout, :infinity},
            :wait_for_version_negotiation},
           from, r_state(xf: r_ssh_xfer(vsn: :undefined) = xf) = state) do
    {:noreply,
       r_state(state, xf: r_ssh_xfer(xf, vsn: {:wait, from, :undefined}))}
  end

  def handle_call({{:timeout, timeout},
            :wait_for_version_negotiation},
           from, r_state(xf: r_ssh_xfer(vsn: :undefined) = xf) = state) do
    tRef = :erlang.send_after(timeout, self(),
                                {:timeout, :undefined, from})
    {:noreply,
       r_state(state, xf: r_ssh_xfer(xf, vsn: {:wait, from, tRef}))}
  end
  ...
  def handle_call({{:timeout, timeout}, msg}, from,
           r_state(req_id: id) = state) do
  ...
  defp do_handle_call({:get_bufinf, bufHandle}, _From,
            s = r_state(inf: i0)) do
    {:reply, :maps.find(bufHandle, i0), s}
  end

  defp do_handle_call({:put_bufinf, bufHandle, b}, _From,
            s = r_state(inf: i0)) do
    {:reply, :ok, r_state(s, inf: :maps.put(bufHandle, b, i0))}
  end

  defp do_handle_call({:erase_bufinf, bufHandle}, _From,
            s = r_state(inf: i0)) do
    {:reply, :ok, r_state(s, inf: :maps.remove(bufHandle, i0))}
  end

  defp do_handle_call({:open, async, fileName, mode}, from,
            r_state(xf: xF) = state) do
    {access, flags, attrs} = open_mode(r_ssh_xfer(xF, :vsn), mode)
    reqID = r_state(state, :req_id)
    :ssh_xfer.open(xF, reqID, fileName, access, flags,
                     attrs)
    case (async) do
      true ->
        {:reply, {:async, reqID},
           update_request_info(reqID, state,
                                 fn {:ok, handle}, state1 ->
                                      open2(reqID, fileName, handle, mode,
                                              async, from, state1)
                                    rep, state1 ->
  ...
  defp do_handle_call({:pread, async, handle, at, length}, from,
            state) do
    case (lseek_position(handle, at, state)) do
      {:ok, offset} ->
        reqID = r_state(state, :req_id)
        :ssh_xfer.read(r_state(state, :xf), reqID, handle, offset,
                         length)
  ...
  defp do_handle_call({:pwrite, async, handle, at, data0}, from,
            state) do
    case (lseek_position(handle, at, state)) do
      {:ok, offset} ->
        data = to_bin(data0)
        reqID = r_state(state, :req_id)
        size = byte_size(data)
        :ssh_xfer.write(r_state(state, :xf), reqID, handle, offset,
                          data)
  ...
  defp do_handle_call({:position, handle, at}, _From, state) do
    case (lseek_position(handle, at, state)) do
      {:ok, offset} ->
        {:reply, {:ok, offset},
           update_offset(handle, offset, state)}
      error ->
        {:reply, error, state}
    end
  end

  defp do_handle_call({:rename, async, fromFile, toFile}, from,
            state) do
    reqID = r_state(state, :req_id)
    :ssh_xfer.rename(r_state(state, :xf), reqID, fromFile, toFile,
                       [:overwrite])
    make_reply(reqID, async, from, state)
  end
  ...
  defp do_handle_call({:make_dir, async, name}, from, state) do
    reqID = r_state(state, :req_id)
    :ssh_xfer.mkdir(r_state(state, :xf), reqID, name,
                      r_ssh_xfer_attr(type: :directory))
    make_reply(reqID, async, from, state)
  end
  ...
  defp do_handle_call({:make_symlink, async, path, targetPath}, from,
            state) do
    reqID = r_state(state, :req_id)
    :ssh_xfer.symlink(r_state(state, :xf), reqID, path,
                        targetPath)
    make_reply(reqID, async, from, state)
  end

  defp do_handle_call({:write_file_info, async, name, info}, from,
            state) do
    reqID = r_state(state, :req_id)
    a = info_to_attr(info)
  ...
  defp do_handle_call(:send_window, _From, state) do
    ...
    [{:send_window,
        {{:win_size, size0},
           {:packet_size, size1}}}] = :ssh.channel_info(r_ssh_xfer(xF, :cm),
  ...
  def handle_ssh_msg({:ssh_cm, _ConnectionManager,
           ...
           r_state(rep_buf: data0) = state0) do
  ...
  def handle_ssh_msg({:ssh_cm, _,
            {:exit_signal, channelId, signal, error0, _}},
           state0) do
  ...
  def handle_msg({:ssh_channel_up, _, _},
           r_state(opts: options, xf: xf) = state) do
    version = :proplists.get_value(:sftp_vsn, options, 6)
    :ssh_xfer.protocol_version_request(xf, version)
    {:ok, state}
  end

  def handle_msg({:timeout, :undefined, from},
           r_state(xf: r_ssh_xfer(channel: channelId)) = state) do
    :ssh_client_channel.reply(from, {:error, :timeout})
    {:stop, channelId, state}
  end

  def handle_msg({:timeout, id, from},
           r_state(req_list: reqList0) = state) do
  ...
  def handle_msg({:DOWN, _Ref, _Type, _Process, _},
           r_state(xf: r_ssh_xfer(channel: channelId)) = state) do
    {:stop, channelId, state}
  end

  def handle_msg({:EXIT, _, :ssh_sftp_stop_channel},
           r_state(xf: r_ssh_xfer(channel: channelId)) = state) do
    {:stop, channelId, state}
  end
  ...
  defp legacy_timeout(userOptions) do
    case ({:proplists.get_value(:connect_timeout,
                                  userOptions),
             :proplists.get_value(:timeout, userOptions)}) do
      {:undefined, :undefined} ->
        userOptions
      {:undefined, tO} ->
  ...
  defp handle_options([{:timeout, _} = opt | rest], sftp, chan,
            ssh) do
    handle_options(rest, [opt | sftp], chan, ssh)
  end

  defp handle_options([{:sftp_vsn, _} = opt | rest], sftp, chan,
            ssh) do
    handle_options(rest, [opt | sftp], chan, ssh)
  end

  defp handle_options([{:window_size, _} = opt | rest], sftp, chan,
            ssh) do
    handle_options(rest, sftp, [opt | chan], ssh)
  end

  defp handle_options([{:packet_size, _} = opt | rest], sftp, chan,
            ssh) do
    handle_options(rest, sftp, [opt | chan], ssh)
  end
  ...
  defp call(pid, msg, timeOut) do
    :ssh_client_channel.call(pid,
                               {{:timeout, timeOut}, msg}, :infinity)
  end

  defp handle_reply(state,
            <<len :: size(32) - unsigned - big - integer,
                reply :: size(len) - binary, rest :: binary>>) do
  ...
  defp do_handle_reply(r_state(xf: xf) = state,
            <<2, version :: size(32) - unsigned - big - integer,
                binExt :: binary>>,
            rest) do
    ...
    r_state(state, xf: r_ssh_xfer(xf, vsn: version,  ext: ext), 
               rep_buf: rest)
  end
  ...
  defp handle_req_reply(state0, {_, reqID, _} = xfReply) do
    case (:lists.keysearch(reqID, 1,
                             r_state(state0, :req_list))) do
      false ->
        state0
      {:value, {_, fun}} ->
  ...
  defp update_request_info(reqID, state, fun) do
    ...
    iD = (r_state(state, :req_id) + 1) &&& 4294967295
  ...
  defp open2(origReqID, fileName, handle, mode, async, from,
            state) do
    i0 = r_state(state, :inf)
    fileMode = (case (:lists.member(:binary,
                                      mode) or :lists.member(:raw, mode)) do
                  true ->
                    :binary
                  false ->
                    :text
      ...
      true ->
        update_request_info(reqID, state0,
                              fn {:ok, fI}, state1 ->
                                   ...
                                   async_reply(origReqID, {:ok, handle}, from,
                                                 state2)
                                 _, state1 ->
                                   async_reply(origReqID, {:ok, handle}, from,
                                                 state1)
  ...
  def info_to_attr(i) when elem(i, 0) === :file_info do
    r_ssh_xfer_attr(permissions: r_file_info(i, :mode), size: r_file_info(i, :size),
        type: r_file_info(i, :type), owner: r_file_info(i, :uid), group: r_file_info(i, :gid),
  ...
  def attr_to_info(a) when elem(a, 0) === :ssh_xfer_attr do
    r_file_info(size: r_ssh_xfer_attr(a, :size), type: r_ssh_xfer_attr(a, :type),
        ...
        mode: r_ssh_xfer_attr(a, :permissions), links: 1, major_device: 0,
        minor_device: 0, inode: 0, uid: r_ssh_xfer_attr(a, :owner),
  ...
  defp open_mode5(modes) do
    ...
    {fl, ac} = (case ({:lists.member(:write, modes),
                  ...
                  {_, _, true} ->
                    {[:append_data],
                       [:read_attributes, :append_data, :write_attributes]}
                  {true, false, false} ->
                    {[:create_truncate], [:write_data, :write_attributes]}
                  {true, true, _} ->
                    {[:open_or_create],
                       [:read_data, :read_attributes, :write_data,
                                                          :write_attributes]}
                  {false, true, _} ->
  ...
  defp add_new_handle(handle, fileMode, inf) do
    :maps.put(handle, r_fileinf(offset: 0, size: 0, mode: fileMode),
                inf)
  end
  ...
  defp put_size(handle, size, state) do
    ...
    case (:maps.find(handle, inf0)) do
      {:ok, fI} ->
        r_state(state, inf: :maps.put(handle, r_fileinf(fI, size: size),
                                  inf0))
      _ ->
        r_state(state, inf: :maps.put(handle,
                                  r_fileinf(size: size, offset: 0), inf0))
    end
  end
  ...
  defp put_offset(handle, offset, state) do
    ...
    case (:maps.find(handle, inf0)) do
      {:ok, fI} ->
        r_state(state, inf: :maps.put(handle, r_fileinf(fI, offset: offset),
                                  inf0))
      _ ->
        r_state(state, inf: :maps.put(handle,
                                  r_fileinf(size: offset, offset: offset), inf0))
    end
  end
  ...
  defp lseek_pos({:cur, offset}, curOffset, _CurSize)
      when is_integer(offset) and -
                                  (1 <<< 63) <= offset and offset < 1 <<< 63 do
  ...
  defp read_rpt(pid, handle, wantedLen, packetSz, fileOpTimeout,
            acc)
      when wantedLen > 0 do
    case (read(pid, handle, min(wantedLen, packetSz),
                 fileOpTimeout)) do
      {:ok, data} ->
        read_rpt(pid, handle, wantedLen - byte_size(data),
  ...
  defp read_rpt(_Pid, _Handle, wantedLen, _PacketSz,
            _FileOpTimeout, acc)
      when wantedLen >= 0 do
    {:ok, acc}
  end
  ...
  defp write_to_remote_tar(pid, sftpHandle, bin, fileOpTimeout) do
    {:ok, {_Window, packet}} = send_window(pid,
                                             fileOpTimeout)
    write_file_loop(pid, sftpHandle, 0, bin, byte_size(bin),
                      packet, fileOpTimeout)
  end

  defp position_buf(pid, sftpHandle, bufHandle, pos,
            fileOpTimeout) do
    {:ok,
       r_bufinf(mode: mode, plain_text_buf: buf0,
           size: size)} = call(pid, {:get_bufinf, bufHandle},
                                 fileOpTimeout)
    case (pos) do
      ...
      _ when (mode == :read and is_integer(pos)) ->
        ...
        cond do
          ...
          skip == 0 ->
            {:ok, pos}
          skip > 0 ->
            case (read_buf(pid, sftpHandle, bufHandle, skip,
                             fileOpTimeout)) do
              {:ok, _} ->
                {:ok, pos}
              other ->
  ...
  defp read_buf(pid, sftpHandle, bufHandle, wantedLen,
            fileOpTimeout) do
    {:ok, {_Window, packet}} = send_window(pid,
                                             fileOpTimeout)
    {:ok, b0} = call(pid, {:get_bufinf, bufHandle},
                       fileOpTimeout)
    case (do_the_read_buf(pid, sftpHandle, wantedLen,
                            packet, fileOpTimeout, b0)) do
      {:ok, resultBin, b} ->
  ...
  defp do_the_read_buf(_Pid, _SftpHandle, wantedLen, _Packet,
            _FileOpTimeout,
            b = r_bufinf(plain_text_buf: plainBuf0, size: size))
      when byte_size(plainBuf0) >= wantedLen do
    <<resultBin :: size(wantedLen) - binary,
        plainBuf :: binary>> = plainBuf0
    {:ok, resultBin,
       r_bufinf(b, plain_text_buf: plainBuf,  size: size + wantedLen)}
  end

  defp do_the_read_buf(pid, sftpHandle, wantedLen, packet,
            fileOpTimeout,
            b0 = r_bufinf(plain_text_buf: plainBuf0, enc_text_buf: encBuf0,
                     chunksize: :undefined))
      when byte_size(encBuf0) > 0 do
    {:ok, decodedBin, b} = apply_crypto(encBuf0, b0)
    do_the_read_buf(pid, sftpHandle, wantedLen, packet,
                      fileOpTimeout,
                      r_bufinf(b, plain_text_buf: <<plainBuf0 :: binary,
                                               decodedBin :: binary>>, 
                             enc_text_buf: <<>>))
  end
  ...
  defp do_the_read_buf(pid, sftpHandle, wantedLen, packet,
      ...
      when byte_size(encBuf0) >= chunkSize0 do
    <<toDecode :: size(chunkSize0) - binary,
        encBuf :: binary>> = encBuf0
  ...
  defp do_the_read_buf(pid, sftpHandle, wantedLen, packet,
            fileOpTimeout, b = r_bufinf(enc_text_buf: encBuf0)) do
    case (read(pid, sftpHandle, packet, fileOpTimeout)) do
      {:ok, encryptedBin} ->
        do_the_read_buf(pid, sftpHandle, wantedLen, packet,
                          fileOpTimeout,
                          r_bufinf(b, enc_text_buf: <<encBuf0 :: binary,
                                                 encryptedBin :: binary>>))
      :eof ->
        {:eof, b}
      other ->
  ...
  defp write_buf(pid, sftpHandle, bufHandle, plainBin,
            fileOpTimeout) do
    {:ok, {_Window, packet}} = send_window(pid,
                                             fileOpTimeout)
    {:ok, b0 = r_bufinf(plain_text_buf: pTB)} = call(pid,
                                                {:get_bufinf, bufHandle},
                                                fileOpTimeout)
    case (do_the_write_buf(pid, sftpHandle, packet,
                             fileOpTimeout,
                             r_bufinf(b0, plain_text_buf: <<pTB :: binary,
                                                       plainBin
                                                       ::
                                                       binary>>))) do
      {:ok, b} ->
  ...
  defp do_the_write_buf(pid, sftpHandle, packet, fileOpTimeout,
            b = r_bufinf(enc_text_buf: encBuf0, size: size))
      when byte_size(encBuf0) >= packet do
    <<binToWrite :: size(packet) - binary,
        encBuf :: binary>> = encBuf0
    case (write(pid, sftpHandle, binToWrite,
                  fileOpTimeout)) do
      :ok ->
  ...
  defp do_the_write_buf(pid, sftpHandle, packet, fileOpTimeout,
            b0 = r_bufinf(plain_text_buf: plainBuf0, enc_text_buf: encBuf0,
                     chunksize: :undefined))
      when byte_size(plainBuf0) > 0 do
    {:ok, encodedBin, b} = apply_crypto(plainBuf0, b0)
    do_the_write_buf(pid, sftpHandle, packet, fileOpTimeout,
                       r_bufinf(b, plain_text_buf: <<>>, 
                              enc_text_buf: <<encBuf0 :: binary,
                                                encodedBin :: binary>>))
  end
  ...
  defp do_the_write_buf(pid, sftpHandle, packet, fileOpTimeout,
            b0 = r_bufinf(plain_text_buf: plainBuf0, enc_text_buf: encBuf0,
                     chunksize: chunkSize0))
      when byte_size(plainBuf0) >= chunkSize0 do
    <<toEncode :: size(chunkSize0) - binary,
        plainBuf :: binary>> = plainBuf0
    {:ok, encodedBin, b} = apply_crypto(toEncode, b0)
    do_the_write_buf(pid, sftpHandle, packet, fileOpTimeout,
                       r_bufinf(b, plain_text_buf: plainBuf, 
                              enc_text_buf: <<encBuf0 :: binary,
                                                encodedBin :: binary>>))
  end

  defp do_the_write_buf(_Pid, _SftpHandle, _Packet, _FileOpTimeout,
            b) do
    {:ok, b}
  end

  defp apply_crypto(in__,
            b = r_bufinf(crypto_state: cState0, crypto_fun: f)) do
    case (f.(in__, cState0)) do
      {:ok, encodedBin, cState} ->
        {:ok, encodedBin, r_bufinf(b, crypto_state: cState)}
      {:ok, encodedBin, cState, chunkSize} ->
        {:ok, encodedBin,
           r_bufinf(b, crypto_state: cState,  chunksize: chunkSize)}
    end
  end
  ...
  defp open_buf(pid, cryptoInitFun, bufInfo0, fileOpTimeout) do
    case (cryptoInitFun.()) do
      {:ok, cryptoState} ->
        open_buf1(pid, bufInfo0, fileOpTimeout, cryptoState,
                    :undefined)
  ...
  defp open_buf1(pid, bufInfo0, fileOpTimeout, cryptoState,
            chunkSize) do
    bufInfo = r_bufinf(bufInfo0, crypto_state: cryptoState, 
                            chunksize: chunkSize)
    bufHandle = make_ref()
    call(pid, {:put_bufinf, bufHandle, bufInfo},
           fileOpTimeout)
    {:ok, bufHandle}
  end
  ...
  def ssh_dbg_format(:terminate,
           {:call, {:ssh_sftp, :terminate, [reason, state]}}) do
    ['Sftp Terminating:\n', :io_lib.format('Reason: ~p,~nState:~n~s', [reason, wr_record(state)])]
  end

  def ssh_dbg_format(:terminate,
           {:return_from, {:ssh_sftp, :terminate, 2}, _Ret}) do
    :skip
  end
