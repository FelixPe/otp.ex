defmodule :m_ssh_sftp do
  ...
  def init([cm, channelId, options]) do
    ...
    case :ssh_connection.subsystem(cm, channelId, ~c"sftp", timeout) do
      :success ->
        xf = r_ssh_xfer(cm: cm, channel: channelId)
        {:ok, r_state(xf: xf, req_id: 0, rep_buf: <<>>, inf: new_inf(), opts: options)}

      :failure ->
        {:stop, {:shutdown, ~c"server failed to start sftp subsystem"}}
  ...
  def handle_ssh_msg(
    ...
    :error_logger.format(~c"ssh: STDERR: ~s\n", [:erlang.binary_to_list(data)])
    {:ok, state}
  end
