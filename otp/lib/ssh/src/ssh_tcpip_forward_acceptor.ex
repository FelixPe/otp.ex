defmodule :m_ssh_tcpip_forward_acceptor do
  ...
  Record.defrecord(:r_circ_buf_entry, :circ_buf_entry, module: :undefined,
                                          ...
                                          value: :undefined)
  def supervised_start(fwdSup, {listenAddrStr, listenPort},
           connectToAddr, chanType, chanCB, connPid) do
  ...
  def start_link(lSock, {listenAddrStr, listenPort},
           connectToAddr, chanType, chanCB, connPid) do
    pid = :proc_lib.spawn_link(fn () ->
                                    acceptor_loop(lSock, listenAddrStr,
                                                    listenPort, connectToAddr,
                                                    chanType, chanCB, connPid)
                               end)
    {:ok, pid}
  end

  defp acceptor_loop(lSock, listenAddrStr, listenPort, connectToAddr,
            chanType, chanCB, connPid) do
    case (:gen_tcp.accept(lSock)) do
      {:ok, sock} ->
        {:ok, {remHost, remPort}} = :inet.peername(sock)
        remHostBin = :erlang.list_to_binary(encode_ip(remHost))
        data = (case (connectToAddr) do
                  :undefined ->
                    <<byte_size(listenAddrStr)
                      ::
                      size(32) - unsigned - big - integer,
                        listenAddrStr :: binary,
                        listenPort :: size(32) - unsigned - big - integer,
                        byte_size(remHostBin)
                        ::
                        size(32) - unsigned - big - integer,
                        remHostBin :: binary,
                        remPort :: size(32) - unsigned - big - integer>>
                  {connectToHost, connectToPort} ->
                    <<byte_size(connectToHost)
                      ::
                      size(32) - unsigned - big - integer,
                        connectToHost :: binary,
        ...
        case (:ssh_connection.open_channel(connPid, chanType,
                                             data, :infinity)) do
          {:ok, chId} ->
            :gen_tcp.controlling_process(sock, connPid)
            send(connPid, {:fwd_connect_received, sock, chId,
                             chanCB})
          _ ->
            :gen_tcp.close(sock)
        end
        acceptor_loop(lSock, listenAddrStr, listenPort,
                        connectToAddr, chanType, chanCB, connPid)
      {:error, :closed} ->
        :ok
    end
  end
  ...
  defp get_fwd_listen_opts(addrStr) do
    case (:inet.getaddr(:erlang.binary_to_list(addrStr),
                          :inet)) do
      {:ok, addr} ->
        {:ok, [{:ip, addr}]}
      {:error, error} ->
