defmodule :m_ssh_connection_handler do
  ...
  def takeover(connPid, _, socket, options) do
    {_, callback, _} =
      :ssh_options.get_value(:user_options, :transport, options, :ssh_connection_handler, 128)
  ...
  def retrieve(r_connection(options: opts), key) do
    try do
      :ssh_options.get_value(:internal_options, key, opts, :ssh_connection_handler, 345)
    catch
      :error, {:badkey, ^key} ->
        :undefined
    else
    ...
    case role do
      :server ->
        r_connection(c,
          ...
          exec: :ssh_options.get_value(:user_options, :exec, opts, :ssh_connection_handler, 426)
        )
  ...
  defp init_ssh_record(role, socket, peerAddr, opts) do
    authMethods =
      :ssh_options.get_value(:user_options, :auth_methods, opts, :ssh_connection_handler, 438)
    ...
    case role do
      ...
      :server ->
        r_ssh(s0,
          s_vsn: vsn,
          s_version: version,
          userauth_methods: :string.tokens(authMethods, ~c","),
          kb_tries_left: 3,
  ...
  def handle_event(
        :cast,
        :socket_control,
        {:wait_for_socket, role},
        r_data(socket: socket, ssh_params: r_ssh(opts: opts))
      ) do
    case :inet.peername(socket) do
      {:ok, peerAddr} ->
        try do
          {protocol, callback, closeTag} =
            :ssh_options.get_value(:user_options, :transport, opts, :ssh_connection_handler, 572)
    ...
    case :inet.getopts(
      ...
      other ->
        call_disconnectfun_and_log_cond(
          ~c"Option return",
          :io_lib.format(~c"Unexpected getopts return:~n  ~p", [other]),
          :ssh_connection_handler,
          609,
          stateName,
          d
  ...
  def handle_event(:internal, {:info_line, _Line}, {:hello, :client}, d) do
    :inet.setopts(r_data(d, :socket), [{:active, :once}])
    :keep_state_and_data
  end

  def handle_event(:internal, {:info_line, line}, {:hello, :server} = stateName, d) do
    send_bytes(~c"Protocol mismatch.", d)

    msg =
      :io_lib.format(~c"Protocol mismatch in version exchange. Client sent info lines.~n~s", [
    ...
    {:stop, {:shutdown, ~c"Protocol mismatch in version exchange. Client sent info lines."}}
  end

  def handle_event(:internal, {:version_exchange, version}, {:hello, role}, d0) do
  ...
  def handle_event(
    ...
    {shutdown, d} =
      :ssh_connection_handler.send_disconnect(
        2,
        :lists.concat([~c"No HELLO received within ", :ssh_lib.format_time_ms(time)]),
        :ssh_connection_handler,
        659,
        stateName,
        d0
  ...
  def handle_event(
        :internal,
        r_ssh_msg_service_accept(name: ~c"ssh-userauth"),
        {:service_request, :client},
        r_data(ssh_params: r_ssh(service: ~c"ssh-userauth") = ssh0) = d0
      ) do
  ...
  def handle_event(:internal, {r_ssh_msg_kexinit(), _}, {:connected, role}, d0) do
    ...
    {:next_state, {:kexinit, role, :renegotiate}, d,
     [:postpone, {:change_callback_module, :ssh_fsm_kexinit}]}
  end

  def handle_event(:internal, r_ssh_msg_disconnect(description: desc) = msg, stateName, d0) do
    ...
    disconnect_fun(~c"Received disconnect: " ++ desc, d)
  ...
  def handle_event(
    ...
    :ssh_connection_handler.disconnect(
      3,
      :io_lib.format(
        ~c"strict KEX violation: unexpected SSH_MSG_IGNORE send_sequence = ~p  recv_sequence = ~p",
        [sendSeq, recvSeq]
      ),
      :ssh_connection_handler,
      716
  ...
  def handle_event(
    ...
    try do
      :ssh_connection.handle_msg(msg, connection0, role, r_data(d0, :ssh_params))
    catch
      class, error ->
        {repls, d1} =
          send_replies(
  ...
  def handle_event(:cast, {:reply_request, resp, channelId}, stateName, d)
    ...
    case :ssh_client_channel.cache_lookup(
      ...
      r_channel() ->
        details = :io_lib.format(~c"Unhandled reply in state ~p:~n~p", [stateName, resp])
  ...
  def handle_event({:call, from}, :get_print_info, stateName, d) do
    reply =
      try do
        ...
        {{:ok, local}, {:ok, remote}} ->
          {{local, remote}, :io_lib.format(~c"statename=~p", [stateName])}
  ...
  def handle_event(
        {:call, from},
        {:global_request, ~c"tcpip-forward" = type,
    ...
    data =
      <<byte_size(listenHost)::size(32)-unsigned-big-integer, listenHost::binary,
  ...
  def handle_event(
    ...
    reply = {:ok, {subSysSup, :erlang.element(2, stateName), opts}}
  ...
  def handle_event(
    ...
    d2 =
      send_msg(
        :ssh_connection.channel_open_msg(type, channelId, winSz, pktSz, data),
        d1
      )
  ...
  def handle_event(
        ...
        {proto, sock, newData},
        stateName,
        d0 = r_data(socket: sock, transport_protocol: proto, ssh_params: sshParams)
      ) do
    try do
      ...
      c, e ->
        ...
        {shutdown, d} =
          :ssh_connection_handler.send_disconnect(
            2,
            :io_lib.format(
              ~c"Bad packet: Couldn't decrypt~n~p:~p~n~P",
              [c, e, __STACKTRACE__, maxLogItemLen]
            ),
            :ssh_connection_handler,
            1198,
            stateName,
          ...
          c, e ->
            ...
            {shutdown, d} =
              :ssh_connection_handler.send_disconnect(
                2,
                :io_lib.format(
                  ~c"Bad packet: Decrypted, but can't decode~n~p:~p~n~P",
                  [c, e, __STACKTRACE__, maxLogItemLen]
                ),
                :ssh_connection_handler,
                1163,
      ...
      {:error, {:exceeds_max_size, packetLen}} ->
        {shutdown, d} =
          :ssh_connection_handler.send_disconnect(
            2,
            :io_lib.format(
              ~c"Bad packet: Size (~p bytes) exceeds max size",
              [packetLen]
            ),
            :ssh_connection_handler,
            1189,
  ...
  def handle_event(:internal, :prepare_next_packet, _StateName, d) do
    enough =
      :erlang.max(
        8,
  ...
  def handle_event(
    ...
    disconnect_fun(~c"Received a transport close", d)
    {:stop_and_reply, {:shutdown, ~c"Connection closed"}, repls, d}
  end
  ...
  def handle_event({:timeout, :max_initial_idle_time}, _Data, _StateName, _D) do
    {:stop, {:shutdown, ~c"Timeout"}}
  end
    ...
    case unexpected_fun(unexpectedMessage, d) do
      :report ->
        msg =
          :lists.flatten(
            :io_lib.format(
              ~c"*** SSH: Unexpected message '~p' received in state '~p'\nRole: ~p\nPeer: ~p\nLocal Address: ~p\n",
              [
                unexpectedMessage,
                stateName,
                r_ssh(ssh, :role),
      ...
      other ->
        msg =
          :lists.flatten(
            :io_lib.format(
              ~c"*** SSH: Call to fun in 'unexpectedfun' failed:~nReturn: ~p\nMessage: ~p\nRole: ~p\nPeer: ~p\nLocal Address: ~p\n",
              [
                other,
                unexpectedMessage,
                r_ssh(ssh, :role),
  ...
  def handle_event(type, ev, stateName, d0) do
    details =
      case (try do
        ...
        ~c"ssh_msg_" ++ _ when type == :internal ->
          :lists.flatten(
            :io_lib.format(
              ~c"Message ~p in wrong state (~p)",
              [:erlang.element(1, ev), stateName]
            )
          )

        _ ->
          :io_lib.format(~c"Unhandled event in state ~p and type ~p:~n~p", [stateName, type, ev])
      end
  ...
  def terminate(:shutdown, _StateName, d0) do
    d =
      send_msg(
        r_ssh_msg_disconnect(code: 11, description: ~c"Terminated (shutdown) by supervisor"),
        d0
      )
  ...
  def available_hkey_algorithms(:client, options) do
    case available_hkey_algos(options) do
      [] ->
        :erlang.error({:shutdown, ~c"No public key algs"})
  ...
  def available_hkey_algorithms(:server, options) do
    case (for a <- available_hkey_algos(options),
      ...
      [] ->
        :erlang.error({:shutdown, ~c"No host key available"})
  ...
  defp set_kex_overload_prefix(
    ...
    case (try do
      ...
      ~c"diffie-hellman-group-exchange-" ++ _ ->
        <<"dh_gex", msg::binary>>

      ~c"diffie-hellman-group" ++ _ ->
        <<"dh", msg::binary>>
  ...
  defp ext_info(
    ...
    sigAlgs =
      for astr <- :string.tokens(sigAlgsStr, ~c","),
          a <-
            (try do
  ...
  def send_disconnect(code, detailedText, module, line, stateName, d) do
    send_disconnect(code, default_text(code), detailedText, module, line, stateName, d)
  end

  def send_disconnect(code, reason, detailedText, module, line, stateName, d0) do
    msg = r_ssh_msg_disconnect(code: code, description: reason)
    d = send_msg(msg, d0)
    logMsg = :io_lib.format(~c"Disconnects with code = ~p [RFC4253 11.1]: ~s", [code, reason])
  ...
  defp call_disconnectfun_and_log_cond(logMsg, detailedText, module, line, stateName, d) do
    case disconnect_fun(logMsg, d) do
      :void ->
        log(:info, d, ~c"~s~nState = ~p~nModule = ~p, Line = ~p.~nDetails:~n  ~s~n", [
          logMsg,
          stateName,
          module,
          line,
  ...
  defp default_text(8) do
    ~c"Protocol version not supported"
  end

  defp default_text(9) do
    ~c"Host key not verifiable"
  end
    ...
    case s do
      r_ssh(role: role) when role == :server or role == :client ->
        {peerRole, peerVersion} =
          case role do
            :server ->
              {~c"Peer client", r_ssh(s, :c_version)}

            :client ->
              {~c"Peer server", r_ssh(s, :s_version)}
          end

        apply(:error_logger, f, [
          ~c"Erlang SSH ~p version: ~s ~s.~nAddress: ~s~n~s version: ~p~nPeer address: ~s~n~s~n",
          [
            role,
            ssh_log_version(),
            crypto_log_info(),
  ...
  defp limit_size(s, len, maxLen) when len > maxLen do
    :io_lib.format(
      ~c"~s ... (~w bytes skipped)",
      [:string.substr(:lists.flatten(s), 1, maxLen), len - maxLen]
    )
  end
  ...
  defp init_inet_buffers_window(socket) do
    ...
    winSz = :proplists.get_value(:recbuf, sockOpts, 10 * 65536)

    pktSz =
      min(
        :proplists.get_value(:buffer, sockOpts, 65536),
        65536
      )
  ...
  def ssh_dbg_on(:tcp) do
    ...
    :dbg.tpl(:ssh_connection_handler, :close_transport, 1, :x)
  end
  ...
  def ssh_dbg_format(
        :connections,
        {:call, {:ssh_connection_handler, :init, [[role, sock, opts]]}}
      ) do
    ...
    [
      :io_lib.format(~c"Starting ~p connection:\n", [role]),
      :io_lib.format(
        ~c"Socket = ~p, Peer = ~s, Local = ~s,~nNon-default options:~n~p",
        [
          sock,
          :ssh_lib.format_address_port(
            iPp,
  ...
  def ssh_dbg_format(
    ...
    [
      ~c"Connection event\n",
      :io_lib.format(~c"EventType: ~p~nEventContent: ~p~nState: ~p~n", [
        eventType,
        eventContent,
        state
      ])
  ...
  def ssh_dbg_format(
        :tcp,
        {:call,
         {:ssh_connection_handler, :handle_event, [:info, {:tcp, sock, tcpData}, state, _Data]}}
      ) do
    [
      ~c"TCP stream data arrived\n",
      :io_lib.format(
        ~c"State: ~p~nSocket: ~p~nTcpData:~n~s",
        [
          state,
          sock,
          :ssh_dbg.hex_dump(
  ...
  def ssh_dbg_format(
        :tcp,
        {:call,
         {:ssh_connection_handler, :handle_event, [:info, {:tcp_error, sock, msg}, state, _Data]}}
      ) do
    [
      ~c"TCP stream data ERROR arrived\n",
      :io_lib.format(~c"State: ~p~nSocket: ~p~nErrorMsg:~p~n", [state, sock, msg])
    ]
  end
  ...
  def ssh_dbg_format(
        :tcp,
        {:call,
         {:ssh_connection_handler, :handle_event, [:info, {:tcp_closed, sock}, state, _Data]}}
      ) do
    [~c"TCP stream closed\n", :io_lib.format(~c"State: ~p~nSocket: ~p~n", [state, sock])]
  end

  def ssh_dbg_format(
        :tcp,
        {:return_from, {:ssh_connection_handler, :handle_event, 4}, _Ret}
      ) do
    :skip
  end
  ...
  def ssh_dbg_format(
        :tcp,
        {:call, {:ssh_connection_handler, :send_bytes, [~c"", _D]}}
      ) do
    :skip
  end
  ...
  def ssh_dbg_format(
        :tcp,
        {:call, {:ssh_connection_handler, :send_bytes, [tcpData, r_data(socket: sock)]}}
      ) do
    [
      ~c"TCP send stream data\n",
      :io_lib.format(
        ~c"Socket: ~p~nTcpData:~n~s",
        [
          sock,
          :ssh_dbg.hex_dump(
            tcpData,
  ...
  def ssh_dbg_format(
        :tcp,
        {:return_from, {:ssh_connection_handler, :send_bytes, 2}, _R}
      ) do
    :skip
  end
  ...
  def ssh_dbg_format(
        :tcp,
        {:call, {:ssh_connection_handler, :close_transport, [r_data(socket: sock)]}}
      ) do
    [~c"TCP close stream\n", :io_lib.format(~c"Socket: ~p~n", [sock])]
  end

  def ssh_dbg_format(
        :tcp,
        {:return_from, {:ssh_connection_handler, :close_transport, 1}, _R}
      ) do
    :skip
  end
  ...
  def ssh_dbg_format(
    ...
    [
      ~c"Renegotiation: start timer (init_renegotiate_timers)\n",
      :io_lib.format(
        ~c"State: ~p  -->  ~p~nrekey_limit: ~p ({ms,bytes})~ncheck_data_size: ~p (ms)~n",
        [
          oldState,
          newState,
          :ssh_options.get_value(
  ...
  def ssh_dbg_format(
    ...
    [
      ~c"Renegotiation: renegotiation forced\n",
      :io_lib.format(
        ~c"~p:renegotiate(~p) called~n",
  ...
  def ssh_dbg_format(
    ...
    [
      ~c"Renegotiation: pause timers\n",
      :io_lib.format(~c"State: ~p  -->  ~p~n", [oldState, newState])
    ]
  end
  ...
  def ssh_dbg_format(
    ...
    [~c"Renegotiation: start rekeying\n"]
  end
  ...
  def ssh_dbg_format(
    ...
    [
      ~c"Renegotiation: check size of data sent\n",
      :io_lib.format(
        ~c"TotalSentSinceRekey: ~p~nMaxBeforeRekey: ~p~nStartRekey: ~p~n",
  ...
  def ssh_dbg_format(
    ...
    extraInfo =
      try do
        ...
        when is_tuple(iPp) and is_tuple(iPs) and
          ...
          :io_lib.format(
            ~c"Peer=~s:~p, Local=~s:~p, User=~p",
            [:inet.ntoa(iPp), portp, :inet.ntoa(iPs), ports, usr]
          )

        {peer, usr, sockname} ->
          :io_lib.format(~c"Peer=~p, Local=~p, User=~p", [peer, sockname, usr])
      end
    ...
    cond do
      reason == :normal or reason == :shutdown or
        ...
        [
          ~c"Connection Terminating:\n",
          :io_lib.format(~c"Reason: ~p, StateName: ~p~n~s", [reason, stateName, extraInfo])
        ]
      ...
      true ->
        [
          ~c"Connection Terminating:\n",
          :io_lib.format(
            ~c"Reason: ~p, StateName: ~p~n~s~nStateData = ~p",
  ...
  def ssh_dbg_format(
    ...
    [
      ~c"Disconnecting:\n",
      :io_lib.format(
        ~c" Module = ~p, Line = ~p, StateName = ~p,~n Code = ~p, Reason = ~p,~n DetailedText =~n ~p",
  ...
  def ssh_dbg_format(
        :connection_handshake,
        {:call, {:ssh_connection_handler, :handshake, [pid, ref, timeout]}},
        stack
      ) do
    {[
       ~c"Connection handshake\n",
       :io_lib.format(~c"Connection Child: ~p~nReg: ~p~nTimeout: ~p~n", [pid, ref, timeout])
     ], [pid | stack]}
  end
  ...
  def ssh_dbg_format(
    ...
    {[
       :lists.flatten(:io_lib.format(~c"Connection handshake result ~p\n", [tag])),
       :io_lib.format(~c"Connection Child: ~p~nRet: ~p~n", [pid, ret])
     ], stack}
  end
end
