defmodule :m_ssh_auth do
  ...
  defp sort_select_mthds(clients, :undefined, servers) do
    sort_select_mthds1(
      clients,
      servers,
      :string.tokens(~c"publickey,keyboard-interactive,password", ~c",")
    )
  end

  defp sort_select_mthds(clients, users0, servers0) do
    sort_select_mthds1(clients, :string.tokens(users0, ~c","), servers0)
  end
  ...
  def password_msg([r_ssh(opts: opts, user: user, service: service) = ssh0]) do
    ...
    {password, ssh} =
      case :ssh_options.get_value(:user_options, :password, opts, :ssh_auth, 103) do
        ...
        :undefined ->
          {ioCb.read_password(~c"ssh password: ", opts), ssh0}
        ...
        {r_ssh_msg_userauth_request(
           ...
           data:
             <<0::size(8)-unsigned-big-integer,
               byte_size(:unicode.characters_to_binary(password))::size(32)-unsigned-big-integer,
    ...
    case :ssh_options.get_value(:user_options, :password, opts, :ssh_auth, 129) do
      ...
      _ ->
        {r_ssh_msg_userauth_request(
           ...
           data:
             <<byte_size("")::size(32)-unsigned-big-integer, ""::binary,
               byte_size(<<>>)::size(32)-unsigned-big-integer, <<>>::binary>>
         ), ssh}
    end
  end
          ...
          {:ok, sig} ->
            sigBlob =
              :erlang.list_to_binary([
                <<byte_size(:unicode.characters_to_binary(sigAlgStr))::size(32)-unsigned-big-integer,
                  :unicode.characters_to_binary(sigAlgStr)::binary>>,
                <<byte_size(sig)::size(32)-unsigned-big-integer, sig::binary>>
              ])
            ...
            {r_ssh_msg_userauth_request(
               ...
               data: [
                 ...
                 <<byte_size(:unicode.characters_to_binary(sigAlgStr))::size(32)-unsigned-big-integer,
                   :unicode.characters_to_binary(sigAlgStr)::binary>>,
                 <<byte_size(pubKeyBlob)::size(32)-unsigned-big-integer, pubKeyBlob::binary>>,
                 <<byte_size(sigBlob)::size(32)-unsigned-big-integer, sigBlob::binary>>
               ]
             ), ssh}
  ...
  def service_request_msg(ssh) do
    {r_ssh_msg_service_request(name: ~c"ssh-userauth"), r_ssh(ssh, service: ~c"ssh-userauth")}
  end

  def init_userauth_request_msg(r_ssh(opts: opts) = ssh) do
    case :ssh_options.get_value(:user_options, :user, opts, :ssh_auth, 210) do
      :undefined ->
        :ssh_connection_handler.disconnect(
          15,
          ~c"Could not determine the users name",
          :ssh_auth,
          213
        )
  ...
  def handle_userauth_request(r_ssh_msg_service_request(name: name = ~c"ssh-userauth"), _, ssh) do
    {:ok, {r_ssh_msg_service_accept(name: name), r_ssh(ssh, service: ~c"ssh-connection")}}
  end
  ...
  def handle_userauth_request(
    ...
    case check_password(user, password, ssh) do
      ...
      {false, ssh1} ->
        {:not_authorized, {user, {:error, ~c"Bad user or password"}},
  ...
  def handle_userauth_request(
    ...
    {:not_authorized, {user, {:error, ~c"Password change not supported"}},
     {r_ssh_msg_userauth_failure(authentications: methods, partial_success: false), ssh}}
  end

  def handle_userauth_request(
        r_ssh_msg_userauth_request(user: user, service: ~c"ssh-connection", method: ~c"none"),
        _,
        r_ssh(
    ...
    case :ssh_options.get_value(:user_options, :no_auth_needed, opts, :ssh_auth, 277) do
      false ->
  ...
  def handle_userauth_request(
    ...
    case kbTriesLeft do
      n when n < 1 ->
        {:not_authorized, {user, {:authmethod, ~c"keyboard-interactive"}},
         {r_ssh_msg_userauth_failure(authentications: methods, partial_success: false), ssh}}

      _ ->
        default =
          {~c"SSH server", ~c"Enter password for \"" ++ user ++ ~c"\"", ~c"password: ", false}
        ...
        {name, instruction, prompt, echo} =
          case :ssh_options.get_value(
            ...
            f when is_function(f, 4) ->
              ...
              f.(peerName, user, ~c"ssh-connection", r_ssh(ssh, :pwdfun_user_state))

            f when is_function(f) ->
              {_, peerName} = r_ssh(ssh, :peer)
              f.(peerName, user, ~c"ssh-connection")
          end
  ...
  def handle_userauth_request(
        r_ssh_msg_userauth_request(user: user, service: ~c"ssh-connection", method: other),
        _,
        r_ssh(userauth_supported_methods: methods) = ssh
      ) do
  ...
  def handle_userauth_info_response(
    ...
    sendOneEmpty =
      :ssh_options.get_value(:user_options, :tstflg, opts, :ssh_auth, 450) == :one_empty or
        :proplists.get_value(
          :one_empty,
  ...
  def handle_userauth_info_response(r_ssh_msg_userauth_info_response(), _Auth) do
    :ssh_connection_handler.disconnect(
      7,
      ~c"Server does not support keyboard-interactive",
      :ssh_auth,
      484
    )
  end
  ...
  defp method_preference(sigKeyAlgs) do
    pubKeyDefs =
      for a <- sigKeyAlgs do
        {~c"publickey", :ssh_auth, :publickey_msg, [a]}
      end

    nonPKmethods = [
      {~c"password", :ssh_auth, :password_msg, []},
      {~c"keyboard-interactive", :ssh_auth, :keyboard_interactive_msg, []}
    ]
  ...
  defp check_user(user, ssh) do
    case :ssh_options.get_value(:user_options, :pk_check_user, r_ssh(ssh, :opts), :ssh_auth, 499) do
      true ->
  ...
  defp check_password(user, password, r_ssh(opts: opts) = ssh) do
    case :ssh_options.get_value(:user_options, :pwdfun, opts, :ssh_auth, 507) do
      :undefined when password == :pubkey ->
        case :lists.keysearch(
               user,
               1,
               :ssh_options.get_value(:user_options, :user_passwords, opts, :ssh_auth, 510)
             ) do
          {:value, {^user, _}} ->
            {true, ssh}
      ...
      checker when is_function(checker, 4) ->
        ...
        case checker.(user, password, peerAddr, privateState) do
          ...
          :disconnect ->
            :ssh_connection_handler.disconnect(14, ~c"", :ssh_auth, 537)
        end
    end
  end

  defp get_password_option(opts, user) do
    passwords = :ssh_options.get_value(:user_options, :user_passwords, opts, :ssh_auth, 542)
    ...
    case :lists.keysearch(user, 1, passwords) do
      ...
      false ->
        :ssh_options.get_value(:user_options, :password, opts, :ssh_auth, 545)
    end
  end
  ...
  defp verify_sig(sessionId, user, service, algBin, keyBlob, sigWLen, r_ssh(opts: opts) = ssh) do
    try do
      ...
      true =
        :lists.member(
          :erlang.list_to_existing_atom(alg),
          :proplists.get_value(
            :public_key,
            :ssh_options.get_value(
              :user_options,
      ...
      :ssh_transport.verify(plainText, :erlang.list_to_existing_atom(alg), sig, key, ssh)
    catch
      _, _ ->
        false
    end
  ...
  defp build_sig_data(sessionId, user, service, keyBlob, alg) do
    sig = [
      <<byte_size(sessionId)::size(32)-unsigned-big-integer, sessionId::binary>>,
      50,
      ...
      <<byte_size("publickey")::size(32)-unsigned-big-integer, "publickey"::binary>>,
      1,
      <<byte_size(:unicode.characters_to_binary(alg))::size(32)-unsigned-big-integer,
        :unicode.characters_to_binary(alg)::binary>>,
      <<byte_size(keyBlob)::size(32)-unsigned-big-integer, keyBlob::binary>>
    ]
  ...
  defp keyboard_interact_get_responses(opts, name, instr, promptInfos) do
    keyboard_interact_get_responses(
      :ssh_options.get_value(:user_options, :user_interaction, opts, :ssh_auth, 601),
      :ssh_options.get_value(:user_options, :keyboard_interact_fun, opts, :ssh_auth, 602),
      :ssh_options.get_value(:user_options, :password, opts, :ssh_auth, 603),
      name,
      instr,
      promptInfos,
      opts
  ...
  defp prompt_user_for_passwords(name, instr, promptInfos, opts) do
    ioCb = :ssh_options.get_value(:internal_options, :io_cb, opts, :ssh_auth, 641)
  ...
  def ssh_dbg_format(
    ...
    {[
       ~c"AUTH srvr: Peer client authorized\n",
       :io_lib.format(~c"user = ~p~n", [user]),
       fmt_req(req, ssh)
     ], stack}
  end
  ...
  def ssh_dbg_format(
        ...
        [{r_ssh_msg_userauth_request(method: ~c"none"), ssh} | stack]
      ) do
    methods = r_ssh(ssh, :userauth_supported_methods)

    {[
       ~c"AUTH srvr: Peer queries auth methods\n",
       :io_lib.format(~c"user = ~p~nsupported methods = ~p ?", [user, methods])
     ], stack}
  end
  ...
  def ssh_dbg_format(
    ...
    {case repl do
       {r_ssh_msg_userauth_pk_ok(), _} ->
         [~c"AUTH srvr: Answer - pub key supported\n"]

       {r_ssh_msg_userauth_failure(), _} ->
         [~c"AUTH srvr: Answer - pub key not supported\n"]

       {other, _} ->
         [
           ~c"AUTH srvr: Answer - strange answer\n",
           :io_lib.format(~c"strange answer = ~p~n", [other])
         ]
     end ++ [:io_lib.format(~c"user = ~p~n", [user]), fmt_req(req, ssh)], stack}
  end
  ...
  def ssh_dbg_format(
        ...
        [{r_ssh_msg_userauth_request(method: ~c"keyboard-interactive") = req, ssh} | stack]
      ) do
    {[
       ~c"AUTH srvr: Ask peer client for password\n",
       :io_lib.format(~c"user = ~p~n", [user]),
       fmt_req(req, ssh)
     ], stack}
  end
  ...
  def ssh_dbg_format(
    ...
    {[
       ~c"AUTH srvr: Peer client authorization failed\n",
       :io_lib.format(~c"user = ~p~n", [user]),
       fmt_req(req, ssh)
     ], stack}
  end
  ...
  def ssh_dbg_format(
    ...
    {[
       ~c"AUTH client: Service ssh-userauth accepted\n",
       case :ssh_options.get_value(:user_options, :user, opts, :ssh_auth, 766) do
         :undefined ->
           :io_lib.format(~c"user = undefined *** ERROR ***", [])

         user ->
           :io_lib.format(~c"user = ~p", [user])
       end
     ], stack}
  end
  ...
  def ssh_dbg_format(
    ...
    {[~c"AUTH client: Query for accepted methods\n", :io_lib.format(~c"user = ~p", [user])],
     [repl | stack]}
  end
  ...
  def ssh_dbg_format(
    ...
    {[
       ~c"AUTH client: Server supports\n",
       :io_lib.format(~c"user = ~p~nmethods = ~p", [user, methods])
     ], stack}
  end
  ...
  def ssh_dbg_format(
    ...
    {[~c"AUTH client: Try auth with\n", :io_lib.format(~c"method = ~p", [method])], stack}
  end
  ...
  def ssh_dbg_format(:authentication, unhandled, stack) do
    ...
    {[
       ~c"UNHANDLED AUTH FORMAT\n",
       :io_lib.format(~c"Unhandled = ~p~nStack = ~p", [unhandled, stack])
     ], stack}
  end
  ...
  defp fmt_req(
    ...
    [
      :io_lib.format(~c"req user = ~p~nreq method = ~p~nsupported methods = ~p", [
        user,
        method,
        methods
      ]),
          ...
          [
            case data do
              <<_::size(8)-unsigned-big-integer, aLen::size(32)-unsigned-big-integer,
                ...
                :io_lib.format(~c"~nkey-type = ~p", [alg])
  ...
  defp fmt_kb_tries_left(n) when is_integer(n) do
    :io_lib.format(~c"~ntries left = ~p", [n - 1])
  end
