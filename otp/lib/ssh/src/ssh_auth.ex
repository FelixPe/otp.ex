defmodule :m_ssh_auth do
  ...
  Record.defrecord(:r_ssh_agent_key, :ssh_agent_key, blob: :undefined,
                                         comment: :undefined)
  Record.defrecord(:r_ssh_agent_identities_response, :ssh_agent_identities_response, keys: :undefined)
  Record.defrecord(:r_ssh_agent_sign_request, :ssh_agent_sign_request, key_blob: :undefined,
                                                  data: :undefined,
                                                  flags: :undefined)
  Record.defrecord(:r_ssh_agent_signature, :ssh_agent_signature, format: :undefined,
                                               blob: :undefined)
  ...
  Record.defrecord(:r_ssh_msg_kex_ecdh_reply, :ssh_msg_kex_ecdh_reply, public_host_key: :undefined,
                                                  ...
                                                  h_sig: :undefined)
  @behaviour :ssh_dbg
  def userauth_request_msg(r_ssh(userauth_methods: serverMethods,
             userauth_supported_methods: userPrefMethods,
             userauth_preference: clientMethods0) = ssh0) do
    case (sort_select_mthds(clientMethods0, userPrefMethods,
                              serverMethods)) do
      [] ->
  ...
  defp sort_select_mthds1(clients, users0, servers0) do
    ...
    users = unique(users0)
    for key <- users, :lists.member(key, servers),
          c <- clients, :erlang.element(1, c) == key do
      c
    end
  end

  defp unique(l) do
    :lists.reverse(:lists.foldl(fn e, acc ->
  ...
  def password_msg([r_ssh(opts: opts, user: user,
              service: service) = ssh0]) do
    ioCb = :ssh_options.get_value(:internal_options, :io_cb,
                                    opts, :ssh_auth, 101)
    {password,
       ssh} = (case (:ssh_options.get_value(:user_options,
                                              :password, opts, :ssh_auth,
                                              103)) do
                 ...
                 pW ->
                   {pW,
                      r_ssh(ssh0, opts: :ssh_options.put_value(:user_options,
                                                             {:password,
                                                                :not_ok},
                                                             opts, :ssh_auth,
                                                             110))}
    ...
    case (password) do
      :not_ok ->
        {:not_ok, ssh}
      _ ->
        {r_ssh_msg_userauth_request(user: user, service: service, method: 'password',
             data: <<0 :: size(8) - unsigned - big - integer,
                       byte_size(:unicode.characters_to_binary(password))
                       ::
  ...
  def keyboard_interactive_msg([r_ssh(user: user, opts: opts,
              service: service) = ssh]) do
    case (:ssh_options.get_value(:user_options, :password,
                                   opts, :ssh_auth, 129)) do
      :not_ok ->
        {:not_ok, ssh}
      _ ->
        {r_ssh_msg_userauth_request(user: user, service: service, method: 'keyboard-interactive',
             data: <<byte_size("")
                     ::
                     size(32) - unsigned - big - integer,
                       "" :: binary,
                       byte_size(<<>>) :: size(32) - unsigned - big - integer,
                       <<>> :: binary>>),
           ssh}
    end
  end
  ...
  def get_public_key(sigAlg, r_ssh(opts: opts)) do
    ...
    case (:ssh_transport.call_KeyCb(:user_key, [keyAlg],
                                      opts)) do
      {:ok, {:ssh2_pubkey, pubKeyBlob}} ->
        {:ok, {:ssh2_pubkey, pubKeyBlob}}
      {:ok, privKey} ->
        try do
          true = :ssh_transport.valid_key_sha_alg(:private,
                                                    privKey, keyAlg)
  ...
  def publickey_msg([sigAlg, r_ssh(user: user, session_id: sessionId,
                      service: service, opts: opts) = ssh]) do
      ...
      {:ok, {_, pubKeyBlob} = key} ->
        ...
        sigRes = (case (key) do
                    {:ssh2_pubkey, ^pubKeyBlob} ->
                      {:ok,
                         :ssh_transport.call_KeyCb(:sign, [pubKeyBlob, sigData],
                                                     opts)}
                    {privKey, ^pubKeyBlob} ->
        ...
        case (sigRes) do
          {:ok, sig} ->
            sigBlob = :erlang.list_to_binary([<<byte_size(:unicode.characters_to_binary(sigAlgStr))
                                                ::
                                                size(32) - unsigned - big -
                                                  integer,
                                                  :unicode.characters_to_binary(sigAlgStr)
                                                  ::
                                                  binary>>,
                                                  <<byte_size(sig)
                                                    ::
                                                    size(32) - unsigned - big -
                                                      integer,
                                                      sig :: binary>>])
            {r_ssh_msg_userauth_request(user: user, service: service, method: 'publickey',
                 data: [1,
                            <<byte_size(:unicode.characters_to_binary(sigAlgStr))
                              ::
                              size(32) - unsigned - big - integer,
                                :unicode.characters_to_binary(sigAlgStr)
                                ::
                                binary>>,
                                <<byte_size(pubKeyBlob)
                                  ::
                                  size(32) - unsigned - big - integer,
                                    pubKeyBlob :: binary>>,
                                    <<byte_size(sigBlob)
  ...
  def init_userauth_request_msg(r_ssh(opts: opts) = ssh) do
    case (:ssh_options.get_value(:user_options, :user, opts,
                                   :ssh_auth, 210)) do
      :undefined ->
        :ssh_connection_handler.disconnect(15, 'Could not determine the users name', :ssh_auth,
                                             213)
      user ->
        {r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'none', data: <<>>),
           r_ssh(ssh, user: user, 
                    userauth_preference: method_preference(r_ssh(ssh, :userauth_pubkeys)), 
                    userauth_methods: :none,  service: 'ssh-connection')}
    end
  end
  ...
  def handle_userauth_request(r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'password',
             data: <<0, sz :: size(32) - unsigned - big - integer,
                       binPwd :: size(sz) - binary>>),
           _, r_ssh(userauth_supported_methods: methods) = ssh) do
  ...
  def handle_userauth_request(r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'none'), _,
           r_ssh(userauth_supported_methods: methods,
               opts: opts) = ssh) do
    case (:ssh_options.get_value(:user_options,
                                   :no_auth_needed, opts, :ssh_auth, 277)) do
      false ->
  ...
  def handle_userauth_request(r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'publickey',
             data: <<0 :: size(8) - unsigned - big - integer,
                       aLen :: size(32) - unsigned - big - integer,
                       bAlg :: size(aLen) - binary,
                       kLen :: size(32) - unsigned - big - integer,
                       keyBlob :: size(kLen) - binary, _ :: binary>>),
           _SessionId,
           r_ssh(userauth_supported_methods: methods) = ssh0) do
    ssh = (case (check_user(user, ssh0)) do
             {true, ssh01} ->
    ...
    case (pre_verify_sig(user, keyBlob, ssh)) do
      true ->
        {:not_authorized, {user, :undefined},
           {r_ssh_msg_userauth_pk_ok(algorithm_name: :erlang.binary_to_list(bAlg),
                key_blob: keyBlob),
              ssh}}
      false ->
        {:not_authorized, {user, :undefined},
  ...
  def handle_userauth_request(r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'publickey',
             data: <<1 :: size(8) - unsigned - big - integer,
                       aLen :: size(32) - unsigned - big - integer,
                       bAlg :: size(aLen) - binary,
                       kLen :: size(32) - unsigned - big - integer,
                       keyBlob :: size(kLen) - binary, sigWLen :: binary>>),
           sessionId,
           r_ssh(user: preVerifyUser,
               userauth_supported_methods: methods) = ssh0) do
    {userOk, ssh} = check_user(user, ssh0)
    case ((preVerifyUser == user or preVerifyUser == :undefined) and userOk and verify_sig(sessionId,
                                                                                             user,
                                                                                             'ssh-connection',
                                                                                             bAlg,
                                                                                             keyBlob,
  ...
  def handle_userauth_request(r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'keyboard-interactive', data: _), _,
           r_ssh(opts: opts, kb_tries_left: kbTriesLeft,
               userauth_supported_methods: methods) = ssh) do
    case (kbTriesLeft) do
      n when n < 1 ->
      ...
      _ ->
        ...
        {name, instruction, prompt,
           echo} = (case (:ssh_options.get_value(:user_options,
                                                   :auth_method_kb_interactive_data,
                                                   opts, :ssh_auth, 384)) do
                      :undefined ->
                        default
                      {_, _, _, _} = v ->
                        v
        ...
        echoEnc = (case (echo) do
                     true ->
                       <<1>>
                     false ->
                       <<0>>
                   end)
        msg = r_ssh_msg_userauth_info_request(name: :unicode.characters_to_list(name),
                  instruction: :unicode.characters_to_list(instruction),
                  language_tag: '', num_prompts: 1,
                  data: <<byte_size(:unicode.characters_to_binary(prompt))
                          ::
                          size(32) - unsigned - big - integer,
                            :unicode.characters_to_binary(prompt) :: binary,
                            echoEnc :: binary>>)
        {:not_authorized, {user, :undefined},
           {msg, r_ssh(ssh, user: user)}}
    end
  end

  def handle_userauth_request(r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: other), _,
           r_ssh(userauth_supported_methods: methods) = ssh) do
    {:not_authorized, {user, {:authmethod, other}},
       {r_ssh_msg_userauth_failure(authentications: methods, partial_success: false),
          ssh}}
  end

  def handle_userauth_info_request(r_ssh_msg_userauth_info_request(name: name, instruction: instr,
             num_prompts: numPrompts, data: data),
           r_ssh(opts: opts) = ssh) do
    promptInfos = decode_keyboard_interactive_prompts(numPrompts,
                                                        data)
    case (keyboard_interact_get_responses(opts, name, instr,
                                            promptInfos)) do
      :not_ok ->
        :not_ok
      responses ->
        {:ok,
           {r_ssh_msg_userauth_info_response(num_responses: numPrompts, data: responses), ssh}}
    end
  end

  def handle_userauth_info_response(r_ssh_msg_userauth_info_response(num_responses: 1,
             data: <<sz :: size(32) - unsigned - big - integer,
                       password :: size(sz) - binary>>),
           r_ssh(opts: opts, kb_tries_left: kbTriesLeft, user: user,
               userauth_supported_methods: methods) = ssh) do
    sendOneEmpty = :ssh_options.get_value(:user_options,
                                            :tstflg, opts, :ssh_auth,
                                            450) == :one_empty or :proplists.get_value(:one_empty,
    ...
    case (check_password(user,
                           :unicode.characters_to_list(password), ssh)) do
      {true, ssh1} when sendOneEmpty == true ->
        {:authorized_but_one_more, user,
           {r_ssh_msg_userauth_info_request(name: '', instruction: '', language_tag: '',
                num_prompts: 0,
                data: <<0 :: size(8) - unsigned - big - integer>>),
              ssh1}}
      {true, ssh1} ->
  ...
  def handle_userauth_info_response({:extra, r_ssh_msg_userauth_info_response()}, r_ssh(user: user) = ssh) do
    {:authorized, user, {r_ssh_msg_userauth_success(), ssh}}
  end

  def handle_userauth_info_response(r_ssh_msg_userauth_info_response(), _Auth) do
    :ssh_connection_handler.disconnect(7, 'Server does not support keyboard-interactive', :ssh_auth, 484)
  end

  defp method_preference(sigKeyAlgs) do
    pubKeyDefs = (for a <- sigKeyAlgs do
                    {'publickey', :ssh_auth, :publickey_msg, [a]}
                  end)
    nonPKmethods = [{'password', :ssh_auth, :password_msg, []}, {'keyboard-interactive',
                                                          :ssh_auth,
  ...
  defp check_user(user, ssh) do
    case (:ssh_options.get_value(:user_options,
                                   :pk_check_user, r_ssh(ssh, :opts), :ssh_auth,
                                   499)) do
      true ->
  ...
  defp check_password(user, password, r_ssh(opts: opts) = ssh) do
    case (:ssh_options.get_value(:user_options, :pwdfun,
                                   opts, :ssh_auth, 507)) do
      :undefined when password == :pubkey ->
        case (:lists.keysearch(user, 1,
                                 :ssh_options.get_value(:user_options,
                                                          :user_passwords, opts,
                                                          :ssh_auth, 510))) do
          {:value, {^user, _}} ->
      ...
      checker when is_function(checker, 4) ->
        r_ssh(pwdfun_user_state: privateState,
            peer: {_, peerAddr = {_, _}}) = ssh
        case (checker.(user, password, peerAddr,
                         privateState)) do
          true ->
            {true, ssh}
          false ->
          ...
          :disconnect ->
            :ssh_connection_handler.disconnect(14, '', :ssh_auth,
                                                 537)
        end
    end
  end
  ...
  defp get_password_option(opts, user) do
    passwords = :ssh_options.get_value(:user_options,
                                         :user_passwords, opts, :ssh_auth, 542)
    case (:lists.keysearch(user, 1, passwords)) do
      {:value, {^user, pw}} ->
        pw
      false ->
        :ssh_options.get_value(:user_options, :password, opts,
                                 :ssh_auth, 545)
    end
  end
  ...
  defp pre_verify_sig(user, keyBlob, r_ssh(opts: opts)) do
    try do
      ...
      :ssh_transport.call_KeyCb(:is_auth_key, [key, user],
                                  opts)
    catch
      _, _ ->
        false
  ...
  defp verify_sig(sessionId, user, service, algBin, keyBlob,
            sigWLen, r_ssh(opts: opts) = ssh) do
    try do
      alg = :erlang.binary_to_list(algBin)
      true = :lists.member(:erlang.list_to_existing_atom(alg),
                             :proplists.get_value(:public_key,
                                                    :ssh_options.get_value(:user_options,
                                                                             ...
                                                                             562)))
      key = :ssh_message.ssh2_pubkey_decode(keyBlob)
      true = :ssh_transport.call_KeyCb(:is_auth_key,
                                         [key, user], opts)
      plainText = build_sig_data(sessionId, user, service,
                                   keyBlob, alg)
      <<algSigLen :: size(32) - unsigned - big - integer,
          algSig :: size(algSigLen) - binary>> = sigWLen
      <<algLen :: size(32) - unsigned - big - integer,
          _Alg :: size(algLen) - binary,
          sigLen :: size(32) - unsigned - big - integer,
          sig :: size(sigLen) - binary>> = algSig
      :ssh_transport.verify(plainText,
                              :erlang.list_to_existing_atom(alg), sig, key, ssh)
    catch
      _, _ ->
        false
    end
  ...
  defp build_sig_data(sessionId, user, service, keyBlob, alg) do
    sig = [<<byte_size(sessionId)
             ::
             size(32) - unsigned - big - integer,
               sessionId :: binary>>,
               50, <<byte_size(:unicode.characters_to_binary(user))
                     ::
                     size(32) - unsigned - big - integer,
                         ...
                         size(32) - unsigned - big - integer,
                           ...
                           <<byte_size("publickey")
                             ...
                             size(32) - unsigned - big - integer,
                               "publickey" :: binary>>,
                               1,
                                   <<byte_size(:unicode.characters_to_binary(alg))
                                     ::
                                     size(32) - unsigned - big - integer,
                                       :unicode.characters_to_binary(alg)
                                       ::
                                       binary>>,
                                       <<byte_size(keyBlob)
                                         ::
  ...
  defp keyboard_interact_get_responses(opts, name, instr, promptInfos) do
    keyboard_interact_get_responses(:ssh_options.get_value(:user_options,
                                                             :user_interaction,
                                                             opts, :ssh_auth,
                                                             601),
                                      :ssh_options.get_value(:user_options,
  ...
  defp keyboard_interact_get_responses(false, :undefined, :undefined, _, _,
            [prompt | _], opts) do
    :ssh_no_io.read_line(prompt, opts)
  end

  defp keyboard_interact_get_responses(true, :undefined, _, name, instr, promptInfos,
            opts) do
    prompt_user_for_passwords(name, instr, promptInfos,
                                opts)
  end

  defp keyboard_interact_get_responses(true, fun, _Pwd, name, instr, promptInfos,
            _Opts) do
  ...
  defp keyboard_interact_fun(kbdInteractFun, name, instr, promptInfos) do
    case (kbdInteractFun.(name, instr, promptInfos)) do
      responses when (is_list(responses) and
  ...
  def ssh_dbg_format(:authentication,
           {:call,
  ...
  def ssh_dbg_format(:authentication,
           ...
           [{r_ssh_msg_service_request(name: name), _} | stack]) do
    {:skip, stack}
  end
  ...
  def ssh_dbg_format(:authentication,
           ...
           [{r_ssh_msg_userauth_request() = req, ssh} | stack]) do
  ...
  def ssh_dbg_format(:authentication,
           ...
           [{r_ssh_msg_userauth_request(method: 'none'), ssh} | stack]) do
  ...
  def ssh_dbg_format(:authentication,
           ...
           [{r_ssh_msg_userauth_request(method: 'publickey',
                 data: <<0 :: size(8) - unsigned - big - integer,
                           _ :: binary>>) = req,
               ssh} |
                stack]) do
    {(case (repl) do
      ...
      end) ++ [:io_lib.format('user = ~p~n', [user]), fmt_req(req, ssh)],
       stack}
  end
  ...
  def ssh_dbg_format(:authentication,
           ...
           [{r_ssh_msg_userauth_request(method: 'keyboard-interactive') = req, ssh} | stack]) do
  ...
  def ssh_dbg_format(:authentication,
           {:return_from, {:ssh_auth, :ssh_msg_userauth_result, 1},
              _Result},
           stack) do
    {:skip, stack}
  end
  ...
  def ssh_dbg_format(:authentication,
           {:call,
              {:ssh_auth, :init_userauth_request_msg,
                 [r_ssh(opts: opts)]}},
           stack) do
    {['AUTH client: Service ssh-userauth accepted\n', case (:ssh_options.get_value(:user_options, :user,
                                        opts, :ssh_auth, 766)) do
           :undefined ->
  ...
  def ssh_dbg_format(:authentication,
           {:return_from,
              {:ssh_auth, :init_userauth_request_msg, 1},
              {repl = r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'none'), _Ssh}},
           stack) do
  ...
  def ssh_dbg_format(:authentication,
           {:call,
              {:ssh_auth, :userauth_request_msg,
                 [r_ssh(userauth_methods: methods)]}},
           [r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: 'none') | stack]) do
  ...
  def ssh_dbg_format(:authentication,
           {:return_from, {:ssh_auth, :userauth_request_msg, 1},
  ...
  defp fmt_req(r_ssh_msg_userauth_request(user: user, service: 'ssh-connection', method: method,
              data: data),
            r_ssh(kb_tries_left: kbTriesLeft,
                userauth_supported_methods: methods)) do
    [:io_lib.format('req user = ~p~nreq method = ~p~nsupported methods = ~p', [user, method, methods]),
         case (method) do
           'none' ->
             ''
           'password' ->
  ...
  defp fmt_bool(<<bool :: size(8) - unsigned - big - integer,
              _ :: binary>>) do
