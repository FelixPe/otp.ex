defmodule :m_ssh_options do
  ...
  def get_value(class, key, opts, _CallerMod, _CallerLine) do
    :erlang.error({:bad_options, class, key, opts,
                     _CallerMod, _CallerLine})
  end

  def get_value(:socket_options, key, opts, defFun, _CallerMod,
           _CallerLine)
      when is_map(opts) do
    :proplists.get_value(key,
                           :maps.get(:socket_options, opts), defFun)
  end
  ...
  def get_value(class, key, opts, _DefFun, _CallerMod,
           _CallerLine) do
    :erlang.error({:bad_options, class, key, opts,
                     _CallerMod, _CallerLine})
  end

  def put_value(:user_options, keyVal, opts, _CallerMod,
           _CallerLine)
      when is_map(opts) do
    put_user_value(keyVal, opts)
  end

  def put_value(:internal_options, keyVal, opts, _CallerMod,
           _CallerLine)
      when is_map(opts) do
    ...
    %{opts
      |
      internal_options:
      put_internal_value(keyVal, internalOpts)}
  end

  def put_value(:socket_options, keyVal, opts, _CallerMod,
           _CallerLine)
      when is_map(opts) do
    socketOpts = :maps.get(:socket_options, opts)
    %{opts
      |
      socket_options: put_socket_value(keyVal, socketOpts)}
  end
  ...
  def delete_key(:internal_options, key, opts, _CallerMod,
           _CallerLine)
      when is_map(opts) do
    internalOpts = :maps.get(:internal_options, opts)
    %{opts
      |
      internal_options: :maps.remove(key, internalOpts)}
  end

  def merge_options(role, newPropList, opts0)
      when (is_list(newPropList) and is_map(opts0)) do
    check_and_save(newPropList, default(role), opts0)
  end

  def handle_options(role, propList0) do
    handle_options(role, propList0,
                     %{socket_options: [], internal_options: %{},
                         key_cb_options: []})
  end

  defp handle_options(role, optsList0, opts0) when (is_map(opts0) and
                                          is_list(optsList0)) do
    optsList1 = :proplists.unfold(:lists.foldr(fn t, acc
                                                      when tuple_size(t) !== 2
                                                           ->
                                                    [{:special_trpt_args, t} |
                                                         acc]
    ...
    try do
      ...
      {initialMap, optsList2} = :maps.fold(fn k,
                                                %{default: vd}, {m, pL} ->
                                                case (config_val(k, roleCnfs,
                                                                   optsList1)) do
                                                  {:ok, v1} ->
                                                    {Map.merge(m, %{k => v1,
                                                                      key_cb_options:
                                             ...
                                             {Map.put(opts0, :key_cb_options,
                                                               :maps.get(:key_cb_options,
                                                                           opts0)),
                                                for {k, v} <- optsList1,
                                                      not
                                                      :maps.is_key(k, opts0) do
                                                  {k, v}
                                                end},
                                             optionDefinitions)
      check_and_save(optsList2, optionDefinitions, initialMap)
    catch
      :error, {eO, kV, reason} when eO == :eoptions or
                                      eO == :eerl_env
                                    ->
        cond do
          reason == :undefined ->
  ...
  defp check_and_save(optsList, optionDefinitions, initialMap) do
  ...
  defp config_val(:modify_algorithms = key, roleCnfs, opts) do
    v = (case (:application.get_env(:ssh, key)) do
           ...
           _ ->
             []
         end) ++ :proplists.get_value(key, roleCnfs,
  ...
  defp save(inet, defs, optMap) when inet == :inet or
                                     inet == :inet6 do
  ...
  defp save({:special_trpt_args, t}, _Defs, optMap)
      when is_map(optMap) do
    %{optMap
      |
      socket_options:
      [t | :maps.get(:socket_options, optMap)]}
  end
    ...
    try do
      ...
      :error, {:badkey, :inet} ->
        %{optMap
          |
          socket_options:
          [value | :maps.get(:socket_options, optMap)]}
      :error, {:badkey, ^key} ->
        %{optMap
          |
          socket_options:
          [{key, value} | :maps.get(:socket_options, optMap)]}
      ...
      :forbidden ->
        :erlang.error({:eoptions, {key, value},
                         :io_lib.format('The option \'~s\' is used internally. The user is not allowed to specify this option.', [key])})
    end
  end
  ...
  defp save(opt, _Defs, optMap) when is_map(optMap) do
    %{optMap
      |
      socket_options:
      [opt | :maps.get(:socket_options, optMap)]}
  end

  def no_sensitive(:rm, %{id_string: _, tstflg: _}) do
    :"*** removed ***"
  end

  def no_sensitive(:filter, opts = %{id_string: _, tstflg: _}) do
    sensitive = [:password, :user_passwords,
                                :dsa_pass_phrase, :rsa_pass_phrase,
                                                      :ecdsa_pass_phrase,
                                                          :ed25519_pass_phrase,
                                                              :ed448_pass_phrase]
  ...
  def no_sensitive(type, t) when is_tuple(t) do
    :erlang.list_to_tuple(no_sensitive(type,
                                         :erlang.tuple_to_list(t)))
  end

  def no_sensitive(_, x) do
    x
  end
  ...
  def default(:server) do
    Map.merge(default(:common), %{subsystems:
                                  %{default: [:ssh_sftpd.subsystem_spec([])],
                                      chk:
                                      fn l ->
                                           is_list(l) and :lists.all(fn {name,
                                                                           {cB,
                                                                              args}} ->
                                    ...
                                    %{default: 2 * 60 * 1000,
                                        ...
                                        class: :user_option},
                                    hello_timeout:
                                    %{default: 30 * 1000, chk: &check_timeout/1,
                                        class: :user_option},
                                    max_sessions:
                                    %{default: :infinity,
                                        chk:
  ...
  def default(:client) do
    Map.merge(default(:common), %{dsa_pass_phrase:
                                  %{default: :undefined,
                                      chk:
                                      fn v ->
                                           check_string(v)
    ...
    %{user_dir:
      %{default: false,
        ...
        %{default: 500,
            ...
            class: :user_option},
        rekey_limit:
        %{default: {3600000, 1024000000},
            chk:
            fn {:infinity, :infinity} ->
                 true
               {mins, :infinity} when (is_integer(mins) and mins > 0)
                                      ->
                 {true, {mins * 60 * 1000, :infinity}}
               {:infinity, bytes} when (is_integer(bytes) and
                                          bytes >= 0)
                                       ->
                 true
               {mins, bytes} when (is_integer(mins) and mins > 0 and
        ...
        %{default: 'publickey,keyboard-interactive,password',
            ...
            class: :user_option},
        send_ext_info:
        %{default: true, chk: &:erlang.is_boolean/1,
            class: :user_option},
        recv_ext_info:
        %{default: true, chk: &:erlang.is_boolean/1,
            class: :user_option},
  ...
  defp read_moduli_file(d, i, acc) do
    case (:io.get_line(d, '')) do
      ...
      data ->
        ...
        try do
          [_Time, _Class, _Tests, _Tries, size, g,
                                                    p] = :string.tokens(line, ' \r\n')
          m = {:erlang.list_to_integer(size),
                 {:erlang.list_to_integer(g),
  ...
  defp check_modify_algorithms(m) when is_list(m) do
    for op_KVs <- m,
          not is_tuple(op_KVs) or tuple_size(op_KVs) !== 2 or not
                                                              :lists.member(:erlang.element(1,
                                                                                              op_KVs),
                                                                              [:append,
                                                                                   :prepend,
                                                                                       :rm]) do
  ...
  defp normalize_mod_algs(kVs, useDefaultAlgs) do
    normalize_mod_algs(:ssh_transport.algo_classes(), kVs,
                         [], useDefaultAlgs)
  end
  ...
  defp normalize_mod_algs([k | ks], kVs0, acc, useDefaultAlgs) do
    ...
    normalize_mod_algs(ks, kVs, [{k, vs} | acc],
                         useDefaultAlgs)
  end
  ...
  defp normalize_mod_alg_list(k, vs, useDefaultAlgs) do
    normalize_mod_alg_list(k,
                             :ssh_transport.algo_two_spec_class(k), vs,
  ...
  defp normalize_mod_alg_list(k, true, [{:client2server, l1}],
            [_, {:server2client, l2}]) do
    [nml1(k, {:client2server, l1}), {:server2client, l2}]
  end

  defp normalize_mod_alg_list(k, true, [{:server2client, l2}],
            [{:client2server, l1}, _]) do
    [{:client2server, l1}, nml1(k, {:server2client, l2})]
  end

  defp normalize_mod_alg_list(k, true,
            [{:server2client, l2}, {:client2server, l1}], _) do
    [nml1(k, {:client2server, l1}), nml1(k,
                                           {:server2client, l2})]
  end

  defp normalize_mod_alg_list(k, true,
            [{:client2server, l1}, {:server2client, l2}], _) do
    [nml1(k, {:client2server, l1}), nml1(k,
                                           {:server2client, l2})]
  end
  ...
  defp nml1(k, {t, v}) when t == :client2server or
                            t == :server2client do
    {t, nml({k, t}, v)}
  end
  ...
  defp check_input_ok(algs) do
    for kVs <- algs,
          not is_tuple(kVs) or tuple_size(kVs) !== 2 do
      error_in_check(kVs, 'Bad preferred_algorithms')
    end
  end

  defp final_preferred_algorithms(options0) do
    result = (case (:ssh_options.get_value(:user_options,
                                             :modify_algorithms, options0,
                                             :ssh_options, 1198)) do
                :undefined ->
                ...
                modAlgs ->
                  rm_non_supported(false,
                                     eval_ops(:ssh_options.get_value(:user_options,
                                                                       :preferred_algorithms,
                                                                       options0,
                                                                       :ssh_options,
                                                                       1204),
    ...
    options1 = :ssh_options.put_value(:user_options,
                                        {:preferred_algorithms, result},
                                        options0, :ssh_options, 1208)
    case (:ssh_options.get_value(:user_options,
                                   :pref_public_key_algs, options1,
                                   :ssh_options, 1209)) do
      :undefined ->
        :ssh_options.put_value(:user_options,
                                 {:pref_public_key_algs,
                                    :proplists.get_value(:public_key, result)},
                                 options1, :ssh_options, 1211)
      _ ->
        options1
    end
  end
  ...
  defp eval_op(:rm, opt, pref, []) when (is_list(opt) and
                                      is_list(pref)) do
    pref -- opt
  end

  defp eval_op(:append, opt, pref, []) when (is_list(opt) and
                                          is_list(pref)) do
    (pref -- opt) ++ opt
  end

  defp eval_op(:prepend, opt, pref, []) when (is_list(opt) and
                                           is_list(pref)) do
    opt ++ pref -- opt
  end
  ...
  defp rmns(k, vs, unsupIsErrorFlg) do
    case (:ssh_transport.algo_two_spec_class(k)) do
      false ->
        rm_unsup(vs, :ssh_transport.supported_algorithms(k),
                   unsupIsErrorFlg, k)
      true ->
        for {{c, vsx}, {c, sup}} <- :lists.zip(vs,
                                                 :ssh_transport.supported_algorithms(k)) do
  ...
  defp rm_unsup(a, b, flg, errInf) do
    case (a -- b) do
      unsup = [_ | _] when flg == true ->
        :erlang.error({:eoptions,
                         {:preferred_algorithms, {errInf, unsup}}, 'Unsupported value(s) found'})
      unsup ->
        a -- unsup
    end
  end
  ...
  defp error_if_empty([{k,
              [{:client2server, []}, {:server2client, []}]}]) do
    :erlang.error({:eoptions, k, 'Empty resulting algorithm list'})
  end
