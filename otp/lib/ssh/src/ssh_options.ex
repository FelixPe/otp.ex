defmodule :m_ssh_options do
  ...
  defp save({key, value}, defs, optMap)
    ...
    try do
      ...
      false ->
        :erlang.error({:eoptions, {key, value}, ~c"Bad value"})

      :forbidden ->
        :erlang.error(
          {:eoptions, {key, value},
           :io_lib.format(
             ~c"The option '~s' is used internally. The user is not allowed to specify this option.",
             [key]
           )}
        )
    end
  ...
  def default(:common) do
    %{
      ...
      auth_methods: %{
        default: ~c"publickey,keyboard-interactive,password",
        chk: fn as ->
          try do
            sup = :string.tokens(~c"publickey,keyboard-interactive,password", ~c",")
            new = :string.tokens(as, ~c",")
  ...
  defp check_pref_public_key_algs(v) do
    ...
    cHK = fn a, ack ->
      case :lists.member(a, pKs) do
        ...
        false ->
          error_in_check(a, ~c"Not supported public key")
      end
    end
    ...
    case :file.open(file, [:read]) do
      {:ok, d} ->
        try do
          ...
          _, _ ->
            error_in_check({:ssh_moduli_file, file}, ~c"Bad format in file " ++ file)
        else
          {:ok, moduli} ->
  ...
  defp read_moduli_file(d, i, acc) do
    case :io.get_line(d, ~c"") do
      ...
      data ->
        ...
        try do
          [_Time, _Class, _Tests, _Tries, size, g, p] = :string.tokens(line, ~c" \r\n")
  ...
  defp valid_hash(x, _) do
    error_in_check(x, ~c"Expect atom or list in fingerprint spec")
  end
  ...
  defp check_modify_algorithms(m) when is_list(m) do
    for op_KVs <- m,
        not is_tuple(op_KVs) or tuple_size(op_KVs) !== 2 or
          not :lists.member(
            :erlang.element(
              1,
              op_KVs
      ...
      error_in_check(op_KVs, ~c"Bad modify_algorithms")
    end
  ...
  defp check_modify_algorithms(_) do
    error_in_check(:modify_algorithms, ~c"Bad option value. List expected.")
  end
  ...
  defp normalize_mod_algs([], [{k, _} | _], _, _) do
    case :ssh_transport.algo_class(k) do
      true ->
        error_in_check(k, ~c"Duplicate key")

      false ->
        error_in_check(k, ~c"Unknown key")
    end
  end

  defp normalize_mod_algs([], [x | _], _, _) do
    error_in_check(x, ~c"Bad list element")
  end
  ...
  defp nml(k, l) do
    for v <- l, not is_atom(v) do
      error_in_check(k, ~c"Bad value for this key")
    end
    ...
    case l -- :lists.usort(l) do
      ...
      dups ->
        error_in_check({k, dups}, ~c"Duplicates")
    end
  ...
  defp check_input_ok(algs) do
    for kVs <- algs,
      ...
      error_in_check(kVs, ~c"Bad preferred_algorithms")
    end
  end

  defp final_preferred_algorithms(options0) do
    result =
      case :ssh_options.get_value(:user_options, :modify_algorithms, options0, :ssh_options, 1198) do
        :undefined ->
          rm_non_supported(
            true,
  ...
  defp rm_unsup(a, b, flg, errInf) do
    case a -- b do
      unsup = [_ | _] when flg == true ->
        :erlang.error(
          {:eoptions, {:preferred_algorithms, {errInf, unsup}}, ~c"Unsupported value(s) found"}
        )
  ...
  defp error_if_empty([{k, []} | _]) do
    :erlang.error({:eoptions, k, ~c"Empty resulting algorithm list"})
  end
  ...
  defp error_if_empty([{k, [{:client2server, []} | _]} | _]) do
    :erlang.error({:eoptions, {k, :client2server}, ~c"Empty resulting algorithm list"})
  end

  defp error_if_empty([{k, [_, {:server2client, []} | _]} | _]) do
    :erlang.error({:eoptions, {k, :server2client}, ~c"Empty resulting algorithm list"})
  end
