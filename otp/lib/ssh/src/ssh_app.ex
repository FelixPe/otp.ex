defmodule :m_ssh_app do
  ...
  def start(_Type, _State) do
    :supervisor.start_link({:local, :ssh_sup}, :ssh_app, [:ssh_sup])
  end
  ...
  def init([:ssh_sup]) do
    ...
    childSpecs =
      for supName <- [:sshd_sup, :sshc_sup] do
        %{
          id: supName,
          start: {:supervisor, :start_link, [{:local, supName}, :ssh_app, [:sshX_sup]]},
          type: :supervisor
        }
      end
