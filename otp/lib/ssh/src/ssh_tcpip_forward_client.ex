defmodule :m_ssh_tcpip_forward_client do
  ...
  Record.defrecord(:r_state, :state, id: :undefined,
                                 cm: :undefined, fwd_socket: :undefined)
  ...
  def handle_msg({:tcp, sock, data},
           r_state(fwd_socket: sock, cm: cM, id: chId) = state) do
    :ssh_connection.send(cM, chId, data)
    :inet.setopts(sock, [{:active, :once}])
    {:ok, state}
  end

  def handle_msg({:tcp_closed, sock},
           r_state(fwd_socket: sock, cm: cM, id: chId) = state) do
    :ssh_connection.send_eof(cM, chId)
    {:stop, chId, r_state(state, fwd_socket: :undefined)}
  end

  def handle_ssh_msg({:ssh_cm, _CM, {:data, _ChannelId, _Type, data}},
           r_state(fwd_socket: sock) = state) do
  ...
  def handle_ssh_msg({:ssh_cm, _CM,
            {:exit_signal, chId, _, _Error, _}},
           state) do
    {:stop, chId, state}
  end

  def handle_ssh_msg({:ssh_cm, _, {:exit_status, chId, _Status}},
           state) do
    {:stop, chId, state}
  end
