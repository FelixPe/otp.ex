defmodule :m_ssh_acceptor_sup do
  ...
  def start_link(systemSup, address, options) do
    case (:supervisor.start_link(:ssh_acceptor_sup,
                                   [systemSup, address, options])) do
      {:error,
  ...
  def restart_child(accSup, address) do
    :supervisor.restart_child(accSup,
                                {:ssh_acceptor_sup, address})
  end

  def init([systemSup, address, options]) do
    supFlags = %{strategy: :one_for_one, intensity: 10,
                   period: 3600}
    childSpecs = [%{id: {:ssh_acceptor_sup, address},
                      start:
