defmodule :m_ssh_cli do
  ...
  def handle_ssh_msg(
    ...
    write_chars(connectionHandler, channelId, 1, ~c"Prohibited.")
    :ssh_connection.reply_request(connectionHandler, wantReply, :success, channelId)
  ...
  def handle_ssh_msg(
    ...
    report = :io_lib.format(~c"Connection closed by peer ~n Error ~p~n", [error])
  ...
  def handle_ssh_msg(
    ...
    report = :io_lib.format(~c"Connection closed by peer ~n Status ~p~n", [status])
    :error_logger.error_report(report)
    {:stop, channelId, state}
  end
  ...
  defp claim_encoding(envValue) do
    try do
      :string.tokens(:erlang.binary_to_list(envValue), ~c".")
    catch
      _, _ ->
        :undefined
    else
  ...
  defp io_request({:put_expand_no_trim, :unicode, expand}, buf, tty, _Group) do
    insert_chars(
  ...
  defp io_request(:delete_line, {_, {_, _}, _, col}, tty, _Group) do
    moveToBeg = move_cursor(col, 0, tty)
    {[moveToBeg, ~c"\e[J"], {[], {[], []}, [], 0}}
  end

  defp io_request({:redraw_prompt, pbs, pbs2, {lB, {bef, aft}, lA}}, buf, tty, _Group) do
    ...
    {t, insertedText} =
      io_request({:insert_chars, :unicode.characters_to_binary(text)}, cleared, tty, _Group)
  ...
  defp ansi_tty(n, l) do
    [~c"\e[", :erlang.integer_to_list(n), l]
  end
  ...
  defp conv_buf([], {lB, {bef, aft}, lA, col}, accWrite, _Tty) do
    {{lB, {bef, aft}, lA, col}, :lists.reverse(accWrite)}
  end

  defp conv_buf([13, 10 | rest], {lB, {bef, aft}, lA, col}, accWrite, tty = r_ssh_pty(width: w)) do
    conv_buf(
      rest,
      {[:lists.reverse(bef) | lB], {[], tl2(aft)}, lA, col + (w - rem(col, w))},
      [10, 13 | accWrite],
      tty
    )
  end

  defp conv_buf([13 | rest], {lB, {bef, aft}, lA, col}, accWrite, tty = r_ssh_pty(width: w)) do
    conv_buf(
      rest,
      {[:lists.reverse(bef) | lB], {[], tl1(aft)}, lA, col + (w - rem(col, w))},
      [13 | accWrite],
      tty
    )
  end

  defp conv_buf([10 | rest], {lB, {bef, aft}, lA, col}, accWrite0, tty = r_ssh_pty(width: w)) do
    accWrite =
      case pty_opt(:onlcr, tty) do
        0 ->
          [10 | accWrite0]
  ...
  defp conv_buf([c | rest], {lB, {bef, aft}, lA, col}, accWrite, tty) do
  ...
  defp insert_chars(chars, {_LB, {_Bef, aft}, lA, _Col} = buf, tty) do
  ...
  defp delete_chars(n, {lB, {bef, aft}, lA, col}, tty) when n > 0 do
    ...
    {[newAft, :lists.duplicate(n, ?\s) | m], {lB, {bef, newAft}, lA, col}}
  end
  ...
  defp delete_chars(n, {lB, {bef, aft}, lA, col}, tty) do
    ...
    {[m1, aft, :lists.duplicate(-n, ?\s) | m2], {lB, {newBef, aft}, lA, newCol}}
  end
  ...
  defp move_rel(n, {_LB, {_Bef, _Aft}, _LA, col} = buf, tty) do
    {newLB, {newBef, newAft}, newLA, newCol} =
      step_over(
        n,
        buf
      )
  ...
  defp move_line(v, {_LB, {_Bef, _Aft}, _LA, col}, tty = r_ssh_pty(width: w))
  ...
  defp start_shell(connectionHandler, state) do
    ...
    r_state(state,
      group:
        :group.start(self(), shellSpawner, [
          {:dumb, get_dumb(r_state(state, :pty))},
          {:expand_below, false},
          {:echo, get_echo(r_state(state, :pty))}
        ]),
      buf: empty_buf()
    )
  end
  ...
  defp start_exec_shell(connectionHandler, cmd, state) do
    ...
    r_state(state,
      group: :group.start(self(), execShellSpawner, [{:expand_below, false}, {:echo, false}]),
      buf: empty_buf()
    )
  end
  ...
  defp exec_direct(connectionHandler, channelId, cmd, execSpec, wantReply, state) do
    fun = fn ->
      cond do
        ...
        true ->
          {:error, ~c"Bad exec fun in server"}
      end
    end
  ...
  defp exec_in_self_group(connectionHandler, channelId, wantReply, state, fun) do
    exec = fn ->
      spawn(fn ->
        case (try do
                ...
                x ->
                  {:error, ~c"Bad exec fun in server. Invalid return value: " ++ t2str(x)}
              end) do
          {:ok, str} ->
            write_chars(connectionHandler, channelId, t2str(str))

          {:error, str} ->
            write_chars(connectionHandler, channelId, 1, ~c"**Error** " ++ t2str(str))
    ...
    {:ok,
     r_state(state,
       group: :group.start(self(), exec, [{:expand_below, false}, {:echo, false}]),
       buf: empty_buf()
     )}
  end
  ...
  def ssh_dbg_format(
    ...
    [
      ...
      case request do
        ...
        _ ->
          :io_lib.format(~c"~nunder construction:~nRequest = ~p", [request])
      end
    ]
  end
  ...
  def ssh_dbg_format(
    ...
    [
      :io_lib.format(~c"CLI conn ~p chan ~p reply", [c, ch]),
      fmt_kv([
        {:channel_id, ch},
        {:type, type(type)},
  ...
  def ssh_dbg_format(
    ...
    [
      :io_lib.format(~c"CLI detail conn ~p chan ~p group ~p", [:"?", ch, group]),
      case arg do
        ...
        _ ->
          :io_lib.format(~c"~nunder construction:~nRequest = ~p", [arg])
      end
    ]
  end
  ...
  defp fmt_kv1({k, v}) do
    :io_lib.format(~c"~n~p: ~p", [k, v])
  end

  defp fmt_kv1({k, :s, v}) do
    :io_lib.format(~c"~n~p: ~s", [k, v])
  end

  defp fmt_kv1({k, :us, v}) do
    :io_lib.format(~c"~n~p: ~ts", [k, v])
  end

  defp fmt_kv1({k, :h, v}) do
    :io_lib.format(~c"~n~p: ~s", [k, [?\n | :ssh_dbg.hex_dump(v)]])
  end
