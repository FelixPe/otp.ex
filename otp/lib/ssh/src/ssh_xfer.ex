defmodule :m_ssh_xfer do
  ...
  def protocol_version_request(xF, version) do
    xf_request(xF, 1,
                 <<version :: size(32) - unsigned - big - integer>>)
  end
  ...
  def open(xF, reqID, fileName, access, flags, attrs) do
    ...
    xf_request(xF, 3,
                 [<<reqID :: size(32) - unsigned - big - integer>>,
                      <<byte_size(:unicode.characters_to_binary(fileName))
                        ::
                        size(32) - unsigned - big - integer,
                          :unicode.characters_to_binary(fileName) :: binary>>,
                          mBits, <<f :: size(32) - unsigned - big - integer>>,
  ...
  def opendir(xF, reqID, dirName) do
    xf_request(xF, 11,
                 [<<reqID :: size(32) - unsigned - big - integer>>,
                      <<byte_size(:unicode.characters_to_binary(dirName))
                        ::
                        size(32) - unsigned - big - integer,
  ...
  def remove(xF, reqID, file) do
    xf_request(xF, 13,
                 [<<reqID :: size(32) - unsigned - big - integer>>,
                      <<byte_size(:unicode.characters_to_binary(file))
                        ::
                        size(32) - unsigned - big - integer,
    ...
    case (:lists.member({extRename, '1'}, ext)) do
      true ->
        extended(xF, reqID, extRename,
                   [<<byte_size(:unicode.characters_to_binary(oldPath))
                      ::
                      size(32) - unsigned - big - integer,
                        :unicode.characters_to_binary(oldPath) :: binary>>,
                        <<byte_size(:unicode.characters_to_binary(newPath))
                          ::
                          size(32) - unsigned - big - integer,
        ...
        xf_request(xF, 18,
                     [<<reqID :: size(32) - unsigned - big - integer>>,
                          <<byte_size(:unicode.characters_to_binary(oldPath))
                            ::
                            size(32) - unsigned - big - integer,
                              :unicode.characters_to_binary(oldPath)
                              ::
                              binary>>,
                              <<byte_size(:unicode.characters_to_binary(newPath))
                                ::
                                size(32) - unsigned - big - integer,
                                  :unicode.characters_to_binary(newPath)
                                  ::
                                  binary>>,
                                  flagBits])
    end
  ...
  def rmdir(xF, reqID, dir) do
    xf_request(xF, 15,
                 [<<reqID :: size(32) - unsigned - big - integer>>,
                      <<byte_size(:unicode.characters_to_binary(dir))
                        ::
                        size(32) - unsigned - big - integer,
  ...
  def extended(xF, reqID, request, data) do
    xf_request(xF, 200,
                 [<<reqID :: size(32) - unsigned - big - integer>>,
                      <<byte_size(:unicode.characters_to_binary(request))
                        ::
                        size(32) - unsigned - big - integer,
  ...
  defp xf_request(xF, op, arg) do
    ...
    :ssh_connection.send(cM, channel,
                           [<<size :: size(32) - unsigned - big - integer, op,
                                data :: binary>>])
  end
  ...
  def xf_send_handle(r_ssh_xfer(cm: cM, channel: channel), reqId, handle) do
    hLen = length(handle)
    size = 1 + 4 + 4 + hLen
    toSend = [<<size :: size(32) - unsigned - big - integer,
                  102, reqId :: size(32) - unsigned - big - integer,
                  hLen :: size(32) - unsigned - big - integer>>,
                  handle]
    :ssh_connection.send(cM, channel, toSend)
  end

  def xf_send_names(r_ssh_xfer(cm: cM, channel: channel, vsn: vsn), reqId,
           namesAndAttrs) do
    count = length(namesAndAttrs)
    {data, len} = encode_names(vsn, namesAndAttrs)
    size = 1 + 4 + 4 + len
    toSend = [<<size :: size(32) - unsigned - big - integer,
                  104, reqId :: size(32) - unsigned - big - integer,
  ...
  def xf_send_status(r_ssh_xfer(cm: cM, channel: channel), reqId, errorCode,
           errorMsg, data) do
    langTag = 'en'
    ...
    toSend = [<<size :: size(32) - unsigned - big - integer,
                  101, reqId :: size(32) - unsigned - big - integer,
  ...
  def xf_send_attr(r_ssh_xfer(cm: cM, channel: channel, vsn: vsn), reqId,
           attr) do
    encAttr = encode_ATTR(vsn, attr)
    aLen = byte_size(encAttr)
    size = 1 + 4 + aLen
    toSend = [<<size :: size(32) - unsigned - big - integer,
                  105, reqId :: size(32) - unsigned - big - integer>>,
                  encAttr]
    :ssh_connection.send(cM, channel, toSend)
  end
  ...
  def xf_send_data(r_ssh_xfer(cm: cM, channel: channel), reqId, data) do
    ...
    size = 1 + 4 + 4 + dLen
    toSend = [<<size :: size(32) - unsigned - big - integer,
                  103, reqId :: size(32) - unsigned - big - integer,
  ...
  def xf_reply(_XF,
           <<101, reqID :: size(32) - unsigned - big - integer,
               ...
               eLen :: size(32) - unsigned - big - integer,
               err :: size(eLen) - binary,
               lLen :: size(32) - unsigned - big - integer,
               lang :: size(lLen) - binary, reply :: binary>>) do
    stat = decode_status(status)
    {:status, reqID,
       {stat, :erlang.binary_to_list(err),
          :erlang.binary_to_list(lang), reply}}
  end

  def xf_reply(_XF,
           <<101, reqID :: size(32) - unsigned - big - integer,
               status :: size(32) - unsigned - big - integer>>) do
    stat = decode_status(status)
    {:status, reqID, {stat, '', '', <<>>}}
  end
  ...
  def xf_reply(_XF,
           <<201, reqID :: size(32) - unsigned - big - integer,
               rData>>) do
  ...
  def decode_ext(<<nameLen :: size(32) - unsigned - big - integer,
             name :: size(nameLen) - binary,
             dataLen :: size(32) - unsigned - big - integer,
             data :: size(dataLen) - binary, tail :: binary>>) do
  ...
  def decode_ace_mask(f) do
    decode_bits(f,
                  [{1, :read_data}, {1, :list_directory}, {2,
                                                             :write_data},
                                                              {2, :add_file},
                                                                  {4,
                                                                     :append_data},
  ...
  def decode_open_flags(vsn, f) when vsn <= 3 do
    decode_bits(f,
                  [{1, :read}, {2, :write}, {4, :append}, {8, :creat},
                                                              {16, :trunc}, {32,
                                                                               :excl}])
  end
  ...
  def decode_open_flags(vsn, f) when vsn >= 4 do
    r = decode_bits(f,
                      [{8, :append_data}, {16, :append_data_atomic}, {32,
                                                                        :text_mode},
                                                                         {64,
                                                                            :read_lock},
                                                                             {128,
  ...
  defp decode_ace_flag(f) do
    decode_bits(f,
                  [{1, :file_inherit}, {2, :directory_inherit}, {4,
                                                                   :no_propagte_inherit},
                                                                    {8,
                                                                       :inherit_only},
                                                                        {16,
  ...
  defp encode_attr_flags(vsn, :all) do
    encode_attr_flags(vsn,
                        [:size, :uidgid, :permissions, :acmodtime, :accesstime,
                                                                       :createtime,
                                                                           :modifytime,
                                                                               :acl,
                                                                                   :ownergroup,
  ...
  defp decode_attrib_bits(f) do
    decode_bits(f,
                  [{1, :readonly}, {2, :system}, {4, :hidden}, {8,
                                                                  :case_insensitive},
                                                                   {16,
                                                                      :arcive},
                                                                       {32,
  ...
  def encode_ATTR(vsn, a) do
    {flags, as} = encode_As(vsn,
                              [{:size, r_ssh_xfer_attr(a, :size)}, {:ownergroup,
                                                        r_ssh_xfer_attr(a, :owner)},
                                                         {:ownergroup,
                                                            r_ssh_xfer_attr(a, :group)},
                                                             {:permissions,
                                                                r_ssh_xfer_attr(a, :permissions)},
                                                                 {:acmodtime,
                                                                    r_ssh_xfer_attr(a, :atime)},
                                                                     {:acmodtime,
                                                                        r_ssh_xfer_attr(a, :mtime)},
                                                                         {:accesstime,
                                                                            r_ssh_xfer_attr(a, :atime)},
                                                                             {:subsecond_times,
                                                                                r_ssh_xfer_attr(a, :atime_nseconds)},
                                                                                 {:createtime,
                                                                                    r_ssh_xfer_attr(a, :createtime)},
                                                                                     {:subsecond_times,
                                                                                        r_ssh_xfer_attr(a, :createtime_nseconds)},
                                                                                         {:modifytime,
                                                                                            r_ssh_xfer_attr(a, :mtime)},
                                                                                             {:subsecond_times,
                                                                                                r_ssh_xfer_attr(a, :mtime_nseconds)},
                                                                                                 {:acl,
                                                                                                    r_ssh_xfer_attr(a, :acl)},
                                                                                                     {:bits,
                                                                                                        r_ssh_xfer_attr(a, :attrib_bits)},
                                                                                                         {:extended,
                                                                                                            r_ssh_xfer_attr(a, :extensions)}],
                              0, [])
    type = encode_file_type(r_ssh_xfer_attr(a, :type))
    result = :erlang.list_to_binary([<<flags
                                       ::
                                       size(32) - unsigned - big - integer>>,
                                         cond do
                                           vsn >= 5 ->
  ...
  defp encode_As(vsn, [{aName, x} | as], flags, acc) do
    case (aName) do
      :size ->
        encode_As(vsn, as, flags ||| 1,
                    [<<x :: size(64) - unsigned - big - integer>> | acc])
      :ownergroup when vsn <= 4 ->
        encode_As(vsn, as, flags ||| 2,
                    [<<x :: size(32) - unsigned - big - integer>> | acc])
      :ownergroup when vsn >= 5 ->
        x1 = :erlang.integer_to_binary(x)
        encode_As(vsn, as, flags ||| 128,
                    [<<byte_size(x1) :: size(32) - unsigned - big - integer,
                         x1 :: binary>> |
                         acc])
      :permissions ->
        encode_As(vsn, as, flags ||| 4,
                    [<<x :: size(32) - unsigned - big - integer>> | acc])
      :acmodtime when vsn <= 3 ->
        encode_As(vsn, as, flags ||| 8,
                    [<<x :: size(32) - unsigned - big - integer>> | acc])
      :accesstime when vsn >= 5 ->
        encode_As(vsn, as, flags ||| 8,
                    [<<x :: size(64) - unsigned - big - integer>> | acc])
      :createtime when vsn >= 5 ->
        encode_As(vsn, as, flags ||| 16,
                    [<<x :: size(64) - unsigned - big - integer>> | acc])
      :modifytime when vsn >= 5 ->
        encode_As(vsn, as, flags ||| 32,
                    [<<x :: size(64) - unsigned - big - integer>> | acc])
      :subsecond_times when vsn >= 5 ->
        encode_As(vsn, as, flags ||| 256,
                    [<<x :: size(64) - unsigned - big - integer>> | acc])
      :acl when vsn >= 5 ->
        encode_As(vsn, as, flags ||| 64, [encode_acl(x) | acc])
      :bits when vsn >= 5 ->
        f = encode_attrib_bits(x)
        encode_As(vsn, as, flags ||| 512,
                    [<<f :: size(32) - unsigned - big - integer>> | acc])
      :extended ->
        encode_As(vsn, as, flags ||| 2147483648,
  ...
  def decode_ATTR(vsn,
    ...
    {type, tail2} = (cond do
                       vsn <= 3 ->
                         {5, tail}
                       true ->
                         <<t :: size(8) - unsigned - big - integer,
                             tL :: binary>> = tail
                         {t, tL}
                     end)
    decode_As(vsn,
                [{:size, r_ssh_xfer_attr(:size)}, {:ownergroup, r_ssh_xfer_attr(:owner)},
                                        {:ownergroup, r_ssh_xfer_attr(:group)}, {:permissions,
                                                                     r_ssh_xfer_attr(:permissions)},
                                                                      {:acmodtime,
                                                                         r_ssh_xfer_attr(:atime)},
                                                                          {:acmodtime,
                                                                             r_ssh_xfer_attr(:mtime)},
                                                                              {:accesstime,
                                                                                 r_ssh_xfer_attr(:atime)},
                                                                                  {:subsecond_times,
                                                                                     r_ssh_xfer_attr(:atime_nseconds)},
                                                                                      {:createtime,
                                                                                         r_ssh_xfer_attr(:createtime)},
                                                                                          {:subsecond_times,
                                                                                             r_ssh_xfer_attr(:createtime_nseconds)},
                                                                                              {:modifytime,
                                                                                                 r_ssh_xfer_attr(:mtime)},
                                                                                                  {:subsecond_times,
                                                                                                     r_ssh_xfer_attr(:mtime_nseconds)},
                                                                                                      {:acl,
                                                                                                         r_ssh_xfer_attr(:acl)},
                                                                                                          {:bits,
                                                                                                             r_ssh_xfer_attr(:attrib_bits)},
                                                                                                              {:extended,
                                                                                                                 r_ssh_xfer_attr(:extensions)}],
                r_ssh_xfer_attr(type: decode_file_type(type)), flags, tail2)
  end

  defp decode_As(vsn, [{aName, aField} | as], r, flags, tail) do
    case (aName) do
      :size when 1 &&& flags == 1 ->
        <<x :: size(64) - unsigned - big - integer,
            tail2 :: binary>> = tail
        decode_As(vsn, as, :erlang.setelement(aField, r, x),
                    flags, tail2)
      ...
      :ownergroup when (128 &&& flags == 128 and vsn >= 5) ->
        <<len :: size(32) - unsigned - big - integer,
            bin :: size(len) - binary, tail2 :: binary>> = tail
        x = :erlang.binary_to_integer(bin)
        decode_As(vsn, as, :erlang.setelement(aField, r, x),
                    flags, tail2)
      ...
      :createtime when (16 &&& flags == 16 and vsn >= 5) ->
        ...
        decode_As(vsn, as, :erlang.setelement(aField, r, x),
                    flags, tail2)
      :subsecond_times when (256 &&& flags == 256 and
                               vsn >= 5)
                            ->
        <<x :: size(32) - unsigned - big - integer,
            tail2 :: binary>> = tail
      ...
      :bits when (512 &&& flags == 512 and vsn >= 5) ->
        <<y :: size(32) - unsigned - big - integer,
            tail2 :: binary>> = tail
        x = decode_attrib_bits(y)
        decode_As(vsn, as, :erlang.setelement(aField, r, x),
                    flags, tail2)
      :extended when 2147483648 &&& flags == 2147483648 ->
  ...
  defp decode_names(vsn, i,
            <<len :: size(32) - unsigned - big - integer,
                fileName :: size(len) - binary,
                lLen :: size(32) - unsigned - big - integer,
                _LongName :: size(lLen) - binary, tail :: binary>>)
      when vsn <= 3 do
  ...
  defp decode_names(vsn, i,
            <<len :: size(32) - unsigned - big - integer,
                fileName :: size(len) - binary, tail :: binary>>)
      when vsn >= 4 do
    name = :unicode.characters_to_list(fileName)
    {a, tail2} = decode_ATTR(vsn, tail)
    [{name, a} | decode_names(vsn, i - 1, tail2)]
  ...
  defp encode_names(vsn, namesAndAttrs) do
    :lists.mapfoldl(fn n, l ->
                         encode_name(vsn, n, l)
                    end,
                      0, namesAndAttrs)
  end

  defp encode_name(vsn, {{nameUC, longNameUC}, attr}, len)
      when vsn <= 3 do
    name = :erlang.binary_to_list(:unicode.characters_to_binary(nameUC))
    nLen = length(name)
    longName = :erlang.binary_to_list(:unicode.characters_to_binary(longNameUC))
    lNLen = length(longName)
    encAttr = encode_ATTR(vsn, attr)
    aLen = byte_size(encAttr)
    newLen = len + nLen + lNLen + 4 + 4 + aLen
    {[<<nLen :: size(32) - unsigned - big - integer>>, name,
                                                           <<lNLen
                                                             ::
                                                             size(32) -
                                                               unsigned - big -
  ...
  defp encode_acl_items([aCE | as]) do
    ...
    [<<type :: size(32) - unsigned - big - integer>>, <<flag
                                                        ::
                                                        size(32) - unsigned -
                                                          big - integer>>,
                                                          <<mask
                                                            ...
                                                            size(32) -
                                                              unsigned - big -
                                                              integer>>,
                                                              <<byte_size(:unicode.characters_to_binary(who))
                                                                ::
                                                                size(32) -
                                                                  unsigned -
                                                                  big - integer,
                                                                  :unicode.characters_to_binary(who)
                                                                  ::
                                                                  binary>> |
                                                                  encode_acl_items(as)]
  end
  ...
  defp decode_acl(<<count :: size(32) - unsigned - big - integer,
              tail :: binary>>) do
  ...
  defp encode_ext([{type, data} | exts]) do
    [<<byte_size(:unicode.characters_to_binary(type))
       ::
       size(32) - unsigned - big - integer,
  ...
  defp decode_extended(<<count :: size(32) - unsigned - big - integer,
              tail :: binary>>) do
  ...
  defp decode_ext(i,
            <<tLen :: size(32) - unsigned - big - integer,
                type :: size(tLen) - binary,
                dLen :: size(32) - unsigned - big - integer,
                data :: size(dLen) - binary, tail :: binary>>,
            acc) do
    decode_ext(i - 1, tail,
                 [{:erlang.binary_to_list(type), data} | acc])
  end
