defmodule :m_ssh_xfer do
  ...
  def open(xF, reqID, fileName, access, flags, attrs) do
    ...
    xf_request(xF, 3, [
      ...
      <<byte_size(:unicode.characters_to_binary(fileName))::size(32)-unsigned-big-integer,
  ...
  def opendir(xF, reqID, dirName) do
    xf_request(xF, 11, [
      <<reqID::size(32)-unsigned-big-integer>>,
      <<byte_size(:unicode.characters_to_binary(dirName))::size(32)-unsigned-big-integer,
  ...
  def close(xF, reqID, handle) do
    xf_request(xF, 4, [
      <<reqID::size(32)-unsigned-big-integer>>,
      <<byte_size(handle)::size(32)-unsigned-big-integer, handle::binary>>
    ])
  end
  ...
  def write(xF, reqID, handle, offset, data) do
    ...
    xf_request(xF, 6, [
      ...
      <<byte_size(handle)::size(32)-unsigned-big-integer, handle::binary>>,
      <<offset::size(64)-unsigned-big-integer>>,
      <<byte_size(data1)::size(32)-unsigned-big-integer, data1::binary>>
    ])
  end
  ...
  def remove(xF, reqID, file) do
    xf_request(xF, 13, [
      <<reqID::size(32)-unsigned-big-integer>>,
      <<byte_size(:unicode.characters_to_binary(file))::size(32)-unsigned-big-integer,
  ...
  def rename(xF, reqID, oldPath, newPath, flags) do
    ...
    extRename = ~c"posix-rename@openssh.com"

    case :lists.member({extRename, ~c"1"}, ext) do
      true ->
        extended(xF, reqID, extRename, [
          <<byte_size(:unicode.characters_to_binary(oldPath))::size(32)-unsigned-big-integer,
            :unicode.characters_to_binary(oldPath)::binary>>,
          <<byte_size(:unicode.characters_to_binary(newPath))::size(32)-unsigned-big-integer,
  ...
  def mkdir(xF, reqID, path, attrs) do
    xf_request(xF, 14, [
      <<reqID::size(32)-unsigned-big-integer>>,
      <<byte_size(:unicode.characters_to_binary(path))::size(32)-unsigned-big-integer,
  ...
  def rmdir(xF, reqID, dir) do
    xf_request(xF, 15, [
      <<reqID::size(32)-unsigned-big-integer>>,
      <<byte_size(:unicode.characters_to_binary(dir))::size(32)-unsigned-big-integer,
  ...
  def symlink(xF, reqID, linkPath, targetPath) do
    ...
    xf_request(xF, 20, [
      <<reqID::size(32)-unsigned-big-integer>>,
      <<byte_size(linkPath1)::size(32)-unsigned-big-integer, linkPath1::binary>>,
      <<byte_size(targetPath1)::size(32)-unsigned-big-integer, targetPath1::binary>>
    ])
  end
  ...
  def extended(xF, reqID, request, data) do
    xf_request(xF, 200, [
      <<reqID::size(32)-unsigned-big-integer>>,
      <<byte_size(:unicode.characters_to_binary(request))::size(32)-unsigned-big-integer,
  ...
  def xf_send_status(xF, reqId, errorCode) do
    xf_send_status(xF, reqId, errorCode, ~c"")
  end
    ...
    case aName do
      ...
      :ownergroup when vsn >= 5 ->
        ...
        encode_As(vsn, as, flags ||| 128, [
          <<byte_size(x1)::size(32)-unsigned-big-integer, x1::binary>>
          | acc
        ])
  ...
  defp encode_name(vsn, {{nameUC, longNameUC}, attr}, len)
    ...
    {[
       ...
       name,
       <<lNLen::size(32)-unsigned-big-integer>>,
       longName,
       encAttr
     ], newLen}
  end
  ...
  defp encode_acl_items([aCE | as]) do
    ...
    [
      ...
      <<byte_size(:unicode.characters_to_binary(who))::size(32)-unsigned-big-integer,
  ...
  defp encode_ext([{type, data} | exts]) do
    [
      <<byte_size(:unicode.characters_to_binary(type))::size(32)-unsigned-big-integer,
