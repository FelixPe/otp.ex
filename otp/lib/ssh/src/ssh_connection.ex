defmodule :m_ssh_connection do
  ...
  def session_channel(connectionHandler, timeout) do
    session_channel(connectionHandler, :undefined,
                      :undefined, timeout)
  end

  def session_channel(connectionHandler, initialWindowSize,
           maxPacketSize, timeout) do
  ...
  def open_channel(connectionHandler, type, chanData, timeout) do
    open_channel(connectionHandler, type, chanData,
                   :undefined, :undefined, timeout)
  end

  defp open_channel(connectionHandler, type, chanData,
            initialWindowSize, maxPacketSize, timeout) do
    case (:ssh_connection_handler.open_channel(connectionHandler,
                                                 type, chanData,
                                                 initialWindowSize,
                                                 maxPacketSize, timeout)) do
      {:open, channel} ->
  ...
  def exec(connectionHandler, channelId, command,
           timeOut) do
    :ssh_connection_handler.request(connectionHandler,
                                      self(), channelId, 'exec', true,
                                      [<<byte_size(:unicode.characters_to_binary(command))
                                         ::
                                         size(32) - unsigned - big - integer,
                                           :unicode.characters_to_binary(command)
                                           ::
                                           binary>>],
                                      timeOut)
  end
  ...
  def shell(connectionHandler, channelId) do
    :ssh_connection_handler.request(connectionHandler,
                                      self(), channelId, 'shell', false, <<>>, 0)
  end

  def subsystem(connectionHandler, channelId, subSystem,
           timeOut) do
    :ssh_connection_handler.request(connectionHandler,
                                      self(), channelId, 'subsystem', true,
                                      [<<byte_size(:unicode.characters_to_binary(subSystem))
                                         ::
                                         size(32) - unsigned - big - integer,
                                           :unicode.characters_to_binary(subSystem)
                                           ::
                                           binary>>],
                                      timeOut)
  end
  ...
  def send(connectionHandler, channelId, type, data,
           timeOut) do
    :ssh_connection_handler.send(connectionHandler,
                                   channelId, type, data, timeOut)
  end

  def send_eof(connectionHandler, channel) do
    :ssh_connection_handler.send_eof(connectionHandler,
                                       channel)
  end

  def adjust_window(connectionHandler, channel, bytes) do
    :ssh_connection_handler.adjust_window(connectionHandler,
                                            channel, bytes)
  end

  def setenv(connectionHandler, channelId, var, value,
           timeOut) do
    setenv(connectionHandler, channelId, true, var, value,
             timeOut)
  end

  defp setenv(connectionHandler, channelId, wantReply, var,
            value, timeOut) do
    case (:ssh_connection_handler.request(connectionHandler,
                                            channelId, 'env', wantReply,
                                            [<<byte_size(:unicode.characters_to_binary(var))
                                               ::
                                               size(32) - unsigned - big -
                                                 ...
                                                 ::
                                                 binary>>,
                                                 <<byte_size(:unicode.characters_to_binary(value))
                                                   ::
                                                   size(32) - unsigned - big -
                                                     integer,
  ...
  def close(connectionHandler, channelId) do
    :ssh_connection_handler.close(connectionHandler,
                                    channelId)
  end

  def reply_request(connectionHandler, true, status, channelId) do
    :ssh_connection_handler.reply_request(connectionHandler,
                                            status, channelId)
  end
  ...
  def ptty_alloc(connectionHandler, channel, options0, timeOut) do
    ...
    {width, pixWidth} = pty_default_dimensions(:width,
                                                 termData)
    {height, pixHeight} = pty_default_dimensions(:height,
                                                   termData)
    pty_req(connectionHandler, channel,
              ...
              :proplists.get_value(:pixel_height, termData,
                                     pixHeight),
  ...
  def window_change(connectionHandler, channel, width, height,
           pixWidth, pixHeight) do
    :ssh_connection_handler.request(connectionHandler,
                                      channel, 'window-change', false,
                                      [<<width
                                         ::
                                         size(32) - unsigned - big - integer>>,
                                           <<height
  ...
  def signal(connectionHandler, channel, sig) do
    :ssh_connection_handler.request(connectionHandler,
                                      ...
                                      [<<byte_size(:unicode.characters_to_binary(sig))
                                         ::
                                         size(32) - unsigned - big - integer,
                                           :unicode.characters_to_binary(sig)
                                           ::
                                           binary>>],
                                      0)
  end
  ...
  def channel_data(channelId, dataType, data0,
           r_connection(channel_cache: cache) = connection, from) do
    case (:ssh_client_channel.cache_lookup(cache,
                                             channelId)) do
      r_channel(remote_id: id, sent_close: false) = channel0 ->
        ...
        {sendList,
           channel} = update_send_window(r_channel(channel0, flow_control: from),
                                           dataType, data, connection)
        replies = :lists.map(fn {sendDataType, sendData} ->
                                  {:connection_reply,
                                     channel_data_msg(id, sendDataType,
                                                        sendData)}
                             end,
                               sendList)
      ...
      _ ->
        {[{:channel_request_reply, from, {:error, :closed}}],
           connection}
    end
  end

  def handle_msg(r_ssh_msg_channel_open_confirmation(recipient_channel: channelId,
             sender_channel: remoteId, initial_window_size: windowSz,
             maximum_packet_size: packetSz),
           r_connection(channel_cache: cache) = connection0, _, _SSH) do
    r_channel(remote_id: :undefined) = (channel = :ssh_client_channel.cache_lookup(cache,
                                                                             channelId))
    :ssh_client_channel.cache_update(cache,
                                       r_channel(channel, remote_id: remoteId, 
  ...
  def handle_msg(r_ssh_msg_channel_open_failure(recipient_channel: channelId, reason: reason,
             description: descr, lang: lang),
           r_connection(channel_cache: cache) = connection0, _, _SSH) do
    channel = :ssh_client_channel.cache_lookup(cache,
                                                 channelId)
    :ssh_client_channel.cache_delete(cache, channelId)
    reply_msg(channel, connection0,
                {:open_error, reason, descr, lang})
  end

  def handle_msg(r_ssh_msg_channel_success(recipient_channel: channelId), connection, _,
           _SSH) do
    reply_msg(channelId, connection, :success)
  end

  def handle_msg(r_ssh_msg_channel_failure(recipient_channel: channelId), connection, _,
           _SSH) do
    reply_msg(channelId, connection, :failure)
  end

  def handle_msg(r_ssh_msg_channel_eof(recipient_channel: channelId), connection, _,
           _SSH) do
    reply_msg(channelId, connection, {:eof, channelId})
  end

  def handle_msg(r_ssh_msg_channel_close(recipient_channel: channelId),
           r_connection(channel_cache: cache) = connection0, _, _SSH) do
    case (:ssh_client_channel.cache_lookup(cache,
                                             channelId)) do
      r_channel(sent_close: closed, remote_id: remoteId,
          flow_control: flowControl) = channel ->
        :ssh_client_channel.cache_delete(cache, channelId)
        {closeMsg, connection} = reply_msg(channel, connection0,
                                             {:closed, channelId})
        connReplyMsgs = (case (closed) do
                           true ->
                             []
                           false ->
        ...
        sendReplyMsgs = (case (flowControl) do
                           :undefined ->
                             []
                           from ->
  ...
  def handle_msg(r_ssh_msg_channel_data(recipient_channel: channelId, data: data),
           connection, _, _SSH) do
    channel_data_reply_msg(channelId, connection, 0, data)
  end

  def handle_msg(r_ssh_msg_channel_extended_data(recipient_channel: channelId,
             data_type_code: dataType, data: data),
           connection, _, _SSH) do
    channel_data_reply_msg(channelId, connection, dataType,
                             data)
  end

  def handle_msg(r_ssh_msg_channel_window_adjust(recipient_channel: channelId,
             bytes_to_add: add),
           r_connection(channel_cache: cache) = connection, _, _SSH) do
    r_channel(send_window_size: size,
        remote_id: remoteId) = (channel0 = :ssh_client_channel.cache_lookup(cache,
                                                                              channelId))
    {sendList,
       channel} = update_send_window(r_channel(channel0, send_window_size: size + add),
                                       0, :undefined, connection)
    replies = :lists.map(fn {type, data} ->
                              {:connection_reply,
  ...
  def handle_msg(r_ssh_msg_channel_open(channel_type: 'session' = type,
             sender_channel: remoteId, initial_window_size: windowSz,
             maximum_packet_size: packetSz),
           r_connection(options: sSHopts) = connection0, :server, _SSH) do
    minAcceptedPackSz = :ssh_options.get_value(:user_options,
                                                 :minimal_remote_max_packet_size,
                                                 sSHopts, :ssh_connection, 573)
    cond do
      minAcceptedPackSz <= packetSz ->
        try do
          setup_session(connection0, remoteId, type, windowSz,
                          packetSz)
        catch
          _, _ ->
      ...
      minAcceptedPackSz > packetSz ->
        failMsg = channel_open_failure_msg(remoteId, 1,
                                             :lists.concat(['Maximum packet size below ',
                                                                minAcceptedPackSz,
                                                                    ' not supported']),
                                             'en')
        {[{:connection_reply, failMsg}], connection0}
  ...
  def handle_msg(r_ssh_msg_channel_open(channel_type: 'forwarded-tcpip', sender_channel: remoteId,
             initial_window_size: windowSize,
             maximum_packet_size: packetSize,
             data: <<_L1 :: size(32) - unsigned - big - integer,
                       connectedHost :: size(_L1) - binary,
                       connectedPort :: size(32) - unsigned - big - integer,
                       _L2 :: size(32) - unsigned - big - integer,
                       _OriginHost :: size(_L2) - binary,
                       _OriginPort :: size(32) - unsigned - big - integer>>),
           r_connection(channel_cache: cache, channel_id_seed: chId,
               suggest_window_size: winSz, suggest_packet_size: pktSz,
               options: options, sub_system_supervisor: subSysSup) = c,
           :client, _SSH) do
    {replyMsg,
       nextChId} = (case (:ssh_connection_handler.retrieve(c,
                                                             {:tcpip_forward,
                                                                connectedHost,
                                                                connectedPort})) do
                      {:ok, {connectToHost, connectToPort}} ->
                        case (:gen_tcp.connect(connectToHost, connectToPort,
                                                 [{:active, false},
                                                      :binary])) do
                          {:ok, sock} ->
                            {:ok,
                               pid} = :ssh_subsystem_sup.start_channel(:client,
                                                                         subSysSup,
                                                                         self(),
                                                                         ...
                                                                         options)
                            :ssh_client_channel.cache_update(cache,
                                                               r_channel(type: 'forwarded-tcpip',
                                                                   sys: 'none',
                                                                   local_id: chId,
                                                                   remote_id: remoteId,
                                                                   user: pid,
                            ...
                            {channel_open_confirmation_msg(remoteId, chId,
                               ...
                               chId + 1}
                          {:error, error} ->
                            {channel_open_failure_msg(remoteId, 2,
                                                        :io_lib.format('Forwarded connection refused: ~p',
                                                                         [error]),
                                                        'en'),
                               chId}
                        end
                      :undefined ->
                        {channel_open_failure_msg(remoteId, 2,
                                                    :io_lib.format('No forwarding ordered', []), 'en'),
                           chId}
                    end)
    {[{:connection_reply, replyMsg}],
       r_connection(c, channel_id_seed: nextChId)}
  end

  def handle_msg(r_ssh_msg_channel_open(channel_type: 'direct-tcpip', sender_channel: remoteId,
             initial_window_size: windowSize,
             maximum_packet_size: packetSize,
             data: <<_L1 :: size(32) - unsigned - big - integer,
                       hostToConnect :: size(_L1) - binary,
                       portToConnect :: size(32) - unsigned - big - integer,
                       _L2 :: size(32) - unsigned - big - integer,
           ...
           :server, _SSH) do
    {replyMsg,
       nextChId} = (case (:ssh_options.get_value(:user_options,
                                                   :tcpip_tunnel_in, options,
                                                   :ssh_connection, 671)) do
                      false ->
                        {channel_open_failure_msg(remoteId, 2, 'Forwarding disabled', 'en'), chId}
                      true ->
                        case (:gen_tcp.connect(:erlang.binary_to_list(hostToConnect),
                                                 portToConnect,
                                                 [{:active, false},
                                                      :binary])) do
                          {:ok, sock} ->
                            {:ok,
                               pid} = :ssh_subsystem_sup.start_channel(:server,
                                                                         subSysSup,
                                                                         self(),
  ...
  def handle_msg(r_ssh_msg_channel_open(channel_type: 'session', sender_channel: remoteId),
           connection, :client, _SSH) do
    failMsg = channel_open_failure_msg(remoteId, 2, 'Connection refused', 'en')
    {[{:connection_reply, failMsg}], connection}
  end

  def handle_msg(r_ssh_msg_channel_open(sender_channel: remoteId), connection, _,
           _SSH) do
    failMsg = channel_open_failure_msg(remoteId, 1, 'Not allowed', 'en')
    {[{:connection_reply, failMsg}], connection}
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'exit-status',
             data: data),
           connection, _, _SSH) do
    <<status :: size(32) - unsigned - big - integer>> = data
    reply_msg(channelId, connection,
                {:exit_status, channelId, status})
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'exit-signal',
             want_reply: false, data: data),
           r_connection(channel_cache: cache) = connection0, _, _SSH) do
    <<_SigLen :: size(32) - unsigned - big - integer,
        sigName :: size(_SigLen) - binary,
        _Core :: size(8) - unsigned - big - integer,
        _ErrLen :: size(32) - unsigned - big - integer,
        err :: size(_ErrLen) - binary,
        _LangLen :: size(32) - unsigned - big - integer,
    ...
    {reply, connection} = reply_msg(channel, connection0,
                                      {:exit_signal, channelId,
                                         :erlang.binary_to_list(sigName),
                                         :erlang.binary_to_list(err),
                                         :erlang.binary_to_list(lang)})
    closeMsg = channel_close_msg(remoteId)
    {[{:connection_reply, closeMsg} | reply], connection}
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'xon-xoff',
             want_reply: false, data: data),
           connection, _, _SSH) do
    <<cDo :: size(8) - unsigned - big - integer>> = data
    reply_msg(channelId, connection,
                {:xon_xoff, channelId, cDo !== 0})
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'window-change',
             want_reply: false, data: data),
           connection0, _, _SSH) do
    <<width :: size(32) - unsigned - big - integer,
        height :: size(32) - unsigned - big - integer,
        pixWidth :: size(32) - unsigned - big - integer,
        pixHeight
        ::
        size(32) - unsigned - big - integer>> = data
    reply_msg(channelId, connection0,
                {:window_change, channelId, width, height, pixWidth,
                   pixHeight})
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'signal',
             data: data),
           connection0, _, _SSH) do
    <<_SigLen :: size(32) - unsigned - big - integer,
        sigName :: size(_SigLen) - binary>> = data
    reply_msg(channelId, connection0,
                {:signal, channelId, :erlang.binary_to_list(sigName)})
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'subsystem',
             want_reply: wantReply, data: data),
           r_connection(channel_cache: cache) = connection, :server, _SSH) do
    <<_SsLen :: size(32) - unsigned - big - integer,
        ssName :: size(_SsLen) - binary>> = data
    r_channel(remote_id: remoteId) = (channel = :ssh_client_channel.cache_lookup(cache,
                                                                           channelId))
    reply = (case (start_subsystem(ssName, connection,
                                     channel,
                                     {:subsystem, channelId, wantReply,
                                        :erlang.binary_to_list(ssName)})) do
               {:ok, pid} ->
  ...
  def handle_msg(r_ssh_msg_channel_request(request_type: 'subsystem'), connection, :client, _SSH) do
    {[], connection}
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'pty-req',
             want_reply: wantReply, data: data),
           connection, :server, _SSH) do
    <<_TermLen :: size(32) - unsigned - big - integer,
    ...
    ptyOpts = (case (:proplists.get_value(:onlcr, ptyOpts0,
                                            :undefined)) do
                 :undefined ->
                   [{:onlcr, 1} | ptyOpts0]
                 _ ->
    ...
    ptyRequest = {termName, width, height, pixWidth,
                    pixHeight, ptyOpts}
    handle_cli_msg(connection, channelId,
                     {:pty, channelId, wantReply, ptyRequest})
  end

  def handle_msg(r_ssh_msg_channel_request(request_type: 'pty-req'), connection, :client, _SSH) do
    {[], connection}
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'shell',
  ...
  def handle_msg(r_ssh_msg_channel_request(request_type: 'shell'), connection, :client, _SSH) do
    {[], connection}
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'exec',
             want_reply: wantReply, data: data),
           connection, :server, _SSH) do
    <<_Len :: size(32) - unsigned - big - integer,
        command :: size(_Len) - binary>> = data
    handle_cli_msg(connection, channelId,
                     {:exec, channelId, wantReply,
                        :erlang.binary_to_list(command)})
  end

  def handle_msg(r_ssh_msg_channel_request(request_type: 'exec'), connection, :client, _SSH) do
    {[], connection}
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId, request_type: 'env',
             want_reply: wantReply, data: data),
           connection, :server, _SSH) do
    <<_VarLen :: size(32) - unsigned - big - integer,
        var :: size(_VarLen) - binary,
        _ValLen :: size(32) - unsigned - big - integer,
        value :: size(_ValLen) - binary>> = data
    handle_cli_msg(connection, channelId,
                     {:env, channelId, wantReply, var, value})
  end

  def handle_msg(r_ssh_msg_channel_request(request_type: 'env'), connection, :client, _SSH) do
    {[], connection}
  end

  def handle_msg(r_ssh_msg_channel_request(recipient_channel: channelId,
             want_reply: wantReply),
           r_connection(channel_cache: cache) = connection, _, _SSH) do
    case (:ssh_client_channel.cache_lookup(cache,
                                             channelId)) do
      r_channel(remote_id: remoteId) when wantReply == true ->
  ...
  def handle_msg(r_ssh_msg_global_request(name: "tcpip-forward", want_reply: wantReply,
             data: <<_Len :: size(32) - unsigned - big - integer,
                       listenAddrStr :: size(_Len) - binary,
                       listenPort :: size(32) - unsigned - big - integer>>),
           r_connection(options: opts) = connection, :server, _SSH) do
    case (:ssh_options.get_value(:user_options,
                                   :tcpip_tunnel_out, opts, :ssh_connection,
                                   909)) do
      false ->
        {[{:connection_reply, request_failure_msg()}],
           connection}
      true ->
        ...
        case (:ssh_tcpip_forward_acceptor.supervised_start(fwdSup,
                                                             {listenAddrStr,
                                                                listenPort},
                                                             :undefined, 'forwarded-tcpip',
                                                             :ssh_tcpip_forward_srv,
                                                             connPid)) do
          {:ok, ^listenPort} when wantReply == true ->
            {[{:connection_reply, request_success_msg(<<>>)}],
               connection}
          {:ok, lPort} when wantReply == true ->
            {[{:connection_reply,
                 request_success_msg(<<lPort
                                       ::
                                       size(32) - unsigned - big - integer>>)}],
               connection}
  ...
  def handle_msg(r_ssh_msg_global_request(name: _Type, want_reply: wantReply,
             data: _Data),
           connection, _Role, _SSH) do
    cond do
      wantReply == true ->
  ...
  def handle_msg(r_ssh_msg_request_failure(),
           r_connection(requests: [{_, from} | rest]) = connection, _,
           _SSH) do
    {[{:channel_request_reply, from, {:failure, <<>>}}],
       r_connection(connection, requests: rest)}
  end

  def handle_msg(r_ssh_msg_request_failure(),
           r_connection(requests: [{_, from, _} | rest]) = connection, _,
           _SSH) do
    {[{:channel_request_reply, from, {:failure, <<>>}}],
       r_connection(connection, requests: rest)}
  end

  def handle_msg(r_ssh_msg_request_success(data: data),
           r_connection(requests: [{_, from} | rest]) = connection, _,
           _SSH) do
  ...
  def handle_msg(r_ssh_msg_request_success(data: data),
           r_connection(requests: [{_, from, fun} | rest]) = connection0, _,
           _SSH) do
  ...
  def handle_msg(r_ssh_msg_disconnect(code: code, description: description),
           connection, _, _SSH) do
  ...
  def handle_stop(r_connection(channel_cache: cache) = connection0) do
    {connection,
       replies} = :ssh_client_channel.cache_foldl(fn channel,
                                                       {connection1, acc} ->
                                                       {reply,
                                                          connection2} = reply_msg(channel,
                                                                                     connection1,
                                                                                     {:closed,
                                                                                        r_channel(channel, :local_id)})
                                                       {connection2,
                                                          reply ++ acc}
                                                  end,
                                                    {connection0, []}, cache)
  ...
  def channel_data_msg(channelId, type, data) do
    r_ssh_msg_channel_extended_data(recipient_channel: channelId, data_type_code: type,
        data: data)
  end
  ...
  def channel_open_msg(type, channelId, windowSize, maxPacketSize,
           data) do
    r_ssh_msg_channel_open(channel_type: type, sender_channel: channelId,
        initial_window_size: windowSize,
        maximum_packet_size: maxPacketSize, data: data)
  end

  def channel_open_confirmation_msg(remoteId, lID, windowSize, packetSize) do
    r_ssh_msg_channel_open_confirmation(recipient_channel: remoteId, sender_channel: lID,
        initial_window_size: windowSize,
        maximum_packet_size: packetSize)
  end

  def channel_open_failure_msg(remoteId, reason, description, lang) do
    r_ssh_msg_channel_open_failure(recipient_channel: remoteId, reason: reason,
  ...
  def channel_request_msg(channelId, type, wantReply, data) do
    r_ssh_msg_channel_request(recipient_channel: channelId, request_type: type,
        want_reply: wantReply, data: data)
  end
  ...
  defp setup_session(r_connection(channel_cache: cache,
              channel_id_seed: newChannelID,
              suggest_window_size: winSz,
              suggest_packet_size: pktSz) = c,
            remoteId, type, windowSize, packetSize)
      when (is_integer(winSz) and is_integer(pktSz)) do
    nextChannelID = newChannelID + 1
    channel = r_channel(type: type, sys: 'ssh', local_id: newChannelID,
                  recv_window_size: winSz, recv_packet_size: pktSz,
                  send_window_size: windowSize,
                  send_packet_size: packetSize, send_buf: :queue.new(),
                  remote_id: remoteId)
    :ssh_client_channel.cache_update(cache, channel)
    openConfMsg = channel_open_confirmation_msg(remoteId,
                                                  newChannelID, winSz, pktSz)
    reply = {:connection_reply, openConfMsg}
    {[reply], r_connection(c, channel_id_seed: nextChannelID)}
  end

  defp start_cli(r_connection(options: options, cli_spec: cliSpec,
              exec: exec, sub_system_supervisor: subSysSup),
            channelId) do
    case (cliSpec) do
      :no_cli ->
        {:error, :cli_disabled}
      {cbModule, args} ->
        :ssh_subsystem_sup.start_channel(:server, subSysSup,
                                           self(), cbModule, channelId, args,
                                           exec, options)
    end
  end

  defp start_subsystem(binName,
            r_connection(options: options, sub_system_supervisor: subSysSup),
            r_channel(local_id: channelId), _ReplyMsg) do
    name = :erlang.binary_to_list(binName)
    case (check_subsystem(name, options)) do
      {callback, opts} when (is_atom(callback) and
                               callback !== :none)
                            ->
        :ssh_subsystem_sup.start_channel(:server, subSysSup,
                                           self(), callback, channelId, opts,
                                           :undefined, options)
      {:none, _} ->
        {:error, :bad_subsystem}
      {_, _} ->
  ...
  defp check_subsystem('sftp' = ssName, options) do
    case (:ssh_options.get_value(:user_options, :subsystems,
                                   options, :ssh_connection, 1155)) do
      :no_subsys ->
  ...
  defp check_subsystem(ssName, options) do
    subsystems = :ssh_options.get_value(:user_options,
                                          :subsystems, options, :ssh_connection,
                                          1164)
    case (:proplists.get_value(ssName, subsystems,
                                 {:none, []})) do
  ...
  defp update_send_window(channel, _, :undefined,
            r_connection(channel_cache: cache)) do
    do_update_send_window(channel, cache)
  end

  defp update_send_window(r_channel(send_buf: sendBuffer) = channel, dataType,
            data, r_connection(channel_cache: cache)) do
    do_update_send_window(r_channel(channel, send_buf: :queue.in({dataType,
                                                            data},
                                                           sendBuffer)),
                            cache)
  end
  ...
  defp get_window(r_channel(send_buf: buffer,
              send_packet_size: packetSize,
              send_window_size: windowSize0) = channel,
            acc0) do
    case (:queue.out(buffer)) do
      {{:value, {_, data} = msg}, newBuffer} ->
        case (handle_send_window(msg, byte_size(data),
                                   packetSize, windowSize0, acc0)) do
          {windowSize, acc, {_, <<>>}} ->
            {:lists.reverse(acc),
               r_channel(channel, send_window_size: windowSize, 
                            send_buf: newBuffer)}
          {windowSize, acc, rest} ->
            get_window(r_channel(channel, send_window_size: windowSize, 
  ...
  defp handle_send_window(msg = {type, data}, size, packetSize,
            windowSize, acc)
    ...
    case (size <= packetSize) do
      ...
      false ->
        <<msg1 :: size(packetSize) - binary,
            msg2 :: binary>> = data
        {windowSize - packetSize, [{type, msg1} | acc],
           {type, msg2}}
    end
  end
  ...
  defp handle_send_window({type, data}, _, packetSize, windowSize, acc)
    ...
    <<msg1 :: size(windowSize) - binary,
        msg2 :: binary>> = data
    {windowSize - windowSize, [{type, msg1} | acc],
       {type, msg2}}
  end
  ...
  defp pty_req(connectionHandler, channel, term, width, height,
            pixWidth, pixHeight, ptyOpts, timeOut) do
    :ssh_connection_handler.request(connectionHandler,
                                      channel, 'pty-req', true,
                                      [<<byte_size(:unicode.characters_to_binary(term))
                                         ::
                                         size(32) - unsigned - big - integer,
  ...
  def encode_pty_opts(opts) do
    ...
    <<byte_size(bin) :: size(32) - unsigned - big - integer,
        bin :: binary>>
  end
  ...
  defp encode_pty_opts2([{:iutf8, value} | opts]) do
    [42, <<value :: size(32) - unsigned - big - integer>> |
             encode_pty_opts2(opts)]
  end
  ...
  def decode_pty_opts(<<_Len :: size(32) - unsigned - big - integer,
             modes :: size(_Len) - binary>>) do
        ...
        case (start_cli(c0, chId)) do
          ...
          {:error, _Error} ->
            reply = {:connection_reply,
                       channel_failure_msg(r_channel(ch0, :remote_id))}
            {[reply], c0}
        end
      _ ->
  ...
  defp channel_data_reply_msg(channelId, connection, dataType, data) do
    case (:ssh_client_channel.cache_lookup(r_connection(connection, :channel_cache),
                                             channelId)) do
      r_channel(recv_window_size: size) = channel ->
        wantedSize = size - byte_size(data)
        :ssh_client_channel.cache_update(r_connection(connection, :channel_cache),
                                           r_channel(channel, recv_window_size: wantedSize))
        reply_msg(channel, connection,
                    {:data, channelId, dataType, data})
      :undefined ->
        {[], connection}
    end
  end
  ...
  defp reply_msg(chId, c, reply) when is_integer(chId) do
    reply_msg(:ssh_client_channel.cache_lookup(r_connection(c, :channel_cache),
                                                 chId),
                c, reply)
  end
  ...
  defp reply_msg(channel, connection,
            {:open_error, _, _, _} = reply) do
    request_reply_or_data(channel, connection, reply)
  end
  ...
  defp request_reply_or_data(r_channel(local_id: channelId, user: channelPid),
            r_connection(requests: requests) = connection, reply) do
    case (:lists.keysearch(channelId, 1, requests)) do
      {:value, {^channelId, from}} ->
        {[{:channel_request_reply, from, reply}],
           r_connection(connection, requests: :lists.keydelete(channelId, 1,
                                                      requests))}
  ...
  def send_environment_vars(connectionHandler, channel, varNames) do
    :lists.foldl(fn var, :success ->
                      case (:os.getenv(var)) do
                        false ->
                          :success
                        value ->
                          setenv(connectionHandler, channel, false, var, value,
                                   :infinity)
                      end
                 end,
                   :success, varNames)
