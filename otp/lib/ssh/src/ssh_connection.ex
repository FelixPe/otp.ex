defmodule :m_ssh_connection do
  ...
  def session_channel(connectionHandler, initialWindowSize, maxPacketSize, timeout) do
    open_channel(connectionHandler, ~c"session", <<>>, initialWindowSize, maxPacketSize, timeout)
  end
  ...
  def exec(connectionHandler, channelId, command, timeOut) do
    :ssh_connection_handler.request(
      ...
      true,
      [
        <<byte_size(:unicode.characters_to_binary(command))::size(32)-unsigned-big-integer,
  ...
  def subsystem(connectionHandler, channelId, subSystem, timeOut) do
    :ssh_connection_handler.request(
      ...
      true,
      [
        <<byte_size(:unicode.characters_to_binary(subSystem))::size(32)-unsigned-big-integer,
  ...
  defp setenv(connectionHandler, channelId, wantReply, var, value, timeOut) do
    case :ssh_connection_handler.request(
           ...
           wantReply,
           [
             <<byte_size(:unicode.characters_to_binary(var))::size(32)-unsigned-big-integer,
               :unicode.characters_to_binary(var)::binary>>,
             <<byte_size(:unicode.characters_to_binary(value))::size(32)-unsigned-big-integer,
  ...
  def ptty_alloc(connectionHandler, channel, options0, timeOut) do
    ...
    pty_req(
      connectionHandler,
      channel,
      :proplists.get_value(:term, termData, :os.getenv(~c"TERM", ~c"vt100")),
  ...
  def signal(connectionHandler, channel, sig) do
    :ssh_connection_handler.request(
      ...
      false,
      [
        <<byte_size(:unicode.characters_to_binary(sig))::size(32)-unsigned-big-integer,
  ...
  def handle_msg(r_ssh_msg_channel_success(recipient_channel: channelId), connection, _, _SSH) do
    reply_msg(channelId, connection, :success)
  end

  def handle_msg(r_ssh_msg_channel_failure(recipient_channel: channelId), connection, _, _SSH) do
    reply_msg(channelId, connection, :failure)
  end

  def handle_msg(r_ssh_msg_channel_eof(recipient_channel: channelId), connection, _, _SSH) do
    ...
    cond do
      minAcceptedPackSz <= packetSz ->
        try do
          ...
          _, _ ->
            failMsg = channel_open_failure_msg(remoteId, 2, ~c"Connection refused", ~c"en")
      ...
      minAcceptedPackSz > packetSz ->
        failMsg =
          channel_open_failure_msg(
            remoteId,
            1,
            :lists.concat([~c"Maximum packet size below ", minAcceptedPackSz, ~c" not supported"]),
            ~c"en"
          )
  ...
  def handle_msg(
        r_ssh_msg_channel_open(
          channel_type: ~c"forwarded-tcpip",
          sender_channel: remoteId,
      ...
      case :ssh_connection_handler.retrieve(
        ...
        {:ok, {connectToHost, connectToPort}} ->
          case :gen_tcp.connect(connectToHost, connectToPort, [{:active, false}, :binary]) do
            {:ok, sock} ->
              ...
              {channel_open_confirmation_msg(remoteId, chId, winSz, pktSz), chId + 1}
            ...
            {:error, error} ->
              {channel_open_failure_msg(
                 ...
                 2,
                 :io_lib.format(
                   ~c"Forwarded connection refused: ~p",
                   [error]
                 ),
                 ~c"en"
               ), chId}
        ...
        :undefined ->
          {channel_open_failure_msg(
             remoteId,
             2,
             :io_lib.format(~c"No forwarding ordered", []),
             ~c"en"
           ), chId}
      end
  ...
  def handle_msg(
    ...
    {replyMsg, nextChId} =
      case :ssh_options.get_value(:user_options, :tcpip_tunnel_in, options, :ssh_connection, 671) do
        false ->
          {channel_open_failure_msg(remoteId, 2, ~c"Forwarding disabled", ~c"en"), chId}
  ...
  def handle_msg(
        r_ssh_msg_channel_open(channel_type: ~c"session", sender_channel: remoteId),
        connection,
        :client,
        _SSH
      ) do
  ...
  def handle_msg(r_ssh_msg_channel_open(sender_channel: remoteId), connection, _, _SSH) do
    failMsg = channel_open_failure_msg(remoteId, 1, ~c"Not allowed", ~c"en")
    {[{:connection_reply, failMsg}], connection}
  end
  ...
  def handle_msg(
        r_ssh_msg_channel_request(request_type: ~c"subsystem"),
        connection,
        :client,
        _SSH
      ) do
  ...
  def handle_msg(
    ...
    ptyOpts =
      case :proplists.get_value(:onlcr, ptyOpts0, :undefined) do
        :undefined ->
          [{:onlcr, 1} | ptyOpts0]
  ...
  def handle_msg(r_ssh_msg_channel_request(request_type: ~c"pty-req"), connection, :client, _SSH) do
    {[], connection}
  end
  ...
  def handle_msg(r_ssh_msg_channel_request(request_type: ~c"shell"), connection, :client, _SSH) do
    {[], connection}
  end
  ...
  def handle_msg(r_ssh_msg_channel_request(request_type: ~c"exec"), connection, :client, _SSH) do
    {[], connection}
  end
  ...
  def handle_msg(r_ssh_msg_channel_request(request_type: ~c"env"), connection, :client, _SSH) do
    {[], connection}
  end
  ...
  def handle_msg(
    ...
    case :ssh_options.get_value(:user_options, :tcpip_tunnel_out, opts, :ssh_connection, 909) do
      false ->
        {[{:connection_reply, request_failure_msg()}], connection}

      true ->
        subSysSup =
          :ssh_options.get_value(:internal_options, :subsystem_sup, opts, :ssh_connection, 915)
  ...
  def handle_msg(r_ssh_msg_disconnect(code: code, description: description), connection, _, _SSH) do
  ...
  defp setup_session(
    ...
    openConfMsg = channel_open_confirmation_msg(remoteId, newChannelID, winSz, pktSz)
  ...
  defp check_subsystem(~c"sftp" = ssName, options) do
    case :ssh_options.get_value(:user_options, :subsystems, options, :ssh_connection, 1155) do
      :no_subsys ->
  ...
  defp check_subsystem(ssName, options) do
    subsystems =
      :ssh_options.get_value(:user_options, :subsystems, options, :ssh_connection, 1164)
  ...
  defp get_window(
    ...
    case :queue.out(buffer) do
      {{:value, {_, data} = msg}, newBuffer} ->
        case handle_send_window(msg, byte_size(data), packetSize, windowSize0, acc0) do
          {windowSize, acc, {_, <<>>}} ->
            {:lists.reverse(acc),
             r_channel(channel,
               send_window_size: windowSize,
  ...
  defp pty_req(
    ...
    :ssh_connection_handler.request(
      ...
      true,
      [
        <<byte_size(:unicode.characters_to_binary(term))::size(32)-unsigned-big-integer,
  ...
  defp pty_default_dimensions(dimension, termData) do
    case :proplists.get_value(dimension, termData, 0) do
      ...
      _ ->
        pixelDim = :erlang.list_to_atom(~c"pixel_" ++ :erlang.atom_to_list(dimension))
