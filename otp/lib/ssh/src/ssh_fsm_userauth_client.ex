defmodule :m_ssh_fsm_userauth_client do
  ...
  def handle_event(:internal, r_ssh_msg_ext_info() = msg, {:userauth, :client},
           d0) do
  ...
  def handle_event(:internal, r_ssh_msg_userauth_success(), {:userauth, :client},
           d0 = r_data(ssh_params: ssh)) do
  ...
  def handle_event(:internal, r_ssh_msg_userauth_failure(), {:userauth, :client} = stateName,
           r_data(ssh_params: r_ssh(userauth_methods: [])) = d0) do
    {shutdown,
       d} = :ssh_connection_handler.send_disconnect(14,
                                                      :io_lib.format('User auth failed for: ~p',
                                                                       [r_data(d0, :auth_user)]),
  ...
  def handle_event(:internal, r_ssh_msg_userauth_failure(authentications: methods),
           stateName = {:userauth, :client},
           d0 = r_data(ssh_params: ssh0)) do
    ssh1 = (case (r_ssh(ssh0, :userauth_methods)) do
              :none ->
    ...
    case (:ssh_auth.userauth_request_msg(ssh1)) do
      {:send_disconnect, code, ssh} ->
        {shutdown,
           d} = :ssh_connection_handler.send_disconnect(code,
                                                          :io_lib.format('User auth failed for: ~p',
                                                                           [r_data(d0, :auth_user)]),
                                                          :ssh_fsm_userauth_client,
                                                          96, stateName,
      ...
      {'keyboard-interactive', {msg, ssh}} ->
        d = :ssh_connection_handler.send_msg(msg,
                                               r_data(d0, ssh_params: ssh))
        {:next_state, {:userauth_keyboard_interactive, :client},
           d}
      {_Method, {msg, ssh}} ->
        d = :ssh_connection_handler.send_msg(msg,
                                               r_data(d0, ssh_params: ssh))
  ...
  def handle_event(:internal, r_ssh_msg_userauth_banner(message: msg), {:userauth, :client},
           d) do
  ...
  def handle_event(:internal, r_ssh_msg_userauth_info_request() = msg,
           {:userauth_keyboard_interactive, :client},
           r_data(ssh_params: ssh0) = d0) do
    case (:ssh_auth.handle_userauth_info_request(msg,
                                                   ssh0)) do
      {:ok, {reply, ssh}} ->
        d = :ssh_connection_handler.send_msg(reply,
                                               r_data(d0, ssh_params: ssh))
        {:next_state,
  ...
  def handle_event(:internal, r_ssh_msg_userauth_failure(),
           {:userauth_keyboard_interactive, :client},
           r_data(ssh_params: ssh0) = d0) do
    prefs = (for {method, m, f,
                    a} <- r_ssh(ssh0, :userauth_preference),
  ...
  def handle_event(:internal, r_ssh_msg_userauth_failure(),
    ...
    d = (case (:ssh_options.get_value(:user_options,
                                        :password, opts,
                                        :ssh_fsm_userauth_client, 138)) do
           :undefined ->
             d0
           _ ->
             r_data(d0, ssh_params: r_ssh(ssh0, opts: :ssh_options.put_value(:user_options,
                                                                      {:password,
                                                                         :not_ok},
                                                                      opts,
                                                                      :ssh_fsm_userauth_client,
  ...
  def handle_event(:internal, r_ssh_msg_userauth_success(),
           {:userauth_keyboard_interactive_info_response, :client},
           d) do
    {:next_state, {:userauth, :client}, d, [:postpone]}
  end

  def handle_event(:internal, r_ssh_msg_userauth_info_request(),
           {:userauth_keyboard_interactive_info_response, :client},
           d) do
    {:next_state, {:userauth_keyboard_interactive, :client},
       d, [:postpone]}
