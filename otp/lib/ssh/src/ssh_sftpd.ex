defmodule :m_ssh_sftpd do
  ...
  def init(options) do
    ...
    root0 = :proplists.get_value(:root, options, ~c"")
  ...
  def handle_ssh_msg(
    ...
    report = :io_lib.format(~c"Connection closed by peer signal ~p~n Error ~p~n", [signal, error])
  ...
  defp handle_op(
    ...
    case isDir do
      false when vsn > 5 ->
        :ssh_xfer.xf_send_status(xF, reqId, 19, ~c"Not a directory")
        state1

      false ->
        :ssh_xfer.xf_send_status(xF, reqId, 4, ~c"Not a directory")
        state1
  ...
  defp handle_op(
    ...
    case isDir do
      true when vsn > 5 ->
        :ssh_xfer.xf_send_status(r_state(state0, :xf), reqId, 24, ~c"File is a directory")
        state0

      true ->
        :ssh_xfer.xf_send_status(r_state(state0, :xf), reqId, 4, ~c"File is a directory")
        state0
  ...
  defp read_dir(
         ...
         xF = r_ssh_xfer(cm: _CM, channel: _Channel, vsn: vsn),
         reqId,
         handle,
         relPath,
         {:cache, files}
  ...
  defp get_long_name(fileName, i) when elem(i, 0) === :file_info do
    longame(
      {fileName, r_file_info(i, :type), r_file_info(i, :size), r_file_info(i, :mtime),
       r_file_info(i, :mode), r_file_info(i, :uid), r_file_info(i, :gid)}
    )
  end
  ...
  defp resolve_symlinks_2([~c"." | restPath], state0, linkCnt, accPath) do
  ...
  defp relate_file_name(file, r_state(cwd: cWD, root: ~c""), canonicalize) do
  ...
  defp make_relative_filename(~c"/") do
    ~c"./"
  end

  defp make_relative_filename(~c"/" ++ file) do
    file
  end
  ...
  defp ensure_trailing_slash_is_preserved(file0, file1) do
    case {:lists.suffix(~c"/", file0), :lists.suffix(~c"/", file1)} do
      {true, false} ->
        file1 ++ ~c"/"
  ...
  defp canonicalize_filename_2([~c".." | rest], [~c"/"] = acc) do
    canonicalize_filename_2(rest, acc)
  end

  defp canonicalize_filename_2([~c".." | rest], [_Dir | paths]) do
    canonicalize_filename_2(rest, paths)
  end

  defp canonicalize_filename_2([~c"." | rest], acc) do
    canonicalize_filename_2(rest, acc)
  end
