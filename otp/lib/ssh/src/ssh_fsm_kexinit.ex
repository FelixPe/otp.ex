defmodule :m_ssh_fsm_kexinit do
  ...
  def handle_event(:internal, {r_ssh_msg_kexinit() = kex, payload},
           {:kexinit, role, reNeg},
           d = r_data(key_exchange_init_msg: ownKex)) do
    ssh1 = :ssh_transport.key_init(peer_role(role),
                                     r_data(d, :ssh_params), payload)
    ssh = (case (:ssh_transport.handle_kexinit_msg(kex,
                                                     ownKex, ssh1, reNeg)) do
    ...
    {:next_state, {:key_exchange, role, reNeg},
       r_data(d, ssh_params: ssh)}
  end

  def handle_event(:internal, r_ssh_msg_kexdh_init() = msg,
           {:key_exchange, :server, reNeg}, d) do
    {:ok, kexdhReply,
       ssh1} = :ssh_transport.handle_kexdh_init(msg,
                                                  r_data(d, :ssh_params))
    :ssh_connection_handler.send_bytes(kexdhReply, d)
    {:ok, newKeys,
       ssh2} = :ssh_transport.new_keys_message(ssh1)
    :ssh_connection_handler.send_bytes(newKeys, d)
    {:ok, extInfo,
       ssh} = :ssh_transport.ext_info_message(ssh2)
    :ssh_connection_handler.send_bytes(extInfo, d)
    {:next_state, {:new_keys, :server, reNeg},
       r_data(d, ssh_params: ssh)}
  end

  def handle_event(:internal, r_ssh_msg_kexdh_reply() = msg,
           {:key_exchange, :client, reNeg}, d) do
    {:ok, newKeys,
       ssh1} = :ssh_transport.handle_kexdh_reply(msg,
                                                   r_data(d, :ssh_params))
    :ssh_connection_handler.send_bytes(newKeys, d)
    {:ok, extInfo,
       ssh} = :ssh_transport.ext_info_message(ssh1)
    :ssh_connection_handler.send_bytes(extInfo, d)
    {:next_state, {:new_keys, :client, reNeg},
       r_data(d, ssh_params: ssh)}
  end

  def handle_event(:internal, r_ssh_msg_kex_dh_gex_request() = msg,
           {:key_exchange, :server, reNeg}, d) do
    {:ok, gexGroup,
       ssh1} = :ssh_transport.handle_kex_dh_gex_request(msg,
                                                          r_data(d, :ssh_params))
  ...
  def handle_event(:internal, r_ssh_msg_kex_dh_gex_request_old() = msg,
           {:key_exchange, :server, reNeg}, d) do
    {:ok, gexGroup,
       ssh1} = :ssh_transport.handle_kex_dh_gex_request(msg,
                                                          r_data(d, :ssh_params))
  ...
  def handle_event(:internal, r_ssh_msg_kex_dh_gex_group() = msg,
           {:key_exchange, :client, reNeg}, d) do
    {:ok, kexGexInit,
       ssh} = :ssh_transport.handle_kex_dh_gex_group(msg,
                                                       r_data(d, :ssh_params))
  ...
  def handle_event(:internal, r_ssh_msg_kex_ecdh_init() = msg,
           {:key_exchange, :server, reNeg}, d) do
    {:ok, kexEcdhReply,
       ssh1} = :ssh_transport.handle_kex_ecdh_init(msg,
                                                     r_data(d, :ssh_params))
  ...
  def handle_event(:internal, r_ssh_msg_kex_ecdh_reply() = msg,
           {:key_exchange, :client, reNeg}, d) do
    {:ok, newKeys,
       ssh1} = :ssh_transport.handle_kex_ecdh_reply(msg,
                                                      r_data(d, :ssh_params))
    :ssh_connection_handler.send_bytes(newKeys, d)
    {:ok, extInfo,
       ssh} = :ssh_transport.ext_info_message(ssh1)
  ...
  def handle_event(:internal, r_ssh_msg_kex_dh_gex_init() = msg,
           {:key_exchange_dh_gex_init, :server, reNeg}, d) do
    {:ok, kexGexReply,
       ssh1} = :ssh_transport.handle_kex_dh_gex_init(msg,
                                                       r_data(d, :ssh_params))
  ...
  def handle_event(:internal, r_ssh_msg_kex_dh_gex_reply() = msg,
           {:key_exchange_dh_gex_reply, :client, reNeg}, d) do
    {:ok, newKeys,
       ssh1} = :ssh_transport.handle_kex_dh_gex_reply(msg,
                                                        r_data(d, :ssh_params))
    :ssh_connection_handler.send_bytes(newKeys, d)
    {:ok, extInfo,
       ssh} = :ssh_transport.ext_info_message(ssh1)
  ...
  def handle_event(:internal, r_ssh_msg_newkeys() = msg,
           {:new_keys, :client, :init}, d0) do
    {:ok, ssh1} = :ssh_transport.handle_new_keys(msg,
                                                   r_data(d0, :ssh_params))
    {msgReq, ssh} = :ssh_auth.service_request_msg(ssh1)
    d = :ssh_connection_handler.send_msg(msgReq,
                                           r_data(d0, ssh_params: ssh))
    {:next_state, {:ext_info, :client, :init}, d}
  end

  def handle_event(:internal, r_ssh_msg_newkeys() = msg,
           {:new_keys, :server, :init}, d) do
    {:ok, ssh} = :ssh_transport.handle_new_keys(msg,
                                                  r_data(d, :ssh_params))
    {:next_state, {:ext_info, :server, :init},
       r_data(d, ssh_params: ssh)}
  end

  def handle_event(:internal, r_ssh_msg_newkeys() = msg,
           {:new_keys, role, :renegotiate}, d) do
    {:ok, ssh} = :ssh_transport.handle_new_keys(msg,
                                                  r_data(d, :ssh_params))
    {:next_state, {:ext_info, role, :renegotiate},
       r_data(d, ssh_params: ssh)}
  end

  def handle_event(:internal, r_ssh_msg_ext_info() = msg, {:ext_info, role, :init},
           d0) do
    d = :ssh_connection_handler.handle_ssh_msg_ext_info(msg,
                                                          d0)
    {:next_state, {:service_request, role}, d,
       {:change_callback_module, :ssh_connection_handler}}
  end

  def handle_event(:internal, r_ssh_msg_ext_info() = msg,
           {:ext_info, role, :renegotiate}, d0) do
    d = :ssh_connection_handler.handle_ssh_msg_ext_info(msg,
                                                          d0)
  ...
  def handle_event(:internal, r_ssh_msg_newkeys() = msg,
           {:ext_info, _Role, :renegotiate}, d) do
    {:ok, ssh} = :ssh_transport.handle_new_keys(msg,
                                                  r_data(d, :ssh_params))
  ...
  def handle_event(:internal, msg, {:ext_info, role, :renegotiate},
           d)
      when is_tuple(msg) do
    {:next_state, {:connected, role}, d,
       [:postpone, {:change_callback_module,
  ...
  def handle_event(type, event, stateName, d) do
    :ssh_connection_handler.handle_event(type, event,
                                           stateName, d)
  end
