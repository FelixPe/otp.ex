defmodule :m_ssh do
  ...
  def connect(socket, userOptions, negotiationTimeout)
      when (is_list(userOptions) and
              negotiationTimeout == :infinity) or
             (is_integer(negotiationTimeout) and negotiationTimeout >= 0) do
    case :ssh_options.handle_options(
      ...
      options = %{} ->
        case valid_socket_to_use(
               socket,
               :ssh_options.get_value(:user_options, :transport, options, :ssh, 165)
             ) do
          :ok ->
  ...
  def connect(_HostOrSocket, portOrOptions, optionsOrTimeout) do
  ...
  def connect(host0, port, userOptions, negotiationTimeout)
    ...
    case :ssh_options.handle_options(
      ...
      options ->
        socketOpts = [
          {:active, false}
          | :ssh_options.get_value(:user_options, :socket_options, options, :ssh, 192)
        ]
        ...
        try do
          ...
          _, :badarg ->
            {:error,
             {:options,
              :ssh_options.get_value(:user_options, :socket_options, options, :ssh, 202)}}
  ...
  def connect(_Host, port, userOptions, negotiationTimeout) do
    bad_arg([{:port, port}, {:options, userOptions}, {:timeout, negotiationTimeout}])
  end
  ...
  defp bad_args(args) do
    ...
    check = fn
      ...
      {:timeout, arg} ->
        isErr.(
          arg == :infinity or (is_integer(arg) and arg >= 0),
          :invalid_timeout
        )
  ...
  defp continue_connect(socket, options0, negTimeout) do
    ...
    address =
      r_address(
        address: sockHost,
        port: sockPort,
        profile: :ssh_options.get_value(:user_options, :profile, options, :ssh, 249)
      )
  ...
  def daemon(socket, userOptions) do
    case :ssh_options.handle_options(
      ...
      %{} = options0 ->
        case valid_socket_to_use(
               socket,
               :ssh_options.get_value(:user_options, :transport, options0, :ssh, 335)
             ) do
          :ok ->
            try do
              ...
              address =
                r_address(
                  address: sockHost,
                  port: sockPort,
                  profile: :ssh_options.get_value(:user_options, :profile, options0, :ssh, 343)
                )
              ...
              case :ssh_system_sup.start_subsystem(:server, address, socket, options) do
                {:ok, pid} ->
                  {:ok, pid}
  ...
  def daemon_info(daemonRef) do
    case :ssh_system_sup.get_daemon_listen_address(daemonRef) do
      {:ok, a} ->
        address =
          case :inet.parse_strict_address(r_address(a, :address)) do
            {:ok, iP} ->
        ...
        opts =
          case :ssh_system_sup.get_options(
                 daemonRef,
                 address
               ) do
            {:ok, optMap} ->
        ...
        {:ok,
         [
           {:port, r_address(address, :port)},
           {:ip, r_address(address, :address)},
  ...
  def shell(connectionRef) when is_pid(connectionRef) do
    case :ssh_connection.session_channel(
      ...
      {:ok, channelId} ->
        ...
        :success =
          :ssh_connection.send_environment_vars(connectionRef, channelId, [~c"LANG", ~c"LC_ALL"])
  ...
  defp fp_fmt(:b64, bin) do
    b64Chars = ~c"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
  ...
  defp maybe_open_listen_socket(host, port, options) do
    opened =
      case :ssh_options.get_value(:socket_options, :fd, options, :ssh, 901) do
  ...
  defp close_listen_socket(listenSocket, options) do
    try do
      {_, callback, _} = :ssh_options.get_value(:user_options, :transport, options, :ssh, 923)
  ...
  defp controlling_process(listenSocket, reqPid, options) do
    {_, callback, _} = :ssh_options.get_value(:user_options, :transport, options, :ssh, 930)
    callback.controlling_process(listenSocket, reqPid)
  end

  defp transport_connect(host, port, socketOpts, options) do
    {_, callback, _} = :ssh_options.get_value(:user_options, :transport, options, :ssh, 934)

    callback.connect(
      host,
      port,
      socketOpts,
      :ssh_options.get_value(:user_options, :connect_timeout, options, :ssh, 935)
    )
  end
  ...
  def ssh_dbg_format(
        :tcp,
        {:call, {:ssh, :controlling_process, [listenSocket, reqPid, _Opts]}}
      ) do
    [
      ~c"TCP socket transferred to\n",
      :io_lib.format(~c"Sock: ~p~nToPid: ~p~n", [listenSocket, reqPid])
    ]
  end

  def ssh_dbg_format(
        :tcp,
        {:return_from, {:ssh, :controlling_process, 3}, _Result}
      ) do
    :skip
  end
  ...
  def ssh_dbg_format(
        :tcp,
        {:call, {:ssh, :close_listen_socket, [listenSocket, _Opts]}}
      ) do
    [~c"TCP socket listening closed\n", :io_lib.format(~c"Sock: ~p~n", [listenSocket])]
  end

  def ssh_dbg_format(
        :tcp,
        {:return_from, {:ssh, :close_listen_socket, 2}, _Result}
      ) do
    :skip
  end
  ...
  def ssh_dbg_format(
        :tcp,
        {:call, {:ssh, :transport_connect, [host, port, sockOpts, _Opts]}},
        stack
      ) do
  ...
  def ssh_dbg_format(
        :tcp,
        {:return_from, {:ssh, :transport_connect, 4}, {:ok, sock}},
        [{:transport_connect, host, port, sockOpts} | stack]
      ) do
    {[
       ~c"TCP connected to\n",
       :io_lib.format(~c"Host: ~p~nPort: ~p~nSockOpts: ~p~nSocket: ~p~n", [
         host,
         port,
         sockOpts,
         sock
  ...
  def ssh_dbg_format(
    ...
    {[
       ~c"TCP connected FAILED to\n",
       :io_lib.format(~c"Host: ~p~nPort: ~p~nSockOpts: ~p~nResult: ~p~n", [
         host,
         port,
         sockOpts,
         result
