defmodule :m_systools_rc do
  ...
  def format_error({:bad_op_before_point_of_no_return, instruction}) do
    :io_lib.format(~c"Bad instruction ~p~nbefore point_of_no_return~n", [instruction])
  end

  def format_error({:no_object_code, mod}) do
    :io_lib.format(~c"No load_object_code found for module: ~w~n", [mod])
  end

  def format_error({:suspended_not_resumed, mods}) do
    :io_lib.format(~c"Suspended but not resumed: ~p~n", [mods])
  end

  def format_error({:resumed_not_suspended, mods}) do
    :io_lib.format(~c"Resumed but not suspended: ~p~n", [mods])
  end

  def format_error({:code_change_not_suspended, mods}) do
    :io_lib.format(~c"Code changed but not suspended: ~p~n", [mods])
  end

  def format_error({:start_not_stop, mods}) do
    :io_lib.format(~c"Started but not stopped: ~p~n", [mods])
  end

  def format_error({:stop_not_start, mods}) do
    :io_lib.format(~c"Stopped but not started: ~p~n", [mods])
  end

  def format_error({:no_such_application, app}) do
    :io_lib.format(~c"Started undefined application: ~w~n", [app])
  end

  def format_error({:removed_application_present, app}) do
    :io_lib.format(~c"Removed application present: ~w~n", [app])
  end

  def format_error(:dup_mnesia_backup) do
    :io_lib.format(~c"Duplicate mnesia_backup~n", [])
  end

  def format_error(:bad_mnesia_backup) do
    :io_lib.format(~c"mnesia_backup in bad position~n", [])
  end

  def format_error({:conflicting_versions, lib, v1, v2}) do
    :io_lib.format(~c"Conflicting versions for ~w, ~ts and ~ts~n", [lib, v1, v2])
  end

  def format_error({:no_appl_vsn, appl}) do
    :io_lib.format(~c"No version specified for application: ~w~n", [appl])
  end

  def format_error({:no_such_module, mod}) do
    :io_lib.format(~c"No such module: ~w~n", [mod])
  end

  def format_error(:too_many_point_of_no_return) do
    :io_lib.format(~c"Too many point_of_no_return~n", [])
  end

  def format_error({:bad_instruction, x}) do
    :io_lib.format(~c"Bad instruction: ~tp~n", [x])
  end

  def format_error({:bad_module, x}) do
    :io_lib.format(~c"Bad module: ~tp(should be atom())~n", [x])
  end

  def format_error({:bad_code_change, x}) do
    :io_lib.format(~c"Bad code_change: ~tp(should be {Mod, Extra})~n", [x])
  end

  def format_error({:bad_change, x}) do
    :io_lib.format(~c"Bad change spec: ~tp(should be soft | {advanced, E})~n", [x])
  end

  def format_error({:bad_mod_type, x}) do
    :io_lib.format(~c"Bad module type: ~tp(should be static | dynamic)~n", [x])
  end

  def format_error({:bad_purge_method, x}) do
    :io_lib.format(~c"Bad purge method: ~tp(should be soft_purge | brutal_purge)~n", [x])
  end

  def format_error({:bad_list, x}) do
    :io_lib.format(~c"Bad list: ~tp~n", [x])
  end

  def format_error({:bad_args_list, x}) do
    :io_lib.format(~c"Bad argument list: ~tp~n", [x])
  end

  def format_error({:bad_node, x}) do
    :io_lib.format(~c"Bad node: ~tp(should be atom())~n", [x])
  end

  def format_error({:bad_application, x}) do
    :io_lib.format(~c"Bad application: ~tp(should be atom())~n", [x])
  end

  def format_error({:bad_func, x}) do
    :io_lib.format(~c"Bad function: ~tp(should be atom())~n", [x])
  end

  def format_error({:bad_lib, x}) do
    :io_lib.format(~c"Bad library: ~tp(should be atom())~n", [x])
  end

  def format_error({:bad_lib_vsn, x}) do
    :io_lib.format(~c"Bad library version: ~tp(should be string())~n", [x])
  end

  def format_error({:bad_timeout, x}) do
    :io_lib.format(~c"Bad timeout: ~tp(should be infinity | int() > 0)~n", [x])
  end

  def format_error({:undef_module, mod}) do
    :io_lib.format(~c"Undefined module: ~p~n", [mod])
  end

  def format_error({:muldef_module, mod}) do
    :io_lib.format(~c"Multiply defined module: ~p~n", [mod])
  end
