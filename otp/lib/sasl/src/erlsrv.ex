defmodule :m_erlsrv do
  ...
  def erlsrv(eVer) do
    root = :code.root_dir()
    :filename.join([root, ~c"erts-" ++ eVer, ~c"bin", ~c"erlsrv.exe"])
  end

  defp current_version() do
    hd(:string.lexemes(:erlang.system_info(:version), ~c"_ "))
  end
  ...
  defp run_erlsrv_interactive(eVer, commands) do
    case (try do
            :erlang.open_port(
              {:spawn, ~c"\"" ++ erlsrv(eVer) ++ ~c"\" readargs"},
              [{:line, 1000}, :eof]
            )
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  def disable_service(eVer, serviceName) do
    run_erlsrv(eVer, ~c"disable " ++ serviceName)
  end
  ...
  def enable_service(eVer, serviceName) do
    run_erlsrv(eVer, ~c"enable " ++ serviceName)
  end

  def remove_service(serviceName) do
    run_erlsrv(~c"remove " ++ serviceName)
  end
  ...
  def rename_service(eVer, fromName, toName) do
    run_erlsrv(eVer, ~c"rename " ++ fromName ++ ~c" " ++ toName)
  end
  ...
  def get_service(eVer, serviceName) do
    case run_erlsrv(eVer, ~c"list " ++ serviceName) do
      :failed ->
      ...
      {:ok, data} ->
        table = [
          ...
          {~c"OnFail", :onfail, ~c"ignore"},
          {~c"Machine", :machine, []},
          {~c"WorkDir", :workdir, []},
          {~c"SName", :sname, []},
          {~c"Name", :name, []},
          {~c"Priority", :priority, ~c"default"},
          {~c"DebugType", :debugtype, ~c"none"},
          {~c"Args", :args, []},
          {~c"InternalServiceName", :internalservicename, []},
          {~c"Comment", :comment, []}
        ]
        ...
        envParts =
          :lists.map(
            fn s ->
              x = :string.trim(s, :leading, ~c"$\t")

              case hd(:string.lexemes(x, ~c"=")) do
                ^x ->
                  {x, ~c""}
      ...
      {:value, {_, name}} ->
        {action, service1} =
          case get_service(
            ...
            _ ->
              {~c"set", :lists.keydelete(:internalservicename, 1, service)}
          end
  ...
  defp build_commands2([{atom, []} | t], a) do
    build_commands2(
      t,
      [~c"-" ++ :erlang.atom_to_list(atom) | a]
    )
  end

  defp build_commands2([{:args, l} | t], a) do
    build_commands2(t, [concat_args(l), ~c"-args" | a])
  end

  defp build_commands2([{atom, value} | t], a) do
    build_commands2(
      t,
      [value, ~c"-" ++ :erlang.atom_to_list(atom) | a]
    )
  end
  ...
  def new_service(newServiceName, oldService, data, restartName) do
    ...
    tmp =
      case :lists.keysearch(:env, 1, tmp1) do
        {:value, {:env, env0}} ->
          env1 = :lists.keydelete(~c"ERLSRV_SERVICE_NAME", 1, env0)

          :lists.keyreplace(
            :env,
            1,
            tmp1,
            {:env, [{~c"ERLSRV_SERVICE_NAME", restartName} | env1]}
          )
    ...
    case found do
      ...
      true ->
        stripArgs = backstrip(args, [~c"++" | tail])
  ...
  defp check_tail([~c"-data", theDataDir | t], otherFlags, _DataDir) do
  ...
  defp split_helper(~c"Env:", {where, 0}) do
    {where + 1, where}
  end
