defmodule :m_erlsrv do
  ...
  defp run_erlsrv_interactive(eVer, commands) do
    case ((try do
            :erlang.open_port({:spawn, '"' ++ erlsrv(eVer) ++ '" readargs'},
                                [{:line, 1000}, :eof])
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
  ...
  defp write_all_data(port, [h | t]) do
    send(port, {self(),
                  {:command,
                     :unicode.characters_to_binary([h, :io_lib.nl()])}})
  ...
  def get_service(eVer, serviceName) do
    case (run_erlsrv(eVer, 'list ' ++ serviceName)) do
      ...
      {:ok, data} ->
        table = [{'Service name', :servicename, []}, {'StopAction', :stopaction, []},
                                            {'OnFail', :onfail, 'ignore'}, {'Machine', :machine, []},
                                                                 {'WorkDir', :workdir,
                                                                    []},
                                                                     {'SName', :sname,
                                                                        []},
  ...
  def store_service(emulatorVersion, service) do
    case (:lists.keysearch(:servicename, 1, service)) do
      ...
      {:value, {_, name}} ->
        {action, service1} = (case (get_service(emulatorVersion,
                                                  name)) do
                                {:error, :no_such_service} ->
                                  {'add', service}
                                _ ->
                                  {'set',
                                     :lists.keydelete(:internalservicename, 1,
                                                        service)}
                              end)
        commands = [action | build_commands(name, service1)]
        case (run_erlsrv_interactive(emulatorVersion,
                                       commands)) do
          {:ok, _} ->
            :ok
          x ->
  ...
  def new_service(newServiceName, oldService, data, restartName) do
    tmp0 = :lists.keydelete(:internalservicename, 1,
                              oldService)
    tmp1 = :lists.keyreplace(:servicename, 1, tmp0,
                               {:servicename, newServiceName})
    tmp = (case (:lists.keysearch(:env, 1, tmp1)) do
             {:value, {:env, env0}} ->
               env1 = :lists.keydelete('ERLSRV_SERVICE_NAME', 1, env0)
               :lists.keyreplace(:env, 1, tmp1,
                                   {:env, [{'ERLSRV_SERVICE_NAME', restartName} | env1]})
             _ ->
               tmp1
           end)
    argsTmp = (case (:lists.keysearch(:args, 1, tmp)) do
                 false ->
                   []
                 {:value, {:args, oldArgs}} ->
                   oldArgs
    ...
    {found, tail} = :lists.foldr(fn a, {flag, accIn} ->
                                      case ({flag, a}) do
                                        {true, _} ->
                                          {flag, accIn}
                                        {false, '++'} ->
                                   ...
                                   {false, []}, args)
    {otherFlags, _DataDir} = (case (found) do
                                true ->
                                  check_tail(tail)
                                false ->
                                  {[], false}
                              end)
    newArgs1 = (case (data) do
                  [] ->
                    otherFlags
                  _ ->
  ...
  defp backstrip(list, tail) do
    :lists.reverse(backstrip2(:lists.reverse(list),
                                :lists.reverse(tail)))
  end
  ...
  defp split_by_env(data) do
    case (:lists.foldl(&split_helper/2, {0, 0}, data)) do
      {_, 0} ->
        {data, []}
      {len, pos} ->
        {:lists.sublist(data, pos),
           :lists.sublist(data, pos + 2, len)}
    end
  end
