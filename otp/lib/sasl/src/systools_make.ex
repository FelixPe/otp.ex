defmodule :m_systools_make do
  use Bitwise
  import :lists, only: [append: 1, filter: 2, foldl: 3,
                          foreach: 2, keysearch: 3, keysort: 2, map: 2,
                          member: 2, reverse: 1]
  require Record
  ...
  def make_script(relName, flags) when (is_list(relName) and
                                 is_list(flags)) do
        ...
        case (get_release(relName, path, modTestP)) do
          {:ok, release, appls, warnings0} ->
            ...
            case (:systools_lib.werror(flags, warnings)) do
              true ->
                warnings1 = (for {:warning, w} <- warnings do
                               w
                             end)
                return({:error, :systools_make,
  ...
  defp do_make_hybrid_boot(tmpVsn, oldBoot, newBoot, args) do
    {:script, {_RelName1, _RelVsn1},
       oldScript} = :erlang.binary_to_term(oldBoot)
    {:script, {newRelName, _RelVsn2},
       newScript} = :erlang.binary_to_term(newBoot)
    fun1 = fn {:progress, :kernel_load_completed} ->
                false
              _ ->
                true
           end
    {_OldKernelLoad, oldRest1} = :lists.splitwith(fun1,
                                                    oldScript)
    {newKernelLoad, newRest1} = :lists.splitwith(fun1,
                                                   newScript)
    fun2 = fn {:progress, :modules_loaded} ->
                false
              _ ->
                true
           end
    {oldModLoad, oldRest2} = :lists.splitwith(fun2,
                                                oldRest1)
    {newModLoad, newRest2} = :lists.splitwith(fun2,
                                                newRest1)
    fun3 = fn {:kernelProcess, _, _} ->
                false
              _ ->
                true
           end
    ...
    fun4 = fn {:progress, :init_kernel_started} ->
                false
              _ ->
                true
           end
    {_OldKernelProcs, oldApps} = :lists.splitwith(fun4,
                                                    oldRest3)
    {newKernelProcs, newApps} = :lists.splitwith(fun4,
                                                   newRest3)
    ...
    {stdlib, sasl} = get_apps(newApps, :undefined,
                                :undefined)
    ...
    boot = :erlang.term_to_binary({:script,
                                     {newRelName, tmpVsn}, script})
    {:ok, boot}
  end
  ...
  defp replace_module_load(old, new, [mP | matchPaths]) do
    replace_module_load(do_replace_module_load(old, new,
                                                 mP),
                          new, matchPaths)
  end
  ...
  defp do_replace_module_load([{:path, [oldAppPath]}, {:primLoad, oldMods} |
                                      oldRest],
            new, mP) do
  ...
  defp get_module_load([{:path, [appPath]}, {:primLoad, mods} | rest],
            mP) do
  ...
  defp replace_paths([{:path, oldPaths} | old], new, matchPaths) do
    ...
    [{:path,
        do_replace_paths(oldPaths, newPath, matchPaths)} |
         old]
  end
  ...
  defp do_replace_paths(old, new, [mP | matchPaths]) do
    do_replace_paths(do_replace_paths1(old, new, mP), new,
                       matchPaths)
  end
  ...
  defp replace_apps([{:apply,
              {:application, :load, [{:application, :sasl, _}]}} |
               script],
            _Stdlib, sasl) do
    [sasl | script]
  end
  ...
  defp add_apply_upgrade(script, args) do
    [{:progress, :started} |
         revScript] = :lists.reverse(script)
    :lists.reverse([{:progress, :started}, {:apply,
                                              {:release_handler,
                                                 :new_emulator_upgrade, args}} |
                                               revScript])
  end
  ...
  def make_tar(relName, flags) when (is_list(relName) and
                                 is_list(flags)) do
  ...
  defp get_release1(file, path, modTestP) do
    ...
    {:ok, appls2} = sort_used_and_incl_appls(appls1,
                                               release)
  ...
  defp check_rel(release) do
    case ((try do
      ...
      {:ok, {name, vsn, evsn, appl, incl}, ws} ->
        {:ok,
           r_release(name: name, vsn: vsn, erts_vsn: evsn,
               applications: appl, incl_apps: incl),
           ws}
      {:error, error} ->
        throw({:error, :systools_make, error})
      error ->
  ...
  defp check_appl(appl) do
    case (filter(fn {app, vsn} when is_atom(app) ->
                      not string_p(vsn)
                    ...
                    {app, vsn, type, incl} when (is_atom(app) and
                                                   is_atom(type) and
                                                   is_list(incl))
                                                ->
                      case ({string_p(vsn), is_app_type(type),
                               a_list_p(incl)}) do
                        {true, true, true} ->
                          false
                        _ ->
      ...
      [] ->
        ...
        {:ok, ws} = mandatory_applications(applsNoIncls,
  ...
  defp mandatory_applications([{:kernel, _, type} | apps], :undefined, stdlib,
            sasl) do
    mandatory_applications(apps, type, stdlib, sasl)
  end

  defp mandatory_applications([{:stdlib, _, type} | apps], kernel, :undefined,
            sasl) do
    mandatory_applications(apps, kernel, type, sasl)
  end

  defp mandatory_applications([{:sasl, _, type} | apps], kernel, stdlib,
            :undefined) do
  ...
  defp split_app_incl([{app, vsn} | appls], apps, incls) do
    split_app_incl(appls, [{app, vsn, :permanent} | apps],
                     incls)
  end
  ...
  defp split_app_incl([{app, vsn, type, incl} | appls], apps, incls)
      when is_list(incl) do
    split_app_incl(appls, [{app, vsn, type} | apps],
                     [{app, incl} | incls])
  end
  ...
  defp collect_applications(release, path) do
    ...
    x = foldl(fn {name, vsn, type}, {ok, errs} ->
                   case (read_application(to_list(name), vsn, path,
                                            incls)) do
                     {:ok, a} ->
  ...
  def read_application(name, vsn, path, incls) do
    read_application(name, vsn, path, incls, false,
                       :no_fault)
  end

  defp read_application(name, vsn, [dir | path], incls, found,
            firstError) do
    case (read_file(name ++ '.app', [dir])) do
      {:ok, term, fullName} ->
        case (parse_application(term, fullName, vsn, incls)) do
          {:error, {:no_valid_version, {^vsn, otherVsn}}}
            ...
            nFE = {:no_valid_version,
                     {{'should be', vsn},
                        {'found file', :filename.join(dir, name ++ '.app'), otherVsn}}}
            read_application(name, vsn, path, incls, true, nFE)
          {:error, {:no_valid_version, {^vsn, _OtherVsn}}} ->
            read_application(name, vsn, path, incls, true,
                               firstError)
          res ->
            res
        end
      {:error, {:parse, _File, {line, _Mod, err}}}
        ...
        read_application(name, vsn, path, incls, found,
                           {:parse_error,
                              {:filename.join(dir, name ++ '.app'), line, err}})
  ...
  defp parse_application({:application, name, dict}, file, vsn, incls)
    ...
    case ((try do
      ...
      [^vsn, id, desc, mods, regs, apps, opts, incs0, mod,
                                                          phases, env, maxT,
                                                                           maxP] ->
        case (override_include(name, incs0, incls)) do
          {:ok, incs} ->
            {:ok,
               r_application(name: name, vsn: vsn, id: id, description: desc,
                   modules: mods, uses: apps, optional: opts,
                   includes: incs, regs: regs, mod: mod,
                   start_phases: phases, env: env, maxT: maxT, maxP: maxP,
  ...
  defp get_items([h | t], dict) do
    item = (case (check_item(keysearch(h, 1, dict), h)) do
              [atom | _] = atoms when (is_atom(atom) and
                                         is_list(atoms))
                                      ->
                case (atoms !== :lists.uniq(atoms)) do
                  true ->
                    throw({:dupl_entry, h,
                             :lists.subtract(atoms, :lists.uniq(atoms))})
                  false ->
                    atoms
                end
              x ->
  ...
  defp check_item({_, {:optional_applications, apps}}, i) do
    case (a_list_p(apps)) do
      true ->
        apps
      _ ->
  ...
  defp check_item(false, :optional_applications) do
    []
  end
  ...
  defp add_top_apps_to_uses(inclApps, [{name, appl} | appls], res) do
    ...
    f = fn usedApp, accIn when usedApp == myTop ->
             accIn -- [myTop]
           usedApp, accIn ->
    ...
    add_top_apps_to_uses(inclApps, appls,
                           res ++ [{name, r_application(appl, uses: newUses)}])
  end
  ...
  defp undefined_applications(appls) do
    uses = append(map(fn {_, a} ->
                           (r_application(a, :uses) -- r_application(a, :optional)) ++ r_application(a, :includes)
                      end,
                        appls))
    defined = map(fn {{x, _}, _} ->
                       x
                  end,
                    appls)
    filter(fn x ->
  ...
  defp sort_used_and_incl_appls([{tuple, appl} | appls], orderedAppls) do
    incls2 = (case (r_application(appl, :includes)) do
                incls when length(incls) > 1 ->
                  sort_appl_list(incls, orderedAppls)
                incls ->
                  incls
              end)
    uses2 = (case (r_application(appl, :uses)) do
    ...
    [{tuple, appl2} | sort_used_and_incl_appls(appls,
                                                 orderedAppls)]
  end
  ...
  defp find_pos([name | incs], orderedAppls) do
    [find_pos(1, name, orderedAppls) | find_pos(incs,
                                                  orderedAppls)]
  end
  ...
  defp find_pos(n, name,
            [{name, _Vsn, _Type} | _OrderedAppls]) do
    {n, name}
  end
  ...
  defp check_xref(appls, path, xrefP) do
    appDirsL = (for {{app, _Appv}, a} <- appls do
                  {app, r_application(a, :dir)}
                end)
    appDirs0 = :sofs.relation(appDirsL)
    appDirs = (case (xrefP) do
                 true ->
                   appDirs0
                 {true, apps} ->
    ...
    {:ok, _} = :xref.set_default(:systools_make, :verbose,
                                   false)
    libPath = (case (path == :code.get_path()) do
                 true ->
                   :code_path
                 false ->
                   path
  ...
  defp check_xref([{app, appDir} | appls]) do
    case (:xref.add_application(:systools_make, appDir,
                                  {:name, app})) do
      {:ok, _App} ->
        check_xref(appls)
      error ->
  ...
  defp check_xref([]) do
    r = (case (:xref.analyze(:systools_make,
                               :undefined_functions)) do
           {:ok, []} ->
             []
           {:ok, undefined} ->
  ...
  defp add_subdirs([dir | dirs]) do
    case (:filelib.is_dir(dir)) do
      false ->
        add_subdirs(dirs) ++ [dir]
      true ->
        subDirs = (for file <- :filelib.wildcard(:filename.join(dir,
                                                                  '**')),
                         :filelib.is_dir(file) do
                     file
                   end)
  ...
  defp generate_script(output, release, appls, flags) do
    ...
    script = {:script,
                {r_release(release, :name), r_release(release, :vsn)},
                [{:preLoaded, preloaded}, {:progress, :preloaded},
                                              {:path,
                                                 create_mandatory_path(appls,
                                                                         pathFlag,
                                                                         variables)},
                                                  {:primLoad, mandatory},
                                                      {:kernel_load_completed},
                                                          {:progress,
                                                             :kernel_load_completed}] ++ load_appl_mods(appls,
                                                                                                          (mandatory ++ preloaded),
                                                                                                          pathFlag,
                                                                                                          variables) ++ [{:path,
                                                                                                                            create_path(appls,
                                                                                                                                          pathFlag,
                                                                                                                                          variables)}] ++ create_kernel_procs(appls) ++ create_load_appls(appls) ++ create_start_appls(appls) ++ script_end(:lists.member(:no_dot_erlang,
                                                                                                                                                                                                                                                                            flags))}
    scriptFile = output ++ '.script'
    case (:file.open(scriptFile,
      ...
      {:ok, fd} ->
        ...
        case (:file.close(fd)) do
          :ok ->
            ...
            case (:file.write_file(bootFile,
                                     :erlang.term_to_binary(script))) do
              :ok ->
                :ok
              {:error, reason} ->
  ...
  defp path_flag(flags) do
    case ({member(:local, flags),
             member(:otp_build, flags)}) do
      {true, _} ->
        :local
      {_, true} ->
        :otp_build
  ...
  defp create_start_appls([{_, a} | t], incl) do
    ...
    case (:lists.member(app, incl)) do
      ...
      false ->
        [{:apply,
            {:application, :start_boot, [app, r_application(a, :type)]}} |
             create_start_appls(t, incl)]
      _ ->
        create_start_appls(t, incl)
    end
  ...
  defp sort_appls([{n, a} | t], missing, circular, visited) do
    {name, _Vsn} = n
    {uses, t1, notFnd1} = find_all(name,
                                     :lists.reverse(r_application(a, :uses)), t, visited,
                                     [], [])
    {incs, t2, notFnd2} = find_all(name,
                                     :lists.reverse(r_application(a, :includes)), t1,
                                     visited, [], [])
    missing1 = (notFnd1 -- r_application(a, :optional)) ++ notFnd2 ++ missing
    case (uses ++ incs) do
      [] ->
        [{n, a} | sort_appls(t, missing1, circular,
                               [n | visited])]
      l ->
        newCircular = (for {n1, _} <- l, n2 <- visited,
                             n1 == n2 do
                         n1
                       end)
  ...
  defp find_all(checkingApp, [name | t], l, visited, found,
            notFound) do
        ...
        case (:lists.member(checkingApp, r_application(r, :includes))) do
          true ->
            case (:lists.keymember(name, 1, visited)) do
              ...
              false ->
                find_all(checkingApp, t, l, visited, found,
                           [name | notFound])
            end
          false ->
            find_all(checkingApp, t, l -- [app], visited,
                       [app | found], notFound)
        end
      false ->
        case (:lists.keymember(name, 1, visited)) do
  ...
  defp find_all(_CheckingApp, [], l, _Visited, found,
            notFound) do
    {found, l, notFound}
  end
  ...
  defp load_appl_mods([{{name, vsn}, a} | appls], mand, pathFlag,
            variables) do
    mods = r_application(a, :modules)
    load_commands(filter(fn mod ->
                              not member(mod, mand)
                         end,
                           mods),
                    cr_path(name, vsn, a, pathFlag,
                              variables)) ++ load_appl_mods(appls, mand,
                                                              pathFlag,
                                                              variables)
  end
  ...
  def pack_app(r_application(name: name, vsn: v, id: id, description: d,
             modules: m, uses: app, optional: opts, includes: incs,
             regs: regs, mod: mod, start_phases: sF, env: env,
             maxT: maxT, maxP: maxP)) do
    {:application, name,
       [{:description, d}, {:vsn, v}, {:id, id}, {:modules, m},
                                                     {:registered, regs},
                                                         {:applications, app},
                                                             {:optional_applications,
                                                                opts},
  ...
  defp mandatory_modules() do
    [:error_handler, :application, :application_controller,
                                       :application_master, :code, :code_server,
                                                                       :erl_eval,
                                                                           :erl_lint,
                                                                               :erl_parse,
                                                                                   :error_logger,
  ...
  def preloaded() do
    :lists.sort([:prim_net, :prim_socket,
                                :socket_registry] ++ [:atomics, :counters,
                                                                    :erl_init,
                                                                        :erl_prim_loader,
                                                                            :erl_tracer,
                                                                                :erlang,
  ...
  defp kernel_processes() do
    [{:heart, :heart, :start, []}, {:logger, :logger_server,
                                      :start_link, []},
                                       {:application_controller,
                                          :application_controller, :start,
                                          fn appls ->
                                               [{_, app}] = filter(fn {{:kernel,
                                                                          _},
                                                                         _App} ->
                                                                        true
                                                                      _ ->
  ...
  defp create_kernel_procs(appls) do
    map(fn {name, mod, func, args} when is_function(args) ->
             {:kernelProcess, name, {mod, func, args.(appls)}}
           {name, mod, func, args} ->
             {:kernelProcess, name, {mod, func, args}}
        end,
          kernel_processes()) ++ [{:progress,
                                     :init_kernel_started}]
  end

  defp mk_tar(relName, release, appls, flags, path1) do
    tarName = (case (get_outdir(flags)) do
                 '' ->
                   relName ++ '.tar.gz'
                 outDir ->
  ...
  defp add_applications(appls, tar, variables, flags, var) do
    res = foldl(fn {{name, vsn}, app}, errs ->
                     case ((try do
                             add_appl(to_list(name), vsn, app, tar, variables,
                                        flags, var)
                           catch
                             :error, e -> {:EXIT, {e, __STACKTRACE__}}
                             :exit, e -> {:EXIT, e}
  ...
  defp add_variable_tar({variable, p}, appls, tar, flags) do
    case (var_tar_flag(flags)) do
      ...
      flag ->
        ...
        case ((try do
                add_applications(appls, varTar, [{variable, p}], flags,
                                   variable)
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                :exit, e -> {:EXIT, e}
  ...
  defp add_system_files(tar, relName, release, path1) do
    ...
    path = (case (:filename.dirname(relName)) do
              '.' ->
                ['.' | path1]
              relDir ->
                [relDir, '.' | path1]
    ...
    case (lookup_file('start.boot', path)) do
      false ->
        case (lookup_file(relName0 ++ '.boot', path)) do
          false ->
            throw({:error,
                     {:tar_error, {:add, :boot, relName, :enoent}}})
          boot ->
  ...
  defp check_relup(file) do
    case (:file.consult(file)) do
      {:ok, [{vsn, upFrom, downTo}]} when (is_list(vsn) and
      ...
      {:ok, _} ->
        throw({:error,
                 {:tar_error, {:add, 'relup', [:invalid_format]}}})
      other ->
  ...
  defp check_sys_config(file) do
    case (:file.consult(file)) do
      {:ok, [sysConfig]} ->
        case (:lists.all(fn {app, keyVals}
                                when (is_atom(app) and is_list(keyVals)) ->
                              true
                            otherConfig when (is_list(otherConfig) and
          ...
          false ->
            throw({:error,
                     {:tar_error, {:add, 'sys.config', [:invalid_format]}}})
        end
      {:ok, _} ->
        throw({:error,
                 {:tar_error, {:add, 'sys.config', [:invalid_format]}}})
  ...
  defp add_appl(name, vsn, app, tar, variables, flags, var) do
    ...
    case (add_to(appDir, name, vsn, variables, var)) do
      ...
      {:ok, toDir} ->
        ...
        add_to_tar(tar, :filename.join(appDir, name ++ '.app'),
                     :filename.join(binDir, name ++ '.app'))
        add_modules(map(fn mod ->
                             to_list(mod)
                        end,
                          r_application(app, :modules)),
                      tar, appDir, binDir, :code.objfile_extension())
    end
  end
  ...
  defp add_modules(modules, tar, appDir, toDir, ext) do
    foreach(fn mod ->
                 add_to_tar(tar, :filename.join(appDir, mod ++ ext),
  ...
  defp add_erts_bin(tar, release, flags) do
    case ({get_flag(:erts, flags),
             member(:erts_all, flags)}) do
      {{:erts, ertsDir}, true} ->
  ...
  defp add_to_tar(tar, fromFile, toFile) do
    case ((try do
            :erl_tar.add(tar, fromFile, toFile,
  ...
  defp mk_path(path0) do
    path1 = map(fn dir when is_atom(dir) ->
  ...
  defp duplicates([h1, h2 | t], l) do
    case ({:erlang.element(1, h1),
             :erlang.element(1, h2)}) do
      {x, x} ->
  ...
  defp read_file(file, path) do
    case (:file.path_open(path, file, [:read])) do
      {:ok, stream, fullName} ->
        return = (case (:systools_lib.read_term_from_stream(stream,
                                                              file)) do
                    {:ok, term} ->
                      {:ok, term, fullName}
                    other ->
  ...
  def format_error({:mandatory_app, name, type}) do
    :io_lib.format('Mandatory application ~w must be of type \'permanent\' in the release file. Is \'~p\'.~n', [name, type])
  end

  def format_error({:duplicate_register, dups}) do
    :io_lib.format('Duplicated register names: ~n~ts',
                     [map(fn {{reg, app1, _, _}, {reg, app2, _, _}} ->
  ...
  def format_error({:duplicate_include, dups}) do
    :io_lib.format('Duplicated application included: ~n~ts',
                     [map(fn {{name, app1, _, _}, {name, app2, _, _}} ->
  ...
  defp form_err({:error_add_appl,
             {name, {:tar_error, what}}}) do
  ...
  defp form_reading({{:dupl_entry, p, dE}, _}) do
  ...
  defp form_reading({:no_valid_version,
             {{_, sVsn}, {_, file, fVsn}}}) do
    :io_lib.format('No valid version (~tp) of .app file found. Found file ~tp with version ~tp~n', [sVsn, file, fVsn])
  end
  ...
  defp form_warn(prefix,
            {{:parse_error, file}, {_, _, app, _, _}}) do
