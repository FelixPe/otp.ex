defmodule :m_systools_make do
  ...
  defp return(:ok, warnings, flags) do
    case member(:silent, flags) do
      ...
      _ ->
        :io.format(~c"~ts", [format_warning(warnings)])
        :ok
    end
  end
  ...
  defp return({:error, mod, error}, _, flags) do
    case member(:silent, flags) do
      ...
      _ ->
        :io.format(~c"~ts", [mod.format_error(error)])
        :error
    end
  end
  ...
  defp get_regexp_path() do
    {:ok, kernelMP} = :re.compile(~c"kernel-[0-9.]+", [:unicode])
    {:ok, stdlibMP} = :re.compile(~c"stdlib-[0-9.]+", [:unicode])
    {:ok, saslMP} = :re.compile(~c"sasl-[0-9.]+", [:unicode])
  ...
  def read_release(file, path) do
    case read_file(file ++ ~c".rel", [~c"." | path]) do
  ...
  defp read_application(name, vsn, [dir | path], incls, found, firstError) do
    case read_file(name ++ ~c".app", [dir]) do
      {:ok, term, fullName} ->
        case parse_application(term, fullName, vsn, incls) do
          ...
          when firstError == :no_fault ->
            nFE =
              {:no_valid_version,
               {{~c"should be", vsn},
                {~c"found file", :filename.join(dir, name ++ ~c".app"), otherVsn}}}
      ...
      when firstError == :no_fault ->
        read_application(
          ...
          incls,
          found,
          {:parse_error, {:filename.join(dir, name ++ ~c".app"), line, err}}
        )
  ...
  defp get_items([h | t], dict) do
    item =
      case check_item(keysearch(h, 1, dict), h) do
        [atom | _] = atoms
        when is_atom(atom) and
               is_list(atoms) ->
          case atoms !== :lists.uniq(atoms) do
            true ->
              throw({:dupl_entry, h, :lists.subtract(atoms, :lists.uniq(atoms))})
  ...
  defp undefined_applications(appls) do
    uses =
      append(
        map(
          fn {_, a} ->
            (r_application(a, :uses) -- r_application(a, :optional)) ++
  ...
  defp locate_src(mod, [dir | dirs]) do
    file = mod_to_filename(dir, mod, ~c".erl")
  ...
  defp smart_guess(dir, incPath) do
    case reverse(:filename.split(dir)) do
      [~c"ebin" | d] ->
        d1 = reverse(d)
        dirs = [:filename.join(d1 ++ [~c"src"]), :filename.join(d1 ++ [~c"src", ~c"e_src"])]
  ...
  defp cr_path(name, vsn, _, true, []) do
    :filename.join([~c"$ROOT", ~c"lib", to_list(name) ++ ~c"-" ++ vsn, ~c"ebin"])
  end
  ...
  defp cr_path(name, vsn, app, true, variables) do
    ...
    case variable_dir(dir, n, vsn, variables) do
      ...
      _ ->
        :filename.join([~c"$ROOT", ~c"lib"] ++ tail)
    end
  end

  defp cr_path(name, _, _, :otp_build, _) do
    :filename.join([~c"$ROOT", ~c"lib", to_list(name), ~c"ebin"])
  end
    ...
    case :lists.prefix(path, dir) do
      true ->
        ...
        case strip_name_ebin(d0, name, vsn) do
          {:ok, d} ->
            {:ok, :filename.join([~c"$" ++ var] ++ d)}

          _ ->
            {:ok, :filename.join([~c"$" ++ var] ++ d0)}
        end
  ...
  defp erts_binary_filter() do
    cmds = [~c"typer", ~c"dialyzer", ~c"ct_run", ~c"yielding_c_fun", ~c"erlc"]
  ...
  defp mk_tar(relName, release, appls, flags, path1) do
    tarName =
      case get_outdir(flags) do
        ...
        outDir ->
          :filename.join(outDir, :filename.basename(relName)) ++ ~c".tar.gz"
      end
  ...
  defp add_system_files(tar, relName, release, path1) do
    ...
    relVsnDir = :filename.join(~c"releases", sVsn)
    add_to_tar(tar, relName ++ ~c".rel", :filename.join(~c"releases", relName0 ++ ~c".rel"))
    add_to_tar(tar, relName ++ ~c".rel", :filename.join(relVsnDir, relName0 ++ ~c".rel"))
    ...
    case lookup_file(~c"start.boot", path) do
      false ->
        case lookup_file(relName0 ++ ~c".boot", path) do
          false ->
            throw({:error, {:tar_error, {:add, :boot, relName, :enoent}}})

          boot ->
            add_to_tar(tar, boot, :filename.join(relVsnDir, ~c"start.boot"))
        end
    ...
    case lookup_file(~c"relup", path) do
      false ->
        :ignore

      relup ->
        check_relup(relup)
        add_to_tar(tar, relup, :filename.join(relVsnDir, ~c"relup"))
    end

    case lookup_file(~c"sys.config.src", path) do
      false ->
        case lookup_file(~c"sys.config", path) do
          false ->
            :ignore

          sys ->
            check_sys_config(sys)
            add_to_tar(tar, sys, :filename.join(relVsnDir, ~c"sys.config"))
        end

      sysSrc ->
        add_to_tar(tar, sysSrc, :filename.join(relVsnDir, ~c"sys.config.src"))
    end
  ...
  defp check_relup(file) do
    case :file.consult(file) do
      ...
      {:ok, _} ->
        throw({:error, {:tar_error, {:add, ~c"relup", [:invalid_format]}}})

      other ->
        throw({:error, {:tar_error, {:add, ~c"relup", [other]}}})
    end
  end
    ...
    case :file.consult(file) do
      {:ok, [sysConfig]} ->
        case :lists.all(
          ...
          false ->
            throw({:error, {:tar_error, {:add, ~c"sys.config", [:invalid_format]}}})
        end
      ...
      other ->
        throw({:error, {:tar_error, {:add, ~c"sys.config", [other]}}})
    end
  end
  ...
  defp add_appl(name, vsn, app, tar, variables, flags, var) do
    ...
    case add_to(appDir, name, vsn, variables, var) do
      ...
      {:ok, toDir} ->
        ...
        binDir = :filename.join(toDir, ~c"ebin")

        add_to_tar(
          tar,
          :filename.join(appDir, name ++ ~c".app"),
          :filename.join(binDir, name ++ ~c".app")
        )
  ...
  defp add_to(appDir, name, vsn, variables, variable) do
    case var_dir(appDir, name, vsn, variables) do
      {:ok, ^variable, restPath} ->
        {:ok, :filename.join(restPath ++ [name ++ ~c"-" ++ vsn])}

      {:ok, _, _} ->
        false

      _ when variable == false ->
        {:ok, :filename.join(~c"lib", name ++ ~c"-" ++ vsn)}
  ...
  defp add_priv(aDir, toDir, tar) do
    priv = :filename.join(aDir, ~c"priv")

    case dirp(priv) do
      true ->
        add_to_tar(tar, priv, :filename.join(toDir, ~c"priv"))
  ...
  defp add_erts_bin(tar, release, ertsDir, filters) do
    ...
    fromDir = :filename.join([to_list(ertsDir), ~c"erts-" ++ eVsn, ~c"bin"])
    toDir = :filename.join(~c"erts-" ++ eVsn, ~c"bin")
  ...
  defp create_include_path(appls, path) do
    ...
    map(
      fn dir ->
        case reverse(:filename.split(dir)) do
          [~c"ebin" | d] ->
            :filename.join(reverse(d) ++ [~c"include"])

          _ ->
            :filename.join(dir, ~c"include")
        end
      end,
  ...
  def format_error(:badly_formatted_release) do
    :io_lib.format(~c"Syntax error in the release file~n", [])
  end

  def format_error({:illegal_name, name}) do
    :io_lib.format(~c"Illegal name (~tp) in the release file~n", [name])
  end

  def format_error({:illegal_form, form}) do
    :io_lib.format(~c"Illegal tag in the release file: ~tp~n", [form])
  end

  def format_error({:missing_parameter, par}) do
    :io_lib.format(~c"Missing parameter (~p) in the release file~n", [par])
  end

  def format_error({:illegal_applications, names}) do
    :io_lib.format(~c"Illegal applications in the release file: ~p~n", [names])
  end

  def format_error({:missing_mandatory_app, name}) do
    :io_lib.format(~c"Mandatory application ~w must be specified in the release file~n", [name])
  end

  def format_error({:mandatory_app, name, type}) do
    :io_lib.format(
      ~c"Mandatory application ~w must be of type 'permanent' in the release file. Is '~p'.~n",
      [name, type]
    )
  end
  ...
  def format_error({:duplicate_register, dups}) do
    :io_lib.format(
      ~c"Duplicated register names: ~n~ts",
      [
        map(
          fn {{reg, app1, _, _}, {reg, app2, _, _}} ->
            :io_lib.format(~c"\t~tw registered in ~w and ~w~n", [reg, app1, app2])
          end,
          dups
        )
      ]
  ...
  def format_error({:undefined_applications, apps}) do
    :io_lib.format(~c"Undefined applications: ~p~n", [apps])
  end

  def format_error({:duplicate_modules, dups}) do
    :io_lib.format(
      ~c"Duplicated modules: ~n~ts",
      [
        map(
          fn {{mod, app1, _}, {mod, app2, _}} ->
            :io_lib.format(~c"\t~w specified in ~w and ~w~n", [mod, app1, app2])
          end,
          dups
        )
      ]
  ...
  def format_error({:included_and_used, dups}) do
    :io_lib.format(~c"Applications both used and included: ~p~n", [dups])
  end

  def format_error({:duplicate_include, dups}) do
    :io_lib.format(
      ~c"Duplicated application included: ~n~ts",
      [
        map(
          fn {{name, app1, _, _}, {name, app2, _, _}} ->
            :io_lib.format(~c"\t~w included in ~w and ~w~n", [name, app1, app2])
          end,
          dups
        )
      ]
  ...
  def format_error({:circular_dependencies, apps}) do
    :io_lib.format(~c"Circular dependencies among applications: ~p~n", [apps])
  end

  def format_error({:not_found, file}) do
    :io_lib.format(~c"File not found: ~tp~n", [file])
  end

  def format_error({:parse, file, {line, mod, what}}) do
    str = mod.format_error(what)
    :io_lib.format(~c"~ts:~w: ~ts\n", [file, line, str])
  end

  def format_error({:read, file}) do
    :io_lib.format(~c"Cannot read ~tp~n", [file])
  end

  def format_error({:open, file, error}) do
    :io_lib.format(~c"Cannot open ~tp - ~ts~n", [file, :file.format_error(error)])
  end

  def format_error({:close, file, error}) do
    :io_lib.format(~c"Cannot close ~tp - ~ts~n", [file, :file.format_error(error)])
  end

  def format_error({:delete, file, error}) do
    :io_lib.format(~c"Cannot delete ~tp - ~ts~n", [file, :file.format_error(error)])
  end
  ...
  def format_error({:warnings_treated_as_errors, warnings}) do
    :io_lib.format(
      ~c"Warnings being treated as errors:~n~ts",
      [
        map(
          fn w ->
            form_warn(~c"", w)
  ...
  defp form_err({:bad_application_name, {name, found}}) do
    :io_lib.format(~c"~p: Mismatched application id: ~p~n", [name, found])
  end

  defp form_err({:error_reading, {name, what}}) do
    :io_lib.format(~c"~p: ~ts~n", [name, form_reading(what)])
  end

  defp form_err({:module_not_found, app, mod}) do
    :io_lib.format(~c"~w: Module (~w) not found~n", [app, mod])
  end

  defp form_err({:error_add_appl, {name, {:tar_error, what}}}) do
    :io_lib.format(~c"~p: ~ts~n", [name, form_tar_err(what)])
  end
  ...
  defp form_reading({:application_vsn, {name, vsn}}) do
    :io_lib.format(~c"Application ~ts with version ~tp not found~n", [name, vsn])
  end
  ...
  defp form_reading({{:bad_param, p}, _}) do
    :io_lib.format(~c"Bad parameter in .app file: ~tp~n", [p])
  end

  defp form_reading({{:dupl_entry, p, dE}, _}) do
    :io_lib.format(~c"~tp parameter contains duplicates of: ~tp~n", [p, dE])
  end

  defp form_reading({{:missing_param, p}, _}) do
    :io_lib.format(~c"Missing parameter in .app file: ~p~n", [p])
  end

  defp form_reading({:badly_formatted_application, _}) do
    :io_lib.format(~c"Syntax error in .app file~n", [])
  end

  defp form_reading({:override_include, apps}) do
    :io_lib.format(~c"Tried to include not (in .app file) specified applications: ~p~n", [apps])
  end

  defp form_reading({:no_valid_version, {{_, sVsn}, {_, file, fVsn}}}) do
    :io_lib.format(
      ~c"No valid version (~tp) of .app file found. Found file ~tp with version ~tp~n",
      [sVsn, file, fVsn]
    )
  end

  defp form_reading({:parse_error, {file, line, error}}) do
    :io_lib.format(~c"Parse error in file: ~tp.  Line: ~w  Error: ~tp; ~n", [file, line, error])
  end

  defp form_reading(w) do
    :io_lib.format(~c"~tp~n", [w])
  end

  defp form_tar_err({:open, file, error}) do
    :io_lib.format(~c"Cannot open tar file ~ts - ~ts~n", [file, :erl_tar.format_error(error)])
  end

  defp form_tar_err({:add, :boot, relName, :enoent}) do
    :io_lib.format(
      ~c"Cannot find file start.boot or ~ts to add to tar file - ~ts~n",
      [relName, :erl_tar.format_error(:enoent)]
    )
  end

  defp form_tar_err({:add, file, error}) do
    :io_lib.format(~c"Cannot add file ~ts to tar file - ~ts~n", [
      file,
  ...
  defp form_warn(prefix, {:source_not_found, {mod, app, _}}) do
    :io_lib.format(~c"~ts~w: Source code not found: ~w.erl~n", [prefix, app, mod])
  end
  ...
  defp form_warn(
    ...
    :io_lib.format(~c"~ts~w: Parse error: ~tp~n", [prefix, app, file])
  end

  defp form_warn(prefix, {:obj_out_of_date, {mod, app, _}}) do
    :io_lib.format(~c"~ts~w: Object code (~w) out of date~n", [prefix, app, mod])
  end

  defp form_warn(prefix, {:exref_undef, undef}) do
    f = fn {m, f, a} ->
      :io_lib.format(~c"~tsUndefined function ~w:~tw/~w~n", [prefix, m, f, a])
    end
  ...
  defp form_warn(prefix, :missing_sasl) do
    :io_lib.format(~c"~tsMissing application sasl. Can not upgrade with this release~n", [prefix])
  end

  defp form_warn(prefix, what) do
    :io_lib.format(~c"~ts~tp~n", [prefix, what])
  end
end
