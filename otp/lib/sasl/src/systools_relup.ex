defmodule :m_systools_relup do
  ...
  defp get_script_from_appup(mode, topApp, baseVsn, ws, rUs) do
    fName =
      :filename.join([
        ...
        to_list(r_application(topApp, :name)) ++ ~c".appup"
      ])
  ...
  defp write_relup_file(relup, opts) do
    ...
    case :file.open(
      ...
      {:ok, fd} ->
        :io.format(fd, ~c"%% ~s~n~tp.~n", [:epp.encoding_to_string(:utf8), relup])
        ...
        case :file.close(fd) do
          ...
          {:error, reason} ->
            {:error, :systools_relup, {:file_problem, {~c"relup", {:close, reason}}}}
        end

      {:error, reason} ->
        {:error, :systools_relup, {:file_problem, {~c"relup", {:open, reason}}}}
    end
  end
  ...
  defp print_error(other) do
    :io.format(~c"Error: ~tp~n", [other])
  end

  def format_error({:file_problem, {file, what}}) do
    :io_lib.format(~c"Could not ~w file ~ts~n", [get_reason(what), file])
  end

  def format_error({:no_relup, file, app, vsn}) do
    :io_lib.format(
      ~c"No release upgrade script entry for ~w-~ts to ~w-~ts in file ~ts~n",
      [r_application(app, :name), r_application(app, :vsn), r_application(app, :name), vsn, file]
    )
  end

  def format_error({:missing_sasl, release}) do
    :io_lib.format(~c"No sasl application in release ~ts, ~ts. Can not be upgraded.", [
  ...
  defp print_warning(w) do
    :io.format(~c"~ts", [format_warning(w)])
  end

  def format_warning(w) do
    format_warning(~c"*WARNING* ", w)
  end

  defp format_warning(prefix, {:erts_vsn_changed, {rel1, rel2}}) do
    :io_lib.format(~c"~tsThe ERTS version changed between ~tp and ~tp~n", [prefix, rel1, rel2])
  end

  defp format_warning(prefix, :pre_R15_emulator_upgrade) do
    :io_lib.format(
      ~c"~tsUpgrade from an OTP version earlier than R15. New code should be compiled with the old emulator.~n",
      [prefix]
    )
  end
