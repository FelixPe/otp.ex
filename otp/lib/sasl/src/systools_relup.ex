defmodule :m_systools_relup do
  ...
  def mk_relup(topRelFile, baseUpRelDcs, baseDnRelDcs, opts) do
    case (check_opts(opts)) do
      [] ->
        r = (try do
               do_mk_relup(topRelFile, baseUpRelDcs, baseDnRelDcs,
  ...
  defp do_mk_relup(topRelFile, baseUpRelDcs, baseDnRelDcs, path,
            opts) do
    case (:systools_make.get_release(to_list(topRelFile),
                                       path)) do
      {:ok, topRel, nameVsnApps, ws0} ->
        case (:lists.member({:warning, :missing_sasl}, ws0)) do
          true ->
            throw({:error, :systools_relup,
                     {:missing_sasl, topRel}})
          false ->
            :ok
        end
        topApps = :lists.map(fn {_, app} ->
                                  app
                             end,
                               nameVsnApps)
        {up, ws1} = foreach_baserel_up(topRel, topApps,
                                         baseUpRelDcs, path, opts, ws0)
        {dn, ws2} = foreach_baserel_dn(topRel, topApps,
                                         baseDnRelDcs, path, opts, ws1)
  ...
  defp done_mk_relup(opts, error) do
    return_error(get_opt(:silent, opts) or get_opt(:noexec,
                                                     opts),
                   error)
  end
  ...
  defp foreach_baserel_up(topRel, topApps, [baseRelDc | baseRelDcs], path,
            opts, ws, acc) do
    baseRelFile = extract_filename(baseRelDc)
    {baseRel, {baseNameVsns, baseApps},
       ws0} = (case (:systools_make.get_release(baseRelFile,
                                                  path)) do
                 {:ok, bR, nameVsnApps, warns} ->
                   case (:lists.member({:warning, :missing_sasl},
                                         warns)) do
                     true ->
    ...
    {rUs1, ws1} = collect_appup_scripts(:up, topApps,
                                          baseRel, ws0 ++ ws, [])
    {rUs2, ws2} = prepend_add_app_scripts(baseRel, topRel,
                                            rUs1, ws1)
    {rUs3, ws3} = append_remove_app_scripts(baseRel, topRel,
                                              rUs2, ws2)
    {rUs4, ws4} = check_for_emulator_restart(topRel,
                                               baseRel, rUs3, ws3, opts)
    case (:systools_rc.translate_scripts(:up, rUs4, topApps,
                                           baseApps)) do
      {:ok, rUs5} ->
        {rUs, ws5} = fix_r15_sasl_upgrade(rUs5, ws4,
                                            baseNameVsns)
        vDR = {r_release(baseRel, :vsn), extract_description(baseRelDc),
                 rUs}
        foreach_baserel_up(topRel, topApps, baseRelDcs, path,
                             opts, ws5, [vDR | acc])
      xXX ->
  ...
  defp foreach_baserel_dn(topRel, topApps, [baseRelDc | baseRelDcs], path,
            opts, ws, acc) do
    baseRelFile = extract_filename(baseRelDc)
    {baseRel, baseApps,
       ws0} = (case (:systools_make.get_release(baseRelFile,
    ...
    {rUs1, ws1} = collect_appup_scripts(:dn, topApps,
                                          baseRel, ws0 ++ ws, [])
    {rUs2, ws2} = prepend_add_app_scripts(topRel, baseRel,
                                            rUs1, ws1)
    {rUs3, ws3} = append_remove_app_scripts(topRel, baseRel,
                                              rUs2, ws2)
    {rUs4, ws4} = check_for_emulator_restart(topRel,
                                               baseRel, rUs3, ws3, opts)
    case (:systools_rc.translate_scripts(:dn, rUs4,
                                           baseApps, topApps)) do
      {:ok, rUs} ->
        vDR = {r_release(baseRel, :vsn), extract_description(baseRelDc),
                 rUs}
  ...
  defp check_for_emulator_restart(r_release(erts_vsn: vsn1, name: n1),
            r_release(erts_vsn: vsn2, name: n2), rUs, ws, opts)
      when vsn1 != vsn2 do
    newRUs = [[:restart_new_emulator] | rUs]
    newWs = [{:erts_vsn_changed, {{n1, vsn1}, {n2, vsn2}}} |
                 ws]
  ...
  defp fix_r15_sasl_upgrade([:restart_new_emulator | restRUs] = rUs, ws,
            baseApps) do
    case (:lists.keyfind(:sasl, 1, baseApps)) do
      {:sasl, vsn} when vsn < '2.2' ->
        {:lists.delete(:restart_emulator,
                         restRUs) ++ [:restart_new_emulator],
  ...
  defp collect_appup_scripts(mode, [topApp | topApps], baseRel, ws, rUs) do
    case (:lists.keysearch(r_application(topApp, :name), 1,
                             r_release(baseRel, :applications))) do
      {:value, {_Name, baseVsn, _Type}} ->
        cond do
          r_application(topApp, :vsn) == baseVsn ->
            collect_appup_scripts(mode, topApps, baseRel, ws, rUs)
          true ->
            {rU1s, ws1} = get_script_from_appup(mode, topApp,
                                                  baseVsn, ws, rUs)
  ...
  defp prepend_add_app_scripts(fromRel, toRel, rU0s, w0s) do
    addedNs = (for {n, _V, t} <- r_release(toRel, :applications),
                     not :lists.keymember(n, 1, r_release(fromRel, :applications)) do
                 {n, t}
               end)
    rUs = (for {n, t} <- addedNs do
             [{:add_application, n, t}]
           end)
    {rUs ++ rU0s, w0s}
  end

  defp append_remove_app_scripts(fromRel, toRel, rU0s, w0s) do
    removedNs = (for {n, _V,
                        _T} <- r_release(fromRel, :applications),
                       not :lists.keymember(n, 1, r_release(toRel, :applications)) do
                   n
                 end)
    rUs = (for n <- removedNs do
  ...
  defp get_script_from_appup(mode, topApp, baseVsn, ws, rUs) do
    fName = :filename.join([r_application(topApp, :dir),
                                to_list(r_application(topApp, :name)) ++ '.appup'])
    {vsnRUs,
       topVsn} = (case (:systools_lib.read_term(fName)) do
                    {:ok, {topVsn0, upVsnRUs, dnVsnRUs}} ->
                      vsnRUs0 = (case (mode) do
                                   :up ->
                                     upVsnRUs
                                   :dn ->
                                     dnVsnRUs
  ...
  def appup_search_for_version(baseVsn, [{vsn, rU} | vsnRUs])
    ...
    case (:re.run(baseVsn, vsn,
                    [:unicode, {:capture, :first, :list}])) do
      {:match, [^baseVsn]} ->
        {:ok, rU}
      _ ->
  ...
  defp write_relup_file(relup, opts) do
    filename = :filename.join(:filename.absname(get_opt(:outdir,
                                                          opts)),
                                'relup')
    case (:file.open(filename,
                       [:write, {:encoding, :utf8}])) do
      {:ok, fd} ->
        :io.format(fd, '%% ~s~n~tp.~n',
                     [:epp.encoding_to_string(:utf8), relup])
  ...
  def format_error({:no_relup, file, app, vsn}) do
    :io_lib.format('No release upgrade script entry for ~w-~ts to ~w-~ts in file ~ts~n',
                     [r_application(app, :name), r_application(app, :vsn), r_application(app, :name), vsn, file])
  end

  def format_error({:missing_sasl, release}) do
    :io_lib.format('No sasl application in release ~ts, ~ts. Can not be upgraded.', [r_release(release, :name), r_release(release, :vsn)])
  end
