defmodule :m_rb do
  ...
  def help() do
    :io.format(~c"~nReport Browser Tool - usage~n")
    :io.format(~c"===========================~n")
    :io.format(~c"rb:start()         - start the rb_server with default options~n")
    :io.format(~c"rb:start(Options)  - where Options is a list of:~n")
    print_options()
    :io.format(~c"rb:h()             - print this help~n")
    :io.format(~c"rb:help()          - print this help~n")
    :io.format(~c"rb:list()          - list all reports~n")
    :io.format(~c"rb:list(Type)      - list all reports of type Type~n")
    :io.format(~c"rb:log_list()      - log list of all reports~n")
    :io.format(~c"rb:log_list(Type)  - log list of all reports of type Type~n")
    :io.format(~c"      currently supported types are:~n")
    print_types()
    :io.format(~c"rb:grep(RegExp)      - print reports containing RegExp.~n")
    :io.format(~c"                     RegExp must be a valid argument for ~n")
    :io.format(~c"                     the function re:run/2 or re:run/3.~n")
    :io.format(~c"rb:filter(Filters) - print reports matching Filters.~n")
    :io.format(~c"                     reports must be proplists.~n")
    :io.format(~c"      Filters is a list of tuples of the following form:~n")
    print_filters()
    :io.format(~c"rb:filter(Filters, Dates)  -~n")
    :io.format(~c"      same as rb:filter/1 but accepts date ranges to filter reports.~n")
    :io.format(~c"      Dates must be of the following form:~n")
    print_dates()
    :io.format(~c"rb:rescan()        - rescans the report directory with same~n")
    :io.format(~c"                     options.~n")
    :io.format(~c"rb:rescan(Options) - rescans the report directory with new~n")
    :io.format(~c"                     options. Options is same as in start/1.~n")
    :io.format(~c"rb:show(Number)    - print report no Number~n")
    :io.format(~c"rb:show(Type)      - print all reports of type Type~n")
    :io.format(~c"rb:show()          - print all reports~n")
    :io.format(~c"rb:start_log(File) - redirect all reports to file or io_device~n")
    :io.format(~c"rb:stop_log()      - close the log file and redirect to~n")
    :io.format(~c"                     standard_io~n")
    :io.format(~c"rb:stop            - stop the rb_server~n")
  end
  ...
  defp print_options() do
    :io.format(~c"      {start_log, FileName}~n")
    :io.format(~c"         - default: standard_io~n")
    :io.format(~c"      {max, MaxNoOfReports}~n")
    :io.format(~c"         - MaxNoOfReports should be an integer or 'all'~n")
    :io.format(~c"         - default: all~n")
    :io.format(~c"      {report_dir, DirString}~n")
    :io.format(~c"         - DirString should be a string without trailing~n")
    :io.format(~c"         - directory delimiter.~n")
    :io.format(~c"         - default: {sasl, error_logger_mf_dir}~n")
    :io.format(~c"      {type, ReportType}~n")
    :io.format(~c"         - ReportType should be a supported type, 'all'~n")
    :io.format(~c"         - or a list of supported types~n")
    :io.format(~c"         - default: all~n")
    :io.format(~c"      {abort_on_error, Bool}~n")
    :io.format(~c"         - Bool: true | false~n")
    :io.format(~c"         - default: false~n")
  end

  defp print_types() do
    :io.format(~c"         - crash_report~n")
    :io.format(~c"         - supervisor_report~n")
    :io.format(~c"         - progress~n")
    :io.format(~c"         - error~n")
  end

  defp print_filters() do
    :io.format(~c"      - {Key, Value}~n")
    :io.format(~c"        includes report containing {Key, Value}~n")
    :io.format(~c"      - {Key, Value, no}~n")
    :io.format(~c"        excludes report containing {Key, Value}~n")
    :io.format(~c"      - {Key, RegExp, re}~n")
    :io.format(~c"        RegExp must be a valid argument for ~n")
    :io.format(~c"        the function re:run/2 or re:run/3.~n")
    :io.format(~c"      - {Key, RegExp, re, no}~n")
    :io.format(~c"        excludes report containing {Key, RegExp}~n")
  end

  defp print_dates() do
    :io.format(~c"      - {StartDate, EndDate}~n")
    :io.format(~c"        StartDate = EndDate = {{Y,M,D},{H,M,S}} ~n")
    :io.format(~c"        prints the reports with date between StartDate and EndDate~n")
    :io.format(~c"      - {StartDate, from}~n")
    :io.format(~c"        prints the reports with date greater than StartDate~n")
    :io.format(~c"      - {EndDate, to}~n")
    :io.format(~c"        prints the reports with date lesser than StartDate~n")
  end
  ...
  def init(options) do
    ...
    data = scan_files(dir ++ ~c"/", max, type)
  ...
  defp open_log_file(fd)
    ...
    case :erlang.whereis(fd) do
      :undefined ->
        :io.format(~c"rb: Registered name not found '~ts'.~n", [fd])
        :io.format(~c"rb: Using standard_io~n")
  ...
  defp open_log_file(fileName) when is_list(fileName) do
    case :file.open(
      ...
      error ->
        :io.format(~c"rb: Cannot open file '~ts' (~w).~n", [fileName, error])
        :io.format(~c"rb: Using standard_io~n")
        :standard_io
    end
  end
    ...
    case :lists.keysearch(:report_dir, 1, options) do
      ...
      _ ->
        case (try do
          ...
          _ ->
            exit(~c"cannot locate report directory")
        end
    end
  end

  defp scan_files(rptDir, max, type) do
    case :file.open(rptDir ++ ~c"/index", [:raw, :read]) do
      {:ok, fd} ->
        case (try do
                :file.read(fd, 1)
              catch
          ...
          _X ->
            _ = :file.close(fd)
            exit(~c"cannot read the index file")
        end
  ...
  defp get_report_data_from_file(dir, no, fileNr, max, type) do
    ...
    case :file.open(fileName, [:read]) do
      ...
      _ ->
        [{no, :unknown, ~c"Can't open file " ++ fname, ~c"???", fname, 0}]
    end
  end

  defp read_reports(no, fd, fname, max, type) do
    :io.format(~c"rb: reading report...")
    ...
    case (try do
      ...
      {:error, [problem | res]} ->
        _ = :file.close(fd)
        :io.format(~c"Error: ~tp~n", [problem])

        :io.format(~c"Salvaged ~p entries from corrupt report file ~ts...~n", [length(res), fname])
      ...
      else__ ->
        :io.format(~c"err ~tp~n", [else__])
        [{no, :unknown, ~c"Can't read reports from file " ++ fname, ~c"???", fname, 0}]
    end
  end
        ...
        case :io.get_chars(fd, :"", size) do
          ...
          list ->
            ...
            case (try do
              ...
              {:EXIT, _} ->
                {:error, ~c"Incomplete erlang term in log"}
  ...
  defp date_str({y, mo, d} = date, {h, mi, s} = time) do
    case :application.get_env(:sasl, :utc_log) do
      {:ok, true} ->
        ...
        :lists.flatten(
          :io_lib.format(
            ~c"~w-~2.2.0w-~2.2.0w ~2.2.0w:~2.2.0w:~2.2.0w UTC",
      ...
      _ ->
        :lists.flatten(
          :io_lib.format(~c"~w-~2.2.0w-~2.2.0w ~2.2.0w:~2.2.0w:~2.2.0w", [y, mo, d, h, mi, s])
        )
    end
  end
  ...
  defp print_list(fd, data0, type) do
    ...
    header = {~c"No", ~c"Type", ~c"Process", ~c"Date     Time"}
    {descrWidth, dateWidth, data} = find_widths(data0, modifier, 7, 13, [])
    format = :lists.concat([~c"~4s~20s ~", descrWidth, ~c"s~20s~n"])
    :io.format(fd, format, :erlang.tuple_to_list(header))
    :io.format(fd, format, [~c"==", ~c"====", ~c"=======", ~c"====     ===="])
  ...
  defp find_widths([h | t], modifier, descrWidth, dateWidth, data) do
    ...
    descr = :io_lib.format(~c"~" ++ modifier ++ ~c"w", [descrTerm])
  ...
  defp print_short_descr(fd, no, type, shortDescr, date, width, dateWidth, modifier) do
    format =
      :lists.concat([~c"~4w~20", modifier, ~c"w ~", width, modifier, ~c"s~", dateWidth, ~c"s~n"])
    ...
    case find_report(data, number) do
      {fname, filePosition} ->
        ...
        case :file.open(fileName, [:read]) do
          ...
          _ ->
            :io.format(~c"rb: can't open file ~tp~n", [fname])
            {:proceed, device}
        end
  ...
  defp find_report([], no) do
    :io.format(~c"There is no report with number ~p.~n", [no])
    :no_report
  end
  ...
  defp check_rep(fd, filePosition, device, regExp, number, abort, log) do
    case read_rep_msg(fd, filePosition) do
      {date, msg} ->
        msgStr = :lists.flatten(:io_lib.format(~c"~tp", [msg]))

        case run_re(msgStr, regExp) do
          :match ->
            :io.format(~c"Found match in report number ~w~n", [number])
      ...
      _ ->
        :io.format(~c"rb: Cannot read from file~n")
        {:proceed, device}
    end
  end
  ...
  defp filter_report([{key, regExp, :re} | t], msg) do
    case :proplists.get_value(key, msg) do
      ...
      value ->
        subject = :lists.flatten(:io_lib.format(~c"~tp", [value]))
  ...
  defp handle_bad_form(date, msg, device, abort, log) do
    :io.format(
      ~c"rb: ERROR! A report on bad form was encountered. " ++
        ~c"It cannot be printed to the log.~n~n"
    )

    :io.format(~c"Details:~n~p ~tp~n~n", [date, msg])

    case {abort, device, open_log_file(log)} do
      {true, :standard_io, :standard_io} ->
        :io.format(~c"rb: Logging aborted.~n")
        {:abort, device}

      {false, :standard_io, :standard_io} ->
        :io.format(~c"rb: Logging resumed...~n~n")
        {:proceed, device}

      {_, _, :standard_io} ->
        :io.format(~c"rb: Can not reopen ~tp. Logging aborted.~n", [log])
        {:abort, device}
      ...
      {true, _, newDevice} ->
        :io.format(
          newDevice,
          ~c"~n~n************************* RB ERROR ************************~n" ++
            ~c"A report on bad form was encountered here and the logging~n" ++
            ~c"process was aborted. Note that there may well be remaining~n" ++
            ~c"reports that haven't yet been logged. Please see the rb~n" ++
            ~c"manual for more info.~n" ++
            ~c"***********************************************************~n",
          []
        )
      ...
      {false, _, newDevice} ->
        :io.format(newDevice, ~c"~n   ********* RB: UNPRINTABLE REPORT ********~n~n", [])
        :io.format(~c"rb: Logging resumed...~n~n")
        {:proceed, newDevice}
    end
  end
