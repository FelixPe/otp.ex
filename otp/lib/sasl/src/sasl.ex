defmodule :m_sasl do
  ...
  require Record
  Record.defrecord(:r_state, :state, sasl_logger: :undefined,
  ...
  def start(_, []) do
    ...
    case (:supervisor.start_link({:local, :sasl_sup}, :sasl,
                                   [])) do
      {:ok, pid} ->
        {:ok, pid, state}
      error ->
  ...
  defp get_logger_dest() do
    case (:application.get_env(:sasl,
                                 :sasl_error_logger)) do
      {:ok, false} ->
        :undefined
      {:ok, :tty} ->
        :standard_io
      {:ok, {:file, file}} when is_list(file) ->
        {:file, file}
      {:ok, {:file, file, modes}} when (is_list(file) and
                                          is_list(modes))
                                       ->
        {:file, file, modes}
      {:ok, bad} ->
  ...
  defp get_mf_dir() do
    case (:application.get_env(:sasl,
      ...
      {:ok, bad} ->
        exit({:bad_config,
                {:sasl, {:error_logger_mf_dir, bad}}})
    end
  end

  defp get_mf_maxb() do
    case (:application.get_env(:sasl,
                                 :error_logger_mf_maxbytes)) do
      ...
      {:ok, bad} ->
        exit({:bad_config,
                {:sasl, {:error_logger_mf_maxbytes, bad}}})
    end
  end

  defp get_mf_maxf() do
    case (:application.get_env(:sasl,
                                 :error_logger_mf_maxfiles)) do
      {:ok, maxF} when (is_integer(maxF) and maxF > 0 and
                          maxF < 256)
                       ->
        maxF
      :undefined ->
      ...
      {:ok, bad} ->
        exit({:bad_config,
                {:sasl, {:error_logger_mf_maxfiles, bad}}})
    end
  end
  ...
  defp add_sasl_logger(dest, level) do
    ...
    :ok = :logger.add_handler(:sasl, :logger_std_h,
                                %{level: level, filter_default: :stop,
                                    filters:
                                    [{:remote_gl,
                                        {&:logger_filters.remote_gl/2, :stop}},
                                         {:sasl_domain,
  ...
  defp add_error_logger_mf({dir, maxB, maxF}) do
    allow_progress()
    :error_logger.add_report_handler(:log_mf_h,
  ...
  def init([]) do
    ...
    releaseH = {:release_handler,
                  {:release_handler, :start_link, []}, :permanent, 2000,
                  :worker, []}
  ...
  def init(:safe) do
    ...
    alarmH = {:alarm_handler,
                {:alarm_handler, :start_link, []}, :permanent, 2000,
                :worker, :dynamic}
    {:ok, {supFlags, [alarmH]}}
  end
