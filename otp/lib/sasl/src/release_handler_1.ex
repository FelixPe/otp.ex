defmodule :m_release_handler_1 do
  use Bitwise
  require Record
  Record.defrecord(:r_eval_state, :eval_state, bins: [], stopped: [],
                                      suspended: [], apps: [],
                                      libdirs: :undefined, unpurged: [],
                                      vsns: [], newlibs: [], opts: [])
  ...
  defp do_check_script(script, libDirs, purgeMods) do
    ...
    case ((try do
            :lists.foldl(fn instruction, evalState1 ->
                              eval(instruction, evalState1)
                         end,
                           r_eval_state(libdirs: libDirs), before)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
            e -> e
      ...
      {:ok, _} ->
        ...
        case ((try do
                :lists.foldl(fn instruction, evalState1 ->
                               ...
                               r_eval_state(apps: apps, libdirs: libDirs, newlibs: newLibs,
                                   opts: opts),
                               before)
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
          ...
          evalState2 when elem(evalState2, 0) === :eval_state ->
            case ((try do
              ...
              :ok ->
                case ((try do
                        :lists.foldl(fn instruction, evalState3 ->
  ...
  def check_old_processes(script, prePurgeMethod) do
    ...
    {:ok,
       :lists.flatmap(fn {:load, {mod, pPM, _PostPurgeMethod}}
  ...
  defp syntax_check_script([illegal | _Script]) do
    throw({:illegal_instruction_after_point_of_no_return,
             illegal})
  end
  ...
  defp eval({:load_object_code, {lib, libVsn, modules}},
            evalState) do
    case (:lists.keysearch(lib, 1,
                             r_eval_state(evalState, :libdirs))) do
      {:value, {^lib, ^libVsn, libDir} = libInfo} ->
        ext = :code.objfile_extension()
        {newBins, newVsns} = :lists.foldl(fn mod,
                                               {bins, vsns} ->
                                               file = :lists.concat([mod, ext])
                                               fName = root_dir_relative_path(:filename.join([libDir,
                                                                                                  'ebin',
                                                                                                      file]))
                                               case (:erl_prim_loader.get_file(fName)) do
                                                 {:ok, bin, fName2} ->
                                                   nVsns = add_vsns(mod, bin,
                                                                      vsns)
                                                   {[{mod, bin, fName2} | bins],
                                                      nVsns}
                                                 :error ->
                                            ...
                                            {r_eval_state(evalState, :bins),
                                               r_eval_state(evalState, :vsns)},
                                            modules)
        newLibs = :lists.keystore(lib, 1,
                                    r_eval_state(evalState, :newlibs), libInfo)
        r_eval_state(evalState, bins: newBins,  newlibs: newLibs, 
                       vsns: newVsns)
  ...
  defp eval(:point_of_no_return, evalState) do
    libs = (case (get_opt(:update_paths, evalState,
                            false)) do
              false ->
    ...
    :lists.foreach(fn {lib, _LibVsn, libDir} ->
  ...
  defp eval({:load,
             {mod, _PrePurgeMethod, postPurgeMethod}},
            evalState) do
    bins = r_eval_state(evalState, :bins)
    {:value, {_Mod, bin, file1}} = :lists.keysearch(mod, 1,
                                                      bins)
    file = root_dir_relative_path(file1)
    {:module, _} = :code.load_binary(mod, file, bin)
    unpurged = do_soft_purge(mod, postPurgeMethod,
                               r_eval_state(evalState, :unpurged))
    r_eval_state(evalState, bins: :lists.keydelete(mod, 1, bins), 
                   unpurged: unpurged)
  end
  ...
  defp eval({:suspend, modules}, evalState) do
    ...
    newSuspended = :lists.foldl(fn modSpec, suspended ->
                                     {module, def__} = (case (modSpec) do
                                                          {mod, modTimeout} ->
                                                            {mod, modTimeout}
                                                          mod ->
                                                            {mod, :default}
                                                        end)
                                     timeout = get_opt(:suspend_timeout,
                                                         evalState, def__)
                                     pids = suspend(module, procs, timeout)
                                     [{module, pids} | suspended]
                                end,
                                  r_eval_state(evalState, :suspended), modules)
    r_eval_state(evalState, suspended: newSuspended)
  end

  defp eval({:resume, modules}, evalState) do
    newSuspended = :lists.foldl(fn mod, suspended ->
                                     :lists.filter(fn {mod2, pids}
                                                          when mod2 == mod ->
                                                        resume(pids)
                                                        false
                                                      _ ->
  ...
  defp eval({:code_change, mode, modules}, evalState) do
    ...
    timeout = get_opt(:code_change_timeout, evalState,
                        :default)
    :lists.foreach(fn {mod, extra} ->
                        vsn = (case (:lists.keysearch(mod, 1, vsns)) do
  ...
  defp eval({:stop, modules}, evalState) do
    procs = get_supervised_procs()
    newStopped = :lists.foldl(fn mod, stopped ->
                                   procs2 = stop(mod, procs)
                                   [{mod, procs2} | stopped]
                              end,
                                r_eval_state(evalState, :stopped), modules)
    r_eval_state(evalState, stopped: newStopped)
  end

  defp eval({:start, modules}, evalState) do
    newStopped = :lists.foldl(fn mod, stopped ->
                                   :lists.filter(fn {mod2, procs}
                                                        when mod2 == mod ->
                                                      start(procs)
                                                      false
                                                    _ ->
  ...
  defp suspend(mod, procs, timeout) do
    :lists.zf(fn {_Sup, _Name, pid, mods} ->
  ...
  defp stop(mod, procs) do
    :lists.zf(fn {:undefined, _Name, _Pid, _Mods} ->
                   false
                 {sup, name, _Pid, mods} ->
                   case (:lists.member(mod, mods)) do
                     true ->
  ...
  def get_supervised_procs() do
    :lists.foldl(fn application, procs ->
                      get_master_procs(application, procs,
  ...
  defp get_supervised_procs(_, root, procs, {:ok, supMod}) do
    get_procs(maybe_supervisor_which_children(root, supMod,
                                                root),
                root) ++ [{:undefined, :undefined, root, [supMod]} |
                              procs]
  end

  defp get_supervised_procs(application, root, procs, {:error, _}) do
    :error_logger.error_msg('release_handler: cannot find top supervisor for application ~w~n', [application])
    get_procs(maybe_supervisor_which_children(root,
                                                application, root),
                root) ++ procs
  end

  defp get_application_names() do
    :lists.map(fn {application, _Name, _Vsn} ->
                    application
               end,
  ...
  defp get_master_procs(application, procs, pid) when is_pid(pid) do
    ...
    get_supervised_procs(application, root, procs,
                           get_supervisor_module(root))
  end
  ...
  defp get_procs([{name, pid, :supervisor, mods} | t], sup)
      when is_pid(pid) do
    [{sup, name, pid, mods} | get_procs(t,
                                          sup)] ++ get_procs(maybe_supervisor_which_children(pid,
                                                                                               name,
                                                                                               pid),
                                                               pid)
  end
  ...
  defp maybe_supervisor_which_children(proc, name, pid) do
    case (get_proc_state(proc)) do
      :noproc ->
        :error_logger.warning_msg('release_handler: a process (~p) exited during supervision tree interrogation. Continuing ...~n', [proc])
        []
      :suspended ->
        :error_logger.error_msg('release_handler: a which_children call to ~p (~w) was avoided. This supervisor is suspended and should likely be upgraded differently. Exiting ...~n', [name, pid])
        :erlang.error(:suspended_supervisor)
      :running ->
        try do
          :supervisor.which_children(pid)
        catch
          :exit, reason when reason !== :timeout and not
                                                     (is_tuple(reason) and :erlang.element(1,
                                                                                             reason) === :nodedown)
                             ->
            []
          :exit, other ->
            :error_logger.error_msg('release_handler: ~p~nerror during a which_children call to ~p (~w). [State: running] Exiting ... ~n', [{:EXIT, other}, name, pid])
  ...
  defp maybe_get_dynamic_mods(name, pid) do
    try do
      ...
      :exit, reason when reason !== :timeout and not
        ...
        []
      :exit, other ->
        :error_logger.error_msg('release_handler: {\'EXIT\',~p}~nerror during a get_modules call to ~p (~w), there may be an error in it\'s childspec. Exiting ...~n', [other, name, pid])
  ...
  defp sync_nodes(id, nodes) do
    ...
    :lists.foreach(fn node ->
                        send({:release_handler, node}, {:sync_nodes, id,
                                                          node()})
                   end,
                     nNodes)
    :lists.foreach(fn node ->
  ...
  defp add_vsns(mod, newBin, vsns) do
    ...
    case (:lists.keysearch(mod, 1, vsns)) do
      {:value, {^mod, oldVsn0, newVsn0}} ->
        :lists.keyreplace(mod, 1, vsns,
                            {mod, replace_undefined(oldVsn0, oldVsn),
