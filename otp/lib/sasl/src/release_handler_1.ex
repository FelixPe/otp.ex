defmodule :m_release_handler_1 do
        ...
        {newBins, newVsns} =
          :lists.foldl(
            fn mod, {bins, vsns} ->
              ...
              fName = root_dir_relative_path(:filename.join([libDir, ~c"ebin", file]))
  ...
  defp eval(:point_of_no_return, evalState) do
    ...
    :lists.foreach(
      fn {lib, _LibVsn, libDir} ->
        ebin = root_dir_relative_path(:filename.join(libDir, ~c"ebin"))
  ...
  defp get_supervised_procs(application, root, procs, {:error, _}) do
    :error_logger.error_msg(
      ~c"release_handler: cannot find top supervisor for application ~w~n",
      [application]
    )
    ...
    case get_proc_state(proc) do
      :noproc ->
        :error_logger.warning_msg(
          ~c"release_handler: a process (~p) exited during supervision tree interrogation. Continuing ...~n",
          [proc]
        )
      ...
      :suspended ->
        :error_logger.error_msg(
          ~c"release_handler: a which_children call to ~p (~w) was avoided. This supervisor is suspended and should likely be upgraded differently. Exiting ...~n",
          [name, pid]
        )
      ...
      :running ->
        try do
          ...
          :exit, other ->
            :error_logger.error_msg(
              ~c"release_handler: ~p~nerror during a which_children call to ~p (~w). [State: running] Exiting ... ~n",
              [{:EXIT, other}, name, pid]
            )
  ...
  defp maybe_get_dynamic_mods(name, pid) do
    try do
      ...
      :exit, other ->
        :error_logger.error_msg(
          ~c"release_handler: {'EXIT',~p}~nerror during a get_modules call to ~p (~w), there may be an error in it's childspec. Exiting ...~n",
          [other, name, pid]
        )
  ...
  defp get_supervisor_module(supPid) do
    case (try do
      ...
      _Other ->
        :io.format(~c"~w: reason: ~w~n", [supPid, _Other])
        {:error, :undefined}
    end
  end
