defmodule :m_disk_log_1 do
  ...
  defp repair(in__, file) do
    ...
    case is_quiet() do
      ...
      _ ->
        :error_logger.info_msg(~c"disk_log: repairing ~tp ...\n", [file])
    end

    tmp = add_ext(file, ~c"TMP")
    ...
    case (try do
      ...
      {:error, _Reason} ->
        case is_quiet() do
          ...
          _ ->
            :error_logger.info_msg(~c"disk_log: chunk error. File ~tp missing.\n\n", [fName])
        end
  ...
  def change_size_wrap(r_handle(filename: fName) = handle, {newMaxB, newMaxF}, version) do
  ...
  def change_size_rotate(
        r_rotate_handle(maxB: maxB, maxF: maxF, file: fName) = handle,
        {newMaxB, newMaxF}
      ) do
  ...
  def do_rotate(
        r_rotate_handle(file: fName, maxF: maxF, cur_fdc: fdC) = rotHandle,
        head
      ) do
    r_cache(fd: fd, c: c) = fdC
    ...
    r_rotate_handle(rotHandle, cur_fdc: newFdC, inode: iNode, curB: fileSize, firstPos: fileSize)
  end
  ...
  def rotate_files(fileName, count) do
    _ =
      :file.rename(
        rotate_file_name(fileName, count - 2),
  ...
  defp rotate_ext_log(handle, bins, head, n0) do
    r_rotate_handle(file: fileName, maxB: maxB, cur_fdc: curFdC, curB: curB, firstPos: firstPos) =
      handle

    {firstBins, lastBins, noBytes, n} = ext_split_bins(curB, maxB, firstPos, bins)
  ...
  defp read_index_file(:truncate, fName, maxF) do
    ...
    _ = :file.delete(add_ext(fName, ~c"idx"))
    {1, 0, 0, 0}
  end
  ...
  def print_index_file(file) do
    :io.format(~c"-- Index begin --~n")

    case :file.read_file(file) do
      {:ok, <<0, 0::size(32), version, curF::size(32), tail::binary>>}
      when version === 2 and 0 < curF and curF < 65000 ->
        :io.format(~c"cur file: ~w~n", [curF])
  ...
  def read_size_file_version(fName) do
    case :file.read_file(add_ext(fName, ~c"siz")) do
