defmodule :m_disk_log_1 do
  ...
  defp handle_chunk(b = <<size :: size(4) - unit(8),
                  1649888321 :: size(4) - unit(8), tail :: binary>>,
            pos, n, ack)
      when size < 65528 do
    case (tail) do
  ...
  defp handle_chunk(b = <<size :: size(4) - unit(8),
                  203500599 :: size(4) - unit(8), tail :: binary>>,
            pos, n, ack) do
    case (tail) do
      <<binTerm :: size(size) - binary, tail2 :: binary>> ->
        handle_chunk(tail2, pos, n - 1, [binTerm | ack])
  ...
  def chunk_read_only(fd, fileName, pos, b, n) do
    fdC = r_cache(fd: fd)
    {_NFdC, reply} = do_chunk_read_only(fdC, fileName, pos,
                                          b, n)
    reply
  end
  ...
  defp handle_chunk_ro(b = <<size :: size(4) - unit(8),
                  1649888321 :: size(4) - unit(8), tail :: binary>>,
            pos, n, ack, bad)
      when size < 65528 do
    case (tail) do
  ...
  defp handle_chunk_ro(b = <<size :: size(4) - unit(8),
    ...
    case (tail) do
      ...
      <<^mD5 :: size(16) - binary, _ :: binary>> ->
        bytesToRead = size + 8 + 16
        {r_continuation(pos: pos - byte_size(b), b: bytesToRead), ack, bad}
      <<_BadMD5 :: size(16) - binary, _ :: size(1) - unit(8),
          tail2 :: binary>> ->
  ...
  defp repair(in__, file) do
    ...
    {:ok,
       {_Alloc, out, {0, _}, _FileSize}} = new_int_file(tmp,
                                                          :none)
    scan_f_read(<<>>, in__, out, file, fSz, tmp, 65536, 0,
                  0)
  end

  defp scan_f_read(b, in__, out, file, fSz, tmp, maxBytes, no,
            bad) do
    case (:file.read(in__, maxBytes)) do
      ...
      {:ok, bin} ->
        ...
        {nB, nMax, ack, nNo, nBad} = scan_f(newBin, fSz, [], no,
                                              bad)
        case (log(out, tmp, :lists.reverse(ack))) do
          {:ok, _Size, newOut} ->
            scan_f_read(nB, in__, newOut, file, fSz, tmp, nMax, nNo,
                          nBad)
  ...
  defp scan_f(b = <<size :: size(4) - unit(8),
                  1649888321 :: size(4) - unit(8), tail :: binary>>,
            fSz, ack, no, bad)
      when size < 65528 do
  ...
  defp scan_f(b = <<_ :: size(8) - unit(8), _ :: binary>>,
            fSz, ack, no, bad) do
    <<_ :: size(8), b2 :: binary>> = b
    scan_f(b2, fSz, ack, no, bad + 1)
  end
  ...
  defp done_scan(in__, out, outName, fName, recoveredTerms,
            badChars) do
    _ = :file.close(in__)
    case ((try do
            fclose(out, outName)
  ...
  def mf_int_open(fName, maxB, maxF, repair, mode, head,
           version) do
    {first, sz, totSz, nFiles} = read_index_file(repair,
                                                   fName, maxF)
    ...
    case (int_file_open(fName, first, 0, 0, head, repair,
                          mode)) do
      {:ok, fdC, fileName, lost, {noItems, noBytes}, fSz} ->
        curCnt = sz + noItems - lost
        {:ok,
           r_handle(filename: fName, maxB: maxB, maxF: newMaxF,
               curF: first, cur_fdc: fdC, cur_name: fileName,
               cur_cnt: curCnt, acc_cnt: - sz, curB: fSz,
               firstPos: noBytes, noFull: 0, accFull: 0),
           totSz + curCnt}
      {:repaired, fdC, fileName, rec, bad, fSz} ->
        {:repaired,
           r_handle(filename: fName, maxB: maxB, cur_name: fileName,
               maxF: newMaxF, curF: first, cur_fdc: fdC, cur_cnt: rec,
               acc_cnt: - rec, curB: fSz, firstPos: 0, noFull: 0,
               accFull: 0),
           rec, bad, totSz + rec}
    end
  end
  ...
  def mf_int_inc(handle, head) do
    r_handle(filename: fName, cur_cnt: curCnt, acc_cnt: accCnt,
    ...
    case ((try do
      ...
      {newF, newMaxF, newFdC, newFileName, nh, firstPos,
         lost} ->
        handle1 = r_handle(handle, cur_fdc: newFdC,  curF: newF, 
                              cur_name: newFileName,  cur_cnt: nh, 
                              acc_cnt: accCnt + curCnt,  maxF: newMaxF, 
                              firstPos: firstPos,  curB: firstPos, 
                              noFull: noFull + 1)
        case ((try do
  ...
  defp mf_int_log(handle, bins, head, no0, wraps) do
    r_handle(curB: curB, maxB: maxB, cur_name: fileName,
        cur_fdc: curFdC, firstPos: firstPos0,
        cur_cnt: curCnt) = handle
    {firstBins, lastBins, noBytes, n} = int_split_bins(curB,
                                                         maxB, firstPos0, bins)
    case (firstBins) do
      [] ->
        r_handle(filename: fName, curF: curF, maxF: maxF,
        ...
        case ((try do
          ...
          {newF, newMaxF, newFdC, newFileName, nh, firstPos,
             lost} ->
            handle1 = r_handle(handle, cur_fdc: newFdC,  curF: newF, 
                                  cur_cnt: nh,  cur_name: newFileName, 
                                  acc_cnt: accCnt + curCnt,  maxF: newMaxF, 
                                  curB: firstPos,  firstPos: firstPos, 
                                  noFull: noFull + 1)
            case ((try do
                    close(curFdC, fileName, :read_write)
                  catch
        ...
        case (fwrite(curFdC, fileName, firstBins, noBytes)) do
          {:ok, newCurFdC} ->
            handle1 = r_handle(handle, cur_fdc: newCurFdC, 
                                  curB: curB + noBytes,  cur_cnt: curCnt + n)
  ...
  defp wrap_int_log(fName, curF, maxF, curCnt, head) do
    ...
    {:ok, newFdC, newFileName, lost, {nh, firstPos},
       _FileSize} = int_file_open(fName, newF, curF, curCnt,
                                    head)
  ...
  def mf_int_chunk(r_handle(curF: fileNo, cur_fdc: fdC,
             cur_name: fileName) = handle,
           {fileNo, pos}, bin, n) do
  ...
  def mf_int_chunk_read_only(r_handle(curF: fileNo, cur_fdc: fdC,
             cur_name: fileName) = handle,
           {fileNo, pos}, bin, n) do
    {newFdC, reply} = do_chunk_read_only(fdC, fileName, pos,
                                           bin, n)
    {r_handle(handle, cur_fdc: newFdC), conv(reply, fileNo)}
  end
  ...
  def mf_sync(r_rotate_handle(file: fName, cur_fdc: fdC) = handle) do
    {reply, newFdC} = fsync(fdC, fName)
    {reply, r_rotate_handle(handle, cur_fdc: newFdC)}
  end

  def mf_int_close(r_handle(filename: fName, curF: curF,
             cur_name: fileName, cur_fdc: curFdC, cur_cnt: curCnt),
           mode) do
  ...
  def mf_ext_open(fName, maxB, maxF, repair, mode, head,
           version) do
    {first, sz, totSz, nFiles} = read_index_file(repair,
                                                   fName, maxF)
    write_size_file(mode, fName, maxB, maxF, version)
    ...
    {:ok, fdC, fileName, lost, {noItems, noBytes},
       curB} = ext_file_open(fName, first, 0, 0, head, repair,
                               mode)
    curCnt = sz + noItems - lost
    {:ok,
       r_handle(filename: fName, maxB: maxB, cur_name: fileName,
  ...
  def mf_ext_inc(handle, head) do
    r_handle(filename: fName, cur_cnt: curCnt, cur_name: fileName,
        acc_cnt: accCnt, curF: curF, maxF: maxF,
        cur_fdc: curFdC, noFull: noFull) = handle
    case ((try do
  ...
  defp mf_ext_log(handle, bins, head, no0, wraps) do
    r_handle(curB: curB, maxB: maxB, cur_name: fileName,
        cur_fdc: curFdC, firstPos: firstPos0,
        cur_cnt: curCnt) = handle
    {firstBins, lastBins, noBytes, n} = ext_split_bins(curB,
                                                         maxB, firstPos0, bins)
    case (firstBins) do
      [] ->
        r_handle(filename: fName, curF: curF, maxF: maxF,
  ...
  defp wrap_ext_log(fName, curF, maxF, curCnt, head) do
    {newF, newMaxF} = inc_wrap(fName, curF, maxF)
    {:ok, newFdC, newFileName, lost, {nh, firstPos},
       _FileSize} = ext_file_open(fName, newF, curF, curCnt,
                                    head)
    {newF, newMaxF, newFdC, newFileName, nh, firstPos, lost}
  end

  def mf_ext_close(r_handle(filename: fName, curF: curF, cur_fdc: curFdC,
             cur_cnt: curCnt),
           mode) do
    res = ((try do
             fclose(curFdC, fName)
  ...
  def change_size_wrap(r_handle(filename: fName) = handle, {newMaxB, newMaxF},
           version) do
  ...
  def change_size_rotate(r_rotate_handle(maxB: maxB, maxF: maxF, file: fName) = handle,
           {newMaxB, newMaxF}) do
    {^maxB, maxF1} = get_size(maxB, maxF)
    cond do
      newMaxF > maxF1 ->
        remove_files(:rotate, fName, maxF1, newMaxF)
        {:ok, r_rotate_handle(handle, maxB: newMaxB,  maxF: newMaxF)}
      newMaxF < maxF1 ->
  ...
  def open_rotate_log_file(fileName, size, head) do
    try do
      case (:filelib.ensure_dir(fileName)) do
        :ok ->
          case (:file.open(fileName,
                             [:raw, :binary, :read, :append])) do
            {:ok, fd} ->
              ...
              {:ok, r_file_info(inode: iNode)} = :file.read_file_info(fileName,
                                                              [:raw])
              {maxB, maxF} = size
              rotHandle = r_rotate_handle(file: fileName, cur_fdc: fdC, maxB: maxB,
                              maxF: maxF, curB: fileSize, firstPos: fileSize,
                              inode: iNode)
  ...
  defp update_rotation(r_rotate_handle(file: fName, maxF: maxF)) do
    maybe_remove_archives(maxF, fName)
    maybe_update_compress(0, maxF, fName)
  end

  defp maybe_remove_archives(count, fName) do
  ...
  defp maybe_update_compress(maxF, maxF, _FName) do
    :ok
  end

  defp maybe_update_compress(n, maxF, fName) do
  ...
  def do_rotate(r_rotate_handle(file: fName, maxF: maxF,
             cur_fdc: fdC) = rotHandle,
           head) do
    r_cache(fd: fd, c: c) = fdC
    ...
    {:ok, r_file_info(inode: iNode)} = :file.read_file_info(fName,
                                                    [:raw])
    r_rotate_handle(rotHandle, cur_fdc: newFdC,  inode: iNode, 
                   curB: fileSize,  firstPos: fileSize)
  end

  def rotate_files(fileName, 0) do
    _ = :file.delete(fileName)
    :ok
  end

  def rotate_files(fileName, 1) do
  ...
  def rotate_files(fileName, count) do
    _ = :file.rename(rotate_file_name(fileName, count - 2),
                       rotate_file_name(fileName, count - 1))
    rotate_files(fileName, count - 1)
  end

  defp rotate_file_name(fileName, count) do
  ...
  def rotate_ext_log(handle, bin, head) do
    rotate_ext_log(handle, bin, head, 0)
  end

  defp rotate_ext_log(handle, [], _Head, n) do
    {:ok, handle, n}
  end

  defp rotate_ext_log(handle, bins, head, n0) do
    r_rotate_handle(file: fileName, maxB: maxB, cur_fdc: curFdC,
    ...
    case (firstBins) do
      ...
      _ ->
        case (fwrite(curFdC, fileName, firstBins, noBytes)) do
          {:ok, newCurFdC} ->
            handle1 = r_rotate_handle(handle, cur_fdc: newCurFdC, 
                                  curB: curB + noBytes)
  ...
  def rotate_write_cache(r_rotate_handle(file: fName, cur_fdc: fdC) = handle) do
    :erlang.erase(:write_cache_timer_is_running)
    r_cache(fd: fd, c: c) = fdC
    {reply, newFdC} = write_cache(fd, fName, c)
    {reply, r_rotate_handle(handle, cur_fdc: newFdC)}
  ...
  def rotate_ext_close(r_rotate_handle(file: fName, cur_fdc: curFdC)) do
    (try do
      fclose(curFdC, fName)
    catch
      :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp ensure_open(filename, head) do
    case (:filelib.ensure_dir(filename)) do
      :ok ->
  ...
  defp delayed_write_close(fd) do
    case (:file.close(fd)) do
      {:error, _} ->
        :file.close(fd)
      res ->
  ...
  defp int_file_open(fName, newFile, oldFile, oldCnt, head, repair,
            mode) do
    n = add_ext(fName, newFile)
    case (int_open(n, repair, mode, head)) do
      {:ok, {_Alloc, fdC, headSize, fileSize}} ->
        lost = write_index_file(mode, fName, newFile, oldFile,
                                  oldCnt)
  ...
  defp ext_file_open(fName, newFile, oldFile, oldCnt, head, repair,
            mode) do
    fileName = add_ext(fName, newFile)
    {:ok,
       {_Alloc, fdC, headSize, fileSize}} = ext_open(fileName,
                                                       repair, mode, head)
    lost = write_index_file(mode, fName, newFile, oldFile,
                              oldCnt)
  ...
  def read_index_file(fName) do
    ...
    case (open_read(fileName)) do
      {:ok, fd} ->
        r = (case (:file.read(fd, 65536)) do
               {:ok,
                  <<0, 0 :: size(32), version, curF :: size(32),
                      tail :: binary>>}
  ...
  defp parse_index(curF, v, curF,
            <<curSz :: size(64), tail :: binary>>, fd, _, totSz,
            nFiles)
      when v === 2 do
    parse_index(curF, v, curF + 1, tail, fd, curSz, totSz,
                  nFiles + 1)
  end

  defp parse_index(curF, v, n, <<sz :: size(64), tail :: binary>>,
            fd, curSz, totSz, nFiles)
      when v === 2 do
    parse_index(curF, v, n + 1, tail, fd, curSz, totSz + sz,
                  nFiles + 1)
  end

  defp parse_index(curF, v, curF,
            <<curSz :: size(32), tail :: binary>>, fd, _, totSz,
            nFiles)
      when v < 2 do
    parse_index(curF, v, curF + 1, tail, fd, curSz, totSz,
                  nFiles + 1)
  ...
  defp parse_index(curF, v, n, <<sz :: size(32), tail :: binary>>,
            fd, curSz, totSz, nFiles)
      when v < 2 do
    parse_index(curF, v, n + 1, tail, fd, curSz, totSz + sz,
                  nFiles + 1)
  ...
  defp write_index_file(:read_only, _FName, _NewFile, _OldFile,
            _OldCnt) do
    0
  end
  ...
  defp write_index_file(:read_write, fName, newFile, oldFile, oldCnt) do
    ...
    case (open_update(fileName)) do
      {:ok, fd} ->
        {offset, szSz} = (case (:file.read(fd, 6)) do
                            :eof ->
                              bin = <<0, 0 :: size(32), 2, newFile :: size(32)>>
                              fwrite_close2(fd, fileName, bin)
                              {10, 8}
                            {:ok, <<0, 0 :: size(32), _Version>>} ->
                              pwrite_close2(fd, fileName, 6,
                                              <<newFile :: size(32)>>)
                              {10, 8}
                            {:ok, <<0, _ :: binary>>} ->
                              pwrite_close2(fd, fileName, 1,
                                              <<newFile :: size(32)>>)
                              {5, 4}
                            {:ok, <<_, _ :: binary>>} ->
  ...
  defp to_8_bytes(<<n :: size(32), t :: binary>>, nT, fileName,
            fd) do
  ...
  defp write_size_file(:read_only, _FName, _NewSize, _NewMaxFiles,
            _Version) do
    :ok
  end

  defp write_size_file(:read_write, fName, newSize, newMaxFiles,
            version) do
    fileName = add_ext(fName, 'siz')
    bin = (cond do
             version === 2 ->
               <<version, newSize :: size(64),
                   newMaxFiles :: size(32)>>
             true ->
  ...
  def read_size_file_version(fName) do
    case (:file.read_file(add_ext(fName, 'siz'))) do
      {:ok,
         <<version, size :: size(64), maxFiles :: size(32)>>}
          when version === 2 ->
  ...
  defp ext_split_bins(maxBs, isFirst, first, [x | last], bs, n) do
    ...
    cond do
      nBs <= maxBs ->
        ext_split_bins(maxBs, isFirst, [first | x], last, nBs,
                         n + 1)
      (isFirst and first === []) ->
        {[x], last, nBs, n + 1}
      true ->
  ...
  defp int_split_bins(maxBs, isFirst, first, [x | last], bs, n) do
    ...
    cond do
      nBs <= maxBs ->
        int_split_bins(maxBs, isFirst, [first | xB], last, nBs,
                         n + 1)
      (isFirst and first === []) ->
        {[xB], last, nBs, n + 1}
      true ->
  ...
  def remove_files(type, fName, n, max) do
    remove_files(type, fName, n, max, :ok)
  end

  defp remove_files(_Type, _FName, n, max, :ok) when n > max do
    :ok
  end

  defp remove_files(_Type, _FName, n, max, {fileName, error})
      when n > max do
    file_error(fileName, error)
  end

  defp remove_files(type, fName, n, max, reply) do
    fileName = (case (type) do
                  :wrap ->
                    add_ext(fName, n)
                  :rotate ->
                    rotate_file_name(fName, n)
                end)
    newReply = (case (:file.delete(fileName)) do
                  :ok ->
                    reply
                  {:error, :enoent} ->
                    reply
  ...
  def get_rotate_size(r_rotate_handle(maxB: maxB, maxF: maxF)) do
    get_size(maxB, maxF)
  end

  defp get_size(maxB, maxF) do
    case (maxF) do
      {newMaxF, _} ->
        {maxB, newMaxF}
      ^maxF ->
  ...
  def fwrite(r_cache(fd: fd, c: c, sz: sz) = fdC, fileName, b,
           size) do
    sz1 = sz + size
    c1 = cache_append(c, b)
    cond do
  ...
  defp pread(r_cache(fd: fd, c: c), fileName, position,
            maxBytes) do
