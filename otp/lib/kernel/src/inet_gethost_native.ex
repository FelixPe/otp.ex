defmodule :m_inet_gethost_native do
  ...
  defp run_once() do
    ...
    {clientHandle, request} =
      receive do
        ...
        when is_reference(reqH) ->
          {reqH, [<<rID::size(32), 1::size(8), proto0::size(8)>>, name0, 0]}

        {reqH, {2, proto1, data1}}
        when is_reference(reqH) ->
          {reqH, <<rID::size(32), 2::size(8), proto1::size(8), data1::binary>>}
      after
        timeout ->
          exit(:normal)
      end
    ...
    receive do
      {^port, {:data, <<^rID::size(32), binReply::binary>>}} ->
  ...
  defp handle_message({clientHandle, {1, proto, name} = req}, state)
    ...
    do_handle_call(clientHandle, req, [<<1::size(8), proto::size(8)>>, name, 0], state)
    main_loop(state)
  end

  defp handle_message({clientHandle, {2, proto, data} = req}, state)
       when is_reference(clientHandle) do
    do_handle_call(clientHandle, req, <<2::size(8), proto::size(8), data::binary>>, state)
    main_loop(state)
  end
  ...
  defp handle_message({:timeout, timerRef, rID}, state) do
    case :ets.lookup(r_state(state, :requests), rID) do
      ...
      [r_request(timer_ref: ^timerRef, req: req, req_ts: :undefined)] ->
      ...
      [r_request(timer_ref: ^timerRef, req_ts: reqTs)] ->
        timeoutTime =
          :erlang.convert_time_unit(reqTs, :native, :millisecond) + r_state(state, :timeout)

        newTimerRef = :erlang.start_timer(timeoutTime, self(), rID, [{:abs, true}])

        true =
          :ets.update_element(r_state(state, :requests), rID, [
            {r_request(:timer_ref), newTimerRef},
            {r_request(:req_ts), :undefined}
          ])
  ...
  defp restart_port(r_state(port: port, requests: requests)) do
    ...
    foreach(
      fn r_request(rid: rID, req: {op, proto, rdata}) ->
        case op do
          1 ->
            :erlang.port_command(
              newPort,
  ...
  defp do_open_port(poolsize, extraArgs) do
    ...
    prog = :filename.join(binDir, ~c"inet_gethost")
  ...
  defp open_executable(prog, opts) do
    try do
      ...
      :error, reason ->
        :erlang.halt(~c"Can not execute " ++ prog ++ ~c" : " ++ term2string(reason))
    end
  end

  defp term2string(term) do
    :unicode.characters_to_list(:io_lib.format(~c"~tw", [term]))
  end
