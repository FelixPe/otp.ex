defmodule :m_wrap_log_reader do
  ...
  Record.defrecord(:r_wrap_reader, :wrap_reader, fd: :undefined,
                                       cont: :undefined, file: :undefined,
                                       file_no: :undefined,
                                       mod_time: :undefined,
                                       first_no: :undefined)
  ...
  def open(file) when is_list(file) do
    case (read_index_file(file)) do
      ...
      {:ok, {curFileNo, _CurFileSz, _TotSz, noOfFiles}} ->
        fileNo = (case (rem(curFileNo + 1, noOfFiles)) do
                    0 ->
                      noOfFiles
                    no ->
                      no
  ...
  def open(file, fileNo) when (is_atom(file) and
                               is_integer(fileNo)) do
    open(:erlang.atom_to_list(file), fileNo)
  end

  def open(file, fileNo) when (is_list(file) and
                               is_integer(fileNo)) do
    case (read_index_file(file)) do
            ...
            case (:disk_log_1.is_head(head)) do
              ...
              _ ->
                case (last_mod_time(fName)) do
                  {:ok, modTime} ->
                    wR = r_wrap_reader(fd: fd, cont: :start, file: file,
                             file_no: fileNo, mod_time: modTime,
  ...
  defp chunk(wR, n, bad) do
    r_wrap_reader(fd: fd, cont: continue, file: file,
        file_no: curFileNo, first_no: firstFileNo) = wR
    case (read_a_chunk(fd, n, continue,
                         add_ext(file, curFileNo))) do
      :eof ->
        case (firstFileNo) do
          :one ->
            {wR, :eof}
  ...
  defp chunk_at_eof(wR, n, bad) do
    r_wrap_reader(file: file, file_no: curFileNo,
        first_no: firstFileNo) = wR
    case (read_index_file(file)) do
      {:ok, indexFile} ->
        {_, _, _, noOfFiles} = indexFile
        newFileNo = (case (rem(curFileNo + 1, noOfFiles)) do
  ...
  defp read_next_file(wR, n, newFileNo, bad) do
    r_wrap_reader(file: file, file_no: curFileNo, mod_time: modTime,
        first_no: firstFileNo) = wR
