defmodule :m_logger_server do
  ...
  require Record
  Record.defrecord(:r_state, :state, tid: :undefined,
                                 async_req: :undefined,
                                 async_req_queue: :undefined,
                                 remote_logger: :undefined)
  def start_link() do
    :gen_server.start_link({:local, :logger},
                             :logger_server, [], [])
  end
  ...
  def update_formatter_config(handlerId, formatterConfig)
    ...
    call({:update_formatter_config, handlerId,
            formatterConfig})
  end
  ...
  def init([]) do
    ...
    :logger_config.create(tid, :proxy,
                                              ...
                                              {&:logger_filters.domain/2,
                                               ...
                                               {:no_domain,
                                                  {&:logger_filters.domain/2,
                                                     {:log, :undefined, []}}}]})
    {:ok,
       simpleConfig} = :logger_simple_h.adding_handler(simpleConfig0)
    :logger_config.create(tid, :simple, simpleConfig)
    {:ok, r_state(tid: tid, async_req_queue: :queue.new())}
  end

  def handle_call({:add_handler, id, module, hConfig}, from,
           r_state(tid: tid) = state) do
    case (:logger_config.exist(tid, id)) do
      true ->
        {:reply, {:error, {:already_exist, id}}, state}
      false ->
        call_h_async(fn () ->
                          call_h(module, :adding_handler, [hConfig],
                                   {:ok, hConfig})
                     end,
                       fn {:ok, hConfig1} ->
                            case (:erlang.function_exported(module, :log, 2)) do
                              true ->
                                ...
                                {:ok, config} = :logger_config.get(tid,
                                                                     :primary)
  ...
  def handle_call({:remove_handler, handlerId}, from,
           r_state(tid: tid) = state) do
  ...
  def handle_call({:add_filter, id, filter}, _From,
           r_state(tid: tid) = state) do
    reply = do_add_filter(tid, id, filter)
    {:reply, reply, state}
  end

  def handle_call({:remove_filter, id, filterId}, _From,
           r_state(tid: tid) = state) do
    reply = do_remove_filter(tid, id, filterId)
    {:reply, reply, state}
  end

  def handle_call({:change_config, setOrUpd, :proxy, config0},
           _From, r_state(tid: tid) = state) do
    default = (case (setOrUpd) do
                 :set ->
                   :logger_proxy.get_default_config()
                 :update ->
    ...
    config = :maps.merge(default, config0)
    reply = (case (:logger_olp.set_opts(:logger_proxy,
                                          config)) do
               :ok ->
  ...
  def handle_call({:change_config, setOrUpd, :primary, config0},
           _From, r_state(tid: tid) = state) do
    {:ok,
       %{handlers:
         handlers} = oldConfig} = :logger_config.get(tid,
                                                       :primary)
    default = (case (setOrUpd) do
                 :set ->
    ...
    config = :maps.merge(default, config0)
    reply = :logger_config.set(tid, :primary,
                                 Map.put(config, :handlers, handlers))
    {:reply, reply, state}
  end

  def handle_call({:change_config, _SetOrUpd, :primary, key,
            value},
           _From, r_state(tid: tid) = state) do
    {:ok, oldConfig} = :logger_config.get(tid, :primary)
    reply = :logger_config.set(tid, :primary,
                                 Map.put(oldConfig, key, value))
    {:reply, reply, state}
  end

  def handle_call({:change_config, setOrUpd, handlerId, config0},
           from, r_state(tid: tid) = state) do
    case (:logger_config.get(tid, handlerId)) do
  ...
  def handle_call({:change_config, setOrUpd, handlerId, key,
            value},
           from, r_state(tid: tid) = state) do
    case (:logger_config.get(tid, handlerId)) do
      {:ok, %{module: module} = oldConfig} ->
  ...
  def handle_call({:update_formatter_config, handlerId,
            newFConfig},
           _From, r_state(tid: tid) = state) do
    reply = (case (:logger_config.get(tid, handlerId)) do
  ...
  def handle_call({:set_module_level, modules, level}, _From,
           state) do
  ...
  def handle_cast({:async_req_reply, _Ref, _Reply} = reply,
           state) do
  ...
  def handle_info({:DOWN, _Ref, _Proc, _Pid, _Reason} = down,
           state) do
    call_h_reply(down, state)
  end

  def handle_info(unexpected, state) when :erlang.element(1,
                                                   unexpected) == :EXIT do
    case (:logger.allow(:debug, :logger_server)) do
      true ->
        _ = :logger_server.do_internal_log(:debug,
                                             %{mfa:
                                               {:logger_server, :handle_info,
                                                  2},
                                                 line: 355, file: 'otp/lib/kernel/src/logger_server.erl'},
                                             %{},
                                             [[{:logger,
                                                  :got_unexpected_message},
                                                   {:process, :logger},
                                                       {:message, unexpected}]])
        :ok
      false ->
  ...
  def handle_info(unexpected, state) do
    case (:logger.allow(:info, :logger_server)) do
      true ->
        _ = :logger_server.do_internal_log(:info,
                                             %{mfa:
                                               {:logger_server, :handle_info,
                                                  2},
                                                 line: 362, file: 'otp/lib/kernel/src/logger_server.erl'},
                                             %{},
                                             [[{:logger,
                                                  :got_unexpected_message},
                                                   {:process, :logger},
  ...
  defp call(request) when is_tuple(request) do
    ...
    case (:erlang.get(:"$logger_cb_process")) do
      true when action == :add_handler or
    ...
    case (:logger_config.get(tid, id)) do
      {:ok, config} ->
        ...
        case (:lists.keymember(fId, 1, filters)) do
          ...
          false ->
            :logger_config.set(tid, id,
                                 Map.put(config, :filters, [filter | filters]))
        end
      error ->
        error
    end
    ...
    case (:logger_config.get(tid, id)) do
      {:ok, config} ->
        ...
        case (:lists.keytake(filterId, 1, filters0)) do
          {:value, _, filters} ->
            :logger_config.set(tid, id,
                                 Map.put(config, :filters, filters))
          false ->
  ...
  defp default_config(:primary) do
    %{level: :notice, metadata: %{}, filters: [],
        filter_default: :log}
  end

  defp default_config(id) do
    %{id: id, level: :all, filters: [],
        filter_default: :log,
  ...
  defp check_config(:primary, [{:metadata, meta} | config])
      when is_map(meta) do
    ...
    try do
      ...
      c, r ->
        case ({c, r, __STACKTRACE__}) do
          {:error, :undef,
             [{^mod, :check_config, [^config], _} | _]} ->
            :ok
          _ ->
            throw({:callback_crashed,
                     {c, r,
                        :logger.filter_stacktrace(:logger_server,
                                                    __STACKTRACE__)}})
        end
    else
      :ok ->
  ...
  defp check_config_change(%{id: id, module: module},
            %{id: id, module: module}) do
    :ok
  end

  defp check_config_change(oldConfig, newConfig) do
    {old, new} = :logger_server.diff_maps(:maps.with([:id,
                                                          :module],
                                                       oldConfig),
                                            :maps.with([:id, :module],
                                                         newConfig))
  ...
  defp call_h(module, function, args, defRet) do
    try do
      ...
      c, r ->
        case ({c, r, __STACKTRACE__}) do
          {:error, :undef,
             [{^module, ^function = :changing_config, args, _} | _]}
              when length(args) === 3 ->
          ...
          {:error, :undef,
            ...
            defRet
          _ ->
            sT = :logger.filter_stacktrace(:logger_server,
                                             __STACKTRACE__)
            case (:logger.allow(:error, :logger_server)) do
              true ->
                _ = :logger_server.do_internal_log(:error,
                                                     %{mfa:
                                                       {:logger_server, :call_h,
                                                          4},
                                                         line: 551, file: 'otp/lib/kernel/src/logger_server.erl'},
                                                     %{},
                                                     [[{:logger,
                                                          :callback_crashed},
                                                           {:process, :logger},
  ...
  defp call_h_async(asyncFun, postFun, from,
            r_state(async_req: :undefined) = state) do
    parent = self()
  ...
  defp call_h_async(asyncFun, postFun, from,
            r_state(async_req_queue: q) = state) do
    {:noreply,
       r_state(state, async_req_queue: :queue.in({asyncFun, postFun,
                                              from},
                                             q))}
  end

  defp call_h_reply({:async_req_reply, ref, reply},
            r_state(async_req: {ref, postFun, from},
                async_req_queue: q) = state) do
    :erlang.demonitor(ref, [:flush])
    _ = :gen_server.reply(from, postFun.(reply))
    {value, newQ} = :queue.out(q)
    newState = r_state(state, async_req: :undefined, 
  ...
  defp call_h_reply({:DOWN, ref, _Proc, pid, reason},
            r_state(async_req: {ref, _PostFun, _From}) = state) do
    case (:logger.allow(:error, :logger_server)) do
      true ->
        _ = :logger_server.do_internal_log(:error,
                                             %{mfa:
                                               {:logger_server, :call_h_reply,
                                                  2},
                                                 line: 594, file: 'otp/lib/kernel/src/logger_server.erl'},
                                             %{},
    ...
    call_h_reply({:async_req_reply, ref,
                    {:error, {:logger_process_exited, pid, reason}}},
                   state)
  end
      ...
      true ->
        _ = :logger_server.do_internal_log(:info,
                                             %{mfa:
                                               {:logger_server, :call_h_reply,
                                                  2},
                                                 line: 603, file: 'otp/lib/kernel/src/logger_server.erl'},
                                             %{},
                                             [[{:logger,
                                                  :got_unexpected_message},
                                                   {:process, :logger},
  ...
  def diff_maps(m1, m2) do
    diffs(:lists.sort(:maps.to_list(m1)),
  ...
  def do_internal_log(level, location, log, [report] = data) do
    do_internal_log(level, location, log, data,
                      {:report, report})
  end
  ...
  defp do_internal_log(level, location, log, data, msg) do
    ...
    case (log) do
      %{meta: %{internal_log_event: true}} ->
        _ = spawn(:logger_simple_h, :log,
                    [%{level: level, msg: msg, meta: meta}, %{}])
      _ ->
        _ = spawn(:logger, :macro_log,
                    [location, level |
                                   data] ++ [Map.put(meta, :internal_log_event,
                                                             true)])
    end
  end
