defmodule :m_gen_sctp do
  ...
  def open(port, opts) when (is_integer(port) and
                             is_list(opts)) do
    open([{:port, port} | opts])
  end
  ...
  def listen(s, backlog) when (is_port(s) and
  ...
  def peeloff(s, assocId) when (is_port(s) and
  ...
  def connect_init(s, addr, port, opts, timeout) do
    case (do_connect(s, addr, port, opts, timeout,
                       false)) do
      :badarg ->
        :erlang.error(:badarg, [s, addr, port, opts, timeout])
      result ->
  ...
  defp do_connectx(s, sockAddrs, opts) when (is_port(s) and
  ...
  defp ensure_sockaddrs([sockAddr | sockAddrs], port, acc) do
    case (sockAddr) do
      ...
      {family, {_, p}} when family === :inet or
                              family === :inet6
                            ->
        ensure_sockaddrs(sockAddrs, port, [sockAddr | acc], p)
      %{family: family} when family === :inet or
                               family === :inet6
                             ->
        sockAddr_1 = :inet.ensure_sockaddr(sockAddr)
        ensure_sockaddrs(sockAddrs, port, [sockAddr_1 | acc],
  ...
  defp set_port([sockAddr | sockAddrs], port) do
    case (sockAddr) do
      {iP, p} when is_tuple(iP) ->
        set_port(sockAddrs, port, sockAddr, p,
                   fn () ->
                        {iP, port}
                   end)
      {family, {addr, p}} ->
  ...
  def send(s, assocId, stream, data) when (is_port(s) and
