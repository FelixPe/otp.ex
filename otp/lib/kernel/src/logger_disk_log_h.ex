defmodule :m_logger_disk_log_h do
      ...
      :ok ->
        {:ok,
           %{log_opts:
             %{file: file, type: type, max_no_bytes: mNB,
                 max_no_files: mNF},
               prev_log_result: :ok, prev_sync_result: :ok,
  ...
  def check_config(_Name, setOrUpdate, oldHConfig, newHConfig0) do
    writeOnce = :maps.with([:type, :file, :max_no_files,
                                              :max_no_bytes],
                             oldHConfig)
    ...
    case (:maps.with([:type, :file, :max_no_files,
                                        :max_no_bytes],
                       newHConfig)) do
      ^writeOnce ->
      ...
      other ->
        ...
        {:error,
           {:illegal_config_change, :logger_disk_log_h, old, new}}
    end
  end
  ...
  defp check_config(hConfig) do
    case (check_h_config(:maps.to_list(hConfig))) do
      ...
      {:error, {key, value}} ->
        {:error,
           {:invalid_config, :logger_disk_log_h, %{key => value}}}
    end
  end
  ...
  defp check_h_config([{:type, type} | config]) when type == :wrap or
                                           type == :halt do
    check_h_config(config)
  end
  ...
  defp merge_default_logopts(name, hConfig) do
    ...
    {defaultNoFiles, defaultNoBytes} = (case (type) do
                                          :halt ->
                                            {:undefined, :infinity}
                                          _wrap ->
                                            {10, 1048576}
    ...
    defaults = %{file: :filename.join(dir, name),
                   ...
                   max_no_bytes: defaultNoBytes, type: type}
    :maps.merge(defaults, hConfig)
  end

  def filesync(name, _Mode, state) do
    result = :disk_log.sync(name)
    maybe_notify_error(name, :filesync, result,
                         :prev_sync_result, state)
  end

  def write(name, mode, bin, state) do
    result = disk_log_write(name, mode, bin)
    maybe_notify_error(name, :log, result, :prev_log_result,
                         state)
  end

  def reset_state(_Name, state) do
    Map.merge(state, %{prev_log_result: :ok,
                         prev_sync_result: :ok, prev_disk_log_info: :undefined})
  end

  def handle_info(name,
           {:disk_log, _Node, log,
              info = {:truncated, _NoLostItems}},
           state) do
    maybe_notify_status(name, log, info,
                          :prev_disk_log_info, state)
  end
  ...
  def handle_info(name, {:disk_log, _Node, log, info = :full},
           state) do
    maybe_notify_status(name, log, info,
                          :prev_disk_log_info, state)
  end
  ...
  defp open_disk_log(name, file, type, maxNoBytes, maxNoFiles) do
    case (:filelib.ensure_dir(file)) do
      :ok ->
        ...
        opts = [{:name, name}, {:file, file}, {:size, size},
                                                  {:type, type}, {:linkto,
                                                                    self()},
                                                                     {:repair,
                                                                        false},
  ...
  defp maybe_notify_error(name, op, result, key,
            %{log_opts: logOpts} = state) do
    {result,
       error_notify_new({name, op, logOpts, result}, result,
                          key, state)}
  end

  defp maybe_notify_status(name, log, info, key, state) do
    error_notify_new({:disk_log, name, log, info}, info,
                       key, state)
  end
