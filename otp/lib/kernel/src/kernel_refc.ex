defmodule :m_kernel_refc do
  ...
  def start_link() do
    :gen_server.start_link({:local, :kernel_refc},
                             :kernel_refc, [], [])
  end

  def scheduler_wall_time(bool) do
    :gen_server.call(:kernel_refc,
                       {:scheduler_wall_time, self(), bool}, :infinity)
  end
  ...
  def handle_call({what, who, false}, _From, state) do
    {reply, cnt} = do_stop(what, :maps.get(what, state),
                             who)
    {:reply, reply, %{state | what => cnt}}
  end

  def handle_call({what, who, true}, _From, state) do
    {reply, cnt} = do_start(what, :maps.get(what, state),
                              who)
    {:reply, reply, %{state | what => cnt}}
  end
