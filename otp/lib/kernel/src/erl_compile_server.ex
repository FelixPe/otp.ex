defmodule :m_erl_compile_server do
  ...
  require Record
  Record.defrecord(:r_st, :st, cwd: [], config: :undefined,
                              timeout: 10 * 1000, jobs: %{})
  def start_link() do
    :gen_server.start_link({:local, :erl_compile_server},
                             :erl_compile_server, [], [])
  end
  ...
  def compile(parameters) do
    :gen_server.call(:erl_compile_server,
                       {:compile, parameters}, :infinity)
  end

  def handle_call({:compile, parameters}, from,
           r_st(jobs: jobs) = st0) do
    ...
    case (verify_context(pathArgs, parameters, st0)) do
      {:ok, st1} ->
        ...
        pidRef = spawn_monitor(fn () ->
                                    exit(do_compile(erlcArgs,
                                                      :unicode.characters_to_list(cwd,
                                                                                    enc),
                                                      enc))
                               end)
  ...
  defp parse_command_line(%{command_line: cmdLine0, cwd: cwd,
              encoding: enc}) do
    cmdLine = :lists.map(fn a ->
                              :unicode.characters_to_list(a, enc)
                         end,
                           cmdLine0)
    parse_command_line_1(cmdLine,
                           :unicode.characters_to_list(cwd, enc), [], [])
  end
  ...
  defp init_config() do
    envVars = ['ERL_AFLAGS', 'ERL_FLAGS', 'ERL_ZFLAGS', 'ERL_COMPILER_OPTIONS', 'ERL_LIBS', 'ERLC_CONFIGURATION']
    env0 = (for name <- envVars do
              {name, :os.getenv(name)}
            end)
    env = (for ({_, val} = p) <- env0, val !== false do
             p
           end)
    {:ok, cwd} = :file.get_cwd()
    make_config([get_path_arg(:pa, cwd), get_path_arg(:pz,
                                                        cwd)],
                  env)
  end
  ...
  defp make_config(pathArgs, env0) do
    ...
    env = :lists.sort(env0)
    pathArgs ++ [libDirSize] ++ [:erlang.iolist_to_binary(for {name,
                                                                 val} <- env do
                                                            [name, ?=, val, ?\n]
                                                          end)]
  end
  ...
  defp gl_request({:put_chars, encoding, chars}, state) do
    gl_put_chars(:unicode.characters_to_binary(chars,
                                                 encoding),
                   state)
  end

  defp gl_request({:put_chars, encoding, module, function, args},
            state) do
    try do
      gl_request({:put_chars, encoding,
