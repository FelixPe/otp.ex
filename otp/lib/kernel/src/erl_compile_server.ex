defmodule :m_erl_compile_server do
  ...
  defp parse_command_line(%{command_line: cmdLine0, cwd: cwd, encoding: enc}) do
    cmdLine =
      :lists.map(
        fn a ->
  ...
  defp parse_command_line_1([~c"-pa", pa | t], cwd, paAcc, pzAcc) do
    parse_command_line_1(t, cwd, [pa | paAcc], pzAcc)
  end

  defp parse_command_line_1([~c"-pz", pz | t], cwd, paAcc, pzAcc) do
    parse_command_line_1(t, cwd, paAcc, [pz | pzAcc])
  end

  defp parse_command_line_1([~c"-extra" | erlcArgs], cwd, paAcc, pzAcc) do
  ...
  defp init_config() do
    envVars = [
      ...
      ~c"ERL_FLAGS",
      ~c"ERL_ZFLAGS",
      ~c"ERL_COMPILER_OPTIONS",
      ~c"ERL_LIBS",
      ~c"ERLC_CONFIGURATION"
    ]
