defmodule :m_net do
  ...
  def getnameinfo(sockAddr, flags) when (is_map(sockAddr) and
                                  is_list(flags)) or
  ...
  def getaddrinfo(host, service)
      when (is_list(host) or host === :undefined) and (is_list(service) or service === :undefined) and not
                                                                                                       (service === :undefined and host === :undefined) do
  ...
  def getifaddrs(filter, namespace) when is_function(filter,
                                               1) and is_list(namespace) do
    do_getifaddrs(filter,
                    fn () ->
                         getifaddrs(namespace)
                    end)
  ...
  defp getifaddrs_filter(%{family: fFamily, flags: fFlags},
  ...
  defp win_getifaddrs(_Filter, adsAddrs, ipAddrTab, ipIfInfo) do
    throw({:error,
             {:insufficient_info, adsAddrs, ipAddrTab, ipIfInfo}})
  end
  ...
  defp win_getifaddrs_iat2([%{index: idx} = ipAddr | ipAddrTab], ipIfInfos,
            acc) do
    case (:prim_net.get_if_entry(%{index: idx})) do
      {:ok, %{name: name} = ifEntry} when name !== '' ->
        {ifAddr, pktIfAddr} = win_getifaddrs_iat3(name, ipAddr,
                                                    ifEntry)
        win_getifaddrs_iat2(ipAddrTab, ipIfInfos,
      ...
      {:ok, %{name: name, description: desc} = ifEntry}
        ...
        case (if_info_search(idx, ipIfInfos)) do
          {:value, %{name: name2}} ->
            {ifAddr, pktIfAddr} = win_getifaddrs_iat3(name2, ipAddr,
                                                        ifEntry)
            win_getifaddrs_iat2(ipAddrTab, ipIfInfos,
                                  [ifAddr, pktIfAddr | acc])
          false ->
            {ifAddr, pktIfAddr} = win_getifaddrs_iat3(desc, ipAddr,
                                                        ifEntry)
            win_getifaddrs_iat2(ipAddrTab, ipIfInfos,
                                  [ifAddr, pktIfAddr | acc])
        end
  ...
  defp win_getifaddrs_iat3(name,
            %{addr: addr, mask: mask,
                bcast_addr: _BCastAddr} = _IpAddr,
            %{type: type, admin_status: aStatus,
                internal_oper_status: _OStatus, phys_addr: physAddr,
                index: idx} = _IfEntry) do
    flags1 = (case (type) do
                :ethernet_csmacd ->
                  [:broadcast, :multicast]
                :software_loopback ->
    ...
    pktSockAddr = %{addr:
                    process_phys_addr(haType, physAddr),
                      family: :packet, hatype: haType, ifindex: idx,
                      pkttype: :host, protocol: 0}
    pktIfAddr = (case (haType) do
                   :loopback ->
                     %{name: name, addr: pktSockAddr, flags: flags}
                   _ ->
                     %{name: name, addr: pktSockAddr,
                         broadaddr:
                         <<255 :: size(8), 255 :: size(8), 255 :: size(8),
                             255 :: size(8), 255 :: size(8), 255 :: size(8)>>,
                         flags: flags}
                 end)
    ifAddr = %{name: name, flags: flags,
  ...
  defp win_getifaddrs_aa2([%{index: idx} = adAddrs | adsAddrs], ipIfInfos,
            acc) do
    case (:prim_net.get_if_entry(%{index: idx})) do
      {:ok, %{name: name} = ifEntry} when name !== '' ->
        {ifAddrs, pktIfAddr} = win_getifaddrs_aa3(name, adAddrs,
                                                    ifEntry)
      ...
      {:ok, %{name: name, description: desc} = ifEntry}
        ...
        case (if_info_search(idx, ipIfInfos)) do
          {:value, %{name: name2}} ->
            {ifAddrs, pktIfAddr} = win_getifaddrs_aa3(name2,
                                                        adAddrs, ifEntry)
            win_getifaddrs_aa2(adsAddrs, ipIfInfos,
                                 [ifAddrs, pktIfAddr | acc])
          false ->
            {ifAddrs, pktIfAddr} = win_getifaddrs_aa3(desc, adAddrs,
                                                        ifEntry)
            win_getifaddrs_aa2(adsAddrs, ipIfInfos,
                                 [ifAddrs, pktIfAddr | acc])
        end
  ...
  defp win_getifaddrs_aa3(name,
    ...
    ifAddrs = (for uCastAddr <- uCastAddrs do
  ...
  defp win_getifaddrs_aa4(name, flags, prefixes,
            %{addr: %{family: fam} = addr} = uCAddr) do
  ...
  defp shortest_matching_prefix(%{addr:
            %{family: :inet, addr: {a, _, _, _}} = addr},
            [], %{length: :undefined}) do
    shortest = (cond do
                  a < 128 ->
                    8
  ...
  defp shortest_matching_prefix(%{addr: %{family: :inet6} = addr}, [],
            %{length: :undefined}) do
  ...
  defp shortest_matching_prefix(%{addr: %{family: fam, addr: addr}} = uCAddr,
            [%{addr: %{family: fam, addr: pAddr},
                 length: pLen} = prefix |
                 prefixes],
            %{length: sPLen} = sPrefix)
      when fam === :inet do
    mask = <<4294967295 <<< (32 - pLen) :: size(32)>>
    case (masked_eq(mask, ipv4_to_bin(addr),
                      ipv4_to_bin(pAddr))) do
      true ->
        cond do
          sPLen === :undefined ->
  ...
  defp shortest_matching_prefix(%{addr: %{family: fam, addr: addr}} = uCAddr,
            ...
            %{length: sPLen} = sPrefix)
      when fam === :inet6 do
    mask = <<340282366920938463463374607431768211455 <<< (128 - pLen)
             ::
             size(128)>>
    case (masked_eq(mask, ipv6_to_bin(addr),
                      ipv6_to_bin(pAddr))) do
      true ->
        cond do
          sPLen === :undefined ->
            shortest_matching_prefix(uCAddr, prefixes, prefix)
  ...
  defp masked_eq(<<m :: size(32)>>, <<a :: size(32)>>,
            <<pA :: size(32)>>) do
    a &&& m === pA &&& m
  end

  defp masked_eq(<<m01 :: size(8), m02 :: size(8),
              m03 :: size(8), m04 :: size(8), m05 :: size(8),
              m06 :: size(8), m07 :: size(8), m08 :: size(8),
              m09 :: size(8), m10 :: size(8), m11 :: size(8),
              m12 :: size(8), m13 :: size(8), m14 :: size(8),
              m15 :: size(8), m16 :: size(8)>>,
            <<a01 :: size(8), a02 :: size(8), a03 :: size(8),
                a04 :: size(8), a05 :: size(8), a06 :: size(8),
                a07 :: size(8), a08 :: size(8), a09 :: size(8),
                a10 :: size(8), a11 :: size(8), a12 :: size(8),
                a13 :: size(8), a14 :: size(8), a15 :: size(8),
                a16 :: size(8)>>,
            <<pA01 :: size(8), pA02 :: size(8), pA03 :: size(8),
                pA04 :: size(8), pA05 :: size(8), pA06 :: size(8),
                pA07 :: size(8), pA08 :: size(8), pA09 :: size(8),
                pA10 :: size(8), pA11 :: size(8), pA12 :: size(8),
                pA13 :: size(8), pA14 :: size(8), pA15 :: size(8),
                pA16 :: size(8)>>) do
    a01 &&& m01 === pA01 &&& m01 and a02 &&& m02 === pA02 &&& m02 and a03 &&& m03 === pA03 &&& m03 and a04 &&& m04 === pA04 &&& m04 and a05 &&& m05 === pA05 &&& m05 and a06 &&& m06 === pA06 &&& m06 and a07 &&& m07 === pA07 &&& m07 and a08 &&& m08 === pA08 &&& m08 and a09 &&& m09 === pA09 &&& m09 and a10 &&& m10 === pA10 &&& m10 and a11 &&& m11 === pA11 &&& m11 and a12 &&& m12 === pA12 &&& m12 and a13 &&& m13 === pA13 &&& m13 and a14 &&& m14 === pA14 &&& m14 and a15 &&& m15 === pA15 &&& m15 and a16 &&& m16 === pA16 &&& m16
  end

  defp ipv4_to_bin({a1, a2, a3, a4}) do
    <<a1 :: size(8), a2 :: size(8), a3 :: size(8),
        a4 :: size(8)>>
  end

  defp ipv6_to_bin({a1, a2, a3, a4, a5, a6, a7, a8}) do
    <<a1 :: size(16), a2 :: size(16), a3 :: size(16),
        a4 :: size(16), a5 :: size(16), a6 :: size(16),
  ...
  defp win_getifaddrs_mask(%{addr: %{addr: _Addr, family: :inet = fam},
              length: len}) do
    <<m1 :: size(8), m2 :: size(8), m3 :: size(8),
        m4 :: size(8)>> = <<4294967295 <<< (32 - len)
                            ::
                            size(32)>>
    %{addr: {m1, m2, m3, m4}, family: fam, port: 0}
  end

  defp win_getifaddrs_mask(%{addr: %{addr: _Addr, family: :inet6 = fam},
              length: len}) do
    <<m1 :: size(16), m2 :: size(16), m3 :: size(16),
        m4 :: size(16), m5 :: size(16), m6 :: size(16),
        m7 :: size(16),
        m8
        ::
        size(16)>> = <<340282366920938463463374607431768211455 <<< (128 - len)
                       ::
                       size(128)>>
    %{addr: {m1, m2, m3, m4, m5, m6, m7, m8}, family: fam,
        flowinfo: 0, port: 0, scope_id: 0}
  end

  defp win_getifaddrs_broadaddr(%{addr: {m1, m2, m3, m4}, family: fam} = _Mask,
            %{addr: %{addr: {pA1, pA2, pA3, pA4}}} = _Prefix) do
    bA1 = 255 &&& (pA1 ||| ~~~ m1)
    bA2 = 255 &&& (pA2 ||| ~~~ m2)
    bA3 = 255 &&& (pA3 ||| ~~~ m3)
    bA4 = 255 &&& (pA4 ||| ~~~ m4)
  ...
  defp win_indexes() do
    case (:prim_net.get_adapters_addresses(%{flags:
                                             %{skip_unicast: true,
                                                 skip_friendly_name: true,
                                                 include_prefix: false}})) do
      {:ok, aA} ->
        indexes = (for %{index: idx} <- aA do
                     idx
                   end)
        :lists.sort(indexes)
      {:error, _} ->
