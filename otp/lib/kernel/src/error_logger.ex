defmodule :m_error_logger do
          ...
          case get_report_cb(meta) do
            rCBFun when is_function(rCBFun, 1) ->
              try do
                ...
                c, r ->
                  {~c"REPORT_CB CRASH: ~tp; Reason: ~tp", [msg, {c, r, __STACKTRACE__}]}
              else
                {f, a} when is_list(f) and is_list(a) ->
                  {f, a}

                other ->
                  {~c"REPORT_CB ERROR: ~tp; Returned: ~tp", [msg, other]}
              end
            ...
            rCBFun when is_function(rCBFun, 2) ->
              try do
                ...
                c, r ->
                  {~c"REPORT_CB CRASH: ~tp; Reason: ~tp", [msg, {c, r}]}
              else
                chardata
  ...
  def get_format_depth() do
    case :application.get_env(:kernel, :error_logger_format_depth, :unlimited) do
