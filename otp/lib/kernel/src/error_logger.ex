defmodule :m_error_logger do
    ...
    case (:erlang.whereis(:error_logger)) do
      :undefined ->
        ...
        case (:supervisor.start_child(:logger_sup,
                                        errorLogger)) do
          {:ok, pid} ->
            :ok = :logger_handler_watcher.register_handler(:error_logger,
                                                             pid)
          error ->
            error
        end
  ...
  def stop() do
    case (:erlang.whereis(:error_logger)) do
      :undefined ->
        :ok
      _Pid ->
        _ = :gen_event.stop(:error_logger,
                              {:shutdown, :stopped}, :infinity)
  ...
  def log(%{level: level, msg: msg, meta: meta},
           _Config) do
    do_log(level, msg, meta)
  end

  defp do_log(level, {:report, msg},
            %{error_logger: %{tag: tag, type: type}} = meta) do
    report = (case (msg) do
                %{label: _, report: r} ->
                  r
                _ ->
  ...
  defp do_log(level, {:report, msg},
            %{error_logger: %{tag: tag}} = meta) do
    {format, args} = (case (msg) do
  ...
  defp do_log(level, {format, args},
            %{error_logger: %{tag: tag}} = meta)
  ...
  defp notify(level, tag0, formatOrType0, argsOrReport,
            %{pid: pid0, gl: gL, error_logger: my}) do
    {tag, formatOrType} = maybe_map_warnings(level, tag0,
                                               formatOrType0)
    pid = (case (:maps.get(:emulator, my, false)) do
             true ->
               :emulator
             _ ->
               pid0
    ...
    case (:error_logger.warning_map()) do
      ...
      level ->
        {fix_warning_tag(level, tag),
           fix_warning_type(level, formatOrType)}
    end
  end
  ...
  def error_msg(format, args) do
    :logger.log(:error,
                  %{label: {:error_logger, :error_msg}, format: format,
                      args: args},
                  meta(:error))
  end
  ...
  def warning_report(type, report) do
    :logger.log(:warning,
                  %{label: {:error_logger, :warning_report},
                      report: report},
  ...
  def warning_msg(format, args) do
    :logger.log(:warning,
                  %{label: {:error_logger, :warning_msg}, format: format,
                      args: args},
                  meta(:warning_msg))
  end
  ...
  def info_msg(format, args) do
    :logger.log(:notice,
                  %{label: {:error_logger, :info_msg}, format: format,
                      args: args},
                  meta(:info_msg))
  end

  def error_info(error) do
    {format, args} = (case (string_p(error)) do
                        true ->
                          {error, []}
                        false ->
                          {'~p', [error]}
    ...
    :logger.log(:notice, format, args,
                  %{error_logger: myMeta, domain: [error]})
  end
  ...
  defp meta(tag, type, meta0) do
    maybe_add_domain(tag, type,
                       Map.put(meta0, :error_logger, %{tag: tag, type: type}))
  end
  ...
  defp report_to_format(%{label: {:error_logger, _}, format: format,
              args: args}) do
    try do
  ...
  def add_report_handler(module, args) when is_atom(module) do
    _ = :logger.add_handler(:error_logger, :error_logger,
  ...
  def delete_report_handler(module) when is_atom(module) do
    case (:erlang.whereis(:error_logger)) do
      pid when is_pid(pid) ->
        return = :gen_event.delete_handler(:error_logger,
                                             module, [])
  ...
  def logfile(:close) do
    case (:erlang.whereis(:error_logger)) do
      pid when is_pid(pid) ->
        case (:gen_event.delete_handler(:error_logger,
                                          :error_logger_file_h, :normal)) do
          {:error, reason} ->
            {:error, reason}
          _ ->
            :ok
  ...
  def logfile(:filename) do
    case (:erlang.whereis(:error_logger)) do
      pid when is_pid(pid) ->
        case (:gen_event.call(:error_logger,
                                :error_logger_file_h, :filename)) do
          {:error, _} ->
            {:error, :no_log_file}
          val ->
            val
  ...
  def tty(true) do
    _ = (case (:lists.member(:error_logger_tty_h,
                               which_report_handlers())) do
           false ->
             case (:logger.get_handler_config(:default)) do
               ...
               _ ->
                 :logger.add_handler(:error_logger_tty_true,
                                       :logger_std_h,
                                       %{filter_default: :stop,
                                           filters:
                                           [{:remote_gl,
  ...
  def tty(false) do
    ...
    _ = (case (:logger.get_handler_config(:default)) do
           {:ok,
              %{module: :logger_std_h,
                  config: %{type: :standard_io}}} ->
  ...
  def get_format_depth() do
    case (:application.get_env(:kernel,
                                 :error_logger_format_depth, :unlimited)) do
