defmodule :m_group_history do
    ...
    case (history_status()) do
      ...
      provider ->
        try do
          ...
          e, r ->
            show_custom_provider_crash(provider, e, r,
                                         __STACKTRACE__)
            disable_history()
            []
        else
  ...
  defp log_options() do
    ...
    [{:name, :"$#group_history"}, {:file, file}, {:repair, true}, {:format,
                                                     :internal},
                                                      {:type, :wrap}, {:size,
                                                                         size},
                                                                          {:notify,
  ...
  defp read_full_log(name) do
    case (:disk_log.chunk(name, :start)) do
      ...
      :eof ->
        []
      {cont, logs} ->
        :lists.reverse(maybe_drop_header(logs) ++ read_full_log(name,
                                                                  cont))
    end
  end
  ...
  defp handle_open_error({:arg_mismatch, optName, currentVal, newVal}) do
    show(:"$#erlang-history-arg-mismatch", 'Log file argument ~p changed value from ~p to ~p and cannot be automatically updated. Please clear the history files and try again.~n', [optName, currentVal, newVal])
  end
  ...
  defp handle_open_error({:invalid_header, term}) do
    show(:"$#erlang-history-invalid-header", 'Shell history expects to be able to use the log files which currently have unknown headers (~p) and may belong to another mechanism.~n', [term])
  end

  defp handle_open_error({:file_error, fileName, reason}) do
    show(:"$#erlang-history-file-error", 'Error handling file ~ts. Reason: ~p~n', [fileName, reason])
  end
  ...
  defp disk_log_info(tag) do
    {^tag, value} = :lists.keyfind(:size, 1,
                                     :disk_log.info(:"$#group_history"))
    value
  end

  defp find_wrap_values() do
    confSize = :application.get_env(:kernel,
                                      :shell_history_file_bytes, 1024 * 512)
  ...
  defp report_repairs(_, good, bad) do
    show(:"$#erlang-history-report-repairs", 'The shell history log file was corrupted and was repaired. ~p bytes were recovered and ~p were lost.~n', [good, bad])
  end

  defp resize_log(name, _OldSize, newSize) do
    show(:"$#erlang-history-resize-attempt", 'Attempting to resize the log history file to ~p...', [newSize])
    _ = (case (open_log_no_size()) do
           {:error, {:need_repair, _}} ->
             _ = repair_log(name)
             _ = open_log_no_size()
           _ ->
    ...
    case (:disk_log.change_size(name, newSize)) do
      :ok ->
        show(:"$#erlang-history-resize-result", 'resized the log history file~n', [])
      {:error, {:new_size_too_small, _, _}} ->
        show(:"$#erlang-history-resize-result", 'failed to resize the log history file (new size is too small)~n', [])
        disable_history()
      {:error, reason} ->
        show(:"$#erlang-history-resize-result", 'failed to resize the log history file (~p)~n', [reason])
        disable_history()
    end
  ...
  defp upgrade_version(_Name, unsupported) do
    show(:"$#erlang-history-upgrade", 'The version for the shell logs found on disk (~p) is not supported by the current version (~p)~n', [unsupported, :EFE_TODO_VSN_MACRO])
    disable_history()
  end
  ...
  defp show_rename_warning() do
    show(:"$#erlang-history-rename-warn", 'A history file with a different path has already been started for the shell of this node. The old name will keep being used for this session.~n', [])
  end

  defp show_invalid_chunk_warning(name, reason) do
    show(:"$#erlang-history-invalid-chunk-warn", 'Invalid chunk in the file ~ts.~nSome entries may have been lost.~nReason ~p.',
           [:proplists.get_value(:file, :disk_log.info(name)),
                reason])
  end

  defp show_invalid_file_warning(fileName) do
    show(:"$#erlang-history-invalid-file", 'Shell history expects to be able to use the file ~ts which currently exists and is not a file usable for history logging purposes.~n', [fileName])
  end

  defp show_unexpected_warning({m, f, a}, term) do
    show(:"$#erlang-history-unexpected-return", 'unexpected return value from ~p:~p/~p: ~p~nshell history will be disabled for this session.~n', [m, f, a, term])
  end

  defp show_unexpected_close_warning() do
    show(:"$#erlang-history-unexpected-close", 'The shell log file has mysteriously closed. Ignoring currently unread history.~n', [])
  end

  defp show_size_warning(_Current, _New) do
    show(:"$#erlang-history-size", 'The configured log history file size is different from the size of the log file on disk.~n', [])
  end

  defp show_custom_provider_crash(provider, class, reason, stackTrace) do
    show(:"$#erlang-history-custom-crash", 'The configured custom shell_history provider \'~p\' crashed. ~nDid you mean to write \'enabled\'?~n~ts~n',
           [provider, :erl_error.format_exception(class, reason,
                                                    stackTrace)])
  end

  defp show_custom_provider_faulty_load_return(provider, return) do
    show(:"$#erlang-history-custom-return", 'The configured custom shell_history provider ~p:load/0 did not return a list.~nIt returned ~p.~n', [provider, return])
  end

  defp show_custom_provider_faulty_add_return(provider, return) do
    show(:"$#erlang-history-custom-return", 'The configured custom shell_history provider ~p:add/1 did not return ok.~nIt returned ~p.~n', [provider, return])
  end
        ...
        case (:logger.allow(:error, :group_history)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:group_history, :show, 3}, line: 443,
                                 file: 'otp/lib/kernel/src/group_history.erl'},
                                 :error, format, args])
          false ->
            :ok
        end
