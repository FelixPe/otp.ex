defmodule :m_inet_dns do
  ...
  defp decode_query_section(bin, n, buffer, qs) do
    case decode_name(bin, buffer) do
      {<<t::size(16), c::size(16), rest::binary>>, name} ->
    ...
    case decode_name(bin, buffer) do
      {<<t::size(16)-unsigned, c::size(16)-unsigned, tTL::size(4)-binary, len::size(16),
        ...
        rR =
          case type do
            :opt ->
              <<extRcode, version, dO::size(1), z::size(15)>> = tTL
              dnssecOk = dO !== 0
  ...
  defp encode_query_section(bin0, comp0, [r_dns_query(domain: dName) = q | qs]) do
    ...
    encode_query_section(<<bin::binary, t::size(16), c::size(16)>>, comp, qs)
  end
  ...
  defp encode_res_section(bin, comp, [
         r_dns_rr(domain: dName, type: type, class: class, func: cacheFlush, ttl: tTL, data: data)
         | rs
       ]) do
  ...
  defp encode_res_section_rr(bin0, comp0, rs, dName, type, class, cacheFlush, tTL, data) do
    t = encode_type(type)
  ...
  defp decode_data(data, :loc, _) do
    case data do
      <<version::size(8), sizeBase::size(4), sizeExp::size(4), horizPreBase::size(4),
        horizPreExp::size(4), vertPreBase::size(4), vertPreExp::size(4), latitude::size(32),
        longitude::size(32), altitude::size(32)>>
      when not (not (version === 0 and (0 <= sizeBase and sizeBase <= 9) and
                       (0 <= sizeExp and sizeExp <= 9) and
                       (0 <= horizPreBase and horizPreBase <= 9) and
                       (0 <= horizPreExp and horizPreExp <= 9) and
                       (0 <= vertPreBase and vertPreBase <= 9) and 0 <= vertPreExp and
                         vertPreExp <= 9)) ->
        {{decode_loc_angle(latitude), decode_loc_angle(longitude)}, decode_loc_altitude(altitude),
         decode_loc_size(sizeBase, sizeExp),
         {decode_loc_size(horizPreBase, horizPreExp), decode_loc_size(vertPreBase, vertPreExp)}}
  ...
  defp decode_data(data, :naptr, buffer) do
    case data do
      <<order::size(16), preference::size(16), data1::binary>> ->
        ...
        {order, preference, :inet_db.tolower(flags), :inet_db.tolower(services), regexp,
         replacement}
  ...
  defp decode_data(data, :uri, _) do
    case data do
      <<prio::size(16), weight::size(16), data1::binary>>
  ...
  defp decode_name_labels([label | labels], name) do
    decode_name_labels(
      labels,
      ~c"." ++ decode_name_label(label, name)
    )
  end
  ...
  defp encode_data(comp, pos, :soa, data) do
    ...
    {<<b::binary, serial::size(32), refresh::size(32)-signed, retry::size(32)-signed,
       expiry::size(32)-signed, minimum::size(32)>>, comp2}
  end
  ...
  defp encode_data(comp, _, :loc, data) do
    ...
    {<<version::size(8), encode_loc_size(size)::binary, encode_loc_size(horizPre)::binary,
       encode_loc_size(vertPre)::binary, encode_loc_angle(latitude)::size(32),
       encode_loc_angle(longitude)::size(32), encode_loc_altitude(altitude)::size(32)>>, comp}
  end
  ...
  defp encode_data(comp, _, :uri, data) do
    ...
    {<<prio::size(16), weight::size(16), :erlang.iolist_to_binary(target)::binary>>, comp}
  end
  ...
  defp name2labels(label, ~c"\\" ++ [c | cs]) do
  ...
  defp decode_loc_angle(x) do
    (x - 2_147_483_648) / 3_600_000
  end

  defp encode_loc_angle(x) when is_float(x) do
    encode_loc_angle(round(x * 3_600_000))
  end

  defp encode_loc_angle(x)
       when is_integer(x) and -2_147_483_648 <= x and
              x <= 2_147_483_647 do
    x + 2_147_483_648
  end
  ...
  defp encode_loc_altitude(x)
       when is_integer(x) and -10_000_000 <= x and
              x <= 4_294_967_295 - 10_000_000 do
    x + 10_000_000
  end
  ...
  defp encode_loc_size(x)
       when is_integer(x) and 0 <= x and
              x <= 9_000_000_000 do
