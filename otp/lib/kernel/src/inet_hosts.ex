defmodule :m_inet_hosts do
  ...
  def gethostbyname(name, type) when (is_list(name) and
                             is_atom(type)) do
    nm = :inet_db.tolower(name)
    case (gethostbyname(nm, type, :inet_hosts_byname)) do
      false ->
        case (gethostbyname(nm, type,
                              :inet_hosts_file_byname)) do
          false ->
            {:error, :nxdomain}
          hostent ->
            {:ok, hostent}
  ...
  def gethostbyname(name, type) when (is_atom(name) and
                             is_atom(type)) do
  ...
  defp gethostbyaddr(iP, type) do
    case (gethostbyaddr(iP, type, :inet_hosts_byaddr)) do
      false ->
        case (gethostbyaddr(iP, type,
                              :inet_hosts_file_byaddr)) do
          false ->
            {:error, :nxdomain}
          hostent ->
            {:ok, hostent}
