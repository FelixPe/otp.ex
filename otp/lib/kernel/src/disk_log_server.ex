defmodule :m_disk_log_server do
  ...
  Record.defrecord(:r_pending, :pending, log: :undefined,
                                   pid: :undefined, req: :undefined,
                                   from: :undefined, attach: :undefined,
                                   clients: :undefined)
  ...
  def open({:ok, a}) do
    ensure_started()
    :gen_server.call(:disk_log_server, {:open, a},
                       :infinity)
  end
  ...
  def close(pid) do
    :gen_server.call(:disk_log_server, {:close, pid},
                       :infinity)
  end
  ...
  def handle_info({:pending_reply, pid, result0}, state) do
    {:value,
       r_pending(log: name, pid: ^pid, from: from, req: request,
           attach: attach,
           clients: clients)} = :lists.keysearch(pid, r_pending(:pid),
  ...
  defp ensure_started() do
    case (:erlang.whereis(:disk_log_server)) do
      :undefined ->
        logSup = {:disk_log_sup,
                    {:disk_log_sup, :start_link, []}, :permanent, 1000,
                    :supervisor, [:disk_log_sup]}
        {:ok, _} = ensure_child_started(:kernel_safe_sup,
                                          logSup)
        logServer = {:disk_log_server,
                       {:disk_log_server, :start_link, []}, :permanent, 2000,
                       :worker, [:disk_log_server]}
        {:ok, _} = ensure_child_started(:kernel_safe_sup,
                                          logServer)
        :ok
      _ ->
  ...
  defp open([{req, from} | l], state) do
    state2 = (case (do_open(req, from, state)) do
                {:pending, state1} ->
                  state1
                {reply, state1} ->
  ...
  defp start_log(name, req, from, state) do
    server = self()
    case (:supervisor.start_child(:disk_log_sup,
                                    [server])) do
      {:ok, pid} ->
  ...
  defp do_internal_open(name, pid, from, {:open, a} = req, attach,
            state) do
    server = self()
    f = fn () ->
  ...
  defp do_all() do
    localSpec = {:"$1", :_}
    local0 = (for l <- :ets.match(:disk_log_names,
                                    localSpec) do
                hd(l)
              end)
    :lists.sort(local0)
