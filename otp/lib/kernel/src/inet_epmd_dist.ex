defmodule :m_inet_epmd_dist do
  ...
  def hs_data(netAddress, socket) do
    nodelay = nodelay()
    r_hs_data(socket: socket, f_send: &:inet_tcp.send/2,
  ...
  defp f_setopts_post_nodeup(socket, nodelay) do
    :inet.setopts(socket,
                    [{:active, true}, {:packet, 4}, {:deliver, :port},
                                                        :binary, nodelay])
  end
  ...
  def f_address(netAddress, node) do
    case (:dist_util.split_node(node)) do
      ...
      other ->
        :dist_util.shutdown(:inet_epmd_dist, 214, node,
                              {:split_node, other})
    end
  end
  ...
  def tick(socket) when is_port(socket) do
    ...
    _ = result === {:error,
                      :closed} and send(self(), {:tcp_closed, socket})
    result
  end
  ...
  def address(host) do
    try do
      ...
      :error, reason ->
        :error_logger.error_msg('error : ~p in ~n    ~p~n', [reason, __STACKTRACE__])
  ...
  defp listen_loop(port, netAddress, listenOptions, lastPort,
            distMod)
      when port <= lastPort do
    case (distMod.listen_port(netAddress, port,
                                listenOptions)) do
      {:error, :eaddrinuse} ->
        listen_loop(port + 1, netAddress, listenOptions,
                      lastPort, distMod)
      result ->
        result
    end
  ...
  def accept({netAddress, stateL}) do
    try do
      ...
      acceptLoop = spawn_link(fn () ->
                                   _ = :erlang.process_flag(:trap_exit, true)
                                   accept_loop(stateL, netAddress, netKernel,
                                                 distMod,
  ...
  defp accept_loop(stateL, netAddress, netKernel, distMod,
            maxPending, acceptors)
      when map_size(acceptors) <= maxPending do
    acceptRef = make_ref()
    acceptor = spawn_link(acceptor_fun(stateL, netAddress,
                                         netKernel, distMod, acceptRef))
    accept_loop(stateL, netAddress, netKernel, distMod,
                  maxPending, Map.put(acceptors, acceptor, acceptRef))
  end

  defp accept_loop(stateL, netAddress, netKernel, distMod,
            maxPending, acceptors) do
    receive do
      msg ->
        case (msg) do
          {:EXIT, acceptor, reason}
            ...
            case (reason) do
              ...
              {:accept, _} ->
                exit(reason)
              _ ->
                :error_logger.warning_msg('~w:~w acceptor ~w failed: ~p',
                                            [:inet_epmd_dist, :accept_loop,
                                                                  acceptor,
                                                                      reason])
                accept_loop(stateL, netAddress, netKernel, distMod,
                              maxPending, :maps.remove(acceptor, acceptors))
          ...
          {:EXIT, ^netKernel, reason} ->
            exit(reason)
          _ ->
            :error_logger.warning_msg('~w:~w unknown message: ~p',
  ...
  defp acceptor_fun(stateL,
            r_net_address(family: family, protocol: protocol) = netAddress,
            netKernel, distMod, acceptRef) do
    fn () ->
         {stateA, peerAddress} = distMod.accept_open(netAddress,
                                                       stateL)
         netAddress_1 = r_net_address(netAddress, address: peerAddress)
         acceptor = self()
         send(netKernel, {:accept, acceptor, netAddress_1,
                            family, protocol})
         receive do
           {^netKernel, :controller, controller} ->
             stateD = distMod.accept_controller(netAddress_1,
                                                  controller, stateA)
             send(controller, {acceptor, :controller, distMod,
                                 stateD})
             exit(acceptRef)
  ...
  def accept_connection(acceptor, netAddress, myNode, allowed,
           setupTime) do
    try do
      netKernel = self()
      controller = :erlang.spawn_opt(fn () ->
                                          accept_controller(acceptor,
                                                              netAddress,
                                                              myNode, allowed,
                                                              setupTime,
                                                              netKernel)
  ...
  defp accept_controller(acceptor, netAddress, myNode, allowed,
    ...
    receive do
      {^acceptor, :controller, distMod, stateD} ->
        ...
        case (distMod.accepted(netAddress, timer, stateD)) do
          r_hs_data() = hsData ->
            :dist_util.handshake_other_started(r_hs_data(hsData, kernel_pid: netKernel, 
                                                           this_node: myNode, 
                                                           timer: timer, 
                                                           allowed: allowed))
          {:error, reason} ->
            :dist_util.shutdown(:inet_epmd_dist, 490,
                                  {{distMod, :accepted}, reason})
        end
    end
  end
  ...
  defp setup(node, type, myNode, longOrShortNames, setupTime,
            netKernel) do
    timer = :dist_util.start_timer(setupTime)
    distMod = pt_get(:dist_mod)
    ...
    {address, version} = (case (call_epmd_function(erlEpmd,
                                                     :address_please,
                                                     [name, host, family])) do
                            {:ok, ip, port, ver} ->
                              {{ip, port}, ver}
                            {:ok, ip} ->
                              case (erlEpmd.port_please(name, ip)) do
                                {:port, port, ver} ->
                                  {{ip, port}, ver}
                                other ->
                                  :dist_util.shutdown(:inet_epmd_dist, 561,
                                                        node,
                                                        {:port_please, other})
                              end
                            other ->
                              :dist_util.shutdown(:inet_epmd_dist, 564, node,
                                                    {:address_please, other})
                          end)
    netAddress_1 = r_net_address(netAddress, host: host, 
                                   address: address)
    :dist_util.reset_timer(timer)
    case (distMod.connect(netAddress_1, timer,
                            connect_options())) do
      r_hs_data() = hsData ->
        :dist_util.handshake_we_started(r_hs_data(hsData, kernel_pid: netKernel, 
                                                    other_node: node, 
                                                    this_node: myNode, 
                                                    timer: timer, 
                                                    ...
                                                    request_type: type))
      {:error, reason} ->
        :dist_util.shutdown(:inet_epmd_dist, 586, node,
                              {{distMod, :connect}, reason})
    end
  end
  ...
  defp split_node(node, longOrShortNames, family) do
    case (:dist_util.split_node(node)) do
          ...
          longOrShortNames === :longnames ->
            case (:inet.parse_strict_address(host, family)) do
              {:ok, _} ->
                {name, host}
              {:error, reason} ->
                :error_logger.error_msg('** System running to use fully qualified hostnames **~n** Hostname ~ts is illegal **~n', [host])
                :dist_util.shutdown(:inet_epmd_dist, 672, node,
                                      {:parse_address, reason})
            end
          (longOrShortNames === :shortnames and 0 < dots) ->
            :error_logger.error_msg('** System NOT running to use fully qualified hostnames **~n** Hostname ~ts is illegal **~n', [host])
      ...
      other ->
        ...
        :dist_util.shutdown(:inet_epmd_dist, 686, node,
                              {:split_node, other})
    end
  end
  ...
  defp pt_get(key) when key === :dist_mod or
                      key === :net_address do
