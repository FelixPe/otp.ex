defmodule :m_erl_distribution do
  ...
  def start_link() do
    do_start_link([{:sname, :shortnames}, {:name,
                                             :longnames}])
  end
  ...
  def start(opts) do
    c = %{id: :net_sup_dynamic,
            ...
            {:erl_distribution, :start_link,
               [Map.merge(opts, %{clean_halt: false,
  ...
  def stop() do
    case (:supervisor.terminate_child(:kernel_sup,
                                        :net_sup_dynamic)) do
      :ok ->
  ...
  def start_link(opts) do
    :supervisor.start_link({:local, :net_sup},
                             :erl_distribution, [opts])
  end

  def init(netArgs) do
    epmd = (case (:init.get_argument(:no_epmd)) do
              {:ok, [[]]} ->
                []
              _ ->
                epmdMod = :net_kernel.epmd_module()
                [%{id: epmdMod, start: {epmdMod, :start_link, []},
                     restart: :permanent, shutdown: 2000, type: :worker,
                     modules: [epmdMod]}]
            end)
    auth = %{id: :auth, start: {:auth, :start_link, []},
               restart: :permanent, shutdown: 2000, type: :worker,
               modules: [:auth]}
  ...
  defp do_start_link([{arg, flag} | t]) do
    case (:init.get_argument(arg)) do
      {:ok, [[name]]} ->
        start_link(%{name: :erlang.list_to_atom(name),
                       name_domain: flag, clean_halt: true,
                       supervisor: :net_sup})
      {:ok, [[name] | _Rest]} ->
          ...
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:erl_distribution, :do_start_link, 1},
                                 line: 113, file: 'otp/lib/kernel/src/erl_distribution.erl'},
                                 :warning, 'Multiple -~p given to erl, using the first, ~p', [arg, name]])
          false ->
            :ok
        end
        start_link(%{name: :erlang.list_to_atom(name),
      ...
      {:ok, [invalid | _]} ->
        case (:logger.allow(:error, :erl_distribution)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:erl_distribution, :do_start_link, 1},
                                 line: 120, file: 'otp/lib/kernel/src/erl_distribution.erl'},
                                 :error, 'Invalid -~p given to erl, ~ts', [arg, :lists.join(' ', invalid)]])
          false ->
            :ok
        end
        do_start_link(t)
