defmodule :m_ram_file do
  ...
  def copy(r_file_descriptor(module: :ram_file) = source,
           r_file_descriptor(module: :ram_file) = dest, length)
  ...
  def position(r_file_descriptor(module: :ram_file, data: port), pos) do
    case (lseek_position(pos)) do
      {:ok, offs, whence} when (is_integer(offs) and
  ...
  def pread(r_file_descriptor(module: :ram_file, data: port), at, sz)
    ...
    cond do
      (is_integer(at) and at >= - (1 <<< 31) and
         ...
         sz >= - (1 <<< 31) and sz < 1 <<< 31) ->
        case (call_port(port,
                          <<17 :: size(8), at :: size(32), sz :: size(32)>>)) do
          {:ok, {0, _Data}} when sz !== 0 ->
            :eof
          {:ok, {_Sz, data}} ->
            {:ok, data}
    ...
    cond do
      ...
      (is_integer(at) and at >= - (1 <<< 31) and
        ...
        try do
          ...
          bin ->
            pwrite_1(port, t, r + 1,
                       [<<18 :: size(8), at :: size(32), bin :: binary>> | cs])
        end
      true ->
        {:error, {r, :einval}}
    end
  ...
  def advise(r_file_descriptor(module: :ram_file, data: port), offset, length,
           advise) do
    cmd0 = <<38, offset :: size(64) - signed,
               length :: size(64) - signed>>
    case (advise) do
      :normal ->
        call_port(port,
  ...
  def allocate(r_file_descriptor(module: :ram_file, data: port), offset,
           length) do
    call_port(port,
                <<39, offset :: size(64) - signed,
                    length :: size(64) - signed>>)
  end
  ...
  defp call_port(port, command) when (is_port(port) and
