defmodule :m_inet do
  ...
  def is_ipv4_address({a, b, c, d}) when (a ||| b ||| c ||| d) &&& ~~~255 === 0 do
    true
  end
  ...
  defp con_opt([opt | opts], r_connect_opts(ifaddr: ifAddr) = r, as) do
    case opt do
      ...
      {:ip, iP} when is_map(ifAddr) ->
        con_opt(opts, r_connect_opts(r, ifaddr: Map.put(ifAddr, :addr, iP)), as)

      {:ip, iP} ->
        con_opt(opts, r_connect_opts(r, ifaddr: iP), as)

      {:port, p} when is_map(ifAddr) ->
        con_opt(opts, r_connect_opts(r, ifaddr: Map.put(ifAddr, :port, p)), as)
  ...
  defp list_opt([opt | opts], r_listen_opts(ifaddr: ifAddr) = r, as) do
    case opt do
      ...
      {:ip, iP} when is_map(ifAddr) ->
        list_opt(opts, r_listen_opts(r, ifaddr: Map.put(ifAddr, :addr, iP)), as)

      {:ip, iP} ->
        list_opt(opts, r_listen_opts(r, ifaddr: iP), as)

      {:port, p} when is_map(ifAddr) ->
        list_opt(opts, r_listen_opts(r, ifaddr: Map.put(ifAddr, :port, p)), as)
  ...
  defp udp_opt([opt | opts], r_udp_opts(ifaddr: ifAddr) = r, as) do
    case opt do
      ...
      {:ip, iP} when is_map(ifAddr) ->
        udp_opt(opts, r_udp_opts(r, ifaddr: Map.put(ifAddr, :addr, iP)), as)

      {:ip, iP} ->
        udp_opt(opts, r_udp_opts(r, ifaddr: iP), as)

      {:port, p} when is_map(ifAddr) ->
        udp_opt(opts, r_udp_opts(r, ifaddr: Map.put(ifAddr, :port, p)), as)
      ...
      when (membership === :add_membership or membership === :drop_membership) and
             tuple_size(mAddr) === 4 and (if__ === :any or tuple_size(if__) === 4) ->
  ...
  defp udp_module_1(opts, address) do
    mod(opts, :udp_module, address, %{inet: :inet_udp, inet6: :inet6_udp, local: :local_udp})
  end
  ...
  defp sctp_opt([opt | opts], mod, r_sctp_opts(ifaddr: ifAddr) = r, as) do
    case opt do
      {:ifaddr, addr} when is_map(addr) ->
        sctp_opt(opts, mod, r_sctp_opts(r, ifaddr: ensure_sockaddr(addr)), as)
      ...
      {:ip, iP} when is_map(ifAddr) ->
        ...
        sctp_opt(opts, mod, r_sctp_opts(r, ifaddr: Map.put(ifAddr, :addr, iP2)), as)
      ...
      {:port, port} ->
        case mod.getserv(port) do
          {:ok, p} when is_map(ifAddr) ->
            sctp_opt(opts, mod, r_sctp_opts(r, ifaddr: Map.put(ifAddr, :port, p)), as)
    ...
    case :inet_db.tolower(self) do
      ...
      _ ->
        case :inet_db.res_option(:domain) do
          ...
          domain ->
            fQDN = :lists.append([self, ~c".", domain])
  ...
  defp gethostbyaddr_self(iP, type) do
    ...
    case :inet_db.res_option(:domain) do
      ...
      domain ->
        {:ok, make_hostent(name ++ ~c"." ++ domain, [iP], [name], type)}
    end
  end
  ...
  def open(fd, bAddr, bPort, opts, protocol, family, type, module)
      when is_integer(fd) and 0 <= fd do
    open_fd(fd, bAddr, bPort, opts, protocol, family, type, module)
  end

  def open(fd_or_OpenOpts, bAddr, bPort, opts, protocol, family, type, module) do
    open_opts(
      fd_or_OpenOpts,
      cond do
  ...
  def open_bind(fd, bAddr, bPort, opts, protocol, family, type, module)
      when is_integer(fd) and 0 <= fd do
    open_fd(fd, bAddr, bPort, opts, protocol, family, type, module)
  end

  def open_bind(fd_or_OpenOpts, bAddr, bPort, opts, protocol, family, type, module) do
    open_opts(
      fd_or_OpenOpts,
      cond do
        bAddr === :undefined ->
  ...
  defp open_fd(fd, bAddr, bPort, opts, protocol, family, type, module) do
    ...
    case :prim_inet.fdopen(protocol, family, type, fd, doNotBind) do
      {:ok, s} ->
  ...
  defp open_opts(fd_or_OpenOpts, bAddr, bPort, opts, protocol, family, type, module) do
    openOpts =
      cond do
    ...
    case f do
      ...
      :port ->
        case genSocketMod.getopts(s, [:fd]) do
          {:ok, [{:fd, fD}]} ->
            ~c"esock[" ++ :erlang.integer_to_list(fD) ++ ~c"]"
  ...
  defp fmt_compat_status_merge([rFlag | rFlags], wFlags, merged) do
    fmt_compat_status_merge(rFlags, :lists.delete(rFlag, wFlags), [rFlag | merged])
  end
  ...
  defp fmt_addr({:ok, addr}, proto) do
    case addr do
      ...
      {:local, path} ->
        ~c"local:" ++ :erlang.binary_to_list(path)
  ...
  defp tcp_sockets() do
    port_list(~c"tcp_inet") ++ :gen_tcp_socket.which_sockets()
  end

  defp udp_sockets() do
    port_list(~c"udp_inet") ++ :gen_udp_socket.which_sockets()
  end

  defp sctp_sockets() do
    port_list(~c"sctp_inet")
  end
  ...
  def format_error(:exbadport) do
    ~c"invalid port state"
  end

  def format_error(:exbadseq) do
    ~c"bad command sequence"
  end
