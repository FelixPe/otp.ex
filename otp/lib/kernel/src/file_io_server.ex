defmodule :m_file_io_server do
  use Bitwise
  require Record
  Record.defrecord(:r_state, :state, handle: :undefined,
                                 owner: :undefined, mref: :undefined,
                                 name: :undefined, buf: :undefined,
                                 read_mode: :undefined, unic: :undefined)
  ...
  def start(owner, fileName, modeList)
      when (is_pid(owner) and
              is_list(fileName) or is_binary(fileName) and
              is_list(modeList)) do
                          ...
                          {readMode, unicodeMode, opts0} ->
                            ...
                            case (:raw_file_io.open(fileName, [:raw | opts])) do
                              ...
                              {:ok, handle} ->
                                ...
                                send(self, {ref, :ok})
                                server_loop(r_state(handle: handle, owner: owner,
                                                mref: m, name: fileName,
  ...
  defp file_request({:advise, offset, length, advise},
            r_state(handle: handle) = state) do
    case (apply(r_file_descriptor(handle, :module), :advise,
                  [handle, offset, length, advise])) do
      {:error, reason} = reply ->
  ...
  defp file_request({:allocate, offset, length},
            r_state(handle: handle) = state) do
    reply = apply(r_file_descriptor(handle, :module), :allocate,
                    [handle, offset, length])
    {:reply, reply, state}
  end

  defp file_request({:pread, at, sz}, state) when at === :cur or
                                          at === {:cur, 0} do
    case (get_chars(sz, :latin1, state)) do
      {:reply, reply, newState} when is_list(reply) or
                                       is_binary(reply)
                                     ->
  ...
  defp file_request({:pread, at, sz},
            r_state(handle: handle, buf: buf) = state) do
  ...
  defp file_request({:pwrite, at, data},
            r_state(handle: handle, buf: buf) = state) do
    case (position(handle, at, buf)) do
      {:error, _} = reply ->
        {:error, reply, state}
  ...
  defp file_request({:read_handle_info, opts},
            r_state(handle: handle) = state) do
    case (apply(r_file_descriptor(handle, :module), :read_handle_info,
                  [handle, opts])) do
      {:error, reason} = reply ->
        {:stop, reason, reply, state}
      reply ->
  ...
  defp file_request(:pid2name, r_state(name: name) = state) do
    {:reply, {:ok, name}, state}
  end
  ...
  defp io_request({:put_chars, enc, chars},
            r_state(buf: <<>>) = state) do
  ...
  defp io_request({:put_chars, enc, mod, func, args},
            r_state() = state) do
    case ((try do
            apply(mod, func, args)
          catch
  ...
  defp io_request({:get_until, enc, _Prompt, mod, func, xtraArgs},
            r_state() = state) do
    get_chars(:io_lib, :get_until, {mod, func, xtraArgs},
                enc, state)
  end
  ...
  defp io_request({:get_line, outEnc, _Prompt},
            r_state(buf: buf, read_mode: mode, unic: inEnc) = state0) do
    try do
      workEnc = (case (inEnc) do
                   {_, _} ->
                     outEnc
                   _ ->
                     inEnc
                 end)
      {res, state} = get_line(:start,
                                convert_enc(buf, inEnc, workEnc), workEnc,
                                state0)
      {:reply, cast(res, mode, workEnc, outEnc), state}
    catch
      :exit, exError ->
        {:stop, exError, {:error, exError},
           r_state(state0, buf: <<>>)}
    end
  end
  ...
  defp io_request({:put_chars, mod, func, args}, r_state() = state) do
    io_request({:put_chars, :latin1, mod, func, args},
                 state)
  end

  defp io_request({:get_until, _Prompt, mod, func, xtraArgs},
            r_state() = state) do
    io_request({:get_until, :latin1, _Prompt, mod, func,
                  xtraArgs},
                 state)
  end
  ...
  defp io_request_loop([_Request | _Tail],
            {:stop, _Reason, _Reply, _State} = result) do
    result
  end

  defp io_request_loop([_Request | _Tail],
            {:error, _Reply, _State} = result) do
    result
  end
  ...
  defp put_chars(chars, :latin1,
            r_state(handle: handle, unic: :latin1) = state) do
    newState = r_state(state, buf: <<>>)
    case (apply(r_file_descriptor(handle, :module), :write,
                  [handle, chars])) do
      {:error, reason} = reply ->
  ...
  defp put_chars(chars, inEncoding,
            r_state(handle: handle, unic: outEncoding) = state) do
    newState = r_state(state, buf: <<>>)
    case (:unicode.characters_to_binary(chars, inEncoding,
                                          outEncoding)) do
  ...
  defp get_line(s, {<<>>, cont}, outEnc,
            r_state(handle: handle, read_mode: mode,
                unic: inEnc) = state) do
    case (apply(r_file_descriptor(handle, :module), :read,
                  [handle, read_size(mode)])) do
      {:ok, bin} ->
        get_line(s, convert_enc([cont, bin], inEnc, outEnc),
                   outEnc, state)
      :eof ->
  ...
  defp get_line(s0, {buf, bCont}, outEnc,
            r_state(unic: inEnc) = state) do
  ...
  defp convert_enc(bin, inEnc, outEnc) do
    case (:unicode.characters_to_binary(bin, inEnc,
                                          outEnc)) do
  ...
  defp get_chars(0, enc,
            r_state(read_mode: readMode, unic: inEncoding) = state) do
    {:reply, cast(<<>>, readMode, inEncoding, enc), state}
  end

  defp get_chars(n, enc,
            r_state(buf: buf, read_mode: readMode, unic: :latin1) = state)
      when (is_integer(n) and n > 0 and
              n <= byte_size(buf)) do
    {b1, b2} = :erlang.split_binary(buf, n)
    {:reply, cast(b1, readMode, :latin1, enc),
       r_state(state, buf: b2)}
  end
  ...
  defp get_chars(n, outEnc,
            r_state(handle: handle, buf: buf, read_mode: readMode,
                unic: :latin1) = state)
      ...
      {:ok, b} ->
        cond do
          ...
          true ->
            ...
            {:reply, cat(buf, b1, readMode, :latin1, outEnc),
               r_state(state, buf: b2)}
        end
      :eof when bufSize === 0 ->
        {:reply, :eof, state}
  ...
  defp get_chars(n, outEnc,
            r_state(handle: handle, buf: buf, read_mode: readMode,
                unic: inEncoding) = state)
      when (is_integer(n) and n > 0) do
    try do
      {bufCount, splitPos} = count_and_find(buf, n,
                                              inEncoding)
      case (bufCount >= n) do
        true ->
          {b1, b2} = (case (splitPos) do
                        :none ->
                          {buf, <<>>}
                        _ ->
                          :erlang.split_binary(buf, splitPos)
                      end)
          {:reply, cast(b1, readMode, inEncoding, outEnc),
             r_state(state, buf: b2)}
        false ->
          ...
          case (apply(r_file_descriptor(handle, :module), :read,
            ...
            {:ok, b} ->
              ...
              {newCount, newSplit} = count_and_find(newBuf, n,
                                                      inEncoding)
              case (newCount >= n) do
                true ->
                  {b01, b02} = (case (newSplit) do
                                  :none ->
                                    {newBuf, <<>>}
                                  _ ->
            ...
            :eof when bufCount === 0 ->
              {:reply, :eof, state}
            :eof ->
              std_reply(cast(buf, readMode, inEncoding, outEnc),
                          r_state(state, buf: <<>>))
            {:error, reason} = error ->
              {:stop, reason, error, r_state(state, buf: <<>>)}
          end
  ...
  defp get_chars(mod, func, xtraArg, outEnc,
            r_state(buf: <<>>) = state) do
    get_chars_empty(mod, func, xtraArg, :start, outEnc,
                      state)
  end

  defp get_chars(mod, func, xtraArg, outEnc,
            r_state(buf: buf) = state) do
    get_chars_apply(mod, func, xtraArg, :start, outEnc,
                      r_state(state, buf: <<>>), buf)
  end

  defp get_chars_empty(mod, func, xtraArg, s, :latin1,
            r_state(handle: handle, read_mode: readMode,
                unic: :latin1) = state) do
    case (apply(r_file_descriptor(handle, :module), :read,
                  [handle, read_size(readMode)])) do
      {:ok, bin} ->
        get_chars_apply(mod, func, xtraArg, s, :latin1, state,
                          bin)
      :eof ->
        get_chars_apply(mod, func, xtraArg, s, :latin1, state,
                          :eof)
  ...
  defp get_chars_empty(mod, func, xtraArg, s, outEnc,
            r_state(handle: handle, read_mode: readMode) = state) do
    case (apply(r_file_descriptor(handle, :module), :read,
                  [handle, read_size(readMode)])) do
      {:ok, bin} ->
        get_chars_apply(mod, func, xtraArg, s, outEnc, state,
                          bin)
      :eof ->
        get_chars_apply(mod, func, xtraArg, s, outEnc, state,
                          :eof)
  ...
  defp get_chars_notempty(mod, func, xtraArg, s, outEnc,
            r_state(handle: handle, read_mode: readMode,
                buf: b) = state) do
    case (apply(r_file_descriptor(handle, :module), :read,
                  [handle, read_size(readMode)])) do
        ...
        case (b) do
          <<>> ->
            get_chars_apply(mod, func, xtraArg, s, outEnc, state,
                              :eof)
          _ ->
            {:stop, :invalid_unicode,
               invalid_unicode_error(mod, func, xtraArg, s), state}
        end
      {:error, reason} = error ->
        {:stop, reason, error, state}
    end
  ...
  defp get_chars_apply(mod, func, xtraArg, s0, :latin1,
            r_state(read_mode: readMode, unic: :latin1) = state, data0) do
    data1 = (case (readMode) do
    ...
    case ((try do
      ...
      {:EXIT, reason} ->
        {:stop, reason, {:error, err_func(mod, func, xtraArg)},
           state}
      s1 ->
  ...
  defp get_chars_apply(mod, func, xtraArg, s0, outEnc,
            r_state(read_mode: readMode, unic: inEnc) = state, data0) do
    try do
      {data1, newBuff} = (case (readMode) do
                            :list when is_binary(data0) ->
                              case (:unicode.characters_to_list(data0,
                                                                  inEnc)) do
                                {tag, decoded, rest} when (decoded !== [] and
                                                             tag === :error) or
                                                            (decoded !== [] and
                                                               tag === :incomplete)
                                                          ->
                            ...
                            :binary when is_binary(data0) ->
                              case (:unicode.characters_to_binary(data0, inEnc,
                                                                    outEnc)) do
                                {tag2, decoded2, rest2}
                                    when (decoded2 !== <<>> and
                                            tag2 === :error) or
      ...
      case ((try do
        ...
        {:stop, result, buf} ->
          {:reply, result,
             r_state(state, buf: cond do
                             is_binary(buf) ->
                               :erlang.list_to_binary([:unicode.characters_to_binary(buf,
                                                                                       outEnc,
                                                                                       inEnc),
                                                           newBuff])
                             is_list(buf) ->
        ...
        {:EXIT, reason} ->
          {:stop, reason, {:error, err_func(mod, func, xtraArg)},
             state}
        s1 ->
          get_chars_notempty(mod, func, xtraArg, s1, outEnc,
                               r_state(state, buf: newBuff))
      end
    catch
      :exit, exReason ->
        {:stop, exReason,
           invalid_unicode_error(mod, func, xtraArg, s0), state}
      :error, errReason ->
        {:stop, errReason,
           {:error, err_func(mod, func, xtraArg)}, state}
    end
  end
  ...
  defp invalid_unicode_error(mod, func, xtraArg, s) do
    try do
      ...
      {:error, {location, :file_io_server, :invalid_unicode},
         location}
    catch
      _, _ ->
  ...
  defp setopts(opts0, state) do
    opts = :proplists.unfold(:proplists.substitute_negations([{:list,
                                                                 :binary}],
  ...
  defp do_setopts(opts, state) do
    case (valid_enc(:proplists.get_value(:encoding, opts,
                                           r_state(state, :unic)))) do
      {:ok, newUnic} ->
        case (:proplists.get_value(:binary, opts)) do
          true ->
            {:reply, :ok,
               r_state(state, read_mode: :binary,  unic: newUnic)}
          false ->
            {:reply, :ok,
               r_state(state, read_mode: :list,  unic: newUnic)}
          :undefined ->
  ...
  defp cat(b1, b2, :binary, inEncoding, outEncoding) do
    case (:unicode.characters_to_binary([b1, b2],
                                          inEncoding, outEncoding)) do
  ...
  defp cast(b, :binary, inEncoding, outEncoding) do
    case (:unicode.characters_to_binary(b, inEncoding,
                                          outEncoding)) do
      good when is_binary(good) ->
        good
      _ ->
  ...
  defp cast(b, :list, inEncoding, outEncoding) do
    try do
      :unicode.characters_to_list(:unicode.characters_to_binary(b,
                                                                  inEncoding,
                                                                  outEncoding),
                                    outEncoding)
    catch
  ...
  defp cafu(<<_ :: utf8, rest :: binary>>, 0, count,
            byteCount, _SavePos, :utf8) do
    cafu(rest, - 1, count + 1, 0, byteCount, :utf8)
  end

  defp cafu(<<_ :: utf8, rest :: binary>>, n, count,
            _ByteCount, savePos, :utf8)
      when n < 0 do
    cafu(rest, - 1, count + 1, 0, savePos, :utf8)
  end

  defp cafu(<<_ :: utf8, rest :: binary>> = whole, n, count,
            byteCount, savePos, :utf8) do
    delta = byte_size(whole) - byte_size(rest)
    cafu(rest, n - 1, count + 1, byteCount + delta, savePos,
           :utf8)
  end

  defp cafu(<<_ :: utf16 - big, rest :: binary>>, 0, count,
            byteCount, _SavePos, {:utf16, :big}) do
    cafu(rest, - 1, count + 1, 0, byteCount, {:utf16, :big})
  end

  defp cafu(<<_ :: utf16 - big, rest :: binary>>, n, count,
            _ByteCount, savePos, {:utf16, :big})
      when n < 0 do
    cafu(rest, - 1, count + 1, 0, savePos, {:utf16, :big})
  end

  defp cafu(<<_ :: utf16 - big, rest :: binary>> = whole, n,
            count, byteCount, savePos, {:utf16, :big}) do
    delta = byte_size(whole) - byte_size(rest)
    cafu(rest, n - 1, count + 1, byteCount + delta, savePos,
           {:utf16, :big})
  end

  defp cafu(<<_ :: utf16 - little, rest :: binary>>, 0,
            count, byteCount, _SavePos, {:utf16, :little}) do
    cafu(rest, - 1, count + 1, 0, byteCount,
           {:utf16, :little})
  end

  defp cafu(<<_ :: utf16 - little, rest :: binary>>, n,
            count, _ByteCount, savePos, {:utf16, :little})
      when n < 0 do
    cafu(rest, - 1, count + 1, 0, savePos,
           {:utf16, :little})
  end

  defp cafu(<<_ :: utf16 - little, rest :: binary>> = whole,
            n, count, byteCount, savePos, {:utf16, :little}) do
    delta = byte_size(whole) - byte_size(rest)
    cafu(rest, n - 1, count + 1, byteCount + delta, savePos,
           {:utf16, :little})
  end

  defp cafu(<<_ :: utf32 - big, rest :: binary>>, 0, count,
            byteCount, _SavePos, {:utf32, :big}) do
    cafu(rest, - 1, count + 1, 0, byteCount, {:utf32, :big})
  end

  defp cafu(<<_ :: utf32 - big, rest :: binary>>, n, count,
            _ByteCount, savePos, {:utf32, :big})
      when n < 0 do
    cafu(rest, - 1, count + 1, 0, savePos, {:utf32, :big})
  end

  defp cafu(<<_ :: utf32 - big, rest :: binary>> = whole, n,
            count, byteCount, savePos, {:utf32, :big}) do
    delta = byte_size(whole) - byte_size(rest)
    cafu(rest, n - 1, count + 1, byteCount + delta, savePos,
           {:utf32, :big})
  end

  defp cafu(<<_ :: utf32 - little, rest :: binary>>, 0,
            count, byteCount, _SavePos, {:utf32, :little}) do
    cafu(rest, - 1, count + 1, 0, byteCount,
           {:utf32, :little})
  end

  defp cafu(<<_ :: utf32 - little, rest :: binary>>, n,
            count, _ByteCount, savePos, {:utf32, :little})
      when n < 0 do
    cafu(rest, - 1, count + 1, 0, savePos,
           {:utf32, :little})
  end

  defp cafu(<<_ :: utf32 - little, rest :: binary>> = whole,
            n, count, byteCount, savePos, {:utf32, :little}) do
    delta = byte_size(whole) - byte_size(rest)
    cafu(rest, n - 1, count + 1, byteCount + delta, savePos,
           {:utf32, :little})
  end
