defmodule :m_logger_backend do
  use Bitwise
  def log_allowed(log, tid, primaryConfig) do
    filters = :maps.get(:filters, primaryConfig, [])
    case (apply_filters(:primary, log, filters,
                          primaryConfig)) do
      :stop ->
        :ok
      log1 ->
  ...
  defp call_handlers(%{level: level} = log, [id | handlers], tid) do
    case (:logger_config.get(tid, id, level)) do
      {:ok, %{module: module} = handlerConfig} ->
        ...
        case (apply_filters(id, log, filters, handlerConfig)) do
          :stop ->
            :ok
          log1 ->
            handlerConfig1 = :maps.without([:level, :filters,
                                                        :filter_default,
                                                            :handlers],
                                             handlerConfig)
            try do
              module.log(log1, handlerConfig1)
            catch
              c, r ->
                                                                           ...
                                                                           {:config,
                                                                              handlerConfig1},
                                                                               {:reason,
                                                                                  {c,
                                                                                     r,
                                                                                     filter_stacktrace(__STACKTRACE__)}}]])
                        :ok
                      false ->
                        :ok
                    end
  ...
  defp do_apply_filters(owner, log,
            [{_Id, {filterFun, filterArgs}} = filter | filters],
            state) do
    try do
      filterFun.(log, filterArgs)
    catch
      c, r ->
        handle_filter_failed(filter, owner, log,
                               {c, r, filter_stacktrace(__STACKTRACE__)})
    else
      :stop ->
        :stop
      :ignore ->
      ...
      log1 = %{level: level, msg: msg, meta: meta}
          when (is_atom(level) and
                  (is_tuple(msg) and tuple_size(msg) == 2) and ((is_list(:erlang.element(1,
                                                                                           msg)) or is_binary(:erlang.element(1,
                                                                                                                                msg))) or is_atom(:erlang.element(1,
                                                                                                                                                                    msg))) and is_list(:erlang.element(2,
                                                                                                                                                                                                         msg)) or :erlang.element(1,
                                                                                                                                                                                                                                    msg) == :report and (is_map(:erlang.element(2,
                                                                                                                                                                                                                                                                                  msg)) or is_list(:erlang.element(2,
                                                                                                                                                                                                                                                                                                                     msg)) and is_tuple(hd(:erlang.element(2,
                                                                                                                                                                                                                                                                                                                                                             msg)))) or :erlang.element(1,
                                                                                                                                                                                                                                                                                                                                                                                          msg) == :string and (is_list(:erlang.element(2,
                                                                                                                                                                                                                                                                                                                                                                                                                                         msg)) or is_binary(:erlang.element(2,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                              msg))) and
                  is_map(meta))
               ->
            ...
            _ = :logger_server.do_internal_log(:debug,
                                                 %{mfa:
                                                   {:logger_backend,
                                                     ...
                                                     line: 123, file: 'otp/lib/kernel/src/logger_backend.erl'},
                                                 log,
                                                 [[{:logger,
