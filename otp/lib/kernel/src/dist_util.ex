defmodule :m_dist_util do
  ...
  Record.defrecord(:r_tick, :tick, read: 0, write: 0, tick: 0,
                                ticked: 0)
  defp dflag2str(1) do
    'PUBLISHED'
  end
  ...
  defp dflag2str(131072) do
    'MAP_TAG'
  end

  defp dflag2str(262144) do
    'BIG_CREATION'
  end

  defp dflag2str(524288) do
    'SEND_SENDER'
  end

  defp dflag2str(1048576) do
    'BIG_SEQTRACE_LABELS'
  end

  defp dflag2str(4194304) do
    'EXIT_PAYLOAD'
  end

  defp dflag2str(8388608) do
    'FRAGMENTS'
  end

  defp dflag2str(16777216) do
    'HANDSHAKE_23'
  end

  defp dflag2str(33554432) do
    'UNLINK_ID'
  end

  defp dflag2str(67108864) do
    'MANDATORY_25_DIGEST'
  end

  defp dflag2str(4294967296) do
    'SPAWN'
  end

  defp dflag2str(8589934592) do
    'NAME_ME'
  end

  defp dflag2str(17179869184) do
    'V4_NC'
  end

  defp dflag2str(34359738368) do
    'ALIAS'
  end
  ...
  Record.defrecord(:r_erts_dflags, :erts_dflags, default: :undefined,
                                       mandatory: :undefined,
                                       addable: :undefined,
  ...
  defp make_this_flags(requestType, addFlags, rejectFlags, otherNode,
            r_erts_dflags() = eDF, nameMeFlg) do
    ...
    flgs1 = flgs0 ||| publish_flag(requestType, nameMeFlg,
                                     otherNode)
    flgs2 = flgs1 ||| addFlags
    flgs2 &&& ~~~ rejectFlags
  end

  def handshake_other_started(r_hs_data(request_type: reqType, add_flags: addFlgs0,
             reject_flags: rejFlgs0,
             require_flags: reqFlgs0) = hSData0) do
    addFlgs = convert_flags(addFlgs0)
    rejFlgs = convert_flags(rejFlgs0)
    reqFlgs = convert_flags(reqFlgs0)
    {preOtherFlags0, nodeOrHost,
       creation} = recv_name(hSData0)
    preOtherFlags = expand_mandatory_25_flag(preOtherFlags0)
    eDF = :erts_internal.get_dflags()
    preThisFlags = make_this_flags(reqType, addFlgs,
                                     rejFlgs, nodeOrHost, eDF, preOtherFlags)
    hSData1 = r_hs_data(hSData0, this_flags: preThisFlags, 
                           other_flags: preOtherFlags,  other_node: nodeOrHost, 
                           other_started: true,  other_creation: creation, 
                           add_flags: addFlgs,  reject_flags: rejFlgs, 
    ...
    chosenFlags = adjust_flags(r_hs_data(hSData3, :this_flags),
                                 r_hs_data(hSData3, :other_flags))
    hSData4 = r_hs_data(hSData3, this_flags: chosenFlags, 
                           other_flags: chosenFlags)
    challengeB = recv_challenge_reply(hSData4, challengeA,
                                        cookie)
    ...
    send_challenge_ack(hSData4,
                         gen_digest(challengeB, cookie))
    :ok
    connection(hSData4)
  end

  def handshake_other_started(oldHsData) when :erlang.element(1,
                                           oldHsData) === :hs_data do
  ...
  defp expand_mandatory_25_flag(flags) do
    cond do
      flags &&& 67108864 !== 0 ->
        flags ||| (4 ||| 16 ||| 256 ||| 128 ||| 512 ||| 1024 ||| 2048 ||| 65536 ||| 131072 ||| 262144 ||| 16777216)
      true ->
        flags
    end
  end

  defp check_dflags(r_hs_data(other_node: node, other_flags: otherFlags,
              other_started: otherStarted,
              require_flags: requiredFlags,
              other_creation: otherCreation) = hSData,
            r_erts_dflags() = eDF) do
    mask = (case (otherCreation) do
              :undefined ->
                1 <<< 32 - 1
              _ ->
                ~~~ 0
    ...
    case (missing) do
      ...
      _ ->
        ...
        error_msg('** ~w: Connection attempt ~s node ~w ~s since it cannot handle ~p.**~n', [node(), dir, node, how, missing])
        :dist_util.shutdown(:dist_util, 282, node,
                              {:check_dflags_failed, missing})
    end
  end
  ...
  defp check_mandatory(mandatory, otherFlags, missing) do
    ...
    newMissing = (case (dFlag &&& otherFlags) do
                    0 ->
  ...
  defp mark_pending(r_hs_data(kernel_pid: kernel, other_node: node,
              this_node: myNode) = hSData) do
    kernelReply = do_mark_pending(kernel, myNode, node,
                                    r_hs_data(hSData, :other_flags))
    {kernelReply,
       case (kernelReply) do
         :ok ->
         ...
         {:ok, dynNodeName, creation} ->
           hSData1 = r_hs_data(hSData, other_node: dynNodeName, 
  ...
  defp do_mark_pending(kernel, myNode, node, flags) do
    send(kernel, {self(),
                    {:accept_pending, myNode, node, publish_type(flags)}})
    receive do
  ...
  def handshake_we_started(r_hs_data(request_type: reqType, this_node: myNode,
             other_node: node, add_flags: addFlgs0,
             reject_flags: rejFlgs0,
             require_flags: reqFlgs0) = preHSData) do
    addFlgs = convert_flags(addFlgs0)
    rejFlgs = convert_flags(rejFlgs0)
    reqFlgs = convert_flags(reqFlgs0)
    eDF = :erts_internal.get_dflags()
    {nameMeFlg, nameToSend} = (case (node()) do
                                 :nonode@nohost ->
    ...
    preThisFlags = make_this_flags(reqType, addFlgs,
                                     rejFlgs, node, eDF, nameMeFlg)
    hSData = r_hs_data(preHSData, this_node: nameToSend, 
                            this_flags: preThisFlags,  add_flags: addFlgs, 
                            reject_flags: rejFlgs,  require_flags: reqFlgs)
    send_name(hSData)
    hSData1 = recv_status(hSData)
    {preOtherFlags0, challengeA,
       creation} = recv_challenge(hSData1)
    preOtherFlags = expand_mandatory_25_flag(preOtherFlags0)
    chosenFlags = adjust_flags(preThisFlags, preOtherFlags)
    hSData2 = r_hs_data(hSData1, this_flags: chosenFlags, 
                           other_flags: chosenFlags,  other_started: false, 
                           other_creation: creation)
  ...
  def handshake_we_started(oldHsData) when :erlang.element(1,
                                           oldHsData) === :hs_data do
  ...
  defp convert_old_hsdata(oldHsData) do
    ...
    true = noMissing > 0
    :erlang.list_to_tuple(oHSDL ++ :lists.duplicate(noMissing,
                                                      :undefined))
  end
  ...
  Record.defrecord(:r_state, :state, kernel: :undefined,
                                 node: :undefined, tick_intensity: :undefined,
                                 socket: :undefined, publish_type: :undefined,
                                 handle: :undefined, f_tick: :undefined,
                                 f_getstat: :undefined, f_setopts: :undefined,
                                 f_getopts: :undefined)
  defp connection(r_hs_data(other_node: node, socket: socket,
              f_address: fAddress, f_setopts_pre_nodeup: fPreNodeup,
              f_setopts_post_nodeup: fPostNodeup) = hSData) do
      ...
      :ok ->
        ...
        case (fPostNodeup.(socket)) do
          :ok ->
            ...
            con_loop(r_state(kernel: r_hs_data(hSData, :kernel_pid), node: node,
                         socket: socket, tick_intensity: tickIntensity,
                         publish_type: pType, handle: dHandle,
                       ...
                       r_tick())
          error1 ->
            :dist_util.shutdown(:dist_util, 519, {node, socket},
                                  {:f_setopts_post_nodeup_failed, error1})
        end
      error2 ->
        :dist_util.shutdown(:dist_util, 524, {node, socket},
  ...
  defp gen_digest(challenge, cookie)
    ...
    :erlang.md5([:erlang.atom_to_list(cookie) |
  ...
  defp gen_challenge() do
    ...
    (a <<< 24 + (e <<< 16) + (g <<< 8) + f ^^^ (b + (c <<< 16)) ^^^ (d + (h <<< 16))) &&& 4294967295
  end

  defp do_setnode(r_hs_data(other_node: node, socket: socket,
              this_node: myNode, other_flags: flags, f_getll: getLL,
              other_creation: creation) = hSData) do
    case (getLL.(socket)) do
      {:ok, port} ->
        namedMe = (case (node()) do
                     :nonode@nohost ->
        ...
        try do
          ...
          :error, :system_limit ->
            error_msg('** Distribution system limit reached, no table space left for node ~w ** ~n', [node])
            :dist_util.shutdown(:dist_util, 577, {node, socket})
          :error, other ->
            exit({other, __STACKTRACE__})
        end
      _ ->
        error_msg('** Distribution connection error, could not get low level port for node ~w ** ~n', [node])
        :dist_util.shutdown(:dist_util, 585, {node, socket})
    end
  end

  defp mark_nodeup(r_hs_data(kernel_pid: kernel, other_node: node,
              other_flags: flags, other_started: otherStarted),
            address, namedMe) do
    send(kernel, {self(),
                    {:nodeup, node, address, publish_type(flags), namedMe}})
    receive do
      {^kernel, :inserted, tickIntensity} ->
        tickIntensity
      {^kernel, :bad_request} ->
        typeT = (case (otherStarted) do
                   true ->
                     'accepting connection'
                   _ ->
                     'initiating connection'
                 end)
        error_msg('Fatal: ~p was not allowed to send {nodeup, ~p} to kernel when ~s~n', [self(), node, typeT])
  ...
  defp con_loop(r_state(kernel: kernel, node: node, socket: socket,
              handle: dHandle, f_getstat: mFGetstat,
              f_setopts: mFSetOpts, f_getopts: mFGetOpts) = conData,
            tick) do
    receive do
      {:tcp_closed, ^socket} ->
        :dist_util.shutdown(:dist_util, 622, {node, socket},
                              :tcp_closed)
      {^kernel, :disconnect} ->
        :dist_util.shutdown(:dist_util, 624, {node, socket},
                              :disconnected)
      {^kernel, :aux_tick} ->
      ...
      {^kernel, :tick} ->
        case (send_tick(conData, tick)) do
          ...
          {:error, :not_responding} ->
            error_msg('** Node ~p not responding **~n** Removing (timedout) connection **~n', [node])
            :dist_util.shutdown(:dist_util, 641, {node, socket},
                                  :net_tick_timeout)
          error1 ->
            :dist_util.shutdown(:dist_util, 643, {node, socket},
                                  {:send_net_tick_failed, error1})
        end
      {from, :get_status} ->
        case (getstat(dHandle, socket, mFGetstat)) do
          {:ok, read, write, _} ->
            ...
            con_loop(conData, tick)
          error2 ->
            :dist_util.shutdown(:dist_util, 651, {node, socket},
                                  {:get_status_failed, error2})
        end
      {from, ref, {:setopts, opts}} ->
        ret = (case (mFSetOpts) do
                 :undefined ->
                   {:error, :enotsup}
                 _ ->
  ...
  defp send_name(r_hs_data(socket: socket, this_node: node,
              f_send: fSend, this_flags: flags)) do
    nameBin = to_binary(node)
    creation = (case (name_type(flags)) do
                  :static ->
    ...
    to_port(fSend, socket,
              [<<?N, flags :: size(64), creation :: size(32),
                   nameLen :: size(16)>>,
                   nameBin])
  end
  ...
  defp send_challenge(r_hs_data(socket: socket, this_node: node,
              this_flags: thisFlags, f_send: fSend),
            challenge) do
    ...
    :ok
    to_port(fSend, socket,
              [<<?N, thisFlags :: size(64), challenge :: size(32),
                   creation :: size(32), nameLen :: size(16)>>,
                   nodeName])
  end

  defp send_challenge_reply(r_hs_data(socket: socket, f_send: fSend), challenge,
            digest) do
    :ok
    to_port(fSend, socket,
              [?r, [(challenge >>> 24) &&& 255,
                        (challenge >>> 16) &&& 255, (challenge >>> 8) &&& 255,
                                                        challenge &&& 255],
                       digest])
  end
  ...
  defp recv_name(r_hs_data(socket: socket, f_recv: recv) = hSData) do
    case (recv.(socket, 0, :infinity)) do
      ...
      other ->
        :dist_util.shutdown(:dist_util, 730, {:no_node, socket},
                              {:recv_name_failed, other})
    end
  end

  defp recv_name_old(hSData,
            [?n, v1, v0, f3, f2, f1, f0 | node] = data) do
  ...
  defp recv_name_new(hSData,
            [?N, f7, f6, f5, f4, f3, f2, f1, f0, cr3, cr2, cr1, cr0,
                                                                    nL1, nL0 |
                                                                             rest] = data) do
    <<flags :: size(64)>> = <<f7, f6, f5, f4, f3, f2, f1,
                                f0>>
    ...
    case (is_name_ok(name, flags)) do
      true ->
        ...
        nodeOrHost = (case (name_type(flags)) do
                        :static ->
  ...
  defp is_name_ok(nodeOrHost, flags) do
    case ({:string.split(nodeOrHost, '@', :all),
             name_type(flags)}) do
      {[name, host], :static} ->
        not :string.is_empty(host) and not
  ...
  defp check_allowed(r_hs_data(allowed: allowed) = hSData, node) do
    case (is_allowed(node, allowed)) do
      ...
      false ->
        ...
        :dist_util.shutdown(:dist_util, 834, node,
                              {:is_allowed, :not_allowed})
    end
  end
  ...
  defp recv_challenge(r_hs_data(socket: socket, f_recv: recv) = hSData) do
    case (recv.(socket, 0, :infinity)) do
      ...
      other ->
        :dist_util.shutdown(:dist_util, 911, {:no_node, socket},
                              {:recv_challenge_failed, other})
    end
  end

  defp recv_challenge_new(r_hs_data(other_node: node),
            [?N, f7, f6, f5, f4, f3, f2, f1, f0, ch3, ch2, ch1, ch0,
                                                                    cr3, cr2,
                                                                             cr1,
                                                                                 cr0,
                                                                                     nL1,
    ...
    {ns, _Residue} = (try do
                        ...
                        :error, :badarg ->
                          :dist_util.shutdown(:dist_util, 929, :no_node,
                                                {:recv_challenge_failed,
                                                   :no_node, msg})
                      end)
    :ok
    try do
      ...
      :error, :badarg ->
        :dist_util.shutdown(:dist_util, 941, :no_node,
                              {:recv_challenge_failed, :no_node, ns})
    else
      ^node ->
        {flags, challenge, creation}
      _ ->
        :dist_util.shutdown(:dist_util, 938, :no_node,
                              {:recv_challenge_failed, :no_node, ns})
    end
  end

  defp recv_challenge_new(_, other) do
    :dist_util.shutdown(:dist_util, 944, :no_node,
                          {:recv_challenge_failed, other})
  end

  defp recv_complement(r_hs_data(socket: socket, f_recv: recv,
              other_flags: flags,
              other_creation: :undefined) = hSData) do
    case (recv.(socket, 0, :infinity)) do
      {:ok, [?c, f7, f6, f5, f4, cr3, cr2, cr1, cr0]} ->
        <<flagsHigh :: size(32)>> = <<f7, f6, f5, f4>>
        <<creation :: size(32)>> = <<cr3, cr2, cr1, cr0>>
        :ok
        r_hs_data(hSData, other_creation: creation, 
                    other_flags: flags ||| (flagsHigh <<< 32))
      other ->
        :dist_util.shutdown(:dist_util, 960, :no_node,
                              {:recv_complement_failed, other})
    end
  end
  ...
  defp recv_challenge_reply(r_hs_data(socket: socket, other_node: nodeB,
              f_recv: fRecv),
            challengeA, cookie) do
      ...
      {:ok, [?r, cB3, cB2, cB1, cB0 | sumB]}
        ...
        case (:erlang.list_to_binary(sumB)) do
          ^sumA ->
            challengeB
          _ ->
            error_msg('** Connection attempt from node ~w rejected. Invalid challenge reply. **~n', [nodeB])
            :dist_util.shutdown(:dist_util, 986, nodeB,
                                  {:recv_challenge_reply_failed, :bad_cookie})
        end
      other ->
        :dist_util.shutdown(:dist_util, 989, {:no_node, socket},
                              {:recv_challenge_reply_failed, other})
    end
  end

  defp recv_challenge_ack(r_hs_data(socket: socket, f_recv: fRecv,
              other_node: nodeB),
            challengeB, cookieA) do
    case (fRecv.(socket, 0, :infinity)) do
      {:ok, [?a | sumB]} when length(sumB) === 16 ->
        ...
        case (:erlang.list_to_binary(sumB)) do
          ^sumA ->
            :ok
          _ ->
            error_msg('** Connection attempt to node ~w cancelled. Invalid challenge ack. **~n', [nodeB])
            :dist_util.shutdown(:dist_util, 1007, {nodeB, socket},
                                  {:recv_challenge_ack_failed, :bad_cookie})
        end
      other ->
        :dist_util.shutdown(:dist_util, 1011, {nodeB, socket},
                              {:recv_challenge_ack_failed, other})
    end
  end

  defp recv_status(r_hs_data(kernel_pid: kernel, socket: socket,
              this_flags: myFlgs, other_node: node,
              f_recv: recv) = hSData) do
    case (recv.(socket, 0, :infinity)) do
      {:ok, 'snamed:' ++ rest} ->
        <<nameLen :: size(16),
            nodeName :: size(nameLen) - binary,
            creation :: size(32),
            _ :: binary>> = :erlang.list_to_binary(rest)
        :dynamic = name_type(myFlgs)
        {:node, _Name, host} = split_node(nodeName)
        ^host = r_hs_data(hSData, :this_node)
        r_hs_data(hSData, this_node: :erlang.binary_to_atom(nodeName,
                                                      :utf8), 
      ...
      {:ok, [?s | stat]} ->
        ...
        case ({stat, name_type(myFlgs)}) do
          {'not_allowed', _} ->
            :dist_util.shutdown(:dist_util, 1038, {node, socket},
                                  {:recv_status_failed, :not_allowed})
          {_, :dynamic} ->
            :dist_util.shutdown(:dist_util, 1041, {node, socket},
        ...
        case (stat) do
          ...
          'ok_simultaneous' ->
            hSData
          other ->
            :dist_util.shutdown(:dist_util, 1064, {node, socket},
                                  {:recv_status_failed, :unknown, other})
        end
      error ->
        :ok
        :dist_util.shutdown(:dist_util, 1070, {node, socket},
                              {:recv_status_failed, error})
    end
  end

  defp recv_status_reply(r_hs_data(socket: socket, other_node: node,
              f_recv: recv)) do
    case (recv.(socket, 0, :infinity)) do
      {:ok, [?s | stat]} ->
        :ok
        case (stat) do
          ...
          'false' ->
            false
          other ->
            :dist_util.shutdown(:dist_util, 1084, {node, socket},
                                  {:recv_status_failed, :unexpected, other})
        end
      error ->
        :ok
        :dist_util.shutdown(:dist_util, 1091, {node, socket},
                              {:recv_status_failed, error})
    end
  end

  defp send_status(r_hs_data(socket: socket, other_node: node,
              other_creation: creation, f_send: fSend),
            :named) do
    :ok
    nameBin = :erlang.atom_to_binary(node, :utf8)
    nameLen = byte_size(nameBin)
    case (fSend.(socket,
                   [?s, 'named:', <<nameLen :: size(16), nameBin :: binary>>,
                               <<creation :: size(32)>>])) do
      {:error, _} = error ->
        :dist_util.shutdown(:dist_util, 1107, {node, socket},
                              {:send_status_failed, error})
      _ ->
        :ok
    end
  end
  ...
  defp send_status(r_hs_data(socket: socket, other_node: node,
    ...
    :ok
    case (fSend.(socket,
                   [?s | :erlang.atom_to_list(stat)])) do
      {:error, _} = error ->
        :dist_util.shutdown(:dist_util, 1116, {node, socket},
                              {:send_status_failed, error})
      _ ->
        :ok
    end
  ...
  defp to_port(fSend, socket, data) do
    case (fSend.(socket, data)) do
      {:error, :closed} ->
        ...
        :ok
      {:error, _} = error ->
        :dist_util.shutdown(:dist_util, 1127, socket,
                              {:f_send_failed, error})
      :ok ->
        :ok
    end
  ...
  defp send_tick(r_state(handle: dHandle, socket: socket,
              tick_intensity: tickIntensity, publish_type: type,
              f_tick: mFTick, f_getstat: mFGetstat),
            tick) do
    r_tick(tick: t0, read: read, write: write,
        ticked: ticked0) = tick
    t = t0 + 1
    ...
    case (getstat(dHandle, socket, mFGetstat)) do
      ...
      {:ok, r, w1, pend} ->
        rDiff = r - read
        w2 = (case (need_to_tick(type, rDiff, w1 - write,
                                   pend)) do
                true ->
                  mFTick.(socket)
                  w1 + 1
        ...
        {:ok,
           r_tick(tick, write: w2,  tick: t1,  read: r, 
                     ticked: ticked1)}
      error ->
        error
    end
  ...
  def start_timer(timeout) do
    spawn_link(:dist_util, :setup_timer,
                 [self(), timeout * 1])
  end
  ...
  def setup_timer(pid, timeout) do
    ...
    after timeout ->
      :ok
      :dist_util.shutdown(:dist_util, 1226, {:timer, pid},
                            :setup_timer_timeout)
    end
  end
  ...
  def net_ticker_spawn_options() do
    opts = :application.get_env(:kernel,
