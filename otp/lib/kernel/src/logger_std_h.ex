defmodule :m_logger_std_h do
  ...
  defp write_to_dev(bin, %{dev: :standard_io} = state) do
    try do
      ...
      _E, _R ->
        :io.put_chars(:standard_error, ~c"Failed to write log message to stdout, trying stderr\n")
  ...
  defp rot_file_name(fileName, count, false) do
    fileName ++ ~c"." ++ :erlang.integer_to_list(count)
  end

  defp rot_file_name(fileName, count, true) do
    rot_file_name(fileName, count, false) ++ ~c".gz"
  end
  ...
  defp decompress_file(fileName) do
    ...
    {:ok, out} =
      :file.open(
        :filename.rootname(fileName, ~c".gz"),
        [:write]
      )
