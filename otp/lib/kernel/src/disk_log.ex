defmodule :m_disk_log do
  ...
  Record.defrecord(:r_rotate_handle, :rotate_handle,
    file: :undefined,
    cur_fdc: :undefined,
    inode: :undefined,
  ...
  defp check_arg([], res) do
    ...
    cond do
      ...
      r_arg(res, :file) === :none ->
        case (try do
                :lists.concat([r_arg(res, :name), ~c".LOG"])
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                :exit, e -> {:EXIT, e}
                e -> e
  ...
  defp check_wrap_arg({:ok, res}, oldSize, version)
       when r_arg(res, :size) === :infinity do
    {:ok, r_arg(res, version: version, old_size: oldSize, size: oldSize)}
  end

  defp check_wrap_arg({:ok, res}, {0, 0}, version) do
    {:ok, r_arg(res, version: version, old_size: r_arg(res, :size))}
  end
  ...
  defp rename_file(file, newFile, r_log(type: :rotate, extra: handle)) do
  ...
  defp compare_arg(:size, sz, a)
       when sz !== r_arg(a, :size) and
  ...
  defp do_open(r_arg(type: :rotate) = a) do
    ...
    case :disk_log_1.open_rotate_log_file(fName, size, head) do
      {:ok, rotHandle} ->
        l =
          r_log(
            name: name,
  ...
  defp do_open(a) do
    ...
    case do_open2(type, format, name, fName, repair, oldSize, mode, head, v) do
  ...
  defp do_format_error({node, error = {:error, _Reason}}) do
    :lists.append(
      :io_lib.format(~c"~p: ", [node]),
      do_format_error(error)
    )
  end

  defp do_format_error({:badarg, arg}) do
    :io_lib.format(~c"The argument ~p is missing, not recognized or not wellformed~n", [arg])
  end

  defp do_format_error({:size_mismatch, oldSize, argSize}) do
    :io_lib.format(
      ~c"The given size ~p does not match the size ~p found on the disk log size file~n",
      [argSize, oldSize]
    )
  end
  ...
  defp do_format_error({:read_only_mode, log}) do
    :io_lib.format(
      ~c"The disk log ~tp has been opened read-only, but the requested operation needs read-write access~n",
      [log]
    )
  end
  ...
  defp do_format_error({:format_external, log}) do
    :io_lib.format(
      ~c"The requested operation can only be applied on internally formatted disk logs, but ~tp is externally formatted~n",
      [log]
    )
  end
  ...
  defp do_format_error({:blocked_log, log}) do
    :io_lib.format(
      ~c"The blocked disk log ~tp does not queue requests, or the log has been blocked by the calling process~n",
      [log]
    )
  end

  defp do_format_error({:full, log}) do
    :io_lib.format(~c"The halt log ~tp is full~n", [log])
  end

  defp do_format_error({:not_blocked, log}) do
    :io_lib.format(~c"The disk log ~tp is not blocked~n", [log])
  end

  defp do_format_error({:not_owner, pid}) do
    :io_lib.format(~c"The pid ~tp is not an owner of the disk log~n", [pid])
  end

  defp do_format_error({:not_blocked_by_pid, log}) do
    :io_lib.format(
      ~c"The disk log ~tp is blocked, but only the blocking pid can unblock a disk log~n",
      [log]
    )
  end
  ...
  defp do_format_error({:new_size_too_small, log, currentSize}) do
    :io_lib.format(
      ~c"The current size ~p of the halt log ~tp is greater than the requested new size~n",
      [currentSize, log]
    )
  end

  defp do_format_error({:halt_log, log}) do
    :io_lib.format(~c"The halt log ~tp cannot be wrapped~n", [log])
  end

  defp do_format_error({:same_file_name, log}) do
    :io_lib.format(~c"Current and new file name of the disk log ~tp are the same~n", [log])
  end

  defp do_format_error({:arg_mismatch, option, firstValue, argValue}) do
    :io_lib.format(
      ~c"The value ~tp of the disk log option ~p does not match the current value ~tp~n",
      [argValue, option, firstValue]
    )
  end

  defp do_format_error({:name_already_open, log}) do
    :io_lib.format(~c"The disk log ~tp has already opened another file~n", [log])
  end

  defp do_format_error({:open_read_write, log}) do
    :io_lib.format(~c"The disk log ~tp has already been opened read-write~n", [log])
  end

  defp do_format_error({:open_read_only, log}) do
    :io_lib.format(~c"The disk log ~tp has already been opened read-only~n", [log])
  end

  defp do_format_error({:not_internal_wrap, log}) do
    :io_lib.format(
      ~c"The requested operation cannot be applied since ~tp is not an internally formatted disk log~n",
      [log]
    )
  end

  defp do_format_error(:no_such_log) do
    :io_lib.format(~c"There is no disk log with the given name~n", [])
  end

  defp do_format_error(:nonode) do
    :io_lib.format(~c"There seems to be no node up that can handle the request~n", [])
  end
  ...
  defp do_format_error({:corrupt_log_file, fileName}) do
    :io_lib.format(~c"The disk log file \"~ts\" contains corrupt data~n", [fileName])
  end

  defp do_format_error({:need_repair, fileName}) do
    :io_lib.format(
      ~c"The disk log file \"~ts\" has not been closed properly and needs repair~n",
      [fileName]
    )
  end

  defp do_format_error({:not_a_log_file, fileName}) do
    :io_lib.format(~c"The file \"~ts\" is not a wrap log file~n", [fileName])
  end

  defp do_format_error({:invalid_header, invalidHeader}) do
    :io_lib.format(~c"The disk log header is not wellformed: ~p~n", [invalidHeader])
  end

  defp do_format_error(:end_of_log) do
    :io_lib.format(~c"An attempt was made to step outside a not yet full wrap log~n", [])
  end

  defp do_format_error({:invalid_index_file, fileName}) do
    :io_lib.format(~c"The wrap log index file \"~ts\" cannot be used~n", [fileName])
  end

  defp do_format_error({:no_continuation, badCont}) do
    :io_lib.format(~c"The term ~p is not a chunk continuation~n", [badCont])
  end
  ...
  defp do_log(r_log(type: :rotate) = l, b, _BSz) do
    {:ok, handle, logged} = :disk_log_1.rotate_ext_log(r_log(l, :extra), b, r_log(l, :head))
    :erlang.put(:log, r_log(l, extra: handle))
    logged
  end
  ...
  defp do_trunc(
         r_log(type: :rotate, head: head, extra: handle) = l,
         :none
       ) do
    handle1 = :disk_log_1.do_rotate(handle, head)

    :disk_log_1.remove_files(
      :rotate,
      r_rotate_handle(handle1, :file),
      0,
  ...
  defp wrap_file_extensions(file) do
    ...
    :lists.filter(fun, [~c"idx", ~c"siz" | fs])
  end
  ...
  defp rotate_file_extensions(file, maxF, n, res) do
    ext = :erlang.integer_to_list(n) ++ ~c".gz"
