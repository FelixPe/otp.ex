defmodule :m_disk_log do
  use Bitwise
  require Record
  Record.defrecord(:r_state, :state, queue: [], messages: [],
                                 parent: :undefined, server: :undefined, cnt: 0,
                                 args: :undefined, error_status: :ok,
                                 cache_error: :ok)
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_arg, :arg, name: 0, version: :undefined,
                               file: :none, repair: true, size: :infinity,
                               old_size: :infinity, type: :halt,
                               format: :internal, linkto: self(), head: :none,
                               mode: :read_write, notify: false, quiet: false,
                               options: [])
  Record.defrecord(:r_cache, :cache, fd: :undefined, sz: 0,
                                 c: [])
  Record.defrecord(:r_halt, :halt, fdc: :undefined,
                                curB: :undefined, size: :undefined)
  Record.defrecord(:r_handle, :handle, filename: :undefined,
                                  maxB: :undefined, maxF: :undefined,
                                  curB: :undefined, curF: :undefined,
                                  cur_fdc: :undefined, cur_name: :undefined,
                                  cur_cnt: :undefined, acc_cnt: :undefined,
                                  firstPos: :undefined, noFull: :undefined,
                                  accFull: :undefined)
  Record.defrecord(:r_rotate_handle, :rotate_handle, file: :undefined,
                                         cur_fdc: :undefined, inode: :undefined,
                                         file_check: :undefined,
                                         maxB: :undefined, maxF: :undefined,
                                         curB: 0, firstPos: :undefined,
                                         compress_on_rotate: true)
  Record.defrecord(:r_log, :log, status: :ok,
                               name: :undefined, blocked_by: :none, users: 0,
                               filename: :undefined, owners: [],
                               type: :undefined, format: :undefined,
                               format_type: :undefined, head: :none,
                               mode: :undefined, size: :undefined,
                               extra: :undefined, version: :undefined)
  Record.defrecord(:r_continuation, :continuation, pid: self(),
                                        pos: :undefined, b: :undefined)
  def open(a) do
  ...
  def truncate(log, head) do
    req(log,
          {:truncate, {:ok, :erlang.term_to_binary(head)},
             :truncate, 2})
  end
  ...
  def reopen(log, newFile, newHead) do
    req(log,
          {:reopen, newFile,
             {:ok, :erlang.term_to_binary(newHead)}, :reopen, 3})
  end

  def breopen(log, newFile, newHead) do
    req(log,
          {:reopen, newFile, {:ok, ensure_binary(newHead)},
             :breopen, 3})
  end

  def next_file(log) do
    req(log, :next_file)
  end
  ...
  defp ichunk(log, more, n)
    ...
    r = req2(r_continuation(more, :pid),
  ...
  defp ichunk_bad_end([b | bs], mode, log, c, bad, a) do
    case ((try do
      ...
      {:EXIT, _} when :read_write === mode ->
        infoList = info(log)
        {:value, {:file, fileName}} = :lists.keysearch(:file, 1,
                                                         infoList)
        file = (case (r_continuation(c, :pos)) do
      ...
      {:EXIT, _} when :read_only === mode ->
        reread = :lists.foldl(fn bin, sz ->
                                   sz + byte_size(bin) + 8
                              end,
                                0, bs)
        newPos = (case (r_continuation(c, :pos)) do
                    pos when is_integer(pos) ->
                      pos - reread
                    {fileNo, pos} ->
                      {fileNo, pos - reread}
  ...
  def istart_link(server) do
    {:ok,
       :proc_lib.spawn_link(:disk_log, :init,
                              [self(), server])}
  end
  ...
  defp check_arg([], res) do
    ...
    cond do
      ...
      r_arg(res, :type) === :wrap ->
        {oldSize,
           version} = :disk_log_1.read_size_file_version(r_arg(res, :file))
        check_wrap_arg(ret, oldSize, version)
      r_arg(res, :type) === :rotate ->
        {:ok, r_arg(res, format: :external)}
      true ->
        ret
    end
  end
  ...
  defp check_arg([{:type, :rotate} | tail], res) do
    check_arg(tail, r_arg(res, type: :rotate))
  end
  ...
  defp check_wrap_arg({:ok, res}, oldSize, version)
      when r_arg(res, :size) === :infinity do
    {:ok,
       r_arg(res, version: version,  old_size: oldSize, 
                size: oldSize)}
  end

  defp check_wrap_arg({:ok, res}, {0, 0}, version) do
    {:ok,
       r_arg(res, version: version,  old_size: r_arg(res, :size))}
  end

  defp check_wrap_arg({:ok, res}, oldSize, version)
      when oldSize === r_arg(res, :size) do
    {:ok, r_arg(res, version: version,  old_size: oldSize)}
  end
  ...
  defp check_wrap_arg({:ok, res}, oldSize, _Version)
      when (r_arg(res, :size) !== oldSize and
              r_arg(res, :mode) === :read_only) do
  ...
  defp handle({from, {:log, format, b}} = message, s) do
    case (:erlang.get(:log)) do
      ...
      r_log(status: :ok, format: :external) = l
        ...
        reply(from, {:error, {:format_external, r_log(l, :name)}},
                s)
      r_log(status: :ok, format: logFormat) ->
        log_loop(s, from, [b], [], :erlang.iolist_size(b),
                   logFormat)
  ...
  defp handle({:alog, format, b} = message, s) do
    case (:erlang.get(:log)) do
      ...
      r_log(status: :ok, format: :external)
        ...
        loop(s)
      r_log(status: :ok, format: logFormat) ->
        log_loop(s, [], [b], [], :erlang.iolist_size(b),
                   logFormat)
  ...
  defp handle({from, :unblock}, s) do
    case (:erlang.get(:log)) do
      ...
      l ->
        reply(from,
                {:error, {:not_blocked_by_pid, r_log(l, :name)}}, s)
    end
  end
    ...
    case (:erlang.get(:log)) do
      ...
      r_log(status: :ok) = l ->
        ...
        case ((try do
          ...
          error ->
            do_exit(s, from, error,
                      {{:failed, error}, [{:disk_log, f, a}]})
        end
      r_log(status: {:blocked, false}) = l ->
        reply(from, {:error, {:blocked_log, r_log(l, :name)}}, s)
      r_log(blocked_by: ^from) = l ->
  ...
  defp handle({from,
             {:change_notify, pid, newNotify}} = message,
            s) do
    case (:erlang.get(:log)) do
      r_log(status: :ok) = l ->
  ...
  defp handle({from, {:change_header, newHead}} = message,
            s) do
    case (:erlang.get(:log)) do
      r_log(mode: :read_only) = l ->
        reply(from, {:error, {:read_only_mode, r_log(l, :name)}}, s)
  ...
  defp handle({from, :inc_wrap_file} = message, s) do
    case (:erlang.get(:log)) do
      ...
      r_log(type: :rotate) = l ->
        reply(from, {:error, {:rotate_log, r_log(l, :name)}}, s)
      r_log(status: :ok) when r_state(s, :cache_error) !== :ok ->
        loop(cache_error(s, [from]))
      r_log(status: :ok) = l ->
        case ((try do
          ...
          {:ok, l2, lost} ->
            :erlang.put(:log, l2)
            notify_owners({r_log(l, :type), lost})
  ...
  defp handle({from, :next_file} = message, s) do
    case (:erlang.get(:log)) do
      r_log(mode: :read_only) = l ->
        reply(from, {:error, {:read_only_mode, r_log(l, :name)}}, s)
      r_log(type: :halt) = l ->
        ...
        case ((try do
          ...
          :closed ->
            ...
            case ((try do
              ...
              :ok ->
                h = merge_head(head, r_log(l, :head))
                case (do_open(r_arg(r_state(s, :args), name: r_log(l, :name), 
                                               repair: :truncate,  head: h, 
                                               file: file))) do
                  {:ok, res, l2, cnt} ->
                    :erlang.put(:log,
                                  r_log(l2, owners: r_log(l, :owners), 
                                          head: r_log(l, :head), 
                                          users: r_log(l, :users)))
                    notify_owners({:truncated, r_state(s, :cnt)})
                    :erlang.erase(:is_full)
              ...
              error ->
                do_exit(s, from, error,
                          {{:failed, error}, [{:disk_log, :reopen, 2}]})
            end
          error ->
            do_exit(s, from, error,
                      {{:failed, error}, [{:disk_log, f, a}]})
  ...
  defp handle({server, {:internal_open, a}}, s) do
    case (:erlang.get(:log)) do
      :undefined ->
        case (do_open(a)) do
          {:ok, res, l, cnt} ->
            ...
            r_arg(size: size, old_size: oldSize) = a
            s1 = r_state(s, args: a,  cnt: cnt)
            case (size === oldSize) do
              true ->
                reply(server, res, s1)
              false ->
                case ((try do
                        do_change_size(:erlang.get(:log), size)
                      catch
                        :error, e -> {:EXIT, {e, __STACKTRACE__}}
                        :exit, e -> {:EXIT, e}
                        e -> e
      ...
      l ->
        ...
        case (compare_arg(r_arg(a, :options), r_state(s, :args), testH,
                            r_log(l, :head))) do
          :ok ->
  ...
  defp handle({:system, from, req}, s) do
    :sys.handle_system_msg(req, from, r_state(s, :parent),
                             :disk_log, [], s)
  end
  ...
  defp log_loop(r_state(cache_error: cE) = s, pids, _Bins, _Sync, _Sz,
            _F)
      when cE !== :ok do
  ...
  defp log_loop(r_state(messages: [m | ms]) = s, pids, bins, sync, sz,
            f) do
    s1 = r_state(s, messages: ms)
    log_loop(m, pids, bins, sync, sz, f, s1)
  end

  defp log_loop({:alog, :internal, b}, pids, bins, sync, sz,
            :internal = f, s) do
    log_loop(s, pids, [b | bins], sync,
               sz + :erlang.iolist_size(b), f)
  end

  defp log_loop({:alog, :binary, b}, pids, bins, sync, sz, f,
            s) do
    log_loop(s, pids, [b | bins], sync,
               sz + :erlang.iolist_size(b), f)
  end

  defp log_loop({from, {:log, :internal, b}}, pids, bins, sync,
            sz, :internal = f, s) do
    log_loop(s, [from | pids], [b | bins], sync,
               sz + :erlang.iolist_size(b), f)
  end

  defp log_loop({from, {:log, :binary, b}}, pids, bins, sync,
            sz, f, s) do
    log_loop(s, [from | pids], [b | bins], sync,
               sz + :erlang.iolist_size(b), f)
  end
  ...
  defp do_change_notify(l, pid, notify) do
    case (is_owner(pid, l)) do
      {true, ^notify} ->
        {:ok, l}
      {true, _OldNotify} when (notify !== true and
                                 notify !== false)
                              ->
        {:error, {:badarg, :notify}}
      {true, _OldNotify} ->
  ...
  defp close_owner(pid, l, s) do
    l1 = r_log(l, owners: :lists.keydelete(pid, 1,
                                         r_log(l, :owners)))
    :erlang.put(:log, l1)
  ...
  defp do_exit(s, from, message0, reason) do
    r = do_stop(s)
    message = (case (r_state(s, :cache_error)) do
                 err when err !== :ok ->
                   err
                 _ when r === :closed ->
                   message0
  ...
  defp proc_q([{from, _R} | tail]) when is_pid(from) do
    send(from, {:disk_log, self(),
                  {:error, :disk_log_stopped}})
    proc_q(tail)
  end
  ...
  defp rename_file(file, newFile, r_log(type: :halt)) do
  ...
  defp rename_file(file, newFile, r_log(type: :wrap)) do
    rename_file(wrap_file_extensions(file), file, newFile,
                  :ok)
  end

  defp rename_file(file, newFile,
            r_log(type: :rotate, extra: handle)) do
    {_MaxB, maxF} = :disk_log_1.get_rotate_size(handle)
    :disk_log_1.rotate_files(r_rotate_handle(handle, :file), maxF)
    rename_file(rotate_file_extensions(file, maxF), file,
                  newFile, :ok)
  end

  defp rename_file([ext | exts], file, newFile0, res) do
    newFile = add_ext(newFile0, ext)
    nRes = (case (:file.rename(add_ext(file, ext),
                                 newFile)) do
              :ok ->
                res
              else__ ->
  ...
  defp compare_arg(:size, sz, a) when (sz !== r_arg(a, :size) and
                                r_arg(a, :type) === :wrap) do
    {:error, {:size_mismatch, r_arg(a, :size), sz}}
  end
  ...
  defp do_open(r_arg(type: :rotate) = a) do
    r_arg(name: name, size: size, mode: mode, head: head0,
        format: format, type: type, file: fName) = a
    head = mk_head(head0, format)
    case (:disk_log_1.open_rotate_log_file(fName, size,
                                             head)) do
      {:ok, rotHandle} ->
        l = r_log(name: name, type: type, format: format,
                filename: fName, size: size, mode: mode,
                extra: rotHandle, format_type: :rotate_ext, head: head)
        {:ok, {:ok, name}, l, 0}
      error ->
        error
    end
  ...
  defp do_open(a) do
    r_arg(type: type, format: format, name: name, head: head0,
        file: fName, repair: repair, size: size, mode: mode,
        quiet: quiet, version: v, old_size: oldSize) = a
    :disk_log_1.set_quiet(quiet)
    head = mk_head(head0, format)
    case (do_open2(type, format, name, fName, repair,
                     oldSize, mode, head, v)) do
      {:ok, ret, extra, formatType, noItems} ->
        l = r_log(name: name, type: type, format: format,
                filename: fName, size: size, format_type: formatType,
                head: head, mode: mode, version: v, extra: extra)
        {:ok, ret, l, noItems}
      error ->
        error
    end
  ...
  defp do_change_size(r_log(type: :wrap) = l, newSize) do
    ...
    {:ok, handle} = :disk_log_1.change_size_wrap(extra,
                                                   newSize, version)
    :erlang.erase(:is_full)
  ...
  defp do_change_size(r_log(type: :rotate, extra: extra) = l, newSize) do
    {:ok, handle} = :disk_log_1.change_size_rotate(extra,
                                                     newSize)
    :erlang.erase(:is_full)
    :erlang.put(:log, r_log(l, extra: handle))
    :ok
  ...
  defp check_size(:halt, newSize) when (is_integer(newSize) and
                                  newSize > 0) do
    :ok
  end
  ...
  defp check_size(type, {newMaxB, newMaxF})
      when ((type === :wrap or type === :rotate) and is_integer(newMaxB) and
              is_integer(newMaxF) and newMaxB > 0 and
              newMaxB <= 1 <<< 64 - 1 and newMaxF > 0 and
              newMaxF < 65000) do
    :ok
  end
  ...
  defp do_inc_rotate_file(r_log(extra: handle, head: head) = l) do
    handle2 = :disk_log_1.do_rotate(handle, head)
    {:ok, r_log(l, extra: handle2)}
  end

  defp do_open2(:halt, :internal, name, fName, repair, size,
            mode, head, _V) do
    case ((try do
  ...
  defp do_open2(:wrap, :internal, name, fName, repair, size,
            mode, head, v) do
    {maxB, maxF} = size
    case ((try do
            :disk_log_1.mf_int_open(fName, maxB, maxF, repair, mode,
                                      head, v)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
  ...
  defp do_open2(:halt, :external, name, fName, repair, size,
            mode, head, _V) do
    case ((try do
  ...
  defp do_open2(:wrap, :external, name, fName, repair, size,
            mode, head, v) do
    {maxB, maxF} = size
    case ((try do
            :disk_log_1.mf_ext_open(fName, maxB, maxF, repair, mode,
                                      head, v)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
  ...
  defp close_disk_log2(l) do
    case (l) do
      ...
      r_log(type: :rotate, extra: handle) ->
  ...
  defp do_format_error({node, error = {:error, _Reason}}) do
    :lists.append(:io_lib.format('~p: ', [node]),
                    do_format_error(error))
  end
  ...
  defp do_format_error({:size_mismatch, oldSize, argSize}) do
    :io_lib.format('The given size ~p does not match the size ~p found on the disk log size file~n', [argSize, oldSize])
  end

  defp do_format_error({:read_only_mode, log}) do
    :io_lib.format('The disk log ~tp has been opened read-only, but the requested operation needs read-write access~n', [log])
  end

  defp do_format_error({:format_external, log}) do
    :io_lib.format('The requested operation can only be applied on internally formatted disk logs, but ~tp is externally formatted~n', [log])
  end

  defp do_format_error({:blocked_log, log}) do
    :io_lib.format('The blocked disk log ~tp does not queue requests, or the log has been blocked by the calling process~n', [log])
  end
  ...
  defp do_format_error({:not_blocked_by_pid, log}) do
    :io_lib.format('The disk log ~tp is blocked, but only the blocking pid can unblock a disk log~n', [log])
  end

  defp do_format_error({:new_size_too_small, log, currentSize}) do
    :io_lib.format('The current size ~p of the halt log ~tp is greater than the requested new size~n', [currentSize, log])
  end
  ...
  defp do_format_error({:arg_mismatch, option, firstValue,
             argValue}) do
    :io_lib.format('The value ~tp of the disk log option ~p does not match the current value ~tp~n', [argValue, option, firstValue])
  end
  ...
  defp do_format_error({:not_internal_wrap, log}) do
    :io_lib.format('The requested operation cannot be applied since ~tp is not an internally formatted disk log~n', [log])
  end
  ...
  defp do_format_error({:need_repair, fileName}) do
    :io_lib.format('The disk log file "~ts" has not been closed properly and needs repair~n', [fileName])
  end
  ...
  def do_info(l, cnt) do
    r_log(name: name, type: type, mode: mode, filename: file,
        extra: extra, status: status, owners: owners,
        users: users, format: format, head: head) = l
    size = (case (type) do
              :wrap ->
    ...
    rW = (case (type) do
            :wrap when mode === :read_write ->
              r_handle(curB: curB, curF: curF, cur_cnt: curCnt,
                  acc_cnt: accCnt, noFull: noFull,
                  accFull: accFull) = extra
              newAccFull = accFull + noFull
              newExtra = r_handle(extra, noFull: 0,  accFull: newAccFull)
              :erlang.put(:log, r_log(l, extra: newExtra))
              [{:no_current_bytes, curB}, {:no_current_items, curCnt},
                                              {:no_items, cnt},
                                                  {:no_written_items,
                                                     curCnt + accCnt},
                                                      {:current_file, curF},
            ...
            :rotate when mode === :read_write ->
              r_rotate_handle(curB: curB) = extra
              [{:no_current_bytes, curB}, {:no_items, cnt}]
            :halt when mode === :read_write ->
              isFull = (case (:erlang.get(:is_full)) do
                          :undefined ->
                            false
                          _ ->
                            true
    ...
    common = [{:name, name}, {:file, file}, {:type, type},
                                                {:format, format}, {:size,
                                                                      size},
                                                                       {:items,
                                                                          cnt},
  ...
  defp do_block(pid, queueLogRecs, l) do
    l2 = r_log(l, status: {:blocked, queueLogRecs}, 
                blocked_by: pid)
    :erlang.put(:log, l2)
  ...
  defp do_log(r_log(format_type: :wrap_int) = l, b, _BSz) do
    case (:disk_log_1.mf_int_log(r_log(l, :extra), b,
                                   r_log(l, :head))) do
  ...
  defp do_log(r_log(format_type: :wrap_ext) = l, b, _BSz) do
    case (:disk_log_1.mf_ext_log(r_log(l, :extra), b,
                                   r_log(l, :head))) do
      {:ok, handle, logged, lost, wraps} ->
        notify_owners_wrap(wraps)
        :erlang.put(:log, r_log(l, extra: handle))
  ...
  defp do_log(r_log(type: :rotate) = l, b, _BSz) do
    {:ok, handle,
       logged} = :disk_log_1.rotate_ext_log(r_log(l, :extra), b,
                                              r_log(l, :head))
    :erlang.put(:log, r_log(l, extra: handle))
    logged
  end
  ...
  defp halt_write(halt, l, b, bs, bSize) do
    case (:disk_log_1.fwrite(r_halt(halt, :fdc), r_log(l, :filename),
                               bs, bSize)) do
      {:ok, newFdC} ->
  ...
  defp do_write_cache(r_log(filename: fName, type: :halt,
              extra: halt) = log) do
    {reply, newFdC} = :disk_log_1.write_cache(r_halt(halt, :fdc),
                                                fName)
  ...
  defp do_write_cache(r_log(type: :rotate, extra: handle) = log) do
    {reply,
       newHandle} = :disk_log_1.rotate_write_cache(handle)
    :erlang.put(:log, r_log(log, extra: newHandle))
    reply
  end

  def do_sync(r_log(filename: fName, type: :halt,
             extra: halt) = log) do
  ...
  def do_sync(r_log(type: type, extra: handle) = log)
      when type == :wrap or type == :rotate do
  ...
  defp do_trunc(r_log(type: :halt) = l, head) do
    ...
    fdC = r_halt(halt, :fdc)
    {reply1, fdC2} = (case (r_log(l, :format)) do
                        :internal ->
    ...
    {reply, newHalt} = (case (:disk_log_1.position(fdC2,
                                                     fName, :cur)) do
  ...
  defp do_trunc(r_log(type: :wrap) = l, head) do
    ...
    newHandle = r_handle(r_log(newLog2, :extra), noFull: 0, 
                                        accFull: 0)
    do_change_size(r_log(newLog2, extra: newHandle, 
                                head: oldHead),
                     {maxB, maxF})
  end

  defp do_trunc(r_log(type: :rotate, head: head, extra: handle) = l,
            :none) do
    handle1 = :disk_log_1.do_rotate(handle, head)
    :disk_log_1.remove_files(:rotate, r_rotate_handle(handle1, :file), 0,
                               r_rotate_handle(handle1, :maxF))
    :erlang.put(:log, r_log(l, extra: handle1))
    :ok
  end
  ...
  defp do_chunk(r_log(format_type: :halt_int, extra: halt) = l, pos,
            b, n) do
    fdC = r_halt(halt, :fdc)
    {newFdC, reply} = (case (r_log(l, :mode)) do
                         :read_only ->
                           :disk_log_1.chunk_read_only(fdC, r_log(l, :filename),
                                                         pos, b, n)
                         :read_write ->
  ...
  defp do_chunk(r_log(format_type: :wrap_int, mode: :read_only,
              extra: handle) = log,
            pos, b, n) do
    {newHandle,
       reply} = :disk_log_1.mf_int_chunk_read_only(handle, pos,
                                                     b, n)
    :erlang.put(:log, r_log(log, extra: newHandle))
    reply
  end

  defp do_chunk(r_log(format_type: :wrap_int, extra: handle) = log,
            pos, b, n) do
    {newHandle, reply} = :disk_log_1.mf_int_chunk(handle,
                                                    pos, b, n)
    :erlang.put(:log, r_log(log, extra: newHandle))
    reply
  end
  ...
  defp do_chunk_step(r_log(format_type: :wrap_int, extra: handle), pos,
            n) do
  ...
  defp monitor_request(pid, req) do
    ...
    receive do
      {:DOWN, ^ref, :process, ^pid, _Info} ->
        {:error, :no_such_log}
      {:disk_log, ^pid, reply} when not
                                    is_tuple(reply) or :erlang.element(2,
                                                                         reply) !== :disk_log_stopped
                                    ->
  ...
  defp wrap_file_extensions(file) do
    {_CurF, _CurFSz, _TotSz,
       noOfFiles} = :disk_log_1.read_index_file(file)
    fs = (cond do
            noOfFiles >= 1 ->
  ...
  defp rotate_file_extensions(file, maxF) do
    rotate_file_extensions(file, maxF, 0, [])
  end

  defp rotate_file_extensions(_File, maxF, maxF, res) do
    res
  end

  defp rotate_file_extensions(file, maxF, n, res) do
    ext = :erlang.integer_to_list(n) ++ '.gz'
    case (:file.read_file_info(add_ext(file, ext))) do
      {:ok, _} ->
        rotate_file_extensions(file, maxF, n + 1, [ext | res])
      _ ->
        rotate_file_extensions(file, maxF, n + 1, res)
    end
  end
