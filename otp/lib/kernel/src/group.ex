defmodule :m_group do
  ...
  def start(drv, shell, options) do
    ...
    spawn_link(:group, :server,
                 [ancestors, drv, shell, options])
  end

  def server(ancestors, drv, shell, options) do
    :erlang.process_flag(:trap_exit, true)
    _ = (for _ <- [:EFE_DUMMY_GEN], shell !== {} do
           :erlang.put(:"$ancestors", ancestors)
         end)
    :edlin.init()
    :erlang.put(:line_buffer,
                  :proplists.get_value(:line_buffer, options,
                                         :group_history.load()))
    :erlang.put(:read_mode, :list)
    :erlang.put(:user_drv, drv)
    expandFun = normalize_expand_fun(options,
      ...
      node when node === node() ->
        case (:user_drv.whereis_group()) do
          :undefined ->
            :undefined
          groupPid ->
            {:dictionary, dict} = :erlang.process_info(groupPid,
                                                         :dictionary)
  ...
  def server_loop(drv, shell, buf0) do
    receive do
      ...
      {:EXIT, ^shell, r} ->
        exit(r)
      notDrvTuple when not
                       is_tuple(notDrvTuple) or tuple_size(notDrvTuple) !== 2 or :erlang.element(1,
                                                                                                   notDrvTuple) !== drv
                       ->
        :group.server_loop(drv, shell, buf0)
    end
  ...
  defp io_request(req, from, replyAs, drv, shell, buf0) do
    case (io_request(req, drv, shell, {from, replyAs},
                       buf0)) do
      {:ok, reply, buf} ->
        io_reply(from, replyAs, reply)
        buf
  ...
  defp io_request({:put_chars, :unicode, chars}, drv, _Shell,
            from, buf) do
    case ((try do
  ...
  defp io_request({:put_chars, :unicode, m, f, as}, drv, _Shell,
            from, buf) do
    case ((try do
            apply(m, f, as)
          catch
  ...
  defp io_request({:put_chars, :latin1, binary}, drv, _Shell,
            from, buf)
      when is_binary(binary) do
    send_drv(drv,
               {:put_chars_sync, :unicode,
                  :unicode.characters_to_binary(binary, :latin1), from})
    {:noreply, buf}
  end

  defp io_request({:put_chars, :latin1, chars}, drv, _Shell, from,
            buf) do
    case ((try do
  ...
  defp io_request({:put_chars, :latin1, m, f, as}, drv, _Shell,
            from, buf) do
    case ((try do
            apply(m, f, as)
          catch
  ...
  defp io_request({:get_chars, encoding, prompt, n}, drv, shell,
            _From, buf) do
    get_chars_n(prompt, :io_lib, :collect_chars, n, drv,
                  shell, buf, encoding)
  end

  defp io_request({:get_line, encoding, prompt}, drv, shell,
            _From, buf) do
    get_chars_line(prompt, :io_lib, :collect_line, [], drv,
                     shell, buf, encoding)
  end

  defp io_request({:get_until, encoding, prompt, m, f, as}, drv,
            shell, _From, buf) do
    get_chars_line(prompt, :io_lib, :get_until, {m, f, as},
                     drv, shell, buf, encoding)
  end

  defp io_request({:get_password, _Encoding}, drv, shell, _From,
            buf) do
  ...
  defp io_request({:get_geometry, :columns}, drv, _Shell, _From,
            buf) do
  ...
  defp io_request({:get_geometry, :rows}, drv, _Shell, _From,
            buf) do
    case (get_tty_geometry(drv)) do
      {_W, h} ->
        {:ok, h, buf}
  ...
  defp io_request({:put_chars, chars}, drv, shell, from, buf) do
    io_request({:put_chars, :latin1, chars}, drv, shell,
                 from, buf)
  end

  defp io_request({:put_chars, m, f, as}, drv, shell, from,
            buf) do
    io_request({:put_chars, :latin1, m, f, as}, drv, shell,
                 from, buf)
  end

  defp io_request({:get_chars, prompt, n}, drv, shell, from,
            buf) do
    io_request({:get_chars, :latin1, prompt, n}, drv, shell,
                 from, buf)
  end

  defp io_request({:get_line, prompt}, drv, shell, from, buf) do
    io_request({:get_line, :latin1, prompt}, drv, shell,
                 from, buf)
  end

  defp io_request({:get_until, prompt, m, f, as}, drv, shell,
            from, buf) do
    io_request({:get_until, :latin1, prompt, m, f, as}, drv,
                 shell, from, buf)
  end

  defp io_request(:get_password, drv, shell, from, buf) do
    io_request({:get_password, :latin1}, drv, shell, from,
                 buf)
  end
  ...
  defp io_requests([r | rs], {:noreply, buf}, from, drv, shell) do
    ...
    io_requests(rs, io_request(r, drv, shell, reqFrom, buf),
                  from, drv, shell)
  end
  ...
  defp check_valid_opts([{:expand_fun, fun} | t]) when is_function(fun,
                                                       1) or
  ...
  defp normalize_expand_fun(options, default) do
    case (:proplists.get_value(:expand_fun, options,
                                 default)) do
  ...
  defp get_chars_n(prompt, m, f, xa, drv, shell, buf, encoding) do
    ...
    case (:erlang.get(:echo)) do
      true ->
        get_chars_loop(pbs, m, f, xa, drv, shell, buf, :start,
                         [], encoding)
      false ->
        get_chars_n_loop(pbs, m, f, xa, drv, shell, buf, :start,
                           encoding)
    end
  end
  ...
  defp get_chars_loop(pbs, m, f, xa, drv, shell, buf0, state,
            lineCont0, encoding) do
    result = (case (not
                    :erlang.get(:dumb) and :erlang.get(:echo)) do
                true ->
    ...
    case (result) do
      {:done, lineCont1, buf} ->
        get_chars_apply(pbs, m, f, xa, drv, shell,
                          append(buf, [], encoding), state, lineCont1, encoding)
  ...
  defp get_chars_apply(pbs, m, f, xa, drv, shell, buf, state0,
            lineCont, encoding) do
    {state, line} = (case (not
                           :erlang.get(:dumb) and :erlang.get(:echo)) do
                       true ->
    ...
    case ((try do
            apply(m, f,
                    [state, cast(line, :erlang.get(:read_mode), encoding),
                                encoding, xa])
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
      ...
      {:stop, result, rest} ->
        case (lineCont) do
          {[cL | lB], _, _} ->
            ...
            multiLinePrompt = :lists.duplicate(:prim_tty.npwcwidthstring(pbs),
                                                 ?\s)
            send_drv_reqs(drv,
                            [{:redraw_prompt, pbs, multiLinePrompt, lineCont1},
                                 :new_prompt])
          _ ->
            :skip
        end
      ...
      state1 ->
        get_chars_loop(pbs, m, f, xa, drv, shell, buf, state1,
                         lineCont, encoding)
    end
  end

  defp get_chars_n_loop(pbs, m, f, xa, drv, shell, buf0, state,
            encoding) do
    case (check_encoding(buf0, encoding)) do
      false ->
        {:error,
           {:error, {:no_translation, :unicode, encoding}}, []}
      true ->
        try do
          apply(m, f,
                  [state, cast(buf0, :erlang.get(:read_mode), encoding),
                              encoding, xa])
        catch
          _, _ ->
            {:error, {:error, err_func(m, f, xa)}, []}
          ...
          state1 ->
            case (get_chars_echo_off(pbs, drv, shell)) do
              ...
              :terminated ->
                {:exit, :terminated}
              buf ->
                get_chars_n_loop(pbs, m, f, xa, drv, shell, buf, state1,
                                   encoding)
            end
        end
    end
  ...
  defp get_line(chars, pbs, cont, drv, shell, encoding) do
    {:more_chars, cont1, rs} = (case (cont) do
                                  [] ->
                                    :edlin.start(pbs)
                                  _ ->
    ...
    get_line1(:edlin.edit_line(chars, cont1), drv, shell,
                new_stack(:erlang.get(:line_buffer)), encoding)
  end

  defp get_line1({:done, cont, rest, rs}, drv, _Shell, _Ls,
            _Encoding) do
    send_drv_reqs(drv, rs)
    {:done, cont, rest}
  end

  defp get_line1({:open_editor, _Cs, cont, rs}, drv, shell, ls0,
            encoding) do
    send_drv_reqs(drv, rs)
    ...
    receive do
      {^drv, {:editor_data, cs1}} ->
        ...
        {:more_chars, newCont,
           newRs} = :edlin.start(:edlin.prompt(cont))
        send_drv_reqs(drv, newRs)
        get_line1(:edlin.edit_line(cs1, newCont), drv, shell,
                    ls0, encoding)
    end
  end

  defp get_line1({:history_up, cs, cont, rs}, drv, shell, ls0,
            encoding) do
    send_drv_reqs(drv, rs)
    case (up_stack(save_line(ls0,
                               :edlin.current_line(cont)))) do
      {:none, _Ls} ->
        send_drv(drv, :beep)
        get_line1(:edlin.edit_line(cs, cont), drv, shell, ls0,
                    encoding)
      {lcs, ls} ->
        send_drv_reqs(drv, :edlin.erase_line())
        {:more_chars, ncont,
           nrs} = :edlin.start(:edlin.prompt(cont))
  ...
  defp get_line1({:history_down, cs, cont, rs}, drv, shell, ls0,
            encoding) do
    send_drv_reqs(drv, rs)
    case (down_stack(save_line(ls0,
                                 :edlin.current_line(cont)))) do
      {:none, _Ls} ->
        send_drv(drv, :beep)
        get_line1(:edlin.edit_line(cs, cont), drv, shell, ls0,
  ...
  defp get_line1({:search, cs, cont, rs}, drv, shell, ls,
            encoding) do
    send_drv_reqs(drv, rs)
    :erlang.put(:search_quit_prompt, cont)
    pbs = prompt_bytes('\e[;1;4msearch:\e[0m ', encoding)
    {:more_chars, ncont, _Nrs} = :edlin.start(pbs,
                                                {:search, :none})
    get_line1(:edlin.edit_line1(cs, ncont), drv, shell, ls,
                encoding)
  end

  defp get_line1({expand, before, cs0, cont, rs}, drv, shell,
            ls0, encoding)
    ...
    matchStr = (case (matches) do
                  [] ->
                    []
                  _ ->
                    :edlin_expand.format_matches(matches, width)
                end)
    cs = (case ({cs1, matchStr}) do
            {_, []} ->
              cs1
            {^cs1, _} when cs1 !== [] ->
              cs1
  ...
  defp get_line1({:search_found, _Cs, _, rs}, drv, shell, ls0,
            encoding) do
    searchResult = :erlang.get(:search_result)
    lineCont = (case (searchResult) do
                  [] ->
                    {[], {[], []}, []}
                  _ ->
    ...
    send_drv_reqs(drv, :edlin.erase_line())
    send_drv_reqs(drv,
                    :edlin.redraw_line({:line, prompt, lineCont,
                                          {:normal, :none}}))
  ...
  defp get_line1({:search_quit, _Cs, _, rs}, drv, shell, ls,
            encoding) do
    case (:edlin.prompt(:erlang.get(:search_quit_prompt))) do
      prompt ->
        searchResult = :erlang.get(:search_result)
        l = (case (searchResult) do
               [] ->
                 {[], {[], []}, []}
               _ ->
                 [last | lB] = :lists.reverse(searchResult)
        ...
        get_line1({:more_chars, nCont, []}, drv, shell,
                    pad_stack(ls), encoding)
    end
  end

  defp get_line1({:search_cancel, _Cs, _, rs}, drv, shell, ls,
            encoding) do
    nCont = :erlang.get(:search_quit_prompt)
    :erlang.put(:search_result, [])
    send_drv_reqs(drv, [:delete_line | rs])
    send_drv_reqs(drv, :edlin.redraw_line(nCont))
    get_line1({:more_chars, nCont, []}, drv, shell, ls,
                encoding)
  end

  defp get_line1({what,
             {:line, prompt, {_, {revCmd0, _}, _}, {:search, :none}},
             _Rs},
            drv, shell, ls0, encoding) do
    {search, ls1, revCmd} = (case (revCmd0) do
                               [19 | revCmd1] ->
                                 {&search_down_stack/2, ls0, revCmd1}
                               [18 | revCmd1] ->
                                 {&search_up_stack/2, ls0, revCmd1}
                               _ ->
                                 {&search_up_stack/2, new_stack(get_lines(ls0)),
                                    revCmd0}
                             end)
    cmd = :lists.reverse(revCmd)
    {ls, newStack} = (case (search.(ls1, cmd)) do
                        {:none, ls2} ->
                          send_drv(drv, :beep)
                          :erlang.put(:search_result, [])
                          ...
                          send_drv(drv,
                                     {:insert_chars, :unicode,
                                        :unicode.characters_to_binary(prompt ++ cmd)})
                          {ls2, {[], {revCmd, []}, []}}
                        {line, ls2} ->
                          ...
                          output = (cond do
                                      length(lines) > 5 ->
                                        ...
                                        (for line1 <- [a, b, c, d, e] do
                                           '\n  ' ++ line1
                                         end) ++ [:io_lib.format('~n  ... (~w lines omitted)',
                                                                   [length(lines) - 5])]
                                      true ->
                                        for line1 <- lines do
                                          '\n  ' ++ line1
  ...
  defp more_data(what, cont0, drv, shell, ls, encoding) do
    receive do
      ...
      {^drv, :eof} ->
        get_line1(:edlin.edit_line(:eof, cont0), drv, shell, ls,
                    encoding)
      {:io_request, from, replyAs, req} when is_pid(from) ->
        ...
        get_line1({:more_chars, cont, []}, drv, shell, ls,
                    encoding)
      {:reply, {from, replyAs}, reply} ->
        io_reply(from, replyAs, reply)
        more_data(what, cont0, drv, shell, ls, encoding)
      ...
      {:EXIT, ^shell, r} ->
        exit(r)
    after get_line_timeout(what) ->
      get_line1(:edlin.edit_line([], cont0), drv, shell, ls,
                  encoding)
    end
  end
  ...
  defp get_line_echo_off(chars, toEnc, pbs, drv, shell) do
    ...
    case (get_line_echo_off1(edit_line(chars, []), drv,
                               shell)) do
  ...
  defp get_line_echo_off1({chars, [], rs}, drv, shell) do
    ...
    receive do
      {^drv, {:data, cs}} ->
        get_line_echo_off1(edit_line(cast(cs, :list), chars),
                             drv, shell)
      {^drv, :eof} ->
  ...
  defp edit_line([?\r, ?\n | cs], chars, rs) do
    {[?\n | chars], remainder_after_nl(cs),
       :lists.reverse([{:put_chars, :unicode, '\n'} | rs])}
  end

  defp edit_line([nL | cs], chars, rs) when nL === ?\r or
                                       nL === ?\n do
    {[?\n | chars], remainder_after_nl(cs),
       :lists.reverse([{:put_chars, :unicode, '\n'} | rs])}
  end

  defp edit_line([erase | cs], [], rs) when erase === ?\d or
                                       erase === ?\b do
    edit_line(cs, [], rs)
  end
  ...
  defp get_password1({chars, []}, drv, shell) do
    receive do
      {^drv, {:data, cs}} ->
        get_password1(edit_password(cast(cs, :list), chars),
                        drv, shell)
      {:io_request, from, replyAs, req} when is_pid(from) ->
        io_request(req, from, replyAs, drv, shell, [])
  ...
  defp append(b, l, fromEnc) do
    append(:unicode.characters_to_list(b, fromEnc), l,
             fromEnc)
  end
