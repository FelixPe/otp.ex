defmodule :m_logger_formatter do
  ...
  defp format_msg({:string, chardata}, meta, config) do
    format_msg({~c"~ts", [chardata]}, meta, config)
  end
  ...
  defp format_msg({:report, report}, %{report_cb: fun} = meta, config)
    ...
    try do
      ...
      c, r ->
        ...
        format_msg(
          {~c"REPORT_CB/1 CRASH: " ++ p ++ ~c"; Reason: " ++ p,
           [
             report,
             {c, r,
      ...
      other ->
        ...
        format_msg(
          {~c"REPORT_CB/1 ERROR: " ++ p ++ ~c"; Returned: " ++ p, [report, other]},
          meta,
          config
        )
    end
  ...
  defp format_msg({:report, report}, %{report_cb: fun} = meta, config)
    ...
    try do
      ...
      c, r ->
        ...
        format_msg(
          {~c"REPORT_CB/2 CRASH: " ++ p ++ ~c"; Reason: " ++ p,
           [
             report,
             {c, r,
              :logger.filter_stacktrace(
      ...
      chardata when is_list(chardata) or is_binary(chardata) ->
        try do
          ...
          _, _ ->
            ...
            format_msg(
              {~c"REPORT_CB/2 ERROR: " ++ p ++ ~c"; Returned: " ++ p, [report, chardata]},
              meta,
              config
            )
        end
      ...
      other ->
        ...
        format_msg(
          {~c"REPORT_CB/2 ERROR: " ++ p ++ ~c"; Returned: " ++ p, [report, other]},
          meta,
          config
        )
    end
  ...
  defp format_msg({format0, args}, depth, opts, single) do
    try do
      ...
      c, r ->
        p = p(single)
        formatError = ~c"FORMAT ERROR: " ++ p ++ ~c" - " ++ p
  ...
  defp truncate(b, msg, a, size) do
    ...
    cond do
      length > size ->
        ...
        case last do
          ?\n ->
            :lists.sublist(flatString, 1, size - 4) ++ ~c"...\n"

          _ ->
            :lists.sublist(flatString, 1, size - 3) ++ ~c"..."
        end
  ...
  defp timestamp_to_datetimemicro(sysTime, config) when is_integer(sysTime) do
    ...
    {{date, time}, utcStr} =
      case offset_to_utc(
        ...
        _ ->
          {:erlang.universaltime_to_localtime(universalTime), ~c""}
      end
  ...
  defp maybe_add_legacy_header(level, %{time: timestamp} = meta, %{legacy_header: true} = config) do
    ...
    header =
      :io_lib.format(
        ~c"=~ts==== ~w-~s-~4w::~2..0w:~2..0w:~2..0w.~6..0w ~s===",
  ...
  defp add_legacy_title(level, meta, config) do
    case :maps.get(:logger_formatter, meta, %{}) do
      ...
      myMeta ->
        ...
        title = :string.uppercase(:erlang.atom_to_list(titleLevel)) ++ ~c" REPORT"
  ...
  defp default_template(%{legacy_header: true}) do
    [[:logger_formatter, :header], ~c"\n", :msg, ~c"\n"]
  end

  defp default_template(%{single_line: true}) do
    [:time, ~c" ", :level, ~c": ", :msg, ~c"\n"]
  end
  ...
  defp offset_to_utc(z) when z === 0 or z === ~c"z" or z === ~c"Z" do
    true
  end

  defp offset_to_utc([?+ | tz]) do
    case :io_lib.fread(~c"~d:~d", tz) do
      {:ok, [0, 0], []} ->
        true
  ...
  defp check_offset(tz) when tz === ~c"" or tz === ~c"Z" or tz === ~c"z" do
    :ok
  end
