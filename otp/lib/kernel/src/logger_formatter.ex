defmodule :m_logger_formatter do
  ...
  def format(%{level: level, msg: msg0, meta: meta}, config0)
    ...
    {bT, aT0} = :lists.splitwith(fn :msg ->
                                      false
                                    _ ->
                                      true
                                 end,
                                   linearTemplate)
    {doMsg, aT} = (case (aT0) do
                     [:msg | rest] ->
                       {true, rest}
                     _ ->
                       {false, aT0}
    ...
    msgStr = (cond do
                doMsg ->
                  config1 = (case (:maps.get(:chars_limit, config)) do
                               :unlimited ->
                                 config
                               size0 ->
                                 size = (case (size0 - :io_lib.chars_length([b,
                                                                                 a])) do
                                           s when s >= 0 ->
                                             s
                                           _ ->
                  ...
                  case (:maps.get(:single_line, config)) do
                    true ->
                      t = :lists.reverse(trim(:lists.reverse(trim(msgStr0,
                                                                    false)),
                                                true))
                      :re.replace(t, ',?\r?\n *', ', ',
  ...
  defp linearize_template(data, [{key, ifExist, else__} | format]) do
    branchForUse = (case (value(key, data)) do
                      {:ok, _Value} ->
                        linearize_template(data, ifExist)
                      :error ->
                        linearize_template(data, else__)
                    end)
    branchForUse ++ linearize_template(data, format)
  end

  defp linearize_template(data, [strOrKey | format]) do
    [strOrKey | linearize_template(data, format)]
  end

  defp linearize_template(_Data, []) do
    []
  end

  defp trim([h | t], rev) when h == ?\s or h == ?\r or
                               h == ?\n do
    trim(t, rev)
  end
  ...
  defp value([key | keys], meta) when :erlang.is_map_key(key,
                                                        meta) do
  ...
  defp format_msg({:report, _} = msg, meta,
            %{report_cb: fun} = config)
      when is_function(fun, 1) or is_function(fun, 2) do
    format_msg(msg, Map.put(meta, :report_cb, fun),
                 :maps.remove(:report_cb, config))
  end

  defp format_msg({:report, report}, %{report_cb: fun} = meta,
            config)
    ...
    try do
      ...
      c, r ->
        ...
        format_msg({'REPORT_CB/1 CRASH: ' ++ p ++ '; Reason: ' ++ p,
                      [report, {c, r,
                                  :logger.filter_stacktrace(:logger_formatter,
                                                              __STACKTRACE__)}]},
                     meta, config)
    else
      {format, args} when (is_list(format) and is_list(args))
        ...
        format_msg({format, args},
                     :maps.remove(:report_cb, meta), config)
      other ->
        p = p(config)
        format_msg({'REPORT_CB/1 ERROR: ' ++ p ++ '; Returned: ' ++ p, [report, other]}, meta,
                     config)
    end
  end
  ...
  defp format_msg({:report, report}, %{report_cb: fun} = meta,
    ...
    try do
      fun.(report,
             :maps.with([:depth, :chars_limit, :single_line],
                          config))
    catch
      c, r ->
        p = p(config)
      ...
      chardata when is_list(chardata) or is_binary(chardata)
        ...
        try do
          ...
          _, _ ->
            p = p(config)
            format_msg({'REPORT_CB/2 ERROR: ' ++ p ++ '; Returned: ' ++ p, [report, chardata]}, meta,
                         config)
        end
      other ->
        p = p(config)
        format_msg({'REPORT_CB/2 ERROR: ' ++ p ++ '; Returned: ' ++ p, [report, other]}, meta,
                     config)
    end
  end
  ...
  defp format_msg(msg, _Meta,
            %{depth: depth, chars_limit: charsLimit,
                single_line: single}) do
  ...
  defp reformat([%{control_char: c} = m | t], depth, true)
      when c === ?p do
    [limit_depth(Map.put(m, :width, 0), depth) | reformat(t,
                                                            depth, true)]
  end
  ...
  defp timestamp_to_datetimemicro(sysTime, config) when is_integer(sysTime) do
    micro = rem(sysTime, 1000000)
    sec = div(sysTime, 1000000)
    universalTime = :erlang.posixtime_to_universaltime(sec)
    {{date, time},
       utcStr} = (case (offset_to_utc(:maps.get(:time_offset,
                                                  config))) do
                    true ->
                      {universalTime, 'UTC '}
                    _ ->
  ...
  defp format_mfa({m, f, a}, _) when (is_atom(m) and
                                is_atom(f) and is_integer(a)) do
    :io_lib.fwrite('~tw:~tw/~w', [m, f, a])
  end

  defp format_mfa({m, f, a}, config) when (is_atom(m) and
  ...
  defp maybe_add_legacy_header(level, %{time: timestamp} = meta,
            %{legacy_header: true} = config) do
    %{title: title} = (myMeta = add_legacy_title(level,
                                                   meta, config))
    {{y, mo, d}, {h, mi, s}, micro,
       utcStr} = timestamp_to_datetimemicro(timestamp, config)
    header = :io_lib.format('=~ts==== ~w-~s-~4w::~2..0w:~2..0w:~2..0w.~6..0w ~s===',
                              [title, d, month(mo), y, h, mi, s, micro, utcStr])
    Map.put(meta, :logger_formatter,
                    Map.put(myMeta, :header, header))
  end
  ...
  defp add_legacy_title(_Level,
            %{logger_formatter: %{title: _} = myMeta}, _) do
    myMeta
  end
  ...
  defp add_legacy_title(level, meta, config) do
    case (:maps.get(:logger_formatter, meta, %{})) do
      %{title: _} = myMeta ->
        myMeta
      myMeta ->
        titleLevel = (case (level === :notice and :maps.find(:error_logger,
                                                               meta)) do
                        {:ok, _} ->
  ...
  defp add_default_config(config0) do
    default = %{chars_limit: :unlimited,
                  error_logger_notice_header: :info, legacy_header: false,
                  single_line: true, time_designator: ?T}
    maxSize = get_max_size(:maps.get(:max_size, config0,
                                       :undefined))
    depth = get_depth(:maps.get(:depth, config0,
                                  :undefined))
    offset = get_offset(:maps.get(:time_offset, config0,
                                    :undefined))
    add_default_template(:maps.merge(default,
                                       Map.merge(config0, %{max_size: maxSize,
                                                              depth: depth,
                                                              time_offset:
                                                              offset})))
  end
  ...
  def check_config(config) do
    {:error,
       {:invalid_formatter_config, :logger_formatter, config}}
  end
  ...
  defp do_check_config([{type, l} | config])
    ...
    case (check_limit(l)) do
      ...
      :error ->
        {:error,
           {:invalid_formatter_config, :logger_formatter,
              {type, l}}}
    end
  end
  ...
  defp do_check_config([{:template, t} | config]) do
    case (check_template(t)) do
      ...
      :error ->
        {:error,
           {:invalid_formatter_template, :logger_formatter, t}}
    end
  end
  ...
  defp do_check_config([c | _]) do
    {:error,
       {:invalid_formatter_config, :logger_formatter, c}}
  end
  ...
  defp check_template([key | t]) when (is_list(key) and
                             is_atom(hd(key))) do
    case (:lists.all(fn x when is_atom(x) ->
                          true
                        _ ->
                          false
                     end,
  ...
  defp check_template([bin | t]) when is_binary(bin) do
