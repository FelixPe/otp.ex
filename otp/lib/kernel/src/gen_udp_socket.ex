defmodule :m_gen_udp_socket do
  ...
  def connect({:"$inet", :gen_udp_socket, {_Server, socket}},
           address, port) do
    {mod, _} = :inet.udp_module([], address)
    domain = domain(mod)
    try do
      (
        dest = (case (mod.getaddr(address)) do
  ...
  defp open_lookup(service, opts0) do
    ...
    try do
      (
        ...
        r_udp_opts(fd: fd, ifaddr: bindIP, port: bindPort,
            opts: openOpts) = val(errRef,
                                    :inet.udp_options(opts_4, mod))
        bindAddr = bind_addr(domain, bindIP, bindPort, fd)
        extraOpts = extra_opts(fd)
        do_open(mod, bindAddr, domain, openOpts, startOpts,
                  extraOpts)
      )
    catch
      {^errRef, reason} ->
  ...
  defp do_open(mod, bindAddr, domain, openOpts, opts,
            extraOpts) do
    {socketOpts, startOpts} = setopts_split(:socket, opts)
    case (start_server(mod, domain, start_opts(startOpts),
                         extraOpts)) do
      {:ok, server} ->
  ...
  defp which_default_bind_address2(domain) do
    case (net_getifaddrs(domain)) do
      {:ok, addrs} ->
        upNonLoopbackAddrs = (for (%{flags: flags,
                                       addr: %{addr: _A}} = addr) <- addrs,
                                    not
  ...
  def send({:"$inet", :gen_udp_socket, {_Server, socket}},
           data) do
  ...
  def send(socket, {_, _} = destination, ancData, data) do
    do_sendmsg(socket, dest2sockaddr(destination),
                 make_iov(data), make_cmsghdrs(ancData))
  end

  def send(socket, addr, port, data)
      when is_tuple(addr) and (tuple_size(addr) === 4 or tuple_size(addr) === 8) and is_integer(port) do
    send(socket, {addr, port}, data)
  end

  def send({:"$inet", :gen_udp_socket, {_, eSock}} = socket, host,
           service, data)
    ...
    case (:socket.getopt(eSock, {:otp, :domain})) do
      {:ok, domain} ->
        case (:inet.getaddr(host, domain)) do
          {:ok, addr} ->
            {:ok, %{mod: mod}} = :socket.getopt(eSock,
                                                  {:otp, :meta})
  ...
  defp do_sendto({:"$inet", :gen_udp_socket, {_Server, socket}}, dest,
            data) do
  ...
  defp do_sendmsg({:"$inet", :gen_udp_socket, {_Server, socket}},
            sockAddr, iOV, ctrl)
  ...
  def recv({:"$inet", :gen_udp_socket, {server, _Socket}}, length,
           timeout) do
    case ((
  ...
  def controlling_process({:"$inet", :gen_udp_socket, {server, _Socket}} = s,
           newOwner)
  ...
  def monitor({:"$inet", :gen_udp_socket,
  ...
  defp setopts_split(filterTags, [opt | opts], true__, false__) do
    ...
    case (member(filterTags, setopt_categories(opt_1))) do
      true ->
        setopts_split(filterTags, opts, [opt_1 | true__],
                        false__)
      false ->
        setopts_split(filterTags, opts, true__,
                        [opt_1 | false__])
    end
  end
  ...
  defp socket_setopt_opts([{_Level, _OptKey} = opt | opts], socket, tag,
            value) do
          ...
          false ->
            case (:maps.is_key(tag, ignore_opt())) do
              true ->
                %{ignore: []}
              false ->
                :maps.merge(case (:maps.is_key(tag,
                                                 server_read_opts())) do
                              true ->
                                %{server_read: []}
                              false ->
  ...
  defp ignore_opt() do
    %{udp_module: [], ip: [], high_msgq_watermark: [],
        low_msgq_watermark: []}
  end

  defp socket_opt() do
    %{buffer: {:otp, :rcvbuf}, debug: {:otp, :debug},
        fd: {:otp, :fd}, broadcast: {:socket, :broadcast},
        ...
        sndbuf: {:socket, :sndbuf}, recvtos: {:ip, :recvtos},
        recvttl: {:ip, :recvttl}, tos: {:ip, :tos},
        ttl: {:ip, :ttl},
  ...
  defp server_read_opts() do
    :maps.merge(%{active: false, mode: :list, header: 0,
                    deliver: :term, read_packets: 5, start_opts: [],
                    exit_on_close: true},
  ...
  defp start_server(mod, domain, startOpts, extraOpts) do
    ...
    case (:gen_statem.start(:gen_udp_socket, arg,
                              startOpts)) do
      {:ok, _} = oK ->
        oK
  ...
  def init(arg) do
    :error_logger.error_report([{:badarg,
                                   {:gen_udp_socket, :init, [arg]}}])
    ...
    case (:socket.open(fD, opts)) do
      {:ok, socket} = oK ->
        case (:socket.info(socket)) do
          %{ctype: :fromfd, domain: ^domain, type: :dgram,
  ...
  def handle_event({:call, from}, :get_socket, _State,
           {r_params(socket: socket), _D}) do
  ...
  def handle_event(:info, {:DOWN, ownerMon, _, _, reason} = _DOWN,
           _State, {r_params(owner_mon: ownerMon) = _P, _D} = p_D) do
    {:stop, {:shutdown, reason}, p_D}
  end

  def handle_event(:info, {:"$socket", socket, :counter_wrap, counter},
           :open = _State, {r_params(socket: socket) = p, d}) do
  ...
  def handle_event({:call, from}, {:setopts, opts}, state,
           {p, d}) do
    {result_1, {p_1, d_1}} = state_setopts(p, d, state,
                                             opts)
    result = (case (result_1) do
                ...
                {:error, {:invalid, _}} ->
                  ...
                  {:error, :einval}
                {:error, :einval} ->
                  _ = :socket.setopt(r_params(p_1, :socket), {:otp, :meta},
                                       meta(d_1))
                  result_1
                _ ->
                  :ok = :socket.setopt(r_params(p_1, :socket), {:otp, :meta},
                                         meta(d_1))
                  result_1
              end)
      ...
      :closed ->
        {:keep_state_and_data,
           [{:reply, from,
               %{rstates: [:closed], wstates: [:closed]}}]}
      _ ->
        {d_1, result} = handle_info(state, r_params(p, :socket),
                                      r_params(p, :owner), d)
  ...
  def handle_event(:info, {:"$socket", socket, :abort, {handle, reason}},
  ...
  defp handle_shutdown(r_params(socket: socket), :open = _State,
            :write = how) do
  ...
  defp handle_unexpected(type, content, state, {p, _D}) do
    :error_logger.warning_report([{:module,
                                     :gen_udp_socket},
                                      {:socket, r_params(p, :socket)}, {:unknown_event,
                                                                   {type,
                                                                      content}},
                                                                    {:state,
                                                                       state}])
  ...
  defp handle_reading(:open = _State, p, %{active: active} = d,
            actionsR)
      when active !== false do
    handle_recv(p, recv_start(d), actionsR, :recv)
  end

  defp handle_reading(r_recv(info: info) = _State, r_params(socket: socket) = p,
            %{active: active} = d, actionsR)
      when active === false do
    _ = socket_cancel(socket, info)
    {d2, actionsR2} = cleanup_recv_reply(p, d, actionsR,
                                           :normal)
    {:next_state, :open,
       {p, recv_stop(%{d2 | active: false})},
       reverse(actionsR2)}
  end
  ...
  defp handle_recv(r_params(socket: socket, recv_method: []) = p,
            %{recv_length: length} = d, actionsR, cS)
      when cS === :recv do
  ...
  defp handle_recv(r_params(socket: socket) = p,
            %{recv_length: length} = d, actionsR, cS)
      when cS === :recv do
  ...
  defp handle_recv_error(p, d, actionsR, reason) do
    {d_1, actionsR_1} = cleanup_recv_reply(p, d, actionsR,
                                             reason)
    case (reason) do
      :closed ->
  ...
  defp cleanup_recv_reply(p, d, actionsR, reason0) do
    ...
    {recv_stop(%{d | active: false}),
       case (d) do
         %{recv_from: from} ->
           [{:reply, from, {:error, reason}}, {{:timeout, :recv},
                                                 :cancel} |
                                                  actionsR]
         %{} ->
           actionsR
  ...
  defp recv_data_deliver(r_params(owner: owner) = p,
            %{mode: mode, deliver: deliver} = d, actionsR, data) do
    {iP, port, ancData, deliverData} = deliver_data(data,
                                                      mode)
    case (d) do
      %{recv_from: from} ->
        {recv_stop(d),
           [{:reply, from,
               {:ok, mk_recv_reply(iP, port, ancData, deliverData)}},
  ...
  defp deliver_recv_msg(pid, active, deliver, socket, iP, port, ancData,
            data) do
    send(pid, mk_recv_msg(active, deliver, socket, iP, port,
                            ancData, data))
  end

  defp mk_recv_msg(true = _Active, :port = _Deliver, socket, iP,
            port, :undefined, data) do
    {socket, {:data, [iP, port, data]}}
  end

  defp mk_recv_msg(true = _Active, :port = _Deliver, socket, iP,
            port, ancData, data) do
    {socket, {:data, [iP, port, ancData, data]}}
  end

  defp mk_recv_msg(_Active, _Deliver, socket, iP, port, :undefined,
            data) do
    {:udp, socket, iP, port, data}
  end

  defp mk_recv_msg(_Active, _Deliver, socket, iP, port, ancData,
            data) do
    {:udp, socket, iP, port, ancData, data}
  end

  defp deliver_data({%{family: :local = fam, path: path},
             <<data :: binary>>},
            mode) do
    {{fam, path}, 0, :undefined,
       deliver_data_mode(data, mode)}
  end

  defp deliver_data({%{family: fam, addr: addr, port: port},
             <<data :: binary>>},
            mode)
  ...
  defp deliver_data(%{addr: %{family: :local = fam, path: path},
              iov: iOV, ctrl: ctrl},
            mode) do
  ...
  defp deliver_data(%{addr: %{family: fam, addr: addr, port: port},
              iov: iOV, ctrl: ctrl},
            mode)
      when fam === :inet or fam === :inet6 do
    data = deliver_data_mode(iOV, mode)
  ...
  defp deliver_data({%{family: :unspec, addr: addr},
             <<data :: binary>>},
            mode)
      when is_binary(addr) do
    {{:unspec, addr}, 0, :undefined,
       deliver_data_mode(data, mode)}
  end

  defp deliver_data({unspec, <<data :: binary>>}, mode)
      when is_binary(unspec) do
    {{:unspec, unspec}, 0, :undefined,
       deliver_data_mode(data, mode)}
  end
  ...
  defp ctrl2ancdata([%{level: :ip, type: tOS, value: value,
               data: _Data} |
               cTRL],
            ancData)
  ...
  defp ctrl2ancdata([%{level: :ip, type: tTL, value: value,
               data: _Data} |
               cTRL],
            ancData)
  ...
  defp ctrl2ancdata([%{level: :ipv6, type: :tclass, value: tClass,
               data: _Data} |
               cTRL],
            ancData) do
  ...
  defp handle_info(state, socket, owner, %{active: active} = d) do
    ...
    {d_1,
       Map.merge(info, %{counters: counters_4,
                           istate: simpleState, owner: owner, active: active})}
  end
