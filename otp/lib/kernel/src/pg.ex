defmodule :m_pg do
  ...
  def demonitor(scope, ref) do
    :gen_server.call(scope, {:demonitor, ref}, :infinity) === :ok and flush(ref)
  end
  ...
  def handle_cast(
        {:sync, peer, groups},
        r_state(scope: scope, remote: remote, scope_monitors: scopeMon, monitored_groups: mG) =
          state
      ) do
    {:noreply, r_state(state, remote: handle_sync(scope, scopeMon, mG, peer, remote, groups))}
  end
  ...
  def handle_info(
    ...
    case :maps.get(peer, remote, []) do
      {mRef, remoteGroups} ->
        ...
        {:noreply, r_state(state, remote: Map.put(remote, peer, {mRef, newRemoteGroups}))}
  ...
  def handle_info(
    ...
    case :maps.get(peer, remote, []) do
      {mRef, remoteMap} ->
        ...
        {:noreply, r_state(state, remote: Map.put(remote, peer, {mRef, newRemoteMap}))}
  ...
  def handle_info(
    ...
    case :maps.take(pid, remote) do
      {{^mRef, remoteMap}, newRemote} ->
        :maps.foreach(
          fn group, pids ->
            leave_remote_update_ets(scope, scopeMon, mG, pids, [group])
          end,
          remoteMap
        )
  ...
  def handle_info(
        {{:DOWN, :scope_monitors}, mRef, :process, _Pid, _Info},
        r_state(scope_monitors: scopeMon) = state
      ) do
    {:noreply, r_state(state, scope_monitors: :maps.remove(mRef, scopeMon))}
  end

  def handle_info(
        {{:DOWN, :group_monitors}, mRef, :process, pid, _Info},
        r_state(group_monitors: gMs, monitored_groups: mG) = state
      ) do
    case :maps.take(mRef, gMs) do
      ...
      {{^pid, group}, newGM} ->
        {:noreply,
         r_state(state,
           group_monitors: newGM,
           monitored_groups: demonitor_group({pid, mRef}, group, mG)
         )}
    end
  end
  ...
  defp handle_discover(
         peer,
         r_state(remote: remote, local: local) = state
       ) do
    :gen_server.cast(
      peer,
    ...
    case :maps.is_key(peer, remote) do
      ...
      false ->
        ...
        {:noreply, r_state(state, remote: Map.put(remote, peer, {mRef, %{}}))}
    end
  end
  ...
  defp sync_groups(scope, scopeMon, mG, remoteGroups, [{group, pids} | tail]) do
