defmodule :m_logger_proxy do
  ...
  def start_link() do
    :logger_olp.start_link(:logger_proxy, :logger_proxy, [],
                             :logger.get_proxy_config())
  end
  ...
  def child_spec() do
    name = :logger_proxy
    %{id: name, start: {:logger_proxy, :start_link, []},
  ...
  def get_default_config() do
    ...
    Map.merge(olpDefault, %{sync_mode_qlen: 500,
                              drop_mode_qlen: 1000, flush_qlen: 5000,
  ...
  def init([]) do
    ...
    :persistent_term.put(:logger_proxy,
                           :logger_olp.get_ref())
    {:ok, :no_state}
  end

  def handle_load({:remote, node, log}, state) do
    case (:erlang.send({:logger_proxy, node}, log,
                         [:nosuspend])) do
      _ok_or_nosuspend ->
        :ok
    end
  ...
  def handle_info(log, state) when (is_tuple(log) and
                             :erlang.element(1, log) == :log) do
    {:load, state}
  end

  def handle_info(_Log, state) do
    state
  end
  ...
  def terminate(_Reason, _State) do
    _ = :erlang.system_flag(:system_logger,
  ...
  def notify({:mode_change, mode0, mode1}, state) do
    ...
    case (:logger.allow(:notice, :logger_proxy)) do
      true ->
        _ = :logger_server.do_internal_log(:notice,
                                             %{mfa: {:logger_proxy, :notify, 2},
                                                 line: 158, file: 'otp/lib/kernel/src/logger_proxy.erl'},
                                             %{},
  ...
  def notify({:flushed, flushed}, state) do
    case (:logger.allow(:notice, :logger_proxy)) do
      true ->
        _ = :logger_server.do_internal_log(:notice,
                                             %{mfa: {:logger_proxy, :notify, 2},
                                                 line: 161, file: 'otp/lib/kernel/src/logger_proxy.erl'},
  ...
  def notify(:restart, state) do
    case (:logger.allow(:notice, :logger_proxy)) do
      true ->
        _ = :logger_server.do_internal_log(:notice,
                                             %{mfa: {:logger_proxy, :notify, 2},
                                                 line: 164, file: 'otp/lib/kernel/src/logger_proxy.erl'},
            ...
            _ = :logger_server.do_internal_log(:debug,
                                                 %{mfa:
                                                   {:logger_proxy, :try_log, 1},
                                                     line: 174, file: 'otp/lib/kernel/src/logger_proxy.erl'},
                                                 %{},
                                                 [[{:logger_proxy, :log_failed},
                                                       {:log, args}, {:reason,
                                                                        {c, r,
                                                                           __STACKTRACE__}}]])
            :ok
