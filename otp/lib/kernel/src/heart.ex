defmodule :m_heart do
  ...
  Record.defrecord(:r_state, :state, port: :undefined,
                                 cmd: :undefined, options: :undefined,
                                 callback: :undefined)
  def start() do
    case (:erlang.whereis(:heart)) do
      :undefined ->
        pid = spawn(:heart, :init,
                      [self(), :erlang.whereis(:init)])
    ...
    receive do
      {from, :set_cmd, newCmd0} ->
        ...
        case ((try do
          ...
          newCmd when (is_binary(newCmd) and
      ...
      {from, :set_callback, callback} ->
        case (callback) do
          ...
          _ ->
            send(from, {:heart,
                          {:error, {:bad_callback, callback}}})
            loop(parent, s)
        end
      {from, :get_callback} ->
        res = (case (r_state(s, :callback)) do
                 :undefined ->
                   :none
                 cb ->
                   {:ok, cb}
