defmodule :m_inet_sctp do
  ...
  def open(opts) do
    case (:inet.sctp_options(opts, :inet_sctp)) do
      {:ok,
         r_sctp_opts(fd: fd, ifaddr: addr, port: port, type: type,
             opts: sOs)} ->
        :inet.open_bind(fd, addr, port, sOs, :sctp, :inet, type,
                          :inet_sctp)
      error ->
        error
    end
      ...
      :ok ->
        case (:prim_inet.getopt(s, :active)) do
          {:ok, active} ->
            ...
            case (:prim_inet.connect(s, sockAddr, timeout)) do
  ...
  def connectx(s, addrs, port, opts) do
    case (:prim_inet.chgopts(s, opts)) do
      :ok ->
  ...
  defp connect_get_assoc(s, %{addr: addr, port: port} = _SockAddr,
            active, timer) do
    connect_get_assoc(s, addr, port, active, timer)
  end
  ...
  def send(s, assocId, stream, data) do
    case (:prim_inet.getopts(s,
                               [{:sctp_default_send_param,
                                   r_sctp_sndrcvinfo(assoc_id: assocId)}])) do
      {:ok,
         [{:sctp_default_send_param,
             r_sctp_sndrcvinfo(flags: flags, context: context, ppid: pPID,
                 timetolive: tTL)}]} ->
        :prim_inet.sendmsg(s,
                             r_sctp_sndrcvinfo(flags: flags, context: context, ppid: pPID,
                                 timetolive: tTL, assoc_id: assocId,
                                 stream: stream),
                             data)
      _ ->
        :prim_inet.sendmsg(s,
                             r_sctp_sndrcvinfo(assoc_id: assocId, stream: stream), data)
    end
  end
