defmodule :m_inet_res do
  ...
  def nnslookup(name, class, type, nSs, timeout) do
    do_nslookup(name, class, type, [{:nameservers, nSs}],
                  timeout)
  end
  ...
  Record.defrecord(:r_options, :options, alt_nameservers: :undefined,
                                   dnssec_ok: :undefined, edns: :undefined,
                                   inet6: :undefined, nameservers: :undefined,
                                   nxdomain_reply: :undefined,
                                   recurse: :undefined, retry: :undefined,
                                   servfail_retry_timeout: :undefined,
                                   timeout: :undefined,
                                   udp_payload_size: :undefined,
                                   usevc: :undefined, verbose: :undefined)
  defp make_options(opts0) do
    opts = (for opt <- opts0 do
              cond do
                is_atom(opt) ->
    ...
    sortedOpts = :lists.ukeysort(1,
                                   case (:lists.keymember(:nameservers, 1,
                                                            opts)) do
                                     true ->
                                       case (:lists.keymember(:alt_nameservers,
                                                                1, opts)) do
                                         false ->
  ...
  defp make_options([{opt, val} | opts] = opts0,
            [opt | names] = names0)
  ...
  defp type_p(type) do
    :lists.member(type,
                    [:a, :aaaa, :mx, :ns, :md, :mf, :cname, :soa, :mb, :mg,
                                                                           :mr,
                                                                               :null,
                                                                                   :wks,
                                                                                       :hinfo,
    ...
    cond do
      ...
      true ->
        case (res_getby_query(name, type, timer)) do
          {:error, _Reason} = error ->
            res_getby_search(name, :inet_db.get_searchlist(), error,
                               type, timer)
          other ->
            other
        end
  ...
  defp res_getby_search(name, [dom | ds], _Reason, type, timer) do
    ...
    case (res_getby_query(queryName, type, timer,
                            :inet_db.res_option(:nameservers))) do
      {:ok, hEnt} ->
        {:ok, hEnt}
      {:error, newReason} ->
  ...
  Record.defrecord(:r_q, :q, options: :undefined,
                             edns: :undefined, dns: :undefined)
  defp res_query(name, class, type, opts, timer) do
    r_q(options: r_options(nameservers: nSs)) = (q = make_query(name,
                                                        class, type, opts))
  ...
  defp res_query_alt(r_q(options: r_options(alt_nameservers: nSs)) = q, reply,
            timer) do
    case (nSs) do
      [] ->
        reply
  ...
  defp make_query(dname, class, type, opts) do
    ...
    case (r_options(options, :edns)) do
      false ->
        r_q(options: options, edns: :undefined,
  ...
  defp make_query(dname, class, type, options, edns) do
    ...
    msg = r_dns_rec(header: r_dns_header(id: id, qr: false, opcode: :query,
                        rd: rD, rcode: 0),
              qdlist: [r_dns_query(domain: dname, type: type, class: class)],
              arlist: aRList)
    case ((
            r_options(options, :verbose)
          )) do
      true ->
        :io.format((
                     'Query: ~p~n'
  ...
  Record.defrecord(:r_sock, :sock, inet: :undefined,
                                inet6: :undefined)
  ...
  defp udp_connect(r_sock(inet6: i), {a, b, c, d, e, f, g, h} = iP,
            port)
      when ((a ||| b ||| c ||| d ||| e ||| f ||| g ||| h) &&& ~~~
                                                              65535 === 0 and
              port &&& ~~~ 65535 === 0) do
  ...
  defp udp_send(r_sock(inet6: i), {a, b, c, d, e, f, g, h} = iP,
            port, buffer)
      when ((a ||| b ||| c ||| d ||| e ||| f ||| g ||| h) &&& ~~~
                                                              65535 === 0 and
              port &&& ~~~ 65535 === 0) do
  ...
  defp udp_recv(r_sock(inet6: i), {a, b, c, d, e, f, g, h} = iP,
            port, timeout, decode)
      when ((a ||| b ||| c ||| d ||| e ||| f ||| g ||| h) &&& ~~~
                                                              65535 === 0 and
              port &&& ~~~ 65535 === 0 and 0 <= timeout) do
    do_udp_recv(i, iP, port, timeout, decode, time(timeout),
                  timeout)
  end

  defp udp_recv(r_sock(inet: i), {a, b, c, d} = iP, port, timeout,
            decode)
      when ((a ||| b ||| c ||| d) &&& ~~~ 255 === 0 and
              port &&& ~~~ 65535 === 0 and 0 <= timeout) do
    do_udp_recv(i, iP, port, timeout, decode, time(timeout),
      ...
      {:ok, reply} ->
        case (decode.(reply)) do
          false when timeout === 0 ->
            do_udp_recv(i, iP, port, timeout, decode, time,
                          pollCnt - 50)
          false ->
            do_udp_recv(i, iP, port, timeout(time), decode, time,
                          pollCnt)
          result ->
            result
        end
  ...
  defp do_query(r_q(options: r_options(retry: retry)) = q, nSs, timer) do
    reason = :timeout
    :lists.all(fn nS when tuple_size(nS) === 2 ->
                    true
                  _ ->
                    false
               end,
                 nSs) or :erlang.error(:badarg, [q, nSs, timer])
  ...
  defp query_nss(q, [], timer, retry, i, s, reason, retryNSs) do
    query_retries(q, :lists.reverse(retryNSs), timer, retry,
                    i + 1, s, reason)
  end

  defp query_nss(r_q(edns: :undefined) = q, nSs, timer, retry, i,
            s, reason, retryNSs) do
    query_nss_dns(q, nSs, timer, retry, i, s, reason,
                    retryNSs)
  end

  defp query_nss(q, nSs, timer, retry, i, s, reason, retryNSs) do
    query_nss_edns(q, nSs, timer, retry, i, s, reason,
                     retryNSs)
  end

  defp query_nss_edns(r_q(options: r_options(udp_payload_size: pSz) = options,
              edns: eDNSQuery) = q,
            [nsSpec | nSs], timer, retry, i, s_0, reason,
            retryNSs) do
    {iP, port} = (nS = servfail_retry_wait(nsSpec))
    {s, result} = query_ns(s_0, eDNSQuery, iP, port, timer,
                             retry, i, options, pSz)
    case (result) do
      {:error, {e, _}} when e === :qfmterror or
                              e === :notimp or e === :servfail or e === :badvers
                            ->
        query_nss_dns(q, [nS | nSs], timer, retry, i, s, reason,
                        retryNSs)
      _ ->
        query_nss_result(q, nSs, timer, retry, i, s, reason,
                           retryNSs, nS, result)
    end
  end

  defp query_nss_dns(r_q(dns: dNSQuery_0) = q_0, [nsSpec | nSs], timer,
            retry, i, s_0, reason, retryNSs) do
    {iP, port} = (nS = servfail_retry_wait(nsSpec))
    r_q(options: options, dns: dNSQuery) = (q = (cond do
                                                 is_function(dNSQuery_0, 0) ->
                                                   r_q(q_0, dns: dNSQuery_0.())
                                                 true ->
                                                   q_0
                                               end))
    {s, result} = query_ns(s_0, dNSQuery, iP, port, timer,
                             retry, i, options, 512)
    query_nss_result(q, nSs, timer, retry, i, s, reason,
                       retryNSs, nS, result)
  end
  ...
  defp query_nss_result(q, nSs, timer, retry, i, s, reason, retryNSs,
            nS, result) do
    case (result) do
      {:ok, _} ->
        _ = udp_close(s)
      ...
      {:error, {e, _} = newReason} when e === :qfmterror or
                                          e === :notimp or e === :refused or
                                          e === :badvers or e === :unknown
                                        ->
        query_nss(q, nSs, timer, retry, i, s, newReason,
                    retryNSs)
      {:error, e = newReason} when e === :formerr or
  ...
  defp query_retries_error(r_q(options: r_options(nxdomain_reply: nxReply)), s,
            reason) do
    _ = udp_close(s)
    case (reason) do
  ...
  defp query_ns(s0, {msg, buffer}, iP, port, timer, retry, i,
            r_options(timeout: tm, usevc: useVC, verbose: verbose), pSz) do
    case (useVC or :erlang.iolist_size(buffer) > pSz) do
      ...
      false ->
        case (udp_open(s0, iP)) do
          {:ok, s} ->
            udpTimeout = :inet.timeout(div(tm * (1 <<< i), retry),
                                         timer)
            case (query_udp(s, msg, buffer, iP, port, udpTimeout,
                              verbose)) do
  ...
  defp query_udp(s, msg, buffer, iP, port, timeout, verbose) do
    ...
    case (case (udp_connect(s, iP, port)) do
      ...
      :ok ->
        decode = fn {recIP, recPort, answer}
  ...
  defp decode_answer(answer, q_Msg, verbose) do
    case (:inet_dns.decode(answer)) do
      {:ok, r_dns_rec(header: h, arlist: aRList) = msg} ->
        ...
        e = (case (:lists.keyfind(:dns_rr_opt, 1, aRList)) do
               false ->
                 0
      ...
      {:error, :formerr} = error ->
        case ((
          ...
          true ->
            :io.format((
                         'Got reply: decode format error~n'
                       ),
                         (
                           []
                         ))
  ...
  defp decode_answer_noerror(r_dns_rec(header: q_H, qdlist: [q_RR]),
            r_dns_rec(qdlist: qDList) = msg, h) do
    cond do
      ...
      true ->
        case (qDList) do
          [rR] ->
            case (r_dns_query(rR, :class) === r_dns_query(q_RR, :class) and r_dns_query(rR, :type) === r_dns_query(q_RR, :type) and :inet_db.eq_domains(r_dns_query(rR, :domain),
                                                                                                                  r_dns_query(q_RR, :domain))) do
              true ->
                {:ok, msg}
              false ->
                {:error, {:noquery, msg}}
  ...
  defp dn_ip({a, b, c, d}) when (a ||| b ||| c ||| d) &&& ~~~
                                                       255 === 0 do
  ...
  defp dn_ipv4([a | as], dn_0) when (is_integer(a) and
                                  a <= 255) do
    dn = [?. | dn_0]
    cond do
      a < 10 ->
  ...
  defp dn_ipv6([w | ws], dn) when (is_integer(w) and
                                w <= 65535) do
    d = w &&& 15
    w_1 = w >>> 4
    c = w_1 &&& 15
