defmodule :m_logger_olp do
  ...
  def start_link(name, module, args, options0)
    ...
    case (check_opts(options)) do
      :ok ->
        :proc_lib.start_link(:logger_olp, :init,
  ...
  def get_default_opts() do
    %{sync_mode_qlen: 10, drop_mode_qlen: 200,
        flush_qlen: 1000, burst_limit_enable: true,
        burst_limit_max_count: 500,
        burst_limit_window_time: 1000,
        overload_kill_enable: false, overload_kill_qlen: 20000,
      ...
      true ->
        _ = :logger_server.do_internal_log(:debug,
                                             %{mfa: {:logger_olp, :restart, 1},
                                                 line: 144, file: 'otp/lib/kernel/src/logger_olp.erl'},
                                             %{},
                                             [[{:logger_olp, :restart},
                                                   {:result, result}]])
        :ok
  ...
  def init([name, module, args, options]) do
    ...
    try do
      ...
      {:ok, cBState} ->
        ...
        state0 = Map.merge(options, %{id: name, idle: true,
                                        module: module, mode_ref: modeRef,
                                        mode: :async, last_qlen: 0,
                                        last_load_ts: t0, burst_win_ts: t0,
                                        burst_msg_count: 0, cb_state: cBState})
  ...
  def handle_call({:"$olp_load", msg}, _From, state) do
    {result, state1} = do_load(msg, :call,
                                 Map.put(state, :idle, false))
    reply_return(result, state1)
  end

  def handle_call(:get_ref, _From,
           %{id: name, mode_ref: modeRef} = state) do
    reply_return({name, self(), modeRef}, state)
  end

  def handle_call({:set_opts, opts0}, _From, state) do
    opts = :maps.merge(:maps.with([:sync_mode_qlen,
  ...
  def handle_call(:get_opts, _From, state) do
    reply_return(:maps.with([:sync_mode_qlen,
                                 :drop_mode_qlen, :flush_qlen,
                                                      :burst_limit_enable,
                                                          :burst_limit_max_count,
                                                              :burst_limit_window_time,
  ...
  def handle_call(:reset, _From,
           %{module: module, cb_state: cBState} = state) do
    state1 = state
    cBState1 = try_callback_call(module, :reset_state,
                                   [cBState], cBState)
    reply_return(:ok,
                   Map.merge(state1, %{idle: true, last_qlen: 0,
                                         last_load_ts:
  ...
  def handle_call(msg, from,
           %{module: module, cb_state: cBState} = state) do
    case (try_callback_call(module, :handle_call,
                              [msg, from, cBState])) do
  ...
  def handle_cast({:"$olp_load", msg}, state) do
    {_Result, state1} = do_load(msg, :cast,
                                  Map.put(state, :idle, false))
  ...
  def handle_cast(msg,
           %{module: module, cb_state: cBState} = state) do
    case (try_callback_call(module, :handle_cast,
                              [msg, cBState])) do
      {:noreply, cBState1} ->
        noreply_return(Map.put(state, :cb_state, cBState1))
      {:stop, reason, cBState1} ->
  ...
  def handle_info(msg,
           %{module: module, cb_state: cBState} = state) do
    case (try_callback_call(module, :handle_info,
                              [msg, cBState])) do
      {:noreply, cBState1} ->
        noreply_return(Map.put(state, :cb_state, cBState1))
      {:stop, reason, cBState1} ->
        {:stop, reason, Map.put(state, :cb_state, cBState1)}
      {:load, cBState1} ->
        {_, state1} = do_load(msg, :cast,
                                Map.merge(state, %{idle: false,
                                                     cb_state: cBState1}))
        noreply_return(state1)
    end
  end

  def terminate({:shutdown, {:overloaded, _QLen, _Mem}},
           %{id: name, module: module, cb_state: cBState,
               overload_kill_restart_after: restartAfter} = state) do
    :erlang.unregister(name)
    case (try_callback_call(module, :terminate,
                              [:overloaded, cBState], :ok)) do
      {:ok, fun} when (is_function(fun, 0) and
                         is_integer(restartAfter))
                      ->
        set_restart_flag(state)
        _ = :timer.apply_after(restartAfter, :logger_olp,
                                 :restart, [fun])
        :ok
      _ ->
        :ok
  ...
  def terminate(reason,
           %{id: name, module: module, cb_state: cBState}) do
    _ = try_callback_call(module, :terminate,
                            [reason, cBState], :ok)
    :erlang.unregister(name)
    :ok
  end
  ...
  defp flush(t1,
    ...
    {:dropped,
       Map.merge(state4, %{mode: set_mode(modeRef, :async),
                             last_qlen: qLen1, last_load_ts: t1})}
  end

  defp handle_load(mode, t1, msg, _CallOrCast,
            state = %{id: _Name, module: module, cb_state: cBState,
                        last_qlen: lastQLen, last_load_ts: _T0}) do
    {doWrite, state1} = limit_burst(state)
    {result, lastQLen1, cBState1} = (cond do
                                       doWrite ->
                                         :ok
                                         cBS = try_callback_call(module,
                                                                   :handle_load,
                                                                   [msg,
                                                                        cBState])
                                         {:ok,
    ...
    state4 = state3
    state5 = state4
    state6 = Map.put(%{state5
                       |
                       last_qlen: lastQLen1}, :last_load_ts, t1)
    state7 = (case (result) do
                :ok ->
                  s = state6
                  s
                _ ->
      ...
      :ok ->
        case (overload_levels_ok(options)) do
          true ->
            :ok
          false ->
            faulty = :maps.with([:sync_mode_qlen, :drop_mode_qlen,
                                                      :flush_qlen],
                                  options)
  ...
  defp do_check_opts([{:overload_kill_restart_after, norA} |
               options])
  ...
  defp set_restart_flag(%{id: name, module: module}) do
    flag = :erlang.list_to_atom(:lists.concat([module, '_',
                                                           name, '_restarting']))
    spawn(fn () ->
  ...
  defp check_load(state = %{id: _Name, mode_ref: modeRef,
                      mode: mode, sync_mode_qlen: syncModeQLen,
                      drop_mode_qlen: dropModeQLen, flush_qlen: flushQLen}) do
    mem = maybe_self_memory(state)
    :ok
    {_, qLen} = :erlang.process_info(self(),
                                       :message_queue_len)
    :ok
    {mode1, _NewDrops, _NewFlushes} = (cond do
                                         qLen >= flushQLen ->
                                           {:flush, 0, 1}
                                         qLen >= dropModeQLen ->
                                           incDrops = (cond do
                                           ...
                                           {set_mode(modeRef, :drop), incDrops,
                                              0}
                                         qLen >= syncModeQLen ->
  ...
  defp maybe_self_memory(%{overload_kill_enable: killIfOL}) do
    killIfOL and do_self_memory()
  end

  defp do_self_memory() do
  ...
  defp limit_burst(%{burst_win_ts: burstWinT0,
              burst_msg_count: burstMsgCount,
              burst_limit_window_time: burstLimitWinTime,
              burst_limit_max_count: burstLimitMaxCnt} = state) do
    cond do
      burstMsgCount >= burstLimitMaxCnt ->
        ...
        case (burstWinT1 - burstWinT0) do
          ...
          _BurstCheckTime ->
            {true,
               Map.merge(state, %{burst_win_ts: burstWinT1,
                                    burst_msg_count: 0})}
        end
      true ->
        {true,
           Map.merge(state, %{burst_win_ts: burstWinT0,
  ...
  defp kill_if_choked(qLen, mem,
            %{overload_kill_enable: killIfOL,
                ...
                overload_kill_mem_size: oLKillMem}) do
    cond do
  ...
  defp notify(note,
            %{module: module, cb_state: cBState} = state) do
    cBState1 = try_callback_call(module, :notify,
                                   [note, cBState], cBState)
    Map.put(state, :cb_state, cBState1)
  end
