defmodule :m_kernel do
  ...
  def start(_, []) do
    ...
    case (:supervisor.start_link({:local, :kernel_sup},
                                   :kernel, [])) do
      {:ok, pid} ->
  ...
  def init([]) do
    ...
    refC = %{id: :kernel_refc,
               ...
               modules: [:kernel_refc]}
    code = %{id: :code_server,
               start: {:code, :start_link, []}, restart: :permanent,
               shutdown: 2000, type: :worker, modules: [:code]}
    ...
    stdError = %{id: :standard_error,
                   ...
                   restart: :temporary, shutdown: 2000, type: :supervisor,
                   modules: [:standard_error]}
    onLoad = %{id: :on_load,
                 start:
                 {:proc_lib, :start_link, [:kernel, :init, [:on_load]]},
                 restart: :transient, shutdown: 2000, type: :worker,
                 modules: [:kernel]}
    user = %{id: :user, start: {:user_sup, :start, []},
               restart: :temporary, shutdown: 2000, type: :supervisor,
               modules: [:user_sup]}
    safeSup = %{id: :kernel_safe_sup,
                  ...
                  type: :supervisor, modules: [:kernel]}
    loggerSup = %{id: :logger_sup,
                    start: {:logger_sup, :start_link, []},
                    restart: :permanent, shutdown: :infinity,
                    type: :supervisor, modules: [:logger_sup]}
    peer = (case (:peer.supervision_child_spec()) do
              {:ok, peerSpec} ->
                [peerSpec]
              :none ->
                []
    ...
    case (:init.get_argument(:mode)) do
      {:ok, [['minimal'] | _]} ->
        {:ok,
           {supFlags,
              [code, stdError | earlyFile] ++ [onLoad |
                                                   lateFile] ++ peer ++ [user,
                                                                             loggerSup,
                                                                                 config,
                                                                                     refC,
                                                                                         safeSup]}}
      _ ->
        distChildren = (case (:application.get_env(:kernel,
                                                     :start_distribution)) do
                          {:ok, false} ->
                            []
                          _ ->
                            start_distribution()
        ...
        inetDb = %{id: :inet_db,
                     start: {:inet_db, :start_link, []}, restart: :permanent,
                     shutdown: 2000, type: :worker, modules: [:inet_db]}
        sigSrv = %{id: :erl_signal_server,
                     start:
                     {:gen_event, :start_link,
                        [{:local, :erl_signal_server}]},
                     restart: :permanent, shutdown: 2000, type: :worker,
                     modules: :dynamic}
        timer = start_timer()
        ...
        {:ok,
           {supFlags,
              [code, stdError | earlyFile] ++ [onLoad, inetDb |
                                                           distChildren] ++ lateFile ++ [sigSrv |
                                                                                             peer] ++ [user,
                                                                                                           loggerSup,
                                                                                                               config,
                                                                                                                   refC,
                                                                                                                       safeSup] ++ timer ++ compileServer}}
    end
  end
  ...
  def init(:safe) do
    supFlags = %{strategy: :one_for_one, intensity: 4,
                   period: 3600}
  ...
  defp start_distribution() do
    rpc = %{id: :rex, start: {:rpc, :start_link, []},
              restart: :permanent, shutdown: 2000, type: :worker,
              modules: [:rpc]}
    global = %{id: :global_name_server,
                 start: {:global, :start_link, []}, restart: :permanent,
                 shutdown: 2000, type: :worker, modules: [:global]}
    distAC = start_dist_ac()
    netSup = %{id: :net_sup,
                 start: {:erl_distribution, :start_link, []},
                 restart: :permanent, shutdown: :infinity,
                 type: :supervisor, modules: [:erl_distribution]}
  ...
  defp start_dist_ac() do
    spec = [%{id: :dist_ac,
                start: {:dist_ac, :start_link, []}, restart: :permanent,
                shutdown: 2000, type: :worker, modules: [:dist_ac]}]
      ...
      {:ok, true} ->
        [%{id: :disk_log_server,
             ...
             %{id: :disk_log_sup,
                 ...
                 restart: :permanent, shutdown: 1000, type: :supervisor,
  ...
  defp start_timer() do
    case (:application.get_env(:kernel, :start_timer)) do
      {:ok, true} ->
        [%{id: :timer_server, start: {:timer, :start_link, []},
             restart: :permanent, shutdown: 1000, type: :worker,
             modules: [:timer]}]
      _ ->
        []
  ...
  defp do_distribution_change(changed, new, removed) do
    case (is_dist_changed(changed, new, removed)) do
      {false, false, false} ->
        :ok
      {c, false, false} ->
        :gen_server.call(:dist_ac, {:distribution_changed, c},
                           :infinity)
      {false, _, false} ->
        :error_logger.error_report('Distribution not changed: Not allowed to add the \'distributed\' parameter.')
        {:error, {:distribution_not_changed, 'Not allowed to add the \'distributed\' parameter'}}
      {false, false, _} ->
        :error_logger.error_report('Distribution not changed: Not allowed to remove the distribution parameter.')
        {:error, {:distribution_not_changed, 'Not allowed to remove the \'distributed\' parameter'}}
    end
  end

  defp is_dist_changed(changed, new, removed) do
    c = (case (:lists.keyfind(:distributed, 1, changed)) do
           false ->
             false
           {:distributed, newDistC} ->
             newDistC
         end)
    n = (case (:lists.keyfind(:distributed, 1, new)) do
           false ->
             false
  ...
  defp is_gg_changed(changed, new, removed) do
    c = (case (:lists.keyfind(:global_groups, 1,
                                changed)) do
           false ->
             false
    ...
    n = (case (:lists.keyfind(:global_groups, 1, new)) do
           false ->
             false
