defmodule :m_kernel do
  ...
  def init([]) do
    ...
    onLoad = %{
      id: :on_load,
      start: {:proc_lib, :start_link, [:kernel, :init, [:on_load]]},
      restart: :transient,
      shutdown: 2000,
      type: :worker,
      modules: [:kernel]
    ...
    peer =
      case :peer.supervision_child_spec() do
        {:ok, peerSpec} ->
          [peerSpec]
         ...
         {supFlags,
          [code, stdError | earlyFile] ++
            [
              onLoad
              | lateFile
            ] ++ peer ++ [user, loggerSup, config, refC, safeSup]}}
         ...
         {supFlags,
          [code, stdError | earlyFile] ++
            ...
            [
              sigSrv
              | peer
            ] ++ [user, loggerSup, config, refC, safeSup] ++ timer ++ compileServer}}
    end
  end
    ...
    case is_dist_changed(changed, new, removed) do
      ...
      {false, _, false} ->
        :error_logger.error_report(
          ~c"Distribution not changed: Not allowed to add the 'distributed' parameter."
        )

        {:error, {:distribution_not_changed, ~c"Not allowed to add the 'distributed' parameter"}}

      {false, false, _} ->
        :error_logger.error_report(
          ~c"Distribution not changed: Not allowed to remove the distribution parameter."
        )

        {:error,
         {:distribution_not_changed, ~c"Not allowed to remove the 'distributed' parameter"}}
    end
  end
