defmodule :m_application_controller do
  ...
  import :lists, only: [foldl: 3, foreach: 2,
                          keydelete: 3, keyfind: 3, keyreplace: 4, map: 2,
                          zf: 2]
  require Record
  Record.defrecord(:r_appl_data, :appl_data, name: :undefined,
                                     ...
                                     maxT: :infinity)
  Record.defrecord(:r_state, :state, loading: [], starting: [],
                                 start_p_false: [], running: [], control: [],
                                 started: [], start_req: [],
                                 conf_data: :undefined)
  Record.defrecord(:r_appl, :appl, name: :undefined,
                                appl_data: :undefined, descr: :undefined,
                                id: :undefined, vsn: :undefined,
                                restart_type: :undefined, inc_apps: :undefined,
                                opt_apps: :undefined, apps: :undefined)
  ...
  def load_application(application) do
    :gen_server.call(:application_controller,
                       {:load_application, application}, :infinity)
  end

  def unload_application(appName) do
    :gen_server.call(:application_controller,
                       {:unload_application, appName}, :infinity)
  end

  def start_application(appName, restartType) do
    :gen_server.call(:application_controller,
                       {:start_application, appName, restartType}, :infinity)
  end

  def start_application_request(appName, restartType) do
    :gen_server.send_request(:application_controller,
                               {:start_application, appName, restartType})
  end

  def is_running(appName) when is_atom(appName) do
    :gen_server.call(:application_controller,
                       {:is_running, appName}, :infinity)
  end
  ...
  def start_boot_application(application, restartType) do
    case ({:application.load(application), restartType}) do
      ...
      {{:error, {:bad_environment_value, env}}, :permanent} ->
        ...
        exit({:error,
                :erlang.list_to_atom(:lists.flatten(txt))})
      {error, _} ->
        error
    end
  end
  ...
  def stop_application(appName) do
    :gen_server.call(:application_controller,
                       {:stop_application, appName}, :infinity)
  end
  ...
  def control_application(appName) do
    :gen_server.call(:application_controller,
                       {:control_application, appName}, :infinity)
  end
  ...
  def prep_config_change() do
    :gen_server.call(:application_controller,
                       :prep_config_change, :infinity)
  end

  def config_change(envPrev) do
    :gen_server.call(:application_controller,
                       {:config_change, envPrev}, :infinity)
  end

  def get_pid_env(master, key) do
    case (:ets.match(:ac_tab,
                       {{:application_master, :"$1"}, master})) do
      [[appName]] ->
        get_env(appName, key)
      _ ->
        :undefined
  ...
  def get_env(appName, key) do
    notFound = make_ref()
    case (:ets.lookup_element(:ac_tab, {:env, appName, key},
                                2, notFound)) do
      ^notFound ->
        :undefined
      val ->
  ...
  def get_env(appName, key, default) do
    :ets.lookup_element(:ac_tab, {:env, appName, key}, 2,
                          default)
  end
  ...
  def get_all_key(appName) do
    case (:ets.lookup(:ac_tab, {:loaded, appName})) do
      [{_, appl}] ->
        {:ok,
           [{:description, r_appl(appl, :descr)}, {:id, r_appl(appl, :id)},
                                                 {:vsn, r_appl(appl, :vsn)},
                                                     {:modules,
                                                        r_appl_data(r_appl(appl, :appl_data), :mods)},
                                                         {:maxP,
                                                            r_appl_data(r_appl(appl, :appl_data), :maxP)},
                                                             {:maxT,
                                                                r_appl_data(r_appl(appl, :appl_data), :maxT)},
                                                                 {:registered,
                                                                    r_appl_data(r_appl(appl, :appl_data), :regs)},
                                                                     {:included_applications,
                                                                        r_appl(appl, :inc_apps)},
                                                                         {:optional_applications,
                                                                            r_appl(appl, :opt_apps)},
                                                                             {:applications,
                                                                                r_appl(appl, :apps)},
                                                                                 {:env,
                                                                                    get_all_env(appName)},
                                                                                     {:mod,
                                                                                        r_appl_data(r_appl(appl, :appl_data), :mod)},
                                                                                         {:start_phases,
                                                                                            r_appl_data(r_appl(appl, :appl_data), :phases)}]}
      _ ->
        :undefined
    end
  end
  ...
  def get_master(appName) do
    :ets.lookup_element(:ac_tab,
                          {:application_master, appName}, 2, :undefined)
  end
  ...
  def permit_application(applName, flag) do
    :gen_server.call(:application_controller,
                       {:permit_application, applName, flag}, :infinity)
  end
  ...
  def set_env(appName, key, val, opts) do
    timeout = :proplists.get_value(:timeout, opts, 5000)
    :gen_server.call(:application_controller,
                       {:set_env, appName, key, val, opts}, timeout)
  end
  ...
  def unset_env(appName, key, opts) do
    timeout = :proplists.get_value(:timeout, opts, 5000)
    :gen_server.call(:application_controller,
                       {:unset_env, appName, key, opts}, timeout)
  end
    ...
    case ((try do
      ...
      {:ok, confData} ->
        case (check_conf_data(confData)) do
          :ok ->
            _ = :ets.new(:ac_tab,
                           [:set, :public, :named_table, {:read_concurrency,
                                                            true}])
            ...
            case ((try do
              ...
              {:ok, newS} ->
                ...
                :gen_server.enter_loop(:application_controller, [],
                                         newS,
      ...
      {:error, {file, line, str}} ->
        reasonStr = :lists.flatten(:io_lib.format('error in config file ~tp (~w): ~ts',
                                                    [file, line, str]))
        send(init, {:ack, self(),
                      {:error, to_string(reasonStr)}})
      {:error, {:file_descriptor, fDString, line, str}} ->
        reasonStr = :lists.flatten(:io_lib.format('error in config read from file descriptor ~tp (~w): ~ts',
                                                    [fDString, line, str]))
        send(init, {:ack, self(),
                      {:error, to_string(reasonStr)}})
    end
  ...
  defp check_conf_data(confData) when is_list(confData) do
    ...
    case (application) do
      {appName, list} when (is_atom(appName) and
                              is_list(list))
                           ->
      ...
      {appName, list} when is_list(list) ->
        errMsg = 'application: ' ++ :lists.flatten(:io_lib.format('~tp',
                                                      [appName])) ++ '; application name must be an atom'
        {:error, errMsg}
      {appName, _List} ->
        errMsg = 'application: ' ++ :lists.flatten(:io_lib.format('~tp',
                                                      [appName])) ++ '; parameters must be a list'
        {:error, errMsg}
      else__ ->
        errMsg = 'invalid application config: ' ++ :lists.flatten(:io_lib.format('~tp',
                                                      [else__]))
        {:error, errMsg}
    end
  end
  ...
  defp check_para([{para, val} | paraList], appName)
    ...
    case (:lists.keymember(para, 1, paraList)) do
      true ->
        errMsg = 'application: ' ++ :erlang.atom_to_list(appName) ++ '; duplicate parameter: ' ++ :erlang.atom_to_list(para)
        {:error, errMsg}
      false ->
  ...
  defp check_para([{para, _Val} | _ParaList], appName) do
    {:error,
       'application: ' ++ :erlang.atom_to_list(appName) ++ '; invalid parameter name: ' ++ :lists.flatten(:io_lib.format('~tp',
                                                                                  [para]))}
  end

  defp check_para([else__ | _ParaList], appName) do
    {:error,
       'application: ' ++ :erlang.atom_to_list(appName) ++ '; invalid parameter: ' ++ :lists.flatten(:io_lib.format('~tp',
                                                                                  [else__]))}
  end
  ...
  defp check_distributed([{app, list} | apps]) when (is_atom(app) and
                                        is_list(list)) do
    check_distributed(apps)
  end
  ...
  def handle_call({:load_application, application}, from, s) do
    case ((try do
      ...
      {:ok, newS} ->
        appName = get_appl_name(application)
        case (cntrl(appName, s,
                      {:ac_load_application_req, appName})) do
          true ->
            {:noreply,
               r_state(s, loading: [{appName, from} | r_state(s, :loading)])}
          false ->
            {:reply, :ok, newS}
        end
      {:error, _} = error ->
  ...
  def handle_call({:start_application, appName, restartType}, from,
           s) do
    r_state(running: running, starting: starting,
        start_p_false: sPF, started: started,
                  ...
                  false ->
                    ...
                    {:noreply,
                       r_state(s, starting: [{appName, restartType, :normal, from} |
                              ...
                              start_req: [{appName, from} | start_req])}
                  true ->
                    sS = r_state(s, start_p_false: [{appName, restartType,
                                                 :normal, from} |
                                                  sPF])
                    {:reply, :ok, sS}
                end
  ...
  def handle_call({:is_running, appName}, _From, s) do
    r_state(running: running) = s
  ...
  def handle_call({:permit_application, appName, bool}, from, s) do
    ...
    case (:lists.keymember(appName, 1, control)) do
      ...
      false ->
        case ({bool, isLoaded, isStarting, isSPF, isStarted,
                 isRunning}) do
          ...
          {true, {true, _Appl}, false, false, false, false} ->
            update_permissions(appName, bool)
            {:reply, :ok, s}
          {true, {true, _Appl}, {:value, _Tuple}, false, false,
             false} ->
            update_permissions(appName, bool)
            {:reply, :ok, s}
          {true, {true, appl}, false, {:value, tuple}, false,
             false} ->
            update_permissions(appName, bool)
            {_AppName2, restartType, :normal, _From} = tuple
            spawn_starter(from, appl, s, :normal)
            sS = r_state(s, starting: [{appName, restartType, :normal,
                                    from} |
                                     starting], 
            ...
            {:noreply, sS}
          {true, {true, appl}, _, _,
             {:value, {^appName, restartType}}, false} ->
            update_permissions(appName, bool)
            spawn_starter(from, appl, s, :normal)
            sS = r_state(s, starting: [{appName, restartType, :normal,
                                    from} |
          ...
          {false, {true, _Appl}, false, false, false, false} ->
            update_permissions(appName, bool)
            {:reply, :ok, s}
          {false, {true, _Appl}, {:value, _Tuple}, false, false,
             false} ->
            update_permissions(appName, bool)
            {:reply, :ok, s}
          {false, {true, _Appl}, false, {:value, _Tuple}, false,
             false} ->
            update_permissions(appName, bool)
  ...
  def handle_call({:stop_application, appName}, _From, s) do
    ...
    case (:lists.keyfind(appName, 1, running)) do
      {_AppName, id} ->
        ...
        {:reply, :ok,
           r_state(s, running: nRunning,  started: nStarted)}
      false ->
  ...
  def handle_call({:change_application_data, applications, config},
           _From, s) do
    oldAppls = :ets.foldl(fn {{:loaded, _AppName}, appl},
                               acc ->
                               [appl | acc]
                             _, acc ->
                               acc
  ...
  def handle_call({:set_env, config, opts}, _From, s) do
    _ = (for {appName, env} <- config do
           add_env(appName, env)
         end)
    case (:proplists.get_value(:persistent, opts, false)) do
      true ->
        {:reply, :ok,
           r_state(s, conf_data: merge_env(r_state(s, :conf_data), config))}
      false ->
        {:reply, :ok, s}
    end
  end
  ...
  def handle_call({:set_env, appName, key, val, opts}, _From, s) do
    ...
    case (:proplists.get_value(:persistent, opts, false)) do
      true ->
        ...
        {:reply, :ok,
           r_state(s, conf_data: change_app_env(r_state(s, :conf_data),
                                            appName, fun))}
      false ->
        {:reply, :ok, s}
    end
  ...
  def handle_call({:control_application, appName}, {pid, _Tag},
           s) do
    control = r_state(s, :control)
    case (:lists.keymember(appName, 1, control)) do
      false ->
        :erlang.link(pid)
        {:reply, true,
           r_state(s, control: [{appName, pid} | control])}
      true ->
        {:reply, false, s}
    end
  end
  ...
  def handle_call({:start_type, appName}, _From, s) do
    starting = r_state(s, :starting)
    startType = (case (:lists.keyfind(appName, 1,
                                        starting)) do
                   false ->
                     :local
  ...
  def handle_call(:info, _From, s) do
    reply = [{:loaded, loaded_applications()}, {:loading,
                                                  r_state(s, :loading)},
                                                   {:started, r_state(s, :started)},
                                                       {:start_p_false,
                                                          r_state(s, :start_p_false)},
                                                           {:running,
                                                              r_state(s, :running)},
                                                               {:starting,
                                                                  r_state(s, :starting)}]
  ...
  defp handle_application_started(appName, res, s) do
    r_state(starting: starting, running: running,
        started: started, start_req: start_req) = s
    start_reqN = reply_to_requester(appName, start_req, res)
    {^appName, restartType, _Type,
       _From} = :lists.keyfind(appName, 1, starting)
    case (res) do
      {:ok, id} ->
        case (appName) do
          :kernel ->
        ...
        nRunning = keyreplaceadd(appName, 1, running,
                                   {appName, id})
        nStarted = keyreplaceadd(appName, 1, started,
                                   {appName, restartType})
        newS = r_state(s, starting: keydelete(appName, 1, starting), 
                      running: nRunning,  started: nStarted, 
      ...
      {:error, r} = error when restartType === :temporary ->
        ...
        {:noreply,
           r_state(s, starting: keydelete(appName, 1, starting), 
                  start_req: start_reqN)}
      {:info, r} when restartType === :temporary ->
        notify_cntrl_started(appName, :undefined, s,
                               {:error, r})
        {:noreply,
           r_state(s, starting: keydelete(appName, 1, starting), 
                  start_req: start_reqN)}
      {errInf, r} when (restartType === :transient and
                          errInf === :error) or
  ...
  def handle_info({:ac_start_application_reply, appName, res},
           s) do
    start_req = r_state(s, :start_req)
    case (:lists.keyfind(appName, 1,
                           starting = r_state(s, :starting))) do
      {_AppName, restartType, type, from} ->
        case (res) do
          ...
          {:takeover, _Node} = takeover ->
            ...
            newStarting = [{appName, restartType, takeover, from} |
                               newStarting1]
            {:noreply, r_state(s, starting: newStarting)}
          {:error, reason} = error when restartType === :permanent
                                        ->
            start_reqN = reply_to_requester(appName, start_req,
                                              error)
  ...
  def handle_info({:ac_change_application_req, appName, msg}, s) do
    ...
    started = r_state(s, :started)
    starting = r_state(s, :starting)
    case ({keyfind(appName, 1, running),
             keyfind(appName, 1, started)}) do
      {{^appName, id}, {_AppName2, type}} ->
        case (msg) do
          ...
          {:takeover, _Node, _RT} when is_pid(id) ->
            ...
            {:noreply, s}
          {:takeover, node, rT} ->
            newS = do_start(appName, rT, {:takeover, node},
                              :undefined, s)
            {:noreply, newS}
          ...
          {:failover, node, rT} ->
            case (:application.get_key(appName, :start_phases)) do
              {:ok, :undefined} ->
                ...
                {:noreply, newS}
              {:ok, _StartPhases} ->
                newS = do_start(appName, rT, {:failover, node},
                                  :undefined, s)
                {:noreply, newS}
            end
          :stop_it ->
          ...
          :start_it when is_pid(id) ->
            ...
            {:noreply, s}
          :start_it ->
            newS = do_start(appName, :undefined, :normal,
                              :undefined, s)
            {:noreply, newS}
          :not_running ->
            nRunning = keydelete(appName, 1, running)
          ...
          :start_it ->
            case ({isLoaded, isStarting, isStarted, isRunning}) do
              ...
              {{true, _Appl}, {:value, tuple}, false, false} ->
                ...
                {:noreply, newS}
              {{true, _Appl}, _, {:value, {^appName, _RestartType}},
                 false} ->
                newS = do_start(appName, :undefined, :normal,
                                  :undefined, s)
      ...
      {appName, _AmPid} ->
        ...
        case (:lists.keyfind(appName, 1, r_state(s, :started))) do
          ...
          {_AppName, type} ->
            ...
            {:stop,
               to_string({:application_terminated, appName, reason}),
               newS}
        end
      false ->
        {:noreply,
           r_state(s, control: del_cntrl(r_state(s, :control), pid))}
    end
  end
  ...
  def terminate(reason, s) do
    ...
    shutdownTimeout = (case (:application.get_env(:kernel,
                                                    :shutdown_timeout)) do
                         :undefined ->
                           :infinity
                         {:ok, t} ->
    ...
    foreach(fn {_AppName, id} when is_pid(id) ->
  ...
  defp maybe_load_application(application, s) do
  ...
  defp load(s,
            {applData, applEnv, incApps, optApps, descr, id, vsn,
               apps}) do
    ...
    appl = r_appl(name: name, descr: descr, id: id, vsn: vsn,
               apps: apps, appl_data: applData, inc_apps: incApps,
               opt_apps: optApps)
    :ets.insert(:ac_tab, {{:loaded, name}, appl})
    newS = foldl(fn app, s1 ->
    ...
    case (get_loaded(appName)) do
      {true, appl} ->
        case (:lists.keymember(appName, 1, running)) do
          ...
          false ->
            foreach(fn appName2 ->
                         case (:lists.keymember(appName2, 1,
                                                  started) or :lists.member(appName2,
                                                                              r_appl(appl, :opt_apps))) do
                           true ->
                             :ok
                           false ->
  ...
  defp do_start(appName, rT, type, from, s) do
    ...
    case (:lists.keymember(appName, 1, r_state(s, :start_req))) do
      false ->
        ...
        starting = (case (:lists.keymember(appName, 1,
                                             r_state(s, :starting))) do
                      false ->
                        [{appName, restartType, type, from} | r_state(s, :starting)]
                      true ->
                        r_state(s, :starting)
                    end)
        r_state(s, starting: starting, 
               start_req: [{appName, from} | start_req])
      true ->
        s
    end
  ...
  defp spawn_starter(from, appl, s, type) do
    spawn_link(:application_controller, :init_starter,
                 [from, appl, s, type])
  end
  ...
  def init_starter(_From, appl, s, type) do
    ...
    :gen_server.cast(:application_controller,
      ...
      _ ->
        ...
        foreach(fn appName ->
                     case (:lists.keymember(appName, 1,
                                              running) or :lists.member(appName,
                                                                          r_appl(appl, :opt_apps))) do
                       true ->
                         :ok
                       false ->
  ...
  defp make_appl_i({:application, name, opts})
    ...
    {r_appl_data(name: name, regs: regs, mod: mod, phases: phases,
  ...
  defp do_change_apps(applications, config, oldAppls) do
    ...
    :lists.foreach(fn {:error, {sysFName, line, str}} ->
                            ...
                            :erlang.apply(:logger, :macro_log,
                                            [%{mfa:
                                               {:application_controller,
                                                  :do_change_apps, 3},
                                                 line: 1588, file: 'otp/lib/kernel/src/application_controller.erl'},
                                                 :error, '~tp: ~w: ~ts~n', [sysFName, line,
                                                                           str],
                                                                %{error_logger:
                                                                  %{tag:
                                                                    :error}}])
  ...
  defp do_change_appl({:ok,
             {applData, env, incApps, optApps, descr, id, vsn,
                apps}},
            oldAppl, config) do
    ...
    r_appl(oldAppl, appl_data: applData,  descr: descr,  id: id, 
                 vsn: vsn,  inc_apps: incApps,  opt_apps: optApps, 
                 apps: apps)
  end
  ...
  defp handle_make_term_error(mod, reason, str) do
    case (:logger.allow(:error, :application_controller)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa:
                           {:application_controller, :handle_make_term_error,
                              3},
                             line: 1669, file: 'otp/lib/kernel/src/application_controller.erl'},
                             :error, 'application_controller: ~ts: ~ts~n', [mod.format_error(reason), str],
  ...
  defp do_config_change([{app, _Id} | apps], envBefore, errors) do
    ...
    appEnvBefore = (case (:lists.keyfind(app, 1,
                                           envBefore)) do
                      false ->
                        []
                      {^app, appEnvBeforeT} ->
  ...
  defp do_config_diff([], appEnvBefore, {changed, new}) do
    removed = :lists.foldl(fn {env, _Value}, acc ->
                                [env | acc]
                           end,
                             [], appEnvBefore)
    {changed, new, removed}
  ...
  defp do_config_diff([{env, value} | appEnvNow], appEnvBefore,
            {changed, new}) do
    case (:lists.keyfind(env, 1, appEnvBefore)) do
      {^env, ^value} ->
        do_config_diff(appEnvNow,
                         :lists.keydelete(env, 1, appEnvBefore), {changed, new})
      {^env, _OtherValue} ->
        do_config_diff(appEnvNow,
      ...
      false ->
        do_config_diff(appEnvNow, appEnvBefore,
                         {changed, [{env, value} | new]})
    end
  end

  defp conf_param_to_conf({:config, fileName}) do
      ...
      {:ok, newEnv} ->
        cond do
          bFName === 'sys' ->
            ...
            {:ok, sysEnv, errors} = check_conf_sys(newEnv, [], [],
                                                     dName)
            case (errors) do
              [] ->
                sysEnv
  ...
  defp conf_param_to_conf({:configfd, fileDescStrP}) do
    fileDescStr = :unicode.characters_to_nfc_list(fileDescStrP)
    isDigit = fn c ->
                   :lists.member(c, '0123456789')
              end
    {fdString, fileDescType} = :lists.splitwith(isDigit,
                                                  fileDescStr)
    fileDesc = (try do
                  :erlang.list_to_integer(fdString)
                catch
                  :error, :badarg ->
                    throw({:error,
                             {:file_descriptor, fileDescStr, :invalid_file_desc,
                                'The given file descriptor has incorrect format. The format should be "FileDescId[.FileType]". Examples: 3 or 3.config'}})
                end)
    case (:lists.member(fileDescType, ['.config', ''])) do
      false ->
        throw({:error,
                 {:file_descriptor, fileDescStr, :invalid_file_desc,
                    :io_lib.format('Cannot parse file descriptor of type: ~ts', [fileDescType])}})
      true ->
        :ok
    end
    case (load_file_descriptor(fileDesc)) do
      {:ok, newEnv} ->
      ...
      {:error, {line, _Mod, str}} ->
        throw({:error,
                 {:file_descriptor, fileDescStr, line, str}})
    end
  end

  defp config_param_to_list({type, [first | _] = configVals})
      when (is_list(first) and
              type === :config or type === :configfd) do
    for val <- configVals do
      {type, val}
    end
  end

  defp config_param_to_list({:config, val}) do
    [{:config, val}]
  end

  defp config_param_to_list({:configfd, val}) do
    [{:configfd, val}]
  end

  defp config_param_to_list(_) do
    []
  end

  defp check_conf() do
    configParameters = :lists.flatmap(&config_param_to_list/1,
                                        :init.get_arguments())
    mergedConf = :lists.foldl(fn configParameter, env ->
  ...
  defp check_conf_sys([file | t], sysEnv, errors, dName)
    ...
    fName = :filename.join(:filename.dirname(file),
                             bFName ++ '.config')
    lName = (case (:filename.pathtype(fName)) do
               :relative when dName !== [] ->
    ...
    case (load_file(lName)) do
      {:ok, newEnv} ->
        check_conf_sys(t, merge_env(sysEnv, newEnv), errors,
                         dName)
      {:error, {line, _Mod, str}} ->
        check_conf_sys(t, sysEnv,
                         [{:error, {lName, line, str}} | errors], dName)
    end
  end
  ...
  defp load_file_descriptor(fileDescriptorId) do
    case (:init.get_configfd(fileDescriptorId)) do
      :none ->
        warningIntervalMs = 20000
        maxConfSizeBytes = 1024 * 1024 * 128
        case (read_fd_until_end_and_close(fileDescriptorId,
          ...
          {:error, reason} ->
            {:error,
               {:none, :read_from_file_descriptor,
  ...
  defp scan_file(str) do
    case (:erl_scan.tokens([], str, 1)) do
      ...
      {:done, result, _} ->
        {:error,
           {:none, :parse_file, :erlang.tuple_to_list(result)}}
      {:more, _} ->
        {:error, {:none, :load_file, 'no ending <dot> found'}}
    end
  end

  defp read_fd_until_end_and_close(fileDescriptorId,
            timeBetweenWarningsMilliseconds, maxSizeBytes) do
    readLimit = 1024
    read = fn read
           ref, _Fd, _Data, dataSize when dataSize > maxSizeBytes
                                          ->
             reason = :io_lib.format('Max size ~w bytes exceeded', [maxSizeBytes])
             {ref, :error, reason}
           ref, fd, data, dataSize ->
             case (:file.read(fd, readLimit)) do
               :eof ->
    ...
    ref = make_ref()
    reader = spawn(fn () ->
                        case (:prim_file.file_desc_to_ref(fileDescriptorId,
                                                            [:read])) do
                          {:ok, fd} ->
                            res = read.(ref, fd, [], 0)
                            :prim_file.close(fd)
                            send(receiver, res)
                          {:error, _} ->
                            send(receiver, {ref, :error, 'Invalid file descriptor'})
                        end
                   end)
    send(reader, {:reader_ref, ref})
    (fn getResult
      ->
       receive do
         {^ref, :ok, data} ->
       ...
       after timeBetweenWarningsMilliseconds ->
         msg = :io_lib.format('Slow -configfd file descriptor ~p. The system will continue to read from the file descriptor and will be blocked until end of file is received.', [fileDescriptorId])
         case (:logger.allow(:warning,
                               :application_controller)) do
           true ->
             :erlang.apply(:logger, :macro_log,
                             [%{mfa:
                                {:application_controller,
                                   :read_fd_until_end_and_close, 3},
                                  line: 2094, file: 'otp/lib/kernel/src/application_controller.erl'},
                                  :warning, msg])
           false ->
             :ok
         end
      ...
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:application_controller, :info_started, 2},
                             line: 2117, file: 'otp/lib/kernel/src/application_controller.erl'},
                             :info, %{label:
                                      {:application_controller, :progress},
                                        report:
                                        [{:application, name}, {:started_at,
                                                                  node}]},
                                        %{domain: [:otp, :sasl],
                                            report_cb:
                                            &:application_controller.format_log/2,
                                            logger_formatter: %{title: 'PROGRESS REPORT'},
                                            error_logger:
                                            %{tag: :info_report,
                                                type: :progress,
                                                report_cb:
                                                &:application_controller.format_log/1}}])
      false ->
        :ok
    end
  end
  ...
  defp info_exited(name, reason, type) do
    case (:logger.allow(:notice,
                          :application_controller)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:application_controller, :info_exited, 3},
                             line: 2129, file: 'otp/lib/kernel/src/application_controller.erl'},
                             :notice, %{label: {:application_controller, :exit},
                                          report:
                                          [{:application, name}, {:exited,
                                                                    reason},
                                                                     {:type,
                                                                        type}]},
                                          %{domain: [:otp],
  ...
  defp limit_report(%{label: {:application_controller, :progress},
              report:
              [{:application, _} = application, {:started_at,
                                                   node}]} = logReport,
            depth) do
    Map.put(logReport, :report,
                         [application, {:started_at,
                                          :io_lib.limit_term(node, depth)}])
  end

  defp limit_report(%{label: {:application_controller, :exit},
              report:
              [{:application, _} = application, {:exited, reason},
                                                    {:type, type}]} = logReport,
            depth) do
    Map.put(logReport, :report,
                         [application, {:exited,
  ...
  defp format_log_single(%{label: {:application_controller, :progress},
  ...
  defp format_log_single(%{label: {:application_controller, :exit},
              report:
              [{:application, name}, {:exited, reason}, {:type,
                                                           type}]},
            %{single_line: true, depth: depth} = formatOpts) do
    p = p(formatOpts)
  ...
  defp format_log_multi(%{label: {:application_controller, :progress},
              report: [{:application, name}, {:started_at, node}]},
  ...
  defp format_log_multi(%{label: {:application_controller, :exit},
              report:
              [{:application, name}, {:exited, reason}, {:type,
                                                           type}]},
            %{depth: depth} = formatOpts) do
    p = p(formatOpts)
    format = :lists.append(['    application: ', p, '~n', '    exited: ', p, '~n', '    type: ', p, '~n'])
    args = (case (depth) do
              :unlimited ->
                [name, reason, type]
              _ ->
