defmodule :m_erl_epmd do
  ...
  require Record
  Record.defrecord(:r_state, :state, socket: :undefined,
                                 port_no: - 1, name: '', family: :undefined)
  ...
  defp getepmdbyname(hostName, timeout) when is_list(hostName) do
    family = (case (:inet_db.res_option(:inet6)) do
                true ->
                  :inet6
                false ->
                  :inet
  ...
  def register_node(name, portNo, family) do
    :gen_server.call(:erl_epmd,
                       {:register, name, portNo, family}, :infinity)
  end
  ...
  def handle_call({:register, name, portNo, family}, _From,
           state) do
    case (r_state(state, :socket)) do
      p when p < 0 ->
        case (do_register_node(name, portNo, family)) do
          {:alive, socket, creation} ->
            s = r_state(state, socket: socket,  port_no: portNo, 
          ...
          error ->
            case (:init.get_argument(:erl_epmd_port)) do
              {:ok, _} ->
                {:reply, {:ok, - 1},
                   r_state(state, socket: - 1,  port_no: portNo,  name: name)}
              :error ->
                {:reply, error, state}
            end
        end
  ...
  def handle_call(:client_info_req, _From, state) do
    reply = {:ok,
               {:r4, r_state(state, :name), r_state(state, :port_no)}}
    {:reply, reply, state}
  end
  ...
  def handle_info(:reconnect, state) when r_state(state, :socket) === -
                                                       1 do
    case (do_register_node(r_state(state, :name),
                             r_state(state, :port_no), r_state(state, :family))) do
      {:alive, socket, _Creation} ->
        {:noreply, r_state(state, socket: socket)}
      _Error ->
        :erlang.send_after(2000, self(), :reconnect)
        {:noreply, state}
    end
  ...
  defp do_register_node(nodeName, tcpPort, family) do
    ...
    case (localhost) do
      {:ok, socket} ->
        ...
        packet = [[(len >>> 8) &&& 255, len &&& 255], ?x,
                                                          [(tcpPort >>> 8) &&& 255,
                                                               tcpPort &&& 255],
                                                              ?M, 0,
                                                                      [(epmd_dist_high() >>> 8) &&& 255,
                                                                           epmd_dist_high() &&& 255],
                                                                          [(epmd_dist_low() >>> 8) &&& 255,
                                                                               epmd_dist_low() &&& 255],
                                                                              [(length(name) >>> 8) &&& 255,
  ...
  defp wait_for_port_reply_cont2(socket, data) do
    [a, b, _Type, _Proto, highA, highB, lowA, lowB, nLenA,
                                                        nLenB | rest] = data
    wait_for_port_reply_name(socket, nLenA <<< 8 ||| nLenB,
                               rest)
    low = lowA <<< 8 ||| lowB
    high = highA <<< 8 ||| highB
