defmodule :m_erl_epmd do
      ...
      p when p < 0 ->
        case do_register_node(name, portNo, family) do
          {:alive, socket, creation} ->
            s = r_state(state, socket: socket, port_no: portNo, name: name, family: family)
            {:reply, {:ok, creation}, s}
  ...
  def handle_info(:reconnect, state) when r_state(state, :socket) === -1 do
    case do_register_node(
           r_state(state, :name),
           r_state(state, :port_no),
           r_state(state, :family)
         ) do
  ...
  defp epmd_dist_high() do
    case :os.getenv(~c"ERL_EPMD_DIST_HIGH") do
      false ->
        6
  ...
  defp epmd_dist_low() do
    case :os.getenv(~c"ERL_EPMD_DIST_LOW") do
      false ->
        6
  ...
  defp parse_line(~c"name " ++ buf0) do
