defmodule :m_gen_tcp_socket do
  ...
  defp connect_lookup(%{family: domain, addr: address, port: port} = _SockAddr, opts0, timer) do
  ...
  defp connect_lookup(domain, address, port, mod, opts0, timer) do
    ...
    try do
      ...
      {addrs, r_connect_opts(fd: fd, ifaddr: bindAddr, port: bindPort, opts: connectOpts)} ->
        bindSockaddr = bind_addr(domain, bindAddr, bindPort)
        extraOpts = extra_opts(fd)
        connect_open(addrs, domain, connectOpts, startOpts, extraOpts, timer, bindSockaddr)
    end
  end

  defp connect_open(addrs, domain, connectOpts, startOpts, extraOpts, timer, bindAddr) do
    case start_server(
           domain,
  ...
  defp bind_addr(domain, %{family: domain} = bindSockaddr, _BindPort) do
    bindSockaddr
  end
        ...
        case :inet.listen_options(
          ...
          {:ok,
           r_listen_opts(
            ...
            bindSockaddr = bind_addr(domain, bindAddr, bindPort)
            extraOpts = extra_opts(fd)
            listen_open(domain, listenOpts, startOpts, extraOpts, backlog, bindSockaddr)
        end
  ...
  defp listen_open(domain, listenOpts, startOpts, extraOpts, backLog, bindAddr) do
    case start_server(domain, [{:timeout, :infinity} | startOpts], extraOpts) do
      {:ok, server} ->
        errRef = make_ref()
  ...
  def sendfile({:"$inet", :gen_tcp_socket, {_Server, socket}}, fileHandle, offset, count) do
    case :socket.getopt(socket, {:otp, :meta}) do
      {:ok, %{packet: _}} ->
        try do
  ...
  def monitor({:"$inet", :gen_tcp_socket, {_Server, eSock}} = socket) do
  ...
  def unrecv(
        {:"$inet", :gen_tcp_socket, {_Server, _Socket}},
        _Data
      ) do
    {:error, :enotsup}
  end
  ...
  def fdopen(fd, opts)
      when is_integer(fd) and 0 <= fd and
             is_list(opts) do
    opts_1 = internalize_setopts(opts)
    {mod, opts_2} = :inet.tcp_module(opts_1)
    domain = domain(mod)
  ...
  defp call(server, call) do
    try do
      ...
      c, e ->
        error_msg(
          ~c"~w call failed: ~n      Call:  ~p~n      Class: ~p~n      Error: ~p~n      Stack: ~p",
  ...
  defp stop_server(server) do
    try do
      :gen_statem.stop(server, {:shutdown, :closed}, :infinity)
    catch
      _, _ ->
        :ok
    else
  ...
  def init({:open, domain, extraOpts, owner}) do
    ...
    case socket_open(domain, extraOpts, extra) do
      {:ok, socket} ->
        ...
        {:ok, :connect, {p, Map.merge(d, %{type: :undefined, buffer: <<>>})}}
  ...
  def init({:prepare, d, owner}) do
    ...
    {:ok, :accept, {p, Map.merge(d, %{type: :undefined, buffer: <<>>})}}
  end

  def init(arg) do
    error_report([{:badarg, {:gen_tcp_socket, :init, [arg]}}])
  ...
  defp socket_open(domain, %{fd: fD} = extraOpts, extra) do
    opts =
      Map.merge(
        ...
        %{dup: false, domain: domain, type: :stream, protocol: proto(domain)}
      )
  ...
  def handle_event({:call, from}, {:getopts, opts}, state, {p, d}) do
    result =
      case state_getopts(p, d, state, opts) do
        {:ok, optVals} ->
  ...
  def handle_event({:call, from}, :info, state, {p, d}) do
    case state do
      ...
      _ ->
        {d_1, result} = handle_info(r_params(p, :socket), r_params(p, :owner), d)
  ...
  def handle_event(
        :info,
        {:"$socket", listenSocket, :completion, {completionRef, completionStatus}},
        r_accept(
          info: {:completion_info, _, completionRef},
          from: from,
  ...
  def handle_event(
    ...
    {:keep_state, {p, Map.put(d, :type, :listen)}, [{:reply, from, result}]}
  end
  ...
  def handle_event({:call, from}, :fdopen, :connect = _State, {r_params(socket: socket) = p, d}) do
    handle_connected(p, Map.put(d, :type, :fdopen), [{:reply, from, {:ok, socket}}])
  end
  ...
  def handle_event(
        ...
        r_connect(info: {:completion_info, _, completionRef}, from: from, addr: addr) = _State,
        {r_params(socket: socket) = p, d}
      ) do
  ...
  def handle_event(
    ...
    {:next_state, :closed, p_D, [{:reply, from, {:error, newReason}}]}
  end
  ...
  defp handle_shutdown(r_params(socket: socket), :closed_write = _State, :read = how) do
    handle_shutdown2(socket, :closed_read_write, how)
  end

  defp handle_shutdown(r_params(socket: socket), :closed_read = _State, :write = how) do
    handle_shutdown2(socket, :closed_read_write, how)
  end

  defp handle_shutdown(r_params(socket: socket), :connected = _State, :write = how) do
  ...
  defp handle_shutdown(r_params(socket: socket), :connected = _State, :read = how) do
  ...
  defp handle_shutdown(r_params(socket: socket), :connected = _State, :read_write = how) do
    handle_shutdown2(socket, :closed_read_write, how)
  end
  ...
  defp handle_unexpected(type, content, state, {p, _D}) do
    warning_msg(
      ~c"Received unexpected event:~n   Socket:     ~p~n   State:      ~p~n   Event Type: ~p~n   Content:    ~p",
      [r_params(p, :socket), state, type, content]
    )
  ...
  defp handle_closed(type, content, state, {p, _D}) do
    case type do
      ...
      _ ->
        warning_msg(
          ~c"Received unexpected event when closed:~n   Socket:     ~p~n   State:      ~p~n   Event Type: ~p~n   Content:    ~p",
          [r_params(p, :socket), state, type, content]
        )
  ...
  defp handle_connect(r_params(socket: socket) = p, d, from, addr, timeout, status)
    ...
    case :socket.connect(socket, addr, :nowait) do
      ...
      {:select, {:select_info, _, _} = info} ->
        {:next_state, r_connect(info: info, from: from, addr: addr),
         {p, Map.put(d, :type, :connect)}, [{{:timeout, :connect}, timeout, :connect}]}
  ...
  defp handle_connect(r_params(socket: socket) = p, d, from, _Addr, _Timeout, :ok) do
    handle_connected(p, Map.put(d, :type, :connect), [
      {{:timeout, :connect}, :cancel},
      {:reply, from, {:ok, socket}}
    ])
  ...
  defp handle_connect(r_params() = p, d, from, _Addr, _Timeout, {:error, _Reason} = error) do
    {:next_state, :connect, {p, d}, [{{:timeout, :connect}, :cancel}, {:reply, from, error}]}
  end
  ...
  defp handle_accept(p, d, from, listenSocket, timeout, status)
    ...
    case :socket.accept(listenSocket, :nowait) do
      ...
      {:select, {:select_info, _, _} = selectInfo} ->
        {:next_state, r_accept(info: selectInfo, from: from, listen_socket: listenSocket),
         {p, Map.put(d, :type, :accept)}, [{{:timeout, :accept}, timeout, :accept}]}

      {:completion, {:completion_info, _, _} = completionInfo} ->
        {:next_state, r_accept(info: completionInfo, from: from, listen_socket: listenSocket),
         {p, Map.put(d, :type, :accept)}, [{{:timeout, :accept}, timeout, :accept}]}
  ...
  defp handle_accept(p, d, from, listenSocket, _Timeout, {:ok, socket}) do
    handle_accept_success(p, d, from, listenSocket, socket)
  end

  defp handle_accept(p, d, from, _ListenSocket, _Timeout, {:error, _Reason} = error) do
    handle_accept_failure(p, d, from, error)
  end
  ...
  defp handle_accept_success(p, d, from, listenSocket, accSocket) do
    ...
    handle_connected(
      r_params(p, socket: accSocket),
      Map.put(d, :type, :accept),
      [{{:timeout, :accept}, :cancel}, {:reply, from, {:ok, accSocket}}]
    )
  end
  ...
  defp handle_recv(p, %{packet: packet, recv_length: length} = d, actionsR, cS) do
    cond do
      0 < length ->
    ...
    case d do
      ...
      %{buffer: <<shortData::binary>>} when cS === :recv ->
        ...
        case socket_recv_peek(r_params(p, :socket), n) do
          {:ok, <<finalData::binary>>} ->
            handle_recv_peek2(p, d, actionsR, packet, <<shortData::binary, finalData::binary>>)
          ...
          {:completion, completion} ->
            {:next_state, r_recv(info: completion), {p, d}, reverse(actionsR)}
  ...
  defp handle_buffered(
         p,
         %{packet: :line, line_delimiter: lineDelimiter, packet_size: packetSize} = d,
         data
       ) do
    decodeOpts = [{:line_delimiter, lineDelimiter}, {:line_length, packetSize}]
  ...
  defp handle_buffered(p, %{packet_size: packetSize} = d, data, decocdeOpts0) do
    ...
    case :erlang.decode_packet(type, data, decodeOpts) do
      ...
      {:error, reason} ->
        warning_msg(
          ~c"Failed decoding message~n   Socket:          ~p~n   Socket server:   ~p~n   Packet type:     ~p~n   byte_size(Data): ~p~n   Reason:          ~p",
  ...
  defp deliver_buffered_data(
         r_params(owner: owner) = p,
         %{active: active, mode: mode, header: header, deliver: deliver, packet: packet} = d,
         data
       ) do
  ...
  defp handle_recv_length(p, %{buffer: buffer} = d, actionsR, length, cS) do
  ...
  defp handle_recv_length(p, d, actionsR, length, buffer, cS)
    ...
    case socket_recv(r_params(p, :socket), length) do
      ...
      {:completion, {:completion_info, _, _} = completionInfo} ->
        {:next_state, r_recv(info: completionInfo), {p, %{d | buffer: buffer}}, reverse(actionsR)}
  ...
  defp handle_recv_decode(p, %{packet_size: packetSize} = d, actionsR, data, decocdeOpts0, cS) do
    decodeOpts = [{:packet_size, packetSize} | decocdeOpts0]

    case :erlang.decode_packet(decode_packet(d), data, decodeOpts) do
      {:ok, decoded, rest} ->
        buffer =
          case rest do
            <<>> ->
  ...
  defp handle_recv_more(p, d, actionsR, bufferedData, cS)
    ...
    case socket_recv(r_params(p, :socket), 0) do
      ...
      {:completion, {:completion_info, _, _} = completionInfo} ->
        {:next_state, r_recv(info: completionInfo), {p, %{d | buffer: bufferedData}},
         reverse(actionsR)}
    ...
    case :maps.is_key(tag, socketOpts) do
      true ->
        case r_params(p, :socket) do
          ...
          socket ->
            case socket_setopt(socket, :maps.get(tag, socketOpts), val) do
              :ok ->
        ...
        case :maps.is_key(tag, server_write_opts()) do
          ...
          false ->
            case :maps.is_key(tag, server_read_opts()) do
              true
              when state === :closed or state === :closed_read or
    ...
    case :maps.is_key(key, socketOpts) do
      true ->
        case r_params(p, :socket) do
          ...
          socket ->
            case socket_getopt(socket, :maps.get(key, socketOpts), val) do
              {:ok, value} ->
  ...
  defp handle_info(socket, owner, %{active: active} = d) do
    ...
    {d_1, Map.merge(info, %{counters: counters_4, owner: owner, active: active})}
  end
  ...
  defp error_msg(f, a) do
    :error_logger.error_msg(f ++ ~c"~n", a)
  end
