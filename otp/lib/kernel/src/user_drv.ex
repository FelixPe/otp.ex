defmodule :m_user_drv do
  ...
  require Record
  Record.defrecord(:r_editor, :editor, port: :undefined, file: :undefined, requester: :undefined)
        ...
        case :logger.allow(:warning, :user_drv) do
          true ->
            :erlang.apply(:logger, :macro_log, [
              %{
                mfa: {:user_drv, :start, 0},
                line: 124,
                file: ~c"otp/lib/kernel/src/user_drv.erl"
              },
              :warning,
              ~c"Multiple -remsh given to erl, using the first, ~p",
              [node]
            ])
  ...
  def start(args) when is_map(args) do
    case :gen_statem.start({:local, :user_drv}, :user_drv, args, []) do
      {:ok, pid} ->
        pid
    ...
    try do
      cond do
        ...
        isTTY and startShell ->
          ...
          {:ok, :init, {args, r_state(user: start_user())}, {:next_event, :internal, tTYState}}

        true ->
          tTYState = :prim_tty.init(%{input: :maps.get(:input, args, true), tty: false})
      ...
      :error, :enotsup ->
        ...
        {:ok, :init, {args, r_state(shell_started: :old, user: start_user())},
  ...
  defp init_standard_error(tTY, newlineCarriageReturn) do
    ...
    :ok =
      :io.setopts(
        :standard_error,
        [{:encoding, encoding}, {:onlcr, newlineCarriageReturn}]
      )
  end

  def init(:internal, tTYState, {args, state = r_state(user: user)}) do
    ...
    case args do
      ...
      %{initial_shell: {:remote, node}} ->
        ...
        exit_on_remote_shell_error(
          node,
          initialShell,
          init_remote_shell(newState, node, initialShell)
        )
  ...
  defp exit_on_remote_shell_error(remoteNode, _, {:error, :noconnection}) do
    :io.format(:standard_error, ~c"Could not connect to ~p\n", [remoteNode])
    :erlang.halt(1)
  end

  defp exit_on_remote_shell_error(remoteNode, {m, _, _}, {:error, reason}) do
    :io.format(:standard_error, ~c"Could not load ~p on ~p (~p)\n", [remoteNode, m, reason])
    :erlang.halt(1)
  end
  ...
  defp init_noshell(state) do
    init_shell(r_state(state, shell_started: false), ~c"")
  end
  ...
  defp init_remote_shell(state, node, {m, f, a}) do
    ...
    case :net_kernel.connect_node(remoteNode) do
      true ->
        case :erpc.call(remoteNode, :code, :ensure_loaded, [m]) do
          {:error, reason} when reason !== :embedded ->
            {:error, reason}

          _ ->
            case :erpc.call(remoteNode, :net_kernel, :get_net_ticktime, []) do
            ...
            group =
              :group.start(
                self(),
                rShell,
                [{:echo, r_state(state, :shell_started) === :new}] ++ group_opts(remoteNode)
              )
    ...
    gr =
      gr_add_cur(
        ...
        :group.start(
          self(),
          initialShell,
          group_opts() ++ [{:echo, r_state(state, :shell_started) === :new}]
        ),
        initialShell
      )
  ...
  defp init_shell(state, slogan) do
    ...
    {:next_state, :server, r_state(state, current_group: gr_cur_pid(r_state(state, :groups))),
     {:next_event, :info,
  ...
  def server({:call, from}, {:start_shell, args}, state = r_state(tty: tTY, shell_started: false)) do
    isTTY = :prim_tty.isatty(:stdin) === true and :prim_tty.isatty(:stdout) === true
    startShell = :maps.get(:initial_shell, args, :undefined) !== :noshell
    oldShell = :maps.get(:initial_shell, args, :undefined) === :oldshell
    ...
    %{read: readHandle, write: writeHandle} = :prim_tty.handles(r_state(newState, :tty))
    ...
    {result, reply} =
      case :maps.get(:initial_shell, args, :undefined) do
        :noshell ->
          {init_noshell(newHandleState), :ok}

        {:remote, node} ->
          case init_remote_shell(newHandleState, node, {:shell, :start, []}) do
            {:error, _} = error ->
        ...
        {:remote, node, initialShell} ->
          case init_remote_shell(newHandleState, node, initialShell) do
            {:error, _} = error ->
              {init_noshell(newHandleState), error}
  ...
  def server(:info, {readHandle, {:data, uTF8Binary}}, state = r_state(read: readHandle))
      when r_state(state, :current_group) === r_state(state, :user) do
    send(r_state(state, :current_group), {self(), {:data, uTF8Binary}})
    :keep_state_and_data
  end

  def server(:info, {readHandle, {:data, uTF8Binary}}, state = r_state(read: readHandle)) do
    case contains_ctrl_g_or_ctrl_c(uTF8Binary) do
      :ctrl_g ->
        {:next_state, :switch_loop, state, {:next_event, :internal, :init}}

      :ctrl_c ->
        case gr_get_info(
               r_state(state, :groups),
               r_state(state, :current_group)
             ) do
          :undefined ->
            :ok
  ...
  def server(:info, {readHandle, :eof}, state = r_state(read: readHandle)) do
  ...
  def server(
        :info,
        {readHandle, {:signal, signal}},
        state = r_state(tty: tTYState, read: readHandle)
      ) do
    {:keep_state,
     r_state(state,
       tty:
         :prim_tty.handle_signal(
           tTYState,
           signal
         )
     )}
  ...
  def server(:info, {requester, :tty_geometry}, r_state(tty: tTYState)) do
  ...
  def server(:info, {requester, :get_unicode_state}, r_state(tty: tTYState)) do
    send(requester, {self(), :get_unicode_state, :prim_tty.unicode(tTYState)})
    :keep_state_and_data
  end

  def server(:info, {requester, :set_unicode_state, bool}, r_state(tty: tTYState) = state) do
    ...
    send(requester, {self(), :set_unicode_state, oldUnicode})
    {:keep_state, r_state(state, tty: newTTYState)}
  end

  def server(:info, {requester, :get_terminal_state}, _State) do
    send(requester, {self(), :get_terminal_state, :prim_tty.isatty(:stdout)})
    :keep_state_and_data
  end

  def server(:info, {requester, {:open_editor, buffer}}, r_state(tty: tTYState) = state) do
    case open_editor(tTYState, buffer) do
      ...
      {editorPort, tmpPath} ->
        {:keep_state,
         r_state(state, editor: r_editor(port: editorPort, file: tmpPath, requester: requester))}
    end
  end

  def server(:info, req, state = r_state(user: user, current_group: curr, editor: :undefined))
      when :erlang.element(
             1,
             req
           ) === user or
    ...
    {newTTYState, newQueue} = handle_req(req, r_state(state, :tty), r_state(state, :queue))
    {:keep_state, r_state(state, tty: newTTYState, queue: newQueue)}
  end
  ...
  def server(
    ...
    {newTTYState, newQueue} = handle_req(:next, r_state(state, :tty), {false, iOQ})
    {:keep_state, r_state(state, tty: newTTYState, queue: newQueue)}
  end
  ...
  def server(
        ...
        r_state(queue: {{origin, monitorRef, reply}, _IOQ})
      ) do
    ...
    case :logger.allow(:info, :user_drv) do
      true ->
        :erlang.apply(:logger, :macro_log, [
          %{mfa: {:user_drv, :server, 3}, line: 500, file: ~c"otp/lib/kernel/src/user_drv.erl"},
          :info,
          ~c"Failed to write to standard out (~p)",
          [reason]
        ])
  ...
  def server(:info, {requester, {:put_chars_sync, _, _, reply}}, _State) do
    send(requester, {:reply, reply, :ok})
    :keep_state_and_data
  end

  def server(:info, {:EXIT, user, :shutdown}, r_state(user: user)) do
    :keep_state_and_data
  end

  def server(:info, {:EXIT, user, _Reason}, state = r_state(user: user)) do
    newUser = start_user()
  ...
  def server(
        ...
        state =
          r_state(
            tty: tTYState,
            editor: r_editor(requester: requester, port: editorPort, file: pathTmp)
          )
      ) do
    ...
    send(requester, {self(), {:editor_data, :string.chomp(unicode)}})
  ...
  def server(:info, {:EXIT, group, reason}, state) do
    case gr_cur_pid(r_state(state, :groups)) do
      ^group when reason !== :die and reason !== :terminated ->
        ...
        case gr_get_info(r_state(state, :groups), group) do
          {ix, {:shell, :start, params}} ->
            ...
            {:ok, gr2} =
              gr_set_cur(
                gr_set_num(gr1, ix, newGroup, {:shell, :start, params}),
                ix
              )

            {:keep_state, r_state(state, tty: newTTyState, current_group: newGroup, groups: gr2)}
      ...
      _ ->
        {:keep_state, r_state(state, groups: gr_del_pid(r_state(state, :groups), group))}
    end
  end
  ...
  def switch_loop(:internal, :init, state) do
    case :application.get_env(:stdlib, :shell_esc, :jcl) do
      :abort ->
      ...
      :jcl ->
        ...
        {:keep_state, r_state(state, tty: newTTYState), {:next_event, :internal, :line}}
    end
  end

  def switch_loop(:internal, :line, state) do
    {:more_chars, cont, rs} = :edlin.start(~c" --> ")
    {:keep_state, {cont, r_state(state, tty: io_requests(rs, r_state(state, :tty)))}}
  end
  ...
  def switch_loop(:internal, {:line, line}, state) do
    case :erl_scan.string(line) do
      ...
      {:error, _, _} ->
        newTTYState =
          io_requests(
            [{:insert_chars, :unicode, "Illegal input\n"}],
            r_state(state, :tty)
          )
  ...
  def switch_loop(:info, {readHandle, {:data, cs}}, {cont, r_state(read: readHandle) = state}) do
    case :edlin.edit_line(
           :unicode.characters_to_list(cs),
           cont
         ) do
      {:done, {[line], _, _}, _Rest, rs} ->
        {:keep_state, r_state(state, tty: io_requests(rs, r_state(state, :tty))),
         {:next_event, :internal, {:line, line}}}

      {:more_chars, newCont, rs} ->
        {:keep_state, {newCont, r_state(state, tty: io_requests(rs, r_state(state, :tty)))}}

      {:blink, newCont, rs} ->
        {:keep_state, {newCont, r_state(state, tty: io_requests(rs, r_state(state, :tty)))}, 1000}
    end
  end

  def switch_loop(:timeout, _, {_Cont, state}) do
    {:keep_state_and_data, {:next_event, :info, {r_state(state, :read), {:data, []}}}}
  end
  ...
  defp switch_cmd(
         [{:atom, _, key}, {:atom, _, v1}, {:atom, _, v2}],
         gr
       ) do
  ...
  defp switch_cmd(cmd, gr)
       when cmd === :c or cmd === :i or
              cmd === :k do
  ...
  defp switch_cmd({:r, node, shell}, gr0)
    ...
    case :erlang.is_alive() do
      ...
      false ->
        {:retry, [{:put_chars, :unicode, ~c"Node is not alive\n"}]}
    end
  end
      ...
      case version > 25 do
        true ->
          [
            {:expand_fun,
             fn b, opts ->
               :erpc.call(node, :edlin_expand, :expand, [b, opts])
             end}
          ]
  ...
  defp io_request({:put_chars, :unicode, chars}, tTY) do
    write(
      :prim_tty.handle_request(
        tTY,
        {:putc, :unicode.characters_to_binary(chars)}
      )
    )
  end
  ...
  defp io_request({:put_expand, :unicode, chars}, tTY) do
    write(
      :prim_tty.handle_request(
        tTY,
        {:expand_with_trim, :unicode.characters_to_binary(chars)}
      )
    )
  end
  ...
  defp io_request({:put_expand_no_trim, :unicode, chars}, tTY) do
    write(
      :prim_tty.handle_request(
        tTY,
        {:expand, :unicode.characters_to_binary(chars)}
      )
    )
  end
  ...
  defp io_request({:insert_chars, :unicode, chars}, tTY) do
    write(
      :prim_tty.handle_request(
        tTY,
        {:insert, :unicode.characters_to_binary(chars)}
      )
    )
  end
  ...
  defp io_request({:insert_chars_over, :unicode, chars}, tTY) do
    write(
      :prim_tty.handle_request(
        tTY,
        {:insert_over, :unicode.characters_to_binary(chars)}
      )
    )
  end
  ...
  defp io_requests(
         [
           {:insert_chars, :unicode, c1},
           {:insert_chars, :unicode, c2}
           | rs
         ],
         tTY
       ) do
    io_requests(
      [{:insert_chars, :unicode, [c1, c2]} | rs],
      tTY
    )
  end
  ...
  defp io_requests(
         [
           {:put_chars, :unicode, c1},
           {:put_chars, :unicode, c2}
           | rs
         ],
         tTY
       ) do
    io_requests(
      [{:put_chars, :unicode, [c1, c2]} | rs],
      tTY
    )
  end
  ...
  defp open_editor(tTY, buffer) do
    ...
    editor = :os.getenv(~c"VISUAL", :os.getenv(~c"EDITOR", defaultEditor))
    tmpFile = :string.chomp(mktemp()) ++ ~c".erl"
      ...
      true ->
        ...
        try do
          editorPort =
            case :os.type() do
              {:win32, _} ->
                [cmd | args] = :string.split(editor, ~c" ", :all)
          ...
          :error, :enoent ->
            ...
            :io.format(:standard_error, ~c"Could not find EDITOR '~ts'.~n", [editor])
            false
        end

      false ->
        :io.format(:standard_error, ~c"Could not find create temp file '~ts'.~n", [tmpFile])
        false
    end
  end
  ...
  defp mktemp() do
    case :os.type() do
      {:win32, _} ->
        :os.cmd(
          ~c"powershell \"write-host (& New-TemporaryFile | Select-Object -ExpandProperty FullName)\""
        )
  ...
  Record.defrecord(:r_group, :group, index: :undefined, pid: :undefined, shell: :undefined)
  Record.defrecord(:r_gr, :gr, next: 0, current: 0, pid: :none, groups: [])
  ...
  defp gr_add_cur(r_gr(next: next, groups: gs), pid, shell) do
    ...
    r_gr(next: next + 1, current: next, pid: pid, groups: gs ++ [gr_new_group(next, pid, shell)])
  end
  ...
  defp gr_set_num(gr = r_gr(groups: groups), i, pid, shell) do
    newGroups = :lists.keystore(i, r_group(:index), groups, gr_new_group(i, pid, shell))
  ...
  defp gr_list(r_gr(current: current, groups: groups)) do
    :lists.flatmap(
      fn
        ...
        r_group(index: i, shell: s) ->
          marker =
            for _ <- [:EFE_DUMMY_GEN], current === i do
              ~c"*"
            end
