defmodule :m_raw_file_io_deflate do
  ...
  def init({owner, secret, [:compressed]}) do
    ...
    z = :zlib.open()
    :ok = :zlib.deflateInit(z, :default, :deflated, 16 + 15,
                              8, :default)
  ...
  def opened({:call, {owner, _Tag} = from}, [:close],
    ...
    response = (case (flush_deflate_state(data)) do
                  :ok ->
  ...
  def opened({:call, {owner, _Tag} = from}, [:position, mark],
           %{owner: owner} = data) do
  ...
  def opened({:call, {owner, _Tag} = from}, [:write, iOVec],
           %{owner: owner} = data) do
  ...
  def opened({:call, {owner, _Tag} = from}, [:read, _Size],
           %{owner: owner}) do
    response = {:error, :ebadf}
    {:keep_state_and_data, [{:reply, from, response}]}
  end

  def opened({:call, {owner, _Tag} = from}, [:read_line],
           %{owner: owner}) do
    response = {:error, :ebadf}
    {:keep_state_and_data, [{:reply, from, response}]}
  end

  def opened({:call, {owner, _Tag} = from}, _Command,
           %{owner: owner}) do
  ...
  defp write(data, iOVec) do
    ...
    case (apply(r_file_descriptor(privateFd, :module), :write,
                  [privateFd, :zlib.deflate(z, iOVec)])) do
      :ok ->
  ...
  defp flush_deflate_state(%{handle: privateFd, zlib: z}) do
    case (apply(r_file_descriptor(privateFd, :module), :write,
                  [privateFd, :zlib.deflate(z, [], :finish)])) do
      :ok ->
        :ok
      other ->
        other
