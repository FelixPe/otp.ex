defmodule :m_user do
  ...
  defp io_request({:put_chars, :unicode, mod, func, args}, port,
            q) do
    case ((try do
            apply(mod, func, args)
          catch
  ...
  defp io_request({:put_chars, :latin1, mod, func, args}, port,
            q) do
    case ((try do
            apply(mod, func, args)
          catch
  ...
  defp io_request({:get_until, enc, prompt, m, f, as}, port, q) do
    get_chars(prompt, :io_lib, :get_until, {m, f, as}, port,
                q, enc)
  end
  ...
  defp io_request({:get_until, prompt, m, f, as}, port, q) do
    io_request({:get_until, :latin1, prompt, m, f, as},
                 port, q)
  end
  ...
  defp get_line_bytes(prompt, port, q, acc, bytes, enc) do
    case (:erlang.get(:shell)) do
      :noshell ->
        get_line_doit(prompt, port, :queue.snoc(q, bytes), acc,
                        enc)
      _ ->
        case (contains_ctrl_g_or_ctrl_c(bytes)) do
          false ->
      ...
      false ->
        ...
        case (srch(bin, ?\n, 0)) do
          ...
          {:match, [{pos, 1}]} ->
            ...
            case (accu) do
              [] ->
                {head, tail} = (case (is_cr_at(pos - 1, bin)) do
                                  false ->
                                    <<h :: size(posPlus) - binary,
                                        t :: binary>> = bin
                                    {h, t}
                                  true ->
                                    posMinus = pos - 1
                                    <<h :: size(posMinus) - binary, _, _,
                                        t :: binary>> = bin
                                    {binrev([], [h, ?\n]), t}
                                end)
                case (tail) do
    ...
    case (:queue.is_empty(q)) do
      true ->
        receive do
          ...
          {:io_request, from, replyAs, request} when is_pid(from)
                                                     ->
            get_chars_req(prompt, m, f, xa, port, q, state, request,
                            from, replyAs, enc)
          {:EXIT, from, what} when node(from) === node() ->
            {:exit, what}
        end
  ...
  defp get_chars_req(prompt, m, f, xtraArg, port, q, state, req,
            from, replyAs, enc) do
    do_io_request(req, from, replyAs, port, :queue.new())
    case (prompt(port, prompt)) do
      :error ->
  ...
  defp get_chars_apply(state0, m, f, xa, port, q, enc) do
    case ((try do
            apply(m, f,
                    [state0, cast(:queue.head(q), enc), enc, xa])
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
            e -> e
  ...
  defp prompt(port, prompt) do
    ...
    case (wrap_characters_to_binary(promptString, :unicode,
                                      encoding)) do
      bin when is_binary(bin) ->
        put_port(bin, port)
      :error ->
  ...
  defp contains_ctrl_g_or_ctrl_c(binOrList) do
    case ({:re.run(binOrList, <<3>>),
             :re.run(binOrList, <<7>>)}) do
      {:nomatch, :nomatch} ->
        false
      _ ->
        true
  ...
  defp cast(data, encoding) do
    ...
    cast(data, :erlang.get(:read_mode), ioEncoding,
           encoding)
  end
    ...
    case ((try do
      ...
      _ ->
        case ((try do
          ...
          l when is_list(l) ->
            :erlang.list_to_binary(for x <- l do
                                     case (x) do
                                       high when high > 255 ->
