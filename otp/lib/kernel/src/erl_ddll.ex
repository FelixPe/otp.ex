defmodule :m_erl_ddll do
  ...
  defp do_load_driver(path, driver, driverFlags) do
    case (:erl_ddll.try_load(path, driver,
                               [{:monitor, :pending_driver}] ++ driverFlags)) do
      {:error, :inconsistent} ->
  ...
  def reload_driver(path, driver) do
    do_load_driver(path, driver,
                     [{:reload, :pending_driver}, {:driver_options,
                                                     [:kill_ports]}])
  end
  ...
  def info(driver) do
    [{:processes, :erl_ddll.info(driver, :processes)},
         {:driver_options,
            :erl_ddll.info(driver, :driver_options)},
             {:port_count, :erl_ddll.info(driver, :port_count)},
                 {:linked_in_driver,
                    :erl_ddll.info(driver, :linked_in_driver)},
                     {:permanent, :erl_ddll.info(driver, :permanent)},
                         {:awaiting_load,
                            :erl_ddll.info(driver, :awaiting_load)},
                             {:awaiting_unload,
                                :erl_ddll.info(driver, :awaiting_unload)}]
  end
