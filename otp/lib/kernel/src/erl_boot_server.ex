defmodule :m_erl_boot_server do
  ...
  require Record
  Record.defrecord(:r_state, :state, priority: 0, version: '',
                                 udp_sock: :undefined, udp_port: :undefined,
                                 listen_sock: :undefined,
                                 listen_port: :undefined, slaves: :undefined,
                                 bootp: :undefined, prim_state: :undefined)
  def start(slaves) do
  ...
  def add_subnet(mask, addr) when (is_tuple(mask) and
                             is_tuple(addr)) do
  ...
  def delete_subnet(mask, addr) when (is_tuple(mask) and
                             is_tuple(addr)) do
  ...
  defp member_address(iP,
            [{{mA, mB, mC, mD}, {eA, eB, eC, eD}} | rest]) do
    {a, b, c, d} = iP
    cond do
  ...
  def init(slaves) do
    ...
    ref = make_ref()
    pid = :proc_lib.spawn_link(:erl_boot_server, :boot_init,
                                 [ref])
    :ok = :gen_tcp.controlling_process(l, pid)
    send(pid, {ref, l})
    :erlang.process_flag(:trap_exit, true)
    {:ok,
       r_state(priority: 0, version: :erlang.system_info(:version),
           udp_sock: u, udp_port: uPort, listen_sock: l,
           listen_port: port, slaves: :ordsets.from_list(slaves),
           bootp: pid)}
  end
  ...
  def handle_info({:udp, u, iP, port, data}, s0) do
    ...
    valid = member_address(iP,
                             :ordsets.to_list(r_state(s0, :slaves)))
    case ({valid, data, token}) do
      {true, ^token, ^token} ->
        case (:gen_udp.send(u, iP, port,
                              ['EBOOTR', r_state(s0, :priority), int16(r_state(s0, :listen_port)),
                                                        r_state(s0, :version)])) do
          :ok ->
            :ok
          {:error, :not_owner} ->
            :error_logger.error_msg('** Illegal boot server connection attempt: not owner of ~w ** ~n', [u])
          {:error, reason} ->
            err = :file.format_error(reason)
            :error_logger.error_msg('** Illegal boot server connection attempt: ~w POSIX error ** ~n', [u, err])
        end
        {:noreply, s0}
      {false, _, _} ->
        :error_logger.error_msg('** Illegal boot server connection attempt: ~w is not a valid address ** ~n', [iP])
        {:noreply, s0}
      {true, _, _} ->
        case ((try do
          ...
          'EBOOTQ' ->
            ...
            :error_logger.error_msg('** Illegal boot server connection attempt: client version is ~s ** ~n', [vsn])
          _ ->
            :error_logger.error_msg('** Illegal boot server connection attempt: unrecognizable request ** ~n', [])
        end
        {:noreply, s0}
    end
  end
  ...
  defp boot_main(listen) do
    tag = make_ref()
    pid = :proc_lib.spawn_link(:erl_boot_server,
                                 :boot_accept, [self(), listen, tag])
  ...
  defp handle_command(s, pS, msg) do
    case ((try do
      ...
      {:list_dir, dir} ->
        ...
        pS2
      {:read_file_info, file} ->
        {res, pS2} = :erl_prim_loader.prim_read_file_info(pS,
                                                            file, true)
