defmodule :m_erl_boot_server do
  ...
  def handle_info({:udp, u, iP, port, data}, s0) do
    token = ~c"EBOOTQ" ++ r_state(s0, :version)
    ...
    case {valid, data, token} do
        ...
        case :gen_udp.send(u, iP, port, [
          ...
          {:error, :not_owner} ->
            :error_logger.error_msg(
              ~c"** Illegal boot server connection attempt: not owner of ~w ** ~n",
              [u]
            )
          ...
          {:error, reason} ->
            ...
            :error_logger.error_msg(
              ~c"** Illegal boot server connection attempt: ~w POSIX error ** ~n",
              [u, err]
            )
        end
      ...
      {false, _, _} ->
        :error_logger.error_msg(
          ~c"** Illegal boot server connection attempt: ~w is not a valid address ** ~n",
          [iP]
        )
      ...
      {true, _, _} ->
        case (try do
                :string.slice(data, 0, length(~c"EBOOTQ"))
              catch
          ...
          ~c"EBOOTQ" ->
            vsn = :string.slice(data, length(~c"EBOOTQ"), length(data))

            :error_logger.error_msg(
              ~c"** Illegal boot server connection attempt: client version is ~s ** ~n",
              [vsn]
            )

          _ ->
            :error_logger.error_msg(
              ~c"** Illegal boot server connection attempt: unrecognizable request ** ~n",
              []
            )
        end
  ...
  defp send_result(s, term) do
    case :gen_tcp.send(s, :erlang.term_to_binary(term)) do
      ...
      error ->
        :error_logger.error_msg(~c"** Boot server could not send result to socket: ~w** ~n", [
          error
        ])
