defmodule :m_logger_filters do
  use Bitwise
  def domain(%{meta: meta} = logEvent,
           {action, compare, matchDomain})
      when (action == :log or action == :stop) and (compare == :super or compare == :sub or compare == :equal or compare == :not_equal or compare == :undefined) and is_list(matchDomain) do
    filter_domain(compare, meta, matchDomain,
  ...
  def level(%{level: l1} = logEvent, {action, op, l2})
      when (action == :log or action == :stop) and (op == :neq or op == :eq or op == :lt or op == :gt or op == :lteq or op == :gteq) and (l2 === :emergency or l2 === :alert or l2 === :critical or l2 === :error or l2 === :warning or l2 === :notice or l2 === :info or l2 === :debug) do
  ...
  defp filter_domain(:super, %{domain: domain}, matchDomain,
            onMatch) do
    is_prefix(domain, matchDomain, onMatch)
  end

  defp filter_domain(:sub, %{domain: domain}, matchDomain,
            onMatch) do
  ...
  defp filter_domain(:not_equal, %{domain: domain}, matchDomain,
            onMatch)
  ...
  defp filter_domain(compare, meta, _, onMatch) do
    case (:maps.is_key(:domain, meta)) do
      false when compare == :undefined or
                   compare == :not_equal
                 ->
        onMatch
      _ ->
  ...
  defp is_prefix(d1, d2, onMatch) when (is_list(d1) and
                                   is_list(d2)) do
  ...
  defp filter_progress(%{msg: {:report, %{label: {_, :progress}}}},
            onMatch) do
    onMatch
  end
