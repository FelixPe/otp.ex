defmodule :m_global_group do
  ...
  require Record
  Record.defrecord(:r_state, :state, sync_state: :no_conf,
                                 group_name: [], nodes: %{}, other_grps: [],
                                 monitor: [], group_publish_type: :normal,
                                 connections: :undefined,
                                 erpc_requests: :undefined,
                                 config_check: :undefined)
  Record.defrecord(:r_gconf, :gconf, parameter_value: :invalid,
                                 node_name: :undefined, group_name: [],
                                 group_publish_type: :normal, group_list: [],
                                 group_map: :all, other_groups: [],
                                 state: :no_conf)
  ...
  def start() do
    :gen_server.start({:local, :global_group},
                        :global_group, [], [])
  end

  def start_link() do
    :gen_server.start_link({:local, :global_group},
                             :global_group, [], [])
  end
  ...
  def init([]) do
    ...
    conns = :lists.foldl(fn {n, %{connection_id: cId}},
                              cs ->
                              Map.put(cs, n, cId)
                         end,
                           %{}, nodes(:visible, %{connection_id: true}))
    s = initial_group_setup(fetch_new_group_conf(true,
                                                   node()),
                              conns, :erpc.reqids_new())
    {:ok, s}
  end
  ...
  defp initial_group_setup(r_gconf(state: {:error, _Err, nodeGrps}), _Conns,
            _Reqs) do
    exit({:error, {:"invalid global_groups definition", nodeGrps}})
  end

  defp initial_group_setup(r_gconf(node_name: nodeName, group_name: defGroupName,
              group_list: defNodesT, group_publish_type: pubTpGrp,
    ...
    newReqs = schedule_conf_changed_checks(defNodes, reqs,
                                             conns)
    nodes0 = :lists.foldl(fn node, acc ->
                               Map.put(acc, node, :sync_error)
                          end,
                            %{}, connectedOwnNodes)
    nodes = :lists.foldl(fn node, acc ->
                              Map.put(acc, node, :no_contact)
                         end,
                           nodes0, notConnectedOwnNodes)
    r_state(group_publish_type: pubTpGrp, sync_state: :synced,
        group_name: defGroupName, nodes: nodes,
        other_grps: defOther, connections: conns,
        erpc_requests: newReqs)
  end

  def handle_call(:sync, _From,
           r_state(nodes: oldNodes, connections: conns) = s) do
    case (lookup_group_conf(true)) do
      r_gconf(state: :no_conf, group_name: defGroupName,
          group_list: _DefNodesT, group_publish_type: pubTpGrp,
          other_groups: defOther) ->
        {:reply, :ok,
           r_state(s, sync_state: :no_conf,  group_name: defGroupName, 
                  nodes: %{},  group_publish_type: pubTpGrp, 
                  other_grps: defOther)}
      r_gconf(state: {:error, _Err, nodeGrps}) ->
        exit({:error, {:"invalid global_groups definition", nodeGrps}})
      r_gconf(group_name: defGroupName, group_list: defNodesT,
          group_publish_type: pubTpGrp, other_groups: defOther) ->
        defNodes = :lists.delete(node(), defNodesT)
        disconnect_nodes(:erlang.nodes() -- defNodes, conns)
        syncSession = make_ref()
        cCMsg = {:conf_check, 3, node(), {self(), syncSession},
                   :sync, defGroupName, pubTpGrp, defNodesT}
        {newNodes, mons} = :lists.foldl(fn n, {nacc, macc} ->
                                             gG = {:global_group, n}
                                             m = :erlang.monitor(:process, gG)
                                             :gen_server.cast(gG, cCMsg)
                                             nS = :maps.get(n, oldNodes,
                                                              :no_contact)
                                             {Map.put(nacc, n, nS),
                                                Map.put(macc, n, m)}
                                        end,
                                          {%{}, %{}}, defNodes)
        {:reply, :ok,
           r_state(s, sync_state: :synced,  group_name: defGroupName, 
  ...
  def handle_call(:global_groups, _From, s) do
    result = (case (r_state(s, :sync_state)) do
                :no_conf ->
                  :undefined
                :synced ->
                  other = :lists.foldl(fn {n, _L}, acc ->
                                            acc ++ [n]
                                       end,
  ...
  def handle_call(:own_nodes, _From, s) do
    nodes = (case (r_state(s, :sync_state)) do
               :no_conf ->
                 [node() | :erlang.nodes()]
               :synced ->
                 get_own_nodes(true)
  ...
  def handle_call({:whereis_name, name}, from, s) do
    case (:global.whereis_name(name)) do
      :undefined ->
        pid = :global_search.start(:whereis,
  ...
  def handle_call({:whereis_name, {:group, group}, name}, from,
           s) do
    case (:lists.keysearch(group, 1, r_state(s, :other_grps))) do
      false ->
        {:reply, :undefined, s}
  ...
  def handle_call({:global_groups_changed, newPara}, _From,
           r_state(erpc_requests: reqs, nodes: oldNodes,
               connections: conns) = s) do
    r_gconf(group_name: newGroupName,
        group_publish_type: pubTpGrp, group_list: newNodesListT,
    ...
    force_nodedown(:erlang.nodes(:connected) -- newNodesList,
                     conns)
    newNodes = :lists.foldl(fn n, nacc ->
                                 nS = :maps.get(n, oldNodes, :no_contact)
                                 Map.put(nacc, n, nS)
                            end,
                              %{}, newNodesList)
    newReqs = schedule_conf_changed_checks(newNodesList,
                                             reqs, conns)
    newS = r_state(s, group_name: newGroupName,  nodes: newNodes, 
                  other_grps: newOther,  group_publish_type: pubTpGrp, 
                  erpc_requests: newReqs,  config_check: :undefined)
    {:reply, :ok, newS}
  end

  def handle_call({:global_groups_added, newPara}, _From,
           r_state(connections: conns, erpc_requests: reqs) = s) do
    r_gconf(group_name: newGroupName,
        group_publish_type: pubTpGrp, group_list: newNodesList,
        other_groups: newOther,
        state: gState) = new_group_conf(true, newPara)
    case (gState) do
      :no_conf ->
    ...
    {newReqs, newNodes} = :lists.foldl(fn n, {racc, nacc} ->
                                            cId = :maps.get(n, conns,
                                                              :not_connected)
                                            nRacc = :erpc.send_request(n,
                                                                         :global_group,
                                                                         :ng_add_check,
                                                                         nGACArgs,
                                                                         {:ng_add_check,
    ...
    newS = r_state(s, sync_state: :synced, 
                  group_name: newGroupName,  nodes: newNodes, 
                  erpc_requests: newReqs,  other_grps: newOther, 
                  group_publish_type: pubTpGrp,  config_check: :undefined)
    {:reply, :ok, newS}
  end

  def handle_call({:global_groups_removed, _NewPara}, _From, s) do
    r_gconf(group_name: newGroupName,
        group_publish_type: pubTpGrp, group_list: _NewNodes,
        other_groups: newOther,
        state: :no_conf) = new_group_conf(true, :undefined)
    newS = r_state(s, sync_state: :no_conf, 
                  group_name: newGroupName,  nodes: %{}, 
                  other_grps: newOther,  group_publish_type: pubTpGrp, 
                  config_check: :undefined)
    {:reply, :ok, newS}
  end

  def handle_call({:ng_add_check, node, pubType, othersNG}, _From,
           r_state(group_publish_type: ownPubType) = s) do
  ...
  def handle_call(:info, _From, s) do
    {inSync, syncError, noContact} = :maps.fold(fn n, :sync,
                                                     {iSacc, sEacc, nCacc} ->
                                                     {[n | iSacc], sEacc, nCacc}
                                                   n, :sync_error,
                                                     {iSacc, sEacc, nCacc} ->
                                                  ...
                                                  {[], [], []}, r_state(s, :nodes))
    reply = [{:state, r_state(s, :sync_state)}, {:own_group_name,
                                             r_state(s, :group_name)},
                                              {:own_group_nodes,
                                                 get_own_nodes(true)},
                                                  {:synced_nodes,
                                                     :lists.sort(inSync)},
                                                      {:sync_error,
                                                         :lists.sort(syncError)},
                                                          {:no_contact,
                                                             :lists.sort(noContact)},
                                                              {:other_groups,
                                                                 r_state(s, :other_grps)},
                                                                  {:monitoring,
                                                                     r_state(s, :monitor)}]
  ...
  def handle_call({:registered_names_test, {:node, :test3844zty}},
           from, s) do
    pid = :global_search.start(:names_test,
                                 {:node, :test3844zty})
    wait = :erlang.get(:registered_names)
    :erlang.put(:registered_names, [{pid, from} | wait])
    {:noreply, s}
  ...
  def handle_call({:registered_names_test, {:node, _Node}}, _From,
           s) do
  ...
  def handle_cast({:conf_check, vsn, node, from, :sync, cCName,
            cCNodes},
           s) do
    handle_cast({:conf_check, vsn, node, from, :sync,
                   cCName, :normal, cCNodes},
                  s)
  end

  def handle_cast({:conf_check, vsn, node, from, :sync, cCName,
            pubType, cCNodes},
           r_state(connections: conns) = s) do
    try do
      cId = (case (:maps.get(node, conns, :undefined)) do
               :undefined ->
                 throw({:noreply, s})
               cId0 ->
                 cId0
      ...
      case (lookup_group_conf(true)) do
        ...
        r_gconf(group_name: ^cCName, group_list: ^cCNodes,
      ...
      class, {reason, label, newRequests} ->
        {:noreply,
           handle_erpc_response(class, reason, label,
                                  r_state(s, erpc_requests: newRequests))}
    else
      noMatch when noMatch == :no_request or
                     noMatch == :no_response
                   ->
  ...
  defp continue_handle_info({:nodeup, node, %{connection_id: :undefined}},
            r_state(connections: conns, erpc_requests: reqs)) do
    s = initial_group_setup(alive_state_change_group_conf(node),
                              conns, reqs)
    send_monitor(r_state(s, :monitor), {:nodeup, node},
                   r_state(s, :sync_state))
    {:noreply, s}
  end

  defp continue_handle_info({:nodeup, node, %{connection_id: cId}},
            r_state(sync_state: :no_conf, connections: conns) = s) do
    send_monitor(r_state(s, :monitor), {:nodeup, node},
                   r_state(s, :sync_state))
  ...
  defp continue_handle_info({:nodeup, node, %{connection_id: cId}},
            r_state(erpc_requests: reqs, connections: conns) = s) do
    ...
    case (member(true, node)) do
      ...
      true ->
        newReqs = :erpc.send_request(node, :global_group,
                                       :get_own_nodes, [],
                                       {:nodeup_conf_check, node, cId}, reqs)
        {:noreply,
           node_state(:sync_error, node,
                        r_state(s, erpc_requests: newReqs,  connections: newConns))}
    end
  end

  defp continue_handle_info({:nodedown, node, %{connection_id: :undefined}},
            s) do
    r_gconf(state: :no_conf, group_name: defGroupName,
        group_list: _DefNodes, group_publish_type: pubTpGrp,
        other_groups: defOther) = alive_state_change_group_conf(:nonode@nohost)
    send_monitor(r_state(s, :monitor), {:nodedown, node},
                   :no_conf)
    {:noreply,
       r_state(s, group_publish_type: pubTpGrp, 
              sync_state: :no_conf,  group_name: defGroupName, 
              nodes: %{},  other_grps: defOther, 
              config_check: :undefined)}
  end

  defp continue_handle_info({:nodedown, node, _Info},
            r_state(sync_state: :no_conf, monitor: monitor,
                connections: conns) = s) do
  ...
  defp continue_handle_info({:nodedown, node, _Info},
            r_state(sync_state: syncState, monitor: monitor,
                connections: conns) = s) do
    send_monitor(monitor, {:nodedown, node}, syncState)
    {:noreply,
       node_state(:no_contact, node,
                    r_state(s, connections: :maps.remove(node, conns)))}
  end

  defp continue_handle_info({:disconnect_node, node},
            r_state(monitor: monitor, sync_state: syncState, nodes: nodes,
                connections: conns) = s) do
    case ({syncState,
             :maps.get(node, nodes, :not_member)}) do
      {:synced, :sync} ->
        send_monitor(monitor, {:nodedown, node}, syncState)
      _ ->
        :ok
  ...
  defp continue_handle_info({:config_ok, 3, {pid, cCSession}, node},
            r_state(config_check: {cCSession, mons},
                connections: conns) = s0)
      when pid == self() do
    try do
      {mon, newMons} = (case (:maps.take(node, mons)) do
                          :error ->
                            throw({:noreply, s0})
                          monTake ->
                            monTake
  ...
  defp continue_handle_info({:config_error, 3, {pid, cCSession}, node},
            r_state(config_check: {cCSession, mons},
                connections: conns) = s0)
      when pid == self() do
    try do
  ...
  defp continue_handle_info({:DOWN, mon, :process, {:global_group, node},
             reason},
            r_state(config_check: {cCSession, mons},
                connections: conns) = s0) do
    try do
      newMons = (case (:maps.take(node, mons)) do
                   {^mon, newMons0} ->
                     newMons0
                   _ ->
                     throw({:noreply, s0})
  ...
  defp log_sync_error(node) do
    txt = :io_lib.format('global_group: Could not synchronize with node ~p~nbecause global_groups parameter were not in agreement.~n', [node])
    :error_logger.error_report(txt)
    :ok
  end

  defp schedule_conf_changed_checks(nodes, requests, connections) do
    :lists.foldl(fn node, requestsAcc ->
  ...
  defp handle_erpc_response(:ok, nodes, {:nodeup_conf_check, node, reqCId},
  ...
  defp handle_erpc_response(:error, {:erpc, :noconnection},
  ...
  defp handle_erpc_response(_, _, {:nodeup_conf_check, node, reqCId},
            r_state(connections: conns) = s) do
    case (:maps.get(node, conns, :undefined)) do
      cId
          when reqCId == cId or reqCId == :not_connected and is_integer(cId)
  ...
  defp handle_erpc_response(:ok, nodes, {:conf_changed_check, node, reqCId},
            r_state(connections: conns) = s)
      when is_list(nodes) do
    case (:maps.get(node, conns, :undefined)) do
      cId
  ...
  defp handle_erpc_response(_, _, {:conf_changed_check, node, reqCId},
            r_state(connections: conns) = s) do
    case (:maps.get(node, conns, :undefined)) do
      cId
          when reqCId == cId or reqCId == :not_connected and is_integer(cId)
  ...
  defp handle_erpc_response(:ok, :agreed, {:ng_add_check, node, reqCId},
            r_state(connections: conns) = s) do
    case (:maps.get(node, conns, :undefined)) do
      cId
          when reqCId == cId or reqCId == :not_connected and is_integer(cId)
  ...
  defp handle_erpc_response(_, _, {:ng_add_check, node, reqCId},
            r_state(connections: conns) = s) do
    case (:maps.get(node, conns, :undefined)) do
      cId
          when reqCId == cId or reqCId == :not_connected and is_integer(cId)
  ...
  defp node_state(:remove, node, r_state(nodes: ns) = s) do
    case (member(true, node)) do
      true ->
        :erlang.error({:removing_node_state_of_member_node,
                         node})
      false ->
        r_state(s, nodes: :maps.remove(node, ns))
    end
  ...
  defp config_scan(_MyNode, pubType, [], own_name, ownNodes,
            otherNodeGrps) do
    {own_name, pubType, :lists.sort(ownNodes),
       :lists.reverse(otherNodeGrps)}
  end

  defp config_scan(myNode, pubType, [grpTuple | nodeGrps],
            own_name, ownNodes, otherNodeGrps) do
    {name, pubTypeGroup, nodes} = grp_tuple(grpTuple)
    case (:lists.member(myNode, nodes)) do
      true ->
        case (own_name) do
          :no_name ->
            config_scan(myNode, pubTypeGroup, nodeGrps, name, nodes,
                          otherNodeGrps)
          _ ->
            {:error, {:"node defined twice", {own_name, name}}}
        end
      false ->
        config_scan(myNode, pubType, nodeGrps, own_name,
                      ownNodes, [{name, nodes} | otherNodeGrps])
    end
  end
  ...
  defp fetch_new_group_conf(gG, nodeName) do
    gGConf = (case (:application.get_env(:kernel,
                                           :global_groups)) do
                :undefined ->
                  :undefined
                {:ok, v} ->
  ...
  defp new_group_conf(gG, kernParamValue, nodeName) do
    case (:persistent_term.get(:global_group, r_gconf())) do
      r_gconf(parameter_value: ^kernParamValue,
          node_name: name) = gConf
  ...
  defp check_exit(exitPid, reason) do
    check_exit_reg(:erlang.get(:registered_names), exitPid,
                     reason)
    check_exit_send(:erlang.get(:send), exitPid, reason)
    check_exit_where(:erlang.get(:whereis_name), exitPid,
                       reason)
  ...
  defp check_exit_reg(reg, exitPid, reason) do
    case (:lists.keysearch(exitPid, 1,
                             :lists.delete(:undefined, reg))) do
      {:value, {^exitPid, from}} ->
  ...
  defp check_exit_send(send, exitPid, _Reason) do
    case (:lists.keysearch(exitPid, 1,
                             :lists.delete(:undefined, send))) do
      {:value, {^exitPid, from, name, msg}} ->
        newSend = :lists.delete({exitPid, from, name, msg},
                                  send)
  ...
  defp check_exit_where(where, exitPid, reason) do
    case (:lists.keysearch(exitPid, 1,
                             :lists.delete(:undefined, where))) do
      {:value, {^exitPid, from}} ->
