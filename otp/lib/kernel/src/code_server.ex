defmodule :m_code_server do
  ...
  defp get_user_lib_dirs_1([dir | dirList]) do
    case :erl_prim_loader.list_dir(dir) do
      {:ok, dirs} ->
        ...
        for p <- paths, :filename.basename(p) === ~c"ebin" do
          p
  ...
  defp handle_call(
         {:add_path, where, dir0, cache},
         _From,
         r_state(namedb: namedb, path: path0) = s
       ) do
  ...
  defp handle_call({:del_paths, names}, _From, r_state(path: path0, namedb: namedb) = s) do
  ...
  defp handle_call(
         ...
         _From,
         r_state(path: path0, namedb: namedb) = s
       ) do
  ...
  defp handle_call({:load_module, pC, mod, file, purge, ensureLoaded}, from, s)
       when is_atom(mod) do
  ...
  defp handle_call(other, _From, s) do
    error_msg(~c" ** Codeserver*** ignoring ~w~n ", [other])
    {:noreply, s}
  end
  ...
  defp make_path(bundleDir, [bundle | tail], res) do
    ...
    ebin = :filename.append(dir, ~c"ebin")
    ...
    case is_dir(ebin) do
      ...
      false ->
        ...
        ebin2 = :filename.join([bundleDir, base ++ ext, base, ~c"ebin"])

        ebins =
          case split_base(base) do
            {appName, _} ->
              ebin3 = :filename.join([bundleDir, base ++ ext, appName, ~c"ebin"])
              [ebin3, ebin2, dir]
  ...
  defp merge_path(path, [~c"." | iPath], acc) do
    rPath = merge_path1(path, iPath, acc)
    [~c"." | :lists.delete(~c".", rPath)]
  end
  ...
  defp get_name_from_splitted([dirName, ~c"ebin"]) do
  ...
  defp discard_after_hyphen(~c"-" ++ _) do
    []
  end
  ...
  defp split_base(baseName) do
    case split(baseName, ~c"-") do
      [_, _ | _] = toks ->
        vsn = :lists.last(toks)
        allButLast = :lists.droplast(toks)
        {join(allButLast, ~c"-"), vsn}
    ...
    case is_dir(dir) do
      ...
      false when pathChoice === :relaxed ->
        case (try do
          ...
          [~c"ebin", app] ->
            dir2 = :filename.join([app ++ archiveExt, app, ~c"ebin"])
            ...
            dir2 =
              cond do
                ...
                true ->
                  ...
                  :filename.join(top ++ [app ++ archiveExt, app, ~c"ebin"])
              end
  ...
  defp add_path(where, dir, path, cache, nameDb)
       when is_atom(dir) do
    add_path(where, :erlang.atom_to_list(dir), path, cache, nameDb)
  end
  ...
  defp create_namedb(path, root) do
    ...
    case lookup_name(~c"erts", db) do
      {:ok, _, _, _} ->
        :ok

      false ->
        ertsDir = :filename.join(root, ~c"erts")
        ...
        case :erl_prim_loader.read_file_info(ertsDir) do
          ...
          _ ->
            do_insert_name(~c"erts", ertsDir, db)
        end
    end
  ...
  defp insert_dir(dir, db) do
    ...
    case get_name_from_splitted(splitted) do
      name when name != ~c"ebin" and name != ~c"." ->
        name
  ...
  defp replace_path1(name, dir, [{p, _} = pair | path], cache, nameDb) do
    case get_name(p) do
      ^name ->
  ...
  defp del_ebin_1([parent, app, ~c"ebin"]) do
  ...
  defp del_ebin_1(path = [_App, ~c"ebin"]) do
    del_ebin_1(:filename.split(absname(:filename.join(path))))
  end

  defp del_ebin_1([~c"ebin"]) do
    del_ebin_1(:filename.split(absname(~c"ebin")))
  end
  ...
  defp do_dir(_Root, :compiler_dir, nameDb) do
    case lookup_name(~c"compiler", nameDb) do
  ...
  defp try_finish_module_1(file, mod, pC, from, ensureLoaded, r_state(moddb: db) = st) do
    case is_sticky(mod, db) do
      true ->
        error_msg(~c"Can't load module '~w' that resides in sticky dir\n", [mod])
  ...
  defp try_finish_module_2(file, mod, pC, from, ensureLoaded, st0) do
    action = fn result, r_state(moddb: db) = st1 ->
      case result do
        ...
        {:error, what} ->
          error_msg(~c"Loading of ~ts failed: ~p\n", [file, what])
      end
    ...
    res =
      case :erlang.finish_loading([pC]) do
        :ok ->
          {:module, mod}
  ...
  defp get_object_code(r_state(path: path) = st, mod) when is_atom(mod) do
    ...
    case :erl_prim_loader.is_basename(modStr) do
      true ->
        case mod_to_bin(path, modStr ++ objfile_extension(), []) do
          {binary, file, newPath} ->
  ...
  defp monitor_loader(r_state(loading: loading0) = st, mod, pid, bin, fName) do
  ...
  defp finish_on_load_report(mod, term) do
    spawn(fn ->
      f = ~c"The on_load function for module ~s returned:~n~P\n"
      e = :error_logger
