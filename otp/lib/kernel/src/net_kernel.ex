defmodule :m_net_kernel do
  ...
  Record.defrecord(:r_tick, :tick, ticker: :undefined, time: :undefined, intensity: :undefined)
  ...
  def publish_on_node(node) when is_atom(node) do
    :global_group.publish(
      :persistent_term.get(
        {:net_kernel, :publish_type},
        :hidden
      ),
      node
    )
  ...
  def start([name, nameDomain, tickTime])
    ...
    start(
      name,
      %{name_domain: nameDomain, net_ticktime: netTickTime, net_tickintensity: 4}
    )
  end

  def start_link(startOpts) do
    case :gen_server.start_link({:local, :net_kernel}, :net_kernel, make_init_opts(startOpts), []) do
      {:ok, pid} ->
        {:ok, pid}
  ...
  defp make_init_opts(opts) do
    nTT1 =
      case :maps.find(:net_ticktime, opts) do
        {:ok, nTT0} ->
          nTT0 * 1000
    ...
    nTI =
      case :maps.find(:net_tickintensity, opts) do
        {:ok, nTI0} ->
          nTI0
    ...
    nD =
      case :maps.find(:name_domain, opts) do
        {:ok, nD0} ->
          nD0
    ...
    dL =
      case split_node(:maps.get(:name, opts)) do
        {~c"undefined", _} ->
          false
  ...
  def init(%{
    ...
    case init_node(name, nameDomain, cleanHalt, distListen) do
      {:ok, node, listeners} ->
        ...
        {:ok,
         r_state(
           node: node,
           type: nameDomain,
           tick: r_tick(ticker: ticker, time: netTicktime, intensity: netTickIntensity),
  ...
  defp do_auto_connect_2(node, :passive_cnct, from, state, connLookup) do
    try do
      ...
      _, _ ->
        :error_logger.error_msg(~c"~n** Cannot get connection id for node ~w~n", [node])
        {:reply, false, state}
    else
      connId ->
        do_auto_connect_2(node, connId, from, state, connLookup)
  ...
  def handle_call({:apply, _Mod, _Fun, _Args}, {pid, _Tag} = from, state)
  ...
  def handle_call(
        {:new_ticktime, t, tP},
        from,
        r_state(tick: r_tick(ticker: tckr, time: oT, intensity: i)) = state
      ) do
    :ok
      ...
      false ->
        ...
        async_reply(
          {:reply, :change_initiated,
           r_state(state, tick: r_tick_change(ticker: tckr, time: nT, intensity: i, how: how))},
          from
        )
    end
  end
  ...
  def handle_call(:get_state, from, state) do
    ...
    {nameType, name} =
      case {:erts_internal.dynamic_node_name(), node()} do
        {false, node} ->
          {:static, node}
    ...
    return = %{started: started, name_type: nameType, name: name, name_domain: nameDomain}
  ...
  def handle_info(
        {:accept, acceptPid, socket, family, proto} = accept,
        state
      ) do
  ...
  def handle_info(
    ...
    case :ets.lookup(:sys_dist, node) do
      ...
      _ ->
        error_msg(~c"Net kernel got ~tw~n", [msg])
        {:noreply, state}
    end
  end

  def handle_info(
        {setupPid, {:nodeup, node, address, type, namedMe} = nodeup},
        r_state(tick: tick) = state
      ) do
    case :ets.lookup(:sys_dist, node) do
      [conn]
      when r_connection(conn, :state) === :pending and r_connection(conn, :owner) === setupPid and
        ...
        verbose({:nodeup, node, setupPid, r_connection(conn, :ctrlr)}, 2, state1)
        {:noreply, state1}
        ...
        cond do
          ...
          true ->
            ...
            case :maps.is_key(oldOwner, r_state(state, :conn_owners)) do
              ...
              false ->
                verbose({:accept_pending, oldOwner, :inconsistency}, 2, state)
                :ok
            end
  ...
  def handle_info({acceptPid, {:wait_pending, node}}, state) do
    case get_conn(node) do
      {:ok, r_connection(state: :up_pending, ctrlr: oldCtrlr, pending_owner: ^acceptPid)} ->
        :ok
  ...
  def handle_info({from, :badcookie, _To, _Mess}, state) do
    :error_logger.error_msg(~c"~n** Got OLD cookie from ~w~n", [getnode(from)])
  ...
  def handle_info(
        ...
        r_state(tick: r_tick_change(ticker: tckr, time: t, intensity: i, how: how)) = state
      ) do
    :ok
    ...
    {:noreply, r_state(state, tick: r_tick(ticker: tckr, time: t, intensity: i))}
  end

  def handle_info(x, state) do
    error_msg(~c"Net kernel got ~tw~n", [x])
    {:noreply, state}
  end
  ...
  defp ensure_node_name(host, state0) when is_list(host) do
    case :string.split(host, ~c"@", :all) do
      [^host] ->
  ...
  defp generate_node_name(host, state0) do
    ...
    case :maps.get(host, namePool, []) do
      [] ->
        ...
        {:erlang.list_to_atom(name ++ ~c"@" ++ host), create_creation(), state0}
  ...
  defp listen_exit(pid, reason, state) do
    case :lists.keymember(pid, r_listen(:listen), r_state(state, :listen)) do
      true ->
        ...
        error_msg(~c"** Netkernel terminating ... **\n", [])
      ...
      node ->
        ...
        case get_conn(node) do
          {:ok, conn} when r_connection(conn, :state) === :up_pending ->
            verbose({:pending_own_exit, pid, reason, node, :up_pending}, 2, state)
            reply_waiting(node, r_connection(conn, :waiting), true)
  ...
  defp ticker_exit(pid, reason, r_state(tick: r_tick(ticker: pid, time: t) = tck) = state) do
  ...
  defp ticker_exit(
    ...
    tckr = restart_ticker(t)
    throw({:noreply, reason, r_state(state, tick: r_tick_change(tckCng, ticker: tckr))})
  end
  ...
  defp restarter_exit(pid, reason, state) do
    case r_state(state, :supervisor) do
      {:restart, ^pid} ->
        ...
        error_msg(~c"** Distribution restart failed, net_kernel terminating... **\n", [])
  ...
  defp delete_connection(r_connection(node: node) = conn, mayBeBarred, state) do
    ...
    case r_connection(conn, :remote_name_type) do
      :dynamic ->
        [_Name, host] = :string.split(:erlang.atom_to_list(node), ~c"@", :all)
  ...
  defp check_options(opts) when is_map(opts) do
    badMap0 =
      case :maps.find(:connection_id, opts) do
        :error ->
          opts
        ...
        {:ok, badCIdVal} ->
          throw({:error, {:bad_option_value, %{connection_id: badCIdVal}}})
      end
    ...
    badMap1 =
      case :maps.find(
        ...
        {:ok, badNRVal} ->
          throw({:error, {:bad_option_value, %{nodedown_reason: badNRVal}}})
      end

    badMap2 =
      case :maps.find(:node_type, badMap1) do
        :error ->
          badMap1

        {:ok, nTVal}
        when nTVal == :visible or
               nTVal == :hidden or nTVal == :all ->
          :maps.remove(:node_type, badMap1)

        {:ok, badNTVal} ->
          throw({:error, {:bad_option_value, %{node_type: badNTVal}}})
      end
  ...
  defp setup(node, connId, type, from, state) do
    case setup_check(node, state) do
      {:ok, l} ->
        ...
        verbose({:setup, node, type, myNode, r_state(state, :type), pid}, 2, state)
  ...
  defp setup_check(node, state) do
    ...
    case :lists.member(node, allowed) do
      false when allowed !== [] ->
        error_msg(~c"** Connection attempt with disallowed node ~w ** ~n", [node])
        {:error, :bad_node}
    ...
    case host1 do
      {:ok, hostPart} ->
        case valid_name_head(head) do
          ...
          false ->
            :error_logger.info_msg(~c"Invalid node name!\nPlease check your configuration\n")
            {:error, :badarg}
        end
      ...
      {:error, type} ->
        :error_logger.info_msg(
          :lists.concat([~c"Can't set ", type, ~c" node name!\nPlease check your configuration\n"])
        )
  ...
  defp valid_name_head(head) do
    {:ok, mP} = :re.compile(~c"^[0-9A-Za-z_\\-]+$", [:unicode])
  ...
  def protocol_childspecs() do
    case :init.get_argument(:proto_dist) do
      ...
      _ ->
        protocol_childspecs([~c"inet_tcp"])
    end
  end
  ...
  defp protocol_childspecs([h | t]) do
    mod = :erlang.list_to_atom(h ++ ~c"_dist")
  ...
  defp start_protos(node, cleanHalt, listen) do
    case :init.get_argument(:proto_dist) do
      ...
      _ ->
        start_protos(node, [~c"inet_tcp"], cleanHalt, listen)
    end
  end
  ...
  defp start_protos_no_listen(node, [proto | ps], ls, cleanHalt) do
    ...
    case ok do
      true ->
        :auth.sync_cookie()
        mod = :erlang.list_to_atom(proto ++ ~c"_dist")
        ...
        l = r_listen(listen: :undefined, address: address, accept: :undefined, module: mod)
        start_protos_no_listen(node, ps, [l | ls], cleanHalt)

      false ->
        s = ~c"invalid node name: " ++ :erlang.atom_to_list(node)
  ...
  defp start_protos_listen(node, ps, cleanHalt) do
    case split_node(node) do
      {~c"undefined", _} ->
        :erlang.error({:internal_error, ~c"Dynamic node name and dist listen both enabled"})
  ...
  defp start_protos_listen(name, host, node, [proto | ps], ls, cleanHalt) do
    ...
    try do
      ...
      :error, :undef ->
        proto_error(cleanHalt, proto, ~c"not supported")
      ...
      {:error, :duplicate_name} ->
        s =
          ~c"the name " ++
            :erlang.atom_to_list(node) ++ ~c" seems to be in use by another Erlang node"
  ...
  defp print_info() do
    nformat(~c"Node", ~c"State", ~c"Type", ~c"In", ~c"Out", ~c"Address")
    ...
    case r_net_address(a, :family) do
      :inet ->
        case r_net_address(a, :address) do
          {iP, port} ->
            :inet_parse.ntoa(iP) ++ ~c":" ++ :erlang.integer_to_list(port)
      ...
      _ ->
        :lists.flatten(:io_lib.format(~c"~p", [r_net_address(a, :address)]))
    end
  end
  ...
  defp nformat(a1, a2, a3, a4, a5, a6) do
    :io.format(~c"~-20s ~-7s ~-6s ~8s ~8s ~s~n", [a1, a2, a3, a4, a5, a6])
  end
  ...
  defp print_info(node) do
    case node_info(node) do
      {:ok, info} ->
        ...
        :io.format(
          ~c"Node     = ~p~nState    = ~p~nType     = ~p~nIn       = ~p~nOut      = ~p~nAddress  = ~s~n",
  ...
  defp handle_async_response(responseType, reqId, result, r_state(req_map: reqMap0) = s0) do
    cond do
      responseType == :down ->
        :ok
    ...
    case :maps.take(reqId, reqMap0) do
      ...
      {{:setopts_new, op}, reqMap1} ->
        case :maps.get(op, reqMap1) do
          ...
          {:setopts_new, from, n} ->
            reqMap2 = Map.put(reqMap1, op, {:setopts_new, from, n - 1})
            {:noreply, r_state(s0, req_map: reqMap2)}
        end
    end
  end
  ...
  defp setopts_new_1(opts, from, r_state(req_map: reqMap0) = state) do
    ...
    sendReq = fn reqMap, n, owner ->
      {send_owner_request(reqMap, {:setopts_new, op}, owner, {:setopts, opts}), n + 1}
    end
    ...
    cond do
      ...
      true ->
        reqMap2 = Map.put(reqMap1, op, {:setopts_new, from, noReqs})
  ...
  defp opts_node(op, node, opts, from, r_state(req_map: reqMap0) = s0) do
    case :ets.lookup(:sys_dist, node) do
      [conn] when r_connection(conn, :state) === :up ->
        reqMap1 = send_owner_request(reqMap0, {op, from}, r_connection(conn, :owner), {op, opts})
