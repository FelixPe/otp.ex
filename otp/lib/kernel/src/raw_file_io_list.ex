defmodule :m_raw_file_io_list do
  ...
  def advise(fd, offset, length, advise) do
    privateFd = r_file_descriptor(fd, :data)
    apply(r_file_descriptor(privateFd, :module), :advise,
            [privateFd, offset, length, advise])
  end

  def allocate(fd, offset, length) do
    privateFd = r_file_descriptor(fd, :data)
    apply(r_file_descriptor(privateFd, :module), :allocate,
            [privateFd, offset, length])
  end

  def position(fd, mark) do
    privateFd = r_file_descriptor(fd, :data)
    apply(r_file_descriptor(privateFd, :module), :position,
            [privateFd, mark])
  end
  ...
  def pwrite(fd, offset, iOData) do
    privateFd = r_file_descriptor(fd, :data)
    apply(r_file_descriptor(privateFd, :module), :pwrite,
            [privateFd, offset, iOData])
  end
  ...
  def read_line(fd) do
    privateFd = r_file_descriptor(fd, :data)
    case (apply(r_file_descriptor(privateFd, :module), :read_line,
                  [privateFd])) do
      {:ok, binary} ->
  ...
  def pread(fd, offset, size) do
    privateFd = r_file_descriptor(fd, :data)
    case (apply(r_file_descriptor(privateFd, :module), :pread,
                  [privateFd, offset, size])) do
      {:ok, binary} ->
        {:ok, :erlang.binary_to_list(binary)}
      other ->
        other
  ...
  def pread(fd, locNums) do
    privateFd = r_file_descriptor(fd, :data)
    case (apply(r_file_descriptor(privateFd, :module), :pread,
                  [privateFd, locNums])) do
      {:ok, locResults} ->
        translatedResults = (for result <- locResults do
                               case (result) do
  ...
  def sendfile(fd, dest, offset, bytes, chunkSize, headers,
           trailers, flags) do
    args = [dest, offset, bytes, chunkSize, headers,
                                                trailers, flags]
    privateFd = r_file_descriptor(fd, :data)
    apply(r_file_descriptor(privateFd, :module), :sendfile,
            [privateFd | args])
  end

  def internal_get_nif_resource(fd) do
    args = []
    privateFd = r_file_descriptor(fd, :data)
    apply(r_file_descriptor(privateFd, :module), :internal_get_nif_resource,
            [privateFd | args])
  end

  def read_handle_info(fd, opts) do
    privateFd = r_file_descriptor(fd, :data)
    apply(r_file_descriptor(privateFd, :module), :read_handle_info,
            [privateFd, opts])
  end
