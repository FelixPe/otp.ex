defmodule :m_global do
  ...
  require Record
  Record.defrecord(:r_conf, :conf, connect_all: :undefined,
                                prevent_over_part: :undefined)
  Record.defrecord(:r_state, :state, conf: :EFE_TODO_NESTED_RECORD,
                                 known: %{}, synced: [], resolvers: [],
                                 syncers: [], node_name: node(),
                                 the_locker: :undefined,
                                 the_registrar: :undefined, trace: :undefined,
                                 global_lock_down: false)
  def start() do
    :gen_server.start({:local, :global_name_server},
                        :global, [], [])
  end

  def start_link() do
    :gen_server.start_link({:local, :global_name_server},
                             :global, [], [])
  end
  ...
  def register_name(name, pid, method0) when is_pid(pid) do
    ...
    fun = fn nodes ->
               case (where(name) === :undefined and check_dupname(name,
                                                                    pid)) do
                 true ->
                   _ = :gen_server.multi_call(nodes, :global_name_server,
    ...
    :ok
    :gen_server.call(:global_name_server, {:registrar, fun},
                       :infinity)
  end
    ...
    case (:ets.lookup(:global_pid_names, pid)) do
      ...
      pidNames ->
        case (:application.get_env(:kernel,
                                     :global_multi_name_action)) do
          {:ok, :allow} ->
            true
          _ ->
            s = 'global: ~w registered under several names: ~tw\n'
            names = [name | for {_Pid, name1} <- pidNames do
                              name1
                            end]
  ...
  def registered_names() do
    mS = :ets.fun2ms(fn {name, _Pid, _M, _R} ->
                          name
                     end)
  ...
  def set_lock(id, nodes, retries) when (is_integer(retries) and
                                     retries >= 0) do
  ...
  defp set_lock({_ResourceId, _LockRequesterId}, [], _Retries,
            _Times) do
    true
  end

  defp set_lock({_ResourceId, _LockRequesterId} = id, nodes,
            retries, times) do
    :ok
  ...
  def disconnect() do
    :gen_server.call(:global_name_server, :disconnect,
                       :infinity)
  end
  ...
  def init([]) do
    ...
    :ok = :net_kernel.monitor_nodes(true,
                                      %{connection_id: true})
    known = :lists.foldl(fn {n, %{connection_id: cId}},
                              cs ->
                              Map.put(cs, {:connection_id, n}, cId)
                         end,
                           %{}, nodes(:visible, %{connection_id: true}))
    _ = :ets.new(:global_locks,
                   [:set, :named_table, :protected])
    _ = :ets.new(:global_names,
                   [:set, :named_table, :protected, {:read_concurrency,
                                                       true}])
    _ = :ets.new(:global_names_ext,
                   [:set, :named_table, :protected])
    _ = :ets.new(:global_pid_names,
                   [:bag, :named_table, :protected])
    _ = :ets.new(:global_pid_ids,
                   [:bag, :named_table, :protected])
    _ = :ets.new(:global_lost_connections,
                   [:set, :named_table, :protected])
    _ = :ets.new(:global_node_resources,
                   [:set, :named_table, :protected])
    ...
    ca = (case (:application.get_env(:kernel,
                                       :connect_all)) do
            {:ok, caBool} when is_boolean(caBool) ->
              caBool
            {:ok, caInvalid} ->
              :erlang.error({:invalid_parameter_value, :connect_all,
                               caInvalid})
            :undefined ->
              caBool = (case (:init.get_argument(:connect_all)) do
                          {:ok, [['false' | _] | _]} ->
                            false
                          _ ->
                            true
                        end)
              :ok = :application.set_env(:kernel, :connect_all,
                                           caBool, [{:timeout, :infinity}])
              caBool
          end)
    pOP = (case (:application.get_env(:kernel,
                                        :prevent_overlapping_partitions)) do
             {:ok, popBool} when is_boolean(popBool) ->
               popBool
             {:ok, popInvalid} ->
               :erlang.error({:invalid_parameter_value,
                                :prevent_overlapping_partitions, popInvalid})
             :undefined ->
               true
           end)
    s = r_state(the_locker: start_the_locker(doTrace),
            known: known, trace: t0,
            the_registrar: start_the_registrar(),
            conf: r_conf(connect_all: ca, prevent_over_part: pOP))
    _ = :rand.seed(:default,
                     rem(:erlang.monotonic_time(:nanosecond), 1000000000) + rem(:erlang.system_time(:nanosecond), 1000000000))
    creX = (:rand.uniform(1 <<< 59 - 1 - -
                                         (1 <<< 59)) - 1) &&& ~~~ (1 <<< 32 - 1)
  ...
  def handle_call({:registrar, fun}, from, s) do
    send(r_state(s, :the_registrar), {:trans_all_known, fun,
                                  from})
    {:noreply, s}
  end

  def handle_call({:register, name, pid, method}, {fromPid, _Tag},
           s0) do
  ...
  def handle_call({:register_ext, name, pid, method, regNode},
           {fromPid, _Tag}, s0) do
    s = ins_name_ext(name, pid, method, regNode, fromPid,
                       [], s0)
    {:reply, :yes, s}
  end
  ...
  def handle_call({:sync, nodes}, from, s) do
    pid = start_sync(:lists.delete(node(),
                                     nodes) -- r_state(s, :synced),
                       from)
  ...
  def handle_call(:disconnect, _From, r_state(known: known) = s0) do
    nodes = :maps.fold(fn {:connection_id, n}, _, ns
                              when is_atom(n) ->
  ...
  def handle_call(:high_level_trace_start, _From, s) do
    ...
    send_high_level_trace()
    {:reply, :ok,
       trace_message(r_state(s, trace: []), {:init, node()}, [])}
  end
  ...
  def handle_call(:high_level_trace_get, _From,
           r_state(trace: trace) = s) do
  ...
  def handle_call(request, from, s) do
    :logger.log(:warning, 'The global_name_server received an unexpected message:\nhandle_call(~tp, ~tp, _)\n', [request, from])
    {:noreply, s}
  end
  ...
  def handle_cast({:exchange, node, nameList, _NameExtList, myTag},
           s) do
    case (:erlang.get({:sync_tag_my, node})) do
      ^myTag ->
  ...
  def handle_cast({:exchange_ops, node, myTag, ops, resolved},
           s0) do
    :ok
    ...
    case (:erlang.get({:sync_tag_my, node})) do
      ^myTag ->
        ...
        :gen_server.cast({:global_name_server, node},
                           {:resolved, node(), resolved, known, :unused,
                              get_names_ext(),
        ...
        case (:erlang.get({:save_ops, node})) do
          {:resolved, hisKnown, names_ext, hisResolved} ->
            ...
            newS = resolved(node, hisResolved, hisKnown, names_ext,
                              s)
            {:noreply, newS}
          :undefined ->
            :erlang.put({:save_ops, node}, ops)
  ...
  def handle_cast({:resolved, node, hisResolved, hisKnown, _Unused,
            names_ext, myTag},
           s) do
    :ok
    case (:erlang.get({:sync_tag_my, node})) do
  ...
  def handle_cast({:new_nodes, node, ops, names_ext, nodes,
            extraInfo},
           s) do
    :ok
    newS = new_nodes(ops, node, names_ext, nodes, extraInfo,
                       s)
    {:noreply, newS}
  end

  def handle_cast({:in_sync, node, _IsKnown},
           r_state(known: known, synced: synced) = s0) do
    :ok
    :lists.foreach(fn pid ->
                        send(pid, {:synced, [node]})
                   end,
                     r_state(s0, :syncers))
    nSynced = (case (:lists.member(node, synced)) do
                 true ->
                   synced
                 false ->
                   [node | synced]
               end)
    s1 = r_state(s0, known: :maps.remove({:pending, node},
                                     known), 
                 synced: nSynced)
    s2 = cancel_locker(node, s1,
                         :erlang.get({:sync_tag_my, node}))
  ...
  def handle_cast({:lock_set, pid, _Set, _HisKnown,
            myTag} = message,
           s) do
  ...
  def handle_cast({:lock_set, _Pid, _Set, _HisKnown} = message,
           s) do
    r_state(the_locker: locker) = s
    send(locker, message)
    {:noreply, s}
  ...
  def handle_info({:EXIT, locker, _Reason} = exit,
           r_state(the_locker: locker) = s) do
    {:stop, {:locker_died, exit},
       r_state(s, the_locker: :undefined)}
  end

  def handle_info({:EXIT, registrar, _} = exit,
           r_state(the_registrar: registrar) = s) do
    {:stop, {:registrar_died, exit},
       r_state(s, the_registrar: :undefined)}
  end
  ...
  def handle_info({:nodedown, node, info}, s0) do
    ...
    nodeDownType = (case (:global_group.participant(node)) do
                      true ->
                        :disconnected
                      false ->
                        :ignore_node
    ...
    known = :maps.remove({:connection_id, node},
                           r_state(s2, :known))
  ...
  def handle_info({:group_nodedown, node, cId},
           r_state(known: known,
               conf: r_conf(connect_all: cA,
                         prevent_over_part: pOP)) = s0) do
    :ok
    s1 = trace_message(s0, {:group_nodedown, node, cId}, [])
    s = (case (:maps.get({:connection_id, node}, known,
                           :not_connected)) do
           ^cId ->
             ...
             :lists.foreach(fn pid ->
                              ...
                              r_state(s2, :syncers))
             nodeDownType = (case (cA and pOP and :global_group.member(node)) do
                               false ->
                                 :ignore_node
                               true ->
                                 :disconnected
  ...
  def handle_info({:nodeup, node, %{connection_id: cId}},
           r_state(known: known, conf: r_conf(connect_all: false)) = s) do
    {:noreply,
       r_state(s, known: Map.put(known, {:connection_id, node},
                                    cId))}
  end

  def handle_info({:nodeup, node, %{connection_id: cId}},
           r_state(known: known) = s0) do
    :ok
    s1 = r_state(s0, known: Map.put(known, {:connection_id, node},
                                       cId))
    s2 = trace_message(s1, {:nodeup, node}, [])
    s3 = (case (:global_group.group_configured()) do
            false ->
  ...
  def handle_info({:group_nodeup, node, cId},
           r_state(known: known) = s0) do
    :ok
    s1 = trace_message(s0, {:group_nodeup, node, cId}, [])
    s2 = (case (:maps.get({:connection_id, node}, known,
                            :not_connected)) do
            ^cId ->
  ...
  def handle_info({:lost_connection, nodeA, xCreationA, opIdA,
            nodeB} = msg,
           r_state(conf: r_conf(connect_all: true,
                       prevent_over_part: true)) = s0) do
    lcKey = {nodeA, nodeB}
    s1 = (case (get_lost_connection_info(lcKey)) do
            ...
            {_, _, tmr} ->
              ...
              rmNode = (case (node() == nodeB) do
                          false ->
                            nodeB
                          true ->
                            nodeA
                        end)
              case (is_node_potentially_known(rmNode,
                                                s0) and :global_group.participant(rmNode)) do
                false ->
                  s0
                true ->
                  {nDType, what} = (case (node_vsn(rmNode, s0)) do
                                      vsn when vsn < 7 ->
                                      ...
                                      vsn ->
                                        ...
                                        case (:global_group.member(rmNode) and vsn >= 8) do
                                          ...
                                          false ->
                                            {:remove_connection, 'requested disconnect from'}
                                        end
                                    end)
                  :logger.log(:warning, '\'global\' at node ~p ~s node ~p in order to prevent overlapping partitions', [node(), what, rmNode])
  ...
  def handle_info({:lost_connection, _NodeA, _XCreationA, _OpIdA,
            _NodeB},
           s) do
    {:noreply, s}
  end
  ...
  def handle_info({:remove_connection, node}, s0) do
    s2 = (case (is_node_potentially_known(node, s0)) do
            false ->
              s0
            true ->
              {nDType, what} = (case (:global_group.member(node)) do
                                  true ->
                                    send(:global_group, {:disconnect_node,
                                                           node})
                                    {:ignore_node, 'excluded global group member'}
                                  false ->
              ...
              :logger.log(:warning, '\'global\' at node ~p ~s node ~p in order to prevent overlapping partitions', [node(), what, node])
              s1
          end)
    {:noreply, s2}
  end
  ...
  def handle_info({:cancel_connect, node, myTag}, s0) do
    s3 = (case (:erlang.get({:sync_tag_my, node})) do
            ^myTag ->
              s1 = cancel_locker(node, s0, myTag)
              reset_node_state(node)
              s2 = r_state(s1, known: :maps.remove({:pending, node},
                                               r_state(s1, :known)))
  ...
  def handle_info({:init_connect_ack, node, hisMyTag, hisHisTag},
           s0) do
    myMyTag = :erlang.get({:sync_tag_my, node})
    myHisTag = :erlang.get({:sync_tag_his, node})
    s1 = (case (myMyTag === hisMyTag and myHisTag === hisHisTag) do
            true ->
              s0
            false ->
  ...
  def handle_info({:prepare_shutdown, from, ref}, s0) do
    s1 = r_state(s0, conf: r_conf(connect_all: false,
    ...
    case (s) do
      r_state(trace: [{node, _Time, _M, nodes, _X} | _]) ->
        ...
        case ({cNode, cNodes}) do
          ...
          _ ->
            {new, _,
               old} = :sofs.symmetric_partition(:sofs.set([cNode |
                                                               cNodes]),
                                                  :sofs.set([node | nodes]))
            m = {:nodes_changed,
  ...
  def handle_info({:DOWN, monitorRef, :process, _Pid, _Info},
           s0) do
  ...
  defp save_node_resource_info(node, mon) do
    newRes = (case (:ets.lookup(:global_node_resources,
                                  node)) do
                [] ->
                  %{mon => :ok}
                [{^node, oldRes}] ->
    ...
    true = :ets.insert(:global_node_resources,
  ...
  defp delete_node_resource_info(mon) do
    case (:ets.lookup(:global_node_resources, mon)) do
      [] ->
        :ok
      [{^mon, node}] ->
        [{^node, oldRes}] = :ets.lookup(:global_node_resources,
                                          node)
  ...
  defp local_lock_check(id, nodes) do
    not
    :lists.member(node(),
                    nodes) or can_set_lock(id) !== false
  end
  ...
  defp check_replies([{_Node, false = reply} | _T], id, replies) do
    trueReplyNodes = (for {n, true} <- replies do
                        n
                      end)
    :ok
    _ = :gen_server.multi_call(trueReplyNodes,
                                 :global_name_server, {:del_lock, id})
    reply
  end
  ...
  defp init_connect(vsn, node, initMsg, hisTag, hisVsn,
            r_state(known: known0) = s0) do
    try do
      s1 = (case (:maps.is_key({:pending, node}, known0)) do
              false ->
                s0
              true ->
                cond do
                  hisVsn < 8 ->
                    ...
                    :logger.log(:error, '\'global\' at node ~p got an out of sync connection attempt from old version ~p node ~p. Disconnecting from it.', [node(), hisVsn, node])
                    throw({:return, s0})
                  true ->
      ...
      case (:lists.keyfind(node, 1, r_state(s1, :resolvers))) do
        {^node, myTag, _Resolver} ->
          ...
          {:locker, _NoLongerAPid, _HisKnown0,
             hisTheLocker} = initMsg
          :ok
          hisKnown = []
          send(r_state(s1, :the_locker), {:his_the_locker, hisTheLocker,
  ...
  defp exchange(node, nameList, resolvers) do
    ...
    case (:erlang.erase({:wait_lock, node})) do
      :lock_is_set ->
        {^node, _Tag, resolver} = :lists.keyfind(node, 1,
                                                   resolvers)
  ...
  defp resolved(node, hisResolved, hisKnown, names_ext, s0) do
    ...
    extraInfo = [{:vsn, :erlang.get({:prot_vsn, node})},
                     {:lock, :erlang.get({:lock_id, node})}]
    s1 = do_ops(ops, node(), names_ext, extraInfo, s0)
    :lists.foreach(fn pid ->
                        send(pid, {:synced, [node]})
                   end,
                     r_state(s1, :syncers))
    s2 = :lists.foldl(fn cnclNode, accS ->
                           f = fn tag, cnclS ->
                        ...
                        s1, hisKnownNodes)
    newNodesMsg = {:new_nodes, node(), ops, names_ext,
                     newNodes, extraInfo}
    newNodesF = fn () ->
                     :maps.foreach(fn n, v when (is_atom(n) and v >= 7) ->
    ...
    s5 = trace_message(s4, {:added, addedNodes},
                         [{:new_nodes, newNodes}, {:abcast, known}, {:ops,
                                                                       ops}])
  ...
  defp new_nodes(ops, connNode, names_ext, nodes, extraInfo,
            s0) do
  ...
  defp ins_name(name, pid, method, fromPidOrNode, extraInfo,
            s0) do
    :ok
    s1 = delete_global_name_keep_pid(name, s0)
    s = trace_message(s1, {:ins_name, node(pid)},
                        [name, pid])
    insert_global_name(name, pid, method, fromPidOrNode,
                         extraInfo, s)
  end

  defp ins_name_ext(name, pid, method, regNode, fromPidOrNode,
            extraInfo, s0) do
    :ok
    s1 = delete_global_name_keep_pid(name, s0)
    dolink_ext(pid, regNode)
    s = trace_message(s1, {:ins_name_ext, node(pid)},
                        [name, pid])
    true = :ets.insert(:global_names_ext,
                         {name, pid, regNode})
    insert_global_name(name, pid, method, fromPidOrNode,
                         extraInfo, s)
  end
  ...
  defp insert_lock({resourceId, lockRequesterId} = id, pid,
            pidRefs, s) do
    ...
    lock = {resourceId, lockRequesterId,
              [{pid, ref} | pidRefs]}
  ...
  defp handle_del_lock({resourceId, lockReqId}, pid, s0) do
    ...
    case (:ets.lookup(:global_locks, resourceId)) do
      [{^resourceId, ^lockReqId, pidRefs}] ->
        remove_lock(resourceId, lockReqId, pid, pidRefs, false,
                      s0)
      _ ->
        s0
    end
  ...
  defp remove_lock(resourceId, lockRequesterId, pid, [{pid, ref}],
            down, s0) do
    :ok
  ...
  defp remove_lock(resourceId, lockRequesterId, pid, pidRefs0,
            _Down, s) do
    :ok
    pidRefs = (case (:lists.keyfind(pid, 1, pidRefs0)) do
                 {^pid, ref} ->
                   delete_node_resource_info(ref)
                   true = :erlang.demonitor(ref, [:flush])
                   true = :ets.delete_object(:global_pid_ids,
  ...
  defp do_ops(ops, connNode, names_ext, extraInfo, s0) do
    :ok
    xInserts = (for {name2, pid2, regNode} <- names_ext,
                      {:insert, {name, pid, method}} <- ops, name === name2,
                      pid === pid2 do
                  {name, pid, regNode, method}
                end)
    s1 = :lists.foldl(fn {name, pid, regNode, method}, s1 ->
                           ins_name_ext(name, pid, method, regNode, connNode,
                                          extraInfo, s1)
                      end,
                        s0, xInserts)
    xNames = (for {name, _Pid, _RegNode,
                     _Method} <- xInserts do
                name
              end)
    inserts = (for {:insert, {name, pid, method}} <- ops,
                     not :lists.member(name, xNames) do
                 {name, pid, node(pid), method}
               end)
    s2 = :lists.foldl(fn {name, pid, _RegNode, method},
                           s2 ->
                           ins_name(name, pid, method, connNode, extraInfo, s2)
                      end,
                        s1, inserts)
    delNames = (for {:delete, name} <- ops do
                  name
                end)
    :lists.foldl(fn name, s ->
  ...
  defp sync_others(nodes) do
    n = (case (:application.get_env(:kernel,
                                      :global_connect_retries)) do
           {:ok, nRetries} when (is_integer(nRetries) and
                                   nRetries >= 0)
                                ->
             nRetries
           _ ->
  ...
  defp insert_global_name(name, pid, method, fromPidOrNode, extraInfo,
            s) do
    ref = :erlang.monitor(:process, pid)
    save_node_resource_info(node(pid), ref)
    true = :ets.insert(:global_names,
                         {name, pid, method, ref})
  ...
  defp del_name(ref, s) do
    nameL = (for {_, name} <- :ets.lookup(:global_pid_names,
                                            ref),
                   {_, _Pid, _Method, ref1} <- :ets.lookup(:global_names,
                                                             name),
                   ref1 === ref do
               name
             end)
  ...
  defp delete_global_name2(name, pid, ref, s) do
    ...
    :ok
    true = :ets.delete_object(:global_pid_names,
                                {pid, name})
    true = :ets.delete_object(:global_pid_names,
                                {ref, name})
  ...
  Record.defrecord(:r_multi, :multi, local: [], remote: [],
                                 known: [], the_boss: :undefined,
                                 just_synced: false, do_trace: :undefined)
  Record.defrecord(:r_him, :him, node: :undefined,
                               locker: :undefined, vsn: :undefined,
                               my_tag: :undefined, his_tag: :undefined)
  ...
  defp loop_the_locker(s) do
    ...
    after 0 ->
      timeout = (case ({r_multi(s, :local), r_multi(s, :remote)}) do
                   {[], []} ->
                     :infinity
                   _ ->
                     cond do
  ...
  defp the_locker_message({:his_the_locker, hisTheLocker, hisKnown0,
             hisTag, myTag} = _HtlMsg,
            s) do
    :ok
    {hisVsn, _HisKnown} = hisKnown0
    true = hisVsn > 4
    him = r_him(node: node(hisTheLocker), my_tag: myTag,
              his_tag: hisTag, locker: hisTheLocker, vsn: hisVsn)
    loop_the_locker(add_node(him, s))
  end

  defp the_locker_message({:cancel, _Node, :undefined, :no_fun} = _CMsg,
            s) do
    :ok
    loop_the_locker(s)
  end
  ...
  defp the_locker_message({:lock_set, pid, true, _HisKnown, myTag} = _Msg,
            s) do
    node = node(pid)
    :ok
  ...
  defp the_locker_message({:lock_set, pid, true, hisKnown}, s) do
    case (find_node_tag(node(pid), s)) do
      {true, myTag, _HisVsn, _HisTag} ->
        the_locker_message({:lock_set, pid, true, hisKnown,
                              myTag},
                             s)
      false ->
        :ok
  ...
  defp select_node(s) do
    ...
    cond do
      ...
      true ->
        ...
        r_him(locker: hisTheLocker, vsn: hisVsn, node: node,
            my_tag: myTag, his_tag: hisTag) = him
        hisNode = [node]
        us = [node() | hisNode]
  ...
  defp lock_is_set(s, him, myTag, known1, lockId) do
    ...
    receive do
      {:lock_set, p, true, _, ^myTag} when node(p) === node ->
        lock_is_set_true_received(s, him, myTag, known1, lockId,
                                    p)
  ...
  defp find_node_tag2(node,
            [r_him(node: node, my_tag: myTag, vsn: hisVsn,
                 his_tag: hisTag) |
                 _]) do
    {true, myTag, hisVsn, hisTag}
  end
  ...
  defp remove_node(node, tag, s) do
    r_multi(s, local: remove_node2(node, tag, r_multi(s, :local)), 
  ...
  defp remove_node2(node, tag,
            [r_him(node: node, my_tag: tag) | rest]) do
    rest
  end
  ...
  defp cancel_locker(node, s, tag, toBeRunOnLockerF) do
    ...
    case (:lists.keyfind(node, 1, resolvers)) do
      {_, ^tag, resolver} ->
        ...
        s1 = trace_message(s, {:kill_resolver, node},
                             [tag, resolver])
  ...
  defp send_cancel_connect(node, myTag, r_state(known: known)) do
    try do
      ...
      send_cancel_connect_message(node,
                                    :erlang.get({:sync_tag_his, node}))
    catch
      :ignore ->
        :ok
  ...
  defp exchange_names([{name, pid, method} | tail], node, ops, res) do
    case (:ets.lookup(:global_names, name)) do
      ...
      [{^name, pid2, method2, _Ref2}] when node() < node ->
        node2 = node(pid2)
        case (:rpc.call(node2, :global, :resolve_it,
                          [method2, name, pid, pid2])) do
          ^pid ->
          ...
          {:badrpc, badrpc} ->
            :logger.log(:info, 'global: badrpc ~w received when conflicting name ~tw was found\n', [badrpc, name])
            op = {:insert, {name, pid, method}}
            exchange_names(tail, node, [op | ops], res)
          else__ ->
            :logger.log(:info, 'global: Resolve method ~w for conflicting name ~tw returned ~tw\n', [method, name, else__])
            op = {:delete, name}
            exchange_names(tail, node, [op | ops], [op | res])
        end
  ...
  defp delete_lock(ref, s0) do
    ...
    f = fn {resourceId, lockRequesterId, pidRefs}, s ->
             ...
             remove_lock(resourceId, lockRequesterId, pid, pidRefs,
                           true, s)
        end
  ...
  defp pid_locks(ref) do
    l = :lists.flatmap(fn {_, resourceId} ->
  ...
  defp handle_nodeup(node,
            r_state(the_locker: theLocker, resolvers: rs,
                known: known) = s0) do
    case (:maps.is_key(node,
                         known) or :lists.keymember(node, 1, rs)) do
      true ->
        s0
      false ->
        ...
        s1 = trace_message(s0, {:new_resolver, node},
                             [myTag, resolver])
        r_state(s1, resolvers: [{node, myTag, resolver} | rs])
    end
  end

  defp handle_nodedown(node, r_state(synced: syncs, known: known0) = s,
            what) do
    ...
    r_state(newS, known: known3, 
              synced: :lists.delete(node, syncs))
  end

  defp inform_connection_loss(node,
            r_state(conf: r_conf(connect_all: true,
                        prevent_over_part: true)) = s) do
    msg = {:lost_connection, node(),
             :erlang.get(:creation_extension) + :erlang.system_info(:creation) - -
                                                                                 (1 <<< 59),
  ...
  defp gns_volatile_multicast(msg, ignoreNode, minVer, alsoPend,
            r_state(known: known)) do
    :maps.foreach(fn node, ver when (is_atom(node) and
                                       node !== ignoreNode and ver >= minVer)
                                    ->
                       _ = :erlang.send({:global_name_server, node}, msg,
                                          [:noconnect])
                     {:pending, node}, ver when (alsoPend == true and
                                                   node !== ignoreNode and
                                                   ver >= minVer)
                                                ->
                       _ = :erlang.send({:global_name_server, node}, msg,
  ...
  defp is_node_potentially_known(node, r_state(known: known)) do
    :maps.is_key(node, known) or :maps.is_key({:pending,
                                                 node},
                                                known)
  end
  ...
  defp make_node_vsn_list(nList, r_state() = s) do
    :lists.map(fn {n, 0} when is_atom(n) ->
                    {n, node_vsn(n, s)}
                  n when is_atom(n) ->
                    {n, node_vsn(n, s)}
  ...
  defp mk_known_list(vsn, r_state(known: known)) when vsn < 7 do
    :lists.foldl(fn {n, _V}, ns when is_atom(n) ->
                      [n | ns]
                    _, ns ->
                      ns
                 end,
  ...
  defp mk_known_list(_Vsn, r_state(known: known)) do
    :lists.foldl(fn {n, _V} = nV, nVs when is_atom(n) ->
                      [nV | nVs]
                    _, ns ->
                      ns
                 end,
  ...
  defp add_to_known(addKnown, r_state(known: known) = s) do
    ...
    {added, newKnown} = :lists.foldl(fun, {[], known},
                                       addKnown)
  ...
  defp save_lost_connection_info(lcKey, xCre, opId, :undefined) do
    tmr = :erlang.start_timer(60 * 60 * 1000, self(),
                                {:lost_connection, lcKey})
    value = {xCre, opId, tmr}
    _ = :ets.insert(:global_lost_connections,
                      {lcKey, value})
    :ok
  end

  defp save_lost_connection_info(lcKey, xCre, opId, oldTmr) do
    _ = :erlang.cancel_timer(oldTmr,
  ...
  defp get_known() do
    :gen_server.call(:global_name_server, :get_known,
                       :infinity)
  end

  defp random_sleep(times) do
    _ = (case (rem(times, 10)) do
           0 ->
             _ = :rand.seed(:exsplus)
           _ ->
             :ok
      ...
      {:ok, nodesNG} ->
        ...
        case (illegalSyncNodes) do
          [] ->
            syncNodes
          _ ->
            {:error, {'Trying to sync nodes not defined in the own global group', illegalSyncNodes}}
        end
      {:error, _} = error ->
        error
    end
  ...
  defp unexpected_message(message, what) do
    :logger.log(:warning, 'The global_name_server ~w process received an unexpected message:\n~tp\n', [what, message])
  end
