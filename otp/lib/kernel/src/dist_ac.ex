defmodule :m_dist_ac do
    ...
    case keysearch(appName, r_appl(:name), r_state(s, :appls)) do
      ...
      when :erlang.element(
        ...
        case (try do
          ...
          {:error, r} ->
            error_msg(~c"Error when restarting application ~p: ~p~n", [appName, r])
            s
        end
  ...
  defp dist_mismatch(appName, node) do
    error_msg(~c"Distribution mismatch for application \"~p\" on nodes ~p and ~p~n", [
      appName,
      node(),
      node
    ])
  ...
  defp error_msg(format, argList)
       when is_list(format) and
              is_list(argList) do
    :error_logger.error_msg(~c"dist_ac on node ~p:~n" ++ format, [node() | argList])
  end
end
