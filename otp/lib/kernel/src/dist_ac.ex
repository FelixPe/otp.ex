defmodule :m_dist_ac do
  use Bitwise
  import :lists, only: [filter: 2, foldl: 3, foreach: 2,
                          keydelete: 3, keyreplace: 4, keysearch: 3, map: 2,
                          mapfoldl: 3, member: 2, zf: 2]
  @behaviour :gen_server
  require Record
  Record.defrecord(:r_state, :state, appls: [], tmp_locals: [],
                                 remote_started: [], known: [], started: [],
                                 tmp_weights: [], dist_loaded: [], t_reqs: [],
                                 s_reqs: [], p_reqs: [])
  Record.defrecord(:r_appl, :appl, name: :undefined,
                                id: :undefined, restart_time: 0, nodes: [],
                                run: [])
  def start_link() do
    case (:gen_server.start_link({:local, :dist_ac},
                                   :dist_ac, [], [])) do
      {:ok, pid} ->
  ...
  def load_application(appName, distNodes) do
    :gen_server.call(:dist_ac,
                       {:load_application, appName, distNodes}, :infinity)
  end
  ...
  defp get_nodes(appName) do
    :gen_server.call(:dist_ac, {:get_nodes, appName},
                       :infinity)
  end
  ...
  defp sync_dacs(appls) do
    res = :global.trans({:dist_ac, :sync_dacs},
                          fn () ->
  ...
  def handle_cast(:init_sync, _S) do
    receive do
      {:go, kernelConfig} ->
        appls = (case (:application.get_env(:kernel,
                                              :distributed)) do
                   {:ok, d} ->
                     dist_check(d)
                   :undefined ->
        ...
        {:noreply,
           r_state(appls: nAppls, known: known,
  ...
  def handle_call({:load_application, appName, distNodes}, _From,
           s) do
    ...
    case ((try do
      ...
      {:EXIT, r} ->
        {:stop, r, {:error, r}, s}
      nAppls ->
        newS = (case (dist_find_nodes(nAppls, appName)) do
                  [] ->
                    s
                  _Nodes ->
  ...
  def handle_call({:takeover_application, appName, restartType},
           from, s) do
    appls = r_state(s, :appls)
    case (keysearch(appName, r_appl(:name), appls)) do
      {:value, appl} when :erlang.element(1,
                                            r_appl(appl, :id)) === :distributed
                          ->
        ...
        tR = r_state(s, :t_reqs)
        {:noreply,
           r_state(s, appls: nAppls,  t_reqs: [{appName, from} | tR])}
      {:value, r_appl(id: :local)} ->
        {:reply, {:error, {:already_running_locally, appName}},
           s}
      _ ->
        {:reply, {:error, {:not_running_distributed, appName}},
           s}
    end
  end

  def handle_call({:permit_application, appName, bool, lockId,
            startInfo},
           from, s) do
    case (:lists.keymember(appName, r_appl(:name),
                             r_state(s, :appls))) do
      false ->
      ...
      true ->
        nAppls = dist_update_run(r_state(s, :appls), appName, node(),
                                   bool)
        newS = r_state(s, appls: nAppls)
        isRunning = keysearch(appName, r_appl(:name), nAppls)
        isMyApp = (case (isRunning) do
                     {:value, r_appl(id: :local)} ->
                       true
                     _ ->
                       false
        ...
        msg = {:dist_ac_new_permission, node(), appName, bool,
                 isMyApp}
        send_msg(msg, nodes)
        case (startInfo) do
          :only_loaded ->
  ...
  def handle_call({:distribution_changed, newDistribution}, _From,
           s) do
    appls = r_state(s, :appls)
  ...
  def handle_call({:get_nodes, appName}, _From, s) do
    alive = intersection(dist_flat_nodes(r_state(s, :appls),
                                           appName),
                           r_state(s, :known))
    {:reply, alive, s}
  end
  ...
  def handle_info({:ac_load_application_req, appName}, s) do
    ...
    send(:application_controller, {:ac_load_application_reply,
                                     appName, :ok})
    {:noreply, newS}
  end
      ...
      true ->
        s2 = (case ((try do
                ...
                {:ok, newS, _} ->
                  newS
                {:error, r} ->
                  send(:application_controller, {:ac_start_application_reply,
                                                   appName, {:error, r}})
                  s
              end)
  ...
  def handle_info({:ac_application_run, appName, res}, s) do
    ...
    send_msg({:dist_ac_app_started, node(), appName, res},
               nodes)
    nId = (case (res) do
             :ok ->
               :local
  ...
  def handle_info({:ac_application_not_run, appName}, s) do
    {:value, appl} = keysearch(appName, r_appl(:name),
                                 appls = r_state(s, :appls))
    nTReqs = del_t_reqs(appName, r_state(s, :t_reqs),
                          {:error, :stopped})
    sReqs = filter(fn {name, from2} when name === appName ->
    ...
    nAppls = keyreplace(appName, r_appl(:name), appls, nAppl)
    {:noreply,
       r_state(s, appls: nAppls,  t_reqs: nTReqs,  s_reqs: sReqs, 
              remote_started: rS)}
  end
  ...
  def handle_info({:ac_application_stopped, appName}, s) do
    ...
    {:noreply,
       r_state(s, appls: nAppls,  started: started,  t_reqs: nTReqs, 
              s_reqs: sReqs,  remote_started: rS)}
  end

  def handle_info({:dist_ac_new_node, _Vsn, node, hisAppls, []},
           s) do
    appls = r_state(s, :appls)
    myStarted = zf(fn appl when r_appl(appl, :id) === :local ->
                        {true, {node(), r_appl(appl, :name)}}
                      _ ->
                        false
                   end,
                     appls)
    send({:dist_ac, node}, {:dist_ac_new_node, 1, node(),
                              appls, myStarted})
    nAppls = dist_merge(appls, hisAppls, node)
    {:noreply,
       r_state(s, appls: nAppls,  known: [node | r_state(s, :known)])}
  end
  ...
  def handle_info({:dist_ac_app_started, node, name, res}, s) do
    case ({keysearch(name, r_appl(:name), r_state(s, :appls)),
      ...
      {{:value, appl}, true} ->
        ...
        nPermitReq = req_del_permit_false(r_state(newS, :p_reqs),
                                            name)
        case ((try do
  ...
  def handle_info({:dist_ac_app_stopped, appName}, s) do
    ...
    case (keysearch(appName, r_appl(:name), appls)) do
      ...
      {:value, appl} ->
        ...
        rStarted = keydelete(appName, 2, r_state(s, :remote_started))
        {:noreply,
           r_state(s, appls: nAppls,  remote_started: rStarted)}
    end
  end
  ...
  def handle_info({:dist_ac_weight, name, weight, node}, s) do
    case (keysearch(name, r_appl(:name), r_state(s, :appls))) do
      {:value, appl} ->
        ...
        case (id) do
          :run_waiting ->
            send({:dist_ac, node}, {:dist_ac_weight, name, 0,
                                      node()})
            {:noreply, s}
          :undefined ->
            {:noreply,
               r_state(s, tmp_locals: [{name, weight, node} |
          ...
          _ ->
            ...
            send({:dist_ac, node}, {:dist_ac_weight, name, myWeight,
                                      node()})
            nTWs = keyreplaceadd(name, 1, r_state(s, :tmp_weights),
                                   {name, myWeight})
  ...
  def handle_info({:nodedown, node}, s) do
    ...
    hisAppls = filter(fn r_appl(name: name,
                             id: {:distributed, n})
                             when node === n ->
                        ...
                        r_state(s, :appls))
    appls2 = zf(fn appl when r_appl(appl, :id) === {:distributed,
                                                 node}
                             ->
                  ...
                  r_state(s, :appls))
    rStarted = filter(fn {node2, _Name} when node2 === node
                                             ->
                           false
                         _ ->
                           true
    ...
    {nPermitReq, appls4, sReqs} = req_del_node(s, node,
                                                 appls3)
    nKnown = :lists.delete(node, r_state(s, :known))
    newS = r_state(s, appls: appls4,  p_reqs: nPermitReq, 
                  known: nKnown,  s_reqs: sReqs, 
                  remote_started: rStarted)
    restart_appls(hisAppls)
    {:noreply, newS}
  end

  def handle_info({:dist_ac_app_loaded, node, name, hisNodes,
            permission, heKnowsMe},
           s) do
    ...
    case (is_loaded(name, s)) do
      true ->
        case (equal_nodes(nodes, hisNodes)) do
          true ->
            ...
            cond do
              not heKnowsMe ->
                msg = {:dist_ac_app_loaded, node(), name, nodes,
                         dist_is_runnable(appls, name), true}
      ...
      false ->
        load = [{{name, node}, hisNodes, permission} |
  ...
  def handle_info({:dist_ac_app_unloaded, node, name}, s) do
    appls = dist_update_run(r_state(s, :appls), name, node,
                              :undefined)
    load = keydelete({name, node}, 1, r_state(s, :dist_loaded))
    {:noreply, r_state(s, appls: appls,  dist_loaded: load)}
  end

  def handle_info({:dist_ac_new_permission, node, appName, false,
            isHisApp},
           s) do
    appls = dist_update_run(r_state(s, :appls), appName, node,
                              false)
  ...
  def handle_info({:dist_ac_new_permission, node, appName, true,
            _IsHisApp},
           s) do
    appls = dist_update_run(r_state(s, :appls), appName, node,
                              true)
  ...
  defp load(appName, s) do
    ...
    distLoaded = get_dist_loaded(appName,
    ...
    appls1 = dist_update_run(appls0, appName, node(),
                               permission)
    {loadedNodes, appls2} = mapfoldl(fn {node, hisNodes,
                                           hisPermission},
                                          appls ->
                                          case (equal_nodes(nodes, hisNodes)) do
                                            true ->
                                              {node,
                                                 dist_update_run(appls, appName,
                                                                   node,
                                                                   hisPermission)}
                                            _ ->
    ...
    foreach(fn node when node !== node() ->
                 msg = {:dist_ac_app_loaded, node(), appName, nodes,
                          permission, member(node, loadedNodes)}
                 send({:dist_ac, node}, msg)
               _ ->
                 :ok
            end,
  ...
  defp unload(appName, s) do
    ...
    {:ok,
       r_state(s, appls: keyreplace(appName, r_appl(:name), appls,
                                nAppl))}
  end

  defp start_appl(appName, s, type) do
    appl = (case (keysearch(appName, r_appl(:name),
                              appls = r_state(s, :appls))) do
              {:value, a} ->
                a
              _ ->
    ...
    case (r_appl(appl, :id)) do
      :local ->
        {:ok, s, false}
      _ ->
        {id, isWaiting} = (case (dist_get_all_nodes(appl)) do
        ...
        {:ok, newS} = req_start_app(r_state(s, appls: nAppls),
                                      appName)
        tmpLocals = keydelete_all(appName, 1,
                                    r_state(newS, :tmp_locals))
        tmpWeights = keydelete_all(appName, 1,
                                     r_state(newS, :tmp_weights))
        rStarted = keydelete(appName, 2, r_state(s, :remote_started))
        started = replaceadd(appName, r_state(newS, :started))
        {:ok,
           r_state(newS, started: started,  tmp_locals: tmpLocals, 
  ...
  defp start_distributed(appl, name, nodes, permittedNodes, s, type) do
    case (find_start_node(nodes, permittedNodes, name,
                            s)) do
      {:ok, node} when node === node() ->
        _ = (case (r_appl(appl, :id)) do
               ...
               _ when type === :reply ->
                 case (:lists.keysearch(name, 2,
                                          r_state(s, :remote_started))) do
                   {:value, {node3, _}} ->
      ...
      {:ok, node} ->
        case (keysearch(name, r_appl(:name), r_state(s, :appls))) do
          {:value, r_appl(id: {:distributed, ^node})} ->
            ...
            {{:distributed, node}, false}
          _ ->
            wait_dist_start(node, appl, name, nodes, permittedNodes,
                              s, type)
        end
      :not_started ->
        wait_dist_start2(appl, name, nodes, permittedNodes, s,
                           type)
      :no_permission ->
  ...
  defp wait_dist_start(node, appl, name, nodes, permittedNodes, s,
            type) do
    :erlang.monitor_node(node, true)
    receive do
      ...
      {:dist_ac_weight, ^name, _Weigth, ^node} ->
        :erlang.monitor_node(node, false)
        send({:dist_ac, node}, {:dist_ac_weight, name,
                                  get_cached_weight(name, s), node()})
        wait_dist_start(node, appl, name, nodes, permittedNodes,
                          s, type)
      {:nodedown, ^node} ->
        :erlang.monitor_node(node, false)
        tmpLocals = filter(fn {name2, _Weight, node2}
  ...
  defp ac_start_it(:req, name) do
    send(:application_controller, {:ac_change_application_req,
                                     name, :start_it})
  end
  ...
  defp ac_takeover(:req, name, node, restartType) do
    send(:application_controller, {:ac_change_application_req,
                                     name, {:takeover, node, restartType}})
  end

  defp ac_failover(name, node, restartType) do
    send(:application_controller, {:ac_change_application_req,
                                     name, {:failover, node, restartType}})
  end
  ...
  defp restart_appls(appls) do
    foreach(fn appl ->
                 ...
                 send_after(r_appl(appl, :restart_time),
  ...
  defp permit(false, {:value, r_appl(id: :undefined)}, _AppName,
            _From, s, _LockId) do
    {:reply, :ok, s}
  end

  defp permit(false, {:value, r_appl(id: id)}, _AppName, _From, s,
            _LockId)
      when :erlang.element(1, id) === :distributed do
    {:reply, :ok, s}
  end

  defp permit(false, {:value, _}, appName, from, s,
            _LockId) do
  ...
  defp permit(true, {:value, r_appl(id: :local)}, _AppName, _From,
            s, _LockId) do
    {:reply, :ok, s}
  end
  ...
  defp do_start_appls(startApps, s) do
    ...
    {:ok,
       foldl(fn appName, newS ->
               ...
               r_state(s, appls: appls), :lists.reverse(sortedStartApps))}
  end

  defp find_start_node(nodes, permittedNodes, name, s) do
    allNodes = intersection(flat_nodes(nodes),
                              permittedNodes)
  ...
  defp find_start_node([anyNodes | nodes], name, s, weight, allNodes)
    ...
    case (find_any_node(:erlang.tuple_to_list(anyNodes),
                          name, s, weight, allNodes)) do
      false ->
        find_start_node(nodes, name, s, weight, allNodes)
      res ->
        res
  ...
  defp find_any_node(anyNodes, name, s, weight, allNodes) do
    case (check_running(name, s,
                          intersection(anyNodes, allNodes))) do
      {:already_started, node} ->
        {:already_started, node}
      false ->
        send_nodes(allNodes,
                     {:dist_ac_weight, name, weight, node()})
        answers = [{weight, node()} | collect_answers(allNodes,
                                                        name, s, [])]
  ...
  defp check_running(name, r_state(remote_started: rStarted, appls: appls),
            anyNodes) do
  ...
  defp send_after(time, msg) when (is_integer(time) and
                             time >= 0) do
    _Pid = spawn_link(:dist_ac, :send_timeout,
                        [self(), time, msg])
    :ok
  end
  ...
  defp keydelete_all(key, n, [h | t]) when :erlang.element(n,
                                                  h) === key do
  ...
  defp equal_nodes([h1 | t1], [h2 | t2]) when (is_tuple(h1) and
                                        is_tuple(h2)) do
    case (equal(:erlang.tuple_to_list(h1),
                  :erlang.tuple_to_list(h2))) do
      true ->
        equal_nodes(t1, t2)
      false ->
        false
  ...
  defp flat_nodes(nodes) when is_list(nodes) do
    foldl(fn node, res when is_atom(node) ->
               [node | res]
  ...
  defp get_dist_loaded(name,
            [{{name, node}, hisNodes, permission} | t]) do
    [{node, hisNodes, permission} | get_dist_loaded(name,
                                                      t)]
  end
  ...
  defp del_dist_loaded(name,
            [{{name, _Node}, _HisNodes, _Permission} | t]) do
  ...
  defp req_start_app(state, name) do
    {:ok,
       foldl(fn {false, appName, true, name2}, s
                    when name === name2 ->
               ...
               state, r_state(state, :p_reqs))}
  end

  defp req_del_permit_true(reqs, name) do
    filter(fn {from, name2, true, _} when name2 === name ->
  ...
  defp req_del_permit_false(reqs, name) do
    filter(fn {from, name2, false, _Nodes}
                  when name2 === name ->
                :gen_server.reply(from, :ok)
                false
              _ ->
  ...
  defp req_del_node(s, node, appls) do
    check_waiting(r_state(s, :p_reqs), s, node, appls, [],
                    r_state(s, :s_reqs))
  end

  defp del_t_reqs(appName, tReqs, res) do
    :lists.filter(fn {aN, from} when appName === aN ->
  ...
  defp check_waiting([{from, appName, false, nodes} | reqs], s, node,
            appls, res, sReqs) do
  ...
  defp dist_check([{appName, time, nodes} | t])
    ...
    p = get_default_permission(appName)
    [r_appl(name: appName, restart_time: time, nodes: nodes,
         run: [{node(), p}]) |
         dist_check(t)]
  end
  ...
  defp dist_check([{appName, :infinity, nodes} | t]) do
    p = get_default_permission(appName)
    [r_appl(name: appName, restart_time: :infinity, nodes: nodes,
         run: [{node(), p}]) |
         dist_check(t)]
  end
  ...
  defp dist_take_control(appls) do
    foreach(fn r_appl(name: appName) ->
                 :application_controller.control_application(appName)
            end,
              appls)
  end
  ...
  defp dist_replace({appName, nodes}, appName, appls) do
    run = (for node <- flat_nodes(nodes) do
             {node, :undefined}
           end)
    keyreplaceadd(appName, r_appl(:name), appls,
                    r_appl(name: appName, restart_time: 0, nodes: nodes,
                        run: run))
  end
  ...
  defp dist_update_run(appls, appName, node, permission) do
    map(fn appl when r_appl(appl, :name) === appName ->
             run = r_appl(appl, :run)
  ...
  defp dist_change_update(appls, [{appName, newNodes} | newDist]) do
    newAppls = do_dist_change_update(appls, appName, 0,
                                       newNodes)
    dist_change_update(newAppls, newDist)
  end

  defp dist_change_update(appls,
            [{appName, newTime, newNodes} | newDist]) do
    newAppls = do_dist_change_update(appls, appName,
                                       newTime, newNodes)
    dist_change_update(newAppls, newDist)
  end
  ...
  defp dist_get_runnable(appls) do
    zf(fn r_appl(name: appName, run: run) ->
            case (keysearch(node(), 1, run)) do
              {:value, {_, true}} ->
                {true, appName}
              _ ->
  ...
  defp dist_del_node(appls, node) do
    map(fn appl ->
             nRun = filter(fn {n, _Runnable} when n === node ->
                                false
                              _ ->
                                true
                           end,
  ...
  defp dist_mismatch(appName, node) do
    error_msg('Distribution mismatch for application "~p" on nodes ~p and ~p~n', [appName, node(), node])
    exit({:distribution_mismatch, appName, node})
  end

  defp error_msg(format, argList) when (is_list(format) and
                                   is_list(argList)) do
