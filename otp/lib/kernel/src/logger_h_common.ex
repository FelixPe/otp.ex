defmodule :m_logger_h_common do
  ...
  def removing_handler(%{id: name, module: module, config: %{olp: olp}}) do
    case :erlang.whereis(:erlang.list_to_atom(:lists.concat([module, ~c"_", name]))) do
      :undefined ->
        :ok
  ...
  defp start(
    ...
    regName = :erlang.list_to_atom(:lists.concat([module, ~c"_", name]))
  ...
  def handle_info(
        {:timeout, tRef, :repeated_filesync},
        state = %{rep_sync_tref: tRef, filesync_repeat_interval: :no_repeat}
      ) do
    {:noreply, state}
  end
  ...
  def terminate(:overloaded = reason, %{id: name} = state) do
    _ = log_handler_info(name, ~c"Handler ~p overloaded and stopping", [name], state)
  ...
  defp call(module, name, op) when is_atom(name) do
    case :logger_olp.call(
           :erlang.list_to_atom(:lists.concat([module, ~c"_", name])),
           op
         ) do
      {:error, :busy} ->
        {:error, :handler_busy}
  ...
  def notify(
    ...
    log_handler_info(
      name,
      ~c"Handler ~p switched from ~p to ~p mode",
      [name, mode0, mode1],
      state
    )
  end
  ...
  def notify({:flushed, flushed}, %{id: name} = state) do
    log_handler_info(name, ~c"Handler ~p flushed ~w log events", [name, flushed], state)
  end

  def notify(:restart, %{id: name} = state) do
    log_handler_info(name, ~c"Handler ~p restarted", [name], state)
  end
            ...
            _ =
              :logger_server.do_internal_log(
                ...
                %{
                  ...
                  line: 398,
                  file: ~c"otp/lib/kernel/src/logger_h_common.erl"
                },
                log,
                [
                  [
    ...
    try do
      ...
      c, r ->
        ...
        case {:logger_formatter, %{}} do
          {^formatter, ^formatterConfig} ->
            ~c"DEFAULT FORMATTER CRASHED\n"

          {defaultFormatter, defaultConfig} ->
            try_format(
              Map.put(log, :msg, {~c"FORMATTER CRASH: ~tp", [:maps.get(:msg, log)]}),
              defaultFormatter,
              defaultConfig
            )
        end
