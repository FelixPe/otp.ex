defmodule :m_logger_h_common do
  ...
  def adding_handler(%{id: name, module: module} = config) do
    ...
    handlerConfig0 = :maps.without([:sync_mode_qlen,
                                        :drop_mode_qlen, :flush_qlen,
                                                             :burst_limit_enable,
                                                                         ...
                                                                         :overload_kill_enable,
                                                                             :overload_kill_qlen,
                                                                                 :overload_kill_mem_size,
                                                                                     :overload_kill_restart_after] ++ [:filesync_repeat_interval],
                                     hConfig0)
    case (module.check_config(name, :set, :undefined,
                                handlerConfig0)) do
      {:ok, handlerConfig} ->
        modifiedCommon = :maps.with([:filesync_repeat_interval],
                                      handlerConfig)
        commonConfig0 = :maps.with([:filesync_repeat_interval],
                                     hConfig0)
        commonConfig = :maps.merge(:maps.merge(get_default_config(),
                                                 commonConfig0),
                                     modifiedCommon)
        case (check_config(commonConfig)) do
          :ok ->
            hConfig = :maps.merge(commonConfig, handlerConfig)
            olpOpts = :maps.with([:sync_mode_qlen, :drop_mode_qlen,
                                                       :flush_qlen,
                                                           :burst_limit_enable,
                                                               :burst_limit_max_count,
                                                                   :burst_limit_window_time,
  ...
  def removing_handler(%{id: name, module: module,
             config: %{olp: olp}}) do
    case (:erlang.whereis(:erlang.list_to_atom(:lists.concat([module,
                                                                  '_',
                                                                      name])))) do
      :undefined ->
        :ok
  ...
  def changing_config(setOrUpdate,
           %{id: name, config: oldHConfig, module: module},
           newConfig0) do
    newHConfig0 = :maps.get(:config, newConfig0, %{})
    noHandlerKeys = [:sync_mode_qlen, :drop_mode_qlen,
                                          :flush_qlen, :burst_limit_enable,
                                                           :burst_limit_max_count,
                                                               :burst_limit_window_time,
                                                                   :overload_kill_enable,
                                                                       :overload_kill_qlen,
                                                                           :overload_kill_mem_size,
                                                                               :overload_kill_restart_after] ++ [:filesync_repeat_interval] ++ [:olp]
    oldHandlerConfig = :maps.without(noHandlerKeys,
                                       oldHConfig)
    newHandlerConfig0 = :maps.without(noHandlerKeys,
                                        newHConfig0)
    case (module.check_config(name, setOrUpdate,
                                oldHandlerConfig, newHandlerConfig0)) do
      {:ok, newHandlerConfig} ->
        modifiedCommon = :maps.with([:filesync_repeat_interval],
                                      newHandlerConfig)
        newCommonConfig0 = :maps.with([:filesync_repeat_interval],
                                        newHConfig0)
        oldCommonConfig = :maps.with([:filesync_repeat_interval],
                                       oldHConfig)
        commonDefault = (case (setOrUpdate) do
                           :set ->
                             get_default_config()
                           :update ->
                             oldCommonConfig
                         end)
        newCommonConfig = :maps.merge(:maps.merge(commonDefault,
                                                    newCommonConfig0),
                                        modifiedCommon)
        case (check_config(newCommonConfig)) do
          :ok ->
            olpDefault = (case (setOrUpdate) do
                            :set ->
                              :logger_olp.get_default_opts()
                            :update ->
                              :maps.with([:sync_mode_qlen, :drop_mode_qlen,
                                                               :flush_qlen,
                                                                   :burst_limit_enable,
                                                                       :burst_limit_max_count,
                                                                           :burst_limit_window_time,
            ...
            case (:logger_olp.set_opts(olp, newOlpOpts)) do
              :ok ->
                :logger_olp.cast(olp,
                                   {:config_changed, newCommonConfig,
                                      newHandlerConfig})
                readOnly = :maps.with([:olp], oldHConfig)
                newHConfig = :maps.merge(:maps.merge(:maps.merge(newCommonConfig,
                                                                   newHandlerConfig),
                                                       readOnly),
                                           newOlpOpts)
  ...
  defp start(olpOpts0,
            %{id: name, module: module,
                config: hConfig} = config0) do
    regName = :erlang.list_to_atom(:lists.concat([module, '_',
                                                              name]))
    childSpec = %{id: name,
                    start:
                    {:logger_olp, :start_link,
                       [regName, :logger_h_common, config0, olpOpts0]},
                    restart: :temporary, shutdown: 2000, type: :worker,
                    modules: [:logger_h_common]}
    case (:supervisor.start_child(:logger_sup,
                                    childSpec)) do
      {:ok, pid, olp} ->
        :ok = :logger_handler_watcher.register_handler(name,
                                                         pid)
        olpOpts = :logger_olp.get_opts(olp)
        {:ok,
           Map.put(config0, :config,
                              Map.put(:maps.merge(hConfig, olpOpts), :olp,
                                                                       olp))}
      {:error, {reason, ch}} when (is_tuple(ch) and
  ...
  def init(%{id: name, module: module, config: hConfig}) do
    ...
    case (module.init(name, hConfig)) do
      {:ok, hState} ->
        commonConfig = :maps.with([:filesync_repeat_interval],
                                    hConfig)
        state = Map.merge(commonConfig, %{id: name,
                                            module: module, ctrl_sync_count: 20,
                                            last_op: :sync,
  ...
  def handle_load(bin,
           %{id: name, module: module, handler_state: handlerState,
               ctrl_sync_count: ctrlSync} = state) do
    cond do
      ctrlSync == 0 ->
        {_, hS1} = module.write(name, :sync, bin, handlerState)
        Map.merge(state, %{handler_state: hS1,
                             ctrl_sync_count: 20, last_op: :write})
      true ->
        {_, hS1} = module.write(name, :async, bin, handlerState)
        Map.merge(state, %{handler_state: hS1,
                             ctrl_sync_count: ctrlSync - 1, last_op: :write})
    end
  end

  def handle_call(:filesync, _From,
           state = %{id: name, module: module,
                       handler_state: handlerState}) do
    {result, handlerState1} = module.filesync(name, :sync,
                                                handlerState)
    {:reply, result,
       Map.merge(state, %{handler_state: handlerState1,
                            last_op: :sync})}
  end

  def handle_cast({:config_changed, commonConfig, hConfig},
           state = %{id: name, module: module,
                       handler_state: handlerState,
                       filesync_repeat_interval: oldFSyncInt}) do
    state1 = (case (:maps.get(:filesync_repeat_interval,
                                commonConfig)) do
                ^oldFSyncInt ->
                  state
                fSyncInt ->
                  set_repeated_filesync(cancel_repeated_filesync(Map.put(state, :filesync_repeat_interval,
                                                                                  fSyncInt)))
              end)
    hS = (try do
  ...
  def handle_info({:timeout, tRef, :repeated_filesync},
           state = %{rep_sync_tref: tRef,
                       filesync_repeat_interval: :no_repeat}) do
    {:noreply, state}
  end

  def handle_info({:timeout, tRef, :repeated_filesync},
           state = %{id: name, rep_sync_tref: tRef, module: module,
                       handler_state: handlerState, last_op: lastOp}) do
    state1 = (cond do
                lastOp == :sync ->
                  state
                true ->
  ...
  def handle_info(info,
           %{id: name, module: module,
               handler_state: handlerState} = state) do
    {:noreply,
       Map.put(state, :handler_state,
                        module.handle_info(name, info, handlerState))}
  end
  ...
  def reset_state(%{id: name, module: module,
             handler_state: handlerState} = state) do
    Map.put(state, :handler_state,
                     module.reset_state(name, handlerState))
  end

  defp call(module, name, op) when is_atom(name) do
    case (:logger_olp.call(:erlang.list_to_atom(:lists.concat([module,
                                                                   '_', name])),
                             op)) do
      {:error, :busy} ->
        {:error, :handler_busy}
  ...
  def notify({:mode_change, mode0, mode1},
           %{id: name} = state) do
  ...
  defp log_handler_info(name, format, args,
            %{module: module,
                handler_state: handlerState} = state) do
    config = (case (:logger.get_handler_config(name)) do
                {:ok, conf} ->
                  conf
                _ ->
                  %{formatter:
    ...
    bin = log_to_binary(%{level: :notice,
                            msg: {format, args}, meta: meta},
                          config)
    {_, hS} = module.write(name, :async, bin, handlerState)
    Map.merge(state, %{handler_state: hS, last_op: :write})
  end

  defp log_to_binary(%{msg: {:report, _},
              meta: %{report_cb: _}} = log,
            config) do
    do_log_to_binary(log, config)
  end

  defp log_to_binary(%{msg: {:report, _}, meta: meta} = log,
            config) do
    defaultReportCb = &:logger.format_otp_report/1
    do_log_to_binary(Map.put(log, :meta,
                                    Map.put(meta, :report_cb, defaultReportCb)),
                       config)
  end
  ...
  defp do_log_to_binary(log, config) do
    {formatter, formatterConfig} = :maps.get(:formatter,
                                               config,
                                               {:logger_formatter,
                                                  %{legacy_header: true,
                                                      single_line: false}})
    ...
    try do
      ...
      c2, r2 ->
        case (:logger.allow(:debug, :logger_h_common)) do
          true ->
            _ = :logger_server.do_internal_log(:debug,
                                                 %{mfa:
                                                   {:logger_h_common,
                                                      :do_log_to_binary, 2},
                                                     line: 398, file: 'otp/lib/kernel/src/logger_h_common.erl'},
                                                 log,
                                                 [[{:formatter_error,
                                                      formatter},
                                                       {:config,
      ...
      c, r ->
          ...
          true ->
            _ = :logger_server.do_internal_log(:debug,
                                                 %{mfa:
                                                   {:logger_h_common,
                                                      :try_format, 3},
                                                     line: 410, file: 'otp/lib/kernel/src/logger_h_common.erl'},
                                                 log,
                                                 [[{:formatter_crashed,
                                                      formatter},
                                                       {:config,
                                                          formatterConfig},
                                                           {:log_event, log},
                                                               {:reason,
                                                                  {c, r,
                                                                     :logger.filter_stacktrace(:logger_h_common,
                                                                                                 __STACKTRACE__)}}]])
            :ok
          false ->
            :ok
        ...
        case ({:logger_formatter, %{}}) do
          ...
          {defaultFormatter, defaultConfig} ->
            try_format(Map.put(log, :msg,
                                      {'FORMATTER CRASH: ~tp', [:maps.get(:msg, log)]}),
                         defaultFormatter, defaultConfig)
        end
    end
  end
  ...
  defp set_repeated_filesync(%{filesync_repeat_interval: fSyncInt} = state)
    ...
    tRef = :erlang.start_timer(fSyncInt, self(),
                                 :repeated_filesync)
