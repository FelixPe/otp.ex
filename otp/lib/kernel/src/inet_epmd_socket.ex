defmodule :m_inet_epmd_socket do
      ...
      spawn_link(fn ->
        receive do
          {^distCtrlTag, :handshake_complete, from, distHandle} ->
            ...
            send(from, {distCtrlTag, :handshake_complete})
  ...
  defp handshake_complete(distCtrl, distCtrlTag, distHandle) do
    send(distCtrl, {distCtrlTag, :handshake_complete, self(), distHandle})
  ...
  defp input_handler(socket, distHandle, first, buffer, size) do
    case first do
      <<packetSize1::size(32), packet1::size(packetSize1)-binary, packetSize2::size(32),
        packet2::size(packetSize2)-binary, rest::binary>> ->
        put_data(distHandle, packetSize1, packet1)
        put_data(distHandle, packetSize2, packet2)
        dataSize = 4 + packetSize1 + 4 + packetSize2
        input_handler(socket, distHandle, rest, buffer, size - dataSize)

      <<packetSize::size(32), packet::size(packetSize)-binary, rest::binary>> ->
        dataSize = 4 + packetSize
        put_data(distHandle, packetSize, packet)
        input_handler(socket, distHandle, rest, buffer, size - dataSize)

      <<packetSize::size(32), packetStart::binary>> ->
        input_handler(socket, distHandle, packetStart, buffer, size - 4, packetSize)

      <<bin::binary>> ->
        cond do
          4 <= size ->
            {first_1, buffer_1, packetSize} =
              input_get_packet_size(
                bin,
                :lists.reverse(buffer)
              )

            input_handler(socket, distHandle, first_1, buffer_1, size - 4, packetSize)

          true ->
            data = input_data(socket)
            buffer_1 = [data | buffer]
            dataSize = byte_size(data)
            input_handler(socket, distHandle, first, buffer_1, size + dataSize)
        end
    end
  end

  defp input_handler(socket, distHandle, packetStart, buffer, size, packetSize) do
    ...
    cond do
      ...
      0 < restSize and buffer === [] ->
        <<packet::size(packetSize)-binary, rest::binary>> = packetStart
        put_data(distHandle, packetSize, packet)
      ...
      true ->
        ...
        <<lastBin::size(lastSize)-binary, rest::binary>> = bin
  ...
  defp input_get_packet_size(first, [bin | buffer]) do
    ...
    cond do
      missingSize <= byte_size(bin) ->
        <<last::size(missingSize)-binary, rest::binary>> = bin
        <<packetSize::size(32)>> = <<first::binary, last::binary>>
        {rest, :lists.reverse(buffer), packetSize}

      true ->
        input_get_packet_size(
          <<first::binary, bin::binary>>,
          buffer
        )
    end
  end
  ...
  def supported() do
    try do
      ...
      :error, :notsup ->
        {:skip, ~c"esock not supported"}

      :error, :undef ->
        {:skip, ~c"esock not configured"}
    else
