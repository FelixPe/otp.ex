defmodule :m_socket do
  ...
  def which_sockets(domain) when domain === :inet or
                        domain === :inet6 or domain === :local do
    :socket_registry.which_sockets({:domain, domain})
  end

  def which_sockets(type) when type === :stream or type === :dgram or
                      type === :seqpacket do
    :socket_registry.which_sockets({:type, type})
  end

  def which_sockets(proto) when proto === :sctp or proto === :tcp or
                       proto === :udp do
  ...
  defp do_i(sockets, infoKeys) do
    lines = (case (i_sockets(sockets, infoKeys)) do
               [] ->
                 []
               infoLines ->
                 [header_line(infoKeys) | infoLines]
             end)
    maxs = :lists.foldl(fn line, max0 ->
                             smax(max0, line)
                        end,
                          :lists.duplicate(length(infoKeys), 0), lines)
    fmt = :lists.append(for n <- maxs do
  ...
  defp i_socket_info(_Proto, _Socket, %{domain: domain} = _Info,
            :domain) do
    :erlang.atom_to_list(domain)
  end

  defp i_socket_info(_Proto, _Socket, %{type: type} = _Info,
            :type) do
    :string.to_upper(:erlang.atom_to_list(type))
  end

  defp i_socket_info(proto, _Socket, %{type: type} = _Info,
            :protocol) do
    :string.to_upper(:erlang.atom_to_list(cond do
                                            proto === 0 ->
                                              case (type) do
                                                :stream ->
                                                  :tcp
  ...
  defp i_socket_info(_Proto, _Socket, %{owner: pid} = _Info,
            :owner) do
  ...
  defp i_socket_info(_Proto, _Socket,
            %{counters: %{read_byte: n}} = _Info, :recv) do
    :erlang.integer_to_list(n)
  end

  defp i_socket_info(_Proto, _Socket,
            %{counters: %{write_byte: n}} = _Info, :sent) do
    :erlang.integer_to_list(n)
  end

  defp i_socket_info(_Proto, _Socket,
            %{rstates: rStates, wstates: wStates} = _Info,
            :state) do
  ...
  def monitor({:"$socket", sockRef} = socket)
      when is_reference(sockRef) do
    case (:prim_socket.setopt(sockRef,
                                {:otp, :use_registry}, true)) do
      :ok ->
  ...
  def supports() do
    (for key1 <- [:ioctl_requests, :ioctl_flags, :options,
                                                     :msg_flags, :protocols] do
       {key1, supports(key1)}
  ...
  def is_supported(:options, level, opt) when (is_atom(level) and
                                       is_atom(opt)) do
  ...
  def open(domain, type, protocol, opts)
      when is_map(opts) do
    case (:prim_socket.open(domain, type, protocol,
                              opts)) do
      {:ok, sockRef} ->
        socket = {:"$socket", sockRef}
        {:ok, socket}
  ...
  defp connect_nowait(sockRef, sockAddr, handle) do
    case (:prim_socket.connect(sockRef, handle,
                                 sockAddr)) do
      :select ->
  ...
  defp accept_deadline(lSockRef, deadline) do
    ...
    case (:prim_socket.accept(lSockRef, accRef)) do
      ...
      :completion ->
        timeout = timeout(deadline)
        receive do
          {:"$socket", {:"$socket", ^lSockRef}, :completion,
    ...
    case (selectTag) do
      {:send, contData} ->
        case (deadline(timeout)) do
          ...
          deadline ->
            hasWritten = false
            send_deadline_cont(sockRef, data, contData, deadline,
                                 hasWritten)
        end
      _ ->
        {:error, {:invalid, cont}}
  ...
  defp send_nowait(sockRef, bin, flags, handle) do
    send_common_nowait_result(handle, :send,
                                :prim_socket.send(sockRef, bin, flags, handle))
  end

  defp send_nowait_cont(sockRef, bin, cont, selectHandle) do
    send_common_nowait_result(selectHandle, :send,
                                :prim_socket.send(sockRef, bin, cont,
                                                    selectHandle))
  end
  ...
  defp send_deadline(sockRef, bin, flags, deadline) do
    handle = make_ref()
    hasWritten = false
    send_common_deadline_result(sockRef, bin, handle,
                                  deadline, hasWritten, :send,
                                  &send_deadline_cont/5,
                                  :prim_socket.send(sockRef, bin, flags,
                                                      handle))
  end
  ...
  defp send_common_nowait_result(handle, op, result) do
    case (result) do
      ...
      {:select, data, contData} ->
        {:select,
           {{:select_info, {op, contData}, handle}, data}}
      ^result ->
        result
    end
  end

  defp send_common_deadline_result(sockRef, data, handle, deadline, hasWritten, op,
            fun, sendResult) do
    case (sendResult) do
      {:select, cont} ->
        timeout = timeout(deadline)
  ...
  def sendto({:"$socket", sockRef} = socket, data,
           {:select_info, selectTag, _} = cont, timeout)
      when is_reference(sockRef) do
    case (selectTag) do
      {:sendto, contData} ->
  ...
  def sendto({:"$socket", sockRef}, data, dest, flags, timeout)
    ...
    case (deadline(timeout)) do
      ...
      deadline ->
        hasWritten = false
        sendto_deadline(sockRef, data, dest, flags, deadline,
                          hasWritten)
    end
  end

  def sendto({:"$socket", sockRef} = socket, [bin], dest, flags,
           timeout)
      when (is_reference(sockRef) and is_binary(bin)) do
    sendto(socket, bin, dest, flags, timeout)
  end

  def sendto({:"$socket", sockRef} = socket, data, dest, flags,
           timeout)
      when (is_reference(sockRef) and is_list(data)) do
    try do
      :erlang.list_to_binary(data)
  ...
  defp sendto_timeout_cont(sockRef, bin, cont, timeout) do
    case (deadline(timeout)) do
      ...
      deadline ->
        hasWritten = false
        sendto_deadline_cont(sockRef, bin, cont, deadline,
                               hasWritten)
    end
  end

  defp sendto_nowait(sockRef, bin, to, flags, handle) do
    send_common_nowait_result(handle, :sendto,
                                :prim_socket.sendto(sockRef, bin, to, flags,
                                                      handle))
  end
  ...
  defp sendto_deadline(sockRef, bin, to, flags, deadline,
            hasWritten) do
    handle = make_ref()
    send_common_deadline_result(sockRef, bin, handle,
                                  deadline, hasWritten, :sendto,
  ...
  defp sendto_deadline_cont(sockRef, bin, cont, deadline, hasWritten) do
    ...
    send_common_deadline_result(sockRef, bin, handle,
                                  deadline, hasWritten, :sendto,
                                  &sendto_deadline_cont/5,
                                  :prim_socket.sendto(sockRef, bin, cont,
                                                        handle))
  end
  ...
  def sendmsg({:"$socket", sockRef} = socket, restData,
           {:select_info, selectTag, _} = cont, timeout) do
    case (selectTag) do
      {:sendmsg, contData} ->
        case (restData) do
          %{iov: iOV} ->
  ...
  defp sendmsg_timeout_cont(sockRef, restData, cont, timeout) do
    case (deadline(timeout)) do
      ...
      :nowait ->
        selectHandle = make_ref()
        sendmsg_nowait_cont(sockRef, restData, cont,
                              selectHandle)
      :handle ->
        selectHandle = timeout
        sendmsg_nowait_cont(sockRef, restData, cont,
      ...
      deadline ->
        hasWritten = false
        sendmsg_deadline_cont(sockRef, restData, cont, deadline,
                                hasWritten)
    end
  end

  defp sendmsg_nowait(sockRef, msg, flags, handle, iOV) do
    send_common_nowait_result(handle, :sendmsg,
                                :prim_socket.sendmsg(sockRef, msg, flags,
                                                       handle, iOV))
  end

  defp sendmsg_nowait_cont(sockRef, restData, cont, selectHandle) do
    send_common_nowait_result(selectHandle, :sendmsg,
                                :prim_socket.sendmsg(sockRef, restData, cont,
                                                       selectHandle))
  end

  defp sendmsg_deadline(sockRef, msg, flags, deadline, hasWritten,
            iOV) do
    handle = make_ref()
    send_common_deadline_result(sockRef, iOV, handle,
                                  deadline, hasWritten, :sendmsg,
  ...
  defp sendmsg_deadline_cont(sockRef, data, cont, deadline, hasWritten) do
    selectHandle = make_ref()
    send_common_deadline_result(sockRef, data, selectHandle,
                                  deadline, hasWritten, :sendmsg,
                                  &sendmsg_deadline_cont/5,
                                  :prim_socket.sendmsg(sockRef, data, cont,
                                                         selectHandle))
  end
  ...
  def sendfile({:"$socket", sockRef} = socket, fileHandle_Cont, offset,
           count, timeout)
      ...
      r_file_descriptor(module: module) = fileHandle ->
        ...
        try do
          ...
          class, reason when (class === :error and
                             ...
                             ->
            case (__STACKTRACE__) do
              [{^module, ^getFRef, args, _} | _] when args === 1 or
                                                        tl(args) === []
                                                      ->
                :erlang.error(:badarg,
                                [socket, fileHandle_Cont, offset, count,
                                                                      timeout])
              _ ->
  ...
  def sendfile(socket, fileHandle_Cont, offset, count,
           timeout) do
    :erlang.error(:badarg,
                    [socket, fileHandle_Cont, offset, count, timeout])
  end
  ...
  defp prim_socket_sendfile(sockRef, {fRef, offset, count}, selectHandle) do
    :prim_socket.sendfile(sockRef, fRef, offset, count,
                            selectHandle)
  end

  defp prim_socket_sendfile(sockRef, {offset, count}, selectHandle) do
    :prim_socket.sendfile(sockRef, offset, count,
                            selectHandle)
  end

  defp sendfile_nowait(sockRef, state, selectHandle) do
    case (prim_socket_sendfile(sockRef, state,
                                 selectHandle)) do
      :select ->
        {fRef, _Offset, _Count} = state
        {:select,
           {:select_info, {:sendfile, fRef}, selectHandle}}
      {:select, bytesSent} ->
        {:select,
           {{:select_info, :sendfile, selectHandle}, bytesSent}}
      result ->
        result
    end
  end
  ...
  defp sendfile_deadline(sockRef, state, bytesSent_0, deadline) do
    ...
    case (prim_socket_sendfile(sockRef, state,
      ...
      {:select, bytesSent} ->
        ...
        receive do
          {:"$socket", _Socket, :select, ^selectHandle} ->
            sendfile_deadline(sockRef,
                                sendfile_next(bytesSent, state), bytesSent_1,
                                deadline)
          {:"$socket", _Socket, :abort, {^selectHandle, reason}} ->
  ...
  def recv({:"$socket", sockRef}, length, flags, timeout)
    ...
    case (deadline(timeout)) do
      ...
      :handle ->
        ...
        recv_nowait(sockRef, length, flags, handle, <<>>)
      :zero ->
        case (:prim_socket.recv(sockRef, length, flags,
                                  :zero)) do
          :ok ->
            {:error, :timeout}
          result ->
  ...
  defp recv_nowait(sockRef, length, flags, handle, acc) do
    case (:prim_socket.recv(sockRef, length, flags,
      ...
      {:select, bin} ->
        {:select,
           {{:select_info, :recv, handle}, bincat(acc, bin)}}
      :select ->
        cond do
          byte_size(acc) === 0 ->
  ...
  defp recv_deadline(sockRef, length, flags, deadline, acc) do
    ...
    case (:prim_socket.recv(sockRef, length, flags,
      ...
      {:select, bin} ->
        ...
        receive do
          {:"$socket", {:"$socket", ^sockRef}, :select, ^handle} ->
            cond do
              0 < timeout ->
                recv_deadline(sockRef, length - byte_size(bin), flags,
      ...
      :completion ->
        timeout = timeout(deadline)
        receive do
          {:"$socket", {:"$socket", ^sockRef}, :completion,
             {^handle, {:ok, _Bin} = oK}}
              when length === 0 ->
            recv_result(acc, oK)
          {:"$socket", {:"$socket", ^sockRef}, :completion,
  ...
  def recvfrom({:"$socket", sockRef}, bufSz, flags, timeout)
    ...
    case (deadline(timeout)) do
      ...
      :handle ->
        ...
        recvfrom_nowait(sockRef, bufSz, handle, flags)
      :zero ->
        case (:prim_socket.recvfrom(sockRef, bufSz, flags,
                                      :zero)) do
          :ok ->
            {:error, :timeout}
          result ->
  ...
  def recvmsg({:"$socket", sockRef}, bufSz, ctrlSz, flags, timeout)
    ...
    case (deadline(timeout)) do
      ...
      :handle ->
        ...
        recvmsg_nowait(sockRef, bufSz, ctrlSz, flags, handle)
      :zero ->
        case (:prim_socket.recvmsg(sockRef, bufSz, ctrlSz,
                                     flags, :zero)) do
          :ok ->
            {:error, :timeout}
          result ->
      ...
      deadline ->
        recvmsg_deadline(sockRef, bufSz, ctrlSz, flags,
                           deadline)
    end
  end
  ...
  def getopt_native({:"$socket", sockRef}, socketOption, valueSpec) do
    :prim_socket.getopt_native(sockRef, socketOption,
                                 valueSpec)
  end
  ...
  def ioctl({:"$socket", sockRef}, setRequest, value)
      when (setRequest === :rcvall_igmpmcast or setRequest === :rcvall_mcast) and (value === :off or value === :on) do
    :prim_socket.ioctl(sockRef, setRequest, value)
  end
  ...
  def ioctl({:"$socket", sockRef}, :sifflags = setRequest, name,
           flags)
  ...
  def ioctl({:"$socket", sockRef}, :sifaddr = setRequest, name, addr)
    ...
    :prim_socket.ioctl(sockRef, setRequest, name,
                         :prim_socket.enc_sockaddr(addr))
  end

  def ioctl({:"$socket", sockRef}, :sifdstaddr = setRequest, name,
           dstAddr)
      when is_list(name) and is_map(dstAddr) do
    :prim_socket.ioctl(sockRef, setRequest, name,
                         :prim_socket.enc_sockaddr(dstAddr))
  end

  def ioctl({:"$socket", sockRef}, :sifbrdaddr = setRequest, name,
           brdAddr)
      when is_list(name) and is_map(brdAddr) do
    :prim_socket.ioctl(sockRef, setRequest, name,
                         :prim_socket.enc_sockaddr(brdAddr))
  end

  def ioctl({:"$socket", sockRef}, :sifnetmask = setRequest, name,
           netMask)
      when is_list(name) and is_map(netMask) do
    :prim_socket.ioctl(sockRef, setRequest, name,
                         :prim_socket.enc_sockaddr(netMask))
  end
  ...
  def ioctl({:"$socket", sockRef}, :siftxqlen = setRequest, name,
           qLen)
  ...
  def cancel({:"$socket", sockRef},
           {:completion_info, completionTag,
              completionHandle} = completionInfo)
      when is_reference(sockRef) do
