defmodule :m_erl_prettypr do
  ...
  defp lay_2(node, ctxt) do
    case :erl_syntax.type(node) do
      ...
      :tuple ->
        es =
          seq(:erl_syntax.tuple_elements(node), floating(text(~c",")), reset_prec(ctxt), &lay/2)

        beside(
          floating(text(~c"{")),
          beside(sep(es), floating(text(~c"}")))
        )

      :list ->
        ctxt1 = reset_prec(ctxt)
        node1 = :erl_syntax.compact_list(node)
        d1 = sep(seq(:erl_syntax.list_prefix(node1), floating(text(~c",")), ctxt1, &lay/2))

        d =
          case :erl_syntax.list_suffix(node1) do
            :none ->
              beside(d1, floating(text(~c"]")))
            ...
            s ->
              follow(
                ...
                beside(
                  floating(text(~c"| ")),
                  beside(lay(s, ctxt1), floating(text(~c"]")))
                )
              )
          end

        beside(floating(text(~c"[")), d)
      ...
      :application ->
        ...
        d1 =
          beside(
            d,
            beside(text(~c"("), beside(sep(as), floating(text(~c")"))))
          )
      ...
      :match_expr ->
        ...
        d3 = follow(beside(d1, floating(text(~c" ="))), d2, r_ctxt(ctxt, :break_indent))
        maybe_parentheses(d3, prec, ctxt)
      ...
      :clause ->
        ...
        d1 = par(seq(:erl_syntax.clause_patterns(node), floating(text(~c",")), ctxt1, &lay/2))
      ...
      :case_expr ->
        ...
        sep([
          par(
            [follow(text(~c"case"), d1, r_ctxt(ctxt1, :break_indent)), text(~c"of")],
      ...
      :if_expr ->
        ...
        sep([follow(text(~c"if"), d, r_ctxt(ctxt1, :break_indent)), text(~c"end")])

      :fun_expr ->
        ctxt1 = reset_prec(ctxt)
        d = lay_clauses(:erl_syntax.fun_expr_clauses(node), :fun_expr, ctxt1)
        sep([follow(text(~c"fun"), d, r_ctxt(ctxt1, :break_indent)), text(~c"end")])
      ...
      :module_qualifier ->
        ...
        beside(d1, beside(text(~c":"), d2))

      :maybe_expr ->
        ctxt1 = reset_prec(ctxt)

        d1 =
          vertical(seq(:erl_syntax.maybe_expr_body(node), floating(text(~c",")), ctxt1, &lay/2))

        es0 = [text(~c"end")]

        es1 =
          case :erl_syntax.maybe_expr_else(node) do
            :none ->
              es0
            ...
            elseNode ->
              ...
              [text(~c"else"), nest(r_ctxt(ctxt1, :break_indent), d3) | es0]
          end

        sep([
          par([text(~c"maybe"), nest(r_ctxt(ctxt1, :break_indent), d1), hd(es1)])
          | tl(es1)
        ])
      ...
      :maybe_match_expr ->
        ...
        d1 =
          lay(
            :erl_syntax.maybe_match_expr_pattern(node),
            set_prec(ctxt, precL)
          )

        d2 =
          lay(
            :erl_syntax.maybe_match_expr_body(node),
            set_prec(ctxt, precR)
          )
          ...
          case attribute_type(node) do
            ...
            when tag === :export_type or
              ...
              beside(
                ...
                beside(
                  text(~c"("),
                  beside(lay(as, ctxt1), floating(text(~c")")))
                )
              )
            ...
            _ ->
              d1 = sep(seq(args, text(~c","), ctxt1, &lay/2))

              beside(
                lay(n, ctxt1),
                beside(text(~c"("), beside(d1, floating(text(~c")"))))
              )
          end

        beside(floating(text(~c"-")), beside(d, floating(text(~c"."))))

      :binary ->
        ctxt1 = reset_prec(ctxt)
        es = seq(:erl_syntax.binary_fields(node), floating(text(~c",")), ctxt1, &lay/2)

        beside(
          floating(text(~c"<<")),
          beside(par(es), floating(text(~c">>")))
        )
      ...
      :binary_field ->
        ...
        d2 =
          case :erl_syntax.binary_field_types(node) do
            ...
            ts ->
              beside(floating(text(~c"/")), lay_bit_types(ts, ctxt1))
          end
      ...
      :block_expr ->
        ctxt1 = reset_prec(ctxt)
        es = seq(:erl_syntax.block_expr_body(node), floating(text(~c",")), ctxt1, &lay/2)
        sep([text(~c"begin"), nest(r_ctxt(ctxt1, :break_indent), sep(es)), text(~c"end")])
      ...
      :catch_expr ->
        ...
        d1 = follow(text(~c"catch"), d, r_ctxt(ctxt, :break_indent))
        maybe_parentheses(d1, prec, ctxt)
      ...
      :class_qualifier ->
        ...
        case :erl_syntax.variable_name(stacktrace) do
          ...
          _ ->
            ...
            beside(
              d1,
              beside(beside(text(~c":"), d2), beside(text(~c":"), d3))
            )
        end
      ...
      :conjunction ->
        par(
          seq(:erl_syntax.conjunction_body(node), floating(text(~c",")), reset_prec(ctxt), &lay/2)
        )

      :disjunction ->
        sep(
          seq(:erl_syntax.disjunction_body(node), floating(text(~c";")), reset_prec(ctxt), &lay/2)
        )
      ...
      :error_marker ->
        ...
        beside(
          text(~c"** "),
          beside(lay_error_info(e, reset_prec(ctxt)), text(~c" **"))
        )
      ...
      :implicit_fun ->
        ...
        beside(floating(text(~c"fun ")), d)

      :list_comp ->
        ctxt1 = reset_prec(ctxt)
        d1 = lay(:erl_syntax.list_comp_template(node), ctxt1)
        d2 = par(seq(:erl_syntax.list_comp_body(node), floating(text(~c",")), ctxt1, &lay/2))
      ...
      :binary_comp ->
        ...
        d2 = par(seq(:erl_syntax.binary_comp_body(node), floating(text(~c",")), ctxt1, &lay/2))
      ...
      :map_comp ->
        ...
        d2 = par(seq(:erl_syntax.map_comp_body(node), floating(text(~c",")), ctxt1, &lay/2))
      ...
      :macro ->
        ...
        d =
          case :erl_syntax.macro_arguments(node) do
            ...
            args ->
              as = seq(args, floating(text(~c",")), set_prec(ctxt1, max_prec()), &lay/2)

              beside(
                lay(n, ctxt1),
                beside(text(~c"("), beside(par(as), floating(text(~c")"))))
              )
          end

        d1 = beside(floating(text(~c"?")), d)
      ...
      :receive_expr ->
        ...
        d2 =
          case :erl_syntax.receive_expr_timeout(node) do
            ...
            t ->
              ...
              d4 = sep(seq(a, floating(text(~c",")), ctxt1, &lay/2))

              sep([
                d1,
                follow(
                  floating(text(~c"after")),
        ...
        sep([text(~c"receive"), nest(r_ctxt(ctxt1, :break_indent), d2), text(~c"end")])
      ...
      :record_expr ->
        ...
        d2 = par(seq(:erl_syntax.record_expr_fields(node), floating(text(~c",")), ctxt1, &lay/2))

        d3 =
          beside(
            beside(floating(text(~c"#")), d1),
            beside(text(~c"{"), beside(d2, floating(text(~c"}"))))
          )
      ...
      :record_field ->
        ...
        case :erl_syntax.record_field_value(node) do
          ...
          v ->
            par(
              [d1, floating(text(~c"=")), lay(v, ctxt1)],
              r_ctxt(ctxt1, :break_indent)
            )
        end
      ...
      :map_expr ->
        {precL, prec, _} = inop_prec(:"#")
        ctxt1 = reset_prec(ctxt)
        d1 = par(seq(:erl_syntax.map_expr_fields(node), floating(text(~c",")), ctxt1, &lay/2))
        d2 = beside(text(~c"\#{"), beside(d1, floating(text(~c"}"))))
      ...
      :try_expr ->
        ctxt1 = reset_prec(ctxt)
        d1 = sep(seq(:erl_syntax.try_expr_body(node), floating(text(~c",")), ctxt1, &lay/2))
        es0 = [text(~c"end")]
        ...
        es1 =
          case :erl_syntax.try_expr_after(node) do
            ...
            as ->
              d2 = sep(seq(as, floating(text(~c",")), ctxt1, &lay/2))
              [text(~c"after"), nest(r_ctxt(ctxt1, :break_indent), d2) | es0]
          end
        ...
        es2 =
          case :erl_syntax.try_expr_handlers(node) do
            ...
            hs ->
              ...
              [text(~c"catch"), nest(r_ctxt(ctxt1, :break_indent), d3) | es1]
          end
        ...
        es3 =
          case :erl_syntax.try_expr_clauses(node) do
            ...
            cs ->
              ...
              [text(~c"of"), nest(r_ctxt(ctxt1, :break_indent), d4) | es2]
          end

        sep([
          par([follow(text(~c"try"), d1, r_ctxt(ctxt1, :break_indent)), hd(es3)])
          | tl(es3)
        ])

      :warning_marker ->
        e = :erl_syntax.warning_marker_info(node)
        beside(text(~c"%% WARNING: "), lay_error_info(e, reset_prec(ctxt)))
      ...
      :type_application ->
        ...
        case :erl_syntax_lib.analyze_type_application(node) do
          ...
          {:list, 1} ->
            ...
            beside(text(~c"["), beside(d1, text(~c"]")))
      ...
      :bitstring_type ->
        ...
        d = seq(d1 ++ d2, floating(text(~c",")), ctxt1, f)

        beside(
          floating(text(~c"<<")),
          beside(par(d), floating(text(~c">>")))
        )
      ...
      :constrained_function_type ->
        ...
        beside(d1, beside(floating(text(~c" when ")), d2))
      ...
      :function_type ->
        ...
        d1 =
          case :erl_syntax.function_type_arguments(node) do
            ...
            arguments ->
              as = seq(arguments, floating(text(~c",")), ctxt1, &lay/2)
              beside(text(~c"("), beside(par(as), floating(text(~c")"))))
          end
      ...
      :map_type ->
        case :erl_syntax.map_type_fields(node) do
          ...
          fs ->
            ctxt1 = reset_prec(ctxt)
            es = seq(fs, floating(text(~c",")), ctxt1, &lay/2)
      ...
      :record_type ->
        ...
        d2 =
          beside(
            d1,
            beside(text(~c"{"), beside(par(es), floating(text(~c"}"))))
          )
      ...
      :tuple_type ->
        case :erl_syntax.tuple_type_elements(node) do
          ...
          elements ->
            es = seq(elements, floating(text(~c",")), reset_prec(ctxt), &lay/2)
    ...
    f = fn t ->
      case :erl_syntax.type(t) do
        :macro ->
          ...
          varName = :erlang.list_to_atom(~c"?" ++ :erlang.atom_to_list(varName0))
  ...
  defp lay_clauses(cs, type, ctxt) do
    vertical(seq(cs, floating(text(~c";")), r_ctxt(ctxt, clause: type), &lay/2))
  end
  ...
  defp append_clause_body(b, d, ctxt) do
    append_clause_body(b, d, floating(text(~c" ->")), ctxt)
  end
  ...
  defp append_guard(g, d, ctxt) do
    par(
      [d, follow(text(~c"when"), g, r_ctxt(ctxt, :sub_indent))],
  ...
  defp lay_bit_types([t | ts], ctxt) do
    beside(
      lay(t, ctxt),
      beside(floating(text(~c"-")), lay_bit_types(ts, ctxt))
    )
  end
  ...
  defp lay_error_info({l, m, t} = t0, ctxt)
    ...
    case (try do
      ...
      s when is_list(s) ->
        cond do
          l > 0 ->
            beside(text(:io_lib.format(~c"~w: ", [l])), text(s))
  ...
  defp lay_type_application(name, arguments, ctxt) do
    ...
    as = seq(arguments, floating(text(~c",")), reset_prec(ctxt), &lay/2)
