defmodule :m_merl_transform do
  ...
  defp pre(t) do
    :merl.switch(
      t,
      [
        {:merl.quote(61, ~c"merl:quote(_@line, _@text) = _@expr"),
        ...
        {:merl.quote(69, [
           ~c"case _@expr of",
           ~c"  merl:quote(_@_, _@text) when _@__@_ -> _@@_; _@_@_ -> 0",
           ~c"end"
         ]), &case_guard/1,
         fn as ->
           case_body(as, t)
  ...
  defp post(t) do
    :merl.switch(
      t,
      [
        {:merl.quote(86, ~c"merl:_@function(_@@args)"),
         [
           ...
           fn [{:args, as}, {:function, f}] ->
             :merl.switch(
               f,
               [
                 {:merl.quote(98, ~c"qquote"),
                  fn [] ->
                    expand_qquote(as, t, 1)
                  end},
                 {:merl.quote(99, ~c"subst"),
                  fn [] ->
                    expand_template(
                      f,
                      as,
    ...
    case :erl_syntax.is_literal(text) do
      true ->
        ...
        case eval_call(line, :quote, as, :failed) do
          ...
          t1 ->
            expand(:merl.qquote(line, ~c"merl:subst(_@tree, _@env)", [{:tree, t1}, {:env, env}]))
        end
  ...
  defp expand_template(f, [pattern | args], t) do
    case :erl_syntax.is_literal(pattern) do
      true ->
        ...
        :merl.qquote(line, ~c"merl:_@function(_@pattern, _@args)", [
          {:function, f},
      ...
      t1 when f === :quote ->
        ...
        case :lists.any(&is_inline_metavar/1, vars) do
          true when is_list(t1) ->
            :merl.qquote(line, ~c"merl:tree([_@template])", [
  ...
  defp pre_expand_match(expr, line, text) do
    ...
    :merl.qquote(line, ~c"{ok, _@out} = merl:match(_@template, _@expr)", [
      {:expr, expr},
      {:out, out},
  ...
  defp var_name(v) when is_integer(v) do
    ...
    :erlang.list_to_atom(~c"Q" ++ :erlang.integer_to_list(v1))
  end
  ...
  defp pre_expand_case(expr, clauses, line) do
    :merl.qquote(line, ~c"merl:switch(_@expr, _@clauses)", [
      {:clauses,
       :erl_syntax.list(
         for c <- clauses do
  ...
  defp pre_expand_case_clause(t) do
    :merl.switch(
      t,
      [
        {:merl.quote(211, ~c"(merl:quote(_@line, _@text)) when _@__@guard -> _@@body"),
        ...
        {:merl.quote(219, ~c"_ -> _@@body"),
         fn env ->
           :merl.qquote(~c"fun () -> _@body end", env)
         end}
      ]
    )
  end
  ...
  defp pre_expand_case_clause(body, guard, line, text) do
    ...
    case guardExprs do
      [] ->
        :merl.qquote(line, [~c"{_@template, ", ~c" fun (_@out) -> _@unused, _@body end}"], param)
      ...
      _ ->
        :merl.qquote(
          ...
          [
            ~c"{_@template, ",
            ~c" fun (_@out) -> _@unused, _@guard end, ",
            ~c" fun (_@out) -> _@unused, _@body end}"
          ],
          param
        )
    end
  ...
  defp dummy_uses(vars) do
    for v <- vars do
      :merl.qquote(250, ~c"_ = _@var", [{:var, :erl_syntax.variable(var_name(v))}])
    end
  end
  ...
  defp make_orelse([c | cs]) do
    :merl.qquote(259, ~c"_@expr orelse _@rest", [
  ...
  defp make_andalso([e | es]) do
    :merl.qquote(265, ~c"_@expr andalso _@rest", [{:expr, e}, {:rest, make_andalso(es)}])
  end
