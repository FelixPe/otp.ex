defmodule :m_merl_transform do
  ...
  defp expand(tree0) do
    tree = pre(tree0)
    post(case (:erl_syntax.subtrees(tree)) do
           [] ->
             tree
           gs ->
    ...
    :merl.switch(t,
                   [{:merl.quote(61, 'merl:quote(_@line, _@text) = _@expr'),
                       ...
                       fn [{:expr, expr}, {:line, line}, {:text, text}] ->
                        ...
                        {:merl.quote(69, ['case _@expr of', '  merl:quote(_@_, _@text) when _@__@_ -> _@@_; _@_@_ -> 0', 'end']), &case_guard/1,
                           fn as ->
                                case_body(as, t)
                           end},
                            fn () ->
    ...
    :merl.switch(t,
                   [{:merl.quote(86, 'merl:_@function(_@@args)'),
                       [{fn [{:args, as}, {:function, f}] ->
                           ...
                           fn [{:args, as}, {:function, f}] ->
                                line = get_location(f)
                                [f1 | as1] = :lists.map(&:erl_syntax.concrete/1,
                                                          [f | as])
  ...
  defp pre_expand_match(expr, line, text) do
    ...
    :merl.qquote(line, '{ok, _@out} = merl:match(_@template, _@expr)',
                   [{:expr, expr}, {:out, out}, {:template,
                                                   :erl_syntax.abstract(template)}])
  end

  defp rewrite_pattern(line, text) do
    t0 = :merl.template(:merl.quote(line, text))
    vars = (for v <- :merl.template_vars(t0),
    ...
    {:merl.alpha(t0,
       ...
       :erl_syntax.list(for v <- vars do
                          :erl_syntax.tuple([:erl_syntax.abstract(var_to_tag(v)),
  ...
  defp pre_expand_case(expr, clauses, line) do
    :merl.qquote(line, 'merl:switch(_@expr, _@clauses)',
                   [{:clauses,
                       :erl_syntax.list(for c <- clauses do
  ...
  defp pre_expand_case_clause(t) do
    :merl.switch(t,
                   [{:merl.quote(211, '(merl:quote(_@line, _@text)) when _@__@guard -> _@@body'),
                       fn [{:body, _}, {:guard, _}, {:line, line}, {:text,
                                                                      text}] ->
                            :erl_syntax.is_literal(text) and :erl_syntax.is_literal(line)
                       end,
                       fn [{:body, body}, {:guard, guard}, {:line, line},
                                                               {:text, text}] ->
                            pre_expand_case_clause(body, guard,
                                                     :erl_syntax.concrete(line),
                                                     :erl_syntax.concrete(text))
                       end},
  ...
  defp pre_expand_case_clause(body, guard, line, text) do
    ...
    param = [{:body, body}, {:guard, guardExprs}, {:out,
                                                     out},
                                                      {:template,
                                                         :erl_syntax.abstract(template)},
                                                          {:unused,
    ...
    case (guardExprs) do
      ...
      _ ->
        :merl.qquote(line, ['{_@template, ', ' fun (_@out) -> _@unused, _@guard end, ', ' fun (_@out) -> _@unused, _@body end}'], param)
    end
  end

  defp dummy_uses(vars) do
    for v <- vars do
      :merl.qquote(250, '_ = _@var',
                     [{:var, :erl_syntax.variable(var_name(v))}])
    end
  end
  ...
  defp make_andalso([e | es]) do
    :merl.qquote(265, '_@expr andalso _@rest',
                   [{:expr, e}, {:rest, make_andalso(es)}])
  end
  ...
  defp is_erlang_var([c | _]) when (c >= ?A and c <= ?Z) or
