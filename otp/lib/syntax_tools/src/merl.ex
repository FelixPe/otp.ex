defmodule :m_merl do
  ...
  def compile_and_load(code, options) do
    case compile(code, options) do
      {:ok, moduleName, binary} ->
        _ = :code.load_binary(moduleName, ~c"", binary)
        {:ok, binary}
  ...
  defp pp_1([g | gs], i) do
    [pp_2(g, i), :lists.duplicate(i, ?\s), ~c"+\n" | pp_1(gs, i)]
  end
  ...
  defp split_forms([], _, [t | _]) do
    fail(~c"incomplete form after ~p", [t])
  end
  ...
  defp parse_error({l, m, r})
       when is_atom(m) and
              is_integer(l) do
    fail(~c"~w: ~ts", [l, m.format_error(r)])
  end

  defp parse_error({{l, c}, m, r})
       when is_atom(m) and
              is_integer(l) and is_integer(c) do
    fail(~c"~w:~w: ~ts", [l, c, m.format_error(r)])
  end
  ...
  defp parse_error(r) do
    fail(~c"unknown parse error: ~tp", [r])
  end
  ...
  defp template_0(tree) do
    case template_1(tree) do
      ...
      {name} when is_list(name) ->
        fail(~c"bad metavariable: '~s'", [tl(name)])
  ...
  defp template_2([g | gs], as, bool) do
    case template_3(g, [], false) do
      ...
      {~c"v" ++ cs} when is_list(cs) ->
        {:erlang.list_to_atom(cs)}

      {~c"n" ++ cs} when is_list(cs) ->
        {:erlang.list_to_integer(cs)}
      ...
      var > 9 and rem(var, 10) === 9 ->
        cond do
          var > 99 and rem(var, 100) === 99 ->
            name = ~c"Q" ++ :erlang.integer_to_list(div(var, 100))
  ...
  defp tree_1({var}) when is_atom(var) do
    :erl_syntax.atom(:erlang.list_to_atom(~c"@" ++ :erlang.atom_to_list(var)))
  end

  defp tree_1({var}) when is_integer(var) do
    :erl_syntax.integer(:erlang.list_to_integer(~c"909" ++ :erlang.integer_to_list(var)))
  end
  ...
  defp tree_1({:*, var}) when is_integer(var) do
    :erl_syntax.integer(:erlang.list_to_integer(~c"9099" ++ :erlang.integer_to_list(var)))
  end
  ...
  defp match_glob([{:*, var} | _], _, _, _) do
    fail(~c"multiple glob variables in same match group: ~w", [var])
  end
