defmodule :m_erl_recomment do
  ...
  defp standalone_comment({l, col, _Ind, text}) do
    leaf_node(l, l + comment_delta(text),
                :erl_syntax.set_pos(:erl_syntax.comment(col - 1, text),
                                      anno(l)))
  end
  ...
  defp build_tree(node) do
    ...
    case (:erl_syntax.subtrees(node)) do
      ...
      ts ->
        ...
        tree_node(minpos(l, min), :erlang.max(l, max),
                    :erl_syntax.type(node), :erl_syntax.get_attrs(node),
                    subtrees)
    end
  end
  ...
  defp expand_comment(c) do
    {l, _Col, ind, text} = c
    :erl_syntax.set_pos(:erl_syntax.comment(ind, text),
                          anno(l))
  end
  ...
  Record.defrecord(:r_leaf, :leaf, min: 0, max: 0,
                                precomments: [], postcomments: [],
                                value: :undefined)
  Record.defrecord(:r_tree, :tree, min: 0, max: 0,
                                type: :undefined, attrs: :undefined,
                                precomments: [], postcomments: [], subtrees: [])
  Record.defrecord(:r_list, :list, min: 0, max: 0,
                                subtrees: [])
  ...
  defp tree_node(min, max, type, attrs, subtrees) do
    r_tree(min: min, max: max, type: type, attrs: attrs,
        subtrees: subtrees)
  end
