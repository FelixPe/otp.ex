defmodule :m_mnesia_dumper do
  ...
  defp perform_dump(initBy, regulator)
    ...
    :ok
    dbg_out(~c"Transaction log dump initiated by ~w~n", [initBy])
  ...
  defp perform_dump(initBy, regulator) do
    ...
    dbg_out(~c"Transaction log dump initiated by ~w: ~w~n", [initBy, logState])
    ...
    case logState do
      ...
      {:error, reason} ->
        {:error, {~c"Cannot prepare log dump", reason}}
    end
  end
    ...
    case :mnesia_log.chunk_log(cont) do
      {c2, recs} ->
        try do
          ...
          _, r when r !== :fatal ->
            reason = {~c"Transaction log dump error: ~tp~n", [{r, __STACKTRACE__}]}
  ...
  defp insert_rec(h, _InPlace, _InitBy, _LogV)
    ...
    cond do
      r_log_header(h, :log_kind) != :trans_log ->
        exit({~c"Bad kind of transaction log", h})

      r_log_header(h, :log_version) == currentVersion ->
        :ok

      r_log_header(h, :log_version) == ~c"4.2" ->
        :ok

      r_log_header(h, :log_version) == ~c"4.1" ->
        :ok

      r_log_header(h, :log_version) == ~c"4.0" ->
        :ok

      true ->
        fatal(~c"Bad version of transaction log: ~p~n", [h])
    end
  ...
  defp perform_update(tid, schemaOps, _DumperMode, _UseDir) do
    ...
    try do
      ...
      _, reason when reason !== :fatal ->
        error = {:error, {~c"Schema update error", {reason, __STACKTRACE__}}}
        close_files(inPlace, error, initBy)
        fatal(~c"Schema update error ~tp ~tp", [{reason, __STACKTRACE__}, schemaOps])
    end
  end
  ...
  defp try_create_disc_only_copy(:disc_only_copies, tab, cs, detsProps) do
    ...
    case :mnesia_monitor.open_dets(tab, args) do
      ...
      {:error, error} ->
        exit({~c"Failed to create dets table", error})
    end
  end
    ...
    case :file.read_file_info(dclF) do
      ...
      {:ok, dclInfo} ->
        ...
        case :file.read_file_info(dcdF) do
          {:error, reason} ->
            :mnesia_lib.dbg_out(~c"File ~tp info_error ~tp ~n", [dcdF, reason])
            true
    ...
    case updateInPlace do
      ...
      false ->
        ...
        try do
          ...
          :error, error ->
            fatal(~c"Cannot copy dets file ~tp to ~tp: ~tp~n", [dat, tmp, error])
        end
    ...
    case :mnesia_monitor.use_dir() do
      true ->
        ...
        case :mnesia_lib.dets_sync_open(tabRef, args) do
          {:ok, ^tabRef} ->
            ...
            try do
              ...
              _, reason ->
                ...
                exit({~c"Dump of table to disc failed", reason})
            after
          ...
          {:error, reason} ->
            :mnesia_lib.unlock_table(tab)
            exit({~c"Open of file before dump to disc failed", reason})
        end
    ...
    case :mnesia_monitor.get_env(:dump_log_load_regulation) do
      ...
      true ->
        ...
        case :mnesia_monitor.start_proc(n, :mnesia_dumper, :regulator_init, [self()]) do
          ...
          {:error, reason} ->
            fatal(~c"Failed to start ~n: ~tp~n", [n, reason])
        end
    end
  end
