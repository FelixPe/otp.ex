defmodule :m_mnesia_dumper do
  ...
  def incr_log_writes() do
    left = :mnesia_lib.incr_counter(:trans_log_writes_left,
                                      - 1)
    cond do
      left === 0 ->
        spawn(fn () ->
  ...
  def opt_dump_log(initBy) do
    reg = (case (:erlang.whereis(:mnesia_dumper_load_regulator)) do
             :undefined ->
               :nopid
             pid when is_pid(pid) ->
               pid
  ...
  defp perform_dump(initBy, regulator)
    ...
    scan_decisions(:mnesia_log.previous_log_file(), initBy,
                     regulator)
    scan_decisions(:mnesia_log.latest_log_file(), initBy,
                     regulator)
  end
  ...
  defp scan_decisions(fname, initBy, regulator) do
    ...
    case (exists) do
      ...
      true ->
        ...
        cont = :start
        try do
          do_perform_dump(cont, false, initBy, regulator,
                            :undefined)
        catch
          :exit, reason when reason !== :fatal ->
            {:error, reason}
  ...
  defp do_perform_dump(cont, inPlace, initBy, regulator, oldVersion) do
    case (:mnesia_log.chunk_log(cont)) do
      {c2, recs} ->
        try do
          insert_recs(recs, inPlace, initBy, regulator,
                        oldVersion)
        catch
          _, r when r !== :fatal ->
  ...
  defp insert_recs([rec | recs], inPlace, initBy, regulator,
            logV) do
    regulate(regulator)
    case (insert_rec(rec, inPlace, initBy, logV)) do
      logH when elem(logH, 0) === :log_header ->
        insert_recs(recs, inPlace, initBy, regulator,
                      r_log_header(logH, :log_version))
      _ ->
  ...
  defp insert_rec(rec, inPlace, initBy, logV)
    ...
    case (:mnesia_recover.wait_for_decision(d, initBy)) do
      {tid, :committed} ->
        do_insert_rec(tid, :mnesia_tm.new_cr_format(rec),
                        inPlace, initBy, logV)
      {tid, :aborted} ->
        case (initBy) do
          :startup ->
  ...
  defp do_insert_rec(tid, rec, inPlace, initBy, logV) do
    case (r_commit(rec, :schema_ops)) do
      ...
      schemaOps ->
        case (val({:schema, :storage_type})) do
          :ram_copies ->
            insert_ops(tid, :schema_ops, schemaOps, inPlace, initBy,
                         logV)
          storage ->
    ...
    case (initBy) do
      :startup ->
        ...
        insert_ops(tid, :disc_only_copies, dO, inPlace, initBy,
                     logV)
      _ ->
        :ignore
    end
  ...
  defp insert_ext_ops(tid, extOps, inPlace, initBy) do
    :lists.foreach(fn {ext, op} ->
                        case (storage_semantics(ext)) do
                          semantics when semantics == :disc_copies or
  ...
  defp disc_insert(_Tid, storage, tab, key, val, op, inPlace,
            initBy) do
    semantics = storage_semantics(storage)
    case (open_files(tab, semantics, storage, inPlace,
                       initBy)) do
      true ->
        case (semantics) do
          :disc_copies when tab != :schema ->
            :mnesia_log.append({:mnesia_dumper, tab},
                                 {{tab, key}, val, op})
            :ok
          _ ->
      ...
      :update_counter ->
        case (dets_incr_counter(tab, key)) do
          true ->
            {recName, incr} = val
            try do
              _ = :mnesia_lib.db_update_counter(storage, tab, key,
                                                  incr)
            catch
              :error, _ when incr < 0 ->
                zero = {recName, key, 0}
  ...
  defp dets_updated(tab, key) do
    case (:erlang.get(:mnesia_dumper_dets)) do
      :undefined ->
        ...
        tree = :gb_trees.insert(tab, :gb_sets.singleton(key),
                                  empty)
  ...
  defp insert(tid, storage, tab, key, [val | tail], op,
            inPlace, initBy) do
    insert(tid, storage, tab, key, val, op, inPlace, initBy)
    insert(tid, storage, tab, key, tail, op, inPlace,
             initBy)
  end

  defp insert(_Tid, _Storage, _Tab, _Key, [], _Op, _InPlace,
            _InitBy) do
    :ok
  end

  defp insert(tid, storage, tab, key, val, op, inPlace,
            initBy) do
    semantics = storage_semantics(storage)
    item = {{tab, key}, val, op}
    case (initBy) do
      :startup ->
        disc_insert(tid, storage, tab, key, val, op, inPlace,
                      initBy)
  ...
  defp insert_op(tid, storage, {{tab, key}, val, op}, inPlace,
            initBy) do
    disc_insert(tid, storage, tab, key, val, op, inPlace,
                  initBy)
  end
  ...
  defp insert_op(tid, _, {:op, :rec, storage, item}, inPlace,
            initBy) do
    {{tab, key}, valList, op} = item
    insert(tid, storage, tab, key, valList, op, inPlace,
             initBy)
  end

  defp insert_op(tid, _,
    ...
    cond do
      n == node() ->
        ...
        case ({fromS, toS}) do
          {{:ext, _FromAlias, _FromMod},
             {:ext, toAlias, toMod}} ->
            disc_delete_table(tab, fromS)
            :ok = toMod.delete_table(toAlias, tab)
            :ok = :mnesia_monitor.unsafe_create_external(tab,
                                                           toAlias, toMod,
                                                           tabDef)
            :ok = toMod.load_table(toAlias, tab,
                                     {:dumper, :change_table_copy_type}, tabDef)
          ...
          _NoneAreExt ->
            case ({fromSem, toSem}) do
              ...
              {:disc_only_copies, :ram_copies}
                ...
                disc_delete_table(tab, fromS)
              {:disc_only_copies, :ram_copies} when :erlang.element(1,
                                                                      fromS) == :ext
                                                    ->
                change_disc_to_ram(tab, cs, fromS, toS, logtmp, initBy)
              {:disc_copies, :disc_only_copies} ->
    ...
    disc_insert(tid, s, :schema, tab, val, :write, inPlace,
                  initBy)
  end

  defp insert_op(tid, _, {:op, :transform, _Fun, tabDef},
            inPlace, initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
  ...
  defp insert_op(tid, _, {:op, :restore_recreate, tabDef},
            inPlace, initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    ...
    insert_op(tid, :ignore, {:op, :create_table, tabDef},
                inPlace, initBy)
  end

  defp insert_op(tid, _, {:op, :create_table, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    ...
    case (initBy) do
            ...
            case (:mnesia_lib.exists(dcd)) do
              ...
              false ->
                :mnesia_log.open_log(:temp,
                                       :mnesia_log.dcd_log_header(), dcd, false,
                                       false, :read_write)
      ...
      _ ->
        ...
        active = :mnesia_lib.intersect(copies,
  ...
  defp insert_op(_Tid, _, {:op, :dump_table, size, tabDef},
            _InPlace, _InitBy) do
    case (size) do
      :unknown ->
        :ignore
  ...
  defp insert_op(tid, _, {:op, :delete_table, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    tab = r_cstruct(cs, :name)
    case (:mnesia_lib.cs_to_storage_type(node(), cs)) do
  ...
  defp insert_op(tid, _, {:op, :clear_table, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    tab = r_cstruct(cs, :name)
    case (:mnesia_lib.cs_to_storage_type(node(), cs)) do
      ...
      storage ->
        ...
        try do
          insert(tid, storage, tab, :_, oid, :clear_table,
                   inPlace, initBy)
        catch
          _, _Reason ->
            {:EXIT, _Reason}
  ...
  defp insert_op(tid, _, {:op, :merge_schema, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
  ...
  defp insert_op(tid, _,
            {:op, :del_table_copy, storage, node, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    tab = r_cstruct(cs, :name)
    cond do
  ...
  defp insert_op(tid, _,
            {:op, :add_table_copy, _Storage, _Node, tabDef},
            inPlace, initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    insert_cstruct(tid, cs, true, inPlace, initBy)
  end

  defp insert_op(tid, _, {:op, :add_snmp, _Us, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    insert_cstruct(tid, cs, true, inPlace, initBy)
  end

  defp insert_op(tid, _, {:op, :del_snmp, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    tab = r_cstruct(cs, :name)
    storage = :mnesia_lib.cs_to_storage_type(node(), cs)
  ...
  defp insert_op(tid, _, {:op, :add_index, pos, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    ...
    case (initBy) do
      :startup when semantics == :disc_only_copies ->
        true = open_files(tab, semantics, storage, inPlace,
                            initBy)
  ...
  defp insert_op(tid, _, {:op, :del_index, pos, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    tab = r_cstruct(cs, :name)
    storage = :mnesia_lib.cs_to_storage_type(node(), cs)
  ...
  defp insert_op(tid, _,
            {:op, :change_table_access_mode, tabDef, _OldAccess,
               _Access},
            inPlace, initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    case (initBy) do
  ...
  defp insert_op(tid, _,
            {:op, :change_table_majority, tabDef, _OldAccess,
               _Access},
            inPlace, initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    case (initBy) do
  ...
  defp insert_op(tid, _,
            {:op, :change_table_load_order, tabDef, _OldLevel,
               _Level},
            inPlace, initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    insert_cstruct(tid, cs, true, inPlace, initBy)
  ...
  defp insert_op(tid, _,
            {:op, :delete_property, tabDef, propKey}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    tab = r_cstruct(cs, :name)
  ...
  defp insert_op(tid, _, {:op, :write_property, tabDef, _Prop},
            inPlace, initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    insert_cstruct(tid, cs, true, inPlace, initBy)
  end
  ...
  defp insert_op(tid, _,
            {:op, :change_table_frag, _Change, tabDef}, inPlace,
            initBy) do
    cs = :mnesia_schema.list2cs(tabDef)
    insert_cstruct(tid, cs, true, inPlace, initBy)
  end
  ...
  defp open_files(tab, storage, updateInPlace, initBy) do
    open_files(tab, storage_semantics(storage), storage,
                 updateInPlace, initBy)
  end
  ...
  defp open_files(_Tab, _Semantics, _Storage, _UpdateInPlace,
            _InitBy) do
    false
  end
  ...
  defp open_disc_copies(tab, initBy) do
    ...
    cond do
      dumpEts == false or initBy == :startup ->
        ...
        :mnesia_log.open_log({:mnesia_dumper, tab},
                               :mnesia_log.dcl_log_header(), dclF,
  ...
  defp close_files(updateInPlace, outcome, initBy) do
    close_files(updateInPlace, outcome, initBy,
                  :erlang.get())
  end

  defp close_files(inPlace, outcome, initBy,
            [{{:mnesia_dumper, tab}, :already_dumped} | tail]) do
    :erlang.erase({:mnesia_dumper, tab})
  ...
  defp close_files(inPlace, outcome, initBy,
    ...
    case (storage_semantics(storage)) do
      :disc_only_copies when initBy != :startup ->
        :ignore
      :disc_copies when (storage != :unknown and
                           tab != :schema)
                        ->
  ...
  defp insert_cstruct(tid, cs, keepWhereabouts, inPlace, initBy) do
    val = :mnesia_schema.insert_cstruct(tid, cs,
                                          keepWhereabouts)
    {:schema, tab, _} = val
    s = val({:schema, :storage_type})
    disc_insert(tid, s, :schema, tab, val, :write, inPlace,
  ...
  defp delete_cstruct(tid, cs, inPlace, initBy) do
    ...
    {:schema, tab, _} = val
    s = val({:schema, :storage_type})
    disc_insert(tid, s, :schema, tab, val, :delete, inPlace,
                  initBy)
    tab
  end
  ...
  defp temp_set_master_nodes() do
    ...
    masters = (for tab <- tabs do
                 {tab,
                    (val({tab, :disc_copies}) ++ val({tab,
                                                        :ram_copies}) ++ val({tab,
                                                                                :disc_only_copies}) ++ external_copies(tab)) -- [node()]}
               end)
  ...
  def dump_to_logfile(storage, tab) do
    case (:mnesia_monitor.use_dir()) do
      true ->
        logtmp = :mnesia_lib.tab2logtmp(tab)
        :file.delete(logtmp)
        case (:disk_log.open([{:name, make_ref()}, {:file,
                                                      logtmp},
                                                       {:repair, false},
                                                           {:linkto,
                                                              self()}])) do
  ...
  defp do_dump_to_logfile(storage, tab, fd) do
    pat = [{:_, [], [:"$_"]}]
    log_terms(:mnesia_lib.db_select_init(storage, tab, pat,
                                           100),
                storage, tab, pat, fd)
  end
  ...
  defp log_terms({objs, cont}, storage, tab, pat, fd) do
    ...
    log_terms(:mnesia_lib.db_select_cont(storage, cont, :_),
                storage, tab, pat, fd)
  end
  ...
  def load_from_logfile(storage, tab, f) do
    case (:disk_log.open([{:name, make_ref()}, {:file, f},
                                                   {:repair, true}, {:linkto,
                                                                       self()}])) do
      {:ok, fd} ->
        chunk_from_log(:disk_log.chunk(fd, :start), fd, storage,
                         tab)
      {:repaired, fd, _, _} ->
        chunk_from_log(:disk_log.chunk(fd, :start), fd, storage,
                         tab)
  ...
  defp chunk_from_log({cont, terms}, fd, storage, tab) do
    ...
    chunk_from_log(:disk_log.chunk(fd, cont), fd, storage,
                     tab)
  end
  ...
  def start_regulator() do
    case (:mnesia_monitor.get_env(:dump_log_load_regulation)) do
      ...
      true ->
        ...
        case (:mnesia_monitor.start_proc(n, :mnesia_dumper,
                                           :regulator_init, [self()])) do
          {:ok, pid} ->
            pid
          {:error, reason} ->
