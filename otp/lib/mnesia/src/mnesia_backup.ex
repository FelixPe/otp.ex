defmodule :m_mnesia_backup do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_backup, :backup, tmp_file: :undefined,
                                  file: :undefined, file_desc: :undefined)
  def open_write(opaqueData) do
    file = opaqueData
    tmp = :lists.concat([file, '.BUPTMP'])
    :file.delete(tmp)
    case (:disk_log.open([{:name, make_ref()}, {:file, tmp},
                                                   {:repair, false}, {:linkto,
                                                                        self()}])) do
      {:ok, fd} ->
  ...
  def write(opaqueData, backupItems) do
    b = opaqueData
    case (:disk_log.log_terms(r_backup(b, :file_desc),
                                backupItems)) do
      :ok ->
        {:ok, b}
      {:error, reason} ->
  ...
  Record.defrecord(:r_restore, :restore, file: :undefined,
                                   file_desc: :undefined, cont: :undefined)
  def open_read(opaqueData) do
    ...
    case (:file.read_file_info(file)) do
      {:error, reason} ->
        {:error, reason}
      _FileInfo ->
        case (:disk_log.open([{:file, file}, {:name,
                                                make_ref()},
                                                 {:repair, false}, {:mode,
                                                                      :read_only},
                                                                       {:linkto,
