defmodule :m_mnesia_index do
  ...
  defp del_object_index2([{{pos, type}, ixt} | tail], soB, storage, tab, key, obj) do
    valsF = index_vals_f(storage, tab, pos)
  ...
  defp del_object_index3([ixK | ixKs], found, type, tab, key, obj, ixt) do
  ...
  def tab2filename(tab, {a}) when is_atom(a) do
    :mnesia_lib.dir(tab) ++ ~c"_-" ++ :erlang.atom_to_list(a) ++ ~c"-.DAT"
  end
  ...
  def tab2filename(tab, pos) when is_integer(pos) do
    :mnesia_lib.dir(tab) ++ ~c"_" ++ :erlang.integer_to_list(pos) ++ ~c".DAT"
  end
  ...
  defp add_ram_index(tab, storage, {pos, _Pref}) do
    type = :ordered
    verbose(~c"Creating index for ~tw ~p ~p~n", [tab, pos, type])
