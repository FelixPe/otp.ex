defmodule :m_mnesia_event do
  ...
  defp handle_any_event(msg, state) do
    report_error(~c"~p got unexpected event: ~tp~n", [:mnesia_event, msg])
    {:ok, state}
  end

  defp handle_table_event({oper, record, transId}, state) do
    report_info(~c"~p performed by ~p on record:~n\t~tp~n", [oper, transId, record])
    {:ok, state}
  end
  ...
  defp handle_system_event(
    ...
    report_error(~c"Checkpoint '~p' has been deactivated, last table copy deleted.\n", [
      checkpoint
    ])
    ...
    case :mnesia.system_info(:fallback_activated) and node !== node() do
      true ->
        case :mnesia_monitor.get_env(:fallback_error_function) do
          {:mnesia, :lkill} ->
            msg =
              ~c"A fallback is installed and Mnesia must be restarted. Forcing shutdown after mnesia_down from ~p...~n"
          ...
          {userMod, userFunc} ->
            msg = ~c"Warning: A fallback is installed and Mnesia got mnesia_down from ~p. ~n"
  ...
  defp handle_system_event({:mnesia_overload, details}, state) do
    report_warning(~c"Mnesia is overloaded: ~tw~n", [details])
    {:ok, state}
  end
  ...
  defp handle_system_event(
    ...
    report_error(~c"mnesia_event got {inconsistent_database, ~tw, ~w}~n", [reason, node])
    {:ok, state}
  end

  defp handle_system_event({:mnesia_user, event}, state) do
    report_info(~c"User event: ~tp~n", [event])
    {:ok, state}
  end

  defp handle_system_event(msg, state) do
    report_error(~c"mnesia_event got unexpected system event: ~tp~n", [msg])
    {:ok, state}
  end

  defp report_info(format0, args0) do
    format = ~c"Mnesia(~p): " ++ format0
    args = [node() | args0]
  ...
  defp report_warning(format0, args0) do
    format = ~c"Mnesia(~p): ** WARNING ** " ++ format0
    args = [node() | args0]
  ...
  defp report_error(format0, args0) do
    format = ~c"Mnesia(~p): ** ERROR ** " ++ format0
    args = [node() | args0]
    :error_logger.format(format, args)
  ...
  defp report_fatal(format, args, binaryCore, coreDumped) do
    ...
    cond do
      ...
      true ->
        report_error(~c"(ignoring core) ** FATAL ** " ++ format, args)
    end
  end
  ...
  defp core_file(coreDir, binaryCore, format, args) do
    ...
    case :file.write_file(coreFile, binaryCore) do
      :ok ->
        report_error(~c"(core dumped to file: ~p)~n ** FATAL ** " ++ format, [coreFile] ++ args)

      {:error, reason} ->
        report_error(
          ~c"(could not write core file: ~p)~n ** FATAL ** " ++ format,
          [reason] ++ args
        )
    end
  end
