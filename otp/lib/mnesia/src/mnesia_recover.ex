defmodule :m_mnesia_recover do
  ...
  def note_log_decision(h, _InitBy)
    ...
    cond do
      ...
      r_log_header(h, :log_version) == ~c"2.0" ->
        verbose(~c"Accepting an old version format of decision log: ~p~n", [v])
        :ok

      true ->
        fatal(~c"Bad version of decision log: ~p~n", [h])
    end
  end
  ...
  def note_log_decision(h, _InitBy)
    ...
    cond do
      ...
      true ->
        fatal(~c"Bad version of decision tab: ~p~n", [h])
    end
  end
  ...
  def note_log_decision(badItem, initBy) do
    exit({~c"Bad decision log item", badItem, initBy})
  end
  ...
  def init([parent]) do
    ...
    :mnesia_lib.verbose(~c"~p starting: ~p~n", [:mnesia_recover, self()])
  ...
  def handle_call({:wait_for_decision, d}, from, state) do
    ...
    cond do
      ...
      true ->
        verbose(~c"Transaction ~p is unclear. Wait for disc nodes: ~w ram: ~w~n", [
          r_decision(d, :tid),
          remoteDisc,
          aliveRam
        ])
  ...
  def handle_call(msg, _From, state) do
    :erlang.error(~c"~p got unexpected call: ~tp~n", [:mnesia_recover, msg])
    {:noreply, state}
  end
  ...
  def handle_cast(msg, state) do
    :erlang.error(~c"~p got unexpected cast: ~tp~n", [:mnesia_recover, msg])
    {:noreply, state}
  end
  ...
  def handle_info({:force_decision, tid}, state) do
    case r_state(state, :unclear_decision) do
      u when r_decision(u, :tid) == tid ->
        verbose(
          ~c"Decided to abort transaction ~p since max_wait_for_decision has been exceeded~n",
          [tid]
        )
  ...
  def handle_info({:EXIT, pid, r}, state)
    ...
    :mnesia_lib.dbg_out(~c"~p was ~tp~n", [:mnesia_recover, r])
    {:stop, :shutdown, state}
  end

  def handle_info(msg, state) do
    :erlang.error(~c"~p got unexpected info: ~tp~n", [:mnesia_recover, msg])
    {:noreply, state}
  end
      ...
      u when r_decision(u, :tid) == tid ->
        ...
        cond do
          outcome == :unclear and waitFor == [] ->
            ...
            verbose(~c"Decided to abort transaction ~p since everybody are uncertain ~p~n", [
              tid,
              certainD
            ])
          ...
          outcome != :unclear ->
            verbose(~c"~p told us that transaction ~p was ~p~n", [node, tid, outcome])
