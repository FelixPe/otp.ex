defmodule :m_mnesia_registry do
  ...
  require Record
  Record.defrecord(:r_state, :state, table: :undefined, ops: [],
                                 link_to: :undefined)
  Record.defrecord(:r_registry_entry, :registry_entry, key: :undefined,
                                          key_size: :undefined,
                                          val_type: :undefined,
                                          val_size: :undefined, val: :undefined)
  Record.defrecord(:r_size, :size, pid: self(), n_values: 0,
                                largest_key: 0, largest_val: 0)
  ...
  defp dump_loop(s) do
    ...
    receive do
      {:write, key, keySize, valType, valSize, val} ->
        rE = r_registry_entry(key: key, key_size: keySize, val_type: valType,
                 val_size: valSize, val: val)
      ...
      {:commit, replyTo} ->
        ...
        case (:mnesia.transaction(&handle_ops/3,
                                    [tab, recName, ops])) do
          {:atomic, :ok} ->
  ...
  defp calc_size([h | t], s) do
    keySize = max(:erlang.element(r_registry_entry(:key_size), h),
                    r_size(s, :largest_key))
    valSize = max(:erlang.element(r_registry_entry(:val_size), h),
                    r_size(s, :largest_val))
    n = r_size(s, :n_values) + 1
    calc_size(t,
                r_size(s, n_values: n,  largest_key: keySize, 
                       largest_val: valSize))
  end
  ...
  defp send_records([h | t], replyTo) do
    ...
    send(replyTo, {:restore, keySize, valSize, valType, key,
                     val})
