defmodule :m_mnesia_loader do
  ...
  def disc_load_table(tab, reason, cs) do
  ...
  defp do_get_disc_copy2(tab, reason, storage, type)
    ...
    case (reason) do
      ...
      _ ->
        ...
        case (:mnesia_monitor.get_env(:dump_disc_copies_at_startup) and :mnesia_dumper.needs_dump_ets(tab)) do
          true ->
            ...
            case (:mnesia_lib.exists(fname)) do
              ...
              false ->
                case (:mnesia_lib.exists(datname)) do
                  true ->
                    :mnesia_lib.dets_to_ets(tab, tab, datname, type, repair,
                                              :no)
                  false ->
                    false
                end
  ...
  defp do_get_disc_copy2(tab, reason, storage = {:ext, alias, mod},
            _Type) do
    cs = val({tab, :cstruct})
  ...
  def net_load_table(tab, {:dumper, {:add_table_copy, _}} = reason,
           ns, cs) do
    try_net_load_table(tab, reason, ns, cs)
  end

  def net_load_table(tab, reason, ns, _Cs) do
    try_net_load_table(tab, reason, ns,
                         try do
  ...
  defp try_net_load_table(tab, reason, ns, cs) do
    storage = (case (elem(cs, 0) === :cstruct) do
                 true ->
                   :mnesia_lib.cs_to_storage_type(node(), cs)
                 false ->
                   :unknown
               end)
      ...
      true ->
        ...
        case (init_receiver(node, tab, storage, cs, reason)) do
          :ok ->
            ...
            {:loaded, :ok}
          err = {:error, _} when :erlang.element(1,
                                                   reason) == :dumper
                                 ->
  ...
  defp init_receiver(node, tab, storage, cs,
            reas = {:dumper, {:add_table_copy, tid}}) do
    :rpc.call(node, :mnesia_lib, :set,
                [{:mnesia_loader, :active_trans}, tid])
    case (start_remote_sender(node, tab, storage,
                                {:add_table_copy, tid})) do
      {senderPid, tabSize, detsData} ->
        start_receiver(tab, storage, cs, senderPid, tabSize,
                         detsData, reas)
      else__ ->
        else__
    end
    ...
    load = fn () ->
                ...
                case (val({tab, :where_to_read}) == node()) do
                  ...
                  _ ->
                    ...
                    {senderPid, tabSize,
                       detsData} = start_remote_sender(node, tab, storage,
                                                         :load)
                    init = table_init_fun(senderPid, storage)
                    args = [self(), tab, storage, cs, senderPid, tabSize,
                                                                     detsData,
                                                                         init]
                    pid = spawn_link(:mnesia_loader, :spawned_receiver,
                                       args)
                    :erlang.put(:mnesia_real_loader, pid)
                    wait_on_load_complete(pid)
                end
           end
    res = (case (:mnesia.transaction(load, 20)) do
             {:atomic, {:error, result}} when :erlang.element(1,
                                                                reason) == :dumper
                                              ->
               {:error, result}
             {:atomic, {:error, result}} ->
  ...
  defp start_remote_sender(node, tab, storage, why) do
    :mnesia_controller.start_remote_sender(node, tab,
                                             self(), storage, why)
    ...
    receive do
      {senderPid, {:first, _} = msg}
        ...
        {sz, data} = mod.receiver_first_message(senderPid, msg,
                                                  alias, tab)
        {senderPid, sz, data}
  ...
  defp table_init_fun(senderPid, storage) do
    fn :read ->
         subscrCache = :ets.new(:subscr_cache,
                                  [:private, :ordered_set])
         :erlang.put(:mnesia_table_receiver_subscr_cache,
                       subscrCache)
         receiver = self()
  ...
  defp start_receiver(tab, storage, cs, senderPid, tabSize, detsData,
            {:dumper, {:add_table_copy, _}}) do
    init = table_init_fun(senderPid, storage)
    case (do_init_table(tab, storage, cs, senderPid,
                          tabSize, detsData, self(), init)) do
      err = {:error, _} ->
  ...
  def spawned_receiver(replyTo, tab, storage, cs, senderPid, tabSize,
           detsData, init) do
    :erlang.process_flag(:trap_exit, true)
    done = do_init_table(tab, storage, cs, senderPid,
                           tabSize, detsData, replyTo, init)
    try do
  ...
  defp do_init_table(tab, storage, cs, senderPid, tabSize, detsInfo,
            origTabRec, init) do
    case (create_table(tab, tabSize, storage, cs)) do
      {^storage, ^tab} ->
        node = node(senderPid)
        :erlang.put(:mnesia_table_receiver,
                      {tab, node, senderPid})
        :mnesia_tm.block_tab(tab)
        case (init_table(tab, storage, init, detsInfo,
                           senderPid)) do
          :ok ->
      ...
      storage == :disc_only_copies ->
        ...
        detsOpts = :lists.keydelete(:estimated_no_objects, 1,
                                      :proplists.get_value(:dets, storageProps,
                                                             []))
        args = [{:file, tmp}, {:keypos, 2},
                                  {:estimated_no_objects, size}, {:repair,
                                                                    :mnesia_monitor.get_env(:auto_repair)},
                                                                     {:type,
                                                                        :mnesia_lib.disk_type(tab,
                                                                                                r_cstruct(cs, :type))} |
  ...
  defp get_data(pid, tabRec, storage) do
    receive do
      ...
      {:mnesia_table_event, _} = subscrEvent ->
        subscrCache = :erlang.get(:mnesia_table_receiver_subscr_cache)
        :ets.insert(subscrCache,
                      {:erlang.unique_integer([:monotonic]), subscrEvent})
        get_data(pid, tabRec, storage)
    end
  end
  ...
  defp ext_init_table(alias, mod, tab, fun, state, sender) do
    :ok = ext_load_table(mod, alias, tab,
                           {:net_load, node(sender)})
    ext_init_table(:read, alias, mod, tab, fun, state,
                     sender)
  end

  defp ext_load_table(mod, alias, tab, reason) do
    cS = val({tab, :cstruct})
    mod.load_table(alias, tab, reason,
  ...
  defp ext_init_table(action, alias, mod, tab, fun, state, sender) do
    case (fun.(action)) do
      ...
      {data, newFun} ->
        case (mod.receive_data(data, alias, tab, sender,
                                 state)) do
          {:more, newState} ->
            ext_init_table({:read, :more}, alias, mod, tab, newFun,
                             newState, sender)
          {{:more, msg}, newState} ->
            ext_init_table({:read, msg}, alias, mod, tab, newFun,
                             newState, sender)
        end
      :end_of_input ->
  ...
  defp init_table(tab, :disc_only_copies, fun, detsInfo,
            sender) do
  ...
  defp finish_copy(storage, tab, cs, senderPid, datBin,
            origTabRec) do
    tabRef = {storage, tab}
    case (handle_last(tabRef, r_cstruct(cs, :type), datBin)) do
      :ok ->
        subscr_postprocess(tabRef, r_cstruct(cs, :record_name))
        send(senderPid, {origTabRec, :no_more})
        ref = :erlang.monitor(:process, senderPid)
        subscr_receiver(tabRef, r_cstruct(cs, :record_name), ref)
        :mnesia_index.init_index(tab, storage)
        snmpify(tab, storage)
  ...
  defp subscr_postprocess(tabRef, recName) do
    case (:erlang.get(:mnesia_table_receiver_subscr_cache)) do
      :undefined ->
        :ok
      subscrCache ->
        :ets.foldl(fn {_, event}, _Acc ->
  ...
  defp subscr_receiver(tabRef = {_, tab}, recName, ref) do
    receive do
      {:mnesia_table_event, {_Op, val, _Tid}} = event
          when :erlang.element(1, val) === tab or
                 :erlang.element(1, val) === :schema
               ->
        handle_subscr_event(event, tabRef, recName)
  ...
  defp handle_subscr_event(event, tabRef = {_, tab}, recName) do
    case (event) do
      {:mnesia_table_event, {op, val, _Tid}}
        ...
        cond do
          ...
          true ->
            handle_event(tabRef, op,
                           :erlang.setelement(1, val, recName))
        end
      {:mnesia_table_event, {op, val, _Tid}}
  ...
  defp down(tab, storage) do
    ...
    :mnesia_controller.sync_del_table_copy_whereabouts(tab,
                                                         node())
    :mnesia_tm.unblock_tab(tab)
    flush_subcrs()
    :down
  ...
  defp max_transfer_size() do
    maxTransferSize = 64000
    case (try do
            :ets.lookup_element(:mnesia_gvar, :max_transfer_size, 2)
          catch
            :error, _ ->
              {:EXIT, {:badarg, []}}
          end) do
  ...
  defp calc_nokeys(storage, tab) do
    ...
    div(max_transfer_size(), binSize) + 1
  end

  def send_table(pid, tab, remoteS, reason) do
    case (try do
  ...
  defp do_send_table(pid, tab, storage, remoteS, loadReason) do
    :erlang.put(:mnesia_table_sender, {tab, node(pid), pid})
    try do
      (
        {init, chunk} = get_chunk_func(pid, tab, storage,
                                         remoteS)
        needLock = need_lock(tab, loadReason)
        case (prepare_copy(pid, tab, storage, needLock)) do
          {:atomic, :ok} ->
  ...
  defp need_lock(tab, {:add_table_copy, tid}) do
  ...
  defp need_lock(tab, :undefined) do
    case (try do
            :ets.lookup_element(:mnesia_gvar,
                                  {:mnesia_loader, :active_trans}, 2)
          catch
            :error, _ ->
              {:EXIT, {:badarg, []}}
          end) do
  ...
  defp update_where_to_write(tab, node) do
    case (val({tab, :access_mode})) do
      ...
      :read_write ->
        ...
        ns = (case (:lists.member(node, current)) do
                true ->
                  current
                false ->
                  [node | current]
  ...
  defp send_more(pid, n, chunk, dataState, tab, storage) do
    receive do
      ...
      {newPid, {:more, msg}} when :erlang.element(1,
                                                    storage) == :ext
                                  ->
        {:ext, alias, mod} = storage
        {newChunk, newState} = mod.sender_handle_info(msg,
                                                        alias, tab, newPid,
                                                        dataState)
      ...
      {_NewPid, {:old_protocol, ^tab}} ->
        ...
        {init, newChunk} = reader_funcs(false, tab, storage,
  ...
  defp get_chunk_func(pid, tab, {:ext, alias, mod}, remoteS) do
    try do
      case (mod.sender_init(alias, tab, remoteS, pid)) do
        {:standard, i, c} ->
          send(pid, {self(),
                       {:first, mod.info(alias, tab, :size)}})
          {i, c}
        {_, _} = res ->
          res
      end
  ...
  defp get_chunk_func(pid, tab, storage, remoteS) do
    try do
      tabSize = :mnesia.table_info(tab, :size)
      keysPerTransfer = calc_nokeys(storage, tab)
      chunkData = :dets.info(tab, :bchunk_format)
      useDetsChunk = storage == remoteS and storage == :disc_only_copies and chunkData != :undefined
      cond do
        useDetsChunk == true ->
          detsInfo = :erlang.system_info(:version)
          send(pid, {self(),
                       {:first, tabSize, {detsInfo, chunkData}}})
        true ->
          send(pid, {self(), {:first, tabSize}})
      end
      reader_funcs(useDetsChunk, tab, storage,
                     keysPerTransfer)
    catch
      _, reason ->
        verbose('Init chunk failed: ~p ~p~n', [tab, reason])
  ...
  defp init_chunk(storage, tab, keysPerTransfer) do
    try do
  ...
  defp chunk(tab, storage, chunk) do
    try do
      :mnesia_lib.db_chunk(storage, chunk)
    catch
      _, reason ->
        verbose('Read chunk failed: ~p ~p~n', [tab, reason])
        throw({:no_exists, tab})
  ...
  defp send_packet(n, pid, chunk, {recs, cont}) when n < 20 do
    case (compression_level()) do
      ...
      level ->
        send(pid, {self(),
                     {:more_z, zlib_compress(recs, level)}})
    end
  ...
  defp finish_copy(pid, tab, storage, remoteS, needLock) do
    ...
    trans = fn () ->
                 ...
                 after 0 ->
                   ...
                   :mnesia_controller.sync_and_block_table_whereabouts(tab,
                                                                         recNode,
                                                                         remoteS,
                                                                         a)
