defmodule :m_mnesia_loader do
  ...
  def disc_load_table(tab, reason, cs) do
    ...
    dbg_out(~c"Getting table ~tp (~p) from disc: ~tp~n", [tab, storage, reason])
    :ok
  ...
  defp do_get_disc_copy2(tab, _Reason, storage, _Type)
    ...
    verbose(~c"Local table copy of ~tp has recently been deleted, ignored.~n", [tab])
    ...
    case reason do
      ...
      _ ->
        case :mnesia_monitor.open_dets(tab, args) do
          ...
          {:error, error} ->
            {:not_loaded, {~c"Failed to create dets table", error}}
        end
    end
  end
  ...
  defp try_net_load_table(tab, _Reason, [], _Cs) do
    verbose(~c"Copy failed. No active replicas of ~tp are available.~n", [tab])
    {:not_loaded, :none_active}
  end

  defp try_net_load_table(tab, reason, ns, cs) do
    storage =
      case elem(cs, 0) === :cstruct do
        true ->
  ...
  defp do_get_network_copy(tab, reason, ns, storage, cs) do
    ...
    case :lists.member(
      ...
      true ->
        dbg_out(~c"Getting table ~tp (~p) from node ~p: ~tp~n", [tab, storage, node, reason])
        :ok
        ...
        case init_receiver(node, tab, storage, cs, reason) do
          :ok ->
            set({tab, :load_node}, node)
            set({tab, :load_reason}, reason)
            dbg_out(~c"Table ~tp copied from ~p to ~p~n", [tab, node, node()])
            {:loaded, :ok}
          ...
          when :erlang.element(
            ...
            verbose(~c"Copy failed: ~tp ~p~n", [tab, err])
            {:not_loaded, err}
  ...
  defp init_receiver(node, tab, storage, cs, reas = {:dumper, {:add_table_copy, tid}}) do
    ...
    case start_remote_sender(node, tab, storage, {:add_table_copy, tid}) do
  ...
  defp init_receiver(node, tab, storage, cs, reason) do
    load = fn ->
      ...
      case val({tab, :where_to_read}) == node() do
        ...
        _ ->
          ...
          {senderPid, tabSize, detsData} = start_remote_sender(node, tab, storage, :load)
    ...
    res =
      case :mnesia.transaction(load, 20) do
        ...
        {:atomic, {:error, result}} ->
          fatal(~c"Cannot create table ~tp: ~tp~n", [[tab, storage], result])
        ...
        {:aborted, _Reas} ->
          verbose(~c"Receiver failed on ~tp from ~p:~nReason: ~tp~n", [tab, node, _Reas])
          :down
      end
  ...
  defp start_remote_sender(node, tab, storage, why) do
    ...
    receive do
      ...
      {:copier_done, ^node} ->
        verbose(~c"Sender of table ~tp crashed on node ~p ~n", [tab, node])
        down(tab, storage)
    end
  end
  ...
  defp table_init_fun(senderPid, storage) do
    fn
      :read ->
        ...
        :erlang.put(
          :mnesia_table_receiver_subscr_cache,
          subscrCache
        )
  ...
  defp start_receiver(
    ...
    case do_init_table(tab, storage, cs, senderPid, tabSize, detsData, self(), init) do
      err = {:error, _} ->
        verbose(~c"Init table failed: ~tp ~p~n", [tab, err])
  ...
  defp wait_on_load_complete(pid) do
    receive do
      ...
      {:EXIT, ^pid, reason} ->
        verbose(~c"Loader crashed : ~tp ~p~n", [pid, reason])
        :erlang.error(reason)
    ...
    case create_table(tab, tabSize, storage, cs) do
      {^storage, ^tab} ->
        ...
        case init_table(tab, storage, init, detsInfo, senderPid) do
          ...
          reason ->
            msg = ~c"[d]ets:init table failed"
            verbose(~c"~ts: ~tp: ~tp~n", [msg, tab, reason])
            send(senderPid, {:copier_done, node()})
            down(tab, storage)
        end
  ...
  defp ext_init_table(action, alias, mod, tab, fun, state, sender) do
    case fun.(action) do
      {:copier_done, node} ->
        verbose(~c"Receiver of table ~tp crashed on ~p (more)~n", [tab, node])
  ...
  defp handle_last({:disc_only_copies, tab}, type, :nobin) do
    ...
    case :file.rename(tmp, dat) do
      ...
      {:error, reason} ->
        {:error, {~c"Cannot swap tmp files", tab, reason}}
    end
  end
  ...
  defp get_chunk_func(pid, tab, {:ext, alias, mod}, remoteS) do
    try do
      ...
      _, reason ->
        verbose(~c"Init chunk failed: ~p ~p~n", [tab, reason])
        throw({:no_exists, tab})
    end
  end
  ...
  defp init_chunk(storage, tab, keysPerTransfer) do
    try do
      ...
      _, reason ->
        verbose(~c"Read chunk failed: ~p ~p~n", [tab, reason])
        throw({:no_exists, tab})
    end
  end
      ...
      after
        0 ->
          ...
          receive do
            ...
            {:copier_done, ^node} ->
              verbose(~c"Tab receiver ~tp crashed (more): ~p~n", [tab, node])
              cleanup_tab_copier(pid, storage, tab)
              :receiver_died
          end
      end
