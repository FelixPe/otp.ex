defmodule :m_mnesia_locker do
  ...
  defp loop(state) do
    receive do
      ...
      {:system, from, msg} ->
        verbose(~c"~p got {system, ~p, ~tp}~n", [:mnesia_locker, from, msg])
      ...
      msg ->
        :erlang.error(~c"~p got unexpected message: ~tp~n", [:mnesia_locker, msg])
        loop(state)
    end
  end
  ...
  defp grant_lock(tid, {:ix_read, ixKey, pos}, lock, oid = {tab, _}) do
    try do
  ...
  defp can_lock(tid, :read, oid = {tab, key}, alreadyQ)
    ...
    tabLocks =
      :ets.select(
        :mnesia_held_locks,
        [{{{{tab, :______WHOLETABLE_____}, :write, :"$1"}}, [], [:"$1"]}]
      )

    check_lock(tid, oid, tabLocks, objLocks, :yes, alreadyQ, :read)
  end
  ...
  defp can_lock(tid, :write, oid = {tab, key}, alreadyQ)
    ...
    tabLocks =
      :ets.select(
        :mnesia_held_locks,
        [{{{{tab, :______WHOLETABLE_____}, :_, :"$1"}}, [], [:"$1"]}]
      )

    check_lock(tid, oid, tabLocks, objLocks, :yes, alreadyQ, :write)
  end
  ...
  defp check_lock(tid, oid, [lockTid | locks], tabLocks, _X, alreadyQ, type) do
  ...
  defp can_queue(waitForTid, tid, oid = {tab, _}, _) do
    case allowed_to_be_queued(waitForTid, tid) do
      true when r_tid(tid, :pid) == r_tid(waitForTid, :pid) ->
        dbg_out(~c"Spurious lock conflict ~w: ~w -> ~w~n", [oid, tid, waitForTid])
  ...
  defp try_waiter(oid, op, simpleOp, lock, replyTo, tid) do
    case can_lock(tid, lock, oid, {:queue, :bad_luck}) do
      ...
      {:no, lucky} ->
        ...
        verbose(
          ~c"** WARNING ** Restarted transaction, possible deadlock in lock queue ~w: cyclic = ~w~n",
          [tid, c]
        )
