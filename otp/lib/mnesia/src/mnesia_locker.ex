defmodule :m_mnesia_locker do
  ...
  Record.defrecord(:r_queue, :queue, oid: :undefined,
                                 tid: :undefined, op: :undefined,
                                 pid: :undefined, lucky: :undefined)
  def start() do
    :mnesia_monitor.start_proc(:mnesia_locker,
                                 :mnesia_locker, :init, [self()])
  end

  def init(parent) do
    :erlang.register(:mnesia_locker, self())
    :erlang.process_flag(:trap_exit, true)
    _ = :ets.new(:mnesia_held_locks,
                   [:ordered_set, :private, :named_table])
    _ = :ets.new(:mnesia_tid_locks,
                   [:ordered_set, :private, :named_table])
    _ = :ets.new(:mnesia_sticky_locks,
                   [:set, :private, :named_table])
    _ = :ets.new(:mnesia_lock_queue,
  ...
  def send_release_tid(nodes, tid) do
    :rpc.abcast(nodes, :mnesia_locker,
                  {self(), {:sync_release_tid, tid}})
  end
  ...
  defp loop(state) do
    receive do
      ...
      {{from, ref},
        ...
        loop(state)
      {from, {:is_locked, oid}} ->
        held = :ets.match_object(:mnesia_held_locks,
                                   {{oid, :_, :_}})
        reply(from, held)
        loop(state)
  ...
  defp set_lock(tid, oid, op) do
    ...
    :ets.insert(:mnesia_held_locks, {{oid, op, tid}})
  end
  ...
  defp try_lock(tid, op, simpleOp, lock, pid, oid) do
    case (can_lock(tid, lock, oid, {:no, :bad_luck})) do
      ...
      {:queue, lucky} ->
        :ok
        :ets.insert(:mnesia_lock_queue,
                      r_queue(oid: oid, tid: tid, op: op, pid: pid, lucky: lucky))
        :ets.insert(:mnesia_tid_locks,
                      {{tid, oid, {:queued, op}}})
    end
  end

  defp grant_lock(tid, :read, lock, oid = {tab, key})
  ...
  defp grant_lock(tid, {:ix_read, ixKey, pos}, lock,
            oid = {tab, _}) do
    try do
  ...
  defp grant_lock(tid, :read, lock, oid) do
    set_lock(tid, oid, lock)
    {:granted, :ok}
  end

  defp grant_lock(tid, :write, lock, oid) do
    set_lock(tid, oid, lock)
    :granted
  end
  ...
  defp can_lock(tid, :read, oid = {tab, key}, alreadyQ)
    ...
    objLocks = :ets.select(:mnesia_held_locks,
                             [{{{oid, :write, :"$1"}}, [], [:"$1"]}])
    tabLocks = :ets.select(:mnesia_held_locks,
                             [{{{{tab, :______WHOLETABLE_____}, :write, :"$1"}},
                                 [], [:"$1"]}])
    check_lock(tid, oid, tabLocks, objLocks, :yes, alreadyQ,
                 :read)
  end
  ...
  defp can_lock(tid, :write, oid, alreadyQ) do
    tab = :erlang.element(1, oid)
    locks = :ets.select(:mnesia_held_locks,
                          [{{{{tab, :_}, :_, :"$1"}}, [], [:"$1"]}])
    check_lock(tid, oid, locks, [], :yes, alreadyQ, :write)
  end

  defp check_lock(tid, oid, [lockTid | locks], tabLocks, _X,
            alreadyQ, type) do
    case (can_queue(lockTid, tid, oid, _X)) do
      {:no, _} = res ->
        res
      res ->
        check_lock(tid, oid, locks, tabLocks, res, alreadyQ,
                     type)
    end
  end
  ...
  defp check_lock(_, _, [], [], x = {:queue, _Tid}, _AlreadyQ,
            _) do
    x
  end

  defp check_lock(tid, oid = {tab, key}, [], [], x, alreadyQ,
            type) do
    cond do
      type == :write ->
  ...
  defp can_queue(tid, tid, _Oid, res) do
    res
  end

  defp can_queue(waitForTid, tid, oid = {tab, _}, _) do
    case (allowed_to_be_queued(waitForTid, tid)) do
      true when r_tid(tid, :pid) == r_tid(waitForTid, :pid) ->
        dbg_out('Spurious lock conflict ~w: ~w -> ~w~n', [oid, tid, waitForTid])
        haveQ = :ets.lookup(:mnesia_lock_queue,
                              oid) != [] or :ets.lookup(:mnesia_lock_queue,
                                                          {tab,
  ...
  defp check_queue(tid, tab, x, alreadyQ) do
    tabLocks = :ets.lookup(:mnesia_lock_queue,
  ...
  defp sort_queue(qL) do
    case (:erlang.get(:pid_sort_order)) do
      ...
      :r9b_plain ->
        :lists.sort(fn r_queue(tid: x), r_queue(tid: y) ->
                         cmp_tid(true, x, y) == 1
                    end,
                      qL)
      :standard ->
  ...
  defp release_remote_non_pending(node, pending) do
    ...
    tids = (for [t] <- allTids, node == node(r_tid(t, :pid)),
                  not :lists.member(t, pending) do
              t
            end)
  ...
  defp do_release_tid(tid) do
    objects = :ets.select(:mnesia_tid_locks,
                            [{{{tid, :_, :_}}, [], [:"$_"]}])
    locks = :lists.map(fn {l} ->
                            l
                       end,
                         objects)
    :ok
  ...
  defp release_lock({tid, oid, {:queued, _}}) do
    :ets.match_delete(:mnesia_lock_queue,
                        r_queue(oid: oid, tid: tid, op: :_, pid: :_, lucky: :_))
  end

  defp release_lock({tid, oid, op}) do
    :ets.delete(:mnesia_held_locks, {oid, op, tid})
  end

  defp rearrange_queue([{_Tid, {tab, key}, _} | locks]) do
    cond do
      key != :______WHOLETABLE_____ ->
        queue = :ets.lookup(:mnesia_lock_queue,
                              {tab,
                                 :______WHOLETABLE_____}) ++ :ets.lookup(:mnesia_lock_queue,
                                                                           {tab,
                                                                              key})
        case (queue) do
          [] ->
      ...
      true ->
        pat = r_queue(oid: {tab, :_}, tid: :_, op: :_, pid: :_,
                  lucky: :_)
  ...
  defp try_waiter({:queue, oid, tid, :read_write, replyTo, _}) do
    try_waiter(oid, :read_write, :read, :write, replyTo,
                 tid)
  end

  defp try_waiter({:queue, oid, tid, iXR = {:ix_read, _, _},
             replyTo, _}) do
  ...
  defp try_waiter(oid, op, simpleOp, lock, replyTo, tid) do
    case (can_lock(tid, lock, oid, {:queue, :bad_luck})) do
      :yes ->
        :ets.match_delete(:mnesia_lock_queue,
                            r_queue(oid: oid, tid: tid, op: op, pid: replyTo,
                                lucky: :_))
        reply = grant_lock(tid, simpleOp, lock, oid)
        reply(replyTo, reply)
        :locked
      ...
      {:no, lucky} ->
        c = r_cyclic(op: simpleOp, lock: lock, oid: oid, lucky: lucky)
        verbose('** WARNING ** Restarted transaction, possible deadlock in lock queue ~w: cyclic = ~w~n', [tid, c])
        :ets.match_delete(:mnesia_lock_queue,
                            r_queue(oid: oid, tid: tid, op: op, pid: replyTo,
                                lucky: :_))
        reply = {:not_granted, c}
  ...
  defp ix_read_res(tab, ixKey, pos) do
    ...
    rks = :mnesia_lib.elems(2,
                              :mnesia_index.db_get(index, ixKey))
    :lists.append(:lists.map(fn real ->
      ...
      node ->
        ...
        case (need_lock(store, tab, key, lock)) do
          :yes ->
            ...
            res = get_rwlocks_on_nodes(ns, make_ref(), store, tid,
                                         oid)
  ...
  defp do_sticky_lock(tid, store, {tab, key} = oid, lock) do
    ...
    send(:mnesia_locker, {self(),
                            {:test_set_sticky, tid, oid, lock}})
    n = node()
    receive do
  ...
  defp sticky_check_majority(w, tab, true, wNodes) when w == :write or
                                       w == :read_write do
  ...
  def sticky_wlock_table(tid, store, tab) do
    sticky_lock(tid, store, {tab, :______WHOLETABLE_____},
                  :write)
  end
  ...
  defp wlock(tid, store, oid, checkMajority) do
    ...
    case (need_lock(store, tab, key, :write)) do
      ...
      :no when (key != :______WHOLETABLE_____ and
                  tab != :______GLOBAL_____)
               ->
        []
      :no ->
  ...
  defp need_lock(store, tab, key, lockPattern) do
    tabL = :ets.match_object(store,
                               {{:locks, tab, :______WHOLETABLE_____},
                                  lockPattern})
    cond do
      tabL == [] ->
        keyL = :ets.match_object(store,
  ...
  defp get_rwlocks_on_nodes([readNode | tail], ref, store, tid, oid) do
    ...
    case (receive_wlocks([readNode], ref, store, oid)) do
      ...
      res ->
        get_wlocks_on_nodes(tail, res, store,
                              {self(), {:write, tid, oid}}, oid)
    end
  end
  ...
  defp receive_wlocks(nodes = [this | ns], res, store, oid) do
    ...
    receive do
      {:mnesia_locker, node, :granted} ->
        receive_wlocks(:lists.delete(node, nodes), res, store,
                         oid)
      {:mnesia_locker, node, {:granted, val}} ->
        case (opt_lookup_in_client(val, oid, :write)) do
          ...
          val2 ->
            receive_wlocks(:lists.delete(node, nodes), val2, store,
                             oid)
        end
    ...
    case (:rpc.call(node, :mnesia_lib, :db_get, args)) do
      {:badrpc, reason} ->
        case (val({tab, :where_to_read})) do
          ...
          _NewNode ->
            c = r_cyclic(op: :read, lock: lock, oid: {tab, key},
                    lucky: :nowhere)
            exit({:aborted, c})
        end
      other ->
  ...
  def ixrlock(tid, store, tab, ixKey, pos) do
    case (val({tab, :where_to_read})) do
      :nowhere ->
        :mnesia.abort({:no_exists, tab})
      node ->
        case (need_lock(store, tab, :______WHOLETABLE_____,
                          :read)) do
          :no when node === node() ->
            ix_read_res(tab, ixKey, pos)
          _ ->
            r = l_request(node, {:ix_read, tid, tab, ixKey, pos},
                            store)
  ...
  def get_held_locks() do
    send(:mnesia_locker, {:get_table, self(),
                            :mnesia_held_locks})
    locks = (receive do
  ...
  defp rewrite_locks([{{oid, op, tid}} | locks], acc) do
    rewrite_locks(locks, [{oid, op, tid} | acc])
  end
  ...
  Record.defrecord(:r_pid_info, :pid_info, serial: :undefined,
                                    number: :undefined, nodename: :undefined,
                                    creation: :undefined)
  defp cmp_tid(r9B, r_tid() = t, r_tid() = t) when r9B == true or
                                        r9B == false do
    0
  end

  defp cmp_tid(r9B, r_tid(counter: c, pid: pid1),
            r_tid(counter: c, pid: pid2))
      when r9B == true or r9B == false do
    cmp_pid_info(r9B, pid_to_pid_info(pid1),
                   pid_to_pid_info(pid2))
  end
  ...
  defp cmp_pid_info(false,
            r_pid_info(serial: s, number: n, nodename: nN, creation: c1),
            r_pid_info(serial: s, number: n, nodename: nN, creation: c2)) do
    cmp(c1, c2)
  end

  defp cmp_pid_info(false, r_pid_info(serial: s, number: n, nodename: nN1),
            r_pid_info(serial: s, number: n, nodename: nN2)) do
    cmp(nN1, nN2)
  end

  defp cmp_pid_info(false, r_pid_info(serial: s, number: n1),
            r_pid_info(serial: s, number: n2)) do
    cmp(n1, n2)
  end
  ...
  defp cmp_pid_info(true,
            r_pid_info(nodename: nN, creation: c, serial: s, number: n1),
            r_pid_info(nodename: nN, creation: c, serial: s, number: n2)) do
    cmp(n1, n2)
  end

  defp cmp_pid_info(true, r_pid_info(nodename: nN, creation: c, serial: s1),
            r_pid_info(nodename: nN, creation: c, serial: s2)) do
    cmp(s1, s2)
  end

  defp cmp_pid_info(true, r_pid_info(nodename: nN, creation: c1),
            r_pid_info(nodename: nN, creation: c2)) do
    cmp(c1, c2)
  end
  ...
  defp pid_to_pid_info(pid) when is_pid(pid) do
    [131, 103, 100, nNL1, nNL0 |
                              rest] = :erlang.binary_to_list(:erlang.term_to_binary(pid))
    [n3, n2, n1, n0, s3, s2, s1, s0,
                                     creation] = drop(bytes2int(nNL1, nNL0),
                                                        rest)
    r_pid_info(serial: bytes2int(s3, s2, s1, s0),
        number: bytes2int(n3, n2, n1, n0), nodename: node(pid),
        creation: creation)
  end
  ...
  defp bytes2int(n1, n0) when (0 <= n1 and n1 <= 255 and
                          0 <= n0 and n0 <= 255) do
    n1 <<< 8 ||| n0
  end

  defp bytes2int(n3, n2, n1, n0) when (0 <= n3 and n3 <= 255 and
