defmodule :m_mnesia_bup do
  ...
  defp abort_restore(r = r_restore(bup_module: mod), what, args, reason) do
    dbg_out(~c"Restore aborted. ~p:~p~p -> ~p~n", [mod, what, args, reason])
    close_read(r)
    throw({:error, reason})
  end
  ...
  defp verify_header([h | rawSchema])
    ...
    cond do
      r_log_header(h, :log_kind) === r_log_header(current, :log_kind) ->
        versions = [~c"0.1", ~c"1.1", r_log_header(current, :log_version)]
        ...
        case :lists.member(r_log_header(h, :log_version), versions) do
          ...
          false ->
            {:error, {~c"Bad header version. Cannot be used as backup.", h}}
        end

      true ->
        {:error, {~c"Bad kind of header. Cannot be used as backup.", h}}
    end
  end

  defp verify_header(rawSchema) do
    {:error,
     {~c"Missing header. Cannot be used as backup.",
      try do
        hd(rawSchema)
      catch
        _, _Reason ->
  ...
  def refresh_cookie(schema, newCookie) do
    case :lists.keysearch(:schema, 2, schema) do
      ...
      false ->
        reason = ~c"No schema found. Cannot be used as backup."
        throw({:error, {reason, schema}})
    end
  end

  defp convert_schema(~c"0.1", schema) do
    convert_0_1(schema)
  end

  defp convert_schema(~c"1.1", schema) do
    current = :mnesia_log.backup_log_header()
  ...
  defp convert_schema(latest, schema) do
    ...
    cond do
      ...
      true ->
        reason = ~c"Bad backup header version. Cannot convert schema."
  ...
  def lookup_schema(key, schema) do
    case :lists.keysearch(key, 2, schema) do
      ...
      false ->
        throw({:error, {~c"Cannot lookup", key}})
    end
  end
      ...
      receive do
        ...
        {^pid, res2} ->
          case res2 do
            ...
            {:error, reason} ->
              {:error, {~c"Cannot install fallback", reason}}
          end
      end
  ...
  defp restore_recs(_, _, _, _, :stop) do
    throw({:error, ~c"restore_recs already stopped"})
  end
  ...
  defp restore_recs(recs, header, schema, ext, {:start, fA}) do
    ...
    try do
      ...
      _, reason ->
        throw({:error, {~c"Bad schema in restore_recs", reason}})
    else
      cs ->
  ...
  defp get_fallback_nodes(fA, ns) do
    ...
    case :lists.member(this, ns) do
      ...
      false ->
        throw({:error, {~c"No disc resident schema on local node", ns}})
    end
  end
  ...
  defp rec_answers([], acc) do
    case {:lists.keysearch(:error, 1, acc), :mnesia_lib.uniq(acc)} do
      ...
      {_, other} ->
        throw({:error, {~c"Different answers", other}})
    end
  end
  ...
  defp fallback_receiver_loop(master, r, fA, state) do
    receive do
      ...
      msg ->
        ...
        tmp = r_fallback_args(fA, :fallback_tmp)
        :file.delete(tmp)
        throw({:error, ~c"Unexpected msg fallback_receiver_loop", msg})
    end
  end
  ...
  defp do_fallback_start(true, true) do
    verbose(~c"Ignoring fallback at startup, but leaving it active...~n", [])
    :mnesia_lib.set(:active_fallback, true)
    :ok
  end

  defp do_fallback_start(true, false) do
    verbose(~c"Starting from fallback...~n", [])
      ...
      {:ok, _Res} ->
        ...
        case :file.rename(tmpSchema, datSchema) do
          ...
          {:error, reason} ->
            ...
            {:error, {~c"Cannot start from fallback. Rename error.", reason}}
        end

      {:error, reason} ->
        {:error, {~c"Cannot start from fallback", reason}}
    end
  end
  ...
  defp restore_tables(recs, header, schema, ext, {:start, localTabs}) do
    dir = :mnesia_lib.dir()
    oldDir = :filename.join([dir, ~c"OLD_DIR"])
  ...
  defp init_dat_files(schema, ext, localTabs) do
    ...
    case :dets.open_file(:schema, args) do
      ...
      {:error, reason} ->
        throw({:error, {~c"Cannot open file", :schema, args, reason}})
    end
  end
          ...
          cond do
            ...
            true ->
              ...
              case :file.rename(tmpFile, dclFile) do
                ...
                {:error, reason} ->
                  :mnesia_lib.fatal(~c"Cannot rename file ~tp -> ~tp: ~tp~n", [
                    tmpFile,
                    dclFile,
                    reason
                  ])
  ...
  defp disc_only_open_fun(:disc_only_copies, r_cstruct(name: tab) = cs) do
    ...
    fn t, lT when t === r_local_tab(lT, :name) ->
      case :mnesia_lib.dets_sync_open(t, args) do
        ...
        {:error, reason} ->
          throw({:error, {~c"Cannot open file", t, args, reason}})
      end
    end
  end
  ...
  defp disc_only_swap_fun(:disc_only_copies, expunge, open, close) do
    fn t, lT when t === r_local_tab(lT, :name) ->
      ...
      case :file.rename(tmpFile, datFile) do
        ...
        {:error, reason} ->
          :mnesia_lib.fatal(~c"Cannot rename file ~tp -> ~tp: ~tp~n", [tmpFile, datFile, reason])
      end
    end
  end
  ...
  defp do_uninstall(clientPid, [pid | pids], goodPids, badNodes, res) do
    receive do
      {:EXIT, ^pid, reason} ->
        badNode = node(pid)
        badRes = {:error, {~c"Uninstall fallback", badNode, reason}}
  ...
  def traverse_backup(source, sourceMod, target, targetMod, fun, acc) do
    ...
    receive do
      {:EXIT, ^pid, reason} ->
        {:error, {~c"Backup traversal crashed", reason}}
    ...
    res =
      case iterate(sourceMod, &trav_apply/5, source, a) do
        ...
        {:error, reason} when targetMod !== :read_only ->
          ...
          {:error, {~c"Backup traversal failed", reason}}
  ...
  defp filter_foldl(fun, acc, [head | tail]) do
    case fun.(head, acc) do
      ...
      other ->
        throw({:error, {~c"Fun must return a list", other}})
    end
  end
