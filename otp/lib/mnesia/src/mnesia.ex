defmodule :m_mnesia do
  ...
  defp start_() do
    ...
    case res do
      :ok ->
        verbose(~c"Mnesia started, ~p seconds~n", [secs])
        :ok

      {:error, {:already_started, :mnesia}} ->
        verbose(~c"Mnesia already started, ~p seconds~n", [secs])
        :ok

      {:error, r} ->
        verbose(~c"Mnesia failed to start, ~p seconds: ~p~n", [secs, r])
        {:error, r}
    end
  end
  ...
  def info() do
    case :mnesia_lib.is_running() do
      :yes ->
        ...
        :io.format(~c"---> Processes holding locks <--- ~n", [])

        :lists.foreach(
          fn l ->
            :io.format(~c"Lock: ~p~n", [l])
          end,
          held
        )

        :io.format(~c"---> Processes waiting for locks <--- ~n", [])

        :lists.foreach(
          fn {oid, op, _Pid, tid, ownerTid} ->
            :io.format(~c"Tid ~p waits for ~p lock on oid ~p owned by ~p ~n", [
              tid,
              op,
              oid,
              ownerTid
        ...
        :io.format(~c"---> Uncertain transactions <--- ~n", [])

        :lists.foreach(
          fn {tid, _, nodes} ->
            :io.format(~c"Tid ~w waits for decision from ~w~n", [tid, nodes])
          end,
          uncertain
        )
  ...
  defp mini_info() do
    :io.format(
      ~c"===> System info in version ~p, debug level = ~p <===~n",
    ...
    :io.format(
      ~c"~w. Directory ~p is ~sused.~n",
      [system_info(:schema_location), system_info(:directory), not__]
    )

    :io.format(~c"use fallback at restart = ~w~n", [system_info(:fallback_activated)])
    running = system_info(:running_db_nodes)
    :io.format(~c"running db nodes   = ~w~n", [running])
    all = :mnesia_lib.all_nodes()
    :io.format(~c"stopped db nodes   = ~w ~n", [all -- running])
  end
  ...
  defp display_system_info(held, queued, tmInfo, uncertain) do
    ...
    :io.format(
      ~c"~w transactions committed, ~w aborted, ~w restarted, ~w logged to disc~n",
      s.([
        :transaction_commits,
    ...
    :io.format(
      ~c"~w held locks, ~w in queue; ~w local transactions, ~w remote~n",
    ...
    :io.format(~c"~w transactions waits for other nodes: ~p~n", [ucount, unodes])
  end

  defp display_tab_info() do
    masterTabs = :mnesia_recover.get_master_node_tables()
    :io.format(~c"master node tables = ~p~n", [:lists.sort(masterTabs)])
    ...
    case get_backend_types() do
      ...
      ts ->
        list_backend_types(ts, ~c"backend types      = ")
    end
    ...
    case get_index_plugins() do
      ...
      ps ->
        list_index_plugins(ps, ~c"index plugins      = ")
    end
    ...
    :io.format(~c"remote             = ~p~n", [:lists.sort(unknown)])
    :io.format(~c"ram_copies         = ~p~n", [:lists.sort(ram)])
    :io.format(~c"disc_copies        = ~p~n", [:lists.sort(disc)])
    :io.format(~c"disc_only_copies   = ~p~n", [:lists.sort(discOnly)])

    for {a, ts} <- ext do
      :io.format(~c"~-19s= ~p~n", [:erlang.atom_to_list(a), ts])
    end
  ...
  defp pp_ix_name(n) do
    :lists.flatten(:io_lib.fwrite(~c"~w", [n]))
  end
  ...
  defp qlc_format({:lookup, 2, keys}, tab, _, lock, _) do
    :io_lib.format(~c"lists:flatmap(fun(V) -> ~w:read(~w, V, ~w) end, ~w)", [
      :mnesia,
      tab,
      lock,
      keys
  ...
  defp qlc_format({:lookup, index, keys}, tab, _, _, _) do
    :io_lib.format(~c"lists:flatmap(fun(V) -> ~w:index_read(~w, V, ~w) end, ~w)", [
      :mnesia,
      tab,
      index,
      keys
