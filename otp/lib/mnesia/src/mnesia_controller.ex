defmodule :m_mnesia_controller do
  ...
  defp try_schedule_late_disc_load(tabs, reason, msgTag) do
    ...
    case :mnesia.transaction(getIntents) do
      ...
      {:atomic, {:retry, badNodes}} ->
        verbose(~c"Retry late_load_tables because bad nodes: ~p~n", [badNodes])
        try_schedule_late_disc_load(tabs, reason, msgTag)

      {:aborted, abortReason} ->
        fatal(~c"Cannot late_load_tables ~tp: ~tp~n", [[tabs, reason, msgTag], abortReason])
    end
  end
  ...
  def merge_schema() do
    ...
    case try_merge_schema(allNodes, [node()], &default_merge/1) do
      ...
      {:aborted, {:throw, str}} when is_list(str) ->
        fatal(~c"Failed to merge schema: ~s~n", [str])

      else__ ->
        fatal(~c"Failed to merge schema: ~p~n", [else__])
    end
  end
  ...
  defp try_merge_schema(nodes, told0, userFun) do
    case :mnesia_schema.merge_schema(userFun) do
      ...
      {:atomic, {~c"Cannot get cstructs", node, reason}} ->
        dbg_out(~c"Cannot get cstructs, Node ~p ~tp~n", [node, reason])
        :timer.sleep(300)
  ...
  def init([parent]) do
    :erlang.process_flag(:trap_exit, true)
    :mnesia_lib.verbose(~c"~p starting: ~p~n", [:mnesia_controller, self()])
  ...
  def handle_call(msg, _From, state) do
    :erlang.error(~c"~p got unexpected call: ~tp~n", [:mnesia_controller, msg])
    noreply(state)
  end
  ...
  defp late_disc_load(
    ...
    verbose(~c"Intend to load tables: ~tp~n", [tabsR])
    :ok
    reply(from, :queued)
  ...
  def handle_cast({:send_table, tab, pid, storage}, state) do
    worker = r_send_table(table: tab, receiver_pid: pid, remote_storage: storage)
    state2 = add_worker(worker, state)
    noreply(state2)
  end
  ...
  def handle_cast({:adopt_orphans, node, tabs}, state) do
    ...
    case (try do
      ...
      _ ->
        ...
        verbose(~c"Logging mnesia_up ~w~n", [node])
  ...
  def handle_cast(msg, state) do
    :erlang.error(~c"~p got unexpected cast: ~tp~n", [:mnesia_controller, msg])
    noreply(state)
  end
  ...
  def handle_info(r_dumper_done(worker_pid: pid, worker_res: res), state) do
    cond do
      ...
      true ->
        fatal(~c"Dumper failed: ~p~n state: ~tp~n", [res, state])
        {:stop, :fatal, state}
    end
  end
  ...
  def handle_info(r_sender_done(worker_pid: pid, worker_res: res), state) do
    ...
    case res do
      ...
      _ ->
        fatal(~c"Sender failed: ~p~n state: ~tp~n", [res, state])
        {:stop, :fatal, state}
    end
  end
  ...
  def handle_info({:EXIT, pid, r}, state)
    ...
    case r_state(state, :dumper_pid) do
      :undefined ->
        dbg_out(~c"~p was ~tp~n", [:mnesia_controller, r])
        {:stop, :shutdown, state}
  ...
  def handle_info({:EXIT, pid, r}, state)
    ...
    case r_state(state, :dumper_queue) do
      [r_schema_commit_lock() | workers] ->
        dbg_out(~c"WARNING: Dumper ~p exited ~tp~n", [pid, r])
      ...
      _Other ->
        fatal(~c"Dumper or schema commit crashed: ~p~n state: ~tp~n", [r, state])
        {:stop, :fatal, state}
    end
  end
  ...
  def handle_info(msg = {:EXIT, pid, r}, state)
    ...
    case :lists.keymember(pid, 1, get_senders(state)) do
      true ->
        fatal(~c"Sender crashed: ~p~n state: ~tp~n", [{pid, r}, state])
        {:stop, :fatal, state}

      false ->
        case :lists.keymember(pid, 1, get_loaders(state)) do
          true ->
            fatal(~c"Loader crashed: ~p~n state: ~tp~n", [r, state])
            {:stop, :fatal, state}

          false ->
            :erlang.error(~c"~p got unexpected info: ~tp~n", [:mnesia_controller, msg])
            noreply(state)
        end
    end
  end
  ...
  defp maybe_log_mnesia_down(n) do
    case :mnesia_lib.is_running() do
      :yes ->
        verbose(~c"Logging mnesia_down ~w~n", [n])
  ...
  defp update_whereabouts(tab, node, state) do
    ...
    dbg_out(
      ~c"Table ~w is loaded on ~w. s=~w, r=~w, lc=~w, f=~w, m=~w~n",
  ...
  defp initial_safe_loads() do
    case val({:schema, :storage_type}) do
      ...
      :disc_copies ->
        ...
        dbg_out(~c"mnesia_downs = ~p~n", [downs])
        tabs = val({:schema, :local_tables}) -- [:schema]
  ...
  defp info([tab | tail]) do
    case val({tab, :storage_type}) do
      :disc_only_copies ->
        info_format(tab, :dets.info(tab, :size), :dets.info(tab, :file_size), ~c"bytes on disc")
      ...
      _ ->
        info_format(tab, :ets.info(tab, :size), :ets.info(tab, :memory), ~c"words of mem")
    end
  ...
  defp info_format(tab, size, mem, media) do
    ...
    :io.format(~c"~ts: with ~s records occupying ~s ~s~n", [strT, strS, strM, media])
  end
  ...
  defp opt_start_sender2([sender | r], pids, kept, loaderQ) do
    ...
    cond do
      ...
      true ->
        verbose(~c"Send table failed ~tp not active on this node ~n", [tab])
  ...
  def start_remote_sender(node, tab, receiver, storage, why) do
    msg =
      case (try do
        ...
        _ ->
          r_send_table(table: tab, receiver_pid: receiver, remote_storage: storage, reason: why)
      end
  ...
  def send_and_reply(
        replyTo,
        r_send_table(table: tab, remote_storage: storage, receiver_pid: pid, reason: reason)
      ) do
  ...
  defp disc_load_table(tab, reason, replyTo, cs) do
    ...
    cond do
      ...
      true ->
        fatal(~c"Cannot load table ~tp from disc: ~tp~n", [tab, res])
    end
  end
