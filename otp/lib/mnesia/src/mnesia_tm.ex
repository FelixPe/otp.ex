defmodule :m_mnesia_tm do
  ...
  def init(parent) do
    ...
    case :mnesia_monitor.use_dir() do
      true ->
        ...
        msg = ~c"Initial dump of log during startup: ~p~n"
    ...
    receive do
      ...
      {from, :start_outer} ->
        try do
          ...
          :error, reason ->
            msg = ~c"Cannot create an ets table for the local transaction store"
      ...
      {tid, :do_commit} ->
        case :gb_trees.lookup(tid, participants) do
          :none ->
            verbose(~c"Tried to commit a non participant transaction ~p~n", [tid])
            doit_loop(state)
      ...
      {tid, {:do_abort, reason}} ->
        ...
        case :gb_trees.lookup(tid, participants) do
          :none ->
            verbose(~c"Tried to abort a non participant transaction ~p: ~tp~n", [tid, reason])
            :mnesia_locker.release_tid(tid)
            doit_loop(state)
      ...
      {from, {:add_store, tid}} ->
        try do
          ...
          :error, reason ->
            msg = ~c"Cannot create an ets table for a nested local transaction store"
            reply(from, {:error, {:system_limit, msg, reason}}, state)
        else
          etab ->
        ...
        case :gb_trees.is_defined(tid, participants) do
          false ->
            case :gb_trees.lookup(tid, coordinators) do
              :none ->
                verbose(~c"** ERROR ** Tried to delete a non transaction ~p~n", [tid])
                doit_loop(state)
      ...
      {from, :transactions_count} ->
        reply(
          from,
          {:transactions_count, :gb_trees.size(participants), :gb_trees.size(coordinators)},
          state
        )

      {:mnesia_down, n} ->
        verbose(~c"Got mnesia_down from ~p, reconfiguring...~n", [n])
      ...
      {from, {:unblock_me, tab}} ->
        case :lists.member(tab, r_state(state, :blocked_tabs)) do
          false ->
            verbose(~c"Wrong dirty Op blocked on ~p ~tp ~p", [node(), tab, from])
      ...
      {:system, from, msg} ->
        dbg_out(~c"~p got {system, ~p, ~tp}~n", [:mnesia_tm, from, msg])
        :sys.handle_system_msg(msg, from, sup, :mnesia_tm, [], state)

      msg ->
        verbose(~c"** ERROR ** ~p got unexpected message: ~tp~n", [:mnesia_tm, msg])
        doit_loop(state)
    end
  end
  ...
  defp is_blocked(
         [tab | tabs],
         r_commit(ram_copies: rCs, disc_copies: dCs, disc_only_copies: dOs, ext: exts) = commit
       ) do
    is_blocked_tab(rCs, tab) or
      is_blocked_tab(
        dCs,
        tab
      ) or
      ...
      {:none, _} ->
        ...
        case :mnesia_lib.key_search_delete(pid, r_participant(:pid), ps) do
          ...
          {p = r_participant(), _RestP} ->
            fatal(~c"Participant ~p in transaction ~p died ~tp~n", [
              r_participant(p, :pid),
              r_participant(p, :tid),
              reason
            ])
  ...
  defp recover_coordinator(tid, etabs) do
    verbose(~c"Coordinator ~p in transaction ~p died.~n", [r_tid(tid, :pid), tid])
    store = hd(etabs)
    ...
    try do
      ...
      _, reason ->
        dbg_out(~c"Recovery of coordinator ~p failed: ~tp~n", [tid, {reason, __STACKTRACE__}])
        protocol = :asym_trans
  ...
  defp restart(mod, tid, ts, fun, args, factor0, retries0, type, why) do
    ...
    case why do
      {:bad_commit, _N} ->
        ...
        log_restart(~c"Restarting transaction ~w: in ~wms ~w~n", [tid, sleepTime, why])
      ...
      _ ->
        ...
        dbg_out(~c"Restarting transaction ~w: in ~wms ~w~n", [tid, sleepTime, why])
  ...
  defp return_abort(fun, args, reason) do
    ...
    dbg_out(~c"Transaction ~p calling ~tp with ~tp failed: ~n ~tp~n", [tid, fun, args, reason])
  ...
  defp do_update(tid, storage, [op | ops], oldRes) do
    try do
      ...
      _, reason ->
        verbose(~c"do_update in ~w failed: ~tp -> {'EXIT', ~tp}~n", [
          tid,
          op,
  ...
  def do_snmp(tid, [head | tail]) do
    try do
      ...
      _, reason ->
        verbose(~c"do_snmp in ~w failed: ~tp -> {'EXIT', ~tp}~n", [
          tid,
          head,
          {reason, __STACKTRACE__}
        ])
  ...
  defp rec_all([node | tail], tid, res, pids) do
    ...
    after
      15000 ->
        :mnesia_lib.verbose(~c"~p: trans ~p waiting ~p~n", [self(), tid, node])
  ...
  def display_info(stream, {:timeout, t}) do
    :io.format(
      stream,
      ~c"---> No info about coordinator and participant transactions, timeout ~p <--- ~n",
      [t]
    )
  end

  def display_info(stream, {:info, part, coord}) do
    :io.format(stream, ~c"---> Participant transactions <--- ~n", [])
    ...
    :io.format(stream, ~c"---> Coordinator transactions <---~n", [])
  ...
  defp pr_participant(stream, p) do
    ...
    :io.format(stream, ~c"with participant objects ~tp~n", [commit])
  end

  defp pr_tid(stream, tid) do
    :io.format(stream, ~c"Tid: ~p (owned by ~p) ~n", [r_tid(tid, :counter), r_tid(tid, :pid)])
  end

  def info(serial) do
    :io.format(~c"Info about transaction with serial == ~p~n", [serial])
  ...
  defp search_pr_coordinator(s, [{tid, _Ts} | tail]) do
    case r_tid(tid, :counter) do
      ^s ->
        :io.format(~c"Tid is coordinator, owner == \n", [])
  ...
  defp search_pr_participant(s, [p | tail]) do
    ...
    cond do
      r_tid(tid, :counter) == s ->
        :io.format(~c"Tid is participant to commit, owner == \n", [])
        pid = r_tid(tid, :pid)
        display_pid_info(pid)
        :io.format(~c"Tid wants to write objects \n", [])
    ...
    case :rpc.pinfo(pid) do
      ...
      info ->
        ...
        pformat(
          ...
          :io_lib.format(~c"~tp", [call]),
          :io_lib.format(~c"~tp", [curr]),
          reds,
          lM
        )
    end
  ...
  defp pformat(a1, a2, a3, a4, a5) do
    :io.format(~c"~-12s ~-21ts ~-21ts ~9w ~4w~n", [a1, a2, a3, a4, a5])
  end
