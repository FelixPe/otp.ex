defmodule :m_mnesia_checkpoint do
  ...
  def most_local_node(name, tab) do
    case (try do
      ...
      {:EXIT, _} ->
        {:error, {~c"No retainer attached to table", [tab, name]}}
  ...
  defp select_writers(cp, tab) do
    case filter_remote(
      ...
      [] ->
        throw(
          {~c"Cannot prepare checkpoint (replica not available)",
  ...
  defp do_activate(cp) do
    ...
    case :mnesia_tm.prepare_checkpoint(nodes, cp) do
      ...
      {_, badNodes} ->
        {:error, {~c"Cannot prepare checkpoint (bad nodes)", [name, badNodes]}}
    end
  end
  ...
  defp check_prep([{:error, reason} | _Replies], name, _Nodes, _IgnoreNew) do
    {:error, {~c"Cannot prepare checkpoint (bad reply)", [name, reason]}}
  end

  defp check_prep([{:badrpc, reason} | _Replies], name, _Nodes, _IgnoreNew) do
    {:error, {~c"Cannot prepare checkpoint (badrpc)", [name, reason]}}
  end
    ...
    case :rpc.multicall(nodes, :mnesia_checkpoint, :call, [name, :collect_pending]) do
      {replies, []} ->
        try do
          ...
          :error, reason ->
            msg = ~c"Cannot create an ets table pending union"
            {:error, {:system_limit, msg, reason}}
        end

      {_, badNodes} ->
        deactivate(nodes, name)
        {:error, {~c"Cannot collect from pending checkpoint", name, badNodes}}
    end
  end
  ...
  defp send_activate([node | nodes], allNodes, name, unionTab, ignoreNew) do
    ...
    case :rpc.call(node, :mnesia_checkpoint, :call, [name, {:activate, pending}]) do
      ...
      {:badrpc, reason} ->
        deactivate(nodes, name)
        :ets.delete(unionTab)
        {:error, {~c"Activation failed (bad node)", name, node, reason}}

      {:error, reason} ->
        deactivate(nodes, name)
        :ets.delete(unionTab)
        {:error, {~c"Activation failed", name, node, reason}}
    end
  end
  ...
  defp start_retainer(cp) do
    ...
    case :supervisor.start_child(
      ...
      {:error, reason} ->
        {:error, {~c"Cannot create checkpoint retainer", name, node(), reason}}
    end
  end
  ...
  def init(cp) do
    ...
    try do
      ...
      :error, reason ->
        msg = ~c"Cannot create an ets table for pending transactions"
        error = {:error, {:system_limit, name, msg, reason}}
        :proc_lib.init_fail(r_checkpoint_args(cp, :supervisor), error, {:exit, :normal})
    else
      pendingTab ->
        rs =
        ...
        dbg_out(~c"Checkpoint ~p (~p) started~n", [name, self()])
  ...
  defp tab2retainer({tab, name}) do
    ...
    :mnesia_lib.dir(:lists.concat([:mnesia_checkpoint, ~c"_", tab, ~c"_", flatName, ~c".RET"]))
  end
  ...
  defp retainer_create(_Cp, r, tab, name, ext = {:ext, alias, mod}) do
    ...
    dbg_out(~c"Checkpoint retainer created ~p ~tp~n", [name, tab])
  ...
  defp retainer_loop(cp = r_checkpoint_args(is_activated: false, name: name)) do
    receive do
      ...
      {:system, from, msg} ->
        dbg_out(~c"~p got {system, ~p, ~tp}~n", [:mnesia_checkpoint, from, msg])
  ...
  defp retainer_loop(cp = r_checkpoint_args(name: name)) do
    receive do
      ...
      msg ->
        dbg_out(~c"~p got ~tp~n", [:mnesia_checkpoint, msg])
    end
  end
