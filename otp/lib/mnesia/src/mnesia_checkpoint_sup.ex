defmodule :m_mnesia_checkpoint_sup do
  ...
  def start() do
    :supervisor.start_link({:local, :mnesia_checkpoint_sup},
  ...
  def init([]) do
    ...
    modules = [:mnesia_checkpoint_sup, :mnesia_checkpoint,
                                           :supervisor]
    killAfter = :mnesia_kernel_sup.supervisor_timeout(:timer.seconds(3))
    workers = [{:mnesia_checkpoint_sup, mFA, :transient,
                  killAfter, :worker, modules}]
    {:ok, {flags, workers}}
  end
