defmodule :m_mnesia_frag do
  ...
  defp verify_numbers(fH, matchSpec) do
    ...
    try do
      ...
      :error, badFrags ->
        :mnesia.abort(
          {~c"match_spec_to_frag_numbers: Fragment numbers out of range", badFrags,
           {:range, 1, n}}
        )
    end
  end
  ...
  defp pick_props(tab, cs, {foreignTab, attr}) do
    ...
    :mnesia_schema.verify(
      :undefined,
      foreignKey,
      {:combine_error, tab, ~c"Multiple levels of foreign_key dependencies", {foreignTab, attr},
       foreignKey}
    )
  ...
  defp set_frag_nodes(_, _, _, _, cs, [], _) do
    :mnesia.abort({:combine_error, r_cstruct(cs, :name), ~c"Too few nodes in node_pool"})
  end
  ...
  defp make_deactivate(tab) do
    ...
    cond do
      baseTab != tab ->
        :mnesia.abort({:combine_error, tab, ~c"Not a base table"})

      foreigners != [] ->
        :mnesia.abort({:combine_error, tab, ~c"Too many foreigners", foreigners})

      r_frag_state(fH, :n_fragments) > 1 ->
        :mnesia.abort({:combine_error, tab, ~c"Too many fragments"})
  ...
  defp verify_multi(tab) do
    ...
    :mnesia_schema.verify(
      :undefined,
      foreignKey,
      {:combine_error, tab, ~c"Op only allowed via foreign table", {:foreign_key, foreignKey}}
    )
  end
  ...
  defp adjust_before_split(fH) do
    ...
    try do
      ...
      :error, badFrags ->
        :mnesia.abort({~c"add_frag: Fragment numbers out of range", badFrags, {:range, 1, n}})
    end
  end
    ...
    case key_to_n(fH, hashKey) do
      ...
      newN ->
        case :erlang.element(newN, fragNames) do
          ...
          _NewFrag ->
            :mnesia.abort({~c"add_frag: Fragment not locked", newN})
        end
    end
  end
  ...
  defp adjust_before_merge(fH) do
    ...
    try do
      ...
      case :lists.member(n, fromFrags2) do
        ...
        false ->
          :mnesia.abort({~c"del_frag: Last fragment number not included", n})
      end
    catch
      :error, badFrags ->
        :mnesia.abort({~c"del_frag: Fragment numbers out of range", badFrags, {:range, 1, n}})
    end
  end
  ...
  defp do_merge(fH, oldN, fragNames, [rec | recs], ops) do
    ...
    case key_to_n(fH, hashKey) do
      newN when newN == lastN ->
        :mnesia.abort({~c"del_frag: Fragment number out of range", newN, {:range, 1, lastN}})
      ...
      newN when oldN == lastN ->
        case :erlang.element(newN, fragNames) do
          ...
          _NewFrag ->
            :mnesia.abort({~c"del_frag: Fragment not locked", newN})
        end
  ...
  defp n_to_frag_name(tab, n) when is_atom(tab) and is_integer(n) do
    :erlang.list_to_atom(:erlang.atom_to_list(tab) ++ ~c"_frag" ++ :erlang.integer_to_list(n))
  end
  ...
  defp key_to_n(fH, key) do
    ...
    cond do
      ...
      true ->
        :mnesia.abort(
          {~c"key_to_frag_number: Fragment number out of range", n,
