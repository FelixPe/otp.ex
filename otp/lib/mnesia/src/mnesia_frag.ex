defmodule :m_mnesia_frag do
  ...
  Record.defrecord(:r_frag_state, :frag_state, foreign_key: :undefined,
  ...
  def lock(activityId, opaque, {:table, tab}, lockKind) do
    case (frag_names(tab)) do
      [^tab] ->
        :mnesia.lock(activityId, opaque, {:table, tab},
                       lockKind)
      frags ->
        deepNs = (for f <- frags do
  ...
  def delete_object(activityId, opaque, tab, rec, lockKind) do
    frag = record_to_frag_name(tab, rec)
    :mnesia.delete_object(activityId, opaque, frag, rec,
                            lockKind)
  end
  ...
  def select(activityId, opaque, tab, matchSpec, limit,
           lockKind) do
    init_select(activityId, opaque, tab, matchSpec, limit,
                  lockKind)
  end

  def all_keys(activityId, opaque, tab, lockKind) do
    match = (for frag <- frag_names(tab) do
  ...
  def index_match_object(activityId, opaque, tab, pat, attr, lockKind) do
    match = (for frag <- frag_names(tab) do
               :mnesia.index_match_object(activityId, opaque, frag,
                                            pat, attr, lockKind)
             end)
    :lists.append(match)
  end
  ...
  def index_read(activityId, opaque, tab, key, attr, lockKind) do
    match = (for frag <- frag_names(tab) do
               :mnesia.index_read(activityId, opaque, frag, key, attr,
                                    lockKind)
             end)
    :lists.append(match)
  end
  ...
  def foldl(activityId, opaque, fun, acc, tab, lockKind) do
    fun2 = fn frag, a ->
                :mnesia.foldl(activityId, opaque, fun, a, frag,
                                lockKind)
           end
  ...
  def foldr(activityId, opaque, fun, acc, tab, lockKind) do
    fun2 = fn frag, a ->
                :mnesia.foldr(activityId, opaque, fun, a, frag,
                                lockKind)
           end
  ...
  defp remote_table_info(activityId, opaque, tab, item) do
    ...
    case (:rpc.call(n, :mnesia, :table_info,
                      [activityId, opaque, tab, item])) do
      {:badrpc, _} ->
  ...
  defp init_select(tid, opaque, tab, pat, limit, lockKind) do
    case (try do
      ...
      fH ->
        ...
        [{fTab, node, type} | nameNodes] = :lists.map(fun,
                                                        fragNumbers)
        initFun = fn fixedSpec ->
                       :mnesia.dirty_sel_init(node, fTab, fixedSpec, limit,
                                                type)
                  end
        res = :mnesia.fun_select(tid, opaque, fTab, pat,
                                   lockKind, fTab, initFun, limit, node, type)
  ...
  def select_cont(tid, ts,
           {:frag_cont, :"$end_of_table", [{tab, node, type} | rest], args}) do
    {spec, lockKind, limit} = args
    initFun = fn fixedSpec ->
                   :mnesia.dirty_sel_init(node, tab, fixedSpec, limit,
                                            type)
              end
    res = :mnesia.fun_select(tid, ts, tab, spec, lockKind,
                               tab, initFun, limit, node, type)
    frag_sel_cont(res, rest, args)
  end

  def select_cont(tid, ts, {:frag_cont, cont, tabL, args}) do
    frag_sel_cont(:mnesia.select_cont(tid, ts, cont), tabL,
                    args)
  end
  ...
  defp do_select(activityId, opaque, tab, matchSpec, lockKind) do
    case (try do
      ...
      {:EXIT, _} ->
        :mnesia.select(activityId, opaque, tab, matchSpec,
                         lockKind)
      fH ->
        ...
        fun = fn num ->
                   name = n_to_frag_name(tab, num)
                   node = val({name, :where_to_read})
                   :mnesia.lock(activityId, opaque, {:table, name},
                                  lockKind)
                   {name, node}
              end
        nameNodes = :lists.map(fun, fragNumbers)
        selectAllFun = fn patchedMatchSpec ->
                            match = (for {name, _Node} <- nameNodes do
                                       :mnesia.dirty_select(name,
                                                              patchedMatchSpec)
                                     end)
                            :lists.append(match)
                       end
        case (for {name, node} <- nameNodes, node != node() do
          ...
          [] ->
            :mnesia.fun_select(activityId, opaque, tab, matchSpec,
                                 :none, :_, selectAllFun)
          remoteNameNodes ->
            type = val({tab, :setorbag})
            selectFun = fn patchedMatchSpec ->
                             ref = make_ref()
                             args = [self(), ref, remoteNameNodes,
                                                      patchedMatchSpec]
                             pid = spawn_link(:mnesia_frag, :local_select, args)
                             localMatch0 = (for {name, node} <- nameNodes,
                                                  node == node() do
                                              :mnesia.dirty_select(name,
                                                                     patchedMatchSpec)
                                            end)
                             localMatch = (case (type) do
                                             :ordered_set ->
                             ...
                             local_collect(ref, pid, type, localMatch,
                                             oldSelectFun)
                        end
            :mnesia.fun_select(activityId, opaque, tab, matchSpec,
                                 :none, :_, selectFun)
  ...
  defp verify_numbers(fH, matchSpec) do
    ...
    fragNumbers = (case (r_frag_state(fH, :hash_module)) do
                     hashMod when hashMod == :mnesia_frag_hash ->
                       :mnesia_frag_hash.match_spec_to_frag_numbers(hashState,
                                                                      matchSpec)
                     hashMod ->
                       hashMod.match_spec_to_frag_numbers(hashState, matchSpec)
                   end)
    n = r_frag_state(fH, :n_fragments)
    verifyFun = fn f when (is_integer(f) and f >= 1 and
                             f <= n)
                          ->
                     false
                   _F ->
    ...
    try do
      ...
      :error, badFrags ->
        :mnesia.abort({'match_spec_to_frag_numbers: Fragment numbers out of range', badFrags, {:range, 1, n}})
    end
  end

  def local_select(replyTo, ref, remoteNameNodes, matchSpec) do
    remoteNodes = :mnesia_lib.uniq(for {_Name,
                                          node} <- remoteNameNodes do
                                     node
                                   end)
    args = [replyTo, ref, remoteNameNodes, matchSpec]
    {replies, badNodes} = :rpc.multicall(remoteNodes,
                                           :mnesia_frag, :remote_select, args)
    case (:mnesia_lib.uniq(replies) -- [:ok]) do
      [] when badNodes == [] ->
        send(replyTo, {:local_select, ref, :ok})
      _ when badNodes != [] ->
        send(replyTo, {:local_select, ref,
                         {:error, {:node_not_running, hd(badNodes)}}})
  ...
  defp do_remote_select(replyTo, ref, [{name, node} | nameNodes],
            matchSpec) do
    cond do
      node == node() ->
        res = (try do
  ...
  defp expand_cstruct(cs, mode) do
    ...
    :mnesia_schema.verify({:alt, [nil, :list]},
                            :mnesia_lib.etype(props), {:badarg, tab, props})
    validKeys = [:foreign_key, :n_fragments, :node_pool,
                                                 :n_ram_copies, :n_disc_copies,
                                                                    :n_disc_only_copies,
                                                                        :n_external_copies,
                                                                            :hash_module,
                                                                                :hash_state]
    ...
    foreignKey = :mnesia_schema.pick(tab, :foreign_key,
                                       props, :undefined)
    {foreignKey2, n, pool, defaultNR, defaultND, defaultNDO,
       defaultNExt} = pick_props(tab, cs, foreignKey)
    badPool = {:bad_type, tab, {:node_pool, pool}}
    :mnesia_schema.verify(:list, :mnesia_lib.etype(pool),
                            badPool)
    notAtom = fn a when is_atom(a) ->
                   false
                 _A ->
                   true
              end
    ...
    nDO = :mnesia_schema.pick(tab, :n_disc_only_copies,
                                props, 0)
    nExt = :mnesia_schema.pick(tab, :n_external_copies,
                                 props, 0)
    posInt = fn i when (is_integer(i) and i >= 0) ->
                  true
                _I ->
                  false
             end
    ...
    hashMod = :mnesia_schema.pick(tab, :hash_module, props,
                                    :mnesia_frag_hash)
    hashState = :mnesia_schema.pick(tab, :hash_state, props,
                                      :undefined)
    hashState2 = hashMod.init_state(tab, hashState)
    fH = r_frag_state(foreign_key: foreignKey2, n_fragments: 1,
             hash_module: hashMod, hash_state: hashState2)
    cond do
      (nR == 0 and nD == 0 and nDO == 0 and nExt == 0) ->
        do_expand_cstruct(cs2, fH, n, pool, defaultNR,
                            defaultND, defaultNDO, defaultNExt, mode)
      true ->
        do_expand_cstruct(cs2, fH, n, pool, nR, nD, nDO, nExt,
                            mode)
    end
  end
  ...
  defp do_expand_cstruct(cs, fH, n, pool, nR, nD, nDO, nExt, mode) do
    ...
    expand_frag_cstructs(n, nR, nD, nDO, nExt, cs2, pool,
                           pool, fH, mode)
  end
  ...
  defp verify_n_fragments(n, cs, mode) when (is_integer(n) and n >= 1) do
    case (mode) do
      :create ->
        r_cstruct(cs, ram_copies: [],  disc_copies: [], 
                disc_only_copies: [],  external_copies: [])
      :activate ->
        reason = {:combine_error, r_cstruct(cs, :name),
                    {:n_fragments, n}}
  ...
  defp verify_n_fragments(n, cs, _Mode) do
    :mnesia.abort({:bad_type, r_cstruct(cs, :name),
                     {:n_fragments, n}})
  end
  ...
  defp pick_props(tab, cs, {foreignTab, attr}) do
    ...
    n = :mnesia_schema.pick(tab, :n_fragments, props,
                              foreignN)
    pool = :mnesia_schema.pick(tab, :node_pool, props,
                                 foreignPool)
    :mnesia_schema.verify(foreignN, n,
                            {:combine_error, tab, {:n_fragments, n}, foreignTab,
                               {:n_fragments, foreignN}})
    ...
    :mnesia_schema.verify(:undefined, foreignKey,
                            {:combine_error, tab, 'Multiple levels of foreign_key dependencies', {foreignTab, attr},
                               foreignKey})
    key = {foreignTab,
             :mnesia_schema.attr_to_pos(attr, attrs)}
    defaultNR = length(val({foreignTab, :ram_copies}))
    defaultND = length(val({foreignTab, :disc_copies}))
    defaultNDO = length(val({foreignTab,
                               :disc_only_copies}))
    defaultNExt = length(val({foreignTab,
                                :external_copies}))
  ...
  defp expand_frag_cstructs(n, nR, nD, nDO, nExt, commonCs, dist, pool, fH,
            mode)
      when (n > 1 and mode == :create) do
    frag = n_to_frag_name(r_cstruct(commonCs, :name), n)
    cs = r_cstruct(commonCs, name: frag)
    {cs2, revModDist, restDist} = set_frag_nodes(nR, nD,
                                                   nDO, nExt, cs, dist, [])
    modDist = :lists.reverse(revModDist)
    dist2 = rearrange_dist(cs, modDist, restDist, pool)
    {fH2, _FromFrags,
       _AdditionalWriteFrags} = adjust_before_split(fH)
    csList = expand_frag_cstructs(n - 1, nR, nD, nDO, nExt,
                                    commonCs, dist2, pool, fH2, mode)
    [cs2 | csList]
  end

  defp expand_frag_cstructs(1, nR, nD, nDO, nExt, commonCs, dist, pool, fH,
            mode) do
    baseProps = r_cstruct(commonCs, :frag_properties) ++ [{:foreign_key,
                                                     r_frag_state(fH, :foreign_key)},
                                                      {:hash_module,
                                                         r_frag_state(fH, :hash_module)},
                                                          {:hash_state,
                                                             r_frag_state(fH, :hash_state)},
                                                              {:n_fragments,
                                                                 r_frag_state(fH, :n_fragments)},
                                                                  {:node_pool,
                                                                     pool}]
    ...
    case (mode) do
      :activate ->
        [baseCs]
      :create ->
        {baseCs2, _, _} = set_frag_nodes(nR, nD, nDO, nExt,
                                           baseCs, dist, [])
        [baseCs2]
    end
  end
  ...
  defp set_frag_nodes(nR, nD, nDO, nExt, cs, [head | tail], acc)
    ...
    set_frag_nodes(nR - 1, nD, nDO, nExt, cs2, tail,
                     [head2 | acc])
  end
  ...
  defp set_frag_nodes(nR, nD, nDO, nExt, cs, [head | tail], acc)
    ...
    {cs2, head2} = set_frag_node(cs, pos, head)
    set_frag_nodes(nR, nD - 1, nDO, nExt, cs2, tail,
                     [head2 | acc])
  end
  ...
  defp set_frag_nodes(nR, nD, nDO, nExt, cs, [head | tail], acc)
    ...
    {cs2, head2} = set_frag_node(cs, pos, head)
    set_frag_nodes(nR, nD, nDO - 1, nExt, cs2, tail,
                     [head2 | acc])
  end
  ...
  defp set_frag_nodes(nR, nD, nDO, nExt, cs, [head | tail], acc)
    ...
    {cs2, head2} = set_frag_node(cs, pos, head)
    set_frag_nodes(nR, nD, nDO, nExt - 1, cs2, tail,
                     [head2 | acc])
  end
  ...
  defp set_frag_node(cs, pos, head) do
    ...
    {node, count2} = (case (head) do
                        {n, count} when (is_atom(n) and is_integer(count) and
                                           count >= 0)
                                        ->
                          {n, count + 1}
                        n when is_atom(n) ->
  ...
  defp insert_dist(cs, node, count, [head | tail], pool) do
    case (head) do
      {node2, count2} when (is_atom(node2) and
  ...
  defp make_activate(tab, props) do
    ...
    case (r_cstruct(cs, :frag_properties)) do
      [] ->
        ...
        [[op]]
      badProps ->
        :mnesia.abort({:already_exists, tab,
                         {:frag_properties, badProps}})
    end
  end
  ...
  defp make_multi_add_frag(tab, sortedNs) when is_list(sortedNs) do
    ...
    moreOps = (for t <- lookup_foreigners(tab) do
  ...
  defp verify_multi(tab) do
    ...
    :mnesia_schema.verify(:undefined, foreignKey,
                            {:combine_error, tab, 'Op only allowed via foreign table',
  ...
  defp make_add_frag(tab, sortedNs) do
    ...
    fH = lookup_frag_hash(tab)
    {fH2, fromIndecies,
       writeIndecies} = adjust_before_split(fH)
    n = r_frag_state(fH2, :n_fragments)
    fragNames = make_frag_names_and_acquire_locks(tab, n,
                                                    writeIndecies, true)
    ...
    newCs = r_cstruct(cs, name: newFrag, 
                    frag_properties: [{:base_table, tab}],  ram_copies: [], 
                    disc_copies: [],  disc_only_copies: [], 
                    external_copies: [])
    {newCs2, _, _} = set_frag_nodes(nR, nD, nDO, nExt,
                                      newCs, sortedNs, [])
    [newOp] = :mnesia_schema.make_create_table(newCs2)
    splitOps = split(tab, fH2, fromIndecies, fragNames, [])
    cs2 = replace_frag_hash(cs, fH2)
    tabDef = :mnesia_schema.vsn_cs2list(cs2)
    baseOp = {:op, :change_table_frag,
                {:add_frag, sortedNs}, tabDef}
  ...
  defp adjust_before_split(fH) do
    hashState = r_frag_state(fH, :hash_state)
    {hashState2, fromFrags,
       additionalWriteFrags} = (case (r_frag_state(fH, :hash_module)) do
                                  hashMod when hashMod == :mnesia_frag_hash ->
    ...
    try do
      ...
      unionFrags = :lists.merge(fromFrags2,
  ...
  defp split(tab, fH, [splitN | splitNs], fragNames, ops) do
    ...
    {_Mod, tid,
       ts} = :mnesia_schema.get_tid_ts_and_lock(tab, :none)
    recs = :mnesia.match_object(tid, ts, splitFrag, pat,
                                  :read)
  ...
  defp make_del_frag(tab) do
    ...
    case (r_frag_state(fH, :n_fragments)) do
      n when n > 1 ->
        ...
        :mnesia_schema.ensure_active(cs)
        {fH2, fromIndecies,
           writeIndecies} = adjust_before_merge(fH)
        fragNames = make_frag_names_and_acquire_locks(tab, n,
                                                        writeIndecies, false)
        mergeOps = merge(tab, fH2, fromIndecies, fragNames, [])
        lastFrag = :erlang.element(n, fragNames)
        [lastOp] = :mnesia_schema.make_delete_table(lastFrag,
                                                      :single_frag)
        cs2 = replace_frag_hash(cs, fH2)
        tabDef = :mnesia_schema.vsn_cs2list(cs2)
  ...
  defp merge(tab, fH, [fromN | fromNs], fragNames, ops) do
    ...
    {_Mod, tid,
       ts} = :mnesia_schema.get_tid_ts_and_lock(tab, :none)
    recs = :mnesia.match_object(tid, ts, fromFrag, pat,
                                  :read)
  ...
  defp make_add_node(tab, node) when is_atom(node) do
    ...
    case (:lists.member(node, pool)) do
      false ->
        ...
        pool2 = pool ++ [node]
        props = r_cstruct(cs, :frag_properties)
        props2 = :lists.keyreplace(:node_pool, 1, props,
                                     {:node_pool, pool2})
        cs2 = r_cstruct(cs, frag_properties: props2)
        tabDef = :mnesia_schema.vsn_cs2list(cs2)
        op = {:op, :change_table_frag, {:add_node, node},
                tabDef}
        [op]
      true ->
  ...
  defp make_del_node(tab, node) when is_atom(node) do
    ...
    case (:lists.member(node, pool)) do
      true ->
        ...
        cs2 = r_cstruct(cs, frag_properties: props)
        tabDef = :mnesia_schema.vsn_cs2list(cs2)
        op = {:op, :change_table_frag, {:del_node, node},
                tabDef}
        [op]
      false ->
  ...
  defp props_to_frag_hash(tab, props) do
    case (:mnesia_schema.pick(tab, :base_table, props,
                                :undefined)) do
      t when t == tab ->
        foreign = :mnesia_schema.pick(tab, :foreign_key, props,
                                        :must)
        n = :mnesia_schema.pick(tab, :n_fragments, props, :must)
        case (:mnesia_schema.pick(tab, :hash_module, props,
                                    :undefined)) do
          :undefined ->
            :no_hash
          hashMod ->
            hashState = :mnesia_schema.pick(tab, :hash_state, props,
                                              :must)
            r_frag_state(foreign_key: foreign, n_fragments: n,
                hash_module: hashMod, hash_state: hashState)
        end
      _ ->
        :no_hash
    end
  ...
  defp lookup_prop(tab, prop) do
    ...
    case (:lists.keysearch(prop, 1, props)) do
      {:value, {^prop, val}} ->
        val
      false ->
        :mnesia.abort({:no_exists, tab, prop,
                         {:frag_properties, props}})
    end
  end
  ...
  def lookup_frag_hash(tab) do
    case (try do
      ...
      {:frag_hash, _K, _N, _S, _D} ->
        :mnesia.abort({:no_hash, tab, :frag_properties,
                         :frag_hash})
      {:EXIT, _} ->
        :mnesia.abort({:no_exists, tab, :frag_properties,
                         :frag_hash})
    end
  end
  ...
  def lookup_foreigners(tab) do
    ...
    for [t] <- :ets.match(:mnesia_gvar,
                            {{:"$1", :frag_hash}, hashPat}) do
      t
    end
  end
  ...
  defp key_to_n(fH, key) do
    hashState = r_frag_state(fH, :hash_state)
    n = (case (r_frag_state(fH, :hash_module)) do
           hashMod when hashMod == :mnesia_frag_hash ->
    ...
    cond do
      (is_integer(n) and n >= 1 and
        ...
        n
      true ->
        :mnesia.abort({'key_to_frag_number: Fragment number out of range', n, {:range, 1, r_frag_state(fH, :n_fragments)}})
    end
  end
  ...
  defp count_frag([frag | frags], dist) do
    ...
    dist4 = incr_nodes(val({frag, :disc_only_copies}),
                         dist3)
  ...
  defp deep_dist([head | tail], deep) do
    ...
    {tag, same, other} = pick_count(kind, count,
                                      [head | tail])
