defmodule :m_mnesia_schema do
  ...
  def init(ignoreFallback) do
    ...
    verbose(~c"Schema initiated from: ~p~n", [source])
    set({:schema, :tables}, [])
  ...
  def delete_schema(ns) when is_list(ns) and ns != [] do
    ...
    reason = ~c"Cannot delete schema on all nodes"
        ...
        case :rpc.multicall(ns, :mnesia_schema, :delete_schema2, []) do
          {replies, []} ->
            case (for r <- replies, r != :ok do
              ...
              badReplies ->
                verbose(~c"~s: ~tp~n", [reason, badReplies])
                {:error, {~c"All nodes not running", badReplies}}
            end

          {_Replies, badNs} ->
            verbose(~c"~s: ~p~n", [reason, badNs])
            {:error, {~c"All nodes not running", badNs}}
        end

      true ->
        verbose(~c"~s: ~p~n", [reason, runningNs])
        {:error, {~c"Mnesia is not stopped everywhere", runningNs}}
    end
  end
    ...
    case :mnesia_lib.ensure_loaded(:mnesia) do
      :ok ->
        case :mnesia_lib.is_running() do
          ...
          _ ->
            {:error, {~c"Mnesia still running", node()}}
        end
  ...
  def ensure_no_schema([h | t]) when is_atom(h) do
    case :rpc.call(h, :mnesia_schema, :remote_read_schema, []) do
      {:badrpc, reason} ->
        {h, {~c"All nodes not running", h, reason}}
    ...
    case dir_exists(dir, useDir) do
      ...
      false ->
        case :file.make_dir(dir) do
          :ok ->
            verbose(~c"Create Directory ~tp~n", [dir])
            :ok

          {:error, reason} ->
            verbose(~c"Cannot create mnesia dir ~tp~n", [reason])
            {:error, {~c"Cannot create Mnesia dir", dir, reason}}
        end
    end
  end
  ...
  defp check_can_write(dir) do
    case :file.read_file_info(dir) do
      ...
      {:ok, _} ->
        {:error, ~c"Not allowed to write in Mnesia dir", dir}

      _ ->
        {:error, ~c"Non existent Mnesia dir", dir}
    end
  end
  ...
  defp do_read_disc_schema(fname, keep) do
    ...
    res =
      case :mnesia_lib.dets_to_ets(:schema, t, fname, :set, repair, :no) do
        ...
        other ->
          {:error, {~c"Cannot read schema", fname, other}}
      end
  ...
  def pick(tab, key, list, default) do
    case :lists.keysearch(key, 1, list) do
      false when default == :must ->
        :mnesia.abort({:badarg, tab, ~c"Missing key", key, list})
  ...
  def check_duplicates(tab, keys) do
    case has_duplicates(keys) do
      ...
      true ->
        :mnesia.abort({:badarg, tab, ~c"Duplicate keys", keys})
    end
  end
  ...
  defp ensure_active(cs, what) do
    ...
    case nodes -- val(w) do
      ...
      ns ->
        expl = ~c"All replicas on diskfull nodes are not active yet"
  ...
  defp verify_backend_type(name, module) do
    ...
    case expectedExports -- exports do
      ...
      _Other ->
        :io.fwrite(:user, ~c"Missing backend_type exports: ~tp~n", [_Other])
        :mnesia.abort({:bad_type, {:backend_type, name, module}})
    end
  end
          ...
          :whole_table ->
            case val({tab, :frag_properties}) do
              ...
              _Props ->
                ...
                verify([], f, {:combine_error, tab, ~c"Too many foreigners", f})
  ...
  defp make_del_table_copy(tab, node) do
    ...
    case :mnesia_lib.cs_to_nodes(cs2) do
      [] when tab == :schema ->
        :mnesia.abort({:combine_error, tab, ~c"Last replica"})

      [] ->
        ensure_active(cs)
        dbg_out(~c"Last replica deleted in table ~tp~n", [tab])
        make_delete_table(tab, :whole_table)
  ...
  defp make_move_table(tab, fromNode, toNode) do
    ...
    verify(false, val({tab, :local_content}), {~c"Cannot move table with local content", tab})
    ensure_active(cs)
  ...
  def do_write_table_property(tab, prop) do
    ...
    case change_prop_in_existing_op(tab, prop, :write_property, store) do
      true ->
        dbg_out(~c"change_prop_in_existing_op(~tp,~p,write_property,Store) -> true~n", [tab, prop])

        :ok

      false ->
        dbg_out(~c"change_prop_in_existing_op(~tp,~p,write_property,Store) -> false~n", [
          tab,
          prop
        ])
  ...
  def do_delete_table_property(tab, propKey) do
    ...
    case change_prop_in_existing_op(tab, propKey, :delete_property, store) do
      true ->
        dbg_out(~c"change_prop_in_existing_op(~tp,~p,delete_property,Store) -> true~n", [
          tab,
          propKey
        ])
      ...
      false ->
        dbg_out(~c"change_prop_in_existing_op(~tp,~p,delete_property,Store) -> false~n", [
          tab,
          propKey
        ])
  ...
  def prepare_commit(tid, commit, waitFor) do
    case r_commit(commit, :schema_ops) do
      ...
      origOps ->
        ...
        case dumperMode do
          :optional ->
            dbg_out(~c"Transaction log dump skipped (~p): ~w~n", [dumperMode, initBy])
  ...
  defp prepare_op(_Tid, {:op, :sync_trans}, {:part, coordPid}) do
    ...
    receive do
      ...
      {:mnesia_down, _Node} = else__ ->
        :mnesia_lib.verbose(~c"sync_op terminated due to ~tp~n", [else__])
        :mnesia.abort(else__)
  ...
  defp prepare_op(_Tid, {:op, :sync_trans}, {:coord, nodes}) do
    case receive_sync(nodes, []) do
      {:abort, reason} ->
        :mnesia_lib.verbose(~c"sync_op terminated due to ~tp~n", [reason])
        :mnesia.abort(reason)
      ...
      node == node() ->
        case :mnesia_lib.val({:schema, :storage_type}) do
          :ram_copies when storage != :ram_copies ->
            error = {:combine_error, tab, ~c"has no disc", node}
            :mnesia.abort(error)
            ...
            case :mnesia_lib.schema_cs_to_storage_type(
              ...
              :ram_copies ->
                ...
                case :mnesia_lib.exists(dat) do
                  true ->
                    :mnesia.abort(
                      {:combine_error, tab, storage, ~c"Table dumped to disc", node()}
                    )
    ...
    cond do
        ...
        case {fromS, toS} do
          {:ram_copies, :disc_copies} ->
            case :mnesia.system_info(:schema_location) do
              ...
              _ ->
                :mnesia.abort({:combine_error, tab, node(), ~c"schema_location must be opt_disc"})
            end
          ...
          {:disc_copies, :ram_copies} ->
            ...
            verify([], dtabs, {~c"Disc resident tables", dtabs, n})
      ...
      fromS == :ram_copies ->
        case :mnesia_monitor.use_dir() do
          true ->
            ...
            case :mnesia_lib.exists(dat) do
              true ->
                :mnesia.abort({:combine_error, tab, node(), ~c"Table dump exists"})
      ...
      fromS == :disc_only_copies ->
        ...
        case :mnesia_lib.dets_to_ets(tab, tab, datname, type, repair, :no) do
          ...
          reason ->
            err = ~c"Failed to copy disc data to ram"
      ...
      storage ->
        ...
        try do
          ...
          _, reason ->
            ...
            :mnesia_lib.important(~c"Transform function failed: '~tp' in '~tp'", [
              reason,
              __STACKTRACE__
            ])

            exit({~c"Bad transform function", tab, fun, node(), reason})
        else
          objs ->
            :mnesia_lib.db_fixtable(storage, tab, false)
            {true, objs, :mandatory}
  ...
  defp prepare_op(_Tid, {:op, :merge_schema, tabDef}, _WaitFor) do
    ...
    case verify_merge(cs) do
      ...
      error ->
        verbose(~c"Merge_Schema ~p failed on ~p: ~tp~n", [_Tid, node(), error])
  ...
  defp create_ram_table(
    ...
    case :mnesia_monitor.unsafe_mktab(tab, args) do
      ...
      {:error, reason} ->
        err = ~c"Failed to create ets table"
        :mnesia.abort({:system_limit, tab, {err, reason}})
    end
  end
  ...
  defp create_disc_table(tab) do
    ...
    case :mnesia_monitor.open_log(fArg) do
      ...
      {:error, reason} ->
        err = ~c"Failed to create disc table"
        :mnesia.abort({:system_limit, tab, {err, reason}})
    end
  end
  ...
  defp create_external_table(alias, tab, mod, cs) do
    case :mnesia_monitor.unsafe_create_external(tab, alias, mod, cs) do
      ...
      {:error, reason} ->
        err = ~c"Failed to create external table"
        :mnesia.abort({:system_limit, tab, {err, reason}})
    end
  end
  ...
  defp transform_obj(tab, recName, key, fun, [obj | rest], newArity, type, ws, ds) do
    ...
    cond do
      ...
      true ->
        exit({~c"Bad key or Record Name", obj, newObj})
    end
  end
  ...
  defp real_suffixes(ext) do
    [~c".DAT", ~c".LOG", ~c".BUP", ~c".DCL", ~c".DCD"] ++ ext_real_suffixes(ext)
  end
  ...
  defp tmp_suffixes(ext) do
    [~c".TMP", ~c".BUPTMP", ~c".RET", ~c".DMP", ~c"."] ++ ext_tmp_suffixes(ext)
  end
  ...
  defp ext_real_suffixes(ext) do
    try do
      ...
      :error, e ->
        verbose(~c"Can't find real ext suffixes (~tp)~n", [e])
        []
    end
  end
  ...
  defp ext_tmp_suffixes(ext) do
    try do
      ...
      :error, e ->
        verbose(~c"Can't find tmp ext suffixes (~tp)~n", [e])
        []
    end
  end
  ...
  def info(tab) do
    ...
    :io.format(~c"-- Properties for ~tw table --- ~n", [tab])
    info2(tab, props)
  end
  ...
  defp make_dump_tables([tab | tabs]) do
    ...
    verify([], discResident, {~c"Only allowed on ram_copies", tab, discResident})
    ...
    case connected -- running do
      [node | _] = otherNodes ->
        ...
        case fetch_cstructs(node) do
          ...
          {:error, reason} ->
            {~c"Cannot get cstructs", node, reason}

          {:badrpc, reason} ->
            {~c"Cannot get cstructs", node, {:badrpc, reason}}
        end
  ...
  defp warn_user_connect_failed(missing) do
    ...
    case (try do
      ...
      {:EXIT, _} ->
        :mnesia_lib.error(~c"Mnesia on ~p could not connect to node(s) ~p~n", [node(), missing])
  ...
  defp do_make_merge_schema(node, needsConv, remoteCs = r_cstruct(name: :schema)) do
    ...
    cond do
      ...
      r_cstruct(cs, :cookie) != r_cstruct(remoteCs, :cookie) and
        ...
        cond do
          ...
          true ->
            str =
              :io_lib.format(
                ~c"Incompatible schema cookies. Please, restart from old backup.~w = ~w, ~w = ~w~n",
      ...
      stCsLocal != stRcsLocal and stRcsLocal != :unknown and
        ...
        str =
          :io_lib.format(
            ~c"Incompatible schema storage types (local). on ~w storage ~w, on ~w storage ~w~n",
      ...
      stCsRemote != stRcsRemote and
        ...
        str =
          :io_lib.format(
            ~c"Incompatible schema storage types (remote). on ~w cs ~w, on ~w rcs ~w~n",
      ...
      cs ->
        cond do
          ...
          true ->
            str =
              :io_lib.format(
                ~c"Bad cookie in table definition ~w: ~w = ~w, ~w = ~w~n",
  ...
  defp merge_storage_type([n | ns], anythingNew, cs, remoteCs, force) do
    ...
    case compare_storage_type(true, local, remote) do
      ...
      other ->
        str =
          :io_lib.format(
            ~c"Cannot merge storage type for node ~w in cstruct ~w with remote cstruct ~w (~w)~n",
            [n, cs, remoteCs, other]
          )
  ...
  defp merge_versions(anythingNew, cs, remoteCs, force) do
    cond do
      ...
      true ->
        str =
          :io_lib.format(
            ~c"Bad cookies. Cannot merge definitions of table ~tw. Local = ~w, Remote = ~w~n",
    ...
    cond do
      ...
      true ->
        str1 =
          :io_lib.format(~c"Cannot merge definitions of table ~tw. Local = ~w, Remote = ~w~n", [
            r_cstruct(cs, :name),
            cs,
            remoteCs
          ])
  ...
  defp announce_im_running([n | ns], schemaCs) do
    ...
    case :erlang.or(
      ...
      false ->
        :mnesia_lib.error(~c"Mnesia on ~p could not connect to node ~p~n", [node(), n])
