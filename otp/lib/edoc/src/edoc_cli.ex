defmodule :m_edoc_cli do
  ...
  defp parse_args(args) do
    init = %{mode: :default, run: :app, app: :no_app,
               files: [], code_paths: [], out_dir: :undefined,
               include_paths: [], continue: false}
    check_opts(:maps.without([:continue],
  ...
  defp check_opts(opts) do
    case (opts) do
      %{run: :app, app: app} when (is_atom(app) and
                                     app != :no_app)
                                  ->
        :ok
      %{run: :app, app: :no_app} ->
    ...
    %{mode: mode, out_dir: outDir, code_paths: codePaths,
        include_paths: includePaths} = opts
    :lists.member(mode,
                    [:default, :chunks]) or :erlang.error(:mode, opts)
    cond do
      is_list(outDir) ->
        :ok
      outDir === :undefined ->
  ...
  defp edoc_opts(opts) do
    edocOpts = (case (:maps.get(:mode, opts)) do
                  :default ->
                    [{:preprocess, true}]
                  :chunks ->
                    [{:doclet, :edoc_doclet_chunks}, {:layout,
                                                        :edoc_layout_chunks},
                                                         {:preprocess, true}]
                end)
    outDir = :maps.get(:out_dir, opts)
    [{:includes, :maps.get(:include_paths, opts)} |
         edocOpts] ++ (for _ <- [:EFE_DUMMY_GEN],
                             outDir != :undefined do
                         {:dir, outDir}
                       end)
  end
