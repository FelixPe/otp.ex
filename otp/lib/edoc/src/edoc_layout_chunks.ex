defmodule :m_edoc_layout_chunks do
  ...
  defp edoc_to_chunk(doc, opts) do
    ...
    metadata = :maps.from_list(meta_deprecated(doc,
                                                 opts) ++ meta_since(doc, opts))
  ...
  defp doc_visibility(_XPath, doc, opts) do
    case ({xpath_to_text('./@private', doc, opts),
             ...
             xpath_to_text('./@hidden', doc, opts)}) do
      {"yes", true, _} ->
        :show
      {"yes", _, _} ->
        :hidden
  ...
  defp doc_entries(doc, opts) do
    types(doc, opts) ++ callbacks(doc,
                                    opts) ++ functions(doc, opts)
  end

  defp types(doc, opts) do
    for tD <- :xmerl_xpath.string('//typedecls/typedecl', doc) do
      type(tD, opts)
    end
  end
  ...
  defp type(doc, opts) do
    ...
    [r_xmlElement(content: content)] = :xmerl_xpath.string('./typedef/argtypes', doc)
    arity = length(content)
    anno = anno(doc, opts)
    signature = [:erlang.list_to_binary(:erlang.atom_to_list(name) ++ '/' ++ :erlang.integer_to_list(arity))]
    entryDoc = doc_contents('./description/fullDescription', doc, opts)
    metadata = :maps.from_list(meta_deprecated(doc,
                                                 opts) ++ meta_since(doc,
                                                                       opts) ++ meta_type_sig(name,
                                                                                                arity,
                                                                                                anno,
                                                                                                entries(opts)))
    docs_v1_entry(:type, name, arity, anno, signature,
                    entryDoc, metadata)
  end
  ...
  defp select_tag(r_tag(name: :type, line: line, origin: :code) = t,
            name, arity, line) do
    typeTree = r_tag(t, :form)
    typeAttr = :erl_syntax.revert(typeTree)
    case (typeAttr) do
      {:attribute, _, type, {^name, _, args}}
          when (:type === type or :opaque === type and
                  length(args) == arity)
               ->
        {true, typeAttr}
      _ ->
  ...
  defp callback(cb = r_tag(name: :callback, origin: :code), opts) do
    r_tag(line: line, data: {{name, arity}, maybeDoc},
        form: form) = cb
    entryDoc = (case (maybeDoc) do
                  :none ->
                    :none
                  _ ->
    ...
    docs_v1_entry(:callback, name, arity, anno, signature,
                    entryDoc, metadata)
  end

  defp functions(doc, opts) do
    for f <- :xmerl_xpath.string('//module/functions/function', doc) do
      function(f, opts)
    end
  end
  ...
  defp function(doc, opts) do
    ...
    {line, signature, spec} = function_line_sig_spec({name,
                                                        arity},
                                                       opts)
    {:source, file} = :lists.keyfind(:source, 1, opts)
    anno = :erl_anno.set_file(file, :erl_anno.new(line))
    ...
    metadata = :maps.from_list(meta_deprecated(doc,
                                                 opts) ++ meta_since(doc,
                                                                       opts) ++ spec)
    docs_v1_entry(:function, name, arity, anno, signature,
                    entryDoc, metadata)
  end

  defp function_line_sig_spec(nA, opts) do
    entries = entries(opts)
    r_entry(name: ^nA, line: line) = (e = :lists.keyfind(nA,
                                                     r_entry(:name), entries))
  ...
  defp annotate_spec_(argClauses,
            {:attribute, pos, :spec, data} = spec) do
    {nA, specClauses} = data
    case ((try do
      ...
      {_, {:function_clause, [{:lists, :zip, _, _} | _]}} ->
        :edoc_report.warning('cannot annotate spec: function and spec clause numbers do not match\n', [])
        spec
      argSpecClauses ->
        newData = {nA,
  ...
  defp annotate_clause(argNames, {:type, pos, :fun, data}) do
    ...
    annArgTypes = (for {name, type} <- :lists.zip(argNames,
                                                    argTypes) do
  ...
  defp annotate_clause(argNames, {:type, pos, :bounded_fun, data}) do
    ...
    {newClause,
       newConstraints} = annotate_bounded_fun_clause(argNames,
                                                       clause, constraints)
  ...
  defp ann_fun_type(name, pos, type) do
    typeVar = :erl_syntax.set_pos(:erl_syntax.variable(name),
                                    pos)
    annType = :erl_syntax.set_pos(:erl_syntax.annotated_type(typeVar,
                                                               type),
                                    pos)
    :erl_syntax.revert(annType)
  end

  defp annotate_bounded_fun_clause(argNames, {:type, pos, :fun, data},
            constraints) do
    [{:type, _, :product, args}, retType] = data
    newVarsAndConstraints = :lists.foldl(fn {name, arg},
                                              acc ->
                                              bounded_fun_arg(%{acc
                                                                |
                                                                name: name,
    ...
    %{new_vars: typeVars,
        new_constraints: newConstraints} = newVarsAndConstraints
    length(argNames) == length(typeVars) or :erlang.error({:bounded_fun_arity,
                                                             typeVars})
    newConstraints2 = (case (retType) do
                         {:var, _, _} ->
    ...
    newData = [{:type, pos, :product,
                  :lists.reverse(typeVars)},
                   retType]
    {{:type, pos, :fun, newData},
       :lists.reverse(newConstraints2)}
  end
  ...
  defp bounded_fun_arg(%{arg: {:var, _, _} = v} = acc) do
    %{new_vars: nVs, new_constraints: nCs, constraints: cs,
        ret_type: retType} = acc
  ...
  defp bounded_fun_arg(%{arg: type} = acc)
    ...
    var = :erl_syntax.revert(:erl_syntax.set_pos(:erl_syntax.variable(name),
                                                   pos))
    bounded_fun_arg_(var, type, acc)
  end
  ...
  defp bounded_fun_arg_(var, type, acc) do
    ...
    c = {:type, pos, :constraint,
           [{:atom, pos, :is_subtype}, [var, type]]}
    %{acc
      |
      new_vars: [var | nVs], new_constraints: [c | nCs]}
  end

  defp get_constraint({:var, _, name}, constraints) do
    f = fn {:type, _, :constraint,
              [_, [{:var, _, cName}, _]]}
               when name === cName ->
             true
           _ ->
  ...
  defp docs_v1(anno, moduleDoc, metadata, docs) do
    r_docs_v1(anno: anno, module_doc: moduleDoc, metadata: metadata,
        docs: docs)
  end
  ...
  defp docs_v1_entry(kind, name, arity, anno, signature, entryDoc,
            metadata) do
    {{kind, name, arity}, anno, signature, entryDoc,
       metadata}
  end
  ...
  defp xmerl_to_binary(xML, opts) do
    :erlang.iolist_to_binary(chunk_to_text(xmerl_to_chunk(xML,
                                                            opts)))
  end
  ...
  defp xpath_to_atom(xPath, doc, opts) do
    :erlang.binary_to_atom(xpath_to_text(xPath, doc, opts),
                             :utf8)
  end

  defp xpath_to_integer(xPath, doc, opts) do
    :erlang.binary_to_integer(xpath_to_text(xPath, doc,
                                              opts))
  end
  ...
  defp format_content(contents, opts) do
    {seeTags,
       otherTags} = :lists.partition(fn r_xmlElement(name: :see) ->
                                          true
                                        _ ->
                                          false
                                     end,
                                       contents)
    :lists.flatten((for t <- otherTags do
  ...
  defp format_element(r_xmlElement() = e, opts) do
    r_xmlElement(name: name, content: content,
  ...
  defp rewrite_see_tags([r_xmlElement(name: :see) | _] = seeTags, opts) do
    grouped = (for t <- seeTags do
                 rewrite_see_tag(t)
               end)
    newXML = {:p, [],
                [{:em, [], ['See also: ']}] ++ :lists.join(', ', grouped) ++ ['.']}
  ...
  defp rewrite_docgen_link({tag, attrL, subEls} = e) when tag === :a or
                                           tag === :see do
    attrs = :maps.from_list(attrL)
    case ({:maps.get(:"docgen-rel", attrs, false),
             :maps.get(:"docgen-href", attrs, false)}) do
      {false, false} ->
        e
      {false, _} ->
      ...
      {shortRel, uRI} ->
        ...
        newAttrs = Map.merge(attrsNoDocgen, %{rel:
                                              expand_docgen_rel(shortRel),
                                                href: uRI})
  ...
  defp expand_docgen_rel(rel) when rel === 'seemfa' or rel === 'seeerl' or rel === 'seetype' or
                      rel === 'seeapp' or rel === 'seecom' or rel === 'seecref' or rel === 'seefile' or
                      rel === 'seeguide' do
    'https://erlang.org/doc/link/' ++ rel
  end

  defp rewrite_equiv_tag(r_xmlElement(name: :equiv) = e) do
    newE = (case (:xmerl_lib.simplify_element(e)) do
              {:equiv, [], [{:expr, [], expr}]} ->
                {:p, [], ['Equivalent to ', expr, '.']}
              {:equiv, [],
                 [{:expr, [], expr}, {:see, _, _} = seeTag]} ->
