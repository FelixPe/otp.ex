defmodule :m_edoc_macros do
  ...
  def std_macros(env) do
    (cond do
     ...
     end) ++ [{:date, &date_macro/3}, {:docRoot,
                                         r_env(env, :root)},
                                          {:link, &link_macro/3}, {:section,
                                                                     &section_macro/3},
                                                                      {:time,
                                                                         &time_macro/3},
                                                                          {:type,
  ...
  def check_defs([{k, d} | ds]) when (is_atom(k) and
                                is_list(d)) do
    check_defs(ds)
  end
  ...
  defp version_macro(s, line, env) do
    date_macro(s, line, env) ++ ' ' ++ time_macro(s, line,
                                                  env)
  end
  ...
  defp link_macro(s, line, env) do
    ...
    target = (case (:edoc_refs.is_top(ref, env)) do
                true ->
                  ' target="_top"'
                false ->
                  ''
  ...
  defp expand_tags([r_tag(data: cs, line: l) = t | ts], defs, env,
            where) do
  ...
  Record.defrecord(:r_state, :state, where: :undefined,
                                 env: :undefined, seen: :undefined)
    ...
    case (:sets.is_element(m, seen)) do
      ...
      false ->
        ...
        case (:dict.find(m, defs)) do
          ...
          :error ->
            :edoc_report.warning(l, r_state(st1, :where), 'undefined macro {@~s}.', [m])
            '??'
        end
    end
  end
  ...
  defp macro_name([c | cs], as, l) when (c >= 192 and c <= 255 and
                                   c !== 215 and c !== 247) do
    macro_name_1(cs, [c | as], l)
  end
  ...
  defp macro_name_1([c | cs], as, l) when (c >= 192 and c <= 255 and
                                   c !== 215 and c !== 247) do
    macro_name_1(cs, [c | as], l)
  end
