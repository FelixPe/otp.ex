defmodule :m_edoc_macros do
  ...
  def check_defs([x | _Ds]) do
    :edoc_report.report(~c"bad macro definition: ~P.", [x, 10])
    exit(:error)
  end
  ...
  defp version_macro(s, line, env) do
    date_macro(s, line, env) ++ ~c" " ++ time_macro(s, line, env)
  end
  ...
  defp link_macro(s, line, env) do
    ...
    :lists.flatten(
      :io_lib.fwrite(
        ~c"<a docgen-rel=\"~ts\" docgen-href=\"~ts\" href=\"~ts\"~ts>~ts</a>",
  ...
  defp type_macro(s, line, env) do
    ...
    :lists.flatten(:io_lib.fwrite(~c"<code>~ts</code>", [txt]))
  end
  ...
  defp expand_macro_def(m, arg, l, defs, st, as) do
    ...
    case :sets.is_element(m, seen) do
      true ->
        throw_error(l, {~c"recursive macro expansion of {@~s}.", [m]})
      ...
      false ->
        ...
        case :dict.find(m, defs) do
          ...
          :error ->
            :edoc_report.warning(l, r_state(st1, :where), ~c"undefined macro {@~s}.", [m])
            ~c"??"
        end
    end
  end
  ...
  defp throw_error(l, :unterminated_macro) do
    throw_error(l, {~c"unexpected end of macro.", []})
  end

  defp throw_error(l, :macro_name) do
    throw_error(l, {~c"missing macro name.", []})
  end

  defp throw_error(l, {:macro_name, s}) do
    throw_error(l, {~c"bad macro name: '@~s...'.", [:lists.reverse(s)]})
  end
