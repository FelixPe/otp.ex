defmodule :m_edoc_doclet do
  ...
  defp gen(sources, app, modules, ctxt) do
    ...
    {modules1, error} = sources(sources, dir, modules, env,
                                  options)
  ...
  defp sources(sources, dir, modules, env, options) do
    ...
    {ms, e} = :lists.foldl(fn src, {set, error} ->
                                source(src, dir, suffix, env, set, private,
                                         hidden, error, options)
                           end,
  ...
  defp source({m, name, path}, dir, suffix, env, set, private,
            hidden, error, options) do
    ...
    case ((try do
      ...
      {:ok, {module, doc}} ->
        ...
        case ((not is_private(doc) or private) and (not
                                                    is_hidden(doc) or hidden)) do
          true ->
  ...
  defp index_file(dir, title) do
    frame2 = {:frame, [{:src, 'modules-frame.html'}, {:name, 'modulesFrame'}, {:title, ''}],
                []}
    frame3 = {:frame, [{:src, 'overview-summary.html'}, {:name, 'overviewFrame'}, {:title, ''}],
                []}
  ...
  defp overview(dir, title, env, opts) do
    ...
    encodingAttribute = r_xmlAttribute(name: :encoding,
  ...
  defp copy_stylesheet(dir, options) do
    case (:proplists.get_value(:stylesheet, options)) do
      :undefined ->
        from = (case (:proplists.get_value(:stylesheet_file,
                                             options)) do
  ...
  defp stylesheet(options) do
    case (:proplists.get_value(:stylesheet, options)) do
      ...
      s ->
        ...
        [{:link,
            [{:rel, 'stylesheet'}, {:type, 'text/css'}, {:href, ref}, {:title, 'EDoc'}], []},
             '\n']
    end
  end
  ...
  defp application_frame(dir, apps, title, cSS) do
    body = ['\n', {:h2, ['Applications']}, '\n', {:table,
                                 [{:width, '100%'}, {:border, 0}],
                                 :lists.append(for {path, app} <- apps do
                                                 [{:tr,
                                                     [{:td, [],
                                                         [{:a,
                                                             [{:href,
