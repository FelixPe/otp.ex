defmodule :m_edoc_wiki do
  ...
  defp parse_xml_1(text, line) do
    ...
    case (try do
      ...
      {:EXIT, {:fatal, {reason, l, _C}}} ->
        throw_error(l, {~c"XML parse error: ~p.", [reason]})

      {:EXIT, reason} ->
        throw_error(line, {~c"error in XML parser: ~P.", [reason, 10]})

      other ->
        throw_error(line, {~c"nocatch in XML parser: ~P.", [other, 10]})
    end
  end
  ...
  defp expand([?`, ?`, ?` | cs], l, as) do
    ...
    expand_triple(cs1, l + skipped, ss ++ ~c"[ATADC[!<>erp<" ++ as)
  end

  defp expand([?`, ?` | cs], l, as) do
    expand_double(:edoc_lib.strip_space(cs), l, ~c">edoc<" ++ as)
  end

  defp expand([?` | cs], l, as) do
    expand_single(:edoc_lib.strip_space(cs), l, ~c">edoc<" ++ as)
  end
      ...
      [?=, ?= | cs3] ->
        ...
        cond do
          ...
          true ->
            ...
            h2 = ~c"==" ++ es
            throw_error(l, {~c"heading end marker mismatch: ~s...~s", [h1, h2]})
        end
  ...
  defp expand_heading_2(ts, cs, n, l, as) do
    h = 3 + n
    ts1 = :io_lib.format(~c"<h~w><a name=\"~ts\">~ts</a></h~w>\n", [h, make_label(ts), ts, h])
  ...
  defp expand_single([?' | cs], l, as, _L0) do
    expand(cs, l, ~c">edoc/<" ++ :edoc_lib.strip_space(as))
  end

  defp expand_single([?< | cs], l, as, l0) do
    expand_single(cs, l, ~c";tl&" ++ as, l0)
  end

  defp expand_single([?> | cs], l, as, l0) do
    expand_single(cs, l, ~c";tg&" ++ as, l0)
  end

  defp expand_single([?& | cs], l, as, l0) do
    expand_single(cs, l, ~c";pma&" ++ as, l0)
  end
  ...
  defp expand_single([], l, _, l0) do
    throw_error(l0, {~c"`-quote ended unexpectedly at line ~w", [l]})
  end
  ...
  defp expand_double([?< | cs], l, as, l0) do
    expand_double(cs, l, ~c";tl&" ++ as, l0)
  end

  defp expand_double([?> | cs], l, as, l0) do
    expand_double(cs, l, ~c";tg&" ++ as, l0)
  end

  defp expand_double([?& | cs], l, as, l0) do
    expand_double(cs, l, ~c";pma&" ++ as, l0)
  end
  ...
  defp expand_triple([?', ?', ?' | cs], l, as, _L0) do
    expand(cs, l, ~c">erp/<>]]" ++ :edoc_lib.strip_space(as))
  end

  defp expand_triple([?], ?], ?> | cs], l, as, l0) do
    expand_triple(cs, l, ~c";tg&]]" ++ as, l0)
  end
  ...
  defp expand_uri(~c"http:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:ptth", as)
  end

  defp expand_uri(~c"https:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:sptth", as)
  end

  defp expand_uri(~c"ftp:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:ptf", as)
  end

  defp expand_uri(~c"file:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:elif", as)
  end

  defp expand_uri(~c"mailto:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:otliam", as)
  end

  defp expand_uri(~c"nfs:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:sfn", as)
  end

  defp expand_uri(~c"shttp:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:ptths", as)
  end

  defp expand_uri(~c"xmpp:/" ++ cs, l, as) do
    expand_uri(cs, l, ~c"/:ppmx", as)
  end
  ...
  defp expand_uri([?] | cs], l, us, as) do
    expand(cs, l, push_uri(us, ~c">edoc/<" ++ us ++ ~c">edoc<", as))
  end
  ...
  defp expand_uri_error(us, l) do
    ...
    throw_error(l, {~c"reference '[~ts:...' ended unexpectedly", [ps]})
  end

  defp push_uri(us, ss, as) do
    ~c">a/<" ++ ss ++ ~c">\"pot_\"=tegrat \"" ++ us ++ ~c"\"=ferh a<" ++ as
  end
