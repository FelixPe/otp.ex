defmodule :m_edoc_doclet_chunks do
  ...
  defp gen(sources, _App, modules, ctxt) do
    dir = :filename.join(r_doclet_context(ctxt, :dir), ~c"chunks")
  ...
  defp sources(sources, dir, modules, env, options) do
    suffix = :proplists.get_value(:file_suffix, options, ~c".chunk")
  ...
  defp source({_M, name, path}, dir, suffix, env, okSet, errorFlag, options0) do
  ...
  defp chunk_file_name(erlName, suffix) do
    :string.join(
      [:filename.basename(erlName, ~c".erl"), suffix],
      ~c""
    )
  end
