defmodule :m_edoc_specs do
  ...
  def type(form, typeDocs) do
    ...
    {typeName, type, args, doc} = (case (data0) do
                                     {r, fs} ->
                                       :record = name
    ...
    r_tag(name: :type, line: get_line(:erlang.element(2, type)),
        origin: :code,
        data: {r_t_typedef(name: typeName, args: d2e(args),
  ...
  def spec(form) do
    ...
    r_tag(name: :spec,
        line: get_line(:erlang.element(2,
                                         :lists.nth(1, typeSpecs))),
        origin: :code,
        data: for typeSpec <- typeSpecs do
  ...
  def dummy_spec(form) do
    ...
    r_tag(name: :spec,
        line: get_line(:erlang.element(2, hd(typeSpecs))),
        origin: :code, data: s)
  end
  ...
  def add_type_data(entries, opts, file, module) do
    typeDefs0 = espec_types(entries)
    typeTable = :ets.new(:etypes, [:ordered_set])
    es1 = expand_records(entries, typeDefs0, typeTable,
                           opts, file, module)
    es = (for e <- es1 do
  ...
  defp get_spec(form) do
    {:spec,
       data0} = :erl_syntax_lib.analyze_wild_attribute(form)
    case (data0) do
      {{f, a}, d} ->
        {r_t_name(name: f), a, d}
      {{m, f, a}, d} ->
  ...
  defp find_type_docs([f | fs], cs, fun) do
    try do
      ...
      {kind, name, lastTypeLine} ->
        ...
        c2 = [c1 | for c <- :erl_syntax.get_postcomments(f),
                         get_tree_line(c) >= lastTypeLine do
                     c
                   end]
        c3 = collect_comments(fs, lastTypeLine)
        r_tag(data: doc0) = fun.(:lists.reverse(c2 ++ c3),
                               lastTypeLine)
        case (strip(doc0)) do
          '' ->
            find_type_docs(fs, cs, fun)
  ...
  defp get_name_and_last_line(f) do
    ...
    case (:edoc_specs.tag(name)) do
      :callback ->
        :ok
      :type ->
        :ok
      _ ->
        :erlang.error(:invalid_tag, [f])
    end
  ...
  defp espec_types(entries) do
    ...
    codeTs = (for (r_tag(name: :type,
                       origin: :code) = t) <- tags do
                t
              end)
    cT = :sets.from_list(for t <- codeTs do
                           type_name(t)
                         end)
    codeTs ++ (for (r_tag(name: :type,
                        origin: :comment) = t) <- tags,
  ...
  defp use_tags([r_tag(origin: :code) = t | ts], e, typeTable,
            nTs) do
  ...
  defp use_tags([t | ts], e, typeTable, nTs) do
    case (t) do
      r_tag(name: :type, origin: :comment) ->
        typeName = type_name(t)
        case (:ets.lookup(typeTable, typeName)) do
          [{^typeName, r_tag(origin: :code), :seen}] ->
            use_tags(ts, e, typeTable, nTs)
          _ ->
            use_tags(ts, e, typeTable, [t | nTs])
        end
  ...
  defp capitalize([c | cs]) when (c >= 224 and c <= 254 and
                            c != 247) do
    [c - 32 | cs]
  end
  ...
  defp d2e({:ann_type, _, [v, t0]}, prec) do
    ...
    t1 = d2e(t0, r)
    t = :erlang.setelement(2, t1,
                             [:erlang.element(3, v) | :erlang.element(2, t1)])
    maybe_paren(p, prec, t)
  end

  defp d2e({:remote_type, _,
             [{:atom, _, m}, {:atom, _, f}, ts0]},
            _Prec) do
    ts = d2e(ts0)
    typevar_anno(r_t_type(name: r_t_name(module: m, name: f), args: ts),
                   ts)
  end

  defp d2e({:type, _, :fun,
             [{:type, _, :product, as0}, ran0]},
            _Prec) do
    ts = ([ran | as] = d2e([ran0 | as0]))
    typevar_anno(r_t_fun(args: as, range: ran), ts)
  end

  defp d2e({:type, _, :fun, [a0 = {:type, _, :any}, ran0]},
            _Prec) do
  ...
  defp d2e({:type, _, :bounded_fun, [t, gs]}, _Prec) do
    [f0 | defs] = d2e([t | gs])
    f = :erlang.setelement(2, f0,
                             :lists.keydelete(:type_variables, 1,
                                                :erlang.element(2, f0)))
  ...
  defp d2e({:type, _, :map_field_assoc, [k, v]}, prec) do
    t = r_t_map_field(assoc_type: :assoc, k_type: d2e(k),
            v_type: d2e(v))
  ...
  defp d2e({:type, _, :map_field_exact, [k, v]}, prec) do
    t = r_t_map_field(assoc_type: :exact, k_type: d2e(k),
            v_type: d2e(v))
    {p, _R} = :erl_parse.type_preop_prec(:"#")
    maybe_paren(p, prec, t)
  end
  ...
  defp d2e({:type, _, :field_type, [name, type0]}, prec) do
    ...
    t = r_t_field(name: r_t_atom(val: :erlang.element(3, name)),
            type: type)
    typevar_anno(t, [type])
  end

  defp d2e({:typed_record_field, {:record_field, l, name},
             type},
            prec) do
    d2e({:type, l, :field_type, [name, type]}, prec)
  end

  defp d2e({:typed_record_field,
             {:record_field, l, name, _E}, type},
            prec) do
    d2e({:type, l, :field_type, [name, type]}, prec)
  end
  ...
  defp d2e({:var, _, typeName}, _Prec) do
    ...
    t1 = :erlang.setelement(2, t,
                              [{:type_variables, typeVar} | :erlang.element(2,
                                                                              t)])
    :erlang.setelement(2, t1,
                         [typeName | :erlang.element(2, t1)])
  end
  ...
  defp typevar_anno(type, ts) do
    ...
    case (:ordsets.to_list(vs)) do
      [] ->
        type
      _ ->
        :erlang.setelement(2, type,
                             [{:type_variables, vs} | :erlang.element(2, type)])
    end
  end
  ...
  defp get_typevars(ts) do
    for t <- ts, t !== :undefined,
          {:type_variables, vs} <- :erlang.element(2, t) do
      vs
    end
  end
  ...
  Record.defrecord(:r_parms, :parms, tab: :undefined,
                                 warn: :undefined, file: :undefined,
                                 line: :undefined)
  defp expand_records(entries, typeDefs, dT, opts, file, module) do
    typeList = (for t <- typeDefs do
    ...
    exportedTypes = (for {:export_type,
                            ts} <- r_module(module, :attributes),
                           is_list(ts), {n, i} <- ts,
                           :ets.member(dT, name = {r_t_name(name: n), i}) do
                       name
                     end)
    _ = :lists.foreach(fn {n, a} ->
  ...
  defp entries([e0 | es], p, opts) do
    e = (case (:edoc_data.hidden_filter([e0], opts)) do
           [] ->
             e0
           [_] ->
  ...
  defp specs([r_tag(line: l, name: :spec, origin: :code,
               data: specs) = tag0 |
               tags],
            p0) do
    p = r_parms(p0, line: l)
    data = (for spec <- specs do
              (
  ...
  defp type_name(r_tag(name: :type,
              data: {r_t_typedef(name: name, args: as), _})) do
    {name, length(as)}
  end
  ...
  defp analyze_type_attribute(form) do
    ...
    case (tag(name)) do
      ...
      _ when name === :record ->
        {:attribute, _, :record,
           {n, fields}} = :erl_syntax.revert(form)
        {:record, {n, fields}}
    end
  end

  def is_tag(:callback) do
    true
  end
