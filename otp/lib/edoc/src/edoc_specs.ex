defmodule :m_edoc_specs do
    ...
    {typeName, type, args, doc} =
      case data0 do
        {r, fs} ->
          ...
          {{:record, r}, {:type, anno, :record, [{:atom, anno, r} | fs]}, [], ~c""}
  ...
  def dummy_spec(form) do
    ...
    as = :lists.join(~c",", :lists.duplicate(arity, ~c"_X"))
    s = :lists.flatten(:io_lib.format(~c"~p(~s) -> true\n", [name, as]))
  ...
  defp find_type_docs([f | fs], cs, fun) do
    try do
      ...
      {kind, name, lastTypeLine} ->
        c0 = :erl_syntax.comment([~c"% @type f(). "])
  ...
  defp d2e({:type, _, :constraint, [sub, ts0]}, _Prec) do
    case {sub, ts0} do
      ...
      _ ->
        throw_error(get_line(:erlang.element(2, sub)), ~c"cannot handle guard", [])
    end
  end
    ...
    case {:ets.lookup(dT, typeName), n} do
      ...
      {[], {:record, r}} ->
        ...
        for _ <- [:EFE_DUMMY_GEN], w do
          :edoc_report.warning(l, file, ~c"reference to untyped record ~w", [r])
        end
  ...
  defp throw_error(line, s, a) do
    :edoc_report.error(line, ~c"", :io_lib.format(s, a))
    throw(:error)
  end
end
