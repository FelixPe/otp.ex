defmodule :m_edoc_data do
  ...
  defp types(tags, env, opts) do
    for r_tag(name: :type, line: line, data: {def__, doc}) <- tags do
      {:typedecl, [{:label, :edoc_types.to_label(def__)}, {:line, line}],
       [:edoc_types.to_xml(def__, env, opts)] ++ description(doc)}
    end
  end
  ...
  defp see(ref, xML, env) do
    ...
    attrs =
      [{:"docgen-rel", docgenRel}, {:"docgen-href", docgenURI}] ++
        href(
          ref,
          env
        )
