defmodule :m_edoc_data do
  ...
  def module(module, entries, env, opts) do
    ...
    out = {:module,
             [{:name, name}, {:root, r_env(env, :root)}, {:encoding,
                                                        r_module(module, :encoding)}] ++ (case (is_private(headerTags)) do
                                                                                     true ->
                                                                                       [{:private,
                                                                                           'yes'}]
                                                                                     false ->
                                                                                       []
                                                                                   end) ++ (case (is_hidden(headerTags)) do
                                                                                              true ->
                                                                                                [{:hidden,
                                                                                                    'yes'}]
                                                                                              false ->
             ...
             module_args(r_module(module, :parameters)) ++ behaviours(r_module(module, :attributes),
                                                                 env) ++ get_doc(headerTags) ++ authors(headerTags) ++ get_version(headerTags) ++ get_since(headerTags) ++ get_copyright(headerTags) ++ get_deprecated(headerTags) ++ sees(headerTags,
                                                                                                                                                                                                                                             env) ++ references(headerTags) ++ todos(headerTags,
                                                                                                                                                                                                                                                                                       opts) ++ [{:typedecls,
                                                                                                                                                                                                                                                                                                    types(allTags,
                                                                                                                                                                                                                                                                                                            env,
                                                                                                                                                                                                                                                                                                            opts)},
  ...
  def get_all_tags(es) do
    :lists.flatmap(fn r_entry(data: ts) ->
                        ts
                   end,
                     es)
  end
  ...
  defp description(desc) do
    ...
    [{:description,
        [{:briefDescription, shortDesc}, {:fullDescription,
                                            desc}]}]
  end
  ...
  defp types(tags, env, opts) do
    for r_tag(name: :type, line: line,
            data: {def__, doc}) <- tags do
      {:typedecl,
         [{:label, :edoc_types.to_label(def__)}, {:line, line}],
         [:edoc_types.to_xml(def__, env,
                               opts)] ++ description(doc)}
    end
  end

  defp functions(es, env, opts) do
    for r_entry(name: {_, _} = n, args: as, export: export,
            data: ts) <- es do
  ...
  defp function_filter(r_entry(name: {_, _}, export: export, data: ts),
            private, hidden) do
    (export and not is_private(ts) or private) and (not
                                                    is_hidden(ts) or hidden)
  end
  ...
  defp callbacks(es, module, env, opts) do
    case (:lists.any(fn r_entry(name: {:behaviour_info, 1}) ->
                          true
                        _ ->
                          false
                     end,
                       es) or :lists.keymember(:callback, 1,
                                                 r_module(module, :attributes))) do
      true ->
        m = r_module(module, :name)
        fs = get_callback_functions(m, :callbacks)
        os1 = get_callback_functions(m, :optional_callbacks)
        fs1 = (for fA <- fs, not :lists.member(fA, os1) do
                 fA
               end)
        req = (cond do
                 fs1 === [] ->
  ...
  defp get_callback_functions(m, callbacks) do
    try do
      for ({f, a} = fA) <- m.behaviour_info(callbacks),
            is_atom(f), is_integer(a), a >= 0 do
        fA
      end
    catch
      _, _ ->
  ...
  defp callback({n, a}, _Env, _Opts) do
    {:callback,
       [{:name, :erlang.atom_to_list(n)}, {:arity,
       ...
       [{:name, :erlang.atom_to_list(n)}, {:arity,
                                             :erlang.integer_to_list(a)},
                                              {:exported, yes_or_no(export)},
                                                  {:private,
                                                     yes_or_no(is_private(ts))},
                                                      {:hidden,
                                                         yes_or_no(is_hidden(ts))},
                                                          {:label,
                                                             :edoc_refs.to_label(:edoc_refs.function(n,
                                                                                                       a))}],
       :lists.append(for clause <- :lists.seq(1,
                                                length(as0)) do
                       get_args(:lists.nth(clause, as0), ts, clause, env, opts)
                     end) ++ get_throws(ts, env, opts) ++ get_equiv(ts,
                                                                      env) ++ get_doc(ts) ++ get_since(ts) ++ get_deprecated(ts,
                                                                                                                               n,
                                                                                                                               a,
                                                                                                                               env) ++ sees(ts,
                                                                                                                                              env) ++ todos(ts,
  ...
  defp get_args(as, ts, clause, env, opts) do
    {args, ret, spec} = signature(ts, as, clause, env, opts)
    [{:args,
        for {a, d} <- args do
          {:arg,
             [{:argName,
                 [:erlang.atom_to_list(a)]}] ++ description(d)}
        end}] ++ spec ++ (case (ret) do
                            [] ->
                              []
                            _ ->
  ...
  defp yes_or_no(true) do
    'yes'
  end

  defp yes_or_no(false) do
    'no'
  end

  defp get_throws(ts, env, opts) do
  ...
  defp get_equiv(ts, env) do
    case (get_tags(:equiv, ts)) do
      [equiv] ->
        ...
        see = (case (get_expr_ref(r_tag(equiv, :data))) do
                 :none ->
                   []
                 ref ->
        ...
        [{:equiv,
            [{:expr, [:erl_prettypr.format(expr)]} | see]}]
      [] ->
        []
    end
  end
  ...
  defp get_expr_ref(expr) do
    case ((try do
      ...
      {:ok, {m, {f, a}}} when (is_atom(m) and is_atom(f) and
                                 is_integer(a))
                              ->
  ...
  defp behaviours(as, env) do
    for {:behaviour, b} <- as, is_atom(b) do
      {:behaviour, href(:edoc_refs.module(b), env),
         [:erlang.atom_to_list(b)]}
    end
  end
  ...
  defp see(ref, xML, env) do
    ...
    attrs = [{:"docgen-rel", docgenRel}, {:"docgen-href", docgenURI}] ++ href(ref,
                                                         env)
    {:see, attrs, xML}
  end
  ...
  defp href(ref, env) do
    [{:href,
        :edoc_refs.get_uri(ref,
                             env)}] ++ (case (:edoc_refs.is_top(ref, env)) do
                                          true ->
                                            [{:target, '_top'}]
                                          false ->
                                            []
  ...
  defp signature(ts, as, clause, env, opts) do
    case (get_tags(:spec, ts)) do
      [t] ->
        ...
        spec1 = :edoc_types.set_arg_names(spec,
                                            for {a, _} <- as1 do
                                              a
                                            end)
  ...
  def get_entry(name, [r_entry(name: name) = e | _Es]) do
    e
  end

  def get_entry(name, [_ | es]) do
    get_entry(name, es)
  end

  def get_tags(tag, [r_tag(name: tag) = t | ts]) do
    [t | get_tags(tag, ts)]
  end

  def get_tags(tag, [_ | ts]) do
    get_tags(tag, ts)
  end
  ...
  def type(t, env, opts) do
    :xmerl_lib.expand_element({:type,
                                 [:edoc_types.to_xml(t, env, opts)]})
  end
  ...
  defp overview_1(title, tags, env, opts) do
    {:overview, [{:root, r_env(env, :root)}],
       [{:title,
           [get_title(tags,
                        title)]}] ++ get_doc(tags) ++ authors(tags) ++ get_copyright(tags) ++ get_version(tags) ++ get_since(tags) ++ sees(tags,
                                                                                                                                             env) ++ references(tags) ++ todos(tags,
                                                                                                                                                                                 opts)}
  end
