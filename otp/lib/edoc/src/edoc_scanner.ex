defmodule :m_edoc_scanner do
  ...
  defp scan1([c | cs], toks, pos) when (c >= 0 and
                                       c <= ?\s) do
    scan1(cs, toks, pos)
  end

  defp scan1([c | cs], toks, pos) when (c >= ?a and
                                       c <= ?z) do
    scan_atom(c, cs, toks, pos)
  end

  defp scan1([c | cs], toks, pos) when (c >= 223 and
                                       c <= 255 and c != 247) do
    scan_atom(c, cs, toks, pos)
  end
  ...
  defp scan1([c | cs], toks, pos) when (c >= 192 and
                                       c <= 222 and c != 215) do
    scan_variable(c, cs, toks, pos)
  end
  ...
  defp scan_escape([o1, o2, o3 | cs], pos) when (o1 >= ?0 and
  ...
  defp scan_escape([o1, o2 | cs], pos) when (o1 >= ?0 and
  ...
  defp scan_hex([?} | cs], pos, hCs) do
    case ((try do
      ...
      val
          when val >= 0 and val < 55296 or val > 57343 and val < 65534 or val > 65535 and val <= 1114111
               ->
        {val, cs, pos}
      _ ->
        {:error, :undefined_escape_sequence}
  ...
  defp scan_integer([c | cs], stack, pos) when (c >= ?0 and
                                        c <= ?9) do
