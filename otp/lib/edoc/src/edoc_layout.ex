defmodule :m_edoc_layout do
  ...
  Record.defrecord(:r_opts, :opts, root: :undefined,
  ...
  defp init_opts(element, options) do
    ...
    r = r_opts(root: get_attrval(:root, element),
  ...
  defp layout_module(r_xmlElement(name: :module, content: es) = e, opts) do
    ...
    title = (case (get_elem(:args, es)) do
               [] ->
                 ['Module ', name]
               _ ->
    ...
    functions = (for ^e <- get_content(:functions, es) do
                   {function_name(e, opts), e}
                 end)
    types = (for ^e <- get_content(:typedecls, es) do
               {type_name(e, opts), e}
             end)
    sortedFs = (cond do
    ...
    body = navigation('top') ++ ['\n', :hr, '\n', '\n', {:h1, title},
                                               '\n'] ++ doc_index(fullDesc,
                                                                 functions,
                                                                 types) ++ shortDesc ++ ['\n'] ++ copyright(es) ++ deprecated(es,
                                                                                                                             'module') ++ ['\n'] ++ version(es) ++ since(es) ++ behaviours(es,
                                                                                                                                                                                   name,
                                                                                                                                                                                   opts) ++ authors(es) ++ references(es) ++ sees(es) ++ todos(es) ++ (cond do
                                                                                                                                                                                                                                                         (fullDesc == []) ->
                                                                                                                                                                                                                                                           []
                                                                                                                                                                                                                                                         true ->
                                                                                                                                                                                                                                                           ['\n',
                                                                                                                                                                                                                                                                {:h2,
                                                                                                                                                                                                                                                                   [{:a,
                                                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                                                       ['Description']}]} |
                                                                                                                                                                                                                                                                    fullDesc]
                                                                                                                                                                                                                                                       end) ++ types(:lists.sort(types),
                                                                                                                                                                                                                                                                       opts) ++ function_index(sortedFs,
                                                                                                                                                                                                                                                                                                 r_opts(opts, :index_columns)) ++ functions(sortedFs,
                                                                                                                                                                                                                                                                                                                                         opts) ++ [:hr,
  ...
  defp module_params(es) do
    as = (for r_xmlElement(content: es1) <- es do
            {get_text(:argName, es1),
               get_content(:fullDescription,
    ...
    case (as) do
      [] ->
        []
      [first | rest] ->
        [:erlang.element(1, first) | for {a, _D} <- rest do
                                       {[', ', a]}
                                     end]
    end
  end
  ...
  defp stylesheet(opts) do
    case (r_opts(opts, :stylesheet)) do
      ...
      cSS ->
        [{:link,
            [{:rel, 'stylesheet'}, {:type, 'text/css'}, {:href, cSS}, {:title, 'EDoc'}], []},
             '\n']
    end
  end
  ...
  defp function_index_rows(fs, cols, title) do
    ...
    (cond do
       ...
       true ->
         [{:tr,
             [{:th, [{:colspan, cols * 2}, {:align, :left}],
                 [title]}]},
              '\n']
     end) ++ :lists.flatmap(&index_row/1,
  ...
  defp index_desc(es) do
    ...
    (case (get_content(:deprecated, es)) do
       ...
       _ ->
         ['(', {:em, ['Deprecated']}, '.) ']
     end) ++ (case (get_content(:briefDescription, desc)) do
                [] ->
                  equiv(es)
                shortDesc ->
                  shortDesc
           ...
           label_anchor(function_header(name, e, ' *'), e)},
            '\n'] ++ [{:div, [{:class, 'spec'}],
                      ((case (for t <- get_contents(:typespec, es) do
                        ...
                        end) ++ (case (for a <- get_contents(:args, es) do
                                         params(a)
                                       end) do
                                   [] ->
                                     []
                                   as ->
                                     :lists.append(for ps <- as do
                                                     [{:p, ps}, '\n']
                                                   end)
                                 end) ++ (case (for ret <- get_contents(:returns,
                                                                          es) do
                                                  returns(ret)
                                                end) do
                                            [] ->
                                              []
                                            rets ->
                                              :lists.append(for rs <- rets do
                                                              [{:p, rs}, '\n']
                                                            end)
                                          end))}] ++ throws(es,
                                                              opts) ++ equiv_p(es) ++ deprecated(es,
                                                                                                   'function') ++ fulldesc(es) ++ since(es) ++ sees(es) ++ todos(es)
  end

  defp function_name(e, opts) do
    atom(get_attrval(:name, e),
           opts) ++ '/' ++ get_attrval(:arity, e)
  end
  ...
  defp params(es) do
    ...
    as1 = (for a <- as, :erlang.element(2, a) != [] do
             a
           end)
    cond do
      as1 == [] ->
  ...
  defp throws(es, opts) do
    case (get_content(:throws, es)) do
      ...
      es1 ->
        [{:p,
            ['throws ', {:code,
                   t_utype(get_elem(:type, es1),
                             opts)}] ++ local_defs(get_elem(:localdef, es1),
                                                     opts)},
             '\n']
    end
  end
  ...
  defp typespec(es, opts) do
    ...
    format_spec(name, type, defs, opts) ++ local_defs(defs,
                                                        opts)
  end
  ...
  defp local_defs(es0, last, opts) do
    ...
    ['\n', {:ul, [{:class, 'definitions'}],
           :lists.reverse(:lists.append(for e1 <- es do
                                          localdef(e1, [], opts)
                                        end),
  ...
  defp localdef(e = r_xmlElement(content: es), last, opts) do
    ...
    [{:li,
        format_type(name, n0, get_elem(:type, es), last, opts)}]
  end

  defp format_spec(name, type, defs,
            r_opts(pretty_printer: :erl_pp) = opts) do
    try do
      ...
      _, _ ->
        format_spec(name, type, defs,
                      r_opts(opts, pretty_printer: :""))
    end
  end
  ...
  defp format_type(prefix, name, type, last,
            r_opts(pretty_printer: :erl_pp) = opts) do
    try do
      l = t_utype(type, opts)
      ...
      _, _ ->
        format_type(prefix, name, type, last,
                      r_opts(opts, pretty_printer: :""))
    end
  end
  ...
  defp pp_type(prefix, type, opts) do
    atom = :erlang.list_to_atom(:lists.duplicate(:string.length(prefix),
                                                   ?a))
  ...
  defp href(e) do
    case (get_attrval(:href, e)) do
      '' ->
        []
      uRI ->
        t = (case (get_attrval(:target, e)) do
               '' ->
                 []
               s ->
                 [{:target, s}]
         ...
         [{:p, ([{:b, ['Behaviours:']}, ' '] ++ seq(&behaviour/1, es1, ['.']))},
              '\n']
                ...
                true ->
                  ...
                  [{:p, [{:b, ['This module defines the ', {:code, [name]}, ' behaviour.']}] ++ req ++ opt}, '\n']
              end)
  end
  ...
  defp todos(es) do
    case (get_elem(:todo, es)) do
      [] ->
        []
      es1 ->
        todos = (for r_xmlElement(content: c) <- es1 do
                   {:li, [{:font, [{:color, :red}], c}]}
                 end)
        [{:p,
            [{:b, [{:font, [{:color, :red}], ['To do']}]}, {:ul, todos}]},
             '\n']
    end
  end
  ...
  defp t_field(r_xmlElement(content: es), opts) do
    t_type(get_elem(:atom, es),
             opts) ++ [' = '] ++ t_utype(get_elem(:type, es), opts)
  end
  ...
  defp xhtml(title, cSS, body, encoding) do
    ...
    [{:html,
        ['\n', {:head,
               ['\n', {:meta, [{:"http-equiv", 'Content-Type'}, {:content, ('text/html; charset=' ++ encString)}],
                      []},
                       '\n', {:title, title}, '\n'] ++ cSS},
  ...
  defp type(e, ds, opts) do
    :xmerl.export_simple_content(t_utype_elem(e,
                                                opts) ++ local_defs(ds, opts),
                                   :xmerl_html)
  end
  ...
  def overview(e = r_xmlElement(name: :overview, content: es), options) do
    ...
    fullDesc = get_content(:fullDescription, desc)
    body = navigation('top') ++ ['\n', {:h1, [title]},
                                    '\n'] ++ copyright(es) ++ version(es) ++ since(es) ++ authors(es) ++ references(es) ++ sees(es) ++ todos(es) ++ fullDesc ++ ['\n',
