defmodule :m_edoc_run do
  ...
  def application(args) do
    f = fn ->
      case parse_args(args) do
        ...
        _ ->
          invalid_args(~c"edoc_run:application/1", args)
      end
    end
  ...
  def files(args) do
    f = fn ->
      case parse_args(args) do
        ...
        _ ->
          invalid_args(~c"edoc_run:files/1", args)
      end
    end
  ...
  def toc(args) do
    f = fn ->
      case parse_args(args) do
        ...
        _ ->
          invalid_args(~c"edoc_run:toc/1", args)
      end
    end
  ...
  def file(args) do
    f = fn ->
      case parse_args(args) do
        ...
        _ ->
          invalid_args(~c"edoc_run:file/1", args)
      end
    end
  ...
  defp invalid_args(where, args) do
    report(~c"invalid arguments to ~ts: ~tw.", [where, args])
    shutdown_error()
  end
  ...
  defp run(f) do
    ...
    case (try do
      ...
      {:EXIT, e} ->
        report(~c"edoc terminated abnormally: ~tP.", [e, 10])
        shutdown_error()

      thrown ->
        report(~c"internal error: throw without catch in edoc: ~tP.", [thrown, 15])
        shutdown_error()
    end
  end
  ...
  defp parse_arg(a) do
    case (try do
      ...
      {:error, _, d} ->
        report(~c"error parsing argument '~ts'", [a])
        :erlang.error(d)
        exit(:error)
    end
  end
