defmodule :m_edoc_extract do
  ...
  def source(forms, comments, file, env, opts) do
    tree = :erl_recomment.quick_recomment_forms(forms,
                                                  comments)
  ...
  defp source1(tree, file0, env, opts, typeDocs) do
    ...
    entries1 = get_tags([header, footer | entries], env2,
                          file, typeDocs)
    entries2 = :edoc_specs.add_type_data(entries1, opts,
                                           file, module)
  ...
  def header(tree, file0, env, _Opts) do
    ...
    [entry] = get_tags([r_entry(footer, name: :header)], env,
                         file)
    r_entry(entry, :data)
  end
  ...
  defp text(text, context, env, opts, where) do
    env1 = add_macro_defs(file_macros(context, env), opts,
                            env)
    ...
    case (:edoc_tags.check_tags(ts1, allow, single,
                                  where)) do
      true ->
        exit(:error)
      false ->
  ...
  defp get_module_info(forms, file) do
    ...
    {name, vars} = (case (:lists.keyfind(:module, 1, l)) do
    ...
    functions = :ordsets.from_list(get_list_keyval(:functions,
                                                     l))
    exports = :ordsets.from_list(get_list_keyval(:exports,
                                                   l))
    attributes = :ordsets.from_list(get_list_keyval(:attributes,
                                                      l))
    records = get_list_keyval(:records, l)
    encoding = :edoc_lib.read_encoding(file, [])
    r_module(name: name, parameters: vars, functions: functions,
  ...
  defp collect(fs, mod) do
    acc = %{comments: [], callbacks: [], specs: [],
              types: [], records: [], functions: [],
              header: :undefined}
    collect(fs, acc, mod)
  end

  defp collect([f | fs], acc, mod) do
    %{comments: cs, types: ts, records: rs,
        header: header} = acc
    case (:erl_syntax_lib.analyze_form(f)) do
      :comment ->
      ...
      {:function, name} ->
        ...
        function = r_entry(name: name, args: args, line: l,
                       export: export,
      ...
      {:attribute, {:module, _}} when header === :undefined ->
        ...
        newAcc = %{acc
                   |
                   comments: [], specs: [], types: [], records: []}
        newHeader = r_entry(name: :module, line: l,
                        data: {comment_text(cs), [], [], ts, rs})
  ...
  defp comment_text([c | cs], ss) do
    ...
    comment_text(cs,
                   [r_comment(line: l,
                        text: for s <- :erl_syntax.comment_text(c) do
  ...
  defp error_redundant_specs(mod, specList, specs) do
    [{redundantMFA, [form]} |
         _] = :lists.sort(specList) -- :lists.sort(:maps.to_list(specs))
    {_, line, _, _} = :erl_syntax.revert(form)
    {_, f, a} = redundantMFA
    :edoc_report.error(line, {r_module(mod, :file), {f, a}}, 'Redundant -spec attribute found. Try setting {preprocess, true}.')
  ...
  defp parameters(clauses) do
    select_names(for ps <- patterns(clauses) do
                   find_names(ps)
                 end)
  end

  defp patterns(cs) do
    :edoc_lib.transpose(for c <- cs do
  ...
  defp tidy_name_1([c | _] = cs) when (c >= 192 and c <= 222 and
                                c !== 215) do
    cs
  end
  ...
  Record.defrecord(:r_tags, :tags, names: :undefined,
                                single: :undefined, module: :undefined,
                                function: :undefined, footer: :undefined)
  defp get_tags(es, env, file) do
    get_tags(es, env, file, :dict.new())
  end

  defp get_tags(es, env, file, typeDocs) do
    tags = r_tags(names: :sets.from_list(:edoc_tags.tag_names()),
               single: :sets.from_list(:edoc_tags.tags(:single)),
  ...
  defp get_tags([r_entry(name: name,
               data: {cs, cbs, specs, types, records}) = e |
               es],
            tags, env, how, file, typeDocs) do
    where = {file, name}
    ts0 = scan_tags(cs)
    ...
    eTypes = (for type <- types ++ records do
                :edoc_specs.type(type, typeDocs)
              end)
    callbacks = get_callbacks(name, cbs, typeDocs)
    [r_entry(e, data: ts ++ eTypes ++ callbacks) | get_tags(es,
                                                        tags, env, how, file,
                                                        typeDocs)]
  end
  ...
  defp callback(f, typeDocs) do
    {:attribute, _, :callback,
       {nA, _}} = (attr = :erl_syntax.revert(f))
    doc = (case (:dict.find({:callback, nA}, typeDocs)) do
             :error ->
               :none
             {:ok, d} ->
               d
           end)
    r_tag(name: :callback, line: :erl_syntax.get_pos(f),
        origin: :code, data: {nA, doc}, form: attr)
  end
  ...
  defp module_macros(env) do
    [{:module,
        :erlang.atom_to_list(r_env(env, :module))}] ++ :edoc_macros.std_macros(env)
  end
  ...
  defp find_type_docs(forms0, comments, env, file) do
    ...
    forms = preprocess_forms(tree)
    env1 = add_macro_defs(:edoc_macros.std_macros(env), [],
                            env)
    f = fn c, line ->
