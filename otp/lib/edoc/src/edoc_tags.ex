defmodule :m_edoc_tags do
         ...
         {:copyright, :text, [:module, :overview, :single]},
             {:deprecated, :xml, [:module, :function, :single]},
                 {:doc, :xml, [:module, :function, :overview, :single]},
                     {:docfile, &parse_file/4, all}, {:end, :text, all},
                                                         {:equiv, &parse_expr/4,
                                                            [:function,
                                                                 :single]},
                                                             {:headerfile,
  ...
  defp scan_tag([c | cs], ss, l, as, ts) when (c >= ?a and
                                           c <= ?z) do
  ...
  defp scan_tag([c | cs], ss, l, as, ts) when (c >= 192 and
                                           c <= 255 and c !== 215 and
                                           c !== 247) do
    scan_tag_1(cs, ss, l, as, [c | ts])
  end
  ...
  defp scan_tag_1([c | cs], ss, l, as, ts) when (c >= ?a and
                                           c <= ?z) do
    scan_tag_1(cs, ss, l, as, [c | ts])
  end
  ...
  defp scan_tag_1([c | cs], ss, l, as, ts) when (c >= ?0 and
                                           c <= ?9) do
    scan_tag_1(cs, ss, l, as, [c | ts])
  end

  defp scan_tag_1([c | cs], ss, l, as, ts) when (c >= 192 and
                                           c <= 255 and c !== 215 and
                                           c !== 247) do
    scan_tag_1(cs, ss, l, as, [c | ts])
  end
  ...
  defp make_tag(cs, l, ss) do
    r_tag(name: aliases(:erlang.list_to_atom(:lists.reverse(cs))),
        line: l, data: append_lines(:lists.reverse(ss)))
  end
  ...
  defp filter_tags([r_tag(name: n, line: l) = t | ts], tags, where,
            ts1) do
      ...
      false ->
        case (where) do
          :no ->
            :ok
          _ ->
            :edoc_report.warning(l, where, 'tag @~s not recognized.', [n])
        end
  ...
  defp check_tags([r_tag(name: t, line: l) | ts], allow, single,
            where, error, seen) do
  ...
  defp parse_tags([r_tag(name: name) = t | ts], how, env, where,
            ts1) do
  ...
  defp parse_spec(data, line, _Env, {_, {f, a}} = where) do
    :edoc_report.warning(line, where, 'EDoc @spec tags are deprecated. Please use -spec attributes instead.', [])
  ...
  defp parse_typedef(data, line, _Env, where) do
    :edoc_report.warning(line, where, 'EDoc @type tags are deprecated. Please use -type attributes instead.', [])
    ...
    case (:edoc_types.is_predefined(t, nAs)) do
      true ->
        case (:edoc_types.is_new_predefined(t, nAs)) do
          ...
          true ->
            :edoc_report.warning(line, where, 'redefining built-in type \'~w\'.', [t])
            def__
        end
      false ->
        def__
  ...
  def check_types(entries, opts, file) do
    ...
    typeTags = (for (r_tag(data: {r_t_typedef(), _}) = tag) <- tags do
                  tag
                end)
    entries2 = :edoc_data.hidden_filter(entries, opts)
    tags2 = :edoc_data.get_all_tags(entries2)
    specTags = (for (r_tag(data: r_t_spec()) = tag) <- tags2 do
                  tag
                end)
    dT = :ets.new(:types, [:bag])
    _ = (for r_tag(line: line,
                 data: {r_t_typedef(name: name, args: as), _}) <- typeTags do
  ...
  defp check_type(r_tag(line: l, data: data), p0, ls, ts) do
    ...
    case (data) do
      ...
      r_t_spec(type: type, defs: defs) ->
        localTypes = (for r_t_def(name: r_t_type(name: n,
                                      args: args)) <- defs do
                        {n, length(args)}
                      end)
  ...
  defp check_used_type(r_t_name(name: n, module: mod) = name, args, p,
            localTypes) do
    nArgs = length(args)
    typeName = {name, nArgs}
    dT = r_parms(p, :tab)
    case (mod !== [] or :lists.member(typeName,
                                        :ets.lookup(dT,
                                                      name)) or :edoc_types.is_predefined(n,
                                                                                            nArgs) or :lists.member(typeName,
                                                                                                                      localTypes)) do
      true ->
        :ok
      false ->
        r_parms(warn: w, line: l, file: file) = p
        _ = (for _ <- [:EFE_DUMMY_GEN], w do
  ...
  defp type_warning(line, file, s, n, nArgs) do
    aS = (for _ <- [:EFE_DUMMY_GEN], nArgs > 0 do
