defmodule :m_edoc_tags do
  ...
  defp scan_lines(~c"TODO:" ++ _ = cs, ss, l, as) do
    scan_tag(cs, ss, l, as, [])
  end
  ...
  defp scan_tag_lines(~c"TODO:" ++ _, s, ss, {ts, l1}, ss1, l, as) do
    scan_lines(s, ss, l, [make_tag(ts, l1, ss1) | as])
  end
    ...
    case :sets.is_element(n, tags) do
      ...
      false ->
        case where do
          ...
          _ ->
            :edoc_report.warning(l, where, ~c"tag @~s not recognized.", [n])
        end
    ...
    case :sets.is_element(t, seen) do
      true ->
        case :sets.is_element(t, single) do
          ...
          true ->
            :edoc_report.report(l, where, ~c"multiple @~s tag.", [t])
      ...
      false ->
        ...
        case :sets.is_element(t, allow) do
          ...
          false ->
            :edoc_report.report(l, where, ~c"tag @~s not allowed here.", [t])
  ...
  defp parse_spec(data, line, _Env, {_, {f, a}} = where) do
    :edoc_report.warning(
      line,
      where,
      ~c"EDoc @spec tags are deprecated. Please use -spec attributes instead.",
      []
    )
    ...
    cond do
      length(as) != a ->
        throw_error(line, ~c"@spec arity does not match.")
      ...
      true ->
        case n do
          ...
          _ ->
            throw_error(line, ~c"@spec name does not match.")
        end
    end
  end
  ...
  defp parse_contact(data, line, _Env, _Where) do
    case :edoc_lib.parse_contact(data, line) do
      {~c"", ~c"", _URI} ->
        throw_error(line, ~c"must specify name or e-mail.")
  ...
  defp parse_typedef(data, line, _Env, where) do
    :edoc_report.warning(
      line,
      where,
      ~c"EDoc @type tags are deprecated. Please use -type attributes instead.",
      []
    )
    ...
    case :edoc_types.is_predefined(t, nAs) do
      true ->
        case :edoc_types.is_new_predefined(t, nAs) do
          false ->
            throw_error(line, {~c"redefining built-in type '~w'.", [t]})

          true ->
            :edoc_report.warning(line, where, ~c"redefining built-in type '~w'.", [t])
            def__
        end
  ...
  defp throw_error(l, {:read_file, file, r}) do
    throw_error(l, {~c"error reading file '~ts': ~w", [:edoc_lib.filename(file), r]})
  end

  defp throw_error(l, {:file_not_found, f}) do
    throw_error(l, {~c"file not found: ~ts", [f]})
  end

  defp throw_error(l, :file_not_string) do
    throw_error(l, ~c"expected file name as a string")
  end
  ...
  defp add_type(dT, name, args, file, line) do
    ...
    case :lists.member(typeName, :ets.lookup(dT, name)) do
      true ->
        ...
        type_warning(line, file, ~c"duplicated type", n, nArgs)
      ...
      false ->
        ...
        _ =
          for _ <- [:EFE_DUMMY_GEN], w do
            type_warning(l, file, ~c"missing type", n, nArgs)
          end
  ...
  defp type_warning(line, file, s, n, nArgs) do
    aS =
      for _ <- [:EFE_DUMMY_GEN], nArgs > 0 do
        ~c"/" ++ :erlang.integer_to_list(nArgs)
      end

    :edoc_report.warning(line, file, s ++ ~c" ~w~s", [n, aS])
  end
end
