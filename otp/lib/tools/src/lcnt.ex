defmodule :m_lcnt do
  ...
  Record.defrecord(:r_stats, :stats, file: :undefined,
                                 line: :undefined, tries: :undefined,
                                 colls: :undefined, time: :undefined,
                                 nt: :undefined, hist: :undefined)
  Record.defrecord(:r_lock, :lock, name: :undefined,
                                id: :undefined, type: :undefined, stats: [])
  Record.defrecord(:r_print, :print, name: :undefined,
                                 id: :undefined, type: :undefined,
                                 entry: :undefined, tries: :undefined,
                                 colls: :undefined, cr: :undefined,
                                 time: :undefined, dtr: :undefined,
                                 hist: :undefined)
  def start() do
  ...
  def rt_mask(node, categories) when (is_atom(node) and
  ...
  def apply(m, f, as) when (is_atom(m) and is_atom(f) and
                           is_list(as)) do
    apply(fn () ->
               :erlang.apply(m, f, as)
          end)
  ...
  def all_conflicts(sort) do
    conflicts([{:max_locks, :none}, {:thresholds, []},
                                        {:combine, false}, {:sort, sort},
                                                               {:reverse,
                                                                  true}])
  end
  ...
  def pid(node, id, serial) when is_atom(node) do
    ...
    l = length(string)
    :erlang.binary_to_term(:erlang.list_to_binary([header,
                                                       bytes16(l), string,
                                                                       bytes32(id),
                                                                           bytes32(serial),
                                                                               0]))
  ...
  def handle_call({:conflicts, inOpts}, _From,
           r_state(locks: locks) = state)
      when is_list(inOpts) do
    default = [{:sort, :time}, {:reverse, false}, {:print,
                                                     [:name, :id, :tries,
                                                                      :colls,
                                                                          :ratio,
                                                                              :time,
    ...
    flocks = filter_locks_type(locks,
                                 :proplists.get_value(:type, opts))
    combos = combine_classes(flocks,
    ...
    print_lock_information(filtered,
  ...
  def handle_call({:locations, inOpts}, _From,
           r_state(locks: locks) = state)
      when is_list(inOpts) do
    default = [{:sort, :time}, {:reverse, false}, {:print,
                                                     [:name, :entry, :tries,
                                                              ...
                                                              {:thresholds,
                                                                 [{:tries, 0},
                                                                  ...
                                                                  {:locations,
                                                                     true}]
    opts = options(inOpts, default)
    printables = filter_print(for {stats,
                                     names} <- combine_locations(locks) do
                                r_print(name: string_names(names),
                                    entry: term2string('~tp:~p',
                                                         [r_stats(stats, :file),
                                                              r_stats(stats, :line)]),
                                    colls: r_stats(stats, :colls),
                                    tries: r_stats(stats, :tries),
                                    cr: percent(r_stats(stats, :colls),
                                                  r_stats(stats, :tries)),
                                    time: r_stats(stats, :time),
                                    dtr: percent(r_stats(stats, :time),
                                                   r_state(state, :duration)))
                              end,
                                opts)
    print_lock_information(printables,
                             :proplists.get_value(:print, opts))
    {:reply, :ok, state}
  end

  def handle_call({:inspect, lockname, inOpts}, _From,
    ...
    iDs = (case ({:proplists.get_value(:full_id, opts),
                    :proplists.get_value(:combine, opts)}) do
             {true, true} ->
               locks_ids(filtered)
             _ ->
               []
           end)
    combos = combine_classes(filtered,
                               :proplists.get_value(:combine, opts))
    case (:proplists.get_value(:locations, opts)) do
      true ->
        :lists.foreach(fn r_lock(name: name, id: id, type: type,
                              stats: stats) ->
                            idString = (case (:proplists.get_value(:full_id,
                                                                     opts)) do
                                          true ->
                                            term2string(:proplists.get_value(name,
                                                                               iDs,
                                                                               id))
                                          _ ->
                                            term2string(id)
                                                        ...
                                                        [{:tries, - 1}, {:colls,
                                                                           - 1},
                                                                            {:time,
                                                                               ...
                                                                               1}]}],
                                                  opts)
                                print_lock_information(filter_print(ps, opts1),
                                                         :proplists.get_value(:print,
                                                                                opts1))
                            end
                       end,
                         combos)
      _ ->
        print = filter_print(locks2print(combos, duration),
                               opts)
        print_lock_information(print,
                                 :proplists.get_value(:print, opts))
    end
    {:reply, :ok, state}
  end

  def handle_call({:histogram, lockname, inOpts}, _From,
           r_state(duration: duration, locks: locks) = state) do
    default = [{:sort, :time}, {:reverse, false}, {:print,
                                                     [:name, :id, :tries,
                                                                      :colls,
                                                                          :ratio,
    ...
    combos = combine_classes(filtered,
                               :proplists.get_value(:combine, opts))
    :lists.foreach(fn r_lock(stats: stats) = l ->
                        sumStats = summate_stats(stats)
                        opts1 = options([{:print,
                                            [:name, :id, :tries, :colls, :ratio,
                                                                             :time,
                                                                                 :duration]},
                                             {:thresholds,
                                                [{:tries, - 1}, {:colls, - 1},
  ...
  def handle_call(:swap_pid_keys, _From,
           r_state(locks: locks) = state) do
    swappedLocks = :lists.map(fn l
  ...
  defp summate_locks([l | ls],
            r_stats(tries: tries, colls: colls, time: time, nt: nt,
                hist: hist)) do
    s = summate_stats(r_lock(l, :stats))
    summate_locks(ls,
                    r_stats(tries: tries + r_stats(s, :tries),
                        colls: colls + r_stats(s, :colls), time: time + r_stats(s, :time),
                        nt: nt + r_stats(s, :nt),
  ...
  defp summate_histogram(hs1, hs2) do
    :erlang.list_to_tuple(for {a,
                                 b} <- :lists.zip(:erlang.tuple_to_list(hs1),
                                                    :erlang.tuple_to_list(hs2)) do
                            a + b
                          end)
  end
  ...
  defp filter_locks(locks, {lockname, ids}) when is_list(ids) do
    for l <- locks, r_lock(l, :name) === lockname,
          :lists.member(r_lock(l, :id), ids) do
      l
    end
  end
  ...
  defp threshold_locks(locks, thresholds) do
    ...
    for l <- locks, r_print(l, :tries) > tries,
          r_print(l, :colls) > colls, r_print(l, :time) > time do
      l
    end
  end
  ...
  defp combine_locations(locations) do
    :gb_trees.values(combine_locations(locations,
                                         :gb_trees.empty()))
  end
  ...
  defp combine_locations([s | ss], name, tree)
    ...
    tree1 = (case (:gb_trees.lookup(key, tree)) do
               :none ->
                 :gb_trees.insert(key, {s, [name]}, tree)
               {:value, {c, names}} ->
                 newNames = (case (:lists.member(name, names)) do
                               true ->
                                 names
                               _ ->
                                 [name | names]
                 ...
                 :gb_trees.update(key,
                                    {r_stats(c, tries: r_stats(c, :tries) + r_stats(s, :tries), 
  ...
  defp locks_ids([r_lock(name: key) = l | ls], out) do
    case (:proplists.get_value(key, out)) do
      ...
      ids ->
        locks_ids(ls,
                    [{key, [r_lock(l, :id) | ids]} | :proplists.delete(key,
                                                                    out)])
    end
  end
  ...
  defp stats2print(stats, duration) do
    :lists.map(fn s ->
                    r_print(entry: term2string('~tp:~p', [r_stats(s, :file), r_stats(s, :line)]),
                        colls: r_stats(s, :colls), tries: r_stats(s, :tries),
                        cr: percent(r_stats(s, :colls), r_stats(s, :tries)),
                        time: r_stats(s, :time), dtr: percent(r_stats(s, :time), duration),
  ...
  defp locks2print(locks, duration) do
    :lists.map(fn l ->
                    r_stats(tries: tries, colls: colls, time: time,
                        hist: hist) = summate_stats(r_lock(l, :stats))
                    cr = percent(colls, tries)
                    dtr = percent(time, duration)
                    r_print(name: r_lock(l, :name), id: r_lock(l, :id), type: r_lock(l, :type),
                        tries: tries, colls: colls,
                        hist: format_histogram(hist), cr: cr, time: time,
                        dtr: dtr)
               end,
                 locks)
  end
  ...
  defp data2state(data, state) do
    duration = time2us(:proplists.get_value(:duration,
                                              data))
  ...
  defp locks2records([{name, id, type, stats} | locks]) do
    [r_lock(name: name, id: clean_id_creation(id), type: type,
         stats: stats2record(stats)) |
         locks2records(locks)]
  end
  ...
  defp stats2record([{{file, line}, {tries, colls, {s, ns, n}},
              hist} |
               stats]) do
    [r_stats(file: file, line: line, hist: hist, tries: tries,
         colls: colls, time: time2us({s, ns}), nt: n) |
         stats2record(stats)]
  end

  defp stats2record([{{file, line}, {tries, colls, {s, ns, n}}} |
               stats]) do
    [r_stats(file: file, line: line, hist: {}, tries: tries,
         colls: colls, time: time2us({s, ns}), nt: n) |
         stats2record(stats)]
  end
  ...
  defp clean_id_creation(id) when is_pid(id) do
    ...
    <<l :: size(lL), node :: size(l) - binary,
        ids :: size(8) - binary, _Creation :: binary>> = rest
    bin2 = :erlang.list_to_binary([h, <<l :: size(lL)>>,
                                          node, ids, <<0 :: size(cL)>>])
  ...
  defp state2list(state) do
    ...
    :lists.zipwith(fn :locks, locks ->
                     ...
                     Keyword.keys(r_state(r_state())), values)
  end
  ...
  defp list2state([:locks | fs], list) do
    locks = (for lock <- :proplists.get_value(:locks, list,
                                                []) do
               list2lock(lock)
             end)
  ...
  defp list2lock([:stats = f | fs], ls) do
    stats = stats2stats(:proplists.get_value(f, ls,
                                               lock_default(f)))
  ...
  defp auto_print_width(locks, print) do
    r = :lists.foldl(fn l, max ->
                          :erlang.list_to_tuple(:lists.reverse(:lists.foldl(fn {:print,
                                                                                  :print},
                                                                                 out ->
                                                                                 [:print |
                                                                                      out]
                                                                               {str,
                                                                                  len},
                                                                                 out ->
                                                                                 [:erlang.min(:erlang.max(length(s(str)) + 1,
                                                                                                            len),
                                                                                                80) |
                                                                                      out]
                                                                            end,
                                                                              [],
                                                                              :lists.zip(:erlang.tuple_to_list(l),
                                                                                           :erlang.tuple_to_list(max)))))
                     end,
                       r_print(id: 4, type: 5, entry: 5, name: 6, tries: 8,
                           colls: 13, cr: 16, time: 11, dtr: 14, hist: 20),
                       locks)
    offsets = [{:id, r_print(r, :id)}, {:name, r_print(r, :name)},
                                     {:type, r_print(r, :type)}, {:entry,
                                                              r_print(r, :entry)},
                                                               {:tries,
                                                                  r_print(r, :tries)},
                                                                   {:colls,
  ...
  defp print_header(opts) do
    header = r_print(name: 'lock', id: 'id', type: 'type', entry: 'location', tries: '#tries',
                 colls: '#collisions', cr: 'collisions [%]', time: 'time [us]', dtr: 'duration [%]', hist: 'histogram [log2(us)]')
    divider = r_print(name: :lists.duplicate(1 + length(r_print(header, :name)),
                                         45),
                  id: :lists.duplicate(1 + length(r_print(header, :id)), 45),
                  type: :lists.duplicate(1 + length(r_print(header, :type)),
                                           45),
                  entry: :lists.duplicate(1 + length(r_print(header, :entry)),
                                            45),
                  tries: :lists.duplicate(1 + length(r_print(header, :tries)),
                                            45),
                  colls: :lists.duplicate(1 + length(r_print(header, :colls)),
                                            45),
                  cr: :lists.duplicate(1 + length(r_print(header, :cr)), 45),
                  time: :lists.duplicate(1 + length(r_print(header, :time)),
                                           45),
                  dtr: :lists.duplicate(1 + length(r_print(header, :dtr)), 45),
                  hist: :lists.duplicate(1 + length(r_print(header, :hist)),
                                           45))
  ...
  defp format_lock(l, [opt | opts]) do
    case (opt) do
      ...
      {:histogram, w} ->
        [{:left, w - length(s(r_print(l, :hist))) - 1,
            s(r_print(l, :hist))} |
             format_lock(l, opts)]
      _ ->
        format_lock(l, opts)
  ...
  defp print_state_information(r_state(locks: locks) = state) do
    ...
    print(kv('duration',
               s(r_state(state, :duration)) ++ ' us' ++ ' (' ++ s(r_state(state, :duration) / 1000000) ++ ' s)'))
    print('\nsummated stats:')
    print(kv('#tries', s(r_stats(stats, :tries))))
    print(kv('#colls', s(r_stats(stats, :colls))))
    print(kv('wait time',
               s(r_stats(stats, :time)) ++ ' us' ++ ' ( ' ++ s(r_stats(stats, :time) / 1000000) ++ ' s)'))
    print(kv('percent of duration',
  ...
  defp term2string({m, f, a}) when (is_atom(m) and is_atom(f) and
                             is_integer(a)) do
  ...
  defp term2string(term) when is_port(term) do
    case (:erlang.term_to_binary(term)) do
      <<_ :: size(2) - binary, ?w, l :: size(8),
          node :: size(l) - binary, ids :: size(32),
          _ :: binary>> ->
        term2string('#Port<~ts.~w>', [node, ids])
      <<_ :: size(2) - binary, ?v, l :: size(16),
          node :: size(l) - binary, ids :: size(32),
          _ :: binary>> ->
        term2string('#Port<~ts.~w>', [node, ids])
      <<_ :: size(2) - binary, ?d, l :: size(16),
          node :: size(l) - binary, ids :: size(32),
          _ :: binary>> ->
