defmodule :m_lcnt do
        ...
        :lists.foreach(
          fn r_lock(name: name, id: id, type: type, stats: stats) ->
            ...
            case combined do
              ...
              _ ->
                print(~c"lock: " ++ term2string(name))
                print(~c"id:   " ++ idString)
                print(~c"type: " ++ term2string(type))
    ...
    case :file.read_file(filename) do
      {:ok, binary} ->
        case :erlang.binary_to_term(binary) do
          ...
          {version, _} ->
            {:reply, {:error, {:mismatch, version, ~c"1.0"}}, state}
        end
  ...
  def handle_call({:save, filename}, _From, state) do
    binary = :erlang.term_to_binary({~c"1.0", state2list(state)})
  ...
  defp string_names([name | names], strings) do
    string_names(names, [term2string(name) ++ ~c"," | strings])
  end
  ...
  defp stats2print(stats, duration) do
    :lists.map(
      fn s ->
        r_print(
          entry: term2string(~c"~tp:~p", [r_stats(s, :file), r_stats(s, :line)]),
  ...
  defp print_state_information(r_state(locks: locks) = state) do
    ...
    print(~c"information:")
    print(kv(~c"#locks", s(length(locks))))

    print(
      kv(
        ~c"duration",
        s(r_state(state, :duration)) ++
          ~c" us" ++ ~c" (" ++ s(r_state(state, :duration) / 1_000_000) ++ ~c" s)"
      )
    )

    print(~c"\nsummated stats:")
    print(kv(~c"#tries", s(r_stats(stats, :tries))))
    print(kv(~c"#colls", s(r_stats(stats, :colls))))

    print(
      kv(
        ~c"wait time",
        s(r_stats(stats, :time)) ++
          ~c" us" ++ ~c" ( " ++ s(r_stats(stats, :time) / 1_000_000) ++ ~c" s)"
      )
    )

    print(
      kv(
        ~c"percent of duration",
        s(percent(r_stats(stats, :time), r_state(state, :duration))) ++ ~c" %"
      )
    )
  ...
  defp kv(key, value, offset) do
    term2string(term2string(~c"~~~ps : ~~s", [offset]), [key, value])
  end
  ...
  defp strings([{:space, n, s} | ss], out) do
    strings(
      ss,
      out ++ term2string(term2string(~c"~~~ws", [n]), [s])
    )
  end
  ...
  defp strings([{:left, n, s} | ss], out) do
    strings(
      ss,
      out ++ term2string(term2string(~c" ~~s~~~ws", [n]), [s, ~c""])
    )
  end

  defp strings([s | ss], out) do
    strings(ss, out ++ term2string(~c"~ts", [s]))
  end

  defp term2string({m, f, a})
       when is_atom(m) and is_atom(f) and
              is_integer(a) do
    term2string(~c"~p:~p/~p", [m, f, a])
  end
  ...
  defp term2string(term) when is_port(term) do
    case :erlang.term_to_binary(term) do
      <<_::size(2)-binary, ?w, l::size(8), node::size(l)-binary, ids::size(32), _::binary>> ->
        term2string(~c"#Port<~ts.~w>", [node, ids])
      ...
      <<_::size(2)-binary, ?d, l::size(16), node::size(l)-binary, ids::size(32), _::binary>> ->
        term2string(~c"#Port<~s.~w>", [node, ids])
    end
  end
  ...
  defp term2string(term) when is_pid(term) do
    case :erlang.term_to_binary(term) do
      <<_::size(2)-binary, ?w, l::size(8), node::size(l)-binary, ids::size(32), serial::size(32),
        _::binary>> ->
        term2string(~c"<~ts.~w.~w>", [node, ids, serial])
      ...
      <<_::size(2)-binary, ?d, l::size(16), node::size(l)-binary, ids::size(32), serial::size(32),
        _::binary>> ->
        term2string(~c"<~s.~w.~w>", [node, ids, serial])
    end
  end

  defp term2string(term) do
    term2string(~c"~w", [term])
  end
