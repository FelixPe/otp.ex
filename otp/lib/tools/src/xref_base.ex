defmodule :m_xref_base do
  ...
  def add_application(state, appDir, options) do
    ...
    validOptions = [{:name, [~c"", &check_name/1]} | optVals]
  ...
  defp analysis(:locals_not_used, :functions) do
    ~c"(L - (OL + range (closure LC | OL))) * ((UU + XU - LU) + domain EE + range EE)"
  end
  ...
  defp analysis({:call, f}, :functions) do
    make_query(~c"range (E | ~tw : Fun)", [f])
  end

  defp analysis({:use, f}, :functions) do
    make_query(~c"domain (E || ~tw : Fun)", [f])
  end

  defp analysis({:module_call, m}, _) do
    make_query(~c"range (ME | ~tw : Mod)", [m])
  end

  defp analysis({:module_use, m}, _) do
    make_query(~c"domain (ME || ~tw : Mod)", [m])
  end

  defp analysis({:application_call, a}, _) do
    make_query(~c"range (AE | ~tw : App)", [a])
  end

  defp analysis({:application_use, a}, _) do
    make_query(~c"domain (AE || ~tw : App)", [a])
  end

  defp analysis({:release_call, r}, _) do
    make_query(~c"range (RE | ~tw : Rel)", [r])
  end

  defp analysis({:release_use, r}, _) do
    make_query(~c"domain (RE || ~tw : Rel)", [r])
  end
  ...
  def format_error({:invalid_options, options}) do
    :io_lib.format(~c"Unknown option(s) or invalid option value(s): ~tp~n", [options])
  end

  def format_error({:invalid_filename, term}) do
    :io_lib.format(~c"A file name (a string) was expected: ~tp~n", [term])
  end

  def format_error({:no_debug_info, fileName}) do
    :io_lib.format(~c"The BEAM file ~tp has no debug info~n", [fileName])
  end

  def format_error({:invalid_path, term}) do
    :io_lib.format(~c"A path (a list of strings) was expected: ~tp~n", [term])
  end

  def format_error({:invalid_query, term}) do
    :io_lib.format(~c"A query (a string or an atom) was expected: ~tp~n", [term])
  end

  def format_error({:not_user_variable, variable}) do
    :io_lib.format(~c"~tp is not a user variable~n", [variable])
  end

  def format_error({:unknown_analysis, term}) do
    :io_lib.format(~c"~tp is not a predefined analysis~n", [term])
  end

  def format_error({:module_mismatch, module, readModule}) do
    :io_lib.format(~c"Name of read module ~tp does not match analyzed module ~tp~n", [
      readModule,
      module
    ])
  end
  ...
  def format_error({:release_clash, {release, dir, oldDir}}) do
    :io_lib.format(
      ~c"The release ~tp read from ~tp clashes with release already read from ~tp~n",
      [release, dir, oldDir]
    )
  end
  ...
  def format_error({:application_clash, {application, dir, oldDir}}) do
    :io_lib.format(
      ~c"The application ~tp read from ~tp clashes with application already read from ~tp~n",
      [application, dir, oldDir]
    )
  end

  def format_error({:module_clash, {module, dir, oldDir}}) do
    :io_lib.format(~c"The module ~tp read from ~tp clashes with module already read from ~tp~n", [
      module,
      dir,
      oldDir
    ])
  ...
  def format_error({:no_such_release, name}) do
    :io_lib.format(~c"There is no analyzed release ~tp~n", [name])
  end

  def format_error({:no_such_application, name}) do
    :io_lib.format(~c"There is no analyzed application ~tp~n", [name])
  end

  def format_error({:no_such_module, name}) do
    :io_lib.format(~c"There is no analyzed module ~tp~n", [name])
  end

  def format_error({:no_such_info, term}) do
    :io_lib.format(
      ~c"~tp is not one of 'modules', 'applications', 'releases' and 'libraries'~n",
      [term]
    )
  end
  ...
  defp do_add_release(dir, relName, oB, oV, oW, state) do
    ...
    case :xref_utils.release_directory(dir, true, ~c"ebin") do
  ...
  defp do_add_directory(dir, appName, bui, rec, ver, war, state) do
    ...
    {fileNames, errors, jams, unreadable} =
      :xref_utils.scan_directory(dir, rec, [~c".beam"], [~c".jam"])
  ...
  defp do_add_a_module(file, appName, builtins, verbose, warnings, state) do
    case :xref_utils.split_filename(file, ~c".beam") do
      false ->
  ...
  defp do_add_libraries(path, verbose, state) do
    ...
    {c, e} = :xref_utils.list_path(path, [~c".beam"])
  ...
  defp do_variables(state) do
    fun = fn
      ...
      {{:tmp, v}, _}, a ->
        :io.format(~c"Bug in ~tp: temporary ~tp~n", [:xref_base, v])
        a
  ...
  defp message(true, what, arg) do
    case what do
      :no_debug_info ->
        :io.format(~c"Skipping ~ts (no debug information)~n", arg)

      :unresolved_summary1 ->
        :io.format(~c"~tp: 1 unresolved call~n", arg)

      :unresolved_summary ->
        :io.format(~c"~tp: ~tp unresolved calls~n", arg)

      :jam ->
        :io.format(~c"Skipping ~ts (probably JAM file)~n", [arg])

      :unreadable ->
        :io.format(~c"Skipping ~ts (unreadable)~n", [arg])

      :xref_attr ->
        :io.format(~c"~ts: Skipping 'xref' attribute ~tw~n", arg)

      :depr_attr ->
        :io.format(~c"~ts: Skipping 'deprecated' attribute ~tw~n", arg)

      :lib_search ->
        :io.format(~c"Scanning library path for BEAM files... ", [])

      :lib_check ->
        :io.format(~c"Checking library files... ", [])

      :set_up ->
        :io.format(~c"Setting up...", arg)

      :done ->
        :io.format(~c"done~n", arg)

      :done_file ->
        :io.format(~c"done reading ~ts~n", arg)

      :error ->
        :io.format(~c"error~n", arg)

      else__ ->
        :io.format(~c"~tp~n", [{else__, arg}])
    end
  end
