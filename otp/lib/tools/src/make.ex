defmodule :m_make do
  ...
  defp normalize_emake(emakeRaw, mods, opts) do
    case emakeRaw do
      ...
      {:error, :enoent} when mods === :undefined ->
        cwdMods =
          for f <- :filelib.wildcard(~c"*.erl") do
            :filename.rootname(f)
          end
      ...
      {:error, error} ->
        :io.format(~c"make: Trouble reading 'Emakefile':~n~tp~n", [error])
        :error
    end
  end
  ...
  defp expand(mod, already) do
    case :lists.member(?*, mod) do
      true ->
        ...
        :lists.foldl(fun, [], :filelib.wildcard(mod ++ ~c".erl"))

      false ->
        mod2 = :filename.rootname(mod, ~c".erl")
  ...
  defp recompile2(objMTime, file, noExec, load, opts) do
    ...
    case check_includes(:lists.append(file, ~c".erl"), includePath, objMTime) do
      true ->
        recompile(file, noExec, load, opts)
  ...
  defp recompile(file, true, _Load, _Opts) do
    :io.format(~c"Out of date: ~ts\n", [file])
  end

  defp recompile(file, false, load, opts) do
    :io.format(~c"Recompile: ~ts\n", [file])
  ...
  defp maybe_load(mod, load, opts) do
    case :compile.output_generated(opts) do
      true ->
        dir = :proplists.get_value(:outdir, opts, ~c".")
        do_load(dir, mod, load)

      false ->
        :io.format(~c"** Warning: No object file created - nothing loaded **~n")
        :ok
    end
  end
