defmodule :m_make do
  ...
  defp sort_options([h | t], make, comp) do
    case (:lists.member(h,
                          [:noexec, :load, :netload, :noload, :emake])) do
      true ->
        sort_options(t, [h | make], comp)
      false ->
  ...
  defp normalize_emake(emakeRaw, mods, opts) do
    case (emakeRaw) do
      ...
      {:error, :enoent} when mods === :undefined ->
        cwdMods = (for f <- :filelib.wildcard('*.erl') do
                     :filename.rootname(f)
                   end)
        [{cwdMods, opts}]
  ...
  defp get_opts_from_emakefile([{makefileMods, o} | rest], mods, opts,
            result) do
    case (members(mods, makefileMods, [], mods)) do
      {[], _} ->
        get_opts_from_emakefile(rest, mods, opts, result)
      {i, restOfMods} ->
        get_opts_from_emakefile(rest, restOfMods, opts,
                                  [{i, o} | result])
    end
  end
