defmodule :m_xref_reader do
  ...
  defp form({:attribute, _Anno, _Attr, _Val}, s) do
    s
  end
  ...
  defp expr({:if, _Anno, cs}, s) do
    clauses(cs, s)
  end

  defp expr({:case, _Anno, e, cs}, s) do
    s1 = expr(e, s)
    clauses(cs, s1)
  end

  defp expr({:receive, _Anno, cs}, s) do
    clauses(cs, s)
  end

  defp expr({:receive, _Anno, cs, to, toEs}, s) do
    s1 = expr(to, s)
    s2 = expr(toEs, s1)
    clauses(cs, s2)
  end

  defp expr({:try, _Anno, es, scs, ccs, as}, s) do
    s1 = expr(es, s)
    s2 = clauses(scs, s1)
    s3 = clauses(ccs, s2)
    expr(as, s3)
  ...
  defp expr({:fun, _Anno, {:clauses, cs}, _Extra}, s) do
    clauses(cs, s)
  end

  defp expr({:fun, anno, {:function, name, arity}}, s) do
    handle_call(:local, r_xrefr(s, :module), name, arity, anno, s)
  end

  defp expr({:fun, _Anno, {:clauses, cs}}, s) do
    clauses(cs, s)
  end

  defp expr({:named_fun, _Anno, :_, cs}, s) do
    clauses(cs, s)
  end

  defp expr({:named_fun, _Anno, name, cs}, s) do
  ...
  defp expr({:match, _Anno, {:var, _, var}, e}, s) do
    s1 = r_xrefr(s, matches: [{var, e} | r_xrefr(s, :matches)])
    expr(e, s1)
  end

  defp expr({:op, _Anno, :orelse, op1, op2}, s) do
    expr([op1, op2], s)
  end

  defp expr({:op, _Anno, :andalso, op1, op2}, s) do
    expr([op1, op2], s)
  end
  ...
  defp term2list({:cons, _Anno, h, t}, l, s) do
    term2list(t, [h | l], s)
  end

  defp term2list({nil, _Anno}, l, _S) do
    reverse(l)
  end
  ...
  defp unresolved(m, f, a, anno, s) do
