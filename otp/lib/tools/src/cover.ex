defmodule :m_cover do
  ...
  defp expand_qualifiers([], _Vs, n) do
    {[], n}
  end
  ...
  defp fix_expr({:if, a, clauses}, line, bump) do
    fixedClauses = fix_clauses(clauses, line, bump)
    {:if, a, fixedClauses}
  end

  defp fix_expr({:case, a, expr, clauses}, line, bump) do
    fixedExpr = fix_expr(expr, line, bump)
    fixedClauses = fix_clauses(clauses, line, bump)
    {:case, a, fixedExpr, fixedClauses}
  end

  defp fix_expr({:receive, a, clauses}, line, bump) do
    fixedClauses = fix_clauses(clauses, line, bump)
  ...
  defp munge_expr({:map, anno, fields}, vars) do
    {mungedFields, vars2} = munge_exprs(fields, vars, [])
    {{:map, anno, mungedFields}, vars2}
  end

  defp munge_expr({:map, anno, arg, fields}, vars) do
    {mungedArg, vars2} = munge_expr(arg, vars)
  ...
  defp munge_expr({:op, anno, op, exprL, exprR}, vars) do
    {mungedExprL, vars2} = munge_expr(exprL, vars)
    {mungedExprR, vars3} = munge_expr(exprR, vars2)
    {{:op, anno, op, mungedExprL, mungedExprR}, vars3}
  end

  defp munge_expr({:op, anno, op, expr}, vars) do
    {mungedExpr, vars2} = munge_expr(expr, vars)
    {{:op, anno, op, mungedExpr}, vars2}
  end

  defp munge_expr({:catch, anno, expr}, vars) do
    {mungedExpr, vars2} = munge_expr(expr, vars)
  ...
  defp munge_expr({:block, anno, body}, vars) do
    {mungedBody, vars2} = munge_body(body, vars)
    {{:block, anno, mungedBody}, vars2}
  end

  defp munge_expr({:if, anno, clauses}, vars) do
    {mungedClauses, vars2} = munge_clauses(clauses, vars)
    {{:if, anno, mungedClauses}, vars2}
  end

  defp munge_expr({:case, anno, expr, clauses}, vars) do
    {mungedExpr, vars2} = munge_expr(expr, vars)
    {mungedClauses, vars3} = munge_clauses(clauses, vars2)
    {{:case, anno, mungedExpr, mungedClauses}, vars3}
  end

  defp munge_expr({:receive, anno, clauses}, vars) do
    {mungedClauses, vars2} = munge_clauses(clauses, vars)
  ...
  defp munge_expr({:named_fun, anno, name, clauses}, vars) do
    {mungedClauses, vars2} = munge_clauses(clauses, vars)
