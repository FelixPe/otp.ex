defmodule :m_xref_compiler do
  use Bitwise
  import :lists, only: [concat: 1, foldl: 3, nthtail: 2,
                          reverse: 1, sort: 1, sublist: 2]
  import :sofs, only: [composite: 2, difference: 2,
                         empty_set: 0, from_term: 1, intersection: 2,
  ...
  def format_error({:type_error, expr}) do
    :io_lib.format('Operator applied to argument(s) of different or invalid type(s): ~ts~n', [expr])
  end
  ...
  defp statements([stmt = {:assign, varType, name, e} | stmts0],
            table, l, uV) do
    case (:dict.find(name, table)) do
      {:ok, _} ->
        throw_error({:variable_reassigned,
                       :xref_parser.t2s(stmt)})
      :error ->
        {type, oType, newE} = t_expr(e, table)
        val = r_xref_var(name: name, vtype: varType, otype: oType,
                  type: type)
        ...
        statements(stmts, newTable, [put | l],
                     [{variable, val} | uV])
    end
  end
  ...
  defp check_expr({:tuple, l}, table) do
    {:expr, type, :vertex, _Consts} = check_constants(l,
                                                        table)
  ...
  defp check_expr(expr = {:set, sOp, e}, table) do
    ...
    op = set_op(sOp)
    nE = function_vertices_to_family(type, oType,
                                       {:call, op, e1})
    {:expr, type, oType, nE}
  end
  ...
  defp check_expr(expr = {:set, sOp, e1, e2}, table) do
    ...
    case (oType) do
      :number ->
        {:expr, :number, :number,
           {:call, ari_op(sOp), nE1, nE2}}
      _Else ->
        {type, newE1, newE2} = (case ({type_ord(type1),
                                         type_ord(type2)}) do
                                  {t1, t2} when t1 === t2 ->
                                    {type1, nE1, nE2}
                                  {t1, t2} when t1 < 2 or t2 < 2 ->
                                    throw_error({:type_error,
                                                   :xref_parser.t2s(expr)})
                                  {t1, t2} when t1 > t2 ->
                                    {type2,
                                       {:convert, oType, type1, type2, nE1},
                                       nE2}
                                  {t1, t2} when t1 < t2 ->
                                    {type1, nE1,
                                       {:convert, oType, type2, type1, nE2}}
                                end)
  ...
  defp check_expr(expr = {:restr, rOp, e1, e2}, table) do
    ...
    case ({oType1, oType2}) do
      {:edge, :vertex} when rOp === :"|||" ->
        {:expr, _, _, r1} = restriction(:"|", e1, type1, nE1,
                                          type2, nE2)
        {:expr, _, _, r2} = restriction(:"||", e1, type1, nE1,
                                          type2, nE2)
        {:expr, type1, :edge, {:call, :intersection, r1, r2}}
      {:edge, :vertex} ->
        restriction(rOp, e1, type1, nE1, type2, nE2)
      {:edge_closure, :vertex} when rOp === :"|||" ->
        {:expr, _, _, r1} = closure_restriction(:"|", type1,
                                                  type2, oType2, nE1, nE2)
        {:expr, _, _, r2} = closure_restriction(:"||", type1,
                                                  type2, oType2, nE1, nE2)
        {:expr, type1, :edge, {:call, :intersection, r1, r2}}
      {:edge_closure, :vertex} ->
  ...
  defp conversions(:vertex, from, to) when (is_atom(from) and
                                     is_atom(to)) do
    :ok
  end

  defp conversions(:edge, from, to) when (is_atom(from) and
                                   is_atom(to)) do
    :ok
  end

  defp conversions(:edge, {:line, line}, to) when (is_atom(to) and
  ...
  defp restriction(rOp, e1, type1, nE1, type2, nE2) do
    ...
    case (nE1) do
      ...
      _ ->
        ...
        {:expr, type1, :edge,
           {:call, :restriction, column, nE1, nE2b}}
    end
  end
  ...
  defp restriction(column, type1, vE, type2, e2)
    ...
    restr = {:call, :union_of_family,
               {:call, :restriction, vE, m}}
    c = {:convert, :vertex, type2, type1, e2}
    f = family_to_function_vertices(type1, :vertex, c)
    {:expr, type1, :edge,
       {:call, :restriction, column, restr, f}}
  end
  ...
  defp check_constants(cs = [c = {:constant, type0, oType, _Con} |
                    cs1],
            table) do
    ...
    case (split(types, cs, table)) do
      [{typeToBe, _Cs}] ->
        s = from_term(for {:constant, _T, _OT, con} <- cs do
                        con
                      end)
        type = what_type(typeToBe)
        e = function_vertices_to_family(type, oType,
                                          {:constants, s})
        {:expr, type, oType, e}
      [{type1, [c1 | _]}, {type2, [c2 | _]} | _] ->
        throw_error({:type_mismatch, make_vertex(type1, c1),
                       make_vertex(type2, c2)})
    end
  end

  defp check_mix([c = {:constant, :Fun, oType, _Con} | cs], :Fun,
            oType, _C0) do
    check_mix(cs, :Fun, oType, c)
  end

  defp check_mix([c = {:constant, type, oType, _Con} | cs],
            type0, oType, _C0)
  ...
  defp check_mix([c | _], _Type0, _OType0, c0) do
    throw_error({:type_mismatch, :xref_parser.t2s(c0),
                   :xref_parser.t2s(c)})
  end
  ...
  defp split([type | types], vs, allSoFar, _Type, table,
            l) do
  ...
  defp special(:edge, {:line, _LineType},
            {:line, :all_line_call}, calls) do
    {:put, {:tmp, :mods},
       {:projection,
          {:quote,
             {:external,
  ...
  defp special(:vertex, :function, {:line, _LineType}, v) do
    {:restriction,
       {:union_of_family,
          {:restriction, {:get, :def_at}, {:domain, v}}},
       {:union_of_family, v}}
  end
       ...
       {:put, {:tmp, :m1},
          ...
          {:image,
             {:projection,
                ...
                {:union,
                   {:image, {:get, callAt}, {:get, {:tmp, :m1}}}}},
             {:get, {:tmp, :ves}}}}}
  end

  defp funs_to_lines(defAt, callAt) do
    t1 = multiple_relative_product({defAt, defAt},
                                     projection(1, callAt))
  ...
  defp save_vars([{i, {:reuse, n}} | dL], d, vs, uVs, l) do
    save_vars(dL, d, [{n, {:save, {:tmp, n}}} | vs], uVs,
                [{n, i} | l])
  end
  ...
  defp save_vars([{{:var, v = {:user, _}}, i} | dL], d, vs, uVs,
            l) do
    n = (case (:dict.fetch(i, d)) do
           {:reuse, n0} ->
             n0
           n0 ->
             n0
  ...
  defp save_vars([{{:var, {:tmp, _}}, _I} | dL], d, vs, uVs,
            l) do
  ...
  defp insert_unput([i = {:get, v = {:tmp, _}} | is], d, l) do
    case (:dict.find(v, d)) do
      ...
      :error ->
        insert_unput(is, :dict.store(v, [], d),
                       [i, {:unput, v} | l])
    end
  end
  ...
  defp evaluate([{:save, var = {:tmp, _}} | p], t,
            s = [val | _]) do
    t1 = update_graph_counter(val, + 1, t)
    evaluate(p, :dict.store(var, val, t1), s)
  end

  defp evaluate([{:save, {:user, name}} | p], t,
            s = [val | _]) do
    r_xref_var(vtype: :user, otype: oType,
        type: type) = :dict.fetch(name, t)
    newVar = r_xref_var(name: name, value: val, vtype: :user,
