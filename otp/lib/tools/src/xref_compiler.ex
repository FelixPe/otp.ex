defmodule :m_xref_compiler do
  ...
  def format_error({:variable_reassigned, expr}) do
    :io_lib.format(~c"Variable assigned more than once: ~ts~n", [expr])
  end

  def format_error({:unknown_variable, name}) do
    :io_lib.format(~c"Variable ~tp used before set~n", [name])
  end

  def format_error({:type_error, expr}) do
    :io_lib.format(~c"Operator applied to argument(s) of different or invalid type(s): ~ts~n", [
      expr
    ])
  end

  def format_error({:type_mismatch, expr1, expr2}) do
    :io_lib.format(~c"Constants of different types: ~ts, ~ts~n", [expr1, expr2])
  end

  def format_error({:unknown_constant, constant}) do
    :io_lib.format(~c"Unknown constant ~ts~n", [constant])
  end
  ...
  defp format_parse_error([~c"invalid_regexp", string, error], line) do
    :io_lib.format(~c"Invalid regular expression \"~ts\"~s: ~ts~n", [
      string,
      line,
      :lists.flatten(error)
    ])
  ...
  defp format_parse_error([~c"invalid_regexp_variable", var], line) do
    :io_lib.format(~c"Invalid wildcard variable ~tp~s (only '_' is allowed)~n", [var, line])
  end

  defp format_parse_error([~c"missing_type", expr], line) do
    :io_lib.format(~c"Missing type of regular expression ~ts~s~n", [expr, line])
  end

  defp format_parse_error([~c"type_mismatch", expr], line) do
    :io_lib.format(~c"Type does not match structure of constant~s: ~ts~n", [line, expr])
  end

  defp format_parse_error([~c"invalid_operator", op], line) do
    :io_lib.format(~c"Invalid operator ~tp~s~n", [op, line])
  end

  defp format_parse_error(error, line) do
    :io_lib.format(~c"Parse error~s: ~ts~n", [line, :lists.flatten(error)])
  end
