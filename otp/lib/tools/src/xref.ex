defmodule :m_xref do
  ...
  def d(directory) do
    fun = fn s ->
               :xref_base.add_directory(s, directory,
                                          {:builtins, true})
          end
    fun1 = fn s ->
                case (fun.(s)) do
  ...
  def add_release(name, dir, options) do
    :gen_server.call(name, {:add_release, dir, options},
                       :infinity)
  end

  def add_application(name, dir) do
    :gen_server.call(name, {:add_application, dir},
                       :infinity)
  end

  def add_application(name, dir, options) do
    :gen_server.call(name, {:add_application, dir, options},
                       :infinity)
  end
  ...
  def add_module(name, file, options) do
    :gen_server.call(name, {:add_module, file, options},
                       :infinity)
  end
  ...
  def add_directory(name, dir, options) do
    :gen_server.call(name, {:add_directory, dir, options},
                       :infinity)
  end

  def replace_module(name, module, file) do
    :gen_server.call(name, {:replace_module, module, file},
                       :infinity)
  end

  def replace_module(name, module, file, options) do
    :gen_server.call(name,
                       {:replace_module, module, file, options}, :infinity)
  end

  def replace_application(name, app, dir) do
    :gen_server.call(name, {:replace_application, app, dir},
                       :infinity)
  end

  def replace_application(name, app, dir, options) do
    :gen_server.call(name,
                       {:replace_application, app, dir, options}, :infinity)
  end
  ...
  def remove_application(name, app) do
    :gen_server.call(name, {:remove_application, app},
                       :infinity)
  end

  def remove_release(name, rel) do
    :gen_server.call(name, {:remove_release, rel},
                       :infinity)
  end
  ...
  def set_library_path(name, path) do
    :gen_server.call(name, {:set_library_path, path},
                       :infinity)
  end

  def set_library_path(name, path, options) do
    :gen_server.call(name,
                       {:set_library_path, path, options}, :infinity)
  end
  ...
  def analyse(name, what, options) do
    :gen_server.call(name, {:analyze, what, options},
                       :infinity)
  end
  ...
  def get_default(name, option) do
    :gen_server.call(name, {:get_default, option},
                       :infinity)
  end

  def set_default(name, optionValues) do
    :gen_server.call(name, {:set_default, optionValues},
                       :infinity)
  end

  def set_default(name, option, value) do
    :gen_server.call(name, {:set_default, option, value},
                       :infinity)
  end
  ...
  def handle_call({:add_application, dir, options}, _From,
           state) do
    case (:xref_base.add_application(state, dir,
                                       options)) do
      {:ok, appName, newState} ->
        {:reply, {:ok, appName}, newState}
      error ->
  ...
  def handle_call({:replace_application, appl, dir}, _From,
           state) do
    case (:xref_base.replace_application(state, appl,
                                           dir)) do
      {:ok, appName, newState} ->
        {:reply, {:ok, appName}, newState}
      error ->
  ...
  def handle_call({:replace_application, appl, dir, opts}, _From,
           state) do
    case (:xref_base.replace_application(state, appl, dir,
                                           opts)) do
      {:ok, appName, newState} ->
        {:reply, {:ok, appName}, newState}
      error ->
  ...
  def handle_call({:set_library_path, path, options}, _From,
           state) do
    case (:xref_base.set_library_path(state, path,
                                        options)) do
      {:ok, newState} ->
        {:reply, :ok, newState}
      error ->
  ...
  def handle_call({:replace_module, module, file, options}, _From,
           state) do
    case (:xref_base.replace_module(state, module, file,
                                      options)) do
      {:ok, ^module, newState} ->
        {:reply, {:ok, module}, newState}
      error ->
  ...
  defp do_functions_analysis(fFun) do
    ...
    {:ok, state1} = :xref_base.set_library_path(state,
                                                  :code_path)
    {:ok, state2} = :xref_base.set_default(state1,
                                             [{:verbose, false}, {:warnings,
                                                                    false}])
    state3 = (case (fFun.(state2)) do
                {:ok, _, s} ->
                  s
                error2 ->
                  throw(error2)
              end)
    {undef, state4} = do_analysis(state3,
                                    :undefined_function_calls)
    {unused, state5} = do_analysis(state4, :locals_not_used)
    {deprecated, _} = do_analysis(state5,
                                    :deprecated_function_calls)
    [{:deprecated, to_external(deprecated)}, {:undefined,
                                                to_external(undef)},
  ...
  defp do_modules_analysis(fFun) do
    ...
    {undef, state4} = do_analysis(state3,
                                    :undefined_functions)
    {deprecated, _} = do_analysis(state4,
