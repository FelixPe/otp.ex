defmodule :m_fprof do
  ...
  def apply({m, f}, args) when (is_atom(m) and is_atom(f) and
                               is_list(args)) do
    apply_1(m, f, args, [])
  end

  def apply(fun, args) when (is_function(fun) and
                            is_list(args)) do
    apply_1(fun, args, [])
  end
  ...
  def apply({m, f}, args, options) when (is_atom(m) and
                                        is_atom(f) and is_list(args) and
                                        is_list(options)) do
    apply_1(m, f, args, options)
  end

  def apply(fun, args, options) when (is_function(fun) and
  ...
  defp apply_1(function, args, options) do
    {[_, procs, continue], options_1} = getopts(options,
    ...
    child = spawn(fn () ->
                       ...
                       receive do
                         {^parent, ^ref, :start_trace} ->
                           case (trace([:start, {:procs, [parent | procs]} |
                                                    options])) do
                             :ok ->
                               (try do
  ...
  require Record
  Record.defrecord(:r_trace_start, :trace_start, procs: :undefined,
                                       mode: :undefined, type: :undefined,
                                       dest: :undefined)
  Record.defrecord(:r_trace_stop, :trace_stop, [])
  Record.defrecord(:r_profile, :profile, src: :undefined,
                                   group_leader: :undefined, dump: :undefined,
                                   flags: :undefined)
  Record.defrecord(:r_profile_start, :profile_start, group_leader: :undefined,
                                         dump: :undefined, flags: :undefined)
  Record.defrecord(:r_profile_stop, :profile_stop, [])
  Record.defrecord(:r_analyse, :analyse, group_leader: :undefined,
                                   dest: :undefined, flags: :undefined,
                                   cols: :undefined, callers: :undefined,
                                   sort: :undefined, totals: :undefined,
                                   details: :undefined)
  ...
  def trace(optionName, value) when is_atom(optionName) do
    trace([{optionName, value}])
  end

  def trace(optionName, value) do
  ...
  def trace(options) when is_list(options) do
    case (getopts(options,
                    [:start, :stop, :procs, :verbose, :file, :tracer,
                                                                 :cpu_time])) do
      {[[], [:stop], [], [], [], [], []], []} ->
        call(r_trace_stop())
      {[[:start], [], procs, verbose, file, tracer, cpuTime],
        ...
        {type, dest} = (case ({file, tracer}) do
                          {[], [{:tracer, pid} = t]} when is_pid(pid) or
                                                            is_port(pid)
                                                          ->
                            t
                          {[:file], []} ->
        ...
        call(r_trace_start(procs: case (procs) do
                        [] ->
                          [self()]
                        [{:procs, p}] when is_list(p) ->
                          p
  ...
  def profile(options) when is_list(options) do
    case (getopts(options,
                    [:start, :stop, :file, :dump, :append])) do
      {[start, [], file, dump, append], []} ->
        ...
        case ({start, file}) do
          {[:start], []} ->
            call(r_profile_start(group_leader: :erlang.group_leader(),
                     dump: target, flags: flags))
          {[], _} ->
            src = (case (file) do
                     [] ->
                       'fprof.trace'
            ...
            call(r_profile(src: src, group_leader: :erlang.group_leader(),
                     dump: target, flags: flags))
          _ ->
            :erlang.error(:badarg, [options])
        end
  ...
  def analyse(options) when is_list(options) do
    case (getopts(options,
                    [:dest, :append, :cols, :callers, :no_callers, :sort,
                                                                       :totals,
                                                                           :details,
                                                                               :no_details])) do
      {[dest, append, cols, callers, noCallers, sort, totals,
                                                          details, noDetails],
         []} ->
        ...
        call(r_analyse(group_leader: :erlang.group_leader(),
                 dest: target, flags: flags,
                 cols: case (cols) do
                         [] ->
                           80
                 ...
                 callers: case ({callers, noCallers}) do
                            {[], []} ->
                              true
                            {[:callers], []} ->
                              true
                 ...
                 details: case ({details, noDetails}) do
                            {[], []} ->
                              true
                            {[:details], []} ->
                              true
  ...
  def save_profile(options) when is_list(options) do
    case (getopts(options, [:file])) do
      {[file], []} ->
        call(r_save_profile(file: case (file) do
                       [] ->
                         'fprof.profile'
                       [{:file, f}] ->
                         f
  ...
  def load_profile(options) when is_list(options) do
    case (getopts(options, [:file])) do
      {[file], []} ->
        call(r_load_profile(file: case (file) do
                       [] ->
                         'fprof.profile'
                       [{:file, f}] ->
                         f
  ...
  Record.defrecord(:r_clocks, :clocks, id: :undefined, cnt: 0,
                                  own: 0, acc: 0)
  Record.defrecord(:r_proc, :proc, id: :undefined,
                                parent: :undefined, spawned_as: :undefined,
                                init_log: [], init_cnt: 2)
  Record.defrecord(:r_misc, :misc, id: :undefined,
                                data: :undefined)
  Record.defrecord(:r_funcstat, :funcstat, callers_sum: :undefined,
                                    called_sum: :undefined, callers: [],
                                    called: [])
  def start() do
    spawn_3step(fn () ->
                     try do
  ...
  def reply({mref, pid}, reply) when (is_reference(mref) and
                                     is_pid(pid)) do
    (try do
  ...
  defp server_loop(state) do
    receive do
      {:fprof_server, {mref, pid} = tag, :"$code_change"}
        ...
        reply(tag, :ok)
        :fprof.unquote(:"$code_change")(state)
  ...
  def unquote(:"$code_change")(state) do
    case (:lists.keysearch(:time, 1,
                             __MODULE__.module_info(:compile))) do
  ...
  defp try_pending_stop(state) do
    case ({:erlang.get(:trace_state),
             :erlang.get(:profile_state),
             :erlang.get(:pending_stop)}) do
  ...
  defp handle_req(r_trace_start(procs: procs, mode: mode, type: :file,
              dest: filename),
            tag, state) do
    case ({:erlang.get(:trace_state),
             :erlang.get(:pending_stop)}) do
  ...
  defp handle_req(r_trace_start(procs: procs, mode: mode, type: :tracer,
              dest: tracer),
            tag, state) do
    case ({:erlang.get(:trace_state),
             :erlang.get(:pending_stop)}) do
      ...
      :running ->
        ...
        case (:erlang.erase(:trace_type)) do
          ...
          :tracer ->
            ...
            :erlang.put(:trace_state, :idle)
            case ({:erlang.get(:profile_state),
                     :erlang.get(:profile_type),
  ...
  defp handle_req(r_profile(src: filename, group_leader: groupLeader,
              dump: dump, flags: flags),
            tag, state) do
    case ({:erlang.get(:profile_state),
             :erlang.get(:pending_stop)}) do
      {{:idle, _}, []} ->
  ...
  defp handle_req(r_profile_start(group_leader: groupLeader, dump: dump,
              flags: flags),
            tag, state) do
    case ({:erlang.get(:profile_state),
             :erlang.get(:pending_stop)}) do
  ...
  defp handle_req(r_profile_stop(), tag, state) do
    case ({:erlang.get(:profile_state),
             :erlang.get(:profile_type)}) do
      {:running, :tracer} ->
        profilePid = :erlang.get(:profile_pid)
        case ({:erlang.get(:trace_state),
                 :erlang.get(:trace_type), :erlang.get(:trace_pid)}) do
  ...
  defp handle_req(r_analyse(dest: dest, flags: flags) = request, tag,
            state) do
  ...
  defp ensure_open(filename, options) when is_atom(filename) or
                                    is_list(filename) do
    :file.open(filename, [{:encoding, :utf8} | options])
  end

  def getopts(list, options) when (is_list(list) and
                                is_list(options)) do
  ...
  defp trace_flags(:normal) do
    [:call, :return_to, :running, :procs,
                                      :garbage_collection, :arity, :timestamp,
                                                                       :set_on_spawn]
  end

  defp trace_flags(:verbose) do
    [:call, :return_to, :send, :receive, :running, :procs,
                                                       :garbage_collection,
                                                           :timestamp,
                                                               :set_on_spawn]
  end
  ...
  defp spawn_link_dbg_trace_client(file, table, groupLeader, dump) do
    case (:dbg.trace_client(:file, file,
                              {&handler/2,
                                 {:init, groupLeader, table, dump}})) do
      pid when is_pid(pid) ->
        :erlang.link(pid)
        pid
      other ->
  ...
  defp handler(:end_of_trace,
            {:error, reason, _, groupLeader, dump}) do
    info(groupLeader, dump, '~nEnd of erroneous trace!~n', [])
    exit(reason)
  end

  defp handler(:end_of_trace,
            {_, tS, groupLeader, table, dump}) do
    dump(dump, :end_of_trace)
  ...
  defp info_suspect_call(groupLeader, _, func, pid) do
    :io.format(groupLeader, '~nWarning: ~tp called in ~p - trace may become corrupt!~n', parsify([func, pid]))
  end
  ...
  defp trace_handler({:trace_ts, pid, :call, _MFA, _TS} = trace,
            _Table, _, dump) do
    stack = :erlang.get(pid)
    dump_stack(dump, stack, trace)
    throw({:incorrect_trace_data, :fprof, 1712,
             [trace, stack]})
  end

  defp trace_handler({:trace_ts, pid, :call, {_M, _F, arity} = func,
             {:cp, cP}, tS} = trace,
  ...
  defp trace_handler({:trace_ts, pid, :call, {_M, _F, args} = mFArgs,
             {:cp, cP}, tS} = trace,
            table, _, dump)
  ...
  defp trace_handler({:trace_ts, pid, :return_to, :undefined,
             tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :return_to,
             {_M, _F, arity} = func, tS} = trace,
            table, _, dump)
      when is_integer(arity) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :spawn, child, mFArgs,
             tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :spawned, parent, mFArgs,
             tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :exit, _Reason, tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    trace_exit(table, pid, tS)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :out, 0, tS} = trace, table, _,
            dump) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :out, {_M, _F, arity} = func,
             tS} = trace,
            table, _, dump)
      when is_integer(arity) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :out, {_M, _F, args} = mFArgs,
             tS} = trace,
            table, _, dump)
      when is_list(args) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :in, 0, tS} = trace, table, _,
            dump) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :in, {_M, _F, arity} = func,
             tS} = trace,
            table, _, dump)
      when is_integer(arity) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :in, {_M, _F, args} = mFArgs,
             tS} = trace,
            table, _, dump)
      when is_list(args) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :gc_minor_start, _Func,
             tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :gc_major_start, _Func,
             tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    trace_gc_start(table, pid, tS)
  ...
  defp trace_handler({:trace_ts, pid, :gc_start, _Func, tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    trace_gc_start(table, pid, tS)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :gc_minor_end, _Func,
             tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
  ...
  defp trace_handler({:trace_ts, pid, :gc_major_end, _Func,
             tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    trace_gc_end(table, pid, tS)
  ...
  defp trace_handler({:trace_ts, pid, :gc_end, _Func, tS} = trace,
            table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    trace_gc_end(table, pid, tS)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :link, _OtherPid, tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  end

  defp trace_handler({:trace_ts, pid, :unlink, _OtherPid,
             tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :getting_linked, _OtherPid,
             tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :getting_unlinked, _OtherPid,
             tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :register, _Name, tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  end

  defp trace_handler({:trace_ts, pid, :unregister, _Name,
             tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :send, _OtherPid, _Msg,
             tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  ...
  defp trace_handler({:trace_ts, pid, :send_to_non_existing_process,
             _OtherPid, _Msg, tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  end

  defp trace_handler({:trace_ts, pid, :receive, _Msg, tS} = trace,
            _Table, _, dump) do
    dump_stack(dump, :erlang.get(pid), trace)
    tS
  end
  ...
  defp trace_handler(trace, _Table, _, dump) do
    dump(dump, trace)
    throw({:incorrect_trace_data, :fprof, 1910, [trace]})
  end
  ...
  defp trace_call(table, pid, func, tS, cP) do
    ...
    case (stack) do
      ...
      [[{:suspend, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 1992,
                 [pid, func, tS, cP, stack]})
      [[{:garbage_collect, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 1995,
                 [pid, func, tS, cP, stack]})
      [[{^cP, _} | _], [{^cP, _} | _] | _] ->
        init_log(table, proc, func)
        :erlang.put(pid,
      ...
      [[{func0, _} | _], [{func0, _} | _], [{^cP, _} | _] |
        ...
        :erlang.put(pid,
                      trace_call_shove(table, pid, func, tS,
                                         trace_return_to_int(table, pid, func0,
                                                               tS, stack)))
      [[{_, tS0} | _] = level0] ->
        init_log(table, proc, func)
        oldStack = (cond do
  ...
  defp trace_call_collapse_1([{func0, _} | _] = stack, [{func0, _} | s1] = s,
            n) do
  ...
  defp trace_call_collapse_2([{func1, _} | [{func2, _} | _] = stack2],
            [{func1, _} | [{func2, _} | _] = s2], n) do
  ...
  defp trace_return_to(table, pid, func, tS) do
    ...
    case (stack) do
      [[{:suspend, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 2140,
                 [pid, func, tS, stack]})
      [[{:garbage_collect, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 2143,
                 [pid, func, tS, stack]})
      [_ | _] ->
        :erlang.put(pid,
  ...
  defp trace_return_to_1(table, pid, func, tS,
            [[{func, _} | level0] | stack1] = stack) do
  ...
  defp trace_return_to_2(table, pid, func, tS,
            [[{func0, _} | level1] | stack1] = stack) do
    case (trace_return_to_2(table, pid, func, tS,
                              [level1 | stack1])) do
      {:undefined, _} = r ->
        r
      {newStack, charged} = r ->
  ...
  defp trace_exit(table, pid, tS) do
    ...
    case (stack) do
      ...
      [] ->
        :ok
      [_ | _] = ^stack ->
        _ = trace_return_to_int(table, pid, :undefined, tS,
                                  stack)
        :ok
    end
    :ok
  ...
  defp trace_out(table, pid, func, tS) do
    ...
    case (stack) do
      [] ->
        :erlang.put(pid,
                      trace_call_push(table, pid, :suspend, tS,
                                        case (func) do
                                          :undefined ->
                                            []
                                          _ ->
  ...
  defp trace_in(table, pid, func, tS) do
    ...
    case (stack) do
      ...
      _ ->
        throw({:inconsistent_trace_data, :fprof, 2303,
                 [pid, func, tS, stack]})
    end
  end
  ...
  defp trace_gc_start(table, pid, tS) do
    ...
    :erlang.put(pid,
                  trace_call_push(table, pid, :garbage_collect, tS,
                                    stack))
  end
    ...
    case (stack) do
      ...
      [[{:garbage_collect, _}], [{func1, _} | _] | _] ->
        :erlang.put(pid,
                      trace_return_to_int(table, pid, func1, tS, stack))
      _ ->
        throw({:inconsistent_trace_data, :fprof, 2329,
                 [pid, tS, stack]})
    end
  end
  ...
  defp init_log(table, r_proc(init_cnt: n, init_log: l) = proc,
            entry) do
    :ets.insert(table,
  ...
  defp trace_clock(_Table, _Pid, _T,
  ...
  defp trace_clock(table, pid, t,
            [[{:garbage_collect, tS0}], [{:suspend, _}]], clock) do
    trace_clock_1(table, pid, t, tS0, :undefined,
                    :garbage_collect, clock)
  end

  defp trace_clock(table, pid, t,
            [[{:garbage_collect, tS0}], [{:suspend, _}], [{func2,
                                                             _} |
                                                              _] |
                                                             _],
            clock) do
    trace_clock_1(table, pid, t, tS0, func2,
                    :garbage_collect, clock)
  end

  defp trace_clock(table, pid, t,
            [[{func0, tS0}, {func1, _} | _] | _], clock) do
  ...
  defp trace_clock(table, pid, t, [[{func0, tS0}]], clock) do
    trace_clock_1(table, pid, t, tS0, :undefined, func0,
                    clock)
  end
  ...
  defp clocks_sum(r_clocks(id: _Id1, cnt: cnt1, own: own1, acc: acc1),
            r_clocks(id: _Id2, cnt: cnt2, own: own2, acc: acc2), id) do
    r_clocks(id: id, cnt: cnt1 + cnt2, own: own1 + own2,
        acc: acc1 + acc2)
  end
  ...
  defp do_analyse_1(table,
            r_analyse(group_leader: groupLeader, dest: io, cols: cols0,
                callers: printCallers, sort: sort, totals: printTotals,
                details: printDetails) = _Analyse) do
    waste = 11
    minCols = waste + 12
    cols = (cond do
              cols0 < minCols ->
    ...
    pidTable = :ets.new(:fprof,
                          [:set, :private, {:keypos, r_clocks(:id)}])
    procTable = :ets.new(:fprof,
                           [:set, :private, {:keypos, r_proc(:id)}])
    ets_select_foreach(table, [{:_, [], [:"$_"]}], 100,
                         fn r_clocks(id: {pid, caller, func}) = clocks ->
    ...
    {firstTS, lastTS,
       _TraceCnt} = (case ({:ets.lookup(procTable, :first_ts),
                              :ets.lookup(procTable, :last_ts_n)}) do
    ...
    :lists.foreach(fn {{pid, _Func}, funcstat} ->
                        :erlang.put(pid,
                                      [funcstat | case (:erlang.get(pid)) do
                                                    :undefined ->
                                                      []
                                                    other ->
                                                      other
    ...
    pidSorted = postsort_r(:lists.sort(:ets.select(pidTable,
                                                     [{:_, [],
                                                         [[{:element, r_clocks(:own),
                                                              :"$_"} |
                                                               :"$_"]]}])))
    ...
    println(dest)
    :lists.foreach(fn {r_clocks() = clocks, procOrPid,
                         funcstatList} ->
                        println(dest, '%  ', :head, '', '')
                        case (procOrPid) do
                          r_proc() ->
                        ...
                        println(dest)
                        :lists.foreach(fn r_funcstat(callers_sum: callersSum,
                                              callers: callers,
                                              called: called) ->
                     ...
                     :lists.map(fn r_clocks(id: pid) = clocks ->
                                     proc = (case (:ets.lookup(procTable,
                                                                 pid)) do
                                               [] ->
                                                 pid
                                               [procX] ->
                                     ...
                                     funcstatList = (case (:erlang.get(pid)) do
                                                       :undefined ->
                                                         []
                                                       fL ->
                                                         fL
  ...
  defp print_proc(dest,
            r_proc(id: _Pid, parent: parent, spawned_as: spawnedAs,
                init_log: initLog)) do
  ...
  def println({io, [w1, w2, w3, w4]}, head,
           r_clocks(id: pid, cnt: cnt, acc: _, own: own), tail, comment)
      when is_pid(pid) do
    :io.put_chars(io,
                    [pad(head, ?\s, 3), flat_format(parsify(pid), ?,, w1),
                                            flat_format(cnt, ?,, w2, :right),
                                                flat_format(:undefined, ?,, w3,
                                                              :right),
                                                    flat_format(own * 0.001, [],
                                                                  w4 - 1,
                                                                  :right),
                                                        pad(tail, ?\s, 4),
                                                            pad(?\s, comment,
  ...
  def println({io, [w1, w2, w3, w4]}, head,
           r_clocks(id: {_M, _F, _A} = func, cnt: cnt, acc: acc,
               own: own),
           tail, comment) do
    :io.put_chars(io,
                    [pad(head, ?\s, 3), flat_format(func, ?,, w1),
                                            flat_format(cnt, ?,, w2, :right),
                                                flat_format(acc * 0.001, ?,, w3,
                                                              :right),
                                                    flat_format(own * 0.001, [],
                                                                  w4 - 1,
                                                                  :right),
  ...
  def println({io, [w1, w2, w3, w4]}, head,
           r_clocks(id: id, cnt: cnt, acc: acc, own: own), tail,
           comment) do
    :io.put_chars(io,
                    [pad(head, ?\s, 3), flat_format(parsify(id), ?,, w1),
                                            flat_format(cnt, ?,, w2, :right),
                                                flat_format(acc * 0.001, ?,, w3,
                                                              :right),
                                                    flat_format(own * 0.001, [],
  ...
  def println({io, [w1, w2, w3, w4]}, head, :head, tail,
           comment) do
    :io.put_chars(io,
                    [pad(head, ?\s, 3), pad(' ', ?\s, w1), pad(?\s, ' CNT ', w2),
                                                             pad(?\s, ' ACC ', w3),
                                                                 pad(?\s, ' OWN',
                                                                       w4 - 1),
                                                                     pad(tail,
  ...
  def println({io, _}, head, {tag, term}, tail, comment) do
    :io.format(io, '~s~tp, ~tp~ts~ts~n',
                 [pad(head, ?\s, 3), parsify(tag), parsify(term), tail,
                                                                      comment])
  end
  ...
  defp funcstat_pd(pid, func1, func0, clocks) do
    :erlang.put({pid, func0},
                  case (:erlang.get({pid, func0})) do
                    :undefined ->
                      r_funcstat(callers_sum: r_clocks(clocks, id: func0),
                          called_sum: r_clocks(id: func0),
                          callers: [r_clocks(clocks, id: func1)])
                    r_funcstat(callers_sum: callersSum,
                        callers: callers) = funcstatCallers ->
                      r_funcstat(funcstatCallers, callers_sum: clocks_sum(callersSum,
                                                                   clocks,
                                                                   func0), 
                                           callers: insert_call(clocks, func1,
                                                                  callers))
                  end)
    :erlang.put({pid, func1},
                  case (:erlang.get({pid, func1})) do
                    :undefined ->
                      r_funcstat(callers_sum: r_clocks(id: func1),
                          called_sum: r_clocks(clocks, id: func1),
                          called: [r_clocks(clocks, id: func0)])
                    r_funcstat(called_sum: calledSum,
                        called: called) = funcstatCalled ->
                      r_funcstat(funcstatCalled, called_sum: clocks_sum(calledSum,
                                                                 clocks,
                                                                 func1), 
                                          called: insert_call(clocks, func0,
                                                                called))
                  end)
  end
  ...
  defp funcstat_sort_r_1([r_funcstat(callers_sum: r_clocks() = clocks, callers: callers,
               called: called) = funcstat |
               l],
            element, r) do
    funcstat_sort_r_1(l, element,
                        [[:erlang.element(element, clocks) |
                              r_funcstat(funcstat, callers: clocks_sort_r(callers,
                                                                   element), 
                                            called: clocks_sort_r(called,
                                                                    element))] |
                             r])
  end
  ...
  defp pad(char, l, size) when (is_integer(char) and
    ...
    cond do
      length >= size ->
        list
      true ->
        :lists.append(:lists.duplicate(size - length, char),
                        list)
    end
  end

  defp pad(l, char, size) when (is_list(l) and
  ...
  defp ets_select_foreach(table, matchSpec, limit, fun) do
    ...
    ets_select_foreach_1(:ets.select(table, matchSpec,
                                       limit),
                           fun)
  end
