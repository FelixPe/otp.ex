defmodule :m_fprof do
  ...
  def unquote(:"$code_change")(state) do
    case :lists.keysearch(:time, 1, __MODULE__.module_info(:compile)) do
      {:value, {:time, {y, m, d, hH, mM, sS}}} ->
        :io.format(
          ~c"~n~w: code change to compile time " ++
            ~c"~4..0w-~2..0w-~2..0w ~2..0w:~2..0w:~2..0w~n",
  ...
  defp handle_req(request, tag, state) do
    :io.format(~c"~n~p:handle_req, unknown request - ~p~n", [:fprof, request])
  ...
  defp handle_other({:EXIT, pid, reason} = other, state)
    ...
    case {:erlang.get(:trace_state), :erlang.get(:trace_pid)} do
      {:running, ^pid} ->
        trace_off()
        :io.format(~c"~n~p:handle_other, unexpected ~p (trace_pid)~n", [:fprof, other])
        :erlang.put(:trace_state, :idle)
        :erlang.erase(:trace_type)
        :erlang.erase(:trace_pid)
        try_pending_stop(state)
      ...
      _ ->
        case {:erlang.get(:profile_state), :erlang.get(:profile_pid)} do
          ...
          _ ->
            :io.format(~c"~n~p:handle_other, unexpected ~p~n", [:fprof, other])
            state
        end
    end
  end
  ...
  defp handle_other(other, state) do
    :io.format(~c"~p:handle_other, unknown - ~p", [:fprof, other])
    state
  end
  ...
  defp handler(
    ...
    info(groupLeader, dump, ~c"Empty trace!~n", [])
  ...
  defp handler(
    ...
    info(groupLeader, dump, ~c"~nEnd of erroneous trace!~n", [])
    exit(reason)
  end
  ...
  defp handler(
    ...
    dump(dump, :end_of_trace)
    info(groupLeader, dump, ~c"~nEnd of trace!~n", [])
  ...
  defp handler(trace, {:init, groupLeader, table, dump}) do
    dump(dump, :start_of_trace)
    info(groupLeader, dump, ~c"Reading trace data...~n", [])
  ...
  defp info_dots(groupLeader, _, n) do
    cond do
      rem(n, 100_000) === 0 ->
        :io.format(groupLeader, ~c",~n", [])

      rem(n, 50000) === 0 ->
        :io.format(groupLeader, ~c".~n", [])

      rem(n, 1000) === 0 ->
        :io.put_chars(groupLeader, ~c".")
  ...
  defp info_suspect_call(groupLeader, _, func, pid) do
    :io.format(
      groupLeader,
      ~c"~nWarning: ~tp called in ~p - trace may become corrupt!~n",
      parsify([func, pid])
    )
  end
  ...
  defp dump_stack(dump, stack, term) do
    ...
    :io.format(dump, ~c"~s~tp.~n", [:lists.duplicate(depth, ~c"  "), parsify(term)])
    true
  end
  ...
  defp dump(dump, term) do
    :io.format(dump, ~c"~tp.~n", [parsify(term)])
    true
  end
  ...
  defp trace_handler({:trace_ts, pid, :call, _MFA, _TS} = trace, _Table, _, dump) do
    ...
    throw({:incorrect_trace_data, :fprof, 1712, [trace, stack]})
  end
  ...
  defp trace_call(table, pid, func, tS, cP) do
    stack = get_stack(pid)
    dbg(0, ~c"trace_call(~p, ~p, ~p, ~p)~n~p~n", [pid, func, tS, cP, stack])
    ...
    case stack do
      ...
      [[{:suspend, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 1992, [pid, func, tS, cP, stack]})

      [[{:garbage_collect, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 1995, [pid, func, tS, cP, stack]})
  ...
  defp trace_return_to(table, pid, func, tS) do
    stack = get_stack(pid)
    dbg(0, ~c"trace_return_to(~p, ~p, ~p)~n~p~n", [pid, func, tS, stack])

    case stack do
      [[{:suspend, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 2140, [pid, func, tS, stack]})

      [[{:garbage_collect, _} | _] | _] ->
        throw({:inconsistent_trace_data, :fprof, 2143, [pid, func, tS, stack]})
  ...
  defp trace_spawn(table, pid, mFArgs, tS, parent) do
    stack = :erlang.get(pid)
    dbg(0, ~c"trace_spawn(~p, ~p, ~p, ~p)~n~p~n", [pid, mFArgs, tS, parent, stack])
  ...
  defp trace_exit(table, pid, tS) do
    ...
    dbg(0, ~c"trace_exit(~p, ~p)~n~p~n", [pid, tS, stack])
  ...
  defp trace_out(table, pid, func, tS) do
    stack = get_stack(pid)
    dbg(0, ~c"trace_out(~p, ~p, ~p)~n~p~n", [pid, func, tS, stack])
  ...
  defp trace_in(table, pid, func, tS) do
    stack = :erlang.get(pid)
    dbg(0, ~c"trace_in(~p, ~p, ~p)~n~p~n", [pid, func, tS, stack])
    ...
    case stack do
      ...
      _ ->
        throw({:inconsistent_trace_data, :fprof, 2303, [pid, func, tS, stack]})
    end
  end

  defp trace_gc_start(table, pid, tS) do
    stack = get_stack(pid)
    dbg(0, ~c"trace_gc_start(~p, ~p)~n~p~n", [pid, tS, stack])
  ...
  defp trace_gc_end(table, pid, tS) do
    stack = :erlang.get(pid)
    dbg(0, ~c"trace_gc_end(~p, ~p)~n~p~n", [pid, tS, stack])
    ...
    case stack do
      ...
      _ ->
        throw({:inconsistent_trace_data, :fprof, 2329, [pid, tS, stack]})
    end
  end
  ...
  defp trace_clock(_Table, _Pid, _T, [[{:suspend, _}], [{:suspend, _}] | _] = _Stack, _Clock) do
    dbg(9, ~c"trace_clock(Table, ~w, ~w, ~w, ~w)~n", [_Pid, _T, _Stack, _Clock])
    :ok
  end
  ...
  defp clock_add(table, id, clock, t) do
    dbg(1, ~c"clock_add(Table, ~w, ~w, ~w)~n", [id, clock, t])
    ...
    try do
      ...
      :error, :badarg ->
        ...
        cond do
          ...
          true ->
            dbg(0, ~c"Negative counter value ~p ~p ~p ~p~n", [x, id, clock, t])
        end
  ...
  defp clocks_add(table, r_clocks(id: id) = clocks) do
    dbg(1, ~c"clocks_add(Table, ~w)~n", [clocks])
  ...
  defp ts_sub({a, b, c} = _T, {a0, b0, c0} = _T0) do
    ...
    cond do
      ...
      true ->
        dbg(9, ~c"Negative counter value ~p ~p ~p~n", [x, _T, _T0])
    end
  ...
  defp do_analyse(table, analyse) do
    dbg(5, ~c"do_analyse_1(~p, ~p)~n", [table, analyse])
    ...
    dbg(5, ~c"do_analyse_1(_, _) ->~p~n", [result])
    result
  end
  ...
  defp do_analyse_1(
    ...
    _Erase = :erlang.erase()
    dbg(2, ~c"erase() -> ~p~n", [_Erase])
    :io.format(groupLeader, ~c"Processing data...~n", [])
    ...
    dbg(3, ~c"Totals0 =  ~p~n", [totals0])
    dbg(3, ~c"PidTable =  ~p~n", [:ets.tab2list(pidTable)])
    dbg(3, ~c"ProcTable =  ~p~n", [:ets.tab2list(procTable)])
    dbg(4, ~c"Totals = ~p~n", [totals])
    ...
    dbg(4, ~c"PidSorted = ~p~n", [pidSorted])
    :io.format(groupLeader, ~c"Creating output...~n", [])
    println(dest, ~c"%% ", [], ~c"Analysis results:", ~c"")
    println(dest, ~c"{  ", :analysis_options, ~c",", ~c"")
    println(dest, ~c" [{", {:callers, printCallers}, ~c"},", ~c"")
    println(dest, ~c"  {", {:sort, sort}, ~c"},", ~c"")
    println(dest, ~c"  {", {:totals, printTotals}, ~c"},", ~c"")
    println(dest, ~c"  {", {:details, printDetails}, ~c"}]}.", ~c"")
    println(dest)

    :lists.foreach(
      fn {r_clocks() = clocks, procOrPid, funcstatList} ->
        println(dest, ~c"%  ", :head, ~c"", ~c"")

        case procOrPid do
          r_proc() ->
            println(dest, ~c"[{ ", clocks, ~c"},", ~c"%%")
    ...
    :io.format(groupLeader, ~c"Done!~n", [])
    :ok
  end
  ...
  defp print_proc(
    ...
    case {parent, spawnedAs, initLog} do
      ...
      {_, :undefined, []} ->
        println(dest, ~c" { ", {:spawned_by, parsify(parent)}, ~c"}].", ~c"")
      ...
      _ ->
        ...
        case {spawnedAs, initLog} do
          {_, []} ->
            println(dest, ~c" { ", {:spawned_as, spawnedAs}, ~c"}].", ~c"")

          {:undefined, _} ->
            println(dest, ~c" { ", {:initial_calls, :lists.reverse(initLog)}, ~c"}].", ~c"")
  ...
  def println({io, _}, head, [], tail, comment) do
    :io.format(io, ~c"~s~ts~ts~n", [pad(head, ?\s, 3), tail, comment])
  end

  def println({io, _}, head, {tag, term}, tail, comment) do
    :io.format(io, ~c"~s~tp, ~tp~ts~ts~n", [
      pad(head, ?\s, 3),
      parsify(tag),
      parsify(term),
      tail,
  ...
  def println({io, _}, head, term, tail, comment) do
    :io.format(io, ~c"~s~tp~ts~ts~n", [pad(head, ?\s, 3), parsify(term), tail, comment])
  end
  ...
  defp flat_format(f, trailer) when is_float(f) do
    :lists.flatten([:io_lib.format(~c"~.3f", [f]), trailer])
  end

  defp flat_format(w, trailer) do
    :lists.flatten([:io_lib.format(~c"~tp", [w]), trailer])
  end
