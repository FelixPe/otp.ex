defmodule :m_lists do
  ...
  def nth(n, [_ | _] = l) when (is_integer(n) and n > 1) do
    nth_1(n, l)
  end
  ...
  def nthtail(n, [_ | _] = l) when (is_integer(n) and n > 1) do
    nthtail_1(n, l)
  end

  defp nthtail_1(1, [_ | t]) do
    t
  end

  defp nthtail_1(n, [_ | t]) do
    nthtail_1(n - 1, t)
  end
  ...
  def seq(first, last) when (is_integer(first) and
  ...
  def seq(first, last, inc) when (is_integer(first) and
                                   is_integer(last) and is_integer(inc) and
                                   inc > 0 and first - inc <= last or inc < 0 and first - inc >= last) do
    n = div(last - first + inc, inc)
    seq_loop(n, inc * (n - 1) + first, inc, [])
  end

  def seq(same, same, 0) when is_integer(same) do
    [same]
  end

  def seq(first, last, inc) do
    :erlang.error(:badarg, [first, last, inc],
  ...
  def sublist(list, 1, l) when (is_list(list) and
  ...
  def sublist([_H | t], s, l) when (is_integer(s) and
                                 s >= 2) do
    sublist(t, s - 1, l)
  end

  def sublist(list, l) when (is_integer(l) and
                          is_list(list)) do
    sublist_2(list, l)
  end
  ...
  def zip([], [_ | _] = ys, {:pad, {x, _}}) do
    for y <- ys do
      {x, y}
    end
  end

  def zip([_ | _] = xs, [], {:pad, {_, y}}) do
    for x <- xs do
      {x, y}
    end
  end
  ...
  def zip3(xs, ys, zs) do
    zip3(xs, ys, zs, :fail)
  end

  def zip3([x | xs], [y | ys], [z | zs], how) do
  ...
  def zip3(xs, ys, zs, :trim) when (is_list(xs) and
                                    is_list(ys) and is_list(zs)) do
    []
  end
  ...
  def zip3([], [], [_ | _] = zs, {:pad, {x, y, _}}) do
    for z <- zs do
      {x, y, z}
    end
  end

  def zip3([], [_ | _] = ys, [], {:pad, {x, _, z}}) do
    for y <- ys do
      {x, y, z}
    end
  end

  def zip3([_ | _] = xs, [], [], {:pad, {_, y, z}}) do
    for x <- xs do
      {x, y, z}
    end
  end
  ...
  def zipwith(f, xs, ys) do
    zipwith(f, xs, ys, :fail)
  end

  def zipwith(f, [x | xs], [y | ys], how) do
  ...
  def zipwith(f, [], [], {:pad, {_, _}}) when is_function(f,
                                                       2) do
    []
  end
  ...
  def zipwith(f, [], [_ | _] = ys, {:pad, {x, _}}) do
    for y <- ys do
      f.(x, y)
    end
  end

  def zipwith(f, [_ | _] = xs, [], {:pad, {_, y}}) do
    for x <- xs do
      f.(x, y)
    end
  end

  def zipwith3(f, xs, ys, zs) do
    zipwith3(f, xs, ys, zs, :fail)
  end

  def zipwith3(f, [x | xs], [y | ys], [z | zs], how) do
  ...
  def zipwith3(f, xs, ys, zs, :trim) when (is_function(f, 3) and
                                       is_list(xs) and is_list(ys) and
                                       is_list(zs)) do
    []
  end

  def zipwith3(f, [], [], [], {:pad, {_, _, _}})
      when is_function(f, 3) do
    []
  end

  def zipwith3(f, [], [], [_ | _] = zs, {:pad, {x, y, _}}) do
    for z <- zs do
      f.(x, y, z)
    end
  end

  def zipwith3(f, [], [_ | _] = ys, [], {:pad, {x, _, z}}) do
    for y <- ys do
      f.(x, y, z)
    end
  end

  def zipwith3(f, [_ | _] = xs, [], [], {:pad, {_, y, z}}) do
    for x <- xs do
      f.(x, y, z)
    end
  end

  def zipwith3(f, [], [y | ys], [z | zs],
           {:pad, {x, _, _}} = how) do
    [f.(x, y, z) | zipwith3(f, [], ys, zs, how)]
  end

  def zipwith3(f, [x | xs], [], [z | zs],
           {:pad, {_, y, _}} = how) do
    [f.(x, y, z) | zipwith3(f, xs, [], zs, how)]
  end

  def zipwith3(f, [x | xs], [y | ys], [],
           {:pad, {_, _, z}} = how) do
  ...
  def merge3([_ | _] = l1, [h2 | t2], [h3 | t3]) do
    :lists.reverse(merge3_1(l1, [], h2, t2, h3, t3), [])
  end

  def merge3([_ | _] = l1, [_ | _] = l2, []) do
    merge(l1, l2)
  end

  def merge3([_ | _] = l1, [], [_ | _] = l3) do
    merge(l1, l3)
  end

  def merge3([_ | _] = l1, [], []) do
    l1
  end

  def merge3([], [_ | _] = l2, [_ | _] = l3) do
    merge(l2, l3)
  end

  def merge3([], [_ | _] = l2, []) do
    l2
  end

  def merge3([], [], [_ | _] = l3) do
    l3
  end
  ...
  def rmerge3([_ | _] = l1, [h2 | t2], [h3 | t3]) do
    :lists.reverse(rmerge3_1(l1, [], h2, t2, h3, t3), [])
  end

  def rmerge3([_ | _] = l1, [_ | _] = l2, []) do
    rmerge(l1, l2)
  end

  def rmerge3([_ | _] = l1, [], [_ | _] = l3) do
    rmerge(l1, l3)
  end

  def rmerge3([_ | _] = l1, [], []) do
    l1
  end

  def rmerge3([], [_ | _] = l2, [_ | _] = l3) do
    rmerge(l2, l3)
  end

  def rmerge3([], [_ | _] = l2, []) do
    l2
  end

  def rmerge3([], [], [_ | _] = l3) do
    l3
  end
  ...
  def rmerge([_ | _] = t1, [h2 | t2]) do
    :lists.reverse(rmerge2_1(t1, h2, t2, []), [])
  end

  def rmerge([_ | _] = l1, []) do
    l1
  end

  def rmerge([], [_ | _] = l2) do
    l2
  end
  ...
  def flatten(list, tail) when (is_list(list) and
                             is_list(tail)) do
  ...
  defp keydelete3(key, n, [h | t]) when :erlang.element(n,
                                                  h) == key do
    t
  end
  ...
  def keyreplace(k, n, l, new) when (is_integer(n) and n > 0 and
                               is_tuple(new)) do
  ...
  defp keytake(key, n, [h | t], l) when :erlang.element(n,
                                                     h) == key do
  ...
  def keystore(k, n, l, new) when (is_integer(n) and n > 0 and
                               is_tuple(new)) do
    keystore2(k, n, l, new)
  end

  defp keystore2(key, n, [h | t], new) when :erlang.element(n,
                                                       h) == key do
    [new | t]
  end
  ...
  def keymerge(index, l1, l2) when (is_integer(index) and
                                index > 0) do
    keymerge_1(index, l1, l2)
  end

  defp keymerge_1(index, [_ | _] = t1, [h2 | t2]) do
    e2 = :erlang.element(index, h2)
    m = keymerge2_1(index, t1, e2, h2, t2, [])
    :lists.reverse(m, [])
  end

  defp keymerge_1(_Index, [_ | _] = l1, []) do
    l1
  end

  defp keymerge_1(_Index, [], [_ | _] = l2) do
    l2
  end

  defp keymerge_1(_Index, [], []) do
    []
  end

  def rkeymerge(index, l1, l2) when (is_integer(index) and
                                index > 0) do
    rkeymerge_1(index, l1, l2)
  end

  defp rkeymerge_1(index, [_ | _] = t1, [h2 | t2]) do
    e2 = :erlang.element(index, h2)
    m = rkeymerge2_1(index, t1, e2, h2, t2, [])
    :lists.reverse(m, [])
  end

  defp rkeymerge_1(_Index, [_ | _] = l1, []) do
    l1
  end

  defp rkeymerge_1(_Index, [], [_ | _] = l2) do
    l2
  end

  defp rkeymerge_1(_Index, [], []) do
    []
  end
  ...
  def ukeymerge(index, l1, l2) when (is_integer(index) and
                                index > 0) do
    ukeymerge_1(index, l1, l2)
  end

  defp ukeymerge_1(index, [h1 | t1], [_ | _] = t2) do
    e1 = :erlang.element(index, h1)
    m = ukeymerge2_2(index, t1, e1, h1, t2, [])
    :lists.reverse(m, [])
  end

  defp ukeymerge_1(_Index, [_ | _] = l1, []) do
    l1
  end

  defp ukeymerge_1(_Index, [], [_ | _] = l2) do
    l2
  end

  defp ukeymerge_1(_Index, [], []) do
    []
  end

  def rukeymerge(index, l1, l2) when (is_integer(index) and
                                index > 0) do
    rukeymerge_1(index, l1, l2)
  end

  defp rukeymerge_1(index, [_ | _] = t1, [h2 | t2]) do
    e2 = :erlang.element(index, h2)
    m = rukeymerge2_1(index, t1, e2, t2, [], h2)
    :lists.reverse(m, [])
  end

  defp rukeymerge_1(_Index, [_ | _] = l1, []) do
    l1
  end

  defp rukeymerge_1(_Index, [], [_ | _] = l2) do
    l2
  end

  defp rukeymerge_1(_Index, [], []) do
    []
  end

  def keymap(fun, index, [tup | tail]) do
    [:erlang.setelement(index, tup,
                          fun.(:erlang.element(index, tup))) |
         keymap(fun, index, tail)]
  end

  def keymap(fun, index, []) when (is_integer(index) and
                                 index >= 1 and is_function(fun, 1)) do
    []
  end

  def enumerate(list1) do
    enumerate(1, 1, list1)
  end

  def enumerate(index, list1) do
    enumerate(index, 1, list1)
  end

  def enumerate(index, step, list1) when (is_integer(index) and
                                     is_integer(step)) do
    enumerate_1(index, step, list1)
  end

  defp enumerate_1(index, step, [h | t]) do
  ...
  def merge(fun, l1, l2) when is_function(fun, 2) do
    merge_1(fun, l1, l2)
  end

  defp merge_1(fun, [_ | _] = t1, [h2 | t2]) do
    :lists.reverse(fmerge2_1(t1, h2, fun, t2, []), [])
  end

  defp merge_1(_Fun, [_ | _] = l1, []) do
    l1
  end

  defp merge_1(_Fun, [], [_ | _] = l2) do
    l2
  end

  defp merge_1(_Fun, [], []) do
    []
  end

  def rmerge(fun, l1, l2) when is_function(fun, 2) do
    rmerge_1(fun, l1, l2)
  end

  defp rmerge_1(fun, [_ | _] = t1, [h2 | t2]) do
    :lists.reverse(rfmerge2_1(t1, h2, fun, t2, []), [])
  end

  defp rmerge_1(_Fun, [_ | _] = l1, []) do
    l1
  end

  defp rmerge_1(_Fun, [], [_ | _] = l2) do
    l2
  end

  defp rmerge_1(_Fun, [], []) do
    []
  end
  ...
  def umerge(fun, l1, l2) when is_function(fun, 2) do
    umerge_1(fun, l1, l2)
  end

  defp umerge_1(fun, [h1 | t1], [_ | _] = t2) do
    :lists.reverse(ufmerge2_2(h1, t1, fun, t2, []), [])
  end

  defp umerge_1(_Fun, [_ | _] = l1, []) do
    l1
  end

  defp umerge_1(_Fun, [], [_ | _] = l2) do
    l2
  end

  defp umerge_1(_Fun, [], []) do
    []
  end

  def rumerge(fun, l1, l2) when is_function(fun, 2) do
    rumerge_1(fun, l1, l2)
  end

  defp rumerge_1(fun, [_ | _] = t1, [h2 | t2]) do
    :lists.reverse(rufmerge2_1(t1, h2, fun, t2, []), [])
  end

  defp rumerge_1(_Fun, [_ | _] = l1, []) do
    l1
  end

  defp rumerge_1(_Fun, [], [_ | _] = l2) do
    l2
  end

  defp rumerge_1(_Fun, [], []) do
    []
  end
  ...
  def umerge3([_ | _] = l1, [h2 | t2], [h3 | t3]) do
    :lists.reverse(umerge3_1(l1, [h2 | h3], t2, h2, [], t3,
                               h3),
                     [])
  end

  def umerge3([_ | _] = l1, [_ | _] = l2, []) do
    umerge(l1, l2)
  end

  def umerge3([_ | _] = l1, [], [_ | _] = l3) do
    umerge(l1, l3)
  end

  def umerge3([_ | _] = l1, [], []) do
    l1
  end

  def umerge3([], [_ | _] = l2, [_ | _] = l3) do
    umerge(l2, l3)
  end

  def umerge3([], [_ | _] = l2, []) do
    l2
  end

  def umerge3([], [], [_ | _] = l3) do
    l3
  end
  ...
  def rumerge3([_ | _] = l1, [h2 | t2], [h3 | t3]) do
    :lists.reverse(rumerge3_1(l1, t2, h2, [], t3, h3), [])
  end

  def rumerge3([_ | _] = l1, [_ | _] = l2, []) do
    rumerge(l1, l2)
  end

  def rumerge3([_ | _] = l1, [], [_ | _] = l3) do
    rumerge(l1, l3)
  end

  def rumerge3([_ | _] = l1, [], []) do
    l1
  end

  def rumerge3([], [_ | _] = l2, [_ | _] = l3) do
    rumerge(l2, l3)
  end

  def rumerge3([], [_ | _] = l2, []) do
    l2
  end

  def rumerge3([], [], [_ | _] = l3) do
    l3
  end
  ...
  def umerge([h1 | t1], [_ | _] = t2) do
    :lists.reverse(umerge2_2(t1, t2, [], h1), [])
  end

  def umerge([_ | _] = l1, []) do
    l1
  end

  def umerge([], [_ | _] = l2) do
    l2
  end
  ...
  def rumerge([_ | _] = t1, [h2 | t2]) do
    :lists.reverse(rumerge2_1(t1, t2, [], h2), [])
  end

  def rumerge([_ | _] = l1, []) do
    l1
  end

  def rumerge([], [_ | _] = l2) do
    l2
  end
  ...
  def all(pred, list) when is_function(pred, 1) do
    case (list) do
      [hd | tail] ->
        case (pred.(hd)) do
          true ->
  ...
  def any(pred, list) when is_function(pred, 1) do
    case (list) do
      [hd | tail] ->
        case (pred.(hd)) do
          true ->
  ...
  def map(f, list) when is_function(f, 1) do
    case (list) do
      [hd | tail] ->
        [f.(hd) | map_1(f, tail)]
      [] ->
  ...
  def flatmap(f, list) when is_function(f, 1) do
    flatmap_1(f, list)
  end

  defp flatmap_1(f, [hd | tail]) do
  ...
  def foldl(f, accu, list) when is_function(f, 2) do
    case (list) do
      [hd | tail] ->
  ...
  defp foldl_1(f, accu, [hd | tail]) do
    foldl_1(f, f.(hd, accu), tail)
  end

  defp foldl_1(_F, accu, []) do
    accu
  end

  def foldr(f, accu, list) when is_function(f, 2) do
    foldr_1(f, accu, list)
  end

  defp foldr_1(f, accu, [hd | tail]) do
    f.(hd, foldr_1(f, accu, tail))
  end

  defp foldr_1(_F, accu, []) do
    accu
  end
  ...
  def partition(pred, l) when is_function(pred, 1) do
    partition_1(pred, l, [], [])
  end
  ...
  defp partition_1(pred, [h | t], as, bs) do
    case (pred.(h)) do
      true ->
        partition_1(pred, t, [h | as], bs)
      false ->
        partition_1(pred, t, as, [h | bs])
    end
  end

  defp partition_1(_Pred, [], as, bs) do
    {reverse(as), reverse(bs)}
  end

  def filtermap(f, list) when is_function(f, 1) do
    filtermap_1(f, list)
  end
  ...
  def foreach(f, list) when is_function(f, 1) do
    foreach_1(f, list)
  end
  ...
  defp foreach_1(_F, []) do
    :ok
  end

  def mapfoldl(f, accu, list) when is_function(f, 2) do
  ...
  defp mapfoldl_1(_F, accu, []) do
    {[], accu}
  end

  def mapfoldr(f, accu, list) when is_function(f, 2) do
  ...
  defp mapfoldr_1(_F, accu, []) do
    {[], accu}
  end

  def takewhile(pred, list) when is_function(pred, 1) do
  ...
  def dropwhile(pred, list) when is_function(pred, 1) do
  ...
  def search(pred, list) when is_function(pred, 1) do
    search_1(pred, list)
  end
  ...
  defp splitwith_1(pred, [hd | tail], taken) do
    case (pred.(hd)) do
      true ->
        splitwith_1(pred, tail, [hd | taken])
      false ->
        {reverse(taken), [hd | tail]}
    end
  end

  defp splitwith_1(_Pred, [], taken) do
    {reverse(taken), []}
  end

  def split(n, list) when (is_integer(n) and n >= 0 and
                          is_list(list)) do
  ...
  defp mergel([[_ | _] = l], acc) do
    rmergel([:lists.reverse(l, []) | acc], [])
  end

  defp mergel([[_ | _] = a, [] | l], acc) do
    mergel([a | l], acc)
  end

  defp mergel([[_ | _] = a, [_ | _] = b, [] | l], acc) do
    mergel([a, b | l], acc)
  end

  defp mergel([[_ | _] = t1, [h2 | t2], [h3 | t3] | l],
            acc) do
    mergel(l, [merge3_1(t1, [], h2, t2, h3, t3) | acc])
  end

  defp mergel([[_ | _] = t1, [h2 | t2]], acc) do
  ...
  defp umergel([[_ | _] = a, [] | l], acc, o) do
    umergel([a | l], acc, o)
  end

  defp umergel([[_ | _] = a, [_ | _] = b, [] | l], acc, o) do
    umergel([a, b | l], acc, o)
  end

  defp umergel([[_ | _] = t1, [h2 | t2], [h3 | t3] | l], acc,
            :asc) do
    umergel(l,
  ...
  defp umergel([[h3 | t3], [h2 | t2], [_ | _] = t1 | l], acc,
            :desc) do
    umergel(l,
              [umerge3_1(t1, [h2 | h3], t2, h2, [], t3, h3) | acc],
              :desc)
  ...
  defp umergel([[h1 | t1], [_ | _] = t2 | l], acc, :asc) do
    umergel(l, [umerge2_2(t1, t2, [], h1) | acc], :asc)
  end

  defp umergel([[_ | _] = t2, [h1 | t1] | l], acc, :desc) do
    umergel(l, [umerge2_2(t1, t2, [], h1) | acc], :desc)
  end

  defp rumergel([[h3 | t3], [h2 | t2], t1 | l], acc, :asc) do
    rumergel(l, [rumerge3_1(t1, t2, h2, [], t3, h3) | acc],
               :asc)
  end
  ...
  defp keymergel(i, [t1, [h2 | t2], [h3 | t3] | l], acc, o)
      when o == :asc do
    m = keymerge3_1(i, t1, [], o, :erlang.element(i, h2),
                      h2, t2, :erlang.element(i, h3), h3, t3)
  ...
  defp rkeymergel(i, [[h3 | t3], [h2 | t2], t1 | l], acc, o)
      when o == :asc do
    m = rkeymerge3_1(i, t1, [], o, :erlang.element(i, h2),
                       h2, t2, :erlang.element(i, h3), h3, t3)
  ...
  defp keymerge3_1(i, [h1 | t1], m, d, e2, h2, t2, e3, h3, t3) do
    case (:erlang.element(i, h1)) do
      e1 when e1 <= e2 ->
        keymerge3_12(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m,
                       d)
      e1 ->
        keymerge3_21(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m,
                       t2)
    end
  end
  ...
  defp keymerge3_12(i, e1, h1, t1, e2, h2, t2, _E3, h3, t3, m,
            _D) do
  ...
  defp keymerge3_21(i, e1, h1, t1, e2, h2, t2, _E3, h3, t3, m,
            _D) do
  ...
  defp rkeymerge3_1(i, [h1 | t1], m, d, e2, h2, t2, e3, h3, t3) do
    case (:erlang.element(i, h1)) do
      e1 when e1 <= e2 ->
        rkeymerge3_12(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m,
                        t2)
      e1 ->
        rkeymerge3_21(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m,
                        d)
    end
  end
  ...
  defp rkeymerge3_12_3(i, e1, h1, t1, e2, h2, t2, [h3 | t3], m) do
    case (:erlang.element(i, h3)) do
      e3 when e2 <= e3 ->
        rkeymerge3_12_3(i, e1, h1, t1, e2, h2, t2, t3, [h3 | m])
      e3 ->
        rkeymerge3_2(i, e1, h1, t1, t2, [h2 | m], t2, e3, h3,
                       t3)
    end
  end
  ...
  defp rkeymerge3_21_3(i, e1, h1, t1, e2, h2, t2, [h3 | t3], m) do
    case (:erlang.element(i, h3)) do
      e3 when e1 <= e3 ->
        rkeymerge3_21_3(i, e1, h1, t1, e2, h2, t2, t3, [h3 | m])
      e3 ->
        rkeymerge3_1(i, t1, [h1 | m], t1, e2, h2, t2, e3, h3,
                       t3)
    end
  end
  ...
  defp ukeymergel(i, [t1, [h2 | t2], [h3 | t3] | l], acc) do
    m = ukeymerge3_1(i, t1, acc, [h2 | h3],
                       :erlang.element(i, h2), h2, t2, [],
                       :erlang.element(i, h3), h3, t3)
  ...
  defp rukeymergel(i, [[h2 | t2], t1 | l], acc) do
    rukeymergel(i, l,
                  [rukeymerge2_1(i, t1, :erlang.element(i, h2), t2, [],
                                   h2) |
                       acc])
  end
  ...
  defp ukeymerge3_1(i, [h1 | t1], d, hdM, e2, h2, t2, m, e3, h3,
            t3) do
    case (:erlang.element(i, h1)) do
      e1 when e1 <= e2 ->
        ukeymerge3_12(i, e1, t1, h1, e2, h2, t2, e3, h3, t3, m,
                        hdM, d)
      e1 when e2 == hdM ->
        ukeymerge3_2(i, e1, t1, h1, t2, hdM, t2, m, e3, h3, t3)
      e1 ->
        ukeymerge3_21(i, e1, t1, h1, e2, h2, t2, e3, h3, t3, m,
                        hdM, t2)
    end
  end
  ...
  defp ukeymerge3_2(i, e1, t1, h1, [h2 | t2], hdM, d, m, e3, h3,
            t3) do
    case (:erlang.element(i, h2)) do
      e2 when e1 <= e2 ->
        ukeymerge3_12(i, e1, t1, h1, e2, h2, t2, e3, h3, t3, m,
  ...
  defp ukeymerge3_12(i, e1, t1, h1, e2, h2, t2, e3, h3, t3, m, _HdM,
            d)
      when e1 <= e3 do
    ukeymerge3_1(i, t1, d, e1, e2, h2, t2, [h1 | m], e3, h3,
                   t3)
  end

  defp ukeymerge3_12(i, e1, t1, h1, e2, h2, t2, e3, _H3, t3, m, hdM,
            _D)
      when e3 == hdM do
    ukeymerge3_12_3(i, e1, t1, h1, e2, h2, t2, m, t3)
  end

  defp ukeymerge3_12(i, e1, t1, h1, e2, h2, t2, _E3, h3, t3, m, _HdM,
            _D) do
  ...
  defp ukeymerge3_12_3(i, e1, t1, h1, e2, h2, t2, m, [h3 | t3]) do
    case (:erlang.element(i, h3)) do
      e3 when e1 <= e3 ->
        ukeymerge3_1(i, t1, t1, e1, e2, h2, t2, [h1 | m], e3,
                       h3, t3)
      _E3 ->
        ukeymerge3_12_3(i, e1, t1, h1, e2, h2, t2, [h3 | m], t3)
    end
  ...
  defp ukeymerge3_21(i, e1, t1, h1, e2, h2, t2, e3, h3, t3, m, _HdM,
            d)
      when e2 <= e3 do
    ukeymerge3_2(i, e1, t1, h1, t2, e2, d, [h2 | m], e3, h3,
                   t3)
  end

  defp ukeymerge3_21(i, e1, t1, h1, e2, h2, t2, e3, _H3, t3, m, hdM,
            _D)
      when e3 == hdM do
    ukeymerge3_21_3(i, e1, t1, h1, e2, h2, t2, m, t3)
  end

  defp ukeymerge3_21(i, e1, t1, h1, e2, h2, t2, _E3, h3, t3, m, _HdM,
            _D) do
  ...
  defp ukeymerge3_21_3(i, e1, t1, h1, e2, h2, t2, m, [h3 | t3]) do
    case (:erlang.element(i, h3)) do
      e3 when e2 <= e3 ->
        ukeymerge3_2(i, e1, t1, h1, t2, e2, t2, [h2 | m], e3,
                       h3, t3)
      _E3 ->
        ukeymerge3_21_3(i, e1, t1, h1, e2, h2, t2, [h3 | m], t3)
    end
  ...
  defp rukeymerge3_1(i, [h1 | t1], d1, d2, e2, h2, t2, m, e3, h3,
            t3) do
    case (:erlang.element(i, h1)) do
      e1 when e1 <= e2 ->
        rukeymerge3_12a(i, e1, h1, t1, e2, h2, t2, e3, h3, t3,
                          m)
      e1 ->
        rukeymerge3_21a(i, e1, h1, t1, e2, h2, t2, e3, h3, t3,
                          m, d1, d2)
    end
  end
  ...
  defp rukeymerge3_12a(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m)
      when e2 <= e3 do
    rukeymerge3_12_3(i, e1, h1, t1, e2, h2, t2, m, e3, h3,
                       t3)
  end
  ...
  defp rukeymerge3_21a(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m, _D1,
            _D2)
      when e1 <= e3 do
    rukeymerge3_21_3(i, e1, h1, t1, e2, h2, t2, m, e3, h3,
                       t3)
  end

  defp rukeymerge3_21a(i, _E1, h1, t1, e2, h2, t2, e3, h3, t3, m, d1,
            d2) do
    rukeymerge3_1(i, t1, d1, d2, e2, h2, t2, [h1 | m], e3,
                    h3, t3)
  end

  defp rukeymerge3_2(i, e1, h1, t1, [h2 | t2], h2M, e2M, m, e3, h3,
            t3) do
    case (:erlang.element(i, h2)) do
      e2 when e1 <= e2 ->
        rukeymerge3_12b(i, e1, h1, t1, e2, h2, t2, e3, h3, t3,
                          m, h2M)
      e2 when e1 == e2M ->
        rukeymerge3_1(i, t1, h1, t1, e2, h2, t2, [h1 | m], e3,
                        h3, t3)
      e2 ->
        rukeymerge3_21b(i, e1, h1, t1, e2, h2, t2, e3, h3, t3,
                          m, h2M)
    end
  end
  ...
  defp rukeymerge3_12b(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m, h2M)
      when e2 <= e3 do
    rukeymerge3_12_3(i, e1, h1, t1, e2, h2, t2, [h2M | m],
                       e3, h3, t3)
  end

  defp rukeymerge3_12b(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m,
            h2M) do
    rukeymerge3_2(i, e1, h1, t1, t2, h2, e2, [h2M | m], e3,
                    h3, t3)
  end

  defp rukeymerge3_21b(i, e1, h1, t1, e2, h2, t2, e3, h3, t3, m, h2M)
      when e1 <= e3 do
    rukeymerge3_21_3(i, e1, h1, t1, e2, h2, t2, [h2M | m],
                       e3, h3, t3)
  end

  defp rukeymerge3_21b(i, _E1, h1, t1, e2, h2, t2, e3, h3, t3, m,
            h2M) do
    rukeymerge3_1(i, t1, h1, t1, e2, h2, t2, [h1, h2M | m],
                    e3, h3, t3)
  end

  defp rukeymerge3_12_3(i, e1, h1, t1, e2, h2, t2, m, e3M, h3M,
            [h3 | t3]) do
    case (:erlang.element(i, h3)) do
      e3 when e2 <= e3 ->
        rukeymerge3_12_3(i, e1, h1, t1, e2, h2, t2, [h3M | m],
                           e3, h3, t3)
      e3 when e2 == e3M ->
        rukeymerge3_2(i, e1, h1, t1, t2, h2, e2, m, e3, h3, t3)
      e3 ->
        rukeymerge3_2(i, e1, h1, t1, t2, h2, e2, [h3M | m], e3,
                        h3, t3)
    end
  end
  ...
  defp rukeymerge3_21_3(i, e1, h1, t1, e2, h2, t2, m, e3M, h3M,
            [h3 | t3]) do
    case (:erlang.element(i, h3)) do
      e3 when e1 <= e3 ->
        rukeymerge3_21_3(i, e1, h1, t1, e2, h2, t2, [h3M | m],
                           e3, h3, t3)
      e3 when e1 == e3M ->
        rukeymerge3_1(i, t1, h1, t1, e2, h2, t2, [h1 | m], e3,
                        h3, t3)
      e3 ->
        rukeymerge3_1(i, t1, h1, t1, e2, h2, t2, [h1, h3M | m],
                        e3, h3, t3)
    end
  end
  ...
  defp rfmergel([[h2 | t2], t1 | l], acc, fun, :asc) do
    rfmergel(l, [rfmerge2_1(t1, h2, fun, t2, []) | acc],
               fun, :asc)
  end
  ...
  defp ufmergel([[h1 | t1], t2 | l], acc, fun) do
    ufmergel(l, [ufmerge2_2(h1, t1, fun, t2, []) | acc],
               fun)
  end
  ...
  defp rufmergel([[h2 | t2], t1 | l], acc, fun) do
    rufmergel(l, [rufmerge2_1(t1, h2, fun, t2, []) | acc],
                fun)
  end
  ...
  defp uniq_1([x | xs], m) do
    case (:erlang.is_map_key(x, m)) do
      true ->
        uniq_1(xs, m)
      false ->
        [x | uniq_1(xs, Map.put(m, x, true))]
    end
  end
  ...
  def uniq(f, l) when is_function(f, 1) do
    uniq_2(l, f, %{})
  end

  defp uniq_2([x | xs], f, m) do
    key = f.(x)
    case (:erlang.is_map_key(key, m)) do
      true ->
        uniq_2(xs, f, m)
      false ->
        [x | uniq_2(xs, f, Map.put(m, key, true))]
    end
  end
