defmodule :m_qlc do
  ...
  def format_error(:not_a_query_list_comprehension) do
    :io_lib.format(~c"argument is not a query list comprehension", [])
  end

  def format_error({:used_generator_variable, v}) do
    :io_lib.format(~c"generated variable ~w must not be used in list expression", [v])
  end

  def format_error(:binary_generator) do
    :io_lib.format(~c"cannot handle binary generators", [])
  end

  def format_error(:too_complex_join) do
    :io_lib.format(~c"cannot handle join of three or more generators efficiently", [])
  end

  def format_error(:too_many_joins) do
    :io_lib.format(~c"cannot handle more than one join efficiently", [])
  end

  def format_error(:nomatch_pattern) do
    :io_lib.format(~c"pattern cannot possibly match", [])
  end

  def format_error(:nomatch_filter) do
    :io_lib.format(~c"filter evaluates to 'false'", [])
  end
  ...
  def format_error({:bad_object, fileName}) do
    :io_lib.format(~c"the temporary file \"~ts\" holding answers is corrupt", [fileName])
  end

  def format_error(:bad_object) do
    :io_lib.format(~c"the keys could not be extracted from some term", [])
  end
  ...
  def format_error({:premature_eof, fileName}) do
    :io_lib.format(~c"\"~ts\": end-of-file was encountered inside some binary term", [fileName])
  end

  def format_error({:tmpdir_usage, why}) do
    :io_lib.format(~c"temporary file was needed for ~w~n", [why])
  end
  ...
  defp wait_for_request(parent, monRef, post) do
    receive do
      ...
      other ->
        :error_logger.error_msg(~c"The qlc cursor ~w received an unexpected message:\n~tp\n", [
          self(),
          other
        ])
  ...
  defp abstract1({:table, tableDesc}, _NElements, _Depth, _A) do
    case :io_lib.deep_char_list(tableDesc) do
      true ->
        {:ok, tokens, _} = :erl_scan.string(:lists.flatten(tableDesc ++ ~c"."), 1, [:text])
  ...
  defp flatten_abstr(
         {:call, a1, {:remote, a2, {:atom, a3, :qlc}, {:atom, a4, :q}}, [lC0 | os]},
         vN0,
         vars,
         body0
       ) do
    ...
    f = fn
      {:generate, annoG, p, lE0}, {vN1, body1} ->
    ...
    var = {:var, a1, v}
    qLC = {:call, a1, {:remote, a2, {:atom, a3, :qlc}, {:atom, a4, :q}}, [lC | os]}
  ...
  defp le_info(
         r_prepared(qh: r_simple_qlc(le: lE, p: p, anno: anno, optz: optz)),
         infOpt
       ) do
  ...
  defp join_info(join, qInfo, qdata, code) do
    ...
    jFil =
      :erlang.term_to_binary(
        {:op, anno, jOp, {:call, anno, {:atom, anno, :element}, [{:integer, anno, c1}, g1]},
         {:call, anno, {:atom, anno, :element}, [{:integer, anno, c2}, g2]}}
      )
  ...
  defp prep_simple_qlc(pVar, anno, lE, opt) do
    ...
    qLC = r_simple_qlc(p: pVar, le: lE, anno: anno, init_value: :not_a_list, optz: optz)
  ...
  defp sort_tmpdir(s, r_qlc_opt(tmpdir: ~c"")) do
    s
  end
  ...
  defp sort_options_global_tmp(s, ~c"") do
    s
  end
  ...
  defp maybe_error_logger(name, why) do
    ...
    apply(:error_logger, name, [
      ~c"qlc: temporary file was needed for ~w\n~ts\n",
      [why, :lists.flatten(x)]
    ])
  end
