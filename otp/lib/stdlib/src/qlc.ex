defmodule :m_qlc do
  ...
  Record.defrecord(:r_qlc_table, :qlc_table, trav_fun: :undefined,
                                     trav_MS: :undefined, pre_fun: :undefined,
                                     post_fun: :undefined, info_fun: :undefined,
                                     format_fun: :undefined,
                                     lookup_fun: :undefined,
                                     parent_fun: :undefined,
                                     key_equality: :undefined,
                                     lu_vals: :undefined, ms: :no_match_spec)
  Record.defrecord(:r_qlc_sort, :qlc_sort, h: :undefined,
                                    keypos: :undefined, unique: :undefined,
                                    compressed: :undefined, order: :undefined,
                                    fs_opts: :undefined, tmpdir_usage: :allowed,
                                    tmpdir: :undefined)
  Record.defrecord(:r_qlc_lc, :qlc_lc, lc: :undefined,
                                  opt: :undefined)
  Record.defrecord(:r_qlc_list, :qlc_list, l: :undefined,
                                    ms: :no_match_spec)
  Record.defrecord(:r_qlc_join, :qlc_join, kind: :undefined,
                                    opt: :undefined, h1: :undefined,
                                    q1: :undefined, c1: :undefined,
                                    h2: :undefined, q2: :undefined,
                                    c2: :undefined)
  Record.defrecord(:r_qlc_cursor, :qlc_cursor, c: :undefined)
  Record.defrecord(:r_qlc_opt, :qlc_opt, unique: false,
                                   cache: false, max_lookup: - 1, join: :any,
                                   tmpdir: '', lookup: :any,
                                   max_list: 512 * 1024, tmpdir_usage: :allowed)
  ...
  def eval(qH, options) do
    case ({options(options,
      ...
      {[gUnique, gCache, tmpDir, maxList, tmpUsage],
         handle} ->
        try do
          prep = prepare_qlc(handle, [], gUnique, gCache, tmpDir,
                               maxList, tmpUsage)
    ...
    case ({options(options,
      ...
      {[gUnique, gCache, tmpDir, maxList, tmpUsage],
        ...
        try do
          ...
          case (setup_qlc(prep, r_setup(parent: self()))) do
            {objs, post, _LocalPost} when is_function(objs) or
                                            is_list(objs)
                                          ->
              try do
  ...
  def format_error({location, mod, reason})
      when is_integer(location) or
             tuple_size(location) === 2 do
  ...
  def info(qH, options) do
    case ({options(options,
      ...
      {b1, b2} when b1 === :badarg or b2 === :badarg ->
        :erlang.error(:badarg, [qH, options])
      {[gUnique, gCache, flat, format, nElements, depth,
                                                      tmpDir, maxList,
                                                                  tmpUsage],
         h} ->
        try do
          prep = prepare_qlc(h, [], gUnique, gCache, tmpDir,
                               maxList, tmpUsage)
          ...
          case (format) do
            ...
            :string ->
              hook = fn {:special, _Anno, string}, _I, _P, _F ->
                          string
                     end
  ...
  def keysort(keyPos, qH, options) do
    case ({is_keypos(keyPos),
             options(options,
                       [:tmpdir, :order, :unique, :compressed, :size,
                                                                   :no_files]),
             get_handle(qH)}) do
      {true, [tmpDir, order, unique, compressed | _], h}
          when h !== :badarg ->
        r_qlc_handle(h: r_qlc_sort(h: h, keypos: {:keysort, keyPos}, unique: unique,
                 compressed: compressed, order: order,
                 fs_opts: listify(options), tmpdir: tmpDir))
      _ ->
  ...
  def q(r_qlc_lc() = qLC_lc, options) do
    case (options(options,
                    [:unique, :cache, :max_lookup, :join, :lookup])) do
      [unique, cache, max, join, lookup] ->
        opt = r_qlc_opt(unique: unique, cache: cache, max_lookup: max,
                  join: join, lookup: lookup)
  ...
  def sort(qH, options) do
    case ({options(options,
      ...
      {[tD, order, unique, compressed | _], h} ->
        r_qlc_handle(h: r_qlc_sort(h: h, keypos: :sort, unique: unique,
                 compressed: compressed, order: order,
                 fs_opts: listify(options), tmpdir: tD))
    end
  end
    ...
    case (options(options,
      ...
      [unique, cache, maxLookup, join, lookup] ->
        case (:erl_scan.string(str, 1, [:text])) do
          {:ok, tokens, _} ->
            scanRes = (case (:erl_eval.extended_parse_exprs(tokens)) do
                         {:ok, [expr0]} ->
                           {:ok, expr0}
                         {:ok, _ExprList} ->
                           :erlang.error(:badarg, [str, options, bindings])
            ...
            case (scanRes) do
              {:ok, expr} ->
                case (:qlc_pt.transform_expression(expr, bindings)) do
                  {:ok, {:call, _, _QlcQ, handle}} ->
                    {:value, qLC_lc, _} = :erl_eval.exprs(handle, bindings)
                    o = r_qlc_opt(unique: unique, cache: cache,
                            max_lookup: maxLookup, join: join, lookup: lookup)
  ...
  def table(traverseFun, options)
    ...
    case ({is_function(traverseFun, 0),
             isFun1 = is_function(traverseFun, 1)}) do
      {false, false} ->
      ...
      _ ->
        case (options(options,
          ...
          [preFun, postFun, infoFun, formatFun, lookupFun,
            ...
            t = r_qlc_table(trav_fun: traverseFun, pre_fun: preFun,
                    post_fun: postFun, info_fun: infoFun,
                    parent_fun: parentFun, trav_MS: isFun1,
                    format_fun: formatFun, lookup_fun: lookupFun,
  ...
  defp options(options0, [key | keys], l)
    ...
    options = (case (:lists.member(key, options0)) do
                 true ->
    ...
    v = (case (:lists.keyfind(key, 1, options)) do
           ...
           {:join, j = :lookup} ->
             {:ok, j}
           {:lookup, lookUp} when is_boolean(lookUp) or
                                    lookUp === :any
                                  ->
             {:ok, lookUp}
           {:max_list_size, max} when (is_integer(max) and
                                         max >= 0)
                                      ->
             {:ok, max}
           {:tmpdir_usage, tmpUsage} when tmpUsage === :allowed or
           ...
           {:unique, unique} when is_boolean(unique) ->
             {:ok, unique}
           {:cache, cache} when is_boolean(cache) or
                                  cache === :list
                                ->
             {:ok, cache}
           {:cache, :ets} ->
           ...
           {:unique_all, uniqueAll} when is_boolean(uniqueAll) ->
             {:ok, uniqueAll}
           {:cache_all, cacheAll} when is_boolean(cacheAll) or
                                         cacheAll === :list
                                       ->
             {:ok, cacheAll}
           {:cache_all, :ets} ->
           ...
           {:flat, flat} when is_boolean(flat) ->
             {:ok, flat}
           {:format, format} when format === :string or
                                    format === :abstract_code or
                                    format === :debug
                                  ->
             {:ok, format}
           {:n_elements, nElements} when nElements === :infinity or
                                           (is_integer(nElements) and
                                              nElements > 0)
                                         ->
             {:ok, nElements}
           {:depth, depth} when depth === :infinity or
                                  (is_integer(depth) and depth >= 0)
                                ->
             {:ok, depth}
           {:order, order} when is_function(order, 2) or
           ...
           {:size, size} when (is_integer(size) and size > 0) ->
             {:ok, size}
           {:no_files, noFiles} when (is_integer(noFiles) and
                                        noFiles > 1)
                                     ->
             {:ok, noFiles}
           {^key, _} ->
  ...
  defp is_keypos(keypos) when (is_integer(keypos) and
                          keypos > 0) do
    true
  end
  ...
  Record.defrecord(:r_optz, :optz, unique: false, cache: false,
                                join_option: :any, fast_join: :no,
                                opt: :undefined)
  Record.defrecord(:r_qlc, :qlc, lcf: :undefined,
                               codef: :undefined, qdata: :undefined,
                               init_value: :undefined, optz: :undefined)
  Record.defrecord(:r_simple_qlc, :simple_qlc, p: :undefined,
                                      le: :undefined, anno: :undefined,
                                      init_value: :undefined, optz: :undefined)
  Record.defrecord(:r_prepared, :prepared, qh: :undefined,
                                    sorted: :no, sort_info: [], sort_info2: [],
                                    lu_skip_quals: [], join: {[], []},
                                    n_objs: :undefined,
                                    is_unique_objects: false, is_cached: false)
  defp cursor_process(h, gUnique, gCache, tmpDir, spawnOptions,
            maxList, tmpUsage) do
    parent = self()
    ...
    cF = fn () ->
              ...
              {objs, post, _LocalPost} = (try do
                                            prep = prepare_qlc(h, :not_a_list,
                                                                 gUnique,
                                                                 gCache, tmpDir,
                                                                 maxList,
                                                                 tmpUsage)
  ...
  defp abstract_code({:special, anno, string}) do
    {:string, anno, string}
  end
  ...
  defp abstract1({:qlc, e0, qs0, opt}, nElements, depth, a) do
    qs = :lists.map(fn {:generate, p, lE} ->
                         {:generate, a, :erlang.binary_to_term(p),
  ...
  defp abstract1({:table, {m, f, as0}}, _NElements, _Depth, anno)
    ...
    {:call, anno,
       {:remote, anno, {:atom, anno, m}, {:atom, anno, f}}, as}
  end
  ...
  defp abstract1({:table, tableDesc}, _NElements, _Depth, _A) do
    case (:io_lib.deep_char_list(tableDesc)) do
      true ->
        {:ok, tokens,
           _} = :erl_scan.string(:lists.flatten(tableDesc ++ '.'), 1,
                                   [:text])
  ...
  defp abstract1({:append, infos}, nElements, depth, a) do
    as = :lists.foldr(fn info, as0 ->
                           {:cons, a, abstract1(info, nElements, depth, a), as0}
                      end,
                        {nil, a}, infos)
    {:call, a,
       {:remote, a, {:atom, a, :qlc}, {:atom, a, :append}},
       [as]}
  end

  defp abstract1({:sort, info, sortOptions}, nElements, depth,
            a) do
    {:call, a,
  ...
  defp abstract1({:keysort, info, kp, sortOptions}, nElements,
            depth, a) do
    {:call, a,
       {:remote, a, {:atom, a, :qlc}, {:atom, a, :keysort}},
       [abstract_term(kp, 1), abstract1(info, nElements, depth,
                                          a),
                                  abstract_term(sortOptions, 1)]}
  end
  ...
  defp abstract1({:list, l, mS}, nElements, depth, a) do
    {:call, a,
       {:remote, a, {:atom, a, :ets},
          {:atom, a, :match_spec_run}},
       [abstract1(l, nElements, depth, a), {:call, a,
                                              {:remote, a, {:atom, a, :ets},
                                                 {:atom, a,
                                                    :match_spec_compile}},
                                              [abstract_term(depth(mS, depth),
                                                               1)]}]}
  end
  ...
  defp abstract1({:list, l}, nElements, depth, _A) do
    abstract_term(depth(:lists.sublist(l, nElements),
                          depth) ++ :"...",
                    1)
  end
  ...
  defp abstr_list([h | t], anno) do
    {:cons, anno, abstr_term(h, anno), abstr_list(t, anno)}
  end

  defp abstr_list(t, anno) do
    abstr_term(t, anno)
  end

  defp flatten_abstr({:call, a1,
             {:remote, a2, {:atom, a3, :qlc}, {:atom, a4, :q}},
             [lC0 | os]},
            vN0, vars, body0) do
    {:lc, anno, e, qs0} = lC0
    f = fn {:generate, annoG, p, lE0}, {vN1, body1} ->
             {vN2, body2, lE} = flatten_abstr(lE0, vN1, vars, body1)
             {{:generate, annoG, p, lE}, {vN2, body2}}
           fil, vN_Body ->
             {fil, vN_Body}
        end
    {qs, {vN3, body}} = :lists.mapfoldl(f, {vN0, body0},
                                          qs0)
    lC = {:lc, anno, e, qs}
  ...
  defp flatten_abstr(t0, vN0, vars, body0) when is_tuple(t0) do
    {vN, body, l} = flatten_abstr(:erlang.tuple_to_list(t0),
                                    vN0, vars, body0)
  ...
  defp next_loop(pid, l, n) when n !== 0 do
    case (monitor_request(pid, :more)) do
      ...
      {:caught, class, reason, stacktrace} ->
        {:current_stacktrace,
           currentStacktrace} = :erlang.process_info(self(),
                                                       :current_stacktrace)
  ...
  Record.defrecord(:r_join, :join, op: :undefined,
                                q1: :undefined, q2: :undefined, wh1: :undefined,
                                wh2: :undefined, cs_fun: :undefined)
  defp le_info(r_prepared(qh: r_simple_qlc(le: lE, p: p, anno: anno, optz: optz)),
            infOpt) do
    qVar = :erlang.term_to_binary({:var, anno, p})
    {:qlc, qVar, [{:generate, qVar, le_info(lE, infOpt)}],
       opt_info(optz)}
  end

  defp le_info(r_prepared(qh: r_qlc(codef: codeF, qdata: qdata,
                    optz: optz)),
            infOpt) do
    code = codeF.()
    ...
    qualInfo1 = (case (r_optz(optz, :fast_join)) do
                   r_qlc_join() = join ->
                     join_info(join, qualInfo0, qdata, code)
                   :no ->
                     qualInfo0
                 end)
    qualInfo = (for i <- qualInfo1, i !== :skip do
                  i
                end)
    {:qlc, e, qualInfo, opt_info(optz)}
  end

  defp le_info(r_prepared(qh: r_qlc_table(format_fun: formatFun, trav_MS: travMS,
  ...
  defp le_info(r_prepared(qh: r_qlc_sort(h: h, keypos: :sort,
                    fs_opts: sortOptions0, tmpdir: tmpDir)),
            infOpt) do
    sortOptions = sort_options_global_tmp(sortOptions0,
                                            tmpDir)
    {:sort, le_info(h, infOpt), sortOptions}
  end

  defp le_info(r_prepared(qh: r_qlc_sort(h: h, keypos: {:keysort, kp},
                    fs_opts: sortOptions0, tmpdir: tmpDir)),
            infOpt) do
    sortOptions = sort_options_global_tmp(sortOptions0,
                                            tmpDir)
  ...
  defp qual_info([{_QNum, _GoI, - 1, :fil} | qdata], code,
            infOpt) do
    [:skip | qual_info(qdata, code, infOpt)]
  end

  defp qual_info([{qNum, _GoI, _SI, :fil} | qdata], code,
            infOpt) do
    [:erlang.element(qNum + 1, code) | qual_info(qdata,
                                                   code, infOpt)]
  end

  defp qual_info([{_QNum, _GoI, _SI, {:gen, r_join()}} | qdata], code,
            infOpt) do
    [:skip | qual_info(qdata, code, infOpt)]
  end

  defp qual_info([{qNum, _GoI, _SI, {:gen, lE}} | qdata], code,
            infOpt) do
    [{:generate, :erlang.element(qNum + 1, code),
        le_info(lE, infOpt)} |
  ...
  defp join_info(join, qInfo, qdata, code) do
    r_qlc_join(kind: kind, q1: qNum1a, c1: c1, q2: qNum2a, c2: c2,
        opt: opt) = join
    {{jQNum, _, _, _}, rev, qNum1, qNum2, _WH1, _WH2,
       csFun} = find_join_data(qdata, qNum1a, qNum2a)
    {cs1_0, cs2_0, compat} = csFun.()
    [cs1, cs2] = (case (compat) do
                    [] ->
    ...
    {{i1, g1}, {i2, g2}, qInfoL} = (case (kind) do
                                      {:merge, _} ->
                                        {jG1, qInfo1} = join_merge_info(qNum1,
                                                                          qInfo,
                                                                          code,
                                                                          g1_0,
                                                                          cs1)
                                        {jG2, qInfo2} = join_merge_info(qNum2,
                                                                          qInfo,
                                                                          code,
                                                                          g2_0,
                                                                          cs2)
                                      ...
                                      _ when rev ->
                                        {jG2, qInfo2} = join_merge_info(qNum2,
                                                                          ...
                                                                          g2_0,
                                                                          cs2)
                                        {j1, qInfo1} = join_lookup_info(qNum1,
                                                                          qInfo,
                                                                          g1_0)
                                      ...
                                      _ ->
                                        {jG1, qInfo1} = join_merge_info(qNum1,
                                                                          ...
                                                                          g1_0,
                                                                          cs1)
                                        {j2, qInfo2} = join_lookup_info(qNum2,
                                                                          qInfo,
                                                                          g2_0)
    ...
    jOpt = [{:join,
               jOptVal}] ++ opt_info(join_unique_cache(opt))
    jFil = :erlang.term_to_binary({:op, anno, jOp,
                                     {:call, anno, {:atom, anno, :element},
                                        [{:integer, anno, c1}, g1]},
                                     {:call, anno, {:atom, anno, :element},
                                        [{:integer, anno, c2}, g2]}})
    p = :erlang.term_to_binary({:cons, anno, g1, g2})
    jInfo = {:generate, jP,
               {:qlc, p, qInfoL ++ [jFil], jOpt}}
    {before, [^i1 | after__]} = :lists.split(qNum1 - 1,
                                               qInfo)
  ...
  defp join_merge_info(qNum, qInfo, code, g, extraConstants) do
    ...
    p = :erlang.binary_to_term(:erlang.element(qNum + 1,
                                                 code))
    case ({p, extraConstants}) do
      ...
      _ ->
        ...
        tP = :erlang.term_to_binary(g)
        cFs = (for {col, constOps} <- extraConstants do
                 (
                   a = anno0()
                   call = {:call, a, {:atom, a, :element},
                             [{:integer, a, col}, ePV]}
                   f = list2op(for {con, op} <- constOps do
  ...
  defp opt_info(r_optz(unique: unique, cache: cache0,
              join_option: joinOption)) do
    cache = (cond do
               cache0 ->
                 :ets
               true ->
    ...
    (for {t, v} <- [{:cache, cache}, {:unique, unique}],
       ...
       {t, v}
     end) ++ (for {t, v} <- [{:join, joinOption}],
                    v === :nested_loop do
                {t, v}
              end)
  end

  defp prepare_qlc(h, initialValue, gUnique, gCache, tmpDir,
            maxList, tmpUsage) do
    gOpt = r_qlc_opt(unique: gUnique, cache: gCache, tmpdir: tmpDir,
               max_list: maxList, tmpdir_usage: tmpUsage)
  ...
  defp prep_le(r_qlc_lc(lc: lC_fun, opt: r_qlc_opt() = opt0) = h, gOpt) do
    r_qlc_opt(unique: gUnique, cache: gCache, tmpdir: tmpDir,
        max_list: maxList, tmpdir_usage: tmpUsage) = gOpt
    ...
    opt = r_qlc_opt(opt0, unique: unique,  cache: cache, 
  ...
  defp prep_le(r_qlc_table(info_fun: iF) = t, gOpt) do
    ...
    prep = r_prepared(qh: t, sort_info: sortInfo, sorted: sorted,
               is_unique_objects: isUnique)
    opt = (cond do
             (isUnique or not r_qlc_opt(gOpt, :unique) and
  ...
  defp prep_le(r_qlc_append(hl: hL), gOpt) do
    case (:lists.flatmap(fn r_prepared(qh: r_qlc_list(l: [])) ->
                              []
      ...
      [prep] ->
        prep
      prepL ->
        cache = :lists.all(fn r_prepared(is_cached: isC) ->
                                isC !== false
                           end,
                             prepL)
  ...
  defp prep_qlc_lc({:simple_v1, pVar, lE_fun, l}, opt, gOpt, _H) do
    ...
    prep_simple_qlc(pVar, anno(l), eval_le(lE_fun, gOpt),
                      opt)
  end

  defp prep_qlc_lc({:qlc_v1, qFun, codeF, qdata0, qOpt}, opt, gOpt,
            _H) do
    f = fn {_QNum, _GoI, _SI, :fil} = qualData, modGens ->
             {qualData, modGens}
           {_QNum, _GoI, _SI, {:gen, r_join()}} = qualData, modGens ->
             {qualData, modGens}
           {qNum, goI, sI, {:gen, lE_fun}}, modGens0 ->
             prep1 = eval_le(lE_fun, gOpt)
             {prep, modGens} = prep_generator(qNum, prep1, qOpt, opt,
                                                modGens0)
    ...
    case (modGens) do
      ...
      _ ->
        ...
        lU_SkipQuals = :lists.flatmap(fn {qNum, _LookUp, fs,
                                            _Prep} ->
                                           [{qNum, fs}]
                                      end,
                                        modGens)
  ...
  defp prep_generator(qNum, prep0, qOpt, opt, modGens) do
    ...
    mSFs = (case (match_specs(qOpt, qNum)) do
              :undefined ->
                {:no_match_spec, []}
              {_, _} = mSFs0 ->
                mSFs0
  ...
  defp prep_gen(r_qlc_table(lu_vals: luV0, ms: mS0, trav_MS: travMS,
              info_fun: iF, lookup_fun: lU_fun,
              key_equality: keyEquality) = lE0,
            prep0, posFun0, {mS, fs}, opt) do
    posFun = posFun0.(keyEquality)
    {luV, {sTag, skipFils}} = find_const_positions(iF,
                                                     lU_fun, posFun, opt)
    lU = luV !== false
    cond do
      ...
      (travMS and mS !== :no_match_spec) ->
        prep = r_prepared(prep0, qh: r_qlc_table(lE0, ms: mS), 
  ...
  defp prep_gen(r_qlc_list(ms: :no_match_spec) = lE0, prep0, _PosFun,
            {mS, fs}, opt)
  ...
  defp may_create_simple(r_qlc_opt(unique: unique, cache: cache) = opt,
            r_prepared(is_cached: isCached,
                is_unique_objects: isUnique) = prep) do
    cond do
  ...
  defp prep_simple_qlc(pVar, anno, lE, opt) do
    check_join_option(opt)
    r_prepared(is_cached: isCached, sort_info: sortInfo,
        sorted: sorted, is_unique_objects: isUnique) = lE
    ...
    optz = r_optz(unique: :erlang.and(unique, not isUnique),
               cache: cachez, opt: opt)
    qLC = r_simple_qlc(p: pVar, le: lE, anno: anno,
  ...
  defp prep_sort(r_qlc_sort(h: r_prepared(is_unique_objects: isUniqueObjs)) = q,
            gOpt) do
    ...
    r_prepared(qh: s, is_cached: true, sort_info: sortInfo,
        sorted: sorted,
        is_unique_objects: :erlang.or(r_qlc_sort(s, :unique),
                                        isUniqueObjs))
  end

  defp prep_qlc(qFun, codeF, qdata0, qOpt, opt) do
    r_qlc_opt(unique: unique, cache: cache, join: join) = opt
    optz = r_optz(unique: unique, cache: cache,
               join_option: join, opt: opt)
    {qdata, sortInfo} = qlc_sort_info(qdata0, qOpt)
    qLC = r_qlc(lcf: qFun, codef: codeF, qdata: qdata,
              init_value: :not_a_list, optz: optz)
    r_prepared(qh: qLC, sort_info: sortInfo,
        is_unique_objects: unique, is_cached: cache !== false)
  end

  defp qlc_sort_info(qdata0, qOpt) do
    f = fn {_QNum, _GoI, _SI, :fil} = qd, info ->
             {qd, info}
           ...
           {qNum, goI, sI, {:gen, prepLE0}}, info ->
             ...
             i = (for {{c, order}, what} <- r_prepared(prepLE, :sort_info2),
                        what === [],
    ...
    sortInfo0 = (for pos <- constant_columns(qOpt, 0),
                       ord <- orders(:yes) do
                   {{pos, ord}, [:template]}
  ...
  defp sort_info(r_prepared(sort_info: sI, sorted: s) = prep, qNum,
            qOpt) do
    sI1 = (for _ <- [:EFE_DUMMY_GEN], s !== :no,
                 is_integer(sz = size_of_qualifier(qOpt, qNum)), sz > 0,
                 (nConstCols = size_of_constant_prefix(qOpt, qNum)) < sz,
                 c <- [nConstCols + 1], ord <- orders(s) do
             {{c, ord}, []}
           end) ++ (for pos <- constant_columns(qOpt, qNum),
                          ord <- orders(:yes) do
                      {{pos, ord}, []}
                    end) ++ (for {posOrd, _} <- sI do
                               {posOrd, []}
                             end)
    sI2 = :lists.usort(sI1)
  ...
  defp sort_unique(true,
            r_qlc_sort(fs_opts: sortOptions, keypos: :sort) = sort) do
    r_qlc_sort(sort, unique: false, 
              fs_opts: :lists.keydelete(:unique, 1,
  ...
  defp short_list(l) do
    r_prepared(qh: r_qlc_list(l: l), sorted: :yes, is_unique_objects: true,
        is_cached: true)
  end

  defp find_const_positions(iF, lU_fun, {keyEquality, posFun},
    ...
    case (call(iF, :keypos, :undefined, [])) do
      :undefined ->
        ...
        find_const_position_idx(indices, keyEquality, posFun,
                                  max, [])
      keyPos ->
        case (pos_vals(keyPos, keyEquality, posFun.(keyPos),
                         max)) do
          false ->
  ...
  defp find_const_position_idx([i | is], keyEquality, posFun, max, l0) do
    case (pos_vals(i, keyEquality, posFun.(i), max)) do
      false ->
        find_const_position_idx(is, keyEquality, posFun, max,
                                  l0)
  ...
  defp pos_vals(pos, :"==", {:usort_needed, values, skipFils},
            max) do
  ...
  defp pos_vals(pos, :"=:=", {:usort_needed, values, skipFils},
            max) do
    pos_vals_max(pos, :lists.sort(nub(values)), skipFils,
                   max)
  end

  defp pos_vals(pos, _KeyEquality, {:values, values, skipFils},
            max) do
  ...
  defp pos_vals_max(pos, values, skip, max) when max === - 1 or
  ...
  defp pref_join(equal, match, prep, qOpt, r_qlc_opt(join: joinOpt)) do
    jQs = (for {keyEquality, qCsL} <- [{:"==", equal}, {:"=:=",
                                                       match}],
                 qCs <- qCsL do
             {keyEquality, qCs}
           end)
    ixL = (for _ <- [:EFE_DUMMY_GEN],
                 joinOpt === :any or joinOpt === :lookup,
                 {kE, qCs} <- jQs do
             pref_lookup_join(kE, qCs, prep, qOpt)
           end)
    mL = (for _ <- [:EFE_DUMMY_GEN],
                joinOpt === :any or joinOpt === :merge,
                {kE, qCs} <- jQs do
            pref_merge_join(kE, qCs, prep, qOpt)
          end)
    {:lists.usort(:lists.append(ixL)),
       :lists.usort(:lists.append(mL))}
  end

  defp pref_lookup_join(keyEquality, {[{q1, c1}, {q2, c2}], skip}, prep,
            qOpt)
      when (is_integer(c1) and is_integer(c2)) do
    r_prepared(qh: r_qlc(qdata: qData)) = prep
    is1 = lookup_qual_data(qData, q1, keyEquality)
    lu2 = (for iC1 <- is1, iC1 === c1 do
             pref_lookup_join2(q2, c2, q1, c1, skip, qOpt,
                                 keyEquality)
           end)
    is2 = lookup_qual_data(qData, q2, keyEquality)
    lu1 = (for iC2 <- is2, iC2 === c2 do
             pref_lookup_join2(q1, c1, q2, c2, skip, qOpt,
                                 keyEquality)
           end)
    family(lu1 ++ lu2)
  end
  ...
  defp pref_lookup_join(kE, [{_, cs1}, {_, cs2}] = l, prep, qOpt)
    ...
    :lists.append(for qC <- selections_no_skip(l) do
  ...
  defp join_indices(r_prepared(qh: r_qlc_table(info_fun: iF, lookup_fun: lU_fun,
                    key_equality: keyEquality, lu_vals: :undefined)),
            kE)
      when (is_function(lU_fun) and
              kE === keyEquality or kE === :"=:=" and keyEquality === :undefined) do
    kpL = (case (call(iF, :keypos, :undefined, [])) do
             :undefined ->
               []
             kp ->
               [kp]
  ...
  defp pref_lookup_join2(q1, c1, q2, c2, skip, qOpt, keyEquality) do
    templCols = compared_template_columns(qOpt, {q1, c1},
                                            keyEquality)
    {{q1, c1, q2, c2},
       {:lookup_join, templCols, keyEquality, skip}}
  end
  ...
  defp pref_merge_join(kE, {[{q1, c1}, {q2, c2}], skip}, prep, qOpt)
    ...
    merge = pref_merge(kE, q1, c1, q2, c2, skip, sort1,
                         sort2, qOpt)
    family_union(merge)
  end
  ...
  defp pref_merge(kE, q1, c1, q2, c2, skip, sort1, sort2, qOpt) do
    col1 = {q1, c1}
    col2 = {q2, c2}
    doSort = (for {{_QNum, col} = qC, sortL} <- [{col1,
                                                    sort1},
                                                     {col2, sort2}],
                    :lists.keymember({col, :ascending}, 1,
                                       sortL) === false do
                qC
              end)
    j = [{{q1, c1, q2, c2},
            {:merge_join, doSort, kE, skip}}]
  ...
  defp check_lookup_option(r_qlc_opt(lookup: true), false) do
    :erlang.error(:no_lookup_to_carry_out,
                    [{:lookup, true}])
  end
  ...
  defp opt_le(r_prepared(qh: r_simple_qlc(le: lE0, optz: optz0) = qLC) = prep0,
            genNum) do
    case (lE0) do
      r_prepared(qh: r_simple_qlc(p: lE_Pvar, le: lE2, optz: optz2)) ->
        cachez = (case (r_optz(optz2, :cache)) do
                    false ->
                      r_optz(optz0, :cache)
                    cache2 ->
                      cache2
                  end)
        optz = r_optz(optz0, cache: cachez, 
                          unique: :erlang.or(r_optz(optz0, :unique),
                                               r_optz(optz2, :unique)))
        pVar = (cond do
                  lE_Pvar === :SQV ->
                    r_simple_qlc(qLC, :p)
        ...
        prep = r_prepared(prep0, qh: r_simple_qlc(qLC, p: pVar,  le: lE2, 
                                     optz: optz))
        opt_le(prep, genNum)
      _ ->
  ...
  defp opt_le(r_prepared(qh: r_qlc(),
              lu_skip_quals: lU_SkipQuals0) = prep0,
            genNum) do
    ...
    {lU_QNum, join, joinSkipFs,
       doSort} = opt_join(r_prepared(prep0, :join), joinOption, qdata0,
                            opt, lU_SkipQuals0)
    {lU_Skip, lU_SkipQuals} = :lists.partition(fn {qNum,
                                                     _Fs} ->
                                                    qNum === lU_QNum
                                               end,
                                                 lU_SkipQuals0)
    lU_SkipFs = :lists.flatmap(fn {_QNum, fs} ->
                                    fs
                               end,
                                 lU_SkipQuals)
    qdata1 = (cond do
    ...
    qdata2 = skip_lookup_filters(qdata1,
                                   lU_SkipFs ++ joinSkipFs)
    f = fn {qNum, goI, sI, {:gen, r_prepared() = prepLE}}, genNum1 ->
  ...
  defp maybe_sort(lE, qNum, doSort, opt) do
    case (:lists.keyfind(qNum, 1, doSort)) do
      {^qNum, col} ->
        ...
        sortOpts = (for dir <- [tmpDir], dir !== '' do
                      {:tmpdir, dir}
                    end)
        sort = r_qlc_sort(h: lE, keypos: {:keysort, col}, unique: false,
                   compressed: [], order: :ascending, fs_opts: sortOpts,
  ...
  defp activate_join_lookup_filter(qNum, qdata) do
    {_, goI2, sI2, {:gen, prep2}} = :lists.keyfind(qNum, 1,
                                                     qdata)
    table2 = r_prepared(prep2, :qh)
    nPrep2 = r_prepared(prep2, qh: r_qlc_table(table2, ms: :no_match_spec))
    :lists.keyreplace(qNum, 1, qdata,
                        {qNum, goI2, sI2, {:gen, nPrep2}})
  end
  ...
  defp opt_join(join, joinOption, qdata, opt, lU_SkipQuals) do
    ...
    case (ix) do
      [{{q1, c1, q2, c2}, skip, kE, lU_fun} | _] ->
        j = r_qlc_join(kind: {:lookup, kE, lU_fun}, q1: q1, c1: c1,
                q2: q2, c2: c2, opt: opt)
        {q2, j, skip, []}
      [] ->
        m = opt_join_merge(m0)
        case (m) do
          [{{q1, c1, q2, c2}, {:merge_join, doSort, kE, skip}} |
               _] ->
            j = r_qlc_join(kind: {:merge, kE}, opt: opt, q1: q1, c1: c1,
                    q2: q2, c2: c2)
  ...
  defp opt_join_lu([{{_Q1, _C1, q2, _C2} = j,
              [{:lookup_join, _KEols, jKE, skip0} | _]} |
               lJ],
            qdata, lU_SkipQuals) do
    {^q2, _, _, {:gen, prep2}} = :lists.keyfind(q2, 1,
                                                  qdata)
    r_qlc_table(ms: mS, key_equality: kE,
        lookup_fun: lU_fun) = r_prepared(prep2, :qh)
    case (mS !== :no_match_spec and :lists.keymember(q2, 1,
                                                       lU_SkipQuals) === false) do
      true ->
        opt_join_lu(lJ, qdata, lU_SkipQuals)
      false ->
        skip = skip_if_possible(jKE, kE, skip0)
        [{j, skip, kE, lU_fun} | opt_join_lu(lJ, qdata,
                                               lU_SkipQuals)]
    end
  end
  ...
  defp setup_le(r_prepared(qh: r_qlc(lcf: qFun, qdata: qdata, init_value: v,
                    optz: optz)),
            post0, setup) do
    {goTo, firstState, post, localPost} = setup_quals(qdata,
                                                        post0, setup, optz)
    objs = fn () ->
  ...
  defp setup_le(r_prepared(qh: r_qlc_table(post_fun: postFun) = table), post,
            setup) do
  ...
  defp setup_le(r_prepared(qh: r_qlc_append(hl: prepL)), post0, setup) do
    ...
    {objsL, {post, localPost}} = :lists.mapfoldl(f,
                                                   {post0, []}, prepL)
    {fn () ->
          append_loop(objsL, 0)
     end,
  ...
  defp setup_le(r_prepared(qh: r_qlc_sort(h: prep, keypos: kp, unique: unique,
                    compressed: compressed, order: order,
                    fs_opts: sortOptions0, tmpdir_usage: tmpUsage,
                    tmpdir: tmpDir)),
            post0, setup) do
    sortOptions = sort_options_global_tmp(sortOptions0,
                                            tmpDir)
  ...
  defp setup_quals(genLoopS, [{_QNum, goI, - 1, :fil} | qdata], gs,
            p, lP, setup) do
    setup_quals(genLoopS, qdata, [{goI, [- 1, - 1]} | gs],
                  p, lP, setup)
  end

  defp setup_quals(genLoopS, [{_QNum, goI, _SI, :fil} | qdata], gs,
            p, lP, setup) do
    setup_quals(genLoopS, qdata,
                  [{goI, [genLoopS, next_state(qdata)]} | gs], p, lP,
                  setup)
  end

  defp setup_quals(genLoopS,
            [{_QNum, goI, _SI, {:gen, r_join()}} | qdata], gs, p, lP,
            setup) do
    setup_quals(genLoopS, qdata,
                  [{goI, [- 1, - 1, - 1]} | gs], p, lP, setup)
  end

  defp setup_quals(genLoopS,
            [{_QNum, goI, sI, {:gen, lE}} | qdata], gs, p, lP,
            setup) do
    {v, nP, lP1} = setup_le(lE, p, setup)
    setup_quals(sI + 1, qdata,
                  [{goI, [genLoopS, next_state(qdata), v]} | gs], nP,
                  lP ++ lP1, setup)
  end
  ...
  defp setup_join(j, qdata, goTo0, firstState0, joinFun, post0) do
    ...
    {{_QN, jGoI, jSI, _}, rev, qNum1, qNum2, wH1, wH2,
       _CsFun} = find_join_data(qdata, qNum1a, qNum2a)
    [{goI1, sI1}] = (for {qNum, goI, sI, _} <- qdata,
                           qNum === qNum1 do
                       {goI, sI}
                     end)
    [{goI2, sI2}] = (for {qNum, goI, sI, _} <- qdata,
                           qNum === qNum2 do
                       {goI, sI}
                     end)
    [h1] = (for {goI, [_Back, _Forth, h]} <- goTo0,
                  goI === goI1 do
              h
            end)
    [{backH2, h2}] = (for {goI, [back, _Forth, h]} <- goTo0,
                            goI === goI2 do
                        {back, h}
                      end)
    h0 = joinFun.(rev, {h1, wH1}, {h2, wH2})
    {h, post, localPost} = unique_cache(h0, post0, [],
                                          join_unique_cache(opt))
    [jBack] = (for {goI, [back, _, _]} <- goTo0,
                     goI === goI1 do
                 back
               end)
    jForth = next_after(qdata, sI1, qNum2)
    goTo1 = :lists.map(fn {goI, _} when goI === jGoI ->
                            {jGoI, [jBack, jForth, h]}
                         ...
                         goTo0)
    goTo = :lists.map(fn s when s === sI1 ->
                           jSI
                         s when s === sI2 ->
  ...
  defp find_join_data(qdata, qNum1, qNum2) do
    [qRev] = (for ({_QN, _GoI, _SI,
                      {:gen,
                         r_join(q1: qN1, q2: qN2, wh1: h1, wh2: h2,
                             cs_fun: csF)}} = q) <- qdata,
                    (cond do
  ...
  defp table_handle(r_qlc_table(trav_fun: traverseFun, trav_MS: travMS,
              pre_fun: preFun, lookup_fun: luF, parent_fun: parentFun,
              lu_vals: luVals, ms: mS),
            post, setup) do
    ...
    parentValue = (cond do
                     ...
                     true ->
                       case (monitor_request(parent,
                                               {:parent_fun, parentFun})) do
                         :error ->
                           post_funs(post)
                           exit(:normal)
                         {:value, value} ->
    ...
    preFunArgs = [{:parent_value, parentValue}, {:stop_fun,
                                                   stopFun}]
  ...
  defp file_loop(bin0, fd_FName, ts0, tF) do
    case (try do
      ...
      {:terms,
         <<size :: size(4) - unit(8), b :: bytes>> = bin, []} ->
        file_loop_read(bin, size - byte_size(b) + 4, fd_FName,
                         tF)
      {:terms,
         <<size :: size(4) - unit(8), _ :: bytes>> = bin, ts} ->
        c = fn () ->
  ...
  defp file_loop2(<<size :: size(4) - unit(8),
              b :: size(size) - bytes, bin :: bytes>>,
            ts) do
  ...
  defp file_loop_read(b, minBytesToRead, {fd, fileName} = fd_FName,
            tF) do
      ...
      {:ok, bin} ->
        case (b) do
          <<size :: size(4) - unit(8), tl :: bytes>>
            ...
            {b1, b2} = :erlang.split_binary(bin,
                                              size - byte_size(tl))
            foo = fn [t], fun ->
                       [t | fun]
                  end
            case (file_loop(:erlang.list_to_binary([b, b1]),
                              fd_FName, [], foo)) do
              [t | fun] ->
          ...
          _ ->
            file_loop(:erlang.list_to_binary([b, bin]), fd_FName,
                        [], tF)
        end
  ...
  defp sort_cursor_list_output(tmpDir, z, unique) do
    fn :close ->
         {:terms, []}
       {:value, noObjects} ->
         fn bTerms when unique or length(bTerms) === noObjects ->
              fn :close ->
                   {:terms, bTerms}
                 bTerms1 ->
  ...
  defp sort_cursor_input_read([object | cont], noObjects) do
    {[:erlang.term_to_binary(object)],
       sort_cursor_input(cont, noObjects + 1)}
  end
  ...
  defp unique_cache(h, post, localPost,
            r_optz(unique: false, cache: false)) do
    {h, post, localPost}
  end

  defp unique_cache(h, post, localPost,
            r_optz(unique: true, cache: false)) do
    e = :ets.new(:qlc, [:set, :private])
    {fn () ->
          no_dups(h, e)
     end,
       [del_table(e) | post], localPost}
  end

  defp unique_cache(h, post, localPost,
            r_optz(unique: false, cache: true)) do
    e = :ets.new(:qlc, [:set, :private])
  ...
  defp unique_cache(h, post, localPost,
            r_optz(unique: true, cache: true)) do
  ...
  defp unique_cache(h, post, localPost,
            r_optz(unique: false, cache: :list) = optz) do
    ref = make_ref()
    f = del_lcache(ref)
    r_qlc_opt(tmpdir: tmpDir, max_list: maxList,
        tmpdir_usage: tmpUsage) = r_optz(optz, :opt)
    {fn () ->
  ...
  defp unique_cache(h, post0, localPost0,
            r_optz(unique: true, cache: :list) = optz) do
    r_qlc_opt(tmpdir: tmpDir, max_list: maxList,
        tmpdir_usage: tmpUsage) = r_optz(optz, :opt)
    size = (cond do
              maxList >= 1 <<< 31 ->
    ...
    {uH, post1, localPost1} = sort_handle(tag_objects(h, 1),
                                            lF1, fF1, uSortOptions, post0,
                                            localPost0, tmpUsageM)
    lF2 = fn objs ->
               :lists.keysort(2, objs)
          end
    fF2 = fn objs ->
               file_sort_handle(objs, {:keysort, 2}, sortOptions,
                                  tmpDir, [], post1, localPost1)
          end
    {sH, post, localPost} = sort_handle(uH, lF2, fF2,
                                          sortOptions, post1, localPost1,
                                          tmpUsageM)
    cond do
      is_list(sH) ->
  ...
  defp sort_list(l, :ascending, true, :sort, _SortOptions,
            _Post) do
    :lists.usort(l)
  end

  defp sort_list(l, :descending, true, :sort, _SortOptions,
            _Post) do
    :lists.reverse(:lists.usort(l))
  end

  defp sort_list(l, :ascending, false, :sort, _SortOptions,
            _Post) do
    :lists.sort(l)
  end

  defp sort_list(l, :descending, false, :sort, _SortOptions,
            _Post) do
    :lists.reverse(:lists.sort(l))
  end

  defp sort_list(l, order, unique, {:keysort, kp}, _SortOptions,
            _Post)
  ...
  defp sort_handle(h, listFun, fileFun, sortOptions, post,
            localPost, tmpUsageM) do
    size = (case (:lists.keyfind(:size, 1, sortOptions)) do
              {:size, size0} ->
                size0
              false ->
                default_option(:size)
  ...
  defp sort_cache([], cL, _Sz,
            {lF, _FF, post, localPost, _TmpUsageM}) do
  ...
  defp sort_cache2(_Cont, _CL, true,
            {_LF, _FF, post, _LocalPost, {:not_allowed, m}}) do
    post_funs(post)
    throw_reason({:tmpdir_usage, m})
  end

  defp sort_cache2(cont, cL, true,
            {_LF, fF, _Post, _LocalPost, {tmpUsage, m}}) do
    maybe_error_logger(tmpUsage, m)
    fF.(:lists.reverse(cL, cont))
  end

  defp file_sort_handle(h, kp, sortOptions, tmpDir, compressed, post,
            localPost) do
    in__ = sort_cursor_input(h, 0)
    unique = :lists.member(:unique,
                             sortOptions) or :lists.keymember(:unique, 1,
                                                                sortOptions)
  ...
  defp ucache([object | cont], uTab, mTab, seqNo,
            localPost) do
  ...
  defp ucache3(object, cont, hash, uTab, mTab, seqNo,
            localPost) do
  ...
  defp ucache_recall(uTab, mTab, seqNo) do
    case (:ets.lookup(mTab, seqNo)) do
      [] = cont ->
        cont
      [{^seqNo, hash}] ->
        object = (case (:ets.lookup(uTab, hash)) do
                    [{^hash, ^seqNo, object0}] ->
                      object0
                    hashSeqObjects ->
                      {^hash, ^seqNo, object0} = :lists.keyfind(seqNo, 2,
                                                                  hashSeqObjects)
                      object0
                  end)
        [object | fn () ->
  ...
  defp lcache1([] = cont,
            {key, localPost, _TmpDir, _MaxList, _TmpUsage}, _Sz,
            acc) do
    local_post(localPost)
    case (:erlang.get(key)) do
      :undefined ->
  ...
  defp lcache1(h, state, sz, acc) when sz < 0 do
    ...
    getFile = (case (:erlang.get(key)) do
                 {:file, fd0, tmpFile, _F} ->
                   {tmpFile, fd0}
  ...
  Record.defrecord(:r_m, :m, id: :undefined,
                             tmpdir: :undefined, max_list: :undefined,
                             tmp_usage: :undefined)
  ...
  defp merge_join([e1 | l1], c1, l2, c2, opt) do
    ...
    m = r_m(id: merge_join_id(), tmpdir: tmpDir,
            max_list: maxList, tmp_usage: tmpUsage)
    merge_join2(e1, :erlang.element(c1, e1), l1, c1, l2, c2,
                  m)
  end
  ...
  defp same_keys2(e1, k1, l1, c1, [e2 | l2] = l2_0, c2, e2_0,
            m) do
    k2 = :erlang.element(c2, e2)
    cond do
      k1 == k2 ->
  ...
  defp same_keys1(e1_0, k1_0, [] = l1, c1, e2, c2, e2_0, l2, m) do
    [[e1_0 | e2_0], [e1_0 | e2] | fn () ->
                                       same_keys(k1_0, e1_0, l1, c1, l2, c2, m)
                                  end]
  end

  defp same_keys1(e1_0, k1_0, [e1 | _] = l1, c1, e2, c2, e2_0, l2,
            m) do
    k1 = :erlang.element(c1, e1)
    cond do
      k1_0 == k1 ->
      ...
      k1_0 < k1 ->
        [[e1_0 | e2_0], [e1_0 | e2] | fn () ->
                                           same_keys(k1_0, e1_0, l1, c1, l2, c2,
                                                       m)
                                      end]
    end
  end
  ...
  defp same_keys_cache(e1, k1, l1, c1, [e2 | l2], c2, e2s, sz0, m) do
    ...
    cond do
      k1 == k2 ->
        ...
        same_keys_cache(e1, k1, l1, c1, l2, c2, [e2 | e2s], sz,
                          m)
      k1 < k2 ->
        cont = fn l1b ->
  ...
  defp same_keys_file(e1, k1, l1, c1, [e2 | l2], c2, e2s, sz0, m) do
    ...
    cond do
      k1 == k2 ->
        sz = decr_list_size(sz0, e2)
        same_keys_file(e1, k1, l1, c1, l2, c2, [e2 | e2s], sz,
                         m)
      k1 < k2 ->
        cont = fn l1b ->
                    merge_join1(e2, k2, l1b, c1, l2, c2, m)
  ...
  defp write_same_keys(e1, [e2 | e2s0], m, e2s, objs) do
    ...
    write_same_keys(e1, e2s0, m, [bE2 | e2s],
                      [[e1 | e2] | objs])
  end
  ...
  defp init_merge_join(r_m(id: mergeId, tmpdir: tmpDir,
              tmp_usage: tmpUsage)) do
    case (tmp_merge_file(mergeId)) do
      ...
      :none ->
        ...
        fName = tmp_filename(tmpDir)
        case (:file.open(fName,
                           [:raw, :binary, :read, :write])) do
          {:ok, fd} ->
  ...
  defp end_all_merge_joins() do
    :lists.foreach(fn id ->
                     ...
                     for {id, _Fd,
                            _FileName} <- :lists.flatten([:erlang.get(:"$_qlc_merge_join_tmpfiles_")]) do
                       id
                     end)
  ...
  defp maybe_error_logger(name, why) do
    ...
    x = :erl_error.format_stacktrace(1, stacktrace, trimmer,
                                       formater)
