defmodule :m_gen_statem do
  ...
  def start_monitor(module, args, opts)
      when is_atom(module) and
             is_list(opts) do
  ...
  defp enter(parent, debug, module, name, hibernateAfterTimeout, state, data, actions) do
    ...
    modules = [module]
    p = r_params(parent: parent, name: name, hibernate_after: hibernateAfterTimeout)
  ...
  def init_it(starter, parent, serverRef, module, args, opts) do
    ...
    try do
      ...
      class, reason ->
        ...
        :proc_lib.init_fail(starter, {:error, reason}, {class, reason, __STACKTRACE__})
    else
      result ->
        init_result(
          starter,
  ...
  def system_code_change(
        {r_params(modules: [module | _] = modules) = p, r_state(state_data: {state, data}) = s},
        _Mod,
        oldVsn,
        extra
      ) do
  ...
  def format_status(
    ...
    header = :gen.format_status_header(~c"Status for state machine", name)
    ...
    newStatusMap =
      case :gen.format_status(mod, opt, statusMap, [pDict, state, data]) do
        %{EXIT: r} ->
          crashed = [{:data, [{~c"State", {state, r}}]}]
        ...
        %{state: s, data: d} = sM ->
          Map.put(sM, :"$status", [{:data, [{~c"State", {s, d}}]}])
      end
  ...
  defp print_event(dev, systemEvent, name) do
    case systemEvent do
      {:in, event, state} ->
        :io.format(dev, ~c"*DBG* ~tp receive ~ts in state ~tp~n", [
          name,
          event_string(event),
          state
        ])
      ...
      {:out, reply, {to, _Tag}} ->
        :io.format(dev, ~c"*DBG* ~tp send ~tp to ~tw~n", [name, reply, to])

      {:enter, module, state} ->
        :io.format(dev, ~c"*DBG* ~tp enter ~tp in state ~tp~n", [name, module, state])

      {:module, module, state} ->
        :io.format(dev, ~c"*DBG* ~tp module ~tp in state ~tp~n", [name, module, state])

      {:start_timer, action, state} ->
        :io.format(dev, ~c"*DBG* ~tp start_timer ~tp in state ~tp~n", [name, action, state])

      {:insert_timeout, event, state} ->
        :io.format(dev, ~c"*DBG* ~tp insert_timeout ~tp in state ~tp~n", [name, event, state])

      {:terminate, reason, state} ->
        :io.format(dev, ~c"*DBG* ~tp terminate ~tp in state ~tp~n", [name, reason, state])
      ...
      when tag === :postpone or
        ...
        stateString =
          case nextState do
            ^state ->
              :io_lib.format(~c"~tp", [state])

            _ ->
              :io_lib.format(~c"~tp => ~tp", [state, nextState])
          end

        :io.format(dev, ~c"*DBG* ~tp ~tw ~ts in state ~ts~n", [
          name,
          tag,
          event_string(event),
          stateString
  ...
  defp event_string(event) do
    case event do
      {{:call, {pid, _Tag}}, request} ->
        :io_lib.format(~c"call ~tp from ~tw", [request, pid])

      {eventType, eventContent} ->
        :io_lib.format(~c"~tw ~tp", [eventType, eventContent])
    end
  end
  ...
  defp loop_event(p, debug, r_state(hibernate: true) = s, event, events) do
    ...
    q = [event | events]
    loop_state_callback(p, debug, s, q, r_state(s, :state_data), event)
  end
  ...
  defp loop_actions_next_event_bad(
         p,
         debug,
         s,
         q,
  ...
  defp callback_mode_result(p, modules, callbackModeResult, [h | t], callbackMode, stateEnter) do
    case callback_mode(h) do
      true ->
        callback_mode_result(p, modules, callbackModeResult, t, h, stateEnter)

      false ->
        case state_enter(h) do
          true ->
            callback_mode_result(p, modules, callbackModeResult, t, callbackMode, true)

          false ->
            {:error, {:bad_return_from_callback_mode, callbackModeResult},
             :erlang.element(
               2,
               :erlang.process_info(
                 self(),
  ...
  defp callback_mode_result(p, modules, callbackModeResult, [], callbackMode, stateEnter) do
    cond do
      ...
      true ->
        r_params(p, modules: modules, callback_mode: callbackMode, state_enter: stateEnter)
    end
  end
  ...
  defp error_info(
         ...
         r_state(postponed: postponed, timers: timers, state_data: {state, data}),
         q
       ) do
    {numTimers, listTimers} = list_timeouts(timers)
    ...
    newState =
      case :maps.find(:"$status", status) do
        :error ->
      ...
      true ->
        :erlang.apply(:logger, :macro_log, [
          %{
            ...
            line: 2635,
            file: ~c"otp/lib/stdlib/src/gen_statem.erl"
          },
          :error,
          %{
  ...
  defp format_client_log({pid, :dead}, _, _) do
    {~c"** Client ~p is dead~n", [pid]}
  end

  defp format_client_log({pid, :remote}, _, _) do
    {~c"** Client ~p is remote on node ~p~n", [pid, node(pid)]}
  end

  defp format_client_log({_Pid, {name, stacktrace}}, p, depth) do
    format = :lists.append([~c"** Client ", p, ~c" stacktrace~n** ", p, ~c"~n"])
