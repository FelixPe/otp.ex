defmodule :m_unicode do
  ...
  defp no_conversion_needed(mL, :latin1, :latin1) do
    is_binary(mL)
  end
  ...
  defp fake_stacktrace(reason, name, args) do
    try do
      :erlang.error(:new_stacktrace, args)
    catch
      :error, _ ->
        [{mod, _, ^args, _} | stk1] = __STACKTRACE__
  ...
  defp error_type(:system_limit = sL) do
    sL
  end
