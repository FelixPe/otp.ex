defmodule :m_file_sorter do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_w, :w, keypos: :undefined, runs: [[]],
                             seq: 1, in: :undefined, out: :undefined,
                             fun_out: :undefined, prefix: :undefined, temp: [],
                             format: :undefined, runsize: :undefined,
                             no_files: :undefined, order: :undefined,
                             chunksize: :undefined, wfd: :undefined,
                             ref: :undefined, z: :undefined, unique: :undefined,
                             hdlen: :undefined, inout_value: :undefined)
  Record.defrecord(:r_opts, :opts, format: binary_term_fun(),
                                size: 524288, no_files: 16, tmpdir: :default,
                                order: :ascending, compressed: false,
                                unique: false, header: 4)
  ...
  def sort(input0, output0, options) do
    case ({is_input(input0), maybe_output(output0),
             options(options)}) do
      {{true, input}, {true, output}, r_opts() = opts} ->
        do_sort(0, input, output, opts, :sort)
      t ->
        badarg(culprit(:erlang.tuple_to_list(t)),
  ...
  def keysort(keyPos, input0, output0, options) do
    r = (case ({is_keypos(keyPos), is_input(input0),
                  maybe_output(output0), options(options)}) do
  ...
  def merge(files0, output0, options) do
    case ({is_files(files0), maybe_output(output0),
             options(options)}) do
  ...
  defp options([{:order, fun} | l], opts) when is_function(fun,
                                                        2) do
  ...
  defp do_sort(keyPos0, input0, output0, opts, do__) do
    ...
    {output, funOut} = wrap_output_terms(format0, output0,
                                           z)
    w = r_w(keypos: keyPos, out: output, fun_out: funOut,
  ...
  defp wrap_input(:term, :merge, files) do
    ...
    input = :lists.reverse(for f <- files do
                             fun.(f)
                           end)
  ...
  defp wrap_input(:term, :sort, inFun) when is_function(inFun,
                                                  1) do
  ...
  defp wrap_output_terms(:term, outFun, _Z) when is_function(outFun,
                                                1) do
  ...
  defp check_objs0(iFun, f, fNs, w, l, i, :nolast,
            [{t, _BT} | os]) do
  ...
  defp ccheck_objs(iFun, f, fNs, w, l, i, lT, [{t, bT} | os],
            cF) do
    case (cF.(lT, t)) do
      true ->
  ...
  defp uccheck_objs(iFun, f, fNs, w, l, i, lT, [{t, bT} | os],
            cF) do
    case (cF.(lT, t)) do
      true ->
        case (cF.(t, lT)) do
  ...
  defp culprit_found(iFun, f, fNs, w, l, i, [_Size | bT]) do
    iFun.(:close)
    check_files(fNs, w,
                  [{f, i, :erlang.binary_to_term(bT)} | l])
  end
  ...
  defp fun_run(i, l, lSz, r_w(runsize: runsize) = w, objs)
      when lSz < runsize do
    {nI, nObjs, nL, nLSz} = fun_objs(objs, l, lSz, runsize,
                                       i, w)
  ...
  defp init_merge([], _I, fs0, r_w(order: order) = w)
    ...
    {:lists.sort(cfun_files(r_w(w, :order)),
                   :lists.reverse(fs0)),
       w}
  end
  ...
  defp merge_files([f1, f2 | fs], l0, lSz, last0, w)
    ...
    {nInEtc, nFs, nL, nLast} = (case (kind) do
                                  :umerge ->
                                    umerge_files(l, f2, fs, inEtc2, ts2, ts,
                                                   inEtc, t2, last)
                                  {:ukmerge, kp} ->
                                    ukmerge_files(l, f2, fs, inEtc2, ts2, ts,
                                                    inEtc, t2, kp, last)
                                  :merge ->
                                    merge_files(l, f2, fs, inEtc2, bT2, ts2T,
                                                  ts, inEtc, t2)
                                  :rumerge ->
                                    rumerge_files(l, f2, fs, inEtc2, ts2, ts,
                                                    inEtc, t2, last)
                                  {:rukmerge, kp} ->
                                    {lt, ltBT} = last
                                    rukmerge_files(l, f2, fs, inEtc2, ts2, ts,
                                                     inEtc, t2, kp, lt, ltBT)
                                  :rmerge ->
                                    rmerge_files(l, f2, fs, inEtc2, bT2, ts2T,
                                                   ts, inEtc, t2)
                                  {:ucmerge, cF} ->
                                    {i2, _} = inEtc2
                                    {i, _} = inEtc
                                    ucmerge_files(l, f2, fs, inEtc2, ts2, i2,
                                                    ts, i, inEtc, t2, cF, last)
                                  {:cmerge, cF} ->
                                    {i2, _} = inEtc2
                                    {i, _} = inEtc
                                    cmerge_files(l, f2, fs, inEtc2, bT2, ts2T,
                                                   i2, ts, i, inEtc, t2, cF)
                                end)
  ...
  defp merge_kind(r_w(order: :ascending, unique: true,
              keypos: 0)) do
    :umerge
  end
  ...
  defp merge_kind(r_w(order: :descending, unique: true,
              keypos: 0)) do
    :rumerge
  end
  ...
  defp umerge_files(l, f2, fs, inEtc2, ts2, [{t, _BT} | ts], inEtc,
            t2, last)
      when t == last do
  ...
  defp umerge_files(l, f2, fs, inEtc2, ts2, [{t, bT} | ts], inEtc,
            t2, _Last)
      when t <= t2 do
    umerge_files([bT | l], f2, fs, inEtc2, ts2, ts, inEtc,
                   t2, t)
  end

  defp umerge_files(l, f2, fs, _InEtc2, _Ts2, [], inEtc, _T2,
            last) do
    {inEtc, [f2 | fs], l, {:last, last}}
  end

  defp umerge_files(l, _F2, fs, inEtc2, ts2, ts, inEtc, _T2,
            last) do
    [f3 | nFs] = insert([ts | inEtc], fs)
    [[{t3, _BT3} | _] = ts3 | inEtc3] = f3
    umerge_files(l, f3, nFs, inEtc3, ts3, ts2, inEtc2, t3,
                   last)
  end

  defp rumerge_files(l, f2, fs, inEtc2, ts2, [{t, _BT} | ts], inEtc,
            t2, last)
      when t == last do
  ...
  defp rumerge_files(l, f2, fs, inEtc2, ts2, [{t, bT} | ts], inEtc,
            t2, _Last)
      when t >= t2 do
    rumerge_files([bT | l], f2, fs, inEtc2, ts2, ts, inEtc,
                    t2, t)
  end

  defp rumerge_files(l, f2, fs, _InEtc2, _Ts2, [], inEtc, _T2,
            last) do
    {inEtc, [f2 | fs], l, {:last, last}}
  end

  defp rumerge_files(l, _F2, fs, inEtc2, ts2, ts, inEtc, _T2,
            last) do
    [f3 | nFs] = rinsert([ts | inEtc], fs)
    [[{t3, _BT3} | _] = ts3 | inEtc3] = f3
    rumerge_files(l, f3, nFs, inEtc3, ts3, ts2, inEtc2, t3,
                    last)
  end

  defp merge_files(l, f2, fs, inEtc2, bT2, ts2, [{t, bT} | ts],
            inEtc, t2)
      when t <= t2 do
    merge_files([bT | l], f2, fs, inEtc2, bT2, ts2, ts,
                  inEtc, t2)
  end

  defp merge_files(l, f2, fs, _InEtc2, _BT2, _Ts2, [], inEtc,
            _T2) do
  ...
  defp merge_files(l, _F2, fs, inEtc2, bT2, ts2, ts, inEtc, _T2) do
    ...
    merge_files(l1, f3, nFs, inEtc3, bT3, ts3, ts2, inEtc2,
                  t3)
  end

  defp rmerge_files(l, f2, fs, inEtc2, bT2, ts2, [{t, bT} | ts],
            inEtc, t2)
      when t >= t2 do
    rmerge_files([bT | l], f2, fs, inEtc2, bT2, ts2, ts,
                   inEtc, t2)
  end

  defp rmerge_files(l, f2, fs, _InEtc2, _BT2, _Ts2, [], inEtc,
            _T2) do
    {inEtc, [f2 | fs], l, {:last, :foo}}
  end
  ...
  defp rmerge_files(l, _F2, fs, inEtc2, bT2, ts2, ts, inEtc, _T2) do
    ...
    [f3 | nFs] = rinsert([ts | inEtc], fs)
    [[{t3, bT3} | ts3] | inEtc3] = f3
    rmerge_files(l1, f3, nFs, inEtc3, bT3, ts3, ts2, inEtc2,
                   t3)
  end

  defp ukmerge_files(l, f2, fs, inEtc2, ts2, [{[t | _I], _BT} | ts],
            inEtc, t2, kp, last)
      when t == last do
  ...
  defp ukmerge_files(l, f2, fs, inEtc2, ts2,
            [{[t0 | _I] = t, bT} | ts], inEtc, t2, kp, _Last)
      when t <= t2 do
    ukmerge_files([bT | l], f2, fs, inEtc2, ts2, ts, inEtc,
                    t2, kp, t0)
  end

  defp ukmerge_files(l, f2, fs, _InEtc2, _Ts2, [], inEtc, _T2, _Kp,
            last) do
    {inEtc, [f2 | fs], l, {:last, last}}
  end

  defp ukmerge_files(l, _F2, fs, inEtc2, ts2, ts, inEtc, _T2, kp,
            last) do
    [f3 | nFs] = insert([ts | inEtc], fs)
    [[{t3, _BT3} | _] = ts3 | inEtc3] = f3
    ukmerge_files(l, f3, nFs, inEtc3, ts3, ts2, inEtc2, t3,
                    kp, last)
  end

  defp rukmerge_files(l, f2, fs, inEtc2, ts2, [{[t | _I], bT} | ts],
  ...
  defp rukmerge_files(l, f2, fs, inEtc2, ts2,
            [{[t0 | _I] = t, bT} | ts], inEtc, t2, kp, _Last,
            lastBT)
      when t >= t2 do
    rukmerge_files([lastBT | l], f2, fs, inEtc2, ts2, ts,
                     inEtc, t2, kp, t0, bT)
  end

  defp rukmerge_files(l, f2, fs, _InEtc2, _Ts2, [], inEtc, _T2, _Kp,
            last, lastBT) do
    {inEtc, [f2 | fs], l, {:last, {last, lastBT}}}
  end

  defp rukmerge_files(l, _F2, fs, inEtc2, ts2, ts, inEtc, _T2, kp,
            last, lastBT) do
    [f3 | nFs] = rinsert([ts | inEtc], fs)
    [[{t3, _BT3} | _] = ts3 | inEtc3] = f3
    rukmerge_files(l, f3, nFs, inEtc3, ts3, ts2, inEtc2, t3,
                     kp, last, lastBT)
  end

  defp ucmerge_files(l, f2, fs, inEtc2, ts2, i2,
            [{t, bT} | ts] = ts0, i, inEtc, t2, cF, last)
      when i < i2 do
    case (cF.(t, t2)) do
      true ->
        case (cF.(t, last)) do
      ...
      false ->
        ...
        ucmerge_files(l, f3, nFs, inEtc3, ts3, i3, ts2, i2,
                        inEtc2, t3, cF, last)
    end
  end

  defp ucmerge_files(l, f2, fs, inEtc2, ts2, i2,
            [{t, bT} | ts] = ts0, i, inEtc, t2, cF, last) do
    case (cF.(t2, t)) do
      true ->
        [f3 | nFs] = cinsert([ts0 | inEtc], fs, cF)
        [[{t3, _BT3} | _] = ts3 | {i3, _} = inEtc3] = f3
  ...
  defp ucmerge_files(l, f2, fs, _InEtc2, _Ts2, _I2, [], _I, inEtc,
            _T2, _CF, last) do
    {inEtc, [f2 | fs], l, {:last, last}}
  end

  defp cmerge_files(l, f2, fs, inEtc2, bT2, ts2, i2,
            [{t, bT} | ts] = ts0, i, inEtc, t2, cF)
      when i < i2 do
    case (cF.(t, t2)) do
      true ->
        cmerge_files([bT | l], f2, fs, inEtc2, bT2, ts2, i2, ts,
                       i, inEtc, t2, cF)
      false ->
        l1 = [bT2 | l]
        [f3 | nFs] = cinsert([ts0 | inEtc], fs, cF)
  ...
  defp cmerge_files(l, f2, fs, _InEtc2, _BT2, _Ts2, _I2, [], _I,
            inEtc, _T2, _CF) do
    {inEtc, [f2 | fs], l, {:last, :foo}}
  end
  ...
  defp read_more(iFun, i, lSz, w) do
    case (iFun.({{:merge, i}, [], lSz, w})) do
      ...
      {{_, l, nLSz}, nInFun} ->
        nW = (case (:lists.member(iFun, r_w(w, :temp))) do
                true ->
  ...
  defp read_fun2(fd, bin, size, fileName, owner) do
    fn :close ->
       ...
       {i, l, lSz, w} ->
         case (read_objs(fd, fileName, i, l, bin, size, lSz,
                           w)) do
  ...
  defp file_loop(l, i, b1, b2, sz, _B1Sz, b2Sz, min, w)
    ...
    {i1, l1, <<>>, sz1} = file_loop(l, i,
                                      :erlang.list_to_binary([b1, b3]), sz, w)
  ...
  defp file_loop1(l, i, hB, 0, kp, f, hdLen) do
    ...
    file_loop2(l, i, b, size,
                 <<size :: size(hdLen) - unit(8)>>, kp, f, hdLen)
  end

  defp file_loop1(l, i, b, sz, kp, f, hdLen) do
    file_loop2(l, i, b, sz, <<sz :: size(hdLen) - unit(8)>>,
                 kp, f, hdLen)
  end
  ...
  defp file_binloop(l, size, sizeB, b, hL) do
    case (b) do
      <<bin :: size(size) - binary,
          nSizeB :: size(hL) - binary, r :: binary>> ->
        <<nSize :: size(hL) - unit(8)>> = nSizeB
        file_binloop([{bin, [sizeB | bin]} | l], nSize, nSizeB,
                       r, hL)
      _ ->
        {l, b, size, sizeB}
    end
  ...
  defp file_binterm_loop(l, size, sizeB, b, fun, hL) do
    case (b) do
      <<binTerm :: size(size) - binary,
        ...
        bT = [sizeB | binTerm]
        term = fun.(binTerm)
        file_binterm_loop([{term, bT} | l], nSize, nSizeB, r,
                            fun, hL)
      <<binTerm :: size(size) - binary>> ->
        term = fun.(binTerm)
  ...
  defp key_loop(keyPos, i, l, size, sizeB, b, fun, hL) do
    case (b) do
      <<binTerm :: size(size) - binary,
        ...
        e = {uniqueKey, bT}
        key_loop(keyPos, i + 1, [e | l], nSize, nSizeB, r, fun,
                   hL)
      <<binTerm :: size(size) - binary>> ->
        uniqueKey = make_key(keyPos, fun.(binTerm))
  ...
  defp merge_loop(keyPos, i, l, size, sizeB, b, fun, hL) do
    case (b) do
      <<binTerm :: size(size) - binary,
        ...
        e = {uniqueKey, bT}
        merge_loop(keyPos, i, [e | l], nSize, nSizeB, r, fun,
                     hL)
      <<binTerm :: size(size) - binary>> ->
        uniqueKey = make_stable_key(keyPos, i, fun.(binTerm))
        nL = [{uniqueKey, [sizeB | binTerm]} | l]
  ...
  defp fun_loop(objs, l, lSz, runSize, {:merge, i}, keypos, fun,
            hdLen) do
    fun_mergeloop(objs, l, lSz, runSize, i, keypos, fun,
                    hdLen)
  end

  defp fun_loop(objs, l, lSz, runSize, i, keypos, fun, hdLen)
      when is_integer(i) do
    fun_keyloop(objs, l, lSz, runSize, i, keypos, fun,
                  hdLen)
  end
  ...
  defp fun_keyloop([b | bs], l, lSz, runSize, i, kp, fun, hL)
    ...
    fun_keyloop(bs, [e | l], lSz + size, runSize, i + 1, kp,
                  fun, hL)
  end
  ...
  defp fun_mergeloop([b | bs], l, lSz, runSize, i, kp, fun, hL)
    ...
    e = {uniqueKey, [<<size :: size(hL) - unit(8)>> | b]}
    fun_mergeloop(bs, [e | l], lSz + size, runSize, i, kp,
                    fun, hL)
  end
  ...
  defp infun(w) do
    ...
    try do
      ...
      {objs, nFun} when (is_function(nFun, 1) and
                           is_list(objs))
                        ->
  ...
  defp tmp_prefix(f, tmpDirOpt) when is_function(f) or
                               f === :undefined do
  ...
  defp cleanup(w) do
    close_out(w)
    w1 = close_input(w)
    f = fn iFun when is_function(iFun) ->
             iFun.(:close)
           {fd, fileName} ->
             _ = :file.close(fd)
             _ = :file.delete(fileName)
  ...
  defp close_file(fd, w) do
    ...
    case (:file.close(fd)) do
      :ok ->
        r_w(w, temp: [fileName | :lists.keydelete(fd, 1,
                                                  r_w(w, :temp))])
      error ->
        file_error(fileName, error, w)
    end
  ...
  defp file_rterms2(fd, l, lSz, fileName, files) when lSz < 16384 do
    case (:io.read(fd, :"")) do
      {:ok, term} ->
        ...
        file_rterms2(fd, [b | l], lSz + byte_size(b), fileName,
                       files)
      :eof ->
        _ = :file.close(fd)
  ...
  defp write_terms(fd, f, [b | bs], args) do
    case (:io.request(fd,
                        {:format, '~p.~n', [:erlang.binary_to_term(b)]})) do
      :ok ->
