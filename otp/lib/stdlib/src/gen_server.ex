defmodule :m_gen_server do
  use Bitwise
  require Record
  Record.defrecord(:r_callback_cache, :callback_cache, module: :undefined,
                                          handle_call: :undefined,
                                          handle_cast: :undefined,
                                          handle_info: :undefined)
  def start(module, args, options) when (is_atom(module) and
                                        is_list(options)) do
  ...
  def start(serverName, module, args, options)
      when (is_tuple(serverName) and is_atom(module) and
              is_list(options)) do
    :gen.start(:gen_server, :nolink, serverName, module,
                 args, options)
  end
  ...
  def start_link(module, args, options) when (is_atom(module) and
                                        is_list(options)) do
  ...
  def start_link(serverName, module, args, options)
      when (is_tuple(serverName) and is_atom(module) and
              is_list(options)) do
    :gen.start(:gen_server, :link, serverName, module, args,
                 options)
  end
  ...
  def start_monitor(module, args, options) when (is_atom(module) and
                                        is_list(options)) do
  ...
  def start_monitor(serverName, module, args, options)
      when (is_tuple(serverName) and is_atom(module) and
              is_list(options)) do
    :gen.start(:gen_server, :monitor, serverName, module,
                 args, options)
  end
  ...
  def abcast(name, request) when is_atom(name) do
    do_abcast([node() | :erlang.nodes()], name,
                cast_msg(request))
  end

  def abcast(nodes, name, request) when (is_list(nodes) and
                                       is_atom(name)) do
  ...
  def multi_call(name, request) when is_atom(name) do
    multi_call([node() | :erlang.nodes()], name, request,
                 :infinity)
  end

  def multi_call(nodes, name, request) when (is_list(nodes) and
                                       is_atom(name)) do
  ...
  defp mc_send([node | nodes], name, alias, request, timer,
            reqs)
  ...
  defp mc_send(_BadNodes, _Name, alias, _Request, timer,
            reqs) do
    unalias(alias)
  ...
  defp mc_recv([[node | mon] | restReqs] = reqs, alias, timer,
            replies, badNodes) do
    receive do
      {[[:alias | ^alias] | ^mon], reply} ->
        :erlang.demonitor(mon, [:flush])
        mc_recv(restReqs, alias, timer,
                  [{node, reply} | replies], badNodes)
      {^alias, ^mon, :process, _, _} ->
        mc_recv(restReqs, alias, timer, replies,
                  [node | badNodes])
      {:timeout, ^timer, ^alias} ->
        unalias(alias)
  ...
  defp mc_recv_tmo([[node | mon] | restReqs], alias, replies,
            badNodes) do
    :erlang.demonitor(mon)
    receive do
      {[[:alias | ^alias] | ^mon], reply} ->
        mc_recv_tmo(restReqs, alias, [{node, reply} | replies],
                      badNodes)
      {^alias, ^mon, :process, _, _} ->
  ...
  def enter_loop(mod, options, state) when (is_atom(mod) and
                                      is_list(options)) do
  ...
  def enter_loop(mod, options, state, timeoutOrHibernate)
      when (is_atom(mod) and is_list(options) and
              timeoutOrHibernate === :infinity or is_integer(timeoutOrHibernate) and timeoutOrHibernate >= 0) or
             (is_atom(mod) and is_list(options) and
  ...
  def enter_loop(mod, options, state, serverName,
           timeoutOrHibernate)
      when (is_atom(mod) and is_list(options) and
              timeoutOrHibernate === :infinity or is_integer(timeoutOrHibernate) and timeoutOrHibernate >= 0) or
             (is_atom(mod) and is_list(options) and
    ...
    loop(parent, name, state, cbCache, timeoutOrHibernate,
  ...
  def enter_loop(mod, options, state, serverName,
    ...
    hibernateAfterTimeout = :gen.hibernate_after(options)
    cbCache = create_callback_cache(mod)
    loop(parent, name, state, cbCache, continue,
           hibernateAfterTimeout, debug)
  end
  ...
  def init_it(starter, parent, name0, mod, args, options) do
    ...
    case (init_it(mod, args)) do
      {:ok, {:ok, state}} ->
        ...
        loop(parent, name, state, cbCache, :infinity,
               hibernateAfterTimeout, debug)
      {:ok, {:ok, state, timeoutOrHibernate}}
          when timeoutOrHibernate === :infinity or is_integer(timeoutOrHibernate) and timeoutOrHibernate >= 0 or
  ...
  defp loop(parent, name, state, cbCache,
            {:continue, continue} = msg, hibernateAfterTimeout,
            debug) do
    reply = try_handle_continue(cbCache, continue, state)
    case (debug) do
      [] ->
        handle_common_reply(reply, parent, name, :undefined,
                              msg, cbCache, hibernateAfterTimeout, state)
      _ ->
        debug1 = :sys.handle_debug(debug, &print_event/3, name,
                                     msg)
        handle_common_reply(reply, parent, name, :undefined,
                              msg, cbCache, hibernateAfterTimeout, state,
                              debug1)
  ...
  defp loop(parent, name, state, cbCache, :hibernate,
            hibernateAfterTimeout, debug) do
    mod = r_callback_cache(cbCache, :module)
    :proc_lib.hibernate(:gen_server, :wake_hib,
                          [parent, name, state, mod, hibernateAfterTimeout,
                                                         debug])
  end

  defp loop(parent, name, state, cbCache, :infinity,
            hibernateAfterTimeout, debug) do
    receive do
      msg ->
        decode_msg(msg, parent, name, state, cbCache, :infinity,
                     hibernateAfterTimeout, debug, false)
    after hibernateAfterTimeout ->
      loop(parent, name, state, cbCache, :hibernate,
             hibernateAfterTimeout, debug)
    end
  end

  defp loop(parent, name, state, cbCache, time,
            hibernateAfterTimeout, debug) do
    msg = (receive do
             input ->
               input
    ...
    decode_msg(msg, parent, name, state, cbCache, time,
                 hibernateAfterTimeout, debug, false)
  end

  defp create_callback_cache(mod) do
    r_callback_cache(module: mod,
  ...
  def wake_hib(parent, name, state, mod, hibernateAfterTimeout,
           debug) do
    msg = (receive do
             input ->
               input
    ...
    cbCache = create_callback_cache(mod)
    decode_msg(msg, parent, name, state, cbCache,
                 :hibernate, hibernateAfterTimeout, debug, true)
  end

  defp decode_msg(msg, parent, name, state, cbCache, time,
            hibernateAfterTimeout, debug, hib) do
    case (msg) do
      {:system, from, req} ->
        :sys.handle_system_msg(req, from, parent, :gen_server,
                                 debug,
                                 [name, state, cbCache, time,
        ...
        terminate(reason,
                    :erlang.element(2,
                                      :erlang.process_info(self(),
                                                             :current_stacktrace)),
                    name, :undefined, msg, mod, state, debug)
      _Msg when debug === [] ->
        handle_msg(msg, parent, name, state, cbCache,
                     hibernateAfterTimeout)
      _Msg ->
        debug1 = :sys.handle_debug(debug, &print_event/3, name,
                                     {:in, msg})
  ...
  defp try_handle_info(r_callback_cache(module: mod, handle_info: handleInfo), msg,
            state) do
    try do
      ...
      r ->
        {:ok, r}
      :error, :undef = r ->
        case (:erlang.function_exported(mod, :handle_info,
                                          2)) do
          false ->
              ...
              true ->
                :erlang.apply(:logger, :macro_log,
                                [%{mfa: {:gen_server, :try_handle_info, 3},
                                     line: 1102, file: 'otp/lib/stdlib/src/gen_server.erl'},
                                     :warning, %{label:
                                                 {:gen_server, :no_handle_info},
                                                   module: mod, message: msg},
                                                   %{domain: [:otp],
                                                       report_cb:
                                                       &:gen_server.format_log/2,
                                                       error_logger:
                                                       %{tag: :warning_msg,
                                                           report_cb:
  ...
  defp handle_msg({:"$gen_call", from, msg}, parent, name, state, cbCache,
            hibernateAfterTimeout) do
    result = try_handle_call(cbCache, msg, from, state)
    case (result) do
      {:ok, {:reply, reply, nState}} ->
        reply(from, reply)
        loop(parent, name, nState, cbCache, :infinity,
               hibernateAfterTimeout, [])
      {:ok, {:reply, reply, nState, timeoutOrHibernate}}
          when timeoutOrHibernate === :infinity or is_integer(timeoutOrHibernate) and timeoutOrHibernate >= 0 or
               ...
               ->
        reply(from, reply)
        loop(parent, name, nState, cbCache, timeoutOrHibernate,
               hibernateAfterTimeout, [])
      {:ok,
         {:reply, reply, nState, {:continue, _} = continue}} ->
        reply(from, reply)
        loop(parent, name, nState, cbCache, continue,
               hibernateAfterTimeout, [])
        ...
        try do
          ...
          terminate(reason,
                      :erlang.element(2,
                                        :erlang.process_info(self(),
                                                               :current_stacktrace)),
                      name, from, msg, mod, nState, [])
        after
          reply(from, reply)
        end
      other ->
        handle_common_reply(other, parent, name, from, msg,
                              cbCache, hibernateAfterTimeout, state)
    end
  end

  defp handle_msg(msg, parent, name, state, cbCache,
            hibernateAfterTimeout) do
        ...
        try do
          ...
          terminate(reason,
                      :erlang.element(2,
                                        :erlang.process_info(self(),
                                                               :current_stacktrace)),
                      name, from, msg, mod, nState, debug)
        after
          _ = reply(name, from, reply, nState, debug)
        end
      other ->
        handle_common_reply(other, parent, name, from, msg,
                              cbCache, hibernateAfterTimeout, state, debug)
    end
  end
  ...
  defp handle_msg(msg, parent, name, state, cbCache,
    ...
    reply = try_dispatch(msg, cbCache, state)
    handle_common_reply(reply, parent, name, :undefined,
                          msg, cbCache, hibernateAfterTimeout, state, debug)
  end

  defp handle_common_reply(reply, parent, name, from, msg, cbCache,
            hibernateAfterTimeout, state) do
    mod = r_callback_cache(cbCache, :module)
    case (reply) do
      {:ok, {:noreply, nState}} ->
        loop(parent, name, nState, cbCache, :infinity,
      ...
      {:ok, {:stop, reason, nState}} ->
        terminate(reason,
                    ...
                    name, from, msg, mod, nState, [])
      {:EXIT, class, reason, stacktrace} ->
        terminate(class, reason, stacktrace, name, from, msg,
                    mod, state, [])
      {:ok, badReply} ->
        terminate({:bad_return_value, badReply},
                    :erlang.element(2,
                                      :erlang.process_info(self(),
                                                             :current_stacktrace)),
                    name, from, msg, mod, state, [])
    end
  end

  defp handle_common_reply(reply, parent, name, from, msg, cbCache,
            hibernateAfterTimeout, state, debug) do
    mod = r_callback_cache(cbCache, :module)
    case (reply) do
      {:ok, {:noreply, nState}} ->
        debug1 = :sys.handle_debug(debug, &print_event/3, name,
      ...
      {:ok, badReply} ->
        terminate({:bad_return_value, badReply},
                    :erlang.element(2,
                                      :erlang.process_info(self(),
                                                             :current_stacktrace)),
                    name, from, msg, mod, state, debug)
    end
  end

  defp reply(name, from, reply, state, debug) do
    reply(from, reply)
    :sys.handle_debug(debug, &print_event/3, name,
                        {:out, reply, from, state})
  end

  def system_continue(parent, debug,
           [name, state, cbCache, time, hibernateAfterTimeout]) do
    loop(parent, name, state, cbCache, time,
           hibernateAfterTimeout, debug)
  end

  def system_terminate(reason, _Parent, debug,
           [name, state, cbCache, _Time,
                                      _HibernateAfterTimeout]) do
    mod = r_callback_cache(cbCache, :module)
    terminate(reason,
                :erlang.element(2,
                                  :erlang.process_info(self(),
                                                         :current_stacktrace)),
                name, :undefined, [], mod, state, debug)
  end

  def system_code_change([name, state, cbCache, time,
                                    hibernateAfterTimeout],
           _Module, oldVsn, extra) do
    mod = r_callback_cache(cbCache, :module)
    case ((try do
      ...
      {:ok, newState} ->
        {:ok,
           [name, newState, cbCache, time, hibernateAfterTimeout]}
      else__ ->
        else__
    end
  end

  def system_get_state([_Name, state, _Mod, _Time,
                                  _HibernateAfterTimeout]) do
    {:ok, state}
  end
  ...
  def system_replace_state(stateFun,
    ...
    {:ok, nState,
       [name, nState, cbCache, time, hibernateAfterTimeout]}
  end
  ...
  defp terminate(reason, stacktrace, name, from, msg, mod, state,
            debug) do
    terminate(:exit, reason, stacktrace, false, name, from,
                msg, mod, state, debug)
  end

  defp terminate(class, reason, stacktrace, name, from, msg, mod,
            state, debug) do
    terminate(class, reason, stacktrace, true, name, from,
                msg, mod, state, debug)
  end

  defp terminate(class, reason, stacktrace, reportStacktrace,
            name, from, msg, mod, state, debug) do
    reply = try_terminate(mod,
                            catch_result(class, reason, stacktrace), state)
    case (reply) do
      {:EXIT, c, r, s} ->
      ...
      _ ->
        case ({class, reason}) do
          ...
          _ when reportStacktrace ->
            error_info(reason, stacktrace, name, from, msg, mod,
                         state, debug)
          _ ->
            error_info(reason, :undefined, name, from, msg, mod,
                         state, debug)
        end
    end
    case (stacktrace) do
  ...
  defp error_info(_Reason, _ST, :application_controller, _From,
            _Msg, _Mod, _State, _Debug) do
    :ok
  end

  defp error_info(reason, sT, name, from, msg, mod, state,
            debug) do
    log = :sys.get_log(debug)
    status = :gen.format_status(mod, :terminate,
                                  %{reason: reason, state: state, message: msg,
                                      log: log},
                                  [:erlang.get(), state])
    reportReason = (cond do
                      sT == :undefined ->
    ...
    case (:logger.allow(:error, :gen_server)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:gen_server, :error_info, 8}, line: 1391,
                             file: 'otp/lib/stdlib/src/gen_server.erl'},
                             :error, %{label: {:gen_server, :terminate},
                                         name: name,
                                         last_message:
  ...
  defp client_stacktrace(from) when (is_pid(from) and
                        node(from) === node()) do
    case (:erlang.process_info(from,
                                 [:current_stacktrace, :registered_name])) do
      :undefined ->
        {from, :dead}
  ...
  defp limit_report(%{label: {:gen_server, :terminate},
              last_message: msg, state: state, log: log,
              reason: reason, client_info: client} = report,
            depth) do
  ...
  defp limit_report(%{label: {:gen_server, :no_handle_info},
              message: msg} = report,
            depth) do
  ...
  defp format_log_single(%{label: {:gen_server, :terminate}, name: name,
              last_message: msg, state: state, log: log,
              reason: reason, client_info: client},
            %{single_line: true, depth: depth} = formatOpts) do
    p = p(formatOpts)
    format1 = :lists.append(['Generic server ', p, ' terminating. Reason: ', p, '. Last message: ', p, '. State: ', p, '.'])
    {serverLogFormat,
       serverLogArgs} = format_server_log_single(log,
                                                   formatOpts)
    {clientLogFormat,
       clientLogArgs} = format_client_log_single(client,
                                                   formatOpts)
    args1 = (case (depth) do
               :unlimited ->
                 [name, fix_reason(reason), msg, state]
               _ ->
                 [name, depth, fix_reason(reason), depth, msg, depth,
                                                                   state, depth]
             end)
  ...
  defp format_log_single(%{label: {:gen_server, :no_handle_info},
              module: mod, message: msg},
            %{single_line: true, depth: depth} = formatOpts) do
    p = p(formatOpts)
  ...
  defp format_log_multi(%{label: {:gen_server, :terminate}, name: name,
              last_message: msg, state: state, log: log,
              reason: reason, client_info: client},
    ...
    {clientFmt, clientArgs} = format_client_log(client,
                                                  formatOpts)
    p = p(formatOpts)
    format = :lists.append((['** Generic server ', p, ' terminating \n** Last message in was ', p, '~n** When Server state == ', p, '~n** Reason for termination ==~n** ', p,
                                                      '~n'] ++ (case (log) do
                                                               [] ->
                                                                 []
                                                               _ ->
    ...
    args = (case (depth) do
              ...
              _ ->
                [name, depth, msg, depth, state, depth, reason1,
                                                            depth] ++ (case (log) do
                                                                         [] ->
                                                                           []
                                                                         _ ->
  ...
  defp format_log_multi(%{label: {:gen_server, :no_handle_info},
              module: mod, message: msg},
            %{depth: depth} = formatOpts) do
    p = p(formatOpts)
  ...
  def format_status(opt, statusData) do
    [pDict, sysState, parent, debug, [name, state, cbCache,
                                                       _Time,
                                                           _HibernateAfterTimeout]] = statusData
    mod = r_callback_cache(cbCache, :module)
    header = :gen.format_status_header('Status for generic server', name)
    status = (case (:gen.format_status(mod, opt,
                                         %{state: state,
                                             log: :sys.get_log(debug)},
                                         [pDict, state])) do
                %{"EXIT": r} = m ->
                  Map.put(m, :"$status", [{:data, [{'State', r}]}])
                %{"$status": s} = m when is_list(s) ->
                  m
                %{"$status": s} = m ->
                  %{m | "$status": [s]}
                %{state: s} = m ->
                  Map.put(m, :"$status", [{:data, [{'State', s}]}])
              end)
    [{:header, header}, {:data,
                           [{'Status', sysState}, {'Parent', parent}, {'Logged events',
                                                           format_log_state(mod,
                                                                              :maps.get(:log,
                                                                                          status))}]} |
                            :maps.get(:"$status", status)]
