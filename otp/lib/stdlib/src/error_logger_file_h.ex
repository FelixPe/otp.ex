defmodule :m_error_logger_file_h do
  ...
  @behaviour :gen_event
  require Record
  Record.defrecord(:r_st, :st, fd: :undefined,
                              filename: :undefined, prev_handler: :undefined,
                              depth: :unlimited)
  ...
  defp init(file, prevHandler) do
    ...
    case (:file.open(file, [:write, {:encoding, :utf8}])) do
      {:ok, fd} ->
        ...
        state = r_st(fd: fd, filename: file,
                    prev_handler: prevHandler, depth: depth)
        {:ok, state}
      error ->
        error
    end
  ...
  def handle_info({:EXIT, fd, _Reason},
           r_st(fd: fd, prev_handler: prevHandler)) do
    case (prevHandler) do
      [] ->
        :remove_handler
      _ ->
        {:swap_handler, :install_prev, [], prevHandler,
           :go_back}
    end
  end
  ...
  defp parse_event({:error_report, _GL,
             {pid, :std_error, args}}) do
