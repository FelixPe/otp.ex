defmodule :m_edlin_type_suggestion do
  ...
  def type_tree(mod, funType, nestings, fT) do
    case :erlang.get({:type_traverser, mod, funType, nestings}) do
      :undefined ->
        res = type_traverser_cache(mod, funType, %{}, length(nestings) + 1, fT)
  ...
  defp type_traverser(mod, {:type, _, :bounded_fun, [fun, constraints]}, visited, level, fT) do
    cl =
      for x <- constraints do
  ...
  defp type_traverser(mod, {:type, _, :fun, [product, return]}, visited, level, fT) do
    p = type_traverser(mod, product, visited, level, fT)
    r = type_traverser(mod, return, visited, level, fT)
    {p, {:return, r}}
  end

  defp type_traverser(mod, {:type, _, :product, childs}, visited, level, fT) do
    cl =
      for x <- childs do
        type_traverser(mod, x, visited, level, fT)
      end
  ...
  defp type_traverser(
    ...
    {:constraint, type_traverser(mod, type1, visited, level, fT),
  ...
  defp type_traverser(_Mod, {:type, _, :map, :any}, _Visited, _Level, _FT) do
    {:type, :map, []}
  end

  defp type_traverser(mod, {:type, _, :map, params}, visited, level, fT) do
    {:map,
     for x <- params do
  ...
  defp type_traverser(mod, {:type, _, :map_field_exact, [type1, type2]}, visited, level, fT) do
    {:map_field_exact, type_traverser(mod, type1, visited, level, fT),
     type_traverser(mod, type2, visited, level, fT)}
  end

  defp type_traverser(mod, {:type, _, :map_field_assoc, [type1, type2]}, visited, level, fT) do
    {:map_field_assoc, type_traverser(mod, type1, visited, level, fT),
     type_traverser(mod, type2, visited, level, fT)}
  end
  ...
  defp type_traverser(mod, {:op, _, op, type1, type2}, visited, level, fT) do
    {:op, op, type_traverser(mod, type1, visited, level, fT),
     type_traverser(mod, type2, visited, level, fT)}
  end

  defp type_traverser(_Mod, {:integer, _, int}, _Visited, _Level, _FT) do
    {:integer, int}
  end

  defp type_traverser(mod, {:type, _, :list, [childType]}, visited, level, fT) do
    {:list, type_traverser(mod, childType, visited, level - 1, fT)}
  end

  defp type_traverser(_Mod, {:type, _, :tuple, :any}, _Visited, _Level, _FT) do
    {:type, :tuple, []}
  end

  defp type_traverser(mod, {:type, _, :tuple, childTypes}, visited, level, fT) do
    {:tuple,
     for x <- childTypes do
       type_traverser(mod, x, visited, level - 1, fT)
     end}
  ...
  defp type_traverser(mod, {:type, _, :union, childTypes}, visited, level, fT) do
    childs =
      for x <- childTypes do
        type_traverser(mod, x, visited, level, fT)
      end
    ...
    childsFiltered =
      for x <- childs, x != :undefined do
        x
      end

    {unionChilds, nonUnionChilds} =
      :lists.partition(
        fn x ->
          case x do
            {:union, _} ->
  ...
  defp type_traverser(mod, {:ann_type, _, [t1, t2]}, visited, level, fT) do
    {:ann_type, type_traverser(mod, t1, visited, level, fT),
     type_traverser(mod, t2, visited, level, fT)}
  end

  defp type_traverser(mod, {:user_type, _, name, params} = t, visited, level, fT)
       when 0 >= level do
    case :maps.is_key(strip_anno(t), visited) do
      false ->
        {:type, mod, name,
         for p <- params do
           type_traverser(mod, p, Map.put(visited, strip_anno(t), true), 0, fT)
         end}
  ...
  defp type_traverser(
         _,
         {:remote_type, _, [{_, _, mod}, {_, _, name}, params]} = t,
         visited,
         level,
         fT
       )
  ...
  defp type_traverser(mod, {:user_type, _, name, params} = t, visited, 1 = level, fT) do
    case :maps.is_key(strip_anno(t), visited) do
      false ->
        case :erlang.get({strip_anno(t), 1}) do
          :undefined ->
            res =
              case lookup_type(mod, name, length(params), fT) do
                :hidden ->
                  {:type, mod, name,
                   for p <- params do
                     type_traverser(mod, p, Map.put(visited, strip_anno(t), true), level, fT)
                   end}
                ...
                type ->
                  {:user_type, mod, name,
                   ...
                   type_traverser(mod, type, Map.put(visited, strip_anno(t), true), level, fT)}
              end
  ...
  defp type_traverser(mod, {:user_type, _, name, params} = t, visited, level, fT) do
    case :maps.is_key(strip_anno(t), visited) do
      false ->
  ...
  defp type_traverser(_, {:type, _, :record, [{:atom, _, record}]}, _Visited, _Level, _FT) do
    {:record, record}
  end
  ...
  defp type_traverser(_, {:type, _, name, params} = t, visited, level, fT) do
    case :maps.is_key(strip_anno(t), visited) do
      false ->
        case :erlang.get({strip_anno(t), 1}) do
          :undefined ->
            res =
              case lookup_type(:erlang, name, length(params), fT) do
                :hidden ->
                  {:type, name,
                   for p <- params do
                     type_traverser(:erlang, p, Map.put(visited, strip_anno(t), true), level, fT)
                   end}

                type ->
                  type_traverser(:erlang, type, Map.put(visited, strip_anno(t), true), level, fT)
              end
      ...
      :none ->
        case :code.get_doc(mod, %{sources: [:debug_info]}) do
          {:ok, r_docs_v1(docs: docs)} ->
            ...
            case fnFunctions do
              [] ->
                case (for {{:type, type2}, {:attribute, _, :type, {_, typeAST, _}}} <- fT,
                          type2 === type do
                        typeAST
                      end) do
                  [] ->
              ...
              [{_, _, _, _, %{signature: [{:attribute, _, :type, {_, typeAST, _}}]}}] ->
                typeAST
            end
  ...
  def get_function_type(mod, fun, arity, fT) do
    case :code.get_doc(mod, %{sources: [:debug_info]}) do
      {:ok, r_docs_v1(docs: docs)} ->
        r =
          :lists.flatten(
            for {{:function, f, a}, _Anno, _Sig, _Doc,
                 %{signature: [{:attribute, _, :spec, {_, funTypes}}]}} <- docs,
                f === fun,
                a === arity do
              funTypes
            end
        ...
        case {mod, r} do
          {:shell_default, []} ->
            :lists.flatten(
              for {{:function_type, {:shell_default, f, a}},
      ...
      _ when mod === :shell_default ->
        :lists.flatten(
          for {{:function_type, {:shell_default, f, a}}, {:attribute, _, :spec, {_, funTypes}}} <-
                fT,
              f === fun,
              a === arity do
            funTypes
  ...
  defp get_arity1({:list, t}, constraints, [{:list, _, _} | nestings]) do
    get_arity1(t, constraints, nestings)
  end

  defp get_arity1({:tuple, lT}, constraints, [{:tuple, args, _}])
       when length(lT) >= length(args) do
    case :edlin_expand.match_arguments1(lT, constraints, args) do
      true ->
        length(lT)
  ...
  defp get_arity1({:tuple, lT}, constraints, [{:tuple, args, _} | nestings])
       when length(lT) >= length(args) + 1 do
    case :edlin_expand.match_arguments1(lT, constraints, args) do
      true ->
        get_arity1(:lists.nth(length(args) + 1, lT), constraints, nestings)
  ...
  defp get_arity1({:map, types}, _Constraints, [{:map, _Keys, [], _, _}]) do
    length(types)
  end

  defp get_arity1({:map, types}, _Constraints, [{:map, _Keys, _Key, _, _}]) do
    length(types)
  end

  defp get_arity1({:map, types}, constraints, [{:map, keys, [], _, _} | nestings]) do
    :lists.flatten(
      for {_, key, _} = t <- types,
  ...
  defp get_arity1({:map, types}, constraints, [{:map, _Keys, key, _, _} | nestings]) do
  ...
  defp get_arity1({:ann_type, _Var, type}, constraints, nestings) do
    get_arity1(type, constraints, nestings)
  end

  defp get_arity1({:user_type, _, _, _, type}, constraints, nestings) do
    get_arity1(type, constraints, nestings)
  end
  ...
  defp get_atoms1({:list, t}, constraints, [{:list, _, _} | nestings]) do
    get_atoms1(t, constraints, nestings)
  end

  defp get_atoms1({:tuple, lT}, constraints, [{:tuple, args, _} | nestings])
       when length(lT) >= length(args) + 1 do
    case :edlin_expand.match_arguments1(lT, constraints, args) do
      true ->
        get_atoms1(:lists.nth(length(args) + 1, lT), constraints, nestings)
  ...
  defp get_atoms1({:map, types}, constraints, [{:map, keys, [], _, _} | nestings]) do
    :lists.flatten(
      for {_, key, _} = t <- types,
          not :lists.member(:erlang.atom_to_list(key), keys) do
        get_atoms1(t, constraints, nestings)
  ...
  defp get_atoms1({:map, types}, constraints, [{:map, _Keys, key, _, _} | nestings]) do
    case (for {_, k, v} <- types,
              k === :erlang.list_to_atom(key) do
            v
          end) do
  ...
  defp get_atoms1({:user_type, _, _, _, type}, constraints, nestings) do
    get_atoms1(type, constraints, nestings)
  end
  ...
  def get_types(constraints, t, nestings, options) do
    ...
    case get_types1(t, constraints, nestings, maxUserTypeExpansions, options) do
      [] ->
        []
  ...
  defp get_types1({:var, _Var} = c, constraints, nestings, maxUserTypeExpansions, options) do
    case get_constraint(c, constraints) do
      {:constraint, _, t} ->
        get_types1(t, constraints, nestings, maxUserTypeExpansions, options)
  ...
  defp get_types1({:union, types}, cs, nestings, maxUserTypeExpansions, options) do
    :lists.flatten(
      for t <- types do
  ...
  defp get_types1({:list, t}, cs, [{:list, _Args, _} | nestings], maxUserTypeExpansions, options) do
    get_types1(t, cs, nestings, maxUserTypeExpansions, options)
  end
  ...
  defp get_types1(
         ...
         maxUserTypeExpansions,
         options
       )
       when length(lT) >= length(args) + 1 do
    case :edlin_expand.match_arguments1(lT, cs, args) do
      true ->
        get_types1(:lists.nth(length(args) + 1, lT), cs, nestings, maxUserTypeExpansions, options)
  ...
  defp get_types1(
    ...
    case (for {_, k, v} <- types,
      ...
      [type] ->
        get_types1(type, cs, nestings, maxUserTypeExpansions, options)
    end
  end

  defp get_types1(
         {:user_type, _Mod, _Name, _Params, type},
         cs,
         nestings,
         maxUserTypeExpansions,
         [:no_print] = options
  ...
  defp get_types1({:user_type, _, _, _, type}, cs, nestings, 0, [:no_print] = options) do
    get_types1(type, cs, nestings, 0, options)
  end

  defp get_types1({:ann_type, _Var, t}, cs, nestings, maxUserTypeExpansions, [:no_print]) do
    get_types1(t, cs, nestings, maxUserTypeExpansions, [:no_print])
  end

  defp get_types1({:ann_type, _Var, _T} = type, cs, [], _MaxUserTypeExpansions, []) do
    {print_type(type, cs), ~c""}
  end

  defp get_types1({:ann_type, _Var, t}, cs, nestings, maxUserTypeExpansions, []) do
  ...
  defp get_types1({:user_type, mod, name, params, type}, cs, nestings, maxUserTypeExpansions, [])
       when maxUserTypeExpansions > 0 do
    title = print_type({:type, mod, name, params}, cs, [])
    elems = :lists.flatten([get_types1(type, cs, nestings, maxUserTypeExpansions - 1, [])])
    %{title: title, elems: elems, options: [{:separator, ~c" :: "}, {:highlight_all}]}
  end

  defp get_types1({:user_type, _, _, _, type}, cs, nestings, 0, []) do
  ...
  defp print_type({:var, name} = var, constraints, visited, options) do
  ...
  defp print_type({{:parameters, ps}, {:return, r}}, cs, v, options) do
    ~c"fun((" ++
      :lists.join(
        ~c", ",
        for x <- ps do
      ...
      ) ++ ~c") -> " ++ print_type(r, cs, v, options) ++ ~c")"
  end

  defp print_type({:list, type}, cs, v, options) do
    ~c"[" ++ print_type(type, cs, v, options) ++ ~c"]"
  end
  ...
  defp print_type({:tuple, types}, cs, v, options)
    ...
    case types1 do
      ...
      _ ->
        ~c"{" ++ :lists.nth(1, types1) ++ ~c", ...}"
    end
  end

  defp print_type({:ann_type, var, type}, cs, v, options) do
    print_type(var, cs, v, options) ++ ~c" :: " ++ print_type(type, cs, v, options)
  end
  ...
  defp print_type({:map, types}, cs, v, options) do
    ...
    ~c"\#{" ++ :lists.join(~c", ", types1) ++ ~c"}"
  end

  defp print_type({:map_field_assoc, type1, type2}, cs, v, options) do
    print_type(type1, cs, v, options) ++ ~c"=>" ++ print_type(type2, cs, v, options)
  end

  defp print_type({:map_field_exact, type1, type2}, cs, v, options) do
    print_type(type1, cs, v, options) ++ ~c":=" ++ print_type(type2, cs, v, options)
  end
  ...
  defp print_type({:op, op, type}, cs, v, options) do
    ~c"op (" ++ :erlang.atom_to_list(op) ++ ~c" " ++ print_type(type, cs, v, options) ++ ~c")"
  end

  defp print_type({:op, op, type1, type2}, cs, v, options) do
    ~c"op (" ++
      print_type(type1, cs, v, options) ++
      ~c" " ++
      :erlang.atom_to_list(op) ++
      ~c" " ++
      print_type(
        type2,
        cs,
  ...
  defp print_type({:record, record}, _Cs, _V, _) do
    ~c"#" ++ :erlang.atom_to_list(record)
  end

  defp print_type({:type, :range, [{:integer, int1}, {:integer, int2}]}, _Cs, _V, _) do
    :erlang.integer_to_list(int1) ++ ~c".." ++ :erlang.integer_to_list(int2)
  end
  ...
  defp print_type({:user_type, mod, name, params, type}, cs, v, options) do
    ...
    case first do
      ...
      _ ->
        print_type({:type, mod, name, params}, cs, v, options) ++
          ~c" :: " ++ print_type(type, cs, v, options)
    end
  end
  ...
  defp extract_param({:integer, value}) do
    :io_lib.format(~c"~p", [value])
  end

  defp extract_param({:type, type, _}) do
    :io_lib.format(~c"~p", [type])
  end
