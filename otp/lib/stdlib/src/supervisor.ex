defmodule :m_supervisor do
  ...
  Record.defrecord(:r_child, :child, pid: :undefined,
                                 id: :undefined, mfargs: :undefined,
                                 restart_type: :undefined,
                                 significant: :undefined, shutdown: :undefined,
                                 child_type: :undefined, modules: [])
  Record.defrecord(:r_state, :state, name: :undefined,
                                 strategy: :one_for_one, children: {[], %{}},
                                 dynamics: :undefined, intensity: 1, period: 5,
                                 restarts: [], dynamic_restarts: 0,
                                 auto_shutdown: :never, module: :undefined,
                                 args: :undefined)
  ...
  def start_link(supName, mod, args) do
    :gen_server.start_link(supName, :supervisor,
                             {supName, mod, args}, [])
  end
  ...
  defp init_children(state, startSpec) do
    ...
    case (check_startspec(startSpec,
                            r_state(state, :auto_shutdown))) do
      {:ok, children} ->
                     ...
                     case (:logger.allow(:error, :supervisor)) do
                       true ->
                         :erlang.apply(:logger, :macro_log,
                                         [%{mfa:
                                            {:supervisor, :start_children, 2},
                                              line: 398, file: 'otp/lib/stdlib/src/supervisor.erl'},
                                              :error, %{label:
                                                        {:supervisor,
                                                           :start_error},
                                                          report:
  ...
  def handle_call({:start_child, childSpec}, _From, state) do
    case (check_childspec(childSpec,
                            r_state(state, :auto_shutdown))) do
      {:ok, child} ->
  ...
  def handle_call({:terminate_child, id}, _From, state) when (not
                                                     is_pid(id) and
  ...
  def handle_call(:which_children, _From, state)
      when r_state(state, :strategy) === :simple_one_for_one do
    r_child(child_type: cT,
        modules: mods) = get_dynamic_child(state)
    reply = dyn_map(fn {:restarting, _} ->
  ...
  def handle_call(:which_children, _From, state) do
    resp = children_to_list(fn id,
                                 r_child(pid: {:restarting, _}, child_type: childType,
                                     modules: mods) ->
                                 {id, :restarting, childType, mods}
                               id,
                                 r_child(pid: pid, child_type: childType,
                                     modules: mods) ->
  ...
  def handle_call(:count_children, _From,
           r_state(dynamic_restarts: restarts) = state)
      when r_state(state, :strategy) === :simple_one_for_one do
    ...
    reply = (case (cT) do
               :supervisor ->
                 [{:specs, 1}, {:active, active}, {:supervisors, sz},
                                                      {:workers, 0}]
               :worker ->
                 [{:specs, 1}, {:active, active}, {:supervisors, 0},
                                                      {:workers, sz}]
             end)
    {:reply, reply, state}
  end
  ...
  def handle_call(:count_children, _From, state) do
    {specs, active, supers, workers} = children_fold(fn _Id,
                                                          child, counts ->
                                                          count_child(child,
                                                                        counts)
                                                     end,
  ...
  defp count_child(r_child(pid: pid, child_type: :worker),
            {specs, active, supers, workers}) do
  ...
  defp count_child(r_child(pid: pid, child_type: :supervisor),
            {specs, active, supers, workers}) do
    case (is_pid(pid) and :erlang.is_process_alive(pid)) do
      true ->
  ...
  def handle_info(msg, state) do
    case (:logger.allow(:error, :supervisor)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:supervisor, :handle_info, 2}, line: 623,
                             file: 'otp/lib/stdlib/src/supervisor.erl'},
                             :error, 'Supervisor received unexpected message: ~tp~n', [msg], %{domain: [:otp],
                                                   error_logger:
                                                   %{tag: :error}}])
      false ->
        :ok
  ...
  defp update_childspec1({[id | oldIds], oldDb}, {ids, db}, keepOld) do
    case (update_chsp(:maps.get(id, oldDb), db)) do
      {:ok, newDb} ->
        update_childspec1({oldIds, oldDb}, {ids, newDb},
                            keepOld)
      false ->
        update_childspec1({oldIds, oldDb}, {ids, db},
                            [id | keepOld])
    end
  end
  ...
  defp update_childspec1({[], oldDb}, {ids, db}, keepOld) do
    ...
    {:lists.reverse(ids ++ keepOld),
       :maps.merge(keepOldDb, db)}
  end
  ...
  defp update_chsp(r_child(id: id) = oldChild, newDb) do
    case (:maps.find(id, newDb)) do
      {:ok, child} ->
        {:ok,
           Map.put(newDb, id, r_child(child, pid: r_child(oldChild, :pid)))}
      :error ->
        false
    end
  end
    ...
    case (find_child(r_child(child, :id), state)) do
      :error ->
        case (do_start_child(r_state(state, :name), child)) do
          ...
          {:ok, pid, extra} ->
            {{:ok, pid, extra},
               save_child(r_child(child, pid: pid), state)}
  ...
  defp do_restart(reason, child, state)
    ...
    case (:logger.allow(:error, :supervisor)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:supervisor, :do_restart, 3}, line: 744,
                             file: 'otp/lib/stdlib/src/supervisor.erl'},
                             :error, %{label: {:supervisor, :child_terminated},
                                         report:
  ...
  defp do_restart(reason, child, state)
    ...
    case (:logger.allow(:error, :supervisor)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:supervisor, :do_restart, 3}, line: 756,
                             file: 'otp/lib/stdlib/src/supervisor.erl'},
                             :error, %{label: {:supervisor, :child_terminated},
                                         report:
                                         [{:supervisor, r_state(state, :name)},
  ...
  defp do_restart(reason, child, state)
    ...
    case (:logger.allow(:error, :supervisor)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:supervisor, :do_restart, 3}, line: 759,
                             file: 'otp/lib/stdlib/src/supervisor.erl'},
                             :error, %{label: {:supervisor, :child_terminated},
                                         report:
                                         [{:supervisor, r_state(state, :name)},
  ...
  defp do_auto_shutdown(child, state) when not
                             r_child(child, :significant) === true do
    {:ok, state}
  end

  defp do_auto_shutdown(_Child,
            state = r_state(auto_shutdown: :any_significant)) do
    {:shutdown, state}
  end
  ...
  defp do_auto_shutdown(_Child,
            state = r_state(auto_shutdown: :all_significant)) do
    case (children_any(fn _, r_child(pid: :undefined) ->
                            false
    ...
    case (add_restart(state)) do
      ...
      {:terminate, nState} ->
        case (:logger.allow(:error, :supervisor)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:supervisor, :restart, 2}, line: 813,
                                 file: 'otp/lib/stdlib/src/supervisor.erl'},
                                 :error, %{label: {:supervisor, :shutdown},
                                             report:
                                             [{:supervisor, r_state(state, :name)},
    ...
    case (do_start_child_i(m, f, a)) do
      ...
      {:error, error} ->
        ...
        case (:logger.allow(:error, :supervisor)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:supervisor, :restart, 3}, line: 840,
                                 file: 'otp/lib/stdlib/src/supervisor.erl'},
                                 :error, %{label: {:supervisor, :start_error},
                                             report:
    ...
    case (do_start_child(r_state(state, :name), child)) do
      ...
      {:error, reason} ->
        ...
        case (:logger.allow(:error, :supervisor)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:supervisor, :restart, 3}, line: 854,
                                 file: 'otp/lib/stdlib/src/supervisor.erl'},
                                 :error, %{label: {:supervisor, :start_error},
                                             report:
                                             [{:supervisor, r_state(state, :name)},
  ...
  defp restart(:rest_for_one, r_child(id: id) = child,
            r_state(name: supName) = state) do
    {chAfter, chBefore} = split_child(id,
                                        r_state(state, :children))
    {return, chAfter2} = restart_multiple_children(child,
                                                     chAfter, supName)
    {return, r_state(state, children: append(chAfter2, chBefore))}
  end

  defp restart(:one_for_all, child,
            r_state(name: supName) = state) do
    children1 = del_child(r_child(child, :id),
                            r_state(state, :children))
    {return, nChildren} = restart_multiple_children(child,
                                                      children1, supName)
  ...
  defp restart_multiple_children(child, children, supName) do
    ...
    case (start_children(children1, supName)) do
      {:ok, nChildren} ->
        {:ok, nChildren}
      {:error, nChildren,
         {:failed_to_start_child, failedId, _Reason}} ->
        newPid = (cond do
    ...
    case (shutdown(child)) do
      ...
      {:error, otherReason} ->
        case (:logger.allow(:error, :supervisor)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:supervisor, :do_terminate, 2}, line: 913,
                                 file: 'otp/lib/stdlib/src/supervisor.erl'},
                                 :error, %{label:
      ...
      {:DOWN, ^mon, :process, ^pid, reason0} ->
        case (unlink_flush(pid, reason0)) do
          :killed ->
            :ok
          :shutdown when not
                         (r_child(child, :restart_type) === :permanent)
                         ->
            :ok
          {:shutdown, _} when not
                              (r_child(child, :restart_type) === :permanent)
  ...
  defp terminate_dynamic_children(state) do
    child = get_dynamic_child(state)
    pids = dyn_fold(fn p, acc when is_pid(p) ->
                      ...
                      %{}, state)
    tRef = (case (r_child(child, :shutdown)) do
              :brutal_kill ->
                :undefined
              :infinity ->
                :undefined
                           ...
                           :erlang.apply(:logger, :macro_log,
                                           [%{mfa:
                                              {:supervisor,
                                                 ...
                                                 1},
                                                line: 1028, file: 'otp/lib/stdlib/src/supervisor.erl'},
                                                :error, %{label:
                                                          {:supervisor,
                                                             :shutdown_error},
                                                            report:
                                                                 ...
                                                                 {:errorContext,
                                                                    :shutdown_error},
                                                                     {:reason,
                                                                        reason},
                                                                         {:offender,
                                                                            extract_child(r_child(child, pid: ls))}]},
                                                            %{domain:
                                                              [:otp, :sasl],
                                                                report_cb:
                                                                &:supervisor.format_log/2,
  ...
  defp wait_dynamic_children(r_child(shutdown: :brutal_kill) = child, pids, sz,
            tRef, eStack) do
    receive do
      {:DOWN, mon, :process, pid, reason0}
  ...
  defp save_child(r_child(mfargs: {m, f, _}) = child, state)
      when r_child(child, :restart_type) === :temporary do
    do_save_child(r_child(child, mfargs: {m, f, :undefined}),
                    state)
  end
  ...
  defp do_save_child(r_child(id: id) = child,
            r_state(children: {ids, db}) = state) do
  ...
  defp del_child(child, state)
    ...
    nChildren = del_child(r_child(child, :id),
                            r_state(state, :children))
  ...
  defp split_child(id, {ids, db}) do
    ...
    {{idsAfter,
        Map.put(dbAfter, id, r_child(ch, pid: :undefined))},
       {idsBefore, dbBefore}}
  end
  ...
  defp find_child(pid, state) when (is_pid(pid) and
                              r_state(state, :strategy) === :simple_one_for_one) do
  ...
  defp find_child_by_pid(pid, r_state(children: {_Ids, db})) do
    fun = fn _Id, r_child(pid: p) = ch, _ when p === pid ->
               throw(ch)
             _, _, :error ->
               :error
          end
  ...
  defp children_map(fun, [id | ids], db, acc) do
    case (fun.(id, :maps.get(id, db))) do
      {:update, child} ->
        children_map(fun, ids, Map.put(db, id, child),
                       [id | acc])
      :remove ->
        children_map(fun, ids, :maps.remove(id, db), acc)
      {:abort, reason} ->
        {:error, {:lists.reverse(ids) ++ [id | acc], db},
           reason}
    end
  end
  ...
  defp children_any1(pred, {key, value, iter}) do
    pred.(key, value) or children_any1(pred,
                                         :maps.next(iter))
  end
  ...
  defp set_flags(flags, state) do
    try do
      ...
      %{strategy: strategy, intensity: maxIntensity,
          period: period, auto_shutdown: autoShutdown} ->
        {:ok,
           r_state(state, strategy: strategy,  intensity: maxIntensity, 
                      period: period,  auto_shutdown: autoShutdown)}
    end
  end
  ...
  defp check_flags({strategy, maxIntensity, period}) do
    check_flags(%{strategy: strategy,
                    intensity: maxIntensity, period: period,
                    auto_shutdown: :never})
  end
  ...
  defp do_check_flags(%{strategy: strategy, intensity: maxIntensity,
              period: period, auto_shutdown: autoShutdown} = flags) do
  ...
  defp validPeriod(period) when (is_integer(period) and
                          period > 0) do
    true
  end
  ...
  defp check_childspec({id, func, restartType, shutdown, childType,
             mods},
            autoShutdown) do
    check_childspec(%{id: id, start: func,
                        restart: restartType, significant: false,
                        shutdown: shutdown, type: childType, modules: mods},
                      autoShutdown)
  end
  ...
  defp do_check_childspec(%{restart: restartType,
              type: childType} = childSpec,
            autoShutdown) do
    id = (case (childSpec) do
            %{id: i} ->
              i
            _ ->
              throw(:missing_id)
          end)
    func = (case (childSpec) do
              %{start: f} ->
                f
              _ ->
                throw(:missing_start)
    ...
    validMods(mods)
    {:ok,
       r_child(id: id, mfargs: func, restart_type: restartType,
  ...
  defp validFunc({m, f, a}) when (is_atom(m) and is_atom(f) and
                             is_list(a)) do
    true
  end
  ...
  defp validShutdown(shutdown) when (is_integer(shutdown) and
                            shutdown >= 0) do
    true
  end
  ...
  defp child_to_spec(r_child(id: id, mfargs: func,
              restart_type: restartType, significant: significant,
              shutdown: shutdown, child_type: childType,
              modules: mods)) do
    %{id: id, start: func, restart: restartType,
        significant: significant, shutdown: shutdown,
        type: childType, modules: mods}
  end
  ...
  defp add_restart(restarts0, now, period) do
    treshold = now - period
    restarts1 = :lists.takewhile(fn r ->
                                      r >= treshold
                                 end,
                                   restarts0)
    [now | restarts1]
  ...
  defp extract_child(child) when is_list(r_child(child, :pid)) do
    [{:nb_children, length(r_child(child, :pid))}, {:id,
                                                r_child(child, :id)},
                                                 {:mfargs, r_child(child, :mfargs)},
                                                     {:restart_type,
                                                        r_child(child, :restart_type)},
                                                         {:significant,
                                                            r_child(child, :significant)},
                                                             {:shutdown,
                                                                r_child(child, :shutdown)},
                                                                 {:child_type,
                                                                    r_child(child, :child_type)}]
  end

  defp extract_child(child) do
    [{:pid, r_child(child, :pid)}, {:id, r_child(child, :id)}, {:mfargs,
                                                      r_child(child, :mfargs)},
                                                       {:restart_type,
                                                          r_child(child, :restart_type)},
                                                           {:significant,
                                                              r_child(child, :significant)},
  ...
  defp report_progress(child, supName) do
    case (:logger.allow(:info, :supervisor)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:supervisor, :report_progress, 2}, line: 1565,
                             file: 'otp/lib/stdlib/src/supervisor.erl'},
  ...
  defp limit_report(%{label: {:supervisor, _Error},
              report:
  ...
  defp format_log_single(%{label: {:supervisor, :progress},
    ...
    p = p(formatOpts)
    {childFormat,
       childArgs} = format_child_log_single(child, 'Started:')
    format = 'Supervisor: ' ++ p ++ '.'
    args = (case (depth) do
              :unlimited ->
                [supName]
  ...
  defp format_log_single(%{label: {:supervisor, _Error},
    ...
    {childFormat,
       childArgs} = format_child_log_single(child, 'Offender:')
    args = (case (depth) do
              :unlimited ->
                [supName, ctxt, reason]
              _ ->
  ...
  defp format_child_log_single(child, tag) do
    ...
    case (:lists.keyfind(:pid, 1, child)) do
      false ->
        {:nb_children, numCh} = :lists.keyfind(:nb_children, 1,
                                                 child)
  ...
  def format_status(_, [_PDict, state]) do
    [{:data, [{'State', state}]}, {:supervisor,
                               [{'Callback', r_state(state, :module)}]}]
  end
