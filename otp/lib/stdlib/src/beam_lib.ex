defmodule :m_beam_lib do
  use Bitwise
  import :lists, only: [append: 1, delete: 2, foreach: 2,
                          keysort: 2, member: 2, reverse: 1, sort: 1,
                          splitwith: 2]
  ...
  def md5(file) do
    case ((try do
      ...
      {:ok, {module, chunks0}} ->
        ...
        {:ok,
           {module,
              :erlang.md5(for {_Id, c} <- chunks do
                            c
                          end)}}
      error ->
        error
  ...
  def format_error({:chunk_too_big, file, chunkId, size, len}) do
    :io_lib.format('~tp: Size of chunk "~s" is ~p bytes, but only ~p bytes could be read~n', [file, chunkId, size, len])
  end
  ...
  def format_error({:missing_backend, file, backend}) do
    :io_lib.format('~tp: Cannot retrieve abstract code because the backend ~p is missing', [file, backend])
  end
  ...
  def make_crypto_key(:des3_cbc = type, string) do
    <<k1 :: size(8) - binary,
        k2 :: size(8) - binary>> = (first = :erlang.md5(string))
    <<k3 :: size(8) - binary,
        iVec :: size(8) - binary>> = :erlang.md5([first |
                                                      reverse(string)])
  ...
  defp compare_dirs(dir1, dir2) do
    ...
    {:sofs.to_external(onlyL1), :sofs.to_external(onlyL2),
       diff}
  end
  ...
  defp build_chunks([{id, data} | chunks]) do
    ...
    size = byte_size(data)
    chunk = [<<bId :: binary, size :: size(32)>>, data |
                                                      pad(size)]
  ...
  defp read_all_but_useless_chunks(file0) when is_atom(file0) or is_list(file0) or
                        is_binary(file0) do
    file = beam_filename(file0)
    {:ok, module, chunkIds0} = scan_beam(file, :info)
    chunkIds = (for {name, _, _} <- chunkIds0,
  ...
  defp read_significant_chunks(file, chunkList) do
    case (read_chunk_data(file, chunkList,
                            [:allow_missing_chunks])) do
      {:ok, {module, chunks0}} ->
        mandatory = mandatory_chunks()
        chunks = filter_significant_chunks(chunks0, mandatory,
                                             file, module)
        {:ok, {module, chunks}}
    end
  end
  ...
  defp filter_significant_chunks([{_, data} = pair | cs], mandatory, file, mod)
    ...
    [pair | filter_significant_chunks(cs, mandatory, file,
                                        mod)]
  end

  defp filter_significant_chunks([{id, :missing_chunk} | cs], mandatory, file,
            mod) do
  ...
  defp filter_funtab([{'FunT' = tag,
              <<l :: size(4) - binary, data0 :: binary>>} |
               cs]) do
  ...
  defp filter_funtab_1(<<important :: size(20) - binary,
              _OldUniq :: size(4) - binary, t :: binary>>,
            zero) do
  ...
  defp read_all_chunks(file0) when is_atom(file0) or is_list(file0) or
                        is_binary(file0) do
    try do
      file = beam_filename(file0)
      {:ok, module, chunkIds0} = scan_beam(file, :info)
      chunkIds = (for {name, _, _} <- chunkIds0 do
                    name
                  end)
      {:ok, ^module, chunks} = scan_beam(file, chunkIds)
      {:ok, module, :lists.reverse(chunks)}
  ...
  defp read_chunk_data(file0, chunkNames0, options)
      when is_atom(file0) or is_list(file0) or
             is_binary(file0) do
    file = beam_filename(file0)
    {chunkIds, names, optional} = check_chunks(chunkNames0,
                                                 file, [], [], [])
    allowMissingChunks = member(:allow_missing_chunks,
                                  options)
    {:ok, module, chunks} = scan_beam(file, chunkIds,
                                        allowMissingChunks, optional)
  ...
  defp check_chunks([chunkName | ids], file, iL, l, o)
    ...
    check_chunks(ids, file, [chunkId | iL],
                   [{chunkId, chunkName} | l], o)
  end
  ...
  defp scan_beam(file, what0, allowMissingChunks,
            optionalChunks) do
  ...
  defp scan_beam(fD, pos, what, mod, data) do
    case (pread(fD, pos, 8)) do
      ...
      {nFD,
         {:ok, <<idL :: size(4) - binary, sz :: size(32)>>}} ->
  ...
  defp get_data(cs, 'Atom' = id, fD, size, pos, pos2, _Mod, data) do
    get_atom_data(cs, id, fD, size, pos, pos2, data,
                    :latin1)
  end
  ...
  defp get_data(:info, id, fD, size, pos, pos2, mod, data) do
    scan_beam(fD, pos2, :info, mod,
                [{id, pos, size} | data])
  end

  defp get_data(chunks, id, fD, size, pos, pos2, mod, data) do
    {nFD, newData} = (case (member(id, chunks)) do
                        true ->
  ...
  defp get_chunk(id, pos, size, fD) do
    case (pread(fD, pos, size)) do
      ...
      {_NFD, :eof} when size > 0 ->
        :erlang.error({:chunk_too_big, filename(fD), id, size,
                         0})
  ...
  defp chunks_to_data([{:atom_chunk, name} | cNs], chunks, file, cs,
            module, atoms, l) do
    {newAtoms, ret} = chunk_to_data(name, "", file, cs,
                                      atoms, module)
    chunks_to_data(cNs, chunks, file, cs, module, newAtoms,
                     [ret | l])
  end

  defp chunks_to_data([{:abst_chunk, name} | cNs], chunks, file, cs,
            module, atoms, l) do
    dbgiChunk = :proplists.get_value('Dbgi', chunks, "")
    {newAtoms, ret} = (case ((try do
                               chunk_to_data(:debug_info, dbgiChunk, file, cs,
                                               atoms, module)
                             catch
                               :error, e -> {:EXIT, {e, __STACKTRACE__}}
                               :exit, e -> {:EXIT, e}
                         ...
                         {dbgiAtoms,
                           ...
                           try do
                             backend.debug_info(:erlang_v1, module, metadata,
                                                  [])
                           catch
                             :error, :undef ->
                               :erlang.error({:missing_backend, file, backend})
                           else
                             {:ok, code} ->
                               {dbgiAtoms,
                                  {:abstract_code, {:raw_abstract_v1, code}}}
                             {:error, _} ->
                               {dbgiAtoms, {:abstract_code, :no_abstract_code}}
                           end
                         {:error, :beam_lib,
                            {:key_missing_or_invalid, path, :debug_info}} ->
                           :erlang.error({:key_missing_or_invalid, path,
                                            :abstract_code})
                         _ ->
                           abstChunk = :proplists.get_value('Abst', chunks, "")
                           chunk_to_data(name, abstChunk, file, cs, atoms,
                                           module)
                       end)
    chunks_to_data(cNs, chunks, file, cs, module, newAtoms,
                     [ret | l])
  ...
  defp chunks_to_data([{id, name} | cNs], chunks, file, cs, module,
            atoms, l) do
    {_Id, chunk} = :lists.keyfind(id, 1, chunks)
    {newAtoms, ret} = chunk_to_data(name, chunk, file, cs,
                                      atoms, module)
    chunks_to_data(cNs, chunks, file, cs, module, newAtoms,
                     [ret | l])
  end
  ...
  defp chunk_to_data(id, :missing_chunk, _File, _Cs, atomTable,
            _Mod) do
    {atomTable, {id, :missing_chunk}}
  end

  defp chunk_to_data(:attributes = id, chunk, file, _Cs, atomTable,
            _Mod) do
    try do
      ...
      :error, :badarg ->
        :erlang.error({:invalid_chunk, file,
                         chunk_name_to_id(id, file)})
    end
  end

  defp chunk_to_data(:compile_info = id, chunk, file, _Cs, atomTable,
            _Mod) do
    try do
  ...
  defp chunk_to_data(:debug_info = id, chunk, file, _Cs, atomTable,
            mod) do
    case (chunk) do
      <<>> ->
        {atomTable, {id, :no_debug_info}}
      <<0 :: size(8), n :: size(8), mode0 :: size(n) - binary,
          rest :: binary>> ->
  ...
  defp chunk_to_data(:abstract_code = id, chunk, file, _Cs,
            atomTable, mod) do
    case (chunk) do
      <<>> ->
  ...
  defp chunk_to_data(:atoms = id, _Chunk, _File, cs, atomTable0,
            _Mod) do
  ...
  defp chunk_to_data(chunkName, chunk, file, cs, atomTable, _Mod)
    ...
    case ((try do
      ...
      {:EXIT, _} ->
        :erlang.error({:invalid_chunk, file,
                         chunk_name_to_id(chunkName, file)})
    end
  end
  ...
  defp attributes(l, r) do
    ...
    {l1, l2} = splitwith(fn t ->
                              :erlang.element(1, t) === k
                         end,
                           l)
    v = append(for {_, a} <- l1 do
                 a
               end)
    attributes(l2, [{k, v} | r])
  end

  defp symbols(<<_Num :: size(32), b :: binary>>, aT0, cs,
            name) do
    aT = ensure_atoms(aT0, cs)
    symbols1(b, aT, name, [], 1)
  end

  defp symbols1(<<i1 :: size(32), i2 :: size(32),
              i3 :: size(32), b :: binary>>,
            aT, name, s, cnt) do
  ...
  defp extract_atoms(<<_Num :: size(32), b :: binary>>, aT,
            encoding) do
  ...
  require Record
  Record.defrecord(:r_bb, :bb, pos: 0, bin: :undefined,
                              source: :undefined)
  ...
  defp read_all(fd, fileName, bins) do
    case (:file.read(fd, 1 <<< 18)) do
      ...
      :eof ->
        :ok = :file.close(fd)
        r_bb(bin: maybe_uncompress(reverse(bins)),
            source: fileName)
      error ->
        :ok = :file.close(fd)
        file_error(fileName, error)
  ...
  defp pread(fD, atPos, size) do
    ...
    skip = atPos - pos
    case (binary) do
      <<_ :: size(skip) - binary, b :: size(size) - binary,
          bin :: binary>> ->
  ...
  defp decrypt_chunk(type, module, file, id, bin) do
    try do
      keyString = get_crypto_key({:debug_info, type, module,
                                    file})
      {^type, key, iVec, _BlockSize} = make_crypto_key(type,
                                                         keyString)
      :ok = start_crypto()
      newBin = :crypto.crypto_one_time(:des_ede3_cbc, key,
                                         iVec, bin, false)
  ...
  defp old_anno_from_term({tag, forms}) when tag === :abstract_v1 or
                               tag === :abstract_v2 do
    try do
  ...
  defp anno_from_term({:debug_info_v1 = tag1,
             :erl_abstract_code = tag2, {forms, opts}}) do
    try do
  ...
  defp call_crypto_server(req) do
    try do
      :gen_server.call(:beam_lib__crypto_key_server, req,
                         :infinity)
    catch
      :exit, {:noproc, _} ->
  ...
  def handle_call({:get_crypto_key, _} = r, from,
           r_state(crypto_key_f: :undefined) = s) do
  ...
  def handle_call({:get_crypto_key, what}, from,
           r_state(crypto_key_f: f) = s) do
    try do
      result = f.(what)
  ...
  defp f_p_s(p, f) do
    case (:file.path_script(p, f)) do
      ...
      {:error, {line, _Mod, _Term} = e} ->
        :erlang.error('file:path_script(~tp,~tp): error on line ~p: ~ts~n', [p, f, line, :file.format_error(e)])
        :ok
  ...
  defp try_load_crypto_fun(keyInfo) when is_list(keyInfo) do
    ...
    foreach(fn {:debug_info, mode, m, key} when is_atom(m)
                                                ->
              ...
              keyInfo)
    fn {:debug_info, mode, m, f} ->
         alt_lookup_key([{:debug_info, mode, m, f}, {:debug_info,
                                                       mode, m, []},
                                                        {:debug_info, mode, [],
                                                           []}],
                          t)
