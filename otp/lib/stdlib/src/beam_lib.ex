defmodule :m_beam_lib do
  ...
  def format_error({:unknown_chunk, file, chunkName}) do
    :io_lib.format(~c"~tp: Cannot find chunk ~p~n", [file, chunkName])
  end

  def format_error({:invalid_chunk, file, chunkId}) do
    :io_lib.format(~c"~tp: Invalid contents of chunk ~p~n", [file, chunkId])
  end

  def format_error({:not_a_beam_file, file}) do
    :io_lib.format(~c"~tp: Not a BEAM file~n", [file])
  end

  def format_error({:file_error, file, reason}) do
    :io_lib.format(~c"~tp: ~tp~n", [file, :file.format_error(reason)])
  end

  def format_error({:missing_chunk, file, chunkId}) do
    :io_lib.format(~c"~tp: Not a BEAM file: no IFF \"~s\" chunk~n", [file, chunkId])
  end

  def format_error({:invalid_beam_file, file, pos}) do
    :io_lib.format(~c"~tp: Invalid format of BEAM file near byte number ~p~n", [file, pos])
  end

  def format_error({:chunk_too_big, file, chunkId, size, len}) do
    :io_lib.format(~c"~tp: Size of chunk \"~s\" is ~p bytes, but only ~p bytes could be read~n", [
      file,
      chunkId,
      size,
      len
  ...
  def format_error({:chunks_different, id}) do
    :io_lib.format(~c"Chunk \"~s\" differs in the two files~n", [id])
  end

  def format_error(:different_chunks) do
    ~c"The two files have different chunks\n"
  end

  def format_error({:modules_different, module1, module2}) do
    :io_lib.format(~c"Module names ~p and ~p differ in the two files~n", [module1, module2])
  end

  def format_error({:not_a_directory, name}) do
    :io_lib.format(~c"~tp: Not a directory~n", [name])
  end

  def format_error({:key_missing_or_invalid, file, chunkId}) do
    :io_lib.format(~c"~tp: Cannot decrypt ~ts because key is missing or invalid", [file, chunkId])
  end

  def format_error(:badfun) do
    ~c"not a fun or the fun has the wrong arity"
  end

  def format_error(:exists) do
    ~c"a fun has already been installed"
  end

  def format_error({:missing_backend, file, backend}) do
    :io_lib.format(~c"~tp: Cannot retrieve abstract code because the backend ~p is missing", [
      file,
      backend
    ])
  end
  ...
  defp diff_only(dir, only) do
    :io.format(~c"Only in ~tp: ~tp~n", [dir, only])
  end
  ...
  defp beam_files(dir) do
    ...
    l = :filelib.wildcard(:filename.join(dir, ~c"*.beam"))
  ...
  defp strip_rel(root, additionalChunks) do
    ...
    strip_fils(
      :filelib.wildcard(:filename.join(root, ~c"lib/*/ebin/*.beam")),
      additionalChunks
    )
  end
  ...
  defp is_useless_chunk(~c"CInf") do
    true
  end
  ...
  defp filter_funtab([
         {~c"FunT" = tag, <<l::size(4)-binary, data0::binary>>}
         | cs
       ]) do
  ...
  defp check_chunks([:atoms | ids], file, iL, l, o) do
    check_chunks(ids, file, [~c"Atom", ~c"AtU8" | iL], [{:atom_chunk, :atoms} | l], [
      ~c"Atom",
      ~c"AtU8" | o
    ])
  end
  ...
  defp check_chunks([:abstract_code | ids], file, iL, l, o) do
    check_chunks(ids, file, [~c"Abst", ~c"Dbgi" | iL], [{:abst_chunk, :abstract_code} | l], [
      ~c"Abst",
      ~c"Dbgi" | o
    ])
  end
  ...
  defp scan_beam(fD, pos, what, mod, data) do
    case pread(fD, pos, 8) do
      {_NFD, :eof} when mod === 17 ->
        :erlang.error({:missing_chunk, filename(fD), ~c"Atom"})
  ...
  defp get_data(cs, ~c"Atom" = id, fD, size, pos, pos2, _Mod, data) do
    get_atom_data(cs, id, fD, size, pos, pos2, data, :latin1)
  end

  defp get_data(cs, ~c"AtU8" = id, fD, size, pos, pos2, _Mod, data) do
  ...
  defp chunks_to_data([{:abst_chunk, name} | cNs], chunks, file, cs, module, atoms, l) do
    dbgiChunk = :proplists.get_value(~c"Dbgi", chunks, "")
    ...
    {newAtoms, ret} =
      case (try do
        ...
        _ ->
          abstChunk = :proplists.get_value(~c"Abst", chunks, "")
  ...
  defp ensure_atoms({:empty, aT}, cs) do
    case :lists.keyfind(~c"AtU8", 1, cs) do
      {_Id, atomChunk} when is_binary(atomChunk) ->
        extract_atoms(atomChunk, aT, :utf8)

      _ ->
        {_Id, atomChunk} = :lists.keyfind(~c"Atom", 1, cs)
  ...
  defp beam_filename(file) do
    :filename.rootname(file, ~c".beam") ++ ~c".beam"
  end
  ...
  def significant_chunks() do
    [~c"Line", ~c"Type" | md5_chunks()]
  end

  defp md5_chunks() do
    [~c"Atom", ~c"AtU8", ~c"Code", ~c"StrT", ~c"ImpT", ~c"ExpT", ~c"FunT", ~c"LitT", ~c"Meta"]
  end

  defp mandatory_chunks() do
    [~c"Code", ~c"ExpT", ~c"ImpT", ~c"StrT"]
  end
  ...
  defp crypto_key_fun_from_file() do
    userConfig = :filename.basedir(:user_config, ~c"erlang")

    case :init.get_argument(:home) do
      {:ok, [[home]]} ->
        crypto_key_fun_from_file_1([~c".", home, userConfig])

      _ ->
        crypto_key_fun_from_file_1([~c".", userConfig])
    end
  end

  defp crypto_key_fun_from_file_1(path) do
    case f_p_s(path, ~c".erlang.crypt") do
      {:ok, keyInfo, _} ->
  ...
  defp f_p_s(p, f) do
    case :file.path_script(p, f) do
      ...
      {:error, {line, _Mod, _Term} = e} ->
        :erlang.error(~c"file:path_script(~tp,~tp): error on line ~p: ~ts~n", [
          p,
          f,
          line,
          :file.format_error(e)
      ...
      {:error, e} when is_atom(e) ->
        :erlang.error(~c"file:path_script(~tp,~tp): ~ts~n", [p, f, :file.format_error(e)])
        :ok
  ...
  defp try_load_crypto_fun(keyInfo) when is_list(keyInfo) do
    ...
    foreach(
      fn
        ...
        other ->
          :erlang.error(~c"unknown key: ~p~n", [other])
      end,
      keyInfo
    )
  ...
  defp try_load_crypto_fun(keyInfo) do
    :erlang.error(~c"unrecognized crypto key info: ~p\n", [keyInfo])
  end
