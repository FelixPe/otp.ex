defmodule :m_shell do
  ...
  def start(noCtrlG, startSync) do
    ...
    ancestors = [self() | case (:erlang.get(:"$ancestors")) do
                            :undefined ->
                              []
                            anc ->
                              anc
  ...
  def start_restricted(rShMod) when is_atom(rShMod) do
    case (:code.ensure_loaded(rShMod)) do
      ...
      {:error, what} = error ->
        :error_logger.error_report(:lists.flatten(:io_lib.fwrite('Restricted shell module ~w not found: ~tp\n',
                                                                   [rShMod,
                                                                        what])))
        error
    end
  ...
  def server(startSync) do
    ...
    rShErr = (case (:application.get_env(:stdlib,
                                           :restricted_shell)) do
    ...
    jCL = (case (:erlang.get(:no_control_g)) do
             true ->
               ' (type help(). for help)'
             _ ->
               ' (press Ctrl+G to abort, type help(). for help)'
           end)
    defaultSessionSlogan = :io_lib.format("Eshell V~s",
                                            [:erlang.system_info(:version)])
    sessionSlogan = (case (:application.get_env(:stdlib,
                                                  :shell_session_slogan,
                                                  defaultSessionSlogan)) do
    ...
    server_loop(0, start_eval(bs, rT, fT, []), bs, rT, fT,
                  [], history, results)
  end

  defp server_loop(n0, eval_0, bs00, rT, fT, ds00, history0,
            results0) do
    n = n0 + 1
    {eval_1, bs0, ds0, prompt} = prompt(n, eval_0, bs00, rT,
                                          fT, ds00)
    {res, eval0} = get_command(prompt, eval_1, bs0, rT, fT,
                                 ds0)
    case (res) do
      {:ok, es0} ->
        case (expand_hist(es0, n)) do
          {:ok, es} ->
            {v, eval, bs, ds} = shell_cmd(es, eval0, bs0, rT, fT,
                                            ds0, :cmd)
            {history, results} = check_and_get_history_and_results()
            add_cmd(n, es, v)
            hB1 = del_cmd(:command, n - history, n - history0,
                            false)
          ...
          {:error, e} ->
            ...
            server_loop(n0, eval0, bs0, rT, fT, ds0, history0,
                          results0)
        end
      ...
      {:error, :interrupted} ->
        ...
        server_loop(n0, eval, bs0, rT, fT, ds0, history0,
                      results0)
      {:error, :tokens} ->
  ...
  defp get_command(prompt, eval, bs, rT, fT, ds) do
    ...
    parse = fn () ->
                 :erlang.put(:"$ancestors", ancestors)
                 exit(case (:io.scan_erl_exprs(:erlang.group_leader(),
                                                 prompt, {1, 1},
                                                 [:text, {:reserved_word_fun,
                                                            resWordFun}])) do
                        {:ok, toks, _EndPos} ->
                          case (toks) do
                            [{:-, _}, {:atom, _, atom} | _] ->
                              specialCase = fn localFunc ->
                                                 ...
                                                 {:done, {:ok, fakeResult, _},
                                                    _} = :erl_scan.tokens([],
                                                                            :erlang.atom_to_list(localFunc) ++ '("' ++ fakeLine ++ '").\n',
                                                                            {1,
                                                                               1},
                                                                            [:text,
                                                                                 {:reserved_word_fun,
                                ...
                                {:ok, funDef} ->
                                  case ({:edlin_expand.shell_default_or_bif(:erlang.atom_to_list(funName)),
                                    ...
                                    {'user_defined', false} ->
                                      ...
                                      {:done, {:ok, fakeResult, _},
                                         _} = :erl_scan.tokens([],
                                                                 'fd(' ++ :erlang.atom_to_list(funName) ++ ', ' ++ fakeLine ++ ').\n',
                                                                 {1, 1},
                                                                 [:text,
                                                                      {:reserved_word_fun,
                                                                         &:erl_scan.reserved_word/1}])
  ...
  defp reconstruct1({:function, anno, name, arity, clauses},
            name) do
    {:named_fun, anno, :RecursiveFuncVar,
       reconstruct1(clauses, name, arity)}
  end

  defp reconstruct1([{:call, anno, {:atom, anno1, name}, args} |
               body],
            name, arity)
      when length(args) === arity do
    [{:call, anno, {:var, anno1, :RecursiveFuncVar},
  ...
  defp reconstruct1([{:call, anno, {:atom, anno1, name}, args} |
            ...
            name, arity) do
    [{:call, anno,
        {:remote, anno1, {:atom, anno1, :shell_default},
           {:atom, anno1, name}},
        reconstruct1(args, name, arity)} |
         reconstruct1(body, name, arity)]
  end

  defp reconstruct1([{:call, anno, {:atom, anno1, fun}, args} |
               body],
            name, arity) do
    case ({:edlin_expand.shell_default_or_bif(:erlang.atom_to_list(fun)),
             :shell.local_func(fun)}) do
      {'user_defined', false} ->
        [{:call, anno,
            {:remote, anno1, {:atom, anno1, :shell_default},
               {:atom, anno1, fun}},
      ...
      {'erlang', false} ->
        [{:call, anno,
            {:remote, anno1, {:atom, anno1, :erlang},
               {:atom, anno1, fun}},
            reconstruct1(args, name, arity)} |
             reconstruct1(body, name, arity)]
      {_, true} ->
        [{:call, anno, {:atom, anno1, fun},
            reconstruct1(args, name, arity)} |
             reconstruct1(body, name, arity)]
    end
  end
  ...
  defp reconstruct1([e | body], name, arity) when is_tuple(e) do
    [:erlang.list_to_tuple(reconstruct1(:erlang.tuple_to_list(e),
                                          name, arity)) |
         reconstruct1(body, name, arity)]
  end

  defp reconstruct1([e | body], name, arity) when is_list(e) do
    [reconstruct1(e, name, arity) | reconstruct1(body, name,
                                                   arity)]
  end
  ...
  defp get_command1(pid, eval, bs, rT, fT, ds) do
    receive do
      ...
      {:EXIT, ^eval, {reason, stacktrace}} ->
        ...
        get_command1(pid, start_eval(bs, rT, fT, ds), bs, rT,
                       fT, ds)
      {:EXIT, ^eval, reason} ->
  ...
  defp prompt(n, eval0, bs0, rT, fT, ds0) do
    case (get_prompt_func()) do
      {m, f} ->
        a = :erl_anno.new(1)
        l = {:cons, a,
               {:tuple, a, [{:atom, a, :history}, {:integer, a, n}]},
               {nil, a}}
        c = {:call, a,
               {:remote, a, {:atom, a, m}, {:atom, a, f}}, [l]}
        {v, eval, bs, ds} = shell_cmd([c], eval0, bs0, rT, fT,
                                        ds0, :pmt)
        {eval, bs, ds,
           case (v) do
             {:pmt, val} ->
  ...
  defp get_prompt_func() do
    case (:application.get_env(:stdlib, :shell_prompt_func,
                                 :default)) do
  ...
  defp expand_expr({:map_field_assoc, a, k, v}, c) do
    {:map_field_assoc, a, expand_expr(k, c),
       expand_expr(v, c)}
  end

  defp expand_expr({:map_field_exact, a, k, v}, c) do
    {:map_field_exact, a, expand_expr(k, c),
       expand_expr(v, c)}
  end
  ...
  defp expand_expr({:record_field, a, r, name, f}, c) do
    {:record_field, a, expand_expr(r, c), name,
       expand_expr(f, c)}
  end

  defp expand_expr({:record, a, r, name, ups}, c) do
    {:record, a, expand_expr(r, c), name,
       expand_fields(ups, c)}
  end
  ...
  defp expand_expr({:receive, a, cs, to, toEs}, c) do
    {:receive, a, expand_cs(cs, c), expand_expr(to, c),
       expand_exprs(toEs, c)}
  end
  ...
  defp expand_cs([{:clause, a, p, g, b} | cs], c) do
    [{:clause, a, p, g, expand_exprs(b, c)} | expand_cs(cs,
                                                          c)]
  end
  ...
  defp expand_fields([{:record_field, a, f, v} | fs], c) do
    [{:record_field, a, expand_expr(f, c),
        expand_expr(v, c)} |
         expand_fields(fs, c)]
  end
  ...
  defp expand_quals([{:generate, a, p, e} | qs], c) do
    [{:generate, a, p, expand_expr(e, c)} | expand_quals(qs,
                                                           c)]
  end
  ...
  defp no_command(n) do
    throw({:error,
             :io_lib.fwrite("~ts: command not found", [:erl_pp.expr(n, enc())])})
  end
  ...
  defp getc(n) do
    {:erlang.get({:command, n}), :erlang.get({:result, n}),
       n}
  end
  ...
  def get_state() do
    ...
    receive do
      {:shell_state, bs, rT, fT} ->
        r_shell_state(bindings: bs, records: :ets.tab2list(rT),
            functions: :ets.tab2list(fT))
    end
  end
  ...
  def get_function(func, arity) do
    ...
    try do
      {:value, {_, fun}} = :lists.keysearch({:function,
                                               {:shell_default, func, arity}},
                                              1, fT)
      fun
    catch
      _, _ ->
  ...
  defp shell_rep(ev, bs0, rT, fT, ds0) do
    receive do
      {:shell_rep, ^ev, {:value, v, bs, ds}} ->
        {v, ev, bs, ds}
      {:shell_rep, ^ev,
         {:command_error, {location, m, error}}} ->
        fwrite_severity(:benign, "~s: ~ts",
      ...
      {:ev_exit, {^ev, class, reason0}} ->
        ...
        {{:EXIT, reason}, start_eval(bs0, rT, fT, ds0), bs0,
           ds0}
  ...
  defp report_exception(class, severity, {reason, stacktrace}, rT) do
    ...
    enc = encoding()
    str = :erl_error.format_exception(i, class, reason,
                                        stacktrace, sF, pF, enc)
    :io.requests([{:put_chars, :latin1, tag}, {:put_chars,
                                                 :unicode, str},
                                                  :nl])
  end
  ...
  defp restricted_eval_loop(shell, bs0, rT, fT, rShMod) do
    receive do
      {:shell_cmd, ^shell, {:eval, es}, w} ->
        ...
        :erlang.put(:restricted_expr_state, [])
        bs = eval_exprs(es, shell, bs0, rT, {:eval, lFH},
                          {:value, nLFH}, w)
        restricted_eval_loop(shell, bs, rT, fT, rShMod)
    end
  end
  ...
  defp do_catch(_Class, _Reason) do
    case (:application.get_env(:stdlib,
                                 :shell_catch_exception, false)) do
      true ->
        true
      _ ->
        false
  ...
  defp exprs([e0 | es], bs1, rT, lf, ef, bs0, w) do
    ...
    case (check_command(prep_check([e0]), rBs)) do
      :ok ->
        ...
        bs = :orddict.from_list(for ({x,
                                        _} = vV) <- :erl_eval.bindings(bs2),
  ...
  defp used_records(e, u0, rT, skip) do
    case (used_records(e)) do
      {:name, name, e1} ->
        u = (case (:lists.member(name, skip)) do
               true ->
                 u0
               false ->
      ...
      {:expr, [e1 | es]} ->
        used_records(es, used_records(e1, u0, rT, skip), rT,
                       skip)
      _ ->
        u0
    end
  ...
  defp local_allowed(f, as, rShMod, bs, shell, rT, fT)
    ...
    case (not_restricted(f, as)) do
      true ->
        local_func(f, as, bs, shell, rT, fT, {:eval, lFH},
                     {:value, nLFH})
      false ->
        {asEv, bs1} = expr_list(as, bs, {:eval, lFH},
                                  {:value, nLFH})
        case (rShMod.local_allowed(f, asEv,
                                     {:erlang.get(:restricted_shell_state),
                                        :erlang.get(:restricted_expr_state)})) do
  ...
  defp non_local_allowed(mForFun, as, rShMod, shell) do
    case (rShMod.non_local_allowed(mForFun, as,
                                     {:erlang.get(:restricted_shell_state),
                                        :erlang.get(:restricted_expr_state)})) do
      {result, {rShShSt, rShExprSt}} ->
        :erlang.put(:restricted_shell_state, rShShSt)
  ...
  defp prep_check({:call, anno, {:atom, _, :f},
             [{:var, _, _Name}]}) do
    {:atom, anno, :ok}
  end
  ...
  defp expand_records(usedRecords, e0) do
    recordDefs = (for {_Name, def__} <- usedRecords do
                    def__
                  end)
    a = :erl_anno.new(1)
    e = prep_rec(e0)
  ...
  defp prep_rec({:atom, {:value, _CommandN, _V} = value,
             :ok}) do
    value
  end
  ...
  def local_func() do
    [:v, :h, :b, :f, :fl, :rd, :rf, :rl, :rp, :rr, :history,
                                                       :results,
                                                           :catch_exception]
  end
  ...
  defp local_func(:v, [{:integer, _, v}], bs, shell, _RT, _FT,
            _Lf, _Ef) do
  ...
  defp local_func(:h, [], bs, shell, rT, _FT, _Lf, _Ef) do
    ...
    cs1 = :lists.filter(fn {{:command, _}, _} ->
                             true
                           {{:result, _}, _} ->
                             true
                           _ ->
  ...
  defp local_func(:f, [{:var, _, name}], bs, _Shell, _RT, _FT,
            _Lf, _Ef) do
    {:value, :ok, :erl_eval.del_binding(name, bs)}
  end

  defp local_func(:f, [_Other], _Bs, _Shell, _RT, _FT, _Lf,
            _Ef) do
  ...
  defp local_func(:fd, [{:atom, _, funName}, funExpr], bs, _Shell,
            _RT, fT, _Lf, _Ef) do
  ...
  defp local_func(:ft, [{:string, _, typeDef}], bs, _Shell, _RT,
            fT, _Lf, _Ef) do
    case (:erl_scan.tokens([], typeDef, {1, 1},
                             [:text, {:reserved_word_fun,
                                        &:erl_scan.reserved_word/1}])) do
      {:done, {:ok, toks, _}, _} ->
        case (:erl_parse.parse_form(toks)) do
          {:ok,
             {:attribute, _, :spec,
                {{funName, arity}, _}} = attrForm} ->
            :ets.insert(fT,
                          [{{:function_type, {:shell_default, funName, arity}},
                              attrForm}])
            {:value, :ok, bs}
  ...
  defp local_func(:td, [{:string, _, typeDef}], bs, _Shell, _RT,
            fT, _Lf, _Ef) do
    case (:erl_scan.tokens([], typeDef, {1, 1},
                             [:text, {:reserved_word_fun,
                                        &:erl_scan.reserved_word/1}])) do
      {:done, {:ok, toks, _}, _} ->
        case (:erl_parse.parse_form(toks)) do
          {:ok,
             {:attribute, _, :type, {typeName, _, _}} = attrForm} ->
  ...
  defp local_func(:rd, [{:string, _, typeDef}], bs, _Shell, rT,
            _FT, _Lf, _Ef) do
    case (:erl_scan.tokens([], typeDef, {1, 1},
                             [:text, {:reserved_word_fun,
                                        &:erl_scan.reserved_word/1}])) do
  ...
  defp local_func(:rd, [{:atom, _, recName0}, recDef0], bs,
            _Shell, rT, _FT, _Lf, _Ef) do
  ...
  defp local_func(:rl, [a], bs0, _Shell, rT, _FT, lf, ef) do
    {[recs], bs} = expr_list([a], bs0, lf, ef)
    {:value, list_records(record_defs(rT, listify(recs))),
       bs}
  end
  ...
  defp local_func(:rr, [_, _] = as0, bs0, _Shell, rT, _FT, lf,
            ef) do
    {[file, sel], bs} = expr_list(as0, bs0, lf, ef)
    {:value, read_and_add_records(file, sel, [], bs, rT),
       bs}
  end

  defp local_func(:rr, [_, _, _] = as0, bs0, _Shell, rT, _FT, lf,
            ef) do
    {[file, sel, options], bs} = expr_list(as0, bs0, lf, ef)
    {:value,
       read_and_add_records(file, sel, options, bs, rT), bs}
  end

  defp local_func(:history, [{:integer, _, n}], bs, _Shell, _RT,
            _FT, _Lf, _Ef) do
    {:value, history(n), bs}
  end

  defp local_func(:history, [_Other], _Bs, _Shell, _RT, _FT, _Lf,
            _Ef) do
    :erlang.raise(:error, :function_clause,
                    [{:shell, :history, 1}])
  end

  defp local_func(:results, [{:integer, _, n}], bs, _Shell, _RT,
            _FT, _Lf, _Ef) do
    {:value, results(n), bs}
  end

  defp local_func(:results, [_Other], _Bs, _Shell, _RT, _FT, _Lf,
            _Ef) do
    :erlang.raise(:error, :function_clause,
                    [{:shell, :results, 1}])
  end

  defp local_func(:catch_exception, [{:atom, _, bool}], bs,
            _Shell, _RT, _FT, _Lf, _Ef)
      when bool or not bool do
    {:value, catch_exception(bool), bs}
  end

  defp local_func(:catch_exception, [_Other], _Bs, _Shell, _RT,
            _FT, _Lf, _Ef) do
    :erlang.raise(:error, :function_clause,
  ...
  defp non_builtin_local_func(f, as, bs, fT) do
    arity = length(as)
    case (:erlang.function_exported(:user_default, f,
                                      arity)) do
      true ->
        {:eval, :erlang.make_fun(:user_default, f, arity), as,
           bs}
      false ->
  ...
  defp shell_default_local_func(f, as, bs, fT) do
    case (:ets.lookup(fT,
                        {:function, {:shell_default, f, length(as)}})) do
      [] ->
  ...
  defp local_func_handler(shell, rT, fT, ef) do
    h = fn lf ->
             fn f, as, bs ->
                  local_func(f, as, bs, shell, rT, fT, {:eval, lf.(lf)},
                               ef)
             end
        end
    {:eval, h.(h)}
  ...
  defp record_fields([{:record_field, _, {:atom, _, field}, _} |
               fs]) do
    [field | record_fields(fs)]
  end
  ...
  defp initiate_records(bs, rT) do
    ...
    rNs2 = (case (:code.is_loaded(:user_default)) do
              {:file, _File} ->
  ...
  defp add_records(rAs, bs0, rT) do
    ...
    case (check_command([], bs1)) do
      ...
      :ok ->
        ...
        :lists.usort(for {name, _} <- recs do
                       name
                     end)
    end
  end
  ...
  defp record_bindings(recs0, bs0) do
    {recs1, _} = :lists.mapfoldl(fn {name, def__}, i ->
                                      {{name, i, def__}, i + 1}
                                 end,
                                   0, recs0)
    recs2 = :lists.keysort(2, :lists.ukeysort(1, recs1))
    :lists.foldl(fn {name, i, def__}, bs ->
  ...
  defp read_records_from_beam(beam, file) do
    case (:beam_lib.chunks(beam, [:abstract_code, 'CInf'])) do
      {:ok,
         {_Mod,
            [{:abstract_code, {version, forms}}, {'CInf', cB}]}} ->
      ...
      {:ok,
         {_Mod,
            [{:abstract_code, :no_abstract_code}, {'CInf', cB}]}} ->
        try_source(file, cB)
      error ->
        error
    end
  ...
  defp parse_file(file, opts) do
    ...
    case (:epp.parse_file(file, includePath,
                            pre_defs(opts))) do
      {:ok, forms} ->
        record_attrs(forms)
      error ->
  ...
  defp list_commands([{{n, :command}, es0}, {{n, :result}, v} | ds],
            rT) do
  ...
  defp list_bindings([{name, val} | bs], rT) do
    case (:erl_eval.fun_data(val)) do
      ...
      false ->
        ...
        valS = pp(val, nl, rT)
        :io.requests([{:put_chars, :latin1, namel}, {:put_chars,
                                                       :unicode, valS},
                                                        :nl])
    end
    list_bindings(bs, rT)
  ...
  defp expand_value(e) do
    substitute_v1(fn {:value, commandN, v} ->
  ...
  defp prep_list_commands(e) do
    a = a0()
    substitute_v1(fn {:value, anno, _V} ->
  ...
  defp substitute_v1(f, t) when is_tuple(t) do
    :erlang.list_to_tuple(substitute_v1(f,
  ...
  defp pp(v, i, d, rT) do
    strings = :application.get_env(:stdlib, :shell_strings,
                                     true) !== false
    :io_lib_pretty.print(v,
                           [{:column, i}, {:line_length, columns()}, {:depth,
                                                                        d},
                                                                         {:line_max_chars,
                                                                            60},
                                                                             {:strings,
  ...
  defp check_env(v) do
    case (:application.get_env(:stdlib, v, 0)) do
      val when (is_integer(val) and val >= 0) ->
        :ok
      val ->
        txt = :io_lib.fwrite('Invalid value of STDLIB configuration parameter~tw: ~tp\n', [v, val])
        :error_logger.info_report(:lists.flatten(txt))
    end
  end

  defp set_env(app, name, val, default) do
    prev = (case (:application.get_env(app, name)) do
              :undefined ->
                default
              {:ok, old} ->
                old
  ...
  def prompt_func(promptFunc) do
    set_env(:stdlib, :shell_prompt_func, promptFunc,
              :default)
  end
