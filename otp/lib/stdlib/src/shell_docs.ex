defmodule :m_shell_docs do
  ...
  Record.defrecord(:r_docs_v1_entry, :docs_v1_entry, kind_name_arity: :undefined,
                                         ...
                                         metadata: :undefined)
  Record.defrecord(:r_config, :config, docs: :undefined,
                                  encoding: :undefined, ansi: :undefined,
                                  io_opts: :io.getopts(), columns: :undefined)
  def supported_tags() do
    [:a, :p, :div, :br, :h1, :h2, :h3, :h4, :h5, :h6, :i,
                                                          :b, :em, :strong,
                                                                       :pre,
                                                                           :code,
                                                                               :ul,
  ...
  def validate(r_docs_v1(module_doc: mDocs, docs: allDocs)) do
    aE = :lists.sort([:a, :p, :div, :br, :h1, :h2, :h3, :h4,
                                                            :h5, :h6, :i, :b,
                                                                              :em,
                                                                                  :strong,
                                                                                      :pre,
                                                                                                  ...
                                                                                                  :ol,
                                                                                                      :li,
                                                                                                          :dl,
                                                                                                              :dt,
                                                                                                                  :dd])
    ^aE = :lists.sort([:i, :b, :em, :strong, :code,
                                                 :a] ++ [:p, :div, :pre, :br,
                                                                             :ul,
                                                                                 :ol,
                                                                                     :li,
                                                                                                         ...
                                                                                                         :h2,
                                                                                                             :h3,
                                                                                                                 :h4,
                                                                                                                     :h5,
                                                                                                                         :h6])
    true = :lists.all(fn elem ->
                           elem === :a or elem === :code or elem === :i or elem === :em or elem === :b or elem === :strong
                      end,
    ...
    true = :lists.all(fn elem ->
                        ...
                        [:p, :div, :pre, :br, :ul, :ol, :li, :dl, :dt, :dd, :h1,
                                                                                :h2,
                                                                                    :h3,
                                                                                        :h4,
                                                                                            :h5,
    ...
    :lists.foreach(fn {_, _Anno, sig, docs, _Meta} ->
  ...
  defp validate_docs({tag, attr, content}, path) do
    ...
    case (:erlang.or(:lists.member(:pre, path),
                       :lists.any(fn h ->
                                       :lists.member(h, path)
                                  end,
                                    [:h1, :h2, :h3, :h4, :h5, :h6]))) do
      true when not
                (tag === :a or tag === :code or tag === :i or tag === :em or tag === :b or tag === :strong)
                ->
        throw({:cannot_put_block_tag_within_pre, tag, path})
      _ ->
        :ok
    end
    case (:lists.member(tag,
                          [:p, :div, :pre, :br, :ul, :ol, :li, :dl, :dt, :dd,
                                                                             :h1,
                                                                                 :h2,
                                                                                     :h3,
                                                                                         :h4,
  ...
  defp normalize_trim(bin, true) when is_binary(bin) do
    ...
    noNewLine = :re.replace(noTab, '\\v', ' ',
                              [:unicode, :global])
    :re.replace(noNewLine, '\\s+', ' ',
  ...
  defp normalize_trim([{:code, attr, content} | t], false) do
    trimmedContent = (case (:lists.reverse(normalize_trim(content,
                                                            false))) do
    ...
    [{:code, attr, trimmedContent} | normalize_trim(t,
                                                      false)]
  end
  ...
  defp normalize_trim([b1, b2 | t], trim) when (is_binary(b1) and
                                      is_binary(b2)) do
    normalize_trim([<<b1 :: binary, b2 :: binary>> | t],
                     trim)
  end
  ...
  defp normalize_space([{block, attr, content} | t]) when not
                                             (block === :a or block === :code or block === :i or block === :em or block === :b or block === :strong) do
  ...
  defp normalize_space(elems) do
    {inlineElems, t} = :lists.splitwith(fn e ->
                                             is_binary(e) or is_tuple(e) and (:erlang.element(1,
                                                                                                e) === :a or :erlang.element(1,
                                                                                                                               e) === :code or :erlang.element(1,
                                                                                                                                                                 e) === :i or :erlang.element(1,
                                                                                                                                                                                                e) === :em or :erlang.element(1,
                                                                                                                                                                                                                                e) === :b or :erlang.element(1,
                                                                                                                                                                                                                                                               e) === :strong)
                                        end,
                                          elems)
  ...
  defp trim_inline([{elem, attr, content} | t], trimSpace) do
    {newContent, contentTrimSpace} = trim_inline(content,
                                                   trimSpace)
    {newT, tTrimSpace} = trim_inline(t, contentTrimSpace)
    isAnchor = elem === :a and :proplists.is_defined(:id,
                                                       attr)
    cond do
  ...
  def render(module, function, r_docs_v1(docs: docs) = d, config)
      when (is_atom(module) and is_atom(function) and
              is_map(config)) do
    render_function(:lists.filter(fn {{:function, f, _},
                                        _Anno, _Sig, _Doc, _Meta} ->
                                       f === function
                                     _ ->
                                       false
  ...
  def render(module, function, arity, r_docs_v1(docs: docs) = d,
           config)
      when (is_atom(module) and is_atom(function) and
              is_integer(arity) and is_map(config)) do
    render_function(:lists.filter(fn {{:function, f, a},
                                        _Anno, _Sig, _Doc, _Meta} ->
                                       f === function and a === arity
                                     _ ->
                                       false
  ...
  def render_type(_Module, type, r_docs_v1(docs: docs) = d, config) do
    render_typecb_docs(:lists.filter(fn {{:type, t, _},
                                           _Anno, _Sig, _Doc, _Meta} ->
                                          t === type
                                        _ ->
                                          false
  ...
  def render_type(_Module, type, arity, r_docs_v1(docs: docs) = d,
           config) do
    render_typecb_docs(:lists.filter(fn {{:type, t, a},
                                           _Anno, _Sig, _Doc, _Meta} ->
                                          t === type and a === arity
                                        _ ->
                                          false
  ...
  def get_callback_doc(module, callback, arity) do
    {:ok, r_docs_v1(docs: docs) = d} = :code.get_doc(module)
    fnFunctions = :lists.filter(fn {{:callback, t, a},
                                      _Anno, _Sig, _Doc, _Meta} ->
  ...
  def render_callback(_Module, callback, r_docs_v1(docs: docs) = d, config) do
    render_typecb_docs(:lists.filter(fn {{:callback, t, _},
                                           _Anno, _Sig, _Doc, _Meta} ->
                                          t === callback
                                        _ ->
                                          false
  ...
  def render_callback(_Module, callback, arity, r_docs_v1(docs: docs) = d,
           config) do
    render_typecb_docs(:lists.filter(fn {{:callback, t, a},
                                           _Anno, _Sig, _Doc, _Meta} ->
                                          t === callback and a === arity
                                        _ ->
                                          false
  ...
  defp get_local_doc(_Missing, moduleDoc, d)
    ...
    normalize_format(:maps.get(hd(:maps.keys(moduleDoc)),
                                 moduleDoc),
                       d)
  end

  defp get_local_doc(missing, :hidden, _D) do
    [{:p, [], ["The documentation for ", missing, " is hidden. This probably means that it is internal and not to be used by other applications."]}]
  end

  defp get_local_doc(missing, none, _D) when none === :none or
                                    none === %{} do
  ...
  defp render_function(fDocs, r_docs_v1(docs: docs) = d, config) do
    grouping = :lists.foldl(fn {_Group, _Anno, _Sig, _Doc,
                                  %{equiv: group}} = func,
                                 acc ->
                              ...
                              :lists.sort(fn a, b ->
                                            ...
                                            fDocs))
    :lists.map(fn {group, members} ->
                    signatures = :lists.flatmap(&render_signature/1,
                                                  :lists.reverse(members))
                    case (:lists.search(fn {_, _, _, doc, _} ->
                                             doc !== %{}
                                        end,
                                          members)) do
  ...
  defp render_signature({{_Type, _F, _A}, _Anno, _Sigs, _Docs,
             %{signature: specs} = meta}) do
    :lists.flatmap(fn aSTSpec ->
                        pPSpec = :erl_pp.attribute(aSTSpec,
                                                     [{:encoding, :unicode}])
                        spec = (case (aSTSpec) do
                                  {_Attribute, _Line, :opaque, _} ->
                                    hd(:string.split(pPSpec, '::'))
                                  _ ->
                                    pPSpec
                                end)
                        binSpec = :unicode.characters_to_binary(:string.trim(spec,
                                                                               :trailing,
                                                                               '\n'))
                        [{:pre, [], [{:strong, [], binSpec}]} |
                             render_meta(meta)]
  ...
  defp render_headers_and_docs(headers, docContents, r_config() = config) do
    ['\n', render_docs(:lists.flatmap(fn header ->
                                        [{:br, [], []}, header]
                                   end,
                                     headers),
                      config),
  ...
  defp render_signature_listing(module, type, r_docs_v1(docs: docs) = d, config) do
    slogan = [{:h2, [],
                 [<<"\t", :erlang.atom_to_binary(module) :: binary>>]},
                  {:br, [], []}]
    case (:lists.filter(fn {{t, _, _}, _Anno, _Sig, _Doc,
                              _Meta} ->
                             type === t
                        end,
                          docs)) do
      [] ->
        render_docs(slogan ++ [<<"There are no ",
                                   :erlang.atom_to_binary(type) :: binary, "s in this module">>],
                      d, config)
      headers ->
        hdr = :lists.flatmap(fn header ->
                                  [{:br, [], []}, render_signature(header)]
                             end,
                               headers)
        render_docs(slogan ++ [{:p, [],
  ...
  defp render_typecb_docs({f, _, _Sig, docs, _Meta} = typeCB,
            r_config(docs: d) = c) do
    render_headers_and_docs(render_signature(typeCB),
                              get_local_doc(f, docs, d), c)
  end
  ...
  defp render_docs(docContents, ind, d = r_config())
    ...
    init_ansi(d)
    try do
      {doc, _} = trimnl(render_docs(docContents, [], 0, ind,
                                      d))
      doc
    after
      clean_ansi()
  ...
  defp init_config(d, config) when is_map(config) do
    ...
    defaultEncoding = :proplists.get_value(:encoding,
                                             defaultOpts, :latin1)
    columns = (case (:maps.find(:columns, config)) do
                 :error ->
                   case (:io.columns()) do
                     {:ok, c} ->
                       c
  ...
  defp render_element({:h1, _, content}, state, 0 = pos, _Ind, d) do
    trimnlnl(render_element({:code, [],
                               [{:strong, [], content}]},
                              state, pos, 0, d))
  end

  defp render_element({:h2, _, content}, state, 0 = pos, _Ind, d) do
    trimnlnl(render_element({:strong, [], content}, state,
                              pos, 0, d))
  end
  ...
  defp render_element({h, _, content}, state, pos, _Ind, d)
    ...
    trimnlnl(render_element({:code, [], content}, state,
                              pos, 2, d))
  end
  ...
  defp render_element({elem, _Attr, _Content} = e, state, pos, ind, d)
      when (pos > ind and
              not
              (elem === :a or elem === :code or elem === :i or elem === :em or elem === :b or elem === :strong)) do
    {docs, newPos} = render_element(e, state, 0, ind, d)
    {['\n', docs], newPos}
  end

  defp render_element({:div, [{:class, what}], content}, state, pos,
            ind, d) do
    {docs, _} = render_docs(content, [:div | state], 0,
                              ind + 2, d)
  ...
  defp render_element({tag, _, content}, state, pos, ind, d)
    ...
    trimnlnl(render_docs(content, [tag | state], pos, ind,
                           d))
  end

  defp render_element(elem, state, pos, ind, d) when pos < ind do
    {docs, newPos} = render_element(elem, state, ind, ind,
                                      d)
    {[pad(ind - pos), docs], newPos}
  end

  defp render_element({:code, _, content}, [:pre | _] = state, pos,
            ind, d) do
  ...
  defp render_element({:code, _, content}, state, pos, ind, d) do
    ...
    {docs, newPos} = render_docs(content, [:code | state],
                                   pos, ind, d)
  ...
  defp render_element({:b, _, content}, state, pos, ind, d) do
    bold = sansi(:bold)
    {docs, newPos} = render_docs(content, state, pos, ind,
                                   d)
    {[bold, docs, ransi(:bold)], newPos}
  end

  defp render_element({:pre, _, content}, state, pos, ind, d) do
    trimnlnl(render_docs(content, [:pre | state], pos,
                           ind + 2, d))
  end

  defp render_element({:ul, [{:class, "types"}], content}, state, _Pos, ind,
            d) do
    {docs, _} = render_docs(content, [:types | state], 0,
                              ind + 2, d)
    trimnlnl(['Types:\n', docs])
  end

  defp render_element({:li, attr, content}, [:types | _] = state, pos,
            ind, c) do
    doc = (case ({:proplists.get_value(:name, attr),
                    :proplists.get_value(:class, attr)}) do
             {:undefined, class} when class === :undefined or
                                        class === "type"
                                      ->
  ...
  defp render_element({:li, [], content}, [:l | _] = state, pos, ind,
            d) do
    bullet = get_bullet(state, r_config(d, :encoding))
    bulletLen = :string.length(bullet)
    {docs, _NewPos} = render_docs(content, [:li | state],
                                    pos + bulletLen, ind + bulletLen, d)
  ...
  defp render_element({:dt, attr, content}, [:dl | _] = state, pos,
            ind, d) do
    since = (case (attr) do
               [{:since, vsn}] ->
  ...
  defp render_element({:dd, _, content}, [:dl | _] = state, pos, ind,
            d) do
    trimnlnl(render_docs(content, [:li | state], pos,
                           ind + 2, d))
  end
  ...
  defp render_element(b, state, pos, ind, d) when is_binary(b) do
    case (:lists.member(:pre, state)) do
      ...
      _ ->
        render_words(split_to_words(b), state, pos, ind, [[]],
                       d)
    end
  end
  ...
  defp render_words([unicodeWord | t], state, pos, ind, acc,
            r_config(columns: cols) = d)
    ...
    cond do
      (newPos > cols - 10 - ind and word !== <<>> and
         not isPunct) ->
        render_words(t, state, wordLength + ind + 1, ind,
                       [[[nlpad(ind), word]] | acc], d)
      true ->
        [line | lineAcc] = acc
        newPosSpc = newPos + 1
  ...
  defp render_type_signature(name,
            r_config(docs: r_docs_v1(metadata: %{types: allTypes}))) do
  ...
  defp get_bullet(state, :unicode) do
    case (length(for :l <- state do
      ...
      level ->
        :lists.nth(level,
                     [<<" â\200¢ " :: utf8>>, <<" ï¿® " :: utf8>>, <<" â\227¼ " :: utf8>>, <<" â\227» "
                                                                     ::
                                                                     utf8>>])
    end
  end
  ...
  defp init_ansi(r_config(ansi: :undefined, io_opts: opts)) do
    case ({:application.get_env(:kernel, :shell_docs_ansi),
             ...
             :proplists.is_defined(:echo,
                                     opts) and :proplists.is_defined(:expand_fun,
                                                                       opts)}) do
      {{:ok, false}, _, _} ->
