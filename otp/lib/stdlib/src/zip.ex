defmodule :m_zip do
  ...
  defp verbose_unzip(fN) do
    :io.format(~c"extracting: ~ts\n", [:io_lib.write_string(fN)])
  end

  defp verbose_zip(fN) do
    :io.format(~c"adding: ~ts\n", [:io_lib.write_string(fN)])
  end
  ...
  defp get_zip_options(files, options) do
    suffixes = [~c".Z", ~c".zip", ~c".zoo", ~c".arc", ~c".lzh", ~c".arj"]
  ...
  defp get_openzip_options(options) do
    opts = r_openzip_opts(open_opts: [:raw, :read], output: &file_io/2, cwd: ~c"")
  ...
  defp put_z_file(0, uncompSize, out0, f, pos0, input, output, opO, _Z, :regular) do
    in0 = []
  ...
  defp put_z_file(8, uncompSize, out0, f, pos0, input, output, opO, z, :regular) do
    ...
    cRC0 = 0
    {out1, pos1, cRC} = put_z_data_loop(uncompSize, in1, out0, pos0, input, output, cRC0, z)
  ...
  defp put_z_data_loop(uncompSize, in0, out0, pos0, input, output, cRC0, z) do
    ...
    case input.({:read, n}, in0) do
      ...
      {uncompressed, in1} ->
        ...
        put_z_data_loop(uncompSize - n, in1, out1, pos0 + sz, input, output, cRC1, z)
    end
  end
  ...
  defp print_comment(~c"") do
    :ok
  end

  defp print_comment(comment) do
    :io.format(~c"Archive comment: ~ts\n", [comment])
  end
  ...
  defp local_file_header_from_info_method_name(
         r_file_info(mtime: mTime, type: type),
         uncompSize,
         compMethod,
         name,
         gPFlag
  ...
  defp server_loop(parent, openZip) do
    receive do
      ...
      {from, {:get_crc32, fileName}} ->
        send(from, {self(), openzip_get_crc32(fileName, openZip)})
        server_loop(parent, openZip)
  ...
  defp get_central_dir(in0, rawIterator, input) do
    ...
    out0 = rawIterator.(eOCD, ~c"", comment, <<>>, acc0)
  ...
  defp check_valid_location(cWD, fileName) do
    case check_dir_level(:filename.split(fileName), 0) do
      {fileOrDir, level} when level < 0 ->
        ...
        :error_logger.format(~c"Illegal path: ~ts, extracting in ~ts~n", [
  ...
  defp check_dir_level([~c"." | parts], level) do
  ...
  defp get_z_data(0, in0, fileName, compSize, input, output, opO, _Z) do
    out0 = output.({:open, fileName, [:write | opO]}, [])
    cRC0 = 0
  ...
  defp get_z_data_loop(0, uncompSize, in__, out, _Input, _Output, cRC0, _Z) do
    {in__, out, uncompSize, cRC0}
  end

  defp get_z_data_loop(compSize, uncompSize, in0, out0, input, output, cRC0, z) do
    n = :erlang.min(16 * 1024, compSize)
