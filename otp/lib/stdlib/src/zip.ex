defmodule :m_zip do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_zip_file, :zip_file, name: :undefined,
                                    info: :undefined, comment: :undefined,
                                    offset: :undefined, comp_size: :undefined)
  Record.defrecord(:r_zip_comment, :zip_comment, comment: :undefined)
  Record.defrecord(:r_unzip_opts, :unzip_opts, output: :undefined,
                                      input: :undefined,
                                      file_filter: :undefined,
                                      open_opts: :undefined,
                                      feedback: :undefined, cwd: :undefined)
  Record.defrecord(:r_zip_opts, :zip_opts, output: :undefined,
                                    input: :undefined, comment: :undefined,
                                    open_opts: :undefined, feedback: :undefined,
                                    cwd: :undefined, compress: :undefined,
                                    uncompress: :undefined)
  Record.defrecord(:r_list_dir_opts, :list_dir_opts, input: :undefined,
                                         raw_iterator: :undefined,
                                         open_opts: :undefined)
  Record.defrecord(:r_openzip_opts, :openzip_opts, output: :undefined,
                                        open_opts: :undefined, cwd: :undefined)
  Record.defrecord(:r_openzip, :openzip, zip_comment: :undefined,
                                   files: :undefined, in: :undefined,
                                   input: :undefined, output: :undefined,
                                   zlib: :undefined, cwd: :undefined)
  Record.defrecord(:r_zip_file_extra, :zip_file_extra, crc32: :undefined)
  Record.defrecord(:r_local_file_header, :local_file_header, version_needed: :undefined,
                                             gp_flag: :undefined,
                                             ...
                                             type: :undefined)
  Record.defrecord(:r_cd_file_header, :cd_file_header, version_made_by: :undefined,
  ...
  Record.defrecord(:r_eocd, :eocd, disk_num: :undefined,
                                start_disk_num: :undefined,
                                entries_on_disk: :undefined,
                                entries: :undefined, size: :undefined,
                                offset: :undefined,
  ...
  defp do_openzip_open(f, options) do
    ...
    {[r_zip_comment(comment: c) | files], in1} = get_central_dir(in0,
                                                       &raw_file_info_etc/5,
                                                       input)
    z = :zlib.open()
    {:ok,
       r_openzip(zip_comment: c, files: files, in: in1, input: input,
  ...
  defp do_openzip_get(r_openzip(files: files, in: in0, input: input,
              output: output, zlib: z, cwd: cWD)) do
    zipOpts = r_unzip_opts(output: output, input: input,
                  file_filter: &all/1, open_opts: [], feedback: &silent/1,
                  cwd: cWD)
  ...
  defp openzip_get_crc32(fileName, r_openzip(files: files)) do
    case (file_name_search(fileName, files)) do
      {_, r_zip_file_extra(crc32: cRC)} ->
        {:ok, cRC}
      _ ->
  ...
  defp do_openzip_get(f,
            r_openzip(files: files, in: in0, input: input, output: output,
                zlib: z, cwd: cWD)) do
    case (file_name_search(f, files)) do
      {r_zip_file(offset: offset), _} = zFile ->
        in1 = input.({:seek, :bof, offset}, in0)
        case (get_z_file(in1, z, input, output, [], &silent/1,
                           cWD, zFile, &all/1)) do
          {:file, r, _In2} ->
            {:ok, r}
          _ ->
  ...
  defp file_name_search(name, files) do
    fun = fn {zipFile, _} ->
               not
               :string.equal(r_zip_file(zipFile, :name), name,
  ...
  def openzip_list_dir(r_openzip(files: files), [:names_only]) do
    {zipFiles, _Extras} = :lists.unzip(files)
    names = (for {r_zip_file(name: name), _} <- zipFiles do
               name
             end)
    {:ok, names}
  end
  ...
  defp do_list_dir(f, options) do
    ...
    r_list_dir_opts(input: input, open_opts: opO,
  ...
  defp get_list_dir_opt([:names_only | rest], opts) do
    get_list_dir_opt(rest,
                       r_list_dir_opts(opts, raw_iterator: fn a, b, c, d, e ->
  ...
  defp keep_old_file({r_zip_file(name: fileName), _}) do
    not
    (:filelib.is_file(fileName) or :filelib.is_dir(fileName))
  end
  ...
  defp get_zip_options(files, options) do
    ...
    opts = r_zip_opts(output: &file_io/2,
               input: get_zip_input({:files, files}),
               open_opts: [:raw, :write], comment: '',
               feedback: &silent/1, cwd: '', compress: :all,
               uncompress: suffixes)
    opts1 = (r_zip_opts(comment: comment) = get_zip_opt(options,
                                                 opts))
  ...
  defp get_unzip_options(f, options) do
    opts = r_unzip_opts(file_filter: &all/1, output: &file_io/2,
               input: get_input(f), open_opts: [:raw],
               feedback: &silent/1, cwd: '')
    get_unzip_opt(options, opts)
  end

  defp get_openzip_options(options) do
    opts = r_openzip_opts(open_opts: [:raw, :read], output: &file_io/2,
               cwd: '')
  ...
  defp get_zip_input({f, b, r_file_info()}) when (is_binary(b) and
                               is_list(f)) do
    &binary_io/2
  end

  defp get_zip_input({f, r_file_info(), b}) when (is_binary(b) and
                               is_list(f)) do
    &binary_io/2
  end
  ...
  defp get_list_dir_options(f, options) do
    opts = r_list_dir_opts(raw_iterator: &raw_file_info_public/5,
               input: get_input(f), open_opts: [:raw])
  ...
  defp put_central_dir(lHS, pos, out0,
            r_zip_opts(output: output, comment: comment)) do
  ...
  defp put_cd_files_loop([{lH, name, pos} | lHRest], output, out0,
            sz0) do
    cDFH = cd_file_header_from_lh_and_pos(lH, pos)
    bCDFH = cd_file_header_to_bin(cDFH)
    b = [<<33639248 :: size(32) - little>>, bCDFH, name]
    out1 = output.({:write, b}, out0)
    sz1 = sz0 + (4 + 2 + 2 + 2 + 2 + 2 + 2 + 4 + 4 + 4 + 2 + 2 + 2 + 2 + 2 + 4 + 4) + r_local_file_header(lH, :file_name_length)
    put_cd_files_loop(lHRest, output, out1, sz1)
  end

  defp put_eocd(n, pos, sz, comment, output, out0) do
    commentSz = length(comment)
    eOCD = r_eocd(disk_num: 0, start_disk_num: 0,
               entries_on_disk: n, entries: n, size: sz, offset: pos,
               zip_comment_length: commentSz)
    bEOCD = eocd_to_bin(eOCD)
    b = [<<101010256 :: size(32) - little>>, bEOCD, comment]
    output.({:write, b}, out0)
  end
  ...
  defp get_comp_method(f, _,
            r_zip_opts(compress: compress, uncompress: uncompress), _) do
  ...
  defp put_z_files([f | rest], z, out0, pos0,
            r_zip_opts(input: input, output: output, open_opts: opO,
                feedback: fB, cwd: cWD) = opts,
            acc) do
    in0 = []
    f1 = add_cwd(cWD, f)
    fileInfo = input.({:file_info, f1}, in0)
    type = r_file_info(fileInfo, :type)
    uncompSize = (case (type) do
                    :regular ->
    ...
    compMethod = get_comp_method(fileName, uncompSize, opts,
                                   type)
    lH = local_file_header_from_info_method_name(fileInfo,
                                                   uncompSize, compMethod,
                                                   fileName, gPFlag)
    bLH = local_file_header_to_bin(lH)
    b = [<<67324752 :: size(32) - little>>, bLH]
    out1 = output.({:write, b}, out0)
    out2 = output.({:write, fileName}, out1)
    {out3, compSize, cRC} = put_z_file(compMethod,
                                         uncompSize, out2, f1, 0, input, output,
                                         opO, z, type)
    fB.(fileName0)
    patch = <<cRC :: size(32) - little,
                compSize :: size(32) - little>>
    out4 = output.({:pwrite, pos0 + 4 + 2 + 2 + 2 + 2 + 2,
                      patch},
                     out3)
    out5 = output.({:seek, :eof, 0}, out4)
    pos1 = pos0 + (4 + 2 + 2 + 2 + 2 + 2 + 4 + 4 + 4 + 2 + 2) + r_local_file_header(lH, :file_name_length)
    pos2 = pos1 + compSize
    lH2 = r_local_file_header(lH, comp_size: compSize,  crc32: cRC)
    thisAcc = [{lH2, fileName, pos0}]
    {out6, subAcc, pos3} = (case (type) do
                              :regular ->
                                {out5, thisAcc, pos2}
                              :directory ->
                                files = input.({:list_dir, f1}, [])
                                revFiles = reverse_join_files(f, files, [])
                                put_z_files(revFiles, z, out5, pos2, opts,
                                              thisAcc)
                            end)
  ...
  defp reverse_join_files(dir, [file | files], acc) do
    reverse_join_files(dir, files,
                         [:filename.join([dir, file]) | acc])
  end
  ...
  defp put_z_file(_Method, sz, out, _F, pos, _Input, _Output,
            _OpO, _Z, :directory) do
    {out, pos + sz, 0}
  end

  defp put_z_file(_Method, 0, out, _F, pos, _Input, _Output, _OpO,
            _Z, :regular) do
    {out, pos, 0}
  end

  defp put_z_file(0, uncompSize, out0, f, pos0, input, output,
            opO, _Z, :regular) do
    in0 = []
  ...
  defp put_z_file(8, uncompSize, out0, f, pos0, input, output,
            opO, z, :regular) do
    in0 = []
    in1 = input.({:open, f, opO -- [:write]}, in0)
    :ok = :zlib.deflateInit(z, :default, :deflated, - 15, 8,
                              :default)
    cRC0 = 0
    {out1, pos1, cRC} = put_z_data_loop(uncompSize, in1,
                                          out0, pos0, input, output, cRC0, z)
  ...
  defp put_z_data_loop(0, _In, out, pos, _Input, _Output, cRC0, _Z) do
    {out, pos, cRC0}
  end

  defp put_z_data_loop(uncompSize, in0, out0, pos0, input, output,
            cRC0, z) do
    ...
    case (input.({:read, n}, in0)) do
      ...
      {uncompressed, in1} ->
        ...
        compressed = :zlib.deflate(z, uncompressed,
                                     get_sync(n, uncompSize))
  ...
  defp raw_long_print_info_etc(r_cd_file_header(comp_size: compSize, uncomp_size: uncompSize,
              last_mod_date: lMDate, last_mod_time: lMTime),
            fileName, fileComment, _BExtraField, acc) do
    mTime = dos_date_time_to_datetime(lMDate, lMTime)
    print_header(compSize, mTime, uncompSize, fileName,
                   fileComment)
    acc
  end
  ...
  defp print_header(compSize, mTime, uncompSize, fileName,
            fileComment) do
    :io.format('~8w ~s ~8w ~2w% ~ts ~ts\n',
                 [compSize, time_to_string(mTime), uncompSize,
                                                       get_percent(compSize,
                                                                     uncompSize),
                                                           fileName,
                                                               fileComment])
  end
  ...
  defp cd_file_header_from_lh_and_pos(lH, pos) do
    r_local_file_header(version_needed: versionNeeded, gp_flag: gPFlag,
        comp_method: compMethod, last_mod_time: lastModTime,
        last_mod_date: lastModDate, crc32: cRC32,
        comp_size: compSize, uncomp_size: uncompSize,
        file_name_length: fileNameLength,
        extra_field_length: extraFieldLength, type: type) = lH
    r_cd_file_header(version_made_by: 20 ||| (3 <<< 8),
        version_needed: versionNeeded, gp_flag: gPFlag,
        comp_method: compMethod, last_mod_time: lastModTime,
        last_mod_date: lastModDate, crc32: cRC32,
        comp_size: compSize, uncomp_size: uncompSize,
        file_name_length: fileNameLength,
        extra_field_length: extraFieldLength,
        file_comment_length: 0, disk_num_start: 0,
        internal_attr: 0,
        external_attr: case (type) do
                         :regular ->
                           420 <<< 16
                         :directory ->
                           484 <<< 16
  ...
  defp cd_file_header_to_bin(r_cd_file_header(version_made_by: versionMadeBy,
              version_needed: versionNeeded, gp_flag: gPFlag,
              comp_method: compMethod, last_mod_time: lastModTime,
              last_mod_date: lastModDate, crc32: cRC32,
              comp_size: compSize, uncomp_size: uncompSize,
              ...
              local_header_offset: localHeaderOffset)) do
    <<versionMadeBy :: size(16) - little,
        versionNeeded :: size(16) - little,
        gPFlag :: size(16) - little,
        compMethod :: size(16) - little,
        lastModTime :: size(16) - little,
        lastModDate :: size(16) - little,
        cRC32 :: size(32) - little,
        compSize :: size(32) - little,
        uncompSize :: size(32) - little,
        fileNameLength :: size(16) - little,
        extraFieldLength :: size(16) - little,
        fileCommentLength :: size(16) - little,
        diskNumStart :: size(16) - little,
        internalAttr :: size(16) - little,
        externalAttr :: size(32) - little,
        localHeaderOffset :: size(32) - little>>
  end

  defp local_file_header_to_bin(r_local_file_header(version_needed: versionNeeded,
              gp_flag: gPFlag, comp_method: compMethod,
              last_mod_time: lastModTime, last_mod_date: lastModDate,
              crc32: cRC32, comp_size: compSize,
              uncomp_size: uncompSize,
              file_name_length: fileNameLength,
              extra_field_length: extraFieldLength)) do
    <<versionNeeded :: size(16) - little,
        gPFlag :: size(16) - little,
        compMethod :: size(16) - little,
        lastModTime :: size(16) - little,
  ...
  defp eocd_to_bin(r_eocd(disk_num: diskNum,
              start_disk_num: startDiskNum,
              entries_on_disk: entriesOnDisk, entries: entries,
              size: size, offset: offset,
              zip_comment_length: zipCommentLength)) do
    <<diskNum :: size(16) - little,
        startDiskNum :: size(16) - little,
        entriesOnDisk :: size(16) - little,
        entries :: size(16) - little, size :: size(32) - little,
        offset :: size(32) - little,
        zipCommentLength :: size(16) - little>>
  end

  defp local_file_header_from_info_method_name(r_file_info(mtime: mTime, type: type), uncompSize,
            compMethod, name, gPFlag) do
    {modDate, modTime} = dos_date_time_from_datetime(mTime)
    r_local_file_header(version_needed: 20, gp_flag: gPFlag,
        comp_method: compMethod, last_mod_time: modTime,
        last_mod_date: modDate, crc32: - 1, comp_size: - 1,
        uncomp_size: uncompSize, file_name_length: length(name),
        extra_field_length: 0, type: type)
  end
  ...
  defp server_loop(parent, openZip) do
    receive do
      ...
      {from, {:get_crc32, fileName}} ->
        send(from, {self(),
                      openzip_get_crc32(fileName, openZip)})
        server_loop(parent, openZip)
      {from, :list_dir} ->
  ...
  def zip_get_crc32(fileName, pid) when is_pid(pid) do
  ...
  def openzip_tt(r_openzip(zip_comment: zipComment, files: files)) do
    ...
    lists_foreach(fn {r_zip_file(comp_size: compSize, name: fileName,
                          comment: fileComment, info: fI),
                        _} ->
  ...
  def openzip_t(r_openzip(zip_comment: zipComment, files: files)) do
    print_comment(zipComment)
    lists_foreach(fn {r_zip_file(name: fileName), _} ->
                       print_file_name(fileName)
                  end,
                    files)
    :ok
  ...
  defp get_central_dir(in0, rawIterator, input) do
    {b, in1} = get_end_of_central_dir(in0,
                                        4 + 2 + 2 + 2 + 2 + 4 + 4 + 2, input)
  ...
  defp get_cd_loop(n, in0, rawIterator, input, acc0) do
    {b, in1} = input.({:read,
                        ...
                        in0)
    bCD = (case (b) do
             <<33639248 :: size(32) - little, xBCD :: binary>> ->
               xBCD
             _ ->
    ...
    {fileName, comment, bExtra} = get_name_extra_comment(b2,
                                                           fileNameLen,
                                                           extraLen, commentLen,
                                                           gPFlag)
  ...
  defp get_name_extra_comment(b, fileNameLen, extraLen, commentLen, gPFlag) do
    try do
      <<bFileName :: size(fileNameLen) - binary,
          bExtra :: size(extraLen) - binary,
          bComment :: size(commentLen) - binary>> = b
      {binary_to_chars(bFileName, gPFlag),
         heuristic_to_string(bComment), bExtra}
    catch
      _, _ ->
        throw(:bad_central_directory)
    end
  ...
  defp find_eocd_header(<<101010256 :: size(32) - little,
              rest :: binary>>) do
    rest
  end
  ...
  defp raw_file_info_etc(cD, fileName, fileComment, bExtraField, acc)
      when elem(cD, 0) === :cd_file_header do
    r_cd_file_header(comp_size: compSize, local_header_offset: offset,
        crc32: cRC) = cD
    fileInfo = cd_file_header_to_file_info(fileName, cD,
                                             bExtraField)
    [{r_zip_file(name: fileName, info: fileInfo,
          comment: fileComment, offset: offset,
          comp_size: compSize),
  ...
  defp raw_file_info_public(cD, fileName, fileComment, bExtraField, acc0) do
    [h1 | t] = raw_file_info_etc(cD, fileName, fileComment,
                                   bExtraField, acc0)
    h2 = (case (h1) do
  ...
  defp cd_file_header_to_file_info(fileName,
            r_cd_file_header(uncomp_size: uncompSize, last_mod_time: modTime,
                last_mod_date: modDate),
            extraField) do
    t = dos_date_time_to_datetime(modDate, modTime)
    type = (case (:lists.last(fileName)) do
              ?/ ->
                :directory
              _ ->
                :regular
            end)
    fI = r_file_info(size: uncompSize, type: type,
             access: :read_write, atime: t, mtime: t, ctime: t,
             mode: 54, links: 1, major_device: 0, minor_device: 0,
             inode: 0, uid: 0, gid: 0)
  ...
  defp get_z_files([{r_zip_file(offset: offset), _} = zFile | rest], z, in0,
            r_unzip_opts(input: input, output: output, open_opts: opO,
                file_filter: filter, feedback: fB, cwd: cWD) = opts,
            acc0) do
    case (filter.(zFile)) do
      true ->
        in1 = input.({:seek, :bof, offset}, in0)
        {in2, acc1} = (case (get_z_file(in1, z, input, output,
                                          opO, fB, cWD, zFile, filter)) do
                         {:file, gZD, inx} ->
                           {inx, [gZD | acc0]}
                         {_, inx} ->
                           {inx, acc0}
  ...
  defp get_z_file(in0, z, input, output, opO, fB, cWD,
            {zipFile, extra}, filter) do
    case (input.({:read,
                    4 + 2 + 2 + 2 + 2 + 2 + 4 + 4 + 4 + 2 + 2},
                   in0)) do
      {:eof, in1} ->
        {:eof, in1}
      {<<67324752 :: size(32) - little, b :: binary>>, in1} ->
        lH = local_file_header_from_bin(b)
        r_local_file_header(gp_flag: gPFlag, comp_method: compMethod,
            file_name_length: fileNameLen,
            extra_field_length: extraLen) = lH
        {compSize, cRC32} = (case (gPFlag &&& 8 === 8) do
                               true ->
                                 {r_zip_file(zipFile, :comp_size), r_zip_file_extra(extra, :crc32)}
                               false ->
                                 {r_local_file_header(lH, :comp_size), r_local_file_header(lH, :crc32)}
                             end)
        {bFileN, in3} = input.({:read, fileNameLen + extraLen},
                                 in1)
        {fileName, _} = get_file_name_extra(fileNameLen,
                                              extraLen, bFileN, gPFlag)
        readAndWrite = (case (check_valid_location(cWD,
                                                     fileName)) do
                          {true, fileName1} ->
                            true
                          {false, fileName1} ->
          ...
          true ->
            case (:lists.last(fileName)) do
              ?/ ->
                ...
                {:dir, in3}
              _ ->
                {out, in4, cRC, _UncompSize} = get_z_data(compMethod,
                                                            in3, fileName1,
                                                            compSize, input,
                                                            output, opO, z)
  ...
  defp check_valid_location(cWD, fileName) do
    case (check_dir_level(:filename.split(fileName), 0)) do
      {fileOrDir, level} when level < 0 ->
        ...
        :error_logger.format('Illegal path: ~ts, extracting in ~ts~n', [add_cwd(cWD, fileName), cWD1])
  ...
  defp get_z_data(8, in0, fileName, compSize, input, output, opO,
            z) do
    :ok = :zlib.inflateInit(z, - 15)
    out0 = output.({:open, fileName, [:write | opO]}, [])
    cRC0 = 0
    {in1, out1, uncompSize, cRC} = get_z_data_loop(compSize,
                                                     0, in0, out0, input,
                                                     output, cRC0, z)
    _ = ((try do
           :zlib.inflateEnd(z)
  ...
  defp get_z_data(0, in0, fileName, compSize, input, output, opO,
            _Z) do
    out0 = output.({:open, fileName, [:write | opO]}, [])
    cRC0 = 0
    {in1, out1, cRC} = copy_data_loop(compSize, in0, out0,
                                        input, output, cRC0)
    out2 = output.({:close, fileName}, out1)
  ...
  defp copy_data_loop(0, in__, out, _Input, _Output, cRC) do
    {in__, out, cRC}
  end

  defp copy_data_loop(compSize, in0, out0, input, output, cRC0) do
    ...
    case (input.({:read, n}, in0)) do
      ...
      {uncompressed, in1} ->
        ...
        copy_data_loop(compSize - n, in1, out1, input, output,
                         cRC1)
    end
  end

  defp get_z_data_loop(0, uncompSize, in__, out, _Input, _Output, cRC0,
            _Z) do
    {in__, out, uncompSize, cRC0}
  end

  defp get_z_data_loop(compSize, uncompSize, in0, out0, input, output,
            cRC0, z) do
    n = :erlang.min(16 * 1024, compSize)
    case (input.({:read, n}, in0)) do
      {:eof, in1} ->
      ...
      {compressed, in1} ->
        ...
        get_z_data_loop(compSize - n,
                          uncompSize + :erlang.iolist_size(uncompressed), in1,
                          out1, input, output, cRC1, z)
    end
  end
  ...
  defp dos_date_time_to_datetime(dosDate, dosTime) do
    <<hour :: size(5), min :: size(6),
        sec :: size(5)>> = <<dosTime :: size(16)>>
    <<yearFrom1980 :: size(7), month :: size(4),
        day :: size(5)>> = <<dosDate :: size(16)>>
  ...
  defp dos_date_time_from_datetime({{year, month, day}, {hour, min, sec}}) do
    ...
    <<dosTime :: size(16)>> = <<hour :: size(5),
                                  min :: size(6), sec :: size(5)>>
    <<dosDate :: size(16)>> = <<yearFrom1980 :: size(7),
                                  month :: size(4), day :: size(5)>>
    {dosDate, dosTime}
  end
  ...
  defp append_bins([_ | _] = list, b) do
    <<b :: binary,
        :erlang.iolist_to_binary(list) :: binary>>
  end
  ...
  defp eocd_and_comment_from_bin(<<diskNum :: size(16) - little,
              startDiskNum :: size(16) - little,
              entriesOnDisk :: size(16) - little,
              entries :: size(16) - little, size :: size(32) - little,
              offset :: size(32) - little,
              zipCommentLength :: size(16) - little,
              comment :: size(zipCommentLength) - binary>>) do
    {r_eocd(disk_num: diskNum, start_disk_num: startDiskNum,
         entries_on_disk: entriesOnDisk, entries: entries,
         size: size, offset: offset,
  ...
  defp cd_file_header_from_bin(<<versionMadeBy :: size(16) - little,
              versionNeeded :: size(16) - little,
              gPFlag :: size(16) - little,
              compMethod :: size(16) - little,
              lastModTime :: size(16) - little,
              ...
              internalAttr :: size(16) - little,
              externalAttr :: size(32) - little,
              localHeaderOffset :: size(32) - little>>) do
    r_cd_file_header(version_made_by: versionMadeBy,
        version_needed: versionNeeded, gp_flag: gPFlag,
        comp_method: compMethod, last_mod_time: lastModTime,
        last_mod_date: lastModDate, crc32: cRC32,
        comp_size: compSize, uncomp_size: uncompSize,
  ...
  defp local_file_header_from_bin(<<versionNeeded :: size(16) - little,
              gPFlag :: size(16) - little,
              compMethod :: size(16) - little,
              lastModTime :: size(16) - little,
              lastModDate :: size(16) - little,
              ...
              uncompSize :: size(32) - little,
              fileNameLength :: size(16) - little,
              extraFieldLength :: size(16) - little>>) do
    r_local_file_header(version_needed: versionNeeded, gp_flag: gPFlag,
        comp_method: compMethod, last_mod_time: lastModTime,
        last_mod_date: lastModDate, crc32: cRC32,
        comp_size: compSize, uncomp_size: uncompSize,
  ...
  defp binary_io({:file_info, b}, _) do
    ...
    r_file_info(size: size, type: type, access: :read_write,
        atime: now, mtime: now, ctime: now, mode: 0, links: 1,
        major_device: 0, minor_device: 0, inode: 0, uid: 0,
        gid: 0)
  end
  ...
  defp binary_io({:pread, pos, n}, {oldPos, b}) do
    case (b) do
      <<_ :: size(pos) - binary, read :: size(n) - binary,
          _Rest :: binary>> ->
        {read, {pos + n, b}}
      _ ->
        {:eof, {oldPos, b}}
  ...
  defp binary_io({:write, data}, {pos, b}) do
    {pos + :erlang.iolist_size(data),
       pwrite_binary(b, pos, data)}
  end
