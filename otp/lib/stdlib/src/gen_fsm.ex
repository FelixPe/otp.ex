defmodule :m_gen_fsm do
  ...
  defp print_event(dev, {:in, msg, stateName}, name) do
    case msg do
      {:"$gen_event", event} ->
        :io.format(dev, ~c"*DBG* ~tp got event ~tp in state ~tw~n", [name, event, stateName])

      {:"$gen_all_state_event", event} ->
        :io.format(dev, ~c"*DBG* ~tp got all_state_event ~tp in state ~tw~n", [
          name,
          event,
          stateName
        ])
      ...
      {:"$gen_sync_event", {from, _Tag}, event} ->
        :io.format(dev, ~c"*DBG* ~tp got sync_event ~tp from ~tw in state ~tw~n", [
          name,
          event,
          from,
          stateName
      ...
      {:"$gen_sync_all_state_event", {from, _Tag}, event} ->
        :io.format(dev, ~c"*DBG* ~tp got sync_all_state_event ~tp from ~tw in state ~tw~n", [
          name,
          event,
          from,
          stateName
      ...
      {:timeout, ref, {:"$gen_timer", message}} ->
        :io.format(dev, ~c"*DBG* ~tp got timer ~tp in state ~tw~n", [
          name,
          {:timeout, ref, message},
          stateName
        ])
      ...
      {:timeout, _Ref, {:"$gen_event", event}} ->
        :io.format(dev, ~c"*DBG* ~tp got timer ~tp in state ~tw~n", [name, event, stateName])

      _ ->
        :io.format(dev, ~c"*DBG* ~tp got ~tp in state ~tw~n", [name, msg, stateName])
    end
  end

  defp print_event(dev, {:out, msg, {to, _Tag}, stateName}, name) do
    :io.format(dev, ~c"*DBG* ~tp sent ~tp to ~tw~n      and switched to state ~tw~n", [
      name,
      msg,
      to,
      stateName
  ...
  defp print_event(dev, {:noreply, stateName}, name) do
    :io.format(dev, ~c"*DBG* ~tp switched to state ~tw~n", [name, stateName])
  end
          ...
          true ->
            :erlang.apply(:logger, :macro_log, [
              %{
                ...
                line: 494,
                file: ~c"otp/lib/stdlib/src/gen_fsm.erl"
              },
              :warning,
  ...
  defp error_info(reason, name, from, msg, stateName, stateData, debug) do
    ...
    case :logger.allow(:error, :gen_fsm) do
      true ->
        :erlang.apply(:logger, :macro_log, [
          %{mfa: {:gen_fsm, :error_info, 7}, line: 608, file: ~c"otp/lib/stdlib/src/gen_fsm.erl"},
          :error,
          %{
