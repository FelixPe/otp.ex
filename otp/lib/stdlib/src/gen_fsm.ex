defmodule :m_gen_fsm do
  ...
  def enter_loop(mod, options, stateName, stateData,
           {scope, _} = serverName)
  ...
  def enter_loop(mod, options, stateName, stateData, serverName,
           timeout) do
  ...
  def init_it(starter, parent, name0, mod, args, options) do
    ...
    case ((try do
      ...
      {:EXIT, reason} ->
        ...
        exit(reason)
      else__ ->
        reason = {:bad_return_value, else__}
        exit(reason)
    end
  end

  defp loop(parent, name, stateName, stateData, mod,
            :hibernate, hibernateAfterTimeout, debug) do
  ...
  defp loop(parent, name, stateName, stateData, mod,
            :infinity, hibernateAfterTimeout, debug) do
    receive do
      msg ->
        decode_msg(msg, parent, name, stateName, stateData, mod,
                     :infinity, hibernateAfterTimeout, debug, false)
    after hibernateAfterTimeout ->
      loop(parent, name, stateName, stateData, mod,
             :hibernate, hibernateAfterTimeout, debug)
    end
  end

  defp loop(parent, name, stateName, stateData, mod, time,
  ...
  def wake_hib(parent, name, stateName, stateData, mod,
           hibernateAfterTimeout, debug) do
    msg = (receive do
             input ->
               input
  ...
  defp decode_msg(msg, parent, name, stateName, stateData, mod,
            time, hibernateAfterTimeout, debug, hib) do
    case (msg) do
      {:system, from, req} ->
        :sys.handle_system_msg(req, from, parent, :gen_fsm,
                                 debug,
  ...
  def system_terminate(reason, _Parent, debug,
    ...
    terminate(reason, name, :undefined, [], mod, stateName,
                stateData, debug)
  end

  def system_code_change([name, stateName, stateData, mod, time,
                                               hibernateAfterTimeout],
  ...
  def system_get_state([_Name, stateName, stateData, _Mod, _Time,
                                                 _HibernateAfterTimeout]) do
    {:ok, {stateName, stateData}}
  end
  ...
  def system_replace_state(stateFun,
           [name, stateName, stateData, mod, time,
                                                 hibernateAfterTimeout]) do
    result = ({nStateName,
                 nStateData} = stateFun.({stateName, stateData}))
    {:ok, result,
  ...
  defp print_event(dev, {:in, msg, stateName}, name) do
    case (msg) do
      ...
      {:"$gen_all_state_event", event} ->
        :io.format(dev, '*DBG* ~tp got all_state_event ~tp in state ~tw~n', [name, event, stateName])
      {:"$gen_sync_event", {from, _Tag}, event} ->
        :io.format(dev, '*DBG* ~tp got sync_event ~tp from ~tw in state ~tw~n', [name, event, from, stateName])
      {:"$gen_sync_all_state_event", {from, _Tag}, event} ->
        :io.format(dev, '*DBG* ~tp got sync_all_state_event ~tp from ~tw in state ~tw~n', [name, event, from, stateName])
  ...
  defp print_event(dev, {:out, msg, {to, _Tag}, stateName},
            name) do
    :io.format(dev, '*DBG* ~tp sent ~tp to ~tw~n      and switched to state ~tw~n', [name, msg, to, stateName])
  end
  ...
  defp handle_msg(msg, parent, name, stateName, stateData, mod,
            _Time, hibernateAfterTimeout) do
    from = from(msg)
    case ((try do
      ...
      {:EXIT,
        ...
        case (:logger.allow(:warning, :gen_fsm)) do
          true ->
            :erlang.apply(:logger, :macro_log,
                            [%{mfa: {:gen_fsm, :handle_msg, 8}, line: 494,
        ...
        loop(parent, name, stateName, stateData, mod, :infinity,
               hibernateAfterTimeout, [])
      {:EXIT, what} ->
        terminate(what, name, from, msg, mod, stateName,
                    stateData, [])
      reply ->
        terminate({:bad_return_value, reply}, name, from, msg,
                    mod, stateName, stateData, [])
    end
  end

  defp handle_msg(msg, parent, name, stateName, stateData, mod,
            _Time, hibernateAfterTimeout, debug) do
    from = from(msg)
    case ((try do
            dispatch(msg, mod, stateName, stateData)
          catch
  ...
  defp dispatch({:timeout, ref, {:"$gen_timer", msg}}, mod, stateName,
            stateData) do
    apply(mod, stateName, [{:timeout, ref, msg}, stateData])
  end

  defp dispatch({:timeout, _Ref, {:"$gen_event", event}}, mod, stateName,
            stateData) do
    apply(mod, stateName, [event, stateData])
  end
  ...
  defp terminate(reason, name, from, msg, mod, stateName,
            stateData, debug) do
    case (:erlang.function_exported(mod, :terminate, 3)) do
      true ->
        case ((try do
          ...
          {:EXIT, r} ->
            fmtStateData = format_status(:terminate, mod,
                                           :erlang.get(), stateData)
            error_info(r, name, from, msg, stateName, fmtStateData,
                         debug)
            exit(r)
          _ ->
            :ok
    ...
    case (reason) do
      ...
      {:shutdown, _} = shutdown ->
        exit(shutdown)
      _ ->
        fmtStateData1 = format_status(:terminate, mod,
                                        :erlang.get(), stateData)
  ...
  defp error_info(reason, name, from, msg, stateName, stateData,
            debug) do
    log = :sys.get_log(debug)
    case (:logger.allow(:error, :gen_fsm)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:gen_fsm, :error_info, 7}, line: 608, file: 'otp/lib/stdlib/src/gen_fsm.erl'},
                             :error, %{label: {:gen_fsm, :terminate},
                                         name: name, last_message: msg,
                                         state_name: stateName,
                                         state_data: stateData, log: log,
                                         reason: reason,
  ...
  defp client_stacktrace(pid) when (is_pid(pid) and
                       node(pid) === node()) do
  ...
  defp limit_report(%{label: {:gen_fsm, :terminate},
              last_message: msg, state_data: stateData, log: log,
              reason: reason, client_info: clientInfo} = report,
            depth) do
  ...
  defp limit_report(%{label: {:gen_fsm, :no_handle_info},
              message: msg} = report,
            depth) do
  ...
  defp format_log_single(%{label: {:gen_fsm, :terminate}, name: name,
              last_message: msg, state_name: stateName,
              state_data: stateData, log: log, reason: reason,
    ...
    {clientFmt,
       clientArgs} = format_client_log_single(clientInfo, p,
                                                depth)
    format = :lists.append(['State machine ', p, ' terminating. Reason: ', p, '. Last event: ', p, '. State: ', p, '. Data: ', p,
                                                           case (log) do
                                                             [] ->
                                                               ''
                                                             _ ->
                                                               ...
                                                               '.'])
    args0 = [name, fixedReason, get_msg(msg), stateName,
                                                  stateData] ++ (case (log) do
                                                                   [] ->
                                                                     []
                                                                   _ ->
                                                                     [log]
  ...
  defp format_log_single(%{label: {:gen_fsm, :no_handle_info},
              module: mod, message: msg},
            %{single_line: true, depth: depth} = formatOpts) do
    p = p(formatOpts)
  ...
  defp format_log_multi(%{label: {:gen_fsm, :terminate}, name: name,
              last_message: msg, state_name: stateName,
              state_data: stateData, log: log, reason: reason,
              client_info: clientInfo},
    ...
    {clientFmt, clientArgs} = format_client_log(clientInfo,
                                                  p, depth)
    format = :lists.append(['** State machine ', p, ' terminating \n' ++ get_msg_str(msg,
                                                     p) ++ '** When State == ',
                                      p, '~n', '**      Data  == ', p, '~n', '** Reason for termination ==~n** ', p, '~n', case (log) do
                                                                [] ->
                                                                  []
                                                                _ ->
                                                                  '** Log ==~n**' ++ p ++ '~n'
                                                              end])
    args0 = [name | get_msg(msg)] ++ [stateName, stateData,
                                                     fixedReason | case (log) do
                                                                     [] ->
                                                                       []
                                                                     _ ->
                                                                       [log]
  ...
  defp format_log_multi(%{label: {:gen_fsm, :no_handle_info},
              module: mod, message: msg},
            %{depth: depth} = formatOpts) do
    p = p(formatOpts)
  ...
  def format_status(opt, statusData) do
    [pDict, sysState, parent, debug, [name, stateName,
                                                stateData, mod, _Time,
                                                                    _HibernateAfterTimeout]] = statusData
    header = :gen.format_status_header('Status for state machine', name)
    log = :sys.get_log(debug)
    specific = (case (format_status(opt, mod, pDict,
                                      stateData)) do
                  s when is_list(s) ->
                    s
                  s ->
    ...
    [{:header, header}, {:data,
                           [{'Status', sysState}, {'Parent', parent}, {'Logged events', log}, {'StateName',
                                                                     stateName}]} |
                            specific]
  end

  defp format_status(opt, mod, pDict, state) do
    defStatus = (case (opt) do
                   :terminate ->
                     state
                   _ ->
