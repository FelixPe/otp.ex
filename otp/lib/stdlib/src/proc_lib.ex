defmodule :m_proc_lib do
  use Bitwise
  import Kernel, except: [spawn_link: 1, spawn_link: 3,
                            spawn: 3, spawn: 1]
  def spawn(f) when is_function(f) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn(:proc_lib, :init_p,
                    [parent, ancestors, f])
  end

  def spawn(m, f, a) when (is_atom(m) and is_atom(f) and
                          is_list(a)) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn(:proc_lib, :init_p,
  ...
  def spawn_link(f) when is_function(f) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_link(:proc_lib, :init_p,
                         [parent, ancestors, f])
  end

  def spawn_link(m, f, a) when (is_atom(m) and is_atom(f) and
                          is_list(a)) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_link(:proc_lib, :init_p,
  ...
  def spawn(node, f) when is_function(f) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn(node, :proc_lib, :init_p,
                    [parent, ancestors, f])
  end

  def spawn(node, m, f, a) when (is_atom(m) and
  ...
  def spawn_link(node, f) when is_function(f) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_link(node, :proc_lib, :init_p,
                         [parent, ancestors, f])
  end

  def spawn_link(node, m, f, a) when (is_atom(m) and
                                is_atom(f) and is_list(a)) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_link(node, :proc_lib, :init_p,
  ...
  def spawn_opt(f, opts) when is_function(f) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_opt(:proc_lib, :init_p,
                        [parent, ancestors, f], opts)
  end

  def spawn_opt(node, f, opts) when is_function(f) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_opt(node, :proc_lib, :init_p,
                        [parent, ancestors, f], opts)
  end

  def spawn_opt(m, f, a, opts) when (is_atom(m) and
                                is_atom(f) and is_list(a)) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_opt(:proc_lib, :init_p,
                        [parent, ancestors, m, f, a], opts)
  end

  def spawn_opt(node, m, f, a, opts) when (is_atom(m) and
                                      is_atom(f) and is_list(a)) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_opt(node, :proc_lib, :init_p,
  ...
  defp spawn_mon(m, f, a) do
    parent = get_my_name()
    ancestors = get_ancestors()
    :erlang.spawn_monitor(:proc_lib, :init_p,
                            [parent, ancestors, m, f, a])
  end

  def hibernate(m, f, a) when (is_atom(m) and is_atom(f) and
                          is_list(a)) do
  ...
  def init_p(parent, ancestors, m, f, a) when (is_atom(m) and
                                             is_atom(f) and is_list(a)) do
    :erlang.put(:"$ancestors", [parent | ancestors])
  ...
  def wake_up(m, f, a) when (is_atom(m) and is_atom(f) and
                          is_list(a)) do
    try do
      apply(m, f, a)
    catch
  ...
  defp exit_p(class, reason, stacktrace) do
    case (:erlang.get(:"$initial_call")) do
      {m, f, a} when (is_atom(m) and is_atom(f) and
                        is_integer(a))
                     ->
        mFA = {m, f, make_dummy_args(a, [])}
        crash_report(class, reason, mFA, stacktrace)
        :erlang.raise(:exit,
                        exit_reason(class, reason, stacktrace), stacktrace)
      _ ->
  ...
  def start(m, f, a) when (is_atom(m) and is_atom(f) and
                          is_list(a)) do
    start(m, f, a, :infinity)
  end

  def start(m, f, a, timeout) when (is_atom(m) and
                                   is_atom(f) and is_list(a)) do
    sync_start(spawn_mon(m, f, a), timeout)
  end

  def start(m, f, a, timeout, spawnOpts) when (is_atom(m) and
                                              is_atom(f) and is_list(a)) do
    monitor = :monitor
    case (:lists.member(monitor, spawnOpts)) do
      ...
      false ->
        case (:lists.keyfind(monitor, 1, spawnOpts)) do
          false ->
            :ok
          {^monitor, _} ->
            :erlang.error(:badarg, [m, f, a, timeout, spawnOpts])
    ...
    sync_start(:proc_lib.spawn_opt(m, f, a,
                                     [:monitor | spawnOpts]),
                 timeout)
  end
  ...
  def start_link(m, f, a) when (is_atom(m) and is_atom(f) and
                          is_list(a)) do
    start_link(m, f, a, :infinity)
  end

  def start_link(m, f, a, timeout) when (is_atom(m) and
                                   is_atom(f) and is_list(a)) do
    sync_start(:proc_lib.spawn_opt(m, f, a,
                                     [:link, :monitor]),
                 timeout)
  ...
  def start_link(m, f, a, timeout, spawnOpts) when (is_atom(m) and
                                              is_atom(f) and is_list(a)) do
    monitor = :monitor
    case (:lists.member(monitor, spawnOpts)) do
      true ->
  ...
  def start_monitor(m, f, a) when (is_atom(m) and is_atom(f) and
                          is_list(a)) do
    start_monitor(m, f, a, :infinity)
  end

  def start_monitor(m, f, a, timeout) when (is_atom(m) and
                                   is_atom(f) and is_list(a)) do
    sync_start_monitor(spawn_mon(m, f, a), timeout)
  end

  def start_monitor(m, f, a, timeout, spawnOpts) when (is_atom(m) and
                                              is_atom(f) and is_list(a)) do
    monitor = :monitor
    case (:lists.member(monitor, spawnOpts)) do
      true ->
  ...
  def init_fail(parent, return, exception) do
    _ = send(parent, {:nack, self(), return})
    case (exception) do
      {:error, reason} ->
        :erlang.error(reason)
      {:exit, reason} ->
  ...
  def init_fail(return, exception) do
    [parent | _] = :erlang.get(:"$ancestors")
  ...
  defp raw_initial_call({x, y, z}) when (is_integer(x) and
  ...
  defp trans_init(:gen, :init_it,
            [:gen_server, _, _, :supervisor, {_, module, _}, _]) do
    {:supervisor, module, 1}
  end

  defp trans_init(:gen, :init_it,
            [:gen_server, _, _, _, :supervisor, {_, module, _},
                                                    _]) do
    {:supervisor, module, 1}
  end

  defp trans_init(:gen, :init_it,
            [:gen_server, _, _, :supervisor_bridge, [module | _],
                                                        _]) do
    {:supervisor_bridge, module, 1}
  end

  defp trans_init(:gen, :init_it,
            [:gen_server, _, _, _, :supervisor_bridge, [module | _],
                                                           _]) do
    {:supervisor_bridge, module, 1}
  end
    ...
    case (:logger.allow(:error, :proc_lib)) do
      true ->
        :erlang.apply(:logger, :macro_log,
                        [%{mfa: {:proc_lib, :crash_report, 4}, line: 584,
                             file: 'otp/lib/stdlib/src/proc_lib.erl'},
                             :error, %{label: {:proc_lib, :crash},
                                         report:
  ...
  defp my_info_1(class, reason, stacktrace) do
    [{:pid, self()}, get_process_info(self(),
                                        :registered_name),
                         {:error_info, {class, reason, stacktrace}},
                             get_ancestors(self()), get_process_info(self(),
                                                                       :message_queue_len),
                                                        get_messages(self()),
  ...
  defp make_neighbour_report(pid) do
    [{:pid, pid}, get_process_info(pid, :registered_name),
                      get_initial_call(pid), get_process_info(pid,
                                                                :current_function),
                                                 get_ancestors(pid),
  ...
  defp proc_info(pid, item) do
    case (:lists.member(node(pid), :erlang.nodes())) do
      true ->
        check(:rpc.call(node(pid), :erlang, :process_info,
                          [pid, item]))
      _ ->
        :hidden
    end
  ...
  def report_cb(%{label: {:proc_lib, :crash},
             report: crashReport},
           extra) do
  ...
  def format(crashReport, encoding, depth) do
    do_format(crashReport,
                %{chars_limit: :unlimited, depth: depth,
                    encoding: encoding, single_line: false})
  end

  defp do_format([ownReport, linkReport], extra) do
    %{encoding: enc, single_line: single,
        chars_limit: limit0} = extra
    indent = (cond do
                single ->
                  ''
                true ->
    ...
    {partLimit, limit} = (case (limit0) do
                            :unlimited ->
                              {limit0, limit0}
                            _ when is_integer(limit0) ->
                              num = length(ownReport)
                              hardcodedSize = length(indent) + length('crasher') + length(nl) + length(sep) + length(sep) * num
                              limit1 = max(limit0 - hardcodedSize, 1)
                              eL = div(limit1, 3)
                              pL = div(limit1 - eL, num)
                              {pL, limit1}
                          end)
    linkFormat = format_link_reports(linkReport, indent,
                                       extra, partLimit)
  ...
  defp format_own_report(ownReport, indent, extra, linkFormatSize,
            partLimit, limit0) do
    ...
    case (separate_error_info(ownReport)) do
      {first, {class, reason, stackTrace}, rest} ->
        ...
        limit = (case (limit0) do
                   :unlimited ->
                     limit0
                   _ when is_integer(limit0) ->
                     sizeOfOther = size(enc, f) + size(enc,
                                                         r) - length(sep) * (length(f) + length(r)) + linkFormatSize
                     max(limit0 - sizeOfOther, 1)
                 end)
        eI = format_exception(class, reason, stackTrace, extra,
                                limit)
  ...
  defp format_link_reports(linkReports, indent, extra, partLimit)
    ...
    %{encoding: enc, depth: depth,
        single_line: single} = extra
    pids = (for {:neighbour,
                   [{:pid, p} | _]} <- linkReports do
              p
            end)
    {p, tl} = p(enc, depth)
    width = (cond do
  ...
  defp format_link_reports(linkReports, indent, extra, partLimit) do
    %{single_line: single} = extra
    myIndent = indent ++ indent
    linkFormat = :lists.join(nl(single, report_separator()),
  ...
  defp format_link_report([link | reps], indent0, extra, partLimit) do
    ...
    linkIndent = ['  ', indent]
    [[indent, 'neighbour:', nl(single, ' '), format_report(rep,
                                                linkIndent, extra, partLimit)] |
  ...
  defp format_report(rep, indent0, extra, limit) do
    ...
    :io_lib.format('~s~' ++ width ++ p, [indent, rep | tl],
                     opts)
  end

  defp format_rep([{:initial_call, initialCall} | rep], indent,
            extra, limit) do
  ...
  defp format_exception(class, reason, stackTrace, extra, limit) do
    ...
    cond do
      single ->
        {p, tl} = p(enc, depth)
        opts = chars_limit_opt(limit)
        [:erlang.atom_to_list(class), ': ', :io_lib.format('~0' ++ p,
                                                          [{reason,
                                                              stackTrace} |
                                                               tl],
                                                          opts)]
      true ->
        ...
        fE = :erl_error.format_exception(1 + length(eI), class,
                                           reason, stackTrace, stackFun, pF,
                                           enc, lim)
        [eI, fE]
    end
  end
  ...
  defp pp_fun(extra, enc) do
    ...
    fn term, i, limit ->
         s = :io_lib.format('~' ++ width ++ '.' ++ :erlang.integer_to_list(i) ++ p,
  ...
  def stop(process, reason, timeout) do
    ...
    remainingTimeout = (try do
                          ...
                          :ok when timeout === :infinity ->
                            :infinity
                          :ok ->
                            timeout - (div(:erlang.monotonic_time(:microsecond) + 999, 1000) - t0)
                        end)
    receive do
