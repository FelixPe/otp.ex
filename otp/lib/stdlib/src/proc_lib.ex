defmodule :m_proc_lib do
  ...
  def start_link(m, f, a, timeout)
      when is_atom(m) and
             is_atom(f) and is_list(a) do
    sync_start(
      :proc_lib.spawn_opt(m, f, a, [:link, :monitor]),
      timeout
    )
  end
  ...
  def start_link(m, f, a, timeout, spawnOpts)
    ...
    sync_start(
      :proc_lib.spawn_opt(m, f, a, [:link, :monitor | spawnOpts]),
      timeout
    )
  end
  ...
  defp make_dummy_args(n, acc) do
    arg = :erlang.list_to_atom(~c"Argument__" ++ :erlang.integer_to_list(n))
      ...
      true ->
        :erlang.apply(:logger, :macro_log, [
          %{
            ...
            line: 584,
            file: ~c"otp/lib/stdlib/src/proc_lib.erl"
          },
          :error,
          %{
    ...
    {partLimit, limit} =
      case limit0 do
        ...
        _ when is_integer(limit0) ->
          ...
          hardcodedSize =
            length(indent) + length(~c"crasher") + length(nl) + length(sep) + length(sep) * num
  ...
  defp format_link_reports(linkReports, indent, extra, partLimit)
    ...
    :io_lib.format(indent ++ ~c"neighbours: ~" ++ width ++ p, [pids | tl], [
  ...
  defp format_link_reports(linkReports, indent, extra, partLimit) do
    ...
    [indent, ~c"neighbours:", nl(single, ~c" "), linkFormat]
  end
  ...
  defp format_report(rep, indent0, extra, limit) do
    ...
    :io_lib.format(~c"~s~" ++ width ++ p, [indent, rep | tl], opts)
  end
  ...
  defp pp_fun(extra, enc) do
    ...
    fn term, i, limit ->
      s =
        :io_lib.format(~c"~" ++ width ++ ~c"." ++ :erlang.integer_to_list(i) ++ p, [term | tl], [
          {:chars_limit, limit}
        ])
