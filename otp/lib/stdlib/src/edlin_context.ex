defmodule :m_edlin_context do
  ...
  def get_context(~c">-" ++ _, l) when is_list(l) do
    {:term}
  end
  ...
  def get_context(
        [],
        r_context(arguments: args, parameter_count: count, nestings: nestings) = _CR
      ) do
        ...
        case mod do
          ...
          _ ->
            case r_context(cR, :parameter_count) + 1 == length(r_context(cR, :arguments)) do
              true ->
                {:function, mod, fun, :lists.droplast(r_context(cR, :arguments)),
                 :lists.last(r_context(cR, :arguments)), r_context(cR, :nestings)}

              _ ->
                {:function, mod, fun, r_context(cR, :arguments), [], r_context(cR, :nestings)}
            end
        end
    end
  end
  ...
  def get_context(
    ...
    {args, unfinished} =
      case count + 1 == length(arguments) do
        true ->
        ...
        case map do
          [] ->
            get_context(
              ...
              r_context(
                nestings: [
                  {:map, fields, fieldToComplete, args, unfinished}
                  | nestings
                ]
              )
            )
  ...
  def get_context(
        [?[ | bef1],
        r_context(arguments: arguments, parameter_count: count, nestings: nestings)
      ) do
    {args, unfinished} =
      case count + 1 == length(arguments) do
        true ->
  ...
  def get_context(
        [?>, ?= | bef1],
        r_context(parameter_count: count, fields: fields) = cR
      ) do
  ...
  def get_context([?. | bef2], cR) do
    ...
    case :edlin_expand.over_word(bef2) do
      {[?# | _Bef3], record} ->
        {:record, record, r_context(cR, :fields), r_context(cR, :current_field), args, unfinished,
  ...
  def get_context(~c"nehw " ++ _Bef2, r_context(arguments: args) = cR) do
    case r_context(cR, :parameter_count) + 1 == length(args) do
      true ->
        {:term, :lists.droplast(args), :lists.last(args)}
  ...
  def get_context(
        bef0,
        r_context(arguments: args, parameter_count: count) = cR
      ) do
    case over_to_opening(bef0) do
      ...
      {bef1, {:fun_, str} = arg} ->
        case count do
          0 ->
            [_, mod, fun | _] = :string.tokens(str, ~c" :/")
            {:fun_, mod, fun}
  ...
  defp read_operator1(~c"mer " ++ bef) do
    {bef, ~c"rem"}
  end

  defp read_operator1(~c"osladna " ++ bef) do
    {bef, ~c"andalso"}
  end

  defp read_operator1(~c"dna " ++ bef) do
    {bef, ~c"and"}
  end

  defp read_operator1(~c"eslero " ++ bef) do
    {bef, ~c"orelse"}
  end

  defp read_operator1(~c"ro " ++ bef) do
    {bef, ~c"or"}
  end
  ...
  defp read_operator1([op1, op2, op3 | bef]) do
    case {:lists.member(op1, operator_string()), :lists.member(op2, operator_string()),
  ...
  defp read_operator1([op1, op2]) do
    case {:lists.member(op1, operator_string()), :lists.member(op2, operator_string())} do
      {true, true} ->
        {[], [op2, op1]}
  ...
  defp read_opening_char(~c"nehw " ++ bef) do
    {bef, ~c"when"}
  end

  defp read_opening_char([oC | bef])
       when oC === ?( or oC === ?[ or
              oC === ?{ or oC === ?, or oC === ?. do
    {bef, [oC]}
  end
      ...
      {bef3, clause} ->
        ...
        case maybeRecord do
          ...
          _Record ->
            [?# | bef5] = bef4
            {bef6, _Var} = :edlin_expand.over_word(bef5)
            {bef6, {:record, _Var ++ ~c"#" ++ _Record ++ clause}}
        end
    end
  end
  ...
  defp over_pid_port_or_ref(bef2) do
    case over_to_opening_paren(?>, bef2) do
      ...
      {bef3, clause} ->
        case bef3 do
          ~c"feR#" ++ bef4 ->
            {bef4, {:ref, ~c"#Ref" ++ clause}}

          ~c"nuF#" ++ bef4 ->
            {bef4, {:funref, ~c"#Fun" ++ clause}}

          ~c"troP#" ++ bef4 ->
            {bef4, {:port, ~c"#Port" ++ clause}}
  ...
  defp over_keyword_or_fun(bef1) do
    case over_keyword_expression(bef1) do
      {bef2, keywordExpression} ->
        {bef2, {:keyword, keywordExpression ++ ~c" end"}}
        ...
        try do
          ...
          _ ->
            case over_fun_function(bef0) do
              {bef3, ~c"fun " ++ _ModFunArr} ->
                {bef3, {:fun_, ~c"fun " ++ _ModFunArr}}
            ...
            {bef6, {numberType, number}} =
              ...
              case :edlin_expand.over_word(bef2) do
                {[?. | bef3], []} ->
                  ...
                  {bef4, {:float, integer ++ ~c"." ++ var}}

                {[?# | bef3], []} ->
                  {bef4, base} = :edlin_expand.over_word(bef3)
                  {bef4, {:integer, base ++ ~c"#" ++ var}}
    ...
    case :edlin_expand.over_word(bef) do
      ...
      {~c" nuf" ++ bef1, modOrFun} ->
        case to_atom(modOrFun) do
          {:ok, _} ->
            over_fun_function(bef1, ~c"fun " ++ modOrFun ++ acc)
  ...
  defp over_to_opening_paren(cC, bef)
       when cC == ?) or cC == ?] or
              cC == ?} or cC == ?> do
  ...
  defp over_to_opening_paren([cC | stack], [oC | bef], word)
       when oC == ?( or
              oC == ?[ or oC == ?{ or
              oC == ?< do
  ...
  defp over_to_opening_paren(stack, [q, nEC | bef], word)
       when q == ?" or
              (q == ?' and nEC != ?$ and
                 nEC != ?\\) do
  ...
  defp over_keyword_expression(~c"dne" ++ bef, expr) do
    {bef1, kWE} = over_keyword_expression(bef)
    over_keyword_expression(bef1, kWE ++ ~c"end" ++ expr)
  end

  defp over_keyword_expression(~c"fi" ++ bef, expr) do
    {bef, ~c"if" ++ expr}
  end

  defp over_keyword_expression(~c"nuf" ++ bef, expr) do
    {bef, ~c"fun" ++ expr}
  end

  defp over_keyword_expression(~c"yrt" ++ bef, expr) do
    {bef, ~c"try" ++ expr}
  end

  defp over_keyword_expression(~c"esac" ++ bef, expr) do
    {bef, ~c"case" ++ expr}
  end

  defp over_keyword_expression(~c"hctac" ++ bef, expr) do
    case over_keyword_expression(bef, []) do
      {bef1, ~c"try" ++ expr1} ->
        {bef1, ~c"try" ++ expr1 ++ ~c"catch" ++ expr}
  ...
  defp over_keyword_expression(~c"nigeb" ++ bef, expr) do
    {bef, ~c"begin" ++ expr}
  end

  defp over_keyword_expression(~c"ebyam" ++ bef, expr) do
    {bef, ~c"maybe" ++ expr}
  end

  defp over_keyword_expression(~c"eviecer" ++ bef, expr) do
    {bef, ~c"receive" ++ expr}
  end
  ...
  defp odd_quotes(q, [q, c | line], acc)
       when c == ?\\ or
              c == ?$ do
  ...
  defp is_binding(word) do
    ...
    :nomatch !== :re.run(normalized, ~c"^[_[:upper:]][[:alpha:]]*$", [:unicode, :ucp])
  end
