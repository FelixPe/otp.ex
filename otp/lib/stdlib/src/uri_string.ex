defmodule :m_uri_string do
  ...
  def transcode(uRIString, options) when is_binary(uRIString) do
    try do
      inEnc = :proplists.get_value(:in_encoding, options,
                                     :utf8)
      outEnc = :proplists.get_value(:out_encoding, options,
                                      :utf8)
  ...
  def allowed_characters() do
    ...
    [{:scheme, scheme}, {:userinfo, userInfo}, {:host,
                                                  host},
                                                   {:ipv4, iPv4}, {:ipv6, iPv6},
                                                                      {:regname,
                                                                         regName},
  ...
  def percent_decode(uRIMap) when is_map(uRIMap) do
    fun = fn k, v when k === :userinfo or k === :host or
  ...
  def quote(d) do
    encode(d, &is_unreserved/1)
  end

  def quote(d, safe) do
    unreservedOrSafe = fn c ->
                            is_unreserved(c) or :lists.member(c, safe)
                       end
  ...
  defp is_alpha(c) when (?A <= c and c <= ?Z) or
                    (?a <= c and c <= ?z) do
    true
  end
  ...
  defp is_hex_digit(c) when (?0 <= c and c <= ?9) or
  ...
  defp get_parsed_binary(input, unparsed) do
    {first, _} = :erlang.split_binary(input,
  ...
  defp hex_to_upper(h) when (?0 <= h and h <= ?9) or
                    (?A <= h and h <= ?F) do
    h
  end
  ...
  defp percent_encode_binary(<<a :: size(4), b :: size(4), rest :: binary>>,
            acc) do
  ...
  defp is_valid_map(%{path: path} = map) do
    starts_with_two_slash(path) and is_valid_map_host(map) or :maps.is_key(:userinfo,
                                                                             map) and is_valid_map_host(map) or :maps.is_key(:port,
                                                                                                                               map) and is_valid_map_host(map) or all_fields_valid(map)
  end
  ...
  defp resolve_path_type(uRIMap) do
    case (:erlang.iolist_to_binary(:maps.get(:path, uRIMap,
                                               <<>>))) do
      <<>> ->
        :empty_path
      <<?/, _ :: bits>> ->
  ...
  defp resolve_map(uRI = %{path: path}, baseURI, :relative_path) do
    normalize_path_segment(:maps.merge(Map.put(uRI, :path,
                                                      merge_paths(path,
                                                                    baseURI)),
                                         :maps.with([:scheme, :userinfo, :host,
                                                                             :port],
  ...
  defp transcode([?%, _C0, _C1 | _Rest] = l, acc, inEnc,
            outEnc) do
  ...
  defp transcode([?%, _C0, _C1 | _Rest] = l, acc, list,
            inEncoding, outEncoding) do
    transcode_pct(l, list ++ acc, <<>>, inEncoding,
                    outEncoding)
  end

  defp transcode([c | rest], acc, list, inEncoding,
            outEncoding) do
    transcode(rest, acc, [c | list], inEncoding,
                outEncoding)
  end
  ...
  defp transcode_pct([?%, c0, c1 | rest] = l, acc, b, inEncoding,
            outEncoding) do
    case (is_hex_digit(c0) and is_hex_digit(c1)) do
      true ->
        int = (cond do
        ...
        transcode_pct(rest, acc, <<b :: binary, int>>,
  ...
  defp transcode_pct([_C | _Rest] = l, acc, b, inEncoding,
            outEncoding) do
  ...
  defp convert_to_binary(binary, inEncoding, outEncoding) do
    case (:unicode.characters_to_binary(binary, inEncoding,
                                          outEncoding)) do
  ...
  defp convert_to_list(binary, inEncoding) do
    case (:unicode.characters_to_list(binary,
                                        inEncoding)) do
      {:error, _List, restData} ->
        throw({:error, :invalid_input, restData})
      {:incomplete, _List, restData} ->
  ...
  defp form_urlencode(cs, [{:encoding, encoding}]) when is_list(cs) or
                                              is_binary(cs) do
  ...
  defp base10_encode(<<h :: utf8, t :: binary>>, acc) when h > 255 do
    base10 = convert_to_binary(:erlang.integer_to_list(h,
                                                         10),
                                 :utf8, :utf8)
    base10_encode(t,
  ...
  defp is_url_char(c) when c === 42 or c === 45 or c === 46 or
  ...
  defp dissect_query_key(<<"&#", t :: binary>>, isList, acc, key, value) do
    dissect_query_key(t, isList, acc, <<key :: binary, "&#">>,
                        value)
  end

  defp dissect_query_key(t = <<?&, _ :: binary>>, isList, acc, key,
            <<>>) do
    dissect_query_value(t, isList, acc, key, true)
  end

  defp dissect_query_key(<<h, t :: binary>>, isList, acc, key, value) do
    dissect_query_key(t, isList, acc, <<key :: binary, h>>,
                        value)
  end
  ...
  defp normalize_case(%{scheme: scheme, host: host} = map) do
    Map.merge(map, %{scheme: to_lower(scheme),
                       host: to_lower(host)})
  end
  ...
  defp to_lower(<<c, cs :: binary>>, acc) when (?A <= c and
                                            c <= ?Z) do
  ...
  defp first_path_segment(<<?/, t :: binary>>, acc) do
    first_path_segment_end(<<t :: binary>>,
                             <<acc :: binary, ?/>>)
  end
  ...
  defp normalize_scheme_based(map, scheme, port, path) when scheme === 'http' or
                                          scheme === "http" do
    normalize_http(map, port, path)
  end

  defp normalize_scheme_based(map, scheme, port, path) when scheme === 'https' or
                                          scheme === "https" do
    normalize_https(map, port, path)
  end

  defp normalize_scheme_based(map, scheme, port, _Path) when scheme === 'ftp' or
                                           scheme === "ftp" do
    normalize_ftp(map, port)
  end

  defp normalize_scheme_based(map, scheme, port, _Path) when scheme === 'ssh' or
                                           scheme === "ssh" do
    normalize_ssh_sftp(map, port)
  end

  defp normalize_scheme_based(map, scheme, port, _Path) when scheme === 'sftp' or
                                           scheme === "sftp" do
    normalize_ssh_sftp(map, port)
  end

  defp normalize_scheme_based(map, scheme, port, _Path) when scheme === 'tftp' or
                                           scheme === "tftp" do
  ...
  defp normalize_default_port(map, port, default) do
    case (port) do
      ^default ->
  ...
  defp normalize_undefined_port(%{port: :undefined} = map) do
    :maps.remove(:port, map)
  end

  defp normalize_undefined_port(%{} = map) do
    map
  end
