defmodule :m_ets do
      ...
      {:fun_data, importList, clauses} ->
        case :ms_transform.transform_from_shell(:ets, clauses, importList) do
          {:error, [{_, [{_, _, code} | _]} | _], _} ->
            :io.format(~c"Error: ~ts~n", [:ms_transform.format_error(code)])
  ...
  def tab2file(table, file, options) do
    try do
      ...
      case :disk_log.open([{:name, name}, {:file, file}, {:repair, :truncate}]) do
        {:ok, ^name} ->
          :ok
      ...
      try do
        ...
        {newState1, num} =
          try do
            ...
            dump_file(:ets.select(table, [{:_, [], [:"$_"]}], 100), logFun, newState, 0)
          after
            try do
  ...
  def file2tab(file, opts) do
    try do
      ...
      try do
        ...
        try do
          do_read_and_verify(readFun, initState, table, strippedOptions, headCount, verify)
        catch
          tReason ->
            :ets.delete(table)
            throw(tReason)
  ...
  defp do_read_and_verify(readFun, initState, table, ftOptions, headCount, verify) do
      ...
      [[traverse, nObjs], qlcOptions] ->
        ...
        formatFun = fn
          ...
          {:lookup, _KeyPos, [value], _NElements, elementFun} ->
            :io_lib.format(~c"~w:lookup(~w, ~w)", [:ets, table, elementFun.(value)])
  ...
  defp choice(height, width, p, mode, table, key, turn, opos) do
    case get_line(p, ~c"c\n") do
      ~c"c\n" when mode === :normal ->
  ...
  defp do_display_item(_Height, width, i, opos) do
    ...
    l2 =
      cond do
        length(l) > width - 8 ->
          :string.slice(l, 0, width - 13) ++ ~c"  ..."

        true ->
          l
      end

    :io.format(~c"<~-4w> ~s~n", [opos, l2])
  end
  ...
  defp re_display(height, width, table, key, [], re, turn, opos) do
    re_search(height, width, table, :ets.next(table, key), re, turn, opos)
  end

  defp re_display(height, width, table, key, [h | t], re, turn, opos) do
    str = to_string(h)
