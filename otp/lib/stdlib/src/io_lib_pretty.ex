defmodule :m_io_lib_pretty do
  ...
  defp print(term, col, ll, d, m, t, recDefFun, enc, str, ord)
       when col <= 0 do
    print(term, 1, ll, d, m, t, recDefFun, enc, str, ord)
  end

  defp print(atom, _Col, _Ll, _D, _M, _T, _RF, enc, _Str, _Ord)
       when is_atom(atom) do
    write_atom(atom, enc)
  end

  defp print(term, col, ll, d, m0, t, recDefFun, enc, str, ord)
    ...
    {_, len, _Dots, _} =
      ...
      case t < 0 do
        true ->
          print_length(term, d, t, recDefFun, enc, str, ord)

        false ->
          intermediate(term, d, t, recDefFun, enc, str, ord)
      end
  ...
  defp print(term, _Col, _Ll, _D, _M, _T, _RF, _Enc, _Str, _Ord) do
    :io_lib.write(term)
  end
  ...
  defp pp_tag_tuple({:dots, _, _, _}, _Col, _Ll, _M, _TInd, _Ind, _LD, _W) do
    ~c"..."
  end
  ...
  def intermediate(term, d, t, rF, enc, str, ord) when t > 0 do
    ...
    case if__ do
      ...
      {_, len, _, _} ->
        find_upper(if__, term, t, d0, 2, d, rF, enc, str, ord, len)
    end
  end

  defp find_upper(lower, term, t, dl, dd, d, rF, enc, str, ord, lastLen) do
    dd2 = dd * 2
    ...
    case if__ do
      ...
      {_, len, _, _} when (len <= t and d1 < d) or d < 0 ->
        find_upper(if__, term, t, d1, dd2, d, rF, enc, str, ord, len)

      _ ->
        search_depth(lower, if__, term, t, dl, d1, rF, enc, str, ord)
    end
  end

  defp search_depth(lower, upper, _Term, t, dl, du, _RF, _Enc, _Str, _Ord)
       when du - dl === 1 do
    case lower do
      {_, ^t, _, _} ->
        lower
  ...
  defp search_depth(lower, upper, term, t, dl, du, rF, enc, str, ord) do
    d1 = div(dl + du, 2)
    if__ = expand(lower, t, d1 - dl)
    ...
    case if__ do
      ...
      _ ->
        search_depth(if__, upper, term, t, d1, du, rF, enc, str, ord)
    end
  end
  ...
  defp print_length(list, d, t, rF, enc, str, ord)
    ...
    case str and printable_list(list, d, t, enc) do
      ...
      {true, prefix} ->
        ...
        {[s | ~c"..."], 3 + :io_lib.chars_length(s), 0, :no_more}

      false ->
        case print_length_list(list, d, t, rF, enc, str, ord) do
  ...
  defp print_length(<<_::bitstring>> = bin, 1, _T, rF, enc, str, ord) do
    more = fn t1, dd ->
  ...
  defp print_length(<<_::bitstring>> = bin, d, t, rF, enc, str, ord) do
    d1 = d - 1
    ...
    case str and rem(bit_size(bin), 8) === 0 and
      ...
      {false, list} when is_list(list) ->
        s = :io_lib.write_string(list, ?")
        {[?<, ?<, s, ~c"/utf8>>"], 9 + :io_lib.chars_length(s), 0, :no_more}
      ...
      {false, true, prefix} ->
        ...
        {[?<, ?<, s | ~c"/utf8...>>"], 12 + :io_lib.chars_length(s), 3, more}
  ...
  defp print_length_map_pairs(term, d, d0, t, rF, enc, str, ord)
       when d === 1 or t === 0 do
    more = fn t1, dd ->
      print_length_map_pairs(term, d + dd, d0, t1, rF, enc, str, ord)
    end
  ...
  defp print_length_map_pairs({k, v, iter}, d, d0, t, rF, enc, str, ord) do
    ...
    {_, len1, _, _} = pair1
    [pair1 | print_length_map_pairs(next, d - 1, d0, tsub(t1, len1), rF, enc, str, ord)]
  end
  ...
  defp print_length_tuple1(tuple, i, d, t, rF, enc, str, ord)
       when d === 1 or t === 0 do
    more = fn t1, dd ->
      print_length_tuple1(tuple, i, d + dd, t1, rF, enc, str, ord)
    end
  ...
  defp print_length_record(tuple, 1, _T, rF, rDefs, enc, str, ord) do
    more = fn t1, dd ->
      print_length_record(tuple, 1 + dd, t1, rF, rDefs, enc, str, ord)
    end
  ...
  defp print_length_fields(term, d, t, tuple, i, rF, enc, str, ord)
       when d === 1 or t === 0 do
    more = fn t1, dd ->
      print_length_fields(term, d + dd, t1, tuple, i, rF, enc, str, ord)
    end

    {:dots, 3, 3, more}
  end

  defp print_length_fields([def__ | defs], d, t, tuple, i, rF, enc, str, ord) do
    e = :erlang.element(i, tuple)
    ...
    field1 = print_length_field(def__, d - 1, t1, e, rF, enc, str, ord)
    {_, len1, _, _} = field1
    t2 = tsub(t1, len1)
    [field1 | print_length_fields(defs, d - 1, t2, tuple, i + 1, rF, enc, str, ord)]
  end

  defp print_length_field(def__, d, t, e, rF, enc, str, ord) do
    name = write_atom(def__, enc)
    nameL = :io_lib.chars_length(name) + 3
    {_, len, dots, _} = field = print_length(e, d, tsub(t, nameL), rF, enc, str, ord)
  ...
  defp print_length_list1(term, d, t, rF, enc, str, ord)
       when d === 1 or
              t === 0 do
    more = fn t1, dd ->
  ...
  defp printable_latin1_list([c | cs], n)
