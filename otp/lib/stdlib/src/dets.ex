defmodule :m_dets do
  ...
  Record.defrecord(:r_cache, :cache, cache: :undefined,
                                 ...
                                 delay: :undefined)
  Record.defrecord(:r_dets_cont, :dets_cont, what: :undefined,
                                     no_objs: :undefined, bin: :undefined,
                                     alloc: :undefined, tab: :undefined,
                                     proc: :undefined,
                                     match_program: :undefined)
  Record.defrecord(:r_open_args, :open_args, file: :undefined,
                                     type: :undefined, keypos: :undefined,
                                     repair: :undefined,
                                     min_no_slots: :undefined,
                                     max_no_slots: :undefined,
                                     ram_file: :undefined,
                                     delayed_write: :undefined,
                                     auto_save: :undefined, access: :undefined,
                                     debug: :undefined)
  ...
  def init_table(tab, initFun, options)
    ...
    case (options(options, [:format, :min_no_slots])) do
      ...
      [format, minNoSlots] ->
        case (treq(tab,
                     {:initialize, initFun, format, minNoSlots})) do
          {:thrown, thrown} ->
            throw(thrown)
          else__ ->
  ...
  def is_compatible_bchunk_format(tab, term) do
    badarg(treq(tab, {:is_compatible_bchunk_format, term}),
             [tab, term])
  end
  ...
  def match(tab, pat, n) do
    badarg(init_chunk_match(tab, pat, :bindings, n,
                              :no_safe),
             [tab, pat, n])
  end
  ...
  defp do_match_delete(proc, {:cont, state, n1}, what, n) do
    do_match_delete(proc, req(proc, {:match_delete, state}),
                      what, n + n1)
  end
  ...
  def repair_continuation(r_dets_cont(match_program: {:match_spec, b}) = cont, mS) do
    case (:ets.is_compiled_ms(b)) do
      true ->
        cont
      false ->
        r_dets_cont(cont, match_program: {:match_spec,
  ...
  def slot(tab, slot) when (is_integer(slot) and
                            slot >= 0) do
  ...
  def istart_link(server) do
    {:ok,
       :proc_lib.spawn_link(:dets, :init, [self(), server])}
  end
    ...
    case (options(opts, [:traverse, :n_objects])) do
      ...
      [traverse, nObjs] ->
        ...
        formatFun = fn {:all, _NElements, _ElementFun} ->
                         as = [tab | for _ <- [[]], opts !== [] do
                                       opts
                                     end]
                         {:dets, :table, as}
                       {:match_spec, mS} ->
                         {:dets, :table,
                            [tab, [{:traverse, {:select, mS}} | listify(opts)]]}
  ...
  def update_counter(tab, key, c) do
    badarg(treq(tab, {:update_counter, key, c}),
             [tab, key, c])
  end
  ...
  defp do_trav(state, proc, acc, fun) do
    case (req(proc, {:match_init, state, :safe})) do
      :"$end_of_table" ->
        acc
      {:cont, {bins, newState}} ->
        do_trav_bins(newState, proc, acc, fun,
                       :lists.reverse(bins))
      error ->
        error
    end
  ...
  defp safe_match(tab, pat, what) do
    do_safe_match(init_chunk_match(tab, pat, what, :default,
                                     :safe),
                    [])
  end
  ...
  defp init_chunk_match(tab, pat, what, n, safe) when (is_integer(n) and
                                           n >= 0) or
                                          n === :default do
    case (compile_match_spec(what, pat)) do
      {spec, mP} ->
        case ((try do
          ...
          proc ->
            case (req(proc, {:match, mP, spec, n, safe})) do
              {:done, l} ->
                {l,
                   r_dets_cont(tab: tab, proc: proc, what: what, bin: :eof,
                       no_objs: :default, alloc: <<>>)}
              {:cont, state} ->
                chunk_match(r_dets_cont(state, what: what,  tab: tab, 
                                       proc: proc),
                              safe)
              error ->
                error
  ...
  defp defaults(tab, args) do
    defaults0 = r_open_args(file: to_list(tab), type: :set, keypos: 1,
                    repair: true, min_no_slots: :default,
                    max_no_slots: :default, ram_file: false,
                    delayed_write: {3000, 14000},
                    auto_save: :timer.minutes(3), access: :read_write,
                    debug: false)
    fun = &repl/2
  ...
  defp repl({:keypos, p}, defs) when (is_integer(p) and
                                      p > 0) do
  ...
  defp is_max_no_slots(i) when (is_integer(i) and i > 0 and
                     i < 1 <<< 31) do
    i
  end
  ...
  defp options(options, [key | keys], l)
    ...
    v = (case (:lists.keysearch(key, 1, options)) do
           {:value, {:format, format}} when format === :term or
                                              format === :bchunk
                                            ->
             {:ok, format}
  ...
  defp req(proc, r) do
    ...
    send(proc, {:"$dets_call", {self(), ref}, r})
    receive do
  ...
  defp pidof({pid, _Tag}) do
    pid
  end

  defp resp({pid, tag} = _From, message) do
  ...
  defp open_file_loop(head, n) when :erlang.element(1,
                                          r_head(head, :update_mode)) === :error do
  ...
  defp open_file_loop(head, n) do
    receive do
      ...
      {:EXIT, pid, _Reason} ->
        ...
        open_file_loop(h2, n)
      {:system, from, req} ->
        :sys.handle_system_msg(req, from, r_head(head, :parent),
                                 :dets, [], head)
    after 0 ->
      open_file_loop2(head, n)
    end
  ...
  defp apply_op(op, from, head, n) do
    case (op) do
      {:add_user, tab, openArgs} ->
        r_open_args(file: fname, type: type, keypos: keypos,
      ...
      {:internal_open, ref, args} ->
        do_internal_open(r_head(head, :parent), r_head(head, :server),
                           from, ref, args)
      ...
      {:where, object} ->
        ...
        resp(from, res)
        h2
      _Message when :erlang.element(1,
                                      r_head(head, :update_mode)) === :error
                    ->
      ...
      writeOp when (r_head(head, :access) === :read_write and
      ...
      writeOp when (is_tuple(writeOp) and
  ...
  defp bug_found(name, op, bad, stacktrace, from) do
    case (:dets_utils.debug_mode()) do
      true ->
        :error_logger.format('** dets: Bug was found when accessing table ~tw,~n** dets: operation was ~tp and reply was ~tw.~n** dets: Stacktrace: ~tw~n', [name, op, bad, stacktrace])
      false ->
  ...
  defp start_auto_save_timer(head) do
    ...
    _Ref = :erlang.send_after(millis, self(),
  ...
  defp stream_op(head, pids, c, n, pid, {:lookup_keys, keys},
            fxd, m) do
    nC = [{{:lookup, pid}, keys} | c]
    stream_loop(head, pids, nC, n, fxd, m)
  end

  defp stream_op(head, pids, c, n, pid, {:insert, _Objects} = op,
            fxd, :dirty = m) do
    nC = [op | c]
    stream_loop(head, [pid | pids], nC, n, fxd, m)
  end

  defp stream_op(head, pids, c, n, pid,
            {:delete_key, _Keys} = op, fxd, :dirty = m) do
    nC = [op | c]
    stream_loop(head, [pid | pids], nC, n, fxd, m)
  end

  defp stream_op(head, pids, c, n, pid,
            {:delete_object, _Os} = op, fxd, :dirty = m) do
    nC = [op | c]
    stream_loop(head, [pid | pids], nC, n, fxd, m)
  end

  defp stream_op(head, pids, c, n, pid, {:member, key}, fxd,
            m) do
    nC = [{{:lookup, [pid]}, [key]} | c]
    stream_loop(head, pids, nC, n, fxd, m)
  end
  ...
  defp stream_end(head, pids0, c, n, next) do
    case ((try do
      ...
      {head1, error} when elem(head1, 0) === :head ->
        ...
        stream_end2(pids0 ++ lPs, pids0, next, n, c, head1,
                      error)
      detsError ->
        throw(detsError)
    end
  ...
  defp penalty(r_head(fixed: {_, [{pid, _}]}),
            [{pid, _Tag} = _From], _C) do
    :ok
  end
  ...
  defp read_file_header(fileName, access, ramFile) do
    ...
    {:ok, fd} = :dets_utils.open(bF,
                                   open_args(access, ramFile))
    {:ok,
       <<version :: size(32)>>} = :dets_utils.pread_close(fd,
                                                            fileName, 16, 4)
    cond do
      version <= 8 ->
        _ = :file.close(fd)
        throw({:error,
                 {:format_8_no_longer_supported, fileName}})
      version === 9 ->
      ...
      {h2, []} ->
        case (:dets_v9.table_parameters(h2)) do
          ...
          parms ->
            ...
            {h2,
               {r_dets_cont(no_objs: :default, bin: bin, alloc: l, tab: tab,
                    proc: self(), what: :bchunk),
                  [binParms]}}
        end
  ...
  defp do_delete_all_objects(head) do
    r_head(fptr: fd, name: tab, filename: fname, type: type,
        keypos: kp, ram_file: ram, auto_save: auto,
    ...
    :dets_v9.initiate_file(fd, tab, fname, type, kp,
                             minSlots, maxSlots, ram, cacheSz, auto, true)
  end
  ...
  defp remove_fix(head, pid, how) do
    case (r_head(head, :fixed)) do
      ...
      {timeStamp, counters} ->
        case (:lists.keysearch(pid, 1, counters)) do
          {:value, {^pid, counter}} when counter === 1 or
                                           how === :close
                                         ->
            :erlang.unlink(pid)
  ...
  defp check_growth(head) do
    ...
    cond do
      noThings > r_head(head, :next) ->
        _Ref = :erlang.send_after(200, self(),
  ...
  defp finfo(h) do
    case ((try do
      ...
      {h2, []} ->
        info = ((try do
                  [{:type, r_head(h2, :type)}, {:keypos, r_head(h2, :keypos)},
                                              {:size, r_head(h2, :no_objects)},
                                                  {:file_size,
                                                     file_size(r_head(h2, :fptr),
                                                                 r_head(h2, :filename))},
                                                      {:filename,
                                                         r_head(h2, :filename)}]
                catch
  ...
  defp do_open_file([tab, openArgs, verb], parent, server, _Ref) do
    case ((try do
      ...
      {:EXIT, _Reason} = error ->
        error
      bad ->
        :error_logger.format('** dets: Bug was found in open_file/2, arguments were~n** dets: ~tw and reply was ~tw.~n', [openArgs, bad])
  ...
  defp do_finit(head, init, format, noSlots) do
    ...
    {how, head1} = (case (format) do
                      ...
                      :term ->
                        ...
                        cond do
                          ...
                          true ->
                            :ok = :dets_utils.truncate(fd, fname, :bof)
                            {:ok, h} = :dets_v9.initiate_file(fd, tab, fname,
                                                                type, kp,
                                                                minSlots,
                                                                maxSlots, ram,
                                                                cacheSz, auto,
    ...
    case (how) do
      ...
      :general_init ->
        ...
        slotNumbers = {r_head(head1, :min_no_slots), :bulk_init,
                         maxSlots}
        {reply, sizeData} = do_sort(head1, slotNumbers, input,
                                      cntrs, fname)
        bulk = true
        case (reply) do
          {:ok, noDups, h1} ->
    ...
    case (find_all_keys(spec, keyPos, [])) do
      [] ->
        case ((try do
          ...
          {head1, []} ->
            newHead = (case (safe) do
                         :safe ->
                           do_safe_fixtable(head1, pidof(from), true)
                         :no_safe ->
                           head1
                       end)
  ...
  defp fopen2(fname, tab) do
    case (:file.read_file_info(fname)) do
      {:ok, _} ->
        ...
        do__ = (case (:dets_v9.check_file_header(fH, fd)) do
                  {:ok, head1} ->
        ...
        case (do__) do
          ...
          {:ok, head} ->
            open_final(head, fname, acc, ram, {3000, 14000}, tab,
                         false)
          {:error, reason} ->
  ...
  defp fopen_existing_file(tab, openArgs) do
    r_open_args(file: fname, type: type, keypos: kp, repair: rep,
        min_no_slots: minSlots, max_no_slots: maxSlots,
        ram_file: ram, delayed_write: cacheSz, auto_save: auto,
        access: acc, debug: debug) = openArgs
    {:ok, fd, fH} = read_file_header(fname, acc, ram)
    minF = :erlang.or(minSlots === :default,
                        minSlots === r_fileheader(fH, :min_no_slots))
    maxF = :erlang.or(maxSlots === :default,
                        maxSlots === r_fileheader(fH, :max_no_slots))
    wh = (case (:dets_v9.check_file_header(fH, fd)) do
            {:ok, head} when (rep === :force and
                                acc === :read_write and
            ...
            {:ok, head} ->
              {:final, head}
            {:error, :not_closed} when (rep === :force and
                                          acc === :read_write)
                                       ->
              m = ', repair forced.'
              {:repair, m}
            {:error, :not_closed} when (rep === true and
                                          acc === :read_write)
                                       ->
              m = ' not properly closed, repairing ...'
              {:repair, m}
    ...
    case (do__) do
      ...
      {:compact, sourceHead} ->
        ...
        {:ok, newSourceHead} = open_final(sourceHead, fname,
                                            :read, false, {3000, 14000}, tab,
                                            debug)
        case ((try do
  ...
  defp do_repair(fd, tab, fname, fH, minSlots, maxSlots,
            openArgs) do
  ...
  defp open_final(head, fname, acc, ram, cacheSz, tab, debug) do
    head1 = r_head(head, access: acc,  ram_file: ram, 
                      filename: fname,  name: tab, 
                      cache: :dets_utils.new_cache(cacheSz))
    init_disk_map(tab, debug)
    :dets_v9.cache_segps(r_head(head1, :fptr), fname,
                           r_head(head1, :next))
    check_growth(head1)
    {:ok, head1}
  end
  ...
  defp fopen_init_file(tab, openArgs) do
    ...
    {:ok, fd} = :dets_utils.open(fileSpec,
                                   open_args(:read_write, ram))
    init_disk_map(tab, debug)
    case ((try do
            :dets_v9.initiate_file(fd, tab, fname, type, kp,
                                     minSlots, maxSlots, ram, cacheSz, auto,
                                     true)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
  ...
  defp compact(sourceHead) do
    r_head(name: tab, filename: fname, fptr: sFd, type: type,
        keypos: kp, ram_file: ram, auto_save: auto) = sourceHead
    tmp = tempfile(fname)
    tblParms = :dets_v9.table_parameters(sourceHead)
    {:ok, fd} = :dets_utils.open(tmp,
    ...
    case (:dets_v9.compact_init(sourceHead, head,
                                  tblParms)) do
      {:ok, newHead} ->
        r = (case (fclose(newHead)) do
               :ok ->
                 :ok = :file.close(sFd)
  ...
  defp fsck(fd, tab, fname, fH, minSlotsArg, maxSlotsArg) do
    r_fileheader(min_no_slots: minSlotsFile,
    ...
    estNoSlots = :erlang.min(maxSlots,
    ...
    case (fsck_try(fd, tab, fH, fname, slotNumbers)) do
      {:try_again, betterNoSlots} ->
        ...
        case (fsck_try(fd, tab, fH, fname,
                         betterSlotNumbers)) do
          {:try_again, _} ->
            _ = :file.close(fd)
  ...
  defp fsck_try(fd, tab, fH, fname, slotNumbers) do
    ...
    openArgs = r_open_args(file: tmp, type: type, keypos: keyPos,
                   repair: false, min_no_slots: estNoSlots,
                   max_no_slots: maxSlots, ram_file: false,
                   delayed_write: {3000, 14000}, auto_save: :infinity,
                   access: :read_write, debug: false)
    case ((try do
            fopen3(tab, openArgs)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
  ...
  defp fsck_try_est(head, fd, fname, slotNumbers, fH) do
    ...
    {reply, sizeData} = do_sort(head, slotNumbers, input,
                                  cntrs, fname)
    bulk = false
    case (reply) do
      {:ok, noDups, h1} ->
  ...
  defp fsck_copy([{_LogSz, pos, bins, _NoObjects} | sizeData],
            head, _Bulk, noDups)
      when is_list(bins) do
    true = noDups === 0
    pWs = [{pos, bins} | :lists.map(fn {_, p, b, _} ->
                                         {p, b}
                                    end,
                                      sizeData)]
  ...
  defp fsck_copy1([szData | l], head, bulk, noDups) do
    ...
    case (cR) do
      {:ok, copied} when copied === expectedSize or
                           noObjects === 0
                         ->
  ...
  defp close_files(true, sizeData, _Head) do
    fun = fn {_Size, _Pos, {fileName, fd}, _No} ->
               _ = close_tmp(fd)
  ...
  defp update_cache(head, keysOrObjects, what) do
    {head1, lU, pwriteList} = update_cache(head,
                                             [{what, keysOrObjects}])
  ...
  defp update_cache(head, toAdd) do
    ...
    {newC, newIns, lookup, found} = cache_binary(head,
                                                   toAdd, c, size0, ins, false,
                                                   [])
    newCache = r_cache(cache, cache: newC,  csize: newSize, 
                          inserts: newIns)
    ...
    cond do
      ...
      size0 === 0 ->
        {r_head(head1, cache: r_cache(newCache, wrtime: time_now())),
           found, []}
      true ->
        {head1, found, []}
    end
  ...
  defp cache_binary(head, [{q, ks} | l], c, seq, ins, lu, f) do
    case (:dets_utils.cache_lookup(r_head(head, :type), ks, c,
                                     [])) do
      false ->
        cache_key_op(head, l, c, seq, ins, true, f, ks, q)
      found ->
        {:lookup, pid} = q
        cache_binary(head, l, c, seq, ins, lu,
                       [{pid, found} | f])
    end
  end
  ...
  defp cache_key_op(head, l, c, seq, ins, lu, f, [k | ks], q) do
    e = {k, {seq, q}}
    cache_key_op(head, l, [e | c], seq + 1, ins, lu, f, ks,
                   q)
  end
  ...
  defp cache_obj_op(head, l, c, seq, ins, lu, f, [o | os], kp, q) do
    ...
    cache_obj_op(head, l, [e | c], seq + 1, ins, lu, f, os,
                   kp, q)
  end
  ...
  defp init_scan(head, noObjs) do
    ...
    case (:dets_utils.find_next_allocated(freeLists, base,
                                            base)) do
      {from, to} ->
  ...
  defp check_safe_fixtable(head) do
    case (r_head(head, :fixed) === false and (:erlang.get(:verbose) === :yes or :dets_utils.debug_mode())) do
      true ->
        :error_logger.format('** dets: traversal of ~tp needs safe_fixtable~n', [r_head(head, :name)])
      false ->
        :ok
    end
  end
  ...
  defp scan(bin, h, from, to, l, ts, r, {c0, type} = c) do
    case (:dets_v9.scan_objs(h, bin, from, to, l, ts, r,
      ...
      :bad_object ->
        {:scan_error,
           :dets_utils.bad_object(:scan, {from, to, bin})}
    end
  end
  ...
  defp scan_read(h, from, _To, min, _L, ts, r, c) do
    ...
    case (:dets_utils.find_allocated(freeLists, from, max,
                                       r_head(h, :base))) do
      <<>> = bin0 ->
        {cont, _} = c
  ...
  defp make_timestamp(monTime, timeOffset) do
    erlangSystemTime = :erlang.convert_time_unit(monTime + timeOffset,
                                                   :native, :microsecond)
    megaSecs = div(erlangSystemTime, 1000000000000)
    secs = div(erlangSystemTime, 1000000) - megaSecs * 1000000
    microSecs = rem(erlangSystemTime, 1000000)
