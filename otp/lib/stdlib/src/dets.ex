defmodule :m_dets do
      ...
      [traverse, nObjs] ->
        ...
        formatFun = fn
          ...
          {:lookup, _KeyPos, [value], _NElements, elementFun} ->
            :io_lib.format(~c"~w:lookup(~w, ~w)", [:dets, tab, elementFun.(value)])
          ...
          {:lookup, _KeyPos, values, _NElements, elementFun} ->
            ...
            :io_lib.format(~c"lists:flatmap(fun(V) -> ~w:lookup(~w, V) end, ~w)", [
              :dets,
              tab,
              vals
            ])
  ...
  defp open_file_loop2(head, n) do
    receive do
      ...
      message ->
        :error_logger.format(~c"** dets: unexpected message(ignored): ~tw~n", [message])
    ...
    case op do
      ...
      :auto_save ->
        case r_head(head, :update_mode) do
          ...
          _Dirty when n === 0 ->
            :dets_utils.vformat(~c"** dets: Auto save of ~tp\n", [r_head(head, :name)])
  ...
  defp bug_found(name, op, bad, stacktrace, from) do
    case :dets_utils.debug_mode() do
      true ->
        :error_logger.format(
          ~c"** dets: Bug was found when accessing table ~tw,~n** dets: operation was ~tp and reply was ~tw.~n** dets: Stacktrace: ~tw~n",
          [name, op, bad, stacktrace]
        )

      false ->
        :error_logger.format(~c"** dets: Bug was found when accessing table ~tw~n", [name])
    end
  ...
  defp penalty(r_head(fixed: {_, [{pid, _}]}), [{pid, _Tag} = _From], _C) do
    :ok
  end
  ...
  defp do_open_file([fname, verbose], parent, server, ref) do
    case (try do
      ...
      bad ->
        :error_logger.format(~c"** dets: Bug was found in open_file/1, reply was ~tw.~n", [bad])
  ...
  defp do_open_file([tab, openArgs, verb], parent, server, _Ref) do
    case (try do
      ...
      bad ->
        :error_logger.format(
          ~c"** dets: Bug was found in open_file/2, arguments were~n** dets: ~tw and reply was ~tw.~n",
          [openArgs, bad]
        )
  ...
  defp fopen2(fname, tab) do
    case :file.read_file_info(fname) do
      {:ok, _} ->
          ...
          case :dets_v9.check_file_header(fH, fd) do
            {:ok, head1} ->
              ...
              try do
                ...
                _ ->
                  {:repair, ~c" has bad free lists, repairing ..."}
              end

            {:error, :not_closed} ->
              m = ~c" not properly closed, repairing ..."
              {:repair, m}
        ...
        case do__ do
          {:repair, mess} ->
            :io.format(:user, ~c"dets: file ~tp~s~n", [fname, mess])
  ...
  defp fopen_existing_file(tab, openArgs) do
    ...
    case do__ do
      ...
      {:compact, sourceHead} ->
        :io.format(:user, ~c"dets: file ~tp is now compacted ...~n", [fname])
        ...
        case (try do
          ...
          _Err ->
            ...
            :io.format(:user, ~c"dets: compaction of file ~tp failed, now repairing ...~n", [
              fname
            ])
  ...
  defp check_safe_fixtable(head) do
    case r_head(head, :fixed) === false and
      ...
      true ->
        :error_logger.format(~c"** dets: traversal of ~tp needs safe_fixtable~n", [
          r_head(head, :name)
        ])
  ...
  defp err(error) do
    case :erlang.get(:verbose) do
      :yes ->
        :error_logger.format(~c"** dets: failed with ~tw~n", [error])
        error
  ...
  defp v_free_list(head) do
    :io.format(~c"FREE LIST ...... \n", [])
    :io.format(~c"~p~n", [:dets_utils.all_free(head)])
    :io.format(~c"END OF FREE LIST \n", [])
  end
end
