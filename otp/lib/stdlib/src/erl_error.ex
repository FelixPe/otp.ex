defmodule :m_erl_error do
  ...
  def format_exception(i, class, reason, stackTrace, stackFun, formatFun, encoding, charsLimit)
    ...
    st = format_stacktrace1(s, trace, formatFun, stackFun, encoding, stLimit, reason, errorMap)
  ...
  def format_call(i, forMForFun, as, formatFun, enc)
      when is_integer(i) and i >= 1 and is_list(as) and
             is_function(formatFun, 2) do
    fF = wrap_format_fun_2(formatFun)
    format_call(~c"", n_spaces(i - 1), forMForFun, as, fF, enc)
  end
  ...
  defp explain_reason({:badkey, v}, :error = cl, [], pF, s, _Enc, cL) do
    format_value(v, "bad key: ", cl, pF, s, cL)
  end

  defp explain_reason({:badmap, v}, :error = cl, [], pF, s, _Enc, cL) do
  ...
  defp explain_reason({:else_clause, v}, :error = cl, [], pF, s, _Enc, cL) do
  ...
  defp format_stacktrace1(sep, stack, pF, sF, enc, cL, reason) do
    format_stacktrace1(sep, stack, pF, sF, enc, cL, reason, get_extended_error(reason, stack))
  end

  defp format_stacktrace1(s0, stack0, pF, sF, enc, cL, reason, errorMap) do
    stack1 =
      :lists.dropwhile(
        fn {m, f, a, _} ->
          sF.(m, f, a)
  ...
  defp format_stacktrace2(_S, _Stack, _N, _PF, _Enc, _CL = 0, _Reason, _ErrorMap) do
    []
  end

  defp format_stacktrace2(s, [{m, f, a, l} | fs], n, pF, enc, cL, reason, errorMap)
    ...
    cs =
      :io_lib.fwrite(
        "~s~s ~ts ~ts~ts",
        [sep(n, s), origin(n, m, f, a), mfa_to_string(m, f, a, enc), location(l), formattedError]
      )
  ...
  defp format_stacktrace2(s, [{m, f, as, _Info} | fs], n, pF, enc, cL, reason, errorMap)
       when is_list(as) do
    a = length(as)
    calledAs = [s, "   called as "]
    c = format_call(~c"", calledAs, {m, f}, as, pF, enc, cL)
    formattedError = call_format_error(errorMap, as, sep(n, s))

    cs =
      :io_lib.fwrite(
        "~s~s ~ts\n~s~ts~ts",
        [sep(n, s), origin(n, m, f, a), mfa_to_string(m, f, a, enc), calledAs, c, formattedError]
      )
  ...
  defp format_stacktrace2(_S, [], _N, _PF, _Enc, _CL, _Reason, _ErrorMap) do
    ~c""
  end
  ...
  defp get_extended_error(
         reason,
         [{m, _F, _As, info} | _] = stackTrace
       ) do
    case :lists.keyfind(:error_info, 1, info) do
      {:error_info, errorInfoMap} when is_map(errorInfoMap) ->
        ...
        try do
          apply(formatModule, formatFunction, [reason, stackTrace])
        catch
          :error, _ ->
            %{}
        end
  ...
  defp format_arg_errors(argNum, [_ | as], errorMap) do
    case errorMap do
      %{^argNum => err} ->
        [firstLine | lines] = :string.lexemes(err, ~c"\r\n")
