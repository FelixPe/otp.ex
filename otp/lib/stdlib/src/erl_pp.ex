defmodule :m_erl_pp do
  ...
  defp lform({:attribute, anno, name, arg}, opts) do
  ...
  defp lform({:eof, _Location}, _Opts) do
    ?\n
  end
  ...
  defp ltype({:ann_type, _Anno, [v, t]}, prec) do
  ...
  defp ltype({:paren_type, _Anno, [t]}, p) do
    ltype(t, p)
  end
  ...
  defp ltype({:type, _Anno, :list, [t]}, _) do
    {:seq, ?[, ?], ?,, [ltype(t)]}
  end
  ...
  defp ltype({:type, anno, :map, :any}, _) do
    simple_type({:atom, anno, :map}, [])
  end

  defp ltype({:type, _Anno, :map, pairs}, prec) do
    {p, _R} = type_preop_prec(:"#")
    e = map_type(pairs)
    maybe_paren(p, prec, e)
  end

  defp ltype({:type, anno, :tuple, :any}, _) do
    simple_type({:atom, anno, :tuple}, [])
  end

  defp ltype({:type, _Anno, :tuple, ts}, _) do
  ...
  defp ltype({:type, _Anno, :binary, [i1, i2]}, _) do
    binary_type(i1, i2)
  end
  ...
  defp ltype({:type, anno, t, ts}, _) do
    simple_type({:atom, anno, t}, ts)
  end

  defp ltype({:user_type, anno, t, ts}, _) do
    simple_type({:atom, anno, t}, ts)
  end

  defp ltype({:remote_type, anno, [m, f, ts]}, _) do
  ...
  defp word(:maybe, wT) do
    :erlang.element(20, wT)
  end

  defp word(:else, wT) do
    :erlang.element(21, wT)
  end

  defp word(:"\#{", wT) do
    :erlang.element(22, wT)
  end
  ...
  defp map_vars(f, forms) when is_list(forms) do
    for form <- forms do
      map_vars(f, form)
    end
  end

  defp map_vars(f, v = {:var, _, _}) do
    f.(v)
  end
