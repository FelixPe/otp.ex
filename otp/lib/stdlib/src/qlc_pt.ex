defmodule :m_qlc_pt do
  ...
  defp transform_expression(lC, bs0, withLintErrors) do
    ...
    f =
      {:function, anno, :bar, ar,
       [
         {:clause, anno, as, [],
          [{:call, anno, {:remote, anno, {:atom, anno, :qlc}, {:atom, anno, :q}}, [lC]}]}
       ]}

    forms0 = [{:attribute, anno, :file, {~c"foo", anno}}, {:attribute, anno, :module, :foo}, f]
  ...
  defp compile_messages(forms, formsNoShadows, options, state) do
    bGenF = fn
      _QId, {:b_generate, anno, _P, _LE} = bGen, gA, a ->
  ...
  defp intro_variables(formsNoShadows, state) do
    ...
    fun = fn
      qId, {t, _A, p0, _E0} = q, {gVs, qIds}, foo
  ...
  defp used_genvar_check(formsNoShadows, state) do
    ...
    f = fn
      qId, {t, annoQ, _P, lE} = q, {qsIVs0, exprs0}, iVsSoFar0
      when t === :b_generate or t === :generate ->
        f = fn var ->
    ...
    f2 = fn id, {:lc, _A, e, qs}, {introVs0, xWarn0} ->
      ...
      funW =
        {:fun, anno,
         {:clauses,
          [
            {:clause, anno, asW, [],
             [{:match, anno, {:var, anno, fun}, funC}, {:call, anno, {:var, anno, fun}, as0}]}
          ]}}
      ...
      lCTuple =
        case qlc_kind(origE, qs, state) do
          :qlc ->
            {:tuple, anno, [{:atom, anno, :qlc_v1}, funW, qCode, qdata, opt]}
  ...
  defp opt_info(
    ...
    szCls =
      ...
      end ++ [{:clause, anno, [{:var, anno, :_}], [], [{:atom, anno, :undefined}]}]
      ...
      :lists.append(
        for {eqType, templateColumns} <- templateInfo do
          for {col, templCols} <- templateColumns do
            {:clause, anno, [abstr(col, anno), eqType], [], [abstr(templCols, anno)]}
          end
        end
      )
    ...
    tCls =
      :lists.sort(tCls0) ++
        [{:clause, anno, [{:var, anno, :_}, {:var, anno, :_}], [], [{nil, anno}]}]
    ...
    nCls =
      for {idNo, n} <- nSortedCols0, n > 0 do
        {:clause, anno, [{:integer, anno, idNo}], [], [{:integer, anno, n - 1}]}
      end ++ [{:clause, anno, [{:var, anno, :_}], [], [{:integer, anno, 0}]}]

    n = [{:n_leading_constant_columns, {:fun, anno, {:clauses, nCls}}}]

    constCls =
      for {idNo, cols} <- constColsFamily do
        {:clause, anno, [{:integer, anno, idNo}], [], [abstr(cols, anno)]}
      end ++ [{:clause, anno, [{:var, anno, :_}], [], [{nil, anno}]}]

    cC = [{:constant_columns, {:fun, anno, {:clauses, constCls}}}]

    mSCls =
      for {g, mS, fs} <- mSQs do
        {:clause, anno, [{:integer, anno, g}], [], [{:tuple, anno, [mS, abstr(fs, anno)]}]}
      end ++ [{:clause, anno, [{:var, anno, :_}], [], [{:atom, anno, :undefined}]}]
  ...
  defp opt_constants(anno, columnConstants) do
    ...
    for idNo <- ns do
      {:clause, anno, [{:integer, anno, idNo}], [], [column_fun(columnConstants, idNo, anno)]}
    end ++ [{:clause, anno, [{:var, anno, :_}], [], [{:atom, anno, :no_column_fun}]}]
  end
  ...
  defp join_quals(joinInfo, qCs, anno, lcNo, extraConstants, allVars) do
    {lastGoI, lastSI} =
      :lists.foldl(
        fn
  ...
  defp join_handle(aP, anno, [f, h, o, c], constants) do
    case {aP, constants} do
      ...
      _ ->
        ...
        cC1 =
          {:clause, anno, [aP], g, [{:cons, anno, o, closure({:call, anno, f, [f, c]}, anno)}]}

        cC2 = {:clause, anno, [{:var, anno, :_}], [], [{:call, anno, f, [f, c]}]}
        case__ = {:case, anno, o, [cC1, cC2]}

        cls = [
          {:clause, anno, [{:var, anno, :_}, {nil, anno}], [], [{nil, anno}]},
          {:clause, anno, [f, {:cons, anno, o, c}], [], [case__]},
          {:clause, anno, [f, c], [[{:call, anno, {:atom, anno, :is_function}, [c]}]],
  ...
  defp simple_template(
         {:call, anno, {:remote, _, {:atom, _, :erlang}, {:atom, _, :element}} = call,
          [{:integer, _, i} = a1, a2]}
       )
       when i > 0 do
  ...
  defp filter1(
         {:call, anno, {:remote, _, {:atom, _, :erlang}, {:atom, _, :is_record}}, [t, r]},
         fs,
         fS
       ) do
    filter1(
  ...
  defp filter1(
         {:call, anno, {:remote, anno1, {:atom, _, :erlang} = m, {:atom, anno2, :is_record}},
          [t, r, _Sz]},
         fs,
         fS
       ) do
    filter1({:call, anno, {:remote, anno1, m, {:atom, anno2, :is_record}}, [t, r]}, fs, fS)
  end
  ...
  defp qdata(
    ...
    {:cons, anno,
     {:tuple, anno,
      [{:integer, anno, qIdNo}, {:integer, anno, goI}, {:integer, anno, sI}, {:atom, anno, :fil}]},
     qdata(qCs, anno)}
  end
  ...
  defp qcode(e, qCs, source, anno, state) do
    cL =
      for {_, c} <-
        ...
        {:bin, anno,
         [{:bin_element, anno, {:string, anno, :erlang.binary_to_list(bin)}, :default, :default}]}
      end
  ...
  defp clauses([{qId, {qIVs, {qualData, goI, s}}} | qCs], rL, fun, go, nGV, e, iVs, st) do
    ...
    cs =
      case qualData do
        {:gen, p, _LE, gV} ->
          generator(s, qIVs, p, gV, nGV, e, iVs, rL, fun, go, goI, anno, st)
  ...
  defp final(rL, iVs, anno, state) do
    ...
    rev =
      {:call, anno, {:remote, anno, {:atom, anno, :lists}, {:atom, anno, :reverse}},
       [{:var, anno, rL}]}
  ...
  defp template(e, rL, fun, go, aT, anno, iVs, state) do
    ...
    grd = [is_list_c(rL, anno)]
    cL = {:clause, anno, as, [grd], [{:call, anno, {:var, anno, fun}, nAs}]}

    f =
      case split_args([next | as0], anno, state) do
        {argsL, argsT} ->
          call = {:call, anno, {:var, anno, fun}, argsL ++ [{:var, anno, aT}]}
        ...
        fNAs ->
          {:fun, anno,
           {:clauses, [{:clause, anno, [], [], [{:call, anno, {:var, anno, fun}, fNAs}]}]}}
      end
  ...
  defp generator(s, qIVs, p, gV, nGV, e, iVs, rL, fun, go, goI, anno, state) do
  ...
  defp generator_list(p, gV, nGV, as, asM, asC, asD, fun, anno, state) do
    as1 =
      pack_args(replace([{:var, anno, gV}], as, {:cons, anno, p, {:var, anno, nGV}}), anno, state)

    as2 =
      pack_args(
        replace([{:var, anno, gV}], as, {:cons, anno, {:var, anno, :_}, {:var, anno, nGV}}),
        anno,
        state
      )

    as3 = pack_args(replace([{:var, anno, gV}], as, {nil, anno}), anno, state)
  ...
  defp generator_cont(p, gV, nGV, e, as0, asM, asC, asD, fun, anno, state) do
    as = pack_args(as0, anno, state)
    cF1 = {:cons, anno, p, {:var, anno, nGV}}
    cF2 = {:cons, anno, {:var, anno, :_}, {:var, anno, nGV}}
    cF3 = {nil, anno}
    cF4 = {:var, anno, e}
    cM = {:clause, anno, [cF1], [], [{:call, anno, {:var, anno, fun}, asM}]}
    cC = {:clause, anno, [cF2], [], [{:call, anno, {:var, anno, fun}, asC}]}
    cD = {:clause, anno, [cF3], [], [{:call, anno, {:var, anno, fun}, asD}]}
  ...
  defp filter(e, anno, qIVs, s, rL, fun, go, goI, iVs, state) do
    ...
    body =
      case is_guard_test(e, state) do
        true ->
          cT = {:clause, anno, [], [[e]], [{:call, anno, {:var, anno, fun}, nAsT}]}

          cF =
            {:clause, anno, [], [[{:atom, anno, true}]], [{:call, anno, {:var, anno, fun}, nAsF}]}

          [{:if, anno, [cT, cF]}]

        false ->
          cT =
            {:clause, anno, [{:atom, anno, true}], [], [{:call, anno, {:var, anno, fun}, nAsT}]}

          cF =
            {:clause, anno, [{:atom, anno, false}], [], [{:call, anno, {:var, anno, fun}, nAsF}]}
  ...
  defp split_args(args, anno, state)
    ...
    {:lists.sublist(args, r_state(state, :maxargs) - 1),
     {:tuple, anno, :lists.nthtail(r_state(state, :maxargs) - 1, args)}}
  end
  ...
  defp next(go, goI, anno) do
    {:call, anno, {:atom, anno, :element}, [{:integer, anno, goI}, {:var, anno, go}]}
  end
  ...
  defp qlcmf(
         {:call, anno1, {:remote, anno2, {:atom, anno3, :qlc}, {:atom, anno4, :q}}, [lC0 | os0]},
         f,
         imp,
         a0,
         no0
    ...
    {{:call, anno1, {:remote, anno2, {:atom, anno3, :qlc}, {:atom, anno4, :q}}, [t | os]}, a,
     no + 1}
  end

  defp qlcmf({:call, anno, {:atom, anno2, :q}, [lC0 | os0]}, f, imp = true, a0, no0)
       when length(os0) < 2 do
    {os, a1, no1} = qlcmf(os0, f, imp, a0, no0)
    {lC, a2, no} = qlcmf(lC0, f, imp, a1, no1)
  ...
  defp qlcmf({:attribute, _Anno, :file, {file, _Line}} = attr, _F, _Imp, a, no) do
  ...
  defp nos({:fun, anno, {:clauses, cs}}, s) do
    nCs =
      for {:clause, cAnno, h0, g0, b0} <- cs do
  ...
  defp nos_pattern({:var, anno, v}, {lI, vs0, uV, a, sg, state}, pVs0)
       when v !== :_ do
    {name, vs, pVs} =
