defmodule :m_rand do
  ...
  defp uniform_range(range, %{next: next, bits: bits} = algHandler, r, v) do
  ...
  def uniform_s({%{bits: bits, next: next} = algHandler, r0}) do
    {v, r1} = next.(r0)
    {(v >>> (bits - 53)) * 1.1102230246251565e-16, {algHandler, r1}}
  end
  ...
  defp uniform_real_s(algHandler, next, m0, bitNo, r1, v1, bits) do
    cond do
      ...
      m0 == 0 ->
        ...
        cond do
          1 <<< 55 <= m1 ->
            {(m1 >>> 3) * :math.pow(2.0, bitNo - 53), {algHandler, r1}}

          1 <<< 54 <= m1 ->
            {(m1 >>> 2) * :math.pow(2.0, bitNo - 54), {algHandler, r1}}

          1 <<< 53 <= m1 ->
            {(m1 >>> 1) * :math.pow(2.0, bitNo - 55), {algHandler, r1}}
  ...
  defp uniform_real_s(%{bits: bits} = algHandler, next, m0, bitNo, r0) do
    {v1, r1} = next.(r0)
  ...
  defp uniform_real_s(%{max: _} = algHandler, next, m0, bitNo, r0) do
    {v1, r1} = next.(r0)
    uniform_real_s(algHandler, next, m0, bitNo, r1, v1 &&& 1 <<< (56 - 1), 56)
  end
  ...
  defp bytes_r(n0, algHandler, next, r0, bytes0, goodBytes, goodBits, shift)
    ...
    bytes1 =
      <<bytes0::binary, v1 >>> shift::size(goodBits), v2 >>> shift::size(goodBits),
        v3 >>> shift::size(goodBits), v4 >>> shift::size(goodBits)>>
  ...
  defp bytes_r(n0, algHandler, next, r0, bytes0, goodBytes, goodBits, shift)
    ...
    {v, r1} = next.(r0)
    bytes1 = <<bytes0::binary, v >>> shift::size(goodBits)>>
    n1 = n0 - goodBytes
    bytes_r(n1, algHandler, next, r1, bytes1, goodBytes, goodBits, shift)
  end

  defp bytes_r(n, algHandler, next, r0, bytes, _GoodBytes, goodBits, _Shift) do
    {v, r1} = next.(r0)
    bits = n <<< 3
    shift = goodBits - bits
    {<<bytes::binary, v >>> shift::size(bits)>>, {algHandler, r1}}
  end
  ...
  defp mk_alg(:exsplus) do
    {%{type: :exsplus, max: 1 <<< (58 - 1), next: &exsp_next/1, jump: &exsplus_jump/1},
     &exsplus_seed/1}
  end
  ...
  defp mk_alg(:dummy = name) do
    {%{
       ...
       bits: 58,
       next: &dummy_next/1,
       uniform: &dummy_uniform/1,
       uniform_n: &dummy_uniform/2
     }, &dummy_seed/1}
  end
  ...
  defp exsp_uniform({algHandler, r0}) do
    {i, r1} = exsp_next(r0)
    {(i >>> (58 - 53)) * 1.1102230246251565e-16, {algHandler, r1}}
  end
  ...
  defp exro928ss_uniform({algHandler, sR}) do
    ...
    {(v >>> (58 - 53)) * 1.1102230246251565e-16, {algHandler, newSR}}
  end
  ...
  defp exrop_uniform({algHandler, r}) do
    {v, r1} = exrop_next(r)
    {(v >>> (58 - 53)) * 1.1102230246251565e-16, {algHandler, r1}}
  end
  ...
  def mwc59(cX)
      when is_integer(cX) and 1 <= cX and
             cX < 133_850_370 <<< (32 - 1) do
    c = cX >>> 32
    x = cX &&& 1 <<< (32 - 1)
    133_850_370 * x + c
  end

  def mwc59_value32(cX1)
      when is_integer(cX1) and 1 <= cX1 and
             cX1 < 133_850_370 <<< (32 - 1) do
    cX = cX1 &&& 1 <<< (32 - 1)
    cX ^^^ (cX &&& 1 <<< (32 - 8 - 1) <<< 8)
  end
  ...
  defp hash58(x) do
    x0 = x &&& 1 <<< (58 - 1)
    x1 = (x0 ^^^ (x0 >>> 29)) * 239_165_597_161_983_181 &&& 1 <<< (58 - 1)
    x2 = (x1 ^^^ (x1 >>> 29)) * 58_188_346_220_211_283 &&& 1 <<< (58 - 1)
    x2 ^^^ (x2 >>> 29)
  end
  ...
  def format_jumpconst58(string) do
    ...
    {:match, matches} = :re.run(string, ~c"0x([a-zA-Z0-9]+)", reOpts)
  ...
  defp get_52({%{bits: bits, next: next} = algHandler, s0}) do
    {int, s1} = next.(s0)
    {1 <<< (bits - 51 - 1) &&& int, int >>> (bits - 51), {algHandler, s1}}
  end

  defp get_52({%{next: next} = algHandler, s0}) do
    {int, s1} = next.(s0)
    {1 <<< 51 &&& int, int &&& 1 <<< (51 - 1), {algHandler, s1}}
  end
