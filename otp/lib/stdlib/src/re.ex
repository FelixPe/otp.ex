defmodule :m_re do
  ...
  def split(subject, rE, options) do
    try do
      {newOpt, convert, limit, strip,
         group} = process_split_params(options, :iodata, - 1,
                                         false, false)
      ...
      case (compile_split(rE, newOpt)) do
        ...
        {preCompiled, numSub, runOpt} ->
          case (:re.run(flatSubject, preCompiled,
                          runOpt ++ [:global])) do
            :nomatch ->
              case (group) do
                true ->
                  convert_any_split_result([[flatSubject]], convert,
                                             unicode, true)
                false ->
                  convert_any_split_result([flatSubject], convert,
                                             unicode, false)
            ...
            {:match, matches} ->
              res = do_split(flatSubject, 0, matches, numSub, limit,
                               group)
              stripped = (case (strip) do
                            true ->
                              backstrip_empty(res, group)
                            false ->
                              res
                          end)
              convert_any_split_result(stripped, convert, unicode,
                                         group)
          end
      end
    catch
  ...
  defp do_split(subj, offset, [[{mainI, mainL} | sub] | t],
            numSub, limit, group) do
    ...
    case ({keptLen, empty_sub(sub), mainL}) do
      ...
      _ ->
        <<_ :: size(offset) - binary,
            keep :: size(keptLen) - binary, _ :: binary>> = subj
        eSub = extend_subpatterns(sub, numSub)
        tail = do_split(subj, newOffset, t, numSub, limit - 1,
                          group)
        case (group) do
          false ->
  ...
  defp dig_subpatterns(subj, [{i, l} | t], acc) do
    <<_ :: size(i) - binary, part :: size(l) - binary,
        _ :: binary>> = subj
  ...
  def replace(subject, rE, replacement, options) do
    try do
      {newOpt, convert} = process_repl_params(options,
                                                :iodata)
      unicode = check_for_unicode(rE, options)
      flatSubject = to_binary(subject, unicode)
      replacement1 = normalize_replacement(replacement,
                                             unicode)
      ioList = do_replace(flatSubject, subject, rE,
                            replacement1, newOpt)
      case (convert) do
        :iodata ->
          ioList
  ...
  defp normalize_replacement(replacement, _Unicode)
      when is_function(replacement, 2) do
    replacement
  end

  defp normalize_replacement(replacement, unicode) do
    to_binary(replacement, unicode)
  end

  defp do_replace(flatSubject, subject, rE, replacement,
            options) do
  ...
  defp process_repl_params(_, _) do
    throw(:badopt)
  end
  ...
  defp process_split_params([h | t], c, l, s, g) do
    {nT, nC, nL, nS, nG} = process_split_params(t, c, l, s,
                                                  g)
    {[h | nT], nC, nL, nS, nG}
  end

  defp process_split_params(_, _, _, _, _) do
    throw(:badopt)
  end

  defp apply_mlist(subject, replacement, mlist) do
    do_mlist(subject, subject, 0, precomp_repl(replacement),
               mlist)
  end
  ...
  defp precomp_repl(<<?\\, x, rest :: binary>>) when x < ?1 or
                                             x > ?9 do
  ...
  defp precomp_repl(repl) when is_function(repl) do
    repl
  end

  defp pick_int(<<x, r :: binary>>) when (x >= ?0 and
                                      x <= ?9) do
  ...
  defp do_mlist(whole, subject, pos, repl,
            [[{mPos, count} | sub] | tail])
      when mPos > pos do
    eatLength = mPos - pos
    <<untouched :: size(eatLength) - binary,
        rest :: binary>> = subject
  ...
  defp do_mlist(whole, subject, pos, repl,
      ...
      when mPos === pos do
    eatLength = count
    <<_ :: size(eatLength) - binary,
        rest :: binary>> = subject
    newData = do_replace(whole, repl, [{mPos, count} | sub])
    [newData | do_mlist(whole, rest, pos + eatLength, repl,
                          tail)]
  end

  defp do_replace(subject, repl, subExprs0)
      when is_function(repl) do
    all = :binary.part(subject, hd(subExprs0))
    subExprs1 = (for {pos, len} <- tl(subExprs0) do
                   cond do
                     (pos >= 0 and len > 0) ->
            ...
            true ->
              ...
              cond do
                sPos < 0 ->
                  <<>>
                true ->
                  <<_ :: size(sPos) - binary, res :: size(sLen) - binary,
                      _ :: binary>> = subject
                  res
              end
          end
  ...
  defp process_parameters([], initialOffset, selectReturn, convertReturn,
            _, _) do
    {[], initialOffset, selectReturn, convertReturn}
  end

  defp process_parameters([{:offset, n} | t], _Init0, select0, return0,
            cC, rE) do
    process_parameters(t, n, select0, return0, cC, rE)
  end

  defp process_parameters([:global | t], init0, select0, return0, cC,
            rE) do
    process_parameters(t, init0, select0, return0, cC, rE)
  end

  defp process_parameters([{:capture, values, type} | t], init0, select0,
            _Return0, cC, rE) do
    process_parameters([{:capture, values} | t], init0,
                         select0, type, cC, rE)
  end

  defp process_parameters([{:capture, values} | t], init0, select0,
            return0, cC, rE) do
    {newTail, init1, select1,
       return1} = process_parameters(t, init0, select0,
                                       return0, cC, rE)
    case (select1) do
      false ->
        case (values) do
          ...
          :all_names ->
            case (:re.inspect(rE, :namelist)) do
              ...
              {:namelist, list} ->
                {[{:capture, [0 | list]} | newTail], init1, :stripfirst,
                   return0}
            end
          :first ->
            {[{:capture, :first} | newTail], init1, :all, return0}
          :all_but_first ->
            {[{:capture, :all} | newTail], init1, :stripfirst,
               return0}
          :none ->
            {[{:capture, :first} | newTail], init1, :none, return0}
          [] ->
  ...
  defp process_parameters([h | t], init0, select0, return0, true, rE) do
    case (copt(h)) do
      ...
      false ->
        {newT, init, select, return} = process_parameters(t,
                                                            init0, select0,
                                                            return0, true, rE)
  ...
  defp postprocess({:match, m}, any, :binary, flat, uni) do
    binarify(postprocess({:match, m}, any, :index, flat,
                           uni),
               flat)
  end

  defp postprocess({:match, m}, any, :list, flat, uni) do
    listify(postprocess({:match, m}, any, :index, flat,
                          uni),
              flat, uni)
  end
  ...
  defp urun2(subject0, rE0, options0) do
    ...
    subject = :unicode.characters_to_binary(subject0,
                                              :unicode)
    rE = (case (rE0) do
  ...
  defp do_grun(flatSubject, subject, unicode, cRLF, rE,
            {options0, needClean}) do
    {strippedOptions, initialOffset, selectReturn,
       convertReturn} = (case ((try do
                                 process_parameters(options0, 0, false, :index,
                                                      needClean, rE)
                               catch
                                 :error, e -> {:EXIT, {e, __STACKTRACE__}}
                                 :exit, e -> {:EXIT, e}
    ...
    try do
      postprocess(loopexec(flatSubject, rE, initialOffset,
                             byte_size(flatSubject), unicode, cRLF,
  ...
  defp loopexec(subject, rE, x, y, unicode, cRLF, options,
            first) do
    case (:re.internal_run(subject, rE,
                             [{:offset, x}] ++ options, first)) do
      {:error, err} ->
        throw({:error, err})
      :nomatch ->
        {:match, []}
      {:match, [{a, b} | more]} ->
        {:match, rest} = (case (b > 0) do
                            true ->
                              loopexec(subject, rE, a + b, y, unicode, cRLF,
                                         options, false)
                            false ->
                              {:match, m} = (case (:re.internal_run(subject, rE,
                                                                      [{:offset,
                                                                          x},
                                                                           :notempty_atstart,
                                                                               :anchored] ++ options,
  ...
  defp badarg_with_cause(args, cause) do
    :erlang.error(:badarg, args,
                    [{:error_info,
                        %{module: :erl_stdlib_errors, cause: cause}}])
  end
