defmodule :m_filelib do
  ...
  defp do_wildcard_3(base, [[:double_star] | rest], result, mod) do
    do_double_star(~c".", [base], rest, result, mod, true)
  end

  defp do_wildcard_3(base, [~c".." | rest], result, mod) do
    case do_is_dir(base, mod) do
      true ->
        matches = [:filename.join(base, ~c"..")]
  ...
  defp compile_wildcard_3([], result) do
    case :lists.reverse(result) do
      ...
      [{:cwd, root} | compiled] ->
        {[root | compiled], length(:filename.join(root, ~c"x")) - 1}
    end
  end

  defp compile_join({:cwd, ~c"."}, file) do
    {:root, 0, file}
  end
  ...
  defp do_exists([p, ~c".." | ps], mod, acc) do
    path =
      case acc do
        [] ->
          p
  ...
  defp default_search_rules() do
    [
      {~c".beam", ~c".erl", erl_source_search_rules()},
      {~c".erl", ~c".yrl", []},
      {~c"", ~c".src", erl_source_search_rules()},
      {~c".so", ~c".c", c_source_search_rules()},
      {~c".o", ~c".c", c_source_search_rules()},
      {~c"", ~c".c", c_source_search_rules()},
      {~c"", ~c".in", basic_source_search_rules()},
      {~c".beam", ~c".asn1", asn1_source_search_rules()},
      {~c"", ~c""}
  ...
  defp erl_source_search_rules() do
    [
      {~c"ebin", ~c"src"},
      {~c"ebin", ~c"esrc"},
      {~c"ebin", :filename.join(~c"src", ~c"*")},
      {~c"ebin", :filename.join(~c"esrc", ~c"*")}
    ]
  end
  ...
  def safe_relative_path(path, ~c"") do
    safe_relative_path(path, ~c".")
  end

  def safe_relative_path(path, cwd) do
    srp_path(:filename.split(path), cwd, :sets.new([{:version, 2}]), [])
  end
  ...
  defp srp_path([~c"." | segs], cwd, seen, acc) do
  ...
  defp srp_path([~c".." | _Segs], _Cwd, _Seen, []) do
    :unsafe
  end

  defp srp_path([~c".." | segs], cwd, seen, [_ | _] = acc) do
  ...
  defp srp_link(path, linkPath, segs, cwd, seen, acc) do
    ...
    case :sets.is_element(fullLinkPath, seen) do
      ...
      false ->
        srp_path(
          :filename.split(linkPath) ++ [:clear | segs],
          cwd,
