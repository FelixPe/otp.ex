defmodule :m_edlin_expand do
  ...
  Record.defrecord(:r_docs_v1_entry, :docs_v1_entry, kind_name_arity: :undefined,
                                         ...
                                         metadata: :undefined)
  Record.defrecord(:r_shell_state, :shell_state, bindings: [],
                                       records: [], functions: [])
  def expand(bef0) do
  ...
  def expand(bef0, opts,
           r_shell_state(bindings: bs, records: rT, functions: fT)) do
    legacyOutput = :proplists.get_value(:legacy_output,
                                          opts, false)
    {_Bef1, word} = over_word(bef0)
    {res, expansion,
       matches} = (case (:edlin_context.get_context(bef0)) do
                     {:string} ->
                       expand_string(bef0)
                     {:binding} ->
                     ...
                     {:fun_, mod, fun} ->
                       arities = (for a <- get_arities(mod, fun) do
                     ...
                     {:function, mod, fun, args, unfinished, nesting} ->
                       ...
                       funExpansion = expand_function_type(mod2, fun, args,
                                                             unfinished,
                                                             nesting, fT)
                       case (word) do
                         [] ->
                         ...
                         _ ->
                           ...
                           functions = (case (args !== [] and :lists.last(args)) do
                                          {:atom, maybeMod} ->
                                            expand_function_name(maybeMod, word,
                                                                   '', fT)
                                          _ ->
                                            {:no, [], []}
                                        end)
                     ...
                     {:map_or_record} ->
                       ...
                       case (bs) do
                         ...
                         _ ->
                           case (:proplists.get_value(:erlang.list_to_atom(var),
                                                        bs)) do
                             :undefined ->
                               expand_record(word, rT)
                             map when is_map(map) ->
                               {:yes, '{', []}
                             recordTuple when (is_tuple(recordTuple) and
                                                 tuple_size(recordTuple) > 0)
                                              ->
                               atom = :erlang.element(1, recordTuple)
                               case (is_atom(atom) and :lists.keysearch(atom, 1,
                                                                          rT)) do
                                 {:value, {^atom, _}} ->
                                   match(word, [atom], '{')
                                 _ ->
                     ...
                     {:record, record, fields, fieldToComplete, args,
                        unfinished, nestings} ->
                       recordExpansion = expand_record_fields(fieldToComplete,
                                                                unfinished,
                                                                record, fields,
                                                                rT, args,
                                                                nestings, fT)
    ...
    matches1 = (case ({res, number_matches(matches)}) do
                  {:yes, 1} ->
                    []
                  _ ->
                    matches
  ...
  defp expand_map(word, bs, binding, keys) do
    case (:proplists.get_value(:erlang.list_to_atom(binding),
                                 bs)) do
      map when is_map(map) ->
        k1 = :sets.from_list(:maps.keys(map))
        k2 = :sets.subtract(k1,
                              :sets.from_list(for k <- keys do
  ...
  defp expand_binding(prefix, bindings) do
    alts = (for {k, _} <- bindings do
              strip_quotes(k)
            end)
    case (match(prefix, alts, '')) do
      {_Res, _Expansion, []} = m ->
        m
      {res, expansion, matches} ->
        {res, expansion,
           [%{title: 'bindings', elems: matches,
                options: [:highlight_all]}]}
    end
  end

  defp expand_record(prefix, rT) do
    alts = (for {name, _} <- rT do
              name
            end)
    case (match(prefix, alts, '{')) do
      {_Res, _Expansion, []} = m ->
      ...
      {res, expansion, matches} ->
        {res, expansion,
           [%{title: 'records', elems: matches,
                options: [:highlight_all]}]}
    end
  end

  defp expand_record_fields(fieldToComplete, word, record, fields, rT,
            _Args, nestings, fT) do
    record2 = :erlang.list_to_atom(record)
    fieldSet2 = :sets.from_list(for f <- fields do
    ...
    case (for {record3, recordSpec} <- rT,
      ...
      [recordType | _] ->
        case (:sets.is_element(fieldToComplete2, fieldSet2)) do
          true ->
            expand_record_field_content(fieldToComplete2,
                                          recordType, word1, nestings, fT)
          false ->
            expand_record_field_name(record2, fieldSet2, recordType,
                                       word1)
        end
      _ ->
        {:no, [], []}
  ...
  defp expand_record_field_name(record, fields, recordType, word) do
    recordFieldsList = extract_record_fields(record,
                                               recordType)
    ...
    case (match(word, alts, '=')) do
      {_Res, _Expansion, []} = m ->
        m
      {res, expansion, matches} ->
        {res, expansion,
           [%{title: 'fields', elems: matches,
                options: [:highlight_all]}]}
    end
  end

  defp expand_record_field_content(field,
            {:attribute, _, :record, {_Record, fieldTypes}}, word,
            nestings, fT) do
    fieldTypesFiltered = (for {:typed_record_field,
                                 {:record_field, _, {_, _, f}},
                                 type1} <- fieldTypes,
                                f == field do
                            type1
                          end) ++ (for {:typed_record_field,
                                          {:record_field, _, {_, _, f}, _},
                                          type1} <- fieldTypes,
                                         f == field do
                                     type1
                                   end)
    case (fieldTypesFiltered) do
      [] ->
        {:no, [], []}
      [type] ->
        t = :edlin_type_suggestion.type_tree(:erlang, type,
                                               nestings, fT)
        types = :edlin_type_suggestion.get_types([], t,
                                                   nestings)
        case (nestings) do
          [] ->
            atoms = :edlin_type_suggestion.get_atoms([], t,
                                                       nestings)
            case ({word, match(word, atoms, ', ')}) do
              {[], {_Res, _Expansion, _}} ->
                {_Res, _Expansion,
                   [%{title: 'types', elems: types, options: [{:hide, :title}]}]}
              {_, {_Res, _Expansion, []} = m} ->
                m
              {_, {res, expansion, matches}} ->
                {res, expansion,
                   [%{title: 'matches', elems: matches,
                        options: [:highlight_all]}]}
            end
          _ ->
            expand_nesting_content(t, [], nestings,
                                     %{title: 'types', elems: types,
                                         options: [{:hide, :title}]})
        end
    end
  end
  ...
  def match_arguments1([p | ps], cs,
           [{:atom, [?' | _] = string} | as]) do
    case (:edlin_context.odd_quotes(?',
                                      :lists.reverse(string))) do
      true ->
        false
      _ ->
  ...
  def is_type(type, cs, string) do
    ...
    types = (for t <- :edlin_type_suggestion.get_types(cs,
                                                         type, [],
                                                         [:no_print]) do
               t
             end)
    try do
      ...
      case (term) do
        atom when is_atom(atom) ->
          ...
          :lists.member(to_list(atom),
                          atoms) or :lists.member(:erlang.atom_to_list(atom),
                                                    atoms) or find_type(types,
                                                                          [:atom,
                                                                               :node,
                                                                                   :module,
                                                                                       :fun])
        ...
        list when (is_list(list) and length(list) > 0) ->
          find_type(types,
                      [:list, :string, :nonempty_list, :maybe_improper_list,
      ...
      _, _ ->
        ...
        try do
          ...
          _, _ ->
            case (a) do
              [{:"#", _}, {:var, _, :Port}, {:"<", _}, {:float, _, _},
                                                       {:">", _}, {:dot, _}] ->
                find_type(types, [:port])
              [{:"#", _}, {:var, _, :Ref}, {:"<", _}, {:float, _, _}, {:".",
                                                                     _},
                                                                      {:float,
                                                                         _, _},
                                                                          {:">",
              ...
              [{:"#", _}, {:var, _, :Fun}, {:"<", _}, {:atom, _,
                                                     :erl_eval},
                                                      {:".", _}, {:float, _, _},
                                                                   {:">", _}] ->
                find_type(types, [:parameters, :function, :fun])
              [{:"<", _}, {:float, _, _}, {:".", _}, {:integer, _, _},
                                                     {:">", _}, {:dot, _}] ->
                find_type(types, [:pid])
  ...
  defp in_range(integer,
            [{:type, :range,
                [{:integer, start}, {:integer, end__}]} |
                 _])
  ...
  defp check_integer_type(types, integer) when integer == 0 do
    find_type(types,
                [:integer, :non_neg_integer,
                               :arity]) or in_range(integer, types)
  end

  defp check_integer_type(types, integer) when integer < 0 do
    find_type(types,
                [:integer, :neg_integer]) or in_range(integer, types)
  end

  defp check_integer_type(types, integer) when integer > 0 do
    find_type(types,
                [:integer, :non_neg_integer,
                               :pos_integer]) or in_range(integer, types)
  end
  ...
  defp expand_function_parameter_type(mod, mFA, funType, args, unfinished, nestings,
            fT) do
    typeTree = :edlin_type_suggestion.type_tree(mod,
                                                  funType, nestings, fT)
    {parameters, constraints1} = (case (typeTree) do
                                    {:function, {{:parameters, parameters1}, _},
                                       constraints} ->
    ...
    case (match_arguments(typeTree, args)) do
      ...
      true ->
        ...
        {t, _Name} = (case (parameter) do
                        ...
                        t1 ->
                          {t1,
                             :edlin_type_suggestion.print_type(t1, [],
                                                                 [{:first_only,
                                                                     true}])}
                      end)
        ts = :edlin_type_suggestion.get_types(constraints1, t,
                                                nestings)
        types = (case (ts) do
                   [] ->
                     []
                   _ ->
                     sectionTypes = (for (%{} = s) <- ts do
                                       s
                                     end)
                     types1 = (case (for ({_, _} = e) <- ts do
                                       e
                                     end) do
                                 [] ->
                                   sectionTypes
                                 elems ->
                                   case (sectionTypes) do
                                     [] ->
                                       elems
                                     sT ->
                                       [%{title: 'simple types', elems: elems,
                                            options: [{:hide, :title}]} |
                                            sT]
                                   end
                               end)
            ...
            case (unfinished) do
              [] ->
                case (t) do
                  atom1 when is_atom(atom1) ->
                    cC = (case (length(args) + 1 < length(parameters)) do
                            true ->
                              ', '
                            false ->
                              ')'
                    ...
                    case (matches) do
                      ...
                      _ ->
                        {res, expansion,
                           [%{title: mFA, elems: [],
                                options:
                                [{:highlight_param, length(args) + 1}]}]}
                    end
                  _ when types == [] ->
                    {:no, [], []}
                  _ ->
                    {:no, [],
                       [%{title: mFA, elems: types,
              ...
              {_, word} ->
                {res, expansion, matches} = ((
                                               ...
                                               atoms1 = :edlin_type_suggestion.get_atoms(constraints1,
                                                                                           t,
                                                                                           nestings)
                                               match(word, atoms1, cC)
                                             ))
                match1 = (case (matches) do
                            [] ->
                              []
                            _ ->
                              atoms = [%{title: 'atoms', elems: matches,
                                           options: [{:hide, :title}]}]
                              [%{title: mFA, elems: atoms,
                                   options:
                                   [{:highlight_param, length(args) + 1}]}]
                          end)
  ...
  defp expand_nesting_content(t, constraints, nestings, section) do
    {nestingType, unfinishedNestingArg,
       _NestingArgs} = (case (:lists.last(nestings)) do
    ...
    types = (for ts <- :edlin_type_suggestion.get_types(constraints,
                                                          t,
    ...
    case (unfinishedNestingArg) do
      [] ->
        case (find_type(types, [nestingType])) do
          true ->
            ...
            nestingArities = :edlin_type_suggestion.get_arity(constraints,
                                                                t, nestings2)
            fold_results(for nestingArity <- nestingArities do
                           (
      ...
      {_, word} ->
        atoms1 = :edlin_type_suggestion.get_atoms(constraints,
                                                    t, nestings)
        {res1, expansion1, matches1} = match(word, atoms1, '')
        {res, expansion, matches} = (case (matches1) do
                                       [] ->
                                         nestings2 = add_to_last_nesting(unfinishedNestingArg,
                                                                           nestings)
                                         nestingArities = :edlin_type_suggestion.get_arity(constraints,
                                                                                             t,
                                                                                             nestings2)
                                       ...
                                       [{word2, _}] ->
                                         nestings2 = add_to_last_nesting({:atom,
                                                                            word2},
                                                                           nestings)
                                         nestingArities = :edlin_type_suggestion.get_arity(constraints,
                                                                                             t,
  ...
  defp extract_record_fields(record,
            {:attribute, _, :record, {record, fields}}) do
    for x <- (for f <- fields do
  ...
  defp extract_record_field({:typed_record_field, {_, _, {:atom, _, field}},
             _}) do
    field
  end

  defp extract_record_field({:typed_record_field,
             {_, _, {:atom, _, field}, _}, _}) do
    field
  end
  ...
  defp fold_completion_result({:yes, cmp1, matches1},
            {:yes, cmp2, matches2}) do
  ...
  defp expand_filepath(pathPrefix, word) do
    ...
    showHidden = (case (word) do
                    '.' ++ _ ->
                      true
                    _ ->
                      false
                  end)
    entries = (case (:file.list_dir(path)) do
                 {:ok, e} ->
                   :lists.map(fn x ->
                                   case (:filelib.is_dir(path ++ '/' ++ x)) do
                                     true ->
                                       x ++ '/'
                                     false ->
                                       x
    ...
    entriesFiltered = (for file <- entries,
                             (case (file) do
                                [?. | _] ->
                                  showHidden
                                _ ->
    ...
    case (match(word, entriesFiltered, [])) do
      {:yes, cmp, [match]} ->
        case (:filelib.is_dir(path ++ '/' ++ word ++ cmp)) do
          true ->
            {:yes, cmp, [match]}
          false ->
            {:yes, cmp ++ '"', [match]}
  ...
  def shell_default_or_bif(fun) do
    case (:lists.member(:erlang.list_to_atom(fun),
  ...
  defp split_at_last_slash(filepath) do
    {file, path} = :lists.splitwith(fn x ->
                                         x != ?/
                                    end,
  ...
  defp print_function_head(modStr, funStr, arity) do
    :lists.flatten(modStr ++ ':' ++ funStr ++ '/' ++ :erlang.integer_to_list(arity))
  end

  defp print_function_head(modStr, funStr, funType, fT) do
    :lists.flatten(print_function_head_from_type(modStr,
                                                   funStr, funType, fT))
  end
                                          ...
                                          for {_N,
                                            ...
                                            case (p) do
                                              ...
                                              {:ann_type, {:var, v}, _T} ->
                                                :erlang.atom_to_list(v)
                                              t ->
                                                :edlin_type_suggestion.print_type(t,
                                                                                    [],
                                                                                    [{:first_only,
                                                                                        true}])
                                            end
  ...
  defp print_function_head_from_type(mod, fun, funType, fT) do
    case (:edlin_type_suggestion.type_tree(:erlang.list_to_atom(mod),
                                             funType, [], fT)) do
      {:function,
         {{:parameters, parameters}, {:return, return}}, _} ->
  ...
  defp expand_module_function(bef0, fT) do
    ...
    case (over_white(bef1, [], 0)) do
      ...
      {[cC, n_Esc | _], _White, _Nwh}
          when (cC === ?] or cC === ?) or cC === ?> or cC === ?} or cC === ?" or cC === ?' and
                  n_Esc !== ?$ and n_Esc !== ?-)
               ->
        {:no, [], []}
      {[], _, _} ->
      ...
      {_, _, _} ->
        case (word) do
          ...
          _ ->
            ...
            completeChar = (case (typeOfExpand) do
                              :help ->
                                ', '
                              :help_type ->
                                ', '
            ...
            fold_results(expand_helper(fT, typeOfExpand, word,
                                         completeChar))
        end
    end
  end
  ...
  defp expand_helper(fT, :all, word, completeChar) do
    [expand_module_name(word, completeChar),
         expand_bifs(word), expand_shell_default(word),
  ...
  defp expand_helper(fT, _, word, completeChar) do
    [expand_module_name(word, completeChar),
         expand_bifs(word), expand_user_defined_functions(fT,
                                                            word),
                                expand_keyword(word)]
  end
  ...
  defp expand_bifs(prefix) do
    alts = (for ({e, a} = eA) <- get_exports(:erlang),
                  :erl_internal.bif(e, a) do
              eA
            end)
    cC = '('
  ...
  defp expand_shell_default(prefix) do
    ...
    case (match(prefix, alts, cC)) do
      ...
      {res, expansion, matches} ->
        {res, expansion,
           [%{title: 'commands', elems: matches,
                options: [:highlight_all]}]}
    end
  end

  defp expand_user_defined_functions(fT, prefix) do
    alts = (for {{:function, {_, name, arity}}, _} <- fT do
              {name, arity}
            end)
    cC = '('
    case (match(prefix, alts, cC)) do
      ...
      {res, expansion, matches} ->
        {res, expansion,
           [%{title: 'user_defined', elems: matches,
                options: [:highlight_all]}]}
    end
  end
  ...
  defp expand_module_name(prefix, cC) do
    alts = (for {m, _, _} <- :code.all_available() do
    ...
    case (match(prefix, alts, cC)) do
      ...
      {res, expansion, matches} ->
        {res, expansion,
           [%{title: 'modules', elems: matches,
                options: [:highlight_all]}]}
    end
  end

  defp get_arities('shell_default' = modStr, funcStr, fT) do
  ...
  defp expand_function_name(modStr, funcPrefix, completeChar, fT) do
    case (to_atom(modStr)) do
      {:ok, mod} ->
        ...
        {res, expansion, matches} = (result = match(funcPrefix,
                                                      exports, completeChar))
        case (matches) do
          [] ->
            result
          _ ->
            {res, expansion,
               [%{title: 'functions', elems: matches,
                    options: [:highlight_all]}]}
        end
      :error ->
        {:no, [], []}
      ...
      {:ok, mod} ->
        case (get_module_types(mod)) do
          {:no, [], []} ->
            {:no, [], []}
          types ->
            {res, expansion, matches} = (result = match(typePrefix,
                                                          types, completeChar))
            case (matches) do
              [] ->
                result
  ...
  defp match(prefix, alts, extra0) do
    ...
    matches = :lists.sort(for {h, a} <- alts2,
                                :lists.prefix(prefix, s = flat_write(h)) do
                            {s, a}
                          end)
    matches2 = :lists.usort(case (extra0) do
                              [] ->
                                for {s, _} <- matches do
                                  {s, []}
                                end
    ...
    case (longest_common_head(for {n, _} <- matches do
                                n
                              end)) do
      {:partial, []} ->
        {:no, [], matches2}
      ...
      {:complete, str} ->
        extra = (case ({extra0, matches}) do
                   {'/', [{^str, n}]} when is_integer(n) ->
  ...
  defp special_sort1([c | a], b) when c == ?{ or c == ?. or
                             c == ?# do
    special_sort1(a, b)
  end

  defp special_sort1(a, [c | b]) when c == ?{ or c == ?. or
                             c == ?# do
    special_sort1(a, b)
  end
  ...
  defp to_legacy_format([%{title: title} | rest]) when title === 'commands' or
                                           title === 'bifs' do
    to_legacy_format(rest)
  end

  defp to_legacy_format([%{title: title, elems: elems} | rest])
      when title === 'modules' or title === 'functions' or title === 'bindings' or
             (title === 'user_defined' and title === 'records') or title === 'fields' or
             title === 'types' or title === 'atoms' or title === 'matches' or
             title === 'keywords' or title === 'typespecs' do
  ...
  def format_matches([%{} | _] = fF, lineWidth) do
    groups = :maps.groups_from_list(fn %{title: title,
                                           elems: t, options: opts} ->
                                         separator = :proplists.get_value(:separator,
                                                                            opts,
                                                                            '\n')
                                      ...
                                      fF)
    s = :lists.flatten(for {matches,
                              f} <- :lists.sort(fn {_, a}, {_, b} ->
                                                     a <= b
                                                end,
                                                  :maps.to_list(groups)) do
  ...
  defp format_title(%{title: mFA, options: options}, _LineWidth) do
    case (:proplists.get_value(:hide, options)) do
      ...
      _ ->
        ...
        highlightAll = :proplists.is_defined(:highlight_all,
                                               options)
        case (highlightAll) do
          true ->
            '\e[;1;4m' ++ mFA ++ '\e[0m' ++ separator
          _ ->
            highlightParam = :proplists.get_value(:highlight_param,
                                                    options, false)
            mFA2 = (case (highlightParam) do
                      false ->
                        mFA
                      _ ->
                        ...
                        prevArgs = '(?:' ++ argPattern3 ++ ',\\s){' ++ :erlang.integer_to_list(previousParams) ++ '}'
                        functionHeadStart = '^([^\\(]+\\(' ++ prevArgs ++ ')'
                        highlightArg = '(' ++ argPattern3 ++ ')'
                        nextArgs = '(?:,\\s' ++ argPattern3 ++ ')*'
                        functionHeadEnd = '(' ++ nextArgs ++ '\\)(?:.*))$'
                        :re.replace(mFA,
                                      functionHeadStart ++ highlightArg ++ functionHeadEnd,
                                      '\\1\e[;1;4m\\2\e[0m\\3', [:global, {:return, :list}, :unicode])
                    end)
            highlight = :proplists.get_value(:highlight, options,
                                               false)
            (case (highlight) do
               false ->
                 mFA2
               _ ->
                 :re.replace(mFA2, ('(\\Q' ++ highlight ++ '\\E)'), '\e[;1;4m\\1\e[0m',
  ...
  defp format_section_matches([%{} | _] = fF, lineWidth, acc) do
    groups = :maps.groups_from_list(fn %{title: title,
                                      ...
                                      fF)
    :lists.flatten(for {matches, f} <- :lists.sort(fn {_,
                                                         a},
                                                        {_, b} ->
                                                        a <= b
                                                   end,
  ...
  defp format_section_matches1(lS, lineWidth, len) do
    ...
    s1 = format_col(opt ++ l,
                      field_width(opt ++ l, lineWidth), len, [], lineWidth,
                      opt)
    s2 = :lists.map(fn line ->
  ...
  defp format_col([{:title, titleLen} | t], width, len, acc0, lL,
            opt) do
  ...
  defp format_col([a | t], width, len, acc0, lL, _Opt) do
    ...
    {h, _} = (case (:string.length(h0) > hmax) do
                true ->
