defmodule :m_epp do
  ...
  def format_error(:cannot_parse) do
    :io_lib.format(~c"cannot parse file, giving up", [])
  end

  def format_error({:bad, w}) do
    :io_lib.format(~c"badly formed '~s'", [w])
  end

  def format_error({:duplicated_argument, arg}) do
    :io_lib.format(~c"argument '~ts' already used", [arg])
  end

  def format_error(:missing_parenthesis) do
    :io_lib.format(~c"badly formed define: missing closing right parenthesis", [])
  end

  def format_error(:missing_comma) do
    :io_lib.format(~c"badly formed define: missing comma", [])
  end
  ...
  def format_error({:call, what}) do
    :io_lib.format(~c"illegal macro call '~ts'", [what])
  end

  def format_error({:undefined, m, :none}) do
    :io_lib.format(~c"undefined macro '~ts'", [m])
  end

  def format_error({:undefined, m, a}) do
    :io_lib.format(~c"undefined macro '~ts/~p'", [m, a])
  end

  def format_error({:depth, what}) do
    :io_lib.format(~c"~s too deep", [what])
  end

  def format_error({:mismatch, m}) do
    :io_lib.format(~c"argument mismatch for macro '~ts'", [m])
  end

  def format_error({:arg_error, m}) do
    :io_lib.format(~c"badly formed argument for macro '~ts'", [m])
  end

  def format_error({:redefine, m}) do
    :io_lib.format(~c"redefining macro '~ts'", [m])
  end

  def format_error({:redefine_predef, m}) do
    :io_lib.format(~c"redefining predefined macro '~s'", [m])
  end

  def format_error({:circular, m, :none}) do
    :io_lib.format(~c"circular macro '~ts'", [m])
  end

  def format_error({:circular, m, a}) do
    :io_lib.format(~c"circular macro '~ts/~p'", [m, a])
  end

  def format_error({:include, w, f}) do
    :io_lib.format(~c"can't find include ~s \"~ts\"", [w, f])
  end

  def format_error({:illegal, how, what}) do
    :io_lib.format(~c"~s '-~s'", [how, what])
  end

  def format_error({:illegal_function, macro}) do
    :io_lib.format(~c"?~s can only be used within a function", [macro])
  end

  def format_error({:illegal_function_usage, macro}) do
    :io_lib.format(~c"?~s must not begin a form", [macro])
  end

  def format_error(:elif_after_else) do
    ~c"'elif' following 'else'"
  end

  def format_error({:NYI, what}) do
    :io_lib.format(~c"not yet implemented '~s'", [what])
  end

  def format_error({:error, term}) do
    :io_lib.format(~c"-error(~tp).", [term])
  end

  def format_error({:warning, term}) do
    :io_lib.format(~c"-warning(~tp).", [term])
  end

  def format_error(:ftr_after_prefix) do
    ~c"feature directive not allowed after exports or record definitions"
  end

  def format_error(:tqstring) do
    ~c"triple-quoted (or more) strings will change meaning in OTP-27.0"
  end

  def format_error(:string_concat) do
    ~c"adjacent string literals without intervening white space\nIn OTP-27.0 this will be a triple-quoted string or an error.\nRewrite them as one string, or insert white space\nbetween the strings."
  end
  ...
  defp com_oding(b, fun, n, com)
       when byte_size(b) >= length(~c"oding") do
    com_c(b, fun, n, com)
  end
  ...
  defp com_encoding([~c"latin", ~c"1" | _]) do
    :latin1
  end

  defp com_encoding([~c"utf", ~c"8" | _]) do
    :utf8
  end
  ...
  defp server(pid, name, options) do
    ...
    case :proplists.get_value(:fd, options) do
      ...
      fd ->
        init_server(pid, name, options, r_epp(st, file: fd, pre_opened: true))
    end
  end
      ...
      {:ok, ms1} ->
        ...
        st =
          r_epp(st0,
            ...
            erl_scan_opts:
              [{:text_fun, keep_ftr_keywords()}, {:reserved_word_fun, resWordFun}] ++
                cond do
                  parseChecks ->
        ...
        enter_file_reply(from, file_name(sourceName), anno, atLocation, :code, deterministic)
        wait_req_scan(st)
  ...
  defp keep_ftr_keywords() do
    ...
    keywords =
      :lists.flatmap(
        &:erl_features.keywords/1,
        features
      )

    f = fn atom ->
      :erlang.atom_to_list(atom) ++ ~c"'"
    end
  ...
  defp predef_macros(file, enabledFeatures0) do
    ...
    availableFeatures =
      for ftr <- :erl_features.all(),
          :maps.get(
            :status,
            :erl_features.info(ftr)
          ) != :rejected do
        ftr
      end
  ...
  defp ftr_macro(features) do
    ...
    fexp = fn ftr ->
      [{:"(", anno}, {:var, anno, arg}, {:")", anno}, {:==, anno}, {:atom, anno, ftr}]
    end
  ...
  defp enter_file(_NewName, inc, from, st)
    ...
    epp_reply(from, {:error, {loc(inc), :epp, {:depth, ~c"include"}}})
    wait_req_scan(st)
  end
  ...
  defp enter_file2(newF, pname, from, st0, atLocation) do
    anno = :erl_anno.new(atLocation)
    enter_file_reply(from, pname, anno, atLocation, :code, r_epp(st0, :deterministic))
  ...
  defp enter_file_reply(from, name, locationAnno, atLocation, where, deterministic) do
  ...
  defp leave_file(from, st) do
    case r_epp(st, :istk) do
      [i | cis] ->
        epp_reply(
          from,
          {:error, {r_epp(st, :location), :epp, {:illegal, ~c"unterminated", i}}}
        )
      ...
      [] ->
        case r_epp(st, :sstk) do
          [oldSt | sts] ->
            ...
            enter_file_reply(from, oldName, anno, currLoc, :code, r_epp(st, :deterministic))
  ...
  defp scan_toks(from, st) do
    r_epp(file: file, location: loc, erl_scan_opts: scanOpts) = st
  ...
  defp scan_toks([{:-, _Lh}, {:else, _Le} = else__ | toks], from, st)
  ...
  defp scan_feature(
         [{:"(", _Ap}, {:atom, _Am, ftr}, {:",", _}, {:atom, _, ind}, {:")", _}, {:dot, _}],
         feature,
         from,
         st
       )
  ...
  defp update_features(st0, ind, ftr, loc) do
    ...
    case :erl_features.keyword_fun(ind, ftr, ftrs0, keywordFun) do
      {:error, reason} ->
        {:error, {reason, loc}}

      {:ok, {ftrs1, resWordFun1}} ->
        macs0 = r_epp(st0, :macs)
        macs1 = Map.put(macs0, :FEATURE_ENABLED, [ftr_macro(ftrs1)])
  ...
  defp scan_define([{:"(", _Ap}, {type, _Am, _} = mac | toks], def__, from, st)
  ...
  defp scan_undef([{:"(", _Alp}, {:atom, _Am, m}, {:")", _Arp}, {:dot, _Ad}], _Undef, from, st) do
    macs = :maps.remove(m, r_epp(st, :macs))
    uses = :maps.remove(m, r_epp(st, :uses))
    scan_toks(from, r_epp(st, macs: macs, uses: uses))
  end

  defp scan_undef([{:"(", _Alp}, {:var, _Am, m}, {:")", _Arp}, {:dot, _Ad}], _Undef, from, st) do
    macs = :maps.remove(m, r_epp(st, :macs))
    uses = :maps.remove(m, r_epp(st, :uses))
    scan_toks(from, r_epp(st, macs: macs, uses: uses))
  end
  ...
  defp scan_include_lib1(
       ...
       when length(r_epp(st, :sstk)) >= 8 do
    epp_reply(from, {:error, {loc(inc), :epp, {:depth, ~c"include_lib"}}})
    wait_req_scan(st)
  end
  ...
  defp scan_ifdef([{:"(", _Alp}, {:atom, _Am, m}, {:")", _Arp}, {:dot, _Ad}], _IfD, from, st) do
  ...
  defp scan_ifdef([{:"(", _Alp}, {:var, _Am, m}, {:")", _Arp}, {:dot, _Ad}], _IfD, from, st) do
    case is_macro_defined(m, st) do
      true ->
        scan_toks(from, r_epp(st, istk: [:ifdef | r_epp(st, :istk)]))
  ...
  defp scan_ifndef([{:"(", _Alp}, {:atom, _Am, m}, {:")", _Arp}, {:dot, _Ad}], _IfnD, from, st) do
    case is_macro_defined(m, st) do
      true ->
  ...
  defp scan_ifndef([{:"(", _Alp}, {:var, _Am, m}, {:")", _Arp}, {:dot, _Ad}], _IfnD, from, st) do
    case is_macro_defined(m, st) do
      true ->
        skip_toks(from, st, [:ifndef])
  ...
  defp scan_else([{:dot, _Ad}], else__, from, st) do
    case r_epp(st, :istk) do
      [:else | cis] ->
        epp_reply(
          from,
          {:error, {loc(else__), :epp, {:illegal, ~c"repeated", :else}}}
        )
      ...
      [] ->
        epp_reply(
          from,
          {:error, {loc(else__), :epp, {:illegal, ~c"unbalanced", :else}}}
        )
  ...
  defp scan_elif(_Toks, elif, from, st) do
    case r_epp(st, :istk) do
      [:else | cis] ->
        epp_reply(
          from,
          {:error, {loc(elif), :epp, {:illegal, ~c"unbalanced", :elif}}}
        )
  ...
  defp scan_endif([{:dot, _Ad}], endif, from, st) do
    case r_epp(st, :istk) do
      ...
      [] ->
        epp_reply(
          from,
          {:error, {loc(endif), :epp, {:illegal, ~c"unbalanced", :endif}}}
        )
  ...
  defp scan_file1(
    ...
    enter_file_reply(from, name, anno, loc(tf), :generated, r_epp(st, :deterministic))
    ms0 = r_epp(st, :macs)
  ...
  defp skip_toks(from, st, [i | sis]) do
    ...
    case :io.scan_erl_form(r_epp(st, :file), :"", r_epp(st, :location), r_epp(st, :erl_scan_opts)) do
      ...
      {:ok, [{:-, _Ah}, {:atom, _Ae, :else} = else__ | _Toks], cl} ->
  ...
  defp macro_pars_cont([{:",", _Ad}, {:var, _, name} = t | ex], args, _T0) do
    check_macro_arg(name, args, t)
    macro_pars_cont(ex, [name | args], t)
  end
  ...
  defp bind_args([{:"(", _Alp}, {:")", _Arp} | toks], _Lm, _M, [], bs) do
    {bs, toks}
  end
  ...
  defp count_args([{:",", _Ac}, {:")", _Arp} | _Toks], lm, m, _NbArgs) do
    throw({:error, lm, {:arg_error, m}})
  end
  ...
  defp expand_macro([{:"?", _}, {:"?", _}, {:var, _Av, v} | ts], anno, rest, bs) do
    case bs do
      %{^v => val} ->
  ...
  defp expand_macro([t | ts], anno, rest, bs) do
    [:erlang.setelement(2, t, anno) | expand_macro(ts, anno, rest, bs)]
  end
  ...
  defp stringify1([t | tokens]) do
    [:io_lib.format(~c" ~ts", [token_src(t)]) | stringify1(tokens)]
  end
  ...
  defp find_mismatch([:var_or_atom | tags], [{:var, _A, _V} = t | ts], _T0) do
    find_mismatch(tags, ts, t)
  end

  defp find_mismatch([:var_or_atom | tags], [{:atom, _A, _N} = t | ts], _T0) do
    find_mismatch(tags, ts, t)
  end
