defmodule :m_erl_scan do
  ...
  def format_error({:illegal, type}) do
    :lists.flatten(:io_lib.fwrite(~c"illegal ~w", [type]))
  end

  def format_error(:char) do
    ~c"unterminated character"
  end

  def format_error({:base, base}) do
    :lists.flatten(:io_lib.fwrite(~c"illegal base '~w'", [base]))
  end
  ...
  defp string1(cs, st, line, col, toks) do
    case scan1(cs, st, line, col, toks) do
      {:more, {cs0, nst, ncol, ntoks, nline, any, fun}} ->
        case fun.(cs0 ++ :eof, nst, nline, ncol, ntoks, any) do
  ...
  defp scan_atom(cs0, st, line, col, toks, ncs0) do
    case scan_name(cs0, ncs0) do
      {:more, ncs} ->
        {:more, {[], st, col, toks, line, ncs, &scan_atom_fun/6}}
  ...
  defp scan_variable(cs0, st, line, col, toks, ncs0) do
    case scan_name(cs0, ncs0) do
      {:more, ncs} ->
        {:more, {[], st, col, toks, line, ncs, &scan_variable_fun/6}}
  ...
  defp scan_dot([c | _] = cs, st, line, col, toks, ncs)
    ...
    tok2(cs, r_erl_scan(st, in_check: false), line, col, toks, ncs, :., 1)
  end
  ...
  defp scan_dot([?% | _] = cs, st, line, col, toks, ncs) do
    anno =
      anno(
        ...
        col,
        st,
        case r_erl_scan(st, :has_fun) and
               r_erl_scan(st, :text_fun).(
                 :dot,
                 ncs
               ) do
          true ->
  ...
  defp scan_newline([?\r | cs], st, line, col, toks) do
    newline_end(cs, st, line, col, toks, 2, ~c"\n\r")
  end
  ...
  defp scan_newline(cs, st, line, col, toks) do
    scan_nl_white_space(cs, st, line, col, toks, ~c"\n")
  end
  ...
  defp scan_nl_spcs([] = cs, st, line, col, toks, n) do
    {:more, {cs, st, col, toks, line, n, &scan_nl_spcs_fun/6}}
  end
  ...
  defp scan_nl_tabs([] = cs, st, line, col, toks, n) do
    {:more, {cs, st, col, toks, line, n, &scan_nl_tabs_fun/6}}
  end
  ...
  defp scan_nl_white_space([] = cs, st, line, col, toks, ncs) do
    {:more, {cs, st, col, toks, line, ncs, &scan_nl_white_space_fun/6}}
  end

  defp scan_nl_white_space(cs, r_erl_scan(has_fun: false) = st, line, :no_col = col, toks, ncs) do
    anno = anno(line)
  ...
  defp newline_end(cs, r_erl_scan(has_fun: false) = st, line, :no_col = col, toks, _N, ncs) do
  ...
  defp skip_white_space([] = cs, st, line, col, toks, n) do
    {:more, {cs, st, col, toks, line, n, &skip_white_space_fun/6}}
  end
  ...
  defp scan_white_space([] = cs, st, line, col, toks, ncs) do
    {:more, {cs, st, col, toks, line, ncs, &scan_white_space_fun/6}}
  end
  ...
  defp scan_char([c | _Cs], _St, line, col, _Toks)
       when is_integer(c) and 0 <= c and c < 1_114_112 do
  ...
  defp scan_string(cs, r_erl_scan() = st, line, col, toks, {wcs, str, line0, col0}) do
    case scan_string0(cs, st, line, col, ?", str, wcs) do
      {:more, ncs, nline, ncol, nstr, nwcs} ->
        state = {nwcs, nstr, line0, col0}
        {:more, {ncs, st, ncol, toks, nline, state, &scan_string/6}}
      ...
      {ncs, nline, ncol, nstr, nwcs} ->
        ...
        scan_string_concat(ncs, st, nline, ncol, [{:string, anno, nwcs} | toks], [])
    end
  end
  ...
  defp scan_string_concat(cs, st, line, col, toks, _) do
    case cs do
      [?" | _] ->
        ...
        scan1(cs, st, line, col, [{:string_concat, anno, ~c""} | toks])

      [] ->
        {:more, {cs, st, col, toks, line, [], &scan_string_concat/6}}
  ...
  defp scan_qatom(cs, r_erl_scan() = st, line, col, toks, {wcs, str, line0, col0}) do
    case scan_string0(cs, st, line, col, ?', str, wcs) do
      {:more, ncs, nline, ncol, nstr, nwcs} ->
        state = {nwcs, nstr, line0, col0}
        {:more, {ncs, st, ncol, toks, nline, state, &scan_qatom/6}}
  ...
  defp scan_string0(cs, r_erl_scan(has_fun: false), line, :no_col = col, q, [], wcs) do
    scan_string_no_col(cs, line, col, q, wcs)
  end

  defp scan_string0(cs, r_erl_scan(has_fun: true), line, :no_col = col, q, str, wcs) do
  ...
  defp scan_escape([?x, h1, h2 | cs], col)
       when is_integer(h1) and
              ((h1 >= ?0 and h1 <= ?9) or (h1 >= ?A and h1 <= ?F) or (h1 >= ?a and h1 <= ?f)) and
              is_integer(h2) and
  ...
  defp scan_escape([?x, h1], _Col)
       when is_integer(h1) and
              ((h1 >= ?0 and h1 <= ?9) or (h1 >= ?A and h1 <= ?F) or (h1 >= ?a and h1 <= ?f)) do
    :more
  end
  ...
  defp scan_hex([c | cs], col, wcs)
       when is_integer(c) and
              ((c >= ?0 and c <= ?9) or (c >= ?A and c <= ?F) or (c >= ?a and c <= ?f)) do
    scan_hex(cs, incr_column(col, 1), [c | wcs])
  end

  defp scan_hex(cs, col, wcs) do
    scan_hex_end(cs, col, wcs, ~c"x{")
  end
  ...
  defp scan_number([?_, next | cs], st, line, col, toks, [prev | _] = ncs, _Us)
       when is_integer(next) and ?0 <= next and next <= ?9 and is_integer(prev) and ?0 <= prev and
              prev <= ?9 do
    scan_number(cs, st, line, col, toks, [next, ?_ | ncs], :with_underscore)
  end

  defp scan_number([?_] = cs, st, line, col, toks, ncs, us) do
    {:more, {cs, st, col, toks, line, {ncs, us}, &scan_number/6}}
  end
  ...
  defp scan_based_int(cs, r_erl_scan() = st, line, col, toks, {b, nCs, bCs, us})
       when is_integer(b) and 2 <= b and
              b <= 1 + ?Z - ?A + 10 do
  ...
  defp scan_based_int([c | cs], st, line, col, toks, b, ncs, bcs, us)
       when is_integer(c) and
              ((is_integer(c) and ?0 <= c and c <= ?9 and c < ?0 + b) or
                 (c >= ?A and b > 10 and c < ?A + b - 10) or
  ...
  defp scan_based_int([?_, next | cs], st, line, col, toks, b, [prev | _] = ncs, bcs, _Us)
       when is_integer(next) and
              ((is_integer(next) and ?0 <= next and next <= ?9 and next < ?0 + b) or
                 (next >= ?A and b > 10 and next < ?A + b - 10) or
                 (next >= ?a and b > 10 and next < ?a + b - 10)) and is_integer(prev) and
              ((is_integer(prev) and ?0 <= prev and prev <= ?9 and prev < ?0 + b) or
  ...
  defp scan_based_int([?_] = cs, st, line, col, toks, b, nCs, bCs, us) do
    {:more, {cs, st, col, toks, line, {b, nCs, bCs, us}, &scan_based_int/6}}
  end
  ...
  defp scan_based_int(cs, st, line, col, toks, b, ncs0, [_ | _] = bcs, us) do
    ncs = :lists.reverse(ncs0)
  ...
  defp scan_fraction([?_] = cs, st, line, col, toks, ncs, us) do
    {:more, {cs, st, col, toks, line, {ncs, us}, &scan_fraction/6}}
  end
  ...
  defp scan_exponent_sign([] = cs, st, line, col, toks, ncs, us) do
    {:more, {cs, st, col, toks, line, {ncs, us}, &scan_exponent_sign/6}}
  end
  ...
  defp scan_exponent([?_] = cs, st, line, col, toks, ncs, us) do
    {:more, {cs, st, col, toks, line, {ncs, us}, &scan_exponent/6}}
  end
  ...
  defp skip_comment([c | cs], st, line, col, toks, n)
       when c !== ?\n and
              is_integer(c) and 0 <= c and c < 1_114_112 do
  ...
  defp skip_comment([] = cs, st, line, col, toks, n) do
    {:more, {cs, st, col, toks, line, n, &skip_comment_fun/6}}
  end
  ...
  defp scan_comment([] = cs, st, line, col, toks, ncs) do
    {:more, {cs, st, col, toks, line, ncs, &scan_comment_fun/6}}
  end
  ...
  defp scan_check(~c"%%ssa%" ++ cs, st, line, col, toks, _Ncs) do
    scan_check1(cs, st, line, toks, col, 7)
  end

  defp scan_check(~c"%%ssa" = cs, st, line, col, toks, ncs) do
    {:more, {cs, st, col, toks, line, ncs, &scan_check/6}}
  end

  defp scan_check(~c"%%ss" = cs, st, line, col, toks, ncs) do
    {:more, {cs, st, col, toks, line, ncs, &scan_check/6}}
  end

  defp scan_check(~c"%%s" = cs, st, line, col, toks, ncs) do
    {:more, {cs, st, col, toks, line, ncs, &scan_check/6}}
  end

  defp scan_check(~c"%%" = cs, st, line, col, toks, ncs) do
    {:more, {cs, st, col, toks, line, ncs, &scan_check/6}}
  end
  ...
  defp scan_check(cs, st = r_erl_scan(comment: true), line, col, toks, ncs) do
    scan_comment(cs, st, line, col, toks, ncs)
  end
  ...
  defp scan_check1(cs, st = r_erl_scan(in_check: true), line, toks, col, noofCols) do
    scan1(cs, st, line, incr_column(col, noofCols), toks)
  end

  defp scan_check1(cs, st, line, toks, col, noofCols) do
    tok2(cs, r_erl_scan(st, in_check: true), line, col, toks, ~c"%ssa%", :"%ssa%", noofCols)
  end

  defp tok2(cs, r_erl_scan(has_fun: false) = st, line, :no_col = col, toks, _Wcs, p) do
  ...
  defp tok2(cs, r_erl_scan(has_fun: false) = st, line, :no_col = col, toks, _Wcs, p, _N) do
    scan1(cs, st, line, col, [{p, anno(line)} | toks])
  end
  ...
  defp tok3(cs, r_erl_scan(has_fun: false) = st, line, :no_col = col, toks, item, _S, sym) do
    scan1(cs, st, line, col, [{item, anno(line), sym} | toks])
  end

  defp tok3(cs, r_erl_scan() = st, line, col, toks, item, string, sym) do
    token =
      {item,
       anno(
         line,
  ...
  defp tok3(cs, r_erl_scan() = st, line, col, toks, item, string, sym, length) do
    token =
      {item,
       anno(
         line,
