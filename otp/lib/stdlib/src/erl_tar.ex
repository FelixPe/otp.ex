defmodule :m_erl_tar do
  ...
  Record.defrecord(:r_sparse_array, :sparse_array,
    entries: [],
    is_extended: false,
    max_entries: 0
  )
  ...
  def format_error(:bad_header) do
    ~c"Unrecognized tar header format"
  end

  def format_error({:bad_header, reason}) do
    :lists.flatten(:io_lib.format(~c"Unrecognized tar header format: ~p", [reason]))
  end

  def format_error({:invalid_header, :negative_size}) do
    ~c"Invalid header: negative size"
  end

  def format_error(:invalid_sparse_header_size) do
    ~c"Invalid sparse header: negative size"
  end

  def format_error(:invalid_sparse_map_entry) do
    ~c"Invalid sparse map entry"
  end

  def format_error({:invalid_sparse_map_entry, reason}) do
    :lists.flatten(:io_lib.format(~c"Invalid sparse map entry: ~p", [reason]))
  end

  def format_error(:invalid_end_of_archive) do
    ~c"Invalid end of archive"
  end
  ...
  def format_error(:integer_overflow) do
    ~c"Failed to parse numeric: integer overflow"
  end

  def format_error({:misaligned_read, pos}) do
    :lists.flatten(
      :io_lib.format(~c"Read a block which was misaligned: block_size=~p pos=~p", [512, pos])
    )
  end

  def format_error(:invalid_gnu_1_0_sparsemap) do
    ~c"Invalid GNU sparse map (version 1.0)"
  end

  def format_error({:invalid_gnu_0_1_sparsemap, format}) do
    :lists.flatten(:io_lib.format(~c"Invalid GNU sparse map (version ~s)", [format]))
  end

  def format_error(:unsafe_path) do
    ~c"The path points above the current working directory"
  end
  ...
  defp extract1(:eof, reader, _, :leading_slash) do
    :error_logger.info_msg(~c"erl_tar: removed leading '/' from member names\n")
    {:ok, :ok, reader}
  end
  ...
  defp mode_to_string(mode) do
    mode_to_string(mode, ~c"xwrxwrxwr", [])
  end
  ...
  defp open1(name, access, raw, opts0)
       when is_list(name) or
              is_binary(name) do
    opts =
      case :lists.member(
    ...
    res =
      case readInfo.(name) do
        ...
        {:ok, r_file_info(type: :symlink) = fi} ->
          add_verbose(opts, ~c"a ~ts~n", [nameInArchive])
  ...
  defp format_octal(octal) do
    :erlang.iolist_to_binary(:io_lib.fwrite(~c"~.8B", [octal]))
  end
  ...
  defp build_pax_entry(header, paxAttrs, opts) do
    ...
    path2 = :filename.join([dir, ~c"PaxHeaders.0", filename])
  ...
  defp build_pax_file([k | rest], attrs, acc) do
    ...
    cond do
      byte_size(record) !== size2 ->
        ...
        record2 = :io_lib.format(~c"~B ~ts=~ts\n", [size3, key, value])
    ...
    header1 =
      cond do
        ...
        true ->
          ...
          r_tar_header(header0, name: safe_join_path(~c"", name))
      end
  ...
  defp checksum(
         <<a::unsigned, b::unsigned, c::unsigned, d::unsigned, rest::binary>>,
         sum
       ) do
  ...
  defp do_fileinfo_to_header(
    ...
    r_tar_header(header, name: name ++ ~c"/", typeflag: ?5)
  end
  ...
  defp write_extracted_element(
    ...
    case typeflag(type) do
      :regular ->
        read_verbose(opts, ~c"x ~ts~n", [name])
        {:ok, {name, bin}}
  ...
  defp write_extracted_element(r_tar_header(name: name0) = header, bin, opts) do
    ...
    created =
      case typeflag(r_tar_header(header, :typeflag)) do
        ...
        :directory ->
          read_verbose(opts, ~c"x ~ts~n", [name0])
        ...
        other ->
          read_verbose(opts, ~c"x ~ts - unsupported type ~p~n", [name0, other])
          :not_written
      end
  ...
  defp create_regular(name, nameInArchive, bin, opts) do
    case write_extracted_file(name, bin, opts) do
      :not_written ->
        read_verbose(opts, ~c"x ~ts - exists, not created~n", [nameInArchive])
        :not_written

      ok ->
        read_verbose(opts, ~c"x ~ts~n", [nameInArchive])
        ok
    end
  end
  ...
  defp create_symlink(name, linkname, opts) do
    case :file.make_symlink(linkname, name) do
      ...
      {:error, :enotsup} ->
        read_verbose(opts, ~c"x ~ts - symbolic links not supported~n", [name])
        :not_written
  ...
  defp make_dirs(name, :dir) do
    :filelib.ensure_dir(:filename.join(name, ~c"*"))
  end
