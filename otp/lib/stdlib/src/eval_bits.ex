defmodule :m_eval_bits do
  ...
  defp expr_grp1([field | fS], bs0, evalFun, errorFun, listOfEvalField, pos) do
    {evalField, bs} = eval_field(field, bs0, evalFun, errorFun, pos)
    expr_grp1(fS, bs, evalFun, errorFun, [evalField | listOfEvalField], pos + 1)
  end

  defp expr_grp1([], bs, _EvalFun, _ErrorFun, listOfFieldData, _Pos) do
  ...
  defp eval_field({:bin_element, anno, {:string, _, s}, size0, options0}, bs0, fun, errorFun, pos) do
    {size1, [type, {:unit, unit}, sign, endian]} = make_bit_type(anno, size0, options0, errorFun)
    {:value, size, bs1} = fun.(size1, bs0)

    {fn ->
       res =
         for c <- s, into: <<>> do
           <<eval_exp_field1(anno, c, size, unit, type, endian, sign, errorFun, pos)::bitstring>>
         end
  ...
  defp eval_field({:bin_element, anno, e, size0, options0}, bs0, fun, errorFun, pos) do
  ...
  defp eval_exp_field1(anno, v, size, unit, type, endian, sign, errorFun, pos) do
    try do
      eval_exp_field(v, size, unit, type, endian, sign)
    catch
      :error, :system_limit ->
        errorFun.(anno, :system_limit, add_eval_pos_to_error_info(__STACKTRACE__, pos))

      :error, _ ->
        errorFun.(anno, :badarg, add_eval_pos_to_error_info(__STACKTRACE__, pos))
    end
  end

  defp add_eval_pos_to_error_info(
         [{mod, fun, arity, meta0} = head, _Skip | rest],
         pos
       ) do
    case :lists.keytake(:error_info, 1, meta0) do
      {:value, {:error_info, %{} = info}, meta1} ->
        meta2 = meta1 ++ [{:error_info, Map.put(info, :override_segment_position, pos)}]
  ...
  def bin_gen({:bin, _, fs}, bin, bs0, bBs0, mfun, efun) do
    bin_gen(fs, bin, bs0, bBs0, mfun, efun, &default_error/3, true)
  end

  def bin_gen({:bin, _, fs}, bin, bs0, bBs0, mfun, efun, errorFun) do
    bin_gen(fs, bin, bs0, bBs0, mfun, efun, errorFun, true)
  end

  defp bin_gen([f | fs], bin, bs0, bBs0, mfun, efun, errorFun, flag)
       when is_function(mfun, 2) and is_function(efun, 2) do
    case bin_gen_field(f, bin, bs0, bBs0, mfun, efun, errorFun) do
      {:match, bs, bBs, rest} ->
  ...
  defp bin_gen([], bin, bs0, _BBs0, _Mfun, _Efun, _ErrorFun, true) do
    {:match, bin, bs0}
  end

  defp bin_gen([], bin, _Bs0, _BBs0, _Mfun, _Efun, _ErrorFun, false) do
    {:nomatch, bin}
  end
  ...
  defp bin_gen_field(
         {:bin_element, anno, {:string, sAnno, s}, size0, options0},
         bin0,
         bs0,
         bBs0,
         mfun,
    ...
    case (try do
      ...
      {:value, size, _BBs} ->
        f = fn c, bin, bs, bBs ->
          bin_gen_field1(bin, type, size, unit, sign, endian, {:integer, sAnno, c}, bs, bBs, mfun)
        end
  ...
  defp bin_gen_field(
         {:bin_element, anno, vE, size0, options0},
         bin,
         bs0,
         bBs0,
         mfun,
  ...
  def match_bits(fs, bin, bs0, bBs, mfun, efun) do
    match_bits(fs, bin, bs0, bBs, mfun, efun, &default_error/3)
  end
  ...
  defp match_bits_1([f | fs], bits0, bs0, bBs0, mfun, efun, errorFun) do
  ...
  defp match_field_1(
    ...
    f = fn c, bin, bs, bBs ->
      match_field(bin, type, size, unit, sign, endian, {:integer, sAnno, c}, bs, bBs, mfun)
    end
