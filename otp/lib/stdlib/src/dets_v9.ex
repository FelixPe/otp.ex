defmodule :m_dets_v9 do
  ...
  Record.defrecord(:r_cache, :cache, cache: :undefined,
                                 ...
                                 delay: :undefined)
  Record.defrecord(:r__hash2, :"$hash2", file_format_version: :undefined,
                               bchunk_format_version: :undefined,
                               file: :undefined, type: :undefined,
                               keypos: :undefined, hash_method: :undefined,
                               n: :undefined, m: :undefined, next: :undefined,
                               min: :undefined, max: :undefined,
                               no_objects: :undefined, no_keys: :undefined,
                               no_colls: :undefined)
  def mark_dirty(head) do
    dirty = [{8, <<0 :: size(32)>>}]
    {_H, :ok} = :dets_utils.pwrite(head, dirty)
    :ok = :dets_utils.sync(head)
    {:ok, _Pos} = :dets_utils.position(head,
                                         r_head(head, :freelists_p))
    :dets_utils.truncate(head, :cur)
  end

  def prep_table_copy(fd, tab, fname, type, kp, ram, cacheSz, auto,
           parms) do
    case (parms) do
      r__hash2(file_format_version: 9, bchunk_format_version: 1,
            n: n, m: m, next: next, min: min, max: max,
            hash_method: hashMethodCode, no_objects: noObjects,
            no_keys: noKeys, no_colls: _NoColls)
        ...
        case (hash_invars(n, m, next, min, max)) do
          ...
          true ->
            init_file(fd, tab, fname, type, kp, min, max, ram,
                        cacheSz, auto, false, m, n, next, hashMethod, noObjects,
                        noKeys)
        end
      _ ->
        throw(:badarg)
  ...
  def initiate_file(fd, tab, fname, type, kp, minSlots0, maxSlots0,
           ram, cacheSz, auto, doInitSegments) do
    ...
    m = (next = minSlots)
    n = 0
    init_file(fd, tab, fname, type, kp, minSlots, maxSlots,
                ram, cacheSz, auto, doInitSegments, m, n, next, :phash2,
                0, 0)
  end

  defp init_file(fd, tab, fname, type, kp, minSlots, maxSlots,
            ram, cacheSz, auto, doInitSegments, m, n, next,
            hashMethod, noObjects, noKeys) do
    ftab = :dets_utils.init_alloc(56 + 28 * 4 + 16 + 4 + 124 + 4 * 256)
    head0 = r_head(m: m, m2: m * 2, next: next, fptr: fd,
                no_objects: noObjects, no_keys: noKeys, maxobjsize: 0,
                n: n, type: type, update_mode: :dirty, freelists: ftab,
                no_collections: :orddict.new(), auto_save: auto,
                hash_bif: hashMethod, has_md5: true, keypos: kp,
                min_no_slots: minSlots, max_no_slots: maxSlots,
                ram_file: ram, filename: fname, name: tab,
                cache: :dets_utils.new_cache(cacheSz), bump: 16,
                base: 56 + 28 * 4 + 16 + 4 + 124 + 4 * 256)
    freeListsPointer = 0
    noColls = <<0 :: size(28 * 4) - unit(8)>>
    fileHeader = file_header(head0, freeListsPointer, 0,
                               noColls)
    w0 = {0,
            [fileHeader | <<0 :: size(4 * 256) - unit(8)>>]}
    :lists.foreach(fn {i1, i2} when (is_integer(i1) and
                                       is_integer(i2))
                                    ->
                        :ok
                      {k, v} ->
                     ...
                     :erlang.erase())
    zero = seg_zero()
    {head1, ws1} = init_parts(head0, 0, no_parts(next),
                                zero, [])
    noSegs = no_segs(next)
    {head2, wsI, wsP} = init_segments(head1, 0, noSegs,
                                        zero, [], [])
    ws2 = (cond do
             doInitSegments ->
               wsP ++ wsI
    ...
    :dets_utils.pwrite(fd, fname,
                         [w0 | :lists.append(ws1) ++ ws2])
  ...
  defp init_segments(head, segNo, noSegs, segZero, wsP, wsI)
    ...
    {newHead, wI, ws} = allocate_segment(head, segZero,
                                           segNo)
    init_segments(newHead, segNo + 1, noSegs, segZero,
                    ws ++ wsP, [wI | wsI])
  end
  ...
  defp allocate_segment(head, segZero, segNo) do
    ...
    case (get_arrpart(partPos)) do
      :undefined ->
        {head1, [initArrPart, arrPartPointer],
           part} = alloc_part(head, segZero, partPos)
        {newHead, initSegment, [segPointer]} = alloc_seg(head1,
                                                           segZero, segNo, part)
        {newHead, initSegment,
  ...
  defp alloc_part(head, partZero, partPos) do
    {newHead, part, _} = :dets_utils.alloc(head,
                                             adjsz(4 * 512))
  ...
  defp alloc_seg(head, segZero, segNo, part) do
    {newHead, segment, _} = :dets_utils.alloc(head,
                                                adjsz(4 * 512))
  ...
  def read_file_header(fd, fileName) do
    {:ok, bin} = :dets_utils.pread_close(fd, fileName, 0,
                                           56 + 28 * 4 + 16 + 4)
    <<freeList :: size(32), cookie :: size(32),
        cP :: size(32), type2 :: size(32), version :: size(32),
        m :: size(32), next :: size(32), kp :: size(32),
        noObjects :: size(32), noKeys :: size(32),
        minNoSlots :: size(32), maxNoSlots :: size(32),
        hashMethod :: size(32), n :: size(32),
        noCollsB :: size(28 * 4) - binary,
        mD5 :: size(16) - binary, flBase :: size(32)>> = bin
    <<_ :: size(12) - binary,
        mD5DigestedPart
        ::
        size(56 + 28 * 4 + 16 + 4 - 16 - 4 - 12) - binary,
        _ :: binary>> = bin
    {:ok, eOF} = :dets_utils.position_close(fd, fileName,
                                              :eof)
    {:ok,
       <<fileSize :: size(32)>>} = :dets_utils.pread_close(fd,
                                                             fileName, eOF - 4,
                                                             4)
    {cL, <<>>} = :lists.foldl(fn lSz,
                                   {acc, <<nN :: size(32), r :: binary>>} ->
                                   cond do
                                     nN === 0 ->
                                       {acc, r}
                                     true ->
    ...
    fH = r_fileheader(freelist: freeList, fl_base: base,
             cookie: cookie, closed_properly: cP,
             type: :dets_utils.code_to_type(type2), version: version,
             m: m, next: next, keypos: kp, no_objects: noObjects,
             no_keys: noKeys, min_no_slots: minNoSlots,
             max_no_slots: maxNoSlots, no_colls: noColls,
             hash_method: hashMethod, read_md5: mD5,
  ...
  def check_file_header(fH, fd) do
    ...
    test = (cond do
              r_fileheader(fH, :cookie) !== 11259375 ->
    ...
    case (test) do
      :ok ->
        ...
        h = r_head(m: r_fileheader(fH, :m), m2: r_fileheader(fH, :m) * 2,
                next: r_fileheader(fH, :next), fptr: fd,
                no_objects: r_fileheader(fH, :no_objects),
                no_keys: r_fileheader(fH, :no_keys), maxobjsize: maxObjSize,
                n: r_fileheader(fH, :n), type: r_fileheader(fH, :type), update_mode: :saved,
                auto_save: :infinity, fixed: false,
                freelists_p: r_fileheader(fH, :freelist), hash_bif: hashBif,
                has_md5: r_fileheader(fH, :has_md5), keypos: r_fileheader(fH, :keypos),
                min_no_slots: r_fileheader(fH, :min_no_slots),
                max_no_slots: r_fileheader(fH, :max_no_slots),
                no_collections: r_fileheader(fH, :no_colls), bump: 16,
  ...
  def cache_segps(fd, fileName, m) do
    ...
    {:ok, bin} = :dets_utils.pread_close(fd, fileName,
                                           arrStart, 4 * noParts)
    cache_arrparts(bin, 56 + 28 * 4 + 16 + 4 + 124, fd,
                     fileName)
  end

  defp cache_arrparts(<<arrPartPos :: size(32), b :: binary>>, pos,
            fd, fileName) do
    arrpart_cache(pos, arrPartPos)
    {:ok, arrPartBin} = :dets_utils.pread_close(fd,
                                                  fileName, arrPartPos, 512 * 4)
    ...
    fn :close ->
       ...
       :read ->
         case ((try do
           ...
           {^ref, :end_of_input} ->
             :end_of_input
           {^ref, {l0, newInitFun}} when (is_list(l0) and
  ...
  defp bulk_objects([t | ts], head, kp, seq, l) do
    ...
    bulk_objects(ts, head, kp, seq + 1,
                   [make_object(head, key, seq, bT) | l])
  end

  defp bulk_objects([], _Head, kp, seq, l) when (is_integer(kp) and
                                         is_integer(seq)) do
    {l, seq}
  end
  ...
  def output_objs(head, slotNums, cntrs) do
    fn :close ->
       ...
       l ->
         ...
         cache = {}
         {nE, nAcc, nCache} = output_slots(es, head, cache,
                                             cntrs, 0, 0)
  ...
  defp output_objs2(e, acc, head, cache, sizeT, slotNums, chunkI) do
    fn :close ->
         ...
         {newHead, nL, _MaxSz,
            _End} = allocate_all_objects(head1, sizeT)
       ...
       l ->
         es = bin2term(l, r_head(head, :keypos))
         {nE, nAcc, nCache} = output_slots(e, es, acc, head,
                                             cache, sizeT, 0, 0)
  ...
  def compact_init(readHead, writeHead, tableParameters) do
    ...
    {newWriteHead, bases, segAddr,
       segEnd} = prepare_file_init(noObjects, noKeys,
                                     noObjsPerSize, sizeT, writeHead)
    input = compact_input(readHead, newWriteHead, sizeT,
                            tuple_size(bases))
    output = fast_output(newWriteHead, sizeT, bases,
                           segAddr, segEnd)
  ...
  defp compact_input(head, wHead, sizeT, noSizes) do
    ...
    cache = :erlang.list_to_tuple(:erlang.tuple_to_list({}) ++ :lists.duplicate(noSizes - tuple_size({}),
                                                                                  [0]))
  ...
  defp compact_read(_Head, wHead, sizeT, cache, [], _Min, [],
            _ASz) do
  ...
  defp compact_read(head, wHead, sizeT, cache, [[from | to] | l],
            min, segBs, aSz) do
      ...
      true ->
        case (:dets_utils.pread_n(r_head(head, :fptr), from, max)) do
          ...
          bin1 when byte_size(bin1) < min ->
            ...
            compact_objs(head, wHead, sizeT, newBin, l, from, to,
                           segBs, cache, aSz)
          newBin ->
            compact_objs(head, wHead, sizeT, newBin, l, from, to,
                           segBs, cache, aSz)
  ...
  defp compact_objs(head, wHead, sizeT, bin, l, from, to, segBs,
            cache, aSz)
      when from === to do
    case (l) do
      [] ->
  ...
  defp compact_objs(head, wHead, sizeT,
            <<size :: size(32), st :: size(32), _Sz :: size(32),
                kO :: binary>> = bin,
            l, from, to, segBs, cache, aSz)
      when st === 305419896 do
    lSize = sz2pos(size)
    size2 = 1 <<< (lSize - 1)
    cond do
      byte_size(bin) >= size2 ->
        nASz = aSz + size2
        <<slotObjs :: size(size2) - binary,
            newBin :: binary>> = bin
        term = (cond do
        ...
        nCache = :erlang.setelement(lSize, cache,
                                      [addr + size2, slotObjs | aL])
        nSegBs = [<<slot :: size(32), size :: size(32),
                      addr :: size(32), lSize :: size(8)>> |
                      segBs]
        compact_objs(head, wHead, sizeT, newBin, l, from1, to,
                       nSegBs, nCache, nASz)
      true ->
        compact_read(head, wHead, sizeT, cache,
                       [[from | to] | l], size2, segBs, aSz)
    end
  end

  defp compact_objs(head, wHead, sizeT,
            <<_ :: size(32), _St :: size(32), _ :: size(32),
                _ :: binary>> = bin,
            l, from, to, segBs, cache, aSz)
      when byte_size(bin) >= 512 * 4 do
    <<_ :: size(512 * 4) - binary, newBin :: binary>> = bin
    compact_objs(head, wHead, sizeT, newBin, l,
                   from + 512 * 4, to, segBs, cache, aSz)
  end
  ...
  defp compact_objs(head, wHead, sizeT,
      ...
      when byte_size(bin) < 512 * 4 do
    compact_read(head, wHead, sizeT, cache,
                   [[from | to] | l], 512 * 4, segBs, aSz)
  end

  defp compact_objs(head, wHead, sizeT, _Bin, l, from, to, segBs,
            cache, aSz) do
    compact_read(head, wHead, sizeT, cache,
                   [[from | to] | l], 0, segBs, aSz)
  end
  ...
  defp read_bchunks(head, {from, to, l}, min, bs, aSz) do
    ...
    case (check_pread_arg(max, head)) do
      ...
      false ->
        {:error,
           :dets_utils.bad_object(:bad_object,
                                    {:read_bchunks, max})}
    end
  end
  ...
  defp bchunks(head, l,
            <<size :: size(32), st :: size(32), _Sz :: size(32),
                kO :: binary>> = bin,
            bs, aSz, from, to)
      when st === 305419896 or st === 61591023 do
    lSize = sz2pos(size)
    size2 = 1 <<< (lSize - 1)
    cond do
      byte_size(bin) >= size2 ->
        ...
        slot = db_hash(key, head)
        b = {lSize, slot, b0}
        bchunks(head, l, newBin, [b | bs], aSz + size2,
                  from + size2, to)
      true ->
  ...
  def bchunk_init(head, initFun) do
    ...
    case ((try do
      ...
      {^ref, {[parmsBin | l], nInitFun}} when (is_list(l) and
                                                 is_function(nInitFun))
                                              ->
        r_head(fptr: fd, type: type, keypos: kp, auto_save: auto,
            cache: cache, filename: fname, ram_file: ram,
            name: tab) = head
        case (try_bchunk_header(parmsBin, head)) do
          {:ok, parms} ->
            r__hash2(no_objects: noObjects, no_keys: noKeys,
                  no_colls: noObjsPerSize) = parms
            cacheSz = :dets_utils.cache_size(cache)
            {:ok, head1} = prep_table_copy(fd, tab, fname, type, kp,
                                             ram, cacheSz, auto, parms)
            sizeT = :ets.new(:dets_init, [])
            {newHead, bases, segAddr,
               segEnd} = prepare_file_init(noObjects, noKeys,
                                             noObjsPerSize, sizeT, head1)
            eCache = :erlang.list_to_tuple(:erlang.tuple_to_list({}) ++ :lists.duplicate(tuple_size(bases) - tuple_size({}),
                                                                                           [0]))
            input = fn :close ->
                         _ = ((try do
                                nInitFun.(:close)
                       ...
                       :read ->
                         do_make_slots(l, eCache, sizeT, newHead, ref, 0,
                                         nInitFun)
                    end
            output = fast_output(newHead, sizeT, bases, segAddr,
                                   segEnd)
            tmpDir = :filename.dirname(r_head(head, :filename))
            reply = ((try do
                       :file_sorter.sort(input, output,
                                           [{:format, :binary}, {:tmpdir,
                                                                   tmpDir},
                                                                    {:header,
                                                                       1}])
                     catch
  ...
  def try_bchunk_header(parmsBin, head) do
    ...
    case ((try do
      ...
      parms when (elem(parms, 0) === :"$hash2" and
       ...
       :read ->
         case ((try do
           ...
           {^ref, :end_of_input} ->
             ...
             :end_of_input
           {^ref, {l, nInitFun}} when (is_list(l) and
                                         is_function(nInitFun))
                                      ->
  ...
  defp do_make_slots(l, cache, sizeT, head, ref, aSz, initFun) do
    case ((try do
      ...
      {cache1, segBs, nASz} when nASz > 60 * 8192 ->
        ...
        {segBs, f}
      {nCache, segBs, nASz} ->
        f = bchunk_input(initFun, sizeT, head, ref, nCache,
                           nASz)
        {segBs, f}
    end
  end

  defp make_slots([{lSize, slot,
              <<size :: size(32), st :: size(32), sz :: size(32),
                  kO :: binary>> = bin0} |
               bins],
            cache, segBs, aSz) do
    bin = (cond do
             st === 305419896 ->
               bin0
             st === 61591023 ->
               <<size :: size(32), 305419896 :: size(32),
                   sz :: size(32), kO :: binary>>
           end)
    bSz = byte_size(bin0)
    true = bSz === 1 <<< (lSize - 1)
    nASz = aSz + bSz
  ...
  defp fast_output_end(head, sizeT) do
    case (:ets.foldl(fn {_Sz, _Pos, cnt, noC}, acc ->
  ...
  defp write_segment_file([<<slot :: size(32), bSize :: size(32),
               addrToBe :: size(32), lSize :: size(8)>> |
               bins],
            bases, head, ws, segAddr, sS) do
    pos = sS + 2 * 4 * slot
    write_segment_file(bins, bases, head, ws, segAddr, sS,
                         pos, bSize, addrToBe, lSize)
  end
  ...
  defp write_segment_file(bins, bases, head, ws, segAddr, sS, pos, bSize,
            addrToBe, lSize)
      when pos === segAddr do
    addr = addrToBe + :erlang.element(lSize, bases)
    nWs = [ws | <<bSize :: size(32), addr :: size(32)>>]
    write_segment_file(bins, bases, head, nWs,
                         segAddr + 2 * 4, sS)
  end
  ...
  defp write_segment_file(bins, bases, head, ws, segAddr, sS, pos, bSize,
    ...
    addr = addrToBe + :erlang.element(lSize, bases)
    noZeros = pos - segAddr
    nWs = [ws | <<0 :: size(noZeros) - unit(8),
                    bSize :: size(32), addr :: size(32)>>]
  ...
  defp write_segment_file(bins, bases, head, ws, segAddr, sS, pos, bSize,
    ...
    addr = addrToBe + :erlang.element(lSize, bases)
    noZeros = pos - segAddr
    nWs = [ws, :dets_utils.make_zeros(noZeros) | <<bSize
                                                   ::
                                                   size(32),
                                                     addr :: size(32)>>]
    nSegAddr = segAddr + noZeros + 2 * 4
  ...
  defp fast_write_sizes([[_Addr] = c | cL], sz, sizeT, head, nCL,
            pwriteList) do
    fast_write_sizes(cL, sz - 1, sizeT, head, [c | nCL],
                       pwriteList)
  end

  defp fast_write_sizes([[addr | c] | cL], sz, sizeT, head, nCL,
            pwriteList) do
    case (:ets.lookup(sizeT, sz)) do
      ...
      [{^sz, position, _ObjCounter, _NoCollections}] ->
        ...
        fast_write_sizes(cL, sz - 1, sizeT, head,
                           [[addr] | nCL],
                           [{pos, :lists.reverse(c)} | pwriteList])
    end
  end

  defp prepare_file_init(noObjects, noKeys, noObjsPerSize, sizeT,
            head) do
    segSz = 512 * 4
    {_, segEnd, _} = :dets_utils.alloc(head, adjsz(segSz))
    head1 = r_head(head, no_objects: noObjects,  no_keys: noKeys)
    true = :ets.insert(sizeT, {1, 0, [], 0})
    :lists.foreach(fn {logSz, noColls} ->
                        true = :ets.insert(sizeT, {logSz + 1, 0, 0, noColls})
                   end,
                     noObjsPerSize)
    {newHead, nL0, maxSz,
       endOfFile} = allocate_all_objects(head1, sizeT)
    [{1, segAddr, [], 0} | nL] = nL0
    ...
    :lists.foreach(fn x ->
                     ...
                     nL)
    bases = :lists.foldl(fn {lSz, p, _D, _N}, a ->
                              :erlang.setelement(lSz, a, p)
                         end,
                           :erlang.make_tuple(maxSz, 0), nL)
    est = :lists.foldl(fn {lSz, _, _, n}, a ->
                            a + (1 <<< (lSz - 1)) * n
                       end,
                         0, nL)
  ...
  defp write_bytes(head, endOfFile, _Est) do
    ...
    sizeInKB = 64
    bin = :erlang.list_to_binary(:lists.duplicate(sizeInKB * 4,
                                                    :lists.seq(0, 255)))
  ...
  defp allocate_all(head, [{lSize, _, data, noCollections} | dTL],
            l) do
    size = 1 <<< (lSize - 1)
    {_Head, addr, _} = :dets_utils.alloc(head, adjsz(size))
    head1 = :dets_utils.alloc_many(head, size,
                                     noCollections, addr)
    noColls = r_head(head1, :no_collections)
    newNoColls = :orddict.update_counter(lSize - 1,
                                           noCollections, noColls)
  ...
  defp bin2term1([<<slot :: size(32), seq :: size(32),
               binTerm :: binary>> |
               bTs],
            kp, l) do
  ...
  defp write_all_sizes(cache, sizeT, head, more) do
    cacheL = :lists.reverse(:erlang.tuple_to_list(cache))
    sz = length(cacheL)
    nCL = (case (:ets.info(sizeT, :size)) do
             1 when more === :no_more ->
  ...
  defp write_sizes([c | cL], sz, sizeT, head) do
    {fileName, fd} = (case (:ets.lookup(sizeT, sz)) do
                        [] ->
  ...
  defp output_slots([e | es], head, cache, sizeT, noKeys, noObjs) do
    output_slots(e, es, [e], head, cache, sizeT, noKeys,
                   noObjs)
  end
  ...
  defp output_slots(e, [e1 | es], acc, head, cache, sizeT, noKeys,
            noObjs)
      when :erlang.element(1, e) === :erlang.element(1, e1) do
    output_slots(e1, es, [e1 | acc], head, cache, sizeT,
                   noKeys, noObjs)
  end

  defp output_slots(e, [], acc, _Head, cache, sizeT, noKeys,
            noObjs) do
    _ = :ets.update_counter(sizeT, :no, {2, noObjs})
    _ = :ets.update_counter(sizeT, :no, {3, noKeys})
    {e, acc, cache}
  ...
  defp output_slots(_E, l, acc, head, cache, sizeT, noKeys,
            noObjs) do
  ...
  defp output_slot(es, head, cache, l, sizeT, noKeys, noObjs) do
    ...
    binObject = [<<bSize :: size(32),
                     305419896 :: size(32)>>,
                     bins | pad]
    cache1 = (cond do
                lSize > tuple_size(cache) ->
                  c1 = :erlang.list_to_tuple(:erlang.tuple_to_list(cache) ++ :lists.duplicate(lSize - tuple_size(cache),
                                                                                                []))
    ...
    pBin = <<slot :: size(32), bSize :: size(32),
               lSize :: size(8)>>
  ...
  defp prep_slot([], head, w) do
    ...
    {[], bins, size, no, kNo, _} = eval_slot(wLs, [],
                                               r_head(head, :type), [], [], 0, 0, 0,
                                               false)
    {bins, size, no, kNo}
  end

  defp prep_set_slot([{_, k, _Seq, _T1, bT1} | l], k, _BT, sz,
            noKeys, noObjs, ws) do
    prep_set_slot(l, k, bT1, sz, noKeys, noObjs, ws)
  end

  defp prep_set_slot([{_, k1, _Seq, _T1, bT1} | l], _K, bT, sz,
            noKeys, noObjs, ws) do
    bSize = byte_size(bT) + 4
    nWs = [ws, <<bSize :: size(32)>> | bT]
    prep_set_slot(l, k1, bT1, sz + bSize, noKeys + 1,
                    noObjs + 1, nWs)
  end

  defp prep_set_slot([], _K, bT, sz, noKeys, noObjs, ws) do
    bSize = byte_size(bT) + 4
    {[ws, <<bSize :: size(32)>> | bT], sz + bSize,
       noKeys + 1, noObjs + 1}
  end
  ...
  defp segment_file(sizeT, head, fileData, segEnd) do
    ...
    newData = (case (data) do
                 {inFile, in0} ->
                   {outFile, out} = temp_file(head, sizeT, i)
                   _ = :file.close(in0)
                   {:ok, in__} = :dets_utils.open(inFile,
                                                    [:raw, :binary, :read])
                   {:ok, 0} = :dets_utils.position(in__, inFile, :bof)
                   seg_file(segAddr, segAddr, in__, inFile, out, outFile,
                              sizeT, segEnd)
                   _ = :file.close(in__)
                   _ = :file.delete(inFile)
                   {outFile, out}
                 objects ->
                   {lastAddr, b} = seg_file(objects, segAddr, segAddr,
                                              sizeT, [])
  ...
  defp seg_file(addr, sS, in__, inFile, out, outFile, sizeT,
            segEnd) do
    case (:dets_utils.read_n(in__, 4500)) do
      ...
      bin ->
        ...
        seg_file(newAddr, sS, in__, inFile, out, outFile, sizeT,
                   segEnd)
    end
  end

  defp seg_file(<<slot :: size(32), bSize :: size(32),
              lSize :: size(8), t :: binary>>,
            addr, sS, sizeT, l) do
  ...
  defp seg_file_item(t, addr, sS, sizeT, l, slot, bSize, lSize) do
    ...
    pointerBin = (cond do
                    ...
                    noZeros > 100 ->
                      [:dets_utils.make_zeros(noZeros) | <<bSize :: size(32),
                                                             collP
                                                             ::
                                                             size(32)>>]
                    true ->
                      <<0 :: size(noZeros) - unit(8), bSize :: size(32),
                          collP :: size(32)>>
                  end)
  ...
  def fsck_input(head, fd, cntrs, fileHeader) do
    maxSz0 = (case (r_fileheader(fileHeader, :has_md5)) do
    ...
    state0 = fsck_read(56 + 28 * 4 + 16 + 4 + 124 + 4 * 256,
                         fd, [], 0)
  ...
  defp fsck_read({:more, bin, sz, l, seq}, pos, f, _MaxSz,
            _Head) do
  ...
  defp read_more_bytes(b, min, pos, f, l, seq) do
    ...
    case (:dets_utils.read_n(f, max)) do
      ...
      bin ->
        ...
        {:cont, l, :erlang.list_to_binary([b, bin]), newPos,
           seq}
    end
  end

  defp fsck_objs(bin = <<sz :: size(32), status :: size(32),
                    tail :: binary>>,
            kp, head, l, seq) do
    cond do
      status === 305419896 ->
  ...
  def do_perform_save(h) do
    ...
    adjustedFileSize = (case (r_head(h, :base)) do
                          1336 ->
                            fileSize
                          base ->
                            fileSize - base
                        end)
    :ok = :dets_utils.write(h1,
    ...
    case (:dets_utils.debug_mode()) do
      true ->
        ...
        case ((try do
                :dets_utils.all_allocated_as_list(tmpHead) === :dets_utils.all_allocated_as_list(h1)
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                :exit, e -> {:EXIT, e}
                e -> e
  ...
  defp file_header(head, freeListsPointer, closedProperly) do
    noColls = (case (r_head(head, :no_collections)) do
                 :undefined ->
                   []
                 nC ->
                   nC
               end)
    l = :orddict.merge(fn _K, v1, v2 ->
                            v1 + v2
                       end,
                         noColls,
                         :lists.map(fn x ->
  ...
  defp file_header(head, freeListsPointer, closedProperly,
            noColls) do
    cookie = 11259375
    typeCode = :dets_utils.type_to_code(r_head(head, :type))
    version = 9
    hashMethod = hash_method_to_code(r_head(head, :hash_bif))
    h1 = <<freeListsPointer :: size(32), cookie :: size(32),
             closedProperly :: size(32)>>
    h2 = <<typeCode :: size(32), version :: size(32),
             r_head(head, :m) :: size(32), r_head(head, :next) :: size(32),
             r_head(head, :keypos) :: size(32),
             r_head(head, :no_objects) :: size(32),
             r_head(head, :no_keys) :: size(32),
             r_head(head, :min_no_slots) :: size(32),
             r_head(head, :max_no_slots) :: size(32),
             hashMethod :: size(32), r_head(head, :n) :: size(32)>>
    digH = [h2 | noColls]
    mD5 = (case (r_head(head, :has_md5)) do
             true ->
               :erlang.md5(digH)
             false ->
               <<0 :: size(16) - unit(8)>>
           end)
    base = (case (r_head(head, :base)) do
              1336 ->
                <<0 :: size(32)>>
              flBase ->
                <<flBase :: size(32)>>
  ...
  defp free_list_to_file(_Ftab, _H, pos, sz, ws, wsSz) when pos > sz do
    {[ws | <<4 + 8 :: size(32), 61591023 :: size(32),
               12345 :: size(32)>>],
       wsSz + 4 + 8}
  end
  ...
  defp free_list_to_file(ftab, h, pos, sz, ws, wsSz) do
    ...
    f = fn n, l, w, s when n === 0 ->
           ...
           n, l, w, s ->
             ...
             size = n1 * 4 + 4 + 8
             header = <<size :: size(32), 61591023 :: size(32),
                          pos :: size(32)>>
             nW = [w, header | l1]
             case (more) do
               :no_more ->
    ...
    {nWs,
       nWsSz} = :dets_utils.tree_to_bin(:erlang.element(pos,
                                                          ftab),
                                          f, max, ws, wsSz)
  ...
  defp free_lists_from_file(h, pos) do
    {:ok, ^pos} = :dets_utils.position(r_head(h, :fptr),
                                         r_head(h, :filename), pos)
  ...
  defp bin_to_tree(bin, h, lastPos, ftab, a0, l) do
    case (bin) do
      <<_Size :: size(32), 61591023 :: size(32),
          12345 :: size(32), _ :: binary>>
          when l === [] ->
        ftab
      <<_Size :: size(32), 61591023 :: size(32),
          12345 :: size(32), _ :: binary>> ->
        :erlang.setelement(lastPos, ftab,
                             :dets_utils.list_to_tree(l))
      <<size :: size(32), 61591023 :: size(32),
          pos :: size(32), t :: binary>>
          when byte_size(t) >= size - 4 - 8 ->
        {nFtab, l1, a1} = (cond do
                             (pos !== lastPos and lastPos !== :start) ->
                               tree = :dets_utils.list_to_tree(l)
                               {:erlang.setelement(lastPos, ftab, tree), [],
                                  - 1}
                             true ->
                               {ftab, l, a0}
                           end)
      ...
      _ ->
        ...
        bin_to_tree(:erlang.list_to_binary([bin | bin2]), h,
                      lastPos, ftab, a0, l)
    end
  end

  defp bin_to_tree1(<<a1 :: size(32), a2 :: size(32),
              a3 :: size(32), a4 :: size(32), t :: binary>>,
            size, a, l)
  ...
  def no_slots(head) do
    {r_head(head, :min_no_slots), r_head(head, :next),
       r_head(head, :max_no_slots)}
  end
    ...
    case (r_head(head, :no_collections)) do
      ...
      cL ->
        ...
        r__hash2(file_format_version: 9, bchunk_format_version: 1,
              ...
              type: r_head(head, :type), keypos: r_head(head, :keypos),
              hash_method: hash_method_to_code(r_head(head, :hash_bif)),
              n: r_head(head, :n), m: r_head(head, :m), next: r_head(head, :next),
              min: r_head(head, :min_no_slots),
              max: r_head(head, :max_no_slots),
              no_objects: r_head(head, :no_objects),
              no_keys: r_head(head, :no_keys), no_colls: noColls)
    end
  end
  ...
  defp re_hash(head, slotStart) do
    ...
    split_bins(fromBin, head, fromSlotPos, toSlotPos, [],
                 [], 0)
  end
  ...
  defp split_bins(fB, head, pos1, pos2, toRead, l, soFar) do
    ...
    cond do
      (nSoFar > 10 * 8192 and toRead !== []) ->
        {newHead, :ok} = re_hash_write(head, toRead, l, pos1,
                                         pos2)
        split_bins(fB, newHead, pos1, pos2, [], [], 0)
      sz1 === 0 ->
        e = {:skip, b1}
        split_bins(fT, head, nPos1, nPos2, toRead, [e | l],
                     nSoFar)
      true ->
        e = {sz1, p1, b1, pos1, pos2}
        newToRead = [{p1, sz1} | toRead]
        split_bins(fT, head, nPos1, nPos2, newToRead, [e | l],
                     nSoFar)
    end
  end
  ...
  defp re_hash_write(head, toRead, l, pos1, pos2) do
    ...
    {head1, binFS, binTS, wsB} = re_hash_slots(bins, l,
                                                 head, z, [], [], [])
  ...
  defp re_hash_slots(bins, [{:skip, b1} | l], head, z, binFS, binTS,
            wsB) do
    re_hash_slots(bins, l, head, z, [b1 | binFS],
                    [z | binTS], wsB)
  end

  defp re_hash_slots([fB | bins], [e | l], head, z, binFS, binTS,
            wsB) do
    ...
    case (re_hash_split(keyObjs, head, [], 0, [], 0)) do
      ...
      {[], 0, _ML, _MSz} ->
        ^sz1 = _MSz + 8
        re_hash_slots(bins, l, head, z, [z | binFS],
                        [b1 | binTS], wsB)
      {kL, kSz, mL, mSz} when (kL !== [] and kSz > 0 and
                                 mL !== [] and mSz > 0)
                              ->
        {head1, fS1, ws1} = updated(head, p1, sz1, kSz, pos1,
                                      kL, true, :foo, :bar)
        {newHead, [{^pos2, bin2}], ws2} = updated(head1, 0, 0,
                                                    mSz, pos2, mL, true, :foo,
                                                    :bar)
        newBinFS = (case (fS1) do
                      [{^pos1, bin1}] ->
                        [bin1 | binFS]
                      [] ->
                        [b1 | binFS]
  ...
  defp re_hash_split([e | keyObjs], head, kL, kSz, mL, mSz) do
    ...
    cond do
      new >= r_head(head, :m) ->
        re_hash_split(keyObjs, head, kL, kSz, [bin | mL],
                        mSz + sz)
      true ->
        re_hash_split(keyObjs, head, [bin | kL], kSz + sz, mL,
                        mSz)
    end
  end
  ...
  def write_cache(head) do
    ...
    case (:dets_utils.is_empty_cache(c)) do
      ...
      false ->
        ...
        maxNoInsertedKeys = :erlang.min(maxInserts,
                                          length(perKey))
  ...
  defp grow(head, extra, segZero) do
    ...
    segNum = div(next, 256)
    {head0, w, ws1} = allocate_segment(head, segZero,
                                         segNum)
  ...
  defp hash_invars(h) do
    hash_invars(r_head(h, :n), r_head(h, :m), r_head(h, :next),
                  r_head(h, :min_no_slots), r_head(h, :max_no_slots))
  end

  defp hash_invars(n, m, next, min, max) do
    :erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(:erlang.and(n &&& (256 - 1) === 0,
                                                                                                                                                                  m &&& (256 - 1) === 0),
                                                                                                                                                      next &&& (256 - 1) === 0),
                                                                                                                                          min &&& (256 - 1) === 0),
                                                                                                                              max &&& (256 - 1) === 0),
  ...
  defp eval_work_list(head, [{key, [{_Seq, {:lookup, pid}}]}]) do
    ...
    maxSize = maxobjsize(head)
    objs = (case (:dets_utils.ipread(head, slotPos,
                                       maxSize)) do
  ...
  defp eval_work_list(head, perKey) do
    ...
    read_buckets(perSlot, slotPositions, bins, head, [], [],
                   [], [], 0, 0, 0)
  end
  ...
  defp read_buckets([wLs | sPs], [{p1, _8} | ss],
            [<<_Zero :: size(32), p2 :: size(32)>> | bs], head,
            pWLs, toRead, lU, ws, noObjs, noKeys, soFar)
      when p2 === 0 do
    {newHead, nLU, nWs, no, kNo} = eval_bucket_keys(wLs, p1,
                                                      0, 0, [], head, ws, lU)
    newNoObjs = no + noObjs
    newNoKeys = kNo + noKeys
    read_buckets(sPs, ss, bs, newHead, pWLs, toRead, nLU,
                   nWs, newNoObjs, newNoKeys, soFar)
  end

  defp read_buckets([workLists | sPs], [{p1, _8} | ss],
            [<<size :: size(32), p2 :: size(32)>> | bs], head, pWLs,
            toRead, lU, ws, noObjs, noKeys, soFar)
      when soFar + size < 10 * 8192 or toRead === [] do
    newToRead = [{p2, size} | toRead]
    newPWLs = [{p2, p1, workLists} | pWLs]
    newSoFar = soFar + size
    read_buckets(sPs, ss, bs, head, newPWLs, newToRead, lU,
                   ws, noObjs, noKeys, newSoFar)
  end

  defp read_buckets(sPs, ss, bs, head, pWLs0, toRead0, lU, ws,
            noObjs, noKeys, soFar)
      when soFar > 0 do
    ...
    case ((try do
      ...
      {:ok, head1, nLU, nWs, no, kNo} ->
        ...
        {newHead, :ok} = :dets_utils.pwrite(head1,
                                              :lists.reverse(nWs))
        read_buckets(sPs, ss, bs, newHead, [], [], nLU, [],
                       newNoObjs, newNoKeys, 0)
      error ->
        bad = :dets_utils.bad_object(:read_buckets,
                                       {bins, error})
        throw(:dets_utils.corrupt_reason(head, bad))
    end
  end

  defp read_buckets([], [], [], head, [], [], lU, ws, noObjs,
            noKeys, 0) do
    {newHead, nWs} = update_no_keys(head, ws, noObjs,
                                      noKeys)
    {newHead, lU, :lists.reverse(nWs)}
  end

  defp eval_buckets([bin | bins], [sP | sPs], head, lU, ws, noObjs,
            noKeys) do
    {pos, p1, wLs} = sP
    ...
    {newHead, nLU, nWs, no, kNo} = eval_bucket_keys(wLs, p1,
                                                      pos, byte_size(bin),
                                                      keyObjs, head, ws, lU)
    eval_buckets(bins, sPs, newHead, nLU, nWs, noObjs + no,
                   noKeys + kNo)
  end
  ...
  defp eval_bucket_keys(wLs, slotPos, pos, oldSize, keyObjs, head, ws,
            lU) do
    {nLU, bins, bSize, no, kNo, ch} = eval_slot(wLs,
                                                  keyObjs, r_head(head, :type), lU,
                                                  [], 0, 0, 0, false)
    {newHead, w1, w2} = updated(head, pos, oldSize, bSize,
                                  slotPos, bins, ch, no, kNo)
    {newHead, nLU, w2 ++ w1 ++ ws, no, kNo}
  end

  defp updated(head, pos, oldSize, bSize, slotPos, bins, ch,
            deltaNoOs, deltaNoKs) do
    binsSize = bSize + 8
    cond do
      (pos === 0 and bSize === 0) ->
        {head, [], []}
      (pos === 0 and bSize > 0) ->
        {head1, newPos, fPos} = :dets_utils.alloc(head,
                                                    adjsz(binsSize))
        newHead = one_bucket_added(head1, fPos - 1)
        w1 = {newPos,
                [<<binsSize :: size(32), 305419896 :: size(32)>> |
                     bins]}
        w2 = {slotPos,
                <<binsSize :: size(32), newPos :: size(32)>>}
        {newHead, [w2], [w1]}
      (pos !== 0 and bSize === 0) ->
        {head1, fPos} = :dets_utils.free(head, pos,
                                           adjsz(oldSize))
        newHead = one_bucket_removed(head1, fPos - 1)
        w1 = {pos + 4, <<61591023 :: size(32)>>}
      ...
      (pos !== 0 and bSize > 0) ->
        ...
        overwrite = (cond do
                       r_head(head, :fixed) !== false ->
                         :erlang.and(:erlang.and(overwrite0 !== false,
                                                   deltaNoOs === 0),
                                       deltaNoKs === 0)
                       true ->
                         overwrite0
          ...
          overwrite ->
            w1 = {pos,
                    [<<binsSize :: size(32), 305419896 :: size(32)>> |
                         bins]}
            w2 = {slotPos,
                    <<binsSize :: size(32), pos :: size(32)>>}
            {head, [w2], [w1]}
          true ->
            {head1, fPosF} = :dets_utils.free(head, pos,
                                                adjsz(oldSize))
            {head2, newPos, fPosA} = :dets_utils.alloc(head1,
                                                         adjsz(binsSize))
  ...
  defp one_bucket_added(h, log2) when r_head(h, :maxobjsize) >= log2 do
    newNoColls = :orddict.update_counter(log2, 1,
                                           r_head(h, :no_collections))
  ...
  defp eval_slot([{key, commands} | wLs] = wLs0,
            [{k, kS, kB, o, os} | kOs1] = kOs, type, lU, ws, no,
            kNo, bSz, ch) do
    case (:dets_utils.cmp(k, key)) do
      0 ->
        old = [o | binobjs2terms(os)]
        {nLU, nWs, sz, no1, kNo1, nCh} = eval_key(key, commands,
                                                    old, type, kB, kS, lU, ws,
                                                    ch)
        eval_slot(wLs, kOs1, type, nLU, nWs, no1 + no,
                    kNo1 + kNo, sz + bSz, nCh)
      -1 ->
        eval_slot(wLs0, kOs1, type, lU, [ws | kB], no, kNo,
                    kS + bSz, ch)
      1 ->
        {nLU, nWs, sz, no1, kNo1, nCh} = eval_key(key, commands,
                                                    [], type, [], 0, lU, ws, ch)
  ...
  defp eval_slot([{key, commands} | wLs], [], type, lU, ws, no,
            kNo, bSz, ch) do
    {nLU, nWs, sz, no1, kNo1, nCh} = eval_key(key, commands,
                                                [], type, [], 0, lU, ws, ch)
    eval_slot(wLs, [], type, nLU, nWs, no1 + no, kNo1 + kNo,
                sz + bSz, nCh)
  end

  defp eval_slot([], [{_Key, size, keyBin, _, _} | kOs], type,
            lU, ws, no, kNo, bSz, ch) do
    eval_slot([], kOs, type, lU, [ws | keyBin], no, kNo,
                size + bSz, ch)
  end
  ...
  defp eval_key(_K, [{_Seq, {:lookup, pid}}], [], _Type,
            _KeyBin, _KeySz, lU, ws, ch) do
    nLU = [{pid, []} | lU]
    {nLU, ws, 0, 0, 0, ch}
  end

  defp eval_key(_K, [{_Seq, {:lookup, pid}}], old0, _Type,
            keyBin, keySz, lU, ws, ch) do
  ...
  defp eval_key(k, comms, orig, type, keyBin, keySz, lU, ws,
            ch) do
    old = :dets_utils.msort(orig)
    case (eval_key1(comms, [], old, type, k, lU, ws, 0,
                      orig)) do
      {:ok, nLU} when old === [] ->
        {nLU, ws, 0, 0, 0, ch}
      {:ok, nLU} ->
  ...
  defp eval_key1([{_Seq, {:insert, term}} | l], cs,
            [{term, _, _}] = old, type = :set, k, lU, ws, no,
            orig) do
    eval_key1(l, cs, old, type, k, lU, ws, no, orig)
  end

  defp eval_key1([{seq, {:insert, term}} | l], cs, old,
            type = :set, k, lU, ws, no, orig) do
    nNo = no + 1 - length(old)
    eval_key1(l, cs, [{term, seq, :insert}], type, k, lU,
                ws, nNo, orig)
  end

  defp eval_key1([{_Seq, {:lookup, pid}} | l], cs, old, type,
            key, lU, ws, no, orig) do
    ...
    cond do
      ...
      true ->
        ...
        eval_key1(l, [], newOld, type, key, nLU, ws, newNo,
                    orig)
    end
  end

  defp eval_key1([{_Seq, :delete_key} | l], _Cs, old, type, k,
            lU, ws, no, orig) do
    newNo = no - length(old)
    eval_key1(l, [], [], type, k, lU, ws, newNo, orig)
  end

  defp eval_key1([{_Seq, {:delete_object, term}} | l], cs,
            [{term, _, _}], type = :set, k, lU, ws, no, orig) do
    eval_key1(l, cs, [], type, k, lU, ws, no - 1, orig)
  end

  defp eval_key1([{_Seq, {:delete_object, _T}} | l], cs, old1,
            type = :set, k, lU, ws, no, orig) do
    eval_key1(l, cs, old1, type, k, lU, ws, no, orig)
  end

  defp eval_key1([{seq, {comm, term}} | l], cs, old, type, k, lU,
            ws, no, orig)
      when type !== :set do
    eval_key1(l, [{term, seq, comm} | cs], old, type, k, lU,
                ws, no, orig)
  end

  defp eval_key1([], cs, old, type = :set, _Key, lU, ws, no,
            orig) do
    [] = cs
  ...
  defp same_terms([e1 | l1], [e2 | l2]) when :erlang.element(1,
                                                       e1) === :erlang.element(1,
                                                                                 e2) do
    same_terms(l1, l2)
  end
  ...
  defp eval_bag([{term1, _S1, op} = n | l] = l0,
            [{term2, _, _} = o | old] = old0, new, no) do
  ...
  defp eval_bag([{_Term1, _Seq1, :delete_object} | l], [] = old,
            new, no) do
    eval_bag(l, old, new, no)
  end

  defp eval_bag([{term, _Seq1, :insert} = n | l], [] = old, new,
            no) do
  ...
  defp bag_object([{term, _, :insert} = n | l], old, new, no, _N,
            term) do
    bag_object(l, old, new, no, [n], term)
  end

  defp bag_object([{term, _, :delete_object} | l], old, new, no,
            _N, term) do
  ...
  defp eval_dupbag([{term1, _S1, op} = n | l] = l0,
            [{term2, _, _} = o | old] = old0, new, no) do
    case ({op, :dets_utils.cmp(term1, term2)}) do
      {:delete_object, -1} ->
  ...
  defp eval_dupbag([{_Term1, _Seq1, :delete_object} | l], [] = old,
            new, no) do
    eval_dupbag(l, old, new, no)
  end

  defp eval_dupbag([{term, _Seq1, :insert} = n | l], [] = old, new,
            no) do
  ...
  defp old_dup_object(l, [{term, _, _} = obj | old], new, no, term,
            n) do
  ...
  defp dup_object([{term, _, :insert} = obj | l], old, new, no,
            term, q) do
    dup_object(l, old, new, no + 1, term, [obj | q])
  end

  defp dup_object([{term, _Seq, :delete_object} | l], old, new,
            no, term, q) do
    newNo = no - length(q)
  ...
  defp update_no_keys(head, ws, deltaObjects, deltaKeys) do
    ...
    newHead = r_head(head, no_objects: newNoObject, 
                        no_keys: newNoKeys)
    nWs = (cond do
  ...
  defp scan_skip(bin, from, to, l, ts, r, type, skip) do
    ...
    case (bin) do
      ...
      <<_ :: size(skip) - binary, _Size :: size(32),
          st :: size(32), _Sz :: size(32), kO :: binary>>
          when (st !== 305419896 and st !== 61591023) ->
        scan_skip(kO, from1 + 12, to, l, ts, r, type,
                    512 * 4 - 12)
      <<_ :: size(skip) - binary, size :: size(32),
          _St :: size(32), sz :: size(32), kO :: binary>>
          when size - 12 <= byte_size(kO) ->
        bin2bins(kO, from1 + 12, to, l, ts, r, type, size, sz)
      <<_ :: size(skip) - binary, size :: size(32),
          _St :: size(32), _Sz :: size(32), _KO :: binary>> ->
  ...
  defp bin2bins(bin, from, to, l, ts, r, type = :set, size,
            objSz0) do
    objsSz1 = size - objSz0
    cond do
      objsSz1 === 8 ->
      ...
      true ->
        objSz = objSz0 - 4
        <<_ :: size(objSz) - binary, nObjSz :: size(32),
            t :: binary>> = bin
  ...
  defp bin2bins(<<objSz :: size(32), bin :: binary>> = kO, from,
            to, l, ts, r, type, size, sz) do
    bins_bag(bin, from, to, l, ts, r, type, size, 1,
               sz - objSz - 4, objSz - 4, size - sz, kO)
  end

  defp bins_set(bin, from, to, l, ts, r, type, size, noObjs,
            _ObjSz0, 8, kO) do
    slot_end(kO, from, to, l, [bin | ts], r, type, size,
               noObjs)
  end
  ...
  defp bins_set(bin, from, to, l, ts, r, type, size, noObjs,
    ...
    <<_ :: size(objSz) - binary, nObjSz :: size(32),
        t :: binary>> = bin
    bins_set(t, from, to, l, [bin | ts], r, type, size,
               noObjs + 1, nObjSz, objsSz - nObjSz, kO)
  end

  defp bins_bag(bin, from, to, l, ts, r, type, size, noObjs, sz,
            objSz, objsSz, kO)
      when sz > 0 do
    <<_ :: size(objSz) - binary, nObjSz :: size(32),
        t :: binary>> = bin
    bins_bag(t, from, to, l, [bin | ts], r, type, size,
               noObjs + 1, sz - nObjSz, nObjSz - 4, objsSz, kO)
  end

  defp bins_bag(bin, from, to, l, ts, r, type, size, noObjs, _Z,
            _ObjSz, 8, kO) do
    slot_end(kO, from, to, l, [bin | ts], r, type, size,
               noObjs)
  end
  ...
  defp bins_bag(bin, from, to, l, ts, r, type, size, noObjs, _Z,
            objSz, objsSz, kO) do
    <<_ :: size(objSz) - binary, sz :: size(32),
        nObjSz :: size(32), t :: binary>> = bin
    bins_bag(t, from, to, l, [bin | ts], r, type, size,
               noObjs + 1, sz - nObjSz - 4, nObjSz - 4, objsSz - sz,
               kO)
  end
  ...
  def file_info(fH) do
    r_fileheader(closed_properly: cP, keypos: kp, m: m, next: next,
        n: n, version: version, type: type,
        no_objects: noObjects, no_keys: noKeys) = fH
    cond do
      cP === 0 ->
        {:error, :not_closed}
      r_fileheader(fH, :cookie) !== 11259375 ->
      ...
      true ->
        {:ok,
           [{:closed_properly, cP}, {:keypos, kp}, {:m, m}, {:n,
                                                               n},
                                                                {:next, next},
                                                                    {:no_objects,
                                                                       noObjects},
  ...
  defp v_segments(_H, <<0 :: size(32), _ :: binary>>, _PartNo,
            _SegNo) do
    :done
  end

  defp v_segments(h, <<seg :: size(32), t :: binary>>, partNo,
            segNo) do
  ...
  defp per_key(head,
            <<binSize :: size(32), 305419896 :: size(32),
                bin :: binary>> = b) do
  ...
  defp per_bag_key(<<size :: size(32), objSz :: size(32),
              t :: binary>> = b,
            keyPos, l) do
    <<keyBin :: size(size) - binary, r :: binary>> = b
    objSz1 = objSz - 4
    size1 = size - objSz - 4
    <<_ :: size(objSz1) - binary,
        keyObjs :: size(size1) - binary, _ :: binary>> = t
    <<_Size :: size(32), bin :: size(objSz) - binary,
        _ :: binary>> = b
    term = :erlang.binary_to_term(t)
    key = :erlang.element(keyPos, term)
    item = {term, - (1 <<< 26), bin}
    per_bag_key(r, keyPos,
                  [{key, size, keyBin, item, keyObjs} | l])
  end
  ...
  defp binobjs2terms(<<objSz :: size(32), t :: binary>> = b) do
    binobjs2terms(b, t, objSz, byte_size(b) - objSz,
                    - (1 <<< 26) + 1, [])
  end
  ...
  defp binobjs2terms(bin, bin1, objSz, size, n, l) do
    ...
    binobjs2terms(t, t1, nObjSz, size - nObjSz, n + 1,
                    [item | l])
  end
  ...
  defp bin2objs2(<<size :: size(32), objSz :: size(32),
              t :: binary>>,
            ts) do
  ...
  defp bin2objs(bin, objSz, size, ts) do
    <<_ :: size(objSz) - binary, nObjSz :: size(32),
        t :: binary>> = bin
    bin2objs(t, nObjSz - 4, size - nObjSz,
               [:erlang.binary_to_term(bin) | ts])
  end

  defp bin2keybins(keysObjs, head) when r_head(head, :type) === :set do
    <<objSz :: size(32), t :: binary>> = keysObjs
    bin2keybins(t, r_head(head, :keypos), objSz - 4,
  ...
  defp bin2keybins2(<<size :: size(32), objSz :: size(32),
              t :: binary>>,
            kp, l) do
  ...
  defp bin2keybins(bin, kp, objSz, _Size = 0, l) do
    ...
    bin2keybins2(t, kp,
                   [{:erlang.element(kp, term), obj} | l])
  end

  defp bin2keybins(bin, kp, objSz, size, l) do
    <<obj :: size(objSz) - binary, nObjSz :: size(32),
        t :: binary>> = bin
    term = :erlang.binary_to_term(obj)
    bin2keybins(t, kp, nObjSz - 4, size - nObjSz,
                  [{:erlang.element(kp, term), obj} | l])
  end
