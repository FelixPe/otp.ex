defmodule :m_dets_v9 do
  ...
  defp v_segments(h, <<seg::size(32), t::binary>>, partNo, segNo) do
    :io.format(~c"<~w>SEGMENT ~w~n", [seg, segNo])
  ...
  defp v_segment(h, segNo, segPos, segSlot) do
    ...
    case (try do
      ...
      {:EXIT, reason} ->
        :dets_utils.vformat(~c"** dets: Corrupt or truncated dets file~n", [])
        :io.format(~c"~nERROR ~tp~n", [reason])

      [] ->
        true

      {size, collP, objects} ->
        :io.format(~c"   <~w>~w: <~w:~p>~w~n", [bucketP, slot, collP, size, objects])
    end
