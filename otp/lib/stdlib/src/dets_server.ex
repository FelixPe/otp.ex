defmodule :m_dets_server do
  ...
  require Record
  Record.defrecord(:r_pending, :pending, tab: :undefined,
                                   ref: :undefined, pid: :undefined,
                                   from: :undefined, reqtype: :undefined,
                                   clients: :undefined)
  Record.defrecord(:r_state, :state, store: :undefined,
                                 parent: :undefined, pending: :undefined)
  ...
  Record.defrecord(:r_cache, :cache, cache: :undefined,
                                 ...
                                 delay: :undefined)
  def start_link() do
    :gen_server.start_link({:local, :dets}, :dets_server,
                             [self()], [])
  end
  ...
  def handle_call({:users, tab}, _From, state) do
    users = :ets.select(r_state(state, :store),
                          [{{:"$1", tab}, [], [:"$1"]}])
    {:reply, users, state}
  end
  ...
  def handle_info({:pending_reply, {ref, result0}}, state) do
    {:value,
       r_pending(tab: tab, pid: pid, from: {fromPid, _Tag} = from,
           reqtype: reqT,
           clients: clients)} = :lists.keysearch(ref, r_pending(:ref),
                                                   r_state(state, :pending))
    store = r_state(state, :store)
    result = (case ({result0, reqT}) do
                {:ok, :add_user} ->
  ...
  defp ensure_started() do
    case (:erlang.whereis(:dets)) do
      :undefined ->
        detsSup = {:dets_sup, {:dets_sup, :start_link, []},
                     :permanent, 1000, :supervisor, [:dets_sup]}
        _ = :supervisor.start_child(:kernel_safe_sup, detsSup)
        detsServer = {:dets, {:dets_server, :start_link, []},
                        :permanent, 2000, :worker, [:dets_server]}
        _ = :supervisor.start_child(:kernel_safe_sup,
                                      detsServer)
        :ok
      _ ->
        :ok
  ...
  defp init() do
    ...
    :dets_registry = :ets.new(:dets_registry,
                                [:set, :named_table])
    :dets_owners = :ets.new(:dets_owners,
                              [:set, :named_table])
  ...
  defp request([{req, from} | l], state) do
    res = (case (req) do
             ...
             {:open, file} ->
               do_internal_open(state, from,
                                  [file, :erlang.get(:verbose)])
             {:open, tab, openArgs} ->
  ...
  defp pending_call(tab, pid, ref, {fromPid, _Tag} = from, args,
            reqT, state) do
    server = self()
    f = fn () ->
             res = (case (reqT) do
    ...
    _ = spawn(f)
    pD = r_pending(tab: tab, ref: ref, pid: pid, reqtype: reqT,
             from: from, clients: [])
  ...
  defp check_pending(tab, from, state, req) do
    case (:lists.keysearch(tab, r_pending(:tab),
                             r_state(state, :pending))) do
      {:value, r_pending(tab: ^tab, clients: clients) = p} ->
        nP = :lists.keyreplace(tab, r_pending(:tab), r_state(state, :pending),
