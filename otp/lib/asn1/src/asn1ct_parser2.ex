defmodule :m_asn1ct_parser2 do
  ...
  Record.defrecord(:r_typereference, :typereference, pos: :undefined,
                                         val: :undefined)
  Record.defrecord(:r_constraint, :constraint, c: :undefined,
                                      e: :undefined)
  Record.defrecord(:r_identifier, :identifier, pos: :undefined,
                                      val: :undefined)
  ...
  defp handle_parse_error(file0, [token | _]) do
    ...
    line = get_line(token)
    error = {:structured_error, {file, line},
               :asn1ct_parser2, {:syntax_error, get_token(token)}}
    {:error, [error]}
  end

  defp do_parse(tokens0) do
    {moduleDefinition,
       tokens1} = parse_ModuleDefinition(tokens0)
  ...
  defp parse_ModuleDefinition([{:typereference, l1, moduleIdentifier} |
               rest0]) do
    :erlang.put(:asn1_module, moduleIdentifier)
    {_DefinitiveIdentifier, rest02} = (case (rest0) do
                                         [{:"{", _} | _Rest01] ->
    ...
    {extensionDefault, rest3} = (case (rest2) do
    ...
    case (rest3) do
      [{:"::=", _L7}, {:BEGIN, _L8} | rest4] ->
        ...
        :erlang.put({:erlang.get(:asn1_module), :imports},
                      imports)
        {r_module(pos: l1, name: moduleIdentifier, defid: [],
             tagdefault: tagDefault,
             extensiondefault: extensionDefault, exports: exports,
  ...
  defp parse_Reference([{:typereference, l1, trefName}, {:"{", _L2}, {:"}",
                                                         _L3} |
                                                          rest]) do
    {tref2Exttref(l1, trefName), rest}
  end

  defp parse_Reference([tref1 = {:typereference, _, _}, {:".", _},
  ...
  defp parse_Reference([tref = {:typereference, _L1, _TrefName} |
               rest]) do
  ...
  defp parse_Imports([{:IMPORTS, _L1} | rest]) do
    {symbolsFromModuleList,
       rest2} = parse_SymbolsFromModuleList(rest)
    case (rest2) do
      [{:";", _L2} | rest3] ->
  ...
  defp parse_SymbolsFromModuleList(tokens, acc) do
    {symbolsFromModule,
       rest} = parse_SymbolsFromModule(tokens)
    try do
      ...
      {sl, _Rest2} when elem(sl, 0) === :SymbolsFromModule ->
        parse_SymbolsFromModuleList(rest,
  ...
  defp parse_SymbolsFromModule(tokens) do
    setRefModuleName = fn n ->
                            fn x when elem(x, 0) === :Externaltypereference ->
    ...
    case (rest) do
      [{:FROM, _L1}, {:typereference, _, name} = tref | [r_identifier(),
                                                             {:",", _} |
                                                                 _] = rest2] ->
        newSymbolList = :lists.map(setRefModuleName.(name),
                                     symbolList)
        {r_SymbolsFromModule(symbols: newSymbolList, module: tref2Exttref(tref)),
           rest2}
      [{:FROM, _L1}, {:typereference, _, name} = tref | [r_identifier(),
                                                             {:FROM, _} |
                                                                 _] = rest2] ->
  ...
  defp parse_ObjectIdentifierValue([r_identifier(val: id), {:"(", _}, {:number, _, num}, {:")",
                                                      _} |
                                                       rest],
            acc) do
    parse_ObjectIdentifierValue(rest,
  ...
  defp parse_ObjectIdentifierValue([r_identifier(val: id), {:"(", _}, r_identifier(val: id2), {:")", _} |
                                                 rest],
            acc) do
    parse_ObjectIdentifierValue(rest,
  ...
  defp parse_ObjectIdentifierValue([r_identifier(val: id), {:"(", _}, {:typereference, _, tref},
                                    {:".", _}, r_identifier(val: id2), {:")", _} | rest],
            acc) do
    parse_ObjectIdentifierValue(rest,
                                  [{:NamedNumber, id,
                                      {:ExternalValue, tref, id2}} |
                                       acc])
  end
  ...
  defp parse_Assignment([{:typereference, l1, name}, {:"::=", _} |
                                           tokens0]) do
    flist = [{:type, &parse_Type/1}, {:class,
                                        &parse_ObjectClass/1}]
    case (parse_or_tag(tokens0, flist)) do
      {{:type, type}, tokens} ->
        {r_typedef(pos: l1, name: name, typespec: type), tokens}
      {{:class, type}, tokens} ->
        {r_classdef(pos: l1, name: name, module: resolve_module(type),
             typespec: type),
           tokens}
    end
  end

  defp parse_Assignment([{:typereference, _, _}, {:"{", _} |
                                       _] = tokens) do
    flist = [&parse_ParameterizedTypeAssignment/1,
  ...
  defp prioritize_error(errors0) do
    ...
    errors2 = (for l <- errors1 do
                 {length(l), l}
               end)
    errors = :lists.sort(errors2)
    [res | _] = (for {_, l} <- errors do
                   l
                 end)
    res
  end
  ...
  defp parse_Type(tokens) do
    {tag, rest3} = (case (tokens) do
                      [{:"[", _} | _] ->
                        parse_Tag(tokens)
                      _ ->
                        {[], tokens}
                    end)
    {tag2, rest4} = (case (rest3) do
                       ...
                       rest31 when elem(tag, 0) === :tag ->
                         {[r_tag(tag, type: {:default, :erlang.get(:tagdefault)})],
                            rest31}
                       rest31 ->
                         {tag, rest31}
                     end)
    flist = [&parse_BuiltinType/1, &parse_ReferencedType/1,
                                       &parse_TypeWithConstraint/1]
    {type, rest5} = parse_or(rest4, flist)
    case (rest5) do
      [{:"(", _} | _] ->
        {constraints, rest6} = parse_Constraints(rest5)
        {r_type(type, tag: tag2, 
  ...
  defp parse_BuiltinType([{:restrictedcharacterstringtype, _,
              stringName} |
               rest]) do
  ...
  defp parse_BuiltinType([{:CHOICE, _}, {:"{", _} | rest]) do
    ...
    case (rest2) do
      [{:"}", _} | rest3] ->
        needExt = not
                  :lists.keymember(:EXTENSIONMARK, 1,
                                     l0) and :erlang.get(:extensiondefault) === :IMPLIED
        l = (case (needExt) do
               true ->
                 l0 ++ [r_EXTENSIONMARK()]
               false ->
  ...
  defp parse_BuiltinType([{:INSTANCE, _}, {:OF, _} | rest]) do
    {definedObjectClass,
       rest2} = parse_DefinedObjectClass(rest)
    case (rest2) do
      [{:"(", _} | _] ->
  ...
  defp parse_BuiltinType([{:SEQUENCE, _}, {:"{", _}, {:"...", line}, {:"}", _} |
                                                    rest]) do
  ...
  defp parse_BuiltinType([{:SEQUENCE, _}, {:"{", _}, {:"...", line}, {:"!", _} |
    ...
    {exceptionIdentification,
       rest2} = parse_ExceptionIdentification(rest)
    case (rest2) do
      [{:"}", _} | rest3] ->
        {r_type(def: r_SEQUENCE(components: [r_EXTENSIONMARK(pos: line,
                                   val: exceptionIdentification)])),
           rest3}
      _ ->
        {componentTypeLists,
           rest3} = parse_ComponentTypeLists2(rest2,
  ...
  defp parse_BuiltinType([{:SEQUENCE, _}, {:"{", _} | rest]) do
    {componentTypeLists,
       rest2} = parse_ComponentTypeLists(rest)
    case (rest2) do
      [{:"}", _} | rest3] ->
        componentTypeLists2 = (case ({for (ext = r_EXTENSIONMARK()) <- componentTypeLists do
                                        ext
                                      end,
                                        :erlang.get(:extensiondefault)}) do
                                 {[], :IMPLIED} ->
  ...
  defp parse_BuiltinType([{:SEQUENCE, _}, {:OF, _} | [r_identifier(), {:"<", _} |
                                                _] = tokens0]) do
  ...
  defp parse_BuiltinType([{:SET, _}, {:"{", _}, {:"...", line}, {:"}", _} |
                                               rest]) do
  ...
  defp parse_BuiltinType([{:SET, _}, {:"{", _}, {:"...", line}, {:"!", _} |
    ...
    case (rest2) do
      [{:"}", _} | rest3] ->
        {r_type(def: r_SET(components: [r_EXTENSIONMARK(pos: line,
                                   val: exceptionIdentification)])),
           rest3}
      _ ->
        {componentTypeLists,
  ...
  defp parse_BuiltinType([{:SET, _}, {:OF, _} | [r_identifier(), {:"<", _} |
                                           _] = tokens0]) do
    {type, tokens} = parse_SelectionType(tokens0)
  ...
  defp parse_BuiltinType([{:ANY, _}, {:DEFINED, _}, {:BY, _},
                                         r_identifier(val: id) | rest]) do
  ...
  defp parse_TypeWithConstraint([{:SEQUENCE, _} | [{:"(", _} | _] = rest0]) do
    ...
    {r_type(def: {:"SEQUENCE OF", type},
         constraint: merge_constraints([constraint])),
       rest5}
  end

  defp parse_TypeWithConstraint([{:SEQUENCE, _}, {:SIZE, _} | [{:"(", _} |
                                             _] = rest0]) do
    {constraint, rest2} = parse_Constraint(rest0)
    r_constraint(c: c) = constraint
    constraint2 = r_constraint(constraint, c: {:element_set,
  ...
  defp parse_TypeWithConstraint([{:SET, _} | [{:"(", _} | _] = rest0]) do
    ...
    {type, rest5} = parse_Type(rest4)
    {r_type(def: {:"SET OF", type},
         constraint: merge_constraints([constraint])),
       rest5}
  end

  defp parse_TypeWithConstraint([{:SET, _}, {:SIZE, _} | [{:"(", _} |
                                        _] = rest0]) do
    {constraint, rest2} = parse_Constraint(rest0)
    r_constraint(c: c) = constraint
    constraint2 = r_constraint(constraint, c: {:element_set,
  ...
  defp parse_ReferencedType(tokens) do
    flist = [&parse_ParameterizedType/1,
                 &parse_DefinedType/1, &parse_SelectionType/1,
                                           &parse_TypeFromObject/1]
    parse_or(tokens, flist)
  end

  defp parse_DefinedType([{:typereference, l1, module}, {:".", _},
                                             {:typereference, _, typeName} |
                                                 tokens]) do
    {r_type(def: r_Externaltypereference(pos: l1, module: module, type: typeName)),
       tokens}
  end
  ...
  defp resolve_module(type, current, imports) do
    case (for r_SymbolsFromModule(symbols: s, module: mod) <- imports,
                r_Externaltypereference(type: t) <- s, type === t do
            mod
          end) do
  ...
  defp parse_ConstraintSpec(tokens) do
    flist = [&parse_GeneralConstraint/1,
  ...
  defp parse_Elements(tokens) do
    flist = [&parse_ObjectSetElements/1,
                 &parse_SubtypeElements/1, &parse_Object/1,
                                               &parse_DefinedObjectSet/1]
    parse_or(tokens, flist)
  end

  defp parse_DefinedObjectClass([{:typereference, _, modName}, {:".", _},
  ...
  defp parse_DefinedObjectClass([tr = {:typereference, _, _ObjClName} |
               rest]) do
  ...
  defp parse_PrimitiveFieldName([{:valuefieldreference, _, fieldName} |
               rest]) do
  ...
  defp parse_FixedTypeValueFieldSpec([{:valuefieldreference, _, vFieldName} |
               rest]) do
    {type, rest2} = parse_Type(rest)
    {unique, rest3} = (case (rest2) do
                         [{:UNIQUE, _} | rest4] ->
                           {:UNIQUE, rest4}
                         _ ->
                           {:undefined, rest2}
    ...
    {optionalitySpec,
       rest5} = parse_ValueOptionalitySpec(rest3)
    ...
    {{tag, vFieldName, type, unique, optionalitySpec},
       rest5}
  end

  defp parse_VariableTypeValueFieldSpec([{:valuefieldreference, _, vFieldName} |
               rest0]) do
    {fieldRef, rest1} = parse_FieldName(rest0)
    {optionalitySpec,
       rest} = parse_ValueOptionalitySpec(rest1)
    case (is_end_delimiter(rest)) do
      true ->
        {{:variabletypevaluefield, vFieldName, fieldRef,
            optionalitySpec},
           rest}
      false ->
        parse_error(rest)
  ...
  defp parse_TypeFieldSpec([{:typefieldreference, _, name} | rest0]) do
    {optionalitySpec,
       rest} = parse_TypeOptionalitySpec(rest0)
    case (is_end_delimiter(rest)) do
      true ->
  ...
  defp parse_FixedTypeValueSetFieldSpec([{:typefieldreference, _, name} | rest0]) do
    ...
    {optionalitySpec,
       rest} = parse_ValueSetOptionalitySpec(rest1)
    case (is_end_delimiter(rest)) do
      true ->
  ...
  defp parse_VariableTypeValueSetFieldSpec([{:typefieldreference, _, name} | rest0]) do
    ...
    case (is_end_delimiter(rest)) do
      true ->
        {{:variabletypevaluesetfield, name, fieldRef,
            optionalitySpec},
           rest}
      false ->
        parse_error(rest)
  ...
  defp parse_RequiredToken([{:typereference, _, wordName} |
               rest] = tokens) do
  ...
  defp parse_DefinedObject([{:typereference, l1, modName}, {:".", _},
                                              r_identifier(val: objName) | rest]) do
    {{:object, r_Externaltypereference(pos: l1, module: modName, type: objName)},
       rest}
  end
  ...
  defp parse_ObjectAssignment([r_identifier(pos: l1, val: objName) | rest]) do
    ...
    case (rest2) do
      [{:"::=", _} | rest3] ->
        ...
        {r_typedef(pos: l1, name: objName,
             typespec: r_Object(classname: class, def: object)),
           rest4}
      _ ->
        parse_error(rest2)
    end
  ...
  defp parse_FieldSetting(tokens) do
    {{_, primFieldName},
       rest} = parse_PrimitiveFieldName(tokens)
  ...
  defp parse_DefinedSyntaxToken([{:typereference, _, _Name}, {t, _} |
                                           _] = tokens)
      when t === :"." or t === :"(" do
    parse_Setting(tokens)
  end

  defp parse_DefinedSyntaxToken([{:typereference, l1, name} = tRef |
               rest] = tokens) do
  ...
  defp parse_Setting(tokens) do
    flist = [{:type_tag, &parse_Type/1}, {:value_tag,
                                            &parse_Value/1},
  ...
  defp parse_DefinedObjectSet([{:typereference, l1, moduleName}, {:".", _},
                                                 {:typereference, l2,
                                                    objSetName} |
                                                     rest]) do
    {{:objectset, l1,
        r_Externaltypereference(pos: l2, module: moduleName, type: objSetName)},
       rest}
  end

  defp parse_DefinedObjectSet([{:typereference, l1, objSetName} | rest]) do
    {{:objectset, l1,
        r_Externaltypereference(pos: l1, module: resolve_module(objSetName),
            type: objSetName)},
       rest}
  end
  ...
  defp parse_ObjectSetAssignment([{:typereference, l1, objSetName} | rest]) do
    ...
    case (rest2) do
      [{:"::=", _} | rest3] ->
        ...
        {r_typedef(pos: l1, name: objSetName,
  ...
  defp parse_ObjectSetElements(tokens) do
    flist = [&parse_ObjectSetFromObjects/1,
  ...
  defp parse_ObjectClassFieldType(tokens) do
    ...
    case (rest) do
      [{:".", _} | rest2] ->
        ...
        oCFT = r_ObjectClassFieldType(classname: class, class: class,
                   fieldname: fieldName)
  ...
  defp parse_GeneralConstraint(tokens) do
    flist = [&parse_UserDefinedConstraint/1,
                 &parse_TableConstraint/1, &parse_ContentsConstraint/1]
    parse_or(tokens, flist)
  end

  defp parse_UserDefinedConstraint([{:CONSTRAINED, _}, {:BY, _}, {:"{", _}, {:"}", _} |
                                                     rest]) do
    {{:constrained_by, []}, rest}
  end

  defp parse_UserDefinedConstraint([{:CONSTRAINED, _}, {:BY, _}, {:"{", _} |
                                            rest]) do
    {param,
       rest2} = parse_UserDefinedConstraintParameter(rest)
    case (rest2) do
      [{:"}", _} | rest3] ->
  ...
  defp parse_UserDefinedConstraintParameter(tokens0, acc) do
    flist = [&parse_GovernorAndActualParameter/1,
  ...
  defp parse_TableConstraint(tokens) do
    flist = [&parse_ComponentRelationConstraint/1,
    ...
    case (rest2) do
      [{:"}", _}, {:"{", _} | rest3] ->
        ...
        case (rest4) do
          [{:"}", _} | rest5] ->
            ret = {:element_set,
                     {:componentrelation, objectSet, atNot}, :none}
            {ret, rest5}
          _ ->
            parse_error(rest4)
        end
  ...
  defp parse_ComponentIdList([r_identifier() = id, {:".", _} | rest], acc) do
    parse_ComponentIdList(rest,
                            [identifier2Extvalueref(id) | acc])
  end

  defp parse_ComponentIdList([r_identifier() = id | rest], acc) do
    {:lists.reverse(acc, [identifier2Extvalueref(id)]),
       rest}
  end
  ...
  defp parse_ActualParameter(tokens) do
    flist = [&parse_Type/1, &parse_Value/1,
                                &parse_ValueSet/1, &parse_DefinedObjectClass/1,
                                                       &parse_Object/1,
                                                           &parse_ObjectSet/1]
    parse_or(tokens, flist)
  end
  ...
  defp parse_ParameterizedTypeAssignment([{:typereference, l1, name} | rest]) do
    ...
    case (rest2) do
      [{:"::=", _} | rest3] ->
        ...
        {r_ptypedef(pos: l1, name: name, args: parameterList,
             typespec: type),
           rest4}
      _ ->
        parse_error(rest2)
  ...
  defp parse_ParameterizedValueAssignment([r_identifier(pos: l1, val: name) | rest]) do
    ...
    case (rest3) do
      [{:"::=", _} | rest4] ->
        ...
        {r_pvaluedef(pos: l1, name: name, args: parameterList, type: type,
             value: value),
           rest5}
      _ ->
        parse_error(rest3)
  ...
  defp parse_ParameterizedValueSetTypeAssignment([{:typereference, l1, name} | rest]) do
    ...
    case (rest3) do
      [{:"::=", _} | rest4] ->
        ...
        {r_pvaluesetdef(pos: l1, name: name, args: parameterList, type: type,
             valueset: valueSet),
           rest5}
      _ ->
        parse_error(rest3)
  ...
  defp parse_ParameterizedObjectAssignment([r_identifier(pos: l1, val: name) | rest]) do
    ...
    case (rest3) do
      [{:"::=", _} | rest4] ->
        ...
        {r_pobjectdef(pos: l1, name: name, args: parameterList,
  ...
  defp parse_SimpleDefinedType([{:typereference, l1, moduleName}, {:".", _},
                                                 {:typereference, _, typeName} |
                                                     rest]) do
  ...
  defp parse_SimpleDefinedValue([{:typereference, l1, moduleName}, {:".", _},
                                                 r_identifier(val: value) | rest]) do
    {{:simpledefinedvalue,
        r_Externalvaluereference(pos: l1, module: moduleName, value: value)},
       rest}
  end

  defp parse_SimpleDefinedValue([r_identifier() = id | rest]) do
    {{:simpledefinedvalue, identifier2Extvalueref(id)},
       rest}
  end
  ...
  defp not_allowed_word(name) do
    :lists.member(name,
                    ['BIT', 'BOOLEAN', 'CHARACTER', 'CHOICE', 'EMBEDDED', 'END', 'ENUMERATED', 'EXTERNAL', 'FALSE', 'INSTANCE', 'INTEGER', 'INTERSECTION', 'MINUS-INFINITY', 'NULL', 'OBJECT', 'OCTET', 'PLUS-INFINITY', 'REAL',
                                                                            'SEQUENCE',
                                                                                'SET',
                                                                                    'TRUE',
                                                                                        'UNION'])
  ...
  defp check_rest([r | rs]) when (?A <= r and r <= ?Z) or
                           r === ?- do
    check_rest(rs)
  end
  ...
  defp parse_AlternativeTypeLists(tokens0) do
    ...
    case (tokens1) do
      [{:",", _} | tokens2] ->
        {extMarker,
           tokens3} = parse_ExtensionAndException(tokens2)
        {extAlts,
           tokens4} = parse_ExtensionAdditionAlternatives(tokens3)
  ...
  defp parse_ExtensionAdditionAlternativesList([{:",", _} | tokens1] = tokens0, acc) do
    try do
      ...
      {extAddAlt, tokens2} ->
        parse_ExtensionAdditionAlternativesList(tokens2,
                                                  [extAddAlt | acc])
    end
  end
  ...
  defp parse_ExtensionAdditionAlternative([{:"[", _}, {:"[", _} | tokens0]) do
    ...
    {groupList,
       tokens3} = parse_AlternativeTypeList(tokens2)
    case (tokens3) do
      [{:"]", _}, {:"]", _} | tokens] ->
        {groupList, tokens}
      _ ->
  ...
  defp parse_NamedType([r_identifier(pos: l1, val: idname) | rest]) do
    {type, rest2} = parse_Type(rest)
    {r_ComponentType(pos: l1, name: idname, typespec: type,
         prop: :mandatory),
       rest2}
  end
  ...
  defp parse_ComponentTypeLists([{:COMPONENTS, _}, {:OF, _} | _] = tokens,
            clist) do
    {compList, rest1} = parse_ComponentTypeList(tokens, [])
    parse_ComponentTypeLists(rest1, clist ++ compList)
  end
  ...
  defp parse_ComponentTypeLists2(tokens, clist) do
    ...
    {clist2, rest2} = parse_OptionalExtensionMarker(rest,
  ...
  defp parse_ComponentTypeList([{:",", _} | [{:COMPONENTS, _}, {:OF, _} |
                                            _] = tokens0],
            acc)
      when acc !== [] do
    {componentType, tokens} = parse_ComponentType(tokens0)
  ...
  defp parse_ExtensionAdditions(tokens = [{:",", _} | _], clist) do
    {extAddList,
       rest2} = parse_ExtensionAdditionList(tokens, [])
  ...
  defp parse_ExtensionAdditionList([{:",", _} | [{:COMPONENTS, _}, {:OF, _} |
                                            _] = tokens0],
            acc) do
    {componentType, tokens} = parse_ComponentType(tokens0)
    parse_ExtensionAdditionList(tokens,
  ...
  defp parse_ExtensionAdditionList([{:",", _}, {:"[", _}, {:"[", _} | tokens], acc) do
    {extAddGroup,
       rest2} = parse_ExtensionAdditionGroup(tokens)
    ...
    case (rest) do
      [{:"]", _}, {:"]", _} | rest2] ->
        {[{:ExtensionAdditionGroup, num} |
              compTypeList] ++ [:ExtensionAdditionGroupEnd],
           rest2}
      _ ->
        parse_error(rest)
    end
  ...
  defp parse_Tag([{:"[", _} | rest]) do
    ...
    {classNumber, rest3} = (case (rest2) do
                              [{:number, _, num} | rest21] ->
                                {num, rest21}
                              _ ->
  ...
  defp parse_Value(tokens) do
    flist = [&parse_BuiltinValue/1,
                 &parse_ValueFromObject/1, &parse_DefinedValue/1]
    parse_or(tokens, flist)
  end
  ...
  defp parse_BuiltinValue(tokens = [{:"{", _} | _Rest]) do
    flist = [&parse_SequenceOfValue/1,
                 &parse_SequenceValue/1, &parse_ObjectIdentifierValue/1]
    parse_or(tokens, flist)
  end
  ...
  defp parse_DefinedValue(tokens) do
    flist = [&parse_ParameterizedValue/1,
                 &parse_DefinedValue2/1]
    parse_or(tokens, flist)
  end

  defp parse_DefinedValue2([{:typereference, l1, tname}, {:".", _},
                                            r_identifier(val: idname) | rest]) do
  ...
  defp parse_SequenceValue([r_identifier(pos: pos, val: idName) | rest], acc) do
    {value, rest2} = parse_Value(rest)
    seqTag = r_seqtag(pos: pos, module: :erlang.get(:asn1_module),
                 val: idName)
    case (rest2) do
      [{:",", _} | rest3] ->
  ...
  defp parse_ValueSetTypeAssignment([{:typereference, l1, name} | rest]) do
    ...
    case (rest2) do
      [{:"::=", _} | rest3] ->
        ...
        {r_valuedef(pos: l1, name: name, type: type, value: valueSet,
  ...
  defp parse_ValueAssignment([r_identifier(pos: l1, val: idName) | rest]) do
    ...
    case (rest2) do
      [{:"::=", _} | rest3] ->
        {value, rest4} = parse_Value(rest3)
        {r_valuedef(pos: l1, name: idName, type: type, value: value,
             module: :erlang.get(:asn1_module)),
           rest4}
      _ ->
        parse_error(rest2)
  ...
  defp parse_SubtypeElements([{:WITH, _}, {:COMPONENTS, _}, {:"{", _}, {:"...", _},
                                                      {:",", _} | tokens]) do
  ...
  defp parse_SubtypeElements([{:WITH, _}, {:COMPONENTS, _}, {:"{", _} |
                                             tokens]) do
    {constraint, rest} = parse_TypeConstraints(tokens)
    case (rest) do
      [{:"}", _} | rest2] ->
  ...
  defp parse_SubtypeElements(tokens) do
    flist = [&parse_ContainedSubtype/1, &parse_Value/1,
                                            &parse_MIN/1, &parse_Type/1]
  ...
  defp parse_TypeConstraints([r_identifier() | rest], acc) do
    {componentConstraint,
       rest2} = parse_ComponentConstraint(rest)
    case (rest2) do
      [{:",", _} | rest3] ->
        parse_TypeConstraints(rest3,
  ...
  defp merge_constraints([], cacc, eacc) do
    :lists.reverse(cacc) ++ [{:element_set, {:Errors, eacc},
                                :none}]
  end

  defp get_line({token, pos, _}) when (is_integer(pos) and
                                   is_atom(token)) do
    pos
  end

  defp get_line({token, pos}) when (is_integer(pos) and
                                is_atom(token)) do
    pos
  end
  ...
  defp get_token({token, pos, value}) when (is_integer(pos) and
                                       is_atom(token)) do
    value
  end
  ...
  defp get_token({token, pos}) when (is_integer(pos) and
                                is_atom(token)) do
    token
  end
