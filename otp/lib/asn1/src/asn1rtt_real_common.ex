defmodule :m_asn1rtt_real_common do
  ...
  defp encode_real(_C, {mantissa, base, exponent})
    ...
    cond do
      octExpLen > 255 ->
        exit({:error,
                {:asn1, {:to_big_exp_in_encode_real, octExpLen}}})
      true ->
        true
    end
    {lenCode, eOctets} = (case (octExpLen) do
                            1 ->
                              {0, octExp}
                            2 ->
                              {1, octExp}
    ...
    bB = 0
    firstOctet = <<1 :: size(1), signBit :: size(1),
                     bB :: size(2), sFactor :: size(2), lenCode :: size(2)>>
    octMantissa = (cond do
                     man > 0 ->
    ...
    <<firstOctet :: binary, eOctets :: binary,
        octMantissa :: binary>>
  end
  ...
  defp encode_real(_C, {_, base, _}) do
    exit({:error,
            {:asn1, {:encode_real_non_supported_encoding, base}}})
  end

  defp encode_real(c, real) when is_list(real) do
    {mantissa, exponent} = (case (:string.lexemes(real,
                                                    'Ee')) do
                              [nR2] ->
                                {nR2, 0}
                              [nR3MB, nR3E] ->
    ...
    {newMantissa, lenDecimal} = (case (mantissa) do
                                   [?. | dec] ->
                                   ...
                                   _ ->
                                     case (:string.lexemes(mantissa, ',.')) do
                                       [num] ->
                                         {:erlang.integer_to_list(:erlang.list_to_integer(num)),
                                            0}
                                       [num, dec] ->
                                         newDec = zeroDecimal.(remove_trailing_zeros(dec))
                                         newMan = :erlang.integer_to_list(:erlang.list_to_integer(num)) ++ newDec
                                         {:erlang.integer_to_list(:erlang.list_to_integer(newMan)),
                                            length(newDec)}
                                     end
                                 end)
    encode_real_as_string(c, newMantissa,
                            exponent - lenDecimal)
  end
  ...
  defp remove_trailing_zeros(intStr) do
    case (:lists.dropwhile(fn ?0 ->
                                true
                              _ ->
                                false
                           end,
                             :lists.reverse(intStr))) do
      [] ->
        '0'
      reversedIntStr ->
  ...
  defp decode_real2(buffer0, _C, len, remBytes1) do
    ...
    cond do
      ...
      true ->
        <<_B7 :: size(1), sign :: size(1), bB :: size(2),
            _FF :: size(2), eE :: size(2)>> = <<first>>
        base = (case (bB) do
                  0 ->
                    2
                  _ ->
        ...
        {firstLen, {exp, buffer3, _Rb2},
           remBytes2} = (case (eE) do
                           0 ->
                             {2, decode_integer2(1, buffer2, remBytes1),
                                remBytes1 + 1}
                           1 ->
                             {3, decode_integer2(2, buffer2, remBytes1),
                                remBytes1 + 2}
                           2 ->
                             {4, decode_integer2(3, buffer2, remBytes1),
                                remBytes1 + 3}
                           3 ->
        ...
        length = len - firstLen
        <<longInt :: size(length) - unit(8),
            restBuff :: binary>> = buffer3
        {{mantissa, buffer4}, remBytes3} = (cond do
                                              sign === 0 ->
                                                {{longInt, restBuff},
                                                   1 + length}
                                              true ->
  ...
  defp decode_integer2(len,
            ...
            removedBytes) do
    <<n :: size(len) - unit(8), buffer2 :: binary>> = <<b2,
                                                          bs :: binary>>
    int = n - (1 <<< (8 * len - 1))
    {int, buffer2, removedBytes}
  end
