  ...
  defp from_type_prim(m, d) do
    ...
    case r_type(d, :def) do
      ...
      when is_tuple(enum) and
        ...
        case nN do
          [] ->
            :io.format(:user, ~c"Enum = ~p~n", [enum])
            :asn1_EMPTY
      ...
      :"OCTET STRING" ->
        s0 = adjust_list(size_random(c), c_string(c, ~c"OCTET STRING"))
      ...
      :NumericString ->
        adjust_list(size_random(c), c_string(c, ~c"0123456789"))

      :TeletexString ->
        adjust_list(size_random(c), c_string(c, ~c"TeletexString"))

      :T61String ->
        adjust_list(size_random(c), c_string(c, ~c"T61String"))

      :VideotexString ->
        adjust_list(size_random(c), c_string(c, ~c"VideotexString"))
      ...
      :GraphicString ->
        adjust_list(size_random(c), c_string(c, ~c"GraphicString"))

      :VisibleString ->
        adjust_list(size_random(c), c_string(c, ~c"VisibleString"))

      :GeneralString ->
        adjust_list(size_random(c), c_string(c, ~c"GeneralString"))

      :PrintableString ->
        adjust_list(size_random(c), c_string(c, ~c"PrintableString"))

      :IA5String ->
        adjust_list(size_random(c), c_string(c, ~c"IA5String"))

      :BMPString ->
        adjust_list(size_random(c), c_string(c, ~c"BMPString"))
      ...
      :UniversalString ->
        adjust_list(size_random(c), c_string(c, ~c"UniversalString"))
    end
  end
