defmodule :m_asn1ct_value do
  ...
  defp from_type(m, typename, type)
    ...
    case (:asn1ct_gen.type(innerType)) do
      ...
      {:constructed, :bif} when typename == [:EXTERNAL] ->
        val = from_type_constructed(m, typename, innerType,
                                      type)
        t = (case (m.maps()) do
               false ->
  ...
  defp get_sequence(m, typename, type) do
    {_SEQorSET, compList} = (case (r_type(type, :def)) do
    ...
    case (m.maps()) do
      false ->
        ...
        :erlang.list_to_tuple([recordTag | for {_, val} <- cs do
                                             val
                                           end])
      true ->
        :maps.from_list(cs)
  ...
  defp get_components2(m, typename, [h | t]) do
    ...
    [{name, from_type(m, typename, h)} | get_components(m,
                                                          typename, t)]
  end
  ...
  defp gen_list(m, typename, oftype, n) do
    [from_type(m, typename, oftype) | gen_list(m, typename,
                                                 oftype, n - 1)]
  end
  ...
  defp from_type_prim(m, d) do
    ...
    case (r_type(d, :def)) do
      ...
      enum when (is_tuple(enum) and
                   :erlang.element(1, enum) == :ENUMERATED)
                ->
        namedNumberList = (case (enum) do
                             {_, _, nNL} ->
                               nNL
                             {_, nNL} ->
                               nNL
                           end)
        nNew = (case (namedNumberList) do
                  {n1, n2} ->
                    n1 ++ n2
                  _ ->
                    namedNumberList
      ...
      :"OBJECT IDENTIFIER" ->
        len = random(3)
        olist = (for _X <- :lists.seq(1, len) do
                   random(1000) - 1
                 end)
        :erlang.list_to_tuple([random(3) - 1, random(40) - 1 |
                                                  olist])
      :"RELATIVE-OID" ->
        len = random(5)
        olist = (for _X <- :lists.seq(1, len) do
      ...
      :UTF8String ->
        l = adjust_list(random(50),
                          [?U, ?T, ?F, ?8, ?S, ?t, ?r, ?i, ?n, ?g, 65535, 65518,
                                                                              1114111,
                                                                                  65535,
                                                                                      4095])
  ...
  defp random_unnamed_bit_string(m, c) do
    bl1 = :lists.reverse(adjust_list(size_random(c),
                                       [1, 0, 1, 1]))
    bl2 = :lists.reverse(:lists.dropwhile(fn 0 ->
                                               true
                                             1 ->
                                               false
                                          end,
                                            bl1))
    val = (case ({length(bl2),
                    get_constraint(c, :SizeConstraint)}) do
             {len, len} ->
               bl2
