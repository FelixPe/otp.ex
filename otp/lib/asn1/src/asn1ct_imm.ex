defmodule :m_asn1ct_imm do
  ...
  defp opt_al({:mul, e0, i}, a0) when is_integer(i) do
    {e, a} = opt_al(e0, a0)
    {{:mul, e, i}, a}
  end
  ...
  defp is_nonbuilding({:fun, _, _, _}) do
    true
  end
  ...
  defp enc_sof_length(len, sv, _Aligned) when is_integer(sv) do
    [{:cond, [[{:eq, len, sv}]]}]
  end
  ...
  defp enc_pre_cg_nonbuilding({:fun, args, b0, dst}, stL) do
    b = enc_pre_cg_1(b0, stL, :outside_seq)
    {:fun, args, b, dst}
  end
  ...
  defp enc_opt({:fun, _, _, dst} = imm, st0) do
  ...
  defp per_fixup([{:fun, args, body, dst} | t]) do
