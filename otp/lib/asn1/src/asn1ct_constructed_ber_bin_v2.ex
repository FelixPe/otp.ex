defmodule :m_asn1ct_constructed_ber_bin_v2 do
  ...
  def gen_encode_sequence(gen, typename, r_type() = d) do
    ...
    {seqOrSet, tableConsInfo,
       compList0} = (case (r_type(d, :def)) do
    ...
    encObj = (case (tableConsInfo) do
                r_simpletableattributes(usedclassfield: used, uniqueclassfield: unique)
                    when used != unique ->
                  false
                r_simpletableattributes(objectsetname: objectSetRef, c_name: attrN,
                    c_index: n, usedclassfield: uniqueFieldName,
                  ...
                  case (r_ObjectSet(r_typedef(oSDef, :typespec), :gen)) do
                    true ->
                      objectEncode = :asn1ct_gen.un_hyphen_var(:lists.concat([:Obj,
                                                                                  attrN]))
                      emit([objectEncode, ' = ', :nl, '   ', {:asis, objSetMod}, ':\'getenc_',
                                                                             objSetName,
                                                                                 '\'('])
    ...
    gen_enc_sequence_call(gen, typename, compList1, 1, ext,
                            encObj)
    emit([:nl, '   BytesSoFar = '])
    case (seqOrSet) do
      :SET when r_SET(r_type(d, :def), :sorted) == :dynamic ->
        :asn1ct_func.need({:ber, :dynamicsort_SET_components,
                             1})
  ...
  defp enc_match_input(r_gen(pack: :record), valName, compList) do
    len = length(compList)
    vars = (for n <- :lists.seq(1, len) do
              :lists.concat(['Cindex', n])
            end)
    recordName = '_'
    emit(['{', :lists.join(',', [recordName | vars]), '} = ',
                                                      valName, :com, :nl])
  end
  ...
  defp enc_match_input(r_gen(pack: :map), valName, compList) do
    ...
    os0 = (for {r_ComponentType(prop: prop, name: name), var} <- zipped,
                 prop !== :mandatory do
             {name, var}
           end)
    f = fn {name, var} ->
  ...
  def gen_decode_sequence(gen, typename, r_type() = d) do
    :asn1ct_name.start()
    :asn1ct_name.new(:tag)
    r_SEQUENCE(tablecinf: tableConsInfo,
        components: cList0) = r_type(d, :def)
    cList = filter_complist(cList0)
    ext = extensible(cList)
    {compList, compList2} = (case (cList) do
                               {rl1, el, rl2} ->
                                 {rl1 ++ el ++ rl2, cList}
                               {rl, el} ->
                                 {rl ++ el, rl ++ el}
    ...
    :asn1ct_name.new(:tlv)
    :asn1ct_name.new(:v)
    {decObjInf, valueIndex} = (case (tableConsInfo) do
                                 r_simpletableattributes(objectsetname: objectSetRef, c_name: attrN,
    ...
    case (gen_dec_sequence_call(gen, typename, compList2,
                                  ext, decObjInf)) do
      :no_terms ->
        :asn1ct_name.new(:rb)
        case (gen) do
      ...
      {leadingAttrTerm, postponedDecArgs} ->
        ...
        case ({leadingAttrTerm, postponedDecArgs}) do
          ...
          {[{objSetRef, leadingAttr, term}], ^postponedDecArgs} ->
            decObj = :asn1ct_gen.un_hyphen_var(:lists.concat([:DecObj,
                                                                  leadingAttr,
                                                                      term]))
            valueMatch = value_match(gen, valueIndex, term)
            {objSetMod, objSetName} = objSetRef
            emit([decObj, ' =', :nl, '   ', {:asis, objSetMod}, ':\'getdec_',
                                                             objSetName, '\'(',
                                                                             valueMatch,
                                                                                 '),',
                                                                                     :nl])
        ...
        case (ext) do
          ...
          _ ->
            emit(['case ', {:prev, :tlv}, ' of', :nl, '[] -> true;', '_ -> exit({error,{asn1, {unexpected,', {:prev, :tlv}, '}}}) % extra fields not allowed',
                                                                     :nl, 'end,',
                                                                              :nl])
        end
        :asn1ct_name.new(:rb)
  ...
  defp dec_external(r_gen(pack: :record), recordName) do
    ...
    emit(['OldFormat={', :lists.join(',', record), '},', :nl, {:call, :ext,
  ...
  defp gen_dec_do_pack(r_gen(pack: :record), recordName, _CompList) do
    ...
    l = [{:asis, recordName} | for var <- all do
                                 {:var, var}
                               end]
  ...
  defp gen_dec_postponed_decs(decObj,
            [{_Cname, {firstPFN, pFNList}, term, tmpTerm, _Tag,
                optOrMand} |
                 rest]) do
  ...
  defp emit_opt_or_mand_check(value, tmpTerm) do
    emit([indent(3), 'case ', tmpTerm, ' of', :nl, indent(6), {:asis,
                                                       value},
                                                        ' ->', {:asis, value}, ';',
                                                                               :nl,
                                                                                   indent(6),
  ...
  def gen_decode_set(gen, typename, r_type() = d) do
    :asn1ct_name.start()
    :asn1ct_name.new(:tag)
    r_SET(tablecinf: tableConsInfo,
        components: tCompList0) = r_type(d, :def)
    tCompList = filter_complist(tCompList0)
    ext = extensible(tCompList)
    toOptional = fn :mandatory ->
                      :OPTIONAL
                    x ->
                      x
                 end
    compList = (case (tCompList) do
                  {rl1, el, rl2} ->
                    rl1 ++ (for (x = r_ComponentType(prop: y)) <- el do
    ...
    case (compList) do
      ...
      _ ->
        ...
        emit([indent(6), {:curr, :else}, ' -> ', :nl, indent(9), '{',
                                                                nextNum, ', ',
                                                                             {:curr,
                                                                                :else},
                                                                                 '}',
    ...
    recordName0 = :lists.concat([get_record_name_prefix(gen),
                                     :asn1ct_gen.list2rname(typename)])
    recordName = :erlang.list_to_atom(recordName0)
    case (gen_dec_sequence_call(gen, typename, compList,
                                  ext, decObjInf)) do
      :no_terms ->
        case (gen) do
          r_gen(pack: :record) ->
  ...
  def gen_encode_sof(erules, typename, _InnerTypename, d)
    ...
    objfun = (case (r_type(d, :tablecinf)) do
                [{:objfun, _} | _R] ->
                  ', ObjFun'
                _ ->
                  ''
              end)
    emit(['   {EncBytes,EncLen} = \'enc_', :asn1ct_gen.list2name(typename), '_components\'(Val', objfun, ',[],0),',
                                                             :nl])
    emit(['   ', {:call, :ber, :encode_tags, ['TagIn', 'EncBytes', 'EncLen']}, '.', :nl,
                                                            :nl])
    gen_encode_sof_components(erules, typename, seqOrSetOf,
                                cont)
  end
  ...
  def gen_decode_sof(erules, typeName, _InnerTypeName, d)
      when elem(d, 0) === :type do
    :asn1ct_name.start()
    {seqOrSetOf, _TypeTag, cont} = (case (r_type(d, :def)) do
                                      {:"SET OF", _Cont} ->
                                        {:"SET OF", :SET, _Cont}
                                      {:"SEQUENCE OF", _Cont} ->
                                        {:"SEQUENCE OF", :SEQUENCE, _Cont}
                                    end)
    typeNameSuffix = :asn1ct_gen.constructed_suffix(seqOrSetOf,
                                                      r_type(cont, :def))
    emit(['   %%-------------------------------------------------', :nl])
    emit(['   %% decode tag and length ', :nl])
    emit(['   %%-------------------------------------------------', :nl])
    :asn1ct_name.new(:tlv)
    emit([{:curr, :tlv}, ' = ', {:call, :ber, :match_tags,
                               [{:prev, :tlv}, 'TagIn']},
                                :com, :nl])
    :asn1ct_name.new(:v)
    emit(['['])
    innerType = :asn1ct_gen.get_inner(r_type(cont, :def))
    contName = (case (:asn1ct_gen.type(innerType)) do
                  atom when is_atom(atom) ->
                    atom
                  _ ->
                    typeNameSuffix
                end)
    gen_dec_line(erules, typeName, contName, [], cont,
                   :mandatory)
    emit([' || ', {:curr, :v}, ' <- ', {:curr, :tlv}, '].', :nl, :nl,
                                                        :nl])
  end

  defp gen_encode_sof_components(gen, typename, seqOrSetOf, r_type() = cont) do
    {objfun, objfun_novar,
       encObj} = (case (r_type(cont, :tablecinf)) do
                    [{:objfun, _} | _R] ->
    ...
    emit(['\'enc_', :asn1ct_gen.list2name(typename), '_components\'([]',
                                                  objfun_novar, ', AccBytes, AccLen) -> ', :nl])
    ...
    emit(['\'enc_', :asn1ct_gen.list2name(typename), '_components\'([H|T]', objfun, ',AccBytes, AccLen) ->',
                                                             :nl])
    typeNameSuffix = :asn1ct_gen.constructed_suffix(seqOrSetOf,
                                                      r_type(cont, :def))
  ...
  def gen_encode_choice(erules, typename, d) when elem(d, 0) === :type do
    ...
    gen_enc_choice(erules, typename, choiceTag, compList1,
                     ext)
    emit([:nl, :nl])
  end
  ...
  def gen_decode_choice(erules, typename, d) when elem(d, 0) === :type do
    ...
    gen_dec_choice(erules, typename, choiceTag, compList1,
                     ext)
    emit(['.', :nl])
  end

  defp gen_enc_sequence_call(erules, topType,
    ...
    cindexPos = (case (order) do
                   :undefined ->
                     pos
                   _ ->
                     order
                 end)
    element = (case (topType) do
                 [:EXTERNAL] ->
    ...
    gen_enc_line(erules, topType, cname, type, element, 3,
                   prop, encObj)
    emit([:com, :nl])
    gen_enc_sequence_call(erules, topType, rest, pos + 1,
                            ext, encObj)
  end
  ...
  defp gen_dec_sequence_call(erules, topType, compList, ext, decObjInf)
    ...
    gen_dec_sequence_call1(erules, topType, compList, 1,
                             ext, decObjInf, [], [])
  end

  defp gen_dec_sequence_call(erules, topType, compList, ext, decObjInf) do
    gen_dec_sequence_call2(erules, topType, compList, ext,
                             decObjInf)
  end

  defp gen_dec_sequence_call1(erules, topType,
            [r_ComponentType(name: cname, typespec: type, prop: prop,
                 tags: tags) |
                 rest],
            num, ext, decObjInf, leadingAttrAcc, argsAcc) do
    {lA, postponedDec} = gen_dec_component(erules, topType,
                                             cname, tags, type, num, prop, ext,
                                             decObjInf)
    emit([:com, :nl])
    case (rest) do
      [] ->
      ...
      _ ->
        :asn1ct_name.new(:bytes)
        gen_dec_sequence_call1(erules, topType, rest, num + 1,
                                 ext, decObjInf, lA ++ leadingAttrAcc,
  ...
  defp gen_dec_sequence_call2(_Erules, _TopType, {[], [], []}, _Ext,
            _DecObjInf) do
    :no_terms
  end

  defp gen_dec_sequence_call2(erules, topType, {root1, eList, root2}, _Ext,
            decObjInf) do
    {lA, argsAcc} = (case (gen_dec_sequence_call1(erules,
                                                    topType, root1 ++ eList, 1,
    ...
    emit([{:curr, :tlv}, ' = ', {:call, :ber,
                               :skip_ExtensionAdditions,
                               [{:prev, :tlv}, {:asis, tagList}]},
                                :com, :nl])
    :asn1ct_name.new(:tlv)
    gen_dec_sequence_call1(erules, topType, root2,
                             length(root1) + length(eList), :noext, decObjInf,
                             lA, argsAcc)
  end
  ...
  defp gen_dec_component(erules, topType, cname, cTags, type, pos, prop,
            ext, decObjInf) do
    innerType = (case (r_type(type, :def)) do
                   r_ObjectClassFieldType(type: oCFTType) ->
                     oCFTType
                   _ ->
                     :asn1ct_gen.get_inner(r_type(type, :def))
                 end)
    prop1 = (case ({prop, ext}) do
    ...
    :asn1ct_name.new(:rb)
    postponedDec = gen_dec_line(erules, topType, cname,
                                  cTags, type, prop1, decObjInf)
    :asn1ct_name.new(:v)
    :asn1ct_name.new(:tlv)
  ...
  defp gen_dec_set_cases(erules, topType, [comp | restComps], pos) do
    ...
    tags = (case (r_type(type, :tag)) do
              [] ->
                for {t1class, t1number} <- cTags do
                  :asn1ct_gen_ber_bin_v2.decode_class(t1class) <<< 10 + t1number
                end
              [firstTag | _] ->
                [:asn1ct_gen_ber_bin_v2.decode_class(r_tag(firstTag, :class)) <<< 10 + r_tag(firstTag, :number)]
            end)
    caseFun = fn tagList = [h | t], fun, n ->
                   semicolon = (case (tagList) do
                                  [_Tag1, _ | _] ->
                                    [';', :nl]
                                  _ ->
                                    ''
  ...
  defp gen_enc_choice2(erules, topType, [h1 | t])
    ...
    emit(['      ', {:asis, cname}, ' ->', :nl])
    {encobj, assign} = (case ({r_type(type, :def),
                                 :asn1ct_gen.get_constraint(r_type(type, :constraint),
  ...
  defp gen_dec_choice(erules, topType, _ChTag, compList, ext) do
    ...
    :asn1ct_name.new(:tlv)
    :asn1ct_name.new(:v)
    emit(['case (case ', {:prev, :tlv}, ' of [Ctemp', {:prev, :tlv}, '] -> Ctemp', {:prev,
                                                    :tlv},
                                                     '; _ -> ', {:prev, :tlv}, ' end)', ' of',
                                                                              :nl])
    ...
    case (ext) do
      ...
      _ ->
        emit([indent(9), '{asn1_ExtAlt,', {:call, :ber, :ber_encode,
                               [{:curr, :else}]},
                                '}', :nl])
    end
    emit([indent(3), 'end', :nl])
  ...
  defp gen_dec_choice_cases(erules, topType, [h | t]) do
    ...
    fcases = fn [{t1class, t1number} | tail], fun ->
    ...
    case ({tags, :asn1ct.get_gen_state_field(:namelist)}) do
      ...
      {[firstT | _RestT], [{^cname, :undecoded} | names]} ->
        decTag = :asn1ct_gen_ber_bin_v2.decode_class(r_tag(firstT, :class)) <<< 10 + r_tag(firstT, :number)
        :asn1ct.add_generated_refed_func({[cname | topType],
                                            :undecoded, [decTag], type})
        :asn1ct.update_gen_state(:namelist, names)
        emit([indent(4), {:curr, :res}, ' = ',
                                            match_tag(r_tag(firstT, :class),
                                                        r_tag(firstT, :number)),
                                                ' -> ', :nl])
        emit([indent(8), '{', {:asis, cname}, ', {\'',
                                                :asn1ct_gen.list2name([cname |
                                                                           topType]),
                                                    '\',', {:curr, :res}, '}};', :nl,
                                                                             :nl])
      {[firstT | restT], _} ->
        emit([indent(4), '{',
                             :asn1ct_gen_ber_bin_v2.decode_class(r_tag(firstT, :class)) <<< 10 + r_tag(firstT, :number),
                                 ', ', {:curr, :v}, '} -> ', :nl])
        emit([indent(8), '{', {:asis, cname}, ', '])
        gen_dec_line(erules, topType, cname, [],
                       r_type(type, tag: restT), prop)
        emit(['};', :nl, :nl])
    end
  ...
  defp match_tag(class, tagNo) when is_integer(tagNo) do
    match_tag1(:asn1ct_gen_ber_bin_v2.decode_class(class),
                 tagNo)
  end
  ...
  defp gen_enc_line(erules, topType, cname,
    ...
    case (:asn1ct_gen.get_constraint(c,
                                       :componentrelation)) do
      {:componentrelation, _, _} ->
        gen_enc_line(erules, topType, cname, type, element,
                       indent, optOrMand, ['{', {:curr, :tmpBytes}, ',_} = '], encObj)
      _ ->
        gen_enc_line(erules, topType, cname, type, element,
                       indent, optOrMand,
  ...
  defp gen_enc_line(erules, topType, cname, type, element, indent,
            optOrMand, encObj)
      when is_list(element) do
    gen_enc_line(erules, topType, cname, type, element,
                   indent, optOrMand,
  ...
  defp gen_enc_line(erules, topType, cname, type, element, indent,
            optOrMand, assign, encObj)
      when is_list(element) do
    indDeep = indent(indent)
    tag = :lists.reverse(for x <- r_type(type, :tag) do
                           :asn1ct_gen_ber_bin_v2.encode_tag_val(:asn1ct_gen_ber_bin_v2.decode_class(r_tag(x, :class)),
                                                                   r_tag(x, :form),
                                                                   r_tag(x, :number))
                         end)
    innerType = :asn1ct_gen.get_inner(r_type(type, :def))
    ...
    emit(indDeep)
    emit(assign)
    gen_optormand_case(optOrMand, erules, topType, cname,
                         type, element)
    case ({type,
             :asn1ct_gen.get_constraint(r_type(type, :constraint),
                                          :componentrelation)}) do
      {r_type(def: r_ObjectClassFieldType(type: {:typefield, _},
        ...
        emit([fun, '(', {:asis, name}, ', ', element, ', ', {:asis,
                                                       restFieldNames},
                                                        '),', :nl])
        emit(indDeep)
        case (optOrMand) do
      ...
      _ ->
        case (whatKind) do
          {:primitive, :bif} ->
            :asn1ct_gen_ber_bin_v2.gen_encode_prim(:ber, type,
                                                     {:asis, tag}, element)
          :ASN1_OPEN_TYPE ->
            case (r_type(type, :def)) do
              r_ObjectClassFieldType() ->
                :asn1ct_gen_ber_bin_v2.gen_encode_prim(:ber,
          ...
          _ ->
            {encFunName, _EncMod, _EncFun} = mkfuncname(topType,
                                                          cname, whatKind, 'enc_', '')
  ...
  defp gen_optormand_case(:mandatory, _Gen, _TopType, _Cname, _Type,
            _Element) do
    :ok
  end

  defp gen_optormand_case(:OPTIONAL, gen, _TopType, _Cname, _Type,
            element) do
  ...
  defp gen_optormand_case({:DEFAULT, defaultValue}, gen, _TopType, _Cname,
            type, element) do
    ...
    case (gen) do
      ...
      r_gen(erule: :ber, der: false, pack: pack) ->
        ind9 = indent(9)
        defMarker = (case (pack) do
                       :record ->
                         :asn1_DEFAULT
                       :map ->
                         :asn1__MISSING_IN_MAP
                     end)
        emit([' case ', element, ' of', :nl, ind9, {:asis, defMarker}, ' ->',
                                                                :nl, ind9,
                                                                         indent(3),
                                                                             '{',
                                                                                 empty_lb(gen),
                                                                                         ...
                                                                                         :nl,
                                                                                             ind9,
                                                                                                 '_ when ',
                                                                                                     element,
                                                                                                         ' =:= '])
        dv = (case (defaultValue) do
                r_Externalvaluereference(module: ^currMod, value: v) ->
                  ['?', {:asis, v}]
                _ ->
                  [{:asis, defaultValue}]
              end)
        emit(dv ++ [' ->', :nl, ind9, indent(3), '{', empty_lb(gen),
                                                    ',0};', :nl, ind9, '_ ->', :nl,
                                                                         indent(12)])
    end
  end

  defp gen_dec_line(erules, topType, cname, cTags, type,
            optOrMand) do
    {[], []} = gen_dec_line(erules, topType, cname, cTags,
                              type, optOrMand, false)
    :ok
  end

  defp gen_dec_line(erules, topType, cname, cTags, type, optOrMand,
            decObjInf) do
    bytesVar = :asn1ct_gen.mk_var(:asn1ct_name.curr(:v))
    tag = (for x <- r_type(type, :tag) do
             :asn1ct_gen_ber_bin_v2.decode_class(r_tag(x, :class)) <<< 10 + r_tag(x, :number)
           end)
    choiceTags = (for {class, number} <- cTags do
    ...
    postpDec = (case (optOrMand) do
                  :mandatory ->
                    gen_dec_call(innerType, erules, topType, cname, type,
                                   bytesVar, tag, :mandatory, ', mandatory, ', decObjInf,
                                   optOrMand)
                  _ ->
                    {firstTag, restTag} = (case (tag) do
                                             [] ->
                                               {choiceTags, []}
                                             [ft | rt] ->
                                               {ft, rt}
                    ...
                    postponedDec = (case (tag) do
                                      ...
                                      [] ->
                                        emit(['[', {:curr, :v}, '|Temp', {:curr, :tlv},
                                                                     '] ->', :nl])
                                        emit([indent(4), '{'])
                                        pdec = gen_dec_call(innerType, erules,
                                                              topType, cname,
                                                              type, bytesVar,
                                                              restTag,
                                                              :mandatory, ', mandatory, ',
                    ...
                    case (optOrMand) do
                      {:DEFAULT, def0} ->
                        ...
                        emit([indent(8), '{', {:asis, def__}, ',', {:prev, :tlv}, '}',
                                                                                  :nl])
                      :OPTIONAL ->
  ...
  defp gen_dec_call({:typefield, _}, _, _, _Cname, type, bytesVar,
            tag, _, _, false, _) do
    :asn1ct_name.new(:reason)
    ...
    emit([indent(9), {:curr, :tmptlv}, ' = ', {:call, :ber,
                                             :decode_open_type,
                                             [bytesVar, {:asis, tag}]},
                                              :com, :nl])
    emit([indent(9), 'case (catch ObjFun(', {:asis, firstPFName}, ', ', {:curr,
                                                    :tmptlv},
                                                     ', ', {:asis, restPFName}, ')) of',
                                                                                 :nl])
  ...
  defp gen_dec_call({:typefield, _}, _, _, cname, type, bytesVar,
            tag, _, _, _DecObjInf, optOrMandComp) do
  ...
  defp gen_dec_call(innerType, gen, topType, cname, type, bytesVar,
            tag, _PrimOptOrMand, _OptOrMand, decObjInf, _) do
    whatKind = :asn1ct_gen.type(innerType)
    gen_dec_call1(whatKind, innerType, topType, cname, type,
                    bytesVar, tag)
    case (decObjInf) do
  ...
  defp gen_dec_call1({:primitive, :bif}, innerType, topType, cname,
            type, bytesVar, tag) do
    case ({:asn1ct.get_gen_state_field(:namelist),
             innerType}) do
  ...
  defp gen_dec_call1(:ASN1_OPEN_TYPE, _InnerType, topType, cname,
            type, bytesVar, tag) do
    case ({:asn1ct.get_gen_state_field(:namelist),
             r_type(type, :def)}) do
      {[{^cname, :undecoded} | rest], _} ->
      ...
      {_, r_ObjectClassFieldType(type: openType)} ->
        :asn1ct_gen_ber_bin_v2.gen_dec_prim(r_type(def: openType),
                                              bytesVar, tag)
      _ ->
        :asn1ct_gen_ber_bin_v2.gen_dec_prim(type, bytesVar, tag)
    end
  ...
  defp gen_dec_call1(whatKind, _, topType, cname, type, bytesVar,
            tag) do
          ...
          [{^cname, list} | rest] when is_list(list) ->
            sindex = (case (whatKind) do
                        r_Externaltypereference() ->
                          ...
                          sI
                        _ ->
                          sI = :asn1ct.maybe_saved_sindex([cname | topType],
                                                            list)
            ...
            suffix = (case (sindex) do
                        i when (is_integer(i) and i > 0) ->
                          :lists.concat(['_', i])
                        _ ->
                          ''
                      end)
            {decFunName, _, _} = mkfuncname(topType, cname,
                                              whatKind, prefix, suffix)
  ...
  defp filter_complist(compList) when is_list(compList) do
    :lists.filter(fn r_ExtensionAdditionGroup() ->
                       false
  ...
  defp mkfuncname(topType, cname, whatKind, prefix, suffix) do
    ...
    case (whatKind) do
      ...
      r_Externaltypereference(module: mod, type: eType) ->
        {:lists.concat(['\'', mod, '\':\'', prefix, eType, suffix, '\'']),
           mod, :lists.concat(['\'', prefix, eType, '\''])}
      {:constructed, :bif} ->
