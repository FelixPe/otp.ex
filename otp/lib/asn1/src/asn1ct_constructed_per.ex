defmodule :m_asn1ct_constructed_per do
  ...
  defp gen_encode_constructed_imm(gen, typename, r_type() = d) do
    ...
    immBody = gen_enc_components_call(gen, typename,
                                        compList, encObj, ext)
  ...
  defp enc_extensions(r_gen(pack: :record), value, extPos, numExt,
            aligned) do
    :asn1ct_imm.per_enc_extensions(value, extPos, numExt,
                                     aligned)
  end

  defp enc_extensions(r_gen(pack: :map), value, extPos, numExt,
            aligned) do
    vars = (for pos <- :lists.seq(extPos,
                                    extPos + numExt - 1) do
              {:var, :lists.concat(['Input@', pos])}
            end)
    undefined = :erlang.atom_to_list(:asn1__MISSING_IN_MAP)
    :asn1ct_imm.per_enc_extensions_map(value, vars,
                                         undefined, aligned)
  end

  defp enc_complist(r_type(def: def__)) do
    case (def__) do
      r_SEQUENCE(tablecinf: tCI, components: cL0,
          extaddgroup: extAddGroup) ->
        case (extAddGroup) do
          :undefined ->
            {cL0, tCI}
  ...
  defp enc_table(gen,
            r_simpletableattributes(objectsetname: objectSet, c_name: attrN, c_index: n,
                usedclassfield: uniqueFieldName,
                uniqueclassfield: uniqueFieldName,
                valueindex: valueIndex0),
            _) do
    {module, objSetName} = objectSet
    r_typedef(typespec: r_ObjectSet(gen: mustGen)) = :asn1_db.dbget(module,
                                                    objSetName)
    case (mustGen) do
      true ->
        valueIndex = valueIndex0 ++ [{n + 1, :ASN1_top}]
        val = make_var(:val)
        {objSetImm, dst} = enc_dig_out_value(gen, valueIndex,
                                               val)
        {{attrN, dst}, objSetImm}
      false ->
        {false, []}
  ...
  defp enc_optionals_1(r_gen(pack: :record) = gen, [{pos, defVals} | t],
            var) do
  ...
  defp enc_optionals_1(r_gen(pack: :map) = gen, [{pos, defVals0} | t],
            v) do
  ...
  defp enc_map_match(r_gen(pack: :map), cs0) do
    ...
    m = (for r_ComponentType(prop: :mandatory, name: name,
                 textual_order: order) <- cs do
    ...
    mand = (case (m) do
              ...
              [_ | _] ->
                patt = {:expr,
                          :lists.flatten(['\#{', :lists.join(',', m), '}'])}
                [{:assign, patt, {:var, :asn1ct_name.curr(:val)}}]
            end)
    os0 = (for r_ComponentType(prop: prop, name: name,
  ...
  defp gen_encode_eag_map([r_ComponentType(name: group, typespec: type) | cs]) do
    case (type) do
      r_type(def: r_SEQUENCE(extaddgroup: g, components: gCs0))
          when is_integer(g) ->
        ns = (for r_ComponentType(name: n, prop: :mandatory) <- gCs0 do
                n
              end)
  ...
  defp gen_encode_eag_record({actualPos, virtualPos, len}) do
    ...
    emit([{:next, :val}, ' = case ', expr, ' of', :nl, 'false -> setelement(', {:asis,
                                                actualPos + 1},
                                                 ', ', {:curr, :val}, ', asn1_NOVALUE);', :nl, 'true -> setelement(',
                                                                               {:asis,
                                                                                  actualPos + 1},
                                                                                   ', ',
                                                                                       {:curr,
  ...
  defp gen_dec_constructed_imm(erule, typename, r_type() = d) do
    {compList, tableConsInfo} = (case (r_type(d, :def)) do
    ...
    emitOpt = (case (optionals) do
                 [] ->
                   :ignore
                 [_ | _] ->
                   gen_dec_optionals(optionals)
               end)
    objSetInfo = (case (tableConsInfo) do
                    r_simpletableattributes(objectsetname: objectSet, c_name: attrN,
                        usedclassfield: uniqueFieldName,
                        uniqueclassfield: uniqueFieldName,
                        valueindex: valIndex) ->
                      f = fn r_ComponentType(typespec: cT) ->
                               case ({:asn1ct_gen.get_constraint(r_type(cT, :constraint),
                                                                   :componentrelation),
                      ...
                      case (:lists.any(f, flat_complist(compList))) do
                        true ->
                          {{attrN,
                              {:deep, objectSet, uniqueFieldName, valIndex}},
    ...
    emitComp = gen_dec_components_call(erule, typename,
                                         compList, decObjInf, ext,
                                         length(optionals))
    ...
    restGroup = {:group,
                   [{:safe, emitObjSets}, {:safe, emitPack}]}
    ...
    fn {accTerm, accBytes} ->
         ...
         case ({accTerm, accBytes}) do
           ...
           {[{objSet, leadingAttr, term}], listOfOpenTypes} ->
             ...
             _ = (for t <- listOfOpenTypes do
                    (
                      gen_dec_open_type(gen, valueMatch, objSet, leadingAttr,
                                          t)
                      emit([:com, :nl])
                    )
                  end)
  ...
  defp dec_external(r_gen(pack: :record) = gen, typename) do
    recordName = :erlang.list_to_atom(record_name(gen,
                                                    typename))
    all = (for term <- :asn1ct_name.all(:term) do
             {:var, term}
           end)
    record = [{:asis, recordName} | all]
    emit(['OldFormat={', :lists.join(',', record), '},', :nl, 'ASN11994Format =', :nl, {:call,
                                                        :ext,
                                                        :transform_to_EXTERNAL1994,
                                                        ['OldFormat']},
                                                         :com, :nl, '{ASN11994Format,', {:curr,
                                                                          :bytes},
                                                                           '}'])
  end
  ...
  defp gen_dec_do_pack(r_gen(pack: :map), _, compList0) do
    ...
    zipped0 = zip_components(compList,
                               :asn1ct_name.all(:term))
    zipped = textual_order(zipped0)
    pF = fn {r_ComponentType(prop: :OPTIONAL), _} ->
              false
            {_, _} ->
              true
         end
    {mandatory, optional} = :lists.partition(pF, zipped)
    l = (for {r_ComponentType(name: name), var} <- mandatory do
           [{:asis, name}, '=>', {:var, var}]
         end)
    emit([{:curr, :res}, ' = \#{', :lists.join(',', l), '}'])
  ...
  defp gen_dec_map_optional([{r_ComponentType(name: name), var} | t]) do
    :asn1ct_name.new(:res)
    emit([:com, :nl, {:curr, :res}, ' = case ', {:var, var}, ' of', :nl,
                                                           '  asn1_NOVALUE -> ', {:prev, :res}, ';',
                                                                                 :nl,
                                                                                     '  _ -> ',
                                                                                         {:prev,
                                                                                            :res},
  ...
  defp gen_dec_merge_maps([m | ms]) do
    :asn1ct_name.new(:res)
    emit([:com, :nl, {:curr, :res}, ' = maps:merge(', {:prev, :res}, ', ',
                                                          {:var, m}, ')'])
  ...
  defp textual_order(l0) do
    l = (for ({r_ComponentType(textual_order: ix), _} = p) <- l0 do
           {ix, p}
         end)
  ...
  defp gen_dec_open_type(erule, val, {xmod, xtype}, leadingAttr,
            {_, {name, restFieldNames}, term, tmpTerm, prop}) do
    ...
    objSet = index_object_set(erule, clType, name, objSet1,
                                classFields)
    key = :erlang.md5(:erlang.term_to_binary({:decode,
                                                objSet, restFieldNames, prop,
                                                extensible}))
    gen = fn _Fd, n ->
  ...
  defp dec_objset_optional_1(n, val) do
    emit([{:asis, n}, '(', {:asis, val}, ', _Id) ->', :nl, {:asis, val},
                                                   ';', :nl])
  end
  ...
  defp dec_objset_default(n, c, leadingAttr, false) do
    emit([{:asis, n}, '(Bytes, Id) ->', :nl, 'exit({\'Type not compatible with table constraint\',{{component,', {:asis, c}, '},{value,Bytes},{unique_name_and_value,', {:asis,
                                                   leadingAttr},
                                                    ',Id}}}).', :nl, :nl])
  end
      ...
      r_Externalvaluereference(module: mod, value: value) ->
        case (:asn1_db.dbget(mod, value)) do
          r_typedef(typespec: r_Object(def: def__)) ->
            ...
            {^nextField, typedef} = :lists.keyfind(nextField, 1,
                                                     fields)
  ...
  defp gen_encode_sof_imm(erule, typename, seqOrSetOf, r_type() = d) do
    ...
    imm0 = (case (:asn1ct_gen.type(conttype)) do
              {:primitive, :bif} ->
              ...
              {:constructed, :bif} ->
                ...
                objArg = (case (r_type(d, :tablecinf)) do
                            [{:objfun, _} | _] ->
                              [{:var, 'ObjFun'}]
                            _ ->
                              []
              ...
              r_Externaltypereference(module: ^currmod, type: ename) ->
                [{:apply, {:local, enc_func(ename), compType},
                    [{:var, 'Comp'}]}]
              r_Externaltypereference(module: eMod, type: ename) ->
                [{:apply, {eMod, enc_func(ename), compType},
                    [{:var, 'Comp'}]}]
              :ASN1_OPEN_TYPE ->
                :asn1ct_gen_per.gen_encode_prim_imm({:var, 'Comp'},
    ...
    :asn1ct_imm.per_enc_sof({:var, 'Val'}, r_type(d, :constraint),
                              :Comp, imm0, aligned)
  end
  ...
  defp do_gen_decode_sof(erules, typeName, seqOrSetOf, d) do
    case (:asn1ct_imm.effective_constraint(:bitstring,
                                             r_type(d, :constraint))) do
      :no ->
        do_gen_decode_fragmented(erules, typeName, seqOrSetOf,
                                   d)
      sizeConstraint ->
        do_gen_decode_sof_plain(erules, typeName, seqOrSetOf, d,
                                  sizeConstraint)
    end
  end

  defp do_gen_decode_fragmented(erules, typeName, seqOrSetOf, d) do
    {_SeqOrSetOf, componentType} = r_type(d, :def)
    objFun = obj_fun_arg(d)
    key = :erlang.md5(:erlang.term_to_binary({:fragmented,
                                                typeName, seqOrSetOf,
                                                componentType}))
    gen = fn _Fd, name ->
  ...
  defp do_gen_dec_fragmented_1(erules, name, typeName, seqOrSetOf,
            componentType, d) do
    objFun = obj_fun_arg(d)
    emit([{:asis, name}, '(Bytes', objFun, ', Acc) ->', :nl])
    {num, buf} = gen_decode_length(:no, erules)
    key = :erlang.md5(:erlang.term_to_binary({typeName,
                                                seqOrSetOf, componentType}))
    gen = fn _Fd, name2 ->
               gen_decode_sof_components(erules, name2, typeName,
                                           seqOrSetOf, componentType, false)
          end
    f = :asn1ct_func.call_gen('dec_fragment', key, gen)
    emit([',', :nl, '{Acc1,Buf1} = ', {:asis, f}, '(', num, ', ', buf, objFun, ', Acc),',
                                                             :nl])
    emit(['if ', num, ' >= 16384 ->', :nl, {:asis, name}, '(Buf1', objFun, ', Acc1);', :nl,
                                                           'true ->', :nl, '{lists:reverse(Acc1),Buf1}', :nl, 'end.',
                                                                               :nl])
  end

  defp do_gen_decode_sof_plain(erules, typeName, seqOrSetOf, d,
            sizeConstraint) do
    {_SeqOrSetOf, componentType} = r_type(d, :def)
    objFun = obj_fun_arg(d)
    ...
    key = :erlang.md5(:erlang.term_to_binary({typeName,
                                                seqOrSetOf, componentType}))
    gen = fn _Fd, name ->
               gen_decode_sof_components(erules, name, typeName,
                                           seqOrSetOf, componentType, true)
          end
    f = :asn1ct_func.call_gen('dec_components', key, gen)
  ...
  defp gen_decode_length(constraint, erule) do
    ...
    imm = :asn1ct_imm.per_dec_length(constraint, true,
                                       is_aligned(erule))
    :asn1ct_imm.dec_slim_cg(imm, 'Bytes')
  end

  defp gen_decode_sof_components(erule, name, typename, seqOrSetOf, cont,
            reverse) do
    ...
    case (:asn1ct_gen.type(conttype)) do
      ...
      {:constructed, :bif} ->
        ...
        emit([{:asis,
                 dec_func(:asn1ct_gen.list2name(newTypename))},
                  '(Bytes', objFun, '),', :nl])
      r_Externaltypereference() = etype ->
        :asn1ct_gen_per.gen_dec_external(etype, 'Bytes')
        emit([:com, :nl])
      :ASN1_OPEN_TYPE ->
        :asn1ct_gen_per.gen_dec_prim(erule,
                                       r_type(def: :ASN1_OPEN_TYPE), 'Bytes')
        emit([:com, :nl])
      _ ->
  ...
  defp obj_fun_arg(r_type(tablecinf: [{:objfun, _} | _])) do
    ', ObjFun'
  end

  defp obj_fun_arg(r_type()) do
    ''
  end

  defp obj_fun_pat(r_type(tablecinf: [{:objfun, _} | _])) do
    ', _'
  end

  defp obj_fun_pat(r_type()) do
    ''
  end
  ...
  defp ext_length([{:ExtensionAdditionGroup, _Num} | t], _,
            acc) do
    ext_length(t, :group, acc)
  end

  defp ext_length([:ExtensionAdditionGroupEnd | t], :group,
            acc) do
  ...
  defp extgrouppos([{:ExtensionAdditionGroup, _Num} | t],
            actualPos, virtualPos, acc) do
  ...
  defp extgrouppos([:ExtensionAdditionGroupEnd | t], actualPos,
            virtualPos, len, acc) do
    extgrouppos(t, actualPos + 1, virtualPos + len,
                  [{actualPos, virtualPos, len} | acc])
  end
  ...
  defp optionals({root1, ext, root2}) do
    ...
    extComps = length(for (c = r_ComponentType()) <- ext do
                        c
                      end)
  ...
  defp optionals([r_ComponentType(typespec: t, prop: {:DEFAULT, val}) | cs],
            pos) do
  ...
  defp create_optionality_table(cs) do
    ...
    optionalsElNum = (for r_ComponentType(prop: o,
                              textual_order: tO) <- cs,
                            isOptional.(o) do
                        tO
                      end)
    {table, _} = :lists.mapfoldl(fn x, num ->
                                      {{num, x}, num + 1}
                                 end,
  ...
  defp add_textual_order1(cs, numIn) do
    :lists.mapfoldl(fn c = r_ComponentType(), num ->
  ...
  defp gen_enc_components_call(erule, topType, {root, extList}, dynamicEnc,
            ext) do
    gen_enc_components_call(erule, topType,
                              {root, extList, []}, dynamicEnc, ext)
  end

  defp gen_enc_components_call(erule, topType, {r1, extList0, r2} = cL,
            dynamicEnc, ext) do
    root = r1 ++ r2
    imm0 = gen_enc_components_call1(erule, topType, root,
                                      dynamicEnc, :noext)
    extImm = (case (ext) do
    ...
    {:extgrouppos,
       extGroupPosLen} = extgroup_pos_and_length(cL)
    extList1 = wrap_extensionAdditionGroups(extList0,
                                              extGroupPosLen)
    extList = (for c <- extList1 do
                 mark_optional(c)
               end)
    imm1 = gen_enc_components_call1(erule, topType, extList,
                                      dynamicEnc, ext)
    imm0 ++ [extImm | imm1]
  end

  defp gen_enc_components_call(erule, topType, compList, dynamicEnc, ext) do
    gen_enc_components_call1(erule, topType, compList,
                               dynamicEnc, ext)
  end
  ...
  defp gen_enc_components_call1(gen, topType, [c | rest], dynamicEnc, ext) do
    r_ComponentType(name: cname, typespec: type, prop: prop,
        textual_order: num) = c
    innerType = :asn1ct_gen.get_inner(r_type(type, :def))
    ...
    [imm | gen_enc_components_call1(gen, topType, rest,
                                      dynamicEnc, ext)]
  end
  ...
  defp def_values(r_type(def: {:"BIT STRING", []}), bs) when is_bitstring(bs) do
    case (:asn1ct.use_legacy_types()) do
      false ->
        [:asn1_DEFAULT, bs]
      true ->
        listBs = (for << <<b :: size(1)>> <- bs >> do
                    b
                  end)
        intBs = :lists.foldl(fn b, a ->
                                  a <<< 1 ||| b
                             end,
                               0, :lists.reverse(listBs))
        sz = bit_size(bs)
        compact = (case (8 - rem(sz, 8)) do
                     8 ->
                       {0, bs}
                     unused ->
  ...
  defp def_values(r_type(def: {:"BIT STRING", [_ | _] = ns}), list)
    ...
    as = (case (:asn1ct.use_legacy_types()) do
            false ->
              [list, bs]
            true ->
              listBs = (for << <<b :: size(1)>> <- bs >> do
  ...
  defp def_values(r_type(def: {:INTEGER, ns}), def__) do
    [:asn1_DEFAULT, def__ | case (:lists.keyfind(def__, 2,
                                                   ns)) do
                              false ->
                                []
                              {val, ^def__} ->
  ...
  defp gen_enc_line_imm(erule, topType, cname, type, element,
            dynamicEnc, ext) do
  ...
  defp gen_enc_line_imm_1(erule, topType, cname, type, element,
            dynamicEnc) do
    atype = (case (type) do
  ...
  defp enc_var_type_call(erule, name, restFieldNames, r_type(tablecinf: tCI),
            fun, val) do
    ...
    objSet = index_object_set(erule, clType, name, objSet1,
                                classFields)
    key = :erlang.md5(:erlang.term_to_binary({:encode,
                                                objSet, restFieldNames,
                                                extensible}))
  ...
  defp enc_objset_imm(erule, typeName, component, objSet,
            restFieldNames, extensible) do
    aligned = is_aligned(erule)
    e = {:error,
           fn () ->
                emit(['exit({\'Type not compatible with table constraint\',{component,', {:asis, component}, '},{value,Val},{unique_name_and_value,\'_\'}})', :nl])
           end}
    [{:cond,
        (for {key, obj} <- objSet do
           [{:eq, {:var, 'Id'}, key} | enc_obj(erule, obj, typeName,
                                              restFieldNames, aligned)]
         end) ++ [[:_, case (extensible) do
                         false ->
                           e
                         true ->
                           case (:asn1ct.use_legacy_types()) do
  ...
  defp enc_obj(erule, obj, typeName, restFieldNames0,
            aligned) do
    val = {:var, 'Val'}
    case (obj) do
      r_typedef(name: {:constructed, :bif}, typespec: type) = def__ ->
      ...
      r_typedef(name: {:primitive, :bif}, typespec: def__) ->
        :asn1ct_gen_per.gen_encode_prim_imm({:var, 'Val'}, def__,
                                              aligned)
      r_typedef(name: type) ->
      ...
      r_Externalvaluereference(module: mod, value: value) ->
        case (:asn1_db.dbget(mod, value)) do
          r_typedef(typespec: r_Object(def: def__)) ->
            ...
            {^nextField, typedef} = :lists.keyfind(nextField, 1,
                                                     fields)
            enc_obj(erule, typedef, typeName, restFieldNames,
                      aligned)
        end
      r_Externaltypereference(module: mod, type: type) ->
        func = enc_func(type)
  ...
  defp gen_dec_components_call(erule, topType, {root, extList}, decInfObj, ext,
            numberOfOptionals) do
    gen_dec_components_call(erule, topType,
                              {root, extList, []}, decInfObj, ext,
                              numberOfOptionals)
  end

  defp gen_dec_components_call(gen, topType, {root1, extList, root2} = cL,
            decInfObj, ext, numberOfOptionals) do
    optTable = create_optionality_table(root1 ++ root2)
    init = {:ignore,
              fn _ ->
                   {[], []}
              end}
    {emitRoot, tpos} = gen_dec_comp_calls(root1 ++ root2,
                                            gen, topType, optTable, decInfObj,
                                            :noext, numberOfOptionals, 1, [])
    emitGetExt = gen_dec_get_extension(gen)
    {:extgrouppos,
       extGroupPosLen} = extgroup_pos_and_length(cL)
    newExtList = wrap_extensionAdditionGroups(extList,
                                                extGroupPosLen)
    {emitExts, _} = gen_dec_comp_calls(newExtList, gen,
                                         topType, optTable, decInfObj, ext,
                                         numberOfOptionals, tpos, [])
    ...
    [init] ++ emitRoot ++ [emitGetExt |
                               emitExts] ++ [finish]
  end

  defp gen_dec_components_call(erule, topType, compList, decInfObj, ext,
            numberOfOptionals) do
    ...
    {cs, _} = gen_dec_comp_calls(compList, erule, topType,
                                   optTable, decInfObj, ext, numberOfOptionals,
                                   1, [])
    [init | cs]
  end
  ...
  defp gen_dec_get_extension(erule) do
    ...
    e = fn imm, st ->
             emit([:nl, '%% Extensions', :nl, '{Extensions,', {:next, :bytes}, '} = ', 'case Ext of', :nl, '0 -> {<<>>,',
                                                                   {:curr,
                                                                      :bytes},
                                                                       '};', :nl,
                                                                              '1 ->',
  ...
  defp gen_dec_comp_calls([c | cs], erule, topType, optTable, decInfObj,
            ext, numberOfOptionals, tpos, acc) do
    l = gen_dec_comp_call(c, erule, topType, tpos, optTable,
                            decInfObj, ext, numberOfOptionals)
    gen_dec_comp_calls(cs, erule, topType, optTable,
                         decInfObj, ext, numberOfOptionals, tpos + 1, [l | acc])
  end
  ...
  defp gen_dec_comp_call(comp, gen, topType, tpos, optTable, decInfObj,
            ext, numberOfOptionals) do
    r_ComponentType(name: cname, typespec: type, prop: prop,
        textual_order: textPos) = comp
    pos = (case (ext) do
             :noext ->
               tpos
             {:ext, epos, _Enum} ->
    ...
    innerType = :asn1ct_gen.get_inner(r_type(type, :def))
    commentString = attribute_comment(innerType, textPos,
                                        cname)
    comment = fn st ->
                   emit([:nl, '%% ', commentString, :nl])
                   st
              end
    preamble = (case ({innerType,
                         is_mandatory_predef_tab_c(ext, prop, decInfObj)}) do
                  {{:typefield, _}, true} ->
                    fn st ->
    ...
    {pre, post} = comp_call_pre_post(gen, ext, prop, pos,
                                       type, textPos, optTable,
                                       numberOfOptionals, ext)
    lines = gen_dec_seq_line_imm(gen, topType, comp, tpos,
                                   decInfObj, ext)
    advBuffer = {:ignore,
                   fn st ->
                        :asn1ct_name.new(:bytes)
  ...
  defp comp_call_pre_post(_Gen, :noext, prop, _, type, textPos, optTable,
            numOptionals, ext) do
    optPos = get_optionality_pos(textPos, optTable)
    element = (case (numOptionals - optPos) do
                 0 ->
                   'Opt band 1'
                 shift ->
  ...
  defp comp_call_pre_post(gen, {:ext, _, _}, prop, pos, type, _, _, _,
            ext) do
    {[fn st ->
  ...
  defp extadd_group_fun(r_gen(pack: :record), prop, type, ext) do
    fn st ->
         emit([';', :nl, '_  ->', :nl, '{'])
         case (type) do
           r_type(def: r_SEQUENCE(extaddgroup: number2,
  ...
  defp dec_map_extaddgroup_no_val(ext, type, comp) do
    l0 = (for r_ComponentType(name: n, prop: p) <- comp do
  ...
  defp gen_dec_choice_line(erule, topType, comp, pre) do
    imm0 = gen_dec_line_imm(erule, topType, comp, false,
                              pre)
    init = {:ignore,
              fn _ ->
                   {[], []}
  ...
  defp gen_dec_line_special(erule, {:typefield, _}, _TopType, comp,
            decInfObj) do
    ...
    fn {_BytesVar, prevSt} ->
         case (decInfObj) do
           false ->
             ...
             imm = :asn1ct_imm.per_dec_open_type(is_aligned(erule))
             bytesVar = :asn1ct_gen.mk_var(:asn1ct_name.curr(:bytes))
             {tmpTerm, tempBuf} = :asn1ct_imm.dec_slim_cg(imm,
                                                            bytesVar)
             emit([:com, :nl])
             r_type(tablecinf: [{:objfun,
                              r_Externaltypereference(module: xmod, type: xtype)}]) = type
             gen_dec_open_type(erule, 'ObjFun', {xmod, xtype}, :_,
                                 {:_, {name, restFieldNames}, :Result, tmpTerm,
                                    :mandatory})
           ...
           {'got objfun through args', 'ObjFun'} ->
             ...
             cond do
               prop === :mandatory ->
                 gen_dec_open_type(erule, 'ObjFun', {xmod, xtype}, :_,
                                     {:_, {name, restFieldNames}, term, tmpTerm,
                                        prop})
               true ->
                 emit(['     {'])
                 gen_dec_open_type(erule, 'ObjFun', {xmod, xtype}, :_,
                                     {:_, {name, restFieldNames}, :_, tmpTerm,
                                        prop})
  ...
  defp gen_enc_choice(erule, topType, {root, exts}, ext) do
    ...
    gen_enc_choices(root, erule, topType, 0, constr,
                      ext) ++ gen_enc_choices(exts, erule, topType, 0, :ext,
                                                ext)
  end
  ...
  defp gen_enc_choices([h | t], erule, topType, pos, constr, ext) do
    ...
    aligned = is_aligned(erule)
    encObj = (case (:asn1ct_gen.get_constraint(r_type(type, :constraint),
                                                 :componentrelation)) do
                :no ->
                  case (r_type(type, :tablecinf)) do
                    [{:objfun, _} | _] ->
                      {'got objfun through args', {:var, 'ObjFun'}}
    ...
    doExt = (case (constr) do
               :ext ->
                 ext
               _ ->
                 :noext
             end)
    tag = (case ({ext, constr}) do
             {:noext, _} ->
    ...
    body = gen_enc_line_imm(erule, topType, cname, type,
                              {:var, 'ChoiceVal'}, encObj, doExt)
    imm = tag ++ body
    [{cname, imm} | gen_enc_choices(t, erule, topType,
                                      pos + 1, constr, ext)]
  end
  ...
  defp emit_getchoice(erule, compList, ext) do
    al = is_aligned(erule)
    imm = (case ({ext, compList}) do
             {:noext, [_]} ->
               {:value, 0}
             {:noext, _} ->
               :asn1ct_imm.per_dec_constrained(0, length(compList) - 1,
                                                 al)
             {{:ext, _, _}, _} ->
  ...
  defp get_input_vars_1(val, i, n) do
    [get_input_var(val, i) | get_input_vars_1(val, i + 1,
                                                n - 1)]
  end
  ...
  defp wrap_eags([{:ExtensionAdditionGroup, _Number} | t0],
            [{actualPos, _, _} | gs], _ExtAddGroupDiff,
            extGroupNum) do
    {extGroupCompList,
       [:ExtensionAdditionGroupEnd |
            t]} = :lists.splitwith(fn r_ComponentType() ->
                                        true
                                      _ ->
                                        false
                                   end,
                                     t0)
    name = :erlang.list_to_atom(:lists.concat(['ExtAddGroup',
                                                   extGroupNum + 1]))
    seq = r_type(def: r_SEQUENCE(extaddgroup: extGroupNum + 1,
                     components: extGroupCompList))
    comp = r_ComponentType(name: name, typespec: seq,
               textual_order: actualPos, prop: :OPTIONAL)
    [comp | wrap_eags(t, gs, length(extGroupCompList) - 1,
                        extGroupNum + 1)]
  end

  defp wrap_eags([r_ComponentType(textual_order: tord) = h | t],
    ...
    [comp | wrap_eags(t, extAddGrpLenPos, extAddGroupDiff,
                        extGroupNum)]
  end

  defp wrap_eags([h | t], extAddGrpLenPos, extAddGroupDiff,
            extGroupNum) do
    [h | wrap_eags(t, extAddGrpLenPos, extAddGroupDiff,
                     extGroupNum)]
  end
  ...
  defp make_var(base) do
    {:var,
       :erlang.atom_to_list(:asn1ct_gen.mk_var(:asn1ct_name.curr(base)))}
  end

  defp attribute_comment(innerType, textPos, cname) do
    dispType = (case (innerType) do
