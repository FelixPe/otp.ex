defmodule :m_asn1ct_constructed_per do
    ...
    immSetExt =
      case ext do
        {:ext, _Pos, numExt2} when numExt2 > 0 ->
          :asn1ct_imm.per_enc_extension_bit({:var, ~c"Extensions"}, aligned)
  ...
  defp enc_table(_Gen, _, r_type(tablecinf: tCInf)) do
    case tCInf do
      [{:objfun, _} | _] ->
        {{~c"got objfun through args", {:var, ~c"ObjFun"}}, []}
  ...
  defp enc_optionals_1(r_gen(pack: :map) = gen, [{pos, defVals0} | t], v) do
    var = {:var, :lists.concat([~c"Input@", pos])}
  ...
  defp enc_map_match(r_gen(pack: :map), cs0) do
    ...
    m =
      for r_ComponentType(prop: :mandatory, name: name, textual_order: order) <- cs do
        [quote_atom(name), ~c":=", :lists.concat([var0, ~c"@", order])]
      end
    ...
    mand =
      case m do
        ...
        [_ | _] ->
          patt = {:expr, :lists.flatten([~c"\#{", :lists.join(~c",", m), ~c"}"])}
  ...
  defp any_non_value_1(val, pos, n) do
    ...
    [var ++ ~c" =/= asn1_NOVALUE" | any_non_value_1(val, pos + 1, n - 1)]
  end
      ...
      case tableConsInfo do
        ...
        _ ->
          case r_type(d, :tablecinf) do
            [{:objfun, _} | _] ->
              {{~c"got objfun through args", ~c"ObjFun"}, false, false}
  ...
  defp gen_dec_pack(gen, typename, compList) do
    case typename do
      ...
      _ ->
        ...
        emit([:com, :nl, ~c"{", {:curr, :res}, ~c",", {:curr, :bytes}, ~c"}"])
    end
  end
  ...
  defp gen_dec_do_pack(r_gen(pack: :record) = gen, typeName, compList) do
    ...
    recordName = [~c"'", record_name(gen, typeName), ~c"'"]
  ...
  defp gen_dec_merge_maps([m | ms]) do
    :asn1ct_name.new(:res)
    emit([:com, :nl, {:curr, :res}, ~c" = maps:merge(", {:prev, :res}, ~c", ", {:var, m}, ~c")"])
  ...
  defp gen_dec_open_type(
    ...
    prefix = :lists.concat([~c"dec_os_", name])
    f = :asn1ct_func.call_gen(prefix, key, gen)
    emit([term, ~c" = ", {:asis, f}, ~c"(", tmpTerm, ~c", ", val, ~c")"])
  end
  ...
  defp dec_objset_optional_1(n, val) do
    emit([{:asis, n}, ~c"(", {:asis, val}, ~c", _Id) ->", :nl, {:asis, val}, ~c";", :nl])
  end
  ...
  defp dec_objset_default(n, c, leadingAttr, false) do
    emit([
      ...
      ~c"(Bytes, Id) ->",
      :nl,
      ~c"exit({'Type not compatible with table constraint',{{component,",
      {:asis, c},
      ~c"},{value,Bytes},{unique_name_and_value,",
      {:asis, leadingAttr},
      ~c",Id}}}).",
      :nl,
      :nl
  ...
  defp dec_objset_1(erule, n, {id, obj}, restFields, typename) do
    emit([{:asis, n}, ~c"(Bytes, Id) when Id =:= ", {:asis, id}, ~c" ->", :nl])
  ...
  defp dec_objset_2(erule, obj, restFields0, typename) do
    case obj do
      ...
      r_typedef(name: {:constructed, :bif}, typespec: type) = def__ ->
        ...
        emit([~c"{Term,_} = ", {:asis, func}, ~c"(Bytes)", :com, :nl, ~c"Term"])

      r_typedef(name: type) ->
        emit([~c"{Result,_} = ", {:asis, enc_func(~c"dec_", type)}, ~c"(Bytes),", :nl, ~c"Result"])

      r_Externaltypereference(module: mod, type: type) ->
        emit(~c"{Term,_} = ")
        func = enc_func(~c"dec_", type)

        case :erlang.get(:currmod) do
          ^mod ->
            emit([{:asis, func}, ~c"(Bytes)"])

          _ ->
            emit([{:asis, mod}, ~c":", {:asis, func}, ~c"(Bytes)"])
        end
  ...
  defp gen_encode_choice_imm(erule, topType, r_type(def: {:CHOICE, compList})) do
    ...
    [
      {:assign, {:expr, ~c"{ChoiceTag,ChoiceVal}"}, ~c"Val"}
      | :asn1ct_imm.per_enc_choice({:var, ~c"ChoiceTag"}, cs, aligned)
    ]
  end
  ...
  defp gen_encode_sof_imm(erule, typename, seqOrSetOf, r_type() = d) do
    ...
    imm0 =
      case :asn1ct_gen.type(conttype) do
        {:primitive, :bif} ->
          :asn1ct_gen_per.gen_encode_prim_imm({:var, ~c"Comp"}, componentType, aligned)
        ...
        {:constructed, :bif} ->
          ...
          [{:apply, {:local, enc, compType}, [{:var, ~c"Comp"} | objArg]}]

        r_Externaltypereference(module: ^currmod, type: ename) ->
          [{:apply, {:local, enc_func(ename), compType}, [{:var, ~c"Comp"}]}]

        r_Externaltypereference(module: eMod, type: ename) ->
          [{:apply, {eMod, enc_func(ename), compType}, [{:var, ~c"Comp"}]}]
    ...
    :asn1ct_imm.per_enc_sof({:var, ~c"Val"}, r_type(d, :constraint), :Comp, imm0, aligned)
  end
  ...
  defp do_gen_decode_sof(erules, typeName, seqOrSetOf, d) do
    case :asn1ct_imm.effective_constraint(
           :bitstring,
           r_type(d, :constraint)
         ) do
      :no ->
  ...
  defp do_gen_decode_fragmented(erules, typeName, seqOrSetOf, d) do
    ...
    key = :erlang.md5(:erlang.term_to_binary({:fragmented, typeName, seqOrSetOf, componentType}))

    gen = fn _Fd, name ->
      do_gen_dec_fragmented_1(erules, name, typeName, seqOrSetOf, componentType, d)
    end

    f = :asn1ct_func.call_gen(~c"dec_components", key, gen)
    emit([{:asis, f}, ~c"(Bytes", objFun, ~c", [])"])
  end

  defp do_gen_dec_fragmented_1(erules, name, typeName, seqOrSetOf, componentType, d) do
    objFun = obj_fun_arg(d)
    emit([{:asis, name}, ~c"(Bytes", objFun, ~c", Acc) ->", :nl])
    {num, buf} = gen_decode_length(:no, erules)
    key = :erlang.md5(:erlang.term_to_binary({typeName, seqOrSetOf, componentType}))

    gen = fn _Fd, name2 ->
      gen_decode_sof_components(erules, name2, typeName, seqOrSetOf, componentType, false)
    end

    f = :asn1ct_func.call_gen(~c"dec_fragment", key, gen)
  ...
  defp do_gen_decode_sof_plain(erules, typeName, seqOrSetOf, d, sizeConstraint) do
    {_SeqOrSetOf, componentType} = r_type(d, :def)
    objFun = obj_fun_arg(d)
    ...
    gen = fn _Fd, name ->
      gen_decode_sof_components(erules, name, typeName, seqOrSetOf, componentType, true)
    end

    f = :asn1ct_func.call_gen(~c"dec_components", key, gen)
    emit([~c",", :nl, {:asis, f}, ~c"(", num, ~c", ", buf, objFun, ~c", [])"])
  end
  ...
  defp gen_decode_length(constraint, erule) do
    emit([~c"%% Length with constraint ", {:asis, constraint}, :nl])
    imm = :asn1ct_imm.per_dec_length(constraint, true, is_aligned(erule))
    :asn1ct_imm.dec_slim_cg(imm, ~c"Bytes")
  end

  defp gen_decode_sof_components(erule, name, typename, seqOrSetOf, cont, reverse) do
    objFun = obj_fun_arg(cont)
    objFunPat = obj_fun_pat(cont)
    emit([{:asis, name}, ~c"(0, Bytes", objFunPat, ~c", Acc) ->", :nl])

    case reverse do
      true ->
        emit([~c"{lists:reverse(Acc),Bytes};", :nl])

      false ->
        emit([~c"{Acc,Bytes};", :nl])
    end

    emit([{:asis, name}, ~c"(Num, Bytes", objFun, ~c", Acc) ->", :nl, ~c"{Term,Remain} = "])
    ...
    case :asn1ct_gen.type(conttype) do
      {:primitive, :bif} ->
        :asn1ct_gen_per.gen_dec_prim(erule, cont, ~c"Bytes")
        emit([:com, :nl])
      ...
      {:constructed, :bif} ->
        ...
        emit([
          {:asis, dec_func(:asn1ct_gen.list2name(newTypename))},
          ~c"(Bytes",
          objFun,
          ~c"),",
          :nl
      ...
      r_Externaltypereference() = etype ->
        :asn1ct_gen_per.gen_dec_external(etype, ~c"Bytes")
        emit([:com, :nl])

      :ASN1_OPEN_TYPE ->
        :asn1ct_gen_per.gen_dec_prim(erule, r_type(def: :ASN1_OPEN_TYPE), ~c"Bytes")
        emit([:com, :nl])

      _ ->
        emit([{:asis, dec_func(conttype)}, ~c"(Bytes),", :nl])
    end

    emit([{:asis, name}, ~c"(Num-1, Remain", objFun, ~c", [Term|Acc]).", :nl])
  end
  ...
  defp enc_var_type_call(erule, name, restFieldNames, r_type(tablecinf: tCI), fun, val) do
    ...
    lambda = {:lambda, [{:var, ~c"Val"}, {:var, ~c"Id"}], imm}

    gen = fn _Fd, n ->
      aligned = is_aligned(erule)
      emit([{:asis, n}, ~c"(Val, Id) ->", :nl])
      :asn1ct_imm.enc_cg(imm, aligned)
      emit([~c".", :nl])
    end

    prefix = :lists.concat([~c"enc_os_", name])
      ...
      {:error,
       fn ->
         emit([
           ~c"exit({'Type not compatible with table constraint',{component,",
           {:asis, component},
           ~c"},{value,Val},{unique_name_and_value,'_'}})",
           :nl
         ])
       end}
    ...
    [
      {:cond,
       for {key, obj} <- objSet do
         [{:eq, {:var, ~c"Id"}, key} | enc_obj(erule, obj, typeName, restFieldNames, aligned)]
       end ++
         [
           [
             :_,
             case extensible do
               ...
               true ->
                 case :asn1ct.use_legacy_types() do
                   false ->
                     {:call, :per_common, :open_type_to_binary, [{:var, ~c"Val"}]}

                   true ->
                     {:call, :per_common, :legacy_open_type_to_binary, [{:var, ~c"Val"}]}
                 end
             end
           ]
         ]}
  ...
  defp enc_obj(erule, obj, typeName, restFieldNames0, aligned) do
    ...
    case obj do
      ...
      r_typedef(name: {:primitive, :bif}, typespec: def__) ->
        :asn1ct_gen_per.gen_encode_prim_imm({:var, ~c"Val"}, def__, aligned)

      r_typedef(name: type) ->
        [{:apply, {:local, enc_func(type), type}, [{:var, ~c"Val"}]}]
      ...
      r_Externaltypereference(module: mod, type: type) ->
        ...
        case :erlang.get(:currmod) do
          ^mod ->
            [{:apply, {:local, func, obj}, [{:var, ~c"Val"}]}]

          _ ->
            [{:apply, {mod, func, obj}, [{:var, ~c"Val"}]}]
        end
    end
  end

  defp gen_enc_obj(erules, name, typename, type) do
    emit([{:asis, name}, ~c"(Val) ->", :nl])
    innerType = :asn1ct_gen.get_inner(r_type(type, :def))
  ...
  defp gen_dec_components_call(
    ...
    finish = fn st ->
      ...
      :asn1ct_func.call(mod, :skipextensions, [{:curr, :bytes}, numExtsToSkip + 1, ~c"Extensions"])
  ...
  defp gen_dec_get_extension(erule) do
    ...
    e = fn imm, st ->
      ...
      emit([:com, :nl, ~c"{", dst, ~c",", dstBuf, ~c"}", :nl, ~c"end"])
      :asn1ct_name.new(:bytes)
      st
    end
      ...
      case {innerType, is_mandatory_predef_tab_c(ext, prop, decInfObj)} do
        {{:typefield, _}, true} ->
          fn st ->
            ...
            emit([~c"{", {:curr, :tmpterm}, ~c", ", {:next, :bytes}, ~c"} = "])
            st
          end
  ...
  defp dec_match_extadd_fun(r_gen(pack: :map), _CompList) do
    fn st ->
      :asn1ct_name.new(:map)
      emit([~c"{", {:curr, :map}, ~c",", {:next, :bytes}, ~c"} = "])
      st
    end
  end
  ...
  defp comp_call_pre_post(_Gen, :noext, prop, _, type, textPos, optTable, numOptionals, ext) do
    ...
    element =
      case numOptionals - optPos do
        ...
        shift ->
          :lists.concat([~c"(Opt bsr ", shift, ~c") band 1"])
      end

    {[
       fn st ->
         emit([~c"case ", element, ~c" of", :nl, ~c"1 ->", :nl])
         st
       end
     ],
     [
  ...
  defp comp_call_pre_post(gen, {:ext, _, _}, prop, pos, type, _, _, _, ext) do
    {[
       fn st ->
         emit([~c"case Extensions of", :nl, ~c"  <<_:", pos - 1, ~c",1:1,_/bitstring>> ->", :nl])
         st
       end
  ...
  defp is_mandatory_predef_tab_c(:noext, :mandatory, {~c"got objfun through args", ~c"ObjFun"}) do
    true
  end

  defp is_mandatory_predef_tab_c(_, _, {~c"got objfun through args", ~c"ObjFun"}) do
    false
  end
  ...
  defp gen_dec_line_open_type(erule, {:ext, ep, _}, pos) when pos >= ep do
    ...
    {:safe,
     fn st ->
       ...
       bytesVar = :asn1ct_gen.mk_var(:asn1ct_name.curr(:bytes))
       {dst, dstBuf} = :asn1ct_imm.dec_slim_cg(imm, bytesVar)
       emit([~c",", :nl, ~c"{TmpValx", pos, ~c",_} = "])

       {dst,
        fn ->
          emit([~c",", :nl, ~c"{TmpValx", pos, ~c",", dstBuf, ~c"}", :nl, ~c"end"])
          st
        end}
     end}
  end
    ...
    fn {_BytesVar, prevSt} ->
      case decInfObj do
        false ->
          ...
          emit([:com, :nl, ~c"{", {:asis, cname}, ~c",{Result,", tempBuf, ~c"}}"])
          {[], prevSt}
        ...
        {~c"got objfun through args", ~c"ObjFun"} ->
          cond do
            ...
            true ->
              ...
              emit([:nl, ~c"    {", {:curr, :tmpterm}, ~c", ", {:curr, :tmpbytes}, ~c"} ="])
          end
    ...
    case :asn1ct_gen.type(atype) do
      ...
      {:constructed, :bif} ->
        ...
        case r_type(type, :tablecinf) do
          [{:objfun, _} | _R] ->
            fn bytesVar ->
              emit([{:asis, decFunc}, ~c"(", bytesVar, ~c", ObjFun)"])
            end

          _ ->
            fn bytesVar ->
              emit([{:asis, decFunc}, ~c"(", bytesVar, ~c")"])
            end
        end
    end
  end
  ...
  defp gen_enc_choices([h | t], erule, topType, pos, constr, ext) do
    ...
    body = gen_enc_line_imm(erule, topType, cname, type, {:var, ~c"ChoiceVal"}, encObj, doExt)
    imm = tag ++ body
  ...
  defp gen_dec_choice(erule, topType, compList, {:ext, _, _} = ext) do
    ...
    case rootList do
      ...
      [_ | _] ->
        emit([~c"<<0:1,Bytes1/bitstring>> ->", :nl])
        :asn1ct_name.new(:bytes)
        gen_dec_choice1(erule, topType, rootList, :noext)
        emit([~c";", :nl, :nl])
    end

    emit([~c"<<1:1,Bytes1/bitstring>> ->", :nl])
    :asn1ct_name.clear()
    :asn1ct_name.new(:bytes)
    :asn1ct_name.new(:bytes)
  ...
  defp gen_dec_choice1(erule, topType, compList, {:ext, _, _} = ext) do
    ...
    bytesVar = :asn1ct_gen.mk_var(:asn1ct_name.curr(:bytes))
    {dst, dstBuf} = :asn1ct_imm.dec_slim_cg(imm, bytesVar)
    emit([:nl, ~c"end,", :nl, ~c"case Choice of", :nl])
    ...
    emit([~c"_ ->", :nl, ~c"{{asn1_ExtAlt,", dst, ~c"},", dstBuf, ~c"}", :nl, ~c"end"])
  end
  ...
  defp gen_dec_choice2(erule, topType, [h0 | t], pos, sep0, pre) do
    ...
    case r_type(type, :def) do
      r_ObjectClassFieldType(type: {:typefield, _}) ->
        emit(~c"{Cname,{Val,NewBytes}} = begin\n")
        gen_dec_choice_line(erule, topType, h, pre)
        emit([:nl, ~c"end,", :nl, ~c"{{Cname,Val},NewBytes}"])

      _ ->
        emit(~c"{Val,NewBytes} = begin\n")
        gen_dec_choice_line(erule, topType, h, pre)
        emit([:nl, ~c"end,", :nl, ~c"{{", {:asis, cname}, ~c",Val},NewBytes}"])
    end
  ...
  defp get_input_var(val, i) do
    :lists.flatten(:io_lib.format(~c"element(~w, ~s)", [i + 1, val]))
  end
  ...
  defp wrap_eags(
    ...
    name = :erlang.list_to_atom(:lists.concat([~c"ExtAddGroup", extGroupNum + 1]))
  ...
  defp attribute_comment(innerType, textPos, cname) do
    ...
    comment = [~c"attribute ", cname, ~c"(", textPos, ~c") with type ", dispType]
