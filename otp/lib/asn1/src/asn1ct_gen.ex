defmodule :m_asn1ct_gen do
  ...
  def pgen(outFile, r_gen(options: options) = gen, code) do
    ...
    n2nConvEnums = (for {:n2n, cName} <- options do
                      cName
                    end)
  ...
  defp pgen_typeorval(erules, n2nConvEnums, code) do
    r_abst(name: module, types: types, values: values,
    ...
    case (erules) do
      r_gen(erule: :jer) ->
        ...
        emit(['typeinfo(Type) ->', :nl, '  exit({error,{asn1,{undefined_type,Type}}}).', :nl, :nl])
    ...
    case (r_gen(erules, :jer)) do
      true ->
        ...
        pgen_types(jER_Rtmod,
                     r_gen(erules, erule: :jer,  jer: false), [], module, types)
        emit(['typeinfo(Type) ->', :nl, '  exit({error,{asn1,{undefined_type,Type}}}).', :nl, :nl])
      false ->
        :ok
    end
  ...
  defp pgen_values([h | t], module) do
    ...
    emit([{:asis, name}, '() ->', :nl, {:asis, value}, '.', :nl,
                                                        :nl])
  ...
  defp pgen_n2nconversion(_Erules,
            r_typedef(name: typeName,
                typespec: r_type(def: {:ENUMERATED, {nN1, nN2}}))) do
    nN = nN1 ++ nN2
  ...
  defp pgen_n2nconversion(_Erules,
            r_typedef(name: typeName,
                typespec: r_type(def: {:ENUMERATED, nN}))) do
  ...
  defp pgen_name2numfunc1(funcName, [{atom, number}],
            :extension_marker) do
    emit([{:asis, funcName}, '(', {:asis, atom}, ') ->', number, ';',
                                                              :nl])
  ...
  defp pgen_num2namefunc1(funcName, [{atom, number}],
            :extension_marker) do
    emit([{:asis, funcName}, '(', number, ') ->', {:asis, atom}, ';',
                                                              :nl])
  ...
  defp pgen_partial_inc_dec(rtmod, erules, module) do
    case (:asn1ct.get_gen_state_field(:inc_type_pattern)) do
      :undefined ->
        :ok
      confList ->
        patternLists = :lists.map(fn {_, p} ->
                                       p
                                  end,
                                    confList)
        pgen_partial_inc_dec1(rtmod, erules, module,
                                patternLists)
  ...
  defp pgen_partial_inc_dec1(rtmod, erules, module, [p | ps]) do
    ...
    rtmod.gen_decode(erules, typeDef)
    gen_dec_part_inner_constr(rtmod, erules, typeDef,
                                [topTypeName])
  ...
  defp gen_partial_inc_dec_refed_funcs(rtmod, r_gen(erule: :ber) = gen) do
    case (:asn1ct.next_refed_func()) do
      ...
      {name, sindex, pattern, type} ->
        typeDef = r_typedef(name: :asn1ct_gen.list2name(name),
                      typespec: type)
        :asn1ct.update_gen_state(:namelist, pattern)
        :asn1ct.set_current_sindex(sindex)
        rtmod.gen_inc_decode(gen, typeDef)
  ...
  defp pgen_partial_types1(erules,
            [{funcName, [topType | restTypes]} | rest]) do
    currMod = :erlang.get(:currmod)
    typeDef = :asn1_db.dbget(currMod, topType)
    traverse_type_structure(erules, typeDef, restTypes,
                              funcName, r_typedef(typeDef, :name))
  ...
  defp traverse_type_structure(erules, r_type(def: def__), [[n]], funcName,
            topTypeName)
    ...
    case (innerType) do
      :"SEQUENCE OF" ->
        {_, type} = def__
        traverse_type_structure(erules, type, [], funcName,
                                  topTypeName)
      wrongType ->
        exit({:error,
                {:configuration_file_error, [n], 'only for SEQUENCE OF components', wrongType}})
    end
  end

  defp traverse_type_structure(erules, type, [[n] | ts], funcName, topTypeName)
      when is_integer(n) do
    traverse_type_structure(erules, type, ts, funcName,
                              topTypeName)
  end

  defp traverse_type_structure(erules, r_type(def: def__), [t | ts], funcName,
            topTypeName) do
    innerType = :asn1ct_gen.get_inner(def__)
    case (innerType) do
      :SET ->
        ...
        traverse_type_structure(erules, r_ComponentType(c, :typespec), ts,
      ...
      :"SEQUENCE OF" ->
        {_, type} = def__
        traverse_SO_type_structure(erules, type, [t | ts],
      ...
      r_Externaltypereference(module: m, type: tName) ->
        ...
        traverse_type_structure(erules, typeDef, [t | ts],
                                  funcName, [r_typedef(typeDef, :name)])
      _ ->
        traverse_type_structure(erules, def__, ts, funcName,
                                  [t | topTypeName])
    end
  end

  defp traverse_type_structure(erules, r_typedef(typespec: def__), [t | ts], funcName,
            topTypeName) do
  ...
  defp traverse_SO_type_structure(erules, type, [n | rest], funcName, topTypeName)
      when is_integer(n) do
    traverse_type_structure(erules, type, rest, funcName,
                              topTypeName)
  end

  defp traverse_SO_type_structure(erules, type, typeList, funcName,
            topTypeName) do
    traverse_type_structure(erules, type, typeList,
                              funcName, topTypeName)
  end

  defp get_component(name, {c1, c2}) when (is_list(c1) and
                                  is_list(c2)) do
  ...
  defp gen_dec_part_inner_constr(rtmod, erules, typeDef, typeName) do
    ...
    case (innerType) do
      :SET ->
        r_SET(components: components) = r_type(def__, :def)
        gen_dec_part_inner_types(rtmod, erules, components,
                                   typeName)
      :SEQUENCE ->
        r_SEQUENCE(components: components) = r_type(def__, :def)
        gen_dec_part_inner_types(rtmod, erules, components,
      ...
      :"SEQUENCE OF" ->
        {_, type} = r_type(def__, :def)
        nameSuffix = constructed_suffix(innerType,
                                          r_type(type, :def))
        :asn1ct_name.clear()
  ...
  defp gen_dec_part_inner_types(rtmod, erules, [componentType | rest],
            typeName) do
    :asn1ct_name.clear()
  ...
  defp gen_dec_part_inner_types(rtmod, erules, {comps1, comps2}, typeName)
    ...
    gen_dec_part_inner_types(rtmod, erules,
                               comps1 ++ comps2, typeName)
  end
  ...
  defp emit_partial_incomplete_decode({funcName, topType, pattern}) do
    ...
    tPattern = (case (:lists.keysearch(funcName, 1,
                                         typePattern)) do
                  {:value, {_, tP}} ->
                    tP
                  _ ->
                    exit({:error,
                            {:asn1_internal_error, :exclusive_decode}})
                end)
    topTypeName = (case (:asn1ct.maybe_saved_sindex(topType,
                                                      tPattern)) do
                     i when (is_integer(i) and i > 0) ->
    ...
    emit([{:asis, funcName}, '(Bytes) ->', :nl, '  decode_partial_incomplete(\'', topTypeName, '\',Bytes,',
                                                         {:asis, pattern}, ').',
                                                                               :nl])
  end
  ...
  defp gen_part_decode_funcs(r_Externaltypereference(module: m, type: t), _TypeName, data) do
    ...
    innerType = (case (r_type(tS, :def)) do
                   r_ObjectClassFieldType(type: oCFTType) ->
                     oCFTType
                   _ ->
  ...
  defp gen_part_decode_funcs({:constructed, :bif}, typeName,
            {_Name, :parts, tag, _Type}) do
    emit(['  case Data of', :nl, '    L when is_list(L) ->', :nl, '      \'dec_', typeName, '\'(lists:map(fun(X) -> element(1, ', {:call, :ber,
                                             :ber_decode_erlang, ['X']},
                                              ') end, L),', {:asis, tag}, ');', :nl, '    _ ->', :nl,
                                                                              '      [Res] = \'dec_',
                                                                                  typeName,
                                                                                      '\'([Data],',
                                                                                          {:asis,
  ...
  defp gen_part_decode_funcs(whatKind, _TypeName,
            {_Name, :parts, _Tag, _Type}) do
    throw({:error, {:asn1, {'only SEQUENCE OF/SET OF may have the partial incomplete directive \'parts\'.', whatKind}}})
  end

  defp gen_part_decode_funcs({:constructed, :bif}, typeName,
            {_Name, :undecoded, tag, _Type}) do
    emit(['  \'dec_', typeName, '\'(Data,', {:asis, tag}, ')'])
  end

  defp gen_part_decode_funcs({:primitive, :bif}, _TypeName,
            {_Name, :undecoded, tag, type}) do
  ...
  defp gen_types(erules, tname, {rootL1, extList, rootL2},
            encDec)
      when (is_list(rootL1) and is_list(rootL2)) do
    gen_types(erules, tname, rootL1, encDec)
    rtmod = ct_gen_module(erules)
    gen_types(erules, tname,
                rtmod.extaddgroup2sequence(extList), encDec)
  ...
  defp gen_types(erules, tname, [{:EXTENSIONMARK, _, _} | t],
            encDec) do
  ...
  def gen_encode_constructed(erules, typename, innerType, d)
    ...
    case (innerType) do
      ...
      :"SEQUENCE OF" ->
        ...
        {_, type} = r_type(d, :def)
        nameSuffix = :asn1ct_gen.constructed_suffix(innerType,
                                                      r_type(type, :def))
  ...
  def gen_decode_constructed(erules, typename, innerType, d)
      when elem(d, 0) === :typedef do
    gen_decode_constructed(erules, typename, innerType,
                             r_typedef(d, :typespec))
  end

  defp pgen_exports(r_gen(options: options) = gen, code) do
    r_abst(types: types, values: values, objects: objects,
        objsets: objectSets) = code
  ...
  defp pgen_dispatcher(gen, types) do
    ...
    noFinalPadding = :lists.member(:no_final_padding,
                                     options)
    noOkWrapper = :proplists.get_bool(:no_ok_wrapper,
                                        options)
    ...
    case (r_gen(gen, :jer)) do
      true ->
        ...
        jerCall = ['?JSON_ENCODE(', {:call, :jer, :encode_jer,
                         [currMod, 'Type', 'Data']},
                          ')']
        case (noOkWrapper) do
          true ->
    ...
    returnRest = :proplists.get_bool(:undec_rest,
                                       r_gen(gen, :options))
    data = (case (r_gen(gen, :erule) === :ber and returnRest) do
              true ->
                'Data0'
              false ->
                'Data'
    ...
    decWrap = (case ({gen, returnRest}) do
    ...
    case ({gen, returnRest}) do
      ...
      {r_gen(erule: :jer), false} ->
        emit(['   Result = ', {:call, :jer, :decode_jer,
                    [currMod, 'Type', decWrap]},
                     ',', :nl])
        result_line(noOkWrapper, ['Result'])
  ...
  defp try_catch() do
    ['  catch', :nl, '    Class:Exception:Stk when Class =:= error; Class =:= exit ->', :nl, '      case Exception of', :nl, '        {error,{asn1,Reason}} ->', :nl, '          {error,{asn1,{Reason,Stk}}};', :nl, '        Reason ->', :nl, '         {error,{asn1,{Reason,Stk}}}', :nl,
                                                            '      end', :nl, 'end.']
  end
  ...
  defp gen_info_functions(gen) do
    ...
    emit(['encoding_rule() -> ', {:asis, erule}, '.', :nl, :nl, 'maps() -> ', {:asis, maps},
                                                 '.', :nl, :nl, 'bit_string_format() -> ', {:asis,
                                                                    :asn1ct.get_bit_string_format()},
                                                                     '.', :nl,
                                                                            :nl,
                                                                                'legacy_erlang_types() -> ',
                                                                                    {:asis,
                                                                                       :asn1ct.use_legacy_types()},
                                                                                        '.',
                                                                                            :nl,
                                                                                                :nl])
  end
  ...
  defp gen_decode_partial_incomplete(r_gen(erule: :ber)) do
    case ({:asn1ct.read_config_data(:partial_incomplete_decode),
      ...
      _ ->
        emitCaseClauses = fn () ->
                               emit(['   {\'EXIT\',{error,Reason}} ->', :nl, '      {error,Reason};', :nl, '    {\'EXIT\',Reason} ->', :nl, '      {error,{asn1,Reason}};', :nl, '    Result ->', :nl, '      {ok,Result}',
                                                                                 :nl,
                                                                                     '  end'])
                          end
        emit(['decode_partial_incomplete(Type,Data0,', 'Pattern) ->', :nl])
        emit(['  {Data,_RestBin} =', :nl, '    ', {:call, :ber,
                            :decode_primitive_incomplete, ['Pattern', 'Data0']},
                             :com, :nl, '  case catch decode_partial_inc_disp(Type,', 'Data) of', :nl])
        emitCaseClauses.()
        emit(['.', :nl, :nl])
  ...
  defp gen_partial_inc_dispatcher([{funcName, topType, _Pattern} | rest],
            typePattern, sep) do
    tPattern = (case (:lists.keysearch(funcName, 1,
                                         typePattern)) do
                  {:value, {_, tP}} ->
    ...
    funcName2 = :asn1ct.maybe_rename_function(:inc_disp,
                                                topType, tPattern)
    topTypeName = (case (:asn1ct.maybe_saved_sindex(topType,
                                                      tPattern)) do
                     i when (is_integer(i) and i > 0) ->
    ...
    emit([sep, 'decode_partial_inc_disp(\'', topTypeName, '\',Data) ->', :nl, '  ', {:asis,
                                             :erlang.list_to_atom(:lists.concat(['dec-inc-',
                                                                                     funcName2]))},
                                              '(Data)'])
  ...
  defp gen_dispatcher_1([f | t], funcName, prefix) do
    ...
    emit([funcName, '(', {:asis, f}, ', Data) -> ', {:asis, func}, '(Data)', ';',
                                                            :nl])
  ...
  defp pgen_info() do
    emit(['info() ->', :nl, '   case ?MODULE:module_info(attributes) of', :nl, '     Attributes when is_list(Attributes) ->', :nl, '       case lists:keyfind(asn1_info, 1, Attributes) of', :nl, '         {_,Info} when is_list(Info) ->', :nl, '           Info;', :nl, '         _ ->',
                                                              :nl, '           []', :nl, '       end;',
                                                                               :nl,
                                                                                   '     _ ->',
                                                                                       :nl,
  ...
  def emit(term) do
    :ok = :file.write(:erlang.get(:gen_file_out),
                        do_emit(term))
  end
  ...
  defp pgen_hrl(r_gen(pack: :record) = gen, code) do
    r_abst(name: module, types: types, values: values,
        ptypes: ptypes) = code
    ret = (case (pgen_hrltypes(gen, module, ptypes ++ types,
                                 0)) do
             0 ->
               case (values) do
                 [] ->
    ...
    case (ret) do
      ...
      y ->
        ...
        :asn1ct.verbose('--~p--~n',
                          [{:generated,
                              :lists.concat([:erlang.get(:outfile), '.hrl'])}],
                          gen)
        y
    end
  end
  ...
  defp gen_record(gen, torPtype, name,
            [r_ComponentType(name: cname, typespec: type) | t], num) do
    num2 = gen_record(gen, torPtype, [cname | name], type,
                        num)
  ...
  defp gen_record(gen, torPtype, name, {clist1, eClist, clist2},
            num)
      when (is_list(clist1) and is_list(eClist) and
              is_list(clist2)) do
    gen_record(gen, torPtype, name,
                 clist1 ++ eClist ++ clist2, num)
  end
  ...
  defp do_gen_record_0(gen, name, cL0) do
    cL = (case (cL0) do
            ...
            {root, ext} ->
              root ++ [{:comment, 'with extensions'}] ++ only_components(ext)
            {root1, ext, root2} ->
              root1 ++ [{:comment,
                           'with extensions'}] ++ only_components(ext) ++ [{:comment,
                                                              'end of extensions'}] ++ root2
            _ when is_list(cL0) ->
              only_components(cL0)
          end)
  ...
  defp do_gen_record_1([r_ComponentType(name: name, prop: prop) | t]) do
    ...
    com = (case (needs_trailing_comma(t)) do
             true ->
               [:com]
             false ->
               []
  ...
  defp gen_head(r_gen(options: options) = gen, mod, hrl) do
    ...
    emit(['%% Generated by the Erlang ASN.1 ', name, ' compiler. Version: ', :asn1ct.vsn(), :nl, '%% Purpose: Encoding and decoding of the types in ', mod, '.', :nl,
                                                         :nl, '-module(\'', mod, '\').', :nl, '-compile(nowarn_unused_vars).',
                                                                                  :nl,
    ...
    deterministic = :proplists.get_bool(:deterministic,
                                          options)
    options1 = (case (deterministic) do
                  true ->
                    :lists.filter(fn {:cwd, _} ->
                                       false
                                     {:outdir, _} ->
                                       false
                                     {:i, _} ->
    ...
    emit(['-asn1_info([{vsn,\'', :asn1ct.vsn(), '\'},', :nl, '            {module,\'', mod, '\'},', :nl, '            {options,',
                                                        :io_lib.format('~p',
                                                                         [options1]),
                                                            '}]).', :nl, :nl])
    jerDefines = (case (gen) do
                    r_gen(erule: :jer) ->
                      true
                    r_gen(jer: true) ->
                      true
    ...
    jerDefines and emit(['-ifdef(jsone).', :nl, '-define(JSON_DECODE(Data),jsone:decode(Data)).', :nl, '-define(JSON_ENCODE(Term),jsone:encode(Term)).', :nl, '-else.', :nl, '-define(JSON_DECODE(Data),jsx:decode(Data,[return_maps])).',
                                                             :nl, '-define(JSON_ENCODE(Term),jsx:encode(Term)).', :nl, '-endif.',
                                                                              :nl])
  end

  defp gen_hrlhead(mod) do
    emit(['%% Generated by the Erlang ASN.1 compiler. Version: ', :asn1ct.vsn(), :nl, '%% Purpose: Erlang record definitions for each named and unnamed', :nl, '%% SEQUENCE and SET, and macro definitions for each value', :nl, '%% definition in module ', mod, '.',
                                                             :nl, :nl])
  end
  ...
  def insert_once(table, object) do
    case (:asn1ct_table.lookup(table,
                                 :erlang.element(1, object))) do
      [] ->
  ...
  defp named_bitstring_value_1(bs, int) do
    b = int &&& 1
    named_bitstring_value_1(<<bs :: bitstring,
                                b :: size(1)>>,
                              int >>> 1)
  end
  ...
  def prim_bif(x) do
    :lists.member(x,
                    [:INTEGER, :ENUMERATED, :REAL, :"OBJECT IDENTIFIER", :"RELATIVE-OID", :NULL, :"BIT STRING", :"OCTET STRING",
                                                                          :ObjectDescriptor,
                                                                              :NumericString,
                                                                                  :TeletexString,
                                                                                      :T61String,
