defmodule :m_asn1rtt_ber do
  ...
  defp decode_incomplete2(_Form = 2, tagNo, v, tagMatch, _) do
    {vlist,
       rest2} = decode_constr_indef_incomplete(tagMatch, v, [])
    {{tagNo, vlist}, rest2}
  end
  ...
  defp decode_constructed_incomplete(directives = [[alt, _] | _], bin)
    ...
    case (incomplete_choice_alt(tagNo, directives)) do
      ...
      {:alt, innerDirectives} ->
        {tlv,
           ^rest} = decode_primitive_incomplete(innerDirectives, v)
        {tagNo, tlv}
      {:alt_parts, _} ->
  ...
  defp return_value(tag, binary) do
    ...
    {:ok,
       {lenVal,
          _RestBinary2}} = get_length_and_value(restBinary)
    {:ok, <<tag :: binary, lenVal :: binary>>}
  end

  defp skip_tag(<<_ :: size(3), 31 :: size(5),
              rest :: binary>>) do
    skip_long_tag(rest)
  end

  defp skip_tag(<<_ :: size(3), _Tag :: size(5),
              rest :: binary>>) do
    {:ok, rest}
  end

  defp skip_long_tag(<<1 :: size(1), _ :: size(7),
              rest :: binary>>) do
    skip_long_tag(rest)
  end

  defp skip_long_tag(<<0 :: size(1), _ :: size(7),
              rest :: binary>>) do
    {:ok, rest}
  end
  ...
  defp skip_optional_tag(<<tag, restTag :: binary>>,
            <<tag, rest :: binary>>) do
  ...
  defp skip_length_and_value(binary) do
    case (decode_length(binary)) do
      ...
      {length, restBinary} ->
        <<_ :: size(length) - unit(8),
            rest :: binary>> = restBinary
        {:ok, rest}
    end
  end
  ...
  defp get_value(binary) do
    case (decode_length(binary)) do
      ...
      {length, restBinary} ->
        <<value :: size(length) - binary,
            _Rest :: binary>> = restBinary
        {:ok, value}
    end
  end
  ...
  defp get_indefinite_value(binary, acc) do
    ...
    {:ok,
       {lenVal,
          restBinary2}} = get_length_and_value(restBinary)
  ...
  defp get_long_tag(<<h :: size(1) - binary, rest :: binary>>,
            acc) do
    case (h) do
      <<0 :: size(1), _ :: size(7)>> ->
        {:ok,
           {:erlang.list_to_binary(:lists.reverse([h | acc])),
              rest}}
      _ ->
  ...
  defp get_length_and_value(bin = <<0 :: size(1), length :: size(7),
                    _T :: binary>>) do
    <<len, val :: size(length) - binary,
        rest :: binary>> = bin
    {:ok, {<<len, val :: binary>>, rest}}
  end

  defp get_length_and_value(bin = <<1 :: size(1), 0 :: size(7),
                    _T :: binary>>) do
  ...
  defp get_length_and_value(<<1 :: size(1), lL :: size(7), t :: binary>>) do
    ...
    {:ok,
       {<<1 :: size(1), lL :: size(7),
            length :: size(lL) - unit(8), value :: binary>>,
          rest2}}
  end
  ...
  defp get_indefinite_length_and_value(binary, acc) do
    ...
    {:ok,
       {lenVal,
          restBinary2}} = get_length_and_value(restBinary)
    get_indefinite_length_and_value(restBinary2,
                                      [lenVal, tag | acc])
  end
  ...
  def match_tags({tag, _V} = tlv, [t | _Tt]) do
    exit({:error,
            {:asn1,
               {:wrong_tag, {{:expected, t}, {:got, tag, tlv}}}}})
  end
  ...
  defp decode_tag_and_length(<<class :: size(2), form :: size(1),
              tagNo :: size(5), 0 :: size(1), length :: size(7),
              v :: size(length) - binary, restBuffer :: binary>>)
      when tagNo < 31 do
    {form, class <<< 16 ||| tagNo, v, restBuffer}
  end

  defp decode_tag_and_length(<<class :: size(2), 1 :: size(1),
              tagNo :: size(5), 1 :: size(1), 0 :: size(7),
              t :: binary>>)
      when tagNo < 31 do
    {2, class <<< 16 + tagNo, t, <<>>}
  end
  ...
  defp decode_tag_and_length(<<class :: size(2), form :: size(1),
              tagNo :: size(5), 1 :: size(1), lL :: size(7),
              length :: size(lL) - unit(8),
              v :: size(length) - binary, t :: binary>>)
      when tagNo < 31 do
    {form, class <<< 16 ||| tagNo, v, t}
  end

  defp decode_tag_and_length(<<class :: size(2), form :: size(1),
              31 :: size(5), 0 :: size(1), tagNo :: size(7),
              0 :: size(1), length :: size(7),
              v :: size(length) - binary, restBuffer :: binary>>) do
    {form, class <<< 16 ||| tagNo, v, restBuffer}
  end

  defp decode_tag_and_length(<<class :: size(2), 1 :: size(1), 31 :: size(5),
              0 :: size(1), tagNo :: size(7), 1 :: size(1),
              0 :: size(7), t :: binary>>) do
    {2, class <<< 16 ||| tagNo, t, <<>>}
  end
  ...
  defp decode_tag_and_length(<<class :: size(2), form :: size(1),
              ...
              1 :: size(1), lL :: size(7),
              length :: size(lL) - unit(8),
              v :: size(length) - binary, t :: binary>>) do
    {form, class <<< 16 ||| tagNo, v, t}
  end

  defp decode_tag_and_length(<<class :: size(2), form :: size(1),
              31 :: size(5), 1 :: size(1), tagPart1 :: size(7),
              0 :: size(1), tagPartLast, buffer :: binary>>) do
    tagNo = tagPart1 <<< 7 ||| tagPartLast
    {length, restBuffer} = decode_length(buffer)
    <<v :: size(length) - binary,
        restBuffer2 :: binary>> = restBuffer
    {form, class <<< 16 ||| tagNo, v, restBuffer2}
  end

  defp decode_tag_and_length(<<class :: size(2), form :: size(1),
              31 :: size(5), buffer :: binary>>) do
  ...
  defp decode_tag(<<0 :: size(1), partialTag :: size(7),
              buffer :: binary>>,
            tagAck) do
  ...
  defp decode_tag(<<_ :: size(1), partialTag :: size(7),
              buffer :: binary>>,
            tagAck) do
  ...
  def encode_integer(val, namedNumberList, tag) when is_atom(val) do
    case (:lists.keyfind(val, 1, namedNumberList)) do
      ...
      _ ->
        exit({:error,
                {:asn1, {:encode_integer_namednumber, val}}})
    end
  end
  ...
  def encode_named_bit_string(c, [{:bit, _} | _] = bits, namedBitList,
           tagIn) do
    do_encode_named_bit_string(c, bits, namedBitList, tagIn)
  end
  ...
  defp do_encode_named_bit_string(size, [firstVal | restVal], namedBitList,
            tagIn) do
    toSetPos = get_all_bitposes([firstVal | restVal],
                                  namedBitList, [])
    bitList = make_and_set_list(size, toSetPos, 0)
  ...
  defp remove_unused_then_dotag(tagIn, unused, binBits) do
    case (unused) do
      ...
      num ->
        ...
        encode_tags(tagIn,
                      [unused,
                           :erlang.binary_to_list(bBits) ++ [lastByte >>> num <<< num]],
                      1 + byte_size(binBits))
    end
  end
  ...
  defp encode_constr_bit_str_bits({{_Min1, max1}, {min2, max2}}, bitListVal,
            tagIn) do
    ...
    case (bitLen) do
      len when len > max2 ->
        exit({:error,
                {:asn1,
                   {:bitstring_length,
                      {{:was, bitLen}, {:maximum, max2}}}}})
      len when (len > max1 and len < min2) ->
        exit({:error,
                {:asn1,
                   {:bitstring_length,
                      {{:was, bitLen},
                         {:not_allowed_interval, max1, min2}}}}})
      _ ->
        {len, unused, octetList} = encode_bitstring(bitListVal)
        encode_tags(tagIn, [unused, octetList], len + 1)
    end
    ...
    cond do
      bitLen > max ->
        exit({:error,
                {:asn1,
                   {:bitstring_length,
                      {{:was, bitLen}, {:maximum, max}}}}})
      bitLen < min ->
        exit({:error,
                {:asn1,
                   {:bitstring_length,
                      {{:was, bitLen}, {:minimum, max}}}}})
      true ->
        {len, unused, octetList} = encode_bitstring(bitListVal)
        encode_tags(tagIn, [unused, octetList], len + 1)
    end
  ...
  defp encode_bitstring([b8, b7, b6, b5, b4, b3, b2, b1 | rest], ack,
            len) do
    val = b8 <<< 7 ||| (b7 <<< 6) ||| (b6 <<< 5) ||| (b5 <<< 4) ||| (b4 <<< 3) ||| (b3 <<< 2) ||| (b2 <<< 1) ||| b1
  ...
  def decode_native_bit_string(buffer, tags) do
    case (match_and_collect(buffer, tags)) do
      ...
      <<unused, bits :: binary>> ->
        ...
        <<val :: size(size) - bitstring,
            _ :: size(unused) - bitstring>> = bits
        val
    end
  end
  ...
  def decode_named_bit_string(buffer, namedNumberList, tags) do
    case (match_and_collect(buffer, tags)) do
      <<0>> ->
        []
      <<unused, bits :: binary>> ->
        bitString = decode_bitstring2(byte_size(bits), unused,
                                        bits)
        decode_bitstring_NNL(bitString, namedNumberList)
    end
  end

  defp decode_bitstring2(1, unused,
            <<b7 :: size(1), b6 :: size(1), b5 :: size(1),
                b4 :: size(1), b3 :: size(1), b2 :: size(1),
                b1 :: size(1), b0 :: size(1), _ :: binary>>) do
    :lists.sublist([b7, b6, b5, b4, b3, b2, b1, b0],
                     8 - unused)
  end

  defp decode_bitstring2(len, unused,
            <<b7 :: size(1), b6 :: size(1), b5 :: size(1),
                b4 :: size(1), b3 :: size(1), b2 :: size(1),
                b1 :: size(1), b0 :: size(1), buffer :: binary>>) do
  ...
  defp decode_bitstring_NNL([b | bitList], [{name, no} | namedNumberList],
            no, result) do
    cond do
      b === 0 ->
  ...
  defp dec_subidentifiers(<<1 :: size(1), h :: size(7), t :: binary>>, av,
            al) do
  ...
  defp decode_length(<<0 :: size(1), length :: size(7),
              t :: binary>>) do
    {length, t}
  end

  defp decode_length(<<1 :: size(1), lL :: size(7),
              length :: size(lL) - unit(8), t :: binary>>) do
    {length, t}
  end

  def dynamicsort_SET_components(listOfEncCs) do
    tagBinL = (for l <- listOfEncCs do
                 (
  ...
  defp dynsort_decode_tag(<<class :: size(2), _Form :: size(1),
              31 :: size(5), buffer :: binary>>) do
  ...
  defp dynsort_decode_tag(<<class :: size(2), _Form :: size(1),
              tagNum :: size(5), _ :: binary>>) do
    {class, tagNum}
  end

  defp dynsort_decode_tag(<<0 :: size(1), partialTag :: size(7),
              _ :: binary>>,
            tagAcc) do
    tagAcc <<< 7 ||| partialTag
  end

  defp dynsort_decode_tag(<<_ :: size(1), partialTag :: size(7),
              buffer :: binary>>,
            tagAcc0) do
  ...
  defp collect_parts([{3, <<unused, bits :: binary>>} | rest],
            _Acc) do
  ...
  defp collect_parts_bit([{3, <<unused, bits :: binary>>} | rest], acc,
            uacc) do
