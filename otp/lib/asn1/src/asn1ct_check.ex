defmodule :m_asn1ct_check do
  ...
  defp printable_string_1({r_seqtag(val: val1), val2}) do
    :erlang.atom_to_list(val1) ++ ~c" " ++ printable_string_1(val2)
  end
  ...
  defp printable_string_1([_ | _] = def__) do
    case :lists.all(&is_integer/1, def__) do
      true ->
        :lists.flatten(:io_lib.format(~c"~p", [def__]))
  ...
  defp normalize_value(s, type, {:DEFAULT, value}, nameList) do
    case (try do
      ...
      err ->
        :asn1ct.warning(
          ~c"could not check default value ~p~nType:~n~p~nNameList:~n~p~n",
          [value, type, err],
          s,
          ~c"could not check default value"
        )
  ...
  defp normalize_boolean(s, _, _) do
    asn1_error(s, {:illegal_value, ~c"BOOLEAN"})
  end
    ...
    case value do
      ...
      {:ValueFromObject, {:object, obj}, fieldNames} ->
        case extract_field(s, obj, fieldNames) do
          ...
          _ ->
            asn1_error(s, {:illegal_value, ~c"BIT STRING"})
        end
  ...
  defp normalize_s_of(sorS, s, value, type, nameList)
    ...
    case (try do
      ...
      _ ->
        :asn1ct.warning(
          ~c"~p could not handle value ~p~n",
          [sorS, value],
          s,
          ~c"could not handle value"
        )
  ...
  defp get_normalized_value(s, val, type, func, addArg) do
    case (try do
      ...
      {:error, _} ->
        :asn1ct.warning(~c"default value not comparable ~p~n", [val], s)
        val
      ...
      _ ->
        :asn1ct.warning(
          ~c"default value not comparable ~p~n",
          [val],
          s,
          ~c"default value not comparable"
        )
  ...
  defp categorize({:object, _, type}) do
    def__ = fn
      d = r_type() ->
        r_typedef(
          name: new_reference_name(~c"type_argument"),
  ...
  defp categorize(s, :object_set, def__, classRef) do
    ...
    name = new_reference_name(~c"object_set_argument")
  ...
  defp finish_constraints_1(
         [
           {:element_set, {:SizeConstraint, {:element_set, root, :none}}, {:set, []} = set}
           | t
         ],
         collapse
       ) do
  ...
  defp check_imported(s, imodule, name, isParsed) do
    case :asn1_db.dbget(imodule, :MODULE) do
      :undefined when isParsed == true ->
        errStr =
          :io_lib.format(~c"Type ~s imported from non existing module ~s~n", [name, imodule])
      ...
      im when elem(im, 0) === :module ->
        case is_exported(im, name) do
          false ->
            errStr =
              :io_lib.format(~c"Imported type ~s not exported from module ~s~n", [name, imodule])
  ...
  defp check_unique_sequence_tags(s, compList) do
    tagComps =
      case :asn1ct_gen.complist_as_tuple(compList) do
        {r1, ext, r2} ->
          r1 ++
  ...
  def format_error({:already_defined, name, prevLine}) do
    :io_lib.format(~c"the name ~p has already been defined at line ~p", [name, prevLine])
  end

  def format_error({:duplicate_identifier, ids}) do
    :io_lib.format(~c"the identifier '~p' has already been used", [ids])
  end

  def format_error({:duplicate_tags, elements}) do
    :io_lib.format(~c"duplicate tags in the elements: ~s", [format_elements(elements)])
  end

  def format_error({:enum_illegal_redefinition, id}) do
    :io_lib.format(~c"'~s' must not be redefined", [id])
  end

  def format_error({:enum_not_ascending, id, n, prev}) do
    :io_lib.format(
      ~c"the values for enumerations which follow '...' must be in ascending order, but '~p(~p)' is less than the previous value '~p'",
      [id, n, prev]
    )
  end

  def format_error({:enum_reused_value, id, val}) do
    :io_lib.format(~c"'~s' has the value '~p' which is used more than once", [id, val])
  end

  def format_error({:illegal_id, id}) do
    :io_lib.format(~c"illegal identifier: ~p", [id])
  end

  def format_error({:illegal_choice_type, ref}) do
    :io_lib.format(~c"expecting a CHOICE type: ~p", [ref])
  end

  def format_error({:illegal_class_name, class}) do
    :io_lib.format(
      ~c"the class name '~s' is illegal (it must start with an uppercase letter and only contain uppercase letters, digits, or hyphens)",
      [class]
    )
  end

  def format_error({:illegal_COMPONENTS_OF, ref}) do
    :io_lib.format(~c"expected a SEQUENCE or SET got: ~p", [ref])
  end

  def format_error(:illegal_external_value) do
    ~c"illegal value in EXTERNAL type"
  end

  def format_error({:illegal_instance_of, class}) do
    :io_lib.format(
      ~c"using INSTANCE OF on class '~s' is illegal, because INSTANCE OF may only be used on the class TYPE-IDENTIFIER",
      [class]
    )
  end
  ...
  def format_error(:illegal_object) do
    ~c"expecting an object"
  end

  def format_error({:illegal_object_field, id}) do
    :io_lib.format(~c"expecting a class field: ~p", [id])
  end

  def format_error({:illegal_oid, :o_id}) do
    ~c"illegal OBJECT IDENTIFIER"
  end

  def format_error({:illegal_oid, :rel_oid}) do
    ~c"illegal RELATIVE-OID"
  end

  def format_error(:illegal_octet_string_value) do
    ~c"expecting a bstring or an hstring as value for an OCTET STRING"
  end

  def format_error({:illegal_typereference, name}) do
    :io_lib.format(
      ~c"'~p' is used as a typereference, but does not start with an uppercase letter",
      [name]
    )
  end

  def format_error(:illegal_table_constraint) do
    ~c"table constraints may only be applied to CLASS.&field constructs"
  end
  ...
  def format_error({:illegal_value, tYPE}) do
    :io_lib.format(~c"expecting a ~s value", [tYPE])
  end

  def format_error({:invalid_fields, fields, obj}) do
    :io_lib.format(~c"invalid ~s in ~p", [format_fields(fields), obj])
  end

  def format_error({:invalid_bit_number, bit}) do
    :io_lib.format(~c"the bit number '~p' is invalid", [bit])
  end

  def format_error(:invalid_table_constraint) do
    ~c"the table constraint is not an object set"
  end

  def format_error(:invalid_objectset) do
    ~c"expecting an object set"
  end

  def format_error({:implicit_tag_before, kind}) do
    ~c"illegal implicit tag before " ++
      case kind do
        :choice ->
          ~c"'CHOICE'"
  ...
  def format_error({:missing_mandatory_fields, fields, obj}) do
    :io_lib.format(~c"missing mandatory ~s in ~p", [format_fields(fields), obj])
  end

  def format_error({:missing_table_constraint, component}) do
    :io_lib.format(
      ~c"the component '~s' is referenced by a component relation constraint using the '@field-name' notation, but does not have a table constraint",
      [component]
    )
  end

  def format_error({:missing_id, id}) do
    :io_lib.format(~c"expected the mandatory component '~p'", [id])
  end

  def format_error({:missing_ocft, component}) do
    :io_lib.format(
      ~c"the component '~s' must be an ObjectClassFieldType (CLASSNAME.&field-name)",
      [component]
    )
  end

  def format_error(:multiple_uniqs) do
    ~c"implementation limitation: only one UNIQUE field is allowed in CLASS"
  end

  def format_error({:namelist_redefinition, name}) do
    :io_lib.format(~c"the name '~s' cannot be redefined", [name])
  end

  def format_error({:param_bad_type, ref}) do
    :io_lib.format(~c"'~p' is not a parameterized type", [ref])
  end
  ...
  def format_error(:reversed_range) do
    ~c"ranges must be given in increasing order"
  end

  def format_error({:syntax_duplicated_fields, fields}) do
    :io_lib.format(~c"~s must only occur once in the syntax list", [format_fields(fields)])
  end

  def format_error(:syntax_nomatch) do
    ~c"unexpected end of object definition"
  end

  def format_error({:syntax_mandatory_in_optional_group, name}) do
    :io_lib.format(
      ~c"the field '&~s' must not be within an optional group since it is not optional",
      [name]
    )
  end

  def format_error({:syntax_missing_mandatory_fields, fields}) do
    :io_lib.format(~c"missing mandatory ~s in the syntax list", [format_fields(fields)])
  end

  def format_error({:syntax_nomatch, actual}) do
    :io_lib.format(~c"~s is not the next item allowed according to the defined syntax", [actual])
  end

  def format_error({:syntax_undefined_field, field}) do
    :io_lib.format(~c"'&~s' is not a field of the class being defined", [field])
  end

  def format_error({:undefined, name}) do
    :io_lib.format(~c"'~s' is referenced, but is not defined", [name])
  end

  def format_error({:undefined_export, ref}) do
    :io_lib.format(~c"'~s' is exported but is not defined", [ref])
  end

  def format_error({:undefined_field, fieldName}) do
    :io_lib.format(~c"the field '&~s' is undefined", [fieldName])
  end

  def format_error({:undefined_import, ref, module}) do
    :io_lib.format(~c"'~s' is not exported from ~s", [ref, module])
  end

  def format_error({:unique_and_default, field}) do
    :io_lib.format(~c"the field '&~s' must not have both 'UNIQUE' and 'DEFAULT'", [field])
  end

  def format_error({:value_reused, val}) do
    :io_lib.format(~c"the value '~p' is used more than once", [val])
  end

  def format_error({:non_unique_object, id}) do
    :io_lib.format(~c"object set with a UNIQUE field value of '~p' is used more than once", [id])
  end
  ...
  defp format_fields([f]) do
    :io_lib.format(~c"field '&~s'", [f])
  end

  defp format_fields([h | t]) do
    [
      :io_lib.format(~c"fields '&~s'", [h])
      | for f <- t do
          :io_lib.format(~c", '&~s'", [f])
        end
    ]
  ...
  defp format_elements([h1, h2 | t]) do
    [:io_lib.format(~c"~p, ", [h1]) | format_elements([h2 | t])]
  end
  ...
  defp new_reference_name(name) do
    case :erlang.get(:asn1_reference) do
      :undefined ->
        ...
        :erlang.list_to_atom(:lists.concat([:internal_, name, ~c"_", 1]))

      num when is_integer(num) ->
        :erlang.put(:asn1_reference, num + 1)
        :erlang.list_to_atom(:lists.concat([:internal_, name, ~c"_", num + 1]))
    end
  end
