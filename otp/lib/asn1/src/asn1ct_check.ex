defmodule :m_asn1ct_check do
  ...
  Record.defrecord(:r_newt, :newt, type: :unchanged,
                                tag: :unchanged, constraint: :unchanged,
                                inlined: :no)
  def check(s,
    ...
    :no_debug
    {pObjSetNames1, terror2} = filter_errors(isPObjSet,
                                               terror)
    ...
    {pObjSetNames, verror3} = filter_errors(isPObjSet,
                                              verror2)
    {objectNames, verror4} = filter_errors(isObject,
                                             verror3)
    newObjects = objects ++ objectNames
    newObjectSets = objSetNames ++ pObjSetNames ++ pObjSetNames1
    {valueSetNames, verror5} = filter_errors(isValueSet,
                                               verror4)
    {oerror, exclO, exclOS} = checko(s,
                                       newObjects ++ newObjectSets, [], [], [])
    :no_debug
    ...
    parObjectSets = :lists.filter(fn {_OSName, :objectset,
                                        _} ->
                                       true
                                     _ ->
                                       false
    ...
    parTypes = :lists.filter(fn {_TypeName, :type, _} ->
                                  true
                                _ ->
                                  false
                             end,
    ...
    allErrors = :lists.flatten([paramError, terror3,
                                                verror5, cerror, oerror,
                                                                     exporterror,
                                                                         importError])
    case (allErrors) do
      [] ->
        ...
        newTypes = :lists.subtract(types,
                                     addClasses) ++ contextSwitchTs ++ instanceOf ++ parTypesNames
        newValues = :lists.subtract(values,
        ...
        {:ok,
           ...
           {newTypes, newValues, parameterizedTypes, newClasses,
              ...
              :lists.subtract(newObjectSets,
                                exclOS) ++ parObjectSetNames}}
      _ ->
        {:error, allErrors}
    end
  end
  ...
  defp context_switch_in_spec() do
    l = [{:external, :EXTERNAL}, {:embedded_pdv, :"EMBEDDED PDV"},
  ...
  defp filter_errors(pred, errorList) do
    ...
    removedNames = :lists.map(element2,
                                removedTupleElements)
    restErrors = :lists.subtract(errorList,
                                   removedTupleElements)
  ...
  defp check_imports_1(s,
            [r_SymbolsFromModule(symbols: imports, module: moduleRef) | sFMs], acc) do
    module = name_of_def(moduleRef)
    refs = (for ref <- imports do
              {try do
    ...
    errors = (for {{:error, _}, ref} <- refs do
                createError.(ref)
              end)
  ...
  defp checkt(s0, names) do
    ...
    check_fold(s0, :lists.reverse(ctxtSwitch),
                 check) ++ types
  end
  ...
  defp check_contextswitchingtypes(s, acc) do
    cSTList = [{:external, :EXTERNAL}, {:embedded_pdv, :"EMBEDDED PDV"},
                                           {:character_string, :"CHARACTER STRING"}]
  ...
  defp do_checkv(s, name, value)
    ...
    try do
      ...
      {:objectsetdef} ->
        {:objectsetdef, name}
      {:asn1_class, _} ->
        r_valuedef(checked: c, pos: pos, name: n, type: type,
            value: def__) = value
        className = r_type(type, :def)
        newSpec = r_Object(classname: className, def: def__)
        newDef = r_typedef(checked: c, pos: pos, name: n,
                     typespec: newSpec)
      ...
      {:valueset, vSet} ->
        ...
        checkedVSDef = r_typedef(checked: true, pos: pos, name: name,
                           typespec: vSet)
  ...
  defp checko(_S, [], acc, exclO, exclOS) do
    {:lists.reverse(acc), :lists.reverse(exclO),
       :lists.reverse(exclOS)}
  end

  defp checko_1(s, r_typedef(typespec: tS) = object, name, exclO,
            exclOS) do
    newS = r_state(s, tname: name)
  ...
  defp check_class(s,
            cDef = r_classdef(checked: ch, name: name, typespec: tS)) do
    case (ch) do
      true ->
        tS
      :idle ->
      ...
      classDef when elem(classDef, 0) === :classdef ->
        case (r_classdef(classDef, :checked)) do
          ...
          false ->
            ...
            store_class(s, true, r_classdef(classDef, typespec: checkedTS),
                          name)
            checkedTS
        end
  ...
  defp instantiate_pclass(s = r_state(parameters: _OldArgs), pClassDef,
            params) do
  ...
  defp check_class_fields(s, [f | fields], acc) do
    newField = (case (:erlang.element(1, f)) do
                  :fixedtypevaluefield ->
                  ...
                  :object_or_fixedtypevalue_field ->
                    ...
                    cat = (case (:asn1ct_gen.type(:asn1ct_gen.get_inner(r_type(type2, :def)))) do
                             def__
                               ...
                               d
                             {:undefined, :user} ->
                               {_, d} = get_referenced_type(s,
                                                              r_Externaltypereference(module: r_state(s, :mname),
                    ...
                    case (cat) do
                      class when elem(class, 0) === :classdef ->
                        toExt = fn r_type(def: cE = r_Externaltypereference()) ->
                                     cE
                                   t ->
                                     t
                                end
                  ...
                  :typefield ->
                    case (f) do
                      {tF, name, {:DEFAULT, type}} ->
                        {tF, name,
                           {:DEFAULT, check_type(s, r_typedef(typespec: type), type)}}
                      _ ->
                        f
                    end
                  _ ->
  ...
  defp check_pobjectset(s, pObjSet) do
    r_pvaluesetdef(pos: pos, name: name, args: args, type: type,
        valueset: valueSet) = pObjSet
    {mod, def__} = get_referenced_type(s, r_type(type, :def))
    case (def__) do
      r_classdef() ->
        className = r_Externaltypereference(module: mod,
                        type: get_datastr_name(def__))
        {:valueset, set} = valueSet
        objectSet = r_ObjectSet(class: className, set: set)
        r_pobjectsetdef(pos: pos, name: name, args: args,
            class: r_type(type, :def), def: objectSet)
      _ ->
        pObjSet
    end
  end

  Record.defrecord(:r_osi, :osi, st: :undefined,
                               classref: :undefined, uniq: :undefined,
                               ext: :undefined)
  ...
  defp check_object(s, _ObjDef,
            r_Object(classname: classRef, def: objectDef)) do
    :no_debug
    _ = check_externaltypereference(s, classRef)
    {classDef, newClassRef} = (case (get_referenced_type(s,
                                                           classRef, true)) do
                                 {mName,
                                    r_classdef(checked: false, name: cLName) = clDef} ->
                                   type = r_Externaltypereference(classRef, :type)
                                   newState = update_state(r_state(s, tname: type),
                                                             mName)
                                   objClass = check_class(newState, clDef)
                                   {r_classdef(clDef, checked: true, 
                                               typespec: objClass),
  ...
  defp check_object(s, _,
            r_ObjectSet(class: classRef0, set: set0) = objSet0) do
    {_, classDef} = get_referenced_type(s, classRef0)
    classRef = check_externaltypereference(s, classRef0)
    {uniqueFieldName,
       uniqueInfo} = (case (get_unique_fieldname(s,
                                                   classDef)) do
                        :no_unique ->
    ...
    oSI0 = r_osi(st: s, classref: classRef, uniq: uniqueInfo,
               ext: false)
    {set1, oSI1} = (cond do
                      is_list(set0) ->
    ...
    objSet = r_ObjectSet(objSet0, uniquefname: uniqueFieldName, 
                          set: set)
  ...
  defp check_object_set(r_Externaltypereference() = ref, r_osi(st: s) = oSI) do
    {_, r_typedef(typespec: oSdef) = oS} = get_referenced_type(s,
                                                         ref)
  ...
  defp check_object_set(r_Externalvaluereference() = ref, r_osi(st: s) = oSI) do
    {refedMod, objName,
       r_Object(def: def__)} = check_referenced_object(s, ref)
    objList = check_object_set_mk(refedMod, objName, def__,
                                    oSI)
    {objList, oSI}
  end
  ...
  defp check_object_set({:object, _, _} = obj0, oSI) do
    ...
    r_Object(def: def__) = check_object(s, r_typedef(typespec: obj0),
                                   r_Object(classname: classRef, def: obj0))
    objList = check_object_set_mk(def__, oSI)
    {objList, oSI}
  end

  defp check_object_set(r_ObjectClassFieldType(classname: objName, fieldname: fieldNames),
            r_osi(st: s) = oSI) do
    set = check_ObjectSetFromObjects(s, objName, fieldNames)
    check_object_set_objset_list(set, oSI)
  end

  defp check_object_set({:ObjectSetFromObjects, obj, fieldNames},
            r_osi(st: s) = oSI) do
  ...
  defp check_object_set({:pt, definedObjSet, paramList0}, oSI) do
    ...
    objectSet = instantiate_pos(s, classRef, pObjSetDef,
                                  paramList)
    check_object_set_objset(objectSet, oSI)
  end

  defp check_object_set({:pos, {:objectset, _, definedObjSet}, params0},
            oSI) do
    r_osi(st: s, classref: classRef) = oSI
    {_, pObjSetDef} = get_referenced_type(s, definedObjSet)
  ...
  defp check_object_set({:pv, {:simpledefinedvalue, definedObject},
             params} = pV,
            oSI) do
    r_osi(st: s, classref: classRef) = oSI
    args = match_parameters(s, params)
    r_Object(def: def__) = check_object(s, pV,
                                   r_Object(classname: classRef,
  ...
  defp check_object_set({:ValueFromObject, {:object, object},
             fieldNames},
            oSI) do
    r_osi(st: s) = oSI
  ...
  defp check_object_set_objset_list_1([:EXTENSIONMARK | t], oSI, acc) do
    check_object_set_objset_list_1(t, r_osi(oSI, ext: true),
                                     acc)
  end
  ...
  defp traverse_seq_set({set, ext}, fun) do
    {traverse_seq_set_1(set, fun),
       traverse_seq_set_1(ext, fun)}
  end
    ...
    case (get_referenced_type(s, objRef)) do
      {refedMod, objectDef}
        ...
        {refedMod, get_datastr_name(objectDef),
           check_object(update_state(s, refedMod), objectDef,
                          r_Object(classname: classRef, def: def__))}
      {refedMod, objectDef}
          when elem(objectDef, 0) === :typedef ->
        {refedMod, get_datastr_name(objectDef),
           check_object(update_state(s, refedMod), objectDef,
                          r_typedef(objectDef, :typespec))}
    end
  end
  ...
  defp instantiate_po(s = r_state(parameters: _OldArgs), _ClassDef, object,
            argsList)
    ...
    check_object(newS, object,
                   r_Object(classname: r_pobjectdef(object, :class), def: r_pobjectdef(object, :def)))
  end

  defp instantiate_pos(s = r_state(parameters: _OldArgs), classRef,
            objectSetDef, argsList) do
    formalParams = get_pt_args(objectSetDef)
    oSet = (case (get_pt_spec(objectSetDef)) do
              {:valueset, set} ->
  ...
  defp gen_incl1(s, fields, [c | cFields]) do
    case (:erlang.element(1, c)) do
      :typefield ->
        true
      :objectfield ->
        case (:lists.keysearch(:erlang.element(2, c), 1,
                                 fields)) do
          {:value, field} ->
            classRef = (case (:erlang.element(3, c)) do
                          r_type(def: ref) ->
                            ref
            ...
            objDef = (case (:erlang.element(2, field)) do
    ...
    case (get_unique_fieldname(s, classDef)) do
      ...
      {_, _} ->
        gen_incl_set1(s, fields,
                        r_objectclass(r_classdef(classDef, :typespec), :fields))
    end
  end
  ...
  defp preprocess_syntax_1(s, [{:valuefieldreference, name} | t], cs,
            mandatory) do
    f = preprocess_check_field(s, name, cs, mandatory)
    [f | preprocess_syntax_1(s, t, cs, mandatory)]
  end

  defp preprocess_syntax_1(s, [{:typefieldreference, name} | t], cs,
            mandatory) do
    f = preprocess_check_field(s, name, cs, mandatory)
    [f | preprocess_syntax_1(s, t, cs, mandatory)]
  end
  ...
  defp preprocess_syntax_1(s, [{token, _} | t], cs, mandatory)
    ...
    [{:token, token} | preprocess_syntax_1(s, t, cs,
                                             mandatory)]
  end
  ...
  defp preprocess_check_field(s, name, cs, mandatory) do
    case (:lists.keyfind(name, 2, cs)) do
      tuple when is_tuple(tuple) ->
        case (not
              mandatory and is_mandatory_class_field(tuple)) do
          true ->
            asn1_error(s,
  ...
  defp match_syntax(s, [{:token, token} | t], [a | as] = args,
            acc) do
    case (a) do
  ...
  defp match_syntax(s, [{:field, field} | t] = fs,
            [a | as0] = args0, acc) do
    try do
  ...
  defp match_syntax_type(_S, _Type, {atom, line}) when (is_atom(atom) and
                                           is_integer(line)) do
    throw(:nomatch)
  end

  defp match_syntax_type(s,
            {:fixedtypevaluefield, name, r_type() = t, _, _} = type,
            r_Externalvaluereference() = valRef0) do
    try do
      get_referenced_type(s, valRef0)
    catch
      {:error, _} ->
        valRef = r_valuedef(name: name, type: t, value: valRef0,
  ...
  defp match_syntax_type(s, {:fixedtypevaluefield, name, r_type(), _, _},
            r_valuedef() = val0) do
  ...
  defp match_syntax_type(s, {:fixedtypevaluefield, name, r_type(), _, _},
            {:ValueFromObject, {:object, object}, fieldNames}) do
  ...
  defp match_syntax_type(s,
            {:fixedtypevaluefield, name, r_type() = t, _, _} = type,
            any) do
    valDef = r_valuedef(name: name, type: t, value: any,
                 module: r_state(s, :mname))
    match_syntax_type(s, type, valDef)
  end

  defp match_syntax_type(_S, {:fixedtypevaluesetfield, name, r_type(), _},
            any) do
    {:match, [{name, any}]}
  end
  ...
  defp match_syntax_type(s, {:objectfield, name, class, _, _},
            {:object, _, _} = objDef) do
    inlinedObjName = :erlang.list_to_atom(:lists.concat([r_state(s, :tname),
                                                             :_, name]))
    objSpec = r_Object(classname: class, def: objDef)
    checkedObj = check_object(s, r_typedef(typespec: objSpec),
                                objSpec)
    inlObj = r_typedef(checked: true, name: inlinedObjName,
                 typespec: checkedObj)
    objKey = {inlinedObjName, inlinedObjName}
    insert_once(s, :inlined_objects, objKey)
    :asn1_db.dbput(:erlang.get(:top_module), inlinedObjName,
                     inlObj)
    {:match, [{name, inlObj}]}
  end
  ...
  defp match_syntax_type(s, {:objectsetfield, name, cDef0, _}, any) do
    ...
    case (match_syntax_objset(s, any, cDef)) do
      r_typedef(typespec: r_ObjectSet() = ts0) = def__ ->
        ...
        {:match,
           [{name, r_typedef(def__, checked: true,  typespec: ts)}]}
      _ ->
        syntax_match_error(s, any)
    end
  end

  defp match_syntax_type(s, {:typefield, name0, _},
            r_type(def: {:pt, _, _} = def__) = actual) do
  ...
  defp match_syntax_type(s, {:typefield, name, _},
            r_type(def: r_ObjectClassFieldType()) = actual) do
    t = check_type(s, r_typedef(typespec: actual), actual)
  ...
  defp match_syntax_type(s, {:typefield, name, _},
            r_type(def: def__) = actual) do
    t = check_type(s, r_typedef(typespec: actual), actual)
    typeName = :asn1ct_gen.type(:asn1ct_gen.get_inner(def__))
    {:match,
       [{name, r_typedef(checked: true, name: typeName, typespec: t)}]}
  end
  ...
  defp match_syntax_type(_S, {:variabletypevaluefield, name, _, _},
            any) do
    {:match, [{name, any}]}
  end

  defp match_syntax_type(_S, {:variabletypevaluesetfield, name, _, _},
            any) do
    {:match, [{name, any}]}
  end
  ...
  defp match_syntax_params(s0, r_ptypedef(name: name) = ptDef,
            r_Externaltypereference(module: m, type: n) = eRef0, args) do
    s = r_state(s0, mname: m,  module: load_asn1_module(s0, m), 
                tname: name)
    ...
    insert_once(s0, :parameterized_objects,
                  {eRefName, :type, tDef})
  ...
  defp match_syntax_objset(s, {:object, :definedsyntax, words},
            classDef) do
    case (words) do
      [word] ->
  ...
  defp printable_string_1({atom, line}) when (is_atom(atom) and
                                is_integer(line)) do
    q(atom)
  end
  ...
  defp check_defaultfields(s, fields, classFields) do
    present = :ordsets.from_list(for {f, _} <- fields do
                                   f
                                 end)
    mandatory0 = get_mandatory_class_fields(classFields)
    mandatory = :ordsets.from_list(mandatory0)
    all = :ordsets.from_list(for f <- classFields do
                               :erlang.element(2, f)
                             end)
    r_state(tname: obj) = s
  ...
  defp check_defaultfields_1(s, [{fName, spec} | fields], classFields,
            acc) do
  ...
  defp is_mandatory_class_field({:variabletypevaluesetfield, _, _,
             :MANDATORY}) do
    true
  end
  ...
  defp merged_name(s, eRef = r_Externaltypereference(module: m)) do
    case ({r_state(s, :mname),
             :lists.member(m, r_state(s, :inputmodules))}) do
      {^m, _} ->
        eRef
      {mergeM, true} ->
  ...
  defp check_value(s,
            r_valuedef(pos: pos, name: name, type: type,
                value: {:valueset, constr})) do
    newType = r_type(type, constraint: [constr])
    {:valueset,
       check_type(s,
                    r_typedef(pos: pos, name: name, typespec: newType), newType)}
  end
  ...
  defp check_valuedef(r_state(recordtopname: topName) = s0, v0) do
    r_valuedef(name: name, type: vtype0, value: value,
        module: modName) = v0
    v = r_valuedef(v0, checked: true)
    vtype1 = expand_valuedef_type(vtype0)
    vtype = check_type(s0, r_typedef(name: name, typespec: vtype1),
                         vtype1)
    ...
    case (def__) do
      r_Externaltypereference(type: recName) = ext ->
        ...
        case (type) do
          r_typedef(typespec: typeSpec0) = typeDef ->
            ...
            s3 = (case (is_contextswitchtype(type)) do
                    true ->
                      s2
                    false ->
                      r_state(s2, recordtopname: [recName | topName])
                  end)
            r_valuedef(value: checkedVal) = check_value(s3,
                                                 r_valuedef(v0, type: typeSpec))
            r_valuedef(v, value: checkedVal)
          r_type() ->
            r_valuedef(value: checkedVal) = check_value(r_state(s2, recordtopname: [recName |
                                                                         topName]),
      ...
      :ASN1_OPEN_TYPE ->
        ...
        checkedV = check_value(sVal,
                                 r_valuedef(name: name, type: aNYType, value: aNYValue,
                                     module: modName))
      ...
      r_SEQUENCE() ->
        ...
        r_valuedef(v, value: normalize_value(sVal, vtype, seqVal,
                                      topName))
      _ ->
        r_valuedef(v, value: normalize_value(sVal, vtype, value,
                                      topName))
  ...
  defp expand_valuedef_type(r_type(def: seq) = type)
    ...
    newComponents = (case (r_SEQUENCE(seq, :components)) do
                       {r1, _Ext, r2} ->
                         r1 ++ r2
                       {root, _Ext} ->
                         root
  ...
  defp expand_valuedef_type(r_type(def: set) = type)
    ...
    newComponents = (case (r_SET(set, :components)) do
                       {r1, _Ext, r2} ->
                         r1 ++ r2
                       {root, _Ext} ->
                         root
  ...
  defp validate_objectidentifier(s, oidType,
            {:ValueFromObject, {:object, obj}, fields}) do
  ...
  defp validate_objectidentifier(s, oidType,
            [{r_seqtag(module: mod, pos: pos, val: atom), val}]) do
  ...
  defp validate_oid(s, oidType,
            [{:NamedNumber, _Name, value} | vrest], acc)
      when is_integer(value) do
    validate_oid(s, oidType, vrest, [value | acc])
  end

  defp validate_oid(s, oidType, [r_Externalvaluereference() = id | vrest], acc) do
    neededOidType = (case (acc) do
                       [] when oidType === :o_id ->
                         :o_id
                       _ ->
                         :rel_oid
  ...
  defp validate_oid_path(_, :o_id, [0, i | _]) when (0 <= i and
                                        i <= 9) do
    :ok
  end

  defp validate_oid_path(_, :o_id, [1, i | _]) when (0 <= i and
                                        i <= 3) do
    :ok
  end
  ...
  defp to_EXTERNAL1990(s,
    ...
    to_EXTERNAL1990(s, rest,
                      [{r_seqtag(t, val: :"indirect-reference"), pCid}, {r_seqtag(t, val: :"direct-reference"), trStx}])
  end
  ...
  defp normalize_value(s, type, {:DEFAULT, value}, nameList) do
    case ((try do
      ...
      {:CHOICE, cType, newNameList} ->
        choiceComponents = get_choice_components(s,
                                                   {:CHOICE, cType})
        normalize_choice(s, value, choiceComponents,
                           newNameList)
  ...
  defp normalize_boolean(s, bool = r_Externalvaluereference(), cType) do
    get_normalized_value(s, bool, cType,
                           &normalize_boolean/3, [])
  end
  ...
  defp normalize_integer(s,
            {:ValueFromObject, {:object, obj}, fieldNames}, _) do
  ...
  defp normalize_choice(s, {:CHOICE, {c, v}}, cType, nameList)
    ...
    case (:lists.keyfind(c, r_ComponentType(:name), cType)) do
      r_ComponentType(typespec: cT, name: name) ->
        {c,
           normalize_value(s, cT, {:DEFAULT, v},
                             [name | nameList])}
      false ->
  ...
  defp normalize_sequence(s, value, components, nameList)
    ...
    normalize_sequence(s, value,
                         :lists.flatten(:erlang.tuple_to_list(components)),
                         nameList)
  end
  ...
  defp normalize_sequence(s, value, components, nameList) do
    normalized_record(:SEQUENCE, s, value, components,
                        nameList)
  end

  defp normalize_set(s, value, components, nameList)
      when is_tuple(components) do
    normalize_set(s, value,
                    :lists.flatten(:erlang.tuple_to_list(components)),
                    nameList)
  end
  ...
  defp normalize_set(s, value, components, nameList) do
    ...
    case (is_record_normalized(s, newName, value,
                                 length(components))) do
      true ->
        value
      _ ->
        sortedVal = sort_value(components, value)
        normalized_record(:SET, s, sortedVal, components,
                            nameList)
    end
  end

  defp sort_value(components, value0) when is_list(value0) do
    {keys0, _} = :lists.mapfoldl(fn r_ComponentType(name: n), i ->
                                      {{n, i}, i + 1}
                                 end,
                                   0, components)
    keys = :gb_trees.from_orddict(:orddict.from_list(keys0))
    value1 = (for ({r_seqtag(val: n), _} = pair) <- value0 do
  ...
  defp normalized_record(sorS, s, value, components, nameList) do
    newName = :erlang.list_to_atom(:lists.concat([get_record_prefix_name(s),
    ...
    case (is_record_normalized(s, newName, value,
      ...
      false ->
        ...
        listOfVals = normalize_seq_or_set(sorS, s, value,
                                            components, nameList, [])
  ...
  defp is_record_normalized(_S, name, value, numComps)
    ...
    tuple_size(value) === numComps + 1 and :erlang.element(1,
                                                             value) === name
  end
  ...
  defp normalize_seq_or_set(sorS, s, [{r_seqtag(val: cname), v} | vs],
            [r_ComponentType(name: cname, typespec: tS) | cs], nameList, acc) do
    newNameList = (case (r_type(tS, :def)) do
                     r_Externaltypereference(type: tName) ->
                       [tName]
                     _ ->
                       [cname | nameList]
                   end)
    nVal = normalize_value(s, tS, {:DEFAULT, v},
                             newNameList)
    normalize_seq_or_set(sorS, s, vs, cs, nameList,
                           [nVal | acc])
  end

  defp normalize_seq_or_set(sorS, s, values = [{r_seqtag(val: cname0), _V} | _Vs],
            [r_ComponentType(prop: :OPTIONAL) | cs], nameList, acc) do
    verify_valid_component(s, cname0, cs)
    normalize_seq_or_set(sorS, s, values, cs, nameList,
                           [:asn1_NOVALUE | acc])
  end
  ...
  defp normalize_seq_or_set(sorS, s, values = [{r_seqtag(val: cname0), _V} | _Vs],
    ...
    nVal = normalize_value(s, tS, {:DEFAULT, value},
                             newNameList)
    normalize_seq_or_set(sorS, s, values, cs, nameList,
                           [nVal | acc])
  end

  defp normalize_seq_or_set(sorS, s, [],
    ...
    nVal = normalize_value(s, tS, {:DEFAULT, value},
                             newNameList)
    normalize_seq_or_set(sorS, s, [], cs, nameList,
                           [nVal | acc])
  end

  defp normalize_seq_or_set(sorS, s, [], [r_ComponentType(prop: :OPTIONAL) | cs],
            nameList, acc) do
    normalize_seq_or_set(sorS, s, [], cs, nameList,
                           [:asn1_NOVALUE | acc])
  end
  ...
  defp normalize_seq_or_set(sorS, s, value = r_Externalvaluereference(), cs, nameList, acc) do
    get_normalized_value(s, value, cs,
                           &normalize_seq_or_set/6, [sorS, nameList, acc])
  end
  ...
  defp normalize_s_of(sorS, s, value, type, nameList)
    ...
    get_normalized_value(s, value, type, &normalize_s_of/5,
                           [sorS, nameList])
  end

  defp normalize_restrictedstring(s, [h | t], cType) when is_list(h) or
                                    is_tuple(h) do
  ...
  defp normalize_objectclassfieldvalue(s, {:opentypefieldvalue, type, value},
            nameList) do
  ...
  defp get_canonic_type(s, type, nameList) do
    {innerType, newType,
       newNameList} = (case (r_type(type, :def)) do
                         :INTEGER = name ->
                           {name, [], nameList}
                         ...
                         ref when elem(ref, 0) === :Externaltypereference ->
                           {_,
                              r_typedef(name: name,
                                  typespec: refedType)} = get_referenced_type(s,
                                                                                ref)
    ...
    {:asn1ct_gen.unify_if_string(innerType), newType,
       newNameList}
  end
  ...
  defp check_ptype(s, type, ts) when elem(ts, 0) === :type do
    ...
    def__ = r_type(ts, :def)
    newDef = (case (def__) do
                seq when elem(seq, 0) === :SEQUENCE ->
                  components = expand_components(s, r_SEQUENCE(seq, :components))
                  r_newt(type: r_SEQUENCE(seq, pname: get_datastr_name(type), 
                                   components: components))
                set when elem(set, 0) === :SET ->
                  components = expand_components(s, r_SET(set, :components))
                  r_newt(type: r_SET(set, pname: get_datastr_name(type), 
                                   components: components))
                _Other ->
                  r_newt()
              end)
  ...
  defp check_type(s = r_state(recordtopname: topName), type, ts)
    ...
    {def__, tag, constr,
       isInlined} = (case (match_parameter(s, r_type(ts, :def))) do
                       r_type(tag: pTag, constraint: _Ctmp, def: dtmp,
                           inlined: inl) ->
                         {dtmp, merge_tags(r_type(ts, :tag), pTag),
                            r_type(ts, :constraint), inl}
                       r_typedef(typespec: r_type(tag: pTag, def: dtmp, inlined: inl)) ->
                         {dtmp, merge_tags(r_type(ts, :tag), pTag),
                            r_type(ts, :constraint), inl}
                       dtmp ->
                         {dtmp, r_type(ts, :tag), r_type(ts, :constraint),
                            r_type(ts, :inlined)}
                     end)
    tempNewDef = r_newt(type: def__, tag: tag,
                     constraint: constr, inlined: isInlined)
    testFun = fn tref ->
    ...
    newDef = (case (def__) do
                ext when elem(ext, 0) === :Externaltypereference ->
                  {refMod, refTypeDef,
                     isParamDef} = (case (get_referenced_type(s, ext)) do
                                      {:undefined, tmpTDef} ->
                                        {:erlang.get(:top_module), tmpTDef,
                                           true}
                                      {tmpRefMod, tmpRefDef} ->
                  ...
                  ct = testFun.(ext)
                  {refType, extRef} = (case (r_typedef(refTypeDef, :checked)) do
                                         true ->
                                         ...
                                         _ ->
                                           ...
                                           :asn1_db.dbput(refMod,
                                                            ...
                                                            newRefTypeDef1)
                                           newS = r_state(s, mname: refMod, 
                                                         module: load_asn1_module(s,
                                                                                    refMod), 
                                                         tname: get_datastr_name(newRefTypeDef1), 
                                                         abscomppath: [], 
                                                         recordtopname: [])
                                           refType1 = check_type(newS,
                                                                   refTypeDef,
                                                                   r_typedef(refTypeDef, :typespec))
                                           newRefTypeDef2 = r_typedef(refTypeDef, checked: true, 
                                                                            typespec: refType1)
                                           tmpName = get_datastr_name(newRefTypeDef2)
                                           :asn1_db.dbput(refMod, tmpName,
                                                            newRefTypeDef2)
                                           case ({refMod == :erlang.get(:top_module),
                                                    isParamDef}) do
                                             {true, true} ->
                                               key = {tmpName, :type,
                                                        newRefTypeDef2}
                                           ...
                                           {refType1,
                                              r_Externaltypereference(module: refMod, pos: pos,
                                                  type: tmpName)}
                                       end)
                  case (:asn1ct_gen.prim_bif(:asn1ct_gen.get_inner(r_type(refType, :def)))) do
                    true ->
                      newC = check_constraints(s, refType,
                                                 constr ++ r_type(refType, :constraint))
                      r_newt(tempNewDef, type: r_type(refType, :def), 
                                      tag: merge_tags(ct, r_type(refType, :tag)), 
                                      constraint: newC)
                    _ ->
                      newExt = r_Externaltypereference(extRef, module: merged_mod(s, refMod, ext))
                      r_newt(tempNewDef, type: check_externaltypereference(s,
                                                                        newExt), 
                                      tag: merge_tags(ct, r_type(refType, :tag)))
                  end
                :ANY ->
                ...
                {:ANY_DEFINED_BY, _} ->
                  ...
                  r_newt(tempNewDef, type: :ASN1_OPEN_TYPE,  tag: ct)
                :INTEGER ->
                  r_newt(tempNewDef, tag: merge_tags(tag,
                                                  r_tag(class: :UNIVERSAL,
                                                      number: 2,
                                                      type: :IMPLICIT,
                                                      form: 0)))
                {:INTEGER, namedNumberList} ->
                  r_newt(tempNewDef, type: {:INTEGER,
                                         check_integer(s, namedNumberList)}, 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 2,
                                                        type: :IMPLICIT,
                ...
                {:"BIT STRING", namedNumberList} ->
                  ...
                  r_newt(tempNewDef, type: {:"BIT STRING", newL}, 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 3,
                                                        type: :IMPLICIT,
                ...
                :EXTERNAL ->
                  ...
                  r_newt(tempNewDef, type: r_Externaltypereference(module: r_state(s, :mname),
                                          type: :EXTERNAL), 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 8,
                ...
                {:"INSTANCE OF", definedObjectClass, constraint} ->
                  iOFDef = check_instance_of(s, definedObjectClass,
                                               constraint)
                  r_newt(tempNewDef, type: iOFDef, 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 8,
                                                        type: :IMPLICIT,
                ...
                {:ENUMERATED, namedNumberList} ->
                  r_newt(tempNewDef, type: {:ENUMERATED,
                ...
                :"EMBEDDED PDV" ->
                  ...
                  r_newt(tempNewDef, type: r_Externaltypereference(module: r_state(s, :mname), type: :"EMBEDDED PDV"), 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 11,
                                                        type: :IMPLICIT,
                ...
                :NumericString ->
                  ...
                  r_newt(tempNewDef, tag: merge_tags(tag,
                                                  r_tag(class: :UNIVERSAL,
                                                      ...
                                                      type: :IMPLICIT,
                                                      form: 0)))
                tString when tString === :TeletexString or
                               tString === :T61String
                             ->
                  check_restrictedstring(s, def__, constr)
                  r_newt(tempNewDef, tag: merge_tags(tag,
                ...
                :"CHARACTER STRING" ->
                  ...
                  r_newt(tempNewDef, type: r_Externaltypereference(module: r_state(s, :mname), type: :"CHARACTER STRING"), 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 29,
                                                        type: :IMPLICIT,
                ...
                seq when elem(seq, 0) === :SEQUENCE ->
                  ...
                  {tableCInf,
                     components} = check_sequence(r_state(s, recordtopname: recordName),
                                                    type, r_SEQUENCE(seq, :components))
                  r_newt(tempNewDef, type: r_SEQUENCE(seq, tablecinf: tablecinf_choose(seq,
                                                                           tableCInf), 
                                               components: components), 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                ...
                {:"SEQUENCE OF", components} ->
                  r_newt(tempNewDef, type: {:"SEQUENCE OF",
                                         check_sequenceof(s, type,
                                                            components)}, 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 16,
                ...
                set when elem(set, 0) === :SET ->
                  ...
                  {sorted, tableCInf,
                     components} = check_set(r_state(s, recordtopname: recordName),
                                               type, r_SET(set, :components))
                  r_newt(tempNewDef, type: r_SET(set, sorted: sorted, 
                                               tablecinf: tablecinf_choose(set,
                                                                             tableCInf), 
                                               components: components), 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        ...
                                                        form: 32)))
                {:"SET OF", components} ->
                  r_newt(tempNewDef, type: {:"SET OF",
                                         check_setof(s, type, components)}, 
                                  tag: merge_tags(tag,
                                                    r_tag(class: :UNIVERSAL,
                                                        number: 17,
                                                        type: :IMPLICIT,
                ...
                {:pt, ptype, paraList} ->
                  ...
                  r_newt(tempNewDef, type: r_type(instance, :def), 
                ...
                r_ObjectClassFieldType(classname: clRef0) = oCFT0 ->
                  ...
                  newTypeDef = maybe_open_type(s, classSpec,
                                                 r_ObjectClassFieldType(oCFT, class: classSpec),
                                                 constr)
                  innerTag = get_innertag(s, newTypeDef)
                  mergedTag = merge_tags(tag, innerTag)
                  ct = (case (is_open_type(newTypeDef)) do
                          true ->
                  ...
                  case (topName) do
                    [] when r_typedef(type, :name) !== :undefined ->
                      r_type(constraint: c,
                          def: simplified) = simplify_type(r_type(def: newTypeDef,
                                                               constraint: constr))
                      r_newt(tempNewDef, type: simplified,  tag: ct, 
                                      constraint: c)
                    _ ->
                ...
                {:TypeFromObject, {:object, object}, typeField} ->
                  ...
                  r_newt(tempNewDef, tag: merge_tags(tag, r_type(checkedT, :tag)), 
    ...
    r_newt(type: tDef, tag: newTags, constraint: newConstr,
        inlined: inlined) = newDef
    r_type(ts, def: tDef,  inlined: inlined, 
            constraint: check_constraints(s, r_type(def: tDef),
                                            newConstr), 
            tag: :lists.map(fn r_tag(type: {:default, tTx}) = tempTag ->
  ...
  defp simplify_type(r_type(tag: tag, def: inner,
              constraint: constr0) = t) do
    case (inner) do
  ...
  defp maybe_open_type(s, r_objectclass(fields: fs) = classSpec,
            r_ObjectClassFieldType(fieldname: fieldRefList) = oCFT, constr) do
  ...
  defp get_componentrelation([{:element_set, {:componentrelation, _, _} = cr,
              :none} |
               _]) do
    cr
  end
  ...
  defp instantiate_ptype(s, ptypedef, paraList) do
    ...
    newType = check_ptype(s, ptypedef,
                            r_type(type, inlined: :yes))
    matchedArgs = match_args(s, args, paraList, [])
    oldArgs = r_state(s, :parameters)
    newS = r_state(s, parameters: matchedArgs ++ oldArgs, 
                  abscomppath: [])
    check_type(newS, r_ptypedef(ptypedef, typespec: newType),
                 newType)
  end
  ...
  defp match_args(s, fA = [formArg | ft], aA = [actArg | at],
            acc) do
    oldParams = r_state(s, :parameters)
    case (categorize_arg(s, formArg, actArg)) do
      [categorizedArg] ->
        match_args(r_state(s, parameters: [{formArg, categorizedArg} |
                                         oldParams]),
                     ft, at, [{formArg, categorizedArg} | acc])
      categorizedArgs ->
        match_args(r_state(s, parameters: categorizedArgs ++ oldParams),
  ...
  defp categorize_arg(s, {governor, param}, actArg) do
    case ({governor_category(s, governor),
             parameter_name_style(param)}) do
  ...
  defp categorize({:object, _, type}) do
    def__ = fn d = r_type() ->
                 r_typedef(name: new_reference_name('type_argument'),
  ...
  defp categorize(s, :object_set, def__, classRef) do
    newObjSetSpec = check_object(s, def__,
                                   r_ObjectSet(class: classRef,
  ...
  defp top_level_intersections([{:element_set, {:intersection, _, _} = c,
              :none}]) do
  ...
  defp coalesce_constraints({:element_set, {tag, {:element_set, a, _}},
             {tag, {:element_set, b, _}}}) do
  ...
  defp remove_extension(tuple) when is_tuple(tuple) do
    l = (for el <- :erlang.tuple_to_list(tuple) do
           remove_extension(el)
         end)
  ...
  defp any_extension_tuple(i, t) when i <= tuple_size(t) do
    any_extension(:erlang.element(i,
                                    t)) or any_extension_tuple(i + 1, t)
  end
  ...
  defp simplify_element_set(s, hostType,
            {:simpletable, {:element_set, type, _}}) do
  ...
  defp finish_constraints_1([{:element_set,
              {:SizeConstraint, {:element_set, root, :none}},
              {:set, []} = set} |
               t],
            collapse) do
    c = {:element_set,
           {:SizeConstraint, {:element_set, root, set}}, :none}
    finish_constraints_1([c | t], collapse)
  end

  defp finish_constraints_1([{:element_set,
              {tag, {:element_set, _, _} = set0}, :none} |
               t],
            collapse0) do
  ...
  defp finish_constraints_1([{:element_set, {:set, [{:a_range, :MAX}]}, _} |
               t],
            collapse) do
    finish_constraints_1(t, collapse)
  end

  defp finish_constraints_1([{:element_set, {:intersection, a0, b0},
              :none} |
               t],
            collapse) do
  ...
  defp single_value_collapse(v) do
    {:SingleValue,
       :ordsets.from_list(single_value_collapse_1(v))}
  end

  defp single_value_collapse_1([{:range, lb, ub} | t]) when (is_integer(lb) and
                                          is_integer(ub)) do
  ...
  defp smart_collapse([_ | _] = l) do
    v = :lists.foldr(fn {:range, lb, ub}, a ->
                          seq(lb, ub) ++ a
                     end,
                       [], l)
    {:SingleValue, v}
  ...
  defp seq(lb, ub) when (is_integer(lb) and
                          is_integer(ub)) do
    :lists.seq(lb, ub)
  end
  ...
  defp resolve_value1(s, hostType, r_Externalvaluereference(value: name) = eRef) do
    case (resolve_namednumber(s, hostType, name)) do
      v when is_integer(v) ->
        v
      :not_named ->
        resolve_value1(s, hostType,
                         get_referenced_value(s, eRef))
    end
  end
  ...
  defp resolve_value1(s, _HostType,
            {:ValueFromObject, {:object, object}, fieldName}) do
  ...
  defp resolve_value1(s, _,
            r_valuedef(value: {:ValueFromObject, {:object, object},
                        fieldName})) do
    get_value_from_object(s, object, fieldName)
  end
  ...
  defp do_get_ref_type_2(r_state(mname: current, inputmodules: modules) = s,
            pos, m, t) do
  ...
  defp save_object_set_instance(s, name, objSetSpec)
    ...
    newObjSet = r_typedef(checked: true, name: name,
                    typespec: objSetSpec)
  ...
  defp renamed_reference(s, name, module) do
    case (:asn1ct_table.exists(:renamed_defs)) do
      false ->
        :undefined
      true ->
        case (:asn1ct_table.match(:renamed_defs,
                                    {:"$1", name, module})) do
          [] ->
            case (:asn1ct_table.exists(:original_imports)) do
              false ->
                :undefined
              true ->
                case (:asn1ct_table.match(:original_imports,
                                            {module, :"$1"})) do
                  [] ->
                    :undefined
                  [[importsList]] ->
  ...
  defp match_parameter(s, {:valueset, {:element_set, r_type() = ts, :none}},
            ps) do
    match_parameter(s, {:valueset, ts}, ps)
  end

  defp match_parameter(_S, r_Externaltypereference(type: name),
            [{r_Externaltypereference(type: name), newName} | _T]) do
    newName
  end

  defp match_parameter(_S, r_Externaltypereference(type: name),
            [{{_, r_Externaltypereference(type: name)}, newName} | _T]) do
    newName
  end

  defp match_parameter(_S, r_Externalvaluereference(value: name),
            [{r_Externalvaluereference(value: name), newName} | _T]) do
    newName
  end

  defp match_parameter(_S, r_Externalvaluereference(value: name),
            [{{_, r_Externalvaluereference(value: name)}, newName} | _T]) do
    newName
  end

  defp match_parameter(_S, r_type(def: r_Externaltypereference(module: m, type: name)),
            [{r_Externaltypereference(module: m, type: name), type}]) do
    type
  end
  ...
  defp match_parameter(_S, {:valueset, r_type(def: r_Externaltypereference(type: name))},
            [{{_, r_Externaltypereference(type: name)}, newName = r_type(def: r_Externaltypereference())} | _T]) do
    r_type(newName, :def)
  end
  ...
  defp match_parameter(s, {:valueset, t = r_type(def: {:pt, _, _Args})},
            _Ps) do
    try do
      ...
      :pobjectsetdef ->
        ...
        objSpec = check_object(s, r_typedef(typespec: objectSet),
                                 objectSet)
        name = :erlang.list_to_atom(:asn1ct_gen.list2name([get_datastr_name(objDef) |
  ...
  defp match_parameter(s,
            {:valueset, {:pos, {:objectset, _, pOSref}, args}},
            ps) do
    match_parameter(s,
                      {:valueset, r_type(def: {:pt, pOSref, args})}, ps)
  end
  ...
  defp imported1(name,
            [r_SymbolsFromModule(symbols: symlist, module: r_Externaltypereference(type: moduleName)) |
                 t]) do
  ...
  defp check_named_number_list(s, nNL0) do
    case (check_unique(nNL0, 2)) do
      [] ->
        nNL1 = (for {:NamedNumber, id, val} <- nNL0 do
  ...
  defp iof_associated_type(s, []) do
    case (:erlang.get(:instance_of)) do
      :undefined ->
        ...
        tag = [r_tag(class: :UNIVERSAL, number: 8, type: :IMPLICIT,
                   form: 32)]
        typeDef = r_typedef(checked: true, name: :"INSTANCE OF",
  ...
  defp iof_associated_type1(s, c) do
    {tableCInf, comp1Cnstr, comp2Cnstr,
       comp2tablecinf} = instance_of_constraints(s, c)
    moduleName = r_state(s, :mname)
    typefield_type = (case (c) do
                        [] ->
                          :ASN1_OPEN_TYPE
                        _ ->
                          {:typefield, :Type}
    ...
    c1TypeTag = [r_tag(class: :UNIVERSAL, number: 6,
                     type: :IMPLICIT, form: 0)]
    typeIdentifierRef = r_Externaltypereference(module: moduleName, type: :"TYPE-IDENTIFIER")
    objectIdentifier = r_ObjectClassFieldType(classname: typeIdentifierRef,
                           class: [], fieldname: {:id, []},
                           type: {:fixedtypevaluefield, :id, r_type(def: :"OBJECT IDENTIFIER")})
    typefield = r_ObjectClassFieldType(classname: typeIdentifierRef, class: [],
                    fieldname: {:Type, []}, type: typefield_type)
    iOFComponents0 = [r_ComponentType(name: :"type-id",
                          typespec: r_type(tag: c1TypeTag, def: objectIdentifier,
                                        constraint: comp1Cnstr),
                          prop: :mandatory, tags: objIdTag),
                          r_ComponentType(name: :value,
                              typespec: r_type(tag: [r_tag(class: :CONTEXT, number: 0,
                                                    type: :EXPLICIT, form: 32)],
                                            def: typefield,
                                            constraint: comp2Cnstr,
                                            tablecinf: comp2tablecinf),
                              prop: :mandatory, tags: [{:CONTEXT, 0}])]
    iOFComponents = textual_order(iOFComponents0)
    r_SEQUENCE(tablecinf: tableCInf,
  ...
  defp instance_of_constraints(s,
            [{:element_set, {:simpletable, c}, :none}]) do
  ...
  defp instance_of_constraints_1(s, type) do
    ...
    cRel = [{:componentrelation,
               {:objectset, :undefined, objectSetRef},
               [{:innermost, [r_Externalvaluereference(module: moduleName, value: :type)]}]}]
    mod = r_state(s, :mname)
    tableCInf = r_simpletableattributes(objectsetname: {mod, name}, c_name: :"type-id",
                    c_index: 1, usedclassfield: :id, uniqueclassfield: :id,
                    valueindex: [])
    {tableCInf, [{:simpletable, name}], cRel,
       [{:objfun, objectSetRef}]}
  end
  ...
  defp check_enum(s, [:EXTENSIONMARK | ext0], used0, acc0) do
    ...
    {root, used, cnt} = check_enum_number_root(acc, used0,
                                                 0, [])
  ...
  defp check_enum_number_root([id | t] = t0, used0, cnt, acc)
    ...
    case (:gb_sets.is_element(cnt, used0)) do
      false ->
        ...
        check_enum_number_root(t, used, cnt + 1,
                                 [{id, cnt} | acc])
      true ->
  ...
  defp check_enum_ext(s, [{:NamedNumber, id, n} | t], used0, c,
            acc) do
    used = check_enum_update_used(s, id, n, used0)
    cond do
      n < c ->
  ...
  defp check_sequence(s, type, comps) do
    ...
    case (check_unique(for c <- components,
                             elem(c, 0) === :ComponentType do
                         c
                       end,
                         r_ComponentType(:name))) do
      [] ->
        ...
        {cRelInf, newComps2} = componentrelation_leadingattr(s,
                                                               newComps)
        compListWithTblInf = get_tableconstraint_info(s, type,
                                                        newComps2)
  ...
  defp expand_components(s, [{:"COMPONENTS OF", type} | t]) do
    compList = expand_components2(s,
                                    get_referenced_type(s, r_type(type, :def)))
    expand_components(s, compList) ++ expand_components(s,
                                                          t)
  end
  ...
  defp expand_components(s, {acc, ext, acc2}) do
    expand_components(s, acc ++ ext ++ acc2)
  end

  defp expand_components(s, {acc, ext}) do
  ...
  defp expand_components2(_S, {_, r_typedef(typespec: refType = r_type(def: r_Externaltypereference()))}) do
  ...
  defp expand_components2(s, {_, oCFT = r_ObjectClassFieldType()}) do
    ...
    type = check_type(s, r_typedef(typespec: uncheckedType),
                        uncheckedType)
  ...
  defp check_unique_sequence_tags(s, compList) do
    tagComps = (case (:asn1ct_gen.complist_as_tuple(compList)) do
                  {r1, ext, r2} ->
                    r1 ++ (for (c = r_ComponentType()) <- ext do
  ...
  defp check_set(s, type, components) do
    {tableCInf, newComponents} = check_sequence(s, type,
                                                  components)
    check_unique_tags(s, collect_components(newComponents),
                        [])
    case ({:lists.member(:der, r_state(s, :options)),
             r_state(s, :erule)}) do
      {true, _} ->
        {sorted, sortedComponents} = sort_components(:der, s,
                                                       newComponents)
        {sorted, tableCInf, sortedComponents}
      {_, pER} when pER === :per or pER === :uper ->
        {sorted, sortedComponents} = sort_components(:per, s,
                                                       newComponents)
        {sorted, tableCInf, sortedComponents}
      _ ->
  ...
  defp sort_components1(cs0) do
    cs1 = (for (r_ComponentType(tags: [tag | _]) = c) <- cs0 do
             {tag_key(tag), c}
           end)
    cs = :lists.sort(cs1)
    for {_, c} <- cs do
  ...
  defp untagged_choice(_S,
            [r_ComponentType(typespec: r_type(tag: [], def: {:CHOICE, _})) | _Rest]) do
    true
  end
  ...
  defp tag_untagged_choice(s,
            [c = r_ComponentType(typespec: r_type(tag: [], def: exRef)) | rest], acc)
      when elem(exRef, 0) === :Externaltypereference do
    case (get_referenced_type(s, exRef)) do
      {_, r_typedef(typespec: r_type(tag: [], def: {:CHOICE, _}))} ->
        tagList = r_ComponentType(c, :tags)
  ...
  defp textual_order(cs) do
    fun = fn c = r_ComponentType(), index ->
  ...
  defp check_selectiontype(s, name, r_type(def: eref))
    ...
    newS = r_state(s, module: load_asn1_module(s, refMod), 
                  mname: refMod,  tname: get_datastr_name(typeDef))
    check_selectiontype2(newS, name, typeDef)
  end

  defp check_selectiontype(s, name, type = r_type(def: {:pt, _, _})) do
    tName = (case (r_state(s, :recordtopname)) do
               [] ->
                 r_state(s, :tname)
               n ->
                 n
  ...
  defp check_selectiontype2(s, name, typeDef) do
    ...
    components = (try do
                    checkedType = check_type(newS, typeDef,
                                               r_typedef(typeDef, :typespec))
  ...
  defp get_choice_components(_S, {:CHOICE, {c1, c2}}) when (is_list(c1) and
                                           is_list(c2)) do
    c1 ++ c2
  end
  ...
  defp check_choice(s, type, components) when is_list(components) do
    components1 = (for (c = r_ComponentType()) <- components do
                     c
                   end)
    case (check_unique(components1, r_ComponentType(:name))) do
      [] ->
        ...
        newComps2 = :lists.filter(fn r_ExtensionAdditionGroup() ->
                                       false
  ...
  defp generate_automatic_tags1([h | t], [tagNo | tagNos])
    ...
    newTs = r_type(ts, tag: [r_tag(class: :CONTEXT, number: tagNo,
                            type: {:default, :IMPLICIT}, form: 0)])
    [r_ComponentType(h, typespec: newTs) | generate_automatic_tags1(t,
                                                        [tagNo + 1 | tagNos])]
  end
  ...
  defp check_unique2([a, b | t], pos, acc) when :erlang.element(pos,
                                                       a) == :erlang.element(pos,
                                                                               b) do
    check_unique2([b | t], pos,
                    [:erlang.element(pos, b) | acc])
  end
  ...
  defp check_each_component2(s = r_state(abscomppath: path,
                  recordtopname: topName),
            type,
            [c = r_ComponentType(name: cname, typespec: ts, prop: prop) | ct],
            acc) do
    newAbsCPath = (case (r_type(ts, :def)) do
                     r_Externaltypereference() ->
                       []
                     _ ->
                       [cname | path]
                   end)
    checkedTs = check_type(r_state(s, abscomppath: newAbsCPath, 
                                  recordtopname: [cname | topName]),
                             type, ts)
    newTags = get_taglist(s, checkedTs)
    newProp = (case (normalize_value(s, checkedTs, prop,
                                       [cname | topName])) do
                 :mandatory ->
                   :mandatory
                 :OPTIONAL ->
    ...
    newC = r_ComponentType(c, typespec: checkedTs,  prop: newProp, 
                  tags: newTags)
  ...
  defp check_each_alternative2(s = r_state(abscomppath: path,
                  recordtopname: topName),
            type, [c = r_ComponentType(name: cname, typespec: ts) | ct], acc) do
    newAbsCPath = (case (r_type(ts, :def)) do
                     r_Externaltypereference() ->
                       []
                     _ ->
  ...
  defp check_each_alternative2(s, type, [otherMarker | ct], acc) do
    check_each_alternative2(s, type, ct,
                              [otherMarker | acc])
  end
  ...
  defp componentrelation_leadingattr(s, [c = r_ComponentType() | cs], compList, acc, compAcc) do
    {lAAcc, newC} = (case ((try do
                             componentrelation1(s, r_ComponentType(c, :typespec),
                                                  [r_ComponentType(c, :name)])
                           catch
                             :error, e -> {:EXIT, {e, __STACKTRACE__}}
                             :exit, e -> {:EXIT, e}
                             e -> e
                       ...
                       {cRI = [{_A1, _B1, _C1, _D1} | _Rest], newTSpec} ->
                         case (leading_attr_index(s, compList, cRI,
                                                    :lists.reverse(r_state(s, :abscomppath)),
                                                    [])) do
                           [] ->
                             {[], c}
                           [{objSet, attr, n, classDef, _Path, valueIndex} |
                             ...
                             res = r_simpletableattributes(objectsetname: oS, c_name: attr,
                                       c_index: n, usedclassfield: uniqFN,
    ...
    componentrelation_leadingattr(s, cs, compList,
                                    lAAcc ++ acc, [newC | compAcc])
  end

  defp componentrelation_leadingattr(s, [notComponentType | cs], compList,
            leadingAttr, newCompList) do
    componentrelation_leadingattr(s, cs, compList,
                                    leadingAttr,
                                    [notComponentType | newCompList])
  end
  ...
  defp get_simple_table_if_used(s, cs) do
    cNames = (for r_ComponentType(name: name) <- cs do
                name
              end)
    justComponents = (for (c = r_ComponentType()) <- cs do
                        c
                      end)
    refedSimpleTable = any_component_relation(s,
                                                justComponents, cNames, [], [])
    get_simple_table_info(s, cs,
                            remove_doubles(refedSimpleTable))
  end
  ...
  defp get_simple_table_info2(s, r_ComponentType(name: name, typespec: tS), [], path) do
    ...
    case (:lists.keymember(:simpletable, 1,
                             r_type(tS, :constraint))) do
      true ->
  ...
  defp simple_table_info(s,
            r_ObjectClassFieldType(classname: clRef, class: objectClass,
                fieldname: fieldName),
            path) do
    objectClassFieldName = (case (fieldName) do
                              {lastFieldName, []} ->
                                lastFieldName
                              {_FirstFieldName, fieldNames} ->
                                :lists.last(fieldNames)
                            end)
    classDef = (case (objectClass) do
                  [] ->
  ...
  defp any_component_relation(s, [r_ComponentType(name: cName, typespec: type) | cs],
            cNames, namePath, acc) do
    cRelPath = (case (:lists.keyfind(:componentrelation, 1,
                                       r_type(type, :constraint))) do
                  {_, _, atNotation} ->
    ...
    innerAcc = (case ({r_type(type, :inlined),
                         :asn1ct_gen.type(:asn1ct_gen.get_inner(r_type(type, :def)))}) do
                  {:no, {:constructed, :bif}} ->
                    {innerCs,
                       newNamePath} = (case (get_components(r_type(type, :def))) do
                                         t when elem(t, 0) === :type ->
                                           {t, namePath}
                                         iC ->
                                           {iC, [cName | namePath]}
    ...
    any_component_relation(s, cs, cNames, namePath,
  ...
  defp any_component_relation(s, [:ExtensionAdditionGroupEnd | cs], cNames,
            namePath, acc) do
    any_component_relation(s, cs, cNames, namePath, acc)
  end
  ...
  defp evaluate_atpath(_, [], cnames,
            {:innermost, atPath = [ref | _Refs]}) do
  ...
  defp evaluate_atpath(s = r_state(abscomppath: topPath), namePath, cnames,
            {:outermost, atPath = [_Ref | _Refs]}) do
    atPathBelowTop = (case (topPath) do
                        [] ->
                          atPath
                        _ ->
  ...
  defp componentrelation1(s,
            c = r_type(def: def__, constraint: constraint,
                    tablecinf: tCI),
            path) do
    ret = (case (:lists.keyfind(:componentrelation, 1,
                                  constraint)) do
             {_, {_, _, objectSet}, atList} ->
               [{_, aL = [r_Externalvaluereference() | _R1]} | _R2] = atList
               classDef = get_ObjectClassFieldType_classdef(s, def__)
               atPath = :lists.map(fn r_Externalvaluereference(value: v) ->
                                        v
                                   end,
                                     aL)
    ...
    case (ret) do
      ...
      {cRelI = [{objSet, _, _, _}], newDef} ->
        ...
        {cRelI,
           r_type(c, tablecinf: [{:objfun, objSet} | tCItmp], 
                  def: newDef)}
      {compRelInf, newDef} ->
        tCItmp = :lists.subtract(tCI, [{:objfun, :anyset}])
        {compRelInf,
           r_type(c, tablecinf: [{:objfun, :anyset} | tCItmp], 
                  def: newDef)}
    end
  end
  ...
  defp innertype_comprel(s, {:CHOICE, cTypeList}, path) do
    case (componentlist_comprel(s, cTypeList, [], path,
                                  [])) do
      :nofunobj ->
        :nofunobj
      {compRelInf, newCs} ->
  ...
  defp componentlist_comprel(s, [c = r_ComponentType(name: name, typespec: type) | cs],
            acc, path, newCL) do
    case ((try do
  ...
  defp leading_attr_index(s, cs, [h = {_, attrP, _, _} | t], absP, acc) do
    attrInfo = (case (:lists.prefix(absP, attrP)) do
                  true ->
  ...
  defp leading_attr_index1(s, [c | cs], arg = {objectSet, _, cDef, p},
            attrInfo = {attr, subAttr}, n) do
  ...
  defp value_match(s, r_ComponentType(typespec: type), name, [at | ats], acc) do
    ...
    components = (case (get_atlist_components(r_type(type, :def))) do
                    [] ->
                      asn1_error(s, {:invalid_element, name})
                    comps ->
                      comps
                  end)
    {index, valueIndex} = component_value_index(s,
                                                  innerType, at, components)
    value_match(s, :lists.nth(index, components), at, ats,
                  [valueIndex | acc])
  end
  ...
  defp get_tableconstraint_info(s, type, [c = r_ComponentType(typespec: checkedTs) | cs],
            acc) do
    accComp = (case (r_type(checkedTs, :def)) do
                 ...
                 {:"SEQUENCE OF", sOType} when (elem(sOType, 0) === :type and
                                      :erlang.element(1,
                                                        r_type(sOType, :def)) == :CHOICE)
                                   ->
                   cTypeList = :erlang.element(2, r_type(sOType, :def))
                   newInnerCList = get_tableconstraint_info(s, type,
                                                              cTypeList)
                   r_ComponentType(c, typespec: r_type(checkedTs, def: {:"SEQUENCE OF",
                                                       r_type(sOType, def: {:CHOICE,
                                                                         newInnerCList})}))
                 {:"SET OF", sOType} when (elem(sOType, 0) === :type and
                                      :erlang.element(1,
                                                        r_type(sOType, :def)) == :CHOICE)
                                   ->
                   cTypeList = :erlang.element(2, r_type(sOType, :def))
                   newInnerCList = get_tableconstraint_info(s, type,
                                                              cTypeList)
                   r_ComponentType(c, typespec: r_type(checkedTs, def: {:"SET OF",
                                                       r_type(sOType, def: {:CHOICE,
                                                                         newInnerCList})}))
                 _ ->
                   c
  ...
  defp get_OCFType(s, fields,
            fieldnameList = [{_FieldType, _PrimFieldName} | _]) do
    get_OCFType(s, fields,
  ...
  defp get_OCFType(s, fields, [primFieldName | rest]) do
    case (:lists.keysearch(primFieldName, 2, fields)) do
      {:value,
         {:fixedtypevaluefield, _, type, _Unique, _OptSpec}} ->
        {:fixedtypevaluefield, primFieldName, type}
      {:value,
         {:objectfield, _, classRef, _Unique, _OptSpec}} ->
        {mName, classDef} = get_referenced_type(s, classRef)
        newS = update_state(r_state(s, tname: get_datastr_name(classDef)),
                              mName)
      ...
      {:value, {:objectsetfield, _, type, _OptSpec}} ->
        {mName, classDef} = get_referenced_type(s,
                                                  r_type(type, :def))
        newS = update_state(r_state(s, tname: get_datastr_name(classDef)),
                              mName)
        checkedCDef = check_class(newS, classDef)
  ...
  defp get_taglist1(s,
            [r_ComponentType(typespec: ts, tags: :undefined) | rest]) do
  ...
  defp merge_tags2([t1 = r_tag(type: :IMPLICIT), t2 | rest], acc) do
    merge_tags2([r_tag(t1, type: r_tag(t2, :type), 
                         form: r_tag(t2, :form)) |
                     rest],
                  acc)
  end

  defp merge_tags2([t1 = r_tag(type: {:default, :IMPLICIT}), t2 |
                                                    rest],
            acc) do
    merge_tags2([r_tag(t1, type: r_tag(t2, :type), 
                         form: r_tag(t2, :form)) |
  ...
  defp merge_tags2([t1 = r_tag(type: {:default, :AUTOMATIC}), t2 |
                                                     rest],
            acc) do
    merge_tags2([r_tag(t1, type: r_tag(t2, :type), 
                         form: r_tag(t2, :form)) |
  ...
  defp duplicate_def_1(s, [{_, def__} | t], name, prev) do
    e = return_asn1_error(s, def__,
                            {:already_defined, name, prev})
  ...
  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
    ...
    findtypes_and_values(t, tacc, vacc, pacc, cacc,
                           [r_typedef(h, :name) | oacc], oSacc)
  end

  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
      when (elem(h, 0) === :typedef and
              elem(r_typedef(h, :typespec), 0) === :ObjectSet) do
    findtypes_and_values(t, tacc, vacc, pacc, cacc, oacc,
                           [r_typedef(h, :name) | oSacc])
  end

  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
      when elem(h, 0) === :typedef do
    findtypes_and_values(t, [r_typedef(h, :name) | tacc], vacc,
                           pacc, cacc, oacc, oSacc)
  end

  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
      when elem(h, 0) === :valuedef do
    findtypes_and_values(t, tacc, [r_valuedef(h, :name) | vacc],
                           pacc, cacc, oacc, oSacc)
  end

  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
      when elem(h, 0) === :ptypedef do
    findtypes_and_values(t, tacc, vacc,
                           [r_ptypedef(h, :name) | pacc], cacc, oacc, oSacc)
  end

  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
      when elem(h, 0) === :classdef do
    findtypes_and_values(t, tacc, vacc, pacc,
                           [r_classdef(h, :name) | cacc], oacc, oSacc)
  end

  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
      when elem(h, 0) === :pvaluedef do
    findtypes_and_values(t, tacc, [r_pvaluedef(h, :name) | vacc],
                           pacc, cacc, oacc, oSacc)
  end

  defp findtypes_and_values([h | t], tacc, vacc, pacc, cacc, oacc, oSacc)
      when elem(h, 0) === :pvaluesetdef do
    findtypes_and_values(t, tacc, [r_pvaluesetdef(h, :name) | vacc],
                           pacc, cacc, oacc, oSacc)
  end
  ...
  def format_error({:illegal_instance_of, class}) do
    :io_lib.format('using INSTANCE OF on class \'~s\' is illegal, because INSTANCE OF may only be used on the class TYPE-IDENTIFIER', [class])
  end
  ...
  def format_error({:implicit_tag_before, kind}) do
    'illegal implicit tag before ' ++ (case (kind) do
            :choice ->
              '\'CHOICE\''
            :open_type ->
              'open type'
  ...
  def format_error({:missing_table_constraint, component}) do
    :io_lib.format('the component \'~s\' is referenced by a component relation constraint using the \'@field-name\' notation, but does not have a table constraint', [component])
  end
  ...
  defp format_fields([h | t]) do
    [:io_lib.format('fields \'&~s\'', [h]) | for f <- t do
                                :io_lib.format(', \'&~s\'', [f])
                              end]
  end
  ...
  defp default_type_list() do
    syntax = r_ComponentType(name: :syntax, typespec: r_type(def: :"OBJECT IDENTIFIER"),
                 prop: :mandatory)
    presentation_Cid = r_ComponentType(name: :"presentation-context-id",
                           typespec: r_type(def: :INTEGER), prop: :mandatory)
    transfer_syntax = r_ComponentType(name: :"transfer-syntax", typespec: r_type(def: :"OBJECT IDENTIFIER"),
                          prop: :mandatory)
    negotiation_items = r_type(def: r_SEQUENCE(components: [presentation_Cid,
                                                  r_ComponentType(transfer_syntax, prop: :mandatory)]))
    context_negot = r_ComponentType(name: :"context-negotiation", typespec: negotiation_items,
                        prop: :mandatory)
    data_value_descriptor = r_ComponentType(name: :"data-value-descriptor",
                                typespec: r_type(def: :ObjectDescriptor),
                                prop: :OPTIONAL)
    data_value = r_ComponentType(name: :"data-value", typespec: r_type(def: :"OCTET STRING"),
                     prop: :mandatory)
    direct_reference = r_ComponentType(name: :"direct-reference", typespec: r_type(def: :"OBJECT IDENTIFIER"),
                           prop: :OPTIONAL, tags: [{:UNIVERSAL, 6}])
    indirect_reference = r_ComponentType(name: :"indirect-reference",
                             typespec: r_type(def: :INTEGER), prop: :OPTIONAL,
                             tags: [{:UNIVERSAL, 2}])
    single_ASN1_type = r_ComponentType(name: :"single-ASN1-type",
                           typespec: r_type(tag: [{:tag, :CONTEXT, 0, :EXPLICIT,
                                                32}],
                                         def: :ANY),
                           prop: :mandatory, tags: [{:CONTEXT, 0}])
    octet_aligned = r_ComponentType(name: :"octet-aligned",
                        typespec: r_type(tag: [{:tag, :CONTEXT, 1, :IMPLICIT, 0}],
                                      def: :"OCTET STRING"),
                        prop: :mandatory, tags: [{:CONTEXT, 1}])
    arbitrary = r_ComponentType(name: :arbitrary,
                    typespec: r_type(tag: [{:tag, :CONTEXT, 2, :IMPLICIT, 0}],
                                  def: {:"BIT STRING", []}),
                    prop: :mandatory, tags: [{:CONTEXT, 2}])
    encoding = r_ComponentType(name: :encoding,
                   typespec: r_type(def: {:CHOICE,
                                       [single_ASN1_type, octet_aligned,
                                                              arbitrary]}),
                   prop: :mandatory)
    eXTERNAL_components1990 = [direct_reference,
                                   indirect_reference, data_value_descriptor,
                                                           encoding]
    abstract = r_ComponentType(name: :abstract, typespec: r_type(def: :"OBJECT IDENTIFIER"),
                   prop: :mandatory)
    transfer = r_ComponentType(name: :transfer, typespec: r_type(def: :"OBJECT IDENTIFIER"),
                   prop: :mandatory)
    abstractTrSeq = r_SEQUENCE(components: [abstract, transfer])
    syntaxes = r_ComponentType(name: :syntaxes,
                   typespec: r_type(def: abstractTrSeq), prop: :mandatory)
    fixed = r_ComponentType(name: :fixed, typespec: r_type(def: :NULL),
                prop: :mandatory)
    negotiations = [syntaxes, syntax, presentation_Cid,
                                          context_negot, transfer_syntax, fixed]
    identification2 = r_ComponentType(name: :identification,
                          typespec: r_type(def: {:CHOICE, negotiations}),
                          prop: :mandatory)
    embeddedPdv_components = [identification2, data_value]
    string_value = r_ComponentType(name: :"string-value", typespec: r_type(def: :"OCTET STRING"),
                       prop: :mandatory)
    characterString_components = [identification2,
                                      string_value]
    [{:EXTERNAL,
        r_type(tag: [r_tag(class: :UNIVERSAL, number: 8, type: :IMPLICIT,
                    form: 32)],
            def: r_SEQUENCE(components: eXTERNAL_components1990))},
         {:"EMBEDDED PDV",
            r_type(tag: [r_tag(class: :UNIVERSAL, number: 11,
                        type: :IMPLICIT, form: 32)],
                def: r_SEQUENCE(components: embeddedPdv_components))},
             {:"CHARACTER STRING",
                r_type(tag: [r_tag(class: :UNIVERSAL, number: 29,
                            type: :IMPLICIT, form: 32)],
                    def: r_SEQUENCE(components: characterString_components))}]
  end

  defp include_default_class(s, module) do
    _ = (for classDef <- default_class_list() do
  ...
  defp include_default_class1(s, module, {name, ts0}) do
    case (:asn1_db.dbget(module, name)) do
      :undefined ->
        ...
        c = r_classdef(checked: true, module: module, name: name,
                typespec: ts)
  ...
  defp default_class_list() do
    [{:"TYPE-IDENTIFIER",
        r_objectclass(fields: [{:fixedtypevaluefield, :id,
                      r_type(tag: [r_tag(class: :UNIVERSAL, number: 6, type: :IMPLICIT,
                                  form: 0)],
                          def: :"OBJECT IDENTIFIER"),
                      :UNIQUE, :MANDATORY},
                       {:typefield, :Type, :MANDATORY}],
            syntax: {:"WITH SYNTAX",
                       [{:typefieldreference, :Type}, :IDENTIFIED, :BY,
                                                                       {:valuefieldreference,
                                                                          :id}]})},
         {:"ABSTRACT-SYNTAX",
            r_objectclass(fields: [{:fixedtypevaluefield, :id,
                          r_type(tag: [r_tag(class: :UNIVERSAL, number: 6,
                                      type: :IMPLICIT, form: 0)],
                              def: :"OBJECT IDENTIFIER"),
                          :UNIQUE, :MANDATORY},
                           {:typefield, :Type, :MANDATORY},
                               {:fixedtypevaluefield, :property,
                                  r_type(tag: [r_tag(class: :UNIVERSAL, number: 3,
                                              type: :IMPLICIT, form: 0)],
                                      def: {:"BIT STRING", []}),
  ...
  defp new_reference_name(name) do
    case (:erlang.get(:asn1_reference)) do
      :undefined ->
        ...
        :erlang.list_to_atom(:lists.concat([:internal_, name, '_',
                                                                  1]))
  ...
  defp get_record_prefix_name(s) do
    case (:lists.keysearch(:record_name_prefix, 1,
                             r_state(s, :options))) do
      {:value, {_, prefix}} ->
        prefix
      _ ->
        ''
