defmodule :m_prepare_templates do
  ...
  def gen_asn1ct_rtt(ms) do
    ...
    :io.nl(fd)
    :io.put_chars(fd, '-module(asn1ct_rtt).\n-export([assert_defined/1,dependencies/1,code/0]).\n\n')
    forms = :lists.sort(:lists.append(for m <- ms do
                                        abstract(m)
                                      end))
    ...
    :io.nl(fd)
    funcs = (for {m,
                    {:function, _, f, a, _} = func} <- forms do
               (
  ...
  defp abstract(file) do
    {:ok,
       {m0,
          [{:abstract_code, abstract}]}} = :beam_lib.chunks(file,
                                                              [:abstract_code])
