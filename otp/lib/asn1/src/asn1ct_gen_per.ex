defmodule :m_asn1ct_gen_per do
  ...
  def gen_encode(erules, typename,
           r_ComponentType(name: cname, typespec: type)) do
  ...
  defp gen_encode_user(erules, d) when elem(d, 0) === :typedef do
    ...
    case (:asn1ct_gen.type(innerType)) do
      ...
      r_Externaltypereference(module: emod, type: etype) ->
        emit([{:asis, emod}, ':', {:asis, enc_func(etype)}, '(Val).',
                                                              :nl])
    end
  end
  ...
  defp gen_encode_prim(erules, d) do
    value = {:var,
               :erlang.atom_to_list(:asn1ct_gen.mk_var(:asn1ct_name.curr(:val)))}
    gen_encode_prim(erules, d, value)
  end

  defp gen_encode_prim(r_gen(erule: :per, aligned: aligned), r_type() = d,
            value) do
    imm = gen_encode_prim_imm(value, d, aligned)
    :asn1ct_imm.enc_cg(imm, aligned)
  end

  def gen_encode_prim_imm(val, r_type(def: type0, constraint: constraint),
           aligned) do
    case (simplify_type(type0)) do
      :k_m_string ->
        ...
        :asn1ct_imm.per_enc_k_m_string(val, type, constraint,
                                         aligned)
      :restricted_string ->
        toBinary = {:erlang, :iolist_to_binary}
        :asn1ct_imm.per_enc_restricted_string(val, toBinary,
                                                aligned)
      {:ENUMERATED, nNL} ->
      ...
      {:INTEGER, nNL} ->
        :asn1ct_imm.per_enc_integer(val, nNL, constraint,
                                      aligned)
      :REAL ->
      ...
      {:"BIT STRING", nNL} ->
        case (:asn1ct.use_legacy_types()) do
          false ->
            :asn1ct_imm.per_enc_bit_string(val, nNL, constraint,
                                             aligned)
          true ->
      ...
      :"OCTET STRING" ->
        case (:asn1ct.use_legacy_types()) do
          false ->
            :asn1ct_imm.per_enc_octet_string(val, constraint,
                                               aligned)
          true ->
  ...
  def gen_decode(erules, typename, type)
    ...
    case (:asn1ct_gen.type(innerType)) do
      {:constructed, :bif} ->
        ...
        emit([:nl, {:asis,
                      dec_func(:asn1ct_gen.list2name(typename))},
                       '(Bytes', objFun, ') ->', :nl])
  ...
  def gen_dec_external(ext, bytesVar) do
    ...
    emit([case (currMod) do
            ^mod ->
              []
            _ ->
              [{:asis, mod}, ':']
          end,
              {:asis, dec_func(type)}, '(', bytesVar, ')'])
  end

  def gen_dec_imm(r_gen(erule: :per, aligned: aligned),
           r_type(def: name, constraint: c)) do
  ...
  defp gen_dec_imm_1({:"BIT STRING", nNL}, constr0, aligned) do
    constr = :asn1ct_imm.effective_constraint(:bitstring,
                                                constr0)
    ...
    case (nNL) do
      ...
      [_ | _] ->
        d = fn v, buf ->
                 as = [v, {:asis, nNL}]
                 call = {:call, :per_common, :decode_named_bit_string,
                           as}
                 emit(['{', call, :com, buf, '}'])
            end
        {:call, d, imm}
  ...
  defp gen_dec_imm_1({:ENUMERATED, namedNumberList}, _Constr,
            aligned) do
  ...
  defp gen_dec_imm_1({:INTEGER, namedNumberList}, constraint,
            aligned) do
  ...
  defp gen_dec_imm_1(:"OCTET STRING", constraint, aligned) do
    szConstr = :asn1ct_imm.effective_constraint(:bitstring,
                                                  constraint)
    imm = :asn1ct_imm.per_dec_octet_string(szConstr,
                                             aligned)
    case (:asn1ct.use_legacy_types()) do
      false ->
  ...
  defp gen_dec_imm_1(:"RELATIVE-OID", _Constraint, aligned) do
    dec = fn v, buf ->
               emit(['{', {:call, :per_common, :decode_relative_oid,
                           [v]},
                            :com, buf, '}'])
          end
    {:call, dec, gen_dec_restricted_string(aligned)}
  ...
  defp gen_dec_k_m_string(type, constraint, aligned) do
    :asn1ct_imm.per_dec_k_m_string(type, constraint,
                                     aligned)
  end
  ...
  defp extaddgroup2sequence([{:ExtensionAdditionGroup, number0} | t],
            extNum, acc) do
    number = (case (number0) do
                :undefined ->
                  1
                _ ->
                  number0
    ...
    {extGroupComps,
       [:ExtensionAdditionGroupEnd |
            t2]} = :lists.splitwith(fn elem ->
                                         elem(elem, 0) === :ComponentType
                                    end,
                                      t)
    extaddgroup2sequence(t2, extNum + 1,
                           [r_ComponentType(name: :erlang.list_to_atom('ExtAddGroup' ++ :erlang.integer_to_list(extNum + 1)),
                                typespec: r_type(def: r_SEQUENCE(extaddgroup: number,
  ...
  defp imm_dec_open_type_1(type, aligned) do
    d = fn openType, buf ->
             ...
             emit(['begin', :nl, '{', {:curr, :tmpval}, ',_} = ', {:asis,
                                                      dec_func(type)},
                                                       '(', openType, '),', :nl, '{',
                                                                                {:curr,
                                                                                   :tmpval},
