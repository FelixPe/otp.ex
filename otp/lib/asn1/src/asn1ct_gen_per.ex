defmodule :m_asn1ct_gen_per do
  ...
  def gen_encode(erules, typename, type)
    ...
    case :asn1ct_gen.type(innerType) do
      {:constructed, :bif} ->
        ...
        emit([{:asis, func}, ~c"(Val", objFun, ~c") ->", :nl])
  ...
  defp gen_encode_user(erules, d) when elem(d, 0) === :typedef do
    ...
    case :asn1ct_gen.type(innerType) do
      ...
      r_Externaltypereference(module: ^currMod, type: etype) ->
        emit([{:asis, enc_func(etype)}, ~c"(Val).", :nl])

      r_Externaltypereference(module: emod, type: etype) ->
        emit([{:asis, emod}, ~c":", {:asis, enc_func(etype)}, ~c"(Val).", :nl])
    end
  end
  ...
  def gen_decode(erules, r_typedef() = type) do
    ...
    emit([:nl, :nl, {:asis, decFunc}, ~c"(Bytes) ->", :nl])
  ...
  defp gen_decode_user(erules, d) when elem(d, 0) === :typedef do
    ...
    case :asn1ct_gen.type(innerType) do
      {:primitive, :bif} ->
        gen_dec_prim(erules, def__, ~c"Bytes")
        emit([~c".", :nl, :nl])

      :ASN1_OPEN_TYPE ->
        gen_dec_prim(erules, r_type(def__, def: :ASN1_OPEN_TYPE), ~c"Bytes")
        emit([~c".", :nl, :nl])
      ...
      r_Externaltypereference() = etype ->
        gen_dec_external(etype, ~c"Bytes")
        emit([~c".", :nl, :nl])
    end
  end
  ...
  defp gen_dec_imm_1(:"OBJECT IDENTIFIER", _Constraint, aligned) do
    dec = fn v, buf ->
      emit([~c"{", {:call, :per_common, :decode_oid, [v]}, :com, buf, ~c"}"])
    end
  ...
  defp gen_dec_imm_1(:"RELATIVE-OID", _Constraint, aligned) do
    dec = fn v, buf ->
      emit([~c"{", {:call, :per_common, :decode_relative_oid, [v]}, :com, buf, ~c"}"])
    end
  ...
  defp gen_dec_bit_string(f, imm) do
    d = fn v, buf ->
      emit([~c"{", {:call, :per_common, f, [v]}, :com, buf, ~c"}"])
    end
  ...
  defp gen_dec_copy_bitstring(imm) do
    d = fn v, buf ->
      emit([~c"{list_to_bitstring([", v, ~c"]),", buf, ~c"}"])
    end
  ...
  defp extaddgroup2sequence([{:ExtensionAdditionGroup, number0} | t], extNum, acc) do
    ...
    extaddgroup2sequence(t2, extNum + 1, [
      r_ComponentType(
        name: :erlang.list_to_atom(~c"ExtAddGroup" ++ :erlang.integer_to_list(extNum + 1)),
        typespec:
          r_type(
            def:
              r_SEQUENCE(
