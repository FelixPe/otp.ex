defmodule :m_asn1rtt_ext do
  ...
  def transform_to_EXTERNAL1990({_, _, _, _} = val) do
    transform_to_EXTERNAL1990(:erlang.tuple_to_list(val),
                                [])
  end
  ...
  defp transform_to_EXTERNAL1990([data_val_desc, data_value], acc)
    ...
    :erlang.list_to_tuple(:lists.reverse([{:"octet-aligned", data_value},
                                              data_val_desc | acc]))
  end

  defp transform_to_EXTERNAL1990([data_val_desc, data_value], acc)
      when is_binary(data_value) do
    :erlang.list_to_tuple(:lists.reverse([{:"single-ASN1-type", data_value},
                                              data_val_desc | acc]))
  end

  defp transform_to_EXTERNAL1990([data_value], acc) when is_list(data_value) or
  ...
  def transform_to_EXTERNAL1994({:EXTERNAL, dRef, indRef, data_v_desc,
            encoding} = v) do
    identification = (case ({dRef, indRef}) do
                        {^dRef, :asn1_NOVALUE} ->
                          {:syntax, dRef}
                        {:asn1_NOVALUE, ^indRef} ->
                          {:"presentation-context-id", indRef}
