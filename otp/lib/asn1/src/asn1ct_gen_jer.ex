defmodule :m_asn1ct_gen_jer do
  ...
  def gen_encode_sequence(gen, typename, r_type() = d) do
    ...
    {sequenceTag, newCompTypes} =
      case r_gen(gen, :pack) do
        :map ->
          {:sequence_map,
           for {binName, type, optOrDefault} <- compTypes do
             {binName, binary_to_atom(binName), type, optOrDefault}
           end}
    ...
    {sequenceTag, :erlang.list_to_atom(:lists.concat([prefix, :asn1ct_gen.list2name(typename)])),
  ...
  defp gen_enc_comptypes(
    ...
    typeInfo = gen_enc_line(erules, topType, cname, type, ~c"Dummy", 3, prop, encObj)
  ...
  defp gen_enc_classtypes(erules, topType, [{tName, r_typedef(typespec: tSpec)} | rest], acc) do
    typeInfo = gen_enc_line(erules, topType, tName, tSpec, ~c"Dummy", 3, :mandatory, false)
  ...
  defp gen_encode_user(erules, r_typedef() = d, _Wrapper) do
    ...
    innerType = :asn1ct_gen.get_inner(r_type(type, :def))
    emit([:typeinfo, ~c"('", :asn1ct_gen.list2name(typename), ~c"') ->", :nl])
    ...
    typeInfo =
      case :asn1ct_gen.type(innerType) do
        ...
        {:primitive, :bif} ->
          gen_encode_prim(:jer, type, ~c"Val")
        ...
        :ASN1_OPEN_TYPE ->
          gen_encode_prim(:jer, r_type(type, def: :ASN1_OPEN_TYPE), ~c"Val")
      end

    emit([~c"  ", {:asis, typeInfo}, ~c";", :nl])
  end
  ...
  defp maybe_legacy_bit_string(bitStrType, sizeConstraint) do
    ...
    type1 =
      case bitStrType do
        ...
        {:"BIT STRING", nNL} ->
          {:erlang.list_to_atom(:lists.concat([type, ~c"_nnl"])), nNL}
      end
  ...
  defp containing_constraint(
    ...
    case :asn1ct_gen.type(innerType) do
      ...
      {:primitive, :bif} ->
        gen_encode_prim(erules, outerType, ~c"Val")
  ...
  def gen_objectset_code(erules, objSet) do
    ...
    emit([:typeinfo, ~c"(", typeName, ~c") ->", :nl])
    emit([~c"  ", {:asis, selectValMap}, ~c";", :nl])
  end
