defmodule :m_asn1ct_gen_jer do
  ...
  def gen_encode_sequence(gen, typename, r_type() = d) do
    ...
    compTypes = gen_enc_comptypes(gen, typename, compList1,
                                    1, encObj, [])
    prefix = :asn1ct_gen.get_record_name_prefix(gen)
    {sequenceTag, newCompTypes} = (case (r_gen(gen, :pack)) do
                                     :map ->
                                       {:sequence_map,
                                          for {binName, type,
                                                 optOrDefault} <- compTypes do
                                            {binName, binary_to_atom(binName),
                                               type, optOrDefault}
                                          end}
                                     _ ->
                                       {:sequence, compTypes}
    ...
    {sequenceTag,
       :erlang.list_to_atom(:lists.concat([prefix,
  ...
  def gen_encode_sof(erules, typename, innerTypename, d)
    ...
    nameSuffix = :asn1ct_gen.constructed_suffix(innerTypename,
                                                  r_type(d, :def))
    {:sof,
       gen_typeinfo(erules, [nameSuffix | typename], cont)}
  end
  ...
  defp gen_enc_comptypes(erules, topType,
            [r_ComponentType(name: cname, typespec: type, prop: prop) | rest],
            pos, encObj, acc) do
    typeInfo = gen_enc_line(erules, topType, cname, type, 'Dummy',
                              3, prop, encObj)
    gen_enc_comptypes(erules, topType, rest, pos, encObj,
                        [{:erlang.atom_to_binary(cname, :utf8), typeInfo,
                            prop} |
                             acc])
  end
  ...
  defp gen_enc_classtypes(erules, topType,
            [{tName, r_typedef(typespec: tSpec)} | rest], acc) do
    typeInfo = gen_enc_line(erules, topType, tName, tSpec,
                              'Dummy', 3, :mandatory, false)
    gen_enc_classtypes(erules, topType, rest,
                         [{tName, typeInfo} | acc])
  end
  ...
  defp gen_enc_line(erules, topType, cname, type, element, indent,
      ...
      when is_list(element) do
    gen_enc_line(erules, topType, cname, type, element,
                   indent, optOrMand, [{:curr, :encV}, ' = '], encObj)
  end

  defp gen_enc_line(erules, topType, cname, type, element, _Indent,
            _OptOrMand, _Assign, encObj)
      when is_list(element) do
    innerType = (case (type) do
                   r_type(def: def__) ->
    ...
    cR = (case (type) do
            r_type(constraint: constraint) ->
              :asn1ct_gen.get_constraint(constraint,
                                           :componentrelation)
            _ ->
              []
          end)
    typeInfo = (case ({type, cR}) do
                      ...
                      :ASN1_OPEN_TYPE ->
                        case (r_type(type, :def)) do
                          r_ObjectClassFieldType() ->
                            gen_encode_prim(:jer, r_type(def: :ASN1_OPEN_TYPE),
                                              element)
                          _ ->
                            gen_encode_prim(:jer, type, element)
                        end
                                                                                          ...
                                                                                          '(Val',
                                                                                              objFun,
                                                                                                  ') ->',
                                                                                                      :nl,
                                                                                                          '   '])
        typeInfo = gen_encode_constructed(erules, typename,
                                            innerType, type)
  ...
  defp gen_encode_user(erules, r_typedef() = d, _Wrapper) do
    ...
    innerType = :asn1ct_gen.get_inner(r_type(type, :def))
    emit([:typeinfo, '(\'', :asn1ct_gen.list2name(typename), '\') ->',
                                                             :nl])
    currentMod = :erlang.get(:currmod)
    typeInfo = (case (:asn1ct_gen.type(innerType)) do
                  {:constructed, :bif} ->
                    gen_encode_constructed(erules, typename, innerType,
                                             type)
                  {:primitive, :bif} ->
  ...
  def gen_encode_prim(erules, r_type(constraint: c) = d, _Value) do
  ...
  defp maybe_legacy_bit_string(bitStrType, sizeConstraint) do
    type = (case (:asn1ct.get_bit_string_format()) do
              :bitstring ->
                :bit_string
              :compact ->
                :compact_bit_string
  ...
  defp containing_constraint(erules,
            [{:contentsconstraint, r_type(def: def__) = outerType, []} |
                 _]) do
    innerType = :asn1ct_gen.get_inner(def__)
    case (:asn1ct_gen.type(innerType)) do
      r_Externaltypereference(module: mod, type: typeName) ->
  ...
  defp containing_constraint(erules, [_ | cs]) do
    containing_constraint(erules, cs)
  end

  defp containing_constraint(_Erules, []) do
    []
  end
  ...
  def gen_objectset_code(erules, objSet) do
    ...
    typeName = {:asis,
                  typeinfo_func(:asn1ct_gen.list2name([objSetName]))}
    selectValMap = :maps.from_list(for {_, selectVal,
                                          typeList} <- set do
                                     {selectVal,
                                        :maps.from_list(gen_enc_classtypes(erules,
                                                                             objSetName,
                                                                             for (tNameType = {_TypeName,
                                                                                                 r_typedef()}) <- typeList do
                                                                               tNameType
                                                                             end,
                                                                             []))}
                                   end)
