defmodule :m_asn1ct_gen_ber_bin_v2 do
  ...
  def gen_encode(erules, typename, type)
    ...
    objFun = (case (:lists.keysearch(:objfun, 1,
                                       r_type(type, :tablecinf))) do
                {:value, {_, _Name}} ->
                  ', ObjFun'
                false ->
                  ''
    ...
    case (:asn1ct_gen.type(innerType)) do
      {:constructed, :bif} ->
        func = {:asis,
                  enc_func(:asn1ct_gen.list2name(typename))}
        emit([:nl, :nl, :nl, '%%================================', :nl, '%%  ',
  ...
  defp gen_encode_user(erules, r_typedef() = d, wrapper) do
    ...
    case (wrapper) do
      true ->
        ...
        tag0 = (for r_tag(class: class, form: form,
                        number: number) <- oTag do
                  encode_tag_val(decode_class(class), form, number)
                end)
        tag = :lists.reverse(tag0)
        emit([funcName, '(Val) ->', :nl, '    ', funcName, '(Val, ', {:asis, tag}, ').',
                                                                  :nl, :nl])
      false ->
        :ok
    end
    ...
    case (:asn1ct_gen.type(innerType)) do
      ...
      r_Externaltypereference(module: emod, type: etype) ->
        emit(['   ', {:asis, emod}, ':', {:asis, enc_func(etype)}, '(Val, TagIn).',
                                                                 :nl])
      :ASN1_OPEN_TYPE ->
        emit(['%% OPEN TYPE', :nl])
        gen_encode_prim(:ber, r_type(type, def: :ASN1_OPEN_TYPE), 'TagIn',
                          'Val')
        emit(['.', :nl])
    end
  end
  ...
  def gen_encode_prim(_Erules, r_type() = d, doTag, value) do
    ...
    maxBitStrSize = (case (bitStringConstraint) do
                       [] ->
                         :none
                       {_, :MAX} ->
                         :none
    ...
    case (type) do
      ...
      :REAL ->
        ...
        emit(['begin', :nl, {:curr, :realval}, ' = ', {:call,
                                               :real_common, :ber_encode_real,
                                               [value]},
                                                :com, :nl, {:curr, :realsize},
                                                               ' = ', {:call,
                                                                     :erlang,
                                                                     :byte_size,
                                                                     [{:curr,
        ...
        case (:asn1ct.use_legacy_types()) do
          ...
          true ->
            call(:encode_bit_string,
                   [{:asis, bitStringConstraint}, value, {:asis, []},
                                                             doTag])
        end
  ...
  defp emit_enc_enumerated_cases([{enumName, enumVal} | t], tags, ext) do
    ...
    emit([{:asis, enumName}, ' -> ', {:call, :ber, :encode_tags,
                                   [tags, {:asis, bytes}, len]},
                                    ';', :nl])
  ...
  def gen_decode(erules, type) when elem(type, 0) === :typedef do
    ...
    tag = (for x <- innerTag do
             decode_class(r_tag(x, :class)) <<< 10 + r_tag(x, :number)
           end)
    funcName0 = (case ({:asn1ct.get_gen_state_field(:active),
                          :asn1ct.get_gen_state_field(:prefix)}) do
                   {true, pref} ->
    ...
    funcName = {:asis,
                  :erlang.list_to_atom(:lists.concat(funcName0))}
    emit([:nl, :nl, funcName, '(Tlv) ->', :nl, '   ', funcName, '(Tlv, ',
                                                       {:asis, tag}, ').', :nl,
                                                                            :nl,
                                                                                funcName,
                                                                                    '(Tlv, TagIn) ->',
  ...
  def gen_decode_selected(erules, type, funcName) do
    ...
    pattern = (case (:lists.keysearch(funcName, 1,
                                        patterns)) do
                 {:value, {_, p}} ->
                   p
                 false ->
    ...
    emit(['  case ', {:call, :ber, :decode_selective,
                [{:asis, pattern}, 'Bin']},
                 ' of', :nl, '    {ok,Bin2} when is_binary(Bin2) ->', :nl, '      {Tlv,_} = ', {:call, :ber, :ber_decode_nif, ['Bin2']},
                                        :com, :nl])
    emit('{ok,')
  ...
  defp gen_decode_selected_type(_Erules, typeDef) do
    ...
    bytesVar = 'Tlv'
    tag = (for x <- r_type(def__, :tag) do
             decode_class(r_tag(x, :class)) <<< 10 + r_tag(x, :number)
           end)
    case (:asn1ct_gen.type(innerType)) do
      :ASN1_OPEN_TYPE ->
        :asn1ct_name.new(:len)
        gen_dec_prim(r_type(def__, def: :ASN1_OPEN_TYPE), bytesVar,
                       tag)
      {:primitive, :bif} ->
        :asn1ct_name.new(:len)
        gen_dec_prim(def__, bytesVar, tag)
      {:constructed, :bif} ->
        topType = (case (r_typedef(typeDef, :name)) do
                     a when is_atom(a) ->
                       [a]
                     n ->
                       n
  ...
  def gen_decode(erules, typename, type)
      when elem(type, 0) === :type do
    innerType = :asn1ct_gen.get_inner(r_type(type, :def))
    functionName = (case (:asn1ct.get_gen_state_field(:active)) do
                      true ->
                        pattern = :asn1ct.get_gen_state_field(:namelist)
                        suffix = (case (:asn1ct.maybe_saved_sindex(typename,
                                                                     pattern)) do
                                    i when (is_integer(i) and i > 0) ->
                                      :lists.concat(['_', i])
                                    _ ->
                        ...
                        :lists.concat(['\'dec-inc-', :asn1ct_gen.list2name(typename),
                                              suffix])
                      _ ->
    ...
    case (:asn1ct_gen.type(innerType)) do
      {:constructed, :bif} ->
        objFun = (case (r_type(type, :tablecinf)) do
                    [{:objfun, _} | _R] ->
                      ', ObjFun'
                    _ ->
                      ''
      ...
      rec when elem(rec, 0) === :Externaltypereference ->
        case ({typename,
                 :asn1ct.get_gen_state_field(:namelist)}) do
          {[cname | _], [{cname, _} | _]} ->
  ...
  def gen_decode(erules, tname, r_ComponentType(name: cname, typespec: type)) do
    ...
    case ({:asn1ct.get_gen_state_field(:active),
             :asn1ct.get_tobe_refed_func(newTname)}) do
      {true, {_, nameList}} ->
  ...
  def gen_dec_prim(att, bytesVar, doTag) do
    ...
    case (newTypeName) do
      ...
      {:INTEGER, nNL} ->
        check_constraint(:decode_integer, [bytesVar, tagStr],
                           ...
                           fn val ->
                                ...
                                emit([{:curr, :val}, ' = '])
                                val.()
                                emit([:com, :nl, {:call, :ber, :number2name,
                                                    [{:curr, :val}, {:asis,
                                                                       nNL}]}])
                           end)
      {:ENUMERATED, nNL} ->
      ...
      :REAL ->
        ...
        emit(['begin', :nl, {:curr, :tmpbuf}, ' = ', {:call, :ber,
                                              :match_tags, [bytesVar, tagStr]},
                                               :com, :nl, {:call, :real_common,
                                                             :decode_real,
                                                             [{:curr,
                                                                 :tmpbuf}]},
                                                              :nl, 'end', :nl])
      ...
      :"OCTET STRING" ->
        check_constraint(:decode_octet_string,
                           [bytesVar, tagStr], constraint,
                           {:erlang, :byte_size}, :identity)
      :restricted_string ->
      ...
      :UniversalString ->
        check_constraint(:decode_universal_string,
                           [bytesVar, tagStr], constraint, {:erlang, :length},
                           :identity)
      :UTF8String ->
    ...
    case (:asn1ct.get_bit_string_format()) do
      :compact ->
        check_constraint(:decode_compact_bit_string,
                           [bytesVar, tagStr], constraint,
                           {:ber, :compact_bit_string_size}, :identity)
      :legacy ->
        check_constraint(:decode_native_bit_string,
                           [bytesVar, tagStr], constraint, {:erlang, :bit_size},
                           fn val ->
                                :asn1ct_name.new(:val)
                                emit([{:curr, :val}, ' = '])
                                val.()
                                emit([:com, :nl, {:call, :ber,
  ...
  defp check_constraint(f, args, constr, preConstr0, returnVal0) do
    preConstr = (case (preConstr0) do
                   ...
                   {mod, name} ->
                     fn v ->
                          :asn1ct_name.new(:c)
                          emit([{:curr, :c}, ' = ', {:call, mod, name, [v]}, :com,
                                                                             :nl])
                          {:curr, :c}
                     end
                 end)
    returnVal = (case (returnVal0) do
                   :identity ->
                     fn val ->
                          val.()
                     end
    ...
    case (constr) do
      ...
      _ ->
        :asn1ct_name.new(:val)
        emit(['begin', :nl, {:curr, :val}, ' = ', {:call, :ber, f, args},
                                            :com, :nl])
        ...
        case (constr) do
          {min, max} ->
            emit([{:asis, min}, ' =< ', preVal, ', ', preVal, ' =< ', {:asis,
                                                            max}])
  ...
  defp gen_dec_enumerated(bytesVar, nNL0, tagStr) do
    ...
    emit(['case ', {:call, :ber, :decode_integer,
                [bytesVar, tagStr]},
                 ' of', :nl])
    nNL = (case (nNL0) do
             {l1, l2} ->
  ...
  def gen_obj_code(erules, _Module, obj)
    ...
    encConstructed = gen_encode_objectfields(clName,
                                               get_class_fields(class), objName,
                                               fields, [])
    emit(:nl)
    gen_encode_constr_type(erules, encConstructed)
    emit(:nl)
    decConstructed = gen_decode_objectfields(clName,
                                               get_class_fields(class), objName,
                                               fields, [])
    emit(:nl)
  ...
  defp gen_encode_objectfields(className,
            [{:typefield, name, optOrMand} | rest], objName,
            objectFields, constrAcc) do
    ...
    maybeConstr = (case ({get_object_field(name,
                                             objectFields),
                            optOrMand}) do
                     {false, :OPTIONAL} ->
                       emitFuncClause.('Val')
    ...
    gen_encode_objectfields(className, rest, objName,
                              objectFields, maybeConstr ++ constrAcc)
  end

  defp gen_encode_objectfields(className,
            [{:objectfield, name, _, _, optOrMand} | rest], objName,
            objectFields, constrAcc) do
    currentMod = :erlang.get(:currmod)
    ...
    case ({get_object_field(name, objectFields),
             optOrMand}) do
      {false, :OPTIONAL} ->
        emitFuncClause.('_,_')
  ...
  defp gen_encode_field_call(_ObjName, _FieldName, r_Externaltypereference(module: m, type: t)) do
    ...
    tag = (for x <- oTag do
             encode_tag_val(decode_class(r_tag(x, :class)), r_tag(x, :form),
                              r_tag(x, :number))
           end)
    cond do
      m == currentMod ->
        ...
        []
      true ->
        emit(['   ', {:asis, m}, ':', {:asis, enc_func(t)}, '(Val, ', {:asis,
                                                            tag},
                                                             ')'])
        []
    end
  ...
  defp gen_encode_field_call(objName, fieldName, type) do
    ...
    case (r_typedef(type, :name)) do
      {:primitive, :bif} ->
        gen_encode_prim(:ber, def__,
                          {:asis, :lists.reverse(tag)}, 'Val')
        []
      {:constructed, :bif} ->
  ...
  defp gen_decode_objectfields(className,
            [{:typefield, name, optOrMand} | rest], objName,
            objectFields, constrAcc) do
    emitFuncClause = fn arg ->
    ...
    gen_decode_objectfields(className, rest, objName,
                              objectFields, maybeConstr ++ constrAcc)
  end
  ...
  defp emit_tlv_format_function1() do
    emit(['tlv_format(Bytes) when is_binary(Bytes) ->', :nl, '  {Tlv,_} = ', {:call, :ber, :ber_decode_nif, ['Bytes']},
                         :com, :nl, '  Tlv;', :nl, 'tlv_format(Bytes) ->', :nl, '  Bytes.', :nl])
  end
  ...
  defp gen_decode_field_call(_ObjName, _FieldName, bytes,
            r_Externaltypereference(module: m, type: t)) do
    currentMod = :erlang.get(:currmod)
    tDef = :asn1_db.dbget(m, t)
    def__ = r_typedef(tDef, :typespec)
    oTag = r_type(def__, :tag)
    ...
    cond do
      m == currentMod ->
        emit(['   ', {:asis, dec_func(t)}, '(', bytes, ', ', {:asis,
                                                       tag},
                                                        ')'])
        []
      true ->
        emit(['   ', {:asis, m}, ':', {:asis, dec_func(t)}, '(', bytes,
                                                             ', ', {:asis, tag},
                                                                    ')'])
        []
    end
  ...
  defp gen_decode_field_call(objName, fieldName, bytes, type) do
    ...
    case (r_typedef(type, :name)) do
      ...
      {:constructed, :bif} ->
        name = :lists.concat([objName, '_', fieldName])
        emit(['   ', {:asis, dec_func(name)}, '(', bytes, ',', {:asis,
                                                          tag},
                                                           ')'])
        [r_typedef(type, name: :erlang.list_to_atom(name))]
      {extMod, typeName} ->
        ...
        []
      typeName ->
        emit(['   ', {:asis, dec_func(typeName)}, '(', bytes, ',',
                                                            {:asis, tag}, ')'])
        []
    end
  end
  ...
  defp gen_decode_default_call(className, fieldName, bytes, type) do
    ...
    case (:asn1ct_gen.type(innerType)) do
      {:constructed, :bif} ->
        emit(['   \'dec_', className, :_, fieldName, '\'(', bytes, ',', {:asis,
                                                           tag},
                                                            ')'])
        [r_typedef(name: :erlang.list_to_atom(:lists.concat([className,
                                                         :_, fieldName])),
             typespec: type)]
      {:primitive, :bif} ->
      ...
      r_Externaltypereference(module: ^currentMod, type: etype) ->
        ...
        []
      r_Externaltypereference(module: emod, type: etype) ->
        emit(['   \'', emod, '\':\'dec_', etype, '\'(', bytes, ', ', {:asis, tag}, ')',
                                                                :nl])
        []
    end
  end
  ...
  defp is_already_generated(operation, name) do
    case (:erlang.get(:class_default_type)) do
      :undefined ->
        ...
        false
      generatedList ->
        case (:lists.member({operation, name},
                              generatedList)) do
          true ->
            true
          false ->
            :erlang.put(:class_default_type,
  ...
  def gen_objectset_code(erules, objSet) do
    ...
    r_Externaltypereference(module: classModule,
        type: className) = r_ObjectSet(def__, :class)
    classDef = :asn1_db.dbget(classModule, className)
    uniqueFName = r_ObjectSet(def__, :uniquefname)
    set = r_ObjectSet(def__, :set)
    emit([:nl, :nl, :nl, '%%================================', :nl, '%%  ', objSetName, :nl, '%%================================',
                                                         :nl])
    case (className) do
  ...
  defp gen_objset_code(erules, objSetName, uniqueFName, set, className,
            classDef) do
    classFields = get_class_fields(classDef)
    internalFuncs = gen_objset_enc(erules, objSetName,
                                     uniqueFName, set, className, classFields,
                                     1, [])
    gen_objset_dec(erules, objSetName, uniqueFName, set,
  ...
  defp gen_objset_enc(erules, objSetName, uniqueName,
            [{objName, val, fields} | t], clName, clFields, nthObj,
            acc) do
    currMod = :erlang.get(:currmod)
    {internalFunc, newNthObj} = (case (objName) do
                                   {:no_mod, :no_name} ->
                                     gen_inlined_enc_funs(fields, clFields,
                                                            objSetName, val,
                                                            nthObj)
                                   {^currMod, name} ->
                                     emit([asis_atom(['getenc_', objSetName]), '(Id) when Id =:= ',
                                                                           {:asis,
                                                                              val},
                                                                               ' ->',
                                                                                   :nl,
    ...
    gen_objset_enc(erules, objSetName, uniqueName, t,
                     clName, clFields, newNthObj, internalFunc ++ acc)
  end

  defp gen_objset_enc(_, objSetName, _UniqueName, [:EXTENSIONMARK],
            _ClName, _ClFields, _NthObj, acc) do
    emit([asis_atom(['getenc_', objSetName]), '(_) ->', :nl, indent(2), 'fun(_, Val, _RestPrimFieldName) ->',
                                                             :nl])
  ...
  defp emit_default_getenc(objSetName, uniqueName) do
    emit([asis_atom(['getenc_', objSetName]), '(ErrV) ->', :nl, indent(3), 'fun(C,V,_) ->',
                                                             :nl, 'exit({\'Type not compatible with table constraint\',{component,C},{value,V}, {unique_name_and_value,', {:asis,
                                                                        uniqueName},
                                                                         ', ErrV}}) end'])
  end

  defp gen_inlined_enc_funs(fields, [{:typefield, _, _} | _] = t,
            objSetName, val, nthObj) do
    emit([asis_atom(['getenc_', objSetName]), '(', {:asis, val}, ') ->',
                                                           :nl, indent(3), 'fun(Type, Val, _RestPrimFieldName) ->',
                                                                               :nl,
                                                                                   indent(6),
                                                                                       'case Type of',
                                                                                           :nl])
  ...
  defp gen_inlined_enc_funs(fields, [_ | rest], objSetName, val, nthObj) do
    gen_inlined_enc_funs(fields, rest, objSetName, val,
                           nthObj)
  end
  ...
  defp gen_inlined_enc_funs1(fields, [{:typefield, name, _} | rest],
            objSetName, sep0, nthObj, acc0) do
    emit(sep0)
    sep = [';', :nl]
    currMod = :erlang.get(:currmod)
    internalDefFunName = :asn1ct_gen.list2name([nthObj,
                                                    name, objSetName])
    {acc, nAdd} = (case (:lists.keyfind(name, 1, fields)) do
                     {_, r_type() = type} ->
                     ...
                     {_, r_Externaltypereference(module: m, type: t)} ->
                       ...
                       cond do
                         ...
                         true ->
                           ...
                           oTag = r_type(type, :tag)
                           tag = (for x <- oTag do
                                    encode_tag_val(decode_class(r_tag(x, :class)),
                                                     r_tag(x, :form), r_tag(x, :number))
                                  end)
  ...
  defp gen_inlined_enc_funs1(fields, [_ | rest], objSetName, sep, nthObj,
            acc) do
    gen_inlined_enc_funs1(fields, rest, objSetName, sep,
                            nthObj, acc)
  end
  ...
  defp emit_enc_open_type(i) do
    indent = indent(i)
    s = [indent, 'case Val of', :nl, indent, indent(2), '{asn1_OPENTYPE,Bin} when is_binary(Bin) ->', :nl, indent,
                                                        indent(4), '{Bin,byte_size(Bin)}' |
                                                                       case (:asn1ct.use_legacy_types()) do
                                                                         false ->
                                                                           [:nl,
                                                                                indent,
  ...
  defp emit_inner_of_fun(tDef = r_typedef(name: {extMod, name}, typespec: type),
            internalDefFunName) do
    oTag = r_type(type, :tag)
    tag = (for x <- oTag do
             encode_tag_val(decode_class(r_tag(x, :class)), r_tag(x, :form),
                              r_tag(x, :number))
    ...
    case ({extMod, name}) do
      {:primitive, :bif} ->
        ...
        {[], 0}
      {:constructed, :bif} ->
        emit([indent(12), '\'enc_', internalDefFunName, '\'(Val, ', {:asis,
                                                       tag},
                                                        ')'])
  ...
  defp emit_inner_of_fun(type, _) when elem(type, 0) === :type do
    ...
    case (r_type(type, :def)) do
      ...
      r_Externaltypereference(module: extMod, type: t) ->
        ...
        emit([indent(9), t, ' ->', :nl, indent(12), extMod, ':\'enc_', t, '\'(Val, ',
                                                                  {:asis, tag},
                                                                      ')'])
    end
    {[], 0}
  ...
  defp gen_objset_dec(erules, objSName, uniqueName,
            [{objName, val, fields} | t], clName, clFields,
            nthObj) do
    currMod = :erlang.get(:currmod)
    newNthObj = (case (objName) do
                   {:no_mod, :no_name} ->
                   ...
                   {^currMod, name} ->
                     ...
                     nthObj
                   {moduleName, name} ->
                     emit([asis_atom(['getdec_', objSName]), '(Id) when Id =:= ', {:asis, val}, ' ->',
                                                                          :nl])
                     emit_ext_fun(:dec, moduleName, name)
                     emit([';', :nl])
                     nthObj
                   _ ->
                     emit([asis_atom(['getdec_', objSName]), '(', {:asis, val}, ') ->', :nl,
                                                                             '    fun \'dec_',
                                                                                 objName,
                                                                                     '\'/3;',
                                                                                         :nl])
  ...
  defp gen_objset_dec(_, objSetName, _UniqueName, [:EXTENSIONMARK],
            _ClName, _ClFields, _NthObj) do
    emit([asis_atom(['getdec_', objSetName]), '(_) ->', :nl, indent(2), 'fun(_,Bytes, _RestPrimFieldName) ->',
                                                             :nl])
  ...
  defp gen_inlined_dec_funs(fields, [{:typefield, _, _} | _] = clFields,
            objSetName, val, nthObj) do
    emit(['\'getdec_', objSetName, '\'(', {:asis, val}, ') ->', :nl])
    emit([indent(3), 'fun(Type, Bytes, _RestPrimFieldName) ->', :nl, indent(6), 'case Type of', :nl])
    gen_inlined_dec_funs1(fields, clFields, objSetName, '',
                            nthObj)
  end

  defp gen_inlined_dec_funs(fields, [_ | clFields], objSetName, val,
            nthObj) do
    gen_inlined_dec_funs(fields, clFields, objSetName, val,
                           nthObj)
  end
  ...
  defp gen_inlined_dec_funs1(fields, [{:typefield, name, prop} | rest],
            objSetName, sep0, nthObj) do
    emit(sep0)
    sep = [';', :nl]
    decProp = (case (prop) do
                 :OPTIONAL ->
                   :opt_or_default
                 {:DEFAULT, _} ->
                   :opt_or_default
    ...
    n = (case (:lists.keyfind(name, 1, fields)) do
           {_, r_type() = type} ->
    ...
    gen_inlined_dec_funs1(fields, rest, objSetName, sep,
                            nthObj + n)
  end
  ...
  defp emit_dec_open_type(i) do
    indent = indent(i)
    s = (case (:asn1ct.use_legacy_types()) do
           false ->
             [indent, 'case Bytes of', :nl, indent, indent(2), 'Bin when is_binary(Bin) -> ', :nl, indent,
                                                             indent(4), '{asn1_OPENTYPE,Bin};', :nl,
                                                                               indent,
                                                                                   indent(2),
                                                                                       '_ ->',
                                                                                           :nl,
  ...
  defp emit_inner_of_decfun(r_typedef(name: {extName, name}, typespec: type), _Prop,
            internalDefFunName) do
    oTag = r_type(type, :tag)
    tag = (for x <- oTag do
             decode_class(r_tag(x, :class)) <<< 10 + r_tag(x, :number)
  ...
  defp emit_inner_of_decfun(r_type() = type, _Prop, _) do
    ...
    case (whatKind) do
      ...
      r_Externaltypereference(module: extMod, type: t) ->
        emit([indent(9), t, ' ->', :nl, indent(12), extMod, ':\'dec_', t, '\'(Bytes, ',
                                                                  {:asis, tag},
                                                                      ')'])
    end
    0
  ...
  defp mk_object_val(val, ack, len) do
    mk_object_val(val >>> 7, [val &&& 127 ||| 128 | ack],
                    len + 1)
  end
