defmodule :m_asn1rtt_per_common do
  ...
  def decode_fragmented(segSz0, buf0, unit) do
    ...
    <<res :: size(segSz) - bitstring,
        buf :: bitstring>> = buf0
    decode_fragmented_1(buf, unit, res)
  end

  defp decode_fragmented_1(<<0 :: size(1), n :: size(7),
              buf0 :: bitstring>>,
            unit, res) do
    sz = n * unit
  ...
  defp decode_fragmented_1(<<1 :: size(1), 0 :: size(1), n :: size(14),
              buf0 :: bitstring>>,
            unit, res) do
    sz = n * unit
    <<s :: size(sz) - bitstring, buf :: bitstring>> = buf0
  ...
  defp decode_fragmented_1(<<1 :: size(1), 1 :: size(1), segSz0 :: size(6),
              buf0 :: bitstring>>,
            unit, res0) do
    segSz = segSz0 * unit * 16384
    <<frag :: size(segSz) - bitstring,
        buf :: bitstring>> = buf0
  ...
  def encode_relative_oid(val) when is_list(val) do
    :erlang.list_to_binary(for x <- val do
                             e_object_element(x)
                           end)
  end
  ...
  defp decode_named_bit_string_1(pos, [1 | bt], names, acc) do
    case (:lists.keyfind(pos, 2, names)) do
      {name, _} ->
        decode_named_bit_string_1(pos + 1, bt, names,
                                    [name | acc])
      false ->
        decode_named_bit_string_1(pos + 1, bt, names,
                                    [{:bit, pos} | acc])
  ...
  defp decode_big_chars_1([h | t]) do
    [:erlang.list_to_tuple(:erlang.binary_to_list(<<h
                                                    ::
                                                    size(32)>>)) |
  ...
  defp e_object_identifier([e1, e2 | tail]) when (e1 >= 0 and e1 < 2 and
                                   e2 < 40) or
                                  e1 === 2 do
    head = 40 * e1 + e2
  ...
  defp ntz(byte) do
    t = {8, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4,
           0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 5, 0, 1, 0,
           2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1,
           0, 3, 0, 1, 0, 2, 0, 1, 0, 6, 0, 1, 0, 2, 0, 1, 0, 3, 0,
           1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2,
           0, 1, 0, 5, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0,
           4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 7, 0, 1,
           0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0,
           1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 5, 0, 1, 0, 2, 0, 1, 0, 3,
           0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
           2, 0, 1, 0, 6, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1,
           0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 5, 0,
  ...
  defp extension_bitmap(val, pos, limit, acc) do
    bit = (case (:erlang.element(pos, val)) do
             :asn1_NOVALUE ->
               0
             :asn1_DEFAULT ->
               0
