defmodule :m_asn1ct_gen_check do
  ...
  def emit(gen, type, default, value) do
    ...
    emit(~c" case ")
    :asn1ct_func.call_gen(~c"is_default_", key, doGen, [value])
    emit([~c" of", :nl, ~c"true -> {[],0};", :nl, ~c"false ->", :nl])
  end
  ...
  defp do_gen(gen, {:CHOICE, cs}, default) do
    ...
    case do_gen(gen, r_type(type, :def), value) do
      ...
      {:exception, func0, args} ->
        ...
        func = :asn1ct_func.call_gen(~c"is_default_choice", key, doGen)
  ...
  defp do_sof(gen, type, default0) do
    ...
    case are_all_literals(cs) do
      ...
      false ->
        ...
        func = :asn1ct_func.call_gen(~c"is_default_sof", key, doGen)
        {:exception, :erlang.atom_to_list(func), []}
    end
  end
  ...
  defp gen_cs_1([h | t], i, acc) do
    var = ~c"E" ++ :erlang.integer_to_list(i)
  ...
  defp gen_cs_2([{var, {:exception, func, args}} | t], sep) do
    [sep, func, ~c"(", var, arg2str(args), ~c")" | gen_cs_2(t, ~c",\n")]
  end
  ...
  defp gen_map_cs_1([{name, {:literal, lit}} | t], i, sep, acc) do
    var = ~c"E" ++ :erlang.integer_to_list(i)
    g = var ++ ~c" =:= " ++ term2str(lit)
    [sep, term2str(name), ~c":=", var | gen_map_cs_1(t, i + 1, ~c",\n", [{:guard, g} | acc])]
  end
  ...
  defp gen_sof_1([{:exception, func, args} | cs], i) do
    ...
    h = ~c"H" ++ numStr
    t = ~c"T" ++ numStr
    prev = ~c"T" ++ :erlang.integer_to_list(i - 1)
