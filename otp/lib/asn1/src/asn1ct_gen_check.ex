defmodule :m_asn1ct_gen_check do
  ...
  defp gen(r_gen(pack: pack) = gen, name, r_type(def: t),
            default) do
  ...
  defp do_gen(_Gen, {:"BIT STRING", []}, default) do
    ...
    case (:asn1ct.use_legacy_types()) do
      false ->
        {:literal, default}
      true ->
        {:exception, need(:check_legacy_bitstring, 2),
           [default]}
    end
  end
  ...
  defp do_gen(_Gen, :"OBJECT IDENTIFIER", default0) do
    ...
    {:exception, need(:check_objectidentifier, 2),
       [default]}
  end

  defp do_gen(gen, {:CHOICE, cs}, default) do
    {tag, value} = default
    [type] = (for r_ComponentType(name: t, typespec: type) <- cs,
                    t === tag do
                type
              end)
  ...
  defp do_gen(_Gen, type, default) do
    case (:asn1ct_gen.unify_if_string(type)) do
      :restrictedstring ->
        {:exception, need(:check_restrictedstring, 2),
           [default]}
      _ ->
        {:literal, default}
    end
  ...
  defp do_named_bitstring(nBL, default0) when is_list(default0) do
    ...
    func = (case (:asn1ct.use_legacy_types()) do
              false ->
  ...
  defp do_named_bitstring(_, default) when is_bitstring(default) do
    ...
    {:exception, need(func, 3),
       [default, bit_size(default)]}
  end
  ...
  defp gen_sof(name, cs) do
    [:erlang.atom_to_list(name), '(Value) ->\n', 'case length(Value) of\n',
                                        :erlang.integer_to_list(length(cs)), ' -> ok;\n_ -> throw(false)\nend,\nT0 = lists:sort(Value)' |
                                                                                 gen_sof_1(cs,
                                                                                             1)]
  end
  ...
  defp gen_sof_1([{:exception, func, args} | cs], i) do
    ...
    [',\n', '[', h, case (cs) do
                ...
                [_ | _] ->
                  ...
                  '] = ', prev, ',\n', func, '(', h, arg2str(args), ')' | gen_sof_1(cs,
                                                                         i + 1)]
  end
  ...
  defp map_components(gen, [{name, r_type(def: def__)} | ts], value) do
    case (:maps.find(name, value)) do
      {:ok, v} ->
        [{name, do_gen(gen, def__, v)} | map_components(gen, ts,
                                                          value)]
      :error ->
