defmodule :m_xmerl_xsd do
  ...
  Record.defrecord(:r_xsd_state, :xsd_state,
    ...
    namespace_nodes: [{~c"xml", :"http://www.w3.org/XML/1998/namespace"}],
    global_namespace_nodes: [],
    checked_namespace_nodes: [{~c"xml", [], :"http://www.w3.org/XML/1998/namespace"}],
    table: :undefined,
    tab2file: false,
    redefine: false,
  ...
  def state2file(s, fileName) when elem(s, 0) === :xsd_state do
    ...
    case (try do
            :ets.tab2file(r_xsd_state(s, :table), :lists.append(fileName, ~c".xss"))
          catch
  ...
  defp validate3(schema, xml, s = r_xsd_state(errors: [])) do
    ...
    save_to_file(s2, :filename.rootname(schema) ++ ~c".tab2")
  ...
  defp initiate_state2(s, [h | t]) do
    error_msg(~c"~w: invalid option: ~p~n", [:xmerl_xsd, h])
    initiate_state2(s, t)
  end
  ...
  defp traverse_content(e = r_xmlElement(name: name), s) do
    case local_name(name) do
      :schema ->
        ...
        thisNS = {~c"#this#", r_xsd_state(s, :schema_name), r_xsd_state(s, :targetNamespace)}
  ...
  defp check_cm2(kind, r_chain(content: s4SCM, occurrence: occ), contentModel, s) do
  ...
  defp check_cm2(kind, r_alternative(content: s4SCM, occurrence: occ), contentModel, s) do
  ...
  defp attribute_use(use, s)
       when use == ~c"optional" or use == ~c"prohibited" or use == ~c"required" do
  ...
  defp attribute_form(form, s) when form == ~c"qualified" or form == ~c"unqualified" do
      ...
      {:fixed, _} ->
        err =
          {error_path(el, :schema), :xmerl_xsd,
           {~c"only one of final/default attributes allowed", r_xmlElement(el, :name)}}
  ...
  defp boolean_to_atom(b) when b == ~c"1" or b == ~c"true" do
    true
  end

  defp boolean_to_atom(b) when b == ~c"0" or b == ~c"false" do
    false
  end
  ...
  defp mk_int_or_atom(v = ~c"unbounded") do
    :erlang.list_to_atom(v)
  end
  ...
  defp mixed(e = r_xmlElement(content: c), s) do
    case {get_attribute_value(:mixed, e, :undefined),
      ...
      {m, _} when m == ~c"1" or m == ~c"true" ->
        {true, s}

      {m, _} when m == ~c"0" or m == ~c"false" ->
        {false, s}
  ...
  defp reserved_attribute(r_xmlAttribute(name: name), r_xmlNamespace(nodes: nSNodes)) do
    ...
    case :string.tokens(nameStr, ~c":") do
      [~c"xmlns" | _] ->
        true

      [prefix, instAtt]
      when instAtt == ~c"type" or instAtt == ~c"nil" or
             instAtt == ~c"schemaLocation" or instAtt == ~c"noNamespaceSchemaLocation" ->
  ...
  defp get_instance_type(r_xmlNamespace(nodes: nodes), atts) do
    case keyNsearch(:"http://www.w3.org/2001/XMLSchema-instance", 2, nodes, []) do
      {prefix, _} ->
        typeAtt = :erlang.list_to_atom(prefix ++ ~c":type")
  ...
  defp wildcard_namespace(e, s) do
    attVal = get_attribute_value(:namespace, e, ~c"##any")
  ...
  defp save_to_file(s = r_xsd_state(tab2file: tF)) do
    case tF do
      true ->
        {:ok, iO} =
          :file.open(
            :filename.rootname(r_xsd_state(s, :schema_name)) ++ ~c".tab",
            [:write]
          )
  ...
  def format_error({:unexpected_rest, uR}) do
    :io_lib.format(
      ~c"XML: The following content of an element didn't validate by the provided schema, ~n~p.",
      [uR]
    )
  end
  ...
  def format_error({:no_schemas_provided}) do
    ~c"Schema: Validator found no schema. A schema must be provided for validation."
  end

  def format_error({:internal_error, reason}) do
    :io_lib.format(~c"An error occurred that was unforeseen, due to ~p.", [reason])
  end

  def format_error({:internal_error, reason, info}) do
    :io_lib.format(~c"An error occurred that was unforeseen, due to ~p: ~p.", [reason, info])
  end

  def format_error({:internal_error, function, info1, info2}) do
    :io_lib.format(~c"An internal error occurred in function ~p with args: ~p,~p.", [
      function,
      info1,
      info2
    ])
  ...
  def format_error({:illegal_content, reason, kind}) do
    :io_lib.format(
      ~c"Schema: The schema violates the content model allowed for schemas.~nReason: ~p,~nkind of schema element: ~p.",
      [reason, kind]
    )
  end
  ...
  def format_error({:no_match, kind}) do
    :io_lib.format(
      ~c"Schema: The schema violates the content model allowed for schemas.~nKind of schema element: ~p.",
      [kind]
    )
  end

  def format_error({:bad_match, s4SC, cM}) do
    :io_lib.format(~c"Schema: The schema missed mandatory elements ~p in ~p.", [s4SC, cM])
  end

  def format_error({:unmatched_mandatory_object, sequenceEl1, sequenceEl2}) do
    :io_lib.format(
      ~c"Schema: The schema should have had an ~p object after the ~p, but it was missing.",
      [sequenceEl2, sequenceEl1]
    )
  end

  def format_error({:parsing_external_schema_failed, file, reason}) do
    :io_lib.format(~c"Schema: Parsing the referenced external schema ~p, failed due to ~p.", [
      file,
      reason
    ])
  end
  ...
  def format_error({:fetch_fun_failed, other}) do
    :io_lib.format(~c"Schema: Fetching this kind of external schema is not supported ~p.", [other])
  end

  def format_error({:element_not_in_schema, [eIIName, _ElQName, _CM]}) do
    :io_lib.format(~c"XML: The XML element ~p are not present in the schema.", [eIIName])
  end

  def format_error({:missing_mandatory_element, cMEl}) do
    :io_lib.format(~c"XML: The XML file missed mandatory element(s) ~p defined in schema.", [cMEl])
  end

  def format_error({:empty_content_not_allowed, c}) do
    :io_lib.format(~c"XML: The XML file missed mandatory element(s): ~p defined in schema.", [c])
  end

  def format_error({:element_not_suitable_with_schema, elName, _S}) do
    :io_lib.format(
      ~c"XML: The XML element: ~p violates the schema, probably to many of same element.",
      [elName]
    )
  end
  ...
  def format_error({:element_not_suitable_with_schema, elName, cMName, _CMEl, _S}) do
    :io_lib.format(
      ~c"XML: The XML element: ~p violates the schema. Schema expected element ~p.",
      [elName, cMName]
    )
  end

  def format_error({:no_element_expected_in_group, xML}) do
    :io_lib.format(~c"XML: The XML element(s) ~p violates the schema. No element was expected.", [
      xML
    ])
  end
  ...
  def format_error({:element_bad_match, e, any, _Env}) do
    :io_lib.format(
      ~c"XML: XML element ~p didn't match into the namespace of schema type any ~p.",
      [e, any]
    )
  end

  def format_error({:match_failure, _XML, _CM, _S}) do
    ~c"XML: A combination of XML element(s) and schema definitions that is not known has occurred. The implementation doesn't support this structure."
  end

  def format_error({:cannot_contain_text, _XMLTxt, cMEl}) do
    :io_lib.format(~c"XML: The schema structure: ~p doesn't allow text", [cMEl])
  end

  def format_error({:missing_mandatory_elements, mandatoryEls}) do
    :io_lib.format(~c"XML: A schema sequence has mandatory elements ~p, that were unmatched.", [
      mandatoryEls
    ])
  end
  ...
  def format_error({:choice_missmatch, t, els}) do
    :io_lib.format(
      ~c"XML: A schema choice structure with the alternatives: ~p doesn't allow the text: ~p.",
      [els, t]
    )
  end
  ...
  def format_error({:no_element_matching_choice, xML}) do
    :io_lib.format(
      ~c"XML: The choice at location: ~p had no alternative that matched the XML structure(s): ~p.",
  ...
  def format_error({:all_missmatch, t, cM}) do
    :io_lib.format(
      ~c"XML: The schema expected one of: ~p, but the XML content was text: ~p at the location: ~p.",
  ...
  def format_error({:element_not_in_all, elName, e, _CM}) do
    :io_lib.format(
      ~c"XML: The element ~p at location ~p in the XML file was not allowed according to the schema.",
      [elName, error_path(e, :undefined)]
    )
  end

  def format_error({:missing_mandatory_elements_in_all, mandatoryEls}) do
    :io_lib.format(~c"XML: The schema elements ~p were missed in the XML file.", [mandatoryEls])
  end

  def format_error({:failed_validating, e, any}) do
    :io_lib.format(
      ~c"XML: The element ~p at location ~p failed validation. It should have been matched by an any schema element ~p",
  ...
  def format_error({:schemaLocation_list_failure, paths}) do
    :io_lib.format(
      ~c"XML: schemaLocation values consists of one or more pairs of URI references, separated by white space. The first is a namespace name the second a reference to a schema: ~p.",
      [paths]
    )
  end
  ...
  def format_error({:element_content_not_nil, xMLEl}) do
    :io_lib.format(
      ~c"XML: The element ~p at position ~p has content of text/elements despite the nillable attribute was true.",
  ...
  def format_error({:attribute_in_simpleType, el, att}) do
    :io_lib.format(
      ~c"XML: The element ~p at location ~p must not have attributes like: ~p since it according to the schema has simpleType type.",
  ...
  def format_error({:required_attribute_missed, el, name}) do
    :io_lib.format(
      ~c"XML: The schema required an attribute ~p in element at location ~p that was missing.",
  ...
  def format_error({:default_and_fixed_attributes_mutual_exclusive, name, default, fix}) do
    :io_lib.format(
      ~c"Schema: It is an error in the schema to assign values for both default and fix for an attribute. Attribute: ~p, default: ~p, fix: ~p.",
      [name, default, fix]
    )
  end

  def format_error({:schema_error, :unexpected_object, _SA, _Err}) do
    ~c"Schema: An unforeseen error case occurred, maybe due to an unimplemented feature."
  end

  def format_error({:attribute_not_defined_in_schema, name}) do
    :io_lib.format(~c"XML: The attribute ~p is not defined in the provided schema.", [name])
  end

  def format_error({:disallowed_namespace, namespace, nS, name}) do
    :io_lib.format(
      ~c"XML: The attribute ~p is not valid because the namespace ~p is forbidden by ~p",
      [name, nS, namespace]
    )
  end
  ...
  def format_error({:cirkular_attributeGroup_reference, name}) do
    :io_lib.format(
      ~c"Schema: Cirkular references to attribute groups are forbidden. One was detected including ~p.",
      [name]
    )
  end
  ...
  def format_error({:could_not_resolve_type, sT}) do
    :io_lib.format(
      ~c"Schema: The simpleType ~p could not be found among the types defined by the provided schema.",
      [sT]
    )
  end

  def format_error({:could_not_check_value_for_type, type}) do
    :io_lib.format(~c"XML: Checking value for type ~p is not implemented.", [type])
  end
  ...
  def format_error({:abstract_element_instance, elName}) do
    :io_lib.format(
      ~c"XML: Elements defined as abstract in the schema must not be instantiated in XML: ~p.",
      [elName]
    )
  end
  ...
  def format_error({:qualified_name_required, localName}) do
    :io_lib.format(
      ~c"XML: Element name ~p in XML instance is not qualified, though the schema requires that.",
      [localName]
    )
  end
  ...
  def format_error({:unqualified_name_required, qualifiedName}) do
    :io_lib.format(
      ~c"XML: Element name ~p in XML instance must be unqualified, according to schema.",
      [qualifiedName]
    )
  end
  ...
  def format_error({:illegal_key_sequence_value, err}) do
    :io_lib.format(
      ~c"XML: The 'key-sequence', (se XML-spec 3.11.4), must be a node with at most one member: ~p",
      [err]
    )
  end

  def format_error({:qualified_node_set_not_correct_for_key, _Err}) do
    ~c"Schema: The 'target node set' and 'qualified node set' (se XML-spec 3.11.4.2.1) must be equal."
  end

  def format_error({:key_value_not_unique, kS}) do
    :io_lib.format(~c"Schema: Key values must be unique within the schema. This is not ~p,", [kS])
  end

  def format_error({:keyref_missed_matching_key, refer}) do
    :io_lib.format(~c"Schema: This keyref had no matching key ~p.", [refer])
  end

  def format_error({:keyref_unexpected_object, _Other}) do
    ~c"Schema: An unforeseen error case occurred, unknown failure cause."
  end

  def format_error({:cardinality_of_fields_not_equal, kR, k}) do
    :io_lib.format(
      ~c"Schema: keyref and the corresponding key must have same cardinality of their fields. Mismatch in this case keyref: ~p, key: ~p.",
      [kR, k]
    )
  end

  def format_error({:could_not_load_keyref, name}) do
    :io_lib.format(~c"Schema: The schema didn't define a keyref with the name ~p.", [name])
  end

  def format_error({:reference_undeclared, kind, ref}) do
    :io_lib.format(~c"Schema: The schema didn't define an ~p with the name ~p.", [kind, ref])
  end

  def format_error({:cyclic_substitutionGroup, sGs}) do
    :io_lib.format(
      ~c"Schema: cyclic substitutionGroup was detected, substitutionGroup structure is ~p.",
      [sGs]
    )
  end
  ...
  def format_error({:substitutionGroup_error, head, sG}) do
    :io_lib.format(
      ~c"Schema: Either of substitutionGroup members ~p or ~p is not defined in the provided schema.",
      [head, sG]
    )
  end

  def format_error({:cyclic_definition, cA}) do
    :io_lib.format(~c"Schema: A forbidden circular definition was detected ~p.", [cA])
  end

  def format_error({:type_of_element_not_derived, memT, headT}) do
    :io_lib.format(
      ~c"Schema: Type in substitutionGroup members should be simpleType or complexType. In this case ~p and ~p were found.",
      [memT, headT]
    )
  end

  def format_error({:derivation_blocked, blockTag, derivation}) do
    :io_lib.format(~c"Derivation by ~p is blocked by the blocking tag ~p.", [derivation, blockTag])
  end

  def format_error({:names_not_equal, qName1, qName2}) do
    :io_lib.format(~c"The type ~p seems to be derived from another type than the base type ~p", [
      qName2,
      qName1
    ])
  end
  ...
  def format_error({:illegal_content_in_extension, ext}) do
    :io_lib.format(
      ~c"The extension content ~p didn't match the content model of the provided schema.",
      [ext]
    )
  end
  ...
  def format_error({seqCho, :expected, other, :found})
      when seqCho == :sequence or seqCho == :choice do
    :io_lib.format(
      ~c"Schema: The restriction content ~p didn't match the content model of the provided schema, ~p was expected.",
      [seqCho, other]
    )
  end

  def format_error({:does_not_support, f, :in_restriction}) do
    :io_lib.format(~c"Schema: The structure ~p is not supported in the implementation.", [f])
  end

  def format_error({:illegal_content_simple_type, cM, typeName}) do
    :io_lib.format(~c"Schema: ~p content is not allowed in a simpleType, as in ~p.", [
      cM,
      typeName
    ])
  end
  ...
  def format_error({:illegal_in_restriction_of_simpleType, x}) do
    :io_lib.format(~c"Schema: The ~p content is illegal in a simpleType.", [x])
  end

  def format_error({:element, name, :not_present_in_restriction}) do
    :io_lib.format(
      ~c"Schema: In a restriction all element names of the restriction must be one of the elements of the base type. ~p is not.",
      [name]
    )
  end
  ...
  def format_error({:invalid_derivation, eA, baseAtts}) do
    :io_lib.format(
      ~c"Schema: An anyAttribute ~p in a restricted derived type must be present among the base type attributes ~p.",
      [eA, baseAtts]
    )
  end
  ...
  def format_error({:wildcard_namespace, nS, :not_subset_of_base_namespace, baseNS}) do
    :io_lib.format(
      ~c"Schema: See XML spec. section 3.10.6. This wildcard namespace ~p is not allowed by the base namespace restrictions ~p.",
      [nS, baseNS]
    )
  end
  ...
  def format_error({:wildcard_namespace_union_not_expressible, nS1, nS2}) do
    :io_lib.format(
      ~c"Schema: See XML spec. section 3.10.6. The union of namespaces ~p and ~p is not expressible.",
      [nS1, nS2]
    )
  end
  ...
  def format_error({:wildcard_namespace_intersection_not_expressible, nS1, nS2}) do
    :io_lib.format(
      ~c"Schema: See XML spec. section 3.10.6. The intersection of namespaces ~p and ~p is not expressible.",
      [nS1, nS2]
    )
  end
  ...
  def format_error({:circular_reference_of_type, tName}) do
    :io_lib.format(
      ~c"Schema: An illicit circular reference involving simple/complex type ~p has been detected.",
      [tName]
    )
  end
  ...
  def format_error({:type_not_uniquely_defined_in_schema, name}) do
    :io_lib.format(
      ~c"Schema: See XML spec. section 3.4.1. Type names whether simple or complex must be unique within the schema. ~p is not.",
      [name]
    )
  end
  ...
  def format_error({:not_uniquely_defined_in_schema, key}) do
    :io_lib.format(
      ~c"Schema: All schema objects of the same kind identified by name must be unique within the schema. ~p is not.",
      [key]
    )
  end

  def format_error({:illegal_ID_value, iD}) do
    :io_lib.format(~c"The ID value ~p is not allowed as an ID value.", [iD])
  end

  def format_error({:incomplete_file, _FileName, _Other}) do
    ~c"Schema: The file containing a schema state must be produced by xmerl_xsd:state2file/[1,2]."
  end

  def format_error({:unexpected_content_in_any, a}) do
    :io_lib.format(
      ~c"Schema: The any type is considered to have no content besides annotation. ~p was found.",
      [a]
    )
  end
  ...
  def format_error({:erroneous_content_in_identity_constraint, iDC, err}) do
    :io_lib.format(
      ~c"Schema: An ~p identity constraint must have one selector and one or more field in content. This case ~p",
      [iDC, err]
    )
  end

  def format_error({:missing_xpath_attribute, iDCContent}) do
    :io_lib.format(~c"Schema: A ~p in a identity constraint must have a xpath attribute.", [
      iDCContent
    ])
  end
  ...
  def format_error({:content_in_anyAttribute, err}) do
    :io_lib.format(
      ~c"Schema: ~p is not allowed in anyAttribute. Content cannot be anything else than annotation.",
      [err]
    )
  end
  ...
  def format_error({:content_in_simpleContent, err}) do
    :io_lib.format(
      ~c"Schema: Content of simpleContent can only be an optional annotation and one of restriction or extension. In this case ~p.",
      [err]
    )
  end
  ...
  def format_error({:complexContent_content_failure, err}) do
    :io_lib.format(
      ~c"Schema: Besides an optional annotation complexContent should have one of restriction or extension. In this case ~p.",
      [err]
    )
  end
  ...
  def format_error({:union_member_type_not_simpleType, illegalType}) do
    :io_lib.format(
      ~c"Schema: ~p is not allowed in a union. Content must be any nymber of simpleType.",
      [illegalType]
    )
  end

  def format_error({:missing_base_type, :restriction, _Other}) do
    ~c"Schema: A restriction must have a base type, either assigned by the 'base' attribute or as a simpleType defined in content."
  end

  def format_error({:content_failure_expected_restriction_or_extension, kind, _}) do
    :io_lib.format(~c"Schema: A ~p had no restriction or extension in content.", [kind])
  end

  def format_error({:content_failure_only_one_restriction_or_extension_allowed, kind, _}) do
    :io_lib.format(~c"Schema: A ~p can only have one of restriction or extension in content.", [
      kind
    ])
  end

  def format_error({:mandatory_component_missing, s4SCMRest, kind}) do
    :io_lib.format(~c"Schema: After matching a ~p the schema should have had content ~p.", [
      kind,
      s4SCMRest
    ])
  end
