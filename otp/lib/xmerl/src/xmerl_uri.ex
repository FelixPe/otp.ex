defmodule :m_xmerl_uri do
  ...
  defp parse_http(~c"//" ++ c0, scheme) do
  ...
  defp ftp_userinfo([], acc) do
    {:lists.reverse(acc), {~c"", ~c""}}
  end

  defp ftp_userinfo(c0 = [?/ | _], acc) do
    {:lists.reverse(acc) ++ c0, {~c"", ~c""}}
  end

  defp ftp_userinfo([?@ | c0], acc) do
    {c0, ftp_userinfo_1(:lists.reverse(acc), 0, ~c"", ~c"")}
  end
  ...
  defp sip_userinfo(userinfo) do
    case :string.tokens(userinfo, ~c":") do
      [user, pass] ->
        {user, pass}
  ...
  defp scan_parameters2([~c"transport" ++ val | rest], out, foo) do
    scan_parameters2(rest, [{:transport, val} | out], foo)
  end

  defp scan_parameters2([~c"user" ++ val | rest], out, foo) do
    scan_parameters2(rest, [{:user, val} | out], foo)
  end

  defp scan_parameters2([~c"method" ++ val | rest], out, foo) do
    scan_parameters2(rest, [{:method, val} | out], foo)
  end

  defp scan_parameters2([~c"ttl" ++ val | rest], out, foo) do
    scan_parameters2(rest, [{:ttl, val} | out], foo)
  end

  defp scan_parameters2([~c"maddr" ++ val | rest], out, foo) do
    scan_parameters2(rest, [{:maddr, val} | out], foo)
  end

  defp scan_parameters2([~c"lr" | rest], out, foo) do
    scan_parameters2(rest, [{:lr, ~c""} | out], foo)
  end
  ...
  defp scan_host2([?. | c0], acc, curF, host, hostF)
       when curF !== 0 do
    scan_host2(c0, [], 0, [~c".", acc | host], [curF | hostF])
  end
  ...
  defp scan_abspath(~c"/" ++ c0) do
    scan_pathsegments(c0, [~c"/"])
  end
  ...
  defp scan_pathsegments(c0, acc) do
    case scan_segment(c0, []) do
      {~c"/" ++ c1, segment} ->
        scan_pathsegments(c1, [~c"/", segment | acc])
  ...
  defp scan_segment(~c";" ++ c0, acc) do
    {c1, paramAcc} = scan_pchars(c0, ~c";" ++ acc)
  ...
  defp scan_segment(c0, acc) do
    case scan_pchars(c0, acc) do
      {~c";" ++ c1, segment} ->
        {c2, paramAcc} = scan_pchars(c1, ~c";" ++ segment)
