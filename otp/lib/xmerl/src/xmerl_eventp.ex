defmodule :m_xmerl_eventp do
      ...
      {:ok, fd} ->
        ...
        opts = scanner_options(options,
                                 [{:continuation_fun, &cont/3, contS},
                                      {:acc_fun, accF}, {:fetch_fun, &fetch/2},
                                                            {:rules,
                                                               &rules_read/3,
                                                               &rules_write/4,
                                                               ''},
  ...
  def stream_sax(fname, callBack, userState, options) do
    ...
    case (:file.open(fname, [:read, :raw, :binary])) do
      {:ok, fd} ->
        ...
        contS = [{b0, fname, fd}]
        opts = scanner_options(options,
                                 [{:acc_fun, accF}, {:close_fun, &close/1},
                                                        {:continuation_fun,
                                                           &cont/3, contS},
                                                            {:fetch_fun,
                                                               &fetch/2},
  ...
  defp cont2(f, exception, sofar, fd, fname, t, s) do
    case ((try do
      ...
      {:ok, bin} ->
        find_good_split(:erlang.list_to_binary([sofar, bin]), f,
                          exception, fd, fname, t, s)
      :eof ->
        :ok = :file.close(fd)
  ...
  defp find_good_split(bin, f, exception, fd, fname, t, s)
      when is_binary(bin) do
    find_good_split(byte_size(bin) - 1, bin, f, exception,
                      fd, fname, t, s)
  end
  ...
  defp find_good_split(size, b, f, exception, fd, fname, t, s) do
    case (b) do
      <<_Bytes :: size(size) - binary, h :: integer,
          tail :: binary>>
          when h == 32 or h == 13 or h == 10 or h == 9 ->
        {subB, _} = :erlang.split_binary(b, size + 1)
        newS = :xmerl_scan.cont_state([{tail, fname, fd} | t],
                                        s)
        f.(:erlang.binary_to_list(subB), newS)
      _ ->
        find_good_split(size - 1, b, f, exception, fd, fname, t,
                          s)
    end
  end
  ...
  defp fetch_URI(uRI, s) do
    ...
    file = path_locate(r_xmerl_scanner(s, :fetch_path), filename,
                         fullname)
    :no_debug
    case (:file.open(file, [:read, :raw, :binary])) do
      {:ok, fd} ->
        contS = :xmerl_scan.cont_state(s)
        newS = :xmerl_scan.cont_state([{:erlang.list_to_binary([]),
                                          file, fd} |
                                           contS],
                                        s)
        {:ok, {:string, []}, newS}
  ...
  defp rules_write(context, name, value,
            r_xmerl_scanner(rules: :undefined) = s) do
