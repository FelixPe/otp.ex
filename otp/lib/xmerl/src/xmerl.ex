defmodule :m_xmerl do
  ...
  defp export_simple1(content, callback, rootAttrs) do
    export1(:xmerl_lib.expand_content(content), callback,
              rootAttrs)
  end
  ...
  defp export1(content, callbacks, rootAttrs)
    ...
    attrs = :xmerl_lib.expand_attributes(rootAttrs, 1,
                                           [{:"#root#", 1}])
    root = r_xmlElement(name: :"#root#", pos: 1, parents: [],
               attributes: attrs)
  ...
  def export_content([r_xmlText(value: text) | es], callbacks) do
    [apply_text_cb(callbacks, text) | export_content(es,
                                                       callbacks)]
  end
  ...
  def export_content([e | es], callbacks) do
    [export_element(e, callbacks) | export_content(es,
                                                     callbacks)]
  end
  ...
  def export_element(e = r_xmlElement(name: tag, pos: pos,
  ...
  def export_element(e, callbackModule, callbackState)
    ...
    export_element(e, callbacks(callbackModule),
                     callbackState)
  end

  def export_element(r_xmlText(value: text), callbackModule,
           _CallbackState) do
    apply_text_cb(callbackModule, text)
  end

  def export_element(e = r_xmlElement(name: tag, pos: pos, parents: parents,
                 attributes: attributes, content: content),
           callbacks, cBstate) do
  ...
  defp tagdef(tag, pos, parents, args, cBs) do
    case (apply_tag_cb(cBs, tag, args)) do
      ...
      {:"#xml-redefine#", data} ->
        export_content(:xmerl_lib.expand_content(data, pos,
                                                   parents),
                         cBs)
      other ->
        other
  ...
  defp check_inheritance(m, visited) do
    case (m.unquote(:"#xml-inheritance#")()) do
      [] ->
        [m | visited]
      mods ->
