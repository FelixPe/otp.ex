defmodule :m_xmerl_xpath do
  ...
  def string(str, node, parents, doc, options) do
    fullParents = (case (parents) do
                     [] ->
                       []
                     [{h, p} | _] when (is_atom(h) and is_integer(p)) ->
                       full_parents(parents, doc)
                   end)
    contextNode = r_xmlNode(type: node_type(node), node: node,
                      parents: fullParents)
    wholeDoc = whole_document(doc)
    context = r_xmlContext(new_context(options), context_node: contextNode, 
    ...
    case (r_xmlContext(newContext, :nodeset)) do
      scalObj = r_xmlObj(type: scalar) when scalar == :boolean or
  ...
  defp whole_document(other) do
    r_xmlNode(type: :root_node, node: r_xmlDocument(content: other),
        parents: [])
  end
  ...
  defp path_expr({:step, {axis, nodeTest, predExpr}},
            s = r_state(context: c, acc: acc)) do
    :no_debug
  ...
  defp eval_pred({:number, n0},
            s = r_state(context: c = r_xmlContext(nodeset: nS,
                                   axis_type: axisType))) do
    len = length(nS)
    case (len >= n0) do
      true ->
        n = (case (axisType) do
  ...
  defp eval_pred(predicate,
            s = r_state(context: c = r_xmlContext(nodeset: nodeSet))) do
    newNodeSet = :lists.filter(fn node ->
                                    thisContext = r_xmlContext(c, context_node: node)
                                    :xmerl_xpath_pred.eval(predicate,
                                                             thisContext)
                               end,
                                 nodeSet)
    newContext = r_xmlContext(c, nodeset: newNodeSet)
  ...
  def write_node(r_xmlNode(pos: pos,
             node: r_xmlNsNode(parents: ps, prefix: prefix))) do
  ...
  defp eval_primary_expr(primExpr, s = r_state(context: context)) do
    newNodeSet = :xmerl_xpath_lib.eval(:primary_expr,
                                         primExpr, context)
  ...
  def axis(axis, nodeTest, context = r_xmlContext(nodeset: nS0),
           acc) do
    newNodeSet = :lists.foldr(fn n, accX ->
                                   axis1(axis, nodeTest, n, accX, context)
                              end,
                                acc, nS0)
    update_nodeset(fwd_or_reverse(axis, context),
                     newNodeSet)
  end
  ...
  defp match_desc([e = r_xmlElement() | t], parents, tok, acc, context) do
    ...
    acc2 = match_desc(get_content(e), newParents, tok, acc1,
                        context)
  ...
  defp match_preceding_sibling(tok, n, acc, context) do
    ...
    case (ps) do
      [r_xmlNode(type: :element, node: r_xmlElement() = pNode) | _] ->
        precedingSiblings = :lists.sublist(get_content(pNode),
                                             1, get_position(node) - 1)
        :lists.foldr(fn e, accX ->
                          thisN = r_xmlNode(type: node_type(e), node: e, parents: ps)
                          match_self(tok, thisN, accX, context)
                     end,
                       acc, precedingSiblings)
      _Other ->
        acc
    end
  end
  ...
  defp match_preceding(tok, n, acc, context) do
    ...
    case (ps) do
      [r_xmlNode(type: :element, node: r_xmlElement() = pNode) = p | _] ->
        precedingSiblings = :lists.sublist(get_content(pNode),
                                             1, get_position(node) - 1)
        acc0 = :lists.foldr(fn e, accX ->
                                 thisN = r_xmlNode(type: node_type(e), node: e,
                                             parents: ps)
                                 match_descendant_or_self(tok, thisN, accX,
                                                            context)
                            end,
                              acc, precedingSiblings)
  ...
  defp match_attribute(tok, n, acc, context) do
    case (r_xmlNode(n, :type)) do
      :element ->
        ...
        :lists.foldr(fn a, accX ->
                          thisN = r_xmlNode(type: :attribute, node: a,
                                      parents: [n | ps])
                          match_self(tok, thisN, accX, context)
                     end,
                       acc, r_xmlElement(e, :attributes))
      _Other ->
        acc
    end
  end
  ...
  defp match_namespace(tok, n, acc, context) do
    case (r_xmlNode(n, :type)) do
      :element ->
        ...
        thisPs = [n | ps]
        acc0 = (case (default) do
                  d when d === [] or d === :"" ->
                    {[], 1}
                  uRI ->
                    defaultNSNode = r_xmlNsNode(parents: thisEPs, pos: 1, prefix: [],
                                        uri: uRI)
                    node = r_xmlNode(type: :namespace, node: defaultNSNode,
                               parents: thisPs)
                    {[node], 2}
                end)
        {nodes, _I} = :lists.foldr(fn {prefix, uRI},
                                        {accX, i} ->
                                        nSNode = r_xmlNsNode(parents: thisEPs, pos: i,
                                                     prefix: prefix, uri: uRI)
                                        thisN = r_xmlNode(pos: i, type: :namespace,
                                                    node: nSNode,
                                                    parents: thisPs)
                                        {[thisN | accX], i + 1}
                                   end,
                                     acc0, nSPairs)
        :lists.foldr(fn thisN, accX ->
                          match_self(tok, thisN, accX, context)
                     end,
                       acc, nodes)
      _Other ->
  ...
  defp update_nodeset(context = r_xmlContext(axis_type: axisType), nodeSet) do
    mapFold = (case (axisType) do
                 :forward ->
                   :mapfoldl
                 :reverse ->
                   :mapfoldr
               end)
    {result, _N} = apply(:lists, mapFold,
                           [fn node, n ->
  ...
  defp node_test(_Test,
            r_xmlNode(type: :attribute, node: r_xmlAttribute(name: :xmlns)), _Context) do
    false
  end
  ...
  defp node_test({:name, {tag, _Prefix, _Local}},
            r_xmlNode(node: r_xmlElement(name: tag)) = _N, _Context) do
    true
  end

  defp node_test({:name, {tag, prefix, local}},
            r_xmlNode(node: r_xmlElement(name: name, expanded_name: eExpName,
                        nsinfo: {_Prefix1, _})),
            context) do
  ...
  defp node_test({:name, {_Tag, prefix, local}},
            r_xmlNode(node: r_xmlElement(name: name, expanded_name: _EExpName,
                        namespace: nS)),
            context) do
    case (expanded_name(prefix, local, context)) do
      [] ->
  ...
  defp node_test({:name, {tag, _Prefix, _Local}},
            r_xmlNode(node: r_xmlAttribute(name: tag)), _Context) do
    true
  end
  ...
  defp node_test({:name, {_Tag, [], local}},
            r_xmlNode(node: r_xmlNsNode(prefix: local)), _Context) do
    true
  end
  ...
  defp node_test({:processing_instruction, name1},
            r_xmlNode(node: r_xmlPI(name: name2)), _Context) do
