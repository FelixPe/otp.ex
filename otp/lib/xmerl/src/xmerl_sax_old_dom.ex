defmodule :m_xmerl_sax_old_dom do
  ...
  defp build_dom(
         ...
         r_xmerl_sax_old_dom_state(tags: t, cno: cN, namespaces: nS0, dom: d) = state
       ) do
    {num, newCN} =
      case cN do
        [] ->
      ...
      _ ->
        throw(
          {:xmerl_sax_old_dom_error,
           ~c"Got end of element: " ++
             localName ++ ~c" but expected: " ++ r_xmlElement(current, :name)}
        )
    end
  end
  ...
  defp build_dom(
         {:processingInstruction, ~c"xml", piData},
         r_xmerl_sax_old_dom_state(dom: d) = state
       ) do
    {vsn, piData1} = find_and_remove_attribute(~c"version", piData, [])
    {enc, piData2} = find_and_remove_attribute(~c"encoding", piData1, [])
    {standalone, piData3} = find_and_remove_attribute(~c"standalone", piData2, :yes)
  ...
  defp parse_attributes([{uri, prefix, localName, attrValue} | as], elName, nameSpace, t, n, acc) do
  ...
  defp convert_to_expanded_name(uri, localName) do
    {:erlang.list_to_atom(uri), :erlang.list_to_atom(localName)}
  end
