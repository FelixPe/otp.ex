defmodule :m_xmerl_regexp do
  ...
  defp setup([?\\, ?c | s], acc) do
    setup(s, ~c"]9-0z-aZ-A_:." ++ [183] ++ ~c"-[" ++ acc)
  end
  ...
  def format_error({:interval_range, what}) do
    [~c"illegal interval range", :io_lib.write_string(what)]
  end

  def format_error({:illegal, what}) do
    [~c"illegal character `", what, ~c"'"]
  end

  def format_error({:unterminated, what}) do
    [~c"unterminated `", what, ~c"'"]
  end

  def format_error({:posix_cc, what}) do
    [~c"illegal POSIX character class ", :io_lib.write_string(what)]
  end
  ...
  def split(string, ~c" ") do
    {:ok, {:regexp, rE}} = parse(~c"[ \t]+")
  ...
  defp char_class(~c"[:" ++ s0, cc0) do
    case posix_cc(s0, cc0) do
      {cc1, ~c":]" ++ s1} ->
        char_class(s1, cc1)

      {_, _S1} ->
        parse_error({:posix_cc, ~c"[:" ++ s0})
    end
  end
  ...
  defp posix_cc(~c"alpha" ++ s, cc) do
    {[{?A, ?Z}, {192, 214}, {216, 223}, {?a, ?z}, {224, 246}, {248, 255} | cc], s}
  end

  defp posix_cc(~c"blank" ++ s, cc) do
    {[?\s, ?\t, 160 | cc], s}
  end

  defp posix_cc(~c"cntrl" ++ s, cc) do
    {[{0, 31}, {127, 159} | cc], s}
  end

  defp posix_cc(~c"digit" ++ s, cc) do
    {[{?0, ?9} | cc], s}
  end

  defp posix_cc(~c"graph" ++ s, cc) do
    {[{33, 126}, {161, 255} | cc], s}
  end

  defp posix_cc(~c"lower" ++ s, cc) do
    {[{?a, ?z}, {224, 246}, {248, 255} | cc], s}
  end

  defp posix_cc(~c"print" ++ s, cc) do
    {[{32, 126}, {160, 255} | cc], s}
  end

  defp posix_cc(~c"punct" ++ s, cc) do
    {[{?!, ?/}, {?:, ??}, {?{, ?~}, {161, 191} | cc], s}
  end

  defp posix_cc(~c"space" ++ s, cc) do
    {[?\s, ?\t, ?\f, ?\r, ?\v, 160 | cc], s}
  end

  defp posix_cc(~c"upper" ++ s, cc) do
    {[{?A, ?Z}, {192, 214}, {216, 223} | cc], s}
  end

  defp posix_cc(~c"xdigit" ++ s, cc) do
    {[{?a, ?f}, {?A, ?F}, {?0, ?9} | cc], s}
  end

  defp posix_cc(s, _Cc) do
    parse_error({:posix_cc, ~c"[:" ++ s})
  end
