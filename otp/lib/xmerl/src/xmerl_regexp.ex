defmodule :m_xmerl_regexp do
  ...
  defp reg4([c0 | s0], sc) when (is_integer(c0) and
  ...
  defp char(?\\, [o1, o2, o3 | s]) when (o1 >= ?0 and
  ...
  defp pack_cc(cc0) do
    cc1 = :lists.usort(fn {cf1, _}, {cf2, _} ->
                            cf1 < cf2
                          {cf1, _}, c ->
                            cf1 < c
                          c, {cf, _} ->
  ...
  defp re_apply({:iclosure, rE, n, m}, more, s, p, subs)
      when n > 0 do
    re_apply(rE, [{:iclosure, rE, n - 1, m} | more], s, p,
               subs)
  end
  ...
  defp in_char_class(c, [{c1, c2} | _Cc]) when (c >= c1 and
                                       c <= c2) do
    true
  end
  ...
  Record.defrecord(:r_dfa_state, :dfa_state, no: :undefined,
                                     nfa: [], trans: [], accept: :no)
  Record.defrecord(:r_c_state, :c_state, no: :undefined,
                                   trans: [], tmin: 0, smin: :none, tmax: 0,
                                   smax: :none, accept: false, spec: [])
  ...
  defp build_combined_nfa(rEAs) do
    ...
    f = r_nfa_state(no: next, edges: epsilon_trans(starts),
            accept: :no)
    {[f | nFA], next}
  end
  ...
  defp build_nfa_list([{rE, action} | rEAs], nFA0, starts, next0) do
    ...
    build_nfa_list(rEAs, nFA1 ++ nFA0, [start | starts],
                     next1)
  end
  ...
  defp build_nfa({:pclosure, rE}, n0, s, nFA0) do
    {nFA1, n1, e1} = build_nfa(rE, n0 + 1, n0, nFA0)
    e = n1
    {[r_nfa_state(no: s, edges: [{:epsilon, n0}]), r_nfa_state(no: e1,
  ...
  defp build_nfa({:char_class, cc}, n, s, nFA) do
    {[r_nfa_state(no: s, edges: [{nfa_char_class(cc), n}]) | nFA],
       n + 1, n}
  end

  defp build_nfa({:comp_class, cc}, n, s, nFA) do
    {[r_nfa_state(no: s, edges: [{nfa_comp_class(cc), n}]) | nFA],
       n + 1, n}
  end
          ...
          false ->
            case (keysearch(s, r_dfa_state(:nfa), ms)) do
              ...
              false ->
                ...
                build_dfa(crs, set, [u | us], n + 1, [{cr, n} | ts], ms,
                            nFA)
            end
        end
      [] ->
  ...
  defp minimise_dfa(dFA0, start, n) do
    case (min_dfa(dFA0)) do
      ...
      {dFA1, rs} ->
        minimise_dfa(min_update(dFA1, rs),
                       min_new_state(start, rs), n)
    end
  end
  ...
  defp comp_apply(cs, p, {dFA, start, _Fail}) do
    comp_apply(:erlang.element(start, dFA), cs, p, dFA,
                 :nomatch)
  end
  ...
  defp comp_apply_tr(r_c_state(trans: :none, accept: a), cs, p, _DFA,
            accept) do
    accept_value(a, cs, p, accept)
  end

  defp comp_apply_tr(r_c_state(trans: tr, tmin: tmin, smin: smin, tmax: tmax,
              smax: smax, accept: a),
            [c | cs] = cs0, p, dFA, accept) do
    nextSt = (cond do
                c <= tmin ->
                  smin
                c >= tmax ->
    ...
    comp_apply(:erlang.element(nextSt, dFA), cs, p + 1, dFA,
                 accept_value(a, cs0, p, accept))
  end

  defp comp_apply_tr(r_c_state(trans: _Tr, accept: a), [], p, _DFA,
            accept) do
  ...
  defp tuplelise_dfa(dFA0, noAccept) do
    dFA1 = map(fn r_dfa_state(no: n, trans: ts, accept: a) ->
                    {tr, tmin, smin, tmax, smax, sp} = build_trans(ts,
                                                                     noAccept)
                    r_c_state(no: n, trans: tr, tmin: tmin, smin: smin, tmax: tmax,
                        smax: smax, accept: fix_accept(a), spec: sp)
               end,
                 dFA0)
  ...
  defp build_trans(ts0, noAccept) do
    {ts1, sp1} = foldl(fn {{_, _}, _} = t, {ts, sp} ->
                            {[t | ts], sp}
                          {_, _} = t, {ts, sp} ->
                            {ts, [t | sp]}
                       end,
    ...
    cond do
      ...
      true ->
        ...
        {:erlang.list_to_tuple(trans), tmin, smin, tmax, smax,
           sp1}
    end
  end
  ...
  defp expand_trans([{{c1, c2}, s}], last, def__) do
    trs = duplicate(c1 - (last + 1),
                      def__) ++ duplicate(c2 - c1 + 1, s)
    {trs, c2 + 1, def__}
  end

  defp expand_trans([{{c1, c2}, s} | crs], last, def__) do
    {trs0, tmax, smax} = expand_trans(crs, c2, def__)
    trs1 = duplicate(c1 - (last + 1),
                       def__) ++ duplicate(c2 - c1 + 1, s) ++ trs0
    {trs1, tmax, smax}
  end
