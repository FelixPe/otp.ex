defmodule :m_xmerl_xpath_scan do
  ...
  defp scan_token(str = [?., h | _], a) when (h >= ?0 and
                                        h <= ?9) do
    scan_number(str, a)
  end
  ...
  defp scan_token([?$ | t], _A) do
    ...
    case (prefix) do
      ...
      _ ->
        {{:var_reference, 1,
            :erlang.list_to_atom(prefix ++ ':' ++ local)},
           t1}
    end
  end
  ...
  defp other_name([], local, t) do
    {{:name, 1, {:erlang.list_to_atom(local), [], local}},
       t}
  end

  defp other_name(prefix, local, t) do
    {{:name, 1,
        {:erlang.list_to_atom(prefix ++ ':' ++ local), prefix,
           local}},
       t}
  end
  ...
  defp scan_prefix(str = [h | _], acc) when h == 32 or h == 13 or
                                     h == 10 or h == 9 do
  ...
  defp scan_local_part(str = [h | _], acc) when h == 32 or h == 13 or
                                     h == 10 or h == 9 do
    {:lists.reverse(acc), str}
  end
  ...
  defp strip_ws([h | t]) when h == 32 or h == 13 or h == 10 or
                          h == 9 do
    strip_ws(t)
  end
