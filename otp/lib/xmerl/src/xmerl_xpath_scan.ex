defmodule :m_xmerl_xpath_scan do
  ...
  defp scan_token(~c".." ++ t, _A) do
    {:rescan, ~c"parent::node()" ++ t}
  end

  defp scan_token(~c"@" ++ t, _A) do
    {:rescan, ~c"attribute::" ++ t}
  end
  ...
  defp scan_token(~c"//" ++ t, _A) do
    {:rescan, ~c"/descendant-or-self::node()/" ++ t}
  end
  ...
  defp scan_token([?$ | t], _A) do
    ...
    case prefix do
      ...
      _ ->
        {{:var_reference, 1, :erlang.list_to_atom(prefix ++ ~c":" ++ local)}, t1}
    end
  end
  ...
  defp operator_name([], ~c"and", t) do
    {{:and, 1, :and}, t}
  end

  defp operator_name([], ~c"or", t) do
    {{:or, 1, :or}, t}
  end

  defp operator_name([], ~c"mod", t) do
    {{:mod, 1, :mod}, t}
  end

  defp operator_name([], ~c"div", t) do
    {{:div, 1, :div}, t}
  end

  defp other_name(prefix, [], ~c"*" ++ t) do
    {{:prefix_test, 1, prefix}, t}
  end

  defp other_name(prefix, local, t = ~c"(" ++ _) do
  ...
  defp other_name(prefix, local, t) do
    {{:name, 1, {:erlang.list_to_atom(prefix ++ ~c":" ++ local), prefix, local}}, t}
  end

  defp node_type_or_function_name([], ~c"comment", t) do
    {{:node_type, 1, :comment}, t}
  end

  defp node_type_or_function_name([], ~c"text", t) do
    {{:node_type, 1, :text}, t}
  end

  defp node_type_or_function_name([], ~c"processing-instruction", t) do
    {{:"processing-instruction", 1, :"processing-instruction"}, t}
  end

  defp node_type_or_function_name([], ~c"node", t) do
    {{:node_type, 1, :node}, t}
  end
  ...
  defp axis([], ~c"ancestor-or-self", t) do
    {{:axis, 1, :ancestor_or_self}, t}
  end

  defp axis([], ~c"ancestor", t) do
    {{:axis, 1, :ancestor}, t}
  end

  defp axis([], ~c"attribute", t) do
    {{:axis, 1, :attribute}, t}
  end

  defp axis([], ~c"child", t) do
    {{:axis, 1, :child}, t}
  end

  defp axis([], ~c"descendant-or-self", t) do
    {{:axis, 1, :descendant_or_self}, t}
  end

  defp axis([], ~c"descendant", t) do
    {{:axis, 1, :descendant}, t}
  end

  defp axis([], ~c"following-sibling", t) do
    {{:axis, 1, :following_sibling}, t}
  end

  defp axis([], ~c"following", t) do
    {{:axis, 1, :following}, t}
  end

  defp axis([], ~c"namespace", t) do
    {{:axis, 1, :namespace}, t}
  end

  defp axis([], ~c"parent", t) do
    {{:axis, 1, :parent}, t}
  end

  defp axis([], ~c"preceding-sibling", t) do
    {{:axis, 1, :preceding_sibling}, t}
  end

  defp axis([], ~c"preceding", t) do
    {{:axis, 1, :preceding}, t}
  end

  defp axis([], ~c"self", t) do
    {{:axis, 1, :self}, t}
  end
  ...
  defp scan_prefix(t = ~c"::" ++ _, acc) do
    {{[], :lists.reverse(acc)}, t}
  end

  defp scan_prefix(~c":" ++ t, acc) do
  ...
  defp scan_number(~c"." ++ t, []) do
    {digits, t1} = scan_digits(t, ~c".0")
    number = :erlang.list_to_float(digits)
    {{:number, 1, number}, t1}
  end

  defp scan_number(~c"." ++ t, acc) do
    {digits, t1} = scan_digits(t, ~c"." ++ acc)
    number = :erlang.list_to_float(digits)
    {{:number, 1, number}, t1}
  end
