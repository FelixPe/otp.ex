defmodule :m_xmerl_xsd_type do
  ...
  def check_simpleType(:normalizedString, value, _S) do
    case (for x <- value, :xmerl_lib.is_char(x),
                ns_whitespace(x) == false do
            x
          end) do
      ^value ->
        {:ok, value}
  ...
  def check_simpleType(:hexBinary, value, _S) do
    ...
    case (for x <- value, isEven.(length(value)),
                isHex.(x) do
            x
          end) do
      ^value ->
  ...
  def check_simpleType(:anyURI, value, s) do
    case (:xmerl_uri.parse(value)) do
      {:error, _} ->
        case ((try do
                :file.read_file_info(:filename.join(r_xsd_state(s, :xsd_base),
                                                      value))
              catch
                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                :exit, e -> {:EXIT, e}
  ...
  defp check_duration('P' ++ value) do
    {date, time} = :lists.splitwith(fn ?T ->
                                         false
                                       _ ->
                                         true
                                    end,
  ...
  defp check_nonPositiveInteger(value) do
    check_constr_int(value, :undefined, 0,
                       :illegal_nonPositiveInteger)
  end

  defp check_negativeInteger(value) do
    check_constr_int(value, :undefined, - 1,
  ...
  defp check_int(value) do
    check_constr_int(value, - 2147483648, 2147483647,
                       :illegal_int)
  end
  ...
  defp check_nonNegativeInteger(value) do
    check_constr_int(value, 0, :undefined,
                       :illegal_nonNegativeInteger)
  end

  defp check_unsignedLong(value) do
    check_constr_int(value, 0, 18446744073709551615,
                       :illegal_unsignedLong)
  end

  defp check_unsignedInt(value) do
    check_constr_int(value, 0, 4294967295,
                       :illegal_unsignedInt)
  end

  defp check_unsignedShort(value) do
    check_constr_int(value, 0, 65535,
                       :illegal_unsignedShort)
  end
  ...
  defp check_time(time) do
    {hMS, tZ} = (case (:lists.split(8, time)) do
                   {t, '.' ++ secFractionsTZ} ->
                     onlyDigits = fn x when (x >= ?0 and x <= ?9) ->
                                       true
                                     _ ->
                                       false
                                  end
                     {secFrac, tZone} = :lists.splitwith(onlyDigits,
                                                           secFractionsTZ)
  ...
  defp check_date(date) do
    {year, month, day} = (case (:string.tokens(date, '-+Z')) do
                            [y, m, d, tZ] ->
  ...
  defp check_gYearMonth(value) do
    {year, month} = (case (:string.tokens(value, '-+Z')) do
                       [y, m, tZ] ->
                         {:ok, _} = check_timezone(tZ)
                         {y, m}
                       [y, m] ->
  ...
  defp check_gYear(value) do
    year = (case (:string.tokens(value, '-+Z')) do
              [y, tZ] ->
                {:ok, _} = check_timezone(tZ)
                y
              [y] ->
  ...
  defp length_fun(t, v) when t == :string or
  ...
  defp length_fun(t, v) when t == :base64Binary or
                       t == :hexBinary do
    fn val ->
  ...
  defp minLength_fun(t, v) when t == :string or
                       t == :normalizedString or t == :token or t == :Name or
                       t == :NCName or t == :language or t == :ID or
                       t == :IDREF or t == :IDREFS or t == :ENTITY or
                       t == :ENTITIES or t == :NMTOKEN or t == :NMTOKENS or
    ...
    fn val ->
         case (:string.len(val) >= v) do
           ...
           false ->
             {:error,
                {:minLength, :string.len(val), :should_at_least_be, v}}
         end
    end
  end
  ...
  defp minLength_fun(t, v) when t == :base64Binary or
                       t == :hexBinary do
    fn val ->
         case (length(val) >= v) do
           true ->
           ...
           false ->
             {:error,
                {:minLength, length(val), :should_at_least_be, v}}
         end
    end
  end
  ...
  defp maxLength_fun(t, v) when t == :string or
                       t == :normalizedString or t == :token or t == :Name or
                       t == :NCName or t == :language or t == :ID or
                       t == :IDREF or t == :IDREFS or t == :ENTITY or
                       t == :ENTITIES or t == :NMTOKEN or t == :NMTOKENS or
    ...
    fn val ->
         case (length(val)) do
           ...
           _ ->
             {:error,
                {:maxLength, :string.len(val), :should_not_be_more_than,
                   v}}
         end
    end
  end
  ...
  defp maxLength_fun(t, v) when t == :base64Binary or
                       t == :hexBinary do
    fn val ->
         case (length(val) <= v) do
           true ->
           ...
           false ->
             {:error,
                {:maxLength, length(val), :should_not_be_more_than, v}}
         end
    end
  end
  ...
  def collapse_ws(val) do
    collapse_ws(:lists.dropwhile(fn wS when wS == 32 or
  ...
  defp collapse_ws([], acc) do
    :lists.reverse(:lists.dropwhile(fn ?\s ->
                                         true
                                       _ ->
                                         false
                                    end,
  ...
  defp maxInclusive_fun(t, v) when t == :integer or
    ...
    fn val ->
         case ((try do
           ...
           _ ->
             {:error,
                {:maxInclusive, val, :should_be_less_than_or_equal_with,
                   v}}
         end
    end
  end

  defp maxInclusive_fun(t, v) when t == :decimal or t == :float or
                       t == :double do
    fn val ->
  ...
  defp maxExclusive_fun(t, v) when t == :integer or
                       t == :positiveInteger or t == :negativeInteger or
                       t == :nonNegativeInteger or t == :nonPositiveInteger or
                       t == :long or t == :unsignedLong or t == :int or
                       t == :unsignedInt or t == :short or
  ...
  defp maxExclusive_fun(t, v) when t == :decimal or t == :float or
                       t == :double do
    fn val ->
         case (:xmerl_xsd_type.compare_floats(val, v)) do
           :lt ->
  ...
  defp minExclusive_fun(t, v) when t == :integer or
                       t == :positiveInteger or t == :negativeInteger or
                       t == :nonNegativeInteger or t == :nonPositiveInteger or
                       t == :long or t == :unsignedLong or t == :int or
                       t == :unsignedInt or t == :short or
  ...
  defp minExclusive_fun(t, v) when t == :decimal or t == :float or
                       t == :double do
    fn val ->
         case (:xmerl_xsd_type.compare_floats(val, v)) do
           :gt ->
  ...
  defp minInclusive_fun(t, v) when t == :integer or
                       t == :positiveInteger or t == :negativeInteger or
                       t == :nonNegativeInteger or t == :nonPositiveInteger or
                       t == :long or t == :unsignedLong or t == :int or
                       t == :unsignedInt or t == :short or
    ...
    fn val ->
         case ((try do
           ...
           _ ->
             {:error,
                {:minInclusive, val, :not_greater_than_or_equal_with,
                   v}}
         end
    end
  end

  defp minInclusive_fun(t, v) when t == :decimal or t == :float or
                       t == :double do
    fn val ->
         case (:xmerl_xsd_type.compare_floats(val, v)) do
           :lt ->
  ...
  defp totalDigits_fun(t, v) when t == :integer or
                       t == :positiveInteger or t == :negativeInteger or
                       t == :nonNegativeInteger or t == :nonPositiveInteger or
                       t == :long or t == :unsignedLong or t == :int or
                       t == :unsignedInt or t == :short or
    ...
    fn val ->
         ...
         length = (case (:lists.member(?., val2)) do
                     true ->
  ...
  defp fractionDigits_fun(t, v) when t == :integer or
                       t == :positiveInteger or t == :negativeInteger or
                       t == :nonNegativeInteger or t == :nonPositiveInteger or
                       t == :long or t == :unsignedLong or t == :int or
                       t == :unsignedInt or t == :short or
    ...
    fn val ->
         ...
         cond do
           ...
           true ->
             {:error,
                {:fractionDigits, len, :to_many_digits_in, val}}
         end
    end
  end
    ...
    case (:string.tokens(str, '.')) do
      [b, dE] ->
        case (:string.tokens(dE, 'Ee')) do
          [d, e] ->
            {sign, :erlang.list_to_integer(b),
               remove_trailing_zeros(d), :erlang.list_to_integer(e)}
          [d] ->
            {sign, :erlang.list_to_integer(b),
      ...
      [b] ->
        case (:string.tokens(str, 'Ee')) do
          [i, e] ->
            {sign, :erlang.list_to_integer(i), '0',
  ...
  defp pow(mantissa, fraction, exponent) do
    mantissa * :math.pow(10,
                           exponent) + :erlang.list_to_integer(fraction) * :math.pow(10,
  ...
  defp remove_trailing_zeros(str) do
    ...
    case (:lists.reverse(:lists.dropwhile(pred,
                                            :lists.reverse(str)))) do
      [] ->
        '0'
      fr ->
        fr
  ...
  def compare_durations(v1, v2) do
    ...
    cmpRes1 = compare_dateTime(normalize_dateTime(add_duration2dateTime(ref1_dateTime,
                                                                          v1)),
                                 normalize_dateTime(add_duration2dateTime(ref1_dateTime,
                                                                            v2)))
    cmpRes2 = compare_dateTime(normalize_dateTime(add_duration2dateTime(ref2_dateTime,
                                                                          v1)),
                                 normalize_dateTime(add_duration2dateTime(ref2_dateTime,
                                                                            v2)))
    cmpRes3 = compare_dateTime(normalize_dateTime(add_duration2dateTime(ref3_dateTime,
                                                                          v1)),
                                 normalize_dateTime(add_duration2dateTime(ref3_dateTime,
                                                                            v2)))
    cmpRes4 = compare_dateTime(normalize_dateTime(add_duration2dateTime(ref4_dateTime,
                                                                          v1)),
                                 normalize_dateTime(add_duration2dateTime(ref4_dateTime,
                                                                            v2)))
    cond do
  ...
  def compare_dateTime(dT1 = {_, _, _, _, _, _, z},
           dT2 = {_, _, _, _, _, _, z}) do
    case (dT1 < dT2) do
      true ->
        :lt
      _ ->
  ...
  def compare_dateTime(p = {_, _, _, _, _, _, {_, _, _}},
           _Q = {y, m, d, h, min, s, :none}) do
    case (compare_dateTime(p,
                             normalize_dateTime({y, m, d, h, min, s,
                                                   {:pos, 14, 0}}))) do
      :lt ->
        :lt
      _ ->
  ...
  def compare_dateTime(_P = {y, m, d, h, min, s, :none},
           q = {_, _, _, _, _, _, {_, _, _}}) do
    case (compare_dateTime(normalize_dateTime({y, m, d, h,
                                                 min, s, {:neg, 14, 0}}),
                             q)) do
      :lt ->
        :lt
  ...
  def compare_dateTime(p, q) when is_list(p) do
    compare_dateTime(normalize_dateTime(dateTime_atoms(p)),
                       q)
  end
  ...
  defp monthValue(m, _Y) when m == 1 or m == 3 or m == 5 or
                        m == 7 or m == 8 or m == 10 or m == 12 do
    31
  end

  defp monthValue(m, _Y) when m == 4 or m == 6 or m == 9 or
                        m == 11 do
    30
  end
  ...
  defp add_duration2dateTime2({syear, smonth, sday, shour, sminute, ssec,
             szone},
            {dyears, dmonths, ddays, dhours, dminutes, dsecs}) do
    ...
    tempDays = (case (maximumDayInMonthFor(eyear,
                                             emonth)) do
  ...
  defp dateTime_atoms(s, sign) do
    ...
    {zone, zoneSign,
       [hour, min, sec]} = (case (:lists.reverse(timeZone)) do
                              'Z' ++ _ ->
    ...
    {set_sign(sign, yY), :erlang.list_to_integer(mM),
       :erlang.list_to_integer(dD),
       :erlang.list_to_integer(hour),
       :erlang.list_to_integer(min), sign_sec(:pos, sec),
  ...
  defp zone_atoms(sign, zone) when is_list(zone) do
    case (:string.tokens(zone, ':')) do
      [h, m] ->
        {sign, :erlang.list_to_integer(h),
           :erlang.list_to_integer(m)}
      _ ->
        :none
    end
  end
  ...
  defp get_digit([h | t], delim, acc, str) when (h >= ?0 and
                                            h <= ?9) do
  ...
  def normalize_dateTime({y, m, d, hour, min, sec, {sign, zH, zM}}) do
    tmpMin = min + set_sign(invert_sign(sign),
                              :erlang.integer_to_list(zM))
    nMin = modulo(tmpMin, 60)
    carry1 = fQuotient(tmpMin, 60)
    tmpHour = hour + set_sign(invert_sign(sign),
                                :erlang.integer_to_list(zH)) + carry1
