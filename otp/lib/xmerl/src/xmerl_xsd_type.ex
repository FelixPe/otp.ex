defmodule :m_xmerl_xsd_type do
  ...
  def check_simpleType(:boolean, ~c"true", _S) do
    {:ok, ~c"true"}
  end

  def check_simpleType(:boolean, ~c"false", _S) do
    {:ok, ~c"false"}
  end

  def check_simpleType(:boolean, ~c"1", _S) do
    {:ok, ~c"1"}
  end

  def check_simpleType(:boolean, ~c"0", _S) do
    {:ok, ~c"0"}
  end
  ...
  defp check_float(v = ~c"NaN") do
    {:ok, v}
  end

  defp check_float(value) do
    case :string.tokens(value, ~c"eE") do
      [mantissa, exponent] ->
  ...
  defp check_duration(~c"-" ++ value) do
    check_duration(value)
  end

  defp check_duration(~c"P" ++ value) do
    {date, time} =
      :lists.splitwith(
        fn
          ?T ->
    ...
    {:ok, _} = check_duration_date(date, [~c"Y", ~c"M", ~c"D"])
    {:ok, _} = check_duration_time(time, [~c"T", ~c"H", ~c"M", ~c"S"])
  end

  defp check_duration_date(~c"", _) do
    {:ok, ~c""}
  end
  ...
  defp check_duration_time(~c"", [_H | _T]) do
    {:ok, ~c""}
  end
  ...
  defp check_duration_time(~c"T" ++ time, tTokens) do
    [_H | _] = time
  ...
  defp check_dateTime(~c"-" ++ dateTime) do
    check_dateTime(dateTime)
  end

  defp check_dateTime(~c"+" ++ _DateTime) do
    {:error, {:invalid_dateTime, :plus_sign}}
  end

  defp check_dateTime(dateTime) do
    [date, time] = :string.tokens(dateTime, ~c"T")
    [y, m, d] = :string.tokens(date, ~c"-")
    {:ok, _} = check_year(y)
  ...
  defp check_timezone(~c"Z") do
    {:ok, ~c"Z"}
  end

  defp check_timezone(tZ) do
    [h, m] = :string.tokens(tZ, ~c":")
  ...
  defp check_date(~c"-" ++ date) do
    check_date(date)
  end

  defp check_date(~c"+" ++ _Date) do
    {:error, {:invalid_date, :plus_sign}}
  end

  defp check_date(date) do
    {year, month, day} =
      case :string.tokens(date, ~c"-+Z") do
        [y, m, d, tZ] ->
  ...
  defp check_gYearMonth(~c"-" ++ value) do
    check_gYearMonth(value)
  end

  defp check_gYearMonth(~c"+" ++ _Value) do
    {:error, {:invalid_gYearMonth, :plus_sign}}
  end

  defp check_gYearMonth(value) do
    {year, month} =
      case :string.tokens(value, ~c"-+Z") do
        [y, m, tZ] ->
          {:ok, _} = check_timezone(tZ)
          {y, m}
  ...
  defp check_gYear(~c"-" ++ value) do
    check_gYear(value)
  end

  defp check_gYear(~c"+" ++ _Value) do
  ...
  defp check_gMonthDay(~c"--" ++ value) do
    {m, ~c"-" ++ dTZ} = :lists.split(2, value)
    {:ok, _} = check_month(m)
    {:ok, _} = check_gDay2(dTZ)
  end

  defp check_gDay(~c"---" ++ value) do
    check_gDay2(value)
  end
  ...
  def facet_fun(type, f) do
    fn _X_ ->
      :error_logger.warning_msg(~c"~w: not valid facet on ~p ~p~n", [:xmerl_xsd_type, type, f])
    end
  end
  ...
  defp whiteSpace_fun(_Type, ~c"preserve") do
    fn val ->
      {:ok, val}
    end
  end

  defp whiteSpace_fun(_Type, ~c"replace") do
    fn val ->
      {:ok, :xmerl_xsd_type.replace_ws(val, [])}
    end
  end

  defp whiteSpace_fun(_Type, ~c"collapse") do
    fn val ->
  ...
  def compare_floats(f1, f2) when f1 == ~c"NaN" or f2 == ~c"NaN" do
  ...
  def compare_floats(f1, f2) when f1 == ~c"INF" or f2 == ~c"-INF" do
    :gt
  end
  ...
  defp str_to_float(string) do
    ...
    case :string.tokens(str, ~c".") do
      [b, dE] ->
        case :string.tokens(dE, ~c"Ee") do
          [d, e] ->
      ...
      [b] ->
        case :string.tokens(str, ~c"Ee") do
          [i, e] ->
            {sign, :erlang.list_to_integer(i), ~c"0", :erlang.list_to_integer(e)}

          _ ->
            {sign, :erlang.list_to_integer(b), ~c"0", 0}
        end
    end
  end
  ...
  defp dateTime_atoms(s, sign) do
    [date, timeZone] = :string.tokens(s, ~c"T")
    [yY, mM, dD] = :string.tokens(date, ~c"-")

    {zone, zoneSign, [hour, min, sec]} =
      case :lists.reverse(timeZone) do
        ~c"Z" ++ _ ->
          {~c"Z", :pos, :string.tokens(timeZone, ~c"Z:")}

        _ ->
          zS = zone_sign(timeZone)

          case :string.tokens(timeZone, ~c"-+") do
            [time, z] ->
              {z, zS, :string.tokens(time, ~c":")}

            [time] ->
              {:none, zS, :string.tokens(time, ~c":")}
          end
      end
  ...
  defp zone_atoms(_Sign, ~c"Z") do
    {:pos, 0, 0}
  end

  defp zone_atoms(sign, zone) when is_list(zone) do
    case :string.tokens(zone, ~c":") do
      [h, m] ->
