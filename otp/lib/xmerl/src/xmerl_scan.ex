defmodule :m_xmerl_scan do
  ...
  def file(f, options) do
    extCharset = (case (:lists.keysearch(:encoding, 1,
                                           options)) do
                    {:value, {_, val}} ->
                      val
                    false ->
  ...
  defp int_file(f, options, _ExtCharset) do
    case (:file.read_file(f)) do
      {:ok, bin} ->
        int_string(:erlang.binary_to_list(bin), options,
                     :filename.dirname(f), f)
      error ->
        error
    end
  end
  ...
  defp int_file_decl(f, options, _ExtCharset) do
    case (:file.read_file(f)) do
      {:ok, bin} ->
        int_string_decl(:erlang.binary_to_list(bin), options,
                          :filename.dirname(f), f)
      error ->
        error
    end
  ...
  def string(str, options) do
    {res, tail, s = r_xmerl_scanner(close_fun: close)} = int_string(str,
                                                        options,
                                                        :file_name_unknown)
    close.(s)
    {res, tail}
  ...
  defp int_string(str, options, xMLBase, fileName) do
    ...
    case (:xmerl_lib.detect_charset(r_xmerl_scanner(s, :encoding),
                                      str)) do
  ...
  defp initial_state0(options, xMLBase) do
    ...
    initial_state(options,
                    r_xmerl_scanner(event_fun: &event/2, hook_fun: &hook/2,
                        acc_fun: &acc/3, fetch_fun: &fetch/2,
                        close_fun: &close/1, continuation_fun: &cont/3,
                        rules_read_fun: &rules_read/3,
                        rules_write_fun: &rules_write/4,
                        rules_delete_fun: &rules_delete/3, xmlbase: xMLBase,
  ...
  defp initial_state([{:rules, read, write, rS} | t], s) do
    s1 = rules_state(rS,
                       r_xmerl_scanner(s, rules_read_fun: read,  rules_write_fun: write, 
                              keep_rules: true))
    initial_state(t, s1)
  end
  ...
  defp initial_state([{:validation, f} | t], s) when f == :off or
  ...
  defp initial_state([{:quiet, f} | t], s) when f == true or
                                       f == false do
  ...
  defp initial_state([{:allow_entities, f} | t], s) when f == true or
                                                f == false do
    initial_state(t, r_xmerl_scanner(s, allow_entities: f))
  end
  ...
  defp fetch_URI(uRI, s) do
    ...
    path = path_locate(r_xmerl_scanner(s, :fetch_path), filename,
                         fullname)
    :no_debug
    {:ok, path, s}
  end
  ...
  defp scan_document(str0,
            s = r_xmerl_scanner(event_fun: event, line: l, col: c,
                    environment: env, encoding: charset, document: document,
                    validation: validateResult)) do
    s1 = event.(r_xmerl_event(event: :started, line: l, col: c,
                    data: :document),
                  s)
    str = (cond do
             charset == 'utf-8' ->
               str0
             charset !== :undefined ->
               :xmerl_ucs.to_unicode(str0,
                                       :erlang.list_to_atom(charset))
             true ->
               str0
           end)
    {prolog, pos, t1, s2} = scan_prolog(str, s1,
                                          _StartPos = 1)
    t2 = scan_mandatory('<', t1, 1, s2,
                          :expected_element_start_tag)
    {res, t3, s3} = scan_element(t2, s2, pos)
    {misc, _Pos1, tail, s4} = scan_misc(t3, s3, pos + 1)
    s5 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s4, :line),
                           col: r_xmerl_scanner(s4, :col), data: :document),
                         s4))
    {res2, s6} = (case (validation_mode(validateResult)) do
                    :off ->
                      {res, cleanup(s5)}
                        ...
                        {:EXIT, {:error, reason}} ->
                          ...
                          cond do
                            r_xmerl_scanner(s5b, :quiet) ->
                              :ok
                            true ->
                              :error_logger.error_msg('~p- fatal: ~p~n',
                                                        [603,
                                                             {:failed_validation,
                                                                reason}])
                              :ok
    ...
    res3 = (case (document) do
              true ->
                content = :lists.reverse(prolog,
  ...
  defp scan_decl(str,
            s = r_xmerl_scanner(event_fun: event, line: l, col: c,
                    environment: _Env, encoding: _Charset,
                    validation: _ValidateResult)) do
    s1 = event.(r_xmerl_event(event: :started, line: l, col: c,
                    data: :document),
                  s)
    case (scan_prolog(str, s1, _StartPos = 1)) do
      ...
      {_, _, [], s2} ->
        {[], [], s2}
      {_, _, t2, s2} ->
        {_, _, s3} = scan_content(t2, s2, [], _Attrs = [],
                                    r_xmerl_scanner(s2, :space), _Lang = [], _Parents = [],
                                    r_xmlNamespace())
        {t2, [], s3}
    end
  end
  ...
  defp scan_prolog('<?xml' ++ t,
            s0 = r_xmerl_scanner(encoding: charset0, col: col, line: l), pos, acc)
      when hd(t) == 32 or hd(t) == 13 or hd(t) == 10 or
             hd(t) == 9 do
    {charset, t3, s3} = (cond do
                           (col == 1 and l == 1 and
                           ...
                           true ->
                             cond do
                               ...
                               true ->
                                 :error_logger.error_msg('~p- fatal: ~p~n',
                                                           [695,
                                                                {:xml_declaration_must_be_first_in_doc,
                                                                   col, l}])
                                 :ok
                             end
                             fatal({:xml_declaration_must_be_first_in_doc, col,
                                      l},
                                     s0)
                         end)
    cond do
  ...
  defp scan_prolog('<!DOCTYPE' ++ t,
            s0 = r_xmerl_scanner(environment: :prolog, encoding: _Charset), pos,
            acc) do
    :no_debug
    :no_debug
  ...
  defp scan_prolog(str = '%' ++ _T,
            s = r_xmerl_scanner(environment: {:external, _}), pos, acc) do
    {t, s1} = scan_ext_subset(str, s)
    {acc, pos, t, s1}
  end

  defp scan_prolog(str,
            s0 = r_xmerl_scanner(user_state: _US, encoding: _Charset), pos,
            acc) do
    :no_debug
    :no_debug
  ...
  defp scan_prolog2('<!DOCTYPE' ++ t, s0 = r_xmerl_scanner(environment: :prolog), pos,
            acc) do
    :no_debug
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 9)
  ...
  defp scan_misc('<!--' ++ t, s0 = r_xmerl_scanner(acc_fun: f, comments: cF), pos,
            acc) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 4)
    {c, t1, s1} = scan_comment(t, s, pos, _Parents = [],
                                 _Lang = [])
    case (cF) do
      true ->
        {acc2, pos2, s3} = (case (f.(c, acc, s1)) do
                              {acc1, s2} ->
                                {acc1, pos + 1, s2}
                              {acc1, pos1, s2} ->
                                {acc1, pos1, s2}
  ...
  defp scan_misc('<?' ++ t, s0 = r_xmerl_scanner(acc_fun: f), pos, acc) do
    ...
    {acc2, pos2, s3} = (case (f.(pI, acc, s1)) do
                          {acc1, s2} ->
                            {acc1, pos + 1, s2}
                          {acc1, pos1, s2} ->
                            {acc1, pos1, s2}
  ...
  defp scan_misc(t = [h | _T], s, pos, acc) when h == 32 or
  ...
  defp scan_xml_decl(t, s) do
    ...
    attr = r_xmlAttribute(name: :version, parents: [{:xml, _XMLPos = 1}],
               value: vsn)
  ...
  defp scan_xml_decl(_T, s = r_xmerl_scanner(event_fun: _Event), _Decl) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [878, preformat([:expected, :one, :"of:"],
                                                    [:"?>", :whitespace_character],
                                                    ',')])
        :ok
    end
    fatal(preformat([:expected, :one, :"of:"],
                      [:"?>", :whitespace_character], ','),
            s)
  end
  ...
  defp scan_xml_decl2('encoding' ++ t, s0 = r_xmerl_scanner(event_fun: event),
            decl0 = r_xmlDecl(attributes: attrs)) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 8)
    {t1, s1} = scan_eq(t, s)
    {encName, t2, s2} = scan_enc_name(t1, s1)
    lowEncName = :xmerl_lib.to_lower(encName)
    attr = r_xmlAttribute(name: :encoding,
               parents: [{:xml, _XMLPos = 1}], value: lowEncName)
    decl = r_xmlDecl(decl0, encoding: lowEncName, 
                      attributes: [attr | attrs])
    s3 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s0, :line),
                           col: r_xmerl_scanner(s0, :col), data: attr),
                         s2))
    case (t2) do
      '?>' ++ _T3 ->
  ...
  defp scan_xml_decl3('standalone' ++ t, s0 = r_xmerl_scanner(event_fun: event),
            decl0 = r_xmlDecl(attributes: attrs)) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 10)
    {t1, s1} = scan_eq(t, s)
    {stValue, t2, s2} = scan_standalone_value(t1, s1)
    attr = r_xmlAttribute(name: :standalone,
               parents: [{:xml, _XMLPos = 1}], value: stValue)
    decl = r_xmlDecl(decl0, standalone: stValue, 
                      attributes: [attr | attrs])
    s3 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s0, :line),
                           col: r_xmerl_scanner(s0, :col), data: attr),
                         s2))
    {_, t3, s4} = strip(t2, s3)
    t4 = scan_mandatory('?>', t3, 2, s4,
                          :expected_xml_decl_endtag)
    return_xml_decl(t4, r_xmerl_scanner(s4, col: r_xmerl_scanner(s4, :col) + 2), decl)
  end

  defp return_xml_decl(t, s = r_xmerl_scanner(hook_fun: _Hook, event_fun: event),
            decl0 = r_xmlDecl(attributes: attrs)) do
    {_, t1, s1} = strip(t, s)
    decl = r_xmlDecl(decl0, attributes: :lists.reverse(attrs))
    s2 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s, :line),
                           col: r_xmerl_scanner(s, :col), data: decl),
                         s1))
    {decl, t1, s2}
  end
  ...
  defp scan_text_decl(t, s = r_xmerl_scanner(event_fun: event)) do
    {r_xmlDecl(attributes: attrs) = decl0, t1,
       s1} = scan_optional_version(t, s)
    t2 = (case (t1) do
            'encoding' ++ _T2 ->
              _T2
            _ ->
              cond do
                ...
                true ->
                  :error_logger.error_msg('~p- fatal: ~p~n',
                                            [973, :expected_encoding_attribute])
                  :ok
              end
    ...
    lowEncName = :xmerl_lib.to_lower(encName)
    {_, t5, s5} = strip(t4, s4)
    attr = r_xmlAttribute(name: :encoding, parents: [{:xml, 1}],
               value: lowEncName)
    decl = r_xmlDecl(decl0, encoding: lowEncName, 
                      attributes: [attr | attrs])
    s6 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s5, :line),
                           col: r_xmerl_scanner(s5, :col), data: attr),
                         s5))
    scan_text_decl(t5, s6, decl)
  end

  defp scan_text_decl('?>' ++ t,
    ...
    {_, t1, s1} = strip(t, s)
    decl = r_xmlDecl(decl0, attributes: :lists.reverse(attrs))
    s2 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s0, :line),
                           col: r_xmerl_scanner(s0, :col), data: decl),
                         s1))
    {decl, t1, s2}
  end
  ...
  defp scan_text_decl([h | _T], s, _) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [1005,
                                       {:unexpected_character_in_text_declaration,
                                          h}])
        :ok
    end
  ...
  defp scan_optional_version('version' ++ t, s0) do
    ...
    attr = r_xmlAttribute(name: :version, parents: [{:xml, 1}],
               value: vsn)
  ...
  defp scan_enc_name([], s = r_xmerl_scanner(continuation_fun: f)) do
    ...
    f.(fn moreBytes, s1 ->
         ...
         fatal_fun(:expected_encoding_name), s)
  end
  ...
  defp scan_enc_name([h | t], s0, delim, acc) when (h >= ?a and
                                           h <= ?z) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
  ...
  defp scan_enc_name([h | _T], s, _Delim, _Acc) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [1043, {:error,
                                            {:unexpected_character_in_Enc_Name,
                                               h}}])
        :ok
    end
    fatal({:error, {:unexpected_character_in_Enc_Name, h}},
            s)
  end
  ...
  defp scan_enc_name2([h | t], s0, delim, acc) when (h >= ?a and
                                           h <= ?z) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    scan_enc_name2(t, s, delim, [h | acc])
  ...
  defp scan_enc_name2([h | t], s0, delim, acc) when (h >= ?0 and
                                           h <= ?9) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    scan_enc_name2(t, s, delim, [h | acc])
  ...
  defp scan_enc_name2([h | t], s0, delim, acc) when h == ?. or
                                          h == ?_ or h == ?- do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    scan_enc_name2(t, s, delim, [h | acc])
  ...
  defp scan_pi(str = [h1, h2, h3 | t], s0 = r_xmerl_scanner(line: l, col: c),
            pos, ps)
  ...
  defp scan_wellknown_pi(str, s, _Pos, _Ps) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [1130, {:invalid_target_name,
                                            :lists.sublist(str, 1, 10)}])
        :ok
    end
    fatal({:invalid_target_name,
             :lists.sublist(str, 1, 10)},
            s)
  end

  defp scan_pi([], s = r_xmerl_scanner(continuation_fun: f), target, l, c,
            pos, ps, acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
  ...
  defp scan_pi('?>' ++ t,
            s0 = r_xmerl_scanner(hook_fun: hook, event_fun: event), target, l, c,
            pos, ps, acc) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 2)
    pI = r_xmlPI(name: target, parents: ps, pos: pos,
             value: :lists.reverse(acc))
    s1 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: l, col: c,
                           data: pI),
                         s))
    {ret, s2} = hook.(pI, s1)
    {ret, t, s2}
  ...
  defp scan_pi([h | _T], s, _Target, _L, _C, _Pos, _Ps,
            _Acc) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [1159, {:expected_whitespace_OR_end_of_PI,
                                            {:char, h}}])
        :ok
    end
    fatal({:expected_whitespace_OR_end_of_PI, {:char, h}},
            s)
  end

  defp scan_pi2([], s = r_xmerl_scanner(continuation_fun: f), target, l, c,
            pos, ps, acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
  ...
  defp scan_doctype2(_T, s, _DTD) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [1244, :expected_end_of_DOCTYPE_declaration])
        :ok
    end
  ...
  defp fetch_DTD(dTDSpec, s) do
    case (fetch_and_parse(dTDSpec, s,
                            [{:text_decl, true}, {:environment,
                                                    {:external, :subset}}])) do
  ...
  defp fetch_and_parse(extSpec,
            s = r_xmerl_scanner(fetch_fun: fetch, rules: rules, xmlbase: xMLBase),
            options0) do
    retS = (case (fetch.(extSpec, s)) do
              {:ok, newS} ->
                newS
              {:ok, :not_fetched, newS} ->
                newS
              {:ok, dataRet,
                 newS = r_xmerl_scanner(fetch_path: fetchPath, user_state: uState,
                            event_fun: event, hook_fun: hook, fetch_fun: fetch1,
                            close_fun: close1, continuation_fun: cont,
                            acc_fun: acc, rules_read_fun: read,
                            rules_write_fun: write, validation: valid,
                            quiet: quiet, encoding: charset)} ->
                evS = event_state(newS)
                hoS = hook_state(newS)
                feS = fetch_state(newS)
                coS = cont_state(newS)
                options = options0 ++ [{:fetch_path, fetchPath},
                                           {:user_state, uState}, {:rules,
                                                                     rules},
                                                                      {:event_fun,
                                                                         event,
                                                                         evS},
    ...
    case (retS) do
      r_xmerl_scanner() ->
        r_xmerl_scanner(retS, text_decl: false, 
    ...
    case (fetch.(extSpec, s)) do
      {:ok, :not_fetched, _NewS} ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1371, {:error_fetching_external_source,
                                                extSpec}])
            :ok
        end
        fatal({:error_fetching_external_source, extSpec}, s)
      {:ok, dataRet, newS} ->
        {string, locationName} = (case (dataRet) do
                                    {:file, f} ->
                                      {get_file(f, s), f}
                                    {:string, str} ->
      ...
      _ ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1385, {:error_fetching_external_resource,
                                                extSpec}])
            :ok
        end
    ...
    case (:ets.match(tab,
      ...
      [l] when is_list(l) ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1412, {:error_missing_declaration_in_DTD,
                                                hd(l)}])
            :ok
        end
      ...
      err ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1414, {:error_missing_declaration_in_DTD,
                                                err}])
            :ok
        end
    ...
    case ((try do
      ...
      err ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1433, {:error_missing_declaration_in_DTD,
                                                err}])
            :ok
        end
        fatal({:error_missing_declaration_in_DTD, err}, s)
    ...
    case (:lists.keysearch(:ID, 2, rest)) do
      {:value, att2} ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1446, {:error_more_than_one_ID_def, n1,
                                                :erlang.element(1, att2)}])
            :ok
        end
        fatal({:error_more_than_one_ID_def, n1,
                 :erlang.element(1, att2)},
                s)
      _ ->
        :ok
  ...
  defp check_attributes([{_, {:enumeration, _}, _, _, _} = attr | t],
            s) do
    ...
    case (:ets.match(tab, {{:entity, :"$1"}, :undeclared})) do
      ...
      [l] when is_list(l) ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1469, {:error_missing_declaration_in_DTD,
                                                hd(l)}])
            :ok
        end
        fatal({:error_missing_declaration_in_DTD, hd(l)}, s)
      err ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1471, {:error_missing_declaration_in_DTD,
                                                err}])
            :ok
        end
        fatal({:error_missing_declaration_in_DTD, err}, s)
    ...
    case (:ets.match(tab, {{:id, :"$1"}, :undeclared})) do
      ...
      [l] when is_list(l) ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1487, {:error_missing_declaration_in_DTD,
                                                hd(l)}])
            :ok
        end
        fatal({:error_missing_declaration_in_DTD, hd(l)}, s)
      err ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1489, {:error_missing_declaration_in_DTD,
                                                err}])
            :ok
        end
        fatal({:error_missing_declaration_in_DTD, err}, s)
  ...
  defp scan_ext_subset(t, s) when hd(t) == 32 or hd(t) == 13 or
  ...
  defp scan_decl_sep(t, s) do
    {pERefName, t1, s1} = scan_pe_reference(t, s)
    {expandedRef,
       s2} = (case (expand_pe_reference(pERefName, s1,
                                          :as_PE)) do
  ...
  defp scan_conditional_sect('IGNORE' ++ t, s0) do
    ...
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 6)
    {_, t1, s1} = strip(t, s)
    t2 = scan_mandatory('[', t1, 1, s,
                          :expected_IGNORE_bracket)
    {_, t3, s3} = strip(t2, s1)
    scan_ignore(t3, s3)
  end
  ...
  defp scan_markup_decl('<!ELEMENT' ++ t,
            r_xmerl_scanner(rules_read_fun: read, rules_write_fun: write,
                rules_delete_fun: delete) = s0) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 9)
    {_, t1, s1} = mandatory_strip(t, s)
    {ename, _NamespaceInfo, t2, s2} = scan_name(t1, s1)
    element = (case (read.(:elem_def, ename, s2)) do
    ...
    s7 = write.(:elem_def, ename,
                  r_xmlElement(element, name: ename,  content: edef, 
  ...
  defp scan_markup_decl('<!ENTITY' ++ t, s0) do
    case (r_xmerl_scanner(s0, :allow_entities)) do
      false ->
        cond do
          r_xmerl_scanner(s0, :quiet) ->
            :ok
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [1698, {:error, :entities_not_allowed}])
            :ok
        end
  ...
  defp scan_markup_decl('<!ATTLIST' ++ t,
            r_xmerl_scanner(rules_read_fun: read, rules_write_fun: write,
                rules_delete_fun: delete) = s0) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 9)
    ...
    {eDEF, mergedAttrs} = (case (read.(:elem_def, ename,
                                         s4)) do
                             :undefined ->
  ...
  defp scan_markup_decl(_Str, s) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n', [1739, :expected_markup])
        :ok
    end
  ...
  defp update_attributes1([a = {name, _Type, _DefaultV, _DefaultD, _Env} |
               attrs],
            oldAttrs) do
  ...
  defp scan_attdef2(t, s, attrs) do
    ...
    {_, t4, s4} = mandatory_strip(t3, s3)
    {{defaultDecl, defaultValue}, t5,
       s5} = scan_default_decl(t4, s4, attType)
    {_, t6, s6} = strip(t5, s5)
    attr = {attName, attType, defaultValue, defaultDecl,
              r_xmerl_scanner(s, :environment)}
  ...
  defp scan_att_type('NOTATION' ++ t, s0) do
    ...
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 8)
    {_, t1, s1} = mandatory_strip(t, s)
    t2 = scan_mandatory('(', t1, 1, s1,
                          :expected_parenthesis_after_NOTATION)
    s2 = s1
    {_, t3, s3} = strip(t2, s2)
  ...
  defp notation_exists(name,
            r_xmerl_scanner(rules_read_fun: read, rules_write_fun: write) = s) do
  ...
  defp scan_entity('%' ++ t, r_xmerl_scanner(rules_write_fun: write) = s0) do
    ...
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    {_, t1, s1} = mandatory_strip(t, s)
    {pEName, _NamespaceInfo, t2,
       s2} = scan_name_no_colons(t1, s1)
    {_, t3, s3} = mandatory_strip(t2, s2)
  ...
  defp scan_entity(t,
            r_xmerl_scanner(rules_write_fun: write, rules_read_fun: read,
                rules_delete_fun: delete) = s) do
    {eName, _NamespaceInfo, t1, s1} = scan_name_no_colons(t,
                                                            s)
    {_, t2, s2} = mandatory_strip(t1, s1)
    ...
    s6 = write.(:entity, eName,
                  {r_xmerl_scanner(s5, :environment), entType, eDef}, s5)
    {t5, s6}
  end
  ...
  defp scan_entity_def('\'' ++ t, s0, eName) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    {eVal, t1, s1} = scan_entity_value(t, s, ?', eName,
                                         :general)
    {eVal, :internal, t1, s1}
  end
      ...
      _ ->
        case (fetch_and_parse(extID, s2,
          ...
          {entity, _Tail, sx} ->
            ...
            {entity, :external, t2,
               r_xmerl_scanner(s2, entity_references: oldRef ++ newRef)}
          {:error, :enoent} ->
  ...
  defp scan_ndata_decl2('NDATA' ++ t,
            s0 = r_xmerl_scanner(rules_read_fun: read, rules_write_fun: write)) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 5)
    {_, t1, s1} = mandatory_strip(t, s)
    {name, _NamespaceInfo, t2, s2} = scan_name(t1, s1)
  ...
  defp scan_element(t, s, pos) do
    scan_element(t, s, pos, r_xmerl_scanner(s, :space), _Lang = [],
                   _Parents = [], r_xmlNamespace())
  end

  defp scan_element(t, s = r_xmerl_scanner(line: l, col: c), pos, spaceDefault,
            lang, parents, nS) do
    {name, namespaceInfo, t1, s1} = scan_name(t, s)
    vc_Element_valid(name, namespaceInfo, s)
    {_, t2, s2} = strip(t1, s1)
    scan_element(t2, s2, pos, name, l, c, _Attrs = [], lang,
                   parents, namespaceInfo, nS, spaceDefault)
  end

  defp scan_element('/', s = r_xmerl_scanner(continuation_fun: f), pos, name,
            startL, startC, attrs, lang, parents, nSI, nS,
            spaceDefault) do
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_element('/' ++ moreBytes, s1, pos, name, startL,
                           startC, attrs, lang, parents, nSI, nS, spaceDefault)
       end,
         fatal_fun(:unexpected_end), s)
  end
  ...
  defp scan_element([], s = r_xmerl_scanner(continuation_fun: f), pos, name,
    ...
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_element(moreBytes, s1, pos, name, startL, startC,
                           attrs, lang, parents, nSI, nS, spaceDefault)
       end,
         fatal_fun(:unexpected_end), s)
  end

  defp scan_element('/>' ++ t,
            s0 = r_xmerl_scanner(hook_fun: hook, event_fun: event, line: l,
                     col: c, xmlbase_cache: xMLBase),
            pos, name, _StartL, _StartC, attrs0, lang, parents, nSI,
            namespace, _SpaceDefault) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 2)
    attrs = :lists.reverse(attrs0)
    e = processed_whole_element(s, pos, name, attrs, lang,
                                  parents, nSI, namespace)
  ...
  defp scan_element('>' ++ t,
            s0 = r_xmerl_scanner(event_fun: event, hook_fun: hook, line: l,
                     col: c, xmlbase_cache: xMLBase, space: spaceOption),
            pos, name, startL, startC, attrs0, lang, parents, nSI,
            namespace, spaceDefault) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    attrs = :lists.reverse(attrs0)
    e0 = processed_whole_element(s, pos, name, attrs, lang,
                                   parents, nSI, namespace)
    r_xmlElement(attributes: attrs1) = e0
    wfc_unique_att_spec(attrs1, s)
    xMLSpace = (case (:lists.keysearch(:"xml:space", r_xmlAttribute(:name),
                                         attrs1)) do
                  false ->
                    spaceDefault
    ...
    ^e0 = processed_whole_element(s, pos, name, attrs1,
                                    lang, parents, nSI, namespace)
    s1 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :started, line: startL,
                           col: startC, data: e0),
                         s))
    {content, t1, s2} = scan_content(t, s1, name, attrs1,
                                       xMLSpace, r_xmlElement(e0, :language),
                                       [{name, pos} | parents], namespace)
    element = r_xmlElement(e0, content: content, 
                      xmlbase: r_xmlElement(e0, :xmlbase))
    s3 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: l, col: c,
                           data: element),
                         s2))
  ...
  defp scan_element(t, s, pos, name, startL, startC, attrs, lang,
            parents, nSI, nS, spaceDefault) do
    {attName, namespaceInfo, t1, s1} = scan_name(t, s)
    {t2, s2} = scan_eq(t1, s1)
    {attType, _DefaultDecl} = get_att_type(s2, attName,
                                             name)
    {attValue, t3a, s3a, isNorm} = scan_att_value(t2, s2,
                                                    attType)
    newNS = check_namespace(attName, namespaceInfo,
                              attValue, nS)
    {t3, s3} = wfc_whitespace_betw_attrs(t3a, s3a)
    {_, t4, s4} = strip(t3, s3)
    attrPos = (case (attrs) do
                 [] ->
                   1
                 [r_xmlAttribute(pos: p) | _] ->
                   p + 1
               end)
    attr = r_xmlAttribute(name: attName,
               parents: [{name, pos} | parents], pos: attrPos,
               language: lang, nsinfo: namespaceInfo, value: attValue,
               normalized: isNorm)
    xMLBase = (cond do
                 attName == :"xml:base" ->
    ...
    s5 = event.(r_xmerl_event(event: :ended, line: line, col: col,
                    data: attr),
                  r_xmerl_scanner(s4, xmlbase: xMLBase,  xmlbase_cache: r_xmerl_scanner(s, :xmlbase)))
    scan_element(t4, s5, pos, name, startL, startC,
                   [attr | attrs], lang, parents, nSI, newNS, spaceDefault)
  end
  ...
  defp get_att_type(s = r_xmerl_scanner(rules_read_fun: read), attName,
            elemName) do
    case (read.(:elem_def, elemName, s)) do
      r_xmlElement(attributes: attrs) ->
  ...
  defp processed_whole_element(s = r_xmerl_scanner(hook_fun: _Hook, xmlbase: xMLBase,
                  line: _L, col: _C, event_fun: _Event),
            pos, name, attrs, lang, parents, nSI, namespace) do
    language = check_language(attrs, lang)
    allAttrs = (case (r_xmerl_scanner(s, :default_attrs)) do
                  true ->
                    defaultAttrs = (for {attName,
                                           attValue} <- get_default_attrs(s,
                                                                            name),
                                          attValue !== :no_value,
                                          not
                                          :lists.keymember(attName, r_xmlAttribute(:name),
                                                             attrs) do
                                      r_xmlAttribute(name: attName,
                                          parents: [{name, pos} | parents],
                                          language: lang, nsinfo: nSI,
                                          namespace: namespace, value: attValue,
                                          normalized: true)
                                    end)
    ...
    {expName,
       expAttrs} = (case (r_xmerl_scanner(s, :namespace_conformant)) do
                      true ->
                        tempNamespace = r_xmlNamespace(namespace, default: [])
                        expAttrsX = (for a <- allAttrs do
                                       r_xmlAttribute(a, namespace: namespace, 
                                              expanded_name: expanded_name(r_xmlAttribute(a, :name),
    ...
    r_xmlElement(name: name, xmlbase: xMLBase, pos: pos,
        parents: parents, attributes: expAttrs,
        language: language, expanded_name: expName, nsinfo: nSI,
        namespace: namespace)
  end
  ...
  defp check_namespace(_, {'xmlns', prefix}, value, nS = r_xmlNamespace(nodes: ns)) do
    r_xmlNamespace(nS, nodes: keyreplaceadd(prefix, 1, ns,
                                 {prefix, :erlang.list_to_atom(value)}))
  end
  ...
  defp expanded_name(name, [], r_xmlNamespace(default: uRI), s) do
    case (uRI) do
      :"http://www.w3.org/XML/1998/namespace" ->
        ...
        fatal(:cannot_bind_default_namespace_to_xml_namespace_name,
                s)
      :"http://www.w3.org/2000/xmlns/" ->
        ...
        fatal(:cannot_bind_default_namespace_to_xmlns_namespace_name,
                s)
      _ ->
        {uRI, name}
    end
    ...
    case (name) do
      ...
      :"xmlns:xml" when value !== :"http://www.w3.org/XML/1998/namespace" ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2328, {:xml_prefix_cannot_be_redeclared,
                                                value}])
            :ok
        end
      ...
      :"xmlns:xmlns" ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2330, {:xmlns_prefix_cannot_be_declared,
                                                value}])
            :ok
        end
          ...
          :"http://www.w3.org/XML/1998/namespace" ->
            cond do
              ...
              true ->
                :error_logger.error_msg('~p- fatal: ~p~n',
                                          [2334,
                                               {:cannot_bind_prefix_to_xml_namespace,
                                                  local}])
                :ok
            end
          ...
          :"http://www.w3.org/2000/xmlns/" ->
            cond do
              ...
              true ->
                :error_logger.error_msg('~p- fatal: ~p~n',
                                          [2336,
                                               {:cannot_bind_prefix_to_xmlns_namespace,
                                                  local}])
                :ok
            end
            fatal({:cannot_bind_prefix_to_xmlns_namespace, local},
                    s)
          _ ->
            n
        end
    ...
    case (:lists.keysearch(prefix, 1, ns)) do
      ...
      false ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2350, {:namespace_prefix_not_declared,
                                                prefix}])
            :ok
        end
  ...
  defp scan_att_value('%' ++ _T, s = r_xmerl_scanner(environment: :prolog),
            _AttType) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
  ...
  defp scan_att_value('%' ++ t,
    ...
    {expandedRef, s2} = (case (expand_pe_reference(name, s1,
                                                     :in_literal)) do
                           tuple when is_tuple(tuple) ->
                             {expRef, sx} = fetch_not_parse(tuple, s1)
                             {entV, _, _S2} = scan_entity_value(expRef, sx,
                                                                  :no_delim,
                                                                  name,
                                                                  :parameter)
  ...
  defp scan_att_value([h | t], s0, :CDATA = aT) when h == ?" or
                                           h == ?' do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
  ...
  defp scan_att_chars([], s = r_xmerl_scanner(continuation_fun: f), h, acc, tmpAcc,
            aT, isNorm) do
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_att_chars(moreBytes, s1, h, acc, tmpAcc, aT,
                             isNorm)
       end,
         fatal_fun(:unexpected_end), s)
  end
  ...
  defp scan_att_chars([h | t], s0, h, acc, tmpAcc, attType, isNorm) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    check_att_default_val(r_xmerl_scanner(s, :validation), tmpAcc,
                            attType, s)
    {acc2, s2, isNorm2} = (cond do
                             attType == :CDATA ->
                               {acc, s, isNorm}
                             true ->
  ...
  defp scan_att_chars('&' ++ t, s0, delim, acc, tmpAcc, aT, isNorm) do
    ...
    case (markup_delimeter(expRef)) do
      true ->
        scan_att_chars(t1, s1, delim, [expRef | acc],
                         [expRef | tmpAcc], aT, isNorm)
      _ ->
        case (t) do
          '#' ++ _ ->
            {t2, s2, isNorm2} = (cond do
            ...
            scan_att_chars(t2, s2, delim, expRef ++ acc, tmpAcc, aT,
                             isNorm2)
          _ ->
            ch = string_to_char_set(r_xmerl_scanner(s, :encoding), expRef)
            scan_att_chars(ch ++ t1, s1, delim, acc, tmpAcc, aT,
                             isNorm)
        end
    end
  end
  ...
  defp scan_att_chars('<' ++ _T, s0, _Delim, _Acc, _, _, _) do
    cond do
      r_xmerl_scanner(s0, :quiet) ->
        :ok
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n', [2445, :unexpected_char])
        :ok
    end
    fatal(:unexpected_char, s0)
  end

  defp scan_att_chars([h | t], s0, delim, acc, _TmpAcc, :CDATA,
            isNorm)
      when h == 32 or h == 13 or h == 10 or h == 9 do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    scan_att_chars(t, s, delim, [?\s | acc], [], :CDATA,
                     isNorm)
  end
  ...
  defp scan_att_chars([h | t], s0, delim, acc, tmpAcc, aT, isNorm)
    ...
    scan_att_chars(t1, s1, delim, [?\s | acc], [], aT,
                     isNorm2)
  end
  ...
  defp scan_att_chars(str, s0, delim, acc, tmpAcc, aT, isNorm) do
    ...
    scan_att_chars(t, s, delim, [ch | acc], [ch | tmpAcc],
                     aT, isNorm)
  end
    ...
    case (:xmerl_lib.is_letter(hd(name))) do
      ...
      _ ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2481, {:illegal_first_character, ent,
                                                name}])
            :ok
        end
    ...
    case (:xmerl_lib.is_letter(hd(name))) do
      ...
      _ ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2489, {:illegal_first_character, iDR,
                                                name}])
            :ok
        end
  ...
  defp check_att_default_val(name, :ID,
            s = r_xmerl_scanner(rules_write_fun: write, rules_read_fun: read,
                    rules_delete_fun: delete)) do
    case (:xmerl_lib.is_name(name)) do
      false ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2498, {:ID_names_must_be_Name_production,
                                                name}])
            :ok
        end
  ...
  defp valid_Char(:dtd, aT, c, s) when aT == :NMTOKEN or
                                 aT == :NMTOKENS do
  ...
  defp scan_content(t, s, name, attrs, space, lang, parents, nS) do
    scan_content(t, s, _Pos = 1, name, attrs, space, lang,
                   parents, nS, _Acc = [], _MarkupDel = [])
  end

  defp scan_content('<', s = r_xmerl_scanner(continuation_fun: f), pos, name, attrs,
            space, lang, parents, nS, acc, _) do
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_content('<' ++ moreBytes, s1, pos, name, attrs,
  ...
  defp scan_content([],
            s = r_xmerl_scanner(environment: {:external, {:entity, _}}), _Pos,
            _Name, _Attrs, _Space, _Lang, _Parents, _NS, acc, _) do
    {:lists.reverse(acc), [], s}
  end

  defp scan_content([], s = r_xmerl_scanner(environment: :internal_parsed_entity),
            _Pos, _Name, _Attrs, _Space, _Lang, _Parents, _NS, acc,
            _) do
    {:lists.reverse(acc), [], s}
  end

  defp scan_content([], s = r_xmerl_scanner(continuation_fun: f), pos, name,
            attrs, space, lang, parents, nS, acc, _) do
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_content(moreBytes, s1, pos, name, attrs, space,
                           lang, parents, nS, acc, [])
       end,
         fatal_fun(:unexpected_end), s)
  end

  defp scan_content('</' ++ t, s0, _Pos, name, _Attrs, _Space, _Lang,
            _Parents, _NS, acc, []) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 2)
    ...
    cond do
      ...
      true ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2570, {:endtag_does_not_match,
                                                {:was, eTagName,
                                                   :should_have_been, name}}])
            :ok
        end
        fatal({:endtag_does_not_match,
                 {:was, eTagName, :should_have_been, name}},
                s)
    end
    {_, t2, s2} = strip(t1, s1)
    case (t2) do
  ...
  defp scan_content([?& | _T] = str,
            r_xmerl_scanner(environment: {:external, {:entity, eName}}) = s0, pos,
            name, attrs, space, lang, parents, nS, acc, _) do
    {_EntV, t1, s1} = scan_entity_value(str, s0, [], eName,
                                          :general)
    scan_content(t1, s1, pos, name, attrs, space, lang,
                   parents, nS, acc, [])
  end

  defp scan_content('&' ++ t,
            r_xmerl_scanner(environment: :internal_parsed_entity) = s, pos, name,
            attrs, space, lang, parents, nS, acc, _) do
  ...
  defp scan_content('&' ++ t, s0, pos, name, attrs, space, lang,
            parents, nS, acc, []) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    {expRef, t1, s1} = scan_reference(t, s)
    case (markup_delimeter(expRef)) do
      true ->
        scan_content(expRef ++ t1, s1, pos, name, attrs, space,
                       lang, parents, nS, acc, expRef)
      _ ->
        scan_content(string_to_char_set(r_xmerl_scanner(s1, :encoding),
                                          expRef) ++ t1,
                       s1, pos, name, attrs, space, lang, parents, nS, acc, [])
    end
  end

  defp scan_content('<!--' ++ t, s0 = r_xmerl_scanner(acc_fun: f, comments: cF), pos,
            name, attrs, space, lang, parents, nS, acc, []) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 4)
    ...
    case (cF) do
      true ->
        ...
        scan_content(t1, s3, pos2, name, attrs, space, lang,
                       parents, nS, acc2, [])
      false ->
        scan_content(t1, s1, pos, name, attrs, space, lang,
                       parents, nS, acc, [])
  ...
  defp scan_content('<' ++ t, s0, pos, name, attrs, space, lang,
    ...
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    {markup, t1, s1} = scan_content_markup(t, s, pos, name,
                                             attrs, space, lang, parents, nS)
    accF = r_xmerl_scanner(s1, :acc_fun)
    {newAcc, newPos, newS} = (case (accF.(markup, acc,
                                            s1)) do
                                {acc2, s2} ->
                                  {acc2, pos + 1, s2}
                                {acc2, pos2, s2} ->
  ...
  defp scan_content([_H | t],
            s = r_xmerl_scanner(environment: {:external, {:entity, _}}), pos,
            name, attrs, space, lang, parents, nS, acc, _) do
    scan_content(t, s, pos, name, attrs, space, lang,
                   parents, nS, acc, [])
  end

  defp scan_content(t,
            s = r_xmerl_scanner(acc_fun: f, event_fun: event, hook_fun: hook,
                    line: _L),
            pos, name, attrs, space, lang, parents, nS, acc,
            markupDel) do
    text0 = r_xmlText(pos: pos, parents: parents)
    s1 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :started, line: r_xmerl_scanner(s, :line),
                           data: text0),
                         s))
    {data, t1, s2} = scan_char_data(t, s1, space, markupDel)
    text = r_xmlText(text0, value: data)
    {ret, s2b} = hook.(text, s2)
    s3 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s2b, :line),
                           data: ret),
                         s2b))
    {newAcc, newPos, newS} = (case (f.(ret, acc, s3)) do
                                {acc4, s4} ->
                                  {acc4, pos + 1, s4}
                                {acc4, pos4, s4} ->
                                  {acc4, pos4, s4}
  ...
  defp scan_content_markup([], s = r_xmerl_scanner(continuation_fun: f), pos, name,
            attrs, space, lang, parents, nS) do
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_content_markup(moreBytes, s1, pos, name, attrs,
                                  space, lang, parents, nS)
       end,
         fatal_fun(:unexpected_end), s)
  end

  defp scan_content_markup('![CDATA[' ++ t, s0, pos, _Name, _Attrs, _Space, _Lang,
            parents, _NS) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 8)
  ...
  defp scan_content_markup('?' ++ t, s0, pos, _Name, _Attrs, _Space, _Lang,
            parents, _NS) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
  ...
  defp scan_content_markup(t, s, pos, _Name, _Attrs, space, lang, parents,
            nS) do
  ...
  defp scan_char_data([],
            s = r_xmerl_scanner(environment: {:external, {:entity, _}}), _Space,
            _MUD, acc) do
    {:lists.reverse(acc), [], s}
  end

  defp scan_char_data([], s = r_xmerl_scanner(environment: :internal_parsed_entity),
            _Space, _MUD, acc) do
    {:lists.reverse(acc), [], s}
  end

  defp scan_char_data([], s = r_xmerl_scanner(continuation_fun: f), space, _MUD,
            acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
  ...
  defp scan_char_data(t = [h | r], s, space, mUD, acc) when h == 32 or
                                                  h == 13 or h == 10 or
                                                  h == 9 do
    cond do
      (mUD === [] and acc === [] and h === ?\n and
      ...
      true ->
        {newAcc, t1, s1} = accumulate_whitespace(t, s, space,
                                                   acc)
        scan_char_data(t1, s1, space, mUD, newAcc)
    end
  end
  ...
  defp scan_char_data([h1, h2 | _T], s, _Space, _MUD, _Acc)
    ...
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [2726, {:error,
                                            {:not_allowed_to_use_Unicode_noncharacters}}])
        :ok
    end
    fatal({:error,
             {:not_allowed_to_use_Unicode_noncharacters}},
  ...
  defp scan_cdata([], s = r_xmerl_scanner(continuation_fun: f), pos, parents,
            acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
  ...
  defp scan_cdata(']]>' ++ t, s0, pos, parents, acc) do
    ...
    {r_xmlText(pos: pos, parents: parents,
         value: :lists.reverse(acc), type: :cdata),
       t, s}
  end
      ...
      true ->
        cond do
          r_xmerl_scanner(s, :quiet) ->
            :ok
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n', [2778, :invalid_char_ref])
            :ok
        end
      ...
      true ->
        cond do
          r_xmerl_scanner(s, :quiet) ->
            :ok
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n', [2786, :invalid_char_ref])
            :ok
        end
        fatal(:invalid_char_ref, s)
    end
    ...
    case ((try do
      ...
      {:EXIT, _} ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2791, :error_scanning_entity_ref])
            :ok
        end
  ...
  defp scan_entity_ref(t, s) do
    ...
    t2 = scan_mandatory(';', t1, 1, s1,
                          :expected_entity_reference_semicolon)
    s2 = s1
  ...
  defp scan_pe_reference(t, s) do
    {name, _NamespaceInfo, t1, s1} = scan_name(t, s)
    t2 = scan_mandatory(';', t1, 1, s1,
                          :expected_parsed_entity_reference_semicolon)
    ...
    case (read.(:parameter_entity, name, s)) do
      :undefined ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2849, {:unknown_parameter_entity, name}])
            :ok
        end
      ...
      err = {:error, _Reason} ->
        cond do
          r_xmerl_scanner(s, :quiet) ->
            :ok
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n', [2851, err])
            :ok
        end
        fatal(err, s)
      tuple when is_tuple(tuple) ->
  ...
  defp expand_reference(name,
            r_xmerl_scanner(environment: {:external, {:entity, _}})) do
    :erlang.atom_to_list(name)
  end

  defp expand_reference(name,
            r_xmerl_scanner(environment: :internal_parsed_entity)) do
    :erlang.atom_to_list(name)
  end
      ...
      {_, :external, {:error, :enoent}} ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [2883, {:error,
                                                {:entity_target_not_found,
                                                   {:error, :enoent}, name}}])
            :ok
        end
        fatal({:error,
                 {:entity_target_not_found, {:error, :enoent}, name}},
                s)
      ...
      _ ->
        cond do
          r_xmerl_scanner(s, :quiet) ->
            :ok
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n', [2943, :assignment_expected])
            :ok
        end
  ...
  defp scan_name_no_colons(str, s) do
    ...
    case (nSC) do
      true ->
        {target, nSI, t1, s1} = scan_name(str,
  ...
  defp scan_name(str = [?: | t],
            s0 = r_xmerl_scanner(namespace_conformant: nSC)) do
    cond do
      nSC == false ->
        :no_debug
        s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
  ...
  defp scan_name(str, s) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n', [3021, {:invalid_name, str}])
        :ok
    end
  ...
  defp scan_nmtoken(str, s, acc, nSC) do
    scan_nmtoken(str, s, acc, _Prefix = [], _Local = acc,
                   nSC, isLatin1(hd(acc), true))
  end
  ...
  defp scan_nmtoken(str, s) do
    ...
    case (:xmerl_lib.is_namechar(ch)) do
      true ->
        scan_nmtoken(t, r_xmerl_scanner(s, col: r_xmerl_scanner(s, :col) + 1), _Acc = [ch],
                       _Prefix = [], _Local = [ch],
  ...
  defp scan_nmtoken([], s = r_xmerl_scanner(continuation_fun: f), acc, prefix,
            local, nSC, isLatin1) do
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_nmtoken(moreBytes, s1, acc, prefix, local, nSC,
                           isLatin1)
       end,
         fn s1 ->
              {:erlang.list_to_atom(:lists.reverse(acc)),
                 namespace_info(prefix, local), [], s1}
         end,
         s)
  end

  defp scan_nmtoken(str = [h | _], s, acc, prefix, local, _NSC,
            true)
      when h == 32 or h == 13 or h == 10 or h == 9 do
    nmString = :lists.reverse(acc)
    {:erlang.list_to_atom(nmString),
       namespace_info(prefix, local), str, s}
  end

  defp scan_nmtoken(str = [?: | _], s, acc, [], _Local, :no_colons,
            _IsLatin1) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3071, {:invalid_NCName,
                                            :lists.sublist(:lists.reverse(acc) ++ str,
                                                             1, 6)}])
        :ok
    end
    fatal({:invalid_NCName,
             :lists.sublist(:lists.reverse(acc) ++ str, 1, 6)},
            s)
  end
  ...
  defp scan_nmtoken([?: | t], s0, acc, [], local, nSC, isLatin1) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    scan_nmtoken(t, s, [?: | acc], :lists.reverse(local),
                   [], nSC, isLatin1)
  end

  defp scan_nmtoken(str = [?: | _T], s, acc, _Prefix, _Local,
            _NSC = true, _IsLatin1) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
      true ->
  ...
  defp scan_nmtoken(str, s0, acc, prefix, local, nSC, isLatin1) do
    ...
    case ({:xmerl_lib.is_namechar(ch), isLatin1}) do
      {true, _} ->
        scan_nmtoken(t, s, [ch | acc], prefix, [ch | local],
                       nSC, isLatin1(ch, isLatin1))
      {_, true} ->
        nmStr = :lists.reverse(acc)
        {:erlang.list_to_atom(nmStr),
           namespace_info(prefix, local), str, s}
      _ ->
        {:lists.reverse(acc), namespace_info(prefix, local),
           str, s}
    end
  end
  ...
  defp scan_system_literal([], s = r_xmerl_scanner(continuation_fun: f), delimiter,
            acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
  ...
  defp scan_system_literal('#' ++ _R, s, _H, _Acc) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3131,
                                       :fragment_identifier_in_system_literal])
        :ok
    end
  ...
  defp scan_system_literal(str, s, delimiter, acc) do
    {ch, t} = to_ucs(r_xmerl_scanner(s, :encoding), str)
    scan_system_literal(t, r_xmerl_scanner(s, col: r_xmerl_scanner(s, :col) + 1),
                          delimiter, [ch | acc])
  end
  ...
  defp scan_pubid_literal([h | _T], s) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3148, {:invalid_pubid_char, h}])
        :ok
    end
    fatal({:invalid_pubid_char, h}, s)
  end

  defp scan_pubid_literal([], s = r_xmerl_scanner(continuation_fun: f), delimiter,
            acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
  ...
  defp scan_pubid_literal([h | t], s, delimiter, acc) do
    case (is_pubid_char(h)) do
      true ->
        scan_pubid_literal(t, r_xmerl_scanner(s, col: r_xmerl_scanner(s, :col) + 1),
                             delimiter, [h | acc])
      false ->
        cond do
          r_xmerl_scanner(s, :quiet) ->
  ...
  defp scan_elem_content(t, s) do
    scan_elem_content(t, s, _Context = :children,
                        _Mode = :unknown, _Acc = [])
  end

  defp scan_elem_content([], s = r_xmerl_scanner(continuation_fun: f), context, mode,
            acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
    ...
    case ({occurrence, context, acc}) do
      ...
      {other, :mixed, _} ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [3242, {:illegal_for_mixed_content,
                                                other}])
            :ok
        end
  ...
  defp scan_elem_content('#PCDATA' ++ _T, s, :not_mixed, _Mode, _Acc) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3249, {:error, {:extra_set_of_parenthesis}}])
        :ok
    end
  ...
  defp scan_elem_content('#PCDATA' ++ _T, s, _Cont, mode, acc)
    ...
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3252, {:error,
                                            {:invalid_format_of_mixed_content}}])
        :ok
    end
  ...
  defp scan_elem_content(',' ++ _T, s, _Context, :choice, _Acc) do
    ...
    fatal({:mixing_comma_and_vertical_bar_in_content_model},
            s)
  end
  ...
  defp scan_elem_content2('(' ++ _T, s, :mixed, _Mode, _Acc) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3274, {:error,
                                            {:element_names_must_not_be_parenthesized_in_mixed_content}}])
        :ok
    end
    fatal({:error,
             {:element_names_must_not_be_parenthesized_in_mixed_content}},
  ...
  defp scan_elem_content2('(' ++ t, s0, context, mode, acc) do
    ...
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    {_, t1, s1} = strip(t, s)
    {inner, t2, s2} = scan_elem_content(t1, s1, :not_mixed,
                                          :unknown, [])
    scan_elem_content(t2, s2, context, mode, [inner | acc])
  end

  defp scan_elem_content2('%' ++ _T, s = r_xmerl_scanner(environment: :prolog), _Context,
            _Mode, _Acc) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
    ...
    case ({occurrence, context}) do
      ...
      {other, :mixed} ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [3294, {:illegal_for_mixed_content,
                                                other}])
            :ok
        end
        fatal({:illegal_for_mixed_content, other}, s1)
      ...
      _ ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [3334, {:error,
                                                {:validity_constraint_Name_Token,
                                                   c}}])
            :ok
        end
  ...
  defp vc_ID_Attribute_Default({_, :ID, _, def__, _}, _S) when def__ == :"#IMPLIED" or
                                            def__ == :"#REQUIRED" do
    :ok
  end
  ...
  defp vc_ID_Attribute_Default({_, :ID, _, def__, _}, s) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3346, {:error,
                                            {:validity_constraint_error_ID_Attribute_Default,
                                               def__}}])
        :ok
    end
    fatal({:error,
  ...
  defp vc_Enumeration({_Name, {_, nameList}, defaultVal, _, _}, s)
    ...
    case (:lists.member(:erlang.list_to_atom(defaultVal),
                          nameList)) do
      true ->
        :ok
      _ ->
        ...
        fatal({:error,
                 {:vc_enumeration, :erlang.list_to_atom(defaultVal),
                    nameList}},
                s)
    end
  end

  defp vc_Enumeration({_Name, {_, _NameList}, _DefaultVal, _, _},
            _S) do
    :ok
  end
  ...
  defp vc_Entity_Name({_Name, :ENTITY, defaultVal, _, _}, s)
    ...
    case (read.(:entity, :erlang.list_to_atom(defaultVal),
                  s)) do
      ...
      _ ->
        ...
        fatal({:error,
                 {:vc_Entity_Name, :erlang.list_to_atom(defaultVal)}},
                s)
    end
  end
  ...
  defp mandatory_delimeter_wfc(t, s) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3417,
                                       {:comma_or_vertical_bar_mandatory_between_names_in_content_model,
                                          t}])
        :ok
    end
    fatal({:comma_or_vertical_bar_mandatory_between_names_in_content_model,
             t},
            s)
  end
      ...
      true ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [3425, {:error,
                                                {:unique_att_spec_required,
                                                   n}}])
            :ok
        end
        fatal({:error, {:unique_att_spec_required, n}}, s)
      _ ->
        case (r_xmerl_scanner(s, :namespace_conformant) and :lists.keymember(eN,
  ...
  defp wfc_whitespace_betw_attrs([wS | _] = l, s) when wS == 32 or wS == 13 or
                                  wS == 10 or wS == 9 do
    {l, s}
  end
  ...
  defp wfc_Entity_Declared({:external, _}, s = r_xmerl_scanner(standalone: :yes),
            name) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3468,
                                       {:reference_to_externally_defed_entity_standalone_doc,
                                          name}])
        :ok
    end
    fatal({:reference_to_externally_defed_entity_standalone_doc,
             name},
            s)
  end
  ...
  defp wfc_Internal_parsed_entity(:internal, value, s) do
    scan_content(value,
                   r_xmerl_scanner(s, environment: :internal_parsed_entity), _Name = [],
                   [], r_xmerl_scanner(s, :space), _Lang = [], _Prnt = [], r_xmlNamespace())
  end
  ...
  defp vc_Element_valid(_Name, {'xmlns', _},
            s = r_xmerl_scanner(namespace_conformant: true)) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
      true ->
  ...
  defp vc_Element_valid(_Name,
            r_xmerl_scanner(environment: :internal_parsed_entity)) do
    :ok
  end

  defp vc_Element_valid(name,
            s = r_xmerl_scanner(rules_read_fun: read, validation: :dtd)) do
    case (read.(:elem_def, name, s)) do
      r_xmlElement(elementdef: :undeclared) ->
        ...
        fatal({:error,
                 {:error_missing_element_declaration_in_DTD, name}},
                s)
      :undefined ->
        cond do
          r_xmerl_scanner(s, :quiet) ->
  ...
  defp scan_notation_decl(t,
    ...
    {_, t4, s4} = strip(t3, s3)
    t5 = scan_mandatory('>', t4, 1, s4,
                          :expected_end_tag_notation_declaration)
    case (read.(:notation, name, s)) do
      :undeclared ->
  ...
  defp scan_entity_value(str, s, delim, name, namespace) do
    scan_entity_value(str, s, delim, _Acc = [], name,
                        namespace, [])
  end

  defp scan_entity_value([],
            s = r_xmerl_scanner(environment: {:external, {:entity, _}}), _Delim,
            acc, _, _, []) do
  ...
  defp scan_entity_value([],
            s = r_xmerl_scanner(environment: {:external, {:entity, _}},
                    validation: :dtd),
            _Delim, _Acc, pEName, _, _) do
    {{:error,
        {:failed_VC_Proper_Declaration_PE_Nesting, 1, pEName}},
       [], s}
  end
  ...
  defp scan_entity_value([], s = r_xmerl_scanner(validation: :dtd), :no_delim, _Acc,
            pEName, _, _PENesting) do
    {{:error,
        {:failed_VC_Proper_Declaration_PE_Nesting, 2, pEName}},
       [], s}
  end

  defp scan_entity_value([], s = r_xmerl_scanner(continuation_fun: f), delim, acc,
            pEName, namespace, pENesting) do
    :no_debug
    f.(fn moreBytes, s1 ->
            scan_entity_value(moreBytes, s1, delim, acc, pEName,
                                namespace, pENesting)
       end,
         fatal_fun(:unexpected_end), s)
  end

  defp scan_entity_value([delim | t], s = r_xmerl_scanner(validation: :dtd), delim,
            _Acc, pEName, _NS, pENesting)
      when length(pENesting) != 0 do
    {{:error,
        {:failed_VC_Proper_Declaration_PE_Nesting, 3, pEName}},
       t, s}
  end

  defp scan_entity_value([delim | t], s0, delim, acc, _PEName, _NS,
            _PENesting) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
  ...
  defp scan_entity_value('%' ++ _T, s = r_xmerl_scanner(environment: :prolog), _, _, _,
            _, _) do
    cond do
      r_xmerl_scanner(s, :quiet) ->
        :ok
  ...
  defp scan_entity_value('%' ++ t, s0, delim, acc, pEName, namespace,
            pENesting) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
    {pERefName, t1, s1} = scan_pe_reference(t, s)
      ...
      true ->
        {expandedRef,
           s2} = (case (expand_pe_reference(pERefName, s1,
                    ...
                    tuple when is_tuple(tuple) ->
                      {expRef, sx} = fetch_not_parse(tuple, s1)
                      {entV, _, s5} = scan_entity_value(expRef, sx, :no_delim,
                                                          [], pERefName,
                                                          :parameter, [])
        ...
        {acc2, _, s4} = scan_entity_value(expandedRef, s3,
                                            :no_delim, acc, pEName, namespace,
                                            [])
        scan_entity_value(t1,
                            r_xmerl_scanner(s4, line: r_xmerl_scanner(s3, :line),  col: r_xmerl_scanner(s3, :col)), delim,
                            :lists.reverse(acc2), pEName, namespace, pENesting)
    end
  end
  ...
  defp scan_entity_value('&' ++ t, s0, delim, acc, pEName, namespace,
    ...
    case (t) do
      '#' ++ _T ->
        {expRef, t1, s1} = scan_reference(t, s)
        tok = pe_nesting_token(expRef ++ t1, namespace,
                                 r_xmerl_scanner(s1, :validation))
        case (markup_delimeter(expRef)) do
          true ->
      ...
      _ ->
        ...
        scan_entity_value(t2, s2, delim,
                            [';', :erlang.atom_to_list(name), '&' | acc], pEName,
                            namespace, pENesting)
    end
  end

  defp scan_entity_value('<![' ++ t, s0, delim, acc, pEName, :parameter = nS,
            pENesting) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 3)
  ...
  defp scan_entity_value('\n' ++ t, s, delim, acc, pEName, namespace,
            pENesting) do
    scan_entity_value(t, r_xmerl_scanner(s, line: r_xmerl_scanner(s, :line) + 1), delim,
                        ['\n' | acc], pEName, namespace, pENesting)
  end

  defp scan_entity_value(str, s0, delim, acc, pEName, namespace,
            pENesting) do
    {ch, t} = to_ucs(r_xmerl_scanner(s0, :encoding), str)
    case (:xmerl_lib.is_char(ch)) do
      true ->
        :no_debug
        s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 1)
        scan_entity_value(t, s, delim, [ch | acc], pEName,
                            namespace, pENesting)
      false ->
        cond do
          r_xmerl_scanner(s0, :quiet) ->
  ...
  defp save_refed_entity_name1(name, pEName,
            s = r_xmerl_scanner(entity_references: eRefs)) do
    case (:lists.keysearch(pEName, 1, eRefs)) do
      {:value, {_, refs}} ->
        newRefs = (case (:lists.member(name, refs)) do
                     true ->
                       refs
                     _ ->
                       [name | refs]
                   end)
        r_xmerl_scanner(s, entity_references: :lists.keyreplace(pEName, 1,
                                                    eRefs, {pEName, newRefs}))
      _ ->
        r_xmerl_scanner(s, entity_references: [{pEName, [name]} | eRefs])
    end
  end

  defp pe_push(tok, stack, _S) when tok == '<!' or tok == '<?' or
  ...
  defp pe_pop(token, _Stack, s = r_xmerl_scanner(validation: :dtd)) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3819, {:error,
                                            {:failed_VC_Proper_Declaration_PE_Nesting,
                                               5, token}}])
        :ok
    end
    fatal({:error,
             {:failed_VC_Proper_Declaration_PE_Nesting, 5, token}},
            s)
  end
  ...
  defp check_entity_recursion(eName,
            s = r_xmerl_scanner(entity_references: entityRefList)) do
    ...
    case ((try do
      ...
      {:EXIT, {:cyclic, _}} ->
        cond do
          ...
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n',
                                      [3849, {:illegal_recursion_in_Entity,
                                                eName}])
            :ok
        end
  ...
  defp scan_comment(str, s) do
    scan_comment(str, s, _Pos = :undefined, _Parents = [],
                   _Lang = [])
  end

  defp scan_comment(str, s = r_xmerl_scanner(col: c, event_fun: event), pos,
            parents, lang) do
    comment = r_xmlComment(pos: pos, parents: parents, language: lang,
                  value: :undefined)
    s1 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :started, line: r_xmerl_scanner(s, :line),
                           col: c, pos: pos, data: comment),
                         s))
    scan_comment1(str, s1, pos, comment, _Acc = [])
  end

  defp scan_comment1([], s = r_xmerl_scanner(continuation_fun: f), pos, comment,
            acc) do
    :no_debug
    f.(fn moreBytes, s1 ->
  ...
  defp scan_comment1('-->' ++ t,
            s0 = r_xmerl_scanner(col: c, event_fun: event, hook_fun: hook), _Pos,
            comment, acc) do
    :no_debug
    s = r_xmerl_scanner(s0, col: r_xmerl_scanner(s0, :col) + 3)
    comment1 = r_xmlComment(comment, value: :lists.reverse(acc))
    s1 = (r_xmerl_scanner() = event.(r_xmerl_event(event: :ended, line: r_xmerl_scanner(s, :line),
                           col: c, data: comment1),
                         s))
  ...
  defp scan_markup_completion_gt(t, s) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3919, {:error,
                                            {:malformed_syntax_entity_completion,
                                               t}}])
        :ok
    end
    fatal({:error,
             {:malformed_syntax_entity_completion, t}},
            s)
  end
      ...
      _ ->
        cond do
          r_xmerl_scanner(s, :quiet) ->
            :ok
          true ->
            :error_logger.error_msg('~p- fatal: ~p~n', [3927, errorMsg])
            :ok
        end
        fatal(errorMsg, s)
    end
  ...
  defp mandatory_strip([], s) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3959, {:error, {:whitespace_was_expected}}])
        :ok
    end
    fatal({:error, {:whitespace_was_expected}}, s)
  end

  defp mandatory_strip(t, s) when hd(t) == 32 or hd(t) == 13 or
                       hd(t) == 10 or hd(t) == 9 do
    strip(t, s, :all)
  end
  ...
  defp mandatory_strip([?% | t], s) when hd(t) == 32 or hd(t) == 13 or
    ...
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3963, {:error, {:whitespace_was_expected}}])
        :ok
    end
    fatal({:error, {:whitespace_was_expected}}, s)
  end
  ...
  defp mandatory_strip(_T, s) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [3967, {:error, {:whitespace_was_expected}}])
        :ok
    end
    fatal({:error, {:whitespace_was_expected}}, s)
  end
  ...
  defp normalize('&' ++ t, s, isNorm) do
    case (scan_reference(t, s)) do
      {expRef, t1, s1} when hd(expRef) == 32 or
  ...
  defp fast_accumulate_whitespace('<' ++ _ = r, s, _T) do
    ...
    {:done,
       {:erlang.element(3, cD), r,
          r_xmerl_scanner(s, col: 1,  line: line + 1)}}
  end
  ...
  defp fast_acc_end(t, s, n, col, c, cD_I) do
    ...
    try do
      ...
      _, _ ->
        accumulate_whitespace(t, s, line, col,
                                :lists.duplicate(n, c) ++ '\n')
    end
  end
  ...
  defp accumulate_whitespace('\t' ++ t, s, line, col, acc) do
    accumulate_whitespace(t, s, line, expand_tab(col),
                            [?\t | acc])
  end
              ...
              _ ->
                {:error,
                   {:schemaLocation_attribute,
                      :namespace_location_not_in_pair}}
            end
          _ ->
  ...
  defp handle_schema_result({:error, reason}, s5) do
    cond do
      ...
      true ->
        :error_logger.error_msg('~p- fatal: ~p~n',
                                  [4125, {:failed_schema_validation, reason}])
        :ok
    end
    ...
    fn s ->
         cond do
           r_xmerl_scanner(s, :quiet) ->
             :ok
           true ->
             :error_logger.error_msg('~p- fatal: ~p~n', [4134, reason])
             :ok
         end
         fatal(reason, s)
    end
  ...
  defp preformat(l1, l2, sep) do
    format1 = :lists.flatten(:lists.duplicate(length(l1) - 1,
                                                '~s ') ++ '~s')
  ...
  defp to_ucs(encoding, chars) when encoding == 'utf-8' or
  ...
  defp utf8_2_ucs([a, b, c, d | rest]) when (a &&& 248 === 240 and
                                       b &&& 192 === 128 and
                                       c &&& 192 === 128 and
                                       d &&& 192 === 128) do
  ...
  defp utf8_2_ucs([a, b, c | rest]) when (a &&& 240 === 224 and
  ...
  defp utf8_2_ucs([a, b | rest]) when (a &&& 224 === 192 and
                                 b &&& 192 === 128) do
    case (b &&& 63 ||| (a &&& 31 <<< 6)) do
      ch when ch >= 128 ->
        {ch, rest}
  ...
  defp ucs_2_utf8(ch) when ch < 65536 do
    [ch &&& 61440 >>> 12 ||| 224, ch &&& 4032 >>> 6 ||| 128,
                                      ch &&& 63 ||| 128]
  end

  defp ucs_2_utf8(ch) when ch < 2097152 do
    [ch &&& 1835008 >>> 18 ||| 240,
         ch &&& 258048 >>> 12 ||| 128, ch &&& 4032 >>> 6 ||| 128,
                                           ch &&& 63 ||| 128]
  end

  defp string_to_char_set(enc, str) when enc === 'utf-8' or
                           enc === :undefined do
    :lists.flatten(for x <- str do
                     ucs_2_utf8(x)
                   end)
  end
