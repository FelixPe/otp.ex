defmodule :m_xmerl_b64Bin_scan do
  ...
  defp scan_token([h | t]) when h == ?A or h == ?Q or h == ?g or
                          h == ?w do
    {{:b04, 1, h}, t}
  end

  defp scan_token([h | t]) when h == ?E or h == ?I or h == ?M or
                          h == ?U or h == ?Y or h == ?c or h == ?k or h == ?o or
                          h == ?s or h == ?0 or h == ?4 or h == ?8 do
    {{:b16x, 1, h}, t}
  end

  defp scan_token([h | t]) when h == ?B or h == ?C or h == ?D or
  ...
  defp scan_token([h | t]) when h == ?a or h == ?b or h == ?d or
  ...
  defp scan_token([h | t]) when h == ?1 or h == ?2 or h == ?3 or
