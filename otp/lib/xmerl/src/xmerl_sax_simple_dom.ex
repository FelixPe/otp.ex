defmodule :m_xmerl_sax_simple_dom do
  ...
  Record.defrecord(:r_xmlObj, :xmlObj, type: :undefined,
                                  value: :undefined)
  Record.defrecord(:r_xmerl_sax_simple_dom_state, :xmerl_sax_simple_dom_state, tags: [],
                                                      cno: [], namespaces: [],
                                                      dom: [])
  ...
  defp build_dom(:endDocument,
            r_xmerl_sax_simple_dom_state(dom: [{tag, attributes, content} | d]) = state) do
    case (d) do
      [:startDocument] ->
        r_xmerl_sax_simple_dom_state(state, dom: [{tag, attributes,
                          :lists.reverse(content)}])
      [r_xmlDecl() = decl, :startDocument] ->
        r_xmerl_sax_simple_dom_state(state, dom: [decl, {tag, attributes,
                                :lists.reverse(content)}])
      _ ->
        state
    end
  ...
  defp build_dom({:startElement, _Uri, localName, _QName,
             attributes},
            r_xmerl_sax_simple_dom_state(tags: t, cno: cN, dom: d) = state) do
    ...
    r_xmerl_sax_simple_dom_state(state, tags: [{:erlang.list_to_atom(localName), num} |
                        t], 
               cno: newCN, 
               dom: [{:erlang.list_to_atom(localName),
                        :lists.reverse(a), []} |
                         d])
  end

  defp build_dom({:endElement, _Uri, localName, _QName},
            r_xmerl_sax_simple_dom_state(tags: [_ | t], cno: [_ | cN],
    ...
    case (:erlang.list_to_atom(localName)) do
      ^cName ->
        r_xmerl_sax_simple_dom_state(state, tags: t,  cno: cN, 
                   dom: [{pName, pAttributes,
                            [{cName, cAttributes, :lists.reverse(cContent)} |
                                 pContent]} |
                             d])
      _ ->
        throw({:sax_simple_dom_error,
                 'Got end of element: ' ++ localName ++ ' but expected: ' ++ cName})
    end
  end

  defp build_dom({:characters, string},
            r_xmerl_sax_simple_dom_state(tags: _T, cno: [num | cN],
                dom: [{name, attributes, content} | d]) = state) do
    r_xmerl_sax_simple_dom_state(state, cno: [num + 1 | cN], 
               dom: [{name, attributes, [string | content]} | d])
  end

  defp build_dom({:ignorableWhitespace, string},
            r_xmerl_sax_simple_dom_state(tags: t, cno: [num | cN],
                dom: [{name, attributes, content} | d]) = state) do
    r_xmerl_sax_simple_dom_state(state, cno: [num + 1 | cN], 
               dom: [{name, attributes,
  ...
  defp build_dom({:startPrefixMapping, prefix, uri},
            r_xmerl_sax_simple_dom_state(namespaces: nS) = state) do
    r_xmerl_sax_simple_dom_state(state, namespaces: [{prefix,
                             :erlang.list_to_atom(uri)} |
                              nS])
  end

  defp build_dom({:endPrefixMapping, prefix},
            r_xmerl_sax_simple_dom_state(namespaces: [{prefix, _} | nS]) = state) do
    r_xmerl_sax_simple_dom_state(state, namespaces: nS)
  end

  defp build_dom({:processingInstruction, 'xml', piData},
            r_xmerl_sax_simple_dom_state(dom: d) = state) do
    ...
    {standalone, piData3} = find_and_remove_attribute('standalone',
                                                        piData2, :yes)
    r_xmerl_sax_simple_dom_state(state, dom: [r_xmlDecl(vsn: vsn, encoding: enc,
                       standalone: standalone, attributes: piData3) |
                       d])
  end

  defp build_dom({:processingInstruction, piTarget, piData},
            r_xmerl_sax_simple_dom_state(cno: [num | cN],
                dom: [r_xmlElement(content: c) = current | d]) = state) do
    r_xmerl_sax_simple_dom_state(state, cno: [num + 1 | cN], 
