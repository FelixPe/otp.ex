defmodule :m_reltool_mod_win do
  ...
  Record.defrecord(:r_state, :state, parent_pid: :undefined,
                                 xref_pid: :undefined, rel_pid: :undefined,
                                 mod_wins: :undefined, sys: :undefined,
                                 common: :undefined, mod: :undefined,
  ...
  Record.defrecord(:r_code_page, :code_page, name: :undefined,
                                     editor: :undefined, find_objs: :undefined,
                                     find_data: :undefined)
  Record.defrecord(:r_find_objs, :find_objs, search: :undefined,
                                     goto: :undefined, radio: :undefined)
  Record.defrecord(:r_find_data, :find_data, start: :undefined,
                                     found: :undefined, history: :undefined)
  def start_link(wxEnv, xref, relPid, common, modName) do
    :proc_lib.start_link(:reltool_mod_win, :init,
  ...
  defp do_init(parent, wxEnv, xref, relPid, c, modName) do
    ...
    s = r_state(parent_pid: parent, xref_pid: xref,
            rel_pid: relPid, sys: sys, mod: mod,
            name: :erlang.atom_to_list(r_mod(mod, :name)), common: c)
      ...
      msg ->
        case (msg) do
          {:system, from, sysMsg} ->
            ...
            :sys.handle_system_msg(sysMsg, from, r_state(s, :parent_pid),
                                     :reltool_mod_win, dbg, s)
          {:cast, _From, :raise} ->
  ...
  defp create_window(r_state(mod: mod, name: modStr) = s) do
    ...
    :wxEvtHandler.connect(book,
                            :command_notebook_page_changed, [{:skip, true}])
  ...
  defp create_code_page(r_state(book: book, code_pages: pages,
              name: modStr) = s,
            pageName) do
  ...
  defp do_create_code_page(r_state(xref_pid: xref, mod: m) = s, pageName) do
    ...
    {:ok, app} = :reltool_server.get_app(xref,
                                           r_mod(m, :app_name))
    erlBin = (case (r_app(app, :is_escript)) do
                false ->
  ...
  defp find_regular_bin(app, mod) do
    ...
    case ((try do
            :filelib.fold_files(srcDir, base, true, find,
                                  {:error, :enoent})
          catch
      ...
      {:error, :enoent} ->
        ...
        case (source_from_beam(beamFile)) do
          {:ok, source} ->
            source
          :error ->
            :unicode.characters_to_binary(['%% Bad luck, cannot find any debug info in the file "', beamFile])
        end
    end
  end
  ...
  defp find_escript_bin(r_app(active_dir: activeDir), mod) do
    ...
    {fun,
       escript} = (case (:filelib.is_regular(activeDir)) do
                     true ->
                     ...
                     false ->
                       ...
                       {fn fullName, _GetInfo, getBin, acc ->
                             :io.format('', [])
                             case (:filename.split(fullName)) do
                               [_AppName, 'ebin', f] when (f === objFile and
                                                        acc === notFound)
                                                     ->
    ...
    try do
      case (:reltool_utils.escript_foldl(fun, notFound,
                                           escript)) do
        {:ok, {:text, bin}} ->
          bin
        {:ok, {:obj, bin}} ->
          bin
        _ ->
          :unicode.characters_to_binary(['%% Bad luck, cannot find the code in the escript ', escript, '.'])
      end
    catch
      reason when is_list(reason) ->
        :unicode.characters_to_binary(['%% Bad luck, cannot find the code in the escript ', escript, ': ', reason])
    end
  end
  ...
  defp handle_event(r_state(xref_pid: xref) = s, wx) do
    case (wx) do
      ...
      r_wx(id: 414,
          event: r_wxCommand(type: :command_text_enter, cmdString: str)) ->
        goto_line(s, str)
      r_wx(event: r_wxBookCtrl(type: :command_notebook_page_changed)) ->
      ...
      r_wx(event: r_wxCommand(type: :command_button_clicked),
      ...
      r_wx(obj: objRef, event: r_wxMouse(type: :enter_window)) ->
        ...
        s
      _ ->
        :error_logger.format('~w~w got unexpected mod event from wx:\n\t~tp\n', [:reltool_mod_win, self(), wx])
        s
    end
  end

  defp redraw_mods(r_state(xref_pid: xref, deps_used_by_ctrl: usedByCtrl,
              deps_uses_ctrl: usesCtrl,
              mod: r_mod(is_pre_included: isPre, is_included: isIncl,
                       uses_mods: usesModNames, used_by_mods: usedByModNames),
              status_bar: bar)) do
    ...
    status = :lists.concat([inclStatus, ' uses ',
                                            length(usesModNames), ' modules and ', ' is used by ',
                                                                         length(usedByModNames),
                                                                             ' modules.'])
    :wxStatusBar.setStatusText(bar, status)
    usesMods = (for m <- usesModNames do
                  select_image(xref, m)
                end)
    usedByMods = (for m <- usedByModNames do
                    select_image(xref, m)
                  end)
  ...
  defp select_image(xref, modName) do
    ...
    image = (case (r_mod(m, :is_included)) do
  ...
  defp goto_line(r_state(active_page: p) = s, lineNo)
    ...
    right = :wxStyledTextCtrl.getLineEndPosition(editor,
                                                   lineNo)
  ...
  defp find_string(r_state(active_page: r_code_page(editor: editor,
    ...
    found2 = (case (res >= 0) do
                true ->
                  ...
                  true
                false ->
                  :wxStatusBar.setStatusText(r_state(s, :status_bar), 'Not found (Hit Enter to wrap search)')
                  false
              end)
    p2 = r_code_page(p, find_data: r_find_data(data, found: found2))
    pages = :lists.keystore(r_code_page(p, :name), r_code_page(:name),
                              r_state(s, :code_pages), p2)
  ...
  defp goto_function(s, editor) do
    ...
    left = :wxStyledTextCtrl.wordStartPosition(editor,
                                                 currentPos, true)
    right = :wxStyledTextCtrl.wordEndPosition(editor,
                                                currentPos, true)
    colonPos = left - 1
    left2 = (case (:wxStyledTextCtrl.getCharAt(editor,
                                                 colonPos)) do
               ?: ->
                 :wxStyledTextCtrl.wordStartPosition(editor, colonPos,
                                                       true)
               _ ->
                 left
             end)
    right2 = (case (:wxStyledTextCtrl.getCharAt(editor,
                                                  right)) do
                ?: ->
                  :wxStyledTextCtrl.wordEndPosition(editor, right + 1,
                                                      true)
                _ ->
                  right
              end)
  ...
  defp do_goto_function(s, [modStr, funStr]) do
    case (:reltool_server.get_mod(r_state(s, :xref_pid),
                                    :erlang.list_to_atom(modStr))) do
  ...
  defp goto_back(r_state(active_page: r_code_page(editor: editor,
                             find_data: data) = page,
              code_pages: pages) = s) do
    case (r_find_data(data, :history)) do
      [prevPos | history] ->
        lineNo = :wxStyledTextCtrl.lineFromPosition(editor,
                                                      prevPos)
        data2 = r_find_data(data, history: history)
        page2 = r_code_page(page, find_data: data2)
        pages2 = :lists.keystore(r_code_page(page2, :name), r_code_page(:name),
                                   pages, page2)
        goto_line(r_state(s, active_page: page2,  code_pages: pages2),
                    lineNo)
      [] ->
  ...
  defp add_pos_to_history(r_state(active_page: page, code_pages: pages) = s,
            currentPos) do
    data = r_code_page(page, :find_data)
    data2 = r_find_data(data, history: [currentPos |
  ...
  defp create_editor(parent) do
    ...
    :lists.foreach(fn style ->
                        setStyle.(style)
                   end,
                     styles)
    :wxStyledTextCtrl.setKeyWords(ed, 0, keyWords())
    :wxStyledTextCtrl.markerDefine(ed, 0, 0,
                                     [{:foreground, {170, 20, 20}}])
    :wxStyledTextCtrl.markerDefine(ed, 0, 0,
                                     [{:background, {200, 120, 120}}])
    :wxStyledTextCtrl.markerDefine(ed, 1, 0,
                                     [{:foreground, {20, 20, 170}}])
    :wxStyledTextCtrl.markerDefine(ed, 1, 0,
                                     [{:background, {120, 120, 200}}])
    :wxStyledTextCtrl.markerDefine(ed, 2, 2,
                                     [{:foreground, {20, 170, 20}}])
    :wxStyledTextCtrl.markerDefine(ed, 2, 2,
                                     [{:background, {200, 255, 200}}])
    :wxStyledTextCtrl.markerDefine(ed, 3, 22,
                                     [{:background, {200, 255, 200}}])
    policy = 1 ||| 16 ||| 8
  ...
  defp create_search_area(parent) do
    ...
    :wxSizer.add(sizer, :wxStaticText.new(parent, - 1, 'Find:'),
                   [{:flag, 2048}])
    tC1 = :wxTextCtrl.new(parent, 413, [{:style, 1024}])
    :wxSizer.add(sizer, tC1,
    ...
    :wxSizer.add(sizer, 15, 15,
                   [{:proportion, 1}, {:flag, 8192}])
    :wxSizer.add(sizer, :wxStaticText.new(parent, - 1, 'Goto Line:'),
                   [{:flag, 2048}])
    tC2 = :wxTextCtrl.new(parent, 414, [{:style, 1024}])
    :wxSizer.add(sizer, tC2,
  ...
  defp keyWords() do
    l = ['after', 'begin', 'case', 'try', 'cond', 'catch', 'andalso', 'orelse', 'end', 'fun', 'if', 'let', 'of', 'receive', 'when', 'bnot', 'not',
                                                             'div', 'rem', 'band', 'and', 'bor', 'bxor',
                                                                                'bsl',
                                                                                    'bsr',
                                                                                        'or',
                                                                                            'xor']
    :lists.flatten((for k <- l do
                      k ++ ' '
                    end) ++ [0])
  end
