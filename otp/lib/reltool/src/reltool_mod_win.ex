defmodule :m_reltool_mod_win do
  ...
  defp create_window(r_state(mod: mod, name: modStr) = s) do
    title =
      :erlang.atom_to_list(:reltool) ++
        ~c" - " ++ :erlang.atom_to_list(r_mod(mod, :app_name)) ++ ~c" - " ++ modStr ++ ~c".erl"
  ...
  defp create_deps_page(s) do
    ...
    usedByCtrl =
      create_mods_list_ctrl(panel, main, ~c"Modules using this", ~c" and their applications")
    ...
    usesCtrl = create_mods_list_ctrl(panel, main, ~c"Used modules", ~c" and their applications")
    ...
    :wxNotebook.addPage(r_state(s2, :book), panel, ~c"Dependencies", [])
    s2
  end
  ...
  defp do_create_code_page(r_state(xref_pid: xref, mod: m) = s, pageName) do
    ...
    toolTip = ~c"Double click on a function call to search the function definition."
  ...
  defp find_regular_bin(app, mod) do
    ...
    srcDir = :filename.join([activeDir, ~c"src"])
    ...
    case (try do
      ...
      {:error, :enoent} ->
        beamFile = :filename.join([activeDir, ~c"ebin", modStr ++ ~c".beam"])
        ...
        case source_from_beam(beamFile) do
          ...
          :error ->
            :unicode.characters_to_binary([
              ~c"%% Bad luck, cannot find any debug info in the file \"",
              beamFile
            ])
        end
    end
  ...
  defp find_escript_bin(r_app(active_dir: activeDir), mod) do
    ...
    {fun, escript} =
      case :filelib.is_regular(activeDir) do
        ...
        false ->
          ...
          srcFile = :lists.concat([modName, ~c".erl"])
          ...
          {fn fullName, _GetInfo, getBin, acc ->
             ...
             case :filename.split(fullName) do
               ...
               [_AppName, ~c"src", f] when f === srcFile ->
                 {:text, getBin.()}
    ...
    try do
      case :reltool_utils.escript_foldl(fun, notFound, escript) do
        ...
        _ ->
          :unicode.characters_to_binary([
            ~c"%% Bad luck, cannot find the code in the escript ",
            escript,
            ~c"."
          ])
      end
  ...
  defp handle_event(r_state(xref_pid: xref) = s, wx) do
    case wx do
      ...
      r_wx(
        ...
        find_regexp_forward(s2, r_state(s2, :name) ++ ~c":")
      ...
      _ ->
        :error_logger.format(~c"~w~w got unexpected mod event from wx:\n\t~tp\n", [
          :reltool_mod_win,
          self(),
          wx
        ])
  ...
  defp goto_line(r_state(active_page: p) = s, str) when is_list(str) do
    try do
      ...
      _, _ ->
        :wxStatusBar.setStatusText(r_state(s, :status_bar), ~c"Not a line number")
        s
    end
  end
    ...
    found2 =
      case res >= 0 do
        true ->
          ...
          :wxStatusBar.setStatusText(r_state(s, :status_bar), ~c"")
          true

        false ->
          :wxStatusBar.setStatusText(
            r_state(s, :status_bar),
            ~c"Not found (Hit Enter to wrap search)"
          )
  ...
  defp goto_function(s, editor) do
    ...
    case [:wxStyledTextCtrl.getCharAt(editor, right2)] do
      ~c"(" ->
        ...
        s2 = add_pos_to_history(s, currentPos)
        do_goto_function(s2, :string.lexemes(text, ~c":"))
  ...
  defp do_goto_function(r_state(active_page: p) = s, [funName]) do
    ...
    find_regexp_forward(s, ~c"^" ++ funName ++ ~c"(")
  end
  ...
  defp do_goto_function(s, [modStr, funStr]) do
    case :reltool_server.get_mod(
      ...
      {:ok, mod} when r_mod(mod, :app_name) !== :"*MISSING*" ->
        s2 = create_code_page(r_state(s, mod: mod), modStr)
        find_regexp_forward(s2, ~c"^" ++ funStr ++ ~c"(")

      {:ok, _} ->
        :wxStatusBar.setStatusText(
          r_state(s, :status_bar),
          ~c"No such module: " ++ modStr
        )
  ...
  defp goto_back(
    ...
    case r_find_data(data, :history) do
      ...
      [] ->
        :wxStatusBar.setStatusText(r_state(s, :status_bar), ~c"No history")
        s
    end
  end
  ...
  defp create_editor(parent) do
    ...
    lW = :wxStyledTextCtrl.textWidth(ed, 33, ~c"9")
