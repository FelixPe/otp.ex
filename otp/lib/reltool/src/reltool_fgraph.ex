defmodule :m_reltool_fgraph do
  ...
  def step(vs, es, pa) do
    :reltool_fgraph.map(fn node = {_, r_fg_v(type: :static)} ->
                             node
                           {key,
                              value = r_fg_v(p: {px, py}, v: {vx, vy},
                                          type: :dynamic)}
  ...
  defp coulomb_repulsion(k0, r_fg_v(p: p0, q: q0), vs, {fx0, fy0})
    ...
    :reltool_fgraph.foldl(fn {k1, _}, f when k1 == k0 ->
                               f
  ...
  defp hooke_attraction(key0, r_fg_v(p: p0), vs, es, {fx0, fy0})
      when (is_float(fx0) and is_float(fy0)) do
    :reltool_fgraph.foldl(fn {{key1, key1}, _}, f ->
                               f
  ...
  defp composition({px1, py1}, {px0, py0}) when (is_float(px1) and
