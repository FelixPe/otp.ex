defmodule :m_reltool_server do
  use Bitwise
  require Record
  Record.defrecord(:r_common, :common, sys_debug: :undefined,
                                  wx_debug: :undefined, trap_exit: :undefined)
  Record.defrecord(:r_mod, :mod, name: :undefined,
                               app_name: :undefined, incl_cond: :undefined,
                               debug_info: :undefined, is_app_mod: :undefined,
                               is_ebin_mod: :undefined, uses_mods: :undefined,
                               exists: :undefined, status: :ok,
                               used_by_mods: [], is_pre_included: :undefined,
                               is_included: :undefined)
  Record.defrecord(:r_app_info, :app_info, description: '', id: '',
                                    vsn: '', modules: [], maxP: :infinity,
                                    maxT: :infinity, registered: [],
                                    opt_apps: [], incl_apps: [],
                                    applications: [], env: [], mod: :undefined,
                                    start_phases: :undefined,
                                    runtime_dependencies: [])
  Record.defrecord(:r_regexp, :regexp, source: :undefined,
                                  compiled: :undefined)
  Record.defrecord(:r_app, :app, name: :undefined,
                               is_escript: :undefined,
                               use_selected_vsn: :undefined,
                               active_dir: :undefined, sorted_dirs: :undefined,
                               vsn: :undefined, label: :undefined,
                               info: :undefined, mods: :undefined,
                               mod_cond: :undefined, incl_cond: :undefined,
                               debug_info: :undefined, app_file: :undefined,
                               app_type: :undefined,
                               incl_app_filters: :undefined,
                               excl_app_filters: :undefined, status: :undefined,
                               uses_mods: :undefined, used_by_mods: :undefined,
                               uses_apps: :undefined, used_by_apps: :undefined,
                               is_pre_included: :undefined,
                               is_included: :undefined, rels: :undefined)
  Record.defrecord(:r_rel_app, :rel_app, name: :undefined,
                                   app_type: :undefined, incl_apps: :undefined)
  Record.defrecord(:r_rel, :rel, name: :undefined,
                               vsn: :undefined, rel_apps: :undefined,
                               load_dot_erlang: true)
  Record.defrecord(:r_sys, :sys, root_dir: :undefined,
                               lib_dirs: :undefined, escripts: :undefined,
                               mod_cond: :undefined, incl_cond: :undefined,
                               apps: :undefined, boot_rel: :undefined,
                               rels: :undefined, emu_name: :undefined,
                               profile: :undefined, excl_lib: :undefined,
                               ...
                               app_file: :undefined, debug_info: :undefined)
  Record.defrecord(:r_state, :state, options: :undefined,
                                 parent_pid: :undefined, common: :undefined,
                                 sys: :undefined, old_sys: :undefined,
                                 status: :undefined, old_status: :undefined,
                                 app_tab: :undefined, old_app_tab: :undefined,
                                 mod_tab: :undefined, old_mod_tab: :undefined,
  ...
  def start_link(options) do
    :proc_lib.start_link(:reltool_server, :init,
                           [[{:parent, self()} | options]], :infinity, [])
  end

  def get_config(pid, inclDef, inclDeriv) do
    :reltool_utils.call(pid,
  ...
  def init([{:parent, parent} | _] = options) do
    try do
      ...
      {:error, reason} ->
        :proc_lib.init_fail(parent, {:error, reason},
                              {:exit, :normal})
      :error, reason ->
  ...
  defp do_init(options) do
    appTab = :ets.new(:reltool_apps1,
                        [:public, :ordered_set, {:keypos, r_app(:name)}])
    oldAppTab = :ets.new(:reltool_apps2,
                           [:public, :ordered_set, {:keypos, r_app(:name)}])
    modTab = :ets.new(:reltool_mods1,
                        [:public, :ordered_set, {:keypos, r_mod(:name)}])
    oldModTab = :ets.new(:reltool_mods2,
                           [:public, :ordered_set, {:keypos, r_mod(:name)}])
    modUsesTab = :ets.new(:reltool_mod_uses,
                            [:public, :bag, {:keypos, 1}])
    s = r_state(options: options, app_tab: appTab,
            old_app_tab: oldAppTab, mod_tab: modTab,
            old_mod_tab: oldModTab, mod_used_by_tab: modUsesTab)
    s2 = parse_options(s)
    {s3, apps, status2} = refresh(s2)
    status3 = analyse(s3, apps, status2)
    fakeBackup = {:ets.tab2list(r_state(s3, :app_tab)),
                    :ets.tab2list(r_state(s3, :mod_tab))}
    s4 = save_old(s3, s3, fakeBackup, status3)
    r_state(parent_pid: parent, sys: sys, common: c) = s4
    :proc_lib.init_ack(parent,
  ...
  defp default_sys() do
    r_sys(root_dir: :reltool_utils.root_dir(),
        lib_dirs: :reltool_utils.erl_libs(), escripts: [],
        incl_cond: :derived, mod_cond: :all, apps: [],
        boot_rel: 'start_clean', rels: :reltool_utils.default_rels(),
        emu_name: 'beam', profile: :development,
        ...
        relocatable: true, rel_app_type: :permanent,
        embedded_app_type: :undefined, app_file: :keep,
        debug_info: :keep)
  end
  ...
  def loop(r_state(sys: sys) = s) do
    receive do
      {:system, from, msg} ->
        :sys.handle_system_msg(msg, from, r_state(s, :parent_pid),
                                 :reltool_server, r_common(r_state(s, :common), :sys_debug),
                                 s)
      {:call, replyTo, ref,
         {:get_config, inclDef, inclDeriv}} ->
      ...
      {:call, replyTo, ref, :undo_config} ->
        s2 = r_state(s, sys: r_state(s, :old_sys),  old_sys: sys, 
                    status: r_state(s, :old_status),  old_status: r_state(s, :status), 
      ...
      {:call, replyTo, ref, {:get_rel, relName}} ->
        reply = (case (:lists.keysearch(relName, r_rel(:name),
                                          r_sys(sys, :rels))) do
                   {:value, rel} ->
      ...
      {:call, replyTo, ref, {:get_mod, modName}} ->
        reply = (case (:ets.lookup(r_state(s, :mod_tab), modName)) do
                   [m] ->
                     {:ok, m}
                   [] ->
      ...
      {:call, replyTo, ref, {:get_app, appName}}
        ...
        reply = (case (:ets.lookup(r_state(s, :app_tab), appName)) do
                   [app] ->
                     {:ok, app}
                   [] ->
      ...
      {:call, replyTo, ref, {:get_apps, kind}} ->
        appNames = (case (kind) do
                      :whitelist ->
                        :ets.select(r_state(s, :app_tab),
                                      [{r_app(is_pre_included: true, _: :_), [],
                                          [:"$_"]}])
                      :blacklist ->
                        :ets.select(r_state(s, :app_tab),
                                      [{r_app(is_pre_included: false, _: :_), [],
                                          [:"$_"]}])
                      :source ->
                        :ets.select(r_state(s, :app_tab),
                                      [{r_app(is_included: :"$1", is_pre_included: :"$2",
                                            _: :_),
                                          [{:"=/=", :"$1", true}, {:"=/=", :"$2", false}],
                                          [:"$_"]}])
                      :derived ->
      ...
      {:call, replyTo, ref, :get_sys} ->
        :reltool_utils.reply(replyTo, ref,
                               {:ok, r_sys(sys, apps: :undefined)})
        :reltool_server.loop(s)
      ...
      {:call, replyTo, ref, {:gen_rel_files, dir}} ->
        status = (case (:reltool_target.gen_rel_files(sys_all_apps(s),
                                                        dir)) do
                    :ok ->
                      {:ok, []}
                    {:error, reason} ->
      ...
      {:EXIT, pid, reason} when pid === r_state(s, :parent_pid) ->
        exit(reason)
      {:call, replyTo, ref, msg} when (is_pid(replyTo) and
                                         is_reference(ref))
                                      ->
        :error_logger.format('~w~w got unexpected call:\n\t~tp\n', [:reltool_server, self(), msg])
        :reltool_utils.reply(replyTo, ref,
                               {:error, {:invalid_call, msg}})
        :reltool_server.loop(s)
      msg ->
  ...
  defp app_update_config([r_app(name: name,
               is_escript: {:inlined, escript}) |
               _],
            _SysApps) do
    :reltool_utils.throw_error('Application ~w is inlined in ~w. Can not change configuration for an inlined application.', [name, escript])
  end

  defp app_update_config([config | configs], sysApps) do
    newSysApps = (case (app_set_config_only(config)) do
                    {:delete, name} ->
  ...
  defp app_set_config_only(r_app(mods: configMods) = config) do
    app_set_config_only(mod_set_config_only(configMods),
                          config)
  end

  defp app_set_config_only([],
            r_app(name: name, incl_cond: :undefined,
                mod_cond: :undefined, use_selected_vsn: :undefined,
                debug_info: :undefined, app_file: :undefined,
                app_type: :undefined, incl_app_filters: :undefined,
                excl_app_filters: :undefined, is_escript: false)) do
    {:delete, name}
  end

  defp app_set_config_only(mods,
            r_app(name: name, incl_cond: inclCond, mod_cond: modCond,
                use_selected_vsn: useSelectedVsn, debug_info: debugInfo,
                app_file: appFile, app_type: appType,
                incl_app_filters: inclAppFilters,
                excl_app_filters: exclAppFilters, vsn: vsn,
                is_escript: isEscript, label: label, info: info,
                active_dir: activeDir, sorted_dirs: sortedDirs)) do
    app = r_app(default_app(name), incl_cond: inclCond, 
                                 mod_cond: modCond, 
                                 use_selected_vsn: useSelectedVsn, 
                                 debug_info: debugInfo,  app_file: appFile, 
                                 app_type: appType, 
                                 incl_app_filters: inclAppFilters, 
                                 excl_app_filters: exclAppFilters,  vsn: vsn, 
                                 mods: mods)
    cond do
      isEscript ->
        r_app(app, is_escript: isEscript,  active_dir: activeDir, 
                 sorted_dirs: sortedDirs,  label: label,  info: info)
      useSelectedVsn === :dir ->
  ...
  defp analyse(r_state(sys: sys) = s, apps, status) do
    ...
    status2 = apps_init_is_included(s, apps, relApps,
                                      status)
  ...
  defp apps_in_rels(rels, apps) do
    allRelApps = :lists.foldl(fn rel, relApps ->
  ...
  defp apps_in_rel(r_rel(name: relName, rel_apps: relApps), apps) do
    ...
    explicit0 = (for r_rel_app(name: appName) <- relApps do
                   {relName, appName}
                 end)
    explicit = mandatory ++ explicit0
    deps = (for rA <- relApps,
                  {appName,
                     optional} <- (case (:lists.keyfind(r_rel_app(rA, :name),
                                                          r_app(:name), apps)) do
                                     r_app(info: info) ->
                                       iA = (case (r_rel_app(rA, :incl_apps)) do
                                               :undefined ->
  ...
  defp more_apps_in_rels([{relName, appName, optional} | relApps], apps,
            acc) do
    case (:lists.member({relName, appName}, acc)) do
      true ->
        more_apps_in_rels(relApps, apps, acc)
      false ->
        case (:lists.keyfind(appName, r_app(:name), apps)) do
          r_app(info: r_app_info(incl_apps: iA) = info) ->
            extra = (for {childName,
                            childOptional} <- build_more_apps(info, iA) do
                       {relName, childName, childOptional}
                     end)
            acc2 = more_apps_in_rels(extra, apps,
                                       [{relName, appName} | acc])
            more_apps_in_rels(relApps, apps, acc2)
          false when optional ->
            more_apps_in_rels(relApps, apps, acc)
          false ->
            :reltool_utils.throw_error('Release ~tp uses non existing application ~w', [relName, appName])
        end
    end
  end
  ...
  defp build_more_apps(r_app_info(applications: aA, opt_apps: oA), iA) do
    aAOpt = (for app <- aA do
               {app, :lists.member(app, oA)}
             end)
    iAOpt = (for app <- iA do
               {app, false}
             end)
  ...
  defp apps_init_is_included(s, apps, relApps, status) do
    :lists.foldl(fn app, accStatus ->
                      app_init_is_included(s, app, relApps, accStatus)
                 end,
                   status, apps)
  end

  defp app_init_is_included(r_state(app_tab: appTab, mod_tab: modTab, sys: sys),
            r_app(name: appName, mods: mods) = a, relApps, status) do
    appCond = (case (r_app(a, :incl_cond)) do
                 :undefined ->
                   r_sys(sys, :incl_cond)
                 _ ->
                   r_app(a, :incl_cond)
               end)
    modCond = (case (r_app(a, :mod_cond)) do
                 :undefined ->
                   r_sys(sys, :mod_cond)
                 _ ->
                   r_app(a, :mod_cond)
               end)
    rels = (for {relName, aN} <- relApps, aN === appName do
              relName
            end)
    {default, isPreIncl, isIncl, status2} = (case ({appCond,
                                                      rels}) do
                                               {:include, _} ->
                                                 {:undefined, true, true,
                                                    status}
                                               {:exclude, []} ->
                                                 {:undefined, false, false,
                                                    status}
                                               {:exclude, [relName | _]} ->
                                                 :reltool_utils.throw_error('Application ~w is used in release ~tp and cannot be excluded',
                                                                              [appName,
                                                                                   relName])
                                               {:derived, []} ->
                                                 {:undefined, :undefined,
    ...
    {mods2, status3} = :lists.mapfoldl(fn mod, acc ->
                                            mod_init_is_included(modTab, mod,
                                                                   modCond,
                                                                   appCond,
                                                                   default, acc)
                                       end,
                                         status2, mods)
    a2 = r_app(a, mods: mods2,  is_pre_included: isPreIncl, 
                is_included: isIncl,  rels: rels)
    :ets.insert(appTab, a2)
    status3
  end

  defp mod_init_is_included(modTab, m, modCond, appCond, default, status) do
    isIncl = (case (appCond) do
                :include ->
    ...
    status2 = (case (:ets.lookup(modTab, r_mod(m, :name))) do
                 [existing] ->
                   case ({r_mod(existing, :is_included), isIncl}) do
                     {false, _} ->
                       :ets.insert(modTab, m2)
                       :reltool_utils.add_warning('Module ~w exists in applications ~w and ~w. Using module from application ~w.',
                                                    [r_mod(m, :name),
                     ...
                     {_, false} ->
                       :reltool_utils.add_warning('Module ~w exists in applications ~w and ~w. Using module from application ~w.',
                                                    ...
                                                    status)
                     {_, _} ->
                       :reltool_utils.throw_error('Module ~w potentially included by two different applications: ~w and ~w.',
                                                    [r_mod(m, :name),
                                                         r_mod(existing, :app_name),
                                                             r_mod(m, :app_name)])
                   end
  ...
  defp app_mark_is_included(r_state(app_tab: appTab, mod_tab: modTab,
    ...
    case (:ets.lookup(appTab, appName)) do
      [a] ->
        case (r_app(a, :is_included)) do
          :undefined ->
            ...
            r_app(info: r_app_info(applications: depNames1,
      ...
      [] ->
        :reltool_utils.throw_error('Application ~tp uses non existing application ~w', [usedByName, appName])
    end
  ...
  defp get_all_mods_and_dependencies(s) do
    :ets.select(r_state(s, :mod_tab),
                  [{r_mod(name: :"$1", uses_mods: :"$2", is_included: true, _: :_),
                      [], [{{:"$1", :"$2"}}]}])
  end

  defp mod_propagate_is_included(s) do
    case (:lists.flatmap(fn {modName, usesModNames} ->
                              mod_mark_is_included(s, modName, usesModNames, [])
                         end,
                           get_all_mods_and_dependencies(s))) do
      [] ->
        :ok
      missingMods ->
        missingApp = default_app(:"*MISSING*", 'missing')
        missingApp2 = r_app(missingApp, label: '*MISSING*', 
                                      info: missing_app_info(''), 
                                      mods: missingMods,  status: :missing, 
                                      uses_mods: [])
  ...
  defp mod_mark_is_included(r_state(app_tab: appTab, mod_tab: modTab,
              sys: sys) = s,
            usedByName, [modName | modNames], acc) do
    acc3 = (case (:ets.lookup(modTab, modName)) do
              [m] ->
                case (r_mod(m, :is_included)) do
                  :undefined ->
                    m2 = (case (r_mod(m, :incl_cond)) do
                    ...
                    acc2 = (case (r_app(a, :is_included)) do
                              :undefined ->
                                modCond = (case (r_app(a, :mod_cond)) do
                                             :undefined ->
                                               r_sys(sys, :mod_cond)
                                ...
                                :ets.insert(appTab, a2)
                                mod_mark_is_included(s, modName,
                                                       for m3 <- mods do
                                                         r_mod(m3, :name)
                                                       end,
                                                       acc)
                    ...
                    mod_mark_is_included(s, modName, r_mod(m2, :uses_mods),
                                           acc2)
                  _ ->
                    acc
                end
  ...
  defp propagate_is_used_by(s) do
    :lists.foreach(fn {mod, usesMods} ->
                        :lists.foreach(fn usedMod ->
                                            :ets.insert(r_state(s, :mod_used_by_tab),
                                                          {usedMod, mod})
                                       end,
                                         usesMods)
                   end,
  ...
  defp app_recap_dependencies(s) do
    :ets.foldl(fn app, _ ->
                 ...
                 :ok, r_state(s, :app_tab))
  end

  defp app_recap_dependencies(s, r_app(mods: mods, is_included: isIncl) = a) do
    {mods2, isIncl2} = mod_recap_dependencies(s, a, mods,
                                                [], isIncl)
    appStatus = (case (:lists.keymember(:missing,
                                          r_mod(:status), mods2)) do
                   true ->
                     :missing
                   false ->
                     :ok
                 end)
    usesMods = (for m <- mods2,
                      r_mod(m, :is_included) === true do
                  r_mod(m, :uses_mods)
                end)
    usesMods2 = :lists.usort(:lists.flatten(usesMods))
    usesApps = (for modName <- usesMods2,
                      m <- :ets.lookup(r_state(s, :mod_tab), modName) do
                  r_mod(m, :app_name)
                end)
    usesApps2 = :lists.usort(usesApps)
    usedByMods = (for m <- mods2,
                        r_mod(m, :is_included) === true do
                    r_mod(m, :used_by_mods)
                  end)
    usedByMods2 = :lists.usort(:lists.flatten(usedByMods))
    usedByApps = (for modName <- usedByMods2,
                        m <- :ets.lookup(r_state(s, :mod_tab), modName) do
                    r_mod(m, :app_name)
                  end)
    usedByApps2 = :lists.usort(usedByApps)
    a2 = r_app(a, mods: mods2,  status: appStatus, 
                uses_mods: usesMods2,  used_by_mods: usedByMods2, 
                uses_apps: usesApps2,  used_by_apps: usedByApps2, 
                is_included: isIncl2)
    :ets.insert(r_state(s, :app_tab), a2)
    :ok
  end

  defp mod_recap_dependencies(s, a, [r_mod(name: modName) = m1 | mods], acc,
            isIncl) do
    case (:ets.lookup(r_state(s, :mod_tab), modName)) do
      [m2] when r_mod(m2, :app_name) === r_app(a, :name) ->
        modStatus = do_get_status(m2)
        {isIncl2, m3} = (case (r_mod(m2, :is_included)) do
                           true ->
                             usedByMods = (for {_,
                                                  n} <- :ets.lookup(r_state(s, :mod_used_by_tab),
                                                                      modName) do
                                             n
                                           end)
                             {true,
                                r_mod(m2, status: modStatus, 
                                        used_by_mods: usedByMods)}
                           _ ->
                             {isIncl,
                                r_mod(m2, status: modStatus,  used_by_mods: [])}
                         end)
        :ets.insert(r_state(s, :mod_tab), m3)
        mod_recap_dependencies(s, a, mods, [m3 | acc], isIncl2)
      [_] when r_app(a, :is_included) == false or
  ...
  defp verify_config(r_state(app_tab: appTab,
              sys: r_sys(boot_rel: bootRel, rels: rels)),
            relApps, status) do
    case (:lists.keymember(bootRel, r_rel(:name), rels)) do
      true ->
        status2 = :lists.foldl(fn rA, acc ->
                                    check_app(appTab, rA, acc)
                               end,
                                 status, relApps)
        :lists.foldl(fn r_rel(name: relName), acc ->
  ...
  defp check_app(appTab, {relName, appName}, status) do
    case (:ets.lookup(appTab, appName)) do
      [r_app(is_pre_included: isPreIncl, is_included: isIncl)]
        ...
        status
      _ ->
        :reltool_utils.throw_error('Release ~tp uses non included application ~w', [relName, appName])
    end
  end
    ...
    ensureApp = fn appName, acc ->
                     case (:lists.member({relName, appName}, relApps)) do
                       true ->
                         acc
                       false ->
                         :reltool_utils.throw_error('Mandatory application ~w is not included in release ~tp', [appName, relName])
                     end
                end
  ...
  defp refresh_app(r_app(name: appName, is_escript: isEscript,
              active_dir: activeDir, label: optLabel, mods: mods,
              status: appStatus) = app,
            force, status) do
    cond do
      force or optLabel === :undefined ->
        {appInfo, ebinMods, status3} = (case (isEscript) do
                                          false ->
                                            base = get_base(appName, activeDir)
                                            defaultVsn = get_vsn_from_dir(appName,
                                                                            base)
                                            ebin = :filename.join([activeDir,
                                                                       'ebin'])
                                            appFile = :filename.join([ebin,
                                                                          :erlang.atom_to_list(appName) ++ '.app'])
                                            {aI,
                                               status2} = read_app_info(appFile,
                                                                          appFile,
                                                                          appName,
                                                                          activeDir,
                                                                          appStatus,
                                                                          defaultVsn,
                                                                          status)
                                            {aI, read_ebin_mods(ebin, appName),
                                               status2}
                                          _ ->
                                            {r_app(app, :info), mods, status}
                                        end)
        appInfoMods = :lists.usort(r_app_info(appInfo, :modules))
        status4 = (case (r_app_info(appInfo, :modules) -- appInfoMods) do
                     [] ->
                       status3
                     duplicatedMods ->
                       :lists.foldl(fn m, s ->
                                         :reltool_utils.add_warning('Module ~w duplicated in app file for application ~w.',
                                                                      [m,
                                                                           appName],
                                                                      s)
                                    end,
                                      status3, duplicatedMods)
                   end)
        appModNames = (case (r_app_info(appInfo, :mod)) do
                         {startModName, _} ->
  ...
  defp read_app_info(_AppFileOrBin, _AppFile, :erts, _ActiveDir,
            _AppStatus, defaultVsn, status) do
    {missing_app_info(defaultVsn), status}
  end

  defp read_app_info(_AppFileOrBin, _AppFile, _AppName, :undefined,
            :missing, defaultVsn, status) do
    {missing_app_info(defaultVsn), status}
  end

  defp read_app_info(appFileOrBin, appFile, appName, _ActiveDir,
            _AppStatus, defaultVsn, status) do
    ...
    case (:reltool_utils.prim_consult(appFileOrBin)) do
      ...
      {:ok, _BadApp} ->
        {missing_app_info(defaultVsn),
           :reltool_utils.add_warning('~w: Illegal contents in app file ~tp, application tuple with arity 3 expected.', [appName, appFile],
                                        status)}
      {:error, text} when text === enoentText ->
        {missing_app_info(defaultVsn),
           :reltool_utils.add_warning('~w: Missing app file ~tp.', [appName, appFile],
                                        status)}
      {:error, text} ->
        {missing_app_info(defaultVsn),
           :reltool_utils.add_warning('~w: Cannot parse app file ~tp (~tp).', [appName, appFile, text],
                                        status)}
    end
  end
  ...
  defp parse_app_info(file, [{key, val} | keyVals], aI, status) do
    case (key) do
      :description ->
        parse_app_info(file, keyVals, r_app_info(aI, description: val),
                         status)
      :id ->
      ...
      :modules ->
        parse_app_info(file, keyVals, r_app_info(aI, modules: val),
                         status)
      :maxP ->
      ...
      :registered ->
        parse_app_info(file, keyVals, r_app_info(aI, registered: val),
                         status)
      :optional_applications ->
        parse_app_info(file, keyVals, r_app_info(aI, opt_apps: val),
                         status)
      :included_applications ->
        parse_app_info(file, keyVals, r_app_info(aI, incl_apps: val),
                         status)
      :applications ->
        parse_app_info(file, keyVals, r_app_info(aI, applications: val),
                         status)
      :env ->
      ...
      :start_phases ->
        parse_app_info(file, keyVals, r_app_info(aI, start_phases: val),
                         status)
      :runtime_dependencies ->
        parse_app_info(file, keyVals,
                         r_app_info(aI, runtime_dependencies: val), status)
      _ ->
        status2 = :reltool_utils.add_warning('Unexpected item ~tp in app file ~tp.', [key, file],
                                               status)
  ...
  defp init_mod(appName, file, fileOrBin, ext) do
    ...
    r_mod(name: modName, app_name: appName,
        incl_cond: :undefined, is_ebin_mod: true,
        uses_mods: usesMods, exists: true)
  end
  ...
  defp wait_for_processto_die(ref, pid, file) do
    ...
    after :timer.seconds(30) ->
      :error_logger.error_msg('~w(~w): Waiting for process ~w to die ~tp\n',
                                [:reltool_server, 1321, pid, file])
      wait_for_processto_die(ref, pid, file)
    end
  end

  defp add_missing_mods(appName, ebinMods, appModNames) do
    ebinModNames = (for m <- ebinMods do
                      r_mod(m, :name)
                    end)
  ...
  defp missing_mod(modName, appName) do
    r_mod(name: modName, app_name: appName,
        incl_cond: :undefined, is_ebin_mod: false,
        exists: false, status: :missing, uses_mods: [])
  end

  defp add_mod_config(mods, modConfigs) do
    addConfig = fn config, acc ->
                     case (:lists.keyfind(r_mod(config, :name), r_mod(:name),
                                            mods)) do
                       r_mod() = m ->
  ...
  defp do_get_config(s, inclDef, inclDeriv) do
    ...
    sys = (case (inclDeriv) do
             false ->
               apps = (for r_app(name: name) <- r_sys(r_state(s, :sys), :apps),
  ...
  defp shrink_app(a) do
    mods = (for m <- r_app(a, :mods),
                  r_mod(m, :incl_cond) !== :undefined do
              r_mod(m, is_app_mod: :undefined,  is_ebin_mod: :undefined, 
                     uses_mods: :undefined,  exists: false)
            end)
    cond do
      r_app(a, :is_escript) ->
        r_app(a, vsn: :undefined,  label: :undefined, 
               info: :undefined,  mods: [],  uses_mods: :undefined)
      true ->
        {dir, dirs, optVsn} = (case (r_app(a, :use_selected_vsn)) do
                                 :undefined ->
                                   {:undefined, [], :undefined}
                                 :vsn ->
                                   {:undefined, [], r_app(a, :vsn)}
                                 :dir ->
                                   {r_app(a, :active_dir), [r_app(a, :active_dir)],
                                      :undefined}
                               end)
        r_app(a, active_dir: dir,  sorted_dirs: dirs,  vsn: optVsn, 
               label: :undefined,  info: :undefined,  mods: mods, 
               uses_mods: :undefined)
    end
  end
  ...
  defp do_save_config(s, filename, inclDef, inclDeriv) do
    ...
    ioList = :io_lib.format('%% ~s\n%% config generated at ~w ~w\n~tp.\n\n',
                              [:epp.encoding_to_string(:utf8), :erlang.date(),
                                                                   :erlang.time(),
                                                                       config])
  ...
  defp read_config(oldSys, {:sys, keyVals}) do
    ...
    apps = (for a <- r_sys(newSys, :apps) do
              r_app(a, mods: :lists.sort(r_app(a, :mods)))
            end)
    rels = (case (r_sys(newSys, :rels)) do
              [] ->
                :reltool_utils.default_rels()
              rs ->
                rs
            end)
    newSys2 = r_sys(newSys, apps: :lists.sort(apps), 
                          rels: :lists.sort(rels))
    case (:lists.keymember(r_sys(newSys2, :boot_rel), r_rel(:name),
                             r_sys(newSys2, :rels))) do
      true ->
        newSys2
      false ->
        :reltool_utils.throw_error('Release ~tp is mandatory (used as boot_rel)', [r_sys(newSys2, :boot_rel)])
    end
  end
  ...
  defp decode(r_sys(apps: apps) = sys,
  ...
  defp decode(r_sys(apps: apps, escripts: escripts) = sys,
    ...
    app2 = decode(app, appKeyVals)
    decode(r_sys(sys, apps: [app2 | apps], 
                    escripts: [file | escripts]),
             sysKeyVals)
  end

  defp decode(r_sys(rels: rels) = sys,
  ...
  defp decode(r_sys(rels: rels) = sys,
    ...
    rel1 = :lists.foldl(fn opt, rel0 ->
                          ...
                          r_rel(name: name, vsn: vsn, rel_apps: []), opts)
  ...
  defp decode(r_sys() = sys, [{key, val} | keyVals]) do
    sys3 = (case (key) do
              ...
              :mod_cond when val === :all or val === :app or
                               val === :ebin or val === :derived or
                               val === :none
                             ->
                r_sys(sys, mod_cond: val)
              :incl_cond when val === :include or val === :exclude or
                                val === :derived
                              ->
              ...
              :profile when val === :development or
                              val === :embedded or val === :standalone
                            ->
                inclSys = :reltool_utils.choose_default(:incl_sys_filters,
                                                          val, false)
                exclSys = :reltool_utils.choose_default(:excl_sys_filters,
                                                          val, false)
                inclApp = :reltool_utils.choose_default(:incl_app_filters,
                                                          val, false)
                exclApp = :reltool_utils.choose_default(:excl_app_filters,
                                                          val, false)
                appType = :reltool_utils.choose_default(:embedded_app_type,
                                                          val, false)
                r_sys(sys, profile: val, 
                         incl_sys_filters: dec_re(:incl_sys_filters, inclSys,
                                                    r_sys(sys, :incl_sys_filters)), 
                         excl_sys_filters: dec_re(:excl_sys_filters, exclSys,
                                                    r_sys(sys, :excl_sys_filters)), 
                         incl_app_filters: dec_re(:incl_app_filters, inclApp,
                                                    r_sys(sys, :incl_app_filters)), 
                         excl_app_filters: dec_re(:excl_app_filters, exclApp,
                                                    r_sys(sys, :excl_app_filters)), 
                         embedded_app_type: appType)
              :excl_lib when val === :otp_root ->
                r_sys(sys, excl_lib: val)
              :incl_sys_filters ->
                r_sys(sys, incl_sys_filters: dec_re(key, val,
                                                  r_sys(sys, :incl_sys_filters)))
              :excl_sys_filters ->
                r_sys(sys, excl_sys_filters: dec_re(key, val,
                                                  r_sys(sys, :excl_sys_filters)))
              :incl_app_filters ->
                r_sys(sys, incl_app_filters: dec_re(key, val,
                                                  r_sys(sys, :incl_app_filters)))
              :excl_app_filters ->
                r_sys(sys, excl_app_filters: dec_re(key, val,
                                                  r_sys(sys, :excl_app_filters)))
  ...
  defp decode(r_app() = app, [{key, val} | keyVals]) do
    app2 = (case (key) do
              ...
              :app_type when val === :permanent or
                               val === :transient or val === :temporary or
              ...
              :incl_app_filters ->
                r_app(app, incl_app_filters: dec_re(key, val,
                                                  r_app(app, :incl_app_filters)))
              :excl_app_filters ->
                r_app(app, excl_app_filters: dec_re(key, val,
                                                  r_app(app, :excl_app_filters)))
              :incl_archive_filters ->
                :io.format('incl_archive_filters is no longer supported in reltool')
              :excl_archive_filters ->
                :io.format('excl_archive_filters is no longer supported in reltool')
  ...
  defp decode(r_app(mods: mods) = app,
            [{:mod, name, modKeyVals} | appKeyVals]) do
  ...
  defp decode(r_rel(rel_apps: relApps) = rel,
            [relApp | keyVals]) do
    {validTypesAssigned, rA} = (case (relApp) do
                                  name when is_atom(name) ->
                                    {true, r_rel_app(name: name)}
                                  {name, inclApps} when (is_atom(name) and
                                                           is_list(inclApps))
                                                        ->
                                    vI = :lists.all(&:erlang.is_atom/1,
                                                      inclApps)
                                    {vI, r_rel_app(name: name, incl_apps: inclApps)}
                                  {name, type} when is_atom(name) ->
                                    {is_type(type),
                                       r_rel_app(name: name, app_type: type)}
                                  {name, type, inclApps} when (is_atom(name) and
                                                                 is_list(inclApps))
                                                              ->
                                    vT = is_type(type)
                                    vI = :lists.all(&:erlang.is_atom/1,
  ...
  defp default_escript_app(file) do
    ...
    r_app(app, is_escript: true,  label: label, 
             info: missing_app_info(''))
  end

  defp refresh(r_state(sys: sys) = s) do
    rootDir = :filename.absname(r_sys(sys, :root_dir))
    libDirs = (for d <- r_sys(sys, :lib_dirs) do
                 :filename.absname(d)
               end)
    escripts = (for e <- r_sys(sys, :escripts) do
                  :filename.absname(e)
                end)
    sourceDirs = libs_to_dirs(rootDir, libDirs)
    mergedApps = merge_app_dirs(sourceDirs, r_sys(sys, :apps))
    {allApps, status2} = escripts_to_apps(escripts,
                                            mergedApps, {:ok, []})
    {refreshedApps, status3} = refresh_apps(r_sys(sys, :apps),
                                              allApps, [], true, status2)
    {patchedApps, status4} = patch_erts_version(rootDir,
                                                  refreshedApps, status3)
    escripts2 = (for a <- patchedApps, r_app(a, :is_escript) do
                   r_app(a, :active_dir)
                 end)
    sys2 = r_sys(sys, root_dir: rootDir,  lib_dirs: libDirs, 
                    escripts: escripts2)
        ...
        case (allLibDirs -- :lists.usort(allLibDirs)) do
          [] ->
            fun = fn base ->
                       ...
                       case (:filelib.is_dir(:filename.join([appDir, 'ebin']),
                                               :erl_prim_loader)) do
                         true ->
                           appDir
                         false ->
            ...
            ertsFiles = (for f <- rootFiles, :lists.prefix('erts', f) do
                           {:erts, fun.(f)}
                         end)
  ...
  defp escripts_to_apps([escript | escripts], apps, status) do
    ...
    appFiles = (case (:reltool_utils.escript_foldl(appFun,
                                                     [], escript)) do
                  {:ok, aF} ->
                    aF
                  {:error, reason1} ->
    ...
    fun = fn fullName, _GetInfo, getBin,
               {fileAcc, statusAcc} ->
               components = :filename.split(fullName)
               case (components) do
                 [appLabel, 'ebin', file] ->
                   case (:filename.extension(file)) do
                     '.app' ->
                       ...
                       {info, statusAcc2} = read_app_info(getBin.(),
                                                            appFileName,
                                                            appName, dir, :ok,
                                                            defaultVsn, status)
                 ...
                 ['.'] ->
                   ...
                   mod = init_mod(escriptAppName, modStr,
                                    {modStr, getBin.()}, ext)
                 ...
                 [file] ->
                   case (:filename.extension(file)) do
                     e when e === ext ->
                       mod = init_mod(escriptAppName, file, {file, getBin.()},
                                        ext)
    ...
    case (:reltool_utils.escript_foldl(fun, {[], status},
                                         escript)) do
      {:ok, {files, status2}} ->
        escriptApp = (case (:lists.keyfind(escriptAppName,
                                             r_app(:name), apps)) do
                        false ->
        ...
        {apps2, status3} = escript_files_to_apps(escriptAppName,
                                                   :lists.sort(files),
                                                   [escriptApp], apps, status2)
  ...
  defp escript_files_to_apps(escriptAppName,
            [{appName, type, dir, modOrInfo} | files], acc, apps,
            status) do
    {newAcc, status3} = (case (type) do
                           :mod ->
                             case (acc) do
                               [app | acc2]
                                   when r_app(app, :name) === r_mod(modOrInfo, :app_name)
                                        ->
                                 mods = :lists.ukeymerge(r_mod(:name), [modOrInfo],
                                                           r_app(app, :mods))
    ...
    escript_files_to_apps(escriptAppName, files, newAcc,
                            apps, status3)
  end

  defp escript_files_to_apps(_EscriptAppName, [], acc, apps, status) do
    {:lists.ukeymerge(r_app(:name), :lists.reverse(acc), apps),
       status}
  end

  defp init_escript_app(appName, escriptAppName, dir, info, mods, apps,
            status) do
    ...
    inclCond = r_app(:lists.keyfind(escriptAppName, r_app(:name),
                                  apps), :incl_cond)
    app2 = r_app(app1, is_escript: isEscript, 
                     label: :filename.basename(dir, '.escript'),  info: info, 
                     mods: mods,  active_dir: dir,  sorted_dirs: [dir], 
                     incl_cond: inclCond)
    case (:lists.keymember(appName, r_app(:name), apps)) do
      true ->
        :reltool_utils.throw_error('~w: Application name clash. Escript ~tp contains application ~w.', [appName, dir, appName])
      false ->
        {app2, status}
    end
  end
  ...
  defp merge_app_dirs([{name, dir} | rest], apps) do
    app = (case (:lists.keyfind(name, r_app(:name), apps)) do
             false ->
               default_app(name, dir)
             oldApp ->
               sortedDirs = :lists.umerge(&:reltool_utils.app_dir_test/2,
                                            [dir], r_app(oldApp, :sorted_dirs))
  ...
  defp set_active_dirs([r_app(sorted_dirs: [activeDir | _]) = app |
               apps]) do
  ...
  defp default_app(name) do
    r_app(name: name, is_escript: false, sorted_dirs: [],
        mods: [], status: :missing)
  end

  defp refresh_apps(configApps, [new | newApps], acc, force,
            status) do
    {new2, status3} = (case (:lists.keymember(r_app(new, :name),
                                                r_app(:name), configApps)) do
                         true ->
                           {info, activeDir, status2} = ensure_app_info(new,
                                                                          status)
                           optLabel = (case (r_app_info(info, :vsn) === r_app(new, :vsn)) do
                                         true ->
                                           r_app(new, :label)
                                         false ->
                                           :undefined
                                       end)
                           refresh_app(r_app(new, label: optLabel, 
    ...
    refresh_apps(configApps, newApps, [new2 | acc], force,
                   status3)
  end
  ...
  defp ensure_app_info(r_app(is_escript: isEscript, active_dir: dir,
              info: info),
            status)
  ...
  defp ensure_app_info(r_app(name: name, sorted_dirs: []) = app, status) do
    ...
    case (app) do
      r_app(incl_cond: :exclude, status: :missing,
          active_dir: dir) ->
        status2 = :reltool_utils.add_warning(reason, [name],
                                               status)
  ...
  defp ensure_app_info(r_app(name: name, vsn: vsn,
              use_selected_vsn: useSelectedVsn, active_dir: activeDir,
              sorted_dirs: dirs, info: :undefined, status: appStatus),
            status) do
    readInfo = fn dir, statusAcc ->
                    ...
                    appFile = :filename.join([ebin,
                                                  :erlang.atom_to_list(name) ++ '.app'])
                    read_app_info(appFile, appFile, name, activeDir,
                                    appStatus, defaultVsn, statusAcc)
               end
    {allInfo, status2} = :lists.mapfoldl(readInfo, status,
                                           dirs)
    allVsns = (for i <- allInfo do
                 r_app_info(i, :vsn)
               end)
    status3 = (case (allVsns -- :lists.usort(allVsns)) do
                 [] ->
                   status2
                 [badVsn | _] ->
                   :reltool_utils.throw_error('~w: Application version clash. Multiple directories contain version ~tp.', [name, badVsn])
               end)
          ...
          true ->
            case (find_vsn(vsn, allInfo, dirs)) do
              {info, vsnDir} ->
                {info, vsnDir, status3}
              false ->
                :reltool_utils.throw_error('~w: No application directory contains selected version ~tp', [name, vsn])
            end
        end
      true ->
        {firstInfo, firstDir, status3}
  ...
  defp save_old(r_state(status: oldStatus, sys: oldSys), newS,
            {oldApps, oldMods}, newStatus) do
    ...
    r_state(newS, old_sys: oldSys,  old_status: oldStatus, 
              status: newStatus)
  end
