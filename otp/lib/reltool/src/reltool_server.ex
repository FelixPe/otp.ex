defmodule :m_reltool_server do
  ...
  defp default_sys() do
    r_sys(
      ...
      mod_cond: :all,
      apps: [],
      boot_rel: ~c"start_clean",
      rels: :reltool_utils.default_rels(),
      emu_name: ~c"beam",
      profile: :development,
      incl_sys_filters: dec_re(:incl_sys_filters, [~c".*"], []),
      excl_sys_filters: dec_re(:excl_sys_filters, [], []),
      incl_app_filters: dec_re(:incl_app_filters, [~c".*"], []),
  ...
  defp parse_options([{key, val} | keyVals], s, c, sys) do
    case key do
      ...
      _ ->
        :reltool_utils.throw_error(~c"Illegal option: ~tp", [{key, val}])
    end
  end
  ...
  defp parse_options(keyVals, _S, _C, _Sys) do
    :reltool_utils.throw_error(~c"Illegal option: ~tp", [keyVals])
  end
  ...
  def loop(r_state(sys: sys) = s) do
    receive do
      ...
      {:call, replyTo, ref, {:get_rel, relName}} ->
        reply =
          case :lists.keysearch(relName, r_rel(:name), r_sys(sys, :rels)) do
            ...
            false ->
              {:error, ~c"No such release: " ++ relName}
          end
      ...
      when is_atom(appName) ->
        reply =
          case :ets.lookup(r_state(s, :app_tab), appName) do
            ...
            [] ->
              {:error, ~c"No such application: " ++ :erlang.atom_to_list(appName)}
          end
      ...
      when is_pid(replyTo) and
             is_reference(ref) ->
        :error_logger.format(~c"~w~w got unexpected call:\n\t~tp\n", [
          :reltool_server,
          self(),
          msg
        ])
  ...
  defp app_update_config(
    ...
    :reltool_utils.throw_error(
      ~c"Application ~w is inlined in ~w. Can not change configuration for an inlined application.",
      [name, escript]
    )
  end
      ...
      for rA <- relApps,
          {appName, optional} <-
            (case :lists.keyfind(r_rel_app(rA, :name), r_app(:name), apps) do
               ...
               false ->
                 :reltool_utils.throw_error(
                   ~c"Release ~tp uses non existing application ~w",
  ...
  defp more_apps_in_rels([{relName, appName, optional} | relApps], apps, acc) do
      ...
      false ->
        case :lists.keyfind(appName, r_app(:name), apps) do
          r_app(info: r_app_info(incl_apps: iA) = info) ->
            extra =
              for {childName, childOptional} <- build_more_apps(info, iA) do
          ...
          false ->
            :reltool_utils.throw_error(~c"Release ~tp uses non existing application ~w", [
              relName,
              appName
            ])
        end
  ...
  defp app_init_is_included(
    ...
    {default, isPreIncl, isIncl, status2} =
      case {appCond, rels} do
        ...
        {:exclude, [relName | _]} ->
          :reltool_utils.throw_error(
            ~c"Application ~w is used in release ~tp and cannot be excluded",
            [appName, relName]
          )
          ...
          case {r_mod(existing, :is_included), isIncl} do
            {false, _} ->
              ...
              :reltool_utils.add_warning(
                ~c"Module ~w exists in applications ~w and ~w. Using module from application ~w.",
                [
                  r_mod(m, :name),
            ...
            {_, _} ->
              :reltool_utils.throw_error(
                ~c"Module ~w potentially included by two different applications: ~w and ~w.",
  ...
  defp app_mark_is_included(r_state(app_tab: appTab, mod_tab: modTab, sys: sys) = s, usedByName, [
    ...
    case :ets.lookup(appTab, appName) do
      ...
      [] ->
        :reltool_utils.throw_error(~c"Application ~tp uses non existing application ~w", [
          usedByName,
          appName
        ])
    end
  ...
  defp mod_propagate_is_included(s) do
    case :lists.flatmap(
      ...
      missingMods ->
        missingApp = default_app(:"*MISSING*", ~c"missing")

        missingApp2 =
          r_app(missingApp,
            label: ~c"*MISSING*",
            info: missing_app_info(~c""),
            mods: missingMods,
            status: :missing,
            uses_mods: []
          )
  ...
  defp verify_config(
    ...
    case :lists.keymember(bootRel, r_rel(:name), rels) do
      ...
      false ->
        :reltool_utils.throw_error(~c"Release ~tp is mandatory (used as boot_rel)", [bootRel])
    end
  end
  ...
  defp check_app(appTab, {relName, appName}, status) do
    case :ets.lookup(appTab, appName) do
      ...
      _ ->
        :reltool_utils.throw_error(~c"Release ~tp uses non included application ~w", [
          relName,
          appName
        ])
    end
  ...
  defp check_rel(relName, relApps, status) do
    ensureApp = fn appName, acc ->
      case :lists.member({relName, appName}, relApps) do
        ...
        false ->
          :reltool_utils.throw_error(
            ~c"Mandatory application ~w is not included in release ~tp",
            [appName, relName]
          )
      end
    end
      ...
      force or optLabel === :undefined ->
        {appInfo, ebinMods, status3} =
          case isEscript do
            false ->
              ...
              ebin = :filename.join([activeDir, ~c"ebin"])
              appFile = :filename.join([ebin, :erlang.atom_to_list(appName) ++ ~c".app"])
          ...
          case r_app_info(appInfo, :modules) -- appInfoMods do
            ...
            duplicatedMods ->
              :lists.foldl(
                fn m, s ->
                  :reltool_utils.add_warning(
                    ~c"Module ~w duplicated in app file for application ~w.",
                    [m, appName],
                    s
                  )
                end,
        ...
        appLabel =
          case appVsn do
            ...
            _ ->
              :erlang.atom_to_list(appName) ++ ~c"-" ++ appVsn
          end
    ...
    case :reltool_utils.prim_consult(appFileOrBin) do
      ...
      {:ok, _BadApp} ->
        {missing_app_info(defaultVsn),
         :reltool_utils.add_warning(
           ~c"~w: Illegal contents in app file ~tp, application tuple with arity 3 expected.",
           [appName, appFile],
           status
         )}
      ...
      {:error, text} when text === enoentText ->
        {missing_app_info(defaultVsn),
         :reltool_utils.add_warning(~c"~w: Missing app file ~tp.", [appName, appFile], status)}

      {:error, text} ->
        {missing_app_info(defaultVsn),
         :reltool_utils.add_warning(
           ~c"~w: Cannot parse app file ~tp (~tp).",
  ...
  defp parse_app_info(file, [{key, val} | keyVals], aI, status) do
    case key do
      ...
      :optional_applications ->
        parse_app_info(file, keyVals, r_app_info(aI, opt_apps: val), status)
      ...
      _ ->
        status2 =
          :reltool_utils.add_warning(
            ~c"Unexpected item ~tp in app file ~tp.",
            [key, file],
            status
          )
  ...
  defp xref_mod({base, bin}) when is_binary(bin) do
    dir = :filename.absname(~c"reltool_server.tmp")
  ...
  defp xref_mod(file) when is_list(file) do
    ...
    {:ok, unknownMods} = :xref.q(pid, ~c"UM", [])
    ...
    after
      :timer.seconds(30) ->
        :error_logger.error_msg(
          ~c"~w(~w): Waiting for process ~w to die ~tp\n",
  ...
  defp read_config(oldSys, filename) when is_list(filename) do
    case :file.consult(filename) do
      ...
      {:ok, content} ->
        :reltool_utils.throw_error(~c"Illegal file content: ~p", [content])

      {:error, reason} ->
        :reltool_utils.throw_error(
          ~c"Illegal config file ~tp: ~ts",
  ...
  defp read_config(oldSys, {:sys, keyVals}) do
    ...
    case :lists.keymember(r_sys(newSys2, :boot_rel), r_rel(:name), r_sys(newSys2, :rels)) do
      ...
      false ->
        :reltool_utils.throw_error(~c"Release ~tp is mandatory (used as boot_rel)", [
  ...
  defp read_config(_OldSys, badConfig) do
    :reltool_utils.throw_error(~c"Illegal content: ~tp", [badConfig])
  end
      ...
      :lists.foldl(
        fn opt, rel0 ->
          case opt do
            ...
            _ ->
              :reltool_utils.throw_error(~c"Illegal rel option: ~tp", [opt])
          end
        end,
    ...
    sys3 =
      case key do
        ...
        :incl_archive_filters ->
          :io.format(~c"incl_archive_filters is no longer supported in reltool")

        :excl_archive_filters ->
          :io.format(~c"excl_archive_filters is no longer supported in reltool")

        :archive_opts when is_list(val) ->
          :io.format(~c"archive_opts is no longer supported in reltool")
      ...
      case key do
        ...
        when is_list(val) and
          ...
          case :filelib.is_dir(val) do
            ...
            false ->
              :reltool_utils.throw_error(~c"Illegal lib dir for ~w: ~tp", [r_app(app, :name), val])
          end

        selectVsn
        when selectVsn === :vsn or
               selectVsn === :lib_dir ->
          :reltool_utils.throw_error(~c"Mutual exclusive options 'vsn' and 'lib_dir'", [])
  ...
  defp decode(
    ...
    case validTypesAssigned do
      ...
      false ->
        :reltool_utils.throw_error(~c"Illegal option: ~tp", [relApp])
    end
  end
  ...
  defp decode(_Acc, keyVal) do
    :reltool_utils.throw_error(~c"Illegal option: ~tp", [keyVal])
  end
  ...
  defp split_escript_name(file) when is_list(file) do
    label = :filename.basename(file, ~c".escript")
    {:erlang.list_to_atom(~c"*escript* " ++ label), label}
  end

  defp default_escript_app(file) do
    {name, label} = split_escript_name(file)
    app = default_app(name, file)
    r_app(app, is_escript: true, label: label, info: missing_app_info(~c""))
  end
  ...
  defp patch_erts_version(rootDir, apps, status) do
    ...
    case :lists.keyfind(appName, r_app(:name), apps) do
      r_app(vsn: vsn) = erts ->
        ...
        cond do
          localRoot === rootDir and vsn === ~c"" ->
            vsn2 = :erlang.system_info(:version)
            erts2 = r_app(erts, vsn: vsn2, label: ~c"erts-" ++ vsn2)
            apps2 = :lists.keystore(appName, r_app(:name), apps, erts2)
            {apps2, status}

          vsn === ~c"" ->
            {apps, :reltool_utils.add_warning(~c"erts has no version", [], status)}
      ...
      false ->
        :reltool_utils.throw_error(~c"erts cannot be found in the root directory ~tp", [rootDir])
    end
  end
  ...
  defp libs_to_dirs(rootDir, libDirs) do
    case :file.list_dir(rootDir) do
      {:ok, rootFiles} ->
        rootLibDir = :filename.join([rootDir, ~c"lib"])
        ...
        case allLibDirs -- :lists.usort(allLibDirs) do
          [] ->
            fun = fn base ->
              ...
              case :filelib.is_dir(
                ...
                false ->
                  :filename.join([rootDir, base, ~c"preloaded"])
              end
            end

            ertsFiles =
              for f <- rootFiles, :lists.prefix(~c"erts", f) do
                {:erts, fun.(f)}
              end
          ...
          [duplicate | _] ->
            :reltool_utils.throw_error(~c"Duplicate library: ~tp", [duplicate])
        end

      {:error, reason} ->
        :reltool_utils.throw_error(
          ~c"Missing root library ~tp: ~ts",
    ...
    case :file.list_dir(lib) do
      {:ok, files} ->
        filter = fn base ->
          ...
          ebinDir = :filename.join([appDir, ~c"ebin"])
  ...
  defp get_vsn_from_dir(appName, base) do
    prefix = :erlang.atom_to_list(appName) ++ ~c"-"
  ...
  defp escripts_to_apps([escript | escripts], apps, status) do
    ...
    appFiles =
      case :reltool_utils.escript_foldl(appFun, [], escript) do
        ...
        {:error, reason1} ->
          :reltool_utils.throw_error(~c"Illegal escript ~tp: ~tp", [escript, reason1])
      end
    ...
    case :reltool_utils.escript_foldl(fun, {[], status}, escript) do
      ...
      {:error, reason2} ->
        :reltool_utils.throw_error(~c"Illegal escript ~tp: ~tp", [escript, reason2])
    end
  end
  ...
  defp init_escript_app(appName, escriptAppName, dir, info, mods, apps, status) do
    ...
    app2 =
      r_app(app1,
        is_escript: isEscript,
        label: :filename.basename(dir, ~c".escript"),
        info: info,
        mods: mods,
        active_dir: dir,
        sorted_dirs: [dir],
    ...
    case :lists.keymember(appName, r_app(:name), apps) do
      true ->
        :reltool_utils.throw_error(
          ~c"~w: Application name clash. Escript ~tp contains application ~w.",
          [appName, dir, appName]
        )
  ...
  defp ensure_app_info(
    ...
    readInfo = fn dir, statusAcc ->
      ...
      ebin = :filename.join([dir, ~c"ebin"])
      defaultVsn = get_vsn_from_dir(name, base)
      appFile = :filename.join([ebin, :erlang.atom_to_list(name) ++ ~c".app"])
    ...
    status3 =
      case allVsns -- :lists.usort(allVsns) do
        ...
        [badVsn | _] ->
          :reltool_utils.throw_error(
            ~c"~w: Application version clash. Multiple directories contain version ~tp.",
            [name, badVsn]
          )
      end
        ...
        cond do
          ...
          true ->
            case find_vsn(vsn, allInfo, dirs) do
              ...
              false ->
                :reltool_utils.throw_error(
                  ~c"~w: No application directory contains selected version ~tp",
                  [name, vsn]
                )
            end
        end
