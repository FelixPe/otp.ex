defmodule :m_reltool_target do
  ...
  defp do_gen_config(r_sys(root_dir: rootDir, lib_dirs: libDirs,
              mod_cond: modCond, incl_cond: appCond, apps: apps,
              boot_rel: bootRel, rels: rels, emu_name: emuName,
              profile: profile, incl_sys_filters: inclSysFiles,
              excl_sys_filters: exclSysFiles,
              incl_app_filters: inclAppFiles,
              excl_app_filters: exclAppFiles,
              relocatable: relocatable, rel_app_type: relAppType,
              embedded_app_type: inclAppType, app_file: appFile,
              debug_info: debugInfo),
            inclDefs) do
    ertsItems = (case (:lists.keyfind(:erts, r_app(:name),
                                        apps)) do
                   false ->
                     []
                   erts ->
    ...
    appsItems = (for a <- apps, r_app(a, :name) !== :"*MISSING*",
                       r_app(a, :name) !== :erts, r_app(a, :is_escript) !== true do
                   do_gen_config(a, inclDefs)
                 end)
    escriptItems = (for a <- apps, r_app(a, :is_escript) do
                      {:escript, r_app(a, :active_dir),
                         emit(:incl_cond, r_app(a, :incl_cond), :undefined,
                                inclDefs)}
                    end)
    defaultRels = :reltool_utils.default_rels()
    relsItems = (for r <- rels do
                   do_gen_config(r, inclDefs)
                 end)
    defaultRelsItems = (for r <- defaultRels do
                          do_gen_config(r, inclDefs)
                        end)
    relsItems2 = (case (inclDefs) do
                    true ->
                      relsItems
                    false ->
    ...
    {:sys,
       emit(:root_dir, rootDir, :code.root_dir(),
              inclDefs) ++ emit(:lib_dirs, libDirs, [],
                                  inclDefs) ++ escriptItems ++ emit(:mod_cond,
                                                                      modCond,
                                                                      :all,
                                                                      inclDefs) ++ emit(:incl_cond,
                                                                                          appCond,
                                                                                          :derived,
                                                                                          inclDefs) ++ ertsItems ++ :lists.flatten(appsItems) ++ emit(:boot_rel,
                                                                                                                                                        bootRel,
                                                                                                                                                        'start_clean',
                                                                                                                                                        inclDefs) ++ relsItems2 ++ emit(:emu_name,
                                                                                                                                                                                          emuName,
                                                                                                                                                                                          'beam',
                                                                                                                                                                                          inclDefs) ++ emit(:relocatable,
                                                                                                                                                                                                              relocatable,
                                                                                                                                                                                                              true,
                                                                                                                                                                                                              inclDefs) ++ emit(:profile,
                                                                                                                                                                                                                                  profile,
                                                                                                                                                                                                                                  :development,
                                                                                                                                                                                                                                  inclDefs) ++ emit(:incl_sys_filters,
                                                                                                                                                                                                                                                      x.(inclSysFiles),
                                                                                                                                                                                                                                                      :reltool_utils.choose_default(:incl_sys_filters,
                                                                                                                                                                                                                                                                                      profile,
                                                                                                                                                                                                                                                                                      inclDefs),
                                                                                                                                                                                                                                                      inclDefs) ++ emit(:excl_sys_filters,
                                                                                                                                                                                                                                                                          x.(exclSysFiles),
                                                                                                                                                                                                                                                                          :reltool_utils.choose_default(:excl_sys_filters,
                                                                                                                                                                                                                                                                                                          profile,
                                                                                                                                                                                                                                                                                                          inclDefs),
                                                                                                                                                                                                                                                                          inclDefs) ++ emit(:incl_app_filters,
                                                                                                                                                                                                                                                                                              x.(inclAppFiles),
                                                                                                                                                                                                                                                                                              :reltool_utils.choose_default(:incl_app_filters,
                                                                                                                                                                                                                                                                                                                              profile,
                                                                                                                                                                                                                                                                                                                              inclDefs),
                                                                                                                                                                                                                                                                                              inclDefs) ++ emit(:excl_app_filters,
                                                                                                                                                                                                                                                                                                                  x.(exclAppFiles),
                                                                                                                                                                                                                                                                                                                  :reltool_utils.choose_default(:excl_app_filters,
                                                                                                                                                                                                                                                                                                                                                  profile,
                                                                                                                                                                                                                                                                                                                                                  inclDefs),
                                                                                                                                                                                                                                                                                                                  inclDefs) ++ emit(:rel_app_type,
                                                                                                                                                                                                                                                                                                                                      relAppType,
                                                                                                                                                                                                                                                                                                                                      :permanent,
                                                                                                                                                                                                                                                                                                                                      inclDefs) ++ emit(:embedded_app_type,
                                                                                                                                                                                                                                                                                                                                                          inclAppType,
                                                                                                                                                                                                                                                                                                                                                          :reltool_utils.choose_default(:embedded_app_type,
                                                                                                                                                                                                                                                                                                                                                                                          profile,
                                                                                                                                                                                                                                                                                                                                                                                          inclDefs),
                                                                                                                                                                                                                                                                                                                                                          inclDefs) ++ emit(:app_file,
                                                                                                                                                                                                                                                                                                                                                                              appFile,
                                                                                                                                                                                                                                                                                                                                                                              :keep,
                                                                                                                                                                                                                                                                                                                                                                              inclDefs) ++ emit(:debug_info,
                                                                                                                                                                                                                                                                                                                                                                                                  debugInfo,
                                                                                                                                                                                                                                                                                                                                                                                                  :keep,
                                                                                                                                                                                                                                                                                                                                                                                                  inclDefs)}
  end

  defp do_gen_config(r_app(name: name, mod_cond: modCond,
              incl_cond: appCond, debug_info: debugInfo,
              app_file: appFile, incl_app_filters: inclAppFiles,
              excl_app_filters: exclAppFiles,
              use_selected_vsn: useSelected, vsn: vsn,
              active_dir: activeDir, mods: mods, is_included: isIncl),
            inclDefs) do
    appConfig = [emit(:mod_cond, modCond, :undefined,
                        inclDefs),
                     emit(:incl_cond, appCond, :undefined, inclDefs),
                         emit(:debug_info, debugInfo, :undefined, inclDefs),
                             emit(:app_file, appFile, :undefined, inclDefs),
                                 emit(:incl_app_filters, inclAppFiles,
                                        :undefined, inclDefs),
                                     emit(:excl_app_filters, exclAppFiles,
                                            :undefined, inclDefs),
                                         cond do
                                           (isIncl and inclDefs) ->
                                             [{:vsn, vsn}, {:lib_dir,
  ...
  defp do_gen_config(r_mod(name: name, incl_cond: appCond,
              debug_info: debugInfo, is_included: isIncl),
            inclDefs) do
    modConfig = [emit(:incl_cond, appCond, :undefined,
                        inclDefs),
                     emit(:debug_info, debugInfo, :undefined, inclDefs)]
  ...
  defp do_gen_config(r_rel(name: name, vsn: vsn, rel_apps: relApps,
              load_dot_erlang: loadDotErlang),
            inclDefs) do
    relAppsConfig = (for rA <- relApps do
  ...
  defp do_gen_config(r_rel_app(name: name, app_type: type,
              incl_apps: inclApps),
            _InclDefs) do
  ...
  defp do_gen_config([h | t], inclDefs) do
    :lists.flatten([do_gen_config(h, inclDefs),
                        do_gen_config(t, inclDefs)])
  end
  ...
  def gen_app(r_app(name: name,
             info: r_app_info(description: desc, id: id, vsn: vsn,
                       modules: mods, maxP: maxP, maxT: maxT, registered: regs,
                       opt_apps: optApps, incl_apps: inclApps,
                       applications: reqApps, env: env, mod: startMod,
                       start_phases: startPhases))) do
    startPhases2 = (case (startPhases) do
                      :undefined ->
                        []
                      _ ->
                        [{:start_phases, startPhases}]
                    end)
    tail = (case (startMod) do
              :undefined ->
                startPhases2
              _ ->
    ...
    {:application, name,
       [{:description, desc}, {:vsn, vsn}, {:id, id},
                                               {:modules, mods}, {:registered,
                                                                    regs},
                                                                     {:applications,
                                                                        reqApps},
  ...
  defp do_gen_rel(r_rel(name: relName, vsn: relVsn,
              rel_apps: relApps),
            r_sys(apps: apps), mergedApps) do
    ertsName = :erts
  ...
  defp strip_rel_info(r_app(name: name, vsn: vsn, app_type: type,
              info: r_app_info(incl_apps: appInclApps)),
            relApps)
      when type !== :undefined do
    relInclApps = (case (:lists.keyfind(name, r_rel_app(:name),
                                          relApps)) do
  ...
  defp merge_apps(r_rel(name: relName, rel_apps: relApps),
            r_sys(apps: apps, rel_app_type: relAppType,
                embedded_app_type: embAppType)) do
    mandatory = [:kernel, :stdlib]
    mergedApps = do_merge_apps(relName, mandatory, apps,
                                 :permanent, [])
    mergedApps2 = do_merge_apps(relName, relApps, apps,
                                  relAppType, mergedApps)
    embedded = (for a <- apps, embAppType !== :undefined,
                      r_app(a, :is_included), r_app(a, :name) !== :erts,
                      r_app(a, :name) !== :"*MISSING*",
                      not
                      :lists.keymember(r_app(a, :name), r_app(:name), mergedApps2) do
                  r_app(a, :name)
                end)
    mergedApps3 = do_merge_apps(relName, embedded, apps,
                                  embAppType, mergedApps2)
    revMerged = :lists.reverse(mergedApps3)
    mergedSortedUsedAndIncs = sort_used_and_incl_apps(revMerged,
                                                        revMerged)
    sort_apps(mergedSortedUsedAndIncs)
  end

  defp do_merge_apps(relName, [r_rel_app(name: name) = rA | relApps], apps,
            relAppType, acc) do
    case (is_already_merged(name, relApps, acc)) do
      ...
      false ->
        ...
        r_app(info: info) = (mergedApp = merge_app(relName, rA,
                                                 relAppType, app))
        r_app_info(applications: children, incl_apps: incNames,
            opt_apps: optNames) = info
        reqNames = (for childName <- children,
                          not :lists.member(childName, optNames),
                          :lists.keymember(childName, r_app(:name), apps) do
                      childName
                    end)
  ...
  defp do_merge_apps(relName, [name | relApps], apps, relAppType,
            acc) do
    case (is_already_merged(name, relApps, acc)) do
      true ->
        do_merge_apps(relName, relApps, apps, relAppType, acc)
      false ->
        ...
        do_merge_apps(relName, [relApp | relApps], apps,
                        relAppType, acc)
    end
  end
  ...
  defp merge_app(relName,
            r_rel_app(name: name, app_type: type, incl_apps: inclApps0),
            relAppType, app) do
    type2 = (case ({type, r_app(app, :app_type)}) do
               {:undefined, :undefined} ->
                 relAppType
    ...
    case (inclApps -- r_app_info(info, :incl_apps)) do
      [] ->
        r_app(app, app_type: type2, 
                 info: r_app_info(info, incl_apps: inclApps))
      badIncl ->
        :reltool_utils.throw_error('~w: These applications are used by release ~ts but are missing as included_applications in the app file: ~p', [name, relName, badIncl])
    end
  end
  ...
  defp do_gen_script(r_rel(name: relName, vsn: relVsn,
              load_dot_erlang: loadErlangRc),
            r_sys(apps: apps), mergedApps, pathFlag, variables) do
    {:value, erts} = :lists.keysearch(:erts, r_app(:name), apps)
    preloaded = (for mod <- r_app(erts, :mods) do
                   r_mod(mod, :name)
                 end)
    mandatory = mandatory_modules()
    early = mandatory ++ preloaded
    {:value, kernelApp} = :lists.keysearch(:kernel,
                                             r_app(:name), mergedApps)
    inclApps = :lists.flatmap(fn r_app(info: r_app_info(incl_apps: i)) ->
                                   i
                              end,
                                mergedApps)
    deepList = [{:preLoaded, :lists.sort(preloaded)},
                    {:progress, :preloaded}, {:path,
                                                create_mandatory_path(mergedApps,
                                                                        pathFlag,
                                                                        variables)},
                                                 {:primLoad,
                                                    :lists.sort(mandatory)},
                                                             ...
                                                             for a <- mergedApps do
                                                               load_app_mods(a,
                                                                 ...
                                                                 {:progress,
                                                                    :modules_loaded},
                                                                     {:path,
                                                                        create_path(mergedApps,
                                                                                      pathFlag,
                                                                                      variables)},
                                                                         kernel_processes(gen_app(kernelApp)),
                                                                             {:progress,
                                                                                :init_kernel_started},
                                                                                 for (a = r_app(name: name,
                                                                                              app_type: type)) <- mergedApps,
                                                                                       name !== :kernel,
                                                                                       type !== :none do
                                                                                   {:apply,
                                                                                      {:application,
                                                                                             ...
                                                                                             case (loadErlangRc) do
                                                                                               ...
                                                                                               false ->
                                                                                                 ...
                                                                                                 {:progress,
                                                                                                    :started}]
    {:ok,
       {:script, {relName, relVsn}, :lists.flatten(deepList)}}
  end

  defp load_app_mods(r_app(mods: mods0) = app, mand, pathFlag,
            variables) do
    path = cr_path(app, pathFlag, variables)
    mods = (for r_mod(name: m, is_included: true) <- mods0,
                  not :lists.member(m, mand) do
              m
            end)
    [{:path, [:filename.join([path])]}, {:primLoad,
                                           :lists.sort(mods)}]
  end

  defp sort_used_and_incl_apps([r_app(info: info) = app | apps], orderedApps) do
    incls2 = (case (r_app_info(info, :incl_apps)) do
                incls when length(incls) > 1 ->
                  sort_appl_list(incls, orderedApps)
                incls ->
                  incls
              end)
    uses2 = (case (r_app_info(info, :applications)) do
               uses when length(uses) > 1 ->
                 sort_appl_list(uses, orderedApps)
               uses ->
                 uses
             end)
    app2 = r_app(app, info: r_app_info(info, incl_apps: incls2, 
                                  applications: uses2))
  ...
  defp find_pos([name | incs], orderedApps) do
    [find_pos(1, name, orderedApps) | find_pos(incs,
                                                 orderedApps)]
  end
  ...
  defp sort_apps([r_app(name: name, info: info) = app | apps],
            missing, circular, visited) do
    {uses, apps1, notFnd1} = find_all(name,
                                        :lists.reverse(r_app_info(info, :applications)),
                                        apps, visited, [], [])
    {incs, apps2, notFnd2} = find_all(name,
                                        :lists.reverse(r_app_info(info, :incl_apps)),
                                        apps1, visited, [], [])
    missing1 = (notFnd1 -- r_app_info(info, :opt_apps)) ++ notFnd2 ++ missing
    case (uses ++ incs) do
      [] ->
        [app | sort_apps(apps, missing1, circular,
                           [name | visited])]
      l ->
        newCircular = (for r_app(name: n) <- l, n2 <- visited,
                             n === n2 do
                         n
                       end)
        circular1 = (case (newCircular) do
                       [] ->
                         circular
                       _ ->
  ...
  defp sort_apps([], missing, circular, _) do
    :reltool_utils.throw_error('Circular dependencies: ~pUndefined applications: ~p\n',
                                 [make_set(circular), make_set(missing)])
  end

  defp find_all(checkingApp, [name | names], apps, visited,
            found, notFound) do
    case (:lists.keyfind(name, r_app(:name), apps)) do
      r_app(info: info) = app ->
        case (:lists.member(checkingApp,
                              r_app_info(info, :incl_apps))) do
          true ->
            case (:lists.member(name, visited)) do
              true ->
                find_all(checkingApp, names, apps, visited, found,
                           notFound)
              false ->
                find_all(checkingApp, names, apps, visited, found,
                           [name | notFound])
          ...
          false ->
            find_all(checkingApp, names, apps -- [app], visited,
                       [app | found], notFound)
        end
      false ->
        case (:lists.member(name, visited)) do
  ...
  defp find_all(_CheckingApp, [], apps, _Visited, found,
            notFound) do
    {found, apps, notFound}
  end
  ...
  defp cr_path(r_app(name: name, vsn: vsn, label: label,
              active_dir: dir),
            true, variables) do
    tail = [label, 'ebin']
    case (variable_dir(dir, :erlang.atom_to_list(name), vsn,
                         variables)) do
      {:ok, varDir} ->
  ...
  defp create_mandatory_path(apps, pathFlag, variables) do
    mandatory = [:kernel, :stdlib]
    make_set(:lists.map(fn r_app(name: name) = app ->
  ...
  defp do_spec_rel_files(r_rel(name: relName) = rel, sys) do
    ...
    variables = (case (r_sys(sys, :excl_lib)) do
                   :otp_root ->
                     (for libDir <- r_sys(sys, :lib_dirs) do
                        {'RELTOOL_EXT_LIB', libDir}
                      end) ++ (for r_app(active_dir: appLibDir,
                                       use_selected_vsn: :dir) <- mergedApps do
    ...
    pathFlag = true
    {:ok, script} = do_gen_script(rel, sys, mergedApps,
                                    pathFlag, variables)
    {:ok, bootBin} = gen_boot(script)
    relIoList = :io_lib.format('~tp.\n\n', [genRel])
    scriptIoList = :io_lib.format('~tp.\n\n', [script])
    [{:write_file, relFile,
        to_utf8_bin_with_enc_comment(relIoList)},
         {:write_file, scriptFile,
            to_utf8_bin_with_enc_comment(scriptIoList)},
  ...
  defp do_gen_spec(r_sys(root_dir: rootDir, excl_lib: exclLib,
              incl_sys_filters: inclRegexps,
              excl_sys_filters: exclRegexps, relocatable: relocatable,
              apps: apps) = sys) do
    relFiles = spec_rel_files(sys)
    {sysFiles, inclRegexps2, exclRegexps2,
       mandatory} = (case (exclLib) do
                       :otp_root ->
                         {[], inclRegexps, exclRegexps, ['lib']}
                       _ ->
                         {:create_dir, _, sF} = spec_dir(rootDir)
                         {eR2, sF2} = strip_sys_files(relocatable, sF, apps,
                                                        exclRegexps)
                         {iR2, binFiles} = spec_bin_files(sys, sF, sF2,
                                                            relFiles,
                                                            inclRegexps)
    ...
    sysFiles2 = [{:create_dir, 'releases',
                    [startFile, {:create_dir, bootVsn,
                                   relFiles}]}] ++ sysFiles
    sysFiles3 = filter_spec(sysFiles2, inclRegexps2,
                              exclRegexps2)
  ...
  defp strip_sys_files(relocatable, sysFiles, apps, exclRegexps) do
    exclRegexps2 = (case (relocatable) do
                      true ->
    ...
    filterErts = fn spec ->
                      ...
                      case (file) do
                        'erts' ->
                          :reltool_utils.throw_error('This system is not installed. The directory ~ts is missing.', [r_app(erts, :label)])
    ...
    sysFiles3 = :lists.foldl(fn f, acc ->
                                  :lists.keydelete(f, 2, acc)
                             end,
                               sysFiles2, ['releases', 'lib', 'bin'])
  ...
  defp replace_dyn_erl(true, {:create_dir, ertsDir, ertsFiles}) do
    [{:create_dir, _, binFiles}] = safe_lookup_spec('bin',
                                                      ertsFiles)
    case (lookup_spec('dyn_erl', binFiles)) do
      [] ->
        case (lookup_spec('erl.ini', binFiles)) do
          ...
          [{:copy_file, erlIni}] ->
            ...
            ertsFiles2 = :lists.keyreplace('bin', 2, ertsFiles,
      ...
      [{:copy_file, dynErlExe}] ->
        ...
        binFiles3 = :lists.keyreplace(erlExe, 2, binFiles2,
  ...
  defp spec_bin_files(sys, allSysFiles, strippedSysFiles, relFiles,
            inclRegexps) do
    [{:create_dir, ertsLabel,
        ertsFiles}] = safe_lookup_spec('erts', strippedSysFiles)
    [{:create_dir, _, binFiles}] = safe_lookup_spec('bin',
                                                      ertsFiles)
    ertsBin = :filename.join([ertsLabel, 'bin'])
    escripts = spec_escripts(sys, ertsBin, binFiles)
    map = fn {:copy_file, file} ->
               {:copy_file, file, :filename.join([ertsBin, file])}
             {:copy_file, newFile, oldFile} ->
               {_, oldFile2} = abs_to_rel_path(ertsBin,
                                                 :filename.join([ertsBin,
                                                                     oldFile]))
               {:copy_file, newFile, oldFile2}
          end
    [{:create_dir, _, oldBinFiles}] = safe_lookup_spec('bin',
                                                         allSysFiles)
    goodNames = (for {:copy_file, f} <- oldBinFiles,
                       not :lists.suffix('.boot', f), not :lists.suffix('.script', f) do
                   f
                 end)
    binFiles2 = (for s <- binFiles,
                       :lists.member(:erlang.element(2, s), goodNames) do
                   map.(s)
                 end)
    bootFiles = (for f <- relFiles,
                       :lists.suffix('.boot', :erlang.element(2, f)) do
                   f
                 end)
    [{:write_file, _,
        bootRel}] = safe_lookup_spec(r_sys(sys, :boot_rel) ++ '.boot',
                                       bootFiles)
    bootFiles2 = :lists.keystore('start.boot', 2, bootFiles,
  ...
  defp spec_escripts(r_sys(apps: apps), ertsBin, binFiles) do
    filter = fn r_app(is_escript: isEscript,
                    is_included: isIncl, is_pre_included: isPre, name: name,
                    active_dir: file) ->
                  cond do
                    name === :"*MISSING*" ->
                      false
  ...
  defp do_spec_escript(file, ertsBin, binFiles) do
    [{:copy_file, escriptExe}] = safe_lookup_spec('escript',
                                                    binFiles)
    escriptExt = '.escript'
    base = :filename.basename(file, escriptExt)
    exeExt = :filename.extension(escriptExe)
    [{:copy_file, base ++ escriptExt, file}, {:copy_file,
                                                base ++ exeExt,
                                                :filename.join([ertsBin,
                                                                    escriptExe])}]
  end
  ...
  defp spec_start_file(r_sys(boot_rel: bootRelName, rels: rels,
              apps: apps)) do
    {:value, erts} = :lists.keysearch(:erts, r_app(:name), apps)
    {:value, bootRel} = :lists.keysearch(bootRelName,
                                           r_rel(:name), rels)
    data = r_app(erts, :vsn) ++ ' ' ++ r_rel(bootRel, :vsn) ++ '\n'
    {r_rel(bootRel, :vsn),
       {:write_file, 'start_erl.data', :unicode.characters_to_binary(data)}}
  end
  ...
  defp spec_lib_files(r_sys(root_dir: rootDir, apps: apps,
              excl_lib: exclLib) = sys) do
    filter = fn r_app(is_escript: isEscript,
                    is_included: isIncl, is_pre_included: isPre, name: name,
    ...
    :lists.flatten(for app <- selectedApps do
                     spec_app(app, sys)
                   end)
  end
  ...
  defp check_apps([mandatory | names], apps) do
    case (:lists.keymember(mandatory, r_app(:name), apps)) do
      false ->
        :reltool_utils.throw_error('Mandatory application ~w is not included in ~p', [mandatory, apps])
      true ->
  ...
  defp spec_app(r_app(label: label, name: name, mods: mods,
              active_dir: sourceDir, incl_app_filters: appInclRegexps,
              excl_app_filters: appExclRegexps) = app,
            r_sys(root_dir: rootDir, incl_app_filters: sysInclRegexps,
                excl_app_filters: sysExclRegexps,
                debug_info: sysDebugInfo) = sys) do
    {:create_dir, _, appFiles} = spec_dir(sourceDir)
    appUpFilename = :erlang.atom_to_list(name) ++ '.appup'
    ebinDir = :filename.join([sourceDir, 'ebin'])
    optAppUpFileSpec = spec_opt_copy_file(ebinDir,
                                            appUpFilename)
    optAppFileSpec = spec_app_file(app, sys, ebinDir)
    modSpecs = (for m <- mods, r_mod(m, :is_included),
                      r_mod(m, :exists) do
                  spec_mod(m, sysDebugInfo)
                end)
    newEbin = {:create_dir, 'ebin',
                 optAppUpFileSpec ++ optAppFileSpec ++ modSpecs}
    appFiles2 = :lists.keystore('ebin', 2, appFiles, newEbin)
    inclRegexps = :reltool_utils.default_val(appInclRegexps,
                                               sysInclRegexps)
    exclRegexps = :reltool_utils.default_val(appExclRegexps,
                                               sysExclRegexps)
    appFiles3 = filter_spec(appFiles2, inclRegexps,
                              exclRegexps)
  ...
  defp spec_mod(mod, debugInfo) do
    ...
    case (:reltool_utils.default_val(r_mod(mod, :debug_info),
                                       debugInfo)) do
      :keep ->
        {:copy_file, file}
      :strip ->
  ...
  defp spec_app_file(r_app(name: name, info: info, mods: mods,
              app_file: appFile) = app,
            r_sys(app_file: sysAppFile), ebinDir) do
    appFilename = :erlang.atom_to_list(name) ++ '.app'
    case (:reltool_utils.default_val(appFile,
                                       sysAppFile)) do
      :keep ->
        spec_opt_copy_file(ebinDir, appFilename)
      :strip ->
        modNames = (for m <- mods, r_mod(m, :is_included),
        ...
        [{:write_file, appFilename,
            to_utf8_bin_with_enc_comment(appIoList)}]
      :all ->
        modNames = (for m <- mods, r_mod(m, :is_included) do
                      r_mod(m, :name)
                    end)
        app2 = r_app(app, info: r_app_info(info, modules: modNames))
        contents = gen_app(app2)
  ...
  defp spec_opt_copy_file(dirName, baseName) do
    case (:filelib.is_regular(:filename.join([dirName,
                                                  baseName]),
                                :erl_prim_loader)) do
      true ->
        [{:copy_file, baseName}]
  ...
  defp do_eval_spec({:create_dir, dir, files}, origSourceDir,
            sourceDir, targetDir) do
    sourceDir2 = :filename.join([sourceDir, dir])
    targetDir2 = :filename.join([targetDir, dir])
    :reltool_utils.create_dir(targetDir2)
    do_eval_spec(files, origSourceDir, sourceDir2,
                   targetDir2)
  end

  defp do_eval_spec({:create_dir, dir, oldDir, files},
            origSourceDir, _SourceDir, targetDir) do
  ...
  defp do_eval_spec({:copy_file, file}, _OrigSourceDir, sourceDir,
            targetDir) do
  ...
  defp do_eval_spec({:copy_file, file, oldFile}, origSourceDir,
            _SourceDir, targetDir) do
    sourceFile = :filename.join([origSourceDir, oldFile])
    targetFile = :filename.join([targetDir, file])
    :reltool_utils.copy_file(sourceFile, targetFile)
  end

  defp do_eval_spec({:write_file, file, bin}, _OrigSourceDir,
            _SourceDir, targetDir) do
    targetFile = :filename.join([targetDir, file])
    :reltool_utils.write_file(targetFile, bin)
  end

  defp do_eval_spec({:strip_beam, file, chunkIds}, _OrigSourceDir,
            sourceDir, targetDir) do
    sourceFile = :filename.join([sourceDir, file])
    targetFile = :filename.join([targetDir, file])
  ...
  defp cleanup_spec({:create_dir, dir, _OldDir, files},
            targetDir) do
    targetDir2 = :filename.join([targetDir, dir])
    cleanup_spec(files, targetDir2)
    :file.del_dir(targetDir2)
  ...
  defp cleanup_spec({:strip_beam, file, _ChunkIds}, targetDir) do
    targetFile = :filename.join([targetDir, file])
    :file.delete(targetFile)
  end
  ...
  defp do_filter_spec(path, {:create_dir, dir, files}, inclRegexps,
            exclRegexps) do
    path2 = opt_join(path, dir)
    case (do_filter_spec(path2, files, inclRegexps,
                           exclRegexps)) do
      [] ->
  ...
  defp do_filter_spec(path, {:create_dir, newDir, oldDir, files},
            inclRegexps, exclRegexps) do
  ...
  defp do_filter_spec(path, {:copy_file, file}, inclRegexps,
            exclRegexps) do
    path2 = opt_join(path, file)
    match(path2, inclRegexps, exclRegexps)
  end

  defp do_filter_spec(path, {:copy_file, newFile, _OldFile},
            inclRegexps, exclRegexps) do
    path2 = opt_join(path, newFile)
    match(path2, inclRegexps, exclRegexps)
  end

  defp do_filter_spec(path, {:write_file, file, _}, inclRegexps,
            exclRegexps) do
    path2 = opt_join(path, file)
    match(path2, inclRegexps, exclRegexps)
  end

  defp do_filter_spec(path, {:strip_beam, file, _ChunkIds},
            inclRegexps, exclRegexps) do
    path2 = opt_join(path, file)
    match(path2, inclRegexps, exclRegexps)
  end
  ...
  defp match(string, inclRegexps, exclRegexps) do
    match(string, inclRegexps) and not
                                   match(string, exclRegexps)
  end
  ...
  defp match(string,
            [r_regexp(source: _, compiled: mP) | regexps]) do
  ...
  defp do_install(relName, targetDir) do
    ...
    case (:string.lexemes(:unicode.characters_to_list(bin),
                            ' \n')) do
      [erlVsn, relVsn | _] ->
        ertsBinDir = :filename.join([targetDir2, 'erts-' ++ erlVsn,
                                                     'bin'])
        ...
        case (:os.type()) do
          ...
          _ ->
            subst_src_scripts(start_scripts(), ertsBinDir, binDir,
                                [{'FINAL_ROOTDIR', targetDir2}, {'EMU', 'beam'}], [:preserve])
        end
        relFile = :filename.join([relDir, relVsn, relName ++ '.rel'])
        :ok = :release_handler.create_RELEASES(targetDir2,
                                                 relFile)
        :ok
      _ ->
  ...
  defp subst([?%, c | rest], vars, result) when (?A <= c and
                                                c <= ?Z) do
