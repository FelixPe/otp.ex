defmodule :m_reltool_target do
  ...
  defp do_gen_rel(
    ...
    case :lists.keysearch(ertsName, r_app(:name), apps) do
      ...
      false ->
        :reltool_utils.throw_error(~c"Mandatory application ~w is not included", [ertsName])
    end
  end
  ...
  defp do_merge_apps(relName, [r_rel_app(name: name) = rA | relApps], apps, relAppType, acc) do
    case is_already_merged(name, relApps, acc) do
      ...
      false ->
        ...
        r_app(info: info) = mergedApp = merge_app(relName, rA, relAppType, app)
        r_app_info(applications: children, incl_apps: incNames, opt_apps: optNames) = info

        reqNames =
          for childName <- children,
              not :lists.member(childName, optNames),
              :lists.keymember(childName, r_app(:name), apps) do
            childName
          end

        do_merge_apps(relName, reqNames ++ incNames ++ relApps, apps, relAppType, [
          mergedApp | acc
        ])
    end
  end
    ...
    case inclApps -- r_app_info(info, :incl_apps) do
      ...
      badIncl ->
        :reltool_utils.throw_error(
          ~c"~w: These applications are used by release ~ts but are missing as included_applications in the app file: ~p",
  ...
  defp sort_apps([], missing, [], _) do
    :reltool_utils.throw_error(~c"Undefined applications: ~p", [make_set(missing)])
  end

  defp sort_apps([], [], circular, _) do
    :reltool_utils.throw_error(~c"Circular dependencies: ~p", [make_set(circular)])
  end

  defp sort_apps([], missing, circular, _) do
    :reltool_utils.throw_error(
      ~c"Circular dependencies: ~pUndefined applications: ~p\n",
  ...
  defp cr_path(r_app(label: label), true, []) do
    :filename.join([~c"$ROOT", ~c"lib", label, ~c"ebin"])
  end
  ...
  defp cr_path(r_app(name: name), :otp_build, _) do
    :filename.join([~c"$ROOT", ~c"lib", :erlang.atom_to_list(name), ~c"ebin"])
  end

  defp cr_path(r_app(active_dir: dir), _, _) do
    :filename.join([dir, ~c"ebin"])
  end
  ...
  defp do_spec_rel_files(r_rel(name: relName) = rel, sys) do
    relFile = relName ++ ~c".rel"
    scriptFile = relName ++ ~c".script"
    bootFile = relName ++ ~c".boot"
    mergedApps = merge_apps(rel, sys)
      ...
      case r_sys(sys, :excl_lib) do
        :otp_root ->
          for libDir <- r_sys(sys, :lib_dirs) do
            {~c"RELTOOL_EXT_LIB", libDir}
          end ++
            for r_app(
                  active_dir: appLibDir,
                  use_selected_vsn: :dir
                ) <- mergedApps do
              {~c"RELTOOL_EXT_LIB", :filename.dirname(appLibDir)}
            end
    ...
    relIoList = :io_lib.format(~c"~tp.\n\n", [genRel])
    scriptIoList = :io_lib.format(~c"~tp.\n\n", [script])
  ...
  defp to_utf8_bin_with_enc_comment(ioList) when is_list(ioList) do
    :unicode.characters_to_binary(~c"%% " ++ :epp.encoding_to_string(:utf8) ++ ~c"\n" ++ ioList)
  end
  ...
  defp do_gen_spec(
    ...
    {sysFiles, inclRegexps2, exclRegexps2, mandatory} =
      case exclLib do
        ...
        _ ->
          ...
          sF3 = [{:create_dir, ~c"bin", binFiles}] ++ sF2
          {sF3, iR2, eR2, [~c"bin", ~c"erts", ~c"lib"]}
      end
    ...
    sysFiles2 =
      [{:create_dir, ~c"releases", [startFile, {:create_dir, bootVsn, relFiles}]}] ++ sysFiles

    sysFiles3 = filter_spec(sysFiles2, inclRegexps2, exclRegexps2)
    sysFiles4 = sysFiles3 ++ [{:create_dir, ~c"lib", libFiles}]
  ...
  defp strip_sys_files(relocatable, sysFiles, apps, exclRegexps) do
    ...
    filterErts = fn spec ->
      ...
      case file do
        ~c"erts" ->
          :reltool_utils.throw_error(
            ~c"This system is not installed. The directory ~ts is missing.",
            [r_app(erts, :label)]
          )
  ...
  defp replace_dyn_erl(true, {:create_dir, ertsDir, ertsFiles}) do
    ...
    case lookup_spec(~c"dyn_erl", binFiles) do
      [] ->
        case lookup_spec(~c"erl.ini", binFiles) do
          [] ->
            true
          ...
          [{:copy_file, erlIni}] ->
            ...
            ertsFiles2 =
              :lists.keyreplace(~c"bin", 2, ertsFiles, {:create_dir, ~c"bin", binFiles2})

            {true, {:create_dir, ertsDir, ertsFiles2}}
        end

      [{:copy_file, dynErlExe}] ->
        erlExe = ~c"erl" ++ :filename.extension(dynErlExe)
        binFiles2 = :lists.keydelete(dynErlExe, 2, binFiles)
        dynErlExe2 = :filename.join([ertsDir, ~c"bin", dynErlExe])
        binFiles3 = :lists.keyreplace(erlExe, 2, binFiles2, {:copy_file, erlExe, dynErlExe2})
        ertsFiles2 = :lists.keyreplace(~c"bin", 2, ertsFiles, {:create_dir, ~c"bin", binFiles3})
        {true, {:create_dir, ertsDir, ertsFiles2}}
    end
  end

  defp spec_bin_files(sys, allSysFiles, strippedSysFiles, relFiles, inclRegexps) do
    [{:create_dir, ertsLabel, ertsFiles}] = safe_lookup_spec(~c"erts", strippedSysFiles)
    ...
    ertsBin = :filename.join([ertsLabel, ~c"bin"])
    ...
    goodNames =
      for {:copy_file, f} <- oldBinFiles,
          not :lists.suffix(~c".boot", f),
          not :lists.suffix(~c".script", f) do
        f
      end
    ...
    bootFiles =
      for f <- relFiles,
          :lists.suffix(~c".boot", :erlang.element(2, f)) do
        f
      end

    [{:write_file, _, bootRel}] =
      safe_lookup_spec(
        r_sys(sys, :boot_rel) ++ ~c".boot",
        bootFiles
      )

    bootFiles2 =
      :lists.keystore(~c"start.boot", 2, bootFiles, {:write_file, ~c"start.boot", bootRel})

    makeRegexp = fn file ->
      ~c"^bin/" ++ :erlang.element(2, file) ++ ~c"(|.escript)$"
    end
  ...
  defp do_check_sys(prefix, specs) do
    case lookup_spec(prefix, specs) do
      [] ->
        :reltool_utils.throw_error(~c"Mandatory system directory ~ts is not included", [prefix])
  ...
  defp spec_start_file(r_sys(boot_rel: bootRelName, rels: rels, apps: apps)) do
    ...
    data = r_app(erts, :vsn) ++ ~c" " ++ r_rel(bootRel, :vsn) ++ ~c"\n"
    {r_rel(bootRel, :vsn), {:write_file, ~c"start_erl.data", :unicode.characters_to_binary(data)}}
  end
  ...
  defp safe_lookup_spec(prefix, specs) do
    case lookup_spec(prefix, specs) do
      [] ->
        :reltool_utils.throw_error(~c"Mandatory system file ~ts is not included", [prefix])
  ...
  defp check_apps([mandatory | names], apps) do
    case :lists.keymember(mandatory, r_app(:name), apps) do
      false ->
        :reltool_utils.throw_error(~c"Mandatory application ~w is not included in ~p", [
          mandatory,
          apps
        ])
  ...
  defp spec_app(
    ...
    appUpFilename = :erlang.atom_to_list(name) ++ ~c".appup"
    ebinDir = :filename.join([sourceDir, ~c"ebin"])
    ...
    newEbin = {:create_dir, ~c"ebin", optAppUpFileSpec ++ optAppFileSpec ++ modSpecs}
    appFiles2 = :lists.keystore(~c"ebin", 2, appFiles, newEbin)
    ...
    case :erl_prim_loader.read_file_info(dir) do
      {:ok, r_file_info(type: :directory)} ->
        case :erl_prim_loader.list_dir(dir) do
          ...
          :error ->
            :reltool_utils.throw_error(~c"list dir ~ts failed", [dir])
        end
      ...
      _ ->
        :reltool_utils.throw_error(~c"read file info ~ts failed", [dir])
    end
  end
  ...
  defp spec_app_file(
    ...
    appFilename = :erlang.atom_to_list(name) ++ ~c".app"
  ...
  defp spec_create_dir(rootDir, sourceDir, baseDir, files) do
    libDir = :filename.join([rootDir, ~c"lib"])
  ...
  def eval_spec(spec, sourceDir, targetDir) do
    ...
    try do
      case :filelib.is_dir(targetDir2) do
        ...
        false ->
          {:error, targetDir2 ++ ~c": " ++ :file.format_error(:enoent)}
      end
    catch
      {:error, text} ->
  ...
  defp do_eval_spec({:strip_beam, file, chunkIds}, _OrigSourceDir, sourceDir, targetDir) do
    sourceFile = :filename.join([sourceDir, file])
    targetFile = :filename.join([targetDir, file])
  ...
  defp filter_spec(list, inclRegexps, exclRegexps) do
    do_filter_spec(~c"", list, inclRegexps, exclRegexps)
  end
  ...
  defp do_filter_spec(path, {:strip_beam, file, _ChunkIds}, inclRegexps, exclRegexps) do
    path2 = opt_join(path, file)
    match(path2, inclRegexps, exclRegexps)
  end
  ...
  defp do_install(relName, targetDir) do
    targetDir2 = :filename.absname(targetDir)
    relDir = :filename.join([targetDir2, ~c"releases"])
    dataFile = :filename.join([relDir, ~c"start_erl.data"])
    ...
    case :string.lexemes(
      ...
      [erlVsn, relVsn | _] ->
        ertsBinDir = :filename.join([targetDir2, ~c"erts-" ++ erlVsn, ~c"bin"])
        binDir = :filename.join([targetDir2, ~c"bin"])
        ...
        case :os.type() do
          ...
          _ ->
            subst_src_scripts(
              ...
              ertsBinDir,
              binDir,
              [{~c"FINAL_ROOTDIR", targetDir2}, {~c"EMU", ~c"beam"}],
              [:preserve]
            )
        end

        relFile = :filename.join([relDir, relVsn, relName ++ ~c".rel"])
      ...
      _ ->
        :reltool_utils.throw_error(~c"~ts: Illegal data file syntax", [dataFile])
    end
  end
  ...
  defp subst_src_script(script, srcDir, destDir, vars, opts) do
    subst_file(
      :filename.join([srcDir, script ++ ~c".src"]),
