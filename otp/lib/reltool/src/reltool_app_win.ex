defmodule :m_reltool_app_win do
  ...
  defp app_title(app) do
    :lists.concat([:reltool, ~c" - ", r_app(app, :label)])
  end
  ...
  defp create_apps_page(s, derived) do
    ...
    usedByCtrl = create_apps_list_ctrl(panel, upper, ~c"Used by")
    ...
    requiredCtrl = create_apps_list_ctrl(panel, upper, ~c"Required")
    ...
    inclCtrl = create_apps_list_ctrl(panel, upper, ~c"Included")
    ...
    usesCtrl = create_apps_list_ctrl(panel, upper, ~c"Uses")
    ...
    :wxNotebook.addPage(r_state(s2, :book), panel, ~c"Application dependencies", [])
    s2
  end
  ...
  defp create_deps_page(s, derived) do
    ...
    usedByCtrl =
      create_mods_list_ctrl(
        panel,
        main,
        ~c"Modules using this",
        ~c" and their applications",
        :undefined,
        :undefined
      )
    ...
    :wxPanel.setSizer(panel, main)
    :wxNotebook.addPage(r_state(s2, :book), panel, ~c"Module dependencies", [])
    s2
  end
  ...
  defp create_mods_page(s, derived) do
    ...
    sourceCtrl =
      create_mods_list_ctrl(panel, mainSz, ~c"Available", ~c"", :whitelist_add, :blacklist_add)
    ...
    whiteCtrl =
      create_mods_list_ctrl(panel, mainSz, ~c"Included", ~c"", :whitelist_del, :blacklist_add)
    ...
    blackCtrl =
      create_mods_list_ctrl(panel, mainSz, ~c"Excluded", ~c"", :whitelist_add, :blacklist_del)
    ...
    derivedCtrl =
      create_mods_list_ctrl(panel, mainSz, ~c"Derived", ~c"", :whitelist_add, :blacklist_add)
    ...
    :wxNotebook.addPage(r_state(s2, :book), panel, ~c"Modules", [])
    s2
  end
  ...
  defp create_mods_list_ctrl(panel, outerSz, title, appText, tick, cross) do
    ...
    toolTip = ~c"Select module(s) or open separate module window with a double click."
  ...
  defp action_to_tool_tip(label, action) do
    case action do
      :whitelist_add when label === ~c"Included" ->
        ~c"Remove selected module(s) from whitelist."

      :whitelist_add ->
        ~c"Add selected module(s) to whitelist."

      :whitelist_del ->
        ~c"Remove selected module(s)from whitelist."

      :blacklist_add when label === ~c"Excluded" ->
        ~c"Remove selected module(s) from blacklist."

      :blacklist_add ->
        ~c"Add selected module(s) to blacklist."
  ...
  defp create_config_page(r_state(app: app) = s) do
    ...
    {latestRadio, selectedRadio, sourceBox} =
      create_double_box(
        panel,
        topSizer,
        ~c"Source selection policy",
        ~c"Use latest version",
        :use_latest_vsn,
        ~c"Use selected version",
        :use_selected_vsn,
        ~c"Directories",
    ...
    {appGlobalRadio, appLocalRadio, appLocalBox} =
      create_double_box(
        ...
        ~c"Use global config",
        :global_incl_cond,
        ~c"Use application specific config",
        :local_incl_cond,
    ...
    :wxNotebook.addPage(r_state(s2, :book), panel, ~c"Application settings", [])
    s2
  end
  ...
  defp handle_event(r_state(sys: sys, app: app) = s, wx) do
    case wx do
      ...
      _ ->
        :error_logger.format(~c"~w~w got unexpected app event from wx:\n\t~tp\n", [
          :reltool_app_win,
          self(),
          wx
        ])
  ...
  defp move_mod(app, {_ItemNo, modStr}, action) do
    ...
    appCond =
      case action do
        ...
        _ ->
          :error_logger.format(
            ~c"~w~w got unexpected mod button event: ~w\n\t ~tp\n",
