defmodule :m_reltool_app_win do
  ...
  Record.defrecord(:r_state, :state, parent_pid: :undefined,
                                 xref_pid: :undefined, mod_wins: :undefined,
                                 sys: :undefined, common: :undefined,
                                 app: :undefined, frame: :undefined,
                                 panel: :undefined, book: :undefined,
  ...
  Record.defrecord(:r_mod_win, :mod_win, name: :undefined,
                                   pid: :undefined)
  def start_link(wxEnv, xref, common, appName) do
    :proc_lib.start_link(:reltool_app_win, :init,
                           [self(), wxEnv, xref, common, appName], :infinity,
                           [])
  end
  ...
  defp do_init(parent, wxEnv, xref, c, appName) do
    ...
    s = r_state(parent_pid: parent, xref_pid: xref, mod_wins: [],
            sys: sys, common: c, app: app)
  ...
  def loop(r_state(xref_pid: xref, common: c, app: app) = s) do
    receive do
      ...
      {:call, replyTo, ref, {:open_mod, modName}} ->
        ...
        {:value, r_mod_win(pid: modPid)} = :lists.keysearch(modName,
                                                      r_mod_win(:name),
                                                      r_state(s2, :mod_wins))
      ...
      {:EXIT, pid, _Reason} = exit ->
        exit_warning(exit)
        s2 = r_state(s, mod_wins: :lists.keydelete(pid, r_mod_win(:pid),
                                               r_state(s, :mod_wins)))
        :reltool_app_win.loop(s2)
      msg ->
  ...
  defp create_apps_page(s, derived) do
    ...
    s2 = r_state(s, app_required_ctrl: requiredCtrl, 
                app_used_by_ctrl: usedByCtrl,  app_incl_ctrl: inclCtrl, 
  ...
  defp create_deps_page(s, derived) do
    ...
    usedByCtrl = create_mods_list_ctrl(panel, main, 'Modules using this', ' and their applications',
                                         :undefined, :undefined)
    :wxSizer.add(main,
                   :wxStaticLine.new(panel, [{:style, 8}]),
                   [{:border, 2}, {:flag,
    ...
    usesCtrl = create_mods_list_ctrl(panel, main, 'Used modules', ' and their applications',
                                       :undefined, :undefined)
    s2 = r_state(s, deps_used_by_ctrl: usedByCtrl, 
  ...
  defp create_mods_page(s, derived) do
    ...
    sourceCtrl = create_mods_list_ctrl(panel, mainSz, 'Available', '',
                                         :whitelist_add, :blacklist_add)
    :wxSizer.add(mainSz,
                   :wxStaticLine.new(panel, [{:style, 8}]),
                   [{:border, 2}, {:flag,
                                     64 ||| 128 ||| 32 ||| 16 ||| 8192}])
    whiteCtrl = create_mods_list_ctrl(panel, mainSz, 'Included', '',
                                        :whitelist_del, :blacklist_add)
    :wxSizer.add(mainSz,
                   :wxStaticLine.new(panel, [{:style, 8}]),
                   [{:border, 2}, {:flag,
                                     64 ||| 128 ||| 32 ||| 16 ||| 8192}])
    blackCtrl = create_mods_list_ctrl(panel, mainSz, 'Excluded', '',
                                        :whitelist_add, :blacklist_del)
    :wxSizer.add(mainSz,
                   :wxStaticLine.new(panel, [{:style, 8}]),
                   [{:border, 2}, {:flag,
                                     64 ||| 128 ||| 32 ||| 16 ||| 8192}])
    derivedCtrl = create_mods_list_ctrl(panel, mainSz, 'Derived', '',
                                          :whitelist_add, :blacklist_add)
    s2 = r_state(s, mods_source_ctrl: sourceCtrl, 
  ...
  defp create_mods_list_ctrl(panel, outerSz, title, appText, tick, cross) do
    listCtrl = :wxListCtrl.new(panel,
                                 [{:style, 32 ||| 1073741824 ||| - 2147483648}])
  ...
  defp create_button(_Panel, sizer, _ListCtrl, _Title, _BitMapName,
            :undefined) do
  ...
  defp create_config_page(r_state(app: app) = s) do
    ...
    {latestRadio, selectedRadio,
       sourceBox} = create_double_box(panel, topSizer, 'Source selection policy', 'Use latest version',
                                        :use_latest_vsn, 'Use selected version', :use_selected_vsn,
                                        'Directories', r_app(app, :sorted_dirs), :version)
    inclSizer = :wxBoxSizer.new(4)
    {appGlobalRadio, appLocalRadio,
       appLocalBox} = create_double_box(panel, inclSizer, 'Application inclusion policy', 'Use global config',
                                          :global_incl_cond, 'Use application specific config',
                                          :local_incl_cond, 'Application specific',
                                          :reltool_utils.incl_conds(),
                                          :incl_cond)
    {modGlobalRadio, modLocalRadio,
       modLocalBox} = create_double_box(panel, inclSizer, 'Module inclusion policy', 'Use global config',
                                          :global_mod_cond, 'Use application specific config', :local_mod_cond,
                                          'Application specific', :reltool_utils.mod_conds(),
                                          :mod_cond)
    :wxSizer.add(topSizer, inclSizer,
                   [{:border, 2}, {:flag,
                                     64 ||| 128 ||| 32 ||| 16 ||| 8192},
                                      {:proportion, 1}])
    s2 = r_state(s, config_app_global: appGlobalRadio, 
  ...
  defp create_double_box(panel, sizer, topLabel, outerText, outerData,
            innerText, innerData, internalLabel, internalChoices,
            internalChoiceData) do
    topSizer = :wxStaticBoxSizer.new(8, panel,
                                       [{:label, topLabel}])
    outerSizer = :wxBoxSizer.new(8)
    outerRadio = :wxRadioButton.new(panel, - 1, outerText,
                                      [{:style, 4}])
    :wxEvtHandler.connect(outerRadio,
                            :command_radiobutton_selected,
                            [{:userData, outerData}])
    innerRadio = :wxRadioButton.new(panel, - 1, innerText)
    :wxEvtHandler.connect(innerRadio,
                            :command_radiobutton_selected,
                            [{:userData, innerData}])
    innerBox = :wxRadioBox.new(panel, - 1, internalLabel,
                                 {- 1, - 1}, {- 1, - 1}, internalChoices, [])
    :wxEvtHandler.connect(innerBox,
  ...
  defp handle_event(r_state(sys: sys, app: app) = s, wx) do
    case (wx) do
      r_wx(obj: objRef, event: r_wxMouse(type: :enter_window)) ->
        ...
        s
      r_wx(obj: listCtrl, userData: :mods_list_ctrl,
          event: r_wxSize(type: :size, size: {w, _H})) ->
        ...
        s
      r_wx(obj: listCtrl, userData: :apps_list_ctrl,
          event: r_wxSize(type: :size, size: {w, _H})) ->
        :wxListCtrl.setColumnWidth(listCtrl, 0, w)
        s
      r_wx(userData: :open_app, obj: listCtrl,
          event: r_wxList(type: :command_list_item_activated,
                     itemIndex: pos)) ->
        appBase = :wxListCtrl.getItemText(listCtrl, pos)
        {appName,
           _AppVsn} = :reltool_utils.split_app_name(appBase)
        {:ok,
           _AppPid} = :reltool_sys_win.open_app(r_state(s, :parent_pid),
                                                  appName)
        s
      r_wx(userData: :open_mod, obj: listCtrl,
          event: r_wxList(type: :command_list_item_activated,
                     itemIndex: pos)) ->
        modName = :erlang.list_to_atom(:wxListCtrl.getItemText(listCtrl,
                                                                 pos))
      ...
      r_wx(userData: :incl_cond,
          event: r_wxCommand(type: :command_radiobox_selected,
                     cmdString: sel)) ->
      ...
      r_wx(userData: :use_selected_vsn) ->
        ...
        {:ok,
           app3} = :reltool_sys_win.set_app(r_state(s, :parent_pid),
                                              app2)
        s2 = r_state(s, app: app3)
        redraw_window(s2)
      r_wx(userData: :version,
          event: r_wxCommand(type: :command_radiobox_selected,
                     cmdString: activeDir)) ->
        app2 = r_app(app, use_selected_vsn: :dir)
        s2 = change_version(s, app2, activeDir)
        redraw_window(s2)
      r_wx(userData: {:mod_button, action, listCtrl},
          event: r_wxCommand(type: :command_button_clicked)) ->
        items = :reltool_utils.get_items(listCtrl)
        handle_mod_button(s, items, action)
      _ ->
        :error_logger.format('~w~w got unexpected app event from wx:\n\t~tp\n', [:reltool_app_win, self(), wx])
        s
    end
  end

  defp create_mod_window(r_state(parent_pid: relPid, xref_pid: xref,
              common: c) = s,
            modName) do
    case (:lists.keysearch(modName, r_mod_win(:name),
                             r_state(s, :mod_wins))) do
      false ->
        wxEnv = :wx.get_env()
        {:ok, pid} = :reltool_mod_win.start_link(wxEnv, xref,
                                                   relPid, c, modName)
  ...
  defp change_version(s, app, newDir) do
    app2 = r_app(app, active_dir: newDir,  label: :undefined, 
                    vsn: :undefined,  info: :undefined)
    {:ok,
       app3} = :reltool_sys_win.set_app(r_state(s, :parent_pid),
                                          app2)
  ...
  defp redraw_apps(r_state(app: r_app(info: appInfo),
              app_used_by_ctrl: usedByCtrl,
              app_required_ctrl: requiredCtrl,
              app_incl_ctrl: inclCtrl, app_uses_ctrl: usesCtrl,
              xref_pid: xref),
            {_SourceMods, _WhiteMods, _BlackMods, _DerivedMods,
               usedByMods, usesMods}) do
    usedByApps = :lists.usort(for {image, _,
                                     m} <- usedByMods do
    ...
    requiredApps = :lists.sort(:lists.map(select,
                                            r_app_info(appInfo, :applications)))
    inclApps = :lists.map(select, r_app_info(appInfo, :incl_apps))
    usesApps = :lists.usort(for {image, _, m} <- usesMods do
                              {r_mod(m, :app_name), image}
                            end)
  ...
  defp do_redraw_apps(listCtrl, appImages) do
    :wxListCtrl.deleteAllItems(listCtrl)
    add = fn {appName, imageId}, {row, prev}
                 when appName !== prev ->
               ...
               :wxListCtrl.setItem(listCtrl, row, 0, str,
                                     [{:imageId, imageId}])
               {row + 1, appName}
             {_, _}, acc ->
               acc
  ...
  defp redraw_mods(r_state(mods_source_ctrl: sourceCtrl,
              mods_white_ctrl: whiteCtrl, mods_black_ctrl: blackCtrl,
              mods_derived_ctrl: derivedCtrl,
              deps_used_by_ctrl: usedByCtrl, deps_uses_ctrl: usesCtrl,
              app: r_app(is_pre_included: isPre, is_included: isIncl),
              status_bar: bar),
            {sourceMods, whiteMods, blackMods, derivedMods,
               usedByMods, usesMods}) do
    inclStatus = (case (isIncl) do
    ...
    status = :lists.concat([inclStatus, length(whiteMods),
                                            ' whitelisted modules and ', length(derivedMods), ' derived modules.'])
  ...
  defp app_to_mods(r_state(xref_pid: xref, app: app)) do
    sourceMods = (for m <- r_app(app, :mods),
                        r_mod(m, :is_included) !== true,
                        r_mod(m, :is_pre_included) !== false do
                    m
                  end)
    whiteMods = (for m <- r_app(app, :mods),
                       r_mod(m, :is_pre_included) === true do
                   m
                 end)
    blackMods = (for m <- r_app(app, :mods),
                       r_mod(m, :is_pre_included) === false do
                   m
                 end)
    derivedMods = (for m <- r_app(app, :mods),
                         r_mod(m, :is_included) === true,
                         r_mod(m, :is_pre_included) !== true do
                     m
                   end)
  ...
  defp do_redraw_mods(listCtrl, imageMods, hasApps) do
    ...
    add = fn {imageId, appName, r_mod(name: modName)}, row ->
               ...
               :wxListCtrl.setItem(listCtrl, row, 0,
                                     :erlang.atom_to_list(modName),
                                     [{:imageId, imageId}])
               case (hasApps) do
                 false ->
                   :ok
                 true ->
                   :wxListCtrl.setItem(listCtrl, row, 1,
  ...
  defp redraw_config(r_state(sys: r_sys(incl_cond: globalIncl,
                     mod_cond: globalSource),
              app: r_app(incl_cond: localIncl, mod_cond: localSource,
                       use_selected_vsn: useSelected, active_dir: activeDir,
              ...
              config_latest: latestRadio,
              config_selected: selectedRadio,
              config_source_box: sourceBox)) do
    redraw_double_box(globalIncl, localIncl, appGlobalRadio,
                        appLocalRadio, appLocalBox,
                        &:reltool_utils.incl_cond_to_index/1)
    redraw_double_box(globalSource, localSource,
                        modGlobalRadio, modLocalRadio, modLocalBox,
                        &:reltool_utils.mod_cond_to_index/1)
    redraw_double_box(false, useSelected, latestRadio,
                        selectedRadio, sourceBox,
                        fn false ->
                             0
                           _ ->
  ...
  defp redraw_double_box(global, local, globalRadio, localRadio,
            localBox, getChoice) do
    appCond = (case (local) do
                 :undefined ->
