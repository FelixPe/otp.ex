defmodule :m_reltool_utils do
  ...
  def erl_libs() do
    erl_libs(:os.getenv(~c"ERL_LIBS", ~c""), :os.type())
  end
  ...
  defp normalize_dir([~c".." | dirs], [_Dir | path]) do
    normalize_dir(dirs, path)
  end

  defp normalize_dir([~c"." | dirs], path) do
    normalize_dir(dirs, path)
  end
  ...
  def default_rels() do
    [
      r_rel(name: ~c"start_clean", vsn: ~c"1.0", rel_apps: []),
      r_rel(name: ~c"start_sasl", vsn: ~c"1.0", rel_apps: [r_rel_app(name: :sasl)]),
      r_rel(name: ~c"no_dot_erlang", vsn: ~c"1.0", rel_apps: [], load_dot_erlang: false)
    ]
  end
  ...
  def choose_default(tag, :standalone, _InclDefs) do
    case tag do
      :incl_sys_filters ->
        [
          ~c"^bin/(erl|epmd)(|\\.exe|\\.ini)$",
          ~c"^bin/start(|_clean).boot$",
          ~c"^bin/no_dot_erlang\\.boot$",
          ~c"^erts.*/bin",
          ~c"^lib$"
        ]
      ...
      :excl_sys_filters ->
        [
          ~c"^erts.*/bin/(erlc|dialyzer|typer)(|\\.exe)$",
          ~c"^erts.*/bin/(start|escript|to_erl|run_erl)(|\\.exe)$",
          ~c"^erts.*/bin/.*(debug|pdb)"
        ]
  ...
  def choose_default(tag, :embedded, _InclDefs) do
    case tag do
      :incl_sys_filters ->
        [~c"^bin", ~c"^erts", ~c"^lib", ~c"^releases"]

      :excl_sys_filters ->
        [
          ~c"^bin/(erlc|dialyzer|typer)(|\\.exe)$",
          ~c"^erts.*/bin/(erlc|dialyzer|typer)(|\\.exe)$",
          ~c"^erts.*/bin/.*(debug|pdb)"
        ]

      :incl_app_filters ->
        [~c"^ebin", ~c"^include", ~c"^priv"]
  ...
  def mod_conds() do
    [~c"all (ebin + app file)", ~c"ebin + derived", ~c"app file + derived", ~c"derived", ~c"none"]
  end
  ...
  def safe_keysearch(key, pos, list, mod, line) do
    case :lists.keysearch(key, pos, list) do
      false ->
        :io.format(~c"~w(~w): lists:keysearch(~tp, ~w, ~tp) -> false\n", [
          mod,
          line,
          key,
          pos,
  ...
  def create_dir(dir) do
    ...
    case :file.make_dir(dir) do
      ...
      {:error, reason} ->
        ...
        throw_error(~c"create dir ~ts: ~ts", [dir, text])
    end
  end
  ...
  def list_dir(dir) do
    case :erl_prim_loader.list_dir(dir) do
      ...
      :error ->
        ...
        throw_error(~c"list dir ~ts: ~ts", [dir, text])
    end
  end
  ...
  def read_file_info(file) do
    case :file.read_file_info(file) do
      ...
      {:error, reason} ->
        text = :file.format_error(reason)
        throw_error(~c"read file info ~ts: ~ts", [file, text])
    end
  end
  ...
  def write_file_info(file, info) do
    case :file.write_file_info(file, info) do
      ...
      {:error, reason} ->
        text = :file.format_error(reason)
        throw_error(~c"write file info ~ts: ~ts", [file, text])
    end
  end
  ...
  def read_file(file) do
    case :file.read_file(file) do
      ...
      {:error, reason} ->
        text = :file.format_error(reason)
        throw_error(~c"read file ~ts: ~ts", [file, text])
    end
  end
  ...
  def write_file(file, bin) do
    case :file.write_file(file, bin) do
      ...
      {:error, reason} ->
        text = :file.format_error(reason)
        throw_error(~c"write file ~ts: ~ts", [file, text])
    end
  end
    ...
    case :filelib.is_dir(dir) do
      true ->
        case :file.list_dir(dir) do
          ...
          {:error, reason} ->
            text = :file.format_error(reason)
            throw_error(~c"delete file ~ts: ~ts\n", [dir, text])
        end
    ...
    case :erl_prim_loader.get_file(from) do
      {:ok, bin, _} ->
        case :file.write_file(to, bin) do
          ...
          {:error, reason} ->
            text = :file.format_error(reason)
            throw_error(~c"copy file ~ts -> ~ts: ~ts\n", [from, to, text])
        end
  ...
  defp do_decode_regexps(key, [regexp | regexps], acc) do
    case (try do
      ...
      _ ->
        ...
        throw({:error, ~c"Illegal option: " ++ text})
    end
  end
