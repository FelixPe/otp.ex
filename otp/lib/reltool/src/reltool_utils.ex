defmodule :m_reltool_utils do
  ...
  def split_app_name(name) do
    ...
    case (:lists.splitwith(pred, :lists.reverse(name))) do
      {vsn, [?- | app]} ->
        {:erlang.list_to_atom(:lists.reverse(app)),
           :lists.reverse(vsn)}
      _ ->
  ...
  def prim_consult(bin) when is_binary(bin) do
    case (:erl_scan.string(:unicode.characters_to_list(bin,
                                                         encoding(bin)))) do
      {:ok, tokens, _EndLine} ->
        prim_parse(tokens, [])
  ...
  def default_rels() do
    [r_rel(name: 'start_clean', vsn: '1.0', rel_apps: []), r_rel(name: 'start_sasl', vsn: '1.0',
                                           rel_apps: [r_rel_app(name: :sasl)]),
                                           r_rel(name: 'no_dot_erlang', vsn: '1.0', rel_apps: [],
  ...
  def choose_default(tag, :standalone, _InclDefs) do
    case (tag) do
      :incl_sys_filters ->
        ['^bin/(erl|epmd)(|\\.exe|\\.ini)$', '^bin/start(|_clean).boot$', '^bin/no_dot_erlang\\.boot$', '^erts.*/bin', '^lib$']
      :excl_sys_filters ->
        ['^erts.*/bin/(erlc|dialyzer|typer)(|\\.exe)$', '^erts.*/bin/(start|escript|to_erl|run_erl)(|\\.exe)$', '^erts.*/bin/.*(debug|pdb)']
      :incl_app_filters ->
        ['^ebin', '^priv']
      :excl_app_filters ->
        ['^ebin/.*\\.appup$']
  ...
  def choose_default(tag, :embedded, _InclDefs) do
    case (tag) do
      ...
      :excl_sys_filters ->
        ['^bin/(erlc|dialyzer|typer)(|\\.exe)$', '^erts.*/bin/(erlc|dialyzer|typer)(|\\.exe)$', '^erts.*/bin/.*(debug|pdb)']
      :incl_app_filters ->
        ['^ebin', '^include', '^priv']
      :excl_app_filters ->
        []
  ...
  def assign_image_list(listCtrl) do
    ...
    for image <- ['wxART_ERROR', 'wxART_WARNING', 'wxART_QUESTION', 'wxART_TICK_MARK', 'wxART_CROSS_MARK', 'wxART_GO_HOME'] do
      :wxImageList.add(art,
  ...
  def get_item(listCtrl) do
    case (:wxListCtrl.getItemCount(listCtrl)) do
      0 ->
        :undefined
      _ ->
        case (:wxListCtrl.getNextItem(listCtrl, - 1,
                                        [{:geometry, 1}, {:state, 4}])) do
          -1 ->
  ...
  def get_selected_items(listCtrl, prevItem, acc) do
    case (:wxListCtrl.getNextItem(listCtrl, prevItem,
                                    [{:geometry, 1}, {:state, 4}])) do
      -1 ->
        acc
      itemNo ->
        case (:wxListCtrl.getItemText(listCtrl, itemNo)) do
          text when text !== '*MISSING*' ->
            get_selected_items(listCtrl, itemNo,
                                 [{itemNo, text} | acc])
          _Text ->
  ...
  def select_item(listCtrl, [{itemNo, text} | items]) do
    case (text === '*MISSING*') do
      ...
      false ->
        stateMask = 4
        state = :wxListCtrl.getItemState(listCtrl, itemNo,
                                           stateMask)
        state2 = state ||| 4
        :wxListCtrl.setItemState(listCtrl, itemNo, state2,
                                   stateMask)
  ...
  def safe_keysearch(key, pos, list, mod, line) do
    case (:lists.keysearch(key, pos, list)) do
      false ->
        ...
        :erlang.error({mod, line, :lists, :keysearch,
                         [key, pos, list]})
      {:value, val} ->
        val
    end
  ...
  defp do_decode_regexps(key, [regexp | regexps], acc) do
    case ((try do
      ...
      {:ok, mP} ->
        do_decode_regexps(key, regexps,
                            [r_regexp(source: regexp, compiled: mP) | acc])
      _ ->
        text = :lists.flatten(:io_lib.format('~tp',
                                               [{key, regexp}]))
