defmodule :m_reltool_sys_win do
  ...
  def init(options) do
    try do
      ...
      :error, reason ->
        :io.format(~c"~tp: ~tp~n", [reason, __STACKTRACE__])
      ...
      {:ok, serverPid, c, sys} ->
        ...
        s =
          r_state(
            ...
            common: c,
            config_file: :filename.absname(~c"config.reltool"),
            target_dir: :filename.absname(~c"reltool_target_dir"),
            app_wins: [],
            sys: sys,
            fgraph_wins: [],
            warning_wins: []
        ...
        s5 =
          :wx.batch(fn ->
            ...
            :wxStatusBar.setStatusText(r_state(s2, :status_bar), ~c"Done.")
            s3 = redraw_apps(s2)
            s4 = redraw_libs(s3)
  ...
  defp restart_server_safe_config(false, parent, reason) do
    ...
    strings = [
      {:wxe_util.get_const(:wxBLACK), ~c"Could not start reltool server:\n\n"},
      {:wxe_util.get_const(:wxRED), reason ++ ~c"\n\n"},
      {:wxe_util.get_const(:wxBLACK),
       :io_lib.format(
         ~c"Resetting the configuration to:~n~n  ~p~n~nDo you want to continue with this configuration?",
         [
           {:sys,
            [
              {:incl_cond, :exclude},
    ...
    case question_dialog_2(~c"Reltool server start error", strings) do
      5100 ->
        do_init([
          {:safe_config, true},
          {:parent, parent},
  ...
  defp exit_dialog(warnings) do
    question = ~c"Do you want to continue despite these warnings?"
            ...
            case :lists.keysearch(objRef, r_fgraph_win(:frame), fWs) do
              ...
              false ->
                ...
                case :lists.member(objRef, wWs) do
                  ...
                  false ->
                    :error_logger.format(~c"~w~w got unexpected message:\n\t~tp\n", [
                      :reltool_sys_win,
                      self(),
                      msg
                    ])
  ...
  defp create_menubar(frame) do
    ...
    :wxMenu.append(file, 301, ~c"Display application dependency graph")
    :wxMenu.append(file, 302, ~c"Display module dependency graph")
    :wxMenu.appendSeparator(file)
    :wxMenu.append(file, 309, ~c"Reset configuration to default")
    :wxMenu.append(file, 308, ~c"Undo configuration (toggle)")
    :wxMenu.append(file, 303, ~c"Load configuration")
    save = :wxMenu.new()
    :wxMenu.append(save, 304, ~c"Save explicit configuration  (neither defaults nor derivates)")
    :wxMenu.append(save, 306, ~c"Save configuration defaults  (defaults only)")
    :wxMenu.append(save, 305, ~c"Save configuration derivates (derivates only))")
    :wxMenu.append(save, 307, ~c"Save extended configuration  (both defaults and derivates)")
    :wxMenu.append(file, -1, ~c"Save configuration", save)
    :wxMenu.appendSeparator(file)
    :wxMenu.append(file, 310, ~c"Generate rel, script and boot files")
    :wxMenu.append(file, 311, ~c"Generate target system")
    :wxMenu.appendSeparator(file)
    :wxMenu.append(file, 5006, ~c"Close")
    :wxMenuBar.append(menuBar, help, ~c"Help")
    :wxMenu.append(help, 300, ~c"Contents")
    :wxMenu.append(help, 5014, ~c"About")
  ...
  defp create_app_page(r_state(book: book) = s) do
    ...
    sourceCtrl = create_app_list_ctrl(panel, sizer, ~c"Available", :whitelist_add, :blacklist_add)
    ...
    whiteCtrl = create_app_list_ctrl(panel, sizer, ~c"Included", :whitelist_del, :blacklist_add)
    ...
    blackCtrl = create_app_list_ctrl(panel, sizer, ~c"Excluded", :whitelist_add, :blacklist_del)
    ...
    derivedCtrl = create_app_list_ctrl(panel, sizer, ~c"Derived", :whitelist_add, :blacklist_add)
    :wxPanel.setSizer(panel, sizer)
    :wxNotebook.addPage(book, panel, ~c"Applications", [])
  ...
  defp create_app_list_ctrl(panel, outerSz, title, tick, cross) do
    ...
    toolTip = ~c"Select application(s) or open separate application window with a double click."
    ...
    create_button(panel, buttonSz, listCtrl, title, ~c"wxART_TICK_MARK", tick)
    create_button(panel, buttonSz, listCtrl, title, ~c"wxART_CROSS_MARK", cross)
  ...
  defp action_to_tool_tip(label, action) do
    case action do
      :whitelist_add when label === ~c"Included" ->
        ~c"Remove selected application(s) from whitelist."

      :whitelist_add ->
        ~c"Add selected application(s) to whitelist."

      :whitelist_del ->
        ~c"Remove selected application(s)from whitelist."

      :blacklist_add when label === ~c"Excluded" ->
        ~c"Remove selected application(s) from blacklist."

      :blacklist_add ->
        ~c"Add selected application(s) to blacklist."
  ...
  defp create_lib_page(r_state(book: book) = s) do
    ...
    toolTip = ~c"Edit application sources."
  ...
  defp redraw_libs(r_state(lib_tree: tree, sys: sys) = s) do
    ...
    libItem = :wxTreeCtrl.appendItem(tree, top, ~c"Library directories", [])
    ...
    escriptItem = append_item(tree, top, ~c"Escript files", :undefined)
  ...
  defp append_root(tree, parent, dir, erts) do
    top = append_item(tree, parent, ~c"Root directory", :undefined)
    data = r_root_data(dir: dir)
    rootItem = append_item(tree, top, dir, data)
    ertsItem = append_item(tree, rootItem, ~c"erts", :undefined)

    for d <- r_app(erts, :sorted_dirs) do
      append_app(tree, ertsItem, :filename.basename(:filename.dirname(d)), d)
    end

    libItem = append_item(tree, rootItem, ~c"lib", :undefined)
    libDir = :filename.join([dir, ~c"lib"])
  ...
  defp create_config_page(r_state(sys: sys, book: book) = s) do
    ...
    appBox =
      :wxRadioBox.new(
        panel,
        -1,
        ~c"Application inclusion policy",
        {-1, -1},
        {-1, -1},
        appConds,
        []
    ...
    appToolTip = ~c"Choose default policy for inclusion of applications. "
    ...
    modBox =
      :wxRadioBox.new(panel, -1, ~c"Module inclusion policy", {-1, -1}, {-1, -1}, modConds, [])

    modToolTip = ~c"Choose default policy for module inclusion."
    ...
    :wxPanel.setSizer(panel, sizer)
    :wxNotebook.addPage(book, panel, ~c"System settings", [])
  ...
  defp create_warning_list(r_state(panel: panel) = s) do
    ...
    :wxListCtrl.insertColumn(listCtrl, 0, ~c"Warnings", [
      {:format, 0},
      {:width, :reltool_utils.get_column_width(listCtrl)}
    ])

    :wxListCtrl.setToolTip(listCtrl, ~c"Warnings are listed in this window")
  ...
  defp redraw_warnings(listCtrl, warnings) do
    ...
    show = fn warning, row ->
      :wxListCtrl.insertItem(listCtrl, row, ~c"")
  ...
  defp create_main_release_page(r_state(book: book) = s) do
    ...
    create = :wxButton.new(panel, -1, [{:label, ~c"Create"}])
    :wxButton.setToolTip(create, ~c"Create a new release.")
    :wxButton.connect(create, :command_button_clicked, [{:userData, :create_rel}])
    :wxSizer.add(buttonSizer, create)
    delete = :wxButton.new(panel, -1, [{:label, ~c"Delete"}])
    :wxButton.setToolTip(delete, ~c"Delete a release.")
    :wxButton.connect(delete, :command_button_clicked, [{:userData, :delete_rel}])
    :wxSizer.add(buttonSizer, delete)
    view = :wxButton.new(panel, -1, [{:label, ~c"View script"}])
    :wxButton.setToolTip(view, ~c"View generated script file.")
    ...
    :wxPanel.setSizer(panel, sizer)
    :wxNotebook.addPage(book, panel, ~c"Releases", [])
  ...
  defp add_release_page(book, r_rel(name: relName, rel_apps: relApps)) do
    ...
    relToolTip =
      ~c"Choose which applications that shall be included in the release resource file."
  ...
  defp root_popup(s, root, tree, item) do
    ...
    :wxMenu.append(popupMenu, 0, ~c"Root dir")
    :wxMenu.appendSeparator(popupMenu)
    :wxMenu.append(popupMenu, 1, ~c"Edit")
    choices = [:edit]
  ...
  defp lib_popup(s, lib, tree, item) do
    popupMenu = :wxMenu.new()
    :wxMenu.append(popupMenu, 0, ~c"Library dir")
    :wxMenu.appendSeparator(popupMenu)
    :wxMenu.append(popupMenu, 1, ~c"Add")
    ...
    choices =
      case :wxTreeCtrl.getItemData(tree, item) do
        ...
        r_lib_data() ->
          :wxMenu.append(popupMenu, 2, ~c"Edit")
          :wxMenu.append(popupMenu, 3, ~c"Delete")
          [:add, :edit, :delete]
      end
  ...
  defp escript_popup(s, file, tree, item) do
    popupMenu = :wxMenu.new()
    :wxMenu.append(popupMenu, 0, ~c"Escript file")
    :wxMenu.appendSeparator(popupMenu)
    :wxMenu.append(popupMenu, 1, ~c"Add")
  ...
  defp handle_event(
    ...
    case event do
      ...
      when userData === :main_window and id === 300 ->
        ...
        helpFile = :filename.join([appDir, ~c"doc", ~c"html", ~c"index.html"])
        url = ~c"file://" ++ :filename.absname(helpFile)
      ...
      index when index >= 0 ->
        tip =
          case :wxListCtrl.getItemText(
            ...
            ~c"" ->
              ~c"Warnings are listed in this window"
  ...
  defp display_warning(s, warning) do
    ...
    frame = :wxFrame.new(:wx.null(), -1, ~c"Warning", [{:pos, pos}])
    panel = :wxPanel.new(frame, [])
    textStyle = 16 ||| 1 ||| 32
    ...
    close = :wxButton.new(panel, -1, [{:label, ~c"Close"}])
    :wxButton.setToolTip(close, ~c"Close window.")
  ...
  defp handle_popup_event(
    ...
    case :lists.nth(pos, choices) do
      :edit ->
        ...
        case select_dir(r_state(s, :frame), ~c"Change root directory", oldDir, style) do
  ...
  defp handle_popup_event(
    ...
    case :lists.nth(pos, choices) do
      :add ->
        ...
        case select_dir(r_state(s, :frame), ~c"Select a library directory to add", cwd, style) do
          {:ok, newDir} ->
      ...
      :edit ->
        ...
        case select_dir(r_state(s, :frame), ~c"Change library directory", oldDir, style) do
          {:ok, newDir} ->
            case :lists.member(newDir, r_sys(sys, :lib_dirs)) do
              true ->
                s
  ...
  defp handle_popup_event(
    ...
    case :lists.nth(pos, choices) do
      :add ->
        ...
        case select_file(r_state(s, :frame), ~c"Select an escript file to add", oldFile2, style) do
          {:ok, newFile} ->
      ...
      :edit ->
        ...
        case select_file(r_state(s, :frame), ~c"Change escript file name", oldFile, style) do
          {:ok, newFile} ->
            case :lists.member(newFile, r_sys(sys, :escripts)) do
              true ->
                s
  ...
  defp handle_system_event(s, event, objRef, userData) do
    :error_logger.format(
      ~c"~w~w got unexpected wx sys event to ~p with user data: ~tp\n\t ~tp\n",
  ...
  defp handle_app_event(s, event, objRef, userData) do
    :error_logger.format(
      ~c"~w~w got unexpected wx app event to ~p with user data: ~tp\n\t ~tp\n",
      [:reltool_sys_win, self(), objRef, userData, event]
    )
  ...
  defp move_app(s, {_ItemNo, appBase}, action) do
    ...
    appCond =
      case action do
        ...
        _ ->
          :error_logger.format(
            ~c"~w~w got unexpected app button event: ~tp ~tp\n",
  ...
  defp update_app_graph(s) do
    ...
    title = :lists.concat([:reltool, ~c" - application graph"])
  ...
  defp update_mod_graph(s) do
    ...
    links = :lists.usort(whiteLinks ++ derivedLinks)
    title = :lists.concat([:reltool, ~c" - module graph"])
    create_fgraph_window(s, title, nodes, links)
  end
  ...
  defp load_config(r_state(status_bar: bar, config_file: oldFile) = s) do
    ...
    case select_file(
           r_state(s, :frame),
           ~c"Select a file to load the configuration from",
           oldFile,
           style
         ) do
      {:ok, newFile} ->
  ...
  defp save_config(r_state(config_file: oldFile) = s, inclDefaults, inclDerivates) do
    ...
    case select_file(
           r_state(s, :frame),
           ~c"Select a file to save the configuration to",
           oldFile,
           style
         ) do
      {:ok, newFile} ->
  ...
  defp gen_rel_files(r_state(status_bar: bar, target_dir: oldDir) = s) do
    ...
    case select_dir(
           r_state(s, :frame),
           ~c"Select a directory to generate rel, script and boot files to",
           oldDir,
           style
         ) do
      {:ok, newDir} ->
  ...
  defp gen_target(r_state(status_bar: bar, target_dir: oldDir) = s) do
    ...
    case select_dir(
           r_state(s, :frame),
           ~c"Select a directory to generate a target system to",
           oldDir,
           style
         ) do
      {:ok, newDir} ->
  ...
  defp check_and_refresh(s, status) do
    case status do
      ...
      {:error, reason} ->
        msg = :lists.flatten(:io_lib.format(~c"Error:\n\n~tp\n", [reason]))
  ...
  defp question_dialog(question, details) do
    ...
    dialog = :wxDialog.new(parent, -1, ~c"Undo dialog", [{:style, dialogStyle}])
