defmodule :m_reltool_sys_win do
  ...
  Record.defrecord(:r_state, :state, parent_pid: :undefined,
                                 server_pid: :undefined, app_wins: :undefined,
                                 sys: :undefined, common: :undefined,
                                 config_file: :undefined,
                                 target_dir: :undefined, boot_dir: :undefined,
                                 frame: :undefined, panel: :undefined,
                                 book: :undefined, rel_book: :undefined,
                                 lib_tree: :undefined, status_bar: :undefined,
                                 source: :undefined, whitelist: :undefined,
                                 blacklist: :undefined, derived: :undefined,
                                 fgraph_wins: :undefined, app_box: :undefined,
                                 mod_box: :undefined, warning_list: :undefined,
                                 warning_wins: :undefined)
  Record.defrecord(:r_root_data, :root_data, dir: :undefined)
  Record.defrecord(:r_lib_data, :lib_data, dir: :undefined,
                                    tree: :undefined, item: :undefined)
  Record.defrecord(:r_escript_data, :escript_data, file: :undefined,
                                        tree: :undefined, item: :undefined)
  Record.defrecord(:r_app_data, :app_data, name: :undefined,
                                    dir: :undefined)
  Record.defrecord(:r_app_win, :app_win, name: :undefined,
                                   pid: :undefined)
  Record.defrecord(:r_fgraph_win, :fgraph_win, frame: :undefined,
                                      pid: :undefined)
  Record.defrecord(:r_root_popup, :root_popup, dir: :undefined,
                                      choices: :undefined, tree: :undefined,
                                      item: :undefined)
  Record.defrecord(:r_lib_popup, :lib_popup, dir: :undefined,
                                     choices: :undefined, tree: :undefined,
                                     item: :undefined)
  Record.defrecord(:r_escript_popup, :escript_popup, file: :undefined,
                                         choices: :undefined, tree: :undefined,
                                         item: :undefined)
  def start_link(opts) do
    :proc_lib.start_link(:reltool_sys_win, :init,
  ...
  defp do_init([{:safe_config, safe}, {:parent, parent} |
                                     options]) do
    case (:reltool_server.start_link(options)) do
      {:ok, serverPid, c, sys} ->
        ...
        s = r_state(parent_pid: parent, server_pid: serverPid,
                common: c, config_file: :filename.absname('config.reltool'),
                target_dir: :filename.absname('reltool_target_dir'), app_wins: [],
                sys: sys, fgraph_wins: [], warning_wins: [])
        s2 = create_window(s)
        s5 = :wx.batch(fn () ->
  ...
  defp restart_server_safe_config(false, parent, reason) do
    :wx.new()
    strings = [{:wxe_util.get_const(:wxBLACK), 'Could not start reltool server:\n\n'},
                   {:wxe_util.get_const(:wxRED), reason ++ '\n\n'},
                       {:wxe_util.get_const(:wxBLACK),
                          :io_lib.format('Resetting the configuration to:~n~n  ~p~n~nDo you want to continue with this configuration?',
                                           [{:sys,
                                               [{:incl_cond, :exclude}, {:app,
                                                                           :kernel,
                                                                           [{:incl_cond,
                                                                               :include}]},
                                                                            {:app,
    ...
    case (question_dialog_2('Reltool server start error', strings)) do
      5100 ->
        do_init([{:safe_config, true}, {:parent, parent}, {:sys,
                                                             [{:incl_cond,
                                                                 :exclude},
                                                                  {:app,
                                                                     :kernel,
  ...
  defp exit_dialog(warnings) do
    ...
    details = :lists.flatten(for w <- warnings do
                               [w, ?\n]
                             end)
    ...
    receive do
      {:system, from, msg} ->
        ...
        :sys.handle_system_msg(msg, from, r_state(s, :parent_pid),
                                 :reltool_sys_win, r_common(common, :sys_debug), s)
            ...
            case (:lists.keysearch(objRef, r_fgraph_win(:frame), fWs)) do
              ...
              false ->
                ...
                case (:lists.member(objRef, wWs)) do
                  ...
                  false ->
                    :error_logger.format('~w~w got unexpected message:\n\t~tp\n', [:reltool_sys_win, self(), msg])
                    :reltool_sys_win.loop(s)
                end
            end
        end
      r_wx(id: 5006, event: r_wxCommand(type: :command_menu_selected),
      ...
      {:call, replyTo, ref, {:open_app, appName}} ->
        ...
        {:value, r_app_win(pid: appPid)} = :lists.keysearch(appName,
                                                      r_app_win(:name),
                                                      r_state(s2, :app_wins))
      ...
      {:EXIT, _Pid, _Reason} = exit ->
        {fWs, aWs} = handle_child_exit(exit, r_state(s, :fgraph_wins),
                                         r_state(s, :app_wins))
        :reltool_sys_win.loop(r_state(s, fgraph_wins: fWs, 
                                     app_wins: aWs))
      msg ->
        :error_logger.format('~w~w got unexpected message:\n\t~tp\n', [:reltool_sys_win, self(), msg])
        :reltool_sys_win.loop(s)
  ...
  defp create_window(s) do
    ...
    s2 = r_state(s, frame: frame,  panel: panel,  book: book, 
                status_bar: bar)
    s3 = :lists.foldl(fn fun, acc ->
                           fun.(acc)
                      end,
                        s2,
                        [&create_app_page/1, &create_lib_page/1,
                                                 &create_main_release_page/1,
                                                     &create_config_page/1])
    s4 = create_warning_list(s3)
    sizer = :wxBoxSizer.new(8)
    :wxSizer.add(sizer, book,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(sizer, r_state(s4, :warning_list),
                   [{:flag, 8192}])
  ...
  defp create_app_page(r_state(book: book) = s) do
    ...
    sourceCtrl = create_app_list_ctrl(panel, sizer, 'Available',
                                        :whitelist_add, :blacklist_add)
    :wxSizer.add(sizer,
                   :wxStaticLine.new(panel, [{:style, 8}]),
                   [{:border, 2}, {:flag,
                                     64 ||| 128 ||| 32 ||| 16 ||| 8192}])
    whiteCtrl = create_app_list_ctrl(panel, sizer, 'Included',
                                       :whitelist_del, :blacklist_add)
    :wxSizer.add(sizer,
                   :wxStaticLine.new(panel, [{:style, 8}]),
                   [{:border, 2}, {:flag,
                                     64 ||| 128 ||| 32 ||| 16 ||| 8192}])
    blackCtrl = create_app_list_ctrl(panel, sizer, 'Excluded',
    ...
    r_state(s, source: sourceCtrl,  whitelist: whiteCtrl, 
           blacklist: blackCtrl,  derived: derivedCtrl)
  end
  ...
  defp create_app_list_ctrl(panel, outerSz, title, tick, cross) do
    ...
    create_button(panel, buttonSz, listCtrl, title, 'wxART_CROSS_MARK',
                    cross)
    ...
    :wxEvtHandler.connect(listCtrl,
                            :command_list_item_activated)
    :wxWindow.connect(listCtrl, :enter_window)
    listCtrl
  end

  defp create_button(panel, sizer, listCtrl, title, bitMapName,
            action) do
  ...
  defp redraw_libs(r_state(lib_tree: tree, sys: sys) = s) do
    ...
    {:ok, erts} = :reltool_server.get_app(r_state(s, :server_pid),
                                            :erts)
    ...
    escriptData = r_escript_data(file: :undefined, tree: tree,
                      item: escriptItem)
  ...
  defp create_config_page(r_state(sys: sys, book: book) = s) do
    ...
    appBox = :wxRadioBox.new(panel, - 1, 'Application inclusion policy', {- 1, - 1},
                               {- 1, - 1}, appConds, [])
    ...
    :wxEvtHandler.connect(appBox,
                            :command_radiobox_selected,
                            [{:userData, :config_incl_cond}])
    modConds = :reltool_utils.mod_conds()
    modBox = :wxRadioBox.new(panel, - 1, 'Module inclusion policy', {- 1, - 1},
                               {- 1, - 1}, modConds, [])
  ...
  defp redraw_config_page(r_state(sys: sys, app_box: appBox,
              mod_box: modBox) = s) do
    appChoice = :reltool_utils.incl_cond_to_index(r_sys(sys, :incl_cond))
    :wxRadioBox.setSelection(appBox, appChoice)
    modChoice = :reltool_utils.mod_cond_to_index(r_sys(sys, :mod_cond))
  ...
  defp create_warning_list(r_state(panel: panel) = s) do
    ...
    :wxEvtHandler.connect(listCtrl,
                            :command_list_item_activated,
                            [{:userData, :warnings}])
    :wxEvtHandler.connect(listCtrl, :motion,
                            [{:userData, :warnings}])
  ...
  defp redraw_warnings(s) do
    {:ok,
       warnings} = :reltool_server.get_status(r_state(s, :server_pid))
  ...
  defp redraw_warnings(listCtrl, warnings) do
    ...
    show = fn warning, row ->
                ...
                :wxListCtrl.setItem(listCtrl, row, 0, warning,
                                      [{:imageId, 1}])
                row + 1
           end
  ...
  defp add_release_page(book, r_rel(name: relName, rel_apps: relApps)) do
    panel = :wxPanel.new(book, [])
    sizer = :wxBoxSizer.new(4)
    appNames = [:kernel, :stdlib | (for rA <- relApps do
                                      r_rel_app(rA, :name)
                                    end) -- [:kernel, :stdlib]]
    relBox = :wxListBox.new(panel, - 1,
                              [{:pos, {- 1, - 1}}, {:size, {- 1, - 1}},
                                                       {:choices,
  ...
  defp do_open_app(s, appBase) when is_list(appBase) do
    {appName,
       _AppVsn} = :reltool_utils.split_app_name(appBase)
  ...
  defp do_open_app(r_state(server_pid: serverPid, common: c,
              app_wins: appWins) = s,
            appName)
    ...
    case (:lists.keysearch(appName, r_app_win(:name), appWins)) do
      false ->
        wxEnv = :wx.get_env()
        {:ok, pid} = :reltool_app_win.start_link(wxEnv,
                                                   serverPid, c, appName)
  ...
  defp root_popup(s, root, tree, item) do
    ...
    choices = [:edit]
    popup = r_root_popup(dir: root, choices: choices, tree: tree,
                item: item)
  ...
  defp lib_popup(s, lib, tree, item) do
    ...
    choices = (case (:wxTreeCtrl.getItemData(tree, item)) do
    ...
    popup = r_lib_popup(dir: lib, choices: choices, tree: tree,
                item: item)
    :wxEvtHandler.connect(popupMenu, :command_menu_selected,
                            [{:userData, {:popup, popup}}])
    :wxWindow.popupMenu(r_state(s, :frame), popupMenu)
  ...
  defp escript_popup(s, file, tree, item) do
    ...
    popup = r_escript_popup(file: file, choices: choices, tree: tree,
                item: item)
    :wxEvtHandler.connect(popupMenu, :command_menu_selected,
                            [{:userData, {:popup, popup}}])
    :wxWindow.popupMenu(r_state(s, :frame), popupMenu)
  ...
  defp handle_event(s,
            r_wx(id: id, obj: objRef, userData: userData,
                event: event) = _Wx) do
    case (event) do
      _ when userData === :warnings or
      ...
      r_wxMouse(type: :enter_window) ->
        s
      _ ->
        case (:wxNotebook.getPageText(r_state(s, :book),
                                        :wxNotebook.getSelection(r_state(s, :book)))) do
          'Applications' ->
  ...
  defp handle_warning_event(s, objRef, _, r_wxMouse(type: :motion, x: x, y: y)) do
    pos = :reltool_utils.wait_for_stop_motion(objRef,
                                                {x, y})
    warning_list_set_tool_tip(:os.type(), objRef, pos)
    s
  end

  defp handle_warning_event(s, objRef, _,
            r_wxList(type: :command_list_item_activated,
                itemIndex: pos)) do
    text = :wxListCtrl.getItemText(objRef, pos)
    r_state(s, warning_wins: [display_warning(s, text) |
                            r_state(s, :warning_wins)])
  end

  defp handle_warning_event(s, _ObjRef, {:warning, frame},
            r_wxCommand(type: :command_button_clicked)) do
    :wxFrame.destroy(frame)
    r_state(s, warning_wins: :lists.delete(frame,
                                       r_state(s, :warning_wins)))
  end
  ...
  defp warning_list_set_tool_tip(_, listCtrl, pos) do
    case (:wxListCtrl.findItem(listCtrl, - 1, pos, 0)) do
      index when index >= 0 ->
        tip = (case (:wxListCtrl.getItemText(listCtrl,
                                               index)) do
                 '' ->
  ...
  defp display_warning(s, warning) do
    ...
    text = :wxTextCtrl.new(panel, - 1,
                             [{:value, warning}, {:style, textStyle}, {:size,
                                                                         {400,
                                                                            150}}])
    attr = :wxTextAttr.new()
    ...
    :wxButton.connect(close, :command_button_clicked,
                        [{:userData, {:warning, frame}}])
  ...
  defp handle_popup_event(r_state(sys: sys) = s, _Type, pos, _ObjRef,
            r_root_popup(dir: oldDir, choices: choices), _Str) do
  ...
  defp handle_popup_event(r_state(sys: sys) = s, _Type, pos, _ObjRef,
            r_lib_popup(dir: oldDir, choices: choices), _Str) do
    case (:lists.nth(pos, choices)) do
      :add ->
        {:ok, cwd} = :file.get_cwd()
        style = 1 ||| 16
          ...
          {:ok, newDir} ->
            case (:lists.member(newDir, r_sys(sys, :lib_dirs))) do
              ...
              false ->
                ...
                {before, [_ | after__]} = :lists.splitwith(pred,
                                                             r_sys(sys, :lib_dirs))
  ...
  defp handle_popup_event(r_state(sys: sys) = s, _Type, pos, _ObjRef,
            r_escript_popup(file: oldFile, choices: choices), _Str) do
    case (:lists.nth(pos, choices)) do
      :add ->
        oldFile2 = (case (oldFile) do
                      :undefined ->
                        {:ok, cwd} = :file.get_cwd()
  ...
  defp handle_system_event(r_state(sys: sys) = s,
            r_wxCommand(type: :command_radiobox_selected, cmdString: choice),
            _ObjRef, :config_mod_cond) do
  ...
  defp handle_system_event(s, event, objRef, userData) do
    :error_logger.format('~w~w got unexpected wx sys event to ~p with user data: ~tp\n\t ~tp\n',
  ...
  defp handle_app_event(s,
            r_wxList(type: :command_list_item_activated, itemIndex: pos),
            listCtrl, _UserData) do
    appName = :wxListCtrl.getItemText(listCtrl, pos)
    do_open_app(s, appName)
  end

  defp handle_app_event(s, r_wxCommand(type: :command_button_clicked), _ObjRef,
            {:app_button, action, listCtrl}) do
    items = :reltool_utils.get_items(listCtrl)
    handle_app_button(s, items, action)
  end

  defp handle_app_event(s, event, objRef, userData) do
    :error_logger.format('~w~w got unexpected wx app event to ~p with user data: ~tp\n\t ~tp\n',
                           [:reltool_sys_win, self(), objRef, userData, event])
    s
  end

  defp handle_app_button(r_state(server_pid: serverPid, status_bar: bar,
              app_wins: appWins) = s,
            items, action) do
    :wxStatusBar.setStatusText(bar, 'Processing libraries...')
    newApps = (for item <- items do
  ...
  defp do_set_sys(r_state(sys: sys, server_pid: serverPid,
              status_bar: bar) = s) do
    :wxStatusBar.setStatusText(bar, 'Processing libraries...')
  ...
  defp move_app(s, {_ItemNo, appBase}, action) do
    ...
    {:ok,
       oldApp} = :reltool_server.get_app(r_state(s, :server_pid),
                                           appName)
    appCond = (case (action) do
                 :whitelist_add ->
                 ...
                 :blacklist_del ->
                   :undefined
                 _ ->
                   :error_logger.format('~w~w got unexpected app button event: ~tp ~tp\n',
                                          [:reltool_sys_win, self(), action,
                                                                         appBase])
  ...
  defp do_set_app(r_state(server_pid: serverPid, status_bar: bar,
              app_wins: appWins) = s,
            newApp) do
    :wxStatusBar.setStatusText(bar, 'Processing libraries...')
    result = :reltool_server.set_app(serverPid, newApp)
    returnApp = (case (result) do
                   {:ok, analysedApp, _Warnings} ->
                     analysedApp
                   {:error, reason} ->
                     display_message(reason, 512)
                     {:ok, oldApp} = :reltool_server.get_app(serverPid,
                                                               r_app(newApp, :name))
                     oldApp
                 end)
    for aW <- appWins do
  ...
  defp redraw_apps(r_state(server_pid: serverPid, source: sourceCtrl,
              whitelist: whiteCtrl, blacklist: blackCtrl,
              derived: derivedCtrl) = s) do
    {:ok, sourceApps} = :reltool_server.get_apps(serverPid,
                                                   :source)
    {:ok, whiteApps} = :reltool_server.get_apps(serverPid,
                                                  :whitelist)
    {:ok, blackApps} = :reltool_server.get_apps(serverPid,
                                                  :blacklist)
    {:ok, derivedApps} = :reltool_server.get_apps(serverPid,
                                                    :derived)
    badApps = fn r_app(used_by_apps: usedBy) = a
                     when usedBy !== [] ->
    ...
    status = :lists.concat([whiteN, ' whitelisted modules and ', derivedN, ' derived modules, ',
  ...
  defp do_redraw_apps(listCtrl, apps, okImage, errImage) do
    ...
    show = fn {imageId, text, app},
                {row, modCount, items} ->
                :wxListCtrl.insertItem(listCtrl, row, '')
                cond do
                  rem(row, 2) === 0 ->
                    :wxListCtrl.setItemBackgroundColour(listCtrl, row,
                                                          {240, 240, 255})
                  true ->
                    :ignore
                end
                :wxListCtrl.setItem(listCtrl, row, 0, text,
                                      [{:imageId, imageId}])
                n = length(for m <- r_app(app, :mods),
                                 r_mod(m, :is_included) === true do
                             m
                           end)
                {row + 1, modCount + n, [{row, text} | items]}
           end
    {_, n, newItems} = :wx.foldl(show, {0, 0, []},
                                   :lists.sort(imageApps))
    :reltool_utils.select_items(listCtrl, oldItems,
  ...
  defp update_app_graph(s) do
    {:ok,
       whiteApps} = :reltool_server.get_apps(r_state(s, :server_pid),
                                               :whitelist)
    {:ok,
       derivedApps} = :reltool_server.get_apps(r_state(s, :server_pid),
                                                 :derived)
    whiteNames = (for a <- whiteApps do
                    r_app(a, :name)
                  end)
    derivedNames = (for a <- derivedApps do
                      r_app(a, :name)
                    end)
    nodes = whiteNames ++ derivedNames
    whiteLinks = (for a <- whiteApps, u <- r_app(a, :uses_apps),
                        u !== r_app(a, :name), :lists.member(u, nodes) do
                    [r_app(a, :name), u]
                  end)
    derivedLinks = (for a <- derivedApps,
                          u <- r_app(a, :uses_apps), u !== r_app(a, :name),
  ...
  defp update_mod_graph(s) do
    ...
    {:ok,
       derivedApps} = :reltool_server.get_apps(r_state(s, :server_pid),
                                                 :derived)
    whiteMods = :lists.usort(for a <- whiteApps,
                                   m <- r_app(a, :mods),
                                   r_mod(m, :is_included) === true do
                               m
                             end)
    derivedMods = :lists.usort(for a <- derivedApps,
                                     m <- r_app(a, :mods),
                                     r_mod(m, :is_included) === true do
                                 m
                               end)
    whiteNames = (for m <- whiteMods do
                    r_mod(m, :name)
                  end)
    derivedNames = (for m <- derivedMods do
                      r_mod(m, :name)
                    end)
    nodes = whiteNames ++ derivedNames
    whiteLinks = (for m <- whiteMods, u <- r_mod(m, :uses_mods),
                        u !== r_mod(m, :name), :lists.member(u, nodes) do
                    [r_mod(m, :name), u]
                  end)
    derivedLinks = (for m <- derivedMods,
                          u <- r_mod(m, :uses_mods), u !== r_mod(m, :name),
                          :lists.member(u, nodes) do
                      [r_mod(m, :name), u]
                    end)
    links = :lists.usort(whiteLinks ++ derivedLinks)
  ...
  defp create_fgraph_window(s, title, nodes, links) do
    ...
    {server, fgraph} = :reltool_fgraph_win.new(panel,
                                                 options)
    choose = fn :"*MISSING*" ->
                  :alternate
                _ ->
  ...
  defp load_config(r_state(status_bar: bar, config_file: oldFile) = s) do
    style = 1 ||| 16
    case (select_file(r_state(s, :frame), 'Select a file to load the configuration from', oldFile, style)) do
      {:ok, newFile} ->
        :wxStatusBar.setStatusText(bar, 'Processing libraries...')
        status = :reltool_server.load_config(r_state(s, :server_pid),
                                               newFile)
  ...
  defp save_config(r_state(config_file: oldFile) = s, inclDefaults,
            inclDerivates) do
    style = 2 ||| 4
    case (select_file(r_state(s, :frame), 'Select a file to save the configuration to', oldFile, style)) do
      {:ok, newFile} ->
        status = :reltool_server.save_config(r_state(s, :server_pid),
                                               newFile, inclDefaults,
                                               inclDerivates)
        check_and_refresh(r_state(s, config_file: newFile), status)
      :cancel ->
        s
  ...
  defp gen_rel_files(r_state(status_bar: bar, target_dir: oldDir) = s) do
    style = 2 ||| 4
    case (select_dir(r_state(s, :frame), 'Select a directory to generate rel, script and boot files to', oldDir, style)) do
      {:ok, newDir} ->
        :wxStatusBar.setStatusText(bar, 'Processing libraries...')
        status = :reltool_server.gen_rel_files(r_state(s, :server_pid),
                                                 newDir)
        check_and_refresh(s, status)
      :cancel ->
        s
  ...
  defp gen_target(r_state(status_bar: bar, target_dir: oldDir) = s) do
    style = 2 ||| 4
    case (select_dir(r_state(s, :frame), 'Select a directory to generate a target system to', oldDir, style)) do
      {:ok, newDir} ->
        :wxStatusBar.setStatusText(bar, 'Processing libraries...')
        status = :reltool_server.gen_target(r_state(s, :server_pid),
                                              newDir)
  ...
  defp select_dir(frame, message, defaultDir, style) do
    dialog = :wxDirDialog.new(frame,
                                [{:title, message}, {:defaultPath, defaultDir},
                                                        {:style, style}])
    choice = (case (:wxMessageDialog.showModal(dialog)) do
                5101 ->
                  :cancel
  ...
  defp question_dialog_2(dialogLabel, strings) do
    parent = :wx.typeCast(:wx.null(), :wxWindow)
    dialogStyle = 64 ||| 536870912 ||| 2048 ||| 1024 ||| 512 ||| 4096
    dialog = :wxDialog.new(parent, - 1, dialogLabel,
                             [{:style, dialogStyle}])
    color = :wxWindow.getBackgroundColour(dialog)
    textStyle = 16 ||| 32 ||| 1073741824
    text = :wxTextCtrl.new(dialog, - 1,
                             [{:size, {600, 400}}, {:style, textStyle}])
