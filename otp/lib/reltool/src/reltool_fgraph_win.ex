defmodule :m_reltool_fgraph_win do
  ...
  Record.defrecord(:r_fg_v, :fg_v, p: {0.0, 0.0},
                                v: {0.0, 0.0}, q: 5.0, m: 1.0, type: :dynamic,
                                color: :default, resides: :undefined,
                                selected: false)
  Record.defrecord(:r_state, :state, parent_pid: :undefined,
                                 frame: :undefined, window: :undefined,
                                 width: :undefined, height: :undefined,
                                 q_slider: :undefined, l_slider: :undefined,
                                 k_slider: :undefined, mouse_act: :undefined,
                                 is_frozen: :undefined, ticker: :undefined)
  Record.defrecord(:r_graph, :graph, pen: :undefined,
                                 brush: :undefined, font: :undefined,
                                 select: :none, offset: {0, 0},
                                 offset_state: false, ke: 0, vs: [], es: [])
  ...
  defp init([parentWin, pid, env, options]) do
    ...
    sQ = :wxSlider.new(parentWin, 90, 20, 1, 500,
                         [{:style, 8}])
    sL = :wxSlider.new(parentWin, 91, 20, 1, 500,
                         [{:style, 8}])
    sK = :wxSlider.new(parentWin, 92, 20, 1, 500,
                         [{:style, 8}])
    ...
    :wxSizer.add(sliderSizer, sQ,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(sliderSizer, sL,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(sliderSizer, sK,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(buttonSizer, sliderSizer,
                   [{:flag, 8192}, {:proportion, 1}])
    windowSizer = :wxBoxSizer.new(4)
    :wxSizer.add(windowSizer, buttonSizer,
                   [{:flag, 8192}, {:proportion, 0}])
    :wxSizer.add(windowSizer, win,
                   [{:flag, 8192}, {:proportion, 1}])
    ...
    :wxButton.setToolTip(win, 'Drag mouse while left mouse button is pressed to perform various operations. Combine with control key to select. Combine with shift key to lock single node.')
    ...
    me = self()
    ticker = spawn_link(fn () ->
                             ticker_init(me)
                        end)
    loop(r_state(parent_pid: pid, q_slider: sQ, l_slider: sL,
             k_slider: sK, mouse_act: 83, frame: parentWin,
             window: win, is_frozen: false, ticker: ticker),
  ...
  defp graph_add_node(key, color, g = r_graph(vs: vs)) do
    q = 20.0
    m = 0.5
    p = {:erlang.float(450 + :rand.uniform(100)),
           :erlang.float(450 + :rand.uniform(100))}
    r_graph(g, vs: :reltool_fgraph.add(key,
                                   r_fg_v(p: p, m: m, q: q, color: color), vs))
  end
  ...
  defp graph_change_node(key, color, g) do
    case (:reltool_fgraph.get(key, r_graph(g, :vs))) do
      :undefined ->
        g
      v ->
        r_graph(g, vs: :reltool_fgraph.set(key, r_fg_v(v, color: color),
                                       r_graph(g, :vs)))
    end
  end
  ...
  defp graph_add_link(key0, key1, g = r_graph(es: es)) do
    k = 60.0
    l = 5.0
    r_graph(g, es: :reltool_fgraph.add({key0, key1},
                                   r_fg_e(k: k, l: l), es))
  end
  ...
  defp ticker_loop(pid, time) do
    receive do after time ->
      ...
      d = :erlang.convert_time_unit(t1 - t0, :native,
                                      :milli_seconds)
      case (round(40 - d)) do
        ms when ms < 0 ->
          ticker_loop(pid, 0)
  ...
  defp delete_edges(es, [key | keys]) do
    edges = :reltool_fgraph.foldl(fn {{k1, k2}, _}, out
                                         when k1 === key ->
                                       [{k1, k2} | out]
  ...
  defp loop(s, g) do
    receive do
      r_wx(id: 80, event: r_wxCommand(type: :command_button_clicked)) ->
        ...
        es2 = set_spring(k, es)
        vs2 = :reltool_fgraph.map(fn {key, v} ->
                                       {key,
                                          r_fg_v(v, selected: false, 
                                                 type: :dynamic,  q: q)}
                                  end,
                                    r_graph(g, :vs))
        {xs, ys} = :reltool_fgraph.foldl(fn {_Key,
                                               r_fg_v(p: {x, y})},
                                              {xs, ys} ->
                                              {[x | xs], [y | ys]}
                                         end,
                                           {[], []}, vs2)
        offset = (case (length(xs)) do
                    0 ->
                      {0, 0}
                    n ->
        ...
        loop(s,
               r_graph(g, vs: vs2,  es: es2,  offset: offset, 
                      offset_state: false))
      r_wx(id: 86, event: r_wxCommand(type: :command_button_clicked)) ->
        isFrozen = (case (r_state(s, :is_frozen)) do
                      true ->
      ...
      r_wx(id: 81, event: r_wxCommand(type: :command_button_clicked)) ->
        vs = :reltool_fgraph.map(fn {key,
                                       v = r_fg_v(selected: true)} ->
      ...
      r_wx(id: 85, event: r_wxCommand(type: :command_button_clicked)) ->
        {vs1, keys} = :reltool_fgraph.foldl(fn {key,
                                                  r_fg_v(selected: true)},
                                                 {vs, ks} ->
      ...
      r_wx(id: 90,
          event: r_wxCommand(type: :command_slider_updated,
                     commandInt: q)) ->
      ...
      r_wx(event: r_wxKey(type: :key_down)) ->
        loop(s, g)
      r_wx(event: r_wxMouse(type: :left_down, shiftDown: shift,
      ...
      r_wx(event: r_wxMouse(type: :motion, shiftDown: shift,
                   controlDown: ctrl, x: x, y: y)) ->
        cond do
          shift ->
      ...
      r_wx(event: r_wxMouse(type: :left_up, shiftDown: shift,
                   controlDown: ctrl, x: x, y: y)) ->
        cond do
          shift ->
      ...
      r_wx(event: r_wxMouse(type: :right_down, x: _X, y: _Y)) ->
        loop(s, g)
      r_wx(event: r_wxMouse(type: :mousewheel,
      ...
      {req, :redraw} ->
        {sizeX, sizeY} = :wxWindow.getSize(r_state(s, :window))
        vs = :reltool_fgraph.step(r_graph(g, :vs), r_graph(g, :es),
  ...
  defp mouse_left_down_move(r_graph(vs: vs) = g, {x, y}) do
    case (coord_to_key(g, {x, y})) do
      ...
      {true, key} ->
        ...
        r_graph(g, vs: :reltool_fgraph.set(key, r_fg_v(v, type: :moving),
                                       vs), 
  ...
  defp coord_to_key(r_graph(vs: vs, offset: {xo, yo}), {x, y}) do
    xr = x - xo
    yr = y - yo
    :reltool_fgraph.foldl(fn {key, r_fg_v(p: {px, py})}, _
  ...
  defp mouse_left_up_move(g = (r_graph(select: select, vs: vs) = g), {x, y},
            shift) do
    case (select) do
      {:node, key, _, ^x, ^y} ->
        :io.format('click: ~p\n', [key])
  ...
  defp mouse_motion_move(g = r_graph(select: {:node, key, _, _, _}, vs: vs),
            {x, y}) do
    {xo, yo} = r_graph(g, :offset)
    v = :reltool_fgraph.get(key, vs)
    v2 = r_fg_v(v, p: {:erlang.float(x - xo),
                    :erlang.float(y - yo)})
  ...
  defp redraw(dC, _Size, g) do
    :wx.batch(fn () ->
                   ...
                   :wxDC.setPen(dC, pen)
                   draw_es(dC, r_graph(g, :vs), r_graph(g, :es), r_graph(g, :offset), pen,
                             brush)
                   ...
                   draw_text(dC, :reltool_fgraph.size(r_graph(g, :vs)),
                               :reltool_fgraph.size(r_graph(g, :es)), r_graph(g, :ke))
                   :ok
              end)
  end
  ...
  defp draw_es(dC, vs, es, po, pen, brush) do
    :reltool_fgraph.foreach(fn {{k1, k2}, _} ->
  ...
  defp draw_arrow(dC, {x0, y0}, {x1, y1}, {x, y}, pen, brush) do
    ...
    points = [{round(x2), round(y2)}, {round(x3),
                                         round(y3)},
                                          {round(x4), round(y4)}]
  ...
  defp draw_line(dC, {x0, y0}, {x1, y1}, {x, y}) do
    :wxDC.drawLine(dC, {round(x0 + x), round(y0 + y)},
                     {round(x1 + x), round(y1 + y)})
  end

  defp draw_vs(dC, vs, {xo, yo}, pen, brush) do
    :reltool_fgraph.foreach(fn {key,
                                  r_fg_v(p: {x, y}, color: color, selected: sel)} ->
                                 string = s(key)
                                 case (sel) do
                                   true ->
                                     :wxPen.setColour(pen, {235, 245, 230})
                                     ...
                                     selProps = {round(x - 12 + xo),
                                                   round(y - 12 + yo), 24, 24}
                                     :wxDC.drawRoundedRectangle(dC, selProps,
                                                                  :erlang.float(10))
                                     :ok
                                   false ->
                                     :ok
                                 ...
                                 nodeProps = {round(x - 8 + xo),
                                                round(y - 8 + yo), 17, 17}
                                 :wxDC.drawRoundedRectangle(dC, nodeProps,
                                                              :erlang.float(10))
                                 :wxDC.drawText(dC, string,
                                                  {round(x + xo),
                                                     round(y + yo)})
                                 :ok
                               _ ->
