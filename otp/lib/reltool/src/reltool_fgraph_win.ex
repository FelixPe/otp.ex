defmodule :m_reltool_fgraph_win do
  ...
  defp init([parentWin, pid, env, options]) do
    :wx.set_env(env)
    bReset = :wxButton.new(parentWin, 80, [{:label, ~c"Reset"}])
    bFreeze = :wxButton.new(parentWin, 86, [{:label, ~c"Freeze"}])
    bLock = :wxButton.new(parentWin, 81, [{:label, ~c"Lock"}])
    bUnlock = :wxButton.new(parentWin, 82, [{:label, ~c"Unlock"}])
    bDelete = :wxButton.new(parentWin, 85, [{:label, ~c"Delete"}])
    ...
    :wxButton.setToolTip(bReset, ~c"Remove selection and unlock all nodes.")
    :wxButton.setToolTip(bFreeze, ~c"Start/stop redraw of screen.")
    :wxButton.setToolTip(bLock, ~c"Lock all selected nodes.")
    :wxButton.setToolTip(bUnlock, ~c"Unlock all selected nodes.")
    :wxButton.setToolTip(bDelete, ~c"Delete all selected nodes.")

    :wxButton.setToolTip(
      sQ,
      ~c"Control repulsive force. This can also be controlled with the mouse wheel on the canvas."
    )

    :wxButton.setToolTip(sL, ~c"Control link length.")
    :wxButton.setToolTip(sK, ~c"Control attractive force. Use with care.")

    :wxButton.setToolTip(
      win,
      ~c"Drag mouse while left mouse button is pressed to perform various operations. Combine with control key to select. Combine with shift key to lock single node."
    )
  ...
  defp draw_text(dC, nvs, nes, _KE) do
    vsString = ~c"#nodes: " ++ :erlang.integer_to_list(nvs)
    esString = ~c"#links: " ++ :erlang.integer_to_list(nes)
