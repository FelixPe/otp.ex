defmodule :m_et_wx_contents_viewer do
  ...
  def handle_event(
    ...
    case id do
      ...
      5003 ->
        ...
        fileName = :lists.flatten([~c"et_contents_viewer_", now_to_string(timeStamp), ~c".txt"])
        style = 2 ||| 4
      ...
      5010 ->
        ...
        text = ~c"<pre>" ++ :wxTextCtrl.getValue(r_state(s, :editor)) ++ ~c"</pre>"
  ...
  def handle_event(r_wx(event: r_wxKey(rawCode: keyCode)), s) do
    case keyCode do
      ...
      _ ->
        :io.format(~c"~p: ignored: ~p~n", [:et_wx_contents_viewer, keyCode])
        {:noreply, s}
    end
  end
  ...
  def handle_event(wx = r_wx(), s) do
    :io.format(~c"~p got an unexpected event: ~tp\n", [self(), wx])
    {:noreply, s}
  end
  ...
  defp create_window(s) do
    ...
    w = r_state(s, :width)
    name = r_state(s, :active_filter)
    title = :lists.concat([:et_wx_contents_viewer, ~c" (filter: ", name, ~c")"])
    winOpt = [{:size, {w, h}}]
  ...
  defp create_file_menu(bar) do
    menu = :wxMenu.new([])
    _ = :wxMenu.append(menu, 5003, ~c"Save")
    _ = :wxMenu.append(menu, 5010, ~c"Print")
    _ = :wxMenu.appendSeparator(menu)
    _ = :wxMenu.append(menu, 5006, ~c"Close")
    _ = :wxMenuBar.append(bar, menu, ~c"File")
    :ok
  end
  ...
  defp create_filter_menu(bar, filters) do
    ...
    _ =
      :wxMenuItem.enable(
        :wxMenu.append(menu, -1, ~c"Select Filter"),
        [{:enable, false}]
      )
    ...
    item = fn
      f, {n, acc} when r_filter(f, :name) === :all ->
        label = :lists.concat([pad_string(r_filter(f, :name), 20, ?\s, :right), ~c"(0)"])
        menuItem = menuitem(menu, -1, label, f)
        {n + 1, [menuItem | acc]}

      f, {n, acc} ->
        name = r_filter(f, :name)
        label = :lists.concat([pad_string(name, 20, ?\s, :right), ~c"(", n, ~c")"])
        menuItem = menuitem(menu, -1, label, f)
        {n + 1, [menuItem | acc]}
    end
    ...
    _ = :wxMenuBar.append(bar, menu, ~c"Filters")
    menuData
  end
    ...
    menuData =
      cond do
        ...
        from === to ->
          _ =
            :wxMenuItem.enable(
              :wxMenu.append(menu, -1, ~c"Hide actor in Viewer "),
              [{:enable, false}]
            )

          _ = :wxMenu.appendSeparator(menu)
          hide = menuitem(menu, -1, ~c"From=To (f|t|b)", {:hide, [from]})
          _ = :wxMenu.appendSeparator(menu)

          _ =
            :wxMenuItem.enable(
              :wxMenu.append(menu, -1, ~c"Show actor in Viewer "),
              [{:enable, false}]
            )

          _ = :wxMenu.appendSeparator(menu)
          show = menuitem(menu, -1, ~c"From=To (F|T|B)", {:show, [from]})
          [show, hide]
        ...
        true ->
          ...
          hide = [
            menuitem(menu, -1, ~c"From (f)", {:hide, [from]}),
            menuitem(menu, -1, ~c"To   (t)", {:hide, [to]}),
            menuitem(menu, -1, ~c"Both (b)", {:hide, [from, to]})
          ]
          ...
          show = [
            menuitem(menu, -1, ~c"From (F)", {:show, [from]}),
            menuitem(menu, -1, ~c"To   (T)", {:show, [to]}),
            menuitem(menu, -1, ~c"Both (B)", {:show, [from, to]})
          ]

          show ++ hide
      end

    _ = :wxMenuBar.append(bar, menu, ~c"Hide")
    menuData
  end
  ...
  defp create_search_menu(bar, s) do
    ...
    _ =
      :wxMenuItem.enable(
        :wxMenu.append(menu, -1, ~c"Search in Viewer "),
        [{:enable, false}]
      )
    ...
    menuData =
      cond do
        r_state(s, :viewer_pid) === :undefined ->
          [menuitem(menu, -1, ~c"Abort search. Display all (a)", {:mode, :all})]
        ...
        from === to ->
          ...
          [
            menuitem(menu, -1, ~c"Forward from this event   (s)", {:mode, modeS}),
            menuitem(menu, -1, ~c"Reverse from this event   (r)", {:mode, modeR}),
            menuitem(
              menu,
              -1,
              ~c"Abort search. Display all (a)",
              {:mode, :all}
            )
          ]
    ...
    _ = :wxMenuBar.append(bar, menu, ~c"Search")
    menuData
  end
