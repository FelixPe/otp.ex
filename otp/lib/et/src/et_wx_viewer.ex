defmodule :m_et_wx_viewer do
  ...
  Record.defrecord(:r_state, :state, parent_pid: :undefined,
                                 ...
                                 events: :undefined, n_events: :undefined,
                                 max_actors: :undefined, actors: :undefined,
                                 refresh_needed: :undefined,
                                 detail_level: :undefined,
                                 hide_actions: :undefined,
                                 hide_actors: :undefined,
                                 display_all: :undefined, context: :undefined,
                                 title: :undefined, frame: :undefined,
                                 menubar: :undefined, packer: :undefined,
                                 width: :undefined, height: :undefined,
                                 scale: :undefined, normal_font: :undefined,
                                 bold_font: :undefined, pen: :undefined,
                                 brush: :undefined, print_psdd: :undefined,
                                 print_d: :undefined, canvas_width: :undefined,
                                 canvas_height: :undefined, canvas: :undefined,
                                 canvas_sizer: :undefined,
                                 scroll_bar: :undefined, y_pos: :undefined,
                                 menu_data: :undefined,
                                 checkbox_data: :undefined,
                                 hide_actions_box: :undefined,
                                 hide_actors_box: :undefined,
                                 status_bar: :undefined, event_file: :undefined,
                                 wx_debug: :undefined, trap_exit: :undefined)
  Record.defrecord(:r_actor, :actor, name: :undefined,
                                 string: :undefined, include: :undefined,
                                 exclude: :undefined)
  Record.defrecord(:r_e, :e, pos: :undefined,
                             key: :undefined, event: :undefined)
    ...
    case (parse_opt(options, default_state(), [])) do
      {:ok, s, collectorOpt} ->
        case (r_state(s, :collector_pid)) do
          ...
          :undefined ->
            case (:et_collector.start_link([{:auto_shutdown, true} |
                                                collectorOpt])) do
              {:ok, collectorPid} ->
                s2 = r_state(s, collector_pid: collectorPid)
                case (:gen_server.start_link(:et_wx_viewer, [s2],
                                               [])) do
                  {:ok, pid} when r_state(s, :parent_pid) !== self() ->
                    :erlang.unlink(pid)
                    {:ok, pid}
  ...
  defp default_state() do
    r_state(parent_pid: self(), collector_pid: :undefined,
        n_events: 0, detail_level: 100, active_filter: :all,
        filters: [r_filter(name: :all,
                      function: fn e ->
                                     e
                                end)],
        event_order: :trace_ts, events_per_page: 100,
        first_event: :first, last_event: :first,
        events: queue_new(), max_actors: 5,
        actors: [create_actor('UNKNOWN')], pending_actor: 'UNKNOWN',
        hide_actions: false, hide_actors: false,
        display_all: true, context: :display,
        refresh_needed: false, scale: 2, canvas_height: 0,
        canvas_width: 0, width: 800, height: 600,
        event_file: :filename.absname('et_viewer.etrace'), wx_debug: 0,
        trap_exit: true)
  end

  defp parse_opt([], s, collectorOpt) do
    {:ok, s,
       [{:parent_pid, r_state(s, :parent_pid)} | collectorOpt]}
  end

  defp parse_opt([h | t], s, collectorOpt) do
    case (h) do
      {:parent_pid, parent} when is_pid(parent) or
                                   parent === :undefined
                                 ->
      ...
      {:title, title} ->
        parse_opt(t, r_state(s, title: name_to_string(title)),
                    collectorOpt)
      {:detail_level, level} when (is_integer(level) and
      ...
      {:width, w} when (is_integer(w) and w > 0) ->
        parse_opt(t, r_state(s, width: w,  canvas_width: w),
                    collectorOpt)
      {:height, wH} when (is_integer(wH) and wH > 0) ->
        parse_opt(t, r_state(s, height: wH,  canvas_height: wH),
                    collectorOpt)
      {:collector_pid, pid} when is_pid(pid) ->
        parse_opt(t, r_state(s, collector_pid: pid), collectorOpt)
      {:collector_pid, :undefined} ->
        parse_opt(t, r_state(s, collector_pid: :undefined),
                    collectorOpt)
      ...
      {:dict_insert, {:filter, name}, fun} ->
        cond do
          (is_atom(name) and is_function(fun)) ->
            ...
            filters = :lists.keydelete(name, r_filter(:name),
                                         r_state(s, :filters))
            collectorOpt2 = [h | collectorOpt]
            parse_opt(t, r_state(s, filters: filters ++ [f]),
                        collectorOpt2)
          true ->
            {:error, {:bad_option, h}}
        end
      ...
      {:actors, actorNames} when is_list(actorNames) ->
        actorNames2 = (case (:lists.member('UNKNOWN', actorNames)) do
                         false ->
                           ['UNKNOWN' | actorNames]
                         true ->
                           actorNames
                       end)
        actors = (for name <- actorNames2 do
                    create_actor(name)
                  end)
        parse_opt(t, r_state(s, actors: actors), collectorOpt)
      {:include, actorNames} when is_list(actorNames) ->
        actors = (for name <- actorNames do
      ...
      {:first_event, _FirstKey} ->
        parse_opt(t, s, collectorOpt)
      {:hide_actors, bool} when bool === true or
                                  bool === false
                                ->
        parse_opt(t, r_state(s, hide_actors: bool), collectorOpt)
      {:hide_actions, bool} when bool === true or
                                   bool === false
                                 ->
        parse_opt(t, r_state(s, hide_actions: bool), collectorOpt)
      {:hide_unknown, bool} when bool === true or
                                   bool === false
                                 ->
        parse_opt(t, s, collectorOpt)
      {:display_mode, _Mode} ->
  ...
  defp do_dict_insert({:filter, name}, fun, s) when (is_atom(name) and
                                           is_function(fun)) do
    f = r_filter(name: name, function: fun)
    filters = :lists.keydelete(name, r_filter(:name),
                                 r_state(s, :filters))
    filters2 = :lists.keysort(r_filter(:name), [f | filters])
    s2 = create_filter_menu(s, r_state(s, :active_filter),
                              filters2)
  ...
  defp do_dict_delete({:filter, name}, s) when (is_atom(name) and
  ...
  def init([s]) when elem(s, 0) === :state do
    ...
    :et_collector.dict_insert(r_state(s, :collector_pid),
                                {:subscriber, self()}, :et_wx_viewer)
            ...
            true ->
              ...
              cond do
                ...
                missing > 0 ->
                  ...
                  {s2, newEvents} = collect_more_events(r_state(s, n_events: n),
  ...
  def handle_info({:et, {:delete_actors, actorNames}}, s)
      when is_list(actorNames) do
    fun = fn n, actors when n === 'UNKNOWN' ->
               actors
             n, actors ->
  ...
  def handle_info(r_wx(id: id,
             event: r_wxCommand(type: :command_menu_selected)),
           s = r_state(filter_menu: {_, data})) do
    ...
    case (get_value(id, 3, r_state(s, :menu_data))) do
      ...
      :load_all ->
        ...
        s2 = (case (select_file(r_state(s, :frame), msg,
                                  r_state(s, :event_file), style)) do
                {:ok, newFile} ->
                  _ = :et_collector.start_trace_client(collectorPid,
                                                         :event_file, newFile)
      ...
      :save_all ->
        ...
        s2 = (case (select_file(r_state(s, :frame), msg,
                                  r_state(s, :event_file), style)) do
                {:ok, newFile} ->
                  :ok = :et_collector.save_event_file(collectorPid,
                                                        newFile,
                                                        [:existing, :write,
                                                                        :keep])
                  r_state(s, event_file: newFile)
      ...
      :print_all_pages = scope ->
        ...
        noreply(s2)
      {:open_viewer, scale} ->
        actors = (for a <- r_state(s, :actors) do
                    r_actor(a, :name)
                  end)
  ...
  def handle_info(r_wx(event: r_wxCommand(type: :command_slider_updated,
                      commandInt: level)),
           s) do
    cond do
      (level >= 0 and level <= 100) ->
                ...
                actors ->
                  ...
                  cond do
                    r_actor(a, :name) === r_actor(pending, :name) ->
                      ...
                      actors2 = :lists.keyreplace(r_actor(a, :name), r_actor(:name),
                                                    actors, a2)
                      displayAll = not
                                   :lists.keymember(true, r_actor(:include), actors2)
  ...
  def handle_info(r_wx(event: r_wxKey(keyCode: keyCode, shiftDown: sD)),
           s) do
    case (keyCode) do
      ?C when sD === true ->
        close_all(s)
  ...
  def handle_info(r_wx(event: r_wxMouse(type: :mousewheel,
                      wheelRotation: rot)),
           s)
      when rot > 0 do
    s2 = scroll_up(s)
  ...
  def handle_info(r_wx(event: r_wxSize(size: {oldW, oldH})) = wx, s) do
    r_wx(event: r_wxSize(type: :size,
                 size: {w, h})) = get_latest_resize(wx)
    s2 = r_state(s, width: w,  height: h,  canvas_width: w, 
                canvas_height: h)
  ...
  defp scroll_first(s) do
    ...
    {s2, newEvents} = collect_more_events(s, :first,
                                            eventsPerPage)
    s3 = (case (newEvents) do
            [] ->
              s2
            [firstE | _] ->
  ...
  defp scroll_last(s) do
    case (collect_more_events(s, :last, - 1)) do
      ...
      {s2, newEvents} ->
        [firstE | _] = newEvents
        s3 = replace_events(r_state(s2, first_event: firstE),
                              newEvents)
        refresh_main_window(s3)
    end
  end
  ...
  defp scroll_up(s, expected) do
    ...
    oldEvents = queue_to_list(r_state(s, :events))
    case (collect_more_events(s, r_state(s, :first_event),
                                - expected2)) do
      {_, []} ->
        s
      {s2, newEvents} ->
        ...
        cond do
          n + newN > eventsPerPage ->
            revAllEvents = :lists.reverse(oldEvents,
  ...
  defp scroll_down(s, expected) do
    ...
    expected2 = adjust_expected(expected, n, eventsPerPage)
    oldEvents = queue_to_list(r_state(s, :events))
    case (collect_more_events(s, r_state(s, :last_event),
                                expected2)) do
      {_, []} ->
        case (collect_more_events(s, r_state(s, :first_event),
                                    n - eventsPerPage)) do
  ...
  defp scroll_changed(s, expected) do
    cond do
      ...
      true ->
        oldPos = event_pos(s)
        newPos = :lists.min([oldPos + expected,
                                 r_state(s, :n_events)])
        case (r_state(s, :first_event)) do
          r_e(key: key, pos: ^oldPos) ->
  ...
  defp jump_up(s, oldKey, oldPos, newPos) do
    ...
    prevE = (cond do
               newPos === 0 ->
                 :first
               true ->
                 fun = fn event, r_e(pos: p) when p >= newPos ->
                            key = :et_collector.make_key(order, event)
                            r_e(event: event, key: key, pos: p - 1)
                          _E, acc ->
                            acc
                       end
                 :et_collector.iterate(r_state(s, :collector_pid), oldKey, try,
                                         fun, r_e(key: oldKey, pos: oldPos))
             end)
    case (collect_more_events(s, prevE,
                                r_state(s, :events_per_page))) do
      {_, []} ->
        s
      {s2, events} ->
        [firstE | _] = events
  ...
  defp jump_down(s, oldKey, oldPos, newPos) do
    ...
    prevE = :et_collector.iterate(r_state(s, :collector_pid),
                                    oldKey, try, fun,
  ...
  defp revert(s) do
    ...
    case (collect_more_events(s, r_state(s, :first_event),
      ...
      {s2, [_PrevEvent]} ->
        collect_more_events(s, r_state(s2, :first_event),
                              eventsPerPage)
    end
  end
  ...
  defp close_all_others(s) do
    ...
    all = :et_collector.dict_match(r_state(s, :collector_pid),
                                     {{:subscriber, :_}, :_})
  ...
  defp open_viewer(scale, filterName, actors, s) do
    filters = (for f <- r_state(s, :filters) do
                 {:dict_insert, {:filter, r_filter(f, :name)}, r_filter(f, :function)}
               end)
    options = [{:parent_pid, r_state(s, :parent_pid)}, {:title,
                                                    r_state(s, :title)},
                                                     {:collector_pid,
                                                        r_state(s, :collector_pid)},
                                                         {:detail_level,
                                                            r_state(s, :detail_level)},
                                                             {:active_filter,
                                                                filterName},
                                                                 {:event_order,
                                                                    r_state(s, :event_order)},
                                                                     {:first_event,
                                                                        r_state(s, :first_event)},
                                                                         {:max_actors,
                                                                            r_state(s, :max_actors)},
                                                                             {:hide_actions,
                                                                                r_state(s, :hide_actions)},
                                                                                 {:hide_actors,
                                                                                    r_state(s, :hide_actors)},
                                                                                     {:actors,
                                                                                        actors},
                                                                                         {:scale,
                                                                                            scale},
    ...
    case (start_link(options)) do
      {:ok, _ViewerPid} ->
        :ok
      {:error, reason} ->
        :ok = :error_logger.format('~p: Failed to start a new window: ~tp~n', [:et_wx_viewer, reason])
    end
  end
  ...
  defp create_main_window(s) do
    ...
    s2 = create_filter_menu(r_state(s, filter_menu: {filterMenu,
                                                 []}),
                              r_state(s, :active_filter), r_state(s, :filters))
    ...
    checkBoxData = [{:wxCheckBox.getId(hideActions),
                       :hide_actions},
    ...
    _ = :wxSizer.add(optSizer, checkSizer,
                       [{:border, 10}, {:flag, 64 ||| 128 ||| 32 ||| 16}])
    detailLevelBox = :wxStaticBoxSizer.new(4, panel,
                                             [{:label, 'Detail level'}])
    detailLevel = :wxSlider.new(panel, - 1,
                                  r_state(s, :detail_level), 0, 100,
    ...
    _ = :wxSizer.add(canvasSizer, canvas,
                       [{:flag, 8192}, {:proportion, 1}])
    _ = :wxSizer.add(canvasSizer, scrollBar,
                       [{:flag, 8192}])
    _ = :wxSizer.add(mainSizer, canvasSizer,
                       [{:flag, 8192}, {:proportion, 1}])
    ...
    s3 = r_state(s2, title: title,  frame: frame,  packer: panel, 
                 normal_font: normalFont,  bold_font: boldFont, 
                 canvas_width: canvasW,  canvas_height: canvasH, 
                 canvas: canvas,  canvas_sizer: canvasSizer, 
                 scroll_bar: scrollBar,  y_pos: 15 * r_state(s, :scale), 
                 pen: :wxPen.new(),  brush: :wxBrush.new(), 
                 print_d: :undefined,  print_psdd: :undefined, 
                 menu_data: menuData,  checkbox_data: checkBoxData, 
                 hide_actions_box: hideActions, 
                 hide_actors_box: hideActors,  status_bar: statusBar)
  ...
  defp event_pos(r_state(first_event: e, events: events,
              n_events: last)) do
    case (e) do
      r_e(pos: pos) ->
        pos
  ...
  defp init_printers(r_state(print_d: :undefined,
              print_psdd: :undefined) = s) do
    pD = :wxPrintData.new()
    pSDD = :wxPageSetupDialogData.new(pD)
    :wxPrintData.setPaperId(pD, 3)
    :wxPageSetupDialogData.setMarginTopLeft(pSDD, {15, 15})
    :wxPageSetupDialogData.setMarginBottomRight(pSDD,
                                                  {15, 15})
  ...
  defp select_fonts(scale) when is_integer(scale) do
    ...
    {:wxFont.new(size, 76, 90, 90, []),
       :wxFont.new(size, 76, 90, 92, [])}
  end
  ...
  defp create_file_menu(bar) do
    menu = :wxMenu.new([])
    data = [menuitem(menu, - 1, 'Clear all events in the Collector', :clear_all),
                menuitem(menu, - 1, 'Load events to the Collector from file', :load_all), menuitem(menu, - 1,
                                                              'Save all events in the Collector to file', :save_all),
                                                       menuitem(menu, 5011, 'Print setup',
                                                                  :print_setup),
                                                           menuitem(menu, - 1,
                                                                      'Print current page',
                                                                      :print_one_page),
  ...
  defp create_viewer_menu(bar) do
    menu = :wxMenu.new([])
    _ = :wxMenuItem.enable(:wxMenu.append(menu, - 1, 'Scroll this Viewer'),
                             [{:enable, false}])
    _ = :wxMenu.appendSeparator(menu)
    d1 = [menuitem(menu, - 1, 'First    (f)', :first), menuitem(menu,
                                                     - 1, 'Last     (l)', :last),
                                              menuitem(menu, - 1, 'Prev     (p)', :prev),
                                                  menuitem(menu, - 1, 'Next     (n)', :next),
                                                      menuitem(menu, - 1, 'Refresh  (r)',
                                                                 :refresh)]
    _ = :wxMenu.appendSeparator(menu)
    d2 = [menuitem(menu, - 1, 'Up   5   (Up)', :up), menuitem(menu, - 1,
                                                  'Down 5   (Down)', :down)]
    _ = :wxMenu.appendSeparator(menu)
    _ = :wxMenuItem.enable(:wxMenu.append(menu, - 1, 'Actor visibility in this Viewer'),
                             [{:enable, false}])
    _ = :wxMenu.appendSeparator(menu)
  ...
  defp create_collector_menu(bar) do
    menu = :wxMenu.new([])
    _ = :wxMenuItem.enable(:wxMenu.append(menu, - 1, 'Scroll all Viewers'),
                             [{:enable, false}])
    _ = :wxMenu.appendSeparator(menu)
    data = [menuitem(menu, - 1, 'First   (F)', :first_all),
                menuitem(menu, - 1, 'Last    (L)', :last_all), menuitem(menu, - 1,
                                                              'Prev    (P)', :prev_all),
                                                       menuitem(menu, - 1, 'Next    (N)',
                                                                  :next_all),
                                                           menuitem(menu, - 1,
                                                                      'Refresh (R)',
                                                                      :refresh_all)]
  ...
  defp create_filter_menu(s = r_state(filter_menu: {menu, data}),
            activeFilterName, filters) do
    :wx.foreach(fn {_, i, _} ->
    ...
    item = fn f, {n, acc} when r_filter(f, :name) === :all ->
              ...
              f, {n, acc} ->
                label = :lists.concat([pad_string(r_filter(f, :name), 20), '(',
                                                                        n, ')'])
                {n + 1, [menuitem(menu, - 1, label, {:data, f}) | acc]}
           end
    d1 = [i1 = :wxMenu.append(menu, - 1, 'Same Filter New Scale'),
              :wxMenu.appendSeparator(menu)]
    :wxMenuItem.enable(i1, [{:enable, false}])
    {:value, filter} = :lists.keysearch(activeFilterName,
                                          r_filter(:name), filters)
    same = :lists.concat([pad_string(activeFilterName, 20),
                              '(=) same    scale'])
    larger = :lists.concat([pad_string(activeFilterName,
                                         20),
                                '(+) bigger  scale'])
    smaller = :lists.concat([pad_string(activeFilterName,
                                          20),
                                 '(-) smaller scale'])
    d2 = [menuitem(menu, - 1, same, {:data, filter, 0}),
              menuitem(menu, - 1, smaller, {:data, filter, - 1}),
                  menuitem(menu, - 1, larger, {:data, filter, 1}),
                      :wxMenu.appendSeparator(menu), i2 = :wxMenu.append(menu,
                                                                           - 1,
  ...
  defp clear_canvas(s) do
    ...
    s2 = r_state(s, refresh_needed: false, 
                y_pos: 15 * r_state(s, :scale),  canvas_width: canvasW, 
                canvas_height: canvasH,  events: queue_new())
  ...
  defp replace_events(s, []) do
    r_state(s, first_event: :first,  last_event: :first, 
           events: queue_new())
  end

  defp replace_events(s, events) do
    queue = :lists.foldl(fn e, q ->
                              queue_in(e, q)
                         end,
                           queue_new(), events)
  ...
  defp collect_more_events(s, prevKey = :last, try) do
    prevE = r_e(event: :undefined, key: prevKey,
                pos: r_state(s, :n_events))
    s2 = r_state(s, first_event: prevE,  last_event: prevE)
    do_collect_more_events(s2, try, prevE, [])
  end
  ...
  defp do_collect_more_events(r_state(collector_pid: collector, event_order: order,
              active_filter: active, filters: filters) = s,
            try, prevE, acc) do
    incr = (cond do
              try < 0 ->
                - 1
    ...
    {:value,
       r_filter(function: filterFun)} = :lists.keysearch(active,
                                                    r_filter(:name), filters)
    {_S, _Incr, _Order, _Active, _FilterFun, lastE,
       newEvents} = :et_collector.iterate(collector, prevKey,
                                            try, &collect_event/2,
                                            {s, incr, order, active, filterFun,
                                               prevE, []})
    expected = abs(try)
    actual = length(newEvents)
    missing = expected - actual
    {s2, acc2, try2} = (cond do
                          try < 0 ->
                            {r_state(s, first_event: lastE), newEvents ++ acc,
                               - missing}
                          true ->
  ...
  defp collect_event(event,
            {s, incr, order, active, filterFun, r_e(pos: prevPos),
               events}) do
    key = :et_collector.make_key(order, event)
    e = r_e(event: event, key: key, pos: prevPos + incr)
    {lastE, events2} = (case ((try do
                                filterFun.(event)
                              catch
                                :error, e -> {:EXIT, {e, __STACKTRACE__}}
                                :exit, e -> {:EXIT, e}
                          ...
                          true ->
                            case (is_hidden(r_event(event, :from), r_event(event, :to),
                                              s)) do
                              true ->
                                {e, events}
                              false ->
                          ...
                          {true, event2} ->
                            ...
                            case (is_hidden(r_event(event2, :from), r_event(event2, :to),
                                              s)) do
                              true ->
                                {e2, events}
                              false ->
                          ...
                          false ->
                            {e, events}
                          bad ->
                            contents = {:bad_filter, r_state(s, :active_filter), bad,
                                          event}
                            event2 = r_event(event, contents: contents, 
                                                from: :bad_filter, 
                                                to: :bad_filter)
  ...
  defp display_event(r_e(event: event) = e, s, dC)
    ...
    {fromRefresh, from} = ensure_actor(r_event(event, :from), s,
                                         dC)
    ...
    s4 = (cond do
            (fromRefresh !== false and toRefresh !== false) ->
              r_state(s3, refresh_needed: true, 
            ...
            fromName === toName ->
              case (r_state(s, :hide_actions)) do
                ...
                false ->
                  ...
                  draw_named_arrow(label, fromName, toName, fromPos,
                                     toPos, e, s3, dC)
              end
            true ->
              label = name_to_string(r_event(event, :label))
  ...
  defp draw_named_arrow(label, fromName, toName, fromPos, toPos, e, s,
            dC) do
  ...
  defp draw_arrow(fromPos, toPos, s, dC) do
    ...
    points = [{round(toPos), round(y)}, {round(x3),
                                           round(y3)},
                                            {round(x4), round(y4)}]
    :wxBrush.setColour(r_state(s, :brush),
                         :wxe_util.get_const(:wxBLACK))
  ...
  defp draw_label(label, fromName, toName, fromPos, toPos, s,
            dC) do
    color = (cond do
    ...
    y = r_state(s, :y_pos)
    write_text(label, x, y, color, r_state(s, :normal_font), s,
                 dC)
    s
  end
  ...
  defp display_all(s) do
    ...
    actors2 = (for a <- actors do
                 r_actor(a, include: false,  exclude: false)
               end)
    s2 = r_state(s, actors: actors2,  display_all: true, 
                hide_actions: false,  hide_actors: false)
    :wxCheckBox.setValue(r_state(s2, :hide_actions_box),
                           r_state(s2, :hide_actions))
    :wxCheckBox.setValue(r_state(s2, :hide_actors_box),
  ...
  defp is_hidden(from, to, s) do
    ...
    case ({fromMatch, toMatch}) do
      ...
      {{:value, f}, {:value, t}} when r_actor(f, :include) or
                                        r_actor(t, :include)
                                      ->
  ...
  defp write_text(text, x, y, color, font, s, dC) do
    ...
    :wxDC.drawText(dC, text,
                     {x, round(y - 15 * r_state(s, :scale) / 2) - 3})
  end
  ...
  defp create_contents_window(event, {s, res}) do
    options = [{:viewer_pid, self()}, {:event, event},
  ...
  defp print_setup(s) do
    s2 = (r_state(print_psdd: pSDD0,
              print_d: pD0) = init_printers(s))
    :wxPageSetupDialogData.setPrintData(pSDD0, pD0)
    pSD = :wxPageSetupDialog.new(r_state(s, :frame),
                                   [{:data, pSDD0}])
  ...
  defp print(r_state(print_d: :undefined,
              print_psdd: :undefined) = s,
            scope) do
    s2 = print_setup(s)
    print(s2, scope)
  end
  ...
  defp print(r_state(print_psdd: pSDD, print_d: pD) = s, scope) do
    ...
    getPageInfo = fn this ->
                       ...
                       printerS = r_state(s, context: :printer,  canvas_width: pW, 
                                         canvas_height: pH)
                       eventsPerPage = events_per_page(printerS, pH)
                       pagedEvents = paged_events(printerS, scope,
                                                    eventsPerPage)
    ...
    printout1 = :wxPrintout.new('Print', onPrintPage,
                                  [{:getPageInfo, getPageInfo}, {:hasPage,
                                                                   hasPage}])
    printout2 = :wxPrintout.new('Print', onPrintPage,
                                  [{:getPageInfo, getPageInfo}, {:hasPage,
                                                                   hasPage}])
    preview = :wxPrintPreview.new(printout1,
                                    [{:printoutForPrinting, printout2}, {:data,
                                                                           pDD}])
    case (:wxPrintPreview.isOk(preview)) do
      true ->
        ...
        :wxPreviewFrame.connect(pF, :close_window,
                                  [{:callback, onClose}])
      false ->
  ...
  defp get_latest_scroll(r_wx(obj: objRef,
              event: r_wxScroll(type: :scroll_changed)) = wx) do
    receive do
      r_wx(obj: ^objRef,
          event: r_wxScroll(type: :scroll_changed)) = wx2 ->
  ...
  defp update_scroll_bar(r_state(scroll_bar: scrollBar, status_bar: statusBar,
              events_per_page: eventsPerPage, n_events: n) = s) do
    opts = [{:refresh, true}]
    {_, lineTopY, lineBotY} = calc_y(s)
    range = lineBotY - lineTopY
    eventPos = (case (event_pos(s)) do
                  1 ->
                    0
                  p ->
                    p
    ...
    cond do
      (n !== 0 and eventsPerPage !== 0) ->
        ...
        :wxScrollBar.setScrollbar(scrollBar,
                                    ...
                                    round(share * range), range,
                                    round(share * range), opts)
      true ->
        :wxScrollBar.setScrollbar(scrollBar, 0, range, range,
                                    range, opts)
    end
    :wxStatusBar.setStatusText(statusBar, where_text(s))
    s
  ...
  defp opt_create_actor(name, tag, s) do
    actors = r_state(s, :actors)
    new = (case (:lists.keysearch(name, r_actor(:name),
                                    actors)) do
             {:value, old} ->
               old
             false ->
  ...
  defp create_actor(name) do
    ...
    r_actor(name: name, string: string, include: false,
        exclude: false)
  end
  ...
  defp pad_string(string, minLen) when (is_integer(minLen) and
                                  minLen >= 0) do
