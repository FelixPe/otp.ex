defmodule :m_et_wx_viewer do
  ...
  defp default_state() do
    r_state(
      ...
      events: queue_new(),
      max_actors: 5,
      actors: [create_actor(~c"UNKNOWN")],
      pending_actor: ~c"UNKNOWN",
      hide_actions: false,
      hide_actors: false,
      display_all: true,
      context: :display,
      ...
      width: 800,
      height: 600,
      event_file: :filename.absname(~c"et_viewer.etrace"),
      wx_debug: 0,
      trap_exit: true
    )
  end
  ...
  defp parse_opt([h | t], s, collectorOpt) do
    case h do
      ...
      {:actors, actorNames} when is_list(actorNames) ->
        actorNames2 =
          case :lists.member(~c"UNKNOWN", actorNames) do
            false ->
              [~c"UNKNOWN" | actorNames]
  ...
  def handle_info(
    ...
    case get_value(id, 3, r_state(s, :menu_data)) do
      ...
      :load_all ->
        style = 1 ||| 4
        msg = ~c"Select a file to load events from"
      ...
      :save_all ->
        style = 2 ||| 4
        msg = ~c"Select a file to save events to"
      ...
      _ ->
        case get_value(id, 3, data) do
          {:data, f = r_filter(), scale} ->
            open_viewer(r_state(s, :scale) + scale, r_filter(f, :name), [~c"UNKNOWN"], s)

          {:data, f = r_filter()} ->
            open_viewer(r_state(s, :scale), r_filter(f, :name), [~c"UNKNOWN"], s)
  ...
  defp open_viewer(scale, filterName, actors, s) do
    ...
    case start_link(options) do
      ...
      {:error, reason} ->
        :ok =
          :error_logger.format(~c"~p: Failed to start a new window: ~tp~n", [
            :et_wx_viewer,
            reason
          ])
    end
  ...
  defp create_main_window(s) do
    ...
    frame =
      :wxFrame.new(:wx.null(), -1, title ++ ~c" (filter: " ++ name ++ ~c")", [
    ...
    :wxMenuBar.append(bar, filterMenu, ~c"Filters and scaling")
    create_help_menu(bar)
    optSizer = :wxBoxSizer.new(4)
    checkSizer = :wxBoxSizer.new(8)
    hideActions = :wxCheckBox.new(panel, -1, ~c"Hide From=To")
    :wxCheckBox.setValue(hideActions, r_state(s, :hide_actions))
    hideActors = :wxCheckBox.new(panel, -1, ~c"Hide (excluded actors)")
    ...
    detailLevelBox = :wxStaticBoxSizer.new(4, panel, [{:label, ~c"Detail level"}])
  ...
  defp create_file_menu(bar) do
    ...
    data = [
      menuitem(menu, -1, ~c"Clear all events in the Collector", :clear_all),
      menuitem(menu, -1, ~c"Load events to the Collector from file", :load_all),
      menuitem(menu, -1, ~c"Save all events in the Collector to file", :save_all),
      menuitem(menu, 5011, ~c"Print setup", :print_setup),
      menuitem(menu, -1, ~c"Print current page", :print_one_page),
      menuitem(
        menu,
        5010,
        ~c"Print all pages",
      ...
      menuitem(
        menu,
        -1,
        ~c"Close all other Viewers, but this (c)",
        :close_all_others
      ),
      menuitem(
        menu,
        -1,
        ~c"Close all Viewers and the Collector)   (C) ",
        :close_all
      )
    ]
    ...
    :wxMenuBar.append(bar, menu, ~c"File")
    data
  end
  ...
  defp create_viewer_menu(bar) do
    ...
    _ =
      :wxMenuItem.enable(
        :wxMenu.append(menu, -1, ~c"Scroll this Viewer"),
        [{:enable, false}]
      )
    ...
    d1 = [
      menuitem(menu, -1, ~c"First    (f)", :first),
      menuitem(menu, -1, ~c"Last     (l)", :last),
      menuitem(menu, -1, ~c"Prev     (p)", :prev),
      menuitem(menu, -1, ~c"Next     (n)", :next),
      menuitem(menu, -1, ~c"Refresh  (r)", :refresh)
    ]

    _ = :wxMenu.appendSeparator(menu)

    d2 = [
      menuitem(menu, -1, ~c"Up   5   (Up)", :up),
      menuitem(menu, -1, ~c"Down 5   (Down)", :down)
    ]
    ...
    _ =
      :wxMenuItem.enable(
        :wxMenu.append(menu, -1, ~c"Actor visibility in this Viewer"),
        [{:enable, false}]
      )

    _ = :wxMenu.appendSeparator(menu)
    d3 = [menuitem(menu, -1, ~c"Display all actors (a)", :display_all)]
    _ = :wxMenuBar.append(bar, menu, ~c"Viewer")
    [d1, d2, d3]
  end
  ...
  defp create_collector_menu(bar) do
    ...
    _ =
      :wxMenuItem.enable(
        :wxMenu.append(menu, -1, ~c"Scroll all Viewers"),
        [{:enable, false}]
      )
    ...
    data = [
      menuitem(menu, -1, ~c"First   (F)", :first_all),
      menuitem(menu, -1, ~c"Last    (L)", :last_all),
      menuitem(menu, -1, ~c"Prev    (P)", :prev_all),
      menuitem(menu, -1, ~c"Next    (N)", :next_all),
      menuitem(menu, -1, ~c"Refresh (R)", :refresh_all)
    ]

    _ = :wxMenuBar.append(bar, menu, ~c"Collector")
    data
  end
  ...
  defp create_filter_menu(s = r_state(filter_menu: {menu, data}), activeFilterName, filters) do
      ...
      fn
        ...
        i ->
          try do
            ...
            _, reason ->
              :io.format(~c"Could not delete item: ~tp, because ~tp.\n", [i, reason])
          end
      end,
      data
    )
    ...
    item = fn
      f, {n, acc} when r_filter(f, :name) === :all ->
        label = :lists.concat([pad_string(r_filter(f, :name), 20), ~c"(0)"])
        {n + 1, [menuitem(menu, -1, label, {:data, f}) | acc]}

      f, {n, acc} ->
        label = :lists.concat([pad_string(r_filter(f, :name), 20), ~c"(", n, ~c")"])
        {n + 1, [menuitem(menu, -1, label, {:data, f}) | acc]}
    end

    d1 = [i1 = :wxMenu.append(menu, -1, ~c"Same Filter New Scale"), :wxMenu.appendSeparator(menu)]
    :wxMenuItem.enable(i1, [{:enable, false}])
    {:value, filter} = :lists.keysearch(activeFilterName, r_filter(:name), filters)
    same = :lists.concat([pad_string(activeFilterName, 20), ~c"(=) same    scale"])
  ...
  defp create_help_menu(bar) do
    menu = :wxMenu.new([])
    _ = menuitem(menu, 5009, ~c"Info", :help)
    :wxMenuBar.append(bar, menu, ~c"Help")
  end
  ...
  defp draw_named_arrow(label, fromName, toName, fromPos, toPos, e, s, dC) do
    case r_state(s, :y_pos) + 15 * r_state(s, :scale) do
      _ when r_state(s, :hide_actors) === true and fromName === ~c"UNKNOWN" ->
        s

      _ when r_state(s, :hide_actors) === true and toName === ~c"UNKNOWN" ->
        s
  ...
  defp draw_label(label, fromName, toName, fromPos, toPos, s, dC) do
    color =
      cond do
        fromName === ~c"UNKNOWN" and toName === ~c"UNKNOWN" ->
          {2, 71, 254}
        ...
        {string, font} =
          cond do
            r_state(s, :context) === :display and r_actor(a, :exclude) ->
              {~c"(" ++ r_actor(a, :string) ++ ~c")", r_state(s, :normal_font)}

            r_state(s, :context) === :display and r_actor(a, :include) ->
              {~c"[" ++ r_actor(a, :string) ++ ~c"]", r_state(s, :bold_font)}
  ...
  defp create_contents_window(event, {s, res}) do
    ...
    case (try do
      ...
      {:error, reason} ->
        :ok =
          :error_logger.format(
            ~c"~p(~p): create_contents_window(~tp) ->~n     ~tp~n",
  ...
  defp print(r_state(print_psdd: pSDD, print_d: pD) = s, scope) do
    ...
    printout1 =
      :wxPrintout.new(~c"Print", onPrintPage, [{:getPageInfo, getPageInfo}, {:hasPage, hasPage}])
  ...
  defp name_to_string(name) do
    case (try do
      ...
      {:EXIT, _} ->
        :lists.flatten(:io_lib.format(~c"~tw", [name]))
