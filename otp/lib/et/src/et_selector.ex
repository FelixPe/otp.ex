  ...
  def parse_event(mod, trace) do
    ...
    case (trace) do
      {:trace, pid, label, info} ->
        parse_event(mod, trace, parsedTS, parsedTS, pid, label,
                      [info])
      {:trace, pid, label, info, extra} ->
        parse_event(mod, trace, parsedTS, parsedTS, pid, label,
                      [info, extra])
      {:trace_ts, pid, label, info, reportedTS} ->
        parse_event(mod, trace, parsedTS, reportedTS, pid,
                      label, [info])
      ...
      {:seq_trace, label, info, reportedTS} ->
        parse_seq_event(trace, parsedTS, reportedTS, label,
                          info)
      {:drop, numberOfDroppedItems} ->
        detailLevel = 20
        {true,
           r_event(detail_level: detailLevel, trace_ts: parsedTS,
               event_ts: parsedTS, from: :undefined, to: :undefined,
               label: :drop,
               contents: [{:label, :drop}, {:detail_level,
                                              detailLevel},
                                               {:from, :undefined}, {:to,
                                                                       :undefined},
                                                                        {:drop,
      ...
      _ ->
        :error_logger.format('~p(~p): Ignoring unknown trace type -> ~tp~n~n', [:et_selector, 212, trace])
        false
    end
  end
  ...
  defp parse_seq_event(trace, parsedTS, reportedTS, label, info) do
    case (info) do
      {:send, serial, from, to, msg} ->
        detailLevel = 15
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               event_ts: parsedTS, from: from, to: to,
               label: {:seq_send, label},
               contents: [{:label, {:seq_send, label}}, {:detail_level,
                                                           detailLevel},
                                                            {:from, from}, {:to,
                                                                              to},
                                                                               {:serial,
        ...
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               ...
               contents: [{:label, {:seq_receive, label}},
                              {:detail_level, detailLevel}, {:from, from}, {:to,
                                                                              to},
                                                                               {:serial,
                                                                                  serial},
                                                                                   {:msg,
      ...
      {:print, serial, from, _, userInfo} ->
        ...
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               event_ts: parsedTS, from: from, to: from,
                                                                               ...
                                                                               {:serial,
                                                                                  serial},
                                                                                   {:user_info,
                                                                                      userInfo}])}
      _ ->
        :error_logger.format('~p(~p): Ignoring unknown trace type -> ~tp~n~n', [:et_selector, 262, trace])
        false
    end
  end

  defp parse_event(mod, trace, parsedTS, reportedTS, from, label,
            contents) do
    case (label) do
      :receive ->
        detailLevel = 35
        ...
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               event_ts: parsedTS, from: from, to: from, label: label,
               contents: [{:label, label}, {:detail_level,
                                              detailLevel},
                                               {:from, from}, {:to, from},
                                                                  {:msg, msg}])}
      :send ->
        ...
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               event_ts: parsedTS, from: from, to: to, label: label,
               contents: [{:label, label}, {:detail_level,
                                              detailLevel},
                                               {:from, from}, {:to, to}, {:msg,
                                                                            msg}])}
      ...
      :call ->
        case (contents) do
          [{m, :trace_me,
              [userDetailLevel, userFrom, userTo, userLabel,
                                                      userContents]}]
              when (m == mod and mod != :undefined) ->
            {true,
               r_event(detail_level: userDetailLevel, trace_ts: reportedTS,
                   event_ts: parsedTS, from: userFrom, to: userTo,
                   label: userLabel, contents: userContents)}
          [{m, :report_event,
              [userDetailLevel, userFrom, userTo, userLabel,
                                                      userContents]}]
              when (m == mod and mod != :undefined) ->
      ...
      :spawn ->
        ...
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               event_ts: parsedTS, from: from, to: newPid,
               label: label,
               contents: [{:label, label}, {:detail_level,
                                              detailLevel},
                                               {:from, from}, {:to, newPid},
      ...
      :link ->
        ...
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               event_ts: parsedTS, from: from, to: linkTo,
               label: label,
               contents: [{:label, label}, {:detail_level,
                                              detailLevel},
                                               {:from, from}, {:to, linkTo}])}
      :unlink ->
        ...
        {true,
           r_event(detail_level: detailLevel, trace_ts: reportedTS,
               event_ts: parsedTS, from: from, to: unlinkFrom,
               label: label,
               contents: [{:label, label}, {:detail_level,
                                              detailLevel},
                                               {:from, from}, {:to,
                                              ...
                                              detailLevel},
                                               {:from, from}, {:to, from},
                                                                  {:gc_items,
                                                                     gcKeyValueList}])}
      _ ->
        :error_logger.format('~p(~p): Ignoring unknown trace type -> ~tp~n~n', [:et_selector, 594, trace])
        false
    end
  end
