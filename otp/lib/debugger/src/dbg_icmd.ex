defmodule :m_dbg_icmd do
  ...
  defp cmd(expr, bs, :running,
            r_ieval(level: le, module: m) = ieval) do
  ...
  defp break_p(mod, line, le, bs) do
    case (:lists.keysearch({mod, line}, 1,
                             :erlang.get(:breakpoints))) do
  ...
  def eval(meta, {mod, cmd, sP}) do
    cmd2 = (case (:lists.reverse(cmd)) do
              [10, ?. | _] ->
                cmd
              [10 | t] ->
                :lists.reverse([10, ?. | t])
  ...
  defp handle_int_msg({:attached, attPid}, status, _Bs,
            r_ieval(level: le, module: m, line: line)) do
    :erlang.put(:attached, attPid)
    :erlang.put(:next_break, :break)
    cond do
      le === 1 ->
        tell_attached({:attached, :undefined, - 1,
                         :erlang.get(:trace)})
      true ->
  ...
  defp handle_int_msg({:old_code, mod}, status, bs,
            r_ieval(level: le, module: m) = ieval) do
    cond do
  ...
  defp handle_int_msg({:break_options, break}, _Status, _Bs,
            _Ieval) do
    {point, _Options} = break
    :erlang.put(:breakpoints,
                  :lists.keyreplace(point, 1, :erlang.get(:breakpoints),
                                      break))
  end
  ...
  defp handle_int_msg({:no_break, m}, _Status, _Bs, _Ieval) do
    :erlang.put(:breakpoints,
                  for ({{mod, _L},
                          _Flags} = b) <- :erlang.get(:breakpoints),
                        mod !== m do
                    b
                  end)
  end
  ...
  defp handle_user_msg({:cmd, :stop}, status, _Bs, _Ieval) do
    case (:lists.member(status,
                          [:running, :wait_at, :wait_after_at])) do
      true ->
        :erlang.put(:next_break, :break)
  ...
  defp handle_user_msg({:set, :stack_trace, flag}, _Status, _Bs,
            _Ieval) do
    set_stack_trace(flag)
  end

  defp handle_user_msg({:get, :bindings, from, sP}, _Status, bs,
            _Ieval) do
    reply(from, :bindings, bindings(bs, sP))
  end

  defp handle_user_msg({:get, :stack_frame, from, {dir, sP}}, _Status,
            _Bs, _Ieval) do
    reply(from, :stack_frame,
            :dbg_istk.stack_frame(dir, sP))
  end

  defp handle_user_msg({:get, :messages, from, _}, _Status, _Bs,
            _Ieval) do
    reply(from, :messages, messages())
  end

  defp handle_user_msg({:get, :backtrace, from, n}, _Status, _Bs,
            ieval) do
  ...
  defp messages() do
    {:messages,
       msgs} = :erlang.process_info(:erlang.get(:self),
                                      :messages)
    msgs
  end
  ...
  defp eval_restricted({from, _Mod, cmd, sP}, bs) do
    case ((try do
      ...
      [{:var, _, var}] ->
        bs2 = bindings(bs, sP)
        res = (case (get_binding(var, bs2)) do
                 {:value, value} ->
                   value
                 :unbound ->
                   :unbound
  ...
  defp eval_nonrestricted({from, _Mod, cmd, _SP}, bs,
            r_ieval(level: le, module: m, line: line) = ieval) do
    case ((try do
            parse_cmd(cmd, line)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
      ...
      forms ->
        ...
        {res, bs1} = :lists.foldl(fn expr, {_Res, bs0} ->
  ...
  defp eval_nonrestricted_1({:match, _, {:var, _, var}, expr}, bs, ieval) do
    ...
    bs3 = (case (:lists.keyfind(var, 1, bs)) do
             {^var, _Value} ->
  ...
  defp eval_nonrestricted_1({:var, _, var}, bs, _Ieval) do
    res = (case (:lists.keyfind(var, 1, bs)) do
             {^var, value} ->
               value
             false ->
               :unbound
  ...
  defp eval_expr(expr, bs, ieval) do
    {:value, res, bs2} = :dbg_ieval.eval_expr(expr, bs,
