defmodule :m_dbg_istk do
  ...
  Record.defrecord(:r_ieval, :ieval, level: 1, line: - 1,
                                 ...
                                 top: false)
  Record.defrecord(:r_e, :e, level: :undefined,
                             mfa: :undefined, line: :undefined, error_info: [],
                             bindings: :undefined, lc: :undefined)
  def init() do
    init([])
  end
  ...
  def push(bs,
           r_ieval(level: le, module: mod, function: name, arguments: as,
               line: li) = ieval,
           lc) do
    entry = r_e(level: le, mfa: {mod, name, as}, line: li,
                bindings: bs, lc: lc)
  ...
  def delayed_stacktrace(:include_args, ieval) do
    r_ieval(module: mod, function: name, arguments: as, line: li,
        error_info: errorInfo) = ieval
    stack0 = [r_e(mfa: {mod, name, as}, line: li,
    ...
    fn numEntries ->
         case (stacktrace(numEntries, stack0, [])) do
           [] ->
             []
           [{_, withArgs} | stack] ->
             [finalize(withArgs) | for {arityOnly, _} <- stack do
                                     finalize(arityOnly)
                                   end]
         end
    end
  ...
  def delayed_stacktrace(:no_args, ieval) do
    r_ieval(module: mod, function: name, arguments: as,
        line: li) = ieval
    stack0 = [r_e(mfa: {mod, name, as}, line: li) |
                  :erlang.get(:dbg_istk)]
    fn numEntries ->
  ...
  defp normalize(r_e(mfa: {m, f, as}, line: li,
              error_info: errorInfo)) do
    loc = {m, li, errorInfo}
  ...
  defp line({mod, line, errorInfo}) do
    cond do
      line > 0 ->
        [{:file, :erlang.atom_to_list(mod) ++ '.erl'}, {:line,
                                                     line} |
                                                      errorInfo]
      true ->
        errorInfo
  ...
  def stack_frame(:down, sP) do
    stack_frame(sP, :down,
                  :lists.reverse(:erlang.get(:dbg_istk)))
  end
  ...
  def backtrace(howMany, ieval) do
    r_ieval(level: level, module: mod, function: name,
        arguments: as) = ieval
    stack0 = [r_e(level: level, mfa: {mod, name, as}) |
                  :erlang.get(:dbg_istk)]
    stack = (case (howMany) do
               :all ->
                 stack0
               n ->
  ...
  def in_use_p(mod, _Cm) do
    case (:erlang.get(:trace_stack)) do
      false ->
        true
      _ ->
        :lists.any(fn r_e(mfa: {m, _, _}) when m === mod ->
                        true
                      _ ->
                        false
                   end,
