defmodule :m_dbg_istk do
  ...
  def delayed_stacktrace(:include_args, ieval) do
    r_ieval(module: mod, function: name, arguments: as, line: li, error_info: errorInfo) = ieval

    stack0 = [
      r_e(mfa: {mod, name, as}, line: li, error_info: errorInfo)
      | :erlang.get(:dbg_istk)
    ]
  ...
  defp normalize(r_e(mfa: {m, fun, as}, line: li, error_info: errorInfo))
       when is_function(fun) do
    loc = {m, li, errorInfo}
    {{fun, length(as), loc}, {fun, as, loc}}
  end

  defp normalize(r_e(mfa: {m, f, as}, line: li, error_info: errorInfo)) do
    loc = {m, li, errorInfo}
  ...
  defp line({mod, line, errorInfo}) do
    cond do
      line > 0 ->
        [
          {:file, :erlang.atom_to_list(mod) ++ ~c".erl"},
          {:line, line}
          | errorInfo
        ]
