defmodule :m_dbg_wx_filedialog_win do
  ...
  def init([parent, id, options0]) do
    name = :proplists.get_value(:message, options0, ~c"Open")
    ...
    :lists.foldl(add, 0, [~c"Name", ~c"Type", ~c"Modified"])
  ...
  defp update_files(files, lC, icons) do
    ...
    :wx.foldl(
      fn f = r_file(name: name, type: typeStr, date: date, color: color), row ->
        :wxListCtrl.insertItem(lC, row, ~c"")
        ...
        temp =
          case :os.type() do
            {:win32, :nt} ->
              ...
              :wxFrame.new(win, -1, ~c"", [{:pos, pos}, {:size, size}, {:style, 8}])
  ...
  defp sort_files(mixed, sort) do
    {ds, fs} =
      :lists.foldr(
        fn
          dir = r_file(type: ~c"directory"), {ds, fs} ->
            {[dir | ds], fs}
  ...
  defp get_file_info([absName | rest], dir, filter, files, dirs) do
    ...
    case isDir do
      true when name === ~c".." ->
        entry = r_file(entry0, type: ~c"directory", icon: :prev_dir)
        get_file_info(rest, dir, filter, files, [entry | dirs])

      true ->
        entry = r_file(entry0, type: ~c"directory", icon: :dir)
        get_file_info(rest, dir, filter, files, [entry | dirs])
  ...
  defp format_date({{y, m, d}, {h, mi, s}}) do
    :lists.flatten(:io_lib.format(~c"~w-~2..0w-~2..0w ~2..0w:~2..0w:~2..0w", [y, m, d, h, mi, s]))
  end
  ...
  defp file_type_and_icon(_Dir, name) do
    case :filename.extension(name) do
      ~c".erl" ->
        {~c"erl src", :erl_src, {0, 90, 0}}

      ~c".hrl" ->
        {~c"erl hrl", :erl_hrl, {0, 90, 0}}

      ~c".beam" ->
        {~c"erl bin", :erl_bin, {0, 0, 0}}
  ...
  defp create_icons(extra) do
    ...
    builtIn0 = [
      {:file, ~c"wxART_NORMAL_FILE"},
      {:dir, ~c"wxART_FOLDER"},
      {:prev_dir, ~c"wxART_GO_DIR_UP"}
    ]
