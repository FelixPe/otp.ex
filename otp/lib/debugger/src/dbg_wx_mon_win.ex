defmodule :m_dbg_wx_mon_win do
  ...
  Record.defrecord(:r_wxHtmlLinkInfo, :wxHtmlLinkInfo, href: :undefined,
                                          target: :undefined)
  Record.defrecord(:r_moduleInfo, :moduleInfo, module: :undefined,
                                      menubtn: :undefined)
  Record.defrecord(:r_procInfo, :procInfo, pid: :undefined,
                                    row: :undefined)
  ...
  Record.defrecord(:r_winInfo, :winInfo, window: :undefined,
                                   ...
                                   focus: :undefined, modules: [],
                                   processes: [], breaks: [],
                                   listbox: :undefined, fbutton: :undefined,
                                   bbutton: :undefined, ebutton: :undefined,
                                   selected: [], stringsbutton: :undefined,
                                   slabel: :undefined, blabel: :undefined)
  def init() do
    :dbg_wx_win.init()
  end
  ...
  defp create_win_batch(title, menus) do
    win = :wxFrame.new(:wx.null(), - 1, title,
                         [{:size, {800, 390}}])
    ...
    listbox = :wxListBox.new(panel, - 1,
                               [{:size, {150, hlb}}, {:style, 32}])
    _ = :wxSizer.add(leftSz, listbox,
                       [{:proportion, 1}, {:border, 3}, {:flag, 8192}])
    ...
    :wxListCtrl.connect(grid, :key_up,
                          [{:id, 1000}, {:skip, true}])
    :wxWindow.connect(win, :enter_window, [{:skip, true}])
    :wxWindow.setFocus(grid)
    _ = :wxSizer.add(mainSz, leftSz,
                       [{:border, 3}, {:flag,
                                         64 ||| 128 ||| 32 ||| 16 ||| 8192}])
    _ = :wxSizer.add(mainSz, grid,
                       [{:border, 3}, {:flag,
                                         64 ||| 128 ||| 32 ||| 16 ||| 8192},
                                          {:proportion, 1}])
    ...
    r_winInfo(window: win, grid: grid, row: 0, focus: 0,
        listbox: listbox, fbutton: fbtn, bbutton: bbtn,
        ebutton: ebtn, stringsbutton: stringsbtn,
  ...
  defp option_buttons(winInfo, [:str_on | flags]) do
    [r_winInfo(winInfo, :stringsbutton) | option_buttons(winInfo,
                                                   flags)]
  end
  ...
  def add_module(winInfo, menuName, mod) do
    ...
    case (:lists.keymember(mod, r_moduleInfo(:module), modules)) do
      false ->
        ...
        menuBtn = :wxMenu.append(menu, - 1,
                                   :erlang.atom_to_list(mod), sub)
        :wxListBox.append(r_winInfo(winInfo, :listbox),
  ...
  def delete_module(winInfo, mod) do
    {:value, modInfo} = :lists.keysearch(mod, r_moduleInfo(:module),
                                           r_winInfo(winInfo, :modules))
    {menu, menuBtn} = r_moduleInfo(modInfo, :menubtn)
    :wxMenu.'Destroy'(menu, menuBtn)
    listBox = r_winInfo(winInfo, :listbox)
    id = :wxListBox.findString(listBox,
                                 :erlang.atom_to_list(mod))
    :wxListBox.delete(listBox, id)
    r_winInfo(winInfo, modules: :lists.keydelete(mod, r_moduleInfo(:module),
                                           r_winInfo(winInfo, :modules)))
  end

  def add_process(winInfo, pid, name, {mod, func, args}, status,
           info) do
    ...
    add = fn () ->
               ...
               cond do
                 rem(row, 2) === 0 ->
                   :wxListCtrl.setItemBackgroundColour(grid, row,
                                                         {240, 240, 255})
                 true ->
                   :ignore
               end
    ...
    r_winInfo(winInfo, processes: [procInfo |
                               r_winInfo(winInfo, :processes)], 
                 row: row + 1)
  end

  def update_process(winInfo, pid, status, info) do
    {:value, procInfo} = :lists.keysearch(pid, r_procInfo(:pid),
                                            r_winInfo(winInfo, :processes))
    grid = r_winInfo(winInfo, :grid)
  ...
  def handle_event(r_wx(event: r_wxSize(size: {w, _})), r_winInfo(grid: grid)) do
    :wx.batch(fn () ->
                   tot = :wx.foldl(fn c, sum ->
                                        sum + :wxListCtrl.getColumnWidth(grid,
                                                                           c)
                                   end,
                                     0, [0, 1, 2, 3])
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_listbox_doubleclicked,
                      cmdString: modS)),
           _WinInfo) do
    {:module, :erlang.list_to_atom(modS), :view}
  end

  def handle_event(r_wx(obj: listBox,
             event: r_wxMouse(type: :right_down, x: x, y: y)),
           r_winInfo(listbox: listBox)) do
  ...
  def handle_event(r_wx(id: 314,
    ...
    names = :wx.foldl(check, [],
                        [r_winInfo(winInfo, :ebutton), r_winInfo(winInfo, :bbutton),
                                                   r_winInfo(winInfo, :fbutton)])
    {:"Auto Attach", names}
  end

  def handle_event(r_wx(id: 271,
             event: r_wxCommand(type: :command_checkbox_clicked)),
           winInfo) do
    check = fn button, namesAcc ->
                 case (:wxCheckBox.isChecked(button)) do
  ...
  def handle_event(r_wx(event: r_wxMouse(type: :enter_window)),
           r_winInfo(grid: grid)) do
    :wxWindow.setFocus(grid)
    :ignore
  end
