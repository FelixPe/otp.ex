defmodule :m_dbg_wx_mon_win do
  ...
  defp create_win_batch(title, menus) do
    ...
    sBox = :wxStaticBox.new(panel, -1, ~c"Auto Attach:")
    sBS = :wxStaticBoxSizer.new(sBox, 8)
    fbtn = :wxCheckBox.new(panel, 314, ~c"First Call")
    _ = :wxSizer.add(sBS, fbtn)
    bbtn = :wxCheckBox.new(panel, 314, ~c"On Break")
    _ = :wxSizer.add(sBS, bbtn)
    ebtn = :wxCheckBox.new(panel, 314, ~c"On Exit")
    _ = :wxSizer.add(sBS, ebtn)
    :wxFrame.connect(panel, :command_checkbox_clicked)
    _ = :wxSizer.add(leftSz, sBS, [{:flag, 8192}])
    sLabel = :wxStaticText.new(panel, -1, ~c"Stack Trace:\n On (with tail)")
    _ = :wxSizer.add(leftSz, sLabel)
    bLabel = :wxStaticText.new(panel, -1, ~c"Back Trace Size:\n 50000")
    _ = :wxSizer.add(leftSz, bLabel)
    stringsBox = :wxStaticBox.new(panel, -1, ~c"Strings:")
    stringsBS = :wxStaticBoxSizer.new(stringsBox, 8)
    stringsbtn = :wxCheckBox.new(panel, 271, ~c"Use range of +pc flag")
  ...
  def add_module(winInfo, menuName, mod) do
    ...
    case :lists.keymember(mod, r_moduleInfo(:module), modules) do
      false ->
        ...
        sub = :wxMenu.new([])
        viewItem = :wxMenu.append(sub, -1, ~c"View")
        ...
        delItem = :wxMenu.append(sub, -1, ~c"Delete")
  ...
  def add_process(winInfo, pid, name, {mod, func, args}, status, info) do
    ...
    funcS = to_string(~c"~w:~tw/~w", [mod, func, length(args)])
    ...
    pid2 = to_string(~c"~p", [pid])

    add = fn ->
      _Dbg = :wxListCtrl.insertItem(grid, row, ~c"")
  ...
  def handle_event(
    ...
    case :wxListBox.hitTest(listBox, {x, y}) do
      ...
      row ->
        ...
        :io.format(~c"Re-loading/interpreting: ~s~n", [modS])
