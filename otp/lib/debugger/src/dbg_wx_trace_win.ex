defmodule :m_dbg_wx_trace_win do
  ...
  def create_win(parent, title, windows, menus) do
    do__ = fn ->
      ...
      iconFile = :dbg_wx_win.find_icon(~c"erlang_bug.png")
  ...
  def display(r_winInfo(window: win, sb: sb), arg) do
    str =
      case arg do
        :idle ->
          ~c"State: uninterpreted"

        {:exit, {mod, line}, reason} ->
          :wxWindow.raise(win)
          :dbg_wx_win.to_string(~c"State: EXITED [~w.erl/~w], Reason:~w", [mod, line, reason])

        {:exit, :null, reason} ->
          :wxWindow.raise(win)
          :dbg_wx_win.to_string(~c"State: EXITED [uninterpreted], Reason:~w", [reason])

        {level, :null, _Line} when is_integer(level) ->
          :dbg_wx_win.to_string(~c"*** Call level #~w (in non-interpreted code)", [level])

        {level, mod, line} when is_integer(level) ->
          :dbg_wx_win.to_string(~c"*** Call level #~w [~w.erl/~w]", [level, mod, line])
        ...
        {status, mod, line} ->
          ...
          :dbg_wx_win.to_string(~c"State: ~w [~w.erl/~w]", [what, mod, line])

        {:running, mod} ->
          :dbg_wx_win.to_string(~c"State: running [~w.erl]", [mod])
  ...
  def eval_output(r_winInfo(eval: r_sub(out: log)) = wi, prefix, term, mod, face) do
    {cW, _, _, _} = :wxTextCtrl.getTextExtent(log, ~c"w")
  ...
  def update_bindings(r_winInfo(bind: r_sub(out: bA)), mod, bs) do
    ...
    :wx.foldl(
      fn {var, val}, row ->
        :wxListCtrl.insertItem(bA, row, ~c"")
  ...
  defp format_term(val, mod, lineLenght, column, depth, limit) do
    ...
    opts = [
      {:line_length, lineLenght},
      {:depth, depth},
      {:chars_limit, limit},
      {:column, column},
      {:strings, useStrings},
      {:encoding, :unicode},
      {:record_print_fun, recFun}
    ]
  ...
  def handle_event(r_wx(id: 414, event: r_wxCommand(cmdString: str)), winInfo) do
    try do
      ...
      _, _ ->
        display(winInfo, {:text, ~c"Not a line number"})
        :ignore
    end
  end
  ...
  def handle_event(
    ...
    case :dbg_wx_code.find(r_sub(code, :out), str, case__, dir) do
      ...
      false ->
        display(wi, {:text, ~c"Not found (Hit Enter to wrap search)"})
        {:win, r_winInfo(wi, find: r_find(find, found: false))}
    end
  end

  def handle_event(
        r_wx(id: 413, event: r_wxCommand(cmdString: ~c"")),
  ...
  defp search_area(parent) do
    ...
    _ = :wxSizer.add(hSz, :wxStaticText.new(parent, -1, ~c"Find: "), [{:flag, 2048}])
    ...
    _ = :wxSizer.add(hSz, :wxStaticText.new(parent, -1, ~c"Goto Line: "), [{:flag, 2048}])
  ...
  defp eval_area(parent) do
    ...
    hSz = :wxBoxSizer.new(4)
    _ = :wxSizer.add(hSz, :wxStaticText.new(parent, -1, ~c"Evaluator: "), [{:flag, 2048}])
    tC = :wxTextCtrl.new(parent, 410, [{:style, 1024}])
    _ = :wxSizer.add(hSz, tC, [{:proportion, 1}, {:flag, 8192}])
    _ = :wxSizer.add(vSz, hSz, [{:flag, 8192}])
    tL = :wxTextCtrl.new(parent, 411, [{:style, 1_073_741_824 ||| 32 ||| 16 ||| 32768}])
  ...
  defp bind_area(parent) do
    ...
    lI = :wxListItem.new()
    :wxListItem.setText(lI, ~c"Name")
    :wxListItem.setAlign(lI, 0)
    :wxListCtrl.insertColumn(bA, 0, lI)
    :wxListItem.setText(lI, ~c"Value")
