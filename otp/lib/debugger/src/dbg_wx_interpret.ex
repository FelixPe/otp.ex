defmodule :m_dbg_wx_interpret do
    ...
    case :filename.extension(name) do
      ~c".erl" ->
        ...
        case :int.interpretable(file) do
          true ->
            {~c"erl src", :erl_src, {0, 0, 0}}

          _ ->
            {~c"erl src no dbg", :erl_src, {128, 128, 128}}
        end

      ~c".hrl" ->
        {~c"erl hrl", :erl_hrl, {128, 128, 128}}

      ~c".beam" ->
        {~c"erl bin", :erl_bin, {128, 128, 128}}

      _ ->
        {~c"file", :file, {128, 128, 128}}
    end
  end
  ...
  defp interpret_all(dir, [], _Mode, window, errors) do
    ...
    all = [~c"Error when interpreting: ", msg]
  ...
  defp format_error({:error, {:app, app}}) do
    ~c"Cannot interpret " ++ :erlang.atom_to_list(app) ++ ~c" modules"
  end
end
