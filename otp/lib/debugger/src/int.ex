defmodule :m_int do
  ...
  defp i2(absMod, dist, _Acc) when is_atom(absMod) or
  ...
  defp n2([absMod], dist) when is_atom(absMod) or
                                 is_list(absMod) do
    del_mod(absMod, dist)
  end
  ...
  def auto_attach(flags, {mod, func, args}) when (is_atom(mod) and
                                           is_atom(func) and is_list(args)) do
    check_flags(flags)
    :dbg_iserver.safe_cast({:set_auto_attach, flags,
                              {mod, func, args}})
  end
  ...
  def break(mod, line) when (is_atom(mod) and
                            is_integer(line)) do
    :dbg_iserver.safe_call({:new_break, {mod, line},
                              [:active, :enable, :null, :null]})
  end

  def delete_break(mod, line) when (is_atom(mod) and
                            is_integer(line)) do
    :dbg_iserver.safe_cast({:delete_break, {mod, line}})
  end

  def break_in(mod, func, arity) when (is_atom(mod) and
                                   is_atom(func) and is_integer(arity)) do
    case (:dbg_iserver.safe_call({:is_interpreted, mod,
                                    func, arity})) do
      {true, clauses} ->
  ...
  def del_break_in(mod, func, arity) when (is_atom(mod) and
                                   is_atom(func) and is_integer(arity)) do
    case (:dbg_iserver.safe_call({:is_interpreted, mod,
                                    func, arity})) do
      {true, clauses} ->
  ...
  def disable_break(mod, line) when (is_atom(mod) and
                            is_integer(line)) do
    :dbg_iserver.safe_cast({:break_option, {mod, line},
                              :status, :inactive})
  end

  def enable_break(mod, line) when (is_atom(mod) and
                            is_integer(line)) do
    :dbg_iserver.safe_cast({:break_option, {mod, line},
                              :status, :active})
  end

  def action_at_break(mod, line, action) when (is_atom(mod) and
                                    is_integer(line)) do
    check_action(action)
    :dbg_iserver.safe_cast({:break_option, {mod, line},
                              :action, action})
  ...
  def test_at_break(mod, line, function) when (is_atom(mod) and
                                      is_integer(line)) do
  ...
  defp check_function({mod, func}) when (is_atom(mod) and
                               is_atom(func)) do
    true
  end
  ...
  def continue(x, y, z) when (is_integer(x) and
  ...
  defp int_mod(absMod, dist) when is_atom(absMod) or
                               is_list(absMod) do
    case (check(absMod)) do
      {:ok, res} ->
        load(res, dist)
  ...
  defp load({mod, src, beam, beamBin, exp, abst}, dist) do
    ...
    case (:erl_prim_loader.get_file(:filename.absname(src))) do
      {:ok, srcBin, _} ->
        ...
        srcBin1 = :unicode.characters_to_binary(srcBin,
                                                  enc(srcBin))
        true = is_binary(srcBin1)
        bin = :erlang.term_to_binary({:interpreter_module, exp,
                                        abst, srcBin1, mD5})
        {:module, ^mod} = :dbg_iserver.safe_call({:load, mod,
                                                    src, bin})
        _ = everywhere(dist,
                         fn () ->
                              true = :erts_debug.breakpoint({mod, :_, :_},
                                                              true) > 0
                         end)
        {:module, mod}
      :error ->
  ...
  defp find_beam_1(beamFile, srcDir) do
    ...
    :lists.foldl(fn _, beam when is_list(beam) ->
                      beam
                    dir, :error ->
  ...
  defp check_beam(beamBin) when is_binary(beamBin) do
    case (:beam_lib.chunks(beamBin,
                             [:abstract_code, :exports])) do
      {:ok,
         {_Mod, [{:abstract_code, :no_abstract_code} | _]}} ->
        :error
      {:ok,
         {_Mod, [{:abstract_code, abst}, {:exports, exp}]}} ->
        {:ok, exp, abst, beamBin}
      _ ->
        :error
    end
  ...
  defp check_beam(beam) when is_list(beam) do
    {:ok, bin,
       _FullPath} = :erl_prim_loader.get_file(:filename.absname(beam))
    check_beam(bin)
  end

  defp is_file(name) do
    :filelib.is_regular(:filename.absname(name),
                          :erl_prim_loader)
  end
  ...
  defp scan_module_name(file) do
    try do
      {:ok, bin,
         _FullPath} = :erl_prim_loader.get_file(:filename.absname(file))
  ...
  defp scan_module_name_1(cont0, b0, bin0, enc) do
    ...
    {chars,
       b1} = (case (:unicode.characters_to_list(:erlang.list_to_binary([b0,
                                                                            bin1]),
                                                  enc)) do
  ...
  defp scan_module_name_3([{:-, _}, {:atom, _, :module}, {:"(", _} |
                                             _] = ts,
            _Chars, _B1, _Bin, _Enc) do
    scan_module_name_4(ts)
  end

  defp scan_module_name_3([{:-, _}, {:atom, _, _} | _], chars, b1, bin,
            enc) do
  ...
  defp scan_module_name_4(ts) do
    {:ok,
       {:attribute, _, :module, m}} = :erl_parse.parse_form(ts)
    true = is_atom(m)
    m
  end
