defmodule :m_int do
  ...
  defp int_mod(absMod, dist)
    ...
    case check(absMod) do
      ...
      {:error, {:app, app}} ->
        :io.format(~c"** Cannot interpret ~p module: ~p~n", [app, absMod])
        :error

      _Error ->
        :io.format(~c"** Invalid beam file or no abstract code: ~tp\n", [absMod])
        :error
    end
  end
  ...
  defp check_application2(~c"kernel-" ++ _) do
    throw({:error, {:app, :kernel}})
  end

  defp check_application2(~c"stdlib-" ++ _) do
    throw({:error, {:app, :stdlib}})
  end

  defp check_application2(~c"erts-" ++ _) do
    throw({:error, {:app, :erts}})
  end

  defp check_application2(~c"gs-" ++ _) do
    throw({:error, {:app, :gs}})
  end

  defp check_application2(~c"debugger-" ++ _) do
  ...
  defp find_src(mod, beam) do
    ...
    case is_file(src0) do
      ...
      false ->
        ...
        src = :filename.join([:filename.dirname(ebinDir), ~c"src", :filename.basename(src0)])
  ...
  defp find_beam_1(beamFile, srcDir) do
    ...
    ebinDir = :filename.join(rootDir, ~c"ebin")
  ...
  defp scan_module_name_3([{:-, _}, {:atom, _, _} | _], chars, b1, bin, enc) do
    scan_module_name_2(~c"", chars, b1, bin, enc)
  end
  ...
  defp del_mod(absMod, dist) do
    mod =
      cond do
        ...
        is_list(absMod) ->
          :erlang.list_to_atom(:filename.basename(absMod, ~c".erl"))
      end
