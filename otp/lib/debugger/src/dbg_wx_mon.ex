defmodule :m_dbg_wx_mon do
  ...
  Record.defrecord(:r_wxHtmlLinkInfo, :wxHtmlLinkInfo, href: :undefined,
                                          target: :undefined)
  Record.defrecord(:r_pinfo, :pinfo, pid: :undefined,
                                 status: :undefined)
  Record.defrecord(:r_state, :state, mode: :undefined,
                                 starter: :undefined, win: :undefined,
                                 focus: :undefined, coords: :undefined,
                                 intdir: :undefined, pinfos: :undefined,
                                 tracewin: :undefined, backtrace: :undefined,
                                 strings: :undefined, attach: :undefined,
                                 sfile: :undefined, changed: :undefined)
  ...
  defp init2(callingPid, mode, sFile, gS) do
    ...
    state1 = r_state(mode: mode, starter: bool, win: win,
                 focus: :undefined, coords: {- 1, - 1},
                 intdir: :erlang.element(2, :file.get_cwd()), pinfos: [],
                 sfile: sFile, changed: false)
    state2 = init_options([:"Search Area", :"Button Area", :"Evaluator Area", :"Bindings Area"],
                            :int.auto_attach(), :int.stack_trace(), 100,
                            [:str_on], state1)
    state3 = init_contents(:int.interpreted(),
                             :int.all_breaks(), :int.snapshot(), state2)
  ...
  defp init_options(traceWin, autoAttach, stackTrace, backTrace,
            strings, state) do
    ...
    case (autoAttach) do
      false ->
        :ignore
      {flags, _Function} ->
        :dbg_wx_mon_win.show_option(r_state(state, :win),
                                      :auto_attach, flags)
    ...
    :dbg_wx_mon_win.show_option(r_state(state, :win), :back_trace,
                                  backTrace)
    select(strings, [:str_on])
    :dbg_wx_mon_win.show_option(r_state(state, :win), :strings,
                                  strings)
    r_state(state, tracewin: traceWin,  backtrace: backTrace, 
               strings: strings)
  end

  defp init_contents(mods, breaks, processes, state) do
    win2 = :lists.foldl(fn mod, win ->
                             :dbg_wx_mon_win.add_module(win, :Module, mod)
                        end,
                          r_state(state, :win), mods)
    win3 = :lists.foldl(fn break, win ->
                             :dbg_wx_mon_win.add_break(win, :Break, break)
                        end,
                          win2, breaks)
    :lists.foldl(fn pidTuple, state0 ->
                      int_cmd({:new_process, pidTuple}, state0)
                 end,
                   r_state(state, win: win3), processes)
  end
  ...
  defp loop(state) do
    receive do
      ...
      r_wx() = guiEvent ->
        cmd = :dbg_wx_mon_win.handle_event(guiEvent,
                                             r_state(state, :win))
      ...
      {:dbg_wx_trace, from, :get_env} ->
        send(from, {:env, self(), :wx.get_env(),
                      :dbg_wx_mon_win.get_window(r_state(state, :win))})
        loop(state)
    end
  end
  ...
  defp gui_cmd(:"Load Settings...", state) do
    window = :dbg_wx_mon_win.get_window(r_state(state, :win))
    case (:dbg_wx_settings.load(window, r_state(state, :coords),
                                  r_state(state, :sfile))) do
      :cancel ->
        state
      {:ok, file} ->
  ...
  defp gui_cmd(:"Save Settings...", state) do
    window = :dbg_wx_mon_win.get_window(r_state(state, :win))
    case (:dbg_wx_settings.save(window, r_state(state, :coords),
                                  r_state(state, :sfile))) do
      :cancel ->
        state
      {:ok, file} ->
  ...
  defp gui_cmd(:Refresh, state) do
    ...
    state2 = r_state(state, win: win,  focus: :undefined, 
                        pinfos: [])
    :lists.foldl(fn pidTuple, s ->
  ...
  defp gui_cmd(:"Interpret...", state) do
    window = :dbg_wx_mon_win.get_window(r_state(state, :win))
    :dbg_wx_interpret.start(window, r_state(state, :coords),
                              r_state(state, :intdir), r_state(state, :mode))
    state
  end
  ...
  defp gui_cmd(:"Conditional Break...", state) do
    window = :dbg_wx_mon_win.get_window(r_state(state, :win))
    :dbg_wx_break.start(window, r_state(state, :coords),
                          :conditional)
    state
  end
  ...
  defp gui_cmd(:"Back Trace Size...", state) do
    window = :dbg_wx_mon_win.get_window(r_state(state, :win))
    what = {:integer,
              :erlang.integer_to_list(r_state(state, :backtrace))}
  ...
  defp gui_cmd({:focus, pid, win}, state) do
    {:value, pInfo} = :lists.keysearch(pid, r_pinfo(:pid),
                                         r_state(state, :pinfos))
  ...
  defp gui_cmd(:default, state) do
    case (:lists.member(:Attach,
                          menus(:enabled, r_state(state, :focus)))) do
      true ->
  ...
  defp int_cmd({:interpret, mod}, state) do
    win = :dbg_wx_mon_win.add_module(r_state(state, :win),
                                       :Module, mod)
  ...
  defp int_cmd({:new_process, {pid, function, status, info}},
            state) do
    name = registered_name(pid)
    pInfo = r_pinfo(pid: pid, status: status)
    win = :dbg_wx_mon_win.add_process(r_state(state, :win), pid,
                                        name, function, status, info)
  ...
  defp int_cmd({:new_status, pid, status, info}, state) do
    ...
    pInfos2 = :lists.keyreplace(pid, r_pinfo(:pid), pInfos,
                                  pInfo2)
    state2 = r_state(state, pinfos: pInfos2)
    :dbg_wx_mon_win.update_process(r_state(state2, :win), pid,
                                     status, info)
  ...
  defp int_cmd({:new_break, break}, state) do
    win = :dbg_wx_mon_win.add_break(r_state(state, :win), :Break,
                                      break)
    r_state(state, win: win)
  end

  defp int_cmd({:delete_break, point}, state) do
    win = :dbg_wx_mon_win.delete_break(r_state(state, :win),
                                         point)
    r_state(state, win: win)
  end
  ...
  defp menus() do
    [{:File, [{:"Load Settings...", 0}, {:"Save Settings...", 2}, :separator, {:Exit, 0}]},
         {:Edit, [{:Refresh, 0}, {:"Kill All", :no}]}, {:Module,
                                                 [{:"Interpret...", 0}, {:"Delete All Modules", :no},
                                                               :separator]},
                                                  {:Process,
                                                     [{:Step, 0}, {:Next, 0},
                                                                      {:Continue,
                                                                                  ...
                                                                                  {:Attach,
                                                                                     0},
                                                                                      {:Kill,
                                                                                         :no}]},
                                                      {:Break,
                                                         [{:"Line Break...", 5}, {:"Conditional Break...", :no},
                                                                       {:"Function Break...",
                                                                          :no},
                                                                           :separator,
                                                                               {:"Enable All",
  ...
  defp load_settings2(settings, state) do
    ...
    [traceWin, autoAttach, stackTrace, backTrace, strings,
                                                      files, breaks] = vals
  ...
  defp loaded({traceWin, autoAttach, stackTrace, backTrace,
             files, breaks}) do
    [traceWin, autoAttach, stackTrace, backTrace, :keep,
                                                      files, breaks]
  end

  defp loaded(settings) when is_list(settings) do
    keys = [:trace_win, :auto_attach, :stack_trace,
                                          :back_trace, :strings, :files,
                                                                     :breaks]
    for key <- keys do
  ...
  defp save_settings(sFile, state) do
    ...
    binary = :erlang.term_to_binary({:debugger_settings,
                                       settings})
  ...
  defp saved(r_state(tracewin: traceWin, backtrace: backTrace,
              strings: strings)) do
    [{:trace_win, traceWin}, {:auto_attach,
                                :int.auto_attach()},
                                 {:stack_trace, :int.stack_trace()},
                                     {:back_trace, backTrace}, {:strings,
                                                                  strings},
                                                                   {:files,
