defmodule :m_dbg_wx_trace do
  ...
  Record.defrecord(:r_state, :state, win: :undefined,
                                 coords: :undefined, pid: :undefined,
                                 meta: :undefined, status: :undefined,
                                 cm: :undefined, cm_obsolete: false,
                                 stack: :undefined, trace: :undefined,
                                 stack_trace: :undefined, backtrace: :undefined,
                                 strings: :undefined)
  def start(pid) do
  ...
  defp start(pid, env, parent, traceWin, backTrace,
            strings) do
  ...
  defp init(pid, parent, meta, traceWin, backTrace,
            strings) do
    :dbg_wx_trace_win.init()
    title = title(pid)
    win = :dbg_wx_trace_win.create_win(parent, title,
                                         traceWin, menus())
    window = :dbg_wx_trace_win.get_window(win)
    :dbg_wx_winman.insert(title, window)
    state1 = r_state(win: win, coords: {- 1, - 1}, pid: pid,
                 meta: meta, status: {:idle, :null, :null},
                 stack: {1, 1}, strings: [:str_on])
    state2 = init_options(traceWin, :int.stack_trace(),
                            backTrace, strings, state1)
  ...
  defp gui_cmd(:"Go To Line", state) do
    win = :dbg_wx_trace_win.helpwin(:gotoline,
                                      r_state(state, :win))
    r_state(state, win: win)
  end
  ...
  defp gui_cmd({:gotoline, line}, state) do
    win = :dbg_wx_trace_win.select_line(r_state(state, :win),
                                          line)
    r_state(state, win: win)
  end
  ...
  defp gui_cmd(:Stop, state) do
    ...
    cond do
      status === :wait_running ->
        win = :dbg_wx_trace_win.mark_line(r_state(state, :win), line,
                                            :break)
        gui_enable_functions(:wait_break)
        gui_enable_updown(r_state(state, :stack_trace),
                            r_state(state, :stack))
        gui_enable_btrace(r_state(state, :trace),
                            r_state(state, :stack_trace))
        :dbg_wx_trace_win.display(r_state(state, :win),
                                    {:wait, mod, line})
  ...
  defp gui_cmd(:Where, state) do
    ...
    win = gui_show_module(r_state(state, :win), mod, line,
                            r_state(state, :cm), r_state(state, :pid), :break)
    gui_update_bindings(r_state(state, :win), mod,
                          r_state(state, :meta))
  ...
  defp gui_cmd(:Messages, state) do
    _ = (case (:int.meta(r_state(state, :meta), :messages)) do
           [] ->
             :dbg_wx_trace_win.eval_output(r_state(state, :win), '< No Messages!\n', :bold)
           messages ->
             :dbg_wx_trace_win.eval_output(r_state(state, :win), '< --- Current Messages ---\n', :bold)
             :lists.foldl(fn msg, n ->
                               str1 = :io_lib.format(' ~w:', [n])
                               :dbg_wx_trace_win.eval_output(r_state(state, :win),
                                                               str1, msg,
                                                               :normal)
                               n + 1
                          end,
  ...
  defp gui_cmd(:"Back Trace", state) do
    ...
    p = p(state)
    :lists.foreach(fn {le, {mod, func, args}} ->
                        str = :io_lib.format('~p > ~w:~tw~ts\n',
                                               [le, mod, func, format_args(args,
                                                                             p)])
                     ...
                     :int.meta(r_state(state, :meta), :backtrace,
                                 r_state(state, :backtrace)))
    :dbg_wx_trace_win.trace_output(r_state(state, :win), '\n')
    state
  end
  ...
  defp gui_cmd(:Up, state) do
    ...
    case (:int.meta(r_state(state, :meta), :stack_frame,
                      {:up, cur})) do
      {new, {:undefined, -1}, _Bs} ->
        stack = {new, max}
      ...
      {new, {mod, line}, bs} ->
        stack = {new, max}
        win = gui_show_module(r_state(state, :win), mod, line,
                                r_state(state, :cm), r_state(state, :pid), :where)
        :dbg_wx_trace_win.update_bindings(r_state(state, :win), mod,
                                            bs)
        gui_enable_updown(r_state(state, :stack_trace), stack)
        :dbg_wx_trace_win.display(r_state(state, :win),
                                    {new, mod, line})
  ...
  defp gui_cmd(:"Line Break...", state) do
    add_break(r_state(state, :win), r_state(state, :coords), :line,
                r_state(state, :cm),
  ...
  defp gui_cmd(:"Conditional Break...", state) do
    add_break(r_state(state, :win), r_state(state, :coords),
                :conditional, r_state(state, :cm),
                :dbg_wx_trace_win.selected_line(r_state(state, :win)))
    state
  end

  defp gui_cmd(:"Function Break...", state) do
    add_break(r_state(state, :win), r_state(state, :coords), :function,
                r_state(state, :cm), :undefined)
    state
  end
  ...
  defp gui_cmd(:"Enable All", state) do
    ...
    :lists.foreach(fn {{mod, line}, _Options}
                          when mod === thisMod ->
  ...
  defp gui_cmd({:"Trace Window", traceWin}, state) do
    ...
    win = :dbg_wx_trace_win.configure(r_state(state, :win),
                                        traceWin)
  ...
  defp gui_cmd(:Debugger, state) do
    ...
    helpFile = :filename.join([:code.lib_dir(:debugger), 'doc',
                                                             'html', 'part_frame.html'])
  ...
  defp gui_cmd({:user_command, cmd}, state) do
    ...
    cond do
      status === :break or status === :wait_break or
        status === :wait_running ->
        cm = r_state(state, :cm)
        arg = (case (r_state(state, :stack)) do
  ...
  defp gui_cmd({:edit, {var, value}}, state) do
    window = :dbg_wx_trace_win.get_window(r_state(state, :win))
    val = (case (r_state(state, :strings)) do
             [] ->
               :dbg_wx_win.to_string('~0ltp', [value])
             [:str_on] ->
               :dbg_wx_win.to_string('~0tp', [value])
           end)
    case (:dbg_wx_win.entry(window, 'Edit variable', var,
                              {:term, val})) do
      :cancel ->
        state
      {^var, term} ->
        cmd = :erlang.atom_to_list(var) ++ '= ' ++ :io_lib.format('~w',
                                                                 [term])
  ...
  defp int_cmd({:new_break, break}, state) do
    win = :dbg_wx_trace_win.add_break(r_state(state, :win),
                                        :Break, break)
    r_state(state, win: win)
  end

  defp int_cmd({:delete_break, point}, state) do
    win = :dbg_wx_trace_win.delete_break(r_state(state, :win),
                                           point)
    r_state(state, win: win)
  end

  defp int_cmd({:break_options, break}, state) do
    win = :dbg_wx_trace_win.update_break(r_state(state, :win),
                                           break)
    r_state(state, win: win)
  end
  ...
  defp meta_cmd({:attached, mod, line, _Trace}, state) do
    win = (cond do
             mod != :undefined ->
               ...
               gui_show_module(r_state(state, :win), mod, line,
                                 r_state(state, :cm), r_state(state, :pid), :break)
             true ->
               r_state(state, :win)
           end)
    r_state(state, win: win,  status: {:init, mod, line}, 
               cm: mod)
  end
  ...
  defp meta_cmd({:re_entry, mod, _Func}, state) do
    ...
    case (r_state(state, :cm)) do
      ^mod when obs === true ->
        win = gui_load_module(r_state(state, :win), mod,
                                r_state(state, :pid))
        r_state(state, win: win,  cm_obsolete: false)
      ^mod ->
        state
      cm ->
        win = gui_show_module(r_state(state, :win), mod, 0, cm,
                                r_state(state, :pid), :break)
  ...
  defp meta_cmd({:exit_at, {mod, line}, reason, cur}, state) do
    ...
    r_state(state, win: win,  cm: mod, 
               status: {:exit, {mod, line}, reason},  stack: stack)
  end
  ...
  defp meta_cmd({:break_at, mod, line, cur}, state) do
    ...
    :dbg_wx_trace_win.display(r_state(state, :win),
                                {:break, mod, line})
    r_state(state, win: win,  cm: mod, 
               status: {:break, mod, line},  stack: stack)
  end
  ...
  defp meta_cmd({:wait_at, mod, line, cur}, state) do
    ...
    :dbg_wx_trace_win.display(r_state(state, :win),
                                {:wait, mod, line})
    r_state(state, win: win,  cm: mod, 
               status: {:wait_break, mod, line},  stack: stack)
  end
  ...
  defp meta_cmd(:idle, state) do
    ...
    :dbg_wx_trace_win.update_bindings(r_state(state, :win),
                                        :undefined, [])
    :dbg_wx_trace_win.display(r_state(state, :win), :idle)
    r_state(state, win: win,  status: {:idle, :null, :null}, 
               cm: :undefined)
  end
  ...
  defp meta_cmd({:eval_rsp, res}, state) do
    :dbg_wx_trace_win.eval_output(r_state(state, :win), [?<], res,
                                    :normal)
    state
  end
  ...
  defp menus() do
    [{:File, [{:Close, :no}]}, {:Edit,
                                  [{:"Go To Line", 0}, {:Search, 1}]},
                                   {:Process,
                                      [{:Step, 0}, {:Next, 0}, {:Continue, 0},
                                                                   {:Finish, 0},
                                                                       {:Skip,
                                                                          :no},
                                                                           {:"Time Out",
                                                                                                               ...
                                                                                                               {:Up,
                                                                                                                  :no},
                                                                                                                   {:Down,
                                                                                                                      :no}]},
                                       {:Break,
                                          [{:"Line Break...", 5}, {:"Conditional Break...", :no}, {:"Function Break...", :no},
                                                                   :separator,
                                                                       {:"Enable All",
                                                                          :no},
                                                                           {:"Disable All",
                                           ...
                                           {:Options,
                                              [{:"Trace Window", :no, :cascade,
                                                  [{:"Search Area", :no, :check}, {:"Button Area", :no,
                                                                         :check},
                                                                          {:"Evaluator Area",
                                                                             :no,
                                                                             :check},
                                                   ...
                                                   {:"Stack Trace", :no, :cascade,
                                                      [{:"Stack On, Tail", :no, :radio}, {:"Stack On, No Tail",
                                                                             :no,
                                                                             :radio},
                                                                              {:"Stack Off",
                                                                                 :no,
  ...
  defp enable(:break) do
    [:Step, :Next, :Continue, :Finish, :Skip, :Kill,
                                                  :Messages]
  end
  ...
  defp gui_show_module(win, mod, line, _Cm, pid, how) do
    win2 = (case (:dbg_wx_trace_win.is_shown(win, mod)) do
              false ->
  ...
  defp gui_update_bindings(win, mod, meta) do
