defmodule :m_dbg_wx_trace do
    ...
    case :int.attached(pid) do
      {:ok, meta} ->
        try do
          ...
          e, r ->
            :io.format(~c"TraceWin Crashed ~p~n", [e])
            :io.format(~c" ~p in ~p~n", [r, __STACKTRACE__])
            exit(r)
        end
  ...
  def title(pid) do
    ~c"Attach Process " ++ :erlang.pid_to_list(pid)
  end
  ...
  defp gui_cmd(:Where, state) do
    ...
    gui_update_bindings(r_state(state, :win), mod, r_state(state, :meta))
  ...
  defp gui_cmd(:Messages, state) do
    _ =
      case :int.meta(r_state(state, :meta), :messages) do
        [] ->
          :dbg_wx_trace_win.eval_output(r_state(state, :win), ~c"< No Messages!\n", :bold)
        ...
        messages ->
          ...
          :lists.foldl(
            fn msg, n ->
              str1 = :io_lib.format(~c" ~w:", [n])
              :dbg_wx_trace_win.eval_output(r_state(state, :win), str1, msg, :normal)
              n + 1
            end,
            1,
            messages
  ...
  defp gui_cmd(:"Back Trace", state) do
    :dbg_wx_trace_win.trace_output(r_state(state, :win), ~c"\nBACK TRACE\n----------\n")
    p = p(state)
    ...
    :lists.foreach(
      fn
        ...
        {le, {fun, args}} ->
          str = :io_lib.format(~c"~p > ~p~ts~n", [le, fun, format_args(args, p)])
          :dbg_wx_trace_win.trace_output(r_state(state, :win), str)
  ...
  defp gui_cmd(:Up, state) do
    ...
    case :int.meta(r_state(state, :meta), :stack_frame, {:up, cur}) do
      {new, {:undefined, -1}, _Bs} ->
        ...
        :dbg_wx_trace_win.update_bindings(r_state(state, :win), :undefined, [])
        gui_enable_updown(r_state(state, :stack_trace), stack)
      ...
      {new, {mod, line}, bs} ->
        ...
        :dbg_wx_trace_win.update_bindings(r_state(state, :win), mod, bs)
        gui_enable_updown(r_state(state, :stack_trace), stack)
  ...
  defp gui_cmd(:"Back Trace Size...", state) do
    ...
    case :dbg_wx_win.entry(win, ~c"Backtrace", :"Backtrace:", {:integer, val}) do
      :cancel ->
        state
  ...
  defp gui_cmd(:Debugger, state) do
    ...
    helpFile = :filename.join([:code.lib_dir(:debugger), ~c"doc", ~c"html", ~c"part_frame.html"])
  ...
  defp gui_cmd({:user_command, cmd}, state) do
    ...
    cond do
      ...
      true ->
        str = ~c"Commands not allowed"
        :dbg_wx_trace_win.eval_output(r_state(state, :win), [?<, str, 10], :bold)
    end
  ...
  defp gui_cmd({:edit, {var, value}}, state) do
    ...
    val =
      case r_state(state, :strings) do
        [] ->
          :dbg_wx_win.to_string(~c"~0ltp", [value])

        [:str_on] ->
          :dbg_wx_win.to_string(~c"~0tp", [value])
      end

    case :dbg_wx_win.entry(window, ~c"Edit variable", var, {:term, val}) do
      :cancel ->
        state
  ...
  defp meta_cmd({:eval_rsp, res}, state) do
    :dbg_wx_trace_win.eval_output(r_state(state, :win), [?<], res, :normal)
    state
  end
