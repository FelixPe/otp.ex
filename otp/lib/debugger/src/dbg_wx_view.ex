defmodule :m_dbg_wx_view do
  use Bitwise
  require Record
  Record.defrecord(:r_state, :state, gs: :undefined,
                                 win: :undefined, coords: :undefined,
                                 mod: :undefined)
  def start(gS, mod) do
  ...
  defp init(gS, env, mod, title) do
    :wx.set_env(env)
    :int.subscribe()
    win1 = :dbg_wx_trace_win.create_win(gS, title, [:"Code Area", :"Search Area"],
                                          menus())
  ...
  defp loop(state) do
    receive do
      guiEvent when :erlang.element(1, guiEvent) === :wx ->
        cmd = :wx.batch(fn () ->
                             :dbg_wx_trace_win.handle_event(guiEvent,
                                                              r_state(state, :win))
                        end)
  ...
  defp gui_cmd(:"Line Break...", state) do
    add_break(r_state(state, :gs), r_state(state, :coords), :line,
                r_state(state, :mod),
  ...
  defp gui_cmd(:"Conditional Break...", state) do
    add_break(r_state(state, :gs), r_state(state, :coords),
                :conditional, r_state(state, :mod),
                :dbg_wx_trace_win.selected_line(r_state(state, :win)))
    state
  end

  defp gui_cmd(:"Function Break...", state) do
    add_break(r_state(state, :gs), r_state(state, :coords), :function,
                r_state(state, :mod), :undefined)
    state
  end
  ...
  defp int_cmd({:new_break, {{mod, _Line}, _Options} = break},
            r_state(mod: mod, win: win) = state) do
    r_state(state, win: :dbg_wx_trace_win.add_break(win, :Break,
                                                break))
  end

  defp int_cmd({:delete_break, {mod, _Line} = point},
            r_state(mod: mod, win: win) = state) do
    r_state(state, win: :dbg_wx_trace_win.delete_break(win,
                                                   point))
  end

  defp int_cmd({:break_options,
             {{mod, _Line}, _Options} = break},
            r_state(mod: mod, win: win) = state) do
    r_state(state, win: :dbg_wx_trace_win.update_break(win,
                                                   break))
  end
  ...
  defp menus() do
    [{:File, [{:Close, 0}]}, {:Edit,
                                [{:"Go To Line", 0}, {:Search, 0}]},
                                 {:Break,
                                    [{:"Line Break...", 5}, {:"Conditional Break...", 13}, {:"Function Break...", 0}, :separator,
                                                                     {:"Enable All", :no},
                                                                         {:"Disable All",
                                                                            :no},
                                                                             {:"Delete All",
