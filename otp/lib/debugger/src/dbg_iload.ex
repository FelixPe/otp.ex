defmodule :m_dbg_iload do
  ...
  defp store_forms([{:function, _, name, arity, cs0} | fs], mod, db, %{exp: exp} = st) do
    fA = {name, arity}
  ...
  defp store_forms([{:attribute, _, :record, {name, defs}} | fs], mod, db, st) do
    nDefs = normalise_rec_fields(defs)

    fields =
      for {:record_field, _, {:atom, _, f}, _} <- nDefs do
        f
      end

    :dbg_idb.insert(db, {:record, mod, name, length(fields)}, fields)
    recs = :maps.get(:recs, st, %{})
    store_forms(fs, mod, db, Map.put(st, :recs, Map.put(recs, name, nDefs)))
  end

  defp store_forms([{:attribute, _, _Name, _Val} | fs], mod, db, st) do
    store_forms(fs, mod, db, st)
  end
  ...
  defp pattern(
         {:record_index, anno, name, field} = _DBG,
         st
       ) do
  ...
  defp guard_test(
         {:call, anno, {:remote, _, {:atom, _, :erlang}, {:atom, _, f}}, as0},
         st
       ) do
  ...
  defp expr({:maybe, anno, es0, {:else, _ElseAnno, cs0}}, lc, st) do
    es1 = exprs(es0, lc, st)
    cs1 = icr_clauses(cs0, lc, st)
  ...
  defp expr({:fun, anno, {:function, {:atom, _, m}, {:atom, _, f}, {:integer, _, a}}}, _Lc, _St)
  ...
  defp expr({:call, anno, {:remote, _, {:atom, _, :erlang}, {:atom, _, :self}}, []}, _Lc, _St) do
    {:dbg, ln(anno), :self, []}
  end
  ...
  defp expr({:call, anno, {:atom, _, :is_record}, [a, {:atom, _, name}]}, lc, st) do
  ...
  defp expr({:call, anno, {:atom, _AnnoA, :record_info}, [_, _] = as0}, lc, st) do
    as = expr_list(as0, st)
    expr(record_info_call(anno, as, st), lc, st)
  end

  defp expr({:call, anno, {:remote, _, {:atom, _, mod}, {:atom, _, func}}, as0}, lc, st) do
    as = expr_list(as0, st)
  ...
  defp expr({:call, anno, {:remote, _, mod0, func0}, as0}, lc, st) do
  ...
  defp expr({:call, anno, {:atom, _, func} = f, as0}, lc, %{ctype: ctypes} = st) do
    as = expr_list(as0, st)
    ar = length(as)
    nA = {func, ar}
    ...
    case :maps.get(nA, ctypes, :undefined) do
      ...
      :undefined ->
        case :erl_internal.bif(func, ar) and bif_type(:erlang, func, ar) do
          false ->
            {:local_call, ln(anno), func, as, lc}
  ...
  defp expr({:try, anno, es0, caseCs0, catchCs0, as0}, lc, st) do
    es = expr_list(es0, st)
  ...
  defp expr({:bin_element, anno, expr0, size0, type0}, _Lc, st) do
    {size1, type} = make_bit_type(anno, size0, type0)
  ...
  defp record_test_in_body(anno, expr, name, lc, st) do
    ...
    expr(
      {:block, anno,
       [
         {:match, anno, var, expr},
         {:call, anno, {:remote, anno, {:atom, anno, :erlang}, {:atom, anno, :is_record}},
  ...
  defp record_info_call(anno, [{:value, _AnnoI, info}, {:value, _AnnoN, name}], st) do
    case info do
      :size ->
  ...
  defp fun_clauses([{:clause, a, h, g, b} | cs], st) do
    [
      {:clause, ln(a), head(h, st), guard(g, st), exprs(b, true, st)}
      | fun_clauses(cs, st)
    ]
  end
  ...
  defp new_var_name() do
    ...
    :erlang.list_to_atom(~c"%" ++ :erlang.integer_to_list(c))
  end
