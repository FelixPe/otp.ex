defmodule :m_dbg_wx_settings do
  ...
  defp open_win(win, pos, sFile, str, what) do
    {sDir, sFileName} = (cond do
                           sFile == :default ->
                             {default_settings_dir(win), 'NoName.state'}
                           true ->
                             {:filename.dirname(sFile),
                                :filename.basename(sFile)}
                         end)
    fD = :wxFileDialog.new(win,
                             [{:message, str}, {:pos, pos}, {:defaultDir, sDir},
                                                                {:defaultFile,
                                                                   sFileName},
                                                                    {:wildCard,
                                                                       '*.state'},
  ...
  defp default_settings_dir(win) do
    ...
    case (:filelib.is_dir(defDir)) do
      true ->
        defDir
      false ->
        xDGDir = :filename.join(:filename.basedir(:user_config,
                                                    'erlang'),
                                  'debugger')
        case (:filelib.is_dir(xDGDir)) do
          true ->
            xDGDir
          false ->
            {:ok, cWD} = :file.get_cwd()
            msg = ['Default directory ', xDGDir, ' does not exist. ', 'Click OK to create it or ', 'Cancel to use other directory.']
            case (:dbg_wx_win.confirm(win, msg)) do
              :ok ->
                toolsDir = :filename.dirname(xDGDir)
                case (:filelib.is_dir(toolsDir)) do
                  true ->
                    case (:file.make_dir(xDGDir)) do
                      :ok ->
                        xDGDir
                      _Error ->
                        cWD
