defmodule :m_dbg_wx_winman do
  ...
  require Record
  Record.defrecord(:r_win, :win, owner: :undefined,
                               title: :undefined, win: :undefined)
  Record.defrecord(:r_state, :state, wins: [])
  def start() do
    :gen_server.start({:local, :dbg_wx_winman},
                        :dbg_wx_winman, [], [])
  end

  def insert(title, win) do
    :gen_server.cast(:dbg_wx_winman,
                       {:insert, self(), title, win})
  end

  def is_started(title) do
    case (:gen_server.call(:dbg_wx_winman,
                             {:is_started, title}, :infinity)) do
      {true, win} ->
        raise(win)
        true
      false ->
  ...
  def handle_call({:is_started, title}, _From, state) do
    reply = (case (:lists.keyfind(title, r_win(:title),
                                    r_state(state, :wins))) do
               false ->
                 false
               win ->
                 {true, r_win(win, :win)}
  ...
  def handle_cast({:insert, pid, title, win}, state) do
    :erlang.link(pid)
    wins = r_state(state, :wins) ++ [r_win(owner: pid, title: title,
                                   win: win)]
    inform_all(wins)
  ...
  def handle_cast({:clear_process, title}, state) do
    ...
    wins = (case (:lists.keyfind(title, r_win(:title),
                                   oldWins)) do
              r_win(owner: pid) ->
  ...
  def handle_info({:EXIT, pid, _Reason}, state) do
    ...
    cond do
      pid === r_win(mon, :owner) ->
        {:stop, :normal, state}
      true ->
        wins2 = :lists.keydelete(pid, r_win(:owner),
                                   r_state(state, :wins))
        inform_all(wins2)
  ...
  defp inform_all(wins) do
    infos = :lists.map(fn r_win(title: title, win: win) ->
                            {title, win}
                       end,
                         wins)
    msg = {:dbg_ui_winman, :update_windows_menu, infos}
    :lists.foreach(fn r_win(owner: pid) ->
                        send(pid, msg)
                   end,
                     wins)
  end
