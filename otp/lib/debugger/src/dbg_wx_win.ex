defmodule :m_dbg_wx_win do
  ...
  def add_break(win, menuName, point) do
    {mod, line} = point
    label = to_string(~c"~w ~5w", [mod, line])
    ...
    sub = :wxMenu.new([])
    dis = :wxMenu.append(sub, -1, ~c"Disable")
    add.(dis, {:break, point, :status})
    del = :wxMenu.append(sub, -1, ~c"Delete")
    add.(del, {:break, point, :delete})
    trigger = :wxMenu.new([])
    enable = :wxMenu.appendRadioItem(trigger, -1, ~c"Enable")
    add.(enable, {:break, point, {:trigger, :enable}})
    tDisable = :wxMenu.appendRadioItem(trigger, -1, ~c"Disable")
    add.(tDisable, {:break, point, {:trigger, :disable}})
    delete = :wxMenu.appendRadioItem(trigger, -1, ~c"Delete")
    add.(delete, {:break, point, {:trigger, :delete}})
    _ = :wxMenu.append(sub, -1, ~c"Trigger Action", trigger)
  ...
  def confirm(win, message) do
    mD =
      :wxMessageDialog.new(win, to_string(message), [{:style, 4 ||| 16}, {:caption, ~c"Confirm"}])
  ...
  def notify(win, message) do
    mD = :wxMessageDialog.new(win, to_string(message), [{:style, 4}, {:caption, ~c"Confirm"}])
  ...
  def open_help(_Parent, helpHtmlFile) do
    :wx_misc.launchDefaultBrowser(~c"file://" ++ helpHtmlFile)
  end
