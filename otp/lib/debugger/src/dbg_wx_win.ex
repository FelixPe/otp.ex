defmodule :m_dbg_wx_win do
  ...
  defp create_menu_item(menu, [{name, _N, :cascade, items} | is], win,
            id0, connect) do
    sub = :wxMenu.new([])
    id = create_menu_item(sub, items, win, id0, false)
    _ = :wxMenu.append(menu, - 1, menu_name(name, :ignore),
                         sub)
    self = self()
    butts = (for {mI, _, _} <- items do
               {mI, :erlang.get(mI)}
             end)
  ...
  defp create_menu_item(menu, [{name, pos} | is], win, id, connect) do
    menuId = (case (:lists.member(name, [:Debugger])) do
                true ->
                  5009
                _ ->
                  id
              end)
    item = :wxMenu.append(menu, menuId,
                            menu_name(name, pos))
    :erlang.put(name, item)
    cond do
      connect ->
        :wxMenu.connect(win, :command_menu_selected,
                          [{:id, menuId}, {:userData, name}])
      true ->
        :ignore
    end
    create_menu_item(menu, is, win, id + 1, connect)
  ...
  defp create_menu_item(menu, [{name, n, :check} | is], win, id,
            connect) do
    item = :wxMenu.appendCheckItem(menu, id,
                                     menu_name(name, n))
    :erlang.put(name, item)
    cond do
      connect ->
  ...
  defp create_menu_item(menu, [{name, n, :radio} | is], win, id,
            connect) do
    item = :wxMenu.appendRadioItem(menu, id,
                                     menu_name(name, n))
    :erlang.put(name, item)
    cond do
      connect ->
  ...
  def add_break(win, menuName, point) do
    ...
    r_break(mb: {menu, menuBtn}, smi: dis, emi: enable,
        dimi: tDisable, demi: delete)
  end
  ...
  def confirm(win, message) do
    mD = :wxMessageDialog.new(win, to_string(message),
                                [{:style, 4 ||| 16}, {:caption, 'Confirm'}])
    res = (case (:wxDialog.showModal(mD)) do
             5100 ->
               :ok
             _ ->
               :cancel
  ...
  def entry(parent, title, prompt, {type, value}) do
    ted = :wxTextEntryDialog.new(parent, to_string(prompt),
                                   [{:caption, to_string(title)}, {:value,
                                                                     value}])
    case (:wxDialog.showModal(ted)) do
      5100 ->
        res = (case (verify(type,
                              :wxTextEntryDialog.getValue(ted))) do
                 {:edit, newVal} ->
                   {prompt, newVal}
                 :ignore ->
                   :cancel
  ...
  defp verify(type, str) do
    case (:erl_scan.string(str, 1, [:text])) do
      {:ok, tokens, _EndLine} when type == :term ->
        case (:erl_eval.extended_parse_term(tokens ++ [{:dot,
                                                          :erl_anno.new(1)}])) do
          {:ok, value} ->
            {:edit, value}
          _Error ->
