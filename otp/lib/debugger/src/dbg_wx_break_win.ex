defmodule :m_dbg_wx_break_win do
  ...
  def create_win(parent, pos, :function, mod, _Line) do
    win =
      :wxDialog.new(parent, -1, ~c"Function Break", [
        {:pos, pos},
        {:style, 536_870_912 ||| 2048 ||| 4096 ||| 64}
      ])

    mainS = :wxBoxSizer.new(8)
    label = :wxStaticText.new(win, -1, ~c"Module:")
    int = :int.interpreted()
    ...
    funLabel = :wxStaticText.new(win, -1, ~c"Function:")
  ...
  def create_win(parent, pos, type, mod, line) do
    ...
    modSz = create_label_of_control(win, ~c"Module:", modT, mod)
  ...
  def update_functions(winInfo, funcs) do
    items =
      :lists.map(
        fn [n, a] ->
          :lists.flatten(:io_lib.format(~c"~tw/~w", [n, a]))
        end,
        funcs
      )
  ...
  def handle_event(
    ...
    :wxDialog.destroy(win)
    {:break, [[:erlang.list_to_atom(mod) | data]], triggered}
  end
  ...
  defp create_trigger_box(win) do
    sBox = :wxStaticBox.new(win, -1, ~c"Trigger Action:")
    sBS = :wxStaticBoxSizer.new(sBox, 8)
    ebtn = :wxRadioButton.new(win, -1, ~c"Enable")
    _ = :wxSizer.add(sBS, ebtn)
    dibtn = :wxRadioButton.new(win, -1, ~c"Disable")
    _ = :wxSizer.add(sBS, dibtn)
    debtn = :wxRadioButton.new(win, -1, ~c"Delete")
