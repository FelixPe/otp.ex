defmodule :m_dbg_wx_break_win do
  ...
  Record.defrecord(:r_wxHtmlLinkInfo, :wxHtmlLinkInfo, href: :undefined,
                                          target: :undefined)
  Record.defrecord(:r_winInfo, :winInfo, type: :undefined,
                                   win: :undefined, entries: :undefined,
                                   trigger: :undefined, listbox: :undefined,
                                   text: :undefined, ok: :undefined, funcs: [])
  def create_win(parent, pos, :function, mod, _Line) do
    win = :wxDialog.new(parent, - 1, 'Function Break',
                          [{:pos, pos}, {:style,
                                           536870912 ||| 2048 ||| 4096 ||| 64}])
    mainS = :wxBoxSizer.new(8)
    label = :wxStaticText.new(win, - 1, 'Module:')
    int = :int.interpreted()
    intStrs = (for m <- int do
                 :erlang.atom_to_list(m)
               end)
    text = :wxComboBox.new(win, - 1,
                             [{:value, :dbg_wx_win.to_string(mod)}, {:choices,
                                                                       intStrs}])
    expand = [{:border, 5}, {:flag, 16 ||| 32 ||| 8192}]
    _ = :wxSizer.add(mainS, label,
                       [{:border, 5}, {:flag, 64 ||| 16 ||| 32}])
    _ = :wxSizer.add(mainS, text, expand)
    funLabel = :wxStaticText.new(win, - 1, 'Function:')
    lB = :wxListBox.new(win, - 1,
                          [{:size, {- 1, 100}}, {:style, 64}])
    ...
    :wxDialog.show(win)
    r_winInfo(type: :function, win: win, text: text, ok: oKButt,
        entries: [], trigger: :enable, listbox: lB, funcs: [])
  end
  ...
  def create_win(parent, pos, type, mod, line) do
    ...
    win = :wxDialog.new(parent, - 1, title,
                          [{:pos, pos}, {:style, style}])
    mainS = :wxBoxSizer.new(8)
    int = :int.interpreted()
    intStrs = (for m <- int do
    ...
    _ = :wxSizer.add(mainS, triggerBox,
                       [{:border, 5}, {:flag,
                                         64 ||| 128 ||| 32 ||| 16 ||| 8192}])
    ...
    :wxDialog.centreOnParent(win)
    :wxDialog.show(win)
    r_winInfo(type: type, win: win, text: modT, entries: entries,
        trigger: trigger, ok: oKButt)
  end

  def update_functions(winInfo, funcs) do
    items = :lists.map(fn [n, a] ->
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_text_updated)),
           r_winInfo(type: :function, text: text, ok: oK)) do
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_text_updated)),
           r_winInfo(text: text, ok: oK, entries: es)) do
    module = :wxComboBox.getValue(text)
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_listbox_selected)),
           r_winInfo(type: :function, listbox: lB, ok: oK)) do
  ...
  def handle_event(r_wx(id: oKorListBox,
             event: r_wxCommand(type: oKorDoubleClick)),
           r_winInfo(type: :function, win: win, listbox: lB, funcs: funcs,
               text: text))
    ...
    breaks = (for index <- indexL do
                [:erlang.list_to_atom(mod) | :lists.nth(index + 1,
                                                          funcs)]
              end)
    :wxDialog.destroy(win)
    {:break, breaks, :enable}
  ...
  def handle_event(r_wx(id: 5100),
           r_winInfo(win: win, text: text, entries: es,
               trigger: trigger)) do
    mod = :wxComboBox.getValue(text)
    data = check_input(es)
    triggered = get_trigger(trigger)
    :wxDialog.destroy(win)
    {:break, [[:erlang.list_to_atom(mod) | data]],
       triggered}
  end
  ...
  defp create_label_of_control(parent, label, control, def__) do
    ...
    border = {:border, 5}
    flag = 32 ||| 16 ||| 2048
    _ = :wxSizer.add(sizer, text,
                       [{:proportion, 1}, {:flag, flag}, border])
    _ = :wxSizer.add(sizer, control,
                       [{:proportion, 3}, {:flag, flag ||| 8192}, border])
    :wxControl.setLabel(control,
  ...
  defp create_trigger_box(win) do
    ...
    {sBS,
       [{ebtn, :enable}, {dibtn, :disable}, {debtn, :delete}]}
  end
