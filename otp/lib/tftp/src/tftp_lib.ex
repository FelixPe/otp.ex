defmodule :m_tftp_lib do
  ...
  defp do_parse_config([{key, val} | tail], config)
    ...
    case (key) do
      ...
      :port ->
        cond do
          (is_integer(val) and val >= 0) ->
            config2 = r_config(config, udp_port: val, 
      ...
      :port_policy ->
        case (val) do
          ...
          minMax when (is_integer(minMax) and minMax > 0) ->
            do_parse_config(tail,
                              r_config(config, port_policy: {:range, minMax, minMax}))
          {:range, min, max} when (max >= min and
      ...
      :udp when is_list(val) ->
        ...
        udpOptions = :lists.foldl(fun, r_config(config, :udp_options),
                                    val)
        do_parse_config(tail,
      ...
      _ when (is_list(key) and is_list(val)) ->
        key2 = to_lower(key)
        val2 = to_lower(val)
        tftpOptions = replace_val(key2, val2,
                                    r_config(config, :user_options))
        do_parse_config(tail,
      ...
      :callback ->
        case (val) do
          {regExp, mod, state} when (is_list(regExp) and
                                       is_atom(mod))
                                    ->
            case (:re.compile(regExp)) do
              {:ok, internal} ->
                callback = r_callback(regexp: regExp, internal: internal,
  ...
  defp do_parse_config([],
            r_config(udp_host: host, udp_options: udpOptions,
                user_options: userOptions,
                callbacks: callbacks) = config) do
    ...
    r_config(config, udp_host: host2,  udp_options: udpOptions2, 
                user_options: tftpOptions,  callbacks: callbacks2)
  end
  ...
  def add_default_callbacks(callbacks) do
    ...
    file = r_callback(regexp: regExp, internal: internal,
               module: :tftp_file, state: [])
    bin = r_callback(regexp: regExp, internal: internal,
              module: :tftp_binary, state: [])
    callbacks ++ [file, bin]
  end
  ...
  def host_to_string(host) do
    case (host) do
      ...
      {a1, a2, a3, a4, a5, a6, a7, a8} ->
        :lists.concat([int16_to_hex(a1), '::', int16_to_hex(a2), '::',
                                                                  int16_to_hex(a3),
                                                                      '::',
                                                                          int16_to_hex(a4),
                                                                              '::',
  ...
  def decode_msg(bin) when is_binary(bin) do
    case (bin) do
      <<1 :: size(16) - integer, tail :: binary>> ->
        case (decode_strings(tail,
                               [:keep_case, :lower_case])) do
          [filename, mode | strings] ->
            options = decode_options(strings)
            r_tftp_msg_req(access: :read, filename: filename,
                mode: to_lower(mode), options: options)
          [_Filename | _Strings] ->
      ...
      <<2 :: size(16) - integer, tail :: binary>> ->
        case (decode_strings(tail,
          ...
          [filename, mode | strings] ->
            options = decode_options(strings)
            r_tftp_msg_req(access: :write, filename: filename,
                mode: to_lower(mode), options: options)
          [_Filename | _Strings] ->
            exit(r_tftp_msg_error(code: :undef, text: 'Missing mode'))
          _ ->
      ...
      <<3 :: size(16) - integer, seqNo :: size(16) - integer,
          data :: binary>> ->
        r_tftp_msg_data(block_no: seqNo, data: data)
      <<4 :: size(16) - integer,
          seqNo :: size(16) - integer>> ->
        r_tftp_msg_ack(block_no: seqNo)
      <<5 :: size(16) - integer,
          errorCode :: size(16) - integer, tail :: binary>> ->
  ...
  defp decode_strings(bin, cases) when (is_binary(bin) and
                              is_list(cases)) do
  ...
  defp decode_string(<<char :: size(8) - integer, tail :: binary>>,
            case__, string) do
    cond do
      char === 0 ->
  ...
  defp decode_error_code(int) do
    case (int) do
      ...
      8 ->
        :badopt
      ^int when (is_integer(int) and int >= 0 and
                   int <= 65535)
                ->
        int
      _ ->
  ...
  def encode_msg(r_tftp_msg_req(access: access, filename: filename, mode: mode,
             options: options)) do
    opCode = (case (access) do
                :read ->
                  1
                :write ->
                  2
              end)
    [<<opCode :: size(16) - integer>>, filename, 0, mode, 0,
                                                              for {key,
                                                                     val} <- options do
                                                                [key, 0, val, 0]
                                                              end]
