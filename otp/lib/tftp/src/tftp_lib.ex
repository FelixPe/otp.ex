defmodule :m_tftp_lib do
  ...
  def host_to_string(host) do
    case host do
      ...
      {a1, a2, a3, a4} ->
        :lists.concat([a1, ~c".", a2, ~c".", a3, ~c".", a4])
    ...
    case bin do
      <<1::size(16)-integer, tail::binary>> ->
        case decode_strings(
          ...
          [_Filename | _Strings] ->
            exit(r_tftp_msg_error(code: :undef, text: ~c"Missing mode"))

          _ ->
            exit(r_tftp_msg_error(code: :undef, text: ~c"Missing filename"))
        end
      ...
      <<5::size(16)-integer, errorCode::size(16)-integer, tail::binary>> ->
        case decode_strings(tail, [:keep_case]) do
          ...
          _ ->
            exit(r_tftp_msg_error(code: :undef, text: ~c"Trailing garbage"))
        end
      ...
      _ ->
        exit(r_tftp_msg_error(code: :undef, text: ~c"Invalid syntax"))
    end
  end
  ...
  defp decode_string(<<>>, _Case, _String) do
    exit(r_tftp_msg_error(code: :undef, text: ~c"Trailing null missing"))
  end
  ...
  defp decode_error_code(int) do
    case int do
      ...
      _ ->
        exit(r_tftp_msg_error(code: :undef, text: ~c"Error code outside range."))
    end
  end
