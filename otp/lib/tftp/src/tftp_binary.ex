defmodule :m_tftp_binary do
  ...
  def prepare(_Peer, access, filename, mode, suggestedOptions, initial)
    ...
    case (try do
      ...
      {:ok, _, _} ->
        {:error, {:undef, ~c"Illegal callback usage. Mode and filename is incompatible."}}
  ...
  def prepare(_Peer, _Access, _Bin, _Mode, _SuggestedOptions, _Initial) do
    {:error, {:undef, ~c"Illegal callback options."}}
  end
  ...
  defp handle_mode(mode, isNativeAscii) do
    case mode do
      ~c"netascii" when isNativeAscii === true ->
        true
      ...
      _ ->
        throw({:error, {:badop, ~c"Illegal mode " ++ mode}})
    end
  end
  ...
  defp do_handle_options(access, bin, [{key, val} | t]) do
    case key do
      ~c"tsize" ->
        case access do
          :read when val === ~c"0" and is_binary(bin) ->
  ...
  defp handle_integer(access, bin, key, val, options, min, max) do
    case (try do
      ...
      _Int ->
        throw({:error, {:badopt, ~c"Illegal " ++ key ++ ~c" value " ++ val}})
    end
  end

  defp lookup_blksize(options) do
    case :lists.keysearch(~c"blksize", 1, options) do
      {:value, {_, val}} ->
        :erlang.list_to_integer(val)
