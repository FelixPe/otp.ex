defmodule :m_tftp_binary do
  ...
  @behaviour :tftp
  require Record
  Record.defrecord(:r_read_state, :read_state, options: :undefined,
                                      blksize: :undefined, bin: :undefined,
                                      is_native_ascii: :undefined,
                                      is_network_ascii: :undefined,
                                      count: :undefined)
  Record.defrecord(:r_write_state, :write_state, options: :undefined,
                                       blksize: :undefined, list: :undefined,
                                       is_native_ascii: :undefined,
                                       is_network_ascii: :undefined)
  def prepare(_Peer, access, filename, mode, suggestedOptions,
    ...
    case ((try do
      ...
      {:ok, isNetworkAscii, acceptedOptions}
        ...
        state = r_read_state(options: acceptedOptions,
                    blksize: lookup_blksize(acceptedOptions), bin: filename,
      ...
      {:ok, isNetworkAscii, acceptedOptions}
        ...
        state = r_write_state(options: acceptedOptions,
                    blksize: lookup_blksize(acceptedOptions), list: [],
                    is_network_ascii: isNetworkAscii,
                    is_native_ascii: isNativeAscii)
        {:ok, acceptedOptions, state}
      {:ok, _, _} ->
  ...
  def prepare(_Peer, _Access, _Bin, _Mode, _SuggestedOptions,
           _Initial) do
  ...
  def open(_Peer, access, filename, mode, negotiatedOptions,
    ...
    case ((try do
            handle_options(access, filename, mode,
                             negotiatedOptions, r_read_state(state, :is_native_ascii))
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
            e -> e
  ...
  def open(_Peer, access, filename, mode, negotiatedOptions,
    ...
    case ((try do
            handle_options(access, filename, mode,
                             negotiatedOptions, r_write_state(state, :is_native_ascii))
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
            e -> e
  ...
  def write(bin, r_write_state(list: list) = state)
    ...
    cond do
      ...
      size < blkSize ->
        bin2 = :erlang.list_to_binary(:lists.reverse([bin |
                                                          list]))
        {:last, bin2}
    end
  end
      ...
      'tsize' ->
        case (access) do
          :read when (val === '0' and is_binary(bin)) ->
            tsize = :erlang.integer_to_list(byte_size(bin))
  ...
  defp upgrade_state({:read_state, options, blksize, bin,
             isNetworkAscii, count}) do
    {:read_state, options, blksize, bin, false,
       isNetworkAscii, count}
  end

  defp upgrade_state({:write_state, options, blksize, list,
             isNetworkAscii}) do
    {:write_state, options, blksize, list, false,
       isNetworkAscii}
  end
