defmodule :m_tftp_engine do
  use Bitwise
  require Record
  Record.defrecord(:r_tftp_msg_req, :tftp_msg_req, access: :undefined,
                                        filename: :undefined, mode: :undefined,
                                        options: :undefined,
                                        local_filename: :undefined)
  Record.defrecord(:r_tftp_msg_data, :tftp_msg_data, block_no: :undefined,
                                         data: :undefined)
  Record.defrecord(:r_tftp_msg_ack, :tftp_msg_ack, block_no: :undefined)
  Record.defrecord(:r_tftp_msg_error, :tftp_msg_error, code: :undefined,
                                          text: :undefined, details: :undefined)
  Record.defrecord(:r_tftp_msg_oack, :tftp_msg_oack, options: :undefined)
  Record.defrecord(:r_config, :config, parent_pid: self(),
                                  udp_socket: :undefined,
                                  udp_options: [:binary, {:reuseaddr, true},
                                                             {:active, :once}],
                                  udp_host: 'localhost', udp_port: 69,
                                  port_policy: :random, use_tsize: false,
                                  max_tsize: :infinity, max_conn: :infinity,
                                  rejected: [], polite_ack: false,
                                  debug_level: :none, timeout: :undefined,
                                  user_options: [], callbacks: [],
                                  logger: :tftp_logger, max_retries: 5)
  Record.defrecord(:r_callback, :callback, regexp: :undefined,
                                    internal: :undefined, module: :undefined,
                                    state: :undefined, block_no: :undefined,
                                    count: :undefined)
  Record.defrecord(:r_daemon_state, :daemon_state, config: :undefined,
                                        n_servers: :undefined,
                                        server_tab: :undefined,
                                        file_tab: :undefined)
  Record.defrecord(:r_server_info, :server_info, pid: :undefined,
                                       req: :undefined, peer: :undefined)
  Record.defrecord(:r_file_info, :file_info, peer_req: :undefined,
                                     pid: :undefined)
  Record.defrecord(:r_sys_misc, :sys_misc, module: :undefined,
                                    function: :undefined, arguments: :undefined)
  Record.defrecord(:r_error, :error, where: :undefined,
                                 code: :undefined, text: :undefined,
                                 filename: :undefined)
  Record.defrecord(:r_prepared, :prepared, status: :undefined,
                                    result: :undefined, block_no: :undefined,
                                    next_data: :undefined,
                                    prev_data: :undefined)
  Record.defrecord(:r_transfer_res, :transfer_res, status: :undefined,
  ...
  def change_config(toPid, options) when is_pid(toPid) do
    ...
    badOptions = (for {key, val} <- options,
                        badKey <- badKeys, key === badKey do
                    {key, val}
                  end)
    case (badOptions) do
      [] ->
        call({:change_config, options}, toPid,
               :timer.seconds(10))
      [{key, val} | _] ->
  ...
  def daemon_start(options) when is_list(options) do
    ...
    :proc_lib.start_link(:tftp_engine, :daemon_init,
                           [config], :infinity)
  end

  def daemon_init(config) when (elem(config, 0) === :config and
    ...
    case ((try do
      ...
      {:ok, socket} ->
        ...
        config2 = r_config(config, udp_socket: socket, 
                              udp_port: actualPort)
        print_debug_info(config2, :daemon, :open,
                           r_tftp_msg_req(filename: ''))
        serverTab = :ets.new(:tftp_daemon_servers,
                               [{:keypos, 2}])
        fileTab = :ets.new(:tftp_daemon_files, [{:keypos, 2}])
        state = r_daemon_state(config: config2, n_servers: 0,
                    server_tab: serverTab, file_tab: fileTab)
        daemon_loop(state)
      {:error, reason} ->
        text = :lists.flatten(:io_lib.format('UDP open ~p -> ~p',
                                               [udpOptions, reason]))
        print_debug_info(config, :daemon, :open,
                           r_error(where: :open, code: :undef, text: text,
                               filename: ''))
  ...
  defp prepare_daemon_udp(r_config(udp_port: port,
              udp_options: udpOptions) = config) do
        ...
        case (:init.get_argument(initArg)) do
          {:ok, [[fdStr]] = badarg} when is_list(fdStr) ->
            case ((try do
              ...
              {:EXIT, _} ->
                text = :lists.flatten(:io_lib.format('Illegal prebound fd ~p: ~p',
                                                       [initArg, badarg]))
                print_debug_info(config, :daemon, :open,
                                   r_error(where: :open, code: :undef, text: text,
                                       filename: ''))
  ...
  def daemon_loop(daemonConfig, n, servers)
    ...
    serverTab = :ets.new(:tftp_daemon_servers,
                           [{:keypos, 2}])
    fileTab = :ets.new(:tftp_daemon_files, [{:keypos, 2}])
    state = r_daemon_state(config: daemonConfig, n_servers: n,
                server_tab: serverTab, file_tab: fileTab)
    req = r_tftp_msg_req(filename: :dummy)
    for pid <- servers do
      :ets.insert(serverTab,
                    r_server_info(pid: pid, req: req, peer: :dummy))
    end
    daemon_loop(state)
  end

  def daemon_loop(r_daemon_state(config: daemonConfig, n_servers: n,
             server_tab: serverTab, file_tab: fileTab) = state)
    ...
    receive do
      {:info, ref, fromPid} when is_pid(fromPid) ->
        ...
        info = internal_info(daemonConfig,
                               :daemon) ++ [{:n_conn, n}] ++ serverInfo
      ...
      {:udp, socket, remoteHost, remotePort, bin}
        ...
        serverConfig = r_config(daemonConfig, parent_pid: self(), 
                                         udp_host: remoteHost, 
                                         udp_port: remotePort)
        msg = ((try do
        ...
        case (msg) do
          req when (elem(req, 0) === :tftp_msg_req and
            ...
            case (:ets.lookup(fileTab, peerReq)) do
              [] ->
                ...
                pid = :proc_lib.spawn_link(:tftp_engine, :server_init,
                                             args)
                :ets.insert(serverTab,
              ...
              [r_file_info(pid: pid)] ->
                warning_msg(daemonConfig, '~p Reuse connection for ~s\n\t~p\n',
                              [pid, peerInfo, r_tftp_msg_req(req, :filename)])
                :tftp_engine.daemon_loop(state)
            end
          req when elem(req, 0) === :tftp_msg_req ->
            ...
            peer = peer_info(serverConfig)
            peerInfo = :lists.flatten(:io_lib.format('~p', [peer]))
            warning_msg(daemonConfig, 'Daemon has too many connections (~p).\n\tRejecting request from ~s\n', [n, peerInfo])
      ...
      {:system, from, msg} ->
        misc = r_sys_misc(module: :tftp_engine, function: :daemon_loop,
                   arguments: [state])
        :sys.handle_system_msg(msg, from,
                                 r_config(daemonConfig, :parent_pid), :tftp_engine, [],
                                 misc)
      {:EXIT, pid, reason}
  ...
  defp upgrade_config({:config, parentPid, udpSocket, udpOptions,
             udpHost, udpPort, portPolicy, useTsize, maxTsize,
             maxConn, rejected, politeAck, debugLevel, timeout,
             userOptions, callbacks}) do
    callbacks2 = :tftp_lib.add_default_callbacks(callbacks)
    logger = :tftp_logger
    maxRetries = 5
    {:config, parentPid, udpSocket, udpOptions, udpHost,
       udpPort, portPolicy, useTsize, maxTsize, maxConn,
       rejected, politeAck, debugLevel, timeout, userOptions,
       callbacks2, logger, maxRetries}
  end
  ...
  def server_init(config, req)
    ...
    config2 = :tftp_lib.parse_config(suggestedOptions,
                                       config1)
                    ...
                    case (post_verify_options(config3, req2,
                      ...
                      {:ok, config4, req3} when acceptedOptions !== [] ->
                        ...
                        {config5, callback3, transferRes} = transfer(config4,
                                                                       ...
                                                                       blockNo,
                                                                       r_prepared())
                        common_loop(config5, callback3, req3, transferRes,
                                      localAccess, blockNo)
                      {:ok, config4, req3} when localAccess === :write ->
                        blockNo = 0
                        common_ack(config4, callback2, req3, localAccess,
                                     blockNo, r_prepared())
                      {:ok, config4, req3} when localAccess === :read ->
                        blockNo = 0
                        common_read(config4, callback2, req3, localAccess,
                                      blockNo, blockNo, r_prepared())
                      {:error, {code, text}} ->
                        {:undefined, error} = callback({:abort, {code, text}},
                                                         config3, callback2,
                                                         req2)
                        send_msg(config3, req, error)
                        terminate(config3, req2,
                                    r_error(where: :post_verify_options, code: code,
                                        text: text,
                                        filename: r_tftp_msg_req(req2, :filename)))
                    end
                  {:undefined, r_tftp_msg_error(code: code, text: text) = error} ->
                    send_msg(config3, req, error)
                    terminate(config3, req,
                                r_error(where: :server_open, code: code, text: text,
                                    filename: r_tftp_msg_req(req2, :filename)))
                end
              {:error, {code, text}} ->
                {:undefined, error} = callback({:abort, {code, text}},
                                                 config2, callback, req2)
                send_msg(config2, req, error)
                terminate(config2, req2,
                            r_error(where: :pre_verify_options, code: code,
                                text: text, filename: r_tftp_msg_req(req2, :filename)))
            end
          {:error, r_tftp_msg_error(code: code, text: text) = error} ->
            send_msg(config3, req, error)
            terminate(config3, req,
                        r_error(where: :match_callback, code: code, text: text,
                            filename: r_tftp_msg_req(req2, :filename)))
        end
      r_error() = error ->
  ...
  def client_start(access, remoteFilename, localFilename,
           options) do
    config = :tftp_lib.parse_config(options)
    config2 = r_config(config, parent_pid: self(), 
                          udp_socket: :undefined)
    req = r_tftp_msg_req(access: access, filename: remoteFilename,
              mode: lookup_mode(r_config(config2, :user_options)),
              options: r_config(config2, :user_options),
              local_filename: localFilename)
    args = [config2, req]
    case (:proc_lib.start_link(:tftp_engine, :client_init,
                                 args, :infinity)) do
  ...
  def client_init(config, req)
    ...
    case (open_free_port(config, :client, req)) do
      {:ok, config2} ->
        req2 = (case (r_config(config2, :use_tsize)) do
                  true ->
                    suggestedOptions = r_tftp_msg_req(req, :options)
                    suggestedOptions2 = :tftp_lib.replace_val('tsize', '0',
                                                                suggestedOptions)
        ...
        case (match_callback(localFilename,
                               r_config(config2, :callbacks))) do
          {:ok, callback} ->
          ...
          {:error, r_tftp_msg_error(code: code, text: text)} ->
            terminate(config, req,
                        r_error(where: :match, code: code, text: text,
                            filename: r_tftp_msg_req(req, :filename)))
        end
      r_error() = error ->
  ...
  defp client_prepare(config, callback, req)
    ...
    case (pre_verify_options(config, req)) do
      :ok ->
        case (callback({:open, :client_prepare}, config,
                         callback, req)) do
          {callback2, {:ok, acceptedOptions}} ->
            optText = 'Internal error. Not allowed to add new options.'
            case (post_verify_options(config, req, acceptedOptions,
                                        optText)) do
              {:ok, config2, req2} ->
                {localAccess, _} = local_file_access(req2)
                blockNo = 0
                {config3, callback3, transferRes} = transfer(config2,
                                                               callback2, req2,
                                                               req2,
                                                               localAccess,
                                                               blockNo, r_prepared())
                client_open(config3, callback3, req2, blockNo,
                              transferRes)
              {:error, {code, text}} ->
                _ = callback({:abort, {code, text}}, config, callback2,
                               req)
                terminate(config, req,
                            r_error(where: :post_verify_options, code: code,
                                text: text, filename: r_tftp_msg_req(req, :filename)))
            end
          {:undefined, r_tftp_msg_error(code: code, text: text)} ->
            terminate(config, req,
                        r_error(where: :client_prepare, code: code, text: text,
                            filename: r_tftp_msg_req(req, :filename)))
        end
      {:error, {code, text}} ->
        _ = callback({:abort, {code, text}}, config, callback,
                       req)
        terminate(config, req,
                    r_error(where: :pre_verify_options, code: code, text: text,
                        filename: r_tftp_msg_req(req, :filename)))
    end
  end

  defp client_open(config, callback, req, blockNo,
            r_transfer_res(status: status, decoded_msg: decodedMsg,
                prepared: prepared)) do
      ...
      :ok when elem(prepared, 0) === :prepared ->
        case (decodedMsg) do
          msg when elem(msg, 0) === :tftp_msg_oack ->
            ...
            case (post_verify_options(config, req, serverOptions,
                                        optText)) do
              {:ok, config2, req2} ->
                {config3, callback2, req3} = do_client_open(config2,
                                                              callback, req2)
                case (localAccess) do
                  :read ->
                    common_read(config3, callback2, req3, localAccess,
                                  blockNo, blockNo, prepared)
                  :write ->
                    common_ack(config3, callback2, req3, localAccess,
                                 blockNo, prepared)
                end
              {:error, {code, text}} ->
                {:undefined, error} = callback({:abort, {code, text}},
                ...
                terminate(config, req,
                            r_error(where: :verify_server_options, code: code,
                                text: text, filename: r_tftp_msg_req(req, :filename)))
            end
          r_tftp_msg_ack(block_no: actualBlockNo) when localAccess === :read ->
            req2 = r_tftp_msg_req(req, options: [])
            {config2, callback2, ^req2} = do_client_open(config,
                                                           callback, req2)
            expectedBlockNo = 0
            common_read(config2, callback2, req2, localAccess,
                          expectedBlockNo, actualBlockNo, prepared)
          r_tftp_msg_data(block_no: actualBlockNo, data: data)
            ...
            {config2, callback2, ^req2} = do_client_open(config,
                                                           callback, req2)
            expectedBlockNo = 1
            common_write(config2, callback2, req2, localAccess,
                           expectedBlockNo, actualBlockNo, data, prepared)
          r_tftp_msg_error(code: code, text: text) ->
            _ = callback({:abort, {code, text}}, config, callback,
                           req)
            terminate(config, req,
                        r_error(where: :client_open, code: code, text: text,
                            filename: r_tftp_msg_req(req, :filename)))
          ...
          msg when is_tuple(msg) ->
            ...
            {:undefined, error} = callback({:abort, {code, text}},
                                             config, callback, req)
            send_msg(config, req, error)
            text2 = :lists.flatten([text, '. ', :io_lib.format('~p',
                                                              [:erlang.element(1,
                                                                                 msg)])])
            terminate(config, req,
                        r_error(where: :client_open, code: code, text: text2,
                            filename: r_tftp_msg_req(req, :filename)))
        end
  ...
  defp do_client_open(config, callback, req) do
    case (callback({:open, :client_open}, config, callback,
                     req)) do
      {callback2, {:ok, finalOptions}} ->
        optText = 'Internal error. Not allowed to change options.'
        case (post_verify_options(config, req, finalOptions,
                                    optText)) do
          {:ok, config2, req2} ->
            {config2, callback2, req2}
          {:error, {code, text}} ->
            {:undefined, error} = callback({:abort, {code, text}},
                                             config, callback2, req)
            send_msg(config, req, error)
            terminate(config, req,
                        r_error(where: :post_verify_options, code: code, text: text,
                            filename: r_tftp_msg_req(req, :filename)))
        end
      {:undefined, r_tftp_msg_error(code: code, text: text) = error} ->
        send_msg(config, req, error)
  ...
  def common_loop(config, callback, req,
           r_transfer_res(status: status, decoded_msg: decodedMsg,
               prepared: prepared),
           localAccess, expectedBlockNo)
      when elem(config, 0) === :config do
    case (status) do
      :ok when elem(prepared, 0) === :prepared ->
        case (decodedMsg) do
          r_tftp_msg_ack(block_no: actualBlockNo) when localAccess === :read ->
            common_read(config, callback, req, localAccess,
                          expectedBlockNo, actualBlockNo, prepared)
          r_tftp_msg_data(block_no: actualBlockNo, data: data)
              when localAccess === :write ->
            common_write(config, callback, req, localAccess,
                           expectedBlockNo, actualBlockNo, data, prepared)
          r_tftp_msg_error(code: code, text: text) ->
            _ = callback({:abort, {code, text}}, config, callback,
                           req)
            terminate(config, req,
                        r_error(where: :common_loop, code: code, text: text,
                            filename: r_tftp_msg_req(req, :filename)))
          ...
          msg when is_tuple(msg) ->
            ...
            text2 = :lists.flatten([text, '. ', :io_lib.format('~p',
                                                              [:erlang.element(1,
                                                                                 msg)])])
            terminate(config, req,
                        r_error(where: :common_loop, code: code, text: text2,
                            filename: r_tftp_msg_req(req, :filename)))
        end
      :error when elem(prepared, 0) === :tftp_msg_error ->
        r_tftp_msg_error(code: code, text: text) = prepared
        ...
        terminate(config, req,
                    r_error(where: :transfer, code: code, text: text,
                        filename: r_tftp_msg_req(req, :filename)))
    end
  end

  def common_loop(config, callback, req, transferRes, localAccess,
           expectedBlockNo) do
    config2 = upgrade_config(config)
    common_loop(config2, callback, req, transferRes,
                  localAccess, expectedBlockNo)
  end

  defp common_read(config, _, req, _, _, _,
            r_prepared(status: :terminate, result: result)) do
    terminate(config, req, {:ok, result})
  end

  defp common_read(config, callback, req, localAccess,
            expectedBlockNo, actualBlockNo, prepared)
      when (actualBlockNo === expectedBlockNo and
              elem(prepared, 0) === :prepared) do
    case (early_read(config, callback, req, localAccess,
                       actualBlockNo, prepared)) do
      {callback2,
         r_prepared(status: :more, next_data: data) = prepared2}
          when is_binary(data) ->
        prepared3 = r_prepared(prepared2, prev_data: data, 
                                   next_data: :undefined)
        do_common_read(config, callback2, req, localAccess,
                         actualBlockNo, data, prepared3)
      {:undefined,
         r_prepared(status: :last, next_data: data) = prepared2}
          when is_binary(data) ->
        prepared3 = r_prepared(prepared2, status: :terminate)
        do_common_read(config, :undefined, req, localAccess,
                         actualBlockNo, data, prepared3)
      {:undefined, r_prepared(status: :error, result: error)} ->
        r_tftp_msg_error(code: code, text: text) = error
        send_msg(config, req, error)
        terminate(config, req,
                    r_error(where: :read, code: code, text: text,
                        filename: r_tftp_msg_req(req, :filename)))
    end
  end
  ...
  defp common_read(config, callback, req, localAccess,
    ...
    case (prepared) do
      r_prepared(status: :more, prev_data: data) when is_binary(data)
                                             ->
        do_common_read(config, callback, req, localAccess,
                         actualBlockNo, data, prepared)
  ...
  defp common_read(config, callback, req, localAccess,
    ...
    case (prepared) do
      r_prepared(status: :more, prev_data: data) when is_binary(data)
        ...
        {config2, callback2,
           transferRes} = wait_for_msg_and_handle_timeout(config,
                                                            callback, req,
                                                            reply, localAccess,
                                                            expectedBlockNo,
                                                            prepared)
        :tftp_engine.common_loop(config2, callback2, req,
                                   transferRes, localAccess, expectedBlockNo)
      r_prepared(status: :last, prev_data: data) when is_binary(data)
                                             ->
        reply = r_tftp_msg_data(block_no: expectedBlockNo, data: data)
        {config2, callback2,
  ...
  defp common_read(config, callback, req, _LocalAccess,
            expectedBlockNo, actualBlockNo, prepared)
      when elem(prepared, 0) === :prepared do
    code = :badblk
    text = 'Unknown transfer ID = ' ++ :erlang.integer_to_list(actualBlockNo) ++ ' (' ++ :erlang.integer_to_list(expectedBlockNo) ++ ')'
    {:undefined, error} = callback({:abort, {code, text}},
                                     config, callback, req)
    send_msg(config, req, error)
    terminate(config, req,
  ...
  defp do_common_read(config, callback, req, localAccess, blockNo,
            data, prepared)
      when (is_binary(data) and
              elem(prepared, 0) === :prepared) do
    nextBlockNo = rem(blockNo + 1, 65536)
    reply = r_tftp_msg_data(block_no: nextBlockNo, data: data)
    {config2, callback2, transferRes} = transfer(config,
                                                   callback, req, reply,
                                                   localAccess, nextBlockNo,
                                                   prepared)
    :tftp_engine.common_loop(config2, callback2, req,
                               transferRes, localAccess, nextBlockNo)
  end

  defp common_write(config, _, req, _, _, _, _,
            r_prepared(status: :terminate, result: result)) do
    terminate(config, req, {:ok, result})
  end

  defp common_write(config, callback, req, localAccess,
            expectedBlockNo, actualBlockNo, data, prepared)
      when (actualBlockNo === expectedBlockNo and
              is_binary(data) and elem(prepared, 0) === :prepared) do
    case (callback({:write, data}, config, callback,
                     req)) do
      {callback2, r_prepared(status: :more) = prepared2} ->
        common_ack(config, callback2, req, localAccess,
                     actualBlockNo, prepared2)
      {:undefined,
         r_prepared(status: :last, result: result) = prepared2} ->
        config2 = pre_terminate(config, req, {:ok, result})
        prepared3 = r_prepared(prepared2, status: :terminate)
        common_ack(config2, :undefined, req, localAccess,
                     actualBlockNo, prepared3)
      {:undefined, r_prepared(status: :error, result: error)} ->
        r_tftp_msg_error(code: code, text: text) = error
        send_msg(config, req, error)
        terminate(config, req,
                    r_error(where: :write, code: code, text: text,
                        filename: r_tftp_msg_req(req, :filename)))
    end
  end
  ...
  defp common_write(config, callback, req, localAccess,
      ...
      when (actualBlockNo === expectedBlockNo - 1 and
              is_binary(data) and elem(prepared, 0) === :prepared) do
    common_ack(config, callback, req, localAccess,
                 expectedBlockNo - 1, prepared)
  end
  ...
  defp common_write(config, callback, req, _, expectedBlockNo,
            actualBlockNo, data, prepared)
      when (is_binary(data) and
              elem(prepared, 0) === :prepared) do
    code = :badblk
  ...
  defp common_ack(config, callback, req, localAccess, blockNo,
            prepared)
      when elem(prepared, 0) === :prepared do
  ...
  defp pre_terminate(config, req, result) do
    cond do
      (r_tftp_msg_req(req, :local_filename) !== :undefined and
        ...
        _ = ((try do
               :proc_lib.init_fail(result, {:throw, :ok})
             catch
               :error, e -> {:EXIT, {e, __STACKTRACE__}}
               :exit, e -> {:EXIT, e}
               e -> e
  ...
  defp terminate(config, req, result) do
    result2 = (case (result) do
                 ...
                 r_error(where: where, code: code, text: text) = error ->
                   print_debug_info(config, req, where,
    ...
    cond do
      ...
      r_tftp_msg_req(req, :local_filename) !== :undefined ->
        close_port(config, :client, req)
        _ = ((try do
               :proc_lib.init_fail(result2, {:throw, :ok})
             catch
               :error, e -> {:EXIT, {e, __STACKTRACE__}}
               :exit, e -> {:EXIT, e}
               e -> e
  ...
  defp open_free_port(config, who, req)
    ...
    case (r_config(config, :port_policy)) do
      ...
      {:range, port, _Max} ->
        ...
        r_error(where: who, code: :undef, text: text,
            filename: r_tftp_msg_req(req, :filename))
    end
  end

  defp transfer(config, callback, req, msg, localAccess,
            nextBlockNo, prepared)
      when elem(prepared, 0) === :prepared do
    ioList = :tftp_lib.encode_msg(msg)
    retries = r_config(config, :max_retries) + 1
    do_transfer(config, callback, req, msg, ioList,
                  localAccess, nextBlockNo, prepared, retries)
  end

  defp do_transfer(config, callback, req, msg, ioList, localAccess,
            nextBlockNo, prepared, retries)
      when (elem(prepared, 0) === :prepared and
              is_integer(retries) and retries >= 0) do
    case (do_send_msg(config, req, msg, ioList)) do
      :ok ->
        {callback2, prepared2} = early_read(config, callback,
                                              req, localAccess, nextBlockNo,
                                              prepared)
        do_wait_for_msg_and_handle_timeout(config, callback2,
                                             req, msg, ioList, localAccess,
                                             nextBlockNo, prepared2, retries)
      {:error, _Reason} when retries > 0 ->
        retries2 = 0
        do_transfer(config, callback, req, msg, ioList,
                      localAccess, nextBlockNo, prepared, retries2)
      {:error, reason} ->
        code = :undef
  ...
  defp wait_for_msg_and_handle_timeout(config, callback, req, msg, localAccess,
            nextBlockNo, prepared) do
    ioList = :tftp_lib.encode_msg(msg)
    retries = r_config(config, :max_retries) + 1
    do_wait_for_msg_and_handle_timeout(config, callback,
                                         req, msg, ioList, localAccess,
                                         nextBlockNo, prepared, retries)
  end

  defp do_wait_for_msg_and_handle_timeout(config, callback, req, msg, ioList, localAccess,
            nextBlockNo, prepared, retries) do
    code = :undef
    ...
    case (wait_for_msg(config, callback, req)) do
      ...
      {config2, decodedMsg} ->
        {config2, callback,
           r_transfer_res(status: :ok, decoded_msg: decodedMsg,
               prepared: prepared)}
    end
  end
  ...
  defp do_send_msg(r_config(udp_socket: socket, udp_host: remoteHost,
              udp_port: remotePort) = config,
            req, msg, ioList) do
    print_debug_info(config, req, :send, msg)
    try do
      :ok = :gen_udp.send(socket, remoteHost, remotePort,
                            ioList)
    catch
      :error, {:badmatch, {:error, :einval = reason}} ->
        error_msg(config, 'Stacktrace; ~p\n gen_udp:send(~p, ~p, ~p, ~p) -> ~p\n',
                    [__STACKTRACE__, socket, remoteHost, remotePort, ioList,
                                                                         {:error,
                                                                            reason}])
    ...
    receive do
      {:udp, socket, remoteHost, remotePort, bin}
          when (is_binary(bin) and
               ...
               ->
        _ = :inet.setopts(socket, [{:active, :once}])
        config2 = r_config(config, udp_host: remoteHost, 
                              udp_port: remotePort)
        decodedMsg = ((try do
                        :tftp_lib.decode_msg(bin)
                      catch
        ...
        {config2, decodedMsg}
      {:udp, socket, host, port, bin} when (is_binary(bin) and
        ...
        {config, decodedMsg}
      {:info, ref, fromPid} when is_pid(fromPid) ->
        type = (case (r_tftp_msg_req(req, :local_filename) !== :undefined) do
                  true ->
                    :client
                  false ->
                    :server
      ...
      {:system, from, msg} ->
        misc = r_sys_misc(module: :tftp_engine, function: :wait_for_msg,
                   arguments: [config, callback, req])
        :sys.handle_system_msg(msg, from,
                                 r_config(config, :parent_pid), :tftp_engine, [], misc)
      {:EXIT, pid, _Reason}
          when r_config(config, :parent_pid) === pid ->
        code = :undef
        text = 'Parent exited.'
        terminate(config, req,
                    r_error(where: :wait_for_msg, code: code, text: text,
                        filename: r_tftp_msg_req(req, :filename)))
      ...
      msg when r_tftp_msg_req(req, :local_filename) === :undefined ->
        ...
        wait_for_msg(config, callback, req)
    after r_config(config, :timeout) * 1000 ->
      print_debug_info(config, req, :recv, :timeout)
      :timeout
    end
  end

  defp early_read(config, callback, req, localAccess,
            _NextBlockNo,
            r_prepared(status: status, next_data: nextData,
                prev_data: prevData) = prepared) do
    cond do
  ...
  def callback(access, config, callback, req) do
    {callback2, result} = do_callback(access, config,
                                        callback, req)
    print_debug_info(config, req, :call,
                       {callback2, result})
    {callback2, result}
  end
  ...
  defp do_callback(:read = fun, config, callback, req)
    ...
    case ((try do
      ...
      {:more, bin, newState} when is_binary(bin) ->
        count = r_callback(callback, :count) + byte_size(bin)
        callback2 = r_callback(callback, state: newState, 
                                  block_no: nextBlockNo,  count: count)
        prepared = r_prepared(status: :more, result: :undefined,
                       block_no: nextBlockNo, next_data: bin)
        verify_count(config, callback2, req, prepared)
      {:last, bin, result} when is_binary(bin) ->
        prepared = r_prepared(status: :last, result: result,
                       block_no: nextBlockNo, next_data: bin)
        {:undefined, prepared}
      {:error, {code, text}} ->
        error = r_tftp_msg_error(code: code, text: text)
      ...
      illegal ->
        ...
        callback({:abort, {code, text, illegal}}, config,
                   callback, req)
    end
  end
  ...
  defp do_callback({:open, type}, config, callback, req)
    ...
    args = [access, filename, r_tftp_msg_req(req, :mode),
                                  r_tftp_msg_req(req, :options), r_callback(callback, :state)]
    peerInfo = peer_info(config)
    _ = fast_ensure_loaded(mod)
    args2 = (case (:erlang.function_exported(mod, fun,
                                               length(args))) do
               true ->
                 args
               false ->
  ...
  defp do_callback({:abort, {code, text}}, config, callback,
            req) do
    error = r_tftp_msg_error(code: code, text: text)
    do_callback({:abort, error}, config, callback, req)
  end

  defp do_callback({:abort, {code, text, details}}, config,
            callback, req) do
    error = r_tftp_msg_error(code: code, text: text, details: details)
    do_callback({:abort, error}, config, callback, req)
  end

  defp do_callback({:abort = fun,
             r_tftp_msg_error(code: code, text: text) = error},
            config, callback, req)
      when (elem(config, 0) === :config and
              elem(callback, 0) === :callback and
  ...
  defp lookup_callback_mod(mod, callbacks) do
    {:value, c} = :lists.keysearch(mod, r_callback(:module),
                                     callbacks)
    {:ok, c}
  end
  ...
  defp internal_info(config, type)
    ...
    [{:type, type}, {:host,
                       :tftp_lib.host_to_string(r_config(config, :udp_host))},
                        {:port, r_config(config, :udp_port)}, {:local_port,
                                                          actualPort},
                                                           {:port_policy,
                                                              r_config(config, :port_policy)},
                                                               {:udp,
                                                                  r_config(config, :udp_options)},
                                                                   {:use_tsize,
                                                                      r_config(config, :use_tsize)},
                                                                       {:max_tsize,
                                                                          r_config(config, :max_tsize)},
                                                                           {:max_conn,
                                                                              r_config(config, :max_conn)},
                                                                               {:rejected,
                                                                                  r_config(config, :rejected)},
                                                                                   {:timeout,
                                                                                      r_config(config, :timeout)},
                                                                                       {:polite_ack,
                                                                                          r_config(config, :polite_ack)},
                                                                                           {:debug,
                                                                                              r_config(config, :debug_level)},
                                                                                               {:parent_pid,
                                                                                                  r_config(config, :parent_pid)}] ++ r_config(config, :user_options) ++ r_config(config, :callbacks)
  end

  defp local_file_access(r_tftp_msg_req(access: access, local_filename: local,
  ...
  defp pre_verify_options(config, req) do
    ...
    case ((try do
      ...
      :ok ->
        case (verify_integer('tsize', 0, r_config(config, :max_tsize),
                               options)) do
          true ->
  ...
  defp post_verify_options(config, req, newOptions, text) do
    ...
    badOptions = (for {key, _Val} <- newOptions,
    ...
    case (:lists.keysearch(key, 1, options)) do
      {:value, {_, val}} when is_list(val) ->
        case ((try do
          ...
          {:EXIT, _} ->
            false
          int when (int >= min and is_integer(min) and
                      max === :infinity)
                   ->
            true
          int when (int >= min and is_integer(min) and
  ...
  defp fast_ensure_loaded(mod) do
    case (:erlang.function_exported(mod, :module_info,
                                      0)) do
      true ->
        :ok
      false ->
  ...
  defp print_debug_info(r_config(debug_level: level) = config, who, where,
            data) do
    cond do
      level === :none ->
        :ok
  ...
  defp do_print_debug_info(config, who, where, r_tftp_msg_data(data: bin) = msg)
    ...
    msg2 = r_tftp_msg_data(msg, data: {:bytes, byte_size(bin)})
    do_print_debug_info(config, who, where, msg2)
  end

  defp do_print_debug_info(config, who, where,
            r_tftp_msg_req(local_filename: filename) = msg)
  ...
  defp do_print_debug_info(config, who, where, data) do
    ...
    peerInfo = :lists.flatten(:io_lib.format('~p',
                                               [peer_info(config)]))
    side = (cond do
    ...
    case ({where, data}) do
      {_,
         r_error(where: ^where, code: code, text: text,
             filename: filename)} ->
  ...
  def system_continue(_Parent, _Debug,
           r_sys_misc(module: mod, function: fun, arguments: args)) do
    apply(mod, fun, args)
  end
