defmodule :m_tftp_engine do
  ...
  def daemon_init(config)
    ...
    case (try do
      ...
      {:ok, socket} ->
        ...
        print_debug_info(config2, :daemon, :open, r_tftp_msg_req(filename: ~c""))
      ...
      {:error, reason} ->
        ...
        print_debug_info(
          ...
          :daemon,
          :open,
          r_error(where: :open, code: :undef, text: text, filename: ~c"")
        )
  ...
  defp prepare_daemon_udp(
    ...
    case :lists.keymember(:fd, 1, udpOptions) do
      ...
      false ->
        initArg = :erlang.list_to_atom(~c"tftpd_" ++ :erlang.integer_to_list(port))
  ...
  def daemon_loop(
    ...
    receive do
      ...
      when is_binary(bin) ->
        ...
        case msg do
          ...
          when elem(req, 0) === :tftp_msg_req and
            ...
            peerReq = {peer, req}
            peerInfo = :lists.flatten(:io_lib.format(~c"~p", [peer]))
            ...
            case :ets.lookup(fileTab, peerReq) do
              ...
              [r_file_info(pid: pid)] ->
                warning_msg(daemonConfig, ~c"~p Reuse connection for ~s\n\t~p\n", [
                  pid,
                  peerInfo,
          ...
          req when elem(req, 0) === :tftp_msg_req ->
            reply = r_tftp_msg_error(code: :enospc, text: ~c"Too many connections")
            peer = peer_info(serverConfig)
            peerInfo = :lists.flatten(:io_lib.format(~c"~p", [peer]))

            warning_msg(
              daemonConfig,
              ~c"Daemon has too many connections (~p).\n\tRejecting request from ~s\n",
              [n, peerInfo]
            )
          ...
          req ->
            reply = r_tftp_msg_error(code: :badop, text: ~c"Illegal TFTP operation")

            warning_msg(daemonConfig, ~c"Daemon received: ~p.\n\tfrom ~p:~p", [
              req,
              remoteHost,
              remotePort
            ])
      ...
      when r_config(daemonConfig, :parent_pid) === pid ->
        close_port(daemonConfig, :daemon, r_tftp_msg_req(filename: ~c""))
        exit(reason)

      {:EXIT, pid, _Reason} = info ->
        case :ets.lookup(serverTab, pid) do
          [] ->
            warning_msg(daemonConfig, ~c"Daemon received: ~p", [info])
            :tftp_engine.daemon_loop(state)
              ...
              :ok ->
                case callback({:open, :server_open}, config3, callback, req2) do
                  {callback2, {:ok, acceptedOptions}} ->
                    ...
                    optText = ~c"Internal error. Not allowed to add new options."
      ...
      {:ok, config2} ->
        req2 =
          case r_config(config2, :use_tsize) do
            true ->
              suggestedOptions = r_tftp_msg_req(req, :options)
              suggestedOptions2 = :tftp_lib.replace_val(~c"tsize", ~c"0", suggestedOptions)
      ...
      :ok when elem(prepared, 0) === :prepared ->
        case decodedMsg do
          msg when elem(msg, 0) === :tftp_msg_oack ->
            ...
            optText = ~c"Protocol violation. Server is not allowed new options"
  ...
  defp do_client_open(config, callback, req) do
    case callback({:open, :client_open}, config, callback, req) do
      {callback2, {:ok, finalOptions}} ->
        optText = ~c"Internal error. Not allowed to change options."
  ...
  defp common_read(config, callback, req, _LocalAccess, expectedBlockNo, actualBlockNo, prepared)
    ...
    text =
      ~c"Unknown transfer ID = " ++
        :erlang.integer_to_list(actualBlockNo) ++
        ~c" (" ++ :erlang.integer_to_list(expectedBlockNo) ++ ~c")"
  ...
  defp do_transfer(
    ...
    case do_send_msg(config, req, msg, ioList) do
      ...
      {:error, reason} ->
        code = :undef
        text = :lists.flatten(:io_lib.format(~c"Transfer failed - giving up -> ~p", [reason]))
        error = r_tftp_msg_error(code: code, text: text)
  ...
  defp send_msg(config, req, msg) do
    case (try do
      ...
      {:EXIT, reason} ->
        code = :undef
        text = ~c"Internal error. Encode failed"
  ...
  defp do_send_msg(
    ...
    try do
      ...
      :error, {:badmatch, {:error, :einval = reason}} ->
        error_msg(config, ~c"Stacktrace; ~p\n gen_udp:send(~p, ~p, ~p, ~p) -> ~p\n", [
          __STACKTRACE__,
          socket,
          remoteHost,
          remotePort,
  ...
  def wait_for_msg(config, callback, req) do
    receive do
      ...
      msg when r_tftp_msg_req(req, :local_filename) !== :undefined ->
        warning_msg(config, ~c"Client received : ~p", [msg])
        wait_for_msg(config, callback, req)

      msg when r_tftp_msg_req(req, :local_filename) === :undefined ->
        warning_msg(config, ~c"Server received : ~p", [msg])
        wait_for_msg(config, callback, req)
    after
  ...
  defp do_callback(:read = fun, config, callback, req)
    ...
    case (try do
      ...
      illegal ->
        code = :undef
        text = ~c"Internal error. File handler error."
  ...
  defp do_match_callback(filename, [c | tail])
    ...
    case (try do
      ...
      details ->
        code = :baduser
        text = ~c"Internal error. File handler not found"
        {:error, r_tftp_msg_error(code: code, text: text, details: details)}
    end
  end
  ...
  defp pre_verify_options(config, req) do
    ...
    case (try do
      ...
      :ok ->
        case verify_integer(~c"tsize", 0, r_config(config, :max_tsize), options) do
          true ->
            case verify_integer(~c"blksize", 0, 65464, options) do
              true ->
                :ok

              false ->
                {:error, {:badopt, ~c"Too large blksize"}}
            end

          false ->
            {:error, {:badopt, ~c"Too large tsize"}}
        end
  ...
  defp verify_reject(config, req, options) do
    ...
    case :lists.member(access, rejected) do
      true ->
        {:error, {:eacces, :erlang.atom_to_list(access) ++ ~c" mode not allowed"}}

      false ->
        for {key, _} <- options, :lists.member(key, rejected) do
          throw({:error, {:badopt, key ++ ~c" not allowed"}})
        end
  ...
  defp lookup_timeout(options) do
    case :lists.keysearch(~c"timeout", 1, options) do
      {:value, {_, val}} ->
  ...
  defp lookup_mode(options) do
    case :lists.keysearch(~c"mode", 1, options) do
      {:value, {_, val}} ->
        val
  ...
  defp do_print_debug_info(config, who, where, data) do
    ...
    case {where, data} do
      {_, r_error(where: ^where, code: code, text: text, filename: filename)} ->
        do_format(config, side, local, ~c"error ~s ->\n\t~p ~p\n\t~p ~p: ~s\n", [
          peerInfo,
          self(),
          filename,
          where,
      ...
      {:open, r_tftp_msg_req(filename: filename)} ->
        do_format(config, side, local, ~c"open  ~s ->\n\t~p ~p\n", [peerInfo, self(), filename])

      {:close, r_tftp_msg_req(filename: filename)} ->
        do_format(config, side, local, ~c"close ~s ->\n\t~p ~p\n", [peerInfo, self(), filename])

      {:recv, _} ->
        do_format(config, side, local, ~c"recv  ~s <-\n\t~p\n", [peerInfo, data])

      {:send, _} ->
        do_format(config, side, local, ~c"send  ~s ->\n\t~p\n", [peerInfo, data])

      {:match, _} when elem(data, 0) === :callback ->
        mod = r_callback(data, :module)
        state = r_callback(data, :state)
        do_format(config, side, local, ~c"match ~s ~p =>\n\t~p\n", [peerInfo, mod, state])
      ...
      {:call, _} ->
        case data do
          {callback, _Result} when elem(callback, 0) === :callback ->
            ...
            do_format(config, side, local, ~c"call ~s ~p =>\n\t~p\n", [peerInfo, mod, state])

          {:undefined, result} ->
            do_format(config, side, local, ~c"call ~s result =>\n\t~p\n", [peerInfo, result])
        end
    end
  end

  defp do_format(config, side, local, format, args) do
    info_msg(config, ~c"~p(~p): " ++ format, [side, local | args])
  end
