defmodule :m_tftp_file do
  ...
  Record.defrecord(:r_file_descriptor, :file_descriptor, module: :undefined,
                                           data: :undefined)
  Record.defrecord(:r_initial, :initial, filename: :undefined,
                                   is_native_ascii: :undefined)
  Record.defrecord(:r_state, :state, access: :undefined,
                                 filename: :undefined,
                                 is_native_ascii: :undefined,
                                 is_network_ascii: :undefined,
                                 root_dir: :undefined, options: :undefined,
                                 blksize: :undefined, fd: :undefined,
                                 count: :undefined, buffer: :undefined)
  def prepare(_Peer, access, filename, mode, suggestedOptions,
           initial)
      when is_list(initial) do
    case ((try do
            handle_options(access, filename, mode, suggestedOptions,
                             initial)
          catch
      ...
      {:ok, filename2, isNativeAscii, isNetworkAscii,
         acceptedOptions} ->
        state = r_state(access: access, filename: filename2,
                    is_native_ascii: isNativeAscii,
                    is_network_ascii: isNetworkAscii,
                    options: acceptedOptions,
                    blksize: lookup_blksize(acceptedOptions), count: 0,
                    buffer: [])
  ...
  def open(peer, access, filename, mode, suggestedOptions,
           initial)
      when is_list(initial) do
    case (prepare(peer, access, filename, mode,
                    suggestedOptions, initial)) do
      {:ok, acceptedOptions, state} ->
        open(peer, access, filename, mode, acceptedOptions,
               state)
      {:error, {code, text}} ->
        {:error, {code, text}}
    end
  ...
  def open(_Peer, access, filename, mode, negotiatedOptions,
           state)
      when elem(state, 0) === :state do
    case ((try do
            handle_options(access, filename, mode,
                             negotiatedOptions, state)
          catch
            :error, e -> {:EXIT, {e, __STACKTRACE__}}
            :exit, e -> {:EXIT, e}
            e -> e
          end)) do
      {:ok, _Filename2, _IsNativeAscii, _IsNetworkAscii,
         options}
  ...
  def open(peer, access, filename, mode, negotiatedOptions,
           state) do
    state2 = upgrade_state(state)
    open(peer, access, filename, mode, negotiatedOptions,
           state2)
  end

  defp do_open(state) when elem(state, 0) === :state do
    case (:file.open(r_state(state, :filename),
                       file_options(state))) do
      {:ok, fd} ->
  ...
  defp file_error(reason) when is_atom(reason) do
    ...
    case (reason) do
      ...
      _ ->
        {:undef,
           details ++ ' (' ++ :erlang.atom_to_list(reason) ++ ')'}
    end
  end
  ...
  def read(r_state(access: :read) = state) do
    ...
    case (:file.read(r_state(state, :fd), blkSize)) do
      {:ok, bin} when (is_binary(bin) and
                         byte_size(bin) === blkSize)
                      ->
        count = r_state(state, :count) + byte_size(bin)
  ...
  def abort(_Code, _Text,
           r_state(fd: fd, access: access) = state) do
    _ = :file.close(fd)
    case (access) do
      :write ->
    ...
    case (key) do
      'tsize' ->
        case (access) do
          ...
          _ ->
            handle_integer(access, filename, key, val, t, 0,
                             :infinity)
        end
      'blksize' ->
  ...
  defp handle_integer(access, filename, key, val, options, min,
            max) do
    case ((try do
      ...
      int when (int >= min and int <= max) ->
        [{key, val} | do_handle_options(access, filename,
                                          options)]
  ...
  defp upgrade_state({:state, access, filename, rootDir, options,
             blkSize, fd, count, buffer}) do
    {:state, access, filename, false, false, rootDir,
       options, blkSize, fd, count, buffer}
  end
