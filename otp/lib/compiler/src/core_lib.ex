defmodule :m_core_lib do
  ...
  defp vu_expr(v,
            r_c_try(arg: e, vars: vs, body: b, evars: evs, handler: h)) do
    case (vu_expr(v, e)) do
      true ->
        true
      false ->
  ...
  defp vu_seg_list(v, ss) do
    :lists.any(fn r_c_bitstr(val: val, size: size) ->
  ...
  defp vu_pat_seg_list(v, ss) do
    :lists.any(fn r_c_bitstr(size: size) ->
                    vu_pattern(v, size)
               end,
                 ss)
  end
  ...
  defp vu_map_pairs(v, [r_c_map_pair(key: key, val: pat) | t]) do
    vu_expr(v, key) or vu_pattern(v, pat) or vu_map_pairs(v,
                                                            t)
  end
