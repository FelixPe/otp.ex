defmodule :m_beam_jump do
  ...
  defp unpack_typed_reg(r_tr(r: reg)) do
    reg
  end

  defp unpack_typed_reg(reg) do
    reg
  end
  ...
  defp unambigous_deallocation([{:call_ext, _, _} | is]) do
    find_deallocation(is)
  end

  defp unambigous_deallocation([{:call, _, _} | is]) do
    find_deallocation(is)
  end

  defp unambigous_deallocation([_ | is]) do
    unambigous_deallocation(is)
  end

  defp unambigous_deallocation([]) do
    true
  end

  defp find_deallocation([{:block, _} | is]) do
    find_deallocation(is)
  end
  ...
  defp find_deallocation([{:call, _, _} | is]) do
    find_deallocation(is)
  end

  defp find_deallocation([{:call_ext, _, _} | is]) do
    find_deallocation(is)
  end

  defp find_deallocation([{:deallocate, _} | _]) do
    true
  end

  defp find_deallocation([{:init_yregs, _} | is]) do
    find_deallocation(is)
  end

  defp find_deallocation([{:line, _} | is]) do
    find_deallocation(is)
  end

  defp find_deallocation([:return]) do
    true
  end

  defp find_deallocation(_) do
    false
  end
  ...
  defp is_shareable([{:badmatch, _} | _]) do
    false
  end
  ...
  defp classify_labels([{:catch, _, {:f, l}} | is], scope0, safe0) do
    scope = scope0 + 1
  ...
  defp classify_labels([{:try, _, {:f, l}} | is], scope0, safe0) do
    scope = scope0 + 1
    safe = classify_add_label(l, scope, safe0)
    classify_labels(is, scope, safe)
  end
  ...
  defp label_used({:f, l}, st) do
    r_st(st, labels: :sets.add_element(l, r_st(st, :labels)))
  end
  ...
  defp is_label_used(l, st) do
    :sets.is_element(l, r_st(st, :labels))
  end
  ...
  defp is_exit_instruction(:if_end) do
    true
  end

  defp is_exit_instruction({:case_end, _}) do
    true
  end

  defp is_exit_instruction({:try_case_end, _}) do
    true
  end

  defp is_exit_instruction({:badmatch, _}) do
    true
  end

  defp is_exit_instruction({:badrecord, _}) do
    true
  end

  defp is_exit_instruction(_) do
    false
  end
  ...
  defp instr_labels({:bs_create_bin, lbl, _, _, _, _, _}) do
    do_instr_labels(lbl)
  end
  ...
  defp instr_labels({:bs_match, {:f, fail}, _Ctx, _List}) do
    [fail]
  end
