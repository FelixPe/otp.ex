defmodule :m_beam_listing do
  ...
  def module(
    ...
    :io.format(file, ~c"module ~p.\n", [mod])
    :io.format(file, ~c"exports ~p.\n", [exp])
    :io.format(file, ~c"attributes ~kp.\n\n", [attr])
  ...
  def module(stream, {mod, exp, attr, code, numLabels}) do
    :io.format(stream, ~c"{module, ~kp}.  %% version = ~w\n", [mod, :beam_opcodes.format_number()])
    ...
    lbl2Fun =
      foldl(
        fn {:function, name, arity, entry, _}, map ->
          Map.put(map, entry, {name, arity})
        end,
        %{},
        code
      )
  ...
  def module(stream, [_ | _] = fs) do
    foreach(
      fn f ->
        :io.format(stream, ~c"~kp.\n", [f])
      end,
      fs
    )
  end

  defp format_asm([{:label, l} | is], lbl2Fun) do
    [:io_lib.format(~c"  {label,~p}.\n", [l]) | format_asm(is, lbl2Fun)]
  end

  defp format_asm([i = {call, _, l} | is], lbl2Fun)
       when call === :call or call === :call_only do
    format_asm_call(l, i, is, lbl2Fun)
  end

  defp format_asm([i = {:call_last, _, l, _} | is], lbl2Fun) do
    format_asm_call(l, i, is, lbl2Fun)
  end

  defp format_asm([i | is], lbl2Fun) do
    [:io_lib.format(~c"    ~kp", [i]), ~c".\n" | format_asm(is, lbl2Fun)]
  end

  defp format_asm([], _) do
    []
  end

  defp format_asm_call({:f, l}, i, is, lbl2Fun) do
    {n, a} = :erlang.map_get(l, lbl2Fun)
    [:io_lib.format(~c"    ~p. % ~p/~p\n", [i, n, a]) | format_asm(is, lbl2Fun)]
  end
end
