defmodule :m_beam_asm do
  ...
  import :lists, only: [append: 1, duplicate: 2,
                          keymember: 3, map: 2, member: 2, splitwith: 2]
  require Record
  ...
  def module(code, extraChunks, compileInfo, compilerOpts) do
    {:ok,
       assemble(code, extraChunks, compileInfo, compilerOpts)}
  end

  defp assemble({mod, exp0, attr0, asm0, numLabels},
            extraChunks, compileInfo, compilerOpts) do
    {1, dict0} = :beam_dict.atom(mod, :beam_dict.new())
    {0,
       dict1} = :beam_dict.fname(:erlang.atom_to_list(mod) ++ '.erl',
                                   dict0)
  ...
  defp shared_fun_wrappers(opts, dict) do
    case (:proplists.get_bool(:no_shared_fun_wrappers,
                                opts)) do
      false ->
  ...
  defp on_load(fs0, attr0) do
    case (:proplists.get_value(:on_load, attr0)) do
      :undefined ->
        {fs0, attr0}
      [{name, 0}] ->
        fs = map(fn {:function, n, 0, entry, is0}
                        when n === name ->
  ...
  defp insert_on_load_instruction(is0, entry) do
    {bef,
       [{:label, ^entry} = el | is]} = splitwith(fn {:label, l}
                                                        when l === entry ->
                                                      false
                                                    _ ->
                                                      true
  ...
  defp assemble_1([{:function, name, arity, entry, asm} | t], exp,
            dict0, acc) do
    dict1 = (case (:sets.is_element({name, arity}, exp)) do
               true ->
  ...
  defp build_file(code, attr, dict, numLabels, numFuncs,
            extraChunks0, compileInfo, compilerOpts) do
    codeChunk = chunk("Code",
                        <<16 :: size(32),
                            :beam_opcodes.format_number() :: size(32),
                            :beam_dict.highest_opcode(dict) :: size(32),
                            numLabels :: size(32), numFuncs :: size(32)>>,
                        code)
    {numAtoms, atomTab} = :beam_dict.atom_table(dict)
    atomChunk = chunk("AtU8", <<numAtoms :: size(32)>>, atomTab)
    ...
    essentials0 = [atomChunk, codeChunk, stringChunk,
                                             importChunk, expChunk, lambdaChunk,
                                                                        literalChunk,
                                                                            metaChunk]
    essentials1 = (for c <- essentials0 do
                     :erlang.iolist_to_binary(c)
                   end)
    mD5 = module_md5(essentials1)
    essentials = finalize_fun_table(essentials1, mD5)
    {attributes, compile} = build_attributes(attr,
                                               compileInfo, mD5)
    attrChunk = chunk("Attr", attributes)
    compileChunk = chunk("CInf", compile)
    checkedChunks = (for {key, value} <- extraChunks do
                       chunk(key, value)
                     end)
    chunks = (case (member(:slim, compilerOpts)) do
                true when numTypes > 0 ->
                  [essentials, attrChunk, typeChunk]
                true when numTypes === 0 ->
                  [essentials, attrChunk]
                false ->
                  [essentials, locChunk, attrChunk, compileChunk,
                                                        checkedChunks,
                                                            lineChunk,
                                                                typeChunk]
              end)
  ...
  defp finalize_fun_table_1(<<"FunT", keep :: size(8) - binary,
              table0 :: binary>>,
            mD5) do
  ...
  defp finalize_fun_table_2(<<keep :: size(20) - binary, 0 :: size(32),
              t :: binary>>,
            uniq, acc) do
    finalize_fun_table_2(t, uniq,
                           <<acc :: binary, keep :: binary, uniq :: size(32)>>)
  end
  ...
  defp build_form(id, chunks0) when (byte_size(id) === 4 and
                               is_list(chunks0)) do
    chunks = :erlang.list_to_binary(chunks0)
    size = byte_size(chunks)
    0 = rem(size, 4)
    <<"FOR1", size + 4 :: size(32), id :: binary,
        chunks :: binary>>
  end

  defp chunk(id, contents) when (byte_size(id) === 4 and
  ...
  defp chunk(id, head, contents)
    ...
    [<<id :: binary, size :: size(32), head :: binary>>,
         contents | pad(size)]
  end
  ...
  defp flatten_exports(exps) do
    :erlang.list_to_binary(map(fn {f, a, l} ->
                                    <<f :: size(32), a :: size(32),
                                        l :: size(32)>>
                               end,
                                 exps))
  ...
  defp flatten_imports(imps) do
    :erlang.list_to_binary(map(fn {m, f, a} ->
                                    <<m :: size(32), f :: size(32),
                                        a :: size(32)>>
                               end,
                                 imps))
  ...
  defp build_attributes(attr, compile, mD5) do
    attrBinary = :erlang.term_to_binary(set_vsn_attribute(attr,
                                                            mD5))
    compileBinary = :erlang.term_to_binary([{:version,
                                               :EFE_TODO_COMPILER_VSN_MACRO} |
                                                compile])
    {attrBinary, compileBinary}
  end

  defp build_line_table(dict) do
    {numLineInstrs, numFnames0, fnames0, numLines,
       lines0} = :beam_dict.line_table(dict)
    numFnames = numFnames0 - 1
    [_ | fnames1] = fnames0
    fnames2 = (for f <- fnames1 do
                 :unicode.characters_to_binary(f)
               end)
    fnames = (for f <- fnames2, into: <<>> do
    ...
    ver = 0
    bits = 0
    <<ver :: size(32), bits :: size(32),
        numLineInstrs :: size(32), numLines :: size(32),
        numFnames :: size(32), lines :: binary,
        fnames :: binary>>
  end
  ...
  defp filter_essentials([<<_Tag :: size(4) - binary, sz :: size(32),
               data :: size(sz) - binary, _Padding :: binary>> |
               t]) do
  ...
  defp make_op({:bif, bif, {:f, _}, [], dest}, dict) do
    encode_op(:bif0, [{:extfunc, :erlang, bif, 0}, dest],
                dict)
  end

  defp make_op({:bif, :raise, _Fail, [_A1, _A2] = args, _Dest},
            dict) do
  ...
  defp make_op({:bif, bif, fail, args, dest}, dict) do
    ...
    case (bif_type(bif, arity)) do
      {:op, op} ->
        make_op(:erlang.list_to_tuple([op, fail |
                                               args ++ [dest]]),
                  dict)
  ...
  defp make_op({:bs_add = op, fail, [src1, src2, unit], dest},
            dict) do
    encode_op(op, [fail, src1, src2, unit, dest], dict)
  end

  defp make_op({:test, cond__, fail, src, {:list, _} = ops},
            dict) do
  ...
  defp make_op({:test, cond__, fail, live, [op | ops], dst},
            dict)
      when is_list(ops) do
    encode_op(cond__, [fail, op, live | ops ++ [dst]], dict)
  end

  defp make_op({:make_fun2, {:f, lbl}, _Index, _OldUniq,
             numFree},
            dict0) do
  ...
  defp make_op({:make_fun3, {:f, lbl}, _Index, _OldUniq, dst,
             {:list, env}},
            dict0) do
    numFree = length(env)
    {fun, dict} = :beam_dict.lambda(lbl, numFree, dict0)
  ...
  defp make_op({:call_fun2, {:f, lbl}, arity, func}, dict0) do
    r_tr(t: r_t_fun(target: {_Name, totalArity})) = func
  ...
  defp make_op(instr, dict) when tuple_size(instr) >= 6 do
    [name | args] = :erlang.tuple_to_list(instr)
  ...
  def encode(tag, n) when (is_integer(n) and n < 0) do
    encode1(tag, negative_to_bytes(n))
  end

  def encode(tag, n) when (is_integer(n) and n < 16) do
    n <<< 4 ||| tag
  end

  def encode(tag, n) when (is_integer(n) and n < 2048) do
    [(n >>> 3) &&& 224 ||| tag ||| 8, n &&& 255]
  end

  def encode(tag, n) when is_integer(n) do
