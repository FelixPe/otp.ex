defmodule :m_beam_asm do
  use Bitwise
  import :lists, only: [append: 1, duplicate: 2, keymember: 3, map: 2, member: 2, splitwith: 2]
  require Record
  ...
  defp assemble_1([{:function, name, arity, entry, asm} | t], exp, dict0, acc) do
    dict1 =
      case :sets.is_element({name, arity}, exp) do
        true ->
  ...
  defp build_file(code, attr, dict, numLabels, numFuncs, extraChunks0, compileInfo, compilerOpts) do
    codeChunk =
      chunk(
        "Code",
    ...
    typeChunk = chunk("Type", <<2::size(32), numTypes::size(32)>>, typeTab)
    ...
    chunks =
      case member(:slim, compilerOpts) do
        ...
        false ->
          [essentials, locChunk, attrChunk, compileChunk, checkedChunks, lineChunk, typeChunk]
      end
