defmodule :m_cerl_trees do
  use Bitwise
  import :cerl, only: [alias_pat: 1, alias_var: 1,
                         ann_c_alias: 3, ann_c_apply: 3, ann_c_binary: 2,
                         ann_c_bitstr: 6, ann_c_call: 4, ann_c_case: 3,
                         ann_c_catch: 2, ann_c_clause: 4, ann_c_cons_skel: 3,
                         ann_c_fun: 3, ann_c_let: 4, ann_c_letrec: 3,
                         ann_c_map: 3, ann_c_map_pair: 4, ann_c_map_pattern: 2,
                         ann_c_module: 5, ann_c_primop: 3, ann_c_receive: 4,
                         ann_c_seq: 3, ann_c_try: 6, ann_c_tuple_skel: 2,
                         ann_c_values: 2, apply_args: 1, apply_op: 1,
                         ...
                         clause_vars: 1, concrete: 1, cons_hd: 1, cons_tl: 1,
                         fun_body: 1, fun_vars: 1, get_ann: 1,
                         is_c_map_pattern: 1, let_arg: 1, let_body: 1,
                         let_vars: 1, letrec_body: 1, letrec_defs: 1,
                         letrec_vars: 1, map_arg: 1, map_es: 1, map_pair_key: 1,
                         ...
                         receive_action: 1, receive_clauses: 1,
                         receive_timeout: 1, seq_arg: 1, seq_body: 1,
                         set_ann: 2, subtrees: 1, try_arg: 1, try_body: 1,
                         try_evars: 1, try_handler: 1, try_vars: 1, tuple_es: 1,
                         type: 1, update_c_alias: 3, update_c_apply: 3,
                         update_c_binary: 2, update_c_bitstr: 6,
                         update_c_call: 4, update_c_case: 3, update_c_catch: 2,
                         update_c_clause: 4, update_c_cons: 3,
                         update_c_cons_skel: 3, update_c_fun: 3,
                         update_c_let: 4, update_c_letrec: 3, update_c_map: 3,
                         update_c_map_pair: 4, update_c_module: 5,
                         update_c_primop: 3, update_c_receive: 4,
                         update_c_seq: 3, update_c_try: 6, update_c_tuple: 2,
                         update_c_tuple_skel: 2, update_c_values: 2,
  ...
  defp map_1(f, t) do
    case (type(t)) do
      ...
      :map ->
        update_c_map(t, map(f, map_arg(t)),
                       map_list(f, map_es(t)))
      :map_pair ->
        update_c_map_pair(t, map(f, map_pair_op(t)),
                            map(f, map_pair_key(t)), map(f, map_pair_val(t)))
      :let ->
        update_c_let(t, map_list(f, let_vars(t)),
                       map(f, let_arg(t)), map(f, let_body(t)))
      :seq ->
      ...
      :call ->
        update_c_call(t, map(f, call_module(t)),
                        map(f, call_name(t)), map_list(f, call_args(t)))
      :primop ->
      ...
      :clause ->
        update_c_clause(t, map_list(f, clause_pats(t)),
                          map(f, clause_guard(t)), map(f, clause_body(t)))
      :alias ->
      ...
      :try ->
        update_c_try(t, map(f, try_arg(t)),
                       map_list(f, try_vars(t)), map(f, try_body(t)),
                       map_list(f, try_evars(t)), map(f, try_handler(t)))
      :catch ->
      ...
      :bitstr ->
        update_c_bitstr(t, map(f, bitstr_val(t)),
                          map(f, bitstr_size(t)), map(f, bitstr_unit(t)),
                          map(f, bitstr_type(t)), map(f, bitstr_flags(t)))
      :letrec ->
  ...
  defp fold_1(f, s, t) do
    case (type(t)) do
      ...
      :clause ->
        fold(f,
               fold(f, fold_list(f, s, clause_pats(t)),
                      clause_guard(t)),
               clause_body(t))
      :alias ->
      ...
      :receive ->
        fold(f,
               fold(f, fold_list(f, s, receive_clauses(t)),
                      receive_timeout(t)),
               receive_action(t))
      :try ->
        fold(f,
               fold_list(f,
                           fold(f,
                                  fold_list(f, fold(f, s, try_arg(t)),
                                              try_vars(t)),
                                  try_body(t)),
                           try_evars(t)),
               try_handler(t))
      ...
      :bitstr ->
        fold(f,
               ...
               bitstr_flags(t))
      :letrec ->
        fold(f, fold_pairs(f, s, letrec_defs(t)),
               letrec_body(t))
      :module ->
        fold_pairs(f,
                     fold_pairs(f,
                                  fold_list(f, fold(f, s, module_name(t)),
                                              module_exports(t)),
                                  module_attrs(t)),
                     module_defs(t))
      :opaque ->
    ...
    case (pre.(t0, s00)) do
      {t, s0} ->
        case (type(t)) do
          ...
          :receive ->
            {cs, s1} = mapfold_list(pre, post, s0,
                                      receive_clauses(t))
          ...
          :binary ->
            {ds, s1} = mapfold_list(pre, post, s0,
                                      binary_segments(t))
          ...
          :bitstr ->
            ...
            post.(update_c_bitstr(t, val, size, unit, type, flags),
                    s5)
          :letrec ->
          ...
          :module ->
            ...
            {es, s2} = mapfold_list(pre, post, s1,
                                      module_exports(t))
  ...
  defp variables(t, s) do
    case (type(t)) do
      ...
      :map_pair ->
        vars_in_list([map_pair_op(t), map_pair_key(t),
                                          map_pair_val(t)],
                       s)
      :let ->
      ...
      :apply ->
        :ordsets.union(variables(apply_op(t), s),
                         vars_in_list(apply_args(t), s))
      :call ->
        :ordsets.union(variables(call_module(t), s),
                         :ordsets.union(variables(call_name(t), s),
      ...
      :receive ->
        :ordsets.union(vars_in_list(receive_clauses(t), s),
      ...
      :module ->
        ...
        vs1 = :ordsets.union(vars_in_list(module_exports(t), s),
                               vs)
  ...
  defp vars_in_defs([{_, post} | ds], s, a) do
    vars_in_defs(ds, s,
                   :ordsets.union(variables(post, s), a))
  end
