defmodule :m_beam_ssa_lint do
  ...
  def format_error({{_M, f, a}, error}) do
    [:io_lib.format(~c"~p/~p: ", [f, a]), format_error_1(error)]
  end
  ...
  defp vvars_assert_phi_vars(phis, i, id, r_vvars(blocks: blocks, branches: branches)) do
    phiVars =
  ...
  defp vvars_block_1(
         [
           r_b_set(op: :new_try_tag, dst: dst, args: args) = i
           | is
         ],
         terminator,
         state
  ...
  defp vvars_block_1(
         [
           r_b_set(op: :kill_try_tag, dst: dst, args: [tag]) = i
           | is
         ],
         terminator,
         state0
  ...
  defp vvars_block_1(
         [
           r_b_set(op: :catch_end, dst: dst, args: [tag, kind]) = i
           | is
         ],
         terminator,
         state0
  ...
  defp vvars_terminator_1(labels0, from, r_vvars(branches: branches) = state0) do
    labels =
      for to <- labels0,
  ...
  defp vvars_save_branch(from, to, state) do
    ...
    case branches0 do
      %{^to => {lblVars, lblTryTags}} ->
        ...
        branch = {vars, tryTags}
        branches = Map.merge(branches0, %{to => merged, {from, to} => branch})
        r_vvars(state, branches: branches)

      _ ->
        branch = {vars, tryTags}
        branches = Map.merge(branches0, %{to => branch, {from, to} => branch})
        r_vvars(state, branches: branches)
    end
  end
  ...
  defp format_error_1({:redefined_variable, name, old, i}) do
    :io_lib.format(
      ~c"Variable ~ts (~ts) redefined by ~ts",
      [format_var(name), format_instr(old), format_instr(i)]
    )
  end

  defp format_error_1({:missing_phi_paths, paths, i}) do
    :io_lib.format(~c"Phi node ~ts doesn't define a value for these branches: ~w", [
      format_instr(i),
      paths
    ])
  end
  ...
  defp format_error_1({:garbage_phi_paths, paths, i}) do
    :io_lib.format(
      ~c"Phi node ~ts defines a value for these unreachable or non-existent branches: ~w",
  ...
  defp format_error_1({:unknown_phi_variable, name, {from, _To}, i}) do
    :io_lib.format(
      ~c"Variable ~ts used in phi node ~ts is undefined on branch ~w",
      [format_var(name), format_instr(i), from]
    )
  end

  defp format_error_1({:unknown_block, label, i}) do
    :io_lib.format(~c"Unknown block ~p referenced in ~ts", [label, i])
  end

  defp format_error_1({:unknown_variable, name, i}) do
    :io_lib.format(~c"Unbound variable ~ts used in ~ts", [format_var(name), format_instr(i)])
  end

  defp format_error_1({:phi_inside_block, name, id}) do
    :io_lib.format(~c"Phi node defining ~ts is not at start of block ~p", [format_var(name), id])
  end

  defp format_error_1({:undefined_label_in_phi, label, i}) do
    :io_lib.format(~c"Unknown block label ~p in phi node ~ts", [label, format_instr(i)])
  end

  defp format_error_1({:succeeded_not_preceded, i}) do
    :io_lib.format(~c"~ts does not reference the preceding instruction", [format_instr(i)])
  end

  defp format_error_1({:succeeded_not_last, i}) do
    :io_lib.format(~c"~ts is not the last instruction in its block", [format_instr(i)])
  end

  defp format_error_1({:not_normalized, i}) do
    :io_lib.format(~c"~ts is not normalized by beam_ssa:normalize/1", [format_instr(i)])
  end

  defp format_error_1({:succeeded_not_followed_by_two_way_br, i}) do
    :io_lib.format(~c"~ts not followed by a two-way branch", [format_instr(i)])
  end

  defp format_error_1({:active_try_tags_on_return, tryTags0, i}) do
    tryTags = format_vars(:gb_sets.to_list(tryTags0))
    :io_lib.format(~c"Try tags ~ts are still active on ~ts", [tryTags, format_instr(i)])
  end
end
