defmodule :m_beam_ssa_alias do
  use Bitwise
  import :lists, only: [any: 2, foldl: 3, reverse: 1, zip: 2]
  require Record
  ...
  Record.defrecord(:r_aas, :aas,
    ...
    kills: :undefined,
    st_map: :undefined,
    orig_st_map: :undefined,
    repeats: :sets.new([{:version, 2}])
  )

  Record.defrecord(:r_liveness_st, :liveness_st,
    in: :sets.new([{:version, 2}]),
    out: :sets.new([{:version, 2}])
  )

  Record.defrecord(:r_vas, :vas,
    status: :undefined,
    parents: [],
    child: :none,
    extracted: [],
  ...
  def opt(stMap0, funcDb0) do
    case any_huge_function(stMap0) do
      ...
      false ->
        funs =
          for f <- :maps.keys(stMap0), :erlang.is_map_key(f, funcDb0), not is_nif(f, stMap0) do
            f
          end
  ...
  defp liveness_blks([{lbl, blk} | blocks], state0, useDefCache) do
    ...
    liveness_blks(blocks, set_block_liveness(lbl, in__, out, state0), useDefCache)
  end
  ...
  defp kills_fun1([{lbl, blk} | blocks], killsMap0, liveness) do
    killsMap = kills_block(lbl, blk, :erlang.map_get(lbl, liveness), killsMap0)
  ...
  defp kills_block(lbl, r_b_blk(is: is, last: last), r_liveness_st(out: out), killsMap0) do
  ...
  defp aa(funs, killsMap, stMap, funcDb) do
    argsInfo =
      foldl(
        fn f = r_b_local(), acc ->
          r_func_info(exported: e, arg_types: aT) =
            :erlang.map_get(
              f,
              funcDb
            )
  ...
  defp aa_fixpoint(
         funs,
         aAS = r_aas(alias_map: aliasMap, call_args: callArgs, func_db: funcDb)
       ) do
    order = aa_breadth_first(funs, funcDb)
    aa_fixpoint(order, order, aliasMap, callArgs, aAS, 16)
  end

  defp aa_fixpoint([f | fs], order, oldAliasMap, oldCallArgs, aAS0 = r_aas(st_map: stMap), limit) do
  ...
  defp aa_fixpoint(
         ...
         oldAliasMap,
         oldCallArgs,
         r_aas(alias_map: oldAliasMap, call_args: oldCallArgs, func_db: funcDb) = aAS,
         _Limit
       ) do
    :skip
  ...
  defp aa_fixpoint([], _, _, _, r_aas(func_db: funcDb, orig_st_map: stMap), 0) do
    :skip
    {stMap, funcDb}
  end
  ...
  defp aa_fixpoint(
         ...
         _OldAliasMap,
         _OldCallArgs,
         r_aas(alias_map: aliasMap, call_args: callArgs, repeats: repeats) = aAS,
         limit
       ) do
    :skip
    ...
    aa_fixpoint(
      ...
      aliasMap,
      callArgs,
      r_aas(aAS, repeats: :sets.new([{:version, 2}])),
      limit - 1
    )
  end
  ...
  defp aa_fun(
         ...
         aAS0 =
           r_aas(alias_map: aliasMap0, call_args: callArgs0, func_db: funcDb, repeats: repeats0)
       ) do
    ...
    :skip
    {sS, r_aas(call_args: callArgs) = aAS} = aa_blocks(linear0, %{0 => sS0}, aAS0)
    :skip
    aliasMap = Map.put(aliasMap0, f, sS)
    prevSS = :maps.get(f, aliasMap0, %{})

    repeats =
      case prevSS !== sS or callArgs0 !== callArgs do
        true ->
          %{^f => r_func_info(in: in__, out: out)} = funcDb
          foldl(&:sets.add_element/2, foldl(&:sets.add_element/2, repeats0, out), in__)
  ...
  defp aa_blocks([{l, r_b_blk(is: is0, last: t0)} | bs0], lbl2SS0, aAS0) do
    %{^l => sS0} = lbl2SS0
  ...
  defp aa_is(
         [
           i = r_b_set(dst: dst, op: op, args: args, anno: anno0)
           | is
         ],
         thisBlock,
         sS0,
  ...
  defp aa_terminator(r_b_br(succ: s, fail: f), sS, thisBlock, lbl2SS) do
    aa_set_block_exit_ss(thisBlock, sS, aa_add_block_entry_ss([s, f], sS, lbl2SS))
  end

  defp aa_terminator(r_b_ret(arg: arg, anno: anno0), sS, thisBlock, lbl2SS0) do
  ...
  defp aa_merge_continue(a, b, [v | vars], parentFixups, aliasFixups) do
    ...
    case a do
      ...
      %{} ->
        :skip
        aa_merge_continue(Map.put(a, v, bVas), b, vars, parentFixups, aliasFixups)
    end
  end
  ...
  defp aa_merge_1(_V, vas, vas, a, b, vars, parentFixups, aliasFixups) do
    :skip
    aa_merge_continue(a, b, vars, parentFixups, aliasFixups)
  end

  defp aa_merge_1(_V, r_vas(status: :aliased), bVas, a, b, vars, parentFixups, aliasFixups) do
    :skip
    aa_merge_continue(a, b, vars, parentFixups, aa_related(bVas) ++ aliasFixups)
  end

  defp aa_merge_1(v, aVas, r_vas(status: :aliased), a, b, vars, parentFixups, aliasFixups) do
    :skip

    aa_merge_continue(
      Map.put(a, v, r_vas(status: :aliased)),
      b,
      vars,
      parentFixups,
  ...
  defp aa_merge_child(
         v,
         r_vas(child: child) = aVas,
         r_vas(child: child) = bVas,
         a,
         b,
         vars,
         parentFixups,
  ...
  defp aa_merge_child(
    ...
    aa_merge_tuple(
      v,
      r_vas(aVas, child: child),
      bVas,
      Map.put(a, v, bVas),
      b,
      vars,
      [{child, v} | parentFixups],
      aliasFixups
    )
  end

  defp aa_merge_child(v, aVas, r_vas(child: :none) = bVas, a, b, vars, parentFixups, aliasFixups) do
    :skip
    aa_merge_tuple(v, aVas, bVas, a, b, vars, parentFixups, aliasFixups)
  end

  defp aa_merge_child(v, aVas, bVas, a, b, vars, parentFixups, aliasFixups) do
    :skip
  ...
  defp aa_merge_tuple(
         v,
         r_vas(tuple_elems: es) = aVas,
         r_vas(tuple_elems: es) = bVas,
         a,
         b,
         vars,
         parentFixups,
  ...
  defp aa_merge_tuple(
         v,
         r_vas(tuple_elems: aEs) = aVas,
         r_vas(tuple_elems: bEs) = bVas,
         a,
         b,
         vars,
         parentFixups,
    ...
    case aa_non_aliasing_tuple_elements(aEs ++ bEs) do
      true ->
        ...
        aa_merge_pair(v, vas, bVas, Map.put(a, v, vas), b, vars, parentFixups, aliasFixups)
  ...
  defp aa_merge_pair(
         v,
         r_vas(pair_elems: es) = aVas,
         r_vas(pair_elems: es) = bVas,
         a,
         b,
         vars,
         parentFixups,
  ...
  defp aa_merge_pair(
         v,
         r_vas(pair_elems: aEs) = aVas,
         r_vas(pair_elems: bEs) = bVas,
         a,
         b,
         vars,
         parentFixups,
    ...
    case r do
      ...
      pair ->
        ...
        aa_merge_extracted(v, vas, bVas, Map.put(a, v, vas), b, vars, parentFixups, aliasFixups)
    end
  end

  defp aa_merge_extracted(
         v,
         r_vas(extracted: aEs) = aVas,
         r_vas(extracted: bEs),
         a,
         b,
         vars,
    ...
    aa_merge_continue(
      Map.put(a, v, r_vas(aVas, extracted: extracted)),
      b,
      vars,
      parentFixups,
      aliasFixups
  ...
  defp aa_merge_parent_fixups(sS0, [{child, parent} | fixups]) do
    ...
    sS = Map.put(sS0, child, r_vas(vas, parents: :ordsets.add_element(parent, parents)))
  ...
  defp aa_merge_ss_successor(blockLbl, newSS, lbl2SS) do
    %{^blockLbl => origSS} = lbl2SS
    Map.put(lbl2SS, blockLbl, aa_merge_ss_successor(origSS, newSS))
  end
  ...
  defp aa_set_status_1(r_b_var() = v, parent, state0) do
    %{^v => r_vas(child: child, extracted: extracted, parents: parents)} = state0
  ...
  defp aa_derive_from(r_b_var() = dst, r_b_var() = parent, state) do
    case state do
      ...
      %{^dst => r_vas(parents: parents) = childVas0, parent => r_vas(child: :none) = parentVas0} ->
        childVas =
          r_vas(childVas0,
            parents:
              :ordsets.add_element(
        ...
        Map.merge(state, %{dst => childVas, parent => parentVas})
    end
  end

  defp aa_update_annotations(
         funs,
         r_aas(alias_map: aliasMap0, st_map: stMap0) = aAS
       ) do
    foldl(
      fn f, {stMapAcc, aliasMapAcc} ->
        %{^f => lbl2SS0} = aliasMapAcc
        %{^f => optSt0} = stMapAcc

        {optSt, lbl2SS} =
          aa_update_fun_annotation(
            optSt0,
            lbl2SS0,
            r_aas(aAS, caller: f)
          )

        {Map.put(stMapAcc, f, optSt), Map.put(aliasMapAcc, f, lbl2SS)}
      end,
      {stMap0, aliasMap0},
      funs
    )
  ...
  defp aa_update_annotation(
    ...
    args = [{tuple, aa_get_element_extraction_status(tuple, sS)}, {idx, aa_get_status(idx, sS)}]
    aa_update_annotation1(args, i, aAS)
  end

  defp aa_update_annotation(i = r_b_set(args: [idx, tuple], op: {:bif, :element}), sS, aAS) do
    args = [
  ...
  defp aa_update_annotation(i = r_b_set(args: [pair], op: {:bif, :hd}), sS, aAS) do
    args = [{pair, aa_get_element_extraction_status(pair, sS)}]
    aa_update_annotation1(args, i, aAS)
  end

  defp aa_update_annotation(i = r_b_set(args: [pair], op: {:bif, :tl}), sS, aAS) do
    args = [{pair, aa_get_element_extraction_status(pair, sS)}]
    aa_update_annotation1(args, i, aAS)
  end
  ...
  defp aa_update_annotation1(argsStatus, i = r_b_set(anno: anno0, args: args, op: op), aAS) do
    {aliased, unique} =
      foldl(
        fn
          {r_b_var() = v, :aliased}, {as, us} ->
  ...
  defp aa_update_annotation1(status, i = r_b_ret(arg: r_b_var() = v, anno: anno0), _AAS) do
    anno =
      case status do
        :aliased ->
  ...
  defp aa_register_extracted(extracted, aggregate, state) do
    ...
    %{
      ...
      extracted => r_vas(parents: parents) = exVas0
    } = state
    ...
    Map.merge(state, %{aggregate => aggVas, extracted => exVas})
  end
  ...
  defp aa_construct_term(dst, values, sS, aAS) do
    case aa_all_vars_unique(
           values,
           sS
         ) and aa_all_dies(values, dst, aAS) do
      true ->
  ...
  defp aa_call(
         dst,
         [r_b_local() = callee | args],
         anno,
         sS0,
         r_aas(alias_map: aliasMap, st_map: stMap) = aAS0
       ) do
    ...
    case sS do
      ...
      %{^pair => r_vas(pair_elems: :none) = vas} ->
        aa_register_extracted(
          dst,
          pair,
          Map.put(sS, pair, r_vas(vas, pair_elems: {element, dst}))
        )
      ...
      when element === :hd ->
        aa_register_extracted(
          dst,
          pair,
          Map.put(sS, pair, r_vas(vas, pair_elems: {:both, dst, t}))
        )
      ...
      when element === :tl ->
        aa_register_extracted(
          dst,
          pair,
          Map.put(sS, pair, r_vas(vas, pair_elems: {:both, h, dst}))
        )
    end
  end
  ...
  defp aa_map_extraction(dst, map, sS, aAS) do
    aa_derive_from(dst, map, aa_alias_inherit_and_alias_if_arg_does_not_die(dst, map, sS, aAS))
  end
    ...
    case sS do
      ...
      %{^tuple => r_vas(tuple_elems: []) = tupleVas} ->
        aa_register_extracted(
          dst,
          tuple,
          Map.put(sS, tuple, r_vas(tupleVas, tuple_elems: [{i, dst}]))
        )
        ...
        case (for {idx, _} <- elems0, i === idx do
          ...
          [] ->
            ...
            aa_register_extracted(
              dst,
              tuple,
              Map.put(sS, tuple, r_vas(tupleVas, tuple_elems: elems))
            )
  ...
  defp aa_make_fun(
         dst,
         callee = r_b_local(name: r_b_literal()),
         env0,
         sS0,
         aAS0 = r_aas(call_args: info0, repeats: repeats0)
       ) do
      ...
      false ->
        case funcDb do
          %{^f => r_func_info(out: children)} ->
            [f | aa_breadth_first(work, children ++ next, :sets.add_element(f, seen), funcDb)]
