defmodule :m_beam_ssa_pre_codegen do
  use Bitwise
  import :lists, only: [all: 2, any: 2, append: 1,
                          duplicate: 2, foldl: 3, last: 1, member: 2,
                          partition: 2, reverse: 1, reverse: 2, seq: 2, sort: 1,
                          sort: 2, usort: 1, zip: 2]
  require Record
  ...
  Record.defrecord(:r_st, :st, ssa: :undefined,
                              args: :undefined, cnt: :undefined, frames: [],
                              intervals: [], res: [], regs: %{},
                              extra_annos: [], location: :undefined)
  defp passes(opts) do
    addPrecgAnnos = :proplists.get_bool(:dprecg, opts)
    ps = [{:assert_no_critical_edges,
             &assert_no_critical_edges/1},
              {:fix_bs, &fix_bs/1}, {:sanitize, &sanitize/1},
                                        {:expand_match_fail,
                                           &expand_match_fail/1},
                                            {:expand_update_tuple,
  ...
  defp function(r_b_function(anno: anno, args: args, bs: blocks0,
              cnt: count0) = f0,
            ps) do
    try do
      location = :maps.get(:location, anno, :none)
      st0 = r_st(ssa: blocks0, args: args, cnt: count0,
                location: location)
      st = :compile.run_sub_passes(ps, st0)
      r_st(ssa: blocks, cnt: count, regs: regs,
          extra_annos: extraAnnos) = st
  ...
  defp fix_bs(r_st(ssa: blocks, cnt: count0) = st) do
    ...
    case (map_size(ctxChain)) do
      ...
      _ ->
        ...
        {linear1, count} = bs_pos_bsm3(linear0, ctxChain,
                                         count0)
  ...
  defp bs_pos_bsm3(linear0, ctxChain, count0) do
    ...
    {gets, count2} = make_bs_getpos_map(rs, savePoints,
                                          count1, [])
    {sets, count} = make_bs_setpos_map(:maps.to_list(rs0),
                                         savePoints, count2, [])
    {bs_insert_bsm3(linear0, gets, sets), count}
  end

  defp make_bs_getpos_map([{ctx, save} = ps | t], savePoints, count,
            acc) do
    savePoint = get_savepoint(ps, savePoints)
    i = r_b_set(op: :bs_get_position, dst: savePoint, args: [ctx])
    make_bs_getpos_map(t, savePoints, count + 1,
                         [{save, i} | acc])
  end
  ...
  defp make_bs_setpos_map([{bef, {ctx, _} = ps} | t], savePoints, count,
            acc) do
    ...
    make_bs_setpos_map(t, savePoints, count + 1,
                         [{bef, i} | acc])
  end
  ...
  defp make_bs_pos_dict([{ctx, pts} | t], count0, acc0) do
    {acc, count} = make_bs_pos_dict_1(pts, ctx, count0,
                                        acc0)
  ...
  defp bs_restores([{l, r_b_blk(is: is, last: last)} | bs], ctxChain, d0,
            rs0) do
    inPos = :maps.get(l, d0, %{})
    {succPos, failPos, rs} = bs_restores_is(is, ctxChain,
                                              inPos, inPos, rs0)
  ...
  defp bs_restores_is([r_b_set(op: :bs_start_match, dst: start) | is],
            ctxChain, sPos0, _FPos, rs) do
    fPos = sPos0
    sPos = Map.put(sPos0, start, start)
    bs_restores_is(is, ctxChain, sPos, fPos, rs)
  end

  defp bs_restores_is([r_b_set(op: :bs_ensure, dst: newPos, args: args) |
               is],
            ctxChain, sPos0, _FPos, rs0) do
  ...
  defp bs_restores_is([r_b_set(anno: %{ensured: _}, op: :bs_match,
               dst: newPos, args: args) |
               is],
            ctxChain, sPos0, _FPos, rs) do
    start = bs_subst_ctx(newPos, ctxChain)
    case (args) do
      [r_b_literal(val: :skip), _FromPos, _Type, _Flags, r_b_literal(val: :all),
                                                   _] ->
        sPos = (fPos = sPos0)
        bs_restores_is(is, ctxChain, sPos, fPos, rs)
      [_, fromPos | _] ->
  ...
  defp bs_restores_is([r_b_set(op: :bs_match, dst: newPos, args: args) = i |
               is],
            ctxChain, sPos0, _FPos, rs0) do
    start = bs_subst_ctx(newPos, ctxChain)
    [_, fromPos | _] = args
  ...
  defp bs_restores_is([r_b_set(op: :bs_extract, args: [fromPos | _]) | is],
            ctxChain, sPos, _FPos, rs) do
  ...
  defp bs_restores_is([r_b_set(op: :call, dst: dst, args: args) | is],
            ctxChain, sPos0, _FPos, rs0) do
    {sPos1, rs} = bs_restore_args(args, sPos0, ctxChain,
                                    dst, rs0)
  ...
  defp bs_restores_is([r_b_set(op: op, dst: dst, args: args) | is],
            ctxChain, sPos0, _FPos, rs0)
      when op === :bs_test_tail or op === :bs_get_tail do
    {sPos, rs} = bs_restore_args(args, sPos0, ctxChain, dst,
                                   rs0)
    fPos = sPos
    bs_restores_is(is, ctxChain, sPos, fPos, rs)
  end

  defp bs_restores_is([r_b_set(op: {:succeeded, :guard}, args: [arg])],
            ctxChain, sPos, fPos0, rs) do
  ...
  defp bs_match_type(r_b_set(args: [r_b_literal(val: :skip), _Ctx, r_b_literal(val: :binary),
                                            _Flags, r_b_literal(val: :all),
                                                        r_b_literal(val: u)])) do
    case (u) do
      1 ->
        :none
      _ ->
  ...
  defp bs_insert_1([{l, r_b_blk(is: is0) = blk} | bs], deferred0, saves,
            restores) do
    is1 = bs_insert_deferred(is0, deferred0)
    {is, deferred} = bs_insert_is(is1, saves, restores, [])
    [{l, r_b_blk(blk, is: is)} | bs_insert_1(bs, deferred, saves,
                                         restores)]
  end
  ...
  defp bs_insert_is([r_b_set(dst: dst) = i | is], saves, restores,
            acc0) do
    pre = (case (restores) do
             %{^dst => r} ->
               [r]
             %{} ->
               []
  ...
  defp bs_rewrite_skip_is([r_b_set(anno: %{ensured: true}, op: :bs_skip) = i0,
  ...
  defp bs_instrs_is([r_b_set(op: {:succeeded, _}) = i | is], ctxChain,
            acc) do
    bs_instrs_is(is, ctxChain, [i | acc])
  end

  defp bs_instrs_is([r_b_set(anno: anno0, op: op, args: args0) = i0 | is],
            ctxChain, acc) do
    args = (for a <- args0 do
    ...
    i = (case ({op, args}) do
           {:bs_match, [r_b_literal(val: :skip), ctx, type | as]} ->
             ...
             r_b_set(i1, anno: anno,  op: :bs_skip, 
                     args: [type, ctx | as])
  ...
  defp bs_combine(dst, ctx, [{l, r_b_blk(is: is0) = blk} | acc]) do
    [r_b_set() = succeeded, r_b_set(anno: anno, op: :bs_match,
                          args: [type, _ | as]) = bsMatch |
                          is1] = reverse(is0)
    cond do
      :erlang.is_map_key(:ensured, anno) ->
        is = reverse(is1,
                       [r_b_set(bsMatch, op: :bs_checked_get,  dst: dst, 
                                     args: [type, ctx | as])])
      ...
      true ->
        is = reverse(is1,
                       [r_b_set(bsMatch, op: :bs_get,  dst: dst, 
                                     args: [type, ctx | as]),
    ...
    case (:erlang.is_map_key(l, blocks0)) do
      ...
      true ->
        r_b_blk(is: is0, last: last0) = (blk0 = :erlang.map_get(l,
                                                            inBlocks))
        case (sanitize_is(is0, last0, inBlocks, blocks0, count0,
                            values0, false, [])) do
          :no_change ->
  ...
  defp sanitize_reachable(blk, blocks) do
    foldl(fn s, a when :erlang.is_map_key(s, a) ->
               a
             s, a ->
               Map.put(a, s, :reachable)
          end,
            blocks, :beam_ssa.successors(blk))
  end

  defp sanitize_is([r_b_set(op: :get_map_element, args: args0) = i0 |
               is],
            last, inBlocks, blocks, count0, values, changed, acc) do
    case (sanitize_args(args0, values)) do
      [r_b_literal() = map, key] ->
        ...
        sanitize_is(is, last, inBlocks, blocks, count, values,
                      true, [i, copy | acc])
      [_, _] = ^args0 ->
        sanitize_is(is, last, inBlocks, blocks, count0, values,
                      changed, [i0 | acc])
      [_, _] = args ->
  ...
  defp sanitize_is([r_b_set(op: :call, dst: callDst) = call,
               r_b_set(op: {:succeeded, :body}, dst: succDst,
                   args: [callDst]) = succ],
            r_b_br(bool: succDst, succ: succLbl, fail: 1) = last0,
            inBlocks, blocks, count, values, changed, acc) do
    case (inBlocks) do
      %{^succLbl
        =>
        r_b_blk(is: [], last: r_b_ret(arg: ^callDst) = last)} ->
        do_sanitize_is(call, [], last, inBlocks, blocks, count,
                         values, true, acc)
      %{} ->
  ...
  defp sanitize_is([r_b_set(op: op, dst: dst) = fail, r_b_set(op: {:succeeded,
                                                :body},
                                           args: [dst])],
            r_b_br(fail: 1), inBlocks, blocks, count, values, _Changed,
            acc)
      when op === :match_fail or op === :resume do
    last = r_b_ret(arg: dst)
    do_sanitize_is(fail, [], last, inBlocks, blocks, count,
                     values, true, acc)
  end

  defp sanitize_is([r_b_set(op: :match_fail, dst: raiseDst),
               r_b_set(op: {:succeeded, :guard}, dst: succDst,
                   args: [raiseDst])],
            r_b_br(bool: succDst) = last0, inBlocks, blocks, count,
            values, _Changed, acc) do
    last = :beam_ssa.normalize(r_b_br(last0, bool: r_b_literal(val: false)))
    sanitize_is([], last, inBlocks, blocks, count, values,
                  true, acc)
  end

  defp sanitize_is([r_b_set(op: {:succeeded, _Kind}, dst: dst,
               args: [arg0]) = i0],
            r_b_br(bool: dst) = last, _InBlocks, _Blocks, count, values,
            _Changed, acc) do
  ...
  defp sanitize_is([r_b_set(op: {:succeeded, kind}, args: [arg0]) | is],
            last, inBlocks, blocks, count, values, _Changed, acc) do
    [] = is
    ...
    cond do
      same === 1 ->
        ...
        sanitize_is(is, r_b_ret(arg: arg), inBlocks, blocks, count,
                      values, true, acc)
      same !== 1 ->
  ...
  defp sanitize_is([r_b_set(op: :bs_test_tail) = i], last, inBlocks,
            blocks, count, values, changed, acc) do
    case (last) do
      r_b_br(succ: same, fail: same) ->
        sanitize_is([], last, inBlocks, blocks, count, values,
                      true, acc)
      _ ->
        do_sanitize_is(i, [], last, inBlocks, blocks, count,
                         values, changed, acc)
    end
  end

  defp sanitize_is([r_b_set(op: :bs_get, args: args0) = i0 | is], last,
            inBlocks, blocks, count, values, changed, acc) do
  ...
  defp sanitize_is([r_b_set() = i | is], last, inBlocks, blocks, count,
            values, changed, acc) do
    do_sanitize_is(i, is, last, inBlocks, blocks, count,
                     values, changed, acc)
  end

  defp sanitize_is([], last, _InBlocks, _Blocks, count, values,
            changed, acc) do
    case (changed) do
      true ->
  ...
  defp do_sanitize_is(r_b_set(op: op, dst: dst, args: args0) = i0, is, last,
            inBlocks, blocks, count, values, changed0, acc) do
    args = sanitize_args(args0, values)
    case (sanitize_instr(op, args, i0, blocks)) do
      {:subst, subst} ->
        sanitize_is(is, last, inBlocks, blocks, count,
                      Map.put(values, dst, subst), true, acc)
      {:ok, i} ->
        sanitize_is(is, last, inBlocks, blocks, count, values,
                      true, [i | acc])
      :ok ->
  ...
  defp sanitize_last(r_b_blk(last: last0) = blk, values) do
    last = (case (last0) do
              r_b_br(bool: r_b_literal()) ->
                last0
              r_b_br(bool: bool) ->
                :beam_ssa.normalize(r_b_br(last0, bool: sanitize_arg(bool,
                                                                  values)))
              r_b_ret(arg: arg) ->
                r_b_ret(last0, arg: sanitize_arg(arg, values))
              r_b_switch(arg: arg) ->
                :beam_ssa.normalize(r_b_switch(last0, arg: sanitize_arg(arg,
                                                                 values)))
            end)
    cond do
      last !== last0 ->
  ...
  defp sanitize_instr({:bif, bif}, [r_b_literal(val: lit1), r_b_literal(val: lit2)],
            _I) do
  ...
  defp sanitize_instr(:is_tagged_tuple,
            [r_b_literal(val: tuple), r_b_literal(val: arity), r_b_literal(val: tag)], _I)
  ...
  defp expand_match_fail(r_st(ssa: blocks0, cnt: count0, args: args,
              location: location) = st) do
    bs = :maps.to_list(blocks0)
    {blocks, count} = expand_mf_bs(bs, length(args),
                                     location, blocks0, count0)
    r_st(st, ssa: blocks,  cnt: count)
  end

  defp expand_mf_bs([{l, r_b_blk(is: is0) = blk} | bs], arity, location,
            blocks0, count0) do
  ...
  defp expand_mf_is([r_b_set(op: :match_fail, anno: anno,
               args: [r_b_literal(val: :function_clause) | args]) = i0 |
               is],
            arity, location, count0, acc) do
    case (anno) do
  ...
  defp expand_mf_is([r_b_set(op: :match_fail) = i | is], _Arity,
            _Location, count, acc) do
  ...
  defp expand_mf_instr(r_b_set(args: [r_b_literal(val: :case_clause) | args]) = i0, is,
            count, acc) do
    i = r_b_set(i0, args: [r_b_literal(val: :case_end) | args])
    {reverse(acc, [i | is]), count}
  end

  defp expand_mf_instr(r_b_set(args: [r_b_literal(val: :if_clause) | args]) = i0, is,
            count, acc) do
    i = r_b_set(i0, args: [r_b_literal(val: :if_end) | args])
    {reverse(acc, [i | is]), count}
  end

  defp expand_mf_instr(r_b_set(args: [r_b_literal(val: :try_clause) | args]) = i0, is,
            count, acc) do
    i = r_b_set(i0, args: [r_b_literal(val: :try_case_end) | args])
    {reverse(acc, [i | is]), count}
  end

  defp expand_mf_instr(r_b_set(args: [r_b_literal(val: :badmatch) | _Args]) = i, is,
            count, acc) do
    {reverse(acc, [i | is]), count}
  end

  defp expand_mf_instr(r_b_set(args: [r_b_literal(val: :badrecord) | _Args]) = i, is,
            count, acc) do
    {reverse(acc, [i | is]), count}
  end

  defp expand_mf_instr(r_b_set(args: [r_b_literal() | _] = args) = i0, is, count0,
            acc) do
    {tuple, count} = new_var(:"@match_fail", count0)
    put = r_b_set(op: :put_tuple, dst: tuple, args: args)
    call = r_b_set(i0, op: :call, 
  ...
  defp create_fc_stubs(fs, r_b_module(name: mod)) do
    ...
    stubs = (for {{name, arity}, location} <- stubs0 do
               (
                 seq = seq(0, arity - 1)
                 args = (for v <- seq do
                           r_b_var(name: v)
                 ...
                 regs = :maps.from_list([{ret, {:x, 0}} | zip(args,
                                                                xRegs)])
                 anno = %{func_info: {mod, name, arity},
                            location: location, parameter_info: %{},
                            registers: regs}
                 fc = r_b_set(op: :match_fail, dst: ret,
                          args: [r_b_literal(val: :function_clause) | args])
  ...
  defp find_fc_errors([r_b_function(bs: blocks) | fs], acc0) do
    f = fn r_b_set(anno: anno, op: :call, args: [r_b_local() | _]), a ->
             case (anno) do
               %{inlined: fA} ->
    ...
    acc = :beam_ssa.fold_instrs(f, :maps.keys(blocks), acc0,
                                  blocks)
  ...
  defp expand_update_tuple_1([{l, r_b_blk(is: is0) = b0} | bs], count0, acc0) do
    case (expand_update_tuple_is(is0, count0, [])) do
      {is, count} ->
        expand_update_tuple_1(bs, count,
                                [{l, r_b_blk(b0, is: is)} | acc0])
      {is, nextIs, count1} ->
        r_b_blk(last: br) = b0
        r_b_br(succ: succ) = br
        nextL = count1
  ...
  defp expand_update_tuple_is([r_b_set(op: :update_tuple, args: [src | args]) = i0 |
               is],
            count0, acc) do
    {setElement, sets,
       count} = expand_update_tuple_list(args, i0, src, count0)
    case ({sets, is}) do
      {[_ | _], [r_b_set(op: :succeeded)]} ->
        {reverse(acc, [setElement | is]), reverse(sets), count}
      {_, _} ->
        expand_update_tuple_is(is, count,
                                 sets ++ [setElement | acc])
    end
  end
  ...
  defp expand_update_tuple_list(args, i0, src, count0) do
    ...
    i = r_b_set(i0, op: :call, 
                args: [r_b_remote(mod: r_b_literal(val: :erlang),
                           name: r_b_literal(val: :setelement), arity: 3),
                           index, src, value])
    {sets, count} = expand_update_tuple_list_1(rest,
                                                 r_b_set(i, :dst), count0, [])
    {i, sets, count}
  end
  ...
  defp expand_update_tuple_list_1([index0, value | updates], src, count0, acc) do
    ...
    setOp = r_b_set(op: :set_tuple_element, dst: dst,
                args: [value, src, index])
    expand_update_tuple_list_1(updates, src, count,
                                 [setOp | acc])
  end
  ...
  defp sort_update_tuple([], acc) do
    append(for {index,
                  value} <- sort(&:erlang.unquote(:">=")/2, acc) do
             [index, value]
           end)
  end
  ...
  defp place_frames(r_st(ssa: blocks) = st) do
    ...
    tried = :gb_sets.empty()
    frames0 = []
    {frames, _} = place_frames_1(ls, blocks, doms, tried,
                                   frames0)
  ...
  defp need_frame_1([r_b_set(op: :old_make_fun, dst: fun) | is],
            {:return, ret} = context) do
    case (need_frame_1(is, context)) do
      true ->
        true
      false ->
        defs = :ordsets.from_list(for r_b_set(dst: dst) <- is do
                                    dst
                                  end)
  ...
  defp need_frame_1([r_b_set(op: :call, dst: val)] = is,
            {:return, ret}) do
    cond do
      val === ret ->
  ...
  defp fix_receives_1([{l, blk} | ls], blocks0, count0) do
    case (blk) do
      r_b_blk(is: [r_b_set(op: :peek_message) | _]) ->
        ...
        {rm, blocks1, count1} = split_rm_blocks(rm0, blocks0,
                                                  count0, [])
        ...
        {blocks2, count2} = recv_crit_edges(rm, loopExit,
                                              blocks1, count1)
        {blocks3, count3} = recv_fix_common(commonUsed,
                                              loopExit, rm, blocks2, count2)
  ...
  defp split_rm_blocks([l | ls], blocks0, count0, acc) do
    ...
    case (need_split(is)) do
      ...
      true ->
        ...
        next = count0
        {blocks, count} = :beam_ssa.split_blocks([l], p,
                                                   blocks0, count0)
        true = count0 !== count
  ...
  defp recv_common(defs, exit, blocks) do
    ...
    {exitDefs, exitUnused} = :beam_ssa.def_unused(rPO, defs,
                                                    blocks)
  ...
  defp rce_insert_edges([l | ls], exit, count0, blocks0) do
    successors = :beam_ssa.successors(:erlang.map_get(l,
                                                        blocks0))
    case (member(exit, successors)) do
      true when successors !== [exit] ->
        {blocks, count} = rce_insert_edge(l, exit, count0,
                                            blocks0)
  ...
  defp rce_insert_edge(l, exit, count, blocks0) do
    r_b_blk(last: last0) = (fromBlk0 = :erlang.map_get(l,
                                                   blocks0))
    toExit = r_b_br(bool: r_b_literal(val: true), succ: exit, fail: exit)
    fromBlk = r_b_blk(fromBlk0, last: rce_reroute_terminator(last0,
                                                         exit, count))
    edgeBlk = r_b_blk(anno: %{}, is: [], last: toExit)
    blocks = Map.merge(blocks0, %{count => edgeBlk,
                                    l => fromBlk})
    {blocks, count + 1}
  end
  ...
  defp recv_fix_common([msg0 | t], exit, rm, blocks0, count0) do
    ...
    blocks1 = :beam_ssa.rename_vars(%{msg0 => msg}, rPO,
                                      blocks0)
    n = length(rm)
  ...
  defp fix_receive([l | ls], defs, blocks0, count0) do
    ...
    {rmDefs, unused} = :beam_ssa.def_unused(rPO, defs,
                                              blocks0)
    def__ = :ordsets.subtract(defs, rmDefs)
    used = :ordsets.subtract(def__, unused)
    {newVars, count} = new_vars(for r_b_var(name: base) <- used do
                                  base
                                end,
                                  count0)
  ...
  defp find_yregs(r_st(frames: [_ | _] = frames, args: args,
              ssa: blocks0) = st) do
    frameDefs = find_defs(frames, blocks0,
  ...
  defp find_defs(frames, blocks, defs) do
    ...
    {frameDefs, _} = find_defs_1([0], blocks, framesSet,
                                   seen, defs, [])
    frameDefs
  end
      ...
      false ->
        case (:gb_sets.is_member(l, seen0)) do
          ...
          false ->
            ...
            {acc, seen} = find_defs_1(ls, blocks, frames, seen1,
                                        defs0, acc0)
  ...
  defp find_yregs_is([r_b_set(dst: dst) = i | is],
            r_dk(d: defs0, k: killed0) = ys, yregs0) do
    ...
    case (:beam_ssa.clobbers_xregs(i)) do
      ...
      true ->
        killed = :sets.union(:sets.from_list(defs0,
                                               [{:version, 2}]),
                               killed0)
        defs = [dst]
  ...
  defp intersect_used(r_b_set(op: :phi, args: args), set) do
    :sets.from_list(for {r_b_var() = v, _} <- args,
  ...
  defp intersect_used(r_b_set(args: args), set) do
    :sets.from_list(intersect_used_keep(used_args(args),
                                          set),
                      [{:version, 2}])
  end
  ...
  defp copy_retval(r_st(frames: frames, ssa: blocks0,
              cnt: count0) = st) do
  ...
  defp copy_retval_1([f | fs], blocks0, count0) do
    r_b_blk(anno: %{yregs: yregs0}, is: is) = :erlang.map_get(f,
                                                          blocks0)
    yregs = collect_yregs(is, yregs0)
    ls = :beam_ssa.rpo([f], blocks0)
    {blocks, count} = copy_retval_2(ls, yregs, :none,
                                      blocks0, count0)
  ...
  defp collect_yregs([r_b_set(op: :copy, dst: y, args: [r_b_var() = x]) | is],
            yregs0) do
    true = :sets.is_element(x, yregs0)
    yregs = :sets.add_element(y,
  ...
  defp copy_retval_2([l | ls], yregs, copy0, blocks0, count0) do
    r_b_blk(is: is0, last: last) = (blk = :erlang.map_get(l,
                                                      blocks0))
    rC = (case ({last, ls}) do
    ...
    case (copy_retval_is(is0, rC, yregs, copy0, count0,
                           [])) do
      {is, count} ->
  ...
  defp copy_retval_is([r_b_set(op: op) = i0], false, yregs, copy, count0,
            acc0)
      when op === :call or op === :old_make_fun do
    {i, count, acc} = place_retval_copy(i0, yregs, copy,
                                          count0, acc0)
    {reverse(acc, [i]), count}
  end
  ...
  defp copy_retval_is([r_b_set(), r_b_set(op: :succeeded)] = is, false, _Yregs,
            copy, count, acc) do
    {reverse(acc, acc_copy(is, copy)), count}
  end

  defp copy_retval_is([r_b_set(op: op, dst: r_b_var(name: retName) = dst) = i0 |
               is],
            rC, yregs, copy0, count0, acc0)
      when op === :call or op === :old_make_fun do
    {i1, count1, acc} = place_retval_copy(i0, yregs, copy0,
                                            count0, acc0)
  ...
  defp copy_retval_is([r_b_set(args: args0) = i0 | is], rC, yregs, copy,
            count, acc) do
    i = r_b_set(i0, args: copy_sub_args(args0, copy))
    case (:beam_ssa.clobbers_xregs(i)) do
      true ->
        copy_retval_is(is, rC, yregs, :none, count,
                         [i | acc_copy(acc, copy)])
      false ->
        copy_retval_is(is, rC, yregs, copy, count, [i | acc])
    end
  ...
  defp place_retval_copy(r_b_set(args: [f | args0]) = i0, yregs0, retCopy,
            count0, acc0) do
    r_b_set(dst: avoid) = retCopy
    yregs = :sets.del_element(avoid, yregs0)
    {args, acc1, count} = copy_func_args(args0, yregs, acc0,
                                           count0)
  ...
  defp copy_func_args_1([r_b_var(name: aName) = a | as], yregs, instrAcc,
            argAcc, count0) do
    case (:sets.is_element(a, yregs)) do
      true ->
        {newVar, count} = new_var(aName, count0)
        copy = r_b_set(op: :copy, dst: newVar, args: [a])
        copy_func_args_1(as, yregs, [copy | instrAcc],
                           [newVar | argAcc], count)
      false ->
        copy_func_args_1(as, yregs, instrAcc, [a | argAcc],
                           count0)
    end
  end
  ...
  defp opt_get_list_is([r_b_set(op: :get_hd, dst: hd, args: [cons]) = getHd,
               r_b_set(op: :get_tl, dst: tl, args: [cons]) = getTl | is],
            res, acc, changed) do
    case (res) do
      %{^hd => {:y, _}} ->
        opt_get_list_is([getTl | is], res, [getHd | acc],
                          changed)
      %{^tl => {:y, _}} ->
  ...
  defp number_is_1([l | ls], n0, blocks0) do
    r_b_blk(is: is0, last: last0) = (bl0 = :erlang.map_get(l,
                                                       blocks0))
  ...
  defp live_intervals(r_st(args: args, ssa: blocks) = st) do
    ...
    intervals1 = add_ranges(for (r_b_var() = v) <- args do
                              {v, {0, 1}}
                            end,
                              intervals0)
  ...
  defp live_interval_blk([l | ls], blocks, liveMap0, intervals0) do
    blk = :erlang.map_get(l, blocks)
    successors = :beam_ssa.successors(blk)
    live1 = live_in_successors(successors, l, blocks,
                                 liveMap0)
    r_b_blk(is: is, last: last) = blk
    firstNumber = first_number(is, last)
    defaultRange = {firstNumber,
                      1 + :beam_ssa.get_anno(:n, last)}
    ranges0 = (for v <- live1 do
                 {v, defaultRange}
               end)
    case ({is, last}) do
  ...
  defp live_interval_blk_is([r_b_set(op: :phi, dst: dst) | is], firstNumber,
            acc0) do
  ...
  defp live_interval_blk_is([r_b_set(args: args, dst: dst) = i | is], firstNumber,
            acc0) do
  ...
  defp update_used([v | vs], first, n, ranges) do
    case (:gb_trees.is_defined(v, ranges)) do
      ...
      false ->
        update_used(vs, first, n,
                      :gb_trees.insert(v, {first, n}, ranges))
    end
  end
  ...
  defp live_in_phis([r_b_set(op: :phi, dst: killed, args: args) | is],
            pred, live0) do
    used = (for {r_b_var() = v, l} <- args, l === pred do
              v
            end)
  ...
  defp reserve_yregs_1(l,
            r_st(ssa: blocks0, cnt: count0, res: res0) = st) do
    blk = :erlang.map_get(l, blocks0)
    yregs = :ordsets.from_list(:sets.to_list(:beam_ssa.get_anno(:yregs,
                                                                  blk)))
    rPO = :beam_ssa.rpo([l], blocks0)
    {def__, unused} = :beam_ssa.def_unused(rPO, yregs,
                                             blocks0)
    usedYregs = :ordsets.subtract(yregs, unused)
    defBefore = :ordsets.subtract(usedYregs, def__)
    {beforeVars, blocks, count} = rename_vars(defBefore, l,
                                                rPO, blocks0, count0)
    insideVars = :ordsets.subtract(usedYregs, defBefore)
    resTryTags0 = reserve_try_tags(l, blocks)
    resTryTags = (for {v, reg} <- resTryTags0 do
                    {v, {reg, count}}
                  end)
  ...
  defp reserve_try_tags(l, blocks) do
    ...
    res1 = (for m <- :maps.values(res0) do
              :maps.to_list(m)
            end)
    res = (for {v, y} <- append(res1) do
             {v, {:y, y}}
           end)
  ...
  defp reserve_try_tags_1([l | ls], blocks, seen0, actMap0) do
    case (:gb_sets.is_element(l, seen0)) do
      ...
      false ->
        ...
        {actMap, seen} = reserve_try_tags_1(ls, blocks, seen1,
                                              actMap1)
  ...
  defp reserve_try_tags_is([r_b_set(op: :kill_try_tag, args: [tag]) | is],
            active) do
  ...
  defp update_act_map([l | ls], active0, actMap0) do
    case (actMap0) do
      %{^l => active1} ->
        actMap = Map.put(actMap0, l,
                                    :maps.merge(active0, active1))
  ...
  defp rename_vars(vs, l, rPO, blocks0, count0) do
    {newVars, count} = new_vars(for r_b_var(name: base) <- vs do
                                  base
                                end,
                                  count0)
    ren = zip(vs, newVars)
  ...
  defp frame_size(r_st(frames: frames, regs: regs,
              ssa: blocks0) = st) do
    blocks = foldl(fn l, blks ->
  ...
  defp turn_yregs(r_st(frames: frames, regs: regs0,
              ssa: blocks) = st) do
    regs1 = foldl(fn l, a ->
                       blk = :erlang.map_get(l, blocks)
  ...
  defp reserve_regs(r_st(args: args, ssa: blocks, intervals: intervals,
              res: res0) = st) do
  ...
  defp reserve_zregs(rPO, blocks, intervals, res) do
    shortLived0 = (for {v, [{start, end__}]} <- intervals,
                         start + 2 === end__ do
                     v
                   end)
  ...
  defp reserve_zreg([r_b_set(op: {:bif, :tuple_size}, dst: dst),
               r_b_set(op: {:bif, :"=:="}, args: [dst, val], dst: bool)],
            last, shortLived, a) do
    case ({val, last}) do
  ...
  defp reserve_zreg([r_b_set(op: {:bif, :tuple_size}, dst: dst)],
            r_b_switch(arg: dst), shortLived, a) do
    reserve_test_zreg(dst, shortLived, a)
  end

  defp reserve_zreg([r_b_set(op: op, dst: dst)], r_b_br(bool: dst), shortLived,
            a) do
    case (use_zreg(op)) do
      :yes ->
        [{dst, :z} | a]
  ...
  defp reserve_zreg([r_b_set(op: op, dst: dst) | is], last, shortLived,
            a) do
    case (use_zreg(op)) do
      :yes ->
  ...
  defp reserve_xregs([l | ls], blocks, xsMap0, res0) do
    r_b_blk(anno: anno, is: is0, last: last) = :erlang.map_get(l,
                                                           blocks)
    xs0 = reserve_terminator(l, is0, last, blocks, xsMap0,
                               res0)
    is1 = reverse(is0)
  ...
  defp res_place_gc_instrs([r_b_set(op: op) = i | is], acc) when op === :call or
                                            op === :old_make_fun do
    case (acc) do
      [] ->
        res_place_gc_instrs(is, [i | acc])
  ...
  defp reserve_xregs_is([r_b_set(op: op, dst: dst, args: args) = i | is],
            res0, xs0, used0) do
  ...
  defp reserve_terminator(l, is, r_b_br(bool: r_b_var(), succ: succ, fail: fail),
            blocks, xsMap, res)
      ...
      {_, _} when is !== [] ->
        case (last(is)) do
          r_b_set(op: :succeeded, args: [arg]) ->
            ...
            case (reserve_terminator(l, [], br, blocks, xsMap,
                                       res)) do
              %{^arg => reg} ->
                %{arg => reg}
              %{} ->
  ...
  defp reserve_terminator(l, is, r_b_br(bool: bool, succ: succ, fail: fail),
            blocks, xsMap, res) do
    case ({bool, fail}) do
      {_, 1} ->
        reserve_terminator_1(l, succ, is, blocks, xsMap, res)
  ...
  defp res_xregs_from_phi([r_b_set(op: :phi, dst: dst, args: args) | is], pred,
            res, acc) do
  ...
  Record.defrecord(:r_i, :i, sort: 1, reg: :none, pool: :x,
                             var: :EFE_TODO_NESTED_RECORD, rs: [])
  Record.defrecord(:r_l, :l, cur: :EFE_TODO_NESTED_RECORD,
                             unhandled_res: [], unhandled_any: [], active: [],
                             inactive: [], free: %{}, regs: [])
  defp linear_scan(r_st(intervals: intervals0, res: res) = st0) do
    st = r_st(st0, intervals: [],  res: [])
    free = init_free(:maps.to_list(res))
    intervals1 = (for int <- intervals0 do
    ...
    {unhandledRes, unhandled} = partition(isReserved,
                                            intervals)
    l = r_l(unhandled_res: unhandledRes,
  ...
  defp set_next_current(r_l(unhandled_res: [cur1 | t1],
              unhandled_any: [cur2 | t2]) = l) do
    case ({cur1, cur2}) do
  ...
  defp set_next_current(r_l(unhandled_res: [],
              unhandled_any: [cur | t]) = l) do
    r_l(l, cur: cur,  unhandled_any: t)
  end

  defp set_next_current(r_l(unhandled_res: [cur | t],
              unhandled_any: []) = l) do
  ...
  defp expire_active([r_i(reg: reg, rs: rs0) = i | is], curBegin, l0,
            acc) do
    {_, _} = reg
  ...
  defp check_inactive(r_l(cur: r_i(sort: curBegin),
              inactive: inAct0) = l0) do
  ...
  defp collect_available(r_l(cur: r_i(reg: {:prefer,
                           {_, _} = prefer}) = i) = l) do
  ...
  defp collect_available([r_i(pool: pool1) | is], r_i(pool: pool2) = cur,
            free)
      when pool1 !== pool2 do
  ...
  defp select_register([{_, _} = reg | _],
            r_l(cur: cur0, regs: regs) = l) do
