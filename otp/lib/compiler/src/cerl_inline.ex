defmodule :m_cerl_inline do
  ...
  defp start(reply, tree, ctxt, opts) do
    ...
    case :proplists.get_bool(:verbose, opts) do
      true ->
        :io.fwrite(~c"Inlining: inline_size=~w inline_effort=~w\n", [size, effort])
  ...
  defp i_case_1(as, e, ctxt, ren, env, s) do
    case i_clauses(as, case_clauses(e), ctxt, ren, env, s) do
      {false, {as1, vs, env1, cs}, s1} ->
        cond do
          cs === [] ->
            report_warning(~c"empty list of clauses in residual program!.\n")
      ...
      mapfoldl(
        fn x, s ->
          ...
          case env__lookup(name, env1) do
            ...
            :error ->
              {n, a} = var_name(x)
              report_warning(~c"export `~w'/~w not defined.\n", [n, a])
              {x, s}
          end
        end,
        s2,
  ...
  defp i_module(e, ctxt, ren, env, s) do
    ...
    case es do
      [] ->
        report_warning(~c"no function definitions remaining in module `~s'.\n", [
  ...
  defp i_params([v | vs], ren, env) do
    ...
    case env__lookup(name, env) do
      ...
      :error ->
        report_internal_error(~c"variable `~w' not bound in pattern.\n", [name])
        exit(:error)
    end
  end
  ...
  defp env__new_tname(env) do
    :rec_env.new_key(
      fn i ->
        :erlang.list_to_atom(~c"@i" ++ :erlang.integer_to_list(i))
      end,
      env
    )
  end
  ...
  defp env__new_fname(a, n, env) do
    :rec_env.new_key(
      fn x ->
        ...
        {:erlang.list_to_atom(:erlang.atom_to_list(a) ++ ~c"_" ++ s), n}
      end,
      env
    )
  end
  ...
  defp report_internal_error(s, vs) do
    report_error(~c"internal error: " ++ s, vs)
  end
