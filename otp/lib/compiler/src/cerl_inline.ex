defmodule :m_cerl_inline do
  use Bitwise
  import :cerl, only: [abstract: 1, alias_pat: 1,
                         alias_var: 1, ann_c_let: 4, apply_args: 1, apply_op: 1,
                         atom_name: 1, atom_val: 1, binary_segments: 1,
                         bitstr_flags: 1, bitstr_size: 1, bitstr_type: 1,
                         bitstr_unit: 1, bitstr_val: 1, c_fun: 2, c_int: 1,
                         c_let: 3, c_seq: 2, c_tuple: 1, c_values: 1, c_var: 1,
                         call_args: 1, call_module: 1, call_name: 1,
                         case_arg: 1, case_arity: 1, case_clauses: 1,
                         catch_body: 1, clause_body: 1, clause_guard: 1,
                         clause_pats: 1, clause_vars: 1, concrete: 1,
                         cons_hd: 1, cons_tl: 1, data_arity: 1, data_es: 1,
                         data_type: 1, fname_arity: 1, fun_body: 1, fun_vars: 1,
                         get_ann: 1, int_val: 1, is_c_atom: 1, is_c_cons: 1,
                         is_c_fname: 1, is_c_int: 1, is_c_list: 1, is_c_seq: 1,
                         is_c_tuple: 1, is_c_var: 1, is_data: 1, is_literal: 1,
                         is_literal_term: 1, let_arg: 1, let_body: 1,
                         let_vars: 1, letrec_body: 1, letrec_defs: 1,
                         list_elements: 1, list_length: 1, make_data_skel: 2,
                         make_list: 1, map_arg: 1, map_es: 1, map_pair_key: 1,
                         map_pair_op: 1, map_pair_val: 1, module_attrs: 1,
                         ...
                         try_evars: 1, try_handler: 1, try_vars: 1,
                         tuple_arity: 1, tuple_es: 1, type: 1,
                         update_c_alias: 3, update_c_apply: 3,
                         update_c_binary: 2, update_c_bitstr: 6,
                         update_c_call: 4, update_c_case: 3, update_c_catch: 2,
                         update_c_clause: 4, update_c_let: 4,
                         update_c_letrec: 3, update_c_map: 3,
                         update_c_map_pair: 4, update_c_module: 5,
                         update_c_primop: 3, update_c_receive: 4,
                         update_c_seq: 3, update_c_try: 6, update_c_values: 2,
                         update_data: 3, values_es: 1, var_name: 1]
  import :lists, only: [foldl: 3, foldr: 3, mapfoldl: 3,
                          member: 2, reverse: 1]
  ...
  require Record
  Record.defrecord(:r_ref, :ref, name: :undefined,
                               opnd: :undefined, loc: :undefined)
  Record.defrecord(:r_opnd, :opnd, expr: :undefined,
                                ren: :undefined, env: :undefined,
                                loc: :undefined, effort: :undefined,
                                no_inline: :undefined)
  Record.defrecord(:r_cache, :cache, expr: :undefined,
                                 size: :undefined)
  Record.defrecord(:r_app, :app, opnds: :undefined,
                               ctxt: :undefined, loc: :undefined)
  def core_transform(code, opts) do
    :cerl.to_records(transform(:cerl.from_records(code),
                                 opts))
  end
  ...
  defp main(tree, ctxt, opts) do
    opts1 = opts ++ [{:inline_size, default_size()},
                         {:inline_effort, default_effort()}, {:inline_unroll,
                                                                default_unroll()}]
    reply = self()
    pid = spawn_link(fn () ->
  ...
  defp i_let_1(v, e, ctxt, ren, env, s) do
    ...
    {[r], ren1, env1, s2} = bind_locals([v], [opnd], ren,
                                          env, s1)
  ...
  defp i_let_2(vs, as, e, ctxt, ren, env, s) do
    {opnds, s1} = mapfoldl(fn e, s ->
                                make_opnd(e, ren__identity(), env, s)
                           end,
                             s, as)
    {rs, ren1, env1, s2} = bind_locals(vs, opnds, ren, env,
                                         s1)
    {e1, s3} = i(let_body(e), ctxt, ren1, env1, s2)
  ...
  defp i_case(e, ctxt, ren, env, s) do
    ...
    {e1, s2} = (case (get_components(case_arity(e), a1)) do
                  {true, as} ->
  ...
  defp i_case_1(as, e, ctxt, ren, env, s) do
    case (i_clauses(as, case_clauses(e), ctxt, ren, env,
                      s)) do
      {false, {as1, vs, env1, cs}, s1} ->
        ...
        {a, s2} = i(c_values(as1), :value, ren__identity(),
                      env1, s1)
  ...
  defp i_clauses(es, cs, ctxt, ren, env, s) do
    {ts, {vs, env0}} = mapfoldl(fn e, {vs, env} ->
                                     {t, vs1, env1} = make_template(e, env)
                                     {t, {vs1 ++ vs, env1}}
                                end,
                                  {[], env}, es)
    vs1 = :lists.reverse(vs)
    {ren1, env1, s1} = foldl(fn v, {ren, env, s} ->
                                  e = env__get(var_name(v), env0)
                                  {opnd, s_1} = make_opnd(e, ren__identity(),
                                                            env, s)
                                  {_, ren1, env1, s_2} = bind_locals([v],
                                                                       [opnd],
                                                                       ren, env,
                                                                       s_1)
                                  {ren1, env1, s_2}
    ...
    case (:cerl_clauses.reduce(cs1, ts)) do
      ...
      {true, {c, _}} ->
        ...
        c2 = update_c_clause(c1, clause_pats(c1),
                               clause_guard(c1), b)
  ...
  defp i_clause_head(c, ts, ren, env, s) do
    ps = clause_pats(c)
    bs = (case (:cerl_clauses.match_list(ps, ts)) do
            {_, bs1} ->
              bs1
            :none ->
              []
          end)
    {_, ren1, env1, s1} = bind_locals(clause_vars(c), ren,
                                        env, s)
    s2 = new_passive_size(get_size_limit(s1), s1)
    {ps1, s3} = mapfoldl(fn p, s ->
    ...
    {set_clause_extras(c1, ren1, env1, size),
       revert_size(s, s5)}
  end
  ...
  defp add_match_bindings(bs, e) do
    case (is_literal(e)) do
      true ->
        e
      false ->
        vs = (for {v, ^e} <- bs, e !== :any do
                v
              end)
        es = (for {_V, ^e} <- bs, e !== :any do
                hd(get_ann(e))
              end)
  ...
  defp i_clause_body(c0, ctxt, s) do
    ...
    c1 = update_c_clause(c, clause_pats(c), clause_guard(c),
                           b)
    {c1, s2}
  end
  ...
  defp i_letrec(e, ctxt, ren, env, s) do
    ...
    {es, b, _, s1} = i_letrec(letrec_defs(e),
                                letrec_body(e), [], ctxt, ren, env, noInline, s)
    case (es) do
      [] ->
        {b, s1}
      _ ->
  ...
  defp i_letrec(es, b, xs, ctxt, ren, env, noInline, s) do
    {opnds, s1} = mapfoldl(fn {_, e}, s ->
                                make_opnd(e, :undefined, :undefined, noInline,
                                            s)
                           end,
                             s, es)
    {rs, ren1, env1, s2} = bind_recursive(for {f,
                                                 _} <- es do
                                            f
                                          end,
                                            opnds, ren, env, s1)
    {xs1, s3} = mapfoldl(fn x, s ->
  ...
  defp i_apply(e, ctxt, ren, env, s) do
    ...
    case (st__get_app_inlined(l, s3)) do
      ...
      false ->
        {es, s4} = mapfoldl(fn opnd, s ->
                                 visit_and_count_size(opnd, s)
                            end,
                              s3, opnds)
        arity = length(es)
        e2 = (case (is_c_fname(e1) and length(es) !== fname_arity(e1)) do
                true ->
                  v = new_var(env)
  ...
  defp i_call_1(static, m, f, arity, as, e, ctxt, ren, env,
            s) do
    {as1, s1} = mapfoldl(fn x, a ->
                              i(x, :value, ren, env, a)
                         end,
                           s, as)
    case (static) do
      true ->
        case (:erl_bifs.is_pure(atom_val(m), atom_val(f),
                                  arity)) do
          true ->
  ...
  defp i_call_4(m, f, as, e, ctxt, env, s) do
    case (reduce_bif_call(atom_val(m), atom_val(f), as,
                            env)) do
      false ->
        i_call_2(m, f, as, e, s)
      {true, e1} ->
  ...
  defp i_primop(e, ren, env, s) do
    {as, s1} = mapfoldl(fn e, s ->
                             i(e, :value, ren, env, s)
                        end,
                          s, primop_args(e))
    n = weight(:primop) + weight(:argument) * length(as)
    {update_c_primop(e, primop_name(e), as),
       count_size(n, s1)}
  end
  ...
  defp i_catch(e, ctxt, ren, env, s) do
    {e1, s1} = (eS1 = i(catch_body(e), safe_context(ctxt),
                          ren, env, s))
    case (is_safe(e1)) do
      true ->
        eS1
  ...
  defp i_receive(e, ctxt, ren, env, s) do
    ...
    ctxt1 = safe_context(ctxt)
    case (i_clauses(receive_clauses(e), ctxt1, ren, env,
                      s2)) do
      {false, {[], _, _, cs}, s3} ->
        cond do
          cs === [] ->
  ...
  defp i_module(e, ctxt, ren, env, s) do
    ...
    e1 = update_c_module(e, module_name(e), xs1,
                           module_attrs(e), es)
    ...
    case (type(e)) do
      ...
      :alias ->
        ...
        case (env__lookup(name, env)) do
          {:ok, r} ->
            v1 = ref_to_var(r)
            {p, s1} = i_pattern(alias_pat(e), ren, env, ren0, env0,
                                  s)
  ...
  defp i_bitstr_pattern(e, ren, env, ren0, env0, s) do
    {val, s1} = i_pattern(bitstr_val(e), ren, env, ren0,
                            env0, s)
  ...
  defp i_map_pair_pattern(e, ren, env, ren0, env0, s) do
    ...
    {val, s2} = i_pattern(map_pair_val(e), ren, env, ren0,
                            env0, s1)
    op = map_pair_op(e)
    s3 = count_size(weight(:map_pair), s2)
    {update_c_map_pair(e, op, key, val), s3}
  ...
  defp inline(e, r_app(opnds: opnds, ctxt: ctxt, loc: l), ren,
            env, s) do
    vs = fun_vars(e)
    cond do
      length(opnds) !== length(vs) ->
        {e, s}
      true ->
        {rs, ren1, env1, s1} = bind_locals(vs, opnds, ren, env,
                                             s)
  ...
  defp copy(r, opnd, e, ctxt, env, s) do
    case (is_c_var(e) and not is_c_fname(e)) do
      true ->
        copy_var(env__get(var_name(e), r_opnd(opnd, :env)), ctxt,
                   env, s)
      false ->
        case (is_literal(e)) do
          true ->
  ...
  defp copy_inline_2(r, e, ctxt, env, s) do
    limit = (case (size_is_active(s)) do
               true ->
                 get_size_limit(s) - get_size_value(s)
               false ->
                 get_size_limit(s)
             end)
    s1 = new_active_size(limit + apply_size(length(r_app(ctxt, :opnds))),
                           s)
    try do
        ...
        case (type(e)) do
          ...
          :call ->
            ...
            case (is_c_atom(m) and is_c_atom(f)) do
              true ->
                as = call_args(e)
                is_safe_list(as) and is_safe_call(atom_val(m),
                                                    atom_val(f), length(as))
              false ->
                false
            end
          _ ->
  ...
  defp make_opnd(e, ren, env, noInline, s) do
    ...
    c = st__get_effort(s1)
    opnd = r_opnd(expr: e, ren: ren, env: env, loc: l, effort: c,
               no_inline: noInline)
    {opnd, s1}
  end
  ...
  defp residualize_operand(opnd, e, s) do
    case (st__get_opnd_effect(r_opnd(opnd, :loc), s)) do
      true ->
        {e1, s1} = i(r_opnd(opnd, :expr), :effect, r_opnd(opnd, :ren),
                       r_opnd(opnd, :env), s)
        {make_seq(e1, e), s1}
      false ->
        {e, s}
  ...
  defp visit_1(opnd, s) do
    case (st__lookup_opnd_cache(r_opnd(opnd, :loc), s)) do
      :error ->
        ...
        s2 = new_passive_size(get_size_limit(s1), s1)
        {e, s3} = i(r_opnd(opnd, :expr), :value, r_opnd(opnd, :ren),
                      r_opnd(opnd, :env), s2)
  ...
  defp make_template(e, vs0, env0) do
    case (is_data(e)) do
      true ->
        {ts, {vs1, env1}} = mapfoldl(fn e, {vs0, env0} ->
                                          {t, vs1, env1} = make_template(e, vs0,
                                                                           env0)
                                          {t, {vs1, env1}}
                                     end,
                                       {vs0, env0}, data_es(e))
  ...
  defp equivalent_clauses_1(e, [c | cs], env) do
    ...
    case (equivalent(e, clause_body(c),
                       :ordsets.union(env, env1))) do
      true ->
      ...
      true ->
        case (is_data(e2)) do
          true ->
            ...
            t1 === t2 and equivalent_lists(data_es(e1), data_es(e2),
                                             env)
          false ->
            false
        end
  ...
  defp equivalent_lists([e1 | es1], [e2 | es2], env) do
    :erlang.and(equivalent(e1, e2, env),
      ...
      true ->
        ...
        cond do
          (is_integer(n) and n > 0 and n <= tuple_size(t)) ->
            ...
            es = :erlang.tuple_to_list(:erlang.setelement(n, t,
                                                            void()))
  ...
  defp set_clause_bodies([c | cs], b) do
    [update_c_clause(c, clause_pats(c), clause_guard(c),
                       b) |
  ...
  defp env__new_fname(a, n, env) do
    :rec_env.new_key(fn x ->
                          s = :erlang.integer_to_list(x)
                          {:erlang.list_to_atom(:erlang.atom_to_list(a) ++ '_' ++ s),
                             n}
                     end,
                       env)
  end

  Record.defrecord(:r_state, :state, free: :undefined,
                                 size: :undefined, effort: :undefined,
                                 unroll: :undefined, cache: :undefined,
                                 var_flags: :undefined, opnd_flags: :undefined,
                                 app_flags: :undefined)
  Record.defrecord(:r_var_flags, :var_flags, lab: :undefined,
                                     referenced: false)
  Record.defrecord(:r_opnd_flags, :opnd_flags, lab: :undefined,
                                      inner_pending: 1, outer_pending: 1,
                                      effect: false)
  Record.defrecord(:r_app_flags, :app_flags, lab: :undefined,
                                     inlined: false)
  defp st__new(effort, size, unroll) do
    etsOpts = [:set, :private, {:keypos, 2}]
    r_state(free: 0, size: counter__new_passive(size),
        effort: counter__new_passive(effort), unroll: unroll,
        cache: :maps.new(), var_flags: :ets.new(:var, etsOpts),
  ...
  defp st__mark_inner_pending(l, s) do
    :ets.update_counter(r_state(s, :opnd_flags), l,
                          {r_opnd_flags(:inner_pending), - 1})
    s
  end
  ...
  defp st__new_opnd_loc(s) do
    v = ({l, _S1} = st__new_loc(s))
    n = r_state(s, :unroll)
    :ets.insert(r_state(s, :opnd_flags),
