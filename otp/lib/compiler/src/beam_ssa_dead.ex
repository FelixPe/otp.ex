defmodule :m_beam_ssa_dead do
  ...
  import :lists, only: [append: 1, foldl: 3, keymember: 3,
                          last: 1, member: 2, reverse: 1, reverse: 2,
                          takewhile: 2]
  require Record
  ...
  Record.defrecord(:r_b_local, :b_local, name: :undefined,
                                   arity: :undefined)
  Record.defrecord(:r_st, :st, bs: :undefined,
                              us: :undefined, skippable: :undefined,
                              test: :none, target: :any)
  def opt(linear0) do
  ...
  defp shortcut_terminator(r_b_br(bool: r_b_literal(val: true), succ: succ0), _Is, from,
            st0) do
    st = r_st(st0, test: :none)
    shortcut(succ0, from, %{}, st)
  end

  defp shortcut_terminator(r_b_br(bool: r_b_var() = bool, succ: succ0,
              fail: fail0) = br,
            is, from, st0) do
    st = r_st(st0, target: :one_way)
    test = get_test(bool, is)
    succBs = bind_var_if_used(succ0, bool, r_b_literal(val: true), st)
    brSucc = shortcut(succ0, from, succBs,
                        r_st(st, test: test))
    failBs = bind_var_if_used(fail0, bool, r_b_literal(val: false),
                                st)
    brFail = shortcut(fail0, from, failBs,
                        r_st(st, test: invert_test(test)))
  ...
  defp shortcut_terminator(r_b_switch(arg: bool, fail: fail0, list: list0) = sw,
            _Is, from, st) do
  ...
  defp shortcut_sw_fail(fail0, list, bool, from, st0) do
    case (list) do
      [{r_b_literal(val: false), _}, {r_b_literal(val: true), _}] ->
        ...
        r_b_br(bool: r_b_literal(val: true), succ: fail) = shortcut(fail0,
                                                       from, %{}, st)
        fail
      _ ->
        fail0
  ...
  defp shortcut_sw_list([{lit, l0} | t], bool, from, st0) do
    ...
    r_b_br(bool: r_b_literal(val: true), succ: l) = shortcut(l0, from,
                                                bind_var(bool, lit, %{}),
                                                r_st(st, target: :one_way))
  ...
  defp shortcut_unsafe_br(br, from, bs, unsetVars,
            r_st(target: target) = st) do
    case (br) do
  ...
  defp shortcut_safe_br(br, from, bs, unsetVars,
            r_st(target: target) = st) do
    case (br) do
      r_b_br(bool: r_b_literal(val: true), succ: l) ->
        case (target) do
  ...
  defp update_unset_vars(l, is, br, unsetVars,
            r_st(skippable: skippable)) do
    case (:erlang.is_map_key(l, skippable)) do
      ...
      false ->
        setInThisBlock = (for r_b_set(dst: v) <- is do
                            v
                          end)
  ...
  defp shortcut_two_way(r_b_br(succ: succ, fail: fail), from, bs0,
            unsetVars0, st0) do
    case (shortcut_2(succ, from, bs0, unsetVars0,
                       r_st(st0, target: fail))) do
  ...
  defp is_br_safe(unsetVars, br, r_st(us: us) = st) do
    case (br) do
      r_b_br(bool: r_b_var() = v, succ: succ, fail: fail) ->
        ...
        not
        :sets.is_element(v,
                           unsetVars) and :sets.is_disjoint(used0,
                                                              unsetVars) and :sets.is_disjoint(used1,
                                                                                                 unsetVars)
      r_b_br(succ: same, fail: same) ->
        not
        is_forbidden(same,
                       st) and :sets.is_disjoint(:erlang.map_get(same, us),
                                                   unsetVars)
    end
  end
  ...
  defp eval_is([r_b_set(op: :phi, dst: dst, args: args) | is], from,
            bs0, st) do
  ...
  defp eval_is([r_b_set(op: {:succeeded, :guard}, dst: dst,
               args: [var])],
            _From, bs, _St) do
    case (bs) do
      %{^var => r_b_literal()} ->
  ...
  defp eval_is([r_b_set(op: {:bif, _}, dst: dst) = i0 | is], from,
            bs, st) do
    i = sub(i0, bs)
  ...
  defp eval_terminator(r_b_switch(arg: arg, fail: fail, list: list) = sw, bs,
            st) do
  ...
  defp eval_switch(list, arg, r_st(test: {_, arg, _} = prevOp),
            fail) do
  ...
  defp normalize_test(:is_tagged_tuple,
  ...
  defp will_succeed({{:not, :is_boolean}, var},
    ...
    case (last) do
      ...
      r_b_br(bool: r_b_var() = bool, succ: succ, fail: fail) ->
        case (comb_is(is, bool, safe0)) do
          ...
          {r_b_set(op: {:bif, :"=:="}, args: [r_b_var() = arg, r_b_literal() = lit]),
             safe} ->
  ...
  defp combine_lists(fail, l1, l2, blocks) do
    ls = :beam_ssa.rpo(for {_, lbl} <- l1 do
                         lbl
                       end,
                         blocks)
  ...
  defp are_lists_compatible([{r_b_literal(val: val1), _} | _],
            [{r_b_literal(val: val2), _} | _]) do
  ...
  defp opt_switch(r_b_blk(last: r_b_switch(arg: arg, list: list0) = sw) = blk,
            tests)
  ...
  defp opt_redundant_tests_is([r_b_set(op: op, args: args, dst: bool) = i0], tests,
            acc) do
  ...
  defp update_successors(r_b_blk(last: r_b_br(bool: bool, succ: succ, fail: fail)),
            bool, test, mustInvert, tests, all0) do
    all1 = update_successor(succ,
                              Map.put(tests, test, not mustInvert), all0)
    update_successor(fail, Map.put(tests, test, mustInvert),
                       all1)
  end
  ...
  defp update_successors(r_b_blk(last: r_b_switch(arg: arg, fail: fail, list: list)),
            tests, all0) do
    all1 = update_successors_sw_fail(list, arg, fail, tests,
                                       all0)
  ...
  defp update_successors_sw([{lit, l} | t], arg, tests, all0) do
    all = update_successor(l,
                             Map.put(tests, {:"=:=", arg, lit}, true), all0)
  ...
  defp used_vars([{l, r_b_blk(is: is) = blk} | bs], usedVars0,
            skip0) do
    ...
    defined0 = (for r_b_set(dst: def__) <- is do
                  def__
                end)
