defmodule :m_beam_trim do
  use Bitwise
  import :lists, only: [any: 2, reverse: 1, reverse: 2, seq: 2, sort: 1]
  require Record
  ...
  defp trim([{:init_yregs, {:list, killed}} = i | is0], [u | us], st) do
  ...
  defp is_recipe_viable({_, trim, moves}, usedRegs) do
    ...
    illegal =
      :ordsets.from_list(
        for {:move, _, dst} <- moves do
          dst
        end
      )

    eliminated =
      for n <- seq(0, trim - 1) do
        {:y, n}
      end
  ...
  defp remap(
         [
           {:bs_create_bin, fail, alloc, live, unit, dst0, {:list, ss0}}
           | is
         ],
         remap
       ) do
  ...
  defp remap(
         [{:bs_start_match4, fail, live, src, dst} | is],
         remap
       ) do
  ...
  defp remap(
         [
           {:update_record, hint, size, src0, dst0, {:list, updates0}}
           | is
         ],
         remap
       ) do
  ...
  defp do_usage(
         [
           {:bs_create_bin, fail, _, _, _, dst, {:list, args}}
           | is
         ],
         safe,
         regs0,
  ...
  defp do_usage([{:bs_get_tail, src, dst, _} | is], safe, regs0, ns0, acc) do
    regs1 = :ordsets.del_element(dst, regs0)
  ...
  defp do_usage([{:bs_set_position, src1, src2} | is], safe, regs0, ns, acc) do
  ...
  defp do_usage(
    ...
    case fail === {:atom, :no_fail} or fail === {:atom, :resume} or
           is_safe_branch(
             fail,
             safe
           ) do
  ...
  defp do_usage([{:call_ext, _, _} | is], safe, regs, ns, acc) do
    u = {regs, ns}
    do_usage(is, safe, regs, ns, [u | acc])
  end
  ...
  defp do_usage([{:call_fun2, _, _, ss} | is], safe, regs0, ns, acc) do
    regs = :ordsets.union(regs0, yregs([ss]))
    u = {regs, ns}
    do_usage(is, safe, regs, ns, [u | acc])
  end

  defp do_usage([{:bif, _, fail, ss, dst} | is], safe, regs0, ns0, acc) do
    case is_safe_branch(fail, safe) do
      true ->
        regs1 = :ordsets.del_element(dst, regs0)
  ...
  defp do_usage([{:gc_bif, _, fail, _, ss, dst} | is], safe, regs0, ns0, acc) do
    case is_safe_branch(fail, safe) do
      true ->
        regs1 = :ordsets.del_element(dst, regs0)
        regs = :ordsets.union(regs1, yregs(ss))
  ...
  defp do_usage(
         [
           {:get_map_elements, fail, s, {:list, list}}
           | is
         ],
         safe,
         regs0,
  ...
  defp do_usage([{:init_yregs, {:list, ds}} | is], safe, regs0, ns0, acc) do
  ...
  defp do_usage([{:make_fun3, _, _, _, dst, {:list, ss}} | is], safe, regs0, ns0, acc) do
    regs1 = :ordsets.del_element(dst, regs0)
    regs = :ordsets.union(regs1, yregs(ss))
    ns = :ordsets.union(yregs([dst]), ns0)
    u = {regs, ns}
  ...
  defp do_usage(
         [
           {:update_record, _, _, src, dst, {:list, ss}}
           | is
         ],
         safe,
         regs0,
  ...
  defp do_usage([{:recv_marker_clear, src} | is], safe, regs0, ns, acc) do
    regs = :ordsets.union(regs0, yregs([src]))
    u = {regs, ns}
    do_usage(is, safe, regs, ns, [u | acc])
  end

  defp do_usage([{:recv_marker_reserve, src} | is], safe, regs0, ns, acc) do
    regs = :ordsets.union(regs0, yregs([src]))
    u = {regs, ns}
    do_usage(is, safe, regs, ns, [u | acc])
  end

  defp do_usage([{:swap, r1, r2} | is], safe, regs0, ns0, acc) do
    ds = yregs([r1, r2])
  ...
  defp do_usage([{:test, _, fail, ss} | is], safe, regs0, ns, acc) do
    case is_safe_branch(fail, safe) do
      true ->
        regs = :ordsets.union(regs0, yregs(ss))
        u = {regs, ns}
  ...
  defp do_usage([{:test, _, fail, _, ss, dst} | is], safe, regs0, ns0, acc) do
    case is_safe_branch(fail, safe) do
      true ->
        regs1 = :ordsets.del_element(dst, regs0)
        regs = :ordsets.union(regs1, yregs(ss))
