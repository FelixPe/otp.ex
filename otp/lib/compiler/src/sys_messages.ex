defmodule :m_sys_messages do
  ...
  defp quote_source_1(file, {startLine, startCol},
            {endLine, endCol}) do
    case (:file.read_file(file)) do
      {:ok, bin} ->
        ...
        case (seek_line(bin, 1, startLine - ctx)) do
          {:ok, bin1} ->
            quote_source_2(bin1, enc, startLine, startCol, endLine,
                             endCol, ctx)
          :error ->
            ''
        end
  ...
  defp quote_source_2(bin, enc, startLine, startCol, endLine, endCol,
            ctx) do
    case (take_lines(bin, enc, startLine - ctx,
                       endLine + ctx)) do
      [] ->
        ''
      lines ->
        lines1 = (case (length(lines) <= 4 + ctx) do
                    true ->
                      lines
                    false ->
                      before = :lists.sublist(lines, 2 + ctx)
                      after__ = :lists.reverse(:lists.sublist(:lists.reverse(lines),
                                                                1 + ctx))
                      before ++ [{0, '...'}] ++ after__
                  end)
        lines2 = decorate(lines1, startLine, startCol, endLine,
                            endCol)
  ...
  defp decorate([{line, text} = l | ls], startLine, startCol,
            endLine, endCol)
  ...
  defp decorate([{_Line, _Text} = l | ls], startLine, startCol,
            endLine, endCol) do
  ...
  defp decorate('', l, ls, startLine, startCol, endLine,
            endCol) do
    [l | decorate(ls, startLine, startCol, endLine, endCol)]
  end

  defp decorate(text, {line, _} = l, ls, startLine, startCol,
            endLine, endCol) do
    [l, {- line, text} | decorate(ls, startLine, startCol,
                                    endLine, endCol)]
  end
  ...
  defp take_lines(bin, enc, here, to) when here <= to do
    ...
    [{here, text_to_string(text, enc)} | take_lines(rest,
                                                      enc, here + 1, to)]
  end
