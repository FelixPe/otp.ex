defmodule :m_sys_core_prepare do
  ...
  def module(mod0, _Opts) do
    ...
    {mod, _} = :cerl_trees.mapfold(&rewrite_recv/2, count,
                                     mod0)
    {:ok, mod, []}
  end

  defp rewrite_recv(r_c_receive(clauses: [], timeout: timeout0,
              action: action),
            count0) do
    ...
    letrec = r_c_letrec(anno: [:letrec_goto], defs: [{loopFun, fun}],
                 body: applyLoop)
    outerLet = r_c_let(vars: [timeoutVal], arg: timeout0,
                   body: letrec)
    {outerLet, count4}
  end

  defp rewrite_recv(r_c_receive(clauses: cs0, timeout: timeout0,
              action: action),
            count0) do
    false__ = r_c_literal(val: false)
    true__ = r_c_literal(val: true)
    ...
    peekLet = r_c_let(vars: [peekSucceeded, msg],
                  arg: primop(:recv_peek_message), body: peekCase)
