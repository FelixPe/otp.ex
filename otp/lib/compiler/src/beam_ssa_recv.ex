defmodule :m_beam_ssa_recv do
  ...
  Record.defrecord(:r_scan, :scan, graph: :beam_digraph.new(),
                                module: :undefined, recv_candidates: %{},
                                ref_candidates: %{})
  def module(r_b_module() = mod0, opts) do
    case (scan(mod0)) do
      r_scan() = scan ->
        ...
        ws = (case (:proplists.get_bool(:recv_opt_info,
                                          opts)) do
                true ->
  ...
  defp scan(r_b_module(body: fs)) do
    case (scan_peek_message(fs)) do
      [_ | _] = rs0 ->
        ...
        modMap = foldl(fn r_b_function(bs: blocks, args: args) = f, acc ->
        ...
        foldl(fn f, scan0 ->
                ...
                r_scan(module: modMap, recv_candidates: rs), fs)
      [] ->
        :none
    end
  end
  ...
  defp scan_rets(blocks) do
    rets = :maps.fold(fn _K, r_b_blk(last: r_b_ret(arg: r_b_var() = retVal)),
                           acc ->
  ...
  defp scan_is([r_b_set(op: {:succeeded, :body})], blk, lbl, _Blocks,
            funcId, state) do
    r_b_br(bool: r_b_var(), succ: succ) = r_b_blk(blk, :last)
    scan_add_edge({funcId, lbl}, {funcId, succ}, state)
  end

  defp scan_is([r_b_set(op: :new_try_tag, dst: dst)], blk, lbl,
            _Blocks, funcId, state) do
    r_b_br(bool: ^dst, succ: succ) = r_b_blk(blk, :last)
    scan_add_edge({funcId, lbl}, {funcId, succ}, state)
  end

  defp scan_is([r_b_set(op: :call, dst: dst, args: [r_b_remote() | _]) = call,
               r_b_set(op: {:succeeded, :body})],
            r_b_blk(last: r_b_br(succ: succ)), lbl, blocks, funcId, state0) do
    case (blocks) do
      %{^succ => r_b_blk(is: [], last: r_b_ret(arg: ^dst))} ->
        si_remote_call(call, lbl, succ, blocks, funcId, state0)
      %{} ->
        state = si_remote_call(call, lbl, succ, blocks, funcId,
                                 state0)
        scan_add_edge({funcId, lbl}, {funcId, succ}, state)
    end
  end

  defp scan_is([r_b_set(op: :call, args: [r_b_remote() | _]) = call | is],
            blk, lbl, blocks, funcId, state0) do
    state = si_remote_call(call, lbl, lbl, blocks, funcId,
                             state0)
    scan_is(is, blk, lbl, blocks, funcId, state)
  end

  defp scan_is([r_b_set(op: :call, dst: dst, args: [r_b_local() | _]) = call,
  ...
  defp scan_is([], r_b_blk(last: r_b_ret()), lbl, _Blocks, funcId,
            state) do
  ...
  defp scan_is([], blk, lbl, _Blocks, funcId, state) do
    foldl(fn succ, acc ->
            ...
            state, :beam_ssa.successors(blk))
  end

  defp scan_add_call(call, callLbl, succLbl, caller,
            r_scan(module: modMap) = state0) do
    r_b_set(dst: dst, args: [r_b_local() = callee | args]) = call
    %{^callee => {_Blocks, params, rets}} = modMap
    {callTranslation,
       callInverse} = scan_translate_call(args, params, %{},
                                            %{})
    state = scan_add_edge({caller, callLbl}, {callee, 0},
                            {callTranslation, callInverse, args}, state0)
    {retTranslation,
       retInverse} = scan_translate_return(rets, dst,
                                             callTranslation)
    scan_add_edge({callee, - 1}, {caller, succLbl},
                    {retTranslation, retInverse, params}, state)
  end

  defp scan_translate_call([arg | args], [param | params], argToParams,
            paramToArgs) do
  ...
  defp scan_translate_return_1([ret | rets], dst, calleeToCaller) do
    scan_translate_return_1(rets, dst,
                              Map.put(calleeToCaller, ret, dst))
  end
  ...
  defp scan_add_edge(from, to, label, state0) do
    state = scan_add_vertex(to,
                              scan_add_vertex(from, state0))
    graph = :beam_digraph.add_edge(r_scan(state, :graph), from,
                                     to, label)
  ...
  defp si_remote_call(r_b_set(anno: anno, dst: dst, args: args) = call,
            calledAt, validAfter, blocks, funcId, state) do
    case (si_remote_call_1(dst, args, validAfter,
                             blocks)) do
      {:makes_ref, extractedAt, ref} ->
        ...
        candidates = Map.put(candidates0, funcId,
                                            [makeRef | makeRefs0])
  ...
  defp si_remote_call_1(dst, [callee | args], lbl, blocks) do
    ...
    case (mFA) do
      {:erlang, :alias, a} when (is_integer(a) and 0 <= a and
                                   a <= 1)
                                ->
        {:makes_ref, lbl, dst}
      {:erlang, :demonitor, 2} ->
      ...
      {:erlang, :make_ref, 0} ->
        {:makes_ref, lbl, dst}
      {:erlang, :monitor, a} when (is_integer(a) and
                                     2 <= a and a <= 3)
                                  ->
        {:makes_ref, lbl, dst}
      {:erlang, :spawn_monitor, a} when (is_integer(a) and
                                           1 <= a and a <= 4)
                                        ->
        rPO = :beam_ssa.rpo([lbl], blocks)
        si_ref_in_tuple(rPO, blocks, dst)
      {:erlang, :spawn_request, a} when (is_integer(a) and
                                           1 <= a and a <= 5)
                                        ->
        {:makes_ref, lbl, dst}
      _ ->
  ...
  defp si_ref_in_tuple_is([r_b_set(op: :get_tuple_element, dst: ref,
               args: [r_b_var() = tuple, pos]) |
               is],
            tuple) do
    case (pos) do
      r_b_literal(val: 1) ->
  ...
  defp plan(scan) do
    r_scan(ref_candidates: refCandidates,
        recv_candidates: receiveCandidates, module: modMap,
        graph: graph) = scan
  ...
  defp propagate_references(candidates, g) do
    roots = :maps.fold(fn funcId, makeRefs, acc ->
  ...
  defp propagate_references_1([{vertex, ref} | vRefs], g, acc0) do
    refs = :maps.get(vertex, acc0,
                       :sets.new([{:version, 2}]))
    acc = (case (:sets.is_element(ref, refs)) do
             true ->
               acc0
             false ->
               acc1 = Map.put(acc0, vertex,
                                      :sets.add_element(ref, refs))
               next = pr_successors(:beam_digraph.out_edges(g, vertex),
                                      ref)
  ...
  defp pr_successors([{{_, fromLbl}, to,
              {translation, _Inverse, args}} |
               edges],
            ref) do
    case (translation) do
      %{^ref => r_b_var() = param} ->
  ...
  defp plan_uses(candidates, refMap, modMap) do
    :maps.fold(fn funcId, receives, acc ->
  ...
  defp plan_uses_1([{lbl, i} | receives], funcId, blocks,
            refMap) do
    case (refMap) do
      %{{^funcId, ^lbl} => refs} ->
        case (search(fn ref ->
  ...
  defp pu_is_ref_used(r_b_set(op: :call, args: [callee | args]), ref, _Lbl,
            _Blocks) do
    mFA = (case (callee) do
             r_b_remote(mod: r_b_literal(val: mod), name: r_b_literal(val: func), arity: arity) ->
               {mod, func, arity}
  ...
  defp pu_is_ref_used(r_b_set(op: :peek_message, dst: msg) = i, ref, lbl,
            blocks) do
    r_b_blk(is: [^i | _]) = (blk = :erlang.map_get(lbl, blocks))
    vs = %{msg => :message, ref => :ref, ref: ref,
             ref_matched: false}
  ...
  defp pu_is_ref_used_last(r_b_blk(last: last) = blk, vs, blocks) do
    succVs = (case (last) do
                r_b_br(bool: r_b_var() = bool, succ: succ, fail: fail) ->
                  case (vs) do
                    %{^bool => {:is_ref, matched}} ->
                      [{succ, %{vs | ref_matched: matched}}, {fail,
                                                                %{vs
                                                                  |
                                                                  ref_matched:
                                                                  not matched}}]
  ...
  defp pu_is_ref_used_is([r_b_set(op: {:bif, bif}, args: args, dst: dst) = i |
               is],
            vs0) do
    cond do
      bif === :"=:=" or bif === :"==" ->
  ...
  defp pu_update_vars(r_b_set(args: args, dst: dst), vs) do
    vars = (for (r_b_var() = v) <- args do
              v
            end)
    all = :lists.all(fn var ->
                          case (vs) do
                            %{^var => :message} ->
                              true
                            %{} ->
  ...
  defp intersect_uses(usageMap, refMap, graph) do
    roots = :maps.fold(fn funcId, uses, acc ->
  ...
  defp intersect_uses_1([{vertex, ref} | vs], refMap, graph, acc0) do
    possibleRefs = :maps.get(vertex, refMap,
                               :sets.new([{:version, 2}]))
    activeRefs0 = :maps.get(vertex, acc0,
                              :sets.new([{:version, 2}]))
    acc = (case ({:sets.is_element(ref, possibleRefs),
                    :sets.is_element(ref, activeRefs0)}) do
             {true, false} ->
               ...
               intersect_uses_1(next, refMap, graph,
                                  Map.put(acc0, vertex, activeRefs))
             {false, _} ->
               acc0
             {_, true} ->
               acc0
  ...
  defp iu_predecessors([{from, _To, {_Translation, inverse, _Args}} |
               edges],
            ref) do
    case (inverse) do
      %{^ref => r_b_var() = arg} ->
  ...
  defp plan_markers(candidates, usageMap) do
    :maps.fold(fn funcId, makeRefs, acc ->
  ...
  defp plan_clears(usageMap, graph) do
    :maps.fold(fn {funcId, _} = vertex, activeRefs, acc ->
  ...
  defp plan_clears_1([{from, to, :branch} | edges], activeRefs,
            usageMap) do
    toRefs = :maps.get(to, usageMap,
                         :sets.new([{:version, 2}]))
    refs = :sets.subtract(activeRefs, toRefs)
    {funcId, fromLbl} = from
    {^funcId, toLbl} = to
    clears = (for ref <- :sets.to_list(refs) do
                {fromLbl, toLbl, ref}
              end)
    clears ++ plan_clears_1(edges, activeRefs, usageMap)
  end

  defp plan_clears_1([{_From, _To, {_, _, _}} | edges], activeRefs,
            usageMap) do
  ...
  defp optimize_1(r_b_function(bs: blocks0, cnt: count0) = f, markers, uses,
            clears) do
    funcId = get_func_id(f)
    {blocks1, count1} = insert_markers(:maps.get(funcId,
                                                   markers, []),
                                         blocks0, count0)
    {blocks2, count2} = insert_uses(:maps.get(funcId, uses,
                                                []),
                                      blocks1, count1)
    {blocks, count} = insert_clears(:maps.get(funcId,
                                                clears, []),
                                      blocks2, count2)
    r_b_function(f, bs: blocks,  cnt: count)
  end

  defp insert_markers([{anno, createdAt, dst, extractedAt, ref} |
               markers],
            blocks0, count0) do
    {markerVar, blocks1, count1} = insert_reserve(createdAt,
                                                    dst, anno, blocks0, count0)
    {blocks, count} = insert_bind(extractedAt, ref,
  ...
  defp insert_reserve(lbl, dst, anno, blocks0, count0) do
    ...
    count = count0 + 1
    reserve = r_b_set(anno: anno, op: :recv_marker_reserve,
                  args: [], dst: var)
  ...
  defp insert_bind(lbl, ref, marker, blocks0, count0) do
    ...
    count = count0 + 1
    bind = r_b_set(op: :recv_marker_bind, args: [marker, ref],
               dst: ignored)
  ...
  defp insert_bind_is([r_b_set(), r_b_set(op: {:succeeded, _})] = is, bind,
            _Last) do
    [bind | is]
  end

  defp insert_bind_is([r_b_set(op: :call, dst: ret)] = is, bind,
            r_b_ret(arg: ret)) do
    [bind | is]
  end
  ...
  defp insert_uses([{_Lbl, r_b_set(op: :call), _Ref} | uses], blocks,
            count) do
    insert_uses(uses, blocks, count)
  end

  defp insert_uses([{lbl, r_b_set(op: :peek_message) = peek0, ref} |
               uses],
            blocks0, count) do
    %{^lbl => r_b_blk(is: is0) = blk} = blocks0
    [^peek0 | is] = is0
  ...
  defp insert_clears_1([{from, to, ref} | clears], count0, acc) do
    ignored = r_b_var(name: {:"@ssa_ignored", count0})
    count = count0 + 1
    clear = r_b_set(op: :recv_marker_clear, args: [ref],
                dst: ignored)
    insert_clears_1(clears, count,
                      [{from, to, [clear]} | acc])
  end
  ...
  defp coi_1([r_b_function(args: args, bs: blocks) = f | fs], acc0) do
    ...
    {defs, _} = foldl(fn var, {defs0, index0} ->
  ...
  defp coi_is([r_b_set(anno: anno, op: :peek_message,
               args: [r_b_var()] = args) |
               is],
            last, blocks, where, defs, ws) do
    [creation] = coi_creations(args, blocks, defs)
    warning = make_warning({:used_receive_marker, creation},
                             anno, where)
    coi_is(is, last, blocks, where, defs, [warning | ws])
  end

  defp coi_is([r_b_set(anno: anno, op: :peek_message, args: [r_b_literal()]) |
               is],
            last, blocks, where, defs, ws) do
  ...
  defp coi_is([r_b_set(anno: anno, op: :recv_marker_reserve) | is],
            last, blocks, where, defs, ws) do
    warning = make_warning(:reserved_receive_marker, anno,
                             where)
    coi_is(is, last, blocks, where, defs, [warning | ws])
  end

  defp coi_is([r_b_set(anno: anno, op: :call, dst: dst,
               args: [r_b_local() | args]) = i |
               is],
            last, blocks, where, defs0, ws0) do
    defs = Map.put(defs0, dst, i)
    ws = (for (r_b_set() = creation) <- coi_creations(args,
                                                  blocks, defs) do
  ...
  defp coi_is([r_b_set(dst: dst) = i | is], last, blocks, where,
            defs0, ws) do
    defs = Map.put(defs0, dst, i)
    coi_is(is, last, blocks, where, defs, ws)
  end
  ...
  defp coi_creations([var | vars], blocks, defs) do
    case (defs) do
      ...
      %{^var
        =>
        r_b_set(op: :get_tuple_element, args: [tuple | _])} ->
  ...
  defp format_opt_info({:passed_marker, creation}) do
    :io_lib.format('INFO: passing reference ~ts', [format_ref_creation(creation)])
  end

  defp format_opt_info({:used_receive_marker, creation}) do
    :io_lib.format('OPTIMIZED: all clauses match reference ~ts', [format_ref_creation(creation)])
  end
