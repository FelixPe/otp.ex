defmodule :m_sys_core_alias do
  ...
  Record.defrecord(:r_sub, :sub, p: %{},
                               v: :sets.new([{:version, 2}]), t: :none)
  ...
  defp def({r_c_var(name: {f, arity}) = name, b0}) do
    try do
      ...
      {b1, _} = :cerl_trees.mapfold(&pre/2, &post/2,
                                      sub_new(:none), b0)
  ...
  defp pre(r_c_clause(pats: pats) = node, sub0) do
    case (get_pattern_keys(pats)) do
      ...
      keys ->
        ...
        sub3 = r_sub(sub2, v: merge_variables(varNames, subNames), 
  ...
  defp post(r_c_clause() = node,
            r_sub(t: {:clause, pats0, keys, v, t}) = sub0) do
  ...
  defp get_variables(nodesList) do
    :sets.from_list(for node <- nodesList,
  ...
  defp put_pattern_keys(patterns, keys) do
    {newPatterns,
       map} = :lists.mapfoldl(&alias_pattern_keys/2,
                                :maps.from_list(keys), patterns)
    0 = map_size(map)
    newPatterns
  end

  defp alias_pattern_keys(r_c_tuple(anno: anno, es: es0) = node, acc0) do
    {es1, acc1} = :lists.mapfoldl(&alias_pattern_keys/2,
                                    acc0, es0)
    nodes_to_alias(:tuple, es0, anno, r_c_tuple(node, es: es1),
                     acc1)
  end
  ...
  defp alias_pattern_keys(r_c_cons(anno: anno, hd: hd0, tl: tl0) = node, acc0) do
    ...
    nodes_to_alias(:cons, [hd0, tl0], anno,
                     r_c_cons(node, hd: hd1,  tl: tl1), acc2)
  end
  ...
  defp ntk_1([node | t], acc0, kind, n) do
    case (:cerl.is_data(node)) do
      true ->
        case (ntk_1(:cerl.data_es(node), [],
                      :cerl.data_type(node), n - 1)) do
          {:ok, key} ->
