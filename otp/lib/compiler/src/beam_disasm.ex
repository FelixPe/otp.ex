defmodule :m_beam_disasm do
  ...
  def format_error({:internal, error}) do
    :io_lib.format(~c"~p: disassembly failed with reason ~P.", [:beam_disasm, error, 25])
  end
  ...
  defp process_chunks(f) do
    case :beam_lib.chunks(
           f,
           [:atoms, ~c"Code", ~c"StrT", :indexed_imports, :labeled_exports]
         ) do
      {:ok,
       {module,
        [
        ...
        lambdaBin = optional_chunk(f, ~c"FunT")
        lambdas = beam_disasm_lambdas(lambdaBin, atoms)
        literalBin = optional_chunk(f, ~c"LitT")
        literals = beam_disasm_literals(literalBin)
        typeBin = optional_chunk(f, ~c"Type")
        ...
        compInfo =
          case optional_chunk(f, ~c"CInf") do
            :none ->
              []
  ...
  defp beam_disasm_types(<<version::size(32), count::size(32), table0::binary>>) do
    case :beam_types.convert_ext(version, table0) do
      :none ->
        exit({:beam_disasm, 272, {:beam_disasm_types, {:unknown_type_version, version}}})
    ...
    case symOp do
      ...
      _ ->
        try do
          ...
          :error, rsn ->
            :ok
            exit({:beam_disasm, 437, {:cannot_disasm_instr, {symOp, arity, rsn}}})
        else
          {args, restBs} ->
            :ok
            {{symOp, args}, restBs}
    ...
    case b >>> 5 do
      7 ->
        ...
        case arg do
          ...
          _ ->
            exit({:beam_disasm, 617, {:decode_int, :weird_bignum_sublength, arg}})
        end
  ...
  defp decode_z_tagged(tag, b, bs, literals, types)
    ...
    case n do
      ...
      _ ->
        exit({:beam_disasm, 654, {:decode_z_tagged, {:invalid_extended_tag, n}}})
    end
  end

  defp decode_z_tagged(_, b, _, _, _) do
    exit({:beam_disasm, 657, {:decode_z_tagged, {:weird_value, b}}})
  end
    ...
    tag =
      case tag0 do
        index when is_integer(index) ->
          {^tag0, {_F, _A, label, _Index, _NumFree, _OldUniq}} = :lists.keyfind(tag0, 1, lambdas)
          {:f, label}
  ...
  defp resolve_inst(
         {:bs_get_integer2 = i, [fail, ms, {:u, live}, size0, {:u, unit}, {:u, flags}, dst0]},
         _,
         _,
         _
       ) do
  ...
  defp resolve_inst(
         {:bs_get_binary2 = i, [fail, ms, {:u, live}, size0, {:u, unit}, {:u, flags}, dst0]},
         _,
         _,
         _
       ) do
  ...
  defp resolve_inst(
         {:bs_get_float2 = i, [fail, ms, {:u, live}, size0, {:u, unit}, {:u, flags}, dst0]},
         _,
         _,
         _
       ) do
  ...
  defp resolve_inst({:bs_start_match3 = i, [fail, bin, {:u, live}, dst]}, _, _, _) do
    {:test, i, fail, live, [bin], dst}
  end

  defp resolve_inst({:bs_get_tail, [src, dst, {:u, live}]}, _, _, _) do
    {:bs_get_tail, src, dst, live}
  end

  defp resolve_inst({:bs_get_position, [src, dst, {:u, live}]}, _, _, _) do
  ...
  defp resolve_inst({:bs_start_match4, [fail, {:u, live}, src, dst]}, _, _, _) do
  ...
  defp resolve_inst(
         {:update_record, [hint, {:u, size}, src, dst, {{{:z, 1}, {:u, _}, list0}}]},
         _,
         _,
         _
       ) do
  ...
  defp resolve_inst({:bs_match, [{fail, ctx, {:z, 1}, {:u, _}, args}]}, _, _, _) do
  ...
  defp resolve_bs_create_bin_list(
         [{:atom, :string} = type, seg0, unit0, flags, offset0, size0 | rest],
         strings
       ) do
    [seg, unit, offset, {:integer, len} = size] = resolve_args([seg0, unit0, offset0, size0])
    <<_::size(offset)-binary, bin::size(len)-binary, _::binary>> = strings
  ...
  defp resolve_bs_create_bin_list(
         [type, seg0, unit0, flags, val0, size0 | rest],
         strings
       ) do
  ...
  defp resolve_bs_match_commands([
         {:atom, :ensure_at_least},
         size,
         unit
         | rest
       ]) do
  ...
  defp resolve_bs_match_commands([{:atom, :integer}, live, flags0, size, unit, dst | rest]) do
  ...
  defp resolve_bs_match_commands([{:atom, :binary}, live, flags0, size, unit, dst | rest]) do
    flags = resolve_bs_match_flags(flags0)
