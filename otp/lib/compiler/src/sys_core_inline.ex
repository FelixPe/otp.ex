defmodule :m_sys_core_inline do
  use Bitwise
  import :lists, only: [foldl: 3, map: 2, mapfoldl: 3,
                          member: 2]
  require Record
  ...
  Record.defrecord(:r_inline, :inline, exports: [], thresh: 0,
                                  inline: [])
  Record.defrecord(:r_fstat, :fstat, func: :undefined,
                                 arity: :undefined, def: :undefined, weight: 0,
                                 inline: false, modified: false)
  Record.defrecord(:r_ifun, :ifun, func: :undefined,
                                arity: :undefined, vars: :undefined,
                                body: :undefined, weight: :undefined)
  def module(r_c_module(exports: es, defs: ds0) = mod, opts) do
    case (inline_option(opts)) do
      {thresh, fs} when (is_integer(thresh) and thresh > 0) or
                          fs !== []
                        ->
  ...
  defp inline(fs0, st0) do
    fs1 = map(fn {r_c_var(name: {f, a}), r_c_fun(body: b)} = def__ ->
                   weight = :cerl_trees.fold(&weight_func/2, 0, b)
                   r_fstat(func: f, arity: a, def: def__, weight: weight)
              end,
                fs0)
    {fs2, is0} = mapfoldl(fn fst, ifs ->
                               case (is_inlineable(fst, r_inline(st0, :thresh),
                                                     r_inline(st0, :inline))) do
                                 true ->
                                   {_, ffun} = r_fstat(fst, :def)
                                   if__ = r_ifun(func: r_fstat(fst, :func),
  ...
  defp inline_inline(r_ifun(body: b, weight: iw) = if__, is) do
    inline = fn r_c_apply(op: r_c_var(name: {f, a}), args: as) = call ->
  ...
  defp kill_id_anns(body) do
    :cerl_trees.map(fn r_c_fun(anno: a0) = cFun ->
                         a = kill_id_anns_1(a0)
                         r_c_fun(cFun, anno: a)
                       r_c_var(anno: a0) = var ->
                         a = kill_id_anns_1(a0)
