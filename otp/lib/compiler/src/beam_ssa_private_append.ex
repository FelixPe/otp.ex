defmodule :m_beam_ssa_private_append do
  use Bitwise
  import :lists, only: [foldl: 3, foldr: 3, keysort: 2, map: 2, reverse: 1]
  require Record
  ...
  def opt(stMap, funcDb) do
    funs =
      for f <- :maps.keys(stMap), :erlang.is_map_key(f, funcDb), not is_nif(f, stMap) do
        f
      end
  ...
  defp find_appends_is(
    ...
    found = Map.put(found0, fun, [{:append, dst, lit} | alreadyFound])
  ...
  defp find_appends_is(
    ...
    case dies and
           is_unique(
             var,
             anno
           ) and is_appendable(anno, segmentInfo) do
      true ->
        alreadyFound = :maps.get(fun, found0, [])
        found = Map.put(found0, fun, [{:append, dst, var} | alreadyFound])
        find_appends_is(is, fun, found)
  ...
  defp is_appendable(anno, r_b_literal(val: [segmentUnit | _]))
    ...
    case anno do
      %{arg_types: %{2 => r_t_bitstring(appendable: true, size_unit: sizeUnit)}} ->
  ...
  Record.defrecord(:r_def_st, :def_st,
    funcdb: :undefined,
    stmap: :undefined,
    defsdb: %{},
    literals: %{},
  ...
  defp find_defs_1(
         [{fun, {:append, dst, _Arg}} | work],
         defSt0 = r_def_st(stmap: stMap)
       ) do
    %{^fun => r_opt_st(ssa: sSA, args: args)} = stMap
    {defsInFun, defSt} = defs_in_fun(fun, args, sSA, defSt0)
    valuesInFun = values_in_fun(fun, defSt)
    :skip
    track_value_in_fun([{dst, :self}], fun, work, defsInFun, valuesInFun, defSt)
  end

  defp find_defs_1(
         [
           {fun, {:track_call_argument, callee, element, idx}}
           | work
         ],
         defSt0 = r_def_st(stmap: stMap)
       ) do
    ...
    {vars, defSt} = get_call_arguments(callee, element, idx, defsInFun, fun, defSt1)
    :skip
    track_value_in_fun(vars, fun, work, defsInFun, valuesInFun, defSt)
  end

  defp find_defs_1(
         [{fun, {:track_result, element}} | work],
         defSt0 = r_def_st(stmap: stMap)
       ) do
    %{^fun => r_opt_st(ssa: sSA, args: args)} = stMap
    {defsInFun, defSt1} = defs_in_fun(fun, args, sSA, defSt0)
  ...
  defp get_results(
         [{_, r_b_blk(last: r_b_ret(arg: r_b_var() = v))} | rest],
         acc,
         element,
         fun,
         defSt
  ...
  defp get_results(
         [{lbl, r_b_blk(last: r_b_ret(arg: r_b_literal(val: lit)))} | rest],
         acc,
         element,
         fun,
         defSt0
  ...
  defp track_value_in_fun([{r_b_var() = v, element} | rest], fun, work, defs, valuesInFun, defSt0)
       when :erlang.is_map_key({v, element}, valuesInFun) do
    :skip
    track_value_in_fun(rest, fun, work, defs, valuesInFun, defSt0)
  end

  defp track_value_in_fun(
         [{r_b_var() = v, element} | rest],
         fun,
         work0,
         defs,
         valuesInFun0,
    ...
    :skip
    valuesInFun = Map.put(valuesInFun0, {v, element}, :visited)

    case defs do
      %{^v => r_b_set(dst: ^v, op: op, args: args)} ->
        case {op, args, element} do
          {:bs_create_bin, [r_b_literal(val: :append), _, arg | _], :self} ->
            track_value_in_fun([{arg, :self} | rest], fun, work0, defs, valuesInFun, defSt0)

          {:bs_create_bin, [r_b_literal(val: :private_append), _, _ | _], :self} ->
            track_value_in_fun(rest, fun, work0, defs, valuesInFun, defSt0)

          {:bs_init_writable, _, :self} ->
            track_value_in_fun(rest, fun, work0, defs, valuesInFun, defSt0)

          {:call, [r_b_local() = callee | _Args], _} ->
            track_value_into_call(callee, element, fun, rest, work0, defs, valuesInFun, defSt0)
          ...
          {:phi, _, _} ->
            {toExplore, defSt} = handle_phi(fun, v, args, element, defSt0)
            track_value_in_fun(toExplore ++ rest, fun, work0, defs, valuesInFun, defSt)

          {:put_tuple, _, _} when element !== :self ->
            track_put_tuple(args, element, rest, fun, v, work0, defs, valuesInFun, defSt0)

          {:put_list, _, _} when element !== :self ->
            track_put_list(args, element, rest, fun, v, work0, defs, valuesInFun, defSt0)
      ...
      %{^v => {:arg, idx}} ->
        track_value_into_caller(element, idx, rest, fun, work0, defs, valuesInFun, defSt0)
    end
  end

  defp track_value_in_fun([{r_b_literal(), _} | rest], fun, work, defs, valuesInFun, defSt) do
  ...
  defp track_value_in_fun(
         ...
         _Defs,
         valuesInFun,
         defSt0 = r_def_st(valuesdb: valuesDb0)
       ) do
    defSt = r_def_st(defSt0, valuesdb: Map.put(valuesDb0, fun, valuesInFun))
    find_defs_1(work, defSt)
  end

  defp track_value_into_call(
         callee,
         element,
         callerFun,
         callerWork,
    ...
    globalWork = [
      {callee, {:track_result, element}}
      | globalWork0
    ]

    track_value_in_fun(callerWork, callerFun, globalWork, callerDefs, callerValuesInFun, defSt0)
  end

  defp track_value_into_caller(
         element,
         argIdx,
         calledFunWorklist,
         calledFun,
         ...
         calledFunDefs,
         calledFunValues,
         defSt0 = r_def_st(funcdb: funcDb, stmap: stMap)
       ) do
    ...
    work =
      for caller <- callers,
        ...
        {caller, {:track_call_argument, calledFun, element, argIdx}}
      end
  ...
  defp track_put_list(
    ...
    case hd do
      r_b_var() ->
        track_value_in_fun([{hd, element} | work], fun, globalWork, defs, valuesInFun, defSt0)
  ...
  defp get_call_arguments(callee, element, idx, defs, fun, defSt0) do
    :maps.fold(
      fn
        _, r_b_set(dst: dst, op: :call, args: [target | args]), {acc, defSt}
        when callee === target ->
          {values, defSt1} = gca(args, element, idx, fun, dst, defSt)
          {values ++ acc, defSt1}
  ...
  defp gca([r_b_var() = v | _], i, element, i, _Fun, _Dst, defSt) do
    {[{v, element}], defSt}
  end

  defp gca([r_b_literal(val: lit) | _], i, :self, i, _Fun, _Dst, defSt)
       when not is_bitstring(lit) do
    {[], defSt}
  end

  defp gca([r_b_literal(val: lit) | _], i, element, i, fun, dst, defSt) do
  ...
  defp handle_phi(fun, dst, args, element, defSt0) do
    foldl(
      fn
        {r_b_literal(val: lit), lbl}, {acc, defStAcc0} ->
          defStAcc =
            add_literal(
              fun,
  ...
  defp patch_appends_f(
         [{lbl, blk = r_b_blk(is: is0, last: last0)} | rest],
         cnt0,
         pD0,
         acc0,
         blockAdditions0
    ...
    acc = [
      {lbl, r_b_blk(blk, is: is ++ extra, last: last)}
      | acc0
    ]
  ...
  defp patch_appends_is([i0 = r_b_set(dst: dst) | rest], pD0, cnt0, acc, blockAdditions0)
    ...
    case patches do
      ...
      [{:append, ^dst, r_b_literal(val: <<>>) = lit}] ->
        ...
        {v, cnt} = new_var(cnt0)
        init = r_b_set(op: :bs_init_writable, dst: v, args: [r_b_literal(val: 256)])

        i =
          r_b_set(i0,
            args: [
              r_b_literal(val: :private_append),
              segInfo,
              v
              | otherArgs
            ]
  ...
  defp patch_appends_ret(last = r_b_ret(arg: r_b_literal(val: lit)), [{:ret, _, element}], cnt0) do
    :skip
  ...
  defp patch_opargs(
         [r_b_literal(val: lit) | args],
         [{idx, lit, element} | patches],
         idx,
         patchedArgs,
         is,
         cnt0
  ...
  defp patch_opargs([arg | args], patches, idx, patchedArgs, is, cnt) do
    :skip
  ...
  defp patch_phi(i0 = r_b_set(op: :phi, args: args0), patches, cnt0) do
    ...
    {args, extra, cnt} =
      foldr(
        fn arg0 = {_, lbl}, {argsAcc, extraAcc, cntAcc} ->
          case l2P do
            %{^lbl => {:phi, _, ^lbl, lit, element}} ->
              ...
              {[{arg, lbl} | argsAcc], [{lbl, extra} | extraAcc], cnt1}
  ...
  defp patch_literal_term(<<>>, :self, cnt0) do
    {v, cnt} = new_var(cnt0)
    i = r_b_set(op: :bs_init_writable, dst: v, args: [r_b_literal(val: 256)])
    {v, [i], cnt}
  end
  ...
  defp patch_literal_tuple(tuple, elements0, cnt) do
    ...
    elements =
      for {:tuple_element, _, _} = e <- elements0 do
        e
      end
  ...
  defp patch_literal_tuple(
    ...
    patch_literal_tuple(litElements, elements, [v | patched], exs ++ extra, idx + 1, cnt)
  end

  defp patch_literal_tuple([lit | litElements], patches, patched, extra, idx, cnt) do
    :skip
  ...
  defp insert_block_additions([blk0 = {l, b = r_b_blk(is: is0)} | revLinear], lbl2Addition, acc) do
    blk =
      case lbl2Addition do
        %{^l => additions} ->
