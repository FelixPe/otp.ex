defmodule :m_beam_clean do
  ...
  defp rootset(fs, root0, attr) do
    root1 = (case (:proplists.get_value(:on_load, attr)) do
               :undefined ->
                 root0
               [onLoad] ->
                 [onLoad | root0]
    ...
    map0 = (for {:function, name, arity, lbl, _} <- fs do
              {{name, arity}, lbl}
            end)
  ...
  defp update_work_list([{:make_fun3, {:f, l}, _, _, _, _} | is],
            sets) do
    update_work_list(is, add_to_work_list(l, sets))
  end
  ...
  require Record
  Record.defrecord(:r_st, :st, lmap: :undefined,
                              entry: :undefined, lc: :undefined)
  defp clean_labels(fs0) do
    st0 = r_st(lmap: [], entry: 1, lc: 1)
    {fs1, r_st(lmap: lmap0, lc: lc)} = function_renumber(fs0,
                                                        st0, [])
  ...
  defp function_renumber([{:function, name, arity, _Entry, asm0} | fs],
            st0, acc) do
    {asm, st} = renumber_labels(asm0, [], st0)
    function_renumber(fs, st,
                        [{:function, name, arity, r_st(st, :entry), asm} | acc])
  end
  ...
  defp renumber_labels([{:label, old} | is], [{:label, new} | _] = acc,
            r_st(lmap: d0) = st) do
    d = [{old, new} | d0]
  ...
  defp renumber_labels([{:label, old} | is], acc, st0) do
    ...
    renumber_labels(is, [{:label, new} | acc],
                      r_st(st0, lmap: d,  lc: new + 1))
  end

  defp renumber_labels([{:func_info, _, _, _} = fi | is], acc, st0) do
    renumber_labels(is, [fi | acc],
                      r_st(st0, entry: r_st(st0, :lc)))
  end
  ...
  defp function_replace([{:function, name, arity, entry, asm0} | fs],
            dict, acc) do
    asm = (try do
             fb = fn old ->
    ...
    function_replace(fs, dict,
                       [{:function, name, arity, entry, asm} | acc])
  end
  ...
  defp swap_moves([{:swap, reg1, reg2} | is]) do
    temp = {:x, 1022}
    [{:move, reg1, temp}, {:move, reg2, reg1}, {:move, temp,
                                                  reg2} |
                                                   swap_moves(is)]
  end
  ...
  defp fix_bs_create_bin([{:bs_create_bin, fail, alloc, live, unit, dst,
              {:list, list}} |
               is]) do
    ...
    try do
      ...
      :invalid_size ->
        [{:move, {:atom, :badarg}, {:x, 0}}, {:call_ext_only, 1,
                                                {:extfunc, :erlang, :error,
                                                   1}} |
                                                 tail]
    else
      sizeCalc0 ->
        ...
        sizeIs0 = bs_size_calc(sizeCalc, fail, sizeReg,
                                 {:x, live + 1})
        sizeIs = [{:move, {:integer, 0}, sizeReg} | sizeIs0]
        restIs = bs_puts(list, fail) ++ [{:move, tmpDst, dst} |
                                             tail]
        case (list) do
          [{:atom, :append}, _, _, _, src | _] ->
            sizeIs ++ [{:bs_append, fail, sizeReg, alloc, live + 1,
                          unit, src, flags, tmpDst} |
                           restIs]
          [{:atom, :private_append}, _, _, _, src | _] ->
            testHeap = {:test_heap, alloc, live + 1}
            sizeIs ++ [testHeap, {:bs_private_append, fail, sizeReg,
                                    unit, src, flags, tmpDst} |
                                     restIs]
          _ ->
            sizeIs ++ [{:bs_init_bits, fail, sizeReg, alloc,
                          live + 1, flags, tmpDst} |
                           restIs]
        end
    end
  ...
  defp fold_size_calc([{unit, {:term, {:integer, size}}} | t], bits,
            acc) do
    fold_size_calc(t, bits + unit * size, acc)
  end

  defp fold_size_calc([{unit, {{:bif, bif}, {:literal, lit}}} = h |
               t],
            bits, acc) do
    try do
      ...
      result ->
        fold_size_calc([{unit, {:term, {:integer, result}}} |
                            t],
                         bits, acc)
    end
  end

  defp fold_size_calc([{u, _} = h | t], bits, acc) when u === 1 or
                                              u === 8 do
    fold_size_calc(t, bits, [h | acc])
  end

  defp fold_size_calc([{u, var} | t], bits, acc) do
    fold_size_calc(t, bits,
                     [{1, {:"*", {:term, {:integer, u}}, var}} | acc])
  end

  defp fold_size_calc([], bits, acc) do
    bytes = div(bits, 8)
    remBits = rem(bits, 8)
    sizes = [{1, {:term, {:integer, remBits}}}, {8,
                                                   {:term, {:integer, bytes}}} |
                                                    acc]
    for ({_, sz} = pair) <- sizes,
  ...
  defp bs_size_calc([{unit, {{:bif, bif}, reg}} | t], fail, sizeReg,
            tmpReg) do
    live = :erlang.element(2, sizeReg) + 1
    [{:gc_bif, bif, fail, live, [reg], tmpReg}, {:bs_add,
                                                   fail,
                                                   [sizeReg, tmpReg, unit],
                                                   sizeReg} |
                                                    bs_size_calc(t, fail,
  ...
  defp bs_size_calc([{unit, {:"*", {:term, term1}, {:term, term2}}} |
               t],
            fail, sizeReg, tmpReg) do
    live = :erlang.element(2, sizeReg) + 1
  ...
  defp bs_size_calc([{unit, {{:instr, instr}, reg}} | t], fail,
            sizeReg, tmpReg) do
    [{instr, fail, reg, tmpReg}, {:bs_add, fail,
                                    [sizeReg, tmpReg, unit], sizeReg} |
                                     bs_size_calc(t, fail, sizeReg, tmpReg)]
  end

  defp bs_size_calc([{unit, {:term, term}} | t], fail, sizeReg,
            tmpReg) do
  ...
  defp bs_puts([{:atom, :string}, _Seg, _Unit, _Flags,
                                              {:string, _} = str, {:integer,
                                                                     size} |
                                                                      is],
            fail) do
  ...
  defp bs_puts([{:atom, :private_append}, _, _, _, _, _ | is],
            fail) do
    bs_puts(is, fail)
  end

  defp bs_puts([{:atom, type}, _Seg, unit, flags0, src, size |
                                                       is],
            fail) do
    op = (case (type) do
            :integer ->
    ...
    i = (cond do
           ...
           true ->
             {:bs_put, fail, {op, unit, {:field_flags, flags}},
                [size, src]}
         end)
    [i | bs_puts(is, fail)]
  end
