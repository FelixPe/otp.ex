defmodule :m_beam_ssa_bsm do
  use Bitwise
  import :lists, only: [foldl: 3, mapfoldl: 3, max: 1,
                          member: 2, nth: 2, reverse: 1, reverse: 2,
                          splitwith: 2, unzip: 1]
  require Record
  ...
  def module(r_b_module(body: fs0) = module, opts) do
    ...
    {fs, _} = :compile.run_sub_passes([{:combine_matches,
                                          &combine_matches/1},
                                           {:accept_context_args,
                                              &accept_context_args/1},
                                               {:combine_matches,
                                                  &combine_matches/1},
                                                   {:allow_context_passthrough,
                                                      &allow_context_passthrough/1},
                                                       {:skip_outgoing_tail_extraction,
                                                          &skip_outgoing_tail_extraction/1},
                                                           {:annotate_context_parameters,
                                                              &annotate_context_parameters/1}],
                                        {fs0, modInfo})
    ws = (case (:proplists.get_bool(:bin_opt_info, opts)) do
            true ->
              collect_opt_info(fs)
            false ->
              []
  ...
  defp analyze_module(r_b_module(body: fs)) do
    foldl(fn r_b_function(args: parameters) = f, i ->
               funcInfo = %{has_bsm_ops: has_bsm_ops(f),
                              parameters: parameters, parameter_info: %{}}
               funcId = get_fa(f)
  ...
  defp hbo_is([r_b_set(op: :nif_start) | _]) do
    :nif_start
  end
  ...
  defp ccc_1([r_b_local() = call | args], ctx, aliases, modInfo) do
    ...
    cond do
      useCount === 1 ->
        ...
        callee = {name, arity}
        paramInfo = funcinfo_get(callee, :parameter_info,
                                   modInfo)
  ...
  defp ccc_1([r_b_remote() = call | _Args], _Ctx, _CtxChain,
            _ModInfo) do
    {:remote_call, call}
  end
  ...
  defp is_tail_binary(r_b_set(op: :bs_match,
              args: [r_b_literal(val: :binary) | rest])) do
  ...
  defp match_context_of_1(var, defs) do
    case (:maps.get(var, defs)) do
      r_b_set(op: :bs_extract, args: [r_b_var() = ctx0]) ->
        r_b_set(op: :bs_match,
            args: [_, r_b_var() = ctx | _]) = :maps.get(ctx0, defs)
        ctx
  ...
  defp funcinfo_set(key, attribute, value, modInfo) do
    funcInfo = :maps.put(attribute, value,
                           :maps.get(key, modInfo, %{}))
  ...
  Record.defrecord(:r_amb, :amb, dominators: :undefined,
                               match_aliases: :undefined, cnt: :undefined,
                               promotions: %{})
  defp alias_matched_binaries(blocks0, counter, aliasMap)
      when aliasMap !== %{} do
    rPO = :beam_ssa.rpo(blocks0)
    {dominators, _} = :beam_ssa.dominators(rPO, blocks0)
    state0 = r_amb(dominators: dominators,
                 match_aliases: aliasMap, cnt: counter)
    {blocks, state} = :beam_ssa.mapfold_blocks(&amb_1/3,
                                                 rPO, state0, blocks0)
  ...
  defp amb_assign_set(r_b_set(op: :phi, args: args0) = i, _Lbl, state0) do
    {args, state} = mapfoldl(fn {arg0, lbl}, acc ->
  ...
  defp amb_assign_set(r_b_set(args: args0) = i, lbl, state0) do
    {args, state} = mapfoldl(fn arg0, acc ->
  ...
  defp amb_get_alias(r_b_remote(mod: mod0, name: name0) = arg0, lbl,
            state0) do
  ...
  defp amb_create_alias(r_b_var() = arg0, context, lbl, state0) do
    ...
    prevPromotions = (for dom <- dominators,
    ...
    case (prevPromotions) do
      ...
      [] ->
        ...
        promotion = r_b_set(op: :bs_get_tail, dst: alias,
                        args: [context])
        promotions = :maps.put({lbl, arg0}, promotion,
                                 promotions0)
        state = r_amb(state0, promotions: promotions, 
                            cnt: counter + 1)
        {alias, state}
    end
  end
  ...
  defp amb_insert_promotions(blocks0, state) do
    f = fn {lbl, r_b_var()}, promotion, blocks ->
             ...
             {before, after__} = splitwith(fn r_b_set(args: args) ->
  ...
  Record.defrecord(:r_cm, :cm, definitions: :undefined,
                              dominators: :undefined, blocks: :undefined,
                              match_aliases: %{}, prior_matches: %{},
                              renames: %{})
  ...
  defp combine_matches(r_b_function(bs: blocks0, cnt: counter0) = f, modInfo) do
    case (funcinfo_get(f, :has_bsm_ops, modInfo)) do
      true ->
        rPO = :beam_ssa.rpo(blocks0)
        {dominators, _} = :beam_ssa.dominators(rPO, blocks0)
        {blocks1, state} = :beam_ssa.mapfold_blocks(fn lbl,
                                                         r_b_blk(is: is0) = block0,
                                                         state0 ->
                                                         {is, state} = cm_1(is0,
                                                                              [],
                                                                              lbl,
                                                                              state0)
                                                         {r_b_blk(block0, is: is),
                                                            state}
                                                    end,
                                                      rPO,
                                                      r_cm(definitions: :beam_ssa.definitions(rPO,
                                                                                             blocks0),
                                                          dominators: dominators,
                                                          blocks: blocks0),
                                                      blocks0)
        blocks2 = :beam_ssa.rename_vars(r_cm(state, :renames), rPO,
                                          blocks1)
        blocks3 = :beam_ssa.trim_unreachable(blocks2)
        aliases = r_cm(state, :match_aliases)
        {blocks4, counter} = alias_matched_binaries(blocks3,
                                                      counter0, aliases)
        blocks = (cond do
                    map_size(aliases) === 0 ->
                      blocks4
  ...
  defp cm_1([r_b_set(op: :bs_start_match, dst: ctx,
               args: [_, src]),
    ...
    case (is_tail_binary(src, r_cm(state0, :definitions))) do
      ...
      false ->
        cm_handle_priors(src, ctx, bool, acc, matchSeq, lbl,
                           state0)
    end
  end
  ...
  defp cm_handle_priors(src, dstCtx, bool, acc, matchSeq, lbl,
            state0) do
    priorCtxs = (case (:maps.find(src,
                                    r_cm(state0, :prior_matches))) do
                   {:ok, priors} ->
    ...
    case (priorCtxs) do
      [ctx | _] ->
        ...
        renames = Map.merge(renames0, %{bool => r_b_literal(val: true),
                                          dstCtx => ctx})
  ...
  defp cm_register_prior(src, dstCtx, lbl, state) do
    ...
    priorMatches = :maps.put(src, priors,
                               r_cm(state, :prior_matches))
  ...
  defp cm_combine_tail(src, dstCtx, bool, acc, state0) do
    ...
    {srcCtx, renames} = cm_combine_tail_1(bool, dstCtx,
                                            srcCtx0, r_cm(state0, :renames))
    aliases = :maps.put(src, {0, srcCtx},
                          r_cm(state0, :match_aliases))
    state = r_cm(state0, match_aliases: aliases, 
                        renames: renames)
    {acc, state}
  end
  ...
  Record.defrecord(:r_aca, :aca, unused_parameters: :undefined,
                               counter: :undefined, parameter_info: %{},
                               match_aliases: %{})
  ...
  defp accept_context_args(r_b_function(bs: blocks0) = f, modInfo0) do
    case (funcinfo_get(f, :has_bsm_ops, modInfo0)) do
      true ->
        parameters = :ordsets.from_list(funcinfo_get(f,
                                                       :parameters, modInfo0))
        state0 = r_aca(unused_parameters: parameters,
        ...
        modInfo = funcinfo_set(f, :parameter_info,
                                 r_aca(state, :parameter_info), modInfo0)
  ...
  defp aca_1(blocks, state) do
    ...
    aca_enable_reuse(r_b_blk(entryBlock, :is), entryBlock, blocks,
                       [], state)
  end

  defp aca_enable_reuse([r_b_set(op: :bs_start_match, args: [_, src]) = i0 |
               rest],
            entryBlock, blocks0, acc, state0) do
    case (aca_is_reuse_safe(src, state0)) do
      true ->
        {i, last0, blocks1, state} = aca_reuse_context(i0,
                                                         entryBlock, blocks0,
                                                         state0)
        is = reverse([i | acc], rest)
        last = :beam_ssa.normalize(last0)
        blocks = :maps.put(0,
                             r_b_blk(entryBlock, is: is,  last: last), blocks1)
  ...
  defp aca_enable_reuse([i | is], entryBlock, blocks, acc, state0) do
    ...
    case (:ordsets.intersection(unusedParams0,
                                  :beam_ssa.used(i))) do
      [] ->
        aca_enable_reuse(is, entryBlock, blocks, [i | acc],
                           state0)
      prematureUses ->
        unusedParams = :ordsets.subtract(unusedParams0,
                                           prematureUses)
        paramInfo = foldl(fn a, ps ->
                               :maps.put(a, {:used_before_match, i}, ps)
                          end,
                            r_aca(state0, :parameter_info), prematureUses)
        state = r_aca(state0, unused_parameters: unusedParams, 
  ...
  defp aca_reuse_context(r_b_set(op: :bs_start_match, dst: dst,
              args: [_, src]) = i0,
            block, blocks0, state0) do
    {state1, last, blocks} = aca_handle_convergence(src,
                                                      state0, r_b_blk(block, :last),
                                                      blocks0)
    aliases = :maps.put(src, {r_b_br(last, :succ), dst},
                          r_aca(state1, :match_aliases))
    paramInfo = :maps.put(src, :suitable_for_reuse,
                            r_aca(state1, :parameter_info))
    state = r_aca(state1, match_aliases: aliases, 
                        parameter_info: paramInfo)
    i = :beam_ssa.add_anno(:accepts_match_contexts, true,
                             i0)
    {i, last, blocks, state}
  end
  ...
  defp aca_handle_convergence(src, state0, last0, blocks0) do
    ...
    convergedPaths = :ordsets.intersection(:ordsets.from_list(succPath),
                                             :ordsets.from_list(failPath))
    convergedLabels = :beam_ssa.rpo(convergedPaths, blocks0)
    case (:maps.is_key(src,
                         :beam_ssa.uses(convergedLabels, blocks0))) do
      true ->
        case (shortest(succPath, failPath)) do
          :left ->
            {succ, blocks, counter} = aca_copy_successors(succ0,
                                                            blocks0,
            ...
            {state, last, blocks}
          :right ->
            {fail, blocks, counter} = aca_copy_successors(fail0,
                                                            blocks0,
                                                            r_aca(state0, :counter))
            state = r_aca(state0, counter: counter)
  ...
  defp aca_copy_successors(lbl0, blocks0, counter0) do
    ...
    {blocks, counter} = aca_cs_1(path, blocks0, counter1,
                                   %{}, bRs, %{})
  ...
  defp aca_cs_build_brs([lbl | path], counter0, acc) do
    aca_cs_build_brs(path, counter0 + 1,
                       Map.put(acc, lbl, counter0))
  end
  ...
  defp aca_cs_1([lbl0 | path], blocks, counter0, vRs0, bRs,
            acc0) do
    block0 = :maps.get(lbl0, blocks)
    lbl = :maps.get(lbl0, bRs)
    {vRs, block, counter} = aca_cs_block(block0, counter0,
                                           vRs0, bRs)
  ...
  defp aca_cs_block(r_b_blk(is: is0, last: last0) = block0, counter0,
            vRs0, bRs) do
  ...
  defp aca_cs_is([r_b_set(op: op, dst: dst0, args: args0) = i0 | is],
            counter0, vRs0, bRs, acc) do
    args = (case (op) do
              :phi ->
  ...
  defp aca_cs_last(r_b_switch(arg: arg0, list: switch0, fail: fail0) = sw,
            vRs, bRs) do
    switch = (for {literal, lbl} <- switch0 do
                {literal, :maps.get(lbl, bRs)}
              end)
    r_b_switch(sw, arg: aca_cs_arg(arg0, vRs), 
            fail: :maps.get(fail0, bRs),  list: switch)
  end

  defp aca_cs_last(r_b_br(bool: arg0, succ: succ0, fail: fail0) = br,
            vRs, bRs) do
    r_b_br(br, bool: aca_cs_arg(arg0, vRs), 
  ...
  defp aca_cs_args_phi([{arg, lbl} | args], vRs, bRs) do
    case (bRs) do
      %{^lbl => new} ->
        [{aca_cs_arg(arg, vRs), new} | aca_cs_args_phi(args,
                                                         vRs, bRs)]
      %{} ->
  ...
  defp allow_context_passthrough({fs, modInfo0}) do
    fsUses = (for (r_b_function(bs: bs) = f) <- fs do
                {f, :beam_ssa.uses(:beam_ssa.rpo(bs), bs)}
              end)
  ...
  defp acp_forward_params(fsUses, modInfo0) do
    f = fn {r_b_function(args: parameters) = func, useMap}, modInfo ->
             paramInfo = foldl(fn param, paramInfo ->
  ...
  defp acp_1(param, [{0, r_b_set(op: :call) = i}], modInfo,
            paramInfo) do
  ...
  Record.defrecord(:r_sote, :sote, definitions: :undefined,
                                mod_info: :undefined, match_aliases: %{})
  ...
  defp skip_outgoing_tail_extraction(r_b_function(bs: blocks0) = f, modInfo) do
    case (funcinfo_get(f, :has_bsm_ops, modInfo)) do
      true ->
        rPO = :beam_ssa.rpo(blocks0)
        state0 = r_sote(definitions: :beam_ssa.definitions(rPO,
                                                        blocks0),
                     mod_info: modInfo)
        {blocks1,
           state} = :beam_ssa.mapfold_instrs(&sote_rewrite_calls/2,
                                               rPO, state0, blocks0)
        {blocks, counter} = alias_matched_binaries(blocks1,
                                                     r_b_function(f, :cnt),
  ...
  defp sote_rewrite_call(call0, [arg | argsIn], argsOut, state0) do
    case (is_tail_binary(arg, r_sote(state0, :definitions))) do
      true ->
        ctxChain = context_chain_of(arg,
                                      r_sote(state0, :definitions))
        case (check_context_call(call0, arg, ctxChain,
                                   r_sote(state0, :mod_info))) do
          :suitable_for_reuse ->
            ...
            call = :beam_ssa.add_anno(:bsm_info, :context_reused,
                                        call0)
      ...
      false ->
        sote_rewrite_call(call0, argsIn, [arg | argsOut],
                            state0)
    end
  end
  ...
  defp annotate_context_parameters(f, modInfo) do
    ...
    paramAnno = :maps.fold(fn k, _V, acc
                             ...
                             paramAnno0, paramInfo)
    {:beam_ssa.add_anno(:parameter_info, paramAnno, f),
       modInfo}
  end

  defp collect_opt_info(fs) do
    foldl(fn r_b_function(bs: blocks) = f, acc0 ->
               rPO = :beam_ssa.rpo(blocks)
               useMap = :beam_ssa.uses(rPO, blocks)
               where = :beam_ssa.get_anno(:location, f, [])
               :beam_ssa.fold_instrs(fn i, acc ->
                                          collect_opt_info_1(i, where, useMap,
                                                               acc)
                                     end,
                                       rPO, acc0, blocks)
          end,
  ...
  defp collect_opt_info_1(r_b_set(op: op, anno: anno, dst: dst) = i, where,
            useMap, acc0) do
  ...
  defp add_unopt_binary_info(r_b_set(op: follow, dst: dst), _Nested, where, useMap,
            acc0)
  ...
  defp add_unopt_binary_info(r_b_set(op: follow, dst: dst), nested, where, useMap,
            acc0)
  ...
  defp add_unopt_binary_info(r_b_set(op: :match_fail), _Nested, _Where, _UseMap,
            acc) do
    acc
  end

  defp add_unopt_binary_info(r_b_switch(anno: anno) = i, nested, where, _UseMap,
            acc) do
    [make_promotion_warning(i, nested, anno, where) | acc]
  end

  defp add_unopt_binary_info(r_b_set(anno: anno) = i, nested, where, _UseMap,
            acc) do
    [make_promotion_warning(i, nested, anno, where) | acc]
  end

  defp add_unopt_binary_info(r_b_ret(anno: anno) = i, nested, where, _UseMap,
            acc) do
    [make_promotion_warning(i, nested, anno, where) | acc]
  end

  defp add_unopt_binary_info(r_b_br(anno: anno) = i, nested, where, _UseMap,
            acc) do
    [make_promotion_warning(i, nested, anno, where) | acc]
  end
  ...
  defp make_warning(term, anno, where) do
    {file, line} = (case (:maps.get(:location, anno,
                                      where)) do
                      {_, _} = location ->
                        location
                      _ ->
  ...
  defp format_opt_info_1({:binary_created,
             r_b_set(op: :call, args: [call | _]), false}) do
    :io_lib.format('binary is used in call to ~s which doesn\'t support context reuse', [format_call(call)])
  end

  defp format_opt_info_1({:binary_created,
             r_b_set(op: :call, args: [call | _]), true}) do
    :io_lib.format('binary is used in term passed to ~s', [format_call(call)])
  end

  defp format_opt_info_1({:binary_created,
             r_b_set(op: {:bif, bIF}, args: args), false}) do
    :io_lib.format('binary is used in ~p/~p which doesn\'t support context reuse', [bIF, length(args)])
  end

  defp format_opt_info_1({:binary_created,
             r_b_set(op: {:bif, bIF}, args: args), true}) do
  ...
  defp format_opt_info_1({:no_match_on_entry, call}) do
    :io_lib.format('binary is used in call to ~s which does not begin with a suitable binary match', [format_call(call)])
  end

  defp format_opt_info_1({:used_before_match,
             r_b_set(op: :call, args: [call | _])}) do
    :io_lib.format('binary is used in call to ~s before being matched', [format_call(call)])
  end

  defp format_opt_info_1({:used_before_match,
             r_b_set(op: {:bif, bIF}, args: args)}) do
