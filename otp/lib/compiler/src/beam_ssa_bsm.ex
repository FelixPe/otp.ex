defmodule :m_beam_ssa_bsm do
  ...
  defp alias_matched_binaries(blocks0, counter, aliasMap)
    ...
    {blocks, state} = :beam_ssa.mapfold_blocks(&amb_1/3, rPO, state0, blocks0)
  ...
  defp combine_matches(r_b_function(bs: blocks0, cnt: counter0) = f, modInfo) do
    case funcinfo_get(f, :has_bsm_ops, modInfo) do
      true ->
        ...
        blocks2 = :beam_ssa.rename_vars(r_cm(state, :renames), rPO, blocks1)
        blocks3 = :beam_ssa.trim_unreachable(blocks2)
        aliases = r_cm(state, :match_aliases)
        {blocks4, counter} = alias_matched_binaries(blocks3, counter0, aliases)
  ...
  defp aca_handle_convergence(src, state0, last0, blocks0) do
    ...
    case :maps.is_key(
           src,
           :beam_ssa.uses(convergedLabels, blocks0)
         ) do
      true ->
  ...
  defp allow_context_passthrough({fs, modInfo0}) do
    fsUses =
      for r_b_function(bs: bs) = f <- fs do
  ...
  defp skip_outgoing_tail_extraction(r_b_function(bs: blocks0) = f, modInfo) do
    case funcinfo_get(f, :has_bsm_ops, modInfo) do
      true ->
        ...
        {blocks1, state} = :beam_ssa.mapfold_instrs(&sote_rewrite_calls/2, rPO, state0, blocks0)
  ...
  defp add_unopt_binary_info(r_b_set(op: :match_fail), _Nested, _Where, _UseMap, acc) do
    acc
  end
  ...
  defp format_opt_info(:context_reused) do
    ~c"OPTIMIZED: match context reused"
  end

  defp format_opt_info({:binary_created, _, _} = promotion) do
    :io_lib.format(~c"BINARY CREATED: ~s", [format_opt_info_1(promotion)])
  end

  defp format_opt_info(other) do
    :io_lib.format(~c"NOT OPTIMIZED: ~s", [format_opt_info_1(other)])
  end

  defp format_opt_info_1({:binary_created, r_b_set(op: :call, args: [call | _]), false}) do
    :io_lib.format(~c"binary is used in call to ~s which doesn't support context reuse", [
      format_call(call)
    ])
  end

  defp format_opt_info_1({:binary_created, r_b_set(op: :call, args: [call | _]), true}) do
    :io_lib.format(~c"binary is used in term passed to ~s", [format_call(call)])
  end

  defp format_opt_info_1({:binary_created, r_b_set(op: {:bif, bIF}, args: args), false}) do
    :io_lib.format(~c"binary is used in ~p/~p which doesn't support context reuse", [
      bIF,
      length(args)
    ])
  end
  ...
  defp format_opt_info_1({:binary_created, r_b_set(op: {:bif, bIF}, args: args), true}) do
    :io_lib.format(~c"binary is used in term passed to ~p/~p", [bIF, length(args)])
  end

  defp format_opt_info_1({:binary_created, r_b_set(op: op), false}) do
    :io_lib.format(~c"binary is used in '~p' which doesn't support context reuse", [op])
  end

  defp format_opt_info_1({:binary_created, r_b_set(op: op), true}) do
    :io_lib.format(~c"binary is used in term passed to '~p'", [op])
  end

  defp format_opt_info_1({:binary_created, r_b_ret(), false}) do
    :io_lib.format(~c"binary is returned from the function", [])
  end

  defp format_opt_info_1({:binary_created, r_b_ret(), true}) do
    :io_lib.format(~c"binary is used in a term that is returned from the function", [])
  end

  defp format_opt_info_1({:unsuitable_call, {call, inner}}) do
    :io_lib.format(
      ~c"binary used in call to ~s, where ~s",
      [format_call(call), format_opt_info_1(inner)]
    )
  end

  defp format_opt_info_1({:remote_call, call}) do
    :io_lib.format(~c"binary is used in remote call to ~s", [format_call(call)])
  end

  defp format_opt_info_1({:fun_call, call}) do
    :io_lib.format(~c"binary is used in fun call (~s)", [format_call(call)])
  end

  defp format_opt_info_1({:multiple_uses_in_call, call}) do
    :io_lib.format(~c"binary is passed as multiple arguments to ~s", [format_call(call)])
  end

  defp format_opt_info_1({:no_match_on_entry, call}) do
    :io_lib.format(
      ~c"binary is used in call to ~s which does not begin with a suitable binary match",
      [format_call(call)]
    )
  end

  defp format_opt_info_1({:used_before_match, r_b_set(op: :call, args: [call | _])}) do
    :io_lib.format(~c"binary is used in call to ~s before being matched", [format_call(call)])
  end

  defp format_opt_info_1({:used_before_match, r_b_set(op: {:bif, bIF}, args: args)}) do
    :io_lib.format(~c"binary is used in ~p/~p before being matched", [bIF, length(args)])
  end

  defp format_opt_info_1({:used_before_match, r_b_set(op: :phi)}) do
    :io_lib.format(~c"binary is returned from an expression before being matched", [])
  end

  defp format_opt_info_1({:used_before_match, r_b_set(op: op)}) do
    :io_lib.format(~c"binary is used in '~p' before being matched", [op])
  end

  defp format_opt_info_1(term) do
    :io_lib.format(~c"~w", [term])
  end

  defp format_call(r_b_local(name: r_b_literal(val: f), arity: a)) do
    :io_lib.format(~c"~p/~p", [f, a])
  end

  defp format_call(r_b_remote(mod: r_b_literal(val: m), name: r_b_literal(val: f), arity: a)) do
    :io_lib.format(~c"~p:~p/~p", [m, f, a])
  end

  defp format_call(fun) do
    :io_lib.format(~c"~p", [fun])
  end
end
