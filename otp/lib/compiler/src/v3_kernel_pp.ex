defmodule :m_v3_kernel_pp do
  ...
  defp format_1(r_k_cons(hd: h, tl: t), ctxt) do
    txt = [~c"[" | format(h, ctxt_bump_indent(ctxt, 1))]
  ...
  defp format_1(r_k_tuple(es: es), ctxt) do
    [?{, format_hseq(es, ~c",", ctxt_bump_indent(ctxt, 1), &format/2), ?}]
  end

  defp format_1(r_k_map(var: r_k_literal(val: m), op: :assoc, es: es), ctxt)
       when is_map(m) and map_size(m) === 0 do
    [~c"~{", format_hseq(es, ~c",", ctxt_bump_indent(ctxt, 1), &format/2), ~c"}~"]
  end
  ...
  defp format_1(r_k_map_pair(key: k, val: v), ctxt) do
    [~c"<", format(k, ctxt), ~c",", format(v, ctxt), ~c">"]
  end

  defp format_1(r_k_binary(segs: s), ctxt) do
    [~c"#<", format(s, ctxt_bump_indent(ctxt, 2)), ~c">#"]
  end
  ...
  defp format_1(r_k_alt(first: o, then: t), ctxt) do
    ctxt1 = ctxt_bump_indent(ctxt, r_ctxt(ctxt, :item_indent))
    [~c"alt", nl_indent(ctxt1), format(o, ctxt1), nl_indent(ctxt1), format(t, ctxt1)]
  end

  defp format_1(
         r_k_letrec_goto(label: label, vars: vs, first: first, then: then, ret: rs),
         ctxt
       ) do
    ctxt1 = ctxt_bump_indent(ctxt, r_ctxt(ctxt, :item_indent))
  ...
  defp format_1(r_k_goto(label: label, args: as), ctxt) do
    [~c"goto ", :erlang.atom_to_list(label), format_args(as, ctxt)]
  end
  ...
  defp format_1(r_k_guard(clauses: gs), ctxt) do
    ...
    [~c"when ", nl_indent(ctxt1), format_vseq(gs, ~c"", ~c"", ctxt1, &format/2)]
  end
  ...
  defp format_1(r_k_break(args: as), ctxt) do
    [~c"<", format_hseq(as, ~c",", ctxt_bump_indent(ctxt, 1), &format/2), ~c">"]
  end
  ...
  defp format_1(
    ...
    [
      ...
      nl_indent(ctxt),
      ~c"export [",
      format_vseq(es, ~c"", ~c",", ctxt_bump_indent(ctxt, 8), &format_fa_pair/2),
      ~c"]",
      nl_indent(ctxt),
      ~c"attributes [",
      format_vseq(
  ...
  defp format_1(r_iset(vars: vs, arg: a, body: b), ctxt) do
    ...
    [
      ~c"set <",
      format_hseq(vs, ~c", ", ctxt_bump_indent(ctxt, 5), &format/2),
      ~c"> =",
      nl_indent(ctxt1),
      format(a, ctxt1),
      nl_indent(ctxt),
  ...
  defp format_1(r_k_opaque(val: v), _Ctxt) do
    [~c"** Opaque: ", :io_lib.write(v), ~c" **\n"]
  end

  defp format_1(type, _Ctxt) do
    [~c"** Unsupported type: ", :io_lib.write(type) | ~c" **"]
  end

  defp format_ret(rs, ctxt) do
    [~c" >> ", ~c"<", format_hseq(rs, ~c",", ctxt_bump_indent(ctxt, 5), &format/2), ~c">"]
  end

  defp format_args(as, ctxt) do
    [?(, format_hseq(as, ~c", ", ctxt_bump_indent(ctxt, 1), &format/2), ?)]
  end
  ...
  defp format_attribute({name, val}, ctxt) when is_list(val) do
    ...
    [
      ...
      ~c" = ",
      ?[,
      format_vseq(val, ~c"", ~c",", ctxt1, fn a, _C ->
        :io_lib.write(a)
      end),
      ?]
    ]
  ...
  defp format_list_tail(tail, ctxt) do
    [~c"|", format(tail, ctxt_bump_indent(ctxt, 1)), ~c"]"]
  end
  ...
  defp format_bin_seg(seg, ctxt) do
    [~c"|", format(seg, ctxt_bump_indent(ctxt, 2))]
  end
