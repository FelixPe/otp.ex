defmodule :m_beam_ssa_throw do
  ...
  defp si_is(
         [
           r_b_set(op: makeFun, args: [r_b_local() = callee | _])
           | _Is
         ],
         _Id,
         _Lbl,
  ...
  defp opt([r_b_function(bs: blocks0) = f | fs], throws, tLHs) do
    ...
    blocks =
      case {:sets.is_element(id, throws), tLHs} do
        {true, %{^id => handlers}} ->
  ...
  defp ois_is(
         [
           r_b_set(op: :get_tuple_element, dst: dst, args: [src, r_b_literal(val: offset)])
           | is
         ],
         ts0
       ) do
