defmodule :m_beam_ssa_throw do
  ...
  Record.defrecord(:r_t_union, :t_union, atom: :none, list: :none,
                                   ...
                                   other: :none)
  Record.defrecord(:r_gst, :gst, tlh_roots: :undefined,
                               tlh_edges: %{},
                               throws: :sets.new([{:version, 2}]))
  Record.defrecord(:r_lst, :lst, suitability: %{},
                               handlers: %{}, blocks: :undefined,
  ...
  defp scan_1([], gst) do
    r_gst(tlh_roots: roots, tlh_edges: edges,
        throws: throws) = gst
    case (:sets.is_empty(throws)) do
      true ->
        :no_throws
      false ->
  ...
  defp propagate_tlhs([{id, handlersA} | roots], edges, acc0) do
    ...
    case (:gb_sets.is_subset(handlersA, handlersB)) do
      ...
      false ->
        ...
        acc = propagate_tlhs(callees, edges,
                               Map.put(acc0, id, merged))
  ...
  defp scan_bs([{lbl, r_b_blk(last: last, is: is)} | bs], id, lst0,
            gst0) do
  ...
  defp si_is([r_b_set(op: :landingpad, args: [_Kind, _Tag]) | is],
            id, lbl, last, lst, gst) do
    vars = {:none, :none, :none}
    si_handler_start(is, id, lbl, last, vars, lst, gst)
  end

  defp si_is([r_b_set(op: :resume, args: [stacktrace, _Reason]) |
               is],
            id, lbl, last, lst, gst) do
    si_handler_end(is, id, lbl, last, stacktrace, lst, gst)
  end

  defp si_is([r_b_set(op: :raw_raise, args: [_, _, stacktrace]) |
               is],
            id, lbl, last, lst, gst) do
    si_handler_end(is, id, lbl, last, stacktrace, lst, gst)
  end

  defp si_is([r_b_set(op: :build_stacktrace, args: [stacktrace]) |
               is],
            id, lbl, last, lst, gst) do
    si_handler_end(is, id, lbl, last, stacktrace, lst, gst)
  end

  defp si_is([r_b_set(op: makeFun, args: [r_b_local() = callee | _]) |
               _Is],
            _Id, _Lbl, _Last, lst, gst)
      when makeFun === :make_fun or
             makeFun === :old_make_fun do
    r_gst(tlh_roots: roots0) = gst
    handlers = (case (:gb_trees.lookup(callee, roots0)) do
                  {:value, handlers0} ->
                    :gb_sets.add(:unsuitable, handlers0)
                  :none ->
                    :gb_sets.singleton(:unsuitable)
                end)
    roots = :gb_trees.enter(callee, handlers, roots0)
    {lst, r_gst(gst, tlh_roots: roots)}
  end

  defp si_is([r_b_set(op: :call, dst: dst,
               args: [r_b_remote(mod: r_b_literal(val: :erlang), name: r_b_literal(val: :throw),
                          arity: 1),
                          _Term]),
               r_b_set(op: {:succeeded, :body}, args: [dst])],
            id, _Lbl, r_b_br(fail: 1), lst, gst) do
  ...
  defp si_is([r_b_set(op: :call, dst: dst,
               args: [r_b_local() = callee | _]),
               r_b_set(op: {:succeeded, :body}, args: [dst])],
            id, _Lbl, r_b_br(fail: fail), lst, gst) do
    handlerId = {id, fail}
  ...
  defp si_handler_start([r_b_set(op: :extract, dst: dst,
               args: [_, r_b_literal(val: idx)]) |
               is],
            id, startLbl, last, vars0, lst, gst) do
  ...
  defp si_handler_start(is, id, startLbl, last, vars, lst0, gst) do
    ...
    r_lst(blocks: blocks, predecessors: preds,
        suitability: suitability, handlers: handlers0) = lst0
    {_, _, stacktrace} = vars
    handlers = (case (suitability) do
                  %{^stacktrace => {:tentative, endLbl}} ->
                    path = :beam_ssa.between(startLbl, endLbl, preds,
                                               blocks)
  ...
  defp add_tlh(id, callee, lst, gst) do
    ...
    r_gst(tlh_roots: roots0) = gst
    %{^id => handler} = handlers
    tLHs0 = (case (:gb_trees.lookup(callee, roots0)) do
               :none ->
                 :gb_sets.singleton(handler)
               {:value, v} ->
                 v
             end)
    tLHs = (case (:gb_sets.is_element(:unsuitable,
                                        tLHs0)) do
              true ->
                tLHs0
              false ->
  ...
  defp opt([r_b_function(bs: blocks0) = f | fs], throws, tLHs) do
    id = get_func_id(f)
    blocks = (case ({:sets.is_element(id, throws), tLHs}) do
                {true, %{^id => handlers}} ->
  ...
  defp opt_bs([{lbl, r_b_blk(last: last, is: is0) = blk} | bs],
            hs) do
  ...
  defp opt_is([r_b_set(op: :call, dst: dst,
               args: [r_b_remote(mod: r_b_literal(val: :erlang), name: r_b_literal(val: :throw),
                          arity: 1),
                          _Term]) = i0,
               r_b_set(op: {:succeeded, :body}, args: [dst]) = succ],
            r_b_br(), hs) do
  ...
  defp opt_throw([{:tentative, start, vars, blocks} | hs],
            thrownType, i) do
    case (opt_is_suitable(start, blocks, vars,
                            thrownType)) do
      true ->
        opt_throw(hs, thrownType, i)
      false ->
  ...
  defp opt_throw([], _ThrownType, r_b_set(args: [_, reason]) = i) do
    mFA = r_b_remote(mod: r_b_literal(val: :erlang), name: r_b_literal(val: :raise),
              arity: 3)
  ...
  defp ois_successors(r_b_br(bool: bool, succ: succ, fail: fail), ts) do
    case (:beam_types.get_singleton_value(ois_get_type(bool,
                                                         ts))) do
      {:ok, true} ->
        [succ]
      {:ok, false} ->
  ...
  defp ois_init_ts({class, reason, stacktrace}, thrownType) do
    ts = %{class => :beam_types.make_atom(:throw),
             reason => thrownType, stacktrace => :any}
    :maps.remove(:none, ts)
  end
  ...
  defp ois_is([r_b_set(op: :get_hd, dst: dst, args: [src]) | is],
            ts0) do
    srcType = ois_get_type(src, ts0)
    {type, _, _} = :beam_call_types.types(:erlang, :hd,
                                            [srcType])
    ts = Map.put(ts0, dst, type)
    ois_is(is, ts)
  end

  defp ois_is([r_b_set(op: :get_tl, dst: dst, args: [src]) | is],
            ts0) do
    srcType = ois_get_type(src, ts0)
    {type, _, _} = :beam_call_types.types(:erlang, :tl,
                                            [srcType])
    ts = Map.put(ts0, dst, type)
    ois_is(is, ts)
  end

  defp ois_is([r_b_set(op: :get_tuple_element, dst: dst,
               args: [src, r_b_literal(val: offset)]) |
               is],
            ts0) do
    type = (case (ts0) do
  ...
  defp ois_is([r_b_set(op: {:bif, :is_atom}, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: {:bif, :is_bitstring}, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: {:bif, :is_binary}, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: {:bif, :is_float}, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: {:bif, :is_integer}, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: {:bif, :is_list}, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: {:bif, :is_map}, dst: dst, args: [src]) |
               is],
            ts) do
    ois_type_test(src, dst, r_t_map(), is, ts)
  end

  defp ois_is([r_b_set(op: {:bif, :is_number}, dst: dst,
               args: [src]) |
               is],
            ts) do
    ois_type_test(src, dst, r_t_number(), is, ts)
  end

  defp ois_is([r_b_set(op: {:bif, :is_tuple}, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: :is_nonempty_list, dst: dst,
               args: [src]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: :is_tagged_tuple, dst: dst,
               args: [src, size, tag]) |
               is],
            ts) do
  ...
  defp ois_is([r_b_set(op: {:bif, :tuple_size}, dst: dst,
               args: [src]) |
               is],
            ts0) do
    srcType = ois_get_type(src, ts0)
    {type, _, _} = :beam_call_types.types(:erlang,
                                            :tuple_size, [srcType])
    ts = Map.put(ts0, dst, type)
    ois_is(is, ts)
  end

  defp ois_is([r_b_set(op: {:bif, :"=:="}, dst: dst,
               args: [lHS, r_b_literal(val: rHS)]) |
               is],
            ts0) do
    type = (case (:beam_types.get_singleton_value(ois_get_type(lHS,
                                                                 ts0))) do
              {:ok, ^rHS} ->
  ...
  defp ois_type_test(src, dst, requiredType, is, ts0) do
    ...
    type = (case (:beam_types.meet(givenType,
                                     requiredType)) do
              ^givenType ->
                :beam_types.make_atom(true)
              :none ->
