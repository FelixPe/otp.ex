defmodule :m_core_lint do
  ...
  def format_error({:arg_mismatch, {f, a}}) do
    :io_lib.format(~c"argument count mismatch in ~w/~w", [f, a])
  end

  def format_error({:illegal_expr, {f, a}}) do
    :io_lib.format(~c"illegal expression in ~w/~w", [f, a])
  end

  def format_error({:illegal_guard, {f, a}}) do
    :io_lib.format(~c"illegal guard expression in ~w/~w", [f, a])
  end

  def format_error({:illegal_try, {f, a}}) do
    :io_lib.format(~c"illegal try expression in ~w/~w", [f, a])
  end

  def format_error({:not_bs_pattern, {f, a}}) do
    :io_lib.format(~c"expecting bit syntax pattern in ~w/~w", [f, a])
  end

  def format_error({:not_pattern, {f, a}}) do
    :io_lib.format(~c"expecting pattern in ~w/~w", [f, a])
  end

  def format_error({:not_var, {f, a}}) do
    :io_lib.format(~c"expecting variable in ~w/~w", [f, a])
  end

  def format_error({:pattern_mismatch, {f, a}}) do
    :io_lib.format(~c"pattern count mismatch in ~w/~w", [f, a])
  end

  def format_error({:return_mismatch, {f, a}}) do
    :io_lib.format(~c"return count mismatch in ~w/~w", [f, a])
  end

  def format_error({:undefined_function, {f, a}}) do
    :io_lib.format(~c"function ~w/~w undefined", [f, a])
  end

  def format_error({:duplicate_var, n, {f, a}}) do
    :io_lib.format(~c"duplicate variable ~p in ~w/~w", [n, f, a])
  end

  def format_error({:unbound_var, n, {f, a}}) do
    :io_lib.format(~c"unbound variable ~p in ~w/~w", [n, f, a])
  end

  def format_error({:undefined_function, {f1, a1}, {f2, a2}}) do
    :io_lib.format(~c"undefined function ~w/~w in ~w/~w", [f1, a1, f2, a2])
  end

  def format_error({:tail_segment_not_at_end, {f, a}}) do
    :io_lib.format(~c"binary tail segment not at end in ~w/~w", [f, a])
  end
