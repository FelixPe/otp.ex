defmodule :m_beam_ssa_check do
  ...
  defp function(tag, f) do
    run_checks(:beam_ssa.get_anno(:ssa_checks, f, []), f,
                 tag)
  end

  defp run_checks([{:ssa_check_when, wantedResult,
              {:atom, _, tag}, args, exprs} |
               checks],
            f, tag) do
    check_function(args, exprs, wantedResult,
                     f) ++ run_checks(checks, f, tag)
  end
  ...
  defp check_function(checkArgs, exprs, {:atom, loc, :pass},
            r_b_function(args: _Args) = f) do
    run_check(checkArgs, exprs, loc, f)
  end

  defp check_function(checkArgs, exprs, {:atom, loc, key},
            r_b_function(args: _Args) = f)
  ...
  defp check_function(_, _, {:atom, loc, result}, f) do
    ...
    [{file,
        [{loc, :beam_ssa_check,
            {:unknown_result_kind, result}}]}]
  end

  defp run_check(checkArgs, exprs, loc, r_b_function(args: funArgs) = f) do
    init_and_run_check(checkArgs, funArgs, %{}, loc, exprs,
                         f)
  end

  defp init_and_run_check([{:var, loc, :_} | checkArgs], [r_b_var() | funArgs],
            env, _, exprs, f) do
    init_and_run_check(checkArgs, funArgs, env, loc, exprs,
                         f)
  end

  defp init_and_run_check([{:var, loc, checkV} | checkArgs],
            [v = r_b_var() | funArgs], env, _, exprs, f) do
    init_and_run_check(checkArgs, funArgs,
                         Map.put(env, checkV, v), loc, exprs, f)
  end
  ...
  defp init_and_run_check([], _, _Env, loc, _Exprs, f) do
    {file, _} = :beam_ssa.get_anno(:location, f)
    [{file,
        [{loc, :beam_ssa_check, :too_few_pattern_args}]}]
  end

  defp init_and_run_check(_, [], _Env, loc, _Exprs, f) do
    {file, _} = :beam_ssa.get_anno(:location, f)
    [{file,
        [{loc, :beam_ssa_check, :too_many_pattern_args}]}]
  end

  defp check_exprs(exprs, env, r_b_function(bs: blocks) = f) do
    code = :lists.foldr(fn lbl, acc ->
                             r_b_blk(is: is, last: last) = :erlang.map_get(lbl,
                                                                       blocks)
  ...
  defp check_expr_seq([{:check_expr, loc, args, anno} |
               rest] = checks,
            [first | code], env0, lastMatchedLoc, file) do
    env = (try do
             :skip
  ...
  defp check_expr_seq([{:check_expr, loc, args, _} | _], [], env,
            lastMatchedLoc, file) do
    [{file,
        [{loc, :beam_ssa_check,
            {:no_match, args, lastMatchedLoc, env}}]}]
  end

  defp op_check([:set, result, {:atom, _, op} | pArgs], pAnno,
  ...
  defp op_check([:set, result, {{:atom, _, :bif},
                            {:atom, _, op}} |
                             pArgs],
            pAnno,
            r_b_set(dst: dst, args: aArgs, op: {:bif, op},
                anno: aAnno) = _I,
            env0) do
    :skip
    env = check_annos(pAnno, aAnno, env0)
  ...
  defp op_check([:none, {:atom, _, :ret} | pArgs], pAnno,
            r_b_ret(arg: aArg, anno: aAnno) = _I, env0) do
    :skip
    env = check_annos(pAnno, aAnno, env0)
    post_args(pArgs, [aArg], env)
  end

  defp op_check([:none, {:atom, _, :br} | pArgs], pAnno,
            r_b_br(bool: aBool, succ: aSucc, fail: aFail,
                anno: aAnno) = _I,
            env0) do
    :skip
    env = check_annos(pAnno, aAnno, env0)
    post_args(pArgs, [aBool, r_b_literal(val: aSucc), r_b_literal(val: aFail)],
                env)
  end

  defp op_check([:none, {:atom, _, :switch}, pArg, pFail,
                                                 {:list, _, pArgs}],
            pAnno,
            r_b_switch(arg: aArg, fail: aFail, list: aList,
                anno: aAnno) = _I,
            env0) do
    :skip
    env1 = env_post(pArg, aArg,
                      env_post(pFail, r_b_literal(val: aFail), env0))
  ...
  defp post_args(pattern, args, _Env) do
    ...
    :erlang.error({:internal_pattern_match_error,
                     :post_args})
  end
  ...
  defp post_phi_args([{:tuple, _, [pVar, pLbl]} | pArgs],
            [{aVar, aLbl} | aArgs], env0) do
  ...
  defp post_switch_args([{:tuple, _, [pVal, pLbl]} | pArgs],
            [{aVal, aLbl} | aArgs], env0) do
    env = env_post(pVal, aVal,
                     env_post(pLbl, r_b_literal(val: aLbl), env0))
  ...
  defp env_post({:local_fun, {:atom, _, n}, {:integer, _, a}},
            r_b_local(name: r_b_literal(val: n), arity: a), env) do
    env
  end

  defp env_post({:external_fun, {:atom, _, m}, {:atom, _, n},
             {:integer, _, a}},
            r_b_remote(mod: r_b_literal(val: m), name: r_b_literal(val: n), arity: a), env) do
    env
  end

  defp env_post({:external_fun, {:atom, _, m}, {:atom, _, n},
             {:integer, _, a}},
            r_b_literal(val: f), env) do
  ...
  defp env_post({:float_epsilon, {:float, _, v},
             {:float, _, epsilon}},
            r_b_literal(val: actual), env) do
  ...
  defp env_post1(pV, actual, env) when :erlang.is_map_key(pV,
                                                     env) do
    :skip
  ...
  defp build_bitstring([{{:integer, _, v}, {:integer, _, n}} | bytes],
            acc) do
  ...
  defp check_annos([{:term, {:atom, _, key}, pTerm} | patterns],
            actual, env0) do
    :skip
    %{^key => aTerm} = actual
    :skip
  ...
  def format_error({:unknown_result_kind, result}) do
    'unknown expected result: ' ++ :erlang.atom_to_list(result)
  end
