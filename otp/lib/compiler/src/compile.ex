defmodule :m_compile do
  use Bitwise
  import :lists, only: [any: 2, flatmap: 2, flatten: 1,
                          foldr: 3, foreach: 2, keyfind: 3, last: 1, map: 2,
    ...
    case (:erlang.get(:compile__sub_pass_times)) do
      ...
      times when is_list(times) ->
        runner = fn name, run, s0 ->
                      ...
                      :erlang.put(:compile__sub_pass_times,
                                    [{name, t2 - t1} |
  ...
  defp do_compile(input, opts0) do
    ...
    case (:lists.member(:no_spawn_compiler_process,
                          opts)) do
      true ->
        intFun.()
      false ->
  ...
  defp expand_opts(opts0) do
    opts = (case ({:proplists.get_value(:debug_info_key,
                                          opts0),
    ...
    opts1 = (case (:proplists.is_defined(:makedep_side_effect,
                                           opts)) do
               true ->
  ...
  defp expand_opt(:r22, os) do
    expand_opt(:r23,
                 [:no_bs_create_bin, :no_shared_fun_wrappers, :no_swap |
                                                                  expand_opt(:no_bsm4,
                                                                               os)])
  end

  defp expand_opt(:r23, os) do
    expand_opt(:no_make_fun3,
                 [:no_bs_create_bin, :no_ssa_opt_float, :no_recv_opt,
                                                            :no_init_yregs |
                                                                expand_opt(:r24,
                                                                             os)])
  end
  ...
  defp expand_opt(:r24, os) do
    expand_opt(:no_type_opt,
                 [:no_bs_create_bin, :no_ssa_opt_ranges |
                                         expand_opt(:r25, os)])
  end
  ...
  defp expand_opt(:no_type_opt = o, os) do
    [o, :no_ssa_opt_type_start, :no_ssa_opt_type_continue,
  ...
  Record.defrecord(:r_compile, :compile, filename: '', dir: '',
                                   base: '', ifile: '', ofile: '', module: [],
                                   abstract_code: [], options: [],
                                   mod_options: [], encoding: :none, errors: [],
                                   warnings: [], extra_chunks: [])
  defp internal({:forms, forms}, opts0) do
    ...
    newForms = (case (with_columns(opts0)) do
                  true ->
                    forms
                  false ->
                    strip_columns(forms)
  ...
  defp build_compile(opts0) do
    ...
    r_compile(options: opts1, mod_options: opts1,
  ...
  defp internal_comp(passes, code0, file, suffix, st0) do
    ...
    st1 = r_compile(st0, filename: file,  dir: dir,  base: base, 
  ...
  defp fold_comp([{:delay, ps0} | passes], run, code,
            r_compile(options: opts) = st) do
  ...
  defp fold_comp([{name, pass} | ps], run, code0, st0) do
    try do
      ...
      :error, reason ->
        es = [{r_compile(st0, :ifile),
                 [{:none, :compile,
                     {:crash, name, reason, __STACKTRACE__}}]}]
  ...
  defp run_tc({name, fun}, code, st) do
    ...
    elapsed = :erlang.convert_time_unit(t2 - t1, :native,
                                          :microsecond)
  ...
  defp print_times(times0, name) do
    fam0 = rel2fam(times0)
    fam1 = (for {w, times} <- fam0 do
              {w, :lists.sum(times)}
            end)
    fam = reverse(:lists.keysort(2, fam1))
    total = (case (:lists.sum(for {_, t} <- fam do
                                t
                              end)) do
               0 ->
                 1
  ...
  defp print_times_1([{name, t} | ts], total) do
    elapsed = :erlang.convert_time_unit(t, :native,
                                          :microsecond)
  ...
  defp comp_ret_ok(code,
            r_compile(warnings: warn0, module: mod, options: opts) = st) do
    ...
    case (werror(st)) do
      ...
      false ->
        ...
        ret1 = (case (member(:binary, opts) and not
                                                member(:no_code_generation,
                                                         opts)) do
                  true ->
                    [code]
                  false ->
        ...
        ret2 = (case (member(:return_warnings, opts)) do
                  true ->
                    ret1 ++ [warn]
                  false ->
                    ret1
  ...
  defp comp_ret_err(r_compile(warnings: warn0, errors: err0,
              options: opts) = st) do
  ...
  defp messages_per_file(ms) do
    t = :lists.sort(for {file, messages} <- ms,
                          m <- messages do
                      {file, m}
                    end)
    prioMs = [:erl_scan, :epp, :erl_parse]
    {prio0, rest} = :lists.mapfoldl(fn m, a ->
                                         :lists.partition(fn {_, {_, mod, _}} ->
                                                               mod === m
                                                             _ ->
                                                               false
                                                          end,
  ...
  defp mpf(ms) do
    for file <- :lists.usort(for {f, _} <- ms do
                               f
                             end) do
      {file,
  ...
  defp passes_1([]) do
    {'.erl',
       [{:parse_module, &parse_module/2} | standard_passes()]}
  end
  ...
  defp select_passes([{:pass, mod} | ps], opts) do
    f = fn code0, st ->
             case (mod.module(code0, r_compile(st, :options))) do
               ...
               other ->
                 es = [{r_compile(st, :ifile),
                          [{:none, :compile, {:bad_return, mod, other}}]}]
  ...
  defp select_passes([{:done, ext} | _], opts) do
    select_passes([{:unless, :binary, {:listing, ext}}],
                    opts)
  end
  ...
  defp select_list_passes_1([{:iff, flag, {:listing, _} = listing} | ps],
            opts, acc) do
    case (member(flag, opts)) do
      true ->
  ...
  defp standard_passes() do
    [{:transform_module, &transform_module/2}, {:iff,
                                                  :makedep_side_effect,
                                                  {:makedep_and_output,
  ...
  defp abstr_passes(abstrStatus) do
    (case (abstrStatus) do
       ...
       :verified_abstr ->
         []
     end) ++ [{:compile_directives, &compile_directives/2},
                  {:delay,
                     [{:iff, :debug_info,
                         {:save_abstract_code, &save_abstract_code/2}}]},
                      {:expand_records, &expand_records/2}, {:iff, :dexp,
                                                               {:listing, 'expand'}},
                                                                {:iff, :E,
                                                                   {:legalize_vars,
                                                                      &legalize_vars/2}},
  ...
  defp core_passes(coreStatus) do
    (case (coreStatus) do
       :non_verified_core ->
         [{:core_lint_module, &core_lint_module/2}, {:unless,
                                                       :no_core_prepare,
                                                       {:pass,
                                                          :sys_core_prepare}},
                                                        {:iff, :dprep,
             ...
             {:core_lint_module, &core_lint_module/2}}]
                 ...
                 [{:unless, :no_copt,
                     [{:core_old_inliner, &test_old_inliner/1,
                         &core_old_inliner/2},
                          {:iff, :doldinline, {:listing, 'oldinline'}}, {:unless,
                                                                 :no_fold,
                                                                 {:pass,
                                                                    :sys_core_fold}},
                                                                  {:iff,
  ...
  defp kernel_passes() do
    [{:pass, :sys_core_bsm}, {:iff, :dcbsm, {:listing, 'core_bsm'}},
                                 {:iff, :clint,
                                    {:core_lint_module, &core_lint_module/2}},
                                     {:v3_kernel, &v3_kernel/2}, {:iff, :dkern,
                                                                    {:listing,
                                                                       'kernel'}},
                                                                     {:iff,
                                                                        :to_kernel,
  ...
  defp asm_passes() do
    [{:delay,
        [{:pass, :beam_a}, {:iff, :da, {:listing, 'a'}}, {:unless,
                                                          :no_postopt,
                                                          [{:pass, :beam_block},
                                                               {:iff, :dblk,
                                                                  {:listing,
         ...
         {:beam_validator_weak, &beam_validator_weak/2},
             {:beam_asm, &beam_asm/2}, {:iff, :strip_types,
                                          {:beam_strip_types,
                                             &beam_strip_types/2}} |
                                           binary_passes()]
  end
  ...
  defp binary_passes() do
    [{:iff, :to_dis, {:to_dis, &to_dis/2}}, {:unless,
                                               :binary,
                                               {:save_binary, &not_werror/1,
                                                  &save_binary/2}}]
  end
  ...
  Record.defrecord(:r_asm_module, :asm_module, module: :undefined,
                                      exports: :undefined, labels: :undefined,
  ...
  defp do_parse_module(defEncoding,
            r_compile(ifile: file, options: opts, dir: dir) = st) do
    sourceName0 = :proplists.get_value(:source, opts, file)
    sourceName = (case (member(:deterministic, opts)) do
                    true ->
    ...
    startLocation = (case (with_columns(opts)) do
                       true ->
                         {1, 1}
                       false ->
                         1
                     end)
    case (:erl_features.keyword_fun(opts,
                                      &:erl_scan.f_reserved_word/1)) do
      {:ok, {features, resWordFun}} ->
        r = :epp.parse_file(file,
                              [{:includes, ['.', dir | inc_paths(opts)]},
                                   {:source_name, sourceName}, {:deterministic,
        ...
        case (r) do
          {:ok, forms0, extra} ->
            ...
            forms = (case (with_columns(opts ++ compile_options(forms0))) do
                       true ->
                         forms0
                       false ->
  ...
  defp metadata_add_features(ftrs, r_compile(extra_chunks: extra) = st) do
    metaData = (case (:proplists.get_value("Meta", extra)) do
                  :undefined ->
                    []
                  bin ->
                    :erlang.binary_to_term(bin)
                end)
    oldFtrs = :proplists.get_value(:enabled_features,
                                     metaData, [])
    newFtrs = (ftrs -- oldFtrs) ++ oldFtrs
    metaData1 = :proplists.from_map(:maps.put(:enabled_features,
                                                newFtrs,
                                                :proplists.to_map(metaData)))
    extra1 = :proplists.from_map(:maps.put("Meta",
  ...
  defp with_columns(opts) do
    case (:proplists.get_value(:error_location, opts,
                                 :column)) do
      :column ->
        true
      :line ->
  ...
  defp clean_parse_transforms_1([{:attribute, l, :compile, c0} | fs], acc)
      when is_list(c0) do
    c = :lists.filter(fn {:parse_transform, _} ->
                           false
                         _ ->
                           true
                      end,
  ...
  defp clean_parse_transforms_1([{:attribute, _, :compile,
              {:parse_transform, _}} |
               fs],
            acc) do
  ...
  defp foldl_transform([t | ts], code0, st) do
    ...
    case (:code.ensure_loaded(t) === {:module,
                                        t} and :erlang.function_exported(t,
                                                                           :parse_transform,
                                                                           2)) do
      true ->
        fun = fn code, s ->
        ...
        try do
          ...
          {:error, es, ws} ->
            {:error,
               r_compile(st, warnings: r_compile(st, :warnings) ++ ws, 
      ...
      false ->
        es = [{r_compile(st, :ifile),
                 [{:none, :compile, {:undef_parse_transform, t}}]}]
        {:error, r_compile(st, errors: r_compile(st, :errors) ++ es)}
    end
  end
  ...
  defp maybe_strip_columns(code, t, st) do
    pTErrorLocation = (case (:erlang.function_exported(t,
                                                         :parse_transform_info,
                                                         0)) do
                         true ->
                           :maps.get(:error_location, t.parse_transform_info(),
                                       :column)
                         false ->
                           :column
                       end)
    configErrorLocation = :proplists.get_value(:error_location,
                                                 r_compile(st, :options), :column)
    cond do
  ...
  defp lint_module(code, st) do
    case (:erl_lint.module(code, r_compile(st, :ifile),
                             r_compile(st, :options))) do
      {:ok, ws} ->
  ...
  defp makedep(code0,
            r_compile(ifile: ifile, ofile: ofile, options: opts) = st) do
    target0 = (case (:proplists.get_value(:makedep_target,
                                            opts)) do
                 :undefined ->
    ...
    target1 = (case (:proplists.get_value(:makedep_quote_target,
                                            opts)) do
                 true ->
                   fun = fn ?$ ->
                              '$$'
    ...
    target = target1 ++ ':'
    {mainRule, phonyRules} = makedep_add_headers(ifile,
                                                   code0, [], length(target),
                                                   target, '', opts)
    makefile = (case (:proplists.get_value(:makedep_phony,
                                             opts)) do
                  true ->
                    mainRule ++ phonyRules
                  _ ->
  ...
  defp makedep_add_headers(ifile,
            [{:attribute, _, :file, {file, _}} | rest], included,
            lineLen, mainTarget, phony, opts) do
    {included1, lineLen1, mainTarget1,
       phony1} = makedep_add_header(ifile, included, lineLen,
                                      mainTarget, phony, file)
  ...
  defp makedep_add_headers(ifile,
            ...
            included, lineLen, mainTarget, phony, opts) do
    case (:proplists.get_value(:makedep_add_missing,
                                 opts)) do
      true ->
        {included1, lineLen1, mainTarget1,
           phony1} = makedep_add_header(ifile, included, lineLen,
  ...
  defp makedep_add_headers(ifile, [_ | rest], included, lineLen,
            mainTarget, phony, opts) do
    makedep_add_headers(ifile, rest, included, lineLen,
                          mainTarget, phony, opts)
  end

  defp makedep_add_headers(_Ifile, [], _Included, _LineLen, mainTarget,
            phony, _Opts) do
    {mainTarget, phony}
  end

  defp makedep_add_header(ifile, included, lineLen, mainTarget, phony,
            file) do
  ...
  defp makedep_output(code, r_compile(options: opts, ofile: ofile) = st) do
    output = (case (:proplists.get_value(:makedep_output,
                                           opts)) do
                :undefined ->
      ...
      is_list(output) ->
        case (:file.write_file(output, code)) do
          :ok ->
            {:ok, code, st}
          {:error, reason} ->
            err = {r_compile(st, :ifile),
                     [{:none, :compile, {:write_error, reason}}]}
            {:error, r_compile(st, errors: r_compile(st, :errors) ++ [err])}
        end
      true ->
        try do
          :io.fwrite(output, '~ts', [code])
        catch
          :error, _ ->
  ...
  defp legalize_vars(code0, st) do
    code = map(fn f = {:function, _, _, _, _} ->
  ...
  defp compile_directives(forms, r_compile(options: opts0) = st0) do
    opts1 = expand_opts(flatten(for {:attribute, _,
                                       :compile, c} <- forms do
                                  c
                                end))
    opts = opts1 ++ opts0
  ...
  defp core(forms, r_compile(options: opts) = st) do
    ...
    mod = :cerl.concrete(:cerl.module_name(core))
    {:ok, core,
       r_compile(st, module: mod,  options: opts, 
  ...
  defp save_abstract_code(code, st) do
    {:ok, code,
       r_compile(st, abstract_code: :erl_parse.anno_to_term(code))}
  end
    ...
    case (member(:encrypt_debug_info, opts2)) do
      true ->
        case (:lists.keytake(:debug_info_key, 1, opts2)) do
          {:value, {_, key}, opts3} ->
            encrypt_debug_info(debugInfo, key,
                                 [{:debug_info_key, :"********"} | opts3])
          false ->
            mode = :proplists.get_value(:crypto_mode, opts2,
                                          :des3_cbc)
            case (:beam_lib.get_crypto_key({:debug_info, mode,
                                              module, oFile})) do
              :error ->
  ...
  defp debug_info_chunk(r_compile(mod_options: modOpts0, options: compOpts,
              abstract_code: abst)) do
    abstOpts = cleanup_compile_options(modOpts0)
    {backend, metadata,
       modOpts} = (case (:proplists.get_value(:debug_info,
                                                compOpts, false)) do
                     ...
                     true ->
                       modOpts1 = :proplists.delete(:debug_info, modOpts0)
                       {:erl_abstract_code, {abst, abstOpts},
                          [:debug_info | modOpts1]}
                     false ->
                       {:erl_abstract_code, {:none, abstOpts}, modOpts0}
                   end)
    debugInfo = :erlang.term_to_binary({:debug_info_v1,
                                          backend, metadata},
                                         [:compressed])
    {debugInfo, modOpts}
  end
  ...
  defp generate_key({type, string}) when (is_atom(type) and
                                  is_list(string)) do
  ...
  defp encrypt({:des3_cbc = type, key, iVec, blockSize},
            bin0) do
    bin1 = (case (rem(byte_size(bin0), blockSize)) do
              0 ->
                bin0
              n ->
                :erlang.list_to_binary([bin0,
                                            :crypto.strong_rand_bytes(blockSize - n)])
            end)
    bin = :crypto.crypto_one_time(:des_ede3_cbc, key, iVec,
                                    bin1, true)
    typeString = :erlang.atom_to_list(type)
    :erlang.list_to_binary([0, length(typeString),
                                   typeString, bin])
  end
  ...
  defp beam_asm(code0,
            r_compile(ifile: file, extra_chunks: extraChunks,
                options: compilerOpts) = st) do
    case (debug_info(st)) do
      {:ok, debugInfo, opts0} ->
        opts1 = (for o <- opts0, effects_code_generation(o) do
                   o
                 end)
        chunks = [{"Dbgi", debugInfo} | extraChunks]
        compileInfo = compile_info(file, compilerOpts, opts1)
        {:ok, code} = :beam_asm.module(code0, chunks,
  ...
  defp beam_strip_types(beam0, r_compile() = st) do
    ...
    chunks = (for {tag, contents} <- chunks0, tag !== 'Type' do
                {tag, contents}
              end)
  ...
  defp compile_info(file, compilerOpts, opts) do
    ...
    info1 = (case (paranoid_absname(file)) do
               [_ | _] = source when (not isSlim and
                                        not isDeterministic)
                                     ->
                 [{:source, source} | info0]
               _ ->
  ...
  defp save_binary(code,
            r_compile(module: mod, ofile: outfile, options: opts) = st) do
      ...
      false ->
        ...
        case (:erlang.atom_to_list(mod)) do
          ^base ->
            save_binary_1(code, st)
          _ ->
            es = [{r_compile(st, :ofile),
                     [{:none, :compile, {:module_name, mod, base}}]}]
            {:error, r_compile(st, errors: r_compile(st, :errors) ++ es)}
        end
    end
  end
      ...
      :ok ->
        case (:file.rename(tfile, ofile)) do
          ...
          {:error, renameError} ->
            es = [{ofile,
                     [{:none, :compile,
                         {:rename, tfile, ofile, renameError}}]}]
            _ = :file.delete(tfile)
            {:error, r_compile(st, errors: r_compile(st, :errors) ++ es)}
        end
      {:error, error} ->
        es = [{tfile,
                 [{:none, :compile, {:write_error, error}}]}]
        {:error, r_compile(st, errors: r_compile(st, :errors) ++ es)}
    end
  end

  defp write_binary(name, bin, st) do
    opts = (case (member(:compressed, r_compile(st, :options))) do
              true ->
                [:compressed]
              false ->
                []
  ...
  defp filter_warnings_1([{source, ws0} | t], ignore) do
    ws = (for w <- ws0, not ignore_warning(w, ignore) do
            w
          end)
  ...
  defp ignore_tags([:nowarn_opportunistic | _], _Ignore) do
    :sets.from_list([:failed, :ignored, :nomatch],
                      [{:version, 2}])
  end
  ...
  defp outfile(base, ext, opts) when is_list(ext) do
    obase = (case (keyfind(:outdir, 1, opts)) do
               {:outdir, odir} ->
  ...
  defp diffable_fix_function({:function, name, arity, entry0, is0},
            labelMap0) do
  ...
  defp diffable_label_map([{:label, old} | is], new, map, acc) do
    case (map) do
      %{^old => newLabel} ->
        diffable_label_map(is, new, map,
                             [{:label, newLabel} | acc])
      %{} ->
        diffable_label_map(is, new + 1, Map.put(map, old, new),
                             [{:label, new} | acc])
    end
  end
  ...
  def compile_asm(file0, _OutFile, opts) do
    file = shorten_filename(file0)
    case (file(file,
                 [:from_asm | make_erl_options(opts)])) do
      {:ok, _Mod} ->
        :ok
      other ->
        other
  ...
  def compile_core(file0, _OutFile, opts) do
    file = shorten_filename(file0)
    case (file(file,
                 [:from_core | make_erl_options(opts)])) do
      {:ok, _Mod} ->
        :ok
      other ->
        other
  ...
  def compile_abstr(file0, _OutFile, opts) do
    file = shorten_filename(file0)
    case (file(file,
                 [:from_abstr | make_erl_options(opts)])) do
      {:ok, _Mod} ->
        :ok
      other ->
        other
  ...
  defp make_erl_options(opts) do
    r_options(includes: includes, defines: defines, outdir: outdir,
        warning: warning, verbose: verbose, specific: specific,
        cwd: cwd) = opts
    options = (for _ <- [:EFE_DUMMY_GEN], verbose do
                 :verbose
               end) ++ (for _ <- [:EFE_DUMMY_GEN], warning !== 0 do
                          :report_warnings
                        end) ++ map(fn {name, value} ->
                                         {:d, name, value}
                                       name ->
                                         {:d, name}
                                    end,
                                      defines)
    options ++ [:report_errors, {:cwd, cwd}, {:outdir,
                                                outdir} |
                                                 for dir <- includes do
                                                   {:i, dir}
                                                 end] ++ specific
  ...
  defp pre_load() do
    l = [:beam_a, :beam_asm, :beam_block, :beam_call_types,
                                              :beam_clean, :beam_dict,
                                                               :beam_digraph,
                                                                   :beam_flatten,
                                                                       :beam_jump,
                                                                           :beam_kernel_to_ssa,
