defmodule :m_sys_core_fold do
  ...
  defp simplify_call(r_c_call(anno: anno0), :maps, :get, [key0, map, default]) do
    ...
    raise =
      r_c_primop(
        name: r_c_literal(val: :match_fail),
    ...
    cs = [
      r_c_clause(
        ...
        pats: [
          r_c_map(
            es: [r_c_map_pair(op: r_c_literal(val: :exact), key: key, val: value)],
            is_pat: true
          )
        ],
      ...
      r_c_clause(
        anno: anno,
        pats: [r_c_map(es: [], is_pat: true)],
        guard: r_c_literal(val: true),
        body: default
      ),
      r_c_clause(anno: anno, pats: [fail], guard: r_c_literal(val: true), body: raise)
    ]

    :cerl.ann_c_let(anno, [key], key0, r_c_case(anno: anno, arg: map, clauses: cs))
  end
  ...
  defp sub_new() do
    r_sub(v: :orddict.new(), s: :sets.new([{:version, 2}]), t: %{})
  end
  ...
  defp sub_set_name(v, val, r_sub(v: s, s: scope, t: tdb0) = sub) do
    ...
    r_sub(sub, v: :orddict.store(v, val, s), s: :sets.add_element(v, scope), t: tdb)
  end
  ...
  defp propagate_compiler_generated(from, to) do
    case is_compiler_generated(from) and not is_compiler_generated(to) do
      true ->
  ...
  defp opt_fun_call(r_c_let(vars: [r_c_var(name: v)], arg: r_c_fun() = funDef, body: body) = let) do
    try do
  ...
  defp do_opt_fun_call(
         v,
         funDef,
         r_c_let(
           arg: r_c_apply(op: r_c_var(name: v), args: callArgs),
           body: rest
         ) = let
       ) do
  ...
  defp simplify_fun_call(v, values, r_c_fun(vars: vars, body: funBody), callArgs) do
    case not :core_lib.is_var_used(
           v,
           values
         ) and length(vars) === length(callArgs) do
      true ->
        r_c_let(vars: vars, arg: :core_lib.make_values(callArgs), body: funBody)
  ...
  defp opt_let_1(r_c_let(vars: vs0, body: b0) = let, arg0, ctxt, sub0) do
  ...
  def format_error({:failed, {:eval_failure, call, reason}}) do
    flatten(
      :io_lib.format(
        ~c"~ts will fail with a '~p' exception",
  ...
  def format_error({:failed, :embedded_binary_size}) do
    ~c"binary construction will fail with a 'badarg' exception (field size for binary/bitstring greater than actual size)"
  end

  def format_error({:failed, {:embedded_unit, unit, size}}) do
    m =
      :io_lib.format(
        ~c"binary construction will fail with a 'badarg' exception (size ~p cannot be evenly divided by unit ~p)",
        [size, unit]
      )
  ...
  def format_error({:failed, :bad_unicode}) do
    ~c"binary construction will fail with a 'badarg' exception (invalid Unicode code point in a utf8/utf16/utf32 segment)"
  end

  def format_error({:failed, :bad_float_size}) do
    ~c"binary construction will fail with a 'badarg' exception (invalid size for a float segment)"
  end

  def format_error({:failed, :bad_map_update}) do
    ~c"map update will fail with a 'badmap' exception"
  end
  ...
  def format_error({:nomatch, {:shadow, line, {name, arity}}}) do
    m =
      :io_lib.format(
        ~c"this clause for ~ts/~B cannot match because a previous clause at line ~p always matches",
        [name, arity, line]
      )
  ...
  def format_error({:nomatch, :guard}) do
    ~c"this clause cannot match because its guard evaluates to 'false'"
  end

  def format_error({:nomatch, {:bit_syntax_truncated, signess, val, sz}}) do
    s =
      case signess do
        :signed ->
          ~c"a 'signed'"
    ...
    f =
      ~c"this clause cannot match because the value ~P will not fit in ~s binary segment of size ~p"
  ...
  def format_error({:nomatch, {:bit_syntax_unsigned, val}}) do
    f =
      ~c"this clause cannot match because the negative value ~P will never match the value of an 'unsigned' binary segment"

    flatten(:io_lib.format(f, [val, 10]))
  end

  def format_error({:nomatch, {:bit_syntax_size, sz}}) do
    f = ~c"this clause cannot match because '~P' is not a valid size for a binary segment"
    flatten(:io_lib.format(f, [sz, 10]))
  end

  def format_error({:nomatch, {:bit_syntax_type, val, type}}) do
    f = ~c"this clause cannot match because '~P' is not of the expected type '~p'"
    flatten(:io_lib.format(f, [val, 10, type]))
  end

  def format_error({:nomatch, {:bit_syntax_unicode, val}}) do
    f = ~c"this clause cannot match because the value ~p is not a valid Unicode code point"
    flatten(:io_lib.format(f, [val]))
  end

  def format_error({:nomatch, :no_clause}) do
    ~c"no clause will ever match"
  end

  def format_error({:nomatch, :clause_type}) do
    ~c"this clause cannot match because of different types/sizes"
  end
  ...
  def format_error({:ignored, {:no_effect, {:erlang, f, a}}}) do
    {fmt, args} =
      case :erl_internal.comp_op(f, a) do
        true ->
          {~c"use of operator ~p has no effect", [f]}

        false ->
          case :erl_internal.bif(f, a) do
            false ->
              {~c"the call to erlang:~p/~p has no effect", [f, a]}

            true ->
              {~c"the call to ~p/~p has no effect", [f, a]}
          end
      end
  ...
  def format_error({:ignored, {:result, call}}) do
    fmt =
      ~c"the result of ~ts is ignored (suppress the warning by assigning the expression to the _ variable)"

    flatten(:io_lib.format(fmt, [format_call(call, true)]))
  end

  def format_error({:ignored, :useless_building}) do
    ~c"a term is constructed, but never used"
  end
  ...
  defp format_call({mod, name, arity}, useProgressiveForm) do
    case is_operator(mod, name, arity) do
      true ->
        ...
        [str, :io_lib.format(~c" operator ~p/~p", [name, arity])]
      ...
      false ->
        ...
        case is_auto_imported(mod, name, arity) do
          true ->
            [str, :io_lib.format(~c" ~p/~p", [name, arity])]

          false ->
            [str, :io_lib.format(~c" ~p:~p/~p", [mod, name, arity])]
        end
    end
  end
