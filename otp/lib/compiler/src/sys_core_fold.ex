defmodule :m_sys_core_fold do
  ...
  import :cerl, only: [ann_c_cons: 3, ann_c_map: 3,
                         ann_c_tuple: 2]
  import :lists, only: [all: 2, any: 2, flatten: 1,
                          foldl: 3, foldr: 3, keyfind: 3, map: 2, mapfoldl: 3,
                          member: 2, reverse: 1, reverse: 2, unzip: 1]
  require Record
  ...
  Record.defrecord(:r_sub, :sub, v: [],
                               s: :sets.new([{:version, 2}]), t: %{},
                               in_guard: false, top: true)
  def module(r_c_module(defs: ds0) = mod, opts) do
    :erlang.put(:no_inline_list_funcs,
  ...
  defp expr(r_c_literal(val: val) = l, ctxt, sub) do
    case (ctxt) do
      :effect ->
        case (val) do
          [] ->
  ...
  defp expr(r_c_cons(anno: anno, hd: h0, tl: t0) = cons, ctxt,
            sub) do
  ...
  defp expr(r_c_map(anno: anno, arg: v0, es: es0) = map, ctxt,
            sub) do
    case (ctxt) do
      :effect ->
  ...
  defp expr(r_c_binary(segments: ss) = bin0, ctxt, sub) do
    case (ctxt) do
      :effect ->
        warn_useless_building(bin0, sub)
      :value ->
        :ok
    end
  ...
  defp expr(r_c_fun() = fun, :effect, sub) do
  ...
  defp expr(r_c_letrec(defs: fs0, body: b0) = letrec, ctxt, sub) do
    fs1 = map(fn {name, fb} ->
                   case (ctxt === :effect and is_fun_effect_safe(name,
                                                                   b0)) do
                     true ->
  ...
  defp expr(r_c_apply(anno: anno, op: op0, args: as0) = apply0, _,
            sub) do
    ...
    case (:cerl.is_data(op1) and not is_literal_fun(op1)) do
      ...
      true ->
        ...
        err = r_c_call(anno: anno, module: r_c_literal(val: :erlang),
  ...
  defp expr(r_c_primop(name: r_c_literal(val: :build_stacktrace)), :effect,
            _Sub) do
    void()
  end
  ...
  defp expr(r_c_catch(anno: anno, body: b), :effect, sub) do
    ...
    try = r_c_try(anno: anno, arg: b, vars: [var], body: var,
  ...
  defp expr(r_c_try(arg: e0, vars: [r_c_var(name: x)], body: r_c_var(name: x),
              handler: r_c_literal(val: false) = false__) = try,
            _, sub) do
  ...
  defp expr(r_c_try(anno: a, arg: e0, vars: vs0, body: b0,
              evars: evs0, handler: h0) = try,
            _, sub0) do
    ...
    case (is_safe_simple(e1)) do
      true ->
        expr(r_c_let(anno: a, vars: vs1, arg: e1, body: b1), :value,
               sub0)
      false ->
        {evs1, sub2} = var_list(evs0, sub0)
        h1 = body(h0, :value, sub2)
        r_c_try(try, arg: e1,  vars: vs1,  body: b1,  evars: evs1, 
                 handler: h1)
    end
  end

  defp expr(r_c_opaque() = o, :effect, _Sub) do
    o
  end
  ...
  defp ifes_1(fVar, r_c_bitstr(val: val, size: size, unit: unit),
            _Safe) do
  ...
  defp ifes_1(fVar, r_c_case(arg: arg, clauses: clauses), safe) do
    ifes_1(fVar, arg, false) and ifes_list(fVar, clauses,
                                             safe)
  end
  ...
  defp ifes_1(fVar, r_c_clause(pats: pats, guard: guard, body: body),
            safe) do
    ifes_list(fVar, pats, false) and ifes_1(fVar, guard,
                                              false) and ifes_1(fVar, body,
                                                                  safe)
  end
  ...
  defp ifes_1(fVar, r_c_letrec(defs: defs, body: body), safe) do
    funs = (for {_, fun} <- defs do
              fun
            end)
    ifes_list(fVar, funs, false) and ifes_1(fVar, body,
                                              safe)
  end
  ...
  defp ifes_1(fVar, r_c_map(arg: arg, es: elements), _Safe) do
    ifes_1(fVar, arg, false) and ifes_list(fVar, elements,
                                             false)
  end
  ...
  defp ifes_1(fVar, r_c_try(arg: arg, handler: handler, body: body),
            safe) do
    ifes_1(fVar, arg, false) and ifes_1(fVar, handler,
                                          safe) and ifes_1(fVar, body, safe)
  end
  ...
  defp pair_list(es, sub) do
    for e <- es do
      pair(e, sub)
    end
  end

  defp pair(r_c_map_pair(key: k0, val: v0) = pair, sub) do
    k = expr(k0, :value, sub)
    v = expr(v0, :value, sub)
  ...
  defp bitstr(r_c_bitstr(val: val, size: size) = binSeg, sub) do
    r_c_bitstr(binSeg, val: expr(val, sub), 
  ...
  defp is_safe_simple(r_c_call(module: r_c_literal(val: :erlang), name: r_c_literal(val: name),
              args: args))
    ...
    case (:erl_internal.bool_op(name, numArgs)) do
      ...
      false ->
        :erl_bifs.is_safe(:erlang, name,
                            numArgs) and (:erl_internal.comp_op(name,
                                                                  numArgs) or :erl_internal.new_type_test(name,
                                                                                                            numArgs))
    end
  end
  ...
  defp will_fail(r_c_call(module: r_c_literal(val: mod), name: r_c_literal(val: name),
              args: args)) do
  ...
  defp bin_un_utf_1([r_c_bitstr(val: r_c_literal(), type: r_c_literal(val: :utf8)) = h | t],
            anno) do
    bin_un_utf_eval(h, anno) ++ bin_un_utf_1(t, anno)
  end

  defp bin_un_utf_1([r_c_bitstr(val: r_c_literal(), type: r_c_literal(val: :utf16)) = h | t],
            anno) do
    bin_un_utf_eval(h, anno) ++ bin_un_utf_1(t, anno)
  end

  defp bin_un_utf_1([r_c_bitstr(val: r_c_literal(), type: r_c_literal(val: :utf32)) = h | t],
            anno) do
    bin_un_utf_eval(h, anno) ++ bin_un_utf_1(t, anno)
  end
    ...
    case (eval_binary(r_c_binary(anno: anno, segments: segments))) do
      r_c_literal(anno: ^anno, val: bytes) when is_binary(bytes) ->
        for b <- :erlang.binary_to_list(bytes) do
          r_c_bitstr(anno: anno, val: r_c_literal(anno: anno, val: b),
  ...
  defp eval_binary_1([r_c_bitstr(val: r_c_literal(val: val), size: r_c_literal(val: sz),
  ...
  defp eval_binary_2(acc, val, size, unit, :float, :little) do
    <<acc :: bitstring,
        val :: size(size * unit) - little - float>>
  end

  defp eval_binary_2(acc, val, size, unit, :float, :big) do
    <<acc :: bitstring,
        val :: size(size * unit) - big - float>>
  end
  ...
  defp useless_call(:effect,
            r_c_call(module: r_c_literal(val: mod), name: r_c_literal(val: name),
                args: args) = call) do
    a = length(args)
  ...
  defp fold_apply(apply, r_c_literal(val: fun), args)
    ...
    cond do
      arity === length(args) ->
        r_c_call(anno: r_c_apply(apply, :anno), module: r_c_literal(val: mod),
            name: r_c_literal(val: name), args: args)
      true ->
        apply
    end
  end
  ...
  defp call(r_c_call(args: as0) = call0, r_c_literal(val: m) = m0,
            r_c_literal(val: n) = n0, sub) do
    as1 = expr_list(as0, :value, sub)
  ...
  defp simplify_call(r_c_call(anno: anno0), :maps, :get,
            [key0, map, default]) do
    anno = [:compiler_generated | anno0]
    key = make_var(anno)
    value = make_var(anno)
    fail = make_var(anno)
    raise = r_c_primop(name: r_c_literal(val: :match_fail),
                args: [r_c_tuple(es: [r_c_literal(val: :badmap), fail])])
    cs = [r_c_clause(anno: anno,
              pats: [r_c_map(es: [r_c_map_pair(op: r_c_literal(val: :exact), key: key,
                                val: value)],
                         is_pat: true)],
              guard: r_c_literal(val: true), body: value),
              r_c_clause(anno: anno, pats: [r_c_map(es: [], is_pat: true)],
                  guard: r_c_literal(val: true), body: default),
                  r_c_clause(anno: anno, pats: [fail], guard: r_c_literal(val: true),
                      body: raise)]
    :cerl.ann_c_let(anno, [key], key0,
                      r_c_case(anno: anno, arg: map, clauses: cs))
  end
  ...
  defp eval_length(call, list, len) do
    ...
    r_c_call(anno: a, module: r_c_literal(anno: a, val: :erlang),
  ...
  defp eval_failure(call, reason) do
    classified = classify_call(call)
    add_warning(call,
                  {:failed, {:eval_failure, classified, reason}})
    call
  end
  ...
  defp clause(r_c_clause(pats: ps0) = cl, cexpr, ctxt, sub0) do
    try do
      ...
      :nomatch ->
        r_c_clause(cl, anno: [:compiler_generated], 
  ...
  defp clause_1(r_c_clause(guard: g0, body: b0) = cl, ps1, cexpr, ctxt,
            sub1) do
    gSub = (case ({cexpr, ps1, g0}) do
              {_, _, r_c_literal()} ->
                sub1
  ...
  defp let_substs(vs0, as0, sub0) do
    ...
    sub2 = sub_add_scope(for r_c_var(name: v) <- vs2 do
                           v
                         end,
                           sub1)
    {vs2, as1,
  ...
  defp let_subst_list([v | vs0], [a | as0], sub) do
  ...
  defp pattern(r_c_var() = pat, isub, osub) do
    case (sub_is_in_scope(pat, isub)) do
      true ->
        ...
        {pat1,
           sub_set_var(pat, pat1, sub_add_scope([v1], osub))}
      false ->
  ...
  defp map_pair_pattern_list(ps0, isub, osub0) do
    {ps, {_, osub}} = mapfoldl(&map_pair_pattern/2,
                                 {isub, osub0}, ps0)
    {ps, osub}
  end

  defp map_pair_pattern(r_c_map_pair(op: r_c_literal(val: :exact), key: k0, val: v0) = pair,
            {isub, osub0}) do
    k = expr(k0, isub)
  ...
  defp bin_pattern(r_c_bitstr(val: e0, size: size0) = pat0, isub, osub0) do
    size2 = (case ({size0, expr(size0, isub)}) do
  ...
  defp var_list(vs, sub0) do
    mapfoldl(fn r_c_var() = v, sub ->
                  pattern(v, sub, sub)
             end,
               sub0, vs)
  end

  defp bin_pat_warn(r_c_bitstr(type: r_c_literal(val: type), val: val0,
              size: r_c_literal(anno: sizeAnno, val: sz), unit: r_c_literal(val: unit),
              flags: fl) = pat) do
    case ({type, sz}) do
    ...
    case ({type, val0}) do
      ...
      {_, r_c_literal(val: val)} ->
        add_warning(pat,
                      {:nomatch, {:bit_syntax_type, val, type}})
        throw(:nomatch)
      {_, _} ->
        :ok
    end
    ...
    case ({type, val0}) do
      {:integer, r_c_literal(val: val)} when is_integer(val) ->
        case (signedness(fl)) do
          :unsigned when val < 0 ->
            add_warning(pat,
                          {:nomatch, {:bit_syntax_unsigned, val}})
            throw(:nomatch)
          _ ->
            :ok
        end
  ...
  defp bit_pat_warn_int(val, 0, :signed, pat) do
    cond do
      ...
      true ->
        add_warning(pat,
                      {:nomatch, {:bit_syntax_truncated, :signed, val, 0}})
        throw(:nomatch)
    end
  end
  ...
  defp bit_pat_warn_int(val, sz, :signed, pat) do
    cond do
      (val < 0 and val >>> (sz - 1) !== - 1) ->
        add_warning(pat,
                      {:nomatch, {:bit_syntax_truncated, :signed, val, sz}})
        throw(:nomatch)
  ...
  defp bit_pat_warn_int(val, sz, :unsigned, pat) do
    cond do
      ...
      true ->
        add_warning(pat,
                      {:nomatch, {:bit_syntax_truncated, :unsigned, val, sz}})
        throw(:nomatch)
    end
  end

  defp bit_pat_warn_unicode(u, _Pat) when (0 <= u and u <= 1114111) do
    :ok
  end
  ...
  defp sub_new() do
    r_sub(v: :orddict.new(), s: :sets.new([{:version, 2}]),
        t: %{})
  end
  ...
  defp sub_set_name(v, val, r_sub(v: s, s: scope, t: tdb0) = sub) do
    ...
    r_sub(sub, v: :orddict.store(v, val, s), 
             s: :sets.add_element(v, scope),  t: tdb)
  end

  defp sub_subst_var(r_c_var(name: v) = var, val0, r_sub(v: s0)) do
    val = propagate_compiler_generated(var, val0)
    [{v, val}] ++ (for {k, r_c_var(name: v1)} <- s0, v1 === v do
                     {k, val}
                   end)
  end

  defp sub_add_scope(vs, r_sub(s: scope0) = sub) do
    scope = foldl(fn v, s when is_integer(v) or is_atom(v)
                               ->
  ...
  defp sub_subst_scope(r_sub(v: s0, s: scope) = sub) do
    initial = (case (s0) do
                 [{negInt, _} | _] when (is_integer(negInt) and
                                           negInt < 0)
                                        ->
                   negInt - 1
                 _ ->
  ...
  defp propagate_compiler_generated(from, to) do
    case (is_compiler_generated(from) and not
                                          is_compiler_generated(to)) do
      true ->
  ...
  defp warn_no_clause_match(caseOrig, caseOpt) do
    ...
    case (any(fn c ->
                ...
                origCs) and all(&is_compiler_generated/1, optCs)) do
      true ->
  ...
  defp shadow_warning([c | cs], line, anno) do
    case (keyfind(:function, 1, anno)) do
      {:function, {name, arity}} ->
        add_warning(c,
                      {:nomatch, {:shadow, line, {name, arity}}})
      _ ->
  ...
  defp opt_bool_clauses([r_c_clause(pats: [r_c_literal(val: lit)],
               guard: r_c_literal(val: true)) = c |
               cs],
            seenT, seenF) do
  ...
  defp opt_bool_clauses([r_c_clause(pats: ps, guard: r_c_literal(val: true)) = c | cs],
            seenT, seenF) do
    case (ps) do
      [r_c_var()] ->
        throw(:impossible)
  ...
  defp opt_bool_not(r_c_case(arg: arg, clauses: cs0) = case0) do
    case (arg) do
      r_c_call(anno: anno, module: r_c_literal(val: :erlang),
          name: r_c_literal(val: :not), args: [expr]) ->
        cs = (for c <- cs0 do
                opt_bool_not_invert(c)
              end) ++ [r_c_clause(anno: [:compiler_generated],
                           pats: [r_c_var(name: :cor_variable)], guard: r_c_literal(val: true),
                           body: r_c_call(anno: anno, module: r_c_literal(val: :erlang),
                                     name: r_c_literal(val: :error),
  ...
  defp eval_case(r_c_case(arg: e,
              clauses: [r_c_clause(pats: ps0, guard: r_c_literal(val: true),
                            body: b)]) = case__,
            sub) do
    es = (case (:cerl.is_c_values(e)) do
            true ->
              :cerl.values_es(e)
            false ->
              [e]
  ...
  defp eval_case_warn(r_c_primop(anno: anno, name: r_c_literal(val: :match_fail),
              args: [_]) = core) do
  ...
  defp case_opt(arg, cs0, sub) do
    ...
    args0 = (case (:cerl.is_c_values(arg)) do
               false ->
                 [arg]
               true ->
                 :cerl.values_es(arg)
    ...
    {args, cs2} = case_opt_args(args0, cs1, sub, litExpr,
                                  [])
    cs = (for {[], c, ps, bs} <- cs2 do
            :cerl.update_c_clause(c, reverse(ps),
  ...
  defp case_opt_nomatch(e, [{[p | _], c, _, _} = current | cs],
            litExpr) do
  ...
  defp case_opt_data_1([{[p0 | ps0], c, psAcc, bs0} | cs], typeSig) do
    ...
    [{ps1 ++ ps0, c, psAcc, bs} | case_opt_data_1(cs,
                                                    typeSig)]
  end
  ...
  defp case_opt_data_2(p, typeSig, bs0) do
    case (case_analyze_pat(p)) do
      ...
      {[v | vs], :none} ->
        ...
        true = is_integer(arity) and 0 <= arity and arity <= 255
        vars = make_vars(ann, arity)
        data = :cerl.ann_make_data(ann, type, vars)
        bs = [{v, data} | (for var <- vs do
                             {var, v}
                           end) ++ bs0]
        {vars, bs}
  ...
  defp make_vars(a, max)
      when is_integer(max) and 0 <= max and max <= 255 do
    make_vars(a, 1, max)
  end
  ...
  defp opt_not_in_let(r_c_let(vars: [_] = vs0, arg: arg0,
              body: body0) = let) do
  ...
  defp opt_not_in_let_1(v, call, body) do
    case (call) do
      r_c_call(module: r_c_literal(val: :erlang), name: r_c_literal(val: :not),
          args: [r_c_var(name: ^v)]) ->
  ...
  defp opt_not_in_let_2(r_c_case(clauses: cs0) = case__, notCall) do
    ...
    cs = (for (r_c_clause(body: b) = c) <- cs0 do
            (
  ...
  defp invert_call(r_c_call(module: r_c_literal(val: :erlang), name: r_c_literal(val: name0),
              args: [_, _]) = call) do
  ...
  defp opt_bool_case_in_let_1([r_c_var(name: v)], arg, r_c_case(arg: r_c_var(name: v)) = case0,
            let, sub) do
  ...
  defp is_bool_expr(r_c_call(module: r_c_literal(val: :erlang), name: r_c_literal(val: name),
              args: args)) do
    numArgs = length(args)
    :erl_internal.comp_op(name,
                            numArgs) or :erl_internal.new_type_test(name,
                                                                      numArgs) or :erl_internal.bool_op(name,
                                                                                                          numArgs)
  end

  defp is_bool_expr(r_c_try(arg: e, vars: [r_c_var(name: x)], body: r_c_var(name: x),
              handler: r_c_literal(val: false))) do
    is_bool_expr(e)
  end
  ...
  defp is_safe_bool_expr_1(r_c_call(module: r_c_literal(val: :erlang),
              name: r_c_literal(val: :is_function), args: [a, r_c_literal(val: arity)]),
            _BoolVars)
  ...
  defp is_safe_bool_expr_1(r_c_call(module: r_c_literal(val: :erlang), name: r_c_literal(val: name),
              args: args),
            boolVars) do
    numArgs = length(args)
    case ((:erl_internal.comp_op(name,
                                   numArgs) or :erl_internal.new_type_test(name,
                                                                             numArgs)) and is_safe_simple_list(args)) do
      true ->
        true
      false ->
        :erl_internal.bool_op(name,
                                numArgs) and is_safe_bool_expr_list(args,
                                                                      boolVars)
    end
  end
  ...
  defp opt_fun_call(r_c_let(vars: [r_c_var(name: v)], arg: r_c_fun() = funDef,
              body: body) = let) do
    try do
      do_opt_fun_call(v, funDef, body)
    catch
      :impossible ->
        let
    else
  ...
  defp opt_fun_call(expr) do
    expr
  end

  defp do_opt_fun_call(v, funDef, r_c_apply(op: r_c_var(name: v), args: callArgs)) do
    values = :core_lib.make_values(callArgs)
    simplify_fun_call(v, values, funDef, callArgs)
  end

  defp do_opt_fun_call(v, funDef,
            r_c_let(arg: r_c_apply(op: r_c_var(name: v), args: callArgs),
                body: rest) = let) do
    values = :core_lib.make_values([rest | callArgs])
    inlined = simplify_fun_call(v, values, funDef, callArgs)
    r_c_let(let, arg: inlined)
  end

  defp do_opt_fun_call(v, funDef,
            r_c_seq(arg: r_c_apply(op: r_c_var(name: v), args: callArgs),
                body: rest) = seq) do
    values = :core_lib.make_values([rest | callArgs])
    inlined = simplify_fun_call(v, values, funDef, callArgs)
    r_c_seq(seq, arg: inlined)
  end

  defp do_opt_fun_call(_, _, _) do
    :impossible
  end

  defp simplify_fun_call(v, values, r_c_fun(vars: vars, body: funBody),
            callArgs) do
    case (not
          :core_lib.is_var_used(v,
                                  values) and length(vars) === length(callArgs)) do
      true ->
        r_c_let(vars: vars, arg: :core_lib.make_values(callArgs),
            body: funBody)
      false ->
        throw(:impossible)
    end
  ...
  defp opt_build_stacktrace(r_c_let(vars: [r_c_var(name: cooked)],
              arg: r_c_primop(name: r_c_literal(val: :build_stacktrace), args: [rawStk]),
              body: body) = let) do
    case (body) do
      r_c_call(module: r_c_literal(val: :erlang), name: r_c_literal(val: :raise),
          args: [class, exp, r_c_var(name: ^cooked)]) ->
        case (:core_lib.is_var_used(cooked,
                                      r_c_cons(hd: class, tl: exp))) do
          true ->
            let
          false ->
      ...
      r_c_case(clauses: cs0) ->
        ...
        case (:core_lib.is_var_used(cooked, case__)) do
          false ->
            cs = (for (r_c_clause(body: b0) = c) <- cs0 do
                    (
                      b = opt_build_stacktrace(r_c_let(let, body: b0))
                      r_c_clause(c, body: b)
                    )
  ...
  defp matches_data([r_c_var(name: v)],
            r_c_case(arg: r_c_var(name: v), clauses: [r_c_clause(pats: [p]) | _])) do
    case (:cerl.is_data(p)) do
      false ->
        :no
      true ->
  ...
  defp delay_build_expr(core, {type, arity} = typeSig) do
    case (:cerl.is_data(core)) do
      ...
      true ->
        case ({:cerl.data_type(core),
                 :cerl.data_arity(core)}) do
          {^type, ^arity} ->
  ...
  defp opt_let_0(r_c_let(arg: arg0) = let, ctxt, sub) do
    arg = body(arg0, :value, sub)
    case (will_fail(arg)) do
      true ->
        arg
      false ->
        opt_let_1(let, arg, ctxt, sub)
    end
  end

  defp opt_let_1(r_c_let(vars: vs0, body: b0) = let, arg0, ctxt,
            sub0) do
    {vs, args, sub1} = let_substs(vs0, arg0, sub0)
    bodySub = update_let_types(vs, args, sub1)
    sub = r_sub(sub1, v: [],  s: :sets.new([{:version, 2}]))
    b = body(b0, ctxt, bodySub)
    arg = :core_lib.make_values(args)
    opt_let_2(let, vs, arg, b, b0, sub)
  end

  defp opt_let_2(let0, vs0, arg0, body, prevBody, sub) do
  ...
  defp move_case_into_arg(r_c_case(arg: r_c_let(vars: outerVars0, arg: outerArg,
                     body: innerArg0) = outer,
              clauses: innerClauses) = inner,
            sub) do
    scopeSub0 = sub_subst_scope(r_sub(sub, t: %{}))
    {outerVars, scopeSub} = var_list(outerVars0, scopeSub0)
    innerArg = body(innerArg0, scopeSub)
    r_c_let(outer, vars: outerVars,  arg: outerArg, 
               body: r_c_case(inner, arg: innerArg,  clauses: innerClauses))
  end

  defp move_case_into_arg(r_c_case(arg: r_c_case(arg: outerArg,
                     clauses: [outerCa0, outerCb]) = outer,
              clauses: innerClauses) = inner0,
            sub) do
    case (is_failing_clause(outerCb)) do
      true ->
        r_c_clause(pats: outerPats0, guard: outerGuard0,
            body: innerArg0) = outerCa0
        scopeSub0 = sub_subst_scope(r_sub(sub, t: %{}))
        {outerPats, scopeSub} = pattern_list(outerPats0,
                                               scopeSub0)
        outerGuard = guard(outerGuard0, scopeSub)
        innerArg = body(innerArg0, scopeSub)
        inner = r_c_case(inner0, arg: innerArg,  clauses: innerClauses)
        outerCa = r_c_clause(outerCa0, pats: outerPats, 
                                guard: outerGuard,  body: inner)
  ...
  defp move_case_into_arg(r_c_case(arg: r_c_seq(arg: outerArg, body: innerArg) = outer,
              clauses: innerClauses) = inner,
            _Sub) do
    r_c_seq(outer, arg: outerArg, 
               body: r_c_case(inner, arg: innerArg,  clauses: innerClauses))
  end
  ...
  defp descend(_Core, r_sub(top: false) = sub) do
    sub
  end

  defp descend(core, r_sub(top: true) = sub) do
  ...
  defp warn_useless_building(core, r_sub(top: top)) do
    case (top) do
      true ->
        add_warning(core, {:ignored, :useless_building})
      false ->
        :ok
    end
  end
    ...
    case (should_suppress_warning(core)) do
      ...
      false ->
        ...
        case (:erlang.get(key)) do
          [{^file, [{^location, :sys_core_fold, ^term}]} | _] ->
            :ok
          ws ->
            :erlang.put(key,
                          [{file, [{location, :sys_core_fold, term}]} | ws])
        end
    end
  end
  ...
  defp get_line([{line, _Column} | _T]) when is_integer(line) do
    line
  end
  ...
  defp get_warnings() do
    :ordsets.from_list(:erlang.erase({:sys_core_fold,
                                        :warnings}))
  end
  ...
  def format_error({:failed, {:eval_failure, call, reason}}) do
    flatten(:io_lib.format('~ts will fail with a \'~p\' exception',
                             [format_call(call, false), reason]))
  end

  def format_error({:failed, :embedded_binary_size}) do
    'binary construction will fail with a \'badarg\' exception (field size for binary/bitstring greater than actual size)'
  end

  def format_error({:failed, {:embedded_unit, unit, size}}) do
    m = :io_lib.format('binary construction will fail with a \'badarg\' exception (size ~p cannot be evenly divided by unit ~p)', [size, unit])
    flatten(m)
  end

  def format_error({:failed, :bad_unicode}) do
    'binary construction will fail with a \'badarg\' exception (invalid Unicode code point in a utf8/utf16/utf32 segment)'
  end

  def format_error({:failed, :bad_float_size}) do
    'binary construction will fail with a \'badarg\' exception (invalid size for a float segment)'
  end

  def format_error({:failed, :bad_map_update}) do
  ...
  def format_error({:nomatch, {:shadow, line, {name, arity}}}) do
    m = :io_lib.format('this clause for ~ts/~B cannot match because a previous clause at line ~p always matches', [name, arity, line])
    flatten(m)
  end
  ...
  def format_error({:nomatch, :guard}) do
    'this clause cannot match because its guard evaluates to \'false\''
  end

  def format_error({:nomatch,
            {:bit_syntax_truncated, signess, val, sz}}) do
    s = (case (signess) do
           :signed ->
             'a \'signed\''
           :unsigned ->
    ...
    f = 'this clause cannot match because the value ~P will not fit in ~s binary segment of size ~p'
    flatten(:io_lib.format(f, [val, 10, s, sz]))
  end

  def format_error({:nomatch, {:bit_syntax_unsigned, val}}) do
    f = 'this clause cannot match because the negative value ~P will never match the value of an \'unsigned\' binary segment'
    flatten(:io_lib.format(f, [val, 10]))
  end

  def format_error({:nomatch, {:bit_syntax_size, sz}}) do
    f = 'this clause cannot match because \'~P\' is not a valid size for a binary segment'
    flatten(:io_lib.format(f, [sz, 10]))
  end

  def format_error({:nomatch, {:bit_syntax_type, val, type}}) do
    f = 'this clause cannot match because \'~P\' is not of the expected type \'~p\''
    flatten(:io_lib.format(f, [val, 10, type]))
  end

  def format_error({:nomatch, {:bit_syntax_unicode, val}}) do
    f = 'this clause cannot match because the value ~p is not a valid Unicode code point'
    flatten(:io_lib.format(f, [val]))
  end

  def format_error({:nomatch, :no_clause}) do
    'no clause will ever match'
  end

  def format_error({:nomatch, :clause_type}) do
    'this clause cannot match because of different types/sizes'
  end

  def format_error({:ignored, {:no_effect, {:erlang, f, a}}}) do
    {fmt, args} = (case (:erl_internal.comp_op(f, a)) do
                     true ->
  ...
  def format_error({:ignored, {:result, call}}) do
    fmt = 'the result of ~ts is ignored (suppress the warning by assigning the expression to the _ variable)'
    flatten(:io_lib.format(fmt, [format_call(call, true)]))
  end

  def format_error({:ignored, :useless_building}) do
    'a term is constructed, but never used'
  end

  defp format_call({:erlang, :make_fun, 3}, _) do
    'fun construction'
  end

  defp format_call({mod, name, arity}, useProgressiveForm) do
    case (is_operator(mod, name, arity)) do
      true ->
        str = (case (useProgressiveForm) do
                 true ->
                   'evaluating'
                 false ->
                   'evaluation of'
      ...
      false ->
        ...
        case (is_auto_imported(mod, name, arity)) do
          true ->
  ...
  defp is_operator(:erlang, name, arity) do
    try do
  ...
  defp is_operator(_, _, _) do
    false
  end

  defp is_auto_imported(:erlang, name, arity) do
    :erl_internal.bif(name, arity)
  end

  defp is_auto_imported(_, _, _) do
    false
  end
