defmodule :m_sys_pre_attributes do
  ...
  def parse_transform(forms, options) do
    ...
    report_verbose(~c"Pre  options: ~p~n", [r_state(s2, :pre_ops)], s2)
    report_verbose(~c"Post options: ~p~n", [r_state(s2, :post_ops)], s2)
    s3 = pre_transform(s2)
    ...
    case h do
      {:attribute, anno, name, val} ->
        case :lists.keyfind(name, 2, r_state(s, :pre_ops)) do
          ...
          {:replace, ^name, newVal} ->
            report_warning(~c"Replace attribute ~p: ~p -> ~p~n", [name, val, newVal], s)
          ...
          {:delete, ^name} ->
            report_warning(~c"Delete attribute ~p: ~p~n", [name, val], s)
  ...
  defp attrs([{:replace, name, newVal} | t], anno, s) do
    report_verbose(~c"Insert attribute ~p: ~p~n", [name, newVal], s)
  ...
  defp report_warning(format, args, s) do
    case is_warning(s) do
      true ->
        :io.format(~c"~p: * WARNING * " ++ format, [:sys_pre_attributes | args])
  ...
  defp report_verbose(format, args, s) do
    case is_verbose(s) do
      true ->
        :io.format(~c"~p: " ++ format, [:sys_pre_attributes | args])
