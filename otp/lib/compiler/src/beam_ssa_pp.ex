defmodule :m_beam_ssa_pp do
  ...
  defp format_anno(key, map) when is_map(map) do
    ...
    [
      :io_lib.format(~c"%% ~s:\n", [key]),
      for {k, v} <- sorted do
        :io_lib.format(~c"%%    ~kw => ~kw\n", [k, v])
      end
    ]
  end

  defp format_anno(key, value) do
    :io_lib.format(~c"%% ~s: ~kp\n", [key, value])
  end
  ...
  defp format_param_info([info | infos], break) do
    [
      :io_lib.format(~c"~s~kp", [break, info])
  ...
  defp format_terminator(
    ...
    :io_lib.format(
      ~c"~s  ~sbr ~ts\n",
      [format_terminator_anno(a), format_i_number(a), format_label(same)]
    )
  end
  ...
  defp format_terminator(r_b_ret(anno: a, arg: arg), funcAnno) do
    :io_lib.format(
      ~c"~s  ~sret ~ts\n",
      [format_terminator_anno(a), format_i_number(a), format_arg(arg, funcAnno)]
    )
  end
  ...
  defp format_var_1(r_b_var(name: {name, uniq})) do
    cond do
      is_atom(name) ->
        :io_lib.format(~c"~ts:~p", [name, uniq])

      is_integer(name) ->
        :io_lib.format(~c"_~p:~p", [name, uniq])
    end
  end
  ...
  defp format_var_1(r_b_var(name: name)) when is_integer(name) do
    ~c"_" ++ :erlang.integer_to_list(name)
  end
  ...
  defp format_arg(r_b_local(name: name, arity: arity), funcAnno) do
    :io_lib.format(~c"(~ts/~p)", [format_arg(name, funcAnno), arity])
  end
  ...
  defp format_instr_anno(%{location: {file, line}} = anno0, funcAnno, args) do
    anno = :maps.remove(:location, anno0)

    [
      :io_lib.format(~c"  %% ~ts:~p\n", [file, line])
  ...
  defp format_instr_anno(%{result_type: t} = anno0, funcAnno, args) do
    ...
    [
      :io_lib.format(~c"  %% Result type:~s~s\n", [break, format_type(t, break)])
      | format_instr_anno(anno, funcAnno, args)
    ]
  end
  ...
  defp format_instr_anno(%{arg_types: ts} = anno0, funcAnno, args) do
    ...
    formatted0 =
      for {idx, arg} <- :lists.zip(iota, args),
      ...
      %{^dst => rs0} ->
        rs1 =
          for {start, end__} <- rs0 do
            :io_lib.format(~c"~p..~p", [start, end__])
          end

        rs = :lists.join(~c" ", rs1)
        :io_lib.format(~c"  %% ~ts: ~s\n", [format_var_1(dst), rs])
  ...
  def format_type(r_t_fun(arity: :any, type: t)) do
    [~c"fun((...) -> ", format_type(t), ~c")"]
  end

  def format_type(r_t_fun(arity: a, type: :any)) do
    [~c"fun((", format_fun_args(a), ~c"))"]
  end

  def format_type(r_t_fun(arity: a, type: t)) do
    [~c"fun((", format_fun_args(a), ~c") -> ", format_type(t), ~c")"]
  end
  ...
  def format_type(r_t_map(super_key: k, super_value: v)) do
    [~c"\#{", format_type(k), ~c"=>", format_type(v), ~c"}"]
  end
  ...
  def format_type(r_t_tuple(elements: es, exact: ex, size: s)) do
    [~c"{", :string.join(format_tuple_elems(s, ex, es, 1), ~c", "), ~c"}"]
  end
  ...
  def format_type(r_t_union(atom: a, list: l, number: n, tuple_set: ts, other: o)) do
    es =
      case a do
        :none ->
          []
  ...
  defp format_fun_args(a) do
    :string.join(:lists.duplicate(a, ~c"_"), ~c", ")
  end
  ...
  defp format_tuple_set(recordSet) do
    :string.join(
      for t <- :ordsets.to_list(recordSet) do
  ...
  defp format_tuple_set_1({{arity, key}, r_t_tuple(size: arity, elements: elems) = tuple}) do
