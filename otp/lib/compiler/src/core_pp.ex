defmodule :m_core_pp do
  ...
  defp maybe_anno(node, fun, ctxt, list) do
    ...
    [~c"( ", fun.(node, ctxt1), nl_indent(ctxt1), ~c"-| ", format_anno(list, ctxt2), ~c" )"]
  end
  ...
  defp format_funcs(fs, ctxt) do
    format_vseq(fs, ~c"", ~c"", ctxt, &format_def/2)
  end
  ...
  defp format_values(vs, ctxt) do
    [?<, format_hseq(vs, ~c",", add_indent(ctxt, 1), &format/2), ?>]
  end
  ...
  defp do_format_bitstr(
    ...
    [~c"#<", val, ~c">(", format_hseq(vs, ~c",", ctxt2, &format/2), ?)]
  end

  defp format_clauses(cs, ctxt) do
    format_vseq(cs, ~c"", ~c"", ctxt, &format_clause/2)
  end
  ...
  defp format_guard_1(r_c_call(module: m, name: n, args: as), ctxt0) do
    ...
    [
      ...
      nl_indent(ctxt3),
      ?(,
      format_vseq(as, ~c"", ~c",", add_indent(ctxt3, 1), &format_guard/2),
      ?)
    ]
  end
  ...
  defp format_list_tail(tail, ctxt) do
    [~c"|", format(tail, add_indent(ctxt, 1)), ~c"]"]
  end
