defmodule :m_wxe_server do
  ...
  def init([silentStart]) do
    ...
    {:ok, r_state(env: env, users: :gb_trees.empty(), cb: :gb_trees.empty(), cb_cnt: 1)}
  end
  ...
  def handle_call(_Request, _From, state) do
    log(~c"wxe_server", 132, ~c"Unknown request ~p sent to ~p from ~p ~n", [
      _Request,
      :wxe_server,
      _From
    ])
  ...
  def handle_cast(_Msg, state) do
    log(~c"wxe_server", 151, ~c"Unknown message ~p sent to ~p~n", [_Msg, :wxe_server])
    {:noreply, state}
  end
  ...
  def handle_info(_Info, state) do
    log(~c"wxe_server", 196, ~c"Unknown message ~p sent to ~p~n", [_Info, :wxe_server])
    {:noreply, state}
  end
  ...
  defp invoke_cb(
         {:_wx_invoke_cb_, funId, ev = r_wx(), ref = r_wx_ref()},
         _S
       ) do
    env = :erlang.get(:wx_env)
    ...
    case :erlang.get(funId) do
      ...
      err ->
        log(~c"wxe_server", 251, ~c"Internal Error ~p~n", [err])
  ...
  defp invoke_cb({:_wx_invoke_cb_, funId, args, _}, _S)
    ...
    case :erlang.get(funId) do
      ...
      err ->
        log(~c"wxe_server", 261, ~c"Internal Error ~p ~p ~p~n", [err, funId, args])
    ...
    cB = fn ->
      ...
      try do
        ...
        _, reason ->
          ...
          log(~c"wxe_server", 290, ~c"Callback fun crashed with {'EXIT, ~p, ~p}~n", [
            reason,
            __STACKTRACE__
          ])
      end
  ...
  defp invoke_callback_fun(r_wx_env(ref: envRef), fun, args, isEvent) do
    ...
    res =
      try do
        ...
        _, reason ->
          log(~c"wxe_server", 303, ~c"Callback fun crashed with {'EXIT, ~p, ~p}~n", [
            reason,
            __STACKTRACE__
          ])
