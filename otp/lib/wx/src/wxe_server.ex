defmodule :m_wxe_server do
  ...
  require Record
  Record.defrecord(:r_state, :state, env: :undefined,
                                 users: :undefined, cleaners: [],
                                 cb: :undefined, cb_cnt: :undefined)
  ...
  def start(silentStart) do
    case (:erlang.get(:wx_env)) do
      :undefined ->
        case (:gen_server.start(:wxe_server, [silentStart],
                                  [])) do
          {:ok, pid} ->
            {:ok, ref} = :gen_server.call(pid, :get_env, :infinity)
            :wx.set_env(env = r_wx_env(ref: ref, sv: pid))
            env
          {:error, {reason, _Stack}} ->
            :erlang.error(reason)
        end
  ...
  def init([silentStart]) do
    ...
    :erlang.put(:wx_env, r_wx_env(ref: env, sv: self()))
    {:ok,
       r_state(env: env, users: :gb_trees.empty(),
           cb: :gb_trees.empty(), cb_cnt: 1)}
  end

  def handle_call(:register_me, {from, _},
           state = r_state(users: users)) do
  ...
  def handle_call(:get_env, _, state = r_state(env: env)) do
    {:reply, {:ok, env}, state}
  end
  ...
  def handle_call(:stop, {_From, _},
           state = r_state(users: users0, cleaners: cs0)) do
    env = :erlang.get(:wx_env)
    users = :gb_trees.to_list(users0)
    cs = :lists.map(fn {_Pid, user} ->
                         spawn_link(fn () ->
                                         cleanup(env, [user])
                                    end)
                    end,
                      users)
    {:noreply,
       r_state(state, users: :gb_trees.empty(), 
                  cleaners: cs ++ cs0)}
  end
  ...
  def handle_call(_Request, _From, state) do
    log('wxe_server', 132, 'Unknown request ~p sent to ~p from ~p ~n', [_Request, :wxe_server, _From])
    reply = :ok
    {:reply, reply, state}
  end

  def handle_cast({:cleaned, from},
           state = r_state(users: users, cleaners: cs0)) do
  ...
  def handle_cast(_Msg, state) do
    log('wxe_server', 151, 'Unknown message ~p sent to ~p~n', [_Msg, :wxe_server])
    {:noreply, state}
  end

  def handle_info({:_wx_invoke_cb_, _, _, _} = cb, state) do
    invoke_cb(cb, state)
    {:noreply, state}
  end
  ...
  def handle_info({:wx_delete_cb, id, evtListener, obj},
           state = r_state(users: users)) do
  ...
  def handle_info({:DOWN, _, :process, pid, _},
           state = r_state(users: users0, cleaners: cs)) do
    try do
      ...
      case (user) do
        ...
        _ ->
          cleaner = spawn_link(fn () ->
                                    cleanup(env, [user])
                               end)
          {:noreply,
             r_state(state, users: users,  cleaners: [cleaner | cs])}
      end
    catch
      _E, _R ->
        {:noreply, state}
  ...
  def handle_info(_Info, state) do
    log('wxe_server', 196, 'Unknown message ~p sent to ~p~n', [_Info, :wxe_server])
    {:noreply, state}
  end

  def terminate(_Reason, _State) do
    :wxe_util.delete_env(case (:erlang.get(:wx_env)) do
                           r_wx_env(ref: _wx_Ref_) ->
                             _wx_Ref_
                           _ ->
                             :erlang.error({:wx, :unknown_env})
                         end)
    :shutdown
  end
  ...
  defp log(mod, line, str, args) do
    :logger.log(:error, str, args,
                  %{domain: [:wx], file: mod, line: line})
  end

  defp handle_connect(object,
            r_evh(handler: :undefined, cb: callback) = evData0, from,
            state0) do
    {funId, state} = attach_fun(callback, state0)
    evData1 = r_evh(evData0, cb: funId)
    case (:wxEvtHandler.connect_impl(object, evData1)) do
      {:ok, handler} ->
        evData = r_evh(evData1, handler: handler, 
                              userdata: :undefined)
  ...
  defp handle_connect(object, evData = r_evh(handler: handler), from,
            state0 = r_state(users: users)) do
    :erlang.put(handler, from)
    case (:gb_trees.lookup(from, users)) do
      {:value, user0 = r_user(events: listeners0)} ->
        user = r_user(user0, events: [{object, evData} | listeners0])
        state = r_state(state0, users: :gb_trees.update(from, user,
                                                    users))
        {:reply, :ok, state}
      :none ->
  ...
  defp invoke_cb({:_wx_invoke_cb_, funId, ev = r_wx(), ref = r_wx_ref()},
            _S) do
    env = :erlang.get(:wx_env)
    case (:erlang.get(funId)) do
      ...
      err ->
        log('wxe_server', 251, 'Internal Error ~p~n', [err])
        invoke_callback_fun(env,
                              fn () ->
                                   :ok
                              end,
                              [], true)
  ...
  defp invoke_cb({:_wx_invoke_cb_, funId, args, _}, _S)
      when (is_list(args) and is_integer(funId)) do
    env = :erlang.get(:wx_env)
    case (:erlang.get(funId)) do
      {fun, _} when is_function(fun) ->
        invoke_callback(env, fun, args, false)
      err ->
        log('wxe_server', 261, 'Internal Error ~p ~p ~p~n', [err, funId, args])
        invoke_callback_fun(env,
                              fn () ->
                                   :ok
                              end,
  ...
  defp invoke_callback(env, fun, args, isEvent) do
    spawn(fn () ->
               :wx.set_env(env)
  ...
  defp invoke_callback_obj(env, pid, ev, ref) do
    cB = fn () ->
              :wx.set_env(env)
              :wxe_util.queue_cmd(env, 9)
              try do
                ...
                _, reason ->
                  :wxEvent.skip(ref)
                  log('wxe_server', 290, 'Callback fun crashed with {\'EXIT, ~p, ~p}~n', [reason, __STACKTRACE__])
              end
              :wxe_util.queue_cmd(:ok, env, 11)
         end
    spawn(cB)
    :ok
  end

  defp invoke_callback_fun(r_wx_env(ref: envRef), fun, args, isEvent) do
    :wxe_util.queue_cmd(envRef, 9)
    res = (try do
             apply(fun, args)
           catch
             _, reason ->
               log('wxe_server', 303, 'Callback fun crashed with {\'EXIT, ~p, ~p}~n', [reason, __STACKTRACE__])
               :ok
           end)
    case (isEvent) do
      false ->
        :wxe_util.queue_cmd(res, envRef, 11)
      true ->
        :wxe_util.queue_cmd(:ok, envRef, 11)
    end
  end
  ...
  defp attach_fun(fun, s = r_state(cb: cB, cb_cnt: next)) do
    case (:gb_trees.lookup(fun, cB)) do
      ...
      :none ->
        ...
        {next,
           r_state(s, cb: :gb_trees.insert(fun, next, cB), 
                  cb_cnt: next + 1)}
    end
  end
  ...
  defp handle_disconnect(object, evh = r_evh(cb: fun), from,
            state0 = r_state(users: users0, cb: callbacks)) do
  ...
  defp match_handler(m = r_evh(et: mET, cb: mCB),
            r_evh(et: eT, cb: cB, handler: handler)) do
  ...
  defp cleanup(env, data) do
    ...
    :lists.foreach(fn r_user(events: evs) ->
                        for ev <- evs do
                          disconnect.(ev)
                        end
                   end,
