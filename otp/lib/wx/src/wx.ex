defmodule :m_wx do
  ...
  def new(options) when is_list(options) do
    ...
    silentStart = :proplists.get_value(:silent_start,
                                         options, false)
    level = calc_level(debug)
    r_wx_env() = :wxe_server.start(silentStart and level === 0)
    set_debug(level)
    null()
  end
  ...
  def set_env(r_wx_env(sv: pid) = env) do
  ...
  def subscribe_events() do
    :gen_server.call(:wxe_master, :subscribe_msgs,
                       :infinity)
  end
  ...
  def get_memory_bin(r_wx_mem(bin: bin, size: size)) do
    <<withCorrectSize :: size(size) - binary,
        _ :: binary>> = bin
    withCorrectSize
  end

  def retain_memory(r_wx_mem() = mem) do
    case (:erlang.get(mem)) do
      {^mem, n} ->
        :erlang.put(mem, n + 1)
      :undefined ->
        :erlang.put(mem, 1)
  ...
  def retain_memory(bin) when is_binary(bin) do
    ...
    retain_memory(r_wx_mem(bin: bin, size: byte_size(bin)))
  end

  def release_memory(r_wx_mem() = mem) do
    case (:erlang.erase(mem)) do
      1 ->
        :ok
      n ->
        :erlang.put(mem, n - 1)
  ...
  def release_memory(bin) when is_binary(bin) do
    release_memory(r_wx_mem(bin: bin, size: byte_size(bin)))
  end
  ...
  defp calc_level(list) when is_list(list) do
    {drv, erl} = :lists.foldl(fn :verbose, {drv, _Erl} ->
                                   {drv, 1}
                                 :trace, {drv, _Erl} ->
                                   {drv, 2}
                                 :driver, {_Drv, erl} ->
  ...
  defp set_debug(level) when is_integer(level) do
    case (get_env()) do
      r_wx_env(debug: old) when old === level ->
        :ok
      env = r_wx_env(sv: server, debug: old) ->
        cond do
          (old > 16 and level > 16) ->
            :ok
          (old < 16 and level < 16) ->
  ...
  def demo() do
    ...
    demo = :filename.join([:filename.dirname(priv),
                               :examples, :demo])
