defmodule :m_wx_object do
  ...
  Record.defrecord(:r_wxHtmlLinkInfo, :wxHtmlLinkInfo, href: :undefined,
                                          target: :undefined)
  def start(mod, args, options) when (is_atom(mod) and
                                     is_list(options)) do
    gen_response(:gen.start(:wx_object, :nolink, mod, args,
                              [:erlang.get(:wx_env) | options]))
  end

  def start(mod, args, options) do
    :erlang.error(:badarg, [mod, args, options])
  end

  def start(name, mod, args, options)
      when (is_tuple(name) and is_atom(mod) and
              is_list(options)) do
    gen_response(:gen.start(:wx_object, :nolink, name, mod,
                              args, [:erlang.get(:wx_env) | options]))
  end

  def start(name, mod, args, options) do
    :erlang.error(:badarg, [name, mod, args, options])
  end

  def start_link(mod, args, options) when (is_atom(mod) and
                                     is_list(options)) do
    gen_response(:gen.start(:wx_object, :link, mod, args,
                              [:erlang.get(:wx_env) | options]))
  end
  ...
  def start_link(name, mod, args, options)
      when (is_tuple(name) and is_atom(mod) and
              is_list(options)) do
    gen_response(:gen.start(:wx_object, :link, name, mod,
                              args, [:erlang.get(:wx_env) | options]))
  end
  ...
  def stop(ref = r_wx_ref(state: pid), reason, timeout)
    ...
    try do
      ...
      _, exitReason ->
        :erlang.error({exitReason,
                         {:wx_object, :stop, [ref, reason, timeout]}})
    end
  end
  ...
  def stop(name, reason, timeout)
    ...
    try do
      ...
      _, exitReason ->
        :erlang.error({exitReason,
                         {:wx_object, :stop, [name, reason, timeout]}})
    end
  end
  ...
  def call(ref = r_wx_ref(state: pid), request, timeout)
    ...
    try do
      ...
      _, reason ->
        :erlang.error({reason,
                         {:wx_object, :call, [ref, request, timeout]}})
    end
  end
  ...
  def call(name, request, timeout)
    ...
    try do
      ...
      _, reason ->
        :erlang.error({reason,
                         {:wx_object, :call, [name, request, timeout]}})
    end
  end
  ...
  def init_it(starter, parent, name, mod, args,
           [wxEnv | options]) do
    case (wxEnv) do
      :undefined ->
        :ok
    ...
    case ((try do
      ...
      {r_wx_ref() = ref, state} ->
        init_it2(ref, starter, parent, name, state, mod,
                   :infinity, debug)
  ...
  defp init_it2(ref, starter, parent, name, state, mod, timeout,
            debug) do
    ...
    case (try do
      ...
      true ->
        ...
        send(starter, {:started, self(), r_wx_ref(ref, state: self())})
  ...
  defp loop(parent, name, state, mod, time, debug) do
    ...
    case (msg) do
      {:system, from, req} ->
        :sys.handle_system_msg(req, from, parent, :wx_object,
                                 debug, [name, state, mod, time])
      {:EXIT, ^parent, reason} ->
  ...
  defp handle_msg(msg, parent, name, state, mod) do
    case ((try do
      ...
      {:EXIT,
         {:undef, [{^mod, :handle_info, [_, _], _} | _]}} ->
        handle_no_reply({:noreply, state}, parent, name, msg,
                          mod, state, [])
      reply ->
  ...
  defp handle_msg({:"$gen_call", from, msg}, parent, name, state, mod,
            debug) do
    case ((try do
            mod.handle_call(msg, from, state)
          catch
  ...
  defp handle_no_reply({:noreply, nState}, parent, name, _Msg, mod,
            _State, []) do
    loop(parent, name, nState, mod, :infinity, [])
  end

  defp handle_no_reply({:noreply, nState, time1}, parent, name, _Msg,
            mod, _State, []) do
    loop(parent, name, nState, mod, time1, [])
  end
  ...
  def system_terminate(reason, _Parent, debug,
           [name, state, mod, _Time]) do
    terminate(reason, name, [], mod, state, debug)
  end

  def system_code_change([name, state, mod, time], _Module, oldVsn,
           extra) do
    case ((try do
  ...
  defp error_info(_Reason, :application_controller, _Msg, _State,
            _Debug) do
    :ok
  end

  defp error_info(reason, name, msg, state, debug) do
    reason1 = (case (reason) do
                 {:undef, [{m, f, a, l} | mFAs]} ->
    ...
    format('** wx object server ~tp terminating \n** Last message in was ~tp~n** When Server state == ~tp~n** Reason for termination == ~n** ~tp~n', [name, msg, state, reason1])
    :sys.print_log(debug)
    :ok
  end
  ...
  def format_status(opt, statusData) do
    [pDict, sysState, parent, debug, [name, state, mod,
                                                       _Time]] = statusData
  ...
  defp format_log_state(mod, log) do
    for event <- log do
      case (event) do
        {:out, msg, from, state} ->
          {:out, msg, from,
             format_status(:terminate, mod, :erlang.get(), state)}
        {:noreply, state} ->
          {:noreply,
             format_status(:terminate, mod, :erlang.get(), state)}
        _ ->
