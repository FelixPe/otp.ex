defmodule :m_wx_object do
  ...
  defp init_it2(ref, starter, parent, name, state, mod, timeout, debug) do
    ...
    case (try do
      ...
      false ->
        exit({ref, ~c"not a wxWindow subclass"})
  ...
  defp print_event(dev, {:in, msg}, name) do
    case msg do
      {:"$gen_call", {from, _Tag}, call} ->
        :io.format(dev, ~c"*DBG* ~tp got call ~tp from ~w~n", [name, call, from])
  ...
  defp print_event(dev, {:out, msg, to, state}, name) do
    :io.format(dev, ~c"*DBG* ~tp sent ~tp to ~w, new state ~tp~n", [name, msg, to, state])
  end
  ...
  defp dbg_opts(name, opts) do
    case (try do
      ...
      {:EXIT, _} ->
        format(~c"~tp: ignoring erroneous debug options - ~tp~n", [name, opts])
        []
  ...
  def format_status(opt, statusData) do
    ...
    header = :gen.format_status_header(~c"Status for wx object ", name)
