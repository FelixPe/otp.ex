defmodule :m_public_key do
  ...
  def pem_entry_decode({:SubjectPublicKeyInfo, der, _}) do
    {_, {:AlgorithmIdentifier, algId, params},
       key0} = der_decode(:SubjectPublicKeyInfo, der)
  ...
  def pem_entry_decode({asn1Type, cryptDer, {cipher, r_PBES2_params()}} = pemEntry,
           password)
      when is_atom(asn1Type) and is_binary(cryptDer) and is_list(cipher) do
    do_pem_entry_decode(pemEntry, password)
  end

  def pem_entry_decode({asn1Type, cryptDer,
            {cipher, {r_PBEParameter(), _}}} = pemEntry,
           password)
      when is_atom(asn1Type) and is_binary(cryptDer) and is_list(cipher) and is_list(password) do
    do_pem_entry_decode(pemEntry, password)
  end

  def pem_entry_decode({asn1Type, cryptDer, {cipher, salt}} = pemEntry,
           password)
      when is_atom(asn1Type) and is_binary(cryptDer) and is_list(cipher) and is_binary(salt) and (:erlang.byte_size(salt) == 8 or :erlang.byte_size(salt) == 16) and is_list(password) do
    do_pem_entry_decode(pemEntry, password)
  end

  def pem_entry_encode(asn1Type, entity,
           {{cipher, r_PBES2_params()} = cipherInfo, password})
      when is_atom(asn1Type) and is_list(password) and is_list(cipher) do
    do_pem_entry_encode(asn1Type, entity, cipherInfo,
                          password)
  end

  def pem_entry_encode(asn1Type, entity,
           {{cipher, {r_PBEParameter(), _}} = cipherInfo, password})
      when is_atom(asn1Type) and is_list(password) and is_list(cipher) do
    do_pem_entry_encode(asn1Type, entity, cipherInfo,
                          password)
  end
  ...
  def pem_entry_encode(asn1Type, entity,
           {{cipher, salt} = cipherInfo, password})
      when is_atom(asn1Type) and is_list(password) and is_list(cipher) and is_binary(salt) and (:erlang.byte_size(salt) == 8 or :erlang.byte_size(salt) == 16) do
    do_pem_entry_encode(asn1Type, entity, cipherInfo,
                          password)
  end

  def der_decode(asn1Type, der)
      when (asn1Type == :PrivateKeyInfo or asn1Type == :EncryptedPrivateKeyInfo) and is_binary(der) do
    try do
  ...
  def der_decode(asn1Type, der) when (is_atom(asn1Type) and
                                is_binary(der)) do
    try do
  ...
  defp handle_pkcs_frame_error(:PrivateKeyInfo, der, _) do
    try do
      {:ok, decoded} = :PKCS-FRAME.decode(:OneAsymmetricKey,
                                            der)
      der_priv_key_decode(decoded)
    catch
      :error, {:badmatch, {:error, _}} = error ->
  ...
  def pkix_decode_cert(derCert, :otp) when is_binary(derCert) do
    try do
      {:ok,
         r_OTPCertificate() = cert} = :pubkey_cert_records.decode_cert(derCert)
      cert
    catch
      :error, {:badmatch, {:error, _}} = error ->
        :erlang.error(error)
  ...
  def decrypt_private(cipherText, r_RSAPrivateKey() = key, options)
    ...
    :crypto.private_decrypt(:rsa, cipherText,
  ...
  def decrypt_public(cipherText, r_RSAPublicKey(modulus: n, publicExponent: e),
           options)
      when (is_binary(cipherText) and is_list(options)) do
    :crypto.public_decrypt(:rsa, cipherText, [e, n],
                             default_options(options))
  end
  ...
  def encrypt_public(plainText, r_RSAPublicKey(modulus: n, publicExponent: e),
           options)
      when (is_binary(plainText) and is_list(options)) do
    :crypto.public_encrypt(:rsa, plainText, [e, n],
                             default_options(options))
  end
  ...
  def generate_key({:rsa, modulusSize, publicExponent}) do
    case (:crypto.generate_key(:rsa,
                                 {modulusSize, publicExponent})) do
      {[e, n],
         [e, n, d, p, q, d_mod_P_1, d_mod_Q_1, invQ_mod_P]} ->
        nint = :crypto.bytes_to_integer(n)
        eint = :crypto.bytes_to_integer(e)
        r_RSAPrivateKey(version: :"two-prime", modulus: nint, publicExponent: eint,
  ...
  def sign(digestOrPlainText, digestType, key, options) do
    case (format_sign_key(key)) do
      ...
      {algorithm, cryptoKey} ->
        try do
          :crypto.sign(algorithm, digestType, digestOrPlainText,
                         cryptoKey, options)
        catch
          :error, {:notsup, _, _} ->
            :erlang.error(:notsup)
  ...
  def verify(digest, :none, signature, key = {_, r_Dss_Parms()},
           options)
      when is_binary(digest) do
    verify({:digest, digest}, :sha, signature, key, options)
  end

  def verify(digestOrPlainText, digestType, signature, key,
           options)
      when is_binary(signature) do
    case (format_verify_key(key)) do
      :badarg ->
        :erlang.error(:badarg,
                        [digestOrPlainText, digestType, signature, key,
                                                                       options])
      {algorithm, cryptoKey} ->
        try do
          :crypto.verify(algorithm, digestType, digestOrPlainText,
                           signature, cryptoKey, options)
        catch
          :error, {:notsup, _, _} ->
            :erlang.error(:notsup)
          :error, {:error, _, _} ->
  ...
  def pkix_dist_points(otpCert) do
    ...
    :lists.foldl(fn point, acc0 ->
                      distPoint = :pubkey_cert_records.transform(point,
                                                                   :decode)
                      [distPoint | acc0]
                 end,
                   [], value)
  ...
  def pkix_sign(r_OTPTBSCertificate(signature: r_SignatureAlgorithm() = sigAlg) = tBSCert, key) do
    ...
    {digestType, _,
       opts} = :pubkey_cert.x509_pkix_sign_types(sigAlg)
    signature = sign(msg, digestType,
                       format_pkix_sign_key(key), opts)
    cert = r_OTPCertificate(tbsCertificate: tBSCert,
               signatureAlgorithm: sigAlg, signature: signature)
  ...
  def pkix_is_issuer(r_OTPCertificate(tbsCertificate: tBSCert),
           r_OTPCertificate(tbsCertificate: candidate)) do
    :pubkey_cert.is_issuer(r_OTPTBSCertificate(tBSCert, :issuer),
                             r_OTPTBSCertificate(candidate, :subject))
  end

  def pkix_is_issuer(r_CertificateList(tbsCertList: tBSCRL),
           r_OTPCertificate(tbsCertificate: candidate)) do
    :pubkey_cert.is_issuer(r_OTPTBSCertificate(candidate, :subject),
                             :pubkey_cert_records.transform(r_TBSCertList(tBSCRL, :issuer),
                                                              :decode))
  end
  ...
  def pkix_crl_issuer(r_CertificateList() = cRL) do
    :pubkey_cert_records.transform(r_TBSCertList(r_CertificateList(cRL, :tbsCertList), :issuer),
                                     :decode)
  end

  def pkix_normalize_name(issuer) when is_binary(issuer) do
    plainGenName = der_decode(:Name, issuer)
    genName = :pubkey_cert_records.transform(plainGenName,
                                               :decode)
  ...
  def pkix_path_validation(r_OTPCertificate() = trustedCert, certChain, options)
    ...
    {verifyFun,
       userState0} = :proplists.get_value(:verify_fun, options,
                                            {fn _, {:bad_cert, _} = reason, _ ->
                                                  {:fail, reason}
                                               ...
                                               []})
    try do
      :pubkey_cert.validate_time(trustedCert, userState0,
                                   verifyFun)
    catch
      {:bad_cert, _} = result ->
        {:error, result}
  ...
  def pkix_path_validation(pathErr, [cert | chain], options0)
    ...
    {verifyFun,
       userstat0} = :proplists.get_value(:verify_fun, options0,
                                           {fn _, {:bad_cert, _} = reason, _ ->
                                                 {:fail, reason}
                                               _, {:extension, _}, userState ->
                                                 {:unknown, userState}
  ...
  def pkix_crls_validate(otpCert, dPAndCRLs0, options) do
    callBack = :proplists.get_value(:update_crl, options,
                                      fn _, currCRL ->
                                           currCRL
                                      end)
  ...
  def short_name_hash({:rdnSequence, _Attributes} = name) do
    ...
    <<hashValue :: size(32) - little,
        _ :: binary>> = :crypto.hash(:sha, hashThis)
    :string.to_lower(:string.right(:erlang.integer_to_list(hashValue,
                                                             16),
                                     8, ?0))
  end

  def pkix_test_data(%{client_chain: clientChain0,
             server_chain: serverChain0}) do
  ...
  def pkix_ocsp_validate(derCert, issuerCert, ocspRespDer, responderCerts,
           nonceExt)
      when is_binary(derCert) do
    pkix_ocsp_validate(pkix_decode_cert(derCert, :otp),
                         issuerCert, ocspRespDer, responderCerts, nonceExt)
  end

  def pkix_ocsp_validate(cert, derIssuerCert, ocspRespDer, responderCerts,
           nonceExt)
      when is_binary(derIssuerCert) do
    pkix_ocsp_validate(cert,
                         pkix_decode_cert(derIssuerCert, :otp), ocspRespDer,
                         responderCerts, nonceExt)
  end

  def pkix_ocsp_validate(cert, issuerCert, ocspRespDer, responderCerts,
           nonceExt) do
    ocspResponse = :pubkey_ocsp.decode_ocsp_response(ocspRespDer)
    ocspCertResponses = (case (ocspResponse) do
                           {:ok, basicOcspResponse = r_BasicOCSPResponse(certs: certs)} ->
                             ocspResponseCerts = (for c <- certs do
                                                    otp_cert(c)
                                                  end)
                             userResponderCerts = (for c <- responderCerts do
                                                     otp_cert(pkix_decode_cert(c,
                                                                                 :plain))
                                                   end)
                             :pubkey_ocsp.verify_ocsp_response(basicOcspResponse,
    ...
    case (ocspCertResponses) do
      {:ok, responses} ->
        case (:pubkey_ocsp.find_single_response(otp_cert(cert),
                                                  otp_cert(issuerCert),
                                                  responses)) do
  ...
  def ocsp_extensions(nonce) do
    for extn <- [:pubkey_ocsp.get_nonce_extn(nonce),
  ...
  def ocsp_responder_id(certDer) do
    :pubkey_ocsp.get_ocsp_responder_id(pkix_decode_cert(certDer,
                                                          :plain))
  end
  ...
  defp set_padding(pad, opts) do
    [{:rsa_padding, pad} | for {t, v} <- opts,
                                 t !== :rsa_padding, t !== :rsa_pad do
                             {t, v}
                           end]
  end
  ...
  defp do_pem_entry_encode(asn1Type, entity, cipherInfo, password) do
    ...
    decryptDer = :pubkey_pem.cipher(der, cipherInfo,
                                      password)
  ...
  defp path_validation([derCert | rest],
  ...
  defp path_validation([cert | _] = path,
            r_path_validation_state(user_state: userState0,
    ...
    try do
      ...
      {:valid, userState} ->
        path_validation(path,
                          r_path_validation_state(validationState, max_path_length: 0, 
  ...
  defp validate(cert,
            r_path_validation_state(working_issuer_name: issuer, working_public_key: key,
                working_public_key_parameters: keyParams,
                permitted_subtrees: permit, excluded_subtrees: exclude,
                last_cert: last, user_state: userState0,
                verify_fun: verifyFun) = validationState0) do
    otpCert = otp_cert(cert)
    {validationState1,
       userState1} = :pubkey_cert.validate_extensions(otpCert,
                                                        validationState0,
                                                        userState0, verifyFun)
    userState2 = :pubkey_cert.validate_time(otpCert,
                                              userState1, verifyFun)
    userState3 = :pubkey_cert.validate_issuer(otpCert,
                                                issuer, userState2, verifyFun)
    userState4 = :pubkey_cert.validate_names(otpCert,
                                               permit, exclude, last,
                                               userState3, verifyFun)
    userState5 = :pubkey_cert.validate_signature(otpCert,
                                                   der_cert(cert), key,
                                                   keyParams, userState4,
                                                   verifyFun)
    userState = (case (last) do
                   false ->
                     :pubkey_cert.verify_fun(otpCert, :valid, userState5,
                                               verifyFun)
                   true ->
    ...
    :pubkey_cert.prepare_for_next_cert(otpCert,
                                         validationState)
  end
  ...
  defp pkix_crls_validate(_, [], _, options, r_revoke_state(details: details)) do
    case (:proplists.get_value(:undetermined_details,
                                 options, false)) do
      false ->
      ...
      true ->
        {:bad_cert,
           {:revocation_status_undetermined,
              {:bad_crls, format_details(details)}}}
    end
  end

  defp pkix_crls_validate(otpCert, [{dP, cRL, deltaCRL} | rest], all,
            options, revokedState0) do
    callBack = :proplists.get_value(:update_crl, options,
                                      fn _, currCRL ->
                                           currCRL
                                      end)
    case (:pubkey_crl.fresh_crl(dP, cRL, callBack)) do
      {:fresh, ^cRL} ->
        do_pkix_crls_validate(otpCert,
                                [{dP, cRL, deltaCRL} | rest], all, options,
                                revokedState0)
      {:fresh, newCRL} ->
        newAll = [{dP, newCRL, deltaCRL} | all -- [{dP, cRL,
                                                      deltaCRL}]]
        do_pkix_crls_validate(otpCert,
                                [{dP, newCRL, deltaCRL} | rest], newAll,
                                options, revokedState0)
      :no_fresh_crl ->
        pkix_crls_validate(otpCert, rest, all, options,
                             revokedState0)
    end
  end

  defp do_pkix_crls_validate(otpCert, [{dP, cRL, deltaCRL} | rest], all,
            options, revokedState0) do
    otherDPCRLs = all -- [{dP, cRL, deltaCRL}]
    case (:pubkey_crl.validate(otpCert, otherDPCRLs, dP,
                                 cRL, deltaCRL, options, revokedState0)) do
      {:undetermined, :unrevoked, r_revoke_state(details: details)}
        ...
        case (:proplists.get_value(:undetermined_details,
          ...
          true ->
            {:bad_cert,
               {:revocation_status_undetermined, {:bad_crls, details}}}
        end
  ...
  defp sort_dp_crls(dpsAndCrls, freshCB) do
    sort_crls(:maps.to_list(:lists.foldl(&group_dp_crls/2,
                                           %{}, dpsAndCrls)),
                freshCB, [])
  end
  ...
  defp combine([{_, cRL} = entry | cRLs], deltaCRLs, dP,
            freshCB, acc) do
    deltaCRL = combine(cRL, deltaCRLs)
    case (:pubkey_crl.fresh_crl(dP, deltaCRL, freshCB)) do
      :no_fresh_crl ->
        combine(cRLs, deltaCRLs, dP, freshCB,
                  [{dP, entry, {:undefined, :undefined}} | acc])
      {:fresh, newDeltaCRL} ->
        combine(cRLs, deltaCRLs, dP, freshCB,
                  [{dP, entry, newDeltaCRL} | acc])
    end
  end

  defp combine(cRL, deltaCRLs) do
    deltas = :lists.filter(fn {_, deltaCRL} ->
  ...
  defp format_rsa_private_key(r_RSAPrivateKey(modulus: n, publicExponent: e,
              privateExponent: d, prime1: p1, prime2: p2,
  ...
  defp ec_curve_spec(r_ECParameters(fieldID: r_FieldID(fieldType: type,
                         parameters: params),
              curve: pCurve, base: base, order: order,
              cofactor: coFactor)) do
    field = format_field(:pubkey_cert_records.supportedCurvesTypes(type),
                           params)
  ...
  defp ec_curve_spec({:namedCurve, oID}) when (is_tuple(oID) and
                                      is_integer(:erlang.element(1, oID))) do
    ec_curve_spec({:namedCurve,
  ...
  defp format_field(:characteristic_two_field = type, params0) do
    r_Characteristic_two(m: m, basis: basisOid,
  ...
  defp normalise_attribute([r_AttributeTypeAndValue(type: _Type,
  ...
  defp normalise_attribute_value(string) do
    ...
    {newBinary,
       _} = :OTP-PUB-KEY.enc_X520CommonName({:utf8String,
                                               normalisedString},
                                              [])
    newBinary
  end
  ...
  defp verify_hostname_extract_fqdn_default({:uri_id, uRI}) do
    %{scheme: 'https', host: host} = :uri_string.normalize(uRI,
                                                       [:return_map])
    host
  end
  ...
  defp verify_hostname_match_default(ref, pres) do
    verify_hostname_match_default0(to_lower_ascii(ref),
                                     to_lower_ascii(pres))
  end
  ...
  defp verify_hostname_match_default0({:uri_id, r},
            {:uniformResourceIdentifier, p}) do
    r == p
  end
  ...
  defp verify_hostname_match_default0({:ip, r}, {:iPAddress, p})
      when length(p) == 4 do
    try do
      :erlang.list_to_tuple(p) == (cond do
                                     (tuple_size(r) == 4) ->
                                       r
                                     is_list(r) ->
  ...
  defp verify_hostname_match_default0({:srv_id, r},
            {{1, 3, 6, 1, 4, 1, 434, 2, 2, 1, 37, 0}, p}) do
    r == p
  end
  ...
  defp verify_hostname_match_loop(refs0, pres0, :undefined, failCB, cert) do
    ...
    :lists.any(fn r ->
                    :lists.any(fn p ->
                                    verify_hostname_match_default(r,
                                                                    p) or failCB.(cert)
                               end,
                                 pres)
               end,
  ...
  def handle_trace(:csp,
           ...
           stack) do
    {:io_lib.format('pkix_decode_cert(Cert, plain) = ~W', [cert, 5]), stack}
  end

  def handle_trace(:csp,
           {:return_from, {:public_key, :ocsp_responder_id, 1},
              return},
           stack) do
    {:io_lib.format('OCSP Responder ID = ~P', [return, 10]), stack}
  end

  def handle_trace(:crt,
           {:call,
              {:public_key, :pkix_decode_cert, [cert, _Type]}},
           stack) do
  ...
  def handle_trace(:csp,
           {:call,
              {:public_key, :pkix_ocsp_validate,
                 [cert, issuerCert | _]}},
           stack) do
    {:io_lib.format('#2 OCSP validation started~nCert = ~W IssuerCert = ~W', [cert, 7, issuerCert, 7]), stack}
  end

  def handle_trace(:csp, {:call, {:public_key, :otp_cert, [cert]}},
           stack) do
    {:io_lib.format('Cert = ~W', [cert, 5]), stack}
  end

  def handle_trace(:csp,
           {:return_from, {:public_key, :pkix_ocsp_validate, 5},
              return},
           stack) do
