defmodule :m_pubkey_cert_records do
  ...
  def decode_cert(derCert) do
    {:ok, cert} = :OTP-PUB-KEY.decode(:OTPCertificate,
                                        derCert)
  ...
  def transform(r_AttributeTypeAndValue(type: id, value: value0) = aTAV, func) do
    {:ok, value} = (case (attribute_type(id)) do
  ...
  def transform(r_NameConstraints(permittedSubtrees: permitted,
             excludedSubtrees: excluded),
           func) do
    r_NameConstraints(permittedSubtrees: transform_sub_tree(permitted,
                                              func),
  ...
  defp decode_supportedPublicKey(r_OTPSubjectPublicKeyInfo(algorithm: pA = r_PublicKeyAlgorithm(algorithm: algo),
              subjectPublicKey: sPK0)) do
  ...
  defp encode_supportedPublicKey(r_OTPSubjectPublicKeyInfo(algorithm: pA = r_PublicKeyAlgorithm(algorithm: algo),
              subjectPublicKey: sPK0)) do
    type = supportedPublicKeyAlgorithms(algo)
    sPK = (case (type) do
             :ECPoint ->
  ...
  defp decode_extensions(exts) do
    :lists.map(fn ext = r_Extension(extnID: id, extnValue: value0) ->
                    case (extension_id(id)) do
                      :undefined ->
                        ext
                      type ->
                        {:ok, value} = :OTP-PUB-KEY.decode(type,
  ...
  defp encode_tbs(tBS = r_OTPTBSCertificate(issuer: issuer0, subject: subject0,
                    subjectPublicKeyInfo: spki0, extensions: exts0)) do
    ...
    r_OTPTBSCertificate(tBS, issuer: issuer,  subject: subject, 
             subjectPublicKeyInfo: spki,  extensions: exts)
  end

  defp decode_tbs(tBS = r_OTPTBSCertificate(issuer: issuer0, subject: subject0,
                    subjectPublicKeyInfo: spki0, extensions: exts0)) do
