defmodule :m_pubkey_ocsp do
  ...
  def decode_ocsp_response(responseDer) do
    resp = :public_key.der_decode(:OCSPResponse,
                                    responseDer)
  ...
  defp match_single_response(issuerName, issuerKey, serialNum,
    ...
    case (serialNum == r_CertID(certID, :serialNumber) and :crypto.hash(hashType,
                                                                   issuerName) == r_CertID(certID, :issuerNameHash) and :crypto.hash(hashType,
                                                                                                                                issuerKey) == r_CertID(certID, :issuerKeyHash)) do
      true ->
        {:ok, response}
      false ->
        match_single_response(issuerName, issuerKey, serialNum,
  ...
  defp do_verify_ocsp_response(r_BasicOCSPResponse(tbsResponseData: responseData,
              signatureAlgorithm: signatureAlgo,
              signature: signature),
            responderCerts, nonce) do
    r_ResponseData(responderID: responderID) = responseData
    case (verify_ocsp_signature(:public_key.der_encode(:ResponseData,
                                                         responseData),
                                  r_AlgorithmIdentifier(signatureAlgo, :algorithm), signature,
                                  responderCerts, responderID)) do
      :ok ->
  ...
  defp verify_ocsp_nonce(responseData, nonce) do
    r_ResponseData(responses: responses,
        responseExtensions: responseExtns) = responseData
  ...
  defp verify_ocsp_signature(responseDataDer, signatureAlgo, signature,
            certs, responderID) do
    case (find_responder_cert(responderID, certs)) do
      {:ok, cert} ->
        do_verify_ocsp_signature(responseDataDer, signature,
                                   signatureAlgo, cert)
      {:error, reason} ->
        {:error, reason}
    end
  ...
  defp do_verify_ocsp_signature(responseDataDer, signature, algorithmID,
            cert) do
    {digestType,
       _SignatureType} = :public_key.pkix_sign_types(algorithmID)
    case (:public_key.verify(responseDataDer, digestType,
                               signature, get_public_key_rec(cert))) do
      true ->
        :ok
      false ->
  ...
  defp get_subject_name(r_OTPCertificate(tbsCertificate: tbsCert)) do
    :public_key.pkix_encode(:Name, r_OTPTBSCertificate(tbsCert, :subject),
                              :otp)
  end
  ...
  def handle_trace(:csp,
           {:call,
              {:pubkey_ocsp, :do_verify_ocsp_response,
                 [basicOcspResponse | _]}},
           stack) do
    r_BasicOCSPResponse(tbsResponseData: r_ResponseData(responderID: responderID,
           ...
           {:call,
              {:pubkey_ocsp, :match_single_response,
                 [_IssuerName, _IssuerKey, _SerialNum,
                                               [r_SingleResponse(thisUpdate: thisUpdate,
  ...
  def handle_trace(:csp,
           {:return_from, {:pubkey_ocsp, :is_responder, 2},
              return},
           stack) do
