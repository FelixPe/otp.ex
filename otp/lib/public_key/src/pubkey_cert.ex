defmodule :m_pubkey_cert do
  ...
  def validate_extensions(otpCert, validationState0, userState0, verifyFun) do
  ...
  defp validate_policy_tree(
    ...
    case explicitPolicyConstraint > 0 or not :pubkey_policy_tree.is_empty(tree) do
      true ->
        validationState
          ...
          verify_fun(
            ...
            {:bad_cert,
             {:policy_requirement_not_met,
              {{:explicit_policy, explicitPolicyConstraint},
               {:policy_set, :pubkey_policy_tree.constrained_policy_node_set(tree)}}}},
            userState0,
            verifyFun
          )
  ...
  def prepare_for_next_cert(
    ...
    validationState1 =
      handle_policy_mappings(
        otpCert,
        validationState0
      )

    validationState =
      r_path_validation_state(validationState1,
        policy_mapping_ext: :undefined,
        current_any_policy_qualifiers: :undefined
      )
  ...
  def prepare_for_next_cert(
    ...
    validationState1 =
      r_path_validation_state(validationState0,
        ...
        valid_policy_tree:
          assert_valid_policy_tree(
            r_path_validation_state(validationState0, :explicit_policy),
  ...
  def match_name(:emailAddress, name, [permittedName | rest]) do
    fun = fn email, permittedEmail ->
      is_valid_email_address(email, permittedEmail, :string.tokens(permittedEmail, ~c"@"))
    end
    ...
    %{cert: serverRootCert, key: serverRootKey} =
      case serverRoot do
        ...
        serverRootConf when is_list(serverRootConf) ->
          root_cert(~c"SERVER ROOT CA", serverRootConf)
      end
    ...
    %{cert: clientRootCert, key: clientRootKey} =
      case clientRoot do
        ...
        clientRootConf when is_list(clientRootConf) ->
          root_cert(~c"CLIENT ROOT CA", clientRootConf)
      end
    ...
    %{cert: rootCert, key: rootKey} =
      case root do
        ...
        rootConf when is_list(rootConf) ->
          root_cert(~c"SERVER ROOT CA", rootConf)
      end
  ...
  defp policy_set(opts, default) do
    case :proplists.get_value(:policy_set, opts, :undefined) do
      :undefined ->
        default
  ...
  defp oidify(oid) when is_list(oid) do
    tokens = :string.tokens(oid, ~c"$.")

    oidList =
      for strInt <- tokens do
  ...
  defp process_policy_tree(
         policyInformation,
         selfSigned,
         r_path_validation_state(valid_policy_tree: tree0) = validationState
       ) do
  ...
  defp any_policy_children([], policyInfoList) do
    :lists.foldl(
      fn r_PolicyInformation(
         ...
         acc0 ->
        node =
          :pubkey_policy_tree.policy_node(
            policy,
            qualifiers,
            [policy]
          )
  ...
  defp any_ext_policy_children(%{expected_policy_set: expPolicySet}, qualifiers, allLeaves) do
  ...
  defp handle_policy_mappings([mappings | rest], otpCert, tree0, validationState) do
    case handle_policy_mapping(mappings, otpCert, tree0, validationState) do
      {:tree, tree} ->
  ...
  defp time_str_2_gregorian_sec(
         :notBefore,
         {:utcTime, [firstDigitYear | _] = utcTime}
       ) do
    y1 = :erlang.list_to_integer([firstDigitYear])

    yearPrefix =
      case y1 > 4 and y1 <= 9 do
        true ->
          [?1, ?9]
  ...
  def time_str_2_gregorian_sec(
        {:generalTime, [y1, y2, y3, y4, m1, m2, d1, d2, h1, h2, m3, m4, s1, s2, ?Z]}
      ) do
  ...
  defp sliding_year_window([y1, y2, m1, m2, d1, d2, h1, h2, m3, m4, s1, s2, z]) do
    ...
    [year1, year2] = :erlang.integer_to_list(div(yearWindow, 100))
  ...
  defp is_dir_name2({t1, str1}, str2)
       when t1 == :printableString or
              t1 == :utf8String do
    is_dir_name2(str1, str2)
  end

  defp is_dir_name2(str1, {t2, str2})
       when t2 == :printableString or
              t2 == :utf8String do
    is_dir_name2(str1, str2)
  end
  ...
  defp strip_many_spaces(strings, keepDeep) do
    split =
      for str <- strings, str != [] do
        :string.trim(str, :leading, ~c" ")
      end

    deepList = :lists.join(~c" ", split)
  ...
  defp is_in_intersection(
    ...
    case is_or_address(orAddr1, orAddr2) do
      ...
      false ->
        [r_GeneralSubtree(base: {:x400Address, ~c""}) | trees]
    end
  end
  ...
  defp is_in_intersection(
    ...
    case case_insensitive_match(name1, name2) do
      ...
      false ->
        [r_GeneralSubtree(base: {type, ~c""}) | trees]
    end
  end
  ...
  defp is_valid_host_or_domain(canditate, permitted) do
    case :string.tokens(canditate, ~c"@") do
      [canditateHost] ->
  ...
  defp is_valid_email_address(canditate, permittedHost, [_]) do
    [_, canditateHost] = :string.tokens(canditate, ~c"@")
  ...
  defp is_double_quoted([~c"\"" | tail]) do
    is_double_quote(:lists.last(tail))
  end

  defp is_double_quoted(~c"%22" ++ tail) do
  ...
  defp is_double_quote(~c"%22") do
    true
  end

  defp is_double_quote(~c"\"") do
    true
  end
  ...
  defp subject(contact, name) do
    opts = [
      {:email, contact ++ ~c"@example.org"},
      {:name, name},
      {:city, ~c"Stockholm"},
      {:country, ~c"SE"},
      {:org, ~c"erlang"},
      {:org_unit, ~c"automated testing"}
    ]
  ...
  defp validity(opts) do
    ...
    genFormat = fn {y, m, d} ->
      :lists.flatten(:io_lib.format(~c"~4..0w~2..0w~2..0w130000Z", [y, m, d]))
    end

    uTCFormat = fn {y, m, d} ->
      [_, _, y3, y4] = :erlang.integer_to_list(y)
      :lists.flatten(:io_lib.format(~c"~s~2..0w~2..0w130000Z", [[y3, y4], m, d]))
    end

    r_Validity(
      notBefore: validity_format(defFrom, genFormat, uTCFormat),
  ...
  defp cert_chain(role, issuerCert, issuerKey, [cAOpts | rest], n, acc) do
    ...
    cert =
      cert(
        ...
        key,
        ~c"webadmin",
        ~c" Intermediate CA " ++ :erlang.integer_to_list(n),
        cAOpts,
        :ca
      )
