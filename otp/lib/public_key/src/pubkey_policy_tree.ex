defmodule :m_pubkey_policy_tree do
  ...
  def add_leaf_siblings({parent, [{_, _} | _] = childNodes},
           siblingFun) do
    {parent,
       :lists.map(fn childNode ->
  ...
  def prune_invalid_nodes({root, childNodes}, invalidNodes) do
    case (prune_invalid_nodes_children(childNodes,
                                         invalidNodes)) do
      [] ->
        empty()
      newChildNodes ->
  ...
  def policy_node(validPolicy, qualifiers, expPolicySet) do
    qualifierSet = (case (qualifiers) do
                      :asn1_NOVALUE ->
                        []
                      _ ->
                        qualifiers
  ...
  defp collect_children_qualifiers(collect,
  ...
  defp children_collect(collect, {parent, childNodes}) do
    collect.(parent) ++ children_collect(collect,
                                           childNodes)
  end

  defp children_collect(collect, [{_, _} = childNode | childNodes]) do
    children_collect(collect,
                       childNode) ++ children_collect(collect, childNodes)
  end
  ...
  defp prune_children(childNodes) when is_list(childNodes) do
    :lists.filtermap(fn {parent, children} ->
  ...
  defp handle_too_long_notice(qualifier) do
    try do
      ...
      :error, _ ->
        r_UserNotice(noticeRef: :asn1_NOVALUE, explicitText: 'User Notice much too long, so value is ignored')
    else
