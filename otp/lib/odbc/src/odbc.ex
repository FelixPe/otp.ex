defmodule :m_odbc do
  ...
  def select(connectionReference, :next, n, :infinity)
      when is_pid(connectionReference) and is_integer(n) and
             n > 0 do
    oDBCCmd = [12, 15, :erlang.integer_to_list(0), ~c";", :erlang.integer_to_list(n), ~c";"]
    call(connectionReference, {:select_cmd, :next, oDBCCmd}, :infinity)
  end
  ...
  def select(connectionReference, {:relative, pos}, n, :infinity)
      when is_pid(connectionReference) and
             ...
             n > 0 do
    oDBCCmd = [12, 13, :erlang.integer_to_list(pos), ~c";", :erlang.integer_to_list(n), ~c";"]
    call(connectionReference, {:select_cmd, :relative, oDBCCmd}, :infinity)
  end
  ...
  def select(connectionReference, {:absolute, pos}, n, :infinity)
      when is_pid(connectionReference) and
             is_integer(pos) and pos > 0 and is_integer(n) and
             n > 0 do
    oDBCCmd = [12, 14, :erlang.integer_to_list(pos), ~c";", :erlang.integer_to_list(n), ~c";"]
    call(connectionReference, {:select_cmd, :absolute, oDBCCmd}, :infinity)
  end
  ...
  def describe_table(connectionReference, table, :infinity)
    ...
    oDBCCmd = [17, ~c"SELECT * FROM " ++ table]
  ...
  defp handle_msg({:connect, oDBCCmd, autoCommitMode, srollableCursors}, timeout, state) do
    ...
    :erlang.port_command(
      ...
      [:erlang.integer_to_list(inetPortSup), ~c";", :erlang.integer_to_list(inetPortOdbc), 0]
    )
  ...
  def handle_info(
    ...
    case :erlang.binary_to_term(binData) do
      ...
      {:error, reason} ->
        report =
          :io_lib.format(~c"ODBC could not end connection gracefully due to ~p~n", [reason])
  ...
  def handle_info(
    ...
    :gen_server.reply(from, :ok)
    {:stop, {:timeout, ~c"Port program is not responding to disconnect, will be killed"}, state}
  end
  ...
  def handle_info(
    ...
    {:stop, {:timeout, ~c"Port-program busy when trying to disconnect,  will be killed"},
     r_state(state,
       pending_request: :undefined,
       reply_to: :undefined,
       num_timeouts: r_state(state, :num_timeouts) + 1
  ...
  def handle_info(info, state) do
    report = :io_lib.format(~c"ODBC: received unexpected info: ~p~n", [info])
    :error_logger.error_report(report)
    {:noreply, state}
  end
