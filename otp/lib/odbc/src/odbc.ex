defmodule :m_odbc do
  ...
  require Record
  Record.defrecord(:r_state, :state, erlang_port: :undefined,
                                 reply_to: :undefined, owner: :undefined,
                                 result_set: :undefined, auto_commit_mode: :on,
                                 absolute_pos: :undefined,
                                 relative_pos: :undefined,
                                 scrollable_cursors: :undefined,
                                 state: :connecting,
                                 pending_request: :undefined, num_timeouts: 0,
  ...
  def disconnect(connectionReference)
    ...
    oDBCCmd = [2]
    case (call(connectionReference, {:disconnect, oDBCCmd},
                 5000)) do
  ...
  def sql_query(connectionReference, sQLQuery, :infinity)
      when (is_pid(connectionReference) and
              is_list(sQLQuery)) do
    oDBCCmd = [6, sQLQuery]
    call(connectionReference, {:sql_query, oDBCCmd},
           :infinity)
  end
  ...
  def select_count(connectionReference, sQLQuery, :infinity)
      when (is_pid(connectionReference) and
              is_list(sQLQuery)) do
    oDBCCmd = [7, sQLQuery]
    call(connectionReference, {:select_count, oDBCCmd},
           :infinity)
  end
  ...
  def first(connectionReference, :infinity)
    ...
    call(connectionReference,
           {:select_cmd, :absolute, oDBCCmd}, :infinity)
  end
  ...
  def first(connectionReference, timeOut)
    ...
    oDBCCmd = [12, 8]
    call(connectionReference,
           {:select_cmd, :absolute, oDBCCmd}, timeOut)
  end
  ...
  def prev(connectionReference, :infinity)
    ...
    oDBCCmd = [12, 11]
    call(connectionReference,
           {:select_cmd, :relative, oDBCCmd}, :infinity)
  end
  ...
  def prev(connectionReference, timeOut)
    ...
    oDBCCmd = [12, 11]
    call(connectionReference,
           {:select_cmd, :relative, oDBCCmd}, timeOut)
  end
  ...
  def select(connectionReference, :next, n, :infinity)
      when (is_pid(connectionReference) and is_integer(n) and
              n > 0) do
    oDBCCmd = [12, 15, :erlang.integer_to_list(0), ';',
  ...
  def select(connectionReference, {:relative, pos}, n,
           :infinity)
      when (is_pid(connectionReference) and
              is_integer(pos) and pos > 0 and is_integer(n) and
              n > 0) do
    oDBCCmd = [12, 13, :erlang.integer_to_list(pos), ';',
                                                         :erlang.integer_to_list(n),
                                                             ';']
    call(connectionReference,
           {:select_cmd, :relative, oDBCCmd}, :infinity)
  ...
  def select(connectionReference, {:absolute, pos}, n,
           :infinity)
      when (is_pid(connectionReference) and
              is_integer(pos) and pos > 0 and is_integer(n) and
              n > 0) do
    oDBCCmd = [12, 14, :erlang.integer_to_list(pos), ';',
                                                         :erlang.integer_to_list(n),
                                                             ';']
    call(connectionReference,
           {:select_cmd, :absolute, oDBCCmd}, :infinity)
  ...
  def param_query(connectionReference, sQLQuery, params, :infinity)
    ...
    oDBCCmd = [16, :erlang.term_to_binary({sQLQuery ++ [0],
                                             noRows, newParams})]
    call(connectionReference, {:param_query, oDBCCmd},
           :infinity)
  end
  ...
  def describe_table(connectionReference, table, :infinity)
    ...
    call(connectionReference, {:describe_table, oDBCCmd},
           :infinity)
  end
  ...
  def init(args) do
    ...
    {:value,
       {:client, clientPid}} = :lists.keysearch(:client, 1,
                                                  args)
    :erlang.monitor(:process, clientPid)
    inet = (case (:gen_tcp.listen(0,
                                    [:inet6, {:ip, :loopback}])) do
              {:ok, dummyport} ->
    ...
    case (:os.find_executable('odbcserver',
      ...
      fileName when is_list(fileName) ->
        port = :erlang.open_port({:spawn, '"' ++ fileName ++ '"'},
                                   [{:packet, 4}, :binary, :exit_status])
        state = r_state(listen_sockets: [listenSocketSup,
                                       listenSocketOdbc],
  ...
  def handle_call({client, msg, timeout}, from,
           state = r_state(owner: client, reply_to: :undefined)) do
    handle_msg(msg, timeout, r_state(state, reply_to: from))
  end

  def handle_call(request = {client, _, timeout}, from,
           state = r_state(owner: client, reply_to: :skip,
                       num_timeouts: n))
      when n < 10 do
    {:noreply, r_state(state, pending_request: {request, from}),
       timeout}
  end

  def handle_call({client, _, _}, from,
           state = r_state(owner: client, num_timeouts: n))
      when n >= 10 do
    :gen_server.reply(from, {:error, :connection_closed})
    {:stop, :too_many_sequential_timeouts,
  ...
  defp handle_msg({:connect, oDBCCmd, autoCommitMode,
             srollableCursors},
            timeout, state) do
    [listenSocketSup,
         listenSocketOdbc] = r_state(state, :listen_sockets)
    {:ok, inetPortSup} = :inet.port(listenSocketSup)
    {:ok, inetPortOdbc} = :inet.port(listenSocketOdbc)
    :erlang.port_command(r_state(state, :erlang_port),
                           [:erlang.integer_to_list(inetPortSup), ';',
                                                                      :erlang.integer_to_list(inetPortOdbc),
                                                                          0])
    newState = r_state(state, auto_commit_mode: autoCommitMode, 
                          scrollable_cursors: srollableCursors)
    case (:gen_tcp.accept(listenSocketSup,
                            port_timeout())) do
      {:ok, supSocket} ->
        :gen_tcp.close(listenSocketSup)
        case (:gen_tcp.accept(listenSocketOdbc,
                                port_timeout())) do
          {:ok, odbcSocket} ->
            :gen_tcp.close(listenSocketOdbc)
            odbc_send(odbcSocket, oDBCCmd)
            {:noreply,
               r_state(newState, odbc_socket: odbcSocket, 
  ...
  defp handle_msg({:commit, _ODBCCmd}, timeout,
            state = r_state(auto_commit_mode: :on)) do
    {:reply, {:error, :not_an_explicit_commit_connection},
       r_state(state, reply_to: :undefined), timeout}
  end

  defp handle_msg({:commit, oDBCCmd}, timeout,
            state = r_state(auto_commit_mode: :off)) do
    odbc_send(r_state(state, :odbc_socket), oDBCCmd)
  ...
  defp handle_msg({:select_cmd, :absolute, oDBCCmd}, timeout,
            state = r_state(result_set: :exists, absolute_pos: true)) do
    odbc_send(r_state(state, :odbc_socket), oDBCCmd)
    {:noreply, state, timeout}
  end

  defp handle_msg({:select_cmd, :relative, oDBCCmd}, timeout,
            state = r_state(result_set: :exists, relative_pos: true)) do
    odbc_send(r_state(state, :odbc_socket), oDBCCmd)
    {:noreply, state, timeout}
  end

  defp handle_msg({:select_cmd, :next, oDBCCmd}, timeout,
            state = r_state(result_set: :exists)) do
    odbc_send(r_state(state, :odbc_socket), oDBCCmd)
    {:noreply, state, timeout}
  end

  defp handle_msg({:select_cmd, _Type, _ODBCCmd}, _Timeout,
            state = r_state(result_set: :undefined)) do
  ...
  defp handle_msg({:select_cmd, _Type, _ODBCCmd}, _Timeout,
            state) do
    reply = (case (r_state(state, :scrollable_cursors)) do
               :on ->
  ...
  def handle_info({:tcp, socket, binData},
           state = r_state(state: :connecting, reply_to: from,
                       odbc_socket: socket)) do
    case (:erlang.binary_to_term(binData)) do
      {:ok, absolutSupport, relativeSupport} ->
        newState = r_state(state, absolute_pos: absolutSupport, 
                              relative_pos: relativeSupport)
        :gen_server.reply(from, :ok)
        {:noreply,
           r_state(newState, state: :connected,  reply_to: :undefined)}
      error ->
  ...
  def handle_info({:tcp, socket, _},
           state = r_state(state: :connected, odbc_socket: socket,
                       reply_to: :skip, pending_request: :undefined)) do
  ...
  def handle_info({:tcp, socket, _},
    ...
    handle_msg(msg, timeout,
                 r_state(state, pending_request: :undefined,  reply_to: from))
  end

  def handle_info({:tcp, socket, binData},
           state = r_state(state: :connected, reply_to: from,
                       odbc_socket: socket)) do
    :gen_server.reply(from, binData)
    {:noreply,
       r_state(state, reply_to: :undefined,  num_timeouts: 0)}
  end
  ...
  def handle_info(:timeout,
           state = r_state(state: :connected,
                       pending_request: :undefined, reply_to: from))
      when from != :undefined do
    :gen_server.reply(from, :timeout)
    {:noreply,
       r_state(state, reply_to: :skip, 
  ...
  def handle_info(:timeout,
    ...
    {:stop, {:timeout, 'Port-program busy when trying to disconnect,  will be killed'},
       r_state(state, pending_request: :undefined, 
                  reply_to: :undefined, 
                  num_timeouts: r_state(state, :num_timeouts) + 1)}
  end
  ...
  def handle_info(:timeout,
           state = r_state(state: :connected,
                       pending_request: {_, pendingFrom})) do
  ...
  def handle_info({port, {:exit_status, 0}},
           state = r_state(erlang_port: port, state: :disconnecting)) do
    {:noreply, state}
  end

  def handle_info({port, {:exit_status, status}},
           state = r_state(erlang_port: port)) do
    {:stop,
       {:port_exit,
          (fn 0 ->
                :normal_exit
  ...
  def handle_info({:tcp_closed, socket},
           state = r_state(odbc_socket: socket,
                       state: :disconnecting)) do
    {:stop, :normal, state}
  end
  ...
  def terminate({:port_exit, _Reason},
           state = r_state(reply_to: :undefined)) do
  ...
  defp connect(connectionReferense, connectionStr, options) do
    {c_AutoCommitMode,
       eRL_AutoCommitMode} = connection_config(:auto_commit,
                                                 options)
    timeOut = connection_config(:timeout, options)
    {c_TraceDriver, _} = connection_config(:trace_driver,
                                             options)
    {c_SrollableCursors,
       eRL_SrollableCursors} = connection_config(:scrollable_cursors,
                                                   options)
    {c_TupleRow, _} = connection_config(:tuple_row, options)
    {binaryStrings, _} = connection_config(:binary_strings,
                                             options)
    {extendedErrors,
       _} = connection_config(:extended_errors, options)
    oDBCCmd = [1, c_AutoCommitMode, c_TraceDriver,
                                        c_SrollableCursors, c_TupleRow,
                                                                binaryStrings,
                                                                    extendedErrors,
                                                                        connectionStr]
    case (call(connectionReferense,
                 {:connect, oDBCCmd, eRL_AutoCommitMode,
  ...
  defp call(connectionReference, msg, timeout) do
    result = ((try do
                :gen_server.call(connectionReference,
  ...
  defp fix_params({{:sql_decimal, precision, scale}, inOut,
             values}) do
  ...
  defp fix_params({{:sql_numeric, precision, scale}, inOut,
             values}) do
  ...
  defp fix_params({:sql_timestamp, inOut, values}) do
    newValues = (case ((try do
                         :lists.map(fn {{year, month, day},
                                          {hour, minute, second}} ->
