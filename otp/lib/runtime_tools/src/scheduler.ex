defmodule :m_scheduler do
  ...
  def get_sample_all() do
    get_sample(:scheduler_wall_time_all)
  end

  defp get_sample(stats) do
    case (:erlang.statistics(stats)) do
      :undefined ->
        :undefined
      list ->
  ...
  defp create_sample(stats, list) do
    sorted = :lists.sort(list)
    tagged = :lists.map(fn {i, a, t} ->
                             {sched_tag(i), i, a, t}
                        end,
                          sorted)
    {stats, tagged}
  ...
  def utilization(seconds) when (is_integer(seconds) and
                          seconds > 0) do
    _ = :erlang.system_flag(:scheduler_wall_time, true)
    t0 = sample()
    receive do after seconds * 1000 ->
      :ok
    end
    t1 = sample()
  ...
  def utilization({stats, ts0}, {stats, ts1}) do
    diffs = :lists.map(fn {{tag, i, a0, t0},
                             {tag, i, a1, t1}} ->
                            {tag, i, a1 - a0, t1 - t0}
                       end,
                         :lists.zip(ts0, ts1))
    {lst0, {a, t, n}} = :lists.foldl(fn {tag, i, adiff,
                                           tdiff},
                                          {lst, acc} ->
    ...
    lst2 = (case (:erlang.system_info(:logical_processors_available)) do
              :unknown ->
                lst1
              lPA ->
  ...
  def utilization({:scheduler_wall_time, _} = t0,
           {:scheduler_wall_time_all, ts1}) do
    utilization(t0, {:scheduler_wall_time, remove_io(ts1)})
  end

  def utilization({:scheduler_wall_time_all, ts0},
           {:scheduler_wall_time, _} = t1) do
