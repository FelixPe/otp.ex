defmodule :m_dbg do
  ...
  def fun2ms(shellFun) when is_function(shellFun) do
    case (:erl_eval.fun_data(shellFun)) do
      {:fun_data, importList, clauses} ->
        case (:ms_transform.transform_from_shell(:dbg, clauses,
                                                   importList)) do
  ...
  def tpe(event, pattern) when event === :send or
                                event === :receive do
    do_tp(event, pattern, [])
  end

  defp do_tp(x, pattern, flags) when is_integer(pattern) or
                                    is_atom(pattern) do
  ...
  defp do_tp(x, pattern, flags) when is_list(pattern) do
    ...
    case (lint_tp(pattern)) do
      {:ok, _} ->
        saveInfo = (case (save_pattern(pattern)) do
        ...
        {:ok,
           do_tp_on_nodes(nodes, x, pattern, flags) ++ saveInfo}
      other ->
        other
    end
  end
  ...
  defp do_tp_on_nodes(nodes, x, p, flags) do
    :lists.map(fn node ->
                    case (:rpc.call(node, :erlang, :trace_pattern,
                                      [x, p, flags])) do
                      n when is_integer(n) ->
                        {:matched, node, n}
                      else__ ->
  ...
  def ctpe(event) when event === :send or
                       event === :receive do
    nodes = req(:get_nodes)
  ...
  def dtp() do
    pt_doforall(fn {key, _}, _ when is_integer(key) ->
                     dtp(key)
                   {_, _}, _ ->
                     :ok
                end,
  ...
  def wtp(fileName) do
    case (:file.open(fileName,
      ...
      {:ok, file} ->
        ...
        pt_doforall(fn {_, val}, _ when is_list(val) ->
  ...
  defp trace_port_control(node, command, arg) do
    case (get_tracer(node)) do
      {:ok, port} when is_port(port) ->
        {:ok,
           (try do
             :rpc.call(node, :erlang, :port_control,
                         [port, command, arg])
           catch
             :error, e -> {:EXIT, {e, __STACKTRACE__}}
             :exit, e -> {:EXIT, e}
  ...
  def trace_port(:file,
      ...
      when (is_list(tail) and is_integer(wrapSize) and
              ...
              wrapCnt < 1 <<< 32) do
    trace_port1(:file, filename,
                  {:wrap, tail, wrapSize, wrapCnt, 0})
  end
  ...
  def trace_port(:ip, {portno, qsiz}) when (is_integer(portno) and
                                      is_integer(qsiz)) do
    fn () ->
         driver = 'trace_ip_drv'
         ...
         l = :lists.flatten(:io_lib.format('~s ~p ~p 2',
                                             [driver, portno, qsiz]))
  ...
  defp trace_port1(:file, filename, options) do
    ...
    fn () ->
         ...
         {wrap, tail} = (case (options) do
                           {:wrap, t, wrapSize, wrapCnt, wrapTime} ->
                             {:lists.flatten(:io_lib.format('w ~p ~p ~p ~p ',
                                                              [wrapSize,
                                                                   wrapCnt,
                                                                       wrapTime,
                                                                           length(name)])),
  ...
  def trace_client(:file, {filename, :wrap, tail}, fD) do
    trace_client(:file, {filename, :wrap, tail, 128 * 1024},
                   fD)
  end

  def trace_client(:file, {filename, :wrap, tail, wrapSize}, fD) do
    trace_client(:file,
                   {filename, :wrap, tail, wrapSize, 8}, fD)
  end

  def trace_client(:file,
  ...
  defp loop({c, t} = surviveLinks, table) do
    receive do
      {from, :i} ->
        modifier = modifier()
        reply = display_info(:lists.map(fn {n, _} ->
                                             n
                                        end,
                                          :erlang.get()),
                               modifier)
      ...
      {_From, :stop} ->
        :lists.foreach(fn {node, {_Relay, port}} ->
                            :rpc.call(node, :dbg, :deliver_and_flush, [port])
                       end,
                         :erlang.get())
        exit(:done)
          ...
          ^c ->
            case (:lists.delete(pid, t)) do
              ^t ->
                ...
                :io.format(:user, '** dbg got EXIT - terminating: ~' ++ modifier ++ 'p~n', [reason])
                exit(:done)
              newT ->
  ...
  defp tracer_loop(handler, hdata) do
    ...
    newHdata = handle_traces(suspended, traces, handler,
                               hdata)
    case (state) do
      :done ->
        exit(:normal)
  ...
  defp trac(proc, how, flags) when is_atom(proc) do
    case (:erlang.get()) do
      [] ->
        {:error, :no_tracers}
      nodes ->
        matched = (for {node, nodeInfo} <- nodes do
  ...
  defp trac(node, {_Replay, tracer}, atomPid, how, flags) do
    case (:rpc.call(node, :dbg, :erlang_trace,
                      [atomPid, how, [get_tracer_flag(tracer) | flags]])) do
      n when is_integer(n) ->
        {:matched, node, n}
      {:badrpc, reason} ->
  ...
  defp relay(node, to) when node != node() do
    case (:erlang.get(node)) do
      :undefined ->
        s = self()
        pid = :erlang.spawn_link(node, :dbg, :do_relay, [s, to])
        receive do
          {:started, remote} ->
  ...
  def dhandler(trace, out) when (:erlang.element(1,
                                             trace) == :trace and
  ...
  def dhandler(trace, out) when (:erlang.element(1,
    ...
    dhandler1(trace, tuple_size(trace) - 1,
                :erlang.element(tuple_size(trace), trace), out(out))
  end
  ...
  defp dhandler1(trace, size, {device, modifier}) do
    ...
    case (:erlang.element(3, trace)) do
      ...
      :send ->
        ...
        :io.format(device, '(~p) ~p ! ~' ++ modifier ++ 'p~n',
                     [from, to, message])
      :call ->
        case (:erlang.element(4, trace)) do
          mFA when size == 5 ->
    ...
    case (:erlang.element(3, trace)) do
      ...
      :call ->
        case (:erlang.element(4, trace)) do
          mFA when size == 5 ->
            message = :erlang.element(5, trace)
            :io.format(device, '(~p) call ~' ++ modifier ++ 's (~' ++ modifier ++ 'p) (Timestamp: ~p)~n',
      ...
      :return ->
        case (:erlang.element(4, trace)) do
          mFA when size == 5 ->
            ret = :erlang.element(5, trace)
            :io.format(device, '(~p) old_ret ~' ++ modifier ++ 's -> ~' ++ modifier ++ 'p (Timestamp: ~p)~n',
      ...
      :return_from ->
        mFA = :erlang.element(4, trace)
        ret = :erlang.element(5, trace)
        :io.format(device, '(~p) returned from ~' ++ modifier ++ 's -> ~' ++ modifier ++ 'p (Timestamp: ~p)~n',
                     [from, ffunc(mFA, modifier), ret, tS])
      :return_to ->
        mFA = :erlang.element(4, trace)
  ...
  defp fargs([a | args], modifier) do
    [:io_lib.format('~' ++ modifier ++ 'p,', [a]) | fargs(args,
                                                       modifier)]
  end
  ...
  defp modifier(device) do
    encoding = (case (:io.getopts(device)) do
  ...
  defp to_pidspec(tag) when tag === :all or tag === :ports or
  ...
  defp to_pid({x, y, z}) do
    to_pid(:lists.concat(['<', :erlang.integer_to_list(x), '.',
  ...
  defp pinfo(p, x) do
    check(:rpc.call(node(p), :erlang, :process_info,
                      [p, x]))
  end
  ...
  defp gen_reader(:ip, {host, portno}) do
    case (:gen_tcp.connect(host, portno,
                             [{:active, false}, :binary])) do
      {:ok, sock} ->
        p(sock, :clear)
  ...
  defp gen_reader(:file, {filename, :wrap, tail, _, wrapCnt}) do
    mk_reader_wrap(wrap_sort(wrap_presort(filename, tail),
                               wrapCnt))
  end
  ...
  defp read_term(readFun, source,
            <<op, size :: size(32)>> = tag) do
    case (op) do
      0 ->
        case (readFun.(source, size)) do
          ...
          list when is_list(list) ->
            {:ok,
               :erlang.binary_to_term(:erlang.list_to_binary(list))}
        end
      1 ->
        {:ok, {:drop, size}}
      junk ->
  ...
  defp check_list(t) do
    case ((try do
            :lists.foldl(fn val, _ ->
                              {:ok, _, _, _} = :erlang.match_spec_test([], val,
                                                                         :trace)
                              :ok
                         end,
                           :ok, t)
                           ...
                           case (match_rear(x, tail)) do
                             ...
                             c ->
                               case (match_0_9(c)) do
                                 true ->
                                   {true,
                                      wrap_encode(:filename.join(dirname, n),
                                                    c)}
                                 false ->
                                   false
                               end
  ...
  defp wrap_sortfix_1([{c1, _} = f1 | [{c2, _} | _] = tail], n, r,
            files)
  ...
  def match_rear(listA, listB) when (is_list(listA) and
                               is_list(listB)) do
    case (match_front(:lists.reverse(listA),
                        :lists.reverse(listB))) do
      false ->
        false
      list ->
        :lists.reverse(list)
  ...
  def match_0_9([h]) when (is_integer(h) and ?0 <= h and
                      h <= ?9) do
    true
  end

  def match_0_9([h | t]) when (is_integer(h) and ?0 <= h and
                          h <= ?9) do
    match_0_9(t)
  end
  ...
  def h() do
    help_display(['The following help items are available:', '   p, c', '       - Set trace flags for processes', '   tp, tpl, ctp, ctpl, ctpg, ltp, dtp, wtp, rtp', '       - Manipulate trace patterns for functions', '   n, cn, ln', '       - Add/remove traced nodes.', '   tracer, trace_port, trace_client, get_tracer, stop, stop_clear', '       - Manipulate tracer process/port', '   i', '       - Info', '', 'call dbg:h(Item) for brief help a brief description', 'of one of the items above.'])
  end

  def h(:p) do
    help_display(['p(Item) -> {ok, MatchDesc} | {error, term()}', ' - Traces messages to and from Item.', 'p(Item, Flags) -> {ok, MatchDesc} | {error, term()}', ' - Traces Item according to Flags.', '   Flags can be one of s,r,m,c,p,sos,sol,sofs,', '   sofl,all,clear or any flag accepted by erlang:trace/3'])
  end

  def h(:c) do
    help_display(['c(Mod, Fun, Args)', ' - Evaluates apply(M,F,Args) with all trace flags set.', 'c(Mod, Fun, Args, Flags)', ' - Evaluates apply(M,F,Args) with Flags trace flags set.'])
  end
  ...
  def h(:tp) do
    help_display(['tp(Module,MatchSpec)', ' - Same as tp({Module, \'_\', \'_\'}, MatchSpec)', 'tp(Module,Function,MatchSpec)', ' - Same as tp({Module, Function, \'_\'}, MatchSpec)', 'tp(Module, Function, Arity, MatchSpec)', ' - Same as tp({Module, Function, Arity}, MatchSpec)', 'tp({Module, Function, Arity}, MatchSpec) -> {ok, MatchDesc} | {error, term()}', ' - Set pattern for traced global function calls.'])
  end

  def h(:tpl) do
    help_display(['tpl(Module,MatchSpec)', ' - Same as tpl({Module, \'_\', \'_\'}, MatchSpec)', 'tpl(Module,Function,MatchSpec)', ' - Same as tpl({Module, Function, \'_\'}, MatchSpec)', 'tpl(Module, Function, Arity, MatchSpec)', ' - Same as tpl({Module, Function, Arity}, MatchSpec)', 'tpl({Module, Function, Arity}, MatchSpec) -> {ok, MatchDesc} | {error, term()}', ' - Set pattern for traced local (as well as global) function calls.'])
  end

  def h(:ctp) do
    help_display(['ctp()', ' - Same as ctp({\'_\', \'_\', \'_\'})', 'ctp(Module)', ' - Same as ctp({Module, \'_\', \'_\'})', 'ctp(Module, Function)', ' - Same as ctp({Module, Function, \'_\'})', 'ctp(Module, Function, Arity)', ' - Same as ctp({Module, Function, Arity})', 'ctp({Module, Function, Arity}) -> {ok, MatchDesc} | {error, term()}', ' - Clear call trace pattern for the specified functions'])
  end

  def h(:ctpl) do
    help_display(['ctpl()', ' - Same as ctpl({\'_\', \'_\', \'_\'})', 'ctpl(Module)', ' - Same as ctpl({Module, \'_\', \'_\'})', 'ctpl(Module, Function)', ' - Same as ctpl({Module, Function, \'_\'})', 'ctpl(Module, Function, Arity)', ' - Same as ctpl({Module, Function, Arity})', 'ctpl({Module, Function, Arity}) -> {ok, MatchDesc} | {error, term()}', ' - Clear local call trace pattern for the specified functions'])
  end

  def h(:ctpg) do
    help_display(['ctpg()', ' - Same as ctpg({\'_\', \'_\', \'_\'})', 'ctpg(Module)', ' - Same as ctpg({Module, \'_\', \'_\'})', 'ctpg(Module, Function)', ' - Same as ctpg({Module, Function, \'_\'})', 'ctpg(Module, Function, Arity)', ' - Same as ctpg({Module, Function, Arity})', 'ctpg({Module, Function, Arity}) -> {ok, MatchDesc} | {error, term()}', ' - Clear global call trace pattern for the specified functions'])
  end
  ...
  def h(:dtp) do
    help_display(['dtp() -> ok', ' - Deletes all saved match_spec\'s.', 'dtp(N) -> ok', ' - Deletes a specific saved match_spec.'])
  end

  def h(:wtp) do
    help_display(['wtp(Name) -> ok | {error, IOError}', ' - Writes all saved match_spec\'s to a file'])
  end

  def h(:rtp) do
    help_display(['rtp(Name) -> ok | {error, Error}', ' - Read saved match specifications from file.'])
  end

  def h(:n) do
    help_display(['n(Nodename) -> {ok, Nodename} | {error, Reason}', ' - Starts a tracer server on the given node.', 'n(Nodename,Type,Data) -> {ok, Nodename} | {error, Reason}', ' - Starts a tracer server with additional args on the given node.'])
  end
  ...
  def h(:tracer) do
    help_display(['tracer() -> {ok, pid()} | {error, already_started}', ' - Starts a tracer server that handles trace messages.', 'tracer(Type, Data) -> {ok, pid()} | {error, Error}', ' - Starts a tracer server with additional parameters'])
  end

  def h(:trace_port) do
    help_display(['trace_port(Type, Parameters) -> fun()', ' - Creates and returns a trace port generating fun'])
  end

  def h(:trace_client) do
    help_display(['trace_client(Type, Parameters) -> pid()', ' - Starts a trace client that reads messages created by a trace port driver', 'trace_client(Type, Parameters, HandlerSpec) -> pid()', ' - Starts a trace client that reads messages created by a', '   trace port driver, with a user defined handler'])
  end

  def h(:get_tracer) do
    help_display(['get_tracer() -> {ok, Tracer}', ' - Returns the process or port to which all trace messages are sent.', 'get_tracer(Node) -> {ok, Tracer}', ' - Returns the process or port to which all trace messages are sent.'])
  end

  def h(:stop) do
    help_display(['stop() -> ok', ' - Stops the dbg server and the tracing of all processes.', '   Does not clear any trace patterns.'])
  end

  def h(:stop_clear) do
    help_display(['stop_clear() -> ok', ' - Deprecated. Stops the dbg server and the tracing of all processes,', '   and clears all trace patterns.'])
  end
