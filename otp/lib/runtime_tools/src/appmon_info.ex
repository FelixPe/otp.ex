defmodule :m_appmon_info do
  ...
  defp format(p) when is_pid(p) and node(p) != node() do
    :erlang.pid_to_list(p) ++ ~c" " ++ :erlang.atom_to_list(node(p))
  end
  ...
  defp format(p) when is_port(p) do
    case :erlang.port_info(p, :id) do
      ...
      {_, pid} ->
        ~c"port " ++ :erlang.integer_to_list(pid)
    end
  end

  defp format(x) do
    :io.format(~c"What: ~p~n", [x])
    ~c"???"
  end
  ...
  defp calc_pinfo(:pinfo, pid) when is_pid(pid) do
    ...
    {:ok, :io_lib.format(~c"Node: ~p, Process: ~p~n~p~n~n", [node(), pid, info])}
  end
  ...
  defp print_state(state) do
    :io.format(~c"Status:~n    Opts: ~p~nClients: ~p~n    WorkStore:~n", [
      r_state(state, :opts),
      r_state(state, :clients)
    ])
