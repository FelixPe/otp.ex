defmodule :m_crashdump_viewer do
  use Bitwise
  require Record
  Record.defrecord(:r_menu_item, :menu_item, index: :undefined,
                                     picture: :undefined, text: :undefined,
                                     depth: :undefined, children: :undefined,
                                     state: :undefined, target: :undefined)
  Record.defrecord(:r_general_info, :general_info, created: :undefined,
                                        slogan: :undefined,
  ...
  Record.defrecord(:r_proc, :proc, pid: :undefined,
                                name: :undefined, init_func: :undefined,
                                parent: 'unknown', start_time: 'unknown', state: :undefined,
                                current_func: :undefined, msg_q_len: 0,
                                msg_q: :undefined, last_calls: :undefined,
                                links: :undefined, monitors: :undefined,
                                mon_by: :undefined, prog_count: :undefined,
                                cp: :undefined, arity: :undefined,
                                dict: :undefined, reds: 0, num_heap_frag: 'unknown',
                                heap_frag_data: :undefined, stack_heap: 0,
                                old_heap: :undefined, heap_unused: :undefined,
                                ...
                                new_heap_top: :undefined, stack_top: :undefined,
                                stack_end: :undefined,
                                old_heap_start: :undefined,
                                old_heap_top: :undefined,
                                old_heap_end: :undefined, memory: :undefined,
                                stack_dump: :undefined, run_queue: 'unknown',
                                int_state: :undefined)
  Record.defrecord(:r_port, :port, id: :undefined,
                                state: :undefined, task_flags: 0,
                                slot: :undefined, connected: :undefined,
                                links: :undefined, name: :undefined,
                                monitors: :undefined, suspended: :undefined,
                                controls: :undefined, input: :undefined,
                                output: :undefined, queue: :undefined,
                                port_data: :undefined)
  Record.defrecord(:r_sched, :sched, name: :undefined,
                                 type: :undefined, process: :undefined,
                                 port: :undefined, run_q: 0, port_q: :undefined,
                                 details: %{})
  Record.defrecord(:r_ets_table, :ets_table, pid: :undefined,
                                     slot: :undefined, id: :undefined,
                                     name: :undefined, is_named: :undefined,
                                     data_type: 'hash', buckets: '-', size: :undefined,
                                     memory: :undefined, details: %{})
  Record.defrecord(:r_timer, :timer, pid: :undefined,
                                 name: :undefined, msg: :undefined,
                                 time: :undefined)
  Record.defrecord(:r_fu, :fu, module: :undefined,
                              uniq: :undefined, index: :undefined,
                              address: :undefined, native_address: :undefined,
                              refc: :undefined)
  Record.defrecord(:r_nod, :nod, name: :undefined,
                               channel: :undefined, conn_type: :undefined,
                               controller: :undefined, creation: :undefined,
                               remote_links: [], remote_mon: [],
                               remote_mon_by: [], error: :undefined)
  Record.defrecord(:r_loaded_mod, :loaded_mod, mod: :undefined,
  ...
  Record.defrecord(:r_hash_table, :hash_table, name: :undefined,
                                      size: :undefined, used: :undefined,
                                      objs: :undefined, depth: :undefined)
  Record.defrecord(:r_index_table, :index_table, name: :undefined,
                                       size: :undefined, limit: :undefined,
                                       used: :undefined, rate: :undefined,
                                       entries: :undefined)
  ...
  Record.defrecord(:r_state, :state, file: :undefined,
                                 dump_vsn: :undefined, wordsize: 4,
                                 num_atoms: 'unknown')
  Record.defrecord(:r_dec_opts, :dec_opts, bin_addr_adj: 0,
                                    base64: true)
  def debug(f) do
  ...
  def init([]) do
    :ets.new(:cdv_dump_index_table,
               [:ordered_set, :named_table, :public])
    :ets.new(:cdv_reg_proc_table,
               [:ordered_set, :named_table, :public])
    :ets.new(:cdv_binary_index_table,
               [:ordered_set, :named_table, :public])
    :ets.new(:cdv_heap_file_chars,
               [:ordered_set, :named_table, :public])
    {:ok, r_state()}
  end
  ...
  def handle_call(:general_info, _From, state = r_state(file: file)) do
    ...
    tW = (case (:erlang.get(:truncated)) do
            true ->
              case (:erlang.get(:truncated_reason)) do
                :undefined ->
                  ['WARNING: The crash dump is truncated. Some information might be missing.']
                reason ->
                  ['WARNING: The crash dump is truncated (' ++ reason ++ '). Some information might be missing.']
              end
            false ->
              []
          end)
    ...
    {:reply, {:ok, genInfo, tW},
       r_state(state, wordsize: wS,  num_atoms: numAtoms)}
  end

  def handle_call({:expand_binary, {offset, size, pos}}, _From,
           r_state(file: file, dump_vsn: dumpVsn) = state) do
    fd = open(file)
    pos_bof(fd, pos)
  ...
  def handle_call(:procs_summary, _From,
           state = r_state(file: file, wordsize: wS)) do
    tW = truncated_warning([:proc])
    procs = procs_summary(file, wS)
    {:reply, {:ok, procs, tW}, state}
  end

  def handle_call({:proc_details, pid}, _From,
           state = r_state(file: file, wordsize: wS,
                       dump_vsn: dumpVsn)) do
    reply = (case (get_proc_details(file, pid, wS,
                                      dumpVsn)) do
               {:ok, proc, tW} ->
                 {:ok, proc, tW}
               other ->
  ...
  def handle_call({:port, id}, _From, state = r_state(file: file)) do
    reply = (case (get_port(file, id)) do
               {:ok, portInfo} ->
  ...
  def handle_call({:ets_tables, pid0}, _From,
           state = r_state(file: file, wordsize: wS)) do
    pid = (case (pid0) do
             :all ->
               :"$2"
  ...
  def handle_call(:internal_ets_tables, _From,
           state = r_state(file: file, wordsize: wS)) do
  ...
  def handle_call(:dist_info, _From, state = r_state(file: file)) do
    tW = truncated_warning([:visible_node, :hidden_node,
                                               :not_connected])
    nods = nods(file)
    {:reply, {:ok, nods, tW}, state}
  end

  def handle_call({:node_info, channel}, _From,
           state = r_state(file: file)) do
    reply = (case (get_node(file, channel)) do
               {:ok, nod} ->
                 tW = truncated_warning([:visible_node, :hidden_node,
                                                            :not_connected])
                 {:ok, nod, tW}
  ...
  def handle_call({:loaded_mod_details, mod}, _From,
           r_state(dump_vsn: dumpVsn, file: file) = state) do
  ...
  def handle_call(:atoms, _From,
           state = r_state(file: file, num_atoms: numAtoms0)) do
  ...
  def handle_call(:persistent_terms, _From,
           state = r_state(file: file, dump_vsn: dumpVsn)) do
  ...
  def handle_call(:allocated_areas, _From,
           state = r_state(file: file)) do
  ...
  def handle_call(:get_dump_versions, _From,
           state = r_state(dump_vsn: dumpVsn)) do
  ...
  defp call(request) do
    :gen_server.call(:crashdump_viewer_server, request,
                       3600000)
  end
  ...
  defp truncated_warning() do
    case (:erlang.get(:truncated_reason)) do
      ...
      reason ->
        ['WARNING: The crash dump is truncated here (' ++ reason ++ '). Some information might be missing.']
    end
  end
  ...
  defp line_head(fd, <<?: :: size(8), ?\r :: size(8)>>, acc,
            n) do
    case (get_chunk(fd)) do
      {:ok, bin} ->
  ...
  defp line_head(fd,
            <<?: :: size(8), space :: size(8), bin :: binary>>, acc,
            _N)
  ...
  defp line_head(fd, <<?: :: size(8), bin :: binary>>, acc,
            _N) do
    put_chunk(fd, bin)
    :lists.reverse(acc)
  end

  defp line_head(fd, <<?\r :: size(8), bin :: binary>>, acc,
            n) do
    line_head(fd, bin, acc, n + 1)
  end

  defp line_head(fd, <<char :: size(8), bin :: binary>>, acc,
            n) do
      ...
      {:ok, r_file_info(type: :regular, access: fileA, size: size)}
        ...
        case (progress_read(fd)) do
          ...
          {:ok, <<"<Erlang crash dump>", _Rest :: binary>>} ->
            r = :io_lib.format('The crashdump ~ts is in the pre-R10B format, which is no longer supported.~n', [file])
            close(fd)
            {:error, r}
          _Other ->
            r = :io_lib.format('~ts is not an Erlang crash dump~n', [file])
  ...
  defp check_dump_version(vsn) do
    dumpVsn = (for l <- :string.lexemes(vsn, '.') do
                 :erlang.list_to_integer(l)
               end)
    cond do
      dumpVsn > [0, 5] ->
        info = 'This Crashdump Viewer is too old for the given Erlang crash dump. Please use a newer version of Crashdump Viewer.'
        {:error, info}
      true ->
        {:ok, dumpVsn}
    end
      ...
      :nomatch ->
        case (progress_read(fd)) do
          {:ok, chunk0} when is_binary(chunk0) ->
            {chunk, n1} = (case (:binary.last(bin)) do
                             ?\n ->
                               {<<?\n, chunk0 :: binary>>,
                                  n + byte_size(bin) - 1}
                             _ ->
  ...
  defp tag(_Fd, <<?\n :: size(8), _ :: binary>> = rest, n,
            gat, di, _Now) do
    {tag_to_atom(:lists.reverse(gat)), :lists.reverse(di),
       rest, n}
  end

  defp tag(fd, <<?\r :: size(8), rest :: binary>>, n, gat,
            di, now) do
    tag(fd, rest, n + 1, gat, di, now)
  end

  defp tag(fd, <<?: :: size(8), idAndRest :: binary>>, n,
            gat, di, :tag) do
    tag(fd, idAndRest, n + 1, gat, di, :id)
  end

  defp tag(fd, <<char :: size(8), rest :: binary>>, n, gat,
            di, :tag) do
  ...
  defp find_truncated_proc({tag, _Id}) when tag == :atoms or
  ...
  defp is_proc_tag(tag) when tag == :proc or
  ...
  defp general_info(file) do
    ...
    fd = open(file)
    pos_bof(fd, start)
    created = (case (get_rest_of_line(fd)) do
                 {:eof, someOfLine} ->
                   someOfLine
                 wholeLine ->
                   wholeLine
    ...
    gI = get_general_info(fd,
                            r_general_info(created: created, slogan: slogan,
                                system_vsn: sysVsn))
    {memTot, memMax} = (case (lookup_index(:memory)) do
                          [{_, memStart}] ->
                            pos_bof(fd, memStart)
                            memory = get_meminfo(fd, [])
                            tot = (case (:lists.keysearch(:total, 1, memory)) do
                                     {:value, {_, t}} ->
                                       t
                                     false ->
                                       ''
                                   end)
                            max = (case (:lists.keysearch(:maximum, 1,
                                                            memory)) do
                                     {:value, {_, m}} ->
                                       m
                                     false ->
    ...
    nodeName = (case (lookup_index(:node)) do
                  [{n, _Start}] ->
                    n
                  [] ->
    ...
    instrInfo = (case (lookup_index(:old_instr_data)) do
                   [] ->
    ...
    r_general_info(gI, node_name: nodeName, 
  ...
  defp count() do
    {count_index(:proc), count_index(:ets),
       count_index(:fu), count_index(:timer)}
  end

  defp procs_summary(file, wS) do
    parseFun = fn fd, pid0 ->
                    pid = :erlang.list_to_pid(pid0)
                    proc = get_procinfo(fd, &main_procinfo/5, r_proc(pid: pid),
                                          wS)
  ...
  defp get_proc_details(file, pid, wS, dumpVsn) do
    case (lookup_index(:proc, pid)) do
      [{_, start}] ->
        fd = open(file)
        {{stack, msgQ, dict},
           tW} = (case (truncated_warning([{:proc, pid}])) do
                    [] ->
        ...
        pos_bof(fd, start)
        proc0 = r_proc(pid: pid, stack_dump: stack, msg_q: msgQ,
                    dict: dict)
  ...
  defp get_procinfo(fd, fun, proc, wS) do
    case (line_head(fd)) do
      ...
      'Spawned as' ->
        ...
        case (r_proc(proc, :name)) do
          :undefined ->
            get_procinfo(fd, fun, r_proc(proc, name: iF,  init_func: iF),
                           wS)
          _ ->
      ...
      'Message queue length' ->
        get_procinfo(fd, fun,
                       r_proc(proc, msg_q_len: :erlang.list_to_integer(bytes(fd))),
                       wS)
      'Reductions' ->
        get_procinfo(fd, fun,
                       r_proc(proc, reds: :erlang.list_to_integer(bytes(fd))), wS)
      'Stack+heap' ->
        get_procinfo(fd, fun,
      ...
      'Memory' ->
        get_procinfo(fd, fun,
                       r_proc(proc, memory: :erlang.list_to_integer(bytes(fd))), wS)
      {:eof, _} ->
        proc
      other ->
  ...
  defp all_procinfo(fd, fun, proc, wS, lineHead) do
    case (lineHead) do
      ...
      'Started' ->
        get_procinfo(fd, fun, r_proc(proc, start_time: bytes(fd)),
                       wS)
      'Last scheduled in for' ->
        get_procinfo(fd, fun,
                       r_proc(proc, current_func: {'Last scheduled in for', string(fd)}), wS)
      'Current call' ->
        get_procinfo(fd, fun,
                       r_proc(proc, current_func: {'Current call', string(fd)}), wS)
      'Number of heap fragments' ->
        get_procinfo(fd, fun, r_proc(proc, num_heap_frag: bytes(fd)),
                       wS)
      'Heap fragment data' ->
        get_procinfo(fd, fun,
                       r_proc(proc, heap_frag_data: bytes(fd)), wS)
      'OldHeap' ->
      ...
      'OldHeap unused' ->
        bytes = :erlang.list_to_integer(bytes(fd)) * wS
        get_procinfo(fd, fun, r_proc(proc, old_heap_unused: bytes),
                       wS)
      'BinVHeap' ->
        bytes = :erlang.list_to_integer(bytes(fd)) * wS
      ...
      'BinVHeap unused' ->
        bytes = :erlang.list_to_integer(bytes(fd)) * wS
        get_procinfo(fd, fun, r_proc(proc, bin_vheap_unused: bytes),
                       wS)
      'OldBinVHeap unused' ->
        bytes = (case (bytes(fd)) do
                   'overflow' ->
                     - 1
                   int ->
        ...
        get_procinfo(fd, fun,
                       r_proc(proc, old_bin_vheap_unused: bytes), wS)
      'New heap start' ->
        get_procinfo(fd, fun,
                       r_proc(proc, new_heap_start: bytes(fd)), wS)
      'New heap top' ->
        get_procinfo(fd, fun, r_proc(proc, new_heap_top: bytes(fd)),
                       wS)
      'Stack top' ->
      ...
      'Old heap start' ->
        get_procinfo(fd, fun,
                       r_proc(proc, old_heap_start: bytes(fd)), wS)
      'Old heap top' ->
        get_procinfo(fd, fun, r_proc(proc, old_heap_top: bytes(fd)),
                       wS)
      'Old heap end' ->
        get_procinfo(fd, fun, r_proc(proc, old_heap_end: bytes(fd)),
                       wS)
      'Last calls' ->
        get_procinfo(fd, fun,
                       r_proc(proc, last_calls: get_last_calls(fd)), wS)
      'Link list' ->
        {links, monitors, monitoredBy} = get_link_list(fd)
        get_procinfo(fd, fun,
                       r_proc(proc, links: links,  monitors: monitors, 
                                 mon_by: monitoredBy),
                       wS)
      'Program counter' ->
        get_procinfo(fd, fun, r_proc(proc, prog_count: string(fd)),
                       wS)
      'CP' ->
      ...
      'Run queue' ->
        get_procinfo(fd, fun, r_proc(proc, run_queue: string(fd)),
                       wS)
      'Internal State' ->
        get_procinfo(fd, fun, r_proc(proc, int_state: string(fd)),
                       wS)
      '=' ++ _next_tag ->
        proc
      other ->
  ...
  defp get_last_calls(fd, <<?: :: size(8)>>, acc, lines) do
    case (get_chunk(fd)) do
      {:ok, bin} ->
        get_last_calls(fd, <<?: :: size(8), bin :: binary>>,
                         acc, lines)
      :eof ->
        case (byte_list_to_string(:lists.reverse(acc))) do
          nextHeading when nextHeading == 'Link list' or nextHeading == 'Dictionary' or
  ...
  defp get_last_calls(fd,
            <<?: :: size(8), ?\s :: size(8), bin :: binary>>, acc,
            lines) do
    headingBin = :erlang.list_to_binary(:lists.reverse(acc,
                                                         [?:]))
  ...
  defp get_last_calls(fd, <<?\n :: size(8), bin :: binary>>, acc,
            lines) do
    get_last_calls(fd, bin, [],
                     [byte_list_to_string(:lists.reverse(acc)) | lines])
  end

  defp get_last_calls(fd, <<?\r :: size(8), bin :: binary>>, acc,
            lines) do
    get_last_calls(fd, bin, acc, lines)
  end

  defp get_last_calls(fd, <<?\s :: size(8), bin :: binary>>, [],
            lines) do
    get_last_calls(fd, bin, [], lines)
  end

  defp get_last_calls(fd, <<char :: size(8), bin :: binary>>, acc,
            lines) do
  ...
  defp get_link_list(fd) do
    case (string(fd)) do
      '[' ++ rest ->
        %{links: links, mons: monitors,
            mon_by: monitoredBy} = get_link_list(rest,
                                                   %{links: [], mons: [],
                                                       mon_by: []})
        {:lists.reverse(links), :lists.reverse(monitors),
  ...
  defp get_link('#Port' ++ _ = portStr, %{links: links} = acc) do
    Map.put(acc, :links, [{portStr, portStr} | links])
  end

  defp get_link('<' ++ _ = pidStr, %{links: links} = acc) do
    Map.put(acc, :links, [{pidStr, pidStr} | links])
  end

  defp get_link('{to,' ++ rest, %{mons: monitors} = acc) do
    Map.put(acc, :mons, [parse_monitor(rest) | monitors])
  end

  defp get_link('{from,' ++ rest, %{mon_by: monitoredBy} = acc) do
    Map.put(acc, :mon_by,
                   [parse_monitor(rest) | monitoredBy])
  end
  ...
  defp parse_monitor(monitor) do
    case (:string.lexemes(monitor, ',{}')) do
      [node, '[]'] ->
  ...
  defp get_pid_from_name(name, node) do
    case (:ets.lookup(:cdv_reg_proc_table,
                        :cdv_dump_node_name)) do
      [{_, ^node}] ->
  ...
  defp maybe_other_node2(channel) do
    ms = :ets.fun2ms(fn {{tag, start}, ch}
                            when (tag === :visible_node and ch === channel) ->
                          {'Visible Node', start}
                        {{tag, start}, ch} when (tag === :hidden_node and
                                                   ch === channel)
                                                ->
                          {'Hidden Node', start}
                        {{tag, start}, ch} when (tag === :not_connected and
                                                   ch === channel)
                                                ->
  ...
  defp expand_memory(fd, pid, dumpVsn) do
    ...
    :erlang.erase(:fd)
    incompleteWarning = (case (:erlang.erase(:incomplete_heap)) do
                           :undefined ->
                             []
                           true ->
  ...
  defp read_literals(fd, decodeOpts) do
    case (lookup_index(:literals, [])) do
      [{_, start}] ->
        [{_, chars}] = :ets.lookup(:cdv_heap_file_chars,
                                     :literals)
  ...
  defp parse_line(addr, line0, decodeOpts, dict0) do
    ...
    {_Term, line, dict} = parse_heap_term(line1, addr,
                                            decodeOpts, dict0)
    [] = skip_blanks(line)
    dict
  end
  ...
  defp split_pid_list_no_space([?> | rest], acc, pids) do
    split_pid_list_no_space(rest, [],
                              [:lists.reverse(acc, [?>]) | pids])
  end
  ...
  defp get_ets_tables(file, pid, wS) do
    parseFun = fn fd, id ->
                    eT = get_etsinfo(fd, r_ets_table(pid: :erlang.list_to_pid(id)),
                                       wS)
  ...
  defp get_etsinfo(fd, etsTable = r_ets_table(details: ds), wS) do
    case (line_head(fd)) do
      ...
      'Buckets' ->
        buckets = :erlang.list_to_integer(:string.trim(bytes(fd),
                                                         :both, ' '))
      ...
      'Words' ->
        ...
        get_etsinfo(fd, r_ets_table(etsTable, memory: {:bytes, bytes}),
                      wS)
      '=' ++ _next_tag ->
        etsTable
      'Chain Length Min' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :chain_min, val)), wS)
      'Chain Length Avg' ->
        val = (try do
        ...
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :chain_avg, val)), wS)
      'Chain Length Max' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :chain_max, val)), wS)
      ...
      'Chain Length Expected Std Dev' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :chain_exp_stddev,
                                                         val)),
                      wS)
      'Fixed' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :fixed, val)), wS)
      'Type' ->
        val = bytes(fd)
      ...
      'Protection' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :protection, val)), wS)
      'Compressed' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :compressed, val)), wS)
      'Write Concurrency' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :write_c, val)), wS)
      'Read Concurrency' ->
        val = bytes(fd)
        get_etsinfo(fd,
                      r_ets_table(etsTable, details: Map.put(ds, :read_c, val)), wS)
      other ->
  ...
  defp get_timers(file, pid) do
    ...
    t1 = lookup_and_parse_index(file, {:timer, pid},
                                  parseFun, 'timers')
    t2 = (case (:ets.lookup(:cdv_reg_proc_table, pid)) do
            [{_, name}] ->
              lookup_and_parse_index(file, {:timer, name}, parseFun,
                                       'timers')
            _ ->
              []
          end)
  ...
  defp nods(file) do
    case (lookup_index(:no_distribution)) do
      [] ->
        ...
        fd = open(file)
        visible = :lists.map(fn {channel, start} ->
                                  get_nodeinfo(fd, channel, :visible, start)
                             end,
                               v)
        hidden = :lists.map(fn {channel, start} ->
                                 get_nodeinfo(fd, channel, :hidden, start)
                            end,
                              h)
        notConnected = :lists.map(fn {channel, start} ->
                                       get_nodeinfo(fd, channel, :not_connected,
                                                      start)
                                  end,
                                    n)
        close(fd)
  ...
  defp get_nodeinfo(fd, channel, type, start) do
    pos_bof(fd, start)
    get_nodeinfo(fd,
                   r_nod(channel: :erlang.list_to_integer(channel),
                       conn_type: type))
  end
  ...
  defp loaded_mods(file) do
    ...
    {cC, oC} = (case (lookup_index(:loaded_modules)) do
                  [{_, startTotal}] ->
                    fd = open(file)
    ...
    {cC, oC,
       lookup_and_parse_index(file, :mod, parseFun, 'modules')}
  end
  ...
  defp get_atoms1(fd, [lastBin], n, atoms) do
    case (get_chunk(fd)) do
      {:ok, bin0} ->
        get_atoms(fd, <<lastBin :: binary, bin0 :: binary>>, n,
                    atoms)
      :eof ->
        end_progress()
  ...
  defp get_atoms1(fd, [bin | bins], n, atoms) do
    update_progress()
    get_atoms1(fd, bins, n - 1,
                 [{n, get_atom(bin)} | atoms])
  end
  ...
  defp parse_persistent_terms([[name0, val0] | terms], decodeOpts, dict) do
    {name, _, _} = parse_term(:erlang.binary_to_list(name0),
                                decodeOpts, dict)
    {val, _, _} = parse_term(:erlang.binary_to_list(val0),
                               decodeOpts, dict)
    [{name, val} | parse_persistent_terms(terms, decodeOpts,
                                            dict)]
  end
  ...
  defp get_persistent_terms1(_Fd, [<<"=", _ :: binary>> | _],
            persistentTerms) do
    persistentTerms
  end
  ...
  defp get_persistent_terms1(fd, [bin | bins], persistent_Terms) do
    get_persistent_terms1(fd, bins,
                            [get_persistent_term(bin) | persistent_Terms])
  end
  ...
  defp allocator_summary(allocators) do
    {sorted, doTotal} = sort_allocator_types(allocators, [],
                                               true)
    {typeTotals0, totals} = sum_allocator_data(sorted,
                                                 doTotal)
    {totalMCS, typeTotals} = (case (:lists.keytake('mseg_alloc', 1,
                                                     typeTotals0)) do
    ...
    {totalBS, totalCS} = (case (totals) do
                            false ->
                              {'N/A', 'N/A'}
                            {tBS, tCS} ->
                              {:erlang.integer_to_list(tBS),
                                 :erlang.integer_to_list(tCS)}
                          end)
  ...
  defp sort_allocator_types([{name, data} | allocators], acc, doTotal) do
    type = (case (:string.lexemes(name, '[]')) do
              [t, _Id] ->
                t
              [^name] ->
                name
    ...
    {newTypeData, newDoTotal} = sort_type_data(type, data,
                                                 typeData, doTotal)
    newAcc = :lists.keystore(type, 1, acc,
                               {type, newTypeData})
  ...
  defp sort_type_data(type, [{key, val0} | data], acc, doTotal) do
    case (:lists.member(key,
                          ['sbmbcs blocks size', 'mbcs blocks size', 'mbcs_pool blocks size', 'sbcs blocks size', 'sbmbcs carriers size', 'mbcs carriers size', 'sbcs carriers size', 'mbcs mseg carriers size', 'mbcs_pool carriers size', 'sbcs mseg carriers size', 'segments_size'])) do
      true ->
        val = :erlang.list_to_integer(hd(val0))
        sort_type_data(type, data, update_value(key, val, acc),
                         doTotal)
      false ->
  ...
  defp sum_allocator_data([{type, data} | allocData], typeAcc, total) do
    ...
    sum_allocator_data(allocData,
                         [{type, typeSum} | typeAcc], newTotal)
  end
  ...
  defp sum_type_data([{key, value} | data], typeAcc, total) do
    newTotal = (case (total) do
                  false ->
                    false
                  {totalBS, totalCS} ->
                    case (:lists.member(key, ['mbcs blocks size', 'mbcs_pool blocks size', 'sbcs blocks size'])) do
                      true ->
                        {totalBS + value, totalCS}
                      false ->
                        case (:lists.member(key, ['mbcs carriers size', 'mbcs_pool carriers size', 'sbcs carriers size'])) do
                          true ->
    ...
    newTypeAcc = (case (:lists.member(key, ['sbmbcs blocks size', 'mbcs blocks size', 'mbcs_pool blocks size', 'sbcs blocks size'])) do
                    true ->
                      update_value(:blocks_size, value, typeAcc)
                    false ->
                      case (:lists.member(key, ['sbmbcs carriers size', 'mbcs carriers size', 'mbcs_pool carriers size', 'sbcs carriers size'])) do
                        true ->
  ...
  defp schedulers(file) do
    fd = open(file)
    schds0 = (case (lookup_index(:scheduler)) do
                [] ->
                  []
                normals ->
                  [{normals, r_sched(type: :normal)}]
              end)
    schds1 = (case (lookup_index(:dirty_cpu_scheduler)) do
                [] ->
                  schds0
                dirtyCpus ->
                  [{dirtyCpus,
                      get_dirty_runqueue(fd, :dirty_cpu_run_queue)} |
                       schds0]
              end)
    schds2 = (case (lookup_index(:dirty_io_scheduler)) do
                [] ->
                  schds1
                dirtyIos ->
                  [{dirtyIos,
                      get_dirty_runqueue(fd, :dirty_io_run_queue)} |
                       schds1]
              end)
  ...
  defp schedulers1(fd, [{scheds, sched0} | tail], acc0) do
    acc1 = :lists.foldl(fn {name, start}, accIn ->
  ...
  defp get_schedulerinfo2(fd, sched = r_sched(details: ds)) do
    case (line_head(fd)) do
      ...
      'Scheduler Sleep Info Flags' ->
        {:more,
           r_sched(sched, details: Map.put(ds, :sleep_info,
                                           bytes(fd, 'None')))}
      'Scheduler Sleep Info Aux Work' ->
        {:more,
           r_sched(sched, details: Map.put(ds, :sleep_aux,
                                           bytes(fd, 'None')))}
      'Current Process State' ->
        {:more,
           r_sched(sched, details: Map.put(ds, :currp_state, bytes(fd)))}
      'Current Process Internal State' ->
        {:more,
           r_sched(sched, details: Map.put(ds, :currp_int_state,
                                           bytes(fd)))}
      'Current Process Program counter' ->
        {:more,
           r_sched(sched, details: Map.put(ds, :currp_prg_cnt,
                                           string(fd)))}
      'Current Process CP' ->
        {:more,
           r_sched(sched, details: Map.put(ds, :currp_cp, string(fd)))}
  ...
  defp get_runqueue_info2(fd, lineHead, sched = r_sched(details: ds)) do
    case (lineHead) do
      ...
      'Run Queue Flags' ->
        {:more,
           r_sched(sched, details: Map.put(ds, :runq_flags,
                                           bytes(fd, 'None')))}
      '=' ++ _next_tag ->
        {:done, sched}
      other ->
  ...
  defp get_limited_stack(fd, n, ds) do
    case (string(fd)) do
      addr = '0x' ++ _ ->
        get_limited_stack(fd, n + 1,
                            Map.put(ds, {:currp_stack, n}, addr))
      '=' ++ _next_tag ->
        ds
      line ->
        get_limited_stack(fd, n + 1,
                            Map.put(ds, {:currp_stack, n}, line))
    end
  end
  ...
  defp parse_heap_term('Ys' ++ line0, addr, decodeOpts, d0) do
    ...
    {term, line, d1} = deref_bin(binp0, offset, sz, line3,
                                   decodeOpts, d0)
  ...
  defp parse_heap_term('Mf' ++ line0, addr, decodeOpts, d0) do
    ...
    case (parse_term(line1, decodeOpts, d0)) do
      {keys, ':' ++ line2, d1} when is_tuple(keys) ->
        {values, line, d2} = parse_tuple(size, line2, addr,
                                           decodeOpts, d1, [])
  ...
  defp parse_heap_term('Mh' ++ line0, addr, decodeOpts, d0) do
    ...
    {nodes, line, d1} = parse_tuple(n, line2, addr,
                                      decodeOpts, d0, [])
  ...
  defp parse_tuple(n, line0, addr, decodeOpts, d0, acc) do
    case (parse_term(line0, decodeOpts, d0)) do
      {term, [?, | line], d} when n > 1 ->
        parse_tuple(n - 1, line, addr, decodeOpts, d,
                      [term | acc])
      {term, line, d} ->
        parse_tuple(n - 1, line, addr, decodeOpts, d,
                      [term | acc])
  ...
  defp parse_term([?D | line0], decodeOpts, d) do
    try do
      ...
      {attab,
         'E' ++ line2} = parse_atom_translation_table(attabSize,
                                                      line1, [])
  ...
  defp parse_atom([?A | line0], d) do
    {n, ':' ++ line1} = get_hex(line0)
    {chars, line} = get_chars(n, line1)
    {:erlang.binary_to_atom(:erlang.list_to_binary(chars),
                              :utf8),
       line, d}
  end
    ...
    case (lookup_binary_index(binp)) do
      ...
      [] ->
        case (:gb_trees.lookup(binp, d)) do
          {:value,
             <<_ :: size(offset) - bytes, sub :: size(sz) - bytes,
                 _ :: bytes>>} ->
            {:value, sub}
          {:value, symbolicBin} ->
  ...
  defp get_id([?. | line], acc, id) do
    get_id(line, [],
             [:erlang.list_to_integer(:lists.reverse(acc)) | id])
  end
  ...
  defp get_binary_1(n, line0, r_dec_opts(base64: true)) do
    ...
    bin = get_binary_base64(:erlang.list_to_binary(base64),
                              <<>>, false)
    {bin, line}
  end
  ...
  defp get_binary_1(offset, size, line, r_dec_opts(base64: false)) do
    ...
    get_binary_hex(size,
                     :lists.sublist(line, offset * 2 + 1, size * 2), [],
                     progress)
  end
  ...
  defp get_binary_1(startOffset, size, line, r_dec_opts(base64: true)) do
    ...
    case (:erlang.list_to_binary(line)) do
      <<_ :: size(startByte) - bytes,
          base64 :: size(numBytes) - bytes, _ :: bytes>> ->
        bin0 = get_binary_base64(base64, <<>>, progress)
        skip = startOffset - div(startOffset, 3) * 3
        <<_ :: size(skip) - bytes, bin :: size(size) - bytes,
            _ :: bytes>> = bin0
        {bin, []}
      _ ->
        {:"#CDVTruncatedBinary", []}
  ...
  defp get_binary_base64(<<chunk0 :: size(4 * 256) - bytes, t :: bytes>>,
            acc0, progress) do
  ...
  defp count_index(tag) do
    :ets.select_count(:cdv_dump_index_table,
                        [{{{tag, :_}, :_}, [], [true]}])
  end
  ...
  defp progress_pmap(report, file, fun, list) do
    ...
    {[], pids} = :lists.foldl(fn _, {l, ps} ->
                                   ...
                                   {p, _Ref} = spawn_monitor(fn () ->
                                                                  progress_map(collector,
                                                                                 reportInterval,
                                                                                 file,
                                                                                 fun,
                                                                                 l1)
  ...
  defp collect(pids, acc) do
    receive do
      ...
      {:DOWN, _Ref, :process, pid, _Error} ->
        warning = 'WARNING: an error occurred while parsing data.\n' ++ (case (:erlang.get(:truncated)) do
                          true ->
  ...
  defp do_update_progress({count, interval, fun, acc}, processed)
    ...
    do_update_progress({interval, interval, fun, fun.(acc)},
                         processed - count)
  end
