defmodule :m_crashdump_viewer do
  ...
  def script_start([fileAtom]) do
    ...
    case :filelib.is_regular(file) do
      ...
      false ->
        :io.format(~c"cdv error: the given file does not exist\n")
        usage()
    end
      ...
      case :erlang.get(:truncated) do
        true ->
          case :erlang.get(:truncated_reason) do
            :undefined ->
              [~c"WARNING: The crash dump is truncated. Some information might be missing."]

            reason ->
              [
                ~c"WARNING: The crash dump is truncated (" ++
                  reason ++ ~c"). Some information might be missing."
              ]
          end
  ...
  defp unexpected(fd, {:part, what}, where) do
    ...
    :io.format(~c"WARNING: Found unexpected line in ~ts:~n~ts ...~n", [where, what])
  end
  ...
  defp truncated_warning() do
    case :erlang.get(:truncated_reason) do
      :undefined ->
        [~c"WARNING: The crash dump is truncated here. Some information might be missing."]

      reason ->
        [
          ~c"WARNING: The crash dump is truncated here (" ++
            reason ++ ~c"). Some information might be missing."
        ]
    end
  end
  ...
  defp compare_pid(~c"<" ++ id, ~c"<" ++ otherId) do
    id >= otherId
  end
  ...
  defp do_read_file(file) do
    ...
    case :file.read_file_info(file) do
      ...
      when fileA === :read or fileA === :read_write ->
        fd = open(file)
        init_progress(~c"Reading file", size)
        ...
        case progress_read(fd) do
          {:ok, <<?=::size(8), tagAndRest::binary>>} ->
            ...
            case tag do
              ...
              _Other ->
                r = :io_lib.format(~c"~ts is not an Erlang crash dump~n", [file])
                close(fd)
                {:error, r}
            end
          ...
          {:ok, <<"<Erlang crash dump>", _Rest::binary>>} ->
            r =
              :io_lib.format(
                ~c"The crashdump ~ts is in the pre-R10B format, which is no longer supported.~n",
                [file]
              )
  ...
  defp check_dump_version(vsn) do
    ...
    cond do
      dumpVsn > [0, 5] ->
        info =
          ~c"This Crashdump Viewer is too old for the given Erlang crash dump. Please use a newer version of Crashdump Viewer."
  ...
  defp find_truncated_proc({tag, pid}) do
    case is_proc_tag(tag) do
      ...
      false ->
        :erlang.put(:truncated_proc, ~c"<0.0.0>")
    end
  end
  ...
  defp get_slogan_and_sysvsn(fd, acc) do
    case string(fd, :eof) do
      ~c"Slogan: " ++ sloganPart when acc == [] ->
        get_slogan_and_sysvsn(fd, [sloganPart])

      ~c"System version: " ++ systemVsn ->
        {:lists.append(:lists.reverse(acc)), systemVsn}

      :eof ->
        {:lists.append(:lists.reverse(acc)), ~c"-1"}
  ...
  defp get_general_info(fd, genInfo) do
    case line_head(fd) do
      ...
      other ->
        unexpected(fd, other, ~c"general information")
        genInfo
    end
  end
  ...
  defp procs_summary(file, wS) do
    ...
    lookup_and_parse_index(file, :proc, parseFun, ~c"processes")
  end
  ...
  defp all_procinfo(fd, fun, proc, wS, lineHead) do
    case lineHead do
      ...
      ~c"Last scheduled in for" ->
        get_procinfo(
          fd,
          fun,
          r_proc(proc, current_func: {~c"Last scheduled in for", string(fd)}),
          wS
        )

      ~c"Current call" ->
        get_procinfo(fd, fun, r_proc(proc, current_func: {~c"Current call", string(fd)}), wS)

      ~c"Number of heap fragments" ->
        get_procinfo(fd, fun, r_proc(proc, num_heap_frag: bytes(fd)), wS)

      ~c"Heap fragment data" ->
      ...
      ~c"arity = " ++ arity ->
        get_procinfo(fd, fun, r_proc(proc, arity: arity -- ~c"\r\n"), wS)
      ...
      other ->
        unexpected(fd, other, ~c"process info")
        get_procinfo(fd, fun, proc, wS)
    end
  end
    ...
    case get_chunk(fd) do
      ...
      :eof ->
        case byte_list_to_string(:lists.reverse(acc)) do
          nextHeading
          when nextHeading == ~c"Link list" or nextHeading == ~c"Dictionary" or
                 nextHeading == ~c"Reductions" ->
            put_chunk(fd, :erlang.list_to_binary(nextHeading ++ ~c":"))
            :lists.reverse(lines)

          lastCallFunction ->
            :lists.reverse(lines, [lastCallFunction ++ ~c":...(truncated)"])
        end
    end
  end
  ...
  defp get_link_list(bin, acc) do
    case :string.split(bin, ~c", ") do
      [link, rest] ->
        get_link_list(rest, get_link(link, acc))

      [link] ->
        get_link(:string.trim(link, :trailing, ~c"]"), acc)
    end
  end

  defp get_link(~c"#Port" ++ _ = portStr, %{links: links} = acc) do
    Map.put(acc, :links, [{portStr, portStr} | links])
  end

  defp get_link(~c"<" ++ _ = pidStr, %{links: links} = acc) do
    Map.put(acc, :links, [{pidStr, pidStr} | links])
  end

  defp get_link(~c"{to," ++ rest, %{mons: monitors} = acc) do
    Map.put(acc, :mons, [parse_monitor(rest) | monitors])
  end

  defp get_link(~c"{from," ++ rest, %{mon_by: monitoredBy} = acc) do
    Map.put(acc, :mon_by, [parse_monitor(rest) | monitoredBy])
  end

  defp get_link(unexpected, acc) do
    :io.format(~c"WARNING: found unexpected data in link list:~n~ts~n", [unexpected])
    acc
  end

  defp parse_monitor(monitor) do
    case :string.lexemes(monitor, ~c",{}") do
      [node, ~c"[]"] ->
        {node, node ++ ~c" node monitor"}
  ...
  defp expand_memory(fd, pid, dumpVsn) do
    ...
    incompleteWarning =
      case :erlang.erase(:incomplete_heap) do
        ...
        true ->
          [~c"WARNING: This process has an incomplete heap. Some information might be missing."]
      end
  ...
  defp read_literals(fd, decodeOpts) do
    case lookup_index(:literals, []) do
      [{_, start}] ->
        ...
        init_progress(~c"Reading literals", chars)
        pos_bof(fd, start)
  ...
  defp parse_message(line0, decodeOpts, d) do
    {msg, ~c":" ++ line1, _} = parse_term(line0, decodeOpts, d)
  ...
  defp read_heap(fd, pid, decodeOpts, dict0) do
    case lookup_index(:proc_heap, pid) do
      [{_, pos}] ->
        ...
        init_progress(~c"Reading process heap", chars)
        pos_bof(fd, pos)
  ...
  defp read_heap(decodeOpts, dict0) do
    ...
    init_progress(~c"Processing terms", map_size(lineMap))
  ...
  defp get_ports(file) do
    ...
    lookup_and_parse_index(file, :port, parseFun, ~c"ports")
  end

  defp port_to_tuple(~c"#Port<" ++ port) do
    [i1, i2] = :string.lexemes(port, ~c".>")
  ...
  defp get_portinfo(fd, port) do
    case line_head(fd) do
      ...
      ~c"Monitors" ->
        monitors0 = :string.lexemes(bytes(fd), ~c"()")

        monitors =
          for mon <- monitors0 do
            [pid, ref] = :string.lexemes(mon, ~c",")
            {pid, pid ++ ~c" (" ++ ref ++ ~c")"}
          end
      ...
      ~c"Port controls linked-in driver" ->
        str = :lists.flatten([~c"Linked in driver: " | string(fd)])
        get_portinfo(fd, r_port(port, controls: str))

      ~c"Port controls forker process" ->
        str = :lists.flatten([~c"Forker process: " | string(fd)])
        get_portinfo(fd, r_port(port, controls: str))

      ~c"Port controls external process" ->
        str = :lists.flatten([~c"External proc: " | string(fd)])
        get_portinfo(fd, r_port(port, controls: str))

      ~c"Port is a file" ->
        str = :lists.flatten([~c"File: " | string(fd)])
        get_portinfo(fd, r_port(port, controls: str))

      ~c"Port is UNIX fd not opened by emulator" ->
        str = :lists.flatten([~c"UNIX fd not opened by emulator: " | string(fd)])
        get_portinfo(fd, r_port(port, controls: str))
      ...
      other ->
        unexpected(fd, other, ~c"port info")
        port
    end
  end
  ...
  defp get_ets_tables(file, pid, wS) do
    ...
    lookup_and_parse_index(file, {:ets, pid}, parseFun, ~c"ets")
  end
  ...
  defp get_etsinfo(fd, etsTable = r_ets_table(details: ds), wS) do
    case line_head(fd) do
      ...
      ~c"Ordered set (AVL tree), Elements" ->
        skip_rest_of_line(fd)
        get_etsinfo(fd, r_ets_table(etsTable, data_type: ~c"tree"), wS)

      ~c"Buckets" ->
        buckets = :erlang.list_to_integer(:string.trim(bytes(fd), :both, ~c" "))
      ...
      ~c"Chain Length Avg" ->
        val =
          try do
            :erlang.list_to_float(:string.trim(bytes(fd), :both, ~c" "))
          catch
            _, _ ->
              ~c"-"
          end
      ...
      ~c"Chain Length Expected Std Dev" ->
        val = bytes(fd)
      ...
      other ->
        unexpected(fd, other, ~c"ETS info")
        etsTable
    end
  end
  ...
  defp get_timers(file, pid) do
    ...
    t1 = lookup_and_parse_index(file, {:timer, pid}, parseFun, ~c"timers")

    t2 =
      case :ets.lookup(:cdv_reg_proc_table, pid) do
        [{_, name}] ->
          lookup_and_parse_index(file, {:timer, name}, parseFun, ~c"timers")
  ...
  defp get_timerinfo_1(fd, timer) do
    case line_head(fd) do
      ...
      ~c"Time left" ->
        timeLeft = :erlang.list_to_integer(bytes(fd) -- ~c" ms")
      ...
      other ->
        unexpected(fd, other, ~c"timer info")
        timer
    end
  end
  ...
  defp get_nodeinfo(fd, nod) do
    case line_head(fd) do
      ...
      ~c"Error" ->
        get_nodeinfo(fd, r_nod(nod, error: ~c"ERROR: " ++ string(fd)))

      ~c"=" ++ _next_tag ->
        nod

      other ->
        unexpected(fd, other, ~c"node info")
        nod
    end
  end
  ...
  defp get_loaded_mod_details(file, mod, decodeOpts) do
    ...
    fd = open(file)
    pos_bof(fd, start)
    initLM = r_loaded_mod(mod: mod, old_size: ~c"No old code exists")
  ...
  defp loaded_mods(file) do
    ...
    {cC, oC} =
      case lookup_index(:loaded_modules) do
        [{_, startTotal}] ->
          ...
          r = get_loaded_mod_totals(fd, {~c"unknown", ~c"unknown"})
          close(fd)
          r
    ...
    {cC, oC, lookup_and_parse_index(file, :mod, parseFun, ~c"modules")}
  end
  ...
  defp get_loaded_mod_totals(fd, {cC, oC}) do
    case line_head(fd) do
      ...
      other ->
        unexpected(fd, other, ~c"loaded modules info")
        {cC, oC}
    end
  end
  ...
  defp do_get_attribute(fd, decodeOpts) do
    ...
    try do
      ...
      _, _ ->
        {~c"WARNING: The term is probably truncated!", ~c"I cannot convert to binary.", bytes}
    else
      {bin, _} ->
        try do
          ...
          _, _ ->
            {~c"WARNING: The term is probably truncated!", ~c"I cannot do binary_to_term/1.", bin}
        else
          term ->
            term
        end
  ...
  defp funs(file) do
    ...
    lookup_and_parse_index(file, :fu, parseFun, ~c"funs")
  end
  ...
  defp get_funinfo(fd, fu) do
    case line_head(fd) do
      ...
      other ->
        unexpected(fd, other, ~c"fun info")
        fu
    end
  end
  ...
  defp get_atoms(fd, numAtoms) do
    case get_chunk(fd) do
      {:ok, bin} ->
        init_progress(~c"Processing atoms", numAtoms)
  ...
  defp allocator_summary(allocators) do
    ...
    {totalMCS, typeTotals} =
      case :lists.keytake(~c"mseg_alloc", 1, typeTotals0) do
        {:value, {_, [{~c"segments_size", segSize}]}, rest} ->
    ...
    {~c"Allocator Summary", [~c"blocks size", ~c"carriers size", ~c"mseg carriers size"],
     [
       {~c"total", [totalBS, totalCS, totalMCS]}
  ...
  defp sort_type_data(type, [{~c"option e", ~c"false"} | data], acc, _)
       when type !== ~c"sbmbc_alloc" do
  ...
  defp sort_type_data(type, [{key, val0} | data], acc, doTotal) do
    case :lists.member(
           key,
           [
             ~c"sbmbcs blocks size",
             ~c"mbcs blocks size",
             ~c"mbcs_pool blocks size",
             ~c"sbcs blocks size",
             ~c"sbmbcs carriers size",
             ~c"mbcs carriers size",
             ~c"sbcs carriers size",
             ~c"mbcs mseg carriers size",
             ~c"mbcs_pool carriers size",
             ~c"sbcs mseg carriers size",
             ~c"segments_size"
           ]
         ) do
      true ->
      ...
      case :lists.member(key, [
        ...
        false ->
          case :lists.member(key, [
            ...
            false ->
              case :lists.member(key, [~c"mbcs mseg carriers size", ~c"sbcs mseg carriers size"]) do
                true ->
  ...
  defp get_hashtableinfo1(fd, hashTable) do
    case line_head(fd) do
      ...
      other ->
        unexpected(fd, other, ~c"hash table information")
        hashTable
    end
  end
  ...
  defp get_indextableinfo1(fd, indexTable) do
    case line_head(fd) do
      ...
      other ->
        unexpected(fd, other, ~c"index table information")
        indexTable
    end
  end
  ...
  defp get_schedulerinfo2(fd, sched = r_sched(details: ds)) do
    case line_head(fd) do
      ~c"Current Process" ->
        {:more, r_sched(sched, process: bytes(fd, ~c"None"))}

      ~c"Current Port" ->
        {:more, r_sched(sched, port: bytes(fd, ~c"None"))}

      ~c"Scheduler Sleep Info Flags" ->
        {:more, r_sched(sched, details: Map.put(ds, :sleep_info, bytes(fd, ~c"None")))}

      ~c"Scheduler Sleep Info Aux Work" ->
        {:more, r_sched(sched, details: Map.put(ds, :sleep_aux, bytes(fd, ~c"None")))}

      ~c"Current Process State" ->
        {:more, r_sched(sched, details: Map.put(ds, :currp_state, bytes(fd)))}

      ~c"Current Process Internal State" ->
        {:more, r_sched(sched, details: Map.put(ds, :currp_int_state, bytes(fd)))}

      ~c"Current Process Program counter" ->
        {:more, r_sched(sched, details: Map.put(ds, :currp_prg_cnt, string(fd)))}

      ~c"Current Process CP" ->
        {:more, r_sched(sched, details: Map.put(ds, :currp_cp, string(fd)))}

      ~c"Current Process Limited Stack Trace" ->
      ...
      other ->
        case r_sched(sched, :type) do
          ...
          _ ->
            unexpected(fd, other, ~c"dirty scheduler information")
            {:done, sched}
        end
    end
  end
  ...
  defp get_runqueue_info2(fd, lineHead, sched = r_sched(details: ds)) do
    case lineHead do
      ...
      ~c"Run Queue Port Length" ->
        rQ = :erlang.list_to_integer(bytes(fd))
        {:more, r_sched(sched, port_q: rQ)}

      ~c"Run Queue Flags" ->
        {:more, r_sched(sched, details: Map.put(ds, :runq_flags, bytes(fd, ~c"None")))}

      ~c"=" ++ _next_tag ->
        {:done, sched}

      other ->
        unexpected(fd, other, ~c"scheduler information")
        {:done, sched}
    end
  end
  ...
  defp parse_heap_term([?l | line0], addr, decodeOpts, d0) do
    {h, ~c"|" ++ line1, d1} = parse_term(line0, decodeOpts, d0)
  ...
  defp parse_heap_term(~c"B16#" ++ line0, addr, _DecodeOpts, d0) do
    {term, line} = get_hex(line0)
    d = :gb_trees.insert(addr, term, d0)
    {term, line, d}
  end

  defp parse_heap_term(~c"B-16#" ++ line0, addr, _DecodeOpts, d0) do
  ...
  defp parse_heap_term(~c"B" ++ line0, addr, _DecodeOpts, d0) do
  ...
  defp parse_heap_term(~c"E" ++ line0, addr, decodeOpts, d0) do
  ...
  defp parse_heap_term(~c"Yh" ++ line0, addr, decodeOpts, d0) do
    {term, line} = get_binary(line0, decodeOpts)
    d = :gb_trees.insert(addr, term, d0)
    {term, line, d}
  end

  defp parse_heap_term(~c"Yc" ++ line0, addr, decodeOpts, d0) do
    {binp0, ~c":" ++ line1} = get_hex(line0)
    {offset, ~c":" ++ line2} = get_hex(line1)
  ...
  defp parse_heap_term(~c"Ys" ++ line0, addr, decodeOpts, d0) do
    {binp0, ~c":" ++ line1} = get_hex(line0)
    {offset, ~c":" ++ line2} = get_hex(line1)
  ...
  defp parse_heap_term(~c"Mf" ++ line0, addr, decodeOpts, d0) do
    {size, ~c":" ++ line1} = get_hex(line0)

    case parse_term(line1, decodeOpts, d0) do
      {keys, ~c":" ++ line2, d1} when is_tuple(keys) ->
  ...
  defp parse_heap_term(~c"Mh" ++ line0, addr, decodeOpts, d0) do
    {mapSize, ~c":" ++ line1} = get_hex(line0)
    {n, ~c":" ++ line2} = get_hex(line1)
  ...
  defp parse_heap_term(~c"Mn" ++ line0, addr, decodeOpts, d) do
    {n, ~c":" ++ line} = get_hex(line0)
    parse_tuple(n, line, addr, decodeOpts, d, [])
  end
  ...
  defp parse_term([?D | line0], decodeOpts, d) do
    try do
      {attabSize, ~c":" ++ line1} = get_hex(line0)
      {attab, ~c"E" ++ line2} = parse_atom_translation_table(attabSize, line1, [])
  ...
  defp get_binary_1(offset, size, line, r_dec_opts(base64: false)) do
    progress = size > 10000
    progress and init_progress(~c"Reading binary", size)
  ...
  defp tag_to_atom(~c"abort") do
    :abort
  end

  defp tag_to_atom(~c"allocated_areas") do
    :allocated_areas
  end

  defp tag_to_atom(~c"allocator") do
    :allocator
  end

  defp tag_to_atom(~c"atoms") do
    :atoms
  end

  defp tag_to_atom(~c"binary") do
    :binary
  end

  defp tag_to_atom(~c"dirty_cpu_scheduler") do
    :dirty_cpu_scheduler
  end

  defp tag_to_atom(~c"dirty_cpu_run_queue") do
    :dirty_cpu_run_queue
  end

  defp tag_to_atom(~c"dirty_io_scheduler") do
    :dirty_io_scheduler
  end

  defp tag_to_atom(~c"dirty_io_run_queue") do
    :dirty_io_run_queue
  end

  defp tag_to_atom(~c"end") do
    :ende
  end

  defp tag_to_atom(~c"erl_crash_dump") do
    :erl_crash_dump
  end

  defp tag_to_atom(~c"ets") do
    :ets
  end

  defp tag_to_atom(~c"fun") do
    :fu
  end

  defp tag_to_atom(~c"hash_table") do
    :hash_table
  end

  defp tag_to_atom(~c"hidden_node") do
    :hidden_node
  end

  defp tag_to_atom(~c"index_table") do
    :index_table
  end

  defp tag_to_atom(~c"instr_data") do
    :instr_data
  end

  defp tag_to_atom(~c"internal_ets") do
    :internal_ets
  end

  defp tag_to_atom(~c"literals") do
    :literals
  end

  defp tag_to_atom(~c"loaded_modules") do
    :loaded_modules
  end

  defp tag_to_atom(~c"memory") do
    :memory
  end

  defp tag_to_atom(~c"mod") do
    :mod
  end

  defp tag_to_atom(~c"persistent_terms") do
    :persistent_terms
  end

  defp tag_to_atom(~c"no_distribution") do
    :no_distribution
  end

  defp tag_to_atom(~c"node") do
    :node
  end

  defp tag_to_atom(~c"not_connected") do
    :not_connected
  end

  defp tag_to_atom(~c"old_instr_data") do
    :old_instr_data
  end

  defp tag_to_atom(~c"port") do
    :port
  end

  defp tag_to_atom(~c"proc") do
    :proc
  end

  defp tag_to_atom(~c"proc_dictionary") do
    :proc_dictionary
  end

  defp tag_to_atom(~c"proc_heap") do
    :proc_heap
  end

  defp tag_to_atom(~c"proc_messages") do
    :proc_messages
  end

  defp tag_to_atom(~c"proc_stack") do
    :proc_stack
  end

  defp tag_to_atom(~c"scheduler") do
    :scheduler
  end

  defp tag_to_atom(~c"timer") do
    :timer
  end

  defp tag_to_atom(~c"visible_node") do
    :visible_node
  end

  defp tag_to_atom(unknownTag) do
    :io.format(~c"WARNING: Found unexpected tag:~ts~n", [unknownTag])
  ...
  defp collect(pids, acc) do
    receive do
      ...
      {:DOWN, _Ref, :process, pid, _Error} ->
        warning =
          ~c"WARNING: an error occurred while parsing data.\n" ++
            case :erlang.get(:truncated) do
              true ->
                ~c"This might be because the dump is truncated.\n"
