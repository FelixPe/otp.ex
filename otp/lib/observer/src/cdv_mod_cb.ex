defmodule :m_cdv_mod_cb do
  ...
  def col_spec() do
    [{~c"Module", 0, 300}, {~c"Current size", 1, 80}, {~c"Old size", 1, 80}]
  end
  ...
  def get_details(id, _) do
    ...
    title = :io_lib.format(~c"~s", [r_loaded_mod(info, :mod)])
  ...
  def detail_pages() do
    [
      ...
      {~c"Current Attributes", &init_curr_attr_page/2},
      {~c"Current Compilation Info", &init_curr_comp_page/2},
      {~c"Old Attributes", &init_old_attr_page/2},
      {~c"Old Compilation Info", &init_old_comp_page/2}
    ]
  end
  ...
  def format({bin, :nq}) when is_binary(bin) do
    :lists.flatten(:io_lib.format(~c"~ts", [bin]))
  end
  ...
  defp info_fields() do
    [
      {~c"Overview",
       [{~c"Name", :mod}, {~c"Current Size", :current_size}, {~c"Old Size", :old_size}]}
    ]
  end
end
