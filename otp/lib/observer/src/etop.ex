defmodule :m_etop do
  ...
  defp check_runtime_config(:sort, s) when s === :runtime or
                           s === :reductions or s === :memory or s === :msg_q do
    :ok
  end

  defp check_runtime_config(:accumulate, a) when a === true or
                                 a === false do
    :ok
  end
  ...
  def start(opts) do
    ...
    config1 = handle_args(:init.get_arguments() ++ opts,
                            r_opts())
  ...
  def update(r_opts(store: store, node: node, tracing: tracing,
             intv: interval) = opts) do
    ...
    procInfo1 = (cond do
                   tracing == :on ->
                     pI = :lists.map(fn pI = r_etop_proc_info(pid: p) ->
  ...
  defp sort(opts, pI) do
    ...
    pI1 = (cond do
             ...
             true ->
               ...
               :lists.map(fn r_etop_proc_info(pid: pid, reds: reds,
                                 runtime: rT) = i ->
                               newI = (case (:ets.lookup(accumTab, pid)) do
                                         [r_etop_proc_info(reds: oldReds, runtime: :-)] ->
                                           r_etop_proc_info(i, reds: reds - oldReds, 
                                                  runtime: :-)
  ...
  defp putopt(key, value, config)
    ...
    send(r_opts(config1, :out_proc), {:config, {key, value},
                                   config1})
    config1
  end
  ...
  def loadinfo(sysI, prev) do
    r_etop_info(n_procs: procs, run_queue: rQ, now: now,
        wall_clock: wC, runtime: rT) = sysI
    cpu = calculate_cpu_utilization(wC, rT,
                                      r_etop_info(prev, :runtime))
    clock = :io_lib.format('~2.2.0w:~2.2.0w:~2.2.0w',
                             :erlang.tuple_to_list(:erlang.element(2,
  ...
  defp calculate_cpu_utilization(_, rTInfo, prevRTInfo) do
    sum = :lists.foldl(fn {{_, a0, t0}, {_, a1, t1}},
                            {aAcc, tAcc} ->
                            {a1 - a0 + aAcc, t1 - t0 + tAcc}
                       end,
                         {0, 0}, :lists.zip(prevRTInfo, rTInfo))
    case (sum) do
      {0, 0} ->
        0
      {active, total} ->
