defmodule :m_etop do
  ...
  def start(opts) do
    ...
    case :net_adm.ping(node) do
      :pang when node != node() ->
        :io.format(~c"Error Couldn't connect to node ~p ~n~n", [node])
        help()
        exit(~c"connection error")
  ...
  defp check_runtime_tools_vsn(node) do
    case :rpc.call(node, :observer_backend, :vsn, []) do
      ...
      _ ->
        exit(~c"Faulty version of runtime_tools on remote node")
    end
  end
    ...
    receive do
      ...
      {:EXIT, ePid, reason} when ePid == r_opts(opts, :out_proc) ->
        case reason do
          ...
          _ ->
            :io.format(~c"Output server crashed: ~tp~n", [reason])
        end
      ...
      {:EXIT, ^reader, :eof} ->
        :io.format(~c"Lost connection to node ~p exiting~n", [r_opts(opts, :node)])
        stop(opts)
        :connection_lost
  ...
  def update(r_opts(store: store, node: node, tracing: tracing, intv: interval) = opts) do
    ...
    info =
      ...
      after
        interval ->
          :io.format(~c"Timeout when waiting for process info from node ~p; exiting~n", [node])
          exit(:timeout)
      end
  ...
  defp output(:graphical) do
    exit({:deprecated, ~c"Use observer instead"})
  end
