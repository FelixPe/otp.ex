defmodule :m_ttb do
  ...
  defp clients(nodes, {:wrap, name}) do
    f = fn node ->
      ...
      {node, {traceFile ++ ~c".", :wrap, ~c".wrp"}, traceFile}
    end
  ...
  defp clients(nodes, {:wrap, name, size, count}) do
    f = fn node ->
      traceFile = name(node, name)
      {node, {traceFile ++ ~c".", :wrap, ~c".wrp", size, count}, traceFile}
    end
  ...
  defp name(node, filename) do
    ...
    :filename.join(
      dir,
      :erlang.atom_to_list(node) ++ ~c"-" ++ file
    )
  end
  ...
  defp print_func(m, f, a) do
    args = arg_list(a, [])
    :io.format(~c"~w:~tw(~ts) ->~n", [m, f, args])
  end

  defp print_result(r) do
    :io.format(~c"~tp~n~n", [r])
  end
  ...
  defp arg_list([a1], acc) do
    acc ++ :io_lib.format(~c"~tw", [a1])
  end

  defp arg_list([a1 | a], acc) do
    arg_list(a, acc ++ :io_lib.format(~c"~tw,", [a1]))
  end
  ...
  def stop(opts) when is_list(opts) do
    ...
    case {fetch, result} do
      ...
      {_, {:stopped, _}} ->
        :io.format(~c"Stored logs in ~ts~n", [:erlang.element(2, result)])
  ...
  defp loop(nodeInfo, sessionInfo) do
    receive do
      ...
      {:node_overloaded, node} ->
        :io.format(~c"Overload check activated on node: ~p.~n", [node])
  ...
  defp do_stop(:nofetch, sender, nodeInfo, sessionInfo) do
    write_config(~c"ttb_last_config", :all)
  ...
  defp do_stop({fetchOrFormat, userDir}, sender, nodeInfo, sessionInfo) do
    ...
    :ok = :filelib.ensure_dir(:filename.join(dir, ~c"*"))
  ...
  defp get_fetch_dir(:undefined, :undefined) do
    ~c"ttb_upload_" ++ ~c"ttb" ++ ts()
  end

  defp get_fetch_dir(:undefined, logname) do
    ~c"ttb_upload_" ++ logname ++ ts()
  end
  ...
  defp ts() do
    ...
    :io_lib.format(~c"-~4.4.0w~2.2.0w~2.2.0w-~2.2.0w~2.2.0w~2.2.0w", [y, m, d, h, min, s])
  end
  ...
  defp copy_partials(dir, num) do
    partialDir = ~c"ttb_partial_result" ++ :erlang.integer_to_list(num)
  ...
  defp fetch_partial_result(node, metaFile, current) do
    dirName = ~c"ttb_partial_result" ++ :erlang.integer_to_list(current)
  ...
  defp decode_filename(3, bin, :latin1) do
    ...
    :io.format(
      ~c"Warning: fetching file with faulty filename encoding ~ts~nWill be written as ~ts~n",
      [file0, file]
    )
  ...
  defp host(node) do
    [_name, host] = :string.lexemes(:erlang.atom_to_list(node), ~c"@")
    host
  end
        ...
        {n, {:local, c, _}, f} ->
          metaFile =
            case f do
              ...
              ^f ->
                absFile = :filename.join(cwd, f) ++ ~c".ti"
                :file.delete(absFile)
                absFile
            end
  ...
  defp format(file, out, handler, disableSort)
    ...
    files =
      case :filelib.is_dir(file) do
        true ->
          list = :filelib.wildcard(:filename.join(file, ~c"ttb_partial_result" ++ ~c"*"))
  ...
  defp collect_files(dirs) do
    :lists.map(
      fn dir ->
        metaFiles = :filelib.wildcard(:filename.join(dir, ~c"*.ti"))

        :lists.map(
          fn m ->
            sub = :filename.rootname(m, ~c".ti")
  ...
  defp read_traci(file) do
    ...
    case :file.read_file(metaFile) do
      ...
      _ ->
        :io.format(~c"Warning: no meta data file: ~ts~n", [metaFile])
        {:dict.new(), []}
    end
  end

  defp get_metafile(file) do
    case :filename.rootname(file, ~c".wrp") do
      ^file ->
        file ++ ~c".ti"

      wrap ->
        :filename.rootname(wrap) ++ ~c".ti"
    end
  end
  ...
  defp check_client(client, file)
    ...
    root = :filename.rootname(file, ~c".wrp")

    case :filename.extension(root) do
      ~c".*" ->
        part1 = :filename.rootname(root, ~c"*")
  ...
  defp display_warning(item, warning) do
    :io.format(~c"Warning: {~tw,~tw}~n", [warning, item])
  end
