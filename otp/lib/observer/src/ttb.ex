defmodule :m_ttb do
  ...
  def tracer(nodes, opt) do
    ...
    do_tracer(nodes, pI, client,
                [{:ttb_control, pid} | traci])
  end
  ...
  defp do_tracer(clients, pI, traci) do
    ...
    ipPortSpec = (case (:proplists.get_value(:queue_size,
                                               traci)) do
                    :undefined ->
                      0
                    qS ->
    ...
    {clientSucc, succ} = :lists.foldl(fn {n, {:local, file},
                                            tF},
                                           {cS, s} ->
                                           {tF2, fileInfo,
                                              shellOutput} = (case (shell) do
                                                                :only ->
                                                                  {:none,
                                                                     :shell_only,
                                                                     defShell}
                                           ...
                                           host = (case (n) do
                                                     ...
                                                     _ ->
                                                       [_,
                                                            h] = :string.lexemes(:erlang.atom_to_list(n),
                                                                                   '@')
                                                       h
                                                   end)
                                           case ((try do
                                             ...
                                             {:ok, ^n} ->
                                               {:ok,
                                                  port} = :dbg.trace_port_control(n,
                                                                                    :get_listen_port)
                                               {:ok, t} = :dbg.get_tracer(n)
                                               :rpc.call(n, :seq_trace,
                                                           :set_system_tracer,
                                                           [t])
                                               :dbg.trace_client(:ip,
                                                                   {host, port},
  ...
  defp opt([{:handler, handler} | o],
            {pI, client, traci}) do
    opt(o, {pI, client, [{:handler, handler} | traci]})
  end

  defp opt([{:timer, {mSec, stopOpts}} | o],
            {pI, client, traci}) do
    opt(o,
  ...
  defp opt([{:overload_check, {mSec, m, f}} | o],
            {pI, client, traci}) do
    opt(o,
  ...
  defp opt([{:queue_size, queueSize} | o],
            {pI, client, traci}) do
  ...
  defp ensure_opt({pI, client, traci}) do
    ...
    needIpTracer = :proplists.get_value(:shell, traci,
                                          false) != false
  ...
  defp store(func, args) do
    last = (case (:ets.last(:ttb_history_table)) do
              :"$end_of_table" ->
                0
              int when is_integer(int) ->
                int
            end)
    :ets.insert(:ttb_history_table,
  ...
  def write_config(configFile, nums, opt) when (is_list(nums) and
  ...
  defp check_config([{:ttb = mod, func, args} | rest], acc) do
    case (:erlang.function_exported(mod, func,
                                      length(args))) do
      true ->
  ...
  defp no_store_p(procsPorts0, flags0) do
    case (transform_flags(to_list(flags0))) do
      ...
      flags ->
        ...
        case (:lists.foldl(fn p, {pMatched, ps} ->
  ...
  defp proc_port(p) when p === :all or p === :ports or
  ...
  defp ensure_no_overloaded_nodes() do
    overloaded = (case (:erlang.whereis(:ttb)) do
                    :undefined ->
                      []
                    _ ->
  ...
  defp seq_trigger_ms([flag | flags], body) do
    case (:lists.member(flag,
                          [:send, :receive, :print, :timestamp])) do
      true ->
        seq_trigger_ms(flags,
  ...
  def stop(opts) when is_list(opts) do
    ...
    result = (case (:erlang.whereis(:ttb)) do
                :undefined ->
                  :ok
                pid when is_pid(pid) ->
  ...
  defp stop_opts(opts) do
    ...
    formatData = (case (:proplists.get_value(:format,
                                               opts)) do
                    :undefined ->
                      false
                    true ->
    ...
    case ({formatData,
             :lists.member(:return_fetch_dir, opts)}) do
      {false, true} ->
        {:fetch, fetchDir}
      {false, false} ->
  ...
  defp init(parent, sessionInfo) do
    ...
    :ets.new(:ttb_history_table,
               [:ordered_set, :named_table, :public])
    send(parent, {:started, self()})
    newSessionInfo = [{:partials, 0}, {:dead_nodes, []} |
                                          sessionInfo]
  ...
  defp loop(nodeInfo, sessionInfo) do
    receive do
      {:init_node, node, metaFile, pI, traci} ->
        ...
        {absoluteMetaFile, metaPid} = (case (:rpc.call(node,
                                                         :observer_backend,
                                                         :ttb_init_node,
                                                         [metaFile, pI,
                                                                        traci])) do
        ...
        loop(:dict.store(node, {absoluteMetaFile, metaPid},
                           nodeInfo),
               sessionInfo)
      {:ip_to_file_trace_port, port, sender} ->
        ports = :proplists.get_value(:ip_to_file_trace_ports,
                                       sessionInfo, [])
        newSessionInfo = [{:ip_to_file_trace_ports,
                             [port | ports]} |
                              sessionInfo]
        send(sender, {:ttb, :ok})
      ...
      {:write_trace_info, key, what} ->
        :dict.fold(fn node, {_MetaFile, metaPid}, _ ->
      ...
      {:noderesumed, node, reporter} ->
        {metaFile, currentSuffix,
           newState} = make_node_alive(node, sessionInfo)
      ...
      {:node_overloaded, node} ->
        ...
        {overloaded, sI} = {:proplists.get_value(:overloaded,
                                                   sessionInfo, []),
      ...
      {:stop, fetchSpec, sender} ->
        case (:proplists.get_value(:shell, sessionInfo,
                                     false)) do
          :only ->
            do_stop(:nofetch, sender, nodeInfo, sessionInfo)
          _ ->
  ...
  defp do_stop({fetchOrFormat, userDir}, sender, nodeInfo,
            sessionInfo) do
    write_config('ttb_last_config', :all)
    localhost = host(node())
    ...
    allNodesAndMeta = :dict.fold(fn node,
                                      {metaFile, metaPid}, nodes ->
                                      :rpc.call(node, :observer_backend,
                                                  :ttb_stop, [metaPid])
                                      [{node, metaFile} | nodes]
                                 end,
    ...
    stop_ip_to_file_trace_ports(sessionInfo)
    :dbg.stop()
    allNodes = :lists.map(fn {node, metaFile} ->
                               spawn(fn () ->
                                          fetch_report(localhost, dir, node,
                                                         metaFile)
                                     end)
                               node
                          end,
  ...
  defp make_node_alive(node, sessionInfo) do
    deadNodes = :proplists.get_value(:dead_nodes,
                                       sessionInfo)
    partials = :proplists.get_value(:partials, sessionInfo)
    {:value, {_, metaFile}, dn2} = :lists.keytake(node, 1,
                                                    deadNodes)
    sessionInfo2 = :lists.keyreplace(:dead_nodes, 1,
                                       sessionInfo, {:dead_nodes, dn2})
    {metaFile, partials + 1,
       :lists.keyreplace(:partials, 1, sessionInfo2,
                           {:partials, partials + 1})}
  end
  ...
  defp ts() do
    {{y, m, d},
       {h, min,
          s}} = :calendar.now_to_local_time(:erlang.timestamp())
  ...
  defp fetch(localhost, dir, node, metaFile) do
    case (host(node) == localhost or is_local(metaFile)) do
      true ->
        ...
        :lists.foreach(fn file0 ->
                            dest = :filename.join(dir,
      ...
      false ->
        {:ok, lSock} = :gen_tcp.listen(0,
                                         [:binary, {:packet, 2}, {:active,
                                                                    false}])
        {:ok, port} = :inet.port(lSock)
        enc = :file.native_name_encoding()
        args = (case (:rpc.call(node, :erlang,
                                  :function_exported,
                                  [:observer_backend, :ttb_fetch, 3])) do
                  true ->
  ...
  defp receive_files(dir, sock, fd, enc) do
    case (:gen_tcp.recv(sock, 0)) do
      ...
      {:ok, <<code, bin :: binary>>} when code == 1 or
                                            code == 2 or code == 3
                                          ->
  ...
  defp decode_filename(3, bin, :latin1) do
    ...
    :io.format('Warning: fetching file with faulty filename encoding ~ts~nWill be written as ~ts~n', [file0, file])
    file
  end
  ...
  defp write_info(nodes, pI, traci) do
    {:ok, cwd} = :file.get_cwd()
    :lists.foreach(fn {n, {:local, c, _}, f} ->
                        metaFile = (case (f) do
                                      :none ->
                                        :none
                                      ^f ->
                        ...
                        send(:ttb, {:init_node, n, {:local, metaFile, port}, pI,
                                      traci1})
                      {n, c, f} ->
                        metaFile = f ++ '.ti'
                        traci1 = [{:node, n}, {:file, c} | traci]
  ...
  defp format(file, out, handler, disableSort)
    ...
    files = (case (:filelib.is_dir(file)) do
               true ->
  ...
  defp format(files, out, handler, disableSort)
    ...
    stopDbg = (case (:erlang.whereis(:dbg)) do
                 :undefined ->
                   true
                 _ ->
                   false
               end)
    details = :lists.foldl(fn file, acc ->
                                [prepare(file) | acc]
                           end,
                             [], files)
    fd = get_fd(out)
  ...
  defp format_opt(opt) when is_list(opt) do
    out = (case (:lists.keysearch(:out, 1, opt)) do
             {:value, {:out, o}} ->
               o
             _ ->
               :standard_io
           end)
    handler = (case (:lists.keysearch(:handler, 1, opt)) do
                 {:value, {:handler, h}} ->
                   h
                 _ ->
                   :undefined
               end)
    disableSort = :proplists.get_value(:disable_sort, opt,
                                         false)
  ...
  defp check_client(client, file) when (is_tuple(client) and
  ...
  defp do_format(fd, details, disableSort, handler) do
    clients = :lists.foldl(fn {fileOrWrap, traci}, acc ->
  ...
  defp collector(fd, [{_, {client, {trace, traci}}} | rest],
            disableSort, commonState) do
    trace1 = update_procinfo(trace)
    commonState2 = handler2(trace1,
    ...
    case (get_next(client)) do
      ...
      next ->
        case (disableSort) do
          false ->
            collector(fd, sort([next | rest]), disableSort,
                        commonState2)
          true ->
  ...
  defp update_procinfo(trace) when :erlang.element(1,
                                        trace) == :seq_trace do
    info = :erlang.element(3, trace)
    info1 = (case (info) do
               {:send, serial, from, to, msg} ->
                 {:send, serial, get_procinfo(from), get_procinfo(to),
                    msg}
               {:receive, serial, from, to, msg} ->
                 {:receive, serial, get_procinfo(from), get_procinfo(to),
                    msg}
  ...
  defp update_procinfo(trace) when :erlang.element(3,
                                        trace) == :send do
  ...
  defp timestamp(trace) when :erlang.element(1,
                                        trace) === :trace_ts or
