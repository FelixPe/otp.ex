defmodule :m_cdv_sched_cb do
  ...
  def col_spec() do
    [{'Id', 1, 50}, {'Type', 2, 100}, {'Current Process', 2, 130}, {'Current Port', 2, 130}, {'Run Queue Length',
                                                           1, 180},
  ...
  def get_details(id, data) do
    proplist = :crashdump_viewer.to_proplist(Keyword.keys(r_sched(r_sched())),
                                               data)
    {:ok, {'Scheduler: ' ++ id, proplist, ''}}
  end

  def get_detail_cols(:all) do
    {[{:sched, 0}, {:process, 0 + 1 + 1}, {:process,
                                             0 + 1 + 1 + 1}],
       true}
  end
  ...
  defp info_fields(type) do
    [{'Scheduler Overview',
        [{'Id', :id}, {'Type', :type}, {'Current Process', :process}, {'Current Port', :port}, {'Sleep Info Flags',
                                                             :sleep_info},
                                                              {'Sleep Aux Work', :sleep_aux}]},
         {run_queues_header(type),
            [{'Flags', :runq_flags}, {'Priority Max Length', :runq_max}, {'Priority High Length', :runq_high}, {'Priority Normal Length',
                                                                   :runq_norm},
                                                                    {'Priority Low Length',
                                                                       :runq_low},
                                                                        {'Port Length',
                                                                           :port_q}]},
             {'Current Process',
                [{'State', :currp_state}, {'Internal State', :currp_int_state}, {'Program Counter',
                                                              :currp_prg_cnt},
                                                               {'CP', :currp_cp},
                                                                   {'Stack',
                                                                      {:currp_stack,
