defmodule :m_cdv_sched_cb do
  ...
  def col_spec() do
    [
      {~c"Id", 1, 50},
      {~c"Type", 2, 100},
      {~c"Current Process", 2, 130},
      {~c"Current Port", 2, 130},
      {~c"Run Queue Length", 1, 180},
      {~c"Port Queue Length", 1, 180}
    ]
  end
  ...
  def get_details(_Id, :not_found) do
    info = ~c"The scheduler you are searching for could not be found."
    {:info, info}
  end
  ...
  def detail_pages() do
    [{~c"Scheduler Information", &init_gen_page/2}]
  end
    ...
    [
      {~c"Scheduler Overview",
       [
         {~c"Id", :id},
         {~c"Type", :type},
         {~c"Current Process", :process},
         {~c"Current Port", :port},
         {~c"Sleep Info Flags", :sleep_info},
         {~c"Sleep Aux Work", :sleep_aux}
       ]},
      {run_queues_header(type),
       [
         {~c"Flags", :runq_flags},
         {~c"Priority Max Length", :runq_max},
         {~c"Priority High Length", :runq_high},
         {~c"Priority Normal Length", :runq_norm},
         {~c"Priority Low Length", :runq_low},
         {~c"Port Length", :port_q}
       ]},
      {~c"Current Process",
       [
         {~c"State", :currp_state},
         {~c"Internal State", :currp_int_state},
         {~c"Program Counter", :currp_prg_cnt},
         {~c"CP", :currp_cp},
         {~c"Stack", {:currp_stack, 0}},
         {~c"     ", {:currp_stack, 1}},
         {~c"     ", {:currp_stack, 2}},
         {~c"     ", {:currp_stack, 3}},
         {~c"     ", {:currp_stack, 4}},
  ...
  defp run_queues_header(dirtyX) do
    ~c"Run Queues (common for all '" ++ :erlang.atom_to_list(dirtyX) ++ ~c"' schedulers)"
  end
end
