defmodule :m_observer_app_wx do
  ...
  def init([notebook, parent, _Config]) do
    ...
    {fg, bGBrush, pen} =
      case :observer_lib.is_darkmode(bG) do
        false ->
          {:wxSystemSettings.getColour(18), :wxBrush.new(:wxSystemSettings.getColour(16)),
  ...
  def handle_event(
    ...
    :observer_lib.display_info_dialog(panel, ~c"Select process first")
    {:noreply, state}
  end
  ...
  def handle_event(
    ...
    case :observer_lib.user_term(panel, ~c"Enter message", ~c"") do
      :cancel ->
        :ok
  ...
  def handle_event(
    ...
    case :observer_lib.user_term(panel, ~c"Enter Exit Reason", ~c"kill") do
      :cancel ->
        :ok
  ...
  defp handle_mouse_click(
    ...
    :observer_wx.set_status(:io_lib.format(~c"Pid: ~p", [pid]))
  ...
  defp create_menus(parent, _) do
    menuEntries = [
      {~c"File",
       [
         r_create_menu(id: 101, text: ~c"Process info"),
         r_create_menu(id: 102, text: ~c"Send Msg"),
         r_create_menu(
           id: 103,
           text: ~c"Kill process"
         )
      ...
      {~c"Trace",
       [
         r_create_menu(id: 104, text: ~c"Trace process"),
         r_create_menu(id: 105, text: ~c"Trace named process"),
         r_create_menu(
           id: 106,
           text: ~c"Trace process tree"
         ),
         r_create_menu(
           id: 107,
           text: ~c"Trace named process tree"
         )
       ]}
    ]
  ...
  defp popup_menu(panel) do
    menu = :wxMenu.new()
    :wxMenu.append(menu, 101, ~c"Process info")
    :wxMenu.append(menu, 104, ~c"Trace process")
    :wxMenu.append(menu, 105, ~c"Trace named process")
    :wxMenu.append(menu, 106, ~c"Trace process tree")
    :wxMenu.append(menu, 107, ~c"Trace named process tree")
    :wxMenu.append(menu, 102, ~c"Send Msg")
    :wxMenu.append(menu, 103, ~c"Kill process")
  ...
  defp box(str0, n2P, fontW) do
    ...
    str =
      cond do
        hd(str0) === ?< ->
          :lists.append(:io_lib.format(~c"~w", [pid]))
