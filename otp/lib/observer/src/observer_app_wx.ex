defmodule :m_observer_app_wx do
  use Bitwise
  import :observer_perf_wx, only: [destroy_gc: 1,
                                     drawRoundedRectangle: 6, drawText: 4,
                                     getTextExtent: 2, haveGC: 0, make_gc: 2,
                                     setBrush: 2, setFont: 3, setPen: 2,
                                     strokeLine: 5, strokeLines: 2]
  @behaviour :wx_object
  require Record
  ...
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 panel: :undefined, apps_w: :undefined,
                                 app_w: :undefined, paint: :undefined,
                                 current: :undefined, app: :undefined,
                                 sel: :undefined, appmon: :undefined,
                                 usegc: false)
  Record.defrecord(:r_paint, :paint, font: :undefined,
                                 fg: :undefined, pen: :undefined,
                                 brush: :undefined, sel: :undefined,
                                 links: :undefined)
  Record.defrecord(:r_app, :app, ptree: :undefined,
                               n2p: :undefined, links: :undefined,
                               dim: :undefined)
  Record.defrecord(:r_box, :box, x: :undefined, y: :undefined,
                               w: :undefined, h: :undefined, s1: :undefined)
  Record.defrecord(:r_str, :str, x: :undefined, y: :undefined,
                               text: :undefined, pid: :undefined)
  def start_link(notebook, parent, config) do
    :wx_object.start_link(:observer_app_wx,
                            [notebook, parent, config], [])
  end

  def init([notebook, parent, _Config]) do
    panel = :wxPanel.new(notebook,
                           [{:size, :wxWindow.getClientSize(notebook)}, {:winid,
                                                                           1}])
    ...
    font = (case (:os.type()) do
              {:unix, _} when (useGC and version28) ->
                :wxFont.new(scale * 12, 71, 90,
                              :wxe_util.get_const(:wxFONTWEIGHT_NORMAL))
              _ ->
                font0 = :wxSystemSettings.getFont(14 + 3)
                :wxFont.setPointSize(font0,
                                       scale * :wxFont.getPointSize(font0))
                font0
            end)
    selCol = :wxSystemSettings.getColour(13)
    {fg, bGBrush,
       pen} = (case (:observer_lib.is_darkmode(bG)) do
                 false ->
                   {:wxSystemSettings.getColour(18),
                      :wxBrush.new(:wxSystemSettings.getColour(16)),
    ...
    {panel,
       r_state(parent: parent, panel: panel, apps_w: apps,
           app_w: drawingArea, usegc: useGC,
           paint: r_paint(font: font, fg: fg, pen: pen, brush: bGBrush,
                      sel: selBrush, links: linkPen))}
  end

  defp setup_scrollbar(appWin, app) do
    setup_scrollbar(:wxWindow.getClientSize(appWin), appWin,
                      app)
  end
  ...
  defp setup_scrollbar({cW, cH}, appWin, r_app(dim: {w0, h0})) do
    ...
    cond do
      ...
      h0 <= cH ->
        :wxScrolledWindow.setScrollbars(appWin, pPC, h,
                                          div(w, pPC) + 1, 0)
      w0 <= cW ->
        :wxScrolledWindow.setScrollbars(appWin, w, pPC, 0,
                                          div(h, pPC) + 1)
      true ->
        :wxScrolledWindow.setScrollbars(appWin, pPC, pPC,
                                          div(w, pPC) + 1, div(h, pPC) + 1)
    end
  end
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_listbox_selected,
                      cmdString: appStr)),
           state = r_state(appmon: appMon, current: prev)) do
    case (appStr) do
      [] ->
        {:noreply, state}
      _ ->
        app = :erlang.list_to_atom(appStr)
        prev !== :undefined and :appmon_info.app(appMon, prev,
                                                   false, [])
  ...
  def handle_event(r_wx(id: id, event: _Sz = r_wxSize(size: size)),
           state = r_state(app: app, app_w: appWin)) do
    id === 5 and setup_scrollbar(size, appWin, app)
    {:noreply, state}
  end

  def handle_event(r_wx(event: r_wxMouse(type: type, x: x0, y: y0)),
           s0 = r_state(app: app, app_w: appWin)) do
    case (app) do
      r_app(ptree: tree) ->
        {x,
           y} = :wxScrolledWindow.calcUnscrolledPosition(appWin,
                                                           x0, y0)
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_menu_selected)),
           state = r_state(panel: panel, sel: :undefined)) do
    :observer_lib.display_info_dialog(panel, 'Select process first')
    {:noreply, state}
  end

  def handle_event(r_wx(id: 101,
             event: r_wxCommand(type: :command_menu_selected)),
           state = r_state(sel: {r_box(s1: r_str(pid: pid)), _})) do
  ...
  def handle_event(r_wx(id: 102,
           ...
           state = r_state(panel: panel,
                       sel: {r_box(s1: r_str(pid: pid)), _})) do
  ...
  def handle_event(r_wx(id: 104,
             event: r_wxCommand(type: :command_menu_selected)),
           state = r_state(sel: {box, _})) do
    :observer_trace_wx.add_processes([box_to_pid(box)])
    {:noreply, state}
  end

  def handle_event(r_wx(id: 105,
             event: r_wxCommand(type: :command_menu_selected)),
           state = r_state(sel: {box, _})) do
  ...
  def handle_event(r_wx(id: 106,
             event: r_wxCommand(type: :command_menu_selected)),
           state = r_state(sel: sel)) do
    get = fn box ->
               box_to_pid(box)
          end
  ...
  def handle_sync_event(r_wx(event: r_wxPaint()), _,
           r_state(app_w: dA, app: app, sel: sel, paint: paint,
               usegc: useGC)) do
    ...
    case (useGC) do
      ...
      true ->
        {sx, sy} = :wxScrolledWindow.calcScrolledPosition(dA,
                                                            {0, 0})
  ...
  def handle_info({:active, node},
           state = r_state(parent: parent, current: curr,
                       appmon: appmon)) do
    create_menus(parent, [])
    pid = (try do
             ^node = node(appmon)
    ...
    curr !== :undefined and :appmon_info.app(pid, curr,
                                               true, [])
  ...
  def handle_info(:not_active, state = r_state(appmon: appMon)) do
    ...
    :lists.member(node(appMon),
                    :erlang.nodes()) and :erlang.exit(appMon, :normal)
    :observer_wx.set_status('')
    {:noreply, r_state(state, appmon: :undefined)}
  end

  def handle_info({:delivery, pid, :app_ctrl, _, apps0},
           state = r_state(appmon: pid, apps_w: lBox, current: curr0)) do
    apps = (for {_, app, {_, _, _}} <- apps0 do
              :erlang.atom_to_list(app)
            end)
    :wxListBox.clear(lBox)
    :wxListBox.appendStrings(lBox,
  ...
  def handle_info({:delivery, _Pid, :app, _Curr, {[], [], [], []}},
           state = r_state(panel: panel)) do
    :wxWindow.refresh(panel)
    {:noreply, r_state(state, app: :undefined,  sel: :undefined)}
  end

  def handle_info({:delivery, pid, :app, curr, appData},
           state = r_state(panel: panel, appmon: pid, current: curr,
                       usegc: useGC, app_w: appWin,
                       paint: r_paint(fg: fg, font: font))) do
    gC = (cond do
            useGC ->
  ...
  defp handle_mouse_click(node = {r_box(s1: r_str(pid: pid)), _}, type,
            state = r_state(app_w: appWin, panel: panel)) do
    case (type) do
      :left_dclick ->
        send(:observer, {:open_link, pid})
      :right_down ->
  ...
  defp create_menus(parent, _) do
    menuEntries = [{'File',
                      [r_create_menu(id: 101, text: 'Process info'), r_create_menu(id: 102, text: 'Send Msg'), r_create_menu(id: 103,
                                                                     text: 'Kill process')]},
                       {'Trace',
                          [r_create_menu(id: 104, text: 'Trace process'), r_create_menu(id: 105, text: 'Trace named process'), r_create_menu(id: 106,
                                                                         text: 'Trace process tree'),
                                                                         r_create_menu(id: 107,
  ...
  defp build_tree({root, p2Name, links, xLinks0}, fontW) do
    ...
    name2P = :gb_trees.from_orddict(:lists.sort(for {pid,
                                                       name} <- p2Name do
                                                  {name, pid}
                                                end))
    ...
    fetch = fn {from, to}, acc ->
                 try do
                   ...
                   [{locate_box(fromPid, [tree]),
                       locate_box(toPid, [tree])} |
                        acc]
                 catch
                   _, _ ->
                     acc
  ...
  defp build_tree2(root, tree0, n2P, fontW) do
    case (:gb_trees.lookup(root, tree0)) do
      ...
      {:value, children} ->
        ...
        {tree, cHs} = :lists.foldr(fn 'port ' ++ _, acc ->
                                        acc
                                      child, {t0, acc} ->
                                        {t, c} = build_tree2(child, t0, n2P,
                                                               fontW)
                                        {t, [c | acc]}
                                   end,
                                     {tree1, []}, children)
  ...
  defp calc_col_start({r_box(w: w), chs}, [max | acc0]) do
    ...
    depth = :lists.foldl(fn child, mDepth ->
  ...
  defp calc_tree_size({box, children}, [col | cols], x, [h0 | hs0]) do
    ...
    startX = x + col + 16
    {boxes, {w, hs}} = calc_tree_sizes(children, cols,
                                         startX, startX, hs1, [])
    y = middle(boxes, h0)
    h = y + r_box(box, :h) + 12
  ...
  defp calc_tree_sizes([child | chs], cols, x0, w0, hs0, acc) do
    ...
    calc_tree_sizes(chs, cols, x0, max(w, w0), hs,
                      [tree | acc])
  end
  ...
  defp box(str0, n2P, fontW) do
    ...
    data = r_str(text: str, x: div(10, 2), y: div(10, 2),
               pid: pid)
  ...
  defp draw(dC, r_app(dim: {_W, _H}, ptree: tree, links: links),
            sel,
            r_paint(font: font, fg: fg, pen: pen, brush: brush,
                links: lPen, sel: selBrush)) do
    setPen(dC, lPen)
    for link <- links do
      draw_xlink(link, dC)
    end
    ...
    case (sel) do
      ...
      {r_box(x: x, y: y, w: w, h: h, s1: str1), _} ->
        ...
        drawRoundedRectangle(dC, x - 1, y - 1, w + 2, h + 2,
                               8.0)
        draw_str(dC, str1, x, y)
    end
  end

  defp draw_tree({box = r_box(x: x, y: y, w: w, h: h, s1: str1),
             chs},
            parent, dC) do
  ...
  defp draw_xlink({r_box(x: x0, y: y0, h: bH, w: bW),
             r_box(x: x1, y: y1)},
            dC)
      when x0 < x1 do
  ...
  defp draw_xlink({r_box(x: x0, y: y0, h: bH),
             r_box(x: x1, w: bW, y: y1)},
            dC)
      when x0 > x1 do
