defmodule :m_etop_txt do
  ...
  Record.defrecord(:r_field_widths, :field_widths,
    cols: :undefined,
    used_cols: :undefined,
    init_func: :undefined,
    reds: :undefined,
  ...
  def do_update(fd, info, prev, config) do
    ...
    case r_etop_info(info, :memi) do
      :undefined ->
        :io.fwrite(fd, ~c" ~-72w~10s~n Load:  cpu  ~8w~n        procs~8w~n        runq ~8w~n", [
          r_opts(config, :node),
          clock,
          cpu,
          nProcs,
      ...
      memi ->
        ...
        :io.fwrite(
          fd,
          ~c" ~-72w~10s~n Load:  cpu  ~8w               Memory:  total    ~8w    binary   ~8w~n        procs~8w                        processes~8w    code     ~8w~n        runq ~8w                        atom     ~8w    ets      ~8w~n",
    ...
    writepinfo(fd, r_etop_info(info, :procinfo), modifier(fd), fieldWidths)
    writedoubleline(fd, fieldWidths)
    :io.nl(fd)
    info
  end
  ...
  defp calc_field_widths(procInfoL) do
    ...
    usedCols =
      15 + initFuncWidth + 8 + redsWidth + 1 + memWidth + 1 + msgQWidth + currFuncWidth + 1
  ...
  defp writedupline(fd, char, r_field_widths(used_cols: usedCols)) do
    line = :lists.duplicate(usedCols, char) ++ ~c"\n"
    :io.fwrite(fd, line, [])
  end
  ...
  defp proc_format(
    ...
    ~c"~-15w~-" ++
      ...
      i2l(reds) ++
      ~c"w ~" ++
      i2l(mem) ++ ~c"w ~" ++ i2l(msgQ) ++ ~c"w ~-" ++ i2l(currFunc) ++ modifier ++ ~c"s~n"
  end

  defp to_string({m, f, a}, modifier) do
    :io_lib.format(~c"~w:~" ++ modifier ++ ~c"w/~w", [m, f, a])
  end

  defp to_string(other, modifier) do
    :io_lib.format(~c"~" ++ modifier ++ ~c"w", [other])
  end
