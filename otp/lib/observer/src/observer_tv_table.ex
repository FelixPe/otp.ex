defmodule :m_observer_tv_table do
  ...
  Record.defrecord(:r_wxHtmlLinkInfo, :wxHtmlLinkInfo, href: :undefined,
                                          target: :undefined)
  Record.defrecord(:r_tab, :tab, name: :undefined,
                               id: :ignore, size: :undefined, memory: 0,
                               owner: :undefined, reg_name: :undefined,
                               protection: :public, type: :set, keypos: 1,
                               heir: :none, compressed: false, fixed: false,
                               storage: :undefined, index: :undefined)
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 frame: :undefined, grid: :undefined,
                                 status: :undefined, sizer: :undefined,
                                 search: :undefined, selected: :undefined,
                                 node: node(), columns: :undefined,
                                 pid: :undefined, source: :undefined,
                                 tab: :undefined, attrs: :undefined,
                                 timer: {false, 30})
  Record.defrecord(:r_opt, :opt, sort_key: 2, sort_incr: true)
  Record.defrecord(:r_search, :search, enable: true,
                                  win: :undefined, name: :undefined,
                                  search: :undefined, goto: :undefined,
                                  radio: :undefined, find: :undefined)
  Record.defrecord(:r_find, :find, start: :undefined,
                                strlen: :undefined, found: :undefined)
  def start_link(parent, opts) do
    :wx_object.start_link(:observer_tv_table,
                            [parent, opts], [])
  end
  ...
  def init([parent, opts]) do
    ...
    try do
      ...
      :wxSizer.add(sizer, r_search(search, :win),
                     [{:flag, 8192 ||| 16 ||| 32 ||| 2}, {:border, 5}])
      :wxWindow.setSizer(panel, sizer)
      :wxSizer.hide(sizer, r_search(search, :win))
      cols = add_columns(grid, 0, columnNames)
      :wxFrame.show(frame)
      {panel,
         r_state(frame: frame, grid: grid, status: statusBar,
             search: search, sizer: sizer, parent: parent,
             columns: cols, pid: holder, source: source,
             tab: r_tab(table, keypos: keyPos), attrs: attrs)}
    catch
      :node_or_table_down ->
  ...
  defp search_area(parent) do
    ...
    :wxSizer.add(hSz, :wxStaticText.new(parent, - 1, 'Find:'),
                   [{:flag, 2048}])
    ...
    :wxSizer.add(hSz, 15, 15,
                   [{:proportion, 1}, {:flag, 8192}])
    :wxSizer.add(hSz, :wxStaticText.new(parent, - 1, 'Goto Entry:'),
                   [{:flag, 2048}])
    ...
    r_search(name: :"Search Area", win: hSz, search: tC1, goto: tC2,
        radio: {nbtn, pbtn, cbtn})
  end

  defp edit(index, r_state(pid: pid, frame: frame)) do
    str = get_row(pid, index, :all_multiline)
    case (:observer_lib.user_term_multiline(frame, 'Edit object:',
                                              str)) do
      :cancel ->
        :ok
      {:ok, term} ->
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_col_click,
                      col: col)),
           state = r_state(pid: pid, grid: grid, selected: oldSel)) do
    selObj = (case (oldSel) do
                :undefined ->
                  :undefined
                _ ->
    ...
    send(pid, {:sort, col + 1})
    case (selObj !== :undefined and search(pid, selObj, - 1,
                                             true, :term)) do
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_selected,
                      itemIndex: index)),
           state = r_state(pid: pid, grid: grid, status: statusBar)) do
  ...
  def handle_event(r_wx(id: 404),
           state = r_state(grid: grid, pid: pid, status: statusBar,
                       selected: index)) do
    str = get_row(pid, index, :all)
  ...
  def handle_event(r_wx(id: 421, event: r_wxCommand(cmdString: str)),
           state = r_state(grid: grid)) do
    try do
  ...
  def handle_event(r_wx(id: 405),
           state = r_state(grid: grid, sizer: sz, search: search,
                       selected: index)) do
    is_integer(index) and :wxListCtrl.setItemState(grid,
                                                     index, 0, 2)
  ...
  def handle_event(r_wx(id: 420, event: r_wxFocus()),
           state = r_state(search: search, pid: pid)) do
    send(pid, {:mark_search_hit, false})
    {:noreply,
       r_state(state, search: r_search(search, find: :undefined))}
  end

  def handle_event(r_wx(id: 420, event: r_wxCommand(cmdString: '')),
           state = r_state(search: search, pid: pid)) do
    send(pid, {:mark_search_hit, false})
    {:noreply,
       r_state(state, search: r_search(search, find: :undefined))}
  ...
  def handle_event(r_wx(id: 420,
             event: r_wxCommand(type: :command_text_enter, cmdString: str)),
           state = r_state(grid: grid, pid: pid, status: sB,
                       search: search = r_search(radio: {next0, _, case0},
                                            find: find)))
      when find !== :undefined do
    dir = :erlang.xor(:wxRadioButton.getValue(next0),
    ...
    case (search(pid, str, pos, dir, case__)) do
      false ->
        ...
        {:noreply,
           r_state(state, search: r_search(search, find: r_find(find, found: false)))}
      row ->
        :wxListCtrl.ensureVisible(grid, row)
        ...
        {:noreply,
           r_state(state, search: r_search(search, find: r_find(start: row,
                                                found: true)))}
    end
  end

  def handle_event(r_wx(id: 420, event: r_wxCommand(cmdString: str)),
           state = r_state(grid: grid, pid: pid, status: sB,
                       search: search = r_search(radio: {next0, _, case0},
                                            find: find))) do
    try do
  ...
  def handle_event(r_wx(id: 400),
           state = r_state(frame: frame, node: node, source: source,
                       tab: table)) do
    :observer_tv_wx.display_table_info(frame, node, source,
                                         table)
    {:noreply, state}
  end

  def handle_event(r_wx(id: 402),
  ...
  def handle_info({:no_rows, n},
           state = r_state(grid: grid, status: statusBar)) do
  ...
  def handle_info({:new_cols, new},
           state = r_state(grid: grid, columns: cols0)) do
  ...
  def terminate(_Event, r_state(pid: pid, attrs: attrs)) do
    r_attrs(odd: odd, even: even, deleted: d, searched: s,
        changed_odd: ch1, changed_even: ch2, new_odd: new1,
        new_even: new2) = attrs
  ...
  Record.defrecord(:r_holder, :holder, node: :undefined,
                                  parent: :undefined, pid: :undefined,
                                  table: :array.new(), n: 0,
                                  columns: :undefined, temp: [],
                                  search: :undefined, source: :undefined,
                                  tabid: :undefined, sort: :undefined,
                                  key: :undefined, type: :undefined,
                                  attrs: :undefined)
  defp init_table_holder(parent, table, mnesiaOrEts, cols, node,
            attrs) do
    tabId = (case (r_tab(table, :id)) do
               :ignore ->
                 r_tab(table, :name)
               id ->
                 id
    ...
    send(self(), :refresh)
    table_holder(r_holder(node: node, parent: parent,
                     source: mnesiaOrEts, tabid: tabId, columns: cols,
                     sort: r_opt(sort_key: r_tab(table, :keypos), sort_incr: true),
                     type: r_tab(table, :type), key: r_tab(table, :keypos),
                     attrs: attrs))
  end

  defp table_holder(s0 = r_holder(parent: parent, pid: pid,
                   table: table)) do
    receive do
      {:get_attr, from, row} ->
      ...
      :refresh when is_pid(pid) ->
        table_holder(s0)
      :refresh ->
        getTab = :rpc.call(r_holder(s0, :node), :observer_backend,
                             :get_table,
                             [self(), r_holder(s0, :tabid), r_holder(s0, :source)])
  ...
  defp handle_new_data_chunk(data, s0 = r_holder(columns: cols, parent: parent)) do
    s1 = (r_holder(n: n,
              columns: newCols) = handle_new_data_chunk2(data, s0))
  ...
  defp handle_new_data_chunk2(:"$end_of_table",
            s0 = r_holder(sort: opt0, key: key, table: old, temp: new)) do
    merged = merge(:array.to_list(old), new, key)
    {opt, sorted} = sort(r_opt(opt0, :sort_key),
                           r_opt(opt0, sort_key: :undefined), merged)
    sortedA = :array.from_list(sorted)
    r_holder(s0, sort: opt,  table: sortedA, 
            n: :array.size(sortedA),  temp: [],  pid: :undefined)
  end

  defp handle_new_data_chunk2(data,
            s0 = r_holder(columns: cols0, source: :ets, temp: tab0)) do
  ...
  defp sort(col, opt = r_opt(sort_key: col, sort_incr: bool),
            table) do
  ...
  defp sort(col, s = r_opt(sort_incr: false), table) do
    {r_opt(s, sort_key: col),
       :lists.reverse(keysort(col, table))}
  end
  ...
  defp search([term, - 1, true, :term],
            s = r_holder(parent: parent, table: table)) do
  ...
  defp search([str, row, dir0, caseSens],
            s = r_holder(parent: parent, n: n, table: table)) do
    opt = (case (caseSens) do
             true ->
               []
             false ->
               [:caseless]
    ...
    res = (case (:re.compile(str, [:unicode | opt])) do
             {:ok, re} ->
  ...
  defp re_search(row, dir, n, re, table) when (row >= 0 and
                                          row < n) do
  ...
  defp get_row(from, row, col, table) do
    case (:array.get(row, table)) do
      ...
      [object | _] when tuple_size(object) >= col ->
        send(from, {self(),
                      format(:erlang.element(col, object))})
      _ ->
        send(from, {self(), ''})
    end
  end
  ...
  defp get_attr(from, row, r_holder(table: table, attrs: attrs)) do
    odd = rem(row, 2) > 0
    what = (case (:array.get(row, table)) do
              [_ | :deleted] ->
  ...
  defp merge2([[a | op] | old], [b | new], key)
      when :erlang.element(key, a) == :erlang.element(key,
                                                        b) do
    case (op) do
      :deleted ->
  ...
  defp delete(row,
            r_holder(tabid: id, table: table, source: source,
                node: node)) do
    [object | _] = :array.get(row, table)
    try do
      case (source) do
        :ets ->
          true = :rpc.call(node, :ets, :delete_object,
                             [id, object])
        :mnesia ->
          :ok = :rpc.call(node, :mnesia, :dirty_delete_object,
                            [id, object])
      end
      :ok
    catch
  ...
  defp insert(object,
            r_holder(tabid: id, source: source, node: node)) do
    try do
      case (source) do
        :ets ->
          true = :rpc.call(node, :ets, :insert, [id, object])
        :mnesia ->
          :ok = :rpc.call(node, :mnesia, :dirty_write,
                            [id, object])
      end
      :ok
    catch
  ...
  defp column_names(node, type, table) do
    case (type) do
      :ets ->
        [1, 2]
      :mnesia ->
        attrs = :rpc.call(node, :mnesia, :table_info,
                            [table, :attributes])
  ...
  def format(bin) when (is_binary(bin) and
