defmodule :m_observer_tv_table do
  ...
  def init([parent, opts]) do
    ...
    title0 = :erlang.atom_to_list(r_tab(table, :name)) ++ ~c" @ " ++ :erlang.atom_to_list(node)
  ...
  defp create_menus(mB) do
    file = :wxMenu.new()
    :wxMenu.append(file, 400, ~c"Table Information\tCtrl-I")
    :wxMenu.append(file, 5001, ~c"Close")
    :wxMenuBar.append(mB, file, ~c"File")
    edit = :wxMenu.new()
    :wxMenu.append(edit, 403, ~c"Edit Object")
    :wxMenu.append(edit, 404, ~c"Delete Object\tCtrl-D")
    :wxMenu.appendSeparator(edit)
    :wxMenu.append(edit, 405, ~c"Search\tCtrl-S")
    :wxMenu.appendSeparator(edit)
    :wxMenu.append(edit, 401, ~c"Refresh\tCtrl-R")
    :wxMenu.append(edit, 402, ~c"Refresh interval...")
    :wxMenuBar.append(mB, edit, ~c"Edit")
    help = :wxMenu.new()
    :wxMenu.append(help, 5009, ~c"Help")
    :wxMenuBar.append(mB, help, ~c"Help")
    :ok
  end

  defp search_area(parent) do
    hSz = :wxBoxSizer.new(4)
    :wxSizer.add(hSz, :wxStaticText.new(parent, -1, ~c"Find:"), [{:flag, 2048}])
    ...
    :wxSizer.add(hSz, :wxStaticText.new(parent, -1, ~c"Goto Entry:"), [{:flag, 2048}])
  ...
  defp edit(index, r_state(pid: pid, frame: frame)) do
    ...
    case :observer_lib.user_term_multiline(frame, ~c"Edit object:", str) do
      :cancel ->
        :ok
  ...
  def handle_event(
    ...
    :wxStatusBar.setStatusText(
      statusBar,
      :io_lib.format(~c"Objects: ~w: ~ts", [n, str])
    )
  ...
  def handle_event(
    ...
    :wxStatusBar.setStatusText(
      statusBar,
      :io_lib.format(~c"Deleted object: ~ts", [str])
    )
  ...
  def handle_event(
        r_wx(id: 420, event: r_wxCommand(cmdString: ~c"")),
        state = r_state(search: search, pid: pid)
      ) do
    send(pid, {:mark_search_hit, false})
    {:noreply, r_state(state, search: r_search(search, find: :undefined))}
  ...
  def handle_event(
    ...
    case search(pid, str, pos, dir, case__) do
      false ->
        :wxStatusBar.setStatusText(sB, :io_lib.format(~c"Not found (regexp): ~ts", [str]))
      ...
      row ->
        ...
        status = ~c"Found: (Hit Enter for next, Shift-Enter for previous)"
  ...
  def handle_info({:error, error}, state = r_state(frame: frame)) do
    errorStr =
      try do
        :io_lib.format(~c"~ts", [error])
        error
      catch
        _, _ ->
          :io_lib.format(~c"~tp", [error])
      end
  ...
  defp table_holder(s0 = r_holder(parent: parent, pid: pid, table: table)) do
    receive do
      ...
      what ->
        :io.format(~c"Table holder got ~tp~n", [what])
        send(parent, {:refresh, 0, r_holder(s0, :n) - 1})
        table_holder(s0)
    end
  end
  ...
  defp get_row(from, row, col, table) do
    case :array.get(row, table) do
      ...
      [object | _] when col === :all_multiline ->
        send(from, {self(), :io_lib.format(~c"~tp", [object])})
  ...
  defp delete_row(row, s0 = r_holder(parent: parent)) do
    case delete(row, s0) do
      ...
      {:error, err} ->
        send(parent, {:error, ~c"Could not delete object: " ++ err})
    end
  end
  ...
  defp delete(
    ...
    try do
      ...
      _, _Error ->
        {:error, ~c"node or table is not available"}
    end
  end
  ...
  defp edit_row(row, term, s0 = r_holder(parent: parent)) do
    case delete(row, s0) do
      ...
      {:error, err} ->
        send(parent, {:error, ~c"Could not edit object: " ++ err})
    end
  end
  ...
  defp format_list(list) do
    case :io_lib.printable_list(list) do
      true ->
        :io_lib.format(~c"\"~ts\"", [map_printable_list(list)])
