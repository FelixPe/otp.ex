defmodule :m_cdv_virtual_list_wx do
  ...
  Record.defrecord(:r_sort, :sort, sort_key: :undefined,
                                sort_incr: true)
  Record.defrecord(:r_holder, :holder, parent: :undefined,
                                  info: :undefined, last_row: :undefined,
                                  sort: :undefined, attrs: :undefined,
                                  callback: :undefined)
  Record.defrecord(:r_state, :state, grid: :undefined,
                                 panel: :undefined, detail_wins: [],
                                 holder: :undefined, callback: :undefined,
                                 trunc_warn: [], menu_cols: [], menu_items: [])
  def start_link(parentWin, callback) do
    :wx_object.start_link({:local, callback},
                            :cdv_virtual_list_wx, [parentWin, callback, :all],
                            [])
  end

  def start_link(parentWin, callback, owner) do
    :wx_object.start_link(:cdv_virtual_list_wx,
                            [parentWin, callback, owner], [])
  end
  ...
  def init([parentWin, callback, owner]) do
    ...
    {holder, tW} = spawn_table_holder(callback, owner,
                                        attrs)
    {grid, menuCols} = create_list_box(panel, holder,
                                         callback, owner)
    ...
    state = r_state(grid: grid, panel: panel, holder: holder,
                callback: callback, trunc_warn: tW, menu_cols: menuCols)
    {panel, state}
  end

  defp create_list_box(panel, holder, callback, owner) do
    style = 8192 ||| 32 ||| 512 ||| 2 ||| 1073741824 ||| -
                                                         2147483648
    ...
    addListEntry = fn {name, align, defSize}, col ->
                        ...
                        :wxListCtrl.setColumnWidth(listCtrl, col,
                                                     defSize * scale)
                        col + 1
                   end
  ...
  defp do_start_detail_win(id,
            r_state(panel: panel, detail_wins: opened, holder: holder,
                callback: callback) = state) do
    newOpened = (case (:lists.keyfind(id, 1, opened)) do
                   false ->
                     data = call(holder, {:get_data, self(), id})
                     case (:cdv_detail_wx.start_link(id, data, panel,
                                                       callback, :cdv)) do
                       {:error, _} ->
                         opened
                       iW ->
  ...
  def handle_call(:new_dump, _From,
           r_state(grid: grid, detail_wins: opened, holder: holder,
               callback: callback) = state) do
    :lists.foreach(fn {_Id, iW} ->
                        :wxFrame.destroy(iW)
                   end,
                     opened)
    ...
    {newHolder, tW} = spawn_table_holder(callback, :all,
                                           attrs)
    {:reply, :ok,
       r_state(state, detail_wins: [],  holder: newHolder, 
                  trunc_warn: tW)}
  end
  ...
  def handle_cast({:detail_win_closed, id},
           r_state(detail_wins: opened) = state) do
  ...
  def handle_event(r_wx(id: menuId,
             event: r_wxCommand(type: :command_menu_selected)),
           r_state(menu_items: menuItems) = state) do
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_right_click,
                      itemIndex: row)),
           r_state(panel: panel, holder: holder,
               menu_cols: menuCols) = state) do
    menu = :wxMenu.new()
    menuItems = :lists.flatmap(fn {type, col} ->
                                    menuId = 202 + col
                                    colText = call(holder,
                                                     {:get_row, self(), row,
                                                        col})
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_activated,
                      itemIndex: row)),
           r_state(holder: holder, menu_cols: menuCols) = state) do
    case (menuCols) do
      [{type, _} | _] ->
  ...
  defp init_table_holder(parent, attrs, callback, infoList0) do
    ...
    table_holder(r_holder(parent: parent, info: info, sort: sort,
                     attrs: attrs, callback: callback))
  end

  defp table_holder(r_holder(callback: callback, attrs: attrs,
              info: info) = s0) do
    receive do
  ...
  defp search_id(callback, rowInfo, id) do
    case (:observer_lib.to_str(get_cell_data(callback, :id,
                                               rowInfo))) do
      ^id ->
        throw(rowInfo)
      _Str ->
  ...
  defp change_sort(col,
            s0 = r_holder(parent: parent, info: info0, sort: sort0)) do
  ...
  defp do_sort(sort = r_sort(sort_key: col, sort_incr: true),
            table) do
    {sort, :lists.keysort(col, table)}
  end

  defp do_sort(sort = r_sort(sort_key: col, sort_incr: false),
            table) do
    {sort, :lists.reverse(:lists.keysort(col, table))}
  end

  defp get_cell_data(callback, colNo, rowInfo) do
    case (:erlang.element(callback.col_to_elem(colNo),
                            rowInfo)) do
      :undefined ->
        ''
      cell ->
  ...
  defp get_row(from, row, col,
            r_holder(callback: callback,
                last_row: {row, rowInfo}) = state) do
  ...
  defp get_row(from, row, col,
            r_holder(callback: callback, info: info) = s0) do
    {data, state} = (case (row >= :array.size(info)) do
                       true ->
                         {'', s0}
                       false ->
