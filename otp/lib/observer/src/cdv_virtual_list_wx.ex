defmodule :m_cdv_virtual_list_wx do
  ...
  def start_detail_win(id) do
    case id do
      ...
      _ ->
        :io.format(~c"cdv: unknown identifier: ~tp~n", [id])
        :ignore
    end
  end
  ...
  defp call(holder, what) when is_pid(holder) do
    ...
    after
      5000 ->
        :io.format(~c"Hanging call ~tp~n", [what])
        ~c""
    end
  end
  ...
  def handle_info(info, state) do
    :io.format(~c"~p:~p, Unexpected info: ~tp~n", [:cdv_virtual_list_wx, 219, info])
    {:noreply, state}
  end
  ...
  def handle_call(msg, _From, state) do
    :io.format(~c"~p:~p: Unhandled call ~tp~n", [:cdv_virtual_list_wx, 242, msg])
    {:reply, :ok, state}
  end
  ...
  def handle_cast(msg, state) do
    :io.format(~c"~p:~p: Unhandled cast ~tp~n", [:cdv_virtual_list_wx, 254, msg])
    {:noreply, state}
  end
  ...
  defp spawn_table_holder(callback, owner, attrs) do
    ...
    holder =
      case owner do
        :all ->
          name = :erlang.list_to_atom(:erlang.atom_to_list(callback) ++ ~c"__holder")
