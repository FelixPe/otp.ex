defmodule :m_observer_sock_wx do
  ...
  Record.defrecord(:r_socket, :socket, id: :undefined,
                                  id_str: :undefined, kind: :undefined,
                                  fd: :undefined, owner: :undefined,
                                  domain: :undefined, type: :undefined,
                                  protocol: :undefined, raddress: :undefined,
                                  laddress: :undefined, rstate: :undefined,
                                  wstate: :undefined, monitored_by: :undefined,
                                  statistics: :undefined, options: :undefined)
  ...
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 ...
                                 sizer: :undefined, fields: :undefined,
                                 node: {node(), true},
                                 opt: :EFE_TODO_NESTED_RECORD,
                                 right_clicked_socket: :undefined,
                                 sockets: :undefined, timer: :undefined,
                                 open_wins: [])
  def start_link(notebook, parent, config) do
    :wx_object.start_link(:observer_sock_wx,
                            [notebook, parent, config], [])
  end

  defp info_fields() do
    gen = [{'General socket info',
              [{'IOV Max', :iov_max}, {'Counter Size (in bits)', :num_cnt_bits}, {'Number of sockets', :num_sockets},
                                                      {'Number of (socket) monitors', :num_monitors}, {'Number of sockets in the \'inet\' domain',
                                                                             :num_dinet},
                                                                              {'Number of sockets in the \'inet6\' domain',
                                                                                 :num_dinet6},
                                                                                  {'Number of sockets in the \'local\' domain',
                                                                                     :num_dlocal},
                                                                                      {'Number of type \'stream\' sockets',
                                                                                         :num_tstreams},
                                                                                          {'Number of type \'dgram\' sockets',
                                                                                             :num_tdgrams},
                                                                                              {'Number of type \'seqpacket\' sockets',
                                                                                                 :num_tseqpkgs},
                                                                                                  {'Number of protocol \'ip\' sockets',
                                                                                                     :num_pip},
                                                                                                      {'Number of protocol \'sctp\' sockets',
                                                                                                         :num_psctp},
                                                                                                          {'Number of protocol \'tcp\' sockets',
                                                                                                             :num_ptcp},
                                                                                                              {'Number of protocol \'udp\' sockets',
                                                                                                                 :num_pudp}]}]
    gen
  end

  defp update_gen_socket_info(r_state(node: {node, true}, fields: fields,
              sizer: sizer)) do
    case (:rpc.call(node, :observer_backend, :socket_info,
                      [])) do
      info when is_list(info) ->
        gen = info_fields()
        :observer_lib.update_info(fields,
                                    :observer_lib.fill_info(gen, info, 'Not Supported'))
  ...
  defp do_init(notebook, parent, config, info) do
    ...
    {genPanel, _GenSizer,
       genFields} = :observer_lib.display_info(panel,
                                                 :observer_lib.fill_info(gen,
                                                                           info,
                                                                           'Not Supported'))
    ...
    :wxSizer.add(sizer, genSizer,
                   [{:flag, 8192 ||| borderFlags ||| 64}, {:proportion, 0},
                                                              {:border, 5}])
    style = 32 ||| 2
    grid = :wxListCtrl.new(panel,
    ...
    listItems = [{'Id', 0, scale * 350}, {'Owner', 0, scale * 100},
                                          {'Fd', 0, scale * 50}, {'Domain', 0,
                                                                 scale * 60},
                                                                  {'Type', 0,
                                                                     scale * 100},
                                                                      {'Protocol', 0,
    ...
    odd = :observer_lib.mix(even,
                              :wxSystemSettings.getColour(13), 0.8)
    opt = r_opt(odd_bg: odd)
    {panel,
       r_state(parent: parent, panel: panel, sizer: sizer,
           fields: genFields, grid: grid, timer: config, opt: opt)}
  end

  def handle_event(r_wx(id: 301),
           state = (r_state(node: node, grid: grid,
                        opt: opt) = state)) do
  ...
  def handle_event(r_wx(obj: obj, event: r_wxClose()),
           r_state(open_wins: opened) = state) do
    newOpened = (case (:lists.keytake(obj, 2, opened)) do
                   false ->
                     opened
                   {:value, _, rest} ->
                     rest
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_col_click,
                      col: col)),
           state = r_state(node: node, grid: grid,
                       opt: opt0 = r_opt(sort_key: key, sort_incr: bool))) do
    opt = (case (col + 2) do
             ^key ->
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_activated,
                      itemIndex: index)),
           state = r_state(grid: grid, sockets: sockets,
                       open_wins: opened)) do
    cond do
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_right_click,
                      itemIndex: index)),
           state = r_state(panel: panel, sockets: sockets)) do
    case (index) do
      -1 ->
        {:noreply, state}
      _ ->
        socket = :lists.nth(index + 1, sockets)
        menu = :wxMenu.new()
        :wxMenu.append(menu, 303, f('Socket info for ~s', [r_socket(socket, :id_str)]))
        :wxMenu.append(menu, 309, f('Close ~p', [r_socket(socket, :id_str)]))
  ...
  def handle_event(r_wx(id: 303),
           state = r_state(grid: grid, right_clicked_socket: socket,
                       open_wins: opened)) do
    case (socket) do
      :undefined ->
        {:noreply, state}
      _ ->
        newOpened = display_socket_info(grid, socket, opened)
        {:noreply,
           r_state(state, right_clicked_socket: :undefined, 
                      open_wins: newOpened)}
    end
  end

  def handle_event(r_wx(id: 304),
           state = r_state(grid: grid, sockets: sockets,
                       open_wins: opened)) do
    case (get_selected_items(grid, sockets)) do
      [] ->
        :observer_wx.create_txt_dialog(r_state(state, :panel), 'No selected sockets', 'Socket Info',
                                         256)
        {:noreply, state}
      selected ->
        newOpened = :lists.foldl(fn s, o ->
  ...
  def handle_event(r_wx(id: 309),
           state = r_state(right_clicked_socket: socket)) do
    case (socket) do
      :undefined ->
        {:noreply, state}
      _ ->
  ...
  def handle_event(r_wx(id: 302),
           state = r_state(grid: grid, timer: timer0)) do
    timer = :observer_lib.interval_dialog(grid, timer0, 10,
                                            5 * 60)
    {:noreply, r_state(state, timer: timer)}
  end

  def handle_event(r_wx(obj: moreEntry, event: r_wxMouse(type: :left_down),
             userData: {:more, more}),
           state) do
  ...
  def handle_event(r_wx(event: r_wxMouse(type: :left_down),
             userData: targetPid),
           state) do
  ...
  def handle_event(r_wx(obj: obj, event: r_wxMouse(type: :enter_window)),
           state) do
    :wxTextCtrl.setForegroundColour(obj, {0, 0, 100, 255})
    {:noreply, state}
  end

  def handle_event(r_wx(obj: obj, event: r_wxMouse(type: :leave_window)),
           state) do
    :wxTextCtrl.setForegroundColour(obj,
  ...
  def handle_info(:refresh_interval,
           state = (r_state(node: node, grid: grid, opt: opt,
                        sockets: oldSockets) = state)) do
  ...
  def handle_info({:active, nodeName},
           r_state(parent: parent, grid: grid, opt: opt,
               timer: timer0) = state0) do
  ...
  def handle_info({:info, {:socket_info_not_available, nodeName}},
           state = r_state(panel: panel)) do
    str = :io_lib.format('Can not fetch socket info from ~p.~nToo old OTP version.', [nodeName])
  ...
  defp create_menus(parent) do
    menuEntries = [{'View',
                      [r_create_menu(id: 304, text: 'Socket info for selected sockets\tCtrl-I'), :separator, r_create_menu(id: 301, text: 'Refresh\tCtrl-R'),
                                                            r_create_menu(id: 302,
  ...
  defp get_sockets(nodeName) when is_atom(nodeName) do
    case (:rpc.call(nodeName, :observer_backend,
                      :get_socket_list, [])) do
      ...
      {:badrpc,
         {:EXIT,
            {:undef,
               [{:observer_backend, :get_socket_list, [], []}]}}} ->
  ...
  defp infomap_to_rec(%{id: id, id_str: idStr, kind: kind, fd: fD,
              owner: owner, domain: domain, type: type,
              protocol: protocol, rstates: rState, wstates: wState,
              monitored_by: monitoredBy, statistics: statistics,
              options: options} = info) do
    r_socket(id: id, id_str: idStr, kind: kind, fd: fD,
        owner: owner, domain: domain, type: type,
        protocol: protocol,
        raddress: :maps.get(:raddress, info, :undefined),
        laddress: :maps.get(:laddress, info, :undefined),
        rstate: rState, wstate: wState,
        monitored_by: monitoredBy, statistics: statistics,
        options: options)
  end

  defp socketrec_to_list(r_socket(id: id, id_str: idStr, kind: kind, fd: fD,
              owner: owner, domain: domain, type: type,
              protocol: protocol, raddress: rAddr, laddress: lAddr,
              rstate: rState, wstate: wState,
              monitored_by: monitoredBy, statistics: statistics,
              options: options)) do
    [{:id, id}, {:id_str, idStr}, {:kind, kind}, {:fd, fD},
                                                     {:owner, owner}, {:domain,
                                                                         domain},
                                                                          {:type,
                                                                             type},
  ...
  defp do_display_socket_info(parent0, r_socket(id_str: idStr) = socketRec) do
    ...
    scale = :observer_wx.get_scale()
    frame = :wxMiniFrame.new(parent, - 1, title,
                               [{:style, 2048 ||| 536870912 ||| 4096 ||| 64},
                                    {:size, {scale * 600, scale * 400}}])
    scrolledWin = :wxScrolledWindow.new(frame,
                                          [{:style,
                                              1073741824 ||| - 2147483648}])
    :wxScrolledWindow.enableScrolling(scrolledWin, true,
                                        true)
    :wxScrolledWindow.setScrollbars(scrolledWin, 20, 20, 0,
                                      0)
    sizer = :wxBoxSizer.new(8)
    :wxWindow.setSizer(scrolledWin, sizer)
    socket = socketrec_to_list(socketRec)
    fields0 = socket_info_fields(socket)
    _UpFields = :observer_lib.display_info(scrolledWin,
                                             sizer, fields0)
  ...
  defp socket_info_fields(socket0) do
    ...
    struct = [{'Overview',
                 [{'Owner', {:click, :owner}}, {'Fd', :fd}, {'Domain', :domain}, {'Type',
                                                                    :type},
                                                                     {'Protocol',
                                                                        :protocol},
                                                                         {'Read State',
                                                                            :rstate},
                                                                             {'Write State',
                                                                                :wstate}]},
                  {:scroll_boxes, [{'Monitored by', 1, {:click, :monitored_by}}]} |
                      struct0]
  ...
  defp extra_fields(socket) do
    ...
    struct = [{'Net', [{'Local Address', :laddress}, {'Remote Address', :raddress}]}, {'Statistics',
                                                        for {key,
                                                               _} <- statistics do
  ...
  defp update_grid2(grid, sel,
            r_opt(sort_key: sort, sort_incr: dir, odd_bg: bG), ports) do
    :wxListCtrl.deleteAllItems(grid)
    update = fn r_socket(id: id, id_str: idStr, owner: owner,
                    fd: fd, domain: domain, type: type, protocol: proto,
                    rstate: rState, wstate: wState),
                  row ->
  ...
  defp socketinfo_available(nodeName) do
    _ = :rpc.call(nodeName, :code, :ensure_loaded,
                    [:observer_backend])
    case (:rpc.call(nodeName, :erlang, :function_exported,
                      [:observer_backend, :get_socket_list, 0])) do
      true ->
        true
      false ->
        false
  ...
  defp popup_unavailable_info(nodeName) do
    send(self(), {:info,
                    {:socket_info_not_available, nodeName}})
    :ok
  end
