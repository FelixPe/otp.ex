  ...
  def get_details(id, _) do
    case :crashdump_viewer.node_info(id) do
      {:ok, info, tW} ->
        ...
        title = :io_lib.format(~c"~s (~s)", [r_nod(info, :name), id])
        {:ok, {title, proplist, tW}}

      {:error, :not_found} ->
        info = ~c"The node you are searching for could not be found."
        {:info, info}
    end
  end
    ...
    [
      {~c"Overview",
       [
         ...
         {~c"Type", :conn_type},
         {~c"Channel", :channel},
         {~c"Controller", {:click, :controller}},
         {~c"Creation", {{:format, &format/1}, :creation}},
         {~c"Extra Info", :error}
       ]},
      {:scroll_boxes,
       [
         {~c"Remote Links", 1, {:click, :remote_links}},
         {~c"Remote Monitors", 1, {:click, :remote_mon}},
         {~c"Remote Monitored By", 1, {:click, :remote_mon_by}}
       ]}
    ]
  end
end
