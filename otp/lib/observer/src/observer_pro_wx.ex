defmodule :m_observer_pro_wx do
  ...
  Record.defrecord(:r_sort, :sort, sort_key: 0 + 1 + 1,
                                sort_incr: false)
  Record.defrecord(:r_holder, :holder, parent: :undefined,
                                  info: :undefined, next: [],
                                  sort: :EFE_TODO_NESTED_RECORD, accum: [],
                                  next_accum: [], attrs: :undefined,
                                  node: :undefined, backend_pid: :undefined,
                                  old_backend: false)
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 grid: :undefined, panel: :undefined,
                                 popup_menu: :undefined,
                                 parent_notebook: :undefined, timer: :undefined,
                                 procinfo_menu_pids: [], sel: {[], []},
                                 right_clicked_pid: :undefined,
                                 holder: :undefined)
  def start_link(notebook, parent, config) do
    :wx_object.start_link(:observer_pro_wx,
  ...
  defp setup(notebook, parent, holder, config) do
    ...
    :wxSizer.add(sizer, grid,
                   [{:flag, 8192 ||| (64 ||| 128 ||| 32 ||| 16)},
                        {:proportion, 1}, {:border, 4}])
    :wxWindow.setSizer(proPanel, sizer)
    state = r_state(parent: parent, grid: grid, panel: proPanel,
                parent_notebook: notebook, holder: holder,
                timer: config)
    {proPanel, state}
  end

  defp create_pro_menu(parent, holder) do
    menuEntries = [{'File', [r_create_menu(id: 205, text: 'Dump to file')]}, {'View',
                                                  [r_create_menu(id: 210, text: 'Accumulate',
                                                       type: :check,
                                                       check: call(holder,
                                                                     {:get_accum,
                                                                        self()})),
                                                       :separator, r_create_menu(id: 203,
                                                                       text: 'Refresh\tCtrl-R'),
                                                                       r_create_menu(id: 204,
                                                                           text: 'Refresh Interval')]},
                                                   {'Trace',
                                                      [r_create_menu(id: 206, text: 'Trace processes'),
                                                           r_create_menu(id: 207, text: 'Trace named processes (all nodes)'),
                                                               r_create_menu(id: 208,
  ...
  defp create_list_box(panel, holder) do
    ...
    listItems = [{'Pid', 2, scale * 120}, {'Name or Initial Func', 0, scale * 200},
                                          {'Reds', 1, scale * 100}, {'Memory', 1,
                                                                  scale * 100},
                                                                   {'MsgQ', 1,
                                                                      scale * 50},
                                                                       {'Current Function', 0,
  ...
  def handle_info({:holder_updated, count},
           state0 = r_state(grid: grid)) do
    state = update_selection(state0)
    :wxListCtrl.setItemCount(grid, count)
    count > 0 and :wxListCtrl.refreshItems(grid, 0,
                                             count - 1)
  ...
  def handle_call(:get_config, _,
           r_state(holder: holder, timer: timer) = state) do
    conf = :observer_lib.timer_config(timer)
    accum = (case (safe_call(holder,
                               {:get_accum, self()})) do
  ...
  def handle_event(r_wx(id: 205),
           r_state(panel: panel, holder: holder) = state) do
  ...
  def handle_event(r_wx(id: 210,
             event: r_wxCommand(type: :command_menu_selected,
                        commandInt: cmdInt)),
           r_state(holder: holder) = state) do
    send(holder, {:accum, cmdInt === 1})
    {:noreply, state}
  end

  def handle_event(r_wx(id: 203,
             event: r_wxCommand(type: :command_menu_selected)),
           r_state(holder: holder) = state) do
    send(holder, :refresh)
    {:noreply, state}
  ...
  def handle_event(r_wx(id: 204),
           r_state(panel: panel, timer: timer0) = state) do
    timer = :observer_lib.interval_dialog(panel, timer0, 1,
                                            5 * 60)
    {:noreply, r_state(state, timer: timer)}
  end

  def handle_event(r_wx(id: 201),
           r_state(right_clicked_pid: pid, sel: sel0) = state) do
  ...
  def handle_event(r_wx(id: 202),
           r_state(panel: panel, right_clicked_pid: pid,
               procinfo_menu_pids: opened) = state) do
    opened2 = start_procinfo(pid, panel, opened)
    {:noreply, r_state(state, procinfo_menu_pids: opened2)}
  end

  def handle_event(r_wx(id: 206),
           r_state(sel: {_, pids}, panel: panel) = state) do
    case (pids) do
      [] ->
  ...
  def handle_event(r_wx(id: 207),
           r_state(sel: {selIds, _Pids}, holder: holder,
               panel: panel) = state) do
    case (selIds) do
      [] ->
        :observer_wx.create_txt_dialog(panel, 'No selected processes', 'Tracer', 256)
        {:noreply, state}
      _ ->
        pidsOrReg = call(holder,
  ...
  def handle_event(r_wx(event: r_wxSize(size: {w, _})),
           r_state(grid: grid) = state) do
    :observer_lib.set_listctrl_col_size(grid, w)
    {:noreply, state}
  end

  def handle_event(r_wx(event: r_wxList(type: :command_list_item_right_click,
                      itemIndex: row)),
           r_state(panel: panel, holder: holder) = state) do
    pid = (case (call(holder,
                        {:get_row, self(), row, :pid})) do
             {:error, :undefined} ->
               :undefined
             {:ok, p} ->
               menu = :wxMenu.new()
               :wxMenu.append(menu, 202, 'Process info for ' ++ :erlang.pid_to_list(p))
               :wxMenu.append(menu, 206, 'Trace selected processes', [{:help, 'Trace selected process identifiers'}])
               :wxMenu.append(menu, 207, 'Trace selected processes by name (all nodes)', [{:help, 'Trace selected processes, if a process have a registered name processes with same name will be traced on all nodes'}])
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_focused,
                      itemIndex: row)),
           r_state(grid: grid, holder: holder) = state) do
    case (row >= 0) do
      true ->
        selIds = [row | :lists.delete(row,
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_activated)),
           r_state(panel: panel, procinfo_menu_pids: opened,
               sel: {_, [pid | _]}) = state) do
    opened2 = start_procinfo(pid, panel, opened)
    {:noreply, r_state(state, procinfo_menu_pids: opened2)}
  end
  ...
  defp update_selection(state = r_state(holder: holder, grid: grid,
                      sel: {selIds0, selPids0})) do
    sel = ({selIds, _SelPids} = call(holder,
  ...
  defp rm_selected(pid, [_Id | ids], [pid | pids], accIds,
            accPids) do
    {:lists.reverse(accIds) ++ ids,
       :lists.reverse(accPids) ++ pids}
  end

  defp rm_selected(pid, [id | ids], [otherPid | pids], accIds,
            accPids) do
    rm_selected(pid, ids, pids, [id | accIds],
                  [otherPid | accPids])
  end
  ...
  defp init_table_holder(parent, accum0, attrs) do
    ...
    backend = :erlang.spawn_link(node(), :observer_backend,
                                   :procs_info, [self()])
    accum = (case (accum0) do
               true ->
                 true
               _ ->
                 []
             end)
    table_holder(r_holder(parent: parent, info: :array.new(),
                     node: node(), backend_pid: backend, attrs: attrs,
                     accum: accum))
  end

  defp table_holder(r_holder(info: info, attrs: attrs, node: node,
              backend_pid: backend, old_backend: old) = s0) do
    receive do
      ...
      :refresh ->
        pid = (case (old) do
                 true ->
                   :erlang.spawn_link(node, :observer_backend,
                                        :etop_collect, [self()])
                 false ->
                   :erlang.spawn_link(node, :observer_backend, :procs_info,
                                        [self()])
               end)
      ...
      {:change_node, newNode} ->
        case (node == newNode) do
          true ->
            table_holder(s0)
          false ->
            _ = :rpc.call(newNode, :code, :ensure_loaded,
                            [:observer_backend])
            case (:rpc.call(newNode, :erlang, :function_exported,
                              [:observer_backend, :procs_info, 1])) do
              true ->
                send(self(), :refresh)
      ...
      {:get_accum, from} ->
        ...
        table_holder(s0)
      {:dump, fd} ->
        collector = :erlang.spawn_link(node, :observer_backend,
                                         :etop_collect, [self()])
        receive do
  ...
  defp change_sort(col,
            s0 = r_holder(parent: parent, info: data, sort: sort0)) do
  ...
  defp change_accum(false, s0 = r_holder(info: info)) do
    send(self(), :refresh)
    accum = (for r_etop_proc_info(pid: pid,
                     reds: reds) <- :array.to_list(info) do
               {pid, reds}
             end)
    r_holder(s0, accum: :lists.sort(accum))
  end

  defp handle_update_old(r_etop_info(procinfo: procInfo0),
            s0 = r_holder(parent: parent,
                     sort: sort = r_sort(sort_key: keyField))) do
    {procInfo1, accum} = accum(procInfo0, s0)
    {_SO, procInfo} = sort(keyField,
                             r_sort(sort, sort_key: :undefined), procInfo1)
    info = :array.from_list(procInfo)
    send(parent, {:holder_updated, :array.size(info)})
    r_holder(s0, info: info,  accum: accum)
  end

  defp handle_update(procInfo0,
            s0 = r_holder(next: next, sort: r_sort(sort_key: keyField))) do
    {procInfo1, accum} = accum(procInfo0, s0)
  ...
  defp update_complete(r_holder(parent: parent, sort: r_sort(sort_incr: incr),
              next: procInfo, accum: accum,
              next_accum: nextAccum) = s0) do
    info = (case (incr) do
              true ->
    ...
    send(parent, {:holder_updated, :array.size(info)})
    r_holder(s0, info: info,  accum: accum === true or nextAccum, 
            next: [],  next_accum: [])
  end
  ...
  defp accum(procInfo0,
            r_holder(accum: previous, next_accum: next)) do
    accum = (for r_etop_proc_info(pid: pid, reds: reds) <- procInfo0 do
               {pid, reds}
             end)
    procInfo = :lists.sort(procInfo0)
    {accum2(procInfo, previous, []),
       :lists.merge(:lists.sort(accum), next)}
  end

  defp accum2([pI = r_etop_proc_info(pid: pid, reds: reds) | pIs],
            [{pid, oldReds} | old], acc) do
    accum2(pIs, old, [r_etop_proc_info(pI, reds: reds - oldReds) | acc])
  end

  defp accum2(pIs = [r_etop_proc_info(pid: pid) | _], [{oldPid, _} | old],
            acc)
      when pid > oldPid do
    accum2(pIs, old, acc)
  end
  ...
  defp sort(col, opt = r_sort(sort_key: col, sort_incr: bool),
            table) do
  ...
  defp sort_name(r_etop_proc_info(name: {_, _, _} = a),
            r_etop_proc_info(name: {_, _, _} = b)) do
    a <= b
  end

  defp sort_name(r_etop_proc_info(name: a), r_etop_proc_info(name: b)) when (is_atom(a) and
                                          is_atom(b)) do
    a <= b
  end
  ...
  defp sort_name_rev(r_etop_proc_info(name: {_, _, _} = a),
            r_etop_proc_info(name: {_, _, _} = b)) do
    a >= b
  end

  defp sort_name_rev(r_etop_proc_info(name: a), r_etop_proc_info(name: b)) when (is_atom(a) and
                                          is_atom(b)) do
    a >= b
  end
  ...
  defp get_name_or_pid(from, indices, procInfo) do
    get = fn r_etop_proc_info(name: name) when is_atom(name) ->
               name
  ...
  defp get_row(from, row, col, info) do
    data = (case (row >= :array.size(info)) do
              true ->
                ''
              false ->
  ...
  defp get_rows_from_pids(from, pids0, info) do
    search = fn idx, r_etop_proc_info(pid: pid),
                  acc0 = {pick0, {idxs, pids}} ->
