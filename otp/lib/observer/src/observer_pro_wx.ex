defmodule :m_observer_pro_wx do
  ...
  defp create_pro_menu(parent, holder) do
    menuEntries = [
      {~c"File", [r_create_menu(id: 205, text: ~c"Dump to file")]},
      {~c"View",
       [
         r_create_menu(
           id: 210,
      ...
      {~c"Trace",
       [
         r_create_menu(id: 206, text: ~c"Trace processes"),
         r_create_menu(id: 207, text: ~c"Trace named processes (all nodes)"),
         r_create_menu(
           id: 208,
           text: ~c"Trace new processes"
         )
       ]}
    ]
  ...
  defp create_list_box(panel, holder) do
    ...
    listItems = [
      {~c"Pid", 2, scale * 120},
      {~c"Name or Initial Func", 0, scale * 200},
      {~c"Reds", 1, scale * 100},
      {~c"Memory", 1, scale * 100},
      {~c"MsgQ", 1, scale * 50},
      {~c"Current Function", 0, scale * 200}
    ]
  ...
  def handle_info(
    ...
    :observer_wx.set_status(:io_lib.format(~c"Number of Processes: ~w", [count]))
    {:noreply, state}
  end
  ...
  def handle_info(info, state) do
    :io.format(~c"~p:~p, Unexpected info: ~tp~n", [:observer_pro_wx, 270, info])
    {:noreply, state}
  end
  ...
  def handle_call(msg, _From, state) do
    :io.format(~c"~p:~p: Unhandled call ~tp~n", [:observer_pro_wx, 290, msg])
    {:reply, :ok, state}
  end

  def handle_cast(msg, state) do
    :io.format(~c"~p:~p: Unhandled cast ~tp~n", [:observer_pro_wx, 294, msg])
    {:noreply, state}
  end
  ...
  def handle_event(
    ...
    case pids do
      [] ->
        :observer_wx.create_txt_dialog(panel, ~c"No selected processes", ~c"Tracer", 256)
        {:noreply, state}
  ...
  def handle_event(
    ...
    pid =
      case call(
        ...
        {:ok, p} ->
          menu = :wxMenu.new()
          :wxMenu.append(menu, 202, ~c"Process info for " ++ :erlang.pid_to_list(p))

          :wxMenu.append(menu, 206, ~c"Trace selected processes", [
            {:help, ~c"Trace selected process identifiers"}
          ])

          :wxMenu.append(menu, 207, ~c"Trace selected processes by name (all nodes)", [
            {:help,
             ~c"Trace selected processes, if a process have a registered name processes with same name will be traced on all nodes"}
          ])

          :wxMenu.append(menu, 211, ~c"Garbage collect processes")
          :wxMenu.append(menu, 201, ~c"Kill process " ++ :erlang.pid_to_list(p))
  ...
  def handle_event(event, state) do
    :io.format(~c"~p:~p: handle event ~tp\n", [:observer_pro_wx, 422, event])
    {:noreply, state}
  end
