defmodule :m_observer_procinfo do
  ...
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 frame: :undefined, notebook: :undefined,
                                 pid: :undefined, pages: [],
                                 expand_table: :undefined, expand_wins: [])
  Record.defrecord(:r_worker, :worker, panel: :undefined,
                                  callback: :undefined)
  Record.defrecord(:r_io, :io, rdata: '')
  def start(process, parentFrame, parent) do
    :wx_object.start_link(:observer_procinfo,
                            [process, parentFrame, parent], [])
  end

  def init([pid, parentFrame, parent]) do
    try do
      table = :ets.new(:observer_expand, [:set, :public])
      title = (case (:observer_wx.try_rpc(node(pid), :erlang,
                                            :process_info,
      ...
      frame = :wxFrame.new(parentFrame, - 1,
                             [:erlang.atom_to_list(node(pid)), ?:, title],
                             [{:style,
      ...
      processPage = init_panel(notebook, 'Process Information', [pid],
                                 &init_process_page/2)
      messagePage = init_panel(notebook, 'Messages', [pid, table],
                                 &init_message_page/3)
      dictPage = init_panel(notebook, 'Dictionary', [pid, table],
                              &init_dict_page/3)
      stackPage = init_panel(notebook, 'Stack Trace', [pid],
                               &init_stack_page/2)
      statePage = init_panel(notebook, 'State', [pid, table],
                               &init_state_page/3)
      ps = (case (:gen_server.call(:observer, :log_status)) do
              true ->
                [init_panel(notebook, 'Log', [pid, table],
                              &init_log_page/3)]
              false ->
                []
            end)
      ...
      :wxFrame.show(frame)
      {frame,
         r_state(parent: parent, pid: pid, frame: frame,
             notebook: notebook,
  ...
  def handle_event(r_wx(id: 5001,
             event: r_wxCommand(type: :command_menu_selected)),
           state) do
    {:stop, :normal, state}
  end

  def handle_event(r_wx(id: 601),
           r_state(frame: frame, pid: pid, pages: pages,
               expand_table: t) = state) do
  ...
  def handle_event(r_wx(obj: obj, event: r_wxMouse(type: :leave_window)),
           state) do
    :wxStaticText.setForegroundColour(obj,
                                        :wxe_util.get_const(:wxBLUE))
    {:noreply, state}
  end

  def handle_event(r_wx(event: r_wxHtmlLink(linkInfo: r_wxHtmlLinkInfo(href: href))),
           r_state(frame: frame, expand_table: t,
               expand_wins: opened0) = state) do
    {type, rest} = (case (href) do
                      '#Term?' ++ keys ->
                        {:cdv_term_cb, keys}
                      '#OBSBinary?' ++ keys ->
                        {:cdv_bin_cb, keys}
    ...
    case (type) do
      ...
      callback ->
        [{'key1', key1}, {'key2', key2}, {'key3',
                                  key3}] = :uri_string.dissect_query(rest)
        id = {:obs,
                {t,
  ...
  def handle_info({:get_debug_info, from},
           state = r_state(notebook: notebook)) do
  ...
  defp init_process_page(panel, pid) do
    wSz = :observer_wx.try_rpc(node(pid), :erlang,
                                 :system_info, [:wordsize])
    fields0 = process_info_fields(pid, wSz)
    {fPanel, _,
       upFields} = :observer_lib.display_info(panel, fields0)
    {fPanel,
       fn () ->
  ...
  defp init_message_page(parent, pid, table) do
    ...
    update = fn () ->
                  case (:observer_wx.try_rpc(node(pid), :erlang,
                                               :process_info,
                                               [pid, :messages])) do
                    {:messages, messages} ->
  ...
  defp init_dict_page(parent, pid, table) do
    ...
    update = fn () ->
                  case (:observer_wx.try_rpc(node(pid), :erlang,
                                               ...
                                               [pid, :dictionary])) do
                    {:dictionary, dict} ->
                      html = :observer_html_lib.expandable_term('Dictionary', dict,
                                                                  table, cs)
                      :wxHtmlWindow.setPage(win, html)
                    _ ->
                      throw(:process_undefined)
  ...
  defp init_stack_page(parent, pid) do
    ...
    update = fn () ->
                  case (:observer_wx.try_rpc(node(pid), :erlang,
                                               :process_info,
                                               [pid, :current_stacktrace])) do
                    {:current_stacktrace, rawBt} ->
                      :wxListCtrl.deleteAllItems(lCtrl)
                      :wx.foldl(fn {m, f, a, info}, row ->
                                     _Item = :wxListCtrl.insertItem(lCtrl, row,
                                                                      '')
                                     rem(row, 2) === 0 or :wxListCtrl.setItemBackgroundColour(lCtrl,
                                                                                                row,
                                                                                                odd)
                                     :wxListCtrl.setItem(lCtrl, row, 0,
                                                           :observer_lib.to_str({m,
                                                                                   f,
                                                                                   a}))
                                     fileLine = (case (info) do
                                                   [{:file, file}, {:line,
                                                                      line}] ->
                                                     :io_lib.format('~ts:~w',
                                                                      [file,
  ...
  defp init_state_page(parent, pid, table) do
    ...
    update = fn () ->
                  ...
                  html = :observer_html_lib.expandable_term('ProcState', stateInfo,
                                                              table, cs)
                  :wxHtmlWindow.setPage(win, html)
             end
    update.()
  ...
  defp fetch_state_info(pid) do
    case (:rpc.call(node(pid), :proc_lib,
                      :translate_initial_call, [pid])) do
      {:proc_lib, :init_p, 5} ->
        []
      {m, _F, _A} ->
  ...
  defp fetch_state_info2(pid, m) do
    ...
    case (:rpc.call(node(pid), :sys, :get_status,
                      [pid, 200])) do
      {:status, _, {:module, _},
         [_PDict, _SysState, _Parent, _Dbg, [header, {:data,
                                                        first},
                                                         {:data, second} |
                                                             _]]} ->
      ...
      {:status, _, {:module, _},
        ...
        case (:lists.keyfind(:format_status, 1,
                               :rpc.call(node(pid), m, :module_info,
                                           [:exports]))) do
          false ->
            opt = {'Format', :unknown}
          _ ->
  ...
  defp init_log_page(parent, pid, table) do
    ...
    update = fn () ->
                  ...
                  expPid = :re.replace(logs, '<0.', '<' ++ pref ++ '.',
                                         [:global, {:return, :list}])
                  nbBlanks = length(pref) - 1
                  re = '(<' ++ pref ++ '.[^>]{1,}>)[ ]{' ++ :erlang.integer_to_list(nbBlanks) ++ '}'
                  look = :re.replace(expPid, re, '\\1',
                                       [:global, {:return, :list}])
                  html = :observer_html_lib.expandable_term('SaslLog', look,
                                                              table, cs)
                  :wxHtmlWindow.setPage(win, html)
             end
    update.()
  ...
  defp create_menus(menuBar) do
    menus = [{'File', [r_create_menu(id: 5001, text: 'Close')]}, {'View',
                                             [r_create_menu(id: 601, text: 'Refresh\tCtrl-R')]}]
    :observer_lib.create_menus(menus, menuBar, :new_window)
  end

  defp process_info_fields(pid, wSz) do
    struct = [{'Overview',
                 [{'Initial Call', :initial_call}, {'Current Function', :current_function}, {'Registered Name',
                                                                 :registered_name},
                                                                  {'Status', :status},
                                                                      {'Message Queue Len',
                                                                         :message_queue_len},
                                                         ...
                                                         {{:words, wSz},
                                                          ...
                                                          {'Stack Size',
                                                             {{:words, wSz},
                                                              ...
                                                              {'GC Min Heap Size',
                                                                 {{:words, wSz},
                                                                    get_gc_info(:min_heap_size)}},
                                                                  {'GC FullSweep After',
                                                                     get_gc_info(:fullsweep_after)}]}]
    case (:observer_wx.try_rpc(node(pid), :erlang,
                                 :process_info, [pid, item_list()])) do
  ...
  defp item_list() do
    [:binary, :catchlevel, :current_function,
                               :error_handler, :garbage_collection,
                                                   :group_leader, :heap_size,
                                                                      :initial_call,
                                                                          :last_calls,
                                                                              :links,
                                                                                  :memory,
  ...
  defp stringify_bins(data) do
    ...
    for {_Ptr, sz, refc} <- bins do
      :lists.flatten(:io_lib.format('<< ~s, refc ~w>>',
                                      [:observer_lib.to_str({:bytes, sz}),
                                           refc]))
    end
  end

  defp local_pid_str(pid) do
    '<0' ++ :re.replace(:erlang.pid_to_list(pid), '<([0-9]{1,})', '',
                       [{:return, :list}])
  end

  defp global_pid_node_pref(pid) do
    [nodePrefix |
         _] = :string.lexemes(:erlang.pid_to_list(pid), '<.')
    nodePrefix
  end
  ...
  defp io_server(state) do
    receive do
      ...
      {pid, :get_data_and_close} ->
        send(pid, {self(), :data,
                     :lists.reverse(r_io(state, :rdata))})
        :normal
      _Unknown ->
        io_server(state)
    end
  ...
  defp io_request({:put_chars, _Encoding, chars},
            state = r_io(rdata: data)) do
    {:ok, :ok, r_io(state, rdata: [chars | data])}
  end

  defp io_request({:put_chars, encoding, module, function, args},
            state) do
    try do
      io_request({:put_chars, encoding,
