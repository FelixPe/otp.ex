defmodule :m_observer_procinfo do
  ...
  Record.defrecord(:r_io, :io, rdata: ~c"")
    ...
    try do
      ...
      title =
        case :observer_wx.try_rpc(node(pid), :erlang, :process_info, [pid, :registered_name]) do
          [] ->
            :io_lib.format(~c"~p", [pid])

          {:registered_name, registered} ->
            :io_lib.format(~c"~tp (~p)", [registered, pid])
      ...
      processPage = init_panel(notebook, ~c"Process Information", [pid], &init_process_page/2)
      messagePage = init_panel(notebook, ~c"Messages", [pid, table], &init_message_page/3)
      dictPage = init_panel(notebook, ~c"Dictionary", [pid, table], &init_dict_page/3)
      stackPage = init_panel(notebook, ~c"Stack Trace", [pid], &init_stack_page/2)
      statePage = init_panel(notebook, ~c"State", [pid, table], &init_state_page/3)

      ps =
        case :gen_server.call(:observer, :log_status) do
          true ->
            [init_panel(notebook, ~c"Log", [pid, table], &init_log_page/3)]
      ...
      :process_undefined ->
        :observer_lib.display_info_dialog(parentFrame, ~c"No such alive process")
        {:stop, :normal}
    end
  end
  ...
  def handle_event(
    ...
    try do
      ...
      :process_undefined ->
        :wxFrame.setTitle(frame, :io_lib.format(~c"*DEAD* ~p", [pid]))
    end
  ...
  def handle_event(
    ...
    case type do
      ...
      callback ->
        [{~c"key1", key1}, {~c"key2", key2}, {~c"key3", key3}] = :uri_string.dissect_query(rest)
  ...
  defp init_message_page(parent, pid, table) do
    ...
    update = fn ->
      case :observer_wx.try_rpc(node(pid), :erlang, :process_info, [pid, :messages]) do
        {:messages, messages} ->
          html = :observer_html_lib.expandable_term(~c"Message Queue", messages, table, cs)
  ...
  defp init_dict_page(parent, pid, table) do
    ...
    update = fn ->
      case :observer_wx.try_rpc(node(pid), :erlang, :process_info, [pid, :dictionary]) do
        {:dictionary, dict} ->
          html = :observer_html_lib.expandable_term(~c"Dictionary", dict, table, cs)
          :wxHtmlWindow.setPage(win, html)
  ...
  defp init_stack_page(parent, pid) do
    ...
    li = :wxListItem.new()
    scale = :observer_wx.get_scale()
    :wxListItem.setText(li, ~c"Module:Function/Arg")
    :wxListCtrl.insertColumn(lCtrl, 0, li)
    :wxListCtrl.setColumnWidth(lCtrl, 0, scale * 300)
    :wxListItem.setText(li, ~c"File:LineNumber")
  ...
  defp init_state_page(parent, pid, table) do
    ...
    update = fn ->
      ...
      html = :observer_html_lib.expandable_term(~c"ProcState", stateInfo, table, cs)
      :wxHtmlWindow.setPage(win, html)
    end
    ...
    case :rpc.call(node(pid), :sys, :get_status, [pid, 200]) do
      {:status, _, {:module, _},
       [
        ...
        [{~c"Behaviour", b}, header] ++ first ++ second

      {:status, _, {:module, _},
       [_PDict, _SysState, _Parent, _Dbg, [header, {:data, first}, otherFormat]]} ->
        [{~c"Behaviour", b}, header] ++ first ++ [{~c"State", otherFormat}]
      ...
      {:status, _, {:module, _}, [_PDict, _SysState, _Parent, _Dbg, otherFormat]} ->
        case :lists.keyfind(:format_status, 1, :rpc.call(node(pid), m, :module_info, [:exports])) do
          false ->
            opt = {~c"Format", :unknown}

          _ ->
            opt = {~c"Format", :overridden}
        end

        [{~c"Behaviour", b}, opt, {~c"State", otherFormat}]
  ...
  defp init_log_page(parent, pid, table) do
    ...
    update = fn ->
      ...
      expPid = :re.replace(logs, ~c"<0.", ~c"<" ++ pref ++ ~c".", [:global, {:return, :list}])
      nbBlanks = length(pref) - 1
      re = ~c"(<" ++ pref ++ ~c".[^>]{1,}>)[ ]{" ++ :erlang.integer_to_list(nbBlanks) ++ ~c"}"
      look = :re.replace(expPid, re, ~c"\\1", [:global, {:return, :list}])
      html = :observer_html_lib.expandable_term(~c"SaslLog", look, table, cs)
      :wxHtmlWindow.setPage(win, html)
    end
  ...
  defp create_menus(menuBar) do
    menus = [
      {~c"File", [r_create_menu(id: 5001, text: ~c"Close")]},
      {~c"View", [r_create_menu(id: 601, text: ~c"Refresh\tCtrl-R")]}
    ]
  ...
  defp process_info_fields(pid, wSz) do
    struct = [
      {~c"Overview",
       [
         {~c"Initial Call", :initial_call},
         {~c"Current Function", :current_function},
         {~c"Registered Name", :registered_name},
         {~c"Status", :status},
         {~c"Message Queue Len", :message_queue_len},
         {~c"Group Leader", {:click, :group_leader}},
         {~c"Priority", :priority},
         {~c"Trap Exit", :trap_exit},
         {~c"Reductions", :reductions},
         {~c"Binary",
          fn data ->
            stringify_bins(data)
          end},
         {~c"Last Calls", :last_calls},
         {~c"Catch Level", :catchlevel},
         {~c"Trace", :trace},
         {~c"Suspending", :suspending},
         {~c"Sequential Trace Token", :sequential_trace_token},
         {~c"Error Handler", :error_handler}
       ]},
      {:scroll_boxes,
       [
         {~c"Links", {:click, :links}},
         {~c"Monitors", {:click, filter_monitor_info()}},
         {~c"Monitored by", {:click, :monitored_by}}
       ]},
      {~c"Memory and Garbage Collection", :right,
       [
         {~c"Memory", {:bytes, :memory}},
         {~c"Stack and Heaps", {{:words, wSz}, :total_heap_size}},
         {~c"Heap Size", {{:words, wSz}, :heap_size}},
         {~c"Stack Size", {{:words, wSz}, :stack_size}},
         {~c"GC Min Heap Size", {{:words, wSz}, get_gc_info(:min_heap_size)}},
         {~c"GC FullSweep After", get_gc_info(:fullsweep_after)}
       ]}
    ]
  ...
  defp local_pid_str(pid) do
    ~c"<0" ++ :re.replace(:erlang.pid_to_list(pid), ~c"<([0-9]{1,})", ~c"", [{:return, :list}])
  end
  ...
  defp global_pid_node_pref(pid) do
    [
      nodePrefix
      | _
    ] = :string.lexemes(:erlang.pid_to_list(pid), ~c"<.")
