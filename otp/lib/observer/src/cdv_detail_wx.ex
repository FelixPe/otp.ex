defmodule :m_cdv_detail_wx do
  ...
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 frame: :undefined, id: :undefined, pages: [])
  def start_link(id, data, parentFrame, callback, app) do
    :wx_object.start_link(:cdv_detail_wx,
                            [id, data, parentFrame, callback, app, self()], [])
  end

  def init([id, data, parentFrame, callback, app,
                                               parent]) do
  ...
  defp init(id, parentFrame, callback, app, parent,
            {title, info, tW}) do
    scale = :observer_wx.get_scale()
    frame = :wxFrame.new(parentFrame, - 1, [title],
                           [{:style,
    ...
    {infoPanel, pages} = create_pages(panel,
                                        callback.detail_pages(), [info])
    :wxSizer.add(sizer, infoPanel,
    ...
    destroy_progress(app)
    {frame,
       r_state(parent: parent, id: id, frame: frame, pages: pages)}
  end
  ...
  defp create_pages(panel, pageSpecs, funArgs) do
    ...
    pages = (for {pageTitle, fun} <- pageSpecs do
  ...
  def terminate(_Reason,
           r_state(parent: parent, id: id, frame: frame)) do
