defmodule :m_observer_sys_wx do
  ...
  defp create_sys_menu(parent) do
    view =
      {~c"View",
       [
         r_create_menu(id: 101, text: ~c"Refresh\tCtrl-R"),
         r_create_menu(id: 102, text: ~c"Refresh interval")
       ]}
  ...
  defp info_fields() do
    sys = [
      {~c"System and Architecture",
       [
         {~c"System Version", :otp_release},
         {~c"ERTS Version", :version},
         {~c"Compiled for", :system_architecture},
         {~c"Emulator Wordsize", :wordsize_external},
         {~c"Process Wordsize", :wordsize_internal},
         {~c"SMP Support", :smp_support},
         {~c"Thread Support", :threads},
         {~c"Async thread pool size", :thread_pool_size}
       ]}
    ]
    ...
    cpu = [
      {~c"CPUs and Threads",
       [
         {~c"Logical CPUs", :logical_processors},
         {~c"Online Logical CPUs", :logical_processors_online},
         {~c"Available Logical CPUs", :logical_processors_available},
         {~c"Schedulers", :schedulers},
         {~c"Online schedulers", :schedulers_online},
         {~c"Available schedulers", :schedulers_available}
       ]}
    ]

    mem = [
      {~c"Memory Usage", :right,
       [
         {~c"Total", {:bytes, :total}},
         {~c"Processes", {:bytes, :processes}},
         {~c"Atoms", {:bytes, :atom}},
         {~c"Binaries", {:bytes, :binary}},
         {~c"Code", {:bytes, :code}},
         {~c"ETS", {:bytes, :ets}}
       ]}
    ]
    ...
    stats = [
      {~c"Statistics", :right,
       [
         {~c"Up time", {:time_ms, :uptime}},
         {~c"Run Queue", :run_queue},
         {~c"IO Input", {:bytes, :io_input}},
         {~c"IO Output", {:bytes, :io_output}}
       ]}
    ]

    limits = [
      {~c"System statistics / limit",
       [
         {~c"Atoms", get_limit_count_info(:atom_count, :atom_limit)},
         {~c"Processes", get_limit_count_info(:process_count, :process_limit)},
         {~c"Ports", get_limit_count_info(:port_count, :port_limit)},
         {~c"ETS", get_limit_count_info(:ets_count, :ets_limit)},
         {~c"Distribution buffer busy limit", get_dist_buf_busy_limit_info()}
       ]}
    ]
  ...
  def handle_info(info, state) do
    :io.format(~c"~p:~p: Unhandled info: ~tp~n", [:observer_sys_wx, 202, info])
    {:noreply, state}
  end
  ...
  def handle_call(msg, _From, state) do
    :io.format(~c"~p~p: Unhandled Call ~tp~n", [:observer_sys_wx, 215, msg])
    {:reply, :ok, state}
  end

  def handle_cast(msg, state) do
    :io.format(~c"~p~p: Unhandled cast ~tp~n", [:observer_sys_wx, 219, msg])
    {:noreply, state}
  end
  ...
  def handle_event(event, state) do
    :io.format(~c"~p:~p: Unhandled event ~tp\n", [:observer_sys_wx, 238, event])
    {:noreply, state}
  end
end
