defmodule :m_observer_sys_wx do
  ...
  Record.defrecord(:r_sys_wx_state, :sys_wx_state, parent: :undefined,
                                        node: :undefined,
                                        parent_notebook: :undefined,
                                        panel: :undefined, sizer: :undefined,
                                        menubar: :undefined, fields: :undefined,
                                        timer: :undefined)
  def start_link(notebook, parent, config) do
    :wx_object.start_link(:observer_sys_wx,
  ...
  def init([notebook, parent, config]) do
    ...
    {fPanel0, _FSizer0,
       fields0} = :observer_lib.display_info(panel,
                                               :observer_lib.fill_info(sys,
                                                                         sysInfo))
    {fPanel1, _FSizer1,
       fields1} = :observer_lib.display_info(panel,
                                               :observer_lib.fill_info(mem,
                                                                         sysInfo))
    :wxSizer.add(hSizer0, fPanel0,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(hSizer0, fPanel1,
                   [{:flag, 8192}, {:proportion, 1}])
    hSizer1 = :wxBoxSizer.new(4)
    {fPanel2, _FSizer2,
       fields2} = :observer_lib.display_info(panel,
                                               :observer_lib.fill_info(cpu,
                                                                         sysInfo))
    {fPanel3, _FSizer3,
       fields3} = :observer_lib.display_info(panel,
                                               :observer_lib.fill_info(stats,
                                                                         sysInfo))
    :wxSizer.add(hSizer1, fPanel2,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(hSizer1, fPanel3,
                   [{:flag, 8192}, {:proportion, 1}])
    hSizer2 = :wxBoxSizer.new(4)
    {fPanel4, _FSizer4,
       fields4} = :observer_lib.display_info(panel,
                                               :observer_lib.fill_info(limits,
                                                                         sysInfo))
    :wxSizer.add(hSizer2, fPanel4,
                   [{:flag, 8192}, {:proportion, 1}])
    borderFlags = 16 ||| 32
    ...
    {panel,
       r_sys_wx_state(parent: parent, parent_notebook: notebook,
           panel: panel, sizer: sizer, timer: timer,
           fields: fields0 ++ fields1 ++ fields2 ++ fields3 ++ fields4)}
  end

  defp create_sys_menu(parent) do
    view = {'View', [r_create_menu(id: 101, text: 'Refresh\tCtrl-R'), r_create_menu(id: 102, text: 'Refresh interval')]}
  ...
  defp update_syspage(r_sys_wx_state(node: node, fields: fields, sizer: sizer)) do
    sysInfo = :observer_wx.try_rpc(node, :observer_backend,
                                     :sys_info, [])
    {sys, mem, cpu, stats, limits} = info_fields()
    :observer_lib.update_info(fields,
                                :observer_lib.fill_info(sys,
                                                          sysInfo) ++ :observer_lib.fill_info(mem,
                                                                                                sysInfo) ++ :observer_lib.fill_info(cpu,
                                                                                                                                      sysInfo) ++ :observer_lib.fill_info(stats,
                                                                                                                                                                            sysInfo) ++ :observer_lib.fill_info(limits,
                                                                                                                                                                                                                  sysInfo))
  ...
  defp get_dist_buf_busy_limit_info() do
    fn data ->
         maybe_convert(:proplists.get_value(:dist_buf_busy_limit,
                                              data))
    end
  end
                                         ...
                                         [maybe_convert(c), maybe_convert(l),
                                                                cond do
                                                                  ...
                                                                  l === :undefined ->
                                                                    ''
                                                                  true ->
                                                                    :io_lib.format('(~s % used)',
                                                                                     [:observer_lib.to_str({:trunc,
                                                                                                              c / l * 100})])
                                                                end]))
    end
  end
  ...
  defp info_fields() do
    sys = [{'System and Architecture',
              [{'System Version', :otp_release}, {'ERTS Version', :version}, {'Compiled for',
                                                    :system_architecture},
    ...
    cpu = [{'CPUs and Threads',
              [{'Logical CPUs', :logical_processors}, {'Online Logical CPUs',
                                            :logical_processors_online},
                                             {'Available Logical CPUs', :logical_processors_available},
                                                 {'Schedulers', :schedulers}, {'Online schedulers',
                                                                      :schedulers_online},
                                                                       {'Available schedulers',
                                                                          :schedulers_available}]}]
    mem = [{'Memory Usage', :right,
              [{'Total', {:bytes, :total}}, {'Processes', {:bytes, :processes}}, {'Atoms',
                                                                    {:bytes,
                                                                       :atom}},
                                                                     {'Binaries',
                                                                        {:bytes,
    ...
    stats = [{'Statistics', :right,
                [{'Up time', {:time_ms, :uptime}}, {'Run Queue', :run_queue}, {'IO Input',
                                                               {:bytes,
                                                                  :io_input}},
                                                                {'IO Output',
                                                                   {:bytes,
    ...
    limits = [{'System statistics / limit',
                 [{'Atoms', get_limit_count_info(:atom_count, :atom_limit)},
                      {'Processes',
                         get_limit_count_info(:process_count, :process_limit)},
  ...
  def handle_info(:refresh_interval,
           r_sys_wx_state(panel: panel, node: node) = state) do
    try do
  ...
  def handle_info({:active, node},
           r_sys_wx_state(parent: parent, panel: panel,
    ...
    try do
      ...
      {:noreply,
         r_sys_wx_state(updState, timer: :observer_lib.start_timer(timer))}
    catch
      :error, {:badrpc, _} ->
        :observer_wx.return_to_localnode(panel, node)
        {:noreply, state}
  ...
  def handle_info(:not_active, r_sys_wx_state(timer: timer) = state) do
    {:noreply,
       r_sys_wx_state(state, timer: :observer_lib.stop_timer(timer))}
  end
  ...
  def handle_event(r_wx(id: 101,
           ...
           r_sys_wx_state(node: node, panel: panel) = state) do
    try do
      update_syspage(state)
    catch
      :error, {:badrpc, _} ->
  ...
  def handle_event(r_wx(id: 102,
             event: r_wxCommand(type: :command_menu_selected)),
           r_sys_wx_state(timer: timer0, parent_notebook: notebook) = state) do
    timer = :observer_lib.interval_dialog(notebook, timer0,
                                            1, 5 * 60)
