defmodule :m_cdv_proc_cb do
  ...
  def col_spec() do
    [{'Pid', 2, 120}, {'Name or Initial Func', 0, 250}, {'State', 0, 100}, {'Reds', 1, 80}, {'Memory',
                                                           1, 80},
                                                            {'MsgQ', 1, 50}]
  end
  ...
  def get_details(id, _) do
    case (:crashdump_viewer.proc_details(id)) do
      {:ok, info, tW} ->
        ...
        proplist0 = :crashdump_viewer.to_proplist(Keyword.keys(r_proc(r_proc())),
                                                    info)
      ...
      {:error, {:other_node, nodeId}} ->
        info = 'The process you are searching for was residing on a remote node. No process information is available. Show information about the remote node?'
        fun = fn () ->
  ...
  def detail_pages() do
    [{'General Information', &init_gen_page/2}, {'Messages', &init_message_page/2}, {'Dictionary',
                                                          &init_dict_page/2},
                                                           {'Stack Dump',
                                                              &init_stack_page/2},
                                                               {'ETS tables',
  ...
  defp init_memory_page(parent, info0, tag, heading) do
    ...
    html = :observer_html_lib.expandable_term(heading, info,
                                                tab,
                                                :observer_lib.colors(parent))
    :cdv_html_wx.start_link(parent, {:expand, html, tab})
  end

  defp init_ets_page(parent, info) do
    pid = :proplists.get_value(:pid, info)
    :cdv_virtual_list_wx.start_link(parent, :cdv_ets_cb,
                                      pid)
  end

  defp init_timer_page(parent, info) do
    pid = :proplists.get_value(:pid, info)
    :cdv_virtual_list_wx.start_link(parent, :cdv_timer_cb,
                                      pid)
  end

  defp info_fields() do
    [{'Overview',
        [{'Initial Call', :init_func}, {:dynamic, :current_func}, {'Registered Name', :name},
                                                         {'Status', :state}, {'Internal State',
                                                                         :int_state},
                                                                          {'Started',
                                                                             :start_time},
                                                                              {'Parent',
         ...
         {:scroll_boxes, [{'Last Calls', 1, {:plain, :last_calls}}]},
             {:scroll_boxes,
                [{'Links', 1, {:click, :links}}, {'Monitors', 2, {:click, :monitors}},
                                               {'Monitored By', 2, {:click, :mon_by}}]},
                 {'Memory and Garbage Collection',
                    [{'Memory', :memory}, {'Stack and Heap', :stack_heap}, {'Old Heap', :old_heap}, {'Heap Unused',
                                                                        :heap_unused},
                                                                         {'Old Heap Unused',
                                                                            :old_heap_unused},
                                                                             {'Binary vheap',
