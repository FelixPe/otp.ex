defmodule :m_cdv_table_wx do
  ...
  def init([parentWin, {columnSpec, info, tW}]) do
    ...
    insert = fn rowData, row ->
      :wxListCtrl.insertItem(grid, row, ~c"")
  ...
  def handle_info(info, state) do
    :io.format(~c"~p:~p: Unhandled info: ~tp~n", [:cdv_table_wx, 78, info])
    {:noreply, state}
  end
  ...
  def handle_call(msg, _From, state) do
    :io.format(~c"~p~p: Unhandled Call ~tp~n", [:cdv_table_wx, 88, msg])
    {:reply, :ok, state}
  end

  def handle_cast(msg, state) do
    :io.format(~c"~p~p: Unhandled cast ~tp~n", [:cdv_table_wx, 92, msg])
    {:noreply, state}
  end
