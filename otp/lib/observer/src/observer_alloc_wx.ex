defmodule :m_observer_alloc_wx do
  use Bitwise
  import :observer_perf_wx, only: [add_data: 5,
                                     interval_dialog: 2, make_win: 4,
                                     precalc: 4, refresh_panel: 4,
  ...
  Record.defrecord(:r_state, :state, time: :EFE_TODO_NESTED_RECORD,
                                 ...
                                 wins: :undefined, mem: :undefined,
                                 samples: :undefined, max: :undefined,
                                 panel: :undefined, paint: :undefined,
                                 appmon: :undefined, async: :undefined)
  def start_link(notebook, parent, config) do
    :wx_object.start_link(:observer_alloc_wx,
  ...
  def init([notebook, parent, config]) do
    try do
      ...
      {topP,
         r_state(parent: parent, panel: panel, wins: windows,
             mem: memWin, paint: paintInfo, time: setup_time(config),
             max: %{})}
    catch
      _, err ->
  ...
  defp setup_time(config) do
    ...
    r_ti(disp: 10 / freq, fetch: freq,
        secs: :maps.get(:secs, config, 60))
  end

  def handle_event(r_wx(id: 102,
             event: r_wxCommand(type: :command_menu_selected)),
           r_state(active: active, panel: panel, appmon: old,
               wins: wins0, time: r_ti(fetch: f0) = ti0) = state) do
  ...
  def handle_info({key, {:promise_reply, {:badrpc, _}}},
           r_state(async: key) = state) do
    {:noreply, r_state(state, active: false,  appmon: :undefined)}
  end

  def handle_info({key, {:promise_reply, sysInfo}},
           r_state(async: key, samples: data, max: max0, active: active,
               wins: wins0,
               time: r_ti(tick: tick, disp: disp0) = ti) = s0) do
    disp = trunc(disp0)
    ...
    {wins, samples} = add_data(info, data, wins0, ti,
                                 active)
    s1 = r_state(s0, time: r_ti(ti, tick: next),  wins: wins, 
                 samples: samples,  max: max,  async: :undefined)
    cond do
      active ->
  ...
  def handle_info({:refresh, seq},
           state = r_state(panel: panel, appmon: node,
                       time: r_ti(tick: seq, disp: dispF) = ti))
      when seq + 1 < dispF * 1.5 do
    next = seq + 1
    r_state(state, :active) and ((try do
    ...
    cond do
      seq === trunc(dispF) - 1 ->
        ...
        {:noreply,
           r_state(state, time: r_ti(ti, tick: next),  async: req)}
      true ->
  ...
  def handle_info({:active, node},
           state = r_state(parent: parent, panel: panel, appmon: old)) do
    create_menus(parent, [])
    try do
      ^node = old
      :wxWindow.refresh(panel)
  ...
  defp restart_fetcher(node,
            r_state(panel: panel, wins: wins0, time: ti) = state) do
    case (:rpc.call(node, :observer_backend, :sys_info,
                      [])) do
      {:badrpc, _} ->
        state
      sysInfo ->
        ...
        {wins, samples} = add_data(info, {0, :queue.new()},
                                     wins0, ti, true)
        :erlang.send_after(div(1000, 10), self(), {:refresh, 0})
        :wxWindow.refresh(panel)
        precalc(r_state(state, active: true,  appmon: node, 
                           time: r_ti(ti, tick: 0),  wins: wins, 
  ...
  defp update_alloc(r_state(mem: grid), fields, max) do
    ...
    update = fn {name, bS, cS}, row ->
                  ...
                  :wxListCtrl.setItem(grid, row, 0,
                                        :observer_lib.to_str(name))
                  :wxListCtrl.setItem(grid, row, 1,
                                        :observer_lib.to_str(div(bS, 1024)))
                  :wxListCtrl.setItem(grid, row, 2,
                                        :observer_lib.to_str(div(cS, 1024)))
                  :wxListCtrl.setItem(grid, row, 3,
                                        :observer_lib.to_str(div(maxV, 1024)))
                  row + 1
             end
  ...
  defp alloc_info([{type, instances} | allocators], typeAcc,
            totalBS, totalCS, includeTotal) do
    {bS, cS, newTotalBS, newTotalCS,
       newIncludeTotal} = sum_alloc_instances(instances, 0, 0,
                                                totalBS, totalCS)
    alloc_info(allocators, [{type, bS, cS} | typeAcc],
                 newTotalBS, newTotalCS,
                 includeTotal and newIncludeTotal)
  end

  defp alloc_info([], typeAcc, totalBS, totalCS, includeTotal) do
    types = (for (x = {_, bS, cS}) <- typeAcc,
                   bS > 0 or cS > 0 do
               x
             end)
  ...
  defp sum_alloc_instances([{_, _, data} | instances], bS, cS, totalBS,
            totalCS) do
    {newBS, newCS, newTotalBS,
       newTotalCS} = sum_alloc_one_instance(data, bS, cS,
                                              totalBS, totalCS)
    sum_alloc_instances(instances, newBS, newCS, newTotalBS,
                          newTotalCS)
  end
  ...
  defp sum_alloc_one_instance([{_,
              [{:blocks, typedBlocks}, {:carriers_size, cS, _, _}]} |
               rest],
            oldBS, oldCS, totalBS, totalCS) do
    bS = sum_alloc_block_list(typedBlocks, 0)
    sum_alloc_one_instance(rest, oldBS + bS, oldCS + cS,
                             totalBS + bS, totalCS + cS)
  end

  defp sum_alloc_one_instance([{_,
              [{:blocks_size, bS, _, _}, {:carriers_size, cS, _,
                                            _}]} |
               rest],
            oldBS, oldCS, totalBS, totalCS) do
    sum_alloc_one_instance(rest, oldBS + bS, oldCS + cS,
  ...
  defp sum_alloc_block_list([{_Type, [{:size, current, _, _}]} | rest],
            acc) do
  ...
  defp create_mem_info(parent) do
    ...
    listItems = [{'Allocator Type', 0, 200}, {'Block size (kB)', 1, 150}, {'Carrier size (kB)', 1, 150}, {'Max Carrier size (kB)',
                                                           1, 150}]
