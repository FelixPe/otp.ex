defmodule :m_observer_alloc_wx do
  ...
  def init([notebook, parent, config]) do
    try do
      ...
      _, err ->
        :io.format(~c"~p crashed ~tp: ~tp~n", [:observer_alloc_wx, err, __STACKTRACE__])
        {:stop, err}
    end
  end
  ...
  defp update_alloc(r_state(mem: grid), fields, max) do
    ...
    update = fn {name, bS, cS}, row ->
      row >= last and :wxListCtrl.insertItem(grid, row, ~c"")
  ...
  defp create_mem_info(parent) do
    ...
    listItems = [
      {~c"Allocator Type", 0, 200},
      {~c"Block size (kB)", 1, 150},
      {~c"Carrier size (kB)", 1, 150},
      {~c"Max Carrier size (kB)", 1, 150}
    ]
