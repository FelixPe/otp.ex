defmodule :m_observer_port_wx do
  ...
  def init([notebook, parent, config]) do
    ...
    listItems = [
      {~c"Id", 0, scale * 150},
      {~c"Connected", 0, scale * 150},
      {~c"Name", 0, scale * 150},
      {~c"Controls", 0, scale * 200},
      {~c"Slot", 1, scale * 50}
    ]
  ...
  def handle_event(
    ...
    case index do
      ...
      _ ->
        port = :lists.nth(index + 1, ports)
        menu = :wxMenu.new()
        :wxMenu.append(menu, 303, ~c"Port info for " ++ :erlang.port_to_list(r_port(port, :id)))
        :wxMenu.append(menu, 305, ~c"Trace selected ports", [{:help, ~c"Trace selected ports"}])

        :wxMenu.append(menu, 306, ~c"Trace selected ports by name (all nodes)", [
          {:help,
           ~c"Trace selected ports, if a process have a registered name processes with same name will be traced on all nodes"}
        ])

        :wxMenu.append(menu, 309, ~c"Close " ++ :erlang.port_to_list(r_port(port, :id)))
    ...
    cond do
      available ->
        ...
        newOpened =
          case port do
            false ->
              send(self(), {:error, ~c"No such port: " ++ portIdStr})
              opened
  ...
  def handle_info(
    ...
    str = :io_lib.format(~c"Can not fetch port info from ~p.~nToo old OTP version.", [nodeName])
    :observer_lib.display_info_dialog(panel, str)
    {:noreply, state}
  end

  def handle_info({:error, error}, r_state(panel: panel) = state) do
    str = :io_lib.format(~c"ERROR: ~ts~n", [error])
    :observer_lib.display_info_dialog(panel, str)
    {:noreply, state}
  end
  ...
  defp create_menus(parent) do
    menuEntries = [
      {~c"View",
       [
         r_create_menu(id: 304, text: ~c"Port info for selected ports\tCtrl-I"),
         :separator,
      ...
      {~c"Trace",
       [
         r_create_menu(id: 305, text: ~c"Trace selected ports"),
         r_create_menu(id: 306, text: ~c"Trace selected ports by name (all nodes)"),
         r_create_menu(
           id: 307,
           text: ~c"Trace new ports"
         )
       ]}
    ]
  ...
  defp do_display_port_info(parent0, portRec) do
    ...
    title = ~c"Port Info: " ++ r_port(portRec, :id_str)
    scale = :observer_wx.get_scale()
    ...
    struct = [
      {~c"Overview",
       [
         ...
         {~c"Connected", {:click, :connected}},
         {~c"Slot", :slot},
         {~c"Controls", :controls},
         {~c"Parallelism", :parallelism},
         {~c"Locking", :locking},
         {~c"Queue Size", {:bytes, :queue_size}},
         {~c"Memory", {:bytes, :memory}}
       ]},
      {:scroll_boxes,
       [
         {~c"Links", 1, {:click, :links}},
         {~c"Monitors", 1, {:click, filter_monitor_info()}},
         {~c"Monitored by", 1, {:click, :monitored_by}}
       ]}
      | inetStruct
    ]
  ...
  defp inet_extra_fields(port) do
    ...
    struct =
      case :proplists.get_value(
        ...
        inet when inet == ~c"tcp_inet" or inet == ~c"udp_inet" or inet == ~c"sctp_inet" ->
          [
            {~c"Inet",
             [
               {~c"Local Address", {:inet, :local_address}},
               {~c"Local Port Number", :local_port},
               {~c"Remote Address", {:inet, :remote_address}},
               {~c"Remote Port Number", :remote_port}
             ]},
            {~c"Statistics",
  ...
  defp stat_name_and_unit(:recv_avg) do
    {~c"Average package size received", {:bytes, :recv_avg}}
  end

  defp stat_name_and_unit(:recv_cnt) do
    {~c"Number of packets received", :recv_cnt}
  end

  defp stat_name_and_unit(:recv_dvi) do
    {~c"Average packet size deviation received", {:bytes, :recv_dvi}}
  end

  defp stat_name_and_unit(:recv_max) do
    {~c"Largest packet received", {:bytes, :recv_max}}
  end

  defp stat_name_and_unit(:recv_oct) do
    {~c"Total received", {:bytes, :recv_oct}}
  end

  defp stat_name_and_unit(:send_avg) do
    {~c"Average packet size sent", {:bytes, :send_avg}}
  end

  defp stat_name_and_unit(:send_cnt) do
    {~c"Number of packets sent", :send_cnt}
  end

  defp stat_name_and_unit(:send_max) do
    {~c"Largest packet sent", {:bytes, :send_max}}
  end

  defp stat_name_and_unit(:send_oct) do
    {~c"Total sent", {:bytes, :send_oct}}
  end

  defp stat_name_and_unit(:send_pend) do
    {~c"Data waiting to be sent from driver", {:bytes, :send_pend}}
  end
