defmodule :m_observer_port_wx do
  ...
  Record.defrecord(:r_port, :port, id: :undefined,
                                connected: :undefined, name: :undefined,
                                controls: :undefined, slot: :undefined,
                                id_str: :undefined, links: :undefined,
                                monitors: :undefined, monitored_by: :undefined,
                                parallelism: :undefined, locking: :undefined,
                                queue_size: :undefined, memory: :undefined,
                                inet: :undefined)
  ...
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 ...
                                 ports: :undefined, timer: :undefined,
                                 open_wins: [])
  def start_link(notebook, parent, config) do
    :wx_object.start_link(:observer_port_wx,
  ...
  def init([notebook, parent, config]) do
    ...
    listItems = [{'Id', 0, scale * 150}, {'Connected', 0, scale * 150},
                                          {'Name', 0, scale * 150}, {'Controls', 0,
                                                                  scale * 200},
                                                                   {'Slot', 1,
                                                                      scale * 50}]
    ...
    {panel,
       r_state(grid: grid, parent: parent, panel: panel,
           timer: config, opt: opt)}
  end

  def handle_event(r_wx(id: 301),
           state = r_state(node: node, grid: grid, opt: opt)) do
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_activated,
                      itemIndex: index)),
           state = r_state(grid: grid, ports: ports,
                       open_wins: opened)) do
    cond do
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_right_click,
                      itemIndex: index)),
           state = r_state(panel: panel, ports: ports)) do
    case (index) do
      -1 ->
        {:noreply, state}
      _ ->
        port = :lists.nth(index + 1, ports)
        menu = :wxMenu.new()
        :wxMenu.append(menu, 303,
                         'Port info for ' ++ :erlang.port_to_list(r_port(port, :id)))
        :wxMenu.append(menu, 305, 'Trace selected ports', [{:help, 'Trace selected ports'}])
        :wxMenu.append(menu, 306, 'Trace selected ports by name (all nodes)', [{:help, 'Trace selected ports, if a process have a registered name processes with same name will be traced on all nodes'}])
        :wxMenu.append(menu, 309,
                         'Close ' ++ :erlang.port_to_list(r_port(port, :id)))
  ...
  def handle_event(r_wx(id: 303),
           state = r_state(grid: grid, right_clicked_port: port,
                       open_wins: opened)) do
    case (port) do
      :undefined ->
        {:noreply, state}
      _ ->
        newOpened = display_port_info(grid, port, opened)
        {:noreply,
           r_state(state, right_clicked_port: :undefined, 
                      open_wins: newOpened)}
    end
  end
  ...
  def handle_event(r_wx(id: 304),
    ...
    case (get_selected_items(grid, ports)) do
      [] ->
        :observer_wx.create_txt_dialog(r_state(state, :panel), 'No selected ports', 'Port Info',
                                         256)
        {:noreply, state}
      selected ->
        newOpened = :lists.foldl(fn p, o ->
  ...
  def handle_event(r_wx(id: 309),
           state = r_state(right_clicked_port: port)) do
    case (port) do
      :undefined ->
        {:noreply, state}
      _ ->
  ...
  def handle_event(r_wx(id: 305),
           r_state(grid: grid, ports: ports) = state) do
    case (get_selected_items(grid, ports)) do
      [] ->
        :observer_wx.create_txt_dialog(r_state(state, :panel), 'No selected ports', 'Tracer',
                                         256)
      selected ->
        selectedIds = (for port <- selected do
                         r_port(port, :id)
                       end)
    ...
    case (get_selected_items(grid, ports)) do
      [] ->
        :observer_wx.create_txt_dialog(r_state(state, :panel), 'No selected ports', 'Tracer',
                                         256)
      selected ->
        idsOrRegs = (for port <- selected do
  ...
  def handle_info({:portinfo_open, portIdStr},
           state = r_state(node: {activeNodeName, activeAvailable},
                       grid: grid, opt: opt, open_wins: opened)) do
    nodeName = node(:erlang.list_to_port(portIdStr))
    available = (case (nodeName) do
                   ^activeNodeName ->
                     activeAvailable
                   _ ->
    ...
    cond do
      available ->
        ...
        {:noreply,
           r_state(state, ports: ports,  open_wins: newOpened)}
      true ->
  ...
  def handle_info({:active, nodeName},
           state = r_state(parent: parent, grid: grid, opt: opt,
                       timer: timer0)) do
    ...
    {:noreply,
       r_state(state, node: {nodeName, available},  ports: ports, 
                  timer: timer)}
  end
  ...
  def handle_info({:info, {:port_info_not_available, nodeName}},
  ...
  defp create_menus(parent) do
    menuEntries = [{'View',
                      [r_create_menu(id: 304, text: 'Port info for selected ports\tCtrl-I'), :separator, r_create_menu(id: 301, text: 'Refresh\tCtrl-R'),
                                                            r_create_menu(id: 302,
                                                                text: 'Refresh Interval...')]},
                       {'Trace',
                          [r_create_menu(id: 305, text: 'Trace selected ports'), r_create_menu(id: 306, text: 'Trace selected ports by name (all nodes)'), r_create_menu(id: 307,
  ...
  defp list_to_portrec(pL) do
    ...
    r_port(id: portId, id_str: :erlang.port_to_list(portId),
  ...
  defp portrec_to_list(r_port(id: id, slot: slot, connected: connected,
              links: links, name: name, monitors: monitors,
              monitored_by: monitoredBy, controls: controls,
              parallelism: parallelism, locking: locking,
              queue_size: queueSize, memory: memory, inet: inet)) do
    [{:id, id}, {:slot, slot}, {:connected, connected},
                                   {:links, links}, {:name, name}, {:monitors,
                                                                      monitors},
                                                                       {:monitored_by,
                                                                          monitoredBy},
                                                                           {:controls,
  ...
  defp port_info_fields(port0) do
    ...
    struct = [{'Overview',
                 [{'Registered Name', :name}, {'Connected', {:click, :connected}}, {'Slot', :slot}, {'Controls',
                                                                        :controls},
                                                                         {'Parallelism',
                                                                            :parallelism},
                                                                             {'Locking',
                                                                                    ...
                                                                                    {:bytes,
                                                                                     ...
                                                                                     {'Memory',
                                                                                        {:bytes,
                                                                                           :memory}}]},
                  {:scroll_boxes,
                     [{'Links', 1, {:click, :links}}, {'Monitors', 1,
                                                   {:click,
                                                      filter_monitor_info()}},
                                                    {'Monitored by', 1,
                                                       {:click,
  ...
  defp inet_extra_fields(port) do
    ...
    struct = (case (:proplists.get_value(:controls,
                                           port)) do
                inet when inet == 'tcp_inet' or inet == 'udp_inet' or inet == 'sctp_inet' ->
                  [{'Inet',
                      [{'Local Address', {:inet, :local_address}}, {'Local Port Number', :local_port}, {'Remote Address',
                                                                          {:inet,
                                                                             :remote_address}},
                                                                           {'Remote Port Number',
                                                                              :remote_port}]},
  ...
  defp update_grid2(grid, sel,
    ...
    update = fn r_port(id: id, slot: slot, connected: connected,
                    name: name, controls: ctrl),
                  row ->
  ...
  defp portinfo_available(nodeName) do
    ...
    case (:rpc.call(nodeName, :erlang, :function_exported,
                      [:observer_backend, :get_port_list, 0])) do
      true ->
        true
      false ->
        false
  ...
  defp popup_unavailable_info(nodeName) do
    send(self(), {:info,
                    {:port_info_not_available, nodeName}})
    :ok
  end
