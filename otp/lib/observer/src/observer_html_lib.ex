defmodule :m_observer_html_lib do
  ...
  def expandable_term(heading, expanded, tab, colors0) do
    colors = convert(colors0)
    header(heading,
             body(expandable_term_body(heading, expanded, tab,
                                         colors),
                    colors))
  end
  ...
  defp expandable_term_body(heading, expanded, tab, colors) do
    ...
    [case (heading) do
       'MsgQueue' ->
         table(attr,
                 [tr([th('WIDTH=70%', 'Message'), th('WIDTH=30%', 'SeqTraceToken')]) | :erlang.element(1,
                                                               :lists.mapfoldl(fn msg,
                                                                                    even ->
                                                                                    {msgq_table(tab,
                                                                                                  msg,
                                                                                                  even,
       ...
       'Message Queue' ->
         table(attr,
                 [tr([th('WIDTH=10%', 'Id'), th('WIDTH=90%', 'Message')]) | :erlang.element(1,
                                                               :lists.mapfoldl(fn msg,
                                                                                    {even,
                                                                                       n} ->
                                                                                    {msgq_table(tab,
       ...
       'StackDump' ->
         table(attr,
                 [tr([th('WIDTH=20%', 'Label'), th('WIDTH=80%', 'Term')]) | :erlang.element(1,
                                                               :lists.mapfoldl(fn entry,
                                                                                    even ->
                                                                                    {stackdump_table(tab,
                                                                                                       entry,
                                                                                                       even,
       ...
       'ProcState' ->
         table(attr,
                 [tr([th('WIDTH=20%', 'Label'), th('WIDTH=80%', 'Information')]) | :erlang.element(1,
                                                               :lists.mapfoldl(fn entry,
                                                                                    even ->
                                                                                    {proc_state(tab,
                                                                                                  entry,
       ...
       _ ->
         table(attr,
                 [tr([th('WIDTH=30%', 'Key'), th('WIDTH=70%', 'Value')]) | :erlang.element(1,
                                                               :lists.mapfoldl(fn entry,
                                                                                    even ->
                                                                                    {dict_table(tab,
                                                                                                  entry,
  ...
  defp all_or_expand(_Tab, term, str, false) when not
                                       is_binary(term) do
    href_proc_port(:lists.flatten(str))
  end

  defp all_or_expand(tab, term, preview, true) when not
                                         is_binary(term) do
    key = ({key1, key2,
              key3} = {:erlang.unique_integer([:positive]), 1, 2})
    :ets.insert(tab, {key, term})
    [href_proc_port(:lists.flatten(preview), false), ?\n,
                                                         href('TARGET="expanded"',
                                                                ['#Term?key1=' ++ :erlang.integer_to_list(key1) ++ '&key2=' ++ :erlang.integer_to_list(key2) ++ '&key3=' ++ :erlang.integer_to_list(key3)],
  ...
  defp html_header(title, javaScript, body) do
    [start_html(), only_html_header(title, javaScript),
                       body, stop_html()]
  end
  ...
  defp href_proc_port('<' ++ ([c | _] = t), acc, lTB) when (?0 <= c and
                                                c <= ?9) do
    {pid0, rest} = split(?>, t)
    pid = '&lt;' ++ pid0 ++ '&gt'
  ...
  defp href_proc_port('[\'#CDVPort\'' ++ t, acc, lTB) do
    {port0, rest} = split(?], t)
    portStr = (case (:string.lexemes(port0, ',.|')) do
                 [x, y] ->
                   port = '#Port&lt;' ++ x ++ '.' ++ y ++ '&gt;'
                   href(port, port)
                 ns ->
  ...
  defp href_proc_port('[\'#CDVPid\'' ++ t, acc, lTB) do
    {pid0, rest} = split(?], t)
    pidStr = (case (:string.lexemes(pid0, ',.|')) do
                [x, y, z] ->
                  pid = '&lt;' ++ x ++ '.' ++ y ++ '.' ++ z ++ '&gt;'
                  href(pid, pid)
                ns ->
  ...
  defp href_proc_port('\'#CDVTruncatedBinary\'' ++ t, acc, lTB) do
    iH = :lists.reverse(:lists.flatten('<FONT COLOR="#FF0000">&lt;&lt;...(Truncated Binary)&gt;&gt;</FONT>'))
    href_proc_port(t, iH ++ acc, lTB)
  end

  defp href_proc_port('\'#CDVNonexistingBinary\'' ++ t, acc, lTB) do
    iH = :lists.reverse(:lists.flatten('<FONT COLOR="#FF0000">&lt;&lt;...(Nonexisting Binary)&gt;&gt;</FONT>'))
    href_proc_port(t, iH ++ acc, lTB)
  end
                ...
                [offset, sizeStr, pos] when from === :cdv ->
                  ...
                  case (:crashdump_viewer.expand_binary(id)) do
                    ...
                    {:ok, previewBin} ->
                      ...
                      cond do
                        lTB ->
                          href('TARGET="expanded"', ['#Binary?offset=' ++ offset ++ '&size=' ++ sizeStr ++ '&pos=' ++ pos],
                                 previewStr)
                        true ->
                          previewStr
                      end
  ...
  defp convert(r_colors(fg: {fR, fB, fG}, even: {eR, eB, eG},
              odd: {oR, oG, oB})) do
    r_colors(fg: :io_lib.format('"#~2.16.0B~2.16.0B~2.16.0B"', [fR, fB, fG]),
