defmodule :m_cdv_html_wx do
  ...
  defp init(parentWin, callback) do
    {htmlWin, state} = init(parentWin, ~c"", :undefined, :cdv)
  ...
  def handle_info(
    ...
    :observer_lib.display_progress_dialog(htmlWin, ~c"Crashdump Viewer", ~c"Reading data")
  ...
  def handle_info(info, state) do
    :io.format(~c"~p:~p: Unhandled info: ~tp~n", [:cdv_html_wx, 97, info])
    {:noreply, state}
  end
  ...
  def handle_call(msg, _From, state) do
    :io.format(~c"~p~p: Unhandled Call ~tp~n", [:cdv_html_wx, 107, msg])
    {:reply, :ok, state}
  end
  ...
  def handle_cast(msg, state) do
    :io.format(~c"~p~p: Unhandled cast ~tp~n", [:cdv_html_wx, 115, msg])
    {:noreply, state}
  end
  ...
  def handle_event(
    ...
    newState =
      case target do
        ~c"#Binary?" ++ binSpec ->
          [{~c"offset", off}, {~c"size", size}, {~c"pos", pos}] =
        ...
        ~c"#OBSBinary?" ++ binSpec ->
          [{~c"key1", preview}, {~c"key2", size}, {~c"key3", hash}] =
            :uri_string.dissect_query(binSpec)
