defmodule :m_cdv_port_cb do
  ...
  def get_details(id, _Data) do
    case :crashdump_viewer.port(id) do
      ...
      {:error, {:other_node, nodeId}} ->
        info =
          ~c"The port you are searching for was residing on a remote node. No port information is available. Show information about the remote node?"
      ...
      {:error, :not_found} ->
        info = ~c"The port you are searching for could not be found."
        {:info, info}
    end
  end
  ...
  def format({i1, i2}) do
    ~c"#Port<" ++ :erlang.integer_to_list(i1) ++ ~c"." ++ :erlang.integer_to_list(i2) ++ ~c">"
  end
    ...
    [
      {~c"Overview",
       [
         {~c"Name", :name},
         {~c"State", :state},
         {~c"Task Flags", :task_flags},
         {~c"Connected", {:click, :connected}},
         {~c"Slot", :slot},
         {~c"Controls", :controls},
         {~c"Input bytes", :input},
         {~c"Output bytes", :output},
         {~c"Queue bytes", :queue},
         {~c"Port data", :port_data}
       ]},
      {:scroll_boxes,
       [
         {~c"Links", 1, {:click, :links}},
         {~c"Monitors", 1, {:click, :monitors}},
         {~c"Suspended", 1, {:click, :suspended}}
       ]}
    ]
  end
end
