defmodule :m_cdv_port_cb do
  ...
  def col_spec() do
    [{'Id', 0, 100}, {'Connected', 0, 120}, {'Name', 0, 150}, {'Controls', 0, 200}, {'Queue',
                                                            1, 100},
                                                             {'Slot', 1, 50}]
  end
  ...
  def get_details(id, _Data) do
    case (:crashdump_viewer.port(id)) do
      {:ok, info, tW} ->
        proplist = :crashdump_viewer.to_proplist(Keyword.keys(r_port(r_port())),
                                                   info)
        {:ok, {id, proplist, tW}}
      {:error, {:other_node, nodeId}} ->
        info = 'The port you are searching for was residing on a remote node. No port information is available. Show information about the remote node?'
        fun = fn () ->
  ...
  defp info_fields() do
    [{'Overview',
        [{'Name', :name}, {'State', :state}, {'Task Flags', :task_flags}, {'Connected',
                                                       {:click, :connected}},
                                                        {'Slot', :slot}, {'Controls',
                                                                       :controls},
                                                                        {'Input bytes',
                                                                                ...
                                                                                {'Queue bytes',
                                                                                   :queue},
                                                                                    {'Port data',
                                                                                       :port_data}]},
         {:scroll_boxes,
            [{'Links', 1, {:click, :links}}, {'Monitors', 1, {:click, :monitors}},
