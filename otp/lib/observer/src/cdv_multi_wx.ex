defmodule :m_cdv_multi_wx do
  ...
  Record.defrecord(:r_state, :state, main_panel: :undefined,
                                 main_sizer: :undefined, menu: :undefined,
                                 menu_sizer: :undefined, callback: :undefined,
                                 pages: :undefined, dyn_panel: :undefined,
                                 dyn_sizer: :undefined, dyn_page: :undefined)
  ...
  def init([notebook, pages]) do
    ...
    leftMenuSizer = :wxStaticBoxSizer.new(8, mainPanel,
                                            [{:label, 'Please select'}])
    leftMenu = :wxListBox.new(mainPanel, - 1,
                                [{:style, 32}, {:choices,
                                                  for {t, _, _} <- pages do
                                                    t
                                                  end}])
    ...
    :wxSizer.add(leftMenuSizer, leftMenu,
    ...
    :wxSizer.add(sizer, dynPanel,
                   [{:flag, 8192 ||| borderFlags ||| 64}, {:proportion, 1},
                                                              {:border, 5}])
    :wxPanel.setSizer(mainPanel, sizer)
    state = load_dyn_page(r_state(main_panel: mainPanel,
                              main_sizer: sizer, menu: leftMenu,
                              menu_sizer: leftMenuSizer, pages: pages,
                              dyn_panel: dynPanel))
    {mainPanel, state}
  end

  def handle_info(:active, state) do
    newState = :wx.batch(fn () ->
  ...
  defp update_left_menu(r_state(main_panel: panel, callback: callback,
              menu: oldMenu, menu_sizer: menuSizer) = state) do
    pages = callback.get_info()
    :wxListBox.disconnect(oldMenu)
    :wxWindow.destroy(oldMenu)
    newMenu = :wxListBox.new(panel, - 1,
                               [{:style, 32}, {:choices,
                                                 for {t, _, _} <- pages do
                                                   t
                                                 end}])
  ...
  defp update_dyn_page(r_state(dyn_page: oldDynPage,
              dyn_sizer: oldDynSizer) = state) do
    :wxSizer.detach(oldDynSizer, oldDynPage)
    :wxWindow.destroy(oldDynPage)
    load_dyn_page(state)
  end

  defp load_dyn_page(r_state(main_sizer: mainSizer, dyn_panel: dynPanel,
              menu: menu, pages: pages) = state) do
    doFreeze = [:wxe_util.get_const(:wxMAJOR_VERSION),
                    :wxe_util.get_const(:wxMINOR_VERSION)] < [2, 9]
