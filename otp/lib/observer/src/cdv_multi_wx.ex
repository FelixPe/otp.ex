defmodule :m_cdv_multi_wx do
  ...
  def init([notebook, pages]) do
    ...
    leftMenuSizer = :wxStaticBoxSizer.new(8, mainPanel, [{:label, ~c"Please select"}])
  ...
  def handle_info(info, state) do
    :io.format(~c"~p:~p: Unhandled info: ~tp~n", [:cdv_multi_wx, 97, info])
    {:noreply, state}
  end
  ...
  def handle_call(msg, _From, state) do
    :io.format(~c"~p:~p: Unhandled Call ~tp~n", [:cdv_multi_wx, 115, msg])
    {:reply, :ok, state}
  end

  def handle_cast(msg, state) do
    :io.format(~c"~p:~p: Unhandled cast ~tp~n", [:cdv_multi_wx, 119, msg])
    {:noreply, state}
  end
