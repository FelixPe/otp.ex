defmodule :m_multitrace do
    ...
    case running() do
      false ->
        {:ok, _} =
          :ttb.tracer(
            :all,
            [{:file, ~c"debug_trace"}, {:handler, {{:multitrace, :handle_debug}, :initial}}]
          )
  ...
  defp print_func(out, {:trace_ts, p, :call, {m, f, a}, c, ts}, n) do
    :io.format(
      out,
      ~c"~w: ~s~nProcess   : ~w~nCall      : ~w:~tw/~w~nArguments : ~tp~nCaller    : ~tw~n~n",
  ...
  defp print_func(out, {:trace_ts, p, :return_from, {m, f, a}, r, ts}, n) do
    :io.format(
      out,
      ~c"~w: ~s~nProcess      : ~w~nReturn from  : ~w:~tw/~w~nReturn value : ~tp~n~n",
      [n, ts(ts), p, m, f, a, r]
    )
  end
  ...
  def handle_gc(out, {:trace_ts, p, :gc_end, info, ts}, _TI, s) do
    case :dict.find(p, s) do
      {:ok, {startInfo, startTime}} ->
        ...
        :io.format(
          out,
          ~c"start\t~w\t~w\t~w\t~w\t~w\t~w\t~w\t~w~nend\t~w\t~w\t~w\t~w\t~w\t~w\t~w\t~w~n~n",
          [
            sM,
            sR,
            sS,
  ...
  defp print_gc_header(out) do
    :io.format(
      out,
      ~c"\tMBuf\tRecent\tStack\tOldHeap\tHeap\tOldBL\tBlock\tProcess/Time(micro sec)~n=========================================================================================~n",
      []
    )
  end
  ...
  def handle_schedule(out, {:trace_ts, p, :out, info, ts}, _TI, s) do
    :io.format(out, ~c"out:~nProcess  : ~w~nTime     : ~s~nFunction : ~tw~n~n", [p, ts(ts), info])
  ...
  def handle_schedule(out, {:trace_ts, p, :in, info, ts}, _TI, s) do
    :io.format(out, ~c"in:~nProcess  : ~w~nTime     : ~s~nFunction : ~tw~n~n", [p, ts(ts), info])
  ...
  defp summary(out, [{p, list} | rest]) do
    ...
    :io.format(out, ~c"Total time 'in' for process ~w: ~w micro seconds~n", [p, sum])
    summary(out, rest)
  end
  ...
  defp ts({_, _, micro} = now) do
    ...
    :io_lib.format(~c"~4.4.0w-~2.2.0w-~2.2.0w ~2.2.0w:~2.2.0w:~2.2.0w,~6.6.0w", [
      y,
      m,
      d,
      h,
  ...
  defp print_header(out, tI) do
    ...
    case :lists.keysearch(:start_time, 1, tI) do
      {:value, {:start_time, [sT]}} ->
        :io.format(out, ~c"~nTracing started on node ~w at ~s~nFlags: ~p~n~n~n", [
          node,
          ts(sT),
          flags
        ])
      ...
      false ->
        :io.format(out, ~c"~nTracing from node ~w~nFlags: ~p~n~n~n", [node, flags])
    end
  end
