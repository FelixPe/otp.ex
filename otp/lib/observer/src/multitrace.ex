defmodule :m_multitrace do
  ...
  defp print_func(out, {:trace_ts, p, :call, {m, f, a}, c, ts},
            n) do
    :io.format(out, '~w: ~s~nProcess   : ~w~nCall      : ~w:~tw/~w~nArguments : ~tp~nCaller    : ~tw~n~n',
                 [n, ts(ts), p, m, f, length(a), a, c])
  end

  defp print_func(out,
            {:trace_ts, p, :return_from, {m, f, a}, r, ts}, n) do
    :io.format(out, '~w: ~s~nProcess      : ~w~nReturn from  : ~w:~tw/~w~nReturn value : ~tp~n~n', [n, ts(ts), p, m, f, a, r])
  end
  ...
  def handle_gc(_Out, {:trace_ts, p, :gc_start, info, ts}, _TI,
           s) do
    :dict.store(p, {info, ts}, s)
  end

  def handle_gc(out, {:trace_ts, p, :gc_end, info, ts}, _TI,
           s) do
    case (:dict.find(p, s)) do
      {:ok, {startInfo, startTime}} ->
        {eM, eR, eS, eO, eH, eOB, eB} = sort(info)
        {sM, sR, sS, sO, sH, sOB, sB} = sort(startInfo)
        :io.format(out, 'start\t~w\t~w\t~w\t~w\t~w\t~w\t~w\t~w~nend\t~w\t~w\t~w\t~w\t~w\t~w\t~w\t~w~n~n',
                     [sM, sR, sS, sO, sH, sOB, sB, p, eM, eR, eS, eO, eH,
                                                                          eOB,
                                                                              eB,
                                                                                  diff(startTime,
                                                                                         ts)])
  ...
  defp print_gc_header(out) do
    :io.format(out, '\tMBuf\tRecent\tStack\tOldHeap\tHeap\tOldBL\tBlock\tProcess/Time(micro sec)~n=========================================================================================~n', [])
  end
  ...
  defp sort([{:mbuf_size, m} | rest],
            {_, r, s, o, h, oB, b}) do
    sort(rest, {m, r, s, o, h, oB, b})
  end

  defp sort([{:recent_size, r} | rest],
            {m, _, s, o, h, oB, b}) do
    sort(rest, {m, r, s, o, h, oB, b})
  end

  defp sort([{:stack_size, s} | rest],
            {m, r, _, o, h, oB, b}) do
    sort(rest, {m, r, s, o, h, oB, b})
  end

  defp sort([{:old_heap_size, o} | rest],
            {m, r, s, _, h, oB, b}) do
    sort(rest, {m, r, s, o, h, oB, b})
  end

  defp sort([{:heap_size, h} | rest],
            {m, r, s, o, _, oB, b}) do
    sort(rest, {m, r, s, o, h, oB, b})
  end

  defp sort([{:old_heap_block_size, oB} | rest],
            {m, r, s, o, h, _, b}) do
    sort(rest, {m, r, s, o, h, oB, b})
  end

  defp sort([{:heap_block_size, b} | rest],
            {m, r, s, o, h, oB, _}) do
    sort(rest, {m, r, s, o, h, oB, b})
  end
  ...
  defp ts({_, _, micro} = now) do
    {{y, m, d},
       {h, min, s}} = :calendar.now_to_local_time(now)
    :io_lib.format('~4.4.0w-~2.2.0w-~2.2.0w ~2.2.0w:~2.2.0w:~2.2.0w,~6.6.0w', [y, m, d, h, min, s, micro])
  end
  ...
  defp init() do
    :ttb.write_trace_info(:start_time,
                            fn () ->
                                 :erlang.now()
                            end)
  end
  ...
  defp print_header(out, tI) do
    {:value, {:node, [node]}} = :lists.keysearch(:node, 1,
                                                   tI)
    {:value, {:flags, flags}} = :lists.keysearch(:flags, 1,
                                                   tI)
    case (:lists.keysearch(:start_time, 1, tI)) do
      {:value, {:start_time, [sT]}} ->
        :io.format(out, '~nTracing started on node ~w at ~s~nFlags: ~p~n~n~n', [node, ts(sT), flags])
      false ->
