defmodule :m_observer_perf_wx do
  ...
  def interval_dialog(parent0, r_ti(fetch: fetch0, secs: secs0) = ti) do
    ...
    flags = [{:flag, 8192 ||| 64 ||| 16 ||| 32}, {:border,
                                                    5},
                                                     {:proportion, 1}]
  ...
  def precalc(ti, {noSamples, q}, paint,
    ...
    case (win) do
      ...
      _SomeThingChanged ->
        ...
        %{scale: {wS, hS}} = (props = window_geom(size, maxDisp,
                                                    ti, panel, paint))
        noGraphs = (try do
                      tuple_size(hd(hs))
                    catch
  ...
  defp window_geom({w, h}, {_, max, _Unit, maxUnit},
    ...
    %{p0: {x0, y0}, p1: {x1, y1}, scale: {scaleW, scaleH},
        txsz: {tW, tH, spaceW}, txt: {bottomTextY, maxTextY},
        strs: {str1, str2, str3}}
  end
  ...
  defp strip([{x, _} | rest], drop, n) when (x > 1.0e-4 and
                                            n > 0) do
    strip(rest, drop, n)
  end
