defmodule :m_observer_tv_wx do
  ...
  def init([notebook, parent, config]) do
    ...
    listItems = [
      {~c"Table Name", 0, scale * 200},
      {~c"Objects", 1, scale * 100},
      {~c"Size (kB)", 1, scale * 100},
      {~c"Owner Pid", 2, scale * 150},
      {~c"Owner Name", 0, scale * 200},
      {~c"Table Id", 0, scale * 250}
    ]
  ...
  def handle_event(
    ...
    case get_table(holder, index) do
      r_tab(protection: :private) ->
        send(self(), {:error, ~c"Table has 'private' protection and cannot be read"})
  ...
  def handle_event(
    ...
    menu = :wxMenu.new()
    :wxMenu.append(menu, 407, ~c"Table info")
    :wxMenu.append(menu, 408, ~c"Show Table Content")
  ...
  def handle_info({:refresh, min, min}, state = r_state(grid: grid)) do
    ...
    :observer_wx.set_status(:io_lib.format(~c"Tables: ~w", [min + 1]))
    {:noreply, state}
  end
  ...
  def handle_info({:refresh, min, max}, state = r_state(grid: grid)) do
    ...
    :observer_wx.set_status(:io_lib.format(~c"Tables: ~w", [max + 1]))
    {:noreply, state}
  end
  ...
  defp create_menus(
    ...
    menuEntries = [
      {~c"View",
       [
         r_create_menu(id: 407, text: ~c"Table information\tCtrl-I"),
         :separator,
         r_create_menu(id: 403, text: ~c"&Ets Tables", type: :radio, check: type == :ets),
         r_create_menu(id: 404, text: ~c"&Mnesia Tables", type: :radio, check: type == :mnesia),
         :separator,
         r_create_menu(
           id: 405,
           text: ~c"View &Unreadable Tables",
           type: :check,
           check: not unR
         ),
         r_create_menu(
           id: 406,
           text: ~c"View &System Tables",
           type: :check,
           check: not sys
         ),
         :separator,
  ...
  def display_table_info(parent0, node, source, table) do
    ...
    title = ~c"Table Info: " ++ :erlang.atom_to_list(r_tab(table, :name))
    frame = :wxMiniFrame.new(parent, -1, title, [{:style, 2048 ||| 536_870_912 ||| 4096 ||| 64}])

    idInfo =
      {~c"Identification and Owner",
       [
         {~c"Name", r_tab(table, :name)},
         {~c"Id",
         ...
         {~c"Named table", r_tab(table, :id) == :ignore},
         {~c"Owner", r_tab(table, :owner)},
         {~c"Owner Name",
      ...
      case source do
        ...
        :mnesia ->
          [
            ...
            {~c"Index positions", list_to_strings(r_tab(table, :index))}
          ]
      end
    ...
    settings =
      {~c"Settings",
       [
         {~c"Source", source},
         {~c"Key Position", r_tab(table, :keypos)},
         {~c"Table Type", r_tab(table, :type)},
         {~c"Protection Mode", r_tab(table, :protection)},
         {~c"Fixed", r_tab(table, :fixed)}
         | mnesiaSettings
       ]}
    ...
    memory =
      {~c"Memory Usage",
       [
         {~c"Number of objects", r_tab(table, :size)},
         {~c"Memory allocated", {:bytes, r_tab(table, :memory)}},
         {~c"Compressed", r_tab(table, :compressed)}
       ]}
  ...
  defp list_to_strings([a | b]) do
    :erlang.integer_to_list(a) ++ ~c" ," ++ list_to_strings(b)
  end
