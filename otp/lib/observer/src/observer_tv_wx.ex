defmodule :m_observer_tv_wx do
  ...
  Record.defrecord(:r_opts, :opts, type: :ets,
                                sys_hidden: true, unread_hidden: true)
  Record.defrecord(:r_sort, :sort, sort_incr: true,
                                sort_key: 2)
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 grid: :undefined, panel: :undefined,
                                 node: node(), opts: :EFE_TODO_NESTED_RECORD,
                                 holder: :undefined, selected: :undefined,
                                 timer: :undefined)
  def start_link(notebook, parent, config) do
    :wx_object.start_link(:observer_tv_wx,
  ...
  def init([notebook, parent, config]) do
    ...
    opts = r_opts(type: :maps.get(:type, config, :ets),
    ...
    listItems = [{'Table Name', 0, scale * 200}, {'Objects', 1, scale * 100},
                                          {'Size (kB)', 1, scale * 100}, {'Owner Pid', 2,
                                                                  scale * 150},
                                                                   {'Owner Name', 0,
                                                                      scale * 200},
                                                                       {'Table Id', 0,
    ...
    {panel,
       r_state(grid: grid, parent: parent, panel: panel, opts: opts,
           timer: config, holder: holder)}
  end

  def handle_event(r_wx(id: 401),
           state = r_state(holder: holder, node: node, opts: opts)) do
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_col_click,
                      col: col)),
           state = r_state(holder: holder)) do
    send(holder, {:sort, col})
    {:noreply, state}
  end

  def handle_event(r_wx(id: id),
           state = r_state(node: node, holder: holder, grid: grid,
                       opts: opt0))
    ...
    case (get_tables2(node, opt)) do
      error = {:error, _} ->
        id === 404 and :wxMenuBar.check(:observer_wx.get_menubar(),
                                          403, true)
        send(self(), error)
        {:noreply, state}
      tables ->
  ...
  def handle_event(r_wx(event: r_wxList(type: :command_list_item_activated,
                      itemIndex: index)),
           state = r_state(holder: holder, node: node,
                       opts: r_opts(type: type), grid: grid)) do
    case (get_table(holder, index)) do
      r_tab(protection: :private) ->
        send(self(), {:error, 'Table has \'private\' protection and cannot be read'})
      r_tab() = table ->
        :observer_tv_table.start_link(grid,
                                        [{:node, node}, {:type, type}, {:table,
                                                                          table}])
      _ ->
        :ignore
    end
  ...
  def handle_event(r_wx(id: 407),
           state = r_state(holder: holder, grid: grid, node: node,
                       opts: r_opts(type: type), selected: sel)) do
    case (sel) do
      :undefined ->
        {:noreply, state}
      r when is_integer(r) ->
  ...
  def handle_call(:get_config, _,
           r_state(timer: timer, opts: opt) = state) do
    r_opts(type: type, sys_hidden: sys,
        unread_hidden: unread) = opt
    conf0 = :observer_lib.timer_config(timer)
    conf = Map.merge(conf0, %{type: type, sys_hidden: sys,
                                unread_hidden: unread})
    {:reply, conf, state}
  end
  ...
  def handle_info(:refresh_interval,
           state = r_state(holder: holder, node: node, opts: opt)) do
  ...
  def handle_info({:active, node},
           state = r_state(parent: parent, holder: holder, grid: grid,
                       opts: opt0, timer: timer0)) do
    {tables,
       opt} = (case (r_opts(opt0, :type) === :mnesia and get_tables2(node,
                                                                  opt0)) do
                 ts when is_list(ts) ->
                   {ts, opt0}
                 _ ->
    ...
    {:noreply,
       r_state(state, node: node,  timer: timer,  opts: opt)}
  end
  ...
  def handle_info({:error, error},
           r_state(panel: panel, opts: opt) = state) do
  ...
  def handle_info({:selected, new, size},
           r_state(grid: grid, selected: old) = state) do
    cond do
  ...
  defp create_menus(parent,
            r_opts(sys_hidden: sys, unread_hidden: unR, type: type)) do
    menuEntries = [{'View',
                      [r_create_menu(id: 407, text: 'Table information\tCtrl-I'), :separator, r_create_menu(id: 403, text: '&Ets Tables',
                                                            type: :radio,
                                                            check: type == :ets),
                                                            r_create_menu(id: 404, text: '&Mnesia Tables',
                                                                type: :radio,
                                                                check: type == :mnesia),
                                                                :separator,
                                                                    r_create_menu(id: 405,
                                                                        text: 'View &Unreadable Tables',
                                                                        type: :check,
                                                                        check: not
                                                                               unR),
                                                                        r_create_menu(id: 406,
                                                                            text: 'View &System Tables',
                                                                            type: :check,
                                                                            check: not
                                                                                   sys),
                                                                            :separator,
                                                                                r_create_menu(id: 401,
                                                                                    text: 'Refresh\tCtrl-R'),
                                                                                    r_create_menu(id: 402,
  ...
  defp get_tables2(node,
            r_opts(type: type, sys_hidden: sys,
                unread_hidden: unread)) do
    args = [type, [{:sys_hidden, sys}, {:unread_hidden,
                                          unread}]]
    case (:rpc.call(node, :observer_backend,
                      :get_table_list, args)) do
      {:badrpc, error} ->
        {:error, error}
      error = {:error, _} ->
        error
  ...
  defp list_to_tabrec(pL) do
    r_tab(name: :proplists.get_value(:name, pL),
  ...
  def display_table_info(parent0, node, source, table) do
    ...
    idInfo = {'Identification and Owner',
                [{'Name', r_tab(table, :name)}, {'Id',
                                          case (r_tab(table, :id)) do
                                           ...
                                           {'Named table', r_tab(table, :id) == :ignore}, {'Owner',
                                                                             r_tab(table, :owner)},
                                                                              {'Owner Name',
    ...
    settings = {'Settings',
                  [{'Source', source}, {'Key Position', r_tab(table, :keypos)}, {'Table Type',
                                                           r_tab(table, :type)},
                                                            {'Protection Mode',
                                                               r_tab(table, :protection)},
                                                                {'Fixed',
                                                                   r_tab(table, :fixed)} |
                                                                    mnesiaSettings]}
    memory = {'Memory Usage',
                [{'Number of objects', r_tab(table, :size)}, {'Memory allocated', {:bytes, r_tab(table, :memory)}},
                                           {'Compressed', r_tab(table, :compressed)}]}
    {_, sizer, _} = :observer_lib.display_info(frame,
  ...
  defp table_holder(s0 = r_holder(parent: parent, tabs: tabs0,
                   sel: sel0)) do
    receive do
      {:get_attr, from, row} ->
      ...
      {:refresh, tabs1} ->
        sTab = get_sel(sel0, tabs0)
        tabs = (case (r_holder(s0, :sort)) do
                  r_sort(sort_incr: false, sort_key: col) ->
                    :array.from_list(:lists.reverse(:lists.keysort(col,
                                                                     tabs1)))
                  r_sort(sort_key: col) ->
  ...
  defp sort(col,
            r_holder(sort: r_sort(sort_key: col, sort_incr: incr) = s,
                tabs: table0) = h) do
    table = :lists.reverse(:array.to_list(table0))
    r_holder(h, sort: r_sort(s, sort_incr: not incr), 
           tabs: :array.from_list(table))
  end

  defp sort(col,
            r_holder(sort: r_sort(sort_incr: incr) = s, tabs: table0) = h) do
    table = (case (incr) do
               false ->
                 :lists.reverse(:lists.keysort(col,
                                                 :array.to_list(table0)))
               true ->
                 :lists.keysort(col, :array.to_list(table0))
             end)
    r_holder(h, sort: r_sort(s, sort_key: col), 
           tabs: :array.from_list(table))
  end
  ...
  defp get_col(2, r_tab() = rec) do
    :observer_lib.to_str(div(:erlang.element(r_tab(:memory),
                                               rec), 1024))
  end
