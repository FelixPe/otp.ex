defmodule :m_cdv_term_cb do
  ...
  defp init_term_page(parentWin, {type, [term, tab]}) do
    ...
    :cdv_multi_wx.start_link(parentWin,
                               [{'Format ~p', :cdv_html_wx,
                                   {type,
                                      format_term_fun('~p', binSaved, tab, cs)}},
                                    {'Format ~tp', :cdv_html_wx,
                                       {type,
                                          format_term_fun('~tp', binSaved, tab,
                                                            cs)}},
                                        {'Format ~w', :cdv_html_wx,
                                           {type,
                                              format_term_fun('~w', binSaved, tab,
                                                                cs)}},
                                            {'Format ~tw', :cdv_html_wx,
                                               {type,
                                                  format_term_fun('~tw', binSaved,
                                                                    tab, cs)}},
  ...
  defp expand([:"#CDVBin", offset, size, pos], true) do
    {:ok, bin} = :crashdump_viewer.expand_binary({offset,
                                                    size, pos})
    bin
  end

  defp expand(bin, tab) when (is_binary(bin) and
  ...
  defp expand(tuple, expand) when is_tuple(tuple) do
    :erlang.list_to_tuple(expand(:erlang.tuple_to_list(tuple),
                                   expand))
  end
