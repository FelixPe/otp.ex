defmodule :m_cdv_bin_cb do
  ...
  defp init_bin_page(parent, {type, bin}) do
    ...
    :cdv_multi_wx.start_link(parent,
                               [{'Format ~p', :cdv_html_wx,
                                   {type, format_bin_fun('~p', bin, cs)}},
                                    {'Format ~tp', :cdv_html_wx,
                                       {type, format_bin_fun('~tp', bin, cs)}},
                                        {'Format ~w', :cdv_html_wx,
                                           {type, format_bin_fun('~w', bin, cs)}},
                                            {'Format ~tw', :cdv_html_wx,
                                               {type,
                                                  format_bin_fun('~tw', bin, cs)}},
                                                {'Format ~s', :cdv_html_wx,
                                                   {type,
                                                      format_bin_fun('~s', bin,
                                                                       cs)}},
                                                    {'Format ~ts', :cdv_html_wx,
                                                       {type,
                                                          format_bin_fun('~ts', bin,
                                                                           cs)}},
                                                        {'Hex', :cdv_html_wx,
                                                           {type,
                                                              hex_binary_fun(bin,
  ...
  defp format_hex(<<b1 :: size(4), b2 :: size(4), bin :: binary>>,
            0) do
    [:erlang.integer_to_list(b1, 16),
         :erlang.integer_to_list(b2, 16), ?,, ?\n, ?\s, ?\s |
                                                            format_hex(bin, 25)]
  ...
  defp format_hex(<<b1 :: size(4), b2 :: size(4), bin :: binary>>,
            n) do
    [:erlang.integer_to_list(b1, 16),
         :erlang.integer_to_list(b2, 16), ?, | format_hex(bin,
                                                            n - 1)]
  end
