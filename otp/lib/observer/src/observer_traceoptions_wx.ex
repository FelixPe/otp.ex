defmodule :m_observer_traceoptions_wx do
  ...
  def process_trace(parent, default) do
    dialog =
      :wxDialog.new(parent, -1, ~c"Process Options", [
    ...
    leftSz = :wxStaticBoxSizer.new(8, panel, [{:label, ~c"Tracing options"}])
    rightSz = :wxStaticBoxSizer.new(8, panel, [{:label, ~c"Inheritance options:"}])
    funcBox = :wxCheckBox.new(panel, -1, ~c"Trace function call", [])
    check_box(funcBox, :lists.member(:functions, default))
    arityBox = :wxCheckBox.new(panel, -1, ~c"Trace arity instead of arguments", [])
    check_box(arityBox, :lists.member(:functions, default))
    sendBox = :wxCheckBox.new(panel, -1, ~c"Trace send message", [])
    check_box(sendBox, :lists.member(:send, default))
    recBox = :wxCheckBox.new(panel, -1, ~c"Trace receive message", [])
    check_box(recBox, :lists.member(:receive, default))
    eventBox = :wxCheckBox.new(panel, -1, ~c"Trace process events", [])
    check_box(eventBox, :lists.member(:events, default))
    schedBox = :wxCheckBox.new(panel, -1, ~c"Trace scheduling of processes", [])
    ...
    exitBox = :wxCheckBox.new(panel, -1, ~c"Trace scheduling of exiting processes", [])
    check_box(exitBox, :lists.member(:exiting, default))
    gCBox = :wxCheckBox.new(panel, -1, ~c"Trace garbage collections", [])
    ...
    {spawnBox, spwnAllRadio, spwnFirstRadio} =
      optionpage_top_right(panel, rightSz, [{:flag, 128}, {:border, 5}], ~c"spawn")

    {linkBox, linkAllRadio, linkFirstRadio} =
      optionpage_top_right(panel, rightSz, [{:flag, 128}, {:border, 5}], ~c"link")
  ...
  def port_trace(parent, default) do
    dialog =
      :wxDialog.new(parent, -1, ~c"Port Options", [{:style, 536_870_912 ||| 2048 ||| 4096 ||| 64}])

    panel = :wxPanel.new(dialog)
    mainSz = :wxBoxSizer.new(8)
    optsSz = :wxStaticBoxSizer.new(8, panel, [{:label, ~c"Tracing options"}])
    sendBox = :wxCheckBox.new(panel, -1, ~c"Trace send message", [])
    check_box(sendBox, :lists.member(:send, default))
    recBox = :wxCheckBox.new(panel, -1, ~c"Trace receive message", [])
    check_box(recBox, :lists.member(:receive, default))
    eventBox = :wxCheckBox.new(panel, -1, ~c"Trace port events", [])
    check_box(eventBox, :lists.member(:events, default))
    schedBox = :wxCheckBox.new(panel, -1, ~c"Trace scheduling of ports", [])
  ...
  defp module_selector(parent, node) do
    ...
    dialog =
      :wxDialog.new(parent, -1, ~c"Select Module or Event", [
        {:style, 536_870_912 ||| 2048 ||| 4096 ||| 64},
    ...
    events = [
      {~c"Messages sent", {:"$trace_event", :send}},
      {~c"Messages received", {:"$trace_event", :receive}}
    ]
    ...
    filter_listbox_data(~c"", allModules, listBox)
  ...
  defp check_selector(parent, parsedChoices) do
    ...
    dialog =
      :wxDialog.new(parent, -1, ~c"Trace Functions", [
        {:style, 536_870_912 ||| 2048 ||| 4096 ||| 64},
        {:size, {400 * scale, 400 * scale}}
      ])
    ...
    :wxSizer.add(panelSz, txtCtrl, [{:flag, 8192}])
    :wxSizer.add(panelSz, listBox, [{:flag, 8192}, {:proportion, 1}])
    selAllBtn = :wxButton.new(panel, -1, [{:label, ~c"Check Visible"}])
    deSelAllBtn = :wxButton.new(panel, -1, [{:label, ~c"Uncheck Visible"}])
    ...
    :wxWindow.setSizer(dialog, mainSz)
    :wxWindow.setFocus(txtCtrl)
    filter_listbox_data(~c"", parsedChoices, listBox, false)
  ...
  def select_matchspec(pid, parent, allMatchSpecs, key) do
    ...
    dialog =
      :wxDialog.new(parent, -1, ~c"Trace Match Specifications", [
        {:style, 536_870_912 ||| 2048 ||| 4096 ||| 64},
        {:size, {400 * scale, 400 * scale}}
      ])
    ...
    panelSz = :wxBoxSizer.new(8)
    mainSz = :wxBoxSizer.new(8)
    txtSz = :wxStaticBoxSizer.new(8, panel, [{:label, ~c"Match specification:"}])
    btnSz = :wxBoxSizer.new(4)
    savedSz = :wxStaticBoxSizer.new(8, panel, [{:label, ~c"Saved match specifications:"}])
    textCtrl = create_styled_txtctrl(panel)
    :wxSizer.add(txtSz, textCtrl, [{:flag, 8192}, {:proportion, 1}])
    addMsBtn = :wxButton.new(panel, -1, [{:label, ~c"New"}])
    editMsBtn = :wxButton.new(panel, -1, [{:label, ~c"Edit"}])
    delMsBtn = :wxButton.new(panel, -1, [{:label, ~c"Delete"}])
    ...
    filter_listbox_data(~c"", choices, listBox)
  ...
  def output(parent, default) do
    ...
    panel = :wxPanel.new(dialog)
    mainSz = :wxBoxSizer.new(8)
    panelSz = :wxStaticBoxSizer.new(8, panel, [{:label, ~c"Output"}])
    winB = :wxCheckBox.new(panel, -1, ~c"Window", [])
    ...
    shellB = :wxCheckBox.new(panel, -1, ~c"Shell", [])
  ...
  defp edit_ms(textCtrl, label0, parent) do
    ...
    try do
      ...
      r_match_spec(
        name: label,
        term: matchSpec,
        str: :io_lib.format(~c"~tw", [matchSpec]),
        func: str
      )
    catch
      :cancel ->
      ...
      error ->
        :observer_wx.create_txt_dialog(parent, error, ~c"Error", 512)
        :ok
    end
  end

  defp get_label(frame) do
    dialog = :wxTextEntryDialog.new(frame, ~c"Enter alias: ")
        ...
        case exprs do
          [{:fun, _, {:clauses, clauses}} | _] ->
            case :ms_transform.transform_from_shell(:dbg, clauses, :orddict.new()) do
              ...
              {:error, _} ->
                throw(~c"Could not convert fun() to match spec")
  ...
  defp optionpage_top_right(panel, topRightSz, options, text) do
    ...
    chkBox = :wxCheckBox.new(panel, -1, ~c"Inherit on " ++ text, [])
    radioSz = :wxBoxSizer.new(8)
    radio1 = :wxRadioButton.new(panel, -1, ~c"All " ++ text, [{:style, 4}])
    radio2 = :wxRadioButton.new(panel, -1, ~c"First " ++ text ++ ~c" only", [])
  ...
  defp parse_function_names(choices) do
    strList =
      for term = {name, arity} <- choices do
        {:erlang.atom_to_list(name) ++ ~c"/" ++ :erlang.integer_to_list(arity), term}
      end
  ...
  defp parse_function_names([{h, term} | t], acc) do
    isFun = :re.run(h, ~c".*-fun-\\d*?-", [:unicode, :ucp])
    isLc = :re.run(h, ~c".*-lc\\$\\^\\d*?/\\d*?-\\d*?-", [:unicode, :ucp])
    isLbc = :re.run(h, ~c".*-lbc\\$\\^\\d*?/\\d*?-\\d*?-", [:unicode, :ucp])
    ...
    parsed =
      cond do
        ...
        isLc !== :nomatch ->
          ~c"List comprehension: " ++ h

        isLbc !== :nomatch ->
          ~c"Bit comprehension: " ++ h
  ...
  defp ttb_file_options(panel, sizer, default) do
    ...
    fileBox = :wxCheckBox.new(panel, -1, ~c"File (Using ttb file tracer)", [])
    ...
    desc = :wxStaticText.new(panel, -1, ~c"File")
    fileName = :proplists.get_value(:filename, default, ~c"ttb")
    fileT = :wxTextCtrl.new(panel, -1, [{:size, {150, -1}}, {:value, fileName}])
    fileB = :wxButton.new(panel, -1, [{:label, ~c"Browse"}])
    ...
    wrapB = :wxCheckBox.new(panel, -1, ~c"Wrap logs")
  ...
  defp get_file(text) do
    ...
    dialog =
      :wxFileDialog.new(
        text,
        [{:message, ~c"Select a file"}, {:defaultFile, str}]
      )
