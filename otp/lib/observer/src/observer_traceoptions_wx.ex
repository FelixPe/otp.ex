defmodule :m_observer_traceoptions_wx do
  ...
  def process_trace(parent, default) do
    dialog = :wxDialog.new(parent, - 1, 'Process Options',
    ...
    {spawnBox, spwnAllRadio,
       spwnFirstRadio} = optionpage_top_right(panel, rightSz,
                                                [{:flag, 128}, {:border, 5}], 'spawn')
    {linkBox, linkAllRadio,
       linkFirstRadio} = optionpage_top_right(panel, rightSz,
                                                [{:flag, 128}, {:border, 5}], 'link')
    spawnBool = :lists.member(:on_spawn,
                                default) or :lists.member(:on_first_spawn,
                                                            default)
    linkBool = :lists.member(:on_link,
                               default) or :lists.member(:on_first_link,
                                                           default)
    ...
    for {radio, opt} <- [{spwnAllRadio, :on_spawn},
                             {spwnFirstRadio, :on_first_spawn}, {linkAllRadio,
                                                                   :on_link},
                                                                    {linkFirstRadio,
                                                                       :on_first_link}] do
      :wxRadioButton.setValue(radio,
                                :lists.member(opt, default))
    end
    for checkBox <- [funcBox, arityBox, sendBox, recBox,
                                                     eventBox, schedBox,
                                                                   exitBox,
                                                                       gCBox] do
    ...
    :wxSizer.add(panelSz, leftSz,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(panelSz, rightSz,
                   [{:flag, 8192}, {:proportion, 1}])
    ...
    case (:wxDialog.showModal(dialog)) do
      5100 ->
        all = [{sendBox, :send}, {recBox, :receive}, {funcBox,
                                                        :functions},
                                                         {arityBox, :arity},
                                                             {eventBox,
                                                                :events},
        ...
        opts = (for {tick, id} <- all, check.(tick) do
                  id
                end)
  ...
  def port_trace(parent, default) do
    dialog = :wxDialog.new(parent, - 1, 'Port Options',
                             [{:style, 536870912 ||| 2048 ||| 4096 ||| 64}])
    panel = :wxPanel.new(dialog)
    mainSz = :wxBoxSizer.new(8)
    ...
    case (:wxDialog.showModal(dialog)) do
      5100 ->
        all = [{sendBox, :send}, {recBox, :receive}, {eventBox,
                                                        :events},
                                                         {schedBox,
                                                            :running_ports}]
        opts = (for {tick, id} <- all,
  ...
  def trace_pattern(parentPid, parent, node, matchSpecs) do
    try do
      {module, mFAs,
         matchSpec} = (case (module_selector(parent, node)) do
                         {:"$trace_event", event} ->
                           mS = select_matchspec(parentPid, parent, matchSpecs,
                                                   event)
  ...
  defp module_selector(parent, node) do
    ...
    dialog = :wxDialog.new(parent, - 1, 'Select Module or Event',
                             [{:style, 536870912 ||| 2048 ||| 4096 ||| 64},
    ...
    events = [{'Messages sent', {:"$trace_event", :send}}, {'Messages received', {:"$trace_event", :receive}}]
    allModules = events ++ (for x <- modules do
                              {:erlang.atom_to_list(x), x}
                            end)
    ...
    :wxTextCtrl.connect(txtCtrl, :command_text_updated,
                          [{:callback,
                              fn r_wx(event: r_wxCommand(cmdString: input)), _ ->
                                   filter_listbox_data(input, allModules,
                                                         listBox)
                              end}])
    :wxListBox.connect(listBox,
  ...
  defp function_selector(parent, node, module) do
    functions = :observer_wx.try_rpc(node, module,
                                       :module_info, [:functions])
    externals = :observer_wx.try_rpc(node, module,
                                       :module_info, [:exports])
    choices = :lists.usort(for {name,
                                  arity} <- externals ++ functions,
  ...
  defp check_selector(parent, parsedChoices) do
    scale = :observer_wx.get_scale()
    dialog = :wxDialog.new(parent, - 1, 'Trace Functions',
                             [{:style, 536870912 ||| 2048 ||| 4096 ||| 64},
                                  {:size, {400 * scale, 400 * scale}}])
    panel = :wxPanel.new(dialog)
    panelSz = :wxBoxSizer.new(8)
    mainSz = :wxBoxSizer.new(8)
    txtCtrl = :wxTextCtrl.new(panel, - 1)
    listBox = :wxCheckListBox.new(panel, - 1,
                                    [{:style, 128}])
    :wxSizer.add(panelSz, txtCtrl, [{:flag, 8192}])
    :wxSizer.add(panelSz, listBox,
                   [{:flag, 8192}, {:proportion, 1}])
    ...
    :wxTextCtrl.connect(txtCtrl, :command_text_updated,
                          [{:callback,
                              fn r_wx(event: r_wxCommand(cmdString: input)), _ ->
                                   filter_listbox_data(input, parsedChoices,
                                                         listBox, false)
                              end}])
    self = self()
    ...
    :wxCheckListBox.connect(listBox,
                              :command_checklistbox_toggled,
                              [{:callback,
                                  fn r_wx(event: r_wxCommand(commandInt: n)), _ ->
                                       send(self, {listBox,
                                                     :wxCheckListBox.isChecked(listBox,
                                                                                 n),
                                                     getClientData.(listBox,
                                                                      n)})
                                  end}])
    check = fn id, bool ->
  ...
  def select_matchspec(pid, parent, allMatchSpecs, key) do
    {matchSpecs, restMS} = (case (:lists.keytake(key, 1,
                                                   allMatchSpecs)) do
                              {:value, {^key, mSs0}, rest} ->
                                {mSs0, rest}
                              false ->
    ...
    scale = :observer_wx.get_scale()
    dialog = :wxDialog.new(parent, - 1, 'Trace Match Specifications',
                             [{:style, 536870912 ||| 2048 ||| 4096 ||| 64},
                                  {:size, {400 * scale, 400 * scale}}])
    panel = :wxPanel.new(dialog)
    panelSz = :wxBoxSizer.new(8)
    ...
    :wxSizer.add(savedSz, listBox,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(panelSz, txtSz,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSizer.add(panelSz, btnSz)
    :wxSizer.add(panelSz, savedSz,
                   [{:flag, 8192}, {:proportion, 1}])
    ...
    :wxButton.connect(addMsBtn, :command_button_clicked,
                        [{:callback, add}])
    :wxButton.connect(editMsBtn, :command_button_clicked,
                        [{:callback, edit}])
    :wxButton.connect(delMsBtn, :command_button_clicked,
                        [{:callback, del}])
    :wxListBox.connect(listBox, :command_listbox_selected,
                         [{:callback, sel}])
    case (:wxDialog.showModal(dialog)) do
      5100 ->
        selId = :wxListBox.getSelection(listBox)
        count = :wxListBox.getCount(listBox)
        mSs = (for id <- :lists.seq(0, count - 1) do
  ...
  def output(parent, default) do
    ...
    case (:wxDialog.showModal(dialog)) do
      5100 ->
        res = [{:window, :wxCheckBox.getValue(winB)}, {:shell,
                                                         :wxCheckBox.getValue(shellB)} |
                                                          getFileOpts.()]
        :wxDialog.destroy(dialog)
        res
      5101 ->
  ...
  defp edit_ms(textCtrl, label0, parent) do
    ...
    try do
      ...
      label = (case (label0 == :new) do
                 true ->
                   get_label(parent)
                 _ ->
                   label0
               end)
      r_match_spec(name: label, term: matchSpec,
          str: :io_lib.format('~tw', [matchSpec]), func: str)
    catch
      :cancel ->
        :ok
      error ->
  ...
  defp ms_from_string(str) do
    try do
      tokens = (case (:erl_scan.string(str)) do
                  {:ok, ts, _} ->
                    ts
                  {:error, {sLine, sMod, sError}, _} ->
                    throw(:io_lib.format('~w: ~ts',
                                           [sLine, sMod.format_error(sError)]))
                end)
      exprs = (case (:erl_parse.parse_exprs(tokens)) do
                 {:ok, t} ->
                   t
                ...
                [{:fun, _, {:clauses, clauses}} | _] ->
                  case (:ms_transform.transform_from_shell(:dbg, clauses,
                    ...
                    {:error, [{_, [{mSLine, mod, mSInfo}]}], _} ->
                      throw(:io_lib.format('~w: ~tp',
                                             [mSLine,
                                                  mod.format_error(mSInfo)]))
                    {:error, _} ->
  ...
  defp filter_listbox_data(input, data, listBox, addClientData) do
    filteredData = (for (x = {str, _}) <- data,
                          :re.run(str, input, [:unicode]) !== :nomatch do
                      x
                    end)
    :wxListBox.clear(listBox)
    :wxListBox.appendStrings(listBox,
                               for {str, _} <- filteredData do
                                 str
                               end)
    addClientData and :wx.foldl(fn {_, term}, n ->
  ...
  defp get_modules(node) do
    :lists.sort(for {module,
                       _} <- :observer_wx.try_rpc(node, :code, :all_loaded,
                                                    []) do
                  module
                end)
  end
  ...
  defp optionpage_top_right(panel, topRightSz, options, text) do
    ...
    radio1 = :wxRadioButton.new(panel, - 1, 'All ' ++ text,
                                  [{:style, 4}])
  ...
  defp parse_function_names(choices) do
    strList = (for (term = {name, arity}) <- choices do
                 {:erlang.atom_to_list(name) ++ '/' ++ :erlang.integer_to_list(arity),
                    term}
               end)
  ...
  defp ttb_file_options(panel, sizer, default) do
    ...
    fileT = :wxTextCtrl.new(panel, - 1,
                              [{:size, {150, - 1}}, {:value, fileName}])
    fileB = :wxButton.new(panel, - 1, [{:label, 'Browse'}])
    :wxSizer.add(nameS, desc,
                   [{:proportion, 0}, {:flag, 2048}])
    :wxSizer.add(nameS, fileT,
                   [{:proportion, 1}, {:flag, 8192 ||| 2048}])
    :wxSizer.add(nameS, fileB,
                   [{:proportion, 0}, {:flag, 2048}])
    wrapB = :wxCheckBox.new(panel, - 1, 'Wrap logs')
    wrapSz = :wxSlider.new(panel, - 1,
                             :proplists.get_value(:wrap_sz, default, 128), 64,
                             10 * 1024,
                             [{:style,
                                 4 ||| :wxe_util.get_const(:wxSL_LABELS)}])
    wrapC = :wxSlider.new(panel, - 1,
                            :proplists.get_value(:wrap_c, default, 8), 2, 100,
    ...
    :wxSizer.add(sizer, top,
                   [{:flag, 8192 ||| 16}, {:border, 20}])
    ...
    enable.(:proplists.get_value(:file, default, false),
    ...
    fn () ->
         [{:file, :wxCheckBox.getValue(fileBox)}, {:filename,
                                                     :wxTextCtrl.getValue(fileT)},
                                                      {:wrap,
                                                         :wxCheckBox.getValue(wrapB)},
                                                          {:wrap_sz,
                                                             :wxSlider.getValue(wrapSz)},
                                                              {:wrap_c,
                                                                 :wxSlider.getValue(wrapC)}]
    end
  end
