defmodule :m_observer_trace_wx do
  ...
  defp create_window(notebook, parentPid, config) do
    ...
    toggleButton = :wxToggleButton.new(panel, 307, ~c"Start Trace", [])
    :wxSizer.add(buttons, toggleButton, [{:flag, 2048}])
    :wxSizer.addSpacer(buttons, 15)

    :wxSizer.add(
      buttons,
      :wxButton.new(panel, 331, [{:label, ~c"Add Nodes"}])
    )

    allButton = :wxButton.new(panel, 308, [{:label, ~c"Add All"}])
    :wxControl.setToolTip(allButton, ~c"Trace all processes and ports")
    :wxSizer.add(buttons, allButton)

    :wxSizer.add(
      buttons,
      :wxButton.new(panel, 309, [{:label, ~c"Add 'new' Processes"}])
    )

    :wxSizer.add(
      buttons,
      :wxButton.new(panel, 310, [{:label, ~c"Add 'new' Ports"}])
    )

    :wxSizer.add(
      buttons,
      :wxButton.new(panel, 311, [{:label, ~c"Add Trace Pattern"}])
    )
  ...
  defp get_default_matchspecs(:funcs) do
    [
      {~c"Return Trace", [{:_, [], [{:return_trace}]}], ~c"fun(_) -> return_trace() end"},
      {~c"Exception Trace", [{:_, [], [{:exception_trace}]}],
       ~c"fun(_) -> exception_trace() end"},
      {~c"Message Caller", [{:_, [], [{:message, {:caller}}]}],
       ~c"fun(_) -> message(caller()) end"},
      {~c"Message Dump", [{:_, [], [{:message, {:process_dump}}]}],
       ~c"fun(_) -> message(process_dump()) end"}
    ]
  end

  defp get_default_matchspecs(:send) do
    [
      {~c"To local node", [{[:"$1", :_], [{:==, {:node, :"$1"}, {:node}}], []}],
       ~c"fun([Pid,_]) when node(Pid)==node() ->\n    true\nend"},
      {~c"To remote node", [{[:"$1", :_], [{:"=/=", {:node, :"$1"}, {:node}}], []}],
       ~c"fun([Pid,_]) when node(Pid)=/=node() ->\n    true\nend"}
    ]
  end
  ...
  defp get_default_matchspecs(:receive) do
    [
      {~c"From local node", [{[:"$1", :_, :_], [{:==, :"$1", {:node}}], []}],
       ~c"fun([Node,_,_]) when Node==node() ->\n    true\nend"},
      {~c"From remote node", [{[:"$1", :_, :_], [{:"=/=", :"$1", {:node}}], []}],
       ~c"fun([Node,_,_]) when Node=/=node() ->\n    true\nend"}
    ]
  end
  ...
  defp create_proc_port_view(parent) do
    ...
    procListItems = [{~c"Process Id", 2, scale * 120}, {~c"Trace Options", 0, scale * 300}]
    ...
    portListItems = [{~c"Port Id", 2, scale * 120}, {~c"Trace Options", 0, scale * 300}]
    ...
    :wxListCtrl.setToolTip(nodes, ~c"Right click to add nodes")
    :wxListCtrl.setToolTip(procs, ~c"Add items from Processes/Ports tab")
    :wxListCtrl.setToolTip(ports, ~c"Add items from Processes/Ports tab")
  ...
  defp create_matchspec_view(parent) do
    ...
    :wxListCtrl.setToolTip(panel, ~c"Add trace pattern with button below")
    :wxPanel.setSizer(panel, mainSz)
    {panel, modules, funcs}
  end
  ...
  defp create_menues(parent) do
    menus = [
      {~c"File",
       [
         r_create_menu(id: 306, text: ~c"Load settings"),
         r_create_menu(id: 305, text: ~c"Save settings")
       ]},
      {~c"Options",
       [
         r_create_menu(id: 312, text: ~c"Output"),
         r_create_menu(id: 313, text: ~c"Default Match Specifications for Functions"),
         r_create_menu(
           id: 314,
           text: ~c"Default Match Specifications for 'send'"
         ),
         r_create_menu(
           id: 315,
           text: ~c"Default Match Specifications for 'receive'"
         ),
         r_create_menu(
           id: 316,
           text: ~c"Default Process Options"
         ),
         r_create_menu(
           id: 317,
           text: ~c"Default Port Options"
         )
       ]}
    ]
  ...
  def handle_event(
    ...
    try do
      ...
      tProcs == [] and tPorts == [] and throw({:error, ~c"No processes or ports traced"})
      nodes == [] and throw({:error, ~c"No nodes traced"})

      haveCallTrace = fn r_titem(opts: os) ->
        :lists.member(:functions, os)
      end

      wStr = ~c"Call trace activated but no trace patterns used"
      ...
      :wxToggleButton.setLabel(toggleBtn, ~c"Stop Trace")
      {:noreply, r_state(state, logwin: logWin)}
    catch
      {:error, msg} ->
        :observer_wx.create_txt_dialog(panel, msg, ~c"Error", 512)
  ...
  def handle_event(
    ...
    :ttb.stop(:nofetch)
    :wxToggleButton.setLabel(toggleBtn, ~c"Start Trace")
    :wxToggleButton.setValue(toggleBtn, false)
  ...
  def handle_event(
        r_wx(id: 5001, obj: obj, event: r_wxCommand(type: :command_menu_selected)),
        state
      ) do
    :wxWindow.close(obj, [])
    {:noreply, state}
        ...
        case get_selected_items(lCtrl, visible) do
          ...
          _ ->
            create_right_click_menu(panel, [
              {341, ~c"Edit process options"},
              {342, ~c"Remove processes"}
            ])
        end
    end
    ...
    case get_visible_ps(tports, nodes, nview) do
      ...
      visible ->
        case get_selected_items(lCtrl, visible) do
          ...
          _ ->
            create_right_click_menu(panel, [{351, ~c"Edit port options"}, {352, ~c"Remove ports"}])
        end
    end
  ...
  def handle_event(
    ...
    case get_selected_items(
      ...
      _ ->
        create_right_click_menu(panel, [{361, ~c"Remove trace patterns"}])
    end
    ...
    case get_selected_items(
      ...
      [module] ->
        case get_selected_items(
          ...
          _ ->
            create_right_click_menu(panel, [
              {371, ~c"Edit matchspecs"},
              {372, ~c"Remove trace patterns"}
            ])
        end
    end
  ...
  def handle_event(
    ...
    menu =
      case get_selected_items(nview, nodes) do
        ...
        _ ->
          [{331, ~c"Add nodes"}, {332, ~c"Remove nodes"}]
      end
  ...
  def handle_event(
    ...
    try do
      case get_selected_items(
        ...
        [] ->
          throw({:error, ~c"No module selected"})
            ...
            case module do
              :Events ->
                ...
                case :lists.all(
                  ...
                  false ->
                    throw({:error, ~c"Can not set match specs for multiple event types"})
                end
  ...
  def handle_event(
    ...
    try do
      ...
      case possible do
        [] ->
          msg =
            ~c"Already selected all connected nodes\nUse the Nodes menu to connect to new nodes first."

          :observer_wx.create_txt_dialog(panel, msg, ~c"No available nodes", 2048)
          throw(:cancel)
  ...
  def handle_event(r_wx(id: iD, event: what), state) do
    :io.format(:user, ~c"~p:~p: Unhandled event: ~p, ~tp ~n", [:observer_trace_wx, 704, iD, what])
    {:noreply, state}
  end
  ...
  def handle_info(any, state) do
    :io.format(~c"~p~p: received unexpected message: ~tp\n", [:observer_trace_wx, self(), any])
    {:noreply, state}
  end
  ...
  defp update_p_view(pidsOrPorts, lCtrl) do
    ...
    :wx.foldl(
      fn r_titem(id: id, opts: opts), row ->
        _Item = :wxListCtrl.insertItem(lCtrl, row, ~c"")
    ...
    case pidsOrPorts do
      [] ->
        :wxListCtrl.setToolTip(lCtrl, ~c"Add items from Processes/Ports tab")

      _ ->
        :wxListCtrl.setToolTip(lCtrl, ~c"Select nodes to see traced processes and ports")
    end
  end
  ...
  defp update_nodes_view(nodes, lCtrl) do
    ...
    case nodes do
      [] ->
        :wxListCtrl.setToolTip(lCtrl, ~c"Right click to add nodes")
  ...
  defp update_modules_view(mods, module, lCtrl) do
    ...
    case mods do
      [] ->
        :wxListCtrl.setToolTip(parent, ~c"Add trace pattern with button below")

      _ ->
        :wxListCtrl.setToolTip(parent, ~c"Select module to see trace patterns")
    end
  end
  ...
  defp ttb_output_args(parent, opts) do
    ...
    toWindow or toShell or toFile or throw({:error, ~c"No output of trace"})
  ...
  defp output_file(true, false, opts) do
    :proplists.get_value(:filename, opts, ~c"ttb")
  end

  defp output_file(true, true, opts) do
    name = :proplists.get_value(:filename, opts, ~c"ttb")
  ...
  defp create_logwindow(parent, true) do
    scale = :observer_wx.get_scale()
    logWin = :wxFrame.new(parent, 380, ~c"Trace Log", [{:size, {750 * scale, 800 * scale}}])
    mB = :wxMenuBar.new()
    file = :wxMenu.new()
    :wxMenu.append(file, 382, ~c"Clear Log\tCtrl-C")
    :wxMenu.append(file, 381, ~c"Save Log\tCtrl-S")
    :wxMenu.append(file, 5001, ~c"Close")
    :wxMenuBar.append(mB, file, ~c"File")
  ...
  defp textformat(trace)
    ...
    :io_lib.format(~c"*** Dropped ~p messages.~n", [:erlang.element(2, trace)])
  end
  ...
  defp textformat(trace)
    ...
    :io_lib.format(~c"*** Seq trace not implemented.~n", [])
  end
  ...
  defp format_trace(trace, size, tS0 = {_, _, mS}) do
    ...
    tS = :io_lib.format(~c"~.2.0w:~.2.0w:~.2.0w:~.6.0w", [h, m, s, mS])
    ...
    case :erlang.element(3, trace) do
      :receive ->
        case :erlang.element(4, trace) do
          ...
          message ->
            :io_lib.format(~c"~s (~100p) << ~100tp~n", [tS, from, message])
        end

      :send ->
        message = :erlang.element(4, trace)
        to = :erlang.element(5, trace)
        :io_lib.format(~c"~s (~100p) ~100p ! ~100tp~n", [tS, from, to, message])

      :call ->
        case :erlang.element(4, trace) do
          mFA when size == 5 ->
            message = :erlang.element(5, trace)
            :io_lib.format(~c"~s (~100p) call ~ts (~100tp) ~n", [tS, from, ffunc(mFA), message])

          mFA ->
            :io_lib.format(~c"~s (~100p) call ~ts~n", [tS, from, ffunc(mFA)])
        end

      :return_from ->
        mFA = :erlang.element(4, trace)
        ret = :erlang.element(5, trace)
        :io_lib.format(~c"~s (~100p) returned from ~ts -> ~100tp~n", [tS, from, ffunc(mFA), ret])

      :return_to ->
        mFA = :erlang.element(4, trace)
        :io_lib.format(~c"~s (~100p) returning to ~ts~n", [tS, from, ffunc(mFA)])

      :spawn when size == 5 ->
        pid = :erlang.element(4, trace)
        mFA = :erlang.element(5, trace)
        :io_lib.format(~c"~s (~100p) spawn ~100p as ~ts~n", [tS, from, pid, ffunc(mFA)])

      op ->
        :io_lib.format(~c"~s (~100p) ~100p ~ts~n", [tS, from, op, ftup(trace, 4, size)])
    end
  end

  defp ffunc({m, f, argl}) when is_list(argl) do
    :io_lib.format(~c"~100p:~100tp(~ts)", [m, f, fargs(argl)])
  end

  defp ffunc({m, f, arity}) do
    :io_lib.format(~c"~100p:~100tp/~100p", [m, f, arity])
  end

  defp ffunc(x) do
    :io_lib.format(~c"~100tp", [x])
  end
  ...
  defp fargs([a]) do
    :io_lib.format(~c"~100tp", [a])
  end

  defp fargs([a | args]) do
    [:io_lib.format(~c"~100tp,", [a]) | fargs(args)]
  end
  ...
  defp ftup(trace, index, index) do
    :io_lib.format(~c"~100tp", [:erlang.element(index, trace)])
  end
  ...
  defp write_file(frame, filename, config) do
    ...
    case :file.write_file(
      ...
      {:error, reason} ->
        ...
        :observer_wx.create_txt_dialog(frame, failMsg, ~c"Error", 512)
    end
  end
  ...
  defp make_ms(name, term, funStr) do
    r_match_spec(name: name, term: term, str: :io_lib.format(~c"~tw", [term]), func: funStr)
  end
