defmodule :m_observer_trace_wx do
  ...
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 panel: :undefined, n_view: :undefined,
                                 proc_view: :undefined, port_view: :undefined,
                                 m_view: :undefined, f_view: :undefined,
                                 logwin: :undefined, nodes: [],
                                 toggle_button: :undefined, tpids: [],
                                 tports: [], def_proc_flags: [],
                                 def_port_flags: [], output: [],
                                 tpatterns: :dict.new(), match_specs: [])
  Record.defrecord(:r_titem, :titem, id: :undefined,
                                 opts: :undefined)
  def start_link(notebook, parentPid, config) do
    :wx_object.start_link(:observer_trace_wx,
                            [notebook, parentPid, config], [])
  end

  def add_processes(pids) when is_list(pids) do
    :wx_object.cast(:observer_wx.get_tracer(),
                      {:add_processes, pids})
  end
  ...
  defp create_window(notebook, parentPid, config) do
    ...
    splitter = :wxSplitterWindow.new(panel,
                                       [{:size, :wxWindow.getClientSize(panel)},
                                            {:style, 128 ||| 0 ||| 256}])
    {nodeProcView, nodeView, processView,
       portView} = create_proc_port_view(splitter)
    {matchSpecView, modView,
       funcView} = create_matchspec_view(splitter)
    :wxSplitterWindow.setSashGravity(splitter, 0.5)
    :wxSplitterWindow.setMinimumPaneSize(splitter, 50)
    :wxSplitterWindow.splitHorizontally(splitter,
                                          nodeProcView, matchSpecView,
                                          [{:sashPosition, 368}])
    ...
    allButton = :wxButton.new(panel, 308, [{:label, 'Add All'}])
    :wxControl.setToolTip(allButton, 'Trace all processes and ports')
    :wxSizer.add(buttons, allButton)
    :wxSizer.add(buttons,
                   :wxButton.new(panel, 309, [{:label, 'Add \'new\' Processes'}]))
    :wxSizer.add(buttons,
                   :wxButton.new(panel, 310, [{:label, 'Add \'new\' Ports'}]))
    :wxSizer.add(buttons,
                   :wxButton.new(panel, 311, [{:label, 'Add Trace Pattern'}]))
    :wxMenu.connect(panel, :command_togglebutton_clicked,
                      [{:skip, true}])
    :wxMenu.connect(panel, :command_button_clicked,
                      [{:skip, true}])
    :wxSizer.add(sizer, buttons,
                   [{:flag, 16 ||| 32 ||| 128}, {:border, 5}, {:proportion,
                                                                 0}])
    :wxWindow.setSizer(panel, sizer)
    mS = parse_ms(:maps.get(:match_specs, config, []),
                    default_matchspecs())
    {panel,
       r_state(parent: parentPid, panel: panel, n_view: nodeView,
           proc_view: processView, port_view: portView,
  ...
  defp get_default_matchspecs(:funcs) do
    [{'Return Trace', [{:_, [], [{:return_trace}]}], 'fun(_) -> return_trace() end'}, {'Exception Trace',
                                               [{:_, [], [{:exception_trace}]}],
  ...
  defp get_default_matchspecs(:send) do
    [{'To local node', [{[:"$1", :_], [{:"==", {:node, :"$1"}, {:node}}], []}], 'fun([Pid,_]) when node(Pid)==node() ->\n    true\nend'},
         {'To remote node', [{[:"$1", :_], [{:"=/=", {:node, :"$1"}, {:node}}], []}], 'fun([Pid,_]) when node(Pid)=/=node() ->\n    true\nend'}]
  end

  defp get_default_matchspecs(:receive) do
    [{'From local node', [{[:"$1", :_, :_], [{:"==", :"$1", {:node}}], []}], 'fun([Node,_,_]) when Node==node() ->\n    true\nend'}, {'From remote node',
                                                           [{[:"$1", :_, :_],
                                                               [{:"=/=", :"$1",
                                                                   {:node}}],
                                                               []}],
  ...
  defp create_proc_port_view(parent) do
    ...
    splitter = :wxSplitterWindow.new(panel,
                                       [{:style, 128 ||| 0 ||| 256}])
    nodes = :wxListCtrl.new(splitter,
                              [{:winid, 330}, {:style, style}])
    procsPortsSplitter = :wxSplitterWindow.new(splitter,
                                                 [{:style, 128 ||| 0 ||| 256}])
    procs = :wxListCtrl.new(procsPortsSplitter,
                              [{:winid, 340}, {:style, style}])
    ports = :wxListCtrl.new(procsPortsSplitter,
    ...
    procListItems = [{'Process Id', 2, scale * 120}, {'Trace Options', 0,
                                             scale * 300}]
    ...
    portListItems = [{'Port Id', 2, scale * 120}, {'Trace Options', 0,
                                             scale * 300}]
    :lists.foldl(addPort, 0, portListItems)
    :wxListItem.destroy(li)
    :wxSplitterWindow.setSashGravity(splitter, 0.0)
    :wxSplitterWindow.setMinimumPaneSize(splitter, 50)
    :wxSplitterWindow.splitVertically(splitter, nodes,
                                        procsPortsSplitter,
                                        [{:sashPosition, 155}])
    :wxSizer.add(mainSz, splitter,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxSplitterWindow.setSashGravity(procsPortsSplitter,
                                       0.5)
    :wxSplitterWindow.setMinimumPaneSize(procsPortsSplitter,
                                           50)
    :wxSplitterWindow.splitHorizontally(procsPortsSplitter,
                                          procs, ports, [{:sashPosition, 182}])
    :wxListCtrl.connect(procs,
                          :command_list_item_right_click)
    :wxListCtrl.connect(ports,
                          :command_list_item_right_click)
    :wxListCtrl.connect(nodes,
                          :command_list_item_right_click)
  ...
  defp create_matchspec_view(parent) do
    ...
    splitter = :wxSplitterWindow.new(panel,
                                       [{:style, 128 ||| 0 ||| 256}])
    modules = :wxListCtrl.new(splitter,
                                [{:winid, 360}, {:style, style ||| 8192}])
    funcs = :wxListCtrl.new(splitter,
    ...
    :wxSplitterWindow.setSashGravity(splitter, 0.0)
    :wxSplitterWindow.setMinimumPaneSize(splitter, 50)
    :wxSplitterWindow.splitVertically(splitter, modules,
                                        funcs, [{:sashPosition, 155}])
    :wxSizer.add(mainSz, splitter,
                   [{:flag, 8192}, {:proportion, 1}])
    :wxListCtrl.connect(modules, :size, [{:skip, true}])
    :wxListCtrl.connect(funcs, :size, [{:skip, true}])
    :wxListCtrl.connect(modules,
                          :command_list_item_selected)
    :wxListCtrl.connect(modules,
                          :command_list_item_right_click)
    :wxListCtrl.connect(funcs,
                          :command_list_item_right_click)
  ...
  defp create_menues(parent) do
    menus = [{'File',
                [r_create_menu(id: 306, text: 'Load settings'), r_create_menu(id: 305, text: 'Save settings')]},
                 {'Options',
                    [r_create_menu(id: 312, text: 'Output'), r_create_menu(id: 313, text: 'Default Match Specifications for Functions'), r_create_menu(id: 314,
                                                                   text: 'Default Match Specifications for \'send\''),
                                                                   r_create_menu(id: 315,
                                                                       text: 'Default Match Specifications for \'receive\''),
                                                                       r_create_menu(id: 316,
                                                                           text: 'Default Process Options'),
                                                                           r_create_menu(id: 317,
  ...
  def handle_event(r_wx(id: 308),
           state = r_state(panel: parent, def_proc_flags: traceOpts)) do
    try do
      opts = :observer_traceoptions_wx.process_trace(parent,
                                                       traceOpts)
      process = r_titem(id: :all, opts: opts)
      {:noreply,
         do_add_processes([process],
  ...
  def handle_event(r_wx(id: 310),
           state = r_state(panel: parent, def_port_flags: traceOpts)) do
    try do
      opts = :observer_traceoptions_wx.port_trace(parent,
                                                    traceOpts)
      port = r_titem(id: :new_ports, opts: opts)
      {:noreply,
         do_add_ports([port], r_state(state, def_port_flags: opts))}
    catch
      :cancel ->
        {:noreply, state}
    end
  ...
  def handle_event(r_wx(id: 311),
           state = r_state(panel: parent, nodes: nodes,
                       match_specs: ms)) do
    node = (case (nodes) do
              [n | _] ->
                n
    ...
    case (:observer_traceoptions_wx.trace_pattern(self(),
                                                    parent, node, ms)) do
      :cancel ->
        {:noreply, state}
      patterns ->
  ...
  def handle_event(r_wx(id: 360,
             event: r_wxList(type: :command_list_item_selected,
                        itemIndex: row)),
           state = r_state(tpatterns: tPs, m_view: mview,
                       f_view: fview)) do
    module = :erlang.list_to_atom(:wxListCtrl.getItemText(mview,
                                                            row))
    update_functions_view(:dict.fetch(module, tPs), fview)
    {:noreply, state}
  end

  def handle_event(r_wx(id: 330,
             event: r_wxList(type: :command_list_item_selected)),
           state = r_state(tpids: tpids, tports: tports, n_view: nview,
                       proc_view: procView, port_view: portView, nodes: ns)) do
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_togglebutton_clicked,
                      commandInt: 1)),
           r_state(panel: panel, nodes: nodes, tpids: tProcs,
               tports: tPorts, tpatterns: tPs0,
               toggle_button: toggleBtn, output: opts) = state) do
    try do
      ...
      tPs == [] and :lists.any(haveCallTrace,
                                 tProcs) and :observer_wx.create_txt_dialog(panel,
                                                                              wStr,
                                                                              'Warning',
                                                                              256)
  ...
  def handle_event(r_wx(event: r_wxCommand(type: :command_togglebutton_clicked,
                      commandInt: 0)),
           r_state(toggle_button: toggleBtn) = state) do
    :ttb.stop(:nofetch)
  ...
  def handle_event(r_wx(id: id, obj: logWin, event: ev),
  ...
  def handle_event(r_wx(id: 5001, obj: obj,
  ...
  def handle_event(r_wx(id: 381, userData: tCtrl),
           r_state(panel: panel) = state) do
  ...
  def handle_event(r_wx(id: 306), r_state(panel: panel) = state) do
    ...
    state2 = (case (:wxFileDialog.showModal(dialog)) do
                5100 ->
                  path = :wxFileDialog.getPath(dialog)
  ...
  def handle_event(r_wx(id: 340,
             event: r_wxList(type: :command_list_item_right_click)),
           state = r_state(panel: panel, proc_view: lCtrl, tpids: tpids,
                       n_view: nview, nodes: nodes)) do
      ...
      visible ->
        case (get_selected_items(lCtrl, visible)) do
          [] ->
            :ok
          _ ->
            create_right_click_menu(panel, [{341, 'Edit process options'}, {342, 'Remove processes'}])
        end
    end
    {:noreply, state}
  end

  def handle_event(r_wx(id: 350,
             event: r_wxList(type: :command_list_item_right_click)),
           state = r_state(panel: panel, port_view: lCtrl,
                       tports: tports, n_view: nview, nodes: nodes)) do
  ...
  def handle_event(r_wx(id: 360,
             event: r_wxList(type: :command_list_item_right_click)),
           state = r_state(panel: panel, m_view: mview,
                       tpatterns: tPs)) do
    case (get_selected_items(mview,
                               :lists.sort(:dict.fetch_keys(tPs)))) do
      [] ->
        :ok
      _ ->
  ...
  def handle_event(r_wx(id: 370,
             event: r_wxList(type: :command_list_item_right_click)),
           state = r_state(panel: panel, m_view: mview, f_view: fview,
                       tpatterns: tPs)) do
    case (get_selected_items(mview,
                               :lists.sort(:dict.fetch_keys(tPs)))) do
      [] ->
        :ok
      [module] ->
        case (get_selected_items(fview,
                                   :dict.fetch(module, tPs))) do
          [] ->
            :ok
          _ ->
            create_right_click_menu(panel, [{371, 'Edit matchspecs'}, {372, 'Remove trace patterns'}])
        end
    end
    {:noreply, state}
  end
  ...
  def handle_event(r_wx(id: 330,
             event: r_wxList(type: :command_list_item_right_click)),
           state = r_state(panel: panel, n_view: nview, nodes: nodes)) do
    menu = (case (get_selected_items(nview, nodes)) do
              [] ->
                [{331, 'Add nodes'}]
              _ ->
  ...
  def handle_event(r_wx(id: 341),
           r_state(panel: panel, tpids: tpids,
               proc_view: procs) = state) do
    try do
      [r_titem(opts: defOpts) |
           _] = (selected = get_selected_items(procs, tpids))
      opts = :observer_traceoptions_wx.process_trace(panel,
                                                       defOpts)
      changed = (for tpid <- selected do
                   r_titem(tpid, opts: opts)
                 end)
      {:noreply,
         do_add_processes(changed,
                            r_state(state, def_proc_flags: opts))}
    catch
      _, _ ->
        {:noreply, state}
  ...
  def handle_event(r_wx(id: 342),
           r_state(tpids: tpids, proc_view: lCtrl, n_view: nview,
  ...
  def handle_event(r_wx(id: 351),
           r_state(panel: panel, tports: tports,
               port_view: ports) = state) do
    try do
      [r_titem(opts: defOpts) |
           _] = (selected = get_selected_items(ports, tports))
      opts = :observer_traceoptions_wx.port_trace(panel,
                                                    defOpts)
      changed = (for tport <- selected do
                   r_titem(tport, opts: opts)
                 end)
      {:noreply,
         do_add_ports(changed, r_state(state, def_port_flags: opts))}
    catch
      _, _ ->
        {:noreply, state}
    end
  ...
  def handle_event(r_wx(id: 352),
           r_state(tports: tports, port_view: lCtrl, n_view: nview,
               nodes: nodes) = state) do
  ...
  def handle_event(r_wx(id: 316),
           r_state(panel: panel, def_proc_flags: pO) = state) do
    try do
      opts = :observer_traceoptions_wx.process_trace(panel,
                                                       pO)
  ...
  def handle_event(r_wx(id: 317),
           r_state(panel: panel, def_port_flags: pO) = state) do
    try do
  ...
  def handle_event(r_wx(id: 313),
           r_state(panel: panel, match_specs: ms) = state) do
    try do
      :observer_traceoptions_wx.select_matchspec(self(),
                                                   panel, ms, :funcs)
    catch
      _, _ ->
        :cancel
  ...
  def handle_event(r_wx(id: 371),
           r_state(panel: panel, tpatterns: tPs, f_view: lCtrl,
               m_view: mview, match_specs: mss) = state) do
    try do
      case (get_selected_items(mview,
                                 :lists.sort(:dict.fetch_keys(tPs)))) do
        [] ->
        ...
        [module] ->
          selected = get_selected_items(lCtrl,
                                          :dict.fetch(module, tPs))
          key = (case (module) do
                   :Events ->
                     selectedEvents = (for r_tpattern(fa: event) <- selected do
                                         event
                                       end)
                     e1 = hd(selectedEvents)
                     case (:lists.all(fn e when e == e1 ->
                                           true
                                         _ ->
                                           false
                                      end,
          ...
          ms = :observer_traceoptions_wx.select_matchspec(self(),
                                                            panel, mss, key)
          changed = (for tP <- selected do
                       r_tpattern(tP, ms: ms)
                     end)
  ...
  def handle_event(r_wx(id: 372),
           r_state(tpatterns: tPs0, f_view: lCtrl,
               m_view: mview) = state) do
    case (get_selected_items(mview,
                               :lists.sort(:dict.fetch_keys(tPs0)))) do
      [] ->
        {:noreply, state}
      [module] ->
        ...
        tPs = (case (fMs) do
                 [] ->
                   ...
                   update_modules_view(:lists.sort(:dict.fetch_keys(new)),
                                         module, mview)
                   new
                 _ ->
  ...
  def handle_event(r_wx(id: 361),
    ...
    case (get_selected_items(mview,
      ...
      [module] ->
        ...
        update_modules_view(:lists.sort(:dict.fetch_keys(tPs)),
                              module, mview)
        {:noreply, r_state(state, tpatterns: tPs)}
    end
  end

  def handle_event(r_wx(id: 312),
           r_state(panel: panel, output: out0) = state) do
    try do
  ...
  def handle_event(r_wx(id: 331),
           r_state(panel: panel, n_view: nview, nodes: ns0) = state) do
    try do
      ...
      case (possible) do
        [] ->
          ...
          throw(:cancel)
        _ ->
          ns = :lists.usort(ns0 ++ :observer_traceoptions_wx.select_nodes(panel,
                                                                            possible))
  ...
  def handle_event(r_wx(id: 332),
           r_state(n_view: nview, nodes: ns0) = state) do
  ...
  def handle_event(r_wx(id: iD, event: what), state) do
    :io.format(:user, '~p:~p: Unhandled event: ~p, ~tp ~n',
                 [:observer_trace_wx, 704, iD, what])
    {:noreply, state}
  end
  ...
  def handle_cast({:add_processes, pids},
           state = r_state(panel: parent, def_proc_flags: traceOpts)) do
    try do
      opts = :observer_traceoptions_wx.process_trace(parent,
                                                       traceOpts)
      pOpts = (for pid <- pids do
                 r_titem(id: pid, opts: opts)
               end)
      s = do_add_processes(pOpts,
                             r_state(state, def_proc_flags: opts))
      {:noreply, s}
    catch
      :cancel ->
  ...
  def handle_cast({:add_ports, ports},
           state = r_state(panel: parent, def_port_flags: traceOpts)) do
    try do
      opts = :observer_traceoptions_wx.port_trace(parent,
                                                    traceOpts)
      pOpts = (for id <- ports do
  ...
  defp do_add_patterns({module, newPs},
            state = r_state(tpatterns: tPs0, m_view: mview,
                        f_view: fview)) do
    old = (case (:dict.find(module, tPs0)) do
             {:ok, prev} ->
               prev
             :error ->
               []
  ...
  defp do_add_processes(pOpts,
            s0 = r_state(n_view: nview, proc_view: lCtrl, tpids: oldPids,
                     nodes: oldNodes)) do
    checkFun = fn pid ->
                    is_pid(pid)
               end
  ...
  defp do_add_ports(pOpts,
            s0 = r_state(n_view: nview, port_view: lCtrl,
                     tports: oldPorts, nodes: oldNodes)) do
    checkFun = fn port ->
                    is_port(port)
               end
  ...
  defp do_add_pid_or_port(pOpts, nview, lCtrl, oldPs, ns0, check) do
    case (merge_trace_items(pOpts, oldPs)) do
      {^oldPs, [], []} ->
        {oldPs, ns0}
      {ps, new, _Changed} ->
        ns1 = :lists.usort(for r_titem(id: id) <- new, check.(id) do
                             node(id)
                           end)
        nodes = (case (:ordsets.subtract(ns1, ns0)) do
                   [] when ns0 == [] ->
  ...
  defp get_visible_ps(pidsOrPorts, nodes, nview) do
    get_visible_ps(pidsOrPorts,
                     get_selected_items(nview, nodes))
  end

  defp get_visible_ps(pidsOrPorts, nodes) do
    for p <- pidsOrPorts,
          is_atom(r_titem(p, :id)) or :lists.member(node(r_titem(p, :id)),
                                                nodes) do
      p
    end
  end
  ...
  defp update_p_view(pidsOrPorts, lCtrl, nodes, nview) do
    update_p_view(get_visible_ps(pidsOrPorts, nodes, nview),
                    lCtrl)
  end
  ...
  defp update_p_view(pidsOrPorts, lCtrl) do
    ...
    :wx.foldl(fn r_titem(id: id, opts: opts), row ->
                   _Item = :wxListCtrl.insertItem(lCtrl, row, '')
                   rem(row, 2) === 0 and :wxListCtrl.setItemBackgroundColour(lCtrl,
                                                                               row,
                                                                               {230,
                                                                                  230,
                                                                                  250})
                   :wxListCtrl.setItem(lCtrl, row, 0,
                                         :observer_lib.to_str(id))
                   :wxListCtrl.setItem(lCtrl, row, 1,
                                         :observer_lib.to_str(opts))
                   row + 1
              end,
                0, pidsOrPorts)
    case (pidsOrPorts) do
    ...
    :wx.foldl(fn node, row ->
                   ...
                   rem(row, 2) === 0 and :wxListCtrl.setItemBackgroundColour(lCtrl,
                                                                               ...
                                                                               {230,
                                                                                  ...
                                                                                  250})
                   :wxListCtrl.setItem(lCtrl, row, 0,
                                         :observer_lib.to_str(node))
                   :lists.member(node,
                                   selected) and :wxListCtrl.setItemState(lCtrl,
                                                                            row,
                                                                            65535,
                                                                            4)
                   row + 1
    ...
    :wx.foldl(fn mod, row ->
                   ...
                   rem(row, 2) === 0 and :wxListCtrl.setItemBackgroundColour(lCtrl,
                                                                               ...
                                                                               {230,
                                                                                  ...
                                                                                  250})
                   :wxListCtrl.setItem(lCtrl, row, 0,
                                         :observer_lib.to_str(mod))
                   mod === module and :wxListCtrl.setItemState(lCtrl, row,
                                                                 65535, 4)
                   row + 1
              end,
                0, mods)
  ...
  defp update_functions_view(funcs, lCtrl) do
    :wxListCtrl.deleteAllItems(lCtrl)
    :wx.foldl(fn r_tpattern(m: m, fa: fA, ms: r_match_spec(str: ms)), row ->
                   _Item = :wxListCtrl.insertItem(lCtrl, row, '')
                   rem(row, 2) === 0 and :wxListCtrl.setItemBackgroundColour(lCtrl,
                                                                               row,
                                                                               {230,
  ...
  defp merge_trace_items([n1 = r_titem(id: newP) | ns],
            [n2 = r_titem(id: newP) | old])
      when newP == :new_processes or newP == :new_ports or
             newP == :all do
  ...
  defp ttb_output_args(parent, opts) do
    ...
    fileOpts = output_file(toFile,
                             :proplists.get_value(:wrap, opts, false), opts)
  ...
  defp create_logwindow(parent, true) do
    scale = :observer_wx.get_scale()
    logWin = :wxFrame.new(parent, 380, 'Trace Log',
                            [{:size, {750 * scale, 800 * scale}}])
    mB = :wxMenuBar.new()
    file = :wxMenu.new()
    ...
    text = :wxTextCtrl.new(logWin, - 1,
                             [{:style, 32 ||| 32768 ||| 1073741824 ||| 16}])
    font = :observer_wx.get_attrib({:font, :fixed})
    attr = :wxTextAttr.new(:wxe_util.get_const(:wxBLACK),
                             [{:font, font}])
    true = :wxTextCtrl.setDefaultStyle(text, attr)
    :wxFrame.connect(logWin, :close_window, [{:skip, true}])
    :wxFrame.connect(logWin, :command_menu_selected,
                       [{:userData, text}])
    :wxFrame.show(logWin)
    {logWin, text}
  end
  ...
  defp setup_ttb(tPs, tPids, tPorts) do
    _R1 = (for {_, fTP} <- tPs do
             setup_tps(fTP, [])
           end)
    _R2 = (for r_titem(id: pid, opts: flags) <- tPids do
             :ttb.p(pid, dbg_flags(:proc, flags))
           end)
    _R3 = (for r_titem(id: port, opts: flags) <- tPorts do
  ...
  defp setup_tps([first = r_tpattern(fa: {f, _}) | rest],
            prev = [r_tpattern(fa: {f, _}) | _]) do
  ...
  defp textformat(trace) when (:erlang.element(1,
                                         trace) == :trace_ts and
                         tuple_size(trace) >= 4) do
    format_trace(trace, tuple_size(trace) - 1,
  ...
  defp get_config(r_state(def_proc_flags: procFlags,
              def_port_flags: portFlags, match_specs: matchSpecs0,
              tpatterns: tracePatterns, output: output)) do
    mSToList = fn r_match_spec(name: id, term: t, func: f) ->
                    [{:name, id}, {:term, t}, {:func, f}]
               end
    matchSpecs = (for {key, mSs} <- matchSpecs0 do
                    {:ms, key,
                       for mS <- mSs do
                         mSToList.(mS)
                       end}
    ...
    moduleTermList = (for {module,
                             fTPs} <- :dict.to_list(tracePatterns) do
                        {:tp, module,
                           for fTP <- fTPs do
                             tPToTuple.(fTP)
                           end}
                      end)
    [{:procflags, procFlags}, {:portflags, portFlags},
                                  {:match_specs, matchSpecs}, {:output, output},
                                                                  {:trace_p,
                                                                     moduleTermList}]
  end

  defp write_file(frame, filename, config) do
    str = ['%%% ', :epp.encoding_to_string(:utf8), '\n%%%\n%%% This file is generated by Observer\n', '%%%\n%%% DO NOT EDIT!\n%%%\n',
                                                     for mSTerm <- :proplists.get_value(:match_specs,
                                                                                          config) do
                                                       :io_lib.format('~tp.~n',
                                                                        [mSTerm])
                                                     end,
                                                         :io_lib.format('~p.~n',
                                                                          [:lists.keyfind(:procflags,
                                                                                            1,
                                                                                            config)]),
                                                             :io_lib.format('~p.~n',
                                                                              [:lists.keyfind(:portflags,
                                                                                                1,
                                                                                                config)]),
                                                                 :io_lib.format('~tp.~n',
                                                                                  [:lists.keyfind(:output,
                                                                                                    1,
                                                                                                    config)]),
                                                                     for moduleTerm <- :proplists.get_value(:trace_p,
                                                                                                              config) do
                                                                       :io_lib.format('~tp.~n',
                                                                                        [moduleTerm])
                                                                     end]
    case (:file.write_file(filename,
                             :unicode.characters_to_binary(str))) do
      :ok ->
        :success
      {:error, reason} ->
  ...
  defp read_settings(filename,
            r_state(match_specs: ms0, def_proc_flags: procFs0,
                def_port_flags: portFs0) = state) do
    case (:file.consult(filename)) do
      {:ok, terms} ->
        ms = parse_ms(terms, ms0)
        procFs1 = :proplists.get_value(:procflags, terms,
                                         []) ++ :proplists.get_value(:traceopts,
                                                                       terms,
                                                                       [])
        procFs = :lists.usort(procFs0 ++ procFs1)
        portFs = :lists.usort(portFs0 ++ :proplists.get_value(:portflags,
                                                                terms, []))
        out = :proplists.get_value(:output, terms, [])
        :lists.foldl(&parse_tp/2,
                       r_state(state, match_specs: ms,  def_proc_flags: procFs, 
                                  def_port_flags: portFs,  output: out),
                       terms)
      {:error, _} ->
        :observer_wx.create_txt_dialog(r_state(state, :panel), 'Could not load settings', 'Error',
                                         512)
        state
    end
  end
  ...
  defp parse_ms(terms, oldMSs) do
    mSs = (case (for {:ms, key, mSs} <- terms do
                   {key,
                      for mS <- mSs do
                        make_ms(mS)
                      end}
  ...
  defp make_ms(mS) do
    [{:func, funStr}, {:name, name}, {:term,
                                        term}] = :lists.keysort(1, mS)
    make_ms(name, term, funStr)
  end

  defp make_ms(name, term, funStr) do
    r_match_spec(name: name, term: term,
        str: :io_lib.format('~tw', [term]), func: funStr)
  end

  defp parse_tp({:tp, mod, fAs}, state) do
    patterns = (for {f, a, list} <- fAs do
