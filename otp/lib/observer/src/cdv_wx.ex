defmodule :m_cdv_wx do
  ...
  Record.defrecord(:r_state, :state, server: :undefined,
                                 file: :undefined, frame: :undefined,
                                 menubar: :undefined, menus: [],
                                 status_bar: :undefined, notebook: :undefined,
                                 main_panel: :undefined, gen_panel: :undefined,
                                 pro_panel: :undefined, port_panel: :undefined,
                                 ets_panel: :undefined, timer_panel: :undefined,
                                 sched_panel: :undefined, fun_panel: :undefined,
                                 atom_panel: :undefined, dist_panel: :undefined,
                                 mod_panel: :undefined, mem_panel: :undefined,
                                 persistent_panel: :undefined,
                                 int_panel: :undefined, active_tab: :undefined)
  def start(file) do
  ...
  def init(file0) do
    ...
    frame = :wxFrame.new(:wx.null(), - 1, 'Crashdump Viewer',
    ...
    case (load_dump(frame, file0)) do
      {:ok, file} ->
        ...
        setup(r_state(server: cdvServer, file: file, frame: frame,
                  status_bar: statusBar, notebook: notebook,
                  main_panel: panel))
      :error ->
  ...
  defp setup(r_state(frame: frame, notebook: notebook) = state) do
    ...
    genPanel = add_page(notebook, 'General', :cdv_info_wx,
                          :cdv_gen_cb)
    proPanel = add_page(notebook, 'Processes', :cdv_virtual_list_wx,
                          :cdv_proc_cb)
    portPanel = add_page(notebook, 'Ports', :cdv_virtual_list_wx,
                           :cdv_port_cb)
    etsPanel = add_page(notebook, 'ETS Tables', :cdv_virtual_list_wx,
                          :cdv_ets_cb)
    timerPanel = add_page(notebook, 'Timers', :cdv_virtual_list_wx,
                            :cdv_timer_cb)
    schedPanel = add_page(notebook, 'Schedulers', :cdv_virtual_list_wx,
                            :cdv_sched_cb)
    funPanel = add_page(notebook, 'Funs', :cdv_virtual_list_wx,
                          :cdv_fun_cb)
    atomPanel = add_page(notebook, 'Atoms', :cdv_virtual_list_wx,
                           :cdv_atom_cb)
    distPanel = add_page(notebook, 'Nodes', :cdv_virtual_list_wx,
                           :cdv_dist_cb)
    modPanel = add_page(notebook, 'Modules', :cdv_virtual_list_wx,
                          :cdv_mod_cb)
    memPanel = add_page(notebook, 'Memory', :cdv_multi_wx,
                          :cdv_mem_cb)
    persistentPanel = add_page(notebook, 'Persistent Terms', :cdv_html_wx,
                                 :cdv_persistent_cb)
    intPanel = add_page(notebook, 'Internal Tables', :cdv_multi_wx,
                          :cdv_int_tab_cb)
    :wxFrame.show(frame)
    ...
    {frame,
       r_state(state, menubar: menuBar,  gen_panel: genPanel, 
                  pro_panel: proPanel,  port_panel: portPanel, 
                  ets_panel: etsPanel,  timer_panel: timerPanel, 
                  sched_panel: schedPanel,  fun_panel: funPanel, 
                  atom_panel: atomPanel,  dist_panel: distPanel, 
                  mod_panel: modPanel,  mem_panel: memPanel, 
                  persistent_panel: persistentPanel, 
                  int_panel: intPanel,  active_tab: genPid)}
  end

  def handle_event(r_wx(event: r_wxBookCtrl(type: :command_notebook_page_changing)),
           r_state(active_tab: previous) = state) do
  ...
  def handle_event(r_wx(id: 5000,
             event: r_wxCommand(type: :command_menu_selected)),
           state) do
    newState = (case (load_dump(r_state(state, :frame),
                                  :undefined)) do
                  {:ok, file} ->
                    panels = [r_state(state, :gen_panel), r_state(state, :pro_panel),
  ...
  def handle_event(r_wx(id: 5006,
             event: r_wxCommand(type: :command_menu_selected)),
           state) do
    {:stop, :normal, state}
  end

  def handle_event(r_wx(id: helpId,
             event: r_wxCommand(type: :command_menu_selected)),
           state)
      when helpId == 5009 or helpId == 1 or helpId == 2 do
    help = get_help_doc(helpId)
    :wx_misc.launchDefaultBrowser(help) or create_txt_dialog(r_state(state, :frame),
                                                               'Could not launch browser: ~n ' ++ help, 'Error',
                                                               512)
    {:noreply, state}
  end

  def handle_event(r_wx(id: 5014,
             event: r_wxCommand(type: :command_menu_selected)),
           state = r_state(frame: frame)) do
    aboutString = 'Display information from an erlang crash dump'
    style = [{:style, 4 ||| 32768}, {:caption, 'About'}]
    :wxMessageDialog.showModal(:wxMessageDialog.new(frame,
                                                      aboutString, style))
    {:noreply, state}
  end
  ...
  defp get_active_pid(r_state(notebook: notebook, gen_panel: gen,
              pro_panel: pro, port_panel: ports, ets_panel: ets,
              timer_panel: timers, fun_panel: funs, atom_panel: atoms,
              dist_panel: dist, mod_panel: mods, mem_panel: mem,
              persistent_panel: persistent, int_panel: int,
              sched_panel: sched)) do
    panel = (case (check_page_title(notebook)) do
               'General' ->
                 gen
               'Processes' ->
                 pro
  ...
  defp do_load_dump(frame, fileName) do
    :ok = :observer_lib.display_progress_dialog(:wx.null(),
                                                  'Crashdump Viewer', 'Loading crashdump')
    ...
    case (:os.getenv('ERL_CRASH_DUMP_SECONDS') == '0' or :os.getenv('ERL_CRASH_DUMP_BYTES') == '0') do
      ...
      false ->
        ...
        case (:filename.absname(fileName)) do
          ^dumpName ->
            warning = 'WARNING: the current crashdump might be overwritten if the crashdump_viewer node crashes.\n\nRenaming the file before inspecting it will remove the problem.\n\nDo you want to continue?'
