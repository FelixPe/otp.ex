defmodule :m_cdv_wx do
  ...
  def init(file0) do
    ...
    frame =
      :wxFrame.new(:wx.null(), -1, ~c"Crashdump Viewer", [
  ...
  defp setup(r_state(frame: frame, notebook: notebook) = state) do
    ...
    genPanel = add_page(notebook, ~c"General", :cdv_info_wx, :cdv_gen_cb)
    proPanel = add_page(notebook, ~c"Processes", :cdv_virtual_list_wx, :cdv_proc_cb)
    portPanel = add_page(notebook, ~c"Ports", :cdv_virtual_list_wx, :cdv_port_cb)
    etsPanel = add_page(notebook, ~c"ETS Tables", :cdv_virtual_list_wx, :cdv_ets_cb)
    timerPanel = add_page(notebook, ~c"Timers", :cdv_virtual_list_wx, :cdv_timer_cb)
    schedPanel = add_page(notebook, ~c"Schedulers", :cdv_virtual_list_wx, :cdv_sched_cb)
    funPanel = add_page(notebook, ~c"Funs", :cdv_virtual_list_wx, :cdv_fun_cb)
    atomPanel = add_page(notebook, ~c"Atoms", :cdv_virtual_list_wx, :cdv_atom_cb)
    distPanel = add_page(notebook, ~c"Nodes", :cdv_virtual_list_wx, :cdv_dist_cb)
    modPanel = add_page(notebook, ~c"Modules", :cdv_virtual_list_wx, :cdv_mod_cb)
    memPanel = add_page(notebook, ~c"Memory", :cdv_multi_wx, :cdv_mem_cb)
    persistentPanel = add_page(notebook, ~c"Persistent Terms", :cdv_html_wx, :cdv_persistent_cb)
    intPanel = add_page(notebook, ~c"Internal Tables", :cdv_multi_wx, :cdv_int_tab_cb)
    :wxFrame.show(frame)
  ...
  def handle_event(
        r_wx(event: r_wxBookCtrl(type: :command_notebook_page_changing)),
  ...
  def handle_event(
    ...
    aboutString = ~c"Display information from an erlang crash dump"
  ...
  def handle_info({:EXIT, pid, _Reason}, state) do
    :io.format(~c"Child (~s) crashed exiting:  ~p ~tp~n", [pid2panel(pid, state), pid, _Reason])
    {:stop, :normal, state}
  end
  ...
  defp default_menus() do
    open = r_create_menu(id: 5000, text: ~c"Open new crash dump")
    quit = r_create_menu(id: 5006, text: ~c"Quit")
    about = r_create_menu(id: 5014, text: ~c"About")
    help = r_create_menu(id: 5009)
    uG = r_create_menu(id: 1, text: ~c"Crashdump viewer user's guide")
    howto = r_create_menu(id: 2, text: ~c"How to interpret crash dump")

    case :os.type() === {:unix, :darwin} do
      false ->
        fileMenu = {~c"File", [open, quit]}
        helpMenu = {~c"Help", [about, help, uG, howto]}
        [fileMenu, helpMenu]

      true ->
        [{~c"File", [open, about, quit]}, {~c"&Help", [help, uG, howto]}]
    end
  end
  ...
  defp do_load_dump(frame, fileName) do
    :ok =
      :observer_lib.display_progress_dialog(
        :wx.null(),
        ~c"Crashdump Viewer",
        ~c"Loading crashdump"
      )
  ...
  defp maybe_warn_filename(fileName) do
    case :os.getenv(~c"ERL_CRASH_DUMP_SECONDS") == ~c"0" or
           :os.getenv(~c"ERL_CRASH_DUMP_BYTES") == ~c"0" do
      true ->
        :continue

      false ->
        dumpName = :filename.absname(:os.getenv(~c"ERL_CRASH_DUMP", ~c"erl_crash.dump"))

        case :filename.absname(fileName) do
          ^dumpName ->
            warning =
              ~c"WARNING: the current crashdump might be overwritten if the crashdump_viewer node crashes.\n\nRenaming the file before inspecting it will remove the problem.\n\nDo you want to continue?"
  ...
  defp get_external_help_doc(2) do
    :filename.join(~c"http://www.erlang.org/doc/apps/erts", help_file(2))
  end

  defp get_external_help_doc(helpId) do
    :filename.join(~c"http://www.erlang.org/doc/apps/observer", help_file(helpId))
  end

  defp observer_doc_dir() do
    :filename.join([:code.lib_dir(:observer), ~c"doc", ~c"html"])
  end

  defp erts_doc_dir() do
    ertsVsn = :erlang.system_info(:version)
    rootDir = :code.root_dir()
    vsnErtsDir = :filename.join(rootDir, ~c"erts-" ++ ertsVsn)
    docDir = :filename.join([~c"doc", ~c"html"])
    ...
    case :filelib.is_dir(vsnErtsDir) do
      ...
      false ->
        :filename.join([rootDir, ~c"erts", docDir])
    end
  end

  defp help_file(5009) do
    ~c"crashdump_help.html"
  end
