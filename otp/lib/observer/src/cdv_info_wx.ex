defmodule :m_cdv_info_wx do
  ...
  Record.defrecord(:r_state, :state, panel: :undefined,
                                 sizer: :undefined, fpanel: :undefined,
                                 callback: :undefined, trunc_warn: [])
  ...
  def init([parentWin, callback]) when is_atom(callback) do
    ...
    {panel, sizer, fPanel} = create_box(parentWin,
                                          infoFields, info)
    {panel,
       r_state(panel: panel, sizer: sizer, fpanel: fPanel,
           callback: callback, trunc_warn: tW)}
  end
  ...
  def handle_call(:new_dump, _From,
           r_state(callback: callback, panel: panel, sizer: sizer,
               fpanel: fPanel) = state) do
    {infoFields, info, tW} = callback.get_info()
    newFPanel = :wx.batch(fn () ->
                               :wxWindow.destroy(fPanel)
                               fP = create_field_panel(panel, sizer, infoFields,
                                                         info)
                               :wxSizer.layout(sizer)
                               fP
                          end)
    {:reply, :ok,
       r_state(state, fpanel: newFPanel,  trunc_warn: tW)}
  end
  ...
  def handle_event(r_wx(event: r_wxMouse(type: :left_down), userData: target),
           state) do
  ...
  defp create_box(parentWin, infoFields, info) do
    ...
    fPanel = create_field_panel(panel, sizer, infoFields,
                                  info)
  ...
  defp create_field_panel(panel, sizer, infoFields, info0) do
    ...
    {fPanel, _FSizer,
       _Fields} = :observer_lib.display_info(panel, info)
    borderFlags = 16 ||| 32
