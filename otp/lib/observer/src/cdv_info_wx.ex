defmodule :m_cdv_info_wx do
  ...
  def handle_info(info, state) do
    :io.format(~c"~p:~p: Unhandled info: ~tp~n", [:cdv_info_wx, 68, info])
    {:noreply, state}
  end
  ...
  def handle_call(msg, _From, state) do
    :io.format(~c"~p~p: Unhandled Call ~tp~n", [:cdv_info_wx, 91, msg])
    {:reply, :ok, state}
  end

  def handle_cast(msg, state) do
    :io.format(~c"~p~p: Unhandled cast ~tp~n", [:cdv_info_wx, 95, msg])
    {:noreply, state}
  end
